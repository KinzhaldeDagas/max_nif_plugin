...
public:
	...
	enum Type 
	{
		...
		eScaleY = 0x200000,
		eScaleZ = 0x400000,
		eExposeWorldTransform = 0x800000,
		eLayer = 0x1000000
	};
...
private:
	...
	DllExport MSTR StripNodeFromName();
...
//! The class contains pointers to the XMLAnimTreeEntry objects and allocates and deletes these objects itself.
//! This class is contained within the NodeAndAnims object.
//! \see XMLAnimTreeEntry
//! \see NodeAndAnims
class XMLAnimTreeEntryList : public FPMixinInterface, private Tab<XMLAnimTreeEntry*>
{
	/*! \internal !*/
...
public:	
	...
	//! \return The index where the entry is, -1 if not found
	DllExport int Find(const MSTR &name, int unique);
...
private:
	...
	int FindNameFP(const MCHAR *name,int unique);
	...
	DllExport void SelectByWildCard(const MCHAR *buf);
...
{
public:
	NodeAndAnims():node(NULL),IKNodeName(NULL),treeEntryList(NULL){};
	...
	DllExport XMLAnimTreeEntryList *GetList()const {return treeEntryList;}
	
	//! \brief set the XMLAnimTreeEntry list
	//! \param[in] list The new XMLAnimTreeEntryList 
	...
	//! \brief Set the node
	//! \param[in] theNode The new INode 
	DllExport void SetNode(INode* theNode) { node = theNode; }

	//! \brief Gets the IKNodeName
	//! \return The IKNodeName (may be null)
	DllExport const MSTR *GetIKNodeName()const { return IKNodeName; }

	//! \brief Sets the IKNodeName
	DllExport void SetIKNodeName(const MCHAR* name);
	
	//! \brief Deletes the data held by the NodeAnim, in particular the list object and the IKNodeName. Must be called once the list is done being used.
...
private:
	...
	XMLAnimTreeEntryList *treeEntryList;
