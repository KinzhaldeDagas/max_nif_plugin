...
public:     
	...
	*/
	bool Contains(INode *node) const
	{
	...
	*/
	...
	\result true if the node was found and removed, false otherwise
	*/
...
/*! \brief A version of INodeTab that automatically calls INodeTab::DisposeTemporary()
		   when it gets destroyed. This is useful when placed on the stack in order to
		   call Interface::GetModContexts(ModContextList&, INodeTab&) safely. For example,
		   an INodeTab might contain INodeTransformed entries which are temporary and
		   need cleaning up.
*/
class INodeTabAutoDispose : public INodeTab
{
public:
	~INodeTabAutoDispose()
	{
