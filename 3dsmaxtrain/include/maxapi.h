...
*/
struct MaxRefEntryData
{
	MSTR nodeName;				//The name of the node
	ULONG nodeHandle;			//The Nodes unique ID
	ULONG parentNodeHandle;		//The Handle of the parent Node
	int nodeLevel;				//The nodes Position in the scene Hierarchy. 

	//Confirms that two MaxRefEntryData objects are identical and not that they are the same Node. Two seperate Max scenes may have unique nodes with identical MaxRefEntryDatas
	bool operator==(const MaxRefEntryData otherData)
	{
		return nodeHandle == otherData.nodeHandle && parentNodeHandle == otherData.parentNodeHandle &&
				nodeLevel == otherData.nodeLevel && nodeName == otherData.nodeName;
	}
};
...
#define MERGE_LIST_ALL    5    //!< Causes a list of objects to be placed into dataList parameter of Interface::MergeFromFile. No actual merging will take place. The mergeAll parameter of Interface::MergeFromFile must be TRUE.
...
#define MERGE_KEEP_SUBTREE		  3    //!< Automatically merge in Children of a node.
...
#define XREF_XREF_MODIFIERS		0  //!< Modifiers will be part of the xrefed object, i.e. modifiers won't be accessible in the main scene's modifier panel, but will be updated on subsequent updates of the xrefed objects
...
#define XREF_MERGE_MODIFIERS		(1<<2) //!< Modifiers will be merged, i.e modifiers will be accessible in the main scene's modifier panel, but won't get update on subsequent updates of the xrefed objects
...
#define XREF_MERGE_CHILDREN			(1<<11) //!< Merge in any children that current node is a parent of.
...
class NameMaker: public MaxHeapOperators 
{
	public:
		 ...
		 */
		virtual void AddName(const MSTR &name) = 0;
		 ...
		 */
		virtual BOOL NameExists(const MSTR& name) = 0;
		virtual ~NameMaker() = default;
};
...
class ViewExp13 : public ViewExp11
{
	public:
		...
		/*! \return Return true if it is using default lights.*/
		virtual bool IsUsingDefaultLights() const = 0;

		//! \brief Get whether the viewport uses one default light, or two.
		//! /*! \return Returns TRUE if the viewport uses one default light, FALSE if two. */
		virtual BOOL GetSingleDefaultLight() const = 0;

		/*! Get the default lights data and their number at the same time.
		* \param[in, out] pDefaultLights - The array of DefaultLight, it should be initialized before calling this method and it will be filled. 
		* It should have a maximum size of 2 as we use a maximum of 2 lights for a viewport.
		* \param[in] sizeOfDefaultLights - The size of the pDefaultLights array initialized before that call.
		* \param[in] bApplyGlobalLevel - Tells if we should we apply or not the global tint and level from the Render Environment on the lights data ?
		* \return Returns the number of DefaultLight we have filled in the array. It should be 0, 1 or 2. 0 meaning no default lights are used by this viewport.*/
...
//! \brief Extends ViewExp18
/*! The following code example shows how to get access to this interface:
\code
ViewExp22& viewExp22 = static_cast<ViewExp22&>(GetCOREInterface7()->getViewExp(0));
\endcode
*/
class ViewExp22 : public ViewExp18
{
public:
	/** Returns the picking selection radius scaling factor. This is used with the mouse cursor size to calculate the selection radius.
	* The selection radius scale is a global property.
	\return Returns the selection radius scaling factor.
	*/
	virtual float GetPickingSelectionRadiusScale() const = 0;

	/** Sets the picking selection radius scaling factor. This is used with the mouse cursor size to calculate the selection radius.
	* The selection radius scale is a global property.
	\param selectionRadiusScale - The new selection radius scale value. Clamped between [0.0001, 1000.0].
	*/
	virtual void SetPickingSelectionRadiusScale(float selectionRadiusScale) = 0;

	/** Returns the picking selection radius in pixels. The selection radius is a function of the cursor size (which scales with
	* the screen DPI) and the selection radius scale factor.
	* The selection radius scale is a global property.
	\return Returns the selection radius in pixels.
	*/
	virtual int GetPickingSelectionRadius() const = 0;

	/** Sets the picking selection pixel radius.
	* The selection radius scale is a global property.
	\param selectionRadius - The new selection radius value in pixels.
	*/
	virtual void SetPickingSelectionRadius(int selectionRadius) = 0;

	/**
    * Set this ViewExp in presentation mode on or off
    * The presentation mode works only on viewport in floating view panels.
	* It is a mode where there are no title bar, no grid, no view cube, no gizmos...
	* The view in presentation mode is always updated even if it is not the active view
    */
	...
	* It is a mode where there are no title bar, no grid, no view cube, no gizmos...
	* The view in presentation mode is always updated even if it is not the active view
    * \return Return true if it is in presentation mode.
    */
    virtual bool IsInPresentationMode() const = 0;
};

//! \brief Extends ViewExp22
/*! The following code example shows how to get access to this interface:
\code
ViewExp23& viewExp23 = static_cast<ViewExp23&>(GetCOREInterface7()->getViewExp(0));
\endcode
*/
class ViewExp23 : public ViewExp22
{
public:
    /** Get access to the IPerViewportFilter to know if an INode is filtered in a Viewport.
    */
    virtual MaxSDK::Graphics::IPerViewportFilter* GetIPerViewportFilter(void) = 0;
};
...
#define HIDE_NONRENDERABLES 0x0200
...
public:
	...
	virtual const MCHAR* GetName() = 0;
	/*! \brief This is the method that does the filtering of the node. It
	returns TRUE if the node is to be hidden; FALSE if it is not hidden.
	\par Parameters:
	<b>SClass_ID sid</b>\n\n
	The Super Class ID of the node.\n\n
	<b>Class_ID cid</b>\n\n
	...
	<b>INode *node</b>\n\n
	Points to the node to check. */
	virtual BOOL IsHidden(SClass_ID sid, Class_ID cid, INode *node) = 0;
...
<b>virtual void CreatePreview(PreviewParams *pvp=nullptr, MSTR *filename=nullptr, MSTR *snippet = nullptr, MAXScript::ScriptSource source  = MAXScript::ScriptSource::NotSpecified)=0;</b>
*/
...
public:
	...
	BOOL	dspCamViewName; //!< If TRUE cam/view name is shown in the preview; otherwise it is not shown.
	BOOL	autoPlay; //!< If TRUE the preview file is played automatically when done; otherwise it is not played automatically.
	// nitrous viewport settings
	int		vpPreset; //!< Specifies the viewport preset used: 0=highquality, 1=standard, 2=performance, 3=dxmode, 4=userdefined
	int		vpStyle; //!< Specifies the viewport visual style used: 0=defaultshading, 2=facets, 3=flatcolor, 4=hiddenline, 5=wireframe, 6=boundingbox, 7=ink, 8=colorink, 9=acrylic, 10=tech, 11=graphite, 12=colorpencil, 13=pastel,  14=clay, 15=modelassist
	BOOL	vpEdgedFaces; //!< If TRUE viewport edged faces mode is used; otherwise it is not used.
	BOOL	vpTextures; //!< If TRUE viewport textures are shown in the preview; otherwise they are not shown.
};
...
//! @{
		...
		To represent progress of a task that can't be cancelled, use other version of ProgressStart().
		When progress bar is up, user can't interact with any UI except cancel button.
		This method must be called from the main thread.

		Note: It is not possible to use the progress bar APIs in the create or modify branches of the command panel.
		In the create or modify branch of the command panel, EndEditParams() gets called from this method.
		This is because most of MAX is "shut down" during a progress operation.
		For example, it is not appropriate for users to be moving objects during an IK calculation,
		or changing the camera lens while a preview is being created (all these operations use ProgressStart() API).
		It is for this same reason that Transform Type-in, Medit, Track View and Video Post windows
		are hidden during a progress operation. Since EndEditParams() is called, the Interface pointer a plug-in
		maintains is no longer valid after the operation is started.
		This method is useful for Utility plug-ins, and Import/Export plug-ins however.

		\param taskName : Identifies the task currently in progress. This is visible for the duration of the task.
		\param dispBar : Unused parameter.
		\param fn : Pointer to a function with this signature: `DWORD WINAPI fn(LPVOID arg)`.
		Pointer can be null. This function is always executed in the main thread.
		If not null, this function is called with argument `arg` passed at the end of ProgressStart().
		\param arg : Argument passed to function `fn`. Can be null
		\return `true` when progress mode was entered successfully, `false` otherwise

		\par Sample Code:
		This code demonstrates the use of ProgressStart(), ProgressEnd(), GetCancel() and SetCancel() APIs.
		There are two ways to use the APIs:
		1. Pass a null function in ProgressStart(). Then perform work and call ProgressUpdate() from client code
		\code
		void Utility::Test1()
		{
			int percent, retval;
			Interface *ip = theUtility.ip;
			ip->ProgressStart(_M("My task"), true, nullptr, nullptr);
			for (int i = 0; i < 1000; i++)
			{
				percent = i/10;
				ip->ProgressUpdate(percent);
				if (ip->GetCancel()) {
					retval = MaxSDK::MaxMessageBox(ip->GetMAXHWnd(), _M("Really Cancel?"), _M("Question"), MB_ICONQUESTION | MB_YESNO);
					if (retval == IDYES)
						break;
					else if (retval == IDNO)
						ip->SetCancel(false);
		...
		2. Use `fn` to perform work. Call ProgressUpdate() in `fn`.
		\code
		DWORD WINAPI fn(LPVOID arg)
		{
			...
			for (int i = 0; i < 1000; i++)
			{
				...
				if (ip->GetCancel())
				{
					switch(MaxSDK::MaxMessageBox(ip->GetMAXHWnd(), _M("Really Cancel"), _M("Question"), MB_ICONQUESTION | MB_YESNO))
					{
						case IDYES:
							return 0;
						case IDNO:
		...
		void Utility::Test1()
		{
			Interface *ip = theUtility.ip;
			LPVOID arg;
			ip->ProgressStart(_M("My task"), true, fn, arg);
			ip->ProgressEnd();
		}
		\endcode
		\see ProgressUpdate, ProgressEnd, SetCancel, GetCancel
		*/
		...
		This uses the same logic as passing nullptr to `fn` and `arg` in other version
		Use this version with `allowCancel = false` to show progress of tasks that can't be cancelled.

		\param taskName : Identifies the task currently in progress. This is visible for the duration of the task.
		\param allowCancel : If `true`, cancel button appears next to progress bar.
		Hit cancel button or press ESC key to set cancel status to true
		If `false`, no cancel button appears and ESC key presses are ignored.
		\return `true` when progress mode was entered successfully, `false` otherwise.
		\see ProgressUpdate, ProgressEnd, SetCancel, GetCancel
		*/
		...
		After ProgressStart() is called, this method should be called periodically to report progress.
		\param pct : Percentage completed (0 to 100). This causes the progress bar to move.
		\param showPct : If `true`, percentage appears next to progress bar.
		\param stepName : Name of the current step of the operation. This is displayed next to progress bar.
		For example, this might change from "Extruding" to "Capping" to "Welding".
		\see ProgressStart, ProgressEnd, SetCancel, GetCancel
		*/
...
//! @{		
		...
		<b>MSTR *snippet</b>\n\n
		The MXS code snippet to execute on each frame.  The returned string from snippet is displayed in the rendering
		output. If this value is NULL, the setting in the preview rendering dialog is used.\n\n
		<b>MAXScript::ScriptSource scriptSource</b>\n\n If snippet is specified, the source of the snippet string. */
		virtual void CreatePreview(PreviewParams* pvp = nullptr, MSTR* filename = nullptr, MSTR* snippet = nullptr,
								   MAXScript::ScriptSource scriptSource = MAXScript::ScriptSource::NotSpecified) = 0;
//! @}
...
//! @{
		...
		might want to snap something to the angle snap value even if the primary
		toggle was off.\n\n
		...
		If TRUE snapping is used even if the primary angle snap toggle is off.
...
//! \name Preserve Stack Selection Related Methods
		//! @{
		// get/set preserve stack selection.
		/*! \remarks Returns TRUE if the Preserve Stack Selection button is on (pressed); otherwise
		FALSE. */
		virtual BOOL GetPreserveStackPosition() = 0;
		/*! \remarks Sets the on/off (pressed/unpressed) state of the Preserve Stack Selection
		button.
		\par Parameters:
		<b>BOOL show</b>\n\n
		TRUE for on; FALSE for off. */
...
//! @{		
		...
		// INode::SetNodeTM(TimeValue t, const Matrix3& tm). If the node
		...
		to the node via INode::SetNodeTM(TimeValue t, const Matrix3\& tm). If the
...
//! @{
	...
	public:
		...
		class id, and super class id pass the filter check or not.
		\par Parameters:
		<b>int index</b>\n\n
		The index of the filter\n\n
		<b>int sid</b>\n\n
...
//! @{        
		...
		/*! \remarks Returns a pointer to the main manager for customizable menus. See Class MaxSDK::CUI::ICuiMenuManager. */
		virtual MaxSDK::CUI::ICuiMenuManager* GetICuiMenuManager() = 0;
		/*! \remarks Returns a pointer to the main manager for customizable quad menus. See Class MaxSDK::CUI::ICuiQuadMenuManager. */
		virtual MaxSDK::CUI::ICuiQuadMenuManager* GetICuiQuadMenuManager() = 0;
//! @}
...
//! @}
		...
		//! \deprecated This has been deprecated as of 3ds Max 2026.
		MAX_DEPRECATED virtual void* GetRightClickMenuManager() final { return nullptr; };
...
//! @{
		...
		\param [in] overrideDriven If TRUE, this method will delete nodes whose
		...
		scene. This parameter allows a driver controller to easily delete driven nodes
		if it needs to.
...
//! @{		
		...
		virtual MSTR GetDir(int which)=0;		// which = APP_XXX_DIR
		...
		virtual MSTR GetPlugInDesc(int i)=0;	// ith description
		...
		virtual MSTR GetPlugInDir(int i)=0;	// ith directory
//! @}
		...
		virtual MSTR GetAssetDir(int i, MaxSDK::AssetManagement::AssetType assetType)=0;		// i'th assetType dir of path
		...
		virtual MSTR GetSessionAssetDir(int i, MaxSDK::AssetManagement::AssetType assetType)=0;
		...
		virtual MSTR GetCurAssetDir(int i, MaxSDK::AssetManagement::AssetType assetType)=0;
...
//! @{
		...
		virtual MSTR GetMatLibFileName()=0;
//! @}
...
//! @{		
		...
		The confirmation prompt is not shown if running in quiet mode.
		\par Parameters:
		...
		virtual int MergeFromFile(const MCHAR *name, 
				...
				MaxSDK::Array<MaxRefEntryData>* dataList = nullptr // additional data for nodes that we wish to merge (handle,parentHandle,nodeLevel)
				)=0;
//! @}
...
//! @{
		...
		/*! \remarks This method is used to specify the background asset used for all viewports.
		\par Parameters:
		<b>AssetUser& asset</b>\n\n
...
//! @{		
		...
		*/
		...
		/*! \remarks Retrieves the renderer's pixel aspect ratio setting. 
		Note : to get the 'Image Aspect Ratio' setting use:\n\n
		<b>GetRendImageAspect()</b>*/
...
//! @{		
		...
		light.		
		\param t - The time to retrieve the ambient light color.
		\param valid - The validity interval to update.  
		The intersection of the provided "valid" interval with the 
		interval for the provided TimeValue and returned as the updated interval value.
		\return  The color as a <b>Point3</b>. */
		virtual Point3 GetAmbient(TimeValue t, Interval &valid) = 0;
		/*! \remarks Use this override when validity interval is irrelevant */
		Point3 GetAmbient(TimeValue t, Interval&& valid = FOREVER) { return GetAmbient(t, valid); }
		...
		*/
		...
		/*! \remarks Use this override when validity interval is irrelevant */
		Point3 GetLightTint(TimeValue t, Interval&& valid = FOREVER) { return GetLightTint(t, valid); }
		...
		\param t - The time at which to return the level.
		\param valid - The validity interval to update.  
		The intersection of the provided "valid" interval with the 
		interval for the provided TimeValue and returned as the updated interval value. */
		virtual float GetLightLevel(TimeValue t, Interval &valid) = 0;
		/*! \remarks Use this override when validity interval is irrelevant */
		float GetLightLevel(TimeValue t, Interval&& valid = FOREVER) { return GetLightLevel(t, valid); }
		...
		*/
		...
		color.
		\param t - The time to retrieve the color.
		\param valid - The validity interval to update.  
		The intersection of the provided "valid" interval with the 
		interval for the provided TimeValue and returned as the updated interval value. */
		virtual Point3 GetBackGround(TimeValue t, Interval &valid) = 0;
		/*! \remarks Use this override when validity interval is irrelevant */
		Point3 GetBackGround(TimeValue t, Interval&& valid = FOREVER) { return GetBackGround(t, valid); }
...
//! @{
		...
		optionally the current selection set, or optionally a specified set of nodes.
		\par Parameters:
		<b>float f</b>\n\n
		The scale factor to apply to the scene.\n\n
		<b>BOOL selected</b>\n\n
		TRUE to scale selected objects only; otherwise the entire scene or the specified set of 
		nodes is scaled.
		<b>Tab<INode*>* nodes</b>\n\n 
		If selected is FALSE, the set of nodes to scale; otherwise the entire scene is scaled.*/
		virtual void RescaleWorldUnits(float f, BOOL selected, Tab<INode*>* nodes = nullptr) = 0;
		//! @}
...
//! \brief Values supported by function Interface7::DoUICustomization
enum CUIDialogPage : int
{
	CUI_PAGE_PREVIOUS = -1, //!< Open "Customize User Interface" dialog on the previously selected tab
	CUI_PAGE_MOUSE = 0,     //!< Open "Customize User Interface" dialog on the "Mouse" tab
	CUI_PAGE_TOOLBARS = 1,  //!< Open "Customize User Interface" dialog on the "Toolbars" tab
	CUI_PAGE_COLORS = 3,    //!< Open "Customize User Interface" dialog on the "Colors" tab
	CUI_PAGE_HOTKEYS = 4,   //!< Open the Hotkey Editor
	CUI_PAGE_MENUS = 5,     //!< Open the Menu Editor on the "Menus" tab
	CUI_PAGE_QUADS = 6,     //!< Open the Menu Editor on the "Quads" tab
};
...
public:
	...
	//! \brief Returns INode that has specified object as base object or modifier
	/*! \param[in] obj The object to search using.
		\param[in] allowXRefNodes If true, will return INode even if under scene xref root node.
		\param[in] allowNonSceneNodes If true, will return INode even if not found under scene root node or (if 
		applicable) under scene xref root node.
		\param[in] allowScriptedPlugins If true, if obj is the delegate of a scripted plugin, will continue search using the scripted plugin.
		\param[in] allowXrefObjects If true, if obj is wrapped by an XRef Object, will continue search using the XRef Object.
		...
		\return The first INode found matching requirements. */
	...
	//! \brief Returns Tab<INode*> which have specified object as base object or modifier
	/*! \param[in] obj The object to search using.
		\param[in] allowXRefNodes If true, will return INode even if under scene xref root node.
		\param[in] allowNonSceneNodes If true, will return INode even if not found under scene root node or (if applicable) under scene xref root node.
		\param[in] allowScriptedPlugins If true, if obj is the delegate of a scripted plugin, will continue search using the scripted plugin.
		\param[in] allowXrefObjects If true, if obj is wrapped by an XRef Object, will continue search using the XRef Object.
		\param[in] handleBranching If true, if obj is a branch of an object (such as Boolean), will continue search using the object.
		\return The INodes found matching requirements. */
	virtual Tab<INode*> FindNodesFromBaseObject(ReferenceTarget* obj, bool allowXRefNodes = true,
			bool allowNonSceneNodes = true, bool allowScriptedPlugins = false, bool allowXrefObjects = false,
			bool handleBranching = false) = 0;
	...
	virtual RenderUIType GetRenderType() = 0;
	virtual void SetRenderType(RenderUIType rtype) = 0;
	...
	virtual BOOL ASnapStatus() = 0;
	virtual void TogglePSnap() = 0;
	virtual BOOL PSnapStatus() = 0;
	virtual float GetGridSpacing() = 0;
	...
	virtual void DoUICustomization(CUIDialogPage pageId) = 0;
...
public:
	...
	*/
	...
	/*! \brief Returns true if a window is currently registered with RegisterModelessRenderWindow().
	* See the documentation of RegisterModelessRenderWindow() or UnRegisterModelessRenderWindow()
	* for more details.
	* \param[in] hWnd Handle to the window for which the information is desired.
	*/
	...
	*/
	enum LoadFromFileFlags {
		...
		*/
		...
		/*! When this flag is set, all the xrefs in the scene are not loaded on the initial update pass*/
		kSkipXRefs = 0x10
	};
...
public:
	...
	\param [in] overrideDrivenTM If true, this method will delete nodes whose 
	TM controllers implement Control::PreventNodeDeletion() to return TRUE. 
	Control::PreventNodeDeletion() controls whether the node can be deleted from 
	the scene. This parameter allows a driver controller to easily delete driven nodes 
	if it needs to.
	\return The number of nodes that were deleted. 
	*/
...
public:
	...
	* Currently it supports saving to the current version of 3ds Max (MAX_RELEASE) and previous
	* three versions. A value of 0 is considered same as MAX_RELEASE.
	...
	*/
	virtual bool SaveSceneAsVersion(
		...
		unsigned long saveAsVersion = 0) = 0; 
...
public:
	...
	*/
	...
	\param[in] play TRUE to enable the play when done option, FALSE to disable */
	...
	* (pre-3ds Max 2013) and for writing non-scene text or binary files where conversion from 
	* wide characters to mbcs characters is required.
	...
	*/
	...
	// Render a 2D bitmap from a procedural texture
	/*! \remarks	This method renders a texmap (or an entire texmap tree) to the specified bitmap.
	\par Parameters:
	<b>Texmap *tex</b>\n\n
	The Texmap to render to a bitmap.\n\n
	<b>Bitmap *bm</b>\n\n
...
public:
	...
	*/
	...
	//! \brief Sets the Rendering Format to Custom 
	virtual void SetRendFormatToCustom() = 0;

	/*! \brief Brings up the standard Save File As dialog box to
	allow the user to save the specified material library.
	\param hWnd The parent window handle.
	\param lib The material library to save. If null, the user material library is saved.
	\param seedName The seed file name.
	\param savedFileName If not null and the save is successful, set to the filename saved to.
	*/
	virtual void FileSaveMatLib(HWND hWnd, MtlBaseLib* lib, const MCHAR* seedName = nullptr,
			MSTR* savedFileName = nullptr) = 0;

	/*! \brief  Fills the FilterList with the entries for saving a Material Library. The entries will include those for
	saving to a previous version of 3ds Max.
	\param filterList The FilterList to populate.
	\par Sample Code:
	\code
	OPENFILENAME ofn;
	memset(&ofn, 0, sizeof(OPENFILENAME));
	FilterList filterList;
	GetCOREInterface17()->GetSaveMatLibFilterList(filterList);
	ofn.lpstrFilter = filterList;
	\endcode
	*/
	virtual void GetSaveMatLibFilterList(FilterList& filterList) = 0;

	/*! \brief  Determines the Save As version based on the contents of the OPENFILENAME.
	\param ofn The OPENFILENAME to determine Save As version based on.
	\return The save as version release number.
	\par Sample Code:
	\code
	TSTR fname;
	ofn.lpstrFile = fname.dataForWrite(MAX_PATH);
	ofn.nMaxFile = MAX_PATH;
	if (GetSaveFileName(&ofn))
	{
		// ensure extension is ".mat"
		TSTR ext;
		SplitFilename(fname, nullptr, nullptr, &ext);
		ext.toLower();
		if (ext != _T(".mat"))
			fname.append(_T(".mat"));

		saveAsVersion = GetCOREInterface17()->GetSaveAsVersionFromOFN(ofn);
		GetCOREInterface17()->SaveMaterialLib(fname, m_matlib, saveAsVersion))
	}
	\endcode
	*/
	virtual int GetSaveAsVersionFromOFN(const OPENFILENAME& ofn) = 0;

	/*! \brief Saves the specified material library to the specified
	file. This method does not bring up a file dialog.
	\param name The file name to save to.
	\param lib The material library to save. If NULL the current material library is saved. 
	\param saveAsVersion The 3ds Max version release number (for example, MAX_RELEASE_R24 for 3ds Max 2022) to save as,
	0 means current version.
	\return True if the library was saved; otherwise 0. 
	*/
	virtual bool SaveMaterialLib(const MCHAR* name, MtlBaseLib* lib, int saveAsVersion) = 0;

	/*! \brief Returns whether can save to the specified 3ds Max version. 3ds Max supports saving to previous 3 versions.
	\param saveAsVersion The 3ds Max version release number (for example, MAX_RELEASE_R24 for 3ds Max 2022) to save as,
	0 means current version.
	\return True if can save to the specified 3ds Max version.
	*/
	virtual bool IsValidSaveAsVersion(int saveAsVersion) = 0;

	/*! \brief When the viewport statistics are enabled (polygons/triangles/Vertices/FPS etc), you may want to stop refreshing them since you are adding multiple nodes to the scene, such as during an import operation,
	and there is no point in refreshing the viewport statistics after each node is added to the scene. Another case is when you are adding multiple viewport RenderItems from a IObjectDisplay2::UpdatePerNodeItems() call.
	Note : calling EnableViewportStatisticsRefresh(true) will also trigger a complete refresh of the viewport statistics, same as calling RefreshViewportStatistics() below.
			If you do not want a refresh when enabling the statistics, please consider using EnableViewportStatistics(bool enable);
	\param enable : if true, the refresh is enabled, if false it is disabled.
	*/
	virtual void EnableViewportStatisticsRefresh(bool enable) = 0;

	/*! \brief When the viewport statistics are enabled (polygons/triangles/Vertices/FPS etc), this method forces a refresh. 
	*/
	virtual void RefreshViewportStatistics() = 0;

	/*! \brief When the viewport statistics are enabled (polygons/triangles/Vertices/FPS etc), you may want to stop refreshing them since you are adding multiple nodes to the scene, such as during an import operation,
	and there is no point in refreshing the viewport statistics after each node is added to the scene. Another case is when you are adding multiple viewport RenderItems from a IObjectDisplay2::UpdatePerNodeItems() call.
	Note : this will enable/disable the statistics, but will never refresh the viewport statistics, if you want to do so, you need to call RefreshViewportStatistics().
	\param enable : if true, the refresh is enabled, if false it is disabled.
	*/
	virtual void EnableViewportStatistics(bool enable) = 0;

	/*! \brief Globally rescales all key frames in one specified interval to another specified interval. This method corresponds to the Re-scale Time dialog accessed from the
	Time Configuration dialog.
	\param oldSegment The interval to scale from
	\param newSegment The interval to scale to
	\param modifyTimeline If true, current animation time line is set to newSegment
	\param scaleToWholeFrames If true, rescaled key frame times are moved to the nearest frame (rounding up)
	*/
	virtual void GlobalScaleTime(Interval oldSegment, Interval newSegment, BOOL modifyTimeline, BOOL scaleToWholeFrames) = 0;

	/*! \brief Whether the specified node is a regular modifiable node in the current scene.
	\param node The node to check. 
	\return true if the specified node is a regular modifiable node in the current scene.
	*/
	virtual BOOL IsSceneNode(INode* node) = 0;

	/*! \brief Returns true if the specified screen coordinate is over a viewport.
	\param[in] pt The screen coordinate.
	\param[out] panelIndex The viewport panel index.
	\param[out] vptIndex The viewport index.
	\param[out] vptID The viewport id.
	\param[out] ptLocal The coordinate within the viewport.
	\param[out] hWnd The viewport HWND.
	\param[out] node If not null, the topmost node at the screen coordinate
	\return true if the specified screen coordinate is over a viewport.
	*/
	virtual bool GetViewportFromScreenCoord(const POINT& pt, int& panelIndex, int& vptIndex, int& vptID, POINT& ptLocal,
			HWND& hWnd, INode** node) const = 0;

	/*! \brief Invalidate all pages in Grid and Snap Settings dialog.
	 */
	virtual void UpdateOsnapDlg() const = 0;

	/*! \brief Enable/Disable Angle Snap. 
	\param enable : if true, Angle Snap is enabled, if false it is disabled.
	*/
	virtual void SetASnapStatus(BOOL enable) = 0;
	/*! \brief Enable/Disable Percent Snap.
	\param enable : if true, Percent Snap is enabled, if false it is disabled.
	*/
	virtual void SetPSnapStatus(BOOL enable) = 0;

	//! \brief Returns the User Name stored in scene files. Defaults to "".
	virtual MSTR GetSceneFileUserName() const = 0;
	/*! \brief Sets the User Name stored in scene files
	\param [in] name - the User Name
	\param [in] persist - whether to persist setting across 3ds Max sessions
	*/
	virtual void SetSceneFileUserName(const MSTR& name, bool persist) = 0;
	//! \brief Returns the Computer Name stored in scene files. Defaults to "".
	virtual MSTR GetSceneFileComputerName() const = 0;
	/*! \brief Sets the Computer Name stored in scene files
	\param [in] name - the Computer Name
	\param [in] persist - whether to persist setting across 3ds Max sessions
	*/
...
class [[nodiscard]] ScopedPrePostRefNotifyDependents : public MaxHeapOperators
{
private:
