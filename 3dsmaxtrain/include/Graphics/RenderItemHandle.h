...
*/
enum RenderItemVisibilityGroup : int
{
...
public:
	...
	*/
	...
	/** Set if the item should be frustum culled before being drawn. For this to happen correctly, you also need to set the local bounding box of the item.
	* Default is off, no frustum culling on the render item, as the frustum culling is made on the render node.
	\param[in] onOff if onOff is true, we will perform frustum culling.
	*/
	GraphicsDriverAPI void SetFrustumCulling(bool onOff);

	/** returns if the item will be be frustum culled before being drawn.
	\return item frustum culling boolean value
	*/
	GraphicsDriverAPI bool GetFrustumCulling() const;

	/** Set the item local bounding box, is used for example for frustum culling.
	\param[in] localBBox the bounding box of the item in local coordinates.
	*/
	GraphicsDriverAPI void SetObjectBox(const Box3& localBBox);

	/** returns the item local bounding box.
	\return item bounding box in local coordinates.
	*/
	GraphicsDriverAPI const Box3& GetObjectBox() const;
};
...
\remarks The input matrix array should contain object-to-world matrices. 
		...
		by GenerateInstanceRenderItem(). Multiple instance render items can share the same 
		VertexBufferHandle.
...
GraphicsDriverAPI bool GenerateInstanceRenderItem(
	...
	This is an extension of the bool GenerateInstanceRenderItem(RenderItemHandle& hInstanceRenderItem, const RenderItemHandle& hTupleMeshHandle, const VertexBufferHandle& hInstanceData) 
	function with more overridable data for instances. The previous method could only
	override the transform matrix in world space and UV from the map channel #1.
	The MaxSDK::Graphics::ViewportInstancing::InstanceDisplayGeometry class lets you override more data per instance. For more details, please 
	see the header file maxsdk\include\Graphics\InstanceDisplayGeometry.h.
	\param hInstanceRenderItem the instance render item handle which can replace original tuple mesh render item.
	\param hTupleMeshHandle handle of a tuple mesh render item
	\param pInstanceDisplayGeometry : an InstanceDisplayGeometry* 
	\return true if successful created the instance render item.
*/
	GraphicsDriverAPI bool GenerateInstanceRenderItem(
	RenderItemHandle& hInstanceRenderItem,
