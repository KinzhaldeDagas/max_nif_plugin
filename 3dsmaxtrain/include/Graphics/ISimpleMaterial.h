...
#define ISIMPLE_MATERIAL_EXT_INTERFACE_ID Interface_ID(0x3e465420, 0x454f05ba)
	...
	namespace Graphics
	{
		...
		*/
		...
		public:
			...
			*/
			virtual const AColor& GetBorderColor(MapUsage usage) const = 0;
		};

		/** ISimpleMaterialExt extends ISimpleMaterial to support more textures.
		To distinguish from a ISimpleMaterial texture, a texture here is called a 'stage texture'. It is like D3D SetTextureStageState.
		To access the property, use the following code 
		\code
		...
		\endcode
		*/
		class ISimpleMaterialExt : public ISimpleMaterial
		{
		public:
			/** Stage texture operation type.*/
			enum TextureOperation
			{
				TextureOperationDisable,				/*!< Disables output from this texture stage */
				TextureOperationModulate,				/*!< Multiply the components of the texure stage. */
				TextureOperationBlendTextureAlpha,		/*!< Linearly blend this texture stage, using the alpha from this stage's texture. */
			};
			...
			*/
			...
			/** Set number of texture stages. \sa ITextureDisplay::SetupTextures()
			\param[in] numberStage number of stage. 
			\return True if successful
			*/
			virtual bool SetTextureStageCount(unsigned int numberStage) = 0;

			/** Returns the number of texture stages.
			\return the number of texture stages.
			*/
			virtual unsigned int GetTextureStageCount() const = 0;

			/** Set stage texture with texture handle. \sa ITextureDisplay::SetupTextures()
			\param[in] stage The stage texture index.
			\param[in] pTexture A texture handle. If pTexture is NULL it will clear the texture.  This can 
			give finer control than using ClearTextures()
			\return True if successful 
			*/
			...
			/** Set stage texture color operation. \sa ITextureDisplay::SetupTextures()
			\param[in] stage The stage texture index.
			\param[in] colorOp color operation.
			\return True if successful 
			*/
