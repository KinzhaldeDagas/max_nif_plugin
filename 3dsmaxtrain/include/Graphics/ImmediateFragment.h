...
public:
	...
	*/
	...
	/** Indicates this fragment if using one depth-stencil buffer.The buffer will not be created internally
		when the fragment is initialized.
		/param[in] bUse this fragment will use one depth-stencil buffer or not.
	*/
	GraphicsDriverAPI virtual void SetUseDepthTarget(bool bUse);

	/** Set current world matrix.
		/param matrix new world matrix.
	*/
	GraphicsDriverAPI virtual void SetWorldMatrix(const Matrix44& matrix);
};
