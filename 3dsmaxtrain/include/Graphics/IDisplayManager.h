...
// The rendering target format used in viewport color pipeline
enum class ColorPipelineTargetFormat : uint8_t
{
	kFormat_FLOAT_32,	// 32 bits float per component, the default format used in viewport color pipeline.
	kFormat_FLOAT_16,	// 16 bits float per component, use this for saving GPU memory with lower color fidelity in viewport color pipeline.
};
...
{
public: 
	/*!Plug-ins that have a mesh representation and use Mesh::render() in their implementation of BaseObject::Display() 
	will need to update their display logic in order to appear correctly in the viewport when the retained mode graphics 
	driver is active.  If a textured lit mesh is required and IsRetainedModeEnabled() returns true, the calls to Mesh::render() 
	...
	*/
	...
	/** Get if the viewport color pipeline is OpenColorIO based
	\return true if the viewport color pipeline is OpenColorIO based, false otherwise
	*/
	virtual bool IsViewportColorPipelineOCIOBased() const = 0;

	/** Get the rendering target format used in viewport color pipeline
	\return one of ColorPipelineTargetFormat enumeration
	*/
	virtual ColorPipelineTargetFormat GetViewportColorPipelineTargetFormat() const = 0;

	/** Set the rendering target format used in viewport color pipeline
	*/
	virtual void SetViewportColorPipelineTargetFormat(ColorPipelineTargetFormat format) = 0;
};
...
inline bool IsRetainedModeEnabled()
{
	...
	return nullptr != pGraphicsManager && pGraphicsManager->IsRetainedModeEnabled();
}
...
{
	IDisplayManager* pGraphicsManager = GetIDisplayManager();
	return nullptr != pGraphicsManager && pGraphicsManager->IsHardwareHitTesting(vpt);
}
