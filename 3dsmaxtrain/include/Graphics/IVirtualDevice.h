	...
	*/
	...
	public:
		...
		*/
		...
		\param[in] offsetArray offset of each vertex buffer, must have same size as the buffer
		*/
		...
		*/
		...
		\param[in] offsetArray offset of each vertex buffer, must have same size as the buffer. 
		Specify -1 to append to last stream out position.
		*/
		...
		*/
		...
		\param[in] targets render targets array
		\param[in] zBuffer z buffer, can be nullptr
		\param[in] uavs UAV targets
		\param[in] initialUAVCounts Initial count array of the UAV. The array should be same size as uavs.
		*/
		virtual bool SetTargetsAndUnorderedAccessViews(TargetHandleArray targets,
			TargetHandle& zBuffer,
			VertexBufferHandleArray uavs,
			unsigned int* initialUAVCounts) = 0;

		/** Clear a Unordered Access View target with initial value.
		This API wraps DX11 API, it works on FLOAT, UNORM, and SNORM unordered access views (UAVs), 
		with format conversion from FLOAT to *NORM where appropriate.
		\param[in] buffer uav buffer to clear
		\param[in] vals 4 unsigned int values to copy to corresponding UAV's RGBA channels.
		*/
