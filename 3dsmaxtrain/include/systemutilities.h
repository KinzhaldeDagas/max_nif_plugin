...
namespace Util
{
	//! \brief Sets the locale for a DLL or thread to match that of the language pack being used at 3ds Max start up.
	/*! This method must be called as soon as possible in the start up sequence of any DLL that may use
		...
		As for threads, this method must be called as early as possible in the routine indicated by _beginthread(),
		_beginthreadex(), or CreateThread()'s start address parameter.

		Calling this method is required in the cases above in order that 3ds Max loads files and Win32 resources from a
		satellite DLL installed by a language pack that may not match the locale of the OS. Otherwise, the product might 
		run with localized resources retrieved from different satellite DLL sets, resulting in a mix of different languages.

		However, if localized resources are not handled by the binary in question, or the binary is unilingual
		(i.e. usually that means 'English only'), then this call is probably not required but should still be made for
		logging purposes.  This call is meant to be used with Microsoft's MUI technology, but not exclusively.

		This method complements the other locale and language retrieval functions in the MaxSDK.

		\param hMyModule - The handle of the dll being loaded. This is used internally for tracking the loading of dlls 
		for debugging purposes.  This parameter should not be specified when called by DllMain() as its default argument
		evaluates to the dll's handle. When calling from a thread proc, a parameter value of 'nullptr' should be specified 
		so that the call is not logged as a dll load.
	*/
...
/*! This function is used to convert a path relative to the 3ds max install to an absolute path. 
	...
	\return true if \p specPath was successfully resolved, false otherwise.
	*/
...
/*! This function is a helper for creating standard windows CSIDL directories
	...
	\return true if \p csidlPath was successfully created, false otherwise.
	*/
...
namespace Util
{
	/* \brief scans a parent folder and all sub-folders for a file
	   ...
	   \return true if filename is successfully found, false otherwise
	*/
	...
	/* \brief Captures current call stack to MSTR
	   ...
	   \return a MSTR containing the captured call stack. Each line contains one stack level.
	*/
	...
	*/
	...
	/* \brief Return whether a directory is writable
	   \param[in] dirName the directory name
	   \return true if directory exists and is writable
	*/
	UtilExport bool IsDirectoryWritable(const MCHAR* dirName);

	enum LocalAppDataFolderId
	{
		LocalAppDataFolder,
		LocalAppDataLowFolder
	};

	/* \brief Get the AppData Local or LocalLow folder
	   \param[in] localAppDataFolderId which to get
	   \param[in] checkWritable if true, check to see whether can create a file in folder
	   \param[out] the AppData Local or LocalLow folder
	   \return true if could get the specified AppData folder and, if checkWritable is true, whether the folder is writable
	*/
	UtilExport bool GetAppDataFolder(LocalAppDataFolderId localAppDataFolderId, bool checkWritable, MSTR& folder);

	//! \brief See if can create files in TEMP folder. If not, set TEMP env var to writable folder
	// This is mainly for when running in low integrity mode - usually points to AppData\Local
	// but for low integrity mode should point to AppData\LocalLow.
	// User may have specified custom TEMP folder, so we want to use existing TEMP folder if possible.
	UtilExport void MaybeUpdateTempFolders();

	//! \brief Get the current process name
	UtilExport MSTR GetProcessName();
