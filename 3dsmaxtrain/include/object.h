...
extern CoreExport Class_ID anyObjectClassID;
...
public:
	...
	CoreExport virtual void* GetInterface(ULONG id)override;
	CoreExport virtual BaseInterface* GetInterface(Interface_ID id)override;
...
/// @{
	...
	/** \return the name that will appear in the modifier stack.
	\param localized If true, then the object name returned should be localized in the language 3ds Max is currently using. Otherwise it should be the object name in English.
						  If a plugin does not provide localized string resources, it can disregard this parameter and always return the object name in English. */
	virtual const MCHAR *GetObjectName(bool localized) const { return _M("Object"); }
...
/**
 * MAX_SILENCE_DEPRECATED_DEFORMER_MAP
 * Silences warning for non-const Deformer::Map deprecation.
 */
#if defined(MAX_SILENCE_DEPRECATED_DEFORMER_MAP)
#define MAX_DEPRECATE_DEFORMER_MAP(str)
#else
#define MAX_DEPRECATE_DEFORMER_MAP(str) [[deprecated(str)]]
#endif
...
public:
	/** Destructor. */
	CoreExport virtual ~Deformer();

	/**
	 * DEPRECATED : See const version for help.
	 * REASON : Deformer::Map should have always been const, now it will be.
	 *		This will allow us to accelerate the deformers using more
	 *		aggressive threading.
	 */
	MAX_DEPRECATE_DEFORMER_MAP(
		"The non-const version of Deformer::Map is deprecated and will be removed in future updates.\n"
		"Define MAX_SILENCE_DEPRECATED_DEFORMER_MAP to silence this warning.")
	CoreExport virtual Point3 Map(int i, Point3 p);

	/** This is the method that is called to deform or alter a single point.
	Note that this method needs to be thread safe, that is why it is const.
	If you absolutely must modify state inside of Map, use a mutex.
	...
	CoreExport virtual Point3 Map(int i, Point3 p) const;

	/**
	 * Override this function and return true if your deformer implements Deformer::Map() const.
	 */
	MAX_DEPRECATE_DEFORMER_MAP(
		"This is a temporary function and will be removed along-side non-const Deformer::Map().\n"
		"Define MAX_SILENCE_DEPRECATED_DEFORMER_MAP to silence this warning.")
	CoreExport virtual bool IsThreadSafe() const;

	/** This is the method that is called to deform or alter a point normal.
	Note that this method needs to be thread safe, that is why it is const.
	If you absolutely must modify state inside of MapNormal, use a mutex.

	By default, this will call your Map() implementation. Override this
	if you need different behavior when deforming normals.

	\param i The index of the normal to be altered.
		Note: An index of -1 may be passed.
		This indicates that the deformer is not being applied to a regular object
		but instead normals that are generated on the fly for display purposes.
	\param normal The normal to be altered.
	\return  The altered normal. */
	CoreExport virtual Point3 MapNormal(int i, Point3 normal) const;
...
public:
	...
	virtual Interface_ID	GetID() override { return IXTCACCESS_INTERFACE_ID; }
...
//! @}
	...
	*/
	...
	from ConvertToType().
	Note that all overrides of this method should either call _super::CanConvertToType
	or test to see if obtype == anyObjectClassID. If true, the method should return true.
	...
	original.
	Note that all overrides of this method should either call _super::ConvertToType
	or test to see if obtype == anyObjectClassID. If true, the object should return itself.
	...
	*/
	...
	virtual BOOL HasUVW() override { return 0; }
	// or on any map channel:
	virtual BOOL HasUVW (int mapChannel) override { return (mapChannel==1) ? HasUVW() : FALSE; }
	...
	// returns TRUE for manipulator objects and FALSE for all others
...
public:
	...
	virtual RefResult NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate) override { return REF_SUCCEED; }

	// from class Animatable
	/** This method returns the super class ID of the creator of the
	clip object. */
	virtual SClass_ID SuperClassID() override { return MPASS_CAM_EFFECT_CLASS_ID; }
	virtual void BeginEditParams(IObjParam *ip, ULONG flags, Animatable *prev=NULL) override {}
	virtual void EndEditParams(IObjParam *ip, ULONG flags, Animatable *next=NULL) override {}
};
...
public:
	...
	virtual void InitNodeName(MSTR& s) override { s = _M("Camera"); }
	virtual int UsesWireColor() override { return FALSE; } // TRUE if the object color is used for display
	...
	BOOL GetMultiPassEffectEnabled(TimeValue t) { Interval interval = FOREVER; return GetMultiPassEffectEnabled(t, interval); }
...
public:
	...
	int IsRenderable() { return(0);}
	virtual void InitNodeName(MSTR& s) override { s = _M("Light"); }
	...
	RefResult EvalLightState(TimeValue time, LightState* ls) { Interval i = FOREVER; return EvalLightState(time, i, ls); }
	/*! \remarks This overload was added to fix compilation errors in legacy code.
	Previously, the way to ignore validity interval was to pass FOREVER. This causes compilation errors in modern compiler.
	FOREVER creates a temporary Interval which can't be assigned to a non-const reference. See /Zc:referenceBinding.
	To ignore validity interval, use overload that doesn't take an interval.*/
	RefResult EvalLightState(TimeValue time, Interval&&, LightState* ls) { return EvalLightState(time, ls); }
...
public:
	...
	virtual void InitNodeName(MSTR& s) override { s = _M("Helper"); }
	/** Implemented by the System. 
	Returns TRUE to indicate the object color is used for display. */
	virtual int UsesWireColor() override { return FALSE; }   // TRUE if the object color is used for display
	virtual BOOL NormalAlignVector(TimeValue t,Point3 &pt, Point3 &norm) override {pt=Point3(0,0,0);norm=Point3(0,0,-1);return TRUE;}
};
...
public:         
	virtual void InitNodeName(MSTR& s) override { s = _M("Object"); }
	SClass_ID SuperClassID() override { return GEOMOBJECT_CLASS_ID; }

	virtual int IsRenderable() override { return(1); }               
...
public:
	BOOL IsParticleSystem() override {return TRUE;}
	...
	int IsDeformable() override {return TRUE;} 
	...
	BOOL CanCacheObject() override {return FALSE;}
...
public: 
	...
	DWORD ID() override { return SHAPE_RECT_RENDERPARAMS_PROPID;}
};
...
class ShapeObject : public GeomObject
{
	...
	Interval cacheValid;
	int cacheType;
	bool mUseNewEndCapVis = true;	// When true, use improved end cap edge visibility
...
public:
	...
	CoreExport virtual BaseInterface* GetInterface(Interface_ID iid) override;

	// from GeomObject
	CoreExport virtual void* GetInterface(ULONG id) override;

	// display functions from BaseObject
	CoreExport virtual unsigned long GetObjectDisplayRequirement() const override;
	CoreExport virtual bool PrepareDisplay(
	...
	virtual BOOL IsShapeObject() override { return TRUE; } 
	...
	\return  Nonzero if a point of intersection was found; otherwise 0. */
	virtual int IntersectRay(TimeValue t, Ray& ray, float& at, Point3& norm) override {return FALSE;}
	virtual void InitNodeName(MSTR& s) override { s = _M("Shape"); }
	SClass_ID SuperClassID()  override{ return SHAPE_CLASS_ID; }
	CoreExport virtual int IsRenderable() override;
	...
	inline float GetThickness(TimeValue t) { Interval interval = FOREVER; return GetThickness(t, interval); }
	...
	inline int GetSides(TimeValue t) { Interval interval = FOREVER; return GetSides(t, interval); }
	...
	inline float GetAngle(TimeValue t) { Interval interval = FOREVER; return GetAngle(t, interval); }
	...
	*/
	...
	CoreExport virtual Mesh* GetRenderMesh(TimeValue t, INode *inode, View& view, BOOL& needDelete) override;
	...
	virtual BOOL HasUVW()  override { return GetGenUVs(); }
	virtual BOOL HasUVW (int mapChannel)  override { return (mapChannel==1) ? HasUVW() : FALSE; }
	virtual void SetGenUVW(BOOL sw)  override { SetGenUVs(sw); }
	virtual void SetGenUVW (int mapChannel, BOOL sw)  override { if (mapChannel==1) SetGenUVW (sw); }
	...
	CoreExport virtual IOResult Save(ISave *isave) override;
	/** Implemented by the System. 
	...
	CoreExport virtual IOResult Load(ILoad *iload) override;		
	/** Implemented by the System.
	...
	CoreExport virtual bool SpecifySaveReferences(ReferenceSaveManager& referenceSaveManager) override;

	/** Implemented by the System. 
	This is an implementation of the Object method. It simply returns
	splineShapeClassID. */
	CoreExport virtual Class_ID PreferredCollapseType() override;
	/**	Implemented by the System. 
	...
	CoreExport virtual BOOL GetExtendedProperties(TimeValue t, MSTR &prop1Label, MSTR &prop1Data, MSTR &prop2Label, MSTR &prop2Data) override;
	/**	Implemented by the System. 
	...
	CoreExport virtual void RescaleWorldUnits(float f) override;
	...
	CoreExport virtual RefResult NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate) override;
	...
	CoreExport virtual RefTargetHandle GetReference(int i) override;
...
public:
	...
	CoreExport virtual Animatable* SubAnim(int i) override;
	/**	This method returns the name of the animatable's name. Derived classes
	implementing this method must pass on references to index 0 to the
	ShapeObject. For example: 
	MSTR SomeShape::SubAnimName(int i, bool localized) {\n\n
	if (i == 0) return ShapeObject::SubAnimName(i, localized);\n\n
	}
	\param i This is the index of the sub-anim's name to return.
	\param localized If true, then the sub-anim name returned is localized. */
	CoreExport virtual MSTR SubAnimName(int i, bool localized = true) override;
	...
	\param pbIndex The index of the parameter to return the dimension of.
	\param localized If true, then the parameter name returned should be localized in the language 3ds Max is currently using. Otherwise it should be the parameter name in English.
	...
	CoreExport virtual int RemapRefOnLoad(int iref) override;
	...
	*/
	...
	\param vpt An interface pointer that may be used to call methods associated with the viewports.
	\return  Nonzero if the item was hit; otherwise 0. */
	CoreExport virtual int HitTest(TimeValue t, INode* inode, int type, int crossing, int flags, IPoint2 *p, ViewExp *vpt) override;
	...
	\param mc A pointer to the modifiers ModContext.
	\return  Nonzero if the item was hit; otherwise 0. */
	CoreExport virtual int HitTest(TimeValue t, INode* inode, int type, int crossing, int flags, IPoint2 *p, ViewExp *vpt, ModContext* mc) override;
	...
	*/
	...
	inline int GetCapSegments(TimeValue t) { Interval interval = FOREVER; return GetCapSegments(t, interval); }
	...
	inline float GetSphereCap(TimeValue t) { Interval interval = FOREVER; return GetSphereCap(t, interval); }
...
public:                                         
	...
	\see class BindingModifierSceneRemove
	*/
...
/**	This class is used as a helper class to remove a binding modifier when the 
node it is bound to deleted. 
For example, if create a Bend WSM node and use 'Bind to Space Warp' to bind another 
node to the Bend WSM node, a 'Bend Binding (WSM)' modifier is placed on that node.
If then delete the Bend WSM node, this class is used to cause the 'Bend Binding (WSM)' 
modifier to be automatically removed.
The SimpleWSMMod class uses an instance of this class, as does the BombMod class which 
is used as a binding modifier but does not derive from SimpleWSMMod.
Example usage in BombMod class:
\code
...
public:
	...
	The modifier is added to the list if 3dsmax is in the middle of deleting one or more nodes, and 3dsmax is not in the
	middle of an undo or redo. The list of modifiers is processed at the end of deleting the nodes.
...
//! @{
	...
	\param localized If true, then the name returned should be localized in the language 3ds Max is currently using. Otherwise it should be the name in English.
	...
	CoreExport virtual MSTR GetName(bool localized) const;
...
//! @{
	...
	virtual void DisableModApps() { NotifyDependents(FOREVER,PART_OBJ,REFMSG_DISABLE); }
	/** This method is used internally. */
	virtual void EnableModApps() {  NotifyDependents(FOREVER,PART_OBJ,REFMSG_ENABLE); }
	...
	CoreExport virtual void DisableMod();

	/** Implemented by the System. 
	This enables the modifier in the history browser (modifier stack). */
	CoreExport virtual void EnableMod();
	...
	\return  Nonzero if enabled; otherwise 0. 
	  */
	virtual int IsEnabled() { return !TestAFlag(A_MOD_DISABLED); }
	...
	CoreExport virtual void DisableModInViews();

	/** Implemented by the System. 
	Enables the modifier in the viewports. */
	CoreExport virtual void EnableModInViews();

	/** Implemented by the System. 
	Returns nonzero if the modifier is enabled in the view ports; otherwise
	zero. */
	virtual int IsEnabledInViews() { return !TestAFlag(A_MOD_DISABLED_INVIEWS); }

	// Same as above but for renderer only
	/** Implemented by the System. 
	This turns off the modifier in the renderer 
	  */
	CoreExport virtual void DisableModInRender();

	/** Implemented by the System. 
	This turns on the modifier in the renderer 
	  */
	CoreExport virtual void EnableModInRender();
	...
	\return  Nonzero if enabled; otherwise 0. 
	  */
	virtual int IsEnabledInRender() { return !TestAFlag(A_MOD_DISABLED_INRENDER); }
//! @}
...
//! @{
	...
	*/
	CoreExport virtual ChannelMask TotalChannelsUsed();
	...
	changed. 
	*/
	CoreExport virtual ChannelMask TotalChannelsChanged();

	// Predefined input types are:
	...
	// Additional types are specified object classes such as NURBS and FFD objects
	...
	/** When InputType() returns anyObjectClassID, this method is used to determine whether the
	modifier can be applied to a specific object. An usage case of this would be a modifier that
	can only be applied to Arnold lights. The modifier could then modify the parameters of the
	Arnold light instance passing through it.
	\param object The object to test.
	\return True if the modifier can be applied to the object*/
	virtual bool AnyObjectFilter(Object* object) const { return true; }
//! @}
...
//! @{
    ...
    /** In cases where a modifier does not exist in the specified save version
    returned by GetSavingVersion(), or where a modifier employs new
    functionality which cannot be retained in the specified save version, it
    may return true to indicate that the owning object should be saved in a
    state with its modifier stack collapsed at least up to this modifier.
    \return Flag indicating whether this modifier should be saved in a
    collapsed state for the specified save version. */
    virtual bool ShouldCollapseOnSave() { return false; }
//! @}
...
//! @{
	...
	CoreExport virtual void EnumModContexts(ModContextEnumProc *proc);
	...
	CoreExport virtual void GetIDerivedObject(ModContext *mc, IDerivedObject *&derObj, int &modIndex);
...
public:
	...
	\param p Array of vertices of the polyline.
	\param n Number of vertices in the polyline. */
	int proc(Point3 *p, int n) override { gw->polyline(n, p, NULL, NULL, 0, NULL); return 0; }
	/** Implemented by the System. 
	Sets the color used to draw the lines.
	\param r Red component in the range 0.0 to 1.0
	\param g Green component in the range 0.0 to 1.0
	\param b Blue component in the range 0.0 to 1.0 */
	void SetLineColor(float r, float g, float b) override {gw->setColor(LINE_COLOR,r,g,b);}
	void SetLineColor(Point3 c) override {gw->setColor(LINE_COLOR,c);}
	void Marker(Point3 *p,MarkerType type) override {gw->marker(p,type);}
};
...
public:
	...
	CoreExport int proc(Point3 *p, int n) override;
	CoreExport void Marker(Point3 *p,MarkerType type) override;
};
...
inline void DoModifiedBox(Box3& box, Deformer& def, PolyLineProc&& lp) { DoModifiedBox(box, def, lp); }
CoreExport void DoModifiedLimit(Box3& box, float z, int axis, Deformer& def, PolyLineProc& lp);
inline void DoModifiedLimit(Box3& box, float z, int axis, Deformer& def, PolyLineProc&& lp) { DoModifiedLimit(box, z, axis, def, lp); }
CoreExport void DrawCenterMark(PolyLineProc& lp, Box3& box);
inline void DrawCenterMark(PolyLineProc&& lp, Box3& box) { DrawCenterMark(lp, box); }

// Some functions to draw mapping icons
CoreExport void DoSphericalMapIcon(BOOL sel,float radius, PolyLineProc& lp);
inline void DoSphericalMapIcon(BOOL sel, float radius, PolyLineProc&& lp) { DoSphericalMapIcon(sel, radius, lp); }
CoreExport void DoCylindricalMapIcon(BOOL sel, float radius, float height, PolyLineProc& lp);
inline void DoCylindricalMapIcon(BOOL sel, float radius, float height, PolyLineProc&& lp) { DoCylindricalMapIcon(sel, radius, height, lp); }
CoreExport void DoPlanarMapIcon(BOOL sel, float width, float length, PolyLineProc& lp);
inline void DoPlanarMapIcon(BOOL sel, float width, float length, PolyLineProc&& lp) { DoPlanarMapIcon(sel, width, length, lp); }
...
public:
	...
	\param dist The 'distance' of the hit. An implementation defined value that is lower for hits that are closer.
...
public:
	...
	implementation of the ISubObjType method. */
	CoreExport MaxIcon *GetIcon() override; 
};
