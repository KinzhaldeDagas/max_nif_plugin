...
//! \name Illumination Related Methods
/// @{
...
/// @{
		 ...
		 * \param localized If true, then the channel name returned should be localized in the language 3ds Max is currently using. Otherwise it should be the channel name in English.
		 *                  If a plugin does not provide localized string resources, it can disregard this parameter and always return the channel name in English.
		 */
		 ...
		 */
		virtual MSTR GetTexChannelInternalName( long nTextureChan ) { return GetTexChannelName(nTextureChan, true); }
