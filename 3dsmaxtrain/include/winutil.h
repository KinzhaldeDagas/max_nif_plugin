...
namespace MaxSDK
{
	 ...
	 */
	template <typename T>
	inline T UIScaled(T value, int monitorID = -1 )
	{
	 ...
	 */
	template <typename T>
	inline T UIUnScaled(T value, int monitorID = -1)
	{
		static_assert(std::is_arithmetic_v<T>, "T must be arithmetic type");
		return static_cast<T>(value / GetUIScaleFactor(monitorID));
	}
	...
	*/
	...
	//-----------------------------------------------------------------------------
	//! \brief Enum for typical usage of OPENFILENAME::lCustData to hold whether the OPENFILENAME instance
	//! is used in a GetOpenFileName or a GetSaveFileName function call. Typically used to determine proper
	//! messages to be displayed or logged.
	//! \sa MinimalMaxFileOpenSaveHookProc 
	...
	//-----------------------------------------------------------------------------
	//! \brief Validates the file name length in file open / save dialog proc.
	/*!
	\param[in] hDlg The window handle passed to the dialog proc
	\param[in] lpofn Pointer to the OPENFILENAME instance. 
	\param[in] messageBoxTitle The title of the message box displayed if filename specified is > MAX_PATH characters
	\return true if the file name length is valid, false if not.
	\sa MinimalMaxFileOpenHookProc
	\note The underlying windows proc for file open / save dialogs does not behave as expected if a filename is selected
	that is longer that the file name buffer in the OPENFILENAME instance. The proc returns success, but the content of the
	file name buffer is the size of the buffer that is required to hold the selected filename. 
	This method detects this case, and the case where the filename is a long file name (file name starts with "\\?"). 3ds Max
	does not support this type of long file name. 
	This function should be called from the proc when handling message WM_NOTIFY, code CDN_FILEOK, and return TRUE if this 
	function returns false.
	\note OPENFILENAME resolves to tagOFNW
	*/
	...
	\endcode
	\note MinimalMaxFileOpenSaveHookProc should preferentially be used as it displays a more accurate warning message
	since it knows via ofn.lCustData whether it is being used in an Open or Save dialog.
	\note The size of the OPENFILENAME lpstrFile string buffer should be at least MAX_PATH, and at least 2000 if
	OFN_ALLOWMULTISELECT is specified.
	*/
	...
	//! \brief A minimal LPOFNHOOKPROC for scaling the toolbar size and icons in standard file open/save dialogs.
	/*!
	Use this LPOFNHOOKPROC when customization of the Open/Save File dialog is being not performed. ofn.lCustData specifies 
	whether being used in an Open or Save dialog.
	Example usage:
	\code
	OPENFILENAME   ofn;
	memset(&ofn, 0, sizeof(ofn));
	...
	BOOL res = GetOpenFileName(&ofn);
	\endcode
	The implementation of this method is identical to MinimalMaxFileOpenHookProc exception for the contents
	of the warning MessageBox when path name is too long.
	\note The size of the OPENFILENAME lpstrFile string buffer should be at least MAX_PATH, and at least 2000 if
	OFN_ALLOWMULTISELECT is specified.
	*/
