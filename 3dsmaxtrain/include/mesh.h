...
class Face: public MaxHeapOperators {	
	...
	public:
		...
		*/
		...
		BOOL	Hidden() const {return flags&FACE_HIDDEN?TRUE:FALSE;}
		...
		DllExport DWORD GetVertIndex(DWORD v0) const;
...
single channel. 
An array of instances of this class is carried by the Mesh class.
All methods of this class are implemented by the system.
*/
...
public:
	...
	*/
	...
	DllExport MeshMap & operator= (const MeshMap & from);
};
...
public:
	...
	void *Addr (void *ptr, int at) const { BYTE *vd=(BYTE *)ptr; return (void *)(vd+at*DataSize()); }

	/** Returns the address of the specified element in the
	data array.
	\param at The zero based index of the element. */
	void *Addr (int at) const { return Addr(data,at); }
...
class SubObjHitList: public MaxHeapOperators {
	private:
		...
		SubObjHitList(const SubObjHitList&) = delete;
		SubObjHitList& operator=(const SubObjHitList&) = delete;
	...
	public:
		...
		*/
		...
		/** Resets the container and frees all its memory. */
		DllExport void Clear();
	};
...
#define SUBHIT_IGNOREOCCLUDED (1 << 30)	//!< Indicates that occluded sub-objects should be ignored.
//!@}
...
Note 1 : There is a set of classes for working with parts of a mesh such as its
...
\see BitArray, Face, TVFace, Point3, TriObject
\see RNormal, RVertex, TriObject, MeshMap, 
Note 2 : if you are going to share this mesh between different classes, you should use the 
class SharedMesh which uses reference counting and gets deleted once its ref count reaches 0.
\see MaxSDK::SharedMesh.

\par List of deprecations from 3dsMax 2024:
<table><tr><th>Deprecated<th>Please use instead
<tr><td><b>BitArray vdSupport</b>, moved to \ref MaxSDK::MeshGeomChannelData. <td>\ref getNumVData() const, \ref getNumVDataSupported() const, \ref vDataSupport(int vd) const, \ref setVDataSupport(int vd, BOOL support)
<tr><td><b>PerData* vData</b>, moved to \ref MaxSDK::MeshGeomChannelData. <td>\ref GetVD(int vdChannel) const;
<tr><td><b>BitArray vertSel</b>, moved to \ref MaxSDK::MeshGeomChannelData. <td>\ref VertSel()
<tr><td><b>BitArray faceSel</b>, moved to \ref MaxSDK::MeshTopoChannelData. <td>\ref FaceSel()
<tr><td><b>BitArray edgeSel</b>, moved to \ref MaxSDK::MeshTopoChannelData. <td>\ref EdgeSel()
<tr><td><b>BitArray vertHide</b>, moved to \ref MaxSDK::MeshGeomChannelData. <td>\ref VertHide()
<tr><td><b>MtlID mtlIndex</b>, moved to \ref MaxSDK::MeshMaterialChannelData. <td>\ref GetMtlIndex() const, \ref SetMtlIndex(MtlID mtlIndex).
<tr><td><b>int numMaps</b><td>\ref getNumMaps() const;
<tr><td><b>MeshMap* maps</b>, moved to \ref MaxSDK::MeshTextureMapChannelData and \ref MaxSDK::MeshVertexColorChannelData.<td>\ref Map(int mp) const;
<tr><td><b>void ZeroTopologyCache()</b><td>Removed completely.
</table>
*/
...
public:
	...
	/*! This can be used avoid race conditions when modifying many Min parallel when some of them share channels.
		...
		\see Mesh::ReleaseChannelLocks(ChannelMask channels) const
		\see Mesh::ShallowCopy(Mesh *amesh, ChannelMask channels)
	*/
	...
	/*! \param[in] channels - Which channels to release the locks for.
		\see Mesh::AcquireChannelLocks(ChannelMask channels) const
		\see Mesh::ShallowCopy(Mesh *amesh, ChannelMask channels)
	*/
	void ReleaseChannelLocks(ChannelMask channels) const { return m_ChannelDataContainer.ReleaseChannelLocks(channels); }

	/*! A helper class for aquiring and releasing Mesh channel locks.
	...
	*/
	...
	{
	public:
		ScopedLockChannels(Mesh* parent, ChannelMask channels) : mChannels(channels), mParent(parent), mResetCalled(false) {
...
private:
	...
	/*! Container that owns the channel objects of this mesh
		\see MaxSDK::MeshChannelData
	*/
	MaxSDK::MeshChannelContainer m_ChannelDataContainer;
		...
		int GetNumVisEdges() const;
		int GetEdgeListHasAll() const;
		void SetEdgeListHasAll(int edgeListHasAll);
		VEdge* GetVisEdge() const;
	...
	public:
		...
		//! \brief Returns the Mesh channel container which provides access to the channel objects themselves.
		/*! To modify a Mesh channel directly, instead of using the Mesh interface, you can fetch the MeshChannelContainer,
		...
		Example to manually reallocate the Mesh faces:
		\code{.cpp}
		...
		*/
		MaxSDK::MeshChannelContainer& GetChannelDataContainer() { return m_ChannelDataContainer; }
		//! \brief Returns a const mesh channel container which provides access to the channel objects themselves.
		/*! \see Mesh::GetChannelDataContainer() */
		const MaxSDK::MeshChannelContainer& GetChannelDataContainer() const { return m_ChannelDataContainer; }

		//! \brief The number of faces in this Mesh. Faces are owned by the \ref MaxSDK::MeshTopoChannelData channel.
		/*! To modify use the Mesh interface methods such as "setNumFaces".
		May need to cast in some cases:
		\code{.cpp}
			(int)numFaces;
		\endcode
		\see MaxSDK::BlockWrite_Value
		\see MaxSDK::MeshTopoChannelData
		*/
		MaxSDK::BlockWrite_Value<int, MaxSDK::MeshTopoChannelData> numFaces;
		//! \brief The mesh faces. Faces are owned by the \ref MaxSDK::MeshTopoChannelData channel.
		/*!  May need to cast in some cases:
		\code{.cpp}
			(Face*)faces;
		\endcode
		...
		*/
		...
		//! \brief The number of verts in this Mesh. Mesh verts are owned by the \ref MaxSDK::MeshGeomChannelData channel.
		/*! To modify use the Mesh interface methods such as "setNumVerts".
		May need to cast in some cases:
		\code{.cpp}
			(int)numVerts;
		\endcode
		\see MaxSDK::BlockWrite_Value
		\see MaxSDK::MeshGeomChannelData
		*/
		MaxSDK::BlockWrite_Value<int, MaxSDK::MeshGeomChannelData> numVerts;
		//! \brief The mesh verts. Mesh verts are owned by the \ref MaxSDK::MeshGeomChannelData channel.
		/* May need to cast in some cases:
		\code{.cpp}
			(Point3*)verts;
		\endcode
		...
		*/
		...
		//! \brief Holds flags specific to the corresponding vertex in the verts array. Owned by the \ref MaxSDK::MeshGeomChannelData channel.
		/*! May need to cast in some cases:
		\code{.cpp}
			(long*)vFlags;
		\endcode
...
/** \name Color per vertex */
//! @{	
		//! \brief Number of color vertices.
		/*! Mesh numCVerts is owned by the \ref MaxSDK::MeshVertexColorChannelData channel.
			May need to cast in some cases:
		\code{.cpp}
			(int)numCVerts;
		\endcode
		\see MaxSDK::BlockWrite_Value
		\see MaxSDK::MeshVertexColorChannelData
		*/
		MaxSDK::BlockWrite_Value<int, MaxSDK::MeshVertexColorChannelData> numCVerts;
		//! \brief Array of color vertices.
		/*! Mesh numCVerts is owned by the \ref MaxSDK::MeshVertexColorChannelData channel.
			May need to cast in some cases:
		\code{.cpp}
			(VertColor*)vertCol;
		...
		*/
		...
		//! \brief Array of color per vertex faces.
		/*! Mesh numCVerts is owned by the \ref MaxSDK::MeshVertexColorChannelData channel.
			May need to cast in some cases:
		\code{.cpp}
			(TVFace*)vcFace;
...
/** \name Texture Coord assignment */
//! @{
		//! \brief Number of texture vertices.
		/*! Mesh numCVerts is owned by the \ref MaxSDK::MeshTextureMapChannelData channel.
			May need to cast in some cases:
		\code{.cpp}
			(int)numTVerts;
		\endcode
		\see MaxSDK::BlockWrite_Value
		\see MaxSDK::MeshTextureMapChannelData
		*/
		MaxSDK::BlockWrite_Value<int, MaxSDK::MeshTextureMapChannelData> numTVerts;
		//! \brief The array of texture vertices.
		/*! This stores the UVW coordinates. For a 2D
			...
			\ref MaxSDK::MeshTextureMapChannelData channel owns this members data.
		\note typedef Point3 UVVert;
			May need to cast in some cases:
		\code{.cpp}
			(int)numTVerts;
		\endcode
		...
		*/
		...
		//! \brief The array of texture faces.
		/*! There needs to be one TVFace for every face,
...
/** \name Material Assignment */
//! @{
		//! \brief Get the object material
		DllExport MtlID GetMtlIndex() const;
		//! \brief Set the object material
...
//! @{		
		...
		\param i Specifies the index of the face to retrieve. */
		Face& getFace(int i) { return faces[i]; }

		/** Returns a pointer to the 'i-th' face.
		\param i Specifies the index of the face address to retrieve. */
...
//! @{		
		...
		/**	Returns the number of texture or vertex color faces for the specified
		channel of this mesh.
		\param mp - Specifies the channel. See "List of Mapping Channel Index Values". */
...
//! @{
		...
		\param mp - The map channel.
		\return Const reference to a MeshMap.*/
		DllExport MeshMap const& Map(int mp) const;
		/*! This method returns the map for the specified map channel.
		\param mp - The map channel.
		\return Reference to a MeshMap.*/
		MeshMap& Map(int mp)
		{
			const Mesh* m = this;
			return const_cast<MeshMap&>(m->Map(mp));
		}
//! @}
...
//! @{		
		...
		*/
		...
		array is numVerts.
		\param vd The vertex data channel. See the page "List of Vertex	Data Index Options".*/
		void *vertexData (int vd) const { return vDataSupport(vd) ? GetVD(vd)->data : NULL; }
...
//! @{		
		...
		/** Returns flags specific to the corresponding vertex in the verts array. Owned by the \ref MaxSDK::MeshGeomChannelData channel.
		\param i Specifies the index of the vertex address to retrieve the flags for. */
		long GetVFlags(int i) { return vFlags[i]; }
//! @}
...
//! @{
		...
		/** If the transformed bounding box presently stored by this mesh is invalid, or corresponds to a
		    transformation differing from that defined the specified matrix transformMat, then the stored transformed
		    bounding box is updated to reflect the transformation of the mesh into the space defined by transformMat.
		    The computed bounding box will then be accessible via a call to getBoundingBox with *tm = transformMat.

			\param[in] transformMat: transformation matrix, including translation vector, defining the transformation
		                             from this mesh's object space, to the space in which the bounding box is to be
		                             computed */
		DllExport void BuildTransformedBoundingBox(const Matrix3& transformMat);

		/** Retrieves the bounding box of the mesh object. If an up-to-date bounding box corresponding to the specified
		    transformation tm is not available, it will be generated via a call to buildBoundingBox, if tm = nullptr,
		    or BuildTransformedBoundingBox, otherwise.
...
//! @{
		...
		\param amesh - Specifies the source Mesh to copy. 
      ...
      DllExport void       ShallowCopy(Mesh *amesh, ChannelMask channels);
		...
		/** Makes a complete copy of the specified channel objects from the specified
		Mesh and store them into this Mesh.
		\param amesh - Specifies the source Mesh to copy. 
		\param channels - Specifies the channels to copy. See \ref ObjectChannels.
		\see MaxSDK::MeshChannelContainer */
...
//! @{
		/*! \brief Retrieves the bits representing the vertex selection status.
		...
		\return  The selected edges status. */
		...
		\return  The hidden vertex status. */
...
//! @{			
		...
		\param f1 
		\param faceNormals an optional parameter to pass in a precomputed face normal list so you can improve performance
...
private:
		...
		Mesh_SpaceAccelData* mpSpaceAccelData = nullptr;
public:
...
DllExport void CombineMeshes(
		Mesh& outMesh, const Mesh& mesh1, const Mesh& mesh2,
		const Matrix3* tm1 = nullptr, const Matrix3* tm2 = nullptr, int whichInv = 0);
 ...
 * sets of faces, splitting the mesh into two meshes.
 \param mesh 
The mesh to slice
\param N
The slicing plane
\param off 
The offset on the slicing plane
\param split
Indicates that mesh should be split into two along the plane, otherwise faces are
created with shared vertices.  Ignored is remove is TRUE
\param remove
Indicates that the faces below the plane should be removed
\param ae
Contains a pointer to the AdjEdgeList for the mesh if precomputed. Providing this will aid
in calculation speeds if pre-computed.  The contents are not updated after the slice operation.
\return TRUE if slice operation resulted in a change in the mesh. */
DllExport BOOL SliceMesh (Mesh & mesh,
						  Point3 N, float off, bool split=FALSE, bool remove=FALSE, AdjEdgeList *ae = nullptr);
