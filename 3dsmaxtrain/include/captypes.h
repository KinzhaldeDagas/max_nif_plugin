 ...
 *<
	...
	DESCRIPTION: Capping type definitions
...
class MeshCapper : public MaxHeapOperators
{
	public:
		...
		*/
		MeshCapPoly *polys;
		/** Constructor. This gets the capper ready for the topology
		of the specified PolyShape.
		\param shape - The PolyShape to initialize the cap data from.
		*/
		CoreExport MeshCapper(PolyShape &shape);
		/** Destructor.
		*/
		CoreExport ~MeshCapper();
		/** Array access operator. Returns the MeshCapPoly as
		specified by the index.
		*/
		CoreExport MeshCapPoly &operator[](int index);
		/** This method is used to modify the mesh based on the
		MeshCapInfo, the flipped state, the smoothing group and an orientation
		matrix.
		\param mesh - The Mesh to modify.
		\param capInfo - The MeshCapInfo describing the properties of the cap.
		\param flip - A switch to indicate if the cap should be oriented normally or flipped.
		...
		\param smooth - The smoothing group for all the faces in the cap.
		\param tm - The orientation matrix. A grid cap generates new vertices inside the
		...
		\param mtlID - This parameter is available in release 2.0 and later only.
		...
		*/
		...
		/** This method is used to modify the MNMesh based on the
		MeshCapInfo, the flipped state, the smoothing group and an orientation
		matrix.
		\param mesh - The Mesh to modify.
		\param capInfo - The MeshCapInfo describing the properties of the cap.
