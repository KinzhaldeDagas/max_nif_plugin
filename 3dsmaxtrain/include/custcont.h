...
//! \deprecated This has been deprecated in 3ds Max 2026. Use GetCOREInterface7()->DoUICustomization(CUI_PAGE_PREVIOUS) instead.
...
public:
	...
	MacroButtonData(const MacroButtonData& other)
	{
		*this = other;
	}
	...
	*/
	...
	int			  macroType = 0;
	ActionTableId tblID = 0;
	int			  cmdID = 0;
	int			  macroScriptID = 0;
	const MCHAR*  label = nullptr;
	const MCHAR*  tip = nullptr;
	
	const MCHAR*  imageName = nullptr;
	int			  imageID = 0;
	ActionItem*   actionItem = nullptr;
	const MCHAR*  iconName = nullptr;
...
public:
	...
	/** This method has to be implemented in order to support drag and drop for
	 * rollups, no matter if custom drop handling is used, or not. The order of
	 * rollup panels is stored in the RollupOrder.cfg file in the UI directory.
	 * The order is stored under a SuperClassID and ClassID (and an optional
	 * context, if available). The RollupCallback has to specify what the
	 * SuperClassID and ClassID is. E.g. for the Modify Panel it is the ClassID
	 * for the currently edited object. For	the DisplayPanel it is only one
	 * ClassID, that has no real class assigned to it, since the rollouts are
	 * independent from the object being edited
	 * (DISPLAY_PANEL_ROLLUPCFG_CLASSID).
	 * 
	 * \param sid[out] The SuperClassID of the object.
	 * \param cid[out] The class ID of the object.
	 * \return TRUE if drag and drop is supported, otherwise FALSE.
	 * \note The default implementation returns FALSE and does not touch sid 
	 *       and cid.
	 * \see GetEditObjContext, ICatRegistry */
	 ...
	 * for drag and drop for rollups, no matter if custom drop handling is
	 * used, or not. It also requires the GetEditObjClassID function to be
	 * implemented and returning TRUE as well.
	 *
	 * The order of rollup panels is stored in the RollupOrder.cfg file in the
	 * UI directory. The order is stored under a SuperClassID and ClassID and
	 * this context string, if present. The RollupCallback has to specify what
	 * the Context is, if present.
	 *
	 * \param context[out] A reference to a string that shall receive the
	 *        current context of the object being edited. This contexts is used
	 *        to store and retrieve the rollup oder and state.
	 * \return TRUE, if the context should be used, otherwise FALSE.
	 *
	 * \note The default implementation returns FALSE and does not touch the
	 *       context string.
	 * \see GetEditObjClassID, ICatRegistry */
...
class ToolItem: public MaxHeapOperators
{
	public: 
		...
		DWORD helpID = 0;
		int w = 0, h = 0;
		int orient = 0; // which orientations does this item apply to?
		/*! \remarks Destructor. */
		virtual ~ToolItem() {}
	protected:
		ToolItem(ToolItemType type, int id, DWORD helpID, int w, int h, int orient)
			: type(type), id(id), helpID(helpID), w(w), h(h), orient(orient) {}
};
...
class ToolButtonItem : public ToolItem 
{
	...
	//!@{
		...
		ToolButtonItem(ToolItemType t,
			...
			int iW, int iH, int wd, int ht, int ID, DWORD hID = 0, const MCHAR* lbl = nullptr,
			int ori = CTB_HORIZ|CTB_VERT|CTB_FLOAT) : ToolButtonItem(t, iW, iH, wd, ht, ID, hID, lbl, ori)
			{ 
				iOutEn = iOE;
				iInEn = iIE;
				iOutDis = iOD;
	...
	private:
		ToolButtonItem(ToolItemType t, int iW, int iH, int wd, int ht, int ID, DWORD hID, const MCHAR* lbl, int ori)
			: ToolItem(t == CTB_PUSHBUTTON ? CTB_PUSHBUTTON : CTB_CHECKBUTTON, ID, hID, wd, ht, ori),
				iw(iW), ih(iH), label(lbl)
		{
			// Use other specialized ToolItem classes to use other ToolItemTypes
			DbgAssert(t == CTB_PUSHBUTTON || t == CTB_CHECKBUTTON);
		}
	//!@}
};
...
The identity of a macro button is determined by an action table id and a command id.\n\n
...
class ToolMacroItem : public ToolItem {
	public:		
		...
		<b>CTB_VERT</b>\n\n
		<b>CTB_FLOAT</b> */
		ToolMacroItem(int wd, int ht, const MacroButtonData* data, int ori = CTB_HORIZ|CTB_VERT|CTB_FLOAT)
			: ToolItem(CTB_MACROBUTTON, 0, 0, wd, ht, ori), md(*data) {}
	};
...
class ToolSeparatorItem : public ToolItem {
	public:
		...
		ToolSeparatorItem(int w, int h = 16, bool vis = true, int ori = CTB_HORIZ | CTB_VERT | CTB_FLOAT)
			: ToolItem(CTB_SEPARATOR, 0, 0, w, h, ori), vis(vis) {}
	};
	...
	public:
		...
		*/
		ToolStatusItem(int w, int h, bool f, int id, DWORD hID = 0, int ori = CTB_HORIZ | CTB_FLOAT)
			: ToolItem(CTB_STATUS, id, hID, w, h, ori), fixed(f) {}
	};
...
class ToolOtherItem : public ToolItem {
	public:
		...
		ToolOtherItem(const MCHAR* cls, int w, int h, int id, DWORD_PTR style = WS_CHILD | WS_VISIBLE,
					int y = CENTER_TOOL_VERTICALLY, const MCHAR* wt = nullptr, DWORD hID = 0, int ori = CTB_HORIZ | CTB_FLOAT)
			: ToolItem(CTB_OTHER, id, hID, w, h, ori), y(y), windowText(wt), className(cls), style(style) {}
};
...
class ToolImageItem : public ToolItem {
	public:
		...
		ToolImageItem(int w, int h, int k, int id, int y = CENTER_TOOL_VERTICALLY, DWORD hID = 0, int ori = CTB_HORIZ | CTB_FLOAT)
			: ToolItem(CTB_IMAGE, id, hID, w, h, ori), y(y), il_index(k) {}
		...
		ToolImageItem(const MSTR& iconName, int w, int h, int id, DWORD hID = 0, int ori = CTB_HORIZ | CTB_FLOAT)
			: ToolItem(CTB_IMAGE, id, hID, w, h, ori), iconName(iconName) {}
	};
...
class ICustToolbar : public ICustomControl {
	public:
		...
		//! \brief This overload was added to allow passing temporary variable to AddTool
		void AddTool(ToolItem&& entry, int pos = -1) { AddTool(entry, pos); }
		...
		//! \brief This overload was added to allow passing temporary variable to AddTool2
		void AddTool2(ToolItem&& entry, int pos = -1) { AddTool2(entry, pos); }
...
class IColorSwatch: public ICustomControl {
	public:
		...
		{ 
			SetUseAlpha(FALSE);
			AColor acolor = SetAColor(AColor(c), notify); 
			return acolor.toRGB();
		} // returns old color
		...
		{
			SetUseAlpha(FALSE);
			AColor acolor = InitAColor(AColor(c), notify);
			return acolor.toRGB();
		} // returns old color
		...
		/** \brief Sets/Gets the context of the color assigned to this swatch. Color
		 * contexts give hint about where and how this color value will be used.
		 * This information may be used to determine if the color conversions
		 * will be used to display the color on the screen. Also color pickers
		 * can use this information to customize UI/UX (for example by limiting
		 * gamut or intensity). 
		 * \see enum class ColorContext
		 */
		virtual void SetColorContext(ColorContext context) {;}
		virtual ColorContext GetColorContext() const { return ColorContext::COLOR_SPACE_CONTEXT_NONE; } 
