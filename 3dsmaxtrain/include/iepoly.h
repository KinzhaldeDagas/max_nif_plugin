...
enum epolyParameters { ep_by_vertex, ep_ignore_backfacing, ep_show_normals,
	...
	ep_edge_chamfer_mitering_type,	// int - mitering type -- quad, tri, or uniform
	ep_edge_chamfer_end_bias,		// float - the end bias amount for the miter
	ep_edge_chamfer_radial_bias,	// float - biasing factor controlling the radius of chamfered corners
	ep_edge_chamfer_depth,			// float - the depth of the chamfer (negative values are concave, 0 is linear, positive values are convex)

    // New for 2021
    ep_vertex_chamfer_num_segments,        // int - number of segments, controlling refinement of the chamfered surface mesh
    ep_vertex_chamfer_depth,               // float - the depth of the chamfer (negative values are concave, 0 is linear, positive values are convex)
    ep_vertex_chamfer_smooth,              // bool - when true, adjusts the smoothing groups of the post-chamfer geometry 
    ep_vertex_chamfer_smooth_type,         // int - controls the smoothing approach (see EP_QUAD_SMOOTH_XXX)
    ep_vertex_chamfer_smooth_threshold,    // float - angle threshold, in degrees, to use when adjusting smoothing groups

    // New for 2021.1
    ep_enable_enhanced_hotkey_extrude,     // bool - when true, enabled postprocessing to clean up geometry and topology during hotkey extrude operations

	// New for 2022.1
	ep_ignore_occluded,					   // bool - ignore occluded sub-objects in selections

    // New for 2023.1
    ep_enable_enhanced_retriangulation     // bool - when true, enables the enhanced triangulation algorithm implemented in MaxSDK::BuilDiagonals
};
...
enum epolyEnumList { epolyEnumButtonOps, epolyEnumCommandModes, epolyEnumPickModes,
	ePolySelLevel, PMeshSelLevel, axisEnum, mapChannelEnum, edgeChamferType
};
...
enum ePolyMethods { epfn_hide, epfn_unhide_all, epfn_named_selection_copy,
	...
	// New for 2021
	epfn_get_edge_chamfer_type, epfn_set_edge_chamfer_type, // Legacy edge chamfer properties support
	epfn_get_edge_chamfer_quad_intersections, epfn_set_edge_chamfer_quad_intersections, // Legacy edge chamfer properties support

	epfn_triangulate //call the MNMesh triangulate method forcing all the polygons to break down into triangle
};
...
//! @{	
	...
	(1-99).
	<b>DWORD flag =MN_SEL</b>\n\n
	The flag used to mark the vertices to be moved (only applies in MN_SEL_VERTEX mode)
	\par Default Implementation:
	<b>{ }</b> */
...
public:
	...
	BEGIN_FUNCTION_MAP_PARENT(EPoly18)
	...
	virtual int EpFnGetEdgeChamferType() { return -1; }
	...
	virtual bool EpFnGetEdgeChamferQuadIntersections() { return FALSE; }
