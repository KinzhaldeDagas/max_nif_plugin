...
#define P_DELETE_LOCAL_NAME_STRING		0x0010000000000000 //!< local_name contains a pointer to a string, delete string when ParamDef is deleted

// Parameter types
#include "paramtype.h"  // parameter type codes

inline ParamType2 base_type(ParamType2 t) { return static_cast<ParamType2>(t & ~TYPE_TAB); } //!< get base type ignoring Tab flag
inline ParamType2 base_type(int t) { return base_type(static_cast<ParamType2>(t)); } //!< get base type ignoring Tab flag
...
//! @{	
	...
	*/
	...
	/*! \remarks Retrieves the value of the specified parameter at the specified time.
	\param id The permanent ID of the parameter.
	\param t The time at which to get the value.
	\param v The value to retrieve is returned here.
	\param tabIndex If the parameter is a Tab\<\> this is the zero based index into the
	table of the value to get.
	\return  TRUE on success; otherwise FALSE.
	\note Use this overload when validity interval is irrelevant
	*/
	template <typename ValueType>
	inline BOOL GetValue(ParamID id, TimeValue t, ValueType& v, int tabIndex = 0);
	...
	template <typename ValueType>
	inline BOOL GetValue(ParamID id, TimeValue t, ValueType& v, Interval&&, int tabIndex = 0);
//! @}
...
//! @{	
	...
	ParamDimension* GetParamDimension(int subAnim) override = 0;
...
//! @{	
	...
	If the parameter block is not a multimap parameter block (the P_MULTIMAP flag of its descriptor is false) and you pass a nonzero map_id,
	the map_id will be converted to zero because the parameter block only contains one map. */
...
//! @{	
	// Get the validity of all params in the paramblock.
...
//! @}
	...
	template <typename T>
	bool GetValueByName(
			const MCHAR* const paramName, const TimeValue t, T& value, Interval& validity, const int tabIndex = 0);
...
public:
	...
	\param v - The value being accessed.
	\param owner - Points to the owner of the parameter block.
	\param id - The permanent ID of the parameter.
	\param tabIndex - If the parameter is a Tab\<\> this is the zero based index of the parameter
	in the table.
	\param t - The current time the get is taking place.\n\n
	\param valid - The validity interval to update.  
	  The intersection of the provided "valid" interval with the 
	  interval for the provided TimeValue and returned as the updated interval value.
...
struct ParamDef: public MaxHeapOperators
{
	...
	/*! This is the fixed non-localized (English) name of the parameter. This name is used 
	by script engines for SubAnim name resolution for when the script specifies the SubAnim
	name in English but 3ds Max is running with a non-English language pack.
	By default, this contains the same pointer as specified by int_name. If the specified internal 
	name is not the same as the English name, the non-localized name should be specified
	using the p_nonLocalizedName paramtag.*/
	const MCHAR* nonLocalized_name; // non-localized (subamin) name if different than int_name
...
private:
	...
	IAutoMParamDlg* mainMDlg;				// main material/mapParamDlg if any
	IAutoEParamDlg* mainEDlg;				// main EffectParamDlg if any
	IAutoRendererParamDlg* mainRDlg;		// main render dialog if any
...
protected:
	...
	\param [in] dlg - The new main Material dialog pointer*/
	void SetMParamDlg(IAutoMParamDlg* dlg) { mainMDlg = dlg; }

	//! \brief Sets the root level effect ParamDlg.
	/*!	This method may be called by derived classes that want
	to handle the effect ParamDlg creation themselves,
	instead of using one of the "paramdlg" creation methods of class ClassDesc2.
	\param [in] dlg - The new main effect dialog pointer */
	void SetEParamDlg(IAutoEParamDlg* dlg) { mainEDlg = dlg; }
...
public:
	/*! \remarks Constructor.  The main ParamDlg and Effect
	...
	class. */
	PB2Export int			NumParamBlockDescs() override { return pbDescs.Count(); }
	/*! \remarks Implemented by the System.\n\n
	This is a method of the base class ClassDesc. This class provides an
	implementation of the method used by plug-ins using the ParamBlock2
	system.\n\n
	...
	PB2Export ParamBlockDesc2*	GetParamBlockDesc(int i) override { return pbDescs[i]; }
	...
	PB2Export int			NumParamMaps() override { return paramMaps.Count(); }
	/*! \remarks Implemented by the System.\n\n
	This is a method of the base class ClassDesc. This class provides an
	...
	*/
	PB2Export IParamMap2*	GetParamMap(int i) override { return paramMaps[i]; }
	...
	inline bool SetUserDlgProc(ParamBlockDesc2* pbd, ParamMap2UserDlgProc* proc = nullptr) override { return SetUserDlgProc(pbd, 0, proc); }
	...
	AutoMParamDlgs for them. The first one becomes the main and the others are
	added to it.
	...
	becomes the main and the others are added to it.
	...
	/*! \remarks Implemented by the System.\n\n
	Returns the main dialog processing routine for the materials editor
	plug-in. */
	PB2Export IAutoMParamDlg* GetMParamDlg() { return mainMDlg; }
	/*! \remarks Implemented by the System.\n\n
	Returns the main dialog processing routine for the rendering effects
	plug-in. */
	PB2Export IAutoEParamDlg* GetEParamDlg() { return mainEDlg; }
	...
	/*!
	   \details \copydetails ClassDesc2::CreateQtWidget(ReferenceMaker&,IParamBlock2&,const MapID,MSTR&,int&,int&)

	   You should only override this method if you want to add a Qt widget to the Render Settings dialog. Otherwise,
	   you should override CreateQtWidget(ReferenceMaker& owner, IParamBlock2& paramBlock, const MapID paramMapID, MSTR&
	   rollupTitle, int& rollupFlags, int& rollupCategory).\n\n 
	   
	   \param [out] tabID Output parameter used to specify the
	   Render Settings dialog tab in which the Qt widget is to be added. To add a Qt widget as a new tab to the Render
	   Settings dialog, assign a unique Class ID to this parameter before returning. To obtain a unique Class ID, run
	   the 3ds Max Class ID generator program, gencid.exe, which is located in your \\maxsdk\\help\\ directory. 
	   \remark
...
public:
	...
	p_nonLocalizedName, _T("Segments"),
	end,
	...
	*/
	PB2Export bool ParamOptionContentValues(ParamID id, Tab<int>& stringItems, Tab<int>* stringItemValues = nullptr);
	...
	of the value to set.
	\return  TRUE on success; otherwise FALSE. */
	BOOL SetValue(ParamID id, TimeValue t, const Point2& v, int tabIndex = 0) { return class_params->SetValue(id, t, v, tabIndex); }
	BOOL SetValue(ParamID id, TimeValue t, const Point3& v, int tabIndex = 0) { return class_params->SetValue(id, t, v, tabIndex); }
	BOOL SetValue(ParamID id, TimeValue t, const Point4& v, int tabIndex = 0) { return class_params->SetValue(id, t, v, tabIndex); }
	/*! \remarks This method is used with static class parameter blocks
	only.\n\n
	...
	of the value to set.
	\return  TRUE on success; otherwise FALSE. */
	BOOL SetValue(ParamID id, TimeValue t, const Color& v, int tabIndex = 0) { return class_params->SetValue(id, t, v, tabIndex); }  // uses Point3 controller
	BOOL SetValue(ParamID id, TimeValue t, const AColor& v, int tabIndex = 0) { return class_params->SetValue(id, t, v, tabIndex); }  // uses Point4 controller
	/*! \remarks This method is used with static class parameter blocks
	only.\n\n
	...
	of the value to set.
	\return  TRUE on success; otherwise FALSE. */
	BOOL SetValue(ParamID id, TimeValue t, const Matrix3& v, int tabIndex = 0) { return class_params->SetValue(id, t, v, tabIndex); }
	...
	description. */
	PB2Export void SetUserDlgProc(MapID map_id, ParamMap2UserDlgProc* proc = nullptr);
	...
	inline void SetUserDlgProc(ParamMap2UserDlgProc* proc = nullptr){ SetUserDlgProc(0, proc); }
...
PB2Export IParamBlock2* UpdateParameterBlock2(
		...
		ParamBlockDesc2* pdescNew, IParamBlock2* newPB = nullptr);
...
PB2Export IParamBlock2* UpdateParameterBlock2(
	...
	Tab<DWORD>& subAnimRenumbering, IParamBlock2* newPB = nullptr);
...
PB2Export int CopyParamBlock2ToParamBlock(
		IParamBlock2* pb2, IParamBlock* pb1, ParamBlockDescID* pdescPB1, int pb1Count);
...
public:
	...
	PB2Export void proc(ILoad* iload) override;
	int Priority() override	{ return 0; }
	PB2Export INT_PTR Execute(int cmd, ULONG_PTR arg1 = 0, ULONG_PTR arg2 = 0, ULONG_PTR arg3 = 0) override;
};
...
public:
	...
	Interface_ID			GetID() override { return IPARAMBLOCK2POSTLOADINFO_ID; }
