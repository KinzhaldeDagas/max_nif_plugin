...
struct ActionDescription
{
	/** A unique identifier for the command (must be unique per table).
		...
		\note This should be a 16 bit value (in the range of 0-65,535) as it is also used as a Windows
		accelerator command ID for keyboard shortcuts, which is a WORD (16 bit unsigned short).
	*/
	int mCmdID;
...
/** Represents an operation contained in an ActionTable. 
	...
	Class IActionManager, Class MAXIcon, Class Interface, class FPMixinInterface
*/
...
public:
	...
	*/
	...
	If an action supports AltExecute, then a mouse icon (with a dark click button) appears in the quad menu
	item that corresponds to the action. When the icon is clicked, the AltExecute method is called.*/
	virtual bool SupportsAltExecute() const { return false; }
	/** Returns true if AltExecute ran successfully.
	If an action supports AltExecute, then a mouse icon (with a dark click button) appears in the quad menu
	item that corresponds to the action. When the icon is clicked, the AltExecute method is called.*/
	virtual bool AltExecute() { return false; }
	/** Returns true if the action supports CloseDialogs. This is called by the internal implementation of
	IActionItemToggleManager::FlipToggleState to close a dialog if when executing the action the action
	was registered via IActionItemToggleManager::RegisterToggleAction. If true, the CloseDialogs method 
	is called when toggling the action state off.*/
	virtual bool SupportsCloseDialogs() const { return false; }
	/** Returns true if CloseDialogs ran successfully.*/
	virtual bool CloseDialogs() { return false; }
	...
	/** Retrieves the persistent action id text. This can be used to store a reference 
	to an action into a file, like it is done by the menu system.
	\param[out] persistentActionId - The retrieved text. */
	CoreExport void GetPersistentActionId(MSTR& persistentActionId);

	/** Retrieves the persistent action id text. This can be used to store a reference
	to an action into a file, like it is done by the menu system.
	\return The persistent action id text. */
	...
	*/
	...
	/** If the ActionItem does produce a dynamic menu (i.e. IsDyamicAction() returns true),
	this method is called by the system to populate the new menu currently being created to add
	items, separators, and submenus. Each item is given an arbitrary id (see ICuiDynamicMenu interface).
	Default implementation is a no-op.
	\param dynMenu - Pointer to dynamic menu interface. Pointer is owned by the system.
	...
	If the item is used from a menu bar, this hwnd will be nullptr.
	\param cursorPos - If the menu is requested by a right-click quad menu, then this parameter is
	the point in the window where the user clicked.
	\sa class ICuiDynamicMenu
	*/
	CoreExport virtual void PopulateDynamicMenu(MaxSDK::CUI::ICuiDynamicMenu* dynMenu, HWND hWnd, const IPoint2& cursorPos);
	/** If the ActionItem does produce a menu (i.e. IsDyamicAction() returns true),
	this method is called by the system when user clicks a menu item thas was added in PopulateDynamicMenu.
	Default implementation is a no-op.
	\param itemId - id of menu item that was clicked by user. Use this id to execute appropriate action.
		This id is passed to ICuiDynamicMenu functions when populating the menu (for example: AddItem(itemCmdId, ...)).
	\sa class ICuiDynamicMenu
	*/
	CoreExport virtual void DynamicMenuItemSelected(int itemId);
...
protected:
	...
	*/
	...
	// Function publishing helper functions
	MSTR fpGetDescriptionText()
	{
		MSTR res;
		GetDescriptionText(res);
		return res;
	}
	...
	MSTR mTooltipKey;
	MaxSDK::Util::Path mIconPath[kIconSizeCount] = {};
	MSTR mMetaData;
};
...
\sa Class FPMixinInterface,  Class ClassDesc, Structure ActionDescription, Class ActionItem, Class ActionCallback, Class ActionContext, Class IActionManager, Class Interface.
*/
...
/** Returns TRUE if the menu item should be indeterminate	
	...
	\param outIdString The string where the action item ID is written to. */
	CoreExport virtual void WritePersistentActionId(int cmdId, MSTR& outIdString);
	...
	*.CUI or *.KBD file to a numeric identifier. 
	...
	CoreExport virtual int ReadPersistentActionId(const MSTR& idString);
...
public:
	...
	*/
	...
	Default implementation is a no-op.
	\param dynMenu - Pointer to dynamic menu interface. Pointer is owned by the system.
	\param actionId - Id of ActionItem that produces the dynamic menu
	\param hWnd - If the menu is requested by a right-click quad menu, 
	then this hWnd is the handle of the window where the click occurred. 
	If the item is used from a menu bar, this hwnd will be nullptr.
	\param cursorPos - If the menu is requested by a right-click quad menu, then this parameter is
	...
	\sa class ICuiDynamicMenu
	*/
	CoreExport virtual void PopulateDynamicMenu(MaxSDK::CUI::ICuiDynamicMenu* dynMenu, int actionId, HWND hWnd, const IPoint2& cursorPos);
	/** If the ActionItem does produce a dynamic menu (i.e. IsDyamicAction() returns true),
	this method is called by the system when user clicks on a menu item that was added in PopulateDynamicMenu.
	Default implementation is a no-op.
	\param actionId - Id of ActionItem that produced the dynamic menu
	\param itemId - id of menu item that was clicked by user. Use this id to execute appropriate action.
		This id is passed to ICuiDynamicMenu functions when populating the menu (for example: AddItem(itemCmdId, ...)).
	\sa class ICuiDynamicMenu
	*/
	CoreExport virtual void DynamicMenuItemSelected(int actionId, int itemId);
...
/** 
An ActionContext is an identifier of a group of keyboard shortcuts. Examples are
...
public:
	...
	\return A pointer to the ActionContext or NULL if it not found.
	*/
	...
	*/
	...
	Note that just the user customized shortcuts will be serialized.
	The newly stored shortcut file will become the current selected shortcut set, which is
	used on 3ds Max startup shortcut loading or when the <b>Customize Shortcuts</b> dialog is opened.
	\param pFileName The path of the file in which the keyboard shortcuts should be stored.
	The file must have the .hsx extension.
	\return TRUE if the file could be successfully saved; FALSE if not.
	\see LoadKeyboardFile, GetKeyboardFile
	*/
	...
	The loaded shortcut file will become the current selected shortcut set, which is used on
	3ds Max startup shortcut loading or when the <b>Customize Shortcuts</b> dialog is opened.
	\param pFileName The path of the file from which the keyboard shortcuts should be loaded.
	The file must have the .hsx extension.
	\return TRUE if the file could be successfully loaded; FALSE if not.
	\see SaveKeyboardFile, GetKeyboardFile
	*/
	virtual BOOL LoadKeyboardFile( const MCHAR* pFileName ) = 0;

	/** Returns the full path to the current keyboard shortcuts file.
	This can be a shortcut factory preset or a user customized shortcut set.
	The returned file will have the .hsx file extension.
	\see LoadKeyboardFile, SaveKeyboardFile
	*/
