...
class INode: public ReferenceTarget, public FPMixinInterface {
	public:
		...
		// Structure definition for use in Get/SetUserPropsAndValues
		struct MSTRKeyValuePair
		{
			bool operator==(const MSTRKeyValuePair& other) const
			{
				return mKey == other.mKey && mValue == other.mValue;
			}
			MSTR mKey; //!< User Property key
			MSTR mValue; //!< User Property value
		};
...
//! @{
		...
		/*! \remarks This method returns an array of key/value pairs. */
		virtual MaxSDK::Array<INode::MSTRKeyValuePair> GetUserPropsAndValues() const = 0;

		/*! \remarks This method sets the user property buffer based on an array of key/value pairs.
...
//! @{		
		...
		*/
		...
		/*! \remarks This method tells if the INode is a scene  node or not.
		\return true if the node is a scene node, false otherwise.
		*/
...
//! @{		
		...
		/*! \remarks Returns TRUE if the bone property is on; FALSE if off.
		\par Default Implementation:
		<b>{return FALSE;}</b> */
		virtual BOOL GetBoneNodeOnOff() {return FALSE;}
		
		/*! \remarks Returns TRUE if the bone property is on at a specfied time; FALSE if off.
		\param t The time value at which to check the bond node.
		\par Default Implementation:
		<b>{return FALSE;}</b> */
		...
		virtual Matrix3 GetStretchTM(TimeValue t, Interval *valid = NULL) { return Matrix3{}; }
...
/*! An overlay XRef will be loaded only if it is a direct descendant of the main file.  This lets the user bring in files to compare against but other users do not see it if they XRef this file. */
...
class INodeTransformed : public INode {
	public:
		...
		void 	SetNodeTM(TimeValue t, const Matrix3& m) override { node->SetNodeTM(t, m); }
		...
		MaxSDK::Array<MSTRKeyValuePair> GetUserPropsAndValues() const {return node->GetUserPropsAndValues();}
		void SetUserPropsAndValues(MaxSDK::Array<MSTRKeyValuePair> pairs) {node->SetUserPropsAndValues(pairs);}
		...
		virtual void GetClassName(MSTR& s, bool localized = true) const override { UNUSED_PARAM(localized); s = _M("INodeTransformed"); } // from Animatable
