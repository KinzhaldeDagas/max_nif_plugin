...
/** Returns the unsigned distance from the 2D point \c x, \c y to the 2D line 
    segment defined by <code>p1-\>p2</code>.
	\param x The x coordinate of the 2D point.
	\param y The y coordinate of the 2D point.
	\param p1 The first end point of the 2D line segment.
	\param p2 The second end point of the 2D line segment.
    A line segment is the finite segment between two end points \c p1 \c p2, 
	different from the infinite line used in distToLine.
	Notice that this routine returns unsigned distance, which means not matter 
	on which side of the line the point is, the returned value is always 
	non-negative.
 */
...
/** Returns z depth at closest point in the segment defined by p1->p2 to x,y
    v = the vector from p1 to p2
    w = the vector from p1 to q=(x,y)
    t = (w dot v) / (v dot v)
	t = clamp(t, 0.0, 1.0)
