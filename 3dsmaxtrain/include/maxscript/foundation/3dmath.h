...
extern ScripterExport void _QuatToEuler(const Quat& q, float* ang);
extern ScripterExport void _EulerToQuat(const float* ang, Quat& q);
...
#	include "..\protocols\vector.inl"
	...
	Point3		to_point3() override { return p; }
	AColor		to_acolor() override { return AColor(p.x / 255.0f, p.y / 255.0f, p.z / 255.0f); }
	Point2		to_point2() override { return Point2 (p.x, p.y); }
	void		to_fpvalue(FPValue& v) override;
	COLORREF	to_colorref() override { return RGB((int)p.x, (int)p.y, (int)p.z); }
...
public:
	...
	Value* set_x(Value** arg_list, int count) override { throw RuntimeError(_M("Constant vector, not settable")); return nullptr; }
	Value* set_y(Value** arg_list, int count) override { throw RuntimeError(_M("Constant vector, not settable")); return nullptr; }
	Value* set_z(Value** arg_list, int count) override { throw RuntimeError(_M("Constant vector, not settable")); return nullptr; }
};
...
#	include "..\protocols\quat.inl"
	...
	Quat		to_quat() override { return q; }
	AngAxis		to_angaxis() override { return AngAxis(q); }
	void		to_fpvalue(FPValue& v) override;
...
#include "../macros/define_implementations.h"
	...
	AngAxis		to_angaxis() override { return aa; }
	Quat		to_quat() override { return Quat(aa); }
	void		to_fpvalue(FPValue& v) override;
...
#include "../macros/define_implementations.h"
	...
	AngAxis		to_angaxis() override { return AngAxis(to_quat()); }
	Quat		to_quat() override { Quat q; _EulerToQuat(angles, q); return Quat(q); }
	void		to_fpvalue(FPValue& v) override;
	float*		to_eulerangles() override { return angles; }
...
#	include "..\protocols\matrix.inl"
	...
	Matrix3&	to_matrix3() override { return m; }
	Quat		to_quat() override { return Quat(m); }
	void		to_fpvalue(FPValue& v) override;
...
#include "../macros/define_implementations.h"
	...
	Point2		to_point2() override { return p; }
	void		to_fpvalue(FPValue& v) override;
...
#	include "..\protocols\vector.inl"
	...
	Point4		to_point4() override { return p; }
	Point3		to_point3() override { return Point3 (p.x, p.y, p.z); }
	AColor		to_acolor() override { return AColor(p.x, p.y, p.z, p.w); }
	Point2		to_point2() override { return Point2 (p.x, p.y); }
	void		to_fpvalue(FPValue& v) override;
	COLORREF	to_colorref() override { return RGB((int)(p.x*255.f), (int)(p.y*255.f), (int)(p.z*255.f)); }
...
#pragma push_macro("new")
#undef new
	ENABLE_STACK_ALLOCATE(Box2Value);
#pragma pop_macro("new")
...
#	include "..\protocols\box.inl"
	...
	Box2&		to_box2() override { return b; }
};

// End of 3ds max 4.2 Extension

// The following class has been added
// in 3ds max 2016.  If your plugin utilizes this new
// mechanism, be sure that your clients are aware that they
// must run your plugin with 3ds max version 2016 or higher.

// Wraps Box3 in SDK
applyable_class_debug_ok(Box3Value)

class Box3Value : public Value
{
public:
	Box3 b;
...
#pragma push_macro("new")
#undef new
	ENABLE_STACK_ALLOCATE(Box3Value);
#pragma pop_macro("new")

	ScripterExport	Box3Value();
	ScripterExport	Box3Value(const Box3& box);	
	ScripterExport	Box3Value(const Point3& pmin, const Point3& pmax);
	ScripterExport	Box3Value(Value* pmin, Value* pmax);

	classof_methods(Box3Value, Value);
	void			collect() override;
	void	sprin1(CharStream* s) override;	
#	define			is_box3(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(Box3Value))
	Box3			to_box3() override { return b; }
