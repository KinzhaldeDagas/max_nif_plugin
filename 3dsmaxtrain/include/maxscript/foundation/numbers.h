...
#  define  integer_type_check(v, where) if (!Number::is_integer_number(v)) type_check(v, Integer, where)
...
// When converted this way it is the preferred Value type checking macro over using type_check macro.
#  define  float_type_check(v, where) if (!Number::is_float_number(v)) type_check(v, Float, where)

	static constexpr bool is_number(Value* v) { return (is_integer_number(v) || is_float_number(v)); }
...
inline constexpr bool Number::is_integer_number(Value* v)
{
	return ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(Integer) ||
			(v)->tag == class_tag(Integer64) || (v)->tag == class_tag(IntegerPtr));
}

inline constexpr bool Number::is_float_number(Value* v)
{
	return ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(Float) ||
