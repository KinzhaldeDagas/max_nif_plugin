...
{
public:
	enum key_type { key_name, key_string, key_int, key_maxclass, key_type_count};
...
#	define	is_dictionary(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MXSDictionaryValue))
	...
	// hashtable hash and compare fns for Value* keys
	static DWORD_PTR value_hash_fn(const void* key);
	static int dict_eq_fn_value(const void* key1, const void* key2);
