...
// Collectable::flags3 - bit 0 set on HashTable if it is the outmost scope of a structure definition.
// this is used by parser to detect case of nested structure definitions where inner definition is directly 
// referencing variables in outer definition.
...
public:
   ...
   using hash_fn_type = DWORD_PTR (*)(const void*);
   using eq_fn_type = int (*)(const void *, const void *);
...
private:
	...
	void			free(); // frees memory associated with table
...
public:
	...
	ScripterExport virtual ~HashTable();
	ScripterExport void		init(UINT primary_size, eq_fn_type key_eq_fn, hash_fn_type key_hash_fn, int vflags);
#	define		is_hashtable(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(HashTable))
	...
	ScripterExport virtual Value*	get(const void* key) const;
	ScripterExport Value*	put(const void* key, const void* val);
	ScripterExport Value*	put_new(const void* key, const void* val);
	ScripterExport Value*	find_key(const void *val) const;
	ScripterExport Value*	set(const void* key, const void* val);
	ScripterExport void		remove(const void* key);
	ScripterExport void		map_keys_and_vals(void (*fn)(const void* key, const void* val, void* arg), void* arg) const;
	ScripterExport void		map_keys_and_vals(HashTabMapper* mapper) const;
	ScripterExport int		num_entries() const { return n_entries; }

	ScripterExport void		reinit(); // frees memory and then re-initializes primary extent
