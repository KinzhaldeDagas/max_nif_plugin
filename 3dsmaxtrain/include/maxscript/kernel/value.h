...
//! \endcond
	...
	/*! \remarks Compare whether 2 values are equivalent.
	The ValuesEqual method is used by the deepEqual function to test to see if two values are equivalent. This
	allows, for example, testing to see if 2 arrays are equivalent. (The == operator on arrays just tests to see if the
	two arrays are the same array).
	Classes that implement this method are Value, Array, DataPair, and MXSDictionary
	\param other The value to compare against this value
    \param strictCompare If true, the type of 'other' must be the same as this value. If false, 'other' must just be comparable
	with this value using the eq_vf method. So, for example, Integer value 1 and Float value 1 would compare as false if strictCompare 
	is true, and true if strictCompare is false.
	\return True if the value is equivalent, false if not.
	...
	\code
		...
		\endcode
	*/
	ScripterExport virtual bool ValuesEqual(Value* other, bool strictCompare);
