...
public:
	...
	MSTR line;
	MSTR file;
	CompileError (const MSTR& d, const MSTR& i, const MSTR& l, const MSTR& f = _T(""));
	CompileError () = default;
	~CompileError ();

	void	sprin1(CharStream* s) const override;
	void	set_file(const MSTR& f);
};
...
class ScripterExport SyntaxError : public CompileError
{
	...
	MSTR got;
public:
	SyntaxError (const MSTR& w, const MSTR& g, const MSTR& l = _T(""), const MSTR& f = _T(""));
	~SyntaxError ();
...
class ScripterExport TypeError : public MAXScriptRuntimeErrorException
{
	...
	MSTR description;
public:
			TypeError (const MSTR& d, Value* t, ValueMetaClass* c = nullptr);
		   ~TypeError ();
...
class ScripterExport NoMethodError : public MAXScriptRuntimeErrorException
{
	...
	MSTR fn_name;
public:
			NoMethodError (const MSTR& fn, Value* t);
		   ~NoMethodError ();
...
class ScripterExport ArgCountError : public MAXScriptRuntimeErrorException
{
	...
	MSTR	fn_name;
public:
			ArgCountError (const MSTR& fn, int w, int g);
		   ~ArgCountError ();
...
public:
			...
			RuntimeError (const MSTR& d1);
			RuntimeError (const MSTR& d1, const MSTR& d2);
			RuntimeError (const MSTR& d1, Value* ii);
			RuntimeError (const MSTR& d1, const MSTR& d2, Value* ii);
			RuntimeError (Value* ii);
		   ~RuntimeError ();

	void	init(const MSTR& d1, const MSTR& d2, Value* ii);
	void	sprin1(CharStream* s) const override;
};
...
public:
			...
			ValueHolderMember  info;
			BOOL	debugBreak;
			UserThrownError (const MSTR& d1, Value* ii, BOOL dbgBreak);
			UserThrownError (const MSTR& di, BOOL dbgBreak);
		   ~UserThrownError ();

	void	init(const MSTR& d1, Value* ii, BOOL dbgBreak);
	void	sprin1(CharStream* s) const override;
};
...
public:
	...
	MSTR desc2;
	ValueHolderMember  info;
	DebuggerRuntimeError (const MSTR& d1);
	DebuggerRuntimeError (const MSTR& d1, const MSTR& d2);
	DebuggerRuntimeError (const MSTR& d1, Value* ii);
	DebuggerRuntimeError (const MSTR& d1, const MSTR& d2, Value* ii);
...
class ScripterExport ConversionError : public MAXScriptRuntimeErrorException
{
	...
	MSTR type;
public:
			ConversionError (Value* v, const MSTR& t);
		   ~ConversionError ();
...
{
	friend ScripterExport void ProcessMAXScriptException(MAXScriptException& e, const MSTR& caption, bool displayErrorMessageBox, bool showSourcePosition, bool isTerminalHandling, bool quietErrors);
public:
	SecurityException(const MSTR& msg, const MSTR& command = _T(""));
	~SecurityException();
...
inline void ProcessUnknownMAXScriptException(const MSTR& caption, bool displayErrorMessageBox, bool showSourcePosition, bool isTerminalHandling, bool quietErrors = false)
{
