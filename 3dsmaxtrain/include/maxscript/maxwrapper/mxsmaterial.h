/*	
 *		MAXMaterials.h - MAX material & map wrapper classes
 *
 *			Copyright (c) John Wainwright 1996
 *
 */

#pragma once

#include "mxsobjects.h"
#include "../../imtl.h"
// forward declarations
class MultiMtl;


/* -------------------------- MAXMaterial -------------------------- */

visible_class (MAXMaterial)

class MAXMaterial : public MAXWrapper
{
public:
	Mtl*		mat;				/* the MAX-side material	*/

				MAXMaterial() {};
				MAXMaterial(Mtl* imat);
	static ScripterExport Value* intern(Mtl* imat);

	static Value* make(MAXClass* cls, Value** arg_list, int count);

	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXMaterial)) ? 1 : MAXWrapper::is_kind_of(c); }
	void		collect() override;
	ScripterExport void sprin1(CharStream* s) override;
	const MCHAR*		class_name() override;

#ifndef DOXYGEN
#include "../macros/define_implementations.h"
#endif
	Value*		copy_vf(Value** arg_list, int count) override { return MAXWrapper::copy_no_undo(arg_list, count); }

	def_property   ( name );
	def_property   ( effectsChannel );
	def_property   ( showInViewport );
	Value*		get_property(Value** arg_list, int count) override;
	Value*		set_property(Value** arg_list, int count) override;

	Mtl*		to_mtl() override { check_for_deletion(); return mat; }
	MtlBase*	to_mtlbase() override { check_for_deletion(); return mat; }
	void		to_fpvalue(FPValue& v) override { v.mtl = mat; v.type = TYPE_MTL; }
};

/* ---------------------- MAXMultiMaterial ----------------------- */

visible_class (MAXMultiMaterial)

class MAXMultiMaterial : public MAXMaterial
{
public:

				MAXMultiMaterial() {};
				MAXMultiMaterial(MultiMtl* imat);
	static ScripterExport Value* intern(MultiMtl* imat);

	static Value* make(MAXClass* cls, Value** arg_list, int count);

	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXMultiMaterial)) ? 1 : MAXMaterial::is_kind_of(c); }
	void		collect() override;
	ScripterExport void sprin1(CharStream* s) override;
	const MCHAR*		class_name() override;

	def_generic (get, "get");
	def_generic (put, "put");
	Value*		copy_vf(Value** arg_list, int count) override { return MAXWrapper::copy_no_undo(arg_list, count); }

	ScripterExport Value* map(node_map& m) override;

	def_property( numsubs );
	def_property( count );

	Mtl*		to_mtl() override { return mat; }
	MtlBase*	to_mtlbase() override { check_for_deletion(); return mat; }
	void		to_fpvalue(FPValue& v) override { v.mtl = mat; v.type = TYPE_MTL; }
};

/* ---------------------- Material Library ----------------------- */

applyable_class (MAXMaterialLibrary)

class MAXMaterialLibrary : public MAXWrapper
{
public:
	MtlBaseLib*	lib = nullptr;

				MAXMaterialLibrary(MtlBaseLib* ilib = nullptr);
	static ScripterExport Value* intern(MtlBaseLib* ilib); // will create a MtlBaseLib if passed null;
#	define		is_materiallibrary(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MAXMaterialLibrary))

				classof_methods (MAXMaterialLibrary, MAXWrapper);
	void		collect() override;
	ScripterExport void sprin1(CharStream* s) override;
	MtlBase*	get_mtlbase(int index);
	MtlBase*	find_mtlbase(const MCHAR* name);
	const MCHAR*		class_name() override;

	/* operations */
	
	ScripterExport Value* map(node_map& m) override;

#ifndef DOXYGEN
#include "../macros/define_implementations.h"
#endif
#	include "..\protocols\arrays.inl"
	Value*		copy_vf(Value** arg_list, int count) override { return MAXWrapper::copy_no_undo(arg_list, count); }

	/* built-in property accessors */

	def_property ( count );
};

/* ------------------------- MAXTexture ------------------------ */

visible_class (MAXTexture)

class MAXTexture : public MAXWrapper
{
public:
	Texmap*	map;				/* the MAX-side map	*/

				MAXTexture(Texmap* imap);
	static ScripterExport Value* intern(Texmap* imap);

	static Value* make(MAXClass* cls, Value** arg_list, int count);

	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXTexture)) ? 1 : MAXWrapper::is_kind_of(c); }
	void		collect() override;
	ScripterExport void sprin1(CharStream* s) override;
	const MCHAR*		class_name() override;

#ifndef DOXYGEN
#include "../macros/define_implementations.h"
#endif
#	include "..\protocols\textures.inl"
	Value*		copy_vf(Value** arg_list, int count) override { return MAXWrapper::copy_no_undo(arg_list, count); }

	def_property( name );

	Texmap*		to_texmap() override { check_for_deletion(); return map; }
	MtlBase*	to_mtlbase() override { check_for_deletion(); return map; }
	void		to_fpvalue(FPValue& v) override { v.tex = map; v.type = TYPE_TEXMAP; }
};

/* -------------------------- MAXMtlBase -------------------------- */

// a generic wrapper for MtlBase classes such as UVGen, XYZGen, TexOut, etc.

visible_class (MAXMtlBase)

class MAXMtlBase : public MAXWrapper
{
public:
	MtlBase*	mtl;				/* the MAX-side mtlbase	*/

				MAXMtlBase(MtlBase* imtl);
	static ScripterExport Value* intern(MtlBase* imtl);

	static Value* make(MAXClass* cls, Value** arg_list, int count);

	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXMtlBase)) ? 1 : MAXWrapper::is_kind_of(c); }
	void		collect() override;
	ScripterExport void sprin1(CharStream* s) override;
	const MCHAR*		class_name() override { return _M("MapSupportClass"); }
	Value*		copy_vf(Value** arg_list, int count) override { return MAXWrapper::copy_no_undo(arg_list, count); }

	MtlBase*	to_mtlbase() override { check_for_deletion(); return mtl; }
};

/* ------------------ MEdit materials virtual array -------------------- */

visible_class (MAXMeditMaterials)

class MAXMeditMaterials : public MAXWrapper
{
public:
				MAXMeditMaterials();
				~MAXMeditMaterials();

				classof_methods (MAXMeditMaterials, MAXWrapper);
	void		collect() override;
	ScripterExport void sprin1(CharStream* s) override;
	MtlBase*	get_mtl(int index);
	MtlBase*	find_mtl(const MCHAR* name);
	static void setup();
	const MCHAR*		class_name() override;

	// operations
	ScripterExport Value* map(node_map& m) override;

#ifndef DOXYGEN
#include "../macros/define_implementations.h"
#endif
#	include "..\protocols\arrays.inl"

	// built-in property accessors
	def_property ( count );
};

extern ScripterExport MAXMeditMaterials* medit_materials;

