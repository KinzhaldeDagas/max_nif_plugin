...
#pragma pop_macro("new")
	...
	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXWrapper)) ? 1 : Value::is_kind_of(c); }
	ScripterExport BOOL		not_on_undo_stack();
	void					collect() override;
	ScripterExport void		gc_trace() override;
	virtual	const MCHAR* class_name() = 0;
	virtual ReferenceTarget* get_max_object() { return (NumRefs()) ? GetReference(0) : nullptr; }
	ScripterExport Value*		copy_no_undo(Value** arg_list, int count);
	BOOL		derives_from_MAXWrapper() override { return TRUE; }
	Value*		deep_copy(HashTable* /* remapper */) override { return this; }

	ScripterExport void make_ref(int ref_no, ReferenceTarget* ref);
	void		drop_MAX_refs() override { DeleteAllRefsFromMe(); }

    // Animatable
	Class_ID ClassID() override { return Class_ID(MAXSCRIPT_WRAPPER_CLASS_ID, 0); }
	SClass_ID SuperClassID() override { return MAXSCRIPT_WRAPPER_CLASS_ID; }
	void GetClassName(MSTR& s, bool localized = true) const override { UNUSED_PARAM(localized); s = _M("MAXWrapper"); } // From Animatable


	// ReferenceMaker
	int			NumRefs() override { return refs.Count(); }
	RefTargetHandle GetReference(int i) override { return refs[i]; }
protected:
	ScripterExport void			SetReference(int i, RefTargetHandle rtarg) override;
public:
	...
	BOOL CanTransferReference(int i) override { return FALSE; } 
...
//			def_generic (dependsOn,	 "dependsOn");
	...
	virtual Control* get_max_pos_controller(ParamDimension** pdim) { return nullptr; }
	virtual Control* get_max_scale_controller(ParamDimension** pdim) { return nullptr; }
	virtual Control* get_max_rotation_controller(ParamDimension** pdim) { return nullptr; }
	virtual Control* get_max_tm_controller(ParamDimension** pdim) { return nullptr; }
	virtual Control* get_max_controller(ParamDimension** pdim) { return nullptr; }
	...
	ScripterExport virtual Mesh* set_up_mesh_access(int access, ReferenceTarget** owner = nullptr);
	ScripterExport virtual Mesh* set_up_mesh_face_access(int index, int access, ReferenceTarget** owner = nullptr);
	ScripterExport virtual Mesh* set_up_mesh_vertex_access(int index, int access, ReferenceTarget** owner = nullptr);
	ScripterExport virtual MNMesh* set_up_mnmesh_access(int access, ReferenceTarget** owner = nullptr);
	ScripterExport virtual MNMesh* set_up_mnmesh_face_access(int index, int access, ReferenceTarget** owner = nullptr);
	ScripterExport virtual MNMesh* set_up_mnmesh_vertex_access(int index, int access, ReferenceTarget** owner = nullptr);
	...
	ScripterExport virtual PatchObject* set_up_patch_access(int access, ReferenceTarget** owner = nullptr);
	ScripterExport virtual PatchObject* set_up_patch_face_access(int index, int access, ReferenceTarget** owner = nullptr);
	ScripterExport virtual PatchObject* set_up_patch_vertex_access(int index, int access, ReferenceTarget** owner = nullptr);
	...
	ReferenceTarget* to_reftarg() override { return get_max_object(); }
	void		     to_fpvalue(FPValue& v) override { v.r = get_max_object(); v.type = TYPE_REFTARG; }
...
// LAM 4/1/00 - exporting the following...
	...
	static ScripterExport MAXSuperClass*    lookup_superclass(SClass_ID sid); // return corresponding MAXSuperClass if exists, othewise null
	...
	BOOL		is_kind_of(ValueMetaClass* c) override { return ((c == class_tag(MAXSuperClass)) ? 1 : Value::is_kind_of(c)); }
...
enum metadata_flags					/* class flags */
{
	md_use_getref           = 0x0001, // parameter block is held as a reference by the class object
	md_use_getref0          = 0x0002, // parameter block is ref 0
	md_use_getref1          = 0x0004, // parameter block is ref 1
	md_no_create            = 0x0008, // class not creatable by default by maxscript, can force create using createInstance
	md_no_access            = 0x0010, // unused
	md_direct_index         = 0x0020, // parm_desc::parm_id values are indices instead of param id values
	md_new_plugin           = 0x0040, // maxclass is being dynamically created
	md_auto_parms           = 0x0080, // automatically build cached paramblock descriptors on first usage
	md_name_clash           = 0x0100, // when exporting maxclass to global variable, a global variable with desired name already existed
	md_initialized          = 0x0200, // maxclass has been initialized
	md_no_cd                = 0x0400, // when automatically creating maxclass from superclassid/classid, no corresponding classdesc existed
	md_was_new_plugin       = 0x0800, // maxclass was dynamically created
	md_needs_localization   = 0x1000, // maxclass needs localized pb1 parameter names to be added
};
...
public:
	...
	static ScripterExport MAXClass* lookup_class(const Class_ID* cid, SClass_ID scid, bool make_new_if_missing = true);
	static MAXClass* lookup_class(const Class_ID& cid, SClass_ID scid, bool make_new_if_missing = true)
	{
		return lookup_class(&cid, scid, make_new_if_missing);
	}
	...
	Value*			superClassOf_vf(Value** arg_list, int count) override;
	Value*			isKindOf_vf(Value** arg_list, int count) override;
	BOOL			is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXClass)) ? 1 : Value::is_kind_of(c); }
#	define is_maxclass(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MAXClass))
	...
	ScripterExport Value* apply(Value** arglist, int count, CallContext* cc=nullptr) override;		// object creation by applying class object
	...
	ScripterExport static Value*	get_pb2_property(IParamBlock2* pb, ParamDef* pd, int tabIndex, TimeValue t, Interval& valid);
	static void		set_pb2_property(IParamBlock2* pb, ParamDef* pd, int tabIndex, TimeValue t, Value* val);
	static ParamDef* get_parm_def(ClassDesc2* cd2, Value* prop, ParamBlockDesc2*& pbd, int& tabIndex, ReferenceTarget* ref = nullptr);
	...
	Class_ID		get_max_class_id() override { return class_id; }
...
public:
	...
	int proc(ReferenceMaker* rm) override
	{
		...
		if (rm->SuperClassID() == MAXSCRIPT_WRAPPER_CLASS_ID && 
			...
			(finalCheckProc == nullptr || (*finalCheckProc)((MAXWrapper*)rm, arg))
			) 
		{
...
public:
	...
	Value*		superClassOf_vf(Value** arg_list, int count) override;
	Value*		isKindOf_vf(Value** arg_list, int count) override;
	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXNode)) ? 1 : MAXWrapper::is_kind_of(c); }
...
#	include "../protocols/node.inl"
	...
	INode*		to_node() override { check_for_deletion(); return node; }
	void		to_fpvalue(FPValue& v) override { v.n = node; v.type = TYPE_INODE; }
...
#	define		is_maxobject(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MAXObject))
	...
	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXObject)) ? 1 : MAXWrapper::is_kind_of(c); }

	void		collect() override;
	ScripterExport void sprin1(CharStream* s) override;
	const MCHAR*		class_name() override;

	BOOL CanTransferReference(int i) override { UNUSED_PARAM(i); return FALSE; }  // prevent mod applications from making us refer to a derived object
	...
	void		to_fpvalue(FPValue& v) override { v.obj = obj; v.type = TYPE_OBJECT; }
...
#	define		is_tab_param(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MAXPB2ArrayParam))
	void		collect() override;
	ScripterExport void		sprin1(CharStream* s) override;
	const MCHAR*		class_name() override { return _M("Array Parameter"); }
...
#	include "../protocols/arrays.inl"
	...
	// NULL get/showprops
	Value* show_props_vf(Value** arg_list, int count) override { return &false_value; }
	Value* get_props_vf(Value** arg_list, int count) override { return &undefined; }
};
...
#	define is_modifier(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MAXModifier))
	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXModifier)) ? 1 : MAXWrapper::is_kind_of(c); }
	void		collect() override;
	ScripterExport void		sprin1(CharStream* s) override;
	const MCHAR*		class_name() override;
	...
	Modifier*	to_modifier() override { check_for_deletion(); return mod; }
	void		to_fpvalue(FPValue& v) override { v.r = mod; v.type = TYPE_REFTARG; }
};
...
public:
	...
	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXControl)) ? 1 : MAXWrapper::is_kind_of(c); }
	void		collect() override;
	ScripterExport void		sprin1(CharStream* s) override;
	const MCHAR*		class_name() override;
#	define is_controller(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MAXControl))
	...
	Control*	to_controller() override { check_for_deletion(); return controller; }
	void		to_fpvalue(FPValue& v) override { v.ctrl = controller; v.type = TYPE_CONTROL; }

	// trap reduceKeys() here to supply controllers dim
	def_visible_generic  ( reduceKeys,				"reduceKeys");

	Value*		get_property(Value** arg_list, int count) override; // LAM - 11/16/02
	Value*		set_property(Value** arg_list, int count) override;
};
...
public:
	...
	void		collect() override;
	ScripterExport void		sprin1(CharStream* s) override;
	const MCHAR*		class_name() override { return _M("SubAnim"); }
	ScripterExport ReferenceTarget* get_max_object() override;
#	define is_subAnim(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MAXSubAnim))
	...
	INode*		to_node() override		{ return subanim_obj() != this ? subanim_obj()->to_node() : Value::to_node(); }
	Mtl*		to_mtl() override		{ return subanim_obj() != this ? subanim_obj()->to_mtl() : Value::to_mtl(); }
	Texmap*		to_texmap() override		{ return subanim_obj() != this ? subanim_obj()->to_texmap() : Value::to_texmap(); }
	Modifier*	to_modifier() override	{ return subanim_obj() != this ? subanim_obj()->to_modifier() : Value::to_modifier(); }
	Control*	to_controller() override { return subanim_obj() != this ? subanim_obj()->to_controller() : Value::to_controller() ; }
	Atmospheric* to_atmospheric() override { return subanim_obj() != this ? subanim_obj()->to_atmospheric() : Value::to_atmospheric(); }
	Effect*		to_effect() override { return subanim_obj() != this ? subanim_obj()->to_effect() : Value::to_effect(); }
	ITrackViewNode* to_trackviewnode() override { return subanim_obj() != this ? subanim_obj()->to_trackviewnode() : Value::to_trackviewnode(); }
	void		to_fpvalue(FPValue& v) override;
};
...
public:
	...
	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXRefTarg)) ? 1 : MAXWrapper::is_kind_of(c); }
	void		collect() override;
	ScripterExport void		sprin1(CharStream* s) override;
	const MCHAR*		class_name() override { return _M("MAXRefTarg"); }
...
public:
	...
	void		collect() override;
	ScripterExport void		sprin1(CharStream* s) override;
	const MCHAR*		class_name() override { return _M("TrackViewNode"); }
	ReferenceTarget* get_max_object() override;
	...
	Control* get_max_prop_controller(Value* prop, ParamDimension** pdim) override;
	BOOL	 set_max_prop_controller(Value* prop, MAXControl* c) override;
	Control* get_max_pos_controller(ParamDimension** pdim) override { return get_max_prop_controller(n_position, pdim); }
	Control* get_max_scale_controller(ParamDimension** pdim) override { return get_max_prop_controller(n_scale, pdim); }
	Control* get_max_rotation_controller(ParamDimension** pdim) override { return get_max_prop_controller(n_rotation, pdim); }
	...
	ITrackViewNode* to_trackviewnode() override { check_for_deletion(); return (parent == nullptr) ? MAXScript_interface->GetTrackViewRootNode() : parent->GetNode(index); }
private:
...
public:
	...
	BOOL					is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXTVUtility)) ? 1 : MAXWrapper::is_kind_of(c); }
	const MCHAR*			class_name() override;
	void					collect() override;
	ScripterExport void		sprin1(CharStream* s) override;
	void					gc_trace() override;
...
public:
	...
	ScripterExport Value* apply(Value** arglist, int count, CallContext* cc=nullptr) override;		// object creation by applying class object
	Class_ID	get_max_class_id() override { return triObjectClassID; } 
};
...
#	define is_mesh(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MeshValue))
	...
	const MCHAR*		class_name() override { return _M("TriMesh"); }
	ReferenceTarget* get_max_object() override { return obj; }
protected:
	void		SetReference(int i, RefTargetHandle rtarg) override;
public:
	RefResult	NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate) override;
	//! Prevent modifier applications from making us refer to a derived object
	BOOL		CanTransferReference(int i) override { UNUSED_PARAM(i); return FALSE; }
	BOOL		IsRealDependency(ReferenceTarget* target) override; 
	...
	Mesh*		to_mesh() override { setup_mesh(); return mesh; }
	void	    to_fpvalue(FPValue& v) override { setup_mesh(); v.msh = mesh; v.type = TYPE_MESH; }
	
	Value*		get_property(Value** arg_list, int count) override;
	Value*		set_property(Value** arg_list, int count) override;

	BitArray	get_vertsel() override { setup_mesh(); return mesh->VertSel(); }
	BitArray	get_facesel() override { setup_mesh(); return mesh->FaceSel(); }
	BitArray	get_edgesel() override { setup_mesh(); return mesh->EdgeSel(); }
	void		set_vertsel(BitArray &sel) override { setup_mesh(MESH_WRITE_ACCESS); mesh->VertSel() = sel; }
	void		set_facesel(BitArray &sel) override { setup_mesh(MESH_WRITE_ACCESS); mesh->FaceSel() = sel; }
	void		set_edgesel(BitArray &sel) override { setup_mesh(MESH_WRITE_ACCESS); mesh->EdgeSel() = sel; }
	Mesh*		set_up_mesh_access(int access, ReferenceTarget** owner = nullptr) override;
	Mesh*		set_up_mesh_face_access(int index, int access, ReferenceTarget** owner = nullptr) override;
	Mesh*		set_up_mesh_vertex_access(int index, int access, ReferenceTarget** owner = nullptr) override;
	void		update_sel() override;
...
public:
	...
	ScripterExport Value* apply(Value** arglist, int count, CallContext* cc=nullptr) override;		// object creation by applying class object
	Class_ID	get_max_class_id() override { return splineShapeClassID; } 
};
...
#	define is_beziershape(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(BezierShapeValue))
	void		collect() override;
	void		sprin1(CharStream* s) override;
	const MCHAR*		class_name() override { return _M("BezierShape"); }
	ReferenceTarget* get_max_object() override { return obj; }
protected:
	void		SetReference(int i, RefTargetHandle rtarg) override;
public:
	...
	BezierShape*		to_beziershape() override { setup_beziershape(); return beziershape; }

	void	    to_fpvalue(FPValue& v) override { setup_beziershape(); v.bezshape = beziershape; v.type = TYPE_BEZIERSHAPE; }
	
	Value*		get_property(Value** arg_list, int count) override;
	Value*		set_property(Value** arg_list, int count) override;

	BezierShape*		set_up_beziershape_access(int access, ReferenceTarget** owner = nullptr);
	BezierShape*		set_up_beziershape_vertex_access(int index, int access, ReferenceTarget** owner = nullptr);
};
...
public:
	...
	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXAtmospheric)) ? 1 : MAXWrapper::is_kind_of(c); }
	void		collect() override;
	ScripterExport void		sprin1(CharStream* s) override;
	const MCHAR*		class_name() override { return _M("Atmospheric"); }
...
#include "../macros/define_implementations.h"
	...
	Atmospheric*	to_atmospheric() override { check_for_deletion(); return atmos; }
};
...
public:
	...
	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXEffect)) ? 1 : MAXWrapper::is_kind_of(c); }
	void		collect() override;
	ScripterExport void		sprin1(CharStream* s) override;
	const MCHAR*		class_name() override { return _M("Effect"); }
...
#include "../macros/define_implementations.h"
	...
	Effect*	to_effect() override { check_for_deletion(); return effect; }
};
...
public:
	...
	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXMultiPassCameraEffect)) ? 1 : MAXWrapper::is_kind_of(c); }
	void		collect() override;
	ScripterExport void		sprin1(CharStream* s) override;
	const MCHAR*		class_name() override { return _M("MultiPassCameraEffect"); }

	def_generic( iscompatible,	"isCompatible");

	IMultiPassCameraEffect*	to_mpassCamEffect() override { check_for_deletion(); return effect; }
};
...
public:
	...
	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXShadowType)) ? 1 : MAXWrapper::is_kind_of(c); }
	void		collect() override;
	ScripterExport void		sprin1(CharStream* s) override;
	const MCHAR*		class_name() override { return _M("MAXShadowType"); }

	ShadowType*	to_shadowtype() override { check_for_deletion(); return shadow; }
};
...
public:
	...
	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXRenderer)) ? 1 : MAXWrapper::is_kind_of(c); }
	void		collect() override;
	ScripterExport void		sprin1(CharStream* s) override;
	const MCHAR*		class_name() override { return _M("MAXRenderer"); }

	Value*		get_property(Value** arg_list, int count) override;
	Value*		set_property(Value** arg_list, int count) override;

	Renderer*	to_renderer() override { check_for_deletion(); return renderer; }
};
...
#	define		is_custattrib(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MAXCustAttrib))
	...
	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(MAXCustAttrib)) ? 1 : MAXWrapper::is_kind_of(c); }
	void		collect() override;
	ScripterExport void		sprin1(CharStream* s) override;
	const MCHAR*		class_name() override { return _M("MAXCustAttrib"); }
...
public:
	...
	void		collect() override;
	void		gc_trace() override;
	const MCHAR*		class_name() override { return _M("<ChangeHandler>"); }
	...
	BOOL		IsRealDependency(ReferenceTarget* target) override { UNUSED_PARAM(target); return FALSE; }
};
...
#define CH_DISABLED		0x0010		// change handler is disabled
...
#	define is_rootnode(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MAXRootNode))
	void		collect() override;
	ScripterExport void	sprin1(CharStream* s) override;
	const MCHAR*		class_name() override { return _M("<root>");	}
...
#include "../macros/define_implementations.h"
	...
	INode*		to_rootnode() override { check_for_deletion(); return node; }
};
...
public:		   
	...
	void Redo() override;
	MSTR Description() override {return MSTR(_M("convertTo()"));}
};
...
public:
	...
	const MCHAR*			class_name() override { return _M("XRefScene"); }
...
public:
	...
	static ScripterExport Value* intern(ReferenceTarget* ref);
	ScripterExport void		sprin1(CharStream* s) override;
	const MCHAR*		class_name() override { return _M("<rootScene>");	}
};
