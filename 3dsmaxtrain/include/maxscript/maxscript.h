...
#define MXS_LISTENER_TO_TOP			(WM_USER + 0x118)
...
inline Boolean* bool_result(bool val) { return val ? &true_value : &false_value; }

/* for testing a value to ensure that it is within range */
template <class T>
inline void MXS_range_check(T _val, T _lowerLimit, T _upperLimit, const MSTR& _desc)
{
	static_assert(std::is_arithmetic_v<T>);
	if (_val < _lowerLimit || _val > _upperLimit)
	{
		MSTR msg;
		msg.printf(_T("%s < %g or > %g: %g"), _desc.data(), EPS(_lowerLimit), EPS(_upperLimit), EPS(_val));
...
ScripterExport void		error_message_box(MAXScriptException& e, const MSTR& caption);
