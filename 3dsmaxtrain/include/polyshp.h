...
//!@{
/*! Parameter space based on segments. This simple interpolation is based on parameter space. If a polyline has four
 * segments, the first segment has parameter values 0-0.25, the second 0.25-0.5, the third 0.5-0.75, and the fourth
 * 0.75-1.0. This is regardless of the length of each segment.
 */
#define POLYSHP_INTERP_SIMPLE 0
/*! Parameter space normalized to curve length. This interpolation normalizes the parameter space to distance along the
 * length of a polyline. So, parameter space 0 is the start, 1.0 is the end, and 0.5 is halfway along the actual length
 * of the curve.
 */
...
#define POLYPT_INVIS_EDGE \
	(1 << 20) //!< \internal Force segment to be invisible on capping. Used internally by capping code
...
public:
	...
	{
		flags2 &= 0xFFFF;
		flags2 |= (DWORD)(id << POLYPT_MATID_SHIFT);
	}
};

/// \internal \defgroup Option_for_PolyLine__Cap3DS___PolyShape__Make3DSCap Option for PolyLine::Cap3DS /
/// PolyShape::Make3DSCap
//!@{
...
public:
	 ...
	 * \param cb A pointer to an IntersectionCallback3D class.  Defaults to nullptr.
	 * If a non-null callback is supplied, the callback will be called for each
	 * intersection with the 'p' argument indicating the intersection point;
	 * 'piece1' will indicate the index of the segment on this PolyLine involved
	 * in the intersection and 'piece2' will be -1.
	 ...
	 * If a non-null callback is supplied, the callback will be called for each
	 * intersection with the 'p' argument indicating the intersection point;
	 * 'piece1' will indicate the index of the segment in 'line' which intersects
	 * and 'piece2' will indicate the index of the segment in this PolyLine which
	 * intersects 'line'.
	 ...
	 * \param cb Pointer to an IntersectionCallback3D class. Defaults to nullptr.
	 * If a non-null callback is supplied, the callback will be called for each
	 * intersection with the 'p' argument indicating the intersection point;
	 * 'piece1' and 'piece2' will indicate the indices of the two segments in
	 * the PolyLine which intersect.
	 */
	CoreExport BOOL SelfIntersects(BOOL findAll = FALSE, IntersectionCallback3D* cb = nullptr); // 2D!
	//! @}
...
public:
	 ...
	 * invalidated, and the object is set to NULL. */
	...
	/** \internal This method is used internally. */
	CoreExport void Render(GraphicsWindow* gw, Material* ma, RECT* rp, int compFlags, int numMat);
	void render(GraphicsWindow* gw, Material* ma, RECT* rp, int compFlags, int numMat) { Render(gw, ma, rp, compFlags, numMat); }
