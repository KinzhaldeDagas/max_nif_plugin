...
public:
	...
	LONG   Ascent = 0;
	LONG   Descent = 0;
	LONG   InternalLeading = 0;
	LONG   ExternalLeading = 0;
	LONG   AveCharWidth = 0;
	LONG   MaxCharWidth = 0;
	LONG   Weight = 0;
	LONG   Overhang = 0;
	LONG   DigitizedAspectX = 0;
	LONG   DigitizedAspectY = 0;
	BCHAR  FirstChar = _T('\0');
	BCHAR  LastChar = _T('\0');
	BCHAR  DefaultChar = _T('\0');
	BCHAR  BreakChar = _T('\0');
	BYTE   Italic = 0;
	BYTE   Underlined = 0;
	BYTE   StruckOut = 0;
	...
	CoreExport BezFontMetrics(const NEWTEXTMETRIC *from);
};
...
public:
	MSTR family;	// Family name (i.e. "Arial")
	MSTR name;		// Full name (i.e. "Arial Italic")
	MSTR style;		// Style name (i.e. "Italic")
	int type = 0;		// See above
	DWORD flags = 0;	// See above
	BezFontMetrics metrics;
	BezFontInfo() {}
	BezFontInfo(MSTR fam, MSTR n, MSTR s, int t, DWORD f, const BezFontMetrics &m) { family=fam; name=n; style=s; type=t; flags=f; metrics=m; }
	CoreExport BezFontInfo &operator=(const BezFontInfo &from);
};
...
public:
	...
	virtual MSTR GetFontDir (int i) = 0;
};
...
public:
	...
	* \param shape the built BezierShape result of the character
