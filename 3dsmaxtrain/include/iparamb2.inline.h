...
#ifdef min
#pragma message("warning: \"min\" macro detected. Consider defining NOMINMAX to prevent it")
#undef min
#endif

#ifdef max
#pragma message("warning: \"max\" macro detected. Consider defining NOMINMAX to prevent it")
#undef max
#endif
...
template <typename ValueType>
inline BOOL IParamBlock2::GetValue(ParamID id, TimeValue t, ValueType& v, Interval&&, int tabIndex)
{
    return GetValue(id, t, v, tabIndex);
}
