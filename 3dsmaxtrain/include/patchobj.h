...
// 7/20/00 TH -- Relax controls
		...
		int NumSubs()  { return 1; }	// Just tell it about the Point Controller Container
		...
		int NumRefs() {return vertCont.Count() + vecCont.Count() + 1;}	// vert conts + vec conts + Point Controller Container
...
class SingleRefMakerPatchNode : public SingleRefMaker{
	public:
		...
		virtual void GetClassName(MSTR& s, bool localized = true) const override { UNUSED_PARAM(localized); s = _M("SingleRefMakerPatchNode"); } // From Animatable
	};
...
class SingleRefMakerPatchMtl : public SingleRefMaker{
	public:
		...
		RefResult NotifyRefChanged( const Interval& changeInt,RefTargetHandle hTarget, 
		PartID& partID, RefMessage message, BOOL propagate );
		virtual void GetClassName(MSTR& s, bool localized = true) const override { UNUSED_PARAM(localized); s = _M("SingleRefMakerPatchMtl"); } // From Animatable
};
