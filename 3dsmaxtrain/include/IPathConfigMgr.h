...
public:
	...
	*/
	virtual MSTR GetDir(int which) const =0;
	...
	*/
	virtual MSTR GetPlugInDesc(int i) const =0;
	...
	*/
	virtual MSTR GetPlugInDir(int i) const =0;
	...
	*/
	virtual MSTR GetAssetDir(int i, MaxSDK::AssetManagement::AssetType assetType) const =0;
	...
	*/
	virtual MSTR GetSessionAssetDir(int i, MaxSDK::AssetManagement::AssetType assetType) const =0;
	...
	*/
	virtual MSTR GetCurAssetDir(int i, MaxSDK::AssetManagement::AssetType assetType) const =0;
	...
	*/
	...
	/*! \brief Returns the name of the .ini file used by 3ds Max.
	...
	*/
	...
	potentially considering the missing file cache.
	...
	\param ignoreMissingPathCache If true, testing does not consider whether the file's path may be in the missing path
	cache. 
	...
	*/ 
	...
	Gets the current Project Folder setting in 3ds Max.  This is the setting to which newly
	...
	*/
	...
	Sets the current Project Folder setting in 3ds Max.  This is the setting to which newly
	created relative paths are set relative to.
	Though this setting can also be set by calling
	\code
	IPathConfigMgr::GetPathConfigMgr()->SetDir(APP_PROJECT_FOLDER_DIR)
	\endcode
	it is highly recommended that this method be used to change this value, as other steps to adjust
	...
	an absolute path.  If the folder path does not exist, 3ds Max will attempt to create it.
	...
	*/
	...
	/*! \brief Initiates the steps to choose and setup a default 3ds Max project.
	This call will initiate the workflow for allowing a user to choose
	and create a default project.  In effect, this method will launch the
	"CreateDefaultProject" macroscript.  The user will be prompted to browse
	to a new directory.
	...
	*/
	...
	/*!\brief Initiates the steps to set up a default 3ds Max project without prompting the user.
	This call will initiate the workflow for setting up a default project.
	In effect, this method will launch the "CreateDefaultProject" macroscript.  It will
	use the passed in parameter as the new Project Folder.
	...
	*/
	...
	/*! \brief Initiates the steps to set an existing 3ds Max project folder as the active project folder.
	This call will initiate the workflow for allowing a user to choose an existing 3ds Max project folder
	as the active project folder.  In effect, this method will launch the "SetProjectFolder" macroscript.
	The user will be prompted to browse to an existing directory.
	\return true if the steps succeeded and the user did not cancel the action
	Will return false immediately if the app is in Quiet Mode.
	*/
	virtual bool DoSetProjectFolderSteps() const = 0;

	/*! \brief Initiates the steps to set an existing 3ds Max project folder as the active project folder without
	prompting the user. This call will set an existing 3ds Max project folder as the active project folder. In effect,
	this method will launch the "SetProjectFolder" macroscript.  It will use the passed in parameter as the new Project
	Folder to set active. \param[in] aDirectory The directory that will be set as the active project folder. This path
	must be absolute. \return true if the steps succeeded and the path is absolute
	*/
	...
	*/
	virtual MSTR GetProjectSubDirectory(int aIndex) const = 0;
