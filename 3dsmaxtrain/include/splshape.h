...
/*! \sa  Class ShapeObject, Class BezierShape, ~{ Shapes and Splines }~.\n\n
\par Description:
...
//2-1-99 watje
		...
		// This method is called by a selection process in response to mouse double-click events to help select the current sub-object (vertex, segment, spline)
	...
	public:
		...
		CoreExport virtual bool UpdatePerNodeItems(const MaxSDK::Graphics::UpdateDisplayContext& updateDisplayContext,
		...
		CoreExport virtual void GetClassName(MSTR& s, bool localized = true) const override;

        using ShapeObject::GetInterface;
		CoreExport void* GetInterface(ULONG id);

		// This is the name that will appear in the history browser.
		CoreExport virtual const MCHAR *GetObjectName(bool localized = true) const override;
...
class SSSegRefineMouseProc : public MouseCallBack {
	...
	public:
		SSSegRefineMouseProc(SplineShape* spl, IObjParam *i) : type(REFINE_VERT) { ss=spl; ip=i; }
		int proc( 
			HWND hwnd, 
			int msg, 
			int point, 
...
class SSSegRefineCMode : public CommandMode {
	...
	public:
		SSSegRefineCMode(SplineShape* spl, IObjParam *i) :
			fgProc(spl), eproc(spl,i), type(REFINE_VERT) {ss=spl;}
		...
		void EnterMode();
		void ExitMode();
		void SetType(int t) { type = t; eproc.SetType(t); }
	};
...
class SSRefineConnectMouseProc : public MouseCallBack {
	...
	public:
		SSRefineConnectMouseProc(SplineShape* spl, IObjParam *i) : type(REFINE_VERT) { ss=spl; ip=i; }
		int proc( 
			HWND hwnd, 
			int msg, 
			int point, 
			int flags, 
			IPoint2 m );
		void SetType(int refineType) { type = refineType; }
	};
...
public:
	...
	virtual void GetClassName(MSTR& s, bool localized = true) const override { UNUSED_PARAM(localized); s = _M("SingleRefMakerSplineNode"); } // From Animatable
};
...
public:
	...
	RefResult NotifyRefChanged( const Interval& changeInt,RefTargetHandle hTarget, 
		PartID& partID, RefMessage message, BOOL propagate );
	virtual void GetClassName(MSTR& s, bool localized = true) const override { UNUSED_PARAM(localized); s = _M("SingleRefMakerSplineMtl"); } // From Animatable
};
...
// Command ID for the Select Similar command
...
// Access to Spline closure prompting settings and overrides, used in the 3ds Max Line object (added in 3ds Max 2024.1)

/*! \remarks Set the master override switch that allows spline editing to ignore the Auto-Weld switch and always prompt for closure.
This is used in the 3ds Max Line object to preserve legacy behavior and always show the closure prompt, regardless of the
state of the Auto Weld switch used in Editable and Edit Spline.\n\n
\par Parameters:
<b>bool option</b>\n\n
When set to 'true', the master spline Auto-Weld switch is ignored and spline editing always displays
a prompt to the user asking whether the spline should be closed when the knot being created is near
the spline's starting knot.\n\n
*/
CoreExport void SetAlwaysShowSplineClosurePrompt(bool option);

/*! \remarks Get the master override switch that allows spline editing to ignore the Auto-Weld switch
and always prompt for closure.\n\n
\return true if the override switch is set; false otherwise.
*/
