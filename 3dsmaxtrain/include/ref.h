...
//! \brief Sent by a node when its name has been changed. 
...
//! \brief Sent by a modifier when its name has been changed. 
/*!  The Modify Panel responds to this message by changing the name displayed in the UI and potentially in the modifier selection dropdown.
     The modifier pointer is sent as partid. For example NotifyDependents(FOREVER, (INT_PTR)modifier, REFMSG_MODIFIER_NAMECHANGE); 
*/
#define REFMSG_MODIFIER_NAMECHANGE							0x00000285

//!@}

//! \brief A parameter block sends the message to its client when it needs the
//! nonlocalized anim name of the ith parameter.
//*! \ref partids is set to a pointer to a GetParamName structure defined in iparamb.h */
//! \sa REFMSG_GET_PARAM_NAME_LOCALIZED
#define REFMSG_GET_PARAM_NAME_NONLOCALIZED				0x00000286

//! \brief A parameter block sends the message to its client when it needs the
//! localized anim name of the ith parameter.
//*! \ref partids is set to a pointer to a GetParamName structure defined in iparamb.h */
//! \sa REFMSG_GET_PARAM_NAME_NONLOCALIZED
...
//! \name Clone Methods
//!@{
		/*! \brief Returns a pointer to an item's clone if it was cloned or NULL otherwise.
			\param from - This is the item to see if cloned.
			\return A pointer to the clone of the item, NULL if not cloned
		*/
...
public:
	...
	RefTargetHandle FindMapping(RefTargetHandle from) override;
	void AddEntry(RefTargetHandle hfrom, RefTargetHandle hto) override;
	void PatchPointer(RefTargetHandle* patchThis, RefTargetHandle oldTarg) override;
	void Backpatch() override;
	bool BackpatchPending() override;
	void Clear() override;
	void ClearBackpatch() override;
	void DeleteThis() override;
	void AddPostPatchProc(PostPatchProc* proc, bool toDelete) override;
};
...
//!@{
		...
		*/
		...
		CoreExport virtual IOResult Save(ISave *isave, ChannelMask /*channels*/) { return Save(isave); }
		...
		*/
		...
		CoreExport virtual IOResult Load(ILoad *iload, ChannelMask /*channels*/) { return Load(iload); }
		...
		*/
		...
		//! \brief Rescale size of all world units in object and its reference hierarchy.
		/*!  Must call:
		\code
		ClearAFlagInHierarchy(rm, A_WORK1);
		ScopedPrePostRefNotifyDependents scopedPrePostRefNotifyDependents(true);
		\endcode
		or
		\code
		ClearAFlagInAllAnimatables(A_WORK1);
		ScopedPrePostRefNotifyDependents scopedPrePostRefNotifyDependents(true);
		\endcode
		before calling this on an object, unless being called from a RescaleWorldUnits
		implementation.
		The ScopedPrePostRefNotifyDependents instance is not required, but improves performance
		by reducing notification messages.
		Note that Interface::RescaleWorldUnits performs these actions.
		This method may be implemented to rescale the size of all world units in an object and
		its reference hierarchy. In the implementation, developers must call:
		\code 
		if (TestAFlag(A_WORK1))
			return;
		SetAFlag(A_WORK1);
		\endcode
		before performing any scaling. This prevents multiple rescales of the same object.
		The default implementation calls RescaleWorldUnits on each of its references, and
		on its custom attribute container if it exists. 
...
//!@{
		...
		*/
		...
		\return Returns TRUE.
		*/
		BOOL IsRefMaker() override { return TRUE; }
...
//!@{
		...
		\return Returns TRUE.
		*/
		BOOL IsRefTarget() override { return TRUE; }
...
//!@{
		...
		\note This method is not called when performing an undo of removing
		a reference to a reference target, or redoing of setting a reference.
		\sa RefAddedUndoRedo
		*/
		...
		//! or redo.
		/*! Called when reference is added because of an undo or a redo. 
		...
		*/
		...
		// If target needs to know, it should override this method.
		//! \brief Called after a reference to this is deleted.
		/*! This is called after deleting a reference to a reference target.
		\note Prior to 3ds Max 2024, this method was called from 
		ReferenceTarget::MaybeAutoDelete only when the target 
		was not deleted when the reference to it was dropped. Starting in 
		3ds Max 2024, this method is called from ReferenceMaker::DeleteReference 
		whenever a reference is dropped.
		\note This method is not called when performing an undo of setting
		a reference to a reference target, or redoing removing a reference
		\param  oldOwner - The ReferenceMaker removing its reference.
		\sa RefDeletedUndoRedo
		*/
		virtual void RefDeleted(ReferenceMaker* oldOwner) {UNUSED_PARAM(oldOwner);}

		//! \brief Called after a reference to this is deleted because of undo 
		//! or redo.
		/*! Called when reference is removed because of an undo or a redo.
		Otherwise it is similar to RefDeleted.
		\param  oldOwner - The ReferenceMaker removing its reference.
		*/
		virtual void RefDeletedUndoRedo(RefMakerHandle oldOwner) {UNUSED_PARAM(oldOwner);}
...
//!@{
		...
		*/
		CoreExport RefResult NotifyDependents(
			const Interval& changeInt, 
			PartID partID, 
			RefMessage message, 
			SClass_ID sclass = NOTIFY_ALL,
			BOOL propagate = TRUE, 
			RefTargetHandle hTarg = nullptr,
			NotifyDependentsOption notifyDependentsOption = REFNOTIFY_ALLOW_OPTIMIZATIONS) override;
		...
		*/
		...
		a driven controller could override this method and call its driver's 
...
public:
	...
	CoreExport ~DeletedRestore();
	CoreExport void Restore(int isUndo) override;
	CoreExport void Redo() override;
	CoreExport MSTR Description() override;
};
...
Important: It is not recommended that 3rd party plugins disable reference messages.
For 3ds Max to function correctly, every call to DisableRefMsgs() needs to be matched with a call to EnableRefMsgs().
...
Note: DisableRefMsgs() can be overriden by EnableDisableRefMsgsOverride()
\see EnableDisableRefMsgsOverride()
*/
...
Important: For 3ds Max to function correctly, every call to DisableRefMsgs() needs to be matched with a call to EnableRefMsgs().
Occasionally, when a plugin needs to re-enable reference messages while in a context in which they have been disabled (very likely by the plugin itself),
it is important that it calls EnableRefMsgs() as many times as DisableRefMsgs() has been called. 
When the plugin finished its task, it should restore the reference message propagation to the previous state.
...
//! \brief Returns the number of times reference messages have been marked as disabled. 0 means reference messages are enabled.
CoreExport DWORD IsRefMsgsDisabled();

//! \brief Force override of reference messages disabled globally.
/*! 
In some cases, a component may use the messaging system to set/query information on a different part of the component. In order for the component to operate properly,
it needs the reference messages to be processed even when reference messages are disabled globally.
Important: For 3ds Max to function correctly, every call to EnableDisableRefMsgsOverride() needs to be matched with a call to DisableDisableRefMsgsOverride().
For example:

\code

	int PManager::NumParticles()
	{
		int theNum = 0;
		PartID partID = (PartID)(&theNum);
		EnableDisableRefMsgsOverride();
		NotifyDependents(FOREVER, partID, kPFMSG_GetNumParticles, NOTIFY_ALL, TRUE);
		DisableDisableRefMsgsOverride();
		return theNum;
	}

\endcode
*/
CoreExport void EnableDisableRefMsgsOverride();

//! \brief Turn off force override of reference messages disabled globally.
CoreExport void DisableDisableRefMsgsOverride();

//! \brief Returns the number of times Force DisableAllMsgs Override has been performed.
CoreExport DWORD IsForceDisableAllMsgsOverriden();
...
protected:
	...
	RefTargetHandle m_rtarget;
	//! Whether to force deletion of held reference when dropping reference to it
	bool m_forceDelete;
	//! Whether this SingleRefMaker was specified as a static instance. Used for Animatable leakage detection at shutdown
...
public:
	...
	/*! \param forceDelete - specifies whether to check to see if held Reference still exists after dropping the
	reference to it. If it does, attempt to delete it by calling DeleteAllRefs() and then DeleteThis() on it. If still
	exists, delete using 'delete'. This parameter is set when the Reference is normally handled as a singleton, and
	either its AutoDelete() implemetation just returns REF_SUCCEED or its DeleteThis() implementation is a no-op. */
	/*! \param isStaticInstance - specifies whether/when this SingleRefMaker is a static instance and should be ignored
	when performing animatable leakage detection at shutdown. */
	CoreExport SingleRefMaker(AutoDropRefOnShutdown autoDropRefOnShutdown = Never, bool forceDelete = false,
	...
	CoreExport SingleRefMaker(ReferenceTarget* refTarg, AutoDropRefOnShutdown autoDropRefOnShutdown = Never,
			bool forceDelete = false, bool isStaticInstance = false);
	...
	//! \brief Get whether SingleRefMaker was specified as a static instance
	...
	//! \brief Deletes this SingleRefMaker, marshaling the deletion to the main 3ds Max thread if needed.
   ...
   // From Animatable
	...
	CoreExport	int NumRefs() override;
	CoreExport	RefTargetHandle GetReference(int i) override;
protected:
	CoreExport	void SetReference(int i, RefTargetHandle rtarg) override;
public:
	CoreExport	BOOL CanTransferReference(int i) override;
private:
...
public:
	...
	/*! \param isStaticInstance - specifies whether/when this SingleRefMaker is a static instance and should be ignored
	when performing animatable leakage detection at shutdown. */
	TypedSingleRefMaker(AutoDropRefOnShutdown autoDropRefOnShutdown = Never, bool forceDelete = false,
			bool isStaticInstance = false)
			: SingleRefMaker(autoDropRefOnShutdown, forceDelete, isStaticInstance)
	{
	}

	//! \brief Constructor
	/*! \param refTarg - the initial typed Reference to be held. */
	/*! \param autoDropRefOnShutdown - specifies whether/when to auto drop the Reference on shutdown. */
	/*! \param forceDelete - specifies whether to check to see if held Reference still exists after dropping the
	reference to it. If it does, attempt to delete it by calling DeleteAllRefs() and then DeleteThis() on it. If still
	exists, delete using 'delete'. This parameter is set when the Reference is normally handled as a singleton, and
	...
	/*! \param isStaticInstance - specifies whether/when this SingleRefMaker is a static instance and should be ignored
	when performing animatable leakage detection at shutdown. */
	TypedSingleRefMaker(T* refTarg, AutoDropRefOnShutdown autoDropRefOnShutdown = Never, bool forceDelete = false,
			bool isStaticInstance = false)
			: SingleRefMaker(const_cast<std::remove_const_t<T>*>(static_cast<T*>(refTarg)), autoDropRefOnShutdown,
					forceDelete, isStaticInstance)
	{
	}

	TypedSingleRefMaker<T>& operator=(const TypedSingleRefMaker<T>& rhs)
	{
		Set(rhs.Get());
		SetAutoDropRefOnShutdown(rhs.GetAutoDropRefOnShutdown());
		return *this;
	}
	...
	T& operator* () const { return *static_cast<T*>(m_rtarget); }
...
private:
	...
	bool m_previousAutobackupRequiredState;
public:
	//! Constructor
	...
	otherwise it's not changed. See IAutoBackup::SetEnable() for more information.
...
//! \brief A RefEnumProc to remove custom attribute containers and note tracks from a RefMaker and its references.
/*! An instance of this class should be used if a session static RefMaker instance adds itself to TrackView, and re-adds
	itself to TrackView on a file reset. If this occurs, any custom attributes or note tracks that were applied to 
	the static instance will still be present after the file reset. 
	An instance of this class can be used to remove custom attributes or note tracks from the instance and its references.
	Example:
	\code
	MyClass::MyClass()
	{
		RegisterNotification(PostResetCallback, this, NOTIFY_POST_SCENE_RESET);
		RegisterNotification(PostResetCallback, this, NOTIFY_SYSTEM_POST_RESET);
		AddToRefHierarchy();
	}
...
class DeleteCustAttribContainerAndNoteTracksProc : public RefEnumProc
{
public:
	int proc(ReferenceMaker* m)
	{
		if (m)
		{
			m->DeleteCustAttribContainer();
			m->DeleteAllNoteTracks();
			m->ClearAllAppData();
		}
