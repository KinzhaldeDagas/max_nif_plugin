...
namespace ToolClips
{
/** \brief The kToolTipKeyRole extends the Q::ItemDataRole in order to 
* retrieve a tool tip key from the data model.
* It can be used in QAbstractItemModel::data() implementations to provide 
* the tooltip system with extended tooltips on QAbstractItemView subclasses 
* like tree or list views. */
...
public:
    ...
    */
    ...
    /** \brief Sets the progressive tooltip to show immediately its level 2 presentation.
    * The progressive tooltip can show several levels of information, the first and most simple one 
    * is a brief description of the feature, the second one is a more developed and detailed 
    * presentation describing more specifically the feature.
    * This method allows to show immediately the level 2 if set to true. 
    * Otherwise the level 2 will appear just after the level 1 has been shown.
    * Set by default to false.
    * \param skipIt The value to be set to skipLevel1 attribute.
    * \see skipLevel1()
    */
    void setSkipLevel1( bool skipIt );

    /** \brief Returns whether or not the progressive tooltip's level 1 is skipped.
