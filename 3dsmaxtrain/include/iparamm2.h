...
class ParamMap2UserDlgProc: public MaxHeapOperators 
{
	public:
		...
		void Update(TimeValue t, IParamMap2* pmap) { Interval valid = FOREVER; Update(t, valid, pmap); }
};
#pragma warning(pop)
...
class IParamMap2 : public InterfaceServer
{
	public:
		...
		virtual IRollupWindow* GetIRollup() { return nullptr; }
		...
		// show or hide associated controls
		...
		/*! \brief Connects the given Qt widget with the paramblock that is managed by this parameter map.
		The method first removes all existing parameter UI connections and then in a second step 
		automatically connects the Qt UI controls, found in the widget, where the Qt objectName 
		matches the name of its associated paramblock parameter. 
		Before the automatic parameter binding starts, the PreConnectUI() callback is triggered on
		the given QMaxParamBlockWidget. When the automatic parameter binding is done PostConnectUI()
		is called on the given QMaxParamBlockWidget. The PostConnectUI() callback would be a good 
		point of time to add manual parameter UI connections and is for instance helpful when 
		setting up a specific connection to data stored in a variable tab parameter.
		\param widget The widget hosting the UI controls that should be connected to the param block.
		\see QMaxParamBlockWidget::PreConnectUI(),QMaxParamBlockWidget::PostConnectUI()
		*/
		virtual void ConnectUI(MaxSDK::QMaxParamBlockWidget* widget) = 0;

		/*! \brief This method removes all existing parameter UI connections held by the ParamMap.
		\note The internal pointer to the old widget will be released, the widget will not be deleted.*/
		virtual void DisconnectUI() = 0;

		/*! \brief Connects a Qt UI control to the specified param block parameter.
		If the UI control already has a parameter connection, it will be disconnected first.
		\param id The ID of the parameter.
		\param uiControl A pointer to the Qt UI control (e.g. a QmaxSpinBox) that should be connected to the param block.
		\param tabIndex If the parameter is a Tab<> this is the zero-based index of the
		parameter in the table. */
		virtual void ConnectUI(ParamID id, QObject* uiControl, int tabIndex = 0) = 0;

		/*! \brief Disconnects a Qt UI control from its associated param block parameter.
		\param uiControl A pointer to the Qt UI control (e.g. a QmaxSpinBox) that should be disconnected from the param block.*/
...
// destroys itself. Returns TRUE if the user selected OK, FALSE otherwise.
...
executes the dialog and then destroys itself.\n\n
...
Note that there is no need to destroy it. It executes the dialog and then
destroys itself. This function is currently not in use.
\par Parameters:
<b>IParamBlock2 *pb</b>\n\n
Points to the parameter block2.\n\n
<b>TimeValue t</b>\n\n
...
// Creates a parameter map to handle the display of parameters in the command panel.
...
//  It maintains a table of secondary ParamDlgs for main ParamDlgs (eg, the one returned 
//  from CreateParamDlg()) and will broadcast appropriate method calls to them
//  as the main receives them
...
ParamDlgs for main ParamDlgs (eg, the one returned from CreateParamDlg()) and
will broadcast appropriate method calls to them as the main receives them.
 */
class IAutoMParamDlg : public ParamDlg
{
	public:
		...
		main <b>IAutoMParamDlg</b>. Use this along with <b>AddDlg()</b> if
		...
		/*! \remarks Returns a pointer to the parameter map2 of this main dialog. */
...
//  It maintains a table of secondary EffectParamDlg for main EffectParamDlg (eg, the one returned 
...
EffectParamDlg for main EffectParamDlg (e.g., the one returned from
...
class IAutoEParamDlg : public SFXParamDlg
{
	public:
		...
		/*! \remarks This method is used for deleting secondary dialogs from a
		main <b>IAutoEParamDlg</b>. Use this along with <b>AddDlg()</b> if
		you are dynamically changing the set of rollups for the plugin, so that
		the <b>P_AUTO_UI</b> system can correctly manage all current secondary
		rollups.
		\par Parameters:
