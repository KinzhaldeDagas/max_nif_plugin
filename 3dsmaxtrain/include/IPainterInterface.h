...
#define PAINTERCANVASINTERFACE_V26_1 0x2d053283
...
//they cover the basics to get you started
	...
	/*! This is so the painter knows where to send the stroke messages
		\param targetCanvas - is the pointer to the canvas */
		virtual BOOL  InitializeCallback(ReferenceTarget* targetCanvas) = 0;//tested
...
//Colors are stored in the color manager. See the color ID defines at the top
	...
	/*! \sa The <a href="group__painter_color_i_ds.html">Color ID defines</a> */
...
//These functions deal with the pressure sensitive devices and mimicing pressure sensitivity
	...
	/*! You can effect Str. Radius, Str and Radius or None
		\sa See the <a href="group__painter_pressure_i_ds.html">Pressure defines</a> in IPainterInterface.h */
		virtual BOOL  GetPressureAffects()=0;//tested
	//! \brief Set what the pressure of a brush affects
	/*! You can effect Str. Radius, Str and Radius or None
		\param affect - One of the <a href="group__painter_pressure_i_ds.html">Pressure defines</a> in IPainterInterface.h */
...
class IPainterCanvasInterface_V5: public MaxHeapOperators 
	...
	/*! the canvas gets a list of all points, normals etc instead of one at a time
		...
		\param hit,mousePos,worldPoint,worldNormal,localPoint,localNormal,bary,index,shift,ctrl,alt,radius,str,
		pressure,node,mirrorOn,worldMirrorPoint,worldMirrorNormal,localMirrorPoint,localMirrorNormal
...
/*! \sa IPainterCanvasInterface_V26_1
    \par Description:
    Updated canvas interface for Max 2024.1. This variant allows the canvas to
    report whether the brush operation it defines can change the geometry or
    transformation of a node. Note that if a particular canvas does not
    implement this version of the interface, it will be assumed that the
    operation can indeed alter node geoemtry, retaining legacy behavior. */
class IPainterCanvasInterface_V26_1: public MaxHeapOperators
{
public:

	//! Returns a flag indicating whether the present brush operation leaves
	//  the node geometry and world space transformation unaltered. When a
	//  value of true is returned, the recalculation of the screen space
	//  bounding boxes for affected nodes will be skipped when updating the
	//  viewport, substantially improving performance on large meshes.
	virtual bool IsGeometryConstant() = 0;
};
