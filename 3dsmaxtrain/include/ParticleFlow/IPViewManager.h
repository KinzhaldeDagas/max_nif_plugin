...
public:
    ...
    */
	...
	/*! \fn virtual IParticleView* GetActiveParticleView() = 0;
    *  \brief Returns the ParticleView which is currently open and shown in PView.
    */
    ...
    *  \brief 
    */
	PFExport static const MCHAR* ActionName(int index);
    ...
    *  \brief 
    */
	PFExport static const MCHAR* PViewItemName(int index);
...
protected:
	...
	static bool m_InGetPViewManager; // true if inside GetPViewManager()
	static IPViewManager* m_manager; // the one and only PViewManager in the scene - set via GetPViewManager
