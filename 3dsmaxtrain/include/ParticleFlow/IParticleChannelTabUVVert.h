...
public:
      ...
      /*! \brief Check whether some particles have shared Tab<UVVert>.
	If true, then there is no need to get a Tab for each particle.
      */
	virtual bool		IsShared() const = 0;

      /*! \brief Gets the total number of actual Tabs (values) in the channel.
      */
	virtual int			GetValueCount() const = 0;

      /*! \brief Gets the value index of a particle.
      */
	virtual int			GetValueIndex(int particleIndex) const = 0;

      /*! \brief Gets the Tab of the valueIndex-th value
	virtual const TabUVVert* GetValueByIndex(int valueIndex) const = 0;
      */

      /*! \brief Gets the Tab for a particle with index particleIndex.
      */
	virtual const TabUVVert* GetValue(int particleIndex) const = 0;

      /*! \brief Gets the global Tab<UVVert> for all particles.
...
public:
	...
	virtual bool	SetValue(int particleIndex, UVVert value) = 0;
      ...
      /*! \brief Copies Tab to be a shared value for particles with indices in "particleIndices".
	Returns true if successful.
      */
	virtual bool	SetValue(Tab<int>& particleIndices, const TabUVVert* value) = 0;

      /*! \brief Creates a Tab of size one from the UVVert value and
	copies the Tab to be a shared value for particles with indices in "particleIndices".
	Returns true if successful.
      */
	virtual bool	SetValue(Tab<int>& particleIndices, UVVert value) = 0;

      /*! \brief Copies Tab to be a global value for all particles.
	Returns true if successful.
      */
	...
	virtual bool	SetValue(UVVert value) = 0;
      ...
      /*! \brief Copies the Tab value from fromParticle to toParticle.
      */
	virtual bool	CopyValue(int fromParticle, int toParticle) = 0;

      /*! \brief Copies the Tab value from fromParticle to toParticles.
      */
	virtual bool	CopyValue(int fromParticle, Tab<int>& toParticles) = 0;

      /*! \brief Copies the Tab value from fromParticle to all particles.
      */
