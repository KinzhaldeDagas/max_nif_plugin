...
\li Main file: The file into which scene entities of a source file are xrefed
\li XRef item: A main file scene entity that xrefs a specific source file scene entity 
\li Source item: A source file scene entity being xrefed into a main file
\li XRef record: Data structure within the main file that holds xref items and other
...
public:
	...
	*/
	...
	\param [in] nodeHandles - A list with handleIds of objects (nodes) to xref.
	...
	*/
	virtual IObjXRefRecord* AddXRefItemsFromFile(
		...
		bool promptObjNames, 
		const Tab<const TCHAR*>* objNames, 
		unsigned int xrefOptions,
		const Tab<INT64>* nodeHandles = nullptr) = 0;
	...
	*/
	...
	//! name that is already used by a main scene object.
	...
	*/
	...
	//! name that is already used by a main scene material.
	...
	*/
	...
	/** \deprecated Deprecated in terms of implementation as of 3ds Max 2017. 
		Please use the function GetTracksStatus/SetTracksStatus instead.
	*/ 
	//! \name XRef options
	...
	/*!	Merged controllers (as opposed to xref'd controllers) will be accessible and
		modifiable in the main scene.  Some references between merged and xref'd items
		of xref'd nodes may be lost.
		\param[in] in_merge - whether nodes will have merged controllers in the main scene.
		*/
	...
	/*!	Merged materials (as opposed to xref'd materials) will be accessible and
		modifiable in the main scene.  Some references between merged and xref'd items
		of xref'd nodes may be lost.
		\param[in] in_merge - whether nodes will have merged materials in the main scene.
		*/
	...
	/*!	Merged manipulators (as opposed to xrefed manipulators) will be accessible and
		modifiable in the main scene.  Some references between merged and xrefed items
		of xref'd nodes may be lost.
		\param[in] in_merge - whether nodes will have merged manipulators in the main scene.
		*/
	...
	/*!	Merged modifiers (as opposed to xref'd modifiers) will be accessible and
		modifiable in the main scene.  Some references between merged and xref'd items
		of xref'd nodes may be lost.
		\param[in] in_merge - how modifiers will appear in the main scene.  
...
// clang-format off
	...
	BEGIN_FUNCTION_MAP_PARENT(IObjXRefManager8)
		...
		PROP_FNS(kfpGetTracksStatus, GetTracksStatus,
			kfpSetTracksStatus, SetTracksStatus, TYPE_ENUM);
	END_FUNCTION_MAP
};
// clang-format on
