...
\param[out] extent - The text's extent as calculated via QFontMetricsF(qApp->font()).boundingRect({}, 0, text).
\return true if the text's extent was calculated (qApp exists, text not null).
*/
...
/*
   ...
   If removeUIScaling is true, size is divided by MaxSDK::GetUIScaleFactor() to remove DPI scaling
	If useQtTextWidth is true, the text extent's width is calculated via qApp->fontMetrics().width(text). The width calculated
	via GetTextExtentPoint32 is sometimes too small for Qt drawn controls.
*/
...
/*!  Wrapper around the windows function GetSystemMetrics.
	In Vista+ when running Aero, the value returned for SM_CXFRAME and SM_CYFRAME is incorrect in order
	to provide backwards compatibility with NT. To get the correct value, also need to add the value
	from SM_CXPADDEDBORDER. The implementation of this method automatically corrects the values returned
	for SM_CXFRAME and SM_CYFRAME.
	Note that SM_CXSIZEFRAME is the same index as SM_CXFRAME, and SM_CYSIZEFRAME as SM_CYFRAME;
	\param nIndex - The system metric or configuration setting to be retrieved.
*/

inline int DLGetSystemMetrics(int nIndex)
{
	int res = GetSystemMetrics(nIndex);
	static bool applyCorrection = true;
	if ( applyCorrection && (nIndex == SM_CXFRAME || nIndex == SM_CYFRAME))
		res += GetSystemMetrics(92); // SM_CXPADDEDBORDER == 92, defined only when _WIN32_WINNT >= 0x0600
	return res;
}
