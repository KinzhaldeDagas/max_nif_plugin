...
/*
	...
	2. Get the IMoFlow Interface from a corresponding BipDriver.
	...
	Example of creating a motion flow.
		IBipDriver *bipDriver = GetCurrentBipDriver(); //see
		POINT graphLocation;
		MSTR str;
		//begin motion flow mode.
		bipDriver->BeginModes(BMODE_MOTIONFLOW);
		//get the motion flow
		IMoFlow *moFlow = bipDriver->GetMoFlow();
		...
		bipDriver->UnifyMotion()
		bipDriver->EndModes(BMODE_MOTIONFLOW);
		return;

*/


// Forward Declarations--note that there are certain classes here (class animal, etc..)
...
class TranInfo: public MaxHeapOperators {
  ...
  public:
	...
	BIPExport		bool operator==(const TranInfo& TI) const;
...
class Snippet: public MaxHeapOperators {
  ...
  public:
	...
	Snippet* NextRealTime(IBipDriver *mc, Script *scr, animal *A, int frame, path_properties *desired_properties, int global_frame, int global_last_clip_start, int currentScript_index, int *transindex, int *found);
...
class IMoFlow: public MaxHeapOperators
{
  public:
	...
	virtual const IBipDriver* GetBipDriver() const = 0;
