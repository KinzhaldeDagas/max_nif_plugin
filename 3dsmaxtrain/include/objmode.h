...
class Transformer: public MaxHeapOperators
{
	...
	public:
		...
		CoreExport virtual void BeginDrag( IPoint2& m, Matrix3& axis );
...
class MoveTransformer : public Transformer
{
 	...
 	public:
		...
		CoreExport void BeginDrag( IPoint2& m, Matrix3& axis );
...
class SelectionProcessor : public MouseCallBack {
	...
	protected:
		...
		/** 
		 * By default, SelectChildren(ViewExp*) will be called
		 * when double click message is triggered.
		 * If this function returns true, system will handle 
		 * OverrideDoubleClickProc(ViewExp*, int) instead.
		 * \return true if double click is handled.
		 */
