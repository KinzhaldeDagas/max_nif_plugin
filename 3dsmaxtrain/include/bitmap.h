...
class bmmHistoryList: public MaxHeapOperators
{
	...
	public:
		...
		BMMExport const MCHAR* DefaultPath	( ) const { return m_initDir.data(); }
...
using BMMRES = unsigned short;              //!< Traps the use of int or BOOL
//!@{
...
class BMM_IOHandler : public MaxHeapOperators 
{
   ...
   public:
	  ...
	  BMMExport const MCHAR* Extension(int index, const MCHAR* e = nullptr);
	  /*! \remarks Returns the number of file name extensions for the IO
	  module. */
	  BMMExport int NumExtensions() const { return ioExtension.Count(); }
	  ...
	  BMMExport int GetDllNum() const { return dllNumber; }
	  ...
	  BMMExport DWORD GetCapabilities() const { return ioCapabilities; }
	  ...
	  \par Operators:
	  */
	  BMMExport BOOL TestCapabilities(DWORD cap) const { return (ioCapabilities & cap) != 0; }
...
class RenderInfo: public MaxHeapOperators {
   ...
   float kx = 0.f, ky = 0.f;            // 3D to 2D projection scale factor 
   float xc = 0.f, yc = 0.f;           // screen origin
   BOOL fieldRender = FALSE;    // field rendered?
   BOOL fieldOdd = FALSE;         // if true, the first field is Odd lines
...
#define MAP_LOADING				 ((DWORD)(1<<0))	//!< This bitmap / storage is currently being loaded, not finalized yet.
...
#define MAP_USE_STORAGE_FILTER   ((DWORD)(1 << 13)) //!< Experimental: storage with this flag wants to handle texture filtering itself (bypassing filter plugins).

#define MAP_STORAGE_CAN_SCALE    ((DWORD)(1 << 14)) //!< Experimental: storage with this flag can fill a passed buffer with the scaled image.
...
*/
   class BitmapInfo
		   ...
		   , private CountedObject<BitmapInfo>
   {
private:
		...
		/*! \see MAP_PROXY */
		BitmapInfo*				proxysubject = nullptr;
		...
		float aspect = 0.f, gamma = 0.f;
		int   fstart = 0, fend = 0;
		DWORD flags = 0, type = 0;

		MaxSDK::ColorManagement::InputColorSpaceInfo colorSpaceInfo;

		//-- User data (what user wants) ----------------------------
		//
		//   Custom dimensions, custom start and end point when
		//   reading sequence of images, frame to fetch/save, etc.
		//

		WORD  cwidth = 0, cheight = 0;
		int   custxpos = 0, custypos = 0;
		int   start = 0, end = 0;
		int   step = 0, preset_al = 0;
		float custgamma = 0.f;
...
//!@{
		...
		#define BMM_CUSTOM_HDR_TYPE	((DWORD)(1 << 6))	//!< Internal Use Only.  Bitmap has custom hdr type that reads storage info from BitMap Info and not from UI.	
...
//!@}
		...
		int fnumber = 0;           //-- Defines frame to
												  //   read or write.
...
//! @{
		...
		BMMExport DWORD          Flags                       ( ) const { return (flags); }
		...
		BMMExport BOOL           TestFlags                   ( DWORD f ) const { return (flags & f); }
//! @}
...
//! @{
		...
		BitmapInfo. */
		BMMExport WORD           Width                       ( ) const { return (width); }
		/*! \remarks Returns the height (vertical dimension) setting of this
		BitmapInfo. */
		BMMExport WORD           Height                      ( ) const { return (height); }
		/*! \remarks Returns the gamma setting property of this BitmapInfo. */
		BMMExport float          Gamma                       ( ) const { return (gamma); }

		/** Returns the name of the color space that's assigned to the bitmap.  
		 * You can not directly set the color space of a bitmap, but you can
		 * request a color space via the SetRequestedColorSpace function. The
		 * system will assign a color space based on the available and requested
		 * color spaces. Based on the color space availability the assigned
		 * color space may be different from the requested color space. If no
		 * color space is requested, system will use input rules and/or other
		 * system heuristics to assign a suitable color space.
		 *
		 * \return  %Name of the currently assigned color space.
		 *
		 * \see     ColorSpaceStatus, SetRequestedColorSpace,
		 *          GetRequestedColorSpace, ColorSpaceSource,
		 *          IModeSettings::GetFileIORule 
		 */
		BMMExport MSTR           ColorSpace					 ( ) const;
		
		/** Returns color space assignment source.  
		 * This value indicates how the current color space is assigned to this
		 * bitmap (user request, rules etc).  
		 *
		 * \return  ColSpaceSource enum for the current source of the assigned
		 *          color space.  
		 *
		 * \see     ColorSpace, SetRequestedColorSpace,
		 *          GetRequestedColorSpaceSource, ColorSpaceStatus 
		 */
		BMMExport MaxSDK::ColorManagement::ColSpaceSource ColorSpaceSource( ) const;


		/** Returns the status of the color space assignment.  
		 * \note    You can ValidateColorSpace() or SetRequestedColorSpace to
		 *          re-assign color space and update the status. 
		 *
		 * \return  ColSpaceStatus enum for the current status of the assigned
		 *          color space.  
		 * \see     ColorSpace, SetRequestedColorSpace, ValidateColorSpace    
		 */
		 ...
		 *
		 * \return  If the ColorSpaceSource is ColSpaceSource::InputRules, this
		 *          will return the name of the rule that's used for assigning
		 *          the color space. Otherwise it will return an empty string.  
		 * \see		ColorSpaceSource, ColSpaceSource, IModeSettings::GetFileIORule
		 */
		 ...
		 * \param   [in] outputConversion The OutputConversion to be used. If
		 *          set to Automatic or NoConversion, no other parameter has to
		 *          be present.  
		 * \param   [in] colorSpaceOrDisplayRequested If outputConversion is set
		 *          to ColorSpaceConversion, pass the requested color space in
		 *          here. If outputConversion is set to DisplayViewTransform,
		 *          pass the requested Display in here.  
		 * \param   [in] viewTransformRequested If outputConversion is set to
		 *          DisplayViewTransform, an optional view transform can be
		 *          passed in here.  
		 * \return  The status indicating if the requested color space or
		 *          display view transform could be assigned.  
		 * \see     ColSpaceStatus, OutputColorConversion, OutputColorSpace,
		 *          OutputDisplay, OutputViewTransform */
		BMMExport MaxSDK::ColorManagement::ColSpaceStatus SetOutputColorConversion(
				OutputColorConversionType outputConversion, const MSTR& colorSpaceOrDisplayRequested = MSTR(),
				const MSTR& viewTransformRequested = MSTR());

		/** Returns the assigned output color space.  
		 * If OutputColorConversion is set to ColorSpaceConversion, this
		 * function returns the assigned output color space, into that the image
		 * is converted during saving. Note that this can be different than what
		 * was requested, as not all color spaces may be available in the
		 * currently used OCIO config.  
		 * \param   [out] status If this is passed in by the caller, it will
		 *          receive the status of the output color space.  
		 * \param   [out] colorSpaceRequested If present, this will receive the
		 *          output color space that was originally requested.  
		 * \return  The output color spaces that is being used to save this
		 *          bitmap, if the ColorConversion is set to
		 *          ColorSpaceConversion.  
		 * \see     OutputColorConversion */
		BMMExport MSTR OutputColorSpace(
				MaxSDK::ColorManagement::ColSpaceStatus* status = nullptr, MSTR* colorSpaceRequested = nullptr) const;

		/** Returns the assigned output display color space.  
		 * If OutputColorConversion is set to DisplayViewTransform, this
		 * function returns the assigned display color space, into that the
		 * image is converted during saving, along with some optional view
		 * transform. Note that this can be different than what was requested,
		 * as not all display color spaces may be available in the currently
		 * used OCIO config.  
		 * \param   [out] status If this is passed in by the caller, it will
		 *          receive the status of the output display color space.  
		 * \param   [out] displayRequested If present, this will receive the
		 *          output display that was originally requested.  
		 * \return  The output display used to save this bitmap along with some
		 *          optional view transform, if ColorConversion is set to
		 *          DisplayViewTransform.  
		 * \see     OutputColorConversion, OutputViewTransform */
		BMMExport MSTR OutputDisplay(
				MaxSDK::ColorManagement::ColSpaceStatus* status = nullptr, MSTR* displayRequested = nullptr) const;

		/** Returns the assigned output view transform.  
		 * If OutputColorConversion is set to DisplayViewTransform, this
		 * function returns the assigned view transform, that is applied to the
		 * image during saving along with the conversion into the assigned
		 * display color space. Note that this can be different than what was
		 * requested, as not all view transforms may be available for the
		 * assigned display in the currently used OCIO config.  
		 * \param   [out] status If this is passed in by the caller, it will
		 *          receive the status of the output view transform.  
		 * \param   [out] viewTransformRequested If present, this will receive
		 *          the output view transform that was originally requested.  
		 * \return  The output view transform applied to the image during saving
		 *          along with the conversion into the assigned display color
		 *          space, if ColorConversion is set to DisplayViewTransform.  
		 * \see     OutputColorConversion, OutputViewTransform */
		BMMExport MSTR OutputViewTransform(MaxSDK::ColorManagement::ColSpaceStatus* status = nullptr,
				MSTR* viewTransformRequested = nullptr) const;

		/*! \remarks Returns the aspect ratio property of this BitmapInfo. */
		BMMExport float          Aspect                      ( ) const { return (aspect);}
		/*! \remarks Returns the type property of this BitmapInfo. See \ref bitmapTypes */
		BMMExport int            Type                        ( ) const { return (type); }
		 ...
		 * %Color space of a bitmap can not be directly set but can be requested
		 * either by the user or by the input rules. The system will assign a
		 * color space based on this request and the currently available color
		 * space. This function will return the requested color space. See
		 * ColorSpace for the currently assigned space.
		 *
		 * \return	%Name of the requested color space. 
		 * 
		 * \see     ColorSpace, ColorSpaceStatus, SetRequestedColorSpace,
		 *			ColorSpaceSource, GetRequestedColorSpaceSource
		*/
		BMMExport MSTR GetRequestedColorSpace() const; 


		/** Sets the requested color space.  
		 * The actual color space assigned may differ if the requested color
		 * space is not available. This function internally calls
		 * ValidateColorSpace to assign a color space based on the availability
		 * of the requested color space. Actually assigned color space may
		 * differ from the requested color space.
		 *
		 * \param   colorSpaceName %Name of the requested color space that's
		 *          being requested. This is either based on the user selection
		 *          or input rules. See IModSettings::GetNumFileIOColorSpaces
		 *          for list of currently available spaces.  
		 * \param   requestSource one of the ColSpaceSource enum values
		 *          indicating why this color space is being requested.  
		 * \return  Status of the current color space assignment.
		 *
		 */
		 ...
		 *
		 * \return  Source of the requested color space, this may be the
		 *          parameter passed to the SetRequestedColorSpace if the color
		 *          space was requested explicitly or an internally assinged
		 *          value if no request is made yet. 
		 */
		BMMExport MaxSDK::ColorManagement::ColSpaceSource GetRequestedColorSpaceSource ( ) const; 


		/** Checks and validates the assigned color space against the currently
		 * available color spaces. If the assigned color space is missing or not
		 * available in the current setup, then it assigns a valid color space
		 * based on the requested color space, file input rules and other
		 * settings and/or heuristics.
		 *
		 * \return  Status of the assigned color space after the validation an
		 *          potential assignment.
		 * \see     ColorSpace, ColorSpaceStatus, GetRequestedColorSpace,
		 *          ColorSpaceSource, SetRequestedColorSpace
		 */
		BMMExport MaxSDK::ColorManagement::ColSpaceStatus ValidateColorSpace();



		/** Clears all the color space information (including the requested and
		 * assigned color space data) to uninitialized state. After this
		 * function is called, the bitmap won't have any color space assigned to
		 * it before ValidateColorSpace or SetRequestedColorSpace is called.
		 * Various functions of the Bitmap class or other internal functions
		 * working on bitmaps may validate and auto assign a color space if the
		 * color space is in unassigned state.
		 * 
		 * \see     ColorSpace, ColorSpaceStatus, GetRequestedColorSpace,
		 *          ColorSpaceSource, SetRequestedColorSpace, ValidateColorSpace
		 */
...
//! @{
		...
		other than 0. */
		BMMExport int            FirstFrame                  ( ) const { return (fstart); }
		/*! \remarks Returns the last frame property of this BitmapInfo. */
		BMMExport int            LastFrame                   ( ) const { return (fend); }
		/*! \remarks Returns the total number of frames setting of this
		BitmapInfo. */
		BMMExport int            NumberFrames                ( ) const { return (fend - fstart + 1); }
		/*! \remarks Returns the current frame setting of this BitmapInfo. */
		BMMExport int            CurrentFrame                ( ) const { return (fnumber); }
		...
		BMMExport WORD           SequenceOutBound            ( ) const { return (loopflag); }
		//-- Generic Write
...
//! @{
		...
		image. */
		BMMExport const MCHAR* Device() const { return m_device.data(); }
...
//! @}
		...
		/*! \remarks Copies the image information of the <b>from</b> BitmapInfo
		to this bitmap. Only the name, device, image characteristics and color
		management related data are copied. User info, such as Custom Width,
		etc. is not copied.\n\n  
		...
		from-\>GetCustomFlags(), from-\>GetColorSpace(),
		from-\>ColorSpaceSource(), from-\>ColorSpaceStatus(),
		from-\>ColorSpaceRule(), from-\>OutputColorConversion(),
		from-\>OutputColorSpace(), from-\>OutputDisplay(),
		from-\>OutputViewTransform()</b>  
		\param from The bitmap whose information will be copied. */
...
//! @{
		...
		BMMExport WORD           CustWidth                   ( ) const { return (cwidth); };
		/*! \remarks Returns the custom height setting of this BitmapInfo. */
		BMMExport WORD           CustHeight                  ( ) const { return (cheight); }
		...
		BitmapInfo. */
		BMMExport int            StartFrame                  ( ) const { return (start); }
		/*! \remarks Returns the custom end frame setting of this BitmapInfo.
		*/
		BMMExport int            EndFrame                    ( ) const { return (end); }
		...
		BMMExport int            GetCustomX                  ( ) const { return custxpos; }
		/*! \remarks Returns the custom Y offset setting of this BitmapInfo. */
		BMMExport int            GetCustomY                  ( ) const { return custypos; }
		...
		BMMExport float          GetCustomGamma              ( ) const { return custgamma; }
		...
		*/
		BMMExport int            GetCustomStep               ( ) const { return step; }
		...
		BMMExport int            GetPresetAlignment          ( ) const { return preset_al; }
//! @}
...
//! @{
		...
		BMMExport DWORD          GetCustomFlags              ( ) const { return (customflags); }
		...
		*/
		BMMExport BOOL           TestCustomFlags             ( DWORD f ) const { return (customflags & f); }
//! @}
		...
		*/
		BMMExport DWORD         GetPiDataSize               ( ) const { return pisize; }
		/**
...
//! @}
	...
	private:
		...
		*/
		...
		const std::unique_ptr<class BitmapInfoImpl> mImpl;
...
{
	private:
	  UWORD* outputGammaTab = nullptr;   // this may be owned by gammaMgr
	  UWORD* privGammaTab = nullptr;     // private gamma table owned by the BitmapIO.
		   
   protected:
	  float gamma = 0.f;        //!< The gamma setting.
	  Bitmap* map = nullptr;         //!< The Bitmap using this OUTPUT handler
	  BitmapStorage* storage = nullptr;     //!< The BitmapStorage used by this INPUT handler

	  int openMode = 0;    //!< The mode that the IO module is open for - see \ref bitmapOpenModeTypes
	  
	  //-- Linked list pointers for multiple output of a single bitmap

	  BitmapIO* prevIO = nullptr;      //!< A linked list pointer to the previous IO module for multiple outputs of a single bitmap.
	  BitmapIO* nextIO = nullptr;      //!< A linked list pointer to the next IO module for multiple outputs of a single bitmap.
...
///@}
	  ...
	  */
	  BMMExport virtual  BOOL           ValidatePiData              ( BitmapInfo *pbi, ILoad *iload = nullptr );
...
//! @{
	  ...
	  BMMExport virtual  BMMRES         GetImageInfoDlg             ( HWND hWnd, BitmapInfo *pbi, const MCHAR *filename = nullptr ) {return BMMRES_NODRIVER;}
	  ...
	  BMMExport virtual void EvalMatch(MSTR& matchString) { matchString = _T(""); }
//! @}
...
//! @}
	  ...
	  \par Default Implementation:
	  <b>{ return NULL; }</b> */
	  BMMExport virtual  PAVIFILE       GetPaviFile                 ( ) { return nullptr; }
...
class BitmapStorage : public BaseInterfaceServer
{
	friend class BitmapManagerImp;
   protected:
	  int openMode = 0;                   // See above
	  UINT usageCount = 0;                 // Number of Bitmaps using this storage
	  BitmapManager* manager = nullptr;

	  int flags = 0;
	  int type = 0;                       // See "Basic bitmap types", below

	  BMM_Color_48 palette[256] = {};               // 256 palette entries max
	  int paletteSlots = 0;
	  UWORD* gammaTable = nullptr;               // Gamma correction table
	  
	  RenderInfo* rendInfo = nullptr;
	  GBuffer* gbuffer = nullptr;

	  MSTR mFilename; // if Read-only mode, the file name read from
	  FILETIME mFileTimeStamp{ 0, 0 }; // if Read-only mode, the file modification time of file when read
   public:
	  ...
	  inline int HasGamma() const { return (gammaTable != nullptr) ? 1 : 0; }
	  /*! \remarks Implemented by the System.\n\n
	  ...
	  /*! This function will return the pointer of the IColorPipeline object that is
	   * used to convert the colors from bitmap's input color space into the
	   * currently active rendering color space. Do not delete the returned pointer. 
	   */
	  BMMExport virtual MaxSDK::ColorManagement::IColorPipeline* GetPipelineToRenderingSpace();

	  /*! \remarks Implemented by the System.\n\n
	  Returns the bitmap manager for the storage. */
	  inline BitmapManager* Manager() const { return manager; }
	  /*! \remarks Implemented by the System.\n\n
	  Returns the mode the storage was opened in. See \ref bitmapOpenModeTypes
	  */
	  inline int OpenMode() const { return openMode; }
	  /*! \remarks Implemented by the System.\n\n
	  Returns the width (horizontal dimension) of the storage's BitmapInfo
	  instance (<b>bi.Width()</b>). */
	  inline int Width() const { return bi.Width(); }
	  /*! \remarks Implemented by the System.\n\n
	  Returns the height (vertical dimension) of the storage's BitmapInfo
	  instance (<b>bi.Height()</b>). */
	  inline int Height() const { return bi.Height(); }
	  /*! \remarks Implemented by the System.\n\n
	  Returns the aspect ratio of the storage's BitmapInfo instance
	  (<b>bi.Aspect()</b>). */
	  inline float Aspect() const { return bi.Aspect(); }
	  /*! \remarks Implemented by the System.\n\n
	  Returns the gamma setting of the storage's BitmapInfo instance
	  (<b>bi.Gamma()</b>). */
	  inline float Gamma() const { return bi.Gamma(); }
	  ...
	  inline int ScaleColors() const { return (flags & MAP_USE_SCALE_COLORS) ? 1 : 0; }
	  /*! \remarks	  Implemented by the System.\n\n
	  ...
	  <b>BMM_Color_fl\& in</b>\n\n
	  The value to convert. */
	  inline void ClampScaleColor(BMM_Color_64& out, const BMM_Color_fl& in) const { if (ScaleColors()) ScaleColor(out, in); else ClampColor(out, in); }
	  /*! \remarks	  Implemented by the System.\n\n
	  Converts in to out, using the value of <b>ScaleColors()</b> to determine
	  ...
	  <b>BMM_Color_fl\& in</b>\n\n
	  The value to convert. */
	  inline void ClampScaleColorA (BMM_Color_64& out, const BMM_Color_fl& in) const { if (ScaleColors()) ScaleColorA(out, in); else ClampColorA(out, in); }
	  
	  /*! \remarks Implemented by the System.\n\n
	  Returns the number of times this image is being used in the system. */
	  inline int UsageCount() const { return usageCount; };
	  /*! \remarks Implemented by the System.\n\n
	  ...
	  */
	  ...
	  BMMExport virtual void           *GetStoragePtr               ( int *bmmType ) { *bmmType = BMM_NO_TYPE; return nullptr; }
	  ...
	  width of the image. */
	  BMMExport virtual void           *GetAlphaPtr                 ( int *bmmType) { *bmmType = BMM_NO_TYPE; return nullptr; }
	  ...
	  storage type, they will automatically get these copy functions as these
	  are implemented in the base class. */
	  BMMExport virtual int             CopyScaleHigh               ( Bitmap *from, HWND hWnd, BMM_Color_64 **buf = nullptr, int w=0, int h=0 ) = 0;
	  /*! \remarks Implemented by the System.\n\n
	  This method copies the specified bitmap to this storage. The source
	  bitmap is scaled to fit using a higher quality but slower algorithm than
	  <b>CopyScaleLow()</b>. This is an internal function implemented within
	  ...
	  storage type, they will automatically get these copy functions as these
	  are implemented in the base class. */
	  BMMExport virtual int             CopyScaleHigh               ( Bitmap *from, HWND hWnd, BMM_Color_fl **buf = nullptr, int w=0, int h=0 ) = 0;
	  ...
	  BMMExport virtual int             CopyImage                   ( Bitmap *from,int operation,BMM_Color_64 fillcolor, BitmapInfo *bmInfo = nullptr) = 0;
	  /*! \remarks Copies the specified bitmap to this storage. The image is cropped or
	  resized as specified.
	  \par Parameters:
	  <b>Bitmap *from</b>\n\n
	  ...
	  If present, the code checks the option flags and acts accordingly.
	  \return  Nonzero if the copy was performed; otherwise 0. */
	  BMMExport virtual int             CopyImage                   ( Bitmap *from,int operation,BMM_Color_fl fillcolor, BitmapInfo *bmInfo = nullptr) = 0;
	  ...
	  //fills an arbitrarily-sized buffer with the scaled copy of the image it holds.
	  //This has potential to use smaller mip levels, avoiding full texture loading and processing 
	  //this is implemented only if the storage has the MAP_STORAGE_CAN_SCALE flag set.
	  //only few destBMMTypes are supported at this time.
	  BMMExport virtual int ResizeInto(void* /*destBuffer*/, int /*destBMMType*/, int /*width*/, int /*height*/, int /*linePadding*/, bool /*highQ*/) { return 0; }
	  ...
	  */
	  ...
	  BMMExport virtual void			UpdateColorConverters		( ); 
	  ...
	  \par Default Implementation:
	  <b>{ return NULL;}</b> */
	  BMMExport virtual void* GetChannel(ULONG channelID, ULONG& chanType) { return gbuffer ? gbuffer->GetChannel(channelID, chanType) : nullptr; }
...
public:
	...
	<b>{ return(0); }</b>\n\n
	*/
	BMMExport int IsHighDynamicRange() override { return 0; }
	...
	\return  Nonzero if pixels were retrieved; otherwise 0. */
	BMMExport int Get16Gray(int x, int y, int pixels, float* ptr) override;
	...
	\return  Nonzero if pixels were stored; otherwise 0. */
	BMMExport int Put16Gray(int x, int y, int pixels, float* ptr) override;
	...
	Pointer to the storage for the retrieved pixels.
	\return  Nonzero if pixels were retrieved; otherwise 0. */
	BMMExport int GetLinearPixels(int x, int y, int pixels, BMM_Color_fl* ptr) override;
	...
	\return  Returns nonzero if pixels were retrieved; otherwise 0. If
	storage has not been allocated 0 is returned. */
	BMMExport int GetPixels(int x, int y, int pixels, BMM_Color_fl* ptr) override;
	...
	\return  Returns nonzero if pixels were stored; otherwise 0. If storage
	has not been allocated 0 is returned. */
	BMMExport int PutPixels(int x, int y, int pixels, BMM_Color_fl* ptr) override;
	...
	Pointer to the storage for the pixels.
	\return  Nonzero if pixels were retrieved; otherwise 0. */
	BMMExport int GetIndexPixels(int x, int y, int pixels, unsigned char* ptr) override = 0;
	...
	The pixels values to store.
	\return  Nonzero if pixels were stored; otherwise 0. */
	BMMExport int PutIndexPixels(int x, int y, int pixels, unsigned char* ptr) override = 0;
	...
	color used to fill the new pixels.
	\return  Nonzero if the image was cropped; otherwise 0. */
	BMMExport int CropImage(int width, int height, BMM_Color_fl fillcolor) override;
	...
	*/
	...
	<b>BMM_Color_fl *ptr</b>\n\n
	The result is returned here - the average over the specified area. */
	BMMExport int GetFiltered(float u, float v, float du, float dv, BMM_Color_fl* ptr) override;
};
...
public:
	...
	\par Default Implementation:
	<b>{ return(1); }</b> */
	BMMExport int IsHighDynamicRange() override { return 1; }
...
class BitmapFilter: public MaxHeapOperators
{
   protected:
	  UINT usageCount = 0;                 // Number of Bitmaps using this storage
	  BitmapManager* manager = nullptr;                    // Pointer to bitmap manager
	  BitmapStorage* storage = nullptr;                    // Pointer to storage itself
	  DWORD flags = 0;                      // Filter flags
	  int dirty = 0;                      // Needs updating flag
	  UINT type = 0;                       // Type index of filter
...
class BitmapDither: public MaxHeapOperators
{
   protected:
	  BitmapStorage* storage = nullptr;                    // Pointer to storage itself
	  int type = 0;                       // Type index of filter
...
*/
class Bitmap
			: public BaseInterfaceServer
			, private CountedObject<Bitmap>
{
   friend class BitmapManagerImp;
   ...
   private:
	  ...
	  BitmapManager* manager = nullptr;                   //!< Manager of this bitmap
	  BitmapIO* output = nullptr;                    //!< Head of output handler list
	  BitmapFilter* filter = nullptr;                    //!< Filtered access methods
	  BitmapDither* dither = nullptr;                    //!< Dither methods
	  BitmapStorage* storage = nullptr;                   //!< Actual storage
	  UINT filterType = 0;                //!< Filtered access type
	  UINT ditherType = 0;                //!< Dither type
	  DWORD modifyID = 0;                  //!< changes when bitmap changes: used in render effects
	  int Free();

	  void* vfbData = nullptr;

	  BitmapNotify* bmNotify = nullptr;		//!< Called when storage has changed so display can update.

	  const std::unique_ptr<class BitmapImpl> mImpl; // private implementation details
...
//! @{
	  ...
	  inline  DWORD Flags()  const { return flags; };
...
///@{
	  ...
	  inline int Paletted() const { if (storage) return storage->Paletted(); return 0; }
	  ...
	  inline int IsDithered() const { if (storage) return storage->IsDithered(); return 0; }
	  /*! \remarks Returns whether the bitmap uses pre-multiplied alpha.
	  \return  If storage has been allocated returns nonzero if the bitmap uses
	  pre-multiplied alpha; otherwise 0. */
	  inline int PreMultipliedAlpha() const { if (storage) return storage->PreMultipliedAlpha(); return 0; }
	  ...
	  inline int HasAlpha()  const { if (storage) return storage->HasAlpha(); return 0; };
	  /*! \remarks	  Returns nonzero if this is a bitmap that supports high dynamic range
	  data; zero if it doesn't. */
	  inline int IsHighDynamicRange() const { if (storage) return storage->IsHighDynamicRange(); return 0; }
	  /*! \remarks This method returns the number of bits per pixel for each
	  color component. For example a 24-bit TARGA has a <b>MaxRGBLevel()</b> of
	  8. */
	  inline int MaxRGBLevel() const { if (storage) return storage->MaxRGBLevel(); return 0; }
	  /*! \remarks Implemented by the System.\n\n
	  Returns the number of bits per pixel in the alpha channel.
	  \return  If storage has not been allocated returns 0. */
	  inline int MaxAlphaLevel() const { if (storage) return storage->MaxAlphaLevel(); return 0; }
...
//! @{
	  ...
	  allocated, returns 0. */
	  inline int ScaleColors() const { if (storage) return storage->ScaleColors(); return 0; };
	  /*! \remarks	  Implemented by the System.\n\n
	  Converts in to out clamping the RGB components to 0 to 65535. The alpha
	  component is not copied.
	  \par Parameters:
...
//! @{
	  ...
	  inline  int              CopyImage                   ( Bitmap *from,int operation,BMM_Color_64 fillcolor, BitmapInfo *bi = nullptr)
...
//! @}
	  ...
	  If present, the code checks the option flags and acts accordingly.
	  \return  Nonzero if the copy was performed; otherwise 0. */
	  inline  int              CopyImage                   ( Bitmap *from,int operation,BMM_Color_fl fillcolor, BitmapInfo *bi = nullptr)
														   { if (storage) return storage->CopyImage(from,operation,fillcolor,bi); return 0; };
...
//! @{
	  ...
	  inline GBuffer *GetGBuffer()  { return storage? storage->GetGBuffer(): nullptr; } 
	  ...
	  */
	  ...
	  Class RenderInfo. */
	  inline RenderInfo*           GetRenderInfo()        { if (storage) return storage->GetRenderInfo(); return nullptr; }

	  /*! \remarks This is used internally. It returns a pointer to the
	  RenderInfo instance allocated by the storage. If this could not be
	  allocated NULL is returned. See Class RenderInfo. */
	  inline RenderInfo*           AllocRenderInfo()     { if (storage) return storage->AllocRenderInfo(); return nullptr; }
...
//! @{
	  ...
	  /*! \remarks Sets the storage pointer to nullptr. */
	  inline    void          NullStorage                ( ) { storage = nullptr; };
//! @}
...
//! @{
    ...
    \endcode      
	  ...
	  banding effects).\n\n
	  <b>BOOL doDisplayTransform=FALSE</b>\n\n
	  Specifies whether the output should be transformed to screen color space.
	  If so, the gamma table parameter is ignored, and the DIB colors are transformed
	  using display gamma (for the gamma mode) or the default "Frame Buffer" 
	  display/view transform (for the OCIO modes).
	  */
	  BMMExport PBITMAPINFO    ToDib                       ( int depth = 24, UWORD *gam=nullptr, BOOL doDither=FALSE, BOOL doDisplayTransform=FALSE );
...
//! @{
	  ...
	  BMMExport int            Display                     ( const MCHAR *title = nullptr, 	int position = BMM_CN, 
			BOOL autonomous = FALSE, BOOL savebutton = TRUE, CropCallback *crop=nullptr, Bitmap *cloneMyVFB = nullptr );
...
//! @}
	  ...
	  DWORD GetModifyID() const { return modifyID; }
	  void SetModifyID(DWORD m) { modifyID = m; }
	  BMMExport void IncrModifyID();

//! \name Handy built-in functions
//! @{
	  //! \brief Print the bitmap (if supported by the host app)
	...
	public:
		...
		*/
		virtual BOOL BrowseBitmapFilesOutput(BitmapInfo* info, HWND hWnd, const MCHAR* title,
				const MCHAR* extension = nullptr, const MCHAR* config = nullptr, const MCHAR* map = nullptr) = 0;
};
...
//TODO - 2025 - remove or replace with override specifying 'bool save'
...
//! \name Creation / Loading
//! @{
	  //-- Creating a new bitmap from scratch -------------------------------
	  //
	  ...
	  */
	  ...
	  //
	  //   BitmapInfo defaults to frame "zero". For multiframe files, such as
	  ...
	  BMMExport virtual Bitmap* Load(BitmapInfo* bi, BMMRES* status = nullptr) = 0;
	  ...
	  /** Applies OpenColorIO based color conversion into a new bitmap of type
	   * BMM_FLOAT_RGBA_32 according to the given parameters.  
	   *
	   * This function uses the IColorPipelineManager to convert the pixel data
	   * of one Bitmap into a new Bitmap with the same dimensions as the source
	   * bitmap and the type BMM_FLOAT_RGBA_32.  
	   * Internally, this function calls ColorConvertInto, so all the parameters
	   * behave the same: If toColorSpaceOrDisplay is passed in only, this
	   * function does a color space conversion (interpreting
	   * toColorSpaceOrDisplay as the name of the color space to convert to).  
	   * If both toColorSpaceOrDisplay and toViewTransform are passed in, the
	   * function does a display/view transform conversion (taking
	   * toColorSpaceOrDisplay as the name of the display).  
	   * The source color space used for the conversion is extracted from the
	   * BitmapInfo of the source Bitmap.  
	   *
	   * \note For information about the available color spaces or display /
	   *       view transforms please check the documentation of
	   *       MaxSDK::ColorManagement::IColorPipelineMgr.
	   *
	   * \param [in] source The bitmap to be converted. Note that the color
	   *        space of this bitmap must not be invalid.  
	   * \param [in] toColorSpaceOrDisplay The name of the color space or
	   *        display (if toViewTransform is present) to convert the pixel
	   *        data into.  
	   * \param [in] toViewTransform The name of the view transform for the
	   *        display /view transform conversion used to convert the pixel
	   *        data into. Note that this view transform has to be valid for the
	   *        given display (toColorSpaceOrDisplay).  
	   *
	   * \return The newly created Bitmap with the data converted according to
	   * the given parameters.  
	   *
	   * \see ColorConvertInto, BitmapInfo::ColorSpace,
	   *      BitmapInfo::ColorSpaceStatus,
	   *      MaxSDK::ColorManagement::IColorPipelineMgr */
	  BMMExport virtual Bitmap* ColorConvert(
			  const Bitmap* source, const MSTR& toColorSpaceOrDisplay, const MSTR* toViewTransform = nullptr) = 0;

	  /** Applies OpenColorIO based color conversion from a given Bitmap into
	   * another according to the given parameters.  
	   *
	   * This function uses the IColorPipelineManager to convert the pixel data
	   * of one bitmap into another. If toColorSpaceOrDisplay is passed in only,
	   * this function does a color space conversion (interpreting
	   * toColorSpaceOrDisplay as the name of the color space to convert to).  
	   * If both toColorSpaceOrDisplay and toViewTransform are passed in, the
	   * function does a display/view transform conversion (taking
	   * toColorSpaceOrDisplay as the name of the display).  
	   ...
	   * \param [in] source The bitmap to be converted. Note that the color
	   *        space of this bitmap must not be invalid.  
	   * \param [in,out] target The bitmap to receive the converted pixel data.
	   *        It has to have the same dimensions as the source Bitmap.  
	   * \param [in] toColorSpaceOrDisplay The name of the color space or
	   *        display (if toViewTransform is present) to convert the pixel
	   *        data into.  
	   * \param [in] toViewTransform The name of the view transform for the
	   ...
	   *        given display (toColorSpaceOrDisplay).  
	   *
	   * \return BMMRES_SUCCESS or BMMRES_INTERNALERROR if some error occurs.  
	   *
	   * \see ColorConvert, BitmapInfo::ColorSpace,
	   *      BitmapInfo::ColorSpaceStatus,
	   *      MaxSDK::ColorManagement::IColorPipelineMgr */
	  BMMExport virtual BMMRES ColorConvertInto(const Bitmap* source, Bitmap* target,
			  const MSTR& toColorSpaceOrDisplay, const MSTR* toViewTransform = nullptr) = 0;
	  //! @}
...
//! @{
	  ...
	  \return  TRUE if the user exited the dialog using OK; otherwise FALSE. */
	  virtual BOOL SelectFileOutput(BitmapInfo* bi, HWND hWnd, const MCHAR* title = nullptr, ULONG* pflags = nullptr,
									const MCHAR* extension = nullptr, const MCHAR* config = nullptr, const MCHAR* map = nullptr) = 0;
	  ...
	  /*! Updates color management related settings and caches in all bitmap
		storages. This includes gamma tables, color spaces names and color
		pipeline caches used to convert native colors into rendering space. This
		will also update all the VFB's.
	  */
...
//! \brief This overload was added to allow passing temporary IAssetAccessor to DeclareBitmapAsset
BMMExport void DeclareBitmapAsset(IEnumAuxAssetsCallback& enumCallback, IAssetAccessor&& anAccessor, BitmapInfo* bitmapInfo, int flags = 0);
...
extern           void			ExtractExtension	( const MCHAR *string, MCHAR *ext, size_t extSize );


//-- Public Utilities ---------------------------------------------------------

/*! \brief Create a filename appended by the specified frame number. This function will pad the filename with zeros.
...
the correct path. When using the second version, you must specify the size in characters of the nameout buffer using nameoutSize.
If the buffer is not big enough to hold the output (including a null terminator), the result is truncated. \n\n
...
BMMExport BOOL			BMMGetFullFilename			( const MCHAR *namein, MCHAR* nameout, size_t nameoutSize ); //!< Resolves the given input name (namein) into the given output (nameout)
//!@}
...
/*! The three out parameters are allowed to be NULL. For non-NULL parameters you must specify the size in characters of the buffer. If the buffer is not big enough to hold the output (including a null terminator), the result is truncated.
	...
	\param[in]  directorySize The size in characters of the directory buffer
	\param[out] filename      A string containing the file name portion of the file path
	\param[in]  filenameSize  The size in characters of the filename buffer
	\param[out] extension     A string containing the file extension portion of the file path
	\param[in]  extensionSize The size in characters of the extension buffer */
...
//
// Note that you must specify the size in characters of the out_uncname buffer using in_uncnameSize.
//  If the buffer is not big enough to hold the output (including a null terminator), the result is truncated.
//
//	BMMFindNetworkShare()
//	---------------------
...
BMMExport ColorPacker *BMMNewColorPacker(
	...
	BYTE* remap=nullptr	// optional remap done at last stage.
	);
