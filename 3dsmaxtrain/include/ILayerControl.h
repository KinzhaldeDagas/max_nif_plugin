...
//! You get this interface by calling  
//! \code static_cast<IAnimLayerControlManager*>(GetCOREInterface(IANIMLAYERCONTROLMANAGER_INTERFACE )) 
//! \endcode
...
public:
	...
	//! it doesn't delete layer controllers if it's not specified in the filter.
	...
	//! \param[in] client The parent of the animatable we want to replace.
	...
	//! \param[in] client The parent of the animatable we want to replace.
	//! \param[in] subnum The 'anim' animatable subanim's number on the client.
	//! \return Returns whether or not we successfully enabled a layer here. Some reasons why we may not be successful is if a
	...
	virtual void SetJustUpToActive(bool val)=0;
	...
	//! that are being merged.  We can only collapse the keys if the two controllers are of the same class and are both keyable.
