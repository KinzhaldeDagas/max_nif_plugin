...
public:
	...
	/*! \brief Gets low level control over drag and drop operations.
		...
		associated high-level drag and drop handler methods described next.\n\n
	...
	\param window - The specified handle to the window in which the drag and drop 
		event is occurring. This is one of the windows that was enabled via 
...
public:
   // each DropClipFormat instance created is kept in the clipFmts table
   /*! \remarks Constructor.\n\n
   Each <b>DropClipFormat</b> instance created is kept in the <b>clipFmts</b>
   table */
   DropClipFormat()
   {
       DropClipFormat* cf = this;
       clipFmts.Append(1, &cf);
   }

   /*! \remarks Destructor.*/
   ~DropClipFormat() override
   {
       for (int i = 0; i < clipFmts.Count(); ++i)
       {
            if (clipFmts[i] == this)
            {
                clipFmts.Delete(i, 1);
...
public:
   ...
   which are listed in \ref dropTypes 
   \par Parameters:
   <b>int code</b>\n\n
   The DropType code.
   \par Default Implementation:
...
public:
   ...
   CoreExport static void SetDragSetting(const MCHAR* setting);
