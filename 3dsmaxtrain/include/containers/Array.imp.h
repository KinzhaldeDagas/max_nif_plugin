...
Array<T>::Array(InputIt begin, InputIt end)
		: mpArray(nullptr)
		, mReservedLen(0)
		, mUsedLen(0)
		, mGrowLen(kDefaultGrowthLength)
...
template <class T> Array<T>::~Array()
{
	...
	static_assert(std::is_trivial<T>::value || (std::is_copy_constructible<T>::value),
		"The type stored by MaxSDK::Array must be trivial or it must have a copy constructor.");
	static_assert(std::is_trivial<T>::value || (std::is_copy_assignable<T>::value),
		"The type stored by MaxSDK::Array must be trivial or it must have a copy assignment operator.");
