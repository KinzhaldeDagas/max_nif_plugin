...
public:
	...
	\return true if the CustAttrib was appended, false if not. Will fail if attribute is null or would cause a circular
	dependency. In this case it is the responsibility of the caller to delete the CustAttrib if need be. */
	virtual bool AppendCustAttrib(CustAttrib* attribute) = 0;
	...
	\return true if the CustAttrib was set, false if not. Will fail if attribute is null, would cause a circular
	dependency, or the index is out of range. In this case it is the responsibility of the caller to delete the
	CustAttrib if need be. */
	virtual bool SetCustAttrib(int i, CustAttrib* attribute) = 0;
	...
	dependency, or the index is out of range. In this case it is the responsibility of the caller to delete the
	CustAttrib if need be. */
	virtual bool InsertCustAttrib(int i, CustAttrib* attribute) = 0;
