...
enum class ColorPipelineMode : uint8_t
{
	...
	kOCIO_ENVVAR,	//!< Using the config file pointed by the OCIO environment variable.
...
{
	kNONE				= 0ul,
	kEXISTS				= 1ul << 0, //!< This color space exists. If flags is zero passed color space name is not valid.
	kIS_DATA			= 1ul << 1, //!< This is a data color space, colors won't be altered by the color pipelines.
	kHAS_FILETRANSFORM	= 1ul << 2	//!< Color space has one or more file transforms in its definition.
};
DEFINE_ENUM_FLAG_OPERATORS(ColorSpaceFlags)

/// Flags that can be passed to functions operating on lists (such as
/// GetNumFileIOColorSpaces() ) to control the list content.
enum class ListOptions : uint32_t
{
	kNONE			= 0ul,
	kUI_NAME		= 1ul << 0,	//!< Instead of returning original names, return decorated UI names (which may not be valid names to pass to functions)
	kINCLUDE_ROLES	= 1ul << 1,	//!< Color space lists will include roles in them.
};
DEFINE_ENUM_FLAG_OPERATORS(ListOptions)
...
public:
	 ...
	 * one of the ColorPipelineMode::kOCIO_XXX modes.  
	 * \see     ColorPipelineMode */
	...
	/** Returns flags for the given color space name.  
	 * \param   [in] colorSpace The name of the color space that's queried.  
	 * \return  A bitmask comprised of ColorSpaceFlags values. If the color
	 *          space does not exist or any error occurs during the operation
	 *          then 0 will be returned.  
	 * \see     ColorSpaceFlags */
	virtual ColorSpaceFlags GetColorSpaceFlags(const MSTR& colorSpace) const = 0;
	 ...
	 * then NOTIFY_COLOR_MANAGEMENT_PRE_CHANGE,
	 ...
	 * mode.  
	 * If the operation is successful, NOTIFY_COLOR_MANAGEMENT_PRE_CHANGE,
	 * NOTIFY_COLOR_MANAGEMENT_POST_CHANGE and
	 ...
	 * \see		ListOptions */
	virtual size_t GetNumRenderingColorSpaces(ListOptions options = ListOptions::kNONE) const = 0;
	 ...
	 * \param   [in] options Options controlling the list to operate on and
	 *			content of the returned value. see ListOptions for available options.  
	 * \return  The name of the %Rendering %Color Spaces. with the given index
	 *          of an empty string (if the given idx is invalid).  
	 * \see		ListOptions */
	virtual MSTR GetRenderingColorSpace(size_t idx, ListOptions options = ListOptions::kNONE ) const = 0;
	 ...
	 * \see		ListOptions */
	virtual size_t GetRenderingColorSpaceIndex(const MSTR& colorSpaceName, ListOptions options = ListOptions::kNONE) const = 0;
	 ...
	 * If the given %Color Space can be applied,
	 ...
	 * applied, NOTIFY_COLOR_MANAGEMENT_PRE_CHANGE,
	 * NOTIFY_COLOR_MANAGEMENT_POST_CHANGE and
	 * NOTIFY_COLOR_MANAGEMENT_POST_CHANGE2 will be sent.  
	 * \note    %Color Space name comparisons in OCIO are always
	 *          case-insensitive.  
	 ...
	 *          IColorPipelineMgr::GetLastErrorText() may have information about
	 *          the cause of the failure.  
	 * \see     GetDataColorSpaceName */
	virtual bool SetDataColorSpaceName(const MSTR& dataColorSpaceName) = 0;


	/** Returns the name of the %Color Space in the current config that is
	 * equivalent to linear-sRGB color space. This %color space is internally
	 * used for converting colors which are assumed to be linear sRGB (such as
	 * the results of legacy %color temperature calculations).  
	 * \return  If a linear-sRGB equivalent %color space is found in the current
	 *          config, name of that %color space is returned. If the heuristics
	 *          fail, this function may return an empty string.   
	 * \see     SetLinearSRGBColorSpaceName */
	virtual MSTR GetLinearSRGBColorSpaceName() const = 0;


	/** Sets the %Color Space that is used for specifying linear sRGB colors or
	 * images. If the given %Color Space can be applied,
	 * NOTIFY_COLOR_MANAGEMENT_PRE_CHANGE, NOTIFY_COLOR_MANAGEMENT_POST_CHANGE
	 * and NOTIFY_COLOR_MANAGEMENT_POST_CHANGE2 will be sent.  
	 * \note    %Color Space name comparisons in OCIO are always
	 *          case-insensitive.  
	 * \param   [in] linSRGBColorSpaceName The name of the %Color Space to be
	 *          used.  
	 * \return  True if the given %Color Space could be set. If the operation
	 *          was unsuccessful it will return false.
	 *          IColorPipelineMgr::GetLastErrorText() may have information about
	 *          the cause of the failure.  
	 * \see     GetLinearSRGBColorSpaceName */
	virtual bool SetLinearSRGBColorSpaceName(const MSTR& linSRGBColorSpaceName) = 0;
	 ...
	 * \see     GetFileIOColorSpace, GetFileIOColorSpaceIndex, ListOptions */
	virtual size_t GetNumFileIOColorSpaces(ListOptions options = ListOptions::kNONE) const = 0;
	 ...
	 * \see     GetNumFileIOColorSpaces, GetFileIOColorSpaceIndex, ListOptions */
	virtual MSTR GetFileIOColorSpace(size_t idx, ListOptions options = ListOptions::kNONE) const = 0;
	 ...
	 * \see     GetNumFileIOColorSpaces, GetFileIOColorSpace, ListOptions */
	virtual size_t GetFileIOColorSpaceIndex(const MSTR& colorSpaceName, ListOptions options = ListOptions::kNONE) const = 0;
	 ...
	 * \param   [in] formatType The output bitmap format type for which the
