 ...
 *  This bit mask is passed in NotifyInfo::callParam with the
 *  NOTIFY_COLOR_MANAGEMENT_POST_CHANGE and NOTIFY_COLOR_MANAGEMENT_POST_CHANGE2
 *  notification codes. In rare occasions it's possible to get those
 *  notifications with none of the bits set, this happens during the "load color
 *  management settings" operation for example, where the loaded settings are
...
enum NotificationChangeMask : uint32_t
{
	...
	OutputSettingsChanged	= 1 << 4,   //!< Settings that affect the color management settings that are used in bitmap saving have changed.
...
public:
	...
	*/
	 ...
	 * NOTIFY_COLOR_MANAGEMENT_POST_CHANGE and
	 ...
	 * management modes. If the operation is successful,
	 * NOTIFY_COLOR_MANAGEMENT_PRE_CHANGE,
	 * NOTIFY_COLOR_MANAGEMENT_POST_CHANGE and
	 ...
	 * ColorPipelineMode::kGAMMA mode. If the operation is successful,
	 * NOTIFY_COLOR_MANAGEMENT_PRE_CHANGE,
	 * NOTIFY_COLOR_MANAGEMENT_POST_CHANGE and
	 * NOTIFY_COLOR_MANAGEMENT_POST_CHANGE2 notifications will be broadcast with
	...
	*/
	...
	/**
	 * Returns the viewing pipeline using the default display and view transform
	 * for the given target.
	 *
	 * \param   [in] eTarget User may have set different displays and view
	 *          transforms for different parts of the system, this parameter
	 *          specifies which target to use.
	 * \param   [in] inverse Whether to create a pipeline with forward (false) or
	 *          inverse (true) display/view transforms.
	 * \param   [in] chanTM Pointer to an array of 16 double precision floating point
	 *          numbers forming a 4x4 matrix which can be used for channel
	 *          swizzling or any linear channel mixing/scaling operations.
	 * \return  If successful, returns a unique_ptr to the IColorPipeline
	...
	/**
	 * Returns the viewing pipeline using the specified display and view
	 * transforms.
	 *
	 * \param   [in] sFromSpace %Name of any color space or role specifying the
	 *          source color space. See Settings() and IModeSettings for
	 *          querying the currently available color spaces.
	 * \param   [in] sDisplay %Name of the target display for the
	 *          transformation. See Settings() and IModeSettings for getting the
	 *          list of available display names.
	 * \param   [in] sView %Name of the view transform to be applied. See
	 *          Settings() and IModeSettings for getting the list of available
	 *          view transforms.
	 * \param   [in] inverse Whether to create a pipeline with forward (false)
	 *          or inverse (true) display/view transforms.
	 * \param   [in] chanTM Pointer to an array of 16 double precision floating
	 *          point numbers forming a 4x4 matrix which can be used for channel
	 *          swizzling or any linear channel mixing/scaling operations.
	 * \return  If successful, returns a unique_ptr to the IColorPipeline
	 *          object. If operation fails it returns an empty unique_ptr. You
	 *          can use GetLastErrorText() and GetLastErrorFunction() functions
	...
	/**
	 * Returns an advanced viewing pipeline with dynamic parameters using the
	 * specified display and view transforms. See IColorPipeline class for
	 * accessing the dynamic parameters that the returned object has.
	 *
	 * \param   [in] sFromSpace %Name of any color space or role specifying the
	 *          source color space. See Settings() and IModeSettings for
	 *          querying the currently available color spaces.
	...
	/** \name Functions to convert between known/internal color spaces or physical colorometry. 
	*/
	///@{ ----------------------------------------------------------------------
	// ColorPipeline for converting colors between param space and rendering space
	virtual std::unique_ptr<IColorPipeline> GetPipeline_ParamSpaceToRenderingSpace() const = 0;
	virtual std::unique_ptr<IColorPipeline> GetPipeline_RenderingSpaceToParamSpace() const = 0;

	// ColorPipeline for converting colors between linear sRGB and rendering space
	virtual std::unique_ptr<IColorPipeline> GetPipeline_LinSRGBToRenderingSpace() const = 0;
	virtual std::unique_ptr<IColorPipeline> GetPipeline_RenderingSpaceToLinSRGB() const = 0;

	// Returns the rendering space color of an ideal blackbody radiation at
	// specified temperature. For maximal flexibility, the returned color is not
	// clamped thus if the color is out of the gamut of the rendering space then
	// it will have negative components. Caller should handle the out-of-gamut
	// colors (simple Color::ClampMin() will work for most simple cases).
	virtual Color GetBlackbodyColor(float tempKelvin) const = 0;

	// Returns the rendering space color of an CIE standard illuminant D series
	// at specified Correlated Color Temperature (CCT). Valid only for
	// 4000-25000K. For maximal flexibility, the returned color is not clamped
	// thus if the color is out of the gamut of the rendering space then it will
	// have negative components. Caller should handle the out-of-gamut colors
	// (simple Color::ClampMin() will work for most simple cases).
	virtual Color GetCIEDaylightColor(float tempKelvin) const = 0;
	///@} ----------------------------------------------------------------------
