...
enum ParamType2 
{
	...
	TYPE_MAXGUID,				//!< Identifies a MaxGuid argument type or return value.
	...
	TYPE_MAXGUID_TAB,                       //!< A table of TYPE_MAXGUID values. See TYPE_MAXGUID.
	...
	TYPE_MAXGUID_BR                 = TYPE_MAXGUID + TYPE_BY_REF,        //!< A by-reference TYPE_MAXGUID value. See TYPE_MAXGUID.
	...
	TYPE_PBLOCK2_TAB_BR             = TYPE_PBLOCK2 + TYPE_TAB + TYPE_BY_REF,        //!< A by-reference table of TYPE_PBLOCK2 values. See TYPE_PBLOCK2.
	...
	TYPE_MAXGUID_TAB_BR             = TYPE_MAXGUID + TYPE_TAB + TYPE_BY_REF,        //!< A by-reference table of TYPE_MAXGUID values. See TYPE_MAXGUID.
	...
	TYPE_MAXGUID_BV                 = TYPE_MAXGUID + TYPE_BY_VAL,         //!< A by-value TYPE_MAXGUID value. See TYPE_MAXGUID.
	...
	TYPE_MAXGUID_TAB_BV             = TYPE_MAXGUID + TYPE_TAB + TYPE_BY_VAL,        //!< A by-value table of TYPE_MAXGUID values. See TYPE_MAXGUID.
...
/** The ColorContext describes in what context a color or texture value is meant
 * to be used. Depending on the context, a color value may be interpreted and
 * displayed differently, based on the current color-management settings.  
 * 
 * To be used in ParamBlockDesc2, following a p_colorContext tag. Only valid for
 * TYPE_RGBA, TYPE_FRGBA and TYPE_TEXMAP.  
 *
 * Example:
 * \code p_colorContext, ColorContext::COLOR_SPACE_CONTEXT_SCENE_SURFACE , \endcode
 *
 * \see ParamBlockDesc2  
 * ~{ Color Context Options }~ */
enum class ColorContext : uint8_t
{
	/** The context of this color/texture is unspecified. This is the default
	 * used by plugins have not been opted in yet to support proper color
	 * context.*/
	COLOR_SPACE_CONTEXT_NONE = 0,

	/** The context of this color/texture is to be used in the scene defining
	 * the color of a surface. The rules of energy conservation will be applied,
	 * what will effectively limit the linear RGB values to be in the range of
	 * 0.0 to 1.0. */
	COLOR_SPACE_CONTEXT_SCENE_SURFACE,

	/** The context of this color/texture is to be used in the scene defining
	 * the color of a light. No energy conservation rules will be applied, any
	 * given values for RGB may be valid. */
	COLOR_SPACE_CONTEXT_SCENE_LIGHT,

	/** The context of this color/texture is to be used as pure data, for things
	 * like normals, absorption coefficients, etc. */
	COLOR_SPACE_CONTEXT_DATA,

	/** The context of this color/texture is the to be used in the UI - defined
	 * in sRGB (IEC 61966-2-1). There is no additional conversion happening
	 * beside the implicit sRGB to monitor ICM conversion of the operating
	 * system. */
	COLOR_SPACE_CONTEXT_SRGB,
	COLOR_SPACE_CONTEXT_UI = COLOR_SPACE_CONTEXT_SRGB,
...
{	
    ...
    /** For Texmap items in Mtls this defines the integer SubTexmap index for this Texmap in the owner Mtl. This is used 
        by TYPE_TEXMAPBUTTON ParamMap2 control types to give to the Material Editor for automatic button and drag And 
        drop handling. 
    ...
    /** Defines the sub-material integer index for this Mtl in an owner Mtl. This is used by TYPE_MTLBUTTON ParamMap2 control 
        types to give to the Material Editor for automatic button and Drag And Drop handling. 
    ...
    */
	...
	/** Specifies context of the color to be used. Depending on the context, the
		color may be interpreted and displayed differently, based on the current
		color-management settings.  
		
		Only valid for TYPE_RGBA, TYPE_FRGBA and TYPE_TEXMAP.
