...
/** If set, this overrides the SNAP_UNSEL_OBJS_ONLY flag
 * to allow snap to selected objects. This is used by shape
 * objects so that they can snap to other selected shapes
 * during creation when "Start New Shape" is off. */
#define SNAP_OVERRIDE_UNSEL_OBJS_ONLY			(1<<17)	

 /** If set, this allows snaps to objects while they are
 * being created (A_OBJ_LONG_CREATE flag set). Used by Spline
 * object so that users can snap to other points in the shape
 * during creation, if desired. */
