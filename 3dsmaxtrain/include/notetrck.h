...
class NoteAnimProperty : public AnimProperty {
	public:		
		TypedSingleRefMaker<NoteTrack> note;
		DWORD ID() {return PROPID_NOTETRACK;}

		NoteAnimProperty(NoteTrack *n=nullptr) {note = n;}
		NoteAnimProperty(const NoteAnimProperty& other)
		{
...
class NoteKey: public MaxHeapOperators {
	public:
		...
		/*! \remarks Constructor. The data members are initialized to default
		values. */
		NoteKey() {time=0;flags=0;}
		...
		NoteKey& operator=(const NoteKey& n) {time=n.time;note=n.note;flags=n.flags;return *this;}
		/*! \remarks Equality operator. */
		bool operator==(const NoteKey& n) const {return time==n.time && flags==n.flags && note==n.note;}
		/*! \remarks Inequality operator. */
		bool operator!=(const NoteKey& n) const {return !(*this == n);}
...
class DefNoteTrack : public NoteTrack {
	public:
		...
		Class_ID ClassID() {return Class_ID(NOTETRACK_CLASS_ID,0);}
		virtual void GetClassName(MSTR& s, bool localized = true) const override { UNUSED_PARAM(localized); s = _M("DefNoteTrack"); } // From Animatable
