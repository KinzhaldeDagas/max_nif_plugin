...
class BezierShape
		...
		, private CountedObject<BezierShape>
{
private:
...
//! \name Render / Select / Snap / HitTest
//! @{
...
//! \name Selection / Update Selection
//! @{
...
//! @}
	...
	/*! \remarks Returns the total number of vertices in the shape. */
...
//! @}
	...
	CoreExport BOOL PerformTrimOrExtend(IObjParam *ip, ViewExp *vpt, ShapeHitData *hit, IPoint2 &m, ShapeContextCallback &cb, int trimType, int trimInfinite, BOOL useLegagyDPoint3Add);
...
// Handy support functions as of 3ds Max 2024.1

//!\brief Find an endpoint weld target in a shape.
/*! This is designed to work with the 3ds Max insert/create mechanism when it's working with the last knot in the shape being edited.
It will use one of two threshold values depending on whether automatic endpoint welding is enabled. Caller has the option
to allow the spline being edited to weld to itself (this will always be the knot being edited ('knotToCompare') welding to the first
knot in the spline).\n\n
If any knots in the shape have endpoints that are appropriate for welding, the function will fill in the 'weldToPolyOut' poly
and 'weldToKnotOut' knot indices with non-negative indices of the candidate knot.\n\n
\param[in] pVpt Pointer to the ViewExp for the viewport being used. This viewport will be used to compute the distance
between endpoint knots in screen space, which is used with the auto-weld distance threshold value.
\param[in] tm Transform of the object.
\param[in] shape Pointer to the BezierShape containing the spline being edited and the weld target candidate splines.
\param[in] polyIndex The index of the spline being edited.
\param[in] knotToCompare The index of the knot that will be compared to candidate weld-target knots.
\param[in] allowSelfWeld True if the caller wants to allow the spline being edited to weld 'knotToCompare' to its own first knot.
\param[in] autoWeld true if End Point Auto-Welding is to be used.
\param[out] weldToPolyOut The index of the spline being welded to or -1 if no weld found.
\param[out] weldToKnotOut The index of the knot in the spline being welded to or -1 if no weld found.
\return true if a weld candidate was found; false otherwise.
*/
...
//!\brief Weld the last knot of a spline to the first knot in the same spline.
/*! This function performs last-to-first knot welding in a spline. It shifts the position of the last knot to align with the first knot,
including the Bezier vectors, if any, sets the first knot type as needed to preserve the spline's shape and closes the spline.\n\n
\param[in] shape Pointer to the BezierShape containing the spline being welded.
\param[in] polyIndex The index of the spline being welded.
*/
CoreExport void BezierShapeDoPolyLastToFirstWeld(BezierShape* shape, int polyIndex);

// Renderable Shape Options

/**	This function retrieves the "Use Fixed Renderable End Cap Material ID" option
 * \return true if the "Use Fixed Renderable End Cap Material ID" option is enabled */

extern CoreExport bool GetUseFixedRenderableEndCapMaterialID();

/**	This function sets the "Use Fixed Renderable End Cap Material ID" option
 * \param sw The new switch setting. When set to 'true', renderable shapes will use the Fixed Renderable End Cap
 * Material ID value. */

extern CoreExport void SetUseFixedRenderableEndCapMaterialID(bool sw);

/**	This function retrieves the "Fixed Renderable End Cap Material ID" value
 * \return The zero-based material ID to use for end caps */

extern CoreExport int GetFixedRenderableEndCapMaterialID();

/**	This function sets the "Fixed Renderable End Cap Material ID" value
 * \param value The zero-based material ID.
 * \return true if the value was successfully set; false otherwise. */

extern CoreExport bool SetFixedRenderableEndCapMaterialID(int value);
