...
public:
	...
	*/
    ...
    \param viewPanelId is the view panel index where the button is. If it is a negative value, we will use the active viewport to refresh the buttons
    \param viewportID is the viewport index in the view panel where the button is. If it is a negative value, we will use the active viewport to refresh the buttons
    */
	virtual void SetEnabled(bool v, int viewPanelId = -1, int viewportID= -1) = 0;
...
public:
	...
	//This will use the active viewport to refresh the viewport text buttons
	CoreExport void SetEnabled(bool v, int viewPanelIndex = -1, int viewportlIndex = -1) override;
	CoreExport MSTR GetLabel()const override;

	/*! The developer simply sends in an unformatted string, this class will then format it correct
	to [ XXXXX ]
	*/
