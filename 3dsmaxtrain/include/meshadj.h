...
public:
	...
	DllExport AdjFaceList(const Mesh& mesh,AdjEdgeList& el);
};
...
<b>selMask</b>) to non-selected ones.\n\n
NOTE: This function leverages interal space accelerating structure to compute the nearest distance
to the selected vertices. Under the worst condition the algorithm complexity is log(n)*n, 
in practice the algoirthm is nearly linear complexity.
\par Parameters:
<b>const Point3* \& vertArray</b>\n\n
The pointer to the first Point3 vertex to check.\n\n
<b>int \& *vertStep</b>\n\n
The vertex memory step in bytes when stepping to next vertex.
<b>const BitArray& \& selMask</b>\n\n
This is the mask indicating the selected vertices, usually it is computed from mesh.VertexTempSel().
<b>float *selDist</b>\n\n
