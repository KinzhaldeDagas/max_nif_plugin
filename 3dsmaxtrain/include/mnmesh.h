...
/** \anchor GeneralMNMeshComponentFlags
...
#define MN_TARG (1<<2)              //!< Indicates that a component is targeted. (the MNMesh methods starting with the words TargetBy.
...
//! \defgroup QuadChamferOptions Quad Chamfer Result Options
//!@{
	enum MN_QCHAM_TYPE 
	{
...
//! \defgroup QuadChamferOptionFlags Quad Chamfer Option Flags
//!@{
...
#define QCHAM_OPTION_ABSOLUTE_EDGE_WEIGHTS (1<<7)   //!< Provides per-edge chamfer weight define by absolute weighting values and scale factor; weight is constant for a particular edge, implying differing weights for edges meeting at a cluster
#define QCHAM_OPTION_USE_INSET (1<<4)				//!< Provides insets from chamfered edges
#define QCHAM_OPTION_SUPPORT_LOOP_INSET (1<<10)     //!< When set, any insets are formed following the Flow Loop definition, otherwise, the Face Inset definition is employed; this parameter has no effect when insets are not requested
...
#define QCHAM_OPTION_BIAS_END_POINTS (1<<8)         //!< Adjusts tension-0.5 endpoint and hub point positions when radius bias is adjusted
#define QCHAM_OPTION_VARIABLE_DEPTH (1<<9)          //!< Indicates variable edge chamfer depth specification, fetched from the mesh's Depth channel
#define QCHAM_OPTION_PATCH_CORNERS (1<<11)          //!< Indicates use of "patch" type corners (otherwise identical to "Uniform")
#define QCHAM_OPTION_RADIAL_CORNERS (1<<12)         //!< Indicates use of "radial" type corners (otherwise identical to "Uniform")
//!@}

//! \defgroup QuadChamferVersionValues Quad Chamfer Version Values
//!@{
#define QCHAM_VERSION_ELWOOD (0)                //!< Elwood (3ds Max 2015)
#define QCHAM_VERSION_PHOENIX (100)             //!< Phoenix (3ds Max 2016) - Includes edge selection fix
#define QCHAM_VERSION_IMOOGI (200)              //!< Imoogi (3ds Max 2018) - Includes Quad Intersections upgrade
#define QCHAM_VERSION_ATHENA_ALPHA1 (290)       //!< Athena (3ds Max 2020) - New options starting with QCHAM_OPTION_FIXED_EDGE_WEIGHTS
#define QCHAM_VERSION_ATHENA_UPDATE1 (295)      //!< Athena (3ds Max 2020) Update 1 - New functionality, and modifications to some geometric calculations, such as chamfer inset point positions, for improved appearance
#define QCHAM_VERSION_ATHENA_UPDATE2 (300)      //!< Athena (3ds Max 2020) Update 2 - New functionality, and fix for texture vertices on "shared" edge types
#define QCHAM_VERSION_ATHENA_UPDATE3 (305)      //!< Athena (3ds Max 2020) Update 3 - Improvements for the chamfered surface geometry at concave face corners
#define QCHAM_VERSION_THESEUS (400)             //!< Theseus (3ds Max 2021) - Extending new chamfer functiontality to EditPoly modifier
#define QCHAM_VERSION_THESEUS_UPDATE1 (405)     //!< Theseus (3ds Max 2021) Update 1 - Improved treatment for chamfers on nearly-flat edges
#define QCHAM_VERSION_THESEUS_UPDATE3 (410)     //!< Theseus (3ds Max 2021) Update 3 - Lower minimum chamfer amount to FLT_EPSILON from 0.001
#define QCHAM_VERSION_HEIMDALL_UPDATE1 (500)    //!< Heimdall (3ds Max 2022) Update 1 - Impose a minimum edge weight for By Weight chamfers to rectify a regression in the zero weight case
#define QCHAM_VERSION_HEIMDALL_UPDATE3 (505)    //!< Heimdall (3ds Max 2022) Update 3 - Adjustments to the Limit Effect treatment to handle cases that were previously distorted unnecessarily
#define QCHAM_VERSION_VESTA_UPDATE1 (600)       //!< Vesta (3ds Max 2023) Update 1 - Enhanced retriangulation approach, based upon the algorithm implemented in MaxSDK::BuildDiagonals with version MaxSDK::TRIANGULATION_VER_2023_1
#define QCHAM_VERSION_VESTA_UPDATE2 (605)       //!< Vesta (3ds Max 2023) Update 2 - Enhanced Limit Effect implementation, resolving geometric edge cases which previously were under-limited
#define QCHAM_VERSION_OLYMPUS (700)             //!< Olympus (3ds Max 2024)  - Employs the further-improved retriangulation approach implemented in MaxSDK::BuildDiagonals with version MaxSDK::TRIANGULATION_VER_2024
//!@}

//! \defgroup Face triangulation algorithms
//!@{
enum MNMeshTriangulationType
{
	TRIANGULATION_LEGACY          = 0,    //!< Legacy triangulation algorithm, implemented in the function MaxSDK::FindDiagonals, and offered by the MNMesh::RetriangulateFace
 	                                      //!< function prior to Max 2024. O(deg * deg) time complexity.
	TRIANGULATION_ENHANCED_2023_1 = 1,    //!< Improved triangulation algorithm, implemented in the function MaxSDK::BuildDiagonals, with version parameter
	                                      //!< MaxSDK::TRIANGULATION_VER_2023_1; generates superior results for multiply-connected faces, faces containing collinear vertices,
	                                      //!< and grossly non-planar faces. O(deg * deg) time complexity on average.
	TRIANGULATION_ENHANCED_2024   = 2     //!< Improved triangulation algorithm, implemented in the function MaxSDK::BuildDiagonals, with version parameter
	                                      //!< MaxSDK::TRIANGULATION_VER_204; improves upon the treatment of the TRIANGULATION_ENHANCED_2023_1 algorithm with better handling
	                                      //!< of non-planar faces, and geometric degeneracies such as spur edges and self-intersecting bounding curves. O(deg * deg) time
	                                      //!< complexity on average.
};
//!@}

//! \defgroup MNFaceFlags MNFace Flags
//!@{
...
//! \defgroup per_edge_data Per-edge Data
//!@{
...
/** If set, all topological links, such as the list of edges, are complete.
		This flag is part of the \ref MNMesh \ref TOPO_CHANNEL, stored in \ref MaxSDK::MNTopoChannelData.*/
...
//! \defgroup SubObjectHitTestFlags Flags for Sub Object Hit-Test
//!@{
...
//! \defgroup SubdivisionFlags Subdivision Flags
//!@{
...
// A wrapper around the int MNVert::orig member added as an optimization. Since the default value of orig is -1,
// we simply add 1 to it in memory. This allows us to allocate an array of MNVerts more quickly because its as all 0's.
// Accessing the value as an int return the correct value by subtracting 1.
class MNVertOrigWrapper : public MaxHeapOperators
{
	friend MNVert;
protected:
	int orig = 0; //!< Original point this vertex comes from. Stored as zero based, return value - 1 upon access.
public:
	MNVertOrigWrapper() = default;
	MNVertOrigWrapper(MNVertOrigWrapper&) = default;
	MNVertOrigWrapper(MNVertOrigWrapper&&) = default;

	MNVertOrigWrapper& operator=(const MNVertOrigWrapper&) = default;
	MNVertOrigWrapper& operator=(MNVertOrigWrapper&&) = default;
	MNVertOrigWrapper& operator=(int& i) { orig = i + 1; return *this; }
	MNVertOrigWrapper& operator=(int&& i) { orig = i + 1; return *this; }

	MNVertOrigWrapper& operator+=(MNVertOrigWrapper&& from) { orig += from.orig; return *this; }
	MNVertOrigWrapper& operator-=(MNVertOrigWrapper&& from) { orig -= from.orig; return *this; }
	MNVertOrigWrapper& operator+=(int&& i) { orig += i; return *this; }
	MNVertOrigWrapper& operator-=(int&& i) { orig -= i; return *this; }

	MNVertOrigWrapper& operator++() { orig++; return *this; };
	MNVertOrigWrapper& operator--() { orig--; return *this; };

	bool operator==(int i) const { return (orig -1) == i; }
	bool operator==(const MNVertOrigWrapper& other) const { return orig == other.orig; }
	operator int() const { return orig - 1; } //!< Auto converts to value type in most cases. May need to type cast.
};
...
public:
	...
	MNVertOrigWrapper orig; //!< Original point this vertex comes from

	/** Initializes the MNVert. */
	MNVert() = default;

	/** Comparison operator. */
	bool operator==(const MNVert & from) const { return (from.p==p)&&(from.ExportFlags()==ExportFlags()); }

	//!< Getter for the point this vertex comes from. Used for tracability of a vertes after some MNMesh manipulation operators.
	int GetOrig() const { return orig; }
	//!< Setter for the point this vertex comes from. Used for tracability of a vertes after some MNMesh manipulation operators.
	void SetOrig(int o) { orig = o; }
};
...
public:
	...
	/** Assignment operator. */
	DllExport bool operator== (const MNMapFace & from) const;
...
public:
	...
	/** This method fills in the table with the full triangulation for the face,
	based on the internal diagonal list. The table is set to size (deg-2)*3.
	The const version doesn't reorder diags.
	\par Parameters:
	<b>Tab\<int\> \&tri</b>\n\n
	The table of triangles. */	
	...
	/** Comparison operator */
	DllExport bool operator== (const MNFace &from) const;
...
MNMesh supports the FlagUser interface and supports the flags listed at \ref MN_MESH_Flags.

\see FlagUser, MNFace, MNVert, MNEdge, MNMap, PerData, MNMeshBorder, Mesh, BitArray, Tab, MNMeshBorder, 
		
\par List of deprecations from 3dsMax 2024:
<table><tr><th>Deprecated<th>Please use instead
<tr><td><b>MNMap* m</b>, Maps moved to \ref MaxSDK::MNVertexColorChannelData, \ref MaxSDK::MNTextureMapChannelData.<td>\ref MNMesh::M(int mapChannel) const
<tr><td><b>int numm</b><td>\ref MNMesh::MNum() const
<tr><td><b>PerData* ed</b>, moved to \ref MaxSDK::MNTopoChannelData.<td>\ref MNMesh::GetED(int vdChannel) const
<tr><td><b>BitArray edSupport</b>, moved to \ref MaxSDK::MNTopoChannelData.<td>\ref MNMesh::eDataSupport(int edChannel) const, \ref  MNMesh::setEDataSupport(int edChannel, BOOL support), \ref MNMesh::EDNum() const, \ref MNMesh::EDNumSupported() const
<tr><td><b>PerData* vd</b>, moved to \ref MaxSDK::MNGeomChannelData.<td>\ref MNMesh::GetVD(int vdChannel) const
<tr><td><b>BitArray vdSupport</b>, moved to \ref MaxSDK::MNGeomChannelData.<td>\ref MNMesh::vDataSupport(int vdChannel) const,\ref MNMesh::setVDataSupport(int vdChannel, BOOL support),\ref MNMesh::VDNum() const,\ref MNMesh::VDNumSupported() const
<tr><td><b>void MAlloc (int, bool)</b><td> The concept of allocated maps versus used maps has been removed. Use \ref SetMapNum() to change the number of maps.
<tr><td><b>void MShrink (int)</b><td>The concept of allocated maps versus used maps has been removed. Use \ref void MNMesh::SetMapNum(int mpnum) to change the number of maps.
</table>
*/
class MNMesh : public BaseInterfaceServer {
	...
	friend class MNMeshUtilities18Impl;
	...
	friend class MaxSDK::MNTopoChannelData; //this class owns private members: nf_alloc, ne_alloc
	friend class MaxSDK::MNGeomChannelData; //this class owns private members: nv_alloc

public:
	//! \brief Using a channel mask, request the locks for those channels. 
	/*! This can be used avoid race conditions when modifying many Min parallel when some of them share channels.
		\param[in] channels - Which channels to aquire a lock for.
		\see MNMesh::ReleaseChannelLocks(ChannelMask channels) const
		\see MNMesh::ShallowCopy(MNMesh *amesh, ChannelMask channels)
	*/
	void AcquireChannelLocks(ChannelMask channels) const { return m_ChannelDataContainer.AcquireChannelLocks(channels); }
	//! \brief Using a channel mask, release the aquired locks for those channels. 
	/*! \param[in] channels - Which channels to release the locks for.
		\see MNMesh::AcquireChannelLocks(ChannelMask channels) const
		\see MNMesh::ShallowCopy(MNMesh *amesh, ChannelMask channels)
	*/
	void ReleaseChannelLocks(ChannelMask channels) const { return m_ChannelDataContainer.ReleaseChannelLocks(channels); }

	/*! A helper class for aquiring and releasing MNMesh channel locks.
	Create an instance of this class to lock the channels, when it is destructed the locks are released.
		\see MNMesh::AcquireChannelLocks(ChannelMask channels) const
		\see MNMesh::ReleaseChannelLocks(ChannelMask channels) const
	*/
	class [[nodiscard]] ScopedLockChannels : public MaxHeapOperators
	{
	public:
		ScopedLockChannels(MNMesh* parent, ChannelMask channels) : mChannels(channels), mParent(parent), mResetCalled(false) {
			mParent->AcquireChannelLocks(mChannels);
		}
		~ScopedLockChannels() {
			Reset();
		}
		//!< Manually release the channel locks.
		void Reset()
		{
			if (!mResetCalled)
				mParent->ReleaseChannelLocks(mChannels);
			mResetCalled = true;
		}
	private:
		ChannelMask mChannels;
		MNMesh* mParent;
		bool mResetCalled;
	};

	//! \brief Returns the MNMesh channel container which provides access to the channel objects themselves.
	/*! To modify a MNMesh channel directly, instead of using the MNMesh interface, you can fetch the MNChannelContainer,
	then grab the appropriate channel object and make the changes directly.
	This is not recommended, mistakes here could cause crashes and data loss.
	Example to manually reallocate the faces:
	\code{.cpp}
		MNChannelContainer& container = mnmesh.GetChannelDataContainer();
		MNTopoChannelData* tc = container.GetOrCreateMNTopoChannelData();
		tc->numf = mewFaceCount;
		tc->f = mewFaces;
		tc->SyncParentInterface(&mnmesh); //Don't forget to sync. This updates the meshes public "BlockWrite" members with the new data.
	\endcode
	\see MaxSDK::MNChannelData
	\see MaxSDK::BlockWrite_Ptr
	\see MaxSDK::BlockWrite_Value */
	MaxSDK::MNChannelContainer& GetChannelDataContainer() { return m_ChannelDataContainer; }
	//! \brief Returns a const mnesh channel container which provides access to the channel objects themselves.
	/*! \see MNMesh::GetChannelDataContainer() */
	const MaxSDK::MNChannelContainer& GetChannelDataContainer() const { return m_ChannelDataContainer; }
	
private:
	/*! Container that owns the channel objects of this mnmesh
	\see MaxSDK::MNChannelData */
	MaxSDK::MNChannelContainer m_ChannelDataContainer;
...
#pragma region Private Member Variables
	...
	int GetNVAlloc() const; //!< Returns the number of allocated verts, greater than or equal to numv.
	int GetNEAlloc() const; //!< Returns the number of allocated edges, greater than or equal to nume.
	int GetNFAlloc() const; //!< Returns the number of allocated faces, greater than or equal to numf.
...
//!@}
	...
	// Stores the current mesh version, for ex MNMESH_VERSION_THESEUS. See IMNMeshVersion.h
	DWORD mMeshVersion;
...
#pragma region Private Methods
	...
	/** If the transformed bounding box presently stored by this mesh is invalid, or corresponds to a transformation
	    differing from that defined the specified matrix transformMat, then the stored transformed bounding box is
	    updated to reflect the transformation of the mesh into the space defined by transformMat. The computed bounding
	    box will then be accessible via a call to getBoundingBox with *tm = transformMat.

		\param[in] transformMat: transformation matrix, including translation vector, defining the transformation from
	                             this mesh's object space, to the space in which the bounding box is to be computed */
	...
	bool RemoveInternalVertexNoTriangulation(int vertex, int& face_to_finddiagonals);
	...
	bool ConnectEdges (DWORD edgeFlag, int segments/*=2*/, int pinch /*= 0*/, int slide /*= 0*/, MNMeshTriangulationType triangulationType /*= TRIANGULATION_ENHANCED_2024*/);
#else
	bool ConnectEdges (DWORD edgeFlag, int segments = 2, int pinch = 0, int slide = 0, MNMeshTriangulationType triangulationType = TRIANGULATION_ENHANCED_2024);
#endif
	...
	// New chamfer methods with extended result type flag (open, invert, or none)
	bool ChamferVertices(DWORD in_flag, MNChamferData* in_mcd, MN_QCHAM_TYPE in_resultType);
	bool ChamferEdges(DWORD in_flag, MNChamferData10& in_mcd, MN_QCHAM_TYPE in_resultType, int in_segments);

	// Original version of EliminateDoubledMappingVerts.  Because it has a performance issue when all verts mapped to the same UV index,
	// it has been replaced by a more efficient version, but kept for backward-compatibility.
	void EliminateDoubledMappingVertsPreVesta();
...
//!@{
	//! \brief The array of edges.
	/*! Can also use \ref E().
	\ref MaxSDK::MNTopoChannelData channel object owns this members data.
	May need to cast in some cases:
	\code{.cpp}
		(MNEdge*)e;
	\endcode
	\see MaxSDK::BlockWrite_Ptr
	\see MaxSDK::MNTopoChannelData */
	MaxSDK::BlockWrite_Ptr< MNEdge, MaxSDK::MNTopoChannelData > e;
	...
	\see MaxSDK::BlockWrite_Ptr
	\see MaxSDK::MNTopoChannelData */
	MaxSDK::BlockWrite_Ptr< MNFace, MaxSDK::MNTopoChannelData > f;
	
	//! \brief Number of MNEdge elements in e
	/*! Can also use \ref ENum().
	\ref MaxSDK::MNTopoChannelData channel object owns this members data.
	May need to cast in some cases:
	\code{.cpp}
		(int)nume;
	...
	\see MaxSDK::BlockWrite_Value
	\see MaxSDK::MNTopoChannelData */
	MaxSDK::BlockWrite_Value< int, MaxSDK::MNTopoChannelData > nume;
	
	//! \brief Number of MNFace elements in f
	/*! Can also use \ref FNum().
	\ref MaxSDK::MNTopoChannelData channel object owns this members data.
	May need to cast in some cases:
	\code{.cpp}
		(int)numf;
	...
	\see MaxSDK::BlockWrite_Value
	\see MaxSDK::MNTopoChannelData */
	MaxSDK::BlockWrite_Value< int, MaxSDK::MNTopoChannelData > numf;

public:

	//! \brief Array of int Tabs that records which edges use each vertex.
	/*! For instance, if edges 3, 5, and 6 have an endpoint at vertex 0, vedg[0] would
	include 3, 5, and 6, though not in any particular order.
	Note that this information is only valid if the MN_MESH_FILLED_IN flag is set.
	Can also use \ref GetVEdge().
	\ref MaxSDK::MNTopoChannelData channel object owns this members data.
	May need to cast in some cases:
	\code{.cpp}
		(Tab<int>*)vedg;
	...
	\see MaxSDK::BlockWrite_Ptr
	\see MaxSDK::MNTopoChannelData */
	MaxSDK::BlockWrite_Ptr< Tab<int>, MaxSDK::MNTopoChannelData > vedg;
	Tab<int>* GetVEdge() const { return vedg; }

	//! \brief Array of int Tabs that records which faces touch each vertex.
	/*! For	instance, if faces 0, 1, 3, 8, and 9 use vertex 0 as one of their corners,
	vfac[0] would include 0, 1, 3, 8, and 9, though not in any particular order.
	Note that this information is  only valid if the MN_MESH_FILLED_IN flag is set.
	Can also use \ref GetVFac().
	\ref MaxSDK::MNTopoChannelData channel object owns this members data.
	May need to cast in some cases:
	\code{.cpp}
		(Tab<int>*)vfac;
	...
	\see MaxSDK::BlockWrite_Ptr
	\see MaxSDK::MNTopoChannelData */
	MaxSDK::BlockWrite_Ptr< Tab<int>, MaxSDK::MNTopoChannelData > vfac;
	Tab<int>* GetVFac() const { return vfac; }

	//! \brief The array of vertices.
	/*! \ref MaxSDK::MNGeomChannelData channel object owns this members data.
	May need to cast in some cases:
	\code{.cpp}
		(MNVert*)v;
	\endcode
	\see MaxSDK::BlockWrite_Ptr
	\see MaxSDK::MNGeomChannelData */
	MaxSDK::BlockWrite_Ptr< MNVert, MaxSDK::MNGeomChannelData > v;

	//! \brief Number of MNVert verts in \ref MNMesh::v.
	/*! \ref MaxSDK::MNGeomChannelData channel object owns this members data.
	May need to cast in some cases:
	\code{.cpp}
		(int)numv;
	...
	\see MaxSDK::BlockWrite_Value
	\see MaxSDK::MNGeomChannelData */
	MaxSDK::BlockWrite_Value< int, MaxSDK::MNGeomChannelData > numv;
...
//!@{
	...
	void *vertexData (int vdChannel) const { return vDataSupport(vdChannel) ? GetVD(vdChannel)->data : nullptr; }
...
//!@{
	...
	The edge data channel. See
	\ref per_edge_data "Edge data channel index values". */
	void *edgeData (int edChannel) const { return eDataSupport(edChannel) ? GetED(edChannel)->data : NULL; }
...
//!@{
	...
	MNMeshTriangulationType triangulationType\n\n
	Triangulation algorithm to employ, as discussed in the description of the
	MNMeshTriangulationType enumeration.
...
//!@{
	...
	*/
	...
	/*
	This removes many vertices and then cleans up all the topology.
	Much much much faster than using RemoveVertex one at a time.
	\par Paramters:
	const int* vertices\n\n
	The vertices to remove.\n\n
	int count\n\n
	The size of the vertices index buffer.\n\n
	*/
...
//!@{
	...
	/** Outputs current MNMesh into the mesh given. This algorithm has been multithreaded.
	Note that even if the MNMesh was originally taken from this Mesh, the internal processing may
	have changed PART_TOPO, PART_GEOM, PART_SELECT, PART_MAPPING, or PART_VERTCOLOR.*/
...
//!@{
	/** Computes the triangulation for each face f[faceList[face]], employing
	the specified retriangulation algorithm. Note that the specified faces are
	not required to possess valid triangulations on entry.

	\param[in] faceList -- indices of faces, in the member f array, for which
	                       triangulations are to be computed
	\param[in] triangulationType -- triangulation algorithm to employ, as
	...
	/** Computes the triangulation for face f[ff], employing the specified
	retriangulation algorithm. Note that the specified face is not required to
	possess a valid triangulation on entry.

	\param[in] ff -- index, in the member f array, of the face for which
	                 a triangulation is to be computed
	\param[in] triangulationType -- triangulation algorithm to employ, as
	                                discussed in the description of the
	                                MNMeshTriangulationType enumeration */
	...
	some way. Employs the legacy algorithm TRIANGULATE_LEGACY, as detailed in
	the description of the MNMeshTriangulationType enumeration.
	\par Parameters:
	int ff\n\n
	...
	*/
	...
	triangulation for the polygon they form. Employs the legacy algorithm
	TRIANGULATE_LEGACY, as detailed in the description of the
	MNMeshTriangulationType enumeration.
	\par Parameters:
	int deg\n\n
	...
	/** Imposes a diagonal on face f[ff] with endpoints at vertices(f[ff]).vtx[d1] and (f[ff]).vtx[d2],
	adjusting any pre - existing diagonals which would have topologically intersected the new diagonal,
	if face ff is not MN_DEAD, and the diagonal defined by vertices d1 and d2 is valid.
	The diagonal is judged to be valid if it does not correspond to an edge of the face, and if d2 != d1.
	If face ff is dead, or the proposed diagonal is invalid, the face is unmodified.
	\param[in] ff: face on which the diagonal defined by vertices d1 and f2 is to be imposed
	\param[in] d1, d2: indices, in the(f[ff]).vtx array, of the endpoint vertices defining the diagonal to be imposed 
	\return true if the diagonal is actually imposed on face ff, otherwise, false  */
...
//!@{
	...
	/** Unifies triangles into polygons across invisible edges.
	Turns a mesh with triangle faces into a mesh with (fewer) polygon
	faces by removing all hidden edges. Calculation is only performed on the
	specified face range.
	\par Parameters:
	int startFace\n\n
	The starting face to work on\n
	int endFace\n\n
	The one greater than the last face to be processed.  Generally number of faces in the mesh. */
...
//!@{
	...
	int edgeIndex\n\n
	The edge index.
	\par const Point3* faceNormals Optional performance parameter to pass in a precomputed face normals list
...
//!@{
	...
	/** Finds border loops composed of all one-sided edges for which all of the vertices
	are also selected. (One-sided edges on a mesh must necessarily be organizable into
	closed loops along the borders of the mesh.)
	\par Parameters:
	MNMeshBorder \& brd\n\n
	The class in which to put the border loops. See
	Class MNMeshBorder for details.\n\n	*/
	...
	\par Parameters:
	MNMeshBorder* b\n\n
	If b = nullptr, FillInBorders finds all the borders with a call to
...
//!@{
	...
	MN_MESH_FILLED_IN, MN_MESH_NO_BAD_VERTS, MN_MESH_VERTS_ORDERED and MN_MESH_USE_MAX2012_WELD_MATH are
...
//!@{
	...
	*/
	...
	is set to size (NUM_HIDDENMAPS + MNum())*2. The two entries for each map
	...
	BestConvexTriangulation; otherwise, RetriangulateFace is employed, in
	conjunction with the TRIANGULATION_LEGACY algorithm.
	\return  The index of the new vertex. */
	...
	*/
	...
	\param triangulationType - Triangulation algorithm to employ, as discussed in the
	description of the MNMeshTriangulationType enumeration.
	*/
...
//!@{
	...
	/** This method welds the closest two vertices within "thresh" of each other which have
	non-matching flags (flag1, flag2). Vertices are only welded pairwise.
	\par Parameters:
	float thresh\n\n
	The welding threshold. Vertices further apart than this distance (in object
	space) will not be welded.\n\n
	DWORD flag1\n\n
	Indicates vertex flag value which represents side 1 of border. (Non-border vertices, those in
	the "interior" of the surface, are ignored even if flagged.)
	DWORD flag2\n\n
	Indicates vertex flag value which represents side 2 of border. (Non-border vertices, those in
	the "interior" of the surface, are ignored even if flagged.)
	\return  True if anything was welded, false otherwise. */
...
//!@{
	 ...
	 * \param       triangulationType Triangulation algorithm to employ, as discussed in the description of the MNMeshTriangulationType enumeration.
	 * \return      Returns true if the operation was successful or false otherwise. 
	 */
...
//!@{
	...
	details.)
	\param fclust A list of face clusters. See the constructors in class MNFaceClusters and
	...
	the face cluster related methods of class MNTempData for information on how
	to set up these clusters based on edge angles and face flags.
	\param cl The cluster we wish to extrude.
	\return  True if any faces were extruded, false if nothing happened. */
	...
	Since MNFaceClusters::GetBorder is expensive, use this overload if
	you've previously computed borders.
	*/
	DllExport bool ExtrudeFaceCluster(
			const MNFaceClusters& fclust, int cl, const Tab<int>& clusterBorders);
	...
	\param mcd The data structure in which the extrusion directions are stored. (Note that
	...
	\param fclust The face clusters.
	\param clustNormals = The cluster normals.
	...
	/*! A version of GetExtrudeDirection where a faceFlag can be
	passed in when there are no relevant face clusters.
	...
	there is no map support for this operation, as there is no well-defined way
	to modify mapping values during an extrusion drag.)
	\param faceFlag which flag to use that defines the extrusion.
	*/
	...
	*/
	...
	/** Performs a Quad Chamfer operation on the flagged edges with options and versioning.
		...
		\param resultType - Indicates what result is desired. QCHAM_CHAMFEREDOBJECT=chamfered object, QCHAM_NOCHAMFERS=object with chamfered edges removed,
			QCHAM_CHAMFERSONLY=object with chamfered edges only
		\param flag - Indicates which vertices to chamfer.
		\param version - Indicates the version to use (see QCHAM_VERSION_XXX above).  Optional; defaults to the latest version
...
//!@{
	...
	\param amesh - Specifies the source MNMesh to copy. 
	\param channels - Specifies the channels to copy. See \ref ObjectChannels.
	...
	/** Drop the channel objects specified by the channels argument. No risk of leaking channel objects.
		For	example if the TOPO_CHANNEL is specified, MeshTopoChannelData is freed,
		if the GEOM_CHANNEL is specified then MeshGeomChannelData is freed, etc.
		Number of maps are always preserved.
		As of 3ds Max 2024, if any MeshMap channels are specified, they flags are also reset.
	\param channels - Specifies the channels to free. Channels not specified are left intact. See \ref ObjectChannels. 
	\param zeroOthers - If nonzero then the topo, geom, vert color, tex map and hidden maps are cleared out.
	\see MNMesh::ShallowCopy(MNMesh *amesh, ChannelMask channels)
	\see MaxSDK::MNChannelContainer
	*/
...
private:
	// Number of dead edges, cached because it is used in HUD display stats and other places. Set to -1 when
	// InvalidateGeomCache() is called to force recalculation on demand.
	int mNumDeadEdges = -1;

	// Number of dead verts, cached because it is used in HUD display stats and other places. Set to -1 when
	// InvalidateGeomCache() is called to force recalculation on demand.
	int mNumDeadVerts = -1;

	// Number of dead faces, cached because it is used in HUD display stats and other places. Set to -1 when
	// InvalidateGeomCache() is called to force recalculation on demand.
	int mNumDeadFaces = -1;
...
//!@{
	...
	*/
	...
	\param generateRenderItemsContext contains all the information that is used to generate all
			the required render items.
	\deprecated Deprecated as of 2026. Please use GenerateRenderItems instead as it is equivalent.
	*/
	MAX_DEPRECATED DllExport void GenerateDeferredRenderItems(
		MaxSDK::Graphics::IRenderItemContainer& renderItems, 
		const MaxSDK::Graphics::GenerateMeshRenderItemsContext& generateRenderItemsContext);
...
private:
	...
	MN_SpaceAccelData* mpSpaceAccelData = nullptr;
...
#pragma region FlagUser Implementation
//! \name FlagUser Implementation
/** MNMesh owns a mnFlags variable rather than inherit from FlagUser. Some flags are owned by channel objects.
	\ref MN_MESH_FILLED_IN flag is stored in \ref MaxSDK::MNTopoChannelData::fillInMeshFlag.
	\ref MN_MESH_VERTS_ORDERED flag is stored in \ref MaxSDK::MNTopoChannelData::vertsOrderedFlag.
	\ref MN_MESH_NO_BAD_VERTS flag is stored in \ref MaxSDK::MNTopoChannelData::noBadVertsFlag.
	\ref MN_MESH_USE_MAX2012_WELD_MATH flag is stored in \ref MaxSDK::MNTopoChannelData::use2012WeldMathFlag.
	To manipulate the MNMesh flags, the \ref FlagUser interface has beem copied to include all flag sources.*/
//!@{
private:
	FlagUser mnFlags;
public:

	/*! \remarks Sets flags. Each bit that is set in <b>fl</b> is assigned the value <b>val</b>.*/
	...
	/*! \remarks	Copies all flag bits over from <b>fl</b>. Same as ImportFlags. */
...
private:
	/*! \remarks Sets all flags that are set in fu. Not inherrited form FlagUser */
	void OrFlags(DWORD fl);

	/*! \remarks Clears all flags that are clear in <b>fu</b>. Not inherrited form FlagUser */
	void AndFlags(DWORD fl);
...
*/
	...
	{
		return (i < clust.Count()) ? clust[i] : -1;
	}
...
public:
	static constexpr Interface_ID ID = IMNTEMPDATA10_INTERFACE_ID;
...
class IMNMeshUtilities8 : public BaseInterface
{
	public:
		...
		\param[in] triangulationType algorithm to be employed when triangulating generated faces */
		virtual bool BridgeTwoEdges(	
			...
			MNMeshTriangulationType triangulationType = TRIANGULATION_ENHANCED_2024) = 0 ;	
		...
		\param[in] in_slide position of the new connection edges, relative to the existing edges. Can be < 0.
 		\param[in] triangulationType triangulation algorithm to employ */
...
public:
		static constexpr Interface_ID ID = IMNMESHUTILITIES10_INTERFACE_ID;
...
public:
	static constexpr Interface_ID ID = IMNMESHUTILITIES13_INTERFACE_ID;
...
public:
	static constexpr Interface_ID ID = IMNMESHUTILITIES14_INTERFACE_ID;
...
public:
	static constexpr Interface_ID ID = IMNMESHUTILITIES15_INTERFACE_ID;
...
public:
	static constexpr Interface_ID ID = IMNMESHUTILITIES16_INTERFACE_ID;
	...
	//! \param[in] params -- The Quad Chamfer parameter data (See \ref MNQuadChamferParameters)
	...
	//!  to modify mapping values during an extrusion drag.)\n\n
	//! \param[in] faceFlag which flag to uses that defines the extrusion. 
	//! \param[in] bias is the blend along the normal and distance from the surface 
	//! \param[in] useLegacyDPoint3Add  if true use pre-2026 DPoint3::operator +() behavior which returned a Point3 instead of a DPoint3
	virtual void GetExtrudeDirection(MNChamferData *mcd, DWORD faceFlag, float bias, bool useLegacyDPoint3Add) = 0;
};

#define IMNMESHUTILITIES17_INTERFACE_ID Interface_ID(0x355a5448, 0x1214322c)

/** Derived parameters class with additional quad chamfer parameters not found in previous versions */
class MNQuadChamferExtendedParameters;

/** Extends the MNMeshUtilities class for 2020 Update 1.

Usage:

\code
IMNMeshUtilities17* l_mesh17 = static_cast<IMNMeshUtilities17*>(mesh.GetInterface(IMNMESHUTILITIES17_INTERFACE_ID));
\endcode */
class IMNMeshUtilities17: public BaseInterface
{
public:
	static constexpr Interface_ID ID = IMNMESHUTILITIES17_INTERFACE_ID;
    virtual Interface_ID GetID() override { return IMNMESHUTILITIES17_INTERFACE_ID; }

    //! Performs chamfers on a mesh, with varying versions
    //! \param[in] params -- The Quad Chamfer parameter data (See \ref MNQuadChamferExtendedParameters)
    //! \return true if mesh altered; false otherwise
    virtual bool QuadChamfer(const MNQuadChamferExtendedParameters& params) = 0;
};

#define IMNMESHUTILITIES18_INTERFACE_ID Interface_ID(0x6c6e34f7, 0x798f5a9d)

/** Parameters class storing data relevant to the vertex chamfer operation */
class MNQuadVertexChamferParameters;
...
\endcode */
class IMNMeshUtilities18: public BaseInterface
{
public:
	static constexpr Interface_ID ID = IMNMESHUTILITIES18_INTERFACE_ID;
    virtual Interface_ID GetID() override { return IMNMESHUTILITIES18_INTERFACE_ID; }

    //! Performs vertex chamfers on a mesh, using the new segmented quad chamfer approach.
    //! \param[in] params -- The quad vertex chamfer parameter data (See \ref MNQuadVertexChamferParameters)
    //! \return true if mesh altered; false otherwise
    ...
    //! \param[in] in_flag the edge flag used. See MNEdge for a description of flag values.
    //! \param[in] in_mcd the chamfer data. 
    //! \param[in] in_resultType Either everything, chamfered edges only (inverted), or without chamfered edges (opened). 
    //! \param[in] in_segments The number of new edges to create per selected edge. If in_segments < 1, then behave as though in_segments = 1.
    ...
    //! \return false if the chamfer operation failed
    //! \post the mesh geometry is valid, even if the operation failed
    //! \param[in] in_flag the vertive flag used. See MNVert for a description of flag values.
    //! \param[in] in_mcd the chamfer data. 
    //! \param[in] in_resultType Either everything, chamfered vertices only (inverted), or without chamfered vertices (opened). 
...
/** Interface class for handling interactive face extrude operations, such as the shift-drag face extrude implemented in
    Editable Poly and Edit Poly.
...
\endcode */
class IMNFaceExtrude: public BaseInterface
{
public:
	static constexpr Interface_ID ID = IMNFACEEXTRUDE_INTERFACE_ID;
    virtual Interface_ID GetID() override { return IMNFACEEXTRUDE_INTERFACE_ID; }

    //! Enumeration defining versions of the face extrude operation.
    enum IMNFaceExtrudeVersionType
    {
        IMN_FACE_EXTRUDE_VER_2021 = 0,    //! Initial implementation, exposed in Editable Poly in 3ds Max 2021.2 and in Edit Poly in 3ds Max 2022
        IMN_FACE_EXTRUDE_VER_2022 = 1,    //! Includes support for partial cut-through, exposed in Editable Poly in 3ds Max 2022.2 and in Edit Poly in 3ds Max 2023
        IMN_FACE_EXTRUDE_VER_2023 = 2,    //! Improved handling of geometric and topological edge cases, exposed in Editable Poly in 3ds Max 2023.1 and in Edit Poly in 3ds Max 2024
        IMN_FACE_EXTRUDE_VER_2024 = 3     //! Implements smoothing of extrusion connecting faces, exposed in Editable Poly in 3ds Max 2024.1 and in Edit Poly in 3ds Max 2025
    };

    //! Sets the version of the face extrude postprocessing that the interface should employ. Note that the interface is set at
    //! construction for the most recent version. Note that if an extrude operation is presently being processed, and version
    //! is different from the value returned by GetVersion() on entry, the the postprocessing operation is terminated, and this
    //! object is reset to an uninitialized state.
    //! \param[in] version -- the IMNFaceExtrudeVersionType value indicating the version of the postprocessing to employ
    virtual void SetVersion(IMNFaceExtrudeVersionType version) = 0;

    // ! Returns the presently set version of the face extrude postprocessing for this interface.
    virtual IMNFaceExtrudeVersionType GetVersion() const = 0;

    //! Performs the topology changes for the face extrude operation, following the approach of MNMesh::ExtrudeFaceClusters,
    //! for the faces flagged with faceFlag, and sets up the data structures required to perform further processing in
    //! UpdateFaceExtrude and EndFaceExtrude.
    //! \param[in] faceFlag -- the MN_XXX selection flag used to faces to be extrude
    virtual void BeginFaceExtrude(DWORD faceFlag = MN_SEL) = 0;
    
    //! Performs intermediate postprocessing after extruded faces have been moved or scaled. Guaranteed not to alter geometry
    //! or topology of entities existing in the mesh prior to the first call to this function for a particular extrude
    //! operation, but may alter flags.
    //! \return flag indicating whether topology was altered
    virtual bool UpdateFaceExtrude() = 0;

    //! Finalizes postprocessing of the extrude operation. May alter vertex MN_WHATEVER flags.
    //! \return flag indicating whether topology was altered
