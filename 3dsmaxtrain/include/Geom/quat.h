...
public:
	Point3 axis;
	float angle = 0.f; // This angle is left-handed!
	...
	AngAxis& operator=(const AngAxis&) = default;
	AngAxis& operator=(AngAxis&&) = default;
	...
	specified Quat. */
	GEOM_EXPORT AngAxis(const Quat& q);
	...
	GEOM_EXPORT AngAxis(const Matrix3& m); // GEOM_EXPORT added in R5.1
...
public:
	...
	unit quaternion. */
	GEOM_EXPORT Quat(const Matrix3& mat);
	/*! \remarks Constructor. The Quat is initialized to the AngAxis passed. */
	GEOM_EXPORT Quat(const AngAxis& aa);
	...
	angle <b>W</b> passed. The quaternion is then normalized.\n\n
	  */
	GEOM_EXPORT Quat(const Point3& V, float W);
	...
	*/
	...
	GEOM_EXPORT Quat& Set(const Matrix3& mat);
	GEOM_EXPORT Quat& Set(const AngAxis& aa);
	...
	GEOM_EXPORT Quat& SetEuler(float X, float Y, float Z);
	GEOM_EXPORT Quat& Invert(); // in place

	/*! \remarks Modifies <b>q</b> so it is on same side of hypersphere as
	<b>qto</b>. */
	GEOM_EXPORT Quat& MakeClosest(const Quat& qto);

	// Comparison
	/*! \remarks Returns nonzero if the quaternions are equal; otherwise 0. */
	GEOM_EXPORT int operator==(const Quat& a) const;
	GEOM_EXPORT int operator!=(const Quat& a) const;
	GEOM_EXPORT int Equals(const Quat& a, float epsilon = 1E-6f) const;
	...
	0. */
	GEOM_EXPORT int IsIdentity() const;
	...
	*/
	GEOM_EXPORT void Normalize(); // normalize
	...
	GEOM_EXPORT void MakeMatrix(Matrix3& mat, bool flag = false) const; // flag added 001031  --prs.
	GEOM_EXPORT void GetEuler(float* X, float* Y, float* Z) const;

	// Binary operators
	/*! \remarks This operator is the same as the <b>/</b> operator. */
	GEOM_EXPORT Quat operator-(const Quat&) const; // RB: Changed these to    // difference of two quaternions
	/*! \remarks This operator is the same as the <b>*</b> operator. */
	GEOM_EXPORT Quat operator+(const Quat&) const; // duplicate * and /         // sum of two quaternions
	/*! \remarks Returns the product of two quaternions. */
	GEOM_EXPORT Quat operator*(const Quat&)const; // product of two quaternions
	...
	important) */
	GEOM_EXPORT Quat operator/(const Quat&) const; // ratio of two quaternions
	GEOM_EXPORT float operator%(const Quat&) const; // dot product
	GEOM_EXPORT Quat Plus(const Quat&) const; // what + should have done
	GEOM_EXPORT Quat Minus(const Quat&) const; // what - should have done
};

/*! \remarks Multiplies the quaternion by a scalar. */
GEOM_EXPORT Quat operator*(float, const Quat&); // multiply by scalar
/*! \remarks Multiplies the quaternion by a scalar. */
GEOM_EXPORT Quat operator*(const Quat&, float); // multiply by scalar
/*! \remarks Divides the quaternion by a scalar. */
GEOM_EXPORT Quat operator/(const Quat&, float); // divide by scalar
/*! \remarks Returns the inverse of the quaternion (1/<b>q</b>). */
GEOM_EXPORT Quat Inverse(const Quat& q); // Inverse of quaternion (1/q)
/*! \remarks Returns the conjugate of a quaternion. */
GEOM_EXPORT Quat Conjugate(const Quat& q);
/*! \remarks Returns the natural logarithm of UNIT quaternion. */
GEOM_EXPORT Quat LogN(const Quat& q);
/*! \remarks Exponentiate quaternion (where <b>q.w</b>==0). */
GEOM_EXPORT Quat Exp(const Quat& q);
...
GEOM_EXPORT Quat squadrev(float angle, // angle of rotation
...
GEOM_EXPORT void RotateMatrix(Matrix3& mat, const Quat& q);
...
GEOM_EXPORT void PreRotateMatrix(Matrix3& mat, const Quat& q);
/*! \remarks Converts the [angle,axis] representation to the equivalent
quaternion. */
GEOM_EXPORT Quat QFromAngAxis(float ang, const Point3& axis);
/*! \remarks Converts the quaternion to the equivalent [angle,axis]
representation. */
GEOM_EXPORT void AngAxisFromQ(const Quat& q, float* ang, Point3& axis);
/*! \remarks Compute the [angle,axis] corresponding to the rotation from
<b>p</b> to <b>q</b>. Returns angle, sets axis. */
GEOM_EXPORT float QangAxis(const Quat& p, const Quat& q, Point3& axis);
...
GEOM_EXPORT void DecomposeMatrix(const Matrix3& mat, Point3& p, Quat& q, Point3& s);
/*! \remarks Returns the transformation of the specified quaternion by the
specified matrix. */
GEOM_EXPORT Quat TransformQuat(const Matrix3& m, const Quat& q);
...
GEOM_EXPORT void QuatToEuler(Quat& q, float* ang);
...
\endcode
*/
GEOM_EXPORT void EulerToQuat(float* ang, Quat& q);
GEOM_EXPORT Quat EulerToQuat(float x, float y, float z);
