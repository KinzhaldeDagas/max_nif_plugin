...
template <>
inline IPoint3 Interpolate(const IPoint3& P0, const IPoint3& P1, const float alpha)
{
...
template <>
inline IPoint2 Interpolate(const IPoint2& P0, const IPoint2& P1, const float alpha)
{
	...
	const PointType &P1)
{
	...
	if (wDotV <= 0.0)
	{
		return float(w.LengthSquared());  // Nearest point is P0
	}

	const double vDotV = v.DotProd(v);
	if (wDotV >= vDotV)
	{
		return float(LengthSquared(P - P1)); // Nearest point is P1
	}
	else
	{
	...
	typename PointTraits< PointType >::StorageType  edgeLengthSquared)
{
	...
	else if (projectionAmount >= edgeLengthSquared)
	{
		...
		return Interpolate(P0, P1, float(alpha));
	}
}
