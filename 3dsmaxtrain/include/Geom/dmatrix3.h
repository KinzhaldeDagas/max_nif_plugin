...
//**************************************************************************/
// DESCRIPTION: Double matrix3 class.
// AUTHOR: Autodesk Inc.
//**************************************************************************/
...
/*! \sa Class DPoint3, Class Matrix3, Class Quat,  Class AngAxis, Structure
...
class DMatrix3
{
	...
	friend DMatrix3 GEOM_EXPORT RotateXMatrix(double angle);
	friend DMatrix3 GEOM_EXPORT RotateYMatrix(double angle);
	friend DMatrix3 GEOM_EXPORT RotateZMatrix(double angle);
	friend DMatrix3 GEOM_EXPORT TransMatrix(const DPoint3& p);
	friend DMatrix3 GEOM_EXPORT ScaleMatrix(const DPoint3& s);
	friend DMatrix3 GEOM_EXPORT RotateYPRMatrix(double yaw, double pitch, double roll);
	friend DMatrix3 GEOM_EXPORT RotAngleAxisMatrix(const DPoint3& axis, double angle);
	friend DMatrix3 GEOM_EXPORT Inverse(const DMatrix3& m);
	friend DPoint3 GEOM_EXPORT operator*(const DMatrix3& a, const DPoint3& v);
	friend DPoint3 GEOM_EXPORT operator*(const DPoint3& v, const DMatrix3& a);
	friend DPoint3 GEOM_EXPORT VectorTransform(const DMatrix3& m, const DPoint3& v);
	friend DMatrix3 GEOM_EXPORT XFormMat(const DMatrix3& xm, const DMatrix3& m);
	friend DPoint3 GEOM_EXPORT VectorTransform(const DPoint3& v, const DMatrix3& m);
	friend void GEOM_EXPORT MatrixMultiply(DMatrix3& outMatrix, const DMatrix3& matrixA, const DMatrix3& matrixB);
	friend void GEOM_EXPORT Inverse(DMatrix3& outMatrix, const DMatrix3& m);
	friend class Quat;
	...
	DPoint3& operator[](int i);

public:
	/*!  Constructor. Matrix is initialized to Identity. */
	GEOM_EXPORT DMatrix3();

	GEOM_EXPORT DMatrix3(const DMatrix3&) = default;
	GEOM_EXPORT DMatrix3(DMatrix3&&) = default;
	GEOM_EXPORT DMatrix3& operator=(const DMatrix3&) = default;
	GEOM_EXPORT DMatrix3& operator=(DMatrix3&&) = default;

	/*!  Returns a reference to the 'i-th' DPoint3 of the matrix. */
	GEOM_EXPORT const DPoint3& operator[](int i) const;
	...
	*/
	...
	/*! \remarks Returns the address of this DMatrix3.\n\n
	...
	GEOM_EXPORT DMRow* GetAddr();
	/*!  Returns the address of this DMatrix3.\n\n
	This provides read-only access to the DMatrix3 elements as a
	2 dimentional array.\n\n
	Also Note: typedef double DMRow[3]; */
	GEOM_EXPORT const DMRow* GetAddr() const;

	/*!  Constructor, Unused.
	The default constructor now initializes the matrix to identity.
	...
	*/
	...
	GEOM_EXPORT DMatrix3(const DPoint3& row0, const DPoint3& row1, const DPoint3& row2, const DPoint3& row3);
	...
	GEOM_EXPORT DMatrix3& Set(const DPoint3& row0, const DPoint3& row1, const DPoint3& row2, const DPoint3& row3);
	...
	/*! An global instance of DMatrix3 set to the identity.  An identity
		...
		[0, 0, 1]\n
		[0, 0, 0] */
	GEOM_EXPORT static const DMatrix3 Identity;
	...
	GEOM_EXPORT int operator==(const DMatrix3& m) const;

	/*!  Compares the elements of this matrix and the one specified element by
		element for inequality. Returns false iff they are unequal.
	\param  mat     The matrix to compare against. */
	GEOM_EXPORT bool operator!=(const DMatrix3& mat) const;
	...
	GEOM_EXPORT int Equals(const DMatrix3& m, double epsilon = 1E-12) const;

	/*!  Subtracts a DMatrix3 from this DMatrix3. */
	GEOM_EXPORT DMatrix3& operator-=(const DMatrix3& m);
	...
	/*!  Multiplies each element of this DMatrix3 by a double.
		 ...
		 \return a reference to this matrix */
	GEOM_EXPORT DMatrix3& operator*=(double a);
	...
	GEOM_EXPORT DPoint3 GetColumn3(int i) const;
	...
	/*!  A version of Orthogonalize() that uses the previous less-accurate version
	of the DPoint::operator + */
	...
	GEOM_EXPORT void SetTrans(const DPoint3& p);
	...
	GEOM_EXPORT void Translate(const DPoint3& p);
	...
	the old way. */
	GEOM_EXPORT void Scale(const DPoint3& s, bool trans = false);

	/*!  Apply an incremental translation transformation to this
	matrix. This is equivalent to multiplying on the LEFT by the transform.
	param  p    Specifies the translation distance. */
	GEOM_EXPORT void PreTranslate(const DPoint3& p);
	/*!  Apply an incremental X rotation transformation to this matrix.
	This is equivalent to multiplying on the LEFT by the transform.
	\param angle    Specifies the X rotation in radians. */
	GEOM_EXPORT void PreRotateX(double angle);
	/*!  Apply an incremental Y rotation transformation to this matrix.
	This is equivalent to multiplying on the LEFT by the transform.
	\param angle    Specifies the Y rotation in radians. */
	GEOM_EXPORT void PreRotateY(double angle);
	/*!  Apply an incremental Z rotation transformation to this matrix.
	This is equivalent to multiplying on the LEFT by the transform.
	\param angle    Specifies the Z rotation in radians. */
	GEOM_EXPORT void PreRotateZ(double angle);
	/*!  Apply an incremental scaling transformation to this matrix.
	This is equivalent to multiplying on the LEFT by the transform.
	\param s The scale values.
	The translation component is unaffected. */
	GEOM_EXPORT void PreScale(const DPoint3& s);
	/*!  Sets this matrix to the identity and the translation components to the
	specified values.
	\param  p     The translation values to store. */
	GEOM_EXPORT void SetTranslate(const DPoint3& p); // makes translation matrix
	...
	GEOM_EXPORT void SetRotateX(double angle); // makes rotation matrix
	/*!  Sets this matrix to the identity and the rotation components to the
	specified Y rotation.
	\param angle     The angle for Y rotation (in radians). */
	GEOM_EXPORT void SetRotateY(double angle);
	/*!  Sets this matrix to the identity and the rotation components to the
	specified Z rotation.
	\param angle     The angle for Z rotation (in radians). */
	GEOM_EXPORT void SetRotateZ(double angle);
	...
	GEOM_EXPORT void SetRotate(double yaw, double pitch, double roll);
	...
	GEOM_EXPORT void SetAngleAxis(const DPoint3& axis, double angle);
	...
	GEOM_EXPORT void SetScale(const DPoint3& s); // makes scale matrix
	...
	GEOM_EXPORT void SetFromToUp(const DPoint3& from, const DPoint3& to, const DPoint3& up);
	...
	GEOM_EXPORT DMatrix3 operator*(const DMatrix3&)const;
	...
	GEOM_EXPORT DPoint3 PointTransform(const DPoint3& p) const;

	/*!  Returns the specified vector transformed by this matrix.
	\param  p     The vector to transform by this matrix. */
	GEOM_EXPORT DPoint3 VectorTransform(const DPoint3& p) const;
	...
	/*! 	 Transforms the specified list of points with this matrix.
	 ...
	 sizeof(data_object)). */
	GEOM_EXPORT void TransformPoints(DPoint3* array, int n, int stride = sizeof(DPoint3));
	...
	GEOM_EXPORT void TransformPoints(
			const DPoint3* array, DPoint3* to, int n, int stride = sizeof(DPoint3), int strideTo = sizeof(DPoint3));
	...
	/*! 	 Transforms the specified list of vectors with this matrix.
	 ...
	 member) you can specify a 'stride' value (for instance
	 sizeof(data_object)). */
	GEOM_EXPORT void TransformVectors(DPoint3* array, int n, int stride = sizeof(DPoint3));
	...
	GEOM_EXPORT void GetYawPitchRoll(double* yaw, double* pitch, double* roll);
	...
	GEOM_EXPORT Matrix3 ToMatrix3();

	/*! Copies a matrix3 into the DMatrix3	*/
	GEOM_EXPORT void FromMatrix3(const Matrix3& tm);
};
...
GEOM_EXPORT DMatrix3 RotateYPRMatrix(double yaw, double pitch, double roll);
...
\return  A new rotation DMatrix3. */
GEOM_EXPORT DMatrix3 RotAngleAxisMatrix(const DPoint3& axis, double angle);

/*!  Return the inverse of the matrix
\param  m The matrix to compute the inverse of. */
GEOM_EXPORT DMatrix3 Inverse(const DMatrix3& m); // return Inverse of matrix
...
GEOM_EXPORT DPoint3 operator*(const DMatrix3& a, const DPoint3& v); // Transform Point with matrix
...
\param  a The matrix to transform the point with.
\return  The transformed DPoint3. */
GEOM_EXPORT DPoint3 operator*(const DPoint3& v, const DMatrix3& a); // Transform Point with matrix
...
GEOM_EXPORT DPoint3 VectorTransform(const DMatrix3& m, const DPoint3& v);
GEOM_EXPORT DPoint3 VectorTransform(const DPoint3& v, const DMatrix3& m);
...
GEOM_EXPORT Point4 TransformPlane(const DMatrix3& m, const Point4& plane);
...
GEOM_EXPORT DMatrix3 XFormMat(const DMatrix3& xm, const DMatrix3& m);
...
GEOM_EXPORT void MirrorMatrix(DMatrix3& tm, Axis axis, bool scaleMatrix = false);
...
GEOM_EXPORT void MatrixMultiply(DMatrix3& outMatrix, const DMatrix3& matrixA, const DMatrix3& matrixB);
...
/*!  Same as Maxtrix3::Inverse. Compute the inverse of the matrix without additional matrix copy
	...
	\param  m The matrix to compute the inverse of. */
GEOM_EXPORT void Inverse(DMatrix3& outMatrix, const DMatrix3& m);
