 ...
 * Silences warning when using the Matrix(bool) constructor.
 * Matrix3 and DMatrix3 are now created as identity by default!
 */
#if defined(MAX_SILENCE_DEPRECATED_MATRIX_BOOL_CTOR)
#define MAX_DEPRECATE_MATRIX_BOOL_CTOR(str)
#else
...
/*! \sa  Class Point3, ~{ %Matrix Representations of 3D Transformations }~, Class Quat,  Class AngAxis, Structure
AffineParts, Class BigMatrix.\n\n
\par Description:
This class implements a 4x3 3D transformation matrix object. Methods
are provided to zero the matrix, set it to the identity, compute its inverse, apply incremental translation, rotation
and scaling, and build new X, Y and Z rotation matrices. Operators are provided for matrix addition, subtraction, and
...
class Matrix3
{
	...
	friend Matrix3 GEOM_EXPORT RotateXMatrix(float angle);
	friend Matrix3 GEOM_EXPORT RotateYMatrix(float angle);
	friend Matrix3 GEOM_EXPORT RotateZMatrix(float angle);
	friend Matrix3 GEOM_EXPORT TransMatrix(const Point3& p);
	friend Matrix3 GEOM_EXPORT ScaleMatrix(const Point3& s);
	friend Matrix3 GEOM_EXPORT RotateYPRMatrix(float Yaw, float Pitch, float Roll);
	friend Matrix3 GEOM_EXPORT RotAngleAxisMatrix(const Point3& axis, float angle);
	friend Matrix3 GEOM_EXPORT Inverse(const Matrix3& M);
	friend Matrix3 GEOM_EXPORT PseudoInverse(const Matrix3& m);
	friend Matrix3 GEOM_EXPORT InverseHighPrecision(const Matrix3& M);
	friend Matrix3 GEOM_EXPORT AffineTranspose(const Matrix3& M);
	friend Point3 GEOM_EXPORT operator*(const Matrix3& A, const Point3& V);
	friend Point3 GEOM_EXPORT operator*(const Point3& V, const Matrix3& A);
	friend Point3 GEOM_EXPORT VectorTransform(const Matrix3& M, const Point3& V);
	friend Matrix3 GEOM_EXPORT XFormMat(const Matrix3& xm, const Matrix3& m);
	friend Point3 GEOM_EXPORT VectorTransform(const Point3& V, const Matrix3& M);
	friend void GEOM_EXPORT MatrixMultiply(Matrix3& outMatrix, const Matrix3& matrixA, const Matrix3& matrixB);
	friend void GEOM_EXPORT Inverse(Matrix3& outMatrix, const Matrix3& M);
	friend void GEOM_EXPORT AffineTranspose(Matrix3& outMatrix, const Matrix3& M);
	friend class Quat;
...
public:
	...
	GEOM_EXPORT Matrix3(const Matrix3&) = default;
	GEOM_EXPORT Matrix3(Matrix3&&) = default;
	GEOM_EXPORT Matrix3& operator=(const Matrix3&) = default;
	GEOM_EXPORT Matrix3& operator=(Matrix3&&) = default;

	/*! \remarks Returns a reference to the 'i-th' Point3 of the matrix. */
	GEOM_EXPORT const Point3& operator[](int i) const;
	/*! \remarks This clears the <b>MAT_IDENT</b> flag to indicate the matrix
	is not the identity. Non-const <b>GetAddr()</b> calls this for you. */
	GEOM_EXPORT void SetNotIdent();
	...
	*/
	...
	GEOM_EXPORT void ValidateFlags();
	...
	the matrix itself and then you can use the [] operator on the result.\n
	Note: <b>GetAddr()</b> always calls <b>SetNotIdent()</b>, which clears all 
	IDENT flags. If you want to re-enable some optimizations, you can use 
	<b>SetIdentFlags()</b> to mark identity components after this call.\n\n
	Also Note: <b>typedef float MRow[3];</b>
	\return  The address of the Matrix3. */
	GEOM_EXPORT MRow* GetAddr();
	/*! \remarks Returns the address of this Matrix3.\n\n
	This provides read-only access to the Matrix3 elements as a
	2 dimentional array.\n\n
	Also Note: <b>typedef float MRow[3];</b> */
	GEOM_EXPORT const MRow* GetAddr() const;

	/*! \remarks Constructor, Unused.
	Default constructor now initializes to identity.
	...
	*/
	...
	GEOM_EXPORT Matrix3(const Point3& U, const Point3& V, const Point3& N, const Point3& T);
	...
	GEOM_EXPORT Matrix3& Set(const Point3& U, const Point3& V, const Point3& N, const Point3& T);
	...
	/*! An global instance of Matrix3 set to the identity.  An identity
		...
		[0, 0, 1]\n
		[0, 0, 0] */
	GEOM_EXPORT static const Matrix3 Identity;
	...
	GEOM_EXPORT int operator==(const Matrix3& M) const;

	/*! \remarks Compares the elements of this matrix and the one specified element by
		element for inequality. Returns false iff they are unequal.
	\param  M     The matrix to compare against. */
	GEOM_EXPORT bool operator!=(const Matrix3& M) const;
	...
	GEOM_EXPORT int Equals(const Matrix3& M, float epsilon = 1E-6f) const;

	/*! \remarks Subtracts a <b>Matrix3</b> from this <b>Matrix3</b>. */
	GEOM_EXPORT Matrix3& operator-=(const Matrix3& M);
	...
	GEOM_EXPORT void Orthogonalize();
	...
	GEOM_EXPORT void SetTrans(const Point3& p);
	...
	GEOM_EXPORT void Translate(const Point3& p);
	...
	the old way. */
	GEOM_EXPORT void Scale(const Point3& s, bool trans = false);

	/*! \remarks Apply an incremental translation transformation to this
	matrix. This is equivalent to multiplying on the LEFT by the transform.
	param  p    Specifies the translation distance. */
	GEOM_EXPORT void PreTranslate(const Point3& p);
	/*! \remarks Apply an incremental X rotation transformation to this matrix.
	This is equivalent to multiplying on the LEFT by the transform.
	\param angle    Specifies the X rotation in radians. */
	GEOM_EXPORT void PreRotateX(float angle);
	/*! \remarks Apply an incremental Y rotation transformation to this matrix.
	This is equivalent to multiplying on the LEFT by the transform.
	\param angle    Specifies the Y rotation in radians. */
	GEOM_EXPORT void PreRotateY(float angle);
	/*! \remarks Apply an incremental Z rotation transformation to this matrix.
	This is equivalent to multiplying on the LEFT by the transform.
	\param angle    Specifies the Z rotation in radians. */
	GEOM_EXPORT void PreRotateZ(float angle);
	/*! \remarks Apply an incremental scaling transformation to this matrix.
	This is equivalent to multiplying on the LEFT by the transform.
	\param s The scale values.
	The translation component is unaffected. */
	GEOM_EXPORT void PreScale(const Point3& s);
	/*! \remarks Sets this matrix to the identity and the translation components to the
	specified values.
	\param  p     The translation values to store. */
	GEOM_EXPORT void SetTranslate(const Point3& p); // makes translation matrix
	...
	GEOM_EXPORT void SetRotateX(float angle); // makes rotation matrix
	/*! \remarks Sets this matrix to the identity and the rotation components to the
	specified Y rotation.
	\param angle     The angle for Y rotation (in radians). */
	GEOM_EXPORT void SetRotateY(float angle);
	/*! \remarks Sets this matrix to the identity and the rotation components to the
	specified Z rotation.
	\param angle     The angle for Z rotation (in radians). */
	GEOM_EXPORT void SetRotateZ(float angle);
	...
	GEOM_EXPORT void SetRotate(const Quat& q);
	...
	GEOM_EXPORT void SetRotate(const AngAxis& aa);
	...
	GEOM_EXPORT void SetRotate(float yaw, float pitch, float roll);
	...
	GEOM_EXPORT void SetAngleAxis(const Point3& axis, float angle);
	...
	GEOM_EXPORT void SetScale(const Point3& s); // makes scale matrix
	...
	GEOM_EXPORT void SetFromToUp(const Point3& from, const Point3& to, const Point3& up);
	...
	GEOM_EXPORT void Invert();

	/*! \remarks Perform matrix multiplication. */
	GEOM_EXPORT Matrix3 operator*(const Matrix3&)const;
	...
	GEOM_EXPORT Point3 PointTransform(const Point3& p) const;

	/*! \remarks Returns the specified vector transformed by this matrix.
	\param  p     The vector to transform by this matrix. */
	GEOM_EXPORT Point3 VectorTransform(const Point3& p) const;
	...
	GEOM_EXPORT void TransformPoints(Point3* array, int n, int stride = sizeof(Point3));
	...
	GEOM_EXPORT void TransformPoints(
			const Point3* array, Point3* to, int n, int stride = sizeof(Point3), int strideTo = sizeof(Point3));
	...
	/*! \remarks	 Transforms the specified list of vectors with this matrix.
	 ...
	 member) you can specify a 'stride' value (for instance
	 <b>sizeof(data_object)</b>). */
	GEOM_EXPORT void TransformVectors(Point3* array, int n, int stride = sizeof(Point3));
	...
	/*! \remarks	 Transforms the specified list of vectors with this matrix and stores the
	 ...
	 \param stride The size increment used when moving to the next source location.
	 \param strideTo  The size increment used when moving to the next storage location. */
	GEOM_EXPORT void TransformVectors(
			const Point3* array, Point3* to, int n, int stride = sizeof(Point3), int strideTo = sizeof(Point3));
	...
	GEOM_EXPORT void GetYawPitchRoll(float* yaw, float* pitch, float* roll);
	...
	\remarks Save and Load are only forward declared in this header!
	If you need to use Save or Load, you must link with maxutil, which
	is where these methods are implemented.
	This way, geom.dll is independent from maxsdk. */
...
GEOM_EXPORT Matrix3 TransMatrix(const Point3& p);
...
GEOM_EXPORT Matrix3 ScaleMatrix(const Point3& s);
...
GEOM_EXPORT Matrix3 RotateYPRMatrix(float Yaw, float Pitch, float Roll);
...
GEOM_EXPORT Matrix3 RotAngleAxisMatrix(const Point3& axis, float angle);

/*! \remarks Return the inverse of the matrix
\param  M The matrix to compute the inverse of. */
GEOM_EXPORT Matrix3 Inverse(const Matrix3& M); // return Inverse of matrix

/*! \remarks Return the inverse of the matrix using doubles for the intermediary results
	\param  M The matrix to compute the inverse of. */
GEOM_EXPORT Matrix3 InverseHighPrecision(const Matrix3& M);
...
GEOM_EXPORT Matrix3 PseudoInverse(const Matrix3& m);
...
GEOM_EXPORT Matrix3 AffineTranspose(const Matrix3& M); // return affine transpose of matrix
...
GEOM_EXPORT Point3 operator*(const Matrix3& A, const Point3& V); // Transform Point with matrix
...
GEOM_EXPORT Point3 operator*(const Point3& V, const Matrix3& A); // Transform Point with matrix
...
GEOM_EXPORT Point3 VectorTransform(const Matrix3& M, const Point3& V);
GEOM_EXPORT Point3 VectorTransform(const Point3& V, const Matrix3& M);
...
GEOM_EXPORT Point4 TransformPlane(const Matrix3& M, const Point4& plane);
...
GEOM_EXPORT Matrix3 XFormMat(const Matrix3& xm, const Matrix3& m);
...
GEOM_EXPORT void MirrorMatrix(Matrix3& tm, Axis axis, bool scaleMatrix = false);
...
GEOM_EXPORT void MatrixMultiply(Matrix3& outMatrix, const Matrix3& matrixA, const Matrix3& matrixB);
...
/*! \remarks Same as Maxtrix3::Inverse. Compute the inverse of the matrix without additional matrix copy
	...
	\param  M The matrix to compute the inverse of. */
GEOM_EXPORT void Inverse(Matrix3& outMatrix, const Matrix3& M);

/*! \remarks Same as Matrix3::AffineTranspose. Computes the affine transpose without additional matrix copy
  \param  outMatrix The inversed matrix.
  \param  M The matrix to compute the affine transpose of. */
GEOM_EXPORT void AffineTranspose(Matrix3& outMatrix, const Matrix3& M);

/*! \remarks Create a rotational matrix oriented to the provided normal.
  \param  normal The normal vector, must be normalized.
  \param  mat The output orientation matrix. */
GEOM_EXPORT void SetMatrixFromNormal(const Point3& normal, Matrix3& mat);
