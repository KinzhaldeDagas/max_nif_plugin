...
*/
class GEOM_EXPORT VertexNormal
{
public:
	Point3 norm; ///< The normal.
	uint32_t smooth = 0; ///< The smoothing groups.
	VertexNormal* next = nullptr; ///< A pointer to the next normal -- this class is a linked list.
	bool init = false; ///< The init variable is used as a flag to indicate if the first normal in the list has been
			   ///< initialized.
	...
	*/
	VertexNormal(const Point3& n, uint32_t s);
	...
	*/
	void AddNormal(const Point3& n, uint32_t s);
