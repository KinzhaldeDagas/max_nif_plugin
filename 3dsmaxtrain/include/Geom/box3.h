...
public:
	...
	that the box is 'empty' and 'uninitialized'. See <b>IsEmpty()</b> and <b>IsInitialized()</b>. */
	GEOM_EXPORT Box3();
	...
	'empty' and 'uninitialized'. See <b>IsEmpty()</b> and <b>IsInitialized()</b>. */
	GEOM_EXPORT void Init();
	...
	GEOM_EXPORT void MakeCube(const Point3& p, float side);
	...
	GEOM_EXPORT Point3 operator[](int i) const;
	...
	GEOM_EXPORT void Translate(const Point3& p); // translate box
	...
	enlarged. */
	GEOM_EXPORT void EnlargeBy(float s); // enlarge by this amount on all sides

	// include an array of points, optionally transformed by tm
	GEOM_EXPORT void IncludePoints(const Point3* pts, int numpoints, Matrix3* tm = nullptr);
	...
	GEOM_EXPORT Box3 operator*(const Matrix3& tm) const;
	...
	/*! \remarks Determines if the box is empty.
	A box is considered empty if any component value of pmin is greater than the
	corresponding component value of pmax.
	\return  true if the box is empty; otherwise false. */
	GEOM_EXPORT bool IsEmpty() const; // is this box empty?
	/*! \remarks Determines if the box is initialized. This indicates the box
	has had specific values set by the developer.
	\return  true if the box is initialized; otherwise false. */
	GEOM_EXPORT bool IsInitialized() const; // is this box initialized?
	/*! \remarks Determines if the specified point <b>p</b> is contained
	in this box.
	\par Parameters:
	<b>const Point3\& p</b>\n\n
	Specifies the point to check.
	\return  true if the specified point is contained in this box;
	otherwise false. */
	GEOM_EXPORT bool Contains(const Point3& p) const; // is point in this box?
	/*! \remarks Determines if the specified Box3 is contained totally
	within this box.
	\par Parameters:
	<b>const Box3\& b</b>\n\n
	Specifies the box to check.
	\return  true if the specified box is entirely contained within this
	box; otherwise false.
	\par Operators:
	*/
	GEOM_EXPORT bool Contains(const Box3& b) const; // is box b totally in this box?
	/*! \remarks Determines if the specified Box3 intersects this box.
	\par Parameters:
	<b>const Box3\& b</b>\n\n
	Specifies the box to check.
	\return  true if the specified Box3 intersects this box; otherwise false.
	\par Operators:
	*/
	GEOM_EXPORT bool Intersects(const Box3& b) const; // does box b intersect this box at all?

	/*! \remarks Determines if the triangle specified by the tree points <b>vert0</b>, <b>vert1</b>, <b>vert2</b> is
	overlapping with this box.
	\return  true if the specified triangle overlaps with this box in some way; otherwise false.
	*/
	GEOM_EXPORT bool TriBoxOverlap(const Point3 vert0, const Point3 vert1,
			const Point3 vert2) const; // does the triangle intersect this box at all?
};
