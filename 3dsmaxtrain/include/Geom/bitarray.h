...
class BitArray
{
	// Hardcoded to 64bits. We could use size_t if we want to be platform dependent.
	enum
	{
		kMAX_LOCALBITS = 8 * sizeof(uint64_t),
	};
...
public:
	...
	public:
		...
		inline bool operator==(int n) const // if( array.NumberSet() == 3 )
		{
			return n?mArray.NumberSetEqualImpl(n): mArray.IsEmpty();
		}

		inline bool operator!=(int n) const // if( array.NumberSet() != 3 )
		{
			return n?!mArray.NumberSetEqualImpl(n):mArray.AnyBitSet();
		}
	...
	private:
		...
		inline NumberSetProxy(const BitArray& a)
				: mArray(a)
		{
		}
...
public:
	...
	\param save - If passed as 1, the old bit values will be preserved when the array is resized. */
	GEOM_EXPORT void SetSize(int n, int save = 0); // save=1:preserve old bit values
	...
	GEOM_FORCEINLINE int operator[](int i) const
	{
		assert(i > -1);
		assert(i < numBits);
		if ((i > -1) && (i < numBits))
	...
	future use. */
	GEOM_EXPORT void Compress();
	/*! \remarks This is not currently implemented and is reserved for
	future use. */
	GEOM_EXPORT void Expand();
	/*! \remarks Reverses the bits in the BitArray.
	\par Parameters:
	<b>BOOL keepZero = FALSE</b>\n\n
	If TRUE the zero bit is kept where it is. */
	GEOM_EXPORT void Reverse(bool keepZero = false); // keepZero=TRUE keeps zero bit where it is
	...
	GEOM_EXPORT void Rotate(int direction, int count); // With wraparound
	...
	GEOM_EXPORT void Shift(int direction, int count, int where = 0); // Without wraparound
	...
	GEOM_EXPORT void EnumSet(BitArrayCallback& cb); // enumerates elements that are 1's
	...
	f, you'd use mult=3:\n\n
	If the bit array has more values than correspond to the size of dset, the
	extra values will be retained at the end after deleting.  The alternate
	method 'DeleteSetAndResize' will remove the extra values by resizing if
	needed.\n\n
	<b>faceSel.DeleteSet (fdel);</b>\n\n
	<b>edgeSel.DeleteSet (fdel, 3);</b> */
	GEOM_EXPORT void DeleteSet(BitArray& dset, int mult = 1);
	GEOM_EXPORT void DeleteSetAndResize(const BitArray& dset, int mult = 1);
	...
	Due to how they are saved, BitArray values need to be saved in their own Chunk. 
	If they are not saved in their own Chunk, and other data values are stored
	in the same Chunk after the BitArray, in some cases the loading of the BitArray
	will try loading the data saved after the BitArray, resulting in load failure or
	incorrect results.
	In 3dsmax 2026, a warning dialog will be displayed if a BitArray is not saved
	in its own Chunk. If this dialog is displayed, the outer save code should be modified
	to Begin/End a Chunk around the BitArray save.
	In order to support Save To Previous though, the old behavior may still be required.
	To handle this, method ISave::DisableChunkWarnings(bool) was added to disable the
	warning dialog. An example of the Save code usage is:
	/code
		// The following is writing a BitArray, but it was not in its own Chunk prior to 3dsmax 2026
		DWORD saveVersion = isave->SavingVersion();
		IOResult ioRes;
		if ((saveVersion != 0) && (saveVersion <= MAX_RELEASE_R27))
		{
			isave->DisableChunkWarnings(true);
			ioRes = mCategories.Save(isave);
			isave->DisableChunkWarnings(false);
		}
		else
		{
			isave->BeginChunk(TargetedIORootNodeCategory_chunkid);
			ioRes = mCategories.Save(isave);
			isave->EndChunk();
		}
	/endcode
	An example of the corresponding Load code would be:
	/code
		// prior to 3dsmax 2026, mCategories was not stored in its own chunk
		USHORT chunkID = iload->PeekNextChunkID();
		if (chunkID == TargetedIORootNodeCategory_chunkid)
		{
			iload->OpenChunk();
			mCategories.Load(iload);
			iload->CloseChunk();
		}
		else
			mCategories.Load(iload);
	...
	*/
	...
	GEOM_EXPORT BitArray& operator=(const BitArray& b);

	// Assignment operators: These require arrays of the same size!
	/*! \remarks AND= this BitArray with the specified BitArray. */
	GEOM_EXPORT BitArray& operator&=(const BitArray& b); // AND=
	/*! \remarks OR= this BitArray with the specified BitArray. */
	GEOM_EXPORT BitArray& operator|=(const BitArray& b); // OR=
	/*! \remarks XOR= this BitArray with the specified BitArray. */
	GEOM_EXPORT BitArray& operator^=(const BitArray& b); // XOR=

	// Binary operators: These require arrays of the same size!
	/*! \remarks AND two BitArrays */
	GEOM_EXPORT BitArray operator&(const BitArray&)const; // AND
	/*! \remarks OR two BitArrays */
	GEOM_EXPORT BitArray operator|(const BitArray&) const; // OR
	/*! \remarks XOR two BitArrays */
	GEOM_EXPORT BitArray operator^(const BitArray&) const; // XOR
	...
	GEOM_EXPORT void Swap(BitArray& other);

private:
	enum
	{
		BITS_PER_DWORD_PTR = (8 * sizeof(uint64_t)),
		BITS_PER_DWORD_PTR_MASK = BITS_PER_DWORD_PTR - 1,
#ifdef GEOM_32BIT
		NSHIFT = 5, // 1 << 5 = 32
		DWORD_PTR_BYTES_SHIFT = 2, // 1 DWORD_PTR << 2 = 4 bytes
#else
		NSHIFT = 6, // 1 << 6 = 64
		DWORD_PTR_BYTES_SHIFT = 3, // 1 DWORD_PTR << 3 = 8 bytes
#endif
	};
	inline int Ndword_ptr(int k) const
	{
		return (int64_t(k) + BITS_PER_DWORD_PTR - 1) / BITS_PER_DWORD_PTR;
	}
	inline int Nbytes(int k) const
	{
		return Ndword_ptr(k) << DWORD_PTR_BYTES_SHIFT;
	}

	inline BitArray(uint64_t localBits_, int numBits_, bool zeroHighBits = false)
			: localBits(localBits_)
			, numBits(numBits_)
	{
		assert(UseLocalBits());
	...
	inline int GetBit(int i) const
	{
		...
		uint64_t bitMask = BitMask(i & BITS_PER_DWORD_PTR_MASK);
		return (bits[bitIndex] & bitMask) ? 1 : 0;
	}
	inline bool UseLocalBits() const
	{
		return numBits <= kMAX_LOCALBITS;
	}
	inline bool UseLocalBits(int n) const
	{
		return n <= kMAX_LOCALBITS;
	}
	inline uint64_t BitMask(int i) const
	...
	{
		return (i < kMAX_LOCALBITS) ? (uint64_t(1) << i) : uint64_t(0);
	}

	// Used internally to treat the bit array the same way whether it's new'ed or
	// simply local.
	inline const uint64_t* GetBitPtr() const
	{
		return UseLocalBits() ? &localBits : bits;
	}

	inline uint64_t* GetBitPtr()
	{
		return UseLocalBits() ? &localBits : bits;
	}

	// Called from the ctor only; initializes an array filled with zeroes.
	GEOM_EXPORT void CreateBitArrayImpl(int n);
	// Called from the ctor only; initializes from an array of bits.
	GEOM_EXPORT void SetBitsFromImpl(const BitArray&);

	GEOM_EXPORT void ClearAllImpl();
	GEOM_EXPORT void SetAllImpl();

	GEOM_EXPORT void SetImpl(int i);
	GEOM_EXPORT void ClearImpl(int i);

	GEOM_EXPORT void SetImpl(int i, int b);
	GEOM_EXPORT int GetNthBitImpl(int i) const;
	GEOM_EXPORT int NumberSetImpl() const;
	GEOM_EXPORT bool IsEmptyImpl() const;

	GEOM_EXPORT BitArray OperatorNotImpl() const;
	GEOM_EXPORT bool CompareBitsImpl(const BitArray&) const;

	GEOM_EXPORT void FreeBitsImpl();
	...
	GEOM_EXPORT void BitArrayAllocated();
	GEOM_EXPORT void BitArrayDeallocated();
#endif

	GEOM_EXPORT bool NumberSetImpl(int n) const; // Exhaustive count; can be dead slow
	GEOM_EXPORT bool NumberSetEqualImpl(int n) const; // Stops as soon as count will be higher
	GEOM_EXPORT bool NumberSetAtLeastImpl(int n) const; // Stops as soon as count reaches limit.

	// Zeroes out bits over numBits so we can always use fast comparisons (memcmp,
	// ==, etc) without having to mask out the last chunk.
	GEOM_EXPORT void ZeroUnusedBitsImpl();
};
