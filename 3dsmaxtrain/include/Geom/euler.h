...
GEOM_EXPORT void QuatToEuler(const Quat& q, float* ang, int type, bool flag = false); // flag added 001101  --prs.
GEOM_EXPORT void EulerToQuat(float* ang, Quat& q, int type);
GEOM_EXPORT void MatrixToEuler(const Matrix3& mat, float* ang, int type, bool flag = false);
GEOM_EXPORT void EulerToMatrix(float* ang, Matrix3& mat, int type);
GEOM_EXPORT float GetEulerQuatAngleRatio(
		Quat& quat1, Quat& quat2, float* euler1, float* euler2, int type = EULERTYPE_XYZ);
GEOM_EXPORT float GetEulerMatAngleRatio(
		Matrix3& mat1, Matrix3& mat2, float* euler1, float* euler2, int type = EULERTYPE_XYZ);
GEOM_EXPORT void ContinuousQuatToEuler(const Quat& quat, float ang[3], int order);
...
public:
	...
	{
		if (et == mRep)
			return Point3(mQ.x, mQ.y, mQ.z);
		else
			return ToEulerAngles(et);
	}
	...
	/*! \remarks Please see method Euler() for fuller explanation. */
	GEOM_EXPORT operator Matrix3() const;
	...
	GEOM_EXPORT void PreApplyTo(Matrix3& m) const; // m = *this * m
	/*! \remarks Given a matrix, m, we can apply the rotation, rv, of
	RotationValue from right side (PoseApplyTo)\n\n
	 <b>rv.PoseApplyTo(m) == m * (Matrix3)rv)</b> */
	GEOM_EXPORT void PostApplyTo(Matrix3& m) const; // m = m * *this
	// *this = *this * aa
	...
	GEOM_EXPORT void PostRotate(const AngAxis& aa);
...
protected:
	GEOM_EXPORT Point3 ToEulerAngles(EulerType et) const;
	GEOM_EXPORT Quat ToQuat() const;
