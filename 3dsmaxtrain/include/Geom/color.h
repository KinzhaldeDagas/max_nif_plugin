...
struct RealPixel
{
	...
	uint8_t r, g, b; // mantissas
	/*! The base 2 exponent of the maximum RGB component. */
	uint8_t e; // exponent
	/*! \remarks Converts the RealPixel format to the Color format. */
	GEOM_EXPORT operator Color();
};

GEOM_EXPORT RealPixel MakeRealPixel(float r, float g, float b);
GEOM_EXPORT void ExpandRealPixel(const RealPixel& rp, float& r, float& g, float& b);

/*! \sa  Class AColor, Structure BMM_Color_24, Structure BMM_Color_32, Structure BMM_Color_48, Structure BMM_Color_64,
Structure BMM_Color_fl, Structure RealPixel.\n\n
\par Description:
This class represents color as three floating point
values: r, g, and b. All methods of this class are implemented by the system.
\par Data Members:
<b>float r,g,b;</b>\n\n
...
public:
	...
	GEOM_EXPORT /*explicit*/ Color(const AColor& c);
	GEOM_EXPORT /*explicit*/ Color(AColor&& c);
	...
	GEOM_EXPORT explicit Color(uint32_t rgb); // from Windows RGB value
	...
	GEOM_EXPORT void ClampMax(); // makes components >= 0.0
	/*! \remarks Makes all the components of the Color \>= 0.0 */
	GEOM_EXPORT void ClampMin(); // makes components <= 1.0
	/*! \remarks Makes all the components fall in the range [0,1] */
	GEOM_EXPORT void ClampMinMax(); // makes components in [0,1]
	...
	//	operator uint32_t() { return RGB(FLto255(r),FLto255(g), FLto255(b)); }
	uint32_t toRGB() const
	{
		return MAX_RGB(FLto255(r), FLto255(g), FLto255(b));
	};

	// Convert to RealPixel
	/*! \remarks Convert the Color to the RealPixel format. */
	GEOM_EXPORT operator RealPixel() const;
	...
	{
		BMM_Color_24 c;
		c.r = (uint8_t) int(r * 255.0f);
		c.g = (uint8_t) int(g * 255.0f);
		c.b = (uint8_t) int(b * 255.0f);
		return c;
	}
	...
	{
		BMM_Color_48 c;
		c.r = (uint16_t) int(r * 65535.0f);
		c.g = (uint16_t) int(g * 65535.0f);
		c.b = (uint16_t) int(b * 65535.0f);
		return c;
	}
	...
	/*! Calculates the chromacity of a black body radiator at a given temperature in kelvins.
		...
		\param intensity The intensity of the color to be generated (the luminance Y of the CIE XYZ color from which the
	   RGB color is converted).
	   \return An RGB value, converted from a CIE XYZ color with the calculated chromacity and
	   provided intensity.
	   \remark The returned color components is guaranteed to have positive values, but may
	   exceed 1.0.
	   */
	static GEOM_EXPORT Color FromKelvinTemperature(const float kelvin_temperature, const float intensity = 1.0f);
	/*! Finds the kelvin temperature which, when converted to RGB, would generate the closest possible color to the
	   current RGB value. For a color that was generated from a kelvin temperature, the returned value should match that
	   temperature exactly. But for an arbitrary color, the returned value will only be an approximation (and may vary
	   wildly if the color is very far from the set of valid black body colors).
	   \return The temperature, in kelvins, which would generate the closest matching color to 'this'.
	   \remark This performs a search through the range of valid color temperatures, and may therefore be a bit
	   expensive.
	   */
	GEOM_EXPORT float FindClosestMatchingKelvinTemperature() const;
	//!@}
};
...
b=2. */
GEOM_EXPORT int MaxComponent(const Color&); // index of the component with the maximum abs value
...
b=2. */
GEOM_EXPORT int MinComponent(const Color&); // index of the component with the minimum abs value
...
GEOM_EXPORT float MaxVal(const Color&); // value of the component with the maximum abs value
/*! \remarks The value of the component with the minimum absolute value.
\par Parameters:
<b>const Color\&</b>\n\n
The color to check.
\return */
GEOM_EXPORT float MinVal(const Color&); // value of the component with the minimum abs value
...
struct LogLUV32Pixel
{
	...
	*/
	GEOM_EXPORT void GetRGB(float rgb[3]) const;
	/*! \remarks Sets the RGB space values.
	\par Parameters:
	<b>const float rgb[3]</b>\n
	The values to set.
	*/
	GEOM_EXPORT void SetRGB(const float rgb[3]);
	...
	The results are stored here.
	*/
	GEOM_EXPORT void GetXYZ(float xyz[3]) const;
	/*! \remarks Sets the XYZ space values.
	\par Parameters:
	<b>const float xyz[3]</b>\n
	The values to set.
	*/
	GEOM_EXPORT void SetXYZ(const float xyz[3]);
	...
	*/
	GEOM_EXPORT static void XYZtoRGB(const float xyz[3], float rgb[3]);
	...
	The output values are stored here.
	*/
	GEOM_EXPORT static void RGBtoXYZ(const float rgb[3], float xyz[3]);
};
