...
/*! This stores specified normal information for a given
...
public:
	...
	/*!   \remarks Identical to the Insert function, with the exception that
    the new normal entries for vertices [pos, pos + num) are initialized to
    be unspecified, rather than retaining the values from the vertices shifted
    to positions [pos + num, deg), where deg is the degree of the face after
    insertion.
	\par Parameters:
	<b>int pos</b>\n\n
	The location within the face where the new normals will be added.\n\n
...
\remarks This interface is not available when you have used the NURMS conversion from Editable Poly 
as it computes normals, so any user specified normals will be erased.\n\n
...
public:
	...
	/*!   \remarks Returns true if there are any explicit normals set
	  */
    ...
    /*!
    Adjusts the number and topology of the normal faces to match the parent
    mesh. For new faces added, or if doCheckDegree is true, for any faces
    encountered whose degree does not match the corresponding face of the
    parent mesh, all normals are set unspecified. If the parent mesh pointer is
    not set, does nothing.
    \param [in] doCheckDegree: when true, the degree of each already-created
                               normal face is checked against the corresponding
                               face in the parent mesh
    */ 
    ...
    faceIndexMap[f] >= 0, the target face faceIndexMap[f] of this object is
    defined as a copy of source face f of object sourceNormals, which may be
    the same as this object. In particular, face faceIndexMap[f] is allocated
    if necessary, set to the same degree as the source face, and any vertex
    normals which are set as specified for the source face are copied to the
    corresponding vertices of the target face. The explicitness of normals is
    also preserved in the copy. Note that the operation will fail if
    sourceNormals is the same as this object, and the index map contains
    undefined copies, i.e. if 0 <= faceIndexMap[f] < faceIndexMap.Count() for
    any f, and faceIndexMap[faceIndexMap[f]] >= 0.
