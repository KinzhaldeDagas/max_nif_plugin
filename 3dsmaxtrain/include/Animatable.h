...
class Animatable : public InterfaceServer, public MaxSDK::Util::Noncopyable
{
	...
	public:	
		...
		//! \brief A callback class for acquiring/releasing a FlagBit.
		/*! This class is a RAII object for acquiring/releasing a FlagBit 
		\sa TestFlagBit, SetFlagBit, ClearFlagBit*/
		class FlagBitHelper : public MaxHeapOperators
		{
			int mFlagBit = 0;
		...
		public:
			FlagBitHelper()
			{
				mFlagBit = RequestFlagBit();
				Animatable::ClearFlagBitInAllAnimatables(mFlagBit);
			}
			~FlagBitHelper()
			{
				ReleaseFlagBit(mFlagBit);
			}
			int FlagBit()
			{
				return mFlagBit;
	...
	private:
		...
		//! \brief Optimized version of IsTreeEntryOpen()
		/*! IsTreeEntryOpen() calls BypassTreeView(), which is an expensive function.
		If you have previously computed the bool value, you can provide it here so you wont
		pay the cost of calling BypassTreeView() again.
		...
		\param bypass_tree_view This is the result of BypassTreeView(), which you have
		previously computed.
		*/
		int IsTreeEntryOpenCached(int type, DWORD tv, bool bypass_tree_view) const;
...
/// @{
		...
		/*! This name is used in 3ds Max's UI and in MAXScript.
			 ...
			 \param localized If true, then the class name returned should be localized in the language 3ds Max is currently using. Otherwise it should be the class name in English.
									If a plugin does not provide localized string resources, it can disregard this parameter and always return the class name in English.
		*/
		...
		//! Returns the name of the plugin class. \see void GetClassName(MSTR& s, bool localized)
		/*! \param localized If true, then the class name returned should be localized in the language 3ds Max is currently using. Otherwise it should be the class name in English.
									If a plugin does not provide localized string resources, it can disregard this parameter and always return the class name in English.
		*/
		MSTR ClassName(bool localized = true) const { MSTR s; GetClassName(s, localized); return s;}
		...
		*/
		...
		\sa Class ClassID, class ID definitions in plugapi.h.
		*/
...
/// @}
		...
		*/
		...
		//! \brief Get the unique handle for the next Animatable object to be created
		/*! Every Animatable is given a handle value when allocated.
		This method returns the handle value that will be used for the next animatable created. 
		This method is primarily intended for testing purposes, in particular for detecting if
		calling ClassDesc::Create() is creating an Animatable-derived object.
		\return The unique handle value for the next Animatable created*/
		CoreExport static AnimHandle GetNextHandle();

		//! \brief Get the number of existing Animatable objects
		CoreExport static DWORD GetNumAnimatables();

		//! \brief Get a list of existing Animatable objects
		...
		//! \brief Clears one or more bits in the Animatable flags in all Animatables
		/*! \param mask The bits to turn off in the Animatable flags*/
		CoreExport static void ClearAFlagInAllAnimatables(DWORD mask);
		...
		//! \brief Clears one or more bits in the Animatable extended flags in all Animatables
		//! \param mask The bits to turn off in the Animatable extended flags
		CoreExport static void ClearAFlagExInAllAnimatables(DWORD mask);
		...
		After calling this method call ClearFlagBitInAllAnimatables to clear the flag in
		all Animatables before starting to use the flag bit.
...
//! @{
		...
		\param localized If true, then the sub-anim name returned should be localized in the language 3ds Max is currently using. Otherwise it should be the sub-anim name in English.
		If a plugin does not provide localized string resources, it can disregard this parameter and always return the sub-anim name in English.
		\return  The name of the 'i-th' parameter. */
		CoreExport virtual MSTR SubAnimName(int i, bool localized) { return _T("no name"); };
//! @}
...
/// @{
		...
		*/
		...
		/*! \remarks Implemented by the System.\n\n
		This method removes and deletes all note tracks on object. */
...
/// @{
		// A driver controller should implement this method to give the 
		// MAX a list of nodes that are part of the system. 
		/*! \remarks The driver controller of a system plug-in should implement this method
		to give 3ds Max a list of nodes that are part of the system. The driver
		...
		driver controller of a system, and should return a list of pointers to
		all nodes that are part of the system. <b>GetInterface()</b> should be
		implemented for the driven TM controllers of the system and return a
		pointer to the driver controller.\n\n
		3ds Max will use <b>GetInterface()</b> in the TM controller of each
		selected node to retrieve the driver controller and then call
		<b>GetSystemNodes()</b> on the driver controller to get the list of
		nodes.
		...
		*/
		...
		SvGraphNodeReference Animatable::SvStdTraverseAnimGraph(IGraphObjectManager *gom, Animatable *owner, int id, DWORD flags)
		{
			...
			if (nodeRef.stat == SVT_PROCEED) {
				int numSubs = NumSubs();
				for (i = 0; i < numSubs; i++) {
					if (Animatable* subAnim = SubAnim(i)) {
						childNodeRef = subAnim->SvTraverseAnimGraph(gom, this, i, flags);
