...
public:
	...
	MSTR nonLocalizedClassName;		//! The non-localized class name
	...
	MSTR internalName;	//! The scripter exposed non-localized internal name
	const MCHAR* internalNamePtr;
	bool useOnlyInternalNameForMAXScriptExposure;	//! Whether to use only the internalName for scripter exposure
	...
	CoreExport virtual const MCHAR* NonLocalizedClassName();
	...
	CoreExport virtual bool UseOnlyInternalNameForMAXScriptExposure();
	...
	CoreExport virtual bool SetUserDlgProc(ParamBlockDesc2* pbd, ParamMap2UserDlgProc* proc=NULL);
