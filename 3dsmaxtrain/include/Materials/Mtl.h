...
public:
	...
	virtual void GetClassName(MSTR& s, bool localized = true) const override { UNUSED_PARAM(localized); s = MSTR(_M("Mtl")); }
...
//! @{
	...
	\param localized If true, then the slot name returned should be localized in the language 3ds Max is currently using. Otherwise it should be the slot name in English.
						  If a plugin does not provide localized string resources, it can disregard this parameter and always return the slot name in English. */
	CoreExport virtual MSTR GetSubMtlSlotName(int i, bool localized);

	/*! \remarks Returns the name of the 'i-th' sub-material that should appear in track view.
	\param i The index of the sub-material whose track view name should be returned.
	\param localized If true, then the sub-material TV name returned should be localized in the language 3ds Max is currently using. Otherwise it should be the sub-material TV name in English.
			 If a plugin does not provide localized string resources, it can disregard this parameter and always return the sub-material TV name in English. */
...
//! @{				
	...
	// return TRUE to prevent the Keep Old Material dialog from popping.
...
//! @{
	...
	//! \param sc This describes the context in which the material should be evaluated.
	//! \param stdID The ID of the channel to perform evaluation on. 
	//! \param outVal The result of the evaluation.
