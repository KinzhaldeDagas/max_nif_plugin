...
public:
	...
	<b>bool localized</b>\n\n
	If true, then the slot name returned should be localized in the language 3ds Max is currently using. Otherwise it should be the slot name in English.
	...
	CoreExport virtual MSTR GetSubTexmapSlotName(int i, bool localized);

	/*! \remarks Returns the name to appear in Track View of the 'i-th' sub-texmap.
	\par Parameters:
	<b>int i</b>\n\n
	Specifies the sub-texmap whose track view name is returned.
	<b>bool localized</b>\n\n
	If true, then the sub-texmap TV name returned should be localized in the language 3ds Max is currently using. Otherwise it should be the sub-texmap TV name in English.
	If a plugin does not provide localized string resources, it can disregard this parameter and always return the sub-texmap TV name in English. */
