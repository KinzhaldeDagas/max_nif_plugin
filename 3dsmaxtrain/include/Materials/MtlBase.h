...
//! @{			
			...
			CoreExport virtual MSTR GetFullName(bool localized);
//! @}
...
//! @{			
			...
			void Update(TimeValue t, Interval&& valid = FOREVER) { Update(t, valid); }
...
//! @{			
			...
			makes the first of them a 'driver' and adds the rest as secondary
			ParamDlgs. The texmap then asks the UVGen and TextureOutput objects to
			CreateParamDlg() for their rollouts and adds them to the driver
			IAutoMParamDlg also.\n\n
			...
			'driver' ParamDlg to switch into the UI a texmap of the same class as
			that currently in the UI. Normally, the driver IAutoParamDlg would
