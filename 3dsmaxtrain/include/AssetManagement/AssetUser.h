	...
	namespace AssetManagement
	{
		...
		public:
			...
			*/
			...
			//! \brief Modifies the capitalization of the asset's filename.
			//! This method is used when writing the file associated with the asset to disk to ensure
			//! that the capitalization of the asset's filename is the same as that of an existing file
			//! that has the same filename but with a different capitalization.
			//! \param filename - The filename with the new capitalization.
			//! It must only differ from the asset's current filename in its capitalization.
			//! If it is different in any other way, the asset's filename is not modified.
			//! \return True if the capitalization of the asset's filename was modified. False otherwise.
			CoreExport bool ModifyCapitalizationOfFileName(const MSTR& filename) const;
