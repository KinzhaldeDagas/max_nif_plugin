...
/*! \sa  Class ColorPicker, \ref Color_Conversion_Utilities 
\par Description:
...
extern CoreExport INT_PTR HSVDlg_Do(
							 ...
							 const MCHAR* name			// name of color being edited
							 );
...
//! \defgroup Color_Conversion_Utilities Color Conversion Utilities
//! @{
/*! \remarks Converts the specified color in the RGB (red, green, blue) format to the HSV 
(hue, saturation, value) format. */
CoreExport void RGBtoHSV (DWORD rgb, int *ho, int *so, int *vo);
/*! \remarks Converts the specified color in the HSV (hue, saturation, value) format to the RGB 
(red, green, blue) format. */
CoreExport DWORD HSVtoRGB (int H, int S, int V);
/*! \remarks Converts the specified color in the HSV (hue, saturation, value) format to the HWBt 
(hue, whiteness, blackness) format. */
CoreExport void HSVtoHWBt (int h, int s, int v, int *ho, int *w, int *bt);
/*! \remarks Converts the specified color in the HWBt (hue, whiteness, blackness) format to the HSV 
(hue, saturation, value) format. */
CoreExport void HWBttoHSV (int h, int w, int bt, int *ho, int *s, int *v);
//! @}
// RB: Added floating point versions
class Color;
//! \addtogroup Color_Conversion_Utilities
//! @{
/*! \remarks Converts the specified color in the RGB (red, green, blue) format to the HSV (hue, saturation, value) format. */
CoreExport Color RGBtoHSV(Color rgb);
/*! \remarks Converts the specified color in the HSV (hue, saturation, value) format to the RGB (red, green, blue) format. */
CoreExport Color HSVtoRGB(Color hsv);
//! @}

// MODELESS Version

/*! \sa  Class HSVCallback, <a href="https://msdn.microsoft.com/en-us/library/windows/desktop/dd183449(v=vs.85).aspx">COLORREF - DWORD</a>, Class IPoint2.\n\n
\par Description:
...
class ColorPicker : public InterfaceServer {
	...
	public:
		...
		virtual void InstallNewCB(DWORD col, HSVCallback *pcb, const MCHAR *name, ColorContext context=ColorContext::COLOR_SPACE_CONTEXT_NONE)=0;
		...
		<b>MCHAR *name</b>\n\n
		The new title string. */
		virtual void InstallNewCB(AColor col, HSVCallback *pcb, const MCHAR *name, ColorContext context=ColorContext::COLOR_SPACE_CONTEXT_NONE){}
...
/*! \remarks Call this function to bring up the modeless color picker. \par
Parameters:  
<b>HWND hwndOwner</b>\n\n  
...
<b>IPoint2* spos</b>\n\n  
The initial screen position of the upper left corner. NULL may be passed for the
default location.\n\n  
...
be used.  
\return A pointer to a ColorPicker object. Returned object is owned by the
system and pointer may become dangling after the HSVCallback::BeingDestroyed()
called on the passed pcallback object. */
...
* if lParam is not null, then it's a pointer to AColor holding the scene-referred
* color (new behavior).
*/
...
/*! \sa  Class ColorPicker, Class HSVCallback, Class Class_ID, Class IPoint2, <a href="https://msdn.microsoft.com/en-us/library/dd183449(v=vs.85).aspx">DWORD--COLORREF Color Format</a>..\n\n
\par Description:
...
page of the Preferences dialog. The chosen picker will be called whenever a
...
class ColPick : public InterfaceServer {
	...
	<b>DWORD *lpc</b>\n\n
	A pointer to the color to be edited. See <a href="https://msdn.microsoft.com/en-us/library/dd183449(v=vs.85).aspx">DWORD
	COLORREF Format</a>.\n\n
	<b>IPoint2 *spos</b>\n\n
	...
	/*! \remarks This method is called to create and return a ColorPicker object
	for the modeless color picker. 
	
	Parameters:  
	<b>HWND hwndOwner</b>\n\n  
	The owning window handle.\n\n  
	<b>DWORD initColor</b>\n\n  
	The inital value of the color. See <a
	href="https://msdn.microsoft.com/en-us/library/dd183449(v=vs.85).aspx">DWORD
	COLORREF Format</a>.\n\n  
	<b>IPoint2* spos</b>\n\n  
	...
	\return The returned object is owned by the system and will be deleted when the
	user closes the picker window. pcallback's BeingDestroyed() function will be
	...
	*/
	...
	virtual	ColorPicker *CreateColorPicker(
		...
		HSVCallback *pcallback, // call back
		const MCHAR *name, 	  // name of color being edited
	...
	This indicates the color picker is being used for the object color in the
	command panel, and the color picker then displays the Add Color button.   
	\return The returned object is owned by the system and will be deleted when
	the user closes the picker window. pcallback's BeingDestroyed() function
	will be called before the object is deleted. */
	virtual	ColorPicker *CreateColorPicker(
		HWND hwndOwner,   // owning window
		AColor initColor,  // initial value of color
		IPoint2* spos,    // starting position of dialog
		HSVCallback *pcallback, // call back 
		const MCHAR *name, 	  // name of color being edited
		BOOL isObjectColor=FALSE,
