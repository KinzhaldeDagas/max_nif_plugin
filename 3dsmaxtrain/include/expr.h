...
public:
	...
	{
		return name == b.name && type == b.type && regNum == b.regNum;
	}
};
...
\snippet samples\maxscript\mxsagni\testonly.cpp TestExpr Example
...
public:
	...
	*/
	const MCHAR *				getExprStr(void)	{ return origStr.data(); }
	...
	the error occurred. */
	const MCHAR *				getProgressStr(void){ return progressStr.data(); }
	...
	DllExport const MCHAR *	getVarName(int type, int i);
...
// pseudo-private: (only to be used by the "instruction" functions
	...
	MaxSDK::Array<ExprVar>	vars;	// named variables
private:
	const MCHAR *		exprPtr;		// pointer to current str pos during parsing
	const MCHAR *		exprStr;		// ptr to original expression string to parse
