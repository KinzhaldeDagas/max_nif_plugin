	...
	public GeomObject
{
	...
	public:
		...
		*/
		...
		void GetClassName(MSTR& s, bool localized = true) const override { UNUSED_PARAM(localized); s = MSTR(_M("TriObject")); }
		...
		const MCHAR *GetObjectName(bool /*localized*/) const override { return _M("Mesh"); }

		CoreExport void RescaleWorldUnits(float f);

		// IO
		CoreExport IOResult Save(ISave *isave) { return Save(isave, ALL_CHANNELS); }
		CoreExport IOResult Save(ISave *isave, ChannelMask channels);
		CoreExport IOResult Load(ILoad *iload) { return Load(iload, ALL_CHANNELS); }
	...
	private:
		void DeformV2(const Deformer* defProc, int useSel);
	};
...
/*
This XTC Object is added to TriObject and PolyObjects that have the NormalSpec interface added to them.
It was designed to clear the normal spec if a modifier did not set the MESH_NORMAL_MODIFIER_SUPPORT
on the specs interface inside the modifier ModifyObject call.  If this was not set the NormalSpec would
be cleared in the PostChanChanged.  The idea was to keeps the normals in sync for instance if a modifier
changed the topology but did not change the topology of the NormalSpec this could introduce a crash.

There was a huge performance penalty though.  The original XTCObject DependsOn TOPO_CHANNEL | GEOM_CHANNEL
which means the stack has to copy the topology of the mesh/mnmesh since it might be cleared which is a
huge performance loss.

This is now changes the DependsOn to TOPO_CHANNEL only now.  So only modifiers that change topology will have 
their NormalSpec cleared if the flag is not set.  This means that unsupported modifiers may generate normals
that do not look right if they only change GEOM_CHANNEL.  These modifiers will need to be fixed.  
Note since a vast magority of modifiers that only do GEOM_CHANNEL changes do it thru the deformer 
interface which does set the flag so they wont be affected other than have a performance boost.
...
public:
	...
	CoreExport XTCObject* Clone() override;
	...
	CoreExport void PreChanChangedNotify(
			TimeValue t, ModContext& mc, ObjectState* os, INode* node, Modifier* mod, bool bEndOfPipeline) override;
	CoreExport void PostChanChangedNotify(
			TimeValue t, ModContext& mc, ObjectState* os, INode* node, Modifier* mod, bool bEndOfPipeline) override;
