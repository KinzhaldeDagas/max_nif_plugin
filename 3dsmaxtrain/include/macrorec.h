...
public:
	...
	virtual void OperandSequence(int count, ...) = 0;
	...
	/** Super enable and option controls. In the Scripting Listener Window Macro 
	...
	/** In the Scripting Listener Window Macro Recorder pull-down menu
...
enum {	mr_int,				//!< An integer. Follow the tag with the int (Basic C type). Example: <pre>91462</pre>
		...
		mr_funcall,			//!< A function call. Follow the tag with the function name, then int, int, \<args\>. The first int parameter is the number of positional arguments, 
							...
							//!< given as MCHAR*, \<arg\> pairs. (MAXScript type). Example: 
		...
		mr_varname,			//!< A variable name. Follow the tag with a string (MCHAR*). (MAXScript type). Example: <pre>_T("baz")</pre>
		mr_create,			//!< A constructor call for the class. Follow the tag with: Class_ID, SClass_ID, int, \<args\>. The int parameter is the number of keyword args 
							//!< following, these are specified in pairs as MCHAR*, \<arg\>. (MAX SDK type). Example: <pre>mr_create, Class_ID(123,456), SClass_ID(12345), 0 ...</pre>
		...
		mr_index, 			//!< An index. Follow the tag with: \<op_arg\>, \<index_arg\>. \<op_arg\> is an \<arg\> specifying the operand to be indexed and \<index_arg\> 
							...
							//!< mr_funcall, _T("copy"), 1, 0, mr_reftarg, oldm);</pre>
		mr_prop,			//!< A property name. Follow the tag with MCHAR*, \<arg\>. The MCHAR* is the property name, the \<arg\> specifies the item being accessed. 
							...
							/** A name. Follow the tag with a string (MCHAR *).   The string is emitted preceeded by a "#" to form a
...
public:
	/** Constructor. Disables the macro recorder automatically.
	 * \param disableNow if true, immediately disable macro recorder. */
	MacroRecorderDisable(bool disableNow = true) : m_doEnable(false) {
		if(disableNow)
			Disable();
	}

	/** Destructor. Enables the macro recorder automatically if it was disabled. */
	...
	void Disable()
	{
		DbgAssert(!m_doEnable);
		MacroRecorder* recorder = macroRec;
		if(recorder != NULL && !m_doEnable)
		{
			recorder->Disable();
			m_doEnable = true;
		}
