...
//!@{
/*! Parameter space based on segments. This simple interpolation is based on parameter space. If a spline has four segments, the first segment is parameter values 0-0.25, the second is 0.25-0.5, the third is 0.5-0.75, and the fourth is 0.75-1.0. This is regardless of the length of each segment. */
...
class SplinePoint: public MaxHeapOperators {
	...
	public:
		...
		CoreExport SplinePoint(const Point3 &p, int a1 = -1, int a2 = -1, int a3 = -1);
		CoreExport SplinePoint& operator=(const SplinePoint &fromSP);
		inline Point3& GetPoint() { return point; }
		inline int GetAux(int index) const { return aux[index]; }
...
/** Developer-defined knot flags
 * Flag bits 8-15 are reserved for use by developers.  These bits should only be used
 * temporarily, since other developers may also use them for other purposes. */
#define SPLINEKNOT_USER1 (1 << 8)	// Start of developer flag bits
...
#define SPLINEKNOT_USER8 (1 << 15)	// End of developer flag bits
...
class SplineKnotAssy: public MaxHeapOperators {
	...
	public:
		...
		inline	int		Ktype() const { return ktype; }
		inline	void	SetKtype(int t) { ktype=t; }
		inline	int		Ltype() const { return ltype; }
		inline	void	SetLtype(int t) { ltype=t; }
		inline	Point3	Knot() const { return knot.point; }
		inline	void	SetKnot(const Point3 &p) { knot.point=p; }
		inline	Point3	InVec() const { return inVec.point; }
		inline	void	SetInVec(const Point3 &p) { inVec.point=p; }
		inline	Point3	OutVec() const { return outVec.point; }
		inline	void	SetOutVec(const Point3 &p) { outVec.point=p; }
		inline	float	GetParm() const { return du; }
		inline	void	SetParm(float p) { du = p; }
		inline	MtlID	GetMatID() const {return (int)((flags>>SPLINE_MATID_SHIFT)&SPLINE_MATID_MASK);}
		...
		/** Allow access as if the in/knot/out components are contained vertices.  
		 * \param index 0=inVec 1=knot 2=outVec  
		 * \param p 0=aux1 1=aux2 2=aux3*/
		CoreExport void SetVert(int index, const Point3 &p);

		inline	SplinePoint GetKnot() const { return knot; }
		inline	SplinePoint GetInVec() const { return inVec; }
		inline	SplinePoint GetOutVec() const { return outVec; }
...
public:
	...
	inline	int		Aux() const { return aux; }
	...
	inline	Point3	InVec() const { return inVec; }
	...
	inline	Point3	OutVec() const { return outVec; }
...
// Used to stop recursion in ComputeBezPoints.
#define SPLINE_COMPUTE_BEZ_IN_USE   (1<<2)
//!@}

/** Beginning with 2024.1, the following special values may be supplied to SetKnotType as the
'index' value in order to set the Spline3D's 'initialType' and 'dragType' knot type values.
These are used in the Create Line and Insert operations for all new knots in Edit and Editable
Spline, and were previously only settable in the Spline3D constructor. */
#define SPLINE_INITIAL_TYPE_INDEX -1    //!< Knot type at initial click.
#define SPLINE_DRAG_TYPE_INDEX    -2    //!< Knot type at drag.
 ...
 * \see SplineKnot, PolyLine*/
	class Spline3D
			...
			, private CountedObject<Spline3D>
	{
...
private:
			...
			int			Interact(ViewExp *vpt,int msg, int point, int vflags, IPoint2 m, Matrix3* mat, IObjParam* ip=NULL );	// Handle mouse interaction
			float		cachedLength;
			BOOL		cacheValid;
...
public:
	...
	inline			int			ParmType() const { return parmType; };

	/** Returns the Knot (point) count. */
	inline			int			KnotCount() const { return knotCount; }
	...
	inline			int			Closed() const { return (flags & SPLINE_CLOSED) ? 1:0; }
	 ...
	 */
	CoreExport		int			AddKnot(const SplineKnot &k,int where = -1);					// Add a knot to the spline
	...
	CoreExport		int			Create(ViewExp *vpt,int msg, int point, int vflags, IPoint2 m, Matrix3* mat, IObjParam *ip=NULL);	// Create the spline
	
	/** Used internally by the free form line object
	 * SPLINE.CPP. See the sample code in
	 * /MAXSDK/SAMPLES/OBJECTS/SPLINE.CPP for an example of this method in
	 * use. */
	CoreExport		int			StartInsert(ViewExp *vpt,int msg, int point, int vflags, IPoint2 theP, Matrix3* mat, int where );	// Start an insertion operation on the spline
	
	/** \internal This method is used internally. */
	CoreExport		int			SetParam(int index,float param);		// Set custom param value
	
	/** \internal This method is used internally. */
	CoreExport		float		GetParam(int index) const;					// Get param value
	
	/** Returns the knot type from the specified knot. 
	 * \param index The index of the knot type to return. 
	 * \see Knot_types*/	
	inline			int			GetKnotType(int index) const { return knots[index].ktype; }
	
	/** Sets the knot type of the specified knot or, if one of the special indices is specified,
	sets the knot type used on initial click or when the user clicks and drags the mouse.
	 * \param index The index of the knot to be set. If SPLINE_INITIAL_TYPE_INDEX, sets the knot
	 type applied at the initial mouse click when creating or inserting (see private member
	 'initialType'). If SPLINE_DRAG_TYPE_INDEX, sets the knot type used when the user clicks
	 and drags (see private member 'dragType').
	...
	inline			int			GetLineType(int index) const { return knots[index].ltype; }
	 ...
	 * \param segment The index of the segment to interpolate.
	 * \param t A value in the range of 0.0 to 1.0. 0 is the first knot and 1 is the second knot.
	 * \param normParam The effective parameter value derived by travelling along the segment to value t.
	 * \return  The interpolated point. */
	...
	/** \internal This method is used internally. */
	CoreExport		int			DrawPhase() const { return drawPhase; }
	
	/** \internal This method is used internally. */
	CoreExport		int			GetiCur() const { return iCur; }
	...
	/**	Get the user flags for a specified knot.  \ref SPLINEKNOT_USER1 through \ref SPLINEKNOT_USER8
	* \param i	Specifies the point to get flags for
	* \param fl	The DWORD mask of the flag(s) to get */
	CoreExport		BOOL		GetKnotUserFlag(int i, DWORD fl) const;

	/**	Set the user flags for a specified knot.  \ref SPLINEKNOT_USER1 through \ref SPLINEKNOT_USER8
	* \param i	Specifies the point to set flags for
	* \param fl	The DWORD mask of the flag(s) to set
	* \param val TRUE to set the flag, FALSE to clear the flag */
	CoreExport		void		SetKnotUserFlag(int i, DWORD fl, BOOL val = TRUE);

	/**	Clear the user flags for a specified knot.  \ref SPLINEKNOT_USER1 through \ref SPLINEKNOT_USER8
	* \param i	Specifies the point to clear flags for
	* \param fl	The DWORD mask of the flag(s) to clear */
	...
	knots times 3. */	
	inline			int			Verts() const { return knotCount*3; }
	 ...
	 * \param spline The spline to prepend. 
	 * \param weldCoincidentLastVertex Defaults to TRUE. Set this to TRUE to 
	 * weld coincident last vertices. Setting it to FALSE will disable welding.
	Returns TRUE if first point auto-welded	
...
* If the segment itself is LTYPE_LINEAR, both knots of the segment are KTYPE_CORNER,
* have zero-length Bezier vectors, or the Bezier vectors of both segment knots point
* directly at the other knot (or any combination of the above), the segment is
* considered to be effectively linear. 'linearTolerance' is a tolerance value that
* is used to determine if the Bezier vectors point directly at the other knot. Given
* a knot point K1 with its bezier vector KV and another knot point K2, if the value:
* Length(Normalize(KV-K1) - Normalize(K2-K1)) < linearTolerance, the vector is
* considered to be effectively linear.
* \param spline The spline to check.
* \param segment The segment index to check.
* \param linearTolerance A tolerance value that is used to determine if the Bezier
* vectors point directly at the other knot. Given a knot point K1 with its Bezier
* vector KV and another knot point K2, if the value:
* Length(Normalize(KV-K1) - Normalize(K2-K1)) < linearTolerance, the vector is
* considered to be effectively linear.
* \return true if the segment is effectively linear; false otherwise.*/

extern CoreExport bool IsSplineSegmentEffectivelyLinear(Spline3D* spline, int segment, float linearTolerance = 0.01f);

// Shape Angle Constraints

/**	This function retrieves the "Enable Shape Angle Constraints" option
* \return true if the Shape Angle Constraints are enabled */

extern CoreExport bool GetEnableShapeAngleConstraints();

/**	This function sets the "Enable Shape Angle Constraints" option
* \param sw The new switch setting. When set to 'true', shape creation will use the Shape Angle Constraint value. */

extern CoreExport void SetEnableShapeAngleConstraints(bool sw);

/**	This function retrieves the "Shape Angle Constraint" value
* \return The shape angle constraint, in degrees */

extern CoreExport float GetShapeAngleConstraint();

/**	This function sets the "Shape Angle Constraint" value
* \param value The shape angle constraint, in degrees. Must be greater than zero.
* \return true if the value was successfully set; false otherwise. */

extern CoreExport bool SetShapeAngleConstraint(float value);

/**	This function provides access to the "Shape Angle Constraint" display information
* This function is used internally to display Shape Angle Constraint angle values while
* editing a spline.
* \param angleOut (out) The angle to be displayed, in degrees.
* \param positionOut (out) The GraphicsWindow coordinates where the angle text is to appear.
* \return true if the angle is to be displayed; false otherwise. */

extern CoreExport bool GetShapeAngleDisplayInfo(float& angleOut, IPoint2& positionOut);
