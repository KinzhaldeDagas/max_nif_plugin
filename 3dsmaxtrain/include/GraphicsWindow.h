...
// The result returned by pick only graphic window.
struct GfxPickResult
{
	int gotHit;
	DWORD hitEpsilon;
	DWORD hitDist;
	GfxPickResult() { gotHit = 0; hitEpsilon = 0xffffffff; hitDist = 0xffffffff; }
};
...
//! \name Driver/Configuration/Support Methods
//! @{
	...
	of these options, it may change in a given driver any time in the
	future.)\n\n
	...
	back-culled, but they display hidden edges as though they are visible). */
	virtual int		querySupport(int what) = 0;
//! @}

//! \name Window/Viewport/Transformations Methods
//! @{
...
//! \name Position/Size/Depth/Clipping Methods
//! @{
...
//! @{
	...
	TRUE, this returns the size of the texture that 3ds Max should send to the
	...
	display in viewport button is turned off, <b>freeTextureHandle()</b> is called.
	\par Parameters:
	<b>BITMAPINFO *bmi</b>\n\n
...
//! @{		
	...
	viewport matrix. This routine should be called any time the graphics window
	...
	/*! \remarks	This method is used to set up a virtual viewport. Note that this is a no-op
	...
	modelling coordinates to normalized projection coordinates matrix. This
	routine also back-transforms each light and the eye point (so that lighting
	can be done in modelling coordinates).\n\n
...
//! @{
	/*! \remarks
	Call to begin or end a pass when doing multiple passes for the purpose of drawing
	wireframe edges over solid geometry. 
	\param pass - indicates which pass:
	  - 0 - solid geometry pass
	  - 1 - wireframe pass
	  - -1 - selected faces pass
	\param onOff - TRUE indicates the start of a pass, FALSE indicates the end of a pass
	\param scaleFact - not used
	*/
...
//! @{
	...
	~{ Rendering Modes and Rendering Limits }~. You
...
//! \name Coordinate Transformation Methods
//! @{
    ...
    /*! \remarks	This method converts coordinates to "<b>h</b>" format device coordinates,
    without integer rounding. Note: This method maps points from the
    GraphicsWindow's current transform to device space. If the GraphicsWindow's
    transform is set to the identity matrix then the mapping is done from
    points specified in world space. Otherwise the points given are transformed
    by the GraphicsWindow transform, and are <b>then</b> considered to be in
    world space. Thus, to get a world-space to screen-space conversion, you
    need to set the transform to the identity with
    <b>gw-\>setTransform(Matrix3(1))</b>.
    \par Parameters:
    <b>const Point3& in</b>\n\n
    The input point.\n\n
    <b>Point3& out</b>\n\n
    The output point in floating point values in the native device coords for
    the device. For HEIDI and OpenGL the origin at the lower left. For Direct3D
    the origin is at the upper left.
    \return DWORD containing the clipping flags for the point. If a flag is
    set it indicates the transformed point lies outside the view volume. See
    \ref View_Volume_Clip_Flags. */
    virtual DWORD hTransPoint(const Point3& in, Point3& out) = 0;
	...
	/*! \remarks	This method is used to convert coordinates to "<b>w</b>" <b>floating
 ...
 * Calls with a "w" in front take Windows device coordinates with the origin at the upper left.
 * Note that these "h" and "w" routines perform no clipping unless otherwise noted
 * (clipping at this level will be very expensive).
...
///@{
	...
	/*! \remarks	Gets the unscaled size of text displayed with wText/hText.
	Note : Only implemented for OGS viewport. Returns 11 on other graphics windows.
	\return  The unscaled point size in points. */
	virtual float GetTextPointSize() const = 0;

	/*! \remarks	Sets the unscaled size of text displayed with wText/hText, in points.
	The dpi scale will be applied internally.
	Note : Only implemented for OGS viewport.
	\par Parameters:
	<b>float point_size</b>\n\n
	This is the unscaled point size\n\n
	*/
	virtual void	SetTextPointSize(float /*point_size*/) = 0;

	/*! \remarks	Gets the width factor of the viewport text.
	Between 0.1 and 10.
	Note : Only implemented for OGS viewport. Returns 1 on other graphics windows.
	\return  The width factor (percentage). */
	virtual float GetTextWidthFactor() const = 0;

	/*! \remarks	Sets the width factor of the viewport text.
	Between 0.1 and 10.
	Note : Only implemented for OGS viewport.
	\par Parameters:
	<b>float factor</b>\n\n
	...
	*/
	virtual void	SetTextWidthFactor(float /*factor*/) = 0;

	/*! \remarks	Gets the weight viewport text.
	Note : Only implemented for OGS viewport. Returns 400 on other graphics windows.
	DONTCARE = 0,
	THIN = 100,
	EXTRALIGHT = 200,
	ULTRALIGHT = 200,
	...
	/*! \remarks	Sets the weight of wText/hText.
	Note : Only implemented for OGS viewport.
	DONTCARE = 0,
	THIN = 100,
	EXTRALIGHT = 200,
	...
	This is the weight\n\n
	*/
	virtual void	SetTextWeight(unsigned /*weight*/) = 0;
	...
	virtual void 	text(const Point3 *xyz, const MCHAR *s) = 0;
...
/** \name Hit Testing Methods
 * The following methods are used for hit testing.
 * Typically, these methods are used in the following sequence:\n
 * Set a hit region using setHitRegion() and then clear the hit code using clearHitCode().
 * Now, render a primitive using the mode GW_PICK and at whatever level you want to check at.
 * For example, if you are interested in vertex hit testing, you can render a series of markers
 * using a rendering mode that includes GW_PICK. The GW_PICK rendering mode will cause the system
 * to perform hit testing rather than actually render the item.\n
 * After each item is rendered, you can check to see if a hit was made using checkHitCode().
 * This method returns TRUE or FALSE based on if a hit was found.
 * If you are looking for just a single hit, you can abort as soon as checkHitCode() returns TRUE.
 * If you are interested in the closest hit, you can loop through all the items calling clearHitCode(),
 * checkHitCode(), and getHitDistance(). After checking all the items, you can simply choose the smallest hit
 * distance from all the items that you have checked.
 */
///@{
	...
	/*! \remarks These functions are avaliable for pick-only graphic window.  
	Note this function is expected to be reentrant with the pick-only graphic window provided by the system.
	*/
	DllExport virtual void hitTestHMarker(const IPoint3 *xyz, MarkerType type, GfxPickResult& res) const;
	DllExport virtual void hitTestMarker(const Point3 *xyz, MarkerType type, GfxPickResult& res) const;
	DllExport virtual void hitTestPolyline(int ct, const Point3 *xyz, int closed, int *es, GfxPickResult& res) const;
	DllExport virtual void hitTestPolylineN(int ct, const Point3 *xyz, int closed, int *es, GfxPickResult& res) const;
	DllExport virtual void hitTestHPolyline(int ct, const IPoint3 *xyz, int closed, int *es, GfxPickResult& res) const;
	DllExport virtual void hitTestPolygon(int ct, const Point3 *xyz, GfxPickResult& res) const;
	DllExport virtual void hitTestPolygonN(int ct, const Point3 *xyz, GfxPickResult& res) const;
	DllExport virtual void hitTestHPolygon(int ct, const IPoint3 *xyz, GfxPickResult& res) const;
///@}
