...
/*! \sa  Class ILoad, ~{ Reference System }~.\n\n
\par Description:
...
public:
	...
	execute. Lower priorities are run first-- allowable values {0..10}: 0 is reserved
	for ParamBlock2PLCB & ParamBlockPLCB.
	This method is overridden, for example, by the ParmBlock2
	PostLoadCallbacks to return 0 so it can execute before the others.
	...
	<b>{ return 5; }</b>
	\note Custom Attributes are added to their owner objects in a Priority 6 PostLoadCallback. Object Custom Attributes and
	object MetaData should be expected only in PostLoadCallbacks with a Priority of 7 or lower. Custom Attributes added in
	PostLoadCallbacks with a higher Priority may be replaced by the Priority 6 PostLoadCallback. 
	\note PostLoadCallbacks that could cause a node evaluation should have a Priority of 7 or lower, as in the evaluation attempts 
	to access a Custom Attribute may occur, for example in a scripted controller. If the PostLoadCallback is just doing something
	like data collection, it should have a Priority of 10.
	\note PostLoadCallbacks with a priority less than 0 will be treated as 0, greater than 10 will be treated as 10.
	*/
...
use: */
#pragma warning(push)
#pragma warning(disable : 4100 6326)
...
public:
	...
	{
		DbgAssert(buf == nullptr || strlen(buf) <= nbytes);
		return Write((void*)buf, nbytes, nwrit);
	}
	IOResult Write(const wchar_t* buf, ULONG nbytes, ULONG* nwrit)
	{
		DbgAssert(buf == nullptr || wcslen(buf) <= nbytes);
		return Write((void*)buf, nbytes, nwrit);
	}
	...
	IOResult Write(const MaxSDK::MaxGuid* buf, ULONG nbytes, ULONG* nwrit)
	{
		return Write((void*)buf, nbytes, nwrit);
	}
...
public:
	...
	\param str The string to write.  The string must be ACP encoded.
	\return  <b>IO_OK</b> - The write was acceptable - no errors.\n\n
	...
	Plug-ins should be written to skip over unknown chunks.
	...
	\see GetSavingVersion(), \ref VersionMacros
	*/
	...
	\param saveAssetResolvedFileName Whether to store the asset's resolved filename.
	...
	// Disable/Enable chunk warnings.
	/*! \remarks This method is used to Disable/Enable chunk warnings. In 3ds Max 2026, if a BitArray instance
	* is saved to the scene file in its own chunk, a warning dialog is displayed. BitArray instances should
	* always be saved in their own chunk. This method is provided to temporarily disable the warning dialog to
	* support where current code is updated to save the BitArray in its own chunk, but Save To Previous requires 
	* that previous behavior also be used. 
	\param disable True to disable chunk warnings, false to re-enable chunk warnings.
	*/
	virtual void DisableChunkWarnings(bool disable) = 0;

	// Return whether chunk warnings are enabled.
...
// TODO: When the SDK breaks, remove this and add virtual method IsLoadingSceneFile()
/// \name ILoad::Execute() Command Values
/// These values may be passed as the \c cmd argument of the ILoad::Execute() function.
//!@{
#define I_EXEC_ILOAD_PERFORMING_SCENE_LOAD \
	1 //!< Execute method returns true if performing a scene file load, false otherwise (merge, mtl lib load, etc).
#define I_EXEC_ILOAD_REFMAP_COUNT \
	2 //!< Execute method returns largest valid value, plus 1, that can be passed to ILoad::GetAddr.
//!@}
...
public:
	...
	may be nullptr if the address is not available. See
	...
	// If GetAddr() returns nullptr, then call this to get the address
	...
	/*! \remarks Number of data bytes remaining in chunk NOT including header. */
	virtual ULONGLONG CurChunkLengthRemaining() = 0; // number of data bytes remaining in chunk NOT including header
	...
	\param nwrit The number of bytes that were read.
...
public:
	...
	*/
	...
	// following methods expose data that is loaded from the SaveConfigData stream that was added to 3ds Max 2012.
	// default value for mFile3dsMaxLanguageID, mFileUserLanguageID, mFileSaveAsVersion, mFileActiveCodePage if not
	// loaded from scene file
	static const DWORD kFileSaveAsVersionNotSpecified = 0;

	//! \brief the 3dsmax language id when the file was saved - kFileSaveAsVersionNotSpecified if not loaded from file
	virtual WORD File3dsMaxLanguageID()
	{
		return kFileSaveAsVersionNotSpecified;
	}
	//! \brief the user language id when the file was saved - kFileSaveAsVersionNotSpecified if not loaded from file
	virtual LANGID FileUserLanguageID()
	{
		return kFileSaveAsVersionNotSpecified;
	}
	//! \brief the 'Save As' version number when the file was saved - kFileSaveAsVersionNotSpecified if not loaded from
	//! file
	virtual DWORD FileSaveAsVersion()
	{
		return kFileSaveAsVersionNotSpecified;
	}
	//! \brief the Active Code Page when the file was saved - kFileSaveAsVersionNotSpecified if not loaded from file
