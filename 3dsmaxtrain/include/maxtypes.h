...
{ 
	// Using static_cast will round down the result of the multiplication which is not always what we want
	// Since the function is constexpr, we can't use any math function such as std::round (not before C++23)
	// Trick used : 3.2 + 0.5 = 3.8 and static_cast<TimeValue>(3.8) = 3 or 3.7 + 0.5 = 4.2 and static_cast<TimeValue>(4.2) = 4.0
	double result = secs * TIME_TICKSPERSEC;
	result += 0.5;
	return static_cast<TimeValue>(result);
}
...
// \brief Description of a file operation
// Used in NOTIFY_FILE_PRE_OPEN, NOTIFY_FILE_POST_OPEN
enum FileIOType
{
