...
class UtilExport CStr: public MaxHeapOperators {
	...
	public:
		...
		/*! \remarks Delete these two overloads to prevent accidental dangling pointers from temporary strings.
		* This will cause compile errors in cases like this: const MCHAR* dangle = MSTR(_T("Hello"));
		* Consider using .data() explicitly if the error is a false positive.
		* Although it is not recommended, you can define MSTR_DISABLE_DANGLING_CHECKS to disble this compiler check. */
#ifndef MSTR_DISABLE_DANGLING_CHECKS
		operator const char* () && = delete;
		operator const char* () const&& = delete;
#endif
		/* \remarks Disable the implicit conversion to bool. Use isNull() to check if string is empty.
		* By default, conversion to bool uses operator char*(). This operator always returns a valid pointer (never nullptr).
		* Therefore, the result of the default conversion to bool is always true, even for empty strings. */
		...
		*/
		...
		int printf(CStr format, ...); // Cannot use const& because of va_start

		/*! \brief Write a formatted string into this CStr using specified locale.
		 * Writes the format string, filled in by the optional arguments into
		 * this CStr.  See the ISO C++ documentation for more information on
		 * printf_l and format strings.
		 * \param format Specifies how to format the destination string.
		...
		arguments. This allows CStr objects to be used to build strings based on a 
		...
		*/
		...
		/*! \brief Write a formatted string into this CStr using specified locale.
		This method is similar to CStr::printf_l. Instead of taking a variable list of
		arguments as parameter, it takes a structure representing a variable list of
		arguments. This allows CStr objects to be used to build strings based on a
		format string, a locale, and a variable number of arguments:
		\code
		void LogMessage_l(const char* format, const _locale_t theLocale, ...) {
			va_list args;
			va_start(args, format);
			CStr buf;
			buf.vprintf_l(format, theLocale, args);
			va_end(args);
			// log the message contained by buf
		}
		\endcode
...
public:
	 ...
	 *
	 * \param string                   The string compared with.
	 */
...
public:
		 ...
		 */
		int printf(const char *format, ...);
		int printf(UTF8Str format, ...); // Cannot use const& because of va_start

		/*! \brief Write a formatted string into this UTF8Str using specified locale.
		 * Writes the format string, filled in by the optional arguments into
		 * this UTF8Str.  See the ISO C++ documentation for more information on
		 * printf_l and format strings.
		 * Note: do not use %ls formatting since conversion from wide string to
		...
		arguments as parameter, it takes a structure representing a variable list of 
		arguments. This allows UTF8Str objects to be used to build strings based on a 
		format string and a variable number of arguments.
		* Note: do not use %ls formatting since conversion from wide string to 
		* narrow string will occur using active code page encoding rather than 
		* utf8 encoding
		*/
		...
		/*! \brief Write a formatted string into this UTF8Str using specified locale.
		This method is similar to UTF8Str::printf_l. Instead of taking a variable list of
		arguments as parameter, it takes a structure representing a variable list of
		arguments. This allows UTF8Str objects to be used to build strings based on a
		format string, a locale, and a variable number of arguments.
		* Note: do not use %ls formatting since conversion from wide string to
...
class UtilExport WStr: public MaxHeapOperators {
	...
	public:
		...
		* Although it is not recommended, you can define MSTR_DISABLE_DANGLING_CHECKS to disble this compiler check. */
#ifndef MSTR_DISABLE_DANGLING_CHECKS
		operator const wchar_t* () && = delete;
		operator const wchar_t* () const&& = delete;
#endif
		/* \remarks Disable the implicit conversion to bool. Use isNull() to check if string is empty.
		* By default, conversion to bool uses operator wchar_t*(). This operator always returns a valid pointer (never nullptr).
		* Therefore, the result of the default conversion to bool is always true, even for empty strings. */
		explicit operator bool() const = delete;
		...
		int printf(WStr format, ...); // Cannot use const& because of va_start

		/**
		 * \brief Write a formatted string into this WStr using specified locale.
		 *
		 * Writes the format string, filled in by the optional arguments into
		 * this WStr.  See the ISO C++ documentation for more information on
		 * printf_l and format strings.
		...
		arguments as parameter, it takes a structure representing a variable list of 
		arguments. This allows WStr objects to be used to build strings based on a 
		format string and a variable number of arguments:
		\code
		...
		*/
		...
		/*! \brief Write a formatted string into this WStr specified locale.
		This method is similar to WStr::printf_l. Instead of taking a variable list of
		arguments as parameter, it takes a structure representing a variable list of
		arguments. This allows WStr objects to be used to build strings based on a
		format string, a locale, and a variable number of arguments:
		\code
...
public:
	 ...
	 * \param caseSensitive       If it is true(default), the search is case sensitive; otherwise the search is case insensitive.
	 */
	bool EndsWith(const WStr& s, bool caseSensitive = true) const;
	 ...
	 * \param caseSensitive       If it is true(default), the search is case sensitive; otherwise the search is case insensitive.
	 */
	bool EndsWith(wchar_t c, bool caseSensitive = true) const;
	 ...
	 * \param caseSensitive       If it is true(default), the search is case sensitive; otherwise the search is case insensitive.
	 */
	bool StartsWith(const WStr& s, bool caseSensitive = true) const;
	 ...
	 * \param caseSensitive       If it is true(default), the search is case sensitive; otherwise the search is case insensitive.
	 */
	bool StartsWith(wchar_t c, bool caseSensitive = true) const;
	 ...
	 * \param string                   The string compared with.
	 */
	bool CaseInsensitiveEqual(const wchar_t* string) const;
	bool CaseInsensitiveEqual(const WStr& string) const;
...
CAUTION: The resulting filter list string is used as-is in the Windows file selector, and
expects pairs of entries (a description entry and a wildcard entry, as shown in the example usage above).
Providing an odd number of Appended strings can result in a crash.\n
...
//-------------------------------------------------------------------------
// This function is used to apply the following rules to the name and 
// extension of a file that is being saved:
// 1) If the file being saved already exists, this function preserves the 
// capitalization of its name and extension, regardless of what the user specified. 
// 2) If the file being saved does not exist, this function forces its extension
// to be lowercase, regardless of what the user specified. The filePath parameter 
// corresponds to the file path selected by the user, and the size parameter 
// corresponds to the size of the buffer that contains that file path. In most
// situations, this function is called with the OPENFILENAME.lpstrFile and 
// OPENFILENAME.nMaxFile values returned by GetSaveFileName. 
// Note, however, that GetSaveFileName can return a filepath without an extension 
// if the user did not explicitly specify one, in which case this function must be 
// called after an extension has been appended to the filepath returned by GetSaveFileName.
//-------------------------------------------------------------------------
