...
terminating NULL is specified using the following \#define:
\code #define NURBS_NAME_SIZE 80 \endcode
<b>NURBSType mType;</b>\n\n
...
public:
	...
	DllExport void SetTransformMatrix(TimeValue t, const SetXFormPacket& xPack);
...
public:
	...
	DllExport void SetXForm(TimeValue t, const Matrix3& mat);
...
public:
	...
	\par Operators:
	*/
	DllExport void SetSeed(const Point2& seed);
};
...
public:
	...
	<b>Point3\& pvec</b>\n\n
	The vector to set. */
	DllExport void SetPVec(TimeValue t, const Point3& pvec); // projection vector
...
public:
	...
	DllExport const NURBSTexturePoint* GetPoint(int u, int v) const;
...
public:
	...
	*/
	...
	FALSE. */
	DllExport BOOL Evaluate(TimeValue t, double u, double v, Point3& pt,
			Point3& dPdU, Point3& dPdV, Point3& d2PdU2, Point3& d2PdV2, Point3& d2PdUdV) const;
	/*! \remarks Retrieves the minimum and maximum valid values for <b>u</b>
...
public:
	...
	DllExport void SetTransformMatrix(TimeValue t, const SetXFormPacket& mat);
...
public:
	...
	<b>Point2\& seed</b>\n\n
	The seed value to set. */
	DllExport void SetSeed(int pnum, const Point2& seed);
...
protected:
	friend DllExport Object* CreateNURBSObject(IObjParam* ip, NURBSSet *nset, const Matrix3& mat);
...
DllExport NURBSResult GenNURBSLatheSurface(NURBSCVCurve& curve, const Point3& origin, const Point3& north,
									float start, float end, NURBSCVSurface& surf);
DllExport NURBSResult GenNURBSSphereSurface(float radius, const Point3& center, const Point3& northAxis, const Point3& refAxis,
					float startAngleU, float endAngleU, float startAngleV, float endAngleV, BOOL open, NURBSCVSurface& surf);
DllExport NURBSResult GenNURBSCylinderSurface(float radius, float height, const Point3& origin, const Point3& symAxis, const Point3& refAxis,
					float startAngle, float endAngle, BOOL open, NURBSCVSurface& surf);
DllExport NURBSResult GenNURBSConeSurface(float radius1, float radius2, float height, const Point3& origin, const Point3& symAxis, const Point3& refAxis,
					float startAngle, float endAngle, BOOL open, NURBSCVSurface& surf);
DllExport NURBSResult GenNURBSTorusSurface(float majorRadius, float minorRadius, const Point3& origin,
					const Point3& symAxis, const Point3& refAxis, float startAngleU, float endAngleU,
...
DllExport Object *CreateNURBSLatheShape(IObjParam* ip, MSTR name, TimeValue t, ShapeObject *shape,
                     const Matrix3& axis, float degrees, int capStart, int capEnd,
...
//! \brief Validates a NURBSId for a NURBS object.
/*  This function validates a NURBSId from an element (sub-object) of a NURBS object.
   \param obj The NURBS object owning the element.
   \param id The NURBSId that needs to be validated.
   \return  id - The supplied NURBSId is valid for the supplied object. or 0 - The supplied NURBSId is not valid. */ 
...
DllExport const MCHAR* 
GetNamedSelSetName(Object* pObj, NURBSSubObjectLevel level, int i);  

// Set the bit array to the named selection set
DllExport NURBSResult 
GetNamedSelSet(Object* pObj, NURBSSubObjectLevel level, const MCHAR* name, BitArray& selSet);

// Set the named selection set the selection in BitArray
DllExport NURBSResult 
SetNamedSelSet(Object* pObj, NURBSSubObjectLevel level, const MCHAR* name, BitArray& sel); 

// Add a new named selection set
DllExport NURBSResult
AppendNamedSelSet(Object* pObj, NURBSSubObjectLevel level, const MCHAR* name, BitArray& sel); 
