...
public:
    ...
    void*			Create(BOOL loading = FALSE) { return new SoundObject; }
    const TCHAR*	ClassName() {return GetString(IDS_SOUND_CLASS);}
	const TCHAR*	NonLocalizedClassName() { return _T("Sound"); }
	SClass_ID		SuperClassID() { return HELPER_CLASS_ID; }
    Class_ID 		ClassID() { return Class_ID(Sound_CLASS_ID1, Sound_CLASS_ID2); }
                  ...
                  SoundObject* th)
{
    ...
    case WM_INITDIALOG:
        ...
        th->pblock->GetValue(PB_SND_MIN_FRONT, t, minF);
        th->pblock->GetValue(PB_SND_MIN_BACK, t, minB);
        th->pblock->GetValue(PB_SND_MAX_FRONT, t, maxF);
        th->pblock->GetValue(PB_SND_MAX_BACK, t, maxB);
/*
...
static ParamUIDesc descParam[] = {
  ...
  // Spatialize
  ParamUIDesc(PB_SND_SPATIALIZE, TYPE_SINGLECHECKBOX, IDC_SPATIALIZE),
...
RefResult
SoundObject::NotifyRefChanged(const Interval& changeInt,
                                   ...
                                   Box3& box )
{
    ...
    if (inode->Selected()) {
        ...
        pblock->GetValue(PB_SND_MIN_FRONT, t, minFront);
        pblock->GetValue(PB_SND_MIN_BACK, t, minBack);
        pblock->GetValue(PB_SND_MAX_FRONT, t, maxFront);
        pblock->GetValue(PB_SND_MAX_BACK, t, maxBack);
...
{
    float length;
    pblock->GetValue(PB_SND_SIZE, t, length);
    mesh.setNumVerts(11);
    mesh.setNumFaces(5);
...
SoundObject::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags)
{
    if ( ! vpt || ! vpt->IsAlive() )
		...
		float radius;
    pblock->GetValue(PB_SND_SIZE, t, radius);
    if (radius <= 0.0)
        return 0;
    BuildMesh(t);
    Matrix3 m;
...
int SoundCreateCallBack::proc(ViewExp* vpt, int msg, int point, int flags, IPoint2 m, Matrix3& mat)
{
	if (!vpt || !vpt->IsAlive())
	{
		// why are we here?
	...
	case MOUSE_MOVE:
		...
		case 1:
			...
			soundObject->pblock->SetValue(PB_SND_SIZE, soundObject->iObjParams->GetTime(), radius);
