...
public:
    ...
    const TCHAR*	NonLocalizedClassName() {return _T("C++ Object Output");}
      ...
      tryAgain:
        if (GetSaveFileName(&ofn)) {
            if (DoesFileExist(fname)) {
                ...
                if (IDYES != MaxSDK::MaxMessageBox(theCppOut.hPanel, buf1, buf2, MB_YESNO | MB_ICONQUESTION)) {
                    goto tryAgain;
                }
            }
...
CppOut::OutputObject(INode *node,TCHAR *fname)
{
	...
	if(file.Open(fname, false, MaxSDK::Util::TextFile::Writer::WRITE_BOM | codepage ))
    {
		...
		file.Printf(_T("    mesh.setNumVerts(%d);\n"), mesh->getNumVerts());
        file.Printf(_T("    mesh.setNumFaces(%d);\n"), mesh->getNumFaces());
        for (i=0; i<mesh->getNumVerts(); i++) {
            Point3 v = (tm  * mesh->verts[i] - center) / maxLen;
            file.Printf(_T("    mesh.setVert(%d, size * Point3(%f,%f,%f));\n"),
                    i, v.x, v.y, v.z);
        }

        for (i=0; i<mesh->getNumFaces(); i++) {
            if (parts.f < 0.0f)
                file.Printf(_T("    mesh.faces[%d].setVerts(%d,%d,%d);\n"),
                ...
                file.Printf(_T("    mesh.faces[%d].setVerts(%d,%d,%d);\n"),
                        ...
                        mesh->faces[i].v[2]);
            if (parts.f < 0.0f) 
                file.Printf(_T("    mesh.faces[%d].setEdgeVisFlags(%d,%d,%d);\n"),
                        i,
                ...
                file.Printf(_T("    mesh.faces[%d].setEdgeVisFlags(%d,%d,%d);\n"),
                        ...
                        mesh->faces[i].getEdgeVis(1) ? 1 : 0,
                        mesh->faces[i].getEdgeVis(2) ? 1 : 0);
            file.Printf(_T("    mesh.faces[%d].setSmGroup(%x);\n"),
