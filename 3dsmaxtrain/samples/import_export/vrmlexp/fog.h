...
class FogObject: public HelperObject {			   
  ...
  public:
    ...
    const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_FOG) : _T("Fog"); }


    // From Object
           ObjectState Eval(TimeValue time);
    void InitNodeName(TSTR& s) { s = GetString(IDS_FOG); }
    ...
    Class_ID ClassID() { return Class_ID(Fog_CLASS_ID1,
                                         Fog_CLASS_ID2);}  
    void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_FOG_CLASS) : _T("Fog"); }
