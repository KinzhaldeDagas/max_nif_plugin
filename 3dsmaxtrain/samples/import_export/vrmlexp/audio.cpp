...
public:
    ...
    void*			Create(BOOL loading = FALSE) { return new AudioClipObject; }
    ...
    Class_ID 		ClassID() { return Class_ID(AudioClip_CLASS_ID1, AudioClip_CLASS_ID2); }
...
static ParamUIDesc descParam[] = {
  ...
  ParamUIDesc(PB_AC_LOOP,
              TYPE_SINGLECHECKBOX, IDC_LOOP),

  // Loop
  ParamUIDesc(PB_AC_START,
              TYPE_SINGLECHECKBOX, IDC_START_ON_LOAD),
...
void AudioClipObject::BeginEditParams( IObjParam *ip, ULONG flags, Animatable *prev )
{
	iObjParams = ip;
	...
	if (pmapParam) {
		...
		pmapParam = CreateCPParamMap(descParam,PARAMDESC_LENGTH,
			...
			GetString(IDS_AUDIO_CLIP_CLASS),
			0);
	}
	if (pmapParam) {
...
RefResult
AudioClipObject::NotifyRefChanged(const Interval& changeInt,
...
{
    float size;
    pblock->GetValue(PB_AC_SIZE, t, size);
#include "acob.inl"    
}
...
AudioClipObject::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags)
{
    if ( ! vpt || ! vpt->IsAlive() )
		...
		float radius;
    pblock->GetValue(PB_AC_SIZE, t, radius);
    if (radius <= 0.0)
        return 0;
    BuildMesh(t);
    Matrix3 m;
...
int AudioClipCreateCallBack::proc(ViewExp* vpt, int msg, int point, int flags, IPoint2 m, Matrix3& mat)
{
	if (!vpt || !vpt->IsAlive())
	{
		// why are we here?
	...
	case MOUSE_MOVE:
		...
		case 1:
			...
			audioClipObject->pblock->SetValue(PB_AC_SIZE, audioClipObject->iObjParams->GetTime(), radius);
...
AudioClipObject::Clone(RemapDir& remap)
{
    ...
    ac->ReplaceReference(0, remap.CloneRef(pblock));
    ac->desc = desc;
    ac->url = url;
