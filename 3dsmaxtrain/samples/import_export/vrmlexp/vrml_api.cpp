...
CallbackTable::GetCallbackMethods(Interface* ip) {
    ...
    DWORD       nSize = _countof(lpBuf);
    
    const MSTR maxPlugCfgPath = ip->GetDir(APP_PLUGCFG_DIR);
    TCHAR       lpINIFileName[MAX_PATH];
    _stprintf(lpINIFileName, _T("%s\\%s"), maxPlugCfgPath.data(), _T("vrmlexp.ini"));
 ...
 // see if the INI files exists.
    ...
    if (findhandle == INVALID_HANDLE_VALUE) {
        ...
        TSTR buf = _T("couldn't find INI file");
        MaxSDK::MaxMessageBox(GetActiveWindow(), buf, " ", MB_OK);
        */
        return FALSE;
    }
	    ...
	    if (libInst)
	    {
         ...
         // Export Lib support for pre-scene export
            if (PreSceneCallback & CallbackType) {
                ...
                if(!lpProc) {
                   TCHAR buf[256];
                    _stprintf(buf, _T("method %hs not implimented"), lpStr);
	                MaxSDK::MaxMessageBox(GetActiveWindow(), buf, _T("Export Lib"), MB_OK);
                }
                AddPreScene(lpProc);
            }
