...
public:
    ...
    void*			Create(BOOL loading = FALSE) { return new ProxSensorObject; }
    ...
    Class_ID 		ClassID() { return Class_ID(ProxSensor_CLASS_ID1, ProxSensor_CLASS_ID2); }
...
static ParamUIDesc descParam[] = {
  ...
  // Enabled
  ParamUIDesc(PB_PS_ENABLED, TYPE_SINGLECHECKBOX, IDC_ENABLE),
	...
	Animatable *prev )
{
	...
	if (pmapParam) {
		...
		pmapParam = CreateCPParamMap(descParam,PARAMDESC_LENGTH,
			...
			GetString(IDS_PROX_SENSOR_CLASS),
			0);
	}
...
RefResult
ProxSensorObject::NotifyRefChanged(const Interval& changeInt,
...
ProxSensorObject::BuildMesh(TimeValue t)
{
    ...
    pblock->GetValue(PB_PS_LENGTH, t, length);
    pblock->GetValue(PB_PS_WIDTH, t, width);
    pblock->GetValue(PB_PS_HEIGHT, t, height);
    float x = width/2.0f;
    float y = length/2.0f;
    float z = height;
    va = Point3(-x, -y, 0.0f);
...
int ProxSensorCreateCallBack::proc(ViewExp* vpt, int msg, int point, int flags, IPoint2 m, Matrix3& mat)
{
	if (!vpt || !vpt->IsAlive())
	{
		// why are we here?
