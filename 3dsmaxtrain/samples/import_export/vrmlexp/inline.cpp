...
public:
	...
	const TCHAR* NonLocalizedClassName() { return _T("Inline"); }
	SClass_ID     SuperClassID() { return HELPER_CLASS_ID; }
    Class_ID      ClassID() { return Class_ID(VRML_INS_CLASS_ID1, VRML_INS_CLASS_ID2); }
...
RefResult
VRMLInsObject::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
...
VRMLInsObject::GetMat(TimeValue t, INode* inode, ViewExp& vpt, Matrix3& tm)
{
    tm = inode->GetObjectTM(t);
		...
		float scaleFactor = vpt->GetVPWorldWidth(tm.GetTrans()) / 360.0f;
		if (scaleFactor!=1.0f)
...
int VRMLInsCreateCallBack::proc(ViewExp* vpt, int msg, int point, int flags, IPoint2 m, Matrix3& mat)
{
	if (!vpt || !vpt->IsAlive())
	{
		// why are we here?
