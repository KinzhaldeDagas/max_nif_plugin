...
public:
	...
	Class_ID       ClassID() { return Class_ID(LOD_CLASS_ID1, LOD_CLASS_ID2); }
...
LODObjPick::HitTest(IObjParam *ip,HWND hWnd,ViewExp *vpt,IPoint2 m,int flags)
{
	...
	{
		// why are we here?
		DbgAssert(!_T("Doing HitTest() on invalid view port!"));
		return FALSE;
	}
...
LODObjPick::Pick(IObjParam *ip,ViewExp *vpt)
{
	...
	if ((node = vpt->GetClosestHit()) != NULL) {
		...
		LODObj *obj = new LODObj();
		int id = lod->lodObjects.Append(1, &obj);

		RefResult ret = lod->ReplaceReference( id, node);

		HWND hw = lod->hRollup;
		int ind = SendMessage(GetDlgItem(hw,IDC_LOD_LIST), LB_ADDSTRING, 0, (LPARAM)(LPCTSTR)obj->listStr.data());
...
{
	LODObject *th = DLGetWindowLongPtr<LODObject *>( hDlg);  
   ...
   case WM_INITDIALOG: {
	...
	case CC_SPINNER_CHANGE:
      ...
      switch( LOWORD(wParam) ) {
         ...
         switch (HIWORD(wParam)) {
			...
			case IDC_LOD_DEL: { // Delete the object from the list
				...
				if (index != LB_ERR) {
					...
					EnableWindow(GetDlgItem(hDlg, IDC_LOD_DIST_SPINNER), FALSE);
					th->SetCurDist(-1.0f);
					th->distSpin->SetValue( -1.0f, FALSE );
		...
		RefResult
			LODObject::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
...
#include "lodob.inl"
		...
		int LODCreateCallBack::proc(ViewExp* vpt, int msg, int point, int flags, IPoint2 m, Matrix3& mat)
		{
			if (!vpt || !vpt->IsAlive())
			{
				// why are we here?
