...
public:
    ...
    void*        Create(BOOL loading = FALSE) { return new BackgroundObject; }
    ...
    Class_ID      ClassID() { return Class_ID(Background_CLASS_ID1, Background_CLASS_ID2); }
...
BackgroundImageDlgProc( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    TCHAR text[MAX_PATH];
    BackgroundObject *th =
        DLGetWindowLongPtr<BackgroundObject *>( hDlg);   
...
static HWND imgDlg = nullptr;

void BackgroundObject::BeginEditParams( IObjParam *ip, ULONG flags, Animatable *prev )
{
	iObjParams = ip;
	...
	if (skyParam && imgDlg) {
		...
		DLSetWindowLongPtr( imgDlg, this);
	} else {
...
void BackgroundObject::EndEditParams( IObjParam *ip, ULONG flags,Animatable *prev)
{
	...
	{
		skyParam->SetParamBlock(nullptr);
		groundParam->SetParamBlock(nullptr);
		DLSetWindowLongPtr( imgDlg, NULL);
	}
}
...
BackgroundObject::BackgroundObject() : HelperObject() 
{
	...
	pb->SetValue(PB_SKY_COLOR1 , 0, blue);
	Point3 red(1.0f, 0.0f, 0.0f);
	pb->SetValue(PB_SKY_COLOR2 , 0, red);
	Point3 green(0.0f, 1.0f, 0.0f);
	pb->SetValue(PB_SKY_COLOR3 , 0, green);
	...
	Point3 groundColor1(0.7f, 0.4f, 0.3f);
	pb->SetValue(PB_GROUND_COLOR1 , 0, groundColor1);
	Point3 groundColor2(1.0f, 0.0f, 0.0f);
	pb->SetValue(PB_GROUND_COLOR2 , 0, groundColor2);
	Point3 groundColor3(0.0f, 1.0f, 0.0f);
	pb->SetValue(PB_GROUND_COLOR3 , 0, groundColor3);
...
RefResult
BackgroundObject::NotifyRefChanged(const Interval& changeInt,
...
{
    float size;
    pblock->GetValue(PB_BG_SIZE, t, size);
#include "bgob.inl"
}
...
BackgroundObject::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags)
{
    if ( ! vpt || ! vpt->IsAlive() )
		...
		float radius;
    pblock->GetValue(PB_BG_SIZE, t, radius);
    if (radius <= 0.0)
        return 0;
    BuildMesh(t);
    Matrix3 m;
...
int BackgroundCreateCallBack::proc(ViewExp* vpt, int msg, int point, int flags, IPoint2 m, Matrix3& mat)
{
	if (!vpt || !vpt->IsAlive())
	{
		// why are we here?
	...
	case MOUSE_MOVE:
		...
		case 1:
			...
			backgroundObject->pblock->SetValue(PB_BG_SIZE, backgroundObject->iObjParams->GetTime(), radius);
...
BackgroundObject::Clone(RemapDir& remap)
{
    ...
    bg->ReplaceReference(0, remap.CloneRef(pblock));
    bg->back = back;
    bg->bottom = bottom;
    bg->front = front;
    bg->left = left;
