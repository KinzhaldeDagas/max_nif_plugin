...
public:
    ...
    void*			Create(BOOL loading = FALSE) { return new NavInfoObject; }
    const TCHAR*	ClassName() {return GetString(IDS_NAV_INFO_CLASS);}
	const TCHAR*	NonLocalizedClassName() { return _T("NavInfo"); }
	SClass_ID		SuperClassID() { return HELPER_CLASS_ID; }
    Class_ID 		ClassID() { return Class_ID(NavInfo_CLASS_ID1, NavInfo_CLASS_ID2); }
...
const TCHAR* navTypes[] = { _T("WALK"), _T("EXAMINE"), _T("FLY"), _T("NONE") };
                  ...
                  NavInfoObject* th)
{
    ...
    case WM_INITDIALOG: {
        ...
        int type;
        th->pblock->GetValue(PB_TYPE, th->iObjParams->GetTime(), type);
        if (type < 0 || type > 3)
            type = 0;
...
static ParamUIDesc descParam[] = {
  ...
  // Loop
  ParamUIDesc(PB_HEADLIGHT, TYPE_SINGLECHECKBOX, IDC_HEADLIGHT),
...
RefResult
NavInfoObject::NotifyRefChanged(const Interval& changeInt,
...
{
    float size;
    pblock->GetValue(PB_NI_SIZE, t, size);
#include "niob.inl"
}
...
NavInfoObject::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags)
{
    if ( ! vpt || ! vpt->IsAlive() )
		...
		float radius;
    pblock->GetValue(PB_NI_SIZE, t, radius);
    if (radius <= 0.0)
        return 0;
    BuildMesh(t);
    Matrix3 m;
...
int NavInfoCreateCallBack::proc(ViewExp* vpt, int msg, int point, int flags, IPoint2 m, Matrix3& mat)
{
	if (!vpt || !vpt->IsAlive())
	{
		// why are we here?
	...
	case MOUSE_MOVE:
		...
		case 1:
			...
			navInfoObject->pblock->SetValue(PB_NI_SIZE, navInfoObject->iObjParams->GetTime(), radius);
