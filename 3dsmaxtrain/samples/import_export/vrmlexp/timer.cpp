...
public:
    ...
    void*			Create(BOOL loading = FALSE) { return new TimeSensorObject; }
    ...
    Class_ID 		ClassID() { return Class_ID(TimeSensor_CLASS_ID1, TimeSensor_CLASS_ID2); }
...
TimeSensorObjPick::Pick(IObjParam *ip,ViewExp *vpt)
{
    ...
    if ((node = vpt->GetClosestHit()) != NULL) {
        ...
        TimeSensorObj *obj = new TimeSensorObj();
...
{
   return UI::MouseCursors::LoadMouseCursor(UI::MouseCursors::Lod);
}
    ...
    if (ob && ob->hRollup) {
        ...
        for (i = 0; i < ob->TimeSensorObjects.Count(); i++) {
            ...
            int ind = SendMessage(GetDlgItem(ob->hRollup,IDC_TimeSensor_LIST),
                                  LB_ADDSTRING, 0,
                                  (LPARAM)(LPCTSTR)obj->listStr.data());
...
//        SetDlgFont( hDlg, th->iObjParams->GetAppHFont() );
		...
		th->pblock->GetValue(PB_LOOP, th->iObjParams->GetTime(), loop);
...
static ParamUIDesc descParam[] = {
  ...
  // Loop
  ParamUIDesc(PB_LOOP, TYPE_SINGLECHECKBOX, IDC_LOOP),

  // Start On Load
  ParamUIDesc(PB_START_ON_LOAD, TYPE_SINGLECHECKBOX, IDC_START_ON_LOAD),
...
void
	TimeSensorObject::BeginEditParams( IObjParam *ip, ULONG flags, Animatable *prev )
{
	iObjParams = ip;
	...
	if (pmapParam) {
		...
		pmapParam = CreateCPParamMap(descParam,PARAMDESC_LENGTH,
			...
			GetString(IDS_TIME_SENSOR_CLASS),
			0);
	}
...
void TimeSensorObject::EndEditParams( IObjParam *ip, ULONG flags,Animatable *prev)
{
...
RefResult
TimeSensorObject::NotifyRefChanged(const Interval& changeInt,
...
{
    float size;
    pblock->GetValue(PB_SIZE, t, size);
#include "clockob.inl"
}
...
TimeSensorObject::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags)
{
    if ( ! vpt || ! vpt->IsAlive() )
		...
		float radius;
    pblock->GetValue(PB_SIZE, t, radius);
    if (radius <= 0.0)
        return 0;
    BuildMesh(t);
    Matrix3 m;
...
int TimeSensorCreateCallBack::proc(ViewExp* vpt, int msg, int point, int flags, IPoint2 m, Matrix3& mat)
{
	if (!vpt || !vpt->IsAlive())
	{
		// why are we here?
	...
	case MOUSE_MOVE:
		...
		case 1:
			...
			timeSensorObject->pblock->SetValue(PB_SIZE, timeSensorObject->iObjParams->GetTime(), radius);
