...
struct AnimRoute {
	...
	AnimRoute(const TCHAR* from, const TCHAR* to, INode* fromNode, INode* node)
	{
		mFromName = from;
		mToName   = to;
		mToNode   = node;
...
struct InterpRoute {
	InterpRoute() {mType = 0;}
	InterpRoute(const TCHAR* interp, int type, const TCHAR* node)
	{
		mInterp = interp;
		mType = type;
		mNode = node;
...
private:
	const TCHAR* point(const Point3& p);
	const TCHAR* scalePoint(const Point3& p);
	const TCHAR* normPoint(const Point3& p);
	const TCHAR* axisPoint(const Point3& p, float ang);
	const TCHAR* texture(const UVVert& uv);
	const TCHAR* color(const Color& c);
	const TCHAR* color(const Point3& c);
	const TCHAR* floatVal(float f);
	...
	void AddInterpolator(const TCHAR* interp, int type, const TCHAR *name);
	void WriteInterpolatorRoutes(int level, BOOL isCamera);
	void AddCameraAnimRoutes(const TCHAR* vrmlObjName, INode* fromNode, INode* top);
	void AddAnimRoute(const TCHAR* from, const TCHAR* to, INode* fromNode, INode* node);
