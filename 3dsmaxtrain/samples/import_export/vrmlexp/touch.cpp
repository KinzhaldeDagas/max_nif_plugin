...
public:
    ...
    void*			Create(BOOL loading = FALSE) { return new TouchSensorObject; }
...
TouchSensorObjPick::Pick(IObjParam *ip,ViewExp *vpt)
{
    ...
    if ((node = vpt->GetClosestHit()) != NULL) {
        ...
        TouchSensorObj *obj = new TouchSensorObj();
...
static ParamUIDesc descParam[] = {
  ...
  // Loop
  ParamUIDesc(PB_TS_ENABLED, TYPE_SINGLECHECKBOX, IDC_ENABLE),
...
void TouchSensorObject::BeginEditParams( IObjParam *ip, ULONG flags, Animatable *prev )
{
	iObjParams = ip;
...
void TouchSensorObject::EndEditParams( IObjParam *ip, ULONG flags,Animatable *prev)
{
...
RefResult
TouchSensorObject::NotifyRefChanged(const Interval& changeInt,
...
{
    float size;
    pblock->GetValue(PB_TS_SIZE, t, size);
#include "touchob.inl"
}
...
TouchSensorObject::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags)
{
    if ( ! vpt || ! vpt->IsAlive() )
		...
		float radius;
    pblock->GetValue(PB_TS_SIZE, t, radius);
    if (radius <= 0.0)
        return 0;
    BuildMesh(t);
    Matrix3 m;
...
int TouchSensorCreateCallBack::proc(ViewExp* vpt, int msg, int point, int flags, IPoint2 m, Matrix3& mat)
{
	if (!vpt || !vpt->IsAlive())
	{
		// why are we here?
	...
	case MOUSE_MOVE:
		...
		case 1:
			...
			touchSensorObject->pblock->SetValue(PB_TS_SIZE, touchSensorObject->iObjParams->GetTime(), radius);
