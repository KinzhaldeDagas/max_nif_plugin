...
public:
	...
	const  TCHAR* NonLocalizedClassName() { return _T("VRBL Export"); }
...
VRBLExport::StartNode(INode* node, Object* obj, int level, BOOL outputName)
{
    ...
    if (obj->SuperClassID() == CAMERA_CLASS_ID ||
     ...
     // Lights and cameras need different top-level names for triggers
        ...
        if (IsMirror(node)) {
            Indent(level+1);
			mStream.Write(_T("ShapeHints {\n"));
            Indent(level+2);
			mStream.Write(_T("vertexOrdering CLOCKWISE\n"));
            Indent(level+1);
			mStream.Write(_T("}\n"));
        }
    }
...
VRBLExport::OutputNormals(Mesh& mesh, int level)
{
    ...
    Indent(level);
	mStream.Write(_T("NormalBinding { value PER_VERTEX_INDEXED }\n"));
                            ...
                            BOOL twoSided, int level)
{
   ...
   if (isMulti) {
        Indent(level);
	   mStream.Write(_T("MaterialBinding { value PER_FACE_INDEXED }\n"));
    }

    // Output the vertices
    Indent(level);
	mStream.Write(_T("Coordinate3 { point [\n"));
        
    for(i = 0; i < numverts; i++) {
        Point3 p = mesh.verts[i];
        Indent(level+1);
		mStream.Write(point(p));
        if (i == numverts-1) {
			mStream.Write(_T("]\n"));
            Indent(level);
			mStream.Write(_T("}\n"));
    ...
    if (numtverts > 0 && td) {
        Indent(level);
		mStream.Write(_T("TextureCoordinate2 { point [\n"));

        for(i = 0; i < numtverts; i++) {
            UVVert p = mesh.getTVert(i);
            Indent(level+1);
			mStream.Write(texture(p));
            if (i == numtverts-1) {
				mStream.Write(_T("]\n"));
                Indent(level);
				mStream.Write(_T("}\n"));
    ...
    if (twoSided) {
		...
		mStream.Write(_T("ShapeHints {\n"));
        Indent(level+1);
		mStream.Write(_T("shapeType UNKNOWN_SHAPE_TYPE\n"));
        Indent(level);
		mStream.Write(_T("}\n"));
    }
    // Output the triangles
    Indent(level);
	mStream.Write(_T("IndexedFaceSet { coordIndex [\n"));
    for(i = 0; i < numfaces; i++) {
    ...
    if (numtverts > 0 && td) {
        Indent(level);
		mStream.Write(_T("textureCoordIndex [\n"));
        for(i = 0; i < numfaces; i++) {
            if (!(mesh.faces[i].flags & FACE_HIDDEN)) {
                Indent(level+1);
    ...
    if (isMulti) {
        Indent(level);
		mStream.Write(_T("materialIndex [\n"));
        for(i = 0; i < numfaces; i++) {
            if (!(mesh.faces[i].flags & FACE_HIDDEN)) {
                Indent(level+1);
...
VRBLExport::OutputMultiMtl(Mtl* mtl, int level)
{
    ...
    Indent(level + 1);
    mStream.Write(_T("specularColor [ "));
    for(i = 0; i < num; i++) {
        sub = mtl->GetSubMtl(i);
        if (!sub)
            continue;
    ...
    Indent(level+1);
	mStream.Write(_T("emissiveColor [ )"));
    for(i = 0; i < num; i++) {
        sub = mtl->GetSubMtl(i);
        if (!sub)
            continue;
...
VRBLExport::OutputMaterial(INode* node, BOOL& twoSided, int level)
{
    ...
    Indent(level);
	mStream.Write(_T("Texture2Transform {\n"));
...
VRBLExport::VrblOutSphere(INode * node, Object *obj, int level)
{
	...
	iSphereParams->GetValue(SPHERE_RECENTER, mStart, basePivot);
	iSphereParams->GetValue(SPHERE_GENUVS, mStart, genUV);
	iSphereParams->GetValue(SPHERE_HEMI, mStart, hemi);
	iSphereParams->GetValue(SPHERE_SMOOTH, mStart, smooth);
    if (!smooth || basePivot || (genUV && td) || hemi > 0.0f)
        return FALSE;

	iSphereParams->GetValue(SPHERE_RADIUS, mStart, radius);
...
VRBLExport::VrblOutCylinder(INode* node, Object *obj, int level)
{
	...
	iCylParams->GetValue(CYLINDER_GENUVS, mStart, genUV);
	iCylParams->GetValue(CYLINDER_SLICEON, mStart, sliceOn);
	iCylParams->GetValue(CYLINDER_SMOOTH, mStart, smooth);
    if (sliceOn || (genUV && td) || !smooth)
        return FALSE;

	iCylParams->GetValue(CYLINDER_RADIUS, mStart, radius);
	iCylParams->GetValue(CYLINDER_HEIGHT, mStart, height);
    Indent(level);
	mStream.Write(_T("Separator {\n"));
    Indent(level+1);
    if (mZUp) {
...
VRBLExport::VrblOutCone(INode* node, Object *obj, int level)
{
	...
	iConeParams->GetValue(CONE_GENUVS, mStart, genUV);
	iConeParams->GetValue(CONE_SLICEON, mStart, sliceOn);
	iConeParams->GetValue(CONE_SMOOTH, mStart, smooth);
	iConeParams->GetValue(CONE_RADIUS2, mStart, radius2);
    if (sliceOn || (genUV &&td) || !smooth || radius2 > 0.0f)
        return FALSE;

	iConeParams->GetValue(CONE_RADIUS1, mStart, radius1);
	iConeParams->GetValue(CONE_HEIGHT, mStart, height);
    Indent(level);
...
VRBLExport::VrblOutCube(INode* node, Object *obj, int level)
{
	...
	iBoxParams->GetValue(BOXOBJ_GENUVS, mStart, genUV);
	iBoxParams->GetValue(BOXOBJ_LSEGS, mStart, lsegs);
	iBoxParams->GetValue(BOXOBJ_WSEGS, mStart, hsegs);
	iBoxParams->GetValue(BOXOBJ_HSEGS, mStart, wsegs);
    if ((genUV && td) || lsegs > 1 || hsegs > 1 || wsegs > 1)
        return FALSE;

	iBoxParams->GetValue(BOXOBJ_LENGTH, mStart, length);
	iBoxParams->GetValue(BOXOBJ_WIDTH, mStart, width);
	iBoxParams->GetValue(BOXOBJ_HEIGHT, mStart, height);
    Indent(level);
	mStream.Write(_T("Separator {\n"));
    Indent(level+1);
    Point3 p = Point3(0.0f,0.0f,height/2.0f);
...
VRBLExport::VrblOutTarget(INode* node, int level)
{
    ...
    if (id != Class_ID(SPOT_LIGHT_CLASS_ID, 0) &&
        ...
        return TRUE;
    Indent(level);
	mStream.Write(_T("AimTarget_ktx_com {\n"));
    if (mGenFields) {
        Indent(level+1);
		mStream.Write(_T("fields [ SFString aimer ]\n"));
    }
    Indent(level+1);
                         ...
                         int type, int level)
{
    ...
    for (int i=0; i<ikeys->GetNumKeys(); i++) {
        ...
        if (i == 0 || TCBIsDifferent(k, oldK)) {
            Indent(level);
			mStream.Write(_T("AnimationStyle_ktx_com {\n"));
            Indent(level+1);
            if (mGenFields)
				mStream.Write(_T("fields [ SFBool loop, SFBitMask splineUse, SFFloat tension, SFFloat continuity, ")
							  _T("SFFloat bias, SFFloat easeTo, SFFloat easeFrom, SFVec3f pivotOffset ]\n"));
            Indent(level+1);
			mStream.Write(_T("splineUse ("));
            
            // Write flags
            BOOL hadOne = FALSE;
            if (k->tens   != 0.0f) {
				mStream.Write(_T("TENSION"));
                hadOne = TRUE;
            }
            if (k->cont   != 0.0f) {
                if (hadOne)
					mStream.Write(_T(" | "));
				mStream.Write(_T("CONTINUITY"));
                hadOne = TRUE;
            }
            if (k->bias   != 0.0f) {
                if (hadOne)
					mStream.Write(_T(" | "));
				mStream.Write(_T("BIAS"));
                hadOne = TRUE;
            }
            if (k->easeIn != 0.0f) {
                if (hadOne)
					mStream.Write(_T(" | "));
				mStream.Write(_T("EASE_TO"));
                hadOne = TRUE;
            }
            if (k->easeOut!= 0.0f) {
                if (hadOne)
					mStream.Write(_T(" | "));
				mStream.Write(_T("EASE_FROM"));
                hadOne = TRUE;
            }
			mStream.Write(_T(")\n"));
        ...
        case KEY_SCL: {
            ...
            mHadAnim = TRUE;
            Indent(level);
			mStream.Write(_T("ScaleKey_ktx_com {\n"));
            Indent(level+1);
            if (mGenFields)
        ...
        case KEY_COLOR:
            ...
            mHadAnim = TRUE;
            Indent(level);
			mStream.Write(_T("ColorKey_ktx_com {\n"));
            Indent(level+1);
            if (mGenFields)
				mStream.Write(_T("fields [ SFLong frame, SFColor color ]\n"));
            Indent(level+1);
            mStream.Printf(_T("frame %d\n"), (k->time - mStart)/GetTicksPerFrame());
            Indent(level+1);
            mStream.Printf(_T("color %s\n"), color(pkey.val));
        ...
        case KEY_POS:
            ...
            mHadAnim = TRUE;
            Indent(level);
			mStream.Write(_T("PositionKey_ktx_com {\n"));
            Indent(level+1);
            if (mGenFields)
				mStream.Write(_T("fields [ SFLong frame, SFVec3f translation ]\n"));
            Indent(level+1);
            mStream.Printf(_T("frame %d\n"), (k->time - mStart)/GetTicksPerFrame());
           p = pkey.val;
            Indent(level+1);
        ...
        case KEY_ROT: {
           ...
           if (!roundToZero(axis.x + rkey.val.axis.x) &&
            ...
            mHadAnim = TRUE;
            Indent(level);
			mStream.Write(_T("RotationKey_ktx_com {\n"));
            Indent(level+1);
            if (mGenFields)
				mStream.Write(_T("fields [ SFLong frame, SFRotation rotation ]\n"));
            Indent(level+1);
            mStream.Printf(_T("frame %d\n"), (k->time - mStart)/GetTicksPerFrame());
            Indent(level+1);
            mStream.Printf(_T("rotation %s\n"),
                             ...
                             int type, int level)
{
    ...
    Indent(level+1);
    if (mGenFields)
		mStream.Write(_T("fields [ SFVec3f pivotOffset ]\n"));
    Indent(level+1);
    ...
    for (i=0; i < timeVals.Count(); i++) {
        ...
        case KEY_SCL:
            ...
            Indent(level+1);
            if (mGenFields)
				mStream.Write(_T("fields [ SFLong frame, SFVec3f scale ]\n"));
            Indent(level+1);
            mStream.Printf(_T("frame %d\n"),
                    (timeVals[i]-mStart)/GetTicksPerFrame());
            Indent(level+1);
...
VRBLExport::VrblOutFileInfo()
{
	...
	mStream.Write(_T("ShapeHints {\n"));
    Indent(2);
	mStream.Write(_T("shapeType SOLID\n"));
    Indent(2);
	mStream.Write(_T("vertexOrdering COUNTERCLOCKWISE\n"));
    Indent(2);
	mStream.Write(_T("faceType CONVEX\n"));
    Indent(1);
	mStream.Write(_T("}\n"));
}
...
VRBLExport::VrblOutAnimationFrames()
{
    if (mType == Export_VRBL && mHadAnim) {
        Indent(1);
		mStream.Write(_T("AnimationFrames_ktx_com {\n"));
        Indent(2);
        if (mGenFields)
			mStream.Write(_T("fields [ SFLong length, SFLong segmentStart, SFLong segmentEnd, SFLong current, SFFloat ")
						  _T("rate ]\n"));
        Indent(2);
	...
	GetAppData(Interface* ip, int id, const TCHAR* def, TCHAR* val, int len)
{
	...
	if (node == NULL)
	{
		_tcsncpy_s(val, len, def, _TRUNCATE);
		return;
	}
	...
	else
	{
		...
		_tcsncpy_s(val, len, data.data(), _TRUNCATE);
	}
}
	...
	WriteAppData(Interface* ip, int id, const TCHAR* val)
{
	...
	size_t len;
	const char* a_string = m_string.ToUTF8(&len);
	len += 4; // 3 for UTF8 BOM, one for null

	char* buf = (char*)MAX_malloc(len);
	strcpy_s(buf, len, "\xef\xbb\xbf"); // UTF8 BOM
	strcat_s(buf, len, a_string);
	...
	SampleRatesDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDOK: {
			...
			const TCHAR* val = once ? _T("once") : _T("custom");
	...
	VrblExportDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		switch(LOWORD(wParam)) {
		case IDC_EXP_HELP: {
			const MSTR helpDir = exp->mIp->GetDir(APP_HELP_DIR);
