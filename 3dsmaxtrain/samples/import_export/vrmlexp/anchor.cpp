...
public:
    ...
    void*			Create(BOOL loading = FALSE) { return new AnchorObject; }
    const TCHAR*	ClassName() {return GetString(IDS_ANCHOR_CLASS);}
	const TCHAR*	NonLocalizedClassName() { return _T("Anchor"); }
	SClass_ID		SuperClassID() { return HELPER_CLASS_ID; }
    Class_ID 		ClassID() { return Class_ID(Anchor_CLASS_ID1, Anchor_CLASS_ID2); }
...
void AnchorObject::BeginEditParams( IObjParam *ip, ULONG flags, Animatable *prev )
{
	iObjParams = ip;
	if (pmapParam) {
...
{
	pblock = NULL;
	triggerObject = cameraObject = NULL;
...
RefResult
AnchorObject::NotifyRefChanged(const Interval& changeInt,
...
{
    float size;
    pblock->GetValue(PB_AN_SIZE, t, size);
#include "anchorob.inl"
}
...
AnchorObject::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags)
{
    ...
    float radius;
    pblock->GetValue(PB_AN_SIZE, t, radius);
    if (radius <= 0.0)
        return 0;
    BuildMesh(t);
    Matrix3 m;
...
int AnchorCreateCallBack::proc(ViewExp* vpt, int msg, int point, int flags, IPoint2 m, Matrix3& mat)
{
	if (!vpt || !vpt->IsAlive())
	{
		// why are we here?
	...
	case MOUSE_MOVE:
		...
		case 1:
			...
			anchorObject->pblock->SetValue(PB_AN_SIZE, anchorObject->iObjParams->GetTime(), radius);
