...
const TCHAR* VRML2Export::point(const Point3& p)
{
	static TCHAR buf[50];
	TCHAR format[20];
...
const TCHAR* VRML2Export::color(const Color& c)
{
	static TCHAR buf[50];
	TCHAR format[20];
	_stprintf(format, _T("%%.%dg %%.%dg %%.%dg"), mDigits, mDigits, mDigits);
...
const TCHAR* VRML2Export::color(const Point3& c)
{
	static TCHAR buf[50];
	TCHAR format[20];
	_stprintf(format, _T("%%.%dg %%.%dg %%.%dg"), mDigits, mDigits, mDigits);
...
const TCHAR* VRML2Export::floatVal(float f)
{
	static TCHAR buf[50];
	TCHAR format[20];
...
const TCHAR* VRML2Export::texture(const UVVert& uv)
{
	static TCHAR buf[50];
	TCHAR format[20];
...
const TCHAR* VRML2Export::scalePoint(const Point3& p)
{
	static TCHAR buf[50];
	TCHAR format[20];
	_stprintf(format, _T("%%.%dg %%.%dg %%.%dg"), mDigits, mDigits, mDigits);
...
const TCHAR* VRML2Export::normPoint(const Point3& p)
{
	static TCHAR buf[50];
	TCHAR format[20];
	_stprintf(format, _T("%%.%dg %%.%dg %%.%dg"), mDigits, mDigits, mDigits);
...
const TCHAR* VRML2Export::axisPoint(const Point3& p1, float angle)
{
	const Point3 p = p1 == Point3::Origin ? Point3::XAxis : p1;
...
// IdFirstChar :: = Any character except:             0x30 - 0x39, 0x0 - 0x20, 0x22, 0x23, 0x27, 0x2b, 0x2c, 0x2d, 0x2e, 0x5b, 0x5c, 0x5d, 0x7b, 0x7d, 0x7f; 
// IdRestChars :: = Any number of characters except :              0x0 - 0x20, 0x22, 0x23, 0x27,       0x2c,       0x2e, 0x5b, 0x5c, 0x5d, 0x7b, 0x7d, 0x7f;
#define CTL_CHARS      0x1f
TSTR VRMLName(const TCHAR* name)
{
	static int seqnum = 0;
	int firstCharacter = 1;
	TSTR buffer = name;
	if (buffer.isNull())
		buffer.printf(_T("__%d"), seqnum++);
	
	TCHAR* cPtr = buffer.dataForWrite();
	while(*cPtr)
	{
		if (*cPtr <= CTL_CHARS ||	// 0x0-0x1f
			*cPtr == _T(' ') ||		// 0x20
			*cPtr == _T('"') ||		// 0x22
			*cPtr == _T('#') ||		// 0x23
			*cPtr == _T('\'') ||		// 0x27
			...
			*cPtr == _T('}') ||		// 0x7d
			*cPtr == 0x7f ||			// 0x7f
			(firstCharacter && ((*cPtr >= _T('0') && *cPtr <= _T('9')) || (*cPtr == _T('+') || *cPtr == _T('-')))) // 0x30-0x39, 0x2b, 0x2d
		)
		{
			*cPtr = _T('_');
		}
	...
	VRML2Export::OutputNormals(Mesh& mesh, int level)
{
	...
	NormalDesc* nd;
	Indent(level);
	mStream.Write(_T("normal "));
	mStream.Write(_T("Normal { vector [\n"));
	size_t width = CurrentWidth();
	Indent(level+1);
	...
	Indent(level);
	mStream.Write(_T("normalPerVertex TRUE\n"));
	...
	int textureNum, BOOL pMirror)
{
	...
	if (mPreLight) {
		if (!mCPVSource) {   // 1 if MAX, 0 if we calc
			...
			Color c;
			Indent(level);
			mStream.Write(_T("colorPerVertex TRUE\n"));
			Indent(level);
			width = CurrentWidth();
			mStream.Write(_T("color Color { color [\n"));
			Indent(level+1);
	...
	if (!mPreLight && isMulti && textureNum == -1) {
		Color c;
		Indent(level);
		mStream.Write(_T("colorPerVertex FALSE\n"));
	...
	if (numtverts > 0 && td && !isWire) {
		Indent(level);
		mStream.Write(_T("texCoordIndex [\n"));
		Indent(level+1);
		width = CurrentWidth();
	...
	int level, int textureNum)
{
	...
	Indent(level++);
	mStream.Write(_T("appearance Appearance {\n"));
	...
	sm->Update(0, i);
	Indent(level);
	mStream.Write(_T("material Material {\n"));
	Color c;
	...
	if (ambin >= difin)
		mStream.Write(_T("ambientIntensity 1.0\n"));
	else
	...
	Indent(level);
	mStream.Write(_T("texture ImageTexture {\n"));
	Indent(level+1);
	mStream.Printf(_T("url \"%s\"\n"), td->url);
	// mStream.Write(_T("repeatS TRUE\n"));
	// mStream.Write(_T("repeatT TRUE\n"));
	Indent(level);
	mStream.Write(_T("}\n"));
	...
	Indent(level);
	mStream.Write(_T("textureTransform TextureTransform {\n"));
	Indent(level+1);
	mStream.Write(_T("center 0.5 0.5\n"));
	...
	VRML2Export::VrmlOutSphereTest(INode * node, Object *obj)
{
	...
	sphereParams->GetValue(SPHERE_RECENTER, mStart, basePivot);
	sphereParams->GetValue(SPHERE_GENUVS, mStart, genUV);
	sphereParams->GetValue(SPHERE_HEMI, mStart, hemi);
	sphereParams->GetValue(SPHERE_SMOOTH, mStart, smooth);
	...
	VRML2Export::VrmlOutSphere(INode * node, Object *obj, int level)
{
	...
	sphereParams->GetValue(SPHERE_RADIUS, mStart, radius);

	Indent(level);

	mStream.Write(_T("geometry "));
	...
	VRML2Export::VrmlOutCylinderTest(INode* node, Object *obj)
{
	...
	cylParams->GetValue(CYLINDER_GENUVS, mStart, genUV);
	cylParams->GetValue(CYLINDER_SLICEON, mStart, sliceOn);
	cylParams->GetValue(CYLINDER_SMOOTH, mStart, smooth);
	...
	BOOL mirrored)
{
	...
	float height;
	cylParams->GetValue(CYLINDER_HEIGHT, mStart, height);
#ifdef MIRROR_BY_VERTICES
	if (mirrored)
		height = - height;
#endif
	...
	VRML2Export::VrmlOutCylinder(INode* node, Object *obj, int level)
{
	...
	cylParams->GetValue(CYLINDER_RADIUS, mStart, radius);
	cylParams->GetValue(CYLINDER_HEIGHT, mStart, height);
	Indent(level);
	mStream.Write(_T("geometry "));
	...
	VRML2Export::VrmlOutConeTest(INode* node, Object *obj)
{
	...
	coneParams->GetValue(CONE_GENUVS, mStart, genUV);
	coneParams->GetValue(CONE_SLICEON, mStart, sliceOn);
	coneParams->GetValue(CONE_SMOOTH, mStart, smooth);
	coneParams->GetValue(CONE_RADIUS2, mStart, radius2);
	...
	BOOL mirrored)
{
	...
	float height;
	coneParams->GetValue(CONE_HEIGHT, mStart, height);
#ifdef MIRROR_BY_VERTICES
	if (mirrored)
		height = - height;
#endif
	...
	VRML2Export::VrmlOutCone(INode* node, Object *obj, int level)
{
	...
	coneParams->GetValue(CONE_RADIUS1, mStart, radius1);
	coneParams->GetValue(CONE_HEIGHT, mStart, height);
	Indent(level);
	...
	VRML2Export::VrmlOutCubeTest(INode* node, Object *obj)
{
	...
	boxParams->GetValue(BOXOBJ_GENUVS, mStart, genUV);
	boxParams->GetValue(BOXOBJ_LSEGS, mStart, lsegs);
	boxParams->GetValue(BOXOBJ_WSEGS, mStart, hsegs);
	boxParams->GetValue(BOXOBJ_HSEGS, mStart, wsegs);
	...
	BOOL mirrored)
{
	...
	float height;
	boxParams->GetValue(BOXOBJ_HEIGHT, mStart, height);
#ifdef MIRROR_BY_VERTICES
	if (mirrored)
		height = - height;
#endif
	...
	VRML2Export::VrmlOutCube(INode* node, Object *obj, int level)
{
	...
	boxParams->GetValue(BOXOBJ_LENGTH, mStart, length);
	boxParams->GetValue(BOXOBJ_WIDTH, mStart, width);
	boxParams->GetValue(BOXOBJ_HEIGHT, mStart, height);
	Indent(level);
	mStream.Write(_T("geometry "));
	if (mZUp) {
	...
	VRML2Export::VrmlOutCamera(INode* node, Object* obj, int level)
{
	...
	Indent(level+1);
	mStream.Write(_T("position 0 0 0\n"));
	Indent(level+1);
	...
	VRML2Export::VrmlOutSound(INode* node, SoundObject* obj, int level)
{
	...
	obj->pblock->GetValue(PB_SND_INTENSITY, mStart, intensity);
	obj->pblock->GetValue(PB_SND_PRIORITY, mStart, priority);
	obj->pblock->GetValue(PB_SND_SPATIALIZE, mStart, spatialize);
	obj->pblock->GetValue(PB_SND_MIN_BACK, mStart, minBack);
	obj->pblock->GetValue(PB_SND_MAX_BACK, mStart, maxBack);
	obj->pblock->GetValue(PB_SND_MIN_FRONT, mStart, minFront);
	obj->pblock->GetValue(PB_SND_MAX_FRONT, mStart, maxFront);
	...
	Indent(level+1);
	mStream.Write(_T("location 0 0 0\n"));
	Indent(level+1);
	...
	VRML2Export::VrmlOutTouchSensor(INode* node, int level)
{
	...
	int enabled;
	obj->pblock->GetValue(PB_TS_ENABLED, mStart, enabled);
	Indent(level);
	mStream.Printf(_T("DEF %s-SENSOR TouchSensor { enabled %s }\n"),mNodes.GetNodeName(node),
		enabled ? _T("TRUE") : _T("FALSE"));

	const TCHAR* vrmlObjName = mNodes.GetNodeName(node);
	INode *otop = NULL;
	...
	int level)
{
	...
	obj->pblock->GetValue(PB_PS_ENABLED, mStart, enabled);
	obj->pblock->GetValue(PB_PS_LENGTH, mStart, length);
	obj->pblock->GetValue(PB_PS_WIDTH, mStart, width);
	obj->pblock->GetValue(PB_PS_HEIGHT, mStart, height);
	Indent(level);
	...
	VRML2Export::VrmlOutBillboard(INode* node, Object* obj, int level)
{
	...
	int screenAlign;
	bb->pblock->GetValue(PB_BB_SCREEN_ALIGN, mStart, screenAlign);
	Indent(level);
	mStream.Printf(_T("DEF %s Billboard {\n"), mNodes.GetNodeName(node));
	if (screenAlign) {
		Indent(level+1);
		mStream.Write(_T("axisOfRotation 0 0 0\n"));
	}
	else {
		Point3 axis(0, 0, 1);
		Indent(level+1);
	...
	VRML2Export::VrmlOutTimeSensor(INode* node, TimeSensorObject* obj, int level)
{
	...
	obj->pblock->GetValue(PB_START_TIME, mStart, start);
	obj->pblock->GetValue(PB_STOP_TIME,  mStart, end);
	obj->pblock->GetValue(PB_LOOP,  mStart, loop);
	obj->pblock->GetValue(PB_START_ON_LOAD,  mStart, startOnLoad);
	...
	Indent(level+1);
	if (startOnLoad)
		mStream.Write(_T("startTime 1\n"));
	else
		mStream.Write(_T("stopTime 1\n"));
	mStream.Write(_T("}\n"));
	if (obj->needsScript) {
		mStream.Printf(_T("DEF %s-SCRIPT Script {\n"),mNodes.GetNodeName(node));
		Indent(1);
		mStream.Write(_T("eventIn SFFloat fractionIn\n"));
		Indent(1);
		mStream.Write(_T("eventOut SFFloat fractionOut\n"));
		Indent(1);
		mStream.Write(_T("url \"javascript:\n"));
		Indent(2);
		mStream.Write(_T("function fractionIn(i) {\n"));
		Indent(2);
	...
	VRML2Export::VrmlOutPointLight(INode* node, LightObject* light, int level)
{
	...
	Indent(level+1);
	mStream.Printf(_T("intensity %s\n"),
	floatVal(light->GetIntensity(mStart)));
	Indent(level+1);
	Point3 col = light->GetRGBColor(mStart);
	...
	if (ls.useAtten) {
		Indent(level+1);
		mStream.Write(_T("attenuation 0 1 0\n"));
	}
	Indent(level);
	mStream.Write(_T("}\n"));
	return TRUE;
...
//    if (numLod > 1) {
	Indent(level);
	mStream.Write(_T("LOD {\n"));
	Indent(level+1);
	Point3 p = node->GetObjTMAfterWSM(mStart).GetTrans();
	mStream.Printf(_T("center %s\n"), point(p));
	Indent(level+1);
	mStream.Write(_T("range [ "));
	...
	while (type) {
		...
		case KEY_POS:
			...
			if (newKeys == 0)
				return;
			timeVals = posTimes;
			name.printf(_T("%s-POS-INTERP"), mNodes.GetNodeName(node));
			Indent(level);
		...
		case KEY_ROT:
			...
			if (newKeys == 0)
				return;
			timeVals = rotTimes;
			name.printf(_T("%s-ROT-INTERP"), mNodes.GetNodeName(node));
			Indent(level);
		...
		case KEY_SCL:
			...
			if (newKeys == 0)
				return;
			timeVals = sclTimes;
			name.printf(_T("%s-SCALE-INTERP"), mNodes.GetNodeName(node));
			Indent(level);
			mStream.Printf(_T("DEF %s PositionInterpolator {\n"), name.data());
			break;
		case KEY_SCL_ORI:
			timeVals = sclTimes;
			name.printf(_T("%s-SCALE-ORI-INTERP"), mNodes.GetNodeName(node));
			Indent(level);
			mStream.Printf(_T("DEF %s OrientationInterpolator {\n"), name.data());
			break;
		case KEY_COLOR:
			...
			if (newKeys == 0)
				return;
			timeVals = posTimes;
			name.printf(_T("%s-COLOR-INTERP"), mNodes.GetNodeName(node));
			Indent(level);
		...
		AddInterpolator(name.data(), type, mNodes.GetNodeName(node));
		// Now check to see if a TimeSensor references this node
		INodeList* l;
		for(l = mTimerList; l; l = l->GetNext()) {
			TimeSensorObject* tso = (TimeSensorObject*)
			...
			for(int j = 0; j < tso->TimeSensorObjects.Count(); j++) {
				...
				if (anim && NodeIsChildOf(node, anim)) {
					...
					_stprintf(timer, _T("%s"),mNodes.GetNodeName(l->GetNode()));
					if (tso->needsScript)
						AddInterpolator(name.data(), KEY_TIMER_SCRIPT, timer);
					else
						AddInterpolator(name.data(), KEY_TIMER, timer);
					break;
				}
			}
		}
	...
	VRML2Export::WriteVisibilityData(INode *node, int level) {
		...
		for(i = 0, t = mStart; i <= frames; i++, t += GetTicksPerFrame()) {
			...
			if (vis != lastVis) {
				mHadAnim = TRUE;
				Indent(level);
				mStream.Write(_T("HideKey_ktx_com {\n"));
				if (mGenFields) {
					Indent(level+1);
					mStream.Write(_T("fields [ SFLong frame] \n"));
				}
				Indent(level+1);
...
static Control* GetController(Object* obj, const TCHAR* name)
{
	...
	if (obj != NULL) {
		...
		one_value_local_tls(prop);			// Keep one local variables
		...
		{
			ProcessMAXScriptException(e, _T("GetController"), false, false, true);
		}
		catch (...)
		{
			ProcessUnknownMAXScriptException(_T("GetController"), false, false, true);
		}
	}
	return c;
}
	...
	VRML2Export::VrmlOutControllers(INode* node, int level)
{
	...
	if (ts != 0) {
		...
		const TCHAR* cycleInterval = floatVal(mCycleInterval);
		mStream.Printf(
	...
	VRML2Export::VrmlOutTopLevelNavInfo(int level, INode* node, BOOL topLevel)
{
	...
	ni->pblock->GetValue(PB_TYPE, mIp->GetTime(), type);
	ni->pblock->GetValue(PB_HEADLIGHT, mIp->GetTime(), headlight);
	ni->pblock->GetValue(PB_VIS_LIMIT, mIp->GetTime(), visLimit);
	ni->pblock->GetValue(PB_SPEED, mIp->GetTime(), speed);
	ni->pblock->GetValue(PB_COLLISION, mIp->GetTime(), collision);
	ni->pblock->GetValue(PB_TERRAIN, mIp->GetTime(), terrain);
	ni->pblock->GetValue(PB_STEP, mIp->GetTime(), step);
	Indent(level);
	...
	VRML2Export::VrmlOutTopLevelBackground(int level, INode* node, BOOL topLevel)
{
	...
	bg->pblock->GetValue(PB_SKY_COLOR1, mIp->GetTime(), col[0]);
	bg->pblock->GetValue(PB_SKY_COLOR2, mIp->GetTime(), col[1]);
	bg->pblock->GetValue(PB_SKY_COLOR3, mIp->GetTime(), col[2]);
	...
	bg->pblock->GetValue(PB_GROUND_COLOR1, mIp->GetTime(), col[0]);
	bg->pblock->GetValue(PB_GROUND_COLOR2, mIp->GetTime(), col[1]);
	bg->pblock->GetValue(PB_GROUND_COLOR3, mIp->GetTime(), col[2]);
	bg->pblock->GetValue(PB_GROUND_COLOR2_ANGLE, mIp->GetTime(), angle2);
	bg->pblock->GetValue(PB_GROUND_COLOR3_ANGLE, mIp->GetTime(), angle3);

	Indent(level+1);
	mStream.Write(_T("groundColor ["));
	for(i = 0; i < numColors + 1; i++)
		mStream.Printf(_T("%s, "), color(col[i]));
	mStream.Write(_T("]\n"));
	...
	VRML2Export::VrmlOutTopLevelFog(int level, INode* node, BOOL topLevel)
{
	...
	float visLimit;
	int type;
	fog->pblock->GetValue(PB_COLOR, mIp->GetTime(), p);
	fog->pblock->GetValue(PB_TYPE, mIp->GetTime(), type);
	fog->pblock->GetValue(PB_VIS_LIMIT, mIp->GetTime(), visLimit);
	Indent(level);
	...
	VRML2Export::VrmlOutAudioClip(int level, INode* node)
{
	...
	ac->pblock->GetValue(PB_AC_PITCH, mIp->GetTime(), pitch);
	ac->pblock->GetValue(PB_AC_LOOP, mIp->GetTime(), loop);
	ac->pblock->GetValue(PB_AC_START, mIp->GetTime(), start);
...
void VRML2Export::VrmlOutFileInfo()
{
	...
	VRML2Export::StartAnchor(INode* node, int& level)
{
	...
	for(l = sb->mSensors; l; l = l->GetNext()) {
		...
		if (obj->ClassID() == AnchorClassID) {
			...
			Indent(level);
			mStream.Write(_T("Anchor {\n"));
			level++;
			Indent(level);
			mStream.Printf(_T("description \"%s\"\n"), ao->description.data());
			int type;
			ao->pblock->GetValue(PB_AN_TYPE, mStart, type);
			if (type == 0) {
				Indent(level);
				...
				if (ao->cameraObject) {
					Indent(level);
					TSTR camName = VRMLName(ao->cameraObject->GetName());
					mStream.Printf(_T("url \"#%s\"\n"), camName.data() );
				}
			}
			Indent(level);
...
VRML2Export::VrmlOutNode(INode* node, INode* parent, int level, BOOL isLOD,
	...
	if (node->IsRootNode()) {
		...
		if (mHasLights && !mHasNavInfo) {
			mStream.Write(_T("NavigationInfo { headlight FALSE }\n"));
		}
	}
	...
	if (!node->IsRootNode()) {
		...
		//At line 5070: mStream.Write(_T("#VRML V2.0 utf8\n\n"));
		if (!mStream.IsFileOpen())
	...
	int frame, TimeValue time)
{
	...
	TSTR buf;
	buf.printf(_T("%d"), frame);
	TSTR wName(filename);
	int extLoc;
	extLoc = wName.last(_T('.'));
	if (extLoc != -1)
		wName.remove(extLoc);
	wName.Append(buf);
	wName.Append(_T(".wrl"));
	...
	mStream.Write(_T("#VRML V2.0 utf8\n\n"));
