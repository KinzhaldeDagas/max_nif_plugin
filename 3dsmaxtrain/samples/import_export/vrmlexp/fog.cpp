...
public:
    ...
    void*			Create(BOOL loading = FALSE) { return new FogObject; }
    const TCHAR*	ClassName() {return GetString(IDS_FOG_CLASS);}
	const TCHAR*	NonLocalizedClassName() { return _T("Fog"); }
	SClass_ID		SuperClassID() { return HELPER_CLASS_ID; }
    Class_ID 		ClassID() { return Class_ID(Fog_CLASS_ID1, Fog_CLASS_ID2); }
...
static const TCHAR* fogTypes[] = { _T("EXPONENTIAL"), _T("LINEAR") };
...
RefResult
FogObject::NotifyRefChanged(const Interval& changeInt,
...
{
    float size;
    pblock->GetValue(PB_FOG_SIZE, t, size);
#include "fogob.inl"    
}
...
FogObject::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags)
{
    if ( ! vpt || ! vpt->IsAlive() )
		...
		float radius;
    pblock->GetValue(PB_FOG_SIZE, t, radius);
    if (radius <= 0.0)
        return 0;
    BuildMesh(t);
    Matrix3 m;
...
int FogCreateCallBack::proc(ViewExp* vpt, int msg, int point, int flags, IPoint2 m, Matrix3& mat)
{
	if (!vpt || !vpt->IsAlive())
	{
		// why are we here?
	...
	case MOUSE_MOVE:
		...
		case 1:
			...
			fogObject->pblock->SetValue(PB_FOG_SIZE, fogObject->iObjParams->GetTime(), radius);
