...
static int MessageBox(int s1, int s2, int option = MB_OK) {
	...
	return MaxSDK::MaxMessageBox(GetActiveWindow(), str1, str2, option);
	}
...
static int MessageBox(const TCHAR *s, int s2, int option = MB_OK) {
	TSTR str1(s);
	TSTR str2(GetString(s2));
	return MaxSDK::MaxMessageBox(GetActiveWindow(), str1, str2, option);
	}
...
void
split_fn(TCHAR *path,TCHAR *file,const TCHAR *pf)
	{
	...
	bs_loc=static_cast<int>(_tcslen(pf));	// SR DCAST64: Downcast to 2G limit.
...
void fin_degammify(Color *col, Color *gamcol) 
{
	if (gammaMgr.IsEnabled()) 
	{
		...
		col->r = deGammaCorrect(gamcol->r, fileInGamma);
		col->g = deGammaCorrect(gamcol->g, fileInGamma);
		col->b = deGammaCorrect(gamcol->b, fileInGamma);
	}
	else 
	{
		*col = *gamcol;
...
int ObjWorker::SetupEnvironment() {
	...
	switch(BG.bgType) {
		...
		case BG_GRADIENT: 
			...
			break;
		case BG_BITMAP: 
			if (_tcslen(BG.bkgd_map)>0) {
				BitmapTex *bmt = NewDefaultBitmapTex();
				bmt->SetMapName(BG.bkgd_map);
...
static Mtl *New3DSDefaultMtl() {
	Mtl *m = NewDefaultMaterial(_T("3DSImport"));
    ...
    auto stdmtl = dynamic_cast<StdMat *>(m);
    if (stdmtl)
        stdmtl->SetShinStr(.707f,0);

	return m;
	}

static BOOL IsSXPName(TCHAR *name) {
	TCHAR fname[30];
	TCHAR ext[5];
	_tsplitpath(name, NULL, NULL, fname, ext );
	return _tcsicmp(ext,_T(".sxp"))==0?1:0;
	}
...
void ObjWorker::AddMeshMtl(SMtl *smtl) {
	...
	m = NewDefaultMaterial(_T("3DSImport"));
    ...
    if (stdmtl)
    {
        ...
        stdmtl->SetFilter(ColorFrom24(smtl->diff), 0);
        stdmtl->SetShading(shade);
        stdmtl->SetShinStr((float)smtl->shin2pct / 100.0f, 0);
        stdmtl->SetOpacity(1.0f - (float)smtl->transparency / 100.0f, 0);
        stdmtl->SetOpacFalloff((float)smtl->xpfall / 100.0f, 0);
        stdmtl->SetFalloffOut(smtl->flags&MF_XPFALLIN ? 0 : 1);
        stdmtl->SetSelfIllum((float)smtl->selfipct / 100.0f, 0);
        stdmtl->SetWireSize(smtl->wiresize, 0);
        stdmtl->SetFaceMap(smtl->flags&MF_FACEMAP ? 1 : 0);
        stdmtl->SetSoften(smtl->flags&MF_PHONGSOFT ? 1 : 0);
        stdmtl->SetWire(smtl->flags&MF_WIRE ? 1 : 0);
        stdmtl->SetTwoSided(smtl->flags&MF_TWOSIDE ? 1 : 0);
        stdmtl->SetTransparencyType(smtl->flags&MF_ADDITIVE ? TRANSP_ADDITIVE : TRANSP_FILTER);
        stdmtl->SetWireUnits(smtl->flags&MF_WIREABS ? 1 : 0);
    }
			...
			if (i==Nrefl) {
				...
				if (i==Ntex2) {
					if (gotTex) {
						...
						Texmap *tm0 = m->GetSubTexmap(GetPrimaryMapSlot(m));
						comp->SetSubTexmap(0,tm0);
						comp->SetSubTexmap(1,txm);
						m->SetSubTexmap(GetPrimaryMapSlot(m),comp);
						if (tm0) 
						...
						m->SetSubTexmap(GetPrimaryMapSlot(m),txm);
                        if (stdmtl)
						    stdmtl->SetTexmapAmt(n,amt,0);
						}
					} 
...
int ObjWorker::GetMatNum(const TCHAR *name) {
	for (int i=0; i<loadMtls.Count(); i++) {
		if (_tcscmp(name,loadMtls[i]->GetName())==0)
			return i;
		}
	assert(0);
	return 0;
...
void ObjWorker::AssignMtl(INode *theINode, Mesh *mesh) {
	...
	// jc 7/10/2006, defect 783101
	// arrays allocated dynamically of size loadMtls.Count()
	// loadMtls is a list of all materials created during the import
	int nloadmtl = loadMtls.Count();
	int* used = new int[nloadmtl];
	int* cused = new int[nloadmtl];
	int* remap = new int[nloadmtl];
...
int
ObjWorker::StartMesh(const TCHAR *iname) {
	if(!FinishUp())
		return 0;
//DebugPrint(_T("Starting mesh %s\n"),iname);
	if(dStream) {
		_ftprintf(dStream,_T("Starting mesh:%s\n"),iname);
		fflush(dStream);
		}
	tm.IdentityMatrix();
	name = TSTR(iname);
...
int
ObjWorker::StartLight(const TCHAR *iname) {
	if(!FinishUp())
		return 0;
	if(dStream) {
		_ftprintf(dStream,_T("Starting mesh:%s\n"),iname);
...
ObjWorker::CreateLight(int type) {
	...
	if(!light) {
		DebugPrint(_T("Light type %d create failure\n"),type);
		return 0;
		}
//DebugPrint(_T("Created light obj %d/%p\n"),type,light);
...
int
ObjWorker::StartCamera(const TCHAR *iname) {
	if(!FinishUp())
		return 0;
	if(dStream) {
		_ftprintf(dStream,_T("Starting mesh:%s\n"),iname);
...
ObjWorker::CreateCamera(int type) {
	...
	if(!camera) {
		DebugPrint(_T("Camera type %d create failure\n"),type);
		return 0;
		}
//DebugPrint(_T("Created camera obj %d/%p\n"),type,camera);
	return 1;
	}
...
ObjWorker::StartKF(ImpNode *node) {
	if(!FinishUp())
		return 0;
	if(dStream) {
		_ftprintf(dStream,_T("Starting KF node\n"));
		fflush(dStream);
		}
	thisNode = node;
	parentNode = NULL;
...
#ifdef DBGPRINT
DebugPrint(_T("WORKER_MESH TM:\n"));
for(int i = 0; i < 4; ++i) {
	Point3 p = tm.GetRow(i);
	DebugPrint(_T("%.4f %.4f %.4f\n"),p.x,p.y,p.z);
	}
#endif	
				Matrix3 itm = Inverse(tm);
#ifdef DBGPRINT
DebugPrint(_T("WORKER_MESH inverse TM:\n"));
for(i = 0; i < 4; ++i) {
	Point3 p = tm.GetRow(i);
	DebugPrint(_T("%.4f %.4f %.4f\n"),p.x,p.y,p.z);
	}
			...
			if(dStream) {
				_ftprintf(dStream,_T("Adding object %s\n"),name.data());
				fflush(dStream);
				}
			Reset();
			return 1;
		...
		case WORKER_KF: {
			...
			if(dStream) {
				_ftprintf(dStream,_T("Finalizing %s\n"),NodeName(thisNode));
				fflush(dStream);
				}
			...
			if(parentNode) {
				if(dStream)
					{
					_ftprintf(dStream,_T("Attaching %s to %s\n"),NodeName(thisNode),NodeName(parentNode));
					fflush(dStream);
					}
		...
		case WORKER_LIGHT: {
			if(dStream) {
				_ftprintf(dStream,_T("Finalizing light %s\n"),NodeName(thisNode));
				fflush(dStream);
				}

			// Must have a light
			if(light) {
//DebugPrint(_T("Light calling AddObject %p (%d) %s\n"),light,lightType,name);
		...
		case WORKER_CAMERA:
			if(dStream) {
				_ftprintf(dStream,_T("Finalizing camera %s\n"),NodeName(thisNode));
				fflush(dStream);
				}
...
ObjWorker::MakeDummy(const TCHAR *name) {
	...
	i->AddNodeToScene(node);
	node->SetName(name);
	AddNode(node,name,OBJ_DUMMY,mesh,_T(""));
	return node;
	}

ImpNode *
ObjWorker::MakeANode(const TCHAR* name, BOOL target, const TCHAR* owner) {
	int type;
	int mtlNum = -1; // -1 is the default of the mtlNum param of ObjWorker::AddNode
	BOOL cstShad = TRUE,rcvShad = TRUE; // default value in max
	Object *obj = NULL;

	// For now, it MUST have an object unless it's a target!
	if(!target) {
		// RB: changed cou
		obj = (Object *)FindObject(name, type, cstShad, rcvShad, mtlNum);
		if(obj == NULL) {
			if(showPrompts)
				MessageBox(name, IDS_DB_NO_OBJECT);
			return NULL;
			}
		UseObject(name);
		}
...
int
ObjWorker::AddNode(ImpNode *node,const TCHAR *name,int type,Mesh *msh,const TCHAR *owner, int mtlNum) {
//DebugPrint(_T("Adding node %s, type %d, mesh:%p\n"),name,type,msh);
...
/*TriObject*/ void *
ObjWorker::FindObject(const TCHAR *name, int &type, int &cstShad, int &rcvShad, int &mtlNum) {
	TSTR wname(name);
	WkObjList *ptr = objects;
	while(ptr) {
...
int
ObjWorker::UseObject(const TCHAR *name) {
	TSTR wname(name);
	WkObjList *ptr = objects;
	while(ptr) {
		if(_tcscmp(ptr->name,wname)==0) {
...
ObjWorker::CompleteScene() {
	...
	while(ptr) {
		if(dStream) {
			_ftprintf(dStream,_T("Found object %s, used:%d\n"),ptr->name.data(),ptr->used);
			fflush(dStream);
			}
		if(!ptr->used) {
			ImpNode *node1, *node2;
			node1 = MakeANode(ptr->name, FALSE, _T(""));
			...
			if (ptr->type==OBJ_SPOTLIGHT || ptr->type==OBJ_CAMERA) {
				...
				// Create a target
				TSTR name = ptr->name + _T(".Target");
				node2 = MakeANode(name, TRUE, _T(""));								
...
ImpNode *
ObjWorker::FindNode(const TCHAR *name) {
	TSTR wname(name);
	WkNodeList *ptr = nodes;
	while(ptr) {
		if(_tcscmp(ptr->name,wname)==0)
...
WkNodeList *
ObjWorker::FindEntry(const TCHAR *name) {
	TSTR wname(name);
	WkNodeList *ptr = nodes;
	while(ptr) {
		if(_tcscmp(ptr->name,wname)==0)
...
ObjWorker::SetNodeId(ImpNode *node,short id) {
	...
	while(ptr) {
		if(ptr->node == node) {
			ptr->id = id;
			if(dStream) {
				_ftprintf(dStream,_T("Setting node ID to %d\n"),id);
				fflush(dStream);
				}
			return 1;
			}
...
ObjWorker::AddKey(KeyList **list,Key *key) {
	if(mode != WORKER_KF) {
		//MaxSDK::MaxMessageBox(NULL, _T("Key add: Not in KF Worker mode"), _T("3DSIMP"), MB_OK);
...
class StudioClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("3D Studio"); }
...
class StudioShapeClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("3D Studio Shape"); }
	SClass_ID		SuperClassID() { return SCENE_IMPORT_CLASS_ID; }
...
MaxSDK::MaxMessageBox(NULL, buf, _T("3DSIMP"), MB_OK);
}
#endif // DUMPING
			...
			while(chunk.size) {
				...
				switch(next.tag)   	{
					...
					case SPOTLIGHT_NODE_TAG:
						nodeLoadNumber++;
						if(dStream) {
							_ftprintf(dStream,_T("nodeLoadNumber:%d\n"),nodeLoadNumber);
							fflush(dStream);
							}
...
//						nodetag = next.tag;
		...
		case SPOTLIGHT_NODE_TAG: 
			...
			break;
		case NODE_ID:
			//MaxSDK::MaxMessageBox(NULL, _T("Reading node id"), _T("3dsimp"), MB_OK);
			RDSHORT(&cur_node_id);	
			if(dStream) {
				_ftprintf(dStream,_T("Got Node ID:%d\n"),cur_node_id);
				fflush(dStream);
				}
			break;
		case NODE_HDR: {
			//MaxSDK::MaxMessageBox(NULL, _T("Reading node hdr"), _T("3dsimp"), MB_OK);
			USHORT npar;
			ImpNode *pnode;
			...
			obname[10] = 0;
			TCHAR prefix[32];
			TSTR Wname = TSTR::FromCP(codePage, obname);
			if(suffix) {
				_tcscpy(prefix,Wname);	
				Wname += TSTR::FromCP(codePage, suffix);
				}
			else
				prefix[0] = 0;

//DebugPrint(_T("Got node %s\n"),obname);

			if (strcmp(obname,"$_$_$_$")==0) {
				//MaxSDK::MaxMessageBox(NULL, _T("Skipping $_$_$_$ object"), _T("3DSIMP"), MB_OK);
				goto skip_node;
				}
			...
			if (strcmp(obname,"DUMMY")==0||strcmp(obname,"$$$DUMMY")==0) {
				...
				obnode = theWorker->MakeANode(Wname,suffix ? TRUE : FALSE, prefix);
				if(obnode == NULL)
					return 0;
			...
			if(npar != NO_PARENT) {
				if(dStream) {
					_ftprintf(dStream,_T("Parent's node id:%d\n"),npar);
					fflush(dStream);
					}
...
//		  	FreeNode(node);
		...
		case BOUNDBOX:	{
				...
				if(dStream) {
					_ftprintf(dStream,_T("Got dummy bounds\n"));
					fflush(dStream);
					}
				chunk.size = 0;
				}
		...
		case INSTANCE_NAME:	{
				...
				iname[10] = 0;
				theWorker->SetInstanceName(obnode, TSTR::FromCP(codePage, iname).data());
				chunk.size = 0;
				}
			break;
		case POS_TRACK_TAG:
...
//					case SHAPE_NOT_OK:
		...
		case SYSTEM_UNIT_SCALE: {
			...
			float susc_factor = (float)GetSystemUnitScale(autoConv ? UNITS_INCHES : type);
			msc_wk = (float)((double)msc_wk / susc_factor);		/* Turn into a mult factor */
			goto skiprest;
			}
//	#ifdef LATER
		case SOLID_BGND:
		...
		case BIT_MAP: {
			...
			split_fn(NULL,BG.bkgd_map,TSTR::FromCP(codePage, buf).data());
			if(_tcsicmp(BG.bkgd_map,_T("none"))==0)
				BG.bkgd_map[0]=0;
			}
		...
		case NAMED_OBJECT:
			...
			while(chunk.size)
				...
				switch(next.tag)
					{
					case N_TRI_OBJECT:
						if(!theWorker->StartMesh(TSTR::FromCP(codePage, obname).data()))
							return 0;
...
//						d.exclude=NULL;
						...
						d.mult=1.0f;
						d.appdata = NULL;
						if(!theWorker->StartLight(TSTR::FromCP(codePage, obname).data()))
							return 0;
						if(get_mchunk(&d)==0)
							return 0;
					...
					case N_CAMERA: {
						...
						c.farplane=1000.0f;
						c.appdata = NULL;
						if(!theWorker->StartCamera(TSTR::FromCP(codePage, obname).data()))
							return 0;
						if(get_mchunk(&c)==0)
							return 0;
...
//			 	antiquated=1;
			...
			mtlnum = theWorker->GetMatNum(TSTR::FromCP(codePage, mtlname).data());
...
//#ifdef LATER
		case DL_EXCLUDE: {
			...
			//	return(0);
			d->excList.AddName(TSTR::FromCP(codePage, oname).data());
			goto skiprest;
			}
