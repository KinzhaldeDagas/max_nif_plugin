...
public:
	...
	int				StartMesh(const TCHAR *name);
	int				StartLight(const TCHAR *name);
	int				CreateLight(int type);
	int				StartCamera(const TCHAR *name);
	...
	int				AddNode(ImpNode *node,const TCHAR *name,int type,Mesh *mesh,const TCHAR *owner,int mtlNum=-1);
	int				SetNodeId(ImpNode *node,short id);
	int				SetNodesParent(ImpNode *node,ImpNode *parent);
	void *			FindObject(const TCHAR *name, int &type, int &cstShad, int &rcvShad, int &mtlNum);
	int				UseObject(const TCHAR *name);
	int				CompleteScene();
	int 			SetupEnvironment();
	int 			FixupExclusionLists();
	ImpNode *		FindNode(const TCHAR *name);
	ImpNode *		FindNodeFromId(short id);
	WkNodeList *	FindEntry(const TCHAR *name);
	...
	const TCHAR *	NodeName(ImpNode *node);
	void			FreeObjList();
	void			FreeNodeList();
	...
	ImpNode *		MakeANode(const TCHAR *name, BOOL target,const TCHAR *owner);
	...
	int 			GetMatNum(const TCHAR *name);
