...
template <>
constexpr int MaxSDK::ColorManagement::ImageLayoutInfo::NumChan<Color_24>()
{
	return 3;
}
template <>
constexpr MaxSDK::ColorManagement::ImageLayoutInfo::ChannelType ImageLayoutInfo::ChanType<Color_24>()
{
	return ChannelType::type_uint8;
}
template <>
constexpr MaxSDK::ColorManagement::ImageLayoutInfo::ChannelOrder ImageLayoutInfo::ChanOrder<Color_24>()
{
...
extern void EightDotThreeName(TSTR& name, bool addToMsgList);  // defined in 3dsexp.cpp
...
BOOL isSXPname(TCHAR *name) {
	TCHAR ext[5];
	_tcsncpy(ext, name+_tcslen(name)-4, 5);
	_tcsupr(ext);
	return(_tcscmp(_T(".SXP"),ext)==0);
	}
		...
		case MAT_SPECULAR: 
			{
				...
				auto conv = pipe
						? pipe->GetColorConverter<Color_24, Color_24>(IColorPipeline::FLAG_OPTIMIZATION_LOSSLESS)
						: nullptr;

				c24 = (Color_24*)data;
				Color_24 gc = conv ? conv->ConvertColor(*c24) : *c24;
				if (dump_mtlchunk(COLOR_24, stream, &gc) == 0)
					return (0);
				if (dump_mtlchunk(LIN_COLOR_24, stream, data) == 0)
					return (0);
			}
			break;
		case MAT_SHININESS:
