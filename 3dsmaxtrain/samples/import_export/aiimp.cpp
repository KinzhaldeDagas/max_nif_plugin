...
class AIShapeClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Adobe Illustrator Shape"); }
...
BOOL AIShapeImport::FinishWorkingShape(BOOL forceFinish, ImpInterface *i) {
	...
	if(spline) {
		...
		if(knots < 2) {
			singularity:
//			DebugPrint(_T("WARN: spline w/%d points\n"),knots);
...
AIShapeImport::DoImport(const TCHAR *filename,ImpInterface *i,Interface *gi, BOOL suppressPrompts) {
	...
	float scaleFactor = float((1.0 / suScale) / 72.0);
...
loop:
	...
	#ifdef DBG1
	DebugPrint(_T("Curveto buffer:%s\n"),buffer);
	#endif
						goto bad_file;
						}
	...
	#ifdef DBG1
	DebugPrint(_T("Current/vec buffer:%s\n"),buffer);
	#endif
						goto bad_file;
						}
					Point3 p(x3 * scaleFactor, y3 * scaleFactor, 0.0f);
	...
	#ifdef DBG1
	DebugPrint(_T("vec/next buffer:%s\n"),buffer);
	#endif
						goto bad_file;
						}
					int lastKnot = spline->KnotCount() - 1;
