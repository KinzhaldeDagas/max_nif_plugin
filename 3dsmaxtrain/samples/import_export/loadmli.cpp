...
get_mapdata_chunk(FILE *stream, int tag, MapData *md) {
	...
	switch (tag) {
		case MAT_MAPNAME:
			...
			// assume we ALWAYS get an ASCII string from 3ds files using max. 12 chars 
			// ( 8.3 filename format ),plus  '\0' termination = 13
			// so read it into a char *
			char aName[13];
			if (!get_mtlchunk(stream, aName )) {
					return(0);
				}
			_tcscpy_s( md->name,_countof(md->name), TSTR::FromCP(codePage, aName).data());

#ifdef DBGLDMLI
		if (dbgldmli)
				_tprintf(_T("  get_map : mapname: %s \n"),md->name);
#endif
			return(1);
		case MAT_MAP_TILING: 
			...
			while(chunk.size)
				...
				switch(nexttag)
					...
					case MAT_NAME:
						strlimit=17;
						char name[17];
						if(get_mtlchunk(stream,name)==0)
							return(0);
						_tcscpy(loadmtl->name, TSTR::FromCP(codePage, name).data());
#ifdef DBGLDMLI
						if (dbgldmli)
							_tprintf(_T(" **** Loading material : %s \n"), loadmtl->name);
#endif
