...
public:
	...
	const TCHAR *	LongDesc();					// Long TCHAR description (i.e. "Autodesk 3D Studio File")
	const TCHAR *	ShortDesc();				// Short TCHAR description (i.e. "3D Studio")
	const TCHAR *	AuthorName();				// TCHAR Author name
	const TCHAR *	CopyrightMessage();			// TCHAR Copyright message
...
class AIClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Adobe Illustrator"); }
...
const TCHAR *
AIExport::LongDesc() {			// Long TCHAR description (i.e. "Targa 2.0 Image File"))
	return GetString(IDS_TH_AI_EXP_LONG);
	}
	
const TCHAR *
AIExport::ShortDesc() {			// Short TCHAR description (i.e. "Targa"))
	return GetString(IDS_TH_AI_EXP_SHORT);
	}

const TCHAR *
AIExport::AuthorName() {			// TCHAR Author name
...
ai_save(const TCHAR *filename, ExpInterface *ei, Interface *gi, AIExport *exp) {
	...
	double suScale = GetSystemUnitScale(UNITS_INCHES);
	float scaleFactor = float(suScale * 72.0);
...
AIExport::DoExport(const TCHAR *filename,ExpInterface *ei,Interface *gi, BOOL suppressPrompts, DWORD options) {
	...
	return ai_save(filename, ei, gi, this);
	}
