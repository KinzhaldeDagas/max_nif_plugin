...
static void MessageBox(int s1, int s2) {
	...
	MaxSDK::MaxMessageBox(GetActiveWindow(), str1.data(), str2.data(), MB_OK);
	}
...
int  MeshMtlList::FindSName(TCHAR *name) {
	for (int i=0; i<Count(); i++) 
		if  ( _tcscmp(name,(*this)[i].sm->name)==0) return i;
	return -1;
	}
...
void EightDotThreeName(TSTR& name, bool addToMsgList) {
	TSTR fExt;
	TSTR fName;
	int  oExt;
	if (name.Length() > 12) {
		...
		_tcsncpy(fName.dataForWrite(), name.data(), OFFSET(oExt));
		if (oExt != -1) {
			fExt.Resize(4);
			_tcsncpy(fExt.dataForWrite(), name.data() + oExt, 4);
		}
		fName.Append(fExt);
		if (fName.Length() > 12)
			fName.Resize(12);
		if (addToMsgList) {
			TSTR buf;
			buf.printf(GetString(IDS_EIGHT_DOT_THREE), name.data(), fName.data());
	...
	if (m->IsSubClassOf( Class_ID(DMTL_CLASS_ID,0) ) ) {
		...
		for (int i=0; i<8; i++) {
         ...
         // gdf convert bitmap filename to 8.3
            EightDotThreeName(name, true);

         // gdf convert to Uppercase
            name.toUpper();

			_tcsncpy(mp->map.name,name.data(),12);
			if (i==Nrefl) {
...
static void IncrName(TCHAR *name, TCHAR *inm, int n) {
	TCHAR buf[20];
	_tcsncpy(buf,name,13);
	buf[13] = 0;
	_stprintf(inm,_T("%s%d\0"),buf,n);
	}
...
void MeshMtlList::ReallyAddMtl(Mtl *m) {
	...
	me.sm = s;
	TCHAR buf[20];
	_tcsncpy(buf,me.m->GetName(),16);
	if (_tcslen(buf)==0) _tcscpy(buf, _T("Matl"));
	buf[16] = 0;
	_tcscpy(s->name,buf);
	int n=0;
	...
	switch (obj->SuperClassID()) { 
		...
		case LIGHT_CLASS_ID: {
            ...
            if (obj->ClassID()==Class_ID(DIR_LIGHT_CLASS_ID,0)) {
                _stprintf(buf, GetString(IDS_DIR_LIGHT), node->GetName());
		        AddToMsgList(msgList, buf);
				//Append(node, obj, OBTYPE_SPOTLIGHT);
            }
            if (obj->ClassID()==Class_ID(FSPOT_LIGHT_CLASS_ID,0)) {
                _stprintf(buf, GetString(IDS_FSPOT_LIGHT), node->GetName());
		        AddToMsgList(msgList, buf);
				//Append(node, obj, OBTYPE_SPOTLIGHT);
            }
            break;
...
INode *ObjectList::FindLookatNode(INode *node) {
	...
	switch(tag)
		...
		case MMAGIC:	/* Mesh editor */
            ...
            for (ix=0; ix<(*theMtls).Count(); ix++)  {
                if (ix == 256) {
                    TCHAR buf[ERROR_MSG_MAX_LEN];
                    _stprintf(buf, GetString(IDS_MATERIAL_LIMIT));
		            AddToMsgList(msgList, buf);
...
#endif
			if(dump_mchunk(SYSTEM_UNIT_SCALE,stream,NULL)==0)
				return(0);
				...
				if (type!=OBTYPE_DUMMY&&type!=OBTYPE_LTARGET&&type!=OBTYPE_CTARGET) {
					if (type==OBTYPE_MESH) {
                        ...
                        if (mesh.numFaces > 65530) {
							TCHAR buf[ERROR_MSG_MAX_LEN];
                            _stprintf(buf, GetString(IDS_OBJ_TOO_MANY_FACES), oe->entry->name);
							AddToMsgList(msgList, buf);
							continue;
						}
						if (mesh.numVerts > 65530) {
							TCHAR buf[ERROR_MSG_MAX_LEN];
                            _stprintf(buf,GetString(IDS_OBJ_TOO_MANY_VERTS), oe->entry->name);
							AddToMsgList(msgList, buf);
							continue;
						}
                    }
...
//					return(0);
		...
		case SYSTEM_UNIT_SCALE: {
			...
			float susc_factor = (float)GetSystemUnitScale(type);
			WRTERR(&susc_factor,sizeof(float));
			}
			break;
//		case SOLID_BGND:
//			if (P.gamma_correct) {
...
//			break;
		case NAMED_OBJECT: {
			...
			CStr name = oe->entry->name.ToCP(codePage).data();
			if(name.Length()>10)
				name.Resize(10);
			        ...
			        if ((mtl = mt.node->GetMtl()) != NULL) {
				        if (IsStdMulti(mtl)) {
					        ...
					        for (int i = 0; i < mtl->NumSubMtls(); i++) {
						        ...
						        if (sub) {
							        mtlnum = mtlNumMap[i];
									if (mtlnum!=-1)// jc 7/10/2006, defect 783102
									// call dump_mchunk to export the chunk MSH_MAT_GROUP 
									// only if the material was found in theMtls
									{
...
//			break;
		...
		case MSH_MAT_GROUP:
			...
			Interface14 *iface = GetCOREInterface14();
			UINT codePage = iface->DefaultTextSaveCodePage();
			TSTR mtlName = mtl->name;
			CStr name = mtlName.ToCP(codePage).data();
			if(name.Length()>16)
				name.Resize(16);
			WRTERR(name.data(),(size_t)(name.Length()+1));
...
//				} 
		...
		case DL_SPOTLIGHT: {
            ...
            if (obj->ClassID()==Class_ID(DIR_LIGHT_CLASS_ID,0)) {
                TCHAR buf[ERROR_MSG_MAX_LEN];
                _stprintf(buf, GetString(IDS_DIR_LIGHT), oe->entry->name);
		        AddToMsgList(msgList, buf);
                break;
            }
            ...
            if (obj->ClassID()==Class_ID(FSPOT_LIGHT_CLASS_ID,0)) {
                TCHAR buf[ERROR_MSG_MAX_LEN];
                _stprintf(buf, GetString(IDS_FSPOT_LIGHT), oe->entry->name);
		        AddToMsgList(msgList, buf);
                break;
            }
			LightObject *lt= (LightObject *)obj;
...
int	KFWriteChunk(unsigned short tag, void *data)	{
	...
	switch(tag)	{
		...
		case BOUNDBOX:	 {
			Box3 bb;
			ViewExp& vpt = theSceneEnum->i->GetViewExp(NULL);
			if ( ! vpt.IsAlive() )
			{
				// why are we here
				DbgAssert(!_T("Invalid viewport!"));
			...
			SceneEntry *se = (SceneEntry *)data;
			se->obj->GetLocalBoundBox(theSceneEnum->i->GetTime(), se->node, vpt.ToPointer(), bb);
			WR3FLOAT(&bb.pmin[0]);
			WR3FLOAT(&bb.pmax[0]);
			}
			break;
		...
		case NODE_HDR: {
			...
			if (se->type == OBTYPE_DUMMY) {
				...
				INode *lan = theObjects->FindLookatNode(se->node);
				ObjectEntry *oe = theObjects->Contains(lan);
				...
				{
					// in case we cannot find lookat node, possibly
					// "Export Selected" is chosen
					name = lan->GetName();
				...
				CStr a_name = name.ToCP(codePage).data();
				if(a_name.Length()>10)
					a_name.Resize(10);
				WERR(a_name.data(),(size_t)(a_name.Length()+1));
				}
			else {
				TSTR name; 
				...
				if (se->type != OBTYPE_MESH) {
					...
					// If the name is not equal to the scene entry name then
					// it is an instance.
			...
			if(pse) {
				...
				short none = -1; //(short)NO_PARENT; Casting NO_PARENT (i.e. 0xFFFF) to short made it -1. How wierd.
				WRSHORT(&none);
				}
			}
			break;
...
//			if (on->appdata) if (!KFWriteChunk(APP_DATA,on->appdata)) return(0);
			...
			// We don't write an instance name if the instance name is the same as the original object's
			// name.
			ObjectEntry *oe = theObjects->Contains(ob);
			assert(oe);
			TSTR mname(oe->entry->name);		// original name
			if (se->type!=OBTYPE_CTARGET&&se->type!=OBTYPE_LTARGET) {
				if (se->type==OBTYPE_DUMMY || !(mname == name)) { 
					...
					UINT codePage = iface->CodePageForLanguage(langID);
					CStr a_name = name.ToCP(codePage).data();
					if (!KFWriteChunk(INSTANCE_NAME,&a_name)) return(0); 
					}
				}
...
_3DSExport::DoExport(const TCHAR *filename,ExpInterface *ei,Interface *gi, BOOL suppressPrompts, DWORD options) {
	...
	int status = _3ds_save(filename, ei, gi, this);
...
// gdf
void
AddToMsgList(Tab<TSTR*>& mList, const TCHAR* msg)
{
    TCHAR buf[ERROR_MSG_MAX_LEN];
    _tcsncpy(buf, msg, ERROR_MSG_MAX_LEN);
    buf[ERROR_MSG_MAX_LEN - 1] = _T('\0');
...
ConvertTo3DSTVerts(ObjectEntry* oe, Tab<UVVert>& tv)
{
    ...
    if (mesh.getNumVerts() != mesh.getNumTVerts()) {
        TCHAR buf[ERROR_MSG_MAX_LEN];
        _stprintf(buf, GetString(IDS_UVVERT_MISMATCH), oe->entry->name);
		AddToMsgList(msgList, buf);
        wMsg = TRUE;
    }
	for (int i = 0; i < mesh.getNumFaces(); i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (vWritten[face.v[j]] && !UVVertEqual(tv[face.v[j]], tvert)) {
                if (!wMsg ) {
                    TCHAR buf[ERROR_MSG_MAX_LEN];
                    _stprintf(buf, GetString(IDS_UVVERT_TOO_MANY_PER_VERT), oe->entry->name);
		            AddToMsgList(msgList, buf);
                    wMsg = TRUE;
                }
            }
...
void
ConvertTo3DSFaces(ObjectEntry* oe, MaxSDK::Array<Vert3ds>& verts, Tab<Face3ds>& faces)
{
    Face3ds tmpFace;
    Vert3ds tmpVert;
    Mesh mesh = oe->tri->GetMesh();
    ...
    verts.setLengthUsed(mesh.getNumVerts());
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (vWritten[face.v[j]]) {
                if (!UVVertEqual(verts[face.v[j]].tv, tmpVert.tv)) {
                    ...
                    verts.insertAt(verts.length(), tmpVert);
                }
            } else {
