...
private:
      virtual void SetReference(int i, RefTargetHandle rtarg) {cont = (IMCControl*)rtarg;}
public:
      ...
      virtual void UpdateRollup(IRollupWindow* iRoll) override;
      void UpdateRollup(HWND hWnd);
...
class JoyDeviceClassDescOld:public ClassDesc {
   ...
   const TCHAR*	 NonLocalizedClassName() { return _T("Joystick Input Device"); }
...
void JoyDeviceBinding::UpdateRollup(HWND hWnd)
{
	DbgAssert(hWnd);
	if (hWnd) {
...
void JoyDeviceBinding::DoPickCont(HWND hWnd)
   ...
   if (GetCOREInterface()->TrackViewPickDlg(hWnd,&res, &filt)) {
      if (ReplaceReference(0,res.anim)!=REF_SUCCEED) {
         MaxSDK::MaxMessageBox(hWnd, _T("Can't create circular reference."), GetString(IDS_RB_JOYDEVICE), MB_OK | MB_ICONEXCLAMATION);
         }
      UpdateContName(hWnd);
      SetStates(hWnd);
      }
...
static INT_PTR CALLBACK JoyDeviceDlgProc(
   ...
   if (!dlg && msg != WM_INITDIALOG) 
	   return FALSE;
   JoyDeviceBinding* b = NULL;
   if (dlg) 
	   b = (JoyDeviceBinding*)dlg->binding;

   switch (msg) {
      case WM_INITDIALOG:
		  dlg = (IMCParamDlg*)lParam;
		  b = static_cast<JoyDeviceBinding*>(dlg ? dlg->binding : nullptr);
		  if (b) {
			  b->UpdateRollup(hWnd);
		  }
...
void JoyDevice::ZeroJoy()
   ...
   switch (res) {
      case JOYERR_UNPLUGGED:
         MaxSDK::MaxMessageBox(NULL, _T("Joystick is unplugged"), _T("Joy Stick"), MB_OK);
         error = TRUE;
         break;

      case MMSYSERR_NODRIVER:
         MaxSDK::MaxMessageBox(NULL, _T("No joystick driver present."), _T("Joy Stick"), MB_OK);
         error = TRUE;
         break;

      case MMSYSERR_INVALPARAM:
         MaxSDK::MaxMessageBox(NULL, _T("Invalid parameter."), _T("Joy Stick"), MB_OK);
         error = TRUE;
         break;
      }
   xbase = ji.dwXpos;
