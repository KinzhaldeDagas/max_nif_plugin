...
class RotationMC : public MCControl {
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_RB_ROTMC) : _T("Rotation Motion Capture");}
		...
		void EditTrackParams(
			TimeValue t, ParamDimensionBase *dim, const TCHAR *pname, 
...
class RotMCClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Rotation Motion Capture"); }
	SClass_ID		SuperClassID() {return CTRL_ROTATION_CLASS_ID;}
	Class_ID		ClassID() {return rotmcControlClassID;}
