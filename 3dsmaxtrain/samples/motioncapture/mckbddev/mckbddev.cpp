...
class KbdDeviceBinding : public MCDeviceBinding {
	public:
		...
		virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("KbdDeviceBinding"); } // from Animatable
		...
		const TCHAR*	GetVKeyName(USHORT vKey);
};
...
class KbdDeviceBindingClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Keyboard Input Device"); }
...
TSTR KbdDeviceBinding::BindingName() 
{
	...
	if (nVKey == 0) {
		bindStr.printf(_T("%s: %s"), keyStr, GetString(IDS_KBD_UNASSIGNED));
	}
	else {
		bindStr.printf(_T("%s: %s"), keyStr, GetVKeyName(nVKey));
	}
...
const TCHAR* KbdDeviceBinding::GetVKeyName(USHORT vKey)
{
...
static INT_PTR CALLBACK KbdDeviceDlgProc(
	...
	switch (msg) {
		case WM_INITDIALOG:
			...
			DLSetWindowLongPtr(GetDlgItem(hWnd,IDC_KBD_ENVGRAPH),lParam);
			break;
...
LRESULT CALLBACK newButtonProc(HWND hButton, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	switch(iMsg) {
	case WM_CREATE:
		kFlags = vKey = 0;
...
INT_PTR CALLBACK KeyPressProc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    switch (iMsg)  {
		case WM_INITDIALOG:
			...
			oldButtonProc = DLGetWindowProc(GetDlgItem(hDlg,ID_FILE_EXIT));
			DLSetWindowProc(GetDlgItem(hDlg, ID_FILE_EXIT), newButtonProc);
...
void KbdDeviceBinding::AssignKey(HWND hParent)
{
	if (DialogBox(hInstance, MAKEINTRESOURCE(IDD_KBD_KEYPRESSDLG), hParent , KeyPressProc)) {
		nVKey = vKey;
		SendMessage(GetDlgItem(hParent, IDC_KBD_KEYEDIT), WM_SETTEXT, 0, (LPARAM)BindingName().data());
	}
}
