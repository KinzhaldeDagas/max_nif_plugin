...
class MouseDeviceClassDescOld:public ClassDesc {
   ...
   const TCHAR*	NonLocalizedClassName() { return _T("Mouse Input Device"); }
...
void MouseDeviceBinding::UpdateRollup(HWND hWnd)
{
	DbgAssert(hWnd);
   ...
   if (!dlg && msg != WM_INITDIALOG) 
	...
	switch (msg) {
		case WM_INITDIALOG:
			...
			if (MouseDeviceBinding* b = static_cast<MouseDeviceBinding*>(dlg ? dlg->binding : nullptr))
				b->UpdateRollup(hWnd);
