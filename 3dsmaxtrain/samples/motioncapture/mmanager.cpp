...
TSTR MCControl::SubAnimName(int i, bool localized)
{
	if (trackName.length())
	{
		TSTR buf = localized ? GetString(IDS_RB_DATA) : _T("Data");
		buf = buf + TSTR(_T(" (")) + trackName + TSTR(_T(")"));
		return buf;
	}
	else
	{
		return localized ? GetString(IDS_RB_DATA) : _T("Data");
	}
}
...
BOOL MCControl::ControllerInScene()
   ...
   DoEnumDependents(&proc);
   return proc.ct>0;
   }

void MCControl::RefDeleted(RefMakerHandle rm)
   {
...
RefResult MCControl::AutoDelete()
   {
   RefDeleted(nullptr); // should not be needed in 2024 since RefDeleted now always called when ref dropped, leaving for safety.
...
void MCControl::RefDeletedUndoRedo(RefMakerHandle rm)
   {
   if (!ControllerInScene()) theMM.UnRegister(this);
   }
...
class MotionManClassDesc:public ClassDesc {
   ...
   const TCHAR*	NonLocalizedClassName() { return _T("Motion Capture"); }
...
class FindSubName : public DependentEnumProc {
   public:     
	  ...
	  int proc(ReferenceMaker *rmaker) {
		  if (rmaker==anim) return DEP_ENUM_CONTINUE;
		  for (int i=0; i<rmaker->NumSubs(); i++) {
			  if (rmaker->SubAnim(i)==anim) {
				  name = rmaker->SubAnimName(i, true);
				  return DEP_ENUM_HALT;
			  }
		  }
...
void MotionManager::SetupList()
   ...
   for (int i=0; i<cont.Count(); i++) {
      ...
      SendDlgItemMessage(
         hPanel,IDC_MM_TRACKS,LB_ADDSTRING,0,(LPARAM)(const TCHAR*)name);
...
void MotionManager::DrawTrack(
   ...
   if (index>=0) {
      ...
      DLTextOut(hdc,rect.left+10,rect.top,name);
      ImageList_Draw(
         hTrackIcons,
...
void MotionManager::CheckMessages()
   {
	   ...
	   // allow mouse messages to pass through
	   while (PeekMessage(&msg, nullptr, WM_MOUSEFIRST, WM_MOUSELAST, PM_REMOVE) && !exitTriggered)
	   {
		   // flag for exit when right mouse click is detected
		   ...
		   // Don't trap left clicks to record controls.
		   if (GetParent(msg.hwnd) == hPanel)
		   {
			   LONG_PTR id = GetWindowLongPtr(msg.hwnd, GWLP_ID);
	   ...
	   if (exitTriggered)
	   {
		   //	this is necessary to flush out any other messages in the queue.
		   //	without this, 3ds max will end up registering keystrokes that may result in unwanted behavior
...
void MotionManager::TrackParams(HWND hWnd,int index)
   ...
   if (index>=0 && index<cont.Count()) {
      cont[index]->EditTrackParams(
         0, NULL, ControlName(index).data(), hWnd, (IObjParam*)ip, 0);
      }
   }
...
static BOOL CALLBACK EnumChildren(HWND hwnd, LPARAM lParam)
   {  
   NamedSelWndProc = DLSetWindowLongPtr(hwnd, NamedSelSubWndProc);
   return FALSE;
   }
...
void MotionManager::Start() 
   ...
   if (!any) {
      ...
      MaxSDK::MaxMessageBox(hPanel, buf1, buf2, MB_OK | MB_ICONEXCLAMATION);
      Stop();
      return;
      }
...
class CaptureStatus : public KeyReduceStatus {
   public:     
      ...
      CaptureStatus(Interface *i, const TCHAR *t) {ip=i;title=t;pp=0;canceled=FALSE;}
	  ...
	  KeyReduceResult Progress(int p) override {
...
void MCParamDlg::DoWindow(HWND hParent, const TCHAR *pname)
   {
...
static INT_PTR CALLBACK MCContParamDlgProc(
   ...
   MCParamDlg *dlg = DLGetWindowLongPtr<MCParamDlg*>(hWnd);
   ...
   for (int i=0; i<cont->NumDeviceBindings(); i++) {
      if (cont->GetDeviceBinding(i)) {
         SetDlgItemText(hPanel,bindingButtons[i],
            cont->GetDeviceBinding(i)->BindingName().data());
      } else {
         SetDlgItemText(hPanel,bindingButtons[i],
...
static INT_PTR CALLBACK GenMCDlgProc(
      HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
   {
   GenMCParamDlg *dlg = DLGetWindowLongPtr<GenMCParamDlg*>(hWnd);
   if (!dlg && msg!=WM_INITDIALOG) return FALSE;
