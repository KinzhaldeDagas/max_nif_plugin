...
namespace {
void PromptMessage(TSTR& message)
{	
	GetCOREInterface()->DisplayTempPrompt(message, 3000);
}
}
...
class MidiDeviceClassDescOld:public ClassDesc {
   ...
   const TCHAR*	 NonLocalizedClassName() { return _T("MIDI Device"); }
   ...
   switch (trigger) {
      case IDC_MIDI_NOTE: {
         ...
         if (note >= 0) {
             ...
             if (speed == 0) // Speed 0 = instant = just assign value
             {
                 curVal = newVal;
             }
             else
...
void MidiDeviceBinding::UpdateRollup(HWND hWnd)
{
	DbgAssert(hWnd);
...
static INT_PTR CALLBACK MidiDeviceDlgProc(
   ...
   if (!dlg && msg != WM_INITDIALOG)
	   return FALSE;
   if (dlg)
	   b = (MidiDeviceBinding*)dlg->binding;

   switch (msg) {
      case WM_INITDIALOG:
		  dlg = (IMCParamDlg*)lParam;
		  b = static_cast<MidiDeviceBinding*>(dlg? dlg->binding: nullptr);
		  if (b) {
			  b->InitDlg(hWnd);
			  b->UpdateRollup(hWnd);
		  }
...
void MidiDevice::Open()
{
	...
	{
		TSTR buf = GetString(IDS_RB_NODEVICE);
		PromptMessage(buf);
		noDevice = TRUE;
	}
}
...
void MidiDevice::Close()
   ...
   if (!noDevice && res!=MMSYSERR_NOERROR) {
      ...
      MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), buf1, buf2, MB_ICONSTOP | MB_OK | MB_APPLMODAL);
      }
   }
