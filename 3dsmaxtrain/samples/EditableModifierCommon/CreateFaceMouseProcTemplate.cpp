...
int CreateFaceMouseProcTemplate::proc(HWND in_hwnd, int in_msg, int in_point, int in_flags, IPoint2 in_m ) {
	...
	ViewExp& l_vpt = m_interface->GetViewExp(in_hwnd);
	if ( ! l_vpt.IsAlive() )
	{
		// why are we here
...
void CreateFaceMouseProcTemplate::Finish(ViewExp& in_vpt) {
	...
	if (l_facePoints >= 3 && CreateFace(l_vertIndices)) {
		...
		if (l_facePoints >= 3) {
			MaxSDK::MaxMessageBox(m_interface->GetMAXHWnd(), GetFailedFaceCreationMessage(), GetFailedFaceCreationTitle(), MB_OK | MB_ICONINFORMATION);
		}
...
void CreateFaceMouseProcTemplate::DrawCreatingFace(HWND in_hwnd, bool bErase, bool bPresent) {
	...
	ViewExp& vpt = m_interface->GetViewExp(in_hwnd);
	Matrix3 affineMat;
	vpt.GetAffineTM(affineMat);
	Point3 affinePos = affineMat * m_points[0].m_worldLocation;
	Point2 tempPos = vpt.MapViewToScreen(affinePos);
	IPoint2 screenPos = IPoint2(tempPos.x, tempPos.y);

	Tab<IPoint2> points;
	points.Append(1,&screenPos,5);
	points.Append(1,&m_mouseLocation,5);

	if (points.Count() > 1) {
		affinePos = affineMat * m_points.last().m_worldLocation;
		tempPos = vpt.MapViewToScreen(affinePos);
		screenPos = IPoint2(tempPos.x, tempPos.y);
		points.Append(1, &screenPos, 1);
	}
...
void CreateFaceMouseProcTemplate::DrawEstablishedFace(GraphicsWindow* in_gw) {
	...
	if (nitrousViewport)
	{
		in_gw->multiplePass(-1, TRUE, 1.020f);
	}
