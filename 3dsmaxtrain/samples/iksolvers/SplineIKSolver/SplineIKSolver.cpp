...
#define EPSILON1_FACTOR 150.0f	// Was 5
#define EPSILON2_FACTOR 1500.0f	// Was 50
#define EPSILON3_FACTOR 3000.0f	// Was 100
...
class SplineIKSolver : public IKSolver { //Added after automatic generation of the project
	...
	public:
		...
		float chainLength; //sum of the bone lengths
		float splLen; //total spline length
		...
		*/
		...
		inline double acos_safe(double x) const
		{
			return x <= -1.0 ? PI :
			x >= 1.0 ? 0.0 : acos(x);
		}

		inline double acos_safe(float x) const
		{
			return x <= -1.0f ? PI :
		...
		void GetClassName(MSTR& s) const {s = GetString(IDS_CLASS_NAME);}
...
//					DebugPrint(_T("Mouse cycle started should lower the tolerances\n"));
				break;

				case REFMSG_MOUSE_CYCLE_COMPLETED:
//					DebugPrint(_T("Mouse cycle end should put back the tolerances\n"));
				break;
			}
...
class SplineIKClassDesc : public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("SplineIKSolver"); }
	SClass_ID		SuperClassID() {return IK_SOLVER_CLASS_ID;}
	Class_ID		ClassID() {return SPLINEIKSOLVER_CLASS_ID;}
...
{
	BaseInterface* gi = linkChain.GetIKGoal();
	ISplineIKGoal2* spline_goal = gi != nullptr
		? reinterpret_cast<ISplineIKGoal2*>(gi->GetInterface(IID_SPLINE_IKGOAL2))
		: NULL;
	if (spline_goal == nullptr) return bInvalidArgument;

	INode* spGoal  = spline_goal->GetGoalNode();
	IIKChainControl*  spChainControl = spline_goal->GetChainControl();
	if (spGoal == nullptr || spChainControl == nullptr) return bInvalidArgument;
...
//	stPoint = spline_goal->SplinePosAt(0.0f, 0); //startpoint coordinates of the spline in world
	Point3  stPoint, runPoint, tang, third_row, norm;
	...
	float uIncrement1, uIncrement2, uIncrement3;
	...
	uIncrement1 = 0.0001f;
	uIncrement2 = 0.00001f;
	uIncrement3 = 0.0000001f;

	///////////////////////////////////////////////////////////////////////////
	// The Spline IK solver works by first creating a cache of some large 
	// number of samples of spline goal positions (this computation is costly).
	// Then it tries to reach a solution by iterating to find the spline points
	// closest to each bone.
	///////////////////////////////////////////////////////////////////////////
	float u = 0.0f;
	eps1 = boneLengths[0]/ EPSILON1_FACTOR;
	eps2 = boneLengths[0]/ EPSILON2_FACTOR;
	eps3 = boneLengths[0]/ EPSILON3_FACTOR;
//watje first ask we need to rebuild cache
	spline_goal->SplinePosAt(0.0f, 1, TRUE); // TRUE means rebuild spline pos cache
	Matrix3 goalTM = spline_goal->GetGoalNode()->GetObjectTM(spline_goal->GetSolveTime());
	Matrix3 toJointMat = goalTM*Inverse(linkChain.parentMatrix);
	// GET_SPLINE_POS() will use the spline goal pos cache
#define	GET_SPLINE_POS(u) (spline_goal->SplinePosAt(u, 1)*toJointMat)

	int iteration2 = 0;

//	DebugPrint(_T("Spline Solve\n"));
...
//		dist2 = FLength(runPoint - veryStPoint);
	...
	if (u > 1.0f) {
		float u_mid = u_min;
		for (u = u_min0 + uIncrement3; u < u_mid; u += uIncrement3) {
			runPoint = GET_SPLINE_POS(u);
			dist = FLength(runPoint - firstJointPositionInWorld);
			if (dist < dist_min) {
				u_min = u;
						...
						if (boneLengths[j] - dist > eps3){
							...
							if (del_u == uIncrement1) {
							  ...
							  u = u0 + uIncrement3;
								...
								j = j + 1;	
								if (j < boneCount) {
								  eps1 = boneLengths[j]/ EPSILON1_FACTOR;
								  eps2 = boneLengths[j]/ EPSILON2_FACTOR;
								  eps3 = boneLengths[j]/ EPSILON3_FACTOR;
								}
							}
							iteration = iteration + 1;
						}
...
//	DebugPrint(_T("%d  %d  %d \n"), iteration, uCount, boneCount);
	...
	for(int kk = 0; kk < boneCount; kk = kk+1){
		float twistAng = kk == 0 ? startTwistAngle : startTwistAngle + (endTwistAngle/(boneCount-1)) * (kk);
		...
		ax_mat.SetRow(jj, CrossProd(boneCoAxes[kk], boneAxes[kk]));
		ax_mat.NoTrans();
		ax_mat.Invert();
