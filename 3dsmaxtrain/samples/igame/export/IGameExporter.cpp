...
static constexpr const TCHAR* mapSlotNames[] = {
		_T("Diffuse"),
		_T("Ambient"),
		_T("Specular"),
		_T("SpecularLevel"),
...
class IGameExporter : public SceneExport {
	public:
		...
		void DumpNamedProperty(CComPtr<IXMLDOMNode>, const TCHAR*, IGameProperty* );
...
class IGameExporterClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("IGameExporter"); }
...
const TCHAR* FixupName(TSTR& name)
{
	TCHAR* cPtr = name.dataForWrite();
	while (*cPtr)
	{
		if (*cPtr == _T('"'))
...
void IGameExporter::ExportChildNodeInfo(CComPtr<IXMLDOMNode> parent, IGameNode * child)
{
	...
	buf = child->GetName();
	AddXMLAttribute(parent,_T("Name"), FixupName( buf ) );
	...
	else
	{
		...
		if(exportControllers)
		{
			...
			else
			{
				...
				if(TV==IGameControl::IGAME_POINTCTRLCONTAINER)
				{
					CreateXMLNode(pXMLDoc,parent,_T("PointControllerContainer"),&prsNode);
...
void IGameExporter::DumpModifiers(CComPtr<IXMLDOMNode> modNode, IGameModifier * m)
{
	...
	if(exportSkin || exportGenMod)
	{
		...
		AddXMLAttribute(propNode,_T("modName"),m->GetInternalName().data());
		bool bS = m->IsSkin();
		if(bS)
			buf.printf(_T("true"));
		else
...
void IGameExporter::DumpNamedProperty(CComPtr<IXMLDOMNode> node, const TCHAR* propName, IGameProperty * prop)
{
	if(!prop)	//fix me NH...
		return;
...
void IGameExporter::DumpMaterial(CComPtr<IXMLDOMNode> node,IGameMaterial * mat, int index, int matID )
{
	...
	AddXMLAttribute(material, _T("Name"), FixupName(buf) );

	buf = mat->GetMaterialClass();
	AddXMLAttribute(material, _T("Class"), FixupName(buf) );
...
int	IGameExporter::DoExport(const TCHAR *name,ExpInterface *ei,Interface *i, BOOL suppressPrompts, DWORD options)
{
	...
	curNode = 0;
	ip->ProgressStart(_T("Exporting Using IGame.."), TRUE, nullptr, nullptr);
