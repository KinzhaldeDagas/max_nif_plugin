...
class SManagerClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Scene Manager"); }
	...
	switch(msg){
		...
		switch (LOWORD(wParam)) {
			...
			case IDC_SAVE:{
				...
				ofn.nMaxFileTitle = 0;
				const MSTR initialDir = MM.ip->GetDir(APP_SCENE_DIR);
				ofn.lpstrInitialDir = initialDir.data();
				ofn.Flags = OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST| OFN_EXPLORER | OFN_ENABLEHOOK |OFN_ENABLESIZING | OFN_HIDEREADONLY;

				if (GetSaveFileName(&ofn))
				{
					if(_tcsicmp(filename + _tcslen(filename) - 4, _T(".txt"))) 
					...
					if (writer.Open(filename, false, encoding) ){
			...
			case IDC_GENERATE:
					...
					MM.Output(_T(""));
					MM.Output(_T("Total Memory Usage: %.2fMB"),(float)MM.totalRAM/1024.0f);
					MM.Output(_T(""));MM.Output(_T("--- END OF REPORT ---"));MM.Output(_T(""));MM.Output(_T(""));
					MM.SetStatus(_T(""));
				break;
...
void SManager::TraverseNode(INode* node, TimeValue t)
{
	BOOL deleteMesh;
	NullView nw;
	MM.SetStatus(_T("Examining Object : %s"),node->GetName());

	//node->RenderBegin(t);
	BeginEnum beginEnum(ip->GetTime());
	node->EnumRefHierarchy( beginEnum, 
    ...
    if (ob!=NULL) {
        ...
        if (ob->SuperClassID() == GEOMOBJECT_CLASS_ID) {
            ...
            if(geo->CanConvertToType(triObjectClassID)) {
				...
				MM.Output(_T("    * Face Count: %i -> %.1f kB"),
					mesh->getNumFaces(),
					fclc);
				MM.Output(_T("    * Vertex Count: %i -> %.1f kB"),
					mesh->getNumVerts(),
					vclc);
...
void SManager::DoObjects(){
	MM.Output(_T("*** Beginning Object Pass  ------------------------------------------------------------------------"));
	MM.Output(_T("***"));
	MM.SetStatus(_T("Beginning Object Pass"));
	CountFaces(ip);
	float fll = (facecount*34.0f)/1024.0f;
	float vll = (vertcount*28.0f)/1024.0f;
	float sll = ((facecount*34.0f)+(vertcount*28.0f))/1024.0f;
	MM.Output(_T(""));
	MM.Output(_T("Total Scene Faces: %i -> %.1f kB"),facecount,fll);
	MM.Output(_T("Total Scene Vertices: %i -> %.1f kB"),vertcount,vll);
	MM.Output(_T("Total Geometry RAM: %.2f kB   |   %.2f MB"),sll,sll/1024.0f);
	MM.Output(_T(""));
...
void SManager::TraverseNodeUVW(INode* node, TimeValue t)
{
	...
	MM.SetStatus(_T("Examining Object UVWs : %s"),node->GetName());

	//node->RenderBegin(t);
	BeginEnum beginEnum(ip->GetTime());
	node->EnumRefHierarchy(beginEnum, true, true, true, false); // use list to track visited refs
        ...
        if (ob->SuperClassID() == GEOMOBJECT_CLASS_ID) {
            ...
            if(geo->CanConvertToType(triObjectClassID)) {
				...
				if(mesh->getNumVertCol()>0||
					mesh->getNumTVerts()>0) MM.Output(_T("%s :"),node->GetName());
				...
				if(mesh->getNumTVerts()>0) {MM.Output(_T("    * UV Channel 1: %i -> %.1f kB"),
					mesh->getNumTVerts(),
					vclc);}
				if(mesh->getNumVertCol()>0) {MM.Output(_T("    * UV Channel 2: %i -> %.1f kB"),
					mesh->getNumVertCol(),
					fclc);}
...
void SManager::DoUVW(){
	MM.Output(_T("*** Beginning UVW Mapping Pass  -------------------------------------------------------------------"));
	MM.Output(_T("***"));
	MM.SetStatus(_T("Beginning UVW Mapping Pass"));
	CountUVW(ip);
	float fll = (UV1count*12.0f)/1024.0f;
	float vll = (UV2count*12.0f)/1024.0f;
	float sll = ((UV1count*12.0f)+(UV2count*12.0f))/1024.0f;
	MM.Output(_T(""));
	MM.Output(_T("Total UV Channel 1: %i -> %.1f kB"),UV1count,fll);
	MM.Output(_T("Total UV Channel 2: %i -> %.1f kB"),UV2count,vll);
	MM.Output(_T("Total Mapping RAM: %.2f kB   |   %.2f MB"),sll,sll/1024.0f);
	MM.Output(_T(""));
	...
	if(m->ClassID()==Class_ID(BMTEX_CLASS_ID,0)) {
		...
		if(_tcsicmp(b->GetMapName(),_T(""))!=0){
			...
			if(MM.biDB.FindName(b->GetMapName())==-1){
				...
				MM.Output(_T("[%s / %s] : %s"),mname,b->GetName(),b->GetMapName());
				MM.Output(_T("    * RAM Statistics:  W:%i, H:%i, BPP:%i  ->  %.1f kB"),
				...
				if (FT==FILTER_SAT) 
					{ mem*=4.0f; }
				MM.Output(_T("    * Total After Filtering:  %.1f MB"),mem/1024.0f);
				MM.Output(_T(""));
				...
				TCHAR p[255],f[255],e[255];
				BMMSplitFilename(b->GetMapName(), p, _countof(p), f, _countof(f), e, _countof(e));
				MM.Output(_T("[%s / %s] : %s"),mname,b->GetName(),b->GetMapName());
				MM.Output(_T("    * Map file '%s%s' could not be found!"),f,e);
				MM.Output(_T(""));
			...
			MM.Output(_T("Map Found: [Undefined/Unallocated]"));
		}
	}
}
...
void EnumMtlTree(MtlBase *mb, MtlEnum &tenum) {
	...
	if (IsMtl(mb)) {
		_tcscpy(tenum.mname,mb->GetName());
		Mtl *m = (Mtl *)mb;
...
void SManager::DoMaps(){
	MM.Output(_T("*** Beginning Bitmap Materials Pass  --------------------------------------------------------------"));
	MM.Output(_T("***"));
	MM.SetStatus(_T("Beginning Bitmap Materials Pass"));
	...
	MM.Output(_T(""));
	MM.Output(_T("*** Checking for duplicate map names..."));
	MM.SetStatus(_T("Checking for duplicate map names"));

	TCHAR p[255],f[255],e[255];
	TCHAR p2[255],f2[255],e2[255];
	for(int y=0;y<biDB.Count();y++){
		BMMSplitFilename(biDB[y], p, _countof(p), f, _countof(f), e, _countof(e));

		for(int d=0;d<biDB.Count();d++){
			BMMSplitFilename(biDB[d], p2, _countof(p2), f2, _countof(f2), e2, _countof(e2));
			if(_tcsicmp(f,f2)==0&& _tcsicmp(e,e2)==0&&_tcsicmp(p,p2)!=0){
				MM.Output(_T("    * Duplicated map in [%s / %s] : %s%s%s"),MM.mtDB[d],MM.txDB[d],p2,f2,e2);
			}
		}
	...
	for (int k=0; k<root->NumberOfChildren(); k++)
	{
		...
		if(ob!=NULL&&ob->SuperClassID()==LIGHT_CLASS_ID)
		{
			...
			if((mSz!=0)&&(ltmp->GetShadow())&&(ltmp->GetShadowMethod()==LIGHTSHADOW_MAPPED)){
				...
				MM.Output(_T("%s -> Shadow Map size: %i --> %0.1f kB"),node->GetName(),mSz,msRAM);
...
void SManager::DoDups(){
	MM.Output(_T("*** Beginning Lighting Pass  ----------------------------------------------------------------------"));
	MM.Output(_T(""));
...
void SManager::DoLights(){
	MM.Output(_T("*** Beginning Render RAM Pass  --------------------------------------------------------------------"));
	MM.Output(_T(""));
	int rWidth = ip->GetRendWidth();
	int rHeight = ip->GetRendHeight();
	float RAMrw = ((rWidth*rHeight)*8.0f)/1024.0f;

	MM.Output(_T("Render Bitmap output: %ix%i --> %.1f kB / %.1f MB"),rWidth,rHeight,RAMrw,RAMrw/1024.0f);
