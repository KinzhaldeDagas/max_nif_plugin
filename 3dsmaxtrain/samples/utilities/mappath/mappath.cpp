...
class RefCheckClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Bitmap/Photometric Paths"); }
...
void BitmapPathEditorDialog::StripSelected()
{
	...
	if (MaxSDK::MaxMessageBox(hDialog, GetString(IDS_STRIPWARNING), szTitle, MB_OKCANCEL | MB_ICONWARNING) != IDOK) {
		return;
	}
...
{
	TSTR szTitle = GetString(IDS_STRIPTITLE);
	if (MaxSDK::MaxMessageBox(hDialog, GetString(IDS_STRIP_ALL_WARNING), szTitle, MB_OKCANCEL | MB_ICONWARNING) != IDOK) {
		return;
	}
	...
	if (!_taccess(mapName, 0)) {
		...
		if (!_taccess(tmpPath, 0)) {
			...
			for (int d = 0; d < TheManager->GetMapDirCount(); d++) {
				_tcscpy(tmpPath, TheManager->GetMapDir(d).data());
				if (tmpPath[_tcslen(tmpPath)-1] != _T('\\')) {
					_tcscat(tmpPath, _T("\\"));
				}
				_tcscat(tmpPath, fname);
...
void BitmapPathEditorDialog::SetActualPath()
{
	...
	MaxSDK::MaxMessageBox(hDialog, msg, GetString(IDS_MESSAGE), MB_OK);
}
				...
				if (SendMessage(GetDlgItem(hDialog, IDC_DEPLIST), LB_GETSEL, i, 0) > 0) {
					...
					if (tmpName && _tcscmp(tmpName, _T(""))) {
						...
						if (fileExists && bOverwrite == FALSE) {
							...
							if (MaxSDK::MaxMessageBox(hDialog, GetString(IDS_COPYERROR), szTitle, MB_OKCANCEL | MB_ICONWARNING) != IDOK) {
								break;
							}
							*/
						}
