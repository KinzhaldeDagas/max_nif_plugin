...
public:
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Clean MultiMaterial"); }
...
MMCleanActions MMCleanActions::mmCleanActions(
   MMCLEAN_ACTIONS, _T("MMCleanActions"), IDS_DLL_CATEGORY, &MMCleanDesc, FP_ACTIONS,
...
static INT_PTR CALLBACK MMCleanDlgProc(
...
int MMClean::FixUpSet::findNotUsedMtlID(Mtl* mtl, NumList& mtlIDNotUsed)
{
   ...
   for (int i=0; i<node_count; i++) {
      ...
      if (ob->CanConvertToType(splineShapeClassID)) { // EditableSpline case
         SplineShape *splshp = (SplineShape *)ob->ConvertToType(mIp.GetTime(), splineShapeClassID);
...
{
   FixUpDialog* p = DLGetWindowLongPtr<FixUpDialog*>(focus);
