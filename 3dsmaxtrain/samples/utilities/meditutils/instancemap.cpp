...
public:
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Instance Duplicate Maps"); }
...
InstanceDuplMapActions InstanceDuplMapActions::mmInstanceDuplMapActions(
   INSTANCEDUPLMAP_ACTIONS, _T("InstanceDuplMapActions"), IDS_DLL_CATEGORY, &InstanceDuplMapDesc, FP_ACTIONS,
...
static INT_PTR CALLBACK InstanceDuplMapDlgProc(
...
// one main. It doesn't have any UI.
...
private:
   ...
   typedef std::unordered_map<BitmapTex*, Mtl*> MapSet;
   ...
   // Make instance out of copies from main (maps[0])
...
void InstanceDuplMap::FixUpSet::findAll()
{
   ...
   for (int i=0; i<count; i++) {
      if ((*mlib)[i]->SuperClassID()==MATERIAL_CLASS_ID) {      //filter out maps if any
         Mtl* mtl = (Mtl*)(*mlib)[i];
         findMaps (mtl);
      }
   }
...
bool InstanceDuplMap::FixUpSet::MakeInstance(Tab<BitmapTex*>& maps)
{
   ...
   int count = maps.Count();
    BitmapTex* map = maps[0];          // make first in the list as main
   int tex_display = map->TestMtlFlag(MTL_TEX_DISPLAY_ENABLED);   //get active status
   ...
   for (int i = 1; i<count; i++){
      ...
      while (p != mMaps.end()) {
         ...
         if(maps[i] == m->first){
            ...
            if(tex_display) {
               m->second->SetActiveTexmap(map);                 
               mIp.ActivateTexture(map, m->second);           //set active status
            }
            // replace instances (if any) with main well 
   ...
   //check if only main, delete if true
   int map_count = 0;                // number duplicates in hash table after erase
   MapSet::iterator main = mMaps.find(map);
   ...
   break;
    ...
    case IDC_DUPL_LIST:
      case CBN_SELENDOK: {
         ...
         BitmapTex* bmt = reinterpret_cast<BitmapTex*>(val);
         if (index != CB_ERR){
...
void InstanceDuplMap::FixUpDialog::appendMapToDuplList(MapSet::iterator p, int index)
{
   ...
   int ith = SendMessage(GetDlgItem(mWnd, IDC_DUPL_LIST), CB_ADDSTRING, 0, (LPARAM)(LPCTSTR)entry_map.data());
...
void InstanceDuplMap::FixUpDialog::appendMapToList(MapSet::iterator p, int index)
{
   ...
   TSTR entry_mtl = p->second->GetFullName(true);
   ...
   lvi.pszText = const_cast<TCHAR*>(entry_map.data());
   ...
   lvi.mask = LVIF_TEXT;
   lvi.iSubItem = 1;
   lvi.pszText = const_cast<TCHAR*>(entry_mtl.data());
