...
class CSIntersection : public IntersectionCallback3D {
	public:
		...
		BOOL Intersect(Point3 p, int piece1, int piece2); // Return FALSE to stop intersect tests
		void Display(GraphicsWindow *gw);
		void GetBBox(Box3 &box);
	};

BOOL CSIntersection::Intersect(Point3 p, int /*piece1*/, int /*piece2*/) {
	table.Append(1, &p);
	return TRUE;
	}
...
class ShapeCheckClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Shape Check"); }
...
void ShapeCheck::Init(HWND hWnd)
	...
	RegisterNotification(PreOpen, this, NOTIFY_SYSTEM_PRE_RESET);
...
void ShapeCheck::Destroy(HWND hWnd)
	...
	UnRegisterNotification(PreOpen, this, NOTIFY_SYSTEM_PRE_RESET);
...
BOOL ShapeCheck::CheckShape(TimeValue t)
	...
	if(os.obj->SuperClassID()==SHAPE_CLASS_ID) {
		...
		for(int i = 0; i < shape.numLines; ++i) {
			Template3D t(&shape.lines[i]);
			if(t.SelfIntersects(TRUE, TRUE, &csi))
				si = TRUE;
			}
		// Now check them against each other
		for(int i = 0; i < shape.numLines; ++i) {
			Template3D tI(&shape.lines[i]);
			for (int j = i + 1; j < shape.numLines; ++j) {
				Template3D tJ(&shape.lines[j]);
				if (tI.Intersects(TRUE, tJ, TRUE, &csi))
					si = TRUE;
				}
			}
		TSTR buf;
...
void ShapeCheck::PreOpen(void* param, NotifyInfo* info) {
	ShapeCheck* shapeCheck = (ShapeCheck*)param;
	if (shapeCheck == NULL)
		return;

	shapeCheck->SetNode(NULL);
}

void ShapeCheck::PreDeleteNode(void* param, NotifyInfo* arg) {
	ShapeCheck* shapeCheck = (ShapeCheck*)param;
	if (shapeCheck == NULL)
		return;

	INode* deletedNode = GetNotifyParam<NOTIFY_SCENE_PRE_DELETED_NODE>(arg);
	if (deletedNode == shapeCheck->theNode)
		shapeCheck->SetNode(NULL);
}
