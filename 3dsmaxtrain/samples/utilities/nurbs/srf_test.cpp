...
const TCHAR* APITestUtilDesc::NonLocalizedClassName()
{
	return _T("NURBS API Test");
}
...
APITestUtil::CombinedTests()
{
   ...
   detnode->SetName(_T("Detach From"));
   ...
   DetachObjects(GetCOREInterface()->GetTime(), detnode, detobj,
               detlist, _T("Detach Test"), copy, relational);
   mpIp->RedrawViews(mpIp->GetTime());
...
{
   APITestUtil *pUtil = DLGetWindowLongPtr<APITestUtil*>(hWnd);
   switch (msg) {
   case WM_INITDIALOG:
