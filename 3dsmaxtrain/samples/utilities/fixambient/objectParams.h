...
bool setMAXScriptValue(ReferenceTarget* obj, LPTSTR name, TimeValue t, T value, int tabIndex)
{
	...
	{
		ProcessUnknownMAXScriptException(_T("setMAXScriptValue"), false, false, true);
	}
	return rval;
}
...
template<class T>
inline bool getMAXScriptValue(ReferenceTarget* obj, LPCTSTR name, TimeValue t, T& value, int tabIndex)
{
	bool rval = false;					// Return false if fails
	assert(obj != NULL);
	ScopedMaxScriptEvaluationContext scopedMaxScriptEvaluationContext;
	...
	{
		ProcessUnknownMAXScriptException(_T("getMAXScriptValue"), false, false, true);
	}
	return rval;
}
...
// Get the parameter
template<class T>
inline bool getMAXScriptValue(ReferenceTarget* obj, LPCTSTR name, TimeValue t, T& value)
{
	bool rval = false;				// Return false if fails
	assert(obj != NULL);
	ScopedMaxScriptEvaluationContext scopedMaxScriptEvaluationContext;
...
Control* getMAXScriptController(ReferenceTarget* obj, LPTSTR name, ParamDimension*& dim)
{
	...
	{
		ProcessUnknownMAXScriptException(_T("getMAXScriptController"), false, false, true);
	}
	return rval;
}
