...
private:
   ...
   typedef std::unordered_map<Mtl*, bool> MaterialSet;
...
private:
   ...
   typedef std::unordered_map<Class_ID, ClassEntry*, CompareID> IDMap;
   ...
   const TCHAR* getString(const Class_ID& id) const;
...
class FixAmbientClassDesc:public ClassDesc2 {
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Fix Ambient"); }
...
FixAmbientScriptActions FixAmbientScriptActions::fixAmbientScriptActions(
   FIX_AMBIENT_ACTIONS, _T("FixAmbientActions"), IDS_FIX_AMBIENT_CATEGORY, &FixAmbientDesc, FP_ACTIONS,
...
static INT_PTR CALLBACK FixAmbientDlgProc(
      HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
   switch (msg) {
      ...
      case WM_COMMAND:
         ...
         case IDC_DO_HELP:
            MaxSDK::IHelpSystem::GetInstance()->ShowProductHelpForTopic(idh_fix_ambient);
            return FALSE;
         }
         break;
...
)
{
   ...
   FixUpDialog* p = DLGetWindowLongPtr<FixUpDialog*>(hwnd);
   if (p == NULL)
   ...
   case WM_CTLCOLORSTATIC:
      ...
      // turn it red.
      if (GetWindowLongPtr(reinterpret_cast<HWND>(l), GWLP_ID) == IDC_FIX_MESSAGE
            && p->mError != 0) {
...
bool FixAmbient::FixUpDialog::initDialog(HWND focus)
{
   ...
   mWndProc = DLGetWindowProc(mWnd);
   if (mWndProc != NULL) {
      DLSetWindowProc(mWnd, windowProc);
   }
...
void FixAmbient::FixUpDialog::appendMaterialToList(MaterialSet::iterator p)
{
   ...
   item.pszText = const_cast<TCHAR*>(entry.data());
...
const TCHAR* FixAmbient::FixUpDialog::getString(const Class_ID& id) const
{
...
void FixAmbient::FixUpDialog::positionWindow()
{
   ...
   if (MaxSDK::Util::GetPrivateProfileInt(kWindowPosition, kStored, 0, fn)) {
      dlgRect.left = MaxSDK::Util::GetPrivateProfileInt(kWindowPosition, kLeft, dlgRect.left, fn);
      dlgRect.top = MaxSDK::Util::GetPrivateProfileInt(kWindowPosition, kTop, dlgRect.top, fn);
      dlgRect.right = MaxSDK::Util::GetPrivateProfileInt(kWindowPosition, kWidth, dlgRect.right, fn);
      dlgRect.bottom = MaxSDK::Util::GetPrivateProfileInt(kWindowPosition, kHeight, dlgRect.bottom, fn);
   }
   else {
...
void FixAmbient::FixUpDialog::saveWindowPos()
{
   ...
   TCHAR buf[64];
   MaxSDK::Util::WritePrivateProfileString(kWindowPosition, kStored, _T("1"), fn);
   _stprintf(buf, _T("%d"), dlgRect.left);
   MaxSDK::Util::WritePrivateProfileString(kWindowPosition, kLeft, buf, fn);
   _stprintf(buf, _T("%d"), dlgRect.top);
   MaxSDK::Util::WritePrivateProfileString(kWindowPosition, kTop, buf, fn);
   _stprintf(buf, _T("%d"), dlgRect.right);
   MaxSDK::Util::WritePrivateProfileString(kWindowPosition, kWidth, buf, fn);
   _stprintf(buf, _T("%d"), dlgRect.bottom);
   MaxSDK::Util::WritePrivateProfileString(kWindowPosition, kHeight, buf, fn);
}
