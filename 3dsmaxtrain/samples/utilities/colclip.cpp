...
class ColorClipClassDesc:public ClassDesc {
   ...
   const TCHAR*	NonLocalizedClassName() { return _T("Color Clipboard"); }
...
static void LoadColorFile(HWND hWnd)
{
   ...
   if (fdir[0] == _T('\0'))
   {
		 _tcscpy(fdir, GetCOREInterface()->GetDir(APP_IMAGE_DIR).data());
   }
   ...
   ofn.Flags           = OFN_EXPLORER | OFN_ENABLEHOOK | OFN_HIDEREADONLY|OFN_FILEMUSTEXIST|OFN_PATHMUSTEXIST | OFN_ENABLESIZING;
   ...
   if (GetOpenFileName(&ofn))
   {
		 ...
		 if (!file.Open(fname))
		 {
			   ...
			   MaxSDK::MaxMessageBox(hWnd, buf1, buf2, MB_ICONEXCLAMATION);
			   return;
		 }
		 TSTR colorLine;

		 // pre-R6
		 // Keep around for backward compatibility with potential existing files. /coz 2023-06-22
		 for (int i = 0; i < 12; i++)
		 {
			   int r, g, b;
...
static void SaveColorFile(HWND hWnd, TSTR& name)
{
	...
	UINT codepage = iface->DefaultTextSaveCodePage(
			true); // dll\PaintLayerMod\ColorPaletteTool.cpp :: SaveColorFile, UTF8 allowed, keep to same.
	...
	for (int i = 0; i < 12; i++)
	{
	    ...
	    COLORREF col = cs->GetAColor().toRGB();
	    ReleaseIColorSwatch(cs);
	    r = GetRValue(col);
	    g = GetGValue(col);
	    b = GetBValue(col);
...
static INT_PTR CALLBACK ColorClipFloaterDlgProc(
   ...
   switch (msg) {
      case WM_INITDIALOG: {
         ...
         AColor color(0.5f,0.5f,0.5f, 1.0f);
