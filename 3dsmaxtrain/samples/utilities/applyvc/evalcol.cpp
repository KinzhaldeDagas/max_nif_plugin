...
void		AddSceneLights(INode* node, SContext* sc, MtlBaseLib* mtls, int* numLights, 
						   RadiosityEffect* radiosity, const EvalVCOptions& evalOptions);
int			LoadMapFiles(INode* node, SContext& sc, MtlBaseLib& mtls, TimeValue t);
TriObject*	GetTriObjectFromNode(INode *node, TimeValue t, int &deleteIt);
BOOL		IsTwoSidedShadows( Object* light );
BOOL		IsTwoSidedMaterial( INode* node );
...
BOOL		intersectTri(Ray* ray, Point3* tri, Point3& baryCoord, Point3& normal, Point3& hitPos, BOOL ignoreBackfacing);
...
void        CalcBaryCoords(Point3& outBaryCoords, const Point3& p0, const Point3& p1, const Point3& p2, const Point3& p);
...
class DefObjLight : public ObjLightDesc 
{
	public:
		...
		BOOL Illuminate(ShadeContext& sc, Point3& normal, Color& color, Point3& dir, float& dot_nl, float& diffCoef) override;
};
...
public:
	...
	LightObject *light;
	BOOL		bIsObscured, bTwoSidedShadows;
...
MeshInstance::MeshInstance(INode* node)
	...
	if (tri) {
		...
		*mesh = *&tri->GetMesh();
		...
		bTwoSidedMaterial = IsTwoSidedMaterial( node );
...
public:
    ...
    virtual int Open(INode *scene, INode *vnode,ViewParams *viewPar,RendParams &rp, HWND hwnd, DefaultLight* defaultLights=NULL,int numDefLights=0, RendProgressCallback* prog = NULL) { return 0; }
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("NullRenderer"); }
    ...
    virtual PauseSupport IsPauseSupported() const override { return Renderer::PauseSupport::Legacy; }
    ...
    static TypedSingleRefMaker<NullRenderer> m_theInstance;
};

TypedSingleRefMaker<NullRenderer> NullRenderer::m_theInstance (new NullRenderer);
...
void EnumRenderRefs(RendParams& rendParams, RefEnumProc& refEnumProc) {
    ...
    if(rendParams.pRadiosity != NULL)
        rendParams.pRadiosity->EnumRefHierarchy(refEnumProc);
    if(rendParams.pToneOp != NULL)
        rendParams.pToneOp->EnumRefHierarchy(refEnumProc);
}
...
void PreRenderInit(RendParams& rendParams) {
	...
	renderBeginEnum.BeginEnumeration();
    EnumRenderRefs(rendParams, renderBeginEnum);
	renderBeginEnum.EndEnumeration();
}
...
void PostRenderInit(RendParams& rendParams) {
	...
	renderEndEnum.BeginEnumeration();
    EnumRenderRefs(rendParams, renderEndEnum);
	renderEndEnum.EndEnumeration();
}

static inline float g_GetTriangleArea(const Point3& v0,const Point3& v1,const Point3& v2)
{
    return 0.5f * CrossProd(v1 - v0, v2 - v0).FLength();
}

//Used to debug to check if we are in a small triangle far from origin
static bool g_IsTriangleAreaSmallComparedToItsCoordinates(const Point3& v0,const Point3& v1,const Point3& v2)
{
    static const Point3 zero(0.f, 0.f, 0.f);//origin

    //To tell we are far from origin, we compare the triangle area with the triangles areas which is one 
    //side of the triangle and the (0,0,0) of the world (see below)

    const float triangleArea      = g_GetTriangleArea(v0,v1,v2); //Area of (v0,v1,2)
    
    static const float factor = 5000.f;//We consider that being "factor" times bigger for an area of the 
    //sides triangles than the original triangle area is a small triangle far from origin or really bigger 
    //than its position in world coords

    {
        const float triangleAreaSide0 = g_GetTriangleArea(v0,v1,zero); //Area of (v0,v1,zero)
        if (triangleArea < factor * triangleAreaSide0){
            return true;
        }
    } 

    {
        const float triangleAreaSide1 = g_GetTriangleArea(v0,zero,v2); //Area of (v0,zero,v2)
        if (triangleArea < factor * triangleAreaSide1){
            return true;
        }
    }
    
    {
        const float triangleAreaSide2 = g_GetTriangleArea(zero,v1,v2); //Area of (zero,v1,v2)
        if (triangleArea < factor * triangleAreaSide2){
            return true;
        }
    }
...
static bool inline g_IsRadiosityColorSpecialDegenerateCaseValue(const Color& col)
{
    //Used to detect a failure case in radiosity computation, so far was only used for really small triangles 
    //compared to their coordinates.
    //See src\dll\srr\radiositySolution.cpp, same variable name : "g_IllumColorSpecialCase"
    static const Color s_IllumColorSpecialCase(0.00123f, 0.00456f, 0.00789f);

    const DWORD r = s_IllumColorSpecialCase.r;
    const DWORD g = s_IllumColorSpecialCase.g;
    const DWORD b = s_IllumColorSpecialCase.b;

    //We can't do == with that sort of floats
    if (fabsf(col.r - s_IllumColorSpecialCase.r) > 1.0e-3f){
        return false;
    }
    
    if (fabsf(col.g - s_IllumColorSpecialCase.g) > 1.0e-3f){
        return false;
    }

    if (fabsf(col.b - s_IllumColorSpecialCase.b) > 1.0e-3f){
        return false;
    }
...
//***************************************************************************
BOOL calcFaceColors(INode* node, TimeValue t, const EvalVCOptions& evalOptions, FaceColorTab& faceColTab, MeshInstanceTab&	instanceList, EvalColProgressCallback* fn /*= NULL*/)
{
	ObjectState ostate;
	Mesh* mesh;
	SContext sc;
	...
	for (unsigned int f = 0; f < (unsigned)numFaces; f++) {
		...
		const Point3 faceCenter = (v0+v1+v2) / 3.f;

		sc.SetFaceNum(f);
		sc.SetMtlNum(face.getMatID());
		sc.CalcNormals();

        //If we are reaching a degenerate case such as a small triangle compared to its coordinates
        bool bDegenerateCaseInLightingOnlyFound = false;
		...
		for (int fv=0; fv<3; fv++) {
			...
			// Setup approximated barycentric coordinates as we are close to one of the verts, we suppose
            //the 2 others coords are negligible so equals. It seems bary coords are not used anyway by radiosity in that case
			...
			if(evalOptions.lightingModel == LightingModel::kLightingOnly) {
				...
				for(int i = 0; i < sc.nLights; ++i) {
					...
					if(lightDesc->Illuminate(sc, normal, lightIllum, dummyDir,
						dot_NL, dummyDiffuseCoef))
					{
                        if (g_IsRadiosityColorSpecialDegenerateCaseValue(lightIllum)){
                            bDegenerateCaseInLightingOnlyFound = true;
                            #ifdef _DEBUG
                                bool b = g_IsTriangleAreaSmallComparedToItsCoordinates(v0,v1,v2); //debug checking
                            #endif
                            //Stop looping in lights, we'll do that in another part of the code later, look for bDegenerateCaseInLightingOnlyFound
                            break;
                        }
						totalIllum += dot_NL * lightIllum;
					}
				}

                if( bDegenerateCaseInLightingOnlyFound ){
                    break;//Exit "for (int fv=0; fv<3; fv++)" loop
                }
        ...
        //We are dealing with a special case such as a really small triangle compared to its coordinates
        if (bDegenerateCaseInLightingOnlyFound){
            bDegenerateCaseInLightingOnlyFound = false;//reset 

            //Cast the ray from the center of the face and we'll assign the result to the 3 vertices at once
            const Point3& vxNormal  = sc.vxNormals[0];
			Point3 viewPoint        = faceCenter + vertexEvalDistance*vxNormal;
			Point3 viewDir          = -vxNormal;
			Point3 lightPos         = viewPoint;

			// render vertex for this face.
			sc.SetViewPoint(viewPoint);
			sc.SetTargetPoint(faceCenter);
			sc.SetViewDir(viewDir);

			// Setup approximated barycentric coordinates of center of face
            sc.SetBaryCoord(Point3(0.33f, 0.33f, 0.33f)); //1/3 of each vertex
			...
			if(evalOptions.lightingModel == LightingModel::kLightingOnly) {
						...
						dot_NL, dummyDiffuseCoef))
					{
                        if (g_IsRadiosityColorSpecialDegenerateCaseValue(lightIllum) ){
                            bDegenerateCaseInLightingOnlyFound = true;
                                ...
                                bool b = g_IsTriangleAreaSmallComparedToItsCoordinates(v0,v1,v2);//debug checking
                            #endif
                            //So far, the only case where we have reached this part of the code is when the 
                            //triangle has the 3 vertices aligned so resulting in a null area
                            lightIllum.r = lightIllum.g = lightIllum.b = 0.f; //zero-ing its contribution (is what was doing the original code anyway)
                        }
                ...
                //Assign to the 3 face vertices
				faceCol->colors[0] = totalIllum;
                faceCol->colors[1] = totalIllum;
                faceCol->colors[2] = totalIllum;
			    ...
			    if((sc.globContext != NULL) 
				    ...
				    && !sc.globContext->pToneOp->GetIndirectOnly())
			    {
				    sc.globContext->pToneOp->ScaledToRGB(faceCol->colors[0]);
                    sc.globContext->pToneOp->ScaledToRGB(faceCol->colors[1]);
                    sc.globContext->pToneOp->ScaledToRGB(faceCol->colors[2]);
			    }

			    faceCol->colors[0].ClampMinMax();
                faceCol->colors[1].ClampMinMax();
                faceCol->colors[2].ClampMinMax();
			}
            else{
                DbgAssert( 0 && _T("We should be in LightingModel::kLightingOnly"));
            }
        }
...
//***************************************************************************
//is inspired from IGameRenderSurfaceImp.cpp (with some improvements with passing parameters by reference of course)
static float det3x3( const Point3& a, const Point3& b, const Point3& c )
{
...
// Thanks to Ruediger Hoefert, Absolute Software
//***************************************************************************
//is copied/pasted from IGameRenderSurfaceImp.cpp (with some improvements with passing parameters by reference of course)
static void CalcBaryCoords(Point3& outBaryCoords, const Point3& p0, const Point3& p1, const Point3& p2, const Point3& p)
{ 
	Point3 tpos[3];
	Point3 cpos;
 ...
 */
	...
	outBaryCoords.x = area_pbc *area_abc ;
	outBaryCoords.y = area_apc *area_abc ;
	outBaryCoords.z = area_abp *area_abc ;
}
...
//***************************************************************************
BOOL calcMixedVertexColors(INode* node, TimeValue t, const EvalVCOptions& evalOptions, ColorTab& vxColTab, MeshInstanceTab&	instanceList, EvalColProgressCallback* fn/*=NULL*/)
{
	ObjectState ostate;
	Mesh* mesh;
	SContext sc;
	...
	for (unsigned int v = 0; v < (unsigned)numVerts; v++) {
		...
		for (int nf = 0; nf < faceList.GetSize(); nf++) {
			if (faceList[nf]) {
                ...
                const Point3& v0 = mesh->verts[f.v[0]];
                const Point3& v1 = mesh->verts[f.v[1]];
                const Point3& v2 = mesh->verts[f.v[2]];

				// Calculate the coordinate of the center of the face
				const Point3 faceCenter         = (v0 + v1 + v2) / 3.f;
                //Shift the viewTarget point toward center of the face
                const Point3 viewTargetShifted  = viewTarget + ((faceCenter - viewTarget) * vertexEvalEspilon);

				// render vertex for this face.
				sc.SetViewPoint  (viewPoint);
				sc.SetTargetPoint(viewTargetShifted);
				sc.SetViewDir    (viewDir);
				sc.SetFaceNum    (nf);
...
LightInfo::LightInfo(INode* node, MtlBaseLib* mtls)
{
	...
	bTwoSidedShadows = IsTwoSidedShadows( ostate.obj );
}
...
LightInfo::LightInfo(ObjLightDesc* objectLightDesc, LightObject* lightObject) {
    ...
    bIsObscured = FALSE;
	if( lightObject!=NULL )
		bTwoSidedShadows = IsTwoSidedShadows( lightObject );
	else if( (objectLightDesc!=NULL) && (objectLightDesc->inode!=NULL) ) {
		ObjectState ostate = objectLightDesc->inode->EvalWorldState(0);
		bTwoSidedShadows = IsTwoSidedShadows( ostate.obj );
	}
	else bTwoSidedShadows = FALSE;
}
...
void sceneLightEnum(
	...
	MtlBaseLib* mtls, 
	int* numLights, 
	RadiosityEffect* radiosity, 
	const EvalVCOptions& evalOptions)
{
	...
	if (ostate.obj->SuperClassID() == LIGHT_CLASS_ID) {
		...
		if (node->Renderable() && light->GetUseLight() &&
...
int LoadMapFiles(INode* node, SContext& sc, MtlBaseLib& mtls, TimeValue t)
{
	// Check for Missing Maps: Not sure how to handle this so we gladly continue.
	//NameTab mapFiles;
	...
	//	// Error! Missing maps.
	//		
	//	//if (MaxSDK::MaxMessageBox(hWnd, "There are missing maps.\nDo you want to render anyway?", "Warning!", MB_YESNO) != IDYES) {
	//	//	return 0;
	//	//}
	//}
...
void SContext::TurnOffObscuredLights(MeshInstance* inst)
	...
	Interval valid;
	float visibility = inst->node->GetVisibility( CurTime(), &valid );
	if (inst->node && ((!inst->node->CastShadows()) || (visibility<=0)))
		return;
			...
			if (allLights[l]->light && allLights[l]->light->GetShadowMethod()!=LIGHTSHADOW_NONE) {
				...
				if ((inst->radsq - LSQ(pc) + v*v) >= 0.0f) {
					...
					if (rayBoundHit(&r, inst->boundingBox)) {
						hitInfo.ignoreBackfacing = !(inst->bTwoSidedMaterial || allLights[l]->bTwoSidedShadows);
...
BOOL intersectMesh(Ray* ray, HitInfo& hitInfo)
{
	...
	for (int nf = 0; nf < mesh->numFaces; nf++) {
		...
		*/
		...
		Point3 n;
		Point3 hp;
		if (intersectTri(ray, tri, bc, n, hp, hitInfo.ignoreBackfacing)) {
			float dl = Length( hp - ray->p );
			if (dl < dist) {
				hitInfo.hitPos = hp;
				dist = dl;
				hit = TRUE;
...
BOOL intersectTri(Ray* ray, Point3* tri, Point3& baryCoord,
				  Point3& normal, Point3& hitPos, BOOL ignoreBackfacing )
{
	Plane p;
	...
	CalcBaryCoords(baryCoord, tri[0], tri[1], tri[2], Pi); //result is in baryCoord now
...
static FPValue GetScriptedProperty( FPValue& thing, const TCHAR* propName ) {

	FPValue retVal( TYPE_INT, 0 );
	BOOL isUndefined = ((thing.type==TYPE_VALUE) && (thing.v==&undefined));
	if( (thing.i!=0) && (!isUndefined) ) 
	{
...
BOOL IsTwoSidedShadows( Object* light ) 
{	// get the scripted property ".shadowGenerator.twoSidedShadows" of the light
	if( light==NULL ) return FALSE;
	FPValue lightValue( TYPE_REFTARG, light );
	FPValue shadGenValue = GetScriptedProperty( lightValue, _T("shadowGenerator") );
	FPValue twoSidedValue = GetScriptedProperty( shadGenValue, _T("twoSidedShadows") );
	return (twoSidedValue.i != 0? TRUE:FALSE);
}

BOOL IsTwoSidedMaterial( INode* node ) 
{	// get the scripted property ".twoSided" of the node's material
	if( (node==NULL) || (node->GetMtl()==NULL) ) return FALSE;
	FPValue mtlValue( TYPE_REFTARG, node->GetMtl() );
	FPValue twoSidedValue = GetScriptedProperty( mtlValue, _T("twoSided") );
	return (twoSidedValue.i != 0? TRUE:FALSE);
}
