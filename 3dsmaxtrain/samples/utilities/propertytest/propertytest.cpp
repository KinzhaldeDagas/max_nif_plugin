...
class PropertyTestClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("PropertyTest"); }
...
void PropertyTest::GetProperties(HWND hDlg)
	...
	for (int i=0; i<numProps; i++) {
		...
		if (pPropSpec->ulKind == PRSPEC_PROPID) {
			...
			_tcsncpy_s(szBuf, _countof(szBuf), TSTR::FromOLESTR(pPropSpec->lpwstr).data(), _TRUNCATE);
			}
...
void PropertyTest::VariantToString(const PROPVARIANT* pProp, TCHAR* szString, int bufSize)
	...
	switch (pProp->vt) {
		case VT_BSTR:
		case VT_LPWSTR:
			_tcsncpy_s(szString, bufSize, TSTR::FromUTF16(pProp->pwszVal).data(), _TRUNCATE);
			break;
		case VT_LPSTR:
			_tcsncpy_s(szString, bufSize, TSTR::FromACP(pProp->pszVal).data(), _TRUNCATE);
			break;
#if !defined( _WIN64 )
