...
// Method exported from core to register analytics data when saving streams to scene file.
CoreExport void LogCustomSceneStreamInfoToAnalytics(unsigned long numStreamsSaved);
...
void CustomSceneStreamManager::GetStreamDataAsArray(int index, Tab<const TCHAR*>& data) const
{
	...
	const size_t nChars = mData[index].mData.AllocatedChars();

	const TCHAR* end_of_string = string + nChars;

	// Deal with lookahead. We can have two types of strings: double null terminated strings or single null terminated strings.
	// Double null terminated strings allow multiple strings to be parsed. For example: abc\0xyz\0\0 -> abc\0 and xyz\0
	// Note that double null terminated strings can contain multiple empty strings: \0\0\0 -> \0 and \0
	// Single null terminated strings only have a single entry.
	// If we have a double null terminated string, we check one character ahead to skip the last null terminator.
	size_t len = _tcslen(string);
	const size_t lookahead = nChars != (len + 1) ? 1 : 0;

	while (string + lookahead < end_of_string)
	{
...
int CustomSceneStreamManager::ValidateWhich(Value* which, bool throwOnError) const
{
	...
	int res;
	bool byName = false;
	if (Number::is_integer_number(which))
		res = which->to_int() - 1;
	else
	{
...
void CustomSceneStreamManager::NotifyProc(void *param, NotifyInfo *info)
{
	...
	CustomSceneStreamManager* instance = static_cast<CustomSceneStreamManager*>(param);
	switch (info->intcode)
	{
	...
	case NOTIFY_FILE_POST_OPEN_PROCESS:
		if (const NotifyFileProcessParam* nfp = GetNotifyParam<NOTIFY_FILE_POST_OPEN_PROCESS>(info))
		{
...
bool CustomSceneStreamManager::SaveToFile(const TCHAR* fileName, bool saveAllStreams, bool overwiteExistingStreams)
{
	...
	if (!mLocked)
	{
		...
		if (!haveDataToStore)
		{
			LogCustomSceneStreamInfoToAnalytics(numStreamsSaved);
			return true;
		}
		IStorage* pFileIStorage = nullptr;
...
void CustomSceneStreamManager::RunCallbacks(CallbackTypes callbackType, const TCHAR* fname)
{
	...
	for (int i = 0; i < mMXSCallbackType.Count(); i++)
	{
		if (mMXSCallbackType[i] == callbackType)
		{
			...
			{
				ProcessUnknownMAXScriptException(MaxSDK::GetResourceStringAsMSTR(IDS_ERROR_RUNNING_CALLBACK), true, true, true);
			}
