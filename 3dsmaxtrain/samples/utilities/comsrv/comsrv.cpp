...
class COMsrvClassDesc : public ClassDesc {
	public:
		...
		const TCHAR*	NonLocalizedClassName() { return _T("COM/DCOM Server Control"); }
...
void COMsrv::Init ( HWND hWnd ) {
	...
	if (!comgup) {
		TSTR msgBoxTitle = GetString(IDS_DLG_TITLE);
		MaxSDK::MaxMessageBox(hWnd, GetString(IDS_CANT_FIND_GUP), msgBoxTitle, MB_OK);
		return;
	}
