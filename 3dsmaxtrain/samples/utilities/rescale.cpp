...
class RescaleUtilClassDesc:public ClassDesc {
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Rescale World Units"); }
...
static INT_PTR CALLBACK  RescaleDlgProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
   ...
   if (msg==WM_INITDIALOG) {
      ...
      DLSetWindowLongPtr(hwndDlg, lParam);
       ...
       if ( (resc = DLGetWindowLongPtr<RescaleUtil *>(hwndDlg) ) == NULL )
         return FALSE; 
      }
...
class RescaleRestore: public RestoreObj {
   ...
   BOOL doSel;
   public:
      RescaleRestore(float f, BOOL sel) { fact = f; doSel = sel; } 
