...
class CollectionClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Resource Collector"); }
...
void CreateNewFilename(const AssetUser& name, AssetUser& newFile)
{
	...
	BMMSplitFilename(name.GetFileName(), NULL, 0, tfile, _countof(tfile), textn, _countof(textn));
		...
		case WM_COMMAND:
			...
			case IDC_GO:
				...
				if(_tcsicmp(ip->GetCurFilePath(),_T(""))==0) 
				{
					...
					MaxSDK::MaxMessageBox(NULL, msg, GetString(IDS_WARNING), MB_OK);
				...
				else
				{
					...
					BMMSplitFilename(ip->GetCurFilePath(), NULL, 0, tfile, _countof(tfile), NULL, 0);
				...
				for (int q=0; q<theCollection.mapPaths.Count(); q++)
				{
					...
					BMMSplitFilename(theCollection.mapPaths[q], NULL, 0, tfile, _countof(tfile), textn, _countof(textn));
					BMMAppendSlash(to_path);
				...
				for (int q=0; q<distributionLister.fileList.Count(); q++)
				{
					...
					BMMSplitFilename(distributionLister.fileList[q], NULL, 0, tfile, _countof(tfile), textn, _countof(textn));
					BMMAppendSlash(to_path);
				...
				if(zipup&&theCollection.WZisHere)
				{
					...
					else
					{
						...
						TCHAR tfile[MAX_PATH];
						TCHAR textn[MAX_PATH];
						BMMSplitFilename(theCollection.WZPath, tpath, _countof(tpath), tfile, _countof(tfile), textn, _countof(textn));
...
void Collection::Init(HWND hWnd)
{
	...
	SetWindowText(GetDlgItem(hWnd,IDC_PATH),ip->GetDir(APP_ARCHIVES_DIR).data());
}
