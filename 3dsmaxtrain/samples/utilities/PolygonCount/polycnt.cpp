...
public:
	...
	const TCHAR* NonLocalizedClassName() { return _T("Polygon Counter"); }
...
void
GetAppData(Interface * ip, int id, const TCHAR* def, TSTR &val)
{
...
void WriteAppData(Interface* ip, int id, const TCHAR* val)
{
    SoundObj* node = ip->GetSoundObject();
    ...
    CStr  theVal = CStr::FromMCHAR(val);
    int   len    = theVal.Length() + 1;
    char* buf    = (char*)MAX_malloc(len);
    strcpy_s(buf, len, theVal);
