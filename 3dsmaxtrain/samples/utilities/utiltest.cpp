...
class UtilTestClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Utility Tester"); }
...
void UtilTest::MakeObject()
	...
	SimpleObject2 *obj = (SimpleObject2*)ip->CreateInstance(
		GEOMOBJECT_CLASS_ID,
	...
	iCylParams->SetValue(CYLINDER_RADIUS,TimeValue(0),30.0f);
	iCylParams->SetValue(CYLINDER_HEIGHT,TimeValue(0),100.0f);
	iCylParams->SetValue(CYLINDER_SEGMENTS,TimeValue(0),10);
...
static INode *MakeGroupObj(Interface *ip,Point3 p)
	...
	iCylParams->SetValue(CYLINDER_RADIUS,TimeValue(0),20.0f);
	iCylParams->SetValue(CYLINDER_HEIGHT,TimeValue(0),50.0f);
...
void UtilTest::SaveToFile()
	...
	MaxSDK::MaxMessageBox(hPanel, _T("Save Completed."), _T("SaveToFile() Test"), MB_OK);
	}

void UtilTest::LoadFromFile()
	{
	ip->LoadFromFile(_T("c:\\savetest.max"));
	MaxSDK::MaxMessageBox(hPanel, _T("File Loaded."), _T("LoadFromFile() Test"), MB_OK);
	}
...
void UtilTest::SetWAV()
	...
	if (iWav) {
		...
		if (!iWav->SetSoundFile(assetMgr->GetAsset(_T("test.wav"),kSoundAsset))) {
			MaxSDK::MaxMessageBox(hPanel, _T("Unable to load TEST.WAV"), _T("Util Test"), MB_OK);
			return;
			}
		// Set the offset to 10 frames.
		iWav->SetStartTime(GetTicksPerFrame() * 10);
	} else {
		MaxSDK::MaxMessageBox(hPanel, _T("No IWaveSound interface"), _T("Util Test"), MB_OK);
		}
	}
...
class TestSoundObj : public SoundObj {
	public:		
		...
		void GetClassName(MSTR& s, bool localized) const override {s=_T("Test Sound Object");}
		...
		{
			TestSoundObj* newObject = new TestSoundObj();
...
class TestSoundObjClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Test Sound Object"); }
