...
class OSDInterfaceTestClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("OSD Interface Test"); }
...
void OSDInterfaceTest::CheckForOSDInterface(INode *node)
{
	...
	if(i) {
		...
		work.printf(GetString(IDS_LINE1_PATTERN), i->TessellationLevel(t, valid));
		...
		work.printf(GetString(IDS_LINE8_PATTERN), i->AdaptiveTessellationLevel(t, valid));
		...
		if(i->Adaptive()) {
			...
			if(obj->IsSubClassOf(polyObjectClassID)) {
				...
				work.printf(GetString(IDS_CAGE_PATTERN), (int)cageObj->GetMesh().numv, (int)cageObj->GetMesh().numf);
			...
			if(obj->SuperClassID() == GEOMOBJECT_CLASS_ID ) {
				...
				if(theMesh) {
					...
					work.printf(GetString(IDS_MESH_PATTERN), (int)theMesh->numVerts, (int)theMesh->numFaces);
					SetDlgItemText(hPanel, IDC_LINE9, work);
