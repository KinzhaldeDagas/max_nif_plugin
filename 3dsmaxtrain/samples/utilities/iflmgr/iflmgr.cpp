...
class IFLMgrClassDesc : public ClassDesc {
	public:
		...
		const TCHAR*	NonLocalizedClassName() { return _T("IFL Manager"); }
...
void IFLMgr::Create ( HWND hWnd ) {
	...
	MSTR workpathMSTR;
	if (workpath[0])
		workpathMSTR = workpath;
	else
		workpathMSTR = ip->GetAssetDir(0,MaxSDK::AssetManagement::kBitmapAsset);
	ofn.lpstrInitialDir = workpathMSTR.data();
	...
	while (1) {
		...
		if (GetSaveFileName(&ofn)) {
			...
			if (BMMIsFile(ofn.lpstrFile)) {
				...
				if (MaxSDK::MaxMessageBox(hWnd, text, tit, MB_APPLMODAL | MB_ICONQUESTION | MB_YESNO) != IDYES)
					continue;
			}
			...
			UINT encoding = iface->DefaultTextSaveCodePage(true);
			MaxSDK::Util::TextFile::Writer writer;
			if ( writer.Open( ofn.lpstrFile, false, MaxSDK::Util::TextFile::Writer::WRITE_BOM | encoding ) ) {

				//-- Get Arguments ------------------------

				int start	= std::max(GetSpinnerValue(hWnd,IDC_START_SP),0);
				int end		= std::min(GetSpinnerValue(hWnd,IDC_END_SP),table.Count()-1);
				...
				writer.Close();
				BMMSplitFilename(ofn.lpstrFile, workpath, _countof(workpath), NULL, 0, NULL, 0);
...
void IFLMgr::Select ( HWND hWnd ) {
	...
	if (workpath[0])
		_tcscpy_s(path, workpath);
	else
		_tcscpy_s(path, ip->GetAssetDir(0,MaxSDK::AssetManagement::kBitmapAsset).data());
	...
	if (TheManager->SelectFileInput(&fbi,hWnd)) {
		...
		if (idx == -1) {
			...
			MaxSDK::MaxMessageBox(hWnd, text, tit, MB_APPLMODAL | MB_ICONSTOP | MB_OK);
			return;
		}
		...
		BMMSplitFilename(fbi.Name(), workpath, _countof(workpath), NULL, 0, NULL, 0);
		return;
	}
...
void IFLMgr::Edit ( HWND hWnd ) {
	...
	ofn.Flags			= OFN_HIDEREADONLY | OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST| OFN_EXPLORER | OFN_ENABLEHOOK | OFN_ENABLESIZING; 
	ofn.lStructSize		= sizeof(OPENFILENAME);
	ofn.hwndOwner		= hWnd;
	ofn.lpstrFile		= filename;
	ofn.nMaxFile		= MAX_PATH;
...
BOOL IFLMgr::EditIFL( HWND hWnd, TCHAR *filename ) {
	...
	// Use default editor
	if (ShellExecute(NULL, _T("open"), filename, NULL, NULL, SW_SHOWNORMAL) > (HINSTANCE) 32) {
		return TRUE; 
		}
...
int IFLMgr::BuildFileList(HWND hWnd, const TCHAR* masktext)
{
	...
	TCHAR* nameBuf = name.dataForWrite();
	for (int i = 0; i < nameLen; ++i)
	{
		if (nameBuf[i] <= _T('9') && nameBuf[i] >= _T('0'))
		{
			nameBuf[i] = _T('?');
		}
	}

	mask.printf(_T("%s%s%s"), path, name, ext);

	if ((mask.first(_T('*')) == -1) && (mask.first(_T('?')) == -1))
	{
		return 0;
	}
...
void IFLMgr::ExtractRoot( TCHAR *filename ) {
	...
	_tsplitpath(filename,dr,di,fn,ex);
	int i = 0;
	while (fn[i++]) {
	...
	_tmakepath_s(root,_countof(root),dr,di,fn,NULL);
	_tcslwr(root);
}
