...
class UMaxFindClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("MAX File Finder"); }
...
void UMaxFind::StartFileFinder()
{
	...
	res = CreateProcess(NULL, szPath.dataForWrite(), NULL, NULL, FALSE, 0, NULL, NULL, &si, &process);
	if (!res) {
		// Error
		TSTR msg = GetString(IDS_ERROR);
		MaxSDK::MaxMessageBox(hPanel, GetString(IDS_NOEXE), msg, MB_OK | MB_ICONEXCLAMATION);
		return;
		}
