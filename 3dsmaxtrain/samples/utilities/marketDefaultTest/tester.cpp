...
static SectionDesc sections1[] = {
	{ _T("Section1"), keys11, _countof(keys11), false, false },
	{ classid12,      keys12, _countof(keys12), true, false },
	{ _T("Section3"), keys13, _countof(keys13), false, false },
	{ _T("Section4"), NULL,   0,                                  false, true },
	{ classid15,      keys15, _countof(keys15), true, false },
	{ classid16,      NULL,   0,                                  true, true },
};

static SectionDesc sections2[] = {
	{ _T("Test1"),    keys21, _countof(keys21), false, false },
	{ classid22,      keys22, _countof(keys22), true, false },
	{ _T("Test3"),    keys23, _countof(keys23), false, false },
	{ _T("Test4"),    NULL,   0,                                  false, true },
	{ classid25,      keys25, _countof(keys25), true, false },
	{ classid26,      NULL,   0,                                  true, true },
};

static FileDesc file1 = { sections1, _countof(sections1) };
static FileDesc file2 = { sections2, _countof(sections2) };
...
static const ULONG clarray[2] = {DMTL_CLASS_ID, 0};
static const ULONG nullclarray[2] = {0, 0};
static const ULONG standinclarray[2] = {STANDIN_CLASS_ID, 0};
static const ULONG junkclarray[2] = {4, 5};
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CfgDefaultTestClassDesc"); }
	...
	const TCHAR* path, const FileDesc* file)
{
	...
	if (f == NULL) {
		mStatus.message(kErrorMessage, _T("Unexpected Error: Cannot create %s\r\n"), path);
		return false;
	}
...
{
	return mDefaultsDir + _T("FactoryDefaults\\") + filename;
}
...
{
	mStatus.message(kInfoMessage, _T("Starting testFiles\r\n"));
	...
	mStatus.message(kInfoMessage, _T("Finished testFiles - %d errors\r\n\r\n"), errors);
...
{
	mStatus.message(kInfoMessage, _T("Starting testFileMissing %s\r\n"), file);
	...
	mStatus.message(kInfoMessage, _T("Finished testFileMissing %s - %d error(s)\r\n\r\n"), file, errors);
...
int MarketDefaultTest::Tester::testLowLevel()
{
	...
	// Cleanup
	deleteFileValues(&file1);
	DeleteFile(GetMarketDefaultsFileName(_T("testLowLevelWrite")).data());
...
{
	mStatus.message(kInfoMessage, _T("Starting testHighLevel\r\n"));
	...
	// Cleanup
	DeleteFile(GetMarketDefaultsFileName(_T("testHighLevel")).data());

	mStatus.message(kInfoMessage, _T("Finished testHighLevel - %d error(s)\r\n\r\n"), errors);
...
{
	mStatus.message(kInfoMessage, _T("Starting testRanges\r\n"));
	...
	mStatus.message(kInfoMessage, _T("Finished testRanges - %d error(s)\r\n\r\n"), errors);
...
int MarketDefaultTest::Tester::checkArray(MarketDefaults* dflts, const KeyDesc* key,
	...
	const TCHAR* valType, const TCHAR* fmt)
{
...
{
	mStatus.message(kErrorMessage, _T("%d"), val);
}	

inline void MarketDefaultTest::Tester::format(float val)
{
	mStatus.message(kErrorMessage, _T("%g"), val);
}	

inline void MarketDefaultTest::Tester::format(const TSTR& val)
{
	mStatus.message(kErrorMessage, _T("%s"), val.data());
}	

inline void MarketDefaultTest::Tester::format(const Point3& val)
{
	mStatus.message(kErrorMessage, _T("(%g, %g, %g)"), val.x, val.y, val.z);
}	

inline void MarketDefaultTest::Tester::format(const Point4& val)
{
	mStatus.message(kErrorMessage, _T("(%g, %g, %g, %g)"), val.x, val.y, val.z, val.w);
}	
...
int MarketDefaultTest::Tester::createHighLevelValues(MarketDefaults* dflts)
{
	...
	errors += highLevelPutError(dflts->PutString(_T("nullstring"), _T("")),
		_T("nullstring"));
	errors += highLevelPutError(dflts->PutIntArray(_T("classid"), 2, (const int*)clarray),
		_T("classid"));
	errors += highLevelPutError(dflts->PutIntArray(_T("nullclassid"), 2, (const int*)nullclarray),
		_T("nullclassid"));
	errors += highLevelPutError(dflts->PutIntArray(_T("standinclassid"), 2, (const int*)standinclarray),
		_T("standinclassid"));
	errors += highLevelPutError(dflts->PutIntArray(_T("junkclassid"), 2, (const int*)junkclarray),
		_T("junkclassid"));
