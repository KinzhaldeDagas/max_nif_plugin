...
class MarketDefaultTestClassDesc:public ClassDesc2 {
	...
	void*			Create(BOOL loading = FALSE) { return &MarketDefaultTest::theInstance; }
	const TCHAR*	ClassName() { return GetString(IDS_CLASS_NAME); }
	const TCHAR*	NonLocalizedClassName() { return _T("Market Defaults Test"); }
...
MarketDefaultTest MarketDefaultTest::theInstance(
	kTestInterfaceID, _T("marketDefaultsTestInterface"), 0, NULL, FP_CORE,
...
{
	std::unique_ptr<ErrorStatus> status;
	int errors = 0;
	...
	if (stream == NULL || !stream->_is_charstream()) {
		...
		SendMessage(edit, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(_T("")));
		UpdateWindow(edit);

		status.reset(new ErrorStatusWindow(edit, level));
	}
	else {
		status.reset(new ErrorStatusStream(static_cast<CharStream*>(stream), level));
	}
...
INT_PTR CALLBACK MarketDefaultTest::MarketDefaultTestDlgProc(
		HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg) {
		case WM_INITDIALOG:
			MarketDefaultTest::theInstance.Init(hWnd);
			break;

		case WM_DESTROY:
			MarketDefaultTest::theInstance.Destroy(hWnd);
			break;

		case WM_COMMAND:
			switch (LOWORD(wParam)) {
			case IDC_TEST:
				MarketDefaultTest::theInstance.Test();
				break;
			}
			break;
		...
		case WM_LBUTTONUP:
		case WM_MOUSEMOVE:
			MarketDefaultTest::theInstance.ip->RollupMouseMessage(hWnd,msg,wParam,lParam); 
			break;
...
INT_PTR CALLBACK MarketDefaultTest::StatusDialogProc(
		HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg) {
		...
		case WM_DESTROY:
			MarketDefaultTest::theInstance.mhStatus = NULL;
			break;
...
void MarketDefaultTest::ErrorStatus::message(int level, const TCHAR* fmt, ...)
{
	if (level <= mLevel) {
		va_list ap;
		va_start(ap, fmt);
...
void MarketDefaultTest::ErrorStatusWindow::puts(const TCHAR* msg)
{
	...
	len += static_cast<int>(_tcslen(msg));	// SR DCAST64: Downcast to 2G limit.
	SendMessage(mhwnd, EM_SETSEL, len, len);
