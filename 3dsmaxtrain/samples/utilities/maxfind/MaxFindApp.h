/****************************************************************************
 ...
 Christer Janson
 September 19, 1998
 MaxFindApp.h - Main Application header
 ***************************************************************************/
#include <buildver.h>
#define THECLASSNAME	_T("CJMAXFINDER_WNDCLASS")
...
class MaxFindApp {
	...
	// Initialization
	MaxFindApp(HINSTANCE hInst, int cshow);
	~MaxFindApp();
	...
	void		DoCD();
	void		ViewFile();
	BOOL		Qualify(TCHAR* filename);
	BOOL		Compare(const TCHAR* s1, const TCHAR* s2);
	BOOL		Compare(const PROPVARIANT& s1, const TCHAR* s2);
	...
	BOOL		ScanDirectory(TCHAR* path, TCHAR* filespec, HWND hList);
