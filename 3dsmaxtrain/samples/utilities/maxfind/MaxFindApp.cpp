...
//	strcpy((char *)&lf.lfFaceName, "Microsoft Sans Serif");
...
void MaxFindApp::SetRegSettings()
	...
	HKEY	hKey;
	DWORD	disp;
	if (RegCreateKeyEx(HKEY_CURRENT_USER, GetRegistryKey(), 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, &disp) == ERROR_SUCCESS) {
...
BOOL MaxFindApp::ScanDirectory(TCHAR* path, TCHAR* filespec, HWND hList)
	...
	_tcsncpy_s(buf, _countof(buf), GetString(IDS_SEARCHDIR), _TRUNCATE);
	_tcsncat_s(buf, _countof(buf), path, _TRUNCATE);

	SendMessage(hStatusPanel, WM_SETTEXT, 0, (LPARAM)buf);

	_tcsncpy_s(filespecbuf, _countof(filespecbuf), filespec, _TRUNCATE);
	pathPtr = filespecbuf;

	pathToken = _tcstok_s(pathPtr, _T(";, "), &next_token);
	while ( pathToken != NULL )
	{

		_tcsncpy_s(buf, _countof(buf), path, _TRUNCATE);
		if (buf[_tcsnlen(buf, _countof(buf) ) - 1] != _T('\\'))
			_tcsncat_s(buf, _countof(buf), _T("\\"), _TRUNCATE);
		_tcsncat_s(buf, _countof(buf), pathToken, _TRUNCATE);
		...
		if (hFind != INVALID_HANDLE_VALUE) {
			do {
				...
				if (!(findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) &&
					...
					if (buf[_tcsnlen(buf, _countof(buf) ) - 1] != _T('\\'))
						_tcsncat_s(buf, _countof(buf), _T("\\"), _TRUNCATE);
					_tcsncat_s(buf, _countof(buf), findData.cFileName, _TRUNCATE);
					if (Qualify(buf)) {
		...
		pathToken = _tcstok_s(NULL, _T(";, "), &next_token);
	}
	...
	if (SendMessage(hCheckSubdirs, BM_GETCHECK, 0, 0) == BST_CHECKED) {
		...
		_tcsncat_s(buf, _countof(buf), dirSpec, _TRUNCATE);
