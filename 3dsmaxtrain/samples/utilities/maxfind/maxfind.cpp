				 ...
				 LPSTR /*lpszCmdLine*/, int nCmdShow)
{
	...
	pApp = new MaxFindApp(hInstance, nCmdShow);
	pApp->Init();
	...
	if (!hWnd) {
		TSTR msgBoxTitle = pApp->GetString(IDS_FATAL_ERROR);
		MaxSDK::MaxMessageBox(NULL, pApp->GetString(IDS_NOWIN_ERROR), msgBoxTitle, MB_OK);
		return 0;
	}
...
TCHAR *MaxFindApp::GetString(int id)
{
	static TCHAR buf[256];

	if (GetInstance())
		return LoadString(GetInstance(), id, buf, _countof(buf)) ? buf : NULL;
	return NULL;
}
