...
INT_PTR CALLBACK AboutDlgProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM /*lParam*/)
{
	switch (message) {
		case WM_INITDIALOG:
			...
			CenterInWindow(hWnd, GetParent(hWnd));
...
void InitOpenGL(HWND hWnd)
{
	...
	wc.lpszClassName = _T("CJOGLFRAME");
	...
	hGLWnd = CreateWindow(_T("CJOGLFRAME"),
		_T(""),
...
void RenderGeometry()
{
	...
	int numfloats = _countof(objectDef);
...
LRESULT CALLBACK oglWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message) {
		case WM_LBUTTONDOWN:
			SetCapture(hWnd);
			if (Trackball.OnLMouseDown(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)))
				InvalidateRect(hWnd, NULL, TRUE);
			break;
		case WM_MBUTTONDOWN:
			SetCapture(hWnd);
			if (Trackball.OnMMouseDown(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)))
				InvalidateRect(hWnd, NULL, TRUE);
			break;
		case WM_RBUTTONDOWN:
			SetCapture(hWnd);
			if (Trackball.OnRMouseDown(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam))) {
				InvalidateRect(hWnd, NULL, TRUE);
			}
			else {
				rbY = GET_Y_LPARAM(lParam);
			}
			break;
		case WM_LBUTTONUP:
			ReleaseCapture();
			if (Trackball.OnLMouseUp(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)))
				InvalidateRect(hWnd, NULL, TRUE);
		...
		case WM_MBUTTONUP:
			ReleaseCapture();
			if (Trackball.OnMMouseUp(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)))
				InvalidateRect(hWnd, NULL, TRUE);
			PostMessage(hWnd, WM_CHECKSPIN, 0, 0);
		...
		case WM_RBUTTONUP:
			ReleaseCapture();
			if (Trackball.OnRMouseUp(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)))
				InvalidateRect(hWnd, NULL, TRUE);
			PostMessage(hWnd, WM_CHECKSPIN, 0, 0);
		...
		case WM_MOUSEMOVE:
			if (Trackball.OnMouseMove(wParam, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam))) {
				InvalidateRect(hWnd, NULL, TRUE);
			}
			else if (wParam & MK_RBUTTON) {
				// Right button down
				distance += (GET_Y_LPARAM(lParam) - rbY)/40.0f;
				rbY = GET_Y_LPARAM(lParam);
				InvalidateRect(hWnd, NULL, TRUE);
			}
