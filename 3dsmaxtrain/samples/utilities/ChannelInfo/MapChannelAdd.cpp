...
class MapChannelAddClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("UVW Mapping Add"); }
...
void MapChannelAdd::ModifyObject(TimeValue t, ModContext &mc, ObjectState * os, INode *node) 
{
	...
	else if (mesh)
	{
		...
		mesh->setNumMaps(mapCount+1, TRUE);
	}
	else if (pmesh)
	{
		int mapCount = pmesh->getNumMaps();

		pmesh->setNumMaps(mapCount+1, TRUE);
//		pmesh->setNumMapPatches(mapCount,0);
//		pmesh->setNumMapVerts(mapCount,0);
	}
...
void MapChannelAdd::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc)
{
