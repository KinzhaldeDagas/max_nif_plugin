...
class MapChannelPasteClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("UVW Mapping Paste"); }
...
//	Mesh &mesh = tobj->GetMesh();		
	...
	else if (mnmesh)
	{
		...
		if (buffer.numRealFaces == mnmesh->numf)
		{
			...
			{
				int sDeg(0), tDeg(0);
				//The number of uvwMNFaces and geomMNFaces might not be the same as numf, for example, copy mesh to EPoly
				if (buffer.copyType == POLYMAPCHANNEL && buffer.uvwMNFaces.Count() > i) 
					sDeg = buffer.uvwMNFaces[i]->deg;
				else if(buffer.geomMNFaces.Count() > i)
					sDeg = buffer.geomMNFaces[i]->deg;
				tDeg = mnmesh->f[i].deg;
		...
		//if last channel reduce the number of channels
			...
			else if ((buffer.pasteToChannelType == TRIGEOMCHANNEL)|| (buffer.pasteToChannelType == CHANNEL_GEOM))
			{
				...
				else
				{
					for (int i = 0; i < buffer.numFaces; i++)
					{
						// Check for attempted pastes that use incorrect indexes
						if (i < mesh->numFaces && i < buffer.uvwFaces.Count())
						{
				...
				// SAFETY: If there are no vertices in the mesh, we can't have any faces
				...
				if (numV == 0)
				{
					if (mesh->numFaces != 0)
					{
						mesh->setNumFaces(0);
						// Notify user
						the_listener->edit_stream->printf(GetString(IDS_ILLEGAL_PASTE1));
					}
				}
				else
				{
					// Additional safety: Make sure no faces refer to out-of-range vertices
					bool ok = true;
					for (int i = 0; i < mesh->numFaces; ++i)
					{
						if (geomFace[i].v[0] >= numV || geomFace[i].v[1] >= numV || geomFace[i].v[2] >= numV)
						{
							ok = false;
							break;
						}
					...
					if (!ok)
					{
						mesh->setNumFaces(0);
						// Notify user
						the_listener->edit_stream->printf(GetString(IDS_ILLEGAL_PASTE2));
					}
				}
			}
			else if ((buffer.pasteToChannelType == TRISELCHANNEL)|| (buffer.pasteToChannelType == CHANNEL_SEL))
			{
				...
				mesh->VertSel().ClearAll();
				const int vertSelSize = mesh->VertSel().GetSize();
				for (int i = 0; i <vertSelSize; i++)
				{
					if (vsw)		
						vsw[i] = 0.0f;
				}
...
void MapChannelPaste::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc)
{
