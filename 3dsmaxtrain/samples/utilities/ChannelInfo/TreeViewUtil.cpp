...
class TreeViewUtilClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Channel Info"); }
...
class ChannelInfoFunctionPublish : public FPStaticInterface {
	public:
		...
		void NameChannel(INode *node, int channelType, int channel, const TCHAR *name);
		void NameSubChannel(INode *node, int channelType, int channel, int subChannel, const TCHAR *name);
		void Update();
		...
		static void SetDialogShowing(bool aVal)	{
			dialogShowing = aVal;
...
static ChannelInfoFunctionPublish theChannelInfoFunctionPublish(
	...
	ChannelInfoFunctionPublish::kDialog, _T("dialog"), -1, TYPE_VOID, FP_NO_REDRAW, 0,
	ChannelInfoFunctionPublish::kCloseDialog, _T("closeDialog"), -1, TYPE_VOID, FP_NO_REDRAW, 0,
	ChannelInfoFunctionPublish::kIsChecked, _T("isChecked"), -1, TYPE_BOOL, FP_NO_REDRAW, 0,
	ChannelInfoFunctionPublish::kCopyChannel, _T("CopyChannel"), -1, TYPE_VOID, FP_NO_REDRAW, 3,
		_T("node"), -1, TYPE_INODE,
		_T("channelType"), -1, TYPE_INT,
		_T("channel"), -1, TYPE_INT,
	ChannelInfoFunctionPublish::kCopySubChannel, _T("CopySubChannel"), -1, TYPE_VOID, FP_NO_REDRAW, 4,
		_T("node"), -1, TYPE_INODE,
		_T("channelType"), -1, TYPE_INT,
		_T("channel"), -1, TYPE_INT,
	...
	ChannelInfoFunctionPublish::kUpdate, _T("update"), -1, TYPE_VOID, FP_NO_REDRAW, 0,
...
bool ChannelInfoFunctionPublish::dialogShowing = false;
...
void ChannelInfoFunctionPublish::CloseDialog()	{
	if(theTreeViewUtil.floaterHWND != NULL)	{
		DestroyWindow(theTreeViewUtil.floaterHWND);
	}
}

bool ChannelInfoFunctionPublish::IsDialogChecked()	{
	return ChannelInfoFunctionPublish::IsDialogShowing();
}
...
void ChannelInfoFunctionPublish::NameChannel(INode *node, int channelType, int channel, const TCHAR *name)
{
    channelType = channelType + 9;
	theTreeViewUtil.NameChannel(node, channelType, channel, -50, name);
}
void ChannelInfoFunctionPublish::NameSubChannel(INode *node, int channelType, int channel, int subChannel, const TCHAR *name)
{
    channelType = channelType + 9;
...
void CopyBuffer::Clear()
	{
		...
		for (int i =0; i < geomMNFaces.Count(); i++)
		{
			if (geomMNFaces[i])
			{			
				geomMNFaces[i]->Clear();
...
void  CopyBuffer::Copy(CopyBuffer *from)
	{
		...
		for (int i = 0; i < from->uvwMNFaces.Count(); i++)
		{
...
static INT_PTR CALLBACK TreeViewUtilDlgProc(
...
static INT_PTR CALLBACK ViewChannelsFloaterDlgProc(
	...
	switch (msg) {
		case WM_INITDIALOG: 
			...
			ChannelInfoFunctionPublish::SetDialogShowing(true);
		...
		case WM_DESTROY: 
			...
			ChannelInfoFunctionPublish::SetDialogShowing(false);
			break;
			}
			...
			switch (LOWORD(wParam)) 
			{
				case IDC_LOCK:
				{
					...
					if (theTreeViewUtil.lockSelection)
						subString.printf(_T("channelInfo.lock = true"));
					else subString.printf(_T("channelInfo.lock = false"));	
				...
				case IDC_SUBCOMPONENTS:
				{
					...
					if (theTreeViewUtil.subComponents)
						subString.printf(_T("channelInfo.subComp = true"));
					else subString.printf(_T("channelInfo.subComp = false"));
					macroRecorder->ScriptString(subString);
					macroRecorder->EmitScript();
...
int TreeViewUtil::GetDeadGeomVerts(Mesh *msh)
{
	...
	return (int)msh->numVerts - numberSet.NumberSet();
}
...
//		MNFace f = msh->f[i];
	...
	int ct = (int)msh->numv - numberSet.NumberSet();
	return ct;
}
...
int TreeViewUtil::GetDeadMapVerts(PatchMesh *msh, int mp)
{
	...
	//check mp if valid for index access for tab tvPatches[] in msh first, then getMapSupport()
	//mp can be <0 because numberMapChannels stars from -2
	if (mp < 0 || !msh->getMapSupport(mp)) return 0;
...
void TreeViewUtil::AddPatch(PatchMesh *msh, INode *node)
{
		...
		//get the number of maps
			...
			for (int j = 0; j < numberMapChannels; j++) //note patches do not support alpha or illum channels
			{
...
void TreeViewUtil::AddPoly(MNMesh *msh, INode *node)
{
		...
		//get the number of maps
			int numberMapChannels = msh->MNum();
...
void TreeViewUtil::InitListView()
{
	...
	lvC.pszText = const_cast<TCHAR*>(name.data());
	...
	lvC.pszText = const_cast<TCHAR*>(cid.data());
	lvC.iSubItem = numCols;
	ListView_InsertColumn(listViewHWND, numCols++, &lvC);
	...
	lvC.fmt = LVCFMT_LEFT;  // left align the column
	lvC.cx = 120;            // width of the column, in pixels
	lvC.pszText = const_cast<TCHAR*>(cname.data());
	lvC.iSubItem = numCols;
	ListView_InsertColumn(listViewHWND, numCols++, &lvC);
	...
	lvC.pszText = const_cast<TCHAR*>(numVerts.data());
	lvC.iSubItem = numCols;
	ListView_InsertColumn(listViewHWND, numCols++, &lvC);
	...
	lvC.fmt = LVCFMT_LEFT;  // left align the column
	lvC.cx = 70;            // width of the column, in pixels
	lvC.pszText = const_cast<TCHAR*>(numFaces.data());
	lvC.iSubItem = numCols;
	ListView_InsertColumn(listViewHWND, numCols++, &lvC);
	...
	lvC.fmt = LVCFMT_LEFT;  // left align the column
	lvC.cx = 70;            // width of the column, in pixels
	lvC.pszText = const_cast<TCHAR*>(numDVerts.data());
	lvC.iSubItem = numCols;
	ListView_InsertColumn(listViewHWND, numCols++, &lvC);
	...
	lvC.fmt = LVCFMT_LEFT;  // left align the column
	lvC.cx = 60;            // width of the column, in pixels
	lvC.pszText = const_cast<TCHAR*>(ksize.data());
	lvC.iSubItem = numCols;
	ListView_InsertColumn(listViewHWND, numCols++, &lvC);
...
LRESULT TreeViewUtil::NotifyHandler( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(pLvdi->hdr.code)
	{
		...
		case LVN_GETDISPINFO:
			...
			{
				case 0:     // node name
					_stprintf(szText, _T("%s"), nodeList[index]->node->GetName());
				...
				case 1:     // channel ID
					...
					{
						_stprintf(szText,GetString(IDS_vsel));
						pLvdi->item.pszText = szText;
					...
					else if (nodeList[index]->channelType == TRIGEOMCHANNEL)
					{
						if (nodeList[index]->subID == -50)
							_stprintf(szText, _T("%s"), GetString(IDS_mesh));
						else if (nodeList[index]->subID == 0)
							_stprintf(szText,  _T("%s"), GetString(IDS_meshx));
						else if (nodeList[index]->subID == 1)
							_stprintf(szText,  _T("%s"), GetString(IDS_meshy));
						else if (nodeList[index]->subID == 2)
							_stprintf(szText,  _T("%s"), GetString(IDS_meshz));
					...
					{
						if (nodeList[index]->subID == -50)
							_stprintf(szText,  _T("%s"), GetString(IDS_poly));
						else if (nodeList[index]->subID == 0)
							_stprintf(szText,  _T("%s"), GetString(IDS_polyx));
						else if (nodeList[index]->subID == 1)
							_stprintf(szText,  _T("%s"), GetString(IDS_polyy));
						else if (nodeList[index]->subID == 2)
							_stprintf(szText,  _T("%s"), GetString(IDS_polyz));
					...
					{
						if (nodeList[index]->subID == -50)
							_stprintf(szText,  _T("%s"), GetString(IDS_patch));
						else if (nodeList[index]->subID == 0)
							_stprintf(szText,  _T("%s"), GetString(IDS_patchx));
						else if (nodeList[index]->subID == 1)
							_stprintf(szText,  _T("%s"), GetString(IDS_patchy));
						else if (nodeList[index]->subID == 2)
							_stprintf(szText,  _T("%s"), GetString(IDS_patchz));
					...
					else 
					{
						...
						{
							if (nodeList[index]->subID == -50)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_alpha));
							else if (nodeList[index]->subID == 0)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_alphax));
							else if (nodeList[index]->subID == 1)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_alphay));
							else if (nodeList[index]->subID == 2)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_alphaz));

						}
						else if (nodeList[index]->channelID == -1)
						{
							if (nodeList[index]->subID == -50)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_illum));
							else if (nodeList[index]->subID == 0)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_illumx));
							else if (nodeList[index]->subID == 1)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_illumy));
							else if (nodeList[index]->subID == 2)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_illumz));

						}
						else if (nodeList[index]->channelID == 0)
						{
							if (nodeList[index]->subID == -50)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_vc));
							else if (nodeList[index]->subID == 0)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_vcx));
							else if (nodeList[index]->subID == 1)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_vcy));
							else if (nodeList[index]->subID == 2)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_vcz));

						}
						else 
						{
							if (nodeList[index]->subID == -50)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_map));
							else if (nodeList[index]->subID == 0)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_mapx));
							else if (nodeList[index]->subID == 1)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_mapy));
							else if (nodeList[index]->subID == 2)
								_stprintf(szText, _T("%d:%s"), nodeList[index]->channelID,GetString(IDS_mapz));

//							_stprintf(szText, "%d : map", nodeList[index]->channelID);
						}
						pLvdi->item.pszText = szText;
					}

//					_stprintf(szText, "%u", nodeList[index]->channelID);
//					pLvdi->item.pszText = szText;
					break;

				case 2:     // Channel name
					_stprintf(szText, _T("%s"), nodeList[index]->channelName.data());
					pLvdi->item.pszText = szText;
					break;

				case 3:     // num verts
					_stprintf(szText, _T("%d"), nodeList[index]->numVerts);
					pLvdi->item.pszText = szText;
					break;

				case 4:     // Number of faces
					_stprintf(szText, _T("%d"), nodeList[index]->numFaces);
					pLvdi->item.pszText = szText;
					break;
				case 5:     // num dead verts
					_stprintf(szText, _T("%d"), nodeList[index]->numOfDeadVerts);
					pLvdi->item.pszText = szText;
					break;

				case 6:     // size
					_stprintf(szText, _T("%d%s"), nodeList[index]->kbsize,GetString(IDS_kb));
					pLvdi->item.pszText = szText;
					break;
...
void TreeViewUtil::NotifyPreDeleteNode(void* param, NotifyInfo* arg)
{
	...
	if(util == NULL) return;
	INode* deleted_node = GetNotifyParam<NOTIFY_SCENE_PRE_DELETED_NODE>(arg);
...
void TreeViewUtil::DeleteChannel(INode *node, int mapID)
{
	if (node)
	{
		...
		if (obj)
		{
			...
			key.printf(_T("MapChannel:%d"),mapID);
			...
			for (int k = 0; k < 3; k++)
			{
				key.printf(_T("MapChannel:%d(%d)"),mapID,k);
				if (node->UserPropExists(key))
				{
					node->SetUserPropString(key,name);
				}				
		...
		if (node)
		{
			if ((nodeList[i]->channelType == TRIMAPCHANNEL) || (nodeList[i]->channelType == POLYMAPCHANNEL)|| (nodeList[i]->channelType == PATCHMAPCHANNEL))
			{
				...
				if (nodeList[i]->subID == -50)
					key.printf(_T("MapChannel:%d"),id);
				else key.printf(_T("MapChannel:%d(%d)"),id,nodeList[i]->subID);
				if (node->UserPropExists(key))
				{
					node->GetUserPropString(key,name);
					if (name.Length() > 0)
					{
						nodeList[i]->channelName.printf(_T("%s"),name);
					}
				}
			}
			else if ((nodeList[i]->channelType == TRIGEOMCHANNEL)||(nodeList[i]->channelType == POLYGEOMCHANNEL))
			{
				...
				if (nodeList[i]->subID == -50)
					key.printf(_T("GeomChannel"));
				else key.printf(_T("GeomChannel(%d)"),nodeList[i]->subID);
					...
					if (HIWORD(wParam)==EN_CHANGE) 
					{
						...
						if (iName)
						{
							if (iName->GotReturn())
							{
								...
								theTreeViewUtil.channelName.printf(_T("%s"),tempName);
						 		ReleaseICustEdit(iName);						 		
								EndDialog(hWnd,1);
...
void TreeViewUtil::NameChannel(INode *node, int channelType, int channelID, int subID, const TCHAR *name)
{
	TSTR tname, key;
	tname.printf(_T("%s"),name);

	if (channelType == CHANNEL_MAP)
	{
		if (subID == -50)
			key.printf(_T("MapChannel:%d"),channelID);
		else key.printf(_T("MapChannel:%d(%d)"),channelID,subID);
	...
	else if (channelType == CHANNEL_GEOM)
	{
		if (subID == -50)
			key.printf(_T("GeomChannel"));
		else key.printf(_T("GeomChannel(%d)"),subID);
		node->SetUserPropString(key,tname);
	}
...
void TreeViewUtil::NameChannel(int sel)
{
	...
	if ((sel >=0) && (sel < nodeList.Count()))
	{
		...
		channelName.printf(_T("%s"),nodeList[sel]->channelName);
...
//pop up name dialog
		...
		if (iret)
		{
			nodeList[sel]->channelName.printf(_T("%s"),channelName);

			INode *node = nodeList[sel]->node;
			TSTR tname, key;
			tname.printf(_T("%s"),channelName);
			...
			if ((nodeList[sel]->channelType == TRIMAPCHANNEL)||(nodeList[sel]->channelType == POLYMAPCHANNEL))
			{
				...
				{
					key.printf(_T("MapChannel:%d"),nodeList[sel]->channelID);
				...
				{
					key.printf(_T("MapChannel:%d(%d)"),nodeList[sel]->channelID,nodeList[sel]->subID);
			...
			else if ((nodeList[sel]->channelType == TRIGEOMCHANNEL)||(nodeList[sel]->channelType == POLYGEOMCHANNEL))
			{
				...
				{
					key.printf(_T("GeomChannel(%d)"),nodeList[sel]->subID);
...
//collapse it to a mesh
	...
	if (channelType == CHANNEL_SEL)
	{
		...
		buffer.name.printf(_T("%s%s %s"),GetString(IDS_COPYBUFFER),node->GetName(),selChan);
	}
	else if (channelType == CHANNEL_GEOM)
	{
		TSTR geomChan(GetString(IDS_GEOMCHANNEL));
		if (subID == -50)
			buffer.name.printf(_T("%s %s %s"),GetString(IDS_COPYBUFFER),node->GetName(),geomChan);
		else if (subID == 0)
			buffer.name.printf(_T("%s %s %s X "),GetString(IDS_COPYBUFFER),node->GetName(),geomChan);
		else if (subID == 1)
			buffer.name.printf(_T("%s %s %s Y "),GetString(IDS_COPYBUFFER),node->GetName(),geomChan);
		else if (subID == 2)
			buffer.name.printf(_T("%s %s %s Z "),GetString(IDS_COPYBUFFER),node->GetName(),geomChan);
	}
	else if (channelType == CHANNEL_MAP)
	{
		...
		if (subID == -50)
			buffer.name.printf(_T("%s%s %s %d"),GetString(IDS_COPYBUFFER),node->GetName(),mapChan, channelID);
		else if (subID == 0)
			buffer.name.printf(_T("%s%s %s %d X"),GetString(IDS_COPYBUFFER),node->GetName(),mapChan,channelID);
		else if (subID == 1)
			buffer.name.printf(_T("%s%s %s %d Y"),GetString(IDS_COPYBUFFER),node->GetName(),mapChan,channelID);
		else if (subID == 2)
			buffer.name.printf(_T("%s%s %s %d Z"),GetString(IDS_COPYBUFFER),node->GetName(),mapChan,channelID);
...
//				TVFace *uvwFace = msh->mapFaces(mapID);
	...
	if (msh)
	{
		if (channelType == CHANNEL_SEL)
		{
			...
			const BitArray& vertSel = msh->VertSel();
			for (int i = 0; i < numVerts; i++)
			{
				if (vsw)
				{
	...
	if (mnmsh)
	{
		...
		else if (channelType == CHANNEL_MAP)
		{
			if (subID == -50)
			{
				...
				int numVerts,numFaces;
				int mapID = channelID;
				// Watch out for empty map channels...
				if (mnmsh->M(mapID) == nullptr)
				{
					buffer.copyType = EMPTY;
					return;
				}
...
{
//get our geom type
	if ((whichChannel < 0) || (whichChannel >= nodeList.Count()) || nodeList.Count() == 0)
		return;
	...
	if (node)
	{
		...
		if (obj)
		{
			...
			if (mod)
			{
				...
				//Above notify event may delete NodeList memeber, so check the count again here to validate the tab memeber access
				if (nodeList.Count() > 0)
				{
					TSTR none(GetString(IDS_NONE));
					if (_tcscmp(none,nodeList[whichChannel]->channelName) == 0)
					{
...
public:		   
	...
	TSTR Description() {return TSTR(_T("Collapse Restore"));}
};
