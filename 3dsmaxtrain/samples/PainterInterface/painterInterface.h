...
extern TCHAR *GetString( int id, TCHAR* buf, int nbOfChar );
...
enum { painterinterface_nodelist,
	   ...
	   painterinterface_str_ui_limitmin,
	   painterinterface_str_ui_limitmax,

	   painterinterface_splineconstraintnode,
...
class PainterInterface :  public ReferenceTarget, public IPainterInterface_V14, public ViewportDisplayCallback, public TabletCallback

	{
	private:
		IPainterRightClickHandler *mpRightClicker;
	...
	public:	
		...
		void			GetClassName(MSTR& s, bool localized) const override { s = _T("thePainterInterface"); }
		SClass_ID		SuperClassID() { return PAINTERINTERFACE_SUPERCLASS_ID; }
...
public:
		...
		// PainterInterface is a singleton. Therefore.....
...
//Painter Interface	methods
		...
		BOOL  InitializeCallback(ReferenceTarget* targetCanvas) override;
		...
		BOOL  InitializeNodesByObjState(int flags, Tab<INode*> &nodeList, Tab<ObjectState> &objList);

		BOOL  UpdateMeshesByObjState(BOOL updatePointGather, Tab<ObjectState> &objList);

		BOOL  InPaintMode();

		BOOL  StartPaintSession();
		BOOL  StartPaintSession (IPainterRightClickHandler *rightClicker);
		BOOL  EndPaintSession();
		void RightClick () { if (mpRightClicker) mpRightClicker->RightClick (); }
		BOOL  BringUpOptions();
...
//Generic access tools
		...
		float GetStrDragLimitMin();
		void SetStrDragLimitMin(float l);

		float GetStrDragLimitMax();
		void SetStrDragLimitMax(float l);
		...
		void  GetPatchFaceData(PatchMesh &patch, Tab<FaceDataFromPatch> &faceData);

		void Shutdown();

		//Spline constraint functionality
		BOOL useSplineConstraint;
		BOOL GetUseSplineConstraint() {return useSplineConstraint;}
		void SetUseSplineConstraint(BOOL onoff) {useSplineConstraint = onoff;}
		BOOL IsSplineConstraintNodeValid();
		BOOL SetSplineConstraintNode(INode* snode);
		void ConstrainBrushToSpline(IPoint2 &mousePos); //Snap the mouse pos to the spline
	...
	private:
		...
		Array* MXS_functions; // used to store functions, array will be garbage collection protected

		BOOL hitState;
		BOOL inPaintMode;
		IPainterCanvasInterface_V5* canvas;
		IPainterCanvasInterface_V5_1* canvas_5_1;
		IPainterCanvasInterface_V7* canvas_7;
		IPainterCanvasInterface_V26_1* canvas_26_1;
		...
		void SaveWindowPos();
		void LoadWindowPos();
		WINDOWPLACEMENT optionsWindowPos;		
};
...
class PaintMode : public CommandMode {
	public:
		...
		PaintMode(PainterInterface *m,IObjParam *i) 
			: fgProc(m), proc(m,i) {ip=i;painterInterface=m;popPaintMode=TRUE;supportQuadMenu=FALSE;}

		int Class() 
		{
			if (supportQuadMenu)
				return MOVE_COMMAND;
			else return MODIFY_COMMAND;
		}		
...
//								{return (oldMode->ChangeFGProc() != CHANGE_FG_SELECTED);} 
		...
		void ExitMode();
		BOOL popPaintMode;
		void SupportQuadMenu(BOOL support) { supportQuadMenu = support; }
private:
		BOOL supportQuadMenu;
...
class PainterInterfaceClassDesc:public ClassDesc2 {
	...
	void*			Create(BOOL loading = FALSE) { 
						thePainterInterface.Init();
						return &thePainterInterface; 
						}
	const TCHAR*	ClassName() { return GetString(IDS_CLASS_NAME); }
	const TCHAR*	NonLocalizedClassName() { return _T("PainterInterface"); }
	SClass_ID		SuperClassID() { return PAINTERINTERFACE_SUPERCLASS_ID; }
	Class_ID		ClassID() { return PAINTERINTERFACE_CLASS_ID; }
...
enum {
		...
		painterfn_gethitnode,

		painterfn_getdragstrlimitmin,
		painterfn_setdragstrlimitmin,

		painterfn_getdragstrlimitmax,
		painterfn_setdragstrlimitmax,

		painterfn_getusesplineconstraint,
		painterfn_setusesplineconstraint,
		painterfn_setsplineconstraintnode,
		painterfn_issplineconstraintnodevalid,
	};
...
class PainterInterfaceActions : public FPStaticInterface {
	public:
		...
		virtual void fnLoadPaintMacro(const TCHAR *macroString, const TCHAR *macroCategory) =0;
		...
		virtual Point2* fnGetHitMousePos(int tabindex, bool removeUIScaling = false)=0;
		...
		virtual BOOL fnGetTestHit(Point2 p, bool applyUIScaling = false)=0;

		virtual void fnClearStroke() = 0;
		virtual void fnAddToStroke(Point2 mousePos,BOOL rebuildGather, BOOL updateViewPort, bool applyUIScaling = false) = 0;
...
//		virtual void fnGetCustomHitPressureData(BOOL &shift, BOOL &ctrl, BOOL &alt, float &pressure)=0;
		...
		virtual float fnGetStrDragLimitMin() = 0;
		virtual void fnSetStrDragLimitMin(float l) = 0;

		virtual float fnGetStrDragLimitMax() = 0;
		virtual void fnSetStrDragLimitMax(float l) = 0;

		virtual BOOL fnGetUseSplineConstraint() = 0;
		virtual void fnSetUseSplineConstraint(BOOL onoff) = 0;
		virtual BOOL fnSetSplineConstraintNode(INode* snode) = 0;
		virtual BOOL fnIsSplineConstraintNodeValid() = 0;
};
...
public:
	...
	BEGIN_FUNCTION_MAP
		...
		FN_2(painterfn_paintgettesthit,TYPE_BOOL,fnGetTestHit,TYPE_POINT2,TYPE_bool);

		VFN_0(painterfn_paintclearstroke,fnClearStroke);
		VFN_4(painterfn_paintaddtostroke,fnAddToStroke,TYPE_POINT2,TYPE_BOOL,TYPE_BOOL,TYPE_bool);
...
//		VFN_4(painterfn_paintgetcustomhitpressuredata,fnGetCustomHitPressureData,TYPE_BOOL_BR,TYPE_BOOL_BR,TYPE_BOOL_BR,TYPE_FLOAT_BR);
		...
		FN_2(painterfn_paintgethitmousepos,TYPE_POINT2,fnGetHitMousePos,TYPE_INT, TYPE_bool);
		...
		FN_1(painterfn_setsplineconstraintnode,TYPE_BOOL, fnSetSplineConstraintNode, TYPE_INODE);
		FN_0(painterfn_issplineconstraintnodevalid,TYPE_BOOL,fnIsSplineConstraintNodeValid);
		PROP_FNS(painterfn_getusesplineconstraint, fnGetUseSplineConstraint, painterfn_setusesplineconstraint, fnSetUseSplineConstraint, TYPE_BOOL);
		...
		FN_1(painterfn_gethitnode,TYPE_INODE, fnGetHitNode,TYPE_INT);

		PROP_FNS(painterfn_getdragstrlimitmin, fnGetStrDragLimitMin, painterfn_setdragstrlimitmin,fnSetStrDragLimitMin,TYPE_FLOAT);
		PROP_FNS(painterfn_getdragstrlimitmax, fnGetStrDragLimitMax, painterfn_setdragstrlimitmax,fnSetStrDragLimitMax,TYPE_FLOAT);
	...
	void fnLoadPaintMacro(const TCHAR *macroString, const TCHAR *macroCategory);
	...
	Point2 *fnGetHitMousePos(int tabindex, bool removeUIScaling = false);
	...
	BOOL fnGetTestHit(Point2 p, bool applyUIScaling = false);

	void fnClearStroke() ;
	void fnAddToStroke(Point2 mousePos,BOOL rebuildGatherData, BOOL updateViewPort, bool applyUIScaling = false);
...
//	void fnGetCustomHitPressureData(BOOL &shift, BOOL &ctrl, BOOL &alt, float &pressure);
	...
	INode* fnGetHitNode(int index);


	float fnGetStrDragLimitMin();
	void fnSetStrDragLimitMin(float l);

	float fnGetStrDragLimitMax();
	void fnSetStrDragLimitMax(float l);

	BOOL fnGetUseSplineConstraint();
	void fnSetUseSplineConstraint(BOOL onoff);
	BOOL fnSetSplineConstraintNode(INode* snode);
	BOOL fnIsSplineConstraintNodeValid();
