...
public:
	...
	void           SetPoint( int i, const CurvePoint& point ) {curvePts[i]=point;}
...
public:
	...
	const TCHAR*            GetText() {return text;}
...
public:
	...
	const TCHAR*   GetPresetName() {return presetName;}
	void  SetPresetName( const TCHAR* name );
...
public:
	StandinBrushPresetContext( Class_ID contextID, const TCHAR* name,
		Class_ID pluginClassID, SClass_ID pluginSClassID );
	virtual ~StandinBrushPresetContext();
	Class_ID             ContextID() {return contextID;}
	const TCHAR*            GetContextName() {return name.data();}
	...
	const TCHAR*            GetParamName( int paramID )      {return NULL;}
...
public:
	...
	Class_ID       ContextID() { return STDBRUSHPRESETCONTEXT_ID; }
	const TCHAR*            GetContextName() { return contextName; }
...
public:
	...
	int               ReadConfig(const TCHAR *cfg=NULL);
	int               WriteConfig(const TCHAR *cfg=NULL);
...
protected:
	...
	// UI related data
	QPointer< MaxSDK::QmaxToolBar > mToolBar;
	HWND           hToolbar, hDialog;
	...
	// Internal methods
	int               ReadPresets(const TCHAR* cfg);
	int               WritePresets(const TCHAR* cfg);
	...
	int               ReadToolbar(const TCHAR* cfg);
	int               WriteToolbar(const TCHAR* cfg);
	...
	void           fnAddPreset();
	void           fnLoadPresetFile( const TCHAR* file );
	void           fnSavePresetFile( const TCHAR* file );
