...
void LightMesh::AddMesh(Mesh *m,  Matrix3 basetm,  ViewExp *vpt, BOOL buildVNorms)
	...
	mVertVisible.SetSize(m->numVerts);
	mVertVisible.ClearAll();
...
//	face = m->faces;
	face = faces.Count() ? faces.Addr(0) : NULL;
...
//	fnorms.SetCount(m->getNumFaces());
	...
	float width = (float)gw->getWinSizeX();
	float height = (float)gw->getWinSizeY();

	
	BOOL isPerspective = vpt->IsPerspView();
	gw->setTransform(Matrix3(1));

	int vertCount = vertsViewSpace.Count();
	Point3 *verts = vertCount ? vertsViewSpace.Addr(0) : NULL;

	faceVisible.SetSize(m->getNumFaces());
	faceVisible.SetAll();

	BitArray vertsBehindTheCamera;
	vertsBehindTheCamera.SetSize(vertCount);
	vertsBehindTheCamera.ClearAll();
	for (int i =0; i < vertCount; i++)
		...
		{
			if (inPoint.z < 0)
				vertsBehindTheCamera.Set(i,TRUE);
		}
		*verts = outPoint ;
		(*verts).z = inPoint.z;
		bb += *verts;
...
// Calculate the surface normal
		...
		if (isPerspective)
		{
			...
			if ( (v0.z > 0.0f) || (v1.z > 0.0f) || (v2.z > 0.0f) ) 
			{
				//need to find one point in screen
				BOOL inScreen = FALSE;
				if ((v0.x >= 0.0) && (v0.x <= width) &&
					(v0.y >= 0.0) && (v0.y <= height) && (v0.z < 0.0f) )
					inScreen = TRUE;
				if ((v1.x >= 0.0) && (v1.x <= width) &&
					(v1.y >= 0.0) && (v1.y <= height) && (v1.z < 0.0f) )
					inScreen = TRUE;

				if ((v2.x >= 0.0) && (v2.x <= width) &&
					(v2.y >= 0.0) && (v2.y <= height) && (v2.z < 0.0f) )
					inScreen = TRUE;
...
//		else faceVisible.Set(i,FALSE);
		...
		face++;	// STEVE A: Moved this down from the loop control to prevent it from being called when count == 0, face == NULL.
...
// Calculate the surface normal
			...
			face++;	// STEVE A: Moved this down from the loop control to prevent it from being called when getNumFaces == 0, face == NULL.
			}
		for (int i = 0; i < m->numVerts; i++) 
			{
			if (normCount[i]!=0)
...
//now stuff the points into the boudningbox list
	for (int m  = 0; m < meshList.Count(); m++)
		...
		if (meshList[m])
			...
			if (!meshList[m]->faces.Count()) continue;
			Face *faces = meshList[m]->faces.Addr(0);
			BitArray facesToAdd;
			facesToAdd.SetSize(meshList[m]->faces.Count());

			for (int i =0; i < meshList[m]->faces.Count(); i++)
				{
...
Point3 BoundsTree::GetWorldCenter()
{
	...
	for (int m  = 0; m < meshList.Count(); m++)
		{
		if (meshList[m] && meshList[m]->vertsWorldSpace.Count())
			{
...
const BitArray& BoundsTree::GetVertexVisibleData(int index)
{
	return meshList[index]->mVertVisible;
}
