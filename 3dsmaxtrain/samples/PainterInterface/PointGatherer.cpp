...
inline float LineToPoint(Point3 p, Point3 lA, Point3 lB, float &u)
{
	Point3 vectorAB = lB-lA;
	float lengthAB = sqrt( DotProd(vectorAB,vectorAB) );
	...
	{
		Point3 vectorAP = p-lA;
		Point3 normVectorAB = (vectorAB*(1.0f/lengthAB));
		distPalongAB = DotProd( normVectorAB, vectorAP );
	}

	if( distPalongAB<=0 )
		u = 0;
	else if( distPalongAB>lengthAB )
		u = 1;
	else
		u = distPalongAB/lengthAB;

	Point3 linePoint = lA + ((lB-lA) * u); // lerp

	float retval = (p-linePoint).Length();
	if( (!_finite(retval)) || (retval<0) )
		retval = 0;
...
if (dot >= 1.0f) dot = 0.9999f; // Michaelson Britt
Angle =  acos(dot); // returns 1.#INF when dot is 1.0?
if (Angle > (3.14/2.0))
	{
	dist = Length(p1-l1);
	u = 0.0001f;
...
/*
DebugPrint(_T("width %d upperLeft %f %f %f   offx %f offy %f offz %f\n"),
		   width,
...
void PointGatherer::FreeCells(BOOL nukeCustomPoints)
{
	...
	for (int i =0; i < CellDataXY.Count(); i++)
		{
...
//DebugPrint(_T("Point %f %f %f added to cell %d %d\n"),p->x,p->y,p->z,x,y);
	...
	for (int i = 0; i < cellData.Count(); i++)
		...
		for (int j = 0; j < cellElementCount[i]; j++)
			...
			dist = LineToPoint(p,a,b,u);
				...
				if (dist <= envDist)
					{
//DebugPrint(_T("dist %f envdist %f\n"),dist,envDist);
...
//DebugPrint(_T("Solving Stroke %f %f %f \n"),a.x,a.y,a.z);
//DebugPrint(_T("               %f %f %f \n"),b.x,b.y,b.z);
