...
void PainterInterface::RunCommand( TSTR category, TSTR command)
	...
	try 
	{
		...
		MaxSDK::VariableGuard<MAXScript::ScriptSource> guard(_tls->scriptSource, MAXScript::ScriptSource::Dynamic);
	...
	{
		ProcessUnknownMAXScriptException(_T("Unwrap UI Script CloseUnwrapUI Macro not found"), true, false, true);
	}
...
//	scriptCallback = TRUE;
	...
	catch (MaxShutdownException)
	{
		...
		{
		ProcessUnknownMAXScriptException(_T("Paint macro not found"), true, false, true);
		}
	vl.util_script->close();
	}
...
void PainterInterface::RunFunction(Value* fn)
{
	...
	{
		fn = NULL;
		ProcessUnknownMAXScriptException(_T("PainterInterface function"), false, true, true);
	}
