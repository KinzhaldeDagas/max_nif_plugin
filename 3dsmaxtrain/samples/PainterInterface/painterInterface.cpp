...
class PainterInterfacePBAccessor : public PBAccessor {
	public:
	void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)
		{ ((PainterInterface*)owner)->OnParamSet( v, id, tabIndex, t ); }
	void Get(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t, Interval &valid)
		{ ((PainterInterface*)owner)->OnParamGet( v, id, tabIndex, t, valid ); }
}  thePainterInterfacePBAccessor;
...
//table of nodes that we check to hit against
	...
	painterinterface_drawring,    _T("drawRing"),  TYPE_BOOL,	0,	IDS_PW_DRAWRING,
		p_accessor,		&thePainterInterfacePBAccessor,
		p_default,		TRUE,
		p_end,
	...
	painterinterface_falloffgraph,    _T("falloffGraph"), 	TYPE_REFTARG, 	P_SUBANIM, 	IDS_PW_FALLOFFGRAPH,
		p_accessor,		&thePainterInterfacePBAccessor,
		p_nonLocalizedName, _T("Falloff Graph"),
		p_end,
	...
	painterinterface_predefinedstrgraph,    _T("predefinedStrGraph"), 	TYPE_REFTARG, 	P_SUBANIM, 	IDS_PW_PREDEFINEDSTRGRAPH,
		p_accessor,		&thePainterInterfacePBAccessor,
		p_nonLocalizedName, _T("Predefined Str. Graph"),
		p_end,
	...
	painterinterface_predefinedsizegraph,    _T("predefinedSizeGraph"), 	TYPE_REFTARG, 	P_SUBANIM, 	IDS_PW_PREDEFINEDSIZEGRAPH,
		p_accessor,		&thePainterInterfacePBAccessor,
		p_nonLocalizedName, _T("Predefined Size Graph"),
		p_end,
	...
	painterinterface_str_ui_limitmin,    _T("strDragLimitMin"), TYPE_FLOAT,	0,	IDS_PW_UISTRLIMITMIN,
		p_default,		0.0f,
		p_end,

	painterinterface_str_ui_limitmax,    _T("strDragLimitMax"), TYPE_FLOAT,	0,	IDS_PW_UISTRLIMITMAX,
		p_default,		1.0f,
		p_end,

	painterinterface_splineconstraintnode,    _T("SplineConstraintNode"), TYPE_INODE,   0,      0,
            p_end,
...
static PainterInterfaceActionsIMP painterInterfaceIMP(PAINTERINTERFACEV5_INTERFACE, _T("thePainterInterface"), 0, &painterInterfaceDesc, 0 ,
  //item ID,  internal_name, description_string, flags
	...
	painterfn_paintgethitmousepos, _T("getHitMousePos"), 0, TYPE_POINT2, 0, 2,
			_T("tabIndex"), 0, TYPE_INT,
			_T("removeUIScaling"), 0, TYPE_bool, f_keyArgDefault, true,
	...
	painterfn_paintaddtostroke, _T("addToStroke"), 0, TYPE_VOID, 0, 4,
			_T("pos"), 0, TYPE_POINT2,
			_T("updatePointGather"), 0, TYPE_BOOL,
			_T("updateViewPort"), 0, TYPE_BOOL,
			_T("applyUIScaling"), 0, TYPE_bool, f_keyArgDefault, true,
	...
	painterfn_paintgettesthit, _T("getTestHit"), 0, TYPE_BOOL, 0, 2,
	...
	painterfn_gethitnode, _T("getHitNode"), 0, TYPE_INODE, 0, 1,
			_T("sampleIndex"), 0, TYPE_INDEX,

	painterfn_setsplineconstraintnode, _T("setSplineConstraintNode"), 0, TYPE_BOOL, 0, 1,
			_T("node"), 0, TYPE_INODE,
	painterfn_issplineconstraintnodevalid, _T("isSplineConstraintNodeValid"), 0, TYPE_BOOL, 0, 0,

	properties,

	painterfn_getusesplineconstraint, painterfn_setusesplineconstraint, _T("useSplineConstraint"), IDS_PW_USESPLINECONSTRAINT,
		TYPE_BOOL,
	...
	painterfn_getdragstrlimitmin, painterfn_setdragstrlimitmin, _T("dragStrLimitMin"), IDS_PW_UISTRLIMITMIN,
		TYPE_FLOAT,

	painterfn_getdragstrlimitmax, painterfn_setdragstrlimitmax, _T("dragStrLimitMax"), IDS_PW_UISTRLIMITMAX,
		TYPE_FLOAT,
...
static void
ShutdownNotifyProc(void* param, NotifyInfo* pInfo)
{
	PainterInterface* pPainterInterface = (PainterInterface*) param;
	pPainterInterface->Shutdown();
}

static void
ResetNotifyProc(void* param, NotifyInfo* pInfo)
{
	PainterInterface* pPainterInterface = (PainterInterface*) param;
	pPainterInterface->CloseOptionsDialog();
}
...
//	UpdateColors();
	...
	useSplineConstraint = FALSE;

	RegisterNotification(ShutdownNotifyProc, this, NOTIFY_SYSTEM_SHUTDOWN);
	RegisterNotification(ResetNotifyProc, this, NOTIFY_SYSTEM_PRE_RESET);
...
PainterInterface::~PainterInterface() 
{	
	CloseTablet();	// Just in case it was left open
	UnRegisterNotification(ShutdownNotifyProc, this, NOTIFY_SYSTEM_SHUTDOWN);
	UnRegisterNotification(ResetNotifyProc, this, NOTIFY_SYSTEM_PRE_RESET);
	Shutdown();  // cleanup for when doing unit tests
}

void PainterInterface::Init() {
	BuildParamBlock();
	CreateCurveControl();
	GetBrushPresetMgr()->Init( this );
}

// LAM - 8/16/04 - moved following from dtor since when the dtor is called, the plugin implementing the
// CurveControl class has been unloaded. Instances of CurveControl are in the pblock, so DeleteAllRefsFromMe() 
// was causing an access violation.
void PainterInterface::Shutdown()
{
	if (paintMode != NULL)
	{
...
void PainterInterface::OnParamSet( PB2Value& v, ParamID id, int tabIndex, TimeValue t) {
	UpdateDependents();
}

void PainterInterface::OnParamGet( PB2Value& v, ParamID id, int tabIndex, TimeValue t, Interval &valid) {
	// no-op
}

void PainterInterface::UpdateDependents() {
	if( canvas_7!=NULL ) canvas_7->OnPaintBrushChanged();

	//FIXME: too time intensive?  any way to store up notifications, and send them as a batch?
	BrushPresetMgr* mgr = GetBrushPresetMgr();
	StdBrushPresetContext* context = GetStdBrushPresetContext();
	if( (mgr!=NULL) && (context!=NULL) )
		mgr->OnContextUpdated( context->ContextID() );
}
...
//setup curve control
		...
		if (curveCtl)
			...
			curveCtl->RegisterResourceMaker(this);
...
//setup curve control
		...
		if (curvePredefinedStrCtl)
			...
			curvePredefinedStrCtl->RegisterResourceMaker(this);
...
//			ICurve *pCurve = NULL;
			...
			pPredefinedStrCurve->SetCanBeAnimated(FALSE);
...
//setup curve control
		...
		if (curvePredefinedSizeCtl)
			...
			curvePredefinedSizeCtl->RegisterResourceMaker(this);
	...
	if (hitState)
		...
		if (inPaintMode)
			...
			if (inStrChange)
				...
				TSTR strn;
				strn.printf(_T("%s %0.2f"), GetString(IDS_GIZMO_STRENGTH), str);
				ipoint.x += 30;
				ipoint.y -= 30;
			...
			if (inRadiusChange)
				...
				gw->wTransPoint(&worldSpaceHit, &ipoint);
				TSTR strn;
				strn.printf(_T("%s %0.2f"), GetString(IDS_GIZMO_RADIUS), radius);
				ipoint.x += 30;
				ipoint.y -= 30;
				gw->wText(&ipoint, strn);
				}
			...
			if (drawTrace)
				...
				if (mirrorEnable)
					{
					for (int i = 1; i < traceWorldSpaceMirror.Count(); i++)
						{
						Point3 plist[2+1];
...
void PainterInterface::GetViewportRect(TimeValue t, ViewExp *vpt, Rect *rect)
{
	...
	{	
		// why are we here?
		DbgAssert(!_T("Doing GetViewportRect() on invalid viewport!"));
		rect->SetW(0);
		rect->SetH(0);
		return;
	}
...
void* PainterInterface::GetInterface(ULONG id)
{
	switch (id)
	{
	case PAINTERINTERFACE_V5:
		return static_cast<IPainterInterface_V5*>(this);
		break;
	case PAINTERINTERFACE_V7:
		return static_cast<IPainterInterface_V7*>(this);
		break;
	case PAINTERINTERFACE_V14:
		return static_cast<IPainterInterface_V14*>(this);
		break;
	default:
...
BaseInterface* PainterInterface::GetInterface( Interface_ID id )
{
	BaseInterface *retVal = NULL;
	if(id == PAINTERINTERFACEV5_INTERFACE)
		retVal = &painterInterfaceIMP;
	return retVal;
}

BOOL  PainterInterface::InitializeCallback(ReferenceTarget* targetCanvas)
{
	if (targetCanvas != nullptr)
	{
		canvas = static_cast<IPainterCanvasInterface_V5*>(targetCanvas->GetInterface(PAINTERCANVASINTERFACE_V5));
		canvas_5_1 = static_cast<IPainterCanvasInterface_V5_1*>(targetCanvas->GetInterface(PAINTERCANVASINTERFACE_V5_1));
		canvas_7 = static_cast<IPainterCanvasInterface_V7*>(targetCanvas->GetInterface(PAINTERCANVASINTERFACE_V7));
		canvas_26_1 = static_cast<IPainterCanvasInterface_V26_1*>(targetCanvas->GetInterface(PAINTERCANVASINTERFACE_V26_1));
	}
	else
	{
		canvas = nullptr;
...
//update quad trees now with the meshes
		...
		{
		DebugPrint(_T("Error PainterInterface::InitializeNodes for some reason pblock is not initialized\n"));
		return FALSE;
		}
...
BOOL  PainterInterface::InitializeNodesByObjState(int flags, Tab<INode*> &nodeList, Tab<ObjectState> &objList)
	{
	UpdateColors();
	if	(pblock)
		{
		...
		for (int i =0; i < nodeList.Count(); i++)
			...
			if (node)
				{
				this->objList.Append(1,&(objList[i]));
				pblock->Append(painterinterface_nodelist,1,&node);
				node->FlagForeground(GetCOREInterface()->GetTime());
				}
			}
//update quad trees now with the meshes
		UpdateMeshesByObjState(TRUE,objList);
...
BOOL  PainterInterface::UpdateMeshes(BOOL updatePointGather) 
	{
	if( objList.Count()>0 ) return UpdateMeshesByObjState( updatePointGather, objList );
			...
			if (sos.obj)
				...
				if (sos.obj->IsSubClassOf(triObjectClassID))
					...
					{						
						...
						MNMesh *pmsh = &pobj->GetMesh();
						polyMesh.setNumVerts(pmsh->numv);
						for (int vi = 0; vi < pmsh->numv; vi++)
							polyMesh.verts[vi] = pmsh->v[vi].p;

						int realFaceCount = 0;
						for (int fi = 0; fi < pmsh->numf; fi++)
						{
							if (!pmsh->f[fi].GetFlag(MN_DEAD))
								realFaceCount += pmsh->f[fi].deg-2;
						}

						polyMesh.setNumFaces(realFaceCount);
						int currentFace = 0;
						Tab<int> triangulation;
						for (int fi = 0; fi < pmsh->numf; fi++)
						{
							if (pmsh->f[fi].GetFlag(MN_DEAD)) continue;
							int numFaces = pmsh->f[fi].deg-2;
							pmsh->f[fi].GetTriangles (triangulation);
							int currentCorner = 0;
							bool hidden = false;
							if (pmsh->f[fi].GetFlag(MN_HIDDEN))
								hidden = true;
							for (int k = 0; k < numFaces; k++)
							{
								int a = triangulation[currentCorner++];
								int b = triangulation[currentCorner++];
								int c = triangulation[currentCorner++];
								polyMesh.faces[currentFace].SetHide(hidden);
								polyMesh.faces[currentFace].v[0] = pmsh->f[fi].vtx[a];
								polyMesh.faces[currentFace].v[1] = pmsh->f[fi].vtx[b];
								polyMesh.faces[currentFace].v[2] = pmsh->f[fi].vtx[c];
								currentFace++;
							}
						}
						msh = &polyMesh;
				...
				if (msh != NULL)
					{
					meshSearchTree.BuildMeshData(msh,basetm,vpt.ToPointer(),i);
					numberNodes++;
					}
				else 
					{	
...
//get mesh
			if (node)
				...
				if ((i < customPoints.Count()) && (customPoints[i] != NULL))
					...
					const BitArray& vertexVisible = meshSearchTree.GetVertexVisibleData(ct);
					pointGather.SetCount(ct, pointCount);
					for (int j = 0; j < pointCount; j++)
						{
						if ((j >= vertexVisible.GetSize()) || (vertexVisible[j]))
							pointGather.Add(&worldSpacePoints[j], j, ct);
						}
					}
...
BOOL  PainterInterface::UpdateMeshesByObjState(BOOL updatePointGather, Tab<ObjectState> &objList) 
	{
...
//FIX
		...
		if (updateViewPort)
		{
			if (node)
			{
				...
				bool isGeometryConstant = false;
				if (canvas_26_1 != nullptr)
				{
					isGeometryConstant = canvas_26_1->IsGeometryConstant();
				}

				node->InvalidateRect(t, isGeometryConstant);
...
void PainterInterface::EndPaintMode()
{
	...
	//end paint mode
		...
		fnCancelStrokeHandler = NULL;
		fnSystemEndHandler = NULL;
		if (MXS_functions != NULL)
			{
			ms_make_collectable(MXS_functions);
			MXS_functions = NULL;
			}
		}
}
...
BOOL  PainterInterface::StartPaintSession() 
	{
		return StartPaintSession (NULL);
	}

BOOL PainterInterface::StartPaintSession (IPainterRightClickHandler *rightClicker)
	{
	mpRightClicker = rightClicker;

	if (rightClicker == NULL)
		paintMode->SupportQuadMenu(TRUE);
	else paintMode->SupportQuadMenu(FALSE);
	...
	if (canvas_5_1)
		...
		if (GetCOREInterface()->GetCommandMode()== paintMode)
		{
			...
			else GetCOREInterface()->SetStdCommandMode(CID_OBJMOVE);
		}
		else GetCOREInterface()->PushCommandMode(paintMode);
		return TRUE;
		}
	else
		{
		...
		inPaintMode = TRUE;
	//start paint mode		
		GetCOREInterface()->PushCommandMode(paintMode);
		CreateCurveControl();
	//make sure to update the meshes
		UpdateMeshes(TRUE);
		OpenTablet();
...
BOOL  PainterInterface::EndPaintSession()  
	...
	if (canvas_5_1)
		...
		if (GetCOREInterface()->GetCommandMode() == paintMode) 
		{
			...
			else if (paintMode->popPaintMode)
				GetCOREInterface()->PopCommandMode();
		}
	//make sure we release the bounds tree since it can consume lots of mememory
		meshSearchTree.FreeCaches();
...
BOOL PainterInterface::SystemEndPaintSession()
{
...
//			RunCommand(macroCallbackCategory,"SystemEndPaintSession");
	...
	// Use conditional again, since some canvases manage to EndPaintSession themselves:
	if (systemEndPaintSession) EndPaintSession();
...
BOOL  PainterInterface::ResizeStr(IPoint2 mousePos, int &mousePoint)
	...
	if (mousePoint == 0)
		...
		{
		float str = oldStr +  (originalPos.y-mousePos.y)/500.0f;

		if (str < GetStrDragLimitMin()) str = GetStrDragLimitMin();
		if (str > GetStrDragLimitMax()) str = GetStrDragLimitMax();
...
BOOL PainterInterface::SetSplineConstraintNode(INode* snode)
{
	if (!snode) return FALSE;

	//Check if the node is a shape object
	ObjectState os = snode->EvalWorldState(GetCOREInterface()->GetTime());
	if (!(os.obj->IsShapeObject())) return FALSE;

	pblock->SetValue(painterinterface_splineconstraintnode, GetCOREInterface()->GetTime(),snode);
	return TRUE;
}
BOOL PainterInterface::IsSplineConstraintNodeValid()
{
	INode* splineConstraintNode = NULL;
	pblock->GetValue(painterinterface_splineconstraintnode,GetCOREInterface()->GetTime(),splineConstraintNode,FOREVER);
	return (splineConstraintNode == NULL) ? FALSE : TRUE;
}
void PainterInterface::ConstrainBrushToSpline(IPoint2 &mousePos)
{
	Interface* ip = GetCOREInterface();
	TimeValue t = ip->GetTime();

	INode* splineConstraintNode = NULL;
	pblock->GetValue(painterinterface_splineconstraintnode,t,splineConstraintNode,FOREVER);
	if (splineConstraintNode == NULL) return;

	ObjectState os = splineConstraintNode->EvalWorldState(t);
	if (!(os.obj->IsShapeObject())) return;
	ShapeObject* obj = (ShapeObject*)os.obj;

	//Get ray from mouse and two positions on the ray.
	...
	Ray ray;
	vpt.MapScreenToWorldRay(float(mousePos.x), float(mousePos.y), ray);
	
	Matrix3 tm = splineConstraintNode->GetObjectTM(t);
	Point3 pos3 = ray.p;
	Point3 pos4 = pos3 + (ray.dir * 1000000.0f);
	Point3 raySegment = pos4 - pos3;
	float rayLengthSq = 1000000.0f * 1000000.0f;

	//Find the closest point on spline to the ray. Check all segments in the spline for the closest
	//point on the segment to the ray and get the distance from that point to the ray.
	Point3 closestPos(0,0,0);
	float closestDist = 3.4E+38f;

	PolyShape shape;
	obj->MakePolyShape(t,shape);
	int numLines = shape.numLines;
	for (int i=0;i<numLines;i++)
	{
		...
		numPts = closed ? numPts : numPts - 1;
		Point3 lastPos;
		for (int d=0;d<numPts;d++)
		{
			Point3 pos1 = d ? lastPos : line->pts[d].p * tm;
			Point3 pos2;
			if (closed && d == numPts - 1) //Re-use first knot if line is closed
			{
				pos2 = lastPos = line->pts[0].p * tm;
			}
			else
			{
				pos2 = lastPos = line->pts[d+1].p * tm;
			}
			Point3 p13 = pos1 - pos3;
			Point3 p43 = pos4 - pos3;
			Point3 p21 = pos2 - pos1;
			float d1 = p13 % p43;
			float d2 = p43 % p21;
			float d3 = p13 % p21;
			float d4 = p43 % p43;
			float d5 = p21 % p21;
			float denom = (d5 * d4) - (d2 * d2);
			if (denom == 0.0f) continue;
			float along = ((d1 * d2) - (d3 * d4)) / denom;
			Point3 segPos;
			if (along < 0.0f) segPos = pos1;
			else if (along > 1.0f) segPos = pos2;
			else segPos = pos1 + ((pos2 - pos1) * along); //Position on spline segment

			float alongVal = (segPos - pos3) % p43;
			float alongRay = alongVal / rayLengthSq;
			Point3 rayPos = pos3 + (raySegment * alongRay); //Position on ray
			float dist = (rayPos - segPos).LengthSquared();
			if (dist < closestDist)
			{
				closestDist = dist;
				closestPos = segPos;
	...
	//Transform into screen coordinates and assign as new mouse pos
	Point3 screenPos;
	gw->transPoint(&closestPos,&screenPos);
	mousePos.x = (int) screenPos.x;
	mousePos.y = (int) screenPos.y;
}
BOOL  PainterInterface::HitTest(IPoint2 mousePos, int &mousePoint, int flags, BOOL rebuildPointGatherData)
{
	...
	//Snap brush to spline if spline constraint is active
	if (useSplineConstraint)
	{
		ConstrainBrushToSpline(mousePos);
	}
	...
	if (mousePoint == 0)
	{
		...
		if (hit)
		{
			...
			else if ((!updateOnMouseUp) )
			{
				RunFunction(fnPaintStrokeHandler);
			}
			TimeValue t = GetCOREInterface()->GetTime();
			if (node)
			{
	...
	else if (mousePoint == 1)
	{
		...
		if (hit)
		{
			...
			if (mirrorEnable) 
				AppendTempMirrorList( worldSpaceHitMirror, worldSpaceNormalMirror, hitPointLocalMirror, normalLocalMirror);
	...
	else if (mousePoint == 5)
	{
		...
		DWORD findex = 0;
		int nodeIndex = -1;
		Matrix3 toWorld(TRUE); // Initialize to Identify matrix
		...
		if ( (hit) || (hitState && (!hit)))
		{
			...
			{
				TimeValue t = GetCOREInterface()->GetTime();
				INode* node = pblock->GetINode(painterinterface_nodelist,0,nodeIndex);
				if (node)
				{
					node->InvalidateRect(t, isGeometryConstant);
				}
...
void PainterInterface::GetSizeAndStr(float &size, float &str)
	...
	str = maxStr;
	size = maxSize;
	if (GetPressureAffects()==PRESSURE_AFFECTS_STR)
		{
		str = minStr + (maxStr - minStr) * fpressure;
		}
	else if (GetPressureAffects()==PRESSURE_AFFECTS_SIZE)
		{
		size = minSize + (maxSize - minSize) * fpressure;
		}
	else if (GetPressureAffects()==PRESSURE_AFFECTS_BOTH)
		{
		size = minSize + (maxSize - minSize) * fpressure;
		str = minStr + (maxStr - minStr) * fpressure;
		}	
...
INode* PainterInterface::GetHitNode( int tabindex)
	{
	if ( (tabindex < 0) || (tabindex >= worldPointList.Count()) )
		tabindex = worldPointList.Count()-1;
	if (tabindex < 0) return NULL;

	return nodeList[tabindex];
...

{
	if (MXS_functions == NULL)
	{
		MXS_functions = new (GC_PERMANENT) Array(5); // garbage projected
		MXS_functions->size = 5;
	}
  	MXS_functions->data[0] = fnStartStrokeHandler	= startStroke;
  	MXS_functions->data[1] = fnPaintStrokeHandler	= paintStroke;
  	MXS_functions->data[2] = fnEndStrokeHandler		= endStroke;
  	MXS_functions->data[3] = fnCancelStrokeHandler	= cancelStroke;
  	MXS_functions->data[4] = fnSystemEndHandler		= systemEnd;

}


void  PainterInterface::GetPatchFaceData(PatchMesh &sourcePatch, Tab<FaceDataFromPatch> &faceData)
{
//add face mapping
	PatchMesh pmesh(sourcePatch);
	
	Matrix3 tm(1);
	pmesh.selLevel = PATCH_OBJECT;
	pmesh.ApplyUVWMap(MAP_FACE,1.0f,1.0f,1.0f,false,false,false,false,tm,1);

	Mesh msh;
	DWORD convertFlags = 0;
	Tab<int> fid;
	ConvertPatchToMeshWithMapping (pmesh, msh, &fid, convertFlags);

	faceData.SetCount(msh.numFaces);
	TVFace *tf = msh.mapFaces(1);
	Point3 *tv = msh.mapVerts(1);

	for (int i = 0; i < msh.numFaces; i++)
	{	
		faceData[i].owningPatch = fid[i];
		faceData[i].st[0].x = tv[tf[i].t[0]].x;
		faceData[i].st[0].y = tv[tf[i].t[0]].y;

		faceData[i].st[1].x = tv[tf[i].t[1]].x;
		faceData[i].st[1].y = tv[tf[i].t[1]].y;

		faceData[i].st[2].x = tv[tf[i].t[2]].x;		
		faceData[i].st[2].y = tv[tf[i].t[2]].y;		
	}

}

float PainterInterface::GetStrDragLimitMin()
{
	float min;
	pblock->GetValue(painterinterface_str_ui_limitmin,0,min,FOREVER);
	return min;	

}
void PainterInterface::SetStrDragLimitMin(float l)
{
	
	pblock->SetValue(painterinterface_str_ui_limitmin,0,l);

}

float PainterInterface::GetStrDragLimitMax()
{
	float max;
	pblock->GetValue(painterinterface_str_ui_limitmax,0,max,FOREVER);
	return max;	

}
void PainterInterface::SetStrDragLimitMax(float l)
{
	
	pblock->SetValue(painterinterface_str_ui_limitmax,0,l);
