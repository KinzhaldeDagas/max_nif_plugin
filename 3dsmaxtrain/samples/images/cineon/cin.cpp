...
GetResIDCaption(int id)
{
    ...
    if (hInst)  return LoadString(hInst, id, captionBuffer, _countof(captionBuffer)) ? captionBuffer : NULL;
    else        return NULL;
}
...
CIN_ImageInfoDialogProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: {
     ...
     // dialog text fields
		SetDlgItemText(hWnd, IDC_INFO_DLG_FILE_NAME,            TSTR::FromCP(codePage, dpxInfo->mCineonImage.GetImageFileName()).data());
        SetDlgItemText(hWnd, IDC_INFO_DLG_FILE_VERSION,         TSTR::FromCP(codePage, dpxInfo->mCineonImage.GetVersionNumber()).data());
        SetDlgItemText(hWnd, IDC_INFO_DLG_FILE_CREATION_DATE,   TSTR::FromCP(codePage, dpxInfo->mCineonImage.GetImageCreationDate()).data());
        SetDlgItemText(hWnd, IDC_INFO_DLG_FILE_CREATION_TIME,   TSTR::FromCP(codePage, dpxInfo->mCineonImage.GetImageCreationTime()).data());
     ...
     // dialog numbered fields
        ...
        _stprintf(buf,_T("%10u"),     dpxInfo->mCineonImage.GetFileSize());
        SetDlgItemText(hWnd, IDC_INFO_DLG_FILE_SIZE,            buf);
        _stprintf(buf,_T("%2u"),      dpxInfo->mCineonImage.GetNumberChannels());
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_NUMBER_CHANNELS,buf);
        _stprintf(buf,_T("%3u"),      dpxInfo->mCineonImage.GetBitsPerPixel());
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_BITS_PER_PIXEL, buf);
        _stprintf(buf,_T("%10u"),     dpxInfo->mCineonImage.GetPixelsPerLine());
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_PIXELS_PER_LINE,buf);
        _stprintf(buf,_T("%10u"),     dpxInfo->mCineonImage.GetLinesPerImage());
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_LINES_PER_IMAGE,buf);
        
#ifndef BASIC_INFO_DLG
        _stprintf(buf,_T("%.2g , %.2g"),dpxInfo->mCineonImage.GetWhitePt(0), dpxInfo->mCineonImage.GetWhitePt(1));
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_WHITE_PT,       buf);
        _stprintf(buf,_T("%.2g , %.2g"),dpxInfo->mCineonImage.GetRedPt(0),   dpxInfo->mCineonImage.GetRedPt(1));
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_RED_PT,         buf);
        _stprintf(buf,_T("%.2g , %.2g"),dpxInfo->mCineonImage.GetGreenPt(0), dpxInfo->mCineonImage.GetGreenPt(1));
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_GREEN_PT,       buf);
        _stprintf(buf,_T("%.2g , %.2g"),dpxInfo->mCineonImage.GetBluePt(0),  dpxInfo->mCineonImage.GetBluePt(1));
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_BLUE_PT,        buf);
        _stprintf(buf,_T"(%3u"),      dpxInfo->mCineonImage.GetDataInterleave());
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_DATA_INTERLEAVE,buf);
        _stprintf(buf,_T("%3u"),      dpxInfo->mCineonImage.GetPacking());
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_PACKING,        buf);
        _stprintf(buf,_T"%2u"),      dpxInfo->mCineonImage.GetSigned());
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_SIGNED,         buf);
        _stprintf(buf,_T"%2u"),      dpxInfo->mCineonImage.GetSense());
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_SENSE,          buf);
        _stprintf(buf,_T"%10u"),     dpxInfo->mCineonImage.GetEOLPadding());
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_EOL_PADDING,    buf);
        _stprintf(buf,"%10u"),     dpxInfo->mCineonImage.GetEOCPadding());
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_EOC_PADDING,    buf);
#endif
        _stprintf(buf,_T("%.2g"),     dpxInfo->mCineonImage.GetImageGamma());
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_GAMMA,          buf);
#ifndef BASIC_INFO_DLG
        _stprintf(buf,_T("%10d"),     dpxInfo->mCineonImage.GetImageXOffset());
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_X_OFFSET,       buf);
        _stprintf(buf,_T("%10d"),     dpxInfo->mCineonImage.GetImageYOffset());
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_Y_OFFSET,       buf);
        _stprintf(buf,_T("%.2g"),     dpxInfo->mCineonImage.GetDeviceXPitch());
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_DEVICE_X_PITCH, buf);
        _stprintf(buf,_T("%.2g"),     dpxInfo->mCineonImage.GetDeviceYPitch());
        SetDlgItemText(hWnd, IDC_INFO_DLG_IMAGE_DEVICE_Y_PITCH, buf);
        _stprintf(buf,_T("%10u"),     dpxInfo->mCineonImage.GetFramePosition());
        SetDlgItemText(hWnd, IDC_INFO_DLG_FRAME_POSITION,       buf);
        _stprintf(buf,_T("%3u"),      dpxInfo->mCineonImage.GetFilmMFGID());
        SetDlgItemText(hWnd, IDC_INFO_DLG_FILM_MFG_ID,          buf);
        _stprintf(buf,_T("%3u"),      dpxInfo->mCineonImage.GetFilmType());
        SetDlgItemText(hWnd, IDC_INFO_DLG_FILM_TYPE,            buf);
        _stprintf(buf,_T("%3u"),      dpxInfo->mCineonImage.GetFilmOffset());
        SetDlgItemText(hWnd, IDC_INFO_DLG_FILM_OFFSET,          buf);
        _stprintf(buf,_T("%.2g"),     dpxInfo->mCineonImage.GetFrameRate());
...
CIN_ControlDialog::SetDensities()
{
 ...
 // should be mSoftClip 0 < 50
    R32     mSoftclip   = 1.0f;
    U32     LUTsize     = (U32) pow(2.0, bitsPerPixel);
    R32     densityStep  = CALIBRATED_DENSITY / (float) LUTsize;

    if (mDensityLUT) free(mDensityLUT);

	mDensityLUT = (U16*) calloc(LUTsize, sizeof(U16));
    if (mDensityLUT == NULL) {
        return FALSE;
    }
    ...
    double Gain = SIXTEEN_BIT_MAX / (1- pow(10.0, (refBlack - refWhite) * densityStep/0.6 ));
    ...
    for (U32 cnt = 0; cnt < LUTsize; cnt++) 
    {
        ...
        dDensityLUT = pow(10.0,(cnt-refWhite)*densityStep/0.6)*Gain - Offset;    
...
BitmapIO_CIN::GetImageInfo(BitmapInfo* bmi)
{
    ...
    MaxSDK::Util::BufferedFile file(bmi->Name(), _T("rb"), false);
    ...
    if (mCineonImage.ReadHeader(stream) == FALSE)
...
{
	MaxSDK::Util::BufferedFile file(bmi->Name(), _T("rb"), true);
	FILE* stream = file.getFile();
	if (stream == nullptr)
	{
 ...
 // bmi->SetFlags(MAP_NOFLAGS);
	bmi->SetType(BMM_TRUE_64);

    BitmapStorage* bms = BMMCreateStorage(bm->Manager(), BMM_TRUE_64);
    if (!bms) {
        *status = ProcessImageIOError(bmi, GetResIDCaption(IDS_CIN_Internal_Error));
        return NULL;
    }
...
BOOL
BitmapIO_CIN::LoadConfigure(void* ptr, DWORD piDataSize)
{
    if (!ptr) {
        assert(FALSE);
        return FALSE;
...
BitmapIO_CIN::Write(int frameNum)
{
	...
	// cin writes small blocks, where block size is image width * 4 bytes. Code does
	// fseek/fwrite/fseek/fwrite. Looks like fseek forces write to disk. Looks like could optimize code to keep
	// track of current write location and only seek if not there. For most writes, that is the case. Looks like low
	// risk tweak. But who even uses this format?
	// Use 16KB buffer size
	MaxSDK::Util::BufferedFile file(filename, _T("wb"), true, false, 1 << 14);
	FILE* stream = file.getFile();
	if (stream == nullptr)
	...
	TSTR sFileName = filename;
	MaxString mFileName;
	sFileName.ToMaxString(mFileName);
	Interface14 *iface = GetCOREInterface14();
	LANGID langID = iface->LanguageToUseForFileIO();
	UINT codePage = iface->CodePageForLanguage(langID);

    cineonImage.SetImageFileName(mFileName.ToCP(codePage));
	...
	if (findhnd != INVALID_HANDLE_VALUE) {
		...
		if (!SystemTimeToTzSpecificLocalTime(NULL, &time, &local))
		    local = time;
		_stprintf(cDate,_T("%d:%02d:%02d"), local.wYear, local.wMonth, local.wDay);
        _stprintf(cTime,_T("%02d:%02d:%02d"), local.wHour, local.wMinute, local.wSecond);
	}
	TSTR sDate = cDate;
	TSTR sTime = cTime;
	MaxString mDate, mTime;
	sDate.ToMaxString(mDate);
	sTime.ToMaxString(mTime);
    cineonImage.SetImageCreationDate(mDate.ToCP(codePage));
    cineonImage.SetImageCreationTime(mTime.ToCP(codePage));
	...
	// Hardware lock ID is no longer used. Always assign it as 0.
	const int lockID = 0;
    sprintf(buf, "%d", lockID);
 ...
 // FIXME
    ...
    for (unsigned int lineCnt = 0; lineCnt < height; lineCnt++) {
		...
		if (!cineonImage.WriteScanLine(stream, (unsigned short*)scanLine, lineCnt, width))
		{
            if (scanLine) { free(scanLine); scanLine = NULL; }
    ...
    if (cineonImage.WriteHeader(stream) == FALSE)
        return ProcessImageIOError(&bi, GetResIDCaption(IDS_CIN_File_IO_Error));
...
TSTR
BitmapIO_CIN::GetINIFilename()
{
    TSTR filename(TheManager->GetDir(APP_PLUGCFG_DIR));
    ...
    filename.append(CIN_INI_FILENAME);
...
{
    TSTR filename(GetINIFilename());
    TCHAR buffer[256];

    MaxSDK::Util::GetPrivateProfileString(CIN_INI_SECTION, CIN_INI_VERSION, buffer, buffer, _countof(buffer), filename);
    mUserData.version = (DWORD) _ttoi(buffer);
    ...
    {
        MaxSDK::Util::GetPrivateProfileString(CIN_INI_SECTION, CIN_INI_REF_WHITE, buffer, buffer, _countof(buffer), filename);
        mUserData.mRefWhite = (DWORD) _ttoi(buffer);
        MaxSDK::Util::GetPrivateProfileString(CIN_INI_SECTION, CIN_INI_REF_BLACK, buffer, buffer, _countof(buffer), filename);
        mUserData.mRefBlack = (DWORD) _ttoi(buffer);
    }
}
...
BitmapIO_CIN::WriteINI()
{
    ...
    wsprintf(buffer,_T("%d"), CIN_USERDATA_VERSION);
    MaxSDK::Util::WritePrivateProfileString(CIN_INI_SECTION, CIN_INI_VERSION, buffer, filename);

    wsprintf(buffer,_T("%d"), mUserData.mRefWhite);
    MaxSDK::Util::WritePrivateProfileString(CIN_INI_SECTION, CIN_INI_REF_WHITE, buffer, filename);
    wsprintf(buffer,_T("%d"), mUserData.mRefBlack);
    MaxSDK::Util::WritePrivateProfileString(CIN_INI_SECTION, CIN_INI_REF_BLACK, buffer, filename);
}

BaseInterface *BitmapIO_CIN::GetInterface(Interface_ID id)
{	
...
bool BitmapIO_CIN::UseLinearStorage(bool save)
{	
	// Logarithmic storage is treated as wanting linear data I/O - gamma on top of log makes little sense
	return true;
}
