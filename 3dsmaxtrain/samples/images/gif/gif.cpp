...
static constexpr char gifsig[] = "GIF87a";
static constexpr const char* gifsigs[] = { "87a", "89a", NULL };
static constexpr long code_mask[13] = {
     0,
     0x0001, 0x0003,
     0x0007, 0x000F,
     0x001F, 0x003F,
...
class GIFClassDesc:public ClassDesc {
     public:
		...
		const TCHAR*  NonLocalizedClassName() { return _T("GIF"); }
...
#ifdef DBG_GIF
		DebugPrint(_T("<BUFFER OVERRUN, y:%d (%d) linelen:%d (%d)>\n"), y, image.h, linelen, image.w);
#endif // DBG_GIF
...
BitmapIO_GIF::ReadHeader() {
...
#ifdef DBG_GIF
	DebugPrint(_T("Header read OK, W:%d H:%d COLPIX:%X BGCOLOR:%d\n"), header.w,header.h,
	...
	for (;;)	/* skip over extension blocks and other junk til get ',' */
	{
		if ((c = GIFGetByte()) == READ_ERROR) 
			goto error;
#ifdef DBG_GIF
			DebugPrint(_T("Got block %X(%d)\n"), c, c);
#endif // DBG_GIF
		if (c == ',') {		
#ifdef DBG_GIF
			DebugPrint(_T("<IMAGE FOUND>\n"));
#endif // DBG_GIF
			break;
			}
		if (c == ';') {	/* semi-colon is end of piccie */
#ifdef DBG_GIF
			DebugPrint(_T("<END OF PIC -- ERROR>\n"));
#endif // DBG_GIF
			goto error;
			}
		...
		if (c == '!')	/* extension block */
		{
			if ((c = GIFGetByte()) == READ_ERROR)	/* skip extension type */
				goto error;
#ifdef DBG_GIF
			DebugPrint(_T("<Found extension block type %X(%d)>\n"), c, c);
#endif // DBG_GIF
			if ( c == 0xf9)
			{
#ifdef DBG_GIF
				DebugPrint(_T("<FOUND GRAPHIC CONTROL BLOCK>\n"));
#endif // DBG_GIF
				...
				if(!GIFREAD(&GCB,sizeof(GIFGraphicControlBlock)))
					goto error;
#ifdef DBG_GIF
				DebugPrint(_T("GCB flags:%X Delay:%d Xparent:%d\n"),GCB.flags,GCB.delay,GCB.xparent);
#endif // DBG_GIF
				for (;;)	 
				{
					if ((c = GIFGetByte()) == READ_ERROR) goto error;
...
#ifdef DBG_GIF
						DebugPrint(_T("<END OF EXT BLOCK>\n"));
#endif // DBG_GIF
						break;
						}
#ifdef DBG_GIF
					DebugPrint(_T("<SKIPPING EXT BLOCK, %d BYTES>\n"), c);
#endif // DBG_GIF
					for(uchar i=0;i<c;i++)
					{
...
#ifdef DBG_GIF
				if(c == 0xfe)
					DebugPrint(_T("<SKIPPING COMMENT>\n"));
				else
				if(c == 0xff)
					DebugPrint(_T("<SKIPPING APPLICATION EXTENSION>\n"));
				else
				if(c == 0x01)
					DebugPrint(_T("<SKIPPING PLAIN TEXT>\n"));
				else
					DebugPrint(_T("<SKIPPING UNKNOWN CONTROL BLOCK>\n"));
#endif // DBG_GIF
				for (;;)	 
				{
					if ((c = GIFGetByte()) == READ_ERROR) goto error;
...
#ifdef DBG_GIF
			DebugPrint(_T("<READING IMAGE INFO>\n"));
#endif // DBG_GIF
...
#ifdef DBG_GIF
		DebugPrint(_T("<IMAGE HAS ALPHA, USING TRUECOLOR>\n"));
#endif // DBG_GIF
...
BMMRES BitmapIO_GIF::Write(int frame) {
	...
	if (openMode != BMM_OPEN_W)
	 ...
	 MaxSDK::VariableGuard<FILE*> guard(outStream, stream);
