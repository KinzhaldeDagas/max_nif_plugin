...
class RLAClassDesc:public ClassDesc {
     public:
		...
		const TCHAR*    NonLocalizedClassName()            { return _T("RLA"); }
...
class RPFClassDesc:public ClassDesc {
     public:
		...
		const TCHAR*    NonLocalizedClassName()            { return _T("RPF"); }
		SClass_ID       SuperClassID ( )                   { return BMM_IO_CLASS_ID;  }
...
INT_PTR BitmapIO_RLA::ImageInfoDlg(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam) {
	switch (message) {
		...
		case WM_INITDIALOG: {
			...
			// Text info
			SetDlgItemText(hWnd, IDC_INFO_USER, TSTR::FromCP(codePage, hdr.user).data());
			SetDlgItemText(hWnd, IDC_INFO_DESC, TSTR::FromCP(codePage, hdr.desc).data());
			SetDlgItemText(hWnd, IDC_INFO_PROGRAM, TSTR::FromCP(codePage, hdr.program).data());
			SetDlgItemText(hWnd, IDC_INFO_CDATE, TSTR::FromCP(codePage, hdr.date).data());
...
BaseInterface *BitmapIO_RLA::GetInterface(Interface_ID id)
{	
...
bool BitmapIO_RLA::UseLinearStorage(bool save)
{	
	if (save)
	{
		return (UserData.rgb == 2);
	}
	else
	{
		// For load, rely on the type of the loaded bitmap
		return bi.Type() == BMM_FLOAT_RGBA_32;
	}
}
