...
static BOOL InitEpsDll()
{
	...
	if (! resourcesLoaded) {
		TCHAR dirName[MAX_PATH_LENGTH];    // Where this DLL resides 
		TCHAR dllName[MAX_PATH_LENGTH];    // Full path name to resource DLL
		TCHAR *chPtr = 0;

		GetModuleFileName (hInstance, dirName, MAX_PATH_LENGTH);

		// Strip off the file name
		chPtr = dirName + _tcslen (dirName);
		while (*(--chPtr) != _T('\\'))
			;
		*(chPtr+1) = 0;

		// Add in "epsres.dll"
		_tcscpy (dllName, dirName);
		_tcscat (dllName, _T("epsres.dll"));
		...
		if (hResource) {
			...
			triedToLoad = TRUE;
			MaxSDK::MaxMessageBox(NULL, _T("EPS Plugin failed to load due to missing resource file EPSRES.DLL"), _T("EPS"), MB_ICONINFORMATION);
			return FALSE;
		}
	}
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL,ULONG fdwReason,LPVOID lpvReserved) 
{
	...
	case DLL_PROCESS_DETACH:
		...
		if( hResource )
		{
			...
			DebugOutputString(_T("eps/eps.cpp: DllMain(DLL_PROCESS_DETACH) called before LibShutdown()\n"));
		}
#endif
		break;
	}
...
const TCHAR* EPSClassDesc::NonLocalizedClassName()
{
	return _T("EPS Image");
}
...
BOOL 
	BitmapIO_EPS::LoadConfigure (void *ptr, DWORD piDataSize)
{
...
{
	//MaxSDK::MaxMessageBox(NULL, _T(".EPS ImageInfo called"), _T("About EPS.BMI"), MB_OK);
	...
	const TCHAR *filename)
{
	...
	if (filename) {
		...
		MaxSDK::MaxMessageBox(hWnd, _T("EPS GetImageInfoDlg called with no file name"), _T("EPS Plugin"), MB_ICONINFORMATION);
	...
	LPARAM lParam) 
{
	...
	case WM_INITDIALOG: {
		...
		if (includesPreview)
			LoadString (hResource, IDS_YES, yesOrNo, _countof(yesOrNo));
		else
			LoadString (hResource, IDS_NO, yesOrNo, _countof(yesOrNo));
	...
	BitmapIO_EPS::Save(const TCHAR *name, Bitmap *bitmap) 
{
	...
	mOutStream = MaxSDK::Util::BufferedFile::OpenFile(name, _T("wb"), true);
	if (mOutStream == NULL)
