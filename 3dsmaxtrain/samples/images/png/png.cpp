...
static float GetPNGGamma(png_fixed_point gamma)
{
	// PNG gammas are stored inverted (i.e. 0.4545 for a normal gamma 2.2 file)
...
public:
	...
	const TCHAR* NonLocalizedClassName()	{ return _T("Portable Network Graphics"); }
...
static void
throw_func(png_structp png, png_const_charp data)
{
...
error_func(png_structp png, png_const_charp data)
{
	...
	MaxSDK::MaxMessageBox(NULL, tsMsgString, plugin_str, MB_OK);
...
// returns true if a dlg is displayed, false if the silent mode was on and thus dlg was not displayed
bool BitmapIO_PNG::DisplayError( BitmapManager* manager, const TCHAR* msg)
{
	LoadString(hResource, IDS_PNG_PLUGIN, plugin_str, MAX_STRING_LENGTH);
	LoadString(hResource, IDS_PNG_ERROR, error_str, MAX_STRING_LENGTH);
	...
	if (manager->SilentMode())
	{
		DebugPrint(_T("%s, %s\n"), plugin_str, error_str);
		return false;
	}
...
BitmapIO_PNG::ReadPNGFile(BitmapInfo *fbi, BitmapManager *manager)
{
	...
	// MAXX-65958: BitmapStorage::Allocate() function we call later in this
	// function registers the new storage with the bitmap manager making this
	// function NOT reentrant:  
	// If LoadBitmap() or LoadInto() is re-entered for some reason, bitmap
	// manager will think that it already has a storage for the filename and
	// will return that registered -but not ready-yet- storage. If any exception/error
	// happens in this function after this point, the storage will be deleted,
	// thus the bitmap that got the storage pointer during the reentrant call
	// will have a dangling pointer.
	// 
	// To eliminate this problem we're working on a temp BitmapInfo until we're
	// sure the function returns successfully. Also we're marking both the
	// bitmap info and the storage with the MAP_LOADING flag which is checked by
	// the BitmapManager during storage match and the "loading" storages will be
	// ignored. /coz 2022-03-03 (updated 2022-08-19)

	BitmapInfo tempInfo(*fbi);

	const TCHAR* fileName = fbi->Name();
	FILE* mInputStream = _tfopen(fileName, _T("rb"));
	if (mInputStream == NULL)
		return NULL;

	// grab the first 8 bytes for testing
	size_t nRead = fread(magic_numbers, 1, 8, mInputStream);
	fclose(mInputStream);
	...
	mInputStream = MaxSDK::Util::BufferedFile::OpenFile(fileName, _T("rb"), true);
	if (mInputStream == NULL)
		return NULL;

	png_bytep*	row_pointers = nullptr;

	png = png_create_read_struct(PNG_VERSION, (void *)this, throw_func, warning_func); //coz
	// MAXX-46348
	/*
	...
	*/
	...
	try
	{
		...
		tempInfo.SetWidth((WORD)info->width);
		tempInfo.SetHeight((WORD)info->height);
		...
		if (info->valid & PNG_INFO_gAMA && info->colorspace.gamma > 0)
		{
			...
			tempInfo.SetGamma(GetPNGGamma(info->colorspace.gamma));
		}

		if (info->valid & PNG_INFO_pHYs)
			tempInfo.SetAspect((float)info->x_pixels_per_unit / (float)info->y_pixels_per_unit);
		else
			tempInfo.SetAspect(1.0f);
		tempInfo.SetFlags(0); // uhhhm. This does nothing!? /coz

		/* expand grayscale images to the full 8 bits */
		/* expand images with transparency to full alpha channels */
		/* I'm going to ignore lineart and just expand it to 8 bits */
		if ((info->color_type == PNG_COLOR_TYPE_PALETTE && info->bit_depth < 8) ||
				(info->color_type == PNG_COLOR_TYPE_GRAY && info->bit_depth < 8) || (info->valid & PNG_INFO_tRNS))
			png_set_expand(png);
		...
		storage->SetFlags(MAP_LOADING);

		if (storage->Allocate(&tempInfo, manager, BMM_OPEN_R) == 0)
		{
			delete storage;
			storage = nullptr;
			fclose(mInputStream);
	...
	{
		// show the error dlg if we are not silenced
		auto msg = UTF8Str(e.what()).ToMSTR();
		DisplayError(manager, msg); // coz: why don't we use ProcessImageIOError?
	...
	// Now that we know the storage is valid, put things in their final,
	// non-temp form.
	if(storage)
	{
		fbi->CopyImageInfo(&tempInfo);
		storage->bi.CopyImageInfo(fbi);
		storage->ClearFlags(MAP_LOADING);
	}
...
BitmapIO_PNG::GetImageInfo(BitmapInfo *fbi) {
	...
	FILE* mInputStream = _tfopen(fbi->Name(), _T("rb"));
	if (mInputStream == NULL)
	...
	*/
	...
	catch (std::exception const& e)
	{
		// show the error dlg if we are not silenced
		auto msg = UTF8Str(e.what()).ToMSTR();
		DisplayError(TheManager, msg); //coz: why don't we use ProcessImageIOError ?
...
{
	//re-entry guard
	DbgAssert(!bLoadReentered);
	MaxSDK::VariableGuard<bool> guard(bLoadReentered, true);
...
BitmapIO_PNG::Save(const TCHAR *filename, Bitmap *map)
{
	...
	FILE* mOutStream = MaxSDK::Util::BufferedFile::OpenFile(filename, _T("wb"), true);
	if (mOutStream == NULL)
	...
	png = png_create_write_struct(PNG_VERSION, (void *)this, throw_func, warning_func); //coz
	// MAXX-46348
	/*
	if (setjmp(png->jmp_buf_local)) {
		if (info)
