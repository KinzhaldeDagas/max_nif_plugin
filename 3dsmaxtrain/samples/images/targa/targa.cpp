...
class TGAClassDesc:public ClassDesc2 {
	public:
		...
		const TCHAR*  NonLocalizedClassName() { return _T("Targa"); }
...
static void InitUserData(TGAUSERDATA *ptr) {
	// Init user data
	memset(ptr,0,sizeof(TGAUSERDATA));
...
public:
	...
	TSTR	GetJobName();
	void	SetJobName(const TSTR& jobname);
	TSTR	GetComment1();
	void	SetComment1(const TSTR& comment);
	TSTR	GetComment2();
	void	SetComment2(const TSTR& comment);
	TSTR	GetComment3();
	void	SetComment3(const TSTR& comment);
	TSTR	GetComment4();
	void	SetComment4(const TSTR& comment);
...
TSTR BitmapIO_Tga_Imp::GetAuthor()
	...
	if (p) {
		p->ReadCfg();
		author = p->UserData.getAuthor();
		delete p;
		}
	return author;
	}

void BitmapIO_Tga_Imp::SetAuthor(const TSTR& author)
	{
	BitmapIO_TGA* p = new BitmapIO_TGA;
	if (p) {
		p->ReadCfg();
		p->UserData.setAuthor(author.data());
		p->WriteCfg();
		delete p;
		}
	}
...
TSTR BitmapIO_Tga_Imp::GetJobName()
	...
	if (p) {
		p->ReadCfg();
		jobname = p->UserData.getJobName();
		delete p;
		}
	return jobname;
	}

void BitmapIO_Tga_Imp::SetJobName(const TSTR& jobname)
	{
	BitmapIO_TGA* p = new BitmapIO_TGA;
	if (p) {
		p->ReadCfg();
		p->UserData.setJobName(jobname.data());
		p->WriteCfg();
		delete p;
		}
	}
...
TSTR BitmapIO_Tga_Imp::GetComment1()
	...
	if (p) {
		p->ReadCfg();
		comment = p->UserData.getComments1();
		delete p;
		}
	return comment;
	}

void BitmapIO_Tga_Imp::SetComment1(const TSTR& comment)
	{
	BitmapIO_TGA* p = new BitmapIO_TGA;
	if (p) {
		p->ReadCfg();
		p->UserData.setComments1(comment.data());
		p->WriteCfg();
		delete p;
		}
	}
...
TSTR BitmapIO_Tga_Imp::GetComment2()
	...
	if (p) {
		p->ReadCfg();
		comment = p->UserData.getComments2();
		delete p;
		}
	return comment;
	}

void BitmapIO_Tga_Imp::SetComment2(const TSTR& comment)
	{
	BitmapIO_TGA* p = new BitmapIO_TGA;
	if (p) {
		p->ReadCfg();
		p->UserData.setComments2(comment.data());
		p->WriteCfg();
		delete p;
		}
	}
...
TSTR BitmapIO_Tga_Imp::GetComment3()
	...
	if (p) {
		p->ReadCfg();
		comment = p->UserData.getComments3();
		delete p;
		}
	return comment;
	}

void BitmapIO_Tga_Imp::SetComment3(const TSTR& comment)
	{
	BitmapIO_TGA* p = new BitmapIO_TGA;
	if (p) {
		p->ReadCfg();
		p->UserData.setComments3(comment.data());
		p->WriteCfg();
		delete p;
		}
	}
...
TSTR BitmapIO_Tga_Imp::GetComment4()
	...
	if (p) {
		p->ReadCfg();
		comment = p->UserData.getComments4();
		delete p;
		}
	return comment;
	}

void BitmapIO_Tga_Imp::SetComment4(const TSTR& comment)
	{
	BitmapIO_TGA* p = new BitmapIO_TGA;
	if (p) {
		p->ReadCfg();
		p->UserData.setComments4(comment.data());
		p->WriteCfg();
		delete p;
		}
	}
...
BitmapStorage *BitmapIO_TGA::ReadTGAFile(BitmapInfo *fbi, BitmapManager *manager, BMMRES *status) {
	...
	MaxSDK::Util::BufferedFile file(fbi->Name(), _T("rb"), true);
	...
	MaxSDK::VariableGuard<FILE*> guard(inStream, stream);
		...
		switch(hdr.pixsize) {
			...
			switch(hdr.imgtype) {
				...
				case 9:
					...
					s = Load8BitPalTGA(fbi, inStream, manager);
					break;
				case 3:
				case 11:
					fbi->SetType(BMM_GRAY_8);   //-- Must check for grayscale
					s = Load8BitGrayTGA(fbi, inStream, manager);
					break;
				}
			break;
			...
			s = Load16BitTGA(fbi, inStream, manager);
			break;
			...
			s = Load24BitTGA(fbi, inStream, manager);
			break;

			case 32:  
			fbi->SetType(BMM_TRUE_32);
				s = Load32BitTGA(fbi, inStream, manager);
			break;
...
//-----------------------------------------------------------------------------
// #> BitmapIO_TGA::ValidatePiData()

BOOL BitmapIO_TGA::ValidatePiData( BitmapInfo *bi, ILoad *pILoad )
{
	DbgAssert(bi);
	if (bi == NULL)
		return FALSE;
	...
	if (piDataSize == sizeof(TGAUSERDATA_old))
	{
	...
	WStr author = UserData.authorW;
	strncpy(UserData.authorA, author.ToCP(codePage), _countof(UserData.authorA)-1);
	WStr jobname = UserData.jobnameW;
	strncpy(UserData.jobnameA, jobname.ToCP(codePage), _countof(UserData.jobnameA)-1);
	WStr comments1 = UserData.comments1W;
	strncpy(UserData.comments1A, comments1.ToCP(codePage), _countof(UserData.comments1A)-1);
	WStr comments2 = UserData.comments2W;
	strncpy(UserData.comments2A, comments2.ToCP(codePage), _countof(UserData.comments2A)-1);
	WStr comments3 = UserData.comments3W;
	strncpy(UserData.comments3A, comments3.ToCP(codePage), _countof(UserData.comments3A)-1);
	WStr comments4 = UserData.comments4W;
	strncpy(UserData.comments4A, comments4.ToCP(codePage), _countof(UserData.comments4A)-1);
...
BOOL BitmapIO_TGA::LoadConfigure ( void *ptr, DWORD piDataSize ) {
	if (ptr == NULL)
		return FALSE;
	...
	if (buf->version == TGAVERSION) {
		if (piDataSize == sizeof(TGAUSERDATA_old))
		{
			memcpy((void*)&UserData,ptr,sizeof(TGAUSERDATA_old));
			UINT codePage;
			if (iload)
			...
			WStr author = WStr::FromCP(codePage, UserData.authorA, _countof(UserData.authorA));
			wcsncpy(UserData.authorW, author.data(), _countof(UserData.authorW)-1);
			WStr jobname = WStr::FromCP(codePage, UserData.jobnameA, _countof(UserData.jobnameA));
			wcsncpy(UserData.jobnameW, jobname.data(), _countof(UserData.jobnameW)-1);
			WStr comments1 = WStr::FromCP(codePage, UserData.comments1A, _countof(UserData.comments1A));
			wcsncpy(UserData.comments1W, comments1.data(), _countof(UserData.comments1W)-1);
			WStr comments2 = WStr::FromCP(codePage, UserData.comments2A, _countof(UserData.comments2A));
			wcsncpy(UserData.comments2W, comments2.data(), _countof(UserData.comments2W)-1);
			WStr comments3 = WStr::FromCP(codePage, UserData.comments3A, _countof(UserData.comments3A));
			wcsncpy(UserData.comments3W, comments3.data(), _countof(UserData.comments3W)-1);
			WStr comments4 = WStr::FromCP(codePage, UserData.comments4A, _countof(UserData.comments4A));
			wcsncpy(UserData.comments4W, comments4.data(), _countof(UserData.comments4W)-1);
		}
		else
		{
			DbgAssert (piDataSize == sizeof(TGAUSERDATA));
...
INT_PTR BitmapIO_TGA::Control(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam) {
	...
	switch (message) {
		...
		case WM_INITDIALOG: {
			...
			SendMessage(GetDlgItem(hWnd,IDC_AUTHORNAME), WM_SETTEXT, 0, (LPARAM)UserData.getAuthor());
			SendMessage(GetDlgItem(hWnd,IDC_JOBNAME), WM_SETTEXT, 0, (LPARAM)UserData.getJobName());
			SendMessage(GetDlgItem(hWnd,IDC_COMMENTS1), WM_SETTEXT, 0, (LPARAM)UserData.getComments1());
			SendMessage(GetDlgItem(hWnd,IDC_COMMENTS2), WM_SETTEXT, 0, (LPARAM)UserData.getComments2());
			SendMessage(GetDlgItem(hWnd,IDC_COMMENTS3), WM_SETTEXT, 0, (LPARAM)UserData.getComments3());
			SendMessage(GetDlgItem(hWnd,IDC_COMMENTS4), WM_SETTEXT, 0, (LPARAM)UserData.getComments4());
...
BMMRES BitmapIO_TGA::GetImageInfo ( BitmapInfo *fbi ) {
	...
	MaxSDK::Util::BufferedFile file(fbi->Name(), _T("rb"), false);
	FILE* stream = file.getFile();
	if (stream == nullptr)
...
INT_PTR BitmapIO_TGA::ImageInfoDlg(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam) {
	switch (message) {
		...
		case WM_INITDIALOG: {
			...
			TCHAR buf[64];
			_stprintf(buf,_T("%dx%d"),hdr.width,hdr.height);
			...
			SetDlgItemText(hWnd, IDC_INFO_AUTHOR, TSTR::FromCP(codePage, extra.authorname).data());

			//-- Handle Job Name -------------------------

			SetDlgItemText(hWnd, IDC_INFO_JOB, TSTR::FromCP(codePage, extra.jobname).data());

			//-- Handle Comments -------------------------

			SetDlgItemText(hWnd, IDC_INFO_COMMENT1, TSTR::FromCP(codePage, &extra.comments[0] ).data());
			SetDlgItemText(hWnd, IDC_INFO_COMMENT2, TSTR::FromCP(codePage, &extra.comments[81] ).data());
			SetDlgItemText(hWnd, IDC_INFO_COMMENT3, TSTR::FromCP(codePage, &extra.comments[162] ).data());
			SetDlgItemText(hWnd, IDC_INFO_COMMENT4, TSTR::FromCP(codePage, &extra.comments[243] ).data());
...
BMMRES BitmapIO_TGA::Write(int frame) {
	...
	if (frame == BMM_SINGLEFRAME) {
		_tcsncpy_s(filename, _countof(filename), bi.Name(), _TRUNCATE);
	} else {
	...
	MaxSDK::Util::BufferedFile file(filename, _T("wb"), true);
	FILE* stream = file.getFile();
	if (stream == nullptr)
	...
	switch(UserData.writeType) {
		case TGA_16_BITS:
		result = Save16BitTGA(stream);
			break;
		case TGA_24_BITS:
			result = Save24BitTGA(stream);
			break;
		case TGA_32_BITS:
			result = Save32BitTGA(stream);
			break;
		}
	...
	if(hasAlpha && UserData.alphaSplit) {
		...
		BMMSplitFilename(bi.Name(), path,_countof(path), file, _countof(file), ext, _countof(ext));
		TSTR workname;
		workname.printf(_T("%sA_%s%s"), path, file, ext);
		if (frame == BMM_SINGLEFRAME) {
			_tcsncpy_s(filename, _countof(filename), workname, _TRUNCATE);
		} else {
		...
		MaxSDK::Util::BufferedFile alphaFile(filename, _T("wb"), true);
		FILE* alphaFileStream = alphaFile.getFile();
		if (alphaFileStream == nullptr)
			return (ProcessImageIOError(&bi));
		...
		switch (SaveAlpha(alphaFileStream))
		{
			case TGA_SAVE_OK:
				return BMMRES_SUCCESS;
			case TGA_SAVE_UNSUPPORTED_TYPE:
...
int BitmapIO_TGA::WriteTargaExtension(FILE *stream, BOOL alphaUsed, BOOL preMultAlpha) {
	...
	/* Set comments */
	memset(ext.comments, 0, sizeof(ext.comments)); 

	TSTR string = UserData.getComments1();
	strncpy(&ext.comments[0],string.ToCP(codePage),80);
	string = UserData.getComments2();
	strncpy(&ext.comments[81],string.ToCP(codePage),80);
	string = UserData.getComments3();
	strncpy(&ext.comments[162],string.ToCP(codePage),80);
	string = UserData.getComments4();
	strncpy(&ext.comments[243],string.ToCP(codePage),80);
	ext.comments[_countof(ext.comments)-1] = 0;
	...
	string = UserData.getAuthor();
	strncpy(ext.authorname,string.ToCP(codePage),41);
	ext.authorname[_countof(ext.authorname)-1] = 0;

	string = UserData.getJobName();
	strncpy(ext.jobname,string.ToCP(codePage),41);
	ext.jobname[_countof(ext.jobname)-1] = 0;
...
void BitmapIO_TGA::GetCfgFilename( TCHAR *filename ) {
	_tcscpy(filename,TheManager->GetDir(APP_PLUGCFG_DIR).data());
...
BOOL BitmapIO_TGA::ReadCfg() {
	...
	MaxSDK::Util::BufferedFile file(filename, _T("rb"), false);
	FILE* stream = file.getFile();
	if (stream == nullptr)
		return (FALSE);
...
void BitmapIO_TGA::WriteCfg() {
	...
	if (SaveConfigure(buf)) {   
		MaxSDK::Util::BufferedFile file(filename, _T("wb"), false);
		FILE* stream = file.getFile();
		if (stream)
		{
