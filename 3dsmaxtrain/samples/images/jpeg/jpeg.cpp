...
// CA - 03/07/05 - 647115: This data structure prevented the jpeg loader from being re-entrant.
// I moved the structure to be a member of BitmapIO_JPEG
//JPEGDATA		data;
...
class JPEGClassDesc:public ClassDesc2 {
	...
	public:
		...
		const TCHAR*    NonLocalizedClassName()            { return _T("JPEG"); }
...
INT_PTR BitmapIO_JPEG::Control(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam) {
...
BOOL BitmapIO_JPEG::LoadConfigure ( void *ptr, DWORD piDataSize ) {
...
BMMRES BitmapIO_JPEG::GetImageInfo ( BitmapInfo *fbi ) {
	...
	MaxSDK::VariableGuard<FILE*> guard(data.input_file, stream);
	JpegInfo(&data);
...
BitmapStorage *BitmapIO_JPEG::Load(BitmapInfo *fbi, Bitmap *map, BMMRES *status) {
	...
	*status = GetImageInfo(fbi);
	...
	BitmapStorage* s = BMMCreateStorage(map->Manager(), stype);
	if(!s) {
		*status = ProcessImageIOError(fbi,BMMRES_INTERNALERROR);
		return NULL;
		}
...
//! [BitmapIO.ProcessImageIOError Example]
...
//   WORD *Buffer = (WORD *)map->GetStoragePtr(&mType);
	...
	MaxSDK::VariableGuard<FILE*> guard(data.output_file, stream);
