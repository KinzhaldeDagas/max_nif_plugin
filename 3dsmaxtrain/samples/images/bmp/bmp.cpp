...
class BMPClassDesc:public ClassDesc2 {
   ...
   public:
	  ...
	  const TCHAR*    NonLocalizedClassName()            { return _T("BMP"); }
...
BOOL BitmapIO_BMP::LoadConfigure ( void *ptr, DWORD piDataSize ) {
...
BMMRES BitmapIO_BMP::Write(int frame) 
{
   ...
   size_t res = fwrite(&hdr, 1, sizeof(BITMAPFILEHEADER), stream);
...
int  BitmapIO_BMP::ReadBimpHeader( FILE *stream ) {
   ...
   size_t res = fread(&hdr,1,sizeof(BITMAPFILEHEADER),stream);
...
void BitmapIO_BMP::WriteCfg() {
   ...
   if (stream)
   {
	   fwrite(&tbuf, 1, sizeof(BMPParams), stream);
   }
}
