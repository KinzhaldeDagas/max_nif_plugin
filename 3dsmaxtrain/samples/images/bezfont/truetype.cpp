...
class TrueTypeImport : public BezFontEX2 {
	private:
		HFONT hFont;
	...
	public:
		...
		BOOL BuildCharacterEx(UINT index, float height, BezierShape &shape, float &width, int FontShapeVersion, bool ggo_native);
		// Our methods
...
static BOOL FAR PASCAL EnumCallBack2(ENUMLOGFONT *lplf, NEWTEXTMETRIC *lpntm, DWORD fontType, LPVOID iptr) {
	...
	BezFontInfo info;
	info.family = (TCHAR*)lplf->elfLogFont.lfFaceName;
	info.name = (TCHAR*)lplf->elfFullName;
	info.style = (TCHAR*)lplf->elfStyle;
	info.type = BEZFONT_TRUETYPE;
	info.flags = BEZFONT_FLAG_USES_METRICS;
...
static BOOL FAR PASCAL EnumCallBack(ENUMLOGFONT *lplf, NEWTEXTMETRIC *lpntm, DWORD fontType, LPVOID iptr) {
	...
	// Also DEVICE_FONTTYPE fonts are potentially usable -- Use them if they're OpenType
	if((fontType & TRUETYPE_FONTTYPE) || ((fontType & DEVICE_FONTTYPE) && (lpntm->ntmFlags & (NTM_TT_OPENTYPE | NTM_PS_OPENTYPE)))) {
...
BOOL TrueTypeImport::BuildCharacter(UINT index, float height, BezierShape &shape, float &width, int fontShapeVersion)  {
	return BuildCharacterEx(index, height, shape, width, fontShapeVersion, false);
}
...
class TTClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("TrueType"); }
