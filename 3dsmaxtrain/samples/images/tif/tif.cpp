...
extern "C" TCHAR* GetString(int id)
{
	...
	if (hInst)
		if (LoadString(hInst, id, buf + bufPos, TIF_MAX_STRING_SIZE))
		{
			...
			if (bufPos >= TIF_MAX_STRINGS * TIF_MAX_STRING_SIZE)
				bufPos = 0;
			return ret;
		}
	return nullptr;
...
public:
	...
	{
		return new BitmapIO_TIF();
	}
	const TCHAR* ClassName() override
	{
		static MSTR class_name = MaxSDK::GetResourceStringAsMSTR(hInst, IDS_TIF);
	...
	Class_ID ClassID() override { return Class_ID(TIFCLASSID, 0); }
	const TCHAR* Category() override
	{
		static MSTR category = MaxSDK::GetResourceStringAsMSTR(hInst, IDS_BITMAP_IO);
...
{
	return (version == other.version && writeType == other.writeType && compressionType == other.compressionType &&
				   writeAlpha == other.writeAlpha && disableControl == other.disableControl &&
				   lightActive == other.lightActive && lumStonits == other.lumStonits && dpi == other.dpi)
			? 1
			: 0;
}
...
class TiffInterfaceImp : public TiffInterface
{
	...
	photometric GetPhotometric() override
	{
		return (photometric)tifDefaults.writeType;
	}
	void SetPhotometric(int newPhotometric) override
	{
		tifDefaults.writeType = newPhotometric;
		tifDefaults.saved = FALSE;
	}

	BOOL GetShowControl() override
	{
		return !tifDefaults.disableControl;
	}
	void SetShowControl(BOOL show = TRUE) override
	{
		tifDefaults.disableControl = !show;
		tifDefaults.saved = FALSE;
	}

	compression GetCompression() override
	{
		return (compression)tifDefaults.compressionType;
	}
	void SetCompression(int newCompress) override
	{
		tifDefaults.compressionType = newCompress;
		tifDefaults.saved = FALSE;
	}

	BOOL GetAlpha() override
	{
		return tifDefaults.writeAlpha;
	}
	void SetAlpha(BOOL onOff) override
	{
		tifDefaults.writeAlpha = onOff;
		tifDefaults.saved = FALSE;
	}
	...
	{
		return tifDefaults.dpi;
	}

	void SetDPI(double newDPI) override
	{
		if (newDPI >= 0.0f)
		{
...
static TiffInterfaceImp tifi(TIFF_OPTIONS_INTERFACE, _T("itifio"), IDS_TIF_DESC, &TIFDesc, 0, tif_get_photometric,
		_T("getType"), IDS_TIFIO_GETTYPE, TYPE_ENUM, tif_photo, 0, 0, tif_set_photometric, _T("setType"),
		IDS_TIFIO_SETTYPE, TYPE_VOID, 0, 1, _T("type"), 0, TYPE_ENUM, tif_photo,

		tif_get_show_control, _T("getShowControl"), IDS_TIFIO_GETSHOWCONTROL, TYPE_ENUM, tif_show_control, 0, 0,
		tif_set_show_control, _T("setShowControl"), IDS_TIFIO_SETSHOWCONTROL, TYPE_VOID, 0, 1, _T("showControl"), 0,
		TYPE_ENUM, tif_show_control,

		tif_get_compression, _T("getCompression"), IDS_TIFIO_GETCOMPRESSION, TYPE_ENUM, tif_compression, 0, 0,
		tif_set_compression, _T("setCompression"), IDS_TIFIO_SETCOMPRESSION, TYPE_VOID, 0, 1, _T("compression"), 0,
		TYPE_ENUM, tif_compression,

		tif_get_alpha, _T("getAlpha"), IDS_TIFIO_GETALPHA, TYPE_ENUM, tif_alpha, 0, 0, tif_set_alpha, _T("setAlpha"),
		IDS_TIFIO_SETALPHA, TYPE_VOID, 0, 1, _T("onOff"), 0, TYPE_ENUM, tif_alpha,

		tif_get_dpi, _T("getDPI"), IDS_TIFIO_GETDPI, TYPE_FLOAT, 0, 0, tif_set_dpi, _T("setDPI"), IDS_TIFIO_SETDPI,
		TYPE_VOID, 0, 1, _T("dpi"), 0, TYPE_FLOAT,

		enums, tif_photo, 5, _T("mono"), tif_write_mono, _T("color"), tif_write_color, _T("logL"), tif_write_logl,
		_T("logLUV"), tif_write_logluv, _T("color16"), tif_write_16bit, tif_show_control, 2, _T("true"), 1, _T("false"),
		0, tif_compression, 2, _T("none"), tif_compress_none, _T("packBits"), tif_compress_packbits, tif_alpha, 2,
		_T("true"), 1, _T("false"), 0,
...
{
	tif = nullptr;
	loadStorage = saveStorage = nullptr;
	inStream = nullptr;
...
//-----------------------------------------------------------------------------
// #> BitmapIO_TIF::Ext(int n)
const TCHAR* BitmapIO_TIF::Ext(int n)
{
	switch (n)
	{
	case 0:
...
BOOL BitmapIO_TIF::LoadConfigure(void* ptr, DWORD piDataSize)
{
	TIFUSERDATA* buf = (TIFUSERDATA*)ptr;
	if (buf->version == TIFVERSION)
	{
		if (!tifDefaults.saved && !UserData.saved)
		{
			// special case.  what the scripter says takes precedence
			return TRUE;
		}
		memcpy_s((void*)&UserData, sizeof(UserData), ptr, sizeof(TIFUSERDATA));
...
BOOL BitmapIO_TIF::SaveConfigure(void* ptr)
{
	if (ptr)
	{
		if (!tifDefaults.saved) // the global tifDefaults takes precedence over the local copy.
		{
			...
			memcpy_s(ptr, sizeof(TIFUSERDATA), (void*) & tifDefaults, sizeof(TIFUSERDATA));
			UserData = tifDefaults;
		}
		else
		{
			UserData.saved = TRUE;
			memcpy_s(ptr, sizeof(TIFUSERDATA), (void*) & UserData, sizeof(TIFUSERDATA));
		}
		return TRUE;
	}
	return FALSE;
...
{
	DialogBoxParam(hInst, MAKEINTRESOURCE(IDD_ABOUT), hWnd, (DLGPROC)AboutCtrlDlgProc, (LPARAM)0);
}
...
static ISpinnerControl* dpiSpinner = nullptr;

INT_PTR BitmapIO_TIF::Control(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG: {
		...
		CheckDlgButton(
				hWnd, IDC_COMPRESSION_PACKBITS, UserData.compressionType == tif_compress_packbits ? TRUE : FALSE);
		...
		else if (UserData.writeType == tif_write_16bit)
		{
			CheckDlgButton(hWnd, IDC_16BIT, TRUE);
			SendMessage(hWnd, WM_COMMAND, IDC_16BIT, 0);
		}
		...
		dpiSpinner = SetupFloatSpinner(hWnd, IDC_DPI_SPIN, IDC_DPI, 0.0f, 12000.0f, float(UserData.dpi), 10.0f);
		return 1;
	}

	case WM_DESTROY:
		if (dpiSpinner != nullptr)
		{
	...
	case WM_COMMAND:
		...
		case IDOK: {
			...
			else if (IsDlgButtonChecked(hWnd, IDC_16BIT))
				UserData.writeType = tif_write_16bit;
			...
			if (IsDlgButtonChecked(hWnd, IDC_SAVE_ALPHA) && (type == tif_write_color || type == tif_write_16bit))
			...
			tifDefaults = UserData;
			WriteCfg();
			// [dl | 07jan2005] Set the bitmap type to the appropriate type
			...
			case tif_write_mono:
				...
				break;
			case tif_write_color:
				// 8 bits per component rgba
				bi.SetType(BMM_TRUE_32);
				break;
			case tif_write_logl:
			case tif_write_logluv:
				bi.SetType(BMM_LOGLUV_32);
				break;
			case tif_write_16bit:
				// 16 bits per component rgba
				bi.SetType(BMM_TRUE_64);
				break;
			default:
				DbgAssert(false);
				bi.SetType(BMM_NO_TYPE);
				break;
			}
			EndDialog(hWnd, 1);
		}
...
{
	static BitmapIO_TIF* bm = nullptr;
...
//-----------------------------------------------------------------------------
// #> BitmapIO_TIF::ShowControl()

BOOL BitmapIO_TIF::ShowControl(HWND hWnd, DWORD flag)
{
	if (!UserData.disableControl)
		return (DialogBoxParam(hInst, MAKEINTRESOURCE(IDD_TIF_CONTROL), hWnd, (DLGPROC)ControlDlgProc, (LPARAM)this));
	else
	{
		return TRUE;
	}
...
{
	//-- Get File Header
	tif = TIFFOpenW(fbi->Name(), "r");

	// tif=TIFFReadHdr(inStream);
	if (!tif)
	{
		DebugPrint(_T("TIFFOpen failed\n"));
		return (ProcessImageIOError(fbi, BMMRES_BADFILEHEADER));
	}

	/* Fill in appropriate info slots */

	std::uint16_t tiff_samples_per_pixel = 0;
	TIFFGetField(tif, TIFFTAG_SAMPLESPERPIXEL, &tiff_samples_per_pixel);
	nsamp = tiff_samples_per_pixel;

	std::uint32_t tiff_imgwidth = 0;
	TIFFGetField(tif, TIFFTAG_IMAGEWIDTH, &tiff_imgwidth);
	fbi->SetWidth(tiff_imgwidth);
	
	std::uint32_t tiff_imgheight = 0;
	TIFFGetField(tif, TIFFTAG_IMAGELENGTH, &tiff_imgheight);
	fbi->SetHeight(tiff_imgheight);
	...
	std::uint16_t tiff_photometric = 0;
	TIFFGetField(tif, TIFFTAG_PHOTOMETRIC, &tiff_photometric);
	std::uint16_t tiff_bitspersample = 0;
	TIFFGetField(tif, TIFFTAG_BITSPERSAMPLE, &tiff_bitspersample);

	switch (tiff_photometric)
	{
	case PHOTOMETRIC_MINISWHITE:
	case PHOTOMETRIC_MINISBLACK:
		if (tiff_bitspersample == 1)
			type = BMM_LINE_ART;
	...
	case PHOTOMETRIC_RGB:
		if (tiff_bitspersample == 8)
			type = (tiff_bitspersample > 3) ? BMM_TRUE_32 : BMM_TRUE_24;
		else if (tiff_bitspersample == 16)
			type = (tiff_bitspersample > 3) ? BMM_TRUE_64 : BMM_TRUE_48;
		else if (tiff_bitspersample == 32)
			type = BMM_FLOAT_RGBA_32;
		else
			type = BMM_NO_TYPE;
		break;
	...
	if (type == BMM_NO_TYPE)
	{
		DebugPrint(_T("Unknown TIF type\n"));
...
BitmapStorage* BitmapIO_TIF::Load(BitmapInfo* fbi, Bitmap* map, BMMRES* status)
{
	...
	*status = BMMRES_SUCCESS;
	...
	if (fbi->GetPiData() != nullptr)
	{
		memcpy_s(fbi->GetPiData(), sizeof(UserData), &UserData, sizeof(UserData));
	}
...
BitmapStorage* BitmapIO_TIF::ReadTIFFile(BitmapInfo* fbi, BitmapManager* manager, BMMRES* status)
{
	...
	std::uint32_t tiff_imglength = 0;
	TIFFGetField(tif, TIFFTAG_IMAGELENGTH, &tiff_imglength);
	height = tiff_imglength;
	...
	std::uint16_t tiff_photometric = 0;
	TIFFGetField(tif, TIFFTAG_PHOTOMETRIC, &tiff_photometric);
	if (tiff_photometric == PHOTOMETRIC_LOGLUV || tiff_photometric == PHOTOMETRIC_LOGL)
	{
		...
		if (tiff_photometric == PHOTOMETRIC_LOGL)
	...
	std::uint16_t tiff_samplesperpixel = 0;
	TIFFGetField(tif, TIFFTAG_SAMPLESPERPIXEL, &tiff_samplesperpixel);
	nsamp = tiff_samplesperpixel;
	...
	PixelBuf8 lbuf(TIFFScanlineSize(tif));
	loadbuf = lbuf.Ptr();
	...
	case PHOTOMETRIC_RGB: 
	{
		std::uint16_t tiff_planarconfig = 0;
		TIFFGetField(tif, TIFFTAG_PLANARCONFIG, &tiff_planarconfig);

		if (tiff_planarconfig == PLANARCONFIG_SEPARATE)
		{
...
BitmapStorage* BitmapIO_TIF::TifReadLineArt(BitmapInfo* fbi, BitmapManager* manager)
{
	...
	uchar byte;
	int nbyte, bytesperrow, x, y, nmax;
	...
	if (tiff_bitspersample != 1)
	{
		DebugPrint(_T("Bad bits per sample\n"));
		goto bail_out;
	}

	nmax = (1 << tiff_bitspersample) - 1;

	PixelBuf line64(width);

	std::uint16_t tiff_orientation = 0;
	TIFFGetField(tif, TIFFTAG_ORIENTATION, &tiff_orientation);
	...
	for (y = 0; y < height; y++)
	{
		...
		for (nbyte = 0; nbyte < bytesperrow; nbyte++)
		{
			...
			int nsh;
			byte = loadbuf[nbyte];
			mask = nmax << (8 - tiff_bitspersample);
			for (nsh = 8 - tiff_bitspersample; nsh >= 0; nsh -= tiff_bitspersample, mask >>= tiff_bitspersample, ++l64)
			{
				c = (byte & mask) >> nsh;
				c = (c * 255) / nmax;
				...
				if (++x >= width)
				{
					goto next_line_please;
				}
			}
		}
		next_line_please:
	
		int max_pos_y = GetMaxBitmapYPosition(y, tiff_orientation, height);
		if (s->PutPixels(0, max_pos_y, width, line64.Ptr()) != 1)
			goto bail_out;
	}
	return s;
}
...
BitmapStorage* BitmapIO_TIF::TifReadGrayScale(BitmapInfo* fbi, BitmapManager* manager)
{
	...
	if (tiff_bitspersample > 8 || tiff_bitspersample == 3 || tiff_bitspersample == 5 || tiff_bitspersample == 7)
	{
		DebugPrint(_T("Bad bits per sample\n"));
		goto bail_out;
	}
	...
	for (y = 0; y < height; y++)
	{
		...
		for (nbyte = 0; nbyte < bytesperrow; nbyte++)
		{
			...
			mask = nmax << (8 - tiff_bitspersample);
			for (nsh = 8 - tiff_bitspersample; nsh >= 0; nsh -= tiff_bitspersample, mask >>= tiff_bitspersample)
			{
				c = (byte & mask) >> nsh;
				c = (c * 255) / nmax;
				if (tiff_photometric == PHOTOMETRIC_MINISWHITE)
				...
				accum += c;
				acc_ix++;
				if (acc_ix == tiff_samplesperpixel)
				{
...
BitmapStorage* BitmapIO_TIF::TifReadChunkyRGB(BitmapInfo* fbi, BitmapManager* manager)
{
	...
	if (tiff_bitspersample == 32)
		s = BMMCreateStorage(manager, BMM_FLOAT_RGBA_32);
	else if (tiff_bitspersample == 16)
		...
		for (y = 0; y < height; y++)
		{
			...
			for (x = 0; x < width; ++x, ++l)
			{
				...
				if (nsamp > 3)
				{
					...
					l->a = *scanptr;
					scanptr += static_cast<std::ptrdiff_t>(nsamp) - 3;
				}
				else
					l->a = 0;
			}
			...
			TIFFGetField(tif, TIFFTAG_ORIENTATION, &tiff_orientation);
			int max_pos_y = GetMaxBitmapYPosition(y, tiff_orientation, height);
			if (s->PutPixels(0, max_pos_y, width, line) != 1)
			{
				delete[] line;
				goto bail_out;
			}
...
BitmapStorage* BitmapIO_TIF::TifReadColPal(BitmapInfo* fbi, BitmapManager* manager)
{
	...
	for (y = 0; y < height; y++)
	{
		if (TIFFReadScanline(tif, loadbuf, y, 0) < 0)
			goto bail_out;
		if (tiff_bitspersample == 4 || tiff_bitspersample == 2 || tiff_bitspersample == 1)
		{
			shft = -1;
			pbuf = loadbuf;
			for (x = 0; x < width; x++)
			{
				...
				{
					byte = *pbuf++;
					shft = 8 - tiff_bitspersample;
				}
				line8[x] = (byte >> shft) & mask;
				shft -= tiff_bitspersample;
				if (shft < 0)
				{
					byte = *pbuf++;
					shft = 8 - tiff_bitspersample;
			...
			int max_pos_y = GetMaxBitmapYPosition(y, tiff_orientation, height);
			if (s->PutIndexPixels(0, max_pos_y, width, line8.Ptr()) != 1)
				goto bail_out;
		}
		else
		{
			int max_pos_y = GetMaxBitmapYPosition(y, tiff_orientation, height);
			if (s->PutIndexPixels(0, max_pos_y, width, loadbuf) != 1) // 8 bits -- just use load buffer
				goto bail_out;
		}
	}
	return s;
...
{
	// Create the bitmap storage
	BitmapStorage* s = BMMCreateStorage(manager, BMM_LOGLUV_32);
	if (s != nullptr)
	{
		bool success = true;

		// Allocate the bitmap storage
		if (s->Allocate(fbi, manager, BMM_OPEN_R))
		{
			...
			if (tiff_photometric == PHOTOMETRIC_LOGL)
			{
				for (int y = 0; y < height; y++)
				{
					if (TIFFReadScanline(tif, loadbuf, y, 0) >= 0)
					{
						...
						TIFFGetField(tif, TIFFTAG_ORIENTATION, &tiff_orientation);
						int max_pos_y = GetMaxBitmapYPosition(y, tiff_orientation, height);
						if (!s->Put16Gray(0, max_pos_y, width, (float*)loadbuf))
						{
							success = false;
							break;
						}
			...
			else if (tiff_photometric == PHOTOMETRIC_LOGLUV)
			{

				std::vector<BMM_Color_fl> bmmPixelBuffer(width);

				for (int y = 0; y < height; y++)
				{
					if (TIFFReadScanline(tif, loadbuf, y, 0) >= 0)
					{
						LogLUV32Pixel* input = (LogLUV32Pixel*)loadbuf;

						// Convert to BMM_Color_fl
						for (int x = 0; x < width; ++x)
						{
							BMM_Color_fl& bmmPixel = bmmPixelBuffer[x];
							LogLUV32Pixel& logluvPixel = input[x];
							logluvPixel.GetRGB(bmmPixel);
						}
						std::uint16_t tiff_orientation = 0;
						TIFFGetField(tif, TIFFTAG_ORIENTATION, &tiff_orientation);
						int max_pos_y = GetMaxBitmapYPosition(y, tiff_orientation, height);
						if (!s->PutPixels(0, max_pos_y, width, &(bmmPixelBuffer[0])))
						{
							success = false;
							break;
						}
		...
		// De-allocate the bitmap storage if there was an error
		if (!success)
		{
			delete s;
			s = nullptr;
...
BMMRES BitmapIO_TIF::Write(int frame)
{
	...
	{
		_tcscpy_s(fileName, _countof(fileName), bi.Name());
	}
	else
	{ // CA - 312242
	...
	if (tif)
	{
		...
		if (multiFrame)
		{
			...
			if (curDir == (tdir_t)(frame - 1)) // sequential.  This is fine.
...
void BitmapIO_TIF::MakeTiffhead()
{
	...
	float tiff_dpi = UserData.dpi;
	TIFFSetField(tif, TIFFTAG_XRESOLUTION, tiff_dpi);
	TIFFSetField(tif, TIFFTAG_YRESOLUTION, tiff_dpi);
	TIFFSetField(tif, TIFFTAG_RESOLUTIONUNIT, static_cast<std::uint16_t>(RESUNIT_INCH));
	...
	else if (UserData.writeType == tif_write_16bit)
	{
		if (write_alpha)
		{
			TIFFSetField(tif, TIFFTAG_BITSPERSAMPLE, 16L, 16L, 16L, 16L);
			TIFFSetField(tif, TIFFTAG_SAMPLESPERPIXEL, 4L);
		}
		else
		{
			TIFFSetField(tif, TIFFTAG_BITSPERSAMPLE, 16L, 16L, 16L);
			TIFFSetField(tif, TIFFTAG_SAMPLESPERPIXEL, 3L);
		}
		TIFFSetField(tif, TIFFTAG_PHOTOMETRIC, PHOTOMETRIC_RGB);
	}
...
static void horizontalDifference16(short* wp, int wc, int stride)
{
	if (wc > stride)
	{
		wc -= stride;
...
void BitmapIO_TIF::ClearBlock(void)
{
	...
	if (lzw_state.enc_incount > 0x007fffff)
	{ /* shift will overflow */
...
{
	int* htab_p = lzw_state.enc_htab + HSIZE;
	long i, m1 = -1;
...
BOOL BitmapIO_TIF::WriteTIF(FILE* stream)
{
	...
	// This is the width, in bytes, of the data contained in "shortstrip". This is used
	// to specify the number of bytes to be encoded to the LZW encoding method.
	long packw;
	...
	do
	{
		...
		for (j = 0; j < rps; j++)
		{
			...
			{ /* color tiff */
				...
				packw = width * spp; // 1 byte per component
			}
			else if (UserData.writeType == tif_write_16bit)
			{
				/* get a row and prepare for encoding */
				...
				GetOutputPixels(0, y, width, scanline_fl); // Get gamma-corrected pixels
				int index = 0;
				BMM_Color_fl* scanptr = scanline_fl;
				WORD* longstrip = (WORD*)shortstrip;
				for (k = 0; k < width; k++, ++scanptr)
				{
					float* pixel = (float*)scanptr;
					for (l = 0; l < spp; ++l)
					{
						longstrip[index++] = BMM_Color_fl::clipColor(pixel[l]);
					}
				}
				packw = width * spp * 2; // 2 bytes per component
			}
			...
			{ /* grey scale tiff */
				...
				packw = width; // 1 byte per component, with a single component per pixel
			}
			else if (UserData.writeType == tif_write_logl) /*Log-Luminance type tiff*/
			{
				...
				int type = 0;
				if (Storage()->IsHighDynamicRange() && Storage() && Storage()->GetStoragePtr(&type) &&
						type == BMM_LOGLUV_32)
				{
					LogLUV32Pixel* pixel = (LogLUV32Pixel*)Storage()->GetStoragePtr(&type);
					pixel += static_cast<std::ptrdiff_t>(y) * width;
				...
				{
					/* get a row and prepare for encoding */
					// [dl | 22feb2005] Use a BMM_Color_fl buffer to prevent clipping.
					GetOutputPixels(0, y, width, scanline_fl); // Get gamma-corrected pixels
					BMM_Color_fl* scanptr = scanline_fl;
					...
					for (k = 0; k < width; k++, ++scanptr)
					{
						...
						Color rgbColor(*scanptr); // construct float color struct from BMM_Color_fl
						Color xyz;
				...
				packw = width * 4; // 4 bytes per component, 1 with a single component per pixel
			}
			else if (UserData.writeType == tif_write_logluv)
			{
				...
				{
					Storage()->GetPixels(
							0, y, width, (BMM_Color_fl*)scanline); // no gamma correction, HDR from LogLUV storage.
				...
				packw = width * 4 * 3; // 4 bytes per component, 3 components per pixel
			}
			else
			{
				MSTR msg = MaxSDK::GetResourceStringAsMSTR(hInst, IDS_NO_WRITE_ALGORITHM);
				TIFFError("tif.cpp", msg.ToUTF8().data());
				packw = 0;
			}
#ifdef ALLOW_LZW_COMPRESSION
...
int BitmapIO_TIF::SaveTIF()
{
	...
	if (UserData.writeType == tif_write_color || UserData.writeType == tif_write_16bit)
	{
		write_alpha = UserData.writeAlpha && map->HasAlpha();
		spp = write_alpha ? 4 : 3;
	}
	else
	{
	...
	bool useLogEncoding = false;

	if (UserData.writeType == tif_write_16bit)
	{
		sStripSize = spp * width * 2; // 2 bytes per sample
	}
	else if (UserData.writeType == tif_write_logl)
	{
	...
	// [dl | 22Feb2005] Allocate a floating-point buffer, which is large enough to contain
	// either 8bit, 16bit, or floating-point data.
	PixelBufFloat sline(scanlineSize); // memory for the internal storage buffer.
	scanline =
			reinterpret_cast<BMM_Color_64*>(sline.Ptr()); // Cast is safe because buffer is garanteed to be large enough
	scanline_fl = sline.Ptr();
	...
	while ((short)(width * rps) <= 0 && rps > 1)
		rps /= 2;

	if (rps == 0 || (useLogEncoding && ((short)(width * rps) <= 0)))
		return TIF_SAVE_WRITE_ERROR;

	/* construct tiff file header */
	MakeTiffhead();
	
	if (!WriteTIF((FILE*)TIFFClientdata(tif)))
...
MSTR BitmapIO_TIF::GetCfgFilename() const
{
	MSTR filename(TheManager->GetDir(APP_PLUGCFG_DIR));
	if (filename.EndsWith('\\'))
	{
		filename += TIFCONFIGNAME;
	}
	else
	{
		filename += _T("\\") TIFCONFIGNAME;
	}
	return filename;
}
...
BOOL BitmapIO_TIF::ReadCfg()
{
	...
	MaxSDK::Util::BufferedFile file(GetCfgFilename().data(), _T("rb"), false);
	FILE* stream = file.getFile();
	if (stream == nullptr)
		return (FALSE);

	fseek(stream, 0, SEEK_END);
	DWORD size = (DWORD)ftell(stream);
...
//-- Find out buffer size
	...
	{
		MaxSDK::Util::BufferedFile file(GetCfgFilename().data(), _T("wb"), false);
		FILE* stream = file.getFile();
		if (stream)
		{
...
//-----------------------------------------------------------------------------
// #> BitmapIO_TIF::GetMaxBitmapYPosition
//

int BitmapIO_TIF::GetMaxBitmapYPosition(int tiff_y_pos, int tiff_orientation, int height)
{
	int max_y_pos = tiff_y_pos;
	switch (tiff_orientation)
	{
	case ORIENTATION_TOPLEFT:
		max_y_pos = tiff_y_pos;
		break;
	case ORIENTATION_BOTLEFT:
		max_y_pos = (height - 1) - tiff_y_pos;
		break;
...
BaseInterface* BitmapIO_TIF::GetInterface(Interface_ID id)
{
	if (id == BITMAPIOMETADATA_INTERFACE_ID)
	{
		return static_cast<BitmapIOMetaData*>(this);
	}
	return BitmapIO::GetInterface(id);
}

bool BitmapIO_TIF::UseLinearStorage(bool save)
{
	if (save)
	{
		// Logarithmic storage is treated as accepting linear input data
		// (Gamma on top of log makes little sense)
		return (UserData.writeType == tif_write_logl || UserData.writeType == tif_write_logluv);
	}
	else
	{
		// Logarithmic storage is treated as giving linear output data
		// (Gamma on top of log makes little sense)
		return (bi.Type() == BMM_LOGLUV_32);
	}
}
...
void MAXErrorHandler(const char* module, const char* fmt, va_list ap)
{
	...
	const TSTR titleFormat = GetString(IDS_MAX_TIF_ERROR);
	if ( module != nullptr )
	{
		_sntprintf_s(title, bufferSize, _TRUNCATE, titleFormat.data(), MSTR::FromUTF8(module).data());
	}
	else
	{
		const TSTR defaultModule = GetString(IDS_LIBTIFF_MODULE);
		_sntprintf_s(title, bufferSize, _TRUNCATE, titleFormat.data(), defaultModule.data());
	}
	
	vsnprintf_s(message, bufferSize, _TRUNCATE, (fmt != nullptr) ? fmt : "", ap);
	//-- GG: 06/26/02
	...
	{
		MaxSDK::MaxMessageBox(GetFocus(), MSTR::FromUTF8(message), title, MB_OK | MB_ICONINFORMATION);
	}
	else
	{
		TheManager->Max()->Log()->LogEntry(SYSLOG_ERROR, NO_DIALOG, nullptr, _T("%s - %s"), title, message);
	}
#else
	if (module != nullptr)
	{
