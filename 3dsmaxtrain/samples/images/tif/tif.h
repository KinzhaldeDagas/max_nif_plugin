...
#define BLOCKSIZE       131072  // changed from 65536 PR 2/4/2014 // changed from 16384 DS 2/15/99
...
class BitmapIO_TIF
		: public BitmapIO
		, public BitmapIOMetaData
{

private:
	Bitmap* loadMap = nullptr;
	BitmapStorage* loadStorage = nullptr;
	BitmapStorage* saveStorage = nullptr;
	FILE* inStream = nullptr;
	FILE* outStream = nullptr;

	BOOL load_alpha = FALSE;
	int nsamp = 0; // Number of samples per pixel
	...
	BYTE *comp_buf;
#endif
	BYTE *shortstrip = nullptr;
	BMM_Color_64 *scanline = nullptr;
	BMM_Color_fl* scanline_fl = nullptr;
	BOOL write_alpha;
	void MakeTiffhead();
	...
	MSTR GetCfgFilename() const;
	BOOL ReadCfg();
	void WriteCfg();

	//helper function to calculate max Y position using the image orientation
	static int GetMaxBitmapYPosition(int tiff_y_pos, int tiff_orientation, int height);
