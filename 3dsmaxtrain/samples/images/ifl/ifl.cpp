...
static TCHAR* extract_filename(const TCHAR* s, int* i)
{
	...
	_tcsncpy_s(line, _countof(line), s, _TRUNCATE);

	TCHAR* ptr = _tcsrchr(line, _T('\n'));
	if (ptr) *ptr = 0;

	int l = static_cast<int>(_tcsnlen(line, _countof(line))); // SR DCAST64: Downcast to 2G limit.
	while (line[--l] == _T(' '));
...
class IFLClassDesc:public ClassDesc {
	...
	public:
		...
		const TCHAR*    NonLocalizedClassName ( )          { return _T("IFL");     }
...
BMMRES BitmapIO_IFL::GetIFLFileList(const TCHAR* file, bool forceReParse) {
	...
	// - if last write time of IFL file is changed
	if (forceReParse || (m_fileList.Count() == 0) || (m_nonLoopedFileList.Count() == 0) || (m_iflName != file) ||
			IFLFileTimeStampChanged()) {

		EmptyFileList();
		m_iflName = _T("");
		m_iflName_timestamp = { 0, 0 };
		...
		m_iflName = file;
		m_iflName_timestamp = GetFileLastWriteTime(m_iflName);
	}
...
void BitmapIO_IFL::ResolveIFLFilePath(const TCHAR* iflFile, const TCHAR* filename, TCHAR* outputBuffer, int bufferSize) {
	...
	BMMSplitFilename(iflFile, tpath, _countof(tpath), NULL, 0, NULL, 0);
	BMMAppendSlash(tpath);
	
	BMMSplitFilename(filename, ipath, _countof(ipath), tfile, _countof(tfile), textn, _countof(textn));
...
TSTR BitmapIO_IFL::GetCfgFilename()
{
...
BOOL BitmapIO_IFL::ReadCfg()
{
	...
	if (fread(&tdata, 1, sizeof(IFLDATA), stream) != sizeof(IFLDATA))
		return (FALSE);
...
void BitmapIO_IFL::WriteCfg()
{
	...
	if (stream)
	{
		fwrite(&data, 1, sizeof(IFLDATA), stream);
	}
}

FILETIME BitmapIO_IFL::GetFileLastWriteTime(const TSTR& fname)
{
	FILETIME ft = { 0, 0 };
	HANDLE file = CreateFile(fname, 0, 0, NULL, OPEN_EXISTING, 0, NULL);
	if (!file)
		return ft;
...
bool BitmapIO_IFL::IFLFileTimeStampChanged() const
{
	FILETIME timestamp = GetFileLastWriteTime(m_iflName);
	return CompareFileTime(&timestamp, &m_iflName_timestamp) != 0;
}
//-- EOF: ifl.cpp -------------------------------------------------------------
