...
#define Alert(x) MaxSDK::MaxMessageBox(GetActiveWindow(), x, _T("Alert"), MB_OK);

const ChannelMask EDITPAT_CHANNELS = (GEOM_CHANNEL|SELECT_CHANNEL|SUBSEL_TYPE_CHANNEL|DISP_ATTRIB_CHANNEL|TOPO_CHANNEL|TEXMAP_CHANNEL|VERTCOLOR_CHANNEL);
...
//EditPatch/Icon names
#define EDITPATCH_REGULAR(type, iconName, cmd) \
	ToolButtonItem(type,iconName,EditPatchIcon::IconSize,EditPatchIcon::IconSize,24,23,cmd)

namespace EditPatchIcon {
	//icon size
...
class EditPatchData; 
class EPM_CreatePatchMouseProc : public CreateFaceMouseProcTemplate {
	EditPatchMod* m_EPatch;
protected:
	TSTR GetFaceCreationUndoMessage() {return GetString(IDS_CREATEPATCH);}
	// The patch-creation command never returns an error, so no error dialog will appear. 
	// Hence no error strings are necessary.
	TSTR GetFailedFaceCreationMessage() {return _T("");}
	TSTR GetFailedFaceCreationTitle() {return _T("");}
	int GetMaxDegree() {return 4;}
	...
	void ShowExistingVertexCursor() {
		SetCursor(GetInterface()->GetSysCursor(SYSCUR_SELECT));
	}
	void ShowCreateVertexCursor() {
		SetCursor( UI::MouseCursors::LoadMouseCursor(UI::MouseCursors::CreateVertex));
	}

	EditPatchData* GetPatchData();
public:
	EPM_CreatePatchMouseProc(EditPatchMod* in_e, Interface* in_i) 
		: CreateFaceMouseProcTemplate(in_i), m_EPatch(in_e) {}
};
...
public:
	...
	MouseCallBack *MouseProc(int *numPoints) {*numPoints=99999; return &proc;}
	ChangeForegroundCallback *ChangeFGProc() {return &fgProc;}
	BOOL ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void EnterMode();
	void ExitMode();
...
class EditPatchMod : public Modifier, IPatchOps, IPatchSelect, ISubMtlAPI, AttachMatDlgUser, public EditSSCB {
...
//watje command mode for the extrude and beevl		
		...
		// whether the weldBoxSize value has been initialized
		...
		int FindSet(const TSTR &setName,int level) const;
		void AddSet(const TSTR &setName,int level);
		void RemoveSet(const TSTR &setName,int level);
		void RemoveAllSets();
		void ClearSetNames();
...
//3-18-99 to suport render steps and removal of the mental tesselator
		...
		void GetClassName(MSTR& s, bool localized) const override { s= TSTR(_T("EditPatchMod")); }
...
//watje bevel and extrusion stuff
		...
		// methods for creating new vertices and patches. These methods retrieve the EditPatchData 
		// from the first modifier context.
		...
		void CreateVertex(EditPatchData& in_data, INode& in_node, Point3 in_point, int& out_index);
		void CreatePatch(EditPatchData& in_data, int in_vertexIndex1,int in_vertexIndex2,int in_vertexIndex3);
		void CreatePatch(EditPatchData& in_data, int in_vertexIndex1,int in_vertexIndex2,int in_vertexIndex3,int in_vertexIndex4);
...
public:
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_TH_EDITPATCH) : _T("Edit Patch"); }
...
class EditPatchClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Edit Patch"); }
	SClass_ID		SuperClassID() { return OSM_CLASS_ID; }
	Class_ID		ClassID() { return Class_ID(EDITPATCH_CLASS_ID,0); }
	const TCHAR* 	Category() { return GetString(IDS_RB_DEFEDIT);}
	void			ResetClassParams(BOOL fileReset);
	int             NumActionTables(){return 1;}
	ActionTable *GetActionTable(int i) {return GetEPActions ();}
	};
...
// compile-time validates size of EPMapVert class to ensure its size doesn't change since instances are 
// saved as a binary blob to scene file. Changes in size will break file i/o
static_assert(sizeof(EPMapVert) == (sizeof(int) + sizeof(Point3) + sizeof(DWORD)), "");
...
// compile-time validates size of EPMapPatch class to ensure its size doesn't change since instances are 
// saved as a binary blob to scene file. Changes in size will break file i/o
static_assert(sizeof(EPMapPatch) == (sizeof(int) + sizeof(DWORD)), "");
...
// compile-time validates size of EPMapUVVert class to ensure its size doesn't change since instances are 
// saved as a binary blob to scene file. Changes in size will break file i/o
static_assert(sizeof(EPMapUVVert) == (sizeof(int) + sizeof(DWORD)), "");
...
class PatchRestore : public RestoreObj {
	public:
		...
		PatchRestore(EditPatchData* pd, EditPatchMod* mod, PatchMesh *patch, const TCHAR *id=_T(""));
...
public:
	HWND hwnd;
	EditPatchMod *ep;
	SingleRefMakerPatchMMtl() {hwnd = NULL; ep = NULL;}
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("SingleRefMakerPatchMMtl"); }
};
...
public:
	HWND hwnd;
	EditPatchMod *ep;
	SingleRefMakerPatchMNode() {hwnd = NULL; ep = NULL;}
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("SingleRefMakerPatchMNode"); }
};
