 ...
 *<
   ...
   IMPORTANT USAGE NOTE:
      ...
      -->         patchData->UpdateChanges(patch);
               patchData->TempData(this)->Invalidate(TOPO_CHANNEL);
               }
...
// draws a dotted line
static void PatchXORDottedLine( HWND hwnd, IPoint2 p0, IPoint2 p1, bool bErase, bool bPresent);
...
EditSSMode*		  EditPatchMod::editSSMode		= NULL;

EPActionCB*			EditPatchMod::mpActions  = NULL;
Tab<ActionItem*>	EditPatchMod::mOverrides;
...
int               EditPatchMod::pickBoxSize  = 0;
int               EditPatchMod::weldBoxSize  = 0;
BOOL              EditPatchMod::weldBoxSizeInitialized = FALSE;
...
static GenSubObjType SOT_Handle(NULL, EditPatchIcon::stackHandle, 0);
static GenSubObjType SOT_Vertex(NULL, EditPatchIcon::stackVertex, 0);
static GenSubObjType SOT_Edge(NULL, EditPatchIcon::stackEdge, 0);
static GenSubObjType SOT_Patch(NULL, EditPatchIcon::stackPatch, 0);
static GenSubObjType SOT_Element(NULL, EditPatchIcon::stackElement, 0);
...
private:
    EditPatchMod* ep = nullptr;
    BOOL linearMapped = FALSE;
};

void PatchActionCallback::PopulateDynamicMenu(ICuiDynamicMenu* dynMenu, int actionId, HWND hWnd, const IPoint2& cursorPos)
{
   switch(ep->GetSubobjectType())
   {
      case EP_VERTEX:
      {
         if(ep->RememberVertThere(hWnd, cursorPos))
         {
            DWORD flags0 = 0, flags1 = 0, flags2 = 0;
            ...
            dynMenu->AddItem(PVERT_RESET, GetString(IDS_TH_RESET_TANGENTS), flags0);
            dynMenu->AddItem(PVERT_COPLANAR, GetString(IDS_TH_COPLANAR), flags1);
            dynMenu->AddItem(0, GetString(IDS_TH_CORNER), flags2);
      ...
      case EP_PATCH:
      {
         if(ep->RememberPatchThere(hWnd, cursorPos))
         {
            ...
            dynMenu->AddItem(PATCH_AUTO, GetString(IDS_TH_AUTOINTERIOR), flags1);
            dynMenu->AddItem(0, GetString(IDS_TH_MANUALINTERIOR), flags2);
         }
//watje new patch mapping
         ...
         dynMenu->AddItem(3, GetString(IDS_PW_LINEARMAPPING), flags3);
         dynMenu->AddSeparator();
         break;
      }
   }
...
void PatchActionCallback::DynamicMenuItemSelected(int actionId, int itemId)
{
    switch (ep->GetSubobjectType())
    {
    case EP_VERTEX:
        ep->SetRememberedVertType(itemId);
        break;
    case EP_PATCH:
        if (itemId == 3)
        {
            ...
            {
                ep->ChangeMappingTypeLinear(TRUE);
            }
        }
        else
        {
            ep->SetRememberedPatchType(itemId);
        }
        break;
    }
}
...
int EditPatchMod::FindSet(const TSTR &setName,int level) const
   {  
...
void EditPatchMod::AddSet(const TSTR &setName,int level)
   {
   assert(level>0 && level<6);
   level = namedSetLevel[level];
...
void EditPatchMod::RemoveSet(const TSTR &setName,int level)
   {
   MaybeFixupNamedSels();
...
void EditPatchMod::ActivateSubSelSet(TSTR &setName)
   ...
   for (int i = 0; i < mcList.Count(); i++) {
      ...
      if (patchData->tempData)
         patchData->TempData(this)->Invalidate(SELECT_CHANNEL);
      }
...
INT_PTR CALLBACK SoftSelectDlgProc( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
   {
   EditPatchMod *pEpm = DLGetWindowLongPtr<EditPatchMod *>( hDlg);
   ...
   switch ( message ) {
      case WM_INITDIALOG: {
         ...
         ReleaseISpinner( SetupIntSpinner   ( hDlg, IDC_EDGE_SPIN, IDC_EDGE, 1, 999, pEpm->mEdgeDist ) );
         ReleaseISpinner( SetupUniverseSpinner ( hDlg, IDC_FALLOFF_SPIN, IDC_FALLOFF, 0.0, 9999999.0, pEpm->mFalloff ) );
         ReleaseISpinner( SetupUniverseSpinner ( hDlg, IDC_PINCH_SPIN, IDC_PINCH, -10.0, 10.0, pEpm->mPinch ) );
         ReleaseISpinner( SetupUniverseSpinner ( hDlg, IDC_BUBBLE_SPIN, IDC_BUBBLE, -10.0, 10.0, pEpm->mBubble ) );
...
#ifdef DBG_NAMEDSELS
   DebugPrint(_T("Context/named sels:\n"));
   for (i=0; i < mcList.Count(); i++) {
      ...
      for(int j = 0; j < EP_NS_LEVELS; ++j) {
         ...
         for(int k = 0; k < pdSel.Count(); ++k)
            DebugPrint(_T("Context %d, level %d, set %d: [%s]\n"), i, j, k, *pdSel.names[k]);
         }
      }  
#endif //DBG_NAMEDSELS
...
#ifdef DBG_NAMEDSELS
      DebugPrint(_T("!!! NO FIXUP REQUIRED !!!\n"));
#endif //DBG_NAMEDSELS
      return;
      }

#ifdef DBG_NAMEDSELS
   DebugPrint(_T("*** Fixing up named sels ***\n"));
#endif //DBG_NAMEDSELS
...
#ifdef DBG_NAMEDSELS
         if(mc != pdc)
            DebugPrint(_T("****** mSel.Count=%d, pdSel.Count=%d ******\n"), mc, pdc);
#endif //DBG_NAMEDSELS
...
/*-------------------------------------------------------------------*/
const ActionTableId kActionID = 0x2880075b;

class EPActionCB : public ActionCallback {
	EditPatchMod *mpMod;
public:
	EPActionCB (EditPatchMod *m) : mpMod(m) { }
	BOOL ExecuteAction(int id);
};

BOOL EPActionCB::ExecuteAction (int id) {
	switch (id) {
	case ID_EDIT_SS:
...
ActionTable* GetEPActions() {
	TSTR name = GetString (IDS_TH_EDITPATCH);
...
class EPEditSSOverride : public IActionItemOverride 
{
protected:
	BOOL mActiveOverride;
	EditPatchMod *mpObj;
public:
	EPEditSSOverride(EditPatchMod *m):mActiveOverride(FALSE),mpObj(m){}
	BOOL IsOverrideActive();
	BOOL StartOverride();
	BOOL EndOverride();
};

BOOL EPEditSSOverride::IsOverrideActive()
{
	return mActiveOverride;
}


BOOL EPEditSSOverride::StartOverride()
{
...
BOOL EPEditSSOverride::EndOverride()
{
...
void EPMapper::RecordTopologyTags(PatchMesh &patch) {
   ...
   // Colors
   for(int i = 0; i < patch.getNumMapVerts(0); ++i)
      patch.mapVerts(0)[i].aux1 = 0xffffffff;
   // Illums
   for(int i = 0; i < patch.getNumMapVerts(MAP_SHADING); ++i)
      patch.mapVerts(MAP_SHADING)[i].aux1 = 0xffffffff;
   // Alphas
   for(int i = 0; i < patch.getNumMapVerts(MAP_ALPHA); ++i)
...
#ifdef VMAP_DEBUG
         if(map.delta != oldDelta)
            DebugPrint(_T("Vec %d delta changed from %.2f %.2f %.2f to %.2 %.2f %.2f\n"),i,oldDelta.x,oldDelta.y,oldDelta.z,map.delta.x,map.delta.y,map.delta.z);
#endif
         }
      }
   ...
   for(int i = 0; i < verts; ++i) {
      ...
      if(pv.vert >= 0) {
         ...
         if (!(pv.vert >= 0 && pv.vert < outPatch.numVerts))
            ...
            MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), warning, title, MB_OK | MB_APPLMODAL);
            }
   ...
   for(int mp=1; mp<inPatch.numTVerts.Count(); ++mp) {
      ...
      if (inPatch.mapPatches(mp)) {
         ...
         if(inPatches >= outPatches)
            ...
            TVPatch dummy;
            for(int i = inPatches; i < outPatches; ++i) {
   ...
   while (IO_OK==(res=iload->OpenChunk())) {
      switch(iload->CurChunkID())  {
         case EPVM_DATA_CHUNK: {
            ...
            for(int i = 0; i < verts; ++i) {
               R3EPMapVert theVert;
               res = iload->ReadVoid(&theVert,sizeof(R3EPMapVert),&nb); // no TCHAR data
            ...
            for(int i = 0; i < vecs; ++i) {
               R3EPMapVert theVec;
               res = iload->ReadVoid(&theVec,sizeof(R3EPMapVert),&nb); // no TCHAR data
...
void EditPatchData::Invalidate(ChannelMask channels,BOOL patchValid)
   {
   if ( tempData ) {
      tempData->Invalidate(channels,patchValid);
      }
   }
...
public:
   ...
   void Restore(int isUndo)
   {
      patchData->Invalidate(ALL_CHANNELS, FALSE);
   }
...
#ifdef DUMPING
// Dump edge map
DebugPrint(_T("Edge map:\n"));
for(i = 0; i < edges; ++i) {
   NewEdge &e = eMap[i];
   DebugPrint(_T("Old edge: %d  New edge: %d (%d %d) %d (%d %d) %d\n"),e.oldEdge,e.v1,e.vec12,e.vec21,e.v2,e.vec23,e.vec32,e.v3);
   }
#endif
...
PatchRestore::PatchRestore(EditPatchData* pd, EditPatchMod* mod, PatchMesh *patch, const TCHAR *id)
   {
   gotRedo = FALSE;
   epd = pd;
   this->mod = mod;
...
void PatchRestore::Restore(int isUndo)
   ...
   if ( epd->tempData && epd->TempData(mod)->PatchCached(t) ) {
      ...
      *patch = oldPatch;
      ...
      epd->TempData(mod)->Invalidate(GEOM_CHANNEL | TOPO_CHANNEL | SELECT_CHANNEL);
      }
   else
   if ( epd->tempData ) {
      epd->TempData(mod)->Invalidate(GEOM_CHANNEL | TOPO_CHANNEL | SELECT_CHANNEL, FALSE);
      }
   if(mod->ip)
...
//    even though it is a temporary object  to update any bind
   ...
   for(int i = 0; i < attPatch->numVecs; ++i)
   ...
   if ( theHold.Holding() )
      theHold.Put(new PatchRestore(patchData,this,patch,_T("DoAttach")));

   // Do the attach
   patch->Attach(attPatch, mat2Offset);
   patchData->UpdateChanges(patch);
   patchData->TempData(this)->Invalidate(TOPO_CHANNEL|GEOM_CHANNEL);
...
//DebugPrint(_T("XFormHandles\n"));
   ...
   for ( int obj = (bUseSel) ? 0 : object; obj < mcList.Count(); obj = (bUseSel) ? obj+1 : mcList.Count() ) {
      ...
      if(!TestAFlag(A_HELD)) {
         ...
         patchData->PreUpdateChanges(patch, FALSE);
         if ( theHold.Holding() )
            theHold.Put(new PatchRestore(patchData,this,patch,_T("XFormHandles")));
         ...
         if(patchData->DoingHandles())
            patchData->ApplyHandlesAndZero(*patch);      // Reapply the driven handle deltas
         else
      ...
      for (int vec = (bUseSel) ? 0 : handleIndex; vec < vecCount; vec = (bUseSel) ? vec+1 : vecCount) {
         ...
         if(primaryKnot >= 0) {
            ...
            if (bUseSel && (lockedHandles || (!shiftPressed && (vert.flags&PVERT_COPLANAR && vert.vectors.Count() > 2)))) {
               ...
               {
                  if (vert.vectors[vv] != vec && patch->vecSel[vert.vectors[vv]]) 
                  {
                     bMultiXForms = true;
                     break;
                  }
...
build_matrix:
      ...
      patchData->TempData(this)->Invalidate(GEOM_CHANNEL);
      patchData->SetFlag(EPD_BEENDONE,TRUE);
      }
...
void EditPatchMod::XFormVerts( 
   ...
   if(!TestAFlag(A_HELD)) {
      handleEdit = FALSE;
//DebugPrint(_T("Handle edit cleared\n"));
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(!TestAFlag(A_HELD)) {
         ...
         patchData->PreUpdateChanges(patch, FALSE);
         if ( theHold.Holding() ) {
            theHold.Put(new PatchRestore(patchData,this,patch,_T("XFormVerts")));
            }
         patchData->vdelta.Zero();     // Reset all deltas
         patchData->ClearHandleFlag();
         wasBroken = FALSE;
...
//    meshAdaptive = patchData->meshAdaptive = patchOb->GetAdaptive();  // Future use (Not used now)
   ...
   if (GetUsePatchNormals()) {
      ...
      int i;
      for (i=0; i<patchOb->NumXTCObjects(); i++)
      {
...
void EditPatchMod::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc)
   {
   if ( mc->localData ) {
...
void EditPatchMod::SelectSubComponent( HitRecord *hitRec, BOOL selected, BOOL all, BOOL invert )
   ...
   while(hitRec) {   
      ...
      patchData->PreUpdateChanges(patch, FALSE);
      if ( theHold.Holding() ) 
         theHold.Put(new PatchRestore(patchData,this,patch,_T("SelectSubComponent")));
      ...
      patchData->UpdateChanges(patch, FALSE);
      if ( patchData->tempData ) {
         patchData->tempData->Invalidate(SELECT_CHANNEL);
         }
      PatchSelChanged();
      }
...
void EditPatchMod::ClearSelection(int level) 
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      patchData->PreUpdateChanges(patch, FALSE);
      if ( theHold.Holding() ) {
         theHold.Put(new PatchRestore(patchData,this,patch,_T("ClearSelection")));
         }
...
void EditPatchMod::SelectAll(int level) 
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      patchData->PreUpdateChanges(patch, FALSE);
      if ( theHold.Holding() ) {
         theHold.Put(new PatchRestore(patchData,this,patch,_T("SelectAll")));
         }
...
void EditPatchMod::InvertSelection(int level) 
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      patchData->PreUpdateChanges(patch, FALSE);
      if ( theHold.Holding() )
         theHold.Put(new PatchRestore(patchData,this,patch,_T("InvertSelection")));
...
void EditPatchMod::SetDisplaySurface(BOOL sw) {
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(!patch)
         continue;
      if ( patchData->tempData ) {
         patchData->TempData(this)->Invalidate(DISP_ATTRIB_CHANNEL);
         }
      }
   nodes.DisposeTemporary();
...
void EditPatchMod::SetRelax(BOOL v, BOOL redraw) {
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if ( patchData->tempData ) {
         patchData->TempData(this)->Invalidate(ALL_CHANNELS);
         }
      }
   nodes.DisposeTemporary();
   if(redraw) {
...
void EditPatchMod::InvalidateGenSurfacePatchData()
{
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if ( patchData->tempData )
         patchData->TempData(this)->Invalidate(ALL_CHANNELS, FALSE);
...
void EditPatchMod::BuildPatchFromShape(TimeValue t, SplineShape *splShape, PatchMesh &pmesh)
{
   ...
   MaxSDK::Array<BindSplines> bindSplineList;

   //add any hook points first
   for (int bct = 0; bct < bShape.bindList.length(); bct++)
      {
      BindSplines bsp;
      Spline3D* pointSpline = NULL;
      Spline3D* segSpline = NULL;
   ...
   for (i = 0; i < bindSplineList.length(); i++)
      {
...
onselect:
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      int index;
      for(index = 0; index < patch->numPatches; index++)
...
void EditPatchMod::ChangeRememberedPatch(int type) {
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(patch == rememberedPatch) {
         ...
         patchData->PreUpdateChanges(patch, FALSE);
         if ( theHold.Holding() )
            theHold.Put(new PatchRestore(patchData,this,patch,_T("ChangeRememberedPatch")));
...
void EditPatchMod::ChangeSelPatches(int type) {
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(patch->patchSel.NumberSet()) {
         ...
         patchData->PreUpdateChanges(patch, FALSE);
         if ( theHold.Holding() )
            theHold.Put(new PatchRestore(patchData,this,patch,_T("ChangeSelPatches")));
...
onselect:
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      int index;
      for(index = 0; index < patch->numVerts; index++)
...
void EditPatchMod::ChangeRememberedVert(int type) {
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(patch == rememberedPatch) {
         ...
         patchData->PreUpdateChanges(patch, FALSE);
         if ( theHold.Holding() )
            theHold.Put(new PatchRestore(patchData,this,patch,_T("ChangeRememberedVert")));
         // Call the vertex type change function
...
void EditPatchMod::ChangeSelVerts(int type) {
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(patch->vertSel.NumberSet()) {
         ...
         patchData->PreUpdateChanges(patch, FALSE);
         if ( theHold.Holding() )
            theHold.Put(new PatchRestore(patchData,this,patch,_T("ChangeSelVerts")));
         // Call the vertex type change function
...
int EditPatchMod::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags, ModContext *mc) { 
   ...
   if (inPatchCreate && Editing()) createPatchMode->proc.DrawEstablishedFace(gw);
   gw->setRndLimits(savedLimits);
   return 0;   
   }
...
void EditPatchMod::ResolveTopoChanges() {
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      for(int set = 0; set < patchData->eselSet.Count(); ++set) {
      ...
      for(int set = 0; set < patchData->pselSet.Count(); ++set) {
...
void EditPatchMod::BeginEditParams( IObjParam *ip, ULONG flags, Animatable *prev )
   ...
   EPEditSSOverride *commandModeOverride = new EPEditSSOverride(this);
...
void EditPatchMod::DoVertDelete() {
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(patch->vertSel.NumberSet()) {
         ...
         patchData->PreUpdateChanges(patch);
         if ( theHold.Holding() )
            theHold.Put(new PatchRestore(patchData,this,patch,_T("DoVertDelete")));
         // Call the vertex delete function
         DeleteSelVerts(patch);
         patchData->UpdateChanges(patch);
         patchData->TempData(this)->Invalidate(TOPO_CHANNEL);
...
void EditPatchMod::DoEdgeDelete() {
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(patch->edgeSel.NumberSet()) {
         ...
         patchData->PreUpdateChanges(patch);
         if ( theHold.Holding() )
            theHold.Put(new PatchRestore(patchData,this,patch,_T("DoEdgeDelete")));
         int edges = patch->getNumEdges();
         int patches = patch->getNumPatches();
         int verts = patch->getNumVerts();
...
void EditPatchMod::DoPatchAdd(int type) {
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(patch->edgeSel.NumberSet()) {
         ...
         patchData->PreUpdateChanges(patch);
         if ( theHold.Holding() )
            theHold.Put(new PatchRestore(patchData,this,patch,_T("DoPatchAdd")));
...
void EditPatchMod::DoPatchDelete() {
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(patch->patchSel.NumberSet()) {
         ...
         patchData->PreUpdateChanges(patch);
         if ( theHold.Holding() )
            theHold.Put(new PatchRestore(patchData,this,patch,_T("DoPatchDelete")));
...
//DebugPrint(_T("Checking patch %d\n"),thePatch);
      ...
      switch(level) {
         ...
         case EP_EDGE:
            if(patch->edgeSel.NumberSet()) {
               ...
               for(int jk = 0; jk < verts; ++jk) {
                  if(vertBreaks[jk]) {
//DebugPrint(_T("*** Starting test for vertex %d ***\n"),jk);
                     ...
                     for(int j = 0; j < v.patches.Count(); ++j) {
                        if(!pChecked[j]) {
                           ...
                           pl->vertex = jk;
//DebugPrint(_T("New loop started\n"));
                           CheckBreakLoop(patch, v, j, pChecked, pl);
                           if(gotLoop) {
//DebugPrint(_T("Got loop, Adding to plt\n"));
                              plt.Add(pl);
                              numNewVerts++;
                              }
                           else {
//DebugPrint(_T("No loop yet, discarding first one\n"));
                              delete pl;
                              gotLoop = TRUE;
                              }
                           }
      ...
      altered = holdNeeded = 1;
      if ( theHold.Holding() )
         theHold.Put(new PatchRestore(patchData,this,patch,_T("DoBreak")));
...
void EditPatchMod::DoExtrude()
{
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      // If any bits are set in the selection set, let's DO IT!!
      if((type == EP_PATCH && patch->patchSel.AnyBitSet())|| (type == EP_EDGE && patch->edgeSel.AnyBitSet())) {
         altered = holdNeeded = TRUE;
         patchData->PreUpdateChanges(patch);
         if ( theHold.Holding() )
            theHold.Put(new PatchRestore(patchData,this,patch));
...
void EditPatchMod::DoFlipNormals(PatchMesh *pmesh, int patchIndex, BOOL element) {
   ...
   for (int i = 0; i < mcList.Count(); i++) {
      ...
      patchData->PreUpdateChanges(patch);
      if ( theHold.Holding() )
         theHold.Put(new PatchRestore(patchData,this,patch,_T("NormalFlip")));
      holdNeeded = TRUE;
...
void EditPatchMod::CreateVertex(EditPatchData& in_data, INode& in_node, Point3 pt, int& newIndex) {
	if (ip == 0) return;
	BOOL holdNeeded = FALSE;
	theHold.Begin();
	RecordTopologyTags();
	...
	EditPatchData *patchData = &in_data;
	PatchMesh *patch = in_data.TempData(this)->GetPatch(ip->GetTime());

	if(patch) {
		patchData->PreUpdateChanges(patch);
		if ( theHold.Holding() ) {
			//RecordTopologyTags();
			theHold.Put(new PatchRestore(patchData,this,patch,_T("CreateVertex")));
		}
		holdNeeded = TRUE;
		// Add the new vertex
		pt = pt * Inverse(in_node.GetObjectTM(ip->GetTime()));
...
void EditPatchMod::CreatePatch(EditPatchData& in_data, int in_vertexIndexA,int in_vertexIndexB,int in_vertexIndexC) {
	if (ip == 0) return;
	BOOL holdNeeded = FALSE;
	ModContextList mcList;  
	INodeTab nodes;	
	...
	if(patch) {
		patchData->PreUpdateChanges(patch);
		if ( theHold.Holding() )
			theHold.Put(new PatchRestore(patchData,this,patch,_T("CreatePatch")));
		holdNeeded = TRUE;
		...
		Point3 vertA = patch->getVert(in_vertexIndexA).p;
		Point3 vertB = patch->getVert(in_vertexIndexB).p;
		Point3 vertC = patch->getVert(in_vertexIndexC).p;
		...
		Tab<int> foundEdgesAB = patch->GetEdge(in_vertexIndexA,in_vertexIndexB);
		Tab<int> foundEdgesBC = patch->GetEdge(in_vertexIndexB,in_vertexIndexC);
		Tab<int> foundEdgesCA = patch->GetEdge(in_vertexIndexC,in_vertexIndexA);

		// allocate the edge vectors when none are selected
		int vecAB = 0, vecBA = 0, vecBC = 0, vecCB = 0, vecCA = 0, vecAC = 0;
		...
		Tab<int> neighboringPatches = patch->GetPatches(in_vertexIndexA);
		for (int i=0; i<neighboringPatches.Count(); ++i)
			neighborSmGroups |= patch->patches[neighboringPatches[i]].smGroup;

		neighboringPatches = patch->GetPatches(in_vertexIndexB);
		for (int i=0; i<neighboringPatches.Count(); ++i)
			neighborSmGroups |= patch->patches[neighboringPatches[i]].smGroup;

		neighboringPatches = patch->GetPatches(in_vertexIndexC);
		for (int i=0; i<neighboringPatches.Count(); ++i)
			neighborSmGroups |= patch->patches[neighboringPatches[i]].smGroup;
		...
		patch->MakeTriPatch(
			numPatches, 
			in_vertexIndexA, vecAB, vecBA,  
			in_vertexIndexB, vecBC, vecCB,  
			in_vertexIndexC, vecCA, vecAC,  
...
void EditPatchMod::CreatePatch(EditPatchData& in_data, int vertIndexA, int vertIndexB, int vertIndexC, int vertIndexD) {
   if (ip == 0) return;
   BOOL holdNeeded = FALSE;
   ModContextList mcList;  
   INodeTab nodes;	
   ...
   if(patch) {
      ...
      Point3 vertA = patch->getVert(vertIndexA).p;
      Point3 vertB = patch->getVert(vertIndexB).p;
      Point3 vertC = patch->getVert(vertIndexC).p;
      Point3 vertD = patch->getVert(vertIndexD).p;
      ...
      // check if any existing edges were selected
      Tab<int> foundEdgesAB = patch->GetEdge(vertIndexA,vertIndexB);
      Tab<int> foundEdgesBC = patch->GetEdge(vertIndexB,vertIndexC);
      Tab<int> foundEdgesCD = patch->GetEdge(vertIndexC,vertIndexD);
      Tab<int> foundEdgesDA = patch->GetEdge(vertIndexD,vertIndexA);

      // allocate the edge vectors when none are selected
      int vecAB = 0, vecBA = 0, vecBC = 0, vecCB = 0, vecCD = 0, vecDC = 0, vecDA = 0, vecAD = 0;
      if (!foundEdgesAB.Count()) {
         vecAB = numVecs; 
         vecBA = numVecs+1;
         numVecs += 2; // need 2 new ones for this edge
      ...
      // find all neighboring smoothing groups
      DWORD neighborSmGroups = 0;
      Tab<int> neighboringPatches = patch->GetPatches(vertIndexA);
      for (int i=0; i<neighboringPatches.Count(); ++i)
         neighborSmGroups |= patch->patches[neighboringPatches[i]].smGroup;

      neighboringPatches = patch->GetPatches(vertIndexB);
      for (int i=0; i<neighboringPatches.Count(); ++i)
         neighborSmGroups |= patch->patches[neighboringPatches[i]].smGroup;

      neighboringPatches = patch->GetPatches(vertIndexC);
      for (int i=0; i<neighboringPatches.Count(); ++i)
         neighborSmGroups |= patch->patches[neighboringPatches[i]].smGroup;

      neighboringPatches = patch->GetPatches(vertIndexD);
      for (int i=0; i<neighboringPatches.Count(); ++i)
         neighborSmGroups |= patch->patches[neighboringPatches[i]].smGroup;
      ...
      patch->MakeQuadPatch(
         numPatches, 
         vertIndexA, vecAB, vecBA,  
         vertIndexB, vecBC, vecCB,  
         vertIndexC, vecCD, vecDC,  
         vertIndexD, vecDA, vecAD, 
...
void EditPatchMod::CreateVertex(Point3 pt, int& newIndex) {
	...
	if (mcList.Count() > 0) {
		EditPatchData* patchData = (EditPatchData*)mcList[0]->localData;
		CreateVertex(*patchData, *nodes[0], pt, newIndex);
	}
...
void EditPatchMod::CreatePatch(int vertIndxA,int vertIndxB,int vertIndxC) {
	...
	if (mcList.Count() > 0) {
		EditPatchData* patchData = (EditPatchData*)mcList[0]->localData;
		CreatePatch(*patchData, vertIndxA, vertIndxB, vertIndxC);
	}
...
void EditPatchMod::CreatePatch(int vertIndxA,int vertIndxB,int vertIndxC,int vertIndxD) {
	...
	if (mcList.Count() > 0) {
		EditPatchData* patchData = (EditPatchData*)mcList[0]->localData;
		CreatePatch(*patchData, vertIndxA, vertIndxB, vertIndxC, vertIndxD);
	}
...
void EditPatchMod::DoEdgeSubdivide() {
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(patch->edgeSel.NumberSet()) {
         ...
         patchData->PreUpdateChanges(patch);
         if ( theHold.Holding() )
            theHold.Put(new PatchRestore(patchData,this,patch,_T("DoEdgeSubdivide")));
         // Call the patch add function
         SubdividePatch(SUBDIV_EDGES, propagate, patch);
         patchData->UpdateChanges(patch);
         patchData->TempData(this)->Invalidate(TOPO_CHANNEL);
...
void EditPatchMod::DoPatchSubdivide() {
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(patch->patchSel.NumberSet()) {
         ...
         patchData->PreUpdateChanges(patch);
         if ( theHold.Holding() )
            theHold.Put(new PatchRestore(patchData,this,patch,_T("DoPatchSubdivide")));
         // Call the patch add function
         SubdividePatch(SUBDIV_PATCHES, propagate, patch);
         patchData->UpdateChanges(patch);
         patchData->TempData(this)->Invalidate(TOPO_CHANNEL);
...
void EditPatchMod::DoVertWeld() {
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(patch->vertSel.NumberSet() > 1) {
         ...
         patchData->PreUpdateChanges(patch);
         if ( theHold.Holding() )
            theHold.Put(new PatchRestore(patchData,this,patch,_T("DoVertWeld")));
...
void EditPatchMod::DoVertWeld(int fromVert, int toVert, ModContext* pTheModContext) {
   ...
   if(patch) {
      patchData->PreUpdateChanges(patch);
      if ( theHold.Holding() )
         theHold.Put(new PatchRestore(patchData,this,patch,_T("VertWeld")));
...
//watje 10-4-99  184681 
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(patch->patchSel.NumberSet()) {
         ...
         patchData->PreUpdateChanges(patch);
         if ( theHold.Holding() ) {
            theHold.Put(new PatchRestore(patchData,this,patch,_T("DoPatchDetach")));
            }
...
//watje 10-4-99 184681
         ...
         for(int jk = 0, i2 = oldVecs; jk < wpatch.numVecs; ++jk, ++i2) {
            pmesh.vecs[i2] = wpatch.vecs[jk];
            pmesh.vecs[i2].p = pmesh.vecs[i2].p * tm;
            }
         for(int jk = 0, i2 = oldPatches; jk < wpatch.numPatches; ++jk, ++i2) {
            ...
            for(int j = 0; j < (p2.type * 2); ++j) // Adjust edge vectors
               p2.vec[j] += oldVecs;
            }
         ...
         for(int chan = -NUM_HIDDENMAPS; chan < pmesh.getNumMaps(); ++chan) {
            if(chan < wpatch.getNumMaps()) {
               ...
               for(int i = 0, i2 = oldTVerts; i < wpatch.getNumMapVerts(chan); ++i, ++i2)
                  pmesh.mapVerts(chan)[i2] = wpatch.mapVerts(chan)[i];
               if(pmesh.mapPatches(chan)) {
                  for(int i = 0, i2 = oldPatches; i < wpatch.numPatches; ++i, ++i2) {
...
void EditPatchMod::SetSelMatIndex(int index)
   ...
   for (int i = 0; i < mcList.Count(); i++) {
      ...
      patchData->PreUpdateChanges(patch, FALSE);
      if (theHold.Holding()) {
         theHold.Put(new PatchRestore(patchData,this,patch,_T("SetSelMatIndex")));
         }
...
void EditPatchMod::DoAccept(TimeValue t)
{

}

void EditPatchMod::SetPinch(TimeValue t, float pinch)
{	
	Rect rect;
	ISpinnerControl* spin = GetISpinner(GetDlgItem(hSoftSelPanel, IDC_PINCH_SPIN));

	spin->SetValue(pinch, TRUE);
	mPinch = pinch;
...
void EditPatchMod::SetBubble(TimeValue t, float bubble)
{
	Rect rect;
	ISpinnerControl* spin = GetISpinner(GetDlgItem(hSoftSelPanel, IDC_BUBBLE_SPIN));

	spin->SetValue(bubble, TRUE);
	mBubble = bubble;
...
void EditPatchMod::SetFalloff(TimeValue t,float falloff)
{
	Rect rect;
	ISpinnerControl* spin = GetISpinner(GetDlgItem(hSoftSelPanel, IDC_FALLOFF_SPIN));

	spin->SetValue(falloff, TRUE);
	mFalloff = falloff;
...
float EditPatchMod::GetPinch(TimeValue t)
{
	return mPinch;
}

float EditPatchMod::GetBubble(TimeValue t)
{
	return mBubble;
}

float EditPatchMod::GetFalloff(TimeValue t)
{
	return mFalloff;
}


void EditPatchMod::EditSoftSelectionMode()
{
...
INT_PTR CALLBACK PatchSelectDlgProc( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
   {
   EditPatchMod *ep = DLGetWindowLongPtr<EditPatchMod *>( hDlg);
   ...
   switch ( message ) {
      case WM_INITDIALOG: {
         ...
         DLSetWindowLongPtr( hDlg, ep);         
         // Set up the editing level selector
         iToolbar = GetICustToolbar(GetDlgItem(hDlg,IDC_SELTYPE));
         // CAL-06/10/03: add handle sub-object mode. (FID #1914)
         iToolbar->AddTool(EDITPATCH_REGULAR(CTB_CHECKBUTTON,EditPatchIcon::vertexSelection,EP_VERTEX));
         iToolbar->AddTool(EDITPATCH_REGULAR(CTB_CHECKBUTTON,EditPatchIcon::handleSelection,EP_HANDLE));
         iToolbar->AddTool(EDITPATCH_REGULAR(CTB_CHECKBUTTON,EditPatchIcon::edgeSelection,EP_EDGE));
         iToolbar->AddTool(EDITPATCH_REGULAR(CTB_CHECKBUTTON,EditPatchIcon::patchSelection,EP_PATCH));
         iToolbar->AddTool(EDITPATCH_REGULAR(CTB_CHECKBUTTON,EditPatchIcon::elementSelection,EP_ELEMENT));
         ReleaseICustToolbar(iToolbar);
         ep->RefreshSelType();
...
INT_PTR CALLBACK PatchOpsDlgProc( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
   ...
   ISpinnerControl* spin = NULL;
   ICustButton* ebut = NULL;

   switch ( message ) {
      case WM_INITDIALOG: {
		  if (!EditPatchMod::weldBoxSizeInitialized)
		  {
			  EditPatchMod::weldBoxSizeInitialized = TRUE;

			  ViewExp22& vpt = static_cast<ViewExp22&>(GetCOREInterface()->GetActiveViewExp());
			  EditPatchMod::pickBoxSize = EditPatchMod::weldBoxSize = vpt.GetPickingSelectionRadius();
		  }
         ...
         //3-18-99 to support render steps and removal of the mental tesselator
      ...
      case WM_DESTROY:
		 ...
		 if( ep->genThreshSpin ) {
			ReleaseISpinner(ep->genThreshSpin);
			ep->genThreshSpin = NULL;
		 }
//3-18-99 to suport render steps and removal of the mental tesselator
            ...
            case IDC_EP_CREATE:
               ...
               case EP_ELEMENT:
                  if (ep->ip->GetCommandMode()==ep->createPatchMode)
                     ep->ip->PopCommandMode();
                  else ep->ip->PushCommandMode(ep->createPatchMode);
                  break;
               default:
                  break;
               }
...
INT_PTR CALLBACK PatchSurfDlgProc( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
   ...
   switch ( message ) {
      ...
      case CC_COLOR_CHANGE:
         ...
         switch (LOWORD(wParam)) {
         case IDC_VERT_COLOR:
            ep->SetPatchColor (Color(iCol->GetAColor()));
            break;
         case IDC_VERT_ILLUM:
            ep->SetPatchColor (Color(iCol->GetAColor()), MAP_SHADING);
            break;
         }
         break;
...
INT_PTR CALLBACK PatchVertSurfDlgProc( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
   ...
   switch ( message ) {
      case WM_INITDIALOG: {
         ...
         DLSetWindowLongPtr( hDlg, ep);         
         CheckSelectBy(hDlg);
         ReleaseISpinner( SetupIntSpinner (hDlg, IDC_VERT_SELRSPIN, IDC_VERT_SELR, 0, 255, selDeltaR) );
         ReleaseISpinner( SetupIntSpinner (hDlg, IDC_VERT_SELGSPIN, IDC_VERT_SELG, 0, 255, selDeltaG) );
         ReleaseISpinner( SetupIntSpinner (hDlg, IDC_VERT_SELBSPIN, IDC_VERT_SELB, 0, 255, selDeltaB) );

         // Select-by-color swatch.
         iCol = GetIColorSwatch (GetDlgItem(hDlg,IDC_VERT_SELCOLOR), selByColor,
      ...
      case CC_COLOR_CHANGE:
         ...
         switch (LOWORD(wParam)) {
         case IDC_VERT_COLOR:
            ep->SetVertColor (Color(iCol->GetAColor()));
            break;
         case IDC_VERT_ILLUM:
            ep->SetVertColor (Color(iCol->GetAColor()), MAP_SHADING);
            break;
         case IDC_VERT_SELCOLOR:
            AColor rgba = iCol->GetAColor();
			selByColor = Point3(rgba);
            break;
         }
         break;
...
int EPM_NormalFlipMouseProc::proc (HWND hwnd, int msg, int point, int flags, IPoint2 m ) {   
   ...
   switch (msg) {
      ...
      case MOUSE_POINT:
         if(HitAPatch(vpt.ToPointer(), &m,  &pix))
...
int EPM_CreateVertMouseProc::proc (HWND hwnd, int msg, int point, int flags, IPoint2 screenPoint) {
   ...
   ViewExp &vpt = mpIP->GetViewExp (hwnd);
   ...
   case MOUSE_FREEMOVE:
      SetCursor( UI::MouseCursors::LoadMouseCursor(UI::MouseCursors::CreateVertex));
...
{
	backspaceRouter.UnRegister(&proc);
	if (!po->hOpsPanel) return;
	ICustButton *but = GetICustButton(GetDlgItem(po->hOpsPanel,IDC_EP_CREATE));
	but->SetCheck(FALSE);
	ReleaseICustButton(but);
...
Point3 EPM_CreatePatchMouseProc::GetVertexWP(int in_vertex)
{
	INode* l_node = NULL;
	if (HasPrimaryNode()) {
		l_node = GetPrimaryNode();
	} else {
		INodeTab l_nodeTab;
		ModContextList l_modContextList;
		GetInterface()->GetModContexts (l_modContextList, l_nodeTab);
		l_node = l_nodeTab[0];
	}
	
	EditPatchData *epd = GetPatchData();
	PatchMesh *patch = epd->TempData(m_EPatch)->GetPatch(GetInterface()->GetTime());

	if(l_node&&patch&&in_vertex>=0&&in_vertex< patch->numVerts)
		return l_node->GetObjectTM(GetInterface()->GetTime()) * patch->verts[in_vertex].p;
	return Point3(0.0f,0.0f,0.0f);
}

bool EPM_CreatePatchMouseProc::HitTestVertex(IPoint2 in_mouse, ViewExp& in_viewport, INode*& out_node, int& out_vertex) 
{                               
	...
	SetPatchHitOverride(HITTYPE_POINT);
	...
	for (HitRecord *hr = in_viewport.GetSubObjHitList().First(); hr != 0; hr = hr->Next()) {
		if (HasPrimaryNode() && hr->modContext->localData != GetPatchData()) {
			continue;
		}
		if (((PatchHitData*)hr->hitData)->type != PATCH_HIT_VERTEX) {
			continue;
		}
	...
	if (l_bestHit != 0) {
		PatchHitData* l_hitData = dynamic_cast<PatchHitData*>(l_bestHit->hitData);
		DbgAssert(l_hitData != 0);
		out_node = l_bestHit->nodeRef;
		out_vertex = l_hitData->index;
	}
	return l_bestHit != 0;
}

EditPatchData* EPM_CreatePatchMouseProc::GetPatchData() {
	INodeTab l_nodeTab;
	ModContextList l_modContextList;
	GetInterface()->GetModContexts (l_modContextList, l_nodeTab);

	if (!HasPrimaryNode()) {
		return static_cast<EditPatchData*>(l_modContextList[0]->localData);
	} else 
	{
		DbgAssert(l_nodeTab.Count() == l_modContextList.Count());
		for (int i = 0; i < l_nodeTab.Count(); i++) {
			if (l_nodeTab[i] == GetPrimaryNode()) {
				return static_cast<EditPatchData*>(l_modContextList[i]->localData);
			}
		}
	}
	DbgAssert(false);
...
bool EPM_CreatePatchMouseProc::CreateVertex( const Point3& in_worldLocation, int& out_vertex )
{
	INode* l_node = NULL;
	if (HasPrimaryNode()) {
		l_node = GetPrimaryNode();
	...
	m_EPatch->CreateVertex(*GetPatchData(), *l_node, in_worldLocation, out_vertex); 
	return out_vertex >= 0;
}

bool EPM_CreatePatchMouseProc::CreateFace( MaxSDK::Array<int>& in_vertices )
{
	DbgAssert(in_vertices.length() == 3 || in_vertices.length() == 4);
	if (in_vertices.length() == 3) {
		m_EPatch->CreatePatch(*GetPatchData(), in_vertices[0], in_vertices[1], in_vertices[2]);
	} else {
		m_EPatch->CreatePatch(*GetPatchData(), in_vertices[0], in_vertices[1], in_vertices[2], in_vertices[3]);
	}
	return true;
}
...
int EPM_VertWeldMouseProc::proc(HWND hwnd, int msg, int point, int flags, IPoint2 screenPoint )
{
   ...
   int vertIndex;
   int result = TRUE;
   ModContext* pTheModContext = NULL;

   switch (msg) {
   case MOUSE_ABORT:
      // Erase old dotted line
      PatchXORDottedLine(hwnd, anchor, lastPoint, true, true);
   case MOUSE_PROPCLICK:
      mpIP->SetStdCommandMode(CID_OBJMOVE);
   ...
   case MOUSE_POINT:
      ...
      switch(point) {
      case 0:
         if (HitAVert(vpt.ToPointer(), &screenPoint,  vertIndex, pTheModContext)) {
            fromVert = vertIndex;
            mpFromModContext = pTheModContext;
            // Draw new dotted line
            anchor = lastPoint = screenPoint;
            PatchXORDottedLine(hwnd, anchor, screenPoint, false, true);
         }
         else {
            result = FALSE;
         }
   ...
   case MOUSE_MOVE:
      // Erase old dotted line
      PatchXORDottedLine(hwnd, anchor, lastPoint, true, false);
      // Draw new dotted line
      PatchXORDottedLine(hwnd, anchor, screenPoint, false, true);
...
void EditPatchMod::SetVertColor(Color clr, int mp) {
   ...
   for (int i = 0; i < mcList.Count(); i++) {
      ...
      if (patch->vertSel.NumberSet()) {
         // Start a restore object...
         if (theHold.Holding()) {
            theHold.Put(new PatchRestore(patchData,this,patch,_T("SetVertColor")));
            }
...
void EditPatchMod::SetPatchColor(Color clr, int mp) {
   ...
   for (int i = 0; i < mcList.Count(); i++) {
      ...
      if (patch->patchSel.NumberSet()) {
         ...
         patchData->PreUpdateChanges(patch, FALSE);
         if (theHold.Holding()) {
            theHold.Put(new PatchRestore(patchData,this,patch,_T("SetPatchColor")));
            }
         // Use the special UVW mapping option built into the PatchMesh class
		 patch->ApplyUVWMap(0, 1.0f, 1.0f, 1.0f, 0, 0, 0, 0, Matrix3{}, mp);
...
BOOL EditPatchMod::HasFaceSelection(ModContext* mc) {
   ...
   if (GetSubobjectType() == EP_PATCH && patch->patchSel.AnyBitSet()) return TRUE;
   ...
   switch ( msg ) {
      ...
      case MOUSE_POINT:
         switch(point) {
            case 0:
               {
               if(HitAKnot(vpt.ToPointer(), &m,  &vert1))
                  {
                  res = TRUE;
                  anchor = lastPoint = m;
                  PatchXORDottedLine(hwnd, anchor, m, false, true);   // Draw it!
//get valid seg list
...
//if a valid segemtn change cursor
      ...
      case MOUSE_MOVE:
         ...
         PatchXORDottedLine(hwnd, anchor, lastPoint, true, false);
         // Draw new dotted line
         PatchXORDottedLine(hwnd, anchor, m, false, true);
         lastPoint = m;
         if(HitASegment(vpt.ToPointer(), &m,  &seg1))
            {
//if a valid segemtn change cursor
...
/*
         if ( HitTest(vpt,&m,HITTYPE_POINT,HIT_ABORTONHIT,1) ) {
            ...
            if (rec )
               {
               SetCursor(UI::MouseCursors::LoadCursor(UI::MouseCursors::Selection));
               }
            }
         SetCursor(LoadCursor(NULL,IDC_ARROW));
*/
...
void EditPatchMod::MaybeClonePatchParts() {
   ...
   if(shiftPressed) {
      ...
      for ( int i = 0; i < mcList.Count(); i++ ) {
         ...
         if(type == EP_PATCH && patch->patchSel.AnyBitSet()) {
            holdNeeded = TRUE;
            patchData->PreUpdateChanges(patch);
            if ( theHold.Holding() )
               theHold.Put(new PatchRestore(patchData,this,patch,_T("MaybeClone")));
            patch->ClonePatchParts();
            patchData->UpdateChanges(patch);
            patchData->TempData(this)->Invalidate(TOPO_CHANNEL);
            }
         else
         if(type == EP_EDGE && patch->edgeSel.AnyBitSet()) {
            holdNeeded = TRUE;
            patchData->PreUpdateChanges(patch);
            if ( theHold.Holding() )
               theHold.Put(new PatchRestore(patchData,this,patch,_T("MaybeClone")));
...
RefResult EditPatchMod::NotifyRefChanged( const Interval& changeInt,RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate ) {
   switch (message) {
      case REFMSG_CHANGE:
         DebugPrint(_T("Ref %p changed!\n"),hTarget);
         objectChanged = TRUE;
         break;
      }
   return REF_SUCCEED;
