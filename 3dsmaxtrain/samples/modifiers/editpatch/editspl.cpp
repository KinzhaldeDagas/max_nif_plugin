...
typedef std::tuple<Point3, int, int, int> ClusterCenterPolyStartknotEndknot;

// Tuple members for clusters:
constexpr int ClusterCenter = 0;
constexpr int ClusterPoly = 1;
constexpr int ClusterStartKnot = 2;
constexpr int ClusterEndKnot = 3;

// File version constants
static constexpr int EDITSPLINEMOD_VERSION_INITIAL = 0;
static constexpr int EDITSPLINEMOD_VERSION_CORRECTED_DPOINT3_ADD = 1;
static constexpr int EDITSPLINEMOD_LATEST_VERSION = EDITSPLINEMOD_VERSION_CORRECTED_DPOINT3_ADD;

typedef std::vector<ClusterCenterPolyStartknotEndknot> ClusterVector;

std::map < INode*, ClusterVector > nodeClusterMap;

static bool GetSegmentClusters(INode* node, BezierShape& shape, const Matrix3& tm)
{
    std::vector<ClusterCenterPolyStartknotEndknot> clusters;

    int polys = shape.splineCount;
    for (int poly = 0; poly < polys; ++poly)
    {
        Spline3D* spline = shape.splines[poly];
        int numSegs = spline->Segments();
        int numKnots = spline->KnotCount();
        const BitArray& ssel = shape.segSel[poly];
        // Special case if all segments selected (entire poly is a cluster)
        if (ssel.NumberSet() == numSegs)
        {
            Box3 box;
            for (int i = 1; i < spline->Verts(); i += 3)
                box += spline->GetVert(i) * tm;
            Point3 center = box.Center();
            clusters.push_back(std::make_tuple(center, poly, 0, numKnots - 1));
        }
        else
        {
            // Not all segments selected; pick out the clusters
            bool closed = !!spline->Closed();
            int startSegment = 0;
            int endSegment = numSegs;

            // Special treatment for selected segment 0 (closed can wrap around)
            if (closed && ssel[0] && ssel[numSegs - 1])
            {
                int startIndex = numSegs - 1;
                for (int i = startIndex - 1; i > 0; --i)
                {
                    if (!ssel[i]) break;
                    startIndex = i;
                }
                // 'startIndex' now has the first segment on the cluster
                int endIndex = 1;
                ...
                // Now we should have startIndex and endIndex filled in. Create a cluster for it by following the knots around
                int firstKnot = startIndex;
                int lastKnot = endIndex;
                Box3 box;
                for (int k = firstKnot; k != (lastKnot + 1); k++)
                {
                    if (k >= numKnots)
                    {
                        k = 0;	// Wrap around
                    }
                    box += spline->GetVert(k * 3 + 1) * tm;
                }
                Point3 center = box.Center();
                clusters.push_back(std::make_tuple(center, poly, firstKnot, lastKnot));

                // We've already processed these segments, so mark the new test span
                startSegment = lastKnot;
                endSegment = firstKnot;
            }

            // Remaining segments are normal, no wraparound
    ...
    // Stash the cluster information associated with the node
    auto it = nodeClusterMap.find(node);
    if (it != nodeClusterMap.end())
        nodeClusterMap.erase(it);
    nodeClusterMap.insert({ node, clusters });
...
EditSSMode*	EditSplineMod::editSSMode = NULL;
ActionCB* EditSplineMod::mpActions = NULL;
SplineToolsActionCB* EditSplineMod::mpSplineToolsActions = NULL;
Tab<ActionItem*> EditSplineMod::mOverrides;
...
static GenSubObjType SOT_Vertex(NULL, EditSplineIcon::vertexStack, 0);
static GenSubObjType SOT_Segment(NULL, EditSplineIcon::segmentStack, 0);
static GenSubObjType SOT_Spline(NULL, EditSplineIcon::splineStack, 0);
...
{
public:
    void PopulateDynamicMenu(ICuiDynamicMenu* dynMenu, int actionId, HWND hWnd, const IPoint2& cursorPos) override;
    void DynamicMenuItemSelected(int actionId, int itemId) override;
    void SetMod(EditSplineMod *es) { this->es = es; }

private:
    EditSplineMod* es = nullptr;
};

void ESActionCallback::PopulateDynamicMenu(ICuiDynamicMenu* dynMenu, int actionId, HWND hWnd, const IPoint2& cursorPos)
{
   switch(es->GetSubobjectLevel())
   {
      case ES_VERTEX:
      {
         if(es->RememberVertThere(hWnd, cursorPos))
         {
            DWORD flags0 = 0, flags1 = 0, flags2 = 0, flags3 = 0, flags4 = 0;
            switch(es->rememberedData)
            {
               case KTYPE_AUTO:
                  flags1 |= ICuiDynamicMenu::kChecked;
                  flags0 |= ICuiDynamicMenu::kDisabled;
                  break;
               case KTYPE_CORNER:
                  flags2 |= ICuiDynamicMenu::kChecked;
                  flags0 |= ICuiDynamicMenu::kDisabled;
                  break;
               case KTYPE_BEZIER:
                  flags3 |= ICuiDynamicMenu::kChecked;
                  break;
               case KTYPE_BEZIER_CORNER:
                  flags4 |= ICuiDynamicMenu::kChecked;
                  break;
               }
            // CAL-02/19/03: add reset tangent. (FID #827)
            dynMenu->AddItem(KTYPE_RESET, GetString(IDS_TH_RESET_TANGENTS), flags0);
            dynMenu->AddItem(KTYPE_AUTO, GetString(IDS_TH_SMOOTH)), flags1;
            dynMenu->AddItem(KTYPE_CORNER, GetString(IDS_TH_CORNER), flags2);
            dynMenu->AddItem(KTYPE_BEZIER, GetString(IDS_TH_BEZIER), flags3);
            dynMenu->AddItem(KTYPE_BEZIER_CORNER, GetString(IDS_TH_BEZIERCORNER), flags4);
      ...
      case ES_SEGMENT:
      {
         if(es->RememberSegThere(hWnd, cursorPos))
         {
            DWORD flags1 = 0, flags2 = 0;
            switch(es->rememberedData)
            {
               case LTYPE_CURVE:
                  flags1 |= ICuiDynamicMenu::kChecked;
            ...
            dynMenu->AddItem(LTYPE_CURVE, GetString(IDS_TH_CURVE), flags1);
            dynMenu->AddItem(LTYPE_LINE, GetString(IDS_TH_LINE), flags2);
            dynMenu->AddSeparator();
         }
         break;
      }
      case ES_SPLINE:
      {
         if(es->RememberPolyThere(hWnd, cursorPos))
         {
            DWORD flags1 = 0, flags2 = 0;
            switch(es->rememberedData)
            {
...
void ESActionCallback::DynamicMenuItemSelected(int actionId, int itemId)
{
    switch (es->GetSubobjectLevel())
    {
    case ES_VERTEX:
        es->SetRememberedVertType(itemId);
        break;
    case ES_SEGMENT:
        es->SetRememberedSegType(itemId);
        break;
    case ES_SPLINE:
        es->SetRememberedPolyType(itemId);
        break;
    }
}
...
// This function applies the current changes to driven handles and their knots, and zeroes everything else
void ShapeVertexDelta::ApplyHandlesAndZero(BezierShape &shape, int handlePoly, int handleVert) {
   ...
   // Apply the deltas  to just the driven handles
   for(int poly = 0; poly < shape.splineCount; ++poly) {
      if(dtab.pUsed[poly]) {
         Spline3D *spline = shape.splines[poly];
         int verts = spline->Verts();
...
void EditSplineData::Apply(TimeValue t,SplineShape *splShape,int selLevel,BOOL showVertNumbers,BOOL SVNSelectedOnly)
   ...
   if ( !GetFlag(ESD_UPDATING_CACHE) && tempData && tempData->ShapeCached(t) ) {
      splShape->shape.DeepCopy( 
         tempData->GetShape(t),
         GEOM_CHANNEL|SELECT_CHANNEL|SUBSEL_TYPE_CHANNEL|DISP_ATTRIB_CHANNEL|TOPO_CHANNEL );     
   ...
   if ( GetFlag(ESD_HASDATA) ) {
      ...
      if(count) {
//DebugPrint(_T("*** Applying old style (%d) ***\n"), count);
...
//DebugPrint(_T("<<<Storing Initial Data>>>\n"));
      mapper.Build(splShape->shape);
      finalShape = splShape->shape;
      }
...
//          shape->dispFlags &= ~DISP_SELSEGMENTS;
   ...
   if ( GetFlag(ESD_UPDATING_CACHE) ) {
      assert(tempData);
//DebugPrint(_T("*** Updating edit spline cache ***\n"));
      tempData->UpdateCache(splShape);
      SetFlag(ESD_UPDATING_CACHE,FALSE);
      }     
   }

void EditSplineData::Invalidate(ChannelMask channels,BOOL shapeValid)
   {
   if ( tempData ) {
      tempData->Invalidate(channels,shapeValid);
      }
   }
...
#ifdef VMAP_DEBUG
   DebugPrint(_T("Updating/Applying Deltas...\n"));
#endif
...
#ifdef VMAP_DEBUG
   DebugPrint(_T("Applying Deltas Complete!\n"));
#endif
   }
...
void ESVertMapper::RecomputeDeltas(BOOL checkTopology, BezierShape &shape, BezierShape &oldShape) {
#ifdef VMAP_DEBUG
   DebugPrint(_T("Recomputing Deltas...\n"));
#endif
   ...
   delete [] indexes;         
#ifdef VMAP_DEBUG
   DebugPrint(_T("Recomputing Deltas Complete!\n"));
#endif
   }
...
//DebugPrint(_T("Preupdating changes\n"));
...
void DoPolyEndAttach(BezierShape *shape, int poly1, int knot1, int poly2, int knot2, BOOL r3Way=TRUE) {
   ...
   if(poly1 == poly2) {
      ...
      BOOL bothAuto = (spline->GetKnotType(knot1) == KTYPE_AUTO && spline->GetKnotType(knot2) == KTYPE_AUTO) ? TRUE : FALSE;
      Point3 p1 = spline->GetKnotPoint(0);
      Point3 p2 = spline->GetKnotPoint(lastKnot);
      // Respect the ordering -- 'knot1' gets welded to 'knot2' which should remain fixed
      if (knot1 == 0)
      {
          // Welding first to last

          // Repoint the knot
          if (!bothAuto) {
              Point3 p1Delta = p2 - p1;
              spline->SetKnotType(lastKnot, KTYPE_BEZIER_CORNER);
              // Compute a reasonable vector
              if (spline->GetKnotType(knot1) & KTYPE_BEZIER)
                  spline->SetOutVec(lastKnot, spline->GetOutVec(0) + p1Delta);
              else {
                  Point3 nextInVectorPoint = spline->GetInVec(1);
                  // Max computes these vectors as halfway to the vector
                  spline->SetOutVec(lastKnot, p2 + ((nextInVectorPoint - p2) * 0.5f));
              }
          }
          spline->DeleteKnot(0);
          // Adjust selection and make the selection sets the right size
          vsel.Shift(LEFT_BITSHIFT, 3);
          vsel.SetSize(spline->Verts(), 1);
          ssel.Shift(LEFT_BITSHIFT, 1);
          ssel.SetSize(spline->Segments(), 1);
      ...
      {
          // Welding last to first

          // Repoint the knot
          if (!bothAuto) {
              Point3 p2Delta = p1 - p2;
              spline->SetKnotType(0, KTYPE_BEZIER_CORNER);
              // Compute a reasonable vector
              if (spline->GetKnotType(knot1) & KTYPE_BEZIER)
                  spline->SetInVec(0, spline->GetInVec(lastKnot) + p2Delta);
              else {
                  Point3 previousOutVectorPoint = spline->GetOutVec(lastKnot - 1);
                  // Max computes these vectors as halfway to the vector
                  spline->SetInVec(0, p2 + ((previousOutVectorPoint - p2) * 0.5f));
              }
          }
      ...
      GetCOREInterface()->DisplayTempPrompt(GetString(IDS_SPLINE_CLOSED), 3000);
      }
   else {      // Connecting two different splines -- Splice 'em together!
      Spline3D *spline2 = shape->splines[poly2];
      BOOL bothAuto = (spline->GetKnotType(knot1) == KTYPE_AUTO && spline2->GetKnotType(knot2) == KTYPE_AUTO) ? TRUE : FALSE;
      ...
      if(!bothAuto) {
          Point3 p1 = spline->GetKnotPoint(lastKnot);
          Point3 p2 = spline2->GetKnotPoint(0);
          Point3 p2Delta = p2 - p1;
          spline2->SetKnotType(0, KTYPE_BEZIER_CORNER);
          // Compute a reasonable vector
          if (spline->GetKnotType(knot1) & KTYPE_BEZIER)
              spline2->SetInVec(0, spline->GetInVec(lastKnot) + p2Delta);
          else {
              Point3 previousOutVectorPoint = spline->GetOutVec(lastKnot - 1);
              // Max computes these vectors as halfway to the previous out-vector
              spline2->SetInVec(0, p2 + ((previousOutVectorPoint - p2) * 0.5f));
          }
      }
      spline->DeleteKnot(lastKnot);
      ...
      GetCOREInterface()->DisplayTempPrompt(GetString(IDS_ENDPOINTS_WELDED), 3000);
      }
   }
...
FilletOrChamferSpline(BezierShape *shape, int poly, float size, int which) {
   ...
   for(int i = outSpline.KnotCount()-1; i >= 0; --i) {
      if(map[i]) {
         ...
         bool prevLinear = IsSplineSegmentEffectivelyLinear(&outSpline, prev);
         bool nextLinear = IsSplineSegmentEffectivelyLinear(&outSpline, i);

         // 2024.1: Spine3D::RefineSegment forces LTYPE_LINEAR segments to be LTYPE_CURVE and changes the endpoint knot types,
         // which is no longer appropriate for our treatment. Stash the line types and make both curve type for this operation.
         int thisLineType = outSpline.GetLineType(i);
         bool thisWasEffectivelyLinear = IsSplineSegmentEffectivelyLinear(&outSpline, i);
         outSpline.SetLineType(i, LTYPE_CURVE);
         int prevLineType = outSpline.GetLineType(prev);
         bool prevWasEffectivelyLinear = IsSplineSegmentEffectivelyLinear(&outSpline, prev);
         outSpline.SetLineType(prev, LTYPE_CURVE);

         // Now refine the segments
         ...
         // Restore the line types, converting "effectively linear" segments into linears
         outSpline.SetLineType(prev, prevWasEffectivelyLinear ? LTYPE_LINE : prevLineType);
         outSpline.SetLineType(k2, thisWasEffectivelyLinear ? LTYPE_LINE : thisLineType);

         if(which == ES_CHAMFER) {
            outSpline.SetKnotType(k1, prevLinear ? KTYPE_CORNER : KTYPE_BEZIER_CORNER);
            outSpline.SetKnotType(k2, nextLinear ? KTYPE_CORNER : KTYPE_BEZIER_CORNER);
            outSpline.SetLineType(k1, LTYPE_LINE);
         }
   ...
   *inSpline = outSpline;
   // Adjust selection data for this spline (clear it as Editable does, otherwise the selection was getting fouled up)
   shape->vertSel[poly].SetSize(outSpline.Verts(),FALSE);
   shape->segSel[poly].SetSize(outSpline.Segments(),FALSE);
//3-31-99 watje
// shape->UpdateBindList();
   shape->InvalidateGeomCache();
   }
...
static void RefineSegment(BezierShape *shape, int poly, int segment, float param, BOOL r3Style, bool makeLinearSegmentsBeziers) {
   Spline3D *spline = shape->splines[poly];
   int knots = spline->KnotCount();
   int verts = spline->Verts();
   int segs = spline->Segments();
   ...
   // Special: If they're refining a line-type segment, compute linear Bezier vectors for it
   if(makeLinearSegmentsBeziers && spline->GetLineType(segment) == LTYPE_LINE) {
   ...
   bool makeAdjacentSegmentsLinear = false;
   if (r3Style) {
       // 2024.1: If the segment is EFFECTIVELY linear, make the inserted knot a corner
       if (IsSplineSegmentEffectivelyLinear(spline, segment))
       {
           newType = KTYPE_CORNER;
           makeAdjacentSegmentsLinear = true;
       }
       else
       {
           if (type1 == KTYPE_CORNER && type2 == KTYPE_CORNER)
               newType = KTYPE_CORNER;
           else if ((type1 & KTYPE_CORNER) || (type2 & KTYPE_CORNER))
               newType = KTYPE_BEZIER_CORNER;
           else if (type1 == KTYPE_AUTO && type2 == KTYPE_AUTO)
               newType = KTYPE_AUTO;
       }
   }

   if (makeAdjacentSegmentsLinear)
       spline->SetLineType(segment, LTYPE_LINE);

   SplineKnot newKnot(newType, makeAdjacentSegmentsLinear ? LTYPE_LINE : LTYPE_CURVE, v03, v02, v12);
...
BOOL SegRefineRecord::Redo(BezierShape *shape,int reRecord) {
   ...
   if(segment >= shape->splines[poly]->Segments())
      return FALSE;
   RefineSegment(shape, poly, segment, param, FALSE, true);
   return TRUE;
   }
...
//#define BEGINNING 0  //az:  090704, commenting out, had no use, conflicting with MXS macros
//#define END 1
//#define FORWARD 1
//#define REVERSE -1
...
static int IntSeg(Point2& p1, Point2& p2, Point2& p3, Point2& p4, Point2& icpt) {
   float C1,C2,DENOM;
   float test1,test2;
   int sgnx1 = 0, sgny1 = 0, sgnx2 = 0, sgny2 = 0, hitend = 0;
   ...
   // Qilin.Ren April 28th, 2009  Defect #1214862
   // Add an epsilon value because the float multiplication above introduces 
   // small errors. 
	float epsilon = FLT_EPSILON;
	float l1 = Length(p2-p1);
	float l2 = Length(p3-p4);
	float lmin = std::min(l1, l2);
	epsilon = (float)((double)lmin * 0.001);

	if(p1.x < p2.x)
	 {
	 if(icpt.x < p1.x - epsilon || icpt.x > p2.x + epsilon)
	  return(0);
	 }
	else
	 {
	 if(icpt.x < p2.x - epsilon || icpt.x > p1.x + epsilon)
	  return(0);
	 }
	if(p1.y < p2.y)
	 {
	 if(icpt.y < p1.y - epsilon || icpt.y > p2.y + epsilon)
	  return(0);
	 }
	else
	 {
	 if(icpt.y < p2.y - epsilon || icpt.y > p1.y + epsilon)
	  return(0);
	 }
	...
	if(p3.x < p4.x)
	 {
	 if(icpt.x < p3.x - epsilon || icpt.x > p4.x + epsilon)
	  return(0);
	 }
	else
	 {
	 if(icpt.x < p4.x - epsilon || icpt.x > p3.x + epsilon)
	  return(0);
	 }
	if(p3.y < p4.y)
	 {
	 if(icpt.y < p3.y - epsilon || icpt.y > p4.y + epsilon)
	  return(0);
	 }
	else
	 {
	 if(icpt.y < p4.y - epsilon || icpt.y > p3.y + epsilon)
	  return(0);
	 }
...
static int FindIntercept(const Point2& p1, const Point2& p2, const Point2& p3, const Point2& p4, Point2& icpt) {
   float C1,C2,DENOM;
   Point2 d1,d2;
   float test1,test2;
...
//    BOOL Orient();
      ...
      BOOL SurroundsPoint(const Point2& point);
   };
...
BOOL ESMTemplate::SurroundsPoint(const Point2& point) {
...
//DebugPrint(_T("Point:%.2f %.2f Point2:%.2f %.2f\n"),point.x,point.y,point2.x,point2.y);
      hits=0;
...
//DebugPrint(_T("Starting FlagInside\n"));
   int segs = spline->Segments();
...
class BoolHit {
   public:
      ...
      void SetForcePoint(BoolHitData& h, const Point3& force);
   };
...
void BoolHit::SetForcePoint(BoolHitData& h, const Point3& force) {
   h.gotForce = TRUE;
   h.force = force;
//DebugPrint(_T("Set force for id# %d to %.4f %.4f\n"),h.id,force.x, force.y);
   }
...
//DebugPrint(_T("Hit %d on poly %d, seg:%d at %.4f %.4f was %.4f percent\n"),location,poly,seg,where.x,where.y,pct);
//Point3 check = spline->InterpBezier3D(seg,pct);
//DebugPrint(_T("Cross-check: %.4f %.4f\n"),check.x,check.y);
...
void BoolHit::Sort() {
   BoolHitData temp;
   unsigned int srtcnt;
   unsigned int srt1,srt2,srt3,srt4;
...
//DebugPrint(_T("Welded 4!\n"));
   ...
   int polys = shape->splineCount;
//DebugPrint(_T("Boolean resulted in %d polys\n"),polys);
   for(i = 0; i < polys; ++i) {
      ...
      if(!(first == last)) {
//DebugPrint(_T("First:%.8f %.8f Last:%.8f %.8f\n"),first.x, first.y, last.x, last.y);
         return FALSE;
         }
      spline->SetKnotType(0, KTYPE_BEZIER_CORNER);
...
static void MakeBezierKnots(Spline3D* spline)
{
    spline->ComputeBezPoints();
    for (int j = 0; j < spline->KnotCount(); ++j)
    {
        switch (spline->GetKnotType(j))
        {
        case KTYPE_AUTO:
            spline->SetKnotType(j, KTYPE_BEZIER);
            break;
        case KTYPE_CORNER:
            spline->SetKnotType(j, KTYPE_BEZIER_CORNER);
            break;
        default:
            // Other types are beziers already, leave as is
            break;
        }
    }
}
...
static int PerformBoolean(BezierShape *shape, int poly1, int poly2, int op, int *newPolyNum) {
   ...
   // For Boolean operations to function properly, all knots in the shapes must be Bezier or Bezier Corner
   MakeBezierKnots(spline1);
   MakeBezierKnots(spline2);
   ...
   *work[1] = *spline2;
   ...
   for(i = 0; i < hit.count; ++i)
      ...
      if(h.pct > 0.999) {
         ...
         RefineSegment(&workShape, h.poly, seg1, h.pct, FALSE, true);
         if(hit.GotForcePoint(h, &force))
   ...
   // 2024.1: Go through the work shape and convert any knots to corner type if the
   // adjacent segments are effectively linear, also convert segments to linear type if the segment is effectively linear
   int numPolys = workShape.SplineCount();
   for (int i = 0; i < numPolys; ++i)
   {
       Spline3D* spline = workShape.splines[i];
       int numKnots = spline->KnotCount();
       int numSegments = spline->Segments();
       int lastKnot = numKnots - 1;
       BOOL closed = spline->Closed();
       bool closureSegmentLinear = true;
       if (closed)
           closureSegmentLinear = IsSplineSegmentEffectivelyLinear(spline, lastKnot);
       for (int k = 0; k < numKnots; ++k)
       {
           bool previousSegmentLinear = closureSegmentLinear;
           if (k > 0)
               previousSegmentLinear = IsSplineSegmentEffectivelyLinear(spline, k - 1);
           bool nextSegmentLinear = closureSegmentLinear;
           if (k != lastKnot)
               nextSegmentLinear = IsSplineSegmentEffectivelyLinear(spline, k);
           // if segments on both sides of this knot are linear, make the knot itself a corner
           if (previousSegmentLinear && nextSegmentLinear)
               spline->SetKnotType(k, KTYPE_CORNER);
       }
       // Now convert any 'effectively' linear segments to linear type
       for (int s = 0; s < numSegments; ++s)
       {
           if (IsSplineSegmentEffectivelyLinear(spline, s))
               spline->SetLineType(s, LTYPE_LINE);
       }
   }
...
static void ChangeVertexType(BezierShape *shape, int poly, int vertex, int type) {
   ...
   // 2023.1: If knot type is changing to one that requires smooth adjacent segments, do that as well
   bool needSmoothSegs = (type == KTYPE_BEZIER || type == KTYPE_BEZIER_CORNER || type == KTYPE_AUTO);
   int knots = spline->KnotCount();
   ...
   if(vertex >= 0) {
      ...
      // If this knot needs smooth adjacent segments, do that now
      if (needSmoothSegs)
      {
          // Do previous segment
          if (vertex > 0 || spline->Closed())
          {
              int prevSeg = (vertex + knots - 1) % knots;
              spline->SetLineType(prevSeg, LTYPE_CURVE);
          }
          // Do next segment
          spline->SetLineType(vertex, LTYPE_CURVE);
      }
      spline->ComputeBezPoints();
      shape->InvalidateGeomCache();
      return;
   ...
   for(int i = 0; i < knots; ++i) {
       if (vsel[i * 3 + 1]) {
           ...
           if (needSmoothSegs)
           {
               ...
               {
                   int prevSeg = (i + knots - 1) % knots;
                   spline->SetLineType(prevSeg, LTYPE_CURVE);
               }
               // Do next segment
               spline->SetLineType(i, LTYPE_CURVE);
           }
       }
   }
   spline->ComputeBezPoints();
...
void ShapeRestore::Restore(int isUndo)
   ...
   if ( esd->tempData && esd->TempData(mod)->ShapeCached(t) ) {
      ...
      esd->TempData(mod)->Invalidate(GEOM_CHANNEL | TOPO_CHANNEL | SELECT_CHANNEL);
      }
   else
   if ( esd->tempData ) {
      esd->TempData(mod)->Invalidate(GEOM_CHANNEL | TOPO_CHANNEL | SELECT_CHANNEL, FALSE);
      }
   if(isUndo)
...
int EditSplineMod::FindSet(const TSTR& setName, int level)
   {  
...
void EditSplineMod::AddSet(const TSTR& setName, int level)
   {
   assert(level>0 && level<4);
...
void EditSplineMod::RemoveSet(const TSTR& setName, int level)
   {
   assert(level>0 && level<4);
...
void EditSplineMod::ActivateSubSelSet(TSTR &setName)
   ...
   for (int i = 0; i < mcList.Count(); i++) {
      ...
      if (shapeData->tempData)
         shapeData->TempData(this)->Invalidate(SELECT_CHANNEL);
      }
...
void EditSplineMod::NewSetByOperator(TSTR &newName,Tab<int> &sets,int op)
   {
   // First do it in the main name list
   if(theHold.Holding())
...
const ActionTableId kActionID = 0x29760f19;
const ActionTableId kSplineToolsActionID = 0x66134e15;

BOOL ActionCB::ExecuteAction(int id) {
    switch (id) {
    case ID_EDIT_SS:
...
static ActionDescription actions[] = {
    {
        ...
        IDS_EDIT_SS,
        IDS_EDIT_SS,
        IDS_TH_EDITSPLINE_CLASS
    }
};

ActionTable* GetActions() {
	TSTR name = GetString (IDS_TH_EDITSPLINE_CLASS);
	...
	pTab = new ActionTable (kActionID, kActionID,
		name, hAccel, _countof(actions), actions, hInstance);
	GetCOREInterface()->GetActionManager()->RegisterActionContext(kActionID, name.data());

	IActionItemOverrideManager *actionOverrideMan = 

		static_cast<IActionItemOverrideManager*>(GetCOREInterface(IACTIONITEMOVERRIDEMANAGER_INTERFACE ));
...
BOOL SplineToolsActionCB::ExecuteAction(int id) {
    switch (id) {
    case ID_SPLINE_SELECT_SIMILAR:
        theHold.Begin();
        bool changed = mpMod->SelectSimilar();
        if (changed)
        {
            theHold.Accept(GetString(IDS_TH_SPLINE_SELECT_SIMILAR));
...
static ActionDescription splineToolsActions[] = {
    {
        ID_SPLINE_SELECT_SIMILAR,
        IDS_TH_SPLINE_SELECT_SIMILAR,
        IDS_TH_SPLINE_SELECT_SIMILAR,
        IDS_SPLINE_TOOLS,
    }
};

ActionTable* GetSplineToolsActions() {
    TSTR name = GetString(IDS_SPLINE_TOOLS);
    HACCEL hAccel = CreateAcceleratorTable(NULL, 0);
    ActionTable* pTab = NULL;
    pTab = new ActionTable(kSplineToolsActionID, kSplineToolsActionID,
        name, hAccel, _countof(splineToolsActions), splineToolsActions, hInstance);
    GetCOREInterface()->GetActionManager()->RegisterActionContext(kSplineToolsActionID, name.data());
...
void EditSplineMod::NSPaste()
   ...
   ShapeNamedVertSelClip *vclip = NULL;
   ShapeNamedSegSelClip  *sclip = NULL;
   ShapeNamedPolySelClip *pclip = NULL;
...
void EditSplineMod::EndOutlineMove(TimeValue t,BOOL accept)
   ...
   if (accept) {
      ...
      for ( int i = 0; i < mcList.Count(); i++ ) {
         ...
         shapeData->TempData(this)->Invalidate(GEOM_CHANNEL);
...
void EditSplineMod::OutlineMove( TimeValue t, float amount ) 
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      {
         shapeData->UpdateChanges(shape);
         shapeData->TempData(this)->Invalidate(TOPO_CHANNEL);
      }
      // mjm - end
...
//DebugPrint(_T("Self-int %d/%d: %.4f %.4f - %.4f %.4f / %.4f %.4f - %.4f %.4f @ %.4f %.4f\n"),i,j,i1.x,i1.y,i2.x,i2.y,
...
HCURSOR PickSplineAttach::GetHitCursor(IObjParam *ip) {
   return UI::MouseCursors::LoadMouseCursor(UI::MouseCursors::Attach);
   }
...
int EditSplineMod::DoAttach(INode *node, bool & canUndo) {
   ...
   shapeData->TempData(this)->Invalidate(GEOM_CHANNEL | TOPO_CHANNEL);
...
EditSplineMod::EditSplineMod()
   {
   version = EDITSPLINEMOD_LATEST_VERSION;
...
void EditSplineMod::XFormHandles( 
   ...
   // Loop thru the objects, doing the main object first
   // If handles aren't locked, we only need to do the main object!
   int objects = lockedHandles ? mcList.Count() : 1;
   for ( int i = 0, object = mainObject; i < objects; i++, object = (object + 1) % objects ) {
      ...
      if(!TestAFlag(A_HELD)) {
         ...
         if(shapeData->DoingHandles())
            shapeData->ApplyHandlesAndZero(*shape);      // Reapply the driven handle deltas
         else
            shapeData->vdelta.Zero();
         }

      // If the main shape, move the handle!
      if(object == mainObject) {
         mainKnot = shape->bezVecVert / 3;
      ...
      if(lockedHandles) {
         ...
         for(int poly = 0; poly < shape->splineCount; ++poly) {
            ...
            for ( int k = 0; k < knots; k++ ) {
               int kvert = k*3+1;
               if(object == mainObject && poly == shape->bezVecPoly && kvert == shape->bezVecVert) {
...
void EditSplineMod::XFormVerts( 
   ...
   Interval valid;
   int numAxis;
   Point3 oldpt,newpt,oldin,oldout,rel,delta, newin;
      ...
      if (numAxis==NUMAXIS_INDIVIDUAL) {
         switch(selLevel) {
            case ES_VERTEX: {
               ...
               for(int poly = 0; poly < shape->splineCount; ++poly) {
                  ...
                  for ( int k = 0; k < knots; k++, vbase+=3 ) {
                     ...
                     if ((k==0) && (!spline->Closed()))
                        {
                        for (int bct = 0; bct < shape->bindList.length(); bct++)
                           {
               ...
               break;
            case ES_SEGMENT: {
                // First, transform any spline knots added as part of the "Connect Copy" mechanism
                int vbase = 1;
                int baseWeight = 0;
                for (int poly = 0; poly < shape->splineCount; ++poly) {
                    shapeData->vdelta.ClearUsed(poly);
                    // Get an empty selection set for this poly
                    BitArray sel = shape->VertexTempSel(poly);
                    sel.ClearAll();
                    Spline3D* spline = shape->splines[poly];
                    int knots = spline->KnotCount();
                ...
                // Now handle the selected segment clusters
                auto clusterData = nodeClusterMap.find(nodes[i]);
                DbgAssert(clusterData != nodeClusterMap.end());
                if (clusterData == nodeClusterMap.end()) continue;
                const ClusterVector& clusters = clusterData->second;

                int numClusters = (int)clusters.size();
                baseWeight = 0;
                std::set<Spline3D*> alteredSplines;
                for (int clusterIndex = 0; clusterIndex < numClusters; ++clusterIndex)
                {
                    const ClusterCenterPolyStartknotEndknot& cluster = clusters[clusterIndex];
                    int poly = std::get<ClusterPoly>(cluster);
                    // Selected vertices - either directly or indirectly through selected faces or edges.
                    BitArray sel = shape->VertexTempSel(poly);
                    int startKnot = std::get<ClusterStartKnot>(cluster);
                    int endKnot = std::get<ClusterEndKnot>(cluster);
                    Spline3D* spline = shape->splines[poly];
                    int numKnots = spline->KnotCount();
                    for (int k = startKnot; ; k = (k + 1) % numKnots)
                    {
                        int vert = k * 3 + 1;
                        //watje 5-20-99
                        BOOL bound = FALSE;
                        ...
                        else if ((k == (numKnots - 1)) && (!spline->Closed()))
                        {
                            for (int bct = 0; bct < shape->bindList.length(); bct++)
                            {
                                if ((shape->bindList[bct].pointSplineIndex == poly) && (shape->bindList[bct].isEnd))
                        ...
                        if ((fabs(vertexWeight) > 0.0001) && (!bound)) {
                            alteredSplines.insert(spline);
                            tmAxis = ip->GetTransformAxis(nodes[i], clusterIndex);
                            mat = nodes[i]->GetObjectTM(t, &valid) * Inverse(tmAxis);
                            imat = Inverse(mat);
                            xproc->SetMat(mat);
                            ...
                            // XForm the cache vertices
                            oldpt = spline->GetVert(vert);
                            newpt = xproc->proc(oldpt, mat, imat, 1.0f);
                            spline->SetVert(vert, newpt);
                            delta = newpt - oldpt;
                            ...
                            if (spline->IsBezierPt(k)) {
                                ...
                                oldout = spline->GetVert(out);
                                float outWeight = shape->VertexWeight(baseWeight + out);
                                if (vertexWeight > .9999f) outWeight = 1.0f;
                                spline->SetVert(out, xproc->proc(oldout, mat, imat, outWeight));
                                // Move the delta's vertices.
                                shapeData->vdelta.Move(poly, out, spline->GetVert(out) - oldout);
                            }
                        ...
                        // When we do the last knot, we're done
                        if (k == endKnot) break;
                    }
                }

                // Prep each altered spline
                for (Spline3D* spline : alteredSplines)
                {
                    spline->ComputeBezPoints();
                }
                if (alteredSplines.size())
                    shape->InvalidateGeomCache();
            }
               break;
            case ES_SPLINE: {
...
public:
    AutoConnectPrompt();
    AutoConnectPrompt(bool usingAutoConnect, bool useConfirmationPopUp, float threshold, int promptStringId = IDS_TH_CONNECT_COINCIDENT);
    BOOL DoIt(ViewExp* vpt, Matrix3& tm, Point3 p1, Point3 p2, float& distanceOut);
    bool WasUserPrompted() { return mWasPrompted; }  // After DoIt is called, this tells whether or not the user was actually prompted
private:
    int mPromptStringId;
	BOOL mPrompted;
	BOOL mDoIt;
...
{
	mPromptStringId = IDS_TH_CONNECT_COINCIDENT;
	mPrompted = FALSE;
	mDoIt = FALSE;
...
AutoConnectPrompt::AutoConnectPrompt(bool tryToAutoConnect, bool useConfirmationPopUp, float threshold, int promptStringId)
{
   mPromptStringId = promptStringId;
   mPrompted = FALSE; 
   mDoIt = FALSE;
...
BOOL AutoConnectPrompt::DoIt(ViewExp* vpt, Matrix3& tm, Point3 p1, Point3 p2, float& distanceOut)
{
    mWasPrompted = false;
	if (!vpt || !vpt->IsAlive())
	{
	...
	if (!mTryToAutoConnect)
		return FALSE;
    distanceOut = Length(sp1 - sp2);
	if (distanceOut >= mThreshold)
		return FALSE;
	if (mPrompted)
		return mDoIt;
	int result = IDYES;
    if (mUseConfirmation)
    {
        TSTR s1 = GetString(mPromptStringId);
        TSTR s2 = GetString(IDS_TH_EDITSPLINE);
        result = MaxSDK::MaxMessageBox(GetActiveWindow(), s1, s2, MB_YESNO);
        mWasPrompted = true;
    }
...
// This vector contains indexes of segments that need a post-transform process to
// turn them into Smooth segments with a linear appearance, one set of segments per
// spline being edited. Each entry consists of a polygon index and segment index
// within that polygon.
typedef EditSplineData* SplineData;
typedef int PolygonIndex;
typedef int SegmentIndex;

static std::vector<std::pair<SplineData, std::vector<std::pair<PolygonIndex, SegmentIndex>>>> extrudePostProcessSegments;
...
void EditSplineMod::TransformStart(TimeValue t)
{
    ...
    // Reset the post-process segment vector
    extrudePostProcessSegments.clear();
	...
	if (shiftPressed)
	{
		bool doExtrude = (GetKeyState(VK_CONTROL) & 0x8000) == 0;
		// CAL-05/23/03: add supporting splines from original to the copy. (FID #827)
		// TH: Only if not doing an extrude
		BOOL addLinks = doExtrude ? FALSE : connectCopy;
        ...
        if (selLevel == ES_VERTEX && doExtrude)
        {
            ModContextList mcList;
            INodeTabAutoDispose nodes;
            ...
            for (int i = 0; i < mcList.Count(); i++)
            {
                ...
                if (!shapeData)
                    continue;
                if (shapeData->GetFlag(ESD_BEENDONE))
                    continue;

                // This collects the segments that need post-processing, organized by EditSplineData
                std::vector<std::pair<PolygonIndex, SegmentIndex>> segmentsToProcess;
                ...
                // Go thru all polygons -- If any end vertex is selected, extrude it
                int polys = shape->SplineCount();
                BOOL altered = FALSE;

                // Extruding simply adds linear segments at the selected end vertices
                for (int poly = 0; poly < polys; ++poly)
                {
                    Spline3D& spline = *shape->splines[poly];
                    int numKnots = spline.KnotCount();
                    ...
                    BitArray& vertSelection = shape->vertSel[poly];
                    if (vertSelection.AnyBitSet())
                    {
                        int lastKnot = numKnots - 1;

                        // Check for extrusions of non-endpoint vertices --
                        // they create new linear polys
                        int oldSplineCount = shape->SplineCount();
                        int startKnot = closed ? 0 : 1;
                        int endKnot = closed ? numKnots : lastKnot;
                        for (int k = startKnot; k < endKnot; ++k)
                        {
                            int vertIndex = k * 3 + 1;
                            if (vertSelection[vertIndex])
                            {
                                // Add post-process information
                                segmentsToProcess.push_back({ shape->SplineCount(), 0 });

                                Spline3D* newSpline = shape->NewSpline();
                                SplineKnot addKnot = spline.GetKnot(k);	// Get the source knot
                                addKnot.SetLtype(LTYPE_LINE);	// We will create a linear segment
                                addKnot.SetKtype(KTYPE_CORNER);	// Corner type knot
                                addKnot.SetAux2(0);	// Clobber topology tag recorded above
                                addKnot.SetInVec(addKnot.Knot());
                                addKnot.SetOutVec(addKnot.Knot());
                                // Add the knot twice (second will be selected below)
                                newSpline->AddKnot(addKnot, -1);
                                newSpline->AddKnot(addKnot, -1);
                                ...
                                // Deselect the source vertex and its vectors
                                shape->vertSel[poly].Clear(vertIndex - 1);
                                shape->vertSel[poly].Clear(vertIndex);
                                shape->vertSel[poly].Clear(vertIndex + 1);
                        ...
                        // Now deal with endpoints of open splines
                        if (!closed)
                        {
                            BitArray& segSelection = shape->segSel[poly];
                            bool firstVertSelected = !!vertSelection[1];
                            bool lastVertSelected = !!vertSelection[lastKnot * 3 + 1];
                            if (firstVertSelected || lastVertSelected)
                            {
                                // Handle last knot
                                if (lastVertSelected)
                                {
                                    // Convert last knot to bez corner at same location
                                    SplineKnot knot = spline.GetKnot(lastKnot);
                                    knot.SetKtype(KTYPE_BEZIER_CORNER);
                                    knot.SetOutVec(knot.Knot());
                                    knot.SetLtype(LTYPE_LINE);
                                    spline.SetKnot(lastKnot, knot);
                                    knot.SetKtype(KTYPE_CORNER);
                                    knot.SetInVec(knot.Knot());
                                    spline.AddKnot(knot);
                                }
                                // Handle first knot
                                if (firstVertSelected)
                                {
                                    // Convert first knot to bez corner at same location
                                    SplineKnot knot = spline.GetKnot(0);
                                    knot.SetKtype(KTYPE_BEZIER_CORNER);
                                    knot.SetInVec(knot.Knot());
                                    ...
                                    knot.SetLtype(LTYPE_LINE);
                                    knot.SetOutVec(knot.Knot());
                                    spline.AddKnot(knot, 0);
                                }
                                int newKnotCount = spline.KnotCount();
                                vertSelection.SetSize(newKnotCount * 3, FALSE);
                                int numSegments = spline.Segments();
                                segSelection.SetSize(numSegments, TRUE);
                                if (firstVertSelected)
                                {
                                    vertSelection.Shift(LEFT_BITSHIFT, 3);
                                    vertSelection.Set(1);
                                    segSelection.Shift(LEFT_BITSHIFT, 1);
                                    segSelection.Clear(0);

                                    // Add post-process information
                                    segmentsToProcess.push_back({ poly, 0 });
                                }
                                if (lastVertSelected)
                                {
                                    vertSelection.Set((newKnotCount - 1) * 3 + 1);
                                    segSelection.Clear(numSegments - 1);

                                    // Add post-process information
                                    segmentsToProcess.push_back({ poly, numSegments - 1 });
                                }
                        ...
                        // All done with this polygon; update the selections
                        int newSplineCount = shape->SplineCount();

                        // Create proper selections for the polys we created
                        if (newSplineCount > oldSplineCount)
                        {
                            shape->UpdateSels(TRUE); // ready selection set info
                            shape->InvalidateGeomCache();
                            for (int i = oldSplineCount; i < newSplineCount; ++i)
                            {
                                shape->segSel[i].ClearAll();
                                shape->vertSel[i].Set(4);	// Select the second vertex (1 * 3 + 1)
                            }
                        }
                    }
                }
                if (altered)
                {
                    ...
                    // Save the segments that need post-processing
                    extrudePostProcessSegments.push_back({ shapeData, segmentsToProcess });
                }
            }
            if (needUndo)
        ...
        else if (selLevel == ES_SEGMENT)
		{
            // Clobber the node cluster map for segments
			...
			for (int i = 0; i < mcList.Count(); i++)
			{
                ...
                // Identify the segment clusters, if any, and process them
                if (GetSegmentClusters(nodes[i], *shape, Matrix3())) {
                    ...
                    // Go thru all polygons -- If any segs selected, copy or extrude 'em
                    int polys = shape->SplineCount();
                    BOOL altered = FALSE;

                    if (doExtrude)
                    {
                        // Extruding simply adds linear segments at the end of selected segment spans
                        for (int poly = 0; poly < polys; ++poly)
                        {
                            BitArray& segSelection = shape->segSel[poly];
                            BitArray& vertSelection = shape->vertSel[poly];
                            if (segSelection.AnyBitSet())
                            {
                                std::vector<int> spanStartKnots;
                                std::vector<int> spanEndKnots;
                                Spline3D& spline = *(shape->splines[poly]);
                                bool closed = (spline.Closed() == TRUE);
                                int numSegments = spline.Segments();
                                int numKnots = spline.KnotCount();
                                for (int seg = 0; seg < numSegments; ++seg)
                                {
                                    if (segSelection[seg])
                                    {
                                        int previousSegment = (seg == 0) ? (closed ? numSegments - 1 : -1) : seg - 1;
                                        int nextSegment = (seg == (numSegments - 1)) ? (closed ? 0 : -1) : seg + 1;
                                        if (previousSegment < 0 || (previousSegment >= 0 && segSelection[previousSegment] == 0))
                                        {
                                            spanStartKnots.push_back(seg);
                                        }
                                        if (nextSegment < 0)
                                        {
                                            spanEndKnots.push_back(seg + 1);
                                        }
                                        else if (nextSegment >= 0 && segSelection[nextSegment] == 0)
                                        {
                                            spanEndKnots.push_back(nextSegment);
                                        }
                                    }
                                }
                                // If there are any ends (must be pairs of starts/ends or we did something wrong), do the operation
                                int numStarts = (int)spanStartKnots.size();
                                int numEnds = (int)spanEndKnots.size();
                                DbgAssert(numStarts == numEnds);
                                if (numStarts && (numStarts == numEnds))
                                {
                                    // Sort the knot arrays
                                    std::sort(spanStartKnots.begin(), spanStartKnots.end());
                                    std::sort(spanEndKnots.begin(), spanEndKnots.end());
                                    int startIndex = 0;
                                    int endIndex = 0;
                                    int startKnot = spanStartKnots[startIndex];
                                    int endKnot = spanEndKnots[endIndex];
                                    // Get a spline to build the new version
                                    Spline3D newSpline;
                                    // And ready a new segment selection set
                                    int newKnotCount = numKnots + numStarts * 2;
                                    BitArray newSegSelection(newKnotCount);
                                    // And a new vertex selection set
                                    BitArray newVertSelection(newKnotCount * 3);
                                    // Go through the knots, and if a knot is a start or end, insert a connecting segment
                                    int destIndex = 0;
                                    for (int k = 0; k < numKnots; ++k)
                                    {
                                        bool vertexIsSelected = !!vertSelection[k * 3 + 1];
                                        SplineKnot knot = spline.GetKnot(k);
                                        if (k == startKnot)
                                        {
                                            // Start of a selected span

                                            // Add post-process information
                                            segmentsToProcess.push_back({ poly, destIndex });

                                            // If not a corner type, both knots need to be bezier corners
                                            // at identical locations. Alter it here.
                                            if (knot.Ktype() != KTYPE_CORNER)
                                                knot.SetKtype(KTYPE_BEZIER_CORNER);

                                            // Set the first knot as a copy
                                            SplineKnot knot2 = knot;
                                            // Linear connection to the next one
                                            knot2.SetLtype(LTYPE_LINE);
                                            knot2.SetAux2(0);	// Clobber topology tag recorded above
                                            knot2.SetOutVec(knot2.Knot());
                                            newSpline.AddKnot(knot2);
                                            newVertSelection.Set(destIndex * 3 + 1, vertexIsSelected ? TRUE : FALSE);
                                            destIndex++;

                                            // Now the original one
                                            knot.SetInVec(knot.Knot());
                                            newSpline.AddKnot(knot);
                                            // We know it was on a selected segment
                                            newSegSelection.Set(destIndex);
                                            newVertSelection.Set(destIndex * 3 + 1, vertexIsSelected ? TRUE : FALSE);
                                            destIndex++;

                                            // Advance to the next span-start index
                                            startIndex++;
                                            startKnot = (startIndex >= numStarts) ? -1 : spanStartKnots[startIndex];
                                        }
                                        else if (k == endKnot)
                                        {
                                            // End of a selected span

                                            // If original was not a corner, both knots need to be bezier corners at identical locations. Alter it here.
                                            if (knot.Ktype() != KTYPE_CORNER)
                                                knot.SetKtype(KTYPE_BEZIER_CORNER);
                                            ...
                                            // Now the original one
                                            knot.SetAux2(0);	// Clobber topology tag recorded above
                                            knot.SetInVec(knot.Knot());
                                            newSpline.AddKnot(knot);
                                            newVertSelection.Set(destIndex * 3 + 1, vertexIsSelected ? TRUE : FALSE);
                                            ...
                                            // Advance to the next span-end index
                                            endIndex++;
                                            endKnot = (endIndex >= numEnds) ? -1 : spanEndKnots[endIndex];
                                        }
                                        else
                                        {
                                            // A span knot; just copy it
                                            newSpline.AddKnot(knot);
                                            newVertSelection.Set(destIndex * 3 + 1, vertexIsSelected ? TRUE : FALSE);
                                            // set its segment selection from the original
                                            if (k < numSegments)
                                            {
                                                newSegSelection.Set(destIndex, segSelection[k]);
                                            }
                                            destIndex++;
                                        }
                                    }
                                    // Close the spline if the source spline was closed
                                    newSpline.SetClosed(spline.Closed());
                                    vertSelection = newVertSelection;
                                    segSelection = newSegSelection;
                                    spline = newSpline;
                                }
                    ...
                    else
                    {
                        ...
                        if (addLinks && linkToOld)
                        {
                            ...
                            // CAL-08/28/03: collect excludeEnds before calling CopySegments because the selection will be
                            // changed. (Defect #509802)
                            for (int poly = 0; poly < polys; ++poly)
                            {
                                if (shape->segSel[poly].NumberSet())
                                {
                                    ...
                                    if (shape->segSel[poly][0])
                                        excludeEnds.Set(2 * poly);
                                    if (shape->segSel[poly][spline->Segments() - 1])
                ...
                // If the splines changed, get new cluster info
                GetSegmentClusters(nodes[i], *shape, Matrix3());
			}
			nodes.DisposeTemporary();
			if (needUndo)
				theHold.Accept(doExtrude ? GetString(IDS_EXTRUDE) : GetString(IDS_TH_COPY_SEGMENT));
			else
				theHold.End(); // Forget it!
		}
		else if (selLevel == ES_SPLINE)
		{
			ModContextList mcList;
			INodeTab nodes;
			Interval valid;
...
void EditSplineMod::TransformFinish(TimeValue t) {
   ...
   // Check for special post-processing of extruded segments...
   for (const auto& shapeEntry : extrudePostProcessSegments)
   {
        EditSplineData* shapeData = shapeEntry.first;
        BezierShape* shape = shapeData->TempData(this)->GetShape(t);
        shapeData->BeginEdit(t);
        shapeData->PreUpdateChanges(shape);

        // Now process the segments
        for (const auto& fixSegment : shapeEntry.second)
        {
            int splineIndex = fixSegment.first; // .first is polygon
            if (splineIndex < shape->SplineCount())
            {
                // Assign reasonable vectors to produce a linear segment appearance
                // with a curve-type segment;
                Spline3D* spline = shape->splines[splineIndex];
                int knotCount = spline->KnotCount();
                int knot1 = fixSegment.second;                  // .second is segment
                int knot2 = (knot1 + 1) % knotCount;
                if (knot1 < knotCount && knot2 < knotCount)
                {
                    Point3 p1 = spline->GetKnotPoint(knot1);
                    Point3 p2 = spline->GetKnotPoint(knot2);
                    spline->SetOutVec(knot1, p1 + (p2 - p1) / 3.0f);
                    spline->SetInVec(knot2, p2 + (p1 - p2) / 3.0f);
                    // Connecting line is linear
                    spline->SetLineType(knot1, LTYPE_LINE);
                }
            }
        }
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      for(int poly = 0; poly < polys; ++poly) {
         ...
         if(!spline->Closed()) {
            ...
            if(knots > 2 && (pSel[1] || pSel[(knots-1)*3+1])) {
               ...
               // Check for closure
               bool doClose = thePrompt.DoIt(vpt.ToPointer(), nodeTM, p1, p2, distanceOut);
               // If the user wasn't prompted, and no closure was done, prompt the user for closure.
               // This allows auto-closure to work in all conditions, with or without prompting, and 
               // lets us prompt the user for closure when auto-closure isn't active.
               if (!doClose && !thePrompt.WasUserPrompted())
               {
                   AutoConnectPrompt thePrompt2(TRUE, TRUE, SplineShape::GetEndPointAutoWeldThreshold(), IDS_CLOSE_SPLINE_QUESTION);
                   doClose = thePrompt2.DoIt(vpt.ToPointer(), nodeTM, p1, p2, distanceOut);
               }
               if(doClose) {
                  // Start a restore object...
                  if ( theHold.Holding() )
      ...
      bool doTheWeld = false;
      int bestPoly1 = 0, bestVert1 = 0, bestPoly2 = 0, bestVert2 = 0;
            ...
            if(pSel[1] || pSel[lastSel1]) {
               ...
               for(int poly2 = 0; poly2 < polys; ++poly2) {
                  ...
                  if(poly1 != poly2 && !spline2->Closed()) {
                     ...
                     if(pSel[1]) {
                        if(thePrompt.DoIt(vpt.ToPointer(), nodeTM, p3, p1, distanceOut)) {
                            if (distanceOut < closestDistance) {
                                doTheWeld = true;
                                bestPoly1 = poly1;
                                bestVert1 = 0;
                                bestPoly2 = poly2;
                        ...
                        if(thePrompt.DoIt(vpt.ToPointer(), nodeTM, p4, p1, distanceOut)) {
                            if (distanceOut < closestDistance) {
                                doTheWeld = true;
                                bestPoly1 = poly1;
                                bestVert1 = 0;
                     ...
                     if(pSel[lastSel1]) {
                        if(thePrompt.DoIt(vpt.ToPointer(), nodeTM, p3, p2, distanceOut)) {
                            if (distanceOut < closestDistance) {
                                doTheWeld = true;
                                bestPoly1 = poly1;
                                bestVert1 = knots1 - 1;
                        ...
                        if(thePrompt.DoIt(vpt.ToPointer(), nodeTM, p4, p2, distanceOut)) {
                            if (distanceOut < closestDistance) {
                                doTheWeld = true;
                                bestPoly1 = poly1;
                                bestVert1 = knots1 - 1;
      ...
      if (doTheWeld) {
          ...
          if (theHold.Holding())
              theHold.Put(new ShapeRestore(shapeData, this, shape));
          // Do the attach
          DoPolyEndAttach(shape, bestPoly1, bestVert1, bestPoly2, bestVert2);
          shapeData->TempData(this)->Invalidate(GEOM_CHANNEL | TOPO_CHANNEL);
          altered = needUndo = TRUE;
          goto changed;
      }
...
//DebugPrint(_T("Transform cancel\n"));
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(shape == rshape) {
         ...
         if(iBestDist < 1000) {        
            ...
            RefineSegment(shape, poly, seg, bestParam, TRUE, false);
            shapeData->UpdateChanges(shape);
            shapeData->TempData(this)->Invalidate(TOPO_CHANNEL);
   ...
   for(int seg = segments - 1; seg >= 0; --seg) {
      if(shape->segSel[poly][seg]) {
         ...
         for(int div = divisions; div > 0; --div) {
            ...
            RefineSegment(shape, poly, seg, pos, TRUE, true);
            }
         }
      }
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(shape == rshape) {
         ...
         if(csp1.bestDist <= crossThreshold) {
            RefineSegment(shape, poly1, seg1, csp1.bestParam, TRUE, true);
            RefineSegment(shape, poly2, seg2, csp2.bestParam, TRUE, true);
            shapeData->UpdateChanges(shape);
            shapeData->TempData(this)->Invalidate(TOPO_CHANNEL);
...
void EditSplineMod::ModifyObject(TimeValue t, ModContext& mc, ObjectState* os, INode* node)
{
    ...
    BezierShape* pShape = &splShape->shape;

    // CAL-03/25/01: pass area selection parameters (turn into API calls in 4.y)
    BezierShapeInterface_Ex41* exInterface =
            (BezierShapeInterface_Ex41*)pShape->GetInterface(BEZIER_SHAPE_INTERFACE_EX41);
    if (exInterface)
    {
    ...
    // Always update the soft selection data
    ...
    // If using soft selections, update the vertex weights
...
void EditSplineMod::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc)
   {
   if ( mc->localData ) {
      EditSplineData *shapeData = (EditSplineData*)mc->localData;
      if ( shapeData ) {
...
void EditSplineMod::DoubleClickSelect(HitRecord *hitRec)
{
	if (!hitRec) return;
	if (!ip) return;
	ip->ClearCurNamedSelSet();
	...
	bool add = inMayaSelectionMode ? GetKeyState(VK_SHIFT) & 0x8000 : GetKeyState(VK_CONTROL) & 0x8000;
	bool subtract = inMayaSelectionMode ? (GetKeyState(VK_CONTROL) & 0x8000) : (GetKeyState(VK_MENU) & 0x8000);
	if (add) subtract = false;
...
//watje 
   ...
   while(hitRec) {   
      ...
      // Checking for vertex extrude -- if shift is pressed, restrict selection to knots only
            ...
            if ( all ) {
               ...
               if ( selected ) {
                  while( hitRec ) {
                     ...
                     if(shapeData != (EditSplineData*)hitRec->modContext->localData)
                        goto vert_done;
                     // If shift is pressed (vertex extrude), restrict selection to knots only
                     if (shiftPressed) {
                         int vert = ((ShapeHitData*)(hitRec->hitData))->index;
                         if ((vert % 3) != 1) continue;
                     }
               ...
               if ( selected ) {
                   // If shift is pressed (vertex extrude), restrict selection to knots only
                   if (shiftPressed && ((vert % 3) != 1)) continue;
                   shape->vertSel[poly].Set(vert);
                  }
               else {
                   // If shift is pressed (vertex extrude), restrict selection to knots only
...
//watje
      ...
      shapeData->UpdateChanges(shape, FALSE);
      if ( shapeData->tempData ) {
         shapeData->tempData->Invalidate(SELECT_CHANNEL);
         }
      }
...
void EditSplineMod::MaybeSelectSingleSpline(BOOL makeUndo) {
   ...
   if(theShape) {
      ...
      if ( theESD->tempData ) {
         theESD->TempData(this)->Invalidate(SELECT_CHANNEL);
         }
      }
   nodes.DisposeTemporary();
   }
...
void EditSplineMod::SetFCLimit() {
   ...
   ClearShapeDataFlag(mcList,ESD_BEENDONE);
   FCLimit = min.length;
// DebugPrint(_T("Fillet/Chamfer limit:%f\n"),min.length);
   }
...
static INT_PTR CALLBACK SplineSurfDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam);
...
void EditSplineMod::ActivateSubobjSel(int level, XFormModes& modes )
   ...
   nodes.DisposeTemporary();
   // The following is necessary to prevent any of our rollups' edit controls from getting the focus -- This
   // was causing subobject level changes using keyboard shortcuts from working properly: You could enter the
   // segment or spline subobject level by pressing '2' or '3' but pressing those keys again to exit back to
   // object level didn't do that -- it placed those keystrokes in the Surface Parameters material ID edit field.
   ...
   if ( mc->localData ) {
      ...
      switch ( selLevel ) {
         ...
         case ES_SEGMENT: {
             Matrix3 otm = node->GetObjectTM(t, &valid);
             Matrix3 tm = node->GetNodeTM(t, &valid);
             if (GetSegmentClusters(node, *shape, tm))
             {
                 // Get the cluster information
                 auto clusterData = nodeClusterMap.find(node);
                 DbgAssert(clusterData != nodeClusterMap.end());
                 if (clusterData != nodeClusterMap.end())
                 {
                     const ClusterVector& clusters = clusterData->second;
                     ...
                     {
                         tm.SetTrans(otm * std::get<ClusterCenter>(clusters[i]));
                         cb->TM(tm, i);
                     }
                 }
             }
         ...
         case ES_SEGMENT:
             if (GetSegmentClusters(node, *shape, tm))
             {
                 ...
                 if (clusterData != nodeClusterMap.end())
                 {
                     ...
                     for (int i = 0; i < numClusters; ++i)
                     {
                         // Hand it the cluster center from the tuple
                         cb->Center(std::get<ClusterCenter>(clusters[i]), i);
                     }
                 }
             }
             break;
...
int EditSplineMod::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags, ModContext *mc) {   
	...
	// Draw spline angle information if necessary
	if (vpt->IsActive())
	{
		float angleValue = 0.0f;
		IPoint2 anglePosition;
		if (GetShapeAngleDisplayInfo(angleValue, anglePosition))
		{
			// Draw the text and remember where it was
			MSTR angleString;
			angleString.printf(_T("%.1f\xB0"), angleValue);
			IPoint3 pos(anglePosition.x, anglePosition.y, 0.0f);
			vpt->getGW()->wText(&pos, angleString);
		}
	}
   ...
   Matrix3 tm = inode->GetObjectTM(t) * (mc->tm ? Inverse(*mc->tm) : Matrix3{});
   gw->setTransform(tm);
...
class SplineModSubModSelectionProcessor : public SubModSelectionProcessor
{
protected:
	// override
	virtual bool HasOverrideDoubleClickProc() const override {
		return true;
	}
	virtual void OverrideDoubleClickProc(ViewExp* vpt, int flags) override {
		// double click to select all
		EditSplineMod* pSplineMod = dynamic_cast<EditSplineMod*>(obj);
		if (pSplineMod)
		{
			if (pSplineMod->GetSelLevel() != ES_OBJECT)
			{
				theHold.Begin();
				pSplineMod->DoubleClickSelect(vpt->ToPointer()->GetSubObjHitList().ClosestHit());
				theHold.Accept(GetString(IDS_TH_SELECTION));
			}
		}
	}
public:
	SplineModSubModSelectionProcessor(TransformModBox *mc, BaseObject* o, IObjParam* i)
		: SubModSelectionProcessor(mc, o, i) {}
};

template <class BaseSplineMode, class ModBoxType>
class SplineDoubleClickModeDecorator : public BaseSplineMode
{
private:
	SplineModSubModSelectionProcessor splineModMouseProc;
	ModBoxType					 theProc;
public:
	SplineDoubleClickModeDecorator(BaseObject* o, IObjParam* i) :
		BaseSplineMode(o, i),
		theProc(o, i),
		splineModMouseProc(&theProc, o, i)
	{
		splineModMouseProc.SetSupportTransformGizmo(TRUE);
	}

	MouseCallBack *MouseProc(int *numPoints)
	{
		*numPoints = 2;
		return &splineModMouseProc;
	}
};

template <class BaseSplineMode, class SelectionProcessor, class ModBoxType>
class DoubleClickModeDecorator : public BaseSplineMode
{
private:
	SelectionProcessor selProc;
	ModBoxType theProc;

public:
	DoubleClickModeDecorator(BaseObject* o, IObjParam* i)
		: BaseSplineMode(o, i)
		, theProc(o, i)
		, selProc(&theProc, o, i)
	{
		selProc.SetSupportTransformGizmo(TRUE);
	}
...
template <class BaseSplineMode>
class SplineDoubleClickModeDecorator<BaseSplineMode, SelectModBox> : public BaseSplineMode
{
private:
	SplineModSubModSelectionProcessor splineModMouseProc;
public:
...
// Class SplineModeRightClickProcessor is a right click processor designed to work in tandem with an owning
// SplineModeSelectionProcessor leftClickProc processing left clicks, in order to support clean right click cancellation of the
// actions implemented by SplineModeSelectionProcessor. When leftClickProc receives the MOUSE_INIT/MOUSE_UNINIT signal, it should
...
// other than SplineModeSelectionProcessor::N_POLY_ACTIONS, will trigger a call to SplineModeSelectionProcessor::AbortOperation on
// behalf of leftClickProc. Any other call be based through to the "base" right click processor defined in the MouseManager at
// the time that Activate() is called. Note that SplineModeSelectionProcessorType should refer to some specialization of the
// SplineModeSelectionProcessor template class.
template <typename SplineModeSelectionProcessorType>
class SplineModeRightClickProcessor : public MouseCallBack
{
...
public:
    ...
    // Constructor.
    //
    // leftClickProc: pointer to the SplineModeSelectionProcessor entity which owns this object; input
    SplineModeRightClickProcessor(SplineModeSelectionProcessorType* leftClickProc);

    // Destructor.
    virtual ~SplineModeRightClickProcessor();
...
private:
    ...
    // SplineModeSelectionProcessor entity which owns this object
    SplineModeSelectionProcessorType* m_leftClickProc;
...
private:
    ...
    SplineModeRightClickProcessor() = delete;

    // Copy and move constructors; should never be called.
    //
    // rightClickProc: a right click processor object; input
    SplineModeRightClickProcessor(const SplineModeRightClickProcessor& rightClickProc) = delete;
    SplineModeRightClickProcessor(const SplineModeRightClickProcessor&& rightClickProc) = delete;

    // Copy and move assignment operators; should never be called.
    //
    // rightClickProc: a right click processor object; input
    SplineModeRightClickProcessor& operator=(const SplineModeRightClickProcessor& rightClickProc) = delete;
    SplineModeRightClickProcessor& operator=(const SplineModeRightClickProcessor&& rightClickProc) = delete;
};

template <typename SplineModeSelectionProcessorType>
SplineModeRightClickProcessor<SplineModeSelectionProcessorType>::SplineModeRightClickProcessor(SplineModeSelectionProcessorType* leftClickProc) :
    MouseCallBack(),
...
template <typename SplineModeSelectionProcessorType>
SplineModeRightClickProcessor<SplineModeSelectionProcessorType>::~SplineModeRightClickProcessor()
{
...
template <typename SplineModeSelectionProcessorType>
void SplineModeRightClickProcessor<SplineModeSelectionProcessorType>::Activate()
{
...
template <typename SplineModeSelectionProcessorType>
void SplineModeRightClickProcessor<SplineModeSelectionProcessorType>::Deactivate()
{
...
template <typename SplineModeSelectionProcessorType>
int SplineModeRightClickProcessor<SplineModeSelectionProcessorType>::proc(HWND hwnd, int msg, int point, int flags, IPoint2 m)
{
    ...
    typename SplineModeSelectionProcessorType::SplineModeActionType actionCurr = m_leftClickProc->GetCurrentAction();
    if (actionCurr != SplineModeSelectionProcessorType::N_SPLINE_ACTIONS)
    {
        switch (msg)
        {
        case MOUSE_ABORT:
...
template <typename SplineModeSelectionProcessorType>
void SplineModeRightClickProcessor<SplineModeSelectionProcessorType>::pan(IPoint2 offset)
{
...
template <typename SplineModeSelectionProcessorType>
int SplineModeRightClickProcessor<SplineModeSelectionProcessorType>::override(int mode)
{
...
template <typename SplineModeSelectionProcessorType>
BOOL SplineModeRightClickProcessor<SplineModeSelectionProcessorType>::SupportTransformGizmo()
{
...
template <typename SplineModeSelectionProcessorType>
void SplineModeRightClickProcessor<SplineModeSelectionProcessorType>::DeactivateTransformGizmo()
{
    if (m_baseProc != nullptr)
    {
...
template <typename SplineModeSelectionProcessorType>
BOOL SplineModeRightClickProcessor<SplineModeSelectionProcessorType>::SupportAutoGrid()
{
    BOOL isSupported = FALSE;
    if (m_baseProc != nullptr)
    {
...
template <typename SplineModeSelectionProcessorType>
BOOL SplineModeRightClickProcessor<SplineModeSelectionProcessorType>::TolerateOrthoMode()
{
...
// Class SplineModeSelectionProcessor is a selection processor, derived from SplineModSubModSelectionProcessor, which may be used in
// place of that class for a particular mode, e.g. move mode, to allow any action from the list defined by the
// SplineModeActionType enumeration to be tied to any hotkey combination list in the SplineModeHotkeyType enumeration, for any
// particular spline geometry selection type enumerated in PShapeSelLevel. If an action has not been defined for a particular
// combination of hotkey and selection type, or if the action does not make sense for a particular selection type, e.g. action
// SPLINE_ACTION_EXTRUDE for selection type SL_SEGMENT, the base class treatment defined in SplineModSubModSelectionProcessor is
// employed. Note that the precise form of the operation is dependent upon the related mode transformation encapsulated in
// transMod, e.g. for move mode, with the SPLINE_ACTION_CLONE action and SL_SEGMENT selection type, dragging moves the new segment,
// while in scale mode, dragging resizes the segment.
class SplineModeSelectionProcessor : public SplineModSubModSelectionProcessor
{
...
public:
	...
	// combination not supported here is passed through to the PolySubModSelectionProcessor's treatment 
	enum SplineModeHotkeyType
	{
		SPLINE_HOTKEY_SHIFT = 0,
		SPLINE_HOTKEY_CTRL_SHIFT = 1,
		N_SPLINE_HOTKEYS = 2
	};

	// Enumeration indexing the possible actions which may be bound to any of the hotkey combinations indexed by
	// SplineMoveModeHotkeyType; these are the actions performed before the transformation defined by transMode
	enum SplineModeActionType
	{
		SPLINE_ACTION_CLONE = 0,    // Creates a copy of the selected entities and their sub-entities (e.g. vertices and edges for a face selection); transMode acts on the new entites
		SPLINE_ACTION_EXTRUDE = 1,    // Creates new segments at the ends of the selected segments; transMode acts on the original selection
		N_SPLINE_ACTIONS = 2
	};
...
public:
	...
	SplineModeSelectionProcessor(TransformModBox* transMode, BaseObject* object, IObjParam* objParam);

	// Destructor.
	virtual ~SplineModeSelectionProcessor();
	...
	//            operation in the relevant mode with selection type selection, or N_POLY_MOVE_ACTIONS to clear hotkey's
	//            presently assigned action, so that treatment will be managed by the base class; input
	// selection: spline geometry selection type (SS_XXX) for which the key combination indicated by hotkey is to trigger action action,
	//            or selection = -1 for all
	void SetHotkeyAction(SplineModeHotkeyType hotkey, SplineModeActionType action, int selection = -1);
    ...
    inline SplineModeActionType GetCurrentAction()
    {
        return m_actionCurr;
    }
...
private:
	...
	static constexpr int m_nSplineSelectionTypes = 4;	// 0=object, 1=vertex, 2=segment, 3=polygon

	// Resource string indices for names of defined actions
	static constexpr int m_actionNames[N_SPLINE_ACTIONS] = { IDS_CLONE, IDS_RB_EXTRUDE };

	// Action currently being processed, if any; if no defined action is being processed, the invalid value N_POLY_ACTIONS is
	// set
	SplineModeActionType m_actionCurr;

	// Entry m_hotkeyActions[key][sel] is the action to be performed if the hotkey combination indicated by key is used during
	// a dragging operation in the relevant mode, with selection type sel according to the SS_XXX enumeration, or
	// N_SPLINE_ACTIONS if combination key is to be passed through to the base class treatment for selection type sel
	SplineModeActionType m_hotkeyActions[N_SPLINE_HOTKEYS][m_nSplineSelectionTypes];
    ...
    SplineModeRightClickProcessor<SplineModeSelectionProcessor>* m_rightClickProc;
...
SplineModeSelectionProcessor::SplineModeSelectionProcessor(TransformModBox* transMode, BaseObject* object, IObjParam* objParam) :
	SplineModSubModSelectionProcessor(transMode, object, objParam),
	m_actionCurr(N_SPLINE_ACTIONS),
	...
	m_transMode(transMode)
{
    ...
    m_rightClickProc = new SplineModeRightClickProcessor<SplineModeSelectionProcessor>(this);
    
    for (int key = 0; key != N_SPLINE_HOTKEYS; ++key)
	{
		for (int sel = 0; sel != m_nSplineSelectionTypes; ++sel)
		{
			m_hotkeyActions[key][sel] = N_SPLINE_ACTIONS;
		}
	}
}

SplineModeSelectionProcessor::~SplineModeSelectionProcessor()
{
...
void SplineModeSelectionProcessor::SetHotkeyAction(SplineModeHotkeyType hotkey, SplineModeActionType action, int selection)
{
	constexpr int nActionTypesTot = N_SPLINE_ACTIONS + 1;
	constexpr bool isActionValidForSelection[nActionTypesTot][m_nSplineSelectionTypes] = { {true, false, true, true},	// All can be cloned except vertex
																						{false, true, true, false},	// Segment and vertex can extrude
																						{true, true, true, true} };

	// Check in debug mode to ensure a valid hotkey combination and selection type
	DbgAssert(hotkey != N_SPLINE_HOTKEYS);
	DbgAssert((selection >= -1) && (selection <= SS_SPLINE));

	// If selection = -1, then set the action for the specified hotkey combination for all mesh geometry selection
	...
	{
		selectionBegin = 0;
		nSelections = m_nSplineSelectionTypes;
	}

	for (int sel = 0; sel != nSelections; ++sel)
	{
		DbgAssert((selectionBegin + sel) < m_nSplineSelectionTypes);
...
int SplineModeSelectionProcessor::proc(HWND hwnd, int msg, int point, int flags, IPoint2 m)
{
	...
	{
		EditSplineMod* es = static_cast<EditSplineMod*>(obj);
		DbgAssert(es != nullptr);
		int selection = es->GetSubobjectLevel();
		DbgAssert((selection >= SS_OBJECT) && (selection <= SS_SPLINE));

		// We are at the beginnning of a mouse operation; note that our actions are disabled if we are in Maya mode, or if an
		// action override is active
		MaxSDK::CUI::IMouseConfigManager* mouseManager = MaxSDK::CUI::GetIMouseConfigManager();
		bool isMayaMode = mouseManager->GetMayaSelection();
		...
		if ((!isMayaMode) && (overridingAction == nullptr))
		{
			...
			// Defined actions are enabled; check flags and selection level, and set appropriate action m_actionCurr if one has
			// been triggered, otherwise, set invalid action to indicate that treatment should be handled entirely by the base
			// class
			m_actionCurr = N_SPLINE_ACTIONS;
			SplineModeHotkeyType hotkey = N_SPLINE_HOTKEYS;
			if (flags & MOUSE_SHIFT)
			{
				hotkey = SPLINE_HOTKEY_SHIFT;
				if (flags & MOUSE_CTRL)
				{
					hotkey = SPLINE_HOTKEY_CTRL_SHIFT;
				}
			}

			if (hotkey != N_SPLINE_HOTKEYS)
			{
	...
	bool isProcessed = false;
	IPoint2& mouseOffset = Offset();
	if (m_actionCurr != N_SPLINE_ACTIONS)
	{
        ...
        case MOUSE_ABORT:
            ...
            // Cancel of transform -- call base class to wrap up safely.
            hasSucceeded = SplineModSubModSelectionProcessor::proc(hwnd, msg, point, flags, m);
            break;
        break;		case MOUSE_POINT:
    ...
    // If we have an operation which is not defined here, or if messsage msg is not supported for the present operation, call
	// call the base class variant to complete processing, ensuring that we abort and clear any operation previously in
	// progress
	if (!isProcessed)
	{
		AbortOperation(hwnd);
...
void SplineModeSelectionProcessor::BeginOperation(bool& isProcessed, BOOL& hasSucceeded, HWND window, int flags, const IPoint2& mousePoint)
{
	constexpr int hitCheckOcclusion = 1 << (HITFLAG_STARTUSERBIT + 1);

	// Check in debug mode to ensure a valid state
	DbgAssert(m_actionCurr != N_SPLINE_ACTIONS);
	...
	isProcessed = false;
	hasSucceeded = TRUE;
	BOOL isSelectionValid = ip->SelectionFrozen();
	if (!isSelectionValid)
	{
		IPoint2 selectionPoint = mousePoint;
		ViewExp& viewport = ip->GetViewExp(window);
		int testFlags = HIT_SELONLY | hitCheckOcclusion;
		BOOL useTransformGizmo = ip->GetUseTransformGizmo();
		BOOL isTransformGizmoSupported = SupportTransformGizmo();
		if (useTransformGizmo && isTransformGizmoSupported)
		{
			testFlags = testFlags | HIT_SWITCH_GIZMO;
		}

		isSelectionValid = HitTest(&viewport, &selectionPoint, HITTYPE_POINT, testFlags);
	}
...
void SplineModeSelectionProcessor::UpdateMousePoint(bool& isProcessed, BOOL& hasSucceeded, HWND window, int point, int flags, const IPoint2& mousePoint)
{
...
void SplineModeSelectionProcessor::CompleteOperation(bool& isProcessed, BOOL& hasSucceeded, HWND window, int point, int flags, const IPoint2& mousePoint)
{
	DbgAssert(m_actionCurr != N_SPLINE_ACTIONS);
	...
	if (m_hasMouseMoved)
	{
		...
		if (hasSucceeded)
		{
			TimeValue timeCurr = ip->GetTime();
			switch (m_actionCurr)
			{
			case SPLINE_ACTION_CLONE:
				es->AcceptCloneSelSubComponents(timeCurr);
				break;
			default:
				break;
			}

			theHold.SuperAccept(GetString(m_actionNames[m_actionCurr]));
			m_actionCurr = N_SPLINE_ACTIONS;
			m_hasMouseMoved = false;
...
void SplineModeSelectionProcessor::AbortOperation(HWND hwnd)
{
	...
	if (m_actionCurr != N_SPLINE_ACTIONS)
	{
        m_transMode->proc(hwnd, MOUSE_POINT, endPoint, 0, m_mousePointBegin);

        // We have some operations that use a superhold and some which don't.
        // Only call the SuperCancel when necessary.
        if (theHold.GetSuperBeginDepth())
            theHold.SuperCancel();
...
static INT_PTR CALLBACK SplineSelectDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam);
static INT_PTR CALLBACK SplineOpsDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam);
void EditSplineMod::BeginEditParams( IObjParam *ip, ULONG flags,Animatable *prev )
   ...
   Tab<CommandMode*> hotkeyActionModes;

   CommandMode* baseMode = nullptr;
   moveMode = new DoubleClickModeDecorator<MoveModBoxCMode, SplineModeSelectionProcessor, MoveModBox>(this, ip);
   baseMode = moveMode;
   hotkeyActionModes.Append(1, &baseMode);
   rotMode = new DoubleClickModeDecorator<RotateModBoxCMode, SplineModeSelectionProcessor, RotateModBox>(this, ip);
   baseMode = rotMode;
   hotkeyActionModes.Append(1, &baseMode);
   uscaleMode = new DoubleClickModeDecorator<UScaleModBoxCMode, SplineModeSelectionProcessor, ScaleModBox>(this, ip);
   baseMode = uscaleMode;
   hotkeyActionModes.Append(1, &baseMode);
   nuscaleMode = new DoubleClickModeDecorator<NUScaleModBoxCMode, SplineModeSelectionProcessor, ScaleModBox>(this, ip);
   baseMode = nuscaleMode;
   hotkeyActionModes.Append(1, &baseMode);
   squashMode = new DoubleClickModeDecorator<SquashModBoxCMode, SplineModeSelectionProcessor, ScaleModBox>(this, ip);
   baseMode = squashMode;
   hotkeyActionModes.Append(1, &baseMode);
   selectMode      = new SplineDoubleClickModeDecorator<SelectModBoxCMode, SelectModBox>(this,ip);

   // Set up the selection processors for the appropiate modes to perform an extrude when the shift hotkey combination is used
   // while dragging, moving cloning to the control-plus-shift hotkey
   int nHotkeyActionModes = hotkeyActionModes.Count();
   for (int mode = 0; mode != nHotkeyActionModes; ++mode)
   {
	   int dummyVal = 0;
	   SplineModeSelectionProcessor* selectionProcessor = static_cast<SplineModeSelectionProcessor*>((hotkeyActionModes[mode])->MouseProc(&dummyVal));
	   DbgAssert(selectionProcessor != nullptr);
	   selectionProcessor->SetHotkeyAction(SplineModeSelectionProcessor::SPLINE_HOTKEY_CTRL_SHIFT, SplineModeSelectionProcessor::SPLINE_ACTION_CLONE, SS_SPLINE);
	   selectionProcessor->SetHotkeyAction(SplineModeSelectionProcessor::SPLINE_HOTKEY_CTRL_SHIFT, SplineModeSelectionProcessor::SPLINE_ACTION_CLONE, SS_SEGMENT);
	   selectionProcessor->SetHotkeyAction(SplineModeSelectionProcessor::SPLINE_HOTKEY_SHIFT, SplineModeSelectionProcessor::SPLINE_ACTION_EXTRUDE, SS_SEGMENT);
       selectionProcessor->SetHotkeyAction(SplineModeSelectionProcessor::SPLINE_HOTKEY_SHIFT, SplineModeSelectionProcessor::SPLINE_ACTION_EXTRUDE, SS_VERTEX);
   }
...
//watje
   ...
   mpActions = new ActionCB(this);
   mpSplineToolsActions = new SplineToolsActionCB(this);
   ip->GetActionManager()->ActivateActionTable(mpActions, kActionID);
   ip->GetActionManager()->ActivateActionTable(mpSplineToolsActions, kSplineToolsActionID);

   //register all of the overrides with this modifier, first though unregister any 	
   //still around for wthatever reason,though EndEditParams should handlethis
   IActionItemOverrideManager *actionOverrideMan = 
...
// watje
   ...
   if (mpSplineToolsActions) {
       ip->GetActionManager()->DeactivateActionTable(mpSplineToolsActions, kSplineToolsActionID);
       delete mpSplineToolsActions;
       mpSplineToolsActions = NULL;
   }
      ...
      for(int poly = polys-1; poly >= 0; --poly) {
         if(shape->polySel[poly]) {
            ...
            *newSpline = *spline;
            ...
            {
                auto theTm = nodes[i]->GetObjectTM(t);
               newSpline->Transform(&theTm);
            }
            if(!copy)
               shape->DeleteSpline(poly);
            }
...
//to handle copying bound vert list 4-27-99
         ...
         if (copy)
            {
            for (int m = 0; m < shape->bindList.length(); m++)
               {
...
UI::MouseCursors::ECursors EditSplineMod::GetBoolCursorID() {
   switch(boolType) {
      case BOOL_UNION:
         return  UI::MouseCursors::BoolUnion;
      case BOOL_SUBTRACTION:
         return  UI::MouseCursors::BoolSubtraction;
      case BOOL_INTERSECTION:
         return  UI::MouseCursors::BoolIntersection;
      }
   assert(0);
   return UI::MouseCursors::BoolIntersection;
   }
...
void EditSplineMod::SetOpsDlgEnables() {
   ...
   ip->GetActionManager()->DeactivateActionTable(&sActionCallback, kSplineToolsActions);
...
static INT_PTR CALLBACK SelectByDlgProc(
   ...
   {
   EditSplineMod *es = DLGetWindowLongPtr<EditSplineMod *>( hDlg);
   switch (msg) {
      case WM_INITDIALOG:
         es = (EditSplineMod *)lParam;
         DLSetWindowLongPtr( hDlg, es);
         es->hSelectBy = hDlg;
...
static INT_PTR CALLBACK SplineSelectDlgProc( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
   ...
   switch ( message ) {
      case WM_INITDIALOG: {
		 ...
		 iToolbar->AddTool(EDITSPLINE_REGULAR(CTB_CHECKBUTTON, EditSplineIcon::vertexSelection, ES_VERTEX));
		 iToolbar->AddTool(EDITSPLINE_REGULAR(CTB_CHECKBUTTON, EditSplineIcon::segmentSelection, ES_SEGMENT));
		 iToolbar->AddTool(EDITSPLINE_REGULAR(CTB_CHECKBUTTON, EditSplineIcon::splineSelection, ES_SPLINE));
         ReleaseICustToolbar(iToolbar);
         es->RefreshSelType();
...
public:
   ...
   const TCHAR *dialogTitle() {return GetString(IDS_TH_MULTIATTACH);}
   const TCHAR *buttonText()  {return GetString(IDS_TH_ATTACH);}
...
static INT_PTR CALLBACK SplineOpsDlgProc( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
   ...
   switch ( message ) {
      case WM_INITDIALOG: {
		 ...
		 iToolbar->AddTool(EDITSPLINE_REGULAR(CTB_CHECKBUTTON, EditSplineIcon::unionIcon, BOOL_UNION));
		 iToolbar->AddTool(EDITSPLINE_REGULAR(CTB_CHECKBUTTON, EditSplineIcon::subtract, BOOL_SUBTRACTION));
		 iToolbar->AddTool(EDITSPLINE_REGULAR(CTB_CHECKBUTTON, EditSplineIcon::intersection, BOOL_INTERSECTION));
		 ...
		 iToolbar->AddTool(EDITSPLINE_REGULAR(CTB_CHECKBUTTON, EditSplineIcon::mirrorHorizontally, MIRROR_HORIZONTAL));
		 iToolbar->AddTool(EDITSPLINE_REGULAR(CTB_CHECKBUTTON, EditSplineIcon::mirrorVertically, MIRROR_VERTICAL));
		 iToolbar->AddTool(EDITSPLINE_REGULAR(CTB_CHECKBUTTON, EditSplineIcon::mirrorBoth, MIRROR_BOTH));
...
//6-16-99 watje
      ...
      case WM_DESTROY:
		 ...
		 if ( es->pEndPointAutoConnectWeldSpinner ) {
			ReleaseISpinner(es->pEndPointAutoConnectWeldSpinner);
			es->pEndPointAutoConnectWeldSpinner = NULL;
		 }
		 if ( es->pConnectCopyThreshSpinner )
		 {
			 ReleaseISpinner(es->pConnectCopyThreshSpinner);
			 es->pConnectCopyThreshSpinner = NULL;
		 }
         if ( es->iUnion ) {
...
IOResult EditSplineMod::Load(ILoad *iload) {
   ...
   useAreaSelect = FALSE;
   version = EDITSPLINEMOD_VERSION_INITIAL;  // Version just before we saved versions
...
void EditSplineMod::HandleTrimExtend(ViewExp *vpt, ShapeHitData *hit, IPoint2 &m, int trimType) {
    const BOOL useLegacyDPoint3Add = version < EDITSPLINEMOD_VERSION_CORRECTED_DPOINT3_ADD;
   ...
   for ( int i = 0; i < mcList.Count(); i++ ) {
      ...
      if(shape == hit->shape) {
         ...
         if ( theHold.Holding() )
            theHold.Put(new ShapeRestore(shapeData,this,shape));
         if(shape->PerformTrimOrExtend(ip, vpt, hit, m, cb, trimType, trimInfinite, useLegacyDPoint3Add)) {
            holdNeeded = TRUE;
            shapeData->UpdateChanges(shape, FALSE);
            shapeData->TempData(this)->Invalidate(TOPO_CHANNEL);
            }
...
MtlID EditSplineMod::GetNextAvailMtlID(ModContext* mc) {
   ...
   if (mtlID == -1) {
      ...
      int poly, polys = shape->splineCount;
      for(poly = 0; poly < polys; ++poly) {
         Spline3D *spline = shape->splines[poly];
         for(int seg = 0; seg < spline->Segments(); ++seg) {
...
BOOL EditSplineMod::HasFaceSelection(ModContext* mc) {
   ...
   if (selLevel == ES_SPLINE && shape->polySel.sel.AnyBitSet()) return TRUE;
...
int   EditSplineMod::GetMaxMtlID(ModContext* mc) {
   ...
   for(int poly = 0; poly < polys; ++poly) {
      Spline3D *spline = shape->splines[poly];
      for(int seg = 0; seg < spline->Segments(); ++seg)
         mtlID = std::max(mtlID, spline->GetMatID(seg));
      }
...
//watje now move any verts thta are within the weld tolerance onto each other
//build list 
	 MaxSDK::Array<WeldList> weldList;
      for(poly = 0; poly < shape->splineCount; ++poly)
         {
         BitArray pSel = shape->VertexTempSel(poly);
         for (int i = 0; i < pSel.GetSize()/3; i++)
      ...
      int ct = (int)weldList.length();
      if (ct >0)
         {
         altered = holdNeeded = TRUE;
         Point3 avgPoint(0.0f,0.0f,0.0f);
         int pointCount = 0;
         for (int j = 0; j < weldList.length(); j++)
            {
            avgPoint += weldList[j].p;
            pointCount++;
            }
...
void EditSplineMod::DoAccept(TimeValue t)
{

}

void EditSplineMod::SetPinch(TimeValue t, float pinch)
{
	Rect rect;
	ISpinnerControl* spin = GetISpinner(GetDlgItem(hSoftSelPanel,IDC_PINCH_SPIN));
	spin->SetValue(pinch, false);
	mPinch = pinch;
	GetClientRectP(GetDlgItem(hSoftSelPanel,IDC_AR_GRAPH),&rect);
	InvalidateRect(hSoftSelPanel,&rect,FALSE);
	UpdateVertexWeights();
	NotifyDependents(FOREVER, PART_TOPO, REFMSG_CHANGE);
	ip->RedrawViews(t,REDRAW_NORMAL);
	ReleaseISpinner(spin);
...
void EditSplineMod::SetFalloff(TimeValue t,float falloff)
{
	Rect rect;
	ISpinnerControl* spin = GetISpinner(GetDlgItem(hSoftSelPanel,IDC_FALLOFF_SPIN));
	spin->SetValue(falloff, false);
	mFalloff = falloff;
	GetClientRectP(GetDlgItem(hSoftSelPanel,IDC_AR_GRAPH),&rect);
	InvalidateRect(hSoftSelPanel,&rect,FALSE);
	UpdateVertexWeights();
...
void EditSplineMod::SetBubble(TimeValue t, float bubble)
{
	Rect rect;
	ISpinnerControl* spin = GetISpinner(GetDlgItem(hSoftSelPanel,IDC_BUBBLE_SPIN));
	spin->SetValue(bubble, false);
	mBubble = bubble;
	GetClientRectP(GetDlgItem(hSoftSelPanel,IDC_AR_GRAPH),&rect);
	InvalidateRect(hSoftSelPanel,&rect,FALSE);
	UpdateVertexWeights();
...
float EditSplineMod::GetPinch(TimeValue t)
{
	ISpinnerControl* spin = GetISpinner(GetDlgItem(hSoftSelPanel,IDC_PINCH_SPIN));
...
float EditSplineMod::GetBubble(TimeValue t)
{
	ISpinnerControl* spin = GetISpinner(GetDlgItem(hSoftSelPanel,IDC_BUBBLE_SPIN));
	float val = spin->GetFVal();
	ReleaseISpinner(spin);
...
float EditSplineMod::GetFalloff(TimeValue t)
{
	ISpinnerControl* spin = GetISpinner(GetDlgItem(hSoftSelPanel,IDC_FALLOFF_SPIN));
	float val = spin->GetFVal();
	ReleaseISpinner(spin);
...
void EditSplineMod::EditSoftSelectionMode()
{
...
bool EditSplineMod::SelectSimilar()
{
    int selectionLevel = GetSubobjectLevel();
    if (selectionLevel != ES_VERTEX && selectionLevel != ES_SEGMENT)
        return false;
    ...
    for (int i = 0; i < mcList.Count(); i++)
    {
        ...
        int numSplines = shape->SplineCount();

        switch (selectionLevel)
        {
        case ES_VERTEX:
        {
            // Find the vertice(s) that are selected and stash them for comparison
            int numSplines = shape->SplineCount();
            const ShapeVSel& vSel = shape->vertSel;
            std::vector<VertexComparison> comparisons;

            for (int i = 0; i < numSplines; ++i)
            {
                const BitArray& selection = vSel.sel[i];
                ...
                for (int k = 0; k < numKnots; ++k)
                {
                    if (selection[k * 3 + 1])
                    {
                        ...
                        int knotType = spline->GetKnotType(k);
                        if (k > 0 || closed)
                            l1 = spline->SegmentLength((k + numSegs - 1) % numSegs);
                        else
                            nSegs = 1;
                        if (k < (numSegs - 1) || closed)
                            l2 = spline->SegmentLength(k);
                        else
                            nSegs = 1;
                        if (nSegs == 2)
                        {
                            SplineKnot knot = spline->GetKnot(k);
                            Point3 point = knot.Knot();
                            Point3 inVec = knot.InVec();
                            Point3 outVec = knot.OutVec();
                            if (inVec == point)
                                hasInVec = false;
                            if (outVec == point)
                                hasOutVec = false;
                            if (hasInVec && hasOutVec)
                                angle = std::acos(Normalize(inVec - point) % Normalize(outVec - point)) * RAD_TO_DEG;
                        }
                        comparisons.push_back(VertexComparison({ knotType, nSegs, angle, l1, l2 }));
                    }
                }
            }

            // If something selected, process!
            if (comparisons.size() != 0)
            {
                bool nodeAltered = false;
                
                // Now find similars and select them all
                for (auto& comparison : comparisons)
                {
                    float minLength1 = comparison.length1 * 0.95f;
                    float maxLength1 = comparison.length1 * 1.05f;
                    float minLength2 = comparison.length2 * 0.95f;
                    float maxLength2 = comparison.length2 * 1.05f;
                    float minAngle = comparison.angle - 1.5f;
                    float maxAngle = comparison.angle + 1.5f;
                    for (int i = 0; i < numSplines; ++i)
                    {
                        BitArray& selection = vSel.sel[i];
                        Spline3D* spline = shape->splines[i];
                        int numKnots = spline->KnotCount();
                        int numSegs = spline->Segments();
                        bool closed = !!spline->Closed();
                        ...
                        {
                            int vertIndex = k * 3 + 1;
                            // Ignore already-selected knots or dissimilar types
                            if (!selection[vertIndex] && comparison.knotType == spline->GetKnotType(k))
                            {
                                // Compute this knot's attributes
                                float l1 = 0.0f;
                                float l2 = 0.0f;
                                int nSegs = 2;
                                float angle = 999.0f;
                                ...
                                if (k < (numSegs - 1) || closed)
                                    ...
                                    nSegs = 1;
                                if (comparison.nSegs != nSegs) continue;

                                // Check vector lengths, in both direction senses
                                bool vecTest1 = l1 >= minLength1 && l1 <= maxLength1 && l2 >= minLength2 && l2 <= maxLength2;
                                bool vecTest2 = l1 >= minLength2 && l1 <= maxLength2 && l2 >= minLength1 && l2 <= maxLength1;
                                if (!vecTest1 && !vecTest2) continue;

                                if (nSegs == 2 && comparison.angle < 999.0f)
                                {
                                    SplineKnot knot = spline->GetKnot(k);
                                    Point3 point = knot.Knot();
                                    Point3 inVec = knot.InVec();
        ...
        case ES_SEGMENT:
        {
            // Find the segment(s) that are selected and stash them for comparison
            const ShapeSSel& polySel = shape->segSel;
            std::vector<SegmentComparison> comparisons;

            for (int i = 0; i < numSplines; ++i)
            {
                const BitArray& selection = polySel.sel[i];
                if (selection.IsEmpty()) continue;
                Spline3D* spline = shape->splines[i];
                int numSegs = selection.GetSize();
                for (int s = 0; s < numSegs; ++s)
                {
                    if (selection[s])
                    {
                        comparisons.push_back(SegmentComparison({ spline->GetLineType(s), spline->SegmentLength(s) }));
                    }
                }
            }
            ...
            if (comparisons.size() != 0)
            {
                ...
                for (auto& comparison : comparisons)
                {
                    float minLength = comparison.length * 0.95f;
                    float maxLength = comparison.length * 1.05f;
                    for (int i = 0; i < numSplines; ++i)
                    {
                        BitArray& selection = polySel.sel[i];
                        Spline3D* spline = shape->splines[i];
                        int numSegs = selection.GetSize();
                        for (int s = 0; s < numSegs; ++s)
                        {
                            if (comparison.lineType == spline->GetLineType(s))
                            {
                                float thisLength = spline->SegmentLength(s);
...
INT_PTR CALLBACK SplineSoftSelectDlgProc( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
   {
   EditSplineMod *esm = DLGetWindowLongPtr<EditSplineMod *>( hDlg);
   ...
   switch ( message ) {
      case WM_INITDIALOG: {
         ...
         DLSetWindowLongPtr( hDlg, esm);        

         CheckDlgButton( hDlg, IDC_SOFT_SELECTION, esm->UseSoftSelections() );
         CheckDlgButton( hDlg, IDC_EDGE_DIST, esm->UseEdgeDists() );

         ReleaseISpinner(SetupIntSpinner   ( hDlg, IDC_EDGE_SPIN, IDC_EDGE, 1, 999, esm->mEdgeDist ));
         ReleaseISpinner(SetupUniverseSpinner ( hDlg, IDC_FALLOFF_SPIN, IDC_FALLOFF, 0.0, 9999999.0, esm->mFalloff ));
         ReleaseISpinner(SetupUniverseSpinner ( hDlg, IDC_PINCH_SPIN, IDC_PINCH, -10.0, 10.0, esm->mPinch ));
         ReleaseISpinner(SetupUniverseSpinner ( hDlg, IDC_BUBBLE_SPIN, IDC_BUBBLE, -10.0, 10.0, esm->mBubble ));
...
BOOL SetupMtlSubNameCombo (HWND hWnd, EditSplineMod *es) {
	...
	if(singleNode == NULL || nodeMtl == NULL || nodeMtl->ClassID() != Class_ID(MULTI_CLASS_ID, 0)) {    //no UI for multi, cloned nodes, and not MultiMtl 
