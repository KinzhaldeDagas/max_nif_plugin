...
extern void DoPolyEndAttach(BezierShape* shape, int poly1, int vert1, int poly2, int vert2, BOOL r3Way = TRUE);

// in mods.cpp
extern HINSTANCE hInstance;

// Vertex-weld threshold size
#define WELDSIZE 6
...
int VertInsertMouseProc::proc(
	...
	if(inserting) {
		Spline3D* theSpline = shape->splines[poly];
		...
		int previousDrawPhase = theSpline->DrawPhase();
		int res = theSpline->Create(vpt.ToPointer(),msg,point,flags,m,&mat,ip);
		int newDrawPhase = theSpline->DrawPhase();

		// Must update the shape's selection set for this spline
		BitArray& vsel = shape->vertSel[poly];
		BitArray& ssel = shape->segSel[poly];
		while(vsel.GetSize() < theSpline->Verts()) {
		...
		while(ssel.GetSize() < theSpline->Segments()) {
		...
		// If still inserting, check for potential closure with other endpoints in the shape.
		// We only need to do this if the Insert it working on the end of the spline.
		int currentKnot = theSpline->GetiCur();
		int lastKnotIndex = theSpline->KnotCount() - 1;
		if (inserting
			&& (currentKnot == lastKnotIndex)
			&& (previousDrawPhase == DRAW_INITIAL_MOUSE_DOWN || previousDrawPhase == DRAW_INITIAL_BEZ_ADJ)
			&& newDrawPhase == DRAW_FREEMOVE_POINT)
		{
			int knotToCompare = lastKnotIndex;
			// If the user was dragging a vector, we use the previous knot as our weld comparison
			if (previousDrawPhase == DRAW_INITIAL_BEZ_ADJ)
				knotToCompare--;
			// Check endpoints of all other open splines in this shape
			int weldToKnot = -1, weldToPoly = -1;
			bool autoWeld = (SplineShape::GetUseEndPointAutoConnect() == BST_CHECKED);
			// Only allow self-welding if autoWeld is active
			bool selfWeldOK = autoWeld;
			bool gotWeldTarget = BezierShapeFindWeldTarget(pVpt, mat, shape, poly, knotToCompare, selfWeldOK, autoWeld, weldToPoly, weldToKnot);
			if (gotWeldTarget)
			{
				// We have a potential weld! Either auto-weld or prompt for closure
				bool weld = true;
				if (!autoWeld)
				{
					// No auto-weld; prompt the user
					TSTR line1(GetString(IDS_TH_CONNECT_COINCIDENT));
					TSTR title(GetString(IDS_TH_SPLINE));
					int result = MaxSDK::MaxMessageBox(GetActiveWindow(), line1, title, MB_YESNO);
					weld = (result == IDYES);
				}
				if (weld)
				{
					// Weld the endpoint to the other spline

					// This operation leaves an extra knot on the spline being
					// created -- Get rid of it!
					theSpline->DeleteKnot(theSpline->KnotCount() - 1);
					DoPolyEndAttach(shape, poly, theSpline->KnotCount() - 1, weldToPoly, weldToKnot, FALSE);
					es->EndVertInsert();
					inserting = FALSE;
					backspaceRouter.UnRegister(&iBack);
					res = CREATE_STOP;
		...
		switch(msg)	{
			...
			case MOUSE_POINT:
				if(InsertWhere(vpt.ToPointer(), &m, &shape, &poly, &seg, &vert)) {
					...
					Spline3D* theSpline = shape->splines[poly];
					theSpline->SetKnotType(SPLINE_INITIAL_TYPE_INDEX, es->newKnotType);
					originalInsert = seg;
					mat = mod->insertTM;
					theSpline->StartInsert(vpt.ToPointer(), msg, point, flags, m, &mat, vert/3+1 );
					// Must update the shape's selection set for this spline
					BitArray& vsel = shape->vertSel[poly];
					BitArray& ssel = shape->segSel[poly];
					while(vsel.GetSize() < theSpline->Verts()) {
					...
					while(ssel.GetSize() < theSpline->Segments()) {
						ssel.SetSize(ssel.GetSize() + 1,1);
						if (vert / 3 + 1 < ssel.GetSize()) {
							ssel.Shift(RIGHT_BITSHIFT, 1, vert / 3 + 1);
							ssel.Clear(vert / 3 + 1);
							}
						}
					inserting = TRUE;
					iBack.SetPtrs(mod, theSpline);
...
int CreateLineMouseProc::proc(
	...
	if(creating) {
		...
		int previousDrawPhase = theSpline->DrawPhase();
		int res = theSpline->Create(pVpt,msg,point,flags,m,&mat,ip);
		int newDrawPhase = theSpline->DrawPhase();
		// Must update the shape's selection set for this spline
			...
			&& newDrawPhase == DRAW_FREEMOVE_POINT)
		{
			...
			if (gotWeldTarget)
			{
				...
				{
					// No auto-weld; prompt the user
					TSTR line1(GetString(IDS_WELD_ENDPOINT_QUESTION));
					TSTR title(GetString(IDS_TH_SPLINE));
					int result = MaxSDK::MaxMessageBox(GetActiveWindow(), line1, title, MB_YESNO);
					weld = (result == IDYES);
				}
	...
	if(inserting) {
		...
		int previousDrawPhase = theSpline->DrawPhase();
		int res = shape->splines[poly]->Create(pVpt,msg,point,flags,m,&mat,ip);
		int newDrawPhase = theSpline->DrawPhase();
		...
		switch(msg)	{
			...
			case MOUSE_POINT:
				...
				if((SplineShape::GetUseEndPointAutoConnect() == BST_CHECKED) &&
					InsertWhere(pVpt, &m, &shape, &poly, &seg, &vert)) {
					Spline3D* theSpline = shape->splines[poly];
					theSpline->SetKnotType(SPLINE_INITIAL_TYPE_INDEX, es->newKnotType);
					vert = es->StartVertInsert(vpt.ToPointer(), shape, poly, seg, vert, &mod);
					if(vert < 0) {
					...
					originalInsert = seg;
					mat = mod->insertTM;
					theSpline->StartInsert(pVpt, msg, point, flags, m, &mat, vert/3+1 );
					// Must update the shape's selection set for this spline
					BitArray& vsel = shape->vertSel[poly];
					BitArray& ssel = shape->segSel[poly];
					while(vsel.GetSize() < theSpline->Verts()) {
					...
					spline = shape->NewSpline(es->newKnotType);
					shape->UpdateSels(TRUE);
					mat = mod->createTM;
					vpt.getGW()->setTransform(mat);	
					if(spline->Create(pVpt,msg,point,flags,m,&mat,ip) == CREATE_CONTINUE) {
						creating = TRUE;
						shape->UpdateSels(TRUE);
