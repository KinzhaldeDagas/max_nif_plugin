...
#define EDITSPLINE_REGULAR(type, iconName, cmd) \
	ToolButtonItem(type,iconName,EditSplineIcon::IconSize,EditSplineIcon::IconSize,24,23,cmd)

namespace EditSplineIcon {
	//icon size
	static int IconSize = 16;

	//Selection rollup
	static const TCHAR* vertexSelection = _T("CommandPanel/Modify/ObjectSpaceModifiers/VertexEditSpline");
	static const TCHAR* segmentSelection = _T("CommandPanel/Modify/ObjectSpaceModifiers/Segment");
	static const TCHAR* splineSelection = _T("CommandPanel/Modify/ObjectSpaceModifiers/Spline");

	//Modifier Stack
	static const TCHAR* vertexStack = _T("CommandPanel/Modify/ModifierStack/VertexEditSplineStack");
	static const TCHAR* segmentStack = _T("CommandPanel/Modify/ModifierStack/SegmentStack");
	static const TCHAR* splineStack = _T("CommandPanel/Modify/ModifierStack/SplineStack");

	//Geometry rollup
	static const TCHAR* mirrorBoth = _T("CommandPanel/Modify/ObjectSpaceModifiers/MirrorBoth");
	static const TCHAR* mirrorHorizontally = _T("CommandPanel/Modify/ObjectSpaceModifiers/MirrorHorizontally");
	static const TCHAR* mirrorVertically = _T("CommandPanel/Modify/ObjectSpaceModifiers/MirrorVertically");
	static const TCHAR* intersection = _T("Boolean2/Intersection");
	static const TCHAR* unionIcon = _T("Boolean2/Union");
	static const TCHAR* subtract = _T("Boolean2/Subtract");
}
...
class ActionCB;
class SplineToolsActionCB;

class EditSplineMod : public Modifier, ISplineOps, ISplineSelect, ISubMtlAPI, AttachMatDlgUser , public EditSSCB{
...
public:
	...
	static SplineToolsActionCB* mpSplineToolsActions;
		...
		// Named selections
		Tab<TSTR*> namedSel[3];
		int FindSet(const TSTR &setName, int level);
		...
		void GetClassName(MSTR& s, bool localized) const override { s= TSTR(_T("EditSplineMod")); }
		...
		UI::MouseCursors::ECursors GetBoolCursorID();
...
public:
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_TH_EDITSPLINE) : _T("Edit Spline"); }
...
class ActionCB : public ActionCallback {
	EditSplineMod *mpMod;
public:
	ActionCB (EditSplineMod *m) : mpMod(m) { }
	BOOL ExecuteAction(int id);
};

ActionTable* GetActions();

class SplineToolsActionCB : public ActionCallback {
	EditSplineMod* mpMod;
public:
	SplineToolsActionCB(EditSplineMod* m) : mpMod(m) { }
	BOOL ExecuteAction(int id);
};
...
protected:
	...
	EditSplineMod *mpObj;
public:
	EditSSOverride(EditSplineMod *m):mActiveOverride(FALSE),mpObj(m){}
...
class EditSplineClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Edit Spline"); }
	SClass_ID		SuperClassID() { return OSM_CLASS_ID; }
	Class_ID		ClassID() { return Class_ID(EDITSPLINE_CLASS_ID,0); }
	const TCHAR* 	Category() { return GetString(IDS_RB_DEFEDIT);}
	void			ResetClassParams(BOOL fileReset);
	int             NumActionTables(){return 2;}
	ActionTable* GetActionTable(int i) {
		switch (i)
		{
		case 0: return GetActions();
		case 1: return GetSplineToolsActions();
		}
		return nullptr;
	}
...
class ESPolyVert {
	public:
		...
		ESPolyVert(int p, int v, const Point3 &d) { poly = p; vert = v; flags = 0; delta = d; }
	};
...
class CrossSectionMouseProc : public MouseCallBack {
	...
	protected:
		...
		void DrawCrossing(HWND hWnd, bool bErase, bool bPresent);
...
public:
	HWND hwnd;
	EditSplineMod *es;
	SingleRefMakerSplineMMtl() {hwnd = NULL; es = NULL;}
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("SingleRefMakerSplineMMtl"); }
};
...
public:
	HWND hwnd;
	EditSplineMod *es;
	SingleRefMakerSplineMNode() {hwnd = NULL; es = NULL;}
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("SingleRefMakerSplineMNode"); }
};
