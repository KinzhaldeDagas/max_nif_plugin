...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_RB_TESSMOD) : _T("Tessellate");}  
	virtual Class_ID ClassID() { return Class_ID(TESSELLATE_CLASS_ID,0);}
	void BeginEditParams(IObjParam  *ip, ULONG flags,Animatable *prev);
	void EndEditParams(IObjParam *ip,ULONG flags,Animatable *next);
	RefTargetHandle Clone(RemapDir& remap);
	const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_TESSMOD) : _T("Tessellate");}
	IOResult Load (ILoad *iload);

	// From modifier
	ChannelMask ChannelsUsed()  {return OBJ_CHANNELS;}
	ChannelMask ChannelsChanged() {return OBJ_CHANNELS-SUBSEL_TYPE_CHANNEL;}
...
public:
	...
	void NotifyInputChanged (const Interval& changeInt, PartID part,
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Tessellate"); }
...
public:
	...
	INT_PTR DlgProc (TimeValue t, IParamMap *map, HWND hWnd,
		UINT msg, WPARAM wParam, LPARAM lParam);
	void DeleteThis () { }
};

INT_PTR TessModDlgProc::DlgProc (TimeValue t, IParamMap *map, HWND hWnd,
							  UINT msg,WPARAM wParam,LPARAM lParam) {
...
void TessMod::ModifyObject (TimeValue t, ModContext &mc, ObjectState *os, INode *node) {
	...
	if (faceType == 0) {
		...
		case MESH_VERTEX:
		{
			...
			const BitArray& vertSel = tobj->GetMesh().VertSel();
			for (int i=0; i<tobj->GetMesh().numFaces; i++) {
				for (int j=0; j<3; j++) {
					if (vertSel[tobj->GetMesh().faces[i].v[j]]) { fsel.Set(i); break; }
				}
			}
			break;
		}
		...
		{
			fsel.SetSize (tobj->GetMesh().numFaces);
			const BitArray& edgeSel = tobj->GetMesh().EdgeSel();
			for (int i=0; i<tobj->GetMesh().numFaces; i++) {
				for (int j=0; j<3; j++) {
					if (edgeSel[i*3+j]) { fsel.Set(i); break; }
				}
			}
			break;
		}
		...
		if (type) { // Face-Center Tri mode
			for (int i=0; i<iterations; i++) {
				...
				if (i<iterations-1) {
					...
					// Defect 769602 Set size of the sel bit array and keep the last selection.
					fsel.SetSize (tobj->GetMesh().numFaces,true); 
...
RefResult TessMod::NotifyRefChanged (const Interval& changeInt,RefTargetHandle hTarget,
									 PartID& partID, RefMessage message, BOOL propagate) {
	int type;
	switch (message) {
		case REFMSG_CHANGE:
		...
		case REFMSG_GET_PARAM_NAME_NONLOCALIZED: {
			GetParamName *gpn = (GetParamName*)partID;
			switch (gpn->index) {
				case PB_TENSION: gpn->name = _T("Tension"); break;
			}
			return REF_HALT;
		}
	}
...
void TessMod::NotifyInputChanged (const Interval& changeInt, PartID part,
									  RefMessage msg, ModContext *mc) {
