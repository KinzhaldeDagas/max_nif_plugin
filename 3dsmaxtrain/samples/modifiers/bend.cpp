...
//      all corresponding class variables have gone, including the ParamMaps since they are replaced 
...
class BendMod : public SimpleMod2 {	
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_RB_BENDMOD) : _T("BendMod"); }  
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_BEND2) : _T("Bend");}

		// --- Inherited virtual methods of SimpleMod
...
public:
	...
	void CalcR(int axis, double angle);
...
class BendClassDesc:public ClassDesc2 {
	...
	// non-localized class name for scripting
	const TCHAR*	NonLocalizedClassName() { return _T("Bend"); }
...
public:
	void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)    // set from v
	{
		...
		IParamBlock2* pblock2 = u->GetParamBlockByID(0);
		DbgAssert(pblock2);
		if (pblock2)
		{
			float from, to;
			...
			case bend_from:
				...
				from = v.f;
				if (from > to) {
					pblock2->SetValue(bend_to, t, from);
				}
				break;
			...
			case bend_to:
				...
				to = v.f;
				if (from > to) {
					pblock2->SetValue(bend_from, t, to);
				}
				break;
			}
		}
...
static ParamBlockDesc2 bend_param_blk ( bend_params, _T("Bend Parameters"),  0, &bendDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, SIMPMOD_PBLOCKREF,
	...
	bend_dir,	_T("BendDir"),	TYPE_FLOAT,	P_RESET_DEFAULT|P_ANIMATABLE,	IDS_DIR,
		...
		p_nonLocalizedName, _T("Direction"),
		p_end,
	...
	bend_axis, 		_T("BendAxis"), TYPE_INT, P_RESET_DEFAULT,	IDS_AXIS,
		...
		p_vals,			0,1,2,
		p_nonLocalizedName, _T("Axis"),
		p_end,
		
	bend_fromto, 		_T("FromTo"), TYPE_BOOL, P_RESET_DEFAULT,	IDS_FROMTO,
		p_default, 		FALSE,
		p_ui, 			TYPE_SINGLECHECKBOX, IDC_BEND_AFFECTREGION,
		p_nonLocalizedName, _T("Limit"),
		p_end,
	...
	bend_from,	_T("BendFrom"),	TYPE_FLOAT,	P_RESET_DEFAULT|P_ANIMATABLE,	IDS_FROM,
		...
		p_nonLocalizedName, _T("LowerLimit"),
		p_end,
	...
	bend_to,	_T("BendTo"),	TYPE_FLOAT,	P_RESET_DEFAULT|P_ANIMATABLE,	IDS_TO,
		...
		p_accessor,		&bendPBAccessor,
		p_nonLocalizedName, _T("UpperLimit"),
		p_end,
...
void BendMod::BeginEditParams( IObjParam *ip, ULONG flags,Animatable *prev )
	{
	// Call the SimpleMod2 method first to take care of its processing.
	this->ip = ip ;
...
void BendMod::EndEditParams( IObjParam *ip, ULONG flags,Animatable *next )
	{
	// We must call the SimpleMod2 method first to take care of its 
	// processing.
...
void BendDeformer::CalcR(int axis, double angle)
	{
	double len = double(0);
	if (!doRegion) {
		switch (axis) {
...
// and returns it.
Point3 BendDeformer::Map(int i, Point3 p) const
{
	double x, y, c, s, yr;
	if (r == 0 && !doRegion)
		return p;
	...
	p.x = px;
	double pz = r * s - x * s;
	p.z = pz; // The point is finally transformed by the inverse of the tm.
	p = p * invtm;
	return p;
}

bool BendDeformer::IsThreadSafe() const
{
	return true;
}
