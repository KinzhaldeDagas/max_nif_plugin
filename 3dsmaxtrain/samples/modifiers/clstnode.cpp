...
#define CLUSTNODECONTAINERMAIN_TVNODE_CLASS_ID Class_ID(0xe27e0f2b, 0x74fad505)
...
class ClustNodeMod : public IClustNodeMod {	
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_RB_CLUSTNODEMOD) : _T("ClustNodeMod"); }  
		virtual Class_ID ClassID() { return Class_ID(CLUSTNODEOSM_CLASS_ID,0);}
		void BeginEditParams(IObjParam *ip, ULONG flags,Animatable *prev);
		void EndEditParams(IObjParam *ip,ULONG flags,Animatable *next);		
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_NODEXFORM) : _T("Linked XForm");}
...
RefResult ClustNodeNotify::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
	{
...
class ClustNodeClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Linked XForm"); }
...
static ParamBlockDesc2 lxform_param_blk ( lxform_params, _T("Parameters"),  0, &clustNodeDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF, 
	...
	lxform_node, 	_T(""),						TYPE_INODE, 	P_READ_SECOND_FLAG_VALUE, P_USE_NODE_TM_VALIDITY,	IDS_PW_NODE,
		p_nonLocalizedName, _T("Node"),
		p_end,

	lxform_backtm, 	_T("backTransform"),		TYPE_BOOL, 		P_RESET_DEFAULT,	IDS_PW_BACKTM,
		p_default, 		TRUE,
		p_ui, 			TYPE_SINGLECHECKBOX, 	IDC_BACKTM,
		p_end,
	p_end
	);
...
INT_PTR LXFormProc::DlgProc (TimeValue t, IParamMap2 *map,
...
ClustNodeMod::~ClustNodeMod()
{
	...
	if (container && notify)
	{
		...
		ITrackViewNode *tvroot = global->GetNode(CLUSTNODECONTAINERMAIN_TVNODE_CLASS_ID);
		if (!tvroot)
			tvroot = tvr;
		if (tvroot)
		{
...
public:
	Matrix3 tm, invtm;
	ClustNodeDeformer(Matrix3 m, Matrix3 mi)
	{
		tm = m;
		invtm = mi;
	}
...
void ClustNodeMod::ModifyObject(
	...
	INode *tnode = NULL;
	pblock->GetValue(lxform_node,t,tnode,valid);
	
	BOOL backtm = TRUE;
	pblock->GetValue(lxform_backtm,t,backtm,valid);
	...
	if (tnode)
		...
		if ((ver >= 600) && (backtm))
		{
			...
			if ((d->selfNode == NULL) || (nodeTest != d->selfNode))
			{
				...
				if ((container) && (nodeTest != pnode) && d->selfNode != nullptr)
				{
			...
			if ((container == NULL) && (baseNode))
			{
				...
				if (!tvroot) 
				{
					...
					global->AddNode(mcontainer, GetString(IDS_RB_CLUSTNODEMOD), CLUSTNODECONTAINERMAIN_TVNODE_CLASS_ID);
					tvroot = mcontainer;
				}
