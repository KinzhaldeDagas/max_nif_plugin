...
public:
	...
	int      NumRefs() { return isShallowCopy ? 0 : ptCont.Count() + 3; }
	RefTargetHandle GetReference(int i);
	void  SetReference(int i, RefTargetHandle rtarg);
	int      NumSubs() { return isShallowCopy ? 0 : 3; }
	...
	int      GridDim(int which) const;
	void  SetGridDim(int dm[3]);
	int      GridIndex(int i, int j, int k);
	void  ExpandSelection(int ix, BOOL on);
	void  BaseClone(FFDNM<T> *to, RemapDir &remap);

	virtual Point3 InverseLattice(Point3 p) const = 0;
	virtual Point3 GetSourcePoint(int i) = 0;
	virtual int GetNameID() const = 0;
	virtual const TCHAR* GetNonLocalizedName() const = 0;
...
public:
	...
	Point3 Map(int ii, Point3 p) const override;
...
template <class T>
bool FFDNMDeformer<T>::IsThreadSafe() const
{
	return true;
}
...
public:
	Point3       InverseLattice(Point3 p) const override { return p; }
	Point3       GetSourcePoint(int i);
	int          GetNameID() const override { return IDS_RB_FFDRECT; }
	const TCHAR* GetNonLocalizedName() const override { return _T("FFD(box)"); }
};

template <class T>
class FFDNMCyl : public FFDNM<T> {
public:
	Point3       InverseLattice(Point3 p) const override;
	Point3       GetSourcePoint(int i);
	int          GetNameID() const override { return IDS_RB_FFDCYL; }
	const TCHAR* GetNonLocalizedName() const override { return _T("FFD(cyl)"); }
	int          GetSetDimDlgID() { return IDD_FFD_SETDIMCYL; }
	BOOL         WrapX() const override { return TRUE; }
...
public:
	...
	void        GetClassName(MSTR& s, bool localized) const override { s = this->GetObjectName(localized); }
...
public:
	...
	FFDNMSquareWSObj(BOOL isShallowCopy = FALSE);
	Class_ID ClassID() { return FFDNMWSSQUARE_CLASS_ID; }
	void     GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_RB_FFDRECT_C) : _T("FFD(Box)"); }
...
public:
	...
	FFDNMCylWSObj(BOOL isShallowCopy = FALSE);
	Class_ID ClassID() { return FFDNMWSCYL_CLASS_ID; }
	void     GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_RB_FFDCYL_C) : _T("FFD(Cyl)"); }
	Modifier *CreateWSMMod(INode *node);
	CreateMouseCallBack* GetCreateMouseCallBack();
...
public:
	...
	// From Animatable
	void     GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_RB_FFDNMWSMOD) : _T("FFD Binding"); }
	SClass_ID   SuperClassID() { return WSM_CLASS_ID; }
	Class_ID ClassID() { return FFDNMWSSQUARE_MOD_CLASS_ID; }
	void     DeleteThis() { delete this; }
	RefTargetHandle Clone(RemapDir &remap);

	const TCHAR    *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_FFDNMWSMOD) : _T("FFD Binding"); }
...
public:
	...
	const TCHAR*  NonLocalizedClassName() { return _T("FFD(box)"); }
...
public:
	...
	const TCHAR*  NonLocalizedClassName() { return _T("FFD Binding"); }
	SClass_ID      SuperClassID() { return WSM_CLASS_ID; }
	Class_ID    ClassID() { return FFDNMWSSQUARE_MOD_CLASS_ID; }
...
public:
	...
	const TCHAR*  NonLocalizedClassName() { return _T("FFD(cyl)"); }
	SClass_ID      SuperClassID() { return OSM_CLASS_ID; }
	Class_ID    ClassID() { return FFDNMOSCYL_CLASS_ID; }
	const TCHAR*   Category() { return GetString(IDS_RB_DEFDEFORMATIONS); }
...
public:
	...
	void Restore(int isUndo) {
		...
		for (int i = 0; i < 3; i++) ffd->dim[i] = undoDim[i];
		ffd->ptCont = undoCont;
		ffd->pointCtrlContainer->SetNumSubControllers(undoCont.Count());
		ffd->pt = undoPt;
...
enum { ffd_params };
enum {
	ffd_disp_lattice, ffd_disp_source, ffd_deform, ffd_falloff,
...
static ParamBlockDesc2 ffdos_box_param_blk(ffd_params, _T("ffdparameters"), 0, &ffdNMSquareOSDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	...
	ffd_disp_lattice, _T("dispLattice"), TYPE_BOOL, 0, IDS_RK_SHOWLATTICE,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_FFD_SHOWLATTICE,
		p_end,
	ffd_disp_source, _T("dispSource"), TYPE_BOOL, 0, IDS_RK_SHOWSOURCE,
		p_default, FALSE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_FFD_SHOWSOURCE,
		p_end,
	...
	ffd_inpoints, _T("inPoints"), TYPE_BOOL, 0, IDS_RK_INPOINTS,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_FFD_INPOINTS,
		p_end,
	ffd_outpoints, _T("outPoints"), TYPE_BOOL, 0, IDS_RK_OUTPOINTS,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_FFD_OUTPOINTS,
		p_end,
...
static ParamBlockDesc2 ffdws_box_param_blk(ffd_params, _T("ffdparameters"), 0, &ffdNMSquareWSDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	...
	ffd_cont, _T("continuity"), TYPE_FLOAT, P_ANIMATABLE, IDS_RK_CONT,
		...
		p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_FFD_CONT, IDC_FFD_CONTSPIN, 0.1f,
		p_end,
	ffd_length, _T("length"), TYPE_WORLD, P_ANIMATABLE, IDS_RK_LENGTH,
		p_default, 0.0,
		p_range, 0.0f, float(1.0E30),
		p_ui, TYPE_SPINNER, EDITTYPE_UNIVERSE, IDC_FFD_LENGTH, IDC_FFD_LENGTHSPIN, SPIN_AUTOSCALE,
		p_end,
	ffd_width, _T("width"), TYPE_WORLD, P_ANIMATABLE, IDS_RK_WIDTH,
		p_default, 0.0,
		p_range, 0.0f, float(1.0E30),
		p_ui, TYPE_SPINNER, EDITTYPE_UNIVERSE, IDC_FFD_WIDTH, IDC_FFD_WIDTHSPIN, SPIN_AUTOSCALE,
		p_end,
	ffd_height, _T("height"), TYPE_WORLD, P_ANIMATABLE, IDS_RK_HEIGHT,
		p_default, 0.0,
		p_range, 0.0f, float(1.0E30),
...
static ParamBlockDesc2 ffdws_cyl_param_blk(ffd_params, _T("ffdparameters"), 0, &ffdNMCylWSDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	...
	ffd_cont, _T("continuity"), TYPE_FLOAT, P_ANIMATABLE, IDS_RK_CONT,
		...
		p_end,
	//radius
	ffd_length, _T("radius"), TYPE_WORLD, P_ANIMATABLE, IDS_RB_RADIUS,
		p_default, 0.0,
		p_range, 0.0f, float(1.0E30),
		p_ui, TYPE_SPINNER, EDITTYPE_UNIVERSE, IDC_FFD_LENGTH, IDC_FFD_LENGTHSPIN, SPIN_AUTOSCALE,
		p_end,
	// height
	ffd_width, _T("height"), TYPE_WORLD, P_ANIMATABLE, IDS_RB_HEIGHT,
		p_default, 0.0,
		p_range, 0.0f, float(1.0E30),
		p_ui, TYPE_SPINNER, EDITTYPE_UNIVERSE, IDC_FFD_WIDTH, IDC_FFD_WIDTHSPIN, SPIN_AUTOSCALE,
		p_end,
...
template <class T>
INT_PTR FFDDlgProc<T>::DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg) {
	case WM_INITDIALOG:
		InitDlg(hWnd);
...
template <class T>
void FFDNM<T>::BaseClone(FFDNM<T> *to, RemapDir &remap)
{
	if (to->isShallowCopy) {
		DbgAssert(to->tmControl == nullptr && to->pblock == nullptr && to->pointCtrlContainer == nullptr);
		to->tmControl = tmControl;
		to->pblock = pblock;
		to->pointCtrlContainer = pointCtrlContainer;
		to->ptCont = ptCont;
	}
	else {
		if (tmControl) to->ReplaceReference(0, remap.CloneRef(tmControl));
		to->ReplaceReference(1, remap.CloneRef(pblock));
		// NOTE: DriverPointControlImp::Clone does not clone the controllers it holds. No idea why not.... 
		to->ReplaceReference(2, remap.CloneRef(pointCtrlContainer));
		int numPts = NumPts();
		to->ptCont.SetCount(numPts);
		for (int i = 0; i < numPts; i++)
			to->ptCont[i] = NULL;
		for (int i = 0; i < numPts; i++) {
			Control* pCont = ptCont[i];
			if (pCont)
				to->ReplaceReference(i + 3, remap.CloneRef(pCont)); // this sets element in ptCont and pointCtrlContainer
		}
	}
	to->pt = pt;
	to->offsets = offsets;
	to->sel = sel;
	to->selLevel = selLevel;
	to->dim[0] = dim[0];
	to->dim[1] = dim[1];
	to->dim[2] = dim[2];
	to->lbox = lbox;
	to->beenDeformed = beenDeformed;
}
...
{
	BOOL notify = FALSE;
	// Plug-in controllers for selected points without controllers if we're animating
	...
	if (Key) {  // RK: Adding option to animate all
		...
		for (int i = 0; i < NumPts(); i++) {
			if ((all || sel[i]) && !GetPtCont(i)) {
				this->ReplaceReference(3 + i, NewDefaultPoint3Controller());
				theHold.Suspend();
				GetPtCont(i)->SetValue(0, &pt[i], TRUE, CTRL_ABSOLUTE);
				theHold.Resume();
				pointCtrlContainer->SetSubController(i, GetPtCont(i));
				notify = TRUE;
			}
		}
		ResumeAnimate();
	...
	if (notify) this->NotifyDependents(FOREVER, 0, REFMSG_SUBANIM_STRUCTURE_CHANGED);
}
...
void FFDNM<T>::Conform()
{
	...
	if (this->SuperClassID() == WSM_OBJECT_CLASS_ID || (dep.Nodes.Count() != 1) ||
...
template <class T>
int FFDNM<T>::GridDim(int which) const
{
	return dim[which];
}
...
void FFDNM<T>::SetGridDim(int dm[3])
{
	...
	pointCtrlContainer->SetNumSubControllers(total);
	pt.SetCount(total);
	...
	this->NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
	this->NotifyDependents(FOREVER, PART_ALL, REFMSG_NODE_NAMECHANGE);
	this->NotifyDependents(FOREVER, (PartID)this, REFMSG_BRANCHED_HISTORY_CHANGED);
	this->NotifyDependents(FOREVER, PART_ALL, REFMSG_SUBANIM_STRUCTURE_CHANGED);
}
...
{
	this->ReplaceReference(subAnim, (RefTargetHandle)control);
	if (subAnim == POINTCTRLCONTAINER_REF) {
		int n = NumPtConts();
		pointCtrlContainer->SetNumSubControllers(n);
		for (int i = 0; i < n; i++)
			if (GetPtCont(i)) pointCtrlContainer->SetSubController(i, GetPtCont(i));
	}
	this->NotifyDependents(FOREVER, 0, REFMSG_CONTROLREF_CHANGE);
	this->NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
	return TRUE;
}
...
void  FFDNM<T>::SetReference(int i, RefTargetHandle rtarg)
{
	if (i == 0)
		...
		if (pointCtrlContainer)
			pointCtrlContainer->SetNumSubControllers(NumPtConts());
	}
	else if (i < NumPts() + 3) {
...
template <class T>
TSTR FFDNM<T>::SubAnimName(int i, bool localized)
{
	if (i == 0)
	{
		return localized ? GetString(IDS_RB_LATTICETM) : _T("Lattice Transform");
	}
	else if (i == 1)
	{
		return localized ? GetString(IDS_RB_PARAMETERS) : _T("FFD Parameters");
	}
	else if (i == 2)
	{
		return localized ? GetString(IDS_RK_POINTCTRLCONTAINER) : _T("Point Controller Container");
	}
	else if (i < NumPts() + 3)
	{
		TSTR str;
		str.printf((localized ? GetString(IDS_RB_CONTROLPOINTN) : _T("Control Point %d")), i - 2);
		return str;
	}
	else
		return TSTR();
	...
	int flags, ModContext *mc)
{
	...
	// Draw...
	if (inode->Selected() && this->SuperClassID() != OSM_CLASS_ID &&
...
void FFDNM<T>::SelectPt(int i, BOOL s, BOOL clearAll)
{
	...
	this->NotifyDependents(FOREVER, PART_SELECT, REFMSG_CHANGE);
}
...
void  FFDNM<T>::ActivateSubobjSel(int level, XFormModes& modes)
{
	...
	this->NotifyDependents(FOREVER, PART_SELECT | PART_DISPLAY | PART_SUBSEL_TYPE, REFMSG_CHANGE);
}
	...
	Point3& val, BOOL localOrigin)
{
	...
	if (level == SEL_LATTICE) {
		...
		// Hold for undo
		if (theHold.Holding() && !this->TestAFlag(A_HELD)) {
			theHold.Put(new MoveRestore<T>(this));
			this->SetAFlag(A_HELD);
		}
...
template <class T>
const TCHAR *FFDNM<T>::GetObjectName(bool localized) const
{
	static TSTR buf;
	buf.printf(_T("%s %dx%dx%d"), localized ? GetString(GetNameID()) : GetNonLocalizedName(), GridDim(1), GridDim(0), GridDim(2));
	return buf;
}
...
{
	// Compute a the transformation out of lattice space into world space   
	// When a WS mod is applied to an FFD WS obj, the points are 
...
template <class T>
void FFDNM<T>::DeleteThis()
{
	HoldSuspend hs;
	this->DeleteAllRefsFromMe();
	delete this;
}
	...
	IObjParam *ip, ULONG flags, Animatable *prev)
{
	...
	if (this->SuperClassID() == OSM_CLASS_ID) {

		// Notify the system that we have an apparatus to display
		TimeValue t = ip->GetTime();
		this->NotifyDependents(Interval(t, t), PART_ALL, REFMSG_BEGIN_EDIT);
		this->NotifyDependents(Interval(t, t), PART_ALL, REFMSG_MOD_DISPLAY_ON);
		this->SetAFlag(A_MOD_BEING_EDITED);
	}

	if (this->SuperClassID() == WSM_OBJECT_CLASS_ID) {
	...
	IObjParam *ip, ULONG flags, Animatable *next)
{
	...
	if (this->SuperClassID() == OSM_CLASS_ID) {
		TimeValue t = ip->GetTime();
		this->ClearAFlag(A_MOD_BEING_EDITED);
		this->NotifyDependents(Interval(t, t), PART_ALL, REFMSG_END_EDIT);
		this->NotifyDependents(Interval(t, t), PART_ALL, REFMSG_MOD_DISPLAY_OFF);
	}
...
ISubObjType *FFDNM<T>::GetSubObjType(int i)
{
	...
	{
	case -1:
		if (this->GetSubObjectLevel() > 0)
			return GetSubObjType(this->GetSubObjectLevel() - 1);
		break;
	case 0:
	...
	FFDNM<T> *ffd, TimeValue t, Matrix3 *mtm, Box3 &box, BOOL ws)
{
	...
	for (int i = 0; i < ffd->dim[0] + (ffd->WrapX() ? 0 : 2); i++) {
...
static void InterpSpline(Point3& p, const float v[4], const Point3 knot[4], float m1, float m2)
{
	float c[4];
	c[0] = -v[2] * m2;
...
template <class T>
Point3 FFDNMDeformer<T>::Map(int ii, Point3 p) const
{
	if (!ffd)
		return p;

	const auto* const_ffd = ffd;

	// Transform into lattice space
	Point3 pp = p * tm;

	// Allow derived classes to do additional inverse transforms
	pp = const_ffd->InverseLattice(pp);

	// Cache flag if we're wrapping X
	bool bWrapX = const_ffd->WrapX();
	...
	if (inVol)
	{
		if (bWrapX)
		{
			if (pp.y < -0.001f || pp.y > 1.001f || pp.z < -0.001f || pp.z > 1.001f)
			{
				return p;
			}
		}
	...
	else if (falloff > 0.0f)
	{
		...
		if (!bWrapX)
		{
			if (pp.x < 0.0f && -pp.x > closest)
			{
				closest = -pp.x;
				found = TRUE;
			}
			if (pp.x > 1.0f && (pp.x - 1.0f) > closest)
			{
				closest = pp.x - 1.0f;
				found = TRUE;
			}
		}
		if (pp.y < 0.0f && -pp.y > closest)
		{
			closest = -pp.y;
			found = TRUE;
		}
		if (pp.y > 1.0f && (pp.y - 1.0f) > closest)
		{
			closest = pp.y - 1.0f;
			found = TRUE;
		}
		if (pp.z < 0.0f && -pp.z > closest)
		{
			closest = -pp.z;
			found = TRUE;
		}
		if (pp.z > 1.0f && (pp.z - 1.0f) > closest)
		{
			closest = pp.z - 1.0f;
			found = TRUE;
		}
	...
	if (_isnan(pp.x) || _isnan(pp.y) || _isnan(pp.z))
	{
		// MAXX-46938 - Fix FFDCyl crash in mcg_invalidobject.ms test:
		// Abort to avoid out of bounds array indexing (see pts[ii] below).
		// Related to NaN or infinity input values, either in 'p', or which affect input bounding box
		// calculations in ModAppEvaluator::RecalcDeformBBox() -> ... -> Mesh::getBoundingBox().
		return p;
	}

	// Find the cell we're in
	pp.x = pp.x * float(const_ffd->dim[0] - (bWrapX ? 0 : 1));
	pp.y = pp.y * float(const_ffd->dim[1] - 1);
	pp.z = pp.z * float(const_ffd->dim[2] - 1);
	int i = int(pp.x);
	int j = int(pp.y);
	int k = int(pp.z);
	...
	if (!bWrapX)
	{
		if (i < 0)
			i = 0;
		if (i > const_ffd->dim[0] - 2)
			i = const_ffd->dim[0] - 2;
	}
	if (j < 0)
		j = 0;
	if (j > const_ffd->dim[1] - 2)
		j = const_ffd->dim[1] - 2;
	if (k < 0)
		k = 0;
	if (k > const_ffd->dim[2] - 2)
		k = const_ffd->dim[2] - 2;
	...
	float fHermiteWeightsZ[4];
	ComputeHermiteBasis(pp.z, fHermiteWeightsZ);
	float fHermiteWeightsY[4];
	ComputeHermiteBasis(pp.y, fHermiteWeightsY);

	int dim0Check = dim[0] + (bWrapX ? 0 : 2);
	...
	for (int ix = 0; ix < 4; ix++)
	{
		...
		DbgAssert(ii >= 00 && ii < dim0Check);
		if (ii < 0 || ii >= dim0Check)
		{
			return p; // MAXX-46938. Shouldn't get here. Abort to avoid index out of bounds.
		}

		for (int jx = 0; jx < 4; jx++)
		{
			InterpSpline(pt[jx], fHermiteWeightsZ, &(pts[ii][j+jx][k]), m1, m2);
		}
		// Interpolate 16 points to get a single 4 point spline
		InterpSpline(knots[ix], fHermiteWeightsY, pt, m1, m2);
	}

	// Finally we get the point
	float fHermiteWeightsX[4];
	ComputeHermiteBasis(pp.x, fHermiteWeightsX);
	InterpSpline(pp, fHermiteWeightsX, knots, m1, m2);
	pp = pp * itm;
...
Point3 FFDNMSquare<T>::GetSourcePoint(int i)
{
	...
	if (this->dim[0] == 0 || this->dim[1] == 0) return Point3(0, 0, 0);

	k = i / (this->dim[0] * this->dim[1]);
	i -= k * (this->dim[0] * this->dim[1]);
	j = i / this->dim[0];
	i -= j * this->dim[0];

	return Point3(
		float(i) / float(this->dim[0] - 1),
		float(j) / float(this->dim[1] - 1),
		float(k) / float(this->dim[2] - 1));
}
...
Point3 FFDNMCyl<T>::GetSourcePoint(int i)
{
	...
	float u = float(i) / float(this->dim[0]);
	float v = float(j) / float(this->dim[1] - 1);
...
template <class T>
Point3 FFDNMCyl<T>::InverseLattice(Point3 p) const
{
	float x = p.x - 0.5f;
	float y = p.y - 0.5f;
...
FFDNMOSMod<T>::FFDNMOSMod()
{
	this->tmControl = NULL;
	this->pblock = NULL;
	this->pointCtrlContainer = NULL;

	this->ReplaceReference(TM_REF, NewDefaultMatrix3Controller());

	// Create the ffd Point Controller Container
	this->ReplaceReference(POINTCTRLCONTAINER_REF, NewDefaultPointControllerContainer());
	int dm[3] = { 4,4,4 };
	this->SetGridDim(dm);
}
	...
	TimeValue t, ModContext &mc, ObjectState *os, INode *node)
{
	...
	// This will intersect our validity with the object's validity.
...
FFDNMSquareWSObj::FFDNMSquareWSObj(BOOL isShallowCopy)
{
	...
	if (!isShallowCopy) {
		GetClassDesc()->MakeAutoParamBlocks(this);
		ReplaceReference(POINTCTRLCONTAINER_REF, NewDefaultPointControllerContainer());
		int dm[3] = { 4,4,4 };
		SetGridDim(dm);
	}
}

FFDNMCylWSObj::FFDNMCylWSObj(BOOL isShallowCopy)
{
	geomValid = selValid = FOREVER;
	selLevelValid = TRUE;
	this->isShallowCopy = isShallowCopy;
...
{
	return IsSubClassOf(obtype) || obtype == defObjectClassID || obtype == anyObjectClassID;
}
...
{
	if (IsSubClassOf(obtype) || obtype == defObjectClassID || obtype == anyObjectClassID)
		return this;
	return NULL;
}
	...
	const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
	...
	else return SimpleWSMMod::NotifyRefChanged(changeInt, hTarget, partID, message, propagate);;
}
...
public:
	...
	// From Animatable
	void DeleteThis() { delete this; }
	void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_RB_FFDSELMOD) : _T("FFD Select"); }
	virtual Class_ID ClassID() { return FFD_SELECT_CLASS_ID; }
	RefTargetHandle Clone(RemapDir& remap);
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_FFDSELMOD) : _T("FFD Select"); }

	// From modifier
	ChannelMask ChannelsUsed() { return GEOM_CHANNEL | SELECT_CHANNEL | SUBSEL_TYPE_CHANNEL; }
...
public:
	...
	const TCHAR*  NonLocalizedClassName() { return _T("FFD Select"); }
	SClass_ID      SuperClassID() { return OSM_CLASS_ID; }
...
void FFDSelMod::NotifyInputChanged(
	const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc)
{
	if (mc->localData && ip) {
		((FFDSelData*)mc->localData)->FreeCache();
		TimeValue t = ip->GetTime();
		NotifyDependents(Interval(t, t), PART_GEOM | PART_SELECT | PART_SUBSEL_TYPE, REFMSG_MOD_EVAL);
	}
}
...
static INT_PTR CALLBACK FFDSelProc(
	HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	FFDSelMod *mod = DLGetWindowLongPtr<FFDSelMod*>(hWnd);
	switch (msg) {
	case WM_INITDIALOG: {
