...
EditSSMode*			EditMeshMod::editSSMode = NULL;
ActionMeshCB*		EditMeshMod::mpActions  = NULL;
Tab<ActionItem*>	EditMeshMod::mOverrides;
...
BOOL EditMeshMod::deleteIsolatedVerts = TRUE;
BOOL EditMeshMod::ignoreBackfaces = FALSE;
BOOL EditMeshMod::ignoreVisEdge = FALSE;
BOOL EditMeshMod::selByVert = FALSE;
int EditMeshMod::weldBoxSize = 0;
int EditMeshMod::pickBoxSize = 0;
BOOL EditMeshMod::weldBoxSizeInitialized = FALSE;
...
void EditMeshClassDesc::ResetClassParams(BOOL fileReset) {
	...
	EditMeshMod::deleteIsolatedVerts = TRUE;
...
void EditMeshData::Apply (TimeValue t, TriObject *triOb, EditMeshMod *mod) {
	...
	if (!GetFlag(EMD_UPDATING_CACHE) && MeshCached(t)) {
		...
		triOb->GetMesh().DeepCopy (GetMesh(t),
			GEOM_CHANNEL|SELECT_CHANNEL|SUBSEL_TYPE_CHANNEL|
			DISP_ATTRIB_CHANNEL|TOPO_CHANNEL|TEXMAP_CHANNEL|VERTCOLOR_CHANNEL);
	...
	if (mdelta.mapSupport.GetSize()>0) {
		int vc=0;
	...
	if (mod->affectRegion && mod->selLevel) {
		...
		if (vsw) {
			...
			triOb->GetMesh().SetFlag(MN_MESH_SOFTSELECTION_WEIGHT_INVALID);
		}
	}
}
...
void EditMeshData::SetVertSel (const BitArray &set, IMeshSelect *mod, TimeValue t) {
	...
	Invalidate (SELECT_CHANNEL);
}

void EditMeshData::SetFaceSel(const BitArray &set, IMeshSelect *mod, TimeValue t) {
	EditMeshMod *em = (EditMeshMod *) mod;
	BeginEdit (t);
...
void EditMeshData::SetEdgeSel(const BitArray &set, IMeshSelect *mod, TimeValue t) {
	EditMeshMod *em = (EditMeshMod *) mod;
	BeginEdit (t);
...
void EditMeshData::SetSel (int nsl, const BitArray & set, IMeshSelect *mod, TimeValue t) {
	switch (nsl) {
...
void EditMeshData::ApplyMeshDelta (MeshDelta & md, MeshDeltaUser *mdu, TimeValue t) {
	...
	if (updateMD) partsChanged |= (TOPO_CHANNEL|GEOM_CHANNEL|SELECT_CHANNEL|TEXMAP_CHANNEL|VERTCOLOR_CHANNEL);
...
public:
	...
	BOOL proc(ModContext *mc) {
		...
		for (int i=0; i<md.vClone.Count(); i++) md.vClone[i].dv *= f;
		return TRUE;
	}
};
...
RefTargetHandle EditMeshMod::Clone(RemapDir& remap) {
	...
	for (int i=0; i<3; i++)
	{
		...
		for (int j=0; j<ids[i].Count(); j++) newmod->ids[i][j] = ids[i][j];
	}
...
void EditMeshMod::ModifyObject(TimeValue t, ModContext &mc, ObjectState * os, INode *node) {
	...
	// If we have specified normals, they should be reasonably well supported by the MeshDelta.
	// So mark them as supported, to prevent them from being removed:
...
void EditMeshMod::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc) {
	if ( mc->localData ) {
		EditMeshData *meshData = (EditMeshData*)mc->localData;
			...
			*/
			...
			if (msci) {
				...
				if (selByVert) 
				{
					...
					{
						if (meshData != hr->modContext->localData) continue;
						if (mesh->VertSel()[hr->hitInfo] && maybeSkip)
					...
					{
						pConv->VertexToElement(*mesh, af, vhit, nsel);
					}
					else
					{
						pConv->VertexToPolygon(*mesh, af, vhit, nsel, GetPolyFaceThresh(), localIgnoreVisEdge);
					}
				} 
				else 
				{
					...
					for (hr=hitRec; hr!=NULL; hr=hr->Next()) 
					{
						if (meshData != hr->modContext->localData) 
							continue;
						if (mesh->FaceSel()[hr->hitInfo] && maybeSkip)
							localIgnoreVisEdge = false;
						fhit.Set (hr->hitInfo);
						if (!all) 
							break;
					...
					if (selLevel == SL_ELEMENT) 
					{
						pConv->FaceToElement (*mesh, af, fhit, nsel);
					}
					else
					{
						pConv->FaceToPolygon (*mesh, af, fhit, nsel, GetPolyFaceThresh(), localIgnoreVisEdge);
					}
				}
			} 
			else 
			{
				...
				for (; hr != NULL; hr=hr->Next()) {
					...
					if (!localSelByVert) 
					{
						...
						{
							mesh->PolyFromFace (hr->hitInfo, nsel, GetPolyFaceThresh(), localIgnoreVisEdge, af);
						}
					} 
					else 
					{
						...
						for (int i=0; i<list.Count(); i++) 
						{
							...
							for (int j=0; j<2; j++) 
							{
								...
								if (selLevel==SL_ELEMENT) 
									...
									mesh->PolyFromFace (me.f[j], nsel, GetPolyFaceThresh(), localIgnoreVisEdge, af);
							}
						}
					}
					if (!all) break;
...
void EditMeshMod::LocalDataChanged (ChannelMask channels) {
	NotifyDependents(FOREVER, channels, REFMSG_CHANGE);
	if (!ip) return;
	ip->ClearCurNamedSelSet();
	InvalidateSurfaceUI ();
	if (channels & SELECT_CHANNEL) InvalidateNumberSelected ();
}
...
void EditMeshMod::RemoveSet (TSTR &setName, int nsl) {
	...
	DebugPrint (_T("EMM: RemoveSet (%s, %d)\n"), setName, nsl);
#endif
	int i;
...
#ifdef NAMED_SELECTION_DEBUG_PRINT
	DebugPrint (_T("EMM: ActivateSubSelSet (%s)\n"), setName);
#endif
	if (selLevel == SL_OBJECT) return;
...
void EditMeshMod::NewSetFromCurSel(TSTR &setName) {
	...
	int i;
#ifdef NAMED_SELECTION_DEBUG_PRINT
	DebugPrint (_T("EditMeshMod::NewSetFromCurSel (%s) - "), setName);
	switch (nsl) {
	case NS_VERTEX: DebugPrint (_T("Vertex Level\n")); break;
	case NS_EDGE: DebugPrint (_T("Edge Level\n")); break;
	default: DebugPrint (_T("Face Level\n")); break;
	}
	DebugPrint (_T("Modifier list of name sets:\n"));
	for (i=0; i<namedSel[nsl].Count(); i++) {
		DebugPrint (_T("Name: %s; ID: %d\n"), *(namedSel[nsl][i]), ids[nsl][i]);
	}
#endif
	...
	for (i = 0; i < mcList.Count(); i++) {
		...
		meshData->BeginEdit (ip->GetTime());
		BitArray *set = NULL;
		if (index>=0 && (set = meshData->selSet[nsl].GetSet(id)) != NULL) *set = meshData->GetSel(nsl);
...
void EditMeshMod::RemoveSubSelSet(TSTR &setName) {
	//DebugPrint (_T("EMM: RemoveSubSelSet (%s)\n"), setName);
	int nsl = namedSetLevel[selLevel];
	int index = FindSet (setName, nsl);
	if (index<0 || !ip) return;
	EndExtrude(ip->GetTime());
...
void EditMeshMod::SetupNamedSelDropDown () {
	//DebugPrint (_T("EMM: SetupNamedSelDropDown\n"));
	if (selLevel == SL_OBJECT) return;
...
int EditMeshMod::NumNamedSelSets () {
	//DebugPrint (_T("EMM: NumNamedSelSets()="));
	int nsl = namedSetLevel[selLevel];
...
void EditMeshMod::SetNamedSelSetName (int i, TSTR & newName) {
	//DebugPrint (_T("EMM: SetNamedSelSetName (%d, %s)\n"), i, newName);
	int nsl = namedSetLevel[selLevel];
...
void EditMeshMod::NewSetByOperator (TSTR &newName, Tab<int> &sets, int op) {
	//DebugPrint (_T("EMM: NewSetByOperator (%s, %d, %d)\n"), newName, sets.Count(), op);
	int nsl = namedSetLevel[selLevel];
	...
	int i;
#ifdef NAMED_SELECTION_DEBUG_PRINT
	DebugPrint (_T("EditMeshMod::NewSetByOperator (%s) - "), newName);
	switch (nsl) {
	case NS_VERTEX: DebugPrint (_T("Vertex Level\n")); break;
	case NS_EDGE: DebugPrint (_T("Edge Level\n")); break;
	default: DebugPrint (_T("Face Level\n")); break;
...
void EditMeshMod::ActivateSubobjSel(int level, XFormModes& modes ) {	
	...
	if (selLevel != old) {
		...
		for ( int i = 0; i < mcList.Count(); i++ ) {
			...
			meshData->Invalidate(SELECT_CHANNEL);
		}
...
int EditMeshMod::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags, ModContext *mc) {	
	...
	if (inBuildFace && CheckNodeSelection (ip, inode)
		&& (mc->localData == createFaceMode->proc.GetMeshData()) && mc->localData) {
				...
				while (IO_OK==(res=iload->OpenChunk())) {
					switch(iload->CurChunkID())  {
						...
						case TOPO_FACEMAP_CHUNK:
							...
							res = iload->ReadVoid(ctmd->td.map.Addr(0),n*sizeof(FaceMap),&nb);
							break;
						...
						case TOPO_TVFACEMAP_CHUNK:
							...
							res = iload->ReadVoid(ctmd->td.tmap.Addr(0),n*sizeof(FaceMap),&nb);
							break;
...
#ifdef _DEBUG
		if (!ret) DebugPrint (_T("Edit Mesh: Repairing MeshDelta internal order on load.\n"));
#endif
		ret = em->mdelta.CheckMapFaces ();
#ifdef _DEBUG
		if (!ret) DebugPrint (_T("Edit Mesh: Repairing MeshDelta mapping faces on load.\n"));
#endif
	}
	return IO_OK;
	}
	...
	for (int i=0; i<mcList.Count(); i++) {
		...
		if (emd->GetMesh(t))
		{
			...
			if (selLevel == SL_VERTEX)
				ct = emd->GetMesh(t)->VertSel().NumberSet();
			else if (selLevel == SL_EDGE)
				ct = emd->GetMesh(t)->EdgeSel().NumberSet();
			else 
				ct = emd->GetMesh(t)->FaceSel().NumberSet();
			if (ct > 0)
				return true;
		}
	}
...
int EditMeshMod::GetMaxMtlID(ModContext* mc) {
	...
	for (int i=0; i<m->numFaces; i++) mtlID = std::max(mtlID, m->faces[i].getMatID());
	return mtlID;
}
...
void ConvertToMeshDeltaPLCB::MyDebugPrint () {
	...
	DebugPrint (_T("Expecting pipeline mesh of %d verts, %d faces\n"), td.inputVerts, td.inputFaces);
	int i = 0;
	int num = td.cverts.Count();
	if (num!=0) {
		DebugPrint (_T("%d Vertex Clones:\n  "), num);
		for (i=0; i<num; i++) {
			DebugPrint (_T("%2d "), td.cverts[i]);
			if ((i<num-1) && (i%10==9)) DebugPrint (_T("\n  "));
		}
		DebugPrint (_T("\n"));
	}
	num=td.dverts.Count();
	if (num!=0) {
		DebugPrint (_T("%d Vertex Deletes:\n  "), num);
		for (i=0; i<num; i++) {
			DebugPrint (_T("%2d "), td.dverts[i]);
			if ((i<num-1) && (i%10==9)) DebugPrint (_T("\n  "));
		}
		DebugPrint (_T("\n"));
	}
	num=td.nverts.Count();
	if (num!=0) {
		DebugPrint (_T("%d Vertex Creates:\n"), num);
		for (i=0; i<num; i++) {
			DebugPrint (_T("  %7.3f, %7.3f, %7.3f\n"), td.nverts[i].x, td.nverts[i].y, td.nverts[i].z);
		}
	}
	num=vd.deltas.Count();
	if (num!=0) {
		DebugPrint (_T("Vertex Moves:\n"));
		for (i=0; i<num; i++) {
			if (vd.deltas[i] == Point3(0,0,0)) continue;
			DebugPrint (_T("  %d: %7.3f, %7.3f, %7.3f\n"), i, vd.deltas[i].x, vd.deltas[i].y, vd.deltas[i].z);
		}
	}
	num=td.map.Count();
	if (num!=0) {
		DebugPrint (_T("Face Remaps:\n"));
		for (int i=0; i<num; i++) {
			DebugPrint (_T("  %d: %3d, %3d, %3d\n"), i, td.map[i].v[0], td.map[i].v[1], td.map[i].v[2]);
		}
	}
	num=td.nfaces.Count();
	if (num!=0) {
		DebugPrint (_T("Face Creates:\n"));
		for (i=0; i<num; i++) {
			DebugPrint (_T("  %3d, %3d, %3d\n"), td.nfaces[i].v[0], td.nfaces[i].v[1], td.nfaces[i].v[2]);
		}
	}
}
	...
	if (emd->flags & EMD_HASDATA) {
		...
		if (td.map.Count()) {
			...
			for (i=0; i<td.map.Count(); i++) {
				...
				temp.f = md.FLut(i);
				if (temp.f>=md.fnum) {
					DbgAssert (temp.f < md.fnum + (DWORD)md.NumFCreate());
		...
		if (md.mapSupport[1] && td.tmap.Count()) {
			for (i=0; i<td.tmap.Count(); i++) {
				...
				temp.f = i;
				if (temp.f>=md.fnum) {
					DbgAssert (temp.f < md.fnum + (DWORD)md.map[1].fCreate.Count());
					temp.Apply (md.fCreate[temp.f-md.fnum].face);
		...
		if (td.attribs.Count()) {
			...
			for (i=0; i<td.attribs.Count(); i++) {
				...
				if (fcTemp.flags) {
					fcTemp.f = md.FLut(i);
					if (fcTemp.f >= md.fnum) {
						DbgAssert (fcTemp.f < md.fnum + (DWORD)md.NumFCreate());
						temp.Apply (md.fCreate[temp.f-md.fnum].face);
...
void EditMeshMod::DoAccept(TimeValue t)
{

}

void EditMeshMod::SetPinch(TimeValue t, float ipinch)
{
...
void EditMeshMod::SetBubble(TimeValue t, float ibubble)
{
...
void EditMeshMod::SetFalloff(TimeValue t,float ifalloff)
{
...
float EditMeshMod::GetPinch(TimeValue t)
{
	ISpinnerControl* spin = GetISpinner (GetDlgItem (hAR, IDC_PINCHSPIN));
...
float EditMeshMod::GetBubble(TimeValue t)
{
	ISpinnerControl* spin = GetISpinner (GetDlgItem (hAR, IDC_BUBBLESPIN));
	float val = spin->GetFVal();
	ReleaseISpinner(spin);
...
float EditMeshMod::GetFalloff(TimeValue t)
{
	ISpinnerControl* spin = GetISpinner (GetDlgItem (hAR, IDC_FALLOFFSPIN));
	float val = spin->GetFVal();
	ReleaseISpinner(spin);
...
void EditMeshMod::EditSoftSelectionMode()
{
