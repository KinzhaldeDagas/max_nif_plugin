...
const ActionTableId kActionID = 0x17d423db;

class ActionMeshCB : public ActionCallback {
	EditMeshMod *mpMod;
public:
	ActionMeshCB (EditMeshMod *m) : mpMod(m) { }
	BOOL ExecuteAction(int id);
};

BOOL ActionMeshCB::ExecuteAction (int id) {
	switch (id) {
	case ID_EDIT_SS:
...
ActionTable* GetEditMeshActions() {
	TSTR name = GetString (IDS_RB_EDITMESH);
	...
	GetCOREInterface()->GetActionManager()->RegisterActionContext(kActionID, 
...
class EditMeshSSOverride : public IActionItemOverride 
{
protected:
	BOOL mActiveOverride;
	EditMeshMod *mpObj;
public:
	EditMeshSSOverride(EditMeshMod *m):mActiveOverride(FALSE),mpObj(m){}
	BOOL IsOverrideActive();
	BOOL StartOverride();
	BOOL EndOverride();
};

BOOL EditMeshSSOverride::IsOverrideActive()
{
	return mActiveOverride;
}


BOOL EditMeshSSOverride::StartOverride()
{
...
BOOL EditMeshSSOverride::EndOverride()
{
...
static GenSubObjType SOT_Vertex(NULL, EditMeshIcon::stackVertex, 0);
static GenSubObjType SOT_Edge(NULL, EditMeshIcon::stackEdge, 0);
static GenSubObjType SOT_Face(NULL, EditMeshIcon::stackFace, 0);
static GenSubObjType SOT_Poly(NULL, EditMeshIcon::stackPolygon, 0);
static GenSubObjType SOT_Element(NULL, EditMeshIcon::stackElement, 0);
...
void EditMeshMod::BeginEditParams( IObjParam *ip, ULONG flags,Animatable *prev ) {
   ...
   if ( !hGeom ) {
      ...
      DLSetWindowLongPtr( hGeom, this);
      DLSetWindowLongPtr(hSel, this);
   }
   SetFlag (EM_EDITING);
   ...
   mpActions = new ActionMeshCB(this);
   ...
   EditMeshSSOverride *commandModeOverride = new EditMeshSSOverride(this);
...
void EditMeshMod::EndEditParams( IObjParam *ip, ULONG flags,Animatable *next ) {
   ...
   if (flags&END_EDIT_REMOVEUI ) {
      ...
      DLSetWindowLongPtr( hGeom, 0 );
      DLSetWindowLongPtr (hSel, 0);
   }
...
static INT_PTR CALLBACK CurveNameDlgProc (HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
   ...
   case WM_COMMAND:
      ...
      case IDOK:
         ...
         GetWindowText(GetDlgItem(hWnd,IDC_EMCURVE_NAME), name->dataForWrite(), name->length()+1);
...
void EditMeshMod::SetSelDlgEnables() {
   ...
   EnableWindow (GetDlgItem (hSel, IDC_EM_DELETE_ISOLATED_VERTS), edg||poly||fac);
...
static INT_PTR CALLBACK SelectDlgProc (HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	EditMeshMod* em = DLGetWindowLongPtr<EditMeshMod *>(hWnd);
	...
	case WM_INITDIALOG:
		{
			...
			ReleaseISpinner ( SetupFloatSpinner (hWnd, IDC_EM_NORMAL_SCALESPIN, IDC_EM_NORMAL_SCALE, 0.01f, 999999999.0f, em->normScale, 0.1f) );

			iToolbar    = GetICustToolbar(GetDlgItem(hWnd,IDC_EM_SELTYPE));
			iToolbar->AddTool(EDITMESH_REGULAR(CTB_CHECKBUTTON, EditMeshIcon::vertexSelection, IDC_SELVERTEX));
			iToolbar->AddTool(EDITMESH_REGULAR(CTB_CHECKBUTTON, EditMeshIcon::edgeSelection, IDC_SELEDGE));
			iToolbar->AddTool(EDITMESH_REGULAR(CTB_CHECKBUTTON, EditMeshIcon::faceSelection, IDC_SELFACE));
			iToolbar->AddTool(EDITMESH_REGULAR(CTB_CHECKBUTTON, EditMeshIcon::polygonSelection, IDC_SELPOLY));
			iToolbar->AddTool(EDITMESH_REGULAR(CTB_CHECKBUTTON, EditMeshIcon::elementSelection, IDC_SELELEMENT));
			ReleaseICustToolbar(iToolbar);
			em->RefreshSelType();
			...
			CheckDlgButton (hWnd, IDC_EM_DELETE_ISOLATED_VERTS, em->deleteIsolatedVerts);
			break;
		}
	...
	case WM_COMMAND:
		{
			...
			case IDC_EM_DELETE_ISOLATED_VERTS:
				em->deleteIsolatedVerts = IsDlgButtonChecked (hWnd, IDC_EM_DELETE_ISOLATED_VERTS)?  TRUE:FALSE;
				break;
...
static INT_PTR CALLBACK GeomDlgProc (HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
   ...
   switch (msg) {
   case WM_INITDIALOG:
	   if (!EditMeshMod::weldBoxSizeInitialized)
	   {
		   EditMeshMod::weldBoxSizeInitialized = TRUE;

		   ViewExp22& vpt = static_cast<ViewExp22&>(GetCOREInterface()->GetActiveViewExp());
		   EditMeshMod::pickBoxSize = EditMeshMod::weldBoxSize = vpt.GetPickingSelectionRadius();
	   }
...
static INT_PTR CALLBACK SurfaceDlgProc (HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	int i = 0, stringID = 0;
	bool isFace = FALSE, isVert = FALSE;
	...
	case WM_INITDIALOG:
		{
			...
			{ // Face stuff:
				...
				for (i=IDC_SMOOTH_GRP1; i<IDC_SMOOTH_GRP1+32; i++)
					SendMessage(GetDlgItem(hWnd,i),CC_COMMAND,CC_CMD_SET_TYPE,CBT_CHECK);
				ReleaseISpinner( SetupIntSpinner(hWnd,IDC_EM_MAT_IDSPIN,IDC_EM_MAT_ID,1,MAX_MATID,1) );
				
				ReleaseISpinner( SetupIntSpinner (hWnd, IDC_EM_MAT_IDSPIN_SEL, IDC_EM_MAT_ID_SEL, 1, MAX_MATID, 1) );
				
				CheckDlgButton(hWnd, IDC_EM_CLEARSELECTION, 1);                                 
				SetupMtlSubNameCombo (hWnd, em); 

				ReleaseISpinner( SetupFloatSpinner(hWnd, IDC_EM_SMOOTH_THRESHSPIN, IDC_EM_SMOOTH_THRESH, 0.0f, 180.0f, autoSmoothThresh, 0.1f) );

				ICustButton* but = GetICustButton(GetDlgItem(hWnd,IDC_EM_NORMAL_FLIPMODE));
				but->SetType(CBT_CHECK);
				ReleaseICustButton(but);

				em->faceUIValid = FALSE;

				IColorSwatch* iCol = GetIColorSwatch (GetDlgItem (hWnd, IDC_EM_VERT_COLOR),
				...
				ReleaseISpinner( SetupFloatSpinner (hWnd, IDC_EM_ALPHASPIN, IDC_EM_ALPHA, 0.0f, 100.0f, 100.0f, .1f) );
			...
			if (GetDlgItem (hWnd, IDC_EM_VERT_SELRSPIN)) 
			{  // Vertex stuff:
				isVert = TRUE;
				ISpinnerControl* spin = SetupFloatSpinner (hWnd, IDC_EM_WEIGHTSPIN, IDC_EM_WEIGHT, 0.0f, 9999999.0f, 1.0f, .1f);
				...
				spin = SetupIntSpinner (hWnd, IDC_EM_VERT_SELRSPIN, IDC_EM_VERT_SELR, 0, 255, selDeltaR);
				ReleaseISpinner(spin);
				spin = SetupIntSpinner (hWnd, IDC_EM_VERT_SELGSPIN, IDC_EM_VERT_SELG, 0, 255, selDeltaG);
				ReleaseISpinner(spin);
				spin = SetupIntSpinner (hWnd, IDC_EM_VERT_SELBSPIN, IDC_EM_VERT_SELB, 0, 255, selDeltaB);
				ReleaseISpinner(spin);
				Color rgb = Color(selByColor);
			...
			if (GetDlgItem (hWnd, IDC_EM_ANGLETHRESH)) 
			{ // Edge stuff:
				ReleaseISpinner( SetupFloatSpinner (hWnd, IDC_EM_ANGLETHRESHSPIN, IDC_EM_ANGLETHRESH, 0.f, 180.f, autoEdgeThresh, 0.1f) );
				
				for (int i=0; i<3; i++) CheckDlgButton (hWnd, autoEdgeTypeIDs[i], (autoEdgeType == i));
			}
	...
	case WM_PAINT:
		{
			...
			Color color = em->selLevel==SL_VERTEX ? em->GetVertColor() : em->GetFaceColor();
			IColorSwatch* iCol = GetIColorSwatch (GetDlgItem(hWnd,IDC_EM_VERT_COLOR), 
				color,
	...
	case CC_COLOR_CHANGE:
		{
			...
			case IDC_EM_VERT_COLOR:
				if (em->selLevel == SL_VERTEX) em->SetVertColor (Color(iCol->GetAColor()));
				else em->SetFaceColor (Color(iCol->GetAColor()));
				break;

			case IDC_EM_VERT_ILLUM:
				if (em->selLevel == SL_VERTEX) em->SetVertColor (Color(iCol->GetAColor()), MAP_SHADING);
				else em->SetFaceColor (Color(iCol->GetAColor()), MAP_SHADING);
				break;

			case IDC_EM_VERT_SELCOLOR:
				selByColor = Point3( iCol->GetAColor());
				break;
			}
			break;
		}
	...
	case WM_COMMAND:
		{
			...
			case IDC_EM_SELECT_BYID: 
				{
					ISpinnerControl* idspin = NULL;
					int mtlID, clearSel;
					idspin = GetISpinner(GetDlgItem(hWnd,IDC_EM_MAT_IDSPIN_SEL));
					DbgAssert(idspin != NULL);
					mtlID = (idspin->IsIndeterminate()) ? -1 : idspin->GetIVal()-1; 
					clearSel = IsDlgButtonChecked(hWnd,IDC_EM_CLEARSELECTION);
					if (em->ip) em->SelectByMat(mtlID, clearSel); 
					ReleaseISpinner(idspin);                                                                   
					break;
				}
...
BOOL EMeshActionCB::ExecuteAction (int id) {
   ...
   case MDUID_EM_VERT_ILLUM:
	   ...
	   break;
//TO DO really need to move this to maxsdk\include\meshdelta.h
#define MDUID_EM_EDIT_SS                   40061
   case MDUID_EM_EDIT_SS:
     em->ToggleCommandMode (McmEditSoftSelection);
	   break;
   default:
	   return FALSE;
	}
...
void EditMeshMod::GetUIParam (meshUIParam uiCode, float & ret) {
   ...
   case MuiNormalSize:
      ret = normScale;
      break;
   case MuiDeleteIsolatedVerts:
      ret = (deleteIsolatedVerts? 1.0f:0.0f);
      break;
   }
}
...
void EditMeshMod::SetUIParam (meshUIParam uiCode, float val) {
   ...
   case MuiNormalSize:
      ...
      break;
   case MuiDeleteIsolatedVerts:
      deleteIsolatedVerts = (val==0.0f? FALSE:TRUE);
      if (hSel != NULL)
         CheckDlgButton(hSel, IDC_EM_DELETE_ISOLATED_VERTS, deleteIsolatedVerts);
      break;
   }
}
...
void EditMeshMod::SetUIParam (meshUIParam uiCode, int val) {
   ...
   case MuiDeleteIsolatedVerts:
      deleteIsolatedVerts = val? TRUE:FALSE;
      if (hSel != NULL)
         CheckDlgButton(hSel, IDC_EM_DELETE_ISOLATED_VERTS, deleteIsolatedVerts);
      break;
   }
...
void EditMeshMod::ToggleCommandMode(meshCommandMode mode) {
   ...
   case McmTurnEdge:
      ...
      break;
   case McmEditSoftSelection:
	   EditSoftSelectionMode();//this function handles the toggling
	   break;
   ...
   case MopWeld:
      if (selLevel == SL_VERTEX) {
         if (!WeldVerts(weldThresh)) {
            ...
            MaxSDK::MaxMessageBox(ip->GetMAXHWnd(), buf1, buf2, MB_OK);
         }
      }
      break;
   ...
   case MopCreateShapeFromEdges:
      ...
      if (DialogBoxParam (hInstance, MAKEINTRESOURCE(IDD_EM_CREATECURVE),
         ...
         if (!didSomething) {
            ...
            MaxSDK::MaxMessageBox(ip->GetMAXHWnd(), buf2, buf1, MB_ICONEXCLAMATION | MB_OK);
            break;
         }
      }
      break;
...
void EditMeshMod::EditVertColor(int mp, int ctrlID, int strID )
{
	...
	IColorSwatch* iCol = NULL;
	iCol = GetIColorSwatch(hWnd, col, GetString(strID));
	if (iCol == NULL) {
		return;
	}
...
BOOL SetupMtlSubNameCombo (HWND hWnd, EditMeshMod* em) {
	...
	for (int i=0; i<mtlIDList.Count(); i++){
		...
		if(mtlIDMeshList.Find(mtlIDList[i]) != -1) {
			...
			int ith = SendMessage(GetDlgItem(hWnd, IDC_EM_MTLID_NAMES_COMBO), CB_ADDSTRING, 0, (LPARAM)(LPCTSTR)buf.data());
...
RefResult
SingleRefMakerMeshMNode::NotifyRefChanged( const Interval& changeInt,RefTargetHandle hTarget, 
...
RefResult
SingleRefMakerMeshMMtl::NotifyRefChanged( const Interval& changeInt,RefTargetHandle hTarget, 
                               PartID& partID, RefMessage message, BOOL propagate ) { 
                                  switch(message) {
         case REFMSG_CHANGE:
            if(hwnd) ValidateUINameCombo(hwnd, em);
