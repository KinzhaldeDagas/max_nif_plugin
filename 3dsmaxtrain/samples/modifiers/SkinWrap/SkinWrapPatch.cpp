...
void PatchDeformPW::ModifyObject(TimeValue t, ModContext &mc, ObjectState * os, INode *incnode) 
{
	...
	if (localData->selfNode.GetRef() == NULL)
		{
		localData->selfNode.SetRef( GetNodeFromModContext(localData) );
		}
...
void PatchDeformPW::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc)
{
...
IOResult PatchDeformPW::SaveLocalData(ISave *isave, LocalModData *pld)
{
	...
	if (ct>0) 
	{
		...
		res = isave->WriteVoid(data, sizeof(ParamSpaceData)*p->paramData.Count(), &nb); // no TCHAR data
	}
	isave->EndChunk();
