...
static_assert(sizeof(ParamSpaceData) == 3 * sizeof(Point3) + sizeof(int), "");

class LocalPatchData : public LocalModData 
	{
	public:
		Tab<ParamSpaceData> paramData;
		MaxSDK::SingleWeakRefMaker selfNode;
		int numPatches;
		float incDelta;
		BOOL resample;
		LocalPatchData()
...
class PatchDeformPW : public Modifier , public IPatchDeformPWModFP {
	public:
		...
		// From Animatable
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_CLASS_NAME) : _T("Skin Wrap Patch"); }
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_CLASS_NAME) : _T("Skin Wrap Patch");}
...
class PatchDeformPWClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Skin Wrap Patch"); }
	SClass_ID		SuperClassID() { return OSM_CLASS_ID; }
	Class_ID		ClassID() { return PATCHDEFORMPW_CLASS_ID; }
...
{
public:
	const LocalPatchData* localData;
	Matrix3 baseTm, patchTm;
	Matrix3 fromWorldToBaseTm;
	Matrix3 fromPatchToBaseTm;
	PatchMesh* patch;

	PatchDeformer(const LocalPatchData* ld, PatchMesh* patch, Matrix3 baseTm, Matrix3 patchTm)
	{
		localData = ld;
		this->baseTm = baseTm;
	...
	Point3 Map(int i, Point3 pt) const override
	{
