...
public:
	void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)  // set from v
	{
		...
		case pb_falloff:
		case pb_distance:
		case pb_facelimit: p->Resample(); break;
		case pb_engine:
			p->Resample();
			if (p->ip)
				p->UpdateUI();
	...
	void TabChanged(
		tab_changes changeCode, Tab<PB2Value>* tab, ReferenceMaker* owner, ParamID id, int tabIndex, int count)
	{
		if (id == pb_meshlist)
		{
...
static ParamBlockDesc2 meshdeformpw_param_blk(meshdeformpw_params, _T("params"), 0, &MeshDeformPWDesc,
	P_AUTO_CONSTRUCT + P_AUTO_UI + P_MULTIMAP, PBLOCK_REF, 3,
	//rollout
	meshdeformpw_params, IDD_MESHPANEL, IDS_PARAMS, 0, 0, NULL, meshdeformpw_advanceparams, IDD_MESHPANELADVANCE,
	IDS_PARAMSADVANCE, 0, 0, NULL, meshdeformpw_displayparams, IDD_MESHPANELDISPLAY, IDS_PARAMSDISPLAY, 0, 0, NULL,
	// params

	pb_mesh, _T("mesh"), TYPE_INODE, 0, IDS_PW_MESH, p_end,

	pb_threshold, _T("threshold"), TYPE_FLOAT, 0, IDS_THRESHOLD, p_default, 5.0f, p_range, 0.001f, 100000.0f, p_ui,
	meshdeformpw_params, TYPE_SPINNER, EDITTYPE_UNIVERSE, IDC_THRESHOLD, IDC_THRESHOLD_SPIN2, 1.0f, p_end,

	pb_engine, _T("engine"), TYPE_INT, 0, IDS_ENGINE, p_default, 1, p_ui, meshdeformpw_params, TYPE_INTLISTBOX,
	IDC_ENGINE_COMBO, 2, IDS_FACE, IDS_VERTEX, p_accessor, &meshdeform_accessor, p_end,

	pb_falloff, _T("falloff"), TYPE_FLOAT, 0, IDS_FALLOFF, p_default, 1.0f, p_range, 0.001f, 10.0f, p_ui,
	meshdeformpw_params, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_FALLOFF, IDC_IDC_FALLOFF_SPIN, 0.1f, p_accessor,
	&meshdeform_accessor, p_end,

	pb_distance, _T("distance"), TYPE_FLOAT, 0, IDS_DISTANCE, p_default, 1.2f, p_range, 0.001f, 10.0f, p_ui,
	meshdeformpw_params, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_DISTANCE, IDC_IDC_DISTANCE_SPIN, 0.1f, p_accessor,
	&meshdeform_accessor, p_end,

	pb_facelimit, _T("faceLimit"), TYPE_INT, 0, IDS_FACELIMIT, p_default, 3, p_range, 0, 30, p_ui, meshdeformpw_params,
	TYPE_SPINNER, EDITTYPE_INT, IDC_FACELIMIT, IDC_IDC_FACELIMIT_SPIN, 1.0f, p_accessor, &meshdeform_accessor, p_end,

	pb_showloops, _T("showLoops"), TYPE_BOOL, 0, IDS_SHOWLOOPS, p_default, TRUE, p_ui, meshdeformpw_displayparams,
	TYPE_SINGLECHECKBOX, IDC_DISPLAYLOOPCHECK, p_end, pb_showaxis, _T("showAxis"), TYPE_BOOL, 0, IDS_SHOWAXIS,
	p_default, TRUE, p_ui, meshdeformpw_displayparams, TYPE_SINGLECHECKBOX, IDC_DISPLAYAXISCHECK, p_end,
	pb_showfacelimit, _T("showFaceLimit"), TYPE_BOOL, 0, IDS_SHOWFACELIMIT, p_default, TRUE, p_ui,
	meshdeformpw_displayparams, TYPE_SINGLECHECKBOX, IDC_DISPLAYFACELIMITCHECK, p_end,

	pb_mirrorshowdata, _T("showMirrorData"), TYPE_BOOL, 0, IDS_SHOWMIRRORDATA, p_default, FALSE, p_ui,
	meshdeformpw_advanceparams, TYPE_SINGLECHECKBOX, IDC_SHOWMIRRORCHECK, p_end,

	pb_mirrorplane, _T("mirrorPlane"), TYPE_INT, P_RESET_DEFAULT, IDS_MIRRORPLANE, p_default, 0, p_range, 0, 2, p_ui,
	meshdeformpw_advanceparams, TYPE_RADIO, 3, IDC_XRADIO, IDC_YRADIO, IDC_ZRADIO, p_end, pb_mirroroffset,
	_T("mirrorOffset"), TYPE_FLOAT, 0, IDS_MIRROROFFSET, p_default, 0.0f, p_range, -10000.0f, 10000.0f, p_ui,
	meshdeformpw_advanceparams, TYPE_SPINNER, EDITTYPE_UNIVERSE, IDC_MIRROROFFSET, IDC_MIRROROFFSET_SPIN, 0.1f, p_end,

	pb_debugpoint, _T(""), TYPE_INT, 0, IDS_FACELIMIT, p_default, 0, p_end,

	pb_meshlist, _T("meshList"), TYPE_INODE_TAB, 0, P_AUTO_UI | P_VARIABLE_SIZE, IDS_MESHLIST, p_ui,
	meshdeformpw_params, TYPE_NODELISTBOX, IDC_LIST1, 0, 0, IDC_REMOVE, p_accessor, &meshdeform_accessor, p_end,

	pb_showunassignedverts, _T("showUnassignedVerts"), TYPE_BOOL, 0, IDS_SHOWUNASSIGNEDVERTS, p_default, FALSE, p_ui,
	meshdeformpw_displayparams, TYPE_SINGLECHECKBOX, IDC_SHOWUNASSIGNEDVERTSCHECK, p_end,

	pb_showverts, _T("showVerts"), TYPE_BOOL, 0, IDS_SHOWVERTS, p_default, TRUE, p_ui, meshdeformpw_displayparams,
	TYPE_SINGLECHECKBOX, IDC_SHOWVERTSCHECK, p_end,

	pb_weightallverts, _T("weightAllVerts"), TYPE_BOOL, 0, IDS_WEIGHTALL, p_default, FALSE, p_ui, meshdeformpw_params,
	TYPE_SINGLECHECKBOX, IDC_WEIGHTALLCHECK, p_end,

	pb_mirrorthreshold, _T("mirrorThreshold"), TYPE_FLOAT, 0, IDS_MIRRORTHRESHOLD, p_default, 0.5f, p_range, 0.0f,
	10000.0f, p_ui, meshdeformpw_advanceparams, TYPE_SPINNER, EDITTYPE_UNIVERSE, IDC_MIRRORTHRESHOLD,
	IDC_MIRRORTHRESHOLD_SPIN, 0.1f, p_end,

	pb_useblend, _T("blend"), TYPE_BOOL, 0, IDS_BLEND, p_default, FALSE, p_enable_ctrls, 1, pb_blenddistance, p_ui,
	meshdeformpw_params, TYPE_SINGLECHECKBOX, IDC_BLENDCHECK, p_end,

	pb_blenddistance, _T("blendDistance"), TYPE_FLOAT, 0, IDS_BLENDDISTANCE, p_default, 5.0f, p_range, 0.0f, 10000.0f,
	p_ui, meshdeformpw_params, TYPE_SPINNER, EDITTYPE_UNIVERSE, IDC_BLEND, IDC_BLEND_SPIN, 0.1f, p_end,
...

{
	...
	case WM_CUSTEDIT_ENTER:
		...
		if (LOWORD(wParam) == IDC_X)
		{
			...
			macroRecorder->FunctionCall(_T("$.modifiers[#Skin_Wrap].meshDeformOps.setLocalScale"), 2, 0, mr_int, 1,
		...
		if (LOWORD(wParam) == IDC_Y)
		{
			...
			macroRecorder->FunctionCall(_T("$.modifiers[#Skin_Wrap].meshDeformOps.setLocalScale"), 2, 0, mr_int, 2,
		...
		if (LOWORD(wParam) == IDC_Z)
		{
			...
			macroRecorder->FunctionCall(_T("$.modifiers[#Skin_Wrap].meshDeformOps.setLocalScale"), 2, 0, mr_int, 3,
		...
		if (LOWORD(wParam) == IDC_STR)
		{
			...
			macroRecorder->FunctionCall(
				_T("$.modifiers[#Skin_Wrap].meshDeformOps.setLocalStr"), 1, 0, mr_float, mod->strSpin->GetFVal());
		}
	...
	case CC_SPINNER_BUTTONDOWN:
	{
		...
		if ((LOWORD(wParam) == IDC_X_SPIN) || (LOWORD(wParam) == IDC_Y_SPIN) || (LOWORD(wParam) == IDC_Z_SPIN))
		{
			//hold
			theHold.Begin();
			for (int i = 0; i < mod->pblock->Count(pb_meshlist); i++)
	...
	case CC_SPINNER_BUTTONUP:
		...
		{
			if ((LOWORD(wParam) == IDC_X_SPIN) || (LOWORD(wParam) == IDC_Y_SPIN) || (LOWORD(wParam) == IDC_Z_SPIN) ||
				(LOWORD(wParam) == IDC_STR_SPIN) || (LOWORD(wParam) == IDC_SCALEMULT_SPIN))
				theHold.Cancel();
			mod->multSpin->SetValue(1.0f, FALSE);
		}
		else
		{
			...
			if ((LOWORD(wParam) == IDC_X_SPIN) || (LOWORD(wParam) == IDC_Y_SPIN) || (LOWORD(wParam) == IDC_Z_SPIN))
			{
				...
				if (LOWORD(wParam) == IDC_X_SPIN)
					macroRecorder->FunctionCall(_T("$.modifiers[#Skin_Wrap].meshDeformOps.setLocalScale"), 2, 0, mr_int,
						1, mr_float, mod->xSpin->GetFVal());
				else if (LOWORD(wParam) == IDC_Y_SPIN)
					macroRecorder->FunctionCall(_T("$.modifiers[#Skin_Wrap].meshDeformOps.setLocalScale"), 2, 0, mr_int,
						2, mr_float, mod->ySpin->GetFVal());
				else if (LOWORD(wParam) == IDC_Z_SPIN)
					macroRecorder->FunctionCall(_T("$.modifiers[#Skin_Wrap].meshDeformOps.setLocalScale"), 2, 0, mr_int,
						3, mr_float, mod->zSpin->GetFVal());
			}
			if (LOWORD(wParam) == IDC_STR_SPIN)
			{
				macroRecorder->FunctionCall(
	...
	case WM_COMMAND:
	{
		...
		{
			macroRecorder->FunctionCall(
				_T("$.modifiers[#Skin_Wrap].meshDeformOps.convertToSkin"), 1, 0, mr_bool, FALSE);
...
//*****************************************************
static FPInterfaceDesc meshdeformpw_interface(MESHDEFORMPW_INTERFACE, _T("meshDeformOps"), 0, &MeshDeformPWDesc,
	FP_MIXIN,

	meshdeform_getselectedvertices, _T("getSelectedVertices"), 0, TYPE_BITARRAY, 0, 1, 
	_T("wrapNodeIndex"), 0,	TYPE_INDEX, 
	meshdeform_setselectedvertices, _T("setSelectVertices"), 0, TYPE_VOID, 0, 3, _T("wrapNodeIndex"), 0, TYPE_INDEX, 
	_T("sel"), 0, TYPE_BITARRAY, _T("update"), 0, TYPE_BOOL,
	...
	meshdeform_reset, _T("fullReset"), 0, TYPE_VOID, 0, 0,

	meshdeform_ouputvertexdata, _T("OutputVertexData"), 0, TYPE_VOID, 0, 2, _T("node"), 0, TYPE_INODE,
...
FPInterfaceDesc* IMeshDeformPWMod::GetDesc() { return &meshdeformpw_interface; }
...
__forceinline float FastInvsqrt(float x)
{
	...
	//x = x * (1.5f - xhalf * x * x);  // 2nd iteration, this can be removed for speed
	return x;
}

inline void FastNormalize(Point3& vec) { vec *= FastInvsqrt(vec.x * vec.x + vec.y * vec.y + vec.z * vec.z); }
...
BOOL RecursePipeAndMatch(LocalMeshData* smd, Object* obj)
{
	...
	if (bct > 0)
	{
		for (int bi = 0; bi < bct; bi++)
		{
			...
			if (RecursePipeAndMatch(smd, bobj))
				return TRUE;
		}
	}
		...
		if (node)
		{
			...
			if (obj != NULL)
			{
				...
				if (xi != NULL)
				{
					...
					//get the resolved non-xref derived object under the nested case
					//why not use the xi->GetSrcItem(true)? Beaucse this function will return the geometry object, not the DerivedObject
					while (obj != NULL)
					{
						IXRefItem* nestedXI = IXRefItem::GetInterface(*obj);
						if (nestedXI != NULL)
						{
							obj = (Object*)nestedXI->GetSrcItem();
						}
						else
						{
							break;
...
void MeshDeformPW::GetTMFromVertex(TimeValue t, int wrapIndex, Mesh* msh, Point3* vnorms, const Matrix3& meshToWorld)
{
	Matrix3  tm(1);
	Interval ivalid;
...
*/
		...
		else
		{
			...
			for (int k = 0; k < potentialEdges.Count(); k++)
			{
				//				if (potentialEdges[k])
				{
					Point3 vec = msh->verts[potentialEdges[k]] - centerVert;
					FastNormalize(vec);
					if (vec.x > x)
						x = vec.x;
					if (vec.y > y)
	...
	for (int i = 0; i < edgeList.edges.Count(); i++)
	{
		...
		if (dist > wrapMeshData[wrapIndex]->initialData[a].dist)
			wrapMeshData[wrapIndex]->initialData[a].dist = dist;
		if (dist > wrapMeshData[wrapIndex]->initialData[b].dist)
			wrapMeshData[wrapIndex]->initialData[b].dist = dist;
	}
	...
	for (int vid = 0; vid < msh->numVerts; vid++)
	{
		...
		pole = pole - t;
		FastNormalize(pole);
		xvec = CrossProd(zvec, pole);
		FastNormalize(xvec);
		yvec = CrossProd(zvec, xvec);
		FastNormalize(yvec);
...
void MeshDeformPW::RebuildSelectedWeights(LocalMeshData* localData, int wrapIndex, const Matrix3& meshTM,
	const Matrix3& baseTM, float falloff, float distance, int faceLimit)
{
	if (wrapMeshData[wrapIndex]->mesh == NULL)
		return;
	...
	for (int i = 0; i < wrapMeshData[wrapIndex]->initialData.Count(); i++)
	{
		...
		pTM.Scale(Point3(wrapMeshData[wrapIndex]->initialData[i].dist * distance *
							 wrapMeshData[wrapIndex]->initialData[i].localScale.x,
			wrapMeshData[wrapIndex]->initialData[i].dist * distance *
				wrapMeshData[wrapIndex]->initialData[i].localScale.y,
			wrapMeshData[wrapIndex]->initialData[i].dist * distance *
				wrapMeshData[wrapIndex]->initialData[i].localScale.z));
	...
	for (int j = 0; j < localData->paramData.Count(); j++)
	{
		if (localData->potentialWeightVerts[j])
		{
			...
			//get our seed face
			int seedFace = localData->faceIndex[wrapIndex]
							   ->f[j];  //wrapMeshData[wrapIndex]->faceIndex[j];//localData->paramData[j].faceIndex;
...
void MeshDeformPW::BuildWeightData(Object* obj, LocalMeshData* localData, int wrapIndex, Mesh* msh, Matrix3 meshTM,
	Matrix3 baseTM, float falloff, float distance, int faceLimit, BitArray pointsToProcess)
{
	int engine;
	...
	if (engine == 0)
	{
		for (int j = 0; j < localData->paramData.Count(); j++)
		{
			...
			if (seedFace >= 0)
			{
				...
				localData->weightData[j]->data.SetCount(0);
		}
	}
	else
	{
		...
		else
		{
			...
			if (wrapMeshData[wrapIndex]->mesh)
			{
				...
				int paramDataCount = localData->paramData.Count();
				for (int j = 0; j < paramDataCount; j++)
				{
					if (!pointsToProcess[j])
						continue;
					if ((ip) && (paramDataCount > 10000))
					{
						TSTR prompt;
						if ((j % 200) == 0)
						{
							float per = (float)j / (float)paramDataCount * 100.0f;
							prompt.printf(_T("%s %d : %d(%d) (%0.0f%%)"), GetString(IDS_STEP2), wrapIndex, j,
								paramDataCount, per);
...
void MeshDeformPW::BuildCurrentTMFromVertex(TimeValue t, int wrapIndex, Mesh* msh, Point3* vnorms,
	const Matrix3& meshTM, const Matrix3& baseTM, const Matrix3& initialBaseTM)
{
	...
	WrapMeshData& curWrapMesh = *wrapMeshData[wrapIndex];
	curWrapMesh.currentVertexTMs.SetCount(msh->numVerts);
	curWrapMesh.defTMs.SetCount(msh->numVerts);
	//now build our matrices
	//get our align vec
	if (msh->numVerts <= 0)
	{
	...
	tbb::parallel_for(
		tbb::blocked_range<int>(0, msh->numVerts), [&](const auto& range) {
			for (int vid = range.begin(); vid < range.end(); ++vid)
			{
				Point3 xvec, yvec, zvec, t;
				Point3 pole;
				t	= msh->verts[vid];
				zvec = vnorms[vid];
				pole = msh->verts[curWrapMesh.initialData[vid].xPoints];
				pole = pole - t;
				FastNormalize(pole);
				xvec = CrossProd(zvec, pole);
				FastNormalize(xvec);
				yvec = CrossProd(zvec, xvec);
				FastNormalize(yvec);
				Matrix3& curTM = curWrapMesh.currentVertexTMs[vid];
				curTM.SetRow(0, xvec);
				curTM.SetRow(1, yvec);
				curTM.SetRow(2, zvec);
				curTM.SetRow(3, t);
				...
				Matrix3& defTM = curWrapMesh.defTMs[vid];
				Matrix3  tmpMatrix1, tmpMatrix2;
				Inverse(tmpMatrix1, curWrapMesh.initialMeshTM);
				MatrixMultiply(defTM, initialBaseTM, tmpMatrix1);
				Inverse(tmpMatrix1, curWrapMesh.initialData[vid].initialVertexTMs);
				MatrixMultiply(tmpMatrix2, defTM, tmpMatrix1);
				MatrixMultiply(defTM, tmpMatrix2, curWrapMesh.currentVertexTMs[vid]);
				MatrixMultiply(tmpMatrix2, defTM, meshTM);
				Inverse(tmpMatrix1, baseTM);
				MatrixMultiply(defTM, tmpMatrix2, tmpMatrix1);
			}
		});
}

void MeshDeformPW::BuildParamData(TimeValue t, Object* obj, LocalMeshData* localData, int wrapIndex, INode* node,
	Matrix3 meshTM, Matrix3 baseTM, float threshold)
{
	//put our base mesh data into mesh space
	Matrix3 tmFromBaseToMesh = baseTM * Inverse(meshTM);
	...
	IRayMeshGridIntersect_InterfaceV1* rt =
		(IRayMeshGridIntersect_InterfaceV1*)ref->GetInterface(RAYMESHGRIDINTERSECT_V1_INTERFACE);
	IRayMeshGridIntersect_InterfaceV2* rt2 =
		(IRayMeshGridIntersect_InterfaceV2*)ref->GetInterface(RAYMESHGRIDINTERSECT_V2_INTERFACE);
	...
	Matrix3 nodeTM = node->GetObjTMBeforeWSM(t);
	...
	for (int i = 0; i < pointCount; i++)
	{
		...
		if ((ip) && (pointCount > 1000))
		{
			...
			if ((i % 100) == 0)
			{
				...
				prompt.printf(_T("%s %d : %d(%d) (%0.0f%%)"), GetString(IDS_STEP1), wrapIndex, i, pointCount, per);
				ip->ReplacePrompt(prompt);
			}
		}
...
void MeshDeformPW::BuildParamDataForce(TimeValue t, Object* obj, LocalMeshData* localData, int wrapIndex, INode* node,
	Matrix3 meshTM, Matrix3 baseTM, float threshold, BitArray pointsToProcess)
{
	//put our base mesh data into mesh space
	Matrix3 tmFromBaseToMesh = baseTM * Inverse(meshTM);
...
void MeshDeformPW::BuildNormals(Mesh* mesh, Tab<Point3>& vnorms, Tab<Point3>& fnorms)
{
	...
	ZeroMemory(vnorms.Addr(0), sizeof(Point3) * mesh->numVerts);
	Point3* vnormsPtr = vnorms.Addr(0);

	fnorms.SetCount(mesh->numFaces);
	Point3* fnormsPtr = fnorms.Addr(0);

	// Compute face normals.
	tbb::parallel_for(
		tbb::blocked_range<int>(0, mesh->numFaces), [&](const auto& range) {
			for (int j = range.begin(); j < range.end(); ++j)
			{
				int	ia	= mesh->faces[j].v[0];
				int	ib	= mesh->faces[j].v[1];
				int	ic	= mesh->faces[j].v[2];
				Point3 a	 = mesh->verts[ia];
				Point3 b	 = mesh->verts[ib];
				Point3 c	 = mesh->verts[ic];
				Point3 vecA  = (a - b);
				Point3 vecB  = (c - b);
				fnormsPtr[j] = CrossProd(vecA, vecB);
				FastNormalize(fnormsPtr[j]);
			}
		});

	// Compute vertex normals. To remove thread looks, spawn n threads and use the thread number
	// as an ID. Process only vertices that are a multiple of that ID. This way, no 2 thread
	// will access the same vertex.
	//
	// Use a prallel_for with a size of 1 to simply spawn n threads. Each thread loops
	// over all faces.
	int num_t = static_cast<int>(ThreadTools::GetTbbMaxThreads());
	tbb::parallel_for(tbb::blocked_range<int>(0, num_t, 1), [&](const auto& range) {
		int t_id = range.begin();
		for (size_t j = 0; j < mesh->numFaces; ++j)
		{
			...
			if (t_id == ia % num_t)
			{
				vnormsPtr[ia] += fnormsPtr[j];
			}
			if (t_id == ib % num_t)
			{
				vnormsPtr[ib] += fnormsPtr[j];
			}
			if (t_id == ic % num_t)
			{
				vnormsPtr[ic] += fnormsPtr[j];
			}
		}
	}, tbb::simple_partitioner{});

	// Final pass, normalize all vertex normals.
	tbb::parallel_for(
		tbb::blocked_range<int>(0, mesh->numVerts), [&](const auto& range) {
			Point3 zero{};
			for (int j = range.begin(); j < range.end(); ++j)
			{
				if (vnormsPtr[j] == zero)
...
void MeshDeformPW::ModifyObject(TimeValue t, ModContext& mc, ObjectState* os, INode* incnode)
{
	...
	INode* selfNode = static_cast<INode*>(localData->selfNode.GetRef());
	if (selfNode == NULL)
	{
		selfNode = GetNodeFromModContext(localData);
		if (selfNode == NULL)
		{
		...
		localData->selfNode.SetRef(selfNode);
		localData->initialBaseTM = selfNode->GetObjTMBeforeWSM(t, &iv);
		;
	}
	...
	Matrix3 baseTM				= selfNode->GetObjTMBeforeWSM(t, &iv);
	localData->currentBaseTM	= selfNode->GetNodeTM(t);
	...
	for (int wrapIndex = 0; wrapIndex < pblock->Count(pb_meshlist); wrapIndex++)
	{
		...
		if (meshos.obj)
		{
			...
			if (node->GetWSMDerivedObject())
			{
				...
				if (!identity)
					wrapMeshData[wrapIndex]->currentMeshTM = node->GetObjTMBeforeWSM(t, &iv);
				else
					wrapMeshData[wrapIndex]->currentMeshTM = meshIdentTM;
			}
			else
				wrapMeshData[wrapIndex]->currentMeshTM = node->GetObjTMBeforeWSM(t, &iv);
	...
	for (int wrapIndex = 0; wrapIndex < pblock->Count(pb_meshlist); wrapIndex++)
	{
		if (wrapMeshData[wrapIndex]->mesh)
		{
			if (wrapMeshData[wrapIndex]->initialData.Count() != wrapMeshData[wrapIndex]->mesh->numVerts)
				cageTopoChange = TRUE;
	...
	if (rebuilduildParamSpace)
	{
		...
		for (int wrapIndex = 0; wrapIndex < pblock->Count(pb_meshlist); wrapIndex++)
		{
			...
			if (node->GetWSMDerivedObject())
			{
				BOOL identity = meshos.TMIsIdentity();
				if (!identity)
					wrapMeshData[wrapIndex]->initialMeshTM = node->GetObjTMBeforeWSM(t, &iv);
				else
					wrapMeshData[wrapIndex]->initialMeshTM = meshIdentityTM;
			}
			else
				wrapMeshData[wrapIndex]->initialMeshTM = node->GetObjTMBeforeWSM(t, &iv);
			...
			{
				BuildParamData(t, os->obj, localData, wrapIndex, node, wrapMeshData[wrapIndex]->initialMeshTM,
					localData->initialBaseTM, threshold);
				if (wrapMeshData[wrapIndex]->selectedVerts.GetSize() != wrapMeshData[wrapIndex]->mesh->numVerts)
				{
					...
					if (ip)
					{
						...
						localData->displayedVerts.SetSize(0);
				}
	...
	if (rebuildWeights)
	{
		...
		for (int wrapIndex = 0; wrapIndex < pblock->Count(pb_meshlist); wrapIndex++)
		{
			...
			if (wrapMeshData[wrapIndex]->vnorms.Count() > 0)
				GetTMFromVertex(t, wrapIndex, wrapMeshData[wrapIndex]->mesh, wrapMeshData[wrapIndex]->vnorms.Addr(0),
					wrapMeshData[wrapIndex]->initialMeshTM);
			...
			BuildWeightData(os->obj, localData, wrapIndex, wrapMeshData[wrapIndex]->mesh,
				wrapMeshData[wrapIndex]->initialMeshTM, localData->initialBaseTM, falloff, distance, faceLimit,
				pointsToProcess);
	...
	for (int wrapIndex = 0; wrapIndex < pblock->Count(pb_meshlist); wrapIndex++)
	{
		if ((wrapMeshData[wrapIndex]->mesh) && (wrapMeshData[wrapIndex]->vnorms.Count() > 0))
			BuildCurrentTMFromVertex(t, wrapIndex, wrapMeshData[wrapIndex]->mesh,
				wrapMeshData[wrapIndex]->vnorms.Addr(0), wrapMeshData[wrapIndex]->currentMeshTM, baseTM,
				localData->initialBaseTM);
	}
	...
	if (localData->rebuildSelectedWeights)
	{
		...
		{
			if (wrapMeshData[wrapIndex]->mesh)
				RebuildSelectedWeights(
					localData, wrapIndex, wrapMeshData[wrapIndex]->initialMeshTM, baseTM, falloff, distance, faceLimit);
		}
		NormalizeWeightData(localData);
			...
			while (facesToProcess.NumberSet() > 0)
			{
				...
				for (int wrapIndex = 0; wrapIndex < pblock->Count(pb_meshlist); wrapIndex++)
				{
					...
					if ((node != NULL) && (wrapMeshData[wrapIndex]->mesh))
					{
						...
						BuildParamDataForce(t, os->obj, localData, wrapIndex, node,
							wrapMeshData[wrapIndex]->initialMeshTM, baseTM, tThreshold, facesToProcess);
					}
				}
				facesToProcess.ClearAll();
...
*/
	...
	for (int wrapIndex = 0; wrapIndex < pblock->Count(pb_meshlist); wrapIndex++)
	{
		...
		if ((wrapMeshData[wrapIndex]->mesh) && (wrapMeshData[wrapIndex]->fnorms.Count() > 0))
			deformer.AddWrapData(wrapIndex, wrapMeshData[wrapIndex]->mesh, wrapMeshData[wrapIndex]->currentMeshTM,
				wrapMeshData[wrapIndex]->fnorms.Addr(0));
		else
			deformer.AddWrapData(
				wrapIndex, wrapMeshData[wrapIndex]->mesh, wrapMeshData[wrapIndex]->currentMeshTM, NULL);
	}
	...
	if (ip)
	{
		...
		localData->localPoint.ZeroCount();
...
void MeshDeformPW::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext* mc) {}
...
IOResult MeshDeformPW::Load(ILoad* iload)
{
	...
	while (IO_OK == (res = iload->OpenChunk()))
	{
		...
		case INITIAL_DATA:
		{
			...
			if (ct > 0)
			{
				res = iload->ReadVoid(initialDataOld.Addr(0), sizeof(InitialTMDataOld) * ct, &nb);
		...
		case INITIALV1_DATA:
		{
			...
			if (ct > 0)
				res = iload->ReadVoid(wrapMeshData[0]->initialData.Addr(0), sizeof(InitialTMData) * ct, &nb);
		...
		case WRAPMESH_DATA:
		{
			...
			{
				iload->ReadVoid(
					wrapMeshData[currentWrapMesh]->initialData.Addr(0), sizeof(InitialTMData) * numberVerts, &nb);
			}
...
IOResult MeshDeformPW::Save(ISave* isave)
{
	...
	for (int wrapIndex = 0; wrapIndex < ct; wrapIndex++)
	{
		...
		if (ict > 0)
			res = isave->WriteVoid(
				wrapMeshData[wrapIndex]->initialData.Addr(0), sizeof(InitialTMData) * ict, &nb);  // no TCHAR data
...
IOResult MeshDeformPW::SaveLocalData(ISave* isave, LocalModData* pld)
{
	...
	if (ct != 0)
	{
		...
		res = isave->Write(&ct, sizeof(ct), &nb);
		if (ct)
			res = isave->WriteVoid(data, sizeof(ParamSpaceData) * p->paramData.Count(), &nb);
		isave->EndChunk();
	}
	...
	while (IO_OK == (res = iload->OpenChunk()))
	{
		...
		case WEIGHT_DATA:
		{
			...
			for (int i = 0; i < ct; i++)
			{
				...
				if (ct2)
				{
					iload->ReadVoid(data.Addr(0), sizeof(BoneInfoOld) * ct2, &nb);
					for (int j = 0; j < ct2; j++)
					{
		...
		case UVSPACE_DATA:
		{
			...
			if (ct)
			{
				...
				iload->ReadVoid(data, sizeof(ParamSpaceData) * p->paramData.Count(), &nb);
			}
			for (int i = 0; i < ct; i++)
			{
		...
		case WEIGHT2_DATA:
		{
			...
			for (int i = 0; i < ct; i++)
			{
				...
				p->weightData[i]->data.SetCount(ct2);
				if (ct2)
					iload->ReadVoid(p->weightData[i]->data.Addr(0), sizeof(BoneInfo) * ct2, &nb);
			}
			break;
		}
		case UVSPACE2_DATA:
...
PipeEnumResult FindMeshDeformOnStack::proc(ReferenceTarget* object, IDerivedObject* dobj, int index)
{
...
int MeshDeformPW::Display(TimeValue t, INode* inode, ViewExp* vpt, int flagst, ModContext* mc)
{
	if (!vpt || !vpt->IsAlive())
	{
		// why are we here
...
void MeshDeformPW::GetWorldBoundBox(TimeValue t, INode* inode, ViewExp* vpt, Box3& box, ModContext* mc)
{
	if (!vpt || !vpt->IsAlive())
	{
		// why are we here
			...
			if (ip)
			{
				...
				if (mnode != NULL)
				{
					...
					for (int i = 0; i < ct; i++)
					{
						...
						{
							Matrix3 pTM = wrapMeshData[wrapIndex]->currentVertexTMs[i];
							Point3  p1(Point3(wrapMeshData[wrapIndex]->initialData[i].dist * distance *
												  wrapMeshData[wrapIndex]->initialData[i].localScale.x,
								 wrapMeshData[wrapIndex]->initialData[i].dist * distance *
									 wrapMeshData[wrapIndex]->initialData[i].localScale.y,
								 wrapMeshData[wrapIndex]->initialData[i].dist * distance *
...
int MeshDeformPW::NumSubObjTypes() { return 1; }
...
void MeshDeformPW::SelectSubComponent(HitRecord* hitRec, BOOL selected, BOOL all, BOOL invert)
{
	if (theHold.Holding())
	{
		for (int wrapIndex = 0; wrapIndex < pblock->Count(pb_meshlist); wrapIndex++)
	...
	while (hitRec)
	{
		...
		BOOL state	 = selected;
		if (invert)
			state = !wrapMeshData[wrapIndex]->selectedVerts[vertIndex];
		if (state)
			wrapMeshData[wrapIndex]->selectedVerts.Set(vertIndex, 1);
		else
			wrapMeshData[wrapIndex]->selectedVerts.Set(vertIndex, 0);
		if (!all)
			break;
		hitRec = hitRec->Next();
	}
	...
	{
		if (wrapMeshData[wrapIndex]->mesh)
			macroRecorder->FunctionCall(_T("$.modifiers[#Skin_Wrap].meshDeformOps.setSelectVertices"), 3, 0, mr_int,
				wrapIndex + 1, mr_bitarray, &(wrapMeshData[wrapIndex]->selectedVerts), mr_bool, TRUE);
	}
...
{
	if ((whichWrapMesh < 0) || (whichWrapMesh >= wrapMeshData.Count()))
		return;
...
int MeshDeformPW::fnGetNumberControlPoints(int whichWrapMesh) { return GetNumberControlPoints(whichWrapMesh); }

Point3* MeshDeformPW::fnGetPointScale(int whichWrapMesh, int index) { return GetPointScale(whichWrapMesh, index); }
...
void MeshDeformPW::UpdateXYZSpinners(BOOL updateMult)
{
	...
	if (xSpin == nullptr)
	{
		...
		if (xSpin != nullptr)  //can get in weird timing state with MXS where ui is partially built so need to double check this
		{
	...
	if (strValid  && strSpin)
	{
		...
		strSpin->SetIndeterminate(TRUE);

	if (updateMult  && multSpin)
...
float MeshDeformPW::fnGetPointStr(int whichWrapMesh, int index) { return GetPointStr(whichWrapMesh, index); }

void MeshDeformPW::fnSetPointStr(int whichWrapMesh, int index, float str) { SetPointStr(whichWrapMesh, index, str); }
...
void MeshDeformPW::BuildLookUpData(LocalMeshData* ld, int wrapIndex)
{
	...
	INode* selfNode = static_cast<INode*>(ld->selfNode.GetRef());
	if (selfNode)
	{
		Matrix3 meshTM = node->GetObjTMBeforeWSM(t, &iv);
		Matrix3 baseTM = selfNode->GetObjTMBeforeWSM(t, &iv);
		...
		for (int i = 0; i < wrapMeshData[wrapIndex]->selectedVerts.GetSize(); i++)
		{
			...
			if (wrapMeshData[wrapIndex]->selectedVerts[i])
			{
				...
				{
					int faceIndex = ld->faceIndex[wrapIndex]
										->f[j];  //wrapMeshData[wrapIndex]->faceIndex[j];//ld->paramData[j].faceIndex;
...
void MeshDeformPW::fnMirrorSelectedVerts() { MirrorSelectedVerts(); }
...
void MeshDeformPW::fnBakeControlPoints() { BakeControlPoints(); }
void MeshDeformPW::fnRetreiveControlPoints() { RetreiveControlPoints(); }
...
void MeshDeformPW::BakeControlPoints()
{
	...
	for (int wrapIndex = 0; wrapIndex < pblock->Count(pb_meshlist); wrapIndex++)
	{
		...
		*pCt	 = ct;
		...
		node->AddAppDataChunk(MESHDEFORMPW_CLASS_ID, OSM_CLASS_ID, CPDATA_APPDATA, sizeof(InitialTMData) * ct,
			initialDataTemp);  // no TCHAR data
	}
}
...
void MeshDeformPW::fnResample() { Resample(); }
...
Matrix3* MeshDeformPW::fnGetPointInitialTM(int whichWrapMesh, int index)
{
	...
	if ((index < 0) || (index >= wrapMeshData[whichWrapMesh]->initialData.Count()))
		...
		return &wrapMeshData[whichWrapMesh]->initialData[index].initialVertexTMs;
}
Matrix3* MeshDeformPW::fnGetPointCurrentTM(int whichWrapMesh, int index)
{
	...
	if ((index < 0) || (index >= wrapMeshData[whichWrapMesh]->currentVertexTMs.Count()))
		...
		return &wrapMeshData[whichWrapMesh]->currentVertexTMs[index];
}
float   MeshDeformPW::fnGetPointDist(int whichWrapMesh, int index) { return GetPointDist(whichWrapMesh, index); }
int		MeshDeformPW::fnGetPointXVert(int whichWrapMesh, int index) { return GetPointXVert(whichWrapMesh, index) + 1; }
...
float MeshDeformPW::GetPointDist(int whichWrapMesh, int index)
{
	...
	if ((index < 0) || (index >= wrapMeshData[whichWrapMesh]->initialData.Count()))
		...
		return wrapMeshData[whichWrapMesh]->initialData[index].dist;
}
int MeshDeformPW::GetPointXVert(int whichWrapMesh, int index)
{
	...
	if ((index < 0) || (index >= wrapMeshData[whichWrapMesh]->initialData.Count()))
		...
		return wrapMeshData[whichWrapMesh]->initialData[index].xPoints;
}

int MeshDeformPW::fnNumberOfVertices(INode* node) { return NumberOfVertices(node); }

int   MeshDeformPW::fnVertNumberWeights(INode* node, int vindex) { return VertNumberWeights(node, vindex); }
float MeshDeformPW::fnVertGetWeight(INode* node, int vindex, int windex) { return VertGetWeight(node, vindex, windex); }
...
PipeEnumResult FindLocalMeshOnStack::proc(ReferenceTarget* object, IDerivedObject* dobj, int index)
{
	ModContext* curModContext = NULL;
	if ((dobj != NULL))
	{
...
void MeshDeformPW::fnReset() { Reset(); }
...
PipeEnumResult FindSkinOnStatck::proc(ReferenceTarget* object, IDerivedObject* dobj, int index)
{
	ModContext* curModContext = NULL;
	if ((dobj != NULL))
	{
...
void MeshDeformPW::ConvertToSkin(BOOL silent) { SkinWraptoSkin(silent); }
void MeshDeformPW::fnConvertToSkin(BOOL silent) { SkinWraptoSkin(silent); }
...
BOOL MeshDeformPW::SkinWraptoSkin(BOOL silent)
{
	...
	{
		TSTR errorMsg;
		errorMsg.printf(_T("%s"), GetString(IDS_ERROR3));
		MaxSDK::MaxMessageBox(this->hWnd, errorMsg, errorMsg, MB_OK | MB_ICONWARNING);
		return FALSE;
	}
	if (selfNode == NULL)
		return FALSE;
		...
		if (wrapNodes[i])
		{
			...
			if (mod == NULL)
			{
				if (!silent)
				{
					...
					TSTR errorMsg;
					errorMsg.printf(_T("%s %s"), GetString(IDS_ERROR1), wrapNodes[i]->GetName());
					MaxSDK::MaxMessageBox(this->hWnd, errorMsg, errorMsg, MB_OK | MB_ICONWARNING);
				}
	...
	TSTR prompt;
	prompt.printf(_T("%s"), GetString(IDS_STEP5));
	ip->ReplacePrompt(prompt);
		...
		if ((iskin) && (iskinImport))
		{
			...
			for (int i = 0; i < numberOfBones; i++)
			{
				if (i == (numberOfBones - 1))
					...
					iskinImport->AddBoneEx(boneNodeList[i], FALSE);
			}
		}
		else
		{
			TSTR errorMsg;
			errorMsg.printf(_T("%s"), GetString(IDS_ERROR2));
			MaxSDK::MaxMessageBox(this->hWnd, errorMsg, errorMsg, MB_OK | MB_ICONWARNING);
	...
	for (int i = 0; i < localData->weightData.Count(); i++)
	{
		...
		for (int j = 0; j < numberOfBones; j++)
		{
			...
			if (skinData[whichWrapMesh]->iskin)
			{
				...
				{
					TSTR errorMsg;
					errorMsg.printf(_T("%s"), GetString(IDS_ERROR4));
					MaxSDK::MaxMessageBox(this->hWnd, errorMsg, errorMsg, MB_OK | MB_ICONWARNING);
	...
	int weightDataCount = localData->weightData.Count();
	for (int i = 0; i < weightDataCount; i++)
	{
		for (int j = 0; j < weights.Count(); j++)
			weights[j] = 0.0;
		int numberOfBones = localData->weightData[i]->data.Count();

		if ((ip) && (weightDataCount > 10000))
		{
			TSTR prompt;
			if ((i % 200) == 0)
			{
				float per = (float)i / (float)weightDataCount * 100.0f;
				prompt.printf(_T("%s : %d(%d) (%0.0f%%)"), GetString(IDS_STEP4), i, weightDataCount, per);
				ip->ReplacePrompt(prompt);
			}
		}
...
void PickControlNode::EnterMode(IObjParam* ip) { mod->iPickButton->SetCheck(TRUE); }

void PickControlNode::ExitMode(IObjParam* ip) { mod->iPickButton->SetCheck(FALSE); }
...
void MeshDeformPW::fnOutputVertexData(INode* whichNode, int whichVert)
{
	...
	if (ld)
	{
		...
		{
			ScriptPrint(_T("Vertex %d\n"), whichVert);
			ScriptPrint(_T("Number of weights %d\n"), ld->weightData[whichVert]->data.Count());
			for (int i = 0; i < ld->weightData[whichVert]->data.Count(); i++)
			{
				...
				if (node)
					ScriptPrint(_T("Weight %f Owner %s\n"), f, node->GetName());
				else
					ScriptPrint(_T("Weight %f Owner NULL\n"), f);
			}
		}
	}
}
