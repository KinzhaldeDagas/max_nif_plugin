...
// compile-time validates size of ParamSpaceData class to ensure its size doesn't change since instances are 
// saved as a binary blob to scene file. Changes in size will break file i/o
static_assert(sizeof(ParamSpaceData) == sizeof(Point3) + 2 * sizeof(float) + 2 * sizeof(int), "");
...
//		Tab<NeighboringVerts*> lookupList;		/// neighbors list for selected vertices
		...
		MaxSDK::SingleWeakRefMaker selfNode;	/// the inode associated with this local data instance
...
// compile-time validates size of InitialTMData class to ensure its size doesn't change since instances are 
// saved as a binary blob to scene file. Changes in size will break file i/o
static_assert(sizeof(InitialTMData) == sizeof(Matrix3) + sizeof(Point3) + sizeof(int) + 2 * sizeof(float), "");
...
class MeshDeformPW : public Modifier, public IMeshDeformPWModFP 
{
	public:
		...
		// From Animatable
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_MESHDEFORM) : _T("Skin Wrap"); }
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_MESHDEFORM) : _T("Skin Wrap");}
...
//published functions		 
		...
		void RebuildSelectedWeights(LocalMeshData *localData,int wrapMeshIndex, const Matrix3& meshTM, const Matrix3& baseTM, float falloff, float distance, int faceLimit);
		...
		void GetTMFromVertex(TimeValue t, int wrapIndex, Mesh *msh, Point3 *vnorms, const Matrix3& meshToWorld);		
		void BuildCurrentTMFromVertex(TimeValue t,int wrapIndex, Mesh *msh, Point3 *vnorms, const Matrix3& meshTM, const Matrix3& baseTM, const Matrix3& initialBaseTM);
...
class MeshDeformPWClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Skin Wrap"); }
	SClass_ID		SuperClassID() { return OSM_CLASS_ID; }
	Class_ID		ClassID() { return MESHDEFORMPW_CLASS_ID; }
	...
	*/
	...
	Point3 Map(int i, Point3 pt)
	{
		...
		else
		{
			...
			WeightData** weightData = localData->weightData.Count() > 0 ? localData->weightData.Addr(0) : NULL;
			WrapMeshData** wrapMeshData = mod->wrapMeshData.Count() > 0 ? mod->wrapMeshData.Addr(0) : NULL;
			DbgAssert( weightData && wrapMeshData );
			if ( weightData && wrapMeshData )
			{
				for (int j = 0; j < ct; j++)
				{
					BoneInfo& bone = weightData[i]->data[j];
					int id = bone.owningBone;
					float weight = bone.normalizedWeight;
					int wrapIndex = bone.whichNode;
					Matrix3& tm = wrapMeshData[wrapIndex]->defTMs[id];
					vec += ((initialPT * tm) -initialPT) * weight;
				}
			}
			initialPT += vec;
...
class SelectRestore : public RestoreObj 
{
	public:
		...
		TSTR Description() { return TSTR(GetString(IDS_SELECT)); }
...
class LocalScaleRestore : public RestoreObj 
{
	public:
		...
		TSTR Description() { return TSTR(GetString(IDS_SCALE)); }
...
class LocalStrRestore : public RestoreObj 
{
	public:
		...
		TSTR Description() { return TSTR(GetString(IDS_STR)); }
