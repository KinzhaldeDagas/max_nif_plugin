...
class FExtrudeMod : public Modifier {	
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_RB_FACEEXTRUDEMOD) : _T("Face Extrude");}  
		virtual Class_ID ClassID() { return Class_ID(SUB_EXTRUDE_CLASS_ID,0);}
		void BeginEditParams(IObjParam  *ip, ULONG flags,Animatable *prev);
		void EndEditParams(IObjParam *ip,ULONG flags,Animatable *next);		
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_FACEEXTRUDEMOD) : _T("Face Extrude");}
...
class FaceExtrudeClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Face Extrude"); }
...
static ParamUIDesc descParam[] = {
	...
	// Backface
	ParamUIDesc(PB_CENTER,TYPE_SINGLECHECKBOX,IDC_EXT_CENTER),
	};
...
RefTargetHandle FExtrudeMod::Clone(RemapDir& remap)
	...
	mod->ReplaceReference(POINT_REF,remap.CloneRef(base));	
...
void FExtrudeMod::ModifyObject(
	...
	if (os->obj->IsSubClassOf(triObjectClassID)) {
		...
		//we need to grab the incoming selection interval because as the
		//incoming selection changes this changes the topology.
		//Basically the incoming selection interval drives the out going channel intervals
		Interval selectionInterval = tobj->ChannelValidity(t, SELECT_CHAN_NUM);
		...
		//we need to include our incoming selection interval since that can change our out going
		//intervals of other channels
		iv = iv & selectionInterval;
...
TSTR FExtrudeMod::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case POINT_REF: return localized ? GetString(IDS_RB_EXTRUDECENT) : _T("Extrude Center");
	default: return _T("");
	}
}
...
RefResult FExtrudeMod::NotifyRefChanged( 
	...
	switch (message) {
		...
		case REFMSG_GET_PARAM_NAME_NONLOCALIZED: {
			GetParamName *gpn = (GetParamName*)partID;
			switch (gpn->index) {
				case PB_AMOUNT:  gpn->name = _T("Amount"); break;
				case PB_SCALE:   gpn->name = _T("Scale");  break;
				}
			return REF_HALT;
			}
		}
