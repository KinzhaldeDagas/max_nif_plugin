...
class SkewMod : public SimpleMod {
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_RB_SKEWMOD) : _T("SkewMod"); }
		virtual Class_ID ClassID() { return Class_ID(SKEWOSM_CLASS_ID,0);}
		void BeginEditParams( IObjParam  *ip, ULONG flags,Animatable *prev);
		void EndEditParams( IObjParam *ip,ULONG flags,Animatable *next);
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_SKEW) : _T("Skew"); }
...
public:
	...
	int doRegion;
	SkewDeformer();
	SkewDeformer(ModContext& mc, float amount, float dir, int naxis, float from, float to, int doRegion,
...
class SkewWSM : public SimpleOSMToWSMObject {
	public:
		...
		RefTargetHandle Clone(RemapDir& remap)
		{
			SkewWSM *newobj = new SkewWSM((SkewMod*)remap.CloneRef(mod));
...
class SkewClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Skew"); }
...
static ParamUIDesc descParam[] = {
	...
	// Affect region
	ParamUIDesc(PB_DOREGION,TYPE_SINGLECHECKBOX,IDC_SKEW_AFFECTREGION),
...
SkewMod::SkewMod() : SimpleMod()
	{	
	ReplaceReference(SIMPMOD_PBLOCKREF, CreateParameterBlock(descVer0, PBLOCK_LENGTH, CURRENT_VERSION));
...
Point3 SkewDeformer::Map(int i, Point3 p) const
{
	float z;
	p = p * tm;
	z = p.z;
...
bool SkewDeformer::IsThreadSafe() const
{
	return true;
}
...
TSTR SkewMod::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_AMOUNT:	return localized ? GetString(IDS_RB_AMOUNT)    : _T("Amount");
		case PB_DIR:		return localized ? GetString(IDS_RB_DIRECTION) : _T("Direction");
