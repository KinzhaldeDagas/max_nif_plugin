...
public:
		...
		BOOL ForceUpdate() const { return mForceUpdate; }
		void ForceUpdate(BOOL val) { mForceUpdate = val; }

		// From Animatable
		void DeleteThis() { delete this; }
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_RB_OPTMOD) : _T("OptMod"); }  
		virtual Class_ID ClassID() { return Class_ID(OPTIMIZEOSM_CLASS_ID,0);}		
		void BeginEditParams( IObjParam  *ip, ULONG flags,Animatable *prev);
		void EndEditParams( IObjParam *ip,ULONG flags,Animatable *next);		
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_OPTIMIZE) : _T("Optimize"); }
...
private:
		virtual void SetReference(int , RefTargetHandle rtarg) {pblock = (IParamBlock*)rtarg;}
public:
...
class OptClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Optimize"); }
...
static ParamUIDesc descParam[] = {
	...
	ParamUIDesc(PB_PRESERVEMAT1,TYPE_SINGLECHECKBOX,IDC_OPT_PRESERVEMAT),	

	// Preserve smooth boundries
	ParamUIDesc(PB_PRESERVESMOOTH1,TYPE_SINGLECHECKBOX,IDC_OPT_PRESERVESMOOTH),
	...
	// Auto edge
	ParamUIDesc(PB_AUTOEDGE,TYPE_SINGLECHECKBOX,IDC_OPT_AUTOEDGE),

	// Manual update
	ParamUIDesc(PB_MANUPDATE,TYPE_SINGLECHECKBOX,IDC_OPT_MANUPDATE),	
	};
...
void OptDlgProc::SetupLevels(IParamMap *map)
	...
	map->GetParamBlock()->GetValue(PB_VIEWS,0,l,for_ever);
	if (l==0) {
...
void OptDlgProc::SetupManButton(HWND hWnd,IParamMap *map)
	...
	Interval for_ever = FOREVER;
	int man = 0;
	map->GetParamBlock()->GetValue(PB_MANUPDATE,0,man,for_ever);
	EnableWindow(GetDlgItem(hWnd,IDC_OPT_UPDATE),man);
	}

INT_PTR OptDlgProc::DlgProc(
...
{	
	pblock = NULL;
	ReplaceReference( 0, CreateParameterBlock(descVer2, PBLOCK_LENGTH, CURRENT_VERSION));	
...
int OptMod::RenderBegin(TimeValue /*t*/, ULONG /*flags*/)
{
	...
	Interval for_ever = FOREVER;
	pblock->GetValue(PB_VIEWS,0,views,for_ever);
	pblock->GetValue(PB_RENDER,0,render,for_ever);	
	for_ever = FOREVER;
	pblock->GetValue(PB_MANUPDATE,0,man,for_ever);
	SetAFlag(A_RENDER);
	if (views!=render || man) {
...
void OptMod::ModifyObject(TimeValue t, ModContext& /*mc*/, ObjectState* os, INode* /*node*/)
{
	...
	try
	{
		...
		int manual_update;
		pblock->GetValue(PB_MANUPDATE,t,manual_update,valid);
		if (manual_update && !mForceUpdate && !TestAFlag(A_RENDER))
			return;
		...
		float faceThresh = 0.0f, edgeThresh = 0.0f, bias = 0.0f, maxEdge = 0.0f;
		int preserveMat = 0, preserveSmooth = 0;
		...
		unsigned int nv = triOb->GetMesh().getNumVerts();
		unsigned int nf = triOb->GetMesh().getNumFaces();

		DWORD flags = 0;
		if (preserveMat) 
			flags |= OPTIMIZE_SAVEMATBOUNDRIES;
		if (preserveSmooth) 
			flags |= OPTIMIZE_SAVESMOOTHBOUNDRIES;
		if (autoEdge) 
			flags |= OPTIMIZE_AUTOEDGE;
		...
		if (faceThresh != 0.0f) {
			...
			// This call can run out memory 
			triOb->GetMesh().Optimize( faceThresh, edgeThresh, bias * 0.5f, maxEdge, flags, this);
		...
		if (pmapParam && pmapParam->GetParamBlock()==pblock && !TestAFlag(A_RENDER)) {
			TSTR buf;
			buf.printf(_T("%d / %d"),nv,triOb->GetMesh().getNumVerts());
			SetWindowText(GetDlgItem(pmapParam->GetHWnd(),IDC_OPT_VERTCOUNT),buf);
			buf.printf(_T("%d / %d"),nf,triOb->GetMesh().getNumFaces());
			SetWindowText(GetDlgItem(pmapParam->GetHWnd(),IDC_OPT_FACECOUNT),buf);
		}
	}
	catch (std::bad_alloc&)
	{
		UtilityInterface::DisplayOutOfMemoryWarning();				
	}
}
	...
	case REFMSG_GET_PARAM_NAME_NONLOCALIZED:
		{
			...
			switch (gpn->index)
			{
			case PB_FACETHRESH1:	gpn->name = _T("Face Threshold L1"); break;
			case PB_FACETHRESH2:	gpn->name = _T("Face Threshold L2"); break;
			case PB_EDGETHRESH1:	gpn->name = _T("Edge Threshold L1"); break;
			case PB_EDGETHRESH2:	gpn->name = _T("Edge Threshold L2"); break;
			case PB_BIAS1:			gpn->name = _T("Bias L1");           break;
			case PB_BIAS2:			gpn->name = _T("Bias L2");           break;
			case PB_MAXEDGE1:		gpn->name = _T("Max Edge Length 1"); break;
			case PB_MAXEDGE2:		gpn->name = _T("Max Edge Length 2"); break;
			default:					gpn->name = TSTR(_T(""));            break;
			}
			return REF_HALT;
		}
