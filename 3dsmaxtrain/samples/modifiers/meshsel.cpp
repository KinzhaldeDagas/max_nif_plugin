...
class ActionCB;
class MeshSelMod : public Modifier, public IMeshSelect, public FlagUser, public EditSSCB {
public:
	DWORD selLevel;
	Tab<TSTR*> namedSel[3];
	Tab<DWORD> ids[3];
	...
	static ActionCB		*mpActions;

	MeshSelMod();

	// From Animatable
	void DeleteThis() { delete this; }
	void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_RB_MESHSELMOD) : _T("Mesh Select");}  
	virtual Class_ID ClassID() { return Class_ID(MESHSELECT_CLASS_ID,0);}		
	RefTargetHandle Clone(RemapDir& remap);
	const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_MESHSELMOD) : _T("Mesh Select");}
...
EditSSMode*	MeshSelMod::editSSMode	= NULL;
ActionCB*	MeshSelMod::mpActions	= NULL;
Tab<ActionItem*> MeshSelMod::mOverrides;

const ActionTableId kActionID = 0x56dc7bf8;

class ActionCB : public ActionCallback {
	MeshSelMod *mpMod;
public:
	ActionCB (MeshSelMod *m) : mpMod(m) { }
...
ActionTable* GetActions() {
	TSTR name = GetString (IDS_RB_MESHSELMOD);
...
protected:
	...
	MeshSelMod *mpObj;
public:
	EditSSOverride(MeshSelMod *m):mActiveOverride(FALSE),mpObj(m){}
...
class MeshSelClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Mesh Select"); }
...
enum { ms_use_softsel, ms_use_edist, ms_edist, ms_affect_backfacing,
		...
		ms_matid, ms_ignore_visible, ms_planar_threshold, ms_use_stack_selection };
...
static ParamBlockDesc2 meshsel_softsel_desc ( ms_pblock,
	...
	ms_affect_backfacing, _T("softselAffectBackfacing"), TYPE_BOOL, P_RESET_DEFAULT, IDS_AFFECT_BACKFACING,
		p_default, true,
		p_ui, ms_map_softsel, TYPE_SINGLECHECKBOX, IDC_MS_SS_BACKFACING,
		p_end,
	...
	ms_falloff, _T("softselFalloff"), TYPE_WORLD, P_ANIMATABLE|P_RESET_DEFAULT, IDS_PW_FALLOFF,
		...
		p_nonLocalizedName, _T("Falloff type"),
		p_end,
	...
	ms_by_vertex, _T("byVertex"), TYPE_BOOL, P_RESET_DEFAULT, IDS_BY_VERTEX,
		p_default, false,
		p_ui, ms_map_main, TYPE_SINGLECHECKBOX, IDC_MS_SEL_BYVERT,
		p_end,

	ms_ignore_backfacing, _T("ignoreBackfacing"), TYPE_BOOL, P_RESET_DEFAULT, IDS_IGNORE_BACKFACING,
		p_default, false,
		p_ui, ms_map_main, TYPE_SINGLECHECKBOX, IDC_MS_IGNORE_BACKFACES,
		p_end,
	...
	ms_ignore_visible, _T("ignoreVisibleEdges"), TYPE_BOOL, P_RESET_DEFAULT, IDS_IGNORE_VISIBLE,
		p_default, false,
		p_ui, ms_map_main, TYPE_SINGLECHECKBOX, IDC_MS_IGNORE_VISEDGE,
		p_end,
	...
	ms_planar_threshold, _T("planarThreshold"), TYPE_ANGLE, P_RESET_DEFAULT, IDS_RB_THRESHOLD,
		...
		p_end,
	ms_use_stack_selection, _T("useStackSelection"), TYPE_BOOL, P_RESET_DEFAULT, IDS_SEL_STACK,
		p_default, false,
		p_ui, ms_map_main, TYPE_SINGLECHECKBOX, IDC_USE_STACK_SELECTION,
		p_end,
	p_end
);
...
void MeshSelMod::ModifyObject (TimeValue t, ModContext &mc, ObjectState *os, INode *node) {
	...
	if (pblock->GetInt(ms_use_stack_selection))
	{
		d->LockSelection(tobj->GetMesh());
	}
...
void MeshSelMod::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc) {
	if (!mc->localData) return;
	if (partID == PART_SELECT) return;

	if (pblock->GetInt(ms_use_stack_selection))
	{
		((MeshSelData*)mc->localData)->InvalidateVDistances();	
		return;
	}
...
int MeshSelMod::HitTest (TimeValue t, INode* inode, int type, int crossing, int flags, IPoint2 *p, ViewExp *vpt, ModContext* mc) {
	...
	if ((hitFlags & SUBHIT_VERTS) && ignoreBackfaces) {
		...
		faceBack.ClearAll ();
		BitArray& vertHide = mesh.VertHide();
		for (int i=0; i<mesh.getNumFaces(); i++) {
			...
			for (int j=0; j<3; j++) vertHide.Set (vv[j]);
			faceBack.Set (i);
		}
		for (int i=0; i<mesh.getNumFaces(); i++) {
			if (faceBack[i]) continue;
			DWORD *vv = mesh.faces[i].v;
			for (int j=0; j<3; j++) vertHide.Clear (vv[j]);
		}
...
void SelectProc::SetEnables (HWND hParams) {
	...
	if (mod->pblock->GetInt(ms_use_stack_selection))
	{
...
void MeshSelData::LockSelection(Mesh& mesh)
{
	vertSel = mesh.VertSel();
	faceSel = mesh.FaceSel();
	edgeSel = mesh.EdgeSel();
...
void MeshSelData::SetCache(Mesh &mesh) {
	...
	if (temp) temp->Invalidate (TOPO_CHANNEL);
	SynchBitArrays ();
}
...
void MeshSelData::SetVertSel(const BitArray &set, IMeshSelect *imod, TimeValue t) {
...
void MeshSelData::SetFaceSel(const BitArray &set, IMeshSelect *imod, TimeValue t) {
	MeshSelMod *mod = (MeshSelMod *) imod;
...
void MeshSelData::SetEdgeSel(const BitArray &set, IMeshSelect *imod, TimeValue t) {
	MeshSelMod *mod = (MeshSelMod *) imod;
...
void MeshSelMod::NSPaste() {
	...
	for (int i = 0; i < mcList.Count(); i++) {
		...
		if (i >= clip->sets.Count()) {
			...
			setList->InsertSet(*clip->sets[i],id,name);
		}
...
RefResult MeshSelMod::NotifyRefChanged (const Interval& changeInt, RefTargetHandle hTarget, 
   		PartID& partID, RefMessage message, BOOL propagate) {
...
void MeshSelMod::InvalidateDialogElement(ParamID elem) {
	if (!pblock) return;
	if (!meshSelDesc.NumParamMaps ()) return;
	IParamMap2 *pmap = meshSelDesc.GetParamMap (ms_map_main);
...
void MeshSelMod::DoAccept(TimeValue t)
{

}
void MeshSelMod::SetPinch(TimeValue t, float pinch)
{
	pblock->SetValue (ms_pinch, t, pinch);

}
void MeshSelMod::SetBubble(TimeValue t, float bubble)
{
	pblock->SetValue (ms_bubble, t, bubble);
}
float MeshSelMod::GetPinch(TimeValue t)
{
	return pblock->GetFloat (ms_pinch, t);
}

float MeshSelMod::GetBubble(TimeValue t)
{
	return pblock->GetFloat (ms_bubble, t);
}

float MeshSelMod::GetFalloff(TimeValue t)
{
	return pblock->GetFloat (ms_falloff, t);
}
void MeshSelMod::SetFalloff(TimeValue t,float falloff)
{
	pblock->SetValue (ms_falloff, t, falloff);
}

void MeshSelMod::EditSoftSelectionMode()
{
	if (!ip) return;
