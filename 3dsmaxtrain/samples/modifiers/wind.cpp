...
class WindObject : public SimpleWSMObject2 // ,IOperatorInterface // Bayboro 9/18/01
{
	public:
		...
		WindObject();
		~WindObject();
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_WIND) : _T("Wind");}
...
class WindClassDesc:public ClassDesc2 
{
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Wind"); }
...
class WindMod : public SimpleWSMMod 
{
	public:				
		...
		// From Animatable
		void GetClassName(MSTR& s, bool localized) const override {s= localized ? GetString(IDS_RB_WINDMOD) : _T("WindMod");}
		SClass_ID SuperClassID() {return WSM_CLASS_ID;}
		void DeleteThis() {delete this;}
		Class_ID ClassID() { return Class_ID(WINDMOD_CLASS_ID,0);}
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_WINDBINDING) : _T("Wind Binding");}
...
class WindModClassDesc:public ClassDesc 
{
	...
	const TCHAR*	NonLocalizedClassName() { return _T("WindMod"); }
	SClass_ID		SuperClassID() {return WSM_CLASS_ID;}
	Class_ID		ClassID() {return Class_ID(WINDMOD_CLASS_ID,0);}
...
static ParamBlockDesc2 wind_param_blk ( wind_params, _T("WindParameters"),  0, &windDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF_NO, 
	...
	PB_HOOPSON,   _T("hoopson"),		TYPE_BOOL, 0, IDS_ECP_HOOPSON,
		p_default, 		FALSE, 
		p_ui,			TYPE_SINGLECHECKBOX, IDC_EP_HOOPSON, 
		p_end,
...
static ParamUIDesc descParamWind[] = {
	...
	ParamUIDesc(PB_HOOPSON,TYPE_SINGLECHECKBOX,IDC_EP_HOOPSON),
...
/*
TSTR WindObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_STRENGTH:		return localized ? GetString(IDS_RB_STRENGTH2)  : _T("Strength");
		case PB_DECAY:			return localized ? GetString(IDS_RB_DECAY)      : _T("Decay");
		case PB_TURBULENCE:	return localized ? GetString(IDS_RB_TURBULENCE) : _T("Turbulence");
		case PB_FREQUENCY:	return localized ? GetString(IDS_RB_FREQUENCY)  : _T("Frequency");
		case PB_SCALE:			return localized ? GetString(IDS_RB_SCALE)      : _T("Scale");
...
Point3 WindField::Force(
	...
	bool bUpdateVariables = false;
	if (!fValid.InInterval(t))
	{
		fValid = FOREVER;
		obj->pblock2->GetValue(PB_DECAY, t, decay, fValid);
		obj->pblock2->GetValue(PB_TURBULENCE, t, turb, fValid);
		obj->pblock2->GetValue(PB_STRENGTH, t, strength, fValid);
		obj->pblock2->GetValue(PB_FREQUENCY, t, freq, fValid);
		obj->pblock2->GetValue(PB_SCALE, t, scale, fValid);
		bUpdateVariables = true;
	}
	...
	if (bUpdateVariables || type==FORCE_SPHERICAL || decay!=0.0f) {
		...
		if (type==FORCE_PLANAR) {
			...
			if (decay!=0.0f) {
				...
				localStrength *= (float)exp(-decay*dist);
				}			
			force *= localStrength * 0.0001f * forceScaleFactor;
		} else {
			float dist;
	...
	if (turb!=0.0f) {
		...
		float localFreq = freq * .01f;
		float localTurb = turb * 0.0001f * forceScaleFactor;

		p    = pt;
		p.x = localFreq * float(t);
		tf.x = RTurbulence(p,scale);
		p    = pt;
		p.y = localFreq * float(t);
		tf.y = RTurbulence(p,scale);
		p    = pt;
		p.z = localFreq * float(t);
		tf.z = RTurbulence(p,scale);

		return force + (localTurb*tf);
	} else {
		return force;
		}
	}
