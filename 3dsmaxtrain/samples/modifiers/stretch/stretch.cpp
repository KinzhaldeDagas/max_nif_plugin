...
class StretchWSM : public SimpleOSMToWSMObject {
	public:
		...
		RefTargetHandle Clone(RemapDir& remap)
		{
			StretchWSM *newobj = new StretchWSM((StretchMod*)remap.CloneRef(mod));
...
static ParamUIDesc descParam[] = {
        ...
        // Affect region
        ParamUIDesc(PB_DOREGION,TYPE_SINGLECHECKBOX,IDC_STRETCH_AFFECTREGION),
...
{
public:
    INT_PTR DlgProc(TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,
                    LPARAM lParam);
    void DeleteThis() {}
};
static StretchDlgProc theStretchProc;

INT_PTR StretchDlgProc::DlgProc(TimeValue t, IParamMap *map,
                             HWND hWnd, UINT msg, WPARAM wParam, 
                             LPARAM lParam )
{
	// Just to be safe, here we reset the interval value to FOREVER between each calls.
	// The validity could be affected by GetValue.
	// TBD, in order to remove the redundant reset, we need to add a signature in paramblk.
    ...
    case CC_SPINNER_CHANGE:
        ...
        case IDC_STRETCH_FROMSPIN: {
            float from, to;
			valid = FOREVER;
			map->GetParamBlock()->GetValue(PB_FROM, t, from, valid);
			valid = FOREVER;
			map->GetParamBlock()->GetValue(PB_TO, t, to, valid);
            if (from>to) {
...
const TCHAR* StretchClassDesc::NonLocalizedClassName()
{
	return _T("Stretch");
}
...
const TCHAR* StretchWSMClassDesc::NonLocalizedClassName()
{
	return _T("Stretch");
}
...
{
    ReplaceReference(SIMPMOD_PBLOCKREF, CreateParameterBlock(descVer, PBLOCK_LENGTH, 0));
        
    pblock->SetValue(PB_AXIS, TimeValue(0), 2/*Z*/);
}

void StretchMod::GetClassName(MSTR& s, bool localized) const
{ 
    static int loaded = 0;
    static TCHAR stringBuf[MAX_STRING_LENGTH];
    ...
    s = localized ? stringBuf : _T("Stretch");
}  
...
const TCHAR * StretchMod::GetObjectName(bool localized) const
{ 
    static int loaded = 0;
    static TCHAR stringBuf[MAX_STRING_LENGTH];
    ...
    return localized ? stringBuf : _T("Stretch");
}

const TCHAR * StretchWSM::GetObjectName(bool localized) const
{ 
    static int loaded = 0;
    static TCHAR stringBuf[MAX_STRING_LENGTH];
                                  ...
                                  Matrix3& invmat)
{
	...
	// TBD, in order to remove the redundant reset, we need to add a signature in paramblk.
	Interval valid = FOREVER;
	pblock->GetValue(PB_STRETCH, t, angle, valid);
	valid = FOREVER;
	pblock->GetValue(PB_AMPLIFY, t, dir, valid);
	valid = FOREVER;
	pblock->GetValue(PB_AXIS, t, axis, valid);
	valid = FOREVER;
	pblock->GetValue(PB_FROM, t, from, valid);
	valid = FOREVER;
	pblock->GetValue(PB_TO, t, to, valid);
	valid = FOREVER;
	pblock->GetValue(PB_DOREGION, t, doRegion, valid);
...
TSTR StretchMod::GetParameterName(int pbIndex, bool localized)
{
    const int kBufLen = 1024;
    static TCHAR buf[kBufLen];
    ...
    if (localized)
    {
        ...
        {
        case PB_STRETCH:
            LoadString(hResource, IDS_STRETCH, buf, kBufLen);
            return TSTR(buf);
        case PB_AMPLIFY:
            LoadString(hResource, IDS_AMPLIFY, buf, kBufLen);
            return TSTR(buf);
        case PB_AXIS:
            LoadString(hResource, IDS_AXIS, buf, kBufLen);
            return TSTR(buf);
        case PB_FROM:
            LoadString(hResource, IDS_FROM, buf, kBufLen);
            return TSTR(buf);
        case PB_TO:
            LoadString(hResource, IDS_TO, buf, kBufLen);
            return TSTR(buf);
        default:
            return TSTR(_T(""));
        }
    ...
    else
    {
        switch (pbIndex)
        {
            case PB_STRETCH: return _T("Stretch");
            case PB_AMPLIFY: return _T("Amplify");
            case PB_AXIS:    return _T("Axis");
            case PB_FROM:    return _T("From");
            case PB_TO:      return _T("To");
...
BOOL StretchMod::GetModLimits(TimeValue t,float &zmin, float &zmax, int &axis)
{
	...
	// TBD, in order to remove the redundant reset, we need to add a signature in paramblk.
	Interval valid = FOREVER;
	pblock->GetValue(PB_DOREGION, t, limit, valid);
	valid = FOREVER;
	pblock->GetValue(PB_FROM, t, zmin, valid);
	valid = FOREVER;
	pblock->GetValue(PB_TO, t, zmax, valid);
	valid = FOREVER;
	pblock->GetValue(PB_AXIS, t, axis, valid);
...
Point3 StretchDeformer::Map(int i, Point3 p) const
{
...
bool StretchDeformer::IsThreadSafe() const
{
	return true;
}
