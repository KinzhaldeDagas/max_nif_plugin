...
class SWrapObject : public SimpleWSMObject {
	public:
		...
		Mesh *cmesh;
		Matrix3 tm;
		std::vector<MaxSDK::VertexNormal> vnorms;
		Tab<Point3> fnorms;
		...
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_AP_SWRAPOBJ) : _T("Conform Object");}
...
class SWrapClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Conform"); }
...
class SWrapMod : public SimpleWSMMod {
	public:				
		...
		// From Animatable
		void GetClassName(MSTR& s, bool localized) const override {s= localized ? GetString(IDS_AP_SWRAPMOD) : _T("Conform Mod");}
		SClass_ID SuperClassID() {return WSM_CLASS_ID;}
		void DeleteThis() {delete this;}
		Class_ID ClassID() { return SWRAPMOD_CLASS_ID;}
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_AP_SWRAPBINDING) : _T("Conform Binding");}
...
class SWrapModClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Conform Mod"); }
	SClass_ID		SuperClassID() {return WSM_CLASS_ID; }
	Class_ID		ClassID() {return SWRAPMOD_CLASS_ID;}
	const TCHAR* 	Category() {return GetSpaceWarpCatString(SPACEWARP_CAT_GEOMDEF);}
	};
...
static ParamUIDesc descParam[] = {
	...
	ParamUIDesc(PB_USESELVERTS,TYPE_SINGLECHECKBOX,IDC_AP_USESELVERTS),
...
class CreateSWrapObjectProc : public MouseCallBack,ReferenceMaker {
	private:
		...
		virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("CreateSWrapObjectProc"); } // from Animatable
...
{ if ( SWrapObj ) { 
	...
	SWrapObj->EndEditParams( (IObjParam*)createInterface, 
		...
		{	// RB 4-9-96: Normally the hold isn't holding when this 
			...
			theHold.Suspend(); 
			SWrapObj->DeleteAllRefsFromMe();
			SWrapObj->DeleteAllRefsToMe();
			SWrapObj->DeleteThis();
			SWrapObj = NULL;
			theHold.Resume();
...
{	int res=TRUE;	
	...
	switch ( msg ) {
		...
		case 0: {
					...
					// link it up
					INode * l_SWrapNode = createInterface->CreateObjectNode( SWrapObj);
					attachedToNode = TRUE;
					assert( l_SWrapNode );					
					createCB = NULL;
					createInterface->SelectNode( l_SWrapNode );
					
					// Reference the new node so we'll get notifications.
					theHold.Suspend();
					ReplaceReference( 0, l_SWrapNode);
					theHold.Resume();
					mat.IdentityMatrix();
					}
				default:				
...
{TSTR name=TSTR(GetString(IDS_AP_OBJECTSTR)) + (custnode ? custname.data() : GetString(IDS_AP_NONE));
...
INT_PTR SWrapObjectDlgProc::DlgProc(
...
{ 
	...
	custnode=NULL;
	pblock = NULL;
	ReplaceReference(0, CreateParameterBlock(SWrapdescVer0, PBLOCK_LENGTH, CURRENT_VERSION));
	assert(pblock);	
...
{ pblock=NULL;
  ...
  std::vector<MaxSDK::VertexNormal>().swap(vnorms); // shrink the vector
...
void SWrapObject::EndEditParams( IObjParam *ip, ULONG flags,Animatable *next)
{		
	this->ip = NULL;
...
TSTR SWrapObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_KIDEFAULT:	return localized ? GetString(IDS_AP_KIDEFAULT) : _T("Projection Distance");
		case PB_USESELVERTS:	return localized ? GetString(IDS_AP_SELVERTS)  : _T("Selected Verts");
		case PB_ICONSIZE:		return localized ? GetString(IDS_AP_ICONSIZE)  : _T("Icon Size");
		case PB_STANDOFF:		return localized ? GetString(IDS_AP_STANDOFF)  : _T("Standoff Distance");
...
SWrapMod::SWrapMod(INode *node,SWrapObject *obj)
	{	
	if (node) ReplaceReference(SIMPWSMMOD_NODEREF,node);	
	}
...
int RayIntersect(Ray& ray, float& at, Point3& norm,Mesh *amesh,std::vector<MaxSDK::VertexNormal> vnorms,Tab<Point3> fnorms)
{
	MaxSDK::VertexNormal vnormal;

	vnormal.AddNormal(Point3(1.0, 1.0, 1.0),1);
	vnormal.AddNormal(Point3(1.0, 1.0, 1.0),1);
	vnormal.AddNormal(Point3(1.0, 1.0, 1.0),1);
	vnormal.AddNormal(Point3(1.0, 1.0, 1.0),1);
...
Point3 DoIntersect(Point3 vert,Ray ray,Mesh *mesh,float kdef,float standoff,int *kfound,Point3 v,std::vector<MaxSDK::VertexNormal> vnorms,Tab<Point3> fnorms) 
{ ray.p=vert;
...
void GetVFLst(Mesh* dmesh,std::vector<MaxSDK::VertexNormal>* vnorms,Tab<Point3>* fnorms)	 
			...
			{ if (obj->cmesh) delete obj->cmesh; 
			  obj->cmesh=new Mesh;
			   obj->cmesh->DeepCopy(&wrapOb->GetMesh(),
			     GEOM_CHANNEL|SELECT_CHANNEL|SUBSEL_TYPE_CHANNEL|TOPO_CHANNEL|TM_CHANNEL);
...
/*		  pm->buildLinkages();
		...
		else if ((towrapOb=IsUseable(os->obj,t)) != NULL)
		{	Point3 tvector;
			float dist;
			float *vssel = NULL;
		  if (selverts) 
			  vssel = towrapOb->GetMesh().getVSelectionWeights();
		  BitArray& vertSel = towrapOb->GetMesh().VertSel();
		  for (int i=0;i<towrapOb->GetMesh().getNumVerts();i++)
		  { 
				if ((!selverts)||(vertSel[i])
							   ||(vssel&&vssel[i]))
				{
