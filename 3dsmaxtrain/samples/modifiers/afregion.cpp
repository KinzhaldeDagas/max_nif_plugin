...
class AFRMod : public Modifier {	
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_RB_AFRMOD) : _T("Affect Region");}  
		virtual Class_ID ClassID() { return Class_ID(AFFECTREGION_CLASS_ID,0);}		
		void BeginEditParams(IObjParam  *ip, ULONG flags,Animatable *prev);
		void EndEditParams(IObjParam *ip,ULONG flags,Animatable *next);		
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_AFRMOD) : _T("Affect Region");}
...
public:
	...
	Point3 p1, p2;
	Tab<Point3>* normals;
	AFRDeformer(ModContext& mc, float f, float p, float b, Point3 pt1, Point3 pt2, Tab<Point3>* n = NULL);
...
class AFRClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Affect Region"); }
...
static ParamUIDesc descParam[] = {
	...
	// Backface
	ParamUIDesc(PB_BACKFACE,TYPE_SINGLECHECKBOX,IDC_AFR_BACKFACE),
...
AFRMod::AFRMod() 
	...
	ReplaceReference(POINT2_REF,NewDefaultPoint3Controller());
	sel[0] = sel[1] = 0;
...
RefTargetHandle AFRMod::Clone(RemapDir& remap)
	...
	mod->ReplaceReference(POINT2_REF,remap.CloneRef(p2));
...
TSTR AFRMod::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case POINT1_REF: return localized ? GetString(IDS_RB_STARTPOINT) : _T("Start Point"); break;
	case POINT2_REF: return localized ? GetString(IDS_RB_ENDPOINT) : _T("End Point"); break;
	...
	switch (message) {
		...
		case REFMSG_GET_PARAM_NAME_NONLOCALIZED: {
			...
			switch (gpn->index) {
				...
				case PB_PINCH:   gpn->name = _T("Pinch");   break;
				case PB_BUBBLE:  gpn->name = _T("Bubble");  break;
				}
			return REF_HALT;
			}
		}
...
Point3 AFRDeformer::Map(int i, Point3 p) const
{
...
bool AFRDeformer::IsThreadSafe() const
{
	return true;
}
