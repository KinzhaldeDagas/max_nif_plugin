...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_CONVERT_TO_POLY) : _T("Turn to Poly"); }  
	virtual Class_ID ClassID() { return CONVERT_TO_POLY_ID; }		
	RefTargetHandle Clone(RemapDir& remap);
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_CONVERT_TO_POLY) : _T("Turn to Poly"); }
    ...
    // Save and load operations for this class
    virtual IOResult Save(ISave* isave);
    virtual IOResult Load(ILoad* iload);
private:

    // Enumeration to track versioning of this modifier
    enum ConvertToPolyVersionType
    {
        CONVERT_TO_POLY_ORIGINAL = 0,
        CONVERT_TO_POLY_2021     = 1
    };

    // Enumeration used to identify data "chunks" saved/loaded by this class
    enum ConvertToPolyChunkType
    {
        CHUNK_MODIFIER = 0,    // Chunk containing data written by the Modifier base class
        CHUNK_VERSION  = 1,    // Chunk containing version number data
    };

    // Start chunk ID, encapsulating all other data chunks; chosen for likely uniqueness, strictly just needs to be different
    // from MODIFIERBASE_CHUNK
    static constexpr USHORT m_convertToPolyStartTag = 32965;

    // Version value for this instance
    int m_convertToPolyVersion;
...
class ConvertToPolyClassDesc : public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Turn to Poly"); }
...
static ParamBlockDesc2 turn_param_desc ( turn_params, _T("convertToPolyParams"),
	...
	turn_keep_convex, _T("keepConvex"), TYPE_BOOL, P_RESET_DEFAULT|P_ANIMATABLE, IDS_KEEP_CONVEX,
		p_default, FALSE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_KEEP_CONVEX,
		p_nonLocalizedName, _T("Keep Convex"),
		p_end,

	turn_limit_size, _T("limitPolySize"), TYPE_BOOL, P_RESET_DEFAULT|P_ANIMATABLE, IDS_LIMIT_SIZE,
		p_default, FALSE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_LIMIT_SIZE,
		p_enable_ctrls,	1, turn_max_size,
		p_nonLocalizedName, _T("Limit Polygon Size"),
		p_end,
	...
	turn_max_size, _T("maxPolySize"), TYPE_INT, P_RESET_DEFAULT|P_ANIMATABLE, IDS_MAX_SIZE,
		...
		p_nonLocalizedName, _T("Max Polygon Size"),
		p_end,

	turn_planar, _T("requirePlanar"), TYPE_BOOL, P_RESET_DEFAULT|P_ANIMATABLE, IDS_PLANAR,
		p_default, FALSE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_PLANAR,
		p_enable_ctrls,	1, turn_thresh,
		p_nonLocalizedName, _T("Require Planar Polygons"),
		p_end,
	...
	turn_softsel, _T("useSoftSelection"), TYPE_BOOL, P_RESET_DEFAULT, IDS_USE_SOFTSEL,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_USE_SOFTSEL,
		p_end,
...
ConvertToPoly::ConvertToPoly():
    m_convertToPolyVersion(CONVERT_TO_POLY_2021)
{
	pblock = NULL;
...
IOResult ConvertToPoly::Save(ISave* isave)
{
    ULONG nWritten;

    // Check in debug mode for a valid save object
    DbgAssert(isave != NULL);

    // First, begin the chunk that will contain all of our data
    isave->BeginChunk(m_convertToPolyStartTag);

    // Next, save base class data
    isave->BeginChunk(CHUNK_MODIFIER);
    IOResult resultMod = Modifier::Save(isave);
    isave->EndChunk();

    // Now, save version number
    isave->BeginChunk(CHUNK_VERSION);
    IOResult resultVer = isave->Write(&m_convertToPolyVersion, sizeof(int), &nWritten);
    DbgAssert(nWritten == sizeof(int));
    isave->EndChunk();

    // Finally, close outer chunk
    isave->EndChunk();

    // The save was successful if both writes completed correctly, otherwise, we have an error
    IOResult result = IO_OK;
    if ((resultMod != IO_OK) || (resultVer != IO_OK))
    {
        result = IO_ERROR;
    }
...
IOResult ConvertToPoly::Load(ILoad* iload)
{

    // Check in debug mode for a valid load object
    DbgAssert(iload != NULL);

    // Load all data for this class; note that we initialize the version number to the original, as files corresponding to that
    // version will lack the version chunk
    IOResult result = IO_OK;
    m_convertToPolyVersion = CONVERT_TO_POLY_ORIGINAL;
    USHORT nextChunk = iload->PeekNextChunkID();
    if (nextChunk != m_convertToPolyStartTag)
    {

        // File written by an old version where ConvertToPoly just relied upon the base class load/save functions; pass along
        // to base class
        result = Modifier::Load(iload);
    }
    else
    {

        // New version with with our chunk IDs; read data
        result = iload->OpenChunk();
        while (result == IO_OK)
        {

            // Open the next chunk, and proceed if valid
            result = iload->OpenChunk();
            if (result == IO_OK)
            {

                // Proceed according to the chunk encountered
                ...
                case CHUNK_VERSION:
                    {
                        ...
                        result = iload->Read(&m_convertToPolyVersion, sizeof(int), &nRead);
                        DbgAssert(nRead == sizeof(int));
                    }
                    break;
                default:
                    DbgAssert(false);
    ...
    // The save was successful if we didn't exit the loop above due to an error state
    if (result != IO_ERROR)
    {
        result = IO_OK;
    }
...
RefResult ConvertToPoly::NotifyRefChanged (const Interval& changeInt, RefTargetHandle hTarget,
										   PartID& partID, RefMessage message, BOOL propagate) {
