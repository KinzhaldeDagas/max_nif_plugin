...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_CONVERT_TO_MESH) : _T("Turn to Mesh"); }  
	virtual Class_ID ClassID() { return CONVERT_TO_MESH_ID; }		
	RefTargetHandle Clone(RemapDir& remap);
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_CONVERT_TO_MESH) : _T("Turn to Mesh"); }
...
class ConvertToMeshClassDesc : public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Turn to Mesh"); }
...
static ParamBlockDesc2 turn_param_desc ( turn_params, _T("convertToMeshParams"),
	...
	turn_use_invis, _T("useInvisibleEdges"), TYPE_BOOL, P_RESET_DEFAULT|P_ANIMATABLE, IDS_USE_INVIS,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_USE_INVIS,
		p_nonLocalizedName, _T("Use Invisible Edges"),
		p_end,
...
void ConvertToMesh::ModifyObject (TimeValue t, ModContext &mc, ObjectState *os, INode *node) {
	...
	case 2: // Invert selection
		...
		if (use_ss) {
			...
			if (vsw) {
				BitArray& vertSel = m.VertSel();
				for (int i=0; i<m.numVerts; i++) {
					vsw[i] = 1.0f - vsw[i];
					if (vsw[i] < 1.0f) vertSel.Clear (i);
				}
			}
		} else {
			m.ClearVSelectionWeights ();
...
RefResult ConvertToMesh::NotifyRefChanged (const Interval& changeInt, RefTargetHandle hTarget,
										   PartID& partID, RefMessage message, BOOL propagate) {
	switch (message) {
	case REFMSG_CHANGE:
