...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_CONVERT_TO_PATCH) : _T("Turn to Patch"); }  
	virtual Class_ID ClassID() { return CONVERT_TO_PATCH_ID; }		
	RefTargetHandle Clone(RemapDir& remap);
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_CONVERT_TO_PATCH) : _T("Turn to Patch"); }
...
class ConvertToPatchClassDesc : public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Turn to Patch"); }
...
static ParamBlockDesc2 turn_param_desc ( turn_params, _T("convertToPatchParams"),
	...
	turn_quads, _T("quadsToQuads"), TYPE_BOOL, P_RESET_DEFAULT|P_ANIMATABLE, IDS_PATCH_QUADS,
		...
		p_ui, TYPE_SINGLECHECKBOX, IDC_QUADS,
		p_nonLocalizedName, _T("Quads to Quad Patches"),
		p_end,
...
RefResult ConvertToPatch::NotifyRefChanged (const Interval& changeInt, RefTargetHandle hTarget,
										   PartID& partID, RefMessage message, BOOL propagate) {
	switch (message) {
	case REFMSG_CHANGE:
