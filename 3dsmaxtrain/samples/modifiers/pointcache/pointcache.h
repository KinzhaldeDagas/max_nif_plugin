...
// 4 - Added pc_load_type_network
#define POINTCACHE_MODIFIERVERSION	4

struct PCHeader
{
	TCHAR cacheSignature[12];
	int fileVersion;
	int numPoints;
	float startFrame;
	float sampleRate;
...
public:
	...
	bool	Read( FBXSDK_NAMESPACE::FbxCache* pCache, int iChannel, TimeValue t, unsigned int iNumPoints );
	bool	Write( FBXSDK_NAMESPACE::FbxCache* pCache, int iChannel, TimeValue t, unsigned int iNumPoints );

protected:
	FBXSDK_NAMESPACE::FbxTime TimeValueToKTime( TimeValue t );
...
public:
	...
	enum FileType			{ ePC2=0, ePTS, eMCC };
...
public:
	...
	bool					RecordToFile(HWND hWnd,bool &canceled, bool doLoad = false); //if we canceled that canceled will be true
...
private:
	...
	static FBXSDK_NAMESPACE::FbxManager*	m_pFbxSdkManager;
	static int m_cacheCount;
	// The FbxManager above owns this cache pointer. 
	FBXSDK_NAMESPACE::FbxCache*				m_pFbxCache;

	bool					m_unicode_pc2_file; // true if reading pc2 file where signature was wchar
	bool					InitFbxSdk();				
...
class OLDPointCacheBase : public Modifier {
	...
	public:
		...
		// From Animatable
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_CLASS_NAME_OLD) : _T("Point Cache"); }
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_CLASS_NAME_OLD) : _T("Point Cache");}
