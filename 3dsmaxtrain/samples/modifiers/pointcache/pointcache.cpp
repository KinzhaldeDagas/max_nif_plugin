...
TSTR GetLastErrorString()
{
	...
	TSTR err;
	err.printf(_T("%s"), (LPCTSTR)lpMsgBuf);
...
bool MCBufferIO::Read( FBXSDK_NAMESPACE::FbxCache* pCache, int iChannel, TimeValue t, unsigned int iNumPoints )
{
	if( IsReadBufferNULL() )
		return false;

	bool res;
	auto theTime = TimeValueToKTime(t);
	if( mbReadDouble )
		res = pCache->Read(iChannel, theTime, mpReadBufferDouble, iNumPoints);
	else
		res = pCache->Read(iChannel, theTime, mpReadBufferFloat, iNumPoints);

	return res;
}

bool MCBufferIO::Write( FBXSDK_NAMESPACE::FbxCache* pCache, int iChannel, TimeValue t, unsigned int iNumPoints )
{
	if( IsWriteBufferNULL() )
		return false;

	auto theTime = TimeValueToKTime(t);
	bool res = pCache->Write( iChannel, theTime, mpWriteBufferFloat, iNumPoints );
	return res;
}

FbxTime MCBufferIO::TimeValueToKTime( TimeValue t )
{
	//    Max:
	...
	const double maxTickToFBXTick = double(FBXSDK_TC_SECOND) / 4800.0;

	FbxTime kt;
	FbxLongLong kll = (FbxLongLong)((double)t * maxTickToFBXTick);
	kt.Set(kll);
	return kt;
}
...
FbxManager* PointCache::m_pFbxSdkManager = NULL;
int PointCache::m_cacheCount = 0;
...
bool PointCache::InitFbxSdk()
{
	...
	{
		m_pFbxSdkManager = FBXSDK_NAMESPACE::FbxManager::Create();
	}
	...
	if( m_pFbxCache == NULL )
		m_pFbxCache = FBXSDK_NAMESPACE::FbxCache::Create(m_pFbxSdkManager, "Point Cache");
...
void PointCache::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_CLASS_NAME) : _T("Point Cache");
}
...
TSTR PointCache::SubAnimName(int i, bool localized)
{
...
public:
	...
	virtual MaxSDK::AssetManagement::AssetUser GetAsset() const override;
	virtual bool SetAsset(const MaxSDK::AssetManagement::AssetUser& aNewAsset) override;
	virtual bool IsInputAsset() const override;

	// asset client information
	virtual MaxSDK::AssetManagement::AssetType GetAssetType() const override;
	virtual const TCHAR* GetAssetDesc() const override;
	virtual const TCHAR* GetAssetClientDesc() const override;
	virtual bool IsAssetPathWritable() const override;
...
////////////////////////
class PointCacheSubAssetAccessor : public IAssetAccessor {
public:

	PointCacheSubAssetAccessor(PointCache* aPointCache, int which);
...
PointCacheSubAssetAccessor::PointCacheSubAssetAccessor(PointCache* aPointCache, int which) :
	mPointCache(aPointCache)
{
	int nCacheFiles = mPointCache->GetNumMCCacheFiles();
	if (which >= 0 && which < nCacheFiles) {
		TSTR fileName;
		if (mPointCache->GetMCCacheFilename(which, fileName)) {
			mAsset = IAssetManager::GetInstance()->GetAsset(fileName, GetAssetType());
		}
	}
}

MaxSDK::AssetManagement::AssetUser PointCacheSubAssetAccessor::GetAsset() const {
	return mAsset;
}

bool PointCacheSubAssetAccessor::SetAsset(const MaxSDK::AssetManagement::AssetUser& aNewAsset) {
	return false;
}

bool PointCacheSubAssetAccessor::IsInputAsset() const {
	return true;
}

MaxSDK::AssetManagement::AssetType PointCacheSubAssetAccessor::GetAssetType() const {
	return MaxSDK::AssetManagement::kAnimationAsset;
}

const TCHAR* PointCacheSubAssetAccessor::GetAssetDesc() const {
	return NULL;
}

const TCHAR* PointCacheSubAssetAccessor::GetAssetClientDesc() const {
	return NULL;
}

bool PointCacheSubAssetAccessor::IsAssetPathWritable() const {
	return false;
}
////////////////////////
...
void PointCache::EnumAuxFiles(AssetEnumCallback& nameEnum, DWORD flags)
{
	...
	if (flags & FILE_ENUM_ACCESSOR_INTERFACE) {
		if ((m_fileType == eMCC) && (GetNumMCCacheFiles() > 0)) {
			...
			if (accessorCacheXML.GetAsset().GetId() != kInvalidId) {
				...
				// Declare the first .mc file. The GetAssetClientDesc() method returns the name
				// of the last .mc file, so user sees in ATS the range of sub files
				// If the first .mc file is missing, the user will see that from this declaration.
				callback->DeclareAsset(accessorCacheMC);
				if (!(flags & FILE_ENUM_SKIP_SUB_FILES)) {
					// only report MISSING files - we don't want to list every single
					// file since we don't asset track them.  However, we want to list missing 
					// files so that users can see they are missing at a glance in the ATS
					TSTR filePath;
					int nCacheFiles = GetNumMCCacheFiles();
					for (int i = 0; i < nCacheFiles; i++) {
						PointCacheSubAssetAccessor accessorCacheMC(this, i);
						AssetUser theAsset = accessorCacheMC.GetAsset();
						if (!theAsset.GetFullFilePath(filePath))
						{
							// Already reported first sub file above, so skip it here
							if (i != 0)
								callback->DeclareAsset(accessorCacheMC);
							if (flags & FILE_ENUM_1STSUB_MISSING)
								break;
						}
					}
				}
		...
		IPathConfigMgr* pathConfig = IPathConfigMgr::GetPathConfigMgr();
		AssetUser theAsset = m_pblock->GetAssetUser(pc_cache_file);
		pathConfig->RecordInputAsset(theAsset,nameEnum,flags);
		if (!(flags & FILE_ENUM_SKIP_SUB_FILES)) {
			if (m_fileType == eMCC) {
				TSTR filePath;
				int nCacheFiles = GetNumMCCacheFiles();
				for (int i = 0; i < nCacheFiles; i++) {
					PointCacheSubAssetAccessor accessorCacheMC(this, i);
					theAsset = accessorCacheMC.GetAsset();
					if (kInvalidId != theAsset.GetId()) {
						if ((flags & FILE_ENUM_MISSING_ONLY) && (flags & FILE_ENUM_1STSUB_MISSING)) {
							// for reporting only first missing subfile
							if (!theAsset.GetFullFilePath(filePath)) {
								nameEnum.RecordAsset(theAsset);
								break;
							}
						}
						else {
							// for normal reporting, this is fine
							pathConfig->RecordInputAsset(theAsset, nameEnum, flags);
						}
					}
				}
...
void PointCache::ModifyObject(TimeValue t, ModContext& mc, ObjectState* os, INode* node)
{
	...
	int loadType = (GetCOREInterface()->IsNetworkRenderServer()) ? m_pblock->GetInt(pc_load_type_network, 0)
																 :
															m_pblock->GetInt(pc_load_type, 0);

	if (m_recordMode)
	{
		DEBUGPRINT(_T("PointCache::ModifyObject: In record mode at time %d\n"), t);
		...
		if ((IsCacheFileOpen()) && (m_recordTime == t))
		{
			if (m_numPoints == m_cacheNumPoints)
			{
				for (int i = 0; i < m_cacheNumPoints; i++)
				{
					...
					else if( m_fileType == eMCC )
					{
						auto objPt = os->obj->GetPoint(i);
						m_mcBufferIO.SetWriteBuffer(i, &objPt);
					}
				}

				DWORD numWrite;
				DEBUGPRINT(_T("PointCache::ModifyObject: Writing File: m_hCacheFile: %d\n"), m_hCacheFile);
				BOOL res(false);
				if( m_fileType == ePC2 ) {
				...
				if (!res)
				{
					DEBUGPRINT(_T("Failed: %s\n"), GetLastErrorString().data() );
				}
	...
	else if (!m_tempDisable && m_doLoad)
	{
		...
		if (IsValidFName(asset.GetFileName()))
		{
			...
			if(m_fileType==ePTS)
			{
				...
				if( bApplySample && m_sampleA && m_firstSample ) {
					...
					if( m_numPoints == m_cacheNumPoints ) {
						...
						DEBUGPRINT(_T("t=%d, tSample=%d findex=%f\n"), t, tSample, fIndex);
			...
			if (
				...
				DEBUGPRINT(_T("t: %d, indexA: %d, indexB: %d, perc: %f\n"), t, indexA, indexB, perc);
...
public:
	...
	void proc(ILoad *iload)
	{
		...
		{
			m_pc->m_pblock->SetValue(pc_load_type_network, 0, load_persample);
		}
...
void PointCache::UpdateCacheInfoStatus( HWND hWnd, const TCHAR* szForceMsg )
{
	...
	if( !bShowData ) {
		...
		str.printf(_T("%d/%d"), (int)m_cacheStartFrame, (int)cacheEndFrame);
...
bool PointCache::IsMultipleChannelCache( const TCHAR* fname )
{
	...
	FbxCache* pTempFbxCache = NULL;
	pTempFbxCache = FbxCache::Create(m_pFbxSdkManager, "Point Cache Temp");

	if( !pTempFbxCache )
		return false;

	// convert to UTF8
	pTempFbxCache->SetCacheFileName( "", TSTR(fname).ToUTF8() );
	pTempFbxCache->SetCacheFileFormat( FbxCache::eMayaCache );
...
bool PointCache::ReadHeader()
{
	if( m_fileType == eMCC ) {
		...
		// Get time values
		FbxTime ktTimePerFrame = m_pFbxCache->GetCacheTimePerFrame();
		FbxTime ktStartTime, ktEndTime, ktSampleRate;
		...
		if( !m_pFbxCache->GetChannelSamplingRate(m_cacheChannel, ktSampleRate) ) {
			...
			FbxLongLong kLongSample = (FbxLongLong)((double)(ktEndTime.Get() - ktStartTime.Get()) / (double)(m_cacheNumSamples-1));
		...
		// Get File Count type
		FbxCache::EMCFileCount pFileCount;
		m_pFbxCache->GetCacheType(pFileCount);
		m_fileCount = pFileCount == FbxCache::eMCOneFile ? eOneFile : eOneFilePerFrame;

		// Evaluate data types
		FbxCache::EMCDataType dataType = FbxCache::eDoubleVectorArray;
		if( !m_pFbxCache->GetChannelDataType(m_cacheChannel, dataType) )
			return false;

		if( dataType == FbxCache::eDoubleVectorArray )
			m_channelDataType = eDoubleVectorArray;
		else if( dataType == FbxCache::eFloatVectorArray )
		...
		m_unicode_pc2_file = false;
		if (strcmp(m_signature,m_cacheSignature) != 0) 
		{
			// handle case where pc2 file was created by simcity when m_signature was declared as TCHAR
			SetFilePointer(m_hCacheFile, 0, NULL, FILE_BEGIN);
			TCHAR UnicodeSignature[12];
			if (!ReadFile(m_hCacheFile, &UnicodeSignature, sizeof(UnicodeSignature), &numRead, NULL) || numRead == 0) return false;
			if (_tcscmp(_T("POINTCACHE2"),UnicodeSignature) != 0) return false;
			m_unicode_pc2_file = true;
		}
...
{
	DEBUGPRINT(_T("PointCache::OpenCacheForRead: %s\n"), file.GetFileName().data());

	HANDLE hFile = NULL;

	const TSTR& fileName = file.GetFileName();
	if (IsValidFName(fileName))
	{
		...
		if (file.GetFullFilePath(path)) 
		{
			...
			if( m_fileType == eMCC ) {
				...
				m_pFbxCache->SetCacheFileName( "", path.GetString().ToUTF8());
				m_pFbxCache->SetCacheFileFormat( FbxCache::eMayaCache );
		...
		{
			hFile = NULL;
			DEBUGPRINT(_T("\tfail: '%s' - %s\n"), file.GetFileName().data() , GetLastErrorString().data() );
		} else {
			DEBUGPRINT(_T("\tsuccess: %d - '%s'\n"), hFile, file.GetFileName().data() );
			
		}
	} else {
		DEBUGPRINT(_T("\tfail: Invalid fname: '%s'\n"), file.GetFileName().data() );
	}
...
{
	DEBUGPRINT(_T("PointCache::OpenCacheForRecord: %s\n"), file.GetFileName().data());
	...
	if(m_fileType==ePTS)
	{
		...
		else
		{
			...
			MaxSDK::MaxMessageBox(iCore->GetMAXHWnd(), error, errorTitle, MB_OK | MB_ICONERROR);
			SaveFilePrompt(hWnd);
	...
	if( m_fileType == ePC2 ) {
		...
		MaxSDK::Util::Path storageNamePath;
		if (asset.GetFullFilePath(storageNamePath))
		{
	...
	if (hFile == INVALID_HANDLE_VALUE)
	{
		...
		DEBUGPRINT(_T("\tfail: but now searching along external path'%s' - %s\n"),path.GetCStr(), GetLastErrorString().data() );
		MaxSDK::Util::Path path(file.GetFileName());		
...
bool PointCache::OpenCacheForRecordMC(const TCHAR* fname)
{
	...
	// convert to UTF8
	m_pFbxCache->SetCacheFileName( "", TSTR(fname).ToUTF8() );
	m_pFbxCache->SetCacheFileFormat( FbxCache::eMayaCache );
	...
	MaxString m_strChannel;
	strChannel.ToMaxString(m_strChannel);

	bool bRes = m_pFbxCache->OpenFileForWrite( 
								(m_fileCount==eOneFile)? FbxCache::eMCOneFile : FbxCache::eMCOneFilePerFrame,
								GetFrameRate(), (const char*)m_strChannel.ToUTF8(), FbxCache::eMCC );
...
bool PointCache::CloseFile(HANDLE hFile)
{
	...
	DEBUGPRINT(_T("PointCache::CloseFile: %d\n"), hFile);
	if(hFile)
	{
		if (!CloseHandle(hFile))
		{
			DEBUGPRINT(_T("\tfail: %d - %s\n"), hFile, GetLastErrorString().data() );
			return false;
		} else {
			DEBUGPRINT(_T("\tsuccess: %d\n"), hFile);
		}
...
bool PointCache::RecordToFile(HWND hWnd,bool &canceled,bool doLoad)
{
	canceled = false; //assume we aren't.
	DEBUGPRINT(_T("PointCache::RecordToFile\n"));
	...
	if (IsValidFName(asset.GetFileName()))
	{
		...
		if (doLoad)
			dep.Nodes[0]->EvalWorldState(recordBegin);
...
{
	m_title = GetString(IDS_CLASS_NAME);
	m_initDir = _T("");
	bmmHistoryList::LoadDefaults();
	
	if (bmmHistoryList::DefaultPath()[0]) 
		_tcscpy_s(PointCache::m_InitialDir, bmmHistoryList::DefaultPath());
	else _tcscpy_s(PointCache::m_InitialDir,GetCOREInterface()->GetDir(APP_EXPORT_DIR).data());
...
INT_PTR WINAPI PC2HookProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	static int paddingToBorder = MaxSDK::UIScaled(12);
	static int offset1 = MaxSDK::UIScaled(14);
	static int offset2 = MaxSDK::UIScaled(24);
	static int offset3 = MaxSDK::UIScaled(28);
	if (message == WM_INITDIALOG)
	{
		...
		if ( GetDlgItem(hDlg, IDC_PLUS_BUTTON) ) {
			...
			wp.rcNormalPosition.left += (offset1 - paddingToBorder);
			wp.rcNormalPosition.right += (offset1 - paddingToBorder);
	...
	else if(message==WM_NOTIFY)
	{
		...
		if (code == CDN_INITDONE && custom)
		{
			...
			//Align the controls
			WINDOWPLACEMENT historyLabelWP;
			WINDOWPLACEMENT fileListWP;
			WINDOWPLACEMENT historyComboWP;
			historyLabelWP.length = sizeof(WINDOWPLACEMENT);
			fileListWP.length = sizeof(WINDOWPLACEMENT);
			historyComboWP.length = sizeof(WINDOWPLACEMENT);

			//Align the history label to the save in combo box
			GetWindowPlacement(GetDlgItem(GetParent(hDlg), lst1), &fileListWP);
			GetWindowPlacement(GetDlgItem(hDlg, IDC_GOTO_STATIC), &historyLabelWP);
			int width = historyLabelWP.rcNormalPosition.right - historyLabelWP.rcNormalPosition.left;
			historyLabelWP.rcNormalPosition.right = fileListWP.rcNormalPosition.left - MaxSDK::UIScaled(8);
			historyLabelWP.rcNormalPosition.left = historyLabelWP.rcNormalPosition.right - width;
			SetWindowPlacement(GetDlgItem(hDlg, IDC_GOTO_STATIC), &historyLabelWP);

			//Align the history combo box to the save in one
			GetWindowPlacement(GetDlgItem(hDlg, IDC_HISTORY), &historyComboWP);
			GetWindowPlacement(GetDlgItem(hDlg, lst1), &fileListWP);
			historyComboWP.rcNormalPosition.left = fileListWP.rcNormalPosition.left;
			SetWindowPlacement(GetDlgItem(hDlg, IDC_HISTORY), &historyComboWP);
		...
		if (code == CDN_FILEOK)
		{
			...
			bool saving = custom && custom->save;
			TSTR messageBoxTitle = MaxSDK::GetResourceStringAsMSTR(saving ? IDS_SAVEFILE : IDS_OPENFILE);
			if (!MaxSDK::ValidateFileNameLength(hDlg, lpofn, messageBoxTitle))
				return TRUE;

			if (custom)
			{
				TCHAR buf[MAX_PATH]{ _T('\0') };
				CommDlg_OpenSave_GetFolderPath(GetParent(hDlg), buf, MAX_PATH);
...
bool PointCache::SaveFilePrompt(HWND hWnd)
{
	...
	ofn.lStructSize = sizeof(OPENFILENAME); // has OFN_ENABLEHOOK
	...
	if (nodes.Count() != 0)
	{
		...
		for (int ii=0; ii<len; ++ii)
		{
			...
			if ((c<'0' || c>'9') &&
				(c<'A' || c>'Z') &&
				(c<'a' || c>'z')
			) str.dataForWrite()[ii] = _T('_');
		}

		str += GetString(IDS_MAYA_CACHE_EXT);

		_tcsncpy(fname, str.data(), str.length());
	}
	...
	ofn.Flags           = OFN_EXPLORER|OFN_HIDEREADONLY|OFN_FILEMUSTEXIST|OFN_ENABLETEMPLATE |OFN_ENABLEHOOK|OFN_PATHMUSTEXIST| OFN_ENABLESIZING;
	...
	if (GetOpenFileName(&ofn))
	{
		...
		if( IsMultipleChannelCache(fname) ) {
			...
			if( GetCOREInterface()->GetQuietMode() ) 	{
				...
				MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), GetString(IDS_MULTI_CHANNEL_WARNING), strTitle.data(), MB_OK | MB_ICONWARNING);
			}
		}
...
{
	DEBUGPRINT(_T("PointCache::Reload\n"));
...
bool PointCache::DeleteCacheFile(HWND hWnd)
{
	...
	if (iCore)
	{
		...
		if (IsValidFName(asset.GetFileName()))
		{
			...
			if (IDYES == MaxSDK::MaxMessageBox(maxWnd, buf1, buf2, MB_YESNO | MB_ICONEXCLAMATION))
			{
				CloseCacheFile(true);
				return (_tremove(asset.GetFileName()) == 0) ? TRUE : FALSE;
			}
		}
	}
	return FALSE;
...
{
	//TODO: breaks if header changes
	int offset = sizeof(m_cacheSignature);
	if (m_unicode_pc2_file)
		offset *= sizeof(wchar_t);
	offset += (sizeof(int)*2) + (sizeof(float)*2) + (sizeof(int));
