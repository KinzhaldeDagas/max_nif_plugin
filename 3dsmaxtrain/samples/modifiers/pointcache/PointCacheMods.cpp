...
{
	bool canceled;
	RecordToFile(m_hWnd,canceled, true);
	UpdateUI();
}
...
void PointCacheParamsMapDlgProc::Update(TimeValue t)
{
	...
	BOOL bIsMC  = (m->m_fileType == PointCache::eMCC);
	...
	EnableWindow(GetDlgItem(thishWnd,IDC_LOAD_TYPE_NETWORK), bIsPC2);
			...
			switch (LOWORD(wParam))
			{
				...
				case IDC_BROWSE:
				{
					...
					if (m->IsValidFName(str))
					{
						...
						HINSTANCE res = ShellExecute( hWnd, _T("open"), path.data(), NULL, NULL, SW_SHOWNORMAL );
					}
				...
				case IDC_RECORD:
				{
					...
					if (!m->RecordToFile(hWnd,canceled)&&canceled==false) //we failed but not because we canceled.
					{
						...
						TSTR msgBoxText = GetString(IDS_ERROR_RECORDING);
				...
				{
					m->NotifyDependents(FOREVER, PART_GEOM, REFMSG_CHANGE);
					m->UpdateUI();
					break;
				}
...
#define PBLOCK_DEFINITION																								\
	...
	pc_playback_frame,	_T("playbackFrame"), TYPE_FLOAT, 	P_ANIMATABLE|P_RESET_DEFAULT, 	IDS_PLAYBACK_FRAME,			\
		...
		p_nonLocalizedName, _T("Playback Frame"), \
		p_end,																											\
	...
	pc_apply_to_whole_object, _T("applyToWholeObject"),	TYPE_BOOL,	P_RESET_DEFAULT,	IDS_APPLY_TO_WHOLE_OBJECT,		\
		p_default,		TRUE,																							\
		p_ui,			TYPE_SINGLECHECKBOX,	IDC_APPLY_TO_WHOLE_OBJECT,												\
		p_end,																											\
	pc_load_type_network, _T("loadTypeNetwork"), TYPE_INT, P_RESET_DEFAULT, IDS_LOAD_TYPE_NETWORK,						\
		p_default, 		load_persample,																					\
		p_range, 		load_persample, load_preload,																	\
		p_accessor,		&pointCachePBAccessor,																			\
		p_ui,			TYPE_INTLISTBOX, IDC_LOAD_TYPE_NETWORK, 2,	IDS_LOADTYPE_PERSAMPLE,								\
																	IDS_LOADTYPE_PRELOAD,								\
...
BaseInterface* PointCacheOSM::GetInterface(Interface_ID id)
{
	if (id == POINTCACHE_INTERFACE)
		return static_cast<IPointCache*>(this);
	else
		return PointCache::GetInterface(id);
}

const TCHAR* PointCacheOSM::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_CLASS_NAME) : _T("Point Cache");
}
...
const TCHAR* PointCacheWSM::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_POINTCACHE_NAMEBINDING) : _T("Point Cache Binding");
}

BaseInterface* PointCacheWSM::GetInterface(Interface_ID id)
{
	if (id == POINTCACHEWSM_INTERFACE)
		return static_cast<IPointCacheWSM*>(this);
	else
		return PointCache::GetInterface(id);
}
