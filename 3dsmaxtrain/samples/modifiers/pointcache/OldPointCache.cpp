...
//published functions		 
	...
	BaseInterface* GetInterface(Interface_ID id) 
		{ 
			...
			return OLDPointCacheBase::GetInterface(id);
		}
...
public:
	...
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_POINTCACHE_NAMEBINDING) : _T("Point Cache Binding"); }
...
public:
	...
	// From Animatable
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_PARTICLECACHE_NAMEBINDING) : _T("Particle Cache Binding"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Point Cache"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Particle Cache"); }
	SClass_ID		SuperClassID() {return WSM_CLASS_ID;}
	Class_ID		ClassID() {return OLDPARTICLECACHE_CLASS_ID;}
	const TCHAR* 	Category() {return GetString(IDS_CATEGORY);}
...
static ParamBlockDesc2 pointcache_param_blk ( pointcache_params, _T("params"),  0, &PointCacheDesc, 
											 ...
											 p_ui, 			TYPE_SINGLECHECKBOX, 	IDC_DISABLE_STACK_CHECK, 
											 ...
											 */
											 ...
											 p_ui, 			TYPE_SINGLECHECKBOX, 	IDC_RELATIVE_CHECK, 
...
static ParamBlockDesc2 particlecache_param_blk ( pointcache_params, _T("params"),  0, &ParticleCacheDesc, 
												...
												p_ui, 			TYPE_SINGLECHECKBOX, 	IDC_ENABLE_CHECK, 
												p_accessor,		&pointCache_accessor,
												p_end, 
												*/
												pb_cache_file, 	_T("cache_file"), 	TYPE_FILENAME, 		P_RESET_DEFAULT,				IDS_CACHEFILE,
...
OLDPointCacheBase::OLDPointCacheBase()
{
	...
	extension.printf(_T("pts"));
	extensionWildCard.printf(_T("*.pts"));
	loadName.printf(_T("%s"),GetString(IDS_OLD_CACHE_FILES));
	hWnd=NULL;
	pblock = NULL; 
	cFrame = NULL;
	nFrame = NULL;
...
FILE* OLDPointCacheBase::OpenFileForWrite( const MaxSDK::AssetManagement::AssetUser& file)
{
	...
	MaxSDK::Util::Path storageNamePath(path);
...
void OLDPointCacheBase::RecordToFile(HWND hWnd)
{
	if (!IsEnabled()) {
		TSTR errorMsg;
		errorMsg.printf(_T("%s"), GetString(IDS_PW_DISABLED_ERROR));
		MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), errorMsg, GetString(IDS_PW_DISABLED_CAPTION), MB_OK | MB_ICONERROR);
		return;
	}
	...
	if (asset.GetId()!=kInvalidId)
	{
		...
		{
			TSTR error;
			error.printf(_T("%s"),GetString(IDS_PW_OPENERROR));
			MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), error, NULL, MB_OK | MB_ICONERROR);
			return;
		}
		...
		//write the header
		//write point count
		//this is now the version I did not put a version number in the file (by mistake) now I want to change it
		...
		if (outFile)
		{
			...
			if (e > s)
			{
				for (int i = s; i <= e; i+=tps)
				{
					...
					int percent = (i*100)/e;
					name.printf(_T("Frame %d of %d"),i/tps,(int)e/tps);
					SetWindowText(GetDlgItem(hWnd,IDC_PERCENT),name);
...
{
public:
	const OLDPointCacheBase* mod = nullptr;
	CacheDeformer(const OLDPointCacheBase* m)
	{
		mod = m;
	}
		...
		if (mod->fileVersion == 0)
		{
			if (i < mod->cacheNumberPoints)
			{
				if (mod->relativeOffset)
					p += ((mod->cFrame[i] + (mod->nFrame[i] - mod->cFrame[i]) * mod->per) - mod->firstFrame[i]) *
							mod->strength;
				else
					p = mod->cFrame[i] + (mod->nFrame[i] - mod->cFrame[i]) * mod->per;
			}
		}
		else
		{
			if (mod->pointCountList[mod->cTime] == mod->pointCountList[mod->nTime])
			{
				if ((i < mod->pointCountList[mod->cTime]) && (i < mod->pointCountList[mod->nTime]))
				{
					if (mod->relativeOffset)
					{
						...
						p_end = mod->cFrame[i] + (mod->nFrame[i]-mod->cFrame[i]) * mod->per;
						p += (p_end-start)*mod->strength;
						*/
					}
					else
						p = mod->cFrame[i] + (mod->nFrame[i] - mod->cFrame[i]) * mod->per;
...
ParticleCache::ParticleCache()
{
	...
	outFile  = NULL;
	recordMode = FALSE;
	extension.printf(_T("par"));
	extensionWildCard.printf(_T("*.par"));
	loadName.printf(_T("%s"),GetString(IDS_PW_CACHEFILESPARTS));
