...
#define RAYMESHGRIDINTERSECT_V2_INTERFACE Interface_ID(0xDE17A11A, 0x8A41F2F3)
...
class IRayMeshGridIntersect_InterfaceV1: public FPMixinInterface 
{
	public:
		...
		BEGIN_FUNCTION_MAP
			...
			FN_1(grid_gethitbary, TYPE_POINT3_BV, fnGetHitBary, TYPE_INT);
			FN_1(grid_gethitnorm, TYPE_POINT3_BV, fnGetHitNorm, TYPE_INT);
		...
		virtual void	fnBuildGrid(TimeValue timeOverride)=0;   //this builds your grid data
		
		virtual void	fnAddNode(INode *node)=0;
		virtual	int		fnIntersectBox(const Point3& min, const Point3& max)=0;
		virtual	int		fnIntersectSphere(const Point3& p, float radius)=0;
		virtual	int		fnIntersectRay(const Point3& p, const Point3& dir, BOOL doubleSided)=0;
		virtual	int		fnIntersectSegment(const Point3& p1, const Point3& p2, BOOL doubleSided)=0;
		...
		virtual	int		fnIntersectSegmentDebug(const Point3& p1, const Point3& p2, BOOL doubleSided,int whichGrid)=0;

		virtual int		fnGetFarthestHit() = 0; 

		virtual void	fnAddMesh(Mesh *msh, const Matrix3& tm)=0;

		virtual int		fnClosestFace(const Point3& p)=0;
...
//for Max Ver 7
class IRayMeshGridIntersect_InterfaceV2 : public FPMixinInterface
{

		BEGIN_FUNCTION_MAP

			FN_2(grid_closestfacethreshold, TYPE_INT, fnClosestFaceThreshold, TYPE_POINT3, TYPE_FLOAT);
			
		END_FUNCTION_MAP

		FPInterfaceDesc* GetDesc();    // <-- must implement 
		virtual int		fnClosestFaceThreshold(const Point3& p, float threshold)=0;
