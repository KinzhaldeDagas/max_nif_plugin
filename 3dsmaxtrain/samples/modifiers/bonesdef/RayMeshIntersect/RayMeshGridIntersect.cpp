...
ClassDesc2* GetRayMeshGridIntersectDesc()
{
	return &rayMeshGridIntersectDesc;
}
...
static FPInterfaceDesc raymeshgridintersect_interface( RAYMESHGRIDINTERSECT_V1_INTERFACE, _T("rayMeshGridIntersectOps"), 0, 
	&rayMeshGridIntersectDesc, FP_MIXIN,
...
static FPInterfaceDesc raymeshgridintersectV2_interface(RAYMESHGRIDINTERSECT_V2_INTERFACE, _T("rayMeshGridIntersectOpsV2"), 0, 
	&rayMeshGridIntersectDesc, FP_MIXIN,
...
static FPInterfaceDesc raymeshgridintersectV3_interface(RAYMESHGRIDINTERSECT_V3_INTERFACE, _T("rayMeshGridIntersectOpsV3"), 0, 
	&rayMeshGridIntersectDesc, FP_MIXIN,
...
void RayMeshGridIntersect::fnPrintStats()
{
	ScriptPrint(_T("Number of faces %0.0f  average number of faces searched %0.0f\n"), gridData.numFaces,
			gridData.summedFaces / gridData.tally);
}
...
void Grid::BuildGrid(TimeValue t)
{
	...
	if (offset < FLT_MIN) // just a check in case we have a mesh all collapsed to 1 point
		offset = 0.001f;
	...
	for (int i = 0; i < ct; i++)
	{
		...
		//		if (potentialXHitList[i])
		{
			...
			if (bry.x < 0.0f || bry.y < 0.0f || bry.z < -0.0f)
				continue; // DS 3/8/97 this test is sufficient
...
int Grid::ClosestFace(const Point3& p)
{
	...
	while (!done)
	{
		...
		{
			if ((!processedFacesHold[i]) &&
					(cellsToCheckXHoldPtr[i] && cellsToCheckYHoldPtr[i] && cellsToCheckZHoldPtr[i]))
			{
