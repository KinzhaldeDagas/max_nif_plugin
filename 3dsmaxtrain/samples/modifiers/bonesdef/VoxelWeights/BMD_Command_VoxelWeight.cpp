...
void BMD_Command_VoxelWeight::Execute(BonesDefMod * mod, BoneModData *bmd)
{
	...
	if ( (iret == false) && (mType == 1) )
	{
		...
		//TSTR msgBoxText = GetString(IDS_HEATMAP_ERROR_MSG);
		//if (MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), msgBoxText, GetString(IDS_HEATMAP_ERROR_MSG), MB_OKCANCEL) == IDOK)
		{
...
//	boneRemap.SetCount(mod->BoneData.Count());
	...
	for (int i = 0; i < mod->BoneData.Count(); i++)
	{
		...
		if ( node != nullptr)
		{
			...
			if (mod->BoneData[i].flags & BONE_SPLINE_FLAG)
			{
				...
				//just a new name to make unique
	...
	//frustrating OGL artifact, the voxelizer uses OGL so we must have a dummy window to host the context
