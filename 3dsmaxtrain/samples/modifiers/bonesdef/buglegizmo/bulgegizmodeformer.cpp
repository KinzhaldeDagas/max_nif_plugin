...
Point3 GizmoBulgeClass::InterpSpline2(float u, const Point3* knot) const
{
	Spline3D spl;
	Point3 knotsPts[14];
...
class GizmoBulgeClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Bulge Angle Deformer"); }
...
/*	skin_gizmoparam_bulge_graph,  _T("graph"),	TYPE_REFTARG, 	P_SUBANIM, 	IDS_PW_GIZMOPARAM_GRAPH, 
	...
	skin_gizmoparam_bulge_use_graph,  _T("use_graph"),	TYPE_BOOL,	0, 	IDS_PW_GIZMOPARAM_USEGRAPH, 
		p_default, 		FALSE,
		p_ui, 			TYPE_SINGLECHECKBOX, IDC_GRAPH_ENABLE,
		p_accessor,		&gizmoJoint_accessor,
		p_end, 
	*/
...
BOOL GizmoBulgeClass::IsVolumeBased() const
{
	return mIsVolumeBasedCache;
}

BOOL GizmoBulgeClass::IsEnabled() const
{
...
BOOL GizmoBulgeClass::IsInVolume(Point3 p, Matrix3 tm) const
{
...
if ((pblock_gizmo_data) && (bonesMod))
	...
	TSTR name;
	name.printf(_T("%s - %s"),GetString(IDS_PW_GIZMOBULGE),bonesMod->GetBoneFlat(parent)->GetName());
...
const TCHAR *GizmoBulgeClass::GetName()
{
	const TCHAR *name;
	pblock_gizmo_data->GetValue(skin_gizmoparam_name,0,name,FOREVER);
	return name;
}
void GizmoBulgeClass::SetName(const TCHAR *name)
{
...
void GizmoBulgeClass::PreDeformSetup(TimeValue t)
{
	...
	if (fabs(dot) == 1.0f)
		...
		currentAngle = fabs(acos(dot) * 180.f / PI);
	...
	// Cache things for the const functions (thread-safe functions).
	...
	twistAngle = 0.f;
	pblock_gizmo_data->GetValue(skin_gizmoparam_bulge_twist, 0, twistAngle, FOREVER);
	mTwistAngleCache = twistAngle;
}



Point3 GizmoBulgeClass::DeformPoint(TimeValue t, int index, Point3 initialP, Point3 p, Matrix3 tm) const
{
	if (!val)
		return p;
	...
	Matrix3 twist(1);
	float twistAngle = mTwistAngleCache;
	twistAngle = twistAngle * PI / 180.f;
...
Point3 GizmoBulgeClass::Map(int ii, Point3 p) const
{
