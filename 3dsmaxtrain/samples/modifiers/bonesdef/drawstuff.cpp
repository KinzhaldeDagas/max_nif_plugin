	...
	int flagst, ModContext *mc)
{
	...
	if ((tsel>=0) && (ip && ip->GetSubObjectLevel() == 1) )
	{
		...
		BOOL envOnTop;
		BOOL crossOnTop;
		pblock_display->GetValue(skin_display_envelopesalwaysontop,0,envOnTop);
		pblock_display->GetValue(skin_display_crosssectionsalwaysontop,0,crossOnTop);
		...
		int wt_markerType;
		this->pblock_weighttable->GetValue(skin_wt_showmarker,0,showWT);
		if ( (showWT) && (weightTableWindow.hWnd) )
		{
			this->pblock_weighttable->GetValue(skin_wt_markertype,0,wt_markerType);
			this->pblock_weighttable->GetValue(skin_wt_markercolor,0,wt_markerColor);
