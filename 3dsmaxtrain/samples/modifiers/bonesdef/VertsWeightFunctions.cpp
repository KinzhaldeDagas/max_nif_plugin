...
float BonesDefMod::RetrieveNormalizedWeight(const BoneModData *bmd, int vid, int bid) const
{
...
void BonesDefMod::RemoveZeroWeights()
{
	...
	float limit = 0.0f;
	pblock_advance->GetValue(skin_advance_clearzerolimit,0,limit);
	ADPForSamples::ADPSkinEventLogger::LogSkinADPEvent(
			ADPForSamples::ADPSkinEventLogger::SkinWaypointType::SKIN_FEATURE_USAGE,
			ADPForSamples::ADPSkinEventLogger::SkinFeatureUsage_EventType::SKIN_REMOVE_ZERO_WEIGHTS, limit);
