...
bool BoneVertexDataManager::AppendItem(int id, int index, const VertexInfluenceListClass& w)
{
	if (index >= maxSize) {
		return false;
	}
...
const VertexInfluenceListClass& BoneVertexDataManager::GetItemAtIndex(int id, int index) const
{
	const auto start = GetDataStart(id);
	return allInfluenceList[start + index];
}
...
void BoneVertexDataManager::UpdateDataAlignment(int newMaxSize)
{
	...
	{
		const int count = pVertexData->Count();
		const int factor = newMaxSize/maxSize;
		int updatedMaxSize = (factor + 1) * maxSize;
		allInfluenceList.SetCount(count * updatedMaxSize);
		for (auto i = (count - 1); i > 0; --i)
		{
			for (auto j = 0; j < maxSize; ++j)
			{
				allInfluenceList[i * updatedMaxSize + j] = allInfluenceList[i * maxSize + j];
			}
		}
...
bool BoneVertexDataManager::IsValidIndex(int id, int index) const
{
	const auto start = GetDataStart(id);
	return (index >= 0) && (index < maxSize) && ((start + index) < allInfluenceList.Count());
}

int BoneVertexDataManager::ReplaceData(int id, const Tab<VertexInfluenceListClass>& w)
{
	UpdateDataAlignment(w.Count());
	const auto start = GetDataStart(id);
	if (start >= 0)
	{
...
int BoneVertexDataManager::PastWeights(int id, const Tab<VertexInfluenceListClass>& w)
{
...
VertexListClass::VertexListClass(const VertexListClass& from)
{
	flags = from.flags;
...
VertexListClass& VertexListClass::operator=(const VertexListClass & rhs)
{
	if (this != &rhs)
	{
		flags = rhs.flags;
...
const VertexInfluenceListClass& VertexListClass::GetItemAtIndex(int index) const
{
	return mgrVertexData->GetItemAtIndex(dataId, index);
}
...
int VertexListClass::WeightCount() const
{
...
int VertexListClass::GetBoneIndex(int index) const
{
...
float VertexListClass::GetNormalizedWeight(int index) const
{
	...
	DbgAssert(index >= 0 && index < totalItems);
	if (index >= 0 && index < totalItems)
		return GetItemAtIndex(index).normalizedInfluences;
	return 0.0f;
}

float VertexListClass::GetWeight(int index) const
{
	if (IsDQOverride())
	{
...
float VertexListClass::GetCurveU(int index) const
{
	if (index >= totalItems)
		return 0.0f;
...
int VertexListClass::GetCurveID(int index) const
{
	if (index >= totalItems)
		return 0;
...
int VertexListClass::GetSegID(int index) const
{
	if (index >= totalItems)
		return 0;
...
Point3 VertexListClass::GetOPoint(int index) const
{
	if (index >= totalItems)
...
Point3 VertexListClass::GetTangent(int index) const
{
	if (index >= totalItems)
		return Point3(0.0f, 0.0f, 0.0f);
...
void VertexListClass::AppendWeight(const VertexInfluenceListClass& w)
{
	if (IsDQOverride())
	{
	...
	mgrVertexData->UpdateDataAlignment(totalItems + 1);
...
void VertexListClass::PasteWeights(const Tab<VertexInfluenceListClass>& w)
{
	if (IsDQOverride())
	{
		if (w.Count())
...
void VertexListClass::RemoveDuplicateBones()
{
	bool doCleanup{ false };
	std::map<int, std::vector<int>> boneId2Indices;
	for (auto i = 0; i < totalItems; i++)
	{
		VertexInfluenceListClass& item(GetItemAtIndex(i));
		auto it = boneId2Indices.find(item.Bones);
		if (it == boneId2Indices.end())
		{
			boneId2Indices.insert({ item.Bones, std::vector<int>{ i } });
		}
		else
		{
			doCleanup = true;
	...
	{
		Tab<VertexInfluenceListClass> temp;
		for (auto& item : boneId2Indices)
		{
			VertexInfluenceListClass& vilcO(GetItemAtIndex(item.second[0]));
			if (item.second.size() > 1)
			{
				for (auto i = 1; i < item.second.size(); ++i)
				{
					VertexInfluenceListClass& vilcI(GetItemAtIndex(item.second[i]));
					vilcO.Influences += vilcI.Influences;
					vilcO.normalizedInfluences += vilcI.normalizedInfluences;
				}
			}
			temp.Append(1, &vilcO);
		}
