...
void BonesDefMod::ApplyPaintWeights(BOOL alt, BOOL shift, INode *incNode)
	{
//5.1.03
	BOOL blendMode = FALSE;
	pblock_param->GetValue(skin_paintblendmode,0,blendMode);
	if (shift)
		blendMode = TRUE;
...
//get original amount
					...
					if(boneID != -1 || bmd->GetDQWeightOverride()) // if we are in the DQ override weight mode there is no bone associated so we can ignore it
						{
//add to it str
						if (bmd->GetDQWeightOverride())
							amount = bmd->GetDQBlendWeight(j);
						else for (int k =0; k < bmd->VertexData[j]->WeightCount(); k++)
							{
						...
						if (blendMode &&  bmd->blurredWeights.Count() != 0)
							{
								...
								float blendStr = str[j];
								if (blendStr < 0.0f) blendStr = 0.001f;
								if (blendStr > 1.0f) blendStr = 1.0f;
				 				amount += (bmd->blurredWeights[j] - amount) * blendStr;
				 		    }
				 		 else
				 			{
							if (alt)
...
//this puts back the original state of the node vc mods and shade state
	for (int  i = 0; i < painterData.Count(); i++)
		...
		if (bmd)
			{
//5.1.05
			bmd->BuildBlurData(ModeBoneIndex); //need to always build the blend data since the user can turn it on any time with the shift key
...
BOOL  BonesDefMod::PaintStroke(
	...
	for (int i = 0; i < selNodeCount; i++)
		{
		ApplyPaintWeights(alt,shift,  GetCOREInterface()->GetSelNode(i));
		}

	lagHit++;
	if (  (lagRate == 0) ||  ((lagHit%lagRate) == 0)) NotifyDependents(FOREVER, PART_GEOM, REFMSG_CHANGE);
...
BOOL  BonesDefMod::EndStroke(int ct, BOOL *hits, IPoint2 *mousePos, 
						  ...
						  Point3 *localMirrorPoint, Point3 *localMirrorNormal	)
	{
	theHold.Restore(); // undo the previous weight deltas
	ApplyPaintWeights(alt[ct-1], shift[ct-1], NULL);
...
BOOL  BonesDefMod::EndStroke()
	{
	theHold.Restore(); // undo the previous weight deltas
	ApplyPaintWeights(painterData[0].alt,painterData[0].shift, NULL);
	theHold.Accept(GetString(IDS_PW_WEIGHTCHANGE));
	NotifyDependents(FOREVER, PART_GEOM, REFMSG_CHANGE);
	RebuildPaintNodes();
	PaintAttribList(TRUE);
...
void BonesDefMod::RebuildPaintNodes()
	...
	for (int i = 0; i < nodes.Count(); i++)
		...
		if ( (os.obj->NumPoints() != painterData[i].bmd->VertexData.Count()) ||
			...
			Point3* p3Array = ct > 0 ? pointList.Addr(0) : nullptr;
			pPainterInterface->LoadCustomPointGather(ct, p3Array, nodes[i]);
			}
		}
...
class BonesDefBrushPresetContext : public IBrushPresetContext {
	public:
		...
		const TCHAR*		GetParamName( int paramID );
		ParamType2			GetParamType( int paramID );

		const MCHAR*		GetDisplayParamName( int paramID );
...
const TCHAR* BonesDefBrushPresetContext::GetContextName()
	{
...
const TCHAR* BonesDefBrushPresetContext::GetParamName( int paramID )
	{
	return _T("Mode");
	}
...
const MCHAR* BonesDefBrushPresetContext::GetDisplayParamName( int paramID )
	{
