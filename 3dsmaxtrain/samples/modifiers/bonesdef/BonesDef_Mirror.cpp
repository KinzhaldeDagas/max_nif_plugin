...
BOOL MirrorData::Enabled()
{
	...
	if (mod)
	{
		mod->pblock_mirror->GetValue(skin_mirrorenabled,0,enabled);
	}
	return enabled;
}
...
int MirrorData::DisplayMirrorData(GraphicsWindow *gw)
{
	...
	Matrix3 mtm;
	float off = 0.;
	mod->pblock_mirror->GetValue(skin_mirroroffset,0,off);
	mod->pblock_mirror->GetValue(skin_mirrorplane,0,mirrorPlaneDir);
	...
	int projection;
	mod->pblock_mirror->GetValue(skin_mirrorprojection,0,projection);
...
void MirrorData::GetMirrorTM(Matrix3 &tm, int &dir)
{
	...
	float offset = 0.;
	mod->pblock_mirror->GetValue(skin_mirroroffset,0,offset);
...
void MirrorData::BuildBonesMirrorData()
{
	...
	float threshold;
	mod->pblock_mirror->GetValue(skin_mirrorthreshold,0,threshold);
...
void MirrorData::BuildVertexMirrorData()
{
	...
	mod->pblock_mirror->GetValue(skin_mirrorfast,0,fastEngine);
...
void MirrorData::PasteBones(TimeValue t, Matrix3 mtm, int mirrorPlaneDir, int sourceBone, int destBone)
{
	...
	sourceBoneData.EndPoint1Control->GetValue(t,&pa);
	sourceBoneData.EndPoint2Control->GetValue(t,&pb);
	...
	for (int j =0; j < sourceCount ; j++)
	{
		...
		sourceBoneData.CrossSectionList[j].InnerControl->GetValue(t,&inner);
		sourceBoneData.CrossSectionList[j].OuterControl->GetValue(t,&outer);
