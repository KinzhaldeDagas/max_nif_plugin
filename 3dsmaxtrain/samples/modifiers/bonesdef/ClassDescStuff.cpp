...
public:
	...
	void Get(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t, Interval &valid)
	{
		...
		case skin_draw_all_envelopes:
			if (p->pblock_display)
			{
				p->pblock_display->GetValue(skin_display_draw_all_envelopes,t,val);
				v.i = val;
			}
			break;
		case skin_draw_vertices:
			if (p->pblock_display)
			{
				p->pblock_display->GetValue(skin_display_draw_vertices,t,val);
				v.i = val;
			}
			break;
		case skin_ref_frame:
			if (p->pblock_advance)
			{
				p->pblock_advance->GetValue(skin_advance_ref_frame,t,val);
				v.i = val;
			}
			break;
		case skin_always_deform:
			if (p->pblock_advance)
			{
				p->pblock_advance->GetValue(skin_advance_always_deform,t,val);
				v.i = val;
			}
			break;
		}
	...
	void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)    // set from v
	{
			...
			if (BonesDefMod::ip)
			{
				...
				else
				{
					...
					ADPForSamples::ADPSkinEventLogger::LogSkinADPEvent(
							...
							ADPForSamples::ADPSkinEventLogger::SkinFeatureUsage_EventType::SKIN_SELECT_VERTICES);
				}
			}
			break;
		...
		case skin_paintblendmode:
			if (v.i != 0)
				ADPForSamples::ADPSkinEventLogger::LogSkinADPEvent(
					ADPForSamples::ADPSkinEventLogger::SkinWaypointType::SKIN_FEATURE_USAGE,
					ADPForSamples::ADPSkinEventLogger::SkinFeatureUsage_EventType::SKIN_PAINT_BLEND_WEIGHTS);
	...
	void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)    // set from v
	{
		...
		case skin_display_shadeweights:
			if (p->pblock_display)
			{
				...
				if (val == FALSE)
				{
					if (BonesDefMod::ip && (BonesDefMod::ip->GetSubObjectLevel() == 1))
					{
						p->RestoreVCMode();
					}
				}
	...
	void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)    // set from v
	{
		...
		case skin_advance_bonelimit:
			...
			ADPForSamples::ADPSkinEventLogger::LogSkinADPEvent(
					...
					ADPForSamples::ADPSkinEventLogger::SkinStatistics_EventType::SKIN_BONEAFFECTS_LIMIT, v.i);
			break;
		}
	}
};
...
public:
	...
	void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)    // set from v
	{
		...
		case skin_mirrorthreshold:
			{
				BOOL manualUpdate;
				p->pblock_mirror->GetValue(skin_mirrormanualupdate,0,manualUpdate);
		...
		case skin_mirrorenabled:
			if (v.i)
			{
				ADPForSamples::ADPSkinEventLogger::LogSkinADPEvent(
						ADPForSamples::ADPSkinEventLogger::SkinWaypointType::SKIN_FEATURE_USAGE,
						ADPForSamples::ADPSkinEventLogger::SkinFeatureUsage_EventType::SKIN_MIRROR_MODE);
...
//		p_ui, 			TYPE_SPINNER, EDITTYPE_FLOAT, IDC_PAINT_STR2,IDC_PAINT_STR_SPIN2,  SPIN_AUTOSCALE,
	...
	skin_local_squash,  _T("localSquash"),	TYPE_FLOAT_TAB,0, 	P_ANIMATABLE, 	IDS_PW_LOCALSQUASH,
		p_nonLocalizedName, _T("Local Squash"),
		p_end,
...
class BonesDefModPostLoad_ADP : public PostLoadCallback
{
public:
	BonesDefMod* n;

	BonesDefModPostLoad_ADP(BonesDefMod* ns)
	{
		n = ns;
	}
	...
	void proc(ILoad * iload) override
	{
		...
		ADPHelpers::HandleADPData(n);
		if (n->currentSelectedGizmo == 0)
		{
			ADPForSamples::ADPSkinEventLogger::LogSkinADPEvent(
					ADPForSamples::ADPSkinEventLogger::SkinWaypointType::SKIN_FEATURE_USAGE,
...
IOResult BonesDefMod::Load(ILoad *iload)
{
	...
	iload->RegisterPostLoadCallback(new BonesDefModPostLoad_ADP(this));
	
	return IO_OK;
}

void BonesDefMod::BeginEditParams(
	IObjParam  *ip_in, ULONG flags_in,Animatable *prev)
{
	inAddBoneMode = FALSE;
	pblock_display->GetValue(skin_display_shadeweights,0,shadeVC);

	editing = TRUE;
	BonesDefMod::ip = ip_in;
	this->flags = flags_in;
	...
	bonesDefDesc.BeginEditParams(ip, this, flags_in, prev);
...
void BonesDefMod::EndEditParams(
	IObjParam *ip_in,ULONG flags_in,Animatable *next)
{
	BonesDefMod::ip = ip_in;
	...
	if (pblock_gizmos->Count(skin_gizmos_list) > 0)
	{
		...
		if (currentSelectedGizmo != -1)
		{
			...
			GizmoClass *gizmo = (GizmoClass *)ref;
			if (gizmo)
				gizmo->EndEditParams(ip, flags_in, NULL);
		}
	}
	...
	if (iEditEnvelopes && iEditEnvelopes->IsChecked()) {
	...
	if (iPaintButton && iPaintButton->IsChecked())
	{
	...
	bonesDefDesc.EndEditParams(ip, this, flags_in, next);
...
TSTR BonesDefMod::SubAnimName(int index, bool localized)
{
	int ct = 0;
	TSTR animName;
	for (int i = 0; i < BoneData.Count(); i++)
	{
		...
		if (boneData.Node)
		{
			if (ct == index)
			{
				animName.printf(_T("%s(%s)"), (localized ? GetString(IDS_BONESTART) : _T("Start Point")), boneData.Node->GetName());
				return animName;
			}
			ct++;
			if (ct == index)
			{
				animName.printf(_T("%s(%s)"), (localized ? GetString(IDS_BONEEND) : _T("End Point")), boneData.Node->GetName());
				return animName;
			}
			ct++;
			for (int j = 0; j < boneData.CrossSectionList.Count(); j++)
			{
				if (ct == index)
				{
					animName.printf(_T("    %s(%0.2f)"), (localized ? GetString(IDS_INNNERRADIUS) : _T("Inner Radius")), boneData.CrossSectionList[j].u);
					return animName;
				}
				ct++;
				if (ct == index)
				{
					animName.printf(_T("    %s(%0.2f)"), (localized ? GetString(IDS_OUTERRADIUS) : _T("Outer Radius")), boneData.CrossSectionList[j].u);
					return animName;
				}
				ct++;
			}
