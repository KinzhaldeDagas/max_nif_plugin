...
Point3 GizmoJointClass::InterpSpline2(float u, const Point3* knot) const
{
	Point3 knotsPts[16];
	...
	Point3 p = ((s * knotsPts[where] + (3.0f * t) * knotsPts[where + 1]) * s + (3.0f * t2) * knotsPts[where2 - 1]) * s +
			t * t2 * knotsPts[where2];
...
BOOL LocalGizmoData::IsAffected(int index) const
{
	for (int i =0; i < affectedVerts.Count(); i++)
	{
...
void BonesDefMod::SelectGizmo(int id)
{
	...
	{
		IRollupWindow *pRollup = ip->GetCommandPanelRollup();
...
void BonesDefMod::AddGizmo()
{
	...
	GizmoClass *gizmo;
	currentSelectedGizmoType = SendMessage(GetDlgItem(hParamGizmos,IDC_DEFORMERS), CB_GETCURSEL,0,0);
	...
	for (int i =0; i<lmdproc.localModList.Count(); i++)
	{
		...
		if (pointList.Count() >0)
		{
			...
			LocalGizmoData *temp = new LocalGizmoData();
...
void BonesDefMod::UpdateGizmoList()
{
	...
	sel = SendMessage(GetDlgItem(hParamGizmos,IDC_LIST1), LB_GETCURSEL,(WPARAM) 0,(LPARAM) 0);

	SendMessage(GetDlgItem(hParamGizmos,IDC_LIST1),	LB_RESETCONTENT,0,0);
	...
	for (int i =0; i < pblock_gizmos->Count(skin_gizmos_list); i++)
	{
		...
		SendMessage(GetDlgItem(hParamGizmos,IDC_LIST1), LB_ADDSTRING,0,(LPARAM) name);
	}
	int iret = SendMessage(GetDlgItem(hParamGizmos,IDC_LIST1), LB_SETCURSEL,sel,0);
	if (iret == LB_ERR)
	{
		int sel = pblock_gizmos->Count(skin_gizmos_list)-1;
		int tret = SendMessage(GetDlgItem(hParamGizmos,IDC_LIST1), LB_SETCURSEL,sel,0L);
		if (tret != LB_ERR)
...
void GizmoParamsMapDlgProc::FilloutText()
{
	...
	//get parent id
	int parent = 0;
	giz->pblock_gizmo_data->GetValue(skin_gizmoparam_joint_parent_id,0,parent);
	if (parent < 0) return;
	const TCHAR *temp = giz->bonesMod->GetBoneName(parent);
	name.printf(_T("%s: %s"),GetString(IDS_PW_GIZMOPARAM_PARENT),temp);
	//set the text of the parent
	SetWindowText(GetDlgItem(giz->hGizmoParams,IDC_PARENT_TEXT),
		name.data());
	//get child id
	int child = 0;
	giz->pblock_gizmo_data->GetValue(skin_gizmoparam_joint_child_id,0,child);
	if (child < 0) return;
	//set the text of the child
	temp = giz->bonesMod->GetBoneName(child);
	name.printf(_T("%s: %s"),GetString(IDS_PW_GIZMOPARAM_CHILD),temp);
	SetWindowText(GetDlgItem(giz->hGizmoParams,IDC_CHILD_TEXT), name.data());
}

INT_PTR GizmoParamsMapDlgProc::DlgProc(
	...
	case WM_INITDIALOG:
		{
			...
			giz->pblock_gizmo_data->GetValue(skin_gizmoparam_joint_keygraph,0,ref);
			if (ref)
			{
				graph = (ICurveCtl *) ref;
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Joint Angle Deformer"); }
...
static ParamBlockDesc2 skin_gizmoparam_blk
(
	skin_gizmoparam, _T("gizmos"),  0, &gizmoJointDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, 0,
	//rollout
	...
	skin_gizmoparam_joint_use_volume,  _T("use_volume"),	TYPE_BOOL,	0, 	IDS_PW_GIZMOPARAM_USEVOLUME,
		p_default, 		FALSE,
		p_ui, 			TYPE_SINGLECHECKBOX, IDC_USE_VOLUME,
		p_end,

	skin_gizmoparam_joint_enable,  _T("enable"),	TYPE_BOOL,	0, 	IDS_PW_ENABLE,
		p_default, 		TRUE,
		p_ui, 			TYPE_SINGLECHECKBOX, IDC_ENABLE,
		p_end,
...
BOOL GizmoJointClass::IsVolumeBased() const
{
	if (!mPreDeformCacheValid)
	{
		throw std::runtime_error{
			__FUNCTION__ " : Accessing stale data. Something changed the value after PreDeformSetup."
		};
	}
	return mIsVolumeBasedCache;
}

BOOL GizmoJointClass::IsEnabled() const
{
	if (!mPreDeformCacheValid)
	{
		throw std::runtime_error{
	...
	return mIsEnabledCache;
}

BOOL GizmoJointClass::IsInVolume(Point3 p, Matrix3 tm) const
{
...
void GizmoJointClass::SetInitialName()
{
	if (pblock_gizmo_data)
	{
		...
		{
			int child = 0;
			pblock_gizmo_data->GetValue(skin_gizmoparam_joint_child_id,0,child);
			TSTR tn;
			tn.printf(_T("%s - %s"), GetString(IDS_PW_GIZMOSJOINT),bonesMod->GetBoneFlat(child)->GetName());

			pblock_gizmo_data->SetValue(skin_gizmoparam_name,0,tn.data());
		}
		else
			pblock_gizmo_data->SetValue(skin_gizmoparam_name,0,GetString(IDS_PW_GIZMOSJOINT));
	}
}

const TCHAR *GizmoJointClass::GetName()
{
	const TCHAR *name = nullptr;
	pblock_gizmo_data->GetValue(skin_gizmoparam_name,0,name);
	return name;
}

void GizmoJointClass::SetName(const TCHAR *name)
{
...
{
	ReferenceTarget* ref = nullptr;
	pblock_gizmo_data->GetValue(skin_gizmoparam_joint_keygraph,0,ref);
...
{
	BOOL editing = FALSE;
	pblock_gizmo_data->GetValue(skin_gizmoparam_joint_editing,0,editing);
	return editing;
}
...
void GizmoJointClass::ResetRotationPlane()
{
	...
	pblock_gizmo_data->GetValue(skin_gizmoparam_joint_parent_id,0,parent);
	pblock_gizmo_data->GetValue(skin_gizmoparam_joint_child_id,0,child);
...
Matrix3 GizmoJointClass::CreateInitialAngles()
{
	...
	if (tdot <= 0.00001f)
		l2.y += 10.f;
	//get its transform
	//create a matrix aligned to the l1 and l2 of the bone
...
BOOL GizmoJointClass::InitialCreation(int count, Point3 *pt, int numbeOfInstances, int *mapTable)
{
	if (count == 0)
	{
		MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), GetString(IDS_PW_VERTEXERROR), NULL, MB_OK);
		return FALSE;
	}
	...
	if ((parent <0) || (child<0))
	{
		MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), GetString(IDS_PW_PARENTERROR), NULL, MB_OK);
		return FALSE;
	}
	...
	graph->RegisterResourceMaker(this);
	...
	float iangle = 0.;
	pblock_gizmo_data->GetValue(skin_gizmoparam_joint_initial_angle,0,iangle);
	stopReferencing = FALSE;
	for (int i=0; i < 20; i++)
	{
...
void GizmoJointClass::GetWorldBoundBox(TimeValue t,INode* inode, ViewExp *vpt, Box3& box, ModContext *mc)
{
	...
	Matrix3 toWorld;
	int id = 0;
	pblock_gizmo_data->GetValue(skin_gizmoparam_joint_parent_id,0,id);
	if (id < 0) return;
...
//this is just a undo record so I know when we get an undo, and when the mouse cycle is completed
...
public:
	...
	MoveRestore(GizmoJointClass *mod)
	{
		...
		ReferenceTarget *ref =NULL;
		mod->pblock_gizmo_data->GetValue(skin_gizmoparam_joint_keygraph,0,ref);
		if (ref)
			graph = (ICurveCtl *) ref;
...
void GizmoJointClass::Move( TimeValue t, Matrix3& partm, Matrix3& tmAxis, Point3& val, Matrix3 tm, BOOL localOrigin)
{
	...
	pblock_gizmo_data->GetValue(skin_gizmoparam_joint_twist,0,twistAngle);
	...
	if (graph)
	{
		...
		{
			int sel = 0;
			pblock_gizmo_data->GetValue(skin_gizmoparam_joint_selection,0,sel,i);
			if (sel)
			{
				ICurve *cv = graph->GetControlCurve(i*3);
				Point3 p;
...
void GizmoJointClass::InvertSelection(int selLevel)
{
	...
	{
		BOOL sel = TRUE;
		pblock_gizmo_data->GetValue(skin_gizmoparam_joint_selection,0,sel,j);
		sel = !sel;
		pblock_gizmo_data->SetValue(skin_gizmoparam_joint_selection,0,sel,j);
	}
	NotifyDependents(FOREVER, PART_SELECT, REFMSG_CHANGE);
...
int GizmoJointClass::Display(TimeValue t, GraphicsWindow *gw, Matrix3 tm)
{
	...
	//NEW FIX
	Point3 dir;
	pblock_gizmo_data->GetValue(skin_gizmoparam_joint_orientation,0,dir);
	...
	for (int i = 0 ; i < 16; i++)
	{
		circle[i] = axis  * iupVecTM * upVecTM  *radius;
		upVecTM.PreRotateZ(inc);
	}
	for (int i = 0 ; i < 16; i++)
		circle[i] = circle[i] * node->GetObjectTM(t)*tm;

	dlp[1] = dir * 50.0f* node->GetObjectTM(t)*tm;
...
void GizmoJointClass::PreDeformSetup(TimeValue t)
{
	...
	int childId = 0;
	Interval iv;
	pblock_gizmo_data->GetValue(skin_gizmoparam_joint_parent_id,0,parentId);
	pblock_gizmo_data->GetValue(skin_gizmoparam_joint_child_id,0,childId);
	...
	{
		Point3 p;
		pblock_gizmo_data->GetValue(skin_gizmoparam_joint_points,0,p,i);
	...
	pblock_gizmo_data->GetValue(skin_gizmoparam_joint_initial_angle,0,initialAngle);
	...
	if (ip)
	{
		...
		//set the text of the parent
		SetWindowText(GetDlgItem(hGizmoParams,IDC_ANGLE_TEXT), angleText.data());
	}
	...
	for ( int i = 0; i < 20; i++)
	{
		...
		pblock_gizmo_data->GetValue(skin_gizmoparam_joint_weights,0,weight,i);
		if (weight == 1.0f)
		{
			deformedPoints[i] = p;
		}
	...
	// Cache things for the const functions (thread-safe).
	mPreDeformCacheValid = true;

	BOOL enabled = FALSE;
	pblock_gizmo_data->GetValue(skin_gizmoparam_joint_enable, 0, enabled);
	mIsEnabledCache = enabled;

	BOOL useVol = FALSE;
	pblock_gizmo_data->GetValue(skin_gizmoparam_joint_use_volume, 0, useVol);
	mIsVolumeBasedCache = useVol;
}

Point3 GizmoJointClass::DeformPoint(TimeValue t, int index, Point3 initialP, Point3 p, Matrix3 tm) const
{
	if (!val)
		return p;
...
Point3 GizmoJointClass::Map(int ii, Point3 p) const
{
	Point3 q(0,0,0), pp;
...
public:
	...
	void proc(ILoad *iload) {
		ICurveCtl *graph;
		ReferenceTarget *ref = nullptr;
		n->pblock_gizmo_data->GetValue(skin_gizmoparam_joint_keygraph,0,ref);
...
void *GizmoJointClass::GetInterface(ULONG id)
{
	if(id == I_RESMAKER_INTERFACE)
		...
		return GizmoClass::GetInterface(id);
}
...
void GizmoJointClass::PasteFromBuffer(IGizmoBuffer *buf)
{
	...
	if (ref)
	{
		...
		for (int i =0; i < 60; i++)
		{
			...
			for (int j = 0; j < buffer->counts[i]; j++)
			{
