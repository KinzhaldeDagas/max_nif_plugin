	...
	HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		{
			...
			float scale = 0.,tolerance = 0.,weight = 0.;
			mod->pblock_param->GetValue(skin_weighttool_tolerance,0,tolerance);
			mod->pblock_param->GetValue(skin_weighttool_scale,0,scale);
			mod->pblock_param->GetValue(skin_weighttool_weight,0,weight);
			...
			TSTR copyStatus;
			copyStatus.printf(_T("%d %s"),ct,GetString(IDS_PW_COPYSTATUS));
	...
	case WM_COMMAND:
		...
		case IDC_BONELIST:
			...
			{
				int fsel = SendMessage(GetDlgItem(hWnd,IDC_BONELIST),LB_GETCURSEL,0,0);
				if (fsel != -1)
				{
					if (fsel < mod->weightToolSelBoneList.Count())
					{
						int boneIndex = mod->weightToolSelBoneList[fsel];
						mod->SelectBone(boneIndex);
		...
		case IDC_PASTEBYPOS_BUTTON:
			{
				float tolerance = iTolerance->GetFVal();
				mod->PasteWeights(TRUE,tolerance);
...
void BonesDefMod::AddWeight(float weight)
{
	...
	for (int i =0; i < objects; i++)
	{
		...
		for ( int j = 0; j < vsel.Count();j++)
		{
			...
			{
				amount = bmd->GetDQBlendWeight(k) + weight;
				bmd->SetDQBlendWeight(k,amount);
				hit = TRUE;
				
			}
			else for (int j = 0; j < ct; j++)
			{
...
void BonesDefMod::ScaleWeight(float scale)
{
	...
	for (int i =0; i < objects; i++)
	{
		...
		for ( int j = 0; j < vsel.Count();j++)
		{
			...
			if (bmd->GetDQWeightOverride())  //since there is no bones associated with the blend weight just use that
			{
				amount = bmd->GetDQBlendWeight(k) * scale;
				bmd->SetDQBlendWeight(k,amount);				
			}
			else for (int l = 0; l < ct; l++)
			{
...
void BonesDefMod::CopyWeights(BoneModData *bmd, INode *node)
{
	...
	if(bmd && node)
	{
		...
		aBoneVertexMgr->SetVertexDataPtr(&vertexWeightCopyBuffer.copyBuffer);
		for (auto j = 0; j < bmd->VertexData.Count(); j++)
		{
			auto *vc = new VertexListClass();
			vc->SetVertexDataManager(j, aBoneVertexMgr);
			vc->flags =  bmd->VertexData[j]->flags;
			auto tmp = bmd->VertexData[j]->CopyWeights();
			vc->PasteWeights(tmp);
			vc->LocalPos =  bmd->VertexData[j]->LocalPos * tm;
			vc->LocalPosPostDeform =  bmd->VertexData[j]->LocalPosPostDeform;
			vertexWeightCopyBuffer.copyBuffer[j] = vc;
		}
	}
...
void BonesDefMod::CopyWeights()
{
	...
	for (int i =0; i < objects; i++)
	{
		...
		auto *bmd = (BoneModData*)mcList[i]->localData;
		for (int j = 0; j < bmd->VertexData.Count(); j++)
		{
			if (bmd->selected[j])
			{
				auto *vc = new VertexListClass();				
				vc->SetVertexDataManager(s, aBoneVertexMgr);
				vc->flags =  bmd->VertexData[j]->flags;
				auto tmp = bmd->VertexData[j]->CopyWeights();
				vc->PasteWeights(tmp);
				vc->LocalPos =  bmd->VertexData[j]->LocalPos * tm;
				vc->LocalPosPostDeform =  bmd->VertexData[j]->LocalPosPostDeform;
				vertexWeightCopyBuffer.copyBuffer[s] = vc;
				s++;
			}
		}
	}
		...
		if (first)
		{
			for (int i = 0 ; i < bmd->VertexData.Count();i++)
			{
				if (bmd->selected[i] && (first))
				{
					...
					for (int j = 0; j < ct; j++)
					{
						int boneIndex =  bmd->VertexData[i]->GetBoneIndex(j);
						float v = bmd->VertexData[i]->GetNormalizedWeight(j);
						if (IsValidBoneIndex(boneIndex))
						{
							INode *node = BoneData[boneIndex].Node;
							TSTR status;
							if (boneIndex == ModeBoneIndex)
								selIndex = j;
							weightToolSelBoneList.Append(1,&boneIndex,100);
							status.printf(_T("%1.3f : %s"),v,node->GetName());
							SIZE size;
							DLGetTextExtent(hdc,  (LPCTSTR)status,&size);
							if (size.cx > width)
								width = size.cx;
							SendMessage(GetDlgItem(weightToolHWND,IDC_BONELIST),LB_ADDSTRING,0,(LPARAM)(const TCHAR*)status);
						}
					}
					first = FALSE;
				}
	...
	for (int i = 0; i < objects; i++)
	{
		...
		for (int i = 0; i < bmd->VertexData.Count(); i++)
		{
			...
			}
			{
				// get the remainder
				float newWeight;
				if (bmd->GetDQWeightOverride()) // since there is no bones associated with the blend weight just use
												// that
				{
					newWeight = bmd->VertexData[i]->GetDQBlendWeight();
				}
				...
				if (bmd->GetDQWeightOverride()) // since there is no bones associated with the blend weight just use
												// that
				{
					bmd->VertexData[i]->SetDQBlendWeight(newWeight);
				}
				else
				{
					int id = -1;
					...
					for (int k = 0; k < numberOfBones; k++)
					{
						...
						if (boneID != ModeBoneIndex)
						{
							...
							if (initialRemainder != 0.0f)
							{
								...
								bmd->VertexData[i]->SetWeight(k, 0.0f);
						}
						else
							id = k;
					}
...
void BonesDefMod::EnableWeightToolControls()
{
	...
	if (weightToolHWND)
	{
		...
		pblock_param->GetValue(skin_filter_vertices,0,FilterVertices);
		if (!FilterVertices)
		{
