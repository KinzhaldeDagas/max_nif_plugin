...
RefTargetHandle GizmoJointMorphClass::Clone(RemapDir& remap)
{
	GizmoJointMorphClass* buffer = new GizmoJointMorphClass();
	buffer->ReplaceReference(0, remap.CloneRef(pblock_gizmo_data));
	...
	switch (msg) {
		...
		case WM_CC_DEL_CURVEPT:
			giz->NotifyDependents(FOREVER, PART_GEOM, REFMSG_CHANGE);
			GetCOREInterface()->RedrawViews(GetCOREInterface()->GetTime());
			break;
		case WM_COMMAND:
			{
...
class GizmoJointMorphClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Morph Angle Deformer"); }
...
BOOL GizmoJointMorphClass::IsVolumeBased() const
{
	return FALSE;
}

BOOL GizmoJointMorphClass::IsEnabled() const
{
	if (!mPreDeformCacheValid)
	{
		throw std::runtime_error{ __FUNCTION__ " : Accessing stale data. Something changed the value after PreDeformSetup." };
	}
	return mIsEnabledCache;
}

BOOL GizmoJointMorphClass::IsInVolume(Point3 p, Matrix3 tm) const
{
	return TRUE;
}
...
if ((pblock_gizmo_data) && (bonesMod))
	...
	TSTR name;
	name.printf(_T("%s - %s"),GetString(IDS_PW_GIZMOJOINT),bonesMod->GetBoneFlat(parent)->GetName());
...
const TCHAR *GizmoJointMorphClass::GetName()
{
const TCHAR *name;
pblock_gizmo_data->GetValue(skin_gizmoparam_name,0,name,FOREVER);
return name;
}
void GizmoJointMorphClass::SetName(const TCHAR *name)
{
...
BOOL GizmoJointMorphClass::InitialCreation(int count, Point3 *pt, int modCount, int *mapTable)  
{
	...
	if (modCount > 1) 
		{
		MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), GetString(IDS_PW_INSTANCEERROR), NULL, MB_OK);
		return FALSE;
		}
...
//get set name
	TSTR name;
	name.printf(_T("Base Morph %3.2f"),initialAngle);
...
float GizmoJointMorphClass::GetCurrentAngle(TimeValue t)
{
	...
	if ((parent < 0) || (child < 0)) return 0.0f;
...
//compute the initial angle between the joints
	...
	if (dot >= 0.9999f)
		...
		initialAngle = 180.0f;
	else initialAngle = fabs(acos(dot) * 180.f/PI);
	return initialAngle;
}
...
//now build remap list by comparing positions.
	...
	if (!found)
		{
		TSTR name;
		name.printf(_T("%s angle %3.2f"),node->GetName(),angle);
		TCHAR *tname = (TCHAR *) name.data();
		pblock_gizmo_data->Append(skin_gizmoparam_jointmorph_names,1,&tname);
...
//assign a name
	if (!found)
		{
		TSTR name;
		name.printf(_T("Stack Morph angle %3.2f"),angle);
		TCHAR *tname = (TCHAR *) name.data();
		pblock_gizmo_data->Append(skin_gizmoparam_jointmorph_names,1,&tname);
		pblock_gizmo_data->Append(skin_gizmoparam_jointmorph_angles,1,&angle);
		pblock_gizmo_data->Append(skin_gizmoparam_jointmorph_vecs,1,&vec);
...
void GizmoJointMorphClass::PreDeformSetup(TimeValue t)
{
	...
	{
		TSTR angleText;
		angleText.printf(GetString(IDS_CURRENT_ANGLE), cBend);
		// set the text of the parent
	...
	// Cache some stuff so the functions can be const (thread-safe).
	...
	int count = pblock_gizmo_data->Count(skin_gizmoparam_jointmorph_morphpos);
	mMorphPosCache.resize(count);
	Point3 pt;
	for (int i = 0; i < count; ++i)
	{
		pblock_gizmo_data->GetValue(skin_gizmoparam_jointmorph_morphpos, 0, pt, FOREVER, i);
		mMorphPosCache[i] = pt;
	}
}
...
Point3 GizmoJointMorphClass::DeformPoint(TimeValue t, int index, Point3 initialP, Point3 p, Matrix3 tm) const
{
...
for (int i =0; i < pblock_gizmo_data->Count(skin_gizmoparam_jointmorph_names); i++) 
	...
	const TCHAR *name = pblock_gizmo_data->GetStr(skin_gizmoparam_jointmorph_names,0,i);
	SendMessage(GetDlgItem(hGizmoParams,IDC_LIST1),
				LB_ADDSTRING,0,(LPARAM) name);
	}
int iret = SendMessage(GetDlgItem(hGizmoParams,IDC_LIST1),
...
ReplaceReference(0,CloneRefHierarchy(buffer->mod->pblock_gizmo_data));
