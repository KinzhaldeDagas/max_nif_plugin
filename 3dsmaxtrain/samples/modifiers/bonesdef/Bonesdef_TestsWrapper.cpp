...
void BonesdefTestsWrapper::TestNormalizeWeights()
{
	...
	// for vertex 10
	// this vertex has three influncing bones index {0,1,2}
	vrtxId = 10;
	index = 0;
	NormalizeWeights(vrtxId);
	...
	// for vertex 11
	// this vertex has only two (see lines between 175 and 185) influencing bones but here 
	// we are testing with three bone indices to see the behavior: index {0,1,2}
	vrtxId = 11;
	index = 0;
	NormalizeWeights(vrtxId);
	weight0 = GetNormalizedWeight(vrtxId, index);
	...
	weight1 = GetNormalizedWeight(vrtxId, index);
	index = 2;
	weight2 = GetNormalizedWeight(vrtxId, index);
	sum = weight0 + weight1 + weight2;
	if (FloatTestHandler)
	{
		FloatTestHandler(sum, 1.000f);
