...
void BonesDefMod::fnWeightTable()
   ...
   // AF (08/01/01) draw the custom menu bar
      ...
      {
         if (auto* menuExt = GetCOREInterface()->GetICuiMenuManager()->GetWinMenuExt(s_weightTableMenuId))
         {
            SetMenu(hWeightTable, menuExt->CreateWindowsMenu());
            menuExt->UpdateWindowsMenu();
         }
      }
      if (iWeightTable)
      {
         iWeightTable->SetCheck(TRUE);
      } 
      } 
   else 
      {
...
//check if inside the move rect for the vert name
         int x = GET_X_LPARAM(lParam);  // horizontal position of cursor 
         int y = GET_Y_LPARAM(lParam);  // vertical position of cursor 
         initialX = x;
         lastX = x;
...
//            ti.lpszText = (LPSTR) szTips[id++]; 
            ti.lpszText = GetString(ttStrings[row]); 
         if (mod->weightTableWindow.GetFlipFlopUI())
            {
            ti.rect.left = 0; 
...
//check if in name list if so we are in select mode
      ...
      case WM_RBUTTONDOWN:
         ...
         initialXR = GET_X_LPARAM(lParam);  // horizontal position of cursor 
         initialYR = GET_Y_LPARAM(lParam);  // vertical position of cursor 
      ...
      case WM_MOUSEMOVE:
         ...
         if (mouseLDown)  
            ...
            if (draggin == FALSE)
               ...
               int offset = 0;
               if (mod->weightTableWindow.GetDragLeftMode())
                  offset = abs(xPos-initialX);
               else offset = abs(yPos-initialY);
               if ( offset > 3 )
                  {
                  draggin = TRUE;
                  theHold.Begin();
...
*/
      case WM_INITDIALOG:
         ...
         TSTR windowsName;
         windowsName.printf(_T("%s"),mod->weightTableWindow.GetName());
...
//       mod->weightTableWindow.DeActivateActionTable();
         ...
         if (mod->iWeightTable)
            mod->iWeightTable->SetCheck(FALSE);
...
INT_PTR CALLBACK AddCustomListDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) 
   ...
   case WM_INITDIALOG:
      ...
      name.printf(_T("%s"),GetString(IDS_PW_CUSTOMLIST));
...
void WeightTableWindow::UpdateUI()
   ...
   name.printf(_T("%s"),GetString(IDS_PW_AFFECTALLVERTS2));
   SendMessage(hGlobalDrop, CB_ADDSTRING, 0, (LPARAM)(LPCTSTR) name.data());        
   
   name.printf(_T("%s"),GetString(IDS_PW_AFFECTSEL));
   SendMessage(hGlobalDrop, CB_ADDSTRING, 0, (LPARAM)(LPCTSTR) name.data());
