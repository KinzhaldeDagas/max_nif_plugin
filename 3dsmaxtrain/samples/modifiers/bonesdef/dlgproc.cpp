...
static HIMAGELIST hArrowImages = NULL;
static HBITMAP hArrowDownImage = NULL;
static HBITMAP hArrowUpImage = NULL;

extern void MoveWindowUp(HWND hwnd, int offset, int extraHeight);

static HWND hWndResizer = NULL;			// The resizing "grip" at the lower right corner

const TCHAR* ARROW_UP_ICON(_T("Common/UIComponents/ArrowUp"));
const TCHAR* ARROW_DOWN_ICON(_T("Common/UIComponents/ArrowDown"));
...
static void ColorChangeNotifyProc(void* param, NotifyInfo* pInfo)
{
	...
	LoadMAXFileIcon(_T("Skin"), hSkinIcons, kBackground, FALSE);
	LoadMAXFileIcon(_T("Skin"), hSkinDisabledIcons, kBackground, TRUE);
	for (int i = 0; i < 12; i++)
	{
...
void BoneTimeChangeCallback::TimeChanged(TimeValue t)
{
	...
	{
		float val = 0.;
		mod->pblock_param->GetValue(skin_local_squash,t, val,  mod->ModeBoneIndex);
		...
		if (mod->ModeBoneEnvelopeSubType<4)
		{
			...
			if ((mod->ModeBoneEnvelopeIndex >= 0) && (mod->ModeBoneEnvelopeIndex < boneData.CrossSectionList.Count()))
				boneData.CrossSectionList[mod->ModeBoneEnvelopeIndex].InnerControl->GetValue(t,&radius,iv);
		}
		else if (mod->ModeBoneEnvelopeSubType<8)
		{
			...
			BoneDataClass &boneData = mod->BoneData[mod->ModeBoneIndex];
			if ((mod->ModeBoneEnvelopeIndex >= 0) && (mod->ModeBoneEnvelopeIndex < boneData.CrossSectionList.Count()))
				boneData.CrossSectionList[mod->ModeBoneEnvelopeIndex].OuterControl->GetValue(t,&radius,iv);
		}
		...
		float oldRadius = 0.;
		mod->pblock_param->GetValue(skin_cross_radius,t,oldRadius);
		if (oldRadius != radius)
...
const TCHAR *BonesDefMod::GetBoneName(int boneIndex)
{
	...
	if (( os.obj->ClassID() == bid) && (boneData.name.Length()) )
	{
		return boneData.name;
	}
	else return boneData.Node->GetName();
}
...
static void ColorsBlendForArrowBitmaps(HBITMAP hBitmap, COLORREF sourceColor, COLORREF destColor)
{
	if(NULL == hBitmap) return;
	BITMAP bmImage;
	GetObject(hBitmap, sizeof(BITMAP), &bmImage);
	int bmImageStride = ((3 * bmImage.bmWidth + 3) & ~3);
	LPBITMAPINFO pBmInfoImage;
	pBmInfoImage = (LPBITMAPINFO)calloc(1, sizeof(BITMAPINFOHEADER) + (bmImageStride * bmImage.bmHeight));
	if(NULL == pBmInfoImage) return;

	HDC dc = GetDC(NULL);

	pBmInfoImage->bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
	pBmInfoImage->bmiHeader.biWidth = bmImage.bmWidth;
	pBmInfoImage->bmiHeader.biHeight = bmImage.bmHeight;
	pBmInfoImage->bmiHeader.biPlanes = 1;
	pBmInfoImage->bmiHeader.biBitCount = 24;
	pBmInfoImage->bmiHeader.biCompression = BI_RGB;
	pBmInfoImage->bmiHeader.biSizeImage = (bmImageStride * bmImage.bmHeight);
	pBmInfoImage->bmiHeader.biXPelsPerMeter = GetDeviceCaps(dc, LOGPIXELSX);
	pBmInfoImage->bmiHeader.biYPelsPerMeter = GetDeviceCaps(dc, LOGPIXELSY);
	pBmInfoImage->bmiHeader.biClrUsed = 0;
	pBmInfoImage->bmiHeader.biClrImportant = 0;

	GetDIBits(dc, hBitmap, 0, bmImage.bmHeight, &pBmInfoImage->bmiColors, pBmInfoImage, DIB_RGB_COLORS);

	HBITMAP hNewImage = CreateCompatibleBitmap(dc, bmImage.bmWidth, bmImage.bmHeight);
	DbgAssert(hNewImage);

	float destR = static_cast<float>(GetRValue(destColor));
	float destG = static_cast<float>(GetGValue(destColor));
	float destB = static_cast<float>(GetBValue(destColor));

	float sourceR = static_cast<float>(GetRValue(sourceColor));
	float sourceG = static_cast<float>(GetGValue(sourceColor));
	float sourceB = static_cast<float>(GetBValue(sourceColor));

	BYTE* pImageBytes = (BYTE*)pBmInfoImage->bmiColors;
	for(int yPos = 0; yPos < pBmInfoImage->bmiHeader.biHeight; ++ yPos)
	{
		for(int xPos = 0; xPos < pBmInfoImage->bmiHeader.biWidth; ++ xPos)
		{
			int iPix = yPos * bmImageStride + xPos * 3;
			if(pImageBytes[iPix] == 0) continue;
			float a = pImageBytes[iPix] / 255.0f;
			float b = 1.0f - a ;

			pImageBytes[iPix] = static_cast<BYTE>(b * destB + a * sourceB);
			pImageBytes[iPix+1] = static_cast<BYTE>(b * destG + a * sourceG);
			pImageBytes[iPix+2] = static_cast<BYTE>(b * destR + a * sourceR);
		}
	}

	SetDIBits(dc, hNewImage, 0, bmImage.bmHeight, &pBmInfoImage->bmiColors,
		pBmInfoImage, DIB_RGB_COLORS);
	ImageList_AddMasked(hArrowImages, hNewImage, RGB(0,0,0));
	DeleteObject(hNewImage);
	ReleaseDC(NULL, dc);
	free(pBmInfoImage);
}

static const int RIGHT_MARGIN_ARROW_BTN = 38;
static const int TOP_MARGIN_ARROW_BTN = 6;
static const int LEFT_MARGIN_BTN_TEXT = 12;
static const int TOP_MARGIN_BTN_TEXT = 5;
static const int DOWN_ARROW_INDEX = 0;
static const int UP_ARROW_INDEX = 1;
static const int TEXT_ARROW_SPACE = 12;
static const int ARROW_IMAGE_WIDTH = 12;
static const int ARROW_IMAGE_HEIGHT = 12;
...

{
	...
	case WM_INITDIALOG:
		{
            ...
            BOOL enableWeightSolver = (MaxSDK::Graphics::IsGpuPresent()) ? TRUE : FALSE;
            EnableWindow(GetDlgItem(hWnd, IDC_WEIGHT_SOLVER), enableWeightSolver);

			for(int i = 0; i < mod->BoneData.Count(); i++)
			{
				mod->BoneData[i].bFilterOut = FALSE;
			}
			mod->RefillListBox();
			...
			if(NULL == hArrowImages)
			{
				const int IMAGE_INIT_COUNT = 0;
				const int IMAGE_GROW_IN_IMAGELIST = 2;
				hArrowImages = ImageList_Create(ARROW_IMAGE_WIDTH, ARROW_IMAGE_HEIGHT, ILC_COLOR24 | ILC_MASK, IMAGE_INIT_COUNT, IMAGE_GROW_IN_IMAGELIST);
			}

			if(NULL == hArrowDownImage)
			{
				hArrowDownImage = LoadBitmap(hInstance, MAKEINTRESOURCE(IDB_DOWN_ARROW_BITMAP));
				ColorsBlendForArrowBitmaps(hArrowDownImage, GetCustSysColor(COLOR_BTNTEXT), GetCustSysColor(COLOR_BTNFACE));
			}

			if(NULL == hArrowUpImage)
			{
				hArrowUpImage = LoadBitmap(hInstance, MAKEINTRESOURCE(IDB_UP_ARROW_BITMAP));
				ColorsBlendForArrowBitmaps(hArrowUpImage, GetCustSysColor(COLOR_BTNTEXT), GetCustSysColor(COLOR_BTNFACE));
			}

			mod->iSortText = GetICustButton(GetDlgItem(hWnd, IDC_SORTBYNAME));
			mod->iSortText->SetType(CBT_PUSH);

			if (mod->sortByIncreace)
			{
				mod->iSortText->SetIconByName(ARROW_UP_ICON);
			}
			else
			{
				mod->iSortText->SetIconByName(ARROW_DOWN_ICON);
			}
			...
			//if
			// (invalid bone id and there are vaid nodes) OR
			// (valid node id and invalid node)
			//then try to find a new bone id
			if(	mod->ModeBoneIndex < 0 && ct > 0 ||
				mod->ModeBoneIndex >= 0 &&
				mod->ModeBoneIndex < mod->BoneData.Count() &&
				mod->BoneData[mod->ModeBoneIndex].Node == nullptr)
			{
				for (int i = 0; i < mod->BoneData.Count(); i++)
				{
			...
			mod->SelectBoneByBoneIndex(mod->ModeBoneIndex);
			...
			if ((ct>0) && mod->IsValidBoneIndex(mod->ModeBoneIndex))
			{
				mod->ResetSelection();
			}
			...
			{
				float val;
				mod->pblock_param->GetValue(skin_local_squash,mod->ip->GetTime(), val, mod->ModeBoneIndex);
			...
			ICustButton *editMask = GetICustButton(GetDlgItem(hWnd, IDC_EDIT_DQMASK));
			if (editMask)
			{
				editMask->SetType(CBT_CHECK);
				editMask->Enable(false);
				ReleaseICustButton(editMask);
			}
			//fillout the combo boxes and set default
			ComboBox_AddString(GetDlgItem(hWnd, IDC_VOXEL_COMBO), GetString(IDS_VOXEL));
			ComboBox_AddString(GetDlgItem(hWnd, IDC_VOXEL_COMBO), GetString(IDS_HEATMAP));
			ComboBox_SetCurSel(GetDlgItem(hWnd, IDC_VOXEL_COMBO), 0);
	...
	case WM_CUSTEDIT_ENTER :
		...
		case IDC_ERADIUS:
			if (Animating())
			{
				Control *c = mod->pblock_param->GetControllerByID(skin_cross_radius);
				float r;
				mod->pblock_param->GetValue(skin_cross_radius,mod->ip->GetTime(),r);
				if (c)
				{
			...
			if (mod->ModeBoneEnvelopeSubType<4)
			{
				...
				if (mod->IsValidBoneIndex(mod->ModeBoneIndex))
				{
					...
					if ((mod->ModeBoneEnvelopeIndex >= 0) && (mod->ModeBoneEnvelopeIndex < boneData.CrossSectionList.Count()))
					{
						boneData.CrossSectionList[mod->ModeBoneEnvelopeIndex].InnerControl->GetValue(mod->currentTime,&inner,iv);
						int tempID = mod->ConvertBoneIndexToBoneID(mod->ModeBoneIndex);
						macroRecorder->FunctionCall(_T("skinOps.setInnerRadius"), 4, 0, mr_reftarg, mod, mr_int, tempID+1, mr_int, mod->ModeBoneEnvelopeIndex+1, mr_float, inner);
					}
				}
			}
			else if (mod->ModeBoneEnvelopeSubType<8)
			{
				...
				if (mod->IsValidBoneIndex(mod->ModeBoneIndex))
				{
					...
					if ((mod->ModeBoneEnvelopeIndex >= 0) && (mod->ModeBoneEnvelopeIndex < boneData.CrossSectionList.Count()))
					{
						boneData.CrossSectionList[mod->ModeBoneEnvelopeIndex].OuterControl->GetValue(mod->currentTime,&outer,iv);
						int tempID = mod->ConvertBoneIndexToBoneID(mod->ModeBoneIndex);
						macroRecorder->FunctionCall(_T("skinOps.setOuterRadius"), 4, 0, mr_reftarg, mod, mr_int, tempID+1, mr_int, mod->ModeBoneEnvelopeIndex+1, mr_float, outer);
					}
				}
			}
			break;
	...
	case CC_SPINNER_CHANGE:
		...
		case IDC_EFFECTSPIN:
			{
				...
				int rsel = SendMessage(GetDlgItem(hWnd,IDC_LIST1),LB_GETCURSEL,0,0);
				int boneIndex = mod->ConvertListIDToBoneIndex(rsel);
				if ( (boneIndex>=0) && (mod->ip && mod->ip->GetSubObjectLevel() == 1) )
				{
					...
					for ( int i = 0; i < objects; i++ )
					{
						...
						mod->SetSelectedVertices(bmd, boneIndex, w);
						if (!update)
		...
		case IDC_SQUASHSPIN:
			{
				...
				if (firstMouse)
				{
					...
					firstMouse = FALSE;
					float q1;
					mod->pblock_param->GetValue(skin_local_squash,mod->ip->GetTime(),q1,mod->ModeBoneIndex);
				...
				mod->pblock_param->GetValue(skin_local_squash,mod->ip->GetTime(),prevQ,mod->ModeBoneIndex);
		...
		case IDC_ERADIUSSPIN:
			float r;
			mod->pblock_param->GetValue(skin_cross_radius,0,r);
			if (Animating())
			{
				Control *c = mod->pblock_param->GetControllerByID(skin_cross_radius);
				float r;
			...
			if (mod->ModeBoneEnvelopeSubType<4)
			{
				if (mod->IsValidBoneIndex(mod->ModeBoneIndex))
				{
					...
					if ((mod->ModeBoneEnvelopeIndex>=0) && (mod->ModeBoneEnvelopeIndex<boneData.CrossSectionList.Count()))
					{
						...
						BOOL animate;
						mod->pblock_advance->GetValue(skin_advance_animatable_envelopes,0,animate);
						if (!animate)
						{
							SuspendAnimate();
							AnimateOff();
						}
						boneData.CrossSectionList[mod->ModeBoneEnvelopeIndex].InnerControl->SetValue(mod->currentTime,&r,TRUE,CTRL_ABSOLUTE);
						if (!animate)
							ResumeAnimate();
						...
						macroRecorder->Enable();
						int tempID = mod->ConvertBoneIndexToBoneID(mod->ModeBoneIndex);
						macroRecorder->FunctionCall(_T("skinOps.setInnerRadius"), 4, 0, mr_reftarg, mod, mr_int, tempID+1, mr_int, mod->ModeBoneEnvelopeIndex+1, mr_float, r);
						macroRecorder->Disable();
					}
				}
			}
			else if (mod->ModeBoneEnvelopeSubType<8)
			{
				if (mod->IsValidBoneIndex(mod->ModeBoneIndex))
				{
					...
					if ((mod->ModeBoneEnvelopeIndex>=0) && (mod->ModeBoneEnvelopeIndex<boneData.CrossSectionList.Count()))
					{
						...
						boneData.CrossSectionList[mod->ModeBoneEnvelopeIndex].OuterControl->SetValue(mod->currentTime,&r,TRUE,CTRL_ABSOLUTE);
						if (!animate)
							ResumeAnimate();
						...
						macroRecorder->Enable();
						int tempID = mod->ConvertBoneIndexToBoneID(mod->ModeBoneIndex);
						macroRecorder->FunctionCall(_T("skinOps.setOuterRadius"), 4, 0, mr_reftarg, mod, mr_index, tempID, mr_index, mod->ModeBoneEnvelopeIndex, mr_float,r);
						macroRecorder->Disable();
					}
				}
			}
	...
	case WM_COMMAND:
		switch (LOWORD(wParam))
		{
		case IDC_WEIGHT_SOLVER:   //process which weight engine to launch
			if (ComboBox_GetCurSel(	GetDlgItem(hWnd,IDC_VOXEL_COMBO)) == 0)
				mod->WeightSolverUI(0);
			else	
				mod->WeightSolverUI(1);
			break;
		case IDC_DQ_ENABLE:
			if (IsDlgButtonChecked(hWnd, IDC_DQ_ENABLE))
				mod->pblock_advance->SetValue(skin_advance_enable_dq_blending,0,TRUE);
			else
				mod->pblock_advance->SetValue(skin_advance_enable_dq_blending,0,FALSE);

			break;
		case IDC_EDIT_DQMASK:
			{
				ICustButton *editMask = GetICustButton(GetDlgItem(hWnd, IDC_EDIT_DQMASK));
				if (editMask)
				{
					bool enableMask = false;
					if (editMask->IsChecked())
						enableMask = true;
					ReleaseICustButton(editMask);
					...
					if (objects > 0)
					{
						for (int i = 0; i < objects; i++)
						{
							BoneModData *bmd = (BoneModData*)mcList[i]->localData;
							if (enableMask)
								bmd->SetDQWeightOverride(enableMask);
							else bmd->SetDQWeightOverride(enableMask);
		...
		case IDC_WEIGHTTOOL:
			if (mod->iWeightTool)
			{
				if (mod->iWeightTool->IsChecked())
				{
					ADPForSamples::ADPSkinEventLogger::LogSkinADPEvent(
							ADPForSamples::ADPSkinEventLogger::SkinWaypointType::SKIN_FEATURE_USAGE,
							ADPForSamples::ADPSkinEventLogger::SkinFeatureUsage_EventType::SKIN_WEIGHT_TOOL);
				}
			}
		...
		case IDC_PAINT:
			if (mod->iPaintButton)
			{
				if (mod->iPaintButton->IsChecked())
				{
					ADPForSamples::ADPSkinEventLogger::EnterPaintWeightMode();
				}
				else
				{
					ADPForSamples::ADPSkinEventLogger::ExitPaintWeightMode();
				}
			}
			mod->StartPaintMode();
		...
		case IDC_EDITENVLOPES:
			if (mod->iEditEnvelopes)
			{
				if (mod->iEditEnvelopes->IsChecked())
				{
					ADPForSamples::ADPSkinEventLogger::EnterEditEnvelopeMode();
				}
				else
				{
					ADPForSamples::ADPSkinEventLogger::ExitEditEnvelopeMode();
				}
			}
			if (mod->ip)
			{
		...
		case IDC_ADD:
			{
				...
				ADPForSamples::ADPSkinEventLogger::sTotalModPoints = 0;
				if (mod->ip)
				{
					ModContextList mcList;
					INodeTab nodes;
		...
		case ID_FALLOFF:
			{
				if (mod->IsValidBoneIndex(mod->ModeBoneIndex))
				{
					...
					macroRecorder->FunctionCall(_T("skinOps.setSelectedBonePropFalloff"), 2,0, mr_reftarg, mod, mr_int, foff+1);
		...
		case ID_PASTE:
			{
				...
				else if (pasteMode == 2)
				{
					...
					int iret = DialogBoxParam(hInstance,MAKEINTRESOURCE(IDD_PASTE_DIALOG), hWnd,PasteDlgProc, (LPARAM)mod);
					if (iret)
					{
						for (int i = 0; i < mod->pasteList.Count(); i++)
						{
							int boneId = mod->pasteList[i];
							macroRecorder->FunctionCall(_T("skinOps.pasteToBone"), 2,0, mr_reftarg, mod, mr_int, boneId+1);
		...
		case ID_ABSOLUTE:
			{
				...
				if (mod->IsValidBoneIndex(mod->ModeBoneIndex))
				{
					...
					macroRecorder->FunctionCall(_T("skinOps.setSelectedBonePropRelative"), 2, 0, mr_reftarg, mod, mr_int, abso);
					mod->NotifyDependents(FOREVER, PART_GEOM, REFMSG_CHANGE);
		...
		case ID_DRAW_ENVELOPE:
			{
				if (mod->IsValidBoneIndex(mod->ModeBoneIndex))
				{
					int disp = mod->iEnvelope->IsChecked();
					if (disp)
					{
					...
					macroRecorder->FunctionCall(_T("skinOps.setSelectedBonePropEnvelopeVisible"), 2,0, mr_reftarg, mod, mr_int, disp);
					mod->NotifyDependents(FOREVER, PART_GEOM, REFMSG_CHANGE);
				}
				break;
			}
		case IDC_REMOVE:
			{
				...
				{
					int tempID = mod->ConvertBoneIndexToBoneID(mod->ModeBoneIndex);
					macroRecorder->FunctionCall(_T("skinOps.removeBone"), 2,0, mr_reftarg, mod, mr_int, tempID+1);
				...
				EnableWindow(GetDlgItem(hWnd,IDC_REMOVE),mod->HasNonNullBones());
				
				break;
			}
		case IDC_SORTBYNAME:
			{
				mod->sortByIncreace = !mod->sortByIncreace;
				if (mod->sortByIncreace)
				{
					mod->iSortText->SetIconByName(ARROW_UP_ICON);
				}
				...
				mod->RefillListBox();
				mod->mParameterListSelection = mod->ConvertBoneIndexToListID(mod->ModeBoneIndex);
				SendMessage(GetDlgItem(hWnd,IDC_LIST1), LB_SETCURSEL, mod->mParameterListSelection, 0);
		...
		case IDC_LIST1:
			{
				if (HIWORD(wParam)==LBN_SELCHANGE) {
					...
					mod->mParameterListSelection = fsel;
					int boneIndex = mod->ConvertListIDToBoneIndex(fsel);
					int tempID = mod->ConvertBoneIndexToBoneID(boneIndex);
					macroRecorder->FunctionCall(_T("skinOps.SelectBone"), 2, 0, mr_reftarg, mod, mr_int, tempID+1);

					mod->cacheValid = TRUE;
					mod->ModeBoneIndex = boneIndex;
					BoneDataClass &boneData = mod->BoneData[mod->ModeBoneIndex];
					if (mod->FilterBones)
					{
		...
		case IDC_WEIGHTABLE:
			{
				ADPForSamples::ADPSkinEventLogger::LogSkinADPEvent(
						ADPForSamples::ADPSkinEventLogger::SkinWaypointType::SKIN_FEATURE_USAGE,
						ADPForSamples::ADPSkinEventLogger::SkinFeatureUsage_EventType::SKIN_WEIGHT_TABLE);
				mod->fnWeightTable();
	...
	case WM_CLOSE:
		{
			if(NULL != hArrowImages)
			{
				ImageList_Destroy(hArrowImages);
				hArrowImages = NULL;
			}
			if(NULL != hArrowDownImage)
			{
				DeleteObject(hArrowDownImage);
				hArrowDownImage = NULL;
			}
			if(NULL != hArrowUpImage)
			{
				DeleteObject(hArrowUpImage);
				hArrowUpImage = NULL;
			}
			EndDialog(hWnd, 0);
			break;
...
static void ReFillListBox(HWND hWnd, BonesDefMod *mod)
{
	...
	for (int i=0; i < mod->dataList.Count(); i++) {
		if (mod->dataList[i] == NULL) {
			TSTR blank(_T("<blank>"));
			SendMessage(GetDlgItem(hWnd,IDC_LIST2), LB_ADDSTRING,0, (LPARAM)(const TCHAR*)blank.data());
		} else {
			SendMessage(GetDlgItem(hWnd,IDC_LIST2), LB_ADDSTRING,0, (LPARAM)(const TCHAR*)mod->dataList[i]->name);
	...
	for (int i=0; i < mod->vertexLoadList.Count(); i++) {
		if (mod->vertexLoadList[i] == NULL) {
			TSTR blank(_T("<blank>"));
			SendMessage(GetDlgItem(hWnd,IDC_LIST_IVERT), LB_ADDSTRING,0, (LPARAM)(const TCHAR*)blank.data());
		} else {
			SendMessage(GetDlgItem(hWnd,IDC_LIST_IVERT), LB_ADDSTRING,0, (LPARAM)(const TCHAR*)mod->vertexLoadList[i]->name);
...
static INT_PTR CALLBACK LoadEnvelopeDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		{
			mod = (BonesDefMod*)lParam;
			DLSetWindowLongPtr(hWnd, lParam);
			hWndResizer = GetDlgItem( hWnd, IDC_RESIZER);	// The resizing "grip" at the lower right corner
			...
			for (int i=0; i < mod->BoneData.Count(); i++)
			{
				...
				if (boneData.Node != NULL) {
					Class_ID bid(BONE_CLASS_ID,0);
					ObjectState os = boneData.Node->EvalWorldState(mod->RefFrame);
			...
			for (int i=0; i < mod->loadBaseNodeData.Count(); i++) {
				SendMessage(GetDlgItem(hWnd,IDC_LIST_CVERT),LB_ADDSTRING,0, (LPARAM)(TCHAR*)mod->loadBaseNodeData[i].node->GetName());
	...
	case WM_COMMAND:
		...
		case IDC_MOVE_UP:
			{
				...
				for (int i = 0; i <selCount;  i++)
				{
					...
					if (moveTo!=0)
					{
						...
						SendMessage(GetDlgItem(hWnd,IDC_LIST2), LB_SETSEL,0,selList[i]);
					}
					selList[i] -= 1;
				}
		...
		case IDC_MOVE_UPVERT:
			{
				int selected = SendMessage(GetDlgItem(hWnd,IDC_LIST_IVERT), LB_GETCURSEL, (WPARAM) 0, (LPARAM) 0);
				...
				if ((selected != 0) && (selected!=-1) )
				{
					selected = SendMessage(GetDlgItem(hWnd,IDC_LIST_IVERT), LB_SETCURSEL, (WPARAM) selected-1, (LPARAM) 0);
				}
				break;
			}
		case IDC_MOVE_DOWNVERT:
			{
				int selected = SendMessage(GetDlgItem(hWnd,IDC_LIST_IVERT), LB_GETCURSEL, (WPARAM) 0, (LPARAM) 0);
				int count = SendMessage(GetDlgItem(hWnd,IDC_LIST_IVERT), LB_GETCOUNT, (WPARAM) 0, (LPARAM) 0);
		...
		case IDC_CREATE_BLANKVERT:
			{
				int insertAt = SendMessage(GetDlgItem(hWnd,IDC_LIST_IVERT), LB_GETCURSEL, (WPARAM) 0, (LPARAM) 0);
		...
		case IDC_MATCH:
			{
				...
				int currentCount= SendMessage(GetDlgItem(hWnd,IDC_LIST1), LB_GETCOUNT,0,0);
				if (currentCount==0) break;
				if (removeCurrentPrefix)
				{
					TCHAR firstString[255];
					SendMessage(GetDlgItem(hWnd,IDC_LIST1), LB_GETTEXT,0, (LPARAM)firstString);
					if (firstString)
					{
						int firstLength = static_cast<int>(_tcslen(firstString));
						for (int i =0; i < firstLength; i++)
						{
							BOOL dif = FALSE;
							TCHAR firstLetter = firstString[i];
							for (int j = 1; j < currentCount; j++)
							{
								TCHAR currentString[255];
								SendMessage(GetDlgItem(hWnd,IDC_LIST1), LB_GETTEXT,j, (LPARAM)currentString);

								TCHAR currentLetter = currentString[i];
				...
				if (removeIncomingPrefix)
				{
					...
					if (firstString)
					{
						int firstLength = static_cast<int>(_tcsclen(firstString));
						for (int i =0; i < firstLength; i++)
						{
							BOOL dif = FALSE;
							TCHAR firstLetter = firstString[i];
				...
				for (int i = 0; i < currentCount; i++)
				{
					...
					SendMessage(GetDlgItem(hWnd,IDC_LIST1), LB_GETTEXT,i, (LPARAM)currentName);
					BOOL hit = FALSE;
...
void CleanDataList(Tab<LoadEnvelopeClass*>& dataList)
{
	int ct = dataList.Count();
	for (int i = 0; i < ct; i++)
	{
		if (dataList[i])
		{
...
void CleanVertexLoadList(Tab<LoadVertexDataClass*>& vertexLoadList)
{
	int ct = vertexLoadList.Count();
	for (int i = 0; i < ct; i++)
	{
		if (vertexLoadList[i])
		{
			if (vertexLoadList[i]->name)
			{
				delete [] vertexLoadList[i]->name;
			}

			int vertexCount = vertexLoadList[i]->vertexData.Count();
			for (int j = 0; j < vertexCount; j++)
			{
				if (vertexLoadList[i]->vertexData[j])
				{
					delete vertexLoadList[i]->vertexData[j];
				}
			}

			int exclusionCount = vertexLoadList[i]->exclusionList.Count();
			for (int j = 0; j < exclusionCount; j++)
			{
				if (vertexLoadList[i]->exclusionList[j])
				{
					delete vertexLoadList[i]->exclusionList[j];
				}
			}

			delete vertexLoadList[i];
			vertexLoadList[i] = NULL;
		}
	}
}

BOOL BonesDefMod::LoadEnvelope(const TCHAR *fname, BOOL asText)
{
	FILE *file = NULL;

	MaxSDK::Util::TextFile::Reader textfile;
	Interface14 *iface = GetCOREInterface14();
	UINT codepage  = iface-> DefaultTextLoadCodePage( );

	if (asText)
	{
		textfile.Open(fname, codepage);
	}
	else file = _tfopen(fname,_T("rb"));

	if (file == NULL && !textfile.IsFileOpen())
	{
		MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), _T("Error loading file, file not found"), _T("Load Error"), MB_OK);
		return FALSE;
	}
	//ver
	TSTR lineRead;
	...
	if (asText)
	{
		lineRead = textfile.ReadLine();
		_stscanf(lineRead,_T("%s %d\n"),tempStr,&ver);
		if(textfile.IsEndOfFile()) ver = -1;
	}
	else
	{
	...
	if ( (ver < 0) || (ver > 14) )
	{
		MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), _T("Error loading file, corrupt file"), _T("Load Error"), MB_OK);
		return FALSE;
	}
	...
	bool loadAsWCHAR = false;
	if(ver >= 4) loadAsWCHAR = true;
	//number bones
	int ct =0;
	if (asText)
	{
	...
	for (int i = 0; i < ct; i++)
	{
		...
		{
			TSTR tempBoneName;
			lineRead = textfile.ReadLine(-1, true);
			tempBoneName = lineRead;
			int first = tempBoneName.first(_T(']'))+1;
			tempBoneName = tempBoneName.remove(0,first+1);
			int l = tempBoneName.Length();
			dataList[i]->name = new TCHAR[l+1];
			_tcscpy_s(dataList[i]->name, (l+1), tempBoneName);
			dataList[i]->name[l] = 0;
		}
		else if(loadAsWCHAR)
		{
		...
		else
		{
			...
			char* temp = new char[fnameLength+1];

			//bone name
			fread(temp, fnameLength, 1, file);
			temp[fnameLength] = 0;

			TSTR nameStr = TSTR::FromCP(codepage, temp);
			fnameLength = nameStr.Length();

			dataList[i]->name = new TCHAR[fnameLength+1];
			_tcscpy_s(dataList[i]->name, (fnameLength + 1), nameStr.data());
		...
		if (nodeCount > 0)
		{
			...
			for (int i =0; i < nodeCount; i++)
			{
				...
				if (asText)
				{
					...
					tempBoneName = tempBoneName.remove(0,first+1);
					int l = tempBoneName.Length();
					vertexLoadList[i]->name = new TCHAR[l+1];
					_tcscpy_s(vertexLoadList[i]->name, (l+1), tempBoneName);
					vertexLoadList[i]->name[l] = 0;
				}
				else if(loadAsWCHAR)
				{
					fread(&fnameLength, sizeof(fnameLength), 1, file);
					...
					{
						CleanDataList(dataList);
						CleanVertexLoadList(vertexLoadList);
						if (file) { fclose(file); }
						return FALSE;
					}
				...
				else
				{
					...
					fnameLength = nameStr.Length();
					vertexLoadList[i]->name = new TCHAR[fnameLength+1];
					_tcscpy_s(vertexLoadList[i]->name, (fnameLength+1), nameStr.data());
					delete [] temp;
				}
				...
				for (int j = 0; j < vertexCount; j++)
				{
					vertexLoadList[i]->vertexData[j] = NULL;
				}

				const BoneVertexMgr aBoneVertexMgr = std::make_shared<BoneVertexDataManager>();
				aBoneVertexMgr->SetVertexDataPtr(&vertexLoadList[i]->vertexData);
				//loop through verts
				for (int j =0; j < vertexCount; j++)
				{
					vertexLoadList[i]->vertexData[j] = new VertexListClass();
					vertexLoadList[i]->vertexData[j]->SetVertexDataManager(j, aBoneVertexMgr);
					if (asText)
					{
						lineRead = textfile.ReadLine();
						_stscanf(lineRead,_T("%s\n"),tempStr);
					...
					{
						lineRead = textfile.ReadLine(-1, true);
						const TCHAR* lineReadPtr = lineRead.data();
						_stscanf(lineReadPtr,_T("%s"),tempStr); //read "vertexWeight"
						if (*lineReadPtr == ' ')
							while(*lineReadPtr && *lineReadPtr == _T(' '))    lineReadPtr++; //strip out leading spaces

						lineReadPtr += _tcslen(tempStr);
						for (int k = 0; k < weightCount; k++)
						{
							_stscanf(lineReadPtr,_T(" %d,%f"),&id,&weight);
							vertexLoadList[i]->vertexData[j]->SetWeightInfo(k,id,weight,weight);
							while(*lineReadPtr && *lineReadPtr != _T(','))	lineReadPtr++; //move to','
							while(*lineReadPtr && *lineReadPtr != _T(' '))	lineReadPtr++; //move to ' '
						}
					}
					else
					...
					if (ver >= 3)
					{
						...
						if (asText) {
							lineRead = textfile.ReadLine(-1, true);
							const TCHAR* lineReadPt = lineRead.data();
							_stscanf(lineReadPt,_T("%s"),tempStr);
							if (*lineReadPt == ' ')
								while(*lineReadPt && *lineReadPt == _T(' '))    lineReadPt++; //strip out leading spaces
							lineReadPt += _tcslen(tempStr);
							for (int k =0; k < weightCount; k++)
							{
								_stscanf(lineReadPt,_T(" %f %d %d %f %f %f %f %f %f"),&u,&curve,&seg,&p.x,&p.y,&p.z,&t.x,&t.y,&t.z);
								vertexLoadList[i]->vertexData[j]->SetWeightSplineInfo(k,u,curve,seg,p,t);
								for(int mm =0; mm < 9; mm++) //go to the next vertexSplineData
								{
									//move to the char
									while(*lineReadPt && *lineReadPt == _T(' '))	lineReadPt++;
									////move to next ' '
									while(*lineReadPt && *lineReadPt != _T(' '))	lineReadPt++;
								}
							}
				...
				if (exCount >0)
				{
					...
					for (int k =0; k < exCount; k++)
					{
						...
						if (asText)
						{
							//lineRead = textfile.ReadLine(); //one dummy line: [exclusionListForBone 0]
							lineRead = textfile.ReadLine();
						}
						...
						if (vertCount > 0)
						{
							...
							if(asText){
								lineRead = textfile.ReadLine();
								const TCHAR* lineReadPtr = lineRead.data();
								_stscanf(lineReadPtr,_T("%s "),tempStr);
								lineReadPtr += _tcslen(tempStr);
								for (int m = 0; m < vertCount; m++)
								{
									int index;
									//move to the next white space
									while(*lineReadPtr && *lineReadPtr != _T(' '))	lineReadPtr++;
									//skip all white space, in case we have several several ' '
									while(*lineReadPtr && *lineReadPtr == _T(' '))	lineReadPtr++;
									_stscanf(lineReadPtr,_T("%d"),&index);
									exList[m] = index;
								}
							}
							else
				...
				//read in DQ info
				if (vertexCount > 0 && loadDQ)
				{
					if (asText)
					{
						lineRead = textfile.ReadLine();
						const TCHAR* lineReadPtr = lineRead.data();
						vertexLoadList[i]->DQWeights.SetCount(vertexCount);
						for (int k = 0; k < vertexCount; k++)
						{
							lineRead = textfile.ReadLine();
							lineReadPtr = lineRead.data();
							float weight = 0;
							_stscanf(lineReadPtr, _T("%f"), &weight);
							vertexLoadList[i]->DQWeights[k] = weight;
						}
					}
					else
					{						
				...
				else
				{
					vertexLoadList[i]->DQWeights.SetCount(vertexCount);
					for (int k = 0; k < vertexCount; k++)
						vertexLoadList[i]->DQWeights[k] = 0.0f;
				}
			}
		}
	}
	...
	if (iret)
	{
		...
		for (int m =0; m < dataList.Count(); m ++)
		{
			...
			{
				//get correct bone
				int boneIndex = ConvertBoneIDToBoneIndex(m);
				...
				if (boneIndex!=-1)
				{
					...
					boneData.FalloffType = dataList[m]->falloffType;
					boneData.flags  = dataList[m]->flags;
					...
					if (pasteEnds)
					{
						...
						boneData.EndPoint1Control->SetValue(currentTime,&dataList[m]->E1,TRUE,CTRL_ABSOLUTE);
						boneData.EndPoint2Control->SetValue(currentTime,&dataList[m]->E2,TRUE,CTRL_ABSOLUTE);
						if(!animate)
							ResumeAnimate();
					}
					if (pasteCross)
					{
						for (int i =(ct-1); i >= 0 ; i--)
							RemoveCrossSection(boneIndex, i);
						for (int i =0; i < dataList[m]->u.Count() ; i++)
						{
							AddCrossSection(boneIndex,dataList[m]->u[i],dataList[m]->inner[i],dataList[m]->outer[i]);
						}
					}
				}
			}
		...
		if (loadVertData || loadExclusionData)
		{
			...
			for (int j = 0; j <  dataList.Count(); j++)
			{
				if (dataList[j])
				{
					...
					boneIDList[id] = ConvertBoneIDToBoneIndex(j);
				}
			}
	...
	TimeValue t,IParamMap2 *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		{
			...
			mod->UpdateGizmoList();
			mod->SelectBoneByListBoxIndex(0);
			if (mod->pblock_gizmos->Count(skin_gizmos_list) > 0)
			{
				...
				gizmo->BeginEditParams(mod->ip, mod->flags, nullptr);
	...
	case WM_COMMAND:
		...
		{
		case IDC_ADD:
			if (0 == SendMessage(GetDlgItem(hWnd, IDC_DEFORMERS), CB_GETCURSEL, 0, 0))
			{
				ADPForSamples::ADPSkinEventLogger::LogSkinADPEvent(
						ADPForSamples::ADPSkinEventLogger::SkinWaypointType::SKIN_FEATURE_USAGE,
						ADPForSamples::ADPSkinEventLogger::SkinFeatureUsage_EventType::SKIN_JOINT_ANGLE_DEFORMER);
			}
			mod->AddGizmo();
		...
		case IDC_DEFORMERS:
			{
				...
				{
					int fsel = SendMessage(GetDlgItem(hWnd,IDC_DEFORMERS), CB_GETCURSEL,0,0);
					macroRecorder->FunctionCall(_T("skinOps.selectGizmoType"), 2, 0, mr_reftarg, mod, mr_int, fsel+1 );
				}
				break;
			}
		case IDC_LIST1:
...
static const int MAX_LENGTH_NAME_PICKER = 1024;

BOOL BonesDefMod::IsNamePickerEmpty() const
{
	ICustEdit *boneName = GetICustEdit(GetDlgItem(hParam, IDC_BONENAME));
	DbgAssert(NULL != boneName);
	if (boneName == nullptr)
		return TRUE;
	TCHAR name[MAX_LENGTH_NAME_PICKER] = {0};
	boneName->GetText(name, MAX_LENGTH_NAME_PICKER);
	ReleaseICustEdit(boneName);
...
void BonesDefMod::ClearNamePicker()
{
	if(!IsNamePickerEmpty())
	{
		ICustEdit *boneName = GetICustEdit(GetDlgItem(hParam,IDC_BONENAME));
		DbgAssert(NULL != boneName);
		if (boneName == nullptr)
		...
		{
			BoneData[i].bFilterOut = FALSE;
		}
		ReleaseICustEdit(boneName);
	}
}

void BonesDefMod::RefreshNamePicker()
{
	if (editing == false)
		return;
	...
	if(!IsNamePickerEmpty())
	{
		ClearNamePicker();
		RefillListBox();
		mParameterListSelection = ConvertBoneIndexToListID(ModeBoneIndex);
		SendMessage(GetDlgItem(hParam, IDC_LIST1), LB_SETCURSEL, mParameterListSelection, 0);
	}
}
...
void BonesDefMod::SelectBoneNamePicker()
{
		...
		for (int i = 0; i < BoneData.Count(); i++)
		{
			if(BoneData[i].Node != NULL)
			{
				if(ModeBoneIndex != i)
				{
					...
					{
						int boneID = ConvertBoneIndexToBoneID(i);
						macroRecorder->FunctionCall(_T("skinOps.SelectBone"), 2, 0, mr_reftarg, this, mr_int, boneID+1);
					}
				}
				else
				{
	...
	if (ct && ct < _countof(name) - 1 && name[ct - 1] != _T('*'))
		_tcscat_s(name, _T("*"));
	...
	for (int i = 0; i < nodeList.Count(); i++)
	{
		if (nodeList[i])
		{
			...
			{
				SelectBone(i);
				int fsel = ConvertBoneIndexToListID(i);
				if (macroRecorder->Enabled())
				{
					int boneID = ConvertBoneIndexToBoneID(i);
					macroRecorder->FunctionCall(_T("skinOps.SelectBone"), 2, 0, mr_reftarg, this, mr_int, boneID+1);
