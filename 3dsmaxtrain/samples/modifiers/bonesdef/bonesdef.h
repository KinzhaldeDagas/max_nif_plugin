...
#include "linshape.h"
#include "ISkin.h"
#include "SkinEngine/ISkinEngine.h"
#include "icurvctl.h"
...
inline const MaxSDK::MaxGuid s_weightTableMenuId{ "5904bbca-89c9-4168-bb5f-a172193b37f4" }; // From Default.mnxb

namespace ADPHelpers {
void HandleADPData(BonesDefMod* mod);
}
...
public:
	...
	Point3 GetInterpCurvePiece3D(int BoneId, int CurveId, int SegId, float distance) const override;
	Point3 GetTangentPiece3D(int BoneId, int CurveId, int SegId, float distance) const override;
private:
	static SRWLOCK mLock;
	BoneModData* mBMD;
...
public:
	...
	BOOL IsAffected(int index) const;
	...
	inline BOOL IsInDeformingList(int index) const
	{
		index = index - start;
...
public:
	...
	virtual int GetNumAssignedBones(int pointIdx) const override;
	...
	//	pointIdx which vertex
	//	boneIdx the ith entry in the bone list
	virtual float GetBoneWeight(int pointIdx, int boneIdx) const override;
	...
	//	pointIdx which vertex
	//	boneIdx the ith entry in the bone list
	virtual int GetSubCurveIndex(int pointIdx, int boneIdx) const override;
	//this returns which sub segment the vertex belongs to
	//	pointIdx which vertex
	//	boneIdx the ith entry in the bone list
	virtual int GetSubSegmentIndex(int pointIdx, int boneIdx) const override;
	//this returns the sub segment U value
	//	pointIdx which vertex
	//	boneIdx the ith entry in the bone list
	virtual float GetSubSegmentDistance(int pointIdx, int boneIdx) const override;
	//this returns the sub segment tangent value
	//	pointIdx which vertex
	//	boneIdx the ith entry in the bone list
	virtual Point3 GetTangent(int pointIdx, int boneIdx) const override;
	//this returns the point on the sub segment
	//	pointIdx which vertex
	//	boneIdx the ith entry in the bone list
	virtual Point3 GetOPoint(int pointIdx, int boneIdx) const override;
	...
	float GetDQBlendWeight(int index) const override;
...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_RB_BONESDEFMOD) : _T("Skin");}
	Class_ID ClassID() { return BONESDEFMOD_CLASS_ID;}
	void BeginEditParams(IObjParam  *ip, ULONG flags,Animatable *prev);
	void EndEditParams(IObjParam *ip,ULONG flags,Animatable *next);
	RefTargetHandle Clone(RemapDir& remap);
	const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_BONESDEFMOD) : _T("Skin");}
...
public:
	...
	{
		ChannelMask channels = GEOM_CHANNEL | TOPO_CHANNEL | SELECT_CHANNEL | SUBSEL_TYPE_CHANNEL;
		// KZ, 3/29/2022: Vert colors (a hidden texmap) are used to display the bone weights in the viewport.
		// They are only used and changed when displaying the envelops, otherwise do not copy them.
		// KZ 8/1/2022: Changed the flags we use to conditionally copy TEXMAP_CHANNEL and VERTCOLOR_CHANNEL.
		// We need to remember if we used the vertex colors on the last evaluation of this mod.
		if (editing || copyVertColorChannel)
		{
			return TEXMAP_CHANNEL | VERTCOLOR_CHANNEL | channels;
		}
		return channels;
	}
	ChannelMask ChannelsChanged() override
	{
		ChannelMask channels = GEOM_CHANNEL;
		// KZ, 3/29/2022: Vert colors (a hidden texmap) are used to display the bone weights in the viewport.
		// They are only used and changed when displaying the envelops, otherwise do not copy them.
		// KZ 8/1/2022: Changed the flags we use to conditionally copy TEXMAP_CHANNEL and VERTCOLOR_CHANNEL.
		// We need to remember if we used the vertex colors on the last evaluation of this mod.
...
//		DbgAssert(res);
	...
	void UpdateTMCacheTable(BoneModData* bmd, TimeValue t, BOOL forceCompleteUpdate = TRUE)
	{
		Interval valid = FOREVER;
		UpdateTMCacheTable(bmd, t, valid, forceCompleteUpdate);
	}
	...
	float RetrieveNormalizedWeight(const BoneModData *bmd, int vid, int bid) const;
	...
	Point3 VertexAnimation(TimeValue t, const BoneModData * bmd, int vertex, int bone, Point3 p);
	...
	ULONG flags;				//flags to used to add the gizmo rollouts to the UI
	...
	BOOL GetDebugMode() {
		...
		{
			pblock_weighttable->GetValue(skin_wt_debugmode,0,value);
		}
		return value;
	}
...
private:
	...
	// KZ 8/1/2022: Did the last evaluation of this mod use vertex colors.
	// This is a good playback performance boost.
	bool copyVertColorChannel = false;
	...
	int mTotalValidBonesCountCache_APD; // Member variable to cache the total valid bones count. Changes trigger ADP logging of skin statistics.
};
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Skin"); }
