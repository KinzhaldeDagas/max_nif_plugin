...
public:
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("GizmoJointClass"); }
	...
	BOOL GetToolTip(int iButton, TSTR& ToolTip, ICurveCtl* pCCtl) override;

	// gizmo interface methods
	BOOL IsEnabled() const override; // returns whether the gizmo is active
	BOOL IsVolumeBased() const override; // returns whether the gizmo is volume based
	...
	BOOL IsInVolume(Point3 p, Matrix3 tm) const override;
	...
	RefTargetHandle Clone(RemapDir& remap)
	{
		...
		mod->ReplaceReference(0, remap.CloneRef(pblock_gizmo_data));
		mod->gizmoTm = gizmoTm;
	...
	void PreDeformSetup(TimeValue t) override;
	void PostDeformSetup(TimeValue t) override;
	...
	Point3 DeformPoint(TimeValue t, int index, Point3 initialP, Point3 p, Matrix3 tm) const override;
	...
	//  I modified this code to make it work slightly differently by passing a spline
	...
	Point3 Map(int ii, Point3 p) const;
	Point3 InterpSpline2(float u, const Point3* knot) const;
	...
	Matrix3 inverseParentTm0, // the inverse of the initial parent tm so that converts from  world space to parent space
			parentTmT; // the matrix of the parent bone at the current time, so the converts from parent space to world
					   // space
	...
	Matrix3 cacheTm; // gizmo space to world space then into initial parent space and then back into world space using
					 // the current parent tm
...
private:
	bool mPreDeformCacheValid = false;
	bool mIsEnabledCache = false;
	bool mIsVolumeBasedCache = false;
	float mTwistAngleCache = 0.f;
};
