...
void WeightRestore::Restore(int isUndo)
{
	...
	// During paint strokes we do a restore of an un-"Accepted" WeightRestore.
	// We do not want to call this update in that case so make sure isUndo == 1.
	if (isUndo)
	{
		if (mod->iPaintButton != nullptr && mod->iPaintButton->IsChecked() && mod->pPainterInterface &&
				mod->pPainterInterface->InPaintMode())
		{
			mod->pPainterInterface->UpdateMeshes(TRUE);
		}
	}
}
...
TSTR WeightRestore::Description() { return TSTR(GetString(IDS_PW_EDIT_WEIGHT)); }
...
void UpdateSquashUIRestore::Restore(int isUndo)
{
	...
	if (isUndo)
	{
		mod->pblock_param->GetValue(skin_local_squash,mod->ip->GetTime(),rval,index);
	}
