...
//returns the influence of the nth bone affecting that vertex
def_struct_primitive_debug_ok ( getVertexWeight,skinOps,    "GetVertexWeight" );
// skinops.GetVertexBoneIDsAndWeights vertexid 
// returns an array containing array of bone ids and array of influences of those bones affecting that vertex
// these arrays can be modified and then passed to SetVertexWeights
def_struct_primitive_debug_ok(getVertexBoneIDsAndWeights, skinOps, "GetVertexBoneIDsAndWeights");
...
def_struct_primitive (isInPaintWeightsMode, skinOps, "isInPaintWeightsMode");
...
namespace {
int isValidSkinNodeInfluencer(BonesDefMod* eo, INode* node)
{
	TCHAR name1[200];
	_tcscpy_s(name1, node->GetName());
	...
	else
	{
		...
		for (int i = 0; i < eo->BoneData.Count(); i++)
		{
			...
			if (boneData.Node)
			{
				...
				if ((node == boneData.Node) && (os.obj->ClassID() != bid))
					return FALSE;
			}
		}
		...
		if (os.obj->SuperClassID() == SHAPE_CLASS_ID)
		{
			...
			if (pathOb->NumberOfCurves(GetCOREInterface()->GetTime()) == 0)
				return FALSE;
		}
	}
...
{
	check_arg_count_with_keys(enableDQOverrideWeighting, 2, count);
...
{
	check_arg_count_with_keys(getVertexDQWeight, 2, count);
...
{
	check_arg_count_with_keys(ringSelection, 1, count);
...
{
	check_arg_count_with_keys(loopSelection, 1, count);
...
{
	check_arg_count_with_keys(shrinkSelection, 1, count);
...
{
	check_arg_count_with_keys(selectParent, 1, count);
...
Value* setSelectedCrossSection_cf(Value** arg_list, int count)
{
	...
	if (objects > 0)
	{
		...
		BOOL animate = FALSE;
		bmod->pblock_advance->GetValue(skin_advance_animatable_envelopes,0,animate);
		if (!animate)
		{
			SuspendAnimate();
			AnimateOff();
...
{
	check_arg_count_with_keys(updateMirror, 1, count);
...
Value* isInPaintWeightsMode_cf(Value** arg_list, int count)
{
	check_arg_count_with_keys(isInPaintWeightsMode, 1, count);

	BonesDefModGuard guard(arg_list[0], key_arg_or_default(node, NULL), NULL, NULL);
	BonesDefMod* bmod = guard.GetMod();

	if (bmod->iPaintButton != nullptr && bmod->iPaintButton->IsChecked() && bmod->pPainterInterface &&
			bmod->pPainterInterface->InPaintMode())
	{
...
Value* addBone_cf(Value** arg_list, int count)
{
    ...
    if (isValidSkinNodeInfluencer(bmod, node))
	{
...
Value* getBoneWeights_cf(Value** arg_list, int count)
{
	...
	check_arg_count_with_keys(getBoneWeights, 2, count);
...
Value* setBoneWeights_cf(Value** arg_list, int count)
{
	...
	check_arg_count_with_keys(setBoneWeights, 4, count);
...
Value* getVertexBoneIDsAndWeights_cf(Value** arg_list, int count)
{
	// <#{boneID array>,<weight array>}>skinops.getVertexBoneIDsAndWeights $.modifiers[#Skin] vertexID [node:<node>]
	check_arg_count_with_keys(getVertexBoneIDsAndWeights, 2, count);
	...
	if (objects != 0)
	{
		...
		if ((vindex < 0) || (vindex >= bmd->VertexData.Count()))
			throw RuntimeError(GetString(IDS_PW_EXCEEDED_VERTEX_COUNT), arg_list[0]);
		int weightCount = bmd->VertexData[vindex]->WeightCount();
		vl.res = new Array(2);
		Array* boneIDs = new Array(weightCount);
		vl.res->append(boneIDs);
		Array * weights = new Array(weightCount);
		vl.res->append(weights);
		for (int subindex = 0; subindex < weightCount; ++subindex)
		{
			float weight = bmod->RetrieveNormalizedWeight(bmd, vindex, subindex);
			weights->append(Float::intern(weight));
			int boneIndex = bmd->VertexData[vindex]->GetBoneIndex(subindex);
			int boneID = bmod->ConvertBoneIndexToBoneID(boneIndex) + 1;
			boneIDs->append(Integer::intern(boneID));
		}
	}
...
void BonesDefMod::AddBone(INode *node, BOOL update, INodeTab* modNodes)
{
	...
	float staticOuter = 0.;
	float staticInner = 0.;
	pblock_param->GetValue(skin_initial_staticenvelope,0,staticEnvelope);
	pblock_param->GetValue(skin_initial_envelope_innerpercent,0,staticInnerPercent);
	pblock_param->GetValue(skin_initial_envelope_outerpercent,0,staticOuterPercent);
	pblock_param->GetValue(skin_initial_envelope_inner,0,staticInner);
	pblock_param->GetValue(skin_initial_envelope_outer,0,staticOuter);

	BOOL mirror = FALSE;
	pblock_mirror->GetValue(skin_mirrorenabled,0,mirror);
	if (mirror)
...
{
	if (boneNode == NULL) return Matrix3{};
...
{
	return isValidSkinNodeInfluencer(eo, node);
}	
