...
class SkinToolsFunctionPublish : public FPStaticInterface {
	public:
		...
		BEGIN_FUNCTION_MAP			
			...
			VFN_7(kPasteNoDialog,  PasteNoDialog, TYPE_BOOL,TYPE_BOOL,TYPE_BOOL,TYPE_BOOL,TYPE_BOOL,TYPE_FLOAT,TYPE_INT)
		...
		void PasteNoDialog(BOOL matchByName,
						   BOOL removeTargetPrefix, BOOL removeTargetSuffix,
						   BOOL removeSourcePrefix, BOOL removeSourceSuffix,
						   float threshold,
						   int interpType);
...
static SkinToolsFunctionPublish theSkinToolsFunctionPublish(
	...
	SkinToolsFunctionPublish::kPasteNoDialog, _T("ImportSkinDataNoDialog"), -1, TYPE_VOID, 0, 7,		
		_T("matchByName"), -1, TYPE_BOOL,
		_T("removeTargetPrefix"), -1, TYPE_BOOL,
		_T("removeTargetSuffix"), -1, TYPE_BOOL,
		_T("removeSourcePrefix"), -1, TYPE_BOOL,
		_T("removeSourceSuffix"), -1, TYPE_BOOL,
		_T("threshold"), -1, TYPE_FLOAT,
		_T("interpolationType"), -1, TYPE_INT,
...
class SkinToolsClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("SkinUtilities"); }
...
static INT_PTR CALLBACK SkinToolsDlgProc(
...
/*				if (os.obj->CanConvertToType(triObjectClassID)) 
				{	
					...
					if (oldObj==tobj) 
					{
						obj = (Object*)CloneRefHierarchy(tobj);
					} 
					else 
					{
						obj = tobj;
				...
				{*/
					obj = (Object*)CloneRefHierarchy(os.obj);
//				}
	...
	// Name the node and make the name unique.
				TSTR name;
				name.printf(_T("SkinData_%s"),node->GetName());
...
//name the channels
				...
				for (int i = 0; i < boneList.Count(); i++)
				{
					TSTR key;
					TSTR tname;
					tname.printf(_T("SkinWeight:%s"),boneList[i]->GetOriginalName());
					key.printf(_T("MapChannel:%d(%d)"),boneList[i]->mapID/3,boneList[i]->mapID%3);
...
void SkinTools::PasteData(BOOL silent)
{
	Interface *ip = GetCOREInterface();
...
/*
						for (int i = 0; i < msh->numVerts; i++)
						{
							DebugPrint(_T("vert %d "), i);
							for (int j = 0; j < boneList.Count(); j++)
							{
								float w = boneList[j]->weights[i].vertWeight;
								if (w != 0.0f)
								{
									DebugPrint(_T(" %s:%f,"), boneList[j]->name,w);
								}
...
void SkinTools::PasteData (INode *snode,INode *tnode, BOOL silent)
{
	Interface *ip = GetCOREInterface();
	TimeValue t = ip->GetTime();
//get our modifier
...
//DebugPrint(_T("match %s\n"),name);							
		...
		//loop through the keys
				...
				if (silent)
				{
					...
					RemoveTargPrefix(removeTargetPrefix);
					RemoveTargSufix(removeTargetSuffix);

					RemoveSourcePrefix(removeSourcePrefix);
					RemoveSourceSufix(removeSourceSuffix);
					...
					else
					{
						...
						for (int i = 0; i < targBoneList.Count(); i++)
						{
							if ( i < pasteData.liveSourceBones.Count())
							{
								int index = pasteData.liveSourceBones[i];
								targBoneList[i]->matchingBones.Append(1,&index);
							}
						}
					}
...
void SkinTools::FilloutListBox()
{
	...
	for (int i = 0; i < pasteData.liveTargetBones.Count(); i++)
	{
		...
		TSTR name;
		name.printf(_T("%s : "),targBoneList[index]->GetDisplayName());
		...
		SendMessage(GetDlgItem(floaterHWND,IDC_TARGETLIST),LB_ADDSTRING,0,(LPARAM)(const TCHAR*)name);
	}

	for (int i = 0; i < pasteData.liveSourceBones.Count(); i++)
	{
		int index = pasteData.liveSourceBones[i];
		SendMessage(GetDlgItem(floaterHWND,IDC_SOURCELIST),LB_ADDSTRING,0,(LPARAM)boneList[index]->GetDisplayName().data());
	}


	for (int i = 0; i < pasteData.deadTargetBones.Count(); i++)
	{
		int index = pasteData.deadTargetBones[i];
		SendMessage(GetDlgItem(floaterHWND,IDC_DEADTARGLIST),LB_ADDSTRING,0,(LPARAM)targBoneList[index]->GetDisplayName().data());
	}


	for (int i = 0; i < pasteData.deadSourceBones.Count(); i++)
	{
		int index = pasteData.deadSourceBones[i];
		SendMessage(GetDlgItem(floaterHWND,IDC_DEADSOURCELIST),LB_ADDSTRING,0,(LPARAM)boneList[index]->GetDisplayName().data());
	}
...
//get our list selection
	...
	for (int i=pasteData.liveTargetBones.Count(); i >=0; i--)
	{
		if (delList[i])
...
//get our list selection
	...
	for (int i=pasteData.deadTargetBones.Count(); i >=0; i--)
	{
		if (delList[i])
...
//get our list selection
	...
	for (int i=pasteData.deadSourceBones.Count(); i >=0; i--)
	{
		if (delList[i])
	...
	//loop through the targets
		...
		//see if all names have it
			...
			for (int j = 1; j < targBoneList.Count(); j++)
			{
				...
				int cnameIdx = cname.Length() - ct -1;
				int firstNameIdx = firstName.Length() - ct -1;
				
				if ( cnameIdx<0    || firstNameIdx<0 || cname[cnameIdx] != firstName[firstNameIdx] )
				{
					same = FALSE;
					j = targBoneList.Count();
				}
...
void SkinToolsFunctionPublish::PasteNoDialog(BOOL matchByName,
				   BOOL removeTargetPrefix, BOOL removeTargetSuffix,
				   BOOL removeSourcePrefix, BOOL removeSourceSuffix,
				   float threshold,
				   int interpType)
{
	theSkinTools.threshold = threshold;	
	theSkinTools.matchType = interpType;
	theSkinTools.matchByName = matchByName;
	
	
	theSkinTools.removeSourcePrefix = removeSourcePrefix;
	theSkinTools.removeSourceSuffix = removeSourceSuffix;
	theSkinTools.removeTargetPrefix = removeTargetPrefix;
	theSkinTools.removeTargetSuffix = removeTargetSuffix;


	theSkinTools.PasteData(TRUE);
