...
class SkinTools : public UtilityObj {
	public:
		...
		void PasteData(BOOL silent = FALSE);
		void PasteData (INode *snode,INode *tnode, BOOL silent = FALSE);
...
//		void MatchByPoints(Mesh *msh, ISkinImportData *skinImp, INode *snode, INode *tnode);
		...
		BOOL matchByName;
		BOOL removeTargetPrefix;
		BOOL removeTargetSuffix;
		BOOL removeSourcePrefix;
		BOOL removeSourceSuffix;
...
class PasteSkinWeights : public Modifier {
	public:
		...
		// From Animatable
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_PASTESKINWEIGHTS) : _T("Paste Skin Weights"); }

		//From Modifier
		ChannelMask ChannelsUsed()  { return GEOM_CHANNEL|TOPO_CHANNEL|VERTCOLOR_CHANNEL|TEXMAP_CHANNEL; }
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_PASTESKINWEIGHTS) : _T("Paste Skin Weights");}
