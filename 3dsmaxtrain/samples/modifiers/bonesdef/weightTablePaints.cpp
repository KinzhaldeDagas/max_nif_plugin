...
void BonesDefMod::PaintAttribList(BOOL updateWeightTool)
{
	if (updateDialogs)
	{
...
void WeightTableWindow::PaintCellName(HDC hdc, int x, int y,int width, BOOL sel, int justify, TSTR name)
{
	...
	{
		SIZE strSize;
		DLGetTextExtent(hdc, (LPCTSTR)name, name.Length(), (LPSIZE)&strSize); 
...

{
	...
	if (indeterminant)
		weightStr.printf(_T("-"));
	else weightStr.printf(_T("%0.3f"),value);

	SIZE strSize;
	DLGetTextExtent(hdc, (LPCTSTR)weightStr, weightStr.Length(), (LPSIZE)&strSize); 
...
void WeightTableWindow::PaintCellAttribute(HDC hdc, int x, int y, BOOL sel, int state)
{
	...
	if (state == STATE_CHECKED)
		stateStr.printf(_T("X"));
	else if (state == STATE_INDETERMIANT)
		stateStr.printf(_T("-"));
	else stateStr.printf(_T(" "));

	int offset;
	DLGetTextExtent(hdc, (LPCTSTR)stateStr, stateStr.Length(), (LPSIZE)&strSize); 
	if (GetFlipFlopUI())
...
void WeightTableWindow::PaintNameListLabel()
{
	...
	if (GetFlipFlopUI())
	{
		vertName.printf(_T("%s"),GetString(IDS_PW_BONEID));
		PaintCellName(hdc,x,y,buttonWidth,FALSE,TEXT_LEFT_JUSTIFIED,vertName);
	}
	else
	{
		vertName.printf(_T("%s"),GetString(IDS_PW_VERTEXID));
...
void WeightTableWindow::PaintNameList()
{
	...
	if (GetFlipFlopUI())
	{
		...
		for (int i = 0; i < numberOfColumns; i++)
		{
			if ((ct >=0) && (ct < vertexPtrList.Count()))
			{
				...
				{
					if (GetReduceLabels())
						vertName.printf(_T("#%d"),vertexPtrList[ct].index,vertexPtrList[ct].node->GetName());
					else vertName.printf(_T("#%d Vert"),vertexPtrList[ct].index,vertexPtrList[ct].node->GetName());
				}
				else
				{
					if (GetReduceLabels())
						vertName.printf(_T("#%d(%s)"),vertexPtrList[ct].index,vertexPtrList[ct].node->GetName());
					else vertName.printf(_T("#%d Vert(%s)"),vertexPtrList[ct].index,vertexPtrList[ct].node->GetName());
				}
	...
	else
	{
		...
		for (int i = 0; i < numberOfRows; i++)
		{
			if ((ct >=0) && (ct < vertexPtrList.Count()))
			{
				vertName.printf(_T("#%d Vert(%s)"),vertexPtrList[ct].index,vertexPtrList[ct].node->GetName());
				if (modDataList.Count() == 1)
				{
					if (GetReduceLabels())
						vertName.printf(_T("#%d"),vertexPtrList[ct].index,vertexPtrList[ct].node->GetName());
...
void WeightTableWindow::PaintAttribListLabel()
{
	...
	if (GetFlipFlopUI())
	{
		...
		y = 0;
		TSTR selectedStr;
		selectedStr.printf(_T("%s"),GetString(IDS_SELECTED));

		PaintCellName(hdc,  x, y, buttonWidth, FALSE, TEXT_LEFT_JUSTIFIED, selectedStr);

		y += textHeight;
		selectedStr.printf(_T("%s"),GetString(IDS_MODIFIED));
		PaintCellName(hdc,  x, y, buttonWidth, FALSE, TEXT_LEFT_JUSTIFIED, selectedStr);

		y += textHeight;
		selectedStr.printf(_T("%s"),GetString(IDS_PW_NORMALIZED));
		PaintCellName(hdc,  x, y, buttonWidth, FALSE, TEXT_LEFT_JUSTIFIED, selectedStr);

		y += textHeight;
		selectedStr.printf(_T("%s"),GetString(IDS_PW_RIGID));
		PaintCellName(hdc,  x, y, buttonWidth, FALSE, TEXT_LEFT_JUSTIFIED, selectedStr);

		y += textHeight;
		selectedStr.printf(_T("%s"),GetString(IDS_PW_RIGIDHANDLE));
		PaintCellName(hdc,  x, y, buttonWidth, FALSE, TEXT_LEFT_JUSTIFIED, selectedStr);

		/*		y += textHeight;
		selectedStr.printf(_T(" "));
	...
	else
	{
		...
		TSTR selectedStr;
		if (shortenLabel)
			selectedStr.printf(_T("%s"),GetString(IDS_PW_SELECTEDSHORT));
		else selectedStr.printf(_T("%s"),GetString(IDS_SELECTED));

		PaintCellNameVertically(hdc, x, 0, height, TEXT_LEFT_JUSTIFIED, selectedStr);

		x += textHeight;
		if (shortenLabel)
			selectedStr.printf(_T("%s"),GetString(IDS_PW_MODIFIEDSHORT));
		else selectedStr.printf(_T("%s"),GetString(IDS_MODIFIED));
		PaintCellNameVertically(hdc, x, 0, height, TEXT_LEFT_JUSTIFIED, selectedStr);

		x += textHeight;
		if (shortenLabel)
			selectedStr.printf(_T("%s"),GetString(IDS_PW_NORMALIZEDSHORT));
		else selectedStr.printf(_T("%s"),GetString(IDS_PW_NORMALIZED));
		PaintCellNameVertically(hdc, x, 0, height, TEXT_LEFT_JUSTIFIED, selectedStr);

		x += textHeight;
		if (shortenLabel)
			selectedStr.printf(_T("%s"),GetString(IDS_PW_RIGIDSHORT));
		else selectedStr.printf(_T("%s"),GetString(IDS_PW_RIGID));
		PaintCellNameVertically(hdc, x, 0, height, TEXT_LEFT_JUSTIFIED, selectedStr);

		x += textHeight;
		if (shortenLabel)
			selectedStr.printf(_T("%s"),GetString(IDS_PW_RIGIDHANDLESHORT));
		else selectedStr.printf(_T("%s"),GetString(IDS_PW_RIGIDHANDLE));
		PaintCellNameVertically(hdc, x, 0, height, TEXT_LEFT_JUSTIFIED, selectedStr);
...
void WeightTableWindow::PaintAttribList()
{
	...
	SIZE strSize;
	TSTR hString(_T("H"));
	DLGetTextExtent(hdc, (LPCTSTR)hString, hString.Length(), (LPSIZE)&strSize); 
	int th = strSize.cy-4;
	...
	if (GetFlipFlopUI())
	{
		...
		for (int j = 0; j < numberOfRows; j++)
		{
			...
			if ((j+firstRow) < activeBones.Count())
			{
				...
				{
					boneName.printf(_T("%s"),mod->BoneData[currentBone].Node->GetName());
		...
		for (int i = 0; i < numberOfColumns; i++)
		{
			...
			for (int j = 0; j < numberOfRows; j++)
			{
				...
				if ( (start >=0) && (start < vertexPtrList.Count()) && ((j+firstRow)<activeBones.Count()))
				{
					...
					float w = 0.0f;
					FixDQOverrideGuard dqOverride(vertexPtrList[start].vertexData);
					for (int k = 0; k < vertexPtrList[start].vertexData->WeightCount(); k++)
					{
						if (vertexPtrList[start].vertexData->GetBoneIndex(k) == currentBone)
						{

							if (vertexPtrList[start].vertexData->IsModified())
								w = vertexPtrList[start].vertexData->GetWeight(k);
							else w = vertexPtrList[start].vertexData->GetNormalizedWeight(k);
							influenced = TRUE;
						}
					}
					BOOL indet = TRUE;
