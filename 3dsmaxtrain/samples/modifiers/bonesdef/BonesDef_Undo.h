...
public:
	...
	int undoBoneIndex, redoBoneIndex;
...
public:
	...
	BOOL updateView;
	BOOL mUndoDQOverride; //undo now has to be aware whether the user is editing skin weights or the DQ blend weights
	BOOL mRedoDQOverride;
...
public:
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("RemoveGizmoRestore"); }
...
//this is hold record for updating selected bone moved
class SelectBoneMoveRestore : public RestoreObj {
public:
	BonesDefMod *mod;
	SelectBoneMoveRestore(BonesDefMod *bmod);
	void Restore(int isUndo);
	void Redo();
	void EndHold();
	TSTR Description();
