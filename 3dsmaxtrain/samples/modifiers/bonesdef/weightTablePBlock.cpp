...
int WeightTableWindow::GetActiveSet()
	...
	if ( (mod) && (mod->pblock_weighttable))
		{
		mod->pblock_weighttable->GetValue(skin_wt_currentvertexset,0,value);
		}
	return value;
	}
...
void WeightTableWindow::SetActiveSet(int set)
	...
	if ( (mod) && (mod->pblock_weighttable))
		{
		int cSet;
		mod->pblock_weighttable->GetValue(skin_wt_currentvertexset,0,cSet);
		if (set != cSet)
			{
			mod->pblock_weighttable->SetValue(skin_wt_currentvertexset,0,set);

			BOOL temp;			
			mod->pblock_weighttable->GetValue(skin_wt_showaffectbones,0,temp);
...
BOOL WeightTableWindow::GetJBMethod()
	...
	if ( (mod) && (mod->pblock_weighttable))
		{
		mod->pblock_weighttable->GetValue(skin_wt_jbuimethod,0,value);
		}
	return value;
	}
