...
//	TSTR name;
	void GetName(TSTR &vertName)
		{
		vertName.printf(_T("#%d Vertex (%s)"),index,node->GetName());
...
//ViewWindow methods
		const TCHAR *GetName();
		ViewWindowMetaData GetMetaData() { return ViewWindowMetaData(ViewWindowMetaData::NOT_UNIQUE); }
		Class_ID ClassID() const { return Class_ID(0x49ae57a7, 0x31ee60f8);}
...
//get sme intial sizes
		...
		void ResetFont()
			...
			DeleteObject(hFixedFontBold);
			hFixedFontBold = CreateFont(GetFontSize(),0,0,0,0,0,0,0,GetUIFontCharset(),0,0,PROOF_QUALITY, VARIABLE_PITCH  | FF_SWISS, _T(""));
...
//ActionItem methods
		virtual BOOL IsChecked()						{if (wTable)
		...
		virtual BOOL ExecuteAction()					{if (wTable)
													return wTable->WtExecute(id);
												else return FALSE;
												}
		virtual int GetId()								{return id;}
		virtual BOOL IsItemVisible()					{if (wTable)
													return TRUE;
												else return FALSE;}
		virtual BOOL IsEnabled()					{if (wTable)
...
class CustomListRestore : public RestoreObj {
	public:
		...
		~CustomListRestore()
			...
			for (int i = 0; i < rcustomLists.Count();i++)
				{
		...
		TSTR Description() { return TSTR(GetString(IDS_PW_SELECT)); }
	};


static int actionCount = 25;
static int actionEdit = 0;
static int actionSets = 6;
static int actionOption = 8;
static int actionOptionEnd = 15;

static int actionIDs[] = {
							IDC_COPY,
							IDC_PASTE,
							IDC_REMOVE_ZERO_WEIGHTS,
							IDC_SELECTALL,
							IDC_SELECTNONE,
							IDC_SELECTINVERT,
