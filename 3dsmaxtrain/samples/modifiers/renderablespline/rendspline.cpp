...
class RendSpline : public Modifier, public RealWorldMapSizeInterface  {
	...
	protected:
		...
		void OnSetWidthLength(HWND hWnd, ParamID lengthID, ParamID widthID, TimeValue t, BOOL widthSet);
		void OnSetAspect(HWND hWnd, TimeValue t, ParamID widthID, ParamID lengthID);
	...
	public:
		...
		// From Animatable
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_CLASS_NAME) : _T("Renderable Spline"); }
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_CLASS_NAME) : _T("Renderable Spline");}
...
public:
		...
		// Need load method to fill-in appropriate params for older files
...
class RendSplineClassDesc : public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Renderable Spline"); }
	SClass_ID		SuperClassID() { return OSM_CLASS_ID; }
	Class_ID		ClassID() { return RendSpline_CLASS_ID; }
...
// Versions
#define RENDSPLINE_MOD_FILE_NEPTUNE 1	// After adding twuist correction and capping options

static ParamBlockDesc2 rendspline_param_blk ( rendspline_params, _T("params"),  P_VERSION, &RendSplineDesc, 
	P_VERSION + P_AUTO_CONSTRUCT + P_AUTO_UI,
	// Version
	RENDSPLINE_MOD_FILE_NEPTUNE,
	PBLOCK_REF,
	//rollout
...
//		p_ui, 			TYPE_SPINNER,		EDITTYPE_UNIVERSE, IDC_THICKNESS,	IDC_THICKSPIN, 0.1f, 
	...
	rnd_v2_autosmooth, 		_T("Autosmooth"), TYPE_BOOL, P_RESET_DEFAULT,	IDS_AUTOSMOOTH, 	
		p_default, 		TRUE, 
		p_ui, 			TYPE_SINGLECHECKBOX, IDC_AUTOSMOOTH, 
		p_accessor,		&updateUI,
		p_end, 
	...
	rnd_twist_correction, _T("twist_correction"), TYPE_BOOL, P_RESET_DEFAULT, 0,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_TWIST_CORRECTION,
		p_accessor, &updateUI,
		p_end,

	rnd_cap, _T("cap"), TYPE_BOOL, P_RESET_DEFAULT, 0,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_CAP,
		p_accessor, &updateUI,
		p_end,

	rnd_quad_cap, _T("quad_cap"), TYPE_BOOL, P_RESET_DEFAULT, 0,
		p_default, FALSE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_QUAD_CAP,
		p_accessor, &updateUI,
		p_end,

	rnd_cap_segments, _T("cap_segments"), TYPE_INT, P_RESET_DEFAULT | P_ANIMATABLE, IDS_CAP_SEGMENTS,
		p_default, 1,
		p_range, 1, 200,
		p_ui, TYPE_SPINNER, EDITTYPE_POS_INT, IDC_CAPSEGS, IDC_CAPSEGS_SPIN, 0.1,
		p_accessor, &updateUI,
		p_end,

	rnd_sphere_cap, _T("sphere_cap"), TYPE_FLOAT, P_RESET_DEFAULT | P_ANIMATABLE, IDS_SPHERE_CAP,
		p_default, 0.0f,
		p_range, 0.0f, 1.0f,
		p_ui, TYPE_SPINNER, EDITTYPE_POS_FLOAT, IDC_CAPSPHERE, IDC_CAPSPHERE_SPIN, 0.01,
		p_accessor, &updateUI,
		p_end,
...
void RendSplineParamsMapDlgProc::EnableButtons(HWND hWnd, TimeValue t, BOOL display, BOOL useView)
{
	...
	// Capping controls
	BOOL cap = mod->pblock->GetInt(rnd_cap) ? TRUE : FALSE;
	EnableWindow(GetDlgItem(hWnd, IDC_CAPPING_OPTIONS), cap);
	EnableWindow(GetDlgItem(hWnd, IDC_QUAD_CAP), cap);

	BOOL quadCap = (cap && (mod->pblock->GetInt(rnd_quad_cap) ? TRUE : FALSE));
	EnableWindow(GetDlgItem(hWnd, IDC_CAP_SEGMENTS_LABEL), quadCap);
	EnableWindow(GetDlgItem(hWnd, IDC_SPHERE_LABEL), quadCap);
	ISpinnerControl* spin = GetISpinner(GetDlgItem(hWnd, IDC_CAPSEGS_SPIN));
	spin->Enable(quadCap);
	ReleaseISpinner(spin);
	spin = GetISpinner(GetDlgItem(hWnd, IDC_CAPSPHERE_SPIN));
	spin->Enable(quadCap);
	ReleaseISpinner(spin);
}
...
void RendSplineParamsMapDlgProc::Update(HWND hWnd, TimeValue t)
{
	...
	// Checkbox controls may need to be disabled (main Update call tends to enable them)
	EnableWindow(GetDlgItem(hWnd, IDC_USE_VIEWPORT), mod->pblock->GetInt(rnd_display));
...
//	float nlength;
	...
	pblock->GetValue(rnd_twist_correction, t, twistCorrect, valid);
	pblock->GetValue(rnd_cap, t, cap, valid);
	pblock->GetValue(rnd_quad_cap, t, quadCap, valid);
	pblock->GetValue(rnd_cap_segments, t, capSegments, valid);
	pblock->GetValue(rnd_sphere_cap, t, sphereCap, valid);

	mAspectLock = aspectLock;

	theHold.Suspend(); // need to suspend Undo

	// Also turn off macro recorder
	macroRec->Disable();
	...
	shape->SetTwistCorrect(twistCorrect);
	
	shape->SetCap(cap);
	shape->SetQuadCap(quadCap);
	shape->SetCapSegments(t, capSegments);
	shape->SetSphereCap(t, sphereCap);
...
void RendSpline::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc)
{
...
//==============================================================================
// CLASS RendSplinePostLoad
//==============================================================================

class RendSplinePostLoad : public PostLoadCallback {
public:
	RendSpline *mod;
	RendSplinePostLoad(RendSpline* rs) : mod(rs) {}
	virtual void proc(ILoad* load);
};

// IMPORTANT: Need to update this whenever new versions are added so that original file versions can be caught and their values defaulted appropriately
void RendSplinePostLoad::proc(ILoad*)
{
	// Make sure old param blocks get filled in with appropriate data for their versions
	IParamBlock2PostLoadInfo *info = (IParamBlock2PostLoadInfo*)(mod->pblock->GetInterface(IPARAMBLOCK2POSTLOADINFO_ID));
	if (info == nullptr)
		return;
	int theVersion = info->GetVersion();
	switch (theVersion) {
	case RENDSPLINE_MOD_FILE_NEPTUNE:
		// These files contain the proper versioning contained in RendSpline class; don't do anything with it
		break;
	default:
		// If we reach here, it's an original version.  Set up newer parameters appropriately
		mod->pblock->SetValue(rnd_twist_correction, 0, FALSE);		// Old versions didn't twist-correct
		mod->pblock->SetValue(rnd_cap, 0, TRUE);					// They always capped and there were no quad or sphere cap options
		mod->pblock->SetValue(rnd_quad_cap, 0, FALSE);
		mod->pblock->SetValue(rnd_cap_segments, 0, 1);
		mod->pblock->SetValue(rnd_sphere_cap, 0, 0.0f);
		break;
	}
}
...
{
	RendSplinePostLoad* cp = new RendSplinePostLoad(this);
	iload->RegisterPostLoadCallback(cp);
