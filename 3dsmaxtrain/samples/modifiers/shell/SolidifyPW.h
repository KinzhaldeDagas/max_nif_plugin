...
constexpr int kVersion_2024_1 = 202401; //this version changes the mapping from instanced to copies
constexpr int kCurrentVersion = kVersion_2024_1;
...
public:
	...
	Point3 IntersectPlaneLine(Point3 norm, float w, Point3 p1, Point3  p2)
	{
		...
		float intersection = norm.x * (p2.x - p1.x) + norm.y * (p2.y - p1.y) + norm.z * (p2.z - p1.z);
		if (fabs(intersection) < 0.0001f) // Line and plane don't intersect
		{
			ct = 0;
			return Point3(0.0f,0.0f,0.0f);
		}

		float invIntersection = 1.0f / intersection;
		float mu = -(td + norm.x * p1.x + norm.y * p1.y + norm.z * p1.z) * invIntersection;
		Point3 tp;
...
public:
		...
		void MakeSolid(int version, Mesh *msh, int segments, float amount, float oamount,int matid = -1,
