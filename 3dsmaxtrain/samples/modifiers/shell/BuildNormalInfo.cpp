...
void BuildMeshInfo::BuildShapeData(PolyLine* shape)
{
	...
	for (int i = 0; i < shapeData.Count(); i++)
	{
		...
		{
			float invULen = 1.0f / uLen;
			shapeData[i].uPer = (shape->pts[i].p.y - shape->pts[0].p.y) * invULen;
			shapeData[i].vPer = (shape->pts[i].p.x - shape->pts[0].p.x) * -invULen;
		}
	}
}

__forceinline void SpinLock(LONG& AtomFlag)
{
	while (InterlockedExchange(&AtomFlag, TRUE))
	{
		SwitchToThread();
	}
}

__forceinline void SpinUnlock(LONG& AtomFlag)
{
	InterlockedExchange(&AtomFlag, FALSE);
}
...
void BuildMeshInfo::BuildData(Mesh* mesh, PolyLine* shape, BOOL fixupEdges)
{
	...
	if (shape)
	{
		...
		for (int j = 0; j < bevelData.Count(); j++)
			bevelData[j].ct = 0;
	...
	{
		closestFaceNorm[j] = -1;
		closestFaceAngle[j] = PI * 3.0f;
	}

	MaxSDK::PerformanceTools::ThreadTools threadData;
	int numThreads =
			threadData.GetNumberOfThreads(MaxSDK::PerformanceTools::ThreadTools::kDeformationThreading, mesh->numFaces);
	int chunkSize = threadData.GetMinNumberElements(MaxSDK::PerformanceTools::ThreadTools::kDeformationThreading);
...
#pragma omp parallel for schedule(static, chunkSize)
	for (int j = 0; j < mesh->numFaces; j++)
	{
		...
		norm = FNormalize(CrossProd(vecA, vecB));
...
#pragma omp critical
	...
	if (degFaces.Count())
	{
		...
		while (!done)
		{
			...
			for (int j = 0; j < degFaces.Count(); j++)
			{
				Point3 norm(0.0f, 0.0f, 0.0f);
				int normCT = 0;
	...
	Tab<LONG> vFlag;
	vFlag.SetCount(mesh->numVerts);
	LONG* vFlagPtr = NULL;
	if (mesh->numVerts > 0)
	{
		vFlagPtr = vFlag.Addr(0);
		ZeroMemory(vFlagPtr, sizeof(LONG) * mesh->numVerts);
	}
...
#pragma omp parallel for schedule(static, chunkSize)
	for (int j = 0; j < mesh->numFaces; j++)
	{
		...
		SpinLock(vFlagPtr[ia]);

		auto mul = norm * 0.5f * static_cast<float>(1.0f / PI);
		vnorms[ia] += cornerAngles[j].a * mul;
		SpinUnlock(vFlagPtr[ia]);

		SpinLock(vFlagPtr[ib]);
		vnorms[ib] += cornerAngles[j].b * mul;
		SpinUnlock(vFlagPtr[ib]);

		SpinLock(vFlagPtr[ic]);
		vnorms[ic] += cornerAngles[j].c * mul;
		SpinUnlock(vFlagPtr[ic]);
	}

#pragma omp parallel for schedule(static, chunkSize)
	for (int j = 0; j < mesh->numVerts; j++)
	{
		vnorms[j] = FNormalize(vnorms[j]);
	}
	...
	if (fixupEdges)
	{
		...
		for (int j = 0; j < e.edges.Count(); j++)
		{
			...
			if ((faceA == -1) || (faceB == -1))
			{
				...
				vPlanes[vertA].ct = 0.0f;
				vPlanes[vertB].ct = 0.0f;
			}
		}
		for (int j = 0; j < e.edges.Count(); j++)
		{
		...
		for (int j = 0; j < mesh->numVerts; j++)
		{
			...
			if ((vPlanes[j].ct == 3) || ((vPlanes[j].ct == 2) && (vPlanes[j].isEdgeVert)))
...
void BuildMeshInfo::MakeSolid(int version, Mesh* mesh, int segments, float amount, float oamount, int matid, int sg, int edgemap,
		float tvOffset, int innerMatID, int outerMatID, BOOL selEdges, BOOL selFaces, BOOL selOuterFaces,
		BOOL fixupEdges, BOOL autoSmooth, float autoSmoothAngle, PolyLine* shape)
{
	mVersion = version;
	if (shape)
	...
	for (int k = 0; k < (segments - 1); k++)
	{
		...
		for (int j = 0; j < sEdges.Count(); j++)
		{
			...
			if (shape)
			{
				...
				vVec = vVec * edgedist /*Length((outerList[index]-innerList[index]))*/ * vPer *
						bevelData[index].realLength;

				p[startVert] = outerList[index] - (outerList[index] - innerList[index]) * uPer;
				p[startVert] += vVec;
			}
			else
				p[startVert] = outerList[index] - (outerList[index] - innerList[index]) * per;
	...
	Face* f = mesh->faces;
	for (int j = 0; j < initialNumberFaces; j++)
	{
		f[startFace] = f[j];
		...
		if (f[j].getEdgeVis(0))
			...
			f[startFace].setEdgeVis(1, EDGE_INVIS);

		if (f[j].getEdgeVis(1))
			f[startFace].setEdgeVis(0, EDGE_VIS);
		else
			f[startFace].setEdgeVis(0, EDGE_INVIS);
		...
		if (edgemap == 0) // copy
		{
			...
			if (initialNumberTVVerts > 0)
			{
				...
				else
				{
					...
					if (uvw)
					{
						for (int j = 0; j < initialNumberTVVerts; j++)
						{
							uvw[startVert] = uvw[j];
							startVert++;
						}
			...
			if (initialNumberTVFaces > 0)
			{
				...
				if (uvwFace)
				{
					for (int j = 0; j < initialNumberTVFaces; j++)
					{
						if (mVersion < kVersion_2024_1)
						{
						...
						{
							uvwFace[startFace].t[0] = uvwFace[j].t[2] + startVert;
							uvwFace[startFace].t[1] = uvwFace[j].t[1] + startVert;
							uvwFace[startFace].t[2] = uvwFace[j].t[0] + startVert;
						}
						startFace++;
					}
			...
			if (initialNumberTVFaces > 0)
			{
				...
				if (uvwFace)
				{
					...
					for (int j = 0; j < edgeLoops.Count(); j++)
					{
						...
						{
							int starti = 0;
							for (int i = (edgeLoops[j]->loop.Count() - 1); i >= 0; i--)
							{
								int eIndex = edgeLoops[j]->loop[i];
		...
		else if (edgemap == 3) // interpolate edge mapping
		{
			...
			if (initialNumberTVFaces > 0)
			{
				...
				if (uvwFace)
				{
					...
					for (int j = 0; j < averageList.Count(); j++)
					{
	...
	if (autoSmooth)
	{
		float sangle = autoSmoothAngle * DEG_TO_RAD;
			...
			for (int i = 0; i < e.edges.Count(); i++)
			{
				...
				if ((faceA != -1) && (faceB != -1))
				{
					...
					if (isSeed)
					{
						...
						Tab<bool> processed;
						processed.SetCount(mesh->numFaces);
						bool* processPtr = processed.Addr(0);
						ZeroMemory(processPtr, sizeof(bool) * mesh->numFaces);
						processed[selFace] = true;
						...
						while (!done)
						{
							...
							for (int k = 0; k < cluster.Count(); k++)
							{
								...
								for (int m = 0; m < 3; m++)
								{
									...
									if (adjFace != -1)
									{
										...
										{
											if (!processPtr[adjFace])
												newCluster.Append(1, &adjFace, 500);
											processPtr[adjFace] = true;
											++numberProcessed;
							...
							if (numberSet == numberProcessed)
								done = TRUE;
							numberSet = numberProcessed;
						}
						// get an unused sg
						DWORD sg = 1;
						for (int k = 0; k < 32; k++)
						...
						// DebugPrint(_T("Cluster %d"),sg);
						int processedCount = processed.Count();
...
void BuildMeshInfo::BevelBySpline(Mesh* mesh, PolyLine* shape, int eMatID, int oMatID, int sg, BOOL autoSmooth,
		float autoSmoothAngle, int map, BOOL selEgdes, BOOL selTop)
{
	int initialNumberFaces, initialNumberVertices;
	initialNumberFaces = mesh->numFaces;
	initialNumberVertices = mesh->numVerts;
