...
public:
	...
	{
		return localized ? GetString(IDS_CLASS_NAME) : _T("Shell");
	}
	...
	{
		s = localized ? GetString(IDS_CLASS_NAME) : _T("Shell");
	}
...
ClassDesc2* GetSolidifyPWDesc()
{
	return &SolidifyPWDesc;
}
		...
		case pb_selouter: {
			if (v.i)
			{
				if (p->ip)
				{
					...
					p->NotifyDependents(FOREVER, PART_SELECT | PART_DISPLAY | PART_SUBSEL_TYPE, REFMSG_CHANGE);
...
static ParamBlockDesc2 solidifypw_param_blk(solidifypw_params, _T("params"), 0, &SolidifyPWDesc,
		...
		// params
		pb_amount, _T("innerAmount"), TYPE_WORLD, P_ANIMATABLE, IDS_IAMOUNT, p_default, 0.0f, p_range, 0.0f, 999000.0f,
		p_ui, TYPE_SPINNER, EDITTYPE_UNIVERSE, IDC_EDIT, IDC_SPIN, 0.01f, p_nonLocalizedName, _T("Inner Amount"), p_end,

		pb_oamount, _T("outerAmount"), TYPE_WORLD, P_ANIMATABLE, IDS_OAMOUNT, p_default, 1.0f, p_range, 0.0f, 999000.0f,
		p_ui, TYPE_SPINNER, EDITTYPE_UNIVERSE, IDC_EDIT2, IDC_SPIN2, 0.01f, p_nonLocalizedName, _T("Outer Amount"),
		p_end,

		pb_overridematid, _T("overrideMatID"), TYPE_BOOL, 0, IDS_OVERRIDEMATID, p_default, FALSE, p_ui,
		TYPE_SINGLECHECKBOX, IDC_MATIDCHECK, p_end,

		pb_matid, _T("matID"), TYPE_INT, P_ANIMATABLE, IDS_MATID, p_default, 1, p_range, 1, 255, p_ui, TYPE_SPINNER,
		EDITTYPE_INT, IDC_MATID, IDC_MATIDSPIN, 0.1f, p_nonLocalizedName, _T("Mat ID"), p_end,


		pb_overridesg, _T("overrideSmoothingGroup"), TYPE_BOOL, 0, IDS_OVERRIDESG, p_default, FALSE, p_ui,
		TYPE_SINGLECHECKBOX, IDC_SMOOTHCHECK, p_end,

		pb_sg, _T("smoothGroup"), TYPE_INT, P_ANIMATABLE, IDS_SG, p_default, 0, p_range, 0, 32, p_ui, TYPE_SPINNER,
		EDITTYPE_INT, IDC_SMOOTHGROUP, IDC_SMOOTHGROUPSPIN, 0.01f, p_nonLocalizedName, _T("Smooth Group"), p_end,

		pb_edgemap, _T("edgeMapping"), TYPE_INT, 0, IDS_EDGEMAPPING, p_default, 0, p_ui, TYPE_INTLISTBOX,
		IDC_EDGEMAPCOMBO, 4, IDS_COPY, IDS_NONE, IDS_STRIP, IDS_INTERPOLATE, p_end,


		pb_tvoffset, _T("tvOffset"), TYPE_FLOAT, P_ANIMATABLE, IDS_TVOFFSET, p_default, 0.05f, p_range, 0.001f, 1.0f,
		p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_TVOFFSET, IDC_TVOFFSETSPIN, 0.01f, p_nonLocalizedName, _T("TV Offset"),
		p_end,

		pb_overrideinnermatid, _T("overrideInnerMatID"), TYPE_BOOL, 0, IDS_OVERRIDEINNERMATID, p_default, FALSE, p_ui,
		TYPE_SINGLECHECKBOX, IDC_MATIDCHECK2, p_end,

		pb_innermatid, _T("matInnerID"), TYPE_INT, P_ANIMATABLE, IDS_INNERMATID, p_default, 1, p_range, 1, 255, p_ui,
		TYPE_SPINNER, EDITTYPE_INT, IDC_MATID2, IDC_MATIDSPIN2, 0.1f, p_nonLocalizedName, _T("Inner MatID"), p_end,

		pb_seledges, _T("selectEdgeFaces"), TYPE_BOOL, 0, IDS_SELECTEDGES, p_default, FALSE, p_ui, TYPE_SINGLECHECKBOX,
		IDC_SELECTEDGESCHECK, p_accessor, &solidifypw_accessor, p_end,

		pb_selinner, _T("selectInnerFaces"), TYPE_BOOL, 0, IDS_SELECTINNER, p_default, FALSE, p_ui, TYPE_SINGLECHECKBOX,
		IDC_SELECTFACESCHECK, p_accessor, &solidifypw_accessor, p_end,


		pb_segments, _T("segments"), TYPE_INT, P_ANIMATABLE, IDS_SEGMENTS, p_default, 1, p_range, 1, 255, p_ui,
		TYPE_SPINNER, EDITTYPE_INT, IDC_EDIT3, IDC_SPIN3, 0.1f, p_end,

		pb_fixupcorners, _T("straightenCorners"), TYPE_BOOL, 0, IDS_FIXUPCORNERS, p_default, FALSE, p_ui,
		TYPE_SINGLECHECKBOX, IDC_FIXCORNERSCHECK, p_end,


		pb_autosmooth, _T("autoSmooth"), TYPE_BOOL, 0, IDS_AUTOSMOOTH, p_default, TRUE, p_ui, TYPE_SINGLECHECKBOX,
		IDC_AUTOSMOOTHCHECK, p_end,

		pb_autosmoothangle, _T("autoSmoothAngle"), TYPE_FLOAT, P_ANIMATABLE, IDS_SG, p_default, 45.0f, p_range, 0.0f,
		180.0f, p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_AUTOSMOOTH, IDC_AUTOSMOOTHSPIN, 1.0f, p_nonLocalizedName,
		_T("Smooth Group"), p_end,


		pb_selouter, _T("selectOuterFaces"), TYPE_BOOL, 0, IDS_SELECTOUTER, p_default, FALSE, p_ui, TYPE_SINGLECHECKBOX,
		IDC_SELECTOUTERFACESCHECK, p_accessor, &solidifypw_accessor, p_end,

		pb_overrideoutermatid, _T("overrideOuterMatID"), TYPE_BOOL, 0, IDS_OVERRIDEOUTERMATID, p_default, FALSE, p_ui,
		TYPE_SINGLECHECKBOX, IDC_MATIDCHECK3, p_end,

		pb_outermatid, _T("matOuterID"), TYPE_INT, P_ANIMATABLE, IDS_OUTERMATID, p_default, 3, p_range, 1, 255, p_ui,
		TYPE_SPINNER, EDITTYPE_INT, IDC_MATID3, IDC_MATIDSPIN3, 0.1f, p_nonLocalizedName, _T("Outer MatID"), p_end,

		pb_bevel, _T("bevel"), TYPE_BOOL, 0, IDS_BEVEL, p_default, FALSE, p_ui, TYPE_SINGLECHECKBOX,
		IDC_BEVELEDGESCHECK, p_end,

		pb_bevelshape, _T("bevelShape"), TYPE_INODE, 0, IDS_BEVELSHAPE, p_ui, TYPE_PICKNODEBUTTON, IDC_SHAPE,
		p_sclassID, SHAPE_CLASS_ID, p_nonLocalizedName, _T("Bevel Shape"), p_end,

		pb_version, _T("version"), TYPE_INT, 0, 0, p_default, kCurrentVersion, p_end, 
	p_end);
...
SolidifyPW::SolidifyPW()
{
	...
	pblock->SetValue(pb_version,0, kCurrentVersion);
}
...
void SolidifyPW::ModifyObject(TimeValue t, ModContext& mc, ObjectState* os, INode* node)
{
	...
	if (os->obj->IsSubClassOf(triObjectClassID))
	{
		...
		if (bevel)
			meshInfo.MakeSolid(pblock->GetInt(pb_version), &mesh, segments, a, oa, matid, sg, edgeMap, tvOffset, imatid, omatid, selEdges, selInner,
					selOuter, fixupCorners, autoSmooth, smoothAngle, &shape.lines[0]);
		else
			meshInfo.MakeSolid(pblock->GetInt(pb_version), &mesh, segments, a, oa, matid, sg, edgeMap, tvOffset, imatid, omatid, selEdges, selInner,
					selOuter, fixupCorners, autoSmooth, smoothAngle, NULL);
...
void SolidifyPW::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext* mc)
{
}
...
int SolidifyPW::Display(TimeValue t, INode* inode, ViewExp* vpt, int flags, ModContext* mc)
{
...
class VersionCallback : public PostLoadCallback
{
public:
	SolidifyPW* mMod = nullptr;
	int mVersion = 0;

	VersionCallback(SolidifyPW* mod, int ver) : mMod(mod), mVersion(ver)
	{
	}
	//called after your pblocks are set
	virtual void proc(ILoad* iload) override;
	int Priority() override { return 0; }
};

void VersionCallback::proc(ILoad*) 
{
	mMod->pblock->SetValue(pb_version, 0, mVersion);

	delete this;
}

//we use the param block pb_version to handle versioing the modifier
//but it was added after the fact so use the ::Load, ::Save to get/set the
//version and then in post callback push it into the param block
#define VERSIONCHUNK		100
IOResult SolidifyPW::Load(ILoad* iload)
{

	// Between Max 2024.1 and Max 2025.0, the Modifier base class data was not properly saved by the shell modifier,
	// while for versions outside of that range, the base class data will be the first saved chunk; as Modifier::Load
	// simply reflexively returns IO_OK, without modifying the state of iload, if the first data chunk encountered is
	// not the base chunk, we may safely call it unconditionally here
	Modifier::Load(iload);
	...
	while (IO_OK == (res = iload->OpenChunk())) 
	{
		...
		{
			case VERSIONCHUNK:
				res = iload->Read(&ver, sizeof(int), &nb);
				break;
		}
		iload->CloseChunk();
	...
	iload->RegisterPostLoadCallback( new VersionCallback(this, ver));
	return IO_OK;
}

IOResult SolidifyPW::Save(ISave* isave)
{
	ULONG nb;

	// Save Modifier base class data
	Modifier::Save(isave);

	// Save the version of the shell operation
	int version = pblock->GetInt(pb_version);	
	isave->BeginChunk(VERSIONCHUNK);
	isave->Write(&version, sizeof(int), &nb);
	isave->EndChunk();

	// Return an enumeration value indicating success
	return IO_OK;
}
...
INT_PTR SolidifyPWDlgProc::DlgProc(TimeValue t, IParamMap2* map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
	case WM_INITDIALOG:
