...
public:
	...
	virtual BOOL IsEnabled(int cmdId) override;
	virtual BOOL IsChecked(int cmdId) override;
...
private:
	...
	static const ActionDescription s_actionDescriptions[];
};
...
const ActionDescription XFormActionTable::s_actionDescriptions[] =   {
	{
		ACTIONID_RESET_XFORM,
...
{
	BuildActionTable(NULL, _countof(s_actionDescriptions), s_actionDescriptions, hInstance);
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Reset XForm"); }
...
void ResetXForm::ResetNodes(const INodeTab& nodesToReset)
{
	...
	for (int i = 0; i < nodesToReset.Count(); i++) {
		...
		//rename the modifier so it's clear it comes from the utility button rather than any normal XForm modifer
		mod->SetName(resetXFormDesc.ClassName());
