...
{
	//! Initialize the BitArrays up front 
	...
	yHitLists.Init();
	zHitLists.Init();
	hitLists.SetCount(maxThread);
	BitArray tmpArray;
	for (int i = 0; i < maxThread; ++i)
	{
		xHitLists.Append(1, &tmpArray);
		yHitLists.Append(1, &tmpArray);
		zHitLists.Append(1, &tmpArray);
		hitLists[i].Init();
	}
}
...
void UniformGrid::InitializeGrid(int size)
{
	width = size;
	if (width < 1) 
	{
...
void UniformGrid::FreeGrid()
{
	...
	{
		xHitLists[i].SetSize(0);
		yHitLists[i].SetSize(0);
		zHitLists[i].SetSize(0);
		hitLists[i].ZeroCount();
	}
}
...
void UniformGrid::LoadPoints(Point3 *p, int count)
{
...
void UniformGrid::TagCells(const Point3& p,float radius, int whichGrid, 
						   Tab<int>& hitList, BitArray& xHitList, BitArray& yHitList, BitArray& zHitList)
{
	float x = 0.0f,y = 0.0f;
	int minX,maxX;
	int minY,maxY;
...
void UniformGrid::ClosestPoint(const Point3& p, float radius, int &pindex, float &d)
{
	int curThread = omp_get_thread_num();
	DbgAssert(curThread < hitLists.Count());
	BitArray& xHitList = xHitLists[curThread];
	BitArray& yHitList = yHitLists[curThread];
	BitArray& zHitList = zHitLists[curThread];
	Tab<int>& hitList = hitLists[curThread];
	xHitList.SetSize(pointBase.Count());
	yHitList.SetSize(pointBase.Count());
	zHitList.SetSize(pointBase.Count());
	xHitList.ClearAll();
	yHitList.ClearAll();
	zHitList.ClearAll();
	hitList.SetCount(0);
	...
	TagCells(p,radius, 0, hitList, xHitList, yHitList, zHitList);
	//find the cell in the YGrid
	TagCells(p,radius, 1, hitList, xHitList, yHitList, zHitList);
	//find the cell in the ZGrid
	TagCells(p,radius, 2, hitList, xHitList, yHitList, zHitList);
...
void UniformGrid::ClosestPoints(Point3 *p, float *radius, int *i, float *d, int count)
{
}


void UniformGrid::DrawBounds(GraphicsWindow *gw, Box3 bounds)
{
...
void UniformGrid::Display(GraphicsWindow *gw)
{
	return;
	Matrix3 tm(1);
	gw->setTransform(tm);
	...
	for (int i = 0; i < width; i++)
	{
		...
		for (int j = 0; j < width; j++)
		{
			...
			DrawBounds(gw, b);
			auto bCenter = b.Center();
			gw->marker(&bCenter,ASTERISK_MRKR);
			index++;
			y += incy;
		}
		z += incz;
