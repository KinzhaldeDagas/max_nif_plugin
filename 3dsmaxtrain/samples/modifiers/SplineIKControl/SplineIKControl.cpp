...
class SMNodeNotify;
class SplineIKControl : public ISplineIKControl,virtual public ISplineIKControl2 {
	public:
...
//		int helpCreationFlag;
		...
		BOOL LinkToRoot();
		BOOL UnLink();
		void GetHelperNodes(INodeTab &helperNodes);

		static IObjParam *ip;			//Access to the interface
		static SplineIKControl* curMod;
		
		// From Animatable
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_CLASS_NAME) : _T("Spline IK Control"); }
...
//		void SetGenUVW(BOOL sw);
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_CLASS_NAME) : _T("Spline IK Control");}
		...
		TSTR SubAnimName(int i, bool localized) override { return localized ? GetString(IDS_PARAMS) : _T("Spline IK Control Parameters"); }
...
public:
		...
		{ 
			if (id == SPLINEIK_CONTROL_INTERFACE) 
				return (ISplineIKControl*)this; 
			if(id==SPLINEIK_CONTROL_INTERFACE2)
				return (ISplineIKControl2*)this;
...
class SplineIKControlClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Spline IK Control"); }
	SClass_ID		SuperClassID() { return OSM_CLASS_ID; }
	Class_ID		ClassID() { return SPLINEIKCONTROL_CLASS_ID; }
	...
	void TabChanged(tab_changes changeCode, Tab<PB2Value>* tab, 
		...
		if (id == ISplineIKControl::sm_point_node_list){
			...
			if (changeCode == tab_ref_deleted){	
				//MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), "All animation for this knot will be lost", "Warning!", MB_ICONSTOP | MB_OK);
	...
	void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)    // set from v
	{
		...
		switch (id)
		{
			...
			case ISplineIKControl::sm_helpersize:
				float size;
				sikc->pblock2->GetValue(ISplineIKControl::sm_helpersize, t, size);
				ct = sikc->pblock2->Count(ISplineIKControl::sm_point_node_list);
	
				for (i = 0; i < ct; i++){
					INode *sm_point_node;
					sikc->pblock2->GetValue(ISplineIKControl::sm_point_node_list, t, sm_point_node, i);
			...
			case ISplineIKControl::sm_helper_centermarker:
				BOOL cmrkr;
				sikc->pblock2->GetValue(ISplineIKControl::sm_helper_centermarker, t, cmrkr);
				ct = sikc->pblock2->Count(ISplineIKControl::sm_point_node_list);
			...
			case ISplineIKControl::sm_helper_axistripod:
				BOOL aTripod;
				sikc->pblock2->GetValue(ISplineIKControl::sm_helper_axistripod, t, aTripod);
				ct = sikc->pblock2->Count(ISplineIKControl::sm_point_node_list);
			...
			case ISplineIKControl::sm_helper_cross:
				BOOL crs;
				sikc->pblock2->GetValue(ISplineIKControl::sm_helper_cross, t, crs);
				ct = sikc->pblock2->Count(ISplineIKControl::sm_point_node_list);
			...
			case ISplineIKControl::sm_helper_box:
				BOOL bx;
				sikc->pblock2->GetValue(ISplineIKControl::sm_helper_box, t, bx);
				ct = sikc->pblock2->Count(ISplineIKControl::sm_point_node_list);
			...
			case ISplineIKControl::sm_helper_screensize:
				BOOL cnScrnSz;
				sikc->pblock2->GetValue(ISplineIKControl::sm_helper_screensize, t, cnScrnSz);
				ct = sikc->pblock2->Count(ISplineIKControl::sm_point_node_list);
			...
			case ISplineIKControl::sm_helper_drawontop:
				BOOL dOnTop;
				sikc->pblock2->GetValue(ISplineIKControl::sm_helper_drawontop, t, dOnTop);
				ct = sikc->pblock2->Count(ISplineIKControl::sm_point_node_list);
...
static ParamBlockDesc2 SplineIKControl_param_blk ( ISplineIKControl::SplineIKControl_params, _T("params"),  0, &SplineIKControlDesc, 
	...
	ISplineIKControl::sm_helpersize, 	_T("helper_size"), TYPE_FLOAT, P_ANIMATABLE + P_RESET_DEFAULT, IDS_AG_SM_HELPERSIZE,
		...
		p_accessor,		&sm_accessor,	
		p_nonLocalizedName, _T("Helper Size"),
	p_end,
	...
	ISplineIKControl::sm_helper_centermarker, 	_T("helper_centermarker"), TYPE_BOOL, P_ANIMATABLE+P_RESET_DEFAULT, IDS_AG_CENTERMARKER,
		p_default, 			FALSE,
		p_ui, 				TYPE_SINGLECHECKBOX, 	IDC_CENTERMARKER_CHECK, 
		p_accessor,		&sm_accessor,	
	...
	ISplineIKControl::sm_helper_axistripod, 	_T("helper_axistripod"), TYPE_BOOL, P_ANIMATABLE+P_RESET_DEFAULT, IDS_AG_AXISTRIPOD,
		p_default, 			FALSE,
		p_ui, 				TYPE_SINGLECHECKBOX, 	IDC_AXISTRIPOD_CHECK, 
		p_accessor,		&sm_accessor,	
...
//	Tab<int> deleted_nodes;
	...
	if((arg != NULL) && (sik->pblock2 != NULL)){
		...
		for(int i=0; i <ct; i++ ){
			INode* someNode;
			sik->pblock2->GetValue(sm_point_node_list, GetCOREInterface()->GetTime(), someNode, i);
		...
		if(deleted_nodes_number){
			//MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), "All animation for this knot will be lost. If this was not intended, hit OK and then CTRL+Z", "Warning!", MB_ICONSTOP | MB_OK);

			for (int i=0; i <sik->deleted_nodes.Count(); i++ ){
...
void SplineIKControl::ModifyObject(TimeValue t, ModContext &mc, ObjectState * os, INode *node) 
{
	...
	if (cid == EDITABLE_SURF_CLASS_ID){  //it's a NURBS point or CV curve
		...
		if (okay) {
			...
			int id;
			splNode = GetNodeFromModData(d,id);
			if (splNode == NULL) {
				DbgAssert(0);
				return;
			}
...
//		}
		...
		notASplineOrNURBS = 1;
		MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), _T("SplineIKControl modifier cannot be assigned to the selected non-shape object!"), _T("Warning!"), MB_ICONSTOP | MB_OK);
...
//			int response;
//			if (!helpCreationFlag)
//				response = MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), "You Have More Knots than Helpers! Want to Create Additional Helpers?", "Warning!", MB_ICONQUESTION | MB_YESNO);
...
int SplineIKControl::noHelperNearThisKnot(Point3 kPos){
	...
	for (int i = 0; i < ct; i++){
		INode *sm_point_node;
		pblock2->GetValue(sm_point_node_list, GetCOREInterface()->GetTime(), sm_point_node, i);
 ...
 Interval SplineIKControl::GetValidity(TimeValue t)
	...
	for (int i = 0; i < pblock2->Count(sm_point_node_list); i= i + 1){
		INode* locNode;
		pblock2->GetValue(sm_point_node_list,t,locNode,valid, i);	
	}
	pblock2->GetValue(sm_helpersize,t,f,valid);
	pblock2->GetValue(sm_helper_centermarker,t,b,valid);
	pblock2->GetValue(sm_helper_axistripod,t,b,valid);	
	pblock2->GetValue(sm_helper_cross,t,b,valid);
	pblock2->GetValue(sm_helper_box,t,b,valid);
	pblock2->GetValue(sm_helper_screensize,t,b,valid);
	pblock2->GetValue(sm_helper_drawontop,t,b,valid);
	pblock2->GetValue(sm_link_types,t,b,valid);	
...
RefTargetHandle SplineIKControl::Clone(RemapDir& remap)
{
	...
	newmod->ReplaceReference(SPLINEIKCONTROL_PBLOCK_REF,remap.CloneRef(pblock2));
...
RefResult SplineIKControl::NotifyRefChanged (const Interval& iv, RefTargetHandle hTarg, PartID& partID, RefMessage msg, BOOL propagate) 
	{
...
void SplineIKControl::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc)
{
...
float SplineIKControl::GetHelperSize(){ // All Helpers Have Same size for now
	float hsize;
	pblock2->GetValue(sm_helpersize, GetCOREInterface()->GetTime(), hsize);
	return hsize;
} 
...
void SplineIKControl::GetHelperNodes(INodeTab &helperNodes)
{
	int ct = pblock2->Count(sm_point_node_list);
	helperNodes.SetCount(ct);
	for (int i = 0; i < ct; i++)
	{ 
		helperNodes[i] = pblock2->GetINode(sm_point_node_list,0,i);
	}
}
