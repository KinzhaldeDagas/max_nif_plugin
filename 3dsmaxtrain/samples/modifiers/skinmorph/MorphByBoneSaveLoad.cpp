	...
	while (IO_OK==(res=iload->OpenChunk())) 
		...
		switch(iload->CurChunkID())  
			...
			case BONE_MORPHPOINTDATA:
				...
				if (ct)
					iload->ReadVoid(boneData[boneIndex]->morphTargets[morphIndex]->d.Addr(0),sizeof(MorphData)*ct, &nb);
				break;
				}
			case BONE_MORPHTM:
				{
...
*/
			case BONE_MORPHNAME:
				...
				boneData[boneIndex]->morphTargets[morphIndex]->name.printf(_T("%s"),name);
				break;
				}
	...
	for (int i = 0; i < boneCount; i++)
		...
		for (int j = 0; j < morphCount; j++)
			...
			if (pointCount > 0)
			{
				...
				res = isave->WriteVoid(boneData[i]->morphTargets[j]->d.Addr(0), sizeof(MorphData)*pointCount, &nb); // no TCHAR data
				isave->EndChunk();
			}
...
//			int nameLength = boneData[i]->morphTargets[j]->name.Length();
			const TCHAR *name = boneData[i]->morphTargets[j]->name.data();
