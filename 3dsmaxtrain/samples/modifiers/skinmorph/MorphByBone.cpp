...
BOOL RecursePipeAndMatchLocalData(MorphByBone* mod, MorphByBoneLocalData *mcd, Object *obj)
{
	SClass_ID		sc;
	IDerivedObject* dobj;
	...
	if ((sc = obj->SuperClassID()) == GEN_DERIVOB_CLASS_ID)
	{
		...
		while (sc == GEN_DERIVOB_CLASS_ID)
		{
			...
			{
				if (mod == dobj->GetModifier(j))
				{
					ModContext *mc = dobj->GetModContext(j);
					if ((mc) && mc->localData == mcd)
					{
						return TRUE;
					}
				}
	...
	if (bct > 0)
	{
		for (int bi = 0; bi < bct; bi++)
		{
			...
			if (RecursePipeAndMatchLocalData(mod,mcd, bobj)) return TRUE;
		}
	}
...
INode* MorphByBone::GetNodeFromLocalData(MorphByBoneLocalData *mc)

{
	...
	for (i = 0; i < dep.Nodes.Count(); i++)
	{
		...
		if (node)
		{
			...
			if (RecursePipeAndMatchLocalData(this, mc, obj))
			{
				return node;
			}
			if (obj != NULL)
...
RefTargetHandle MorphByBone::Clone(RemapDir& remap)
{
	...
	if (newmod->GetCurveCtl())
		newmod->GetCurveCtl()->RegisterResourceMaker(newmod);
...
void MorphByBone::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc)
{
...
//	 = GetNode(currentBone);
	...
	if ((activeMorph == -1) || (force))
	{
		...
		TSTR name;
		name.printf(_T("Morph %d"),boneData[cBone]->morphTargets.Count());
...
void MorphByBone::UpdateLocalUI()
{
	...
	{		
		...
		if ((id >=0) && (id<pblock->Count(pb_targnodelist)))
		{
			...
			TSTR name;
			if (node == NULL)
				name.printf(_T("%s"),GetString(IDS_NONE));
			else
			{
...
void MorphByBone::SetMorphName(const TCHAR *s,int whichBone, int whichMorph)
{
	if ((currentBone < 0) || (currentBone >= boneData.Count()) ||
		(currentMorph < 0) || (currentMorph >= boneData[currentBone]->morphTargets.Count()))
	{
...
void MorphByBone::SetNode(INode *node,int whichBone, int whichMorph)
{
	...
	theHold.Begin();
	theHold.Put(new MorphUIRestore(this));  //Workaround to handle UI update after redo
...
//get the mesh
	...
	if (sos.obj->IsSubClassOf(triObjectClassID))
		...
		if (!msh->GetFlag(MN_MESH_FILLED_IN))
		{
			msh->FillInMesh();
		}
		if (!msh->GetFlag(MN_MESH_VERTS_ORDERED))
		{
			msh->OrderVerts();
		}
	...
	//if poly object we can use that
		...
		{
		PolyObject *pobj = (PolyObject*)sos.obj;
		MNMesh *stackMesh = &pobj->GetMesh();		//stackMesh is what is flowing up the stack and we can only change it geom points

		MNMesh *tempMesh = new MNMesh(*stackMesh);  // note we make a copy of the mesh because the select loop command can reorder vertices.  
													//this causes a topo change and Skinmorph cannot not affacte topo or it will leads to a crash

		//convert vert sel to edge sel
		BitArray vsel, esel;

		if (!tempMesh->GetFlag(MN_MESH_FILLED_IN))
		{
			tempMesh->FillInMesh();
		}
		if (!tempMesh->GetFlag(MN_MESH_VERTS_ORDERED))
		{
			tempMesh->OrderVerts();
		}

		if (tempMesh->numv != numberVerts) return FALSE;
	...
	//loop through the selection
		...
		esel.SetSize(tempMesh->nume);
		esel.ClearAll();

		for (int i = 0; i < tempMesh->nume; i++)
			{
			int a,b;
			a = tempMesh->e[i].v1;
			b = tempMesh->e[i].v2;
		...
		//do loop
		if (mode == LOOP_SEL)
			tempMesh->SelectEdgeLoop (esel);
		else if (mode == RING_SEL)
		{
			tempMesh->SelectEdgeRing(esel);
		}
		...
		delete tempMesh;  // remember to delete the mesh since we copied it
...
const TCHAR*	MorphByBone::GetMorphName(INode *node, int index)
{
	for (int j = 0; j < pblock->Count(pb_bonelist); j++)
	{
		INode *pnode = GetNode(j);
...
int		MorphByBone::GetMorphIndex(INode *node, const TCHAR *name)
{
	for (int j = 0; j < pblock->Count(pb_bonelist); j++)
	{
		INode *pnode = GetNode(j);
		if (pnode == node)
		{
			...
			{
				if (_tcscmp(name,boneData[j]->morphTargets[k]->name) == 0)
					return k;
			}
		}
	}
...
void MorphTargets::CreateGraph( MorphByBone *mod, IParamBlock2 *pblock )
{
	if (externalFalloffID == -1)
	{
		...
		graph->RegisterResourceMaker(mod);
