...
*/
HTREEITEM AddItemToTree(HWND hwndTV, TVITEM &tvi, LPCTSTR lpszItem, int nLevel, HTREEITEM parent)
{ 
//    TVITEM tvi; 
    ...
    tvi.pszText = const_cast<TCHAR*>(lpszItem); //LPSTR_TEXTCALLBACK;//
		...
		case WM_COMMAND:
			...
			switch (LOWORD(wParam)) 
				...
				case IDC_ADD_BUTTON:
					{
					theHold.Begin();
					theHold.Put(new MorphUIRestore(mod)); //Workaround to handle UI update after redo
					mod->AddNodes();
					theHold.Put(new MorphUIRestore(mod)); //Workaround to handle UI update after redo
...
HCURSOR PickControlNode::GetDefCursor(IObjParam *ip)
    ...
    if ( !hCur ) 
		{
        hCur = UI::MouseCursors::LoadMouseCursor(UI::MouseCursors::Selection);
        }
	return hCur;
	}
