...
// compile-time validates size of MorphData class to ensure its size doesn't change since instances are 
// saved as a binary blob to scene file. Changes in size will break file i/o
static_assert(sizeof(MorphData) == 2 * sizeof(int) + 3 * sizeof(Point3), "");
...
class MorphByBone : public Modifier, public  IMorphByBone {
	public:
		...
		// From Animatable
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_CLASS_NAME) : _T("Skin Morph"); }
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_CLASS_NAME) : _T("Skin Morph");}
...
public:
		...
		INode* GetNodeFromLocalData(MorphByBoneLocalData *mc);
...
//		void UpdateTreeListWeights();
		...
		void SetMorphName(const TCHAR *s, int whichBone, int whichMorph);	
		...
		const TCHAR*	GetMorphName(INode *node, int index);
		int		GetMorphIndex(INode *node, const TCHAR *name);

		void	AddBone(INode *node);
		void	fnAddBone(INode *node);
		void	fnRemoveBone(INode *node);
		void	fnSelectBone(INode *node,const TCHAR* morphName);
		INode*	fnGetSelectedBone();
		const TCHAR*	fnGetSelectedMorph();
		...
		void	fnRemoveMorph(INode *node, const TCHAR *name);
		...
		void	fnResetOrientation(INode *node, const TCHAR *name);

		void	fnReloadTarget(INode *node, const TCHAR *name);
		void	fnMirrorPaste(INode *node);

		void	fnEditFalloffGraph(INode *node, const TCHAR *name);
		void	fnSetExternalNode(INode *node, const TCHAR *name,INode *exnode);
		...
		const TCHAR*	fnBoneGetMorphName(INode *node, int morphIndex);
		void	fnBoneSetMorphName(INode *node, int morphIndex,const TCHAR* name);
		...
		float	fnGetWeight(INode *node, const TCHAR*name);
...
public:
	...
	const TCHAR *dialogTitle() {return GetString(IDS_PW_SELECTBONES);};
	const TCHAR *buttonText() {return GetString(IDS_PW_SELECT);};
...
class MorphByBoneClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Skin Morph"); }
	SClass_ID		SuperClassID() { return OSM_CLASS_ID; }
	Class_ID		ClassID() { return MORPHBYBONE_CLASS_ID; }
...
class MorphRestore : public RestoreObj {
	public:
		...
		TSTR Description() { return TSTR(GetString(IDS_MOVE)); }
	};
...
class SelectionRestore : public RestoreObj {
	public:
		...
		TSTR Description() { return TSTR(GetString(IDS_SELECTION)); }
	};
...
class ResetOrientationMorphRestore : public RestoreObj 
{
	public:
		...
		TSTR Description() { return TSTR(GetString(IDS_RESETORIENTATION)); }
