...
class SurfrevMod: public Modifier, public RealWorldMapSizeInterface {
	protected:
		Control *axisControl;
	...
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_TH_LATHE_CLASS) : _T("Lathe"); }  
		virtual Class_ID ClassID() { return Class_ID(SURFREVOSM_CLASS_ID,0);}		
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_TH_LATHE) : _T("Lathe"); }
		IOResult Load(ILoad *iload);
		IOResult Save(ISave *isave);


		SurfrevMod(BOOL create);
		virtual ~SurfrevMod();
...
public:
		...
		void BuildMeshFromShape(TimeValue t,ModContext &mc, ObjectState * os, const Matrix3 &axis, Mesh &mesh);
		void BuildPatchFromShape(TimeValue t,ModContext &mc, ObjectState * os, const Matrix3 &axis, PatchMesh &pmesh);
...
//		5 of this modifier uses the new weld core method
		int ver;
...
INT_PTR SurfrevDlgProc::DlgProc(
...
class SurfrevClassDesc:public ClassDesc {
	...
	void*			Create(BOOL loading = FALSE)
    {
        ...
        return new SurfrevMod(!loading);
    }
	const TCHAR*	ClassName() { return GetString(IDS_TH_LATHE_CLASS); }
	const TCHAR*	NonLocalizedClassName() { return _T("Lathe"); }
...
static ParamUIDesc descParam[] = {
	...
	// Weld Core
	ParamUIDesc(PB_WELDCORE,TYPE_SINGLECHECKBOX,IDC_WELDCORE),
	...
	// Flip Normals
	ParamUIDesc(PB_FLIPNORMALS,TYPE_SINGLECHECKBOX,IDC_FLIPNORMALS),
...
SurfrevMod::SurfrevMod(BOOL create)
	{
	pblock = NULL;
	axisControl = NULL;
//watje new mapping ver < 4 of this modifier use the old mapping
//		5 uses the new weld core method
	ver = 5;

	doAlign = FALSE;

	// Create a parameter block
	ReplaceReference(PBLOCKREF, CreateParameterBlock(descVer7, PBLOCK_LENGTH, CURRENT_VERSION));
	...
	ReplaceReference(AXISREF, NewDefaultMatrix3Controller()); 
...
TSTR SurfrevMod::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case AXISREF: return localized ? GetString(IDS_TH_AXIS) : _T("Axis");
	case PBLOCKREF: return localized ? GetString(IDS_RB_PARAMETERS) : _T("Parameters");
...
RefTargetHandle SurfrevMod::Clone(RemapDir& remap)
	{
	SurfrevMod* newmod = new SurfrevMod(false);	
	newmod->ReplaceReference(PBLOCKREF,remap.CloneRef(pblock));
	newmod->ReplaceReference(AXISREF, remap.CloneRef(axisControl)); 
...
void SurfrevMod::ModifyObject(TimeValue t, ModContext &mc, ObjectState * os, INode *node) 
	...
	if(doAlign) {
		...
		auto packets = SetXFormPacket(axis);
		axisControl->SetValue(t, &packets);
		}
	 		
//DebugPrint(_T("Surfrev modifying object\n"));
	...
	os->obj->UnlockObject();
//DebugPrint(_T("Surfrev modification complete!\n"));
	}
...
TSTR SurfrevMod::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_DEGREES: return localized ? GetString(IDS_TH_DEGREES)  : _T("Degrees");
		case PB_SEGS:    return localized ? GetString(IDS_TH_SEGMENTS) : _T("Segments");
		default:         return TSTR(_T(""));
		}
	}
...
void SurfrevPostLoadCallback::proc(ILoad *iload) {
	...
	DWORD oldVer = ((SurfrevMod*)(cb->GetTarget()))->pblock->GetVersion();
...
void SurfrevMod::UpdateUI()
{
	if (ip == NULL)
		return;
	SurfrevDlgProc* dlg = static_cast<SurfrevDlgProc*>(pmapParam->GetUserDlgProc());
	dlg->UpdateRWS();
}

BOOL SurfrevMod::GetUsePhysicalScaleUVs()
{
    return ::GetUsePhysicalScaleUVs(this);
}


void SurfrevMod::SetUsePhysicalScaleUVs(BOOL flag)
{
    BOOL curState = GetUsePhysicalScaleUVs();
    if (curState == flag)
        return;
    if (theHold.Holding())
        theHold.Put(new RealWorldScaleRecord<SurfrevMod>(this, curState));
...
void SurfrevMod::BuildMeshFromShape(TimeValue t,ModContext &mc, ObjectState * os, const Matrix3 &axis, Mesh &mesh) {
	BOOL texturing;
	pblock->GetValue(PB_MAPPING, TimeValue(0), texturing, FOREVER);
	BOOL genMatIDs;
	pblock->GetValue(PB_GEN_MATIDS, TimeValue(0), genMatIDs, FOREVER);
	...
	//the old method just transformed the point into the axis space and checked the x value, the problem is if you scaled x/y or rotated the axis around the Z
	//the values would be off.  The new method just test distance from the center of the axis
	bool useNewWeldCoreMethod = false;
	if (ver >= 5)
		useNewWeldCoreMethod = true;
...
#endif //TEST_TAPER
				mesh.setVert(vert++, p);
				if(level == 1 && !inverse && d > dim.coreThresh) {
					Point3 ip = p * itm;
					if(ip.z > 0.0f)
						inverse = TRUE;
					}
	...
	if(texturing) {
		int tvertex = 0;
        float radius = axis.GetTrans().Length();
		...
		for(int level = 0; level < TVlevels; ++level) {
			...
			for(poly = 0; poly < polys; ++poly) {
				...
				for(tp = 0; tp < texPts; ++tp) {
                    ...
                    float vScale = usePhysUVs ? ((float) radians*radius) : 1.0f;
					mesh.setTVert(tvertex++, UVVert(tU, vScale*tV, 0.0f));
					}
				}
			}
		}
	...
	if(degrees < 360.0f && anyClosed && (capStart || capEnd)) {
		...
		if(capStart) {
			...
			if(texturing)
				MakeMeshCapTexture(mesh, Inverse(mat1), oldFaces, mesh.numFaces, GetUsePhysicalScaleUVs());
			}
		if(capEnd) {
			...
			// If texturing, create the texture faces and vertices
			if(texturing)
				MakeMeshCapTexture(mesh, Inverse(mat2), oldFaces, mesh.numFaces, GetUsePhysicalScaleUVs());
			}
		}
...
//DebugPrint(_T("BV:%d V:%d v1:%d v2:%d v3:%d v4:%d\n"),baseVert, vert, v1, v2, v3, v4);
	...
	if(weldCore && weldFlags.AnyBitSet()) {
		...
		if(mesh.tvFace && faceDel.AnyBitSet()) {
			int outTVFace = 0;
			for(int i = 0; i < faces; ++i) {
				if(!faceDel[i])
...
void SurfrevMod::SetAxis(TimeValue t, int type) {
	switch(type) {
		case X_AXIS:
		{
			auto packet = SetXFormPacket(RotateYMatrix(HALFPI));
			axisControl->SetValue(t, &packet);
			break;
		}
		case Y_AXIS:
		{
			auto packet = SetXFormPacket(RotateXMatrix(-HALFPI));
			axisControl->SetValue(t, &packet);
			break;
		}
		case Z_AXIS:
		{
			auto packet = SetXFormPacket(Matrix3(TRUE));
			axisControl->SetValue(t, &packet);
			break;
		}
		}
...
void SurfrevMod::BuildPatchFromShape(TimeValue t,ModContext &mc, ObjectState * os, const Matrix3 &axis, PatchMesh &pmesh) {
	ShapeObject *shape = (ShapeObject *)os->obj;
...
//DebugPrint(_T("Making patch %d: %d (%d %d) %d (%d %d) %d (%d %d) %d (%d %d)\n"),np, a, ab, ba, b, bc, cb, c, cd, dc, d, da, ad);
	...
	if(degrees < 360.0f && anyClosed && (capStart || capEnd)) {
		...
		if(capStart) {
			...
			if(texturing)
				MakePatchCapTexture(pmesh, Inverse(mat1), oldPatches, pmesh.numPatches, usePhysUVs);
			}
		if(capEnd) {
			int baseVert = 0;
			int baseVec = 0;
			...
			// If texturing, create the texture patches and vertices
			if(texturing)
				MakePatchCapTexture(pmesh, Inverse(mat2), oldPatches, pmesh.numPatches, usePhysUVs);
			}
		}
