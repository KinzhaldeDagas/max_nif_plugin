...
class MeltMod : public SimpleMod {
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_MELTMOD) : _T("Melt"); }
		virtual Class_ID ClassID() { return Class_ID(MELT_ID1,MELT_ID2);}
		void BeginEditParams( IObjParam  *ip, ULONG flags,Animatable *prev);
		void EndEditParams( IObjParam *ip,ULONG flags,Animatable *next);
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_MELTMOD) : _T("Melt"); }
...
static ParamUIDesc descParam[] = {
	...
	ParamUIDesc(PB_NEGAXIS,TYPE_SINGLECHECKBOX,IDC_NEGATIVE),
};
...
MeltMod::MeltMod() : SimpleMod()
	{
	ReplaceReference(SIMPMOD_PBLOCKREF, CreateParameterBlock(descVer4, PBLOCK_LENGTH, CURRENT_VERSION));
...
Point3 MeltDeformer::Map(int i, Point3 p) const
{
	...
	if (xw == 0.0 && yw == 0.0 && zw == 0.0)
		xw = yw = zw = 1.0f; // Kill singularity for XW,YW,ZW
	if (x == 0.0 && y == 0.0 && z == 0.0)
		x = y = z = 1.0f; // Kill singularity for XYZ
	...
	if (axis == 0)
		if (p.z < (bbox.pmin.z + zbr))
			goto skipmelt;
	if (axis == 1)
		if (p.y < (bbox.pmin.y + zbr))
			goto skipmelt;
	if (axis == 2)
		if (p.x < (bbox.pmin.x + zbr))
			goto skipmelt;


	if (axis == 0)
		realmax = (float)hypot((bbox.pmax.x - cx), (bbox.pmax.y - cy));
	if (axis == 1)
		realmax = (float)hypot((bbox.pmax.x - cx), (bbox.pmax.z - cz));
	if (axis == 2)
		realmax = (float)hypot((bbox.pmax.z - cz), (bbox.pmax.y - cy));
...
bool MeltDeformer::IsThreadSafe() const
{
	return true;
}
...
TSTR MeltMod::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_MELTAMT:	return localized ? GetString(IDS_MELTAMOUNT)  : _T("Melt Amount");
		case PB_YB:			return localized ? GetString(IDS_SPREAD)      : _T("Spread %");
		case PB_ZB:			return localized ? GetString(IDS_CUTOFF)      : _T("Cut Off (obsolete)");
		case PB_CONFINE:	return localized ? GetString(IDS_CONFINE)     : _T("Confine To Gizmo");
		case PB_VISTYPE:	return localized ? GetString(IDS_SOLIDITY)    : _T("Solidity Preset");
		case PB_VISVAL:		return localized ? GetString(IDS_SOLIDITYVAL) : _T("Solidity Custom Value");
		case PB_AXIS:		return localized ? GetString(IDS_AXIS)        : _T("Axis");
		case PB_NEGAXIS:	return localized ? GetString(IDS_NEGAXIS)     : _T("Negative Axis");
...
class DecayClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Melt"); }
