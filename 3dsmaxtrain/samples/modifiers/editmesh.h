...
#include "../Include/MeshCommandIDs.h"
...
const ChannelMask EDITMESH_CHANNELS = (GEOM_CHANNEL|SELECT_CHANNEL|SUBSEL_TYPE_CHANNEL|DISP_ATTRIB_CHANNEL|TOPO_CHANNEL|TEXMAP_CHANNEL|VERTCOLOR_CHANNEL);
...
class EditMeshMod : public Modifier, public IMeshSelect, public MeshDeltaUser,
							 public ISubMtlAPI, public AttachMatDlgUser, public EditSSCB {
public:
	...
	static ActionMeshCB*	mpActions;

	static float normScale;
	static bool showVNormals, showFNormals;
	static BOOL deleteIsolatedVerts;
	...
	static BOOL weldBoxSizeInitialized;
	...
	void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_RB_EDITMESHMOD) : _T("EditMeshMod"); }
...
public:
	...
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_EDITMESH) : _T("Edit Mesh"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Edit Mesh"); }
	...
	ActionTable* 	GetActionTable(int i) {return GetEditMeshActions ();}
};
...
public:
	...
	void Invalidate (ChannelMask channels,BOOL meshValid=TRUE);
	BOOL MeshCached(TimeValue t);
	void UpdateCache(TimeValue t, TriObject *triOb);

	// LocalModData
	void* GetInterface(ULONG id) override { if (id == I_MESHSELECTDATA) return(IMeshSelectData*)this; else return LocalModData::GetInterface(id); }

	// IMeshSelectData methods:
	BitArray GetVertSel() override { return mdelta.vsel; }
	BitArray GetFaceSel() override { return mdelta.fsel; }
	BitArray GetEdgeSel() override { return mdelta.esel; }
	BitArray GetSel (int nsl);
	void AddVertHide(const BitArray set, IMeshSelect *mod, TimeValue t);	// (Not an IMeshSelectData method)
	void ClearVertHide(IMeshSelect *mod, TimeValue t);	// (Not an IMeshSelectData method)
	void SetVertSel(const BitArray &set, IMeshSelect *mod, TimeValue t) override;
	void SetFaceSel(const BitArray &set, IMeshSelect *mod, TimeValue t) override;
	void SetEdgeSel(const BitArray &set, IMeshSelect *mod, TimeValue t) override;
	void SetSel (int nsl, const BitArray &set, IMeshSelect *mod, TimeValue t);
	GenericNamedSelSetList & GetNamedVertSelList () override { return selSet[NS_VERTEX]; }
	GenericNamedSelSetList & GetNamedEdgeSelList () override { return selSet[NS_EDGE]; }
	GenericNamedSelSetList & GetNamedFaceSelList () override { return selSet[NS_FACE]; }
	...
	void ApplyMeshDelta (MeshDelta & md, MeshDeltaUser *mdu, TimeValue t) override;
	MeshDelta *GetCurrentMDState () override { return &mdelta; }
	void MoveSelection(int level, TimeValue t, Matrix3& partm, Matrix3& tmAxis, Point3& val, BOOL localOrigin) override;
	void RotateSelection(int level, TimeValue t, Matrix3& partm, Matrix3& tmAxis, Quat& val, BOOL localOrigin) override;
	void ScaleSelection(int level, TimeValue t, Matrix3& partm, Matrix3& tmAxis, Point3& val, BOOL localOrigin) override;
	void ExtrudeSelection(int level, BitArray* sel, float amount, float bevel, BOOL groupNormal, Point3* direction) override;
};
...
public:
	...
	int ID() { return CID_MESH_CREATEVERT; }
...
class CreateFaceMouseProc : public CreateFaceMouseProcTemplate {
	EditMeshMod *m_EMesh;
protected:
	TSTR GetFaceCreationUndoMessage() {return GetString (IDS_RB_BUILDFACE);}
	TSTR GetFailedFaceCreationMessage() {return GetString(IDS_RB_DUPFACEWARNING);}
	TSTR GetFailedFaceCreationTitle() {return GetString(IDS_RB_EDITMESHTITLE);}
	...
	void DisplayDataChanged() {m_EMesh->LocalDataChanged(DISP_ATTRIB_CHANNEL);}
	
	void ShowExistingVertexCursor() { SetCursor(GetInterface()->GetSysCursor(SYSCUR_SELECT));}
	void ShowCreateVertexCursor();

public:
	CreateFaceMouseProc(EditMeshMod* in_e, IObjParam* in_i);
	EditMeshData* GetMeshData();
};
...
public:
	...
	int Class() { return MODIFY_COMMAND; }
	int ID() { return CID_MESH_BUILDFACE; }
...
public:
	...
	const TCHAR *dialogTitle() { return GetString(IDS_EM_ATTACH_LIST); }
...
public:
	...
	int Class() { return MODIFY_COMMAND; }
	int ID() { return CID_MESH_DIVIDEEDGE; }
	MouseCallBack *MouseProc(int *numPoints) {*numPoints=1; return &proc;}
	ChangeForegroundCallback *ChangeFGProc() {return &fgProc;}
	BOOL ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void EnterMode();
...
public:
	...
	int Class() { return MODIFY_COMMAND; }
	int ID() { return CID_MESH_DIVIDEFACE; }
	MouseCallBack *MouseProc(int *numPoints) {*numPoints=1; return &proc;}
	ChangeForegroundCallback *ChangeFGProc() {return &fgProc;}
	BOOL ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void EnterMode();
...
public:
	...
	int Class() { return MODIFY_COMMAND; }
	int ID() { return CID_MESH_TURNEDGE; }
	MouseCallBack *MouseProc(int *numPoints) {*numPoints=1; return &proc;}
	ChangeForegroundCallback *ChangeFGProc() {return &fgProc;}
	BOOL ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void EnterMode();
...
public:
	...
	int Class() { return MODIFY_COMMAND; }
	int ID() { return CID_MESH_EXTRUDE; }
...
public:
	ChamferCMode (EditMeshMod* m, IObjParam *i) :
		fgProc(m), mouseProc(&eproc,m,i), eproc(m,i) {em=m;}
	int Class() { return MODIFY_COMMAND; }
	int ID() { return CID_MESH_MCHAMFER; }
	MouseCallBack *MouseProc(int *numPoints) { *numPoints=2; return &mouseProc; }
	ChangeForegroundCallback *ChangeFGProc() { return &fgProc; }
	BOOL ChangeFG( CommandMode *oldMode ) { return oldMode->ChangeFGProc() != &fgProc; }
	void EnterMode();
...
public:
	...
	int Class() { return MODIFY_COMMAND; }
	int ID() { return CID_MESH_CUTEDGE; }
...
public:
	WeldVertCMode(EditMeshMod* mod, IObjParam *i) :
		fgProc(mod), mouseProc(&eproc,mod,i), eproc(mod,i) {em=mod;}
	int Class() { return MODIFY_COMMAND; }
	int ID() { return CID_MESH_WELDVERT; }
	MouseCallBack *MouseProc(int *numPoints) { *numPoints=2; return &mouseProc; }
	ChangeForegroundCallback *ChangeFGProc() { return &fgProc; }
	BOOL ChangeFG( CommandMode *oldMode ) { return oldMode->ChangeFGProc() != &fgProc; }
	void EnterMode();
...
public:
	...
	int Class() { return MODIFY_COMMAND; }
	int ID() { return CID_MESH_FLIPNORM; }
	MouseCallBack *MouseProc(int *numPoints) {*numPoints=1; return &proc;}
	ChangeForegroundCallback *ChangeFGProc() {return &fgProc;}
	BOOL ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void EnterMode();
...
public:
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("SingleRefMakerMeshMNode"); } // from Animatable
};
...
public:	
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("SingleRefMakerMeshMMtl"); } // from Animatable
};
