 ...
 *<
	...
	HISTORY: created 10 November, 1995
			...
			12/28/2020 - Added End Cap Cache (N. Hoang, M. Kaustinen)
...
#define MIN_SEGS 1.0f
#define MAX_SEGS 10000.0f
...
// Cap Cache
class ExtrudeLocalData : public LocalModData
{
public:
	ExtrudeLocalData()
		: mCapType(0)
	...
	~ExtrudeLocalData() = default;

	LocalModData* Clone()
	{
		ExtrudeLocalData* d = new ExtrudeLocalData();
		return d;
	}

	void	Invalidate()
	{
		mCapValidity.SetEmpty();
		mHierValidity.SetEmpty();
	}

	void	SetCapValidity(const Interval& v, const int capType)
	{
		mCapValidity = v;
		mCapType = capType;
	}

	void	SetHierachyValidity(const Interval& v)
	{
		mHierValidity = v;
	}

	bool	IsCapValid(const TimeValue t, const int capType)
	{
		return (mCapValidity.InInterval(t) && (mCapType == capType));
	}

	bool	IsHierarchyValid(const TimeValue t)
	{
		return (mHierValidity.InInterval(t));
	}

	const MeshCapInfo& GetMeshCapInfo() const
	{
		DbgAssert(!mCapValidity.Empty());
		return mCapInfo;
	}

	MeshCapInfo& GetMeshCapInfo()
	{
		return mCapInfo;
	}

	ShapeHierarchy& GetShapeHierarchy()
	{
		return mHierarchy;
	}

private:
	ShapeHierarchy mHierarchy;
	MeshCapInfo mCapInfo;
	Interval	mHierValidity;
	Interval	mCapValidity;
	int mCapType;
};
...
class ExtrudeMod
		: public Modifier
		, public RealWorldMapSizeInterface
{
...
public:
	...
	{
		s = TSTR(_T("ExtrudeMod"));
	}
	virtual Class_ID ClassID()
	{
		return Class_ID(EXTRUDEOSM_CLASS_ID, 0);
	}
	RefTargetHandle Clone(RemapDir& remap);
	const TCHAR* GetObjectName(bool localized) const override
	{
		return localized ? GetString(IDS_DB_EXTRUDE) : _T("Extrude");
	}
	IOResult Load(ILoad* iload);
	IOResult Save(ISave* isave);

	ExtrudeMod(BOOL create);
	virtual ~ExtrudeMod();
	...
	{
		// Did the Geom Channel change, if so, invalidate object cache
		if ((partID | TOPO_CHANNEL) || (partID | GEOM_CHANNEL))
		{
			ExtrudeLocalData* hs = static_cast<ExtrudeLocalData*>(mc->localData);
			if (hs)
				hs->Invalidate();
		}
	}
...
void ExtrudeDlgProc::UpdateRWS()
{
	BOOL usePhysUVs = mod->GetUsePhysicalScaleUVs();
	CheckDlgButton(mhWnd, IDC_REAL_WORLD_MAP_SIZE, usePhysUVs);
	EnableWindow(GetDlgItem(mhWnd, IDC_REAL_WORLD_MAP_SIZE), mod->HasUVW());
}

INT_PTR ExtrudeDlgProc::DlgProc(TimeValue t, IParamMap* map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
	case WM_INITDIALOG:
...
public:
	...
	void* Create(BOOL loading = FALSE)
	{
		...
		return new ExtrudeMod(!loading);
	}
	const TCHAR* ClassName()
	{
		return GetString(IDS_RB_EXTRUDE_CLASS);
	}
...
static ParamUIDesc descParam[] = {
	// Amount
	ParamUIDesc(PB_AMOUNT, EDITTYPE_UNIVERSE, IDC_AMOUNT, IDC_AMOUNTSPINNER, MIN_AMOUNT, MAX_AMOUNT, 0.5f),

	// Segments
	ParamUIDesc(PB_SEGS, EDITTYPE_INT, IDC_SEGMENTS, IDC_SEGMENTSPINNER, MIN_SEGS, MAX_SEGS, 1.0f),

	// Capping start
	ParamUIDesc(PB_CAPSTART, TYPE_SINGLECHECKBOX, IDC_CAPSTART),

	// Capping end
	ParamUIDesc(PB_CAPEND, TYPE_SINGLECHECKBOX, IDC_CAPEND),
	...
	// Texture coords
	ParamUIDesc(PB_MAPPING, TYPE_SINGLECHECKBOX, IDC_GENMAPPING),

	// Multi material IDs
	ParamUIDesc(PB_GEN_MATIDS, TYPE_SINGLECHECKBOX, IDC_GEN_MATIDS),

	// Shape material IDs
	ParamUIDesc(PB_USE_SHAPEIDS, TYPE_SINGLECHECKBOX, IDC_USE_SHAPE_IDS),

	// Smoothing
	ParamUIDesc(PB_SMOOTH, TYPE_SINGLECHECKBOX, IDC_SMOOTH)
};
...
void ExtrudeMod::ModifyObject(TimeValue t, ModContext& mc, ObjectState* os, INode* node)
{
	...
	case NURBS_OUTPUT:
	{
		...
		Object* nobj = CreateNURBSExtrudeShape(ip, GetString(IDS_RB_EXTRUDE), t, shape, amount, capStart, capEnd,
				capType, texturing, genMatIds, useShapeIDs);
		if (suspended)
		{
			theHold.Resume();
		}
...
void ExtrudeMod::BeginEditParams(IObjParam* ip, ULONG flags, Animatable* prev)
{
	...
	pmapParam = CreateCPParamMap(descParam, PARAMDESC_LENGTH, pblock, ip, hInstance, MAKEINTRESOURCE(IDD_EXTRUDEPARAM),
		...
		const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
	...
	case REFMSG_GET_PARAM_NAME_LOCALIZED:
	{
		...
		gpn->name = GetParameterName(gpn->index, true);
		return REF_HALT;
	}
	...
	{
		GetParamName* gpn = (GetParamName*)partID;
		gpn->name = GetParameterName(gpn->index, false);
		return REF_HALT;
	}
	}
	return (REF_SUCCEED);
...
TSTR ExtrudeMod::GetParameterName(int pbIndex, bool localized)
{
	switch (pbIndex)
	{
	case PB_AMOUNT:
		return localized ? GetString(IDS_TH_AMOUNT) : _T("Amount");
	case PB_SEGS:
		return localized ? GetString(IDS_TH_SEGMENTS) : _T("Segments");
	default:
		return TSTR(_T(""));
	}
}
...
public:
	ParamBlockPLCB* cb;
	ExtrudePostLoadCallback(ParamBlockPLCB* c)
	{
		cb = c;
	}
...
void ExtrudePostLoadCallback::proc(ILoad* iload)
{
	...
	DWORD oldVer = ((ExtrudeMod*)(cb->GetTarget()))->pblock->GetVersion();
...
BOOL ExtrudeMod::GetUsePhysicalScaleUVs()
{
...
IOResult ExtrudeMod::Load(ILoad* iload)
{
	...
	iload->RegisterPostLoadCallback(
			new ExtrudePostLoadCallback(new ParamBlockPLCB(versions, NUM_OLDVERSIONS, &curVersion, this, 0)));
	return IO_OK;
}

static void MakeMeshCapTexture(Mesh& mesh, const Matrix3& itm, int fstart, int fend, BOOL usePhysUVs)
{
	if (fstart == fend)
		return;
...
void ExtrudeMod::BuildMeshFromShape(TimeValue t, ModContext& mc, ObjectState* os, Mesh& mesh, BOOL simple)
{
	...
	// Get our underlying object's validity interval to aid in caching
	Interval	mGeoValidity = shape->ChannelValidity(t, GEOM_CHAN_NUM);
	mGeoValidity &= shape->ChannelValidity(t, TOPO_CHAN_NUM);

	// Use localData to access our cache
	if (mc.localData == nullptr)
	{
		mc.localData = new ExtrudeLocalData();
	}

	ExtrudeLocalData* hs = static_cast<ExtrudeLocalData*>(mc.localData);
	DbgAssert(hs);

	if (!(hs->IsHierarchyValid(t)))
	{
		//ShapeHierarchy& hier = hs->GetShapeHierarchy();
		pShape.OrganizeCurves(t, &(hs->GetShapeHierarchy())); //&hier);
		hs->SetHierachyValidity(mGeoValidity);
	}
	else {
		pShape.cachedHier = hs->GetShapeHierarchy();
		pShape.hierCacheValid = TRUE;
	}
	
	// Need to flip the reversed curves in the shape!
	pShape.Reverse(hs->GetShapeHierarchy().reverse);
	...
	{	
		// Rebuild our cache if necessary
		if (!hs->IsCapValid(t, capType))
		{
			pShape.MakeCap(t, hs->GetMeshCapInfo(), capType);

			// Get our underlying object's validity interval to set our cache interval
			Interval	mGeoValidity = os->obj->ChannelValidity(t, GEOM_CHAN_NUM);
			mGeoValidity &= os->obj->ChannelValidity(t, TOPO_CHAN_NUM);
			hs->SetCapValidity(mGeoValidity, capType);
		}

		const MeshCapInfo& capInfo = hs->GetMeshCapInfo();
...
static void MakePatchCapTexture(PatchMesh& pmesh, const Matrix3& itm, int pstart, int pend, BOOL usePhysUVs)
{
	if (pstart == pend)
		return;
...
void ExtrudeMod::BuildPatchFromShape(TimeValue t, ModContext& mc, ObjectState* os, PatchMesh& pmesh)
{
	...
	for (poly = 0; poly < polys; ++poly)
	{
		...
		{
			int sm = 0;
			BOOL firstSmooth = (spline->GetLineType(0) == LTYPE_CURVE && spline->GetLineType(segs - 1) == LTYPE_CURVE &&
									   (spline->GetKnotType(0) == KTYPE_AUTO || spline->GetKnotType(0) == KTYPE_BEZIER))
					? TRUE
					: FALSE;
			for (seg = 0; seg < segs; ++seg, vec += 4)
			{
				...
				// If the vertex is not smooth, go to the next group!
				if (seg > 0 &&
						!(spline->GetLineType(prevseg) == LTYPE_CURVE && spline->GetLineType(seg) == LTYPE_CURVE &&
								(spline->GetKnotType(seg) == KTYPE_AUTO || spline->GetKnotType(seg) == KTYPE_BEZIER)))
				{
					sm++;
					if (sm > 2)
						sm = 1;
				...
				pmesh.MakeQuadPatch(np, a, ab, ba, b, bc, cb, c, cd, dc, d, da, ad, vec, vec + 1, vec + 2, vec + 3,
						smooth ? smoothGroup : 0);
