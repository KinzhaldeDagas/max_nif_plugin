...
void MorphR3::Clamp_chanNum()
{
	...
	int bankSizeLimit = (int)chanBank.size() - 10;
	if (chanNum > bankSizeLimit)
	{
		//we only expand the bank size if there is data in the last 10 channels
		bool hasData = false;
		int start = (int)chanBank.size() - 10;
		for (int i = start; i < chanBank.size(); i++)
		{
	...
	bankSizeLimit = (int)chanBank.size() - 10;
	if (chanNum > bankSizeLimit) chanNum = (int)chanBank.size() - 10;
}
...
void MorphR3::VScroll(int code, short int cpos ) {
	...
	int maxPos = (int)chanBank.size();
	SetScrollRange((HWND)vScr, SB_CTL, 0, maxPos, TRUE);
...
float MorphR3::GetIncrements()
{
	...
	pblock->GetValue(PB_AD_VALUEINC, 0, idTmp, valid);
	...
	if(hwChannelParams) {
		...
		if(bank.mName != emptychannel)
		{
			for(int i=0; i<=numtargs; i++) {
				if(i==0) refnum = 101+channum%100;
				else refnum = 200+i+channum%100*MAX_PROGRESSIVE_TARGETS;

				int refOffset = GetRefIDOffset(channum);

				node = (INode*) GetReference(refnum+refOffset);
				if(node) {
					name = node->GetName();
					SendDlgItemMessage( hwChannelParams, IDC_TARGETLIST, 
...
void MorphR3::ChannelOp(int targ, int src, int flags)
{
	if(flags==OP_MOVE)
	{
		...
		//
		int srcOffset = GetRefIDOffset(src);
		int targOffset = GetRefIDOffset(targ);
		INode * srcNode  = (INode*) GetReference( 101+src%100+srcOffset );
		SetReference(101+targ%100+targOffset,NULL); // null this out so that the replace reference will work. The code: chanBank[targ] = chanBank[src]; copied the pointer we are setting to where it is being set.
		ReplaceReference(101+targ%100+targOffset,srcNode);
		...
		for(int i=1; i<=chanBank[targ].mNumProgressiveTargs; i++) {
			targRefnum = ((targ%100) * MAX_PROGRESSIVE_TARGETS)+i+200	;
			srcRefnum = ((src%100) * MAX_PROGRESSIVE_TARGETS)+i+200	;
			SetReference(targRefnum+targOffset,NULL); // null this out so that the replace reference will work
			ReplaceReference(targRefnum+targOffset,GetReference( srcRefnum+srcOffset ));
		}
		...
		if(chanBank[targ].mConnection)
		{
			...
			std::vector<TargetCache>& target_caches(chanBank[targ].TargetCaches());
			for(int i=0; i<chanBank[targ].mNumProgressiveTargs; i++) 
			{ 
				target_caches[i].Init(target_caches[i].mTargetINode); 
			}
		}
	...
	else if(flags==OP_SWAP)
	{
		...
		// protect controller from deletion
		bool lockTargControl = targControl && targControl->TestAFlag(A_LOCK_TARGET) == 0;
		if (lockTargControl)
			targControl->SetAFlag(A_LOCK_TARGET);

		morphChannel targBank = chanBank[targ];
		targBank.CopyTargetPercents( chanBank[targ] );
		INode * targNode  = (INode*) GetReference( 101+targ%100+targOffset );
		...
		for(int i=1; i<=targBank.mNumProgressiveTargs; i++) {
			targRefnum = (targ%100 * MAX_PROGRESSIVE_TARGETS)+i+200	;
			progTargs[i-1] = (INode *) GetReference( targRefnum + targOffset);
		}
		...
		if (lockTargControl)
			targControl->ClearAFlag(A_LOCK_TARGET);

		SetReference(101+src%100+srcOffset,NULL); // null this out so that the replace reference will work. The code: chanBank[targ] = chanBank[src]; copied the pointer we are setting to where it is being set.
		ReplaceReference(101+src%100+srcOffset,targNode);
        ...
        for( int i=1; i<=targBank.mNumProgressiveTargs; ++i) {
			srcRefnum = (src%100 * MAX_PROGRESSIVE_TARGETS)+i+200	;
			SetReference(srcRefnum+srcOffset,NULL); // null this out so that the replace reference will work
			ReplaceReference(srcRefnum+srcOffset, progTargs[i-1] );
		}

		if(chanBank[src].mConnection)
		{
			chanBank[src].buildFromNode(chanBank[src].mConnection, FALSE, GetCOREInterface()->GetTime());
			std::vector<TargetCache>& target_caches(chanBank[src].TargetCaches());
...
float MorphR3::GetCurrentTargetPercent() const
{
	const morphChannel& bank = CurrentChannel();
	if (bank.iTargetListSelection == 0)
		return bank.mTargetPercent;

	return (bank.iTargetListSelection > 0 && bank.iTargetListSelection <= bank.mNumProgressiveTargs)
			? bank.TargetCaches()[bank.iTargetListSelection - 1].mTargetPercent
			: 0.0f;
}
...
void MorphR3::SetCurrentTargetPercent(const float& fval)
{
	morphChannel& bank = CurrentChannel();
	if (bank.iTargetListSelection == 0)
		bank.mTargetPercent = fval;
	else if (bank.iTargetListSelection > 0 && bank.iTargetListSelection <= bank.mNumProgressiveTargs)
		bank.TargetCaches()[bank.iTargetListSelection - 1].mTargetPercent = fval;
}