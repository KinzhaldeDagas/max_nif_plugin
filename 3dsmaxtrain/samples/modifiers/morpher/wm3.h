...
#define MR3_NUM_CHANNELS 100 // NOT A LIMIT ANY MORE JUST THE NUMBER OF CHANNELS THAT WE ADD WHEN WE NEED MORE
#define MAX_PROGRESSIVE_TARGETS 25
#define REFS_PER100_CHANNELS 3000 // really 2700 but this makes it nice to keep the math straight
...
#define MR3_NONLOCALIZED_NAME 0x0121
...
#define MR3_MC_CHUNKCOUNT 0x0191
#define MR3_MC_SUBCHUNK \
	0x0200 /// NOTE NO LOAD CHUNKS ID MUST BE PAST THIS SINCE THIS IS THE START OF THE ARRAY OF CHANNELS WHICH IS
		   /// UNDEFINED
...
public:
	...
	void Init(INode* nd);
	void Reset(void);
	const Point3& GetPoint(const long index) const
	{
		if (index >= 0 && index < mNumPoints)
			return mTargetPoints[index];
		return junkpoint;
	}
...
struct delta_validity {
	bool any_deltas_valid = true;
	const std::vector<uint8_t>* valid_deltas;
};
...
*/
class morphChannel : public IMorpherChannel
{
public:
	// Construct/Destruct
	~morphChannel();
	...
	TSTR mName;
	// The internal name of the morphchannel
	TSTR mNonLocalizedName;
	...
	// inModify means that the we are inside an evaluation which limits what we should do like update tghe UI and send
	// notify messages
	MorphExport void buildFromNode(
			INode* node, BOOL resetTime = TRUE, TimeValue t = 0, BOOL picked = FALSE, BOOL inModify = FALSE);
	...
	float GetTargetPercent(const int which) const;
	void ReNormalize();
	void CopyTargetPercents(const morphChannel& chan);

	// Channel getters/setters
	MorphExport const std::vector<Point3>& Points() const;
	MorphExport std::vector<Point3>& Points();

	MorphExport const std::vector<Point3>& Deltas() const;
	MorphExport std::vector<Point3>& Deltas();

	MorphExport const std::vector<double>& Weights() const;
	MorphExport std::vector<double>& Weights();

	const std::vector<TargetCache>& TargetCaches() const;
	std::vector<TargetCache>& TargetCaches();

	//// Computes delta validity info.
	//// This call is expensive, but its results are cached internally.
	//// The MUST be called before GetValidDeltas.
	//delta_validity ComputeValidDeltas();

	//delta_validity GetValidDeltas() const;

	// from interface IMorpherChannel interface:
	bool IsActive() const override;
	const TCHAR* GetName(bool localized) override;
	void SetName(const TCHAR* pName, bool localized) override;
	int NumPoints() const override;
	INode* GetTarget(int pTargetIndex) override;
	int GetTargetCount() const override;
	Point3 GetTargetPoint(int pTargetIndex, int pId) override;	

	bool SetTargetPercent(int pTargetIndex, double pTargetPercent) override;

	double GetTargetPercent(int pTargetIndex) override;
	Point3 GetPoint(int pId) override;
	void SetPoint(int pId, Point3& pPoint) override;
	Point3 GetDelta(int pId) override;
	void SetDelta(int pId, Point3& pPoint) override;
	double GetInitPercent() override;
	double GetWeight(int pId) override;
	void SetWeight(int pId, double pValue) override;
	INode* GetConnection() override;
	void SetConnection(INode* pNode) override;

	Control* GetControl() override;	
	bool IsProgressive() const override;
	int NumTargets() const override;
	bool GetBaseDeltas(Tab<Point3>& deltas) const override;
	bool GetProgressiveTargetDeltas(int pTargetIndex, Tab<Point3>& deltas) const override;
	bool GetAnimationKeysData(Tab<long>& vKeyTime, Tab<float>& vKeyValue) override;
	void Reset(bool active, bool modded, int numPoints) override;
...
private:
	...
	std::vector<TargetCache> mTargetCaches;
};
...
public:
	...
	MorphExport BOOL RightClick(IObjParam* ip, ViewExp* vpt)
	{
		return TRUE;
	}
	...
	MorphExport PickNodeCallback* GetFilter()
	{
		return this;
	}
...
public:
	...
	void MakeSelectionCache(Object* obj);
	...
	MorphExport bool AreWeCached() const;
...
public:
	...
	MorphExport static IObjParam* ip;
	...
	TimeValue mSelectionCacheTime = INFINITY;
	...
	{
		s = localized ? MSTR(GetString(IDS_CLASS_NAME)) : MSTR(L"Morpher");
	}
	...
	{
		return localized ? GetString(IDS_CLASS_NAME) : _T("Morpher");
	}
	...
	MorphExport void Add100Channels();
	MorphExport unsigned int GetRefIDOffset(int chanNum);
	MorphExport unsigned int GetChannelOffset(int refID);
...
public:
	...
	{
		return chanNum + chanSel;
	}
	const morphChannel& CurrentChannel() const
	{
		return chanBank[chanNum + chanSel];
	}
	morphChannel& CurrentChannel()
	{
		return chanBank[chanNum + chanSel];
	}

	float GetCurrentTargetPercent() const;
	...
	{
		return (200 + targNum + ((chanNum % 100) * MAX_PROGRESSIVE_TARGETS));
	}
	...
	// helper function:
	int GetChannelId(morphChannel* pCh)
	{
		if (pCh)
		{
	...
	void SetNumChannels(int ct) override;
	int NumChannels() const override;
	IMorpherChannel* GetChannel(int pChnlId, bool pDefaultInit = false) override;

	bool AddProgressiveMorph(int pChnlId, INode* pShapeTarget) override;
	void DeleteAllChannels() override;
	void SetMaxInterface(IObjParam* pMaxInterface) override;
	bool MakeCache(Object* pObj) override;
	void NukeCache() override;
...
//private:
	// Caches for performance, reuse memory.
	std::vector<float> delta_x_cache;
	std::vector<float> delta_y_cache;
	std::vector<float> delta_z_cache;
...
public:
	...
	// Adds a 100 extra channels on top of what there is
	void Add100Channels();
	// given a mat index return the correct ref id
	int GetRefID(int matID);
	// set the number of channels to match the modidier
	void SyncChannels(int ct);
	...
	void GetClassName(MSTR& s, bool localized) const override
	{
		s = localized ? GetString(IDS_MORPHMTL) : _T("Morpher");
	}
	...
	{
		if (i == 100)
			return localized ? GetString(IDS_MTL_BASENAME) : _T("Base ");
		TCHAR s[25] = {};
		...
		if (morphp)
		{
			if (i > 100)
				i--;
			if (i < morphp->chanBank.size())
			{
				_stprintf_s(s, _T("Mtl %i (%s)"), i + 1,
						localized ? morphp->chanBank[i].mName.data() : morphp->chanBank[i].mNonLocalizedName.data());
				return s;
			}
		}
		else
		{
			if (i > 100)
				i--;
			_stprintf_s(s, localized ? GetString(IDS_MTL_MAPNAME) : _T("Mat %i "), i + 1);
			return s;
		}
...
public:
	// create a param block description based on the number of channels
	// channel100Count is the numberOfChannels/100
	ParamBlockDescID* MakeParamDescr(int channel100Count);
...
class GetMorphMod : public PickObjectProc
{
public:
	M3Mat* mp;
...
public:
	MorphR3* mp;
	MorphR3PostLoadCallback()
	{
		mp = NULL;
	}
	MorphR3PostLoadCallback(MorphR3* m)
	{
		mp = m;
	}
...
struct XChannelData
{
	std::vector<std::wstring> vTargetsNames;
...
bool ExportMorpherChannelData(INode* lMaxNode, IMorpherChannel* pChannel, std::vector<std::wstring>& vTargetsNames,
		std::vector<std::pair<long, float>>& vTargetsKeys, std::vector<double>& vTargetsPercents,
		std::vector<std::vector<Point3>>& vTargetsData);
MorphExport bool ExportMorpherData(INode* pMaxNode);
