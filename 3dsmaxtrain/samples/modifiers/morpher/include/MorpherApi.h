...
public:
	...
	*/
	...
	/**
		Sets the number of channels using inrecements of 100, this will only grow the number channels
		\param numChannels the number of channels to add
	*/
	MorphExport void SetNumChannels(int numChannels);
...
public:
	...
	*/
	MorphExport const MCHAR* GetName(bool localized);

	/**
		Renames the morph channel. 
		\param newName - the name to be set
		\param localized - If true, then the localized name is updated. Otherwise the nonlocalized name is updated.
	*/
	MorphExport void SetName(const MCHAR* newName, bool localized);
	...
	*/
	...
	/** 
		Bubbles up or down the selected morph by it weight to keep the list sorted
		\param morphIndex The index of the morph target the bubble should start from. 		
		\return True if succeeded.
	*/
	MorphExport bool SortProgressiveMorphTargets(int morphIndex);
	...
	*/
	...
	/**
		returns the total number of progressive morphs.
		\param morphIndex which morph channel
	*/
	MorphExport int GetTargetCacheSize() const;

	/**
		returns the connection node of the specified progressive morph.
		\param targetIndex index of the progressive morph
	*/
	MorphExport INode* GetProgressiveMorphTargetNode(int targetIndex);
