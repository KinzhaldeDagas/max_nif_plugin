...
class M3MatClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Morpher"); }
...
//		TCHAR s[25];
	...
	//		mTex[i]->SetName(s);
		...
		ReplaceReference(100, NewDefaultMaterial(_T("MorpherMtl")));  //100 is the base material
...
TSTR M3Mat::SubAnimName(int i, bool localized)
{
					...
					for(int k=101; k<=200; k++){
						if( morphp->GetReference(j) && morphp->GetReference(j) == mp->GetReference(k) ) {
							if(morphp->hMaxWnd) 
							{
								...
								MaxSDK::MaxMessageBox(morphp->hMaxWnd, cyclic, GetString(IDS_CLASS_NAME), MB_OK);
							}
							return;
						}
					}
...
RefResult M3Mat::NotifyRefChanged(
	...
	switch (message) {
		...
		case REFMSG_GET_PARAM_NAME_NONLOCALIZED: {
			GetParamName *gpn = (GetParamName*)partID;
			TCHAR s[50];
			_stprintf(s, _T("Channel %i"), gpn->index);
			gpn->name = s;
			return REF_HALT;
			}
		}
