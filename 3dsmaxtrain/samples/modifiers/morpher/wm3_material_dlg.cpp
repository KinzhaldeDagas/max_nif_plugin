...
namespace
{
	GetMorphMod theModPickmode;
}

static INT_PTR CALLBACK PanelDlgProc(
	HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	M3MatDlg *dlg = DLGetWindowLongPtr<M3MatDlg*>(hWnd);
	switch (msg) {
	case WM_INITDIALOG:
		dlg = (M3MatDlg*)lParam;
...
void M3MatDlg::UpdateSubMtlNames() {
	...
	int subMatIndex = theMtl->listSel;
	if (subMatIndex > 100)  //the base material is at location 100 so skip it
		subMatIndex++;

	for (int i=0; i<NSUBMTL; i++) {
		if (subMatIndex == 100)
			subMatIndex++;
		Mtl *m = theMtl->GetSubMtl(subMatIndex);
		subMatIndex++;
		...
		_stprintf(s,GetString(IDS_MTL_MAPNAMEDOTS),i+1+theMtl->listSel);
		SetWindowText(label,s);
	}

	Mtl *mb = theMtl->GetSubMtl(100);
	if (mb)  nm = mb->GetFullName(true);
	else  nm = GetString(IDS_DS_NONE);
...
void M3MatDlg::Clamp_listSel()
{
	...
	if(theMtl->listSel>=theMtl->mTex.Count()-10) theMtl->listSel = theMtl->mTex.Count()-10;
	int min = 0,max = 0;
	HWND vScr = GetDlgItem(hPanel,IDC_LISTSCROLL);
	GetScrollRange((HWND)vScr, SB_CTL,&min,&max);
	if (max < theMtl->mTex.Count()-10)
		SetScrollRange((HWND)vScr, SB_CTL,0,theMtl->mTex.Count()-10,TRUE);
...
void M3MatDlg::UpdateMorphInfo(int upFlag)
{
	...
	if(theMtl->morphp)
	{
		...
		if (theMtl->mTex.Count() < theMtl->morphp->chanBank.size() )
		{
			while (theMtl->mTex.Count() < theMtl->morphp->chanBank.size() )
				theMtl->Add100Channels();
		}
		...
		int index = 0;
		for( i=0;i<theMtl->morphp->chanBank.size();i++)
		{
			if (i == 100) continue; //skip the base material
			if(theMtl->mTex[i]) 
				theMtl->mTex[i]->SetName(theMtl->morphp->chanBank[index].mName);
			index++;
		}
		...
		if(upFlag==UD_LINK)
		{
			...
			{
				if (i == 100) continue; //skip the base material
				Control *c = theMtl->morphp->chanBank[index].cblock->GetController(0);
	...
	HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_MORPH_MAT10:
			{
				int matIndex = (id-IDC_MORPH_MAT1)+theMtl->listSel;
				if (matIndex >= 100)
					matIndex++;
				PostMessage(hwmedit,WM_SUB_MTL_BUTTON, matIndex ,(LPARAM)theMtl);
			}
			break;
...
void M3MatDlg::ReloadDialog()
{
	...
	int q = 0;
	if (theMtl->pblockMat)
		theMtl->pblockMat->GetValue(100, ip->GetTime(), q, valid);
