...
wm3_SwapMorph_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_SwapMorph [Morpher Modifier] [Morph Index] [MorphIndex]"));
	integer_type_check(arg_list[2], _T("WM3_SwapMorph [Morpher Modifier] [Morph Index] [MorphIndex]"));
	...
	int index = morphIndexA;
	if (morphIndexB > index)
			index = morphIndexB;
	while (index >= (int)mp->chanBank.size())
	{
		mp->Add100Channels();
	}
...
wm3_MoveMorph_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MoveMorph [Morpher Modifier] [Morph Source Index] [Morph Destination Index]"));
	integer_type_check(arg_list[2], _T("WM3_MoveMorph [Morpher Modifier] [Morph Source Index] [Morph Destination Index]"));
...
wm3_GetProgressiveMorphTension_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_GetProgressiveMorphTension [Morpher Modifier] [Morph Index]"));
	...
	morphIndex--;
	if(morphIndex>=(int)mp->chanBank.size()) morphIndex = (int)mp->chanBank.size()-1;  //to get these the chan has to exist first
	if(morphIndex<0) morphIndex = 0;

	
	float tension = maxMorpher.GetMorphChannel(morphIndex).GetProgressiveMorphTension();
	return_value (Float::intern(tension));	
}
...
wm3_SetProgressiveMorphTension_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_SetProgressiveMorphTension [Morpher Modifier] [Morph Index] [Tension]"));
	float_type_check(arg_list[2], _T("WM3_SetProgressiveMorphTension [Morpher Modifier] [Morph Index] [Tension]"));
...
wm3_GetProgressiveMorphNode_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_GetProgressiveMorphNode [Morpher Modifier] [Morph Index]  [Progressive Morph Index]"));
	integer_type_check(arg_list[2], _T("WM3_GetProgressiveMorphNode [Morpher Modifier] [Morph Index]  [Progressive Morph Index]"));
	...
	if (node != NULL)
	{
		return_value (MAXNode::intern(node));	
	}
	return &undefined;
}
...
wm3_AddProgressiveMorphNode_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_AddProgressiveMorphNode [Morpher Modifier] [Morph Index]  [Node]"));	
	...
	int morphIndex = arg_list[1]->to_int(); 
	morphIndex--;
	if(morphIndex >= (int)mp->chanBank.size()) {
		morphIndex = (int)mp->chanBank.size()-1;
	}
	if(morphIndex<0) {
		morphIndex = 0;
	}
...
wm3_DeleteProgressiveMorphNode_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_DeleteProgressiveMorphNode [Morpher Modifier] [Morph Index] [Progressive Morph Index]"))	
	integer_type_check(arg_list[2], _T("WM3_DeleteProgressiveMorphNode [Morpher Modifier] [Morph Index] [Progressive Morph Index]"));
...
wm3_GetProgressiveMorphWeight_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_GetProgressiveMorphWeight [Morpher Modifier] [Morph Index]  [Progressive Node]"));
	...
	if (node == NULL) return_value (Float::intern(weight));	
...
wm3_SetProgressiveMorphWeight_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_SetProgressiveMorphWeight [Morpher Modifier] [Morph Index]  [Progressive Node] [Weight]"));
	type_check(arg_list[2], MAXNode, _T("WM3_SetProgressiveMorphWeight [Morpher Modifier] [Morph Index]  [Progressive Node] [Weight]"));
	float_type_check(arg_list[3], _T("WM3_SetProgressiveMorphWeight [Morpher Modifier] [Morph Index]  [Progressive Node] [Weight]"));
	...
	MaxMorphModifier maxMorpher(mp);
	const int pMorphCount = maxMorpher.GetMorphChannel(morphIndex).NumProgressiveMorphTargets();	

	int pMorphIndex = -1;
	if (node ==	maxMorpher.GetMorphChannel(morphIndex).GetMorphTarget())
	{
		pMorphIndex = 0;
	}
	else
	{
		for (int i = 0; i < maxMorpher.GetMorphChannel(morphIndex).GetTargetCacheSize(); i++)
		{
			if (node == maxMorpher.GetMorphChannel(morphIndex).GetProgressiveMorphTargetNode(i))
			{
				pMorphIndex = i + 1;
				break;
			}
...
wm3_NumberOfProgressiveMorphs_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("wm3_NumberOfProgressiveMorphs [Morpher Modifier] [Morph Index]"));
	...
	MaxMorphModifier maxMorpher(mp);
	return_value (Integer::intern(maxMorpher.GetMorphChannel(morphIndex).NumProgressiveMorphTargets()));
...
wm3_GetCurrentMarker_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(index));
}
...
wm3_SetCurrentMarker_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("wm3_SetCurrentMarker [Morpher Modifier] [Marker Index]"));
...
wm3_CreateMarker_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("wm3_CreateMarker [Morpher Modifier] [Morph Index] [Marker Name]"));
...
wm3_DeleteMarker_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("wm3_DeletMarker [Morpher Modifier] [Morph Index]"));
...
wm3_GetMarkerIndex_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_GetMarkerIndex [Morpher Modifier] [Marker Index]"));
	...
	return_value (Integer::intern(id));
}
...
wm3_GetMarkerName_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_GetMarkerName [Morpher Modifier] [Marker Index]"));
...
Value* wm3_mc_bfn_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_BuildFromNode [Morpher Modifier] [Channel Index] [Target]"));
	type_check(arg_list[2], MAXNode, _T("WM3_MC_BuildFromNode [Morpher Modifier] [Channel Index] [Target]"));

	ReferenceTarget* obj = arg_list[0]->to_modifier();
	if (!check_ValidMorpher(obj, arg_list))
	{
		return &false_value;
	}
	...
	while (sel >= (int)mp->chanBank.size())
	{
		mp->Add100Channels();
	}
	...
	int refIDOffset = mp->GetRefIDOffset(sel);
	mp->ReplaceReference(101 + (sel % 100) + refIDOffset, node);
...
wm3_SetMarkerData_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("wm3_SetMarkerData [Morpher Modifier] [Marker Index]  [Morph Index] [Marker Name]"));
	integer_type_check(arg_list[2], _T("wm3_SetMarkerData [Morpher Modifier] [Marker Index]  [Morph Index] [Marker Name]"));
...
wm3_mc_rebuild_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_Rebuild [Morpher Modifier] [Channel Index]"));
	...
	if(sel>=(int)mp->chanBank.size()) sel = (int)mp->chanBank.size()-1;  //to get these the chan has to exist first
	if(sel<0) sel = 0;
...
wm3_mc_delete_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_Delete [Morpher Modifier] [Channel Index]"));
...
{
	check_arg_count_with_keys(wm3_mc_getname, 2, count);
	type_check(arg_list[0], MAXModifier, _T("WM3_MC_GetName [Morpher Modifier] [Channel Index] [localizedName:<bool>]"));
	integer_type_check(arg_list[1], _T("WM3_MC_GetName [Morpher Modifier] [Channel Index] [localizedName:<bool>]"));
	...
	MaxMorphModifier maxMorpher(mp);
	return new String(maxMorpher.GetMorphChannel(sel).GetName(localizedName));
}
...
{
	check_arg_count_with_keys(wm3_mc_setname, 3, count);
	type_check(arg_list[0], MAXModifier, _T("WM3_MC_SetName [Morpher Modifier] [Channel Index] [Name String] [localizedName:<bool>]"));
	integer_type_check(arg_list[1], _T("WM3_MC_SetName [Morpher Modifier] [Channel Index] [Name String] [localizedName:<bool>]"));
	type_check(arg_list[2], String, _T("WM3_MC_SetName [Morpher Modifier] [Channel Index] [Name String] [localizedName:<bool>]"));
	...
	MaxMorphModifier maxMorpher(mp);
	maxMorpher.GetMorphChannel(sel).SetName(arg_list[2]->to_string(), localizedName);
	return &true_value;
}
...
wm3_mc_getamount_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_GetValue [Morpher Modifier] [Channel Index]"));
	...
	return_value (Float::intern(tmpVal));
}
...
wm3_mc_setamount_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_SetValue [Morpher Modifier] [Channel Index] [Value]"));
	float_type_check(arg_list[2], _T("WM3_MC_SetValue [Morpher Modifier] [Channel Index] [Value]"));
...
wm3_mc_hastarget_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_HasTarget [Morpher Modifier] [Channel Index]"));
...
wm3_mc_gettarget_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_GetTarget [Morpher Modifier] [Channel Index]"));
	...
	if (target != NULL) 
	{
		return_value (MAXNode::intern(target));
	}
	return &undefined;
}
...
wm3_mc_settarget_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_SetTarget [Morpher Modifier] [Channel Index] [Node]"));
...
wm3_mc_isvalid_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_IsValid [Morpher Modifier] [Channel Index]"));
...
wm3_mc_hasdata_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_HasData [Morpher Modifier] [Channel Index]"));
...
wm3_mc_isactive_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_IsActive [Morpher Modifier] [Channel Index]"));
...
wm3_mc_setactive_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_SetActive [Morpher Modifier] [Channel Index] [true/false]"));
	if (!Number::is_integer_number(arg_list[2]))
...
wm3_mc_getuselimits_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_GetUseLimits [Morpher Modifier] [Channel Index]"));
...
wm3_mc_setuselimits_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_SetUseLimits [Morpher Modifier] [Channel Index] [true/false]"));
	if (!Number::is_integer_number(arg_list[2]))
...
wm3_mc_getlimitmax_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_GetLimitMAX [Morpher Modifier] [Channel Index]"));
	...
	return_value (Float::intern(mp->chanBank[sel].mSpinmax));
}
...
wm3_mc_getlimitmin_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_GetLimitMIN [Morpher Modifier] [Channel Index]"));
	...
	return_value (Float::intern(mp->chanBank[sel].mSpinmin));
}
...
wm3_mc_setlimitmax_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_SetLimitMAX [Morpher Modifier] [Channel Index] [Float Value]"));
	float_type_check(arg_list[2], _T("WM3_MC_SetLimitMAX [Morpher Modifier] [Channel Index] [Float Value]"));
...
wm3_mc_setlimitmin_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_SetLimitMIN [Morpher Modifier] [Channel Index] [Float Value]"));
	float_type_check(arg_list[2], _T("WM3_MC_SetLimitMIN [Morpher Modifier] [Channel Index] [Float Value]"));
...
wm3_mc_getvertsel_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_GetUseVertexSel [Morpher Modifier] [Channel Index]"));
...
wm3_mc_setvertsel_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_SetUseVertexSel [Morpher Modifier] [Channel Index] [true/false]"));
	if (!Number::is_integer_number(arg_list[2]))
...
wm3_mc_getmemuse_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_GetMemUse [Morpher Modifier] [Channel Index]"));
	...
	return_value (Float::intern(tmSize));
}
...
wm3_mc_getnumpts_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_NumPts [Morpher Modifier] [Channel Index]"));
	...
	MaxMorphModifier maxMorpher(mp);
	return_value (Integer::intern(maxMorpher.GetMorphChannel(sel).NumMorphPoints()));
}
...
wm3_mc_getnummpts_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_NumMPts [Morpher Modifier] [Channel Index]"));
	...
	return_value (Integer::intern(static_cast<int>(mp->chanBank[sel].Points().size())));	// SR DCAST64: Downcast to 2G limit.
}
...
wm3_mc_getmorphpoint_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_GetMorphPoint [Morpher Modifier] [Channel Index] [Index]"));
	integer_type_check(arg_list[2], _T("WM3_MC_GetMorphPoint [Morpher Modifier] [Channel Index] [Index]"));
...
wm3_mc_getmorphweight_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_GetMorphWeight [Morpher Modifier] [Channel Index] [Index]"));
	integer_type_check(arg_list[2], _T("WM3_MC_GetMorphWeight [Morpher Modifier] [Channel Index] [Index]"));
	...
	MaxMorphModifier maxMorpher(mp);
	return_value (Float::intern((float)maxMorpher.GetMorphChannel(sel).GetMorphPointWeight(arg_list[2]->to_int())));
}
...
/*===========================================================================*\
 |	Checks if the specified morph channel is progressive or not.
\*===========================================================================*/

Value* wm3_IsProgressive_cf(Value** arg_list, int count)
{
	// wm3_IsProgressive $.modifiers[#morpher] morphIndex
	check_arg_count(wm3_IsProgressive_cf, 2, count);
	type_check(arg_list[0], MAXModifier, _T("wm3_IsProgressive [Morpher Modifier] [Morph Index]"));
	integer_type_check(arg_list[1], _T("wm3_IsProgressive [Morpher Modifier] [Morph Index]"));

	ReferenceTarget* obj = arg_list[0]->to_modifier();
	if (!check_ValidMorpher(obj, arg_list))
		return &false_value;

	Modifier* mp = arg_list[0]->to_modifier();
	if (mp)
	{
		IMorpher* iMorpher = (IMorpher*)mp->GetInterface(I_MORPHER_INTERFACE_ID);
		if (iMorpher)
		{
			int morphIndex = arg_list[1]->to_int() - 1;
			
			int numChannels = iMorpher->NumChannels();
			if (morphIndex < 0 || morphIndex >= numChannels)
				throw RuntimeError(GetString(IDS_W3_INVALID_CHANNEL_ID), Integer::intern(morphIndex));

			const IMorpherChannel* iChannel = static_cast <IMorpherChannel*>(iMorpher->GetChannel(morphIndex));
			if (iChannel)
			{
				if (iChannel->IsProgressive())
					return &true_value;
			}
		}
	}
...
/*==============================================================================*\
 |	Gets the deltas for the channel's default/initial morph
\*==============================================================================*/

Value* wm3_GetBaseDeltas_cf(Value** arg_list, int count)
{
	// wm3_IsProgressive $.modifiers[#morpher] morphIndex
	check_arg_count(wm3_GetProgressiveTargetDeltas_cf, 2, count);
	type_check(arg_list[0], MAXModifier, _T("wm3_GetProgressiveTargetDeltas [Morpher Modifier] [Morph Index]"));
	integer_type_check(arg_list[1], _T("wm3_GetProgressiveTargetDeltas [Morpher Modifier] [Morph Index]"));
	...
	if (mp)
	{
		...
		if (iMorpher)
		{
			...
			{
				Tab<Point3> deltas;
				iChannel->GetBaseDeltas(deltas);
				one_typed_value_local_tls(Array * result);
				vl.result = new Array(deltas.Count());
				Point3* pts = deltas.Addr(0);
				for (int i = 0; i < deltas.Count(); ++i)
				{
					vl.result->append(new Point3Value(pts[i]));
				}
...
/*==============================================================================*\
 |	Gets the deltas for the channel's target morphs
\*==============================================================================*/

Value* wm3_GetProgressiveTargetDeltas_cf(Value** arg_list, int count)
{
	// wm3_IsProgressive $.modifiers[#morpher] morphIndex
	check_arg_count(wm3_GetProgressiveTargetDeltas_cf, 3, count);
	type_check(arg_list[0], MAXModifier, _T("wm3_GetProgressiveTargetDeltas [Morpher Modifier] [Morph Index] [Target Index] (use 0 for the base)"));
	integer_type_check(arg_list[1], _T("wm3_GetProgressiveTargetDeltas [Morpher Modifier] [Morph Index] [Target Index] (use 0 for the base)"));
	integer_type_check(arg_list[2], _T("wm3_GetProgressiveTargetDeltas [Morpher Modifier] [Morph Index] [Target Index] (use 0 for the base)"));
	...
	if (mp)
	{
		...
		if (iMorpher)
		{
			...
			if (iChannel)
			{
				int targetIndex = arg_list[2]->to_int() - 1;
				if (targetIndex < 0 || targetIndex >= iChannel->NumTargets())
					throw RuntimeError(GetString(IDS_W3_INVALID_TARGET_ID), Integer::intern(targetIndex+1));
				
				Tab<Point3> deltas;
				iChannel->GetProgressiveTargetDeltas(targetIndex, deltas);
				one_typed_value_local_tls(Array * result);
				vl.result = new Array(deltas.Count());
				Point3* pts = deltas.Addr(0);
				for (int i = 0; i < deltas.Count(); ++i)
