...
{ 
	...
	{
		targpercents_undo[j] = mp->chanBank[idx].TargetCaches()[j - 1].mTargetPercent;
	}
}
...
void Restore_FullChannel::Restore(int isUndo) 
{
	...
	if (isUndo) {
		...
		{
			targpercents_redo[j] = bank.TargetCaches()[j - 1].mTargetPercent;
		}
	}
	...
	for (int j = 1; j < ntargs_undo; j++)
	{
		target_caches[j - 1].mTargetPercent = targpercents_undo[j];
	}
...
void Restore_FullChannel::Redo() 
{
	...
	for (int j = 1; j < ntargs_redo; j++)
	{
		target_caches[j - 1].mTargetPercent = targpercents_redo[j];
	}
