      ...
      case WM_COMMAND:
         switch (LOWORD(wParam)) {
            case IDOK:
               ...
               mp->chanBank[mp->recordTarget].mNonLocalizedName = nametoUse;
   ...
   switch (msg) {
      ...
      case WM_PAINT:
      {
         ...
         for( q=0;q<5;q++ )
         {
            ...
            if(q==1) FilCol = RGB(255,160,0); // no morph data, changed channel
            if(q==2) FilCol = RGB(0,255,0);	// active and valid channel
            if(q==3) FilCol = RGB(10,10,255);	// channel has data, no target
            if(q==4) FilCol = RGB(90,90,90); // disabled channel
   ...
   switch (msg) {
      ...
      case WM_COMMAND:
         ...
         case IDC_ALL_ACTIVATE:
            {
               for(int i=0;i<mp->chanBank.size();i++) mp->chanBank[i].mActiveOverride = TRUE;
         ...
         case IDC_ALL_DEACTIVATE:
            {
               for(int i=0;i<mp->chanBank.size();i++) mp->chanBank[i].mActiveOverride = FALSE;
               mp->Update_channelFULL();
               mp->NotifyDependents(FOREVER,PART_ALL,REFMSG_CHANGE);
               mp->ip->RedrawViews(mp->ip->GetTime());
               break;}
         ...
         case IDC_MTLASSIGN:{
            if(mp->ip->GetSelNodeCount()==1 ) 
            {
			   ...
			   mmtl->SyncChannels((int)mp->chanBank.size());
               ...
               {     
                  //UNDO it all
                  mmtl->DeleteReference(102);

                  mmtl->morphp = NULL; 
                  mp->morphmaterial = NULL;

                  mmtl->obName.printf(_T(""));
                  for(int i=0;i<mp->chanBank.size();i++)
                  {
            ...
            {
               TSTR msgBoxText = GetString(IDS_MTL_NOOBJ);
   ...
   switch (msg) {
      ...
      case WM_COMMAND:
         ...
         case IDC_COMPACT:{
               ...
               TCHAR s[50];
               _stprintf(s,GetString(IDS_CHANNELS_MOVED),numMoved);
      ...
      HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
   ...
   switch (msg) {
      case WM_INITDIALOG:{
         ...
         for(int i=0;i<(int)mp->chanBank.size();i++)
         {
            TCHAR tmp[255];
            int Cind = i+1;
      ...
      case WM_COMMAND:
         ...
         if ( HIWORD(wParam) == LBN_SELCHANGE ) {
            ...
            int targIdx = (int)SendMessage(clist,LB_GETCURSEL,0,0);
         ...
         switch (LOWORD(wParam)) {
            ...
            case IDC_PERFORMOP:
               int targIdx = (int)SendMessage(GetDlgItem(hWnd,IDC_CLIST),LB_GETCURSEL,0,0);
               theHold.Begin();
               theHold.Put( new Restore_Display( mp ) );
               theHold.Put(new Restore_FullChannel(mp, targIdx, FALSE));
