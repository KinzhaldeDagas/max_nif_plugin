...
public:
   ...
   const TCHAR    *dialogTitle()       { return GetString(IDS_LOADMULTI);  }
   const TCHAR    *buttonText()        { return GetString(IDS_MULTIBTN);   }
   ...
   {
	  MacroRecorder::MacroRecorderDisable macroDisableGuard(true);
      int i;
      Tab<int> freeC;
      freeC.ZeroCount();

	  int numOpenChannels = 0;
      for(i=0;i<mp->chanBank.size();i++)
      {
         if(!mp->chanBank[i].mModded) 
			 numOpenChannels++;
      }
	  if (nodeTab.Count() >= mp->chanBank.size())
	  {
		  while (nodeTab.Count() >= mp->chanBank.size())
		  {
			  mp->Add100Channels();
		  }
	  }
      ...
      if(nodeTab.Count()>freeC.Count())
      {
         ...
         MaxSDK::MaxMessageBox(mp->hwChannelList, buf1, buf2, MB_ICONSTOP | MB_OK);
         goto leapOut;
      }
      ...
      for(i=0;i<nodeTab.Count();i++)
      {
		 ...
		 int refIDOffset = mp->GetRefIDOffset(freeC[i]);
         mp->ReplaceReference(101+freeC[i]%100+refIDOffset,nodeTab[i]);
      ...
      HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
   ...
   switch (msg) {
      ...
      case WM_INITDIALOG:{
         ...
         TCHAR s[15];
         _stprintf(s, _T("%d - %d"), mp->chanNum+1,mp->chanNum+10);
      ...
      case WM_COMMAND:
         ...
         if (notify==CBN_SELENDOK){
            if(id==IDC_MARKERLIST){
               int mkSel = (int)SendMessage(GetDlgItem(hWnd, IDC_MARKERLIST), CB_GETCURSEL, 0, 0);
               if(mkSel>=0){
                  mp->chanSel = 0;
         ...
         case ID_RELOADCHANNEL:
         {
            ...
            for(int i=0; i<bank.mNumProgressiveTargs; i++) {
               bank.TargetCaches()[i].Init(bank.TargetCaches()[i].mTargetINode); }
         ...
         case IDC_UPDATETARGS:
            ...
            for(int i=0;i<mp->chanBank.size();i++)
            {
               ...
               std::vector<TargetCache>& target_caches = mp->chanBank[i].TargetCaches();
			   for (int j = 0; j < mp->chanBank[i].mNumProgressiveTargs; j++)
			   {
				   target_caches[j].Init(target_caches[j].mTargetINode);
			   }
			}
   ...
   switch (msg) {
      case WM_INITDIALOG:{
         ...
         pUpDownToolbar = GetICustToolbar(GetDlgItem(hWnd, IDC_UP_DOWN));
            ...
            ToolButtonItem target_up(CTB_PUSHBUTTON, 0, 0, 2, 2, 16, 15, 24, 23, IDC_TARGET_UP);
            pUpDownToolbar->AddTool(target_up);
            ToolButtonItem target_down(CTB_PUSHBUTTON, 1, 1, 3, 3, 16, 15, 24, 23, IDC_TARGET_DOWN);
            pUpDownToolbar->AddTool(target_down);
...
//       prevTargetProc = DLSetWindowProc(GetDlgItem( hWnd, IDC_TARGETLIST ), TargetListBoxProc);
         DLSetWindowLongPtr(GetDlgItem( hWnd, IDC_TARGETLIST ), mp);
         ...
         mp->cCurvature = SetupFloatSpinner(hWnd, IDC_CURVE_EDIT_SPIN, IDC_CURVE_EDIT,
            MorphR3::kTensionMin, MorphR3::kTensionMax, mp->chanBank[mp->CurrentChannelIndex()].mCurvature, 0.01f);

         mp->cTargetPercent = SetupFloatSpinner(hWnd, IDC_TARGET_PERCENT_SPIN, IDC_TARGET_PERCENT_EDIT,
            MorphR3::kProgressiveTargetWeigthMin, MorphR3::kProgressiveTargetWeigthMax, mp->GetCurrentTargetPercent(), 0.01f);
      ...
      case WM_CUSTEDIT_ENTER:
         switch (LOWORD(wParam)) {
            case IDC_CHANNAME:{
               ...
               if(_tcscmp(strTmp,mp->chanBank[mp->chanNum+mp->chanSel].mName)!=0)
               {
                  // Set channel name and channel 'dirty/changed' bit
                  mp->chanBank[mp->chanNum+mp->chanSel].mName = TSTR(strTmp);
                  mp->chanBank[mp->chanNum+mp->chanSel].mNonLocalizedName = TSTR(strTmp);
                  mp->chanBank[mp->chanNum+mp->chanSel].mModded = TRUE;
               }
               icT->SetText( strTmp );
      ...
      case WM_COMMAND:{
         ...
         case IDC_TARGETLIST:
            mp->CurrentChannel().iTargetListSelection =
               (int)SendDlgItemMessage( hWnd, IDC_TARGETLIST, LB_GETCURSEL, 0, 0);
            if(mp->cTargetPercent && mp->CurrentChannel().iTargetListSelection >= 0)
               mp->cTargetPercent->SetValue(mp->GetCurrentTargetPercent(), FALSE);
         break;
         ...
         case IDC_MAKE:{
                  ...
                  DialogBoxParam( hInstance,  MAKEINTRESOURCE( IDD_NAMECAP ),
         ...
         case IDC_CHANNUM:{
               ...
               for(int x=0;x<mp->chanBank.size();x++)
               {
                  if(mp->chanBank[x].mModded)
                  {
                     _stprintf(s,_T("[%i]  %s"),(x+1),mp->chanBank[x].mName.data());
                     minfo.wID = 5000+x;
                     minfo.dwTypeData = s;
               ...
               AppendMenu(mainMenu, MF_BYCOMMAND | MF_POPUP, reinterpret_cast<UINT_PTR>(hCi),
                          GetString(IDS_ACTIVE));
         ...
         case IDC_EXTRACT:{
            ...
            if(mp->ip->GetSelNodeCount()==1)
            {
               ...
               if( pObj->SuperClassID() == GEN_DERIVOB_CLASS_ID)
               {
                  ...
                  Object *osMr = ((Object*)CloneRefHierarchy(osp.obj))->ConvertToType(t,defObjectClassID);


                  for(int z=0;z<mp->chanBank[cIndex].mNumPoints;z++)
                  {
                     osMr->SetPoint(z,mp->chanBank[cIndex].Points()[z]);
                     osMr->SetWeight(z,mp->chanBank[cIndex].Weights()[z]);
                  }
               ...
               else
               {
                  ...
                  TSTR msgBoxText = GetString(IDS_EXTRACT_MOD_ERROR);
                  MaxSDK::MaxMessageBox(hWnd, msgBoxText, GetString(IDS_CLASS_NAME), MB_OK);
               }
            }  // node sel count check
            else
            {
               // this will only happen if the mod stack is 'pinned'
               TSTR msgBoxText = GetString(IDS_EXTRACT_GRAB_ERROR);
               MaxSDK::MaxMessageBox(hWnd, msgBoxText, GetString(IDS_CLASS_NAME), MB_OK);
            }
...
void MorphR3::BeginEditParams( IObjParam *ip, ULONG flags,Animatable *prev )
{
   ...
   DLSetWindowLongPtr(hwLegend, this);
   ...
   DLSetWindowLongPtr(hwGlobalParams, this);
   ...
   DLSetWindowLongPtr(hwChannelList, this);
   ...
   DLSetWindowLongPtr(hwChannelParams, this);
   ...
   DLSetWindowLongPtr(hwAdvanced, this);
...
// The following static method would be better as a method on the morphChannel class. I have
...
//
float GetTargetPercentage( morphChannel * bank, int index )
{
      float targPerc = 0.0;
      if ( index == 0 ) {
         ...
         targPerc = bank->TargetCaches()[index-1].mTargetPercent;
      }
...
static void SetTargetPercentage( morphChannel * bank, int index, float perc )
{
   if(index==0) {
      ...
      bank->TargetCaches()[index-1].mTargetPercent = perc;
   }

}

// The following static method would be better as a method on the morphCannel class. I have
// placed them here as we are not able to break SDK compatability this release. They should
// be moved during the next SDK-breaking release cycle. (tb)
//
...
void IMorphClass::SortProgressiveTarget(MorphR3 *mp, int morphIndex, int pMorphIndex )
{
#pragma warning (push)
#pragma warning (disable:4996) // disable deprecation warnings
   if (theHold.Holding())
      theHold.Put(new Restore_FullChannel(mp, morphIndex) );
//if only one morph done
...
//while not done
   {
      ...
      prev = pMorphIndex - 1;
   //see it at bottom
      float cWeight = GetTargetPercentage(&bank, pMorphIndex);
      if (pMorphIndex == 0)
      {

         float nWeight = GetTargetPercentage(&bank, next);
         if (  cWeight <= nWeight )
         {
            done = TRUE;
         }
         ...
         {
            SwapPTargets(mp,morphIndex,pMorphIndex, next,false);
            pMorphIndex++;
         }
      }
   //see if at the top
      else if (pMorphIndex == (num))
      {
         float pWeight = GetTargetPercentage(&bank, prev);
         if ( cWeight  >= pWeight)
         {
            done = TRUE;
         }
      ...
      //else swap with prev
      //set morph to prev
            SwapPTargets(mp,morphIndex,pMorphIndex, prev,false);
            pMorphIndex--;
         }
      }
      else
