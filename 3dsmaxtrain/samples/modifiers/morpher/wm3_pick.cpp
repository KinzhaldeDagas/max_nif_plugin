...
BOOL  GetMorphNode::Pick(IObjParam *ip,ViewExp *vpt)
	...
	if (node) {
		...
		morphChannel &bank = mp->CurrentChannel();
		if(bank.mConnection ) {
			int refnum = (mp->CurrentChannelIndex()%100*MAX_PROGRESSIVE_TARGETS) + 201 + bank.mNumProgressiveTargs;
			int refIDOffset = mp->GetRefIDOffset(mp->CurrentChannelIndex());
			...
			int refIDOffset = mp->GetRefIDOffset(mp->CurrentChannelIndex());
			mp->ReplaceReference(101+(mp->CurrentChannelIndex()%100)+refIDOffset,node);
