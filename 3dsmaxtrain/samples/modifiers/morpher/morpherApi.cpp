...
// class CountModContextProc:
BOOL CountModContextProc::proc(ModContext* mc)
{
	DbgAssert(mc != NULL);
...
int CountModContextProc::GetModContextCount() const
{
	return mModContexts.Count();
}

ModContext* CountModContextProc::GetModContext(int i) const
{
...
void MaxMorphModifier::SetNumChannels(int numChannels)
{
	if (numChannels < 0) return;
	if (mMorpher!=nullptr && numChannels >= (int)mMorpher->chanBank.size())
	{
		while (numChannels >= (int)mMorpher->chanBank.size())
		{
			mMorpher->Add100Channels();
		}
	}
}
...
void MaxMorphChannel::Init(morphChannel* pChannel, int channelId)
{
	...
	DbgAssert(0 <= channelId );
	if (pChannel->mp != nullptr)
	{
		DbgAssert(channelId < pChannel->mp->chanBank.size());
	}
	mChannel = pChannel;
...
void MaxMorphChannel::Reset(bool active, bool modded, int numPoints)
{
	if (mChannel)
	{
		...
		mChannel->Points().resize(numPoints);
		mChannel->Deltas().resize(numPoints);
		mChannel->Weights().resize(numPoints);
	}
}
...
const MCHAR* MaxMorphChannel::GetName(bool localized)
{
	if (mChannel)
	{
		return localized ? mChannel->mName.data() : mChannel->mNonLocalizedName.data();
	}
	else
	{
		return NULL;
...
void MaxMorphChannel::SetName(const MCHAR* newName, bool localized)
{
	if (mChannel)
	{
		...
		{
			mChannel->mNonLocalizedName = newName;
		}
	}
}
...
{
	return (mChannel) ? mChannel->mConnection : nullptr;
}
...
MorphExport bool MaxMorphChannel::SetMorphTarget(INode* newTarget, TimeValue t)
{
	...
	if (GetMorphNode::IsValidMorphTargetType(mChannel->mp, newTarget, t))
	{
		...
		int refIDOffset = mChannel->mp->GetRefIDOffset(mChannelId);		
		mChannel->mp->ReplaceReference(101 + (mChannelId%100)+refIDOffset, newTarget);
...
Point3 MaxMorphChannel::GetMorphPoint(int pointIndex) const
{
	...
	return mChannel->Points()[pointIndex];
}
...
void MaxMorphChannel::SetMorphPoint(int pointIndex, const Point3& newPoint)
{
	if (NULL != mChannel && NULL != mChannel->mp && IsValidMorphPointIndex(pointIndex))
	{
		...
		mChannel->Points()[pointIndex] = newPoint;
...
Point3 MaxMorphChannel::GetMorphPointDelta(int pointIndex) const
{
	...
	return mChannel->Deltas()[pointIndex];
}
...
void MaxMorphChannel::SetMorphPointDelta(int pointIndex, const Point3& newDelta)
{
	if (NULL != mChannel && NULL != mChannel->mp && IsValidMorphPointIndex(pointIndex))
	{
		...
		mChannel->Deltas()[pointIndex] = newDelta;
		mChannel->mp->NotifyDependents(FOREVER, PART_GEOM, REFMSG_CHANGE);
	}
}
...
double MaxMorphChannel::GetMorphPointWeight(int pointIndex) const
{
	...
	return mChannel->Weights()[pointIndex];
}
...
void MaxMorphChannel::SetMorphPointWeight(int pointIndex, double newWeight)
{
	if (NULL != mChannel && NULL != mChannel->mp && IsValidMorphPointIndex(pointIndex))
	{
		...
		mChannel->Weights()[pointIndex] = newWeight;
		mChannel->mp->NotifyDependents(FOREVER, PART_GEOM, REFMSG_CHANGE);
	}
}
...
INode* MaxMorphChannel::GetProgressiveMorphTarget(int targetIndex) const
{
	...
	else if (0 == targetIndex)
	{
		...
		int refOffset = mChannel->mp->GetRefIDOffset(mChannelId);
		DbgAssert(mChannel->mp->GetReference(101 + mChannelId%100 + refOffset) == morphTarget);
	}
	else 
	{
		morphTarget = mChannel->TargetCaches()[targetIndex-1].mTargetINode;
		int refOffset = mChannel->mp->GetRefIDOffset(mChannelId);
		DbgAssert(mChannel->mp->GetReference(200 + targetIndex + mChannelId%100*MAX_PROGRESSIVE_TARGETS + refOffset) == morphTarget);
	}
	return morphTarget;
}
...
float MaxMorphChannel::GetProgressiveMorphWeight(int targetIndex) const
{
	...
	if (NULL != mChannel && IsValidProgressiveMorphTargetIndex(targetIndex))
	{
		...
		{
			weight = mChannel->TargetCaches()[targetIndex-1].mTargetPercent;
		}
	}
	return weight;
}
...
MorphExport bool MaxMorphChannel::SetProgressiveMorphWeight( int morphIndex, float newValue )
{
	if (NULL != mChannel && NULL != mChannel->mp && IsValidProgressiveMorphTargetIndex(morphIndex))
	{
		...
		{
			mChannel->TargetCaches()[morphIndex-1].mTargetPercent = newValue;
		}
		return SortProgressiveMorphTargets(morphIndex);
	}
	return false;
}
...
Point3 MaxMorphChannel::GetProgressiveMorphPoint(int morphIndex, int pointIndex) const
{
	...
	if (NULL != mChannel && IsValidProgressiveMorphTargetIndex(morphIndex) && IsValidMorphPointIndex(pointIndex))
	{
		if (0 == morphIndex)
		{
			point = mChannel->Points()[pointIndex];
		}
		else if (morphIndex >= 1)
		{
			point = mChannel->TargetCaches()[morphIndex-1].GetPoint(pointIndex);
		}
	}
	return point;
}

int MaxMorphChannel::GetTargetCacheSize() const
{
	if (IsValid())
	{
		return (int)mChannel->TargetCaches().size();
	}
	return 0;
}

MorphExport INode* MaxMorphChannel::GetProgressiveMorphTargetNode(int targetIndex)
{
	if (IsValid() && IsValidMorphPointIndex(targetIndex))
	{
		return mChannel->TargetCaches()[targetIndex].RefNode();
	}
