...
class ProjectionModTypeUVW : public IProjectionModType
{
	...
	public:
		...
		virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("ProjectionModTypeUVW"); }
...
private:
		virtual void		SetReference(int i, RefTargetHandle rtarg) { mpPBlock=(IParamBlock2*)rtarg; }
public:
		...
		int			FindMainFace(ProjectionState &projState, int iTargetFace, Tab<int> &tabFaceHits, int centerFaceHit);
		...
		// ProjectionState &projState - just the projection state data
		// int iTargetFace - the target face that is to be checked
		// int mainFace - this is the face to try to build from
		// Tab<Point3> &results - this is the results
		void ReprojectFace(ProjectionState &projState, int iTargetFace, int mainFace, Tab<int> &tabFaceHits, Tab<Point3> &uvResults);
...
//This class stores the data needed for each thread when we do the intersection
//it contains the inter sector call and the faces that the intersector will work on
//note the intersector is unique per thread
class IntersectorThreadData
{
public:
	IProjectionIntersector*		pPInter;
	int mStartFace, mEndFace;
};
...
public:
	...
	IProjectionIntersector*		pPInter;
	Tab<IntersectorThreadData>	mPInterWorkChunks;  // data for each thread
	Matrix3						mat;
	...
	void Cleanup() {
		...
		for (int i = 0; i < mPInterWorkChunks.Count(); i++)
		{
			if (mPInterWorkChunks[i].pPInter)
				mPInterWorkChunks[i].pPInter->DeleteThis();
			mPInterWorkChunks[i].pPInter = NULL;
		}
