...
class ProjectionModTypeUVWClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Project Mapping"); }
...
static ParamBlockDesc2 theProjectionModTypeUVWBlockDesc (
	...
	ProjectionModTypeUVW::pb_targetSameAsSource, _T("sameAsSource"), TYPE_BOOL, P_RESET_DEFAULT, IDS_PARAM_TARGET_SAME_AS_SOURCE,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_TARGET_SAME_AS_SOURSE,
		p_accessor,		&theProjectionModTypeUVW_accessor,
		p_end,
...
void ProjectionModTypeUVW::Project(Tab<INode*> &tabSourceNodes, int iSelIndex, int iNodeIndex)
{
	...
	double secondsPassed = timer.EndTimer()/1000.0;
	//ScriptPrint(_T("Projection Time %3.2f seconds\n"),secondsPassed);
}
...
bool ProjectionModTypeUVW::InitProjectionState( ProjectionState &projState, INode *pNodeSrc, ObjectState * osSrc, INode *pNodeTarget, ProjectionHolderUVW *pMod, BitArray *pSOSel, IProjectionModData *pPModData )
{
	...
	if( projState.iProjType == ProjectionState::pt_sameTopology ) {
		...
		int chunkSize = omp_get_max_threads();
		projState.mPInterWorkChunks.SetCount(chunkSize);
		for (int i = 0; i < chunkSize; i++)
		{
			projState.mPInterWorkChunks[i].pPInter = NULL;
		}

		//this initialize our thread data, we crate a data chunk for each thread
		//that contains an intersector and the faces that the insector is to work on
		//this is pretty heavy weight but since I cant break the SDK only way for now.
		for (int i = 0; i < chunkSize; i++)
		{
			projState.mPInterWorkChunks[i].pPInter = pPInterMgr->CreateProjectionIntersector();
			if( !projState.mPInterWorkChunks[i].pPInter )
				return false;

			projState.mPInterWorkChunks[i].pPInter->InitRoot( *(projState.objectSource), objectCage, srcTM );
			if( !projState.mPInterWorkChunks[i].pPInter->RootValid() ) 
			{
				for (int k = 0; k <= i; k++)
				{
					projState.mPInterWorkChunks[k].pPInter->DeleteThis();
					projState.mPInterWorkChunks[k].pPInter = NULL;
				}
				return false;
			}
		}
...
//this is a class used to hold the output from the intersector. Updating the UVWs while in the thread is not safe 
//so the hit data needs to be stored off.  Then processed once all the threads are done
class HitDataResult
{
public:
	Tab<int> tabPerFaceHit;
...
void ProjectionModTypeUVW::FillInHoldDataClusterProjection( ProjectionState &projState   )
{
	...
	//this has to be protected from other threads
	int vertexBufferIndex = 0;

	if (projState.mPInterWorkChunks.Count() > 0)
	{

		Tab<HitDataResult*> hitData;
		hitData.SetCount(iNumFaces);
		for (int i = 0; i < hitData.Count(); i++)
		{
			hitData[i] = new HitDataResult();
		}

		int faceInc = iNumFaces;
		if (projState.mPInterWorkChunks.Count() > 1)
			faceInc = iNumFaces/(projState.mPInterWorkChunks.Count()-1);

		//this computes which range of faces will work on which thread
		int currentFace = 0;
		for (int i = 0; i < projState.mPInterWorkChunks.Count(); i++)
		{
			projState.mPInterWorkChunks[i].mStartFace = currentFace;
			currentFace += faceInc;
			projState.mPInterWorkChunks[i].mEndFace = currentFace-1;
			if (projState.mPInterWorkChunks[i].mEndFace > iNumFaces)
				projState.mPInterWorkChunks[i].mEndFace = iNumFaces-1;

		}
#pragma omp parallel for
		for (int k = 0; k < projState.mPInterWorkChunks.Count(); k++)
		{
			Point3 norm;
			Point3 ptCenter;
			Point3 ptCenterUVW;
			...
			for( int i=projState.mPInterWorkChunks[k].mStartFace; i<=projState.mPInterWorkChunks[k].mEndFace; i++ ) 
			{
				...
				BOOL bInterRes =  projState.mPInterWorkChunks[k].pPInter->RootClosestFace( pointAndNormal, 
										IProjectionIntersector::FINDCLOSEST_IGNOREBACKFACING, 
										fDist, dwFace, tabBary );
				if ( !bInterRes )
					bInterRes = projState.mPInterWorkChunks[k].pPInter->RootClosestFace( pointAndNormal, 
										IProjectionIntersector::FINDCLOSEST_CAGED | IProjectionIntersector::FINDCLOSEST_IGNOREBACKFACING, 
										fDist, dwFace, tabBary );
				if ( !bInterRes && !projState.mIgnoreBackFaces)
					bInterRes = projState.mPInterWorkChunks[k].pPInter->RootClosestFace( pointAndNormal, 0, fDist, dwFace, tabBary );
				...
				hitData[i]->tabPerFaceHit = tabPerFaceHit;
				hitData[i]->iCenterFace = iCenterFace;
				hitData[i]->ptCenter = ptCenter;
				hitData[i]->ptCenterUVW = ptCenterUVW;
			}

		}
		for( int i=0; i<iNumFaces; i++ ) 
		{
			FixMapFaceData( projState, i, hitData[i]->tabPerFaceHit, hitData[i]->iCenterFace, hitData[i]->ptCenter, hitData[i]->ptCenterUVW,vertexBufferIndex );
		}
	...
	{	
		...
		for( int i=0; i<iNumFaces; i++ ) 
		{
			...
			for( int j=0; j< geoFace.numVerts; j++ ) 
			{
				...
				if ( (tvVert >= 0) && (tvVert < lookupData.Count()) )
				{
...
void ProjectionModTypeUVW::FixMapFaceData( ProjectionState &projState, int iTargetFace, Tab<int> &tabFaceHits, int iCenterFaceHit, Point3 &ptCenter, Point3 &ptCenterUVW , int &vertexBufferOffset)
{
	...
	int mainFace = -1;		//this is the face used to do our approx.  we use bary coords to get an approx
	...
	{		
		...
		if (!hitAllSameFace)
		{
			...
			//need to find our main face
			//the best we can do here is to find the closest face
			if (fixupVertices)
			{
				mainFace = FindMainFace(projState, iTargetFace, tabFaceHits,iCenterFaceHit);
				if (mainFace != -1)
	...
	if (fixupVertices )
	{
		...
		if (iCenterFaceHit != -1)
			mainFace = iCenterFaceHit;

		Tab<Point3> results;

		ReprojectFace(projState, iTargetFace,  mainFace, tabFaceHits, results);
...
//build a tm for our check face
	...
	{
		mainXYZs[0] = hitFaceCentroids[0];
		mainXYZs[1] = hitFaceCentroids[1];

		Point3 vecA = Normalize(mainXYZs[1] - mainXYZs[0]);
		Point3 vecB(1.0f,0.0f,0.0f);

		for (int i = 2; i < hitFaceCentroids.Count(); i++)
		{		
			mainXYZs[2] = hitFaceCentroids[i];

			//make sure last one does not cause a degenerate face
			vecB = Normalize(mainXYZs[2] - mainXYZs[0]);
		...
		tm.SetRow(3,mainXYZs[0]);
		itm = Inverse(tm);
	}
	else
	{
		GenFace geoFace = GetChannelFace(projState.objectTarget, targetFace, VERTEX_CHANNEL_NUM);
		mainXYZs[0] = (*projState.objectTarget->GetVert(geoFace.verts[0])) * projState.mat;
		mainXYZs[1] = (*projState.objectTarget->GetVert(geoFace.verts[1])) * projState.mat;

		Point3 vecA = Normalize(mainXYZs[1] - mainXYZs[0]);
		Point3 vecB = Normalize(mainXYZs[2] - mainXYZs[0]);

		for (int i = 2; i < geoFace.numVerts; i++)
		{		
			mainXYZs[2] = (*projState.objectTarget->GetVert(geoFace.verts[i])) * projState.mat;
	...
	BOOL iret = FALSE;
	for (int checkFaceIndex = 0; checkFaceIndex < checkFaces.Count(); checkFaceIndex++)
	{
		//get our test faces
		testFaces.Append(1,&checkFaces[checkFaceIndex],100);	
			...
			for (int i = 0; i < testFaces.Count(); i++)
			{
				...
				if (!mProcessedFaces[testFace])
				{
					...
					//if seam
						...
						{	
							int edgeIndex = projState.objectTarget->GetFaceEdgeIndex(targetFace, k);
...
int ProjectionModTypeUVW::FindMainFace(ProjectionState &projState, int iTargetFace, Tab<int> &tabFaceHits, int centerFaceHit)
{
	...
	if (seamFound)
	{
		...
		for (int j = 0; j < numVerts; j++)
		{
			...
			{			
				...
				//that is our main face since that face will most likely have the most coverage
				if (d > dist)
				{
					dist = d;
					mainFace = tabFaceHits[j];
//					iCenterFaceHit = -1;
				}
			}
		}
	...
	//if we still dont have a main face use centroids
...
void ProjectionModTypeUVW::ReprojectFace(ProjectionState &projState, int iTargetFace, int mainFace, Tab<int> &tabFaceHits, Tab<Point3> &results)
{
	results.SetCount(0,FALSE);
	// We need create and calculate new map verts
	//get our main UVs position
	Point3 mainUVs[3];
	Point3 mainXYZs[3];
	...
	GenFace face = GetChannelFace(projState.objectSource, mainFace, miSourceMapChannel);
	GenFace geoFace = GetChannelFace(projState.objectSource, mainFace, VERTEX_CHANNEL_NUM);

	mainUVs[0] = (*GetChannelVert(projState.objectSource, face.verts[0], miSourceMapChannel));
	mainUVs[0].z = 0.0f;
	mainXYZs[0] = (*projState.objectSource->GetVert(geoFace.verts[0]));
	mainUVs[1] = (*GetChannelVert(projState.objectSource, face.verts[1], miSourceMapChannel));
	mainUVs[1].z = 0.0f;
	mainXYZs[1] = (*projState.objectSource->GetVert(geoFace.verts[1]));

	//compute our geo plane equation
	Point3 vecGeoA = Normalize(mainXYZs[1] - mainXYZs[0]);
	Point3 vecGeoB = Normalize(mainXYZs[2] - mainXYZs[0]);

	for (int i = 2; i < face.numVerts; i++)
	{		
		mainUVs[2] = (*GetChannelVert(projState.objectSource, face.verts[i], miSourceMapChannel));
		mainUVs[2].z = 0.0f;
		mainXYZs[2] = (*projState.objectSource->GetVert(geoFace.verts[i]));

		//make sure last one does not cause a degenerate face
		vecGeoB = Normalize(mainXYZs[2] - mainXYZs[0]);
	...
	for( int j=0; j<faceTarget.numVerts; j++ ) 
	{
		//project our point onto the main face				
		...
		p = p * tm ;
		Point3 bry;
		bry = BaryCoords(mainXYZs[0], mainXYZs[1], mainXYZs[2], p);

		//compute the new uvw using the new bary coords
		Point3 uv(0.0f,0.0f,0.0f);
		uv.x = mainUVs[0].x * bry.x + mainUVs[1].x * bry.y + mainUVs[2].x * bry.z; 
		uv.y = mainUVs[0].y * bry.x + mainUVs[1].y * bry.y + mainUVs[2].y * bry.z; 
		uv.z = 0.0f;//mainXYZs[0].z * bry.x + mainXYZs[1].z * bry.y + mainXYZs[2].z * bry.z; 
