...
class ProjectionHolderUVWClassDesc : public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Project Mapping Holder"); }
...
void ProjectionHolderUVW::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc)
{
}
...
IOResult ProjectionHolderUVWData::Load(ILoad *pILoad)
{
	...
	while (IO_OK==(res=pILoad->OpenChunk())) {
		...
		case SOURCE_TYPE_CHUNK:
			pILoad->Read( &miSource, sizeof(int), &uRead );
			break;
		case MAP_CHANNEL_CHUNK:
			pILoad->Read( &miMapChannel, sizeof(int), &uRead );
			break;
		case P3_DATA_COUNT_CHUNK:
			pILoad->Read( &iP3Count, sizeof(int), &uRead );
			break;
		case P3_DATA_CHUNK:
			if( iP3Count > 0 ) {
			...
			break;
		case MATID_DO_CHUNK:
			pILoad->Read( &mDoMaterialIDs, sizeof(bool), &uRead );
			break;
		case MATID_COUNT_CHUNK:
			pILoad->Read( &iMatIDCount, sizeof(int), &uRead );
			break;
		case MATID_CHUNK:
			if( iMatIDCount > 0 ) {
		...
		case FACES_DATA_COUNT_CHUNK:
			pILoad->Read( &iGenFaceCount, sizeof(int), &uRead );
			break;
		case FACES_DATA_CHUNK:
			if( iGenFaceCount > 0 ) {
				...
				for( int i=0; i<iGenFaceCount; i++ ) {
					...
					pILoad->Read( &iNumVerts, sizeof(unsigned char), &uRead );
					mFaces[i].numVerts = iNumVerts;
					if( iNumVerts > 0 ) {
						mFaces[i].verts = new DWORD[iNumVerts];
						pILoad->Read( (DWORD*)mFaces[i].verts, iNumVerts*sizeof(DWORD), &uRead);
					}
				}
			}
			break;
...
IOResult ProjectionHolderUVWData::Save(ISave *pISave)
{
	...
	pISave->Write( &miSource, sizeof(int), &nb );
	pISave->EndChunk();

	pISave->BeginChunk(MAP_CHANNEL_CHUNK); 
	pISave->Write( &miMapChannel, sizeof(int), &nb );
	pISave->EndChunk();

	int iCount = mP3Data.Count();
	pISave->BeginChunk(P3_DATA_COUNT_CHUNK);
	pISave->Write( &iCount, sizeof(int), &nb );
	pISave->EndChunk();
	...
	if( mFaces.Count() > 0 ) {
		...
		for(int i=0; i<iCount; i++ ) {
			...
			if( mFaces[i].numVerts > 0 )
				pISave->Write( (DWORD*)mFaces[i].verts, mFaces[i].numVerts*sizeof(DWORD), &nb );
		}
		pISave->EndChunk();
	}

	pISave->BeginChunk(MATID_DO_CHUNK); 
	pISave->Write( &mDoMaterialIDs, sizeof(bool), &nb );
	pISave->EndChunk();
