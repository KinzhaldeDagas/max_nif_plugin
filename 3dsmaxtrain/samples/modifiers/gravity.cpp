...
class GravityObject : public ForceObject //,IOperatorInterface // Bayboro 9/18/01
{
   public:
      ...
      const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_GRAVITY) : _T("Gravity");}
...
class GravityClassDesc:public ClassDesc2 
{
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Gravity"); }
...
class GravityMod : public SimpleWSMMod 
{
   public:           
      ...
      // From Animatable
      void GetClassName(MSTR& s, bool localized) const override {s= localized ? GetString(IDS_RB_GRAVITYMOD) : _T("GravityMod");}
      SClass_ID SuperClassID() {return WSM_CLASS_ID;}
      void DeleteThis() {delete this;}
      Class_ID ClassID() { return Class_ID(GRAVITYMOD_CLASS_ID,0);}
      RefTargetHandle Clone(RemapDir& remap);
      const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_GRAVITYBINDING) : _T("Gravity Binding");}
...
class GravityModClassDesc:public ClassDesc 
{
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("GravityMod"); }
   SClass_ID      SuperClassID() {return WSM_CLASS_ID;}
   Class_ID    ClassID() {return Class_ID(GRAVITYMOD_CLASS_ID,0);}
...
/*
TSTR GravityObject::GetParameterName(int pbIndex, bool localized)
   {
   switch (pbIndex) {
