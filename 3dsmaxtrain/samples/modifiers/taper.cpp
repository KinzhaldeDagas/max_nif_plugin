...
class TaperMod : public SimpleMod {
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_RB_TAPERMOD) : _T("TaperMod"); }
		virtual Class_ID ClassID() { return Class_ID(TAPEROSM_CLASS_ID,0);}
		void BeginEditParams( IObjParam  *ip, ULONG flags,Animatable *prev );
		void EndEditParams( IObjParam *ip,ULONG flags,Animatable *next );
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_TAPER) : _T("Taper"); }
...
class TaperDeformer : public Deformer
	...
	public:
		...
		BOOL doX, doY, sym;
		TaperDeformer();
		TaperDeformer(TimeValue t, ModContext& mc, float amt, float crv, int naxis, int eaxis, int sym, float from,
				float to, int doRegion, Matrix3& modmat, Matrix3& modinv);
...
class TaperWSM : public SimpleOSMToWSMObject {
	public:
		...
		RefTargetHandle Clone(RemapDir& remap)
		{
			TaperWSM *newobj = new TaperWSM((TaperMod*)remap.CloneRef(mod));
...
class TaperClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Taper"); }
...
static ParamUIDesc descParam[] = {
	...
	// Symmetry
	ParamUIDesc(PB_SYMMETRY,TYPE_SINGLECHECKBOX,IDC_TAPER_SYMMETRY),

	// Affect region
	ParamUIDesc(PB_DOREGION,TYPE_SINGLECHECKBOX,IDC_TAPER_AFFECTREGION),
...
Point3 TaperDeformer::Map(int i, Point3 p) const
{
	float f, z, l;
...
bool TaperDeformer::IsThreadSafe() const
{
	return true;
}
...
TSTR TaperMod::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_AMT:		return localized ? GetString(IDS_RB_AMOUNT)    : _T("Amount");
		case PB_CRV:		return localized ? GetString(IDS_RB_CURVITURE) : _T("Curvature");
		case PB_FROM:		return localized ? GetString(IDS_RB_FROM)      : _T("Lower Limit");
		case PB_TO:			return localized ? GetString(IDS_RB_TO)        : _T("Upper Limit");
