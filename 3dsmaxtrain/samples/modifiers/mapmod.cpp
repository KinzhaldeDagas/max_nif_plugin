...
class MapMod : public MappingMod, public RealWorldMapSizeInterface {	
	public:
		...
		IParamArray* GetParamBlock() override { return pblock; }
		...
		void GetClassName(MSTR& s, bool localized) const override {s= localized ?GetString(IDS_RB_MAPMOD) : _T("MapMod");}
		virtual Class_ID ClassID() {return Class_ID(UVWMAPOSM_CLASS_ID,0);}
		void BeginEditParams( IObjParam  *ip, ULONG flags,Animatable *prev);
		void EndEditParams( IObjParam *ip,ULONG flags,Animatable *next);		
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_UVWMAP) : _T("UVW Map"); }
		CreateMouseCallBack* GetCreateMouseCallBack() {return NULL;} 
		BOOL AssignController(Animatable *control,int subAnim);
		int SubNumToRefNum(int subNum);

		ChannelMask ChannelsUsed()  {return GEOM_CHANNEL|TOPO_CHANNEL|SELECT_CHANNEL|SUBSEL_TYPE_CHANNEL|VERTCOLOR_CHANNEL;}
		ChannelMask ChannelsChanged() {return TEXMAP_CHANNEL|VERTCOLOR_CHANNEL|TOPO_CHANNEL; }		
...
public:
		...
		{ 
			...
			return MappingMod::GetInterface(id);
		} 
	};
...
class UVWMapClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("UVW Map"); }
...
static ParamUIDesc descParam[] = {
	...
	// Cap
	ParamUIDesc(PB_CAP,TYPE_SINGLECHECKBOX,IDC_MAP_CAP),
...
//	ParamUIDesc(PB_NURBS_SRF,TYPE_SINGLECHECKBOX, IDC_NURBS_TXT_SRF),
...
#define NEWMAPCALC_CHUNKID	0x0110  //this is just  a flag to remove the old fudge factor from the fit
...
public:
	...
	void proc (ILoad *iload) {
		if (mm && mm->GetParamBlock() ) {
			int mapChan;
			mm->GetParamBlock()->GetValue (PB_MAPCHANNEL, 0, mapChan, FOREVER);
			if (!mapChan) mm->GetParamBlock()->SetValue (PB_MAPCHANNEL, 0, 1);
		}
		delete this;
	}
};
...
IOResult MapMod::Load(ILoad *iload) {
	...
	IOResult res;
	mOldFudgeFactor = true;
	while (IO_OK==(res=iload->OpenChunk())) {
		switch (iload->CurChunkID()) {	
			case NEWMAP_CHUNKID:
				SetAFlag(A_PLUGIN1);
				break;
			case NEWMAPCALC_CHUNKID:
				mOldFudgeFactor = false;
				break;
			}
		iload->CloseChunk();
...
IOResult MapMod::Save(ISave *isave)
	...
	if (mOldFudgeFactor == false)
	{
		isave->BeginChunk(NEWMAPCALC_CHUNKID);
		isave->EndChunk();
	}
...
TSTR MapMod::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case TM_REF: return localized ? GetString(IDS_RB_GIZMO) : _T("Gizmo");
	case PBLOCK_REF: return localized ? GetString(IDS_RB_PARAMETERS) : _T("Parameters");
	default: return TSTR(_T(""));
	}
}

RefResult MapMod::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate) 
{
	switch (message)
	{
		case REFMSG_CHANGE:
...
// 								proc->EnableNURBSButton( pmapParam->GetHWnd(), GetModObjTypes() );
		...
		case REFMSG_GET_PARAM_NAME_NONLOCALIZED:
		{
			...
			switch (gpn->index)
			{
				case PB_UTILE:  gpn->name = _T("U Tile"); break;
				case PB_VTILE:  gpn->name = _T("V Tile"); break;
				case PB_WTILE:  gpn->name = _T("W Tile"); break;
				case PB_LENGTH: gpn->name = _T("Length"); break;
				case PB_WIDTH:  gpn->name = _T("Width");  break;
				case PB_HEIGHT: gpn->name = _T("Height"); break;
				default:        gpn->name = TSTR(_T("")); break;
			}
			return REF_HALT;
		}
...
{
    BOOL usePhysUVs = flags & CONTROL_SET_RWS_ON;
	switch (level)
	{
		case 1: // Modifier box
...
void MapMod::ModifyObject(TimeValue t, ModContext &mc, ObjectState *os, INode *node)
	...
	if (type !=MAP_XYZTOUVW)
	{  
		//this is to fix an issue about the object mapper which does not understand real world space
		//it adds a 0.5 offset to uncenter the map since it is expected to go from 0 to 1 but in realworld space
		//this does not apply since maps are an arbitrary size, oh and the reason we offset 1 and not 0.5 is that the 
		//tm is scled down by half so we need to double up the offset
		if ((mOldFudgeFactor==false) && GetUsePhysicalScaleUVs() && (type == MAP_PLANAR))
		{
			tm.Translate(Point3(-1.0f,-1.0f,0.0f));
		}
...
BaseInterface* MappingMod::GetInterface(Interface_ID id)
{
...
void MapModPipelineEnumProc::SetMapType()
{
		SuspendAnimate();
		AnimateOff();
        mpMod->GetParamBlock()->SetValue(PB_MAPTYPE, 0, *type);
        if (*type == MAP_CYLINDRICAL)
            mpMod->GetParamBlock()->SetValue(PB_CAP, 0, true);
    ...
    if (os->obj->IsSubClassOf(triObjectClassID)) {
        ...
        if (mesh.selLevel == MESH_FACE || mesh.selLevel == MESH_OBJECT) {
            ...
            if ((mapped || mesh.selLevel == MESH_FACE) && !(flags & CONTROL_RESET)) {
                ...
                SuspendAnimate();
                AnimateOff();
                pMod->GetParamBlock()->SetValue(PB_MAPTYPE, 0, type);
                ResumeAnimate();
            }
        }
    } else if (os->obj->IsSubClassOf(polyObjectClassID)) {
        ...
        if (this->ClassID() == Class_ID(UVWMAPOSM_CLASS_ID,0)) {
            ...
            if( pMod->GetUsePhysicalScaleUVs() ) {
                ...
                if (type == MAP_CYLINDRICAL) {
                    ...
                    SetTile(t, 1, 1.0f);
                    pMod->GetParamBlock()->SetValue(PB_CAP,t,FALSE);
