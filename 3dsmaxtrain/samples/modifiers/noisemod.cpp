...
class NoiseMod : public SimpleMod {
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_RB_NOISEMOD) : _T("NoiseMod"); }
		virtual Class_ID ClassID() {return Class_ID(NOISEOSM_CLASS_ID,0);}
		void BeginEditParams( IObjParam  *ip, ULONG flags,Animatable *prev);
		void EndEditParams( IObjParam *ip,ULONG flags,Animatable *next);
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_NOISE) : _T("Noise"); }
...
class NoiseDeformer : public Deformer
	...
	public:
		...
		NoiseDeformer();
		NoiseDeformer(ModContext& mc, TimeValue t, int seed, float scale, int fractal, float iterations, float rough,
				int anim, float freq, Point3 strength, TimeValue phase, Matrix3& modmat, Matrix3& modinv);
...
class NoiseWSM : public SimpleOSMToWSMObject {
	public:
		...
		RefTargetHandle Clone(RemapDir& remap)
		{
			NoiseWSM *newobj = new NoiseWSM((NoiseMod*)remap.CloneRef(mod));
...
static ParamUIDesc descParam[] = {
	...
	// Fractal
	ParamUIDesc(PB_FRACTAL,TYPE_SINGLECHECKBOX,IDC_MODNOISE_FRACTAL),
	...
	// Animate
	ParamUIDesc(PB_ANIMATE,TYPE_SINGLECHECKBOX,IDC_MODNOISE_ANIMATE),
...
Point3 NoiseDeformer::Map(int i, Point3 p) const
{
	Point3 d, sp;
	p = p * tm;
...
bool NoiseDeformer::IsThreadSafe() const
{
	return true;
}
...
TSTR NoiseMod::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_SEED:			return localized ? GetString(IDS_RB_SEED)       : _T("Seed");
		case PB_SCALE:			return localized ? GetString(IDS_RB_SCALE)      : _T("Scale");
		case PB_FRACTAL:		return localized ? GetString(IDS_RB_FRACTAL)    : _T("Fractal");
		case PB_ROUGH:			return localized ? GetString(IDS_RB_ROUGH)      : _T("Rough");
		...
		case PB_ITERATIONS:	return localized ? GetString(IDS_RB_ITERATIONS) : _T("Iterations");
