...
class DeleteMod : public Modifier {	
	public:		
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_RB_DELETEMOD) : _T("DeleteMesh");}  
		virtual Class_ID ClassID() { return Class_ID(DELETE_CLASS_ID,0);}		
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_DELETEMOD) : _T("DeleteMesh");}
...
class DeleteClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("DeleteMesh"); }
