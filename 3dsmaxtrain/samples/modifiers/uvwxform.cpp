...
public:
	...
	{
		s = localized ? GetString(IDS_RB_UVWXFORMMOD) : _T("UVW Xform");
	}
	...
	{
		return Class_ID(UVW_XFORM2_CLASS_ID, 0);
	}
	RefTargetHandle Clone(RemapDir& remap);
	const TCHAR* GetObjectName(bool localized) const override
	{
		return localized ? GetString(IDS_RB_UVWXFORMMOD) : _T("UVW Xform");
	}
	...
	{
		return TEXMAP_CHANNEL | VERTCOLOR_CHANNEL | TOPO_CHANNEL | GEOM_CHANNEL;
	}
...
public:
	...
	Class_ID ClassID()
	{
		return Class_ID(UVW_XFORM_CLASS_ID, 0);
	}
...
public:
	...
	const TCHAR* ClassName()
	{
		return GetString(IDS_RB_UVWXFORMMOD);
	}
	const TCHAR* NonLocalizedClassName()
	{
		return _T("UVW Xform");
...
ClassDesc* GetUVWXFormModDesc()
{
	return &uvwxformDesc;
}
static UVWXForm2ClassDesc uvwxform2Desc;
ClassDesc* GetUVWXFormMod2Desc()
{
	return &uvwxform2Desc;
}
...
static ParamUIDesc descParam[] = {
	// U Tile
	ParamUIDesc(PB_UTILE, EDITTYPE_FLOAT, IDC_MAP_UTILE, IDC_MAP_UTILESPIN, -BIGFLOAT, BIGFLOAT, SPIN_AUTOSCALE),

	// V Tile
	ParamUIDesc(PB_VTILE, EDITTYPE_FLOAT, IDC_MAP_VTILE, IDC_MAP_VTILESPIN, -BIGFLOAT, BIGFLOAT, SPIN_AUTOSCALE),

	// W Tile
	ParamUIDesc(PB_WTILE, EDITTYPE_FLOAT, IDC_MAP_WTILE, IDC_MAP_WTILESPIN, -BIGFLOAT, BIGFLOAT, SPIN_AUTOSCALE),

	// U Offset
	ParamUIDesc(PB_UOFFSET, EDITTYPE_UNIVERSE, // EDITTYPE_FLOAT,
			IDC_MAP_UOFFSET, IDC_MAP_UOFFSETSPIN, -BIGFLOAT, BIGFLOAT, SPIN_AUTOSCALE),

	// V Offset
	ParamUIDesc(PB_VOFFSET, EDITTYPE_UNIVERSE, // EDITTYPE_FLOAT,
			IDC_MAP_VOFFSET, IDC_MAP_VOFFSETSPIN, -BIGFLOAT, BIGFLOAT, SPIN_AUTOSCALE),

	// W Offset
	ParamUIDesc(PB_WOFFSET, EDITTYPE_UNIVERSE, // EDITTYPE_FLOAT,
			IDC_MAP_WOFFSET, IDC_MAP_WOFFSETSPIN, -BIGFLOAT, BIGFLOAT, SPIN_AUTOSCALE),

	// U Flip
	ParamUIDesc(PB_UFLIP, TYPE_SINGLECHECKBOX, IDC_MAP_UFLIP),

	// V Flip
	ParamUIDesc(PB_VFLIP, TYPE_SINGLECHECKBOX, IDC_MAP_VFLIP),

	// W Flip
	ParamUIDesc(PB_WFLIP, TYPE_SINGLECHECKBOX, IDC_MAP_WFLIP),

	// Channel
	ParamUIDesc(PB_CHANNEL, TYPE_RADIO, chanIDs, 2),

	// MapChannel
	ParamUIDesc(PB_MAPCHANNEL, EDITTYPE_POS_INT, IDC_MAP_CHAN, IDC_MAP_CHAN_SPIN, 1, MAX_MESHMAPS - 1, SPIN_AUTOSCALE),

	// W Angle
	ParamUIDesc(PB_WANGLE, EDITTYPE_FLOAT, // SS 9/20/2002: added rotation
			IDC_MAP_WANGLE, IDC_MAP_WANGLESPIN, -720.0f, 720.0f, 1.0f, stdAngleDim),

	// Center of rotation
	ParamUIDesc(PB_ROTCENTER, TYPE_SINGLECHECKBOX, IDC_MAP_ROTCENTER),

	// Apply to whole object
	ParamUIDesc(PB_ENTIREOBJECT, TYPE_SINGLECHECKBOX, IDC_MAP_ENTIREOBJECT), // alexc - july.22.2003
};
...
static ParamBlockDescID descVer3[] = {
	...
	{ TYPE_INT, NULL, FALSE, PB_WFLIP }, // W Flip
	{ TYPE_INT, NULL, FALSE, PB_CHANNEL }, // channel
	{ TYPE_INT, NULL, FALSE, PB_MAPCHANNEL }, { TYPE_FLOAT, NULL, TRUE, PB_WANGLE }, // W Angle
...
static ParamBlockDescID descVer4[] = {
	// alexc - july.23.2003 - added "use entire object" check box
	{ TYPE_FLOAT, NULL, TRUE, PB_UTILE }, // U Tile
	{ TYPE_FLOAT, NULL, TRUE, PB_VTILE }, // V Tile
	{ TYPE_FLOAT, NULL, TRUE, PB_WTILE }, // W Tile
	{ TYPE_FLOAT, NULL, TRUE, PB_UOFFSET }, // U Offset
...
public:
	UVWXFormMod* xm;
	XSetChannelToOne(UVWXFormMod* xMod)
	{
		xm = xMod;
	}
...
void XSetChannelToOne::proc(ILoad* iload)
{
	if (xm && xm->pblock)
	{
		...
		if (!mapChan)
			xm->pblock->SetValue(PB_MAPCHANNEL, 0, 1);
	}
	delete this;
}
...
public:
	UVWXFormMod* xm;
	UVWXFormPostLoadCallback(UVWXFormMod* m)
			: xm(m)
	{
	}
	void proc(ILoad* iload)
	{
		...
		(new ParamBlockPLCB(versions, NUM_OLDVERSIONS, &curVersion, xm, 0))->proc(iload); // proc contains "delete this"
		if (oldVer < 3)
		{
...
void UVWXFormMod::BeginEditParams(IObjParam* ip, ULONG flags, Animatable* prev)
{
	...
	pmapParam = CreateCPParamMap(descParam, PARAMDESC_LENGTH, pblock, ip, hInstance, MAKEINTRESOURCE(IDD_UVWXFORMPARAM),
...
void UVWXFormMod::GetParams(Params& params, TimeValue t, Interval& iv)
{
	...
	if (params.tile.x == 0.0f)
		params.tile.x = 1.0f;
	if (params.tile.y == 0.0f)
		params.tile.y = 1.0f;
	if (params.tile.z == 0.0f)
		params.tile.z = 1.0f;
}
...
void UVWXFormMod::CreateTransformMatrix(Matrix3& tm, const Params& params)
{
	...
	tm.Scale(Point3(params.uflip ? -1.0f : 1.0f, params.vflip ? -1.0f : 1.0f, params.wflip ? -1.0f : 1.0f), TRUE);
	tm.Translate(Point3(params.uflip ? 1.0f : 0.0f, params.vflip ? 1.0f : 0.0f, params.wflip ? 1.0f : 0.0f));
...
void UVWXFormMod::polyObjectModify(PolyObject* polyOb, TimeValue t)
{
	...
	// check if the channel is available in this mesh
	if ((params.mapChannel < -NUM_HIDDENMAPS) || (params.mapChannel >= mesh.MNum()))
		return;
	...
	if (params.channel)
		polyOb->UpdateValidity(VERT_COLOR_CHAN_NUM, iv);
	else
		polyOb->UpdateValidity(TEXMAP_CHAN_NUM, iv);
}
...
void UVWXFormMod::patchObjectModify(PatchObject* patchOb, TimeValue t)
{
	...
	if (params.channel)
		patchOb->UpdateValidity(VERT_COLOR_CHAN_NUM, iv);
	else
		patchOb->UpdateValidity(TEXMAP_CHAN_NUM, iv);
}
...
public:
	...
	int NewTVertCount() const
	{
		return static_cast<int>(mItems.size() +
				std::count_if(mItems.begin(), mItems.end(), OnSelectionBorder)); // SR DCAST64: Downcast to 2G limit.
	}

	template <typename TVerts>
	void MapTVerts(TVerts& mapVerts)
	{
		int newVertIndex = static_cast<int>(mItems.size()); // SR DCAST64: Downcast to 2G limit.
		...
		const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
	...
	case REFMSG_GET_PARAM_NAME_LOCALIZED:
	{
		...
		case PB_WTILE:
			...
			break;
		case PB_UOFFSET:
			gpn->name = GetString(IDS_RB_UOFFSET);
			break;
		case PB_VOFFSET:
			gpn->name = GetString(IDS_RB_VOFFSET);
			break;
		case PB_WOFFSET:
			gpn->name = GetString(IDS_RB_WOFFSET);
			break;
		case PB_UFLIP:
			gpn->name = GetString(IDS_UVWX_UFLIP);
			break;
		case PB_VFLIP:
			gpn->name = GetString(IDS_UVWX_VFLIP);
			break;
		case PB_WFLIP:
			gpn->name = GetString(IDS_UVWX_WFLIP);
			break;
		case PB_CHANNEL:
		...
		case PB_MAPCHANNEL:
			...
			break; // LAM - 3/7/03 - defect 164105
		case PB_WANGLE:
			gpn->name = GetString(IDS_UVWX_WANGLE);
			break;
		case PB_ROTCENTER:
			gpn->name = GetString(IDS_UVWX_ROTCENTER);
			break;
		}
		return REF_HALT;
	}
	...
	case REFMSG_GET_PARAM_NAME_NONLOCALIZED:
	{
		...
		case PB_WTILE:
			...
			break;
		case PB_UOFFSET:
			gpn->name = _T("U Offset");
			break;
		case PB_VOFFSET:
			gpn->name = _T("V Offset");
			break;
		case PB_WOFFSET:
			gpn->name = _T("W Offset");
			break;
		case PB_UFLIP:
			gpn->name = _T("U Flip");
			break;
		...
		case PB_MAPCHANNEL:
			...
			break; // LAM - 3/7/03 - defect 164105
		case PB_WANGLE:
			gpn->name = _T("Rotation Angle");
			break;
		case PB_ROTCENTER:
			gpn->name = _T("Rotation Center");
			break;
		}
		return REF_HALT;
	}
