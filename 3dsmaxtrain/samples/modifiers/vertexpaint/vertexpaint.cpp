...
class VertexPaintClassDesc:public ClassDesc {
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("OldVertexPaint"); }
...
static INT_PTR CALLBACK VertexPaintDlgProc(
   ...
   VertexPaint *mod = DLGetWindowLongPtr<VertexPaint*>(hWnd);
   ...
   case WM_INITDIALOG:
      ...
      mod->iColor->SetColor(Color(mod->lastColor));
      ...
      ReleaseISpinner( SetupFloatSpinner(hWnd, IDC_ALPHA_SPIN, IDC_ALPHA, 0.0f, 100.0f, mod->alpha*100.0f, 0.1f) );
      break;
   ...
   case CC_COLOR_CHANGE:
      if (LOWORD(wParam) == IDC_COLOR) {
         ...
         mod->lastColor = iCol->GetAColor().toRGB();
         }
      break;
    ...
    LPARAM lParam) 
{
   switch (uMsg) {
      ...
      case WM_LBUTTONDBLCLK: {
         ...
         break;
      case WM_DESTROY:
         DLSetWindowLongPtr(hwnd, colorSwatchOriginalWndProc); 
         // Fallthrough...
      default:
...
void VertexPaint::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc)
   {
      ...
      if(mesh->selLevel == MESH_OBJECT || mesh->selLevel == MESH_VERTEX) {
         ...
         for (int v=0; v < d->GetNumColors(chan); v++) {
            ...
            if (mf && mv) {
               ...
               for(int fc = 0; fc < d->GetNVert(v).faces.Count() && SelFaceColsAllSame ; fc++) {
            ...
            if(SelFaceColsAllSame) {
               ...
               for(int vcfc = 0; vcfc < d->GetNVCVert(ivc,chan).faces.Count() && CreateNewColor == FALSE; vcfc++) {
                  ...
                  for(int fc = 0; fc < d->GetNVert(v).faces.Count() && CreateNewColor == FALSE ; fc++) {
               ...
               if(CreateNewColor)
               {
                  ...
                  for(int fc = 0;  fc < d->GetNVert(v).faces.Count() ; fc++)
                  {
                     if(!mesh->FaceSel()[d->GetNVert(v).faces[fc]])
                        continue;
...
void VertexPaint::BeginEditParams( IObjParam *ip, ULONG flags,Animatable *prev )
   ...
   if (!hParams) {
      ...
      for (int i=0; i<NUMPALETTES; i++) {
         colorSwatchOriginalWndProc = DLSetWindowLongPtr(hPaletteWnd[i], colorSwatchSubclassWndProc); 
         }
...
void VertexPaint::PaletteButton(HWND hWnd) {
   ...
   if (iPal && iColor) {
      iColor->SetAColor(iPal->GetAColor(), TRUE);
   }
}
...
void VertexPaintData::AllocColorData(int numcols, int chan) {
   ColorData* newColorData = NULL;
   AlphaData* newAlphaData = NULL;
...
LocalModData* VertexPaintData::Clone()
   ...
   for (int j=0; j<3; j++) {
      if(nvcverts[j]) {
         ...
         for(int i=0 ; i<numnvcverts[j]; i++ ) d->nvcverts[j][i] = nvcverts[j][i];
...
void VertexPaintData::SynchVerts(MNMesh &m) {
   ...
   for (int i=0; i < mpMNMesh->FNum() ; i++){   
      // for each vertex of each face
      for (int j=0; j < mpMNMesh->F(i)->deg ; j++){      
