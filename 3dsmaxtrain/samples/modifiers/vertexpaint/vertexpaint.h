...
public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_CLASS_NAME) : _T("OldVertexPaint"); }  
		virtual Class_ID ClassID() { return VERTEXPAINT_CLASS_ID;}		
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_CLASS_NAME) : _T("OldVertexPaint"); }
		...
		//From Modifier
		ChannelMask ChannelsUsed()  { return GEOM_CHANNEL|TOPO_CHANNEL|VERTCOLOR_CHANNEL|SUBSEL_TYPE_CHANNEL|SELECT_CHANNEL|DISP_ATTRIB_CHANNEL|TEXMAP_CHANNEL; }
