...
BOOL VertexPaint::ActivatePaint(BOOL bOnOff, BOOL bPick)
	...
	if (bOnOff) {
		...
		if (!hPaintCursor) {
			hPaintCursor = UI::MouseCursors::LoadMouseCursor(UI::MouseCursors::Paint);
		}
		if (!hNoPaintCursor) {
			hNoPaintCursor = LoadCursor(NULL,IDC_CROSS);
		}
		if (!hDropperCursor) {
			hDropperCursor = UI::MouseCursors::LoadMouseCursor(UI::MouseCursors::Dropper);
		}
...
void PaintMouseProc::DoPickColor(HWND hWnd, IPoint2 m)
		...
		if (mesh && pView.IsAlive()) 
		{
			...
			if (mesh->IntersectRay(ray, at, norm, fi, bary)) 
			{
				...
				if (pMapVerts) {
					...
					pModifier->iColor->SetColor(PickCol);
				} else {
					pModifier->iColor->SetColor (Color(1.0f, 1.0f, 1.0f));
				}
				if (pModifier->affectChannel[2]) {
	...
	{
		Color c((uint32_t)node->GetWireColor());
		pModifier->iColor->SetColor(c);
	}
...
{
	// TODO ColorManagement: convert vertex paint data to floating point. /coz 2023-06-20
	return iColor->GetAColor().toRGB();
}
