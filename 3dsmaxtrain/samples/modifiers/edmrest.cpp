...
void VertexEditRestore::Restore(int isUndo) {
	...
	meshData->Invalidate(GEOM_CHANNEL,FALSE);
...
void MeshEditRestore::Restore(int isUndo) {
	...
	meshData->Invalidate (TOPO_CHANNEL|GEOM_CHANNEL|SELECT_CHANNEL, FALSE);	
...
void FaceChangeRestore::Restore(int isUndo) {
	...
	meshData->Invalidate(TOPO_CHANNEL,FALSE);
