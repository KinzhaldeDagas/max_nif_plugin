...
class NormalizeSpline: public Modifier {
	...
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_PW_NSPLINE) : _T("Normalize Spl."); }
		virtual Class_ID ClassID() { return Class_ID(PW_PATCH_TO_SPLINE1,PW_PATCH_TO_SPLINE2);}
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_PW_NSPLINE) : _T("Normalize Spl."); }
...
class NormalizeSplineClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Normalize Spl."); }
...
TSTR NormalizeSpline::GetParameterName(int pbIndex, bool localized)
	{
	return TSTR(_T(""));
	}
