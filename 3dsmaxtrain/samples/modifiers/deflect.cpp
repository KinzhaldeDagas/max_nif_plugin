...
class DeflectObject : public SimpleWSMObject2 {
	public:
		...
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_DEFLECTOR) : _T("Deflector");}
...
class DeflectorClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Deflector"); }
...
class DeflectMod : public SimpleWSMMod {
	public:				
		...
		// From Animatable
		void GetClassName(MSTR& s, bool localized) const override {s= localized ? GetString(IDS_RB_DEFLECTMOD) : _T("DeflectMod");}
		SClass_ID SuperClassID() {return WSM_CLASS_ID;}
		void DeleteThis() {delete this;}
		Class_ID ClassID() { return Class_ID(DEFLECTMOD_CLASS_ID,0);}
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_DEFLECTORBINDING) : _T("Deflector Binding");}
...
class DeflectorModClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("DeflectMod"); }
	SClass_ID		SuperClassID() {return WSM_CLASS_ID; }
	Class_ID		ClassID() {return Class_ID(DEFLECTMOD_CLASS_ID,0);}
...
// geo_param param IDs
enum FFD_Deflect_Params { deflect_bounce, deflect_width, deflect_height, deflect_variation, deflect_chaos,deflect_friction, 
...
static ParamBlockDesc2 deflect_param_blk ( deflect_params, _T("DeflectorParameters"),  0, &deflectDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF_NO, 
	...
	deflect_width,  _T("width"), 			TYPE_WORLD, 	P_ANIMATABLE + P_RESET_DEFAULT, 	IDS_RB_WIDTH, 
		p_default, 		0.0,	
		p_ms_default,	10.0,
		p_range, 		0.0f, 9999999.0f,
	...
	deflect_height,  _T("length"), 			TYPE_WORLD, 	P_ANIMATABLE + P_RESET_DEFAULT, 	IDS_RB_LENGTH, 
		p_default, 		0.0,	
		p_ms_default,	10.0,
		p_range, 		0.0f, 9999999.0f,
	...
	deflect_inherit_vel,  _T("inheritVelocity"),	TYPE_FLOAT, 	P_ANIMATABLE + P_RESET_DEFAULT, 	IDS_PW_INHERITVEL, 
		...
		p_nonLocalizedName, _T("Inherit Velocity"),
		p_end, 
...
//and stuff it into our param block
	...
	if (colp)
		...
		colp->SetWidth(pblock2->GetControllerByID(deflect_width));
		colp->SetHeight(pblock2->GetControllerByID(deflect_height));
		colp->SetQuality(pblock2->GetControllerByID(deflect_quality));
		}
	...
	public:
		...
		void proc(ILoad *iload) {  
					 ...
					 (LOWORD(iload->GetFileSaveVersion()) < 48)) // MAX v.3.9, build 48
			{
				...
				ParamID defl_chan[3] = {deflect_variation, deflect_chaos, deflect_friction};
				for( int i=0; i<3; i++)
				...
				cont = n->pblock2->GetControllerByID(deflect_inherit_vel);
...
//	colp = (CollisionPlane *) rt;
	...
	if (colp==NULL)
	{
		...
		if (colp)
		{
			colp->SetWidth(obj->pblock2->GetControllerByID(deflect_width));
			colp->SetHeight(obj->pblock2->GetControllerByID(deflect_height));
			colp->SetQuality(obj->pblock2->GetControllerByID(deflect_quality));
		}
	}
