...
static GenSubObjType SOT_Handle(NULL, SelectPatchIcon::stackHandle, 0);
static GenSubObjType SOT_Vertex(NULL, SelectPatchIcon::stackVertex, 0);
static GenSubObjType SOT_Edge(NULL, SelectPatchIcon::stackEdge, 0);
static GenSubObjType SOT_Patch(NULL, SelectPatchIcon::stackPatch, 0);
static GenSubObjType SOT_Element(NULL, SelectPatchIcon::stackElement, 0);
...
class PatchSelMod : public Modifier, public IPatchSelect, public EditSSCB {	
public:
	DWORD selLevel;
	...
	void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_PS_PATCHSELMOD) : _T("Patch Select");}  
	virtual Class_ID ClassID() { return PATCHSELECT_CLASS_ID;}		
	RefTargetHandle Clone(RemapDir& remap);
	const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_PS_PATCHSELMOD) : _T("Patch Select");}
...
public:
	...
	TSTR GetParameterName (int paramID, bool localized);
...
//--- Action table and overrides for PatchSelMod
const ActionTableId kActionID = 0x5baf7000;

class ActionCB : public ActionCallback {
	PatchSelMod *mpMod;
public:
	ActionCB (PatchSelMod *m) : mpMod(m) { }
...
ActionTable* GetActions() {
	TSTR name = GetString (IDS_PS_PATCHSELMOD);
...
protected:
	...
	PatchSelMod *mpObj;
public:
	EditSSOverride(PatchSelMod *m):mActiveOverride(FALSE),mpObj(m){}
...
EditSSMode*			PatchSelMod::editSSMode = NULL;

ActionCB*			PatchSelMod::mpActions  = NULL;
Tab<ActionItem*>	PatchSelMod::mOverrides;
...
class PatchSelClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Patch Select"); }
	SClass_ID		SuperClassID() { return OSM_CLASS_ID; }
	Class_ID		ClassID() { return PATCHSELECT_CLASS_ID; }
...
void PatchSelMod::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc) {
			...
			if (selLevel == SEL_PATCH) {
				...
				for (int i=0; i<d->pmesh->numPatches; i++) {
					for (int j=0; j<d->pmesh->patches[i].type; j++) {
						if (vhit[d->pmesh->patches[i].v[j]]) { nsel.Set (i, invert ? !nsel[i] : selected); break; }
					}
				}
...
void PatchSelMod::InvertSelection(int selLevel) {
	...
	for (int i=0; i<list.Count(); i++) {
		...
		case SEL_HANDLE:
		{
			BitArray invVecSel = ~d->vecSel;
			d->SetVecSel(invVecSel, this, ip->GetTime());
			break;
		}
		case SEL_VERTEX:
		{
			BitArray invVertSel = ~d->vertSel;
			d->SetVertSel(invVertSel, this, ip->GetTime());
			break;
		}
		case SEL_PATCH:
		case SEL_ELEMENT:
		{
			BitArray invPatchSel = ~d->patchSel;
			d->SetPatchSel(invPatchSel, this, ip->GetTime());
			break;
		}
		case SEL_EDGE:
		{
			BitArray invEdgeSel = ~d->edgeSel;
			d->SetEdgeSel(invEdgeSel, this, ip->GetTime());
			break;
		}
		}
	}
...
void PatchSelMod::SelectFrom(int from) {
	...
	for (int i=0; i<list.Count(); i++) {
		...
		case SEL_HANDLE:
			if (from==SEL_VERTEX)
			{
				BitArray vecSel = d->SelHandleByVert();
				d->SetVecSel(vecSel, this, ip->GetTime());
			}
			else if (from==SEL_EDGE)
			{
				BitArray vecSel = d->SelHandleByEdge();
				d->SetVecSel(vecSel, this, ip->GetTime());
			}
			else
			{
				BitArray vecSel = d->SelHandleByPatch();
				d->SetVecSel(vecSel, this, ip->GetTime());
			}
			break;
		case SEL_VERTEX: 
			if (from>=SEL_PATCH)
			{
				BitArray vertSel = d->SelVertByPatch();
				d->SetVertSel(vertSel, this, ip->GetTime());
			}
			else
			{
				BitArray vertSel = d->SelVertByEdge();
				d->SetVertSel(vertSel, this, ip->GetTime());
			}
			break;
		case SEL_PATCH:
			if (from==SEL_VERTEX)
			{
				BitArray patchSel = d->SelPatchByVert();
				d->SetPatchSel(patchSel, this, ip->GetTime());
			}
			else
			{
				BitArray patchSel = d->SelPatchByEdge();
				d->SetPatchSel(patchSel, this, ip->GetTime());
			}
			break;
		case SEL_ELEMENT:
			if (from==SEL_VERTEX)
			{
				BitArray patchSel = d->SelElementByVert();
				d->SetPatchSel(patchSel, this, ip->GetTime());
			}
			else if (from==SEL_EDGE)
			{ 
				BitArray patchSel = d->SelElementByEdge();
				d->SetPatchSel(patchSel, this, ip->GetTime());
			}
			else
			{
				BitArray patchSel = d->SelElementByPatch();
				d->SetPatchSel(patchSel, this, ip->GetTime());
			}
			break;
		case SEL_EDGE:
			if (from==SEL_VERTEX)
			{ 
				BitArray edgeSel = d->SelEdgeByVert();
				d->SetEdgeSel(edgeSel, this, ip->GetTime());
			}
			else
			{
				BitArray edgeSel = d->SelEdgeByPatch();
				d->SetEdgeSel(edgeSel, this, ip->GetTime());
			}
			break;
		}
...
static INT_PTR CALLBACK PatchSelectProc (HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {	
	PatchSelMod *mod = DLGetWindowLongPtr<PatchSelMod*>(hWnd);
	...
	case WM_INITDIALOG:
		...
		// CAL-06/10/03: add handle sub-object mode. (FID #1914)
		iToolbar->AddTool(SELECTPATCH_REGULAR(CTB_CHECKBUTTON, SelectPatchIcon::vertexSelection, PO_VERTEX));
		iToolbar->AddTool(SELECTPATCH_REGULAR(CTB_CHECKBUTTON, SelectPatchIcon::handleSelection, PO_HANDLE));
		iToolbar->AddTool(SELECTPATCH_REGULAR(CTB_CHECKBUTTON, SelectPatchIcon::edgeSelection, PO_EDGE));
		iToolbar->AddTool(SELECTPATCH_REGULAR(CTB_CHECKBUTTON, SelectPatchIcon::patchSelection, PO_PATCH));
		iToolbar->AddTool(SELECTPATCH_REGULAR(CTB_CHECKBUTTON, SelectPatchIcon::elementSelection, PO_ELEMENT));
		ReleaseICustToolbar(iToolbar);
...
BitArray PatchSelData::SelHandleByPatch() {
	...
	for (int i=0; i<pmesh->getNumVerts(); i++) {
		PatchVert &v = pmesh->verts[i];
		for (int p = 0; p < v.patches.Count(); p++)
      {
         if (patchSel[v.patches[p]]) 
         {
            for(int j = 0; j < v.vectors.Count(); ++j) 
            {
               nsel.Set(v.vectors[j]);
...
void PatchSelMod::NewSetFromCurSel(TSTR &setName) {
	...
	for (int i = 0; i < mcList.Count(); i++) {
		...
		// CAL-06/10/03: add handle sub-object mode. (FID #1914)
		case NS_HANDLE:	
			if (index>=0 && (set = meshData->hselSet.GetSet(id)) != NULL) {
				*set = meshData->vecSel;
		...
		case NS_PATCH:	// Patch and Element
			if (index>=0 && (set = meshData->pselSet.GetSet(id)) != NULL) {
...
RefResult PatchSelMod::NotifyRefChanged (const Interval& changeInt, RefTargetHandle hTarget, 
   		PartID& partID, RefMessage message, BOOL propagate) {
	return(REF_SUCCEED);
}
...
void PatchSelMod::DoAccept(TimeValue t)
{

}

void PatchSelMod::SetPinch(TimeValue t, float pinch)
{
...
void PatchSelMod::SetBubble(TimeValue t, float bubble)
{
...
void PatchSelMod::SetFalloff(TimeValue t,float falloff)
{
...
float PatchSelMod::GetPinch(TimeValue t)
{
	return mPinch;
}

float PatchSelMod::GetBubble(TimeValue t)
{
	return mBubble;
}

float PatchSelMod::GetFalloff(TimeValue t)
{
	return mFalloff;
}


void PatchSelMod::EditSoftSelectionMode()
{
...
INT_PTR CALLBACK SoftSelectDlgProc( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
	{
	PatchSelMod *pEpm = DLGetWindowLongPtr<PatchSelMod *>( hDlg);
	...
	switch ( message ) {
		case WM_INITDIALOG: {
			...
			spin = SetupIntSpinner   ( hDlg, IDC_EDGE_SPIN, IDC_EDGE, 1, 999, pEpm->mEdgeDist );
			ReleaseISpinner(spin);
			spin = SetupFloatSpinner ( hDlg, IDC_FALLOFF_SPIN, IDC_FALLOFF, 0.0, 9999999.0, pEpm->mFalloff );
			ReleaseISpinner(spin);
			spin = SetupFloatSpinner ( hDlg, IDC_PINCH_SPIN, IDC_PINCH, -10.0, 10.0, pEpm->mPinch );
			ReleaseISpinner(spin);
			spin = SetupFloatSpinner ( hDlg, IDC_BUBBLE_SPIN, IDC_BUBBLE, -10.0, 10.0, pEpm->mBubble );
			ReleaseISpinner(spin);
