...
IObjParam *WaveObject::ip        = NULL;
IParamMap *WaveObject::pmapParam = NULL;
HWND       WaveObject::hSot      = NULL;

class SinWaveObject : public WaveObject {	
	public:				
		SinWaveObject();

		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_RIPPLE) : _T("Ripple");}		
...
class LinWaveObject : public WaveObject {	
	public:		
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_WAVE) : _T("Wave"); }
...
class SinWaveClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Ripple"); }
...
class LinWaveClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Wave"); }
	SClass_ID		SuperClassID() { return WSM_OBJECT_CLASS_ID; }
	Class_ID		ClassID() { return Class_ID(LINWAVE_OBJECT_CLASS_ID,0); }
	const TCHAR* 	Category() { return GetSpaceWarpCatString(SPACEWARP_CAT_GEOMDEF);}
	};
...
class WaveMod : public SimpleWSMMod {
	public:
		...
		// From Animatable
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_RB_WAVEMOD) : _T("WaveMod"); }
...
class SinWaveMod : public WaveMod {
	public:		
		...
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_RIPPLE_BINDING) : _T("Ripple Binding");}
...
class LinWaveMod : public WaveMod {
	public:		
		...
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_WAVEBINDING) : _T("Wave Binding");}
...
WaveObject::WaveObject()
	{
	ReplaceReference(0, CreateParameterBlock(descObjVer1, OBJPBLOCK_LENGTH, CURRENT_OBJVERSION));
	assert(pblock);	
	}
...
TSTR WaveObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_AMPLITUDE:	return localized ? GetString(IDS_RB_AMPLITUDE1) : _T("Amplitude 1");
		case PB_AMPLITUDE2:	return localized ? GetString(IDS_RB_AMPLITUDE2) : _T("Amplitude 2");
		case PB_WAVELEN:		return localized ? GetString(IDS_RB_WAVELEN)    : _T("Wave Length");
		case PB_PHASE:			return localized ? GetString(IDS_RB_PHASE)      : _T("Phase");
		case PB_DECAY:			return localized ? GetString(IDS_RB_DECAY)      : _T("Decay");
...
Point3 SinWaveDeformer::Map(int i, Point3 p) const
{
	Point3 pt = p * tm;
...
bool SinWaveDeformer::IsThreadSafe() const
{
	return true;
}
...
Point3 LinWaveDeformer::Map(int i, Point3 p) const
{
	if (dist == 0.0f)
		return p;
...
bool LinWaveDeformer::IsThreadSafe() const
{
	return true;
}
...
WaveMod::WaveMod(INode *node,WaveObject *obj)
	...
	ReplaceReference(SIMPWSMMOD_PBLOCKREF, CreateParameterBlock(descModVer0, MODPBLOCK_LENGTH, CURRENT_MODVERSION));
...
TSTR WaveMod::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_FLEX:	return localized ? GetString(IDS_RB_FLEXIBILITY) : _T("Flexibility");
		default:			return localized ? GetString(IDS_RB_PARAMETERS)  : _T("Parameters");
		}
	}
...
WaveOMod::WaveOMod()
	{		
	ReplaceReference(SIMPMOD_PBLOCKREF, CreateParameterBlock(descOModVer0, OMODPBLOCK_LENGTH, CURRENT_OMODVERSION));
...
TSTR WaveOMod::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_AMPLITUDE:	return localized ? GetString(IDS_RB_AMPLITUDE1) : _T("Amplitude 1");
		case PB_AMPLITUDE2:	return localized ? GetString(IDS_RB_AMPLITUDE2) : _T("Amplitude 2");
...
RefTargetHandle SinWaveOMod::Clone(RemapDir& remap) 
	...
	newob->SimpleModClone(this, remap);
	BaseClone(this, newob, remap);
	return newob;
	}
