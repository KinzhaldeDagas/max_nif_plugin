...
class MirrorMod : public Modifier {	
	public:		
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_RB_MIRRORMOD) : _T("Mirror");}  
		virtual Class_ID ClassID() {return MIRRORMOD_CLASS_ID;}
		void BeginEditParams(IObjParam  *ip, ULONG flags,Animatable *prev);
		void EndEditParams(IObjParam *ip,ULONG flags,Animatable *next);		
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_MIRRORMOD) : _T("Mirror");}
...
class MirrorModClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Mirror"); }
...
static ParamUIDesc descParam[] = {	
	...
	// Copy
	ParamUIDesc(PB_COPY,TYPE_SINGLECHECKBOX,IDC_MIRROR_COPY),	
...
MirrorMod::MirrorMod() 
	...
	tmControl  = NULL; 	
	pblock     = NULL;
	ReplaceReference(PBLOCK_REF, CreateParameterBlock(descVer1, PBLOCK_LENGTH, CURRENT_VERSION));	
...
void MirrorMod::ModifyObject(
	...
	if (os->obj->IsSubClassOf(triObjectClassID)) {
		...
		BitArray holdFaceSel = mesh.FaceSel();
		int originalFaceCount = mesh.numFaces;
		if (copy) {
		...
		//sometimes the interface sticks around but its data gets cleared so need to watch for this
		if  (nspec)
		{
			if ((nspec->GetNumNormals() == 0) || (nspec->GetNumFaces() == 0) )
				nspec = NULL;
		}
		...
		if (nspec && copy)
		{			
			nspec->SetNumFaces(mesh.numFaces);
			int currentFaceIndex = originalFaceCount;
			for (int faceIndex = 0; faceIndex < holdFaceSel.GetSize(); faceIndex++)
			{
				if (holdFaceSel[faceIndex])
				{
					if ( (faceIndex < nspec->GetNumFaces()) && (currentFaceIndex < nspec->GetNumFaces()))
					{
						MeshNormalFace& sourceFace = nspec->Face(faceIndex);
						MeshNormalFace& destFace = nspec->Face(currentFaceIndex);
						destFace = sourceFace;					
						currentFaceIndex++;
					}
				}
		...
		if (axis<3) {
			...
			if (mesh.GetFlag (MN_MESH_FILLED_IN)) {
				...
				for (int i=0; i<mesh.ENum(); i++) {
					int faceNo = mesh.e[i].f1;
					DbgAssert(faceNo >= 0 && faceNo < mesh.FNum());
					if (faceNo >= 0 && faceNo < mesh.FNum() &&
						mesh.f[faceNo].GetFlag (MN_SEL))
					{
...
TSTR MirrorMod::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case TM_REF: return localized ? GetString(IDS_RB_MIRRORCENTER) : _T("Mirror Center"); break;
...
RefResult MirrorMod::NotifyRefChanged(
	...
	switch (message) {
		...
		case REFMSG_GET_PARAM_NAME_NONLOCALIZED: {
			GetParamName *gpn = (GetParamName*)partID;
			switch (gpn->index) {
				case PB_OFFSET: gpn->name = _T("Mirror Offset"); break;
				}
			return REF_HALT;
			}
		}
...
void MirrorPostLoadCallback::proc(ILoad *iload) {
	...
	DWORD oldVer = ((MirrorMod*)(cb->GetTarget()))->pblock->GetVersion();
