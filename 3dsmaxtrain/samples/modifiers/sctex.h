...
#include <Materials/LightDescImp.h>
...
public:
	...
	void ClosestPoint(const Point3& p, float radius, int &i, float &d);
...
private:
	...
	void TagCells(const Point3& p,float radius, int whichGrid, Tab<int>& hitList, BitArray& xHitList, BitArray& yHitList, BitArray& zHitList);
	...
	Tab<Tab<int>> hitLists; //Use separate Tab<int> list for each thread
	Tab<BitArray> xHitLists, yHitLists, zHitLists; //Use separate hit list for each thread
