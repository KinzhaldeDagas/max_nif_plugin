...
class BombObject: public WSMObject {
	...
	public:
		...
		float GetStrength(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetStrength)
		float GetGravity(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetGravity)
		float GetChaos(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetChaos)
		TimeValue GetDetonation(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetDetonation)
		float GetSpin(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetSpin)
		float GetFalloff(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetFalloff)
		int GetFalloffOn(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetFalloffOn)
		int GetMinFrag(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetMinFrag)
		int GetMaxFrag(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetMaxFrag)
		int GetSeed(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetSeed)

		//  inherited virtual methods:

		// From Animatable
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_RB_BOMBOBJECT_CLASS) : _T("MeshBombObject"); }  
...
public:
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_BOMB) : _T("MeshBomb");}
...
class BombClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Bomb"); }
...
class BombMod : public Modifier {
	...
	public:
		...
		ChannelMask ChannelsChanged() { return GEOM_CHANNEL|TOPO_CHANNEL|SELECT_CHANNEL; }
		...
		// From Animatable
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_RB_BOMBMOD) : _T("MeshBombMod"); }  
...
public:
		...
		void EndEditParams( IObjParam *ip, ULONG flags,Animatable *next );
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_BOMBBINDING) : _T("MeshBomb Binding"); }
	...
	switch (message) {
		...
		case REFMSG_GET_PARAM_NAME_LOCALIZED: {
			...
			switch (gpn->index) {
				...
				case PB_GRAVITY:    gpn->name = GetString(IDS_RB_GRAVITY);    break;
				case PB_CHAOS:      gpn->name = GetString(IDS_RB_CHAOS);      break;
				case PB_DETONATION: gpn->name = GetString(IDS_RB_DETONATION); break;
				case PB_SPIN:       gpn->name = GetString(IDS_RB_SPIN);       break;
		...
		case REFMSG_GET_PARAM_NAME_NONLOCALIZED: {
			GetParamName *gpn = (GetParamName*)partID;
			switch (gpn->index) {
				case PB_STRENGTH:   gpn->name = _T("Strength");   break;
				case PB_GRAVITY:    gpn->name = _T("Gravity");    break;
				case PB_CHAOS:      gpn->name = _T("Chaos");      break;
				case PB_DETONATION: gpn->name = _T("Detonation"); break;
				case PB_SPIN:       gpn->name = _T("Spin");       break;
				case PB_FALLOFF:    gpn->name = _T("Falloff");    break;
				}
			return REF_HALT;
			}
		}
...
RefResult BombMod::NotifyRefChanged(
