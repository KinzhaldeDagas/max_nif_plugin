...
public:
   ...
   ChannelMask ChannelsUsed()  { return GEOM_CHANNEL | TOPO_CHANNEL | DISP_APPROX_CHANNEL | TEXMAP_CHANNEL; }
   ChannelMask ChannelsChanged() { return DISP_APPROX_CHANNEL; }

   void ModifyObject(TimeValue t, ModContext &mc, ObjectState *os, INode *node);
   RefTargetHandle Clone(RemapDir& remap);
   Class_ID ClassID() { return DISP_APPROX_CLASSID; }
   virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("DispApproxMod"); } // from Animatable
   Class_ID InputType() { return triObjectClassID; }
   void BeginEditParams(IObjParam  *ip, ULONG flags,Animatable *prev);
   void EndEditParams(IObjParam *ip,ULONG flags,Animatable *next);
   IOResult Save(ISave *isave);
   IOResult Load(ILoad *iload);
   const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_CT_DISP_APPROX) : _T("Displacement Approx.");}
...
public:
  ...
  const TCHAR*   NonLocalizedClassName() { return _T("Disp Approx"); }
...
public:
   ...
   virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("MeshMesherMod"); } // from Animatable
   Class_ID InputType() { return triObjectClassID; }
   void BeginEditParams(IObjParam  *ip, ULONG flags,Animatable *prev);
   void EndEditParams(IObjParam *ip,ULONG flags,Animatable *next);
   IOResult Save(ISave *isave);
   IOResult Load(ILoad *iload);
   const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_CT_MESH_MESHERBINDING) : _T("Displace Mesh Binding");}
...
public:
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Displace Mesh"); }
   SClass_ID      SuperClassID() { return WSM_CLASS_ID; }
   Class_ID    ClassID() { return MESH_MESHER_CLASSID; }
   const TCHAR*   Category() { return GetString(IDS_RB_DEFEDIT);}
   void        ResetClassParams(BOOL fileReset) { /* Noop */ }
...
{
   DispApproxMod *pMod = DLGetWindowLongPtr<DispApproxMod*>(hWnd);
      ...
      case IDC_ADVANCED: {
         ...
         if (retval == 1) {
            ...
            if ((sParams.mStyle == SUBDIV_DELAUNAY && sParams.mTris > 200000) ||
               ...
               if (MaxSDK::MaxMessageBox(hWnd, warning, title, MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON2) == IDYES)
                  confirm = TRUE;
