...
class TwistMod : public SimpleMod {
   ...
   public:
      ...
      void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_RB_TWISTMOD) : _T("TwistMod"); }
      virtual Class_ID ClassID() { return Class_ID(TWISTOSM_CLASS_ID,0);}
      void BeginEditParams( IObjParam  *ip, ULONG flags,Animatable *prev );
      void EndEditParams( IObjParam *ip, ULONG flags,Animatable *next );
      RefTargetHandle Clone(RemapDir& remap);
      const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_TWIST) : _T("Twist"); }
...
public:
	...
	int doRegion, doBias;
	TwistDeformer();
	TwistDeformer(TimeValue t, ModContext& mc, float angle, int naxis, float bias, float from, float to, int doRegion,
...
class TwistClassDesc:public ClassDesc {
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Twist"); }
...
static ParamUIDesc descParam[] = {
   ...
   // Affect region
   ParamUIDesc(PB_DOREGION,TYPE_SINGLECHECKBOX,IDC_TWIST_AFFECTREGION),
...
Point3 TwistDeformer::Map(int i, Point3 p) const
   {
...
bool TwistDeformer::IsThreadSafe() const
{
	return true;
}
...
TSTR TwistMod::GetParameterName(int pbIndex, bool localized)
   {
   switch (pbIndex) {
      case PB_ANGLE: return localized ? GetString(IDS_RB_ANGLE) : _T("Angle");
      ...
      case PB_BIAS:  return localized ? GetString(IDS_RB_BIAS)  : _T("Bias");
