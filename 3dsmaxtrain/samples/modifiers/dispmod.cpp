 ...
 *<
	...
	HISTORY: 10/21/95
	11/16/2024 - M.Kaustinen Performance Improvements
...
template <class T>
class DispPathAccessor : public IAssetAccessor
{
public:
	DispPathAccessor<T>(T* aDisp)
			: mDisp(aDisp)
	{
	}
	...
	{
		return MaxSDK::AssetManagement::kBitmapAsset;
	}
...
template <class T>
MaxSDK::AssetManagement::AssetUser DispPathAccessor<T>::GetAsset() const
{
	return mDisp->bi.GetAsset();
}

template <class T>
bool DispPathAccessor<T>::SetAsset(const MaxSDK::AssetManagement::AssetUser& aNewAsset)
{
	mDisp->bi.SetAsset(aNewAsset);
	mDisp->FreeBitmap();
	mDisp->LoadBitmap();
	mDisp->NotifyDependents(FOREVER, PART_GEOM, REFMSG_CHANGE);
	return true;
}

// Mod Data Cache for Vertex Normals and UVs
class DispModData : public LocalModData
{
private:
	Interval iValid; // Validity Interval
	int	iMap; // What Channel are we using

public:
	Tab<Point3>	vNorms; // Vertex Normal Array
	Tab<Point3>	existingUVs; // UV values
	...
	void	SetNormalCount(int num)
	{
		if (num <= 0)
			vNorms.ZeroCount();
		else
		{
			vNorms.SetCount(num, FALSE);
			ZeroMemory(vNorms.Addr(0), sizeof(Point3) * num);
		}
	}

	void	SetUVCount(int num)
	{
		if (num <= 0)
			existingUVs.ZeroCount();
		else
		{
			existingUVs.SetCount(num, FALSE);
			ZeroMemory(existingUVs.Addr(0), sizeof(Point3) * num);
		}
	}
	...
	void SetValidity(Interval iv, int chan)
	{
		iValid = iv;
		iMap = chan;
	}

	bool CheckValidity(TimeValue t, int chan)
	{
		return (iValid.InInterval(t) && (iMap == chan));
	}

	LocalModData* Clone()
	{
		// Base copy only, don't copy cache
		auto* clone = new DispModData();
		return clone;
	}
};
...
public:
	...
	{
		s = localized ? GetString(IDS_RB_DISPMOD) : _T("DispMod");
	}
	virtual Class_ID ClassID()
	{
		return Class_ID(DISPLACEOSM_CLASS_ID, 0);
	}
	void BeginEditParams(IObjParam* ip, ULONG flags, Animatable* prev);
	void EndEditParams(IObjParam* ip, ULONG flags, Animatable* next);
	const TCHAR* GetObjectName(bool localized) const override
	{
		return localized ? GetString(IDS_RB_DISPLACE) : _T("Displace");
	}
	...
	// See if we have to blow out our cache
	void NotifyInputChanged(const Interval& changeInt, PartID part, RefMessage message, ModContext* mc) override
	{
		if (!(part & (PART_TOPO | PART_GEOM)))
			return;
		
		DispModData* dmd = (DispModData*)mc->localData;
		if (dmd)
			dmd->ResetValidity();
	}
...
public:
	void EnumAuxFiles(AssetEnumCallback& nameEnum, DWORD flags)
	{
		...
		if (flags & FILE_ENUM_ACCESSOR_INTERFACE)
		{
			...
			callback->DeclareAsset(DispPathAccessor<DispMod>(this));
		}
		else
		{
...
public:
	...
	Class_ID ClassID()
	{
		return Class_ID(DISPLACE_OBJECT_CLASS_ID, 0);
	}
	void EnumAuxFiles(AssetEnumCallback& nameEnum, DWORD flags)
	{
		if ((flags & FILE_ENUM_CHECK_AWORK1) && TestAFlag(A_WORK1))
		...
		{
			IEnumAuxAssetsCallback* callback = static_cast<IEnumAuxAssetsCallback*>(&nameEnum);
			callback->DeclareAsset(DispPathAccessor<DisplaceObject>(this));
		}
		else
		{
			bi.EnumAuxFiles(nameEnum, flags);
	...
	{
		if (i == 0)
			return SimpleWSMObject::SubAnimName(i, localized);
		else
			return _T("Displacement Map");
	}
...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override
	{
		s = localized ? GetString(IDS_RB_DISPLACEMOD) : _T("DisplaceMod");
	}
	...
	Class_ID ClassID()
	{
		return Class_ID(DISPLACE_WSM_CLASS_ID, 0);
	}
	RefTargetHandle Clone(RemapDir& remap);
	const TCHAR* GetObjectName(bool localized) const override
	{
		return localized ? GetString(IDS_RB_DISPLACEBINDING) : _T("Displace Binding");
	}
...
public:
	...
	{
		return new DispMod(!loading);
	}
	const TCHAR* ClassName()
	{
		return GetString(IDS_RB_DISPLACE_CLASS);
	}
	...
	{
		return GetString(IDS_RB_DEFDEFORMATIONS);
	}
};
...
public:
	...
	const TCHAR* Category()
	{
		return GetSpaceWarpCatString(SPACEWARP_CAT_GEOMDEF);
	}
};
...
static ParamUIDesc descParam[] = {
	...
	// U Tile
	ParamUIDesc(PB_UTILE, EDITTYPE_FLOAT, IDC_DISP_UTILE, IDC_DISP_UTILESPIN, -999999999.0f, 999999999.0f, 0.01f),

	// V Tile
	ParamUIDesc(PB_VTILE, EDITTYPE_FLOAT, IDC_DISP_VTILE, IDC_DISP_VTILESPIN, -999999999.0f, 999999999.0f, 0.01f),

	// W Tile
	ParamUIDesc(PB_WTILE, EDITTYPE_FLOAT, IDC_DISP_WTILE, IDC_DISP_WTILESPIN, -999999999.0f, 999999999.0f, 0.01f),

	// Blur
	ParamUIDesc(PB_BLUR, EDITTYPE_FLOAT, IDC_DISP_BLUR, IDC_DISP_BLURSPIN, 0.0f, 10.0f, 0.01f),

	// Strength
	ParamUIDesc(PB_STRENGTH, EDITTYPE_UNIVERSE, IDC_DISP_STRENGTH, IDC_DISP_STRENGTHSPIN, -999999999.0f, 999999999.0f,
			SPIN_AUTOSCALE),

	// Decay
	ParamUIDesc(PB_DECAY, EDITTYPE_FLOAT, IDC_DISP_DECAY, IDC_DISP_DECAYSPIN, -999999999.0f, 999999999.0f, 0.005f),

	// Center lum
	ParamUIDesc(PB_CENTERLUM, TYPE_SINGLECHECKBOX, IDC_DISP_CENTERLUM),

	// U Flip
	ParamUIDesc(PB_UFLIP, TYPE_SINGLECHECKBOX, IDC_DISP_UFLIP),

	// V Flip
	ParamUIDesc(PB_VFLIP, TYPE_SINGLECHECKBOX, IDC_DISP_VFLIP),

	// W Flip
	ParamUIDesc(PB_WFLIP, TYPE_SINGLECHECKBOX, IDC_DISP_WFLIP),

	// Center
	ParamUIDesc(PB_CENTERL, EDITTYPE_FLOAT, IDC_DISP_CENTERL, IDC_DISP_CENTERLSPIN, 0.0f, 1.0f, 0.005f),

	// Length
	ParamUIDesc(PB_LENGTH, EDITTYPE_UNIVERSE, IDC_DISP_LENGTH, IDC_DISP_LENGTHSPIN, -999999999.0f, 999999999.0f,
			SPIN_AUTOSCALE),

	// Width
	ParamUIDesc(PB_WIDTH, EDITTYPE_UNIVERSE, IDC_DISP_WIDTH, IDC_DISP_WIDTHSPIN, -999999999.0f, 999999999.0f,
			SPIN_AUTOSCALE),

	// Height
	ParamUIDesc(PB_HEIGHT, EDITTYPE_UNIVERSE, IDC_DISP_HEIGHT, IDC_DISP_HEIGHTSPIN, -999999999.0f, 999999999.0f,
			SPIN_AUTOSCALE),

	// Cap
	ParamUIDesc(PB_CAP, TYPE_SINGLECHECKBOX, IDC_DISP_CAP),

	// Axis
	ParamUIDesc(PB_AXIS, TYPE_RADIO, axisIDs, 3),

	// Use existingUVs mapping
	ParamUIDesc(PB_USEMAP, TYPE_SINGLECHECKBOX, IDC_DISP_USEMAP),

	// Apply mapping
	ParamUIDesc(PB_APPLYMAP, TYPE_SINGLECHECKBOX, IDC_DISP_APPLYMAP),
	...
	// Which map channel?
	ParamUIDesc(
			PB_MAPCHANNEL, EDITTYPE_POS_INT, IDC_DISP_CHAN, IDC_DISP_CHAN_SPIN, 1, MAX_MESHMAPS - 1, SPIN_AUTOSCALE),
};
...
{
public:
	DispDlgProc(DispMod* m);
	DispDlgProc(DisplaceObject* o);
	~DispDlgProc();
	...
	void Drop(ReferenceTarget* dropThis, HWND hwnd, POINT p, SClass_ID type, DADMgr* srcMgr = NULL,
...
DispDlgProc::DispDlgProc(DispMod* m)
{
	mod = m;
	obj = NULL;
}

DispDlgProc::DispDlgProc(DisplaceObject* o)
{
	mod = NULL;
	obj = o;
}

DispDlgProc::~DispDlgProc()
{
	mod = NULL;
	obj = NULL;
}

void DispDlgProc::RedrawViews()
{
	Interface* ip = obj ? obj->ip : mod->ip;
	ip->RedrawViews(ip->GetTime(), REDRAW_NORMAL);
}

SClass_ID DispDlgProc::GetDragType(HWND hwnd, POINT p)
{
...
BOOL DispDlgProc::OkToDrop(ReferenceTarget* dropThis, HWND hfrom, HWND hto, POINT p, SClass_ID type, BOOL isNew)
{
	if (hfrom == hto)
		return FALSE;
...
ReferenceTarget* DispDlgProc::GetInstance(HWND hwnd, POINT p, SClass_ID type)
{
	if (hwnd == GetDlgItem(hWnd, IDC_DISP_PICKMAP))
	{
...
void DispDlgProc::Drop(ReferenceTarget* dropThis, HWND hwnd, POINT p, SClass_ID type, DADMgr*, BOOL)
{
	if (hwnd == GetDlgItem(hWnd, IDC_DISP_PICKMAP))
	{
	...
	else if (hwnd == GetDlgItem(hWnd, IDC_DISP_PICKIMAGE))
	{
		...
		if (mod)
		{
			AssetUser asset = IAssetManager::GetInstance()->GetInstance()->GetAsset(bmc->GetName(), kBitmapAsset);
			mod->bi.SetAsset(asset);
			mod->FreeBitmap();
			mod->LoadBitmap();
			mod->NotifyDependents(FOREVER, PART_GEOM, REFMSG_CHANGE);
		}
...
void DispDlgProc::DisableControls()
{
	...
	BitmapInfo& bitmapInfo = NULL != mod ? mod->bi : obj->bi;

	if (bitmapInfo.GetAsset().GetId() != kInvalidId)
	{
		GetBMName(bitmapInfo, fname);
	}
	...
	int type = -1;
	if (mod)
		mod->pblock->GetValue(PB_MAPTYPE, 0, type, FOREVER);
	else if (obj)
		obj->pblock->GetValue(PB_MAPTYPE, 0, type, FOREVER);

	BOOL cent = FALSE;
	if (mod)
		mod->pblock->GetValue(PB_CENTERLUM, 0, cent, FOREVER);
	else if (obj)
		obj->pblock->GetValue(PB_CENTERLUM, 0, cent, FOREVER);
	...
	BOOL existingUVs = FALSE;
	if (mod)
		mod->pblock->GetValue(PB_USEMAP, 0, existingUVs, FOREVER);
	if (existingUVs)
	{
		l = FALSE;
		w = FALSE;
	...
	existingUVs = !existingUVs;
	EnableWindow(GetDlgItem(hWnd, IDC_DISP_PLANAR), existingUVs);
	EnableWindow(GetDlgItem(hWnd, IDC_DISP_CYL), existingUVs);
	EnableWindow(GetDlgItem(hWnd, IDC_DISP_SPHERE), existingUVs);
	EnableWindow(GetDlgItem(hWnd, IDC_DISP_BALL), existingUVs);
	if (mod)
		EnableWindow(GetDlgItem(hWnd, IDC_DISP_CAP), existingUVs);
...
void DispDlgProc::SetImageName(HWND hWnd)
{
	...
	if (but)
	{
		if (fname.length() > 0)
			...
			but->SetText(GetString(IDS_RB_NONE));
...
void DispDlgProc::PickImage(HWND hWnd)
{
	...
	if (!bi->Name() || bi->Name()[0] == _T('\0'))
	{
		TSTR path = IPathConfigMgr::GetPathConfigMgr()->GetDir(APP_IMAGE_DIR);
		if (path[path.length() - 1] != _T('\\'))
		{
			path += _T("\\");
		}
		bi->SetName(path.data());
	}
	if (TheManager->SelectFileInputEx(bi, GetCOREInterface()->GetMAXHWnd(), GetString(IDS_RB_SELECTDISPIMAGE)))
	{
		if (mod)
		{
			mod->FreeBitmap();
...
void DispDlgProc::SetMapName(HWND hWnd)
{
	...
	if (but)
	{
		if ((mod && mod->tmap) || (obj && obj->tmap))
		{
			TSTR fname;
			if (mod)
				fname = mod->tmap->GetFullName(true);
			else
				fname = obj->tmap->GetFullName(true);
			but->SetText(fname);
...
void DispDlgProc::PickMap(HWND hWnd)
{
	...
	MtlBase* mtl = ip->DoMaterialBrowseDlg(
			GetCOREInterface()->GetMAXHWnd(), BROWSE_MAPSONLY | BROWSE_INCNONE | BROWSE_INSTANCEONLY, newMat, cancel);
	if (cancel)
	{
		if (newMat)
			mtl->MaybeAutoDelete();
...
void DispDlgProc::RemoveMap(HWND hWnd)
{
	...
	else if ((obj) && (obj->tmap))
		mlib1.FindMtlByName(obj->tmap->GetName());
...
{
	BitmapInfo bi;
	if (TheManager->SelectFileInputEx(&bi, GetCOREInterface()->GetMAXHWnd(), GetString(IDS_RB_SELECTIMAGE)))
	{
...
INT_PTR DispDlgProc::DlgProc(TimeValue t, IParamMap* map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
	case WM_INITDIALOG:
	...
	case WM_COMMAND:
		...
		case IDC_DISP_PICKIMAGE:
			theHold.Begin();
			if (mod)
				theHold.Put(new BitmapRestore(mod));
			else if (obj)
				theHold.Put(new BitmapObjRestore(obj));
			PickImage(hWnd);
			theHold.Accept(GetString(IDS_PW_PICKMAP));
			DisableControls();
			break;
...
static void DisplacementMapNotify(void* param, NotifyInfo* info)
{
	// we get this notification when a NOTIFY_BITMAP_CHANGED is sent
	// this fixes the bug 664728. Laurent R. 08/05
	DispMod* l_dispMod = (DispMod*)param;

	switch (info->intcode)
	{
	case NOTIFY_BITMAP_CHANGED:
	{
		const TCHAR* l_fileName = GetNotifyParam<NOTIFY_BITMAP_CHANGED>(info);
		if (l_fileName == NULL || !_tcscmp(l_fileName, l_dispMod->bi.Name()))
		{
			// load the correct file name
			l_dispMod->LoadBitmap();
			if (l_dispMod->thebm != NULL)
				// file loaded successfully
				l_dispMod->NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
		}
		break;
	}
	}
...
DispMod::DispMod(BOOL create)
{
	...
	ReplaceReference(PBLOCK_REF, CreateParameterBlock(descVer6, PBLOCK_LENGTH, CURRENT_VERSION));
	...
	RegisterNotification(DisplacementMapNotify, this, NOTIFY_BITMAP_CHANGED);
}

DispMod::~DispMod()
{
	UnRegisterNotification(DisplacementMapNotify, this, NOTIFY_BITMAP_CHANGED);
...
void DispMod::BeginEditParams(IObjParam* ip, ULONG flags, Animatable* prev)
{
	...
	pmapParam = CreateCPParamMap(descParam, PARAMDESC_LENGH, pblock, ip, hInstance, MAKEINTRESOURCE(IDD_DISPLACEPARAM),
...
void DispMod::EndEditParams(IObjParam* ip, ULONG flags, Animatable* next)
{
	...
	delete squashMode;
	squashMode = NULL;
	delete faceAlignMode;
	faceAlignMode = NULL;
	delete regionFitMode;
	regionFitMode = NULL;
...
RefTargetHandle DispMod::Clone(RemapDir& remap)
{
	...
	if (TestAFlag(A_PLUGIN1))
		...
		newmod->ClearAFlag(A_PLUGIN1);
	newmod->ReplaceReference(PBLOCK_REF, remap.CloneRef(pblock));
	if (tmap)
		newmod->ReplaceReference(2, tmap);
	if (tmControl)
	{
	...
	if ((errorReturn != BMMRES_SUCCESS) && (netRendering))
	{
		if (errorReturn == BMMRES_FILENOTFOUND)
			GetCOREInterface()->Log()->LogEntry(
					SYSLOG_ERROR, NO_DIALOG, NULL, _T("%s: %s"), GetString(IDS_PW_MAPMISSING), bi.Name());
		else
			GetCOREInterface()->Log()->LogEntry(
					SYSLOG_ERROR, NO_DIALOG, NULL, _T("%s: %s"), GetString(IDS_PW_MAPERROR), bi.Name());
	}
	TheManager->SetSilentMode(silentMode);
}
...
ChannelMask DispMod::ChannelsChanged()
{
	// We only update the TEXMAP channel if applymap is enabled
	if (pblock->GetInt(PB_APPLYMAP))
		return GEOM_CHANNEL | TEXMAP_CHANNEL;
...
void DispMod::SetReference(int i, RefTargetHandle rtarg)
{
	...
	case 2:
		...
		{
			((DispDlgProc*)pmapParam->GetUserDlgProc())->SetMapName(pmapParam->GetHWnd());
		}
		break;
	}
}
...
TSTR DispMod::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case TM_REF:
		return localized ? GetString(IDS_RB_GIZMO) : _T("Gizmo");
	case PBLOCK_REF:
		return localized ? GetString(IDS_RB_PARAMETERS) : _T("Parameters");
	case 2:
		return _T("Displacement Map");
	default:
		return TSTR(_T(""));
		...
		const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
	...
	case REFMSG_GET_PARAM_NAME_LOCALIZED:
	{
		...
		case PB_STRENGTH:
			...
			break;
		case PB_BLUR:
			gpn->name = GetString(IDS_RB_BLUR);
			break;
		case PB_DECAY:
			gpn->name = GetString(IDS_RB_DECAY);
			break;
		case PB_CENTERL:
			gpn->name = GetString(IDS_RB_CENTERPOINT);
			break;
		case PB_LENGTH:
		...
		case PB_HEIGHT:
			...
			break;
		case PB_CHANNEL:
			gpn->name = GetString(IDS_UVWX_CHANNEL);
			break;
		case PB_MAPCHANNEL:
			gpn->name = GetString(IDS_UVWX_MAPCHANNEL);
			break;
		default:
			gpn->name = TSTR(_T(""));
			break;
	...
	case REFMSG_GET_PARAM_NAME_NONLOCALIZED:
	{
		...
		case PB_HEIGHT:
			...
			break;
		case PB_CHANNEL:
			gpn->name = _T("Map or Vertex Color");
			break;
		case PB_MAPCHANNEL:
			gpn->name = _T("Map Channel");
			break;
		default:
			gpn->name = TSTR(_T(""));
			break;
...
public:
	DispMod* mm;
	DispSetChannelToOne(DispMod* mapMod)
	{
		mm = mapMod;
	}
	void proc(ILoad* iload)
	{
		if (mm && mm->pblock)
		{
			...
			if (!mapChan)
				mm->pblock->SetValue(PB_MAPCHANNEL, 0, 1);
		}
		delete this;
	}
};
...
IOResult DispMod::Load(ILoad* iload)
{
	...
	iload->RegisterPostLoadCallback(new FixDimsPLCB(this));

	iload->RegisterPostLoadCallback(new ParamBlockPLCB(versions, NUM_OLDVERSIONS, &curVersion, this, PBLOCK_REF));
...
public:
	...
	float utile, vtile;
	float offset, mult, decay;
	Point3* existingUVs;
	Point3* normals;

	// Precomputed blur values
	float	blurFactor;
	Point3	p3Blur;
	...
	DispDeformer(Matrix3& tm, Matrix3& invtm, Bitmap* bm, Texmap* tmap, int type, float strength, float utile,
			float vtile, int uflip, int vflip, float blur, float decay, int centerLum, float center, int cap,
			Tab<Point3>* existingUVs, Tab<Point3>* normals, TimeValue t);
	Point3 Map(int i, Point3 p) const;
	Point3 MapForce(Point3 p) const;
	Point2 MapPoint(Point3 p, Point3& v, float& dist, Point3 norm) const;
	float GetPixel(Point3 pt, float u, float v, float w) const;
	BOOL InRange(const Point2& uv) const;
};

DispDeformer::DispDeformer(Matrix3& tm, Matrix3& invtm, Bitmap* bm, Texmap* tmap, int type, float strength, float utile,
		float vtile, int uflip, int vflip, float blur, float decay, int centerLum, float center, int cap,
		Tab<Point3>* existingUVs, Tab<Point3>* normals, TimeValue t)
{
	this->tm = tm;
	...
	// precompute blur factors
	if (blur != 0.0f)
		blurFactor = (blur * 10.0f + 1.0f);
	else
		blurFactor = 0.0f;

	p3Blur = Point3(blur / 10.0f + 0.001f, blur / 10.0f + 0.001f, blur / 10.0f + 0.001f);

	this->decay = decay;
	if (existingUVs && existingUVs->Count())
		this->existingUVs = existingUVs->Addr(0);
	else
		this->existingUVs = nullptr;

	if (normals && normals->Count())
		this->normals = normals->Addr(0);
	else
...
{
	int l = 0;
	if (fabs(p.y) > fabs(p[l]))
		l = 1;
	if (fabs(p.z) > fabs(p[l]))
		l = 2;
	return l;
}

Point2 DispDeformer::MapPoint(Point3 p, Point3& v, float& dist, Point3 norm) const
{
	switch (type)
	{
	case MAP_PLANAR:
...
float DispDeformer::GetPixel(Point3 pt, float u, float v, float w) const
{
	if (!thebm && !tmap)
		return 1.0f;
	...
	{
		BMM_Color_64 c;
		int bmWidth = thebm->Width();
		int bmHeight = thebm->Height();
		if (blurFactor == 0.0f)
		{
			int w = bmWidth - 1;
			int h = bmHeight - 1;
		...
		{
			float wb = blurFactor / float(bmWidth);
			float hb = blurFactor / float(bmHeight);
	...
	if (tmap)
	{
		...
		shadeContext.pt = pt;
		shadeContext.dpt = p3Blur;
		shadeContext.curTime = curTime;

		AColor c = tmap->EvalColor(shadeContext);
		f += c.r * 0.299f;
		f += c.g * 0.587f;
		f += c.b * 0.114f;
	}
...
BOOL DispDeformer::InRange(const Point2& uv) const
{
	if (existingUVs)
		return TRUE;
...
Point3 DispDeformer::Map(int i, Point3 p) const
{
	Point3 norm;
	Point3 uv; 
	...
	if (existingUVs)
	{
		...
		if (decay != 0.0f) {
			float dist = Length(p * tm);
			m *= (float)exp(-decay * dist);
		}

	}
	else
	{
		Point3 mappedPoint = p * tm;
		float dist;
		Point2 uv2 = MapPoint(mappedPoint, norm, dist, normals ? normals[i] : Point3(1, 0, 0));

		if (!InRange(Point2(uv2.x, uv2.y)))
			return p;
		...
		norm = Normalize(VectorTransform(invtm, norm));
	}
	...
	float s = GetPixel(p, uv.x * utile, uv.y * vtile, uv.z) + offset; 
...
Point3 DispDeformer::MapForce(Point3 p) const
{
	Point3 v;
	float dist;

	Point3 mappedPoint = p * tm;
	Point2 uv = MapPoint(mappedPoint, v, dist, Point3(1, 0, 0));
	if (!InRange(uv))
		return Point3(0, 0, 0);
	...
	float s = GetPixel(p, uv.x * utile, uv.y * vtile, 0.0f);
	s = (s + offset) * m;
	v = Normalize(VectorTransform(invtm, v));
	return v * (s * 0.00001f * forceScaleFactor);
}
...
void DispMod::ModifyObject(TimeValue t, ModContext& mc, ObjectState* os, INode* node)
{
	...
	if (!tmControl || (flags & CONTROL_OP) || (flags & CONTROL_INITPARAMS))
		InitControl(mc, os->obj, type, t, os);

	Interval valid = GetValidity(t);

	Matrix3 invtm = CompMatrix(t, &mc, NULL);
	Matrix3 tm = Inverse(invtm);

	// Get our modData
	if (mc.localData == nullptr)
		mc.localData = new DispModData;
	...
	// Populate the per vertex normals and uvs if necessary
	bool bUseNormals = (cap || useExisting);
	bool bUseUVs = (useExisting);

	if (bUseNormals && !dmd->CheckValidity(t, chan))
	{
		Interval	meshInterval = os->obj->ChannelValidity(t, GEOM_CHANNEL);
		meshInterval &= os->obj->ChannelValidity(t, TOPO_CHANNEL);
		dmd->SetValidity(meshInterval, chan);
		...
		if (os->obj->IsSubClassOf(triObjectClassID))
		{
			...
			if (bUseNormals)
			{
				...
				dmd->SetNormalCount(numVerts);
				if (bUseUVs) //tf && useExisting)
					dmd->SetUVCount(numVerts);

				Face* thisFace = mesh.faces;
				for (int i = 0; i < mesh.getNumFaces(); i++)
				{
					Point3 v0 = mesh.verts[thisFace->v[1]] - mesh.verts[thisFace->v[0]];
					Point3 v1 = mesh.verts[thisFace->v[2]] - mesh.verts[thisFace->v[1]];
					Point3 n = v0 ^ v1;

					for (int j = 0; j < 3; j++)
					{
						if (bUseUVs) //tf && useExisting)
							dmd->existingUVs[thisFace->v[j]] = tv[tf[i].t[j]];

						dmd->vNorms[thisFace->v[j]] += n;
					}
				...
				for (int i = 0; i < numVerts; i++)
					dmd->vNorms[i] = Normalize(dmd->vNorms[i]);
			}
		}
		else if (os->obj->IsSubClassOf(patchObjectClassID))
		{
			...
			PatchTVert* tv = nullptr;
			TVPatch* tf = nullptr;
			if (bUseUVs && mesh.getMapSupport(chan))
			{
				tv = mesh.tVerts[chan];
				tf = mesh.tvPatches[chan];
				if (tv == nullptr || tf == nullptr)
				{
					bUseUVs = false;
					bUseNormals = cap;
				}
			...
			{
				const int		numVerts = mesh.getNumVerts();
				const int		numVectors = mesh.getNumVecs();
				const int		cacheSize = numVerts + numVectors;

				dmd->SetNormalCount(cacheSize);
				if (bUseUVs)
					dmd->SetUVCount(cacheSize);

				const int numPatches = mesh.getNumPatches();
				for (int i = 0; i < numPatches; i++)
				{
					const int count = mesh.patches[i].type;

					for (int j = 0; j < count; j++)
					{
						if (bUseUVs)
						{
							dmd->existingUVs[mesh.patches[i].v[j]] = tv[tf[i].tv[j]];

							dmd->existingUVs[numVerts + mesh.patches[i].vec[j * 2]] = tv[tf[i].tv[j]];
							if (j == 0)
								dmd->existingUVs[numVerts + mesh.patches[i].vec[count * 2 - 1]] = tv[tf[i].tv[j]];
							else
								dmd->existingUVs[numVerts + mesh.patches[i].vec[j * 2 - 1]] = tv[tf[i].tv[j]];

							dmd->existingUVs[numVerts + mesh.patches[i].interior[j]] = tv[tf[i].tv[j]];
							...
							if (!(mesh.patches[j].flags & PATCH_LINEARMAPPING))
							{
								...
								if (index != -1)
								{
									dmd->existingUVs[numVerts + mesh.patches[i].vec[j * 2]] = tv[tf[i].handles[j * 2]];
									if (j == 0)
										dmd->existingUVs[numVerts + mesh.patches[i].vec[count * 2 - 1]] =
										tv[tf[i].handles[count * 2 - 1]];
									else
										dmd->existingUVs[numVerts + mesh.patches[i].vec[j * 2 - 1]] =
										tv[tf[i].handles[j * 2 - 1]];
								}
							}
							if (!(mesh.patches[j].flags & PATCH_AUTO))
							{
								...
								if (index != -1)
									dmd->existingUVs[numVerts + mesh.patches[i].interior[j]] = tv[tf[i].interiors[j]];
							}
						}
						...
						if (j == 0)
							...
							v1 = mesh.vecs[mesh.patches[i].vec[j * 2 - 1]].p - mesh.verts[mesh.patches[i].v[j]].p;

						Point3 n = v0 ^ v1;

						dmd->vNorms[mesh.patches[i].v[j]] += n;
						dmd->vNorms[numVerts + mesh.patches[i].interior[j]] += n;
						dmd->vNorms[numVerts + mesh.patches[i].vec[j * 2]] += n;
						if (j == 0)
							dmd->vNorms[numVerts + mesh.patches[i].vec[count * 2 - 1]] += n;
						else
							dmd->vNorms[numVerts + mesh.patches[i].vec[j * 2 - 1]] += n;
					}
				}
		...
		else if (os->obj->IsSubClassOf(polyObjectClassID))
		{
			...
			if (bUseNormals)
			{
				MNMapFace* tf = mesh.M(chan)->f;
				Point3* tv = mesh.M(chan)->v;
				...
				if (bUseNormals)
				{
					...
					MNFace* thisFace = mesh.f;
					for (int i = 0; i < mesh.numf; i++)
					{
						const int degree = thisFace->deg;

						// Note that Mesh doesn't normalize, but Poly does
						const Point3 norm = mesh.GetFaceNormal(i, TRUE);
						for (int j = 0; j < degree; j++)
						{
							if (bUseUVs)
								dmd->existingUVs[thisFace->vtx[j]] = tv[tf[i].tv[j]];

							dmd->vNorms[thisFace->vtx[j]] += norm;
						}
						thisFace++;
					}
	...
	// Apply the UVs (but only if not using existingUVs)
	if (apply && !bUseUVs)
	{
		os->obj->ApplyUVWMap(type, utile, vtile, wtile, uflip, vflip, wflip, FALSE, tm, chan);
	}

	DispDeformer deformer(tm, invtm, thebm, tmap, type, strength, utile, vtile, uflip, vflip, blur, decay, centerLum,
		center, cap, bUseUVs ? &(dmd->existingUVs) : nullptr, bUseNormals ? &(dmd->vNorms) : nullptr, t);
...
void DisplaceObject::BeginEditParams(IObjParam* ip, ULONG flags, Animatable* prev)
{
	...
	{
		hSot = ip->AddRollupPage(hInstance, MAKEINTRESOURCE(IDD_DISPLACE_SOT), DefaultSOTProc, GetString(IDS_RB_SOT),
				(LPARAM)ip, APPENDROLL_CLOSED);

		pmapParam = CreateCPParamMap(descParam, PARAMDESC_LENGH_SW, pblock, ip, hInstance,
				MAKEINTRESOURCE(IDD_DISPLACEPARAM_WS), GetString(IDS_RB_PARAMETERS), 0);
	}
	pmapParam->SetUserDlgProc(new DispDlgProc(this));
}

void DisplaceObject::EndEditParams(IObjParam* ip, ULONG flags, Animatable* next)
{
...
public:
	DisplaceObject* obj;
	FixDimsWSPLCB1(DisplaceObject* o)
	{
		obj = o;
	}
	void proc(ILoad* iload);
...
public:
	DisplaceObject* obj;
	FixDimsWSPLCB2(DisplaceObject* o)
	{
		obj = o;
	}
	void proc(ILoad* iload);
...
IOResult DisplaceObject::Load(ILoad* iload)
{
	...
	iload->RegisterPostLoadCallback(new FixDimsWSPLCB1(this));

	iload->RegisterPostLoadCallback(new ParamBlockPLCB(versions, NUM_OLDVERSIONS, &curVersion, this, 0));

	iload->RegisterPostLoadCallback(new FixDimsWSPLCB2(this));
...
TSTR DisplaceObject::GetParameterName(int pbIndex, bool localized)
{
	switch (pbIndex)
	{
	case PB_UTILE:
		return localized ? GetString(IDS_RB_UTILE) : _T("U Tile");
	case PB_VTILE:
		return localized ? GetString(IDS_RB_VTILE) : _T("V Tile");
	case PB_WTILE:
		return localized ? GetString(IDS_RB_WTILE) : _T("W Tile");
	case PB_BLUR:
		return localized ? GetString(IDS_RB_BLUR) : _T("Blur");
	case PB_STRENGTH:
		return localized ? GetString(IDS_RB_STRENGTH2) : _T("Strength");
	case PB_DECAY:
		return localized ? GetString(IDS_RB_DECAY) : _T("Decay");
	case PB_CENTERL:
		return localized ? GetString(IDS_RB_CENTERPOINT) : _T("Center");
	case PB_LENGTH:
		return localized ? GetString(IDS_RB_LENGTH) : _T("Length");
	case PB_WIDTH:
		return localized ? GetString(IDS_RB_WIDTH) : _T("Width");
	case PB_HEIGHT:
		return localized ? GetString(IDS_RB_HEIGHT) : _T("Height");
	default:
		return TSTR(_T(""));
	}
}
...
#define NUM_SEGS 16
	...
	else if (type == MAP_SPHERICAL || type == MAP_BALL)
	{
...
int DisplaceObjCreateCallBack::proc(ViewExp* vpt, int msg, int point, int flags, IPoint2 m, Matrix3& mat)
{
	Point3 d;
...
Deformer& DisplaceWSMod::GetDeformer(TimeValue t, ModContext& mc, Matrix3& mat, Matrix3& invmat)
{
	...
	deformer = DispDeformer(tm, invtm, dobj->thebm, dobj->tmap, type, strength, utile, vtile, uflip, vflip, blur, decay,
			centerLum, center, cap, NULL, NULL, t);
	return deformer;
}
...
Point3 DisplaceField::Force(TimeValue t, const Point3& pos, const Point3& vel, int index)
{
	...
	if (dobj == NULL)
		return Point3(0.0f, 0.0f, 0.0f); // 667105 watje xrefs can change the base object type through proxies which
										 // will cause this to be null or the user can copy a new object type over ours
	...
	DispDeformer deformer(tm, invtm, thebm, dobj->tmap, type, strength, utile, vtile, uflip, vflip, blur, decay,
			centerLum, center, cap, NULL, NULL, t);
		...
		const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
	...
	{
	case REFMSG_CHANGE:
		if (hTarget == pblock && pmapParam && pmapParam->GetUserDlgProc())
		{
			((DispDlgProc*)pmapParam->GetUserDlgProc())->DisableControls();
		}
		if (tmap == hTarget)
	...
	return SimpleWSMObject::NotifyRefChanged(changeInt, hTarget, partID, message, propagate);
}
