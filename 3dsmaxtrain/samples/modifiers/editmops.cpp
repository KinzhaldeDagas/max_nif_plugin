...
void Matrix3DebugPrint (Matrix3 & tm) {
	if (tm.GetIdentFlags() & POS_IDENT) DebugPrint (_T("   No translation\n"));
	if (tm.GetIdentFlags() & ROT_IDENT) DebugPrint (_T("   No rotation\n"));
	if (tm.GetIdentFlags() & SCL_IDENT) DebugPrint (_T("   No scaling\n"));
	for (int i=0; i<4; i++) {
...
void EditMeshMod::Attach (INode *node, bool & canUndo) {
	...
	if (clearMapChannel)
		mesh->SetFlag(MESH_CLEARMAPCHANNEL);
	else
		mesh->ClearFlag(MESH_CLEARMAPCHANNEL);
	...
	for (int i = 0; i < mcList.Count(); i++) {
		...
		case SL_EDGE:
		{
			...
			for (j=0; j<mesh->numFaces; j++) {
				...
				if ((k<2) && (edgeSel[j*3+(k+1)%3] || edgeSel[j*3+(k+2)%3]))
...
float EditMeshMod::GetAlpha (int mp, int *num, bool *differs) {
	...
	for (int i = 0; i < mcList.Count(); i++) {
		...
		BitArray& vertSel = mesh->VertSel();
		for (int i=0; i<mesh->getNumFaces(); i++) {
			if (fsel && !faceSel[i]) continue;
			DWORD *tt = cf[i].t;
			DWORD *vv = mesh->faces[i].v;
			for (int j=0; j<3; j++) {
				if (vsel && !vertSel[vv[j]]) continue;
				if (num) (*num)++;
				if (!init) {
					alpha = cv[tt[j]].x;
					init = TRUE;
...
void EditMeshMod::SetEdgeVis(BOOL vis) {
	...
	for (int i = 0; i < mcList.Count(); i++) {
		...
		for (int j=0; j<maxedge; j++) {
			if (edgeSel[j]) tmd.SetSingleEdgeVis (*mesh, j, vis, af);										
		}
		meshData->ApplyMeshDelta (tmd, this, ip->GetTime());
...
void EditMeshMod::AutoEdge(float thresh, int type) {
	...
	for (int i = 0; i < mcList.Count(); i++) {
		...
		for (int j=0; j<edges.Count(); j++) {						
			...
			DbgAssert (edges[j].f[1]!=UNDEFINED);
...
void ExplodeToObjects (Mesh *mesh, float thresh, INode *node, TSTR &name,
	...
	for (mp=-NUM_HIDDENMAPS; mp<mesh->getNumMaps(); mp++) {
...
int PickFaceMouseProc::proc (HWND hwnd, int msg, int point, int flags, IPoint2 m) {
	...
	case MOUSE_FREEMOVE:
		...
		vpt.SnapPreview (m, m, NULL, SNAP_FORCE_3D_RESULT);//|SNAP_SEL_OBJS_ONLY);
		if (HitTestFaces(m,vpt.ToPointer(),NULL,NULL)) SetCursor(ip->GetSysCursor(SYSCUR_SELECT));
		else SetCursor(LoadCursor(NULL,IDC_ARROW));
...
int CreateVertMouseProc::proc (HWND hwnd, int msg, int point, int flags, IPoint2 m ) {
	if (!hCurCreateVerts) hCurCreateVerts = UI::MouseCursors::LoadMouseCursor(UI::MouseCursors::CreateVertex);
...
Point3 CreateFaceMouseProc::GetVertexWP(int in_vertex)
{
	...
	INode *l_node = l_nodeTab[0];
	//do we need to call BeginEdit?
	Mesh *mesh = GetMeshData()->GetMesh(GetInterface()->GetTime());
	if(l_node&&in_vertex>=0&&in_vertex<mesh->numVerts)
		return l_node->GetObjectTM(GetInterface()->GetTime()) * mesh->verts[in_vertex];
...
bool CreateFaceMouseProc::HitTestVertex (IPoint2 in_mouse, ViewExp &in_viewport, INode*& out_node, int &out_vertex) {
	...
	for (HitRecord *hr = in_viewport.GetSubObjHitList().First(); hr != 0; hr = hr->Next()) {
		if (HasPrimaryNode() && hr->modContext->localData != GetMeshData()) {
			continue;
		}
...
EditMeshData* CreateFaceMouseProc::GetMeshData() {
	INodeTab l_nodeTab;
	ModContextList l_modContextList;
	GetInterface()->GetModContexts (l_modContextList, l_nodeTab);

	if (!HasPrimaryNode()) {
		return static_cast<EditMeshData*>(l_modContextList[0]->localData);
	} else {
		DbgAssert(l_nodeTab.Count() == l_modContextList.Count());
		for (int i = 0; i < l_nodeTab.Count(); i++) {
			if (l_nodeTab[i] == GetPrimaryNode()) {
				return static_cast<EditMeshData*>(l_modContextList[i]->localData);
			}
		}
	}
	DbgAssert(false);
...
bool CreateFaceMouseProc::CreateVertex(const Point3& in_worldLocation, int& out_vertex) {
	INode* l_node;
	if (HasPrimaryNode()) l_node = GetPrimaryNode();
	else {
		INodeTab l_nodeTab;
		ModContextList l_modContextList;
		GetInterface()->GetModContexts (l_modContextList, l_nodeTab);
	...
	out_vertex = m_EMesh->CreateVertex(in_worldLocation, GetMeshData(), l_node);
	return out_vertex >= 0;
}

bool CreateFaceMouseProc::CreateFace(MaxSDK::Array<int>& in_vertices) {
	return m_EMesh->CreateFace(GetMeshData(), in_vertices.asArrayPtr(), (int)in_vertices.length());
}

CreateFaceMouseProc::CreateFaceMouseProc(EditMeshMod* in_e, IObjParam* in_i) 
: CreateFaceMouseProcTemplate(in_i), m_EMesh(in_e) {
	if (!hCurCreateVerts) hCurCreateVerts = UI::MouseCursors::LoadMouseCursor(UI::MouseCursors::CreateVertex);
}
...
void MAttachHitByName::proc(INodeTab &nodeTab) {
	...
	if (nodes[0]->GetMtl()) {
		for (int i=0; i<nodeTab.Count(); i++) {
			if (nodeTab[i]->GetMtl() && (nodes[0]->GetMtl()!=nodeTab[i]->GetMtl())) 
         {
            ret = DoAttachMatOptionDialog (em->ip, em);
            break;
         }
		}
	}
