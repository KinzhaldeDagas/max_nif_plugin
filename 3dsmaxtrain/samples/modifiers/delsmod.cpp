...
class SDeleteMod : public Modifier {	
	public:		
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_AP_SDELETEMOD) : _T("DeleteSpline");}  
		virtual Class_ID ClassID() { return Class_ID(SDELETE_CLASS_ID,0);}		
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_AP_SDELETEMOD) : _T("DeleteSpline");}
...
class SDeleteClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("DeleteSpline"); }
