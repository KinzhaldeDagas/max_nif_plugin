...
#define AFFECT_BEHIND_CAM_CHUNK  1060
#define ANIMATED_CAMERA_SUPPORT_CHUNK  1070
#define INVCAMERA_TM_CHUNK 1080
#define OBJECT_TM_CHUNK 1090
...
class CamMapObj : public WSMObject {
  public:
   ...
   void GetClassName(MSTR& s, bool localized) const override { s= localized ?GetString(IDS_CAMMAPOBJECT) : _T("Camera Mapping Object"); }
   ...
   const TCHAR *GetObjectName(bool localized) const override { return localized ? OBJ_OBJECT_NAME : _T("CameraMap"); }
...
class CameraMapModData : public LocalModData
{
public:
	INode* mSelfNode = nullptr;
	Matrix3 mObjectTm;
	bool mReset = false;
	...
	LocalModData* Clone()
	{
		CameraMapModData* d = new CameraMapModData();
		d->mSelfNode = NULL;
		d->mObjectTm = mObjectTm;
		return d;
	}
};
...
class CamMapWSMod : public ICamMapWSMod {
  public:
   ...
   // Affect points behind the camera
   BOOL affectBehindCam;
   ...
   // --- Methods From Animatable ---
   void DeleteThis() { delete this; }
   void GetClassName(MSTR& s, bool localized) const override { s= localized ?GetString(IDS_CAMERAMAPPINGMODIFIER) : _T("Camera Mapping Modifier"); }
...
public:
   ...
   // --- Methods From BaseObject ---
   const TCHAR *GetObjectName(bool localized) const override { return localized ? WSMOD_OBJECT_NAME : _T("Camera Map Binding"); }
   CreateMouseCallBack* GetCreateMouseCallBack() { return NULL; } 

   // --- Methods From Modifier ---
   ChannelMask ChannelsUsed()
      { return GEOM_CHANNEL|TOPO_CHANNEL|SELECT_CHANNEL|SUBSEL_TYPE_CHANNEL|TEXMAP_CHANNEL|VERTCOLOR_CHANNEL; } // mjm - 3.10.99
   ChannelMask ChannelsChanged()
      { return VERTCOLOR_CHANNEL|TEXMAP_CHANNEL|GEOM_CHANNEL; } // mjm - 3.10.99
   Class_ID InputType() 
   ...
   int		getAffectBehindCam();
   void  setAffectBehindCam(BOOL b);
   ...
   { 
	...
	// return the GetInterface() of its super class
	  return WSModifier::GetInterface(id);
   } 
   //******************************
   BOOL useNewDepthMethod;
   BOOL mAnimatedCameraSupport = TRUE;
};
...
class CamMapObjClassDesc : public ClassDesc {
  public:
   ...
   const TCHAR*   NonLocalizedClassName() { return _T("CameraMap"); }
...
class CamMapWSModClassDesc : public ClassDesc {
  public:
   ...
   const TCHAR*   NonLocalizedClassName() { return _T("Camera Map"); }
...
static FPInterfaceDesc CamMapWSMod_interface(
    CAMMAP_WSM_INTERFACE, _T("interface"), 0, &camMapWSModDesc, FP_MIXIN,
      ...
      CamMapWSMod::get_affect_behind_cam, CamMapWSMod::set_affect_behind_cam, _T("affectBehindCam"), 0, TYPE_BOOL,
      p_end
      );
...
static INT_PTR CALLBACK CamMapWSModDlgProc(HWND hWnd, UINT msg, 
   ...
   CamMapWSMod *cm = DLGetWindowLongPtr<CamMapWSMod *>(hWnd);
...
RefResult CamMapWSMod::NotifyRefChanged(const Interval& changeInt, 
...
IOResult CamMapWSMod::Save(ISave* isave)
{
   ...
   isave->BeginChunk(AFFECT_BEHIND_CAM_CHUNK);
   isave->Write(&affectBehindCam, sizeof(affectBehindCam), &nb);
   isave->EndChunk();

   if (mAnimatedCameraSupport)
   {
       isave->BeginChunk(ANIMATED_CAMERA_SUPPORT_CHUNK);
       isave->EndChunk();
...
IOResult CamMapWSMod::Load(ILoad* iload) {
   ...
   affectBehindCam = FALSE;
   mAnimatedCameraSupport = FALSE;
   ...
   while ( IO_OK == (res = iload->OpenChunk()) )
   {
      switch ( iload->CurChunkID() )
      {
         ...
         case NEWDEPTH_CHUNK: // watje
            ...
            break;
         case AFFECT_BEHIND_CAM_CHUNK:
            res=iload->Read(&affectBehindCam, sizeof(affectBehindCam), &nb);
            break;
         case ANIMATED_CAMERA_SUPPORT_CHUNK:
             mAnimatedCameraSupport = TRUE;
             break;
...
RefTargetHandle CamMapWSMod::Clone(RemapDir& remap) {
   ...
   newMod->editMod = NULL;
   newMod->affectBehindCam = affectBehindCam;
   newMod->channel = channel;
   newMod->mAnimatedCameraSupport = mAnimatedCameraSupport;
...
// mjm - begin - 3.10.99
   ...
   BitArray affectVerts( nVerts );

   float distance, x, y, z;
   for (int i=0; i<nVerts; i++) {
      x = UVW[i].x; y = UVW[i].y; z = UVW[i].z;
	  // Vert is affected if in front of the camera,
	  // or if the Affect Behind Option is enabled.
      affectVerts.Set( i, affectBehindCam || (z<=0) );
...
Interval CamMapWSMod::LocalValidity(TimeValue t) {
   if (camRef && mAnimatedCameraSupport) {
      Interval valid = FOREVER;
   ...
   if (node) {
      ...
      if (os.obj->SuperClassID() == CAMERA_CLASS_ID) {
         if (cm->SetCameraNode(node)) {
            ...
            MaxSDK::MaxMessageBox(ip->GetMAXHWnd(), buf, MESSAGE_TITLE, MB_OK | MB_ICONEXCLAMATION);
         }
      }
   }
   return TRUE;
...
int CamMapWSMod::getAffectBehindCam() {
   return affectBehindCam;
}

void CamMapWSMod::setAffectBehindCam(BOOL b) {
	affectBehindCam = b;
}
...
// mjm - end
   ...
   IOResult SaveLocalData(ISave* isave, LocalModData* pld) override;
   ...
   // --- Methods From BaseObject ---
   const TCHAR *GetObjectName(bool localized) const override { return localized ? MOD_OBJECT_NAME : _T("Camera Map Modifier"); }
   CreateMouseCallBack* GetCreateMouseCallBack() { return NULL; } 
   ...
   ChannelMask ChannelsUsed()
      { return GEOM_CHANNEL|TOPO_CHANNEL|SELECT_CHANNEL|SUBSEL_TYPE_CHANNEL|TEXMAP_CHANNEL|VERTCOLOR_CHANNEL; } // mjm - 3.10.99
   ChannelMask ChannelsChanged()
      { return VERTCOLOR_CHANNEL|TEXMAP_CHANNEL; } // mjm - 3.10.99
   Class_ID InputType() 
      { return Class_ID(TRIOBJ_CLASS_ID, 0); }
   void ModifyObject(TimeValue t, ModContext &mc, 
      ObjectState *os, INode *node);
...
static FPInterfaceDesc CamMapMod_interface(
    CAMMAP_MOD_INTERFACE, _T("interface"), 0, &camMapModDesc, FP_MIXIN,
      ...
      CamMapMod::get_affect_behind_cam, CamMapMod::set_affect_behind_cam, _T("affectBehindCam"), 0, TYPE_BOOL,
      p_end
      );
...
static INT_PTR CALLBACK CamMapModDlgProc(HWND hWnd, UINT msg, 
   ...
   CamMapMod *cm = DLGetWindowLongPtr<CamMapMod *>(hWnd);
   if (!cm && msg != WM_INITDIALOG ) return FALSE;
...
// mjm - begin - 3.10.99
         ...
         cm->iMapID->Enable(cm->channel);
         CheckRadioButton(hWnd, IDC_MAP_CHAN_1, IDC_MAP_CHAN_0, (cm->channel) ? IDC_MAP_CHAN_1 : IDC_MAP_CHAN_0);
		 CheckDlgButton(hWnd, IDC_ANIMATED_CHECK, cm->mAnimatedCameraSupport);
...
// mjm - end
      ...
      case WM_COMMAND:
         ...
         {
// mjm - begin - 3.10.99
    		 case IDC_ANIMATED_CHECK:
	    		 cm->mAnimatedCameraSupport = IsDlgButtonChecked(hWnd, IDC_ANIMATED_CHECK);
				 if (cm->mAnimatedCameraSupport)
				 {
					 ModContextList mcList;
					 INodeTab nodeTab;
					 GetCOREInterface()->GetModContexts(mcList, nodeTab);
					 for (size_t index = 0; index < mcList.size(); ++index)
					 {
						 if (mcList[index]->localData)
						 {
							 CameraMapModData* localData = static_cast<CameraMapModData*>(mcList[index]->localData);
							 localData->mReset = true;
						 }
					 }
...
IOResult CamMapMod::Save(ISave* isave) {
   ...
   isave->Write(&mat, sizeof(Matrix3), &nb);
   isave->EndChunk();
   ...
   isave->BeginChunk(INVCAMERA_TM_CHUNK);   
   isave->Write(&mCameraInvTm, sizeof(Matrix3), &nb);
   isave->EndChunk();
   ...
   while (IO_OK == (res = iload->OpenChunk())) {
      switch(iload->CurChunkID()) {
         case HAVE_CAMERA_DATA_CHUNK:
            ...
            break;
         case MATRIX3_CHUNK:
            res=iload->Read(&mat, sizeof(Matrix3), &nb);
            break;
         case FOV_CHUNK:
         ...
         case ANIMATED_CAMERA_SUPPORT_CHUNK:
             mAnimatedCameraSupport = TRUE;
             break;
		 case INVCAMERA_TM_CHUNK:
			 res = iload->Read(&mCameraInvTm, sizeof(Matrix3), &nb);
			 break;
		 case VERSION_CHUNK:
...
/*
class CameraMapEnumProc : public DependentEnumProc
{
public:
...
int CameraMapEnumProc::proc(ReferenceMaker* rmaker)
{
...
BOOL RecursePipeAndMatch(LocalModData* smd, Object* obj)
{
	SClass_ID sc;
	IDerivedObject* dobj;
...
INode* GetNodeFromModContext(CamMapMod* mod, LocalModData* smd, int& which)
{

	int i;

	CameraMapEnumProc dep;
...
void CamMapMod::ModifyObject(TimeValue t, ModContext& mc, ObjectState* os, INode* node)
{
    ...
    //create local data since we need to store a local tm per instance
	if (mc.localData == nullptr)
	{
		mc.localData = new CameraMapModData();
	}
	CameraMapModData* localData = static_cast<CameraMapModData*>(mc.localData);
    //if we dont have the node that owns this modifier find it
	if (localData->mSelfNode == nullptr) 
	{		        
        int which = 0;
		localData->mSelfNode = GetNodeFromModData(localData, which);
	}
    ...
    INode* cam = getCamNode(t);
	// Get the matrix to transform the objects points to world space
	if (mAnimatedCameraSupport  || localData->mReset)
	{
		localData->mObjectTm = localData->mSelfNode->GetObjectTM(t);
		mCameraInvTm = Inverse(cam->GetObjectTM(t));
		// we can bump the version since we now store the split tm, previous version only stored mat so
        // so we could not support instance in that case
		mVersion = 202403; 
		localData->mReset = false;
	}

    if (mVersion >= 202403)
	    mat = localData->mObjectTm * mCameraInvTm;
   ...
   BOOL hasMap = mesh.mapSupport(channel);
   BOOL useSel = mesh.selLevel == MESH_FACE ? TRUE : FALSE;
   ...
   {
	   TVFace *tvFaces = mesh.mapFaces(channel);
	   Face* tFaces = mesh.faces;
	   int nMapVerts = mesh.getNumMapVerts(channel);

	   int nNewMapVerts = 0;
	   Tab<DWORD> vmap;
	   std::vector<UVVert> newUVVert;
	   vmap.SetCount(nMapVerts);
	   for (int i=0; i<vmap.Count(); i++) vmap[i] = -1;
  
		//Create the new uvs
		BitArray& faceSel = mesh.FaceSel();
		DbgAssert(faceSel.GetSize()==nFaces);
		for(int i = 0; i<nFaces; i++)
		{
			if(!faceSel[i]) continue;
			for (int j=0; j<3; j++) {
				if (affectVerts[tFaces[i].v[j]])
				{
					if (vmap[tvFaces[i].t[j]]==-1)
					{
						vmap[tvFaces[i].t[j]] = nMapVerts + nNewMapVerts++;
						newUVVert.push_back(UVW[tFaces[i].v[j]]);
					}
					tvFaces[i].t[j] = vmap[tvFaces[i].t[j]];
				}
			}
		}
		int nFinalMapVerts = nMapVerts + nNewMapVerts;
		mesh.setNumMapVerts(channel, nFinalMapVerts, true);

		UVVert *uvVerts = mesh.mapVerts(channel);
		for (int i=nMapVerts, j = 0; i<nFinalMapVerts; i++,j++) {
			uvVerts[i] = newUVVert[j];
		}
   }
...
// This method is called when one of the items we reference changes.
RefResult CamMapMod::NotifyRefChanged(const Interval& changeInt, 
   RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate) {
   switch (message) {
      case REFMSG_TARGET_DELETED:   
         if (hTarget == camRef) {
...
RefTargetHandle CamMapMod::Clone(RemapDir& remap) {
   ...
   newMod->affectBehindCam = affectBehindCam;
   newMod->channel = channel;
   newMod->aspectRatio = aspectRatio;
   newMod->mAnimatedCameraSupport = mAnimatedCameraSupport;
   BaseClone(this, newMod, remap);
   return newMod;
}
...
// mjm - begin - 2.3.99
   if (camNode->TestForLoop(FOREVER,this)==REF_SUCCEED) {
      ...
      // Create the matrix that represents the relative position of the 
      // object to the camera.
	  mCameraInvTm = Inverse(camMat);

      //now need to reset each instance of the modifier to get the local space of the modified object
	  ModContextList mcList;
	  INodeTab nodeTab;
      GetCOREInterface()->GetModContexts(mcList, nodeTab);
	  for (size_t index = 0; index < mcList.size(); ++index)
      ...
      //we picked a new camera so we can use the new version since we now store both the local to inverse transform
	  mVersion = 202403;
...
int CamMapMod::getAffectBehindCam() {
   return affectBehindCam;
}

void CamMapMod::setAffectBehindCam(BOOL b) {
	affectBehindCam = b;
}


IOResult CamMapMod::SaveLocalData(ISave* isave, LocalModData* pld)
{
    
	CameraMapModData* p;
	IOResult res;

	p = (CameraMapModData*)pld;
	isave->BeginChunk(OBJECT_TM_CHUNK);
	res = p->mObjectTm.Save(isave);
	isave->EndChunk();

	return IO_OK;
}

IOResult CamMapMod::LoadLocalData(ILoad* iload, LocalModData** pld)

{
	IOResult res;

	CameraMapModData* p = new CameraMapModData();
	Matrix3 tm;
	while (IO_OK == (res = iload->OpenChunk()))
	{
		switch (iload->CurChunkID())
		{
		case OBJECT_TM_CHUNK:
			tm.Load(iload);
			p->mObjectTm = tm;
			break;
		}
