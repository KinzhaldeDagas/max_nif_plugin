...
def_struct_primitive_debug_ok ( flexOps_getNumberVertices,  flexOps, "GetNumberVertices" );
...
def_struct_primitive_debug_ok ( flexOps_getVertexWeight,    flexOps, "GetVertexWeight" );
...
def_struct_primitive_debug_ok ( flexOps_isVertEdge,         flexOps, "isEdgeVertex" );
...
// BitArray selList = arg_list[1]->to_bitarray();
   if (objects != 0)
      ...
      int index;
      if (Number::is_number(ival))   // single index
         {
         index = ival->to_int()-1;
         if ((index <0) || (index >= lmd->wsel.Count()) )
   ...
   lmod->NotifyDependents(FOREVER, PART_GEOM, REFMSG_CHANGE);
...
//    if (vertID >= bmd->VertexData.Count() ) throw RuntimeError(GetString(IDS_PW_EXCEEDED_VERTEX_COUNT), arg_list[0]);
      ...
      if (Number::is_number(weightsval) && Number::is_number(vertsval))  // single index
         {
         ...
         for (int i = 0; i < wval->size; i++)
            ...
            if ( (Number::is_number(vertval)) && (Number::is_number(weightval)))  // single index
               {
...
//maybe should use COREinterface instead of local interface ????
   ...
   if (objects != 0)
      ...
      LagModData *lmd = (LagModData*)mcList[0]->localData;
      if (Number::is_number(vertsval))  // single index
         {
         int vertID = vertsval->to_int()-1;
         ct = lmd->esel[vertID];
         }
...
//    if (vertID >= bmd->VertexData.Count() ) throw RuntimeError(GetString(IDS_PW_EXCEEDED_VERTEX_COUNT), arg_list[0]);
      ...
      if (Number::is_number(vertsval))  // single index
         ...
         for (int i = 0; i < vval->size; i++)
            {
            Value *vertval = vval->data[i];
            if ( (Number::is_number(vertval)) )  // single index
               {
               int vertID = vertval->to_int()-1;
               if (vertID >= lmd->SpringList.Count()) throw RuntimeError(GetString(IDS_PW_ERROR_COUNT), arg_list[0]);
               lmd->esel[vertID] = 1;
...
//                      mod->addMode);  // identifier of radio button to select 
         ...
         if (mod->edgeLength>0)
            {
            maxStr.printf(_T(" %s %0.1f"),GetString(IDS_AVERAGE),mod->edgeLength);
            SendMessage(GetDlgItem(hWnd, IDC_EDGEDATA), WM_SETTEXT, 0, (LPARAM)(const TCHAR *)maxStr);

            maxStr.printf(_T(" %s %0.1f\n"),GetString(IDS_MAX),mod->maxEdgeLength);
            SendMessage(GetDlgItem(hWnd, IDC_EDGEDATA1), WM_SETTEXT, 0, (LPARAM)(const TCHAR *)maxStr);

            maxStr.printf(_T(" %s %0.1f\n"),GetString(IDS_MIN),mod->minEdgeLength);
            SendMessage(GetDlgItem(hWnd, IDC_EDGEDATA2), WM_SETTEXT, 0, (LPARAM)(const TCHAR *)maxStr);
