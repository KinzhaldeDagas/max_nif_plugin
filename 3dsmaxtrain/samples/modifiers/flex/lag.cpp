...
// compile-time validates size of EdgeBondage class to ensure its size doesn't change since instances are 
// saved as a binary blob to scene file. Changes in size will break file i/o
static_assert(sizeof(EdgeBondage) == sizeof(float) + 3 * sizeof(int), "");
...
public:
	...
	void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)    // set from v
	{
		...
		switch (id)
		{
			...
			case lag_force_node:
				DebugPrint(_T("setting forcee\n"));
				break;
			case lag_springcolors:
			case lag_customspringdisplay:
				if (p->ip)
					{
					p->NotifyDependents(FOREVER, PART_GEOM, REFMSG_CHANGE);
...
//	IDD_LAGPARAM, IDS_RB_PARAMETERS, 0, 0, NULL,
	...
	lag_paint_backface, _T("paintBackface"),	TYPE_BOOL, 		 P_RESET_DEFAULT,	IDS_LAG_STRENGTH,
		p_default, 		TRUE, 
		p_ui, 			lag_map_params,TYPE_SINGLECHECKBOX, 	IDC_PROJECT_THROUGH_CHECK, 
		p_end, 
	lag_force_node,    _T("forceNode"),  TYPE_INODE_TAB,		0,	P_AUTO_UI|P_VARIABLE_SIZE,	IDS_LAG_FORCE_NODES,
		...
		p_nonLocalizedName, _T("Force Nodes"),
		p_end,
	lag_absolute, _T("absolute"),	TYPE_BOOL, 		P_RESET_DEFAULT,	IDS_LAG_ABSOLUTE,
		p_default, 		FALSE, 
		p_ui, 			lag_map_weights,TYPE_SINGLECHECKBOX, 	IDC_ABSOLUTE, 
		p_end, 
	...
	lag_chase, _T("chase"),	TYPE_BOOL, 		P_RESET_DEFAULT,	IDS_LAG_CHASE,
		p_default, 		TRUE, 
		p_ui, 			lag_map_params,TYPE_SINGLECHECKBOX, 	IDC_CHASE, 
		p_enable_ctrls,		2,lag_strength,lag_sway,
		p_end, 

	lag_center, _T("enableCenter"),	TYPE_BOOL, 		P_RESET_DEFAULT,	IDS_LAG_ENABLECENTER,
		p_default, 		TRUE, 
		p_ui, 			lag_map_params,TYPE_SINGLECHECKBOX, 	IDC_CENTER, 
	...
	lag_endframeon, _T("enableEndFrame"),	TYPE_BOOL, 		P_RESET_DEFAULT,	IDS_LAG_ENABLEENDFRAME,
		p_default, 		FALSE, 
		p_ui, 			lag_map_advanceparams,TYPE_SINGLECHECKBOX, 	IDC_ENDFRAMEON, 
	...
	lag_collider_node,    _T("colliderNode"),  TYPE_INODE_TAB,		0,	P_AUTO_UI|P_VARIABLE_SIZE,	IDS_LAG_COLLIDER_NODES,
		...
		p_accessor,	&lag_accessor,
		p_nonLocalizedName, _T("Collider Nodes"),
		p_end,
	...
	lag_stretch_str, 	_T("stretchStrength"), 	TYPE_FLOAT, 	P_ANIMATABLE | P_RESET_DEFAULT, 	IDS_LAG_STRETCHSTRENGTH, 
		...
		p_nonLocalizedName, _T("Stretch Strength"),
		p_end, 
	...
	lag_stretch_sway, 	_T("stretchSway"), 	TYPE_FLOAT, 	P_ANIMATABLE | P_RESET_DEFAULT, 	IDS_LAG_STRETCHSWAY, 
		...
		p_nonLocalizedName, _T("Stretch Sway"),
		p_end, 
	...
	lag_torque_str, 	_T("torqueStrength"), 	TYPE_FLOAT, 	P_ANIMATABLE | P_RESET_DEFAULT, 	IDS_LAG_TORQUESTRENGTH, 
		...
		p_nonLocalizedName, _T("Torque Strength"),
		p_end, 
	...
	lag_torque_sway, 	_T("torqueSway"), 	TYPE_FLOAT, 	P_ANIMATABLE | P_RESET_DEFAULT, 	IDS_LAG_TORQUESWAY, 
		...
		p_nonLocalizedName, _T("Torque Sway"),
		p_end, 
	...
	lag_displaysprings, _T("displaySprings"),	TYPE_BOOL, 		P_RESET_DEFAULT,	IDS_LAG_DISPLAYSPRINGS,
		p_default, 		FALSE, 
		p_ui, 			lag_map_advancesprings,TYPE_SINGLECHECKBOX, 	IDC_SHOWSPRINGS, 
		p_end, 

	lag_holdlength, _T("holdLength"),	TYPE_BOOL, 		P_RESET_DEFAULT,	IDS_LAG_HOLDLENGTH,
		p_default, 		FALSE, 
		p_ui, 			lag_map_advancesprings,TYPE_SINGLECHECKBOX, 	IDC_HOLDLENGTH, 
	...
	lag_enable_advance_springs,_T("enableAdvanceSprings"),	TYPE_BOOL, P_RESET_DEFAULT,	IDS_LAG_ENABLESPRINGS,
		p_default, 		FALSE, 
		p_ui, 			lag_map_advancesprings,TYPE_SINGLECHECKBOX, 	IDC_ENABLE, 
...
class LagWeightRestore : public RestoreObj {
	public:
		...
		TSTR Description() { return TSTR(GetString(IDS_PW_CHANGE_WEIGHT)); }
	};
...
class LagSpringRestore : public RestoreObj {
	public:
		...
		TSTR Description() { return TSTR(GetString(IDS_LAG_CREATESPRING)); }
	};
...
if (hitList.Count() ==1 )
	...
	{
   for (int pct = 1; pct < hitList.Count(); pct++)
		{
		for (int i = 0; i< lmd->SpringList.Count(); i++)
			{
			if (boundingBox.Contains(lmd->SpringList[i].LocalPt))
		...
		if (lmd)
		{
			...
			case MOUSE_POINT:
				...
				else if (mouseHitList.Count() != 0)
				{
					...
					for (i = 0; i < lmd->SpringList.Count(); i++)
						FalloffList[i] = -10.0f;
					BuildFalloffList( vpt.ToPointer(), mouseHitList,lmd,obj);
					for (i=0;i<lmd->SpringList.Count();i++)
					{
			...
			case MOUSE_MOVE:
					...
					)
				{
					...
					if(HitTest(lmd, vpt.ToPointer(),&m,HITTYPE_CIRCLE,0, obj) ) {
				...
				else
				{
					if (mouseHitList.Count() > 1)
					{
						if (mouseHitList[mouseHitList.Count()-1] != m)
						{
							...
							if ( HitTest(lmd,vpt.ToPointer(),&m,HITTYPE_CIRCLE,HIT_ABORTONHIT,obj) ) 
							{
	...
	switch (msg) {
		case WM_INITDIALOG: {
			...
			int iret = SendMessage(GetDlgItem(hWnd,IDC_QUALITY_COMBO),
				LB_ADDSTRING,0,(LPARAM)(const TCHAR*)blank.data());
		...
		case WM_DESTROY:
			{
				if (mod->iPaintButton)
				{
					...
					mod->iPaintButton = nullptr;
				}
				if (mod->iVWeight)
				{
					ReleaseISpinner(mod->iVWeight);
					mod->iVWeight = NULL;
				}
				break;
			}
...
INT_PTR AdvanceDlgProc::DlgProc(
		TimeValue t,IParamMap2 *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
	{
	switch (msg) {
		case WM_INITDIALOG: {
...
INT_PTR AdvanceSpringDlgProc::DlgProc(
		TimeValue t,IParamMap2 *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
	{
	switch (msg) {
		case WM_INITDIALOG: {
...
INT_PTR SimpleSoftDlgProc::DlgProc(
		TimeValue t,IParamMap2 *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
	{
	switch (msg) {
		case WM_INITDIALOG: {
...
//7-1-99
	...
	if (rmaker->ClassID()==XREFOBJ_CLASS_ID)    
		{
		xRefFound = TRUE;
		}
...
static void RenderFrameStart(void *param, NotifyInfo *info) {
	...
	TimeValue* t = GetNotifyParam<NOTIFY_RENDER_PREEVAL>(info);

	Modifier *m = (Modifier *) mod;
	if (t && m->IsEnabled() && m->IsEnabledInRender())
		mod->CheckCache(*t);

}

// de-inlined from ilag.h:
LocalModData * LagModData::Clone()
{
...
// mjm - begin - 5.10.99
	if (container && notify)
		...
		ITrackViewNode *tvroot = global->GetNode(LAGCONTAINERMAIN_TVNODE_CLASS_ID);
		if (tvroot) 
			{
...
RefTargetHandle LagMod::Clone(RemapDir& remap)
   ...
   mod->ReplaceReference(POINT1_REF,remap.CloneRef(p1));
...
//	DebugPrint(_T("time t %d start frame %d\n"),t,StartFrame);
...
TSTR LagMod::SubAnimName(int i, bool localized)
{
	if (i == POINT1_REF)
		return localized ? GetString(IDS_RB_LAGPOINTS) : _T("Center");
	return _T("");
}
...
// compile-time validates size of TempStorageClass class to ensure its size doesn't change since instances are 
// saved as a binary blob to scene file. Changes in size will break file i/o
static_assert(sizeof(TempStorageClass) == sizeof(Point3) + sizeof(float) + sizeof(BYTE) + sizeof(BOOL) + 3, "");
...
//read in data
				...
				for (int i=0;i<ct;i++)
					{
					iload->ReadVoid(&tsc, sizeof(tsc), &nb);
...
Point3 LagDeformer::Map(int i, Point3 p)
	...
	if (lmd)
		...
		if (i < lmd->SpringList.Count())
			...
			bool isPoint = true;
			Point3 norm(0,0,0);
			if (p != lmd->pointCache[i])
			{
				isPoint = false;
				norm = p - lmd->pointCache[i];
				p = lmd->pointCache[i];
			}
...
//				p = (lmd->SpringList[i].pos * lmd->InverseTM);
			...
			if (isPoint == false)  //best guess for a normal since we cannot solve for a normal using our system
				p = p+norm;
			}
...
void LagMod::AddForceField()
{
	...
	for (int i = 0; i < ct; i++)
	{
       ...
       if (obref != NULL)
       {
          ...
          if (wsmObj != NULL) {
             ff[i] = wsmObj->GetForceField(node);
          }
       }
		}
		else ff[i]=NULL;
	...
	for (int i = 0; i < ct; i++)
	{
		...
		if (node != NULL)
		{
			...
			if (obref != NULL)
			{
				...
				if (wsmObj != NULL) {
					colliderObjects[i] = wsmObj->GetCollisionObject(node);
				}
			}
		} // if node
	} // for
...
class LagEnableAbove : public GeomPipelineEnumProc
	{
public:  
	LagEnableAbove(LagMod *me, bool enable): mLag(me),mEnable(enable),mFound(false),mCt(0) {}
    PipeEnumResult proc(ReferenceTarget *object, IDerivedObject *derObj, int index);
    bool mFound;
    bool mEnable;
	int mCt;
    LagMod  *mLag;  //what we are looking for on the stack
   
protected:
    LagEnableAbove(); // disallowed
    LagEnableAbove(LagEnableAbove& rhs); // disallowed
    LagEnableAbove& operator=(const LagEnableAbove& rhs); // disallowed
};

PipeEnumResult LagEnableAbove::proc(
   ReferenceTarget *object, 
   IDerivedObject *derObj, 
   int index)
{
	if ((derObj != NULL) && (object) ) 
	{
		Modifier *m = derObj->GetModifier(index); //get the one on the stack, may be locked
		if (object == mLag)
			mFound= TRUE;

		BOOL en = m->IsEnabled();
		BOOL env = m->IsEnabledInViews();
		//				DebugPrint(_T("%s enable state %d view %d\n"),name,en,env);
		if (!mEnable)
		{
			if (!mFound)
			{
				mLag->enableStates.Append(1,&en,1);
				mLag->enableViewStates.Append(1,&env,1);
				m->DisableMod();
				m->DisableModInViews();
			}
		}
		else 
		{
			if (!mFound)
			{
				if (!mLag->enableStates[mCt])
					m->DisableMod();
				else m->EnableMod();
				if (!mLag->enableViewStates[mCt++])
					m->DisableModInViews();
...
// then osm stack
	...
	if ((sc = obj->SuperClassID()) == GEN_DERIVOB_CLASS_ID)
	{
		...
		LagEnableAbove pipeEnumProc(this,enable ? true:false);
		EnumGeomPipeline(&pipeEnumProc, obj);

		ct = pipeEnumProc.mCt; //get this count back out
	}
...

{
   ...
   for (int k=0;k<colliderObjects.Count();k++)
		{
...
//if (flag !=0) DebugPrint(_T("got here\n"));
			Point3 l = p1-p2;
			...
			if (len < 0.0001f)
				{
//				DebugPrint(_T("Illegal Spring Length %d\n"),j);
				}
			else 
				{
...
class LagModGeomPipelineEnumProc: public  GeomPipelineEnumProc
{
public:	
	LagModGeomPipelineEnumProc(ModContext& md) : mFound(false), mModCtx(md) { }
	PipeEnumResult proc(ReferenceTarget *object, IDerivedObject *derObj, int index);
  bool mFound;
	ModContext& mModCtx;

protected:
	LagModGeomPipelineEnumProc(); // disallowed
	LagModGeomPipelineEnumProc(LagModGeomPipelineEnumProc& rhs); // disallowed
	LagModGeomPipelineEnumProc& operator=(const LagModGeomPipelineEnumProc& rhs); // disallowed
};

PipeEnumResult LagModGeomPipelineEnumProc::proc(
	ReferenceTarget *object, 
	IDerivedObject *derObj, 
	int index)
{
	ModContext* curModCtx = NULL;
	if ((derObj != NULL) && (curModCtx = derObj->GetModContext(index)) == &mModCtx) 
	{
		DbgAssert(object != NULL);
		DbgAssert(object->ClassID() == Class_ID(LAZYID));
		mFound = true;
...
INode* LagMod::GetNodeFromModContext(ModContext *smd, int &which)
   ...
   for ( int i = 0; i < dep.Nodes.Count(); i++)
		...
		if (node)
			{
			Object* obj = node->GetObjectRef();
			LagModGeomPipelineEnumProc pipeEnumProc(*smd);
			EnumGeomPipeline(&pipeEnumProc, obj);
...
//to prevent an infintie loop of lag and scatter trying to update themselves 
	...
	int nv = os->obj->NumPoints();
	// CA - 3/30/04 - If no points to deform, return
	if (nv == 0)
		return;
...
//add a new inode to trackview
		...
		if (!tvroot) 
			...
			global->AddNode(mcontainer, GetString(IDS_PW_FLEXDATA), LAGCONTAINERMAIN_TVNODE_CLASS_ID);
			tvroot = mcontainer;
			}
...
//load up old pre beta files
      ...
      // 682283 watje sometimes on old file this is bogus pointer
      if (d->SelfNode && (d->SelfNode->SuperClassID()!=BASENODE_CLASS_ID))
      {
         d->SelfNode = NULL;
         container = NULL;
      }
...
//this is here to handle a merge you will have a lmd nad self node but no container which must be recreated
	else if ((container == NULL) && (lmd) && (lmd->SelfNode) && (!mLoadingLag) )
		{
//add a new inode to trackview
		Interface *tip = ip;
...
//add a new a container
	...
	else 	if ((container == NULL) && (!mLoadingLag))
		{
		return;
		}
		...
		if ( (lmd->edgeSprings.Count() > 0) )
            ...
            for (int j=0; j<nv;j++)
				...
				if (largest > 0)
					...
					maxStr.printf(_T("%s %0.3f"),GetString(IDS_SUGGESTED),f);
					}
				if (ip)
					{
					SendMessage(GetDlgItem(hAdvanceSprings, IDC_SUGEST), WM_SETTEXT, 0, (LPARAM) (const TCHAR*) maxStr);
					maxStr.printf(_T("%s %d(%d)"),GetString(IDS_SPRINGCOUNT),lmd->edgeSprings.Count(),largest);
					SendMessage(GetDlgItem(hAdvanceSprings, IDC_INFO), WM_SETTEXT, 0, (LPARAM) (const TCHAR*) maxStr);
					}
				suggested = f;
...
for (int i =0;i<ct;i++)
		...
		isave->WriteVoid(&tsc, sizeof(tsc), &nb); // no TCHAR data
		}
...
if (ct>0)
	...
	isave->WriteVoid(p->edgeSprings.Addr(0),sizeof(EdgeBondage)*ct,&nb); // no TCHAR data
	isave->EndChunk();
}
	...
	while (IO_OK==(res=iload->OpenChunk())) {
		switch(iload->CurChunkID())  {
			...
			case EDGESPRING_DATA_CHUNK:
				ct = p->edgeSprings.Count();
				iload->ReadVoid(p->edgeSprings.Addr(0),sizeof(EdgeBondage)*ct, &nb);
				break;
			case ID_CHUNK:
				iload->Read(&id,sizeof(int), &nb);
				p->id = id;
...
static INT_PTR CALLBACK VertexInfluenceDlgProc(
   ...
   LagMod *mod = DLGetWindowLongPtr<LagMod*>(hWnd);
...
while (startList.Count() != 0)
   ...
   for (int i = 0; i < startList.Count(); i++)
		...
		if (changePer > stretchPercentage)
			{
//DebugPrint(_T("initial length %f newLength %f\n"), initialDist, vecDist);
...
for (int k=0; k < lmd->edgeSprings.Count(); k++)
	{
	Point3 pa,pb;
	int a,b;
...
for (int k = 0; k < spDepth; k++)
	...
	for (int i =0; i < lmd->SpringList.Count(); i++)
		...
		TSTR name;
		name.printf(_T("%d%% %s"),(int)(perDone*100.f),GetString(IDS_LAG_PERCANCEL));
		...
		if (subSelection)
			...
			if (iret==-32767) 
				{
				i = lmd->SpringList.Count();
            // SR NOTE64: j was unused (and undefined here)
            // j = lmd->SpringList.Count(); 
				k = spDepth+1;
				}
			}
