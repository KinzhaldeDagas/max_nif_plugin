...
#define LAGCONTAINERMAIN_TVNODE_CLASS_ID Class_ID(0xe27e0f2b, 0x74fad494)
...
{
	// Note: do not change the value of 'v' - held by pb2
	INode *node = (INode*) v.r;
	if (!node || node->TestForLoop(FOREVER,(ReferenceMaker *) mod)!=REF_SUCCEED) 
		return FALSE;
...
// JBW: direct ParamBlock access is added
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_RB_LAGMOD) : _T("Flex");}  
		virtual Class_ID ClassID() { return Class_ID(LAZYID);}		
		void BeginEditParams(IObjParam  *ip, ULONG flags,Animatable *prev);
		void EndEditParams(IObjParam *ip,ULONG flags,Animatable *next);		
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_LAGMOD) : _T("Flex");}
		BOOL AssignController(Animatable *control, int subAnim); //AF (5/03/01)

		// From modifier
		ChannelMask ChannelsUsed()  {return GEOM_CHANNEL|SELECT_CHANNEL|SUBSEL_TYPE_CHANNEL|TOPO_CHANNEL;}
...
class LagClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Flex"); }
