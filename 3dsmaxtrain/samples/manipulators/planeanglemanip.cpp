...
class PlaneAngleClassDesc: public ClassDesc2 
{
	...
	const TCHAR*    NonLocalizedClassName() { return _T("Plane Angle"); }
...
PlaneAngleManipulator::UpdateShapes(TimeValue t, TSTR& toolTip)
{
    ...
    if (mhTarget) {
        ...
        pTargetManip->GetPBlock()->GetValue(kPlaneAngleAngle, t, angle);
        pTargetManip->GetPBlock()->GetValue(kPlaneAngleSize, t, size);
        pTargetManip->GetPBlock()->GetValue(kPlaneAngleDistance, t, distance);
        pTargetManip->GetPBlock()->GetValue(kPlaneAngleOrigin, t, origin);
        pTargetManip->GetPBlock()->GetValue(kPlaneAngleUpVec, t, upVec);
        pTargetManip->GetPBlock()->GetValue(kPlaneAngleNormalVec, t, normalVec);
        ...
        TSTR nodeName = mpINode->GetName();
        
        toolTip.printf(_T("%s [%s: %5.2f]"), nodeName.data(), GetString(IDS_ANGLE),
...
int PlaneAngleManipCreateCallBack::proc(ViewExp *pVpt, int msg, int point, int flags,
    ...
    if ( ! pVpt || ! pVpt->IsAlive() )
		{
			// why are we here?
			DbgAssert(!_T("Doing proc() on invalid view port!"));
			return FALSE;
		}
...
PlaneAngleManipulator::Clone(RemapDir& remap)
{
    ...
    pNewManip->ReplaceReference(0, remap.CloneRef(mpPblock));
...
PlaneAngleManipulator::OnMouseMove(TimeValue t, ViewExp* pVpt, IPoint2& m, DWORD flags, ManipHitData* pHitData)
{
    if ( ! pVpt || ! pVpt->IsAlive() )
		{
			// why are we here?
			DbgAssert(!_T("Invalid view port!"));
			return;
		}
...
class IKSwivelManipClassDesc: public ClassDesc2 
{
	...
	const TCHAR*    NonLocalizedClassName() { return _T("IK Swivel Manip"); }
	SClass_ID       SuperClassID() { return HELPER_CLASS_ID; }
...
Class_ID IKSwivelManipulator::ClassID()
{
	return IKSwivelManip_CLASS_ID;
}
void
...
IKSwivelManipulator::UpdateShapes(TimeValue t, TSTR& toolTip)
{
	if (mhTarget)
    {
        ...
        BOOL enabled;
        pParamBlock->GetValue(IIKChainControl::kVHDisplay, t, enabled);
        ...
        float size;
        pParamBlock->GetValue(IIKChainControl::kVHSize, t, size);
        float length;
        pParamBlock->GetValue(IIKChainControl::kVHLength, t, length);
        ...
        float swivel = pIKSys->SwivelAngle(t);
        ...
        toolTip.printf(_T("%s [%s: %5.2f]"), GetString(IDS_SWIVEL_ANGLE), nodeName.data(),
...
class IKStartSpTwistManipClassDesc: public ClassDesc2 
{
	...
	const TCHAR*    NonLocalizedClassName() { return _T("IK Spline Start Twist Manip"); }
	SClass_ID       SuperClassID() { return HELPER_CLASS_ID; }
...
Class_ID IKStartSpTwistManipulator::ClassID()
{
	return IKStartSpTwistManip_CLASS_ID;
}
...
IKStartSpTwistManipulator::UpdateShapes(TimeValue t, TSTR& toolTip)
{
    ...
    ;	//--------------------------------------------------------------
	    ...
	    IKSolver* ikSolver = pIKSys->Solver();
        BOOL enabled;
        pParamBlock->GetValue(IIKChainControl::kTwistHStartDisplay, t, enabled);
        ...
        float size;
        pParamBlock->GetValue(IIKChainControl::kTwistHStartSize, t, size);
        float length;
        pParamBlock->GetValue(IIKChainControl::kTwistHStartLength, t, length);
    ...
    //	Matrix3 parentTM = inGoal ? mpINode->GetParentTM(t)
        ...
        float startTwist = pIKSys->TwistHStartAngle(t);
	    // Given an end effector axis, there corresponds to a zero plane normal
	    // that forms the reference for the swivel angle. This zero vector
	    // is produced by the zero-plane-map. In general, the solver can have
	    // its own map. If it doesn't, we use the default zero-plane-map, which
        ...
        toolTip.printf(_T("%s [%s: %5.2f]"), GetString(IDS_AG_TWISTST_ANGLE), nodeName.data(),
...
class IKEndSpTwistManipClassDesc: public ClassDesc2 
{
	...
	const TCHAR*    NonLocalizedClassName() { return _T("IK Spline End Twist Manip"); }
	SClass_ID       SuperClassID() { return HELPER_CLASS_ID; }
...
Class_ID IKEndSpTwistManipulator::ClassID()
{
	return IKEndSpTwistManip_CLASS_ID;
}
...
IKEndSpTwistManipulator::UpdateShapes(TimeValue t, TSTR& toolTip)
{
	if (mhTarget)
	{
	    ...
	    IKSolver* ikSolver = pIKSys->Solver();
        BOOL enabled;
        pParamBlock->GetValue(IIKChainControl::kTwistHEndDisplay, t, enabled);
	    // If the ik solver it uses does not use the twist angle, we don't
	    // proceed.
        if (ikSolver == NULL ||
		    pStartJoint == NULL ||
        ...
        float size;
        pParamBlock->GetValue(IIKChainControl::kTwistHEndSize, t, size);
        float length;
        pParamBlock->GetValue(IIKChainControl::kTwistHEndLength, t, length);
    ...
    //	Matrix3 parentTM = inGoal ? mpINode->GetParentTM(t)
        ...
        float endTwist = pIKSys->TwistHEndAngle(t);
	    // Given an end effector axis, there corresponds to a zero plane normal
	    // that forms the reference for the swivel angle. This zero vector
	    // is produced by the zero-plane-map. In general, the solver can have
	    // its own map. If it doesn't, we use the default zero-plane-map, which
        ...
        toolTip.printf(_T("%s [%s: %5.2f]"), GetString(IDS_AG_TWISTEND_ANGLE), nodeName.data(),
