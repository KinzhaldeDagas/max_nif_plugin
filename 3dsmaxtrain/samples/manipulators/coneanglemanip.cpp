...
class ConeAngleClassDesc: public ClassDesc2 
{
	...
	const TCHAR*    NonLocalizedClassName() { return _T("Cone Angle"); }
	...
	public:
		INT_PTR DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
		{
			switch (msg) 
			{
				...
				{
					int val = 0;
					map->GetParamBlock()->GetValue(kConeAngleUseSquare, t, val);
...
( kConeAngleParams,
  ...
  p_default,      FALSE, 
  p_ui,           TYPE_SINGLECHECKBOX, 	IDC_USE_SQUARE, 
...
void ConeAngleManipulator::SetValues(const Point3& origin, const Point3& direction,
...
ConeAngleManipulator::UpdateShapes(TimeValue t, TSTR& toolTip)
{
    ...
    if (mhTarget) {
        ...
        BOOL useSquare;
        pTargetManip->GetPBlock()->GetValue(kConeAngleAngle,     t, angle);
        pTargetManip->GetPBlock()->GetValue(kConeAngleDistance,  t, distance);
        pTargetManip->GetPBlock()->GetValue(kConeAngleDirection, t, direction);
        pTargetManip->GetPBlock()->GetValue(kConeAngleUseSquare, t, useSquare);
        pTargetManip->GetPBlock()->GetValue(kConeAngleAspect, t, aspect);
...
ConeAngleManipulator::OnButtonDown(TimeValue t, ViewExp* pVpt, IPoint2& m, DWORD flags, ManipHitData* pHitData)
{
	...
	{
		// why are we here?
		DbgAssert(!"Doing OnMouseMove() on invalid view port!");
		return;
	}
	origOffset = 0.0f;
...
ConeAngleManipulator::OnMouseMove(TimeValue t, ViewExp* pVpt, IPoint2& m, DWORD flags, ManipHitData* pHitData)
{
    if ( ! pVpt || ! pVpt->IsAlive() )
        ...
        mpPblock->GetValue(kConeAngleAngle,     t, curAngle);
...
class HotspotManipClassDesc: public ClassDesc2 
{
	...
	const TCHAR*    NonLocalizedClassName() { return _T("Hotspot Manip"); }
	SClass_ID       SuperClassID() { return HELPER_CLASS_ID; }
...
SpotLightHotSpotManipulator::UpdateShapes(TimeValue t, TSTR& toolTip)
{
	if (mhTarget)
	{
        ...
        toolTip.printf(_T("%s [%s: %5.2f]"), nodeName.data(), GetString(IDS_HOTSPOT),
...
class FalloffManipClassDesc: public ClassDesc2 
{
	...
	const TCHAR*    NonLocalizedClassName() { return _T("Falloff Manipulator"); }
	SClass_ID       SuperClassID() { return HELPER_CLASS_ID; }
...
SpotLightFalloffManipulator::UpdateShapes(TimeValue t, TSTR& toolTip)
{
	if (mhTarget)
	{
        ...
        toolTip.printf(_T("%s [%s: %5.2f]"), nodeName.data(), GetString(IDS_FALLOFF), 
