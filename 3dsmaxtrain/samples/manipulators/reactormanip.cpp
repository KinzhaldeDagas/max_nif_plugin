...
class ReactorAngleManipClassDesc: public ClassDesc2 
{
	...
	const TCHAR*    NonLocalizedClassName() { return _T("Reactor Angle Manip"); }
...
void ReactorAngleManipulator::DeleteThis()
{
	delete this;
}

Class_ID ReactorAngleManipulator::ClassID()
{
	return ReactorAngleManip_CLASS_ID;
}
...
ReactorAngleManipulator::UpdateShapes(TimeValue t, TSTR& toolTip)
{
	if (mhTarget)
	{
		...
		toolTip.printf(_T("%s %s %s: %5.2f"), nodeName.data(), pReactor->getReactionName(selected),
				GetString(IDS_INFLUENCE), (float)RadToDeg(pReactor->getInfluence(selected)));
		...
		{
			pMesh->setVert(i,
					origin - Inverse(tm).GetTrans() +
							(pMesh->getVert(i) * mat * Inverse(tm))); //*Inverse(tm)*actualNode->GetNodeTM(t) );
																	  ////(Inverse(tm))*actualNode->GetNodeTM(t)
		}
		...
		for (i = 0; i < 4; i++)
		{
			...
			p = origin - Inverse(tm).GetTrans() +
					Point3(rad * (float)sin(a), rad * (float)cos(a), distance * Length(direction)) * mat * Inverse(tm);
			wires.Append(PolyPt(p));
			shape.Append(wires);
		}
...
ReactorAngleManipulator::OnButtonDown(TimeValue t, ViewExp* pVpt, IPoint2& m, DWORD flags, ManipHitData* pHitData)
{
	...
	{
		// why are we here?
		DbgAssert(!_T("Doing OnButtonDown() on invalid view port!"));
		return;
	}
	...
	float distance;
	Point3 origin;
	mpPblock->GetValue(kConeAngleDistance,  t, distance);
	mpPblock->GetValue(kConeAngleDirection, t, direction);
	mpPblock->GetValue(kConeAngleOrigin, t, origin);
...
ReactorAngleManipulator::OnMouseMove(TimeValue t, ViewExp* pVpt, IPoint2& m, DWORD flags, ManipHitData* pHitData)
{
	...
	{
		// why are we here?
		DbgAssert(!_T("Doing OnMouseMove() on invalid view port!"));
		return;
	}
...
class PositionManipClassDesc: public ClassDesc2 
{
	...
	const TCHAR*    NonLocalizedClassName() { return _T("Position Manip"); }
	SClass_ID       SuperClassID() { return HELPER_CLASS_ID; }
...
void PositionManipulator::DeleteThis()
{
	delete this;
}

Class_ID PositionManipulator::ClassID()
{
...
PositionManipulator::UpdateShapes(TimeValue t, TSTR& toolTip)
{
    ...
    if (mhTarget) {
		...
		if (pb2)
		{
			...
			pb2->GetValue(PositionManipulator::kPositionManipPos, t, pos);
			pb2->GetValue(PositionManipulator::kPositionManipRadius, t, radius);
                                ...
                                DWORD flags, ManipHitData* pHitData)
{
	...
	GetPBlock()->GetValue(PositionManipulator::kPositionManipPos, t, mInitPos);
	if (which == 0) 
	{
		mInitPoint = m;
	} 
...
class ReactorPosValueManipClassDesc: public ClassDesc2 
{
	...
	const TCHAR*    NonLocalizedClassName() { return _T("Position Value"); }
	SClass_ID       SuperClassID() { return HELPER_CLASS_ID; }
...
Class_ID ReactorPositionValueManip::ClassID()
{
	return ReactorPosValueManip_CLASS_ID;
}
                                ...
                                DWORD flags, ManipHitData* pHitData)
{
	...
	case 0:
		...
		break;
	case 1:
		mpPblock->GetValue(ReactorPositionValueManip::kPositionManipPosValue, t, mInitPos, reactor->getSelected());
		break;
	case 2:
		mpPblock->GetValue(ReactorPositionValueManip::kPositionManipPosState, t, mInitPos, reactor->getSelected());
		break;
	case 3:
...
void ReactorPositionValueManip::UpdateShapes(TimeValue t, TSTR& toolTip)
{
	...
	}else //it's a controller
	{
		...
		actualNode = GetCOREInterface()->GetINodeByName(nodeName.data());
	}
	if (actualNode) {
