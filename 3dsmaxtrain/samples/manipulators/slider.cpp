...
class SliderClassDesc: public ClassDesc2 
{
	...
	const TCHAR*    NonLocalizedClassName() { return _T("Slider"); }
...
static ParamBlockDesc2 SliderParamBlock (
	...
	kSliderHide,	_T("hide"),   TYPE_BOOL,			0,	IDS_HIDE,
		p_default,		FALSE,
		p_ui, 			TYPE_SINGLECHECKBOX, 	IDC_HIDE, 
		p_end,
...
SliderManipulator::UpdateShapes(TimeValue t, TSTR& toolTip)
{
    ...
    if (mhTarget) {
        ...
        pTargetManip->GetPBlock()->GetValue(kSliderValue, t, value);
        pTargetManip->GetPBlock()->GetValue(kSliderMin, t, min);
        pTargetManip->GetPBlock()->GetValue(kSliderMax, t, max);
        pTargetManip->GetPBlock()->GetValue(kSliderXPos, t, xPos);
        pTargetManip->GetPBlock()->GetValue(kSliderYPos, t, yPos);
        pTargetManip->GetPBlock()->GetValue(kSliderWidth, t, width);
        pTargetManip->GetPBlock()->GetValue(kSliderHide, t, hide);
        pTargetManip->GetPBlock()->GetValue(kSliderName, t, pName);
...
SliderManipulator::GenerateShapes(TimeValue t)
{
    ...
    GraphicsWindow* pGW = GetCOREInterface()->GetActiveViewExp().getGW();
...
int SliderManipCreateCallBack::proc(ViewExp* pVpt, int msg, int point, int flags, IPoint2 m, Matrix3& mat)
{
                                ...
                                DWORD flags, ManipHitData* pHitData)
{
    ...
    if (which == 1) {
        BOOL hide;
        GetPBlock()->GetValue(kSliderHide, t, hide);
        SetHideValue(!hide);
    }
                               ...
                               DWORD flags, ManipHitData* pHitData)
{
    if ( ! pVpt || ! pVpt->IsAlive() )
        ...
        GetPBlock()->GetValue(kSliderValue, t, value);
        GetPBlock()->GetValue(kSliderMin, t, min);
        GetPBlock()->GetValue(kSliderMax, t, max);
        GetPBlock()->GetValue(kSliderXPos, t, xPos);
        GetPBlock()->GetValue(kSliderYPos, t, yPos);
        GetPBlock()->GetValue(kSliderWidth, t, width);
        GetPBlock()->GetValue(kSliderHide, t, hide);
