...
static const TCHAR* eulerTypes[] = {
    _T("XYZ"), // IDS_RB_EULERTYPE0
    _T("XZY"), // IDS_RB_EULERTYPE1
    _T("YZX"), // IDS_RB_EULERTYPE2
    _T("YXZ"), // IDS_RB_EULERTYPE3
    _T("ZXY"), // IDS_RB_EULERTYPE4
    _T("ZYX"), // IDS_RB_EULERTYPE5
    _T("XYX"), // IDS_RB_EULERTYPE6
    _T("YZY"), // IDS_RB_EULERTYPE7
    _T("ZXZ"), // IDS_RB_EULERTYPE8
};
...
static int xyzRotIDs[] = {IDS_PRS_LOCALXROTATION, IDS_PRS_LOCALYROTATION, IDS_PRS_LOCALZROTATION};
static int xyzAxisIDs[] = {IDS_RB_XAXIS, IDS_RB_YAXIS, IDS_RB_ZAXIS};

static const TCHAR* xyzRots[] = {_T("Local X Rotation"), _T("Local Y Rotation"), _T("Local Z Rotation")};
...
void LocalEulerDlg::EndingEdit(LocalEulerRotation* next, ULONG flags)
    {
    switch (cur) {
        case EDIT_X:
		cont->rotX->EndEditParams(ip, flags, next ? next->rotX : NULL);
            break;
        case EDIT_Y:
			cont->rotY->EndEditParams(ip, flags, next ? next->rotY : NULL);
            break;
        case EDIT_Z:
			cont->rotZ->EndEditParams(ip, flags, next ? next->rotZ : NULL);
            break;
        }
    cont = NULL;
    ip   = NULL;
...
void LocalEulerDlg::BeginingEdit(LocalEulerRotation *cont, IObjParam *ip,
    ...
    switch (cur) {
        case EDIT_X:
			cont->rotX->BeginEditParams(ip, BEGIN_EDIT_MOTION, prev ? prev->rotX : NULL);
            break;
        case EDIT_Y:
			cont->rotY->BeginEditParams(ip, BEGIN_EDIT_MOTION, prev ? prev->rotY : NULL);
            break;
        case EDIT_Z:
			cont->rotZ->BeginEditParams(ip, BEGIN_EDIT_MOTION, prev ? prev->rotZ : NULL);
            break;
        }   
    UpdateWindow(hWnd);
    }
...
static INT_PTR CALLBACK LocalEulerParamDialogProc(HWND hDlg, UINT message,
    ...
    {
    LocalEulerDlg *dlg = DLGetWindowLongPtr<LocalEulerDlg*>(hDlg);
...
class LocalEulerClassDesc : public ClassDesc {
	...
	const TCHAR*    NonLocalizedClassName() { return _T("Local Euler XYZ"); }
...
void LocalEulerRotation::GetClassName(MSTR& s, bool localized) const
    {       
    TSTR format = localized ? GetString(IDS_PRS_LOCALEULERNAME) : _T("Local Euler %s");
    s.printf(format, localized ? GetString(eulerIDs[order]) : eulerTypes[order]);
    }
...
TSTR LocalEulerRotation::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case EULER_X_REF: return localized ? GetString(xyzRotIDs[orderings[order][0]]) : xyzRots[orderings[order][0]];
	case EULER_Y_REF: return localized ? GetString(xyzRotIDs[orderings[order][1]]) : xyzRots[orderings[order][1]];
	case EULER_Z_REF: return localized ? GetString(xyzRotIDs[orderings[order][2]]) : xyzRots[orderings[order][2]];
	default: return _T("");
	}
}
...
DWORD LocalEulerRotation::GetDefaultInTan() 
{
   return HYBRID_LINEAR;
}
DWORD LocalEulerRotation::GetDefaultOutTan() 
{
   return HYBRID_LINEAR;
}
DWORD LocalEulerRotation::GetSubAnimCurveColor(int subNum) 
{ 
                                         ...
                                         Animatable *prev )
{
	if(GetLocked()==false)
	{
		...
		if (flags & BEGIN_EDIT_MOTION) {
			...
			if (dlg) {
				dlg->BeginingEdit(this, ip, dynamic_cast<LocalEulerRotation*>(prev));
				dlg->Init();
			} else {
...
void LocalEulerRotation::EndEditParams(IObjParam *ip, ULONG flags,Animatable *next)
{
	...
	if (next && next->ClassID()==ClassID()) {
		cont = dynamic_cast<LocalEulerRotation*>(next);
...
void LocalEulerRotation::SetOrder(int newOrder)
   {
   if (order != newOrder) {
...
BOOL LocalEulerRotation::GetIKJoints2(InitJointData2 *posData,InitJointData2 *rotData)
   ...
   Point3 rot;
   if (!jp->pfInit) {
      Control::GetValue(GetCOREInterface()->GetTime(), &qt, CTRL_ABSOLUTE);
