...
//		LinkConstValidatorClass validator;							
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? LINKCTRL_NAME : _T("Link Constraint");}
		void BeginEditParams(IObjParam *ip, ULONG flags,Animatable *prev); 
		void EndEditParams(IObjParam *ip, ULONG flags,Animatable *next); 
		int NumSubs()  {return 2;}
		Animatable* SubAnim(int i) {return (i == 0) ? tmControl : ltControl;}
		TSTR SubAnimName(int i, bool localized) override
		{
			if (i == 0)
			{
				return localized ? GetString(IDS_AG_LINKPARAMS) : _T("Link Params");	
			}
			else
			{				
				return localized ? GetString(IDS_AG_LINKTIMES) : _T("Link Times");
			}
		}
		...
		{
			return tmControl->IsKeyAtTime(t,flags) || (ltControl && ltControl->IsKeyAtTime(t,flags));
		}
