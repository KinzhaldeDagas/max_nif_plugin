...
BOOL ReactionPickMode::FilterAnimatable(Animatable* anim)
{
	...
	return anim->SuperClassID() == CTRL_FLOAT_CLASS_ID || anim->SuperClassID() == CTRL_POSITION_CLASS_ID ||
			anim->SuperClassID() == CTRL_POINT3_CLASS_ID || anim->SuperClassID() == CTRL_SCALE_CLASS_ID ||
			anim->SuperClassID() == CTRL_ROTATION_CLASS_ID || anim->SuperClassID() == BASENODE_CLASS_ID;
}

BOOL ReactionPickMode::HitTest(IObjParam* ip, HWND hWnd, ViewExp* vpt, IPoint2 m, int flags)
{
...
void ReactionDriverPickMode::NodePicked(ReferenceTarget* anim)
{
	Animatable* parent = nullptr;
	int subAnim = 0;
...
void ReplaceDriverPickMode::NodePicked(ReferenceTarget* anim)
{
	Animatable* parent = nullptr;
	int subAnim = 0;
	...
	if (parent != nullptr)
	{
		mSet->ReplaceDriver((ReferenceTarget*)parent, subAnim);
		GetCOREInterface()->RedrawViews(GetCOREInterface()->GetTime());
	}
}

bool ReplaceDriverPickMode::LoopTest(ReferenceTarget* anim)
{
	// for(int i = 0; i < mSet->DrivenCount(); i++)
	//{
	//	if (ReferenceTarget* driven = mSet->Driven(i))
	//	{
	//		if (driven == anim)
	//			return false;

	//		if(anim->TestForLoop(FOREVER, driven) != REF_SUCCEED)
	//			return false;
	//	}
	//}
	return true;
...
bool ReactionDrivenPickMode::LoopTest(ReferenceTarget* anim)
{
	// don't allow the owner or anthing below the owner.
	if (ReactionDriver* driver = mDriver->GetReactionDriver())
	{
...
bool ReactionDrivenPickMode::FilterAnimatable(Animatable* anim)
{
	if (anim)
	{
...
void ReactionDrivenPickMode::NodePicked(ReferenceTarget* anim)
{
	...
	if (parent != nullptr)
	{
		...
		if (reactor != nullptr)
		{
			if (Control* c = GetControlInterface(anim))
				reactor->Copy(c);
			...
			if (ReactionDriver* driver = mDriver->GetReactionDriver())
			{
				if (mDriverIDs.Count() <= 0)
				{
					int defaultID = -1;
					mDriver->GetReactionDriver()->AddState(nullptr, defaultID, true, GetCOREInterface()->GetTime());
					mDriverIDs.Append(1, &defaultID);
				}

				BOOL succeed = FALSE;
				if (driver->Owner()->SuperClassID() == BASENODE_CLASS_ID)
					succeed = reactor->reactTo(driver->Owner(), GetCOREInterface()->GetTime(), false);
				else
					succeed = reactor->reactTo(static_cast<ReferenceTarget*>(driver->Owner()->SubAnim(driver->SubIndex())), GetCOREInterface()->GetTime(), false);
				if (succeed)
				{
					for (int i = 0; i < mDriverIDs.Count(); i++)
						reactor->CreateReactionAndReturn(true, nullptr, GetCOREInterface()->GetTime(), mDriverIDs[i]);
				}
			}
		}
	}
...
bool AnimPopupMenu::wireable(Animatable* parent, int subnum)
{
	...
	if (anim != nullptr && (c = GetControlInterface(anim)) != nullptr)
		return true;
...
bool AnimPopupMenu::Filter(PickObjectCallback* cb, Animatable* anim, Animatable* child,
...
)
{
	...
	// In the case of driven picking, filter out invalid children like irreplaceable controllers
	auto* pickMode = dynamic_cast<ReactionDrivenPickMode*>(cb);
	...
	for (int i = 0; i < anim->NumSubs(); i++)
	{
		if (anim->SubAnim(i))
		{
			valid = Filter(cb, anim->SubAnim(i), nullptr);
			if (valid)
				return valid;
		}
	}
...
bool AnimPopupMenu::add_subanim_params(PickObjectCallback* cb, Tab<AnimEntry*>& wparams, const TCHAR* name,
		Tab<ReferenceTarget*>& parents, int level, Tab<ReferenceTarget*>& roots)
{
		...
		for (int i = 0; i < parent->NumSubs(); i++)
		{
			...
			for (int p2 = 0; p2 < parents.Count(); p2++)
			{
				if (p2 != 0)
				{
					...
					{
						if (subAnimName == parents[p2]->SubAnimName(i2, true)) // if there are no differences in the names
						{
			...
			if (match && subs.Count())
			{
				...
				{
					if (add_subanim_params(cb, wparams, nullptr, subs, 0, roots))
						subAdded = true;
					for (int z = 0; z < subAnimPairs.Count(); z++)
						delete subAnimPairs[z];
				}
				else if (subs[0]->NumSubs() > 0 || subs[0]->GetCustAttribContainer() != nullptr)
				{
	...
	// if(!cb->MultipleChoice())
	{
		...
		if (cc != nullptr)
		{
			...
			ccs.Append(1, &cc);
			if (add_subanim_params(cb, wparams, nullptr, ccs, 0, roots))
				subAdded = true;
		}
...
void AnimPopupMenu::build_params(PickObjectCallback* cb, Tab<AnimEntry*>& wparams, Tab<ReferenceTarget*> roots)
{
	...
	for (int j = 0; j < wparams.Count(); j++)
		if (wparams[j]->pname != nullptr)
			free((void*)wparams[j]->pname);
	wparams.ZeroCount();
	...
	add_subanim_params(cb, wparams, nullptr, roots, 1, roots);
}
...
void AnimPopupMenu::DoPopupMenu(PickObjectCallback* cb, Tab<ReferenceTarget*> nodes)
{
	...
	GetCursorPos(&mp);
	HWND hwnd = ip->GetActiveViewExp().GetHWnd();
