...
#define REACTORDLG_CLASS_ID Class_ID(0x30b077b9, 0x6be05856)
#define REACTIONDLG_CLASS_ID Class_ID(0x3e536a25, 0x75e17cc6)
...
#define GetIReactorInterface(cd) (IReactor*)(cd)->GetInterface(REACTOR_INTERFACE)
...
public:
	...
	enum
	{
		react_to, create_reaction, delete_reaction, get_reaction_count, select_reaction, get_selected_reaction,
		...
		get_editing_state, set_editing_state,
		get_use_curve, set_use_curve, get_curve,
		get_create_mode, set_create_mode,
	};
	// Function Map For Mixin Interface
	//*************************************************
	BEGIN_FUNCTION_MAP
		FNT_1(react_to, TYPE_BOOL, reactTo, TYPE_REFTARG);
		...
		PROP_FNS(get_create_mode, getCreateReactionMode, set_create_mode, setCreateReactionMode, TYPE_BOOL);
	...
	virtual BOOL reactTo(ReferenceTarget* anim, TimeValue t = GetCOREInterface()->GetTime(), bool createReaction = true) = 0;
	virtual BOOL CreateReaction(const TCHAR* buf = nullptr, TimeValue t = GetCOREInterface()->GetTime()) = 0;

	virtual BOOL DeleteReaction(int i = -1) = 0;

	virtual int getReactionCount() = 0;

	virtual const TCHAR* getReactionName(int i) = 0;
	...
	virtual void setCreateReactionMode(BOOL edit) = 0;
	virtual BOOL getCreateReactionMode() = 0;
