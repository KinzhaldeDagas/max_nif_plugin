...
#define get_reactor_cont() \
	...
	if (id != REACTORFLOAT_CLASS_ID && id != REACTORPOS_CLASS_ID && id != REACTORP3_CLASS_ID && \
			id != REACTORROT_CLASS_ID && id != REACTORSCALE_CLASS_ID) \
		throw RuntimeError(GetString(IDS_NOT_A_REACTOR), arg_list[0]); \
	IReactor* react = (IReactor*)cont;


Value* setReactionState_cf(Value** arg_list, int count)
{
	...
	if (sel > 0 && sel <= react->getReactionCount())
	{
		...
		{
		case REACTORFLOAT:
			if (Number::is_number(arg_list[2]))
			{
				...
				throw RuntimeError(GetString(IDS_NOT_A_VALID_STATE), arg_list[2]);
		case REACTORROT:
...
Value* getReactionState_cf(Value** arg_list, int count)
{
...
Value* setReactionValue_cf(Value** arg_list, int count)
{
	check_arg_count(getReactionState, 3, count);
	get_reactor_cont();
	int sel = arg_list[1]->to_int();
	...
	if (sel > 0 && sel <= react->getReactionCount())
	{
		...
		case FLOAT_VAR:
			if (Number::is_number(arg_list[2]))
			{
				...
				throw RuntimeError(GetString(IDS_NOT_A_VALID_VALUE), arg_list[2]);
		case QUAT_VAR:
			if (is_quat(arg_list[2]))
			{
				q = arg_list[2]->to_quat();
...
Value* getReactionValue_cf(Value** arg_list, int count)
{
	check_arg_count(getReactionState, 2, count);
	get_reactor_cont();
	int sel = arg_list[1]->to_int();
...
Value* reactTo_cf(Value** arg_list, int count)
{
	...
	else if (is_controller(arg_list[1]))
	{
		...
		throw RuntimeError(_T("Not a Controller or a Node: "), arg_list[1]);
	return NULL;
	// Compiler wants a return value. How about NULL.
	// Should never happen, given the logic.
}

Value* createReaction_cf(Value** arg_list, int count)
{
...
Value* deleteReaction_cf(Value** arg_list, int count)
{
	check_arg_count(deleteReaction, 2, count);
	get_reactor_cont();
	int sel = arg_list[1]->to_int();
	if (sel > 0 && sel <= react->getReactionCount())
		react->DeleteReaction(sel - 1);
	return &ok;
}

Value* getSelectedReaction_cf(Value** arg_list, int count)
{
...
Value* selectReaction_cf(Value** arg_list, int count)
{
	check_arg_count(selectReaction, 2, count);
	get_reactor_cont();
	int sel = arg_list[1]->to_int();
	if (sel > 0 && sel <= react->getReactionCount())
		react->setSelected(sel - 1);
	return &ok;
}

Value* setReactionInfluence_cf(Value** arg_list, int count)
{
	...
	if (sel > 0 && sel <= react->getReactionCount())
		react->setInfluence(sel - 1, inf);
	return &ok;
}

Value* setReactionStrength_cf(Value** arg_list, int count)
{
	...
	if (sel > 0 && sel <= react->getReactionCount())
		react->setStrength(sel - 1, str);
	return &ok;
}

Value* setReactionFalloff_cf(Value** arg_list, int count)
{
	check_arg_count(setReactionFalloff, 3, count);
	get_reactor_cont();
	int sel;
	if (is_integer(arg_list[1]))
		sel = arg_list[1]->to_int();
	else
		throw RuntimeError(GetString(IDS_NOT_AN_INT), arg_list[1]);
	float fall = arg_list[2]->to_float();
	if (sel > 0 && sel <= react->getReactionCount())
		react->setFalloff(sel - 1, fall);
	return &ok;
}

Value* setReactionName_cf(Value** arg_list, int count)
{
	...
	if (sel > 0 && sel <= react->getReactionCount())
		react->setReactionName(sel - 1, name);
	react->NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
	return &ok;
}

Value* getReactionCount_cf(Value** arg_list, int count)
{
...
Value* getReactionInfluence_cf(Value** arg_list, int count)
{
	check_arg_count(getReactionInfluence, 2, count);
	get_reactor_cont();
	int sel = arg_list[1]->to_int();
	if (sel > 0 && sel <= react->getReactionCount())
		return_value(Float::intern(react->getInfluence(sel - 1)));
	else
		return &false_value;
}

Value* getReactionStrength_cf(Value** arg_list, int count)
{
	check_arg_count(getReactionStrength, 2, count);
	get_reactor_cont();
	int sel = arg_list[1]->to_int();
	if (sel > 0 && sel <= react->getReactionCount())
		return_value(Float::intern(react->getStrength(sel - 1)));
	else
		return &false_value;
}

Value* getReactionFalloff_cf(Value** arg_list, int count)
{
	check_arg_count(getReactionFalloff, 2, count);
	get_reactor_cont();
	int sel = arg_list[1]->to_int();
	if (sel > 0 && sel <= react->getReactionCount())
		return_value(Float::intern(react->getFalloff(sel - 1)));
	else
		return &false_value;
}

Value* getReactionName_cf(Value** arg_list, int count)
{
	check_arg_count(getReactionName, 2, count);
	get_reactor_cont();
	int sel = arg_list[1]->to_int();
	if (sel > 0 && sel <= react->getReactionCount())
		return_value(Name::intern(react->getReactionName(sel - 1)));
	else
		return &false_value;
}
