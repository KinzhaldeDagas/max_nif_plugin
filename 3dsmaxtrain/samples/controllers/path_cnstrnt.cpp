...
class PathPosition : public   IPathPosition , public ILockedTrackImp
{
   public:
      int incFlag;
...
// for viewport manipulation up
      ...
      DWORD flags;
      Point3 curPos, basePointLocal, basePointWorld;
      Interval ivalid;
      Quat curRot;
...
//    BOOL stopReferencing;
      ...
      Matrix3 CalcRefFrame(Point3 PathNormal, Point3 av_tan, 
      ...
      void GetClassName(MSTR& s, bool localized) const override {s = localized ? PATH_CONTROL_CNAME : _T("Path Constraint");}
      ...
      TSTR SubAnimName(int i, bool localized) override { return localized ? GetString(IDS_RB_PATHPARAMS) : _T("Path Parameters"); }
...
//and GetExtentTimeStep allows the controller to work with TrackView faster
	  ...
	  // Override this function: when changing parent there is nothing to do and we want to avoid calls the SetValue.
	  virtual BOOL ChangeParents(TimeValue t,const Matrix3& oldP,const Matrix3& newP,const Matrix3& tm) {return TRUE;}
   };
...
void PathTimeChangeCallback::TimeChanged(TimeValue t){
   ...
   Control *cont = pcontroller->pblock->GetControllerByID(path_path_weight, selection); 
...
class PathClassDesc:public ClassDesc2 {
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Path Constraint"); }
   SClass_ID      SuperClassID() { return CTRL_POSITION_CLASS_ID; }
...
void PickPathNode::EnterMode(IObjParam *ip)
   {
      ...
      GetCOREInterface()->PushPrompt(_T("Select Path Target"));
   }
...
public:
   ...
   { 
      if (id == path_path_weight || id == path_path_list){
         ...
         if (p == NULL) {
           // This is possible if the PB2 is on delete path from another
           // node. Suppose, for example, we have:
           // nodeA -> pathController -> PB2 -> nodeB.
           // When nodeA is deleted first, deletion process will proceed
           // to, and stop at, nodeB. Now, PB2's owner is deleted. When it
           // comes to delete nodeB, it will notify PB2 that RefTargetDeleted.
           // This calls TabChanged. At this time, owner is null. Since the
           // owner is already deleted, we don't have to do the following.
           return;
         }
   ...
   void Get(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t, Interval &valid)
   {
      ...
      switch(id)
      {
         ...
         case path_path:
            ...
            int ct = p->pblock->Count(path_path_list);
            if ( ct > 0){
               p->pblock->GetValue(path_path_list, t, aNode, 0);
               if ( aNode != NULL){
                  v.r = aNode;
               }
               else{
         ...
         switch (msg) 
         {
            case WM_INITDIALOG:
            {
               ...
               if (p->GetnonNULLNumPathTargets() < 1) { 
                  ...
                  // there is a valid selection 
                  // automatically means there is AT LEAST one target
                  Control *cont = p->pblock->GetControllerByID(path_path_weight, selection); 
                  // until it is animated, paramblock doesn't have a controller assigned in it yet.
                  if (spin) {
               ...
               if (LOWORD(wParam) == IDC_PATH_LIST && HIWORD(wParam) == LBN_SELCHANGE)
               {
                  ...
                  // there is a valid selection 
                     ...
                     // there are more than one targets
                        ...
                        Control *cont = p->pblock->GetControllerByID(path_path_weight, selection); // until it is animated, paramblock doesn't have a controller assigned in it yet.
                        if (spin && (cont != NULL)) {
                           if (cont->IsKeyAtTime(t,KEYAT_POSITION)){
                              spin->SetKeyBrackets(TRUE);
                           } 
   ...
   void PathPosition::RedrawListbox(TimeValue t, int sel)
      {
         ...
         if (ct > 0){
            ...
            ISpinnerControl* spin = GetISpinner(GetDlgItem(hWnd, IDC_PATH_WEIGHT_SPINNER));
			DbgAssert(spin);
            if (spin && last_selection >= 0 && last_selection < ctf) {
...
static ParamBlockDesc2 path_paramblk (path_params, _T("PathParameters"),  0, &pathCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PATHPOS_PBLOCK_REF, 
   ...
   path_follow,      _T("follow"),     TYPE_BOOL,        P_RESET_DEFAULT,     IDS_JW_FOLLOW,
      p_default,     FALSE, 
      p_ui,          TYPE_SINGLECHECKBOX, IDC_PATH_FOLLOW, 
   ...
   path_path,        _T("path"),       TYPE_INODE,       0,                IDS_RB_PATH,
      p_accessor,    &path_accessor,
      p_nonLocalizedName, _T("Path Constraint"),
      p_end, 

   path_bank,        _T("bank"),       TYPE_BOOL,        P_RESET_DEFAULT,     IDS_JW_BANK,
      p_default,     FALSE, 
      p_ui,          TYPE_SINGLECHECKBOX, IDC_PATH_BANK, 
      p_enabled,     FALSE,
   ...
   path_bank_amount, _T("bankAmount"), TYPE_FLOAT,       P_ANIMATABLE|P_RESET_DEFAULT,    IDS_JW_BANKAMOUNT, 
      ...
      p_accessor,    &path_accessor,
      p_enabled,     FALSE,
      p_nonLocalizedName, _T("Bank Amount"),
      p_end, 
   ...
   path_allow_upsidedown, _T("allowUpsideDown"), TYPE_BOOL, P_RESET_DEFAULT,     IDS_JW_ALLOWUPSIDEDOWN,
      p_default,     FALSE, 
      p_ui,          TYPE_SINGLECHECKBOX, IDC_PATH_ALLOWFLIP, 
      p_enabled,     FALSE,
      p_accessor,    &path_accessor,
      p_end, 
   ...
   path_constant_vel, _T("constantVel"),  TYPE_BOOL,        P_RESET_DEFAULT,     IDS_JW_CONSTANTVEL,
      p_default,     TRUE,
      p_ui,          TYPE_SINGLECHECKBOX, IDC_PATH_CONSTVEL, 
      p_accessor,    &path_accessor,
      p_end, 
   ...
   path_axis_flip,      _T("axisFlip"),      TYPE_BOOL,        P_RESET_DEFAULT,     IDS_JW_AXISFLIP,
      p_default,     FALSE, 
      p_ui,          TYPE_SINGLECHECKBOX, IDC_PATH_FLIP, 
      p_enabled,     FALSE,
      p_accessor,    &path_accessor,
      p_end, 
...
//watje
   ...
   pblock->SetControllerByID(path_percent, 0, ctrl);
...
RefTargetHandle PathPosition::Clone(RemapDir& remap)
   {
      ...
      p->ReplaceReference(PATHPOS_PBLOCK_REF, remap.CloneRef(pblock));
      p->flags      = flags;
...
void PathPosition::Copy(Control *from)
{
   ...
   if (from->ClassID()==ClassID()) {
      ...
      ReplaceReference(PATHPOS_PBLOCK_REF, CloneRefHierarchy(ctrl->pblock));
      curPos   = ctrl->curPos;
      curRot   = ctrl->curRot;
      flags    = ctrl->flags;
      ...
      from->GetValue(GetCOREInterface()->GetTime(), &fval); // to know the object position before the
...
float PathPosition::GetPercent(TimeValue t, BOOL noClip)
   {  
   float per;
// DebugPrint(_T("in GetPercent1: start_valid = %d  end_valid = %d\n"), ivalid.Start(), ivalid.End());
   pblock->GetValue(path_percent, t, per, ivalid);
// DebugPrint(_T("in GetPercent3: start_valid = %d  end_valid = %d\n"), ivalid.Start(), ivalid.End());
...
//    DebugPrint(_T("in CalculateTangentOnPath: start_valid = %d  end_valid = %d\n"), ivalid.Start(), ivalid.End());
...
Matrix3 PathPosition::CalcRefFrame(Point3 PathNormal, Point3 av_tan, TimeValue t, ShapeObject *pathOb, INode* path)
{
      Matrix3 otm = path->GetObjTMAfterWSM(t);
      Matrix3 tm(1);
      float u = GetPercent(t);
//    DebugPrint(_T("4: start_valid = %d  end_valid = %d\n"), ivalid.Start(), ivalid.End());

	  pblock->GetValue(path_bank_amount, t, bankAmount, ivalid);
	  bankAmount = FromBankUI(bankAmount);
	  pblock->GetValue(path_smoothness, t, tracking, ivalid);
	  tracking = FromTrackUI(tracking);
...
//    DebugPrint(_T("66: start_valid = %d  end_valid = %d\n"), objstvalid.Start(), objstvalid.End());

      ivalid &= os.Validity(t);

//    DebugPrint(_T("2: start_valid = %d  end_valid = %d\n"), ivalid.Start(), ivalid.End());
      if (!pathOb) continue;
      curPos += pathWt * PointOnPath(t, pathOb, path);
...
// Compute the normal to the plane of path#1 by sampling points on the path
      ...
      if (Follow()) {
         curRot = Quat(CalcRefFrame(pathNorm, average_tangent, t, pathOb, path));
      }
      else {
         curRot.Identity();
      }
   ...
   if (total_path_weight > 0.0){
      curPos =  curPos/total_path_weight;
      ...
      if (Relative()){
         curPos = curPos + (basePointWorld - InitialPosition);
      }
   }
   else {
      curPos = basePointWorld;
//    curRot.Identity();
   }

   oldTargetNumber = ct;
// DebugPrint(_T("3: start_valid = %d  end_valid = %d\n"), ivalid.Start(), ivalid.End());
...
Point3 PathPosition::SampleSplineCurves(float percentage, TimeValue anim_time, TimeValue t)
   ...
   for (int i = 0; i < ct; i++) {
      ...
      pblock->GetValue(path_path_weight, anim_time, pathWt, i);
      total_path_weight += pathWt;
...
void PathPosition::MouseCycleStarted(TimeValue t)
{
   ...
   objectTM = dep.node->GetObjTMAfterWSM(t,&valid);
...
{
   if(GetLocked()==true||incFlag) return;
   int ct = pblock->Count(path_path_list);
   if (ct<1) return;
   if (method==CTRL_RELATIVE) {
	  ...
	  // The numerical algorithm in SplineToPoint() to compute the neatest point on Spline is not robust enough to handle the case of (0,0,0) relative movement, in which case
	  // this algorithm does incorrectly return a different position than the original one. Ideally we need a better numerical algorithm and 
	  // for now I just kick out the (0,0,0) case for a safe fix to the related defect.
	  if (inputVec.Equals(Point3(0,0,0), 0))
		  return;

      INode *path;
      ct = pblock->Count(path_path_list);
      pblock->GetValue(path_path_list, t, path, 0);
...
//need to put everything in screen space then ignore the z component
            Matrix3 worldToScreenTm;
            GetCOREInterface()->GetActiveViewExp().GetAffineTM(worldToScreenTm);
...
INode* PathPosition::GetNode(int nodeNumber){
   if (nodeNumber >= 0 && nodeNumber < pblock->Count(path_path_list)){
      INode *path;
      pblock->GetValue(path_path_list, 0, path, nodeNumber);
      return path;
   }
   return NULL;
}
...
float PathPosition::GetTargetWeight(int targetNumber){
   ...
   if (targetNumber >= 0 && targetNumber < ct){
      ...
      float pathWt;
      pblock->GetValue(path_path_weight, GetCOREInterface()->GetTime(), pathWt, targetNumber);
      return pathWt;
   }
   else{
      return 0.0f;
...
void PathPosition::EndEditParams( IObjParam *ip, ULONG flags,Animatable *next )
{
	if(editCont) //make sure it's up
	{
	   ...
	   if (pmap != NULL)
	   {
		  if (next && next->ClassID() == ClassID() && ((PathPosition*)next)->pblock  && ((PathPosition*)next)->GetLocked()==false)
		  {
...
//! [Animatable.SetProperty Example]
...
//! [Animatable.GetProperty Example]
   ...
   void proc(ILoad *iload)
   {
      if (old_version)
      {
         ...
         if (p->old_percent != NULL)
            p->pblock->SetControllerByID(path_percent, 0, p->old_percent, FALSE);
      }
      else
  ...
  if (path) {
      ...
      if (os.obj->IsShapeObject()) {
         ...
         if (pathOb->NumberOfCurves(t)) {
            Point3 p0, p1, d;
            float per, dt = 0.01f;
            pblock->GetValue(path_percent, t, per);
...
float PathPosition::IncIKParam(TimeValue t, int index, float delta)
   ...
   if (jp) {      
      if (jp->Limited(0)) {         
         pblock->GetValue(path_percent, t, v);
         gotV = TRUE;
         }
   ...
   pblock->GetControllerByID(path_percent)->SetValue(t,&delta,FALSE,CTRL_RELATIVE); //3rd parameter true
   return delta;
   }

void PathPosition::ClearIKParam(Interval iv,int index) 
{
	pblock->GetControllerByID(path_percent)->DeleteTime(iv,TIME_INCRIGHT|TIME_NOSLIDE);
}
...
void JointParamsPath::SpinnerChange(
   ...
   if (set && interactive) {
      ...
      c->pblock->GetControllerByID(path_percent)->SetValue(ui->ip->GetTime(),&val,TRUE,CTRL_ABSOLUTE);
