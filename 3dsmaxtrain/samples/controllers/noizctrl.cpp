...
class BaseNoiseControl : public INoiseControl ,public ILockedTrackImp
{
	public:		
		...
		BaseNoiseControl(BOOL loading = FALSE);
...
public:
		...
		void GetStrength(TimeValue t,float *strength) {
			if (cont) cont->GetValue(t,strength);
			}
	};
...
class FloatNoiseControl : public BaseNoiseControl {
	public:
		...
		FloatNoiseControl(BOOL loading = FALSE): BaseNoiseControl(loading) {
			cont=NULL;
			// Check whether "this" controller is set as the default controller to avoid infinite recursion
			if (GetDefaultController(CTRL_FLOAT_CLASS_ID) == GetFloatNoiseDesc())
				ReplaceReference(Control::NumRefs(),CreateInterpFloat());
			else
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? FLOATNOISE_CONTROL_CNAME : _T("Noise Float");}
...
class FloatNoiseClassDesc: public ClassDesc {
	...
	void*			Create(BOOL loading) { return new FloatNoiseControl(loading); }
	const TCHAR*	ClassName() { return FLOATNOISE_CONTROL_CNAME; }
	const TCHAR*	NonLocalizedClassName() { return _T("Noise Float"); }
	SClass_ID		SuperClassID() { return CTRL_FLOAT_CLASS_ID; }
	Class_ID		ClassID() { return Class_ID(FLOATNOISE_CONTROL_CLASS_ID,0); }
...
class PositionNoiseControl : public BaseNoiseControl {
	public:
		...
		PositionNoiseControl(BOOL loading = FALSE): BaseNoiseControl(loading) {
			cont=NULL;
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? POSITIONNOISE_CONTROL_CNAME : _T("Noise Position");}
...
class PositionNoiseClassDesc:public ClassDesc {
	public:
	int 			IsPublic() { return 1; }
	void*			Create(BOOL loading) { return new PositionNoiseControl(loading); }
	const TCHAR*	ClassName() { return POSITIONNOISE_CONTROL_CNAME; }
	const TCHAR*	NonLocalizedClassName() { return _T("Noise Position"); }
	SClass_ID		SuperClassID() { return CTRL_POSITION_CLASS_ID; }
	Class_ID		ClassID() { return Class_ID(POSITIONNOISE_CONTROL_CLASS_ID,0); }
	const TCHAR* 	Category() { return _T("");  }
	};
...
class Point3NoiseControl : public BaseNoiseControl {
	public:
		...
		Point3NoiseControl(BOOL loading = FALSE): BaseNoiseControl(loading) {
			cont=NULL;
			// Check whether "this" controller is set as the default controller to avoid infinite recursion
			if (GetDefaultController(CTRL_POINT3_CLASS_ID) == GetPoint3NoiseDesc())
				ReplaceReference(Control::NumRefs(),CreateInterpPoint3());
			else
				ReplaceReference(Control::NumRefs(),NewDefaultPoint3Controller());
			Point3 v(50.0f,50.0f,50.0f);			
			cont->SetValue(0,&v);
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? POINT3NOISE_CONTROL_CNAME : _T("Noise Point3");}
...
class Point3NoiseClassDesc:public ClassDesc {
	public:
	int 			IsPublic() { return 1; }
	void*			Create(BOOL loading) { return new Point3NoiseControl(loading); }
	const TCHAR*	ClassName() { return POINT3NOISE_CONTROL_CNAME; }
	const TCHAR*	NonLocalizedClassName() { return _T("Noise Point3"); }
	SClass_ID		SuperClassID() { return CTRL_POINT3_CLASS_ID; }
	Class_ID		ClassID() { return Class_ID(POINT3NOISE_CONTROL_CLASS_ID,0); }
	const TCHAR* 	Category() { return _T("");  }
	};
...
class RotationNoiseControl : public BaseNoiseControl {
	public:
		...
		RotationNoiseControl(BOOL loading = FALSE): BaseNoiseControl(loading) {
			Point3 strength;
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? ROTATIONNOISE_CONTROL_CNAME : _T("Noise Rotation");}
...
class RotationNoiseClassDesc:public ClassDesc {
	public:
	int 			IsPublic() { return 1; }
	void*			Create(BOOL loading) { return new RotationNoiseControl(loading); }
	const TCHAR*	ClassName() { return ROTATIONNOISE_CONTROL_CNAME; }
	const TCHAR*	NonLocalizedClassName() { return _T("Noise Rotation"); }
	SClass_ID		SuperClassID() { return CTRL_ROTATION_CLASS_ID; }
	Class_ID		ClassID() { return Class_ID(ROTATIONNOISE_CONTROL_CLASS_ID,0); }
	const TCHAR* 	Category() { return _T("");  }
	};
...
class ScaleNoiseControl : public BaseNoiseControl {
	public:
		...
		ScaleNoiseControl(BOOL loading = FALSE): BaseNoiseControl(loading) {
			Point3 strength;
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? SCALENOISE_CONTROL_CNAME : _T("Noise Scale");}
...
class ScaleNoiseClassDesc:public ClassDesc {
	public:
	int 			IsPublic() { return 1; }
	void*			Create(BOOL loading) { return new ScaleNoiseControl(loading); }
	const TCHAR*	ClassName() { return SCALENOISE_CONTROL_CNAME; }
	const TCHAR*	NonLocalizedClassName() { return _T("Noise Scale"); }
	SClass_ID		SuperClassID() { return CTRL_SCALE_CLASS_ID; }
	Class_ID		ClassID() { return Class_ID(SCALENOISE_CONTROL_CLASS_ID,0);}
	const TCHAR* 	Category() { return _T("");  }
	};
...
class RangeRestore : public RestoreObj {
	public:
		...
		TSTR Description() { return _T("Noise control range"); }
	};
...
static void DrawNoiseGraph(BaseNoiseControl *cont,HWND hWnd,HDC hdc)
	...
	for (int x = 0; x < rect.right + STEPSIZE; x += STEPSIZE) {
		...
		v = float(t) * float(0.0055) * cont->frequency + Perm(cont->seed);
		if (cont->fractal) {
...
static LRESULT CALLBACK NoiseGraphWinProc(
		HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
   BaseNoiseControl *cont = DLGetWindowLongPtr<BaseNoiseControl*>(hWnd);
...
class NoiseCtrlWindow {
	public:
		HWND hWnd;
		Control *cont;
		NoiseCtrlWindow(HWND hWnd, Control *cont)
			{this->hWnd=hWnd; this->cont=cont;}
	};
static MaxSDK::Array<NoiseCtrlWindow> noiseCtrlWindows;


static void UnRegisterNoiseCtrlWindow(HWND hWnd)
{	
	for (int i=0; i<noiseCtrlWindows.length(); i++) {
		if (hWnd==noiseCtrlWindows[i].hWnd) {
			noiseCtrlWindows.removeAt(i);
			return;
		}
	}	
}

static HWND FindOpenNoiseCtrlWindow(Control *cont)
{	
	for (size_t i=0; i<noiseCtrlWindows.length(); i++)
	{
		if (cont == noiseCtrlWindows[i].cont) { // One dialog per-control 
...
static void RegisterNoiseCtrlWindow(HWND hWnd, Control* cont)
{
	const HWND existingHandle = FindOpenNoiseCtrlWindow(cont);
	if(existingHandle == nullptr)
	{
		NoiseCtrlWindow rec(hWnd, cont);
		noiseCtrlWindows.append(rec); // Not already in the list, append it!
	}
}
...
class NoiseDlg : public ReferenceMaker, public TimeChangeCallback {
	public:
		...
		virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("NoiseDlg"); } // from Animatable
...
private:
		virtual void SetReference(int i, RefTargetHandle rtarg) {cont=(BaseNoiseControl*)rtarg;}
public:
...
NoiseDlg::NoiseDlg(
	...
	if(hWnd == nullptr) {
		...
		DbgAssert(false); // Failed to create the dialog.
		return;
	}
	
	TSTR title = TSTR(GetString(IDS_RB_NOISECONTROLTITLE)) + pname;
...
void NoiseDlg::Invalidate()
	{
	valid = FALSE;
	// 'hWnd' must be not null otherwise Max's entire UI will refresh.
	if(hWnd != nullptr)
	{
...
void NoiseDlg::SetupUI(HWND hWnd)
   ...
   DLSetWindowLongPtr(GetDlgItem(hWnd,IDC_NOISE_GRAPH),cont);
	valid = FALSE;
	Update();
	}
...
static INT_PTR CALLBACK NoiseDlgProc(
		HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
   NoiseDlg *dlg = DLGetWindowLongPtr<NoiseDlg*>(hWnd);
	...
	switch (msg) {
		...
		case WM_DESTROY:						
            // We're about to delete dlg; make sure to zero out the stored dlg pointer in the window's long pointer
		...
		DWORD flags)
{
	if(GetLocked()==false)
	{
		HWND hCur = FindOpenNoiseCtrlWindow(this);
		if (hCur) {
		...
		RegisterNoiseCtrlWindow(dlg->hWnd,this);
	}
}

//------------------------------------------------------------

BaseNoiseControl::BaseNoiseControl(BOOL loading) 
	{	
...
TSTR BaseNoiseControl::SubAnimName(int i, bool localized)
{
	if (i < Control::NumSubs())
		return Control::SubAnimName(i, localized);
	return localized ? GetString(IDS_RB_NOISESTRENGTH) : _T("Noise Strength");
}

BOOL BaseNoiseControl::AssignController(Animatable *control,int subAnim) 
{
	if(GetLocked()==false) //client is locked, don't assign child
	{
		...
		NotifyDependents(FOREVER, PART_ALL, REFMSG_CONTROLREF_CHANGE, NOTIFY_ALL, FALSE);
		return TRUE;
	}
	return FALSE;
}
...
float BaseNoiseControl::NoiseAtTime(TimeValue t,int s,int index)
	...
	// Compute ramping.  If time range is disabled, let ramp at 1.0
	...
	// With certain frame rates, calculating whole frame random values always returned zero.
	// Adding the 0.0005 just edges it off of a whole number and we get a desirable result.
	float v = float(t) * float(0.0055) * frequency + Perm(s);	
	if (fractal) {
...
void BaseNoiseControl::EditTimeRange(Interval range,DWORD flags)
	{
	if (GetLocked()==false&&!(flags&EDITRANGE_LINKTOKEYS)) {
		HoldRange();
		this->range = range;
