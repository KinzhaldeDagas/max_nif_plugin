...
Jiggle::Jiggle(int ctrlType, BOOL loading)
{
	...
	RegisterNotification(NotifyPostMerge, this, NOTIFY_OBJECT_XREF_POST_MERGE);
}
...
Jiggle::~Jiggle()
{
	...
	mSelfNode.SetRef(nullptr);
	UnRegisterNotification(NotifyPostMerge, this, NOTIFY_OBJECT_XREF_POST_MERGE);
}
		...
		const Interval& /*iv*/, RefTargetHandle hTarg, PartID& /*partID*/, RefMessage msg, BOOL /*propagate*/)
{
	...
	// PartId : PART_TM is when you'd clear a "stdvector cache".
...
TSTR Jiggle::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case JIGGLE_CONTROL_REF: return localized ? GetString(IDS_POS_CONTROL) : _T("Position");
	case JIGGLE_PBLOCK_REF1: return localized ? GetString(IDS_DYN_PARAMS) : _T("Spring Dynamics");
	case JIGGLE_PBLOCK_REF2: return localized ? GetString(IDS_FORCE_PARAMS) : _T("Forces, Limits and Precision");
	default: return _T("");
	}
}
	...
	if ( t >= ReferenceFrame && steps != 0 && !(xeffect == 0.0f && yeffect == 0.0f && zeffect == 0.0f))
	{
		...
		if (!validStart)
		{
			...
			TimeValue startTime = SpringSysGlobalSettings::IsAlwaysAccurate()?
				ReferenceFrame: std::max(ReferenceFrame,
...
{	
		...
		mSelfNode.GetRef() == NULL) )
	{
		...
		INode* pSelfNode = dep.GetNode();

		if (pSelfNode == NULL) 
		{
			pSelfNode = GetCOREInterface()->GetRootNode();
		}

		if (!pSelfNode->IsRootNode() && this->SuperClassID() == CTRL_POSITION_CLASS_ID )
		{
			pSelfNode = pSelfNode->GetParentNode();
		}
		
		//check for an IK controller and iterate through parents until you don't have one
		INode* tempNode = pSelfNode;
		...
		if (this->SuperClassID() == CTRL_POINT3_CLASS_ID )
		{
			...
			while (pSelfNode && !pSelfNode->IsRootNode() && TestForSubLoop(pSelfNode->GetTMController(), this))
				pSelfNode = pSelfNode->GetParentNode();
		}
		...
		{
			dyn_pb->SetValue(jig_control_node, 0, ((INode*)NULL), 0); 
			if (pSelfNode->IsRootNode())
				pSelfNode = NULL;
			flags |= HAS_NO_PARENT;
		}

		//Add the selfInfluence spring
		if ( !partsys->GetParticle(0)->GetSprings()->length() )
		{
		...
		mSelfNode.SetRef(pSelfNode);
	}
	return TRUE;
}

void Jiggle::NotifyPostMerge( void* param, NotifyInfo* info )
{
	if( param &&
		info &&
		info->intcode == NOTIFY_OBJECT_XREF_POST_MERGE)
	{
		Jiggle* pJiggle = static_cast<Jiggle*>(param);
		if(pJiggle)
		{
			//After XRecord is updated, the temp root is deleted. So the target node of mSelfNode will receive notification and become null. 
			//Actually ,the target node will be reset in the above function SetSelfReference() after the flags become initial values
			//and the target node of mSelfNode become null.
			pJiggle->flags = 0;
		}
	}
}
