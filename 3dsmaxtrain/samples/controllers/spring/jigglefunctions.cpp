...
{
	INode* baseNode = NULL;
	baseNode = dynamic_cast<INode*>(mSelfNode.GetRef());
...
static MaxSDK::Array<JiggleWindow> jiggleWindows;
...
{	
	for (int i=0; i<jiggleWindows.length(); i++) 
	{
		if (hWnd==jiggleWindows[i].hWnd) 
		{
			jiggleWindows.removeAt(i);
			return;
		}
	}	
}
