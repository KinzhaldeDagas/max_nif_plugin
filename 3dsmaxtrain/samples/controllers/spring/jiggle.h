...
#define GetSpringControllerInterface(cd) (Jiggle*)(cd)->GetInterface(SPRING_CONTROLLER_INTERFACE)

class Jiggle
		: public LockableControl
		, public IJiggle
		, public SpringSysClient
{
public:
	int type; // the type of controller it is
	bool validStart;
	MaxSDK::SingleWeakRefMaker mSelfNode;
	BOOL ctrlValid;
...
public:
	...
	{
		posCtrl->CommitValue(t);
		ctrlValid = true;
	}
	void RestoreValue(TimeValue t)
	{
		posCtrl->RestoreValue(t);
	}
...
protected:
	static void NotifyPostMerge(void* param, NotifyInfo* info);

	// Set to true during GetValue.
	// Used to skip refchange messages called from internal solve.
...
class PosJiggle : public Jiggle {
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_CLASS_NAME) : _T("Spring");}
...
class PosJiggleClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Spring"); }
	SClass_ID		SuperClassID() {return CTRL_POSITION_CLASS_ID;}
	Class_ID		ClassID() {return JIGGLE_POS_CLASS_ID;}
...
static ParamBlockDesc2 jig_param_blk ( jig_dynamics_params, _T("JiggleDynamicsParameters"),  0, &PosJiggleDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, JIGGLE_PBLOCK_REF1, 
	...
	jig_control_node,    _T(""),  TYPE_INODE_TAB,		0,	0,	IDS_JIG_CONTROL_NODES,
		p_nonLocalizedName, _T("Control Nodes"),
		p_end,
...
static ParamBlockDesc2 jig_force_param_blk ( jig_force_params, _T("ForceAndLimits"),  0, &PosJiggleDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, JIGGLE_PBLOCK_REF2, 
	...
	jig_xeffect,  _T("x_effect"), 			TYPE_FLOAT, 	P_ANIMATABLE, 	IDS_JIGGLE_X, 
		...
		p_accessor,		&forcesPBAccessor,
		p_nonLocalizedName, _T("X Effect"),
		p_end, 
	jig_yeffect,  _T("y_effect"), 			TYPE_FLOAT, 	P_ANIMATABLE, 	IDS_JIGGLE_Y, 
		...
		p_accessor,		&forcesPBAccessor,
		p_nonLocalizedName, _T("Y Effect"),
		p_end, 
	jig_zeffect,  _T("z_effect"), 			TYPE_FLOAT, 	P_ANIMATABLE, 	IDS_JIGGLE_Z, 
		...
		p_accessor,		&forcesPBAccessor,
		p_nonLocalizedName, _T("Z Effect"),
		p_end, 
...
class JiggleDlg : public ReferenceMaker {
	public:
		...
		virtual void GetClassName(MSTR& s, bool localized) const override { UNUSED_PARAM(localized); s = _M("JiggleDlg"); } // From Animatable
