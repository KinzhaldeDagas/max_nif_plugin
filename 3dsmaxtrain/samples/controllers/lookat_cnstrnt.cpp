...
class LookAtConstRotation : public	ILookAtConstRotation  ,public ILockedTrackImp
{
	private:
		bool isBBoxValid;
		bool mInGetValue{ false };
		TimeValue mLastEvalTime{ std::numeric_limits<int>::min() };
	...
	public:
		...
		void SetVectorLineLength(float len) override;
		float GetVectorLineLength() const override;
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? LOOKAT_CONSTRAINT_CNAME : _T("LookAt Constraint");}
		...
		TSTR SubAnimName(int i, bool localized) override { return localized ? GetString(IDS_AG_LOOKATPARAMS) : _T("LookAt Constraint"); }
...
private:
		void GetDisplayDottedLine(
			...
			int i, 
			int count);
		void GetDisplaySolidLine(
			Point3& sourcePosition, 
			Point3& targetPosition, 
			TimeValue t);
		void BuildWorldBoundBox(TimeValue t);
		virtual void SetReference(int i, RefTargetHandle rtarg);

		float GetTotalLookAtTargetWeight(TimeValue t);
public:
...
void LookAtConstTimeChangeCallback :: TimeChanged(TimeValue t){
	...
	Control *cont = lookAt_controller->pblock->GetControllerByID(lookat_target_weight, selection); 
...
class LookAtConstClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("LookAt Constraint"); }
	SClass_ID		SuperClassID() { return CTRL_ROTATION_CLASS_ID; }
...
void PickLookAtNode::EnterMode(IObjParam *ip)
	{
		...
		GetCOREInterface()->PushPrompt(_T("Select LookAt Target Object"));
	}
...
//*****************************************************
  ...
  static FPInterfaceDesc lookAt_constraint_interface(
    LOOKAT_CONSTRAINT_INTERFACE, _T("constraints"), 0, &lookAtCD, 0,
		...
		properties,
		ILookAtConstRotation::get_vector_line_length, ILookAtConstRotation::set_vector_line_length, _T("VectorLineLength"), 0, TYPE_FLOAT,
...
//										EnableWindow(GetDlgItem(p->hWnd, IDC_LOOKAT_CONS_UPNODE_PICKNODE), TRUE);
						...
						if (p->GetnonNULLNumLookAtTargets() < 1) { 
							...
							// there is a valid selection 
							// automatically means there is AT LEAST one target
							Control *cont = p->pblock->GetControllerByID(lookat_target_weight, selection); 
							// until it is animated, paramblock doesn't have a controller assigned in it yet.
							if (spin) {
						...
						case IDC_LOOKAT_TARG_LIST:
							...
							if (HIWORD(wParam) == LBN_SELCHANGE){
								...
								// there are more than one targets
									...
									Control *cont = p->pblock->GetControllerByID(lookat_target_weight, selection); // until it is animated, paramblock doesn't have a controller assigned in it yet.
									if (spin && (cont != NULL)) {
										if (cont->IsKeyAtTime(t,KEYAT_ROTATION)){
											spin->SetKeyBrackets(TRUE);
										} 
...
static ParamBlockDesc2 lookAt_const_paramblk (lookAt_const_params, _T("LookAtConsParameters"),  0, &lookAtCD, P_AUTO_CONSTRUCT + P_AUTO_UI, LOOKAT_ROT_PBLOCK_REF, 
	...
	lookat_vector_line_length, 	_T("lookat_vector_length"), TYPE_FLOAT, P_ANIMATABLE + P_RESET_DEFAULT, IDS_AG_LOOKAT_VECTOR_LENGTH,
		...
		p_accessor,		&lookAt_const_accessor,	
		p_nonLocalizedName, _T("Vector Length"),
		p_end,
	...
	lookat_target_axis_flip, _T("target_axisFlip"),	TYPE_BOOL,	P_RESET_DEFAULT,	IDS_AG_LOOKAT_TARGET_AXISFLIP,
		p_default, 		FALSE, 
		p_ui, 			TYPE_SINGLECHECKBOX, IDC_LOOKAT_CONS_TARGET_FLIP, 
		p_accessor,		&lookAt_const_accessor,
		p_enabled,		TRUE,
		p_end, 
	...
	lookat_upnode_world,	_T("upnode_world"), TYPE_BOOL,	P_RESET_DEFAULT,	IDS_AG_LOOKAT_UPNODE_W,
		p_default, 		TRUE, 
		p_ui, 			TYPE_SINGLECHECKBOX, IDC_LOOKAT_CONS_UPNODE_W, 
		p_accessor,		&lookAt_const_accessor,
		p_end,
	...
	lookat_upnode_pick,  _T("pickUpNode"), 		TYPE_INODE,  0,					IDS_AG_LOOKAT_UPNODE_PICKNODE,
		...
		p_nonLocalizedName, _T("Pick Upnode"),
		p_end,
	...
	lookat_StoUP_axis_flip, _T("StoUP_axisFlip"),	TYPE_BOOL,	P_RESET_DEFAULT,	IDS_AG_LOOKAT_S_UP_AXISFLIP,
		p_default, 		FALSE, 
		p_ui, 			TYPE_SINGLECHECKBOX, IDC_LOOKAT_CONS_S_UP_FLIP, 
		p_accessor,		&lookAt_const_accessor,
		p_end, 

	viewline_length_abs,	_T("viewline_length_abs"), TYPE_BOOL,	P_RESET_DEFAULT,	IDS_AG_LOOKAT_CONS_VL_ABS,
		p_default, 		FALSE, 
		p_ui, 			TYPE_SINGLECHECKBOX, IDC_AG_LOOKAT_CONS_VL_ABS, 
		p_accessor,		&lookAt_const_accessor,
		p_end,
...
RefTargetHandle LookAtConstRotation::Clone(RemapDir& remap)
    ...
    p->ReplaceReference(LOOKAT_ROT_PBLOCK_REF, remap.CloneRef(pblock));
...
void LookAtConstRotation::Copy(Control *from)
{
	...
	if (from->ClassID()==ClassID()) {
		...
		ReplaceReference(LOOKAT_ROT_PBLOCK_REF, CloneRefHierarchy(ctrl->pblock));
		curRot   = ctrl->curRot;
		mLocked  = ctrl->mLocked;
	} else {
		if(from&&GetLockedTrackInterface(from))
		  mLocked = GetLockedTrackInterface(from)->GetLocked();
		from->GetValue(GetCOREInterface()->GetTime(), &fvalRot);	//  to know the object lookat before the
		initialLookAtFlag = 1;
...
float LookAtConstRotation::GetTotalLookAtTargetWeight(TimeValue t)
{
	float total_lookat_target_weight = 0.0f;
	int count = pblock->Count(lookat_target_list);
	...
	return total_lookat_target_weight;
}
...
Point3 LookAtConstRotation::GetTargetPosition(TimeValue t)
	...
	for (int i = 0; i < ct; i++) {
		...
		pblock->GetValue(lookat_target_list, t, lookat_target, i);
	...
	if (total_lookat_target_weight > more_than_and_very_close_to_zero ){
...
Quat LookAtConstRotation::ComputeLookAtQuat(TimeValue t){
	...
	if(pblock->Count(lookat_target_list)>0){
		...
		pblock->GetValue(lookat_upnode_pick, 0, upn);
		...
		if (GetUpnodeWorld()) {
			...
			if (pblock->Count(lookat_target_list) == 1 && Get_upnode_control() == 0){
				...
				pblock->GetValue(lookat_target_list, t, the_only_target, 0);
...
void LookAtConstRotation::GetDisplayDottedLine(
	Point3& sourcePosition, 
	Point3& targetPosition, 
	INode* lookat_target, 
	TimeValue t, 
...
{
	Interval iv;
	sourcePosition = sourceTM.GetTrans();
	...
	pblock->GetValue(lookat_target_weight, t, targetWt, i);
	...
	if (VLAbs())
	{
		float total_weight = GetTotalLookAtTargetWeight(t);
		total_weight = abs(total_weight) > more_than_and_very_close_to_zero ? total_weight : 1.0f;
		targetPosition = sourcePosition + count * source_target_unit_vector * (vector_length/total_weight) * targetWt;
	}
	else
	{
...
void LookAtConstRotation::GetDisplaySolidLine(
	Point3& sourcePosition, 
	Point3& targetPosition, 
	TimeValue t)
{
	...
	Point3& r = sourcePosition;
	Point3& q = targetPosition;

	r = sourceTM.GetTrans();
	q = GetTargetPosition(t);
...
int LookAtConstRotation::Display(
		TimeValue t, INode* inode, ViewExp *vpt, int flags)
	{
		...
		gw->setTransform(Matrix3{});		// sets the graphicsWindow to world
		...
		for (int i = 0; i < ct; i++)
		{
			...
			{
				continue; // skip the for loop if the selected object is null  
			}

			Point3 sourcePosition;
			Point3 targetPosition;
			GetDisplayDottedLine(
				sourcePosition, 
				targetPosition, 
				lookat_target, 
				t, 
			...
			DWORD targColor =  lookat_target->GetWireColor();
...
void LookAtConstRotation::BuildWorldBoundBox(TimeValue t)
{
	bbox.Init();
	int ct = pblock->Count(lookat_target_list);
	if (ct <= 0)
...
RefResult LookAtConstRotation::NotifyRefChanged (const Interval& iv, RefTargetHandle hTarg, PartID& partID, RefMessage msg, BOOL propagate) 
	{
...
INode* LookAtConstRotation::GetNode(int nodeNumber){
	if (nodeNumber >= 0 && nodeNumber < pblock->Count(lookat_target_list)){
		INode *lookAt_targ;
		pblock->GetValue(lookat_target_list, 0, lookAt_targ, nodeNumber);
		return lookAt_targ;
	}
	return NULL;
}
...
float LookAtConstRotation::GetTargetWeight(int targetNumber){
	if (targetNumber >= 0 && targetNumber < pblock->Count(lookat_target_list)){
		float targetWt;
		pblock->GetValue(lookat_target_weight, GetCOREInterface()->GetTime(), targetWt, targetNumber);
		return targetWt;
	}
	else{
		return 0.0f;
...
void LookAtConstRotation::SetVectorLineLength(float len)
{
	pblock->SetValue(lookat_vector_line_length, GetCOREInterface()->GetTime(), len, 0);
}

float LookAtConstRotation::GetVectorLineLength() const
{
	float v = 0.0f;
	pblock->GetValue(lookat_vector_line_length, GetCOREInterface()->GetTime(), v, 0);
	return v;
}
...
void LookAtConstRotation::EndEditParams( IObjParam *ip, ULONG flags,Animatable *next )
{
	if(editCont)
	{
		...
		if (pmap != NULL)
		{
			if (next && next->ClassID() == ClassID() && ((LookAtConstRotation*)next)->pblock&&((LookAtConstRotation*)next)->GetLocked()==false)
			{
