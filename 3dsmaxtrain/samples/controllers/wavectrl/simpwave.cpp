...
class IWaveControl: public FPMixinInterface
{
public:

	FPInterfaceDesc* GetDesc(); 

	virtual int GetNbWaves() const = 0;
	virtual bool AppendWave() = 0;
	virtual bool InsertWave(int ndx) = 0;
	virtual bool DeleteWave(int ndx) = 0;

	virtual const TCHAR* GetName(int ndx) const = 0;
	virtual void SetName(int ndx, const TCHAR* val) = 0;

	virtual float GetPeriod(int ndx) const = 0;
	virtual void SetPeriod(int ndx, float val) = 0;

	virtual float GetDuty(int ndx) const = 0;
	virtual void SetDuty(int ndx, float val) = 0;

	virtual float GetAmplitude(int ndx) const = 0;
	virtual void SetAmplitude(int ndx, float val) = 0;

	virtual float GetPhase(int ndx) const = 0;
	virtual void SetPhase(int ndx, float val) = 0;

	virtual int GetBiasType(int ndx) const = 0;
	virtual void SetBiasType(int ndx, int val) = 0;

	virtual float GetBias(int ndx) const = 0;
	virtual void SetBias(int ndx, float val) = 0;

	virtual int GetType(int ndx) const = 0;
	virtual void SetType(int ndx, int val) = 0;
	virtual int GetEffect(int ndx) const = 0;
	virtual void SetEffect(int ndx, int val) = 0;
	virtual BOOL IsEnabled(int ndx) const = 0;
	virtual void Enable(int ndx, BOOL val) = 0;
	virtual BOOL GetInverted(int ndx) const = 0;
	virtual void SetInverted(int ndx, BOOL val) = 0;
	virtual BOOL GetFlipped(int ndx) const = 0;
	virtual void SetFlipped(int ndx, BOOL val) = 0;
};


class BaseWaveControl : public BaseWaveControlInternal,
	public IWaveControl
{
public:
	enum fnID {
	...
	// --- enum for the enum types
	enum { BIAS_TYPES, TYPE_TYPES, EFFECT_TYPES };

	BEGIN_FUNCTION_MAP
 		RO_PROP_FN(kGetNbWaves, GetNbWaves, TYPE_INT);
		FN_0(kAppend, TYPE_bool, AppendWave);
		FN_1(kInsert, TYPE_bool, InsertWave, TYPE_INDEX);
		FN_1(kDelete, TYPE_bool, DeleteWave, TYPE_INDEX);
		FN_1(kGetName, TYPE_STRING, GetName, TYPE_INDEX);
		VFN_2(kSetName, SetName, TYPE_INDEX, TYPE_STRING);
		FN_1(kGetPeriod, TYPE_FLOAT, GetPeriod, TYPE_INDEX);
		VFN_2(kSetPeriod, SetPeriod, TYPE_INDEX, TYPE_FLOAT);
		FN_1(kGetDuty, TYPE_FLOAT, GetDuty, TYPE_INDEX);
		VFN_2(kSetDuty, SetDuty, TYPE_INDEX, TYPE_FLOAT);		
		FN_1(kGetAmplitude, TYPE_FLOAT, GetAmplitude, TYPE_INDEX);
		VFN_2(kSetAmplitude, SetAmplitude, TYPE_INDEX, TYPE_FLOAT);
		FN_1(kGetPhase, TYPE_FLOAT, GetPhase, TYPE_INDEX);
		VFN_2(kSetPhase, SetPhase, TYPE_INDEX, TYPE_FLOAT);
		FN_1(kGetBiasType, TYPE_ENUM, GetBiasType, TYPE_INDEX);
		VFN_2(kSetBiasType, SetBiasType, TYPE_INDEX, TYPE_ENUM);
		FN_1(kGetBias, TYPE_FLOAT, GetBias, TYPE_INDEX);
		VFN_2(kSetBias, SetBias, TYPE_INDEX, TYPE_FLOAT);
		FN_1(kGetType, TYPE_ENUM, GetType, TYPE_INDEX);
		VFN_2(kSetType, SetType, TYPE_INDEX, TYPE_ENUM);
		FN_1(kGetEffect, TYPE_ENUM, GetEffect, TYPE_INDEX);
		VFN_2(kSetEffect, SetEffect, TYPE_INDEX, TYPE_ENUM);
		FN_1(kGetEnabled, TYPE_BOOL, IsEnabled, TYPE_INDEX);
		VFN_2(kSetEnabled, Enable, TYPE_INDEX, TYPE_BOOL);
		FN_1(kGetInverted, TYPE_BOOL, GetInverted, TYPE_INDEX);
		VFN_2(kSetInverted, SetInverted, TYPE_INDEX, TYPE_BOOL);
		FN_1(kGetFlipped, TYPE_BOOL, GetFlipped, TYPE_INDEX);
		VFN_2(kSetFlipped, SetFlipped, TYPE_INDEX, TYPE_BOOL);
	END_FUNCTION_MAP

	///////////////////////////////////////////////////////////////////////////
	// IWaveControl methods
	///////////////////////////////////////////////////////////////////////////
	...
	bool InsertWave(int ndx) {
		bool valid = ndx >= 0 && ndx < waves;
		if (valid) {
			Insert(ndx);
			NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
		}
	...
	const TCHAR* GetName(int ndx) const {
		return (ndx >= 0 && ndx < waves)? name[ndx].data() : _T("");
	}
	void SetName(int ndx, const TCHAR* val) {
		if (ndx >= 0 && ndx < waves) {
			name[ndx] = val;
			NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
		}
	...
	float GetPeriod(int ndx) const {
		return (ndx >= 0 && ndx < waves)? period[ndx]: 0.0f;
	}
	void SetPeriod(int ndx, float val) {
		if (ndx >= 0 && ndx < waves) {
			period[ndx] = val;
			NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
		}
	...
	float GetDuty(int ndx) const {
		return (ndx >= 0 && ndx < waves)? duty[ndx]: 0.0f;
	}
	void SetDuty(int ndx, float val) {
		if (ndx >= 0 && ndx < waves) {
			duty[ndx] = val;
			NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
		}
	}
	float GetAmplitude(int ndx) const {
		return (ndx >= 0 && ndx < waves)? amplitude[ndx]: 0.0f;
	}
	void SetAmplitude(int ndx, float val) {
		if (ndx >= 0 && ndx < waves) {
			amplitude[ndx] = val;
			NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
		}
	...
	float GetPhase(int ndx) const {
		return (ndx >= 0 && ndx < waves)? phase[ndx]: 0.0f;
	}
	void SetPhase(int ndx, float val) {
		if (ndx >= 0 && ndx < waves) {
			phase[ndx] = val;
			NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
		}
	...
	int GetBiasType(int ndx) const {
		return (ndx >= 0 && ndx < waves)? biasType[ndx]: BIAS_CENTERED;
	}
	void SetBiasType(int ndx, int val) {
		if (ndx >= 0 && ndx < waves) {
			biasType[ndx] = val;
			NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
		}
	...
	float GetBias(int ndx) const {
		return (ndx >= 0 && ndx < waves)? bias[ndx]: 0.0f;
	}
	void SetBias(int ndx, float val) {
		if (ndx >= 0 && ndx < waves) {
			bias[ndx] = val;
			NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
		}
	...
	int GetType(int ndx) const {
		return (ndx >= 0 && ndx < waves)? type[ndx]: TYPE_SINE;
	}
	void SetType(int ndx, int val) {
		if (ndx >= 0 && ndx < waves) {
			type[ndx] = val;
			NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
		}
	...
	int GetEffect(int ndx) const {
		return (ndx >= 0 && ndx < waves)? effect[ndx]: EFFECT_ADD;
	}
	void SetEffect(int ndx, int val) {
		if (ndx >= 0 && ndx < waves) {
			effect[ndx] = val;
			NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
		}
	...
	BOOL IsEnabled(int ndx) const {
		return (ndx >= 0 && ndx < waves)? enabled[ndx]: TRUE;
	}
	void Enable(int ndx, BOOL val) {
		if (ndx >= 0 && ndx < waves) {
			enabled[ndx] = val;
			NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
		}
	...
	BOOL GetInverted(int ndx) const {
		return (ndx >= 0 && ndx < waves)? inverted[ndx]: FALSE;
	}
	void SetInverted(int ndx, BOOL val) {
		if (ndx >= 0 && ndx < waves) {
			inverted[ndx] = val;
			NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
		}
	...
	BOOL GetFlipped(int ndx) const {
		return (ndx >= 0 && ndx < waves)? flipped[ndx]: FALSE;
	}
	void SetFlipped(int ndx, BOOL val) {
		if (ndx >= 0 && ndx < waves) {
			flipped[ndx] = val;
			NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
		}
	...
	static FPInterfaceDesc	s_interfaceDesc;

	FPInterfaceDesc*	GetDesc() { return &s_interfaceDesc; }
	BaseInterface*		GetInterface(Interface_ID id);
	  ...
	  virtual int		GetWaveformsCount() {return waves;}
	  virtual void		SetWaveformsCount(int iCount) {waves = iCount;}
   };
...
class FloatWaveControl : public BaseWaveControl {
   public:
      ...
      void GetClassName(MSTR& s, bool localized) const override {s = localized ? FLOATWAVE_CONTROL_CNAME : _T("Waveform Float");}
...
class FloatWaveClassDesc:public ClassDesc {
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Waveform Float"); }
   SClass_ID      SuperClassID() { return CTRL_FLOAT_CLASS_ID; }
   Class_ID    ClassID() { return Class_ID(FLOATWAVE_CONTROL_CLASS_ID,0); }
...
class RangeRestore : public RestoreObj {
   public:
      ...
      TSTR Description() { return _T("Wave control range"); }
   };
...
static LRESULT CALLBACK WaveGraphWinProc(
      HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
   {
   BaseWaveControl *cont = DLGetWindowLongPtr<BaseWaveControl*>(hWnd);
...
//--------------------------------------------------------------
#define IID_WAVEFORM Interface_ID( 0x15ED4348, 0xEE8C685D)

//===========================================================================
// Function Publishing descriptor for waveform Interface
//===========================================================================
// Container object interface instance and descriptor
FPInterfaceDesc BaseWaveControl::s_interfaceDesc(
	IID_WAVEFORM,			// Interface id
	_T("WaveformCtrl"),		// Interface name used by maxscript - don't localize it!
	0,						// Res ID of description string
	GetFloatWaveDesc(),		// Class descriptor						
	FP_MIXIN,

	// - Methods -
	BaseWaveControl::kAppend, _T("Append"), 0, TYPE_bool, 0, 0,
	BaseWaveControl::kInsert, _T("Insert"), 0, TYPE_bool, 0, 1,
		_T("wave index"), 0, TYPE_INDEX,
	BaseWaveControl::kDelete, _T("Delete"), 0, TYPE_bool, 0, 1,
		_T("wave index"), 0, TYPE_INDEX,
	BaseWaveControl::kGetName, _T("GetName"), 0, TYPE_STRING, 0, 1,
		_T("wave index"), 0, TYPE_INDEX,
	BaseWaveControl::kSetName, _T("SetName"), 0, TYPE_VOID, 0, 2,
		_T("wave index"), 0, TYPE_INDEX,
		_T("name"), 0, TYPE_STRING,
	BaseWaveControl::kGetPeriod, _T("GetPeriod"), 0, TYPE_FLOAT, 0, 1,
		_T("wave index"), 0, TYPE_INDEX,
	BaseWaveControl::kSetPeriod, _T("SetPeriod"), 0, TYPE_VOID, 0, 2,
		_T("wave index"), 0, TYPE_INDEX,
		_T("period"), 0, TYPE_FLOAT,
	BaseWaveControl::kGetDuty, _T("GetDutyCycle"), 0, TYPE_FLOAT, 0, 1,
		_T("wave index"), 0, TYPE_INDEX,
	BaseWaveControl::kSetDuty, _T("SetDutyCycle"), 0, TYPE_VOID, 0, 2,
		_T("wave index"), 0, TYPE_INDEX,
		_T("duty cycle"), 0, TYPE_FLOAT,
	BaseWaveControl::kGetAmplitude, _T("GetAmplitude"), 0, TYPE_FLOAT, 0, 1,
		_T("wave index"), 0, TYPE_INDEX,
	BaseWaveControl::kSetAmplitude, _T("SetAmplitude"), 0, TYPE_VOID, 0, 2,
		_T("wave index"), 0, TYPE_INDEX,
		_T("amplitude"), 0, TYPE_FLOAT,
	BaseWaveControl::kGetPhase, _T("GetPhase"), 0, TYPE_FLOAT, 0, 1,
		_T("wave index"), 0, TYPE_INDEX,
	BaseWaveControl::kSetPhase, _T("SetPhase"), 0, TYPE_VOID, 0, 2,
		_T("wave index"), 0, TYPE_INDEX,
		_T("phase"), 0, TYPE_FLOAT,
	BaseWaveControl::kGetBiasType, _T("GetBiasType"), 0, TYPE_ENUM, BIAS_TYPES, 0, 1,
		_T("wave index"), 0, TYPE_INDEX,
	BaseWaveControl::kSetBiasType, _T("SetBiasType"), 0, TYPE_VOID, 0, 2,
		_T("wave index"), 0, TYPE_INDEX,
		_T("bias type"), 0, TYPE_ENUM, BIAS_TYPES,
	BaseWaveControl::kGetBias, _T("GetBias"), 0, TYPE_FLOAT, 0, 1,
		_T("wave index"), 0, TYPE_INDEX,
	BaseWaveControl::kSetBias, _T("SetBias"), 0, TYPE_VOID, 0, 2,
		_T("wave index"), 0, TYPE_INDEX,
		_T("bias"), 0, TYPE_FLOAT,
	BaseWaveControl::kGetType, _T("GetType"), 0, TYPE_ENUM, TYPE_TYPES, 0, 1,
		_T("wave index"), 0, TYPE_INDEX,
	BaseWaveControl::kSetType, _T("SetType"), 0, TYPE_VOID, 0, 2,
		_T("wave index"), 0, TYPE_INDEX,
		_T("effect type"), 0, TYPE_ENUM, TYPE_TYPES,
	BaseWaveControl::kGetEffect, _T("GetEffect"), 0, TYPE_ENUM, EFFECT_TYPES, 0, 1,
		_T("wave index"), 0, TYPE_INDEX,
	BaseWaveControl::kSetEffect, _T("SetEffect"), 0, TYPE_VOID, 0, 2,
		_T("wave index"), 0, TYPE_INDEX,
		_T("effect type"), 0, TYPE_ENUM, EFFECT_TYPES,
	BaseWaveControl::kGetEnabled, _T("IsEnabled"), 0, TYPE_BOOL, 0, 1,
		_T("wave index"), 0, TYPE_INDEX,
	BaseWaveControl::kSetEnabled, _T("Enable"), 0, TYPE_VOID, 0, 2,
		_T("wave index"), 0, TYPE_INDEX,
		_T("enabled state"), 0, TYPE_BOOL,
	BaseWaveControl::kGetInverted, _T("GetInverted"), 0, TYPE_BOOL, 0, 1,
		_T("wave index"), 0, TYPE_INDEX,
	BaseWaveControl::kSetInverted, _T("SetInverted"), 0, TYPE_VOID, 0, 2,
		_T("wave index"), 0, TYPE_INDEX,
		_T("inverted"), 0, TYPE_BOOL,
	BaseWaveControl::kGetFlipped, _T("GetFlipped"), 0, TYPE_BOOL, 0, 1,
		_T("wave index"), 0, TYPE_INDEX,
	BaseWaveControl::kSetFlipped, _T("SetFlipped"), 0, TYPE_VOID, 0, 2,
		_T("wave index"), 0, TYPE_INDEX,
		_T("flipped"), 0, TYPE_BOOL,

	enums,
	BIAS_TYPES, 4,
		_T("centered"), BIAS_CENTERED,
		_T("positive"), BIAS_POSITIVE,
		_T("negative"), BIAS_NEGATIVE,
		_T("manual"), BIAS_MANUAL,

	TYPE_TYPES, 5,
		_T("square"), TYPE_SQUARE,
		_T("sine"), TYPE_SINE,
		_T("sawtooth"), TYPE_SAWTOOTH,
		_T("triangle"), TYPE_TRIANGLE,
		_T("halfsine"), TYPE_HALFSINE,

	EFFECT_TYPES, 4,
		_T("add"), EFFECT_ADD,
		_T("multiply"), EFFECT_MULTIPLY,
		_T("clamp_above"), EFFECT_CLAMP_ABOVE,
		_T("clamp_below"), EFFECT_CLAMP_BELOW,

	properties,
		BaseWaveControl::kGetNbWaves, FP_NO_FUNCTION, _T("nbWaves"), 0, TYPE_INT,
...
///////////////////////////////////////////////////////////////////////////////
FPInterfaceDesc* IWaveControl::GetDesc()
{
	return &BaseWaveControl::s_interfaceDesc;
}

BaseInterface* BaseWaveControl::GetInterface(Interface_ID id)
{
	if (id == IID_WAVEFORM) 
		return static_cast<IWaveControl*>(this); 
	else
		return NULL;
} 
...
class WaveDlg : public ReferenceMaker {
   public:
      ...
      virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("WaveDlg"); } // from Animatable
...
private:
      virtual void SetReference(int i, RefTargetHandle rtarg) {cont=(BaseWaveControl*)rtarg;}
public:
   };
...
WaveDlg::WaveDlg(
   ...
   this->dim  = dim;
   valid = FALSE;
   theHold.Suspend(); // LAM - 8/21/04 - defect 589682
   ...
   hWnd = CreateDialogParam(
      ...
      WaveDlgProc,
      (LPARAM)this); 
   TSTR title = TSTR(GetString(IDS_TH_WAVECONTROLTITLE)) + pname;
...
static void RemoveTrailingZeros(TCHAR* buf) 
{
   int n = static_cast<int>(_tcslen(buf)); // SR DCAST64: Downcast to 2G limit.
   for (int i=0; i<n; i++) 
	 {
      if (buf[i] == _T('.') || buf[i] == _T(',')) 
			{
         for (intptr_t j = n-1; j > i+1; j--) 
				 {
            if (buf[j]==_T('0')) 
               buf[j] = 0;
            else 
...
void WaveDlg::Update()
   ...
   if (!valid && hWnd) {
      ...
      for(int i = 0; i < cont->waves; ++i)
         SendMessage(GetDlgItem(hWnd,IDC_WAVELIST), LB_ADDSTRING, 0, (LPARAM)(LPCTSTR)cont->name[i].data());
      SendMessage(GetDlgItem(hWnd,IDC_WAVELIST), LB_SETCURSEL, cont->index, 0);
      SendMessage(GetDlgItem(hWnd,IDC_WAVENAME), WM_SETTEXT, 0, (LPARAM)(LPCTSTR)cont->name[index].data());
      ...
      case IDC_WAVENAME:
         switch(notify) {
            ...
            case EN_CHANGE:
               ...
               SendMessage(GetDlgItem(hWnd,IDC_WAVELIST), LB_INSERTSTRING, index, (LPARAM)(LPCTSTR)cont->name[index].data());
...
static INT_PTR CALLBACK WaveDlgProc(
      HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
   {
   WaveDlg *dlg = DLGetWindowLongPtr<WaveDlg*>(hWnd);
...
//! [StdControl.Extrapolate Example]
