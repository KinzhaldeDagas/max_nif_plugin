...
#define MAINLAYERCONTROLMANAGER_CNAME GetString(IDS_MAINLAYERCONTROLMANAGER)
...
// this is used to find the dependents parent of the subanim reference
// private namespace
namespace {
...
static void PositionWindowNearCursor(HWND hwnd)
{
	...
	if (cur.x + winwid > scrwid - 10)
		cur.x = scrwid - winwid - 10; // check too far right
	if (cur.x < 10)
		cur.x = 10; // check too far left
	if (cur.y + winhgt > scrhgt - 50)
		cur.y = scrhgt - 50 - winhgt; // check if too low
	if (cur.y < 10)
		cur.y = 10; // check for too high s45
...
MainLayerControlManager* GetMLCMan();
...
{
	if (anim != NULL &&
			(anim->ClassID() == (FLOATLAYER_CONTROL_CLASS_ID) || anim->ClassID() == (POINT3LAYER_CONTROL_CLASS_ID) ||
					anim->ClassID() == (POSLAYER_CONTROL_CLASS_ID) || anim->ClassID() == (ROTLAYER_CONTROL_CLASS_ID) ||
...
static bool LayerActiveOnlyFilter(
		Animatable* anim, Animatable* parent, int subAnimIndex, Animatable* grandParent, INode* node)
{
	if (IsLayerControl(parent) == true && GetMLCMan())
	{
		MainLayerControlManager* man = GetMLCMan();
		if (man->GetFilterActiveOnlyTrackView() == FALSE)
		{
			auto* lC = dynamic_cast<LayerControl*>(parent);
...
static bool LayerNoWeightsFilter(
		Animatable* anim, Animatable* parent, int subAnimIndex, Animatable* grandParent, INode* node)
{
...
{
	auto* tb = GetCOREInterface()->GetTrackBar();
	if (tb != nullptr)
	{
		auto* filterManager = static_cast<ITrackBarFilterManager*>(tb->GetInterface(TRACKBAR_FILTER_MANAGER_INTERFACE));
		if (filterManager != nullptr)
		{
			filterManager->RegisterFilter(LayerActiveOnlyFilter, nullptr, GetString(IDS_LAYER_ACTIVE_ONLY_FILTER),
					Class_ID(0x1adf422d, 0x38145d5f), true);
			filterManager->RegisterFilter(LayerNoWeightsFilter, nullptr, GetString(IDS_LAYER_NO_WEIGHTS_FILTER),
					Class_ID(0x500513af, 0x7f6267e8));
		}
	}
}
...
INT_PTR CALLBACK addLayerProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		{
			SendDlgItemMessage(hDlg, IDC_LAYERS, CB_ADDSTRING, 0, (LPARAM)(LPCTSTR)info->layerNames[i]->data());
		}
		...
		case IDC_LAYERS:
			...
			case CBN_DROPDOWN:
			{
				...
				SetWindowPos(comboCtl, NULL, 0, 0, rect.w() - 1, GetListHieght(comboCtl),
						SWP_NOZORDER | SWP_NOMOVE | SWP_NOREDRAW | SWP_NOACTIVATE);
				SendMessage(
						comboCtl, CB_SETEDITSEL, 0, MAKELPARAM(0, 0)); // this sets the edited text selection to be null
			...
			case CBN_SELCHANGE:
			{ // select a new item from the combo box
			...
			case CBN_EDITUPDATE:
			{
				TSTR* newName = new TSTR();
				GetWindowText((HWND)lParam, *newName);
...
public:
	...
	Class_ID ClassID()
	{
		return Class_ID(LAYERDLG_CLASS_ID, 0);
	}
...
public:
	...
	{
		GetMLCMan()->NotifySelectionChanged();
	}
	void Redo()
	{
		GetMLCMan()->NotifySelectionChanged();
...
class MainLayerControlRestore : public RestoreObj
{
public:
	MainLayerControl* mLC;
	...
	MainLayerControlRestore(MainLayerControl* c)
	{
		mLC = c;
		ulayers = mLC->layers;
...
{
public:
	LayerControl* cont = nullptr;
	Tab<int> umLCs, rmLCs; // the mains this guy belongs to.
	Tab<Control*> uconts, rconts; // actually controls that make it up
	int uactive, ractive;
	Tab<bool> uFrozenFlags, rFrozenFlags;
	...
	LayerControlRestore(LayerControl* c)
	{
		...
		umLCs = cont->mLCs;
		uactive = cont->active;
		uFrozenFlags = cont->mbInternalFrozenFlags;
	}
	void Restore(int isUndo)
	{
		rconts = cont->conts;
		rmLCs = cont->mLCs;
		ractive = cont->active;
		rFrozenFlags = cont->mbInternalFrozenFlags;

		cont->conts = uconts;
		cont->mLCs = umLCs;
		cont->active = uactive;
		cont->mbInternalFrozenFlags = uFrozenFlags;
	...
	void Redo()
	{
		...
		cont->mLCs = rmLCs;
		cont->active = ractive;
		cont->mbInternalFrozenFlags = rFrozenFlags;
		cont->NotifyDependents(FOREVER, 0, REFMSG_CHANGE);
		cont->NotifyDependents(FOREVER, 0, REFMSG_SUBANIM_STRUCTURE_CHANGED);
	}
};
...
public:
	...
	TSTR Description()
	{
		return _T("Layer control remove item");
	}
};
...
class MonitorNodesEleRestore : public RestoreObj
{

public:
	MainLayerControl* cont;
	int which;

	MonitorNodesEleRestore(MainLayerControl* cont, int which)
			: cont(cont)
			, which(which)
	{
	}
	void Restore(int isUndo)
	{
		ReferenceTarget* rt = nullptr;
		cont->monitorNodes.Insert(which, 1, &rt);
	}
	void Redo()
	{
		cont->monitorNodes.Delete(which, 1);
	}
};
...
class LayerOutputControl
		: public Control
		, public IUnReplaceableControl
{
public:
	LayerOutputControl()
			: paramID(-1)
			, lC(NULL){};
	LayerOutputControl(LayerControl* l, int id)
			: paramID(id)
			, lC(l){};
	~LayerOutputControl()
	{
	...
	BOOL CanInstanceController()
	{
		return FALSE;
	} // in theory only for tranform but you never know
	RefResult NotifyRefChanged(
			const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
	{
		return REF_SUCCEED;
	...
	{
		if (id == I_UNREPLACEABLECTL)
			return (IUnReplaceableControl*)this;
		else
	...
	Class_ID ClassID()
	{
		return LAYEROUTPUT_CONTROL_CLASS_ID;
	}
	SClass_ID SuperClassID()
	{
	...
	{
		s = localized ? GetString(IDS_LAYEROUTPUT_CLASS_NAME) : _T("Layer Output");
	}
...
public:
	...
	{
		return new LayerOutputControl();
	}
	const TCHAR* ClassName()
	{
		return GetString(IDS_LAYEROUTPUT_CLASS_NAME);
	}
	const TCHAR* NonLocalizedClassName()
	{
		return _T("Layer Output");
	}
	SClass_ID SuperClassID()
	{
		return CTRL_FLOAT_CLASS_ID;
...
public:
	LayerControl* lC;
	int howMany;
	AnimHandle lcHandle; // AnimHandle of the LayerControl

	LayerControlPLCB(LayerControl* e, int hMany)
	{
		lC = e;
		howMany = hMany;
		lcHandle = lC ? Animatable::GetHandleByAnim(lC) : 0;
	}

	// MAXX-7521: bumping up priority of this callback over default value 5 means that
	// this gets executed before some other PLCBs that were deleting the MainLayerControl
	// owning the LayerControl for this PLCB
	...
	{
		// MAXX-7521: make sure nothing has deleted the LayerControl since the PLCB creation
		DbgAssert(Animatable::GetAnimByHandle(lcHandle) == lC);
		if (Animatable::GetAnimByHandle(lcHandle) != lC)
			return;
		...
		{
			int param_index = i - kX + 1;
			LayerOutputControl* control = (LayerOutputControl*)lC->pblock->GetControllerByIndex(param_index, 0);
			if (control)
			{
				control->lC = lC;
...
{
public:
	FloatLayerControl(BOOL loading)
			: LayerControl(loading)
	{
		Init();
	}
	FloatLayerControl()
	...
	void GetOutputValue(TimeValue t, void* val, Interval& valid) override;
	void SetValue(TimeValue t, void* val, int commit, GetSetMethod method) override;
	LayerControl* DerivedClone() override;
	void Init();

	Class_ID ClassID() override
	{
		return (FLOATLAYER_CONTROL_CLASS_ID);
	}
	...
	void GetClassName(MSTR& s, bool localized) const override
	{
		s = localized ? FLOATLAYER_CNAME : _T("Float Layer");
	}
};
...
{
public:
	explicit RotationLayerControl(BOOL loading)
			: LayerControl(loading)
	{
		Init();
	}
	RotationLayerControl()
	{
		Init();
	}
	...
	Class_ID ClassID() override
	{
		return (ROTLAYER_CONTROL_CLASS_ID);
	}
	SClass_ID SuperClassID() override
	{
	...
	void GetClassName(MSTR& s, bool localized) const override
	{
		s = localized ? ROTLAYER_CNAME : _T("Rotation Layer");
	}

	// overriden by RotationLayerControl since every time we change conts we need to make sure to see if we can blend
	// the conts 3 floats as a euler or as a quat.

	BOOL AssignController(Animatable* control, int subAnim) override;
	void AddLayer(Control* control, int newMLC) override;
	void DeleteLayerInternal(int active) override;
	void CopyLayerInternal(int active) override;
	Control* PasteLayerInternal(int active) override;
	void CollapseLayerInternal(int index) override;

	void CheckOkayToBlendAsEulers();
	// not used right now.. if we can blend it, we can collapse it as euler (though maybe not as per key frame)BOOL
	// CheckOkayToCollapseAsEuler();
	BOOL okayToBlendEulers; // this is set everytime a controller is added or deleted or assigned. It is then used by
							// GetValue.
};
...
public:
	...
	{
		return FLOATLAYER_CNAME;
	}
	const TCHAR* NonLocalizedClassName() override
	{
		return _T("Float Layer");
	}
	SClass_ID SuperClassID() override
	{
		return CTRL_FLOAT_CLASS_ID;
...
static ParamBlockDesc2 layer_paramblk(0, _T("Parameters"), 0, &floatLayerCD, P_AUTO_CONSTRUCT, PBLOCK_INIT_REF_ID,
		// params
		kLayerCtrlAverage, _T("average"), TYPE_BOOL, 0, IDS_AF_LIST_AVERAGE, p_default, FALSE, p_end,
		kX, _T("exposedValue"), TYPE_FLOAT, P_ANIMATABLE | P_TRANSIENT, IDS_VALUE, p_nonLocalizedName, _T("Value"), p_end,
		p_end);

static FPInterfaceDesc layerControlInterface(LAYER_CONTROLLER_INTERFACE, _T("layer"), 0, &floatLayerCD, FP_MIXIN,
	...
	ILayerControl::getLayerMute, _T("getLayerMute"), 0, TYPE_bool, 0, 1, 
		_T("index"), 0, TYPE_INDEX, f_validator, &layerValidator,
	ILayerControl::setLayerMute, _T("setLayerMute"), 0, TYPE_VOID, 0, 2, 
		_T("listIndex"), 0, TYPE_INDEX, f_validator, &layerValidator, 
		_T("mute"), 0, TYPE_bool,
...
RefResult LayerControl::NotifyRefChanged(
		const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
	...
	if (settingLock == false)
	{
		...
		case REFMSG_CHANGE:
			...
			if (pblock)
			{
				...
				if (inNotify == false)
				{
					...
					{
						int param_index = i - kX + 1;
						auto* control = (LayerOutputControl*)pblock->GetControllerByIndex(param_index, 0);
						if (control)
		...
		// may need to unlock or lock the correct main layer controller.
...
void LayerControl::CheckForLock(RefTargetHandle hTarget, bool lockVal) const
{
	...
	{
		Control* c = conts[i];
		if (static_cast<RefTargetHandle>(c) == hTarget)
		{
			lockFound = true;
			break;
		}
	...
	{
		MainLayerControl* mlc = GetMLCMan()->GetNthMLC(mLCs[i]);
		if (mlc)
...
LayerControl::LayerControl(const LayerControl& ctrl)
{
	...
	bool frozenFlag;
	mbInternalFrozenFlags.ZeroCount();
	for (int i = 0; i < ctrl.mbInternalFrozenFlags.Count(); ++i)
	{
		frozenFlag = ctrl.mbInternalFrozenFlags[i];
		mbInternalFrozenFlags.Append(1, &frozenFlag);
	}
...
{
	// make sure we NULL the pointer back to the layer control
	if (pblock)
	{
		int stop = GetEndOfOutputControls();
		for (int i = kX; i <= stop; ++i)
		{
			int index = i - kX + 1;
			auto* control = dynamic_cast<LayerOutputControl*>(pblock->GetControllerByIndex(index, 0));
			if (control)
				control->lC = nullptr;
		}
	}
...
void LayerControl::CreateController(int i)
{
	...
	pblock->SetControllerByIndex(i - kX + 1, 0, c, FALSE);
}
...
LayerControl& LayerControl::operator=(const LayerControl& ctrl)
{
	...
	active = ctrl.active;
	mLocked = ctrl.mLocked;
	needToSetUpParentNode = ctrl.needToSetUpParentNode; // wouldn't be cloned in this case, but may be getting set from
														// somebody who was cloned
	mLCs.ZeroCount();
	for (int i = 0; i < ctrl.mLCs.Count(); ++i)
	{
		int val = ctrl.mLCs[i];
		mLCs.Append(1, &val);
	}
	...
	for (int i = 0; i < ctrl.mbInternalFrozenFlags.Count(); ++i)
	{
		bool frozenFlag = ctrl.mbInternalFrozenFlags[i];
		mbInternalFrozenFlags.Append(1, &frozenFlag);
	}
	return *this;
}
	...
	if (needToSetUpParentNode)
	{
		...
		for (int z = 0; z < dep.nodes.Count(); ++z)
		{
			...
			for (int i = 0; i < mLCs.Count(); ++i)
			{
				...
				MainLayerControl* mLC = GetMLCMan()->GetNthMLC(mLCs[i]);
				if (mLC) // post save/load callback scripts can muck with the layers so it may not exist and needs to be
						 // checked
					mLC->AddNode(node);
			}
		}
...
RefTargetHandle LayerControl::Clone(RemapDir& remap)
{
	...
	for (int i = 0; i < mbInternalFrozenFlags.Count(); ++i)
	{
		bool frozenFlag = mbInternalFrozenFlags[i];
		ctrl->mbInternalFrozenFlags.Append(1, &frozenFlag);
	}
	ctrl->active = active;
	ctrl->mLocked = mLocked;

	ctrl->needToSetUpParentNode = true; // was cloned so inside ::GetValue we need to find it's node and add it to the
										// MainLayerControl Manager.
	return ctrl;
}
...
void LayerControl::AddLayer(Control* newControl, int newMLC)
{
	...
	bool bInternalFrozen = false;
	mbInternalFrozenFlags.Append(1, &bInternalFrozen);
	if (theHold.Holding())
	{
		theHold.Put(rest2);
...
BOOL LayerControl::InheritsParentTransform()
{
	...
	for (int i = 0; i <= GetLayerActive(); ++i)
...
Animatable* LayerControl::SubAnim(int i)
{
	...
	if (man->GetFilterActiveOnlyTrackView() == FALSE)
	{
		...
		{
			MainLayerControl* mLC = GetMLCMan()->GetNthMLC(mLCs[i - conts.Count()]);
			if (mLC && mLC->pblock)
	...
	else
	{
		...
		{
			MainLayerControlManager* mLCMan = GetMLCMan();
			if (mLCMan && active < mLCs.Count())
			{
				MainLayerControl* mLC = mLCMan->GetNthMLC(mLCs[active]);
				if (mLC && mLC->pblock)
					return mLC->GetWeightControl();
			}
		}
...
TSTR LayerControl::SubAnimName(int i, bool localized)
{
	TSTR name;
	...
	if (man->GetFilterActiveOnlyTrackView() == FALSE)
	{
		...
		else if (i < conts.Count() * 2)
		{
			...
			if (mLC && !mLC->GetName().isNull())
			{
				name = (localized ? TSTR(GetString(IDS_AF_LIST_WEIGHT)) : TSTR(_T("Weight"))) + _T(": ") + mLC->GetName();
			}
			else if (conts[i])
			{
				TSTR cname;
				conts[i]->GetClassName(cname, localized);
				name = (localized ? TSTR(GetString(IDS_AF_LIST_WEIGHT)) : TSTR(_T("Weight"))) + _T(": ") + cname;
			}
		}
		else if (i == 2 * conts.Count())
			name = localized ? GetString(IDS_MZ_OUTPUT_TRACK) : _T("Output Track");
	}
	else
	{
		if (i == 0)
		{
			if (active >= mLCs.Count())
				return name;
			MainLayerControl* mLC = GetMLCMan()->GetNthMLC(mLCs[active]);
			if (mLC && !mLC->GetName().isNull())
			{
				name = mLC->GetName();
			}
			...
			{
				conts[active]->GetClassName(name, localized);
			}
		}
		else if (i == 1)
		{
			...
			if (mLC && !mLC->GetName().isNull())
			{
				name = mLC->GetName();
				name += _T(" ");
				name += localized ? GetString(IDS_AF_LIST_WEIGHT) : _T("Weight");
			}
			else if (conts[active])
			{
				conts[active]->GetClassName(name, localized);
...
TSTR LayerControl::ControllerName(int i)
{
	...
	if (i < conts.Count())
	{
		...
		{
			conts[i]->GetClassName(name, true);
		}
	}
	return name;
}
...
BOOL LayerControl::AssignController(Animatable* control, int subAnim)
{
	...
	if (GetLocked() == true ||
			(SubAnim(subAnim) && GetLockedTrackInterface(SubAnim(subAnim)) &&
					GetLockedTrackInterface(SubAnim(subAnim))->GetLocked() == true))
		return FALSE;
	if (GetMLCMan()->GetFilterActiveOnlyTrackView() == FALSE)
	{
		...
		{
			MainLayerControl* mLC = GetMLCMan()->GetNthMLC(mLCs[subAnim - conts.Count()]); //#867594 MZ
			Control* c = GetControlInterface(control);
			if (c != NULL &&
					mLC != NULL) // mLC may be NULL when closing max or a file.. it may have been deleted already.
	...
	else
	{
		...
		{
			MainLayerControl* mLC = GetMLCMan()->GetNthMLC(mLCs[active]); //#867594 MZ
			Control* c = GetControlInterface(control);
			if (c != NULL &&
					mLC != NULL) // mLC may be NULL when closing max or a file.. it may have been deleted already
				mLC->SetWeightControl(c);
...
#define LOCK_CHUNK 0x2535 // the lock value, used by individual layer controls and the main lc.
#define INTERNALFROZEN_CHUNK 0x2555 // the internal frozen value, used as flag in the heirarchy of XRref controller.
...
IOResult LayerControl::Save(ISave* isave)
{
	...
	for (int i = 0; i < mbInternalFrozenFlags.Count(); i++)
	{
		isave->BeginChunk(INTERNALFROZEN_CHUNK);
		bInternalFrozen = mbInternalFrozenFlags[i];
		isave->Write(&bInternalFrozen, sizeof(bool), &nb);
		isave->EndChunk();
	}
...
IOResult LayerControl::Load(ILoad* iload)
{
	...
	if (ClassID() == POINT3LAYER_CONTROL_CLASS_ID || ClassID() == POSLAYER_CONTROL_CLASS_ID ||
			ClassID() == ROTLAYER_CONTROL_CLASS_ID || ClassID() == SCALELAYER_CONTROL_CLASS_ID)
	{
	...
	while (IO_OK == (res = iload->OpenChunk()))
	{
		...
		break;
		case INTERNALFROZEN_CHUNK:
		{
			bool bInternalFrozen;
			res = iload->Read(&bInternalFrozen, sizeof(bInternalFrozen), &nb);
			mbInternalFrozenFlags.Append(1, &bInternalFrozen);
			break;
		}
		}
...
void LayerControl::SetLayerActiveInternal(int a)
{
	if (dlg)
	{
		if (conts.Count())
		{
			conts[active]->EndEditParams(dlg->ip, END_EDIT_REMOVEUI, conts[a]);
		}
		if (conts.Count())
		{
			conts[a]->BeginEditParams(dlg->ip, paramFlags, conts[active]);
		}
	}
...
void LayerControl::SetLayerName(int index, TSTR name)
{
	if (index < 0 || index >= mLCs.Count())
		return;
	MainLayerControl* mLC = GetMLCMan()->GetNthMLC(mLCs[index]);
...
void LayerControl::DeleteLayerInternal(int a)
{
	...
	conts.Delete(a, 1);
	mLCs.Delete(a, 1);
	mbInternalFrozenFlags.Delete(a, 1);

	if (active > a)
		active--;
	if (active > conts.Count() - 1)
...
BOOL CollapseSameControllerType(F* values, Interval range, Tab<TimeValue>& ontoKeyTimes, Tab<TimeValue>& fromKeyTimes,
		Control* onto, Control* from)
{
...
static BOOL GetKeyTimesAndRange(
		Control* onto, Control* from, Tab<TimeValue>& ontoKeyTimes, Tab<TimeValue>& fromKeyTimes, Interval& range)
{
	Interval interval = FOREVER;
...
{
	if (onto->GetXController()->IsKeyable() == FALSE ||
			onto->GetXController()->ClassID() != from->GetXController()->ClassID() ||
		...
		return FALSE;
	if (onto->GetYController()->IsKeyable() == FALSE ||
			onto->GetYController()->ClassID() != from->GetYController()->ClassID() ||
		...
		return FALSE;
	if (onto->GetZController()->IsKeyable() == FALSE ||
			onto->GetZController()->ClassID() != from->GetZController()->ClassID() ||
	...
	subOnto[0] = onto->GetXController();
	subFrom[0] = from->GetXController();
	...
	subOnto[1] = onto->GetYController();
	subFrom[1] = from->GetYController();
	...
	subOnto[2] = onto->GetZController();
	subFrom[2] = from->GetZController();
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		{
			what = CollapseSameControllerType<ILinFloatKey, float>(
					fValues[i], bigRange, ontoKeyTimes[i], fromKeyTimes[i], subOnto[i], subFrom[i]);
			if (what == FALSE)
				break;
		}
		else if (subOnto[i]->ClassID() == Class_ID(HYBRIDINTERP_FLOAT_CLASS_ID, 0))
		{
			what = CollapseSameControllerType<IBezFloatKey, float>(
					fValues[i], bigRange, ontoKeyTimes[i], fromKeyTimes[i], subOnto[i], subFrom[i]);
			if (what == FALSE)
				break;
		}
		else if (subOnto[i]->ClassID() == Class_ID(TCBINTERP_FLOAT_CLASS_ID, 0))
		{
			...
			what = CollapseSameControllerType<ITCBFloatKey, float>(
					fValues[i], bigRange, ontoKeyTimes[i], fromKeyTimes[i], subOnto[i], subFrom[i]);
			if (what == FALSE)
				break;
		}
		...
		{
			what = CollapseSameControllerType<IBoolFloatKey, float>(
					fValues[i], bigRange, ontoKeyTimes[i], fromKeyTimes[i], subOnto[i], subFrom[i]);
			if (what == FALSE)
				break;
		}
...
BOOL LayerControl::CollapseKeys(int index, Control* onto, Control* from)
{
	...
	if (onto->SuperClassID() == CTRL_FLOAT_CLASS_ID)
	{
		...
		{
			what = CollapseSameControllerType<ILinFloatKey, float>(
					values, range, ontoKeyTimes, fromKeyTimes, onto, from);
		}
	...
	else if (onto->SuperClassID() == CTRL_POSITION_CLASS_ID || onto->SuperClassID() == CTRL_POINT3_CLASS_ID)
	{
		...
		{
			what = CollapseSameControllerType<ILinPoint3Key, Point3>(
					values, range, ontoKeyTimes, fromKeyTimes, onto, from);
			//	OutputLinearPositionControl(realNumKeys,keyControl,samplesNode,range);
		}
		else if (onto->ClassID() == Class_ID(HYBRIDINTERP_POSITION_CLASS_ID, 0) ||
				onto->ClassID() == Class_ID(HYBRIDINTERP_COLOR_CLASS_ID, 0) ||
				onto->ClassID() == Class_ID(HYBRIDINTERP_POINT3_CLASS_ID, 0))
		{
			what = CollapseSameControllerType<IBezPoint3Key, Point3>(
					values, range, ontoKeyTimes, fromKeyTimes, onto, from);
		...
		{
			what = CollapseSameControllerType<ITCBPoint3Key, Point3>(
					values, range, ontoKeyTimes, fromKeyTimes, onto, from);
	...
	else if (onto->SuperClassID() == CTRL_SCALE_CLASS_ID)
	{
		...
		{
			what = CollapseSameControllerType<ILinScaleKey, ScaleValue>(
					values, range, ontoKeyTimes, fromKeyTimes, onto, from);
		...
		{
			what = CollapseSameControllerType<IBezScaleKey, ScaleValue>(
					values, range, ontoKeyTimes, fromKeyTimes, onto, from);
		...
		{
			what = CollapseSameControllerType<ITCBScaleKey, ScaleValue>(
					values, range, ontoKeyTimes, fromKeyTimes, onto, from);
	...
	else if (onto->SuperClassID() == CTRL_POINT4_CLASS_ID)
	{
		...
		if (onto->ClassID() == Class_ID(HYBRIDINTERP_POINT4_CLASS_ID, 0) ||
				onto->ClassID() == Class_ID(HYBRIDINTERP_FRGBA_CLASS_ID, 0))
		{
			what = CollapseSameControllerType<IBezPoint4Key, Point4>(
					values, range, ontoKeyTimes, fromKeyTimes, onto, from);
		...
		{
			what = CollapseSameControllerType<ITCBPoint4Key, Point4>(
					values, range, ontoKeyTimes, fromKeyTimes, onto, from);
		}
		if (values)
		{
...
void LayerControl::CollapseLayerInternal(int index)
{
	...
	for (i = index - 1; i > -1; --i)
	{
		MainLayerControl* mLC = man->GetNthMLC(mLCs[i]);
		if (mLC && mLC->mute == false)
			break;
	}
	if (i == -1)
	...
	if (onto->GetInterface(I_MIXERDRIVENINTERFACE))
	{
...
void LayerControl::DisableLayer()
{
	...
	if (node && GetLocked() == false)
	{
		...
		Tab<MainLayerControlManager::LCInfo> existingLayers;
			...
			if (mLC)
			{
				for (int k = 0; k < existingLayers.Count(); ++k)
				{
					...
					for (f = 0; f < lC->mLCs.Count(); ++f)
					{
						if (lC->mLCs[f] == mLCs[i])
						{
							// okay this main layer control still exists
					...
					if (f == lC->mLCs.Count())
					{
						auto* mLCRestore = new MainLayerControlRestore(mLC);
						int j;
						for (j = mLC->monitorNodes.Count() - 1; j > -1; --j)
						{
							...
							{
								INodeMonitor* nm =
										static_cast<INodeMonitor*>(mLC->monitorNodes[j]->GetInterface(IID_NODEMONITOR));
								if (nm->GetNode() == node)
									break;
							}
						}
						...
						{
							delete mLCRestore; // Didn't end up using this.
						}
					}
				}
			}
...
bool LayerControl::GetLayerMute(int index)
{
	if (index < 0 || index >= mLCs.Count())
		return false;
	auto* mLC = GetMLCMan()->GetNthMLC(mLCs[index]);
	if (mLC)
		return mLC->mute;
...
void LayerControl::SetLayerLocked(int index, bool locked)
{
	if (index < 0 || index >= mLCs.Count())
		return;
	GetMLCMan()->SetLayerLocked(mLCs[index], locked); // calling this makes sure UI get's set up correctly
}
...
void LayerControl::SetLayerInternalFrozen(int index, bool frozen)
{
	if (index < 0 || index >= mbInternalFrozenFlags.Count())
		return;

	mbInternalFrozenFlags[index] = frozen;
}

bool LayerControl::GetLayerInternalFrozen(int index)
{
	if (index < 0 || index >= mbInternalFrozenFlags.Count())
		return false;

	return mbInternalFrozenFlags[index];
}
...
AssignControllerRestore::AssignControllerRestore(BOOL p)
{
	parity = p;
}
...
TSTR AssignControllerRestore::Description()
{
	return _T("Assign Controller");
}
...
void LayerControl::DisableLayer(Animatable* client, int subNum)
{
	...
	for (int i = mLCs.Count() - 1; i > -1; --i)
	{
		if (MainLayerControl* mLC = man->GetNthMLC(mLCs[i]))
		{
			for (int j = 0; j < mLC->layers.Count(); ++j)
			{
				...
				if (what == this)
				{
					MainLayerControlRestore* rest = NULL;
					if (theHold.Holding())
						rest = new MainLayerControlRestore(mLC);
...
static MaxSDK::Array<LayerCtrlWindow> layerCtrlWindows;

static void RegisterLayerCtrl(HWND hWnd, HWND hParent, Control* cont)
{
	LayerCtrlWindow rec(hWnd, hParent, cont);
	layerCtrlWindows.append(rec);
}

static void UnRegisterLayerCtrlWindow(HWND hWnd)
{
	for (int i = 0; i < layerCtrlWindows.length(); i++)
	{
		if (hWnd == layerCtrlWindows[i].hWnd)
		{
			layerCtrlWindows.removeAt(i);
			return;
		}
	}
}
...
static HWND FindOpenLayerCtrl(HWND hParent, Control* cont)
{
	for (int i = 0; i < layerCtrlWindows.length(); i++)
	{
		if (hParent == layerCtrlWindows[i].hParent && cont == layerCtrlWindows[i].cont)
		{
		...
		TimeValue t, ParamDimensionBase* dim, const TCHAR* pname, HWND hParent, IObjParam* ip, DWORD flags)
{
	...
	LayerControlTrackDlg* dlg = new LayerControlTrackDlg(ip, this);
	dlg->Init(hParent);
...
{
	BOOL average = FALSE;
	pblock->GetValue(kLayerCtrlAverage, 0, average);
	if (average)
	{
...
static int eulerIDs[] = { IDS_EULERTYPE0, IDS_EULERTYPE1, IDS_EULERTYPE2, IDS_EULERTYPE3, IDS_EULERTYPE4,
	IDS_EULERTYPE5, IDS_EULERTYPE6, IDS_EULERTYPE7, IDS_EULERTYPE8 };
	...
	if (cont->dlg == this)
	{
		if (cont->conts.Count())
		{
			if (cont->conts[cont->active])
				cont->conts[cont->active]->EndEditParams(ip, END_EDIT_REMOVEUI, NULL);
		}
	}
...
void LayerControlDlg::Reset(IObjParam* i, LayerControl* c)
{
	if (cont->dlg == this)
	{
		...
		{
			if (cont->conts[cont->active])
				cont->conts[cont->active]->EndEditParams(i, END_EDIT_REMOVEUI, NULL);
		}
		if (c->conts.Count())
		{
			if (cont->conts[cont->active])
				c->conts[cont->active]->BeginEditParams(i, c->paramFlags, NULL);
		}
	}
	ReplaceReference(0, c);
	ip = i;
...
void LayerControlDlg::Init(HWND hParent)
{
	...
	if (cont->dlg == this)
	{
		...
		{
			if (cont->conts[cont->active])
				cont->conts[cont->active]->BeginEditParams(ip, cont->paramFlags, NULL);
		}
	}
}
...
void LayerControlDlg::UpdateList()
{
	...
	for (i = 0; i < cont->conts.Count(); i++)
	{
		...
		if (ListView_GetItemCount(hList) <= i)
		{
			...
			item.iItem = i;
			item.iSubItem = 0;
			item.pszText = const_cast<TCHAR*>(name.data());
			...
			item.pszText = const_cast<TCHAR*>(name.data());
			item.cchTextMax = static_cast<int>(_tcslen(name));
			ListView_SetItemText(hList, i, 1, const_cast<TCHAR*>(name.data()));
		}
		else
		{
			ListView_SetItemText(hList, i, 0, const_cast<TCHAR*>(name.data()));
			name.printf(_T("%.1f"), cont->GetLayerWeight(i, GetCOREInterface()->GetTime()) * 100.0f);
			ListView_SetItemText(hList, i, 1, const_cast<TCHAR*>(name.data()));
		}
	}
	int itemCt = ListView_GetItemCount(hList);
	for (int x = itemCt; x > i; x--)
...
void LayerControlDlg::StartEdit(int a)
{
	if (cont->dlg)
	{
		if (cont->conts.Count())
		{
			cont->conts[a]->BeginEditParams(ip, cont->paramFlags, cont->conts[a]);
		}
	}
}
...
void LayerControlDlg::EndEdit(int a)
{
	if (cont->dlg)
	{
		if (cont->conts.Count())
		{
			cont->conts[a]->EndEditParams(ip, END_EDIT_REMOVEUI, NULL);
		}
	}
}
...
void LayerControlDlg::SetButtonStates()
{
	...
	if (mLC)
	{
		...
		iName->Enable();
		iName->SetText(cont->GetLayerName(sel).data());

		float weight = cont->GetLayerWeight(sel, GetCOREInterface()->GetTime());
		iWeight->Enable();
		iWeight->SetValue(weight * 100.0f, 0);

		iWeight->SetKeyBrackets(mLC->pblock->KeyFrameAtTimeByID(kLayerCtrlWeight, GetCOREInterface()->GetTime(), sel));
	...
	BOOL average = FALSE;
	cont->pblock->GetValue(kLayerCtrlAverage, 0, average);
...
void LayerControlDlg::WMCommand(int id, int notify, HWND hCtrl)
{
	...
	case IDC_EULER_ORDER_X:
		...
		{
			int res = SendDlgItemMessage(hWnd, IDC_EULER_ORDER_X, CB_GETCURSEL, 0, 0);
			if (res != CB_ERR)
			{
				RotationLayerControl* rLC = static_cast<RotationLayerControl*>(cont);
	...
	case IDC_EULER_ORDER_Y:
		if (notify == CBN_SELCHANGE)
		{
			int res = SendDlgItemMessage(hWnd, IDC_EULER_ORDER_Y, CB_GETCURSEL, 0, 0);
			if (res != CB_ERR)
			{
				RotationLayerControl* rLC = static_cast<RotationLayerControl*>(cont);
	...
	case IDC_EULER_ORDER_Z:
		if (notify == CBN_SELCHANGE)
		{
			int res = SendDlgItemMessage(hWnd, IDC_EULER_ORDER_Z, CB_GETCURSEL, 0, 0);
			if (res != CB_ERR)
			{
				RotationLayerControl* rLC = static_cast<RotationLayerControl*>(cont);
...
RefResult LayerControlDlg::NotifyRefChanged(
		const Interval& iv, RefTargetHandle rtarg, PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message)
	{
	case REFMSG_CHANGE:
...
static INT_PTR CALLBACK LayerControlDlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	LayerControlDlg* ld = DLGetWindowLongPtr<LayerControlDlg*>(hWnd);

	static BOOL undoHeldHere =
			FALSE; // AF -- this flag is used to trigger an undo begin in the even of a WM_CUSTEDIT_ENTER
	...
	case WM_INITDIALOG:
	{
		...
		if (ld->cont->SuperClassID() == CTRL_ROTATION_CLASS_ID)
		{
			...
			{
				SendDlgItemMessage(hWnd, IDC_EULER_ORDER_X, CB_ADDSTRING, 0, (LPARAM)GetString(eulerIDs[i]));
			}
			...
			for (i = 0; i < NUM_SUPPORTED_EULERS; i++)
			{
				SendDlgItemMessage(hWnd, IDC_EULER_ORDER_Y, CB_ADDSTRING, 0, (LPARAM)GetString(eulerIDs[i]));
			}
			val = rLC->GetEulerYAxisOrder();
			SendDlgItemMessage(hWnd, IDC_EULER_ORDER_Y, CB_SETCURSEL, val, 0);
			...
			for (i = 0; i < NUM_SUPPORTED_EULERS; i++)
			{
				SendDlgItemMessage(hWnd, IDC_EULER_ORDER_Z, CB_ADDSTRING, 0, (LPARAM)GetString(eulerIDs[i]));
			}
			val = rLC->GetEulerZAxisOrder();
			SendDlgItemMessage(hWnd, IDC_EULER_ORDER_Z, CB_SETCURSEL, val, 0);
		...
		case IDC_CONTROLLER_WEIGHT_LIST:
		{
			...
			case LVN_ITEMCHANGED:
			{
				...
				if ((((NMLISTVIEW*)lParam)->uNewState & LVIS_SELECTED) ||
						(((NMLISTVIEW*)lParam)->uOldState & LVIS_SELECTED))
					ld->SetButtonStates();
			}
				return 0;
			case NM_DBLCLK:
				if (((LPNMITEMACTIVATE)lParam)->iItem >= 0 &&
						((LPNMITEMACTIVATE)lParam)->iItem < ld->cont->conts.Count())
				{
	...
	case WM_DESTROY:
		// We're about to delete ld; make sure to zero out the stored ld pointer in the window's long pointer
...
{
	TSTR name;
	cont->GetClassName(name, true);
	if (cont->SuperClassID() != CTRL_ROTATION_CLASS_ID)
	{
		return ip->AddRollupPage(
				hInstance, MAKEINTRESOURCE(IDD_LAYERPARAMS_MOTION), LayerControlDlgProc, name, (LPARAM)this);
	}
	else
	{
		return ip->AddRollupPage(
				hInstance, MAKEINTRESOURCE(IDD_ROTLAYERPARAMS_MOTION), LayerControlDlgProc, name, (LPARAM)this);
	}
}
...
HWND LayerControlTrackDlg::CreateWin(HWND hParent)
{
	if (cont->SuperClassID() != CTRL_ROTATION_CLASS_ID)
	{
		return CreateDialogParam(
				hInstance, MAKEINTRESOURCE(IDD_LAYERPARAMS_TRACK), hParent, LayerControlDlgProc, (LPARAM)this);
	}
	else
	{
		return CreateDialogParam(
				hInstance, MAKEINTRESOURCE(IDD_ROTLAYERPARAMS_TRACK), hParent, LayerControlDlgProc, (LPARAM)this);
	}
}
...
public:
	BOOL non;
	ReferenceMaker* me;
	CheckForNonListDlg(ReferenceMaker* m)
	{
		non = FALSE;
		me = m;
	}
	int proc(ReferenceMaker* rmaker)
	{
		if (rmaker == me)
			return DEP_ENUM_CONTINUE;
		if (rmaker->SuperClassID() != REF_MAKER_CLASS_ID && rmaker->ClassID() != Class_ID(LAYERDLG_CLASS_ID, 0))
		{
			non = TRUE;
...
void FloatLayerControl::GetValue(TimeValue t, void* val, Interval& valid, GetSetMethod method)
{
	if (needToSetUpParentNode) // not completely optimal but it guarentees that we will recheck and find our parent node
							   // and add to the main list after we are cloned
		SetParentNodesOnMLC();
...
void FloatLayerControl::GetOutputValue(TimeValue t, void* val, Interval& valid)
{
	float* v = (float*)val;
	float weight;
	...
	*v = 0.0f;
	...
	// CAL-10/18/2002: conts.Count() and pblock->Count(pblock->IndextoID(kLayerCtrlWeight)) might go out of sync while
	// undo. (459225)
...
void FloatLayerControl::SetValue(TimeValue t, void* val, int commit, GetSetMethod method)
{
	if (!conts.Count() || GetLocked() == true)
		return;
	float weight;

	MainLayerControl* pMainLayerControl = GetMLCMan()->GetNthMLC(mLCs[active]);
	...
	if (method == CTRL_ABSOLUTE)
	{
		...
		weight = pMainLayerControl ? pMainLayerControl->GetWeight(t, valid) : 1.0f;
		...
		for (i = 0; i < active; i++)
		{
			...
			if (auto *mLC = GetMLCMan()->GetNthMLC(mLCs[i]))
			{
				weight = mLC->GetWeight(t, valid);
	...
	{
		assert(active >= 0);
		if (pMainLayerControl)
		{
			weight = pMainLayerControl->GetWeight(t, valid);
			*((float*)val) *= weight;
		}
		if (conts[active])
			conts[active]->SetValue(t, val, commit, method);
...
{
public:
	Point3LayerControl(BOOL loading)
			: LayerControl(loading)
	{
		Init();
	}
	...
	Class_ID ClassID()
	{
		return (POINT3LAYER_CONTROL_CLASS_ID);
	}
	SClass_ID SuperClassID()
	{
	...
	void GetClassName(MSTR& s, bool localized) const override
	{
		s = localized ? POINT3LAYER_CNAME : _T("Point3 Layer");
	}

	void SetUpPblockOnClone(LayerControl* newObj);
};

void Point3LayerControl::GetValue(TimeValue t, void* val, Interval& valid, GetSetMethod method)
{
	if (needToSetUpParentNode) // not completely optimal but it guarentees that we will recheck and find our parent node
							   // and add to the main list after we are cloned
		SetParentNodesOnMLC();
...
void Point3LayerControl::GetOutputValue(TimeValue t, void* val, Interval& valid)
{
	Point3* v = (Point3*)val;
	float weight;
...
void Point3LayerControl::SetValue(TimeValue t, void* val, int commit, GetSetMethod method)
{
	if (!conts.Count() || GetLocked() == true)
		return;
	float weight;
	...
	{
		assert(active >= 0);
		if (auto* mLC = GetMLCMan()->GetNthMLC(mLCs[active]))
		{
			weight = mLC->GetWeight(t, valid);
...
public:
	...
	const TCHAR* ClassName()
	{
		return POINT3LAYER_CNAME;
	}
	const TCHAR* NonLocalizedClassName()
	{
		return _T("Point3 Layer");
	}
	SClass_ID SuperClassID()
	{
		return CTRL_POINT3_CLASS_ID;
...
ClassDesc* GetPoint3LayerDesc()
{
	return &point3LayerCD;
}
...
// per instance list controller block
static ParamBlockDesc2 point3_layer_paramblk(0, _T("Parameters"), 0, &point3LayerCD, P_AUTO_CONSTRUCT,
		PBLOCK_INIT_REF_ID,
		// params

		kLayerCtrlAverage, _T("average"), TYPE_BOOL, 0, IDS_AF_LIST_AVERAGE, p_default, FALSE, p_end,

		kX, _T("exposedX"), TYPE_FLOAT, P_ANIMATABLE | P_TRANSIENT, IDS_RB_X, p_nonLocalizedName, _T("X"), p_end, kY,
		_T("exposedY"), TYPE_FLOAT, P_ANIMATABLE | P_TRANSIENT, IDS_RB_Y, p_nonLocalizedName, _T("Y"), p_end, kZ,
		_T("exposedZ"), TYPE_FLOAT, P_ANIMATABLE | P_TRANSIENT, IDS_RB_Z, p_nonLocalizedName, _T("Z"), p_end, p_end);
...
{
public:
	Point4LayerControl(BOOL loading)
			: LayerControl(loading)
	{
		Init();
	}
	...
	Class_ID ClassID()
	{
		return (POINT4LAYER_CONTROL_CLASS_ID);
	}
	SClass_ID SuperClassID()
	{
	...
	void GetClassName(MSTR& s, bool localized) const override
	{
		s = localized ? POINT4LAYER_CNAME : _T("Point4 Layer");
	}
};

void Point4LayerControl::GetValue(TimeValue t, void* val, Interval& valid, GetSetMethod method)
{
	if (needToSetUpParentNode) // not completely optimal but it guarentees that we will recheck and find our parent node
							   // and add to the main list after we are cloned
		SetParentNodesOnMLC();
...
void Point4LayerControl::GetOutputValue(TimeValue t, void* val, Interval& valid)
{
	Point4* v = (Point4*)val;
	float weight;
...
void Point4LayerControl::SetValue(TimeValue t, void* val, int commit, GetSetMethod method)
{
	if (!conts.Count() || GetLocked() == true)
		return;
	float weight;
...
public:
	...
	const TCHAR* ClassName()
	{
		return POINT4LAYER_CNAME;
	}
	const TCHAR* NonLocalizedClassName()
	{
		return _T("Point4 Layer");
	}
	SClass_ID SuperClassID()
	{
		return CTRL_POINT4_CLASS_ID;
...
ClassDesc* GetPoint4LayerDesc()
{
	return &point4LayerCD;
}
...
// per instance list controller block
static ParamBlockDesc2 point4_layer_paramblk(0, _T("Parameters"), 0, &point4LayerCD, P_AUTO_CONSTRUCT,
		PBLOCK_INIT_REF_ID,
		// params
		...
		kX, _T("exposedX"), TYPE_FLOAT, P_ANIMATABLE | P_TRANSIENT, IDS_RB_X, p_nonLocalizedName, _T("X"), p_end, kY,
		_T("exposedY"), TYPE_FLOAT, P_ANIMATABLE | P_TRANSIENT, IDS_RB_Y, p_nonLocalizedName, _T("Y"), p_end, kZ,
		_T("exposedZ"), TYPE_FLOAT, P_ANIMATABLE | P_TRANSIENT, IDS_RB_Z, p_nonLocalizedName, _T("Z"), p_end, kW,
		_T("exposedW"), TYPE_FLOAT, P_ANIMATABLE | P_TRANSIENT, IDS_RB_W, p_nonLocalizedName, _T("W"), p_end, p_end);
...
{
public:
	PositionLayerControl(BOOL loading)
			: LayerControl(loading)
	{
		Init();
	}
	PositionLayerControl()
	{
		Init();
	}
	...
	Class_ID ClassID()
	{
		return (POSLAYER_CONTROL_CLASS_ID);
	}
	SClass_ID SuperClassID()
	{
	...
	void GetClassName(MSTR& s, bool localized) const override
	{
		s = localized ? POSLAYER_CNAME : _T("Position Layer");
	}
};
...
void PositionLayerControl::GetValue(TimeValue t, void* val, Interval& valid, GetSetMethod method)
{
	if (needToSetUpParentNode) // not completely optimal but it guarentees that we will recheck and find our parent node
							   // and add to the main list after we are cloned
		SetParentNodesOnMLC();
...
void PositionLayerControl::GetOutputValue(TimeValue t, void* val, Interval& valid)
{
	Matrix3 tm(1);
	float weight;
...
void PositionLayerControl::SetValue(TimeValue t, void* val, int commit, GetSetMethod method)
{
	if (!conts.Count() || GetLocked() == true)
		return;
	float weight;
...
public:
	...
	const TCHAR* ClassName()
	{
		return POSLAYER_CNAME;
	}
	const TCHAR* NonLocalizedClassName()
	{
		return _T("Position Layer");
	}
	SClass_ID SuperClassID()
	{
		return CTRL_POSITION_CLASS_ID;
...
ClassDesc* GetPositionLayerDesc()
{
	return &posLayerCD;
}
...
static ParamBlockDesc2 pos_layer_paramblk(0, _T("Parameters"), 0, &posLayerCD, P_AUTO_CONSTRUCT, PBLOCK_INIT_REF_ID,
		...
		kX, _T("exposedX"), TYPE_FLOAT, P_ANIMATABLE | P_TRANSIENT, IDS_RB_X, p_nonLocalizedName, _T("X"), p_end, kY,
		_T("exposedY"), TYPE_FLOAT, P_ANIMATABLE | P_TRANSIENT, IDS_RB_Y, p_nonLocalizedName, _T("Y"), p_end, kZ,
		_T("exposedZ"), TYPE_FLOAT, P_ANIMATABLE | P_TRANSIENT, IDS_RB_Z, p_nonLocalizedName, _T("Z"), p_end,
...
public:
	...
	EulerRecheck(RotationLayerControl* lC)
	{
		rLC = lC;
	};
	void Restore(int isUndo)
	{
		rLC->CheckOkayToBlendAsEulers();
	}
	void Redo()
	{
		rLC->CheckOkayToBlendAsEulers();
...
BOOL RotationLayerControl::AssignController(Animatable* control, int subAnim)
{
	if (GetLocked() == true ||
		...
		return FALSE;
	if (theHold.Holding())
		theHold.Put(new EulerRecheck(
				this)); // this is makes sure that we reset whether or not we can blend as an euler for rot layers.
...
{
	int order = 1;
	pblock->GetValue(kX_order, GetCOREInterface()->GetTime(), order);
	return --order;
}
...
{
	int order;
	pblock->GetValue(kY_order, GetCOREInterface()->GetTime(), order);
	return --order;
}
...
{
	int order;
	pblock->GetValue(kZ_order, GetCOREInterface()->GetTime(), order);
	return --order;
}
...
{
	BOOL what = FALSE;
	pblock->GetValue(kBlendEulerAsQuat, GetCOREInterface()->GetTime(), what);
	return what;
}
...
BOOL RotationLayerControl::GetValueAsEuler(TimeValue t, void* val, Interval& valid, GetSetMethod method, BOOL output)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		{
		case 0:
			tm.RotateX(currentVal[i]);
			break;
		case 1:
			tm.RotateY(currentVal[i]);
			break;
		case 2:
			tm.RotateZ(currentVal[i]);
			break;
		}
	}
...
void RotationLayerControl::GetValue(TimeValue t, void* val, Interval& valid, GetSetMethod method)
{
	...
	{ // Addative method
		for (int i = 0; i < stop; i++)
		{
			...
			if (weight != 0.0f && conts[i])
			{
				...
				weightedRot = Slerp(IdentQuat(), weightedRot, std::min(std::max(weight, 0.0f), 1.0f));
				weightedRot.Normalize();
	...
	{ // pose to pose blending
		...
		for (int i = 0; i < stop; i++)
		{
			...
			if (weight != 0.0f && conts[i])
			{
				...
				weightedRot = Slerp(lastRot, weightedRot, std::min(std::max(weight, 0.0f), 1.0f));
				weightedRot.Normalize();
...
void RotationLayerControl::GetOutputValue(TimeValue t, void* val, Interval& valid)
{
...
void RotationLayerControl::SetValue(TimeValue t, void* val, int commit, GetSetMethod method)
{
	if (!conts.Count() || GetLocked() == true)
		return;
	...
	{ // Addative method
		if (!gimbal_axis)
			for (int i = 0; i < active; i++)
			{
				...
				{
					weight = mLC->GetWeight(t, valid);
					Quat weightedRot = Slerp(IdentQuat(), Quat(localTM), std::min(std::max(weight, 0.0f), 1.0f));
		...
		{
			AngAxis na = *aa;
			if (!(conts[active]->ClassID() == Class_ID(LOOKAT_CONSTRAINT_CLASS_ID, 0)) && !gimbal_axis)
	...
	{ // pose to pose blending
		...
		if (!gimbal_axis)
			for (int i = 0; i < active; i++)
			{
				...
				{
					weight = mLC->GetWeight(t, valid);
					Quat weightedRot = Slerp(lastRot, Quat(localTM), std::min(std::max(weight, 0.0f), 1.0f));
					lastRot = weightedRot;
					weightedRot = weightedRot * Quat(initTM);
...
public:
	...
	const TCHAR* ClassName()
	{
		return ROTLAYER_CNAME;
	}
	const TCHAR* NonLocalizedClassName()
	{
		return _T("Rotation Layer");
	}
	SClass_ID SuperClassID()
	{
		return CTRL_ROTATION_CLASS_ID;
...
ClassDesc* GetRotationLayerDesc()
{
	return &rotLayerCD;
}
...
static ParamBlockDesc2 rot_layer_paramblk(0, _T("Parameters"), 0, &rotLayerCD, P_AUTO_CONSTRUCT, PBLOCK_INIT_REF_ID,
		...
		kX, _T("exposedX"), TYPE_ANGLE, P_ANIMATABLE | P_TRANSIENT, IDS_RB_X, p_nonLocalizedName, _T("X"), p_end, kY,
		_T("exposedY"), TYPE_ANGLE, P_ANIMATABLE | P_TRANSIENT, IDS_RB_Y, p_nonLocalizedName, _T("Y"), p_end, kZ,
		_T("exposedZ"), TYPE_ANGLE, P_ANIMATABLE | P_TRANSIENT, IDS_RB_Z, p_nonLocalizedName, _T("Z"), p_end, kX_order,
		_T("eulerXOrder"), TYPE_INT, 0, IDS_XORDER, p_default, 1, p_range, 1, NUM_SUPPORTED_EULERS, p_end, kY_order,
		_T("eulerYOrder"), TYPE_INT, 0, IDS_YORDER, p_default, 3, p_range, 1, NUM_SUPPORTED_EULERS, p_end, kZ_order,
		_T("eulerZOrder"), TYPE_INT, 0, IDS_ZORDER, p_default, 5, p_range, 1, NUM_SUPPORTED_EULERS, p_end,
		kBlendEulerAsQuat, _T("BlendEulerAsQuat"), TYPE_BOOL, 0, IDS_BLEND_EULER_AS_QUAT, p_default, FALSE, p_end,
...
{
public:
	ScaleLayerControl(BOOL loading)
			: LayerControl(loading)
	{
		Init();
	}
	...
	Class_ID ClassID()
	{
		return (SCALELAYER_CONTROL_CLASS_ID);
	}
	SClass_ID SuperClassID()
	{
	...
	void GetClassName(MSTR& s, bool localized) const override
	{
		s = localized ? SCALELAYER_CNAME : _T("Scale Layer");
	}
};


void ScaleLayerControl::GetValue(TimeValue t, void* val, Interval& valid, GetSetMethod method)
{
	if (needToSetUpParentNode) // not completely optimal but it guarentees that we will recheck and find our parent node
							   // and add to the main list after we are cloned
		SetParentNodesOnMLC();
...
void ScaleLayerControl::GetOutputValue(TimeValue t, void* val, Interval& valid)
{
	float weight;
...
void ScaleLayerControl::SetValue(TimeValue t, void* val, int commit, GetSetMethod method)
{
	if (!conts.Count() || GetLocked() == true)
		return;
...
public:
	...
	const TCHAR* ClassName()
	{
		return SCALELAYER_CNAME;
	}
	const TCHAR* NonLocalizedClassName()
	{
		return _T("Scale Layer");
	}
	SClass_ID SuperClassID()
	{
		return CTRL_SCALE_CLASS_ID;
...
************************************************************/
class MainLayerClassDesc : public ClassDesc2
{
public:
	int IsPublic()
	{
	...
	const TCHAR* ClassName()
	{
		return MAINLAYER_CNAME;
	}
	const TCHAR* NonLocalizedClassName()
	{
		return _T("Main Layer");
	}
	SClass_ID SuperClassID()
	{
	...
	Class_ID ClassID()
	{
		return MAINLAYER_CONTROL_CLASS_ID;
	}
	const TCHAR* Category()
	{
		return _T("");
	...
	void* Create(BOOL loading)
	{
		return new MainLayerControl(loading);
	}
...
static MainLayerClassDesc mainLayerCD;
ClassDesc* GetMainLayerDesc()
{
	return &mainLayerCD;
}

void MainLayerControl::Init()
{
	// make the paramblock
	mainLayerCD.MakeAutoParamBlocks(this);
}


// per instance list controller block
static ParamBlockDesc2 main_layer_paramblk(0, _T("Parameters"), 0, &mainLayerCD, P_AUTO_CONSTRUCT, PBLOCK_INIT_REF_ID,
		// params
		kName, _T("name"), TYPE_STRING, 0, IDS_MZ_NAME, p_end, kLayerCtrlWeight, _T("weight"), TYPE_FLOAT,
		P_TV_SHOW_ALL + P_ANIMATABLE, IDS_AF_LIST_WEIGHT, p_default, 1.0f, p_range, -10000.0f, 10000.0f, p_dim,
		stdPercentDim,
...
MainLayerControl::MainLayerControl(BOOL loading)
{
	pblock = NULL;
	Init();
	paramFlags = 0;
	...
	pblock->SetControllerByID(kLayerCtrlWeight, 0, CreateInterpFloat());
}

MainLayerControl::MainLayerControl(const MainLayerControl& ctrl)
{
	pblock = NULL;
	DeleteAllRefs();
	ReplaceReference(0, ctrl.pblock);
...
RefResult MainLayerControl::NotifyRefChanged(
		const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
	...
	case REFMSG_NODEMONITOR_TARGET_DELETED:
		// since the MainLayerControl owns the weight control (that's in our pblock) as a subanim that shows up in
		// trackbar, etc.. we need to propagate messages to dependents so such things as redrawing of the trackbar
		// happens correctly Defect #   MZ 797495 note that we modified this some more for  868689.  We would propagate
		// REFMSG_CHANGE instead of just the message for any message we received which was wrong. to fix 797495 it looks
		// like we just needed to do this propagation for just these messages which are used by trackbar.
		if (propagate && pblock && (hTarget == pblock))
		{
		...
		if (message == REFMSG_NODEMONITOR_TARGET_DELETED)
		{
			int nodeCount = monitorNodes.Count();
			for (int i = nodeCount - 1; i >= 0; --i)
			{
				if (monitorNodes[i] == hTarget)
				{
					int refNum = 1 + layers.Count() + i;
					DeleteReference(refNum);
					if (theHold.Holding())
						theHold.Put(new MonitorNodesEleRestore(this, i));
					monitorNodes.Delete(i, 1);
				}
			}
		}
		if (message == REFMSG_TARGET_DELETED)
		{
			// null the reference pointer when the reference target is deleted.
			if (hTarget == pblock)
			{
				pblock = nullptr;
			}
			// remove slot when the reference target is deleted.
		...
		if (message == REFMSG_CHANGE)
			main_layer_paramblk.InvalidateUI();
		break;
	}
	return REF_SUCCEED;
}

int MainLayerControl::NumRefs()
{
	// numControllers+monitorNodes +pblock
	return layers.Count() + monitorNodes.Count() + 1;
}

RefTargetHandle MainLayerControl::GetReference(int i)
{
	if (i == 0)
		return pblock; // pblock
	else if (i <= layers.Count())
		return layers[i - 1]; // all of the layers that look back at this guy
	else if (i <= (layers.Count() + monitorNodes.Count()))
		return monitorNodes[i - (layers.Count() + 1)];
	return NULL;
}

BOOL MainLayerControl::IsRealDependency(ReferenceTarget* rtarg)
{
	// make the references to LayerControls be weak, since we don't want to prevent them from deleting
	// if the animatable the layerControl controls got deleted(just as INodeMonitor Tab does)
	// This solves the issue when loading xref scenes, some temp animatable objects contain animation layers but later
	// it doesn't get merged but deleted, since previously we hold those layers as strong reference, those layer won't
...
int MainLayerControl::NumSubs()
{
	return 1;
}


Animatable* MainLayerControl::SubAnim(int i)
{
	if (i == 0)
		return pblock;
	else
...
TSTR MainLayerControl::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0:
...
int MainLayerControl::SubNumToRefNum(int subNum)
{
	if (subNum == 0)
		return subNum;
	return -1;
...
void MainLayerControl::SetReference(int i, RefTargetHandle rtarg)
{
	if ((i == 0) || (rtarg && rtarg->ClassID() == Class_ID(PARAMETER_BLOCK2_CLASS_ID, 0)))
	{
		// set the parameter block
...
IOResult MainLayerControl::Save(ISave* isave)
{
	ULONG nb;
...
IOResult MainLayerControl::Load(ILoad* iload)
{
	ULONG nb;
	IOResult res = IO_OK;
	int ix = 0;
...
Control* MainLayerControl::GetDefaultControl(SClass_ID sclassID)
{
...
TSTR MainLayerControl::GetName()
{
	const TCHAR* n = NULL;
	pblock->GetValue(kName, 0, n);
	return TSTR(n);
}

void MainLayerControl::SetName(TSTR& name)
{
...
void MainLayerControl::NotifyLayers()
{
	for (int j = 0; j < layers.Count(); ++j)
	{
		LayerControl* lC = layers[j];
...
float MainLayerControl::GetWeight(TimeValue t, Interval& valid)
{
	float val;
...
void MainLayerControl::SetWeight(TimeValue t, float weight)
{
...
Control* MainLayerControl::GetWeightControl()
{
	return pblock->GetControllerByID(kLayerCtrlWeight);
}


void MainLayerControl::SetWeightControl(Control* weight)
{
	if (weight)
		pblock->SetControllerByID(kLayerCtrlWeight, 0, weight);
	for (int j = 0; j < layers.Count(); ++j)
	{
		LayerControl* lC = layers[j];
		if (lC)
...
void MainLayerControl::AddLayerControl(LayerControl* lC)
{
	int j;
	for (j = 0; j < layers.Count(); ++j)
	{
		if (layers[j] == lC)
		{
			break;
	...
	if (j == layers.Count()) // if not existing
	{
		LayerControl* null_ptr = NULL;
		;
		layers.Append(1, &null_ptr);
		ReplaceReference(layers.Count(), static_cast<RefTargetHandle>(lC));
	}
}

void MainLayerControl::AppendLayerControl(LayerControl* lC)
{
	Resize(layers.Count() + 1);
	ReplaceReference(layers.Count(), static_cast<RefTargetHandle>(lC));
}

MainLayerControl::~MainLayerControl()
{
	DeleteAllRefs();
}

void MainLayerControl::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? MAINLAYER_CNAME : _T("Main Layer");
}

void MainLayerControl::Resize(int numLayers)
{
...
MainLayerControl& MainLayerControl::operator=(const MainLayerControl& ctrl)
{
	int i;
...
void MainLayerControl::NotifyForeground(TimeValue t)
{
	for (int i = 0; i < layers.Count(); ++i)
		if (layers[i])
...
Control* MainLayerControlManager::GetKeyableController(Control* control)
{
	if (controllerType == IAnimLayerControlManager::eBezier)
	{
		if (control->SuperClassID() == CTRL_ROTATION_CLASS_ID)
		{
			return static_cast<Control*>(
					GetCOREInterface()->CreateInstance(CTRL_ROTATION_CLASS_ID, Class_ID(EULER_CONTROL_CLASS_ID, 0)));
		}
		else if (control->SuperClassID() == CTRL_POSITION_CLASS_ID)
		{
			return static_cast<Control*>(
					GetCOREInterface()->CreateInstance(CTRL_POSITION_CLASS_ID, IPOS_CONTROL_CLASS_ID));
		}
		else if (control->SuperClassID() == CTRL_SCALE_CLASS_ID)
		{
			return static_cast<Control*>(
					GetCOREInterface()->CreateInstance(CTRL_SCALE_CLASS_ID, ISCALE_CONTROL_CLASS_ID));
		}
	...
	else if (controllerType == IAnimLayerControlManager::eLinear)
	{
		...
		{
			return static_cast<Control*>(
					GetCOREInterface()->CreateInstance(CTRL_SCALE_CLASS_ID, Class_ID(LININTERP_SCALE_CLASS_ID, 0)));
		}
		else if ((control->SuperClassID() == CTRL_FLOAT_CLASS_ID))
		{
			return static_cast<Control*>(
					GetCOREInterface()->CreateInstance(CTRL_FLOAT_CLASS_ID, Class_ID(LININTERP_FLOAT_CLASS_ID, 0)));
		}
		else if ((control->SuperClassID() == CTRL_POINT4_CLASS_ID))
		{
			return static_cast<Control*>(
					GetCOREInterface()->CreateInstance(CTRL_POINT4_CLASS_ID, Class_ID(TCBINTERP_POINT3_CLASS_ID, 0)));
		}
		else if ((control->SuperClassID() == CTRL_POINT3_CLASS_ID))
		{
			return static_cast<Control*>(
					GetCOREInterface()->CreateInstance(CTRL_POINT3_CLASS_ID, Class_ID(TCBINTERP_POINT4_CLASS_ID, 0)));
		}
	}
...
void MainLayerControlManager::UpdateLayerControls()
{
	int numMLCs = GetNumMLCs();
	for (int i = 0; i < numMLCs; ++i)
	{
		MainLayerControl* mLC = GetNthMLC(i);
		if (mLC)
		{
			for (int j = 0; j < mLC->layers.Count(); ++j)
			{
				...
				{
					lC->NotifyDependents(FOREVER, 0, REFMSG_CHANGE);
					lC->NotifyDependents(FOREVER, 0, REFMSG_CONTROLREF_CHANGE, TREE_VIEW_CLASS_ID,
							FALSE); // we really only want to send this to the treeview, not others (like the mixer).
				}
			}
		}
	}
...
void MainLayerControlManager::SetFilterActiveOnlyTrackView(bool val)
{
	if (val != filterActive)
	{
		filterActive = val;
...
bool MainLayerControlManager::GetFilterActiveOnlyTrackView()
{
	return filterActive;
}

void MainLayerControlManager::SetJustUpToActive(bool v)
{
	if (v != justUpToActive)
	{
		justUpToActive = v;
...
bool MainLayerControlManager::GetJustUpToActive()
{
	return justUpToActive;
}


void MainLayerControlManager::SetCollapseControllerType(IAnimLayerControlManager::ControllerType type)
{
	controllerType = type;
	/*	getCfgMgr().setSection(_T("AnimationLayers"));
...
IAnimLayerControlManager::ControllerType MainLayerControlManager::GetCollapseControllerType()
{
	return controllerType;
}
void MainLayerControlManager::SetCollapsePerFrame(bool v)
{
...
bool MainLayerControlManager::GetCollapsePerFrame()
{
	return collapseAllPerFrame;
}
void MainLayerControlManager::SetCollapsePerFrameActiveRange(bool v)
{
...
bool MainLayerControlManager::GetCollapsePerFrameActiveRange()
{
	return collapsePerFrameActiveRange;
}
void MainLayerControlManager::SetCollapseRange(Interval range)
{
	collapseRange = range;
	/*	getCfgMgr().setSection(_T("AnimationLayers"));
...
Interval MainLayerControlManager::GetCollapseRange()
{
	return collapseRange;
}


TSTR MainLayerControlManager::GetLayerName(int index)
{
	MainLayerControl* mLC = GetNthMLC(index);
	TSTR name;
	if (mLC)
	{
		name = mLC->GetName();
...
void MainLayerControlManager::SetLayerName(int index, TSTR name)
{
	if (index == 0)
		return;
	MainLayerControl* mLC = GetNthMLC(index);
...
bool MainLayerControlManager::GetLayerMute(int index)
{
	MainLayerControl* mLC = GetNthMLC(index);
	bool mute = false;
	if (mLC)
...
void MainLayerControlManager::SetLayerMute(int index, bool mute)
{
	MainLayerControl* mLC = GetNthMLC(index);
	if (mLC)
	{
...
bool MainLayerControlManager::GetLayerLocked(int index)
{
	MainLayerControl* mLC = GetNthMLC(index);
	bool locked = false;
	if (mLC)
...
void MainLayerControlManager::SetLayerLocked(int index, bool locked)
{
	MainLayerControl* mLC = GetNthMLC(index);
	if (mLC)
	{
...
bool MainLayerControlManager::GetLayerOutputMute(int index)
{
	MainLayerControl* mLC = GetNthMLC(index);
	bool mute = false;
	if (mLC)
...
void MainLayerControlManager::SetLayerOutputMute(int index, bool mute)
{
	MainLayerControl* mLC = GetNthMLC(index);
	if (mLC)
	{
...
float MainLayerControlManager::GetLayerWeight(int index, TimeValue t)
{
	MainLayerControl* mLC = GetNthMLC(index);
	float weight = 0.0f;
	Interval valid = FOREVER;
...
void MainLayerControlManager::SetLayerWeight(int index, TimeValue t, float weight)
{
	MainLayerControl* mLC = GetNthMLC(index);
	if (mLC)
	{
...
Control* MainLayerControlManager::GetLayerWeightControl(int index)
{
	MainLayerControl* mLC = GetNthMLC(index);
	Control* c = NULL;
	if (mLC)
...
bool MainLayerControlManager::SetLayerWeightControl(int index, Control* c)
{
	MainLayerControl* mLC = GetNthMLC(index);
...
bool MainLayerControl::IsHardLocked()
{
		...
		if (lC)
		{
			for (int j = 0; j < lC->mLCs.Count(); ++j)
			{
				if (lC->mLCs[j] == myIndex)
				{
					...
					break; // only one mainlayercontrol per layercontrol
				}
			}
		}
		if (anim)
...
void MainLayerControl::SetLocked(bool val, bool setAll)
{
	...
	// If this layer is internally frozen, then it can not be unlocked.
	if (val == false && GetLayerInternalFrozen())
		return;
	...
	if (settingLock == false)
	{
		...
		if (MainLayerControlManager::toolbarCtrl)
			MainLayerControlManager::toolbarCtrl->Refresh(); // refresh the UI
...
void MainLayerControl::UnlockChildren(ReferenceTarget* anim)
{
	if (anim)
	{
		for (int i = 0; i < anim->NumSubs(); ++i)
...
void MainLayerControlManager::DisableLayerNodes(Tab<INode*>& nodeTab)
{
	...
	for (int i = 0; i < existingLayers.Count(); ++i)
	{
		...
		{
			MainLayerControl* mLC = GetMLCMan()->GetNthMLC(lC2->mLCs[f]);
...
void MainLayerControl::AddNode(INode* node) // add nodes to the monitor list.
{
	int j;
...
bool MainLayerControl::GetLayerInternalFrozen()
{
	int myIndex = GetMLCMan()->GetMLCIndex(this);
	for (int i = 0; i < layers.Count(); ++i)
	{
		...
		if (lC)
		{
			for (int j = 0; j < lC->mLCs.Count(); ++j)
			{
				if (lC->mLCs[j] == myIndex && j < lC->mbInternalFrozenFlags.Count())
				{
					return lC->mbInternalFrozenFlags[j];
				}
			}
		}
	}
...
void MainLayerControl::SetLayerInternalFrozen(bool frozen)
{
	int myIndex = GetMLCMan()->GetMLCIndex(this);
	for (int i = 0; i < layers.Count(); ++i)
	{
		...
		if (lC)
		{
			for (int j = 0; j < lC->mLCs.Count(); ++j)
			{
				if (lC->mLCs[j] == myIndex && j < lC->mbInternalFrozenFlags.Count())
				{
					lC->mbInternalFrozenFlags[j] = frozen;
					break; // only one mainlayercontrol per layercontrol
				}
			}
		}
...
class MainLayerControlManagerActionCallback : public ActionCallback
{
public:
	MainLayerControlManagerActionCallback()
	{
	}
	BOOL KeyboardShortcut(int id);
	BOOL ExecuteAction(int id)
	{
		return KeyboardShortcut(id);
	}
};


BOOL MainLayerControlManagerActionCallback::KeyboardShortcut(int cmd)
{
	if (GetCOREInterface()->GetSelNodeCount() < 1)
		return FALSE;
	IAnimLayerControlManager* manager =
			static_cast<IAnimLayerControlManager*>(GetCOREInterface(IANIMLAYERCONTROLMANAGER_INTERFACE));
...
class MainLayerControlManagerActionTable : public ActionTable
{
public:
	MainLayerControlManagerActionTable(ActionTableId id, ActionContextId contextId, TSTR& name, HACCEL hDefaults,
			int numIds, ActionDescription* pOps, HINSTANCE hInst);
	~MainLayerControlManagerActionTable();
...
;
MaxIcon* MainLayerControlManagerActionTable::mi1 = nullptr;
MaxIcon* MainLayerControlManagerActionTable::mi2 = nullptr;
MaxIcon* MainLayerControlManagerActionTable::mi3 = nullptr;
MaxIcon* MainLayerControlManagerActionTable::mi4 = nullptr;
MaxIcon* MainLayerControlManagerActionTable::mi5 = nullptr;
MaxIcon* MainLayerControlManagerActionTable::mi6 = nullptr;
MaxIcon* MainLayerControlManagerActionTable::mi7 = nullptr;
MaxIcon* MainLayerControlManagerActionTable::mi8 = nullptr;
MaxIcon* MainLayerControlManagerActionTable::mi9 = nullptr;
MaxIcon* MainLayerControlManagerActionTable::mi10 = nullptr;

// Constructor
MainLayerControlManagerActionTable::MainLayerControlManagerActionTable(ActionTableId id, ActionContextId contextId,
		TSTR& name, HACCEL hDefaults, int numIds, ActionDescription* pOps, HINSTANCE hInst)
		: ActionTable(id, contextId, name, hDefaults, numIds, pOps, hInst)
{
}

// Destructor
MainLayerControlManagerActionTable::~MainLayerControlManagerActionTable()
{
	if (mi1 != NULL)
	{
		delete mi1;
...
BOOL MainLayerControlManagerActionTable::IsChecked(int cmdId)
{
	return 0;
}

BOOL MainLayerControlManagerActionTable::IsEnabled(int cmdId)
{
	...
	{
	case ID_ENABLE_ANIM_LAYERS:
		return man->ShowEnableAnimLayersBtn();
	case ID_ANIM_LAYER_PROPERTIES:
		return man->ShowAnimLayerPropertiesBtn();
	case ID_ADD_ANIM_LAYER:
		return man->ShowAddAnimLayerBtn();
	case ID_DELETE_ANIM_LAYER:
		return man->ShowDeleteAnimLayerBtn();
	case ID_COPY_ANIM_LAYER:
		return man->ShowCopyAnimLayerBtn();
	case ID_PASTE_ACTIVE_ANIM_LAYER:
		return man->ShowPasteActiveAnimLayerBtn();
	case ID_PASTE_NEW_LAYER:
		return man->ShowPasteNewLayerBtn();
	case ID_COLLAPSE_ANIM_LAYER:
		return man->ShowCollapseAnimLayerBtn();
	case ID_DISABLE_ANIM_LAYER:
		return man->ShowDisableAnimLayerBtn();
	case ID_SELECT_NODES_FROM_ACTIVE:
		return man->ShowSelectNodesFromActive();
	}
	return 0;
}

MaxIcon* MainLayerControlManagerActionTable::GetIcon(int cmdId)
{
	switch (cmdId)
	{
	case ID_ENABLE_ANIM_LAYERS:
		if (!mi1)
			mi1 = new MaxBmpFileIcon(_T("Common/Layers"));
		return mi1;
	case ID_ANIM_LAYER_PROPERTIES:
		if (!mi2)
			mi2 = new MaxBmpFileIcon(_T("AnimationLayer/AnimLayerProperties"));
		return mi2;
	case ID_ADD_ANIM_LAYER:
		if (!mi3)
			mi3 = new MaxBmpFileIcon(_T("AnimationLayer/AddAnimLayer"));
		return mi3;
	case ID_DELETE_ANIM_LAYER:
		if (!mi4)
			mi4 = new MaxBmpFileIcon(_T("AnimationLayer/DeleteAnimLayer"));
		return mi4;
	case ID_COPY_ANIM_LAYER:
		if (!mi5)
			mi5 = new MaxBmpFileIcon(_T("AnimationLayer/CopyAnimLayer"));
		return mi5;
	case ID_PASTE_ACTIVE_ANIM_LAYER:
		if (!mi6)
			mi6 = new MaxBmpFileIcon(_T("AnimationLayer/PasteActiveAnimLayer"));
		return mi6;
	case ID_PASTE_NEW_LAYER:
		if (!mi7)
			mi7 = new MaxBmpFileIcon(_T("AnimationLayer/PasteNewLayer"));
		return mi7;
	case ID_COLLAPSE_ANIM_LAYER:
		if (!mi8)
			mi8 = new MaxBmpFileIcon(_T("AnimationLayer/CollapseAnimLayer"));
		return mi8;
	case ID_DISABLE_ANIM_LAYER:
		if (!mi9)
			mi9 = new MaxBmpFileIcon(_T("AnimationLayer/DisableAnimLayer"));
		return mi9;
	case ID_SELECT_NODES_FROM_ACTIVE:
		if (!mi10)
			mi10 = new MaxBmpFileIcon(_T("AnimationLayer/SelectActiveLayerObjects"));
		return mi10;
	}
	return NULL;
}
...
static ActionDescription animLayerActions[] = { ID_ENABLE_ANIM_LAYERS, IDS_ENABLE_ANIM_LAYERS, IDS_ENABLE_ANIM_LAYERS,
	IDS_ANIM_LAYERS, ID_ANIM_LAYER_PROPERTIES, IDS_ANIM_LAYER_PROPERTIES, IDS_ANIM_LAYER_PROPERTIES, IDS_ANIM_LAYERS,
	ID_ADD_ANIM_LAYER, IDS_ADD_ANIM_LAYER, IDS_ADD_ANIM_LAYER, IDS_ANIM_LAYERS, ID_DELETE_ANIM_LAYER,
	IDS_DELETE_ANIM_LAYER, IDS_DELETE_ANIM_LAYER, IDS_ANIM_LAYERS, ID_COPY_ANIM_LAYER, IDS_COPY_ANIM_LAYER,
	IDS_COPY_ANIM_LAYER, IDS_ANIM_LAYERS, ID_PASTE_ACTIVE_ANIM_LAYER, IDS_PASTE_ACTIVE_ANIM_LAYER,
	IDS_PASTE_ACTIVE_ANIM_LAYER, IDS_ANIM_LAYERS, ID_PASTE_NEW_LAYER, IDS_PASTE_NEW_LAYER, IDS_PASTE_NEW_LAYER,
	IDS_ANIM_LAYERS, ID_COLLAPSE_ANIM_LAYER, IDS_COLLAPSE_ANIM_LAYER, IDS_COLLAPSE_ANIM_LAYER, IDS_ANIM_LAYERS,
	ID_DISABLE_ANIM_LAYER, IDS_DISABLE_ANIM_LAYER, IDS_DISABLE_ANIM_LAYER, IDS_ANIM_LAYERS, ID_SELECT_NODES_FROM_ACTIVE,
	IDS_SELECT_NODES_FROM_ACTIVE, IDS_SELECT_NODES_FROM_ACTIVE, IDS_ANIM_LAYERS
...
private:
	...
	static MainLayerControlManagerActionTable* mLCManActionTable;
	// Memory is owned by this class.
	static MainLayerControlManagerActionCallback* mLCManActionCallback;
...
MainLayerControlManagerActionTable* LayerControlActionManager::mLCManActionTable = NULL;
MainLayerControlManagerActionCallback* LayerControlActionManager::mLCManActionCallback = NULL;
// Constructor
...
ActionTable* LayerControlActionManager::GetActionTable()
{
	if (!mLCManActionTable)
	{
		...
		mLCManActionTable = new MainLayerControlManagerActionTable(
				kActions, kContext, name, NULL, numOps, animLayerActions, hInstance);
...
void LayerControlActionManager::SetUpActions()
{
	...
	{
		mLCManActionCallback = new MainLayerControlManagerActionCallback();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
//				MainLayerControlManager
//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////


class MainLayerControlManagerClassDesc : public ClassDesc2
{
public:
	MainLayerControlManagerClassDesc()
	{
	}
	int IsPublic()
	{
	...
	const TCHAR* ClassName()
	{
		return _T("MainLayerControlManager");
	}
	const TCHAR* NonLocalizedClassName()
	{
		return _T("MainLayerControlManager");
	...
	Class_ID ClassID()
	{
		return MAINLAYERCONTROLMANAGER_CLASS_ID;
	}
	const TCHAR* Category()
	{
		return _T("");
	...
	{
		MainLayerControlManager::LoadFromIni();
	}
	int NumActionTables()
	{
		return 1;
	} // in order to register or callback we need to manual set this up!
	ActionTable* GetActionTable(int i)
	{
		return LayerControlActionManager::GetInstance().GetActionTable();
	}
};

static MainLayerControlManagerClassDesc mLCManCD;
ClassDesc* GetMainLayerControlManagerDesc()
{
	return &mLCManCD;
}


class MainLayerControlAccessor : public PBAccessor
{
public:
	TSTR GetLocalName(ReferenceMaker* owner, ParamID id, int tabIndex, bool /*localized*/)
	{
		...
		{
			TSTR name;
			if (id == MainLayerControlManager::mainlayers)
			{
				auto* manager = dynamic_cast<MainLayerControlManager*>(owner);
				if (manager != nullptr)
				{
					MainLayerControl* mLC = manager->GetNthMLC(tabIndex);
					if (mLC)
						return mLC->GetName();
				}
			}
		}
		return GetString(IDS_MAINLAYER);
	}
};

static MainLayerControlAccessor theMainLayerControlAccessor;

static ParamBlockDesc2 MainLayerControlManager_pblk(0, _T("mainLayerControlManagerParams"), 0, &mLCManCD,
		P_AUTO_CONSTRUCT, 0,

		MainLayerControlManager::mainlayers, _T("AnimLayers"), TYPE_REFTARG_TAB, 0,
		P_COMPUTED_NAME + P_VARIABLE_SIZE + P_SUBANIM, IDS_MAINLAYER, p_accessor, &theMainLayerControlAccessor, p_end,
		p_end);

static MainLayerControlManager* theMainLayerControlManager = nullptr;
MainLayerControlManager* GetMLCMan()
{
	if (theMainLayerControlManager)
	{
		if (theMainLayerControlManager->pblock == nullptr)
			mLCManCD.MakeAutoParamBlocks(theMainLayerControlManager);
		return theMainLayerControlManager;
	}
	else
	{
		theMainLayerControlManager = new MainLayerControlManager;
	}
	return theMainLayerControlManager;
}

void AddMainLayerControlManagerToScene()
{
	ITrackViewNode* tvRoot = GetCOREInterface()->GetTrackViewRootNode();
	if (tvRoot->FindItem(MAINLAYERCONTROLMANAGER_CLASS_ID) < 0)
	{
		HoldSuspend hs;
		tvRoot->AddController(GetMLCMan(), _T("Anim Layer Control Manager"), MAINLAYERCONTROLMANAGER_CLASS_ID);
	}
}
...
void PostResetOrOpen(void* param, NotifyInfo* info)
{
	if (info->intcode == NOTIFY_POST_SCENE_RESET)
	{
		...
		GetMLCMan()->EnumRefHierarchy(deleteCustAttribContainerAndNoteTracksProc, false, false, false, false);
		return;
	}
	AddMainLayerControlManagerToScene();
...
{
	if (info && info->callParam && info->intcode == NOTIFY_FILE_PRE_OPEN)
	{
		// if we are in open and the flag says it's not a max file preopen (it's a render preset for example) then don't
		// delete the animation layers!
		if (IOTYPE_MAX != *GetNotifyParam<NOTIFY_FILE_PRE_OPEN>(info))
			return;
	}
	GetMLCMan()->MainLayer_DeleteMe();
}
...
void ShutDown(void* param, NotifyInfo* info)
{
	...
	if (MainLayerControlManager::toolbarCtrl)
	{
		delete MainLayerControlManager::toolbarCtrl;
		MainLayerControlManager::toolbarCtrl = NULL;
	}
	GetMLCMan()->MainLayer_DeleteMe();
}
...
void PreMerge(void* param, NotifyInfo* info)
{
	...
	MainLayerControlManager* mlcmP = static_cast<MainLayerControlManager*>(param);
	mlcmP->SaveExistingMLCs();
}

void PostMerge(void* param, NotifyInfo* info)
{
	MainLayerControlManager* mlcmP = static_cast<MainLayerControlManager*>(param);
	mlcmP->RevertExistingMLCs();
}

void NotifySelectionChanged(void* param, NotifyInfo* info)
{
	MainLayerControlManager* mlcmP = static_cast<MainLayerControlManager*>(param);
	mlcmP->NotifySelectionChanged();
}

AnimLayerToolbarControl* MainLayerControlManager::toolbarCtrl = NULL; // leaks need to figure out not to leak..

MainLayerControlManager::MainLayerControlManager()
		: pblock(NULL)
{

	theMainLayerControlManager = this; // always singleton know excuses!
	...
	RegisterNotification(PostResetOrOpen, this, NOTIFY_SYSTEM_POST_RESET);
	RegisterNotification(PostResetOrOpen, this, NOTIFY_FILE_POST_OPEN);
	RegisterNotification(PostResetOrOpen, this, NOTIFY_SYSTEM_POST_NEW);
	RegisterNotification(PostResetOrOpen, this, NOTIFY_POST_SCENE_RESET);
	RegisterNotification(PreMerge, this, NOTIFY_FILE_PRE_MERGE);
	RegisterNotification(PostMerge, this, NOTIFY_FILE_POST_MERGE3);
...
MainLayerControlManager::~MainLayerControlManager()
{
	...
	UnRegisterNotification(PostResetOrOpen, this, NOTIFY_SYSTEM_POST_RESET);
	UnRegisterNotification(PostResetOrOpen, this, NOTIFY_FILE_POST_OPEN);
	UnRegisterNotification(PostResetOrOpen, this, NOTIFY_SYSTEM_POST_NEW);
	UnRegisterNotification(PostResetOrOpen, this, NOTIFY_POST_SCENE_RESET);
	UnRegisterNotification(PreMerge, this, NOTIFY_FILE_PRE_MERGE);
	UnRegisterNotification(PostMerge, this, NOTIFY_FILE_POST_MERGE3);
	UnRegisterNotification(::NotifySelectionChanged, this, NOTIFY_SELECTIONSET_CHANGED);

	DeleteAllRefs();
	theMainLayerControlManager = NULL;
}

Class_ID MainLayerControlManager::ClassID()
{
	return MAINLAYERCONTROLMANAGER_CLASS_ID;
}

SClass_ID MainLayerControlManager::SuperClassID()
{
	return CTRL_USERTYPE_CLASS_ID;
}

void MainLayerControlManager::GetClassName(MSTR& s, bool localized) const
{
	s = MAINLAYERCONTROLMANAGER_CNAME;
}

void MainLayerControlManager::LoadFromIni()
{
	/*
	int i;
	getCfgMgr().setSection(_T("AnimationLayers"));
...
void MainLayerControlManager::SetUpCustUI(ICustToolbar* toolbar, int id, HWND hWnd, HWND hParent)
{
...
const TCHAR* MainLayerControlManager::GetComboToolTip()
{
...
void MainLayerControlManager::NotifySelectionChanged()
{
	...
	if (GetNumMLCs() > 0) // only do all this if we have any layers.
	{
		...
		{
			Tab<MainLayerControlManager::LCInfo> layerControls;
			GetLayerControls(nodeTab, layerControls);
			BitArray bArray;
			int numMLCs = GetNumMLCs();
			bArray.SetSize(numMLCs);
...
bool MainLayerControlManager::ShowEnableAnimLayersBtn()
{
	return (GetCOREInterface()->GetSelNodeCount() > 0);
}
bool MainLayerControlManager::ShowAnimLayerPropertiesBtn()
{
	return true;
}
bool MainLayerControlManager::ShowAddAnimLayerBtn()
{
	return (activeLayers.Count() > 0);
}
bool MainLayerControlManager::ShowDeleteAnimLayerBtn()
{
	return (activeLayers.Count() == 1 && activeLayers[0] != 0 && AreAnyActiveLayersLocked() == false);
}
bool MainLayerControlManager::ShowCopyAnimLayerBtn()
{
	return (activeLayers.Count() == 1 && copyLayers == false);
}
bool MainLayerControlManager::ShowPasteActiveAnimLayerBtn()
{
	return (activeLayers.Count() == 1 && copyLayers == true && AreAnyActiveLayersLocked() == false);
}
bool MainLayerControlManager::ShowPasteNewLayerBtn()
{
	return (activeLayers.Count() >= 1 && copyLayers == true);
}
bool MainLayerControlManager::ShowCollapseAnimLayerBtn()
{
	return (activeLayers.Count() == 1 && activeLayers[0] != 0 && AreAnyActiveLayersLocked() == false);
}
bool MainLayerControlManager::ShowDisableAnimLayerBtn()
{
	return (activeLayers.Count() == 1 && canDisable == true && AreAnyActiveLayersLocked() == false);
}
void* MainLayerControlManager::GetInterface(ULONG Ifaceid)
{
	return Control::GetInterface(Ifaceid);
}

bool MainLayerControlManager::AreAnyActiveLayersLocked()
{
	for (int i = 0; i < activeLayers.Count(); ++i)
	{
		MainLayerControl* mLC = GetNthMLC(activeLayers[i]);
...
void MainLayerControlManager::InvalidateUI()
{
	if (pblock)
		MainLayerControlManager_pblk.InvalidateUI(pblock->LastNotifyParamID());
}

RefTargetHandle MainLayerControlManager::Clone(RemapDir& remap)
{
	auto* newstate = new MainLayerControlManager();
	if (pblock)
...
int MainLayerControlManager::NumSubs()
{
	return 1;
}

int MainLayerControlManager::NumRefs()
{
	return 1 + existingMainLayerControls.Count(); // save a reference to the current clip assocations on save so we
												  // can reinsert it in after the pblock gets merged.
}

Animatable* MainLayerControlManager::SubAnim(int i)
{
	if (i == 0)
		return pblock;
	else
...
TSTR MainLayerControlManager::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0:
		return localized ? GetString(IDS_ANIM_LAYERS) : _T("Anim Layers");
	default:
		return _T("");
	}
}
...
int MainLayerControlManager::NumParamBlocks()
{
	return 1;
}
IParamBlock2* MainLayerControlManager::GetParamBlock(int i)
{
	if (i == 0)
		return pblock;
	else
...
IParamBlock2* MainLayerControlManager::GetParamBlockByID(BlockID id)
{
	if (pblock && pblock->ID() == id)
		return pblock;
	else
		return NULL;
}

RefTargetHandle MainLayerControlManager::GetReference(int i)
{
	if (i == 0)
		return (RefTargetHandle)pblock;
	else if (i < (1 + existingMainLayerControls.Count()))
		return static_cast<RefTargetHandle>(existingMainLayerControls[i - 1]);
	else
		return (RefTargetHandle)NULL;
}
void MainLayerControlManager::SetReference(int i, RefTargetHandle rtarg)
{
	if (i == 0)
	{
		pblock = (IParamBlock2*)rtarg;
	...
	{
		existingMainLayerControls[i - 1] = static_cast<MainLayerControl*>(rtarg);
	}
}


RefResult MainLayerControlManager::NotifyRefChanged(
		const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message)
	{
	...
	case REFMSG_CHANGE:
	{
		MainLayerControlManager_pblk.InvalidateUI();
		break;
	}
	}
	return REF_SUCCEED;
...
MainLayerControl* MainLayerControlManager::GetNthMLC(int i)
{
	MainLayerControl* result = NULL;

	if (pblock && i > -1 && i < GetNumMLCs())
	{
		ReferenceTarget* c = NULL;
		pblock->GetValue(mainlayers, 0, c, i);
		DbgAssert(c);
		result = dynamic_cast<MainLayerControl*>(c);
	}
	return result;
}


int MainLayerControlManager::GetNumMLCs()
{
	if (pblock)
	{
		return pblock->Count(mainlayers);
	}
	else
		return 0;
}

void MainLayerControlManager::ResetLayerMLCs(int index)
{

	// we need to get all of the LayerContrl's in the scene we do that by getting all of the nodes with layers and then
	// all of the nodes.
	Tab<INode*> nodes;
	GetAllNodes(nodes);
	...
	GetLayerControlsInternal(nodes, layerControls);
...
void MainLayerControlManager::DeleteNthMLC(int index)
{
	if (pblock == NULL || index < 0 || index >= GetNumMLCs())
	{
		return;
	}
	pblock->Delete(mainlayers, index, 1);
}

void MainLayerControlManager::AppendMLC(MainLayerControl* mLC)
{
	...
	if (pblock)
	{
		...
		pblock->Append(mainlayers, 1, &rA);
	}
}


void MainLayerControlManager::GetNodesLayers(Tab<INode*>& nodeTab, Tab<int>& layers, Tab<bool>& layersOnAll)
{
...
void MainLayerControlManager::SetLayerActiveNodes(int index, Tab<INode*>& nodeTab)
{
...
void MainLayerControlManager::InsertMLC(int location, MainLayerControl* mLC)
{
	if (pblock == NULL) // never should happen, but just in case
		mLCManCD.MakeAutoParamBlocks(this);
	if (pblock)
	{
		ReferenceTarget* rA = (ReferenceTarget*)mLC;
		pblock->Insert(mainlayers, location, 1, &rA);
	}
	NotifySelectionChanged();
}

void MainLayerControlManager::DeleteNthLayer(int index)
{
	if (pblock == NULL || index < 0 || index >= GetNumMLCs())
	{
		return;
...
static INT_PTR CALLBACK DeleteWarningProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
	case WM_INITDIALOG:
...
bool MainLayerControlManager::ContinueDelete()
{
	if (askBeforeDeleting && GetCOREInterface()->GetQuietMode() == FALSE)
	{
		int what = DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_ASKBEFOREDELETING), GetCOREInterface()->GetMAXHWnd(),
				DeleteWarningProc, 0);
		if (what > 0)
			return true;
		else
...
void MainLayerControlManager::DeleteNthLayerNodes(int index, Tab<INode*>& nodeTab)
{
	if (index == 0) // don't delete the base layer
		return;
	Tab<MainLayerControlManager::LCInfo> existingLayers;
	MainLayerControl* mLC = GetNthMLC(index);
	LayerControl* lC1 = nullptr;
	LayerControl* lC2 = nullptr;
...
void MainLayerControlManager::SetLayerActive(int index)
{
	if (pblock == nullptr || index < 0 || index >= this->GetNumMLCs())
		return;

	auto* mLC = GetNthMLC(index);
	if (mLC == nullptr)
	{
		return;
	}
...
int MainLayerControlManager::GetLayerActive()
{
	return -1; // activeLayer;
}

int MainLayerControlManager::GetNthMLC(TSTR& name)
{
	MainLayerControl* mLC = nullptr;
	int numMLCs = GetNumMLCs();
	for (int i = 0; i < numMLCs; ++i)
	{
		mLC = GetNthMLC(i);
...
int MainLayerControlManager::GetMLCIndex(TSTR& name)
{
	MainLayerControl* mLC = nullptr;
	int numMLCs = GetNumMLCs();
	for (int i = 0; i < numMLCs; ++i)
...
int MainLayerControlManager::GetMLCIndex(MainLayerControl* m)
{
	MainLayerControl* mLC = nullptr;
	int numMLCs = GetNumMLCs();
	for (int i = 0; i < numMLCs; ++i)
...
void MainLayerControlManager::SaveExistingMLCs()
{
	int count = GetNumMLCs();
	for (int j = 0; j < count; ++j)
	{
		auto* mLC = GetNthMLC(j);
		if (mLC)
		{
			MainLayerControl* null_ptr = nullptr;
			existingMainLayerControls.Append(1, &null_ptr);
			ReplaceReference(existingMainLayerControls.Count(), static_cast<RefTargetHandle>(mLC));
		}
	}
}

void MainLayerControlManager::RevertExistingMLCs()
{
	int existing_count = existingMainLayerControls.Count();
	if (existing_count == 0)
	{
		NotifySelectionChanged();
		return;
	...
	std::unordered_map<MainLayerControl*, int> mainLayerControls;
	typedef std::pair<MainLayerControl*, int> MotionLayerControl_Index;
	std::unordered_map<std::wstring, MotionLayerControl_Index> mainLayerControlsByName;
	int mlc_count = GetNumMLCs();
	for (int j = 0; j < mlc_count; ++j)
	{
		MainLayerControl* mLC = GetNthMLC(j);
		if (mLC)
		{
			mainLayerControls.insert(MotionLayerControl_Index(mLC,j));
			std::wstring mlcName = mLC->GetName().data();
			mainLayerControlsByName.insert(
					std::pair<std::wstring, MotionLayerControl_Index>(mlcName, MotionLayerControl_Index(mLC, j)));
		}
	}

	for (int i = 0; i < existing_count; ++i)
	{
		MainLayerControl* mLC = existingMainLayerControls[i];
		// only reinsert if it doesn't exist...
		MainLayerControl* testMLC = nullptr;
		int testMLC_index = -1;
		// MAXX-62499 - fix slow code when large number of clip associations. Original code searched
		// by MainLayerControl* and by MainLayerControl name, and was using first one found. 
		// Following code is set up to match that behavior
		auto iter = mainLayerControls.find(mLC);
		if (iter != mainLayerControls.end())
		{
			testMLC = iter->first;
			testMLC_index = iter->second;
		}
		std::wstring mlcName = mLC->GetName().data();
		auto iter_by_name = mainLayerControlsByName.find(mlcName);
		if (iter_by_name != mainLayerControlsByName.end())
		{
			int index = iter_by_name->second.second;
			if (testMLC_index == -1 || index < testMLC_index)
			{
				testMLC = iter_by_name->second.first;
				testMLC_index = index;
			}
		}
		...
		if (testMLC == nullptr)
		{
			...
			mainLayerControls.insert(MotionLayerControl_Index(mLC, newIndex));
			mainLayerControlsByName.insert(
					std::pair<std::wstring, MotionLayerControl_Index>(mlcName, MotionLayerControl_Index(mLC, newIndex)));
		...
		else
		{
			...
			// first though make sure that the found testMLC isn't equal to mLC, which can happen with xref objects with layers.
			if (testMLC != mLC)
			{
				...
				for (int k = 0; k < mLC->layers.Count(); ++k)
				{
					...
					if (lC)
					{
						for (int z = 0; z < lC->mLCs.Count(); ++z)
						{
							if (lC->mLCs[z] == i) // i is old index!
							{
								lC->mLCs[z] = testMLC_index; // testMLC_index is the new index
								break; // only one layer per object!
							}
						}
						testMLC->AddLayerControl(lC);
						// now we need to make sure that the existing mlc has the layers added to it
						// ZCQ: some times it won't find the node for example if the layer control is used as a Custom
						// Attributes' controller then when enumerating its references, you won't find any node
						// reference because CustomAttributeContainer is not hold by any reference maker but as a
						// property of animatable base object... so we try to add the layerControl no matter a node
						// reference maker is found and we will also make sure the valid nodes monitored by mLC be added
						// to testMLC in the codes below. append node from INodeMonitor. It is safe these nodes&controls
						// are monitored by reference system we don't need worry adding invalid ones or they became
						// invalid later.
						GetNodesProc dep;
				...
				int nodeCount = mLC->monitorNodes.Count();
				for (int k = 0; k < nodeCount; ++k)
				{
					if (mLC->monitorNodes[k])
					{
						INodeMonitor* nm =
								static_cast<INodeMonitor*>(mLC->monitorNodes[k]->GetInterface(IID_NODEMONITOR));
						INode* newNode = nm->GetNode();
						if (newNode)
							testMLC->AddNode(newNode);
					}
				}
			}
		}
	...
	existingMainLayerControls.ZeroCount();
	NotifySelectionChanged();

}

class MainLayerControlManagerPostLoadCallback : public PostLoadCallback
{
public:
	MainLayerControlManager* s;
	MainLayerControlManagerPostLoadCallback(MainLayerControlManager* r)
	{
		s = r;
	}
	void proc(ILoad* iload);
...
void MainLayerControlManagerPostLoadCallback::proc(ILoad* iload)
{
	int count = s->GetNumMLCs();
	if (count != 0)
	{
		int numMLCsRemoved = 0;
		// ResetLayerMLCs call is expensive. If no nodes are being affected by current set of main layer controls, don't need to call when one gets deleted
		bool doResetLayerMLCs;
		{
			Tab<INode*> nodes;
			s->GetAllNodes(nodes);
			doResetLayerMLCs = nodes.Count() != 0;
		}

		for (int j = count - 1; j > -1; --j)
		{
			MainLayerControl* mLC = s->GetNthMLC(j);
			if (mLC == nullptr)
			{
				s->DeleteNthMLC(j);
				numMLCsRemoved++;
			}
			else if (j != 0 || (s->GetNumMLCs() == 1)) // not base layer or only have base layer at this point
			{
				bool hasLayerControls = false;
				for (int j = 0; j < mLC->layers.Count(); ++j)
				{
					if (mLC->layers[j] != nullptr)
					{
						hasLayerControls = true;
						break;
					}
				}

				if (!hasLayerControls) // should be deleted
				{
					s->DeleteNthMLC(j);
					if (doResetLayerMLCs)
						s->ResetLayerMLCs(j);
				...
				else
				{
					...
					for (int j = mLC->layers.Count() - 1; j >= 0; --j)
					{
						if (mLC->layers[j] == nullptr)
						{
							mLC->layers.Delete(j, 1);
					...
					for (int j = mLC->monitorNodes.Count() - 1; j >= 0; --j)
					{
						INode* node = nullptr;
						if (mLC->monitorNodes[j])
						{
		...
		if (numMLCsRemoved != 0)
		{
			TSTR msgFmt = GetString(IDS_N_MAINLAYERCONTROLSREMOVED);
			GetCOREInterface()->Log()->LogEntry(SYSLOG_INFO, NO_DIALOG, nullptr, msgFmt.data(), numMLCsRemoved);
			mprintf(msgFmt.data(), numMLCsRemoved);
			iload->SetObsolete();
		}
	}
	delete this;
...
IOResult MainLayerControlManager::Load(ILoad* iload)
{

	MainLayerControlManagerPostLoadCallback* ctrlcb = new MainLayerControlManagerPostLoadCallback(this);
...
IOResult MainLayerControlManager::Save(ISave* isave)
{
	ULONG nb;
	int v;
...
void MainLayerControlManager::MainLayer_DeleteMe()
{
...
void MainLayerControlManager::GetLayerControlsInternal(Tab<INode*>& nodes, Tab<LCInfo>& existingLayers)
{
...
void MainLayerControlManager::GetLayerControls(Tab<INode*>& incomingNodes, Tab<LCInfo>& existingLayers)
{
	...
	// go through and get all of the
	GetLayerControlsInternal(nodes, existingLayers);
}
...
void MainLayerControlManager::MakeLayerControl(TrackViewPick res, LayerControl* existingLayer, INode* node)
{
	...
	Control* newClone = current; // dont clone, in case we have wires, expose tm, mixer, etc..//static_cast<Control
								 // *>(current->Clone());
	// set the base as the first layer.

	// now we need to sync up the layer and it's mains, if they exist, otherwise,make sure the layer has the same
	// number of controls.
	int active = 0;
	if (existingLayer == NULL) // no existing layer so we need to make a mainlayercontrol, set it up
	{
		MainLayerControl* mLC = GetNthMLC(0);
		if (mLC == nullptr)
		{
			mLC = static_cast<MainLayerControl*>(
					GetCOREInterface()->CreateInstance(CTRL_USERTYPE_CLASS_ID, MAINLAYER_CONTROL_CLASS_ID));
			MSTR baseName = GetString(IDS_BASE_LAYER);
...
BOOL MainLayerControlManager::CanEnableAnimLayer(
		ReferenceTarget* anim, ReferenceTarget* client, int subNum, INode* node, XMLAnimTreeEntryList* list)
{
...
BOOL MainLayerControlManager::CanEnableAnimLayer(ReferenceTarget* anim, ReferenceTarget* client, int subNum)
{

	if (anim == NULL || client == NULL || subNum < 0 || subNum >= client->NumSubs() || anim != client->SubAnim(subNum))
	{
		return FALSE;
	}
...
BOOL MainLayerControlManager::EnableAnimLayer(ReferenceTarget* anim, ReferenceTarget* client, int subNum)
{
...
LayerControl* MainLayerControlManager::BuildLayerControl(TrackViewPick res)
{
	LayerControl* list = NULL;
	if ((res.anim->SuperClassID() == CTRL_FLOAT_CLASS_ID))
	{
		list = static_cast<LayerControl*>(
				GetCOREInterface()->CreateInstance(CTRL_FLOAT_CLASS_ID, (FLOATLAYER_CONTROL_CLASS_ID)));
	}
	else if ((res.anim->SuperClassID() == CTRL_POSITION_CLASS_ID))
	{
		list = static_cast<LayerControl*>(
				GetCOREInterface()->CreateInstance(CTRL_POSITION_CLASS_ID, (POSLAYER_CONTROL_CLASS_ID)));
	}
	else if ((res.anim->SuperClassID() == CTRL_POINT3_CLASS_ID))
	{
		list = static_cast<LayerControl*>(
				GetCOREInterface()->CreateInstance(CTRL_POINT3_CLASS_ID, (POINT3LAYER_CONTROL_CLASS_ID)));
	}

	else if ((res.anim->SuperClassID() == CTRL_ROTATION_CLASS_ID))
	{
		list = static_cast<LayerControl*>(
				GetCOREInterface()->CreateInstance(CTRL_ROTATION_CLASS_ID, (ROTLAYER_CONTROL_CLASS_ID)));
	}
	else if ((res.anim->SuperClassID() == CTRL_SCALE_CLASS_ID))
	{
		list = static_cast<LayerControl*>(
				GetCOREInterface()->CreateInstance(CTRL_SCALE_CLASS_ID, (SCALELAYER_CONTROL_CLASS_ID)));
	}
	return list;
}


void MainLayerControlManager::GetActiveLayersNodes(Tab<INode*>& nodeTab, Tab<int>& layers)
{
	layers.ZeroCount();
	Tab<MainLayerControlManager::LCInfo> layerControls;
	GetLayerControls(nodeTab, layerControls);
...
void MainLayerControlManager::SelectNodesFromActive()
{
	if (activeLayers.Count() == 1)
	{
		Tab<INode*> nodeTab;
...
bool MainLayerControlManager::ShowSelectNodesFromActive()
{
	if (activeLayers.Count() == 1)
		return TRUE;
	return FALSE;
}
void MainLayerControlManager::GetNodesActiveLayer(Tab<INode*>& nodeTab)
{
	nodeTab.ZeroCount();
	MainLayerControl* mLC = NULL;
	int newFlagBit = -1;
...
void MainLayerControlManager::GetAllNodes(Tab<INode*>& nodeTab)
{
	nodeTab.ZeroCount();
	MainLayerControl* mLC = NULL;
	int newFlagBit = -1;  // this is used to ensure nodeTab has unique entries. Use a std::unordered_set<INode*> instead?
	int numMLCs = GetNumMLCs();
	for (int i = 0; i < numMLCs; ++i)
	{
		mLC = GetNthMLC(i);
...
bool MainLayerControlManager::GetLayerInternalFrozen(int index)
{
	if (index >= 0 && index < GetNumMLCs())
	{
		MainLayerControl* mLC = GetNthMLC(index);
		if (mLC)
			return mLC->GetLayerInternalFrozen();
	}

	return false;
}

void MainLayerControlManager::SetLayerInternalFrozen(int index, bool frozen)
{
	if (index >= 0 && index < GetNumMLCs())
	{
		MainLayerControl* mLC = GetNthMLC(index);
		if (mLC)
		{
			mLC->SetLayerInternalFrozen(frozen);
			mLC->SetLocked(frozen);
		}
	}
}

void MainLayerControlManager::CopyLayerNodes(int index, Tab<INode*>& nodeTab)
{
	if (index < 0)
		return;
	Tab<MainLayerControlManager::LCInfo> existingLayers;
...
void MainLayerControlManager::PasteLayerNodes(int index, Tab<INode*>& nodeTab)
{
	...
	if (index == -1)
	{
		...
		IAnimLayerControlManager* IAnimMan =
				static_cast<IAnimLayerControlManager*>(GetCOREInterface(IANIMLAYERCONTROLMANAGER_INTERFACE));
	...
	{
		// don't paste over a non-existent or locked layer
		MainLayerControl* mLC = GetNthMLC(index);
		if (mLC == nullptr || mLC->GetLocked() == true)
			return;
	}
	...
	// get the mainlayercontrol of the clip we are pasting too, soo we can lock it if needed.
	// we also get one control, doesn't matter which, that was pasted. If that control is locked then so is the new
	// mainlayercontrol
	MainLayerControl* mLC = GetNthMLC(index);
	Control* pasted = NULL;
	for (int j = 0; j < existingLayers.Count(); ++j)
...
void MainLayerControlManager::CollapseLayerNodes(int index, Tab<INode*>& nodeTab)
{
	if (index <= 0)
		return;
	Tab<MainLayerControlManager::LCInfo> existingLayers;
	...
	if (mLC && mLC->mute == false && mLC->GetLocked() == false) // not muted or locked at all
	{
		...
		if (somethingCollapsed)
		{
			...
			BroadcastNotification<NOTIFY_ANIM_LAYERS_DISABLED>(&nodeTab);
		}
		if (theHold.Holding())
			theHold.Put(new NotifySelectionRestore());
	}
...
class IAnimLayerControlManager_Imp : public IAnimLayerControlManagerInternal
{

public:
	static IAnimLayerControlManager& GetInstance()
	{
		return _theInstance;
	}
	...
	enum
	{
		kShowAnimLayersManagerToolbar,
		kEnableAnimLayersDlg,
		kEnableAnimLayers_FPS,
		kCanEnableAnimLayer,
		kEnableAnimLayer,
		kGetLayerCount,
		kGetNodesLayers,
		kSetLayerActive,
		kSetLayerActiveNodes,
		kGetActiveLayersNodes,
		kGetNodesActiveLayer,
		kAddLayer,
		kAddLayerDlg,
		kDeleteLayer,
		kDeleteLayerNodes,
		...
		kCollapseLayerNodes,
		kDisableLayerNodes,
		kSetFilterActiveOnlyTrackView,
		kGetFilterActiveOnlyTrackView,
		kSetJustUpToActive,
		kGetJustUpToActive,
		kAnimLayerPropertiesDlg,
		kSetCollapseControllerType,
		kGetCollapseControllerType,
		kSetCollapsePerFrame,
		kGetCollapsePerFrame,
		kSetCollapsePerFrameActiveRange,
		kGetCollapsePerFrameActiveRange,
		kSetCollapseRange,
		kGetCollapseRange,
		kEnum,
		kSetLayerWeightControl,
		kGetLayerWeightControl,
		kGetLayerLocked,
		kSetLayerLocked,
		kRefreshAnimLayersManagerToolbar
	};
	...
	FN_10(kEnableAnimLayers_FPS, TYPE_INT, EnableAnimLayers_FPS, TYPE_INODE_TAB_BR, TYPE_BOOL, TYPE_BOOL, TYPE_BOOL,
			TYPE_BOOL, TYPE_BOOL, TYPE_BOOL, TYPE_BOOL, TYPE_BOOL, TYPE_BOOL);
	...
	PROP_FNS(kGetFilterActiveOnlyTrackView, GetFilterActiveOnlyTrackView, kSetFilterActiveOnlyTrackView,
			SetFilterActiveOnlyTrackView, TYPE_bool);
	...
	int EnableAnimLayers_FPS(Tab<INode*>& nodeTab, BOOL pos, BOOL rot, BOOL scale, BOOL iK, BOOL object, BOOL cA,
			BOOL mode, BOOL mat, BOOL other);
	...
	// These 3 functions are added in the Phoenix for XRef extension.
	bool GetLayerInternalFrozen(int index);
	void SetLayerInternalFrozen(int index, bool frozen);
	void AddLayerForNode(MSTR& name, ILayerControl* pSourceLayerCtrl, INode* pNode, bool useActiveControllerType);
	//
...
bool LayerManagerIndexValidator::Validate(FPInterface* fpi, FunctionID fid, int paramNum, FPValue& val, TSTR& msg)
{
	...
	if (val.i < lowerMargin)
	{
		msg.printf(GetString(IDS_MZ_LOWER_INDEX_ERROR), lowerMargin + 1); /*End up calling _vsnwprintf_s*/ /* Flawfinder: ignore */
		return false;
	}

	int highMargin = static_cast<IAnimLayerControlManager*>(fpi)->GetLayerCount();
	if (fid == IAnimLayerControlManager_Imp::kPasteLayerNodes)
	{
		highMargin++;
	}
	...
	if (val.i >= highMargin)
	{
		msg.printf(GetString(IDS_MZ_UPPER_INDEX_ERROR), ((IAnimLayerControlManager*)fpi)->GetLayerCount()); /*End up calling _vsnwprintf_s*/ /* Flawfinder: ignore */
		return false;
	}
...
IAnimLayerControlManager_Imp IAnimLayerControlManager_Imp::_theInstance(IANIMLAYERCONTROLMANAGER_INTERFACE,
		_T("AnimLayerManager"), 0, NULL, FP_CORE, 
	...
	IAnimLayerControlManager_Imp::kGetLayerWeightControl, _T("getLayerWeightControl"), 0, TYPE_CONTROL, 0, 1,
		_T("listIndex"), 0, TYPE_INDEX, f_validator, &layerManagerValidator,
	IAnimLayerControlManager_Imp::kSetLayerWeightControl, _T("setLayerWeightControl"), 0, TYPE_bool, 0, 2,
		_T("listIndex"), 0, TYPE_INDEX, f_validator, &layerManagerValidator, 
...
{
	QString toolBarName(QString::fromWCharArray(GetString(IDS_ANIMATION_LAYERS)));
	QToolBar* tb = GetCOREInterface()->GetQmaxMainWindow()->findChild<QToolBar*>(toolBarName);
	if (tb)
	{
		tb->setVisible(show);
	}
...
{
	if (DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_ENABLE_ANIM_LAYERS), GetCOREInterface()->GetMAXHWnd(),
				enableAnimFiltersProc, 0))
	{
		DWORD flag = 0;
...
{
	auto* spin = GetISpinner(GetDlgItem(hDlg, spinID));
...
void IAnimLayerControlManager_Imp::AnimLayerPropertiesDlg()
{
	...
	DialogBoxParam(
			hInstance, MAKEINTRESOURCE(IDD_LAYER_PROPERTIES), GetCOREInterface()->GetMAXHWnd(), layerPropertiesProc, 0);
}
...
void IAnimLayerControlManager_Imp::AddLayerDlg(Tab<INode*>& nodeTab)
{
	...
	if (DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_ADD_LAYER), GetCOREInterface()->GetMAXHWnd(), addLayerProc,
				(LPARAM)&addLayerInfo))
	{
...
const TCHAR* IAnimLayerControlManager_Imp::GetComboToolTip()
{
	MainLayerControlManager* man = GetMLCMan();
	return man->GetComboToolTip();
}

bool MakeSureWeCanMakeLayer(XMLAnimTreeEntryList* list, int item, MaxSDK::Array<TrackViewPick>& newLayersToMake)
{
	...
	for (int i = 0; i < parentIndices.Count(); ++i)
	{
		...
		if (IsLayerControl(parent->GetAnim()))
			return false;
		for (int j = 0; j < newLayersToMake.length(); ++j)
		{
...
{
	MaxSDK::Array<NodeAndAnims> nodeAnims;
	MaxSDK::Array<TrackViewPick> newLayersToMake;
	...
	for (int z = 0; z < nodeTab.Count(); ++z)
	{
		...
		Control* pNodeTMController = node->GetTMController();
		// if the node control is a xref control(even nested), we need get the src controller!
		if (pNodeTMController)
		{
			IXRefCtrl* pXRefCtrl = (IXRefCtrl*)pNodeTMController->GetInterface(IID_XREF_CTRL);
			if (pXRefCtrl)
			{
				pNodeTMController = pXRefCtrl->GetSourceCtrl(true);
			}
		}
		...
		for (int i = 0; i < nodeAnims.length(); ++i)
		{
			for (int j = 0; j < nodeAnims[i].GetList()->Count(); ++j)
			{
				...
				// if bipDriver control.. continue
				if (GetBipDriverInterface(anim))
				{
					continue;
				}
				if (GetControlInterface(anim) != NULL)
				{
					...
					// if parent is the transform controller.or the controller belong to Xref controller.
					if (pNodeTMController == res.client)
					{
						if (superClass == CTRL_POSITION_CLASS_ID && (IAnimLayerControlManager::ePos & filter))
						{
							if (MakeSureWeCanMakeLayer(list, j, newLayersToMake) == true)
								newLayersToMake.append(res);
							continue;
						}
						else if (superClass == CTRL_ROTATION_CLASS_ID && (IAnimLayerControlManager::eRot & filter))
						{
							if (MakeSureWeCanMakeLayer(list, j, newLayersToMake) == true)
								newLayersToMake.append(res);
							continue;
						}
						else if (superClass == CTRL_SCALE_CLASS_ID && (IAnimLayerControlManager::eScale & filter))
						{
							if (MakeSureWeCanMakeLayer(list, j, newLayersToMake) == true)
								newLayersToMake.append(res);
							continue;
					...
					if ((IAnimLayerControlManager::eObject & filter) && (entry->IsType(XMLAnimTreeEntry::eBaseObject)))
					{
						if (control->IsLeaf())
						{
							if (MakeSureWeCanMakeLayer(list, j, newLayersToMake) == true)
						...
						else if (superClass == CTRL_POSITION_CLASS_ID && (IAnimLayerControlManager::ePos & filter))
						{
							if (MakeSureWeCanMakeLayer(list, j, newLayersToMake) == true)
								newLayersToMake.append(res);
							continue;
					...
					else if ((IAnimLayerControlManager::eMod & filter) && (entry->IsType(XMLAnimTreeEntry::eModObject)))
					{
						if (control->IsLeaf())
						{
							if (MakeSureWeCanMakeLayer(list, j, newLayersToMake) == true)
					...
					else if ((IAnimLayerControlManager::eIK & filter) && (entry->IsType(XMLAnimTreeEntry::eIK)))
					{
						if (control->IsLeaf())
						{
							if (MakeSureWeCanMakeLayer(list, j, newLayersToMake) == true)
					...
					else if ((IAnimLayerControlManager::eMat & filter) && (entry->IsType(XMLAnimTreeEntry::eMatParams)))
					{
						if (control->IsLeaf())
						{
							if (MakeSureWeCanMakeLayer(list, j, newLayersToMake) == true)
		...
		if (newLayersToMake.length() > 0)
		{
			...
			MainLayerControlManager* man = GetMLCMan();
			for (int i = 0; i < newLayersToMake.length(); ++i)
			{
	...
	{
		BroadcastNotification<NOTIFY_ANIM_LAYERS_ENABLED>(&nodeTab);
		MainLayerControlManager* man = GetMLCMan();
		man->NotifySelectionChanged();
		if (theHold.Holding())
			theHold.Put(new NotifySelectionRestore());
...
bool IAnimLayerControlManager_Imp::GetLayerInternalFrozen(int index)
{
	MainLayerControlManager* man = GetMLCMan();
	return man->GetLayerInternalFrozen(index);
}
...
{
	MainLayerControlManager* man = GetMLCMan();
	MainLayerControl* mLC = man->GetNthMLC(index);
	TSTR name;
	if (mLC)
	{
		name = mLC->GetName();
...
void IAnimLayerControlManager_Imp::SetLayerInternalFrozen(int index, bool frozen)
{
	MainLayerControlManager* man = GetMLCMan();
	man->SetLayerInternalFrozen(index, frozen);
}
...
void IAnimLayerControlManager_Imp::AddLayerForNode(
		MSTR& name, ILayerControl* pSourceLayerCtrl, INode* pNode, bool useActiveControllerType)
{
	MainLayerControlManager* man = GetMLCMan();
	man->AddLayerForNode(name, pSourceLayerCtrl, pNode, useActiveControllerType);
}

// used internal for adding layers to temp nodes created when xrefing nodes from other files.
void MainLayerControlManager::AddLayerForNode(
		MSTR& name, ILayerControl* pSourceLayerCtrl, INode* pNode, bool useActiveControllerType)
{
	Tab<MainLayerControlManager::LCInfo> existingLayers;
	MainLayerControlManager* man = GetMLCMan();
	...
	DbgAssert(theHold.IsSuspended()); // since used during xref objects, hold should be suspended.
	...
	if (mLC && pNode)
	{
		oneAtTime.ZeroCount();
		existingLayers.ZeroCount();
		oneAtTime.Append(1, &pNode);
		// since the input nodes may be a temp node during xref process, it is not monitored by any MLC
		// so use the internal version.
		man->GetLayerControlsInternal(oneAtTime, existingLayers);
		for (int i = 0; i < existingLayers.Count(); ++i)
		{
			LayerControl* lC = existingLayers[i].lC;
			if (lC != static_cast<LayerControl*>(pSourceLayerCtrl))
			{
				continue;
			}
...
void AnimLayerToolbarControl::EnableDisableWeight()
{
	...
	else
	{
		...
		MainLayerControl* mLC = man->GetNthMLC(sel);
		spin->SetKeyBrackets(mLC->pblock->KeyFrameAtTimeByID(kLayerCtrlWeight, GetCOREInterface()->GetTime(), sel));
	}
...
void AnimLayerToolbarControl::SetSelected(int which)
{
	int i;
	MainLayerControlManager* man = GetMLCMan();
	int numLayers = man->currentLayers.Count();
	...
	DbgAssert(((which < numLayers) || (numLayers == 0)) && (which >= 0));
	if (which >= numLayers)
		which = numLayers - 1;
	if (which < 0)
		which = 0;
	man->SetLayerActiveNodes(man->currentLayers[which], nodeTab);
	macroRecorder->FunctionCall(
			_T("AnimLayerManager.setLayerActiveNodes"), 2, 0, mr_int, man->currentLayers[which] + 1, mr_sel);
...
TSTR AnimLayerToolbarControl::FormatName(TSTR& name, int nLen2, HWND hDlg)
{
	...
	SIZE nLen;
	DLGetTextExtent(hDc, m_buf.data(), m_buf.Length(), &nLen);
...
void AnimLayerToolbarControl::SetComboSize(HWND hCombo)
{
	...
	for (i = 0; i < man->currentLayers.Count(); ++i)
	{
		...
		{
			if (ltcMan->GetUnLockOverride())
				name += GetString(IDS_OVERRIDEN);
			else
				name += GetString(IDS_LOCKED);
		}
		SIZE size;
		if (DLGetTextExtent(hDc, name, &size))
		{
			...
			if (size.cy > layerNameHeight)
				layerNameHeight = size.cy;
		}
	}
	...
	// If the drop down list must have a bigger Heigth
	if (layerNameHeight > SendMessage(hCombo, CB_SETITEMHEIGHT, 0, 0))
	{
		SendMessage(hCombo, CB_SETITEMHEIGHT, 0, layerNameHeight + MaxSDK::UIScaled(4));
	}
	...
	{
		int width =
				layerNameWidth + m_iconColWidth + GetSystemMetrics(SM_CXVSCROLL) + GetSystemMetrics(SM_CXBORDER) + 4;
...
void AnimLayerToolbarControl::OnDraw(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
	...
	else
	{
		...
		if (man->GetLayerLocked(which) == true)
		{
			if (ltcMan->GetUnLockOverride() == true)
				nam += GetString(IDS_OVERRIDEN);
			else
				nam += GetString(IDS_LOCKED);
		}
	...
	ExtTextOut(lpdis->hDC, xTop + 2, yTop + 2, ETO_CLIPPED | ETO_OPAQUE, &rectSub, buf, buf.length(), NULL);
...
INT_PTR MainLayerControlManager::RenameLayer(HWND hWnd, int which)
{

	if (which == 0)
	{
		TSTR title = GetString(IDS_ANIM_LAYERS);
		MaxSDK::MaxMessageBox(GetActiveWindow(), GetString(IDS_CANNOT_RENAME_BASE_LAYER), title, MB_OK);
		return 0;
	}
	return DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_LAYER_RENAME), GetCOREInterface()->GetMAXHWnd(),
			MainLayerControlManager::RenameLayerProc, (LPARAM)which);
}

INT_PTR CALLBACK MainLayerControlManager::RenameLayerProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		which = (int)lParam;
		if (which >= 0)
			SetDlgItemText(hWnd, IDC_EDIT_LAYERNAME, GetMLCMan()->GetLayerName(which).data());
		break;
	...
	case WM_COMMAND:
		...
		case IDOK:
			if (which >= 0)
			{
				...
				TSTR buf = GetWindowText(hWnd, IDC_EDIT_LAYERNAME);
				TCHAR* bufp = buf.dataForWrite();
				int len = buf.Length();
				while (len > 0 && (bufp[len - 1] == _T(' ') || bufp[len - 1] == _T('\t')))
				{
					bufp[len - 1] = _T('\0');
					len--;
				}
				while (len > 0 && (bufp[0] == _T(' ') || bufp[0] == _T('\t')))
				{
					bufp++;
					len--;
				}
				if (len == 0)
				{
					TSTR msg = GetString(IDS_LAYER_NO_NAME);
					MaxSDK::MaxMessageBox(hWnd, msg, GetString(IDS_ANIM_LAYERS), MB_OK);
					return FALSE;
				}
				TSTR newName = bufp;
				int numMLCs = GetMLCMan()->GetNumMLCs();
				for (int i = 0; i < numMLCs; ++i)
				{
					if (i != which && GetMLCMan()->GetLayerName(i) == newName)
					{
						TSTR msg;
						msg.printf(msg, GetString(IDS_LAYER_DUP_NAME), bufp);
						MaxSDK::MaxMessageBox(hWnd, msg, GetString(IDS_ANIM_LAYERS), MB_OK);
						return FALSE;
					}
				}
				if (which >= 0)
					GetMLCMan()->SetLayerName(which, newName);
			}
			EndDialog(hWnd, TRUE);
			return FALSE;
		}
		...
		case ANIMLAYERMGR_COMBOBOX_ID:
			...
			case CBN_SELCHANGE:
			{
				int sel = (int)SendMessage(
						(HWND)lParam, CB_GETITEMDATA, SendMessage((HWND)lParam, CB_GETCURSEL, 0, 0), 0);
				if (sel >= 0)
				{
					MainLayerControlManager* man = GetMLCMan();
					INodeTab nodeTab;
					...
					macroRecorder->FunctionCall(
							_T("AnimLayerManager.setLayerActiveNodes"), 2, 0, mr_int, sel + 1, mr_sel);
					theHold.Accept(GetString(IDS_SET_ACTIVE));
					theHold.SuperAccept(GetString(IDS_SET_ACTIVE));
				}
				bHandled = true;
