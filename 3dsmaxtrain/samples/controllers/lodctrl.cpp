...
class LODCtrl : public LockableStdControl{
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? LOD_CONTROL_CNAME : _T("LOD Controller");}
...
class LODUtil : 
	...
	public:
		...
		INodeTab nodes;
		int disp;
		MaxSDK::Array<GroupIDMapEntry> clonedGroupIDMap;
...
static LODUtil theLODUtil;
 ...
 class LODClonePostPatchProc: public PostPatchProc
 {
 private:
 	RemapDir* mRemap;
 public:
 	int Proc(RemapDir& remap);
 	RemapDir* GetRemapDir();
 	void SetRemapDir(RemapDir* _Remap);
 };
 static LODClonePostPatchProc theLodProc;
 
 RemapDir* LODClonePostPatchProc::GetRemapDir()
 {
 	return mRemap;
 }
 
 void LODClonePostPatchProc::SetRemapDir(RemapDir* _Remap)
 {
 	mRemap = _Remap;
 }
 
 int LODClonePostPatchProc::Proc(RemapDir &remap)
 {
 	if(mRemap)
 	{
...
class LODClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("LOD Controller"); }
	SClass_ID		SuperClassID() {return CTRL_FLOAT_CLASS_ID; }
	Class_ID		ClassID() {return LOD_CONTROL_CLASS_ID;}
...
class LODUtilClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Level of Detail"); }
...
RefTargetHandle LODCtrl::Clone(RemapDir &remap)
	...
	cont->SetValue(0,&val);
#endif
 	if(theLodProc.GetRemapDir() != &remap)
 	{
 		remap.AddPostPatchProc(&theLodProc, false);
 		theLodProc.SetRemapDir(&remap);
 	}
...
float LODCtrl::EvalVisibility(
	...
	// [dl | 13apr2004] Fix for bug#517152. mental ray needs the validity interval
	// to be correctly initialized to correctly update the visibility of the node on
	// every frame. But since we can't tell the exact validity interval of the LOD
	// controller, we set it to be only valid at the current time.
	valid &= Interval(t,t);
...
{
	clonedGroupIDMap.removeAll();
}

WORD LODUtil::FindClonedGroupID(WORD gid)
{
	for (int i=0; i<clonedGroupIDMap.length(); i++)
	{
		if (clonedGroupIDMap[i].srcID == gid)
			return clonedGroupIDMap[i].dstID;
	}
	
	// First of this group to be cloned
	WORD newID = FindUniqueGroupID();
	GroupIDMapEntry entry(gid, newID);
	clonedGroupIDMap.append(entry);
	return newID;
}
...
WORD LODUtil::FindUniqueGroupID()
{
	...
	// set the used group id in the clonedGroupIDMap.
	for (int i=0; i<clonedGroupIDMap.length(); i++) {
		used.Set(clonedGroupIDMap[i].dstID); // the srcID should already be set by CheckIDs method
	}
...
void LODUtil::CreateNewSet()
	...
	if (!grpHead) {
		...
		MaxSDK::MaxMessageBox(hWnd, buf1, buf2, MB_ICONSTOP);
		return;
		}	
	...
	MaxSDK::Array<NodeSize> nodesizes;
	for (int i=0; i < ip->GetSelNodeCount(); i++)
	{
		if (ip->GetSelNode(i)==grpHead) continue;
		if (IsAncestorSelected(ip->GetSelNode(i)->GetParentNode())) continue;
	...
	for (size_t i=0, ct = nodesizes.length(); i<ct; i++) {
		LODCtrl *cont = new LODCtrl;		
		cont->grpID    = grpID;
		cont->order    = (int)i;
		cont->viewport = i==0;
...
void LODUtil::ResetSet()
	...
	float maxFaces = float(nodesizes[nodesizes.length()-1].faces);	
	maxFaces = float(sqrt(maxFaces));

	// Reset LOD controllers		
	float prev = 0.0f;
	for (int i=0, ct = nodes.Count(); i<ct; i++) {		
