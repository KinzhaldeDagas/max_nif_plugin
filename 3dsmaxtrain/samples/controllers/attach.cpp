...
public:
	AttachCtrl* cont;
	IObjParam* ip;
	SetPosMouseProc(AttachCtrl* c, IObjParam* i)
	{
		cont = c;
		ip = i;
	}
...
int AttachCtrl::mActiveUIKeyIndex = -1;
...
public:
	...
	Class_ID ClassID()
	{
		return ATTACH_CONTROL_CLASS_ID;
	}
...
ClassDesc* GetAttachControlDesc()
{
	return &attachCD;
}
...
void AttachCtrl::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? ATTACH_CONTROL_CNAME : _T("Attachment");
}
...
IOResult AttachCtrl::Load(ILoad* iload)
{
	...
	//key clean up make sure they are sorted
	SortAndDeleteDuplicateKeys();

	//old files may have keys with duplicate times remove them
	if (keys.length() > 1)
	{
		for (int i = 0; i < keys.length() - 1; ++i)
		{
			if (keys[i].time == keys[i + 1].time) //just need to look at the next one since we are sorted
			{
				keys.removeAt(i);
				--i;
			}
...
int AttachCtrl::GetKeyTimes(Tab<TimeValue>& times, Interval range, DWORD flags)
{
	int skip = 0;
	for (int i = 0; i < keys.length(); i++)
	{
...
int AttachCtrl::GetKeySelState(BitArray& sel, Interval range, DWORD flags)
{
	int c = 0;
	int skip = 0;
	for (int i = 0; i < keys.length(); i++)
...
BOOL AttachCtrl::GetNextKeyTime(TimeValue t, DWORD flags, TimeValue& nt)
{
	...
	{
		// iterate backwards
		for (int i = (int)num_keys - 1; i >= 0; i--)
		{
			if (keys[i].time < t)
			{
				nt = keys[i].time;
...
void AttachCtrl::ScaleTime(Interval iv, float s)
{
	if (GetLocked() == false)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			{
				keys[i].time = int(s * float(keys[i].time - iv.Start())) + iv.Start();
			}
			else if (keys[i].time > iv.End())
			{
				keys[i].time += delta;
			}
		}
...
void AttachCtrl::CloneSelectedKeys(BOOL offset)
{
	if (GetLocked() == false)
	{
		...
		MaxSDK::Array<AKey> selected_keys;
		selected_keys.reserve(num_selected);
...
int AttachCtrl::HitTestTrack(TrackHitTab& hits, Rect& rcHit, Rect& rcTrack, float zoom, int scroll, DWORD flags)
{
	...
	for (int i = 0; i < n; i++)
	{
		if (flags & HITTRACK_SELONLY && !(keys[i].flags & KEY_SELECTED))
			continue;
		if (flags & HITTRACK_UNSELONLY && (keys[i].flags & KEY_SELECTED))
			continue;
...
void AttachCtrl::SortAndDeleteDuplicateKeys(bool fromUISpinner)
{
	TimeValue uiCheckTime(0);
	if (fromUISpinner)
	{		
		//just mark our active time key so we can keep tarck of it after the sort
		if ((mActiveUIKeyIndex >= 0) && (mActiveUIKeyIndex < keys.length()))
		{
			keys[mActiveUIKeyIndex].flags |= KEY_MARKED;
			uiCheckTime = keys[mActiveUIKeyIndex].time;
		}
	}
	...
	if (fromUISpinner)
	{
		//make sure the UI key is set back right
		for (int keyIndex = 0; keyIndex < keys.length(); keyIndex++)
		{
			//make sure we dont delete the active key
			if (keys[keyIndex].flags & KEY_MARKED)
			{
				mActiveUIKeyIndex = keyIndex;
				keys[keyIndex].flags &= ~KEY_MARKED;
			}
			//remove the non marked duplicates
			else if (keys[keyIndex].time == uiCheckTime)
			{
				keys.removeAt(keyIndex);
				--keyIndex;
			}
			//can bail since we are sorted if tested time is greater than our check time
			else if (keys[keyIndex].time > uiCheckTime)
				break;
		}
	}
	...
	{
		//clean out duplicate keys
		//super inefficient but there should never be more than a dozen keys here
		//Just looks for selected keys and marks and keys that have the same time as the selected kets
		for (int keyIndex = 0; keyIndex < keys.length(); keyIndex++)
		{
			//make sure we dont delete the active key
			if (keys[keyIndex].flags & KEY_SELECTED)
			{
				TimeValue checkTime = keys[keyIndex].time;
				for (int keyIndexCheck = 0; keyIndexCheck < keys.length(); keyIndexCheck++)
				{
					//make sure we dont delete the selected keys
					if (keys[keyIndexCheck].flags & KEY_SELECTED) continue;
					if (keys[keyIndexCheck].time == checkTime)
					{
						keys[keyIndexCheck].flags |= KEY_MARKED;  //mark the key to be deleted
					}
					//can bail since we are sorted if tested time is greater than our check time
					else if (keys[keyIndexCheck].time > checkTime)
						break;
		...
		for (int keyIndex = 0; keyIndex < keys.length(); keyIndex++)
		{
			...
			{
				keys.removeAt(keyIndex);
				//if we remove a key before the mActiveUIKeyIndex we must offset the mActiveUIKeyIndex also
				//since we can have the UI up but it was not directly moved so need to keep it in the same
				//spot in the array
				if (keyIndex < mActiveUIKeyIndex) 
					--mActiveUIKeyIndex;
				--keyIndex;
			}
		}
...
RefResult AttachCtrl::NotifyRefChanged(
		const Interval& valid, RefTargetHandle targ, PartID& partID, RefMessage msg, BOOL propagate)
{
	switch (msg)
	{
	case REFMSG_CHANGE:
...
Interval AttachCtrl::CompValidity(TimeValue t)
{
	...
	if (keys.length())
	{
		...
		else if (t > keys[keys.length() - 1].time)
		{
	...
	//need to take into account if the node is animated or not
	//if animated and past the keys need to recompute from scratch
	if (node) 
	{
		//our tracks are up to date but need to compute the correct validity so if time changes
		//we lookup the new value along without having to update the paths keys data
		//we are dependent on the nodes geometry so need to intersect the interval with that
		ObjectState os = node->EvalWorldState(t);
		if (os.obj) //can be null in rare edge cases
			v &= os.obj->ObjectValidity(t);
		//also since we are in world space we need to intersect it with the nodes transform interval
		Interval tmValidity(FOREVER);
		node->GetNodeTM(t, &tmValidity);
		v &= tmValidity;
	}
	return v;
}

void AttachCtrl::GetValue(TimeValue t, void* val, Interval& valid, GetSetMethod method)
{

	TimeValue oldTime = t;
	t = ApplyEase(t, valid);

	//only pull from the cache if trackValid is true since if trackvalid is false the cache is about to get thrown away
	if ((method == CTRL_RELATIVE) && trackValid) 
	{
		//only want to cache whole frames
		auto cacheResult = mTimeCache.find(t);
		if ((t%GetTicksPerFrame() == 0) && (cacheResult != mTimeCache.end())) //if we are in the cache use it
		{
			mValid.SetInstant(t);
			valid &= this->mValid;
			Matrix3* tm = (Matrix3*)val;			
			*tm = cacheResult->second; //just return the cache value
			return;
		}
	}
	if (!this->mValid.InInterval(t))
	{
		this->mValid = CompValidity(t);
		PrepareTrack(t);
		...
		if (this->mValid.Empty())
			this->mValid.Set(oldTime, oldTime);
	}
	if (method == CTRL_RELATIVE)
	{
		...
		//cache off our value if on a whole frame
		if (t%GetTicksPerFrame() == 0)
			mTimeCache[t] = *tm;
	}
	else
	{
	...
	for (int i = 0; i < keys.length(); i++)
	{
		if (keys[i].time > t)
		{
			if (i)
			{
				n0 = i - 1;
				n1 = i;
				u = float(t - keys[i - 1].time) / float(keys[i].time - keys[i - 1].time);
...
Point3 AttachCtrl::PointOnPath(TimeValue t)
{
	...
	else if (numberKeys)
	{
		...
		p = keys[n0].pos * v[0] + keys[n1].pos * v[1] + keys[n0].dout * v[2] + keys[n1].din * v[3];
	}
	return p;
}
...
Quat AttachCtrl::QuatOnPath(TimeValue t)
{
	...
	else if (numberKeys)
	{
		...
		q = squadrev(ang, axis, keys[n0].quat, keys[n0].qa, keys[n1].qb, keys[n1].quat, u);
	}
	return q;
}
...
{
	//we only store the path between the keys so if we only have 1 key
	//we need to continually update the paths since it is outside the 
	//paths range.
	//NOTE this causes a bug that if you have multiple keys and are outside the range
	//of the paths range and have more than 1 key the object will stop 
	//following so this really does need to be fixed.	
	size_t keysLength = keys.length();
	if (trackValid && (keysLength != 1))
	{
		return;
	}

	if (!trackValid) //if are tracks are actually invalid flush our cache
		mTimeCache.clear();

	trackValid = TRUE;
	if (!node)
		return;
	if (manUpdate && !doManUpdate)
		return;
	doManUpdate = FALSE;
	...
	for (int i = 0; i < keysLength; i++)
	{
		...
		// Evaluate the node
		ObjectState os = node->EvalWorldState(keysLength == 1 ? t : k.time);
		TriObject* obj = NULL;
		BOOL needsDel = FALSE;		
		...
		{
			if (os.obj->CanConvertToType(triObjectClassID)) //major performance hit should have native support for at least PolyObject also
			{
				Object* oldObj = os.obj;
				obj = (TriObject*)os.obj->ConvertToType(keysLength == 1 ? t : k.time, triObjectClassID);
				needsDel = (obj != oldObj);
			}
		}
		...
		{
			Interval iv = FOREVER;
			Matrix3 tm = node->GetObjTMAfterWSM(keysLength == 1 ? t : k.time, &iv);
			Mesh& m = obj->GetMesh();
			if (m.getNumFaces())
			{
				...
				// new code from Larry!
				//might be able to optimize this a bit and store off local space
				//so if the object is only tranforming and not deforming we only 
				//have to translate back to world space
				Point3 v0 = m.verts[m.faces[f].v[0]] * tm;
				Point3 v1 = m.verts[m.faces[f].v[1]] * tm;
				Point3 v2 = m.verts[m.faces[f].v[2]] * tm;
				k.pos = v0 * k.u0 + v1 * k.u1 + v2 * (1.0f - k.u0 - k.u1);
	...
	//maxscript can add keys that bypass the normal UI and can end up with duplicates so need
	//to check for those on an update
	if (keys.length() > 1)
	{
		for (int i = 0; i < keys.length() - 1; ++i)
		{
...
void AttachCtrl::CompFirstDeriv()
{
	...
	keys[0].dout = t * (3.0f * (keys[1].pos - keys[0].pos) - keys[1].din);
}
...
void AttachCtrl::CompLastDeriv()
{
	...
	keys[n - 1].din = -t * (3.0f * (keys[n - 2].pos - keys[n - 1].pos) + keys[n - 2].dout);
}
...
void AttachCtrl::BeginEditParams(IObjParam* ip, ULONG flags, Animatable* prev)
{
	if (GetLocked() == false)
	{
		RegisterClasses();
		...
		editCont = this;
		uiValid = FALSE;
		hWnd = ip->AddRollupPage(hInstance, MAKEINTRESOURCE(IDD_ATTACH_PARAMS), AttachParamDialogProc,
				GetString(IDS_RB_ATTACHPARAMS), (LPARAM)this);
...
void AttachCtrl::EndEditParams(IObjParam* ip, ULONG flags, Animatable* next)
{
...
void AttachCtrl::SpinnerChange(int id)
{
	HoldTrack();
	// assert(index>=0);
	if (mActiveUIKeyIndex < 0)
	{
		AddNewKey(ip->GetTime(), 0);
		mActiveUIKeyIndex = GetKeyIndex(ip->GetTime());
	}
	if (mActiveUIKeyIndex >= 0)
	{
		switch (id)
		...
		{
			TimeValue newTime = iTime->GetIVal();

			keys[mActiveUIKeyIndex].time = newTime;
			ip->SetTime(newTime);

			//need to resort since key times can be out of order
			//and maybe delete duplicate keys
			SortAndDeleteDuplicateKeys(true);

			break;
		}
		case IDC_ATTACH_KEYFACESPIN:
			keys[mActiveUIKeyIndex].face = iFace->GetIVal() - 1;
			break;
		case IDC_ATTACH_KEYASPIN:
			keys[mActiveUIKeyIndex].u0 = iA->GetFVal();
			UpdateBaryGraph();
			break;
		case IDC_ATTACH_KEYBSPIN:
			keys[mActiveUIKeyIndex].u1 = iB->GetFVal();
			UpdateBaryGraph();
			break;
		case IDC_ATTACH_KEYTENSSPIN:
			keys[mActiveUIKeyIndex].tens = FromTCBUI(iTens->GetFVal());
			UpdateTCBGraph();
			break;
		case IDC_ATTACH_KEYCONTSPIN:
			keys[mActiveUIKeyIndex].cont = FromTCBUI(iCont->GetFVal());
			UpdateTCBGraph();
			break;
		case IDC_ATTACH_KEYBIASSPIN:
			keys[mActiveUIKeyIndex].bias = FromTCBUI(iBias->GetFVal());
			UpdateTCBGraph();
			break;
		case IDC_ATTACH_KEYEASETOSPIN:
			keys[mActiveUIKeyIndex].easeIn = FromEaseUI(iEaseTo->GetFVal());
			UpdateTCBGraph();
			break;
		case IDC_ATTACH_KEYEASEFROMSPIN:
			keys[mActiveUIKeyIndex].easeOut = FromEaseUI(iEaseFrom->GetFVal());
			UpdateTCBGraph();
			break;
		}
	}
...
void AttachCtrl::Command(int id, LPARAM lParam)
{
	...
	case IDC_PREVKEY:
		...
		if (mActiveUIKeyIndex < 0)
		{
			...
			{
				mActiveUIKeyIndex = (int)keys.length() - 1;
			}
			...
			else
			{
				for (int kk = 0; kk < keys.length(); ++kk)
				{
					...
					{
						mActiveUIKeyIndex = kk - 1;
						break;
					}
				}
			}
		}
		else if (mActiveUIKeyIndex == 0)
			mActiveUIKeyIndex = (int)keys.length() - 1;
		else
			mActiveUIKeyIndex--;
		ip->SetTime(keys[mActiveUIKeyIndex].time);
		break;
		...
		if (mActiveUIKeyIndex < 0)
		{
			...
			else
			{
				for (int kk = 0; kk < keys.length(); ++kk)
				{
					if (ip->GetTime() < keys[kk].time)
					{
						mActiveUIKeyIndex = kk;
						break;
					}
				}
			}
		}
		else if (mActiveUIKeyIndex == keys.length() - 1)
			mActiveUIKeyIndex = 0;
		else
			mActiveUIKeyIndex++; // if (index >= 0)
		ip->SetTime(keys[mActiveUIKeyIndex].time);
		break;
	...
	case IDC_ATTACH_SETPOS:
		...
		if (!setPosButton && mActiveUIKeyIndex < 0)
		{
			iFace->Disable();
			iA->Disable();
			iB->Disable();
	...
	case IDC_ATTACH_FACEPOSGRAPH:
		...
		case BARYGRAPH_MOVE:
		{
			HoldTrack();
			Point2 bary;
			SendDlgItemMessage(hWnd, IDC_ATTACH_FACEPOSGRAPH, WM_GETBARY, 0, (LPARAM)&bary);
			keys[mActiveUIKeyIndex].u0 = bary.x;
			keys[mActiveUIKeyIndex].u1 = bary.y;
			Invalidate();
			NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
			UpdateBaryGraph();
			ip->RedrawViews(ip->GetTime());
...
{
	if (mActiveUIKeyIndex >= 0 && hWnd)
	{
		TCBGraphParams tp;
		tp.tens = keys[mActiveUIKeyIndex].tens;
		tp.cont = keys[mActiveUIKeyIndex].cont;
		tp.bias = keys[mActiveUIKeyIndex].bias;
		tp.easeTo = keys[mActiveUIKeyIndex].easeIn;
		tp.easeFrom = keys[mActiveUIKeyIndex].easeOut;
...
void AttachCtrl::UpdateBaryGraph()
{
	if (mActiveUIKeyIndex >= 0 && hWnd)
	{
		Point2 pt(keys[mActiveUIKeyIndex].u0, keys[mActiveUIKeyIndex].u1);
...
void AttachCtrl::UpdateUI()
{
	if (!uiValid)
	{
		...
		else
		{
			...
			TSTR buf;
			buf.printf(_T("%d"), mActiveUIKeyIndex + 1);
			iStat->SetText(buf);

			iTime->SetValue(keys[mActiveUIKeyIndex].time, FALSE);
			iFace->SetValue((int)keys[mActiveUIKeyIndex].face + 1, FALSE);
			iA->SetValue(keys[mActiveUIKeyIndex].u0, FALSE);
			iB->SetValue(keys[mActiveUIKeyIndex].u1, FALSE);
			iTens->SetValue(ToTCBUI(keys[mActiveUIKeyIndex].tens), FALSE);
			iCont->SetValue(ToTCBUI(keys[mActiveUIKeyIndex].cont), FALSE);
			iBias->SetValue(ToTCBUI(keys[mActiveUIKeyIndex].bias), FALSE);
			iEaseTo->SetValue(ToEaseUI(keys[mActiveUIKeyIndex].easeIn), FALSE);
			iEaseFrom->SetValue(ToEaseUI(keys[mActiveUIKeyIndex].easeOut), FALSE);
...
void AttachCtrl::DestroyWindow()
{
	...
	iEaseTo = iEaseFrom = iTime = iFace = iA = iB = iTens = iCont = iBias = NULL;
	iStat = NULL;
...
BOOL PickObjectMode::HitTest(IObjParam* ip, HWND hWnd, ViewExp* vpt, IPoint2 m, int flags)
{
	...
	if (node)
	{
		...
		if (os.obj->CanConvertToType(triObjectClassID))
		{
			...
			else
			{
				...
				MaxSDK::MaxMessageBox(ip->GetMAXHWnd(), buf2, buf1, MB_OK | MB_ICONEXCLAMATION);
			}
		}
...
int SetPosMouseProc::proc(HWND hWnd, int msg, int point, int flags, IPoint2 m)
{
	...
	case MOUSE_POINT:
		...
		else
		{
			if (animated)
			{
				...
				int res = MaxSDK::MaxMessageBox(hWnd, string1, string2, MB_ICONQUESTION | MB_YESNO);
				if (res != IDYES)
				{
					theHold.Cancel();
...
{
	if (cont->iSetPos)
		cont->iSetPos->SetCheck(TRUE);
}

void SetPosCMode::ExitMode()
{
	if (cont->iSetPos)
		cont->iSetPos->SetCheck(FALSE);
}
...
static LRESULT CALLBACK BaryGraphWindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		...
		oldBary = *bary;
		SetCapture(hWnd);
		SendMessage(GetParent(hWnd), WM_COMMAND, IDC_ATTACH_FACEPOSGRAPH, MAKELPARAM(BARYGRAPH_DOWN, 0));
		// Fallthrough
	...
	case WM_MOUSEMOVE:
		if (mDown)
		{
			...
			int y = GET_Y_LPARAM(lParam) - 5;
			float u = float(x) / float(rect.w() - 11);
			float v = 1.0f - float(y) / float(rect.h() - 11);

			bary->x = v;
			bary->y = 1.0f - v - u;
			SendMessage(GetParent(hWnd), WM_COMMAND, IDC_ATTACH_FACEPOSGRAPH, MAKELPARAM(BARYGRAPH_MOVE, 0));
		}
		break;
	...
	case WM_LBUTTONUP:
		mDown = FALSE;
		ReleaseCapture();
		SendMessage(GetParent(hWnd), WM_COMMAND, IDC_ATTACH_FACEPOSGRAPH, MAKELPARAM(BARYGRAPH_UP, TRUE));
		break;
	...
	case WM_RBUTTONDOWN:
		...
		*bary = oldBary;
		ReleaseCapture();
		SendMessage(GetParent(hWnd), WM_COMMAND, IDC_ATTACH_FACEPOSGRAPH, MAKELPARAM(BARYGRAPH_UP, FALSE));
		break;
	...
	case WM_PAINT:
	{
		...
		if (IsWindowEnabled(hWnd))
		{
			...
			IPoint2 pt(int(pt0.x * bary->x + pt1.x * bary->y + pt2.x * (1.0f - bary->x - bary->y)),
	...
	case WM_DESTROY:
		// We're about to delete bary; make sure to zero out the stored bary pointer in the window's long pointer
...
public:
	...
	RefResult NotifyRefChanged(
			const Interval& iv, RefTargetHandle hTarg, PartID& partID, RefMessage msg, BOOL propagate);
	int NumRefs()
	{
		return 1;
	}
...
AttachKeyDlg::AttachKeyDlg(AttachCtrl* c, const TCHAR* pname, IObjParam* ip, HWND hParent)
{
	valid = FALSE;
	cont = NULL;
	hWnd = NULL;
	...
	theHold.Resume();
	hWnd = CreateDialogParam(hInstance, MAKEINTRESOURCE(IDD_ATTACH_DIALOG), hParent, AttachDlgProc, (LPARAM)this);
...
public:
	BOOL non;
	ReferenceMaker* me;
	CheckForNonAttachDlg(ReferenceMaker* m)
	{
		non = FALSE;
		me = m;
	}
	int proc(ReferenceMaker* rmaker)
	{
		...
		if (rmaker->SuperClassID() != REF_MAKER_CLASS_ID && rmaker->ClassID() != ATTACHDLG_CLASS_ID)
		{
			non = TRUE;
...
void AttachKeyDlg::SpinnerChange(int id)
{
	...
	{
		if (!(cont->keys[index].flags & KEY_SELECTED))
			continue;
		switch (id)
		{
		case IDC_ATTACH_KEYTIMESPIN:
		...
		for (int i = 0; i < cont->keys.length(); i++)
		{
			if (cont->keys[i].flags & KEY_SELECTED)
			{
				if (i)
					...
					n = cont->keys.length() - 1;
			}
		}
		for (int i = 0; i < cont->keys.length(); i++)
		{
			if (i == n)
				cont->keys[i].flags |= KEY_SELECTED;
			else
				cont->keys[i].flags &= ~KEY_SELECTED;
		}
		cont->NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
...
void AttachKeyDlg::UpdateTCBGraph(float t, float c, float b, float easeIn, float easeOut)
{
	TCBGraphParams tp;
	tp.tens = t;
	tp.cont = c;
...
void AttachKeyDlg::Update()
{
	...
	for (int i = 0; i < cont->keys.length(); i++)
	{
		...
		else
		{
			...
			if (b != cont->keys[i].u1)
				common[2] = FALSE;
			if (tens != cont->keys[i].tens)
				common[3] = FALSE;
			if (cnt != cont->keys[i].cont)
				common[4] = FALSE;
			if (bias != cont->keys[i].bias)
				common[5] = FALSE;
			if (easeTo != cont->keys[i].easeIn)
				common[6] = FALSE;
			if (easeFrom != cont->keys[i].easeOut)
				common[7] = FALSE;
			if (face != cont->keys[i].face)
				common[8] = FALSE;
			selIndex = -1;
		}
	}
...
void AttachCtrl::DumpState()
{
	DebugPrint(_T("Path Keys count %d\n"), keys.length());
	for (int i = 0; i < keys.length(); ++i)
	{
		DebugPrint(_T("[%d : %d] "),i,keys[i].time/GetTicksPerFrame());
	}
	DebugPrint(_T("\n"));
	
	//whole frame time cache of the result since this can be a heavy weight controller

	DebugPrint(_T("Time Cache \n"));
	size_t index = 0;
	for (const auto& cache : mTimeCache)
	{
		DebugPrint(_T("[%d : %d] "), index, cache.first/GetTicksPerFrame());
		++index;
	}
	DebugPrint(_T("\n"));

	DebugPrint(_T("validity [%d:%d]\n"), mValid.Start() / GetTicksPerFrame(), mValid.End() / GetTicksPerFrame());
	
	
	if (trackValid)
		DebugPrint(_T("Point Path is valid \n"));
	else 
		DebugPrint(_T("Point Path is NOT valid \n"));
}
