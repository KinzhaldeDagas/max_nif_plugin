...
void LinkConstTimeChangeCallback :: TimeChanged(TimeValue t){
	...
	Control *cont = link_controller->pblock->GetControllerByID(link_start_time, selection); 
...
class LinkConstClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Link Constraint"); }
...
void PickLinkNode::EnterMode(IObjParam *ip)
	...
	GetCOREInterface()->PushPrompt(GetString(IDS_SELECT_TARGET_OBJECT));
	}
...
//			}
	...
	void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)    // set from v
	{
		...
		switch (id){
			...
			case link_start_time:
				{
				IParamMap2* pmap = p->pblock->GetMap();
				p->bRedrawListbox = true;
				break;
			}
		...
		INT_PTR DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
		{
			...
			switch (msg) 
			{
				case WM_INITDIALOG:
				{
					...
					toolinfo.hwnd		= p->hWnd;
					toolinfo.hinst		= hInstance;
					toolinfo.uId		= (UINT_PTR)GetDlgItem( p->hWnd, IDC_LINK_LIST );
					toolinfo.lpszText = LPSTR_TEXTCALLBACK;
					SendMessage( hTooltip, TTM_ADDTOOL, 0, (LPARAM) (LPTOOLINFO) &toolinfo );
				...
				case WM_NOTIFY:
					{
						...
						case TTN_GETDISPINFO:
							...
							NMTTDISPINFO& info = *(LPNMTTDISPINFO)lParam;
							info.lpszText = const_cast<TCHAR*>(_T(""));
							int sel = SendDlgItemMessage( p->hWnd, IDC_LINK_LIST, LB_GETCURSEL, 0, 0 );
							if ( sel != LB_ERR )
							{
								INode* node = p->GetNode( sel );
								if ( node )
									info.lpszText = const_cast<TCHAR*>(node->GetName());
							}
							break;
						}
					}
	...
	void LinkConstTransform::RedrawListbox(TimeValue t,  int sel)
		{
			...
			int lb_count =  SendDlgItemMessage(hWnd, IDC_LINK_LIST, LB_GETCOUNT, 0, 0);
			...
			// russom - July 6, 2006 - 801403
			// Make certain we don't use a selection index greater
			// than the link_target_list count.
			if( selection >= ct )
				selection = ct-1;
			...
			for (int i = 0; i < ct; i++){
				...
				INode* node = pblock->GetINode(link_target_list, t, i);
				const TCHAR* name = node ? node->GetName() : _T("World");

				TSTR str;
				str.printf( _T("%d\t%s"), linkTime / GetTicksPerFrame(), name );

				if(bAddContent)
					SendDlgItemMessage(hWnd, IDC_LINK_LIST, LB_ADDSTRING, 0, (LPARAM)(LPCTSTR) str.data());
				else
				{
					TSTR buf = GetListBoxItemText(hWnd, IDC_LINK_LIST, i);
					if(buf == str)
					{
					  //do nothing
					}
					...
					{
						SendDlgItemMessage(hWnd, IDC_LINK_LIST, LB_DELETESTRING, i, (LPARAM)0);
						SendDlgItemMessage(hWnd, IDC_LINK_LIST, LB_INSERTSTRING, i, (LPARAM)(LPCTSTR) str.data());
					}
				}
			}
...
static ParamBlockDesc2 link_const_paramblk (link_const_params, _T("LinkConsParameters"),  0, &linkCD, P_AUTO_CONSTRUCT + P_AUTO_UI, LINKCTRL_PBLOCK_REF, 
	...
	link_target_list,  _T(""),			TYPE_INODE_TAB, 0,	P_VARIABLE_SIZE,	IDS_AG_LINK_LIST,
		...
		p_nonLocalizedName, _T("Link List"),
		p_end,
	...
	link_key_hierarchy_mode, 	_T("key_hierarchy_mode"),	TYPE_INT, 0, IDS_AG_LINK_KEY_HIERARCHY_MODE,
		...
		p_accessor,		&link_const_accessor,
		p_end,
	link_key_selection, _T(""), 	TYPE_BOOL_TAB, 0,  P_VARIABLE_SIZE, IDS_AG_LINK_KEY_SELECTED, 
		p_accessor,		&link_const_accessor,	
		p_end, 
...
LinkConstTransform::LinkConstTransform(BOOL loading)
	...
	linkConstraintVersion = LINK_CONSTRAINT_V4;
	pblock = NULL;
	tmControl = NULL;
	ltControl = NULL;
	...
	if (!loading) {
		...
		LinkTimeControl* ltcont = new LinkTimeControl();
		ltcont->SetOwner( this );
		ReplaceReference(LINKCTRL_LTCTL_REF,ltcont);
		}
...
LinkConstTransform::~LinkConstTransform()
	...
	DeleteAllRefsFromMe();
	if(ltControl)ltControl->SetOwner( NULL );
	}
...
void LinkConstTransform::NotifyPreDeleteNode(void* parm, NotifyInfo* arg){
	...
	if((arg != NULL) && (linkC->pblock != NULL)){
		if (INode* deleted_node = GetNotifyParam<NOTIFY_SCENE_PRE_DELETED_NODE>(arg))
		{
		int ct = linkC->pblock->Count(link_target_list);
		int deleted_nodes_number = 0;
		for(int i=0; i <ct; i++ ){
			INode* someNode;
			linkC->pblock->GetValue(link_target_list, GetCOREInterface()->GetTime(), someNode, i);
...
RefTargetHandle LinkConstTransform::Clone(RemapDir& remap)
	...
	p->ReplaceReference(LINKCTRL_LTCTL_REF,remap.CloneRef(ltControl));
	p->ltControl->SetOwner( p );
    p->ReplaceReference(LINKCTRL_PBLOCK_REF, remap.CloneRef(pblock));
	p->mLocked = mLocked;
...
BOOL LinkConstTransform::AssignController(Animatable *control,int subAnim)
	...
	if (control->ClassID()==ClassID()) return FALSE;
		if(tmControl&&GetLockedTrackInterface(tmControl)&&GetLockedTrackInterface(tmControl)->GetLocked()==true)
			return FALSE;
...
void LinkConstTransform::Copy(Control *from)
{
	...
	if (from->ClassID() == ClassID())
	{
		...
		ReplaceReference(0, (ReferenceTarget*)CloneRefHierarchy(lc->tmControl));
		mLocked = lc->mLocked;
	}
	else if (from->ClassID() != Class_ID(DUMMY_CONTROL_CLASS_ID, 0))
	{
		if (from && GetLockedTrackInterface(from) && GetLockedTrackInterface(from)->GetLocked())
			mLocked = true;
		ReplaceReference(0, (ReferenceTarget*)CloneRefHierarchy(from));
	}
	else {
		return;
	}
...
void LinkConstTransform::GetValue( TimeValue t, void *val, Interval &valid, GetSetMethod method)
{
	...
	if (bRedrawListbox)
	{
		RedrawListbox(t, last_selection);
	}
	*ptm = GetParentTM(t, &valid);
	tmControl->GetValue(t, val, valid, method);

	mInGetValue = false;
	mLastEvalTime = std::numeric_limits<int>::min();
}
...
Matrix3 LinkConstTransform::CompTM(TimeValue t, int i){
	...
	pblock->GetValue(link_target_list, t, sNode, i); 
...
//	int ctf = pblock->Count(link_start_time);
	if (ct == 0) {		
		...
		tmControl->GetValue(t,&tm,CTRL_RELATIVE);
   ...
   int i;
	...
	if (newNodeFlag){ // we are adding a node at a new frame#
		...
		int selected =0;
		pblock->Insert(link_key_selection, i, 1, &selected);
...
//		case 0: No key, so we don't do anything here.
		case 1: // Key_nodes_only // Key the source object, the fromNode target object and the toNode target object
			...
			if (ct == 1 ){	// only one node to key -- the fromNode
				INode *fromNode;
...
//				theHold.Accept(GetString(IDS_AG_LINK_LIST));
			...
			if (ct == 1){		// there is only one target
				INode *fromNode(NULL), *fromParentNode(NULL);
				fromNode = pblock->GetINode(link_target_list, t, i);
				if (fromNode != NULL) {
//					theHold.Begin();
					fromNode->GetTMController()->CopyKeysFromTime(t, t, flags);
//					theHold.Accept(GetString(IDS_AG_LINK_LIST));
				...
				while (fromParentNode != GetCOREInterface()->GetRootNode() && fromParentNode){
//					theHold.Begin();
...
BOOL LinkConstTransform::DeleteTarget(int i)
	...
	pblock->Delete(link_key_selection, i, 1); // deletes the node from nodelist;
	macroRec->Enable();
	state = 2;
	macroRec->EmitScript();
...
void LinkConstTransform::SortNodes(int sel)
	...
	for (int i = 0; i < cnt; i++){
		INode *sNode;
		pblock->GetValue(link_target_list, 0, sNode, i);
		onodes[i] =  sNode;
	}
	...
	Tab<int> key_selections;
	key_selections.SetCount(cnt);
	for (int i = 0; i < cnt; i++){

		int sel = pblock->GetTimeValue(link_key_selection, t, i);
		key_selections[i] =  sel;
	}

	int curCurSel;
	INode *selINode1=NULL;
	if(sel==-1)
	{
		curCurSel = SendDlgItemMessage(hWnd, IDC_LINK_LIST, LB_GETCURSEL, 0, 0);
	}
	else
		curCurSel = sel;

	if(curCurSel>=0)
		pblock->GetValue(link_target_list, 0, selINode1, curCurSel);
	...
	for (int i = 0; i < cnt; i++) {
		...
		key_selection = key_selections[map[i]];
		pblock->SetValue(link_key_selection, 0, key_selection, i);
	}

	INode *selINode2=NULL;
	if(curCurSel>=0)
		pblock->GetValue(link_target_list, 0, selINode2, curCurSel);
...
// if they are unequal, the list has been re-organized.
	...
	if (curCurSel>=0 && (selINode1 != selINode2)){
		if(frameNoSpinnerUp) {
...
int LinkConstTransform::RemapRefOnLoad(int iref){
	int cc = iref;
	if (linkConstraintVersion <= LINK_CONSTRAINT_V1){
		if (iref >= 1){

			return iref + 3;
		}
	}
	else if ( linkConstraintVersion <= LINK_CONSTRAINT_V3 ) {
		if ( iref >= LINKCTRL_LTCTL_REF ) {
			return iref + 1;
		}
	}
	return iref;
...
RefTargetHandle LinkConstTransform::GetReference(int i)
	...
	if ( i == LINKCTRL_LTCTL_REF )
		return ltControl;

	int bb = nodes.Count();
	i = i - LINKCTRL_CORE_REFs;
...
void LinkConstTransform::SetReference(int i, RefTargetHandle rtarg)
	{
		...
		switch (i)
		{
			...
			case LINKCTRL_LTCTL_REF:
				ltControl = dynamic_cast<LinkTimeControl*>(rtarg);
			break;
		}

		if (i >= LINKCTRL_CORE_REFs){
			nodes[i-LINKCTRL_CORE_REFs] = (INode*)rtarg;
		}

	}

RefResult LinkConstTransform::NotifyRefChanged (const Interval& iv, RefTargetHandle hTarg, PartID& partID, RefMessage msg, BOOL propagate) 
	{
...
INode* LinkConstTransform :: GetNode(int nodeNumber){
	if (nodeNumber >= 0 && nodeNumber < pblock->Count(link_target_list)){
		INode *link_targ;
		pblock->GetValue(link_target_list, 0, link_targ, nodeNumber);
		return link_targ;
	}
	return NULL;
}
...
int LinkConstTransform::Getlink_key_mode(){
	int key_mode;
	pblock->GetValue(link_key_mode, GetCOREInterface()->GetTime(), key_mode);
...
int LinkConstTransform::Getlink_key_nodes_mode(){
	int key_n_mode;
	pblock->GetValue(link_key_nodes_mode, GetCOREInterface()->GetTime(), key_n_mode);
...
int LinkConstTransform::Getlink_key_hierarchy_mode(){
	int key_h_mode;
	pblock->GetValue(link_key_hierarchy_mode, GetCOREInterface()->GetTime(), key_h_mode);
	...
	if (GetLocked() == false)
	{
		...
		else
		{
			...
			if (spin != NULL)
			{
				...
				int linkTime =
						(int)pblock->GetTimeValue(link_start_time, GetCOREInterface()->GetTime(), last_selection);
...
void LinkConstTransform::EndEditParams( IObjParam *ip, ULONG flags,Animatable *next )
{
	...
	if (pmap != NULL)
	{
		if (next && next->ClassID() == ClassID() && ((LinkConstTransform*)next)->pblock  && ((LinkConstTransform*)next)->GetLocked()==false)
		{
...
public:
	...
	{
		if (p->linkConstraintVersion <= LINK_CONSTRAINT_V1)
			{
			...
			for (int i = 0; i < nc; i++){
				const TCHAR *str1 = p->nodes[i]->GetName();
//				p->AddNewLink(p->nodes[i],p->times[i]);
				...
				p->ReplaceReference(i+LINKCTRL_CORE_REFs, NULL);	// nodes to NULL, times aren't references
			}		
		...
		// check we have a LinkTimeControl, and set its parent
		LinkTimeControl* ltcont = (LinkTimeControl*)p->GetReference( LINKCTRL_LTCTL_REF );
		//Make sure key selection list is sync-ed with the number of constraints. This is pretty much a no-op for all new files where we save the selection to disk
		p->pblock->SetCount(link_key_selection, p->pblock->Count(link_target_list));

		if ( ltcont == NULL )
		{
			ltcont = new LinkTimeControl();
			p->ReplaceReference( LINKCTRL_LTCTL_REF, ltcont );
		}
...
// don't put any paramblock stuff in "load" since paramblock is not set up yet
	...
	linkConstraintVersion = LINK_CONSTRAINT_V2; // from R3.1 to 10/27
	...
	while (IO_OK==(res=iload->OpenChunk())) {
		switch (iload->CurChunkID()) {
			...
			case LOCK_CHUNK:
				...
				break;
			case TIMES_CHUNK:
				linkConstraintVersion = LINK_CONSTRAINT_V1;
