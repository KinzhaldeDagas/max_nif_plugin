...
class IndePosition : public LockableControl, public IAdjustTangents {
   public:
      Control *posX;
      Control *posY;
      Control *posZ;
      ...
      void GetClassName(MSTR& s, bool localized) const override {s = localized ? IPOS_CONTROL_CNAME : _T("Position XYZ");}
	  ...
	  void *GetInterface(ULONG id) {
		  switch (id) {
			  case I_TANGENT:
				  return (IAdjustTangents*)this;
		  }
		  return LockableControl::GetInterface(id);
	  }
	  ...
	  void GetKeyInfoTime(TimeValue t, IKey *key); // only returns time & flags
...
public:
	  ...
	  // IAdjustTangents method
	  bool GetTangents(TimeValue t, int element, DWORD hitFlags,ParamDimensionBase *dim,float *angle,float *length,DWORD *flags,DWORD *kflags);
	  bool AdjustTangents(TimeValue t, int element, DWORD hitFlags, ParamDimensionBase *dim,float angle,float length,DWORD flags,DWORD kflags);
	  DWORD GetKeyFlags(TimeValue t);
      ...
      class SetValueRestore : public RestoreObj {
          ...
          bool m_preSetValue;
      public:
          SetValueRestore(IndePosition* cont, bool preSetValue) : m_cont(cont), m_preSetValue(preSetValue) 
          {
...
class IndePoint3 : public IndePosition {
   public:
      ...
      void GetClassName(MSTR& s, bool localized) const override {s = localized ? IPOINT3_CONTROL_CNAME : _T("Point3 XYZ");}
      void GetValue(TimeValue t, void *val, Interval &valid, GetSetMethod method);
      RefTargetHandle Clone(RemapDir& remap);
   };
...
class IndeColor : public IndePoint3 {
   public:
      ...
      void GetClassName(MSTR& s, bool localized) const override {s = localized ? ICOLOR_CONTROL_CNAME : _T("Color RGB");}      
      ParamDimension* GetParamDimension(int i) {return stdColor255Dim;}
      RefTargetHandle Clone(RemapDir& remap);
   };

class IndePoint4 : public LockableControl {
public:
   Control *posX;
   Control *posY;
   Control *posZ;
   ...
   void GetClassName(MSTR& s, bool localized) const override {s = localized ? IPOINT4_CONTROL_CNAME : _T("Point4 XYZW");}
   void DeleteThis() {delete this;}    
   int IsKeyable() {return 1;}      
...
public:
   ...
   class SetValueRestore : public RestoreObj {
       ...
       bool m_preSetValue;
   public:
       SetValueRestore(IndePoint4* cont, bool preSetValue) : m_cont(cont), m_preSetValue(preSetValue) 
       {
           m_cont->SetAFlag(A_HELD); 
       }
       void Restore(int isUndo) {
...
public:
   ...
   void GetClassName(MSTR& s, bool localized) const override {s = localized ? IACOLOR_CONTROL_CNAME : _T("Color RGBA");}     
...
class IPosClassDesc:public ClassDesc {
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Position XYZ"); }
   SClass_ID      SuperClassID() {return CTRL_POSITION_CLASS_ID;}
   Class_ID    ClassID() {return IPOS_CONTROL_CLASS_ID;}
...
class IPoint3ClassDesc:public ClassDesc {
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Point3 XYZ"); }
   SClass_ID      SuperClassID() {return CTRL_POINT3_CLASS_ID;}
   Class_ID    ClassID() {return IPOINT3_CONTROL_CLASS_ID;}
   const TCHAR*   Category() {return _T("");}
   };
...
class IColorClassDesc:public ClassDesc {
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Color RGB"); }
   SClass_ID      SuperClassID() {return CTRL_POINT3_CLASS_ID;}
   Class_ID    ClassID() {return ICOLOR_CONTROL_CLASS_ID;}
   const TCHAR*   Category() {return _T("");}
   };
...
public:
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Point4 XYZW"); }
   SClass_ID      SuperClassID() {return CTRL_POINT4_CLASS_ID;}
   Class_ID    ClassID() {return IPOINT4_CONTROL_CLASS_ID;}
   const TCHAR*   Category() {return _T("");}
};
...
public:
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Color RGBA"); }
   SClass_ID      SuperClassID() {return CTRL_POINT4_CLASS_ID;}
   Class_ID    ClassID() {return IACOLOR_CONTROL_CLASS_ID;}
   const TCHAR*   Category() {return _T("");}
};
...
class IPosDlg {
   public:
      ...
      void Init();
	  void EndingEdit(IndePosition* next, ULONG flags);
...
void IPosDlg::EndingEdit(IndePosition* next, ULONG flags)
   {
   switch (cur) {
      case EDIT_X:
		  if (cont && cont->posX)
			  cont->posX->EndEditParams(ip, flags,next? next->posX : NULL);
         break;
      case EDIT_Y:
		  if (cont && cont->posY)
			  cont->posY->EndEditParams(ip, flags, next ? next->posY : NULL);
         break;
      case EDIT_Z:
		  if (cont && cont->posZ)
			  cont->posZ->EndEditParams(ip, flags, next ? next->posZ : NULL);
         break;
      }
   cont = NULL;
   ip   = NULL;
...
void IPosDlg::BeginingEdit(IndePosition *cont,IObjParam *ip,IndePosition *prev)
   ...
   switch (cur) {
      case EDIT_X:
		  if (cont && cont->posX)
			  cont->posX->BeginEditParams(ip,BEGIN_EDIT_MOTION,prev ? prev->posX : NULL);
         break;
      case EDIT_Y:
		  if (cont && cont->posY)
			  cont->posY->BeginEditParams(ip,BEGIN_EDIT_MOTION,prev ? prev->posY : NULL);
         break;
      case EDIT_Z:
		  if (cont && cont->posZ)
			  cont->posZ->BeginEditParams(ip,BEGIN_EDIT_MOTION, prev ? prev->posZ : NULL);
         break;
      }  
   UpdateWindow(hWnd);
   }
...
static INT_PTR CALLBACK IPosParamDialogProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
   {
   IPosDlg *dlg = DLGetWindowLongPtr<IPosDlg*>(hDlg);
...
TSTR IndePosition::SubAnimName(int i, bool localized)
{
	if (ClassID() == IPOS_CONTROL_CLASS_ID)
	{
		switch (i)
		{
		case IPOS_X_REF: return localized ? GetString(IDS_RB_XPOSITION) : _T("X Position");
		case IPOS_Y_REF: return localized ? GetString(IDS_RB_YPOSITION) : _T("Y Position");
		case IPOS_Z_REF: return localized ? GetString(IDS_RB_ZPOSITION) : _T("Z Position");
		default: return _T("");
		}
	}
	else if (ClassID() == IPOINT3_CONTROL_CLASS_ID)
	{
		switch (i)
		{
...
int IndePosition::NumKeys()
   {
   int xnum = posX? posX->NumKeys(): 0;
   int ynum = posY? posY->NumKeys(): 0;
   int znum = posZ? posZ->NumKeys(): 0;
   keyTimes.ZeroCount();
   for (int i = 0, j = 0, k = 0;
   ...
   Tab<TimeValue>& times, Interval range, DWORD flags)
{
   ...
   UINT i = 0, xc = xtimes.Count();
   UINT j = 0, yc = ytimes.Count();
   UINT k = 0, zc = ztimes.Count();
   ...
   zKeys.ClearAll();
   i = j = k = 0;
   for (UINT n = 0; n < times.Count(); ++n) {
      // RK: 06/18/02, 430812, check if count > 0
      // nleonard: 08/13/04, and if index < count
      if (xtimes.Count() > 0 && i < xtimes.Count() && xtimes[i] == times[n]) { 
         xKeys.Set(n);
         ++i;
      }
      if (ytimes.Count() > 0 && j < ytimes.Count() && ytimes[j] == times[n]) {
         yKeys.Set(n);
         ++j;
      }
      if (ztimes.Count() > 0 && k < ztimes.Count() && ztimes[k] == times[n]) {
         zKeys.Set(n);
         ++k;
      }
   }
   ...
   if (times.Count() > 0) {
	   int numKeys = NumKeys();
	   for (skip = 0; skip < numKeys; skip++)
		   if (GetKeyTime(skip) == times[0])
			   break;
   }
   return skip;
}
...
void IndePosition::BeginEditParams(IObjParam *ip, ULONG flags,Animatable *prev)
{
    if(GetLocked()==false)
    {
        this->ip = ip;
        ...
        if (dlg) {
            dlg->BeginingEdit(this, ip, dynamic_cast<IndePosition*>(prev));
        } else {
...
{  
   ...
   if (next && next->ClassID()==ClassID()) {
	  ...
	  if(cont->GetLocked()==true)
		  cont = NULL; //just delete dialog
      }

   if (dlg) {
      if (cont) {
         dlg->EndingEdit(cont, flags);
      } else {
         delete dlg;
         dlg = NULL;
         }
...
// Returns the incoming control if its not a list control.
Control* TryGetListSubControl(Control* control)
{
	if (control == nullptr) {
		return nullptr;
	}
	IListControl* listControl = GetIListControlInterface(control);
	if (listControl == nullptr) {
		return control;
	}
	Control* activeSubCtrl = listControl->GetSubCtrl(listControl->GetActive());
	if (activeSubCtrl != nullptr) {
		return activeSubCtrl;
	}
	return control;
}

IAdjustTangents* FindTangentInterface(Control* control)
{
	Control* listSubControl = TryGetListSubControl(control);
	if(listSubControl != nullptr) {
		control = listSubControl;
	}
	if(control != nullptr) {
		return GetTangentInterface(control);
	}
	return nullptr;
}

bool IndePosition::GetTangents(TimeValue t, int element, DWORD hitFlags, ParamDimensionBase *dim,float *angle,float *length, DWORD *flags, DWORD *kflags)
{
	bool result = true;
	...
	switch (element) {
		...
		case 0: {
			IAdjustTangents *adjTanX = FindTangentInterface(posX);
			if (adjTanX)
				result = result && adjTanX->GetTangents(t,0,hitFlags,dim,angle,length,flags,kflags);
			else
				result = false;
			break;
		}
		...
		case 1: {
			IAdjustTangents *adjTanY = FindTangentInterface(posY);
			if (adjTanY)
				result = result && adjTanY->GetTangents(t,0,hitFlags,dim,angle,length,flags,kflags);
			else
				result = false;
			break;
		}
		...
		case 2: {
			IAdjustTangents *adjTanZ = FindTangentInterface(posZ);
			if (adjTanZ)
				result = result && adjTanZ->GetTangents(t,0,hitFlags,dim,angle,length,flags,kflags);
			else
				result = false;
			break;
		}
...
bool IndePosition::AdjustTangents(TimeValue t, int element, DWORD hitFlags, ParamDimensionBase *dim,float angle,float length,DWORD flags,DWORD kflags)
{
	bool result = false;

	if (GetLocked()==true) {
		DebugPrint(_T("IndePosition locked, returning"));
		return result;
	}
	switch (element) {
		case 0: {
			IAdjustTangents *adjTanX = FindTangentInterface(posX);
			if (adjTanX)
				result |= adjTanX->AdjustTangents(t, 0, hitFlags, dim, angle, length, flags, kflags);
			else
				result = false;
			break;
		}
		...
		case 1: {
			IAdjustTangents *adjTanY = FindTangentInterface(posY);
			if (adjTanY)
				result |= adjTanY->AdjustTangents(t,0,hitFlags,dim,angle,length,flags,kflags);
			else
				result = false;
			break;
		}
		...
		case 2: {
			IAdjustTangents *adjTanZ = FindTangentInterface(posZ);
			if (adjTanZ)
				result |= adjTanZ->AdjustTangents(t,0,hitFlags,dim,angle,length,flags,kflags);
			else
				result = false;
			break;
		}
...
int IndePosition::GetORT(int type)
   {
   if (posX) 
	   return posX->GetORT(type);  // AF -- you got to pick something...
   else if (posY) 
	   return posY->GetORT(type);  
   else if (posZ) 
	   return posZ->GetORT(type);  
   return 0;
   }
...
void IndePosition::GetKeyInfoTime(TimeValue t, IKey *key)
   {
   bool inTanUser=true, outTanUser=true;
   
   key->time = t;
   key->flags = -1;

   auto grabKeyFlags = [&inTanUser, &outTanUser, key, t](Control* cont)
   {
       cont = TryGetListSubControl(cont);
       if (cont)
       {
           IKeyControl* ikCtrl = (IKeyControl*)cont->GetInterface(I_KEYCONTROL);
           if (ikCtrl)
           {
               int xIdx = cont->GetKeyIndex(t);
               if (xIdx >= 0)
               {
                   IKey* k = (IKey*)malloc(ikCtrl->GetKeySize());
                   ikCtrl->GetKey(xIdx, k);
                   if (GetInTanType(k->flags) != BEZKEY_USER)
                       inTanUser = false;
                   if (GetOutTanType(k->flags) != BEZKEY_USER)
                       outTanUser = false;
                   if (key->flags == -1)
                       key->flags = k->flags;
                   else
                       key->flags |= k->flags;
                   if (!inTanUser)
                       SetInTanType(key->flags, GetInTanType(k->flags));
                   if (!outTanUser)
                       SetOutTanType(key->flags, GetOutTanType(k->flags));
                   free(k);
               }
           }
       }
   ...
   // If all of the contributing curve handles are BEZKEY_USER type, return BEZKEY_USER.
   // Otherwise we preserve whatever auto type handles have been encountered along the way,
   // to indicate to the user that some auto behavior is happening
   if (key->flags != -1) {
	   if (inTanUser)
...
DWORD IndePosition::GetKeyFlags(TimeValue t)
   {
   IKey key;
   GetKeyInfoTime(t, &key);
   return key.flags;
   }
...
{
   // Base classe's ctor, IndePoint4::IndePoint4(BOOL loading), has already been 
   // called and has initialized references IPOS_X_REF, IPOS_Y_REF, IPOS_Z_REF
   HoldSuspend	hs;
   if (!loading) {
      float v = 1.0f;
...
{
   IndeAColor *pos = new IndeAColor(TRUE);
   pos->mLocked = mLocked;
   pos->ReplaceReference(IPOS_X_REF, remap.CloneRef(posX));
   pos->ReplaceReference(IPOS_Y_REF, remap.CloneRef(posY));
   pos->ReplaceReference(IPOS_Z_REF, remap.CloneRef(posZ));
...
TSTR IndePoint4::SubAnimName(int i, bool localized)
{
   ...
   Tab<TimeValue>& times, Interval range, DWORD flags)
{
   ...
   for (UINT n = 0; n < times.Count(); ++n) {
      ...
      if (wtimes.Count() > 0 && m < wtimes.Count() && wtimes[m] == times[n]) { 
         wKeys.Set(n);
         ++m;
      }
   }
...
IOResult IndePoint4::Save(ISave *isave)
{
	Control::Save(isave);
...
IOResult IndePoint4::Load(ILoad *iload)
{
	ULONG nb;
	IOResult res;
...
///A TrackBar filter for hiding the keys reported by the IndePos controllers.
///The subTree nature of the TrackBar already includes the keys reported by the 
///IndePos controller since it gets them directly from the subAnims.
bool NoIndePosFilter(Animatable* anim, Animatable* parent, int subAnimIndex, Animatable* grandParent, INode* node)
{
   if(anim != NULL && 
      (anim->ClassID() == IPOS_CONTROL_CLASS_ID ||
      anim->ClassID() == IPOINT3_CONTROL_CLASS_ID ||
      anim->ClassID() == ICOLOR_CONTROL_CLASS_ID ||
      anim->ClassID() == IPOINT4_CONTROL_CLASS_ID ||
      anim->ClassID() == IACOLOR_CONTROL_CLASS_ID))
   {
      return false;
   }
   return true;
...
void IndePosNotifyStartup(void *param, NotifyInfo *info)
{
   ...
   if (tb != NULL)
   {
      ...
      {
         filterManager->RegisterFilter(NoIndePosFilter, NULL, _T(""), Class_ID(0x1ebf325f, 0x68696c7e), true, false);
      }
   }
}

///Registers a startup callback to add a TrackBar filter
class IndePosTrackBarFilterRegister
{
public:
   IndePosTrackBarFilterRegister()
   {
      RegisterNotification(IndePosNotifyStartup, NULL, NOTIFY_SYSTEM_STARTUP);
   }
};
IndePosTrackBarFilterRegister theIndePosTrackBarFilterRegister;
