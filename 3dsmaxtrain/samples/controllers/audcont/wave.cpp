...
BOOL WaveForm::InitOpen(const AssetUser& lpszPathName)
{
	PAVISTREAM stream;
	...
	if(!lpszPathName.GetFullFilePath(path))
	{
		return FALSE;
	}
	...
	if (AVIStreamOpenFromFile(&stream, path.GetCStr(), streamtypeAUDIO, 0, OF_READ, NULL)) {
		AVIFileExit();
		return FALSE;
	}
...
void WaveForm::DumpDebug()
{
	...
	_stprintf(tbuf, _T("Max value: %d\n"), GetMaxValue());
	DebugOutputString(tbuf);

	for (i = 0; i< (GetEndSample() - GetBeginSample()); i++) {
		sample = abs(GetSample(i, kLeftChannel));
		_stprintf(tbuf, _T("Sample[%05ld]: %d\n"), i, sample);
		DebugOutputString(tbuf);
	}
}
...
int WaveForm::GetSample(long sampleNo, WaveForm::Channel channel)
{
	...
	if (GetNumChannels() == 2) {
		if (channel == WaveForm::kLeftChannel)
			sampleNo = sampleNo * 2;
		else if (channel == WaveForm::kRightChannel)
			sampleNo = sampleNo * 2 + 1;
		else if (channel == WaveForm::kMixChannels)
