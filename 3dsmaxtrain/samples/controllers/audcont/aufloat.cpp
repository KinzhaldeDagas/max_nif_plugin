...
extern BOOL GetSoundFile(HWND hWnd,AssetUser &name,TSTR &dir); // CoreExport
...
public:
   ...
   void GetClassName(MSTR& s, bool localized) const override {s = localized ? AUDIO_FLOAT_CONTROL_CNAME : _T("AudioFloat");}
   ...
   void EditTrackParams(TimeValue t, ParamDimensionBase *dim, const TCHAR *pname,
...
class AudioFloatClassDesc:public ClassDesc {
   ...
   const TCHAR* NonLocalizedClassName() { return _T("AudioFloat"); }
   SClass_ID      SuperClassID() { return CTRL_FLOAT_CLASS_ID; }
   Class_ID    ClassID() { return Class_ID(AUDIO_FLOAT_CONTROL_CLASS_ID1,AUDIO_FLOAT_CONTROL_CLASS_ID2); }
...
class AudioFloatDlg : public ReferenceMaker {
   public:
      ...
      virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("AudioFloatDlg"); }

      RefResult NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate);
      int NumRefs() {return 1;}
      RefTargetHandle GetReference(int i) {return cont;}
private:
      virtual void SetReference(int i, RefTargetHandle rtarg) {cont=(AudioFloatControl*)rtarg;}
public:
...
void AudioFloatDlg::Update()
{
   if (!valid && hWnd) {
	  ...
	  const TSTR& fileName = cont->GetFile().GetFileName();
      if (!fileName.isNull())
         SendMessage(GetDlgItem(hWnd, IDC_AUDAMP_FILENAME), WM_SETTEXT, 0, (LPARAM)(LPCTSTR)fileName.data());
      else
...
void AudioFloatDlg::WMCommand(int id, int notify, HWND hCtrl)
{
   ...
   switch (id) {
      ...
      case IDC_AUDAMP_RECENABLE:
         ...
         if (cont->enableRuntime) {
            if ((mmerr = cont->rtwave->StartRecording()) == 0) {
               ...
               TSTR msgBoxText = GetString(IDS_CJ_DEVICE_BUSY);
               MaxSDK::MaxMessageBox(hWnd, msgBoxText, GetString(IDS_CJ_PROGNAME), MB_OK);
            }
         } 
         else {
      ...
      case IDC_BTN_BROWSE: {
            ...
            TSTR dir = cont->GetFile().GetFileName();
            if (dir.Length() == 0) {
               // Use sound dir if no file.
               dir = ip->GetDir(APP_SOUND_DIR);
            }
            else {
               int lc = dir.last((int)'\\');
               if (lc == -1) {
			...
			AssetUser asset = cont->GetFile();
            GetSoundFile(hCtrl, asset, dir);
			cont->SetFile(asset);

            if (asset.GetId()!=kInvalidId) {
               cont->wave->InitOpen(asset);
               SendMessage(GetDlgItem(hWnd, IDC_AUDAMP_FILENAME), WM_SETTEXT, 0, (LPARAM)(LPCTSTR)asset.GetFileName().data());
               EnableChannelUI(cont->wave->GetNumChannels() == 2);
            }
            Change();
         }
      ...
      case IDC_BTN_REMOVESOUND:
         ...
         cont->SetFile(AssetUser());
         SendMessage(GetDlgItem(hWnd, IDC_AUDAMP_FILENAME), WM_SETTEXT, 0, (LPARAM)_T(""));
...
RefResult AudioFloatDlg::NotifyRefChanged(const Interval& changeInt, 
...
static INT_PTR CALLBACK AudioFloatDlgProc(HWND hWnd, UINT msg, WPARAM wParam,
   LPARAM lParam)
{
   AudioFloatDlg *dlg = DLGetWindowLongPtr<AudioFloatDlg*>(hWnd);
...
void AudioFloatControl::Copy(Control* from)
{
   if(GetLocked()==false)
   {
	   ...
	   if (from->ClassID() == ClassID()) {
		  ...
		  mLocked = ((AudioFloatControl*)from)->mLocked;
	   }
	   else {
...
void AudioFloatControl::RefDeleted(RefMakerHandle rm)
{
   int c=0;
...
#define FILENAME_ASSET_CHUNK 0x010D
...
IOResult AudioFloatControl::Save(ISave *isave)
{
   ...
   isave->BeginChunk(FILENAME_ASSET_CHUNK);
   szFilenameAsset.Save(isave);
   isave->EndChunk();
      ...
      switch (iload->CurChunkID()) {
		...
		case LOCK_CHUNK:
         ...
         case FILENAME_CHUNK: {
			...
			if(assetMgr)
			{
				AssetUser asset = assetMgr->GetAsset(buf,kSoundAsset);
				SetFile(asset);
				if (asset.GetId()!=kInvalidId) {
