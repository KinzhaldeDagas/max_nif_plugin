...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override {s = localized ? AUDIO_SCALE_CONTROL_CNAME : _T("AudioScale");}
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("AudioScale"); }
	SClass_ID		SuperClassID() { return CTRL_SCALE_CLASS_ID; }
	Class_ID		ClassID() { return Class_ID(AUDIO_SCALE_CONTROL_CLASS_ID1,AUDIO_SCALE_CONTROL_CLASS_ID2); }
...
void AudioScaleControl::Copy(Control* from)
{
	...
	if (from->ClassID() == ClassID()) {
		...
		mLocked = ((AudioScaleControl*)from)->mLocked;
	}
	else {
...
void AudioScaleControl::RefDeleted(RefMakerHandle rm)
{
	int c=0;
