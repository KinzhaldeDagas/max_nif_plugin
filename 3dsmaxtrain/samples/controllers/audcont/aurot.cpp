...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override {s = localized ? AUDIO_ROTATION_CONTROL_CNAME : _T("AudioRotation");}
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("AudioRotation"); }
	SClass_ID		SuperClassID() { return CTRL_ROTATION_CLASS_ID; }
	Class_ID		ClassID() { return Class_ID(AUDIO_ROTATION_CONTROL_CLASS_ID1,AUDIO_ROTATION_CONTROL_CLASS_ID2); }
...
void AudioRotationControl::Copy(Control* from)
{
	if(GetLocked()==false)
	{
		...
		if (from->ClassID() == ClassID()) {
			...
			mLocked = ((AudioRotationControl*)from)->mLocked;
		}
		else {
...
void AudioRotationControl::RefDeleted(RefMakerHandle rm)
{
	int c=0;
