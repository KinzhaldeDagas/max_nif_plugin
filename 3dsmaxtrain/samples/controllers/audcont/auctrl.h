...
class AudioBaseControl : public LockableStdControl {
	public:		
		...
		BOOL FixupFilename(TSTR &name,const TCHAR *dir);

		void EnumAuxFiles(AssetEnumCallback& nameEnum, DWORD flags);

		const AssetUser& GetFile();
		void SetFile(const AssetUser& file);

protected:
	AssetUser szFilenameAsset;
