...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override {s = localized ? AUDIO_POSITION_CONTROL_CNAME : _T("AudioPosition");}
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("AudioPosition"); }
	SClass_ID		SuperClassID() { return CTRL_POSITION_CLASS_ID; }
	Class_ID		ClassID() { return Class_ID(AUDIO_POSITION_CONTROL_CLASS_ID1,AUDIO_POSITION_CONTROL_CLASS_ID2); }
...
void AudioPositionControl::Copy(Control* from)
{
	if(GetLocked()==false)
	{
		...
		if (from->ClassID() == ClassID()) {
			...
			mLocked = ((AudioPositionControl*)from)->mLocked;
		}
		else {
...
void AudioPositionControl::RefDeleted(RefMakerHandle rm)
{
	int c=0;
