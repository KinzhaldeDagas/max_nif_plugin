...
class OrientConstRotation : public	IOrientConstRotation  ,public ILockedTrackImp
{
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? ORIENT_CONTROL_CNAME : _T("Orientation Constraint");}
		...
		TSTR SubAnimName(int i, bool localized) override { return localized ? GetString(IDS_AG_ORIENTATIONPARAMS) : _T("Orientation Constraint"); }
...
class OrientConstClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Orientation Constraint"); }
	SClass_ID		SuperClassID() { return CTRL_ROTATION_CLASS_ID; }
...
void PickOrientNode::EnterMode(IObjParam *ip)
	...
	GetCOREInterface()->PushPrompt(_T("Select Orientation Target Object"));
	}
...
void OrientConstRotation::Copy(Control *from)
{
	...
	if (from->ClassID()==ClassID()) {
		...
		from->GetValue(GetCOREInterface()->GetTime(), &fvalRot);	// to know the object orientation before the
		firstTimeFlag = 1;
...
INode* OrientConstRotation::GetNode(int targetNumber){
	if (targetNumber >= 0 && targetNumber < pblock->Count(orientation_target_list)){
		INode *orient_targ;
		pblock->GetValue(orientation_target_list, 0, orient_targ, targetNumber);
		return orient_targ;
	}
	return NULL;
}
...
float OrientConstRotation::GetTargetWeight(int targetNumber){
	if (targetNumber >= 0 && targetNumber < pblock->Count(orientation_target_list)){
		float targetWt;
		pblock->GetValue(orientation_target_weight, GetCOREInterface()->GetTime(), targetWt, targetNumber);
		return targetWt;
	}
	else{
		return 0.0f;
