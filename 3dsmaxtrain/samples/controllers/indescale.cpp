...
class IndeScale : public LockableControl
{
   public:
      ...
      void GetClassName(MSTR& s, bool localized) const override
         { s = localized ? ISCALE_CONTROL_CNAME : _T("ScaleXYZ"); }
      void DeleteThis()
         { delete this; }     
      int IsKeyable()
         { return 1; }     
...
public:
      ...
      class SetValueRestore : public RestoreObj {
          ...
          bool m_preSetValue;
      public:
          SetValueRestore(IndeScale* cont, bool preSetValue) : m_cont(cont), m_preSetValue(preSetValue) 
          {
...
class IScaleDlg
{
   public:
      ...
      void Init();
	  void EndingEdit(IndeScale* next, ULONG flags);
...
void IScaleDlg::EndingEdit(IndeScale* next, ULONG flags)
{
   switch (cur)
   {
   case EDIT_X:
	   cont->scaleX->EndEditParams(ip, flags,next? next->scaleX :NULL);
      break;
   case EDIT_Y:
	   cont->scaleY->EndEditParams(ip, flags,next ? next->scaleY :NULL);
      break;
   case EDIT_Z:
	   cont->scaleZ->EndEditParams(ip, flags,next ? next->scaleZ: NULL);
      break;
   }
   cont = NULL;
   ip   = NULL;
...
void IndeScale::SetValue(TimeValue t, void *val, int commit, GetSetMethod method)
{
   if(GetLocked()==false)
   {
	   ...
	   if (method==CTRL_RELATIVE)
	   {
		  ...
		  if (scaleX)
		  {
			 scaleX->GetValue(t,&childScale);
			 v->s.x *= childScale;
		  }
		  if (scaleY)
		  {
			 scaleY->GetValue(t,&childScale);
			 v->s.y *= childScale;
		  }
		  if (scaleZ)
		  {
			 scaleZ->GetValue(t,&childScale);
			 v->s.z *= childScale;
		  }
...
TSTR IndeScale::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case ISCALE_X_REF: return localized ? GetString(IDS_MM_XSCALE) : _T("X Scale");
	case ISCALE_Y_REF: return localized ? GetString(IDS_MM_YSCALE) : _T("Y Scale");
	case ISCALE_Z_REF: return localized ? GetString(IDS_MM_ZSCALE) : _T("Z Scale");
	default: return _T("");
	}
}
...
BOOL IndeScale::AssignController(Animatable *control,int subAnim)
{
   if(GetLocked()==false)
   {
	   ...
	   {
		  float ctrlScale;
		  ((Control *)control)->GetValue(0,&ctrlScale);
		  if (ctrlScale == 0.0f)
		  {
			 ctrlScale = 1.0f;
...
{  
   if(GetLocked()==false)
   {
	   ...
	   if (dlg)
		  dlg->BeginingEdit(this,ip,dynamic_cast<IndeScale*>(prev));
	   else
...
void IndeScale::EndEditParams(IObjParam *ip, ULONG flags,Animatable *next)
{
	...
	{
		cont = dynamic_cast<IndeScale*>(next);
