...
class BoolControl : public LockableStdControl
{
	public:				
		...
		int IsKeyable() override {return 1;}
		BOOL IsAnimated() override {return keys.length()?TRUE:FALSE;}
		Class_ID ClassID() override {return BOOL_CONTROL_CLASS_ID;}
		SClass_ID SuperClassID() override {return CTRL_FLOAT_CLASS_ID;}
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? BOOL_CONTROL_CNAME : _T("On/Off");}

		// Set-key mode support
		void CommitSetKeyBuffer(TimeValue t) override;
		void RevertSetKeyBuffer() override;
		void RevertSetKeyBuffer(BOOL clearHeldFlag);
		BOOL SetKeyBufferPresent() override {return setkeyBuffer? TRUE: FALSE;}
		...
		BOOL CanApplyEaseMultCurves() override {return !GetLocked();}
		...
		int NumKeys() override {return (int)keys.length();}
		TimeValue GetKeyTime(int index) override {return keys[index].time;}
		int GetKeyIndex(TimeValue t) override;
		void DeleteKeyAtTime(TimeValue t) override;
		BOOL IsKeyAtTime(TimeValue t,DWORD flags) override;
		void DeleteTime(Interval iv, DWORD flags) override;
		void ReverseTime(Interval iv, DWORD flags) override;
		void ScaleTime(Interval iv, float s) override;
		void InsertTime(TimeValue ins, TimeValue amount) override;
		BOOL SupportTimeOperations() override {return TRUE;}
		void DeleteKeys(DWORD flags) override;
		void DeleteKeyByIndex(int index) override;
		void SelectKeys(TrackHitTab& sel, DWORD flags) override;
		void SelectKeyByIndex(int i,BOOL sel) override;
		void FlagKey(TrackHitRecord hit) override;
		int GetFlagKeyIndex() override;
		int NumSelKeys() override;
		void CloneSelectedKeys(BOOL offset=FALSE) override;
		void AddNewKey(TimeValue t,DWORD flags) override;
		BOOL IsKeySelected(int index) override;
		BOOL CanCopyTrack(Interval iv, DWORD flags) override {return TRUE;}
		BOOL CanPasteTrack(TrackClipObject *cobj,Interval iv, DWORD flags) override {return cobj->ClassID()==ClassID();}
		TrackClipObject *CopyTrack(Interval iv, DWORD flags) override;
		void PasteTrack(TrackClipObject *cobj,Interval iv, DWORD flags) override;
		int GetSelKeyCoords(TimeValue &t, float &val,DWORD flags) override;
		void SetSelKeyCoords(TimeValue t, float val,DWORD flags) override;
		int SetSelKeyCoordsExpr(ParamDimension *dim,const TCHAR *timeExpr, const TCHAR *valExpr, DWORD flags) override;
		...
		// StdControl methods
		void GetValueLocalTime(TimeValue t, void *val, Interval &valid, GetSetMethod method=CTRL_ABSOLUTE) override {GetValue(t,val,valid,method);}
		...
		void* CreateTempValue() override {return new float;}
		void DeleteTempValue(void *val) override {delete (float*)val;}
		void ApplyValue(void *val, void *delta) override {*((float*)val) += *((float*)delta);}
		void MultiplyValue(void *val, float m) override {*((float*)val) *= m;}
...
class BoolClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("On/Off"); }
...
void BoolControl::Copy(Control *from)
{
	...
	{	
		if (from) {
			...
			if (v.Empty()) 
			{
				// the source has no range, so just do a SetValue of the current value
				// to avoid creating a key at TIME_NegInfinity
				t = v.Start();				// any time will do: v.Start(), 0...
				from->GetValue(t, &s, iv);
				SetValueLocalTime(t, &s, 1, CTRL_ABSOLUTE);
			}
			else 
			{
