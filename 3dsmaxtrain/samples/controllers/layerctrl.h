...
class MainLayerControl;
class LayerControl;
class AnimLayerToolbarControl;
class MainLayerControlManagerPostLoadCallback;

#pragma region class MainLayerControlManager

class MainLayerControlManager
		: public Control
		, private CountedObject<MainLayerControlManager>
{
	friend class LayerControl;
	friend class MainLayerControlManagerPostLoadCallback;
public:
	typedef enum {mainlayers} Enums;
	...
	virtual ~MainLayerControlManager ();
...
public:
	...
	int  GetNumMLCs();
	MainLayerControl *GetNthMLC(int which);
	int GetNthMLC(TSTR &name);
	int GetMLCIndex(TSTR &name);
	int GetMLCIndex(MainLayerControl *mlc);
	void AppendMLC(MainLayerControl *mLC);
	void InsertMLC(int location,MainLayerControl *mLC);
	...
	void MainLayer_DeleteMe(); // Not meant to override non-vitrual "ReferenceMaker::DeleteMe()"
...
private:	//this is actually set and stored from trackview.
	...
	Tab<MainLayerControl*> existingMainLayerControls; //new for handling merging.. the clip assocations in there get deleted.. need to reset them!
	...
	// we don't check if the nodes are monitored by any of the MainLayerControls,simply enum all anims of the input nodes
	// and collect all the layer controllers.
	void GetLayerControlsInternal(Tab<INode*> &nodes, Tab<LCInfo> &layerControls);
};

#pragma endregion
class LayerControlMotionDlg;

#define MAINLAYER_CONTROL_CLASS_ID	Class_ID(0x225546f7, 0x75e02662)
#define MAINLAYER_CNAME GetString(IDS_MAINLAYER)

#pragma region class MainLayerControl
/* 	Main Layer Control contains
	1) references to the list that make it up.
	2) references to the nodes that have the layers...
	layers can only be set active, or be copy, or have weights set on main. */
class MainLayerControl
		: public Control
		, private CountedObject<MainLayerControl>
{
	public:
		...
		MainLayerControl(BOOL loading=FALSE);
		MainLayerControl(const MainLayerControl& ctrl);
		virtual ~MainLayerControl();	
		void Init();
		
		void GetClassName(MSTR& s, bool localized) const override;
		SClass_ID		SuperClassID() override {return  CTRL_USERTYPE_CLASS_ID;}
		Class_ID ClassID() override { return MAINLAYER_CONTROL_CLASS_ID; }

		MainLayerControl& operator=(const MainLayerControl& ctrl);
		void Resize(int numLayers);
		//float AverageWeight(float weight);	// Not implemented
		...
		void AddLayerControl(LayerControl* lC); //it only append if LC is not in this main layer control.
...
public:
		...
		bool GetLayerInternalFrozen();
		void SetLayerInternalFrozen(bool frozen);
private:
...
//! This class extends the interfaces of IAnimLayerControlManager.
class IAnimLayerControlManagerInternal  : public IAnimLayerControlManager {

public:

	//! Get whether or not the layer is internally frozen. 
	//! An internally frozen layer cannot be operated by users.
	//! \param[in] index Which layer to see if it's internally frozen.
	//! \return Whether or not the layer is internally frozen or not.
	virtual bool GetLayerInternalFrozen(int index)=0;

	//! Set whether or not a particular layer is internally frozen.
	//! An internally frozen layer cannot be operated by users.
	//! \param[in] frozen the frozen value for the specified layer. If true the layer is frozen
	//! if false it is unfrozen.	
	//! \param[in] index Which layer output to internally freeze or not.
	virtual void SetLayerInternalFrozen(int index, bool frozen)=0;

	/*! \internal use!*/
	//! For XRef extension when updating the source controller animation layer by using the animation layer in the main scene.
	//! \param[in] name the animation layer name in the main scene.
	//! \param[in] pSourceLayerCtrl the layer control from the source node as one base.
	//! \param[in] pNode the source node.
	//! \param[in] useActiveControllerType create the control flag that using the active type of default type.
	virtual void AddLayerForNode(MSTR &name, ILayerControl* pSourceLayerCtrl,INode* pNode, bool useActiveControllerType)=0;
};

//! This class extends the interfaces of ILayerControl.
class ILayerControlInternal : public ILayerControl {
...
//! \see ILayerControlEx
#define GetILayerControlExInterface(cd) \
	static_cast<ILayerControlInternal*> ((cd)->GetInterface(LAYER_CONTROLLER_INTERFACE))
...
class LayerControl
		: public Control
		, public ILayerControlInternal
		, public ILockedTrackImp
		, private CountedObject<LayerControl>
{

	public:
		//the next 2 tabs should always have a 1-1 correspondance!
		Tab<int> mLCs; //the mains this guy belongs to.
		Tab<Control*> conts;	//actually controls that make it up
		//It has a 1-1 correspondence with the above 2 tabs.
		Tab<bool> mbInternalFrozenFlags; //record internal frozen flag of the main.
		...
		//callsed by the MainLayerControl
		...
		//From ILayerControlInternal	
		virtual bool GetLayerInternalFrozen(int index);
		virtual void SetLayerInternalFrozen(int index, bool frozen);
...
public:
	...
	private:
		...
		void CheckForLock(RefTargetHandle hTarget, bool lockVal) const; // maybe set up lock on ourselves
