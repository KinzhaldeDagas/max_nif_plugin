...
public:
	...
	void*			Create(BOOL loading = FALSE) { return new BaseLimitCtrl(); }
	const TCHAR*	ClassName() { return GetString(IDS_BASELIMITCTRL_CLASS_NAME); }
	const TCHAR*	NonLocalizedClassName() { return _T("Base Limit"); }
...
Control* BaseLimitCtrl::GetDefaultControlForDataType() 
{
	switch(SuperClassID()) {
		...
		case POINTCTRLCONTAINER_SUPERCLASS_ID:
			return NewDefaultPointControllerContainer();
...
void BaseLimitCtrl::GetClassName(MSTR& s, bool localized) const 
{
	s = localized ? GetString(IDS_BASELIMITCTRL_CLASS_NAME) : _T("Base Limit");
}

TSTR BaseLimitCtrl::SubAnimName(int i, bool localized)
{
	if (i == 0)
		return localized ? GetString(IDS_BASELIMIT_PARAMS) : _T("base_parameters");
	else
	{
		TSTR name(localized ? GetString(IDS_LIMITCTRL_LIMITEDCTRL) : _T("Limited Controller"));
		if (GetLimitedControl())
		{
			TSTR clsName;
			GetLimitedControl()->GetClassName(clsName, localized);
			name.Append(_T(": "));
			name.Append(clsName);
		}
		return name;
...
const TCHAR * FloatLimitCtrlClassDesc::ClassName()
{ 
	return GetString(IDS_FLOATLIMITCTRL_CLASS_NAME); 
}

const TCHAR* FloatLimitCtrlClassDesc::NonLocalizedClassName()
{
	return _T("Float Limit");
}
...
void FloatLimitCtrl::SetUpperWidth(const TimeValue& in_t, float in_w)
{
	...
	if (IsUpperLimitActive() && IsLowerLimitActive()) {
		...
		if (lowerWidth + in_w > upper - lower)
			in_w = std::max(0.f, upper - lower - lowerWidth);
	}
...
void FloatLimitCtrl::SetLowerWidth(const TimeValue& in_t, float in_w)
{
	...
	if (IsUpperLimitActive() && IsLowerLimitActive()) {
		...
		if (upperWidth + in_w > upper - lower)
			in_w = std::max(0.f, upper - lower - upperWidth);
	}
...
///////////////////////////////////////////////////////////////////////////////
//	Animatable methods
TSTR FloatLimitCtrl::SubAnimName(int i, bool localized)
{
	if (i < BaseLimitCtrl::NumSubs())
		return BaseLimitCtrl::SubAnimName(i, localized);
	else
		return localized ? GetString(IDS_FLOATLIMIT_PARAMS) : _T("Limits");
}
...
void FloatLimitCtrl::GetClassName(MSTR& s, bool localized) const 
{
	s = localized ? GetString(IDS_FLOATLIMITCTRL_CLASS_NAME) : _T("Float Limit");
}
	...
	)
{
	...
	switch (in_message) {
		...
		case WM_DESTROY:						
            ...
            ::SetWindowLongPtr(in_hWnd, GWLP_USERDATA, (LONG_PTR)(nullptr));
            delete ld;
			break;
