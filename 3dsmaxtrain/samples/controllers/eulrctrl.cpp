...
static const TCHAR* xyzRots[] = {_T("X Rotation"), _T("Y Rotation"), _T("Z Rotation")};
...
void EulerDlg::EndingEdit(EulerRotation* next, ULONG flags)
   {
   Control *nextSub = NULL;
	if(next)
		nextSub = next->SubControl(cur);
   cont->SubControl(cur)
     ->EndEditParams(ip, flags, nextSub);
   cont = NULL;
   ip   = NULL;
   }
...
class EulerClassDesc:public ClassDesc {
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Euler XYZ"); }
...
void EulerRotation::GetClassName(MSTR& s, bool localized) const
   {
   TSTR format = localized ? GetString(IDS_RB_EULERNAME) : _T("Euler %s");
...
TSTR EulerRotation::SubAnimName(int i, bool localized)
{
	switch (i)
	{
   ...
   if(GetLocked()==false)
   {
	   if (flags&BEGIN_EDIT_HIERARCHY) {
		  ...
		  if (prev && prev->ClassID()==ClassID() && 
			  (ui = static_cast<InterpCtrlUI*>(prev->GetProperty(PROPID_INTERPUI))) != NULL) {
			 JointParams *prevjp = static_cast<JointParams*>(prev->GetProperty(PROPID_JOINTPARAMS));
			 if (prevjp != nullptr) {
				 prevjp->EndDialog(ui);
			 }
			 ui->cont = this;
	   ...
	   if (flags&BEGIN_EDIT_MOTION) {
		  ...
		  if (dlg) {
			 dlg->BeginingEdit(this, ip, dynamic_cast<EulerRotation*>(prev));
			 dlg->Init();
		  } else {
...
{  
	...
	{
		cont = dynamic_cast<EulerRotation*>(next);
		if (cont && cont->GetLocked() == true) {
