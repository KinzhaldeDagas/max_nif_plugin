...
TSTR BlockControl::SubAnimName(int i, bool localized)
{
	TSTR name;
	if (i < names.Count())
	{
	...
	{
		controls[i]->GetClassName(name, localized);
	}
...
BOOL BlockControl::AssignController(Animatable *control,int subAnim) 
   ...
   MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), (LPCTSTR) GetString(IDS_PW_ERROR_MSG), (LPCTSTR) NULL, MB_OK | MB_ICONWARNING | MB_APPLMODAL);
   return FALSE;
   }
...
void BlockControl::NotifyAllDriven()
{
if (!suspendNotifies)
   {
   ...
   for (i=0; i<count; i++) 
      {
      if ( (backPointers[i]) && (backPointers[i]->driver))
         {
         backPointers[i]->UpdateDriven();
   ...
   for (i=0; i<count; i++) 
      ...
      if (externalBackPointers[i])
         {
         externalBackPointers[i]->UpdateDriven();
...
Control* BlockControl::BuildDriven(TrackViewPick res,Control* list, BOOL createdList)
{
...
//paste back relevant keys
         ...
         DrivenControl *driven = (DrivenControl *) drivenControl;
         backPointers.Append(1,&driven,1);
...
//load up list box with sub anims names
      for (int i = 0; i < blk->NumSubs(); i++)
         {
         TSTR finalName = blk->SubAnimName(i, true);
