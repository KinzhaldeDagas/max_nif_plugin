...
static DriverBlockClassDesc driverBlockCD;
ClassDesc* GetDriverBlockDesc() { return &driverBlockCD; }

static INT_PTR CALLBACK DriverBlockDlgProc(
...
INT_PTR CALLBACK DriverBlockPropDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
INT_PTR CALLBACK KeyPropDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);

DriverBlockDlg::DriverBlockDlg(
	DriverBlockControl *cont,
	...
	)
{
	...
	DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_DRIVERBLOCKPARAMS), hParent, DriverBlockDlgProc, (LPARAM)this);
	this->cont->iop = NULL;
}
...
void DriverBlockDlg::Invalidate()
{

}

void DriverBlockDlg::Update()
{
	if (!valid && hWnd) {
		valid = TRUE;
	}
...
void DriverBlockDlg::SetupUI(HWND hWnd)
{
	this->hWnd = hWnd;
	SetupList();
	valid = FALSE;
...
void DriverBlockDlg::SetButtonStates()
{
	int sel = SendMessage(GetDlgItem(hWnd, IDC_LIST1),
		LB_GETCURSEL, 0, 0);
...
void DriverBlockDlg::EnableButtons()
{
	int sel = SendMessage(GetDlgItem(hWnd, IDC_LIST1),
		LB_GETCURSEL, 0, 0);
	if (sel >= 0) {
...
void DriverBlockDlg::WMCommand(int id, int notify, HWND hCtrl)
{
	switch (id) {
	case IDC_LIST_NAME: {
		int sel = SendMessage(GetDlgItem(hWnd, IDC_LIST1),
	...
	case IDC_ADD:
		theHold.Begin();
		theHold.Put(new DriverBlockAdd(cont));
		theHold.Suspend();
		cont->AddBlock(hCtrl);
		SetupList();
		EnableButtons();
...
void DriverBlockDlg::Change(BOOL /*redraw*/)
{

}


class CheckForNonDriverBlockDlg : public DependentEnumProc {
public:
	BOOL non;
	ReferenceMaker *me;
	CheckForNonDriverBlockDlg(ReferenceMaker *m) { non = FALSE; me = m; }
	int proc(ReferenceMaker *rmaker) {
		if (rmaker == me) return DEP_ENUM_CONTINUE;
		if (rmaker->SuperClassID() != REF_MAKER_CLASS_ID &&
			rmaker->ClassID() != Class_ID(DRIVERBLOCKDLG_CLASS_ID, 0)) {
			non = TRUE;
...
void DriverBlockDlg::MaybeCloseWindow()
{
	CheckForNonDriverBlockDlg check(cont);
...
static INT_PTR CALLBACK DriverBlockDlgProc(
	HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	DriverBlockDlg *dlg = DLGetWindowLongPtr<DriverBlockDlg*>(hWnd);

	switch (msg) {
	case WM_INITDIALOG:
	{
		dlg = (DriverBlockDlg*)lParam;
		...
		{
			Color c = dlg->cont->Blocks[0]->color;
			IColorSwatch *cs = GetIColorSwatch(GetDlgItem(hWnd, IDC_COLOR),
				c, _T("Block Color"));
			cs->SetColor(c);
			ReleaseIColorSwatch(cs);
	...
	case CC_COLOR_CHANGE: {
		...
		if (sel != -1)
		{
			...
			dlg->cont->Blocks[sel]->color = Color(cs->GetAColor());
			if (HIWORD(wParam)) {
...
void DriverBlockControl::EditTrackParams(
	TimeValue t,
	ParamDimensionBase *dim,
	const TCHAR *pname,
	HWND hParent,
	...
	if (found)
	{
		...
		if (EDITTRACK_MOUSE&flags)
		{
			...
			if (id == 1)
			{
				...
				DriverBlockDlg the_driver_block_dialog(this, dim, ip, hParent);
			}
			else
			{
...
DriverBlockControl::DriverBlockControl()
{
	range.SetEmpty();
	blendControl = NULL;
...
int DriverBlockControl::NumSubs()
{
	return Blocks.Count() + 1;
}

Animatable* DriverBlockControl::SubAnim(int i)
{
	if (i == 0)
		return blendControl;
...
TSTR DriverBlockControl::SubAnimName(int i, bool localized)
{
	TSTR name;
	if (i == 0)
		return localized ? GetString(IDS_PW_BLEND) : _T("Blend");
	...
	{
		Blocks[i - 1]->GetClassName(name, localized);
	}
	return name;
}


BOOL DriverBlockControl::AssignController(Animatable *control, int subAnim)
{
	if (subAnim == 0)
	{
...
int DriverBlockControl::NumRefs()
{
	return Blocks.Count() + 1;
}

RefTargetHandle DriverBlockControl::GetReference(int i)
{
	DbgAssert(i >= 0);
...
void DriverBlockControl::SetReference(int i, RefTargetHandle rtarg)
{
	DbgAssert(i >= 0);
	DbgAssert(i < Blocks.Count() + 1);
	if (i == 0)
...
RefResult DriverBlockControl::NotifyRefChanged(
	...
	BOOL propagate)
{
	...
	case REFMSG_CHANGE:
		//			Invalidate();			
		//			DebugPrint(_T("Ref changed should have driven get updated\n"));
		break;
	}
	return REF_SUCCEED;
}
...
void DriverBlockControl::UpdateControl(int index)
{
	...
	Blocks[index]->NotifyAllDriven();
}


void DriverBlockControl::HoldRange()
{
...
void DriverBlockControl::HoldTrack()
{
	if (theHold.Holding() && !TestAFlag(A_HELD)) {
		SetAFlag(A_HELD);
		theHold.Put(new DriverBlockRest(this));
	}
}



void DriverBlockControl::EditTimeRange(Interval range, DWORD flags)
{
...
void DriverBlockControl::SelectCurve(BOOL sel)
{
	isCurveSelected = sel;
	if (!sel)
	{
...
BOOL DriverBlockControl::IsCurveSelected()
{
	return isCurveSelected;
}


void DriverBlockControl::UpdateRange()
{
...
void DriverBlockControl::MapKeys(TimeMap *map, DWORD flags)
{
...
void DriverBlockControl::FlagKey(TrackHitRecord hit)
{
	int n = BlockKeys.Count();
	for (int i = 0; i < n; i++) {
...
int DriverBlockControl::GetFlagKeyIndex()
{
...
int DriverBlockControl::NumSelKeys()
{
	int ct = 0;
	for (int i = 0; i < BlockKeys.Count(); i++)
	{
...
int DriverBlockControl::NumKeys()
{
	//DebugPrint(_T("Num keys %d\n"),BlockKeys.Count());
	return BlockKeys.Count() * 2;
}

TimeValue DriverBlockControl::GetKeyTime(int index)
{
...
void DriverBlockControl::CloneSelectedKeys(BOOL offset)
{
...
void DriverBlockControl::DeleteKeys(DWORD flags)
{
	//DebugPrint(_T("Delete Me \n"));

	if (theHold.Holding() && !TestAFlag(A_HELD)) {
		theHold.Put(new DriverBlockDeleteKey(this));
		SetAFlag(A_HELD);
	}
	theHold.Accept(GetString(IDS_PW_DELETEKEY_COMMANDNAME));
	//HoldRange();
...
BOOL DriverBlockControl::IsKeySelected(int index)
{
	int whichEnd = index % 2;
...
void DriverBlockControl::CopyKeysFromTime(TimeValue src, TimeValue dst, DWORD flags)
{
	//DebugPrint(_T("Copy Me %d to %d\n"),src,dst);
}


BOOL DriverBlockControl::IsKeyAtTime(TimeValue t, DWORD flags)
{
...
int DriverBlockControl::GetKeyTimes(Tab<TimeValue> &times, Interval range, DWORD flags)
{
...
int DriverBlockControl::GetKeySelState(BitArray &sel, Interval range, DWORD flags)
{
...
void DriverBlockControl::AddKey(TimeValue t, int whichBlock)
{
	...
	if (theHold.Holding() && !TestAFlag(A_HELD)) {
		theHold.Put(new DriverBlockAddKey(this));
		SetAFlag(A_HELD);
	}
	else
	{
	...
	theHold.Accept(GetString(IDS_PW_ADDKEY_COMMANDNAME));
	...
	if (AddDialogSelect != -1)
	{
		...
		//		Blocks[AddDialogSelect]->NotifyAllDriven();
	}
...
void DriverBlockControl::AddNewKey(TimeValue t, DWORD flags)
{
}
void DriverBlockControl::SelectKeys(TrackHitTab& sel, DWORD flags)
{
	if (theHold.Holding() && !TestAFlag(A_HELD)) {
		theHold.Put(new DriverBlockRest(this));
		SetAFlag(A_HELD);
	...
	theHold.Accept(GetString(IDS_PW_SELECTKEY_COMMANDNAME));
...
void DriverBlockControl::SelectKeyByIndex(int i, BOOL sel)
{
	HoldTrack();
	int whichBlock = i / 2;
	int whichEnd = i % 2;
...
void DriverBlockControl::MoveKeys(ParamDimensionBase *dim, float delta, DWORD flags)
{
}

void DriverBlockControl::SetSelKeyCoords(TimeValue t, float val, DWORD flags)
{
...
int DriverBlockControl::GetSelKeyCoords(TimeValue &t, float &val, DWORD flags)
{
	TimeValue at = 0;
	BOOL tfound = FALSE;
...
int DriverBlockControl::HitTestTrack(TrackHitTab& hits, Rect& rcHit, Rect& rcTrack, float zoom, int scroll, DWORD flags)
{
	int n = BlockKeys.Count();
...
int DriverBlockControl::PaintTrack(ParamDimensionBase *dim, HDC hdc, Rect& rcTrack, Rect& rcPaint, float zoom, int scroll, DWORD flags)
{
	...
	for (int i = 0; i < BlockKeys.Count(); i++)
	{
		if (((!BlockKeys[i].startSelected) && (!BlockKeys[i].endSelected)))
		{
			...
			TSTR name = SubAnimName(subid + 1, true);
...
int DriverBlockControl::PaintFCurves(
	ParamDimensionBase *dim,
	HDC hdc,
	Rect& rcGraph,
	Rect& rcPaint,
...
int DriverBlockControl::HitTestFCurves(ParamDimensionBase *dim, TrackHitTab& hits,
...
IOResult DriverBlockControl::Save(ISave *isave)
{
	ULONG nb;
	//names
	//block keys
...
class DriverBlockPostLoadCallback :public  PostLoadCallback
{
public:
	DriverBlockControl      *s;
	DriverBlockPostLoadCallback(DriverBlockControl *r) { s = r; }
	void proc(ILoad *iload);
};

void DriverBlockPostLoadCallback::proc(ILoad *iload)
{
	if (s)
	{
...
IOResult DriverBlockControl::Load(ILoad *iload)
{
	int ID = 0;
	ULONG nb;
	IOResult res = IO_OK;
	...
	DriverBlockPostLoadCallback* mplcb = new DriverBlockPostLoadCallback(this);
...
int DriverBlockControl::LoadBlock()
...
//--------------------------------------------------------------------
int DriverBlockControl::DeleteBlock(int whichBlock)
{
...
int DriverBlockControl::SaveBlock(int whichBlock)
{
...
tryAgain:
	if (GetSaveFileName(&ofn)) {
		if (DoesFileExist(fname)) {
			...
			if (IDYES != MaxSDK::MaxMessageBox(hWnd, buf1, buf2, MB_YESNO | MB_ICONQUESTION))
            {
				goto tryAgain;
			}
		}
...
int DriverBlockControl::AddBlockName(ReferenceTarget *anim, ReferenceTarget *client, int subNum, NameList &names)

{
	...
	TSTR np = TSTR(client->SubAnimName(subNum, true));
	TSTR Slash(_T("/"));
	nodeName += Slash;
	nodeName += np;
...
TSTR* DriverBlockControl::GetBlockName(ReferenceTarget *anim, ReferenceTarget *client, int subNum)
{
...
Control* DriverBlockControl::BuildListControl(TrackViewPick res, BOOL &createdList)
{
	Control *list = NULL;
	createdList = FALSE;
...
Control* DriverBlockControl::BuildDriven(TrackViewPick res, Control* list, BOOL createdList)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		{
			driven = (Control*)list->SubAnim(i);
			if (driven->ClassID() == DRIVENFLOAT_CONTROL_CLASS_ID)
			{
		...
		{
			driven = (Control*)list->SubAnim(i);
			if (driven->ClassID() == DRIVENPOS_CONTROL_CLASS_ID)
			{
				//			list->AssignController(res.anim,count);
				return driven;
			}
		...
		{
			driven = (Control*)list->SubAnim(i);
			if (driven->ClassID() == DRIVENPOINT3_CONTROL_CLASS_ID)
			{
				//			list->AssignController(res.anim,count);
				return driven;
			}
		...
		{
			driven = (Control*)list->SubAnim(i);
			if (driven->ClassID() == DRIVENROTATION_CONTROL_CLASS_ID)
			{
				//			list->AssignController(res.anim,count);
				return driven;
			}
		...
		{
			driven = (Control*)list->SubAnim(i);
			if (driven->ClassID() == DRIVENSCALE_CONTROL_CLASS_ID)
			{
				//			list->AssignController(res.anim,count);
				return driven;
			}
	...
	if (res.anim->SuperClassID() == CTRL_FLOAT_CLASS_ID)
	{
		driven = (Control*)new DrivenFloatControl;
		isFloat = TRUE;
	}
	else if (res.anim->SuperClassID() == CTRL_POSITION_CLASS_ID)
		driven = (Control*)new DrivenPosControl;
	else if (res.anim->SuperClassID() == CTRL_POINT3_CLASS_ID)
		driven = (Control*)new DrivenPoint3Control;
	else if (res.anim->SuperClassID() == CTRL_ROTATION_CLASS_ID)
	{
		driven = (Control*)new DrivenRotationControl;
		isRotation = TRUE;
	}
	else if (res.anim->SuperClassID() == CTRL_SCALE_CLASS_ID)
		driven = (Control*)new DrivenScaleControl;
	if (createdList)
	{
		...
		list->AssignController(driven, count + 1);
	}
	else
	{
		list->AssignController(driven, count);
	}
	return driven;
}

//watje 4-24-99
int DriverBlockControl::AppendBlockNoDriven(BlockControl *b, int i, TSTR *name, Control *bdata)
{
...
int DriverBlockControl::AppendBlock(BlockControl *b, int i, TrackViewPick res, int where)
{
	Control *list;
	BOOL createdList = FALSE;
	//check for list control if not add
	list = BuildListControl(res, createdList);
	//check if list has a driven control
	Control *drivenControl;
	drivenControl = BuildDriven(res, list, createdList);
	...
	b->backPointers[i] = (DrivenControl *)drivenControl;



	AddBlockName(res.anim, res.client, res.subNum, b->names);
	//add driven controls to the selected tracks and put the original as a sub anim of these driven
	//set driven	to have reference to driver
	drivenControl->ReplaceReference(1, this);
	//copy selected track into driven sub
	drivenControl->ReplaceReference(0, (Control*)CloneRefHierarchy(res.anim));
	...
	if (res.anim->SuperClassID() == CTRL_FLOAT_CLASS_ID)
	{
		...
		b->tempControls[i]->DeleteKeys(TRACK_DOALL);
		b->tempControls[i]->SetValue(0, &f);
		DrivenFloatControl *driven = (DrivenFloatControl *)drivenControl;

		driven->scratchControl = (Control *)CloneRefHierarchy(res.anim);
		NukeEaseAndMult((Control *)driven->scratchControl);
		driven->scratchControl->DeleteKeys(TRACK_DOALL);
		driven->scratchControl->SetValue(0, &f);
		//now replace track with driven
		//	int bc = Blocks.Count()-1;
		driven->blockID.Append(1, &bc, 1);
		driven->subID.Append(1, &i, 1);
	}
	else if (res.anim->SuperClassID() == CTRL_POSITION_CLASS_ID)
	{
		Point3 f(0.0f, 0.0f, 0.0f);
		b->tempControls[i]->DeleteKeys(TRACK_DOALL);
		b->tempControls[i]->SetValue(0, &f);
		DrivenPosControl *driven = (DrivenPosControl *)drivenControl;
	...
	else if (res.anim->SuperClassID() == CTRL_POINT3_CLASS_ID)
	{
		...
		b->tempControls[i]->DeleteKeys(TRACK_DOALL);
		b->tempControls[i]->SetValue(0, &f);
		DrivenPoint3Control *driven = (DrivenPoint3Control *)drivenControl;
	...
	else if (res.anim->SuperClassID() == CTRL_ROTATION_CLASS_ID)
	{
		...
		b->tempControls[i]->DeleteKeys(TRACK_DOALL);
		b->tempControls[i]->SetValue(0, &f);
		DrivenRotationControl *driven = (DrivenRotationControl *)drivenControl;
	...
	else if (res.anim->SuperClassID() == CTRL_SCALE_CLASS_ID)
	{
		...
		DrivenScaleControl *driven = (DrivenScaleControl *)drivenControl;
...
int DriverBlockControl::AppendBlock(BlockControl *b, int i, TrackViewPick res, Control *bdata, int where)
{
	Control *list;
	BOOL createdList = FALSE;
	//check for list control if not add
...
int DriverBlockControl::AddBlock(HWND hWnd)
{
	MaxSDK::Array<TrackViewPick> res;
	DriverBlockTrackViewFilter filter;
	...
	if (GetCOREInterface()->TrackViewPickMultiDlg(hWnd, &res, &filter, PICKMULTI_FLAG_ANIMATED | PICKMULTI_FLAG_VISTRACKS))
	{
		...
		if (DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_BLOCKPROP), hWnd, DriverBlockPropDlgProc, (LPARAM)this))
		{
			...
			for (int i = 0; i < res.length(); i++)
			{
				...
				res[i].anim->DeleteAllNoteTracks();
				//need to make copy
				//need to check is controller really attched or is there no keys and just constant
				if (res[i].anim != NULL)
				{
					//check if this controler already has a driven attached don't create another one just update its blockid and subid table
...
int DriverBlockControl::AddSelected(HWND hWnd)
{
	MaxSDK::Array<TrackViewPick> res;
#define ID_TV_GETSELECTED	680
	...
	if (res.length() > 0)
	{
		...
		if (DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_BLOCKPROP),
			hWnd, DriverBlockPropDlgProc, (LPARAM)this))
		{
			BlockControl *b = new BlockControl;
			Blocks.Append(1, &b, 1);
			propStart *= GetTicksPerFrame();
...
int DriverBlockControl::ReplaceBlock(HWND hWnd, int whichBlock)
{
	MaxSDK::Array<TrackViewPick> res;
	DriverBlockTrackViewFilter filter;
...
void DriverBlockControl::AttachAddToList(int where, TrackViewPick res)
{
	if (res.anim)
	{
		...
		else
		{
			...
			TSTR str;
			TSTR str2;
			res.client->GetClassName(str, true);
			res.anim->GetClassName(str2, true);
...
int DriverBlockControl::RecurseSubs(TSTR matchString, Class_ID pid, Animatable* anim, TrackViewPick& r)
{
	if (anim)
	{
		for (int j = 0; j < anim->NumSubs(); j++)
		{
			...
			if (sub)
			{
				TSTR subAnim(anim->SubAnimName(j, true));
...
void DriverBlockControl::MatchNode(Tab<BOOL> selSet, HWND hParent)
{
	//popup node selection box
	Interface *ip = GetCOREInterface();
	DriverMatchNodeViewFilter filter;
	TrackViewPick res;
...
int DriverBlockControl::AttachAddMoveUp(int i)
{
	if (i <= 0) return 0;
	TSTR *Name;
	TrackViewPick ref;
...
int DriverBlockControl::AttachAddMoveDown(int i)
{
...
int DriverBlockControl::AttachAdd(HWND hListWnd)
{
	MaxSDK::Array<TrackViewPick> res;
	HWND hWnd = GetCOREInterface()->GetMAXHWnd();
	DriverBlockTrackViewFilterAdd filter;
...
int DriverBlockControl::AttachAddNullAt(HWND hListWnd, int where)
...
int DriverBlockControl::BuildNewBlock()

{
	...
	for (int i = 0; i < ccount; i++)
	{
		...
		else
		{
			...
			AppendBlockNoDriven(b, ct, pName, (Control *)c);
			ct++;
		}
...
int DriverBlockControl::AttachDeleteAt(HWND hListWnd, int where)
{
	//add names list
...
RefTargetHandle DriverBlockControl::Clone(RemapDir& remap)
{
...
void DriverBlockControl::GetValue(
...
void DriverBlockControl::GetValue2(
...
void DriverBlockControl::GetValue3(Control *sub,
...
void DriverBlockControl::Update(Control *sub,
...
void DriverBlockControl::SetValue(
...
INT_PTR CALLBACK AddBlockDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	DriverBlockControl *mb = DLGetWindowLongPtr<DriverBlockControl*>(hWnd);

	switch (msg)
	{
	case WM_INITDIALOG:
	{
		mb = (DriverBlockControl*)lParam;
		DLSetWindowLongPtr(hWnd, lParam);
		//load up list box 
		for (int i = 0; i < mb->Blocks.Count(); i++)
		{
			TSTR name = mb->SubAnimName(i + 1, true);
			SendMessage(GetDlgItem(hWnd, IDC_LIST1),
				LB_ADDSTRING, 0, (LPARAM)(const TCHAR*)name);
		}
		break;
...
INT_PTR CALLBACK DriverBlockPropDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_INITDIALOG:
	{
		...
		IColorSwatch *cs = GetIColorSwatch(GetDlgItem(hWnd, IDC_COLOR),
			Color(RGB(88, 88, 188)), _T("Block Color"));
		mb->propColor = Color(cs->GetAColor());
		ReleaseIColorSwatch(cs);
...
INT_PTR CALLBACK AttachBlockDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_INITDIALOG:
	{
		...
		//load up list box 
		IParamBlock2* pblock2 = mb->propContainer->GetParamBlockByID(container_params);
		DbgAssert(pblock2);
		if (pblock2)
		{
			for (int i = 0; i < pblock2->Count(container_refs); i++)
			{
				const TCHAR* name = NULL;
				pblock2->GetValue(container_names, 0, name, FOREVER, i);
				SendMessage(GetDlgItem(hWnd, IDC_LIST1),
	...
	case WM_COMMAND:
		...
		case IDC_MATCH_CONTROL:
		{
			Tab<BOOL> selSet;
			IParamBlock2* pblock2 = mb->propContainer->GetParamBlockByID(container_params);
			selSet.SetCount(pblock2->Count(container_names));
			for (int i = 0; i < pblock2->Count(container_names); i++)
			{
...
BOOL DriverBlockTrackViewFilter::proc(Animatable *anim, Animatable *client, int /*subNum*/)
{
	...
	if (anim->CanCopyTrack(FOREVER, 0) && anim->IsAnimated() && anim->CanCopyAnim()
		...
		&& (anim->ClassID() != Class_ID(POINTCTRLCONTAINER_CLASS_ID, 0))
...
BOOL DriverBlockTrackViewFilterAdd::proc(Animatable *anim, Animatable *client, int /*subNum*/)
...
BOOL DriverMatchNodeViewFilter::proc(Animatable *anim, Animatable *client, int /*subNum*/)
