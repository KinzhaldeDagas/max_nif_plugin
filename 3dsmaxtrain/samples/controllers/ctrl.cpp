...
void initClassDescArray(void)
{
   if( !classDescCount )
   {
      ...
      classDescArray[classDescCount++] = GetTransformListDesc();
	...
	classDescArray[classDescCount++] = GetMainLayerControlManagerDesc();
	...
	classDescArray[classDescCount++] = GetMainLayerDesc();
...
int computeHorizontalExtent(HWND hListBox, BOOL useTabs, int cTabs, LPINT lpnTabs)
{
   ...
   // if the list box is empty, just return 0
   if (width ==0)
      return 0;
   ...
   { /* compute the buffer size */
      ...
      if (useTabs) {
         DWORD sz = GetTabbedTextExtent(dc, Buffer, static_cast<int>(_tcslen(Buffer)), cTabs, lpnTabs );
         cx = LOWORD(sz);
      }
      else {
         SIZE sz;
