...
class SurfPosition : public ISurfPosition, public TimeChangeCallback,public ILockedTrackImp {
   public:
      Control *uCont, *vCont;
      INode *surf;
      int align;
      ...
      void GetClassName(MSTR& s, bool localized) const override {s = localized ? SURF_CONTROL_CNAME : _T("Surface");}
...
class SurfClassDesc:public ClassDesc {
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Surface"); }
   SClass_ID      SuperClassID() {return CTRL_POSITION_CLASS_ID;}
   Class_ID    ClassID() {return SURF_CONTROL_CLASSID;}
...
TSTR SurfPosition::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_RB_U) : _T("U"); break;
	case 1: return localized ? GetString(IDS_RB_V) : _T("V"); break;
	}
	return _T("");
}
      ...
      Animatable *control,int subAnim)
{
	if(GetLocked()==false)
	{
		if(SubAnim(subAnim) && GetLockedTrackInterface(SubAnim(subAnim)) &&GetLockedTrackInterface(SubAnim(subAnim))->GetLocked()==true)
			return FALSE;
...
RefResult SurfPosition::NotifyRefChanged(
      const Interval& iv, RefTargetHandle hTarg, 
...
void SurfPosition::Copy(Control *from)
{
   ...
   if (from->ClassID()==ClassID()) {
      ...
      ReplaceReference(U_REF,remap->CloneRef(cont->uCont));
      ReplaceReference(V_REF,remap->CloneRef(cont->vCont));
...
static INT_PTR CALLBACK SurfParamDlgProc(
   ...
   {
   SurfPosition *co = DLGetWindowLongPtr<SurfPosition*>(hWnd);
   switch (msg) {
      case WM_INITDIALOG:

         co = (SurfPosition*)lParam;
         DLSetWindowLongPtr(hWnd, co);
         co->SetupDialog(hWnd);
