...
// compile-time validates size of BKey class to ensure its size doesn't change since instances are 
// saved as a binary blob to scene file. Changes in size will break file i/o
static_assert(sizeof(BKey) == sizeof(TimeValue) + sizeof(DWORD) + sizeof(float), "");
...
static int __cdecl CompareBKeys(const void* left, const void* right)
{
	BKey* k1 = (BKey*)left;
	BKey* k2 = (BKey*)right;
...
class BoolCntrlKeyTable : public MaxSDK::Array<BKey>
{
public:     
	// track flags
	DWORD tflags;  
...
{
	if (length() == 0){    // no keys are present yet
		valid = FOREVER;
		return;
	}
	else if (length() == 1){
		...
		if (t >= (*this)[length()-1].time){
			val = (*this)[length()-1].val;
			valid.SetInstant(t);
			return; 
		}
...
{
	int n = (int)length(); // downcast to 2 gigs
	if (n==0) 
		return(-1);
	if (start < 0)  
		start = 0;
...
{  
	int h = rcGraph.h()-1;
	int n = (int)length(); // downcast to 2 Gigs
	int xo = 0, x, y[1], yo[1], i, k;
	Interval valid;
	TimeValue t, dt=1, pwt, wt=0, kx;
	float val=0.0;
	float fy, m;
	int pkey=-1, nkey;
...
class BoolCntrl : public IBoolCntrl, public ISetKey,public ILockedTrackImp
{
public:  
	...
	void GetClassName(MSTR& s, bool localized) const override {s = localized ? BOOL_CONTROL_CNAME : _T("Boolean Controller");}  
	...
	int GetFCurveExtents(
		ParamDimensionBase *dim,
		float &min, 
		float &max, 
		DWORD flags) { 
			min = 0.0f; max = 1.0f; return 1; 
	}
	...
	int NumKeys() { return (int)keys.length();} // downcast to 2 Gigs
	...
	// From IKeyControl
	int GetNumKeys() {return (int) keys.length();} // downcast to 2 Gigs
...
{
	if (!NumSelKeys()||GetLocked()==true) return; 
	...
	if(GetLocked()==false)
	{
		if (TestAFlag(A_SET)) {    
			...
			if (ivalid.InInterval(t)) {
				if (keys.length()==0) {
					if (Animating() && 
						(	GetCOREInterface10()->GetAutoKeyDefaultKeyOn() && 
						t!=0 && 
						(t != GetCOREInterface10()->GetAutoKeyDefaultKeyTime())
						)
						)
					{
						BKey k;
						...
						k.time = GetCOREInterface10()->GetAutoKeyDefaultKeyTime();
						keys.AddKey(this,GetCOREInterface10()->GetAutoKeyDefaultKeyTime(),k);
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Boolean Controller"); }
	SClass_ID      SuperClassID() { return CTRL_FLOAT_CLASS_ID; }
...
public:
	...
	TSTR Description() { return _T("BoolCntrlRestore"); }
};
...
public:
	...
	int length() { return (int)tab.length();} // downcast to 2 Gigs
	void Append(BKey &k) { tab.reserve(tab.length() + 10); tab.append(k); }
...
IOResult BoolCntrl::Save(ISave *isave){
	...
	if (keys.length()>0) {
		...
		isave->WriteVoid(keys.asArrayPtr(), sizeof(BKey) * (ULONG) keys.length(), &nb); // downcast to 2 Gigs
		isave->EndChunk();      
	}
		...
		case BKEY_CHUNK:
			{
				...
				keys.setLengthUsed(nkeys);
				if (nkeys>0) {
					res = iload->ReadVoid(keys.asArrayPtr(), nkeys * sizeof(BKey),&nb);
				}
			}
			break;
		case LOCK_CHUNK:
...
{
	if (from!=NULL&&GetLocked()==false)
	{
		DbgAssert(from->SuperClassID()==SuperClassID());
		float v;
		Interval iv;         
		int num;
		keys.removeAll();
		...
		BoolCntrl *fromB = dynamic_cast<BoolCntrl*>(from);
		if(fromB)
			mLocked = fromB->mLocked;
...
{
	if(GetLocked()==true||keys.TrackNotKeyable()) return;
	if (setkeyBuffer) {
...
//! [Animatable.SetSelKeyCoordsExpr Example]
...
void BoolCntrl::MapKeys(TimeMap *map,DWORD flags)
{
	if(GetLocked()==false)
	{
		int n = (int) keys.length(); // counting down, must use int, not a size_t
		BOOL changed = FALSE;
...
void BoolCntrl::DeleteTime(Interval iv, DWORD flags)
{
	if(GetLocked()==false)
	{
		...
		int n = (int) keys.length(); // for counting down, must use int, not a size_t
		int d = iv.Duration()-1;
		if (d<0) d = 0;
...
{
	HoldTrack();
	if (sel) keys[i].Setflags(KEY_ALLSEL);
	else     keys[i].Clearflags(KEY_ALLSEL);
	NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
}
...
void BoolCntrl::PasteTrack(TrackClipObject *cobj,Interval iv, DWORD flags)
{
	if(GetLocked()==false)
	{
		...
		size_t new_size = cob->tab.length();
		keys.reserve(keys.length() + new_size);
	...
	DWORD flags)
{
	...
	float cval;
	Interval valid(0, 0);
	GetValue(GetAnimStart(), &cval, valid);
...
void BoolCntrlKeyTable::KeysChanged(BOOL timeChanged)
{
	...
	if (!TestTFlag(RANGE_UNLOCKED)) {
		if (length()) {
			range.Set((*this)[0].time,(*this)[length()-1].time);
		} else {
			range.SetEmpty();
		}
	}
...
{  
	if(GetLocked()==false)
	{
		...
		if (GetSetKeyMode()) {
			...
			if(keys.TrackNotKeyable() && Animating())		// CAL-10/8/2002: test if track is keyable
				return;
			if (!TestAFlag(A_SET)) {            
				HoldTrack();
	...
	Rect& rcPaint,float tzoom,int tscroll,float vzoom,int vscroll,DWORD flags, DWORD tflags)  
{
	...
	DLGetTextExtent(hdc,_T("1234567890"),&size);
	...
	int el,int index,BOOL sel,float dval,float kval,int ktime,int x0,int y0,BOOL onTop)
{
	...
	if (showStat && sel) {
		...
		_stprintf(buf,_T("%s, %.3f"),(const TCHAR *)stime,dim->Convert(kval));
		DLTextOut(hdc,x0+4,y0+shift,buf);
	}
	return TRUE;
}
...
{  
	...
	ivalid.SetEmpty();
	if(GetLocked()==true)
		flags |= PAINTCURVE_FROZEN|PAINTCURVE_FROZENKEYS;
...
int BoolCntrl::AppendKey(IKey *key)
{
	if(GetLocked()==false)
	{
		...
		int res = (int) keys.append(k); // downcast to 2 Gigs
		keys.KeysChanged(FALSE);
		ivalid.SetEmpty();
		NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
		return res;
...
void* BoolCntrl::GetInterface(ULONG id)
{
	...
	return IBoolCntrl::GetInterface(id);
}


int BoolCntrl::GetKeyTimes(
...
BOOL BoolCntrl::GetNextKeyTime(TimeValue t,DWORD flags,TimeValue &nt)
{
	int count = static_cast<int>(keys.length());
	
	if (!count) 
		return FALSE;

	// Set startNdx to the index of greatest key <= t
	int startNdx = keys.FindKey(t);	
	if (flags & NEXTKEY_RIGHT) {
		if (startNdx == -1)
			startNdx = 0;
		for (int i = startNdx; i < count; i++) {
			if (keys[i].time > t ) {
				nt = keys[i].time;
				return TRUE;
				}
