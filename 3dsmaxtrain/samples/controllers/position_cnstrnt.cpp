...
class PosConstPosition : public	IPosConstPosition ,public ILockedTrackImp
{
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? POS_CONTROL_CNAME : _T("Position Constraint");}
		...
		TSTR SubAnimName(int i, bool localized) override { return localized ? GetString(IDS_AG_POSITIONPARAMS) : _T("Position Constraint"); }
...
void PosConstTimeChangeCallback::TimeChanged(TimeValue t){
	...
	Control *cont = pcontroller->pblock->GetControllerByID(position_target_weight, selection); 
...
class PosConstClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Position Constraint"); }
	SClass_ID		SuperClassID() { return CTRL_POSITION_CLASS_ID; }
...
void PickControlNode::EnterMode(IObjParam *ip)
	{
		...
		GetCOREInterface()->PushPrompt(_T("Select Position Target Object"));
	}
			...
			switch (msg) {
				...
				case WM_INITDIALOG:
					...
					if (p->GetnonNULLNumPosTargets() < 1) { 
						...
						// there is a valid selection 
						// automatically means there is AT LEAST one target
						Control *cont = p->pblock->GetControllerByID(position_target_weight, selection); 
						// until it is animated, paramblock doesn't have a controller assigned in it yet.
						if (spin) {
...
//								all buttons - delete, weight_edit, weight_spinner -- are enabled
								...
								Control *cont = p->pblock->GetControllerByID(position_target_weight, selection); // until it is animated, paramblock doesn't have a controller assigned in it yet.
								if (spin && (cont != NULL)) {
									if (cont->IsKeyAtTime(t,KEYAT_POSITION)){
										spin->SetKeyBrackets(TRUE);
									}
...
RefTargetHandle PosConstPosition::Clone(RemapDir& remap) // gets called when the controller gets copied
    ...
    p->ReplaceReference(POSPOS_PBLOCK_REF, remap.CloneRef(pblock));
...
void PosConstPosition::Copy(Control *from) // gets called when my controller is applied over the top of another controller
{
	...
	if (from->ClassID()==ClassID()) {
		...
		ReplaceReference(POSPOS_PBLOCK_REF, CloneRefHierarchy(ctrl->pblock));
		curval   = ctrl->curval;
		flags    = ctrl->flags;
...
void PosConstPosition::Update(TimeValue t){
	...
	for (int i = 0; i < ct; i++) {
		...
		pblock->GetValue(position_target_list, t, pos_target, i);
...
INode* PosConstPosition :: GetNode(int nodeNumber){
	if (nodeNumber >= 0 && nodeNumber < pblock->Count(position_target_list)){
		INode *pos_targ;
		pblock->GetValue(position_target_list, 0, pos_targ, nodeNumber);
		return pos_targ;
	}
	return NULL;
}
...
float PosConstPosition::GetTargetWeight(int targetNumber){
	...
	if (targetNumber >= 0 && targetNumber < ct){
		float posWt;
		pblock->GetValue(position_target_weight, GetCOREInterface()->GetTime(), posWt, targetNumber);
		return posWt;
	}
	else{
		return 0.0f;
...
void PosConstPosition::EndEditParams( IObjParam *ip, ULONG flags,Animatable *next )
{
	if(editCont)
	{
		...
		if (pmap != NULL)
		{
			if (next && next->ClassID() == ClassID() && ((PosConstPosition*)next)->pblock  &&  ((PosConstPosition*)next)->GetLocked()==false)
			{
