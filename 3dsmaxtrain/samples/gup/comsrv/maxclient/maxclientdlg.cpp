...
bool CMaxClientDlg::CreateObjectInstance(REFCLSID rclsid, REFIID riid, LPVOID* ppv)
{
	...
	// Note: We normally use MaxSDK::MaxMessageBox instead of MessageBox. Unfortunately,
	// it is not convenient to use MaxSDK::MaxMessageBox in this project because its output
	// is an executable. For this reason, we use MessageBox in this function.

	if (IsDlgButtonChecked(IDC_LOCAL)) // If local, simply use registry
	{
		hr = CoCreateInstance(rclsid, NULL, CLSCTX_LOCAL_SERVER, riid,ppv);

		if (FAILED(hr))
		{
			MessageBox(_T("Error Creating Local Instance of object"));
			return false;
		}
	}
	else // If remote, use given server name
	{
		TCHAR server_name[MAX_PATH];
		if (!GetDlgItemText(IDC_SERVERNAME, server_name, MAX_PATH))
		{
			MessageBox(_T("Need a server name if not set to Local"));
			return false;
		}
		else
		{
			...
			{
				TCHAR txt[256];
				_stprintf(txt, _T("Error Creating Remote Instance (hr:%0X)"), hr);
				MessageBox(txt);
				return false;
			}
			else
...
void CMaxClientDlg::OnLoadMaxFile()
{
	...
	try
	{
		...
		{
			CString txt;
			txt.Format(_T("Error loading %s"), a_name);
			MessageBox(LPCTSTR(txt), _T("Load MAX Scene"));
		}
		else
		{
			bMaxFileLoaded = true;
...
void CMaxClientDlg::OnScript()
{
	...
	try
	{
		...
		{
			CString txt;
			txt.Format(_T("Error executing %s"), a_name);
			MessageBox(LPCTSTR(txt), _T("Execute MAX Script"));
		}
	}
	catch(_com_error& e)
	{
		TRACE(_T("Error Executing %s\n"), a_name);
		TRACE(_T("%s\n"), e.ErrorMessage());
		if (e.ErrorInfo())
		{
			TRACE(_T("%s\n"), e.Description());
...
void CMaxClientDlg::OnRender() {
	...
	try {
		...
		CT2OLE b_name(const_cast<LPTSTR>(LPCTSTR(CameraName)));

#ifdef RENDER_REGION

		pMax->OpenRenderer((LPOLESTR)b_name,pBif,TRUE);		//-- Open Renderer with camera and Output Settings

#else

		pMax->OpenRenderer((LPOLESTR)b_name,pBif,FALSE);		//-- Open Renderer with camera and Output Settings
		...
		float frameno = (float)(_tstof(framestr));
		...
		TRACE(_T("Error Rendering Frame\n"));
		TRACE(_T("%s\n"),e.ErrorMessage());
		if (e.ErrorInfo()) {
			TRACE(_T("%s\n"),e.Description());
		}
...
void CMaxClientDlg::OnCancelRender() {
	pMax->CancelRenderer();
	m_MessagesControl.SetWindowText(_T("Cancel Pending..."));
}
...
void CMaxClientDlg::OnListcameras() {
	...
	try {												//-- Exception Handling
		...
		TRACE(_T("Error Enumerating Cameras\n"));
		TRACE(_T("%s\n"),e.ErrorMessage());
		if (e.ErrorInfo()) {
			TRACE(_T("%s\n"),e.Description());
		}
...
STDMETHODIMP CMaxRendererEvents::OnRenderDone( void ) {
	...
	dlg->m_MessagesControl.SetWindowText(_T("Rendering Complete"));
	dlg->UpdateControls();
	return S_OK;
}
