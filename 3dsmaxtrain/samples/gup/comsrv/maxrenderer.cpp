...
int CamListImp::callback(INode *node) {
	...
	if (ob!=NULL) {
		if (ob->SuperClassID()==CAMERA_CLASS_ID) {
			if (fire_back) {
				cmax->Fire_OnEnumCameras(CT2OLE(node->GetName()));
			} else {
				if (!camNode) {
...
void maxRndProgressCB::SetTitle( const TCHAR *title ) {
	if (cmax) {
		cmax->OnRenderMessage( CT2OLE((TCHAR *) title) );
	}
}
...
STDMETHODIMP CMaxRenderer::LoadScene(BSTR SceneName) {
	COLE2T filename(SceneName);
	Interface *max = Max();
	if (!max->LoadFromFile(filename)) {
		TCHAR errtxt[MAX_PATH];
		_stprintf(errtxt,GetString(IDS_NO_FILE_ERROR),static_cast<LPCWSTR>(filename));
		return Error(errtxt);
	}
	return S_OK;
}
...
STDMETHODIMP CMaxRenderer::SaveScene(BSTR SceneName) {
	...
	if (!max->SaveToFile(filename)) {
		TCHAR errtxt[MAX_PATH];
		// if you're scratching your head at the static_cast, see https://docs.microsoft.com/en-us/cpp/cpp-conformance-improvements-2017
		_stprintf(errtxt,GetString(IDS_NO_FILE_ERROR),static_cast<LPCWSTR>(filename));
		return Error(errtxt);
	}
	return S_OK;
...
STDMETHODIMP CMaxRenderer::OpenRenderer(BSTR CameraName, IMaxBitmapInfo* pBif, BOOL region) {
	...
	clp->fire_back	= false;
	clp->cmax		= this;
	COLE2T a_name(CameraName);
	_tcsncpy_s(clp->name, _countof( clp->name), a_name, _TRUNCATE);
	EnumTree(clp);

	if (!clp->camNode) {
		TCHAR errtxt[MAX_PATH];
		_stprintf(errtxt,GetString(IDS_NO_CAMERA_ERROR), static_cast<LPCWSTR>(a_name));
		delete clp;
		return Error(errtxt);
	}
...
//
#pragma message("SafeSceneLoading TODO : CMaxRenderer::ExecuteMAXScriptFile ScriptSource")
...
STDMETHODIMP CMaxRenderer::ExecuteMAXScriptString(BSTR String) {
	COLE2T string(String);
	if (!ExecuteStringScript(string, MAXScript::ScriptSource::Dynamic))
		return Error(GetString(IDS_SCRIPT_ERROR));
	return S_OK;
}
