...
DWORD SaveVerificationGUP::Start()
{
	...
	RegisterNotification(PostSaveCallback, this, NOTIFY_FILE_POST_SAVE_PROCESS);
...
void SaveVerificationGUP::Stop()
{
	...
	UnRegisterNotification(PostSaveCallback, this, NOTIFY_FILE_POST_SAVE_PROCESS);
}
...
void SaveVerificationGUP::PostSaveCallback(void* param, NotifyInfo* info)
{
	...
	if (pBailOutManager && pBailOutManager->IsBailOut())
	{
		return;
	}

	SaveVerificationGUP* pThis = (SaveVerificationGUP*)param;
	if (const NotifyFileProcessParam* nfp = GetNotifyParam<NOTIFY_FILE_POST_SAVE_PROCESS>(info))
	{
		if (nfp->fileOperationSuccessful)
		{
			pThis->ValidateFile(nfp->fname);
		}
	}
}	
...
void SaveVerificationGUP::ValidateFile(const TSTR& filename)
{
	...
	if (!tmpFileName.isNull())
	{
		...
		_tsplitpath(tmpFileName, drive, dir, NULL, NULL);
		_tmakepath_s(fullpath, _countof(fullpath), drive, dir, NULL, NULL);
		if (!DoesFileExist(fullpath))
		{
			Interface* ip = GetCOREInterface();
			TSTR strErrorPathMissing = GetString(IDS_ERROR_PATH_MISSING);
			ip->Log()->LogEntry(SYSLOG_ERROR,
								DISPLAY_DIALOG,
								GetString(IDS_SAVEVERIFICATION),
								strErrorPathMissing,
								fullpath);
		}
		...
		{
			Interface* ip = GetCOREInterface();
			TSTR strErrorFileCorrupt = GetString(IDS_ERROR_FILE_CORRUPT);
			ip->Log()->LogEntry(SYSLOG_ERROR,
								DISPLAY_DIALOG,
								GetString(IDS_SAVEVERIFICATION),
								strErrorFileCorrupt,
...
bool SaveVerificationGUP::ValidateStreams(const TSTR& filename)
{
	...
	res = pInterface11->OpenMAXStorageFile(filename.ToUTF16(), &pIStorage);
	if (res == S_OK)
	{
		if (!HasStream(pIStorage, _T("AutodeskDataPackage")))
		{
			// Verify that the Scene stream is present.
			if (!HasStream(pIStorage, _T("Scene")) && !HasStream(pIStorage, _T("Scene_Compressed")))
				validFile = false;

			// Verify that the Config stream is present.
			if (!HasStream(pIStorage, _T("Config")) && !HasStream(pIStorage, _T("Config_Compressed")))
				validFile = false;

			// Verify that the ClassData stream is present.
			if (!HasStream(pIStorage, _T("ClassData")) && !HasStream(pIStorage, _T("ClassData_Compressed")))
				validFile = false;

			// Verify that the DllDirectory stream is present.
			if (!HasStream(pIStorage, _T("DllDirectory")) && !HasStream(pIStorage, _T("DllDirectory_Compressed")))
				validFile = false;

			// Verify that the AssetMeta stream is present. This is the last stream written.
			if (!HasStream(pIStorage, _T("FileAssetMetaData3")))
				validFile = false;
		}
...
bool SaveVerificationGUP::HasStream(IStorage* storage, TSTR stream)
{
	...
	HRESULT res = storage->OpenStream (stream.ToUTF16(), NULL,  STGM_READ | STGM_SHARE_EXCLUSIVE, 0, &pIStream);
	if (res == S_OK)  
	{
		pIStream->Release();
		pIStream = NULL;
