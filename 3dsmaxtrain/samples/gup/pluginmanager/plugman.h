...
BOOL pluginHasClassesRegistered( int mainCode, Interface *ip );
...
{
public:
	PluginBucket(bool selected, const std::set<TSTR>& paths) : mChecked(selected), mPaths(paths) {}
	bool ContainsPlugin(const TSTR& path) const;
	bool mChecked{ false };
	std::set<TSTR> mPaths;
};
...
class PlugMgrUtility : public GUP, public FPStaticInterface, public MaxSDK::Util::Noncopyable 
{
	...
	protected:
		...
		// A wrapper for LoadClass since can pass a null in as ClassDesc*
		void fpLoadClass(ClassDesc* cd);
		...
		// dispatch map
		BEGIN_FUNCTION_MAP
			VFN_1(pm_loadClass,    fpLoadClass, TYPE_CLASS);
	...
	private:
		...
		void InsertDirectorySelector(HWND pd,
											  int index,
