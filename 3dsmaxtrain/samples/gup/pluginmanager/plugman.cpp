...
class PlugMgrClassDesc : public ClassDesc2 {
   ...
   const TCHAR*   NonLocalizedClassName() { return _T("Plug-in Manager"); }
...
void PlugMgrUtility::LoadClass(ClassDesc* cd)         // ensure deferred plugin class is loaded - JBW 7.4.00
{
   ...
   if (cd)
   {
		...
		ClassEntry* ce = cdir.FindClassEntry(cd->SuperClassID(), cd->ClassID());
		if (ce)
			ce->FullCD();
   }
}
...
const MCHAR* PlugMgrUtility::GetPluginDllDirectory(int index)	
{
	...
	return GetCOREInterface()->GetDllDir()[index].GetDirectoryName();
}
...
void PlugMgrUtility::fpLoadClass(ClassDesc* cd)
{
	if (cd == nullptr)
		throw MAXException(GetString(IDS_UNDEFINED_CLASS));
	LoadClass(cd);
}
