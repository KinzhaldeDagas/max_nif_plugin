...
#include <wtypes.h>
#include "PluginPackageManager/PluginPackageManager.h"
...
bool PluginBucket::ContainsPlugin(const TSTR& path) const
{
	return mPaths.find(path) != mPaths.cend();
}
...
void newPluginLoadedProc(void *param, NotifyInfo *info)
{
   ...
   {
      PlugMgrUtility* pmu = static_cast<PlugMgrUtility*>(param);
      DllDesc* dsc = GetNotifyParam<NOTIFY_PLUGIN_LOADED>(info);
      if (!dsc) return;
...
void dbgMsg(const TCHAR *format, ...)
{
	...
	MaxSDK::MaxMessageBox(NULL, buf, _T("Debug Msg"), MB_OK);
}
...
static const TCHAR *GetColumnName( int i )
{
	switch (i)
	{
...
{
	const PluginData *d1 = reinterpret_cast<const PluginData*>(arg1);
	const PluginData *d2 = reinterpret_cast<const PluginData*>(arg2);
...
/*===========================================================================*\
| Rebuild the internal representation of the main DLL Directory
\*===========================================================================*/
...
bool rebuildCache( Interface *ip, PlugMgrUtility *pmu, HWND hWnd )
{
	...
	for (int i=0; i<dd->Count() ;i++)
	{
		...
		for (int a=0; a<numItem; a++)
		{
			...
			if (onOff)
			{
				if (pmu && pmu->pdrec[a].ContainsPlugin(MaxSDK::Util::Path(myDesc->GetDirectoryName()).Normalize().ConvertToLowerCase().GetString()))
				{
					processPlugin = true;
					break;
				}
	...
	LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		{
			pmu = reinterpret_cast<PlugMgrUtility*>(lParam);
	...
	case WM_NOTIFY:
		{
			if(pmu->UI_active)
			{
				...
				if(nm->code == LVN_ITEMCHANGED&&nm->idFrom==IDC_PLUGDIR)
				{
					...
					for(int a=0;a<numItem;a++)
					{
						bool bTmp = ListView_GetCheckState(pd,a) ? true : false;
						if (pmu->pdrec[a].mChecked != bTmp) needRebuild = TRUE;
						pmu->pdrec[a].mChecked = bTmp;
					}
					if(needRebuild)
					{
...
void PlugMgrUtility::InsertDirectorySelector(HWND pd,
															int index,
															const TCHAR* plugInDesc,
															const TCHAR* plugInAuthor,
															const TCHAR* plugInVersion,
															const TCHAR* plugInPath)
{
	if (pd != nullptr && index >= 0 && plugInDesc != nullptr && plugInPath != nullptr)
	{
		LV_ITEM item;
		item.mask = LVIF_TEXT | LVIF_IMAGE;
		item.iImage = -1;
		item.iItem = index;
		item.iSubItem = 0;
		item.pszText = const_cast<wchar_t *>(plugInDesc);
		item.cchTextMax = static_cast<int>(_tcslen(plugInDesc)) + 1;   // SR DCAST64: Downcast to 2G limit.
		...
		if (!plugInAuthor) { plugInAuthor = _T(""); }
		item.pszText = const_cast<wchar_t*>(plugInAuthor);
		item.cchTextMax = static_cast<int>(_tcslen(plugInAuthor)) + 1; // SR DCAST64: Downcast to 2G limit.
		...
		if (!plugInVersion) { plugInVersion = _T(""); }
		item.pszText = const_cast<wchar_t*>(plugInVersion);
		item.cchTextMax = static_cast<int>(_tcslen(plugInVersion)) + 1; // SR DCAST64: Downcast to 2G limit.
		ListView_SetItem(pd, &item);
		...
		item.pszText = const_cast<wchar_t *>(plugInPath);
		item.cchTextMax = static_cast<int>(_tcslen(plugInPath)) + 1; // SR DCAST64: Downcast to 2G limit.
		ListView_SetItem(pd, &item);

		ListView_SetCheckState(pd, index, TRUE);
	}
}

void PlugMgrUtility::Init(HWND hWnd)
{
	using MaxSDK::PluginPackageManager;
	using MaxSDK::PluginPackageComponent;
	...
	lvcolumn.pszText = const_cast<TCHAR*>(GetColumnName(0));
	lvcolumn.cx = 35;
	lvcolumn.iSubItem = 0;
	...
	lvcolumn.mask = LVCF_FMT | LVCF_SUBITEM | LVCF_WIDTH | LVCF_TEXT;
	lvcolumn.fmt = LVCFMT_LEFT;
	lvcolumn.pszText = const_cast<TCHAR*>(GetColumnName(1));
	lvcolumn.cx = 140;
	lvcolumn.iSubItem = 1;
	...
	lvcolumn.mask = LVCF_FMT | LVCF_SUBITEM | LVCF_WIDTH | LVCF_TEXT;
	lvcolumn.fmt = LVCFMT_LEFT;
	lvcolumn.pszText = const_cast<TCHAR*>(GetColumnName(2));
	lvcolumn.cx = 220;
	lvcolumn.iSubItem = 2;
	...
	lvcolumn.mask = LVCF_FMT | LVCF_SUBITEM | LVCF_WIDTH | LVCF_TEXT;
	lvcolumn.fmt = LVCFMT_LEFT;
	lvcolumn.pszText = const_cast<TCHAR*>(GetColumnName(3));
	lvcolumn.cx = 70;
	lvcolumn.iSubItem = 3;
	...
	lvcolumn.mask = LVCF_FMT | LVCF_SUBITEM | LVCF_WIDTH | LVCF_TEXT;
	lvcolumn.fmt = LVCFMT_LEFT;
	lvcolumn.pszText = const_cast<TCHAR*>(GetColumnName(4));
	lvcolumn.cx = 60;
	lvcolumn.iSubItem = 4;
	...
	lvcolumn.mask = LVCF_FMT | LVCF_SUBITEM | LVCF_WIDTH | LVCF_TEXT;
	lvcolumn.fmt = LVCFMT_LEFT;
	lvcolumn.pszText = const_cast<TCHAR*>(GetColumnName(5));
	lvcolumn.cx = 500;
	lvcolumn.iSubItem = 5;
	...
	lvcolumn.mask = LVCF_FMT | LVCF_SUBITEM | LVCF_WIDTH | LVCF_TEXT;
	lvcolumn.fmt = LVCFMT_LEFT;
	lvcolumn.pszText = GetString(IDS_AUTHOR);
	lvcolumn.cx = 100;
	lvcolumn.iSubItem = 1;
	...
	lvcolumn.mask = LVCF_FMT | LVCF_SUBITEM | LVCF_WIDTH | LVCF_TEXT;
	lvcolumn.fmt = LVCFMT_LEFT;
	lvcolumn.pszText = GetString(IDS_VERSION);
	lvcolumn.cx = 75;
	lvcolumn.iSubItem = 2;
	ListView_InsertColumn(pd, 2, &lvcolumn);
	...
	lvcolumn.cx = 500;
	lvcolumn.iSubItem = 3;
	ListView_InsertColumn(pd, 3, &lvcolumn);

	int nPIE = ip->GetPlugInEntryCount();

	PluginPackageManager* pManager = PluginPackageManager::GetInstance();
	size_t nPNativeBundle = pManager->GetPackageCount();
	size_t nPackagesWithPlugins = 0;
	// count the native bundles that contain plugins
	for (size_t j = 0; j < nPNativeBundle; j++)
	{
		const MaxSDK::PluginPackageInfo* pInfo = pManager->GetPackage(j);
		const MCHAR* pName = pInfo->GetPackageName();
		const MCHAR* pPath = pInfo->GetPackageDirectory();
		size_t numPlugins  = pManager->GetPackageComponentCount(j, PluginPackageManager::Components::Plugins);
		if (pName != nullptr && pPath != nullptr && numPlugins > 0)
		{
			++nPackagesWithPlugins;
		}
	}
	pdrec.reserve(nPIE + nPackagesWithPlugins);
	...
	for(; i < nPIE; i++)
	{
		bool madeByAutodesk = false;
		const MSTR plugInDescStr = ip->GetPlugInDesc(i);
		// "Standard MAX plug-ins" are defined as INI_KEY_STDPLUGS_DESC in ..\include\ini_section_key_names.h.
		// "Additional MAX plug-ins" is read from file Plugin.UserSettings.ini.
		if (_tcscmp(plugInDescStr, _T("Standard MAX plug-ins")) == 0)
		{
			madeByAutodesk = true;
			_tcsncpy(pluginDescBuf, GetString(IDS_KEY_STDPLUGS_DESC), MAX_PATH);
		}
		else if (_tcscmp(plugInDescStr, _T("Additional MAX plug-ins")) == 0)
		{
			_tcsncpy(pluginDescBuf, GetString(IDS_ADDITIONAL_PLUGS_DESC), MAX_PATH);
		}
		else
		{
			_tcsncpy(pluginDescBuf, plugInDescStr, MAX_PATH);
		}

		std::set<TSTR> s;
		TSTR plugInDir = MaxSDK::Util::Path(ip->GetPlugInDir(i)).Normalize().ConvertToLowerCase().GetString();
		s.emplace(plugInDir);
		pdrec.push_back(PluginBucket(true, s));
		InsertDirectorySelector(pd, i, pluginDescBuf, madeByAutodesk ? GetString(IDS_AUTODESK) : nullptr, nullptr, plugInDir);
	}
	...
	for (size_t j = 0; j < nPNativeBundle; j++)
	{
		...
		size_t nPluginComponents = pManager->GetPackageComponentCount(j, PluginPackageManager::Components::Plugins);
		if (pName != nullptr && pPath != nullptr && nPluginComponents > 0)
		{
			std::set<TSTR> s;
			while (nPluginComponents)
			{
				--nPluginComponents;
				const PluginPackageComponent* pComp = pManager->GetPackageComponent(j, PluginPackageManager::Components::Plugins, nPluginComponents);
				if (pComp)
				{
					MSTR path = MaxSDK::Util::Path(pComp->GetComponentFullPath()).RemoveLeaf().AddTrailingBackslash().Normalize().ConvertToLowerCase().GetString();
					s.emplace(path);
				}
			}

			pdrec.push_back(PluginBucket(true, s));
			InsertDirectorySelector(pd, i++, pName, pInfo->GetCompanyName(), pInfo->GetPackageVersionString(), pPath);
		}
	}
