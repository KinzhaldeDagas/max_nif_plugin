...
class nullCD:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("NULL"); }
...
bool LoadNewDLL( HWND hWnd, Interface *ip)
{
		...
		if(manager->GetPlugInEntryCount() > 0)	{
			_tcsncpy(initDir, manager->GetPlugInDir(0).data(), MAX_PATH);
		...
		ofn.Flags = OFN_FILEMUSTEXIST | OFN_EXPLORER | OFN_ENABLEHOOK | OFN_ENABLESIZING | OFN_HIDEREADONLY;
...
BOOL pluginHasClassesRegistered( int mainCode, Interface *ip )
{
	...
	for(int i=0;i<cd->Count();i++)
	{
		...
		{
			ClassEntry *ce = &(*scl)[q];
			if(ce->DllNumber() == mainCode) return TRUE;
		}
	}
	return FALSE;
}
