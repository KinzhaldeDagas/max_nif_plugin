...
bool MXSGeometryChecker::IsSupported(INode * node)
{
	...
	try 
	{
		...
		bool bresult = result->to_bool()?true:false;
		return bresult;
	}
	...
	{
		ProcessMAXScriptException(e, MaxSDK::GetResourceStringAsMSTR(IDS_REGISTER_EXCEPTION_SUPPORT), true, true, true);
	}
	catch (...)
	{
		ProcessUnknownMAXScriptException(MaxSDK::GetResourceStringAsMSTR(IDS_REGISTER_EXCEPTION_SUPPORT), true, true, true);
	}
...
IGeometryChecker::ReturnVal MXSGeometryChecker::GeometryCheck(TimeValue t,INode *node, IGeometryChecker::OutputVal &val)
{
	...
	ScopedMaxScriptEvaluationContext scopedMaxScriptEvaluationContext;
	MAXScript_TLS* _tls = scopedMaxScriptEvaluationContext.Get_TLS();
	three_value_locals_tls(t,node,index);	
...
void MXSGeometryChecker::ShowPropertyDlg()
{
	...
	{
		ProcessMAXScriptException(e, MaxSDK::GetResourceStringAsMSTR(IDS_REGISTER_EXCEPTION_POPUP), true, true, true);
	}
	catch (...)
	{
		ProcessUnknownMAXScriptException(MaxSDK::GetResourceStringAsMSTR(IDS_REGISTER_EXCEPTION_POPUP), true, true, true);
	}
...
MSTR MXSGeometryChecker::TextOverride()
{
	...
	{
		ProcessMAXScriptException(e, MaxSDK::GetResourceStringAsMSTR(IDS_REGISTER_EXCEPTION_TEXT), true, true, true);
	}
	catch (...)
	{
		ProcessUnknownMAXScriptException(MaxSDK::GetResourceStringAsMSTR(IDS_REGISTER_EXCEPTION_TEXT), true, true, true);
	}
...
void MXSGeometryChecker::DisplayOverride(TimeValue t, INode* node, HWND hwnd,Tab<int> &results)
{
	...
	ScopedMaxScriptEvaluationContext scopedMaxScriptEvaluationContext;
	MAXScript_TLS* _tls = scopedMaxScriptEvaluationContext.Get_TLS();
	four_value_locals_tls(t,node,hwnd,index);	
	...
	{
		ProcessMAXScriptException(e, MaxSDK::GetResourceStringAsMSTR(IDS_REGISTER_EXCEPTION_DISPLAY), true, true, true);
	}
	catch (...)
	{
		ProcessUnknownMAXScriptException(MaxSDK::GetResourceStringAsMSTR(IDS_REGISTER_EXCEPTION_DISPLAY), true, true, true);
	}
