...
// High priority callbacks are called before non-high priority callbacks.
// Ideally, ViewportDisplayCallback should provide a Priority() property
// (such as PostLoadCallback) so display callbacks can be sorted. However,
// SDK is frozen so right now simplified to high priority or not.
#define HIGH_PRIORITY_CALLBACK_INTERFACE Interface_ID(0xc5b791e, 0x66e58a4)

//handles drawing the resulting values in the viewport. The text button is handled by the viewportbutton system.
class GeometryViewportDisplay : public ViewportDisplayCallback, public ISceneCallbackPriority
{
public:
...
class GeometryManagerClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("xView"); }
...
DWORD IGeometryCheckerManager_Imp::Start()
{
	...
	MSTR name, category;
	name.printf(_T("%s"),GetString(IDS_DISPLAY_COLOR));
	category.printf(_T("%s"),GetString(IDS_GEOMETRY_CHECKER));
	...
	name.printf(_T("%s"),GetString(IDS_TEXT_COLOR));
...
void IGeometryCheckerManager_Imp::DeleteThis()
{
...
void IGeometryCheckerManager_Imp::SetUpButton()
{
	...
	if(checker&&GetGeometryCheckerOn())
	{
		...
		ResetButtonLocations(GetCOREInterface()->GetActiveViewExp().ToPointer());
...
void IGeometryCheckerManager_Imp::PopupMenuSelect()
{
	...
	for (int i = 0; i < mGeometryCheckers.Count();i++)
	{
		if(mGeometryCheckers[i])
		{
			...
			AppendMenu(  hMenu,      // handle to menu to be changed
				  ...
				  mGeometryCheckers[i]->GetName().data() // menu-item content
				  );
		}
	...
	ViewExp& vexp =GetCOREInterface()->GetActiveViewExp();
	if(vexp.IsAlive())
	{
	...
	case WM_INITDIALOG:
		{
			...
			_stprintf(newStr,str,man->mNumFaces);
...
void IGeometryCheckerManager_Imp::ClearResults()
{
	mValid = false;
	mLastValidTime = -1;
	mCheckResult = GetString(IDS_FAILED);
	mReturnVal = IGeometryChecker::eFail;
	DeleteCachedResults();
	SetUpButton();
}
	...
	if(GetGeometryCheckerOn())
	{
		...
		if(runningCheck==false)
		{
			...
			if(checker&&mNotifyMgr.NumRefs()>0)
			{
				...
				MSTR type;
				TCHAR count[32];
				_stprintf(count,_T("%d"),totalCount);
				if(noSelectionWasValid==false)
				{
					...
					case IGeometryChecker::eFail:
						...
						break;
					case IGeometryChecker::eVertices:
						type = GetString(IDS_VERTICES);
						mCheckResult = MSTR(count) + _M(" ") + type;
						break;
					case IGeometryChecker::eEdges:
						type = GetString(IDS_EDGES);
						mCheckResult = MSTR(count) + _M(" ") + type;
						break;
					case IGeometryChecker::eFaces:
						type = GetString(IDS_FACES);
						mCheckResult = MSTR(count) + _M(" ") + type;
						break;
					}
				}
...
RefResult IGeometryCheckerManager_Imp::NotifyFunc(const Interval& changeInt,
            RefTargetHandle hTarget, PartID& partID,
            RefMessage message, BOOL propagate)
{
...
void GeometryViewportDisplay::Display(TimeValue t, ViewExp *vpt, int flags)
{
	...
	{
		if(vpt->IsActive() && mGeomCheckerMan->NotValid(t)==true)
		{
...
BaseInterface* GeometryViewportDisplay::GetInterface(Interface_ID id)
{
	BaseInterface* priorityInterface = ISceneCallbackPriority::GetInterface(id);
	if (priorityInterface != nullptr)
	{
		return priorityInterface;
	}

	return ViewportDisplayCallback::GetInterface(id);
}

int GeometryViewportDisplay::GetPriority() const
{
	return 10;
}
...
void DisplayIn3DViewport::DisplayResults(DisplayIn3DViewport::DisplayParams &params,TimeValue t,INode *node, HWND hwnd, IGeometryChecker::ReturnVal type,Tab<int> &indices)
{
	...
	ObjectState os = node->EvalWorldState(t);
	IGeometryChecker::OutputVal val;
	ViewExp& vpt = GetCOREInterface()->GetViewExp(hwnd);
	if( ! vpt.IsAlive())
		return;
			...
			case IGeometryChecker::eFaces:
				{
					...
					else
					{
						...
						if(params.mSeeThrough==false)
							zbuffer = GW_Z_BUFFER;
						gw->setRndLimits(GW_BACKCULL|GW_SHADE_SEL_FACES|GW_COLOR_VERTS|zbuffer|illum);
						bool dontDrawSelected = poly->GetSubselState()==MNM_SL_FACE ? true :false;
		...
		Value *displayOverrideFunc)
{
	...
	//make stuff NULL
	five_value_locals_tls(checkerFunc, isSupportedFunc, popupFunc, textOverrideFunc, displayOverrideFunc);
	vl.checkerFunc = (checkerFunc == &undefined) ? NULL : checkerFunc;
	vl.isSupportedFunc = (isSupportedFunc == &undefined) ? NULL : isSupportedFunc;
	vl.popupFunc = (popupFunc == &undefined) ? NULL : popupFunc;
	vl.textOverrideFunc = (textOverrideFunc == &undefined) ? NULL : textOverrideFunc;
	vl.displayOverrideFunc = (displayOverrideFunc == &undefined) ? NULL : displayOverrideFunc;
	...
	if (vl.checkerFunc)
	{
		...
		if (!GetMAXScriptFunctionParameterCount(vl.checkerFunc, parameterCount))
			throw RuntimeError (GetString(IDS_REGISTER_BAD_FUNC), vl.checkerFunc);
		if (parameterCount != 3)
			throw RuntimeError (GetString(IDS_REGISTER_BAD_NUM_ARGS_CHECKER), Integer::intern(parameterCount));
		vl.checkerFunc = CreateWrappedMAXScriptFunction(vl.checkerFunc, _tls);
	}
	else
		throw RuntimeError(GetString(IDS_REGISTER_NEED_CHECKER));
	if (vl.isSupportedFunc)
	{
		int parameterCount = 0;
		if (!GetMAXScriptFunctionParameterCount(vl.isSupportedFunc, parameterCount))
			throw RuntimeError (GetString(IDS_REGISTER_BAD_FUNC), vl.isSupportedFunc);
		if (parameterCount != 1)
			throw RuntimeError (GetString(IDS_REGISTER_BAD_NUM_ARGS_SUPPORT), Integer::intern(parameterCount));
		vl.isSupportedFunc = CreateWrappedMAXScriptFunction(vl.isSupportedFunc, _tls);
	}
	else
	...
	{
		int parameterCount = 0;
		if (!GetMAXScriptFunctionParameterCount(vl.popupFunc, parameterCount))
			throw RuntimeError (GetString(IDS_REGISTER_BAD_FUNC), vl.popupFunc);
		if (parameterCount != 0)
			throw RuntimeError (GetString(IDS_REGISTER_BAD_NUM_ARGS_POPUP), Integer::intern(parameterCount));
		vl.popupFunc = CreateWrappedMAXScriptFunction(vl.popupFunc, _tls);
	}
	if (vl.textOverrideFunc)
	{
		int parameterCount = 0;
		if (!GetMAXScriptFunctionParameterCount(vl.textOverrideFunc, parameterCount))
			throw RuntimeError (GetString(IDS_REGISTER_BAD_FUNC), vl.textOverrideFunc);
		if (parameterCount != 0)
			throw RuntimeError (GetString(IDS_REGISTER_BAD_NUM_ARGS_TEXT), Integer::intern(parameterCount));
		vl.textOverrideFunc = CreateWrappedMAXScriptFunction(vl.textOverrideFunc, _tls);
	}
	if (vl.displayOverrideFunc)
	{
		int parameterCount = 0;
		if (!GetMAXScriptFunctionParameterCount(vl.displayOverrideFunc, parameterCount))
			throw RuntimeError (GetString(IDS_REGISTER_BAD_FUNC), vl.displayOverrideFunc);
		if (parameterCount != 4)
			throw RuntimeError (GetString(IDS_REGISTER_BAD_NUM_ARGS_DISPLAY), Integer::intern(parameterCount));
		vl.displayOverrideFunc = CreateWrappedMAXScriptFunction(vl.displayOverrideFunc, _tls);
	}
	...
	MXSGeometryChecker *mxsChecker = new MXSGeometryChecker(vl.checkerFunc, vl.isSupportedFunc,val, name,  vl.popupFunc, vl.textOverrideFunc,
		vl.displayOverrideFunc);
	int index = RegisterGeometryChecker(mxsChecker);

	//add the inputted funcs to the array for later garbage collection
	if (vl.checkerFunc)
		MXSGeometryChecker::mxsFuncs->append(vl.checkerFunc);
	if (vl.isSupportedFunc)
		MXSGeometryChecker::mxsFuncs->append(vl.isSupportedFunc);
	if (vl.popupFunc)
		MXSGeometryChecker::mxsFuncs->append(vl.popupFunc);
	if (vl.textOverrideFunc)
		MXSGeometryChecker::mxsFuncs->append(vl.textOverrideFunc);
	if (vl.displayOverrideFunc)
		MXSGeometryChecker::mxsFuncs->append(vl.displayOverrideFunc);
...
void IGeometryCheckerManager_Imp::SavePreferences ()
{
	...
	BOOL res = MaxSDK::Util::WritePrivateProfileString(kINI_SECTION_NAME, kINI_SEE_THROUGH, buf, iniFileName);
	DbgAssert(res);
	val = GetAutoUpdate()== true ? 1 : 0;
	_stprintf_s(buf, _countof(buf), _T("%d"),val);
	res = MaxSDK::Util::WritePrivateProfileString(kINI_SECTION_NAME,kINI_AUTO_UPDATE, buf, iniFileName);
	DbgAssert(res);
	val = GetDisplayTextUpTop()== true ? 1 : 0;
	_stprintf_s(buf, _countof(buf), _T("%d"), val);
	res = MaxSDK::Util::WritePrivateProfileString(kINI_SECTION_NAME, kINI_DISPLAY_TEXT_UP_TOP, buf, iniFileName);
	DbgAssert(res);
}
...
void IGeometryCheckerManager_Imp::LoadPreferences ()
{
	...
	if (pathMgr->DoesFileExist(iniFileName)) 
	{
		...
		// The ini file should always be saved and read with the same locale settings
		MaxLocaleHandler localeGuard;
		_stprintf_s(defaultVal, _countof(defaultVal), _T("%d"), ((GetSeeThrough()== true) ? 1 : 0));
		if (MaxSDK::Util::GetPrivateProfileString(kINI_SECTION_NAME, kINI_SEE_THROUGH, defaultVal, buf, _countof(buf), iniFileName))
		{
			int val = _tstoi(buf);
			mSeeThrough = ( (val==1) ? true: false);
		}
		_stprintf_s(defaultVal, _countof(defaultVal), _T("%d"), ((GetAutoUpdate()== true) ? 1 : 0));
		if (MaxSDK::Util::GetPrivateProfileString(kINI_SECTION_NAME,kINI_AUTO_UPDATE, defaultVal, buf, _countof(buf), iniFileName))
		{
			int val = _tstoi(buf);
			mAutoUpdate = ( (val==1) ? true: false);
		}
		_stprintf_s(defaultVal, _countof(defaultVal), _T("%d"), ((GetDisplayTextUpTop()== true) ? 1 : 0));
		if (MaxSDK::Util::GetPrivateProfileString(kINI_SECTION_NAME, kINI_DISPLAY_TEXT_UP_TOP, defaultVal, buf, _countof(buf), iniFileName))
		{
			int val = _tstoi(buf);
