...
{ 
	...
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_MARBLE) : _T("Marble"); }  
...
class MarbleClassDesc:public ClassDesc2 {
	...
	const TCHAR*	ClassName() { return GetString(IDS_DS_MARBLE_CDESC); }
	const TCHAR*	NonLocalizedClassName() { return _T("Marble"); }
	SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
	Class_ID 		ClassID() { return marbleClassID; }
...
static ParamBlockDesc2 marble_param_blk ( marble_params, _T("parameters"),  0, &marbleCD, P_AUTO_CONSTRUCT + P_AUTO_UI, 1, 
	...
	marble_width,		_T("width"),	TYPE_FLOAT,	P_ANIMATABLE,	IDS_DS_MARBWIDTH,
		...
		p_nonLocalizedName, _T("Vein width"),
		p_end,
...
void Marble::Init() {
	...
	col[0] = pblock->GetColor(marble_color1);
	col[1] = pblock->GetColor(marble_color2);
	width = pblock->GetFloat(marble_width);
	RegisterDistanceDefault(_T("Marble Params"), _T("Size"), pblock->GetFloat(marble_size), IN_TO_M(70.0f));
	size = GetDistanceDefault(_T("Marble Params"), _T("Size"));
...
ParamDlg* Marble::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp) {
	...
	IAutoMParamDlg* dlg = marbleCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
void Marble::ReadSXPData(const TCHAR *name, void *sxpdata) {
...
TSTR Marble::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
...
TSTR Marble::SubAnimName(int i, bool localized)
{
	switch (i)
	{
...
RefResult Marble::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
   PartID& partID, RefMessage message, BOOL propagate ) {
	switch (message) {
		case REFMSG_CHANGE:
			ivalid.SetEmpty();
...
RGBA Marble::EvalColor(ShadeContext& sc) {
	...
	if (d<=.0005) return  mapOn[0]&&subTex[0] ? subTex[0]->EvalColor(sc): RGBA(col[0]);
	else if (d>=.9995) return  mapOn[1]&&subTex[1] ? subTex[1]->EvalColor(sc): RGBA(col[1]);
