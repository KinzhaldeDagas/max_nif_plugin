...
//	SpeckleDlg *paramDlg;
	...
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s = localized ?  GetString(IDS_DS_SPECKLE) : _T("Speckle"); }  
...
class SpeckleClassDesc : public ClassDesc2 {
	public:
		...
		const TCHAR*	ClassName() { return GetString(IDS_DS_SPECKLE_CDESC); }
		const TCHAR*	NonLocalizedClassName() { return _T("Speckle"); }
		SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
		Class_ID 		ClassID() { return speckleClassID; }
...
static constexpr int nameIDs[] = { IDS_DS_SIZE, IDS_DS_COL1, IDS_DS_COL2 };

// The names of the parameters in the parameter block
static constexpr const TCHAR* nonlocalizedNames[] = { _T("Size"), _T("Color 1"), _T("Color 2") };
...
INT_PTR SpeckleDlgProc::DlgProc(
...
TSTR Speckle::SubAnimName(int i, bool localized)
{
	switch (i)
	{
...
RefResult Speckle::NotifyRefChanged(const Interval& changeInt, 
	RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate ) {
	switch (message) {
		case REFMSG_CHANGE:
...
//	return speckleDlg;
	...
	IAutoMParamDlg* dlg = speckleCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
// 'i-th' sub-map.
TSTR Speckle::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
