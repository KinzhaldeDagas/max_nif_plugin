...
//	PlanetDlg *paramDlg;
	...
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_PLANET) : _T("Planet"); }  
...
class PlanetClassDesc : public ClassDesc2 {
	public:
		...
		const TCHAR*	ClassName() { return GetString(IDS_DS_PLANET_CDESC); }
		const TCHAR*	NonLocalizedClassName() { return _T("Planet"); }
		SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
		Class_ID 		ClassID() { return planetClassID; }
...
static float noise(float x, float y, float z) {
	...
	iy = (int)my;
	iz = (int)mz;
	fx = (float)fmod(mx,1.0);
	fy = (float)fmod(my,1.0);
	fz = (float)fmod(mz,1.0);
...
static ParamBlockDesc2 planet_param_blk ( planet_params, _T("parameters"),  0, &planetCD, P_AUTO_CONSTRUCT + P_AUTO_UI, 1, 
	...
	planet_size,	_T("continentSize"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_CONTSIZE,
		...
		p_nonLocalizedName, _T("Continent Size"),
		p_end,
	planet_island,	_T("islandFactor"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_ISLAND,
		...
		p_nonLocalizedName, _T("Island Factor"),
		p_end,
	planet_percent,	_T("oceanPercent"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_PERCENT,
		...
		p_nonLocalizedName, _T("Ocean Percent"),
		p_end,
	...
	planet_blend,	_T("blendWaterLand"), TYPE_BOOL,			0,				IDS_PW_BLENDWL,
		p_default,		TRUE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_BLEND,
		p_end,
...
// The nonlocalized names of the parameters in the parameter block
...
TSTR Planet::SubAnimName(int i, bool localized)
{
	switch (i)
	{
...
RefResult Planet::NotifyRefChanged(const Interval& changeInt, 
	RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate ) {
	switch (message) {
		case REFMSG_CHANGE:
...
ParamDlg* Planet::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp) {
	...
	IAutoMParamDlg* dlg = planetCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
void Planet::Update(TimeValue t, Interval& ivalid) {		
	if (!texValidity.InInterval(t)) {
		...
		{
			ParamID pid = pblock->IndextoID(planet_color1 + i);
			pblock->GetValue(pid, t, col[i], texValidity);
			col[i].ClampMinMax();
		}
...
// --- Methods inherited from Tex3D ---
void Planet::ReadSXPData(const TCHAR *name, void *sxpdata) {
