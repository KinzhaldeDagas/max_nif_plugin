...
//	SmokeDlg *paramDlg;
	...
	// This is reference #1 of this class.
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_SMOKE) : _T("Smoke"); }  
...
class SmokeClassDesc : public ClassDesc2 {
	public:
		...
		const TCHAR*	ClassName() { return GetString(IDS_DS_SMOKE_CDESC); }
		const TCHAR*	NonLocalizedClassName() { return _T("Smoke"); }
		SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
		Class_ID 		ClassID() { return smokeClassID; }
...
TSTR Smoke::SubAnimName(int i, bool localized)
{
	switch (i)
	{
...
//	return smokeDlg;	
	...
	IAutoMParamDlg* dlg = smokeCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
// 'i-th' sub-map.
TSTR Smoke::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
