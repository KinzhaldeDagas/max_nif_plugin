...
//	WaterDlg *paramDlg;
	...
	// This is reference #1 of this class.
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_WATER) : _T("Waves"); }  
...
class WaterClassDesc : public ClassDesc2 {
	public:
		...
		const TCHAR*	ClassName() { return GetString(IDS_DS_WATER_CDESC); }
		const TCHAR*	NonLocalizedClassName() { return _T("Waves"); }
		SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
		Class_ID 		ClassID() { return waterClassID; }
...
static ParamBlockDesc2 water_param_blk ( water_params, _T("parameters"),  0, &waterCD, P_AUTO_CONSTRUCT + P_AUTO_UI, 1, 
	...
	water_num,	_T("numWaveSets"),   TYPE_INT,			P_ANIMATABLE,	IDS_DS_NUMWAVSETS,
		...
		p_nonLocalizedName, _T("Num Wave Sets"),
		p_end,
	...
	water_size,	_T("waveRadius"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_WAVERAD,
		...
		p_nonLocalizedName, _T("Wave Radius"),
		p_end,
...
TSTR Water::SubAnimName(int i, bool localized)
{
	switch (i)
	{
...
RefResult Water::NotifyRefChanged(const Interval& changeInt, 
	RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate ) {
	switch (message) {
		case REFMSG_CHANGE:
...
//	return waterDlg;
	...
	IAutoMParamDlg* dlg = waterCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
// 'i-th' sub-map.
TSTR Water::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
