...
//	SplatDlg *paramDlg;
	...
	// This is reference #1 of this class.
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_SPLAT) : _T("Splat"); }  
...
class SplatClassDesc : public ClassDesc2 {
	public:
		...
		const TCHAR*	ClassName() { return GetString(IDS_DS_SPLAT_CDESC); }
		const TCHAR*	NonLocalizedClassName() { return _T("Splat"); }
		SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
		Class_ID 		ClassID() { return splatClassID; }
...
static constexpr int nameIDs[] = { IDS_DS_SIZE, IDS_DS_THRESH, IDS_DS_ITER, IDS_DS_COL1, IDS_DS_COL2 };

// The nonlocalized names of the parameters in the parameter block
static constexpr const TCHAR* nonlocalizedNames[] = { _T("Size"),
                                      _T("Threshold"),
                                      _T("Iterations"),
                                      _T("Color 1"),
                                      _T("Color 2") };
...
TSTR Splat::SubAnimName(int i, bool localized)
{
	switch (i)
	{
...
RefResult Splat::NotifyRefChanged(const Interval& changeInt, 
	RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate ) {
	switch (message) {
		case REFMSG_CHANGE:
...
//	return splatDlg;
	...
	IAutoMParamDlg* dlg = splatCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
// 'i-th' sub-map.
TSTR Splat::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
...
// --- Methods inherited from Tex3D ---
void Splat::ReadSXPData(const TCHAR *name, void *sxpdata) {
