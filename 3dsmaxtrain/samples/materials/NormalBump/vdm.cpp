	...
	public IMtlRender_Compatibility_MtlBase  
{
	...
	const TCHAR*			NonLocalizedClassName() { return _T("Vector Displacement"); }
...
void VDM::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_VDM_CLASS_NAME_IMP) : _T("Vector Displacement");
}
...
TSTR VDM::SubAnimName(int i, bool localized)
{
	switch (i)
	{
...
{
	IAutoMParamDlg* dlg = VDMDesc.CreateParamDlgs(hwMtlEdit, imp, this);
	return dlg;	
}
...
TSTR VDM::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_VDM_MAP_VECTOR) : _T("Vector");
