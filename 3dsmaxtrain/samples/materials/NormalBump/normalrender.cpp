	...
	public IMtlRender_Compatibility_MtlBase  
{
	...
	const TCHAR*			ClassName() { return GetString(IDS_GN_CLASS_NAME); }
	const TCHAR*			NonLocalizedClassName() { return _T("Normal Bump"); }
...
static ParamBlockDesc2 gnormal_param_blk (
	gnormal_params, _T("params"),  0, &GnormalDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF, 
	//rollout
	IDD_GNORMAL_PANEL, IDS_PARAMS, 0, 0, NULL,
	// params
	gn_mult_spin, 		_T("mult_spin"), 		TYPE_FLOAT, 	P_ANIMATABLE, 	IDS_GN_MULT_SPIN, 
		p_default, 		1.0f, 
		p_range, 		-100.0f,1000.0f, 
		p_ui, 			TYPE_SPINNER,		EDITTYPE_FLOAT, IDC_GN_MULT_EDIT,	IDC_GN_MULT_SPIN, 0.1f, 
		p_nonLocalizedName, _T("Multiplier"),
		p_end,
	gn_bmult_spin, 		_T("bump_spin"), 		TYPE_FLOAT, 	P_ANIMATABLE, 	IDS_GN_BMULT_SPIN, 
		p_default, 		1.0f, 
		p_range, 		-100.0f,1000.0f, 
		p_ui, 			TYPE_SPINNER,		EDITTYPE_FLOAT, IDC_GN_BMULT_EDIT,	IDC_GN_BMULT_SPIN, 0.1f, 
		p_nonLocalizedName, _T("Bump Multiplier"),
		p_end,
	gn_map_normal,		_T("normal_map"),		TYPE_TEXMAP,  P_OWNERS_REF,  IDS_GN_MAP_NORMAL,
		p_refno,		1,
		p_subtexno,		0,
		p_ui,				  TYPE_TEXMAPBUTTON,   IDC_GN_NORMAL_MAP,
		p_nonLocalizedName, _T("Normal"),
		p_end,
	gn_map_bump,		_T("bump_map"),		TYPE_TEXMAP,  P_OWNERS_REF,  IDS_GN_MAP_BUMP,
		p_refno,		2,
		p_subtexno,		1,
		p_ui,				  TYPE_TEXMAPBUTTON,   IDC_GN_BUMP_MAP,
		p_nonLocalizedName, _T("Additional Bump"),
		p_end,
	gn_map1on,		_T("map1on"),		TYPE_BOOL,	0,	IDS_GN_MAP1ON,
		p_default,			TRUE,
		p_ui,				TYPE_SINGLECHECKBOX,		IDC_GN_MAP1ON,
		p_end,
	gn_map2on,		_T("map2on"),		TYPE_BOOL,	0,	IDS_GN_MAP2ON,
		p_default,			TRUE,
		p_ui,				TYPE_SINGLECHECKBOX,		IDC_GN_MAP2ON,
		p_end,
	gn_method,		_T("method"),		TYPE_INT,	0,		IDS_GN_METHOD_RADIO,
		p_default,         0,
		p_ui, 			TYPE_RADIO, 4, IDC_GN_UV_RAD, IDC_GN_XYZ_RAD, IDC_GN_SCREEN_RAD, IDC_GN_WORLD_RAD,
		p_vals,			0,1,2,3,
		p_end,
	gn_flip_red,	_T("flipred"),		TYPE_BOOL,	0,	IDS_GN_FLIP_RED,
		p_default,			false,
		p_ui,				TYPE_SINGLECHECKBOX,		IDC_GN_FLIP_RED,
		p_end,
	gn_flip_green,	_T("flipgreen"),	TYPE_BOOL,	0,	IDS_GN_FLIP_GREEN,
		p_default,			false,
		p_ui,				TYPE_SINGLECHECKBOX,		IDC_GN_FLIP_GREEN,
		p_end,
	gn_swap_rg,		_T("swap_rg"),		TYPE_BOOL,	0,	IDS_GN_SWAP_RG,
		p_default,			false,
		p_ui,				TYPE_SINGLECHECKBOX,		IDC_GN_SWAP_RG,
		p_end,
	p_end
	);
...
void Gnormal::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_GN_CLASS_NAME_IMP) : _T("Normal Bump");
}
...
TSTR Gnormal::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_PARAMS) : _T("Parameters");
...
RefResult Gnormal::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
...
{
	IAutoMParamDlg* dlg = GnormalDesc.CreateParamDlgs(hwMtlEdit, imp, this);
	return dlg;	
}
...
TSTR Gnormal::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_GN_MAP_NORMAL) : _T("Normal");
	case 1: return localized ? GetString(IDS_GN_MAP_BUMP) : _T("Additional Bump");
   ...
   if( sc.backFace ) frontNormal = -frontNormal;
	...
	case TANGENTUV_SPACE: {
		...
		if ((static_cast<MaxSDK::Graphics::INormalMappingManager*>(GetCOREInterface(NORMALMAPPINGMGR_INTERFACE))->GetTangentBasisMode()) == MaxSDK::Graphics::kMikkT) {
			p = ((p.x*tangentBasis.uBasis) + (p.y*tangentBasis.vBasis) + (p.z*tangentBasis.nBasis)).Normalize();
		}
		else {
