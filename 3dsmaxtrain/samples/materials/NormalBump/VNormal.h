...
class VNormalChannel {
	public:
		...
		void				InitTangentBasis( Mesh& mesh, Matrix3& tm, int mapChannel, BOOL useMikkTangent );
		BOOL				ValidTangentBasis();
		TangentBasis&		GetTangentBasis( int faceIndex, int vertIndex, int faceVertIndex, BOOL useMikkTangent);

		void				InitNormals( Mesh& mesh, Matrix3& tm, BOOL useMikkTangent );
		BOOL				ValidNormals();
		Point3&				GetNormal( int faceIndex, int vertIndex, int faceVertIndex, BOOL useMikkTangent);

		int					GetMapChannel() {return mapChannel;}

		const float& GetTangentSign(int faceIndex, int vertIndex, int faceVertIndex);

private:
		void InitMikkTangentBasis(Mesh& mesh, Matrix3& tm, int mapChannel);
		TangentBasis& GetMikkTangentBasis(int faceIndex, int vertIndex, int faceVertIndex);
		void InitMikktNormals(Mesh& mesh, Matrix3& tm);
		Point3& GetMikktNormal(int faceIndex, int vertIndex, int faceVertIndex);
