...
class Matte : public Mtl, public IReshading, public MaxSDK::Graphics::IParameterTranslator, private MaxSDK::NotificationAPI::INotificationCallback
{  
   public:
      ...
      void GetClassName(MSTR& s, bool localized) const override {s= localized ?GetString(IDS_DS_MATTE_SHADOW) : _T("Matte/Shadow");}  
...
//       NotifyChanged();
      ...
      TSTR GetSubTexmapSlotName(int i, bool localized) override {
         return localized ? GetString(IDS_DS_MAP) : _T("Map"); 
         }
      ...
      // From ref
      int NumRefs() {return GetSavingVersion()==2000?1:3;}
...
// end - ke/mjm - 03.16.00 - merge reshading code
	  ...
	  // From IParameterTranslate
	  bool GetParameterValue(const TimeValue t, const MCHAR* shaderParamName, void* value, ShaderParameterType type) override;
	  bool GetShaderInputParameterName(SubMtlBaseType type, int subMtlBaseIndex, MSTR& shaderInputParamName) override;
	  bool OnPreParameterTranslate() override;

	  // Notification API hooks
	  MaxSDK::NotificationAPI::IImmediateNotificationClient* m_notification_client = nullptr;
...
class MatteClassDesc:public ClassDesc2 {
   ...
   const TCHAR*  ClassName() {return GetString(IDS_DS_MATTE_SHADOW_CDESC); }
   const TCHAR*  NonLocalizedClassName() { return _T("Matte/Shadow"); }
   SClass_ID      SuperClassID() {return MATERIAL_CLASS_ID;}
   Class_ID       ClassID() {return matteClassID;}
...
static ParamBlockDesc2 matte_param_blk ( matte_params, _T("parameters"),  0, &matteCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PB_REF, 
   ...
   matte_reflection_amount,   _T("amount"), TYPE_PCNT_FRAC, P_ANIMATABLE,  IDS_DS_REFLAMT,
      ...
      p_nonLocalizedName, _T("Reflection Amount"),
      p_end,  
...
Matte::Matte(BOOL loading) : mReshadeRQ(RR_None), // mjm - 06.02.00
   m_notification_client(MaxSDK::NotificationAPI::INotificationManager::GetManager()->RegisterNewImmediateClient())
   {  
// dlg = NULL;
   pblock = NULL;
...
// InRegisterNotification(NotifyPostSaveOld, (void *)this, NOTIFY_FILE_POST_SAVE_OLD);
	...
	if (mpShaderManager)
	{
		MaxSDK::Graphics::IShaderManagerCreator::GetInstance()->DeleteShaderManager(mpShaderManager);
		mpShaderManager = nullptr;
	}

	if (m_notification_client != nullptr)
	{
		MaxSDK::NotificationAPI::INotificationManager::GetManager()->RemoveClient(m_notification_client);
	}
   }
...
BaseInterface* Matte::GetInterface(Interface_ID id) {
	if (id == ISHADER_MANAGER_INTERFACE_ID)
		return GetShaderManager();
	if (id == IPARAMETER_TRANSLATOR_INTERFACE_ID)
		return static_cast<MaxSDK::Graphics::IParameterTranslator*>(this);

	return Mtl::GetInterface(id);
}

MaxSDK::Graphics::IShaderManager* Matte::GetShaderManager()
{
	if (!mpShaderManager)
	{
		mpShaderManager = MaxSDK::Graphics::IShaderManagerCreator::GetInstance()->CreateShaderManager(
			MaxSDK::Graphics::IShaderManager::ShaderTypeAMG, _M("matte_shadow"), _M(""), this);
	}
...
ParamDlg* Matte::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp)
   ...
   IAutoMParamDlg* dlg = matteCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
static void Clamp(float& val) { val = std::clamp(val, 0.f, 1.f); }
...
static void LBound( Color& c, float min = 0.0f )
{
   if( c.r < min ) c.r = min;
   if( c.g < min ) c.g = min;
   if( c.b < min ) c.b = min;
   ...
   if ( SHADECONTEXT_IS_PREPASS( sc ) )
   {
      ...
      for ( int i = 0;  i < sc.nLights;  i++ )
      {
         ...
         { 
            ...
            l->Illuminate( sc, normal, lightCol, L, NL, diffCoef );
         }
      }
      return;
   }
...
void Matte::Update(TimeValue t, Interval& valid)
   ...
   BOOL bActive = GetCOREInterface()->GetUseEnvironmentMap();
   ReferenceTarget* pSubTexmap = bActive?GetCOREInterface()->GetEnvironmentMap():nullptr;

   // Different environment?
   if (envmap != pSubTexmap)
   {
		ReplaceReference(ENVMAP_REF, pSubTexmap);
		NotifyChanged();
   }
...
TSTR Matte::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_DS_PARAMETERS) : _T("Parameters");
	case 1: return localized ? GetString(IDS_DS_MAP) : _T("Map");
	default: return _T("");
	}
}
...
void Matte::SetReference(int i, RefTargetHandle rtarg)
   ...
   switch (i) {
	  ...
	  case 1: reflmap = (Texmap*)rtarg;  break;
	  case 2: envmap = (Texmap*)rtarg;  break;
...
RefTargetHandle Matte::Clone(RemapDir &remap)
   ...
   *((MtlBase*)mtl) = *((MtlBase*)this);  // copy superclass stuff
   ...
   if (envmap)
	   mtl->ReplaceReference(ENVMAP_REF, remap.CloneRef(envmap));
...
bool Matte::GetParameterValue(const TimeValue t, const MCHAR* shaderParamName, void* value, ShaderParameterType type)
{
	if (_tcscmp(shaderParamName, _M("shadow_color")) == 0)
	{
		Color col = pblock->GetColor(matte_color, t);
		*((Point4*)value) = Point4(col.r, col.g, col.b, 1.0f);
		return true;
	}
	if (_tcscmp(shaderParamName, _M("shadow_brightness")) == 0)
	{
		*((float*)value) = pblock->GetInt(matte_receive_shadows, t) ? 1.0f - pblock->GetFloat(matte_shadow_brightness, t) : 0.0;
		return true;
	}
	if (_tcscmp(shaderParamName, _M("background")) == 0)
	{		
		Interval valid;
		Color col = GetCOREInterface()->GetBackGround(t, valid);
		*((Point4*)value) = Point4(col.r, col.g, col.b, 1.0f);
		return true;
	}
...
bool Matte::GetShaderInputParameterName(SubMtlBaseType type, int subMtlBaseIndex, MSTR& shaderInputParamName)
{
	return false;
}

bool Matte::OnPreParameterTranslate()
{
	return false;
}
