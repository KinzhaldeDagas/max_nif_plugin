...
//		float	filterSz;
	...
	Color GetTranslucentColor(TimeValue t);
		...
		BOOL MapActive(int i) const { return (i >= 0 && i < MAX_TEXTURE_CHANNELS) ? (*maps)[i].IsActive() : FALSE; }
		...
		void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_KE_STANDARD2) : _T("Standard (Legacy)"); }  
