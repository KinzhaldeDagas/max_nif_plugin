...
namespace Blending {
   ...
   struct Overlay : public BlendMode< Overlay > {
      enum { NameResourceID = IDS_BLENDMODE_OVERLAY };
		float operator()( const float fg, const float bg ) const {
			return std::clamp( bg <= 0.5f ? 2.0f * fg * bg
   ...
   struct SoftLight : public BlendMode< SoftLight > {
      enum { NameResourceID = IDS_BLENDMODE_SOFTLIGHT };
		float operator()(const float a, const float b) const {
			return std::clamp( a <= 0.5f ? b * (b + 2.0f*a*(1.0f - b))
   ...
   struct HardLight : public BlendMode< HardLight > {
      enum { NameResourceID = IDS_BLENDMODE_HARDLIGHT };
		float operator()(const float a, const float b) const {
			return std::clamp( a <= 0.5f ? 2.0f*a*b
...
#pragma warning( pop )
	...
	{
		return std::clamp<float>( m_Opacity, Default::OpacityMin, Default::OpacityMax );
	}
	...
	Layer::NameType Layer::ProcessName( bool localized ) const
	{
...
namespace Composite {
	...
	void LayerDialog::initCombo()
	{
		...
		HWND ctrl = GetDlgItem(m_hWnd, IDC_COMP_BLEND_TYPE);
		for (const Blending::BaseBlendMode* mode : Blending::ModeList)
		{
...
namespace Composite {
	namespace {
      ...
      BlockParam::Visible,    _T("mapEnabled"),
         ...
         p_accessor,         &Accessor,
         p_nonLocalizedName, _T("Map 1 Enable"),
      p_end,
      BlockParam::Mask_DEPRECATED,       _T(""),
         ...
         p_accessor,         &Accessor,
         p_refno,             1,
         p_nonLocalizedName, _T("Map Mask"),
      p_end,
      BlockParam::VisibleMask,_T("maskEnabled"),
         ...
         p_accessor,         &Accessor,
         p_nonLocalizedName, _T("Map 2 Enable"),
      p_end,
	  ...
	  BlockParam::Opacity,    _T("opacity"),
         ...
         p_range,             Default::OpacityMin,
							  Default::OpacityMax,
         p_accessor,          &Accessor,
         p_nonLocalizedName, _T("Map Opacity"),
	  p_end,
...
TSTR Texture::SubAnimName(int i, bool localized)
{
...
const TCHAR*   Composite::Desc::NonLocalizedClassName() { return _T("Composite"); }
...
void Composite::Texture::AddLayer() {
	...
	if (nb_layer() >= Param::LayerMax) {
        TSTR msgBoxText = GetString(IDS_ERROR_MAXLAYER);
		MaxSDK::MaxMessageBox(0, msgBoxText, GetString(IDS_ERROR_TITLE), MB_APPLMODAL | MB_OK | MB_ICONERROR);
		return;
	}
...
void Composite::Texture::DeleteLayer( size_t Index ) {
	...
	// we do not delete it.
	if (nb_layer() == 1) {
		TSTR msgBoxText = GetString(IDS_ERROR_MINLAYER);
		MaxSDK::MaxMessageBox(0, msgBoxText, GetString(IDS_ERROR_TITLE), MB_APPLMODAL | MB_OK | MB_ICONERROR );
		return;
	  ...
	  accumulator( ShadeContext& shade ) : sc(shade) { }
		...
		AColor operator()( const AColor& bg, const value_type& Layer ) {
		 ...
		 const float alpha  ( fg.a + (1 - fg.a) * bg.a );
			...
			// We composite.  This is an "over" mode of composition, same as
			// photoshop uses.
...
{
	// This is an "over" mode of composition, same as photoshop uses.
	// On blending mode: http://www.pegtop.net/delphi/articles/blendmodes/index.htm
...
RefTargetHandle Composite::Texture::Clone(RemapDir &remap) {
	...
	*((MtlBase*)NewMtl) = *((MtlBase*)this);  // copy superclass stuff
	...
	NewMtl->m_ViewportTextureList.clear();
...
TSTR Composite::Texture::GetSubTexmapSlotName(int i, bool localized)
{
	if (m_FileVersion == kMax2009)
	{
		...
		DbgAssert(layer != end());
		if (is_subtexmap_idx_tex(i))
			return layer->ProcessName(localized);
		else
			return TSTR(layer->ProcessName(localized)) + _T(" (Mask)");
	}
	else
	{
		DbgAssert(0);
												...
												BOOL            propagate )
{
	...
	case REFMSG_GET_PARAM_NAME_LOCALIZED: {
			...
			gpn->name = GetSubTexmapSlotName(gpn->index, true);
			return REF_HALT;
		}

	case REFMSG_GET_PARAM_NAME_NONLOCALIZED: {
			GetParamName *gpn = (GetParamName*)partID;
			gpn->name = GetSubTexmapSlotName(gpn->index, false);
			return REF_HALT;
		}
...
void Composite::Texture::InitMultimap( TimeValue  time,
	...
	const int nb_tex = std::min<int>({ int(CountVisibleLayer()), Param::LayerMaxShown, callback.NumberTexturesSupported() });

	iterator it( begin() );
	for( int i = 0; i < nb_tex && it != end(); ++it ) {
		if (it->IsVisible()) {
			PBITMAPINFO bmi = NULL;
			Interval valid = GetValidity();
			if (CreateViewportDisplayBitmap(bmi, time, callback, valid, it, false, 0, 0) && bmi)
			{
