...
#pragma region Layer Data
	...
	public:
		...
		void UpdateCache( ) const {
			...
			m_Layer->Update(m_Layer->CurrentTime(), valid);
		}
...
namespace Composite {
	...
	public:
		...
		NameType ProcessName( bool localized ) const;
...
namespace Composite {
	...
	public:
		...
		// MultiTex virtuals
		void           SetNumSubTexmaps(int n) override       { SetNumMaps(n); }
		...
		int            NumSubTexmaps() override { return int(nb_texture()); }
		...
		void        GetClassName(MSTR& s, bool localized) const override { s = localized ? GetCompositeDesc()->ClassName() : GetCompositeDesc()->NonLocalizedClassName(); }
...
namespace Composite {
	...
	public:
		...
		const _tstring  Title()                  const {
			if (m_Layer) return m_Layer->ProcessName(true);
