...
//       return pblock->GetFloat(PB_BLUR,t); 
      ...
      void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_FLATMIRROR) : _T("Flat Mirror"); }  
...
class MirrorClassDesc:public ClassDesc2 {
   ...
   const TCHAR*  ClassName() { return GetString(IDS_DS_FLATMIRROR_CDESC); }
   const TCHAR*	 NonLocalizedClassName() { return _T("Flat Mirror"); }
   SClass_ID      SuperClassID() { return TEXMAP_CLASS_ID; }
   Class_ID       ClassID() { return mirrorClassID; }
...
static ParamBlockDesc2 fmirror_param_blk ( fmirror_params, _T("parameters"),  0, &mirrorCD, P_AUTO_CONSTRUCT + P_AUTO_UI, 0, 
   ...
   fmirror_applytofaceid,  _T("applyToFaceID"), TYPE_BOOL,        0,          IDS_PW_APPLYTOFACEID,
      p_default,     FALSE,
      p_ui,       TYPE_SINGLECHECKBOX, IDC_MIR_APPLY_MTLID,
      p_end,
...
// paramDlg = dm;
// return dm;  
   IAutoMParamDlg* dlg = mirrorCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
TSTR Mirror::SubAnimName(int i, bool localized)
{
	switch (i)
	{
...
static int nameID[] = { IDS_DS_BLUR,
                        IDS_DS_DISTAMT,
                        IDS_DS_NSLEV,
                        IDS_DS_NSSIZ,
                        IDS_DS_NSPHS };

static constexpr const TCHAR* nonlocalizedName[] = { _T("Blur"),
                                     _T("Distortion Amount"),
                                     _T("Noise Levels"),
                                     _T("Noise Size"),
                                     _T("Noise Phase") };

RefResult Mirror::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
   PartID& partID, RefMessage message, BOOL propagate ) {
   switch (message) {
      case REFMSG_CHANGE:
         if (hTarget == pblock)
...
// camPlane = TransformPlane(vp.affineTM, plane);
   ...
   MaxSDK::MaxMessageBox(NULL, GetName(), _T(" Mirror Test"), MB_OK | MB_ICONEXCLAMATION);
#endif
...
RGBA Mirror::EvalColor(ShadeContext& sc) {
   ...
   if (mir) {
      ...
      int w = mir->bm->Width(); 
      int h = mir->bm->Height();
      float nsx(0.0f), nsy(0.0f), nsblur(0.0f);
      switch (distortType) {
         case DISTORT_NOISE: {
            Point3 p = sc.PObj();
      ...
      if (applyBlur) {
         ...
         FILE *f = sc.DebugFile();
         if (f) {
            _ftprintf(f,_T("w=%d, h=%d org = (%d, %d) "), w, mir->bm->Height(),mir->org.x, mir->org.y);
            _ftprintf(f, _T("s = (%d, %d), c = (%x, %x, %x, %x)\n"),
