...
{ 
   ...
   public:
      ...
      void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_MIX) : _T("Mix"); }  
...
class MixClassDesc:public ClassDesc2 {
   ...
   const TCHAR*  ClassName() { return GetString(IDS_DS_MIX_CDESC); }
   const TCHAR*  NonLocalizedClassName() { return _T("Mix"); }
   SClass_ID      SuperClassID() { return TEXMAP_CLASS_ID; }
   Class_ID       ClassID() { return mixClassID; }
...
//       em->EnableAffectRegion (t);
      ...
      case CC_SPINNER_CHANGE:
      {
         ...
         mix->Update(GetCOREInterface()->GetTime(), valid);
         GetClientRectP(GetDlgItem(hWnd,IDC_MIXCURVE),&rect);
...
void Mix::Init() 
{
	...
	ivalid.SetEmpty();
	mapValid.SetEmpty();
	col[0] = pblock->GetColor(mix_color1);
	col[1] = pblock->GetColor(mix_color2);
	mix = pblock->GetFloat(mix_mix);
	crvA = pblock->GetFloat(mix_curvea);
	crvB = pblock->GetFloat(mix_curveb);
...
AColor Mix::EvalColor(ShadeContext& sc) {
   ...
   if (m<.0001f)
      c = texout->Filter(sub[0] ? sub[0]->EvalColor(sc): AColor(col[0]));
   else if (m>0.9999f)   
      c = texout->Filter(sub[1] ? sub[1]->EvalColor(sc): AColor(col[1]));
   else {
      AColor c0  = sub[0] ? sub[0]->EvalColor(sc): AColor(col[0]);
      AColor c1  = sub[1] ? sub[1]->EvalColor(sc): AColor(col[1]);
...
// Gradient main AutoMParamDlg so it will keep them up-to-date automatically
   ...
   IAutoMParamDlg* dlg = mixCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
TSTR Mix::SubAnimName(int i, bool localized)
{
	switch (i)
	{
