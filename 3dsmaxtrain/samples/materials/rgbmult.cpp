...
class RGBMult: public Texmap { 
	...
	BOOL loadingOld;
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_RGBMULT) : _T("RGB Multiply"); }  
...
class RGBMultClassDesc:public ClassDesc2 {
	...
	const TCHAR*	ClassName() { return GetString(IDS_DS_RGBMULT_CDESC); }
	const TCHAR*	NonLocalizedClassName() { return _T("RGB Multiply"); }
	SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
	Class_ID 		ClassID() { return rgbmultClassID; }
...
AColor RGBMult::EvalColor(ShadeContext& sc) {
	...
	AColor c0 = (subTex[0]&&mapOn[0])? subTex[0]->EvalColor(sc): AColor(col[0]);
	AColor c1 = (subTex[1]&&mapOn[1])? subTex[1]->EvalColor(sc): AColor(col[1]);
	AColor c;
	c.r = c0.r*c1.r;
	c.g = c0.g*c1.g;
	c.b = c0.b*c1.b;
...
ParamDlg* RGBMult::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp) {
	IAutoMParamDlg* dlg = maskCD.CreateParamDlgs(hwMtlEdit, imp, this);
	return dlg;
...
{
	col[i] = c;
	ParamID id = i==0?rgbmult_color1:rgbmult_color2;
...
TSTR RGBMult::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
...
TSTR RGBMult::SubAnimName(int i, bool localized)
{
	switch (i)
	{
...
RefResult RGBMult::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
   PartID& partID, RefMessage message, BOOL propagate ) {
	switch (message) {
		case REFMSG_CHANGE:
			ivalid.SetEmpty();
