...
class CompositeMat : public Mtl, public IReshading  {	
	public:
		...
		Class_ID ClassID() {return COMPOSITE_MATERIAL_CLASS_ID; }
		SClass_ID SuperClassID() {return MATERIAL_CLASS_ID;}
		void GetClassName(MSTR& s, bool localized) const override {s= localized ?GetString(IDS_PW_COMPOSITE_MATERIAL) : _T("Composite");}  
...
class CompositeMatClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Composite"); }
	SClass_ID		SuperClassID() {return MATERIAL_CLASS_ID;}
	Class_ID 		ClassID() {return COMPOSITE_MATERIAL_CLASS_ID;}
...
TSTR CompositeMatPBAccessor::GetLocalName(ReferenceMaker* owner, ParamID id, int tabIndex, bool localized)
	{
	CompositeMat* p = (CompositeMat*)owner;
	TSTR out;
	switch (id)
		...
		case compmat_amount:
			...
			if (localized)
				...
				out.printf(_T("%s %d"), _T("Amount"), tabIndex + 1);
			break;
			}
		case compmat_mtls:
			{
			...
			name = sm1->GetFullName(localized);
			...
			if (tabIndex == 0)
				...
				if (localized)
					...
					out.printf(_T("%s: %s"), _T("Base"), name);
				}
			else
				{
				if (localized)
					...
					out.printf(_T("%s %d: %s"), _T("Material"), tabIndex, name);
				}
			break;
			}
		}
...
static ParamBlockDesc2 compmat_param_blk ( compmat_params, _T("parameters"),  0, &compositematCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PB_REF, 
	...
	compmat_map_on,	_T("mapEnables"), TYPE_BOOL_TAB, MAX_NUM_MTLS-1, 0, IDS_MAPENABLE,
		p_default,		TRUE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_MAPON1, IDC_MAPON2, IDC_MAPON3, IDC_MAPON4, IDC_MAPON5,
...
INT_PTR CompMatDlgProc::DlgProc(
...
//		pblock2->SetValue(compmat_mtls,0,m,i);
        ...
        Mtl* mtl = (Mtl*)NewDefaultMaterial(_T("Composite"));
		hs.Resume();
...
ParamDlg* CompositeMat::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp)
	{
	mainDlg = compositematCD.CreateParamDlgs(hwMtlEdit, imp, this);
	compmat_param_blk.SetUserDlgProc(new CompMatDlgProc(this));
//	dlg = new CompositeMatDlg(hwMtlEdit, imp, this);
	return mainDlg;
	}

TSTR CompositeMat::GetSubMtlSlotName(int i, bool localized) {
	TSTR name;
	if (i == 0)
		name.printf(_T("%s"), localized ? GetString(IDS_PW_BASE) : _T("Base"));
	else
		name.printf(_T("%s %d"), localized ? GetString(IDS_PW_MAT) : _T("Mat."), i);
	return name;
	}
...
void CompositeMat::Update(TimeValue t, Interval& valid)
	...
	if (!ivalid.InInterval(t))
	{
		...
		for (int i = 0; i < MAX_NUM_MTLS; i++)
		{
			Mtl* sub = NULL;
			float u;
			pblock2->GetValue(compmat_mtls,t,sub,ivalid,i);
			if (sub != NULL)
			{
				//use valid not ivalid because we don't want sub material dirty our interval state
				sub->Update(t,valid);
				if (i != MAX_NUM_MTLS-1)
					pblock2->GetValue(compmat_amount,t,u,ivalid,i);
			}
		}
...
TSTR CompositeMat::SubAnimName(int i, bool localized)
{
	switch (i)
	{
