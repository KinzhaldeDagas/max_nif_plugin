...
// this class is needed for loading max r1 files.
class OldTexmapsClassDesc:public ClassDesc2 {
public:
	int 			IsPublic() { return 0; }
	void*			Create(BOOL loading) { 	return new Texmaps(NULL); }
	const TCHAR*	ClassName() { return NULL; }
	const TCHAR*	NonLocalizedClassName() { return NULL; }
...
class TexmapsClassDesc:public ClassDesc2 {
	public:
	int 			IsPublic() { return 0; }
	void*			Create(BOOL loading) { 	return new Texmaps(NULL); }
	const MCHAR*	ClassName() { return GetString(IDS_DS_CLASSTEXMAPS); }
	const MCHAR*	NonLocalizedClassName() { return _M("Texmaps"); }
...
{
	if (i & 1)
		return client ? client->GetSubTexmapTVName(i / 2, localized) : TSTR(_T(""));
	else
	{
		TSTR nm;
...
ReferenceTarget* Texmaps::GetLastNotifyTarget()
{
	return mLastNotifyTarget;
}
