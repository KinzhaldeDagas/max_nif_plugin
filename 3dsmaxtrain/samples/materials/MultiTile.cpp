...
TSTR Texture::GetSubTexmapSlotName(int i, bool localized)
{
	if (m_CompositeTexture && (i >= 0) && (i < NumSubTexmaps()))
	{
		_tostringstream oss;
	...
	if ( m_CompositeTexture )
	{
		if ( m_CompositeTexture->nb_layer() >= Composite::Param::LayerMax ) {
			if ( !GetCOREInterface()->GetQuietMode() ) {
				TSTR msgBoxText = GetString(IDS_ERROR_MAXTILE);
...
void Texture::DeleteTile( size_t Index/* = 0*/ )
{
	if ( m_CompositeTexture )
	{
		if ( m_CompositeTexture->nb_layer() == 1) {
			if ( !GetCOREInterface()->GetQuietMode() ) {
				TSTR msgBoxText = GetString(IDS_ERROR_MINTILE);
				MaxSDK::MaxMessageBox(0, msgBoxText, GetString(IDS_ERROR_TITLE), MB_APPLMODAL | MB_OK | MB_ICONERROR);
			}
			return;
		}
			...
			if ( !GetCOREInterface()->GetQuietMode() ) {
				TSTR msgBoxText = GetString(IDS_ERROR_TILE_INDEX);
				MaxSDK::MaxMessageBox(0, msgBoxText, GetString(IDS_ERROR_TITLE), MB_APPLMODAL | MB_OK | MB_ICONERROR);
			}
			return;
		}
...
const MCHAR* Texture::GetPatternedFilePrefix() const
{
	...
	{
		return m_PatternedFilePrefix.c_str();
	}
	return nullptr;
}
...
void Texture::GetClassName(MSTR& s, bool localized) const
{ 
	s = localized ? GetMultiTileDesc()->ClassName() : GetMultiTileDesc()->NonLocalizedClassName();
}
...
IOResult Texture::Load( ILoad *iLoad )
{
	...
	while( IO_OK == (res = iLoad->OpenChunk()) ) 
	{
		...
		switch( chunk_id ) 
		{
		case static_cast<int>(ChunkID::Header):
			{
				res = MtlBase::Load( iLoad );
				DbgAssert( IO_OK == res );
			}
			break;
		case static_cast<int>(ChunkID::PatternFormat):
			{
				ULONG nb;
			...
			break;
		case static_cast<int>(ChunkID::PatternPrefix):
			{
				MCHAR *buffer;
			...
			break;
		case static_cast<int>(ChunkID::ViewportQuality): {
			{
				ULONG nb;
...
const TCHAR* Desc::NonLocalizedClassName() { return _T("MultiTile"); }
