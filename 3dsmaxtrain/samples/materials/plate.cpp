...
class Plate: public Texmap { 
	...
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_FLATPLATE) : _T("Thin Wall Refraction"); }  
...
class PlateClassDesc:public ClassDesc2 {
	...
	const TCHAR*	ClassName() { return GetString(IDS_DS_FLATPLATE_CDESC); }
	const TCHAR*	NonLocalizedClassName() { return _T("Thin Wall Refraction"); }
	SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
	Class_ID 		ClassID() { return plateClassID; }
...
static ParamBlockDesc2 plate_param_blk ( plate_params, _T("parameters"),  0, &plateCD, P_AUTO_CONSTRUCT + P_AUTO_UI, 0, 
	...
	plate_thick,	_T("thicknessOffset"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_THICKFACT,
		...
		p_nonLocalizedName, _T("Thickness Offset"),
		p_end,
	plate_reframt,	_T("bumpMapEffect"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_REFRAMT,
		...
		p_nonLocalizedName, _T("Bump Map Effect"),
		p_end,
...
ParamDlg* Plate::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp) {
	IAutoMParamDlg* dlg = plateCD.CreateParamDlgs(hwMtlEdit, imp, this);
	return dlg;
	}
...
TSTR Plate::SubAnimName(int i, bool localized)
{
	switch (i)
	{
...
RefResult Plate::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
   PartID& partID, RefMessage message, BOOL propagate ) {
	switch (message) {
		case REFMSG_CHANGE:
			ivalid.SetEmpty();
...
*/
	...
	if (devw>200){
		...
		MaxSDK::MaxMessageBox(NULL, _T("hi"), _T(" Plate Test"), MB_OK | MB_ICONEXCLAMATION);
		}
#endif
