...
class Output: public Texmap { 
	...
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_OUTPUT) : _T("Output"); }  
...
class OutputClassDesc:public ClassDesc2 {
	...
	const TCHAR*	ClassName() { return GetString(IDS_DS_OUTPUT_CDESC); }
	const TCHAR*	NonLocalizedClassName() { return _T("Output"); }
	SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
	Class_ID 		ClassID() { return outputClassID; }
...
{
	if (i==0) 
		subTex[0] = (Texmap *)rtarg; 
	else if (i==1) 
		texout = (TextureOutput *)rtarg;
	else 
...
TSTR Output::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_DS_MAP) : _T("Map");
	default: DbgAssert(0); return _T("");
	}
}
...
TSTR Output::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return GetSubTexmapTVName(0, localized);
	case 1: return localized ? GetString(IDS_DS_OUTPUT) : _T("Output");
	case 2: return localized ? GetString(IDS_DS_PARAMETERS) : _T("Parameters");
	default: DbgAssert(0); return _T("");
	}
}

RefResult Output::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
								   PartID& partID, RefMessage message, BOOL propagate ) 
{
	switch (message) {
		case REFMSG_CHANGE:
