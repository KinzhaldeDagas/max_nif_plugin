...
{
   SmartPtr()
      : SmartResource<Type*, Free>()
   {}
   explicit SmartPtr( Type* ptr )
      : SmartResource<Type*, Free>( ptr )
   {}
   SmartPtr( const SmartPtr& src ) 
      : SmartResource<Type*, Free>( src )
   {}

   // Need to redefine those here.
   SmartPtr& operator=( Type* ptr )                { return Assign(ptr); }
   SmartPtr& operator=( const SmartResource<Type*, Free>& ptr ) { return Assign(ptr); }

   SmartPtr& Assign( Type* ptr )
      { SmartPtr NewValue( ptr ); this->swap( NewValue ); return *this; }
   SmartPtr& Assign( const SmartResource<Type*, Free>& ptr )
      { SmartPtr NewValue( ptr ); this->swap( NewValue ); return *this; }
   ...
   bool  IsNull() { return SmartResource<Type*, Free>::IsNull() || this->get() == NULL; }
};
...
{
   SmartInterface()
      : SmartPtr<Type, DeleteIface>()
   {}
   explicit SmartInterface( Type* ptr )
      : SmartPtr<Type, DeleteIface>( ptr )
   {}
   SmartInterface( const SmartPtr<Type, DeleteIface>& src )
      : SmartPtr<Type, DeleteIface>( src )
   {}
...
namespace
{
	void DeleteHBITMAP( HBITMAP ptr )
	{
		DeleteObject( ptr );
	}
}
...
// used during set up maps for viewport display.
