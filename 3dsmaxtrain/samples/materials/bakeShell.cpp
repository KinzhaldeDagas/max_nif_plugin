...
class BakeShell : public Mtl, public IReshading  {	
	...
	// Methods to access sub-materials of meta-materials
		...
		TSTR GetSubMtlSlotName(int i, bool localized) override
		{
			if (localized)
			{
				return i ? GetString(IDS_KE_BAKED_MTL) : GetString(IDS_KE_ORIG_MTL);
			}
			else
			{
				return i ? _T("Baked Material") : _T("Original Material");
			}
		}
		...
		TSTR GetSubTexmapSlotName(int i, bool localized) override { return _T( "" ); }
...
class BakeShellClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Shell Material"); }
	SClass_ID		SuperClassID() {return MATERIAL_CLASS_ID;}
	Class_ID 		ClassID() {return bakeShellClassID;}
...
INT_PTR BakeShellDlgProc::DlgProc(
...
void BakeShell::Init()
{
	...
	ReplaceReference( ORIG_REF, NewDefaultMaterial(_T("BakeShell")));

	// this shd be the hw material eventually
	ReplaceReference( BAKED_REF, NewDefaultMaterial(_T("BakeShell")));
...
// up-to-date and deleted as necessary.  
	...
	IAutoMParamDlg* dlg = _bakeShellCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
{
	 int useMtl;
	 mpBlock->GetValue( bakeShell_vp_n_mtl, 0, useMtl, FOREVER );
	 ...
	 mtl = (useMtl == 0 && mpOrigMtl) ? mpOrigMtl : mpBakedMtl;
	if (mtl != NULL)
		req |= mtl->Requirements(subMtlNum);
...
void BakeShell::GetClassName(MSTR& s, bool localized) const
{
	s= localized ?GetString(IDS_KE_BAKE_SHELL_IMP) : _T("Shell Material");
}
...
TSTR BakeShell::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_DS_PARAMETERS) : _T("Parameters");
	case 1: return GetSubMtlTVName(0, localized);
	case 2: return GetSubMtlTVName(1, localized);
	default: return _T("");
	}
}
...
//			if (hTarget==mpBlock) mValidInt.SetEmpty();
		...
		case REFMSG_SUBANIM_STRUCTURE_CHANGED:
			if ((hTarget!=this) && (hTarget!=mpBlock))
				return REF_SUCCEED;
...
void BakeShell::EnumAuxFiles(AssetEnumCallback& nameEnum, DWORD flags) 
{
