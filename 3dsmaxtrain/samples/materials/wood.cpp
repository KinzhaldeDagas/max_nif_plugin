...
static		Class_ID			woodClassID(WOOD_CLASS_ID);
...
static ParamBlockDesc2 wood_param_blk ( wood_params, _T("parameters"),  0, &woodCD, P_AUTO_CONSTRUCT + P_AUTO_UI, 1, 
	...
	wood_thickness,	_T("thickness"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_WOODSIZE,
		...
		p_nonLocalizedName, _T("Grain Thickness"),
		p_end,
	wood_rnoise,	_T("radialNoise"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_WOODR1,
		...
		p_nonLocalizedName, _T("Radial Noise"),
		p_end,
	wood_anoise,	_T("axialNoise"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_WOODR2,
		...
		p_nonLocalizedName, _T("Axial Noise"),
		p_end,
...
ParamDlg* Wood::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp) 
{
	...
	IAutoMParamDlg* dlg = woodCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
void Wood::ReadSXPData(const TCHAR *name, void *sxpdata) 
{
...
TSTR Wood::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
...
TSTR Wood::SubAnimName(int i, bool localized)
{
	switch (i)
	{
...
RefResult Wood::NotifyRefChanged(	const Interval& changeInt, RefTargetHandle hTarget, 
...
//	p *= FACT/size;
	...
	if (d<=.0005f) 
		return  (mapOn[0]&&subTex[0]) ? subTex[0]->EvalColor(sc): RGBA(col[0]);
	else 
		if (d>=.9995) 
			return  (mapOn[1]&&subTex[1]) ? subTex[1]->EvalColor(sc): RGBA(col[1]);
