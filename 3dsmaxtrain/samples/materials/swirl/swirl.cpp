...
class Swirl: public Texmap { 
   ...
   public:
      ...
      void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_SWIRL) : _T("Swirl"); }  
...
class SwirlClassDesc:public ClassDesc2 {
   ...
   const MCHAR*   ClassName() { return GetString(IDS_SWIRL); }
   const MCHAR*   NonLocalizedClassName() { return _M("Swirl"); }
   SClass_ID      SuperClassID() { return TEXMAP_CLASS_ID; }
   Class_ID       ClassID() { return SwirlClassID; }
...
static ParamBlockDesc2 swirl_param_blk (swirl_params, _T("parameters"),  0, &swirlCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	...
	swirl_hs, _M("Swirl_Intensity"), TYPE_FLOAT, P_ANIMATABLE, IDS_SWIRLINTENS,
		...
		p_nonLocalizedName, _T("Swirl Intensity"),
		p_end,
	...
	swirl_cv, _M("Color_Contrast"), TYPE_FLOAT, P_ANIMATABLE, IDS_COLCONTRAST,
		...
		p_nonLocalizedName, _T("Color Contrast"),
		p_end,
	...
	swirl_vg, _M("Center_Position_Y"), TYPE_FLOAT, P_ANIMATABLE, IDS_CENTERY,
		...
		p_nonLocalizedName, _T("Center Position Y"),
		p_end,
	...
	swirl_hg, _M("Center_Position_X"), TYPE_FLOAT, P_ANIMATABLE, IDS_CENTERX,
		...
		p_accessor, &swirl_accessor,
		p_nonLocalizedName, _T("Center Position X"),
		p_end,
	...
	swirl_ls, _M("Swirl_Amount"), TYPE_FLOAT, P_ANIMATABLE, IDS_SWIRLAMT,
		...
		p_nonLocalizedName, _T("Swirl Amount"),
		p_end,
	...
	swirl_h, _M("Constant_Detail"), TYPE_INT, P_ANIMATABLE, IDS_DETAIL, 
		...
		p_nonLocalizedName, _T("Constant Detail"),
		p_end,
	...
	swirl_rs, _M("Random_Seed"), TYPE_FLOAT, P_ANIMATABLE, IDS_RANDOMSEED, 
		...
		p_nonLocalizedName, _T("Random Seed"),
		p_end,
	...
	swirl_map1, _M("swirlMap"), TYPE_TEXMAP, P_OWNERS_REF, IDS_SWIRL_MAP1,
		...
		p_nonLocalizedName, _T("Swirl Map"),
		p_end,
	...
	swirl_map2, _M("baseMap"), TYPE_TEXMAP, P_OWNERS_REF, IDS_SWIRL_MAP2,
		...
		p_nonLocalizedName, _T("Base Map"),
		p_end,
...
ParamDlg* Swirl::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp) {
	...
	IAutoMParamDlg* dlg = swirlCD.CreateParamDlgs(hwMtlEdit, imp, this);
	// add the secondary dialogs to the main one
	dlg->AddDlg(uvGenDlg);
...
TSTR Swirl::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_SWIRL_PARAMNAME) : _T("Swirl");
	case 1: return localized ? GetString(IDS_BASE) : _T("Base");
...
TSTR Swirl::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_COORDS) : _T("Coordinates");
	case 1: return localized ? GetString(IDS_PARAMS) : _T("Parameters");
...
RefResult Swirl::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
   PartID& partID, RefMessage message, BOOL propagate ) {
   switch (message) {
      case REFMSG_CHANGE:
         ivalid.SetEmpty();
...
IOResult Swirl::Save(ISave *isave) { 
	...
	isave->BeginChunk(PARAM2_CHUNK); // Load sets member if chunk is present
	isave->EndChunk();
