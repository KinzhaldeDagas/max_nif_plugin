...
#define MV_CONFIGNAME					_T("SME.ini")
#define MV_SEC_NODEVIEW					_T("Node View Section")
#define MV_KEY_MULTISUB_EMPTY_SLOTS		_T("MultiSub Material - Use Empty Slots")

TSTR GetCfgFilename()
{
	TSTR filename(GetCOREInterface()->GetDir(APP_PLUGCFG_LN_DIR));
	if (!filename.EndsWith(_T('\\')))
	{
		filename.append(_T("\\"));
	}
	filename.append(MV_CONFIGNAME);

	return filename;
}

bool GetMultisubEmptySlots()
{
	...
	MaxSDK::Util::GetPrivateProfileString(MV_SEC_NODEVIEW, MV_KEY_MULTISUB_EMPTY_SLOTS, _T("1"), temp, tempSize, filename);
	return (_ttoi(temp) > 0);
}
...
public:
   ...
   float Convert(float value) override { return value+1; };
   float UnConvert(float value) override { return value-1; };
};

static MultiIDDimension theMultiIDDim;

class MtlIdValidator : public PBValidator
{
    BOOL Validate(PB2Value &v)
    {
        return v.i >= 0 && v.i < 60000; // allow to set more from script than from the UI, but not more than the MtlID max limit 
    }
};

static MtlIdValidator mtlIdValidator;
...
static int findPrimeForSize(int size)
   {
   for (int exponentialPrime : exponentialPrimes)
   {
      if (exponentialPrime > size)
      {
         return exponentialPrime;
      }
   }
...
class MultiDlg: public ParamDlg {
   public:     
      ...
      ~MultiDlg();
      INT_PTR BasicPanelProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam );
      ...
      Class_ID ClassID() override {return MULTI_MATERIAL_CLASS_ID;  }
...
public:
   ...
   BOOL     Param1;
   BOOL     ignoreNotify;
   BOOL		isBatchChange; // UH 01/11/10
   IParamBlock2 *pblock;   // ref #0      
   int      maxMtlId;   // current max material id used in the list...needs to be updated when anything changes

   BOOL     loadingOld;
   void     AddMtl(ReferenceTarget *rt = nullptr, int id=0, const TCHAR *name=nullptr) override;
   void     AddMtlByCheckingSMESetting(bool checkSME = false, ReferenceTarget *rt = nullptr, int id = 0, const TCHAR *name = nullptr);
   void     DeleteSelMtl();
   ...
   void     SetNumSubMtlsByCheckingSMESetting( int n, bool checkSME = false);
   void     GetSubMtlName( int mtlid, TSTR &s ) override;
   void     SetSubMtlAndName( int mtlid, Mtl *m, const TSTR &subMtlName ) override;
   ...
   void     GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_RB_MULTISUBOBJECT) : _T("Multi/Sub-Object"); }
   ...
   Mtl *       GetSubMtl( int i ) override 
            { 
               ...
               Interval iv;
               int mtlOn = true;
               int mtlIdx = hashTab[idx];
               pblock->GetValue(multi_ons, 0, mtlOn, iv, mtlIdx);

               return mtlOn ? subMtl[mtlIdx] : nullptr;
            }
   ...
   int      NumSubs() override         { return subMtl.Count(); }
   Animatable *  SubAnim( int i ) override;
   TSTR SubAnimName(int i, bool localized) override;
   int      SubNumToRefNum( int subNum ) override { return subNum+1; }

   // From ref
   int      NumRefs() override { return subMtl.Count()+1; }
...
public:
   ...
   BOOL     SupportsRenderElements() override         { return TRUE; }
   ReshadeRequirements  GetReshadeRequirements() override   { return mReshadeRQ; }        // mjm - 06.02.00
...
class MultiClassDesc:public ClassDesc2 {
   ...
   void*         Create(BOOL loading) override {  return new Multi(loading); }
   const TCHAR*  ClassName() override { return GetString(IDS_RB_MULTISUBOBJECT_CDESC); }
   const TCHAR*  NonLocalizedClassName() { return _T("Multi/Sub-Object"); }
   SClass_ID      SuperClassID() override { return MATERIAL_CLASS_ID; }
   Class_ID       ClassID() override { return MULTI_MATERIAL_CLASS_ID; }
   const TCHAR*   Category() override { return _T("");  }
// PW: new descriptor data accessors added.  Note that the 
//      internal name is hardwired since it must not be localized.
   const TCHAR*   InternalName() override { return _T("multiSubMaterial"); } // returns fixed parsable name (scripter-visible name)
...
// the sole purpose of this class is to enable creation of a Multi material 
// without the 10 default materials
class EmptyMultiClassDesc: public MultiClassDesc {
public:
    int         IsPublic() override { return FALSE; }

    void*      Create(BOOL loading) override {  return new Multi(loading, FALSE); }
    Class_ID    ClassID() override { return Class_ID(MULTI_CLASS_ID, 1); }

    const TCHAR*    InternalName() override { return _T("multiSubMaterial_empty"); }  // returns fixed parsable name (scripter-visible name)
};

static EmptyMultiClassDesc emptyMultiCD;

ClassDesc* GetEmptyMultiDesc() { 
	return &emptyMultiCD;  
}
...
static ParamBlockDesc2 multi_param_blk ( multi_params, _T("parameters"),  0, &multiCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF, 
   //rollout
   IDD_MULTI, IDS_DS_MULTI_PARAMS, 0, 0, nullptr, 
   // params
...
static INT_PTR CALLBACK  PanelDlgProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
   ...
   if (msg==WM_INITDIALOG) {
      ...
      DLSetWindowLongPtr(hwndDlg, lParam);
       ...
       if ( (theDlg = DLGetWindowLongPtr<MultiDlg *>(hwndDlg) ) == nullptr )
         return FALSE; 
      }
   theDlg->isActive = 1;
...
void MultiDlg::SetTime(TimeValue t) {
   if (t!=curTime) {
      ...
      curTime = t;
      theMtl->Update(curTime,valid);
      // Since nothing is time varying, can skip InvalidateRect
      }
   }

MultiDlg::~MultiDlg() {
   theMtl->SetParamDlg(nullptr); 
   DLSetWindowLongPtr(hPanelBasic, NULL );
   hPanelBasic = nullptr;
   for (int i=0; i<NSUBMTLS; i++) {
...
void MultiDlg::DrawPStamp(HDC hdc, Rect &rect, int i ) {
   ...
   if (m==nullptr) {
      ...
      PStamp *ps = m->GetPStamp(theMtl->mPStampSize);
      if (!ps)
      {
          ps = m->CreatePStamp(theMtl->mPStampSize, TRUE);
      }
      int w = ps->Width();
      int h = ps->Height();
...
LRESULT CALLBACK PStampWndProc( HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam ) 
{
   ...
   MultiDlg *theDlg = DLGetWindowLongPtr<MultiDlg *>(hwParent);
   if (theDlg==nullptr) return FALSE;
...
LRESULT CALLBACK DupMsgWndProc( HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam ) 
{
    switch (msg) {
      case WM_PAINT:
         ...
         HDC hdc = BeginPaint( hwnd, &ps );
         if (!IsRectEmpty(&ps.rcPaint)) {
            FillRect(hdc, &ps.rcPaint, ColorMan()->GetBrush(kBackgroundEven));
            SetTextColor(hdc, RGB(255,0,0));
            SetBkMode(hdc, TRANSPARENT);
            auto txt = GetWindowText(hwnd);
            RECT rect{};
            GetClientRect(hwnd, &rect);
            DLDrawText(hdc, txt, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
            }
         EndPaint( hwnd, &ps );
         }
      break;
...
INT_PTR MultiDlg::BasicPanelProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam ) {
    ...
    switch (msg)    {
      case WM_INITDIALOG:   {    
         ...
         for (int i=0; i<NSUBMTLS; i++) {
            ...
            RECT index_rect = {};
            GetWindowRect(iIndex[i]->GetHwnd(), &index_rect);
            RECT stamp_rect = {};
            GetWindowRect(hwnd, &stamp_rect);
            POINT pt{ stamp_rect.left + ((stamp_rect.right - stamp_rect.left) / 2), index_rect.top + ((index_rect.bottom - index_rect.top) / 2) };
            ...
            MoveWindow(hwnd, pt.x - (stamp_size / 2), pt.y - (stamp_size / 2), stamp_size, stamp_size, FALSE );
            DLSetWindowLongPtr( hwnd, PStampWndProc);
            DLSetWindowLongPtr( hwnd, this);

            hwnd = GetDlgItem(hwndDlg, IDC_DUP_IDS);
            DLSetWindowLongPtr( hwnd, DupMsgWndProc);
      ...
      case WM_COMMAND:  
          switch (id) {
            ...
            case IDC_MULTI_ADD:
               if (theMtl->subMtl.Count() >= 1000) // don't allow more than 1000 materials
                  break;
               RemovePStampHilite();
               theHold.Begin();
               theMtl->AddMtlByCheckingSMESetting(true);
            ...
            case IDC_MTL_ID9: 
                   ...
                   HIWORD(wParam)==EN_CHANGE ) 
               {
                  ...
                  if ( HIWORD(wParam)==EN_KILLFOCUS || iIndex[n]->GotReturn() ) 
                  {
                     ...
                     Interval iv;
                     int oldMtlId;
					 int ndx = n+theMtl->offset;
					 if (ndx < theMtl->pblock->Count(multi_ids))
						theMtl->pblock->GetValue( multi_ids, 0, oldMtlId, iv, ndx );
					 else
						 oldMtlId = iIndex[n]->GetInt()-1;

                     // clamp keyboard input for sub material id#'s
                     const int val = std::clamp(iIndex[n]->GetInt(), 1, 60000);
                     iIndex[n]->SetText(val);
...
void MultiDlg::UpdatePStamp( int i ) {
   ...
   InvalidateRect(hPStamp, nullptr, FALSE);
}

void MultiDlg::UpdateSubMtlNames() {
   IColorSwatch* cs = nullptr; 
   ...
   int i;
   for (i=0; i<theMtl->subMtl.Count()-theMtl->offset && i<NSUBMTLS; i++) {
      Mtl *m = theMtl->subMtl[i+theMtl->offset];
      TSTR nm, label;
      if (m) 
         nm = m->GetFullName(true);
      else 
      ...
      if ((m==nullptr)||(m->IsMultiMtl()||m->NumSubMtls()>0))
...
void MultiDlg::SetThing(ReferenceTarget *m) {
   assert (m->ClassID()==MULTI_MATERIAL_CLASS_ID);
   assert (m->SuperClassID()==MATERIAL_CLASS_ID);
   RemovePStampHilite();
   if (theMtl) theMtl->paramDlg = nullptr;
   theMtl = (Multi *)m;
...
void MultiDlg::SetNumMats(HWND hWnd)
   ...
   if (res>=0) {
      ...
      if (res==theMtl->subMtl.Count())
         return;
      RemovePStampHilite();
      HCURSOR c = SetCursor(LoadCursor(nullptr,IDC_WAIT));
      theHold.Begin();
      theMtl->SetNumSubMtlsByCheckingSMESetting(res, true);
...
void Multi::Reset() {
   ...
   for (int i=0; i<subMtl.Count(); i++) {
      ...
      ReferenceTarget* mtl = (ReferenceTarget*)NewDefaultMaterial(_T("Multi"));
      hs.Resume();
...
void* Multi::GetInterface(ULONG id)
{
   if( id == IID_IReshading )
      ...
      return MultiMtl::GetInterface(id);
}

void Multi::NotifyChanged() 
{
	ivalid.SetEmpty();
	if ( !isBatchChange ) // UH 01/11/10 
...
Multi::Multi(BOOL loading, BOOL createDefaultSubMtls) : mReshadeRQ(RR_None) // mjm - 06.02.00 // mjm - 10.11.99 - added createDefaultSubMtls parameter
{
   ...
   if (!loading) 
   {
	   if (createDefaultSubMtls) // mjm - 10.11.99
		   SetNumSubMtls(NSUBMTLS);
	   else if (pblock) {
		   pblock->SetCount(multi_mtls, 0);
		   pblock->SetCount(multi_ons, 0);
		   pblock->SetCount(multi_names, 0);
		   pblock->SetCount(multi_ids, 0);
	   }
   }
...
static int __cdecl cmpNames( const void *arg1, const void *arg2 ) {
   ...
   Mtl * m2 = s2->mtl;
   if (m1&&m2) {
      int res = _tcsicmp(m1->GetFullName(true).data(), m2->GetFullName(true).data());
      if (res == 0) goto useid;
      return res;
      }
   if (m1) return -1;
...
class DelSubRestore: public RestoreObj {
   ...
   public:
      ...
      void Redo() override;
      TSTR Description() override { return _T("DelSubMtlRestore"); }
   };
...
void DelSubRestore::Redo() {
   if (mtl->paramDlg)
      mtl->paramDlg->RemovePStampHilite();
   mtl->ReplaceReference(nsub+1, nullptr);
...
void Multi::AddMtlByCheckingSMESetting(bool checkSME, ReferenceTarget *rt, int id, const TCHAR *name)
{
   SuspendSetKeyMode();
   ...
   int curID = rt ? id : (MaxSubMtlID() + 1);
   curID = std::min(std::max(curID, 0), 60000 - 1);
   ...
   if (curID > maxMtlId)
       maxMtlId = curID; // 5/2/01 1:26pm --MQM-- moved from #if block below to fix callback/out-of-sync problems
   ...
   pblock->SetValue(multi_ids, 0, curID, n);

   if(rt)
   {
      pblock->SetValue(multi_names,0,name,n);
	  pblock->SetValue(multi_mtls,0,rt,n); // UH 01/11/10- calls ReplaceReference(n+1,rt) internally
   }
   else
   {
      pblock->SetValue(multi_names,0,_T(""),n); // LAM 11/21/00
      if (!checkSME || (checkSME && !GetMultisubEmptySlots())) {
         HoldSuspend hs; // LAM - 6/12/04 - defect 571821
         ReferenceTarget* mtl = (ReferenceTarget*)NewDefaultMaterial(_T("Multi"));
         if (mtl) GetCOREInterface()->AssignNewName((Mtl*)mtl);
         hs.Resume();
         pblock->SetValue(multi_mtls, 0, mtl, n); // UH 01/11/10- calls ReplaceReference(n+1,mtl) internally
      }
   }
   ...
   if ( hashTabDirty )
      ...
      hashTab.AddAssociation(curID, n);

   ResumeSetKeyMode();
}

void Multi::AddMtl(ReferenceTarget *rt, int id, const TCHAR *name)
{
	AddMtlByCheckingSMESetting(false, rt, id, name);
}

void Multi::SetNumSubMtlsByCheckingSMESetting(int num, bool checkSME) {
   
   // UH 01/19/10 Limit the submat count to the match the range of allowed IDs
   if ( num < 1 ) num = 1;
   if ( num > 1000 ) num = 1000;

   isBatchChange = TRUE;  // UH 01/11/10 send REFMSG_CHANGE once at the end.
   ...
   if (n>0) {
      ...
      theHold.Suspend();
	  for (int i = 0; i < n; i++)
		AddMtlByCheckingSMESetting(checkSME);
   ...
   ClampOffset();
   hashTabDirty = 1;                            // MQM 3/5/01 - 11:45am - faster?
   isBatchChange = FALSE; // UH 01/11/10 send REFMSG_CHANGE once at the end:
...
void Multi::SetNumSubMtls(int num) {
	...
	SetNumSubMtlsByCheckingSMESetting(num, false);
...
void Multi::DeleteSelMtl() {
	if (selected>=0&&selected<subMtl.Count()) {
		...
		theHold.Suspend();
		ReplaceReference(selected+1, nullptr);

		// The param block updates below will cause referrers to this target
		// to try to update themselves from the internal data. The hash must be 
		// marked as dirty before deleting items from the param block to prevent
		// clients from pulling bad data.
...
class SetSubRestore: public RestoreObj {
   ...
   public:
      SetSubRestore() = default;

      SetSubRestore(Multi *m, int i, Mtl *mt, int mid, const TCHAR *newName=nullptr) : 
         multi(m), mtl(mt), nsub(i), mtlid(mid)
      {
         if (newName) {
            doname = TRUE;
            newnm = newName; 
      ...
      void Restore(int isUndo) override {
         if (nsub>=0) {
            ...
            multi->pblock->SetValue( multi_mtls, 0, sm, nsub );
            if (doname) {
               const TCHAR *pname = oldnm.data();
      ...
      TSTR Description() override { return _T("SetSubMtlRestore"); }
   };

void Multi::SetSubMtlAndName(int mtlid, Mtl *m, const TSTR &nm) {
   int j = FindSubMtl(mtlid);
   if (theHold.Holding()) 
      theHold.Put(new SetSubRestore(this,j,m,mtlid,nm.data()));
   theHold.Suspend();
   if (j>=0) {
      pblock->SetValue( multi_mtls, 0, m, j );
      ...
      pblock->SetValue( multi_mtls, 0, m, n );
...
TSTR Multi::GetSubMtlSlotName(int i, bool localized) {
   TSTR s;
   const TCHAR* name = nullptr;
   Interval iv;
...
TSTR Multi::SubAnimName(int i, bool localized)
{
	const TCHAR* name = nullptr;
	Interval iv;
	int id;
	...
	TSTR nm;
	if (subMtl[i])
		nm.printf(_T("%s: %s"), s.data(), subMtl[i]->GetFullName(localized).data());
	else
		nm.printf(_T("%s: None"), s.data());
	return nm;
}

RefResult Multi::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
         ...
         if (IsMtl(hTarget)) {
            ...
            for (int i=0; i<n; i++) {
               if (subMtl[i] && ((Mtl *)hTarget==subMtl[i])) {
                  ...
                  subMtl[i]->DiscardPStamp(mPStampSize);
...
public:
   ...
   Multi2PostLoadCallback(Multi *r, BOOL b, Tab<BOOL> bl, BOOL oldA/*,  NameTab sNames*/) {s=r;Param1 = b;ons = bl; oldArray = oldA; /*subNames = sNames;*/}
        ...
        // 420314 : CR
   // Need to make the priority higher. The tables need to be fixed up as soon as possible.
