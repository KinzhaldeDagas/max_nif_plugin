...
class Falloff: public Texmap { 
	...
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_FALLOFF) : _T("Falloff"); }  
...
class FalloffClassDesc:public ClassDesc2 {
	...
	const TCHAR*	ClassName() { return GetString(IDS_DS_FALLOFF_CDESC); }
	const TCHAR*	NonLocalizedClassName() { return _T("Falloff"); }
	SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
	Class_ID 		ClassID() { return falloffClassID; }
...
static ParamBlockDesc2 falloff_param_blk ( falloff_params, _T("parameters"),  0, &falloffCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF, 
	...
	falloff_perp,	_T("perpendicularValue"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_PERVAL,
		...
		p_nonLocalizedName, _T("Perpendicular Value:"),
		p_end,
	falloff_parallel,	_T("parallelValue"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_PARVAL,
		...
		p_nonLocalizedName, _T("Parallel Value:"),
		p_end,
...
INT_PTR FalloffDlgProc::DlgProc(
...
ParamDlg* Falloff::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp) {
	IAutoMParamDlg* dlg = falloffCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
TSTR Falloff::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_DS_PARAMETERS) : _T("Parameters");
	default: assert(0); return _T("null");
	}
}
...
RefResult Falloff::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
   PartID& partID, RefMessage message, BOOL propagate ) {
	switch (message) {
		case REFMSG_CHANGE:
			ivalid.SetEmpty();
