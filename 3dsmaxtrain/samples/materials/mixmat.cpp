...
class MixMat : public Mtl, public IReshading
{
	...
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override {s= localized ?GetString(IDS_RB_BLENDMAT) : _T("Blend");}  
	...
	// Methods to access sub-materials of meta-materials
		...
		TSTR GetSubMtlSlotName(int i, bool localized) override
		{
			if (localized)
			{
				return i ? GetString(IDS_RB_MATERIALTWO) : GetString(IDS_RB_MATERIALONE);
			}
			else
			{
				return i ? _T("Material 2") : _T("Material 1");
			}
		}
		...
		TSTR GetSubTexmapSlotName(int i, bool localized) override {return localized ? GetString(IDS_RB_MASK) : _T("Mask");}
...
class MixMatClassDesc:public ClassDesc2 {
	...
	const TCHAR*	ClassName() {return GetString(IDS_RB_BLENDMAT_CDESC); }
...
//			em->EnableAffectRegion (t);
		...
		case CC_SPINNER_CHANGE:
			mixmat->Mtl::Update(GetCOREInterface()->GetTime());
			GetClientRectP(GetDlgItem(hWnd,IDC_MIXCURVE),&rect);
...
: mReshadeRQ(RR_None) // mjm - 06.02.00
{	
	mVptShadingToggle.SetMaterial(this);
...
void MixMat::Init()
	...
	ReplaceReference(SUB1_REF, NewDefaultMaterial(_T("Blend")));
	ReplaceReference(SUB2_REF, NewDefaultMaterial(_T("Blend")));
...
BaseInterface* MixMat::GetInterface(Interface_ID id)
{
	if (id == mVptShadingToggle.GetID())
		return &mVptShadingToggle;
	return Mtl::GetInterface(id);
}
...
float MixMat::GetSelfIllum(int mtlNum, BOOL backFace)
{
	Mtl* pActiveMtl = UseMtl();
	if (pActiveMtl != NULL)
		return pActiveMtl->GetSelfIllum(mtlNum,backFace);
	return 0.0f;
}
Color MixMat::GetSelfIllumColor(int mtlNum, BOOL backFace)
{
	Mtl* pActiveMtl = UseMtl();
	if (pActiveMtl != NULL)
		return pActiveMtl->GetSelfIllumColor(mtlNum,backFace);
	return Color(0,0,0);
}

BOOL MixMat::GetSelfIllumColorOn(int mtlNum, BOOL backFace)
{
	Mtl* pActiveMtl = UseMtl();
	if (pActiveMtl != NULL)
		return pActiveMtl->GetSelfIllumColorOn(mtlNum,backFace);
	return FALSE;
}
...
TSTR MixMat::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_DS_PARAMETERS) : _T("Parameters");
	case 1: return GetSubMtlTVName(0, localized); // GetString(IDS_RB_MATERIALONE);
	case 2: return GetSubMtlTVName(1, localized); // GetString(IDS_RB_MATERIALTWO);
	case 3: return GetSubTexmapTVName(0, localized); // String(IDS_RB_MASK);
	default: return _T("");
	}
}
