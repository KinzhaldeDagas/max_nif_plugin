...
class PartBlur : public Texmap { 
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_RB_PARTBLUR) : _T("Particle MBlur"); } // mjm - 2.3.99
		void DeleteThis() {delete this;}	

		int NumSubs() {return 1;}  
		Animatable* SubAnim(int i) {return pblock;}
		TSTR SubAnimName(int i, bool localized) override {return localized ? GetString(IDS_DS_PARAMETERS) : _T("Parameters");}
...
class PartBlurClassDesc:public ClassDesc2 {
	...
	const TCHAR*	ClassName() { return GetString(IDS_RB_PARTBLUR_CDESC); }
	const TCHAR*	NonLocalizedClassName() { return _T("Particle MBlur"); }
	SClass_ID		SuperClassID() {return TEXMAP_CLASS_ID;}
	Class_ID 		ClassID() {return PARTBLUR_CLASSID;}
...
static ParamBlockDesc2 pblur_param_blk ( pblur_params, _T("parameters"),  0, &partBlurCD, P_AUTO_CONSTRUCT + P_AUTO_UI, 0, 
	...
	pblur_sharp,	_T("sharp"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_RB_SHARP,
		...
		p_nonLocalizedName, _T("Sharpness"),
		p_end,
...
ParamDlg* PartBlur::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp)
	{
	IAutoMParamDlg* dlg = partBlurCD.CreateParamDlgs(hwMtlEdit, imp, this);
	return dlg;
	}
...
AColor PartBlur::EvalColor(ShadeContext& sc)
	...
	if (ob && ob->IsParticleSystem()) {
		...
		if (size2<=sharp) {
			...
			//
			// If chkMtlAPI is not used, then the particle ID is simply the material ID. 
			//
			int particleID = (chkMtlAPI && chkMtlAPI->SupportsParticleIDbyFace()) ?
					chkMtlAPI->GetParticleFromFace(sc.FaceNumber()) : sc.mtlNum;

			Point3 pos  = sc.PointTo(sc.P(),REF_WORLD);
			Point3 ppos = obj->ParticlePosition(t,particleID);
			Point3 v    = obj->ParticleVelocity(t,particleID);

			s = sharp/size2;

			int ct      = obj->ParticleCenter(t,particleID);
