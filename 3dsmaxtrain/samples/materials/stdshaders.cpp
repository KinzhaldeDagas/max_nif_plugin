...
static ParamID colParamID[NCOLBOX] = { shdr_ambient, shdr_diffuse, shdr_specular, shdr_self_illum_color };
#define N_SI_CLR     3
#define N_AMB_CLR    0
...
{
	BOOL res;
	ICustButton* iBut = GetICustButton(GetDlgItem(hWnd,id));
	res = iBut->IsChecked();
...
static void SetupLockButton(HWND hWnd,int id, BOOL check)
{
	...
	iBut->SetIconByName(_T("MaterialEditor/LinkFields"),16,16);
	ReleaseICustButton(iBut);
}

static void SetupPadLockButton(HWND hWnd,int id, BOOL check) {
	ICustButton* iBut = GetICustButton(GetDlgItem(hWnd,id));
	iBut->SetIconByName(_T("Common/Lock"),16,16);
	iBut->SetType(CBT_CHECK);
	ReleaseICustButton(iBut);
}
...
public:
	...
	virtual TSTR  GetTexChannelName( long nTex, bool localized ) override;
	...
	BOOL KeyAtTimeByID(ParamID id,TimeValue t);
	...
	void GetClassName(MSTR& s, bool localized) const override { s = GetName(localized); }
	...
	IParamBlock2* GetParamBlockByID(BlockID id) { return (pblock && pblock->ID() == id) ? pblock : NULL; }
...
public:
	...
	RefTargetHandle Clone( RemapDir &remap, StdShaderImp* mnew=NULL );
	...
	virtual TSTR GetName(bool localized) const override;
...
public:
	...
	BOOL KeyAtCurTime(ParamID id) { return pShader->KeyAtTimeByID(id,curTime); } 
	void DeleteThis() { delete this; }
	...
	void SetCurRGB(Color rgb) { curRGB = rgb; }
};
...
static constexpr int texMButtonsIDC[NMBUTS] = {
...
static constexpr int texmapFromMBut[NMBUTS] = { ID_AM, ID_DI, ID_SP, ID_SS, ID_SH, ID_SI, ID_OP };

// localized channel names
static constexpr int texNameIDS[STD2_NMAX_TEXMAPS] = {
	IDS_DS_AMBIENT,
	IDS_DS_DIFFUSE,
	IDS_DS_SPECULAR,
	IDS_DS_SHIN_STR,
...
static constexpr const TCHAR* nonlocalizedTexNames[STD2_NMAX_TEXMAPS] = {
	_T("Ambient Color"),
	_T("Diffuse Color"),
	_T("Specular Color"),
	_T("Specular Level"),
	_T("Glossiness"),
	_T("Self-Illumination"),
	_T("Opacity"),
	_T("Filter Color"),
	_T("None"),
	_T("None"),
...
static constexpr const TCHAR* texInternalNames[STD2_NMAX_TEXMAPS] = {
...
static constexpr int channelType[] = {
...
static constexpr int stdIDToChannel[N_ID_CHANNELS] = { 0, 1, 2, 4, 3, 5, 6, 7, -1, -1, -1, -1, -1, -1, -1, -1 };
...
TSTR  StdShaderImp::GetTexChannelName( long nTex, bool localized )
{
	return localized ? GetString(texNameIDS[nTex]) : nonlocalizedTexNames[nTex];
}
...
BOOL StdShaderImp::KeyAtTimeByID(ParamID id,TimeValue t) { return pblock->KeyFrameAtTimeByID(id,t); }
TSTR StdShaderImp::GetName(bool localized) const { return localized ? GetString(IDS_KE_STDSHADER) : _T("genericShader"); }
...
RefResult StdShaderImp::NotifyRefChanged(
...
TSTR StdShaderImp::SubAnimName(int i, bool localized)
{
...
static INT_PTR CALLBACK  StdShaderDlgProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
	StdShaderDlg* theDlg = NULL;
	if (msg == WM_INITDIALOG) {
		theDlg = (StdShaderDlg*)lParam;
		DLSetWindowLongPtr(hwndDlg, lParam);
	} else {
		if ( (theDlg = DLGetWindowLongPtr<StdShaderDlg *>(hwndDlg) ) == NULL )
			return FALSE; 
	}
	theDlg->isActive = 1;
...
ShaderParamDlg* StdShaderImp::CreateParamDialog(HWND hOldRollup, HWND hwMtlEdit, IMtlParams *imp, StdMat2* theMtl, int rollupOpen, int ) {
	...
	if ( hOldRollup )
		...
		StdShaderDlgProc, 
		rollupName(theMtl->GetShader()).data(),
		(LPARAM)pDlg , 
...
void  StdShaderDlg::LoadDialog(BOOL draw) 
{
	if (pShader && hRollup) {
		...
		IRollupWindow* pRollup = pMtlPar->GetMtlEditorRollup();
		pRollup->SetPanelTitle( pRollup->GetPanelIndex(hRollup), rollupName(pShader).data());
		...
		// Make sure controls are disabled if locked or if value can't be edited.
		IParamBlock2 * shaderpblock = pShader->pblock;
		if ( shaderpblock ) {
			for( long i = 0; i < NCOLBOX; ++i ) {
				cs[i]->UpdateEnableState( shaderpblock, shaderpblock->GetAnimNum( colParamID[i] ) );
			}
			softSpin->UpdateEnableState( shaderpblock, shaderpblock->GetAnimNum( shdr_soften ) );
			shSpin->UpdateEnableState( shaderpblock, shaderpblock->GetAnimNum( shdr_glossiness ) );
			ssSpin->UpdateEnableState( shaderpblock, shaderpblock->GetAnimNum( shdr_spec_lvl ) );
			siSpin->UpdateEnableState( shaderpblock, shaderpblock->GetAnimNum( shdr_self_illum_amnt ) );
		}
...
void StdShaderDlg::SetMtlColor(int i, Color c) {
	...
	case 0: //ambient
		...
		if ( pShader->GetLockAD() ){
			pMtl->SetDiffuse(c,curTime);
			cs[1]->SetColor( c );
			if (pShader->GetLockDS() ){
				pMtl->SetSpecular(c, curTime);
				cs[2]->SetColor(c);
			}
		}
		break;
	...
	case 2: // specular
		...
		break;
	case 3: 
		pMtl->SetSelfIllumColor(c, curTime);
		break;
	}
}
...
{
	if (lock) {
		TSTR msgBoxText = GetString(IDS_DS_LOCKAD);
		if (IDYES != MaxSDK::MaxMessageBox(hwmEdit, msgBoxText, GetString(IDS_DS_LOCKCOL), MB_YESNO)) {
...
void StdShaderDlg::UpdateColSwatches() 
{
	for(int i=0; i < NCOLBOX; i++) {
		if ( cs[ i ] ) {
			cs[i]->SetKeyBrackets( pShader->KeyAtTimeByID(colParamID[i],curTime) );
...
static LRESULT CALLBACK HiliteWndProc( HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam ) 
{
	...
	StdShaderDlg *theDlg = DLGetWindowLongPtr<StdShaderDlg *>(hwParent);
...
{
	if (lock) {
		TSTR msgBoxText = GetString(IDS_DS_LOCK_DS);
		if (IDYES != MaxSDK::MaxMessageBox(hwmEdit, msgBoxText, GetString(IDS_DS_LOCKCOL), MB_YESNO)) {
...
INT_PTR StdShaderDlg::PanelProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam ) 
{
	...
	case CC_SPINNER_CHANGE: 
		...
		switch (id) {
		case IDC_SH_SPIN: 
			pMtl->SetShininess(PcToFrac(shSpin->GetIVal()), curTime);
			UpdateHilite();
			break;
		case IDC_SS_SPIN: 
			pMtl->SetShinStr(PcToFrac(ssSpin->GetIVal()),curTime);
			UpdateHilite();
			break;
		case IDC_SOFT_SPIN: 
			...
			break;
		case IDC_SI_SPIN: 
			pMtl->SetSelfIllum(PcToFrac(siSpin->GetIVal()),curTime);
			break;
...
void phongIllum(ShadeContext &sc, IllumParams &ip, float softThresh, BOOL clrSelfIllum ) 
{
	...
	isShiny = (ip.channels[stdIDToChannel[ID_SS]].r > 0.0f);
	if (isShiny == TRUE) 
...
public:
	...
	TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_CONSTANT) : _T("Constant"); }
...
public:
	...
	const TCHAR*  NonLocalizedClassName() { return _T("Constant"); }
	SClass_ID      SuperClassID() { return SHADER_CLASS_ID; }
	Class_ID       ClassID() { return ConstantClassID; }
...
Constant2::Constant2(){ Reset(); }

ExposureMaterialControlDesc Constant2::msExpMtlControlDesc(
	constCD,
...
public:
	...
	TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_PHONG) : _T("Phong"); }
	void Illum(ShadeContext &sc, IllumParams &ip){
		phongIllum( sc, ip, softThresh, selfIllumClrOn ); 
		ShadeTransmission(sc, ip, ip.channels[ID_RR], ip.refractAmt);
		ShadeReflection( sc, ip, ip.channels[ID_RL] ); 
...
public:
	...
	const TCHAR*  NonLocalizedClassName() { return _T("Phong"); }
	SClass_ID      SuperClassID() { return SHADER_CLASS_ID; }
	Class_ID       ClassID() { return PhongClassID; }
	const TCHAR*   Category() { return _T("");  }
...
// shader parameters
static ParamBlockDesc2 phong2_param_blk ( 
	shdr_params, _T("shaderParameters"),  0, &phongCD, P_AUTO_CONSTRUCT + P_USE_PARAMS, 
	// pblock refno
	0, 
...
ExposureMaterialControlDesc Phong2::msExpMtlControlDesc(
	phongCD,
	IDS_EXPOSURE_MATERIAL_CONTROL,
	IDS_NO_EXPOSURE,
	IDS_INVERTSELFILLUM,
...
public:
	...
	TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_BLINN) : _T("Blinn"); }
	void Illum(ShadeContext &sc, IllumParams &ip);
	void AffectReflection(ShadeContext &sc, IllumParams &ip, Color &rcol) 
	{ 
		rcol *= ip.channels[ID_SP]; };
...
public:
	...
	const TCHAR*  NonLocalizedClassName() { return _T("Blinn"); }
	SClass_ID      SuperClassID() { return SHADER_CLASS_ID; }
	Class_ID       ClassID() { return BlinnClassID; }
	const TCHAR*   Category() { return _T("");  }
...
// shader parameters
static ParamBlockDesc2 blinn2_param_blk ( 
	shdr_params, _T("shaderParameters"),  0, &blinnCD, P_AUTO_CONSTRUCT, 0, 
	// // use params from existing descriptor
	// &const_param_blk
	// params
	shdr_ambient,   _T("ambient"),   TYPE_RGBA,  P_ANIMATABLE,  IDS_DS_AMBIENT,   
...
ExposureMaterialControlDesc Blinn2::msExpMtlControlDesc(
	blinnCD,
	IDS_EXPOSURE_MATERIAL_CONTROL,
	IDS_NO_EXPOSURE,
	IDS_INVERTSELFILLUM,
...
void Blinn2::Illum(ShadeContext &sc, IllumParams &ip) {
	...
	for (int i=0; i<sc.nLights; i++) {
		...
		if ( l ) { // l may be == null if a light class is not available in the current config
			...
			Point3 L;
			Point3 normal = sc.Normal();
			if (l->Illuminate(sc,normal,lightCol,L,NL,diffCoef)) {
				if (l->ambientOnly) {
					ip.ambIllumOut += lightCol;
					continue;
				}
...
public:
	...
	TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_METAL) : _T("Metal"); }
	void Illum(ShadeContext &sc, IllumParams &ip);
	void AffectReflection(ShadeContext &sc, IllumParams &ip, Color &rcol)
...
public:
	...
	const TCHAR*  NonLocalizedClassName() { return _T("Metal"); }
	SClass_ID      SuperClassID() { return SHADER_CLASS_ID; }
	Class_ID       ClassID() { return MetalClassID; }
	const TCHAR*   Category() { return _T("");  }
...
/*******
// shader parameters
static ParamBlockDesc2 metal2_param_blk ( 
shdr_params, _T("shaderParameters"),  0, &metalCD, P_AUTO_CONSTRUCT + P_USE_PARAMS, 
// pblock refno
0, 
// use params from existing descriptor
&const_param_blk
...
// shader parameters
static ParamBlockDesc2 metal2_param_blk ( 
	shdr_params, _T("shaderParameters"),  0, &metalCD, P_AUTO_CONSTRUCT, 0, 
	// params
	shdr_ambient,   _T("ambient"),   TYPE_RGBA,  P_ANIMATABLE,  IDS_DS_AMBIENT,   
	p_default,     Color(0, 0, 0), 
	p_nonLocalizedName, _T("Ambient Color"), 
...
ExposureMaterialControlDesc Metal2::msExpMtlControlDesc(
	metalCD,
	IDS_EXPOSURE_MATERIAL_CONTROL,
	IDS_NO_EXPOSURE,
	IDS_INVERTSELFILLUM,
...
void Metal2::Illum(ShadeContext &sc, IllumParams &ip)
{
	...
	Color lightCol;
	BOOL gotKav = FALSE;
	float kav(0.0f), fav0(0.0f), m2inv(0.0f), NV(0.0f);
