...
class Tint: public MultiTex { 
	...
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_RGBTINT) : _T("RGB Tint"); }  
...
class TintClassDesc:public ClassDesc2 {
	...
	const TCHAR*	ClassName() { return GetString(IDS_DS_RGBTINT_CDESC); }
	const TCHAR*	NonLocalizedClassName() { return _T("RGB Tint"); }
	SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
	Class_ID 		ClassID() { return tintClassID; }
...
static ParamBlockDesc2 rgbtint_param_blk ( rgbtint_params, _T("parameters"),  0, &tintCD, P_AUTO_CONSTRUCT + P_AUTO_UI, 0, 
	...
	rgbtint_map1_on,	_T("map1Enabled"), TYPE_BOOL,			0,				IDS_JW_MAP1ENABLE,
		p_default,		TRUE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_MAPON,
		p_end,
	p_end
);
...
ParamDlg* Tint::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp) {
	IAutoMParamDlg* dlg = tintCD.CreateParamDlgs(hwMtlEdit, imp, this);
	return dlg;
...
TSTR Tint::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
...
TSTR Tint::SubAnimName(int i, bool localized)
{
	switch (i)
	{
...
RefResult Tint::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
   PartID& partID, RefMessage message, BOOL propagate ) {
	switch (message) {
		case REFMSG_CHANGE:
			ivalid.SetEmpty();
