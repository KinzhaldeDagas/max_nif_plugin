...
static constexpr int texmapFromMBut[] = { 0, 1, 2, 3, 4, 5, 6, 8, 9 };

// channel names
#define ON_NTEXMAPS  10

static int constexpr texNameIDS[STD2_NMAX_TEXMAPS] = {
...
static constexpr int stdIDToChannel[N_ID_CHANNELS] = { 0, 1, 2, 3, 4, 5, 6, 7, -1, -1, -1, -1, -1, -1, -1, -1 };
...
#define NCOLBOX 4
static constexpr int colID[NCOLBOX] = { IDC_STD_COLOR1, IDC_STD_COLOR2, IDC_STD_COLOR3, IDC_SI_COLOR };
static ParamID colParamID[NCOLBOX] = { onb_ambient, onb_diffuse, onb_specular, onb_self_illum_color };
#define N_SI_CLR     3
#define N_AMB_CLR    0
...
static ParamBlockDescID ONShaderPB[ ON_SHADER_NPARAMS ] = {
   ...
   { TYPE_FLOAT, NULL, TRUE, static_cast<DWORD>(-1) },              // diffuse reflectivity
...
public:
   ...
   TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_OREN_NAYAR_BLINN) : _T("Oren-Nayar-Blinn"); }
   ...
   TSTR SubAnimName(int i, bool localized) override { return localized ? GetString(IDS_KE_OREN_PARMS) : _T("Oren Nayar Parameters"); };
...
class OrenNayarBlinnShaderClassDesc:public ClassDesc2 {
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Oren-Nayar-Blinn"); }
   ...
   for (int i=0; i<sc.nLights; i++) {
      ...
      if (l->Illuminate( sc, normal, lightCol, L, NL, kL)) {
         ...
         if (l->affectDiffuse){
            float diffIntens;
            Point3 scV = sc.V();
            Color d = OrenNayarIllum( sc.Normal(), L, scV, ip.channels[ID_DIFF_ROUGH].r * Pi*0.5f, ip.channels[ID_DI], &diffIntens, NL );
...
static INT_PTR CALLBACK  OrenNayarShaderDlgProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
   ...
   if (msg == WM_INITDIALOG) {
      ...
      DLSetWindowLongPtr(hwndDlg, lParam);
       ...
       if ( (theDlg = DLGetWindowLongPtr<OrenNayarShaderDlg *>(hwndDlg) ) == NULL )
         return FALSE; 
   }
   theDlg->isActive = 1;
...
RefResult OrenNayarBlinnShader::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
                             PartID& partID, RefMessage message, BOOL propagate ) 
{
   switch (message) {
      case REFMSG_CHANGE:
...
//       ShowWindow( siSpin->GetHwnd(), SW_SHOW );
      ...
      if ( shaderpblock ) {
		  ...
		  softSpin->UpdateEnableState( shaderpblock, shaderpblock->GetAnimNum( onb_soften ) );
		  shSpin->UpdateEnableState( shaderpblock, shaderpblock->GetAnimNum( onb_glossiness ) );
		  ssSpin->UpdateEnableState( shaderpblock, shaderpblock->GetAnimNum( onb_specular_level ) );
		  siSpin->UpdateEnableState( shaderpblock, shaderpblock->GetAnimNum( onb_self_illum_amnt ) );
		  dlevSpin->UpdateEnableState( shaderpblock, shaderpblock->GetAnimNum( onb_diffuse_level ) );
		  roughSpin->UpdateEnableState( shaderpblock, shaderpblock->GetAnimNum( onb_roughness ) );
	  }
...
INT_PTR OrenNayarShaderDlg::PanelProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam ) 
{
