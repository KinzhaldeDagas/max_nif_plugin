...
Color OrenNayarIllum(const Point3& N, const Point3& L, Point3& V, float rough, const Color& rho, float* pDiffIntensOut, float NL = -2.0f );

float GaussHighlight( float gloss, float aniso, float orient,  
					  const Point3& N, const Point3& V, const Point3& L, const Point3& U, float* pNL );
...
inline float Dot(const Point3& a, const Point3& b ) { return a.x * b.x + a.y * b.y + a.z * b.z; }
inline float Len2(const Point3& a ) { return Dot(a,a); }
inline float Len(const Point3& a ) { return float( sqrt( Len2(a))); }
...
inline float Min(const Color& c ){ return Min( c.r, c.g, c.b ); }
inline float Max( float a, float b ) { return (a < b) ? b : a; }
inline float Max( float a, float b, float c ) { return (a < b) ? Max( b, c ) : Max(a,c); }
inline float Max(const Color& c ){ return Max( c.r, c.g, c.b ); }

inline float NormClr( Color& a ){ float m = Max( a ); if(m !=0.0f) a /= m; return m; }

inline float Bound(float x, float min = 0.0f, float max = 1.0f) { return std::clamp(x, min, max); }
inline float Bound0_1(float x) { return Bound(x, 0.f, 1.f); }
...
inline Color Bound(const Color& c )
...
Point3 RotateVec( const Point3& p, const Point3& axis, float rdn );
