...
Color OrenNayarIllum( const Point3& N, const Point3& L, Point3& V, float rough, const Color& rho, float* pDiffIntensOut, float NL )
{
...
float GaussHighlight( float gloss, float aniso, float orient,  
                 const Point3& N, const Point3& V, const Point3& L, const Point3& T, float* pNL )
{
   float out = 0.0f;
...
// DbgAssert( ax >= 0.0f && ay >= 0.0f );
   ...
   if (NH > 0.0f) {
      ...
      float orv = orient * 180.0f;
      Point3 T1 = T;
      if ( orv != 0.0f )
         T1 = RotateVec( T, N, DegToRdn(orv));
...
//
Point3 RotateVec( const Point3& p, const Point3& a, float rdn )
{
