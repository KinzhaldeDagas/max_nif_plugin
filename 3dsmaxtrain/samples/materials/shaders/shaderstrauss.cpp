...
static constexpr int texmapFromMBut[] = { 0, 1, 2, 3 };
...
static constexpr int stdIDToChannel[N_ID_CHANNELS] = { -1, 0, -1, 1, -1, -1, 3, 4, -1, -1, -1, -1, -1, -1, -1, -1 };
...
static constexpr int colID[STRAUSS_NCOLBOX] = { IDC_STD_COLOR2  };
...
static ParamBlockDescID StraussShaderPB[ STRAUSS_SHADER_NPARAMS ] = {
   ...
   { TYPE_FLOAT, NULL, TRUE,static_cast<DWORD>(-1) },  // ambient level
...
protected:
   ...
   TSTR GetTexChannelName( long nChan, bool localized ) override { return localized ? GetString(texNameIDS[nChan]) : nonlocalizedTexNames[nChan]; }
   ...
   TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_STRAUSS) : _T("Strauss"); }
   ...
   TSTR SubAnimName(int i, bool localized) override { return localized ? GetString(IDS_KE_STRAUSS_PARMS) : _T("Strauss Parameters"); };
...
class StraussShaderClassDesc:public ClassDesc2 {
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Strauss"); }
   SClass_ID      SuperClassID() { return SHADER_CLASS_ID; }
   Class_ID       ClassID() { return StraussShaderClassID; }
...
// BOOL dimDiffuse = ip.hasComponents & HAS_REFLECT;
   ...
   for (int i=0; i<sc.nLights; i++) {
      ...
      if (l->Illuminate( sc, normal, lightClr, L, NL, Kl)) {
         if (l->ambientOnly) {
...
RefResult StraussShader::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
                             PartID& partID, RefMessage message, BOOL propagate ) 
{
   switch (message) {
      case REFMSG_CHANGE:
...
{
   cs[0]->SetKeyBrackets( pShader->KeyAtTimeByID(st_diffuse,curTime) );
