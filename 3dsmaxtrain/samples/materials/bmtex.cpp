//*********************************************************************/
// Copyright (c) 1994-2020 Autodesk, Inc.
// All rights reserved.
//
...
MaxSDK::AssetManagement::AssetUser BMTexPathAccessor::GetAsset() const {
	return mTex->GetMap();
}


bool BMTexPathAccessor::SetAsset(const MaxSDK::AssetManagement::AssetUser& aNewAsset)	{
	mTex->pblock->SetValue(bmtex_filename, 0, aNewAsset);
	return true;
}
...
MaxSDK::AssetManagement::AssetType BMTexPathAccessor::GetAssetType() const	{
...
//{ 	return new BMTex; }
	const TCHAR*	ClassName() { return GetString(IDS_DS_BITMAP_CDESC); }
	const TCHAR*	NonLocalizedClassName() { return _T("Bitmap"); }
	...
	void Get(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t, Interval &valid)    // get into v
	{
		...
		switch (id)
		{
			case bmtex_filename:
			{
				...
				TSTR assetId = IAssetManager::GetInstance()->AssetIdToString(bmt->GetMap().GetId());
				v.ReplaceString(assetId);
				break;
			}
...
static ParamBlockDesc2 bmtex_param_blk ( bmtex_params, _T("parameters"),  0, &bmTexCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF, 
	...
	bmtex_clipu,	_T("clipu"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_CLIPU,
		...
		p_nonLocalizedName, _T("Clip U Offset"),
		p_end,
	bmtex_clipv,	_T("clipv"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_CLIPV,
		...
		p_accessor,		&bmtex_accessor,
		p_nonLocalizedName, _T("Clip V Offset"),
		p_end,
	bmtex_clipw,	_T("clipw"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_CLIPW,
		...
		p_accessor,		&bmtex_accessor,
		p_nonLocalizedName, _T("Clip U Width"),
		p_end,
	bmtex_cliph,	_T("cliph"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_CLIPH,
		...
		p_accessor,		&bmtex_accessor,
		p_nonLocalizedName, _T("Clip V Width"),
		p_end,
	bmtex_jitter,	_T("jitter"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_JITTERAMT,
		...
		p_nonLocalizedName, _T("Jitter Placement"),
		p_end,
	bmtex_usejitter,	_T("useJitter"), TYPE_BOOL,			0,		IDS_PW_USEJITTER,
		p_default,		FALSE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_BM_JITTER,
		p_end,
	bmtex_apply,	_T("apply"), TYPE_BOOL,			0,		IDS_PW_APPLY,
		p_default,		FALSE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_BM_CLIP,
		p_end,
	...
	bmtex_premultalpha, _T("preMultAlpha"), TYPE_BOOL,		0,		IDS_PW_PREMULT,
		p_default,		TRUE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_ALPHA_PREMULT,
		p_end,
...
static ParamBlockDesc2 bmtex_time_param_blk ( bmtex_time, _T("time parameters"),  0, &bmTexCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCKTIME_REF, 
	...
	bmtex_matidtime, _T("tieTimeToMatIDs"), TYPE_BOOL,		0,		IDS_PW_MATIDTIME,
		p_default,		FALSE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_MATIDTIME_CHECK,
		p_accessor,		&bmtextime_accessor,
		p_end,
...
BOOL BMTexDlg::KeyAtCurTime(int id) { return theBMTex->pblock->KeyFrameAtTimeByIndex(id,curTime); }
...
void BMTexDlg::ShowCropImage()
{
	...
	if (m_displayBitmap )
	{
		...
		CropCallback* cropCallback = nullptr;
		...
		int junk = m_displayBitmap->Display(GetString(IDS_DS_CROP_TITLE), BMM_RND, TRUE, FALSE, cropCallback);
	}
	else
	{
		DbgAssert(false);
...
{
	// MAXX-66567: close the crop image window if the file / storage changes
	RemoveCropImage();
...
{	
    ...
    switch (msg)    {
		...
		case WM_COMMAND:  
			...
			BroadcastNotification<NOTIFY_BITMAP_CHANGED>(theBMTex->m_bi.Name());
					//theBMTex->NotifyChanged();
...
void BMTex::Init()
{
	...
	if (texout)
		...
		ReplaceReference(2, GetNewDefaultTextureOutput());
...
void BMTex::Notify(NotifyInfo* info)
{
	...
	case NOTIFY_BITMAP_CHANGED: 
		{
			const TCHAR* nm = GetNotifyParam<NOTIFY_BITMAP_CHANGED>(info);
  ...
  m_proxiesWereDisabledForRendering(false)
{
	...
	clipValid = FOREVER;
	fileNotFound = FALSE;
	bmTexCD.MakeAutoParamBlocks(this);	// make and initialize paramblock2
	Init();
...
Bitmap* BMTex::GetActiveBitmap() const {
	...
	if ((bitmap == nullptr) && !m_theBitmapList.isEmpty())
	{
...
//! [BMTex.OnInteractiveCreation Example]
...
void BMTex::OnInteractiveCreation()
{
	BitmapLoadDlg();
}
...
void BMTex::SetMapName(const TCHAR *name, bool isUIAction) { 
	...
	if (name) 
	{
		// Make sure that the input rules are run when the bitmap is set just
		// through the filename. Note that if it's set though the bitmapInfo
		// instead, then it may have its cs info populated with the user request.
		m_bi.ResetColorSpaceInfo();

		SetMap(IAssetManager::GetInstance()->GetAsset(name,kBitmapAsset));
	}
}

void BMTex::SetMap(const MaxSDK::AssetManagement::AssetUser& asset, bool isUIAction)
{
	TCHAR uncname[MAX_PATH];
	if (BMMGetUniversalName(uncname, _countof(uncname), asset.GetFileName()))
...
const MaxSDK::AssetManagement::AssetUser& BMTex::GetMap() 
{
	UpdateBIName(); // CCJ 4/27/99
	return m_bi.GetAsset();
}
...
BMMRES BMTex::LoadBitmap(TimeValue t, bool quiet, bool reload) {
	...
	// BMTex::Update may not be called before this function. We need to make sure start 
	// time is up-to-date before loading a frame from bitmap.
	pblockTime->GetValue(bmtex_start, t, startTime, FOREVER);
...
//		}
	...
	// This trap will detect the gamma sync issue between the BMTex and
	// underlying storage. See MAXX-70810.
#if defined(_HYBRID) || defined(_DEBUG)
	{
		Bitmap* curBitmap = m_theBitmap.GetBitmap();
		BitmapStorage* curStorage = curBitmap ? curBitmap->Storage() : nullptr;

		if(curStorage)
		{
			float storageGamma = curStorage->Gamma();

			float biGamma = m_bi.Gamma();

			float gamDiff = storageGamma - biGamma;
			if (abs(gamDiff) > 0.01f)
			{
				DbgAssertMsg(false, _T("Mismatch in storage gamma and bi gamma"));
			}
		}
	}
#endif
...
void BMTex::GetClassName(MSTR& s, bool localized) const { 

	s= localized ? GetString(IDS_DS_BITMAP) : _T("Bitmap"); 
} 
...
void BMTex::ReloadBitmap(bool isUIAction)
{
	if (GetActiveBitmap() != NULL)
	{
		loadFailed = FALSE;
		FreeBitmap();
		LoadBitmap(GetCOREInterface()->GetTime(), TRUE, TRUE); /// TODO: return value is lost here. /coz 2020-05-08

		// Re-load the bitmap in-place, forcing a reload. (Why? /coz 2022-02-21)
	...
	if (pmap != NULL)
		pmap->SetText(bmtex_bitmap, (MCHAR*)m_bi.Name());


	if (paramDlg)
		paramDlg->EnableAlphaButtons(FALSE);

	// Trigger the bitmap proxy pre-caching dialog if the reload occurred
...
void BMTex::EnumAuxFiles(AssetEnumCallback& nameEnum, DWORD flags) {
...
BITMAPINFO* BMTex::GetVPDisplayDIB(TimeValue t, TexHandleMaker& thmaker, Interval &valid, BOOL mono, BOOL forceW, BOOL forceH) {   
	...
	{
		// bug :424747
		//the image has multiple frames, the validity should be only for the current frame
...
// created.  It can act as a 'main' ParamDlg to which you can add any number of 
...
// Gradient main AutoMParamDlg so it will keep them up-to-date automatically

	// create the rollout dialogs
	uvGenDlg = uvGen->CreateParamDlg(hwMtlEdit, imp);	
	IAutoMParamDlg* dlg = bmTexCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
void BMTex::Update(TimeValue t, Interval& valid) {		
	...
	if (!ivalid.InInterval(t)) {
		//We only concern about parameter changes
		//Missing map causes ivalid dirty, we don't want to notify dependents
		if (!clipValid.InInterval(t))
		{
	...
	if (GetActiveBitmap()&&!bmWasSet) {
		...
		CommandMode* pCmdMode = nullptr;
		if (ip->GetCommandStackSize() > 0)
		{
			pCmdMode = ip->GetCommandMode();
		}
		if (!(pCmdMode && pCmdMode->ID() == CID_PLAYANIMATION))
		{
			//Don't try to reload bitmap during animation
			ivalid.SetInstant(t);  // force bitmap to be reloaded 
		}		
	}
...
TSTR BMTex::GetFullName(bool localized) {
	TSTR cnm, nm, fname;
	GetClassName(cnm, localized);
	UpdateBIName();
	GetBMName(m_bi, fname);
	const TCHAR* s = fname.Length() > 0 ? fname.data() : cnm.data();
	nm.printf(_T("%s (%s)"), GetName().data(), s);
	return nm;
	}

TSTR BMTex::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_DS_COORDINATES) : _T("Coordinates");
	case 1: return localized ? GetString(IDS_DS_PARAMETERS) : _T("Parameters");
	case 2: return localized ? GetString(IDS_DS_OUTPUT) : _T("Output");
	case 3: return localized ? GetString(IDS_DS_PARAMETERS) : _T("Parameters");
	default: assert(0); return _T("");
	}
}

RefResult BMTex::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
...
void BMTex::BitmapLoadDlg()	{
	...
	if (_tcscmp(s_lastName, _T(""))) {
		...
		_tcscpy(dummy, TheManager->GetDir(APP_IMAGE_DIR).data());
...
{
	ReloadBitmap(false);
	BroadcastNotification<NOTIFY_BITMAP_CHANGED>(m_bi.Name());
	if (ip) ip->MtlChanged();
...
//! [BMTex.GetInterface Example]
BaseInterface* BMTex::GetInterface(Interface_ID id)
{
	if (id == BITMAPTEX_INTERFACE)
	{
		return static_cast<BitmapTex*>(this);
	}
	else if (id == MATERIAL_BROWSER_ENTRY_INSTANCE_CALLBACK_INTERFACE_ID)
	{
		// BMTex implements IMaterialBrowserEntryInstanceCallback
		return static_cast<IMaterialBrowserEntryInstanceCallback*>(this);
	}

	if (BaseInterface* pBaseInterface = FPMixinInterface::GetInterface(id))
	{
...
{
	// if the proxy slot already has the full resolution image, don't do anything
	// otherwise we'll end up with full res image in both slots, one listening to
	// the file changes other not, causing file-sync hell. (MAXX-66567) 
	// /coz 2022-02-24
	if(!m_nonProxyBitmapInfo.TestFlags(MAP_PROXY))
		return;
...
void BMTex::BitmapHolder::SetBitmap(Bitmap* bitmap) {
	...
	// The bitmap should first have been freed in order to avoid memory leaks.
...
{
	// Register the notification with only one bitmap; registering it with both
	// bitmaps results in a crash when the notification is received and the
	// bitmaps are reloaded (the crash occurs when the notification system
	// attempts to notify the second, now defunct bitmap).

	// Register the notification with the proxy bitmap. The notification is
	// responsible for updating the viewport, so it must be registered against
	// the proxy bitmap which is used for viewport display. The notification
	// also triggers a full reload if the subject changes, which would be
	// equivalent if registered against either bitmap.

	// Note: The above logic is not super safe: If the proxy system is turned-off
	// globally, we load the full-res image into the m_proxyBitmap slot (by
	// design). If any code wants to make sure that it loads the full resolution
	// one (such as ShowCropImage() ) then it issues ProxyModeDisableGuard,
	// which causes another copy of the bitmap be loaded into the
	// m_NonProxyBitmap slot (bug? design flaw?). If this new copy doesn't
	// listen to the file changes, it won't free itself when the file is deleted
	// on the disk so there will be a stale shared storage in the memory. In
	// this case the subsequent attempts to load the image will find the storage
	// in the memory and will happily use it, unaware of the missing file.
	// (MAXX-66567) /coz 2022-02-24
...
void BMTex::SetBitmapInfo(const BitmapInfo& bi)
{
	if( bi.Name() != NULL )
	{
		m_bi = bi;
