...
static constexpr const TCHAR *suffixes[6] = { 
	_T("_UP"),
	_T("_DN"),
	_T("_LF"),
	_T("_RT"),
...
void BuildSphereMap(Bitmap *bm, EnvSampler& env) {
	...
	MaxSDK::MaxMessageBox(NULL, _T("The spherical map"), _T(" Auto Cubic Test"), MB_OK | MB_ICONEXCLAMATION);
	bm->UnDisplay();
#endif
	}
...
//			DeleteCriticalSection(&csect); 
		...
		int WriteBM(Bitmap *bm, const TCHAR *name);
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_ACUBIC_NAME) : _T("Reflect/Refract"); }  
...
class ACubicClassDesc:public ClassDesc2 {
	...
	const TCHAR*	ClassName() { return GetString(IDS_DS_ACUBIC_NAME_CDESC); }
	const TCHAR*	NonLocalizedClassName() { return _T("Reflect/Refract"); }
	SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
	Class_ID 		ClassID() { return acubicClassID; }
...
static ParamBlockDesc2 acubic_param_blk ( acubic_params, _T("parameters"),  0, &acubicCD, P_AUTO_CONSTRUCT + P_AUTO_UI, 0, 
	...
	acubic_bluroffset,	_T("blurOffset"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_BLUROFFS,
		...
		p_nonLocalizedName, _T("Blur Offset"),
		p_end,
	acubic_near,	_T("near"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_FALLNEAR,
		...
		p_dim, 			stdWorldDim,
		p_nonLocalizedName, _T("Near Value"),
		p_end,
	...
	acubic_far,		_T("far"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_FALLFAR,
		...
		p_dim, 			stdWorldDim,
		p_nonLocalizedName, _T("Far Value"),
		p_end,
	...
	acubic_useatmospheric,	_T("useAtmosphericMap"), TYPE_BOOL,			0,		IDS_PW_USEATMOSPHERIC,
		p_default,		TRUE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_USE_ENVMAP,
		p_accessor,		&acubic_accessor,
		p_end,
	acubic_applyblur,	_T("apply"), TYPE_BOOL,			0,		IDS_PW_APPLY,
		p_default,		TRUE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_ACUBE_BLUR,
		p_accessor,		&acubic_accessor,
		p_end,
	...
	acubic_bitmap_names,	_T("bitmapName"),   TYPE_FILENAME_TAB,		6,	0,	IDS_DS_BITMAP,
		p_enabled,		FALSE,
//		p_ui, 			TYPE_BITMAPBUTTON, IDC_FILE_UP,IDC_FILE_DN,IDC_FILE_LF,IDC_FILE_RT,IDC_FILE_FR,IDC_FILE_BK,
		p_assetTypeID,	kBitmapAsset,
		p_end,

	acubic_outputname,		_T("outputname"),   TYPE_FILENAME,			0,	IDS_PW_OUTPUTNAME,
		p_end,
...
void ACubicDlgProc::StuffDlgName(int i) {
	TSTR fname;
	const TSTR& fullName = theTex->files[i].GetFileName();
...
// mjm - end
		bmc->SetName(theTex->files[i].GetFileName());
	return bmc;
}

void ACubicDlgProc::Drop(ReferenceTarget *dropThis, HWND hwnd, POINT p, SClass_ID type, DADMgr*, BOOL) {
...
// mjm - end
	{
		DADBitmapCarrier *bmc = (DADBitmapCarrier *)dropThis;
		theTex->files[i] = IAssetManager::GetInstance()->GetAsset(bmc->GetName(),kBitmapAsset);
		AssetUser asset = theTex->files[i];
		theTex->pblock->SetValue(acubic_bitmap_names,0,asset,i);
		StuffDlgName(i);
	}
}
...
void ACubicDlgProc::BrowseInFile(int id) {
	...
	theTex->biInFile.SetAsset(theTex->files[iside]);
	BOOL silent = TheManager->SetSilentMode(TRUE);
...
//	int res = TheManager->SelectFileInputEx(&theTex->biInFile,hPanel,getCubeName(iside));
	TheManager->SetSilentMode(silent);
	if (res) {
		AssetUser fullAsset = theTex->biInFile.GetAsset();
		TSTR path;
		TSTR file;
		TSTR ext;
		if (theTex->files[iside].GetId()==fullAsset.GetId()) 
			goto done;
		theTex->files[iside] = fullAsset;
		theTex->pblock->SetValue(acubic_bitmap_names,0,theTex->files[iside],iside);
		StuffDlgName(iside);
		SplitFilename(fullAsset.GetFileName(),&path, &file, &ext);
		int n = file.Length();
		if (n>=3) {
			TSTR suf = file.Substr(n-3,3);
			suf.toUpper();
			if (_tcscmp(suffixes[iside],suf)==0) {
				...
				for (int i=0; i<6; i++) {
					...
					theTex->files[i] = IAssetManager::GetInstance()->GetAsset(fullName,kBitmapAsset);
					theTex->pblock->SetValue(acubic_bitmap_names,0,theTex->files[i],i);
					StuffDlgName(i);
					if (theTex->useFile) {
						theTex->FreeMaps();
	...
	theTex->biInFile.SetAsset(AssetUser());
	}



INT_PTR ACubicDlgProc::DlgProc(
...
MaxSDK::AssetManagement::AssetUser ACubicPathAccessor::GetAsset() const	{
	return mBlock->GetAssetUser(acubic_bitmap_names, 0, mTabIndex);
}

bool ACubicPathAccessor::SetAsset(const MaxSDK::AssetManagement::AssetUser& aNewAsset)	{
	mBlock->SetValue(acubic_bitmap_names, 0, aNewAsset, mTabIndex);
	return true;
}

void ACubic::EnumAuxFiles(AssetEnumCallback& nameEnum, DWORD flags)	{
	...
	if (useFile) {
		for (int i=0; i<6; i++) {
			if(flags & FILE_ENUM_ACCESSOR_INTERFACE)	{
				...
				biInFile.SetAsset(files[i]);
				biInFile.EnumAuxFiles(nameEnum,flags);
			}
			
		}
	}
	biInFile.SetAsset(AssetUser());
...
//	return dm;	
	...
	ip = imp;
	IAutoMParamDlg* dlg = acubicCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
//	pblock->SetValue(acubic_node,t,(INode *) NULL);
	if (!ivalid.InInterval(t)) {
		...
		for (int i = 0 ; i < 6; i++)
			{
			files[i] = pblock->GetAssetUser( acubic_bitmap_names,t,i);
			}	

		// > 10/30/02 - 10:36am --MQM-- 
		// be sure to update the UI.  this probably isn't the best fix...
		ACubicDlgProc *paramDlg =  (ACubicDlgProc*)acubic_param_blk.GetUserDlgProc();
		if ( paramDlg != NULL )  
		{
         for ( int i = 0;  i < 6;  i++ ) 
...
static void ACubicMsg(const TCHAR* msg) {
	if (GetCOREInterface()->GetQuietMode()) {
		...
		MaxSDK::MaxMessageBox(NULL, msg, GetString(IDS_DS_REFL_ERROR), MB_OK);
	}
...
//					maps->bitmap[i] = bp->bm;
				...
				if (maps->bitmap[i]!=NULL) {
					...
					if (w!=h) {
						ACubicNotSquareMsg(files[i].GetFileName());
//						ACubicNotSquareMsg(bp->bi.Name());
						FreeMaps();
						files[i].Reset();
						pblock->SetValue(acubic_bitmap_names,0,files[i],i);
						ACubicDlgProc *paramDlg =  (ACubicDlgProc*)acubic_param_blk.GetUserDlgProc();
						if (paramDlg)
							paramDlg->StuffDlgName(i);
						goto bail;
					...
					if (mapSize<0) mapSize = w; 
						...
						if (mapSize!=w) {
							ACubicWrongSizeMsg(files[i].GetFileName());
							FreeMaps();
							goto bail;
							}
						}
					files[i] = biInFile.GetAsset(); // DS 2/14/2000
...
TSTR ACubic::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_DS_PARAMETERS) : _T("Parameters");
	default: return _T("");
	}
}
...
int ACubic::WriteBM(Bitmap *bm, const TCHAR *name) {
	TSTR upname = biOutFile.Name();

	MaxSDK::Util::Path outpath(name);
	outpath.ConvertToAbsolute();

	biOutFile.SetName(outpath.GetCStr());
...
{
	// Disable bitmap proxies for this operation
	BitmapProxyTemporaryDisableGuard bitmapProxyDisableGuard;
...
fail:
	...
	if (success) {
		for (int i=0; i<6; i++) {
			...
			files[i] = IAssetManager::GetInstance()->GetAsset(fullname,kBitmapAsset); 


			biOutFile.SetAsset(files[i]);
			pblock->SetValue( acubic_bitmap_names,ip->GetTime(), files[i] ,i);
			ACubicDlgProc *paramDlg =  (ACubicDlgProc*)acubic_param_blk.GetUserDlgProc();
			if (paramDlg) paramDlg->StuffDlgName(i);
		}
		biOutFile.SetAsset(AssetUser());
		if (useFile) {
			FreeMaps();
			NotifyChanged();
		}
...
static constexpr int nameID[] = {IDS_DS_MAPSIZE,
                       IDS_DS_BLUR,
                       IDS_DS_BLUROFFS,
                       IDS_DS_NEARRANGE,
                       IDS_DS_FARRANGE};

static constexpr const TCHAR* nonlocalizedName[] = {_T("Map Size"),
                                    _T("Blur"),
                                    _T("Blur Offset"),
                                    _T("Near Range"),
                                    _T("Far Range")};

RefResult ACubic::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
   PartID& partID, RefMessage message, BOOL propagate ) {
	switch (message) {
		case REFMSG_CHANGE:
			if (hTarget == pblock)
...
CubicMap *ACubic::FindMap(int nodeNum) {
   for (CubicMap* cm=maps; cm!=NULL; cm = cm->next)
...
int ACubic::BuildMaps(TimeValue t, RenderMapsContext &rmc) {
	...
	if (cm->AllocMaps(size)) {
		...
		for (int i=0; i<6; i++) {	
			...
			if ( gc && !gc->inMtlEdit )
				BroadcastNotification<NOTIFY_BEGIN_RENDERING_REFLECT_REFRACT_MAP>(gc);
			...
			MaxSDK::MaxMessageBox(NULL, getCubeName(i), _T(" Auto Cubic Test"), MB_OK | MB_ICONEXCLAMATION);
				...
				switch (cube->pblock->GetVersion()) {
					...
					default:
						if (Param1)
						{
							...
							for (int i = 0 ; i < 6; i++)
							{
								cube->pblock->SetValue( acubic_bitmap_names,t,cube->files[i],i);
							}	
		...
		switch(cid=iload->CurChunkID())  {
			...
			case NAME_CHUNK+5:
				...
				if (IO_OK==iload->ReadWStringChunk(&buf)) 
					files[cid-NAME_CHUNK] = IAssetManager::GetInstance()->GetAsset(buf,kBitmapAsset);
				}
				break;
