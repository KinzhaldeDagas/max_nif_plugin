...
// JBW: use an IParamBlock2
		...
		void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_RB_GRADIENT) : _T("Gradient"); }
...
public:
	...
	const TCHAR*	ClassName() {return GetString(IDS_RB_GRADIENT_CDESC); }
	const TCHAR*	NonLocalizedClassName() { return _T("Gradient"); }
	SClass_ID		SuperClassID() {return TEXMAP_CLASS_ID;}
	Class_ID 		ClassID() {return gradClassID;}
...
static ParamBlockDesc2 grad_param_blk ( grad_params, _T("parameters"),  0, &gradCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF, 
	...
	grad_center,	_T("color2Pos"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_RB_CENTER2,
		...
		p_nonLocalizedName, _T("Color 2 Position"),
		p_end,
	...
	grad_amount,	_T("noiseAmount"), TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_NOISEAMT,
		...
		p_nonLocalizedName, _T("Noise Amount"),
		p_end,
	...
	grad_phase,		_T("noisePhase"), TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_NSPHS,
		...
		p_nonLocalizedName, _T("Noise Phase"),
		p_end,
	...
	grad_thresh_smooth, _T("noiseThresholdSMooth"), TYPE_FLOAT,	P_ANIMATABLE,	IDS_RB_THRESHOLDSMOOTHING,
		...
		p_nonLocalizedName, _T("Threshold Smoothing"),
		p_end,
...
static ParamBlockDescID pbdesc3[] = {	
	...
	{ TYPE_FLOAT, NULL, TRUE, grad_phase }, 	// phase
	{ TYPE_FLOAT, NULL, TRUE, grad_center }, 	// center
	{ TYPE_INT, NULL, FALSE, static_cast<DWORD>(-1) }, 				// turbulence
...
AColor Gradient::EvalFunction(
	...
	if (a>center) {
		...
		return (mapOn[1]&&subTex[1]) ? subTex[1]->EvalColor(sc): AColor(col[1]);
		}

	Color c1, c2;
	c1 = mapOn[n1]&&subTex[n1] ? Color(subTex[n1]->EvalColor(sc)): col[n1];
	c2 = mapOn[n2]&&subTex[n2] ? Color(subTex[n2]->EvalColor(sc)): col[n2];
...
// Gradient main AutoMParamDlg so it will keep them up-to-date automatically
	...
	IAutoMParamDlg* dlg = gradCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
void Gradient::Update(TimeValue t, Interval& valid) 
	...
	if (!mapValid.InInterval(t))
	{
		...
		uvGen->Update(t,mapValid);
		texout->Update(t,mapValid);
		for (int i=0; i<NSUBTEX; i++) {
			if (subTex[i]) 
...
TSTR Gradient::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_RB_COLOR1) : _T("Color 1");
	case 1: return localized ? GetString(IDS_RB_COLOR2) : _T("Color 2");
	case 2: return localized ? GetString(IDS_RB_COLOR3) : _T("Color 3");
...
TSTR Gradient::SubAnimName(int i, bool localized)
{
	switch (i)
	{
...
RefResult Gradient::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
   PartID& partID, RefMessage message, BOOL propagate ) 
{
	switch (message) 
	{
