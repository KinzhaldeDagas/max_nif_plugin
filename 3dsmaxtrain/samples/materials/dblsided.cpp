...
class DoubleSided : public Mtl, public IReshading, public GeneralMaterialViewportShading, public IDX9DataBridge {	
	public:
		BOOL Param1;
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_RB_DOUBLESIDED) : _T("Double Sided");}  
		...
		TSTR GetSubMtlSlotName(int i, bool localized) override
		{
			if (localized)
			{
				return i ? GetString(IDS_RB_BACK) : GetString(IDS_RB_FACING);
			}
			else
			{
				return i ? _T("Back") : _T("Facing");
			}
		}
...
class DoubleSidedClassDesc:public ClassDesc2 {
	...
	const TCHAR*	ClassName() {return GetString(IDS_RB_DOUBLESIDED_CDESC); }
	const TCHAR*	NonLocalizedClassName() { return _T("Double Sided"); }
	SClass_ID		SuperClassID() {return MATERIAL_CLASS_ID;}
	Class_ID 		ClassID() {return dblsidedClassID;}
...
{	
	...
	dxDoubleSided = new DxStdMtl2(this);
	GeneralMaterialViewportShading::SetMaterial(this);
}

DoubleSided::~DoubleSided()
{
	delete dxDoubleSided;
}

void DoubleSided::Init()
	{
	ReplaceReference(SUB1_REF, NewDefaultMaterial(_T("DoubleSidedMtl")));
	ReplaceReference(SUB2_REF, NewDefaultMaterial(_T("DoubleSidedMtl")));
...
BOOL DoubleSided::GetTransparencyHint(TimeValue t, Interval& valid)
{
	if (UseMtl())
	{
		if ((sub[0] && sub[0]->GetTransparencyHint(t, valid))
			|| (sub[1] && sub[1]->GetTransparencyHint(t, valid)))
		{
			return TRUE;
		}
	}
...
ParamDlg* DoubleSided::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp)
	{
	IAutoMParamDlg* dlg = dblsidedCD.CreateParamDlgs(hwMtlEdit, imp, this);
	return dlg;
	}
...
*/
	...
	{
		ivalid.SetInfinite();
		//use valid not ivalid because we don't want sub material dirty our interval state
		for (int i=0; i<2; i++) 
...
TSTR DoubleSided::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_DS_PARAMETERS) : _T("Parameters");
	case 1: return GetSubMtlTVName(0, localized); // GetString(IDS_RB_FACINGMAT);
	case 2: return GetSubMtlTVName(1, localized); // GetString(IDS_RB_BACKMATERIAL);
	default: return _T("");
	}
}
...
BaseInterface *DoubleSided::GetInterface(Interface_ID id)
{
	...
	if (id == VIEWPORT_SHADER_CLIENT_INTERFACE) {
		...
		return static_cast<IDX9VertexShader *>(dxDoubleSided);
	}
	else if(id==EFFECT_FILE_INTERFACE){
		return static_cast<IEffectFile*>(dxDoubleSided);
	}
	else if (id == IID_MATERIAL_VIEWPORT_SHADING)
		return (GeneralMaterialViewportShading*)( this );
else{
