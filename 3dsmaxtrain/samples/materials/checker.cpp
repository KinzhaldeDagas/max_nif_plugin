...
class Checker: public Texmap { 
	...
	public:
		...
		void SetColor(ParamID pid, Color c, TimeValue t);
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_CHECKER) : _T("Checker"); }  
...
class CheckerClassDesc:public ClassDesc2 {
	...
	const TCHAR*	ClassName() { return GetString(IDS_DS_CHECKER_CDESC); } 
	const TCHAR*	NonLocalizedClassName() { return _T("Checker"); }
	SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
	Class_ID 		ClassID() { return checkerClassID; }
...
static ParamBlockDesc2 checker_param_blk ( checker_params, _T("parameters"),  0, &checkerCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF, 
	...
	checker_map1_on,	_T("map1Enabled"), TYPE_BOOL,			0,				IDS_JW_MAP1ENABLE,
		p_default,		TRUE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_CHKMAP1,
		p_end,
	checker_map2_on,	_T("map2Enabled"), TYPE_BOOL,			0,				IDS_JW_MAP2ENABLE,
		p_default,		TRUE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_CHKMAP2,
		p_end,
...
void Checker::Init() {
	...
	ivalid.SetEmpty();
	mapValid.SetEmpty();
	col[0] = pblock->GetColor(checker_color1);
	col[1] = pblock->GetColor(checker_color2);
	blur = pblock->GetFloat(checker_blur);
	mapOn[0] = mapOn[1] = 1;
	}
...
Bitmap *Checker::BuildBitmap(int size) {
	...
	if (bitMapAssetUser.GetId() == MaxSDK::AssetManagement::kInvalidId)
		bitMapAssetUser = MaxSDK::AssetManagement::IAssetManager::GetInstance()->GetAsset(_T("checkerTemp"), MaxSDK::AssetManagement::kBitmapAsset);
...
AColor Checker::EvalFunction(ShadeContext& sc, float u, float v, float du, float dv) {
	...
	if (a<=.0005f) 
		return mapOn[0]&&subTex[0] ? subTex[0]->EvalColor(sc): AColor(col[0]);
	else if (a>=.9995f)
		return mapOn[1]&&subTex[1] ? subTex[1]->EvalColor(sc): AColor(col[1]);
	else {
		AColor c0  = mapOn[0]&&subTex[0] ? subTex[0]->EvalColor(sc): AColor(col[0]);
		AColor c1  = mapOn[1]&&subTex[1] ? subTex[1]->EvalColor(sc): AColor(col[1]);
...
// up-to-date and deleted as necessary.  
	...
	IAutoMParamDlg* dlg = checkerCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
void Checker::SetColor(ParamID pid, Color c, TimeValue t)
{
	if (pid == checker_color1)
		col[0] = c;
	else
		col[1] = c;
	
	pblock->SetValue( pid, t, c);
}
...
TSTR Checker::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
...
TSTR Checker::SubAnimName(int i, bool localized)
{
	switch (i)
	{
...
RefResult Checker::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
   PartID& partID, RefMessage message, BOOL propagate ) {
	switch (message) {
		case REFMSG_CHANGE:
			{
