...
//	NoiseDlg *paramDlg;
	float Turb(Point3 p, float lev) const;
	public:
		Noise();
		~Noise() { 
#ifdef SHOW_3DMAPS_WITH_2D
		...
		float NoiseFunction(Point3 p,  float limLev, float smWidth) const;
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_RB_NOISE) : _T("Noise"); }  
...
class NoiseClassDesc:public ClassDesc2 {
	...
	const TCHAR*	ClassName() { return GetString(IDS_RB_NOISE_CDESC); }
	const TCHAR*	NonLocalizedClassName() { return _T("Noise"); }
	SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
	Class_ID 		ClassID() { return noiseClassID; }
...
static ParamBlockDesc2 noise_param_blk ( noise_params, _T("parameters"),  0, &noiseCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF, 
	...
	noise_size,	_T("size"),   TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_NOISESIZE,
		...
		p_nonLocalizedName, _T("Noise Size"),
		p_end,
	...
	noise_levels,	_T("levels"), TYPE_FLOAT,			P_ANIMATABLE,	IDS_DS_NSLEV,
		...
		p_nonLocalizedName, _T("Noise Levels"),
		p_end,
	noise_lowthresh, _T("thresholdLow"), TYPE_FLOAT,	P_ANIMATABLE,	IDS_RB_LOWTHRESH,
		...
		p_nonLocalizedName, _T("Low Threshold"),
		p_end,
	noise_hithresh, _T("thresholdHigh"), TYPE_FLOAT,	P_ANIMATABLE,	IDS_RB_HIGHTHRESH,
		...
		p_accessor,		&noise_accessor,
		p_nonLocalizedName, _T("High Threshold"),
		p_end,
...
// up-to-date and deleted as necessary.  
	...
	IAutoMParamDlg* dlg = noiseCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
void Noise::ReadSXPData(const TCHAR *name, void *sxpdata) {
...
void Noise::ComputeAvgValue()
{
	...
	std::array<float, NAVG> to_sum;
	tbb::parallel_for(
			tbb::blocked_range<int>{ 0, NAVG }, [&to_sum, this](const tbb::blocked_range<int>& range) {
		for (int i = range.begin(); i < range.end(); ++i)
		{
			...
			to_sum[i] = NoiseFunction(p, levels, 0.0f);
		}
	});

	{
		float sum = 0.0f;
		for (float val : to_sum)
		{
			sum += val;
		}
	...
	tbb::parallel_for(tbb::blocked_range<int>{ 0, NAVG }, [&to_sum](const tbb::blocked_range<int>& range) {
		for (int i = range.begin(); i < range.end(); ++i)
		{
			Point3 p;
			p.x = float(rand()) / 100.0f;
			p.y = float(rand()) / 100.0f;
			p.z = float(rand()) / 100.0f;
			float phase = float(rand()) / 100.0f;
			to_sum[i] = fabs(noise4(p, phase));
		}
	});
...
TSTR Noise::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_DS_COLOR1) : _T("Color 1");
	case 1: return localized ? GetString(IDS_DS_COLOR2) : _T("Color 2");
...
TSTR Noise::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case XYZGEN_REF: return localized ? GetString(IDS_DS_COORDINATES) : _T("Coordinates");
	case PBLOCK_REF: return localized ? GetString(IDS_DS_PARAMETERS) : _T("Parameters");
	case TEXOUT_REF: return localized ? GetString(IDS_DS_OUTPUT) : _T("Output");
...
RefResult Noise::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
   PartID& partID, RefMessage message, BOOL propagate ) {
	switch (message) {
		case REFMSG_CHANGE:
			ivalid.SetEmpty();
...
float Noise::Turb(Point3 p, float lev) const
{
	float sum = 0.0f;
	float l, f = 1.0f;
	float ml = levels;
...
float Noise::NoiseFunction(Point3 p, float limitLev, float smWidth) const
{
	float res(0.0f);
	float lev = limitLev;
	if (limitLev < (1.0f - BLENDBAND))
		return avgValue;
