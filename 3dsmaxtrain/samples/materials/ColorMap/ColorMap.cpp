...
class ColorMapClassDesc: public ClassDesc2
 {
	...
	const TCHAR*			NonLocalizedClassName() { return _T("ColorMap"); }
...
static ParamBlockDesc2 colormap_param_blk (gnormal_params, _T("params"),  0, &ColorMapDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF, 
	...
	// params
		colormap_solid_color,	 _T("solidcolor"),	TYPE_FRGBA,	P_ANIMATABLE,	IDS_SOLID_COLOR,	
			...
			p_nonLocalizedName, _T("Solid Color"),
		p_end,
		...
		colormap_reverse_gamma,	_T("ReverseGamma"), TYPE_BOOL, P_ANIMATABLE,	IDS_REVERSE_GAMMA,
			...
			p_nonLocalizedName, _T("Reverse Gamma"),
		p_end,
	p_end
);
...
void ColorMap::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_CLASS_NAME_IMP) : _T("Color Map");
}
...
TSTR ColorMap::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_PARAMS) : _T("Parameters");
	default: return GetSubTexmapTVName(i, localized);
	}
}
...
{
	IAutoMParamDlg* dlg = ColorMapDesc.CreateParamDlgs(hwMtlEdit, imp, this);
	return dlg;	
}
...
TSTR ColorMap::GetSubTexmapSlotName(int i, bool localized)
{
