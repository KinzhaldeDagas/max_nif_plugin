...
class CMtl: public Mtl, public IReshading {
	...
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_TOPBOTTOM) : _T("Top/Bottom"); }  
		void Fixup();  //TEMPORARY
...
class CMtlClassDesc:public ClassDesc2 {
	...
	const TCHAR*	ClassName() { return  GetString(IDS_DS_TOPBOTTOM_CDESC); }
	const TCHAR*	NonLocalizedClassName() { return _T("Top/Bottom"); }
	SClass_ID		SuperClassID() { return MATERIAL_CLASS_ID; }
	Class_ID 		ClassID() { return cmtlClassID; }
...
void CMtl::Init() {
	ivalid.SetEmpty();
	mapValid.SetEmpty();
	ReplaceReference(1, NewDefaultMaterial(_T("CompositeMtl")));
	ReplaceReference(2, NewDefaultMaterial(_T("CompositeMtl")));
...
ParamDlg* CMtl::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp) {
	ip = imp;
	IAutoMParamDlg* dlg = cMtlCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
TSTR CMtl::GetSubMtlSlotName(int i, bool localized) {
	switch(i) {
		case 0:  return localized ? GetString(IDS_DS_TOP) : _T("Top"); 
		case 1:  return localized ? GetString(IDS_DS_BOTTOM) : _T("Bottom"); 
		default: return _T("");
		}
	}
...
TSTR CMtl::SubAnimName(int i, bool localized)
{
	if (i == 0)
		return localized ? GetString(IDS_DS_PARAMETERS) : _T("Parameters");
	else
		return GetSubMtlTVName(i - 1, localized);
}

RefResult CMtl::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
   PartID& partID, RefMessage message, BOOL propagate ) {
	switch (message) {
		case REFMSG_CHANGE:
			ivalid.SetEmpty();
