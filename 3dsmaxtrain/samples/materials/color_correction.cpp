...
#include "Graphics/ISimpleMaterial.h"
...
inline AColor clampMin( const AColor& c, const AColor& minVal ) {
...
namespace HSL {
   ...
   float CalcHue( const Color& c ) {
      const float min_ = std::min({c.r,c.g,c.b});
      const float max_ = std::max({c.r,c.g,c.b});
   ...
   float CalcLum( const Color& c ) {
      return clampMin((std::max({c.r,c.g,c.b}) + std::min({c.r,c.g,c.b})) / 2, 0 );
   }
   ...
   namespace Rewire {
      ...
      template< typename SubType, int ResourceID >
         struct RewireHelper : RewireBase {
            ...
            _tstring Name() const override { return _tstring( GetString( ResourceID ) ); }
         };
      ...
      struct RewireListType {
         ...
         size_t         size()  const { return _countof(RewireArray); }
      private:
      } RewireList;
   }
}
   ...
   namespace Lightness {
      ...
      template< typename BPHelper >
         ...
         public:
			 ...
			 {}
            Advanced(float gain, float gamma, float pivot, float lift, int exp, float printer ) 
               : m_Gain    (gain)
               , m_Gamma   (gamma)
               , m_Pivot   (pivot)
               , m_Lift    (lift)
...
namespace ColorCorrection { namespace {
   ...
   ParamBlockDesc2 ParamBlock ( 
      ...
      BlockParam::RewireMode, _T("rewireMode"),       // ID, int_name
         ...
         p_nonLocalizedName, _T("Rewire Mode"),
      p_end,
      ...
      BlockParam::HueTint,    _T("tint"),
		 ...
		 p_default,			AColor(0,0,0,1),
         p_nonLocalizedName, _T("Hue Tint"),
      p_end,
      BlockParam::HueTintStrength, _T("tintStrength"),
         ...
         p_nonLocalizedName, _T("Hue Strength"),
      p_end,
      ...
      BlockParam::id, name, TYPE_FLOAT, P_ANIMATABLE, IDS_DS_LIGHTNESS_##constant, \
      ...
      DECLARE_HELPER__( Contrast,   _T("contrast"),    CONTRAST  )
      DECLARE_HELPER__( Brightness, _T("brightness"),  BRIGHTNESS )
      ...
      // Helper for easing declaration
      #undef DECLARE_HELPER__
      #define DECLARE_HELPER__( id, name, constant, default, min, max, step, nonLocalizedName ) \
      BlockParam::id, name, TYPE_FLOAT, P_ANIMATABLE, IDS_DS_LIGHTNESS_##constant, \
         p_ui,          BlockParam::LightnessStandardRollout, \
         ...
         p_accessor,   &LightnessAccessor, \
         p_tooltip,     IDS_CC_TOOLTIP_##constant, \
         p_nonLocalizedName, _T( #nonLocalizedName ), \
      p_end,

      DECLARE_HELPER__( RGBGain,   _T("gainRGB"),   GAIN_RGB,   100.0f, -1000.00f,   1000.0f, 0.10f, LightnessGainRGB )
      DECLARE_HELPER__( RGain,     _T("gainR"),     GAIN_R,     100.0f, -1000.00f,   1000.0f, 0.10f, LightnessGainR )
      DECLARE_HELPER__( GGain,     _T("gainG"),     GAIN_G,     100.0f, -1000.00f,   1000.0f, 0.10f, LightnessGainG )
      DECLARE_HELPER__( BGain,     _T("gainB"),     GAIN_B,     100.0f, -1000.00f,   1000.0f, 0.10f, LightnessGainB )

      DECLARE_HELPER__( RGBGamma,  _T("gammaRGB"),  GAMMA_RGB,    1.0f,     0.01f, 100000.0f, 0.01f, LightnessGammaRGB )
      DECLARE_HELPER__( RGamma,    _T("gammaR"),    GAMMA_R,      1.0f,     0.01f, 100000.0f, 0.01f, LightnessGammaR )
      DECLARE_HELPER__( GGamma,    _T("gammaG"),    GAMMA_G,      1.0f,     0.01f, 100000.0f, 0.01f, LightnessGammaG )
      DECLARE_HELPER__( BGamma,    _T("gammaB"),    GAMMA_B,      1.0f,     0.01f, 100000.0f, 0.01f, LightnessGammaB )

      DECLARE_HELPER__( RGBPivot,  _T("pivotRGB"),  PIVOT_RGB,    1.0f,     0.01f, 100000.0f, 0.01f, LightnessPivotRGB )
      DECLARE_HELPER__( RPivot,    _T("pivotR"),    PIVOT_R,      1.0f,     0.01f, 100000.0f, 0.01f, LightnessPivotR )
      DECLARE_HELPER__( GPivot,    _T("pivotG"),    PIVOT_G,      1.0f,     0.01f, 100000.0f, 0.01f, LightnessPivotG )
      DECLARE_HELPER__( BPivot,    _T("pivotB"),    PIVOT_B,      1.0f,     0.01f, 100000.0f, 0.01f, LightnessPivotB )

      DECLARE_HELPER__( RGBLift,   _T("liftRGB"),   LIFT_RGB,     0.0f,    -2.00f,      2.0f, 0.01f, LightnessLiftRGB )
      DECLARE_HELPER__( RLift,     _T("liftR"),     LIFT_R,       0.0f,    -2.00f,      2.0f, 0.01f, LightnessLiftR )
      DECLARE_HELPER__( GLift,     _T("liftG"),     LIFT_G,       0.0f,    -2.00f,      2.0f, 0.01f, LightnessLiftG )
      DECLARE_HELPER__( BLift,     _T("liftB"),     LIFT_B,       0.0f,    -2.00f,      2.0f, 0.01f, LightnessLiftB )

      DECLARE_HELPER__( Printer,   _T("printerLights"), PRINT,    5.0f,   -40.00f,   6000.0f, 0.01f, LightnessPrinter )
...
namespace ColorCorrection {
	...
	public:
		...
		// MtlBase virtual methods
		virtual ParamDlg* CreateParamDlg(HWND editor_wnd, IMtlParams *imp);

		// Texmap virtual methods
		virtual AColor EvalColor(ShadeContext& sc);
		virtual AColor Correct( const AColor& c, TimeValue t );
		...
		void        GetClassName(MSTR& s, bool localized) const override { s = localized ? GetColorCorrectionDesc()->ClassName() : GetColorCorrectionDesc()->NonLocalizedClassName(); }
		void        DeleteThis();
	...
	public:
		...
		void InitMultimap      (TimeValue  time, MtlMakerCallback &callback );
		void PrepareHWMultimap (TimeValue  time, IHardwareMaterial *hw_material, ::Material *material, MtlMakerCallback &callback  );
		void PrepareSWMultimap (TimeValue  time, ::Material *material, MtlMakerCallback &callback  );
	...
	private:
		...
		bool CreateViewportBitmap(PBITMAPINFO& bmi, TimeValue time, TexHandleMaker &callback, Interval& validInterval);
		void ResetDisplayViewportTex();
		...
		// for multimap display
		TexHandle*            m_DisplayTexHandle;
		Interval              m_DisplayTexInterval;
		RewireData            mRewireData;
		bool                  mChangingRewireMode; // true when changing values because of a change of wiring mode
		MaxSDK::ColorManagement::ColorPipelineMode mColorCorrectionMode;
		AColor				m_DefaultColor;
		float				m_HueShift;
		float				m_Saturation;
		AColor				m_HueTint;
		float				m_HueTintStrength;
		int					m_LightnessMode;
		float				m_Brightness;
		float				m_Contrast;
		float				m_Gain;
...
namespace ColorCorrection {
	...
	class ColorProc : public ParamMap2UserDlgProc
	{
		...
		void DrawSpectrum( HWND ctrl, float h = -180, float s = 1.0f, float l = 0.5f )
		{
			...
			Color hsl( h, s, std::clamp(l,0.0f,1.0f) );
			Color rgb;
			...
			for( int i = 0; i < width; i++, hsl[0] += inc ) {
				...
				int   r  (std::clamp( int(rgb[0] * 255.0f), 0, 255 ) );
				int   g  (std::clamp( int(rgb[1] * 255.0f), 0, 255 ) );
				int   b  (std::clamp( int(rgb[2] * 255.0f), 0, 255 ) );
				::SetPixelV( memdc, i, 0, RGB(r, g, b) );
			}
	...
	public:
		ColorProc( IMtlParams *imp )
			...
			, m_OldStaticWndProc(nullptr)
			, m_OldDynamicWndProc(nullptr)
			, m_hWnd(nullptr)
		{
		}
	...
	private:
		IMtlParams*     m_MtlParam;
		IParamBlock2*   m_ParamBlock; //< The parameter block that owns this instance
		WNDPROC         m_OldStaticWndProc;
		WNDPROC         m_OldDynamicWndProc;
		HWND            m_hWnd;
	};

	void ColorProc::DeleteThis() 
	{
		delete this;
	}
	...
	class LightnessProc : public ParamMap2UserDlgProc
	{
		...
		void DrawSpectrum( HWND ctrl, const Pred& pred )
		{
			...
			for( int i = 0; i < width; i++, val += inc ) {
				float rgb( pred(val) );
				int   x  (std::clamp( int(rgb * 255.0f), 0, 255 ) );
	...
	public:
		...
		//this toggles our advance or standard Light UI.  It used to be that this was separated into 
		//2 separate rollups but the param map swapping code was problematic, so I just show/hide 
		//ui instead to make it simpler and more stable.
...
int            ColorCorrection::Desc::IsPublic()     { return TRUE; }
...
const TCHAR* ColorCorrection::Desc::NonLocalizedClassName() { return _T("Color Correction"); }
...
ParamDlg* ColorCorrection::Texture::CreateParamDlg(HWND editor_wnd, IMtlParams *imp) 
{
	...
	ClassDesc2* cd = GetColorCorrectionDesc();
	ParamDlg* main = cd->CreateParamDlg( ID, editor_wnd, imp, this, TextureRollout );
					 cd->CreateParamDlg( ID, editor_wnd, imp, this, ChannelRollout );
					 cd->CreateParamDlg( ID, editor_wnd, imp, this, ColorRollout );
					 cd->CreateParamDlg( ID, editor_wnd, imp, this, LightnessStandardRollout );

	cd->SetUserDlgProc( &ParamBlock, ColorRollout,   new ColorProc(imp) );
	cd->SetUserDlgProc( &ParamBlock, TextureRollout, new TextureProc()  );
		LightnessProc* light_proc = new LightnessProc( editor_wnd, imp, this );
		int lightMode = m_ParamBlock->GetInt( LightnessMode, imp->GetTime() );
		light_proc->SetLightUI(lightMode);
	cd->SetUserDlgProc( &ParamBlock, LightnessStandardRollout, light_proc);

	m_LightProc = light_proc;
...
RefResult ColorCorrection::Texture::NotifyRefChanged( 
...
ColorCorrection::Texture::~Texture()
{
	...
	if (m_DisplayTexHandle)
	{
		m_DisplayTexHandle->DeleteThis();
		m_DisplayTexHandle = NULL;
	}
}
...
AColor ColorCorrection::Texture::Correct( const AColor& c, TimeValue t )
{
   ...
   if (hsl[1] > 0.0) // Only do anything for saturations above Zero
   {
      // Hue Shift
      hsl[0] = rotate( hsl[0] + m_HueShift, 0.0f, 360.0f );
      hsl[1] = clampMin ( hsl[1] *m_Saturation,0.0f );
   }

   // Tint calculation
   AColor tint     = m_HueTint;
   ...
   if (hue_tint >= 0.0f && hue_tint <= 360.0f)
      hsl[0] = (hsl[0] + (hue_tint - hsl[0]) * m_HueTintStrength);

   res = AColor( HSLtoRGB( hsl ), res.a );

   // Perform Lightness variation
   // Standard?
   int mode = m_LightnessMode;
   if (0 == mode) 
   {
      res = AColor( Transform( res, Standard( m_Brightness, m_Contrast) ), res.a );
   }
   else 
   {
      res = AColor( Transform( res, Advanced<RGB>(m_AdvanceRGB[AdvanceIndex::gain],m_AdvanceRGB[AdvanceIndex::gamma],
		  m_AdvanceRGB[AdvanceIndex::pivot],m_AdvanceRGB[AdvanceIndex::lift],m_Exp,m_Printer) ), res.a );

      if (FALSE != m_EnableRGB[AdvanceEnable::R]) {
         res.r = Advanced<R>( m_AdvanceR[AdvanceIndex::gain],m_AdvanceR[AdvanceIndex::gamma],m_AdvanceR[AdvanceIndex::pivot],
			 m_AdvanceR[AdvanceIndex::lift],m_Exp,m_Printer )( res.r );
      }
      if (FALSE != m_EnableRGB[AdvanceEnable::G]) {
         res.g = Advanced<G>( m_AdvanceG[AdvanceIndex::gain],m_AdvanceG[AdvanceIndex::gamma],m_AdvanceG[AdvanceIndex::pivot],
			 m_AdvanceG[AdvanceIndex::lift],m_Exp,m_Printer )( res.g );
      }
      if (FALSE != m_EnableRGB[AdvanceEnable::B]) {
         res.b = Advanced<B>( m_AdvanceB[AdvanceIndex::gain],m_AdvanceB[AdvanceIndex::gamma],m_AdvanceB[AdvanceIndex::pivot],
			 m_AdvanceB[AdvanceIndex::lift],m_Exp,m_Printer )( res.b );
      }
   }
...
void ColorCorrection::Texture::ResetDisplayViewportTex()
{
	m_DisplayTexInterval.SetEmpty();
	if (m_DisplayTexHandle)
	{
		m_DisplayTexHandle->DeleteThis();
		m_DisplayTexHandle = NULL;
...
BITMAPINFO* ColorCorrection::Texture::GetVPDisplayDIB(TimeValue t, TexHandleMaker& thmaker, Interval &valid, BOOL mono, BOOL forceW, BOOL forceH)
{
	if(!m_Map)
	{
		return __super::GetVPDisplayDIB(t, thmaker, valid, mono, forceW, forceH);
	}

	BITMAPINFO * bmi = NULL;
	// Set the interval and initialize the map.
	ResetDisplayViewportTex();
	m_DisplayTexInterval.SetInfinite(); 
	
	Interval mapInterval;
	mapInterval.SetEmpty();
	if (CreateViewportBitmap(bmi, t, thmaker, mapInterval))
	{
		m_DisplayTexInterval = m_DisplayTexInterval & mapInterval;
	
	}	
	
	valid = m_DisplayTexInterval;

	return bmi;
}

bool ColorCorrection::Texture::CreateViewportBitmap(PBITMAPINFO& bmi, TimeValue time, TexHandleMaker &callback, Interval& validInterval)
{
	if (m_Map) 
	{
		bmi = m_Map->GetVPDisplayDIB( time, callback, validInterval );
		if (!bmi)
		{
			DebugPrint(_T("Texture::InitMultimap Exiting early, m_Map->GetVPDisplayDIB returned NULL\n"));
			return false;
		}
		...
		for( int y = 0; y < h; ++y ) {
			for( int x = 0; x < w; ++x, data += 4 ) {
				...
				data[2] = (unsigned char)(std::clamp<float>(res.r * 0xFF, 0, 0xFF ));
				data[1] = (unsigned char)(std::clamp<float>(res.g * 0xFF, 0, 0xFF ));
				data[0] = (unsigned char)(std::clamp<float>(res.b * 0xFF, 0, 0xFF ));
				data[3] = (unsigned char)(std::clamp<float>(res.a * 0xFF, 0, 0xFF ));
			}
		}
	} 
	else 
	{
		...
		AColor defaultColor;
		if (!m_ParamBlock->GetValue( BlockParam::DefaultColor, time, defaultColor, validInterval, 0))
		{
			defaultColor.r = defaultColor.g = defaultColor.b = 0.0f; //set to black.
		}

		BMM_Color_fl color( Correct( defaultColor, time ) );
                                      ...
                                      MtlMakerCallback &callback )
{
	...
	mapInterval.SetEmpty();
	if (CreateViewportBitmap(bmi, time, callback, mapInterval))
	{
		m_ViewportIval = m_ViewportIval & mapInterval;
                                                ...
                                                MtlMakerCallback &callback )
{
	if (m_ViewportHandle)
	{
		material->texture.setLengthUsed( 1 );
		callback.GetGfxTexInfoFromTexmap( time, material->texture[0], m_Map );
...
void ColorCorrection::Texture::SetupTextures(TimeValue t, DisplayTextureHelper &cb)
{
	...
	// Are we still valid?  If not, initialize...
	if ( !m_DisplayTexInterval.InInterval(t) || colorCorrectionModeChanged)
	{
		// Set the interval and initialize the map.
		ResetDisplayViewportTex();
		m_DisplayTexInterval.SetInfinite(); 
		...
		Interval mapInterval;
		mapInterval.SetEmpty();
		if (CreateViewportBitmap(bmi, t, cb, mapInterval))
		{
			m_DisplayTexInterval = m_DisplayTexInterval & mapInterval;
			m_DisplayTexHandle = cb.MakeHandle( bmi );
		}	
	}
	...
	if (m_DisplayTexHandle)
	{
		cb.UpdateTextureMapInfo( t, ISimpleMaterial::UsageDiffuse, m_Map );
		pISimpleMtl->SetTexture(m_DisplayTexHandle, ISimpleMaterial::UsageDiffuse); 
	}
}

BaseInterface* ColorCorrection::Texture::GetInterface(Interface_ID id)
{
...
void ColorCorrection::Texture::Update( TimeValue t, Interval& valid )
{
	...
	if (!m_mapValidity.InInterval(t))
	{
		...
		if (m_Map)
			m_Map->Update( t, m_mapValidity );
	}


	//cache the values used in EvalColor

	m_DefaultColor = m_ParamBlock->GetAColor(BlockParam::DefaultColor,t);
	m_HueShift = m_ParamBlock->GetFloat(BlockParam::HueShift, t);
	m_Saturation = m_ParamBlock->GetFloat(BlockParam::Saturation,t );

	//! Saturation is only used in one place where it is modified - performing the modification here just once

	m_Saturation = 1.0f + m_Saturation/100.0f;

	m_HueTint = m_ParamBlock->GetAColor(BlockParam::HueTint,t);
	m_HueTintStrength = m_ParamBlock->GetFloat(BlockParam::HueTintStrength,t)/100.0f;
	m_LightnessMode = m_ParamBlock->GetInt(BlockParam::LightnessMode,t);
	m_Brightness = m_ParamBlock->GetFloat(BlockParam::Brightness,t)/100.0f;
	m_Contrast = m_ParamBlock->GetFloat(BlockParam::Contrast,t)/100.0f;

	m_Exp =  m_ParamBlock->GetInt( BlockParam::ExposureMode, t );
	m_Printer =  m_ParamBlock->GetFloat(BlockParam::Printer, t );

	m_EnableRGB[AdvanceEnable::R] = m_ParamBlock->GetInt(BlockParam::EnableR,t);
	m_EnableRGB[AdvanceEnable::G] = m_ParamBlock->GetInt(BlockParam::EnableG,t);
	m_EnableRGB[AdvanceEnable::B] = m_ParamBlock->GetInt(BlockParam::EnableB,t);

	m_AdvanceRGB[AdvanceIndex::gain] =  m_ParamBlock->GetFloat( BlockParam::RGBGain,  t ) / 100.0f; 
	m_AdvanceRGB[AdvanceIndex::gamma] = m_ParamBlock->GetFloat( BlockParam::RGBGamma, t );
	m_AdvanceRGB[AdvanceIndex::pivot] = m_ParamBlock->GetFloat( BlockParam::RGBPivot, t );
	m_AdvanceRGB[AdvanceIndex::lift] = m_ParamBlock->GetFloat( BlockParam::RGBLift,  t );

	m_AdvanceR[AdvanceIndex::gain] =  m_ParamBlock->GetFloat( BlockParam::RGain,  t ) / 100.0f; 
	m_AdvanceR[AdvanceIndex::gamma] = m_ParamBlock->GetFloat( BlockParam::RGamma, t );
	m_AdvanceR[AdvanceIndex::pivot] = m_ParamBlock->GetFloat( BlockParam::RPivot, t );
	m_AdvanceR[AdvanceIndex::lift] = m_ParamBlock->GetFloat( BlockParam::RLift,  t );

	m_AdvanceG[AdvanceIndex::gain] =  m_ParamBlock->GetFloat( BlockParam::GGain,  t ) / 100.0f; 
	m_AdvanceG[AdvanceIndex::gamma] = m_ParamBlock->GetFloat( BlockParam::GGamma, t );
	m_AdvanceG[AdvanceIndex::pivot] = m_ParamBlock->GetFloat( BlockParam::GPivot, t );
	m_AdvanceG[AdvanceIndex::lift] = m_ParamBlock->GetFloat( BlockParam::GLift,  t );

	m_AdvanceB[AdvanceIndex::gain] =  m_ParamBlock->GetFloat( BlockParam::BGain,  t ) / 100.0f; 
	m_AdvanceB[AdvanceIndex::gamma] = m_ParamBlock->GetFloat( BlockParam::BGamma, t );
	m_AdvanceB[AdvanceIndex::pivot] = m_ParamBlock->GetFloat( BlockParam::BPivot, t );
	m_AdvanceB[AdvanceIndex::lift] = m_ParamBlock->GetFloat( BlockParam::BLift,  t );
...
Interval ColorCorrection::Texture::Validity( TimeValue t )
{
	Interval v = FOREVER;
	Update(t,v);
	return v;
...
void ColorCorrection::Texture::SetSubTexmap( int i, Texmap *m )
{
	if( i == 0 ) {
...
Texmap* ColorCorrection::Texture::GetSubTexmap( int i )
{
	if( i == 0 )
...
MSTR ColorCorrection::Texture::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_DS_TEXMAP) : _T("Map");
	default: return _T("");
	}
}

void ColorCorrection::Texture::DeleteThis()
{
	delete this;
}

int ColorCorrection::Texture::NumRefs()
{
	return 2;
}

RefTargetHandle ColorCorrection::Texture::GetReference( int i )
{
	switch( i ) {
...
void ColorCorrection::Texture::SetReference( int i, RefTargetHandle v )
{
	switch( i ) {
...
IParamBlock2 * ColorCorrection::Texture::GetParamBlockByID( BlockID id )
{
	if (id == BlockParam::ID)
		return m_ParamBlock;
...
void ColorCorrection::Texture::GetUVTransform( Matrix3 &uvtrans )
{
	if(m_Map)
...
int ColorCorrection::Texture::GetTextureTiling()
{
	if(m_Map)
...
int ColorCorrection::Texture::GetUVWSource()
{
	if(m_Map)
...
int ColorCorrection::Texture::GetMapChannel()
{
	if(m_Map)
...
Point3 ColorCorrection::Texture::EvalNormalPerturb( ShadeContext& sc )
{
	if (m_Map)
...
bool ColorCorrection::Texture::IsLocalOutputMeaningful( ShadeContext& sc )
{
	if (m_Map)
...
int ColorCorrection::Texture::NumSubs()
{
	return 2;
}

Animatable* ColorCorrection::Texture::SubAnim( int i )
{
	if (i == 0)
		return m_ParamBlock;
	else
...
TSTR ColorCorrection::Texture::SubAnimName(int i, bool localized)
{
	if (i == 0)
		return localized ? GetString(IDS_DS_PARAMETERS) : _T("Parameters");
	else
...
IOResult ColorCorrection::Texture::Save(ISave *isave) { 
	IOResult res;
	// Save common stuff
...
IOResult ColorCorrection::Texture::Load(ILoad *iload) 
{ 
