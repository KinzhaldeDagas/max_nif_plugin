...
class Mask: public Texmap { 
	...
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_MASK) : _T("Mask"); }  
...
class MaskClassDesc:public ClassDesc2 {
	...
	const TCHAR*	ClassName() { return GetString(IDS_DS_MASK_CDESC); }
	const TCHAR*	NonLocalizedClassName() { return _T("Mask"); }
	SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
	Class_ID 		ClassID() { return maskClassID; }
...
static ParamBlockDesc2 mask_param_blk ( mask_params, _T("parameters"),  0, &maskCD, P_AUTO_CONSTRUCT + P_AUTO_UI, 2, 
	...
	mask_invert,	_T("maskInverted"), TYPE_BOOL,			0,				IDS_PW_INVERT,
		p_default,		FALSE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_INVERT_MASK,
		p_end,
	p_end
);
...
void Mask::Init() {
	ivalid.SetEmpty();
	mapValid.SetEmpty();
	invertMask = (pblock->GetInt(mask_invert) != 0) ? TRUE : FALSE;
	}
...
TSTR Mask::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_DS_MAP) : _T("Map");
	case 1: return localized ? GetString(IDS_DS_MASK) : _T("Mask");
	default: return _T("");
	}
}
...
TSTR Mask::SubAnimName(int i, bool localized)
{
	if (i < 2)
