...
class VCol: public Texmap { 
	...
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? GetString(IDS_DS_VCOL) : _T("Vertex Color"); }  
...
class VColClassDesc:public ClassDesc2 {
	...
	const TCHAR*	ClassName() { return GetString(IDS_DS_VCOL_CDESC); }
	const TCHAR*	NonLocalizedClassName() { return _T("Vertex Color"); }
	SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
	Class_ID 		ClassID() { return vcolClassID; }
...
ParamDlg* VCol::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp) {
	...
	IAutoMParamDlg* dlg = vcolCD.CreateParamDlgs(hwMtlEdit, imp, this);
...
RefResult VCol::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
   PartID& partID, RefMessage message, BOOL propagate ) {
	switch (message) {
//pblock2
		case REFMSG_CHANGE:
...
//			gpn->name= TSTR(GetString(name_id[gpn->index]));
		...
		case REFMSG_GET_PARAM_NAME_NONLOCALIZED: {
			GetParamName *gpn = (GetParamName*)partID;
//			gpn->name= nonlocalizedName[gpn->index];
			return REF_HALT;
			}
		}
	return(REF_SUCCEED);
...
void VCol::FilloutNames(HWND hWnd)
{
	...
	//loop through our channels
		TSTR blank(_T(" "));
		SendMessage(GetDlgItem(hWnd,IDC_NAMECOMBO),	CB_ADDSTRING,0,(LPARAM)(const TCHAR*)blank);		
		NameData data;
		data.channel = -1;
		data.subid = -1;						
	...
	//add them to our channel list and the UI
		for (int i = 0; i < 99; i++)
			...
			for (int j = -1; j < 3; j++)
				...
				if (j == -1)
					key.printf(_T("MapChannel:%d"),i);
				else key.printf(_T("MapChannel:%d(%d)"),i,j);
				if (node->UserPropExists(key))
					...
					if (name.Length() > 0)
						{
						if (j == -1)
							name.printf(_T("%d:%s : %s"),i,GetString(IDS_ALL),name);
						else if (j == 0)
							name.printf(_T("%d:%s : %s"),i,GetString(IDS_RED),name);						
						else if (j == 1)
							name.printf(_T("%d:%s : %s"),i,GetString(IDS_GREEN),name);						
						else if (j == 2)
							name.printf(_T("%d:%s : %s"),i,GetString(IDS_BLUE),name);						
						NameData data;
						data.channel = i;
						data.subid = j;
						SendMessage(GetDlgItem(hWnd,IDC_NAMECOMBO),	CB_ADDSTRING,0,(LPARAM)(const TCHAR*)name);						
						channelData.Append(1,&data,10);
						ct++;
						if ((mapID == i) && (subID == j))
							sel = ct;
