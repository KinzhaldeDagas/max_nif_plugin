...
bool RapidRenderSession::setup_for_new_render(const TimeValue t)
{
    ...
    if(m_render_job != nullptr)
    {
        ...
        // Translate the scene
        m_rendering_process.SetRenderingProgressTitle(MaxSDK::GetResourceStringAsMSTR(IDS_RENDER_INIT_SCENE).data());
    ...
    {
        m_rendering_logger.LogMessage(IRenderingLogger::MessageType::Error, MaxSDK::GetResourceStringAsMSTR(IDS_ERROR_INIT_RAPID).data());
        return false;
    }
}
    ...
    if(!m_renderer_paused)
    {
        ...
        if((sync_result == rti::RTI_SUCCESS) && DbgVerify(m_scene_root_translator != nullptr))
        {
            ...
            if(!m_termination_criteria_reached || scene_updated || frame_buffer_update_needed)
            {
                ...
                const bool multiple_frames_left = !m_termination_criteria_reached && ((m_termination_iterations == 0) || ((m_termination_iterations - total_iterations_submitted) > 1));
                const bool enable_buffering = (m_is_interactive_renderer ? false : multiple_frames_left); // disable buffering for active shade because it's buggy and worsens the perceived performance
                ...
                case rti::RTI_OUT_OF_MEMORY:
                    m_rendering_logger.LogMessage(IRenderingLogger::MessageType::Error, MaxSDK::GetResourceStringAsMSTR(IDS_RENDERING_OUT_OF_MEMORY).data());
                    return false;
                default:
                    m_rendering_logger.LogMessage(IRenderingLogger::MessageType::Error, MaxSDK::GetResourceStringAsMSTR(IDS_RENDERING_ERROR).data());
                    return false;
                }
            }
            else
...
void RapidRenderSession::swap(const	rti::SwapParams	&params)
{
        ...
        if(!is_down_res_frame)
        {
            // Report progress information
            {
                ...
                else 
                {
                    ...
                    const float overall_progress = std::min(std::max({ iteration_progress, time_progress, quality_progress }), 1.0f);
            ...
            if(!m_termination_criteria_reached)
            {
                ...
                m_rendering_logger.LogMessage(IRenderingLogger::MessageType::Progress, MaxSDK::GetResourceStringAsMSTR(IDS_TERMINATING_ITERATIONS).data());
                }
                ...
                {
                    m_termination_criteria_reached = true;
                m_rendering_logger.LogMessage(IRenderingLogger::MessageType::Progress, MaxSDK::GetResourceStringAsMSTR(IDS_TERMINATING_TIME).data());
                }
                ...
                {
                    m_termination_criteria_reached = true;
                m_rendering_logger.LogMessage(IRenderingLogger::MessageType::Progress, MaxSDK::GetResourceStringAsMSTR(IDS_TERMINATING_QUALITY).data());
                }
                ...
                {
                    m_termination_criteria_reached = true;
                m_rendering_logger.LogMessage(IRenderingLogger::MessageType::Progress, MaxSDK::GetResourceStringAsMSTR(IDS_TERMINATING_USER_REQUEST).data());
                }
            }
        }
    }
    ...
    {
        // Run the noise filter, on the last iteration ONLY
        const bool enable_noise_filter = m_termination_criteria_reached; 
...
void RapidRenderSession::execute_noise_filter(const	rti::SwapParams	&params)
{
    ...
    {
        m_rendering_logger.LogMessage(IRenderingLogger::MessageType::Info, MaxSDK::GetResourceStringAsMSTR(IDS_NOISEFILTER_STARTED).data());
...
#pragma warning( disable: 4996 )
            ...
            else  
            {
                ...
                {
                case rti::RTI_OUT_OF_MEMORY:
                    m_rendering_logger.LogMessage(IRenderingLogger::MessageType::Error, MaxSDK::GetResourceStringAsMSTR(IDS_NOISEFILTER_OUT_OF_MEMORY).data());
                    break;
                case rti::RTI_FILTER_ABORTED:
                    m_rendering_logger.LogMessage(IRenderingLogger::MessageType::Info, MaxSDK::GetResourceStringAsMSTR(IDS_NOISEFILTER_ABORTED).data());
                    break;
                default:
                    m_rendering_logger.LogMessage(IRenderingLogger::MessageType::Error, MaxSDK::GetResourceStringAsMSTR(IDS_NOISEFILTER_ERROR).data());
                    break;
                }
            }
        }
