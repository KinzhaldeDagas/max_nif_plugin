...
void RapidRTRendererPlugin::AddTabToDialog(ITabbedDialog* dialog, ITabDialogPluginTab* tab)
{
    ...
    dialog->AddRollout(MaxSDK::GetResourceStringAsMSTR(IDS_RAPID_CLASS_NAME).data(),	NULL,
...
bool RapidRTRendererPlugin::HasRequirement(const Renderer::Requirement requirement) 
{
    switch(requirement)
    {
    case Renderer::kRequirement_Wants32bitFPOutput:
        // Rapid always needs floating-point buffer (because we process the exposure control in post)
        return true;
    case Renderer::kRequirement_NoGBufferForToneOpPreview:
        // No GBuffers generated by Rapid.
        return true;
    case Renderer::kRequirement_SupportsConcurrentRendering:
...
const MCHAR* RapidRTRendererPlugin::ClassDescriptor::NonLocalizedClassName()
{
	return _T("ART Renderer");
}
