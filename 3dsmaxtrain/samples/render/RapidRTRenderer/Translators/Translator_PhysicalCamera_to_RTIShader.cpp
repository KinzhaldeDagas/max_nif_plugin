...
bool Translator_PhysicalCamera_to_RTIShader::EvaluateCameraParameters(CameraParameters& parameters, const TimeValue t, const IRenderSessionContext& context)
{
    ...
    if(physical_camera != nullptr)
    {
        ...
        const float oldFOV              = physical_camera->GetFOV(t, validity);//Not taking into account safe frame possible change in FOV
        const float newFOV              = camera_container.GetPerspectiveFOVRadians(t, validity);//Taking into account possible change in FOV
        bool bDifferentFOVs = fabsf(newFOV - oldFOV) > 1.0e-3f;
        IPhysicalCamera* pClone         = nullptr;
        if (bDifferentFOVs){
            //Clone the current physical camera to compute the lens focal length, to avoid creating an infinite recursion by modifying its params
            RemapDir *pRemap    = NewRemapDir();//dummy remapdir
			pClone              = static_cast<IPhysicalCamera*>(physical_camera->Clone(*pRemap));
            pClone->SetFOV(t, newFOV);//Change the FOV in the camera to compute the focal lens
            parameters.lens_focal_length = pClone->GetEffectiveLensFocalLength(t, validity) * physical_camera->GetCropZoomFactor(t, validity);
            pRemap->DeleteThis();
            pClone->DeleteMe();
        }else{
        ...
        //Exposure value from physical camera
        parameters.exposure_value = physical_camera->GetEffectiveEV(t, validity);
...
bool Translator_PhysicalCamera_to_RTIShader::CameraParameters::operator==(const CameraParameters& other) const
{
    return validity == other.validity
        ...
        && exposure_value == other.exposure_value;
}
