													   ...
													   int dirlights, std::vector<rti::Vec4f> *direction_spread, std::vector<rti::Vec4f> *color_visi)
{
	if(mtlbase != NULL)
	{
		...
		const MSTR maxRootDir = GetCOREInterface()->GetDir(APP_MAX_SYS_ROOT_DIR);
		const MSTR amgDataPath = maxRootDir + _T("AMG\\Data\\");
							...
							if (!metaInfo.isNull()) // There's a real bitmap - use it
							{
								...
								if (ptr)
								{
									...
									if(result != TranslationResult::Aborted)
									{
										...
										if (texture_translator != nullptr)
										{
											...
											set_shader_bool(*shader, (parameter_name + CStr("_on")).data(), (res.x > 1));
										}
									}
									else
									{
								...
								else
								{
									...
									if(result != TranslationResult::Aborted)
									{
										...
										{
											IPoint2 res = texture_translator->get_texture_resolution();
											set_shader_int(*shader, (parameter_name + CStr("_x")).data(), res.x);
											set_shader_int(*shader, (parameter_name + CStr("_y")).data(), res.y);
											set_shader_float(*shader, (parameter_name + CStr("_1ox")).data(), 1.0f/res.x);
											set_shader_float(*shader, (parameter_name + CStr("_1oy")).data(), 1.0f/res.y);
										}
									}
									else
									{
