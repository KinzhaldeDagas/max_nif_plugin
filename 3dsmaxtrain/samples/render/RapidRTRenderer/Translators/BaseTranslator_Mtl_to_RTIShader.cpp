...
void BaseTranslator_Mtl_to_RTIShader::NotificationCallback_NotifyEvent(const MaxSDK::NotificationAPI::IGenericEvent& genericEvent, void* userData)
{
    if (genericEvent.GetNotifierType() == NotifierType_Material && genericEvent.GetEventType() == EventType_Material_Uncategorized)
    {
        // Non-parameterblock change (E.g., ShowShadedInViewport flag), in which we're not interested.
        return;
    }
...
std::unique_ptr<Translator> BaseTranslator_Mtl_to_RTIShader::Allocator::operator()(const Key& key, TranslatorGraphNode& translator_graph_node, const IRenderSessionContext& render_session_context, const TimeValue /*initial_time*/) const
{
    ...
    else
    {
        ...
        {
            TSTR error_message;
            error_message.printf(MaxSDK::GetResourceStringAsMSTR(IDS_UNSUPPORTED_MATERIAL), material->GetName().data(), material->ClassName(true).data());
