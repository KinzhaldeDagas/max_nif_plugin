...
bool Translator_RTIRenderOptions::get_physical_value_for_white(float& value, const TimeValue t, Interval& valid) const
{
    ...
    if(tone_op != nullptr)
    {
        ...
        GetRenderSessionContext().GetLogger().LogMessage(IRenderingLogger::MessageType::Info, MaxSDK::GetResourceStringAsMSTR(IDS_WARNING_OUTLIER_CLAMP_EXPOSURE_CONTROL).data());
        return false;
    }
    else
    {
