...
enum NoiseFilter_RenderElement::ParameterID : int
{
    ParamID_Enabled = 0,
    ParamID_Bitmap = 1,
    ParamID_Strength = 2,
...
void NoiseFilter_RenderElement::GetClassName(MSTR& s, bool localized) const
{
    s = localized ? GetTheClassDescriptor().ClassName() : GetTheClassDescriptor().NonLocalizedClassName();
}
...
TSTR NoiseFilter_RenderElement::SubAnimName(int i, bool localized)
{
	IParamBlock2* param_block = GetParamBlock(i);
	if (param_block && param_block->GetDesc()->local_name != 0)
	{
		return localized ? TSTR(param_block->GetLocalName()) : param_block->GetDesc()->int_name;
	}
	else
	{
		return _T("");
...
const MCHAR* NoiseFilter_RenderElement::ClassDescriptor::NonLocalizedClassName()
{
	return _T("ART Renderer Noise Filter");
}
