...
#ifdef DBG
	_tprintf(_T("QuantAddHist: quant_incr = %d \n"),quant_incr);
...
#ifdef DBG
	_tprintf(_T("cutr = %d cutg = %d cutb = %d \n"),cutr,cutg,cutb);
   _tprintf(_T("maxr = %f maxg = %f maxb = %f \n"),maxr,maxg,maxb);
#endif
...
int QuantImp::Partition(BMM_Color_48 *pal, int palsize, BMM_Color_64 *forceCol)  {
	...
	ig=(36>>3)+1; 
	ib=(56>>3)+1; 
	_tprintf(_T("===(%d,%d,%d) wt = %d, mr = %d, mg = %d, mb = %d \n"),
	...
	if (forceCol) {
#if 0
		_tprintf(_T(" FORCING BG COLOR INTO PALETTE (%d,%d,%d)\n"),
	...
	for(k=0; k<lut_size; k++)
		_tprintf(_T(" sorted_pal[%d] = (%d,%d,%d)\n"),k,pal[k].r,pal[k].g,pal[k].b);
#endif
...
#ifdef DBGDITH
		if (dbgdith)
			DebugPrint(_T("  closest(%d,%d,%d) = pal[%d] = (%d,%d,%d) x=%d y=%d, hash# %d\n"),
...
#ifdef DBGDITH
		if (dbgdith) 
			DebugPrint(_T("rgb = (%d,%d,%d) , err=(%d,%d,%d) closest=%d (%d,%d,%d) x=%d y=%d\n"),	
...
#ifdef DBGDITH
	if (dbgdith)
		DebugPrint(_T(" find_closest_col of (%d,%d,%d) = pal[%d] = (%d,%d,%d) \n"),
