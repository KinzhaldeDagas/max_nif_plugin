        ...
        else
        {
                ...
                if(param_block != nullptr)
                {
                    for(const ParamDef& param_def : *param_block)
                    {
                        if(param_def.type == TYPE_BITMAP)
                        {
                            ...
                            resolution.x = std::max(resolution.x, bitmap_resolution.x);
                            resolution.y = std::max(resolution.y, bitmap_resolution.y);
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < num_sub_texmaps; ++i)
            {
                ...
                if(subTexmap != nullptr) 
                {
                    ...
                    resolution.x = std::max(resolution.x, sub_texmap_resolution.x);
                    resolution.y = std::max(resolution.y, sub_texmap_resolution.y);
                }
            }
