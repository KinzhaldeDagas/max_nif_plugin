...
// Tangents can't be shared across normal discontinuities, so we store an array of tangents, 
// where each element corresponds to a different normal
class MeshCollector::TangentNode
{
public:
    // Adds a face's tangent to this node. If the normal already exists in the array, 
    // we add the tangents to this element, otherwise we create a new element.
    // The method returns the number of elements added to the array during this call
    unsigned int add_face_tangents(const UVTangentVectors& face_tangents, const Point3& face_vertex_normal);

    // Adds our normalized tangents to the array
    void push_back_tangents(std::vector<UVTangentVectors>& tangents) const;

    // Returns the index of the tangent which normal matches the given one
    unsigned int get_matching_tangent_index(const Point3& normal) const;

private:

    struct Less { bool operator()(const Point3& a, const Point3& b) const; };

    std::vector<UVTangentVectors> m_tangents;
    std::map<Point3, int, Less> m_tangentIndices;
};
