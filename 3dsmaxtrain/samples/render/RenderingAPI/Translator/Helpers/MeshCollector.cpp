...
{
    auto it = m_tangentIndices.find(face_vertex_normal);
    if (it == m_tangentIndices.end())
    {
        m_tangentIndices[face_vertex_normal] = (int)m_tangents.size();
        m_tangents.push_back(face_tangents);
        return 1;
    }

    // Add in the tangent (will be normalized later)
    UVTangentVectors& tangent = m_tangents[it->second];
    tangent.tangentU += face_tangents.tangentU;
    tangent.tangentV += face_tangents.tangentV;
    return 0;
}
...
void MeshCollector::TangentNode::push_back_tangents(std::vector<UVTangentVectors>& tangents) const
{
    ...
    {
        tangents.push_back(UVTangentVectors(Normalize(tangent.tangentU), Normalize(tangent.tangentV)));
    }
}

unsigned int MeshCollector::TangentNode::get_matching_tangent_index(const Point3& normal) const
{
    auto it = m_tangentIndices.find(normal);
    if (it == m_tangentIndices.end()) // Normal not matched... not supposed to happen
    {
        DbgAssert(false);
        return 0;
    }
...
bool MeshCollector::TangentNode::Less::operator()(const Point3& a, const Point3& b) const
{
