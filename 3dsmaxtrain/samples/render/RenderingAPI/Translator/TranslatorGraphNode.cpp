...
TranslatorGraphNode::~TranslatorGraphNode()
{
    // translator has a pointer to me, and some of them access it in their destructor
    // so we need to destroy the translator now and not later
...
const Translator* TranslatorGraphNode::AcquireChildTranslatorInternal(const TranslatorKey& key, const TimeValue t, ITranslationProgress& translation_progress, TranslationResult& result)
{
    ...
    else
    {
        ...
        // sake.
        DbgAssert(false);
        m_render_session_context.GetLogger().LogMessage(IRenderingLogger::MessageType::Error, MaxSDK::GetResourceStringAsMSTR(IDS_ERROR_ACQUIREWHILEKEYFRAME).data());
        return nullptr;
    }
}
