...
bool SceneContainer_Base::is_node_hidden(const SClass_ID& /*scid*/, const Class_ID& /*cid*/, INode& node) const
{
    if(!m_render_settings_container.GetRenderHiddenObjects())
    {
        const bool bRes = node.IsNodeHidden(true) || (m_render_settings_container.GetHideFrozenObjects() && node.IsFrozen());
        if (bRes) {
            return true;
        }
        
        //TODO: Check per viewport filtering for this node when PerViewFiltering affect rendering
        //if (mpViewExp){
        //    IPerViewportFilter* pIPerViewportFilter = mpViewExp->GetIPerViewportFilter();
        //    if (pIPerViewportFilter && pIPerViewportFilter->IsNodeFiltered(scid, cid, &node)) {
        //        return true;
        //    }
        //}
...
void SceneContainer_Base::flatten_node_hierarchy_recursively(INode& node, const TimeValue t) const
{
    ...
    if(!const_cast<INode&>(node).IsRootNode())    // Root nodes are empty shells, we don't render them.
    {
        ...
        {
            const SClass_ID scid = object_state.obj->SuperClassID();
            const Class_ID cid   = object_state.obj->ClassID();
            switch(scid)
            {
            case SHAPE_CLASS_ID:
...
{
    NotifierType notifierType = genericEvent.GetNotifierType();
    switch (notifierType)
    {
    ...
    {
        size_t eventType = genericEvent.GetEventType();
        switch (eventType)
        {
...
void SceneContainer_Base::set_view(ViewExp& pViewExp)
{
    mpViewExp = dynamic_cast<ViewExp23*>(&pViewExp);
}
