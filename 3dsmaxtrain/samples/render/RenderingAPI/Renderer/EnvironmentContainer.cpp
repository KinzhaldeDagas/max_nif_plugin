...
void EnvironmentContainer::GetClassName(MSTR& s, bool localized) const
{
...
TSTR EnvironmentContainer::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0:
		if (m_param_block && m_param_block->GetDesc()->local_name != 0)
		{
			return localized ? MSTR(m_param_block->GetLocalName()) : m_param_block->GetDesc()->int_name;
		}
		else
		{
			return _T("");
...
const MCHAR* EnvironmentContainer::ClassDescriptor::NonLocalizedClassName()
{
	return _T("Environment Container");
}
