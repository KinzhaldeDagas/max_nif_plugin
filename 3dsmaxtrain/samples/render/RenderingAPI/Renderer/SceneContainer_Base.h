...
public:
    ...
    void set_view(ViewExp& viewExp);//Needed for per viewport filtering for nodes
...
private:
    ...
    // Returns whether the node is hidden, for our rendering purposes
    bool is_node_hidden(const SClass_ID& scid, const Class_ID& cid, INode& node) const;
...
private:
    ...
    //Needed for per viewport filtering for nodes querying
    ViewExp23* mpViewExp = nullptr;
};
