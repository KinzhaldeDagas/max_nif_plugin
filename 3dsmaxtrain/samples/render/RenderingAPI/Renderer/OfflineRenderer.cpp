...
{	
    ...
    m_impl->m_render_session_context->get_progress().SetRenderingProgressTitle(MaxSDK::GetResourceStringAsMSTR(IDS_PROGRESS_INITIAL_TITLE).data());
	...
	if (!rp.inMtlEdit) 
    {
		BroadcastNotification<NOTIFY_PRE_RENDER>(&rp);
    }
        ...
        if((m_impl->m_offline_render_session != nullptr) || m_impl->m_render_session_context->do_missing_maps_dialog(t, hwnd))
        {
            ...
            if(m_impl->m_offline_render_session != nullptr)
            {
                if(m_impl->m_offline_render_session->TranslateScene(t))
                {
                    ...
                    if(!m_impl->m_render_session_context->get_progress().HasAbortBeenRequested())
                    {
                        ...
                        BroadcastNotification<NOTIFY_PRE_RENDERFRAME>(&rgc);
                        ...
                        if(render_success && !render_aborted)
                        {
                            ...
                            {
                                m_impl->m_render_session_context->get_progress().SetRenderingProgressTitle(MaxSDK::GetResourceStringAsMSTR(IDS_PROCESSING_RENDER_ELEMENTS).data());
                        ...
                        BroadcastNotification<NOTIFY_POST_RENDERFRAME>(&rgc);
                        ...
                        {
                            // Output error
                            m_impl->m_render_session_context->get_logger().LogMessage(IRenderingLogger::MessageType::Error, MaxSDK::GetResourceStringAsMSTR(IDS_ERROR_RENDER_ELEMENTS).data());
                            return 0;
                        }
                        else if(!render_success || render_aborted)
                        {
                            ...
                            {
                                // Error rendering
                                m_impl->m_render_session_context->get_logger().LogMessage(IRenderingLogger::MessageType::Error, MaxSDK::GetResourceStringAsMSTR(IDS_RENDERING_FAILED).data());
                            }
                            return 0;
                        }
                    }
                ...
                {
                    // Error translating
                    m_impl->m_render_session_context->get_logger().LogMessage(IRenderingLogger::MessageType::Error, MaxSDK::GetResourceStringAsMSTR(IDS_TRANSLATION_FAILED).data());
                    return 0;
                }
            }
            else
            {
                ...
                if (!m_impl->m_open_rend_params.inMtlEdit)
                    m_impl->m_render_session_context->get_logger().LogMessage(IRenderingLogger::MessageType::Error, MaxSDK::GetResourceStringAsMSTR(IDS_ERROR_RENDER_SESSION_CREATION).data());
                return 0;
            }
        }
        else
...
void UnifiedRenderer::Close(HWND /*hwnd*/, RendProgressCallback* prog)
{
    if(DbgVerify(m_impl->m_render_session_context != nullptr))
    {
        ...
        DbgVerify(ExecuteMAXScriptScript(_T("gc light:true"), MAXScript::ScriptSource::NonEmbedded, true));
    }
...
void UnifiedRenderer::GetClassName(MSTR& s, bool localized) const 
{
    s = localized ? GetClassDescriptor().ClassName() : GetClassDescriptor().NonLocalizedClassName();
}
...
TSTR UnifiedRenderer::SubAnimName(int i, bool localized)
{
	IParamBlock2* param_block = GetParamBlock(i);
	if (param_block && param_block->GetDesc()->local_name != 0)
	{
		return localized ? MSTR(param_block->GetLocalName()) : param_block->GetDesc()->int_name;
	}
	else
	{
		return _T("");
