...
#include "../../NotificationSystem/NotificationAPIUtils.h"
...
void CameraContainer_Base::set_view(const ViewExp& view_exp)
{
    ...
    if(vpt != nullptr)
    {
        mpViewExp13 = vpt;//Store it to use it later in fov calculation if safe frame is on
        ...
        new_view_params.zoom = Max::NotificationAPI::Utils::GetOrthoZoomFromViewExpUsingSafeFrame(vpt->GetZoom(), *mpViewExp13);//Taking into account safe frame if it is on
        new_view_params.fov  = Max::NotificationAPI::Utils::GetFOVFromViewExpUsingSafeFrame (vpt->GetFOV() , *mpViewExp13);//Taking into account safe frame if it is on
...
void CameraContainer_Base::update_view_params_cache(const TimeValue t, Interval& validity) const
{
    if(!m_view_params_cache.validity.InInterval(t))
    {
        ...
        if(camera_node != nullptr)
        {
            ...
            if(object_state.obj != nullptr)
            {
                ...
                if(object_state.obj->SuperClassID() == CAMERA_CLASS_ID)
                {
                    ...
                    const float currentFOV  = camera_object->GetFOV(t, m_view_params_cache.validity);
                    const float fov_radians = (nullptr != mpViewExp13) ? Max::NotificationAPI::Utils::GetFOVFromViewExpUsingSafeFrame(currentFOV, *mpViewExp13) : currentFOV;//Taking into account safe frame if it is on
                    ...
                    if(physical_camera != nullptr)
                    {
                        ...
                        //m_view_params_cache.lens_focal_length = GetEffectiveLensFocalLength(physical_camera, m_view_params_cache.lens_focal_length, t, fov_radians);
                        m_view_params_cache.dof_enabled = physical_camera->GetDOFEnabled(t, m_view_params_cache.validity);
                        m_view_params_cache.aperture_radius = physical_camera->GetLensApertureRadius(t, m_view_params_cache.validity, true);//May be a wrong value when using safe frame but doesn't seem to be used in ART renderer anyway
        ...
        {
            m_view_params_cache.film_width = GetCOREInterface()->GetRendApertureWidth() / GetSystemUnitScale(UNITS_MILLIMETERS);
        }
