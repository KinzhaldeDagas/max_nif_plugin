...
class RendVueClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("VUE File Renderer"); }
...
int VueRenderer::Open(
	...
	if (DoesFileExist(vueFileName, false)) {
		TSTR buf;
		buf.printf(GetString(IDS_FILE_ALREADY_EXISTS),vueFileName);
		int ret = MaxSDK::MaxMessageBox(NULL, buf, GetString(IDS_VRENDTITLE), MB_YESNO | MB_ICONEXCLAMATION);
	...
	if (!vuefile->CanWrite()) {
		...
		MaxSDK::MaxMessageBox(NULL, buf, GetString(IDS_RENDER_ERR), MB_OK | MB_ICONEXCLAMATION);
		delete vuefile;
		vuefile = NULL;
		return(0);
		}

	vuefile->Write(_T("VERSION 202\n"));
...
bool VueRenderer::IsStopSupported() const
{
    return false;
}

void VueRenderer::StopRendering()
{
    // Do nothing
}

Renderer::PauseSupport VueRenderer::IsPauseSupported() const
{
    return PauseSupport::Legacy;
}

void VueRenderer::PauseRendering()
{
    // Do nothing
}

void VueRenderer::ResumeRendering()
{
    // Do nothing
}

bool VueRenderer::HasRequirement(Requirement requirement)
{
    // No requirements
    return false;
}

IInteractiveRender* VueRenderer::GetIInteractiveRender()
{
    return nullptr;
}

void VueRenderer::GetVendorInformation(MSTR& info) const
{
    info = ClassName(true);
}

void VueRenderer::GetPlatformInformation(MSTR& info) const
{
    // nothing
}

bool VueRenderer::CompatibleWithAnyRenderElement() const
{
    return false;
}

bool VueRenderer::CompatibleWithRenderElement(IRenderElement& pIRenderElement) const
{
    return false;
}
