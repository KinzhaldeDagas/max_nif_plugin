		...
		PartID& partID, RefMessage message, BOOL propagate) 
{
	...
	switch (message) {
		...
		case REFMSG_GET_PARAM_NAME_LOCALIZED: {
			...
			switch (gpn->index) {
				case PB_ENABLE:      gpn->name = GetString(IDS_KE_ENABLE);      break;
				case PB_QUALITY:     gpn->name = GetString(IDS_KE_QUALITY);     break;
				case PB_SUBSAMP_TEX: gpn->name = GetString(IDS_KE_SUBSAMP_TEX); break;
		...
		case REFMSG_GET_PARAM_NAME_NONLOCALIZED: {
			gpn = (GetParamName*)partID;
			switch (gpn->index) {
				case PB_ENABLE:      gpn->name = _T("Enable");              break;
	...
	switch (message) {
		...
		case REFMSG_GET_PARAM_NAME_LOCALIZED: {
			...
			switch (gpn->index) {
				case PB_ENABLE:            gpn->name = GetString(IDS_KE_ENABLE);          break;
				case PB_QUALITY:           gpn->name = GetString(IDS_KE_QUALITY);         break;
				case PB_SUBSAMP_TEX_ADAPT: gpn->name = GetString(IDS_KE_SUBSAMP_TEX);     break;
				case PB_ADAPT_ENABLE:      gpn->name = GetString(IDS_KE_ADAPT_ENABLE);    break;
				case PB_ADAPT_THRESHOLD:   gpn->name = GetString(IDS_KE_ADAPT_THRESHOLD); break;
				default:                   gpn->name = _T("");                            break;
			}
			return REF_HALT;
		}
		...
		case REFMSG_GET_PARAM_NAME_NONLOCALIZED: {
			...
			switch (gpn->index) {
				case PB_ENABLE:            gpn->name = _T("Enable");              break;
				case PB_QUALITY:           gpn->name = _T("Quality");             break;
				case PB_SUBSAMP_TEX_ADAPT: gpn->name = _T("Sub-sample Textures"); break;
				case PB_ADAPT_ENABLE:      gpn->name = _T("Enable Adaptive");     break;
				case PB_ADAPT_THRESHOLD:   gpn->name = _T("Adaptive Threshold");  break;
				default:                   gpn->name = _T("");                    break;
			}
			return REF_HALT;
		}
