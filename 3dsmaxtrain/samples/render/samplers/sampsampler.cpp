...
public:
		...
		TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_SINGLE_SAMPLER) : _T("Single"); }
...
class SingleSamplerClassDesc : public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Single"); }
...
{
	pParamBlk = NULL;
	ReplaceReference(0, CreateParameterBlock(pbDesc, PBLOCK_LENGTH, CURRENT_VERSION));
	DbgAssert(pParamBlk);
...
const TCHAR* SingleSampler::GetDefaultComment()
{
