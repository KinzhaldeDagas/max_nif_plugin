...
{
    if (m_ViewUndoID >= 0 && GetCOREInterface()){
    ...
    if (m_ViewUndoID >= 0){
        ...
        if (activeViewID == m_ViewUndoID){ //Is that the viewport we are monitoring ?
            ...
            if (m_ViewNode == activeViewNode){
                ...
                const float new_fov = Utils::GetFOVFromViewExpUsingSafeFrame(activeViewExp->GetFOV(), *activeViewExp);
...
void MaxViewNotifier::UpdateOurStoredData(ViewExp& viewExp)
{
    viewExp.GetAffineTM(m_StoredMatrix);
    m_view_persp        = viewExp.IsPerspView();
    m_view_fov          = Utils::GetFOVFromViewExpUsingSafeFrame(viewExp.GetFOV(), viewExp);
    m_view_focal_dist   = viewExp.GetFocalDist();
    MaxSDK::NotificationAPIUtils::GetViewportLabelFromUndoIDIncludingExtendedViews(m_ViewUndoID, m_CurrentViewportLabel);
}

void MaxViewNotifier::NotifyEvent_NodesFiltered(void)
{
    NotifyEvent(ViewEvent::MakeViewNodesFilteredEvent(m_ViewUndoID));
}
