...
static bool s_GetParamNameFromParamBlock1(MSTR& outParamName, RefTargetHandle , IParamBlock* pBlock1, int paramIndex)
{
    ...
    pBlock1->NotifyDependents(FOREVER, (PartID)&gpn, REFMSG_GET_PARAM_NAME_LOCALIZED, NOTIFY_ALL, FALSE);
...
float Max::NotificationAPI::Utils::GetFOVFromViewExpUsingSafeFrame(float currentFOV, ViewExp& viewExp)
{
    float fov = currentFOV;

    Interface* ip = GetCOREInterface();
    if (ip && viewExp.getSFDisplay()){ //Is safe frame display enabled ?
        float fScaleFact        = 1.0f;
        const float rendAspect  = ip->GetRendPixelAspect();//Is the pixel aspect, not the image aspect
        const int rendWith      = ip->GetRendWidth();
        const int rendHeight    = ip->GetRendHeight();
        float as                = 1.0f;
    	
        GraphicsWindow* pGW = viewExp.getGW();
        if (nullptr != pGW){
		    if(rendHeight > 0)
			    as = rendAspect * rendWith / rendHeight;
		    else
			    as = 4.0f / 3.0f;

            const int viewportWidth  = pGW->getWinSizeX();
            const int viewportHeight = pGW->getWinSizeY();

		    fScaleFact = float(viewportWidth) / (viewportHeight * as);
		    if(fScaleFact < 1.0f){
			    fScaleFact = 1.0f;
            }
        }

        //Not supporting canvas mode
        /*if (IsCanvasNavigationMode())
		{
			fScaleFact *= scale2d; 
		}
        */

		fov = (float)(2.0 * atan(fScaleFact * tan((double)fov * 0.5)));//converting from persp.realFOV to persp.FOV (see View3D::RecalcGWMatrix())

    }//End of safe frame display on

    return fov;
}

float Max::NotificationAPI::Utils::GetOrthoZoomFromViewExpUsingSafeFrame(float currentZoom, ViewExp& viewExp)
{
    float zoom = currentZoom;

    Interface* ip = GetCOREInterface();
    if (ip && viewExp.getSFDisplay() && FALSE == viewExp.IsPerspView()){ //Is safe frame display on in orthographic view ?
            
        // The scale factor added by Safe Frame or 2D Zoom ("Lock Zoom/Pan" mode)
	    float fScaleFact        = 1.0f;
        const float rendAspect  = ip->GetRendPixelAspect();//Is the pixel aspect, not the image aspect
        const int rendWith      = ip->GetRendWidth();
        const int rendHeight    = ip->GetRendHeight();
        ...
        zoom = fScaleFact * currentZoom; //Computing from para.realZoom to para.zoom (see View3D::RecalcGWMatrix())
    }//End of safe frame display on
