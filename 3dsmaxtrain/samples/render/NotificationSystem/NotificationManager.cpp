...
void NotificationManager::NotifyViewEvent(const ViewEventType eventType, const int viewID) const
{
    NotificationSystemAutoCriticalSection lock_guard(m_notifiers_lock);
    ...
    {
        for (auto& viewNotifierMap : m_ViewNotifiers)
        {
            MaxViewNotifier* pViewNotif = viewNotifierMap.second;
            if (nullptr != pViewNotif && pViewNotif->GetMaxViewID() == viewID){
                pViewNotif->NotifyEvent_NodesFiltered();
            }
        }
    }
    break;
