...
//--------------------------------------------------------------------------------
static unsigned long randmasks[] = {
	0,			// 0
	0,			// 1
	0x0003,		// 2
	0x0006,		// 4
...
static inline int pixOffset(int ix, int iy) {
...
inline AccumRec& pixOffsetRec(int ix, int iy) {
	if (!fieldRender) return paccum[iy][ix];	
	else {
		return paccum[((iy<<1)+nField)][ix];
		}
	}

#define GETACC(ix,iy) pixOffsetRec(ix,iy) 
...
#ifdef DOACCUM
	paccum = new AccumRecPtr[bmh];
	// Let's create a buffer per line instead of one that contains all pixels in one buffer
	for(i=0;i<bmh;i++)
		paccum[i] = new AccumRec[bmw];
	...
	for (passNum=0; passNum<NPASSES; passNum++) {
		...
		for (nField=0; nField< (fieldRender?2:1); nField++) {
			if (randOrder[passNum]) {
				...
				unsigned long mask;	// mask to XOR with to create sequence 
				unsigned long element; // one element of random sequence 
				int iy, ix; // iy and ix numbers for a pixel 
				int numPix=0;
...
#ifdef DISPPASS1
			bm->RefreshWindow();
			MaxSDK::MaxMessageBox(NULL, _T("hi"), _T("After Pass 1"), MB_OK | MB_ICONEXCLAMATION);
#endif
...
static inline int TestZDepth(int ix, int iy, float &contribution) {
	int poffs;
	int cov;	
	if (nodeIdBuf[poffs= pixOffset(ix,iy)]!=curObj) 
	{
		if (pixZ<zbuf[poffs]) 
		{
			...
			{
				// If the coverage is not full, let's look at the g-buffer to see if there any other triangles
				// between here and the effect
				// Since we do not care about transparency, this loop just ignores it.
				// between here and the effect
				int newiy = fieldRender ? ( (iy<<1) + nField ) : iy;
				if (gbRdr2->StartPixel(ix,newiy)>0) {
					BOOL gotLayers = FALSE;
					while (gbRdr2->StartNextLayer()) 
					{
						...
						cov+=gbData2.coverage;
					}
				}
			}
			// rounding errors in the gbuffer will often lead to a total smaller than full coverage when summing all fragments
			// this is just the type of inaccuracy that you have to deal with using these type of algorithms
			if(cov>=254)
			{
				contribution =0;
				return 0;
		}
			contribution = (254-cov)/254.0f;
			return 1;
		}
	}
	contribution = 1.0f;
...
//			DebugPrint(_T("f = %d ,pcol = (%d,%d,%d) col = %d,%d,%d,%d  (%d,%d,%d,%d)\n"),
...
static void doPixFilter(int x, int y, int frac) {
	...
	if (doTransp) {
		...
		if (nodeIdBuf[poffs= pixOffset(x,y)]!=curObj) {
			if (pixZ<zbuf[poffs]) 
				cov=0.0; // if moving pixel is behind object, skip it.
			}
		frac*=cov;
	}
//	CR: Commented this piece out, all pixels need to be considered even if they have 0 contribution
//	The blurring alogirhm counts the number of accumulated pixels, if we differ based on visibility, we change the blur
//	if (frac==0) 
//		return;
	pixRec[nacc].f = frac;
