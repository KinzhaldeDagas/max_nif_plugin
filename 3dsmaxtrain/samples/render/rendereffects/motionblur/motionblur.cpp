...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override { s = localized ? ::GetString(IDS_MOT_BLUR) : _T("Motion Blur"); }
	...
	// Effect
	TSTR GetName(bool localized) const override { return localized ? ::GetString(IDS_MOT_BLUR) : _T("Motion Blur"); }
...
static IMBOpsImp imb_interface(IMB_INTERFACE, _T("ImageMotionBlur"), 0, &motBlurCD, 0,	p_end	);
...
static ParamBlockDesc2 motBlur_param_blk(motBlur_params, _T("imageMotionBlur parameters"), 0, &motBlurCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	...
	prm_transp, _T("transparency"), TYPE_BOOL, 0, IDS_TRANSP,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX,  IDC_TRANSP,
		p_end,
	p_end
	);
...
TSTR MotionBlur::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_MOT_BLUR_PARAMS) : _T("Motion Blur Parameters");
	default: return _T("");
	}
}
...
RefResult MotionBlur::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message)
	{
	case REFMSG_CHANGE:
