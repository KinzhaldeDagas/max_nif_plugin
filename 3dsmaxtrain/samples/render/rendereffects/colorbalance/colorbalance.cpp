...
// ----------------------------------------
class ColorBalance: public Effect8
{
public:
	// parameters
	IParamBlock2* pblock;
...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_COL_BAL) : _T("Color Balance"); }
	...
	// Effect
	TSTR GetName(bool localized) const override { return localized ? GetString(IDS_COL_BAL) : _T("Color Balance"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Color Balance"); }
...
static ParamBlockDesc2 colBal_param_blk(colBal_params, _T("colorBalance parameters"), 0, &colBalCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	...
	prm_pLum, _T("preserveLum"), TYPE_BOOL, P_ANIMATABLE, IDS_P_LUM,
		p_default, FALSE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_LUMINOSITY,
		p_nonLocalizedName, _T("Preserve Luminosity"),
		p_end,
	prm_iBack, _T("ignoreBack"), TYPE_BOOL, P_ANIMATABLE, IDS_I_BACK,
		p_default, FALSE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_BACKGROUND,
		p_nonLocalizedName, _T("Ignore Background"),
		p_end,
	p_end
	);
...
TSTR ColorBalance::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_COL_BAL_PARAMS) : _T("Color Balance Parameters");
	default: return _T("");
	}
}
...
RefResult ColorBalance::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message)
	{
	case REFMSG_CHANGE:
...
{
	int storageType, alphaStorageType;
	WORD *pS = (WORD*)bm->GetStoragePtr(&storageType); // rgb
	WORD *pA = (WORD*)bm->GetAlphaPtr(&alphaStorageType);   // alpha
	if(storageType != BMM_TRUE_48)
		pS = NULL;
	if(alphaStorageType != BMM_GRAY_16)
		pA = NULL;
	...
	*/
	...
	else
	{
		...
		BMM_Color_fl* buf = new BMM_Color_fl[bm->Width()];
...
bool ColorBalance::SupportsBitmap(Bitmap& bitmap) {

	// All bitmaps are supported since the storage is never directly accessed.
	return true;
}
