...
public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_CLASS_NAME) : _T("Brightness and Contrast"); }
		...
		// Effect methods
		TSTR GetName(bool localized) const override { return localized ? GetString(IDS_NAME) : _T("Brightness and Contrast"); }
		...
		//Inherited from the Mergeable Interface
		ReferenceTarget* GetMergeableRef();
		TSTR GetMergeableRefName();
		void PostMerge();
	};
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Brightness and Contrast"); }
...
static ParamBlockDesc2 briteCon_param_blk(briteCon_params, _T("briteCon parameters"), 0, &briteConCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	...
	prm_iBack, _T("ignoreBack"), TYPE_BOOL, P_ANIMATABLE, IDS_IGN_BACK,
		p_default, FALSE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_IGN_BACKGROUND,
...
TSTR BriteConEffect::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_PARAMS) : _T("Brightness and Contrast Parameters");
	default: return _T("");
	}
}
...
RefResult BriteConEffect::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate) 
{
	switch (message)
	{
	case REFMSG_CHANGE:
