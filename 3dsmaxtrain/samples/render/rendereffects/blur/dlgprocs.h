...
public:
	...
	MainDlgProc() : numCheckedIn(0), mAllDlgsCreated(false), mp_blurMgr(NULL) { mHWnds[0] = mHWnds[1] = mHWnds[2] = NULL; }
...
{
	MainDlgProc *mpMainDlgProc;
	void DeleteThis() { }
	INT_PTR DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
	void enableControls(TimeValue t, IParamMap2* map, HWND hWnd);

public:
	BlurDataDlgProc() : mpMainDlgProc(NULL) { }
	BlurDataDlgProc(MainDlgProc *mpMain) : mpMainDlgProc(mpMain) { }
};
...
public:
	SelDataDlgProc() : mpMainDlgProc(NULL) { }
	SelDataDlgProc(MainDlgProc *mpMain) : mpMainDlgProc(mpMain) { }
};
