...
void BlurRadial::notifyPrmChanged(ParamID prmID)
{
	switch (prmID)
	{
...
bool BlurRadial::calcRotPixOffsets(double theta)
{
	...
	{
		x1 = -std::round(m_inRad*c); y1 = -std::round(m_inRad*s);
	...
	{
		x2 = std::round(m_outRad*c); y2 = std::round(m_outRad*s);
...
bool BlurRadial::doBlur(CompMap *pCompMap, BitmapAccessor &bmFrom, BitmapAccessor &bmTo )
{
	...
	x = int(m_orig.x);
	y = int(m_orig.y);
	int maxX = std::max( abs(x), abs(m_imageW - x) );
	int maxY = std::max( abs(y), abs(m_imageH - y) );
	int maxXY = std::max(maxX, maxY);
	maxDist = sqrt( sqr(static_cast<double>(maxX)) + sqr(static_cast<double>(maxY)) );
	...
	for (i=1; i<=maxX; i++)
	{
		...
		m_inRad = std::round(pixRad*(1-m_trail)); m_outRad = std::round(pixRad*outMult*(1+m_trail));
	...
	for (j=1; j<=maxXY; j++) {
		for (i=j+1; i<=maxXY; i++) {
			theta = atan2( static_cast<double>(j), static_cast<double>(i) );
			dist = sqrt( sqr(static_cast<double>(i)) + sqr(static_cast<double>(j)) );
			distRatio = dist/maxDist;
			pixRad = distRatio * m_pixRad;
			m_inRad = std::round(pixRad*(1-m_trail)); m_outRad = std::round(pixRad*outMult*(1+m_trail));
			if ( !calcRotPixOffsets(theta) )
...
void BlurRadial::blur(TimeValue t, CompMap *pCompMap, Bitmap *bm, RenderGlobalContext *gc)
{
	...
	m_pixRad = (int)( 0.5*floor(std::max(m_imageW, m_imageH)*(fTemp*PERCENT2DEC) ) );
