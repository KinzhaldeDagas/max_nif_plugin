...
#if defined( INTEL_COMPILER ) || (defined( _MSC_VER ) && (_MSC_VER >= 1400))
  #include "xmmintrin.h"
#endif
#include <Geom/trig.h>
...
void BlurDirectional::notifyPrmChanged(ParamID prmID)
{
	switch (prmID)
	{
		case prmDirRot:
...
//	if (m_leftRad) // count pixels left of center
	...
	{
		x1 = -std::round(m_leftRad*c); y1 = -std::round(m_leftRad*s);
...
//	if (m_rightRad) // count pixels right of center
	...
	{
		x2 = std::round(m_rightRad*c); y2 = std::round(m_rightRad*s);
...
//	if (m_upRad) // count pixels above center
	...
	{
		x1 = std::round(m_upRad*s); y1 = -std::round(m_upRad*c);
...
//	if (m_downRad) // count pixels below center
	...
	{
		x2 = -std::round(m_downRad*s); y2 = std::round(m_downRad*c);
...
bool BlurDirectional::do1DBoxRot(CompMap *pCompMap, BitmapAccessor &bmFrom, BitmapAccessor &bmTo)
{
	BitmapColorAccessor &mapFrom = bmFrom.map, &mapTo = bmTo.map;
	BitmapAlphaAccessor &alphaFrom = bmFrom.alpha, &alphaTo = bmTo.alpha;
	...
	if (m_uPixRad) // blur only in u direction
	{
		...
		int leftRad( std::round(m_uPixRad*(1-m_uTrail)) ), rightRad( std::round(m_uPixRad*(1+m_uTrail)) );
	...
	else // blur only in v direction
	{
		...
		int upRad( std::round(m_vPixRad*(1-m_vTrail)) ), downRad( std::round(m_vPixRad*(1+m_vTrail)) );
	...
	if (mapTo)
	{
		...
		{
			if (pCompMap)
				blendPixel(mapIndex, bmFrom, bmTo, mp_blurCache[mapIndex], (*pCompMap)[mapIndex].brighten, (*pCompMap)[mapIndex].blend);
			else
			{
				srcIdx = mapIndex*3;
...
bool BlurDirectional::do1DBoxNoRot(CompMap *pCompMap, BitmapAccessor &bmFrom, BitmapAccessor &bmTo)
{
	BitmapColorAccessor &mapFrom = bmFrom.map, &mapTo = bmTo.map;
	BitmapAlphaAccessor &alphaFrom = bmFrom.alpha, &alphaTo = bmTo.alpha;
...
bool BlurDirectional::do2DBoxRot(CompMap *pCompMap, BitmapAccessor &bmFrom, BitmapAccessor &bmTo)
{
	BitmapColorAccessor &mapFrom = bmFrom.map, &mapTo = bmTo.map;
	BitmapAlphaAccessor &alphaFrom = bmFrom.alpha, &alphaTo = bmTo.alpha;
...
bool BlurDirectional::do2DBoxNoRot(CompMap *pCompMap, BitmapAccessor &bmFrom, BitmapAccessor &bmTo)
{
	BitmapColorAccessor &mapFrom = bmFrom.map, &mapTo = bmTo.map;
	BitmapAlphaAccessor &alphaFrom = bmFrom.alpha, &alphaTo = bmTo.alpha;

#ifdef INTEL_COMPILER
	#pragma message("Note: INTEL_COMPILER is defined, using Intel compiler specific optimizations")
	if ( gBlurSSE_Enabled )
	{
		float totWts;
...
bool BlurDirectional::doBlur(CompMap *pCompMap, BitmapAccessor &bmFrom, BitmapAccessor &bmTo )
{
	BitmapColorAccessor &mapFrom = bmFrom.map, &mapTo = bmTo.map;
	BitmapAlphaAccessor &alphaFrom = bmFrom.alpha, &alphaTo = bmTo.alpha;
	...
	if(m_blurValid && mapTo)
	{
		...
		for (int i=0; i<m_imageSz; i++)
			if (pCompMap)
			{
				blendPixel(i, bmFrom, bmTo, mp_blurCache[i], (*pCompMap)[i].brighten, (*pCompMap)[i].blend);
			}
			else
			{
				srcIdx = i*3;
				mapTo[srcIdx]   = (USHORT)(mp_blurCache[i].r * MAX_COL16);
				mapTo[srcIdx+1] = (USHORT)(mp_blurCache[i].g * MAX_COL16);
				mapTo[srcIdx+2] = (USHORT)(mp_blurCache[i].b * MAX_COL16);
				if (alphaTo)
	...
	{
		return do2DBoxNoRot(pCompMap, bmFrom, bmTo);
	}

	if ( m_rot == 0.0f )
	{
		if ( (!m_uPixRad && m_vPixRad) || (m_uPixRad && !m_vPixRad) )
			return do1DBoxNoRot(pCompMap, bmFrom, bmTo);
		else
			return do2DBoxNoRot(pCompMap, bmFrom, bmTo);
	}
	else
	{
		if ( (!m_uPixRad && m_vPixRad) || (m_uPixRad && !m_vPixRad) )
			return do1DBoxRot(pCompMap, bmFrom, bmTo);
		else
			return do2DBoxRot(pCompMap, bmFrom, bmTo);
	}
}
...
void BlurDirectional::blur(TimeValue t, CompMap *pCompMap, Bitmap *bm, RenderGlobalContext *gc)
{
	...
	m_uPixRad = (int)( 0.5*floor(std::max(m_imageW, m_imageH)*(fTemp*PERCENT2DEC) ) );

	mp_blurMgr->getBlurValue(prmDirVPixRad, t, fTemp, FOREVER);
	LimitValue(fTemp, 0.0f, 1000.0f); // mjm - 9.30.99
	m_vPixRad = (int)( 0.5*floor(std::max(m_imageW, m_imageH)*(fTemp*PERCENT2DEC) ) );
	...
	BitmapAccessor bmTo(bm, FALSE, m_affectAlpha);

	m_blurValid = doBlur(pCompMap, bmFrom, bmTo);
};
