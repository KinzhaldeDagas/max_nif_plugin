...
void MainDlgProc::checkIn(int id, HWND hDlg)
{
	mHWnds[id] = hDlg;
	numCheckedIn++;
...
void MainDlgProc::placeChildren()
{
	// compute locations for child dialogs
	Rect rc;
	GetWindowRect(mHWnds[idMain], &rc);
	int originX = rc.left;
	int originY = rc.top;

	// find display area of tab control
	HWND hTabCtrl = GetDlgItem(mHWnds[idMain], IDC_TAB);
...
void MainDlgProc::notifyChildrenCreated(BlurMgr *blurMgr)
{
	...
	switch( TabCtrl_GetCurSel( GetDlgItem(mHWnds[idMain], IDC_TAB) ) )
	{
		case 0:
...
void MainDlgProc::UpdateCurveControl()
{
	assert(mp_blurMgr);
...
void MainDlgProc::ShowCurveControl(BOOL show)
{
	assert(mp_blurMgr);
...
void MainDlgProc::SetThing(ReferenceTarget *m)
{
...
INT_PTR MainDlgProc::DlgProc(TimeValue t, IParamMap2* map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
...
INT_PTR BlurDataDlgProc::DlgProc(TimeValue t, IParamMap2* map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch(msg)
	{
		case WM_INITDIALOG:
			mpMainDlgProc->checkIn(idBlurData, hWnd);
...
INT_PTR SelDataDlgProc::DlgProc(TimeValue t, IParamMap2* map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	ICurve *pCurve;
	...
	switch(msg)
	{
		case WM_INITDIALOG:
		{
			mpMainDlgProc->checkIn(idSelData, hWnd);
		...
		case WM_SHOWWINDOW:
		{
			...
			BitArray ba = mpMainDlgProc->getBlurMgr()->getCCtrl()->GetDisplayMode();
		...
		case WM_CC_SEL_CURVEPT:
			...
			DebugPrint(_T("For curve at address: %p, "), pCurve);
			DebugPrint(_T("a curve point was %s.\n"), (newNumCurvePts > oldNumCurvePts) ? _T("selected") : _T("deselected"));
			DebugPrint(_T("There are now %d points selected.\n\n"), newNumCurvePts);
		...
		case WM_CC_CHANGE_CURVEPT:
			...
			DebugPrint(_T("For curve at address: %p, curve point %d has changed.\n"), pCurve, ptIndex);
			cPt = pCurve->GetPoint(t, ptIndex);
			DebugPrint(_T("The point's new value is: [%g, %g]\n\n"), cPt.p.x, cPt.p.y);
			break;
		...
		case WM_CC_CHANGE_CURVETANGENT:
			...
			pCurve = (ICurve *)lParam;
			ptIndex = LOWORD(wParam);
			DebugPrint(_T("For curve at address: %p, curve tangent %d has changed.\n"), pCurve, ptIndex);
			cPt = pCurve->GetPoint(t, ptIndex);
			DebugPrint(_T("The point's new in-tangent is: [%g, %g]\n"), cPt.in.x, cPt.in.y);
			DebugPrint(_T("The point's new out-tangent is: [%g, %g]\n"), cPt.out.x, cPt.out.y);
			DebugPrint(_T("The point's tangent flags are:\n"));
			DebugPrint(_T("  CURVEP_BEZIER   - %d\n"), (cPt.flags && CURVEP_BEZIER)   ? 1 : 0);
			DebugPrint(_T("  CURVEP_CORNER   - %d\n"), (cPt.flags && CURVEP_CORNER)   ? 1 : 0);
			DebugPrint(_T("  CURVEP_LOCKED_Y - %d\n"), (cPt.flags && CURVEP_LOCKED_Y) ? 1 : 0);
			DebugPrint(_T("  CURVEP_LOCKED_X - %d\n"), (cPt.flags && CURVEP_LOCKED_X) ? 1 : 0);
			DebugPrint(_T("  CURVEP_SELECTED - %d\n"), (cPt.flags && CURVEP_SELECTED) ? 1 : 0);
			DebugPrint(_T("  CURVEP_ENDPOINT - %d\n"), (cPt.flags && CURVEP_ENDPOINT) ? 1 : 0);
			break;
		...
		case WM_CC_DEL_CURVEPT:
			...
			pCurve = (ICurve *)lParam;
			ptIndex = LOWORD(wParam);
			DebugPrint(_T("For curve at address: %p, curve point %d was deleted.\n"), pCurve, ptIndex);
			break;
		...
		case WM_COMMAND:
			...
			else if ( (LOWORD(wParam) == IDC_SEL_BRT_CURVE) && (HIWORD(wParam) == BN_CLICKED) )
			{
				...
				mpMainDlgProc->getBlurMgr()->getCCtrl()->SetDisplayMode(ba);
				break;
			}
