...
void SelLum::notifyPrmChanged(ParamID prmID)
{
	switch (prmID)
	{
		case prmLumMin:
...
bool SelLum::doSelect( BitmapAccessor &bmFrom )
{
	BitmapColorAccessor& mapFrom = bmFrom.map;
	...
	{
		srcIndex = mapIndex*3;
		lum = luminanceNormFloat(mapFrom[srcIndex], mapFrom[srcIndex+1], mapFrom[srcIndex+2]); // grabs specular hilights better than max(r,g,b)
		...
		for(p->v = p->startv; p->v <= p->Maxv; p->v += p->step){
			for (p->u =- p->m_feathRad; p->u <= p->m_feathRad; p->u++){
				...
				if (p->mp_radMap[p->index] != 0.0f){
					...
					p->mp_radMap[p->index] = std::min(p->mp_radMap[p->index], p->m_normRadMask[p->normRadIdx]);
				}
			} // for u
		}// for i
			...
			for (int v=-m_feathRad; v<=m_feathRad; v++)
			{
				for (int u=-m_feathRad; u<=m_feathRad; u++)
				{
					...
					if (mp_radMap[index] != 0.0f)
					{
						...
						mp_radMap[index] = std::min(mp_radMap[index], m_normRadMask[normRadIdx]);
					}
				}
			}
		}
...
bool SelLum::doComposite(TimeValue t, CompMap &compMap)
{
	for (int mapIndex=0; mapIndex<m_imageSz; mapIndex++)
	{
		...
		{
			compMap[mapIndex].brighten = std::max( compMap[mapIndex].brighten, calcBrighten(t, mp_radMap[mapIndex]) );
			compMap[mapIndex].blend = std::max( compMap[mapIndex].blend, calcBlend(t, mp_radMap[mapIndex]) );
		}
	}
	return true;
}
...
void SelLum::select(TimeValue t, CompMap &compMap, Bitmap *bm, RenderGlobalContext *gc)
{
	...
	if (!m_featherValid)
	{
		...
		m_feathRad = (int)floor(std::max(m_imageW, m_imageH)*(fTemp*PERCENT2DEC));
