...
void SelMatIds::notifyPrmChanged(ParamID prmID)
{
	switch (prmID)
	{
		case prmMatIdsIds:
...
bool SelMatIds::doSelect( BitmapAccessor &bmFrom )
{
	...
	for (int mapIndex = 0; mapIndex < m_imageSz; mapIndex++)
	{
		...
		if (IdInList(mp_srcMatId[mapIndex]))
		{
			lum = luminanceNormFloat(mp_srcColor[mapIndex]);
