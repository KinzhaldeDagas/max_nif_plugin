...
protected:
	...
	// render effects dialog, these curve controls will be switched via SetThing() in the MainDlgProc class.
	ICurveCtl *mp_CCtl;
	...
	ParamID m_selActiveIds[numSels];
...
public:
	...
	IParamBlock2 *pbMain, *pbBlurData, *pbSelData;

	// parammaps are shared among instances
	static IParamMap2 *pmMain;
	...
	static ParamBlockDesc2 pbdMain;
	...
	BOOL getBlurValue(ParamID id, TimeValue t, ValueType& v, Interval&&, int tabIndex = 0) { Interval valid = FOREVER; return getBlurValue(id, t, v, valid, tabIndex); }
	...
	template <class ValueType>
	BOOL getSelValue(ParamID id, TimeValue t, ValueType& v, Interval&&, int tabIndex = 0) { Interval valid = FOREVER; return getSelValue(id, t, v, valid, tabIndex); }
...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_CLASS_NAME) : _T("Blur"); }
	...
	// Effect
	TSTR GetName(bool localized) const override { return localized ? GetString(IDS_NAME) : _T("Blur"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Blur"); }
