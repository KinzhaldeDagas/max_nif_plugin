...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_FILE_OUT) : _T("File Output"); }
	...
	// Effect
	TSTR GetName(bool localized) const override { return localized ? GetString(IDS_FILE_OUT) : _T("File Output"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("File Output"); }
...
static ParamBlockDesc2 pbdFOut(pbIDFOut, _T("fileOut parameters"), 0, &fileOutCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	...
	prmActive, _T("active"), TYPE_BOOL, P_ANIMATABLE, IDS_ACTIVE,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDB_ACTIVE,
		p_end,
	prmAffectSrc, _T("affectSource"), TYPE_BOOL, P_ANIMATABLE, IDS_AFFECT_SRC,
		p_default, FALSE,
		p_ui, TYPE_SINGLECHECKBOX, IDB_AFFECT_SRC,
		p_nonLocalizedName, _T("Affect Source"),
		p_end,
	...
	prmCameraNode, _T("cameraNode"), TYPE_INODE, 0, IDS_CAM_NODE,	// not animatable
		...
		p_nonLocalizedName, _T("Camera Node"),
		p_end,
	prmNearZ, _T("nearZ"), TYPE_FLOAT, P_ANIMATABLE, IDS_NEARZ,
		...
		p_nonLocalizedName, _T("Near Z Depth"),
		p_end,
	prmFarZ, _T("farZ"), TYPE_FLOAT, P_ANIMATABLE, IDS_FARZ,
		...
		p_accessor, &fOutAccessor,
		p_nonLocalizedName, _T("Far Z Depth"),
		p_end,
	prmFitScene, _T("fitScene"), TYPE_BOOL, P_ANIMATABLE, IDS_FIT_SCENE,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDB_FIT_SCENE,
		p_nonLocalizedName, _T("Fit Entire Scene"),
		p_end,
	p_end
	);
		...
		if(IO)
		{
			if (pBi->GetPiData())
			{
				if (!IO->ValidatePiData(pBi))
				  ...
				  IO->LoadConfigure(pBi->GetPiData(), pBi->GetPiDataSize());
			}
...
INT_PTR FileOutDlgProc::DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
...
TSTR FileOut::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_FILE_OUT_PARAMS) : _T("File Output Parameters");
	default: return _T("");
	}
}
...
RefResult FileOut::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
	static ParamID prmID = -1;
	switch (message)
	{
		case REFMSG_CHANGE:
			prmID = mp_pblock->LastNotifyParamID();
			switch (prmID)
			{
				case prmCameraNode:
				{
...
void FileOut::processImage()
{
	...
	int srcType, srcAlphaType, destType, destAlphaType;
	mp_srcMap = (WORD*)mp_srcBM->GetStoragePtr(&srcType);
	mp_srcAlpha = (WORD*)mp_srcBM->GetAlphaPtr(&srcAlphaType);
	WORD *writeMap = (WORD*)mp_writeBM->GetStoragePtr(&destType);
	WORD *writeAlpha = (WORD*)mp_writeBM->GetAlphaPtr(&destAlphaType);
	// Only access the storage directly if it's of the correct type.
	if(srcType != BMM_TRUE_48)
		mp_srcMap = NULL;
	if(srcAlphaType != BMM_GRAY_16)
		mp_srcAlpha = NULL;
	if(destType != BMM_TRUE_48)
		writeMap = NULL;
	if(destAlphaType != BMM_GRAY_16)
		writeAlpha = NULL;
	...
	else if(mp_srcBM->IsHighDynamicRange() || mp_writeBM->IsHighDynamicRange()) {

		BMM_Color_fl* buf = new BMM_Color_fl[mp_srcBM->Width()];
...
void FileOut::processLum()
{
	...
	if (m_writing)
	{
		int srcAlphaType, destType, destAlphaType;
		mp_srcAlpha = (WORD*)mp_srcBM->GetAlphaPtr(&srcAlphaType);
		WORD *writeMap = (WORD*)mp_writeBM->GetStoragePtr(&destType);
		WORD *writeAlpha = (WORD*)mp_writeBM->GetAlphaPtr(&destAlphaType);
		// Only access the storage directly if it's of the correct type.
		...
		if(mp_srcMap&&mp_srcAlpha &&writeMap &&writeAlpha)
		{
			...
			for(int i=0;i<mp_srcBM->Height();i++)
			{
				...
				{
					buf[j].r = buf[j].g = buf[j].b = (buf[j].r*0.2125f + buf[j].g*0.7154f + buf[j].b*0.0721f); // r = g = b = source luminance
				}
				mp_writeBM->PutPixels(0,i,mp_srcBM->Width(),buf);
				if (m_affectSrc)
	...
	else if (m_affectSrc)
	{
		...
		else if(mp_srcBM->IsHighDynamicRange()) {
...
void FileOut::processDepth()
{
	...
	DWORD zType;
	mp_zBuf = (float *)mp_srcBM->GetChannel(BMM_CHAN_Z, zType);
	if (!mp_zBuf)
		return;
		...
		if(writeMap&&writeAlpha&&mp_srcMap&&mp_srcAlpha)
		{
			...
			for(int i=0;i<mp_srcBM->Height();i++)
			{
				...
				for(int j=0;j<mp_srcBM->Width();j++)
				{
					...
					if ( (z < 0.0f) || (z > 1.0f) )
						z = 1.0f; // values outside of range set to black
					buf[j].r = buf[j].g = buf[j].b = (1.0f-z); // r = g = b = normalized z
				}
				mp_writeBM->PutPixels(0,i,mp_srcBM->Width(),buf);
				if (m_affectSrc)
					mp_srcBM->PutPixels(0,i,mp_srcBM->Width(),buf);
...
static ItemType GetNextItem(const TCHAR *s, int& nextc, int &n1, int &n2 )
{
	BOOL gotN1 = FALSE;
	BOOL gotN2 = FALSE;
	int i = nextc;
...
static bool ParseFrameList(const TCHAR *s, IntTab& nums)
{
	int n1, n2, nc(0);
	nums.SetCount(0);
	while (s[nc] != 0)
...
bool FileOut::SupportsBitmap(Bitmap& bitmap) {

	// All bitmaps are supported sine the storage is never directly accessed.
	return true;
}
