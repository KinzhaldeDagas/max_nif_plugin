...
BOOL WINAPI DllMain(HINSTANCE hDLLInst, DWORD fdwReason, LPVOID lpvReserved) {
   ...
   if( fdwReason == DLL_PROCESS_ATTACH )
   {
      ...
      hInst = hDLLInst;				// Hang on to this DLL's instance handle.
...
class FGClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName()		{ return _T("Film Grain"); }
...
TSTR FilmGrain::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_PARAMETERS) : _T("Parameters");
	default: return _T("");
	}
}
...
RefResult FilmGrain::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,PartID& partID, RefMessage message, BOOL propagate) {
	switch (message) {
		case REFMSG_CHANGE:
...
static BOOL WINAPI fxCallBack(LPVOID lpparam, int /*done*/, int /*total*/, const TCHAR* /*msg*/) {
