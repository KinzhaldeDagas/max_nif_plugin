...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_MP_DOF) : _T("Depth of Field"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Depth of Field"); }
...
INT_PTR MultiPassDOFDialogHandler::DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, 
										UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG: {
		...
		EnableWindow(GetDlgItem(hWnd, IDSP_FOCAL_DEPTH), p_end);
		break;
	}
	case WM_COMMAND:
...
static ParamBlockDesc2 multiPassDOF_param_blk(multiPassDOF_params, _T("multiPassDOF parameters"), 0, &multiPassDOFClassDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	...
	prm_useTargetDistance, _T("useTargetDistance"), TYPE_BOOL, P_ANIMATABLE, IDS_USE_TARGET_DIST,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_USE_TARGET_DIST,
		p_nonLocalizedName, _T("Use Target Distance"),
		p_end,
	prm_focalDepth, _T("focalDepth"), TYPE_FLOAT, P_ANIMATABLE, IDS_FOCAL_DEPTH,
		...
		p_nonLocalizedName, _T("Focal Depth"),
		p_end,
	prm_displayPasses, _T("displayPasses"), TYPE_BOOL, P_ANIMATABLE, IDS_DISPLAY_PASSES,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_DISPLAY_PASSES,
		p_nonLocalizedName, _T("Display Passes"),
		p_end,
	prm_useOriginalLocation, _T("useOriginalLocation"), TYPE_BOOL, P_ANIMATABLE, IDS_USE_ORIG_LOCATION,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_USE_ORIG_LOCATION,
		p_nonLocalizedName, _T("Use Original Location"),
		p_end,
	prm_totalPasses, _T("totalPasses"), TYPE_INT, P_ANIMATABLE, IDS_TOTAL_PASSES,
		...
		p_nonLocalizedName, _T("Total Passes"),
		p_end,
	prm_sampleRadius, _T("sampleRadius"), TYPE_FLOAT, P_ANIMATABLE, IDS_SAMPLE_RADIUS,
		...
		p_dim, 			stdWorldDim,
		p_nonLocalizedName, _T("Sample Radius"),
		p_end,
	prm_sampleBias, _T("sampleBias"), TYPE_FLOAT, P_ANIMATABLE, IDS_SAMPLE_BIAS,
		...
		p_range, 0.0f, 1.0f,
		p_nonLocalizedName, _T("Sample Bias"),
		p_end,
	prm_normalizeWeights, _T("normalizeWeights"), TYPE_BOOL, P_ANIMATABLE, IDS_NORMALIZE_WEIGHTS,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_NORMALIZE_WEIGHTS,
		p_nonLocalizedName, _T("Normalize Weights"),
		p_end,
	prm_ditherStrength, _T("ditherStrength"), TYPE_FLOAT, P_ANIMATABLE, IDS_DITHER_STRENGTH,
		...
		p_nonLocalizedName, _T("Dither Strength"),
		p_end,
	prm_tileSize, _T("tileSize"), TYPE_INT, P_ANIMATABLE, IDS_TILE_SIZE,
		...
		p_nonLocalizedName, _T("Tile Size"),
		p_end,
	prm_disableFiltering, _T("disableFiltering"), TYPE_BOOL, P_ANIMATABLE, IDS_DISABLE_FILTERING,
		p_default, FALSE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_DISABLE_FILTERING,
		p_nonLocalizedName, _T("Disable Filtering"),
		p_end,
	prm_disableAntialiasing, _T("disableAntialiasing"), TYPE_BOOL, P_ANIMATABLE, IDS_DISABLE_ANTIALIASING,
		p_default, FALSE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_DISABLE_ANTIALIASING,
		p_nonLocalizedName, _T("Disable Antialiasing"),
		p_end,
	p_end
	);
...
TSTR
MultiPassDOF::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_MP_DOF_PARAMS) : _T("Depth of Field Parameters");
	default: return _T("");
	}
}
...
MultiPassDOF::Clone(RemapDir &remap)
{
	...
	pNewMultiPassDOF->ReplaceReference( 0, remap.CloneRef(mpPBlock) );
...
RefResult
MultiPassDOF::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message)
	{
		case REFMSG_CHANGE:
...
MultiPassDOF::Apply(INode *pCameraNode, CameraObject *pCameraObject, int passNum, TimeValue &overrideRenderTime)
{
	...
	if (passNum == 0)
	{
		...
		pCameraObject->EvalCameraState(overrideRenderTime, valid, &cameraState);
		originalCameraTM = pCameraNode->GetObjTMAfterWSM(overrideRenderTime);
	...
	{
		// First, build a transform matrix that describes only the offset of the camera
		...
		Point3 newCameraLocation = Point3(uOffset, vOffset, 0.0f);
		Point3 initialTargetLocation = Point3(0.0f, 0.0f, -focalDepth);
		Point3 newNVector = (newCameraLocation - initialTargetLocation).Unify();
		Point3 newUVector = CrossProd(Point3(0,1,0), newNVector).Unify();
		Point3 newVVector = CrossProd(newNVector, newUVector).Unify();
		Matrix3 offsetMatrix(newUVector, newVVector, newNVector, newCameraLocation);

		// Then, pre-transform the orignal transform matrix by the offset matrix
        viewParams.affineTM = offsetMatrix * originalCameraTM;
	}
