...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_MP_MOT_BLUR) : _T("Motion Blur"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Motion Blur"); }
...
TSTR
MultiPassMotionBlur::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_MP_MOT_BLUR_PARAMS) : _T("Motion Blur Parameters");
	default: return _T("");
	}
}
...
MultiPassMotionBlur::Clone(RemapDir &remap)
{
	...
	pNewMultiPassMotionBlur->ReplaceReference( 0, remap.CloneRef(mpPBlock) );
...
RefResult
MultiPassMotionBlur::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message)
	{
		case REFMSG_CHANGE:
