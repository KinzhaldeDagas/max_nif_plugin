...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Luminance Render Element"); }
...
void LuminationRenderElement::BaseClone(
	...
	)
{
	...
	MaxRenderElement::BaseClone(from, to, remap);

	LuminationRenderElement* fromMap = static_cast< LuminationRenderElement* >( from );
	LuminationRenderElement* toMap = static_cast< LuminationRenderElement* >( to );

	toMap->ReplaceReference( LUM_PARAM_REF_NO, remap.CloneRef(fromMap->pParamBlk) );
}
...
void LuminationRenderElement::SetElementName( const TCHAR* pStr ) 
{
...
void LuminationRenderElement::PostIllum(ShadeContext& sc, IllumParams& ip)
{
    ...
    if(sc.ScreenCoord().y < (sc.globContext->devHeight * PERCENTAGE_TO_KEEP))
    {
        ...
        if((sc.globContext != NULL) && (sc.globContext->pToneOp != NULL))
            sc.globContext->pToneOp->ScaledToRGB(lum);
    }
    else
    {
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Illuminance Render Element"); }
	SClass_ID		SuperClassID() { return RENDER_ELEMENT_CLASS_ID; }
...
void IlluminationRenderElement::BaseClone(
	ReferenceTarget*  from,
	ReferenceTarget*  to,
	RemapDir&         remap
	)
{
	...
	IlluminationRenderElement* fromMap = static_cast< IlluminationRenderElement* >( from );
	IlluminationRenderElement* toMap = static_cast< IlluminationRenderElement* >( to );

	toMap->ReplaceReference( ILLUM_PARAM_REF_NO, remap.CloneRef(fromMap->pParamBlk) );
}
...
void IlluminationRenderElement::SetElementName( const TCHAR* pStr ) 
{
    pParamBlk->SetValue( eleName, 0, pStr );
}
    ...
    if(sc.ScreenCoord().y < (sc.globContext->devHeight * PERCENTAGE_TO_KEEP))
    {
        ...
        if(ip.pMtl != NULL) {
            ...
            for (int i=0; i<sc.nLights; i++) 
            {
                ...
                if (l->Illuminate(sc, normal, illumIncr, dummyDir,
        ...
        // Scale color to RGB using the tone operator
        if((sc.globContext != NULL) && (sc.globContext->pToneOp != NULL))
            sc.globContext->pToneOp->ScaledToRGB(illum);
    }
    else
    {
        int iBitmapLegendY = sc.ScreenCoord().y - (sc.globContext->devHeight * PERCENTAGE_TO_KEEP);
