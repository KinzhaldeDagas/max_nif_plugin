...
static int GetNumberOfPseudoColors()
{
	static WORD LanguageID = PRIMARYLANGID(MaxSDK::Util::GetLanguageID());

	if (LanguageID == LANG_JAPANESE)
	{
		return 119;    // Should be at least >= 50
	}
	else
	{
		return 100;    // Should be at least >= 50
	}
}
