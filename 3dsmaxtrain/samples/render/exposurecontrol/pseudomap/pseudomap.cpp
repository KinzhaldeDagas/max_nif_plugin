...
public:
	...
	const TCHAR* NonLocalizedClassName() { return _T("Pseudo Color Exposure Control"); }
...
public:
    ...
    virtual void ScaledToRGB(float energy[3]) const;
    virtual float ScaledToRGB(float energy) const;
    
    virtual float GetPhysicalUnit(TimeValue t, Interval& valid) const;
    ...
    virtual TSTR GetName(bool localized) const override { return name; }
...
public:
    ...
    virtual void Update(const TimeValue t, INode* const /*cameraNode*/, const IPoint2& /*renderResolution*/, const float /*pixelAspectRatio*/, Interval& validity) override { Update(t, validity); }
    virtual void ScaledToRGB(float energy[3], const Point2& /*xyCoord*/) const override { ScaledToRGB(energy); }
    virtual float ScaledToRGB(float energy, const Point2& /*xyCoord*/) const override { return ScaledToRGB(energy); }
    virtual const ThreadedProcessor* AllocateThreadedProcessor() const override { return nullptr; }
    virtual bool CanInvert() const override { return false; }
    virtual void InverseMap(float rgb[3], const Point2& /*xyCoord*/) const override { InverseMap(rgb); }
    virtual float InverseMap(float rgb, const Point2& /*xyCoord*/) const override { return InverseMap(rgb); }
    virtual void InverseMap(float rgb[3]) const override { }
    virtual float InverseMap(float rgb) const override { return rgb; }
...
private:
	...
	virtual void BindParameter(const MCHAR * paramName, LPVOID value);
	virtual void BindParameter(const TimeValue t, const MCHAR * paramName, LPVOID value);
	virtual void InitializeBinding(){}
...
private:
	...
	IRTShaderNode * m_pRTShaderNode;  //For viewport tone mapping
...
public:
    ...
    INT_PTR DialogProc(TimeValue t, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
...
ParamBlockDesc2 mainPBDesc2(
    ...
    // Parameters
        ...
        PseudoExposureControl::PB_SCALE, _T("physicalScale"), TYPE_FLOAT, P_ANIMATABLE, IDS_SCALE,
            ...
            p_nonLocalizedName, _T("Physical Scale"),
        p_end,

        // Automatic
        PseudoExposureControl::PB_AUTOMATIC, _T("automatic"), TYPE_BOOL, 0, IDS_AUTOMATIC,
            p_default, false,
            p_ui, TYPE_SINGLECHECKBOX, IDC_MAIN_AUTOMATIC,
        p_end,
  ...
  mpShaderManager(NULL)
{
    if(!loading) {
        ...
        SpecialFX::Update(GetCOREInterface()->GetTime());
    }

	m_pRTShaderNode= IRTShaderManager::GetRTShaderManager()->AddShaderNode(this);
}
...
void PseudoExposureControl::GetClassName(MSTR& s, bool localized) const
{
    s = localized ? GetString(IDS_PSEUDO_EXP_CTL_CLASS_NAME) : _T("Pseudo Color Exposure Control");
}
...
void PseudoExposureControl::buildColorTable()
{
    ...
    if (dsp == DSP_COLORED) 
    {
        Color col;
        int   hue; 
        float deltaHue = -COLOR_TABLE_HUE_RANGE / GetNumberOfPseudoColors();

        for(int i = 0; i < GetNumberOfPseudoColors(); i++) {
    ...
    else 
    {
        ...
        ULONG a;
        float delta = COLOR_TABLE_NUMBER_OF_GRAY / GetNumberOfPseudoColors();
    ...
    _conversionFactor *= GetNumberOfPseudoColors();
    /* used for log10 mapping */
    _cfB = 1.0 / (1.0 + _minBrightness);
    _cfA = GetNumberOfPseudoColors();
...
TSTR PseudoExposureControl::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case REF_PARAMS: return localized ? GetString(IDS_PSEUDO_EXP_CTL_CLASS_NAME) : _T("Pseudo Color Exposure Control");
	case REF_RENDER_ELEMENT: return (_currentRenderElement != NULL) ? _currentRenderElement->GetName(localized) : MSTR(_T(""));
	default: return _T("");
	}
}
                                      ...
                                      )
{
    ...
    toMap->ReplaceReference( REF_PARAMS, remap.CloneRef(fromMap->_params) );
    toMap->ReplaceReference( REF_RENDER_ELEMENT, remap.CloneRef(fromMap->_currentRenderElement) );
}
...
void PseudoExposureControl::ScaledToRGB( float* radiance ) const
{
    if ( _active ) {
        int i;
        float brightness;
        ...
        if (i < 0) 
            i = 0;
        else if (i >= GetNumberOfPseudoColors()) 
            i = GetNumberOfPseudoColors() - 1;
...
void PseudoExposureControl::GetEnergyRangeFromIndex(int colorIndex, float& floor, float& ceiling) {
    ...
    if((colorIndex < 0) || (colorIndex >= GetNumberOfPseudoColors())) {
        floor = ceiling = 0;
    }
    else {
        ...
        if(_scaleFunction == SCALE_LOGARITHMIC) {
            floor = (pow(10.0f, floorIndex / _cfA) / _cfB) - 1.0f;
            ceiling = (pow(10.0f, ceilingIndex / _cfA) / _cfB) - 1.0f;
        }
        else {
...
float PseudoExposureControl::ScaledToRGB(float energy) const
{
    if ( _active ) {
...
{
    assert(iSizeOf >= sizeof(ULONG)*GetNumberOfPseudoColors());
...
bool PseudoExposureControl::GetParameterValue(
	const TimeValue t, 
	const MCHAR* shaderParamName, 
	void* value, 
	MaxSDK::Graphics::IParameterTranslator::ShaderParameterType shaderParamType)
{
	bool result = true;
	if(_tcscmp(shaderParamName,_M("active"))==0)
	{
		*(BOOL*)value = Active( t ) != 0;
	}
	else if(_tcscmp(shaderParamName,_M("maximum"))==0)
	{
		float maxBrightnessParam;
		_params->GetValue(PB_MAXIMUM, t, maxBrightnessParam, FOREVER);
		*(float*)value = maxBrightnessParam;
	}
	else if(_tcscmp(shaderParamName,_M("minimum"))==0)
	{
		float minBrightnessParam;
		_params->GetValue(PB_MINIMUM, t, minBrightnessParam, FOREVER);
		*(float*)value = minBrightnessParam;
	}
	else if(_tcscmp(shaderParamName,_M("display"))==0)
	{
		int displayParam;
		_params->GetValue(PB_DISPLAY, t, displayParam, FOREVER);
		*(int*)value = displayParam;
	}
	else if(_tcscmp(shaderParamName,_M("physicalscale"))==0)
	{
		float physScaleParam;
		_params->GetValue(PB_SCALE, t, physScaleParam, FOREVER);
		*(float*)value = physScaleParam;
	}
	else if(_tcscmp(shaderParamName,_M("quantity"))==0)
	{
		int quantityParam;
		quantityParam = static_cast<IToneOperatorExtension::Quantity>(_params->GetInt(PB_QUANTITY, t));
		*(int*)value = quantityParam;
	}
	else if(_tcscmp(shaderParamName,_M("scalefunction"))==0)
	{
		int scaleFunctionParam;
		scaleFunctionParam = static_cast<ScaleFunction>(_params->GetInt(PB_SCALE_FUNC, t));
		*(int*)value = scaleFunctionParam;
	}
	else if(_tcscmp(shaderParamName,_M("unitsystemused"))==0)
	{
		int unitSystemUsed = _params->GetInt(PB_UNITSYSTEM_USED);
		*(int*)value = unitSystemUsed;
	}
	else if(_tcscmp(shaderParamName,_M("automatic"))==0)
	{
		BOOL  automaticUsed = GetAutomatic();
		*(BOOL*)value = automaticUsed;
	}
	else if(_tcscmp(shaderParamName,_M("processBG"))==0)
	{
		// To do: use false as default temp
		const BOOL  processBG = false;
		*(BOOL*)value = processBG;
	}
	else if(_tcscmp(shaderParamName,_M("nbcolors"))==0)
	{
		// To do: use 100 as default temp
		const int  nbcolors = 100;
		*(int*)value = nbcolors;
	}
	else if (_tcscmp(shaderParamName,_M("env_tex"))==0)
	{
		result = false;
	}
	else
...
bool PseudoExposureControl::GetShaderInputParameterName(
	SubMtlBaseType type, 
	int subMtlBaseIndex, 
...
bool PseudoExposureControl::OnPreParameterTranslate()
{
	return true;
}
                                      ...
                                      )
{
   ...
   if ( gc && !gc->inMtlEdit )
	   BroadcastNotification<NOTIFY_BEGIN_RENDERING_TONEMAPPING_IMAGE>(gc);
...
BaseInterface* PseudoExposureControl::GetInterface(Interface_ID id) {
	...
	else if (ISHADER_MANAGER_INTERFACE_ID == id)
	{
...
void PseudoExposureControl::SetCurrentRenderElement(bool setToNone) {
	...
	// First synchronize the elements to the state of the exposure control
	{
		...
		int n = (NULL != rendElemMgr)?rendElemMgr->NumRenderElements():0;
		while (--n >= 0) {
...
void PseudoExposureControl::BindParameter(const MCHAR * paramName, LPVOID value)
{
	TimeValue t = GetCOREInterface()->GetTime();
	BindParameter(t, paramName, value);
}

void PseudoExposureControl::BindParameter(const TimeValue t, const MCHAR * paramName, LPVOID value)
{
	if(_tcscmp(paramName,_M("msl_max_ToneOpPseudo_1_active"))==0)
	{
		*(BOOL*)value = Active( t ) != 0;
	}
	else if(_tcscmp(paramName,_M("msl_max_ToneOpPseudo_1_maximum"))==0)
	{
		float maxBrightnessParam;
		_params->GetValue(PB_MAXIMUM, t, maxBrightnessParam, FOREVER);
		*(float*)value = maxBrightnessParam;
	}
	else if(_tcscmp(paramName,_M("msl_max_ToneOpPseudo_1_minimum"))==0)
	{
		float minBrightnessParam;
		_params->GetValue(PB_MINIMUM, t, minBrightnessParam, FOREVER);
		*(float*)value = minBrightnessParam;
	}
	else if(_tcscmp(paramName,_M("msl_max_ToneOpPseudo_1_display"))==0)
	{
		int displayParam;
		_params->GetValue(PB_DISPLAY, t, displayParam, FOREVER);
		*(int*)value = displayParam;
	}
	else if(_tcscmp(paramName,_M("msl_max_ToneOpPseudo_1_physicalscale"))==0)
	{
		float physScaleParam;
		_params->GetValue(PB_SCALE, t, physScaleParam, FOREVER);
		*(float*)value = physScaleParam;
	}
	else if(_tcscmp(paramName,_M("msl_max_ToneOpPseudo_1_quantity"))==0)
	{
		int quantityParam;
		quantityParam = static_cast<IToneOperatorExtension::Quantity>(_params->GetInt(PB_QUANTITY, t));
		*(int*)value = quantityParam;
	}
	else if(_tcscmp(paramName,_M("msl_max_ToneOpPseudo_1_scalefunction"))==0)
	{
		int scaleFunctionParam;
		scaleFunctionParam = static_cast<ScaleFunction>(_params->GetInt(PB_SCALE_FUNC, t));
		*(int*)value = scaleFunctionParam;
	}
	else if(_tcscmp(paramName,_M("msl_max_ToneOpPseudo_1_unitsystemused"))==0)
	{
		int unitSystemUsed = _params->GetInt(PB_UNITSYSTEM_USED);
		*(int*)value = unitSystemUsed;
	}
	else if(_tcscmp(paramName,_M("msl_max_ToneOpPseudo_1_automatic"))==0)
	{
		BOOL  automaticUsed = GetAutomatic();
		*(BOOL*)value = automaticUsed;
	}
	else if(_tcscmp(paramName,_M("msl_max_ToneOpPseudo_1_processBG"))==0)
	{
		// To do: use false as default temp
		const BOOL  processBG = false;
		*(BOOL*)value = processBG;
	}
	else if(_tcscmp(paramName,_M("msl_max_ToneOpPseudo_1_nbcolors"))==0)
	{
		// To do: use 100 as default temp
		const int  nbcolors = 100;
		*(int*)value = nbcolors;
...
MaxSDK::Graphics::IShaderManager* PseudoExposureControl::GetShaderManager()
{
	MSTR rootPathName = GetCOREInterface()->GetDir(APP_TEMP_DIR);
	MSTR fullName = rootPathName + MSTR(_M("NVIDIA\\shaders_standard\\metaSL\\XMSL\\max_ToneOpPseudo.xmsl"));
...
INT_PTR PseudoExposureControlDialog::DialogProc(TimeValue t, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
            ...
            case IDC_COLOR_LEGEND:
                {
                    ...
                    if((point.x >= 0) && (point.y >= 0)) {
                        ...
                        if(ImageList_GetImageInfo(m_hImageList, 0, &imageInfo)) {
                            ...
                            int colorWidth = m_legendWidth / GetNumberOfPseudoColors();
                            int gapSize = m_legendWidth - (colorWidth * GetNumberOfPseudoColors());
                            ...
                            if(point.x < startGapSize)
                                ...
                                colorIndex = GetNumberOfPseudoColors() - 1;
                            else
					...
					for (int i = des->count; --i >= 0; ) {
						...
						if (!(pd.flags & P_RESET_DEFAULT)) {
							switch (pd.type) {
								...
								case TYPE_STRING: {
									const TCHAR* s = pb->GetStr(pd.ID, t);
									if (s != NULL) {
										if (pd.cur_def.s != NULL)
											free((void*)pd.cur_def.s);
...
void PseudoExposureControlDialog::UpdateLegendBitmap(unsigned long* pseudoColorMap) {
        ...
        && (m_legendHeight > 0) && (m_hWnd != NULL) && (m_hImageList != NULL) ) 
    {
        ...
        // control
        DbgAssert(GetNumberOfPseudoColors() <= m_legendWidth);
...
void PseudoExposureControlDialog::UpdateLegendValues() {
    ...
    if((mp_mainParamMap != NULL) && (mp_exposureControl != NULL)) {
        ...
        mp_exposureControl->GetEnergyRangeFromIndex(GetNumberOfPseudoColors() / 4, min, max);
        avg = (min + max) / 2.0f;
        _sntprintf(buf, 12, formatString, avg);
        SetDlgItemText(hWnd, IDC_LEGEND_25, buf);

        mp_exposureControl->GetEnergyRangeFromIndex(GetNumberOfPseudoColors() / 2, min, max);
        avg = (min + max) / 2.0f;
        _sntprintf(buf, 12, formatString, avg);
        SetDlgItemText(hWnd, IDC_LEGEND_50, buf);

        mp_exposureControl->GetEnergyRangeFromIndex(GetNumberOfPseudoColors() * 3 / 4, min, max);
        avg = (min + max) / 2.0f;
        _sntprintf(buf, 12, formatString, avg);
        SetDlgItemText(hWnd, IDC_LEGEND_75, buf);

        mp_exposureControl->GetEnergyRangeFromIndex(GetNumberOfPseudoColors() - 1, min, max);
