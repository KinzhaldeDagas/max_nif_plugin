...
static float CLAMP(float x) { return std::clamp(x, 0.f, 1.f); }

static const TCHAR* shadowIllumOutStr = _T("shadowIllumOut");
static const TCHAR* inkIllumOutStr = _T("Ink");
static const TCHAR* paintIllumOutStr = _T("Paint");
...
namespace {
	...
	// Utility function to turn off gamma correction on non-color maps
	void TurnOffGammaCorrection(PBBitmap* pPBBitmap)
	{
		if (pPBBitmap)
		{
			pPBBitmap->bi.ResetCustomFlag(BMM_CUSTOM_FILEGAMMA);
			pPBBitmap->bi.SetCustomFlag(BMM_CUSTOM_GAMMA);
			pPBBitmap->bi.SetCustomGamma(1.0f);
		}
	}
...
public:
		...
		TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_DIFFUSE_BAKE_ELEMENT) : _T("DiffuseMap"); }
		...
		const int FindParamByName1( const TCHAR*  name ) { 
			if( _tcscmp( name, GetString( paramNames[0] )) == 0 )
				return 1;
			if( _tcscmp( name, GetString( paramNames[1] )) == 0 )
				return 2;
			return 0; 
		}
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("DiffuseMap"); }
...
DiffuseBakeElement::DiffuseBakeElement()
{
	...
	SetElementName( GetName(true).data());
	mpRenderBitmap = NULL;
}
...
public:
		...
		TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_SPECULAR_BAKE_ELEMENT) : _T("SpecularMap"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("SpecularMap"); }
	SClass_ID		SuperClassID() { return BAKE_ELEMENT_CLASS_ID; }
	Class_ID 		ClassID() { return specularBakeElementClassID; }
	const TCHAR* 	Category() { return _T(""); }
...
public:
		...
		TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_LIGHT_BAKE_ELEMENT) : _T("LightingMap"); }
...
//		void PostAtmosphere(ShadeContext& sc, float z, float zPrev);
		...
		const int FindParamByName1( const TCHAR*  name ) { 
			for( int i = 0; i < GetNParams(); ++i ){
				if( _tcscmp( name, GetString( paramNames[i] ) ) == 0 )
					return i+1;
			}
			return 0; 
		}
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("LightingMap"); }
	SClass_ID		SuperClassID() { return BAKE_ELEMENT_CLASS_ID; }
	Class_ID 		ClassID() { return lightBakeElementClassID; }
	const TCHAR* 	Category() { return _T(""); }
...
static ParamBlockDesc2 lightBakeParamBlk(PBLOCK_NUMBER, _T("Lighting bake element parameters"), 0, &lightBakeCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	...
	LightBakeElement::directOn, _T("directOn"), TYPE_BOOL, 0, IDS_DIRECT_ILLUM_ON,
		p_ui, TYPE_SINGLECHECKBOX, IDC_DIRECT_ON,
		p_default, TRUE,
		p_end,
	LightBakeElement::indirectOn, _T("indirectOn"), TYPE_BOOL, 0, IDS_INDIRECT_ILLUM_ON,
		p_ui, TYPE_SINGLECHECKBOX, IDC_INDIRECT_ON,
		p_default, TRUE,
		p_end,
...
public:
		...
		TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_REFLECTREFRACT_BAKE_ELEMENT) : _T("FixedReflectRefractMap"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("FixedReflectRefractMap"); }
	SClass_ID		SuperClassID() { return BAKE_ELEMENT_CLASS_ID; }
	Class_ID 		ClassID() { return reflectRefractBakeElementClassID; }
	const TCHAR* 	Category() { return _T(""); }
...
public:
		...
		TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_SHADOW_BAKE_ELEMENT) : _T("ShadowsMap"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("ShadowsMap"); }
	SClass_ID		SuperClassID() { return BAKE_ELEMENT_CLASS_ID; }
	Class_ID 		ClassID() { return shadowBakeElementClassID; }
	const TCHAR* 	Category() { return _T(""); }
...
class NormalsBakeElement : public BaseBakeElement{
	public:
		...
		TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_NORMALS_BAKE_ELEMENT) : _T("NormalsMap"); }
		...
		const int FindParamByName1( const TCHAR*  name ) { 
			if( _tcscmp( name, GetString( paramNames[0] )) == 0 ) return 1;
			if( _tcscmp( name, GetString( paramNames[1] )) == 0 ) return 2;
			return 0; 
		}
		// currently only type 1 == boolean or 0 == undefined
		int  GetParamType1( int nParam ) { 
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("NormalsMap"); }
	SClass_ID		SuperClassID() { return BAKE_ELEMENT_CLASS_ID; }
	Class_ID 		ClassID() { return normalsBakeElementClassID; }
	const TCHAR* 	Category() { return _T(""); }
...
void NormalsBakeElement::PostIllum(ShadeContext& sc, IllumParams& ip)
{
	...
	IProjection_WorkingModelInfo* workingModelInfo = !render_projectionOn ? nullptr: GetWorkingModelInfo( sc );
	RenderInstance* workingModelInst = !render_projectionOn ? sc.globContext->GetRenderInstance( sc.NodeID() ) : (workingModelInfo ? workingModelInfo->GetRenderInstance() : nullptr);
	...
	default: //default is tangent space
		...
		if ( !render_projectionOn )
		{
			...
			GetVNormalMgr()->GetTangentBasis( sc, render_mapChannel, tangentBasis );
			t[0] = tangentBasis.uBasis;
			t[1] = tangentBasis.vBasis;
			if ( ( static_cast< MaxSDK::Graphics::INormalMappingManager* >( GetCOREInterface( NORMALMAPPINGMGR_INTERFACE ) )->GetTangentBasisMode() ) == MaxSDK::Graphics::kMikkT )
			{
				t[2] = tangentBasis.nBasis; //use un-normalized normal
			}
			else
			{
				t[2] = sc.OrigNormal(); //use normalized normal
			}
			if ( sc.backFace )
	...
	float a;
	if( render_useHeightAsAlpha && workingModelInfo )
...
void NormalsBakeElement::GetPBBitmap(PBBitmap* &pPBBitmap) const
{
	BaseBakeElement::GetPBBitmap(pPBBitmap);
	TurnOffGammaCorrection(pPBBitmap);
}
...
public:
		...
		TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_COMPLETE_BAKE_ELEMENT) : _T("CompleteMap"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CompleteMap"); }
	SClass_ID		SuperClassID() { return BAKE_ELEMENT_CLASS_ID; }
	Class_ID 		ClassID() { return completeBakeElementClassID; }
	const TCHAR* 	Category() { return _T(""); }
...
public:
		...
		TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_BLEND_BAKE_ELEMENT) : _T("BlendMap"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("BlendMap"); }
	SClass_ID		SuperClassID() { return BAKE_ELEMENT_CLASS_ID; }
	Class_ID 		ClassID() { return blendBakeElementClassID; }
	const TCHAR* 	Category() { return _T(""); }
...
static ParamBlockDesc2 blendBakeParamBlk(PBLOCK_NUMBER, _T("Blend bake element parameters"), 0, &blendBakeCD, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	...
	BlendBakeElement::ambientOn, _T("ambientOn"), TYPE_BOOL, 0, IDS_AMBIENT_ON,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_AMBIENT_ON,
		p_end,
	...
	BlendBakeElement::emissionOn, _T("emissionOn"), TYPE_BOOL, 0, IDS_EMISSION_ON,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_EMISSION_ON,
		p_end,
	...
	BlendBakeElement::refractionOn, _T("refractionOn"), TYPE_BOOL, 0, IDS_REFRACTION_ON,
		p_default, TRUE,
		p_ui, TYPE_SINGLECHECKBOX, IDC_REFRACTION_ON,
		p_end,
...
public:
		...
		TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_ALPHA_BAKE_ELEMENT) : _T("AlphaMap"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("AlphaMap"); }
	SClass_ID		SuperClassID() { return BAKE_ELEMENT_CLASS_ID; }
	Class_ID 		ClassID() { return alphaBakeElementClassID; }
	const TCHAR* 	Category() { return _T(""); }
...
void AlphaBakeElement::GetPBBitmap(PBBitmap* &pPBBitmap) const
{
	BaseBakeElement::GetPBBitmap(pPBBitmap);
	TurnOffGammaCorrection(pPBBitmap);
}
		...
		public BaseBakeElement
{
	public:
		...
		TSTR GetName(bool localized) const override { return localized ? GetString(IDS_KE_HEIGHT_BAKE_ELEMENT) : _T("HeightMap"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("HeightMap"); }
	SClass_ID		SuperClassID() { return BAKE_ELEMENT_CLASS_ID; }
	Class_ID 		ClassID() { return heightBakeElementClassID; }
	const TCHAR* 	Category() { return _T(""); }
...
void HeightBakeElement::GetPBBitmap(PBBitmap* &pPBBitmap) const
{
	BaseBakeElement::GetPBBitmap(pPBBitmap);
	TurnOffGammaCorrection(pPBBitmap);
}
...
class AmbientOcclusionBakeElement : public BaseBakeElement {
...
public:
	...
	TSTR		GetName(bool localized) const override { return localized ? GetString(IDS_AMBIENTOCCLUSION_BAKE_ELEMENT_UI) : _T("Ambient Occlusion"); }
	...
	// The BaseBakeElement implementation now returns TRUE, however the Ambient Occlusion Element is
	// an exception and should not be visible to scanline.
	BOOL IsVisibleToScanlineRenderer() override {
		return FALSE;
	}
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Ambient Occlusion"); }
	SClass_ID		SuperClassID() { return BAKE_ELEMENT_CLASS_ID; }
	Class_ID 		ClassID() { return ambientOcclusionBakeElementClassID; }
	const TCHAR* 	Category() { return _T(""); }
...
const int AmbientOcclusionBakeElement::FindParamByName1( const TCHAR*  name ) { 
	for( int i = 0; i < GetNParams(); ++i ){
		if( _tcscmp( name, GetString( paramNames[i] )) == 0 )
			return i+1;
	}
...
FPValue AmbientOcclusionBakeElement::GetParamFPValueMin1( int nParam ) {
	...
	else if (nParam == 4 || nParam == 5 || nParam == 6)
		return FPValue(TYPE_FLOAT, 0.0f);
...
FPValue AmbientOcclusionBakeElement::GetParamFPValueMax1( int nParam ) {
	if( nParam==1 )
		...
		return FPValue(TYPE_FLOAT, 999999999.0f);
	return BaseBakeElement::GetParamFPValueMin1( nParam );
}
