...
TempMoveRestore::TempMoveRestore (EditPolyObject *em) {
	...
	mapInit = new Tab<UVVert>[em->mm.MNum() + NUM_HIDDENMAPS];
	for (int mapChannel=-NUM_HIDDENMAPS; mapChannel<em->mm.MNum(); mapChannel++) {
...
void TempMoveRestore::Restore (EditPolyObject *em) {
	...
	IMNMeshUtilities8* mesh8 = em->mm.GetTypedInterface<IMNMeshUtilities8>();
	for (int i=0; i<em->mm.numv; i++) {
		if (em->mm.v[i].GetFlag (MN_DEAD)) continue;
...
void TopoPipelineClientRestore::FillStorage(Tab<BaseInterface*>& storage, MNMesh& sourceMesh, BaseInterface** excludedInterfaces, int nInterfacesExcluded)
{
    std::set<BaseInterface*> interfacesExcluded;

    // Reset storage
    ClearStorage(storage);

    // Build a set from our array of excluded interfaces for efficient lookup
    DbgAssert((excludedInterfaces != nullptr) || (nInterfacesExcluded == 0));
    for (int i = 0; i != nInterfacesExcluded; ++i)
    {
        interfacesExcluded.insert(excludedInterfaces[i]);
    }

    // Make copies of the topo channels of each non-excluded, IPipelineClient-derived interface for use during undo/redo
    int nInterfaces = sourceMesh.NumInterfaces();
    for (int i = 0; i != nInterfaces; ++i)
    {
        BaseInterface* sourceInterface = sourceMesh.GetInterfaceAt(i);
        DbgAssert(sourceInterface != nullptr);
        IPipelineClient* pipelineClient = static_cast<IPipelineClient*>(sourceInterface->GetInterface(PIPELINECLIENT_INTERFACE));
        if (pipelineClient != nullptr)
        {
            size_t nEntries = interfacesExcluded.count(sourceInterface);
            if (nEntries == 0)
            {
                BaseInterface* cloneInterface = sourceInterface->CloneInterface();
                storage.Append(1, &cloneInterface);
            }
        }
    }
}
...
void TopoPipelineClientRestore::Before(MNMesh& beforeMesh, const Tab<BaseInterface*>& excludedInterfaces)
{

    // Make copies of the "before" topo channels, for each interface that has not been specifically excluded
    BaseInterface** interfacesExcluded = nullptr;
    int nInterfacesExcluded = excludedInterfaces.Count();
    if (nInterfacesExcluded != 0)
    {
        interfacesExcluded = &(excludedInterfaces[0]);
    }

    FillStorage(mTopoChannelsBefore, beforeMesh, interfacesExcluded, nInterfacesExcluded);
}
...
void TopoPipelineClientRestore::After(MNMesh& afterMesh, const Tab<BaseInterface*>& excludedInterfaces)
{

    // Make copies of the "after" topo channels, for each interface that has not been specifically excluded
    BaseInterface** interfacesExcluded = nullptr;
    int nInterfacesExcluded = excludedInterfaces.Count();
    if (nInterfacesExcluded != 0)
    {
    ...
    FillStorage(mTopoChannelsAfter, afterMesh, interfacesExcluded, nInterfacesExcluded);
}
...
void TopoChangeRestore::After () {
	...
	int mapChannel;
	int mapCount = NUM_HIDDENMAPS + mpEditPoly->mm.MNum();	// Note: assuming that start has <= MNum().
	...
	for (mapChannel=-NUM_HIDDENMAPS; mapChannel<mpEditPoly->mm.MNum(); mapChannel++) {
		int offsetMapChannel = NUM_HIDDENMAPS + mapChannel;
	...
	vertexDataCount = start.VDNum();
	if (mpEditPoly->mm.VDNum() > vertexDataCount) vertexDataCount = mpEditPoly->mm.VDNum();
	if (vertexDataCount) {
	...
	edgeDataCount = start.EDNum();
	if (mpEditPoly->mm.EDNum() > edgeDataCount) edgeDataCount = mpEditPoly->mm.EDNum();
	if (edgeDataCount) {
...
void TopoChangeRestore::Restore (int isUndo) {
	...
	// Undo map changes:
	for (int mapChannel=-NUM_HIDDENMAPS; mapChannel<mpEditPoly->mm.MNum(); mapChannel++) {
		if (mpEditPoly->mm.M(mapChannel)->GetFlag (MN_DEAD)) continue;
...
MapChangeRestore::MapChangeRestore (EditPolyObject *e, int mapChannel) : mpEditPoly(e), mMapChannel(mapChannel), mAfterCalled(false) {
	if ((mpEditPoly->mm.MNum() <= mMapChannel) || (mpEditPoly->mm.M(mMapChannel)->GetFlag (MN_DEAD))) {
		preVerts.SetCount (0);
		preFaces.SetCount (0);
...
CreateOnlyRestore::CreateOnlyRestore (EditPolyObject *e) {
	...
	omvnum.SetCount (mpEditPoly->mm.MNum() + NUM_HIDDENMAPS);
	omfnum.SetCount (mpEditPoly->mm.MNum() + NUM_HIDDENMAPS);
	omapsUsed.SetSize (mpEditPoly->mm.MNum() + NUM_HIDDENMAPS);
	for (int mapChannel=-NUM_HIDDENMAPS; mapChannel<mpEditPoly->mm.MNum(); mapChannel++) {
		int offsetMapChannel = mapChannel + NUM_HIDDENMAPS;
...
void CreateOnlyRestore::After () {
	...
	int mapChannel;
	if (mpEditPoly->mm.MNum() + NUM_HIDDENMAPS > omvnum.Count()) {
		int oldnumm = omvnum.Count();
		omvnum.SetCount (mpEditPoly->mm.MNum() + NUM_HIDDENMAPS);
		omfnum.SetCount (mpEditPoly->mm.MNum() + NUM_HIDDENMAPS);
		omapsUsed.SetSize (mpEditPoly->mm.MNum() + NUM_HIDDENMAPS, true);
		for (int offsetMapChannel=oldnumm; offsetMapChannel<mpEditPoly->mm.MNum() + NUM_HIDDENMAPS; offsetMapChannel++) {
			omvnum[offsetMapChannel] = 0;
			omfnum[offsetMapChannel] = 0;
			omapsUsed.Clear (offsetMapChannel);
		}
	}
	nmapsUsed.SetSize (mpEditPoly->mm.MNum() + NUM_HIDDENMAPS);
	if (!nmverts) nmverts = new Tab<UVVert>[mpEditPoly->mm.MNum() + NUM_HIDDENMAPS];
	if (!nmfaces) nmfaces = new Tab<MNMapFace>[mpEditPoly->mm.MNum() + NUM_HIDDENMAPS];
	for (mapChannel=-NUM_HIDDENMAPS; mapChannel<mpEditPoly->mm.MNum(); mapChannel++) {
		int offsetMapChannel = NUM_HIDDENMAPS + mapChannel;
		MNMap *map = mpEditPoly->mm.M(mapChannel);
...
MeshVertRestore::MeshVertRestore(EditPolyObject* pEditPoly, int nVerticesProcessed, bool allowVariation):
    mpEditPoly(pEditPoly)
{
    DbgAssert(mpEditPoly != nullptr);
    int nVerticesInit = (pEditPoly->mm).numv;
    m_nVerticesProcessed = nVerticesProcessed;
    if ((m_nVerticesProcessed < 0) || (m_nVerticesProcessed > nVerticesInit))
    {
        m_nVerticesProcessed = nVerticesInit;
    }

    m_nVerticesMin = nVerticesInit;
    m_nVerticesMax = nVerticesInit;
    if (allowVariation)
    {
        m_nVerticesMin = m_nVerticesProcessed;
        m_nVerticesMax = INT_MAX;
    }

    undo.SetCount(m_nVerticesProcessed);
    for (int v = 0; v != m_nVerticesProcessed; ++v)
    {
        undo[v] = ((mpEditPoly->mm).v[v]).p;
    }
}

void MeshVertRestore::Restore(int isUndo)
{
    if (((mpEditPoly->mm).numv >= m_nVerticesMin) && ((mpEditPoly->mm).numv <= m_nVerticesMax))
    {
        if (isUndo)
        {
            redo.SetCount(m_nVerticesProcessed);
            for (int v = 0; v != m_nVerticesProcessed; ++v)
            {
                redo[v] = ((mpEditPoly->mm).v[v]).p;
            }
        }

        DbgAssert((undo.Count()) == m_nVerticesProcessed);
        for (int v = 0; v != m_nVerticesProcessed; ++v) 
        {
            ((mpEditPoly->mm).v[v]).p = undo[v];
        }
...
{
    int nVerticesRedo = redo.Count();
    if (((mpEditPoly->mm).numv >= m_nVerticesMin) && ((mpEditPoly->mm).numv <= m_nVerticesMax) && (nVerticesRedo == m_nVerticesProcessed))
    {
        for (int v = 0; v != m_nVerticesProcessed; ++v)
        {
            ((mpEditPoly->mm).v[v]).p = redo[v];
        }
...
CollapseDeadFacesRestore::CollapseDeadFacesRestore (EditPolyObject *e) {
    Tab<BaseInterface*> excludedInterfaces;
	...
	if (!ct) return;
	undoMap.SetCount (mpEditPoly->mm.MNum()+NUM_HIDDENMAPS);
	for (int mapChannel=-NUM_HIDDENMAPS; mapChannel<mpEditPoly->mm.MNum(); mapChannel++) {
		int offsetMapChannel = mapChannel + NUM_HIDDENMAPS;
	...
	if (pNormFace)
	{
        ...
        // We properly treat the undo/redo of changes in the specified normals container in this class, and
        // TopoPipelineClientRestore can muck things up if the normal indices changed on a call to CheckNormals, so exclude
        // pNorm from treatment in mTopoPipelineRestore
        BaseInterface* baseNormals = pNorm;
        excludedInterfaces.Append(1, &baseNormals);
	}

	// ktong|17/2/2006|740288|Update TOPO_CHANNELS pipeline clients for undo/redo.
	// Copy the before data channels for use during undo.
	mTopoPipelineRestore.Before(mpEditPoly->mm, excludedInterfaces);
}
...
void CollapseDeadFacesRestore::Restore (int isUndo)
{
    ...
    // We properly treat the undo/redo of changes in the specified normals container in this class, and
    // TopoPipelineClientRestore can muck things up if the normal indices changed on a call to CheckNormals, so exclude pNorm
    // from treatment in mTopoPipelineRestore
    MNNormalSpec* pNorm = mpEditPoly->mm.GetSpecifiedNormals();
    if (pNorm != nullptr)
    {
        BaseInterface* baseNormals = pNorm;
	...
	mTopoPipelineRestore.After(mpEditPoly->mm, excludedInterfaces);
