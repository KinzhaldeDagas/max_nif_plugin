...
static int editablePolyMeshSelLevel[] = { 
			MNM_SL_OBJECT, 
			MNM_SL_VERTEX, 
			MNM_SL_EDGE,
...
#define EPOLY_POINT_CTRL_CONTAINER_REF 1
...
class EditPolyObject : public PolyObject, public EventUser, public ISubMtlAPI,
			public EPoly20, public AttachMatDlgUser, public IMeshSelect,
...
private:
	...
	// Returns the triangulation type corresponding to the present setting of the ep_enable_enhanced_retriangulation
	// parameter block entry.
	MNMeshTriangulationType GetTriangulationType();

	// Retriangulates the specified faces, employing the algorithm specified via the ep_enable_enhanced_retriangulation
	// parameter block entry.
	//
	// faceList:   entry faceList[f] is the index, in the mm.f array, of a face to be retriangulated; input
	// nFacesList: number of faces in list faceList; input
	void TriangulateFaces(const int* faceList, int nFacesList);
	inline void TriangulateFaces(const IntTab& faceList)
	{
		int nFacesList = faceList.Count();
		if (nFacesList != 0)
		{
			TriangulateFaces(&(faceList[0]), nFacesList);
		}
	}
	...
	PointControllerContainer			*pointCtrlContainer;		// Point Controller Container track
    ...
    // Map from EPoly13 grip enum, exposed in the SDK, to the actual enum from EPolyManipulatorGrip, which may include further
    // options
    static const EPolyManipulatorGrip::ManipulateGrips m_gripTypeMap[EPoly13::EPolyManipulatorGripCount];

    // If a shift-drag extrude operation with posprocessing is presently underway, points to the undo object for that
    // operation, otherwise, null pointer
    TopoChangeRestore* m_extrudeRestore;

    // When m_extrudeRestore is non-null, stores the initial number of vertices in the mesh after the call to BeginFaceExtrude,
    // but before any postprocessing is conducted
    int m_nVerticesExtrudeInit;
...
public:
	...
	MNTempData	*TempData ();
	void		InvalidateTempData (ChannelMask channels=TOPO_CHANNEL|GEOM_CHANNEL);
	...
	void		GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_EDITABLE_POLY) : _T("Editable Poly");}
	void		BeginEditParams(IObjParam  *ip, ULONG flags,Animatable *prev);
	void		EndEditParams(IObjParam *ip, ULONG flags,Animatable *next);
	int			NumSubs() { return 2; }		// only the pblock and the Point Controller Container
...
public:
	...
	// Object methods
	const TCHAR		*GetObjectName(bool localized) const override { return localized ? GetString(IDS_EDITABLE_POLY) : _T("Editable Poly");}
	...
	void 	EpfnDragChamfer (float amount, float tensionOrDepth, TimeValue t);    // Note that value tensionOrDepth is interpreted as tension for an edge chamfer, depth for a vertex chamfer
	...
	void 	BeginVertexColorModify (int mapChannel=0, DWORD flag = MN_SEL);
	...
	int EpFnGetEdgeChamferType() override;
	void EpFnSetEdgeChamferType(int chamferType) override;
	bool EpFnGetEdgeChamferQuadIntersections() override;
	void EpFnSetEdgeChamferQuadIntersections(bool quadIntersections) override;
	...
	BitArray 	GetVertSel() override { mm.getVertexSel(vsel); return vsel; }
	BitArray 	GetFaceSel() override { mm.getFaceSel(fsel); return fsel; }
	BitArray 	GetEdgeSel() override { if (mm.GetFlag (MN_MESH_FILLED_IN)) mm.getEdgeSel(esel); return esel; }
	bool		EpGetVerticesByFlag (BitArray & vset, DWORD flags, DWORD fmask=0x0) override { return mm.getVerticesByFlag (vset, flags, fmask); }
	bool 		EpGetEdgesByFlag (BitArray & eset, DWORD flags, DWORD fmask=0x0) override { return mm.getEdgesByFlag (eset, flags, fmask); }
	bool 		EpGetFacesByFlag (BitArray & fset, DWORD flags, DWORD fmask=0x0) override { return mm.getFacesByFlag (fset, flags, fmask); }
	...
	void 					EpSetVertexFlags (const BitArray &vset, DWORD flags, DWORD fmask=0x0, bool undoable=true) override;
	void 					EpSetEdgeFlags (const BitArray &eset, DWORD flags, DWORD fmask = 0x0, bool undoable=true) override;
	void 					EpSetFaceFlags (const BitArray &fset, DWORD flags, DWORD fmask = 0x0, bool undoable=true) override;
	void 					SetSel (int nsl, const BitArray & set, IMeshSelect *imod, TimeValue t);
	...
	int				EpfnGetNumMapChannels () { return mm.MNum(); }
	...
	bool			EpfnMakeHardEdges (DWORD flag=MN_SEL);		// Make edges indicated by 'flag' unsmoothed.  Returns true if altered.
	bool			EpfnMakeSmoothEdges (DWORD flag=MN_SEL);	// Make edges indicated by 'flag' smoothed.  Returns true if altered.

	void			EpfnSelectHardEdges ();		// Select all hard (unsmoothed) edges
	void			EpfnSelectSmoothEdges ();	// Select all smoothed edges

	// EPoly methods
	void 			LocalDataChanged (ChannelMask channels);
	void			UpdateEdgeColorDisplay();
	void 			RefreshScreen ();
	...
	ChannelMask		PartsChangedByOp (int operation);
	...
	int 			GetMNSelLevel() { return editablePolyMeshSelLevel[selLevel]; }
    ...
    // Overrides the variant defined in PolyObject to ensure that our selLevel member is updated consistently with the
    // underlying MNMesh selection level when this function is called. Calls the PolyObject variant after setting selLevel.
    // Note that as with the PolyObject variant, s is presumed to correspond to a member of the PMeshSelLevel enumeration
    // defined in mnmesh.h.
    virtual void SetSubSelState(DWORD s) override;
    ...
    int         EpfnGetLastOperation() { return mLastOperation; }
    ...
    // Get and set functions for grip state, based upon EPoly13 grip enumeration exposed in the SDK
...
private:
	...
	// File version for backward compatibility
	static const int VERSION_INITIAL = 0;
	static const int VERSION_CORRECTED_DPOINT3_ADD = 1;
	static const int VERSION_CURRENT = VERSION_CORRECTED_DPOINT3_ADD;
	int mVersion;
};
...
protected:
	...
	void DisplayDataChanged() {m_EPoly->LocalDataChanged(DISP_ATTRIB_CHANNEL);}
...
public:
	AttachPickMode() : mpEditPoly(NULL), ip(NULL), mSuspendUI(false) { }
...
private:
	bool mSuspendUI; //just a variable to suspend drawing the button since the material condense dialog hoses up pick modes
};
...
public:
	ChamferCMode (EditPolyObject* e, IObjParam *i) :
	  fgProc(e), mouseProc(&eproc,e,i), eproc(e,i), mpEditPoly(e) { ip = i; }
	int Class() { return MODIFY_COMMAND; }
...
public:
	...
	int Class() { return MODIFY_COMMAND; }
	int ID() { return CID_POLY_LIFT_FROM_EDGE; }
	MouseCallBack *MouseProc(int *numPoints) {*numPoints=2; return &proc;}
	ChangeForegroundCallback *ChangeFGProc() {return &fgProc;}
	BOOL ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void EnterMode();
...
public:
	...
	int Class() { return MODIFY_COMMAND; }
	int ID() { return CID_POLY_PICK_LIFT_EDGE; }
	MouseCallBack *MouseProc(int *numPoints) {*numPoints=1; return &proc;}
	ChangeForegroundCallback *ChangeFGProc() {return &fgProc;}
	BOOL ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void EnterMode();
...
class TopoPipelineClientRestore
{
    ...
    void FillStorage(Tab<BaseInterface*>& storage, MNMesh& sourceMesh, BaseInterface** excludedInterfaces = nullptr, int nInterfacesExcluded = 0);
    void UseStorage(MNMesh& targetMesh, Tab<BaseInterface*>& storage);
public:
    ~TopoPipelineClientRestore();

    // If the variant of Before or After specifying excludedInterfaces is called, then interface mesh.interfaces[i], where mesh
    // is beforeMesh or afterMesh as appropriate, will be added to the list of interfaces to be treated by this class only if
    // it does not appear in the excludedInterfaces list.
    void Before(MNMesh& beforeMesh);
    void Before(MNMesh& beforeMesh, const Tab<BaseInterface*>& excludedInterfaces);
    void After(MNMesh& afterMesh);
    void After(MNMesh& afterMesh, const Tab<BaseInterface*>& excludedInterfaces);
...
class TopoChangeRestore : public RestoreObj {
	...
	MNNormalSpec *mNormBefore;
	MNNormalSpec *mNormAfter;
...
public:
	...
	{	
		start = mpEditPoly->mm; 
		if (start.GetSpecifiedNormals())
		{
			if (mNormBefore) delete mNormBefore;
			mNormBefore = new MNNormalSpec();
			*mNormBefore = *start.GetSpecifiedNormals();
		}
	}
	void After ();
	void Restore (int isUndo);
...
class MeshVertRestore : public RestoreObj {
	...
	EditPolyObject *mpEditPoly;
    int m_nVerticesProcessed;
    int m_nVerticesMin;
    int m_nVerticesMax;

public:

    // When a valid, value of nVerticesProcessed is specified, which is to say, a value which is non-negative and not greater
    // than the number of vertices in mesh pEditPoly->mm on construction, Restore and Redo operations are applied only to the
    // first nVerticesProcessed mesh vertices, otherwise, we apply the operation to all mesh vertices which exist on
    // construction. If allowVariation is true, we process operations so long as the number of vertices remains greater than or
    // equal to the number to be processed, otherwise, we demand that the number of vertices remains constant.
	MeshVertRestore(EditPolyObject* pEditPoly, int nVerticesProcessed = -1, bool allowVariation = false);
	void Restore(int isUndo);
	void Redo();
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Editable Poly"); }
...
public:
	HWND hwnd;
	EditPolyObject *ep;
	SingleRefMakerPolyMtl() {hwnd = NULL; ep = NULL;}
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("SingleRefMakerPolyMtl"); } // from Animatable
};
...
public:
	HWND hwnd;
	EditPolyObject *ep;
	SingleRefMakerPolyNode() {hwnd = NULL; ep = NULL;}
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("SingleRefMakerPolyNode"); } // from Animatable
};
...
private:
	...
	int HitTest(IPoint2 &m, ViewExp& vpt,int& subIndex);

public:
	SubobjectPickProc (EPoly* e, IObjParam *i) : mpEPoly(e), ip(i), hitSubLevel(-1), hitSubIndex(-1) 
	{ 
		mpEditPoly = (EditPolyObject *)mpEPoly; 
	}
	int proc(HWND hwnd, int msg, int point, int flags, IPoint2 m );
	void SetSubobjectLevel();
	int hitSubLevel;
	int hitSubIndex;
...
public:
	SubobjectPickMode(EditPolyObject* e, IObjParam *i) : fgProc(e), proc(e,i), mpEditPoly(e), inMode(false), mInOverride(false)  
	{ 
		TSTR name,category;

		name.printf(_T("%s"),GetString(IDS_SUBOBJECT_PICK_COLOR));
		category.printf(_T("%s"),GetString(IDS_SUBOBJECT_PICK_CAT));
		bool iret;
		iret = ColorMan()->RegisterColor(SUBOBJECT_PICK_COLORID,				name,		category, RGB(255,199,27));
	}
	int Class() { return MODIFY_COMMAND; }
...
private:
	...
	//return 0 none, 1 vertex, 2 edge, 3 face/poly
	int HitTest(IPoint2 &m, ViewExp& vpt);	


public:
	PointToPointProc (EPoly* e, IObjParam *i) : mpEPoly(e), ip(i), mFirstHit(-1), mSecondHit(-1) , mPointToPointPath(nullptr), mInLoop(false)
	{ 
		mpEditPoly = (EditPolyObject *)mpEPoly; 
		mInLoop = false;
		mFirstHit = -1;
...
public:
	...
	PointToPointMode(EditPolyObject* e, IObjParam *i) : fgProc(e), proc(e,i), mpEditPoly(e), mInMode(false), mShiftLaunched(false)	
	{ 
	}
