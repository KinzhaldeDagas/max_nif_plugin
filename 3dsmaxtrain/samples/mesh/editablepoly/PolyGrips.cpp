...
// ==============================================================================
// class EditablePolyGrip
// ==============================================================================

EditablePolyGrip::EditablePolyGrip()
	: mpEditPoly(NULL)
	, mPolyID(0)
	, mReentryLock(false)
{}
...
void EditablePolyGrip::Init(EditPolyObject* m,int polyID)
{
	if (m != NULL)
	{
		mpEditPoly = m;
...
EditPolyObject* EditablePolyGrip::GetEditPolyObj()
{
	 return mpEditPoly;
}
int  EditablePolyGrip::GetPolyID()
{
	return mPolyID;
}
void EditablePolyGrip::SetPolyID( int iPolyID)
{
	mPolyID = iPolyID;
}
void EditablePolyGrip::SetUpUI()
{
...
bool EditablePolyGrip::SupportsOkayApplyCancel()
{
	return true;
}
void EditablePolyGrip::CommonOK(TimeValue t)
{
	if(!mReentryLock)
	{
		...
		if (mpEditPoly != NULL)
		{
			mpEditPoly->HideGrip(false);
			mpEditPoly->EpPreviewAccept();
			mpEditPoly->ClearOperationSettings();
		}
		// This action is completed. No more need for this pointer.
		mpEditPoly = NULL;
	}
}
void EditablePolyGrip::CommonApply(TimeValue t)
{
	if(!mReentryLock)
	{
		MaxSDK::VariableGuard<bool> guard(mReentryLock, true);
		...
		if (mpEditPoly != NULL)
		{
			...
			mpEditPoly->EpPreviewBegin(GetPolyID());
		}
	}
	// The action is NOT completed yet. It will end either with an OK or a CANCEL.
}
void EditablePolyGrip::CommonCancel()
{
	if(!mReentryLock)
	{
		MaxSDK::VariableGuard<bool> guard(mReentryLock, true);
...
// ==============================================================================
// class EditablePolyManipulatorGrip
// ==============================================================================


void EditablePolyManipulatorGrip::SetUpVisibility(BOOL isVisible, int indexGrip)
{
...
void EditablePolyManipulatorGrip::UpdateGripVisibility()
{
	if(NULL != mpEditPolyObject)
	{
		mpEditPolyObject->SetUpManipGripVisibility();
	}
}

BaseInterface* EditablePolyManipulatorGrip::GetInterface(Interface_ID id)
{
...
{
	return EditablePolyManipulatorGrip::EditablePolyManipulatorGripCount;
}
...
bool EditablePolyManipulatorGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eFalloff:
		GetEditPolyObject()->getParamBlock()->GetValue (ep_ss_falloff, t, value.mFloat);
		break;
	case eBubble:
		GetEditPolyObject()->getParamBlock()->GetValue (ep_ss_bubble, t, value.mFloat);
		break;
	case ePinch:
		GetEditPolyObject()->getParamBlock()->GetValue (ep_ss_pinch, t, value.mFloat);
		break;
	case eSetFlow:
	case eLoopShift:
	case eRingShift:
    ...
    case eEdgeDepth:
        {
            int num = 0;
            bool uniform = true;
            value.mFloat = (GetEditPolyObject())->GetEdgeDataValue(EDATA_DEPTH, &num, &uniform, MN_SEL, t);
        }
        break;
	case eVertexCrease:
		{
			int num = 0;
			bool uniform(true);
			value.mFloat = GetEditPolyObject()->GetVertexDataValue (VDATA_CREASE, &num, &uniform, MN_SEL, t);
		}
		break;
	case eVertexWeight:
		{
...
bool EditablePolyManipulatorGrip::StartSetValue(int which,TimeValue t)
{
	...
	case eEdgeDepth:
		{
			if (((GetEditPolyObject())->GetMeshPtr())->nume != 0)
            {
                GetEditPolyObject()->BeginPerDataModify(MNM_SL_EDGE, EDATA_DEPTH);
            }
		}
		break;
	case eVertexCrease:
		{
			...
			GetEditPolyObject()->BeginPerDataModify (MNM_SL_VERTEX, VDATA_CREASE);
		}
		break;
	case eVertexWeight:
		{
	...
	case eEdgeDepth: 
		{
			if (((GetEditPolyObject())->GetMeshPtr())->nume != 0)
            {
                (GetEditPolyObject())->SetEdgeDataValue(EDATA_DEPTH, value.mFloat, MN_SEL, t);
            }
		}
		break;
	case eVertexCrease:
		{
			if (!GetEditPolyObject()->GetMeshPtr()->numv) break;
			GetEditPolyObject()->SetVertexDataValue (VDATA_CREASE, value.mFloat, MN_SEL, t);
		}
		break;
	case eVertexWeight:
		{
	...
	case eEdgeDepth:
		{
            ...
            {
                theHold.Begin();
                (GetEditPolyObject())->SetEdgeDataValue(EDATA_DEPTH, fResetValue, MN_SEL, t);
                theHold.Accept(GetString(IDS_CHANGE_DEPTH));
            }
		}
		break;
	case eVertexCrease:
		{
			if (!GetEditPolyObject()->GetMeshPtr()->numv) break;
			theHold.Begin ();
			GetEditPolyObject()->SetVertexDataValue (VDATA_CREASE, fResetValue, MN_SEL, t);
			theHold.Accept (GetString(IDS_CHANGE_CREASE_VALS));
		}
		break;	
	case eVertexWeight:
	...
	case eBevelType:
		{
			...
			int type;
			//GetEditPolyObj()->getParamBlock()->GetValue (ep_bevel_type, 0, type);
			GetEditPolyObj()->getParamBlock()->GetValue (ep_bevel_type, 0, type);
			switch(type)
			{
				case 0:
					...
					break;
				case 1:
					string =  GetString(IDS_LOCAL_NORMAL);
					break;
				case 2:
...
bool BevelGrip::GetComboOptions(int which, Tab<IBaseGrip::ComboLabel*> &comboOptions)
{
	if(which== eBevelType)
	{
		...
		label->mIcon = string;
		label->mLabel = GetString(IDS_LOCAL_NORMAL);
		comboOptions.Append(1,&label);
		label = new IBaseGrip::ComboLabel();
...
bool BevelGrip::GetRange(int which,IBaseGrip::GripValue &minRange, IBaseGrip::GripValue &maxRange)
{
	...
	case eBevelBias:
	{
		theDef = GetEditPolyObj()->getParamBlock()->GetParamDef(ep_bevel_bias);
		if (theDef.flags&P_HAS_RANGE)
		{
			minRange.mFloat = theDef.range_low.f;
			maxRange.mFloat = theDef.range_high.f;
...
{
	if (GetEditPolyObj()->getParamBlock()->GetInt(ep_bevel_type,0) == 1)
		GetIGripManager()->HideGripItem(3, false);
	else
		GetIGripManager()->HideGripItem(3, true);

	EditablePolyGrip::SetUpUI();
}
...
bool BevelGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eBevelType:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_bevel_type, t, value.mInt);
		break;
	case eBevelHeight:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_bevel_height, t, value.mFloat);
		break;
	case eBevelOutline:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_bevel_outline, t, value.mFloat);
		break;
	case eBevelBias:
		GetEditPolyObj()->getParamBlock()->GetValue(ep_bevel_bias, t, value.mFloat);
		break;
...
bool BevelGrip::SetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	case eBevelOutline:
		...
		break;
	case eBevelBias:
		GetEditPolyObj()->getParamBlock()->SetValue(ep_bevel_bias, t, value.mFloat);
		GetEditPolyObj()->getParamBlock()->GetValue(ep_bevel_bias, t, value.mFloat);
		break;
...
bool BevelGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eBevelHeight:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_bevel_height,t)==TRUE)? true : false;
	case eBevelOutline:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_bevel_outline,t)==TRUE) ? true : false;
	case eBevelBias:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_bevel_bias, t) == TRUE) ? true : false;
...
bool BevelGrip::ResetValue(TimeValue t, int which)
{
	...
	case eBevelBias:
	{
		...
		GetEditPolyObj()->getParamBlock()->SetValue(ep_bevel_bias, t, resetVal);
	}
	break;
...
bool BevelGrip::GetResetValue(int which,IBaseGrip::GripValue &resetValue)
{
	...
	case eBevelBias:
	{
		const ParamDef& pd = pbd->paramdefs[ep_bevel_bias];
		resetValue.mFloat = pd.def.f;
	}
	break;
	default:
	...
	case eExtrudeFaceType:
		{
			//Group Normals, 1 for Local Normals, 2 for By Polygon 
			int type;
			GetEditPolyObj()->getParamBlock()->GetValue (ep_extrusion_type, 0, type);
			switch(type)
			{
			case 0:
				string = GetString(IDS_GROUP);
...
bool ExtrudeFaceGrip::GetRange(int which,IBaseGrip::GripValue &minRange, IBaseGrip::GripValue &maxRange)
{
	...
	case eExtrudeFaceHeight:
		...
		break;
	case eExtrudeBias:
		theDef = GetEditPolyObj()->getParamBlock()->GetParamDef(ep_extrude_bias);
		if (theDef.flags&P_HAS_RANGE)
		{
			minRange.mFloat = theDef.range_low.f;
			maxRange.mFloat = theDef.range_high.f;
...
{
	if (GetEditPolyObj()->getParamBlock()->GetInt(ep_extrusion_type, 0) == 1)
...
bool ExtrudeFaceGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eExtrudeFaceType:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_extrusion_type, t, value.mInt);
		break;
	case eExtrudeFaceHeight:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_face_extrude_height, t, value.mFloat);
		break;
	case eExtrudeBias:
		GetEditPolyObj()->getParamBlock()->GetValue(ep_extrude_bias, t, value.mFloat);
		break;
	default:
		return false;
	}
...
bool ExtrudeFaceGrip::SetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	case eExtrudeFaceHeight:
		...
		break;
	case eExtrudeBias:
		GetEditPolyObj()->getParamBlock()->SetValue(ep_extrude_bias, t, value.mFloat);
		GetEditPolyObj()->getParamBlock()->GetValue(ep_extrude_bias, t, value.mFloat);
		break;
...
bool ExtrudeFaceGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eExtrudeFaceHeight:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_face_extrude_height,t)==TRUE)? true : false;
	case eExtrudeBias:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_extrude_bias, t) == TRUE) ? true : false;
	default:
		return false;
	}
...
bool ExtrudeFaceGrip::ResetValue(TimeValue t, int which)
{
	...
	case eExtrudeBias:
	{
		const float fResetVal = 1.0f;
		theHold.Begin();
		GetEditPolyObj()->getParamBlock()->SetValue(ep_extrude_bias, t, fResetVal);
		theHold.Accept(GetString(IDS_EXTRUDE));
	}
	break;
...
bool ExtrudeFaceGrip::GetResetValue(int which,IBaseGrip::GripValue &resetValue)
{
	...
	case eExtrudeBias:
		{
			const ParamDef& pd = pbd->paramdefs[ep_extrude_bias];
			resetValue.mFloat = pd.def.f;
		}
		break;
...
bool ExtrudeEdgeGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eExtrudeEdgeHeight:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_edge_extrude_height, t, value.mFloat);
		break;
	case eExtrudeEdgeWidth:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_edge_extrude_width, t, value.mFloat);
		break;
	default:
		return false;
	}
...
bool ExtrudeEdgeGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eExtrudeEdgeHeight:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_edge_extrude_height,t)==TRUE)? true : false;
	case eExtrudeEdgeWidth:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_edge_extrude_width,t)==TRUE) ? true : false;
	default:
		return false;
	}
...
bool ExtrudeVertexGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eExtrudeVertexHeight:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_vertex_extrude_height, t, value.mFloat);
		break;
	case eExtrudeVertexWidth:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_vertex_extrude_width, t, value.mFloat);
		break;
	default:
		return false;
	}
...
bool ExtrudeVertexGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eExtrudeVertexHeight:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_vertex_extrude_height,t)==TRUE)? true : false;
	case eExtrudeVertexWidth:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_vertex_extrude_width,t)==TRUE) ? true : false;
	default:
		return false;
	}
...
bool OutlineGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eOutlineAmount:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_outline, t, value.mFloat);
		break;
	default:
		return false;
	}
...
bool OutlineGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eOutlineAmount:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_outline,t)==TRUE) ? true : false;
	default:
		return false;
	}
	...
	case eInsetType:
		{
			...
			int type;
			//GetEditPolyObj()->getParamBlock()->GetValue (ep_Inset_type, 0, type);
			GetEditPolyObj()->getParamBlock()->GetValue (ep_inset_type, 0, type);
			switch(type)
			{
			case 0:
...
bool InsetGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eInsetType:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_inset_type, t, value.mInt);
		break;
	case eInsetAmount:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_inset, t, value.mFloat);
		break;
	default:
		return false;
	}
...
bool InsetGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eInsetAmount:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_inset,t)==TRUE)? true : false;
	default:
		return false;
	}
...
bool ConnectEdgeGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eConnectEdgeSegments:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_connect_edge_segments, t, value.mInt);
		break;
	case eConnectEdgePinch:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_connect_edge_pinch, t, value.mInt);
		break;
	case eConnectEdgeSlide:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_connect_edge_slide, t, value.mInt);
		break;
	default:
		return false;
	}
...
bool ConnectEdgeGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eConnectEdgeSegments:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_connect_edge_segments,t)==TRUE)? true : false;
	case eConnectEdgePinch:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_connect_edge_pinch,t)==TRUE) ? true : false;
	case eConnectEdgeSlide:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_connect_edge_slide,t)==TRUE) ? true : false;
	default:
		return false;
	}
...
bool ChamferVerticesGrip::GetRange(int which, IBaseGrip::GripValue &minRange, IBaseGrip::GripValue &maxRange)
{
    ParamDef theDef;
    bool isFound = true;
    IParamBlock2* params = (GetEditPolyObj())->getParamBlock();
    switch (which)
    {
    case eChamferVerticesAmount:
        theDef = params->GetParamDef(ep_vertex_chamfer);
        break;
    case eChamferVerticesSegments:
        theDef = params->GetParamDef(ep_vertex_chamfer_num_segments);
        break;
    case eChamferVerticesDepth:
        theDef = params->GetParamDef(ep_vertex_chamfer_depth);
        break;
    case eChamferVerticesSmoothThreshold:
        theDef = params->GetParamDef(ep_vertex_chamfer_smooth_threshold);
        break;
    default:
        isFound = false;
        break;
...
bool ChamferVerticesGrip::GetValue(int which, TimeValue t, IBaseGrip::GripValue& value)
{
    ...
    {
    case eChamferVerticesAmount:
        params->GetValue(ep_vertex_chamfer, t, value.mFloat, interval);
        break;
    case eChamferVerticesSegments:
        params->GetValue(ep_vertex_chamfer_num_segments, t, value.mInt, interval);
        break;
    case eChamferVerticesDepth:
        params->GetValue(ep_vertex_chamfer_depth, t, value.mFloat, interval);
        break;
    case eChamferVerticesOpen:
        {
            value.mbool = false;
            BOOL isOpen = FALSE;
            params->GetValue(ep_vertex_chamfer_open, t, isOpen, interval);
            if (isOpen)
            {
                value.mbool = true;
            }
    ...
    case eChamferVerticesSmooth:
        ...
        break;
    case eChamferVerticesSmoothType:
        params->GetValue(ep_vertex_chamfer_smooth_type, t, value.mInt, interval);
        break;
    case eChamferVerticesSmoothThreshold:
        params->GetValue(ep_vertex_chamfer_smooth_threshold, t, value.mFloat, interval);
        break;
    default:
        isFound = false;
        break;
...
bool ChamferVerticesGrip::SetValue(int which, TimeValue t, IBaseGrip::GripValue& value)
{
    ...
    case eChamferVerticesAmount:
        ...
        params->SetValue(ep_vertex_chamfer, t, value.mFloat);
        params->GetValue(ep_vertex_chamfer, t, value.mFloat, interval);
        break;
    case eChamferVerticesSegments:
        if (value.mInt < 0)
        ...
        params->SetValue(ep_vertex_chamfer_num_segments, t, value.mInt);
        params->GetValue(ep_vertex_chamfer_num_segments, t, value.mInt, interval);
        break;
    case eChamferVerticesDepth:
        ...
        params->SetValue(ep_vertex_chamfer_depth, t, value.mFloat);
        params->GetValue(ep_vertex_chamfer_depth, t, value.mFloat, interval);
        break;
    case eChamferVerticesOpen:
        {
            ...
            params->SetValue(ep_vertex_chamfer_open, t, isOpen);
            params->GetValue(ep_vertex_chamfer_open, t, isOpen, interval);
        }
        break;
    case eChamferVerticesSmooth:
        {
            ...
            params->SetValue(ep_vertex_chamfer_smooth, t, doSmooth);
            params->GetValue(ep_vertex_chamfer_smooth, t, doSmooth, interval);
            MaybeEnableControls();
        }
        break;
    case eChamferVerticesSmoothType:
        if (value.mInt < EPOLY_EDGE_CHAMFER_SMOOTH_ALL)
        {
            value.mInt = EPOLY_EDGE_CHAMFER_SMOOTH_ALL;
        }
        else if (value.mInt > EPOLY_EDGE_CHAMFER_SMOOTH_CHAMFER)
        {
            value.mInt = EPOLY_EDGE_CHAMFER_SMOOTH_CHAMFER;
        }

        params->SetValue(ep_vertex_chamfer_smooth_type, t, value.mInt);
        params->GetValue(ep_vertex_chamfer_smooth_type, t, value.mInt, interval);
        break;
    case eChamferVerticesSmoothThreshold:
        if (value.mFloat < 0.0f)
        ...
        params->SetValue(ep_vertex_chamfer_smooth_threshold, t, value.mFloat);
        params->GetValue(ep_vertex_chamfer_smooth_threshold, t, value.mFloat, interval);
        break;
    default:
        isFound = false;
...
bool ChamferVerticesGrip::ShowKeyBrackets(int which,TimeValue t)
{
    ...
    {
    case eChamferVerticesAmount:
        hasKey = params->KeyFrameAtTimeByID(ep_vertex_chamfer, t);
        break;
    case eChamferVerticesSegments:
        hasKey = params->KeyFrameAtTimeByID(ep_vertex_chamfer_num_segments, t);
        break;
    case eChamferVerticesDepth:
        hasKey = params->KeyFrameAtTimeByID(ep_vertex_chamfer_depth, t);
        break;
    case eChamferVerticesSmoothThreshold:
        hasKey = params->KeyFrameAtTimeByID(ep_vertex_chamfer_smooth_threshold, t);
    default:
        break;
    }
...
bool ChamferVerticesGrip::ResetValue(TimeValue t, int which)
{
    ...
    {
    case eChamferVerticesAmount:
        paramID = ep_vertex_chamfer;
        break;
    case eChamferVerticesSegments:
        paramID = ep_vertex_chamfer_num_segments;
        break;
    case eChamferVerticesDepth:
        paramID = ep_vertex_chamfer_depth;
        break;
    case eChamferVerticesOpen:
        paramID = ep_vertex_chamfer_open;
        break;
    case eChamferVerticesSmooth:
        paramID = ep_vertex_chamfer_smooth;
        break;
    case eChamferVerticesSmoothType:
        paramID = ep_vertex_chamfer_smooth_type;
        break;
    case eChamferVerticesSmoothThreshold:
        paramID = ep_vertex_chamfer_smooth_threshold;
        break;
    default:
        isFound = false;
        break;
...
void ChamferVerticesGrip::MaybeEnableControls()
{
    ...
    params->GetValue(ep_vertex_chamfer_smooth, 0, doSmooth, interval);
    if (doSmooth)
    {
...
bool ChamferEdgeGrip::GetText(int which, TSTR& string)
{
	...
	case eChamferEdgeAmount:
	{
		string = GetString(IDS_EDGE_CHAMFER_AMOUNT); // + TSTR(": ") + TSTR(str);
	}
	break;
	case eChamferEdgeSegments:
	{
		string = GetString(IDS_CONNECT_EDGE_SEGMENTS); //+ TSTR(": ") + TSTR(str);
	}
	break;
	case eChamferEdgeTension:
	{
	...
	{
		int miteringType;
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_mitering_type, 0, miteringType);
		switch (miteringType)
		{
...
bool ChamferEdgeGrip::GetRange(int which, IBaseGrip::GripValue& minRange, IBaseGrip::GripValue& maxRange)
{
	...
	case eChamferEdgeSegments:
		...
		break;
	case eChamferEdgeTension:
		theDef = GetEditPolyObj()->getParamBlock()->GetParamDef(ep_edge_chamfer_tension);
		if (theDef.flags & P_HAS_RANGE)
		{
			minRange.mFloat = 0.0f;
			maxRange.mFloat = 1.0f;
		...
		break;
	case eChamferEdgeSmoothThreshold:
		theDef = GetEditPolyObj()->getParamBlock()->GetParamDef(ep_edge_chamfer_smooth_threshold);
		if (theDef.flags & P_HAS_RANGE)
		{
			minRange.mFloat = 0.0f;
		...
		break;
	case eChamferEdgeEndBias:
		theDef = GetEditPolyObj()->getParamBlock()->GetParamDef(ep_edge_chamfer_end_bias);
		if (theDef.flags & P_HAS_RANGE)
		{
			minRange.mFloat = 0.0f;
			maxRange.mFloat = 1.0f;
		...
		break;
	case eChamferEdgeRadialBias:
		theDef = GetEditPolyObj()->getParamBlock()->GetParamDef(ep_edge_chamfer_radial_bias);
		if (theDef.flags & P_HAS_RANGE)
		{
			minRange.mFloat = 0.0f;
			maxRange.mFloat = 1.0f;
		...
		break;
	case eChamferEdgeDepth:
		theDef = GetEditPolyObj()->getParamBlock()->GetParamDef(ep_edge_chamfer_depth);
		if (theDef.flags & P_HAS_RANGE)
		{
			minRange.mFloat = -0.5f;
			maxRange.mFloat = 1.0f;
...
bool ChamferEdgeGrip::GetValue(int which, TimeValue t, IBaseGrip::GripValue& value)
{
	...
	{
	case eChamferEdgeAmount:
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer, t, value.mFloat);
		break;
	case eChamferEdgeSegments:
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_segments, t, value.mInt);
		break;
	case eChamferEdgeTension:
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_tension, t, value.mFloat);
		break;
	case eChamferEdgeOpen:
	{
		// need to go BOOL(int) to bool
		int val = 0;
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_open, t, val);
	...
	{
		int val = 0;
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_invert, t, val);
		value.mbool = (val == 0 ? false : true);
	}
	break;
	case eChamferEdgeSmooth:
	{
		// need to go BOOL(int) to bool
		int val = 0;
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_smooth, t, val);
		value.mbool = (val == 0 ? false : true);
	}
	break;
	case eChamferEdgeSmoothType:
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_smooth_type, t, value.mInt);
		break;
	case eChamferEdgeSmoothThreshold:
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_smooth_threshold, t, value.mFloat);
		break;
	case eChamferEdgeMiteringType:
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_mitering_type, t, value.mInt);
		break;
	case eChamferEdgeEndBias:
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_end_bias, t, value.mFloat);
		break;
	case eChamferEdgeRadialBias:
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_radial_bias, t, value.mFloat);
		break;
	case eChamferEdgeDepth:
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_depth, t, value.mFloat);
		break;
	default:
		return false;
	}
...
void ChamferEdgeGrip::MaybeEnableControls()
{
	...
	switch (miteringType)
	{
	case EPOLY_EDGE_CHAMFER_MITER_UNIFORM: // Uniform
	case EPOLY_EDGE_CHAMFER_MITER_RADIAL: // Radial
	case EPOLY_EDGE_CHAMFER_MITER_PATCH: // Patch
		...
		break;
	case EPOLY_EDGE_CHAMFER_MITER_QUAD: // Quad
		...
		break;
	case EPOLY_EDGE_CHAMFER_MITER_TRI: // Tri
	...
	int smoothingOn;
	GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_smooth, 0, smoothingOn);
...
bool ChamferEdgeGrip::SetValue(int which, TimeValue t, IBaseGrip::GripValue& value)
{
	...
	case eChamferEdgeSegments:
		...
		break;
	case eChamferEdgeTension:
		value.mFloat = std::clamp(value.mFloat, 0.0f, 1.0f);
		GetEditPolyObj()->getParamBlock()->SetValue(ep_edge_chamfer_tension, t, value.mFloat);
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_tension, t, value.mFloat);
		break;
	case eChamferEdgeOpen:
	{
		...
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_open, t, val);
		GetEditPolyObj()->getParamBlock()->EnableNotifications(false);
		GetEditPolyObj()->getParamBlock()->SetValue(ep_edge_chamfer_invert, t, false);
		IGripManager* gripManager = GetIGripManager();
		gripManager->ResetGripUI(eChamferEdgeInvert); 
		GetEditPolyObj()->getParamBlock()->EnableNotifications(true);
	}
	break;
	case eChamferEdgeInvert:
	{
		int val = (int)value.mbool;
		GetEditPolyObj()->getParamBlock()->SetValue(ep_edge_chamfer_invert, t, val);
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_invert, t, val);
		GetEditPolyObj()->getParamBlock()->EnableNotifications(false);
		GetEditPolyObj()->getParamBlock()->SetValue(ep_edge_chamfer_open, t, false);
		IGripManager* gripManager = GetIGripManager();
	...
	{
		int val = (int)value.mbool;
		GetEditPolyObj()->getParamBlock()->SetValue(ep_edge_chamfer_smooth, t, val);
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_smooth, t, val);
		MaybeEnableControls();
	}
	break;
	case eChamferEdgeSmoothType:
		if (value.mInt > EPOLY_EDGE_CHAMFER_SMOOTH_CHAMFER) // go back to zero
			value.mInt = EPOLY_EDGE_CHAMFER_SMOOTH_ALL;
		if (value.mInt < EPOLY_EDGE_CHAMFER_SMOOTH_ALL)
			value.mInt = EPOLY_EDGE_CHAMFER_SMOOTH_CHAMFER;
		GetEditPolyObj()->getParamBlock()->SetValue(ep_edge_chamfer_smooth_type, t, value.mInt);
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_smooth_type, t, value.mInt);
		break;
	case eChamferEdgeSmoothThreshold:
		value.mFloat = std::clamp(value.mFloat, 0.0f, 180.0f);
		GetEditPolyObj()->getParamBlock()->SetValue(ep_edge_chamfer_smooth_threshold, t, value.mFloat);
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_smooth_threshold, t, value.mFloat);
		break;
	case eChamferEdgeMiteringType:
		if (value.mInt > EPOLY_EDGE_CHAMFER_MITER_PATCH) // go back to zero
			value.mInt = EPOLY_EDGE_CHAMFER_MITER_QUAD;
		GetEditPolyObj()->getParamBlock()->SetValue(ep_edge_chamfer_mitering_type, t, value.mInt);
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_mitering_type, t, value.mInt);
		MaybeEnableControls();
		break;
	case eChamferEdgeEndBias:
		value.mFloat = std::clamp(value.mFloat, 0.0f, 1.0f);
		GetEditPolyObj()->getParamBlock()->SetValue(ep_edge_chamfer_end_bias, t, value.mFloat);
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_end_bias, t, value.mFloat);
		break;
	case eChamferEdgeRadialBias:
		value.mFloat = std::clamp(value.mFloat, 0.0f, 1.0f);
		GetEditPolyObj()->getParamBlock()->SetValue(ep_edge_chamfer_radial_bias, t, value.mFloat);
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_radial_bias, t, value.mFloat);
		break;
	case eChamferEdgeDepth:
		value.mFloat = std::clamp(value.mFloat, -0.5f, 1.0f);
		GetEditPolyObj()->getParamBlock()->SetValue(ep_edge_chamfer_depth, t, value.mFloat);
		GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_depth, t, value.mFloat);
		break;
	default:
		return false;
...
bool ChamferEdgeGrip::ShowKeyBrackets(int which, TimeValue t)
{
	...
	{
	case eChamferEdgeAmount:
		return !!GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_edge_chamfer, t);
	case eChamferEdgeTension:
		return !!GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_edge_chamfer_tension, t);
	case eChamferEdgeSegments:
		return !!GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_edge_chamfer_segments, t);
	case eChamferEdgeSmoothThreshold:
		return !!GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_edge_chamfer_smooth_threshold, t);
	case eChamferEdgeEndBias:
		return !!GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_edge_chamfer_end_bias, t);
	case eChamferEdgeRadialBias:
		return !!GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_edge_chamfer_radial_bias, t);
	case eChamferEdgeDepth:
		return !!GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_edge_chamfer_depth, t);
	}
	return false;
}
...
bool ChamferEdgeGrip::ResetValue(TimeValue t, int which)
{
	...
	{
	case eChamferEdgeAmount:
		paramID = ep_edge_chamfer;
		break;
	case eChamferEdgeSegments:
		paramID = ep_edge_chamfer_segments;
		break;
	case eChamferEdgeTension:
		paramID = ep_edge_chamfer_tension;
		break;
	case eChamferEdgeOpen:
		paramID = ep_edge_chamfer_open;
		break;
	case eChamferEdgeInvert:
		paramID = ep_edge_chamfer_invert;
		break;
	case eChamferEdgeSmooth:
		paramID = ep_edge_chamfer_smooth;
		break;
	case eChamferEdgeSmoothType:
		paramID = ep_edge_chamfer_smooth_type;
		break;
	case eChamferEdgeSmoothThreshold:
		paramID = ep_edge_chamfer_smooth_threshold;
		break;
	case eChamferEdgeMiteringType:
		paramID = ep_edge_chamfer_mitering_type;
		break;
	case eChamferEdgeEndBias:
		paramID = ep_edge_chamfer_end_bias;
		break;
	case eChamferEdgeRadialBias:
		paramID = ep_edge_chamfer_radial_bias;
		break;
	case eChamferEdgeDepth:
		paramID = ep_edge_chamfer_depth;
		break;
	default:
		isFound = false;
		break;
...
{
	// cast is needed here to prevent build error - compiler can't decide which operator == to use.
	if(mPoly1Picked == const_cast<const TCHAR*>(GetString(IDS_BRIDGE_PICKING_POLYGON_1)))
		string = GetString(IDS_BRIDGE_PICKING_POLYGON_1);
	else if(mPoly2Picked == const_cast<const TCHAR*>(GetString(IDS_BRIDGE_PICKING_POLYGON_2)))
...
bool BridgePolygonGrip::GetText(int which,TSTR &string)
{
	...
	case eBridgePolygonsSmooth:
		{
			string =GetString(IDS_SMOOTH);
		}
		break;
	case eBridgePolygonsTwist1:
		{
			string =GetString(IDS_TWIST1);
		}
		break;
	case eBridgePolygonsTwist2:
		{
			string =GetString(IDS_TWIST2);
		}
		break;
	case eBridgePolygonsType:
		{
			//Group Normals, 1 for Local Normals, 2 for By Polygon 
			int type;
			//GetEditPolyObj()->getParamBlock()->GetValue (ep_BridgePolygons_type, 0, type);
			GetEditPolyObj()->getParamBlock()->GetValue (ep_bridge_selected, 0, type);
			switch(type)
			{
			case 0:
...
bool BridgePolygonGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eBridgePolygonsSegments:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_bridge_segments, t, value.mInt);
		break;
	case eBridgePolygonsTaper:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_bridge_taper, t, value.mFloat);
		break;
	case eBridgePolygonsBias:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_bridge_bias, t, value.mFloat);
		break;
	case eBridgePolygonsSmooth:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_bridge_smooth_thresh, t, value.mFloat);
		break;
	case eBridgePolygonsTwist1:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_bridge_twist_1, t, value.mInt);
		break;
	case eBridgePolygonsTwist2:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_bridge_twist_2, t, value.mInt);
		break;
	case eBridgePolygonsType:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_bridge_selected, t, value.mInt);
		break;
...
{
	int val;
	GetEditPolyObj()->getParamBlock()->GetValue (ep_bridge_selected, GetCOREInterface()->GetTime(), val);
	if( 1 == val )
	{
...
void BridgePolygonGrip::Cancel()
{
	...
	EditPolyObject* editable_poly = GetEditPolyObj();
	if (editable_poly)
	{
		editable_poly->ExitPickBridgeModes();
	}
	CommonCancel();
}
...
bool BridgePolygonGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eBridgePolygonsSegments:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_bridge_segments,t)==TRUE)? true : false;
	case eBridgePolygonsTaper:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_bridge_taper,t)==TRUE)? true : false;
	case eBridgePolygonsBias:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_bridge_bias,t)==TRUE)? true : false;
	case eBridgePolygonsSmooth:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_bridge_smooth_thresh,t)==TRUE)? true : false;
	case eBridgePolygonsTwist1:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_bridge_twist_1,t)==TRUE)? true : false;
	case eBridgePolygonsTwist2:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_bridge_twist_2,t)==TRUE) ? true : false;
	default:
		return false;
...
{
	// cast is needed here to prevent build error - compiler can't decide which operator == to use.
	if(mEdge1Picked == const_cast<const TCHAR*>(GetString(IDS_BRIDGE_PICKING_BORDER_1)))
		string = GetString(IDS_BRIDGE_PICKING_BORDER_1);
	else if(mEdge2Picked ==  const_cast<const TCHAR*>(GetString(IDS_BRIDGE_PICKING_BORDER_2)))
...
{
	// cast is needed here to prevent build error - compiler can't decide which operator == to use.
	if(mEdge1Picked == const_cast<const TCHAR*>(GetString(IDS_BRIDGE_PICKING_EDGE_1)))
		string =  GetString(IDS_BRIDGE_PICKING_EDGE_1);
	else if (mEdge2Picked ==  const_cast<const TCHAR*>(GetString(IDS_BRIDGE_PICKING_EDGE_2)))
...
bool BridgeEdgeGrip::GetText(int which,TSTR &string)
{
	...
	case eBridgeEdgesAdjacent:
		{
			string =GetString(IDS_BRIDGE_ADJACENT);
		}
		break;
	case eBridgeEdgesReverseTriangulation:
		{
			string =GetString(IDS_BRIDGE_REVERSE_TRI);
		}
		break;
	case eBridgeEdgesType:
		{
...
bool BridgeEdgeGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	case eBridgeEdgesSmooth:
		...
		break;
	case eBridgeEdgesAdjacent:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_bridge_adjacent, t, value.mFloat);
		break;
	case eBridgeEdgesReverseTriangulation:
		{
			int val = 0;
			GetEditPolyObj()->getParamBlock()->GetValue (ep_bridge_reverse_triangle, t, val);
			value.mbool = (val == 0 ? false:true) ;
		}
		break;
	case eBridgeEdgesType:
...
bool BridgeEdgeGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	case eBridgeEdgesSmooth:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_bridge_smooth_thresh,t)==TRUE)? true : false;
	case eBridgeEdgesAdjacent:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_bridge_adjacent,t)==TRUE)? true : false;
	case eBridgeEdgesReverseTriangulation:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_bridge_reverse_triangle,t)==TRUE)? true : false;
	default:
		return false;
...
bool RelaxGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eRelaxAmount:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_relax_amount, t, value.mFloat);
		break;
	case eRelaxIterations:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_relax_iters, t, value.mInt);
		break;
	case eRelaxBoundaryPoints:
		{
			int val = 0;
			GetEditPolyObj()->getParamBlock()->GetValue (ep_relax_hold_boundary, t, val );
			value.mbool = ( val == 0 ? false:true);
		}
		break;
	case eRelaxOuterPoints:
		{
			int val = 0;
			GetEditPolyObj()->getParamBlock()->GetValue (ep_relax_hold_outer, t, val );
			value.mbool = ( val == 0 ? false:true);
		}
		break;
	default:
...
bool RelaxGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	case eRelaxAmount:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_relax_amount,t)==TRUE)? true : false;
	case eRelaxIterations:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_relax_iters,t)==TRUE) ? true : false;
	case eRelaxBoundaryPoints:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_relax_hold_boundary,t)==TRUE) ? true : false;
	case eRelaxOuterPoints:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_relax_hold_outer,t)==TRUE) ? true : false;
	default:
		return false;
	}
...
void HingeGrip::GetGripName(TSTR &string)
{
	...
	// cast is needed here to prevent build error - compiler can't decide which operator == to use.
	if(mEdgePicked == const_cast<const TCHAR*>(GetString(IDS_PICKING_LIFT_EDGE)))
		string = mEdgePicked;
	else
		string =GetString(IDS_LIFT_FROM_EDGE);
}
...
{
	mEdgePicked =  GetString(IDS_PICKING_LIFT_EDGE);
...
void HingeGrip::SetEdgePicked(int num)
{
	...
	mEdgePicked = TSTR(GetString(IDS_EDGE)) + _T(" ") + buf;
...
{
	mEdgePicked =  GetString(IDS_PICK_LIFT_EDGE);
	mPickIcon = PICK_EMPTY_PNG;
	GetIGripManager()->ResetGripUI((int)eHingePick);
}
...
bool HingeGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	case eHingeAngle:
		{
			float valueRad = 0.0f;
			GetEditPolyObj()->getParamBlock()->GetValue (ep_lift_angle, t, valueRad);
			value.mFloat = RadToDeg(valueRad);
		}
		break;
	case eHingeSegments:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_lift_segments, t, value.mInt);
		break;
	case eHingePick:
		{
			//value.mbool, 0 means not in pick mode, 1 does.
...
void HingeGrip::Cancel()
{
	...
	if (editable_poly)
	{
		editable_poly->ExitPickLiftEdgeMode();
	}
	CommonCancel();
}
...
bool HingeGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	case eHingeAngle:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_lift_angle,t)==TRUE)? true : false;
	case eHingeSegments:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_lift_segments,t)==TRUE) ? true : false;
	default:
		return false;
	}
...
{
	// cast is needed here to prevent build error - compiler can't decide which operator == to use.
	if(mSplinePicked == const_cast<const TCHAR*>(GetString(IDS_PICKING_SPLINE)))
...
bool ExtrudeAlongSplineGrip::GetText(int which,TSTR &string)
{
	...
	case eExtrudeAlongSplineAlignToFaceNormal:
		{
			string =GetString(IDS_EXTRUDE_ALONG_SPLINE_ALIGN);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eExtrudeAlongSplineRotation:
		{
...
bool ExtrudeAlongSplineGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eExtrudeAlongSplineSegments:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_extrude_spline_segments, t, value.mInt);
		break;
	case eExtrudeAlongSplineTaperAmount:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_extrude_spline_taper, t, value.mFloat);
		break;
	case eExtrudeAlongSplineTaperCurve:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_extrude_spline_taper_curve, t, value.mFloat);
		break;
	case eExtrudeAlongSplineTwist:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_extrude_spline_twist, t, value.mFloat);
		break;
	case eExtrudeAlongSplineRotation:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_extrude_spline_rotation, t, value.mFloat);
		break;
	case eExtrudeAlongSplineAlignToFaceNormal:
		{
			//need to go BOOL (int) to bool
			int val;
			GetEditPolyObj()->getParamBlock()->GetValue (ep_extrude_spline_align, t, val);
			value.mbool =  ( val==0 ? false :true);
		}
		break;
	case eExtrudeAlongSplinePickSpline:
...
{
	int val;
	GetEditPolyObj()->getParamBlock()->GetValue (ep_extrude_spline_align,  GetCOREInterface()->GetTime(), val);
	if(!val)
	{
	...
	else
	{
		...
		GetEditPolyObj()->getParamBlock()->GetValue (ep_extrude_spline_node, TimeValue(0), splineNode);
		if (splineNode) 
...
bool ExtrudeAlongSplineGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	case eExtrudeAlongSplineSegments:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_extrude_spline_segments,t)==TRUE)? true : false;
	case eExtrudeAlongSplineTaperAmount:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_extrude_spline_taper,t)==TRUE) ? true : false;
	case eExtrudeAlongSplineTaperCurve:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_extrude_spline_taper_curve,t)==TRUE) ? true : false;
	case eExtrudeAlongSplineTwist:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_extrude_spline_twist,t)==TRUE) ? true : false;
	case eExtrudeAlongSplineAlignToFaceNormal:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_extrude_spline_align,t)==TRUE) ? true : false;
	case eExtrudeAlongSplineRotation:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_extrude_spline_rotation,t)==TRUE) ? true : false;
	default:
		return false;
	}
...
bool WeldVerticesGrip::GetText(int which,TSTR &string)
{
	...
	case eWeldVerticesWeldThreshold:
		{
			string =GetString(IDS_WELDTHRESHOLD);
		}
		break;
	case eWeldVerticesNum:
		{
...
bool WeldVerticesGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eWeldVerticesWeldThreshold:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_weld_threshold, t, value.mFloat);
		break;
	default:
		return false;
	}
...
bool WeldVerticesGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eWeldVerticesWeldThreshold:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_weld_threshold,t)==TRUE)? true : false;
	default:
		return false;
...
bool WeldEdgeGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eWeldEdgeWeldThreshold:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_edge_weld_threshold, t, value.mFloat);
		break;
	default:
		return false;
	}
...
bool WeldEdgeGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eWeldEdgeWeldThreshold:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_edge_weld_threshold,t)==TRUE)? true : false;
	default:
		return false;
	}
...
bool MSmoothGrip::GetText(int which,TSTR &string)
{
	...
	case eMSmoothSmoothness:
		{
			string =GetString(IDS_MS_SMOOTHNESS);// + TSTR(": ") + TSTR(str);
		}
		break;
	case eMSmoothSmoothingGroups:
		{
			string =GetString(IDS_MS_SEP_BY_SMOOTH);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eMSmoothMaterials:
		{
			string =GetString(IDS_MS_SEP_BY_MATID);//+ TSTR(": ") + TSTR(str);
		}
		break;
	default:
		return false;
...
bool MSmoothGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eMSmoothSmoothness:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_ms_smoothness, t, value.mFloat);
		break;
	case eMSmoothSmoothingGroups:
		{
			int val = 0;
			GetEditPolyObj()->getParamBlock()->GetValue (ep_ms_sep_smooth, t, val);
			value.mbool = (val == 0 ? false:true);
		}
		break;
	case eMSmoothMaterials:
		{
			int val = 0;
			GetEditPolyObj()->getParamBlock()->GetValue (ep_ms_sep_mat, t, val);
			value.mbool = ( val == 0 ? false:true);
		}
		break;
	default:
...
bool MSmoothGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eMSmoothSmoothness:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_ms_smoothness,t)==TRUE)? true : false;
	case eMSmoothSmoothingGroups:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_ms_sep_smooth,t)==TRUE) ? true : false;
	case eMSmoothMaterials:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_ms_sep_mat,t)==TRUE) ? true : false;
	default:
		return false;
	}
...
bool TessellateGrip::GetText(int which,TSTR &string)
{
	...
	case eTessellateType:
		{
			//Edge,Face
			int type;
			//GetEditPolyObj()->getParamBlock()->GetValue (ep_Tessellate_type, 0, type);
			GetEditPolyObj()->getParamBlock()->GetValue (ep_tess_type, 0, type);
			switch(type)
			{
			case 0:
				string =  GetString(IDS_TESS_EDGE);
				break;
			case 1:
				string =  GetString(IDS_TESS_FACE);
				break;
	...
	case eTessellateTension:
		{
			string =GetString(IDS_TENSION);// + TSTR(": ") + TSTR(str);
		}
		break;
	default:
		return false;
...
bool TessellateGrip::GetComboOptions(int which, Tab<IBaseGrip::ComboLabel*> &comboOptions)
{
	if(which== eTessellateType)
	{
		...
		label->mIcon = string;
		label->mLabel = GetString(IDS_TESS_EDGE);
		comboOptions.Append(1,&label);
		label = new IBaseGrip::ComboLabel();
		string = TESSELLATE_FACE_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_TESS_FACE);
		comboOptions.Append(1,&label);
		return true;
	}
	return false;
...
bool TessellateGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eTessellateType:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_tess_type, t, value.mInt);
		break;
	case eTessellateTension:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_tess_tension, t, value.mFloat);
		break;
	default:
		return false;
	}
...
{
	int val;
	GetEditPolyObj()->getParamBlock()->GetValue (ep_tess_type,  GetCOREInterface()->GetTime(), val);
	if( 1 == val )
...
bool TessellateGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	case eTessellateType:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_tess_type,t)==TRUE)? true : false;
	case eTessellateTension:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_tess_tension,t)==TRUE) ? true : false;
	default:
		return false;
	}
