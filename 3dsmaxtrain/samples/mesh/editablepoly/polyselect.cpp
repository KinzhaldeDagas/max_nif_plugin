...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_POLYMESH_SEL_MOD) : _T("Poly Select");}  
	virtual Class_ID ClassID() { return POLYMESH_SEL_CLASS_ID;}		
	RefTargetHandle Clone(RemapDir& remap);
	const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_POLYMESH_SEL_MOD) : _T("Poly Select");}
	...
	ChannelMask ChannelsChanged() {return SELECT_CHANNEL|SUBSEL_TYPE_CHANNEL|GEOM_CHANNEL|TOPO_CHANNEL;}	// There is no real select channel for PolyObjects; changes to selection affect vertices (GEOM_CHANNEL) and faces and edges (TOPO_CHANNEL)
...
public:
    ...
    // As the Backfacing and Occluded culling options in the modifier rollout should be active only for selection operations,
    // the corresponding parameter block entries are enforced for POINT_RGN hit tests via the HitTest function only between a
    // pair of calls to BeginPointSelectionHitTesting and EndPointSelectionHitTesting.
...
private:
    ...
    // When true, calls to HitTest for a POINT_RGN test type are interpreted as being selection hit tests, so that the
    // Backfacing and Occluded culling options in the modifier rollout are honored
...
public:
	...
	DoubleClickSelector doubleClickSelector;
	...
	void FreeCache();
	void SynchBitArrays();
	//copies all the mesh selections into the local data cache
	void LockSelection(MNMesh& mesh);
	...
	BitArray GetVertSel() override { return vertSel; }
	BitArray GetFaceSel() override { return faceSel; }
	BitArray GetEdgeSel() override { return edgeSel; }
	...
	GenericNamedSelSetList & GetNamedVertSelList () override { return vselSet; }
	GenericNamedSelSetList & GetNamedEdgeSelList () override { return eselSet; }
	GenericNamedSelSetList & GetNamedFaceSelList () override { return fselSet; }
...
class PolySelectClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Poly Select"); }
...
class PolyMeshSelSubModSelectionProcessor : public SubModSelectionProcessor
{
protected:
	// override
	...
	virtual void OverrideDoubleClickProc(ViewExp* vpt, int flags) override
	{
		...
		PolyMeshSelMod* pPolyObj = dynamic_cast<PolyMeshSelMod*>(obj);
		if (NULL != pPolyObj)
		{
			theHold.Begin();
			...
			// TODO: Use this once select edge loop has been implemented
			//const TCHAR* opDesc = GetString(pPolyObj->GetSelLevel() == MNM_SL_EDGE ? IDS_SELECT_EDGE_LOOP : IDS_SELECT);
			const TCHAR* opDesc = GetString(IDS_SELECT);
			theHold.Accept(opDesc);
		}
	}

public:
	PolyMeshSelSubModSelectionProcessor(TransformModBox* mc, BaseObject* o, IObjParam* i)
...
using PolyDoubleClickModeDecorator = DoubleClickModeDecorator<BasePolyMode, PolyMeshSelSubModSelectionProcessor, ModBoxType>;
...
enum { ps_use_softsel, ps_use_edist, ps_edist, ps_affect_backfacing,
		...
		ps_ignore_occluded, ps_use_stack_selection };
...
static ParamBlockDesc2 polysel_softsel_desc ( ps_pblock,
	...
	ps_edist, _T("softselEdgeDist"), TYPE_INT, P_RESET_DEFAULT|P_ANIMATABLE, IDS_EDGE_DIST,
		...
		p_nonLocalizedName, _T("Edge Distance"),
		p_end,

	ps_affect_backfacing, _T("softselAffectBackfacing"), TYPE_BOOL, P_RESET_DEFAULT, IDS_AFFECT_BACKFACING,
		p_default, true,
		p_ui, ps_map_softsel, TYPE_SINGLECHECKBOX, IDC_AFFECT_BACKFACING,
		p_end,
	...
	ps_by_vertex, _T("byVertex"), TYPE_BOOL, P_RESET_DEFAULT, IDS_SEL_BY_VERTEX,
		p_default, false,
		p_ui, ps_map_main, TYPE_SINGLECHECKBOX, IDC_MS_SEL_BYVERT,
		p_end,
	...
	ps_ignore_occluded, _T("ignoreOccluded"), TYPE_BOOL, P_RESET_DEFAULT, IDS_IGNORE_OCCLUDED,
		p_default, FALSE,
		p_accessor, &thePolyMeshSelModPBAccessor,
		p_end,
	...
	ps_matid, _T("materialID"), TYPE_INT, P_RESET_DEFAULT|P_TRANSIENT, IDS_MATERIAL_ID,
		...
		p_end,
	ps_use_stack_selection, _T("useStackSelection"), TYPE_BOOL, P_RESET_DEFAULT, IDS_USE_STACK_SELECTION,
		p_default, false,
		p_ui, ps_map_main, TYPE_SINGLECHECKBOX, IDC_USE_STACK_SELECTION,
		p_end,
...
void PolyMeshSelMod::ModifyObject (TimeValue t, ModContext &mc, ObjectState *os, INode *node) {
	...
	if (pblock->GetInt(ps_use_stack_selection))
	{
		d->LockSelection(pobj->GetMesh());
	}
	...
	pobj->GetMesh().dispFlags &= ~dispFlagMask;
	if ((selLevel != SEL_VERTEX) || !ip || !ip->GetShowEndResult())
		pobj->GetMesh().SetDispFlag (levelDispFlags[selLevel]);

	// sync edge selection to the face edge sel bitarray
	// this is needed since if we FillinMesh later we rebuild 
	// the edge selection from the face border 
	// edge selection info MNFace::edgeSel
	{
		MNMesh& msh = pobj->GetMesh();
		for (size_t faceIndex = 0; faceIndex < msh.numf; ++faceIndex)
		{
			MNFace& f = msh.f[faceIndex];
			if (f.GetFlag(MN_DEAD))
				continue;
			size_t degree = f.deg;
			for (int cornerIndex = 0; cornerIndex < degree; ++cornerIndex)
			{
				int edgeIndex = f.edg[cornerIndex];
				f.edgsel.Set(cornerIndex, msh.e[edgeIndex].GetFlag(MN_SEL));
			}
		}
	}
...
void PolyMeshSelMod::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc) {
	if (!mc->localData) return;
	if (partID == PART_SELECT) return;
	if (pblock->GetInt(ps_use_stack_selection))	
	{
		((PolyMeshSelData*)mc->localData)->InvalidateVDistances();		
		return;
	}
...
void PolyMeshSelMod::BeginEditParams (IObjParam  *ip, ULONG flags,Animatable *prev) {
    typedef PolySelectionProcessor<PolyMeshSelSubModSelectionProcessor, PolyMeshSelMod> PolySelectModSelectionProcessor;
    ...
    selectMode = new DoubleClickModeDecorator<SelectModBoxCMode, PolySelectModSelectionProcessor, SelectModBox>(this, ip);
...
void PolyMeshSelMod::SetupBackFaceCulling(TimeValue t, const HitRegion& hr, GraphicsWindow* gw, DWORD& hitFlags)
{
    ...
    {
    	pblock->GetValue(ps_ignore_backfacing, t, ignoreBackfacing);
	    pblock->GetValue(ps_ignore_occluded, t, ignoreOccluded);
    }
...
int PolyMeshSelMod::HitTest (TimeValue t, INode* inode, int type, int crossing, int flags, IPoint2 *p, ViewExp *vpt, ModContext* mc) {
	...
	int selByVert = 0;
	pblock->GetValue (ps_by_vertex, t, selByVert);
	...
	DWORD hitFlags = flags;
	DWORD hitLev = 0;

	if (selLevel > SEL_VERTEX && selByVert)
	{
		hitLev = hitLevel[SEL_VERTEX];
		hitFlags |= SUBHIT_MNUSECURRENTSEL;
	}
	else
	{
		hitLev = hitLevel[selLevel];
	}
	
	SetupBackFaceCulling(t, hr, gw, hitFlags);
    ...
    if ((hr.type != POINT_RGN) && (hitFlags & SUBHIT_IGNOREOCCLUDED))
    {
        TCHAR* statusMsg = GetString(IDS_COMPUTING_OCCLUSION_MESSAGE);
        waitingStatus.SetWaitingStatus(statusMsg);
    }

	res = pMesh->SubObjectHitTest(gw, gw->getMaterial(), &hr, hitFlags | hitLev, hitList);
...
//		gw->polyline (2, rp, NULL, NULL, FALSE, es);
	...
	if (selLevel == SEL_VERTEX) {
		...
		if (pblock) pblock->GetValue (ps_use_softsel, t, affectRegion);
...
void PolyMeshSelMod::ActivateSubobjSel(int level, XFormModes& modes) {
	...
	for (int i = 0; i < list.Count(); i++)
	{
		...
		pData->doubleClickSelector.Reset();
	}
...
void PolyMeshSelMod::SelectSubComponent (HitRecord *firstHit, BOOL selected, BOOL all, BOOL invert) {
	...
	for (HitRecord* hr = firstHit; hr != NULL; hr = hr->Next())
	{
		PolyMeshSelData* pData = (PolyMeshSelData*)hr->modContext->localData;
		switch (selLevel)
		{
		case SEL_VERTEX:
			pData->doubleClickSelector.OnSingleClickVertex(hr->hitInfo);
			break;
		case SEL_EDGE:
			pData->doubleClickSelector.OnSingleClickEdge(hr->hitInfo);
			break;
		case SEL_FACE:
			pData->doubleClickSelector.OnSingleClickFace(hr->hitInfo);
			break;
		}
	...
	int selByVert=0;
	pblock->GetValue (ps_by_vertex, TimeValue(0), selByVert);
...
void PolyMeshSelMod::DoubleClickSelect(HitRecord *hitRec)
{
	if (hitRec == NULL) return;
	if (!ip) return;
	if (selLevel == SEL_OBJECT) return;

	const bool inMayaSelectionMode = MaxSDK::CUI::GetIMouseConfigManager()->GetMayaSelection();
	...
	PolyMeshSelData* pData = (PolyMeshSelData*)hitRec->modContext->localData;
	...
	if (!pData->GetNewSelection())
	{
		...
		BitArray& newSelection = *pData->GetNewSelection();

		if (selLevel == SEL_VERTEX)
		{
			newSelection = pData->doubleClickSelector.OnDoubleClickVertex(mm, add, sub, hitRec->hitInfo);
		}
		else if (selLevel == SEL_FACE)
		{
			newSelection = pData->doubleClickSelector.OnDoubleClickFace(mm, add, sub, hitRec->hitInfo);
		}
		else if (selLevel == SEL_EDGE)
		{
			newSelection = pData->doubleClickSelector.OnDoubleClickEdge(mm, add, sub, hitRec->hitInfo);
		}
		else if (selLevel == SEL_ELEMENT)
		{
			if (sub && !add)
				ClearSelection(SEL_ELEMENT);
			else
				SelectAll(SEL_ELEMENT);
		}
	}

	pData->ApplyNewSelection(selLevel);
	LocalDataChanged();
}
...
void SelectProc::SetEnables (HWND hParams) {
    ...
    IParamBlock2* params = mod->GetParamBlock(0);
    ...
    but = GetICustButton(GetDlgItem(hParams, IDC_MS_CULL_BACKFACE));
    if (but != nullptr)
    {
        ...
        if (!obj)
        {
            ...
            params->GetValue(ps_ignore_backfacing, timeCurr, ignoreBackfacing, interval);
    ...
    but = GetICustButton(GetDlgItem(hParams, IDC_MS_CULL_OCCLUDED));
    if (but != nullptr)
    {
        ...
        if (!obj)
        {
            BOOL ignoreOccluded = FALSE;
            Interval interval = FOREVER;
            params->GetValue(ps_ignore_occluded, timeCurr, ignoreOccluded, interval);
	...
	if (params->GetInt(ps_use_stack_selection))
	{
		...
		EnableWindow (GetDlgItem (hParams, IDC_MS_SELBYMAT_BOX), FALSE);
		EnableWindow (GetDlgItem (hParams, IDC_MS_SELBYMAT_TEXT), FALSE);
		...
		{	
			spin->Enable (FALSE);
			EnableWindow (GetDlgItem (hParams, IDC_MS_EDGE_RING_SEL_SPIN), FALSE);
			ReleaseISpinner (spin);
		}
...
INT_PTR SelectProc::DlgProc (TimeValue t, IParamMap2 *map,
	...
	case WM_INITDIALOG:
	{
		...
		l_iToolbar->AddTool(EDITPOLY_REGULAR(CTB_CHECKBUTTON, EditPolyIcon::vertexSelection, IDC_SELVERTEX));
		l_iToolbar->AddTool(EDITPOLY_REGULAR(CTB_CHECKBUTTON, EditPolyIcon::edgeSelection, IDC_SELEDGE));
		l_iToolbar->AddTool(EDITPOLY_REGULAR(CTB_CHECKBUTTON, EditPolyIcon::borderSelection, IDC_SELBORDER));
		l_iToolbar->AddTool(EDITPOLY_REGULAR(CTB_CHECKBUTTON, EditPolyIcon::polygonSelection, IDC_SELFACE));
		l_iToolbar->AddTool(EDITPOLY_REGULAR(CTB_CHECKBUTTON, EditPolyIcon::elementSelection, IDC_SELELEMENT));
		ReleaseICustToolbar(l_iToolbar);
		...
		ICustButton* but = GetICustButton(GetDlgItem(hWnd, IDC_MS_CULL_BACKFACE));
		if (but != nullptr)
		{
		...
		but = GetICustButton(GetDlgItem(hWnd, IDC_MS_CULL_OCCLUDED));
		if (but != nullptr)
		{
			but->SetType(CBT_CHECK);
			ReleaseICustButton(but);
	...
	case WM_COMMAND:
		...
		case IDC_MS_SELBYMAT:
			mod->pblock->GetValue (ps_matid, t, matid);
		...
		case IDC_MS_CULL_BACKFACE:
		{
			theHold.Begin();
			BOOL checkboxValue = FALSE;
			mod->pblock->GetValue(ps_ignore_backfacing, TimeValue(0), checkboxValue);
			mod->pblock->SetValue(ps_ignore_backfacing, TimeValue(0), !checkboxValue);
			theHold.Accept(GetString(IDS_PARAMETERS));
			break;
		}

		case IDC_MS_CULL_OCCLUDED:
		{
			theHold.Begin();
			BOOL checkboxValue = FALSE;
			mod->pblock->GetValue(ps_ignore_occluded, TimeValue(0), checkboxValue);
			mod->pblock->SetValue(ps_ignore_occluded, TimeValue(0), !checkboxValue);
			theHold.Accept(GetString(IDS_PARAMETERS));
			break;
		}
		case IDC_USE_STACK_SELECTION:
		{
			SetEnables(hWnd);
			return FALSE;
			break;
...
void SoftSelProc::SetEnables (HWND hWnd) {
	...
	pblock->GetValue (ps_use_softsel, TimeValue(0), softSel);
	pblock->GetValue (ps_use_edist, TimeValue(0), useEDist);
...
void PolyMeshSelMod::OnParamSet(PB2Value& v, ParamID id, int tabIndex, TimeValue t) {
	...
	case ps_paintsel_mode:
		...
		break;
	case ps_ignore_backfacing:
	{
		// When we enable ignore back facing, we disable ignore occluded
		if (v.i)
			pblock->SetValue(ps_ignore_occluded, t, !v.i);

		// Make sure our button's state reflects its status
		HWND hwnd = GetHWnd(pblock, ps_map_main);
		ICustButton* pBut = GetICustButton(GetDlgItem(hwnd, IDC_MS_CULL_BACKFACE));
		if (pBut != nullptr)
		{
			pBut->SetCheck(v.i);
	...
	case ps_ignore_occluded:
	{
		// When we enable ignore occluded, we disable ignore back facing
		if (v.i)
			pblock->SetValue(ps_ignore_backfacing, t, !v.i);

		// Make sure our button's state reflects its status
		HWND hwnd = GetHWnd(pblock, ps_map_main);
		ICustButton* pBut = GetICustButton(GetDlgItem(hwnd, IDC_MS_CULL_OCCLUDED));
		if (pBut != nullptr)
		{
			pBut->SetCheck(v.i);
			ReleaseICustButton(pBut);
...
const TCHAR *PolyMeshSelMod::GetPaintUndoString (PaintModeID mode) {
...
void PolyMeshSelData::LockSelection(MNMesh& mesh)
{
	mesh.getVertexSel (vertSel);
	mesh.getFaceSel (faceSel);
	mesh.getEdgeSel (edgeSel);
...
void PolyMeshSelData::SetVertSel(const BitArray &set, IMeshSelect *imod, TimeValue t) {
...
void PolyMeshSelData::SetFaceSel(const BitArray &set, IMeshSelect *imod, TimeValue t) {
	PolyMeshSelMod *mod = (PolyMeshSelMod *) imod;
...
void PolyMeshSelData::SetEdgeSel(const BitArray &set, IMeshSelect *imod, TimeValue t) {
	PolyMeshSelMod *mod = (PolyMeshSelMod *) imod;
...
RefResult PolyMeshSelMod::NotifyRefChanged (const Interval& changeInt, RefTargetHandle hTarget, 
   		PartID& partID, RefMessage message, BOOL propagate) 
{
...
void PolyMeshSelMod::BeginPointSelectionHitTesting()
{
    m_doPointSelectionHitTest = true;
}

void PolyMeshSelMod::EndPointSelectionHitTesting()
{
    m_doPointSelectionHitTest = false;
}

void PolyMeshSelMod::InvalidateDialogElement (ParamID elem) {
	if (!pblock) return;
	if (!pSelDesc.NumParamMaps ()) return;
	IParamMap2 *pmap = pSelDesc.GetParamMap (ps_map_main);
