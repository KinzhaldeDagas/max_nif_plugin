...
void DebugPrintLiveBits (BitArray & bitArray) {
	...
	for (bit=0; bit<bitArray.GetSize(); bit++) {
		...
		DebugPrint(_T("%2d "), bit);
		numLive++;
		if (numLive%8==7) DebugPrint(_T("\n"));
	}
	if (numLive%8) DebugPrint(_T("\n"));
}

#define EDIT_NORMALS_CLASS_ID Class_ID(0x4aa52ae3, 0x35ca1cde)

static GenSubObjType SOT_Normal(NULL,_T("CommandPanel/Modify/ModifierStack/NormalStack"), NULL);
...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_EDIT_NORMALS_MOD) : _T("Edit Normals");}  
	virtual Class_ID ClassID() { return EDIT_NORMALS_CLASS_ID;}		
	RefTargetHandle Clone(RemapDir& remap);
	const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_EDIT_NORMALS_MOD) : _T("Edit Normals");}
	void* GetInterface(ULONG id) { return Modifier::GetInterface(id); }

	// From modifier
	ChannelMask ChannelsUsed()  {return TOPO_CHANNEL|GEOM_CHANNEL|EXTENSION_CHANNEL;}	// Channels used by normals
	ChannelMask ChannelsChanged() {return TOPO_CHANNEL|GEOM_CHANNEL|SUBSEL_TYPE_CHANNEL|EXTENSION_CHANNEL;}	// Channels normals are stored in - and we set to object level.
	Class_ID InputType() {return mapObjectClassID;}	// Disallow non-mappable objects like cameras.
...
private:
	virtual void SetReference(int i, RefTargetHandle rtarg) { mpParamBlock = (IParamBlock2 *) rtarg; }
public:
...
public:
	...
	void SetGeomValid (const Interval & iValid) { mGeomValid = iValid; }
	void SetTopoValid (const Interval & iValid) { mTopoValid = iValid; }
...
void NormalSelectNotify::Restore (int isUndo) {
	mpMod->LocalDataChanged(SELECT_CHANNEL);
}
...
void NormalTransformRestore::Restore (int isUndo) {
	...
	mpMod->LocalDataChanged(GEOM_CHANNEL);
}
...
void NormalEditRestore::Restore (int isUndo) {
	...
	mpMod->LocalDataChanged(TOPO_CHANNEL|GEOM_CHANNEL);
}
...
class EditNormalsClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Edit Normals"); }
...
void EditNormalProc::SetEnables (HWND hParams, int numSelected) {
	...
	int selectBy;
	mpMod->GetParamBlock(0)->GetValue (en_select_by, TimeValue(0), selectBy);
...
INT_PTR EditNormalProc::DlgProc (TimeValue t, IParamMap2 *map,
...
protected:
	...
	bool SetTargetBase();
	void DrawAverageLine (HWND hWnd, bool bErase, bool bPresent);
...
bool ENAverageNormalsMouseProc::GetTargetCenter(Point3 &center)
{
   ...
   int i;
	for (i=0; i<list.Count(); i++) if (nodes[i] == mpNode) break;
...
void ENAverageNormalsMouseProc::DrawAverageLine(HWND hWnd, bool bErase, bool bPresent)
{
	if (!ip || !mpNode) return;
	...
	GraphicsWindow *gw = ip->GetViewExp(hWnd).getGW();
...
int ENAverageNormalsMouseProc::proc (HWND hwnd, int msg, int point, int flags, IPoint2 m)
{
	...
	case MOUSE_ABORT:
		if (mpNode) {
			if (mLineDrawn) DrawAverageLine(hwnd, true, true);		// erase the previous line
			mpNode = NULL;
		}
		res = FALSE;
		break;
	...
	case MOUSE_POINT:
		...
		if ((hr = HitTest (vpt.ToPointer(), m)) != NULL) {
	...
	case MOUSE_MOVE:
		if (mpNode) {
			if (mLineDrawn) DrawAverageLine(hwnd, true, false);			// erase the previous line
			mMouse = m;
			DrawAverageLine(hwnd, false, true);
			mLineDrawn = true;
		}
...
static ParamBlockDesc2 edit_normals_desc ( en_pblock,
	...
	en_ignore_backfacing, _T("ignoreBackfacing"), TYPE_BOOL, 0, IDS_EN_IGNORE_BACKFACING,
		p_default, false,
		p_ui, TYPE_SINGLECHECKBOX, IDC_EN_IGNORE_BACKFACING,
		p_end,

	en_show_handles, _T("showHandles"), TYPE_BOOL, 0, IDS_EN_SHOW_HANDLES,
		p_default, false,
		p_ui, TYPE_SINGLECHECKBOX, IDC_EN_SHOW_HANDLES,
		p_end,
...
static FPInterfaceDesc editNormalsModInterfaceDesc (EDIT_NORMALS_MOD_INTERFACE,
	...
	enfn_get_vertex_id, _T("GetVertexID"), 0, TYPE_INDEX, 0, 3,
		...
		_T("corner"), 0, TYPE_INDEX,
		_T("node"), 0, TYPE_INODE, f_keyArgDefault, NULL,
	enfn_get_vertex, _T("GetVertex"), 0, TYPE_POINT3_BV, 0, 2,
	...
	enums,
		enprop_sel_level, 5,
			_T("Object"), EN_SL_OBJECT,
			_T("Normal"), EN_SL_NORMAL,
			_T("Vertex"), EN_SL_VERTEX,
			_T("Edge"), EN_SL_EDGE,
			_T("Face"), EN_SL_FACE,
...
int EditNormalsMod::EnfnGetSelLevel () {
	...
	int selBy;
	mpParamBlock->GetValue (en_select_by, TimeValue(0), selBy);
	switch (selBy) {
...
RefTargetHandle EditNormalsMod::Clone(RemapDir& remap) {
	...
	mod->ReplaceReference (0, remap.CloneRef(mpParamBlock));
...
void EditNormalsMod::ModifyPolyObject (TimeValue t, EditNormalsModData *pData, PolyObject *pobj) {
   ...
   int i;
	for (i=0; i<pobj->NumXTCObjects(); i++)
	{
...
void EditNormalsMod::ModifyTriObject (TimeValue t, EditNormalsModData *pData, TriObject *tobj) {
	...
	MeshNormalSpec *pPipeNormals = (MeshNormalSpec *) tobj->GetMesh().GetInterface (MESH_NORMAL_SPEC_INTERFACE);
	if (pPipeNormals) {
		//Parent must be set before copy any data. Then it can inform the parent about topology changes.
	...
	// (Note: This object may already exist, in which case we don't need to add it.)
   int i;
	for (i=0; i<tobj->NumXTCObjects(); i++)
	{
...
void EditNormalsMod::ModifyObject (TimeValue t, ModContext &mc, ObjectState *os, INode *node) {
	...
	if (!pobj && !tobj) return;	// If it's not an object we support, don't go any further.
...
int EditNormalsMod::HitTest (TimeValue t, INode* inode, int type, int crossing, int flags, IPoint2 *p, ViewExp *vpt, ModContext* mc) {
	...
	mpParamBlock->GetValue (en_ignore_backfacing, t, ignoreBackfacing);
	...
	float dispLength;
	int showHandles;
	mpParamBlock->GetValue (en_display_length, t, dispLength);
	mpParamBlock->GetValue (en_show_handles, t, showHandles);
...
void EditNormalsMod::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *pMC) {
	if (!pMC->localData) return;
...
DWORD EditNormalsMod::AddSet(TSTR &setName) {
   ...
   int pos;
	for (pos=0; pos<nsCount; pos++) if (setName < *(mNamedSelNames[pos])) break;
	if (pos == nsCount) {
...
bool EditNormalsMod::EnfnRotate (Quat & rotation, TimeValue t) {
	...
	for (int i=0; i<mcList.Count(); i++) {
		...
		if (theHold.Holding() && !pData->GetFlag(ENMD_NT_HELD))
			theHold.Put(new NormalTransformRestore(this, pData));
		if (pData->GetFlag(ENMD_TRIMESH)) {
			if (pData->GetTriNormals()->Transform (rotMatrix, true, nullptr, false)) ret = true;
		} else {
			if (pData->GetPolyNormals()->Transform (rotMatrix, true, nullptr, false)) ret = true;
		}
	}
	nodes.DisposeTemporary();
	LocalDataChanged(GEOM_CHANNEL);
...
RefResult EditNormalsMod::NotifyRefChanged (const Interval& changeInt, RefTargetHandle hTarget, 
   		PartID& partID, RefMessage message, BOOL propagate) {
	if ((message == REFMSG_CHANGE) && (hTarget == mpParamBlock)) {
		...
		ParamID pid = mpParamBlock->LastNotifyParamID();
...
void EditNormalsMod::UpdateDialog() {	
	...
	for (int i = 0; i < mcList.Count(); i++) {
		...
		if ((pData->GetTriNormals() == NULL) && (pData->GetPolyNormals() == NULL))
			continue;
...
void EditNormalsMod::InvalidateDialogElement (ParamID elem) {
...
void EditNormalsMod::LocalDataChanged (ChannelMask channels) {
	ChannelMask partsChanged = DISP_ATTRIB_CHANNEL;
	partsChanged |= partsChanged & (TOPO_CHANNEL|GEOM_CHANNEL);
	NotifyDependents(FOREVER, partsChanged, REFMSG_CHANGE);
	if (mpInterface && (channels & (SELECT_CHANNEL|TOPO_CHANNEL))) {
		UpdateDialog ();
		UpdateNamedSelDropDown ();
	}
}
...
bool EditNormalsMod::BreakNormals (bool toAverage) {
	...
	for (int i=0; i<list.Count(); i++) {
		...
		if ((pData->GetTriNormals()==NULL) && ((pData->GetPolyNormals()==NULL))) {
				continue;	// Swatje this can happen if the user sends an empty mesh into the modifier 
			}
...
//				DbgAssert (0);
				continue;	// Shouldn't happen.. watje Took the assert out, this can happen if the user sends an empty mesh into the modifier 
			}
			theHold.Put (new NormalEditRestore (this, pData));
...
bool EditNormalsMod::ResetNormals () {
	...
	for (int i=0; i<list.Count(); i++) {
		...
		if ((pData->GetTriNormals()==NULL) && ((pData->GetPolyNormals()==NULL))) {
				continue;	// watje this can happen if the user sends an empty mesh into the modifier 
			}
...
bool EditNormalsMod::CopyNormal () {
	...
	//this condition no normals have been selected so we bail
	if (whichContext == -1)
	 	return false;
	...
	// Find our normal:
   int whichNormal;
	for (whichNormal=0; whichNormal<pData->EnfnGetNumNormals(); whichNormal++) {
...
bool EditNormalsMod::EnfnBreakNormals (BitArray *normalSelection, INode *pNode, bool toAverage) {
	...
	nodes.DisposeTemporary();
	if (ret) LocalDataChanged(TOPO_CHANNEL|GEOM_CHANNEL);
	return ret;
}
...
bool EditNormalsMod::EnfnMakeNormalsExplicit (BitArray *normalSelection, INode *pNode) {
	...
	nodes.DisposeTemporary();
	if (ret) LocalDataChanged(DISP_ATTRIB_CHANNEL);
	return ret;
}
...
bool EditNormalsMod::EnfnAverageNormals (bool useThresh, float threshold, BitArray *normalSelection, INode *pNode) {
	...
	nodes.DisposeTemporary();
	if (ret) LocalDataChanged(GEOM_CHANNEL);
...
bool EditNormalsMod::EnfnAverageGlobalNormals (bool useThresh, float threshold) {
	...
	if (useThresh) {
		...
		MaxSDK::Array<BitArray> vertexSelection;
		MaxSDK::Array<BitArray> vertexNearby;
		for (int i=0; i<list.Count(); i++) {
			// NOTE: Tab<> doesn't initialize the BitArray. So, append it one by one.
			BitArray emptyBitArray;
			vertexSelection.append(emptyBitArray);
			vertexNearby.append(emptyBitArray);
...
void EditNormalsMod::EnfnSetNormalExplicit (int normalIndex, bool value, INode *pNode) {
	...
	LocalDataChanged(GEOM_CHANNEL|TOPO_CHANNEL);
}
...
bool EditNormalsModData::EnfnSetSelection (BitArray & selection) {
	...
	properSelection.SetSize (properSize, TRUE);
...
void EditNormalsModData::EnfnConvertVertexSelection (BitArray & vertexSelection, BitArray & normalSelection) {
	if (GetFlag (ENMD_TRIMESH)) {
		...
		normalSelection.SetSize (mpLocalTriNormals->GetSelection().GetSize());
	} else {
	...
	if (!vertexSelection.NumberSet()) return;
	...
	if (GetFlag (ENMD_TRIMESH)) {
		...
		for (int i=0; i<maxFace; i++) {
			...
			for (int j=0; j<3; j++) {
				...
				if (vertexID >= vertexSelection.GetSize()) continue;
...
void EditNormalsModData::EnfnConvertEdgeSelection (BitArray & edgeSelection, BitArray & normalSelection) {
	...
	if (GetFlag (ENMD_TRIMESH)) {
		...
		for (int i=0; i<maxFace; i++) {
			...
			for (int j=0; j<3; j++) {
				int edgeID = i*3+j;
				if (edgeID >= edgeSelection.GetSize()) continue;
...
void EditNormalsModData::EnfnConvertFaceSelection (BitArray & faceSelection, BitArray & normalSelection) {
	...
	normalSelection.ClearAll();
	if (!faceSelection.NumberSet()) return;
...
int EditNormalsModData::EnfnGetNumFaces () {
	if (GetFlag (ENMD_TRIMESH)) {
		...
		return mpLocalPolyNormals ? mpLocalPolyNormals->GetNumFaces() : 0;
	}
}
