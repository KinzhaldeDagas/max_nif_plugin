...
// Edge chamfer types
enum {
	EPOLY_STANDARD_EDGE_CHAMFER = 0,	// (Legacy) Mitering type = tri
	EPOLY_QUAD_EDGE_CHAMFER				// (Legacy) Mitering type = quad
};

// Edge chamfer smoothing types
enum {
	EPOLY_EDGE_CHAMFER_SMOOTH_ALL = 0,		// Entire object
	EPOLY_EDGE_CHAMFER_SMOOTH_CHAMFER		// Just the chamfers we created
};

// Edge chamfer mitering types
enum
{
	EPOLY_EDGE_CHAMFER_MITER_QUAD = 0,		// Legacy "Quad" Chamfer
	EPOLY_EDGE_CHAMFER_MITER_TRI,			// Legacy "Standard" Chamfer
	EPOLY_EDGE_CHAMFER_MITER_UNIFORM,		// Maya Uniform style (Legacy Quad Chamfer + Quad Intersections)
	EPOLY_EDGE_CHAMFER_MITER_RADIAL,		// Same as uniform style but with radial corners
	EPOLY_EDGE_CHAMFER_MITER_PATCH			// Same as uniform style but with patch corners
