...
void EditPolyObject::LocalDataChanged (ChannelMask channels) {
	bool sel = (channels & SELECT_CHANNEL) ? TRUE : FALSE;
	bool topo = (channels & TOPO_CHANNEL) ? TRUE : FALSE;
	bool geom = (channels & GEOM_CHANNEL) ? TRUE : FALSE;
	bool vertCol = (channels & VERTCOLOR_CHANNEL) ? true : false;
	bool texmap = (channels & TEXMAP_CHANNEL) ? true : false;
	InvalidateTempData (channels);
	if (geom||topo||sel||vertCol) InvalidateSurfaceUI ();
	if (topo) {
		...
		// Make sure to ready the edge colors, if necessary
	...
	if (geom||topo) {
		...
		if (pNorm)
		{
			...
			if (topo)
            {
                pNorm->ClearFlag (MNNORMAL_NORMALS_BUILT);
            }
	...
	if (sel) {
		...
		if (pblock) pblock->GetValue (ep_surf_subdivide, TimeValue(0), useSubdiv);
...
// If we're coloring any edges specially, do so here
void EditPolyObject::UpdateEdgeColorDisplay() {
	// Default to no display
	bool display = false;
	mm.dispFlags &= ~MNDISP_USE_EDGE_COLORS;
	// Check for hard edge display
	bool colorHardEdges = pblock->GetInt(ep_hard_edge_display) ? true : false;
	if(colorHardEdges) {
		Color hardColor = pblock->GetColor(ep_hard_edge_display_color);
		// Make it into a Color24 and clamp it so it can't reach [0,0,0], which is the semaphore for "smooth"
		Color24 theColor((uchar)(hardColor.r * 255.0f), (uchar)(hardColor.g * 255.0f), (uchar)(hardColor.b * 255.0f));
		theColor.r = std::clamp<uint8_t>(theColor.r, 1, 255);
		theColor.g = std::clamp<uint8_t>(theColor.g, 1, 255);
		theColor.b = std::clamp<uint8_t>(theColor.b, 1, 255);
		Color24 black(0, 0, 0);
		// Set up edge color channel and fill it with the "hard" color for hard edges
		// and black for smoothed edges
		mm.setEDataSupport(EDATA_COLOR);
		Color24 *colors = (Color24 *)mm.edgeData(EDATA_COLOR);
		if(colors) {
			for(int i = 0; i < mm.nume; ++i) {
				// Default the edge to smooth (black)
				colors[i] = black;
				MNEdge &theEdge = mm.e[i];
				if(theEdge.GetFlag(MN_DEAD)) continue;
				// Single-face edges aren't hard
				if(theEdge.f2 < 0) continue;
				MNFace &face1 = mm.f[theEdge.f1];
				MNFace &face2 = mm.f[theEdge.f2];
			...
			DbgAssert(0);	// This shouldn't happen
		}
	}
	if(display) {
		...
		// Turn off the edge color display and flush the data in the MNMesh
		mm.dispFlags &= ~MNDISP_USE_EDGE_COLORS;
		mm.setEDataSupport(EDATA_COLOR, FALSE);
	}
}
...
void EditPolyObject::RefreshScreen () {
	...
	if (ip) ip->RedrawViews (ip->GetTime (), REDRAW_NORMAL | REDRAW_RESET_PROGRESS);
}
...
void EditPolyObject::SetSubSelState(DWORD s)
{
    static constexpr DWORD editPolySelLevel[MNM_SL_FACE + 1] = {EP_SL_OBJECT, EP_SL_VERTEX, EP_SL_EDGE, EP_SL_FACE};

    // Set our selection level member based upon the specified MNMesh selection level
    if ((s >= 0) && (s <= MNM_SL_FACE))
    {
        selLevel = editPolySelLevel[s];
    }

    // Call the base class variant to set the selection level on our mesh
    PolyObject::SetSubSelState(s);
}

ChannelMask EditPolyObject::PartsChangedByOp (int operation)
{
	switch (operation)
	{
	case epop_ns_copy:
	...
	case epop_select_hard_edges:
	case epop_select_smooth_edges:
	...
	case epop_remove:
		return GEOM_CHANNEL | TOPO_CHANNEL | TEXMAP_CHANNEL | VERTCOLOR_CHANNEL | SELECT_CHANNEL;

	case epop_autosmooth:
	case epop_clear_smg:
	case epop_make_hard_edges:
	case epop_make_smooth_edges:
		return TOPO_CHANNEL;

	case epop_remove_iso_map_verts:
		return TEXMAP_CHANNEL | VERTCOLOR_CHANNEL;
	}
...
void EditPolyObject::ApplyMeshOp (MNMesh & mesh, MNTempData *temp, int operation) {
	int msl = editablePolyMeshSelLevel[selLevel];
	switch (operation) {
	case epop_hide:
...
void EditPolyObject::EpActionButtonOp (int opcode) {
	...
	case epop_weld_sel:
		...
		ret = false;
		theHold.Begin ();
		switch (editablePolyMeshSelLevel[selLevel]) {
		case MNM_SL_VERTEX:
	...
	case epop_create_shape:
		if (editablePolyMeshSelLevel[selLevel] != MNM_SL_EDGE) break;
		...
		if (i>= mm.nume) {
			...
			MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), buf2, buf1, MB_ICONEXCLAMATION | MB_OK);
			break;
		}
	...
	case epop_selby_vc:
		...
		int selByIllum;
		pblock->GetValue (ep_vert_color_selby, t, selByIllum);
		EpfnSelectVertByColor (add, sub, selByIllum ? -1 : 0, t);
		break;

	case epop_retriangulate:
		if (editablePolyMeshSelLevel[selLevel] != MNM_SL_FACE) break;
		theHold.Begin ();
	...
	case epop_make_hard_edges:
		theHold.Begin ();
		EpfnMakeHardEdges ();
		theHold.Accept (GetString (IDS_HARD_EDGES));
		LocalDataChanged ();
		RefreshScreen ();
		break;
	...
	case epop_make_smooth_edges:
		theHold.Begin ();
		EpfnMakeSmoothEdges ();
		theHold.Accept (GetString (IDS_SMOOTH_EDGES));
		LocalDataChanged ();
		RefreshScreen ();
		break;
	...
	case epop_select_hard_edges:
		theHold.Begin ();
		EpfnSelectHardEdges ();
		theHold.Accept (GetString (IDS_SELECT_HARD_EDGES));
		LocalDataChanged ();
		RefreshScreen ();
		break;
	...
	case epop_select_smooth_edges:
		...
		theHold.Accept (GetString (IDS_SELECT_SMOOTH_EDGES));
		LocalDataChanged ();
		RefreshScreen ();
		break;
	}
...
void EditPolyObject::MoveSelection(int msl, TimeValue t, Matrix3& partm,
	...
	if (msl == MNM_SL_CURRENT) msl = editablePolyMeshSelLevel[selLevel];
...
void EditPolyObject::Move (TimeValue t, Matrix3& partm, Matrix3& tmAxis, 
	...
	Transform(editablePolyMeshSelLevel[selLevel], t, partm, tmAxis, localOrigin, TransMatrix(val), 0);
	macroRecorder->FunctionCall(_T("move"), 2, 0, mr_meshsel, editablePolyMeshSelLevel[selLevel], this, mr_point3, &val);  // JBW : macrorecorder
}
...
void EditPolyObject::Rotate (TimeValue t, Matrix3& partm, Matrix3& tmAxis, 
	...
	Matrix3 mat;
	val.MakeMatrix(mat);
	Transform(editablePolyMeshSelLevel[selLevel], t, partm, tmAxis, localOrigin, mat, 1);
	// SCA: disabled following because there is no rotate command on subobject levels.
	//macroRecorder->FunctionCall(_T("rotate"), 2, 0, mr_meshsel, editablePolyMeshSelLevel[selLevel], this, mr_quat, &val);  // JBW : macrorecorder
}
...
void EditPolyObject::Scale (TimeValue t, Matrix3& partm, Matrix3& tmAxis, 
		Point3& val, BOOL localOrigin) {
	Transform (editablePolyMeshSelLevel[selLevel], t, partm, tmAxis, localOrigin, ScaleMatrix(val), 2);	
	// SCA: disabled following because there is no scale command on subobject levels.
	//macroRecorder->FunctionCall(_T("scale"), 2, 0, mr_meshsel, editablePolyMeshSelLevel[selLevel], this, mr_point3, &val);  // JBW : macrorecorder
}
		...
		for (i=0; i<nv; i++) {
			...
			if (numAxis == NUMAXIS_INDIVIDUAL) {
				...
				if(refCoordSystem == COORDS_LOCAL_ALIGNED) {
					Point3 norm = axis2obj.VectorTransform(Point3(0,0,1));
					Point3 tangent;
					if(norm.x == 0.0f && norm.y == 0.0f)
						tangent = Point3(1,0,0);
					else
						tangent = Point3(0,0,1) ^ norm;
					axis2obj = MakeZXMatrix(norm, tangent);
				}
...
bool EditPolyObject::EpfnHide (int msl, DWORD flag) {
	...
	if (ret) {
		...
		macroRecorder->EmitScript ();
		LocalDataChanged (DISP_ATTRIB_CHANNEL|SELECT_CHANNEL|GEOM_CHANNEL|TOPO_CHANNEL);	// sca GEOM added because of bounding box change.
	} else {
...
bool EditPolyObject::EpfnUnhideAll (int msl) {
	...
	if (ret) {
		...
		macroRecorder->EmitScript ();
		LocalDataChanged (DISP_ATTRIB_CHANNEL|GEOM_CHANNEL|TOPO_CHANNEL);	// sca GEOM added because of bbox change.
	} else {
		if (faceFlags) delete faceFlags;
		if (vertFlags) delete vertFlags;
	}
...
void EditPolyObject::BeginPointSelectionHitTesting()
{
    m_doPointSelectionHitTest = true;
}

void EditPolyObject::EndPointSelectionHitTesting()
{
...
void EditPolyObject::CloneSelSubComponents(TimeValue t) {
	...
	theHold.Begin();
	theHold.Put (new ComponentFlagRestore (this, editablePolyMeshSelLevel[selLevel]));
	...
	LocalDataChanged (TOPO_CHANNEL|GEOM_CHANNEL|SELECT_CHANNEL);
	RefreshScreen ();
}
...
void EditPolyObject::AcceptCloneSelSubComponents(TimeValue t) {
	...
	INodeTab nodes;
	ip->GetModContexts(mcList,nodes);
	EpfnDetachToObject (name, editablePolyMeshSelLevel[selLevel], MN_SEL, false, nodes[0], t);
	nodes.DisposeTemporary ();
	RefreshScreen ();
}

void EditPolyObject::ExtrudeSelSubComponents(TimeValue timeCurr, bool doPostprocessFaceExtrude)
{

    // Proceed only if the current selection type makes sense, and we are presently editing paramters
    int selection = editablePolyMeshSelLevel[selLevel];
    if ((selection != MNM_SL_OBJECT) && (ip != nullptr))
    {
        DbgAssert(pointToPointMode != nullptr);

        // We have a valid selection; if we are not in point-to-point mode, perform the extrusion operation
        bool isInPointToPointMode = false;
        if (pointToPointMode != nullptr)
        {
            isInPointToPointMode = pointToPointMode->mInMode;
        }

        if (isInPointToPointMode)
        {
            theHold.SuperCancel();
        }
        else
        {
            DbgAssert(m_extrudeRestore == nullptr);
            ...
            // Perform the extrude operation according to selection type; note that with an edge selection, we extrude only
            // boundary edges
            bool isExtruded = false;
            TopoChangeRestore* topoChange = new TopoChangeRestore(this);
            topoChange->Before();
            switch (selection)
            {
            case MNM_SL_VERTEX:
                {
                    Tab<Point3> dummyDirections;
                    MNChamferData dummyData;

					MNMeshTriangulationType triangulationType = GetTriangulationType();
                    isExtruded = mm.ExtrudeVertices(MN_SEL, &dummyData, dummyDirections, triangulationType);
                }
                break;
            case MNM_SL_EDGE:
                if (!mSuspendCloneEdges)
                {
                    isExtruded = EpfnExtrudeOpenEdges(MN_SEL);
                }
            ...
            case MNM_SL_FACE:
                {
                    ...
                    // If we want to perform postprocessing on our extrude operation, manage it through the IMNFaceExtrude
                    // interface, otherwise, simply perform the topology change now
                    Interval interval = FOREVER;
                    BOOL isEnhancedExtrudeActive = FALSE;
                    pblock->GetValue(ep_enable_enhanced_hotkey_extrude, timeCurr, isEnhancedExtrudeActive, interval);
                    if (doPostprocessFaceExtrude && isEnhancedExtrudeActive)
                    {

                        // Because a postprocessed extrude operation can renumber and/or delete mesh geometric entities, the
                        // change should be made to the object over the whole timeline, not at a particular keyframe; if we are
                        // keying, suspend now
                        SuspendAnimate();
                        SuspendSetKeyMode();

                        // Perform initial topology changes for the extrusion
                        IMNFaceExtrude* extrudeInterface = mm.GetTypedInterface<IMNFaceExtrude>();
                        DbgAssert(extrudeInterface != nullptr);
                        extrudeInterface->SetVersion(IMNFaceExtrude::IMN_FACE_EXTRUDE_VER_2024);
                        extrudeInterface->BeginFaceExtrude(MN_SEL);
                        m_nVerticesExtrudeInit = mm.numv;
                        isExtruded = true;

                        // As we are performing a face extrude with postprocessing, we may have further topological changes;
                        // initialize our undo object
                        m_extrudeRestore = new TopoChangeRestore(this);
                        m_extrudeRestore->Before();
                    }
                    else
                    {
                        MNFaceClusters clusters(mm, MN_SEL);
                        isExtruded = mm.ExtrudeFaceClusters(clusters);
                    }
                }
                break;
            default:
            ...
            // If the extrude operation was successful, record the necessary information in the undo system
            if (isExtruded)
            {
                topoChange->After();
                theHold.Begin();
                theHold.Put(topoChange);
                theHold.Accept(GetString(IDS_EXTRUDE));
            }
            else
            {
            ...
            // Flag our potentially changed channels, and refresh the screen
            LocalDataChanged(TOPO_CHANNEL | GEOM_CHANNEL);
            RefreshScreen();
        }
    }
}

void EditPolyObject::UpdateExtrudeSelSubComponentsPostprocessed(TimeValue timeCurr)
{

    // Proceed only if the enhanced face extrusion treatment is active, if we have a face selection, whence we may require
    // postprocessing, and if we are presently editing parameters
    Interval interval = FOREVER;
    BOOL isEnhancedExtrudeActive = FALSE;
    pblock->GetValue(ep_enable_enhanced_hotkey_extrude, timeCurr, isEnhancedExtrudeActive, interval);
    int selection = editablePolyMeshSelLevel[selLevel];
    if (isEnhancedExtrudeActive && (selection == MNM_SL_FACE) && (ip != nullptr))
    {

        // Update the postprocessing for the present geometry through the IMNFaceExtrudeInterface
        IMNFaceExtrude* extrudeInterface = mm.GetTypedInterface<IMNFaceExtrude>();
        DbgAssert(extrudeInterface != nullptr);
        extrudeInterface->UpdateFaceExtrude();
...
void EditPolyObject::FinalizeExtrudeSelSubComponentsPostprocessed(TimeValue timeCurr)
{
    ...
    if (isEnhancedExtrudeActive && (selection == MNM_SL_FACE) && (ip != nullptr))
    {
        ...
        // Finalize the postprocessing for the present geometry through the IMNFaceExtrude interface
        IMNFaceExtrude* extrudeInterface = mm.GetTypedInterface<IMNFaceExtrude>();
        DbgAssert(extrudeInterface != nullptr);
        extrudeInterface->EndFaceExtrude();

        // Set the "after" state for our undo object, which encompasses the entirety of the extrude operation after the initial
        // topology change
        DbgAssert(m_extrudeRestore != nullptr);
        if (m_extrudeRestore != nullptr)
        {
            m_extrudeRestore->After();
            theHold.Begin();
            theHold.Put(m_extrudeRestore);
            theHold.Accept(GetString(IDS_EXTRUDE));
            m_extrudeRestore = nullptr;
        }
        ...
        // If we were keying before we began the postprocessed extrude operation, resume now
        ResumeAnimate();
        ResumeSetKeyMode();
    }
}
...
int EditPolyObject::EpfnCreateVertex(Point3 pt, bool pt_local, bool select) {
	...
	if (select) {
		...
		LocalDataChanged (GEOM_CHANNEL|SELECT_CHANNEL);
	}
	else 
...
int EditPolyObject::EpfnCreateEdge (int v1, int v2, bool select) {
	...
	MNMeshTriangulationType triangulationType = GetTriangulationType();
	mm.SeparateFace (ff, v1pos, v2pos, nf, ne, TRUE, select, false, triangulationType);
	...
	if (select) {
		...
		LocalDataChanged(TOPO_CHANNEL|SELECT_CHANNEL);
	} else {
...
int EditPolyObject::EpfnCreateFace(int *v, int deg, bool select) {
	// Validate that the vertices they want to use are OK (in range and alive)
	for (int i = 0; i < deg; ++i)
	{
		if (v[i] < 0 || v[i] >= mm.numv)
			return -1;	// Out of range!
		MNVert& theVertex = mm.v[v[i]];
		if (theVertex.GetFlag(MN_DEAD))
			return -1;
	}
	...
	if (macroRecorder->Enabled()) {
		...
		TCHAR vertexList[500];
		TCHAR vertexID[40];
		int stringPos=0;
		vertexList[stringPos++] = _T('(');
		for (int i=0; i<deg; i++) {
			_stprintf (vertexID, _T("%d"), v[i]+1);
			memcpy (vertexList + stringPos, vertexID, _tcslen(vertexID) * sizeof(TCHAR));
			stringPos += static_cast<int>(_tcslen(vertexID));	// SR DCAST64: Downcast to 2G limit.
			if (i+1<deg) vertexList[stringPos++] = _T(',');
		}
		vertexList[stringPos++] = _T(')');
		vertexList[stringPos++] = _T('\0');
...
bool EditPolyObject::EpfnCapHoles (int msl, DWORD targetFlags) {
	...
	const int oldFNum = mm.FNum();

	MNMeshTriangulationType triangulationType = GetTriangulationType();
	mm.FillInBorders (&brdr, triangulationType);

	// Assign the new faces to a new smoothing group
	DWORD newsm = mm.GetNewSmGroup();
	for (i = oldFNum; i < mm.FNum(); i++) mm.F(i)->smGroup = newsm;
...
void EditPolyObject::CollapseDeadVerts () {
	...
	// Have to delete vertices in the MNMesh, in the Point Controller Container, and in the actual controller array.
...
bool EditPolyObject::DeleteVerts (DWORD flag) {
	...
	LocalDataChanged (GEOM_CHANNEL|TOPO_CHANNEL|SELECT_CHANNEL);
	return true;
}
...
bool EditPolyObject::EpfnDeleteIsoVerts () {
	...
	for (int i=0; i<mm.numv; i++) {
		...
		mm.v[i].SetFlag (MN_DEAD);
		partsChanged = GEOM_CHANNEL|SELECT_CHANNEL;
	}
	...
	if (partsChanged&GEOM_CHANNEL) CollapseDeadVerts ();
...
bool EditPolyObject::EpfnDeleteIsoMapVerts () {
	...
	for (int mapChannel = -NUM_HIDDENMAPS; mapChannel < mm.MNum(); mapChannel++) {
		...
		if (prevNumMapVerts > mm.M(mapChannel)->numv)
			partsChanged |= (mapChannel<1) ? VERTCOLOR_CHANNEL : TEXMAP_CHANNEL;
	}
...
bool EditPolyObject::RemoveVerts(DWORD flag)
{
	...
	bool retVal = mm.RemoveVertices(verts_to_delete.data(), int(verts_to_delete.size()));
...
bool EditPolyObject::RemoveEdges(DWORD flag)
{
	...
	for (i = 0; i < mm.nume; i++)
	{
		...
		if (mm.e[i].f2 < 0)
			continue;
		if (mm.e[i].f1 == mm.e[i].f2)
			continue;
		if (mm.e[i].GetFlag(flag))
			break;
	}
	...
	if (l_removeVert)
	{
		...
		for (int j = 0; j < mm.numv; j++)
		{
			...
			{
				verts_to_delete.push_back(j);
			}
		}
		l_retVal |= mm.RemoveVertices(verts_to_delete.data(), int(verts_to_delete.size()));
	}
...
bool EditPolyObject::DeleteFaces (DWORD flag, bool delIsoVerts) {
   ...
   int i;
	...
	ChannelMask invalid = TOPO_CHANNEL|SELECT_CHANNEL;
...
void EditPolyObject::EpfnAttach (INode *node, INode *myNode, TimeValue t) {
	...
	if (clearMapChannel)
		mm.SetFlag(MN_CLEARMAPCHANNEL);
	else
		mm.ClearFlag(MN_CLEARMAPCHANNEL);
	...
	int ofnum = mm.numf;
    int nNormalsInit = 0;
    const MNNormalSpec* normalsInit = mm.GetSpecifiedNormals();
    if (normalsInit != nullptr)
    {
        nNormalsInit = normalsInit->GetNumNormals();
    }
    ...
    // If any specified normals were copied from the attached mesh's specified normals container, transform the copied normals
    // to our object space
    MNNormalSpec* normals = mm.GetSpecifiedNormals();
    if (normals != nullptr)
    {
        BitArray doTransformNormal;

        // Build a selection array indicating the normals to be transformed
        int nNormals = normals->GetNumNormals();
        DbgAssert(nNormals >= nNormalsInit);
        doTransformNormal.SetSize(nNormals);
        doTransformNormal.ClearAll();
        for (int norm = nNormalsInit; norm != nNormals; ++norm)
        {
            doTransformNormal.Set(norm);
        }

        // Perform transformation
        normals->Transform(relativeTransform, true, &doTransformNormal);
    }
	...
	if (pobj != NULL)
	{
		...
		if ( pobj->pointCtrlContainer && ( pobj->pointCtrlContainer->GetNumSubControllers() == pobj->mm.numv ) )
		{
			CreateContArray();
			SynchContArray( mm.numv );
			if ( pointCtrlContainer && ( pointCtrlContainer->GetNumSubControllers() == mm.numv ) )
			{
				for ( int i=ovnum; i<mm.numv; ++i )
				{
					Control* c = pobj->pointCtrlContainer->GetSubController( i-ovnum );
					if ( c == NULL )
						continue;
	...
	LocalDataChanged (ALL_CHANNELS - SUBSEL_TYPE_CHANNEL);
	DbgAssert ( mm.CheckAllData() );
...
bool EditPolyObject::EpfnBreakVerts (DWORD flag) {
	...
	pblock->GetValue (ep_vertex_break, TimeValue(0), l_distance);

	IMNMeshUtilities8* l_meshToBreak = mm.GetTypedInterface<IMNMeshUtilities8>();
...
int EditPolyObject::EpfnDivideFace (int face, Tab<float> &bary, bool select) {
	...
	if (select && nv >= 0) mm.v[nv].SetFlag (MN_SEL);
	if (tchange) {
		tchange->After ();
		theHold.Put (tchange);
	}
	...
	if (macroRecorder->Enabled()) {
		// since we don't have macrorecorder support for arrays,
		// we need to throw together a string and pass it as an mr_name.
		TCHAR baryCoordString[500];
		TCHAR baryValue[40];
		int stringPos=0;
		baryCoordString[stringPos++] = _T('(');
		for (int i=0; i<bary.Count(); i++) {
			_stprintf (baryValue, _T("%f"), bary[i]);
			memcpy (baryCoordString + stringPos, baryValue, _tcslen(baryValue) * sizeof(TCHAR));
			stringPos += static_cast<int>(_tcslen(baryValue));	// SR DCAST64: Downcast to 2G limit.
			if (i+1<bary.Count()) baryCoordString[stringPos++] = _T(',');
		}
		baryCoordString[stringPos++] = _T(')');
		baryCoordString[stringPos++] = _T('\0');
	...
	if (select) {
		...
		LocalDataChanged (ALL_CHANNELS - SUBSEL_TYPE_CHANNEL - SELECT_CHANNEL);
	}
...
bool EditPolyObject::EpfnExtrudeMesh (MNMesh & mesh, MNTempData *temp, int msl, DWORD flag) {
    enum EdgeVertexType {FIRST_VERTEX = 0, SECOND_VERTEX = 1, N_VERTICES_EDGE = 2};
    ...
    case MNM_SL_EDGE:
        {
            ...
            // Need to fix bad verts before extruding so that the call to OrderVerts after the extrude will not create new vertices,
            // which would cause a mismatch between the number of vertices stored in the pChamData and in the mesh.
            if (!mesh.GetFlag(MN_MESH_NO_BAD_VERTS))
                 mesh.EliminateBadVerts();

            // Do the topology change:
            int nVerticesOriginal = mesh.numv;
            if (msl == MNM_SL_EDGE)
            {
                IntTab flaggedVertices;

                // In the (very common) case of a zero-width extrude, we want to avoid unnecessary duplication of flagged
                // vertices on unflagged edges, which may occur in MNMesh::ExtrudeEdges when multiple unflagged edges connect
                // to a single flagged vertex; form a minimum width tolerance based upon the average edge length of non-flagged
                // edges connected to vertices with one or more flagged edges
                int nEdgesTot = 0;
                float lengthAvg = 0.0f;
                DbgAssert(mesh.GetFlag(MN_MESH_FILLED_IN));
	            mesh.ClearVFlags(MN_WHATEVER);
	            mesh.PropegateComponentFlags(MNM_SL_VERTEX, MN_WHATEVER, MNM_SL_EDGE, flag);
                for (int v = 0; v != nVerticesOriginal; ++v)
                {
                    bool isDead = (mesh.v[v]).GetFlag(MN_DEAD);
                    if (!isDead)
                    {
                        bool isVertexFlagged = (mesh.v[v]).GetFlag(MN_WHATEVER);
                        if (isVertexFlagged)
                        {
                            flaggedVertices.Append(1, &v, 10);
                            const IntTab& vertexEdges = mesh.vedg[v];
                            int nEdges = vertexEdges.Count();
                            for (int e = 0; e != nEdges; ++e)
                            {
                                const MNEdge& edgeCurr = mesh.e[vertexEdges[e]];
                                bool isEdgeFlagged = edgeCurr.GetFlag(flag);
                                if (!isEdgeFlagged)
                                {
                                    lengthAvg += Length((mesh.v[edgeCurr[FIRST_VERTEX]]).p - (mesh.v[edgeCurr[SECOND_VERTEX]]).p);
                                    ++nEdgesTot;
                                }
                            }
                        }
                ...
                lengthAvg = lengthAvg / (((float)nEdgesTot) + FLT_MIN);
                float widthTol = FLT_EPSILON * lengthAvg;

                // Generate new vertices and inset/extrude directions
                if (!(mesh.ExtrudeEdges(flag, pChamData, tUpDir, triangulationType))) return false;
                pblock->GetValue(ep_edge_extrude_height, TimeValue(0), height);
                pblock->GetValue(ep_edge_extrude_width, TimeValue(0), width);

                // If the specified width is zero to tolerance, check each flagged vertex for cases of multiple unflagged edges
                // between consective flagged edges about the vertex, and remove the resulting duplicate vertices and
                // associated degenerate edges and faces
                if (width < widthTol)
                {
                    IntTab sequenceBeginEdges,nUnflaggedEdgesSequence,distalVertices,degenerateEdges;

                    // The operations in MNMesh::ExtrudeEdges break the ordering of the vedg and vfac tables; reorder them now,
                    // so that we can identify edge sequences bounded by consecutive flagged edges
                    mesh.OrderVerts();

                    // Check flagged vertices for redundant duplicates, and remediate
                    int nVerticesFlagged = flaggedVertices.Count();
                    for (int v = 0; v != nVerticesFlagged; ++v)
                    {
                        DbgAssert((!((mesh.v[flaggedVertices[v]]).GetFlag(MN_DEAD))) && ((mesh.v[flaggedVertices[v]]).GetFlag(MN_WHATEVER)));
                        const IntTab& vertexEdges = mesh.vedg[flaggedVertices[v]];
                        const IntTab& vertexFaces = mesh.vfac[flaggedVertices[v]];
                        int nEdges = vertexEdges.Count();
                        int nFaces = vertexFaces.Count();
                        DbgAssert((nEdges == nFaces) || (nEdges == (nFaces + 1)));

                        // Build list of flagged edges about this vertex
                        sequenceBeginEdges.ZeroCount();
                        for (int e = 0; e != nEdges; ++e)
                        {
                            bool isFlagged = (mesh.e[vertexEdges[e]]).GetFlag(flag);
                            if (isFlagged)
                            {
                                sequenceBeginEdges.Append(1, &e, 10);
                            }
                        }

                        // Count the number of unflagged edges in each sequence, bounded by consecutive flagged edges
                        int nSequences = sequenceBeginEdges.Count();
                        DbgAssert(nSequences > 0);
                        nUnflaggedEdgesSequence.SetCount(nSequences);
                        for (int seq = 0; seq != nSequences; ++seq)
                        {
                            int indexEndEdge = (seq + 1) % nSequences;
                            nUnflaggedEdgesSequence[seq] = sequenceBeginEdges[indexEndEdge] - sequenceBeginEdges[seq] - 1;
                            if (nUnflaggedEdgesSequence[seq] < 0)
                            {
                                nUnflaggedEdgesSequence[seq] += nEdges;
                            }
                        }

                        // If the present vertex lies on a boundary, we must not "wrap around" the vedg/vfac lists; instead,
                        // adjust to obtain sequences of unflagged edges between each boundary and the closest flagged edge,
                        // noting that unflagged edges in sequence seq are defined by vertexEdges[indexEdge], where
                        // indexEdge = (sequenceBeginEdges[seq] + e + 1) % nEdges, where e is on the range
                        // [0, nUnflaggedEdgesSequence[seq]) if non-empty
                        if (nEdges != nFaces)
                        {
                            int indexLastSequence = nSequences;
                            ++nSequences;
                            sequenceBeginEdges.SetCount(nSequences);
                            nUnflaggedEdgesSequence.SetCount(nSequences);
                            sequenceBeginEdges[indexLastSequence] = nEdges - 1;
                            nUnflaggedEdgesSequence[indexLastSequence] = sequenceBeginEdges[0];
                            nUnflaggedEdgesSequence[indexLastSequence - 1] = nEdges - 1 - sequenceBeginEdges[indexLastSequence - 1];
                        }

                        // If any sequence has more than one unflagged edge, it will have duplicate, coincident vertices; flag
                        // extraneous vertices, and related degenerate faces and degenerate/duplicate edges, as MN_DEAD, and
                        // clean up face vertex and edge lists, edge vertex and face lists, and vertex vedg and vfac tables
                        for (int seq = 0; seq != nSequences; ++seq)
                        {
                            if (nUnflaggedEdgesSequence[seq] > 1)
                            {

                                // Sequence with multiple unflagged edges; determine indices of degenerate edges connecting
                                // duplicate vertices
                                distalVertices.SetCount(nUnflaggedEdgesSequence[seq]);
                                for (int e = 0; e != nUnflaggedEdgesSequence[seq]; ++e)
                                {
                                    const MNEdge& edgeCurr = mesh.e[vertexEdges[(sequenceBeginEdges[seq] + e + 1) % nEdges]];
                                    distalVertices[e] = edgeCurr[FIRST_VERTEX];
                                    if (distalVertices[e] == flaggedVertices[v])
                                    {
                                        distalVertices[e] = edgeCurr[SECOND_VERTEX];
                                        DbgAssert(distalVertices[e] != flaggedVertices[v]);
                                    }
                                }

                                int nDegenerateEdges = nUnflaggedEdgesSequence[seq] - 1;
                                degenerateEdges.SetCount(nDegenerateEdges);
                                for (int e = 0; e != nDegenerateEdges; ++e)
                                {
                                    int indexEdgeAtDistalVertex = 0;
                                    degenerateEdges[e] = invalidIndex;
                                    int indexDuplicateVertex = distalVertices[e + 1];
                                    const IntTab& distalVertexEdges = mesh.vedg[distalVertices[e]];
                                    int nEdgesDistalVertex = distalVertexEdges.Count();
                                    while ((degenerateEdges[e] == invalidIndex) && (indexEdgeAtDistalVertex != nEdgesDistalVertex))
                                    {
                                        int indexVertexOnEdge = 0;
                                        const MNEdge& edgeCurr = mesh.e[distalVertexEdges[indexEdgeAtDistalVertex]];
                                        while ((degenerateEdges[e] == invalidIndex) && (indexVertexOnEdge != N_VERTICES_EDGE))
                                        {
                                            if (edgeCurr[indexVertexOnEdge] == indexDuplicateVertex)
                                            {
                                                degenerateEdges[e] = distalVertexEdges[indexEdgeAtDistalVertex];
                                            }

                                            ++indexVertexOnEdge;
                                        }

                                        ++indexEdgeAtDistalVertex;
                                    }

                                    DbgAssert(degenerateEdges[e] != invalidIndex);
                                }

                                // Weld the vertices on the ends of each degenerate edge; this will fix up the topology in the
                                // manner described above
                                bool areAllEdgesWelded = true;
                                for (int e = 0; e != nDegenerateEdges; ++e)
                                {
                                    bool isWelded = mesh.WeldEdge(degenerateEdges[e]);
                                    if (!isWelded)
                                    {
                                        areAllEdgesWelded = false;
                                    }
                                }

                                DbgAssert(areAllEdgesWelded);
                            }
                        }
                    }
                }
            ...
            {
                if (!mesh.ExtrudeVertices (flag, pChamData, tUpDir, triangulationType)) return false;
                pblock->GetValue (ep_vertex_extrude_height, TimeValue(0), height);
                pblock->GetValue (ep_vertex_extrude_width, TimeValue(0), width);
            }

            // Apply map changes based on base width:
            for (mapChannel=-NUM_HIDDENMAPS; mapChannel<mesh.MNum(); mapChannel++)
            {
            ...
            for (i = 0; i < mesh.numv; ++i)
            {
                bool isDead = (mesh.v[i]).GetFlag(MN_DEAD);
                if (!isDead)
                {
                    (mesh.v[i]).p += tDelta[i];
                }
            ...
            // Move the points up:
            DbgAssert((tUpDir.Count()) == nVerticesOriginal);
            for (i = 0; i < tUpDir.Count(); ++i)
            {
                (mesh.v[i]).p += (tUpDir[i]) * height;
            }
	...
	case MNM_SL_FACE:
		...
		pblock->GetValue (ep_face_extrude_height, TimeValue(0), height);
...
bool EditPolyObject::EpfnBevelMesh (MNMesh & mesh, MNTempData *temp, int msl, DWORD flag) {
	...
	pblock->GetValue (ep_bevel_height, TimeValue(0), extHeight);
	pblock->GetValue (ep_bevel_outline, TimeValue(0), outline);
	...
	if (outline) {
		int extType;
		pblock->GetValue (ep_bevel_type, TimeValue(0), extType);
		Tab<Point3> *outDir;
...
bool EditPolyObject::EpfnInsetMesh (MNMesh & mesh, MNTempData *temp, int msl, DWORD flag) {
	...
	float inset;
	pblock->GetValue (ep_inset, TimeValue(0), inset);
	...
	if (inset) {
		...
		if (mapInvert) mapInverter.Initialize (mesh, mPreserveMapSettings);
		...
		if (mapInvert) mapInverter.Apply (mesh, pDelta, outDir->Count());
	}
...
bool EditPolyObject::EpfnOutline (DWORD flag) {
	float outline;
	pblock->GetValue (ep_outline, TimeValue(0), outline);
	...
	if (outline) {
		...
		if (ret && mapInvert) mapInverter.Apply (mm, pDelta, outDir->Count());
	}

	if (ret) {
		ApplyDelta (tDelta, this, GetCOREInterface()->GetTime());
...
bool EditPolyObject::EpfnChamferMesh (MNMesh & mesh, MNTempData *temp, int msl, DWORD flag) {
	...
	// Merge across any hidden edges and elminate collinear vertices on the input mesh prior to processing, consistent with the
	// approach taken by the Chamfer modifier
	mesh.MakePolyMesh();

	if (!DoChamferMesh (mesh, temp, msl, flag)) return false;

	// Check the chamfer type.  If quad chamfering, we can leave now.
    bool isQuadChamfer = false;
	if (msl == MNM_SL_EDGE)
    {
	    int miteringType = EPOLY_EDGE_CHAMFER_MITER_TRI;
		pblock->GetValue(ep_edge_chamfer_mitering_type, TimeValue(0), miteringType);
        if (miteringType != EPOLY_EDGE_CHAMFER_MITER_TRI)
        {
            isQuadChamfer = true;
        }
    }
    ...
    {
        DbgAssert(msl == MNM_SL_VERTEX);
        int nSegments = 0;
        Interval interval = FOREVER;
        pblock->GetValue(ep_vertex_chamfer_num_segments, 0, nSegments, interval);
        if (nSegments > 0)
        {
            isQuadChamfer = true;
        }
	...
	IMNTempData10* l_tempData10 = temp->GetTypedInterface<IMNTempData10>();
	...
	float chamfer = 0.0f;
	int		l_smooth = 0;
	int		l_smooth_type = EPOLY_EDGE_CHAMFER_SMOOTH_ALL;
	float	l_smooth_threshold = 30.0f;
	switch (msl) {
	case MNM_SL_VERTEX:
		pblock->GetValue (ep_vertex_chamfer, TimeValue(0), chamfer);
		break;
	case MNM_SL_EDGE:
		pblock->GetValue (ep_edge_chamfer, TimeValue(0), chamfer);
		pblock->GetValue (ep_edge_chamfer_smooth, TimeValue(0), l_smooth);
		pblock->GetValue (ep_edge_chamfer_smooth_type, TimeValue(0), l_smooth_type);
		pblock->GetValue (ep_edge_chamfer_smooth_threshold, TimeValue(0), l_smooth_threshold);
		break;
	}
	...
	if(l_smooth) {
		switch(l_smooth_type) {
		case EPOLY_EDGE_CHAMFER_SMOOTH_ALL:
			mesh.AutoSmooth(l_smooth_threshold * DEG_TO_RAD, false, false);
			break;
		case EPOLY_EDGE_CHAMFER_SMOOTH_CHAMFER:
			{
			// Smooth the chamfer faces only (flagged with MN_FACE_CREATED) and leave the border unsmoothed		
			MNMeshUtilities mu(&mesh);
			mu.AutoSmoothIsland(l_smooth_threshold * DEG_TO_RAD, MN_FACE_CREATED);
			break;
			}
		}
	}
...
bool EditPolyObject::DoExtrusionMesh (MNMesh & mesh, MNTempData *temp, int msl, DWORD flag, bool isBevel) {
	...
	if (isBevel) {
		pblock->GetValue(ep_bevel_type, TimeValue(0), extType);
		pblock->GetValue(ep_bevel_bias, TimeValue(0), bias);
	}
	else {
		pblock->GetValue(ep_extrusion_type, TimeValue(0), extType);
		pblock->GetValue(ep_extrude_bias, TimeValue(0), bias);
	}
	
	if (flag != MN_SEL) {
		// will need to rebuild face clusters.
		temp->Invalidate (TOPO_CHANNEL);
	}
	...
	case 0:	// Group normals
        {
            MNFaceClusters* fclust = temp->FaceClusters(flag);
	...
	case 1:	// Local normals
        {
            ...
            if (ret)
            {
                IMNMeshUtilities16* l_mesh16 = mesh.GetTypedInterface<IMNMeshUtilities16>();
				const bool useLegacyDPoint3Add = mVersion < VERSION_CORRECTED_DPOINT3_ADD;
				l_mesh16->GetExtrudeDirection(temp->ChamferData(), flag, bias, useLegacyDPoint3Add);
            }
        }
		break;
	...
	case 2:	// poly-by-poly extrusion.
        {
            ...
            if (ret)
            {
                ...
                // Group faces into clusters and employ these in forming directions along which to displace vertices,
                // consistent with Edit Poly implementation
		        MNFaceClusters faceClusters(mesh, flag);
                faceClusters.GetNormalsCenters(mesh, clusterNormals, clusterCenters);
                mesh.GetExtrudeDirection(temp->ChamferData(), &faceClusters, &(clusterNormals[0]));
            }
        }
		break;
	}
...
void EditPolyObject::EpfnDragBevel (float amount, float height, TimeValue t) {
	...
	if (amount) {
		...
		if (mm.numv == outDir->Count() && mm.numv == delta.Count())
		{
			Point3 *od = outDir->Addr(0);
		...
		{
			DbgAssert(0 && _T("EpfnDragBevel count mismatch"));
		}
	}
...
{
    constexpr float angleFactorVertex = 0.5f;

	MNChamferData *mcd = NULL;
	if(temp)
		mcd = temp->ChamferData();
	
	bool	ret		= false;
	DWORD	l_flag	= MN_SEL;
	float	l_amount = 0.0f;
	float	l_tension = 0.0f;
	int		l_open	= 0; 
	int		l_invert	= 0; 
	int		l_segments = 1;
	bool	l_bOpen = false; 
	bool	l_bInvert = false;
	int		l_smooth = 0;
	int		l_smooth_type = EPOLY_EDGE_CHAMFER_SMOOTH_ALL;
	float	l_smooth_threshold = 30.0f;
	int		l_miteringType = EPOLY_EDGE_CHAMFER_MITER_UNIFORM;
	float	l_endBias = 0.5f;
	float	l_radialBias = 0.0f;
	float	l_depth = 0.5f;

    Interval interval = FOREVER;
	switch (msl) {
	case MNM_SL_VERTEX:
        pblock->GetValue(ep_vertex_chamfer, 0, l_amount, interval);
        pblock->GetValue(ep_vertex_chamfer_open, 0, l_open, interval);
        pblock->GetValue(ep_vertex_chamfer_num_segments, 0, l_segments, interval);
        pblock->GetValue(ep_vertex_chamfer_depth, 0, l_depth, interval);
        if (l_segments > 0)
        {
            pblock->GetValue(ep_vertex_chamfer_smooth, 0, l_smooth, interval);
            pblock->GetValue(ep_vertex_chamfer_smooth_type, 0, l_smooth_type, interval);
            pblock->GetValue(ep_vertex_chamfer_smooth_threshold, 0, l_smooth_threshold, interval);
        }

		break;
	case MNM_SL_EDGE:
		pblock->GetValue (ep_edge_chamfer, TimeValue(0), l_amount, interval);
		pblock->GetValue (ep_edge_chamfer_tension, TimeValue(0), l_tension, interval);
		pblock->GetValue (ep_edge_chamfer_open, TimeValue(0), l_open, interval);
		pblock->GetValue (ep_edge_chamfer_invert, TimeValue(0), l_invert, interval);
		pblock->GetValue (ep_edge_chamfer_segments, TimeValue(0), l_segments, interval);
		pblock->GetValue(ep_edge_chamfer_mitering_type, TimeValue(0), l_miteringType, interval);
		pblock->GetValue(ep_edge_chamfer_end_bias, TimeValue(0), l_endBias, interval);
		pblock->GetValue(ep_edge_chamfer_radial_bias, TimeValue(0), l_radialBias, interval);
		pblock->GetValue(ep_edge_chamfer_depth, TimeValue(0), l_depth, interval);
        if (l_miteringType != EPOLY_EDGE_CHAMFER_MITER_TRI)
        {
		    pblock->GetValue(ep_edge_chamfer_smooth, TimeValue(0), l_smooth, interval);
	    	pblock->GetValue(ep_edge_chamfer_smooth_type, TimeValue(0), l_smooth_type, interval);
    		pblock->GetValue(ep_edge_chamfer_smooth_threshold, TimeValue(0), l_smooth_threshold, interval);
        }
	...
	case MNM_SL_VERTEX:
		{
            MN_QCHAM_TYPE geometryOption = QCHAM_CHAMFEREDOBJECT;
            if (l_bOpen)
            {
                geometryOption = QCHAM_NOCHAMFERS;
            }

            // Employ the legacy vertex chamfer in the zero segment case, and the new quad algorithm in any other case
            if (l_segments == 0)
            {
    			IMNMeshUtilities18* l_meshToChamfer = mesh.GetTypedInterface<IMNMeshUtilities18>();
	    		if (l_meshToChamfer != NULL )
		    	{
				    ret = l_meshToChamfer->ChamferVertices (l_flag, mcd, geometryOption);
			    }
            }
            else
            {
                DWORD optionFlags = QCHAM_OPTION_LIMIT_EFFECT;
                MNQuadVertexChamferParameters params(l_flag, optionFlags, geometryOption, l_segments, l_amount, l_depth, 1.0f, angleFactorVertex);
                IMNMeshUtilities18* chamferInterface = mesh.GetTypedInterface<IMNMeshUtilities18>();
                DbgAssert(chamferInterface != NULL);
                if (chamferInterface != NULL)
                {
                    ret = chamferInterface->QuadVertexChamfer(params);
                }
            }
		}
		break;
	case MNM_SL_EDGE:
		{
			switch(l_miteringType) 
			{
			case EPOLY_EDGE_CHAMFER_MITER_TRI: 
				{
					IMNTempData10* l_tempData10 = temp->GetTypedInterface<IMNTempData10>();
					DbgAssert(l_tempData10);
			
					IMNMeshUtilities18* l_meshToChamfer = mesh.GetTypedInterface<IMNMeshUtilities18>();
					if (l_meshToChamfer != NULL )
					{
						MN_QCHAM_TYPE geometryOption = l_bOpen ? (l_bInvert ? QCHAM_CHAMFERSONLY : QCHAM_NOCHAMFERS) : QCHAM_CHAMFEREDOBJECT;
						ret = l_meshToChamfer->ChamferEdges(l_flag, l_tempData10->ChamferData(), geometryOption, l_segments);
					}
				} 
				break;
			case EPOLY_EDGE_CHAMFER_MITER_QUAD: 
			case EPOLY_EDGE_CHAMFER_MITER_UNIFORM:
			case EPOLY_EDGE_CHAMFER_MITER_RADIAL:
			case EPOLY_EDGE_CHAMFER_MITER_PATCH:
				{
					MN_QCHAM_TYPE geomOptions = l_bOpen ? ( l_bInvert ? QCHAM_CHAMFERSONLY : QCHAM_NOCHAMFERS) : QCHAM_CHAMFEREDOBJECT;
					bool isUniformBaseType = l_miteringType == EPOLY_EDGE_CHAMFER_MITER_UNIFORM || l_miteringType == EPOLY_EDGE_CHAMFER_MITER_PATCH || l_miteringType == EPOLY_EDGE_CHAMFER_MITER_RADIAL;
					DWORD optionsFlags =
							QCHAM_OPTION_LIMIT_EFFECT |
							(isUniformBaseType ? QCHAM_OPTION_QUAD_INTERSECTIONS : 0) |
							(isUniformBaseType ? QCHAM_OPTION_FIXED_EDGE_WEIGHTS : 0) |
							(l_miteringType == EPOLY_EDGE_CHAMFER_MITER_PATCH ? QCHAM_OPTION_PATCH_CORNERS : 0) |
							(l_miteringType == EPOLY_EDGE_CHAMFER_MITER_RADIAL ? QCHAM_OPTION_RADIAL_CORNERS : 0) |
							QCHAM_OPTION_BIAS_END_POINTS;

					IMNMeshUtilities17* chamferInterface = mesh.GetTypedInterface<IMNMeshUtilities17>();
					DbgAssert(chamferInterface);
					if (chamferInterface)
					{
						// This specifies the version number to be used so old scenes won't break
						MNQuadChamferExtendedParameters params(l_amount, 0.0f, 1.0f, l_segments, l_tension, l_depth, 0.5f, 0, 0.0f, l_endBias, optionsFlags, geomOptions, l_flag, QCHAM_VERSION_OLYMPUS, l_radialBias, 1.0f);
						chamferInterface->QuadChamfer(params);
					}

					// Returning true here instead of whatever result obtained from QuadChamfer will prevent regression crash MAXX-51397 and
					// restore previous behavior, which is nothing happen on one-sided edges for Quad and Uniform chamfers. 
					// This behavior would have to be implemented separately.
					ret = true;
                }
                break;
            }
    ...
    if (l_smooth)
    {
        float angleTol = DegToRad(l_smooth_threshold);
        switch (l_smooth_type)
        {
        case EPOLY_EDGE_CHAMFER_SMOOTH_ALL:
            mesh.AutoSmooth(angleTol, false, false);
            break;
        case EPOLY_EDGE_CHAMFER_SMOOTH_CHAMFER:
            {
                ...
                // Smooth the chamfer faces only (flagged with MN_FACE_CREATED) and leave the border unsmoothed
                MNMeshUtilities mu(&mesh);
                mu.AutoSmoothIsland(angleTol, MN_FACE_CREATED);
            }
            break;
        }
    }
...
// This maintains a copy of the pointer to topo change restore object for quad chamfer restoration during mouse interaction
//
// The existing chamfer mouse proc mechanism uses DragMove/DragMoveRestore to roll back the changes as the mouse moves. It's a simple delta recording
// because the legacy chamfer doesn't do topo changes interactively. Because Quad Chamfer can adjust segments on the fly you can get topo changes that
// would have to be dealt with and undone as the mouse moves. I could make a new container to hold these changes, but all the information necessary to
// roll back the topo changes are actually stored by the TopoChangeRestore object when it is created and added to the hold by the DoChamfer method,
// called in EpfnBeginChamfer at the start of the dragging. Rather than have all that (potentially large amount) of restore information duplicated,
// we grab the pointer to the restore object, which will remain there while we're inside the SuperHold dragging the mouse, storing it in this class.
// When we need to roll back the changes on each "tick" of the mouse, we just call its "Restore" method to do so.

class TopoChangeRestoreHolder {
public:
	TopoChangeRestore *theRestore;
	TopoChangeRestoreHolder() { theRestore = NULL; }
	void Set(TopoChangeRestore *t) { theRestore = t; }
	void Clear() { theRestore = NULL; }
	TopoChangeRestore *Get() { return theRestore; }
};

static TopoChangeRestoreHolder theTopoChangeRestore;
...
bool EditPolyObject::DoChamfer (int msl, DWORD flag) {
	...
	if (ret) {
		...
		LocalDataChanged (ALL_CHANNELS - SUBSEL_TYPE_CHANNEL);
		RefreshScreen ();
		theTopoChangeRestore.Set(tchange);		// Keep a pointer to the topo change restore object while we're dragging
	} else {
		delete tchange;
		theTopoChangeRestore.Clear();			// TH 11/18/13 -- topo change info no longer valid
	}
	theHold.Accept (GetString (IDS_CHAMFER));
	return ret;
}
...
void EditPolyObject::EpfnBeginChamfer (int msl, TimeValue t) {
	...
	// Determine whether this is a quad-based mitering type, or a legacy chamfer
    bool isLegacy = true;
    int selLevel = GetMNSelLevel();
    ...
    {
        DbgAssert(selLevel == MNM_SL_VERTEX);

        // Vertex chamfer; we use the legacy approach for zero segments, and the new segmented quad vertex chamfer
        // implementation in any other case
        int nSegments = 0;
        Interval interval = FOREVER;
        pblock->GetValue(ep_vertex_chamfer_num_segments, 0, nSegments, interval);
        if (nSegments != 0)
	...
	if (!DoChamfer (msl, MN_SEL) || (isLegacy && !TempData()->ChamferData()->vmax.Count())) {
		theHold.SuperCancel ();
		return;
	}
	...
	sel.SetSize (mm.numv);
	sel.ClearAll ();
	if (isLegacy && (selLevel == MNM_SL_EDGE))
    {
    ...
    {
		// TODO -- controllers for quad chamfer
	}
...
void EditPolyObject::EpfnEndChamfer (bool accept, TimeValue t) {
	...
	// Proceed according to selection type
    int selLevel = GetMNSelLevel();
    if (selLevel == MNM_SL_EDGE)
    {
        int miteringType = EPOLY_EDGE_CHAMFER_MITER_TRI;
	...
	// No longer need the topo change restore information; clear it.
	theTopoChangeRestore.Clear();
}

void EditPolyObject::EpfnDragChamfer (float amount, float tensionOrDepth, TimeValue t) {
	if (!inChamfer) return;
	...
	if (selLevel == MNM_SL_EDGE)
	{
		...
		if (miteringType == EPOLY_EDGE_CHAMFER_MITER_TRI)
		{
			...
			Tab<Point3> delta;
			IMNTempData10* l_tempData10 = TempData()->GetTypedInterface<IMNTempData10>();
			...
			if(l_smooth) {
				...
				switch(l_smooth_type) {
				case EPOLY_EDGE_CHAMFER_SMOOTH_ALL:
					mm.AutoSmooth(l_smooth_threshold * DEG_TO_RAD, false, false);
					break;
				case EPOLY_EDGE_CHAMFER_SMOOTH_CHAMFER:
					{
						// Smooth the chamfer faces only (flagged with MN_FACE_CREATED) and leave the border unsmoothed
						MNMeshUtilities mu(&mm);
						mu.AutoSmoothIsland(l_smooth_threshold * DEG_TO_RAD, MN_FACE_CREATED);
						break;
					}
				}
		...
		else
		{
			...
			// Quad edge chamfer (quad, uniform, patch or radial mitering)
			TopoChangeRestore* theTopoChangeInit = theTopoChangeRestore.Get();

			// If the mesh was successfully altered during a previous chamfer operation performed on this drag, restore
			// the mesh to its initial state via the TopoChangeRestore helper object theTopoChangeInit, otherwise, the
			// mesh must presently be in its initial state, whence we instantiate a new TopoChangeRestore object,
			// setting the Before state, and setting in the theTopoChangeRestore container
			TopoChangeRestore* theTopoChange = theTopoChangeInit;
			if (theTopoChange != nullptr)
			{
				theTopoChange->Restore(false);
			}
			else
			{
				theTopoChange = new TopoChangeRestore(this);
				theTopoChange->Before();
				theTopoChangeRestore.Set(theTopoChange);
			}
			...
			bool isAltered = DoChamferMesh(mm, NULL, selLevel);

			// If a new TopoChangeRestore helper object was instantiated above, update as appropriate; if the mesh was
			// actually altered by the chamfer operation above, set the After state, otherwise, the mesh is still in
			// its initial state, whence we should the deallocate the helper object, and rest the theTopoChangeRestore
			// container
			if (theTopoChangeInit == nullptr)
			{
				if (isAltered)
				{
					theTopoChange->After();
	...
	else
	{
		...
		// Vertex chamfer; we must take the same approach for legacy/quad vertex chamfers, because the distinction
		// depends upon number of segments, which may change during interactive chamfer, thus we call EpfnChamferMesh
		// to ensure that the mesh is fully updated in the legacy case
		TopoChangeRestore* theTopoChangeInit = theTopoChangeRestore.Get();

		// If the mesh was successfully altered during a previous chamfer operation performed on this drag, restore the
		// mesh to its initial state via the TopoChangeRestore helper object theTopoChangeInit, otherwise, the mesh
		// must presently be in its initial state, whence we instantiate a new TopoChangeRestore object, setting the
		// Before state, and setting in the theTopoChangeRestore container
		TopoChangeRestore* theTopoChange = theTopoChangeInit;
		if (theTopoChange != nullptr)
		{
			theTopoChange->Restore(false);
		...
		TopoChangeRestore *tchange = new TopoChangeRestore(this);
		tchange->Before();
		MNTempData* tempData = TempData();

		// Perform the chamfer operation
		bool isAltered = EpfnChamferMesh(mm, tempData, selLevel, MN_SEL);
...
bool EditPolyObject::DoInsetMesh (MNMesh & mesh, MNTempData *temp, int msl, DWORD flag) {
	...
	int insetType;
	pblock->GetValue(ep_inset_type, TimeValue(0), insetType);
	...
	if (ret) {
		temp->Invalidate (TOPO_CHANNEL);	// Forces reevaluation of face clusters.
		temp->freeBevelInfo ();
		IMNMeshUtilities14* l_mesh14 = mesh.GetTypedInterface<IMNMeshUtilities14>();
		l_mesh14->GetExtrudeDirection(temp->ChamferData(), flag, bias);
	}
...
bool EditPolyObject::EpfnAlignToGrid (int msl, DWORD flag) {
	...
	Matrix3 atm, otm, res;
	float zoff;
	GetCOREInterface()->GetActiveViewExp().GetConstructionTM(atm);
	...
	GetCOREInterface()->GetModContexts(mcList,nodes);
	if (nodes.Count())
		otm = nodes[0]->GetObjectTM(GetCOREInterface()->GetTime());
	nodes.DisposeTemporary();
...
bool EditPolyObject::EpfnAlignToView (int msl, DWORD flag) {
	...
	Matrix3 atm, otm, res;
	float zoff;
	GetCOREInterface()->GetActiveViewExp().GetAffineTM(atm);
	atm = Inverse(atm);
...
void EditPolyObject::CollapseVertexAnims( int dest, const Tab<int>& verts )
{
	...
	for ( int i=0; i<verts.Count(); ++i ) {
		...
		Control* c2 = pointCtrlContainer->GetSubController( verts[i] );
		if ( c2 == NULL )
			continue;
		Tab<TimeValue> t2;
	...
	Control* c1 = pointCtrlContainer->GetSubController( dest );
	...
	c1 = pointCtrlContainer->GetSubController( dest );
	if ( c1 == NULL )
		return;
...
bool EditPolyObject::WeldBorderVertsAndVertexAnims( float thresh, DWORD flag )
{
	...
	if ( ret && pointCtrlContainer && ( pointCtrlContainer->GetNumSubControllers() == mm.numv ) ) {
...
bool EditPolyObject::EpfnMeshSmoothMesh (MNMesh & mesh, int msl, DWORD flag) {
	...
	int i;
	pblock->GetValue (ep_ms_smoothness, TimeValue(0), smoothness);
	pblock->GetValue (ep_ms_sep_smooth, TimeValue(0), sepSmooth);
	pblock->GetValue (ep_ms_sep_mat, TimeValue(0), sepMat);
	...
	MNMeshTriangulationType triangulationType = GetTriangulationType();
	mesh.SplitFacesUsingBothSidesOfEdge(0, triangulationType);
...
bool EditPolyObject::EpfnTessellateMesh (MNMesh & mesh, int msl, DWORD flag) {
	...
	float tens;
	int type;
	pblock->GetValue (ep_tess_tension, TimeValue(0), tens);
	pblock->GetValue (ep_tess_type, TimeValue(0), type);
	switch (msl) {
		int i, num;
	case MNM_SL_OBJECT:
...
bool EditPolyObject::EpfnSlice(Point3 planeNormal, Point3 planeCenter, bool flaggedFacesOnly, DWORD faceFlags)
{
	if (flaggedFacesOnly) {
      int i;
		for (i=0; i<mm.numf; i++) {
	...
	BOOL sliceSplit;
	pblock->GetValue (ep_split, TimeValue(0), sliceSplit);
	TopoChangeRestore *tchange = NULL;
	if (theHold.Holding ()) {
		tchange = new TopoChangeRestore (this);
		tchange->Before ();
	...
	if (!mm.Slice (planeNormal, offset, MNEPS,
		sliceSplit?true:false, FALSE, flaggedFacesOnly, faceFlags, triangulationType)) {
		if (tchange) delete tchange;
		return false;
	}
	if (tchange) {
...
bool EditPolyObject::EpfnSliceMesh(MNMesh & mesh, int msl, DWORD flag)
{
	...
	bool ret = mesh.Slice (planeNormal, offset, MNEPS,
		sliceSplit?true:false, false, (msl==MNM_SL_FACE)?true:false, flag, triangulationType);
	return ret;
}
...
int EditPolyObject::EpfnCutMesh (MNMesh & mesh) {
	...
	pblock->GetValue (ep_cut_start_level, TimeValue(0), cutStartLevel);
	pblock->GetValue (ep_cut_start_index, TimeValue(0), cutStartIndex);
	pblock->GetValue (ep_cut_start_coords, TimeValue(0), cutStart);
	pblock->GetValue (ep_cut_end_coords, TimeValue(0), cutEnd);
	pblock->GetValue (ep_cut_normal, TimeValue(0), cutNormal);
	pblock->GetValue (ep_split, TimeValue(0), cutSplit);
	...
	case MNM_SL_VERTEX:
		endVertex = mesh.Cut (cutStartIndex, cutEnd, cutNormal, cutSplit?true:false, triangulationType);
		break;
	...
	case MNM_SL_EDGE:
		...
		endVertex = mesh.Cut (nv, cutEnd, cutNormal, cutSplit?true:false, triangulationType);
		break;

	case MNM_SL_FACE:
		endVertex = mesh.CutFace (cutStartIndex, cutStart, cutEnd, cutNormal, cutSplit?true:false, triangulationType);
		break;
	}
...
int EditPolyObject::EpfnCutVertex (int startv, Point3 destination, Point3 projDir) {
	int splitme;
	pblock->GetValue (ep_split, TimeValue(0), splitme);
	TopoChangeRestore *tchange = NULL;
	if (theHold.Holding ()) {
		tchange = new TopoChangeRestore (this);
		tchange->Before ();
	...
	MNMeshTriangulationType triangulationType = GetTriangulationType();
	int ret = mm.Cut (startv, destination, projDir, splitme?true:false, triangulationType);
	if (tchange) {
		tchange->After ();
		theHold.Put (tchange);
	}
...
int EditPolyObject::EpfnCutEdge (int e1, float prop1, int e2, float prop2, Point3 projDir) {
	...
	MNMeshTriangulationType triangulationType = GetTriangulationType();
	int vret = mm.CutEdge (e1, prop1, e2, prop2, projDir, splitme?true:false, triangulationType);
	if (tchange) {
		tchange->After ();
		theHold.Put (tchange);
	}
...
int EditPolyObject::EpfnCutFace (int f1, Point3 start, Point3 dest, Point3 projDir) {
	...
	MNMeshTriangulationType triangulationType = GetTriangulationType();
	f1 = mm.CutFace (f1, start, dest, projDir, splitme?true:false, triangulationType);
	if (tchange) {
		tchange->After ();
		theHold.Put (tchange);
	}
...
bool EditPolyObject::EpfnWeldVertToVert (int v1, int v2) {
	...
	if ( ret && mm.v[v2].GetFlag (MN_DEAD) ) {
		...
		if ( pointCtrlContainer && ( pointCtrlContainer->GetNumSubControllers() == mm.numv ) ) {
			Control* c1 = pointCtrlContainer->GetSubController( v1 );
			Control* c2 = pointCtrlContainer->GetSubController( v2 );
			if ( c2 ) {
				if ( c1 == NULL ) {
					c1 = (Control*)CloneRefHierarchy(c2);
...
bool EditPolyObject::EpfnWeldMeshVerts (MNMesh & mesh, DWORD flag) {
	...
	int i;
	float thresh;
	pblock->GetValue (ep_weld_threshold, TimeValue(0), thresh);
	if ( &mesh == &mm ) {
...
bool EditPolyObject::EpfnWeldMeshEdges (MNMesh & mesh, DWORD flag) {
	float thresh;
	pblock->GetValue (ep_edge_weld_threshold, TimeValue(0), thresh);
...
bool EditPolyObject::EpfnConnectMeshEdges (MNMesh & mesh, DWORD edgeFlag) {
	...
	pblock->GetValue (ep_connect_edge_segments, TimeValue(0), connectSegs);
	pblock->GetValue (ep_connect_edge_pinch, TimeValue(0), connectPinch);
	pblock->GetValue (ep_connect_edge_slide, TimeValue(0), connectSlide);

	IMNMeshUtilities8* l_meshToConnect = mesh.GetTypedInterface<IMNMeshUtilities8>();

	bool l_ret = false; 
	if (l_meshToConnect != NULL )
	{
		MNMeshTriangulationType triangulationType = GetTriangulationType();
		l_ret = l_meshToConnect->ConnectEdges(edgeFlag, connectSegs,connectPinch, connectSlide, triangulationType);
	}
...
bool EditPolyObject::EpfnConnectEdges (DWORD edgeFlag) {
	...
	IMNMeshUtilities8* l_meshToConnect = mm.GetTypedInterface<IMNMeshUtilities8>();
...
{
	MNMeshTriangulationType triangulationType = GetTriangulationType();
	return mesh.ConnectVertices(vertexFlag, triangulationType);
}
...
bool EditPolyObject::EpfnConnectVertices (DWORD vertexFlag) {
	...
	bool areVerticesConnected = EpfnConnectMeshVertices(mm, vertexFlag);
	if (!areVerticesConnected)
	{
		if (tchange) delete tchange;
		return false;
	}
...
bool EditPolyObject::EpfnExtrudeAlongSplineMesh (MNMesh & mesh, MNTempData *pTempData, DWORD faceFlag) {
	...
	pblock->GetValue (ep_extrude_spline_node, t, pSplineNode);
	...
	pblock->GetValue (ep_extrude_spline_segments, TimeValue (0), segments);
	pblock->GetValue (ep_extrude_spline_taper, TimeValue (0), taper);
	pblock->GetValue (ep_extrude_spline_taper_curve, TimeValue(0), taperCurve);
	pblock->GetValue (ep_extrude_spline_twist, TimeValue(0), twist);
	pblock->GetValue (ep_extrude_spline_rotation, TimeValue(0), rotation);
	pblock->GetValue (ep_extrude_spline_align, TimeValue(0), align);
	...
	if (ret) pTempData->Invalidate (ALL_CHANNELS);
...
bool EditPolyObject::EpfnLiftFromEdgeMesh (MNMesh & mesh, MNTempData *pTemp, DWORD faceFlag) {
	...
	float liftAngle;
	pblock->GetValue (ep_lift_edge, TimeValue(0), liftEdge);
	pblock->GetValue (ep_lift_angle, TimeValue(0), liftAngle);
	pblock->GetValue (ep_lift_segments, TimeValue(0), segments);
	...
	if (ret) pTemp->Invalidate (ALL_CHANNELS);
...
void EditPolyObject::EpfnAutoSmooth (TimeValue t) {
   int i;
	...
	if (i==mm.numf) return;
	float angle;
	pblock->GetValue (ep_face_smooth_thresh, t, angle);
...
{
	IntTab facesSelected;

	TopoChangeRestore* tchange = nullptr;
	if (theHold.Holding())
	{
		tchange = new TopoChangeRestore(this);
		tchange->Before();
	...
	// Build alist of active, selected faces
	for (int f = 0; f != mm.numf; ++f)
	{
		MNFace& faceCurr = mm.f[f];
		bool isDead = faceCurr.GetFlag(MN_DEAD);
		bool isSelected = faceCurr.GetFlag(flag);
		if ((!isDead) && isSelected)
		{
			facesSelected.Append(1, &f, 10);
		}
	}

	// Retriangulate the selected faces, employing the algorithm indicated by the ep_enable_enhanced_retriangulation
	// parameter block entry
	TriangulateFaces(facesSelected);

	int nFacesSelected = facesSelected.Count();
	if (nFacesSelected != 0)
	{
		delete tchange;
		return false;
...
void EditPolyObject::BeginPerDataModify (int mnSelLevel, int channel) {
	if (mnSelLevel == MNM_SL_CURRENT) mnSelLevel = editablePolyMeshSelLevel[selLevel];
...
void EditPolyObject::BeginVertexColorModify (int mapChannel, DWORD flag) {
	if ((mm.MNum() <= mapChannel) || mm.M(mapChannel)->GetFlag (MN_DEAD)) InitVertColors (mapChannel);
	if (mchange) delete mchange;
	mchange = new MapChangeRestore (this, mapChannel);

	if (mm.selLevel == MNM_SL_VERTEX)
	{
		//need to make sure all the flagged vertices are unique since data can be shared across multiple faces
		//move all our selections into a vector and bitarray
		std::vector<int> selectedVerts;
		BitArray selectedVertsBits;
		selectedVerts.reserve(mm.numv);
		selectedVertsBits.SetSize(mm.numv);
		selectedVertsBits.ClearAll();


		MNMap& map = *mm.M(mapChannel);
		std::vector<int> selectedUVWs;
		BitArray selectedUVWsBits;
		selectedUVWs.reserve(map.numv);
		selectedUVWsBits.SetSize(map.numv);
		selectedUVWsBits.ClearAll();

		//get our geom selections from flags to a index vector and bitarray
		for (int vertexIndex = 0; vertexIndex < mm.numv; ++vertexIndex)
		{
			if (mm.v[vertexIndex].GetFlag(MN_DEAD)) continue;
			if (mm.v[vertexIndex].GetFlag(flag))
			{
				selectedVerts.push_back(vertexIndex);
				selectedVertsBits.Set(vertexIndex, TRUE);
			}
		}

		//convert our selected geom verts to uvws vector and bitarray
		for (int faceIndex = 0; faceIndex < mm.numf; ++faceIndex)
		{
			const MNFace& face = mm.f[faceIndex];
			const MNMapFace& mapFace = map.f[faceIndex];
			if (face.GetFlag(MN_DEAD)) continue;
			int degree = face.deg;
			for (int corner = 0; corner < degree; ++corner)
			{
				int vertexIndex = face.vtx[corner];
				int uvwIndex = mapFace.tv[corner];
				if (selectedVertsBits[vertexIndex])
				{
					if (!selectedUVWsBits[uvwIndex])
					{
						selectedUVWsBits.Set(uvwIndex, TRUE);
						selectedUVWs.push_back(uvwIndex);
					}
				}
			}
		}

		//see if any of our selected uvws are attached to a non selected vertex
		//if that is a case we need to add a new uvw since that uvw is shared across
		//a selected and non selected vertex
		bool needToMakeUnique = false;
		int newId = -1;
		BitArray uvwsWeCanUse(selectedUVWsBits);

		std::vector<int*> cornerPtrs;
		cornerPtrs.reserve(map.numf * 4);
		for (int faceIndex = 0; faceIndex < mm.numf; ++faceIndex)
		{
			const MNFace& face = mm.f[faceIndex];
			const MNMapFace& mapFace = map.f[faceIndex];
			...
			for (int corner = 0; corner < degree; ++corner)
			{
				int vertexIndex = face.vtx[corner];
				int uvwIndex = mapFace.tv[corner];
				//geom is not selected and uvw is meaning a non selected geom color will change
				if ((selectedUVWsBits[uvwIndex]) &&
					(!selectedVertsBits[vertexIndex]))
				{
					needToMakeUnique = true;
					//we cannot reuse this entry since it being used by a non selected vertex
					//so remove it
					uvwsWeCanUse.Set(uvwIndex, FALSE);
				}
				//hold a vector of all our corners attached to out selected vertices
				//so we can quickly change them
				if ((selectedUVWsBits[uvwIndex]) &&
					(selectedVertsBits[vertexIndex]))
				{
					int* cornerPtr = &mapFace.tv[corner];
					cornerPtrs.push_back(cornerPtr);
				}
			}
		}
		//if all our selected uvws are only on the selected vertices we can just skip
		//otherwise wee need to add a new uvw so they are not shared
		if (needToMakeUnique)
		{
			//all are shared so need to add a new one
			int newUVWIndex = -1;
			//if we have no entries create a new one
			if (uvwsWeCanUse.IsEmpty())
			{
				newUVWIndex = map.NewVert(UVVert(0, 0, 0));
			}
			//other use a non shared entry by searching thru the unused list
			else
			{
				for (int uvwIndex = 0; uvwIndex < uvwsWeCanUse.GetSize(); ++uvwIndex)
				{
					if (uvwsWeCanUse[uvwIndex])
					{
						newUVWIndex = uvwIndex;
						break;
					}
				}
			}
			//set all the selected UVW corners to our new index
			for (int* corner : cornerPtrs)
			{
				*corner = newUVWIndex;
			}
			LocalDataChanged(VERTCOLOR_CHANNEL);
		}
	}
...
void EditPolyObject::SetVertexDataValue (int channel, float value, DWORD flags, TimeValue t) {
	...
	if (change) {
		...
		LocalDataChanged(TOPO_CHANNEL); // this is passed versus geom because it will trigger a topo change in subdiv mdifiers ( the OSD watches for geom and topo changes to determine what to do), ideally we need to come up with a new part ID when we break the SDK
		RefreshScreen ();
...
void EditPolyObject::SetEdgeDataValue (int channel, float value, DWORD flags, TimeValue t) {
	...
	if (change) {
		macroRecorder->FunctionCall(_T("$.EditablePoly.setEdgeData"), 2, 0,
			mr_int, channel, mr_float, value);
		LocalDataChanged(TOPO_CHANNEL);  // FIX ideally we need to come up with a new part ID when we break the SDK
		RefreshScreen ();
...
void EditPolyObject::InitVertColors (int mapChannel) {
	...
	if (mapChannel>=mm.MNum()) mm.SetMapNum (mapChannel+1);
...
Color EditPolyObject::GetVertexColor (bool *uniform, int *num, int mapChannel, DWORD flags, TimeValue t) {
	...
	if ((mm.MNum() <= mapChannel) || (mm.M(mapChannel)->GetFlag (MN_DEAD))) {
		if (num) {
			for (i=0; i<mm.numv; i++) {
				if (mm.v[i].GetFlag (MN_DEAD)) continue;
...
void EditPolyObject::SetVertexColor (Color clr, int mapChannel, DWORD flags, TimeValue t) {
   ...
   int i;
	...
	LocalDataChanged ((mapChannel<1) ? VERTCOLOR_CHANNEL : TEXMAP_CHANNEL);
	RefreshScreen ();
}
...
void EditPolyObject::EpfnSelectVertByColor (BOOL add, BOOL sub, int mapChannel, TimeValue t) {
	MNMapFace* cf = NULL;
	UVVert* cv = NULL;
	if ((mapChannel>=mm.MNum()) || (mapChannel<-NUM_HIDDENMAPS) || mm.M(mapChannel)->GetFlag (MN_DEAD)) {
		cf = NULL;
		cv = NULL;
	} else {
	...
	Color selColor;
	pblock->GetValue (ep_vert_sel_color, t, selColor);
	pblock->GetValue (ep_vert_selc_r, t, deltaR);
	pblock->GetValue (ep_vert_selc_g, t, deltaG);
	pblock->GetValue (ep_vert_selc_b, t, deltaB);
...
int EditPolyObject::EpFnGetEdgeChamferType()
{
	ScriptPrint(GetString(IDS_WARNING_CHAMFER_TYPE_OBSOLETE));
	...
	{
	case EPOLY_EDGE_CHAMFER_MITER_TRI:
		return EPOLY_STANDARD_EDGE_CHAMFER;
	case EPOLY_EDGE_CHAMFER_MITER_QUAD:
	case EPOLY_EDGE_CHAMFER_MITER_UNIFORM:
	case EPOLY_EDGE_CHAMFER_MITER_RADIAL:
	case EPOLY_EDGE_CHAMFER_MITER_PATCH:
		return EPOLY_QUAD_EDGE_CHAMFER;
	default:
		return -1;
	}
}

void EditPolyObject::EpFnSetEdgeChamferType(int chamferType)
{
	ScriptPrint(GetString(IDS_WARNING_CHAMFER_TYPE_OBSOLETE));
	switch (chamferType)
	{
	case EPOLY_STANDARD_EDGE_CHAMFER:
		pblock->SetValue(ep_edge_chamfer_type, TimeValue(0), EPOLY_STANDARD_EDGE_CHAMFER);
		pblock->SetValue(ep_edge_chamfer_mitering_type, TimeValue(0), EPOLY_EDGE_CHAMFER_MITER_TRI);
		break;
	case EPOLY_QUAD_EDGE_CHAMFER:
	{
		int quadInt = -1;
		pblock->GetValue(ep_edge_chamfer_quad_intersections, TimeValue(0), quadInt);
		bool hasQuadIntersection = quadInt != 0;
		pblock->SetValue(ep_edge_chamfer_type, TimeValue(0), EPOLY_QUAD_EDGE_CHAMFER);
		pblock->SetValue(ep_edge_chamfer_mitering_type, TimeValue(0), hasQuadIntersection ? EPOLY_EDGE_CHAMFER_MITER_UNIFORM : EPOLY_EDGE_CHAMFER_MITER_QUAD);
	}
	break;
	default:
		throw MAXException(GetString(IDS_ERROR_SET_CHAMFER_TYPE));
		DbgAssert(0);
		break;
	}
}

bool EditPolyObject::EpFnGetEdgeChamferQuadIntersections()
{
	ScriptPrint(GetString(IDS_WARNING_CHAMFER_QUAD_INTERSECTIONS_OBSOLETE));

	int miteringType = -1;
	pblock->GetValue(ep_edge_chamfer_mitering_type, TimeValue(0), miteringType);
	if (miteringType == EPOLY_EDGE_CHAMFER_MITER_TRI)
	{
		ScriptPrint(GetString(IDS_WARNING_GET_QUAD_INTERSECTIONS));
		return false;
	}

	int quadIntersections = -1;
	pblock->GetValue(ep_edge_chamfer_quad_intersections, TimeValue(0), quadIntersections);

	return quadIntersections == 1;
}

void EditPolyObject::EpFnSetEdgeChamferQuadIntersections(bool quadIntersections)
{
	ScriptPrint(GetString(IDS_WARNING_CHAMFER_QUAD_INTERSECTIONS_OBSOLETE));
	...
	if (miteringType == EPOLY_EDGE_CHAMFER_MITER_TRI)
	{
		ScriptPrint(GetString(IDS_WARNING_SET_QUAD_INTERSECTIONS));
		return;
	}

	pblock->SetValue(ep_edge_chamfer_quad_intersections, TimeValue(0), quadIntersections);
	pblock->SetValue(ep_edge_chamfer_mitering_type, TimeValue(0), quadIntersections ? EPOLY_EDGE_CHAMFER_MITER_UNIFORM : EPOLY_EDGE_CHAMFER_MITER_QUAD);
}
...
bool CreateCurveFromMeshEdges (MNMesh & mesh, INode *onode, Interface *ip, TSTR & name, bool curved, DWORD flag) {
	...
	hs.Resume();
	INode *node = GetCOREInterface()->CreateObjectNode (shape);
	hs.Suspend();

	INode *nodeByName = GetCOREInterface()->GetINodeByName (name);
	if (nodeByName != node) {
		if (nodeByName) GetCOREInterface()->MakeNameUnique(name);
		node->SetName (name);
	}
	Matrix3 ntm = onode->GetNodeTM(GetCOREInterface()->GetTime());
	node->SetNodeTM (GetCOREInterface()->GetTime(),ntm);
	node->FlagForeground (GetCOREInterface()->GetTime(),FALSE);
...
bool EditPolyObject::EpfnBridgeMesh (MNMesh & mesh, int epolySelLevel, DWORD flag)
{
	...
	switch (epolySelLevel) {
		...
		case EP_SL_FACE:
			...
			break;
		case EP_SL_EDGE:
			IMNMeshUtilities8* l_meshToBridge = mesh.GetTypedInterface<IMNMeshUtilities8>();
			
			if (l_meshToBridge != NULL )
			{
				if (useSel) 
					ret = l_meshToBridge->BridgeSelectedEdges(flag, smoothThresh, segments, adjacentAngle, l_bReverseTri, triangulationType);
				else
					ret = l_meshToBridge->BridgeTwoEdges(targ1, targ2, segments, 0, 0, 0, false, triangulationType);
			}
			break;
	}
...
bool EditPolyObject::EpfnBridge (int epolySelLevel, DWORD flag) {
	...
	LocalDataChanged (TOPO_CHANNEL|SELECT_CHANNEL|GEOM_CHANNEL|VERTCOLOR_CHANNEL|TEXMAP_CHANNEL);
	return true;
}
...
bool EditPolyObject::EpfnMakePlanarIn (int dim, int msl, DWORD flag, TimeValue t) {
	...
	float *vss = ((flag == MN_SEL) && (msl == editablePolyMeshSelLevel[selLevel])) ? mm.getVSelectionWeights() : NULL;
	...
	if (macroRecorder->Enabled()) {
		...
		if (msl != editablePolyMeshSelLevel[selLevel]) {
			if (flag != MN_SEL) {
