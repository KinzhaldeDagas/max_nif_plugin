...
BOOL EPolyActionCB::ExecuteAction (int id) {
	...
	case ID_EP_SEL_LEV_VERTEX:
		if(mpObj->GetEPolySelLevel() == EP_SL_VERTEX)
			mpObj->SetEPolySelLevel (EP_SL_OBJECT);
		else
	...
	case ID_EP_SEL_LEV_BORDER:
		if(mpObj->GetEPolySelLevel() == EP_SL_BORDER)
			mpObj->SetEPolySelLevel (EP_SL_OBJECT);
		else
	...
	case ID_EP_SEL_LEV_ELEMENT:
		if(mpObj->GetEPolySelLevel() == EP_SL_ELEMENT)
			mpObj->SetEPolySelLevel (EP_SL_OBJECT);
		else
	...
	case ID_EP_SEL_BYVERT:
		theHold.Begin ();
		mpObj->getParamBlock()->GetValue (ep_by_vertex, TimeValue(0), checkboxValue);
	...
	case ID_EP_SEL_IGBACK:
		theHold.Begin ();
		mpObj->getParamBlock()->GetValue (ep_ignore_backfacing, TimeValue(0), checkboxValue);
		mpObj->getParamBlock()->SetValue (ep_ignore_backfacing, TimeValue(0), !checkboxValue);
		theHold.Accept (GetString (IDS_PARAMETERS));
		break;

	case ID_EP_SEL_IGOCCLUDED:
		theHold.Begin();
		mpObj->getParamBlock()->GetValue(ep_ignore_occluded, TimeValue(0), checkboxValue);
		mpObj->getParamBlock()->SetValue(ep_ignore_occluded, TimeValue(0), !checkboxValue);
		theHold.Accept(GetString(IDS_PARAMETERS));
		break;
	...
	case ID_EP_PREVIEW_SEL:
		mpObj->getParamBlock()->GetValue(ep_select_mode,TimeValue(0),checkboxValue);
		theHold.Begin();
	...
	case ID_EP_SS_ON:
		theHold.Begin ();
		mpObj->getParamBlock()->GetValue (ep_ss_use, TimeValue(0), checkboxValue);
	...
	case ID_EP_SS_BACKFACE:
		theHold.Begin ();
		mpObj->getParamBlock()->GetValue (ep_ss_affect_back, TimeValue(0), checkboxValue);
	...
	case ID_EP_SS_LOCK:
		theHold.Begin ();
		mpObj->getParamBlock()->GetValue (ep_ss_lock, TimeValue(0), checkboxValue);
...
/*		Changed since paint btn command seems better MZ.. leaving in in case of regressions
	...
	case ID_EP_NONE_CONSTRAINT:
		mpObj->getParamBlock()->GetValue (ep_constrain_type, TimeValue(0), constrainType);
	...
	case ID_EP_SEL_RING_UP:
		{
			...
			mpObj->LocalDataChanged(SELECT_CHANNEL);
			mpObj->RefreshScreen ();
...
static ActionDescription epolyActions[] = {
	...
	ID_EP_SUBOBJECT_PICK,
		IDS_EP_SUBOBJECT_PICK,
		IDS_EP_SUBOBJECT_PICK,
		IDS_EDITABLE_POLY,

	ID_EP_POINT_TO_POINT,
		IDS_EP_POINT_TO_POINT,
		IDS_EP_POINT_TO_POINT,
		IDS_EDITABLE_POLY
...
constexpr int kNumEPolyActions = sizeof(epolyActions) / sizeof(ActionDescription);
...
ActionTable* GetEPolyActions() {
	...
	*/
		...
		*/
		...
		item = pTab->GetAction(ID_EP_SUBOBJECT_PICK);
		if(item)
		{
			item->GetDescriptionText(desc);
			actionOverrideMan->RegisterActionItemOverride(item,desc); 
