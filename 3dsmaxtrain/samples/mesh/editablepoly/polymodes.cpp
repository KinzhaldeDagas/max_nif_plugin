...
#include "shape.h"
#include "winutil.h"
#include <Geom/dpoint3.h>
#include <Geom/ipoint3.h>
#include <Geom/dmatrix3.h>
...
int EditPolyObject::EpActionGetCommandMode()
{
	...
	if (currentMode == extrudeVEMode) {
		if (editablePolyMeshSelLevel[selLevel] == MNM_SL_EDGE) return epmode_extrude_edge;
		else return epmode_extrude_vertex;
	}
	if (currentMode == extrudeMode) return epmode_extrude_face;
	if (currentMode == chamferMode) {
		if (editablePolyMeshSelLevel[selLevel] == MNM_SL_EDGE) return epmode_chamfer_edge;
	...
	{
		if (editablePolyMeshSelLevel[selLevel] == MNM_SL_VERTEX)
			return epmode_cut_vertex;
		else if(editablePolyMeshSelLevel[selLevel]==MNM_SL_EDGE)
...
void EditPolyObject::EpActionToggleCommandMode (int mode) {
	...
	case epmode_chamfer_edge:
		// Special case - only use DeleteMode to exit mode if in correct SO level,
		// Otherwise use EnterCommandMode to switch SO levels and enter mode again.
		if ((currentMode==cmd) && (editablePolyMeshSelLevel[selLevel]==MNM_SL_EDGE)) ip->DeleteMode (cmd);
		else {
			EpPreviewCancel ();
			EpfnClosePopupDialog ();	// Cannot have command mode, settings at same time.
			EnterCommandMode (mode);
...
void EditPolyObject::EnterCommandMode(int mode) {
	...
	case epmode_create_edge:
		if (editablePolyMeshSelLevel[selLevel] != MNM_SL_EDGE) ip->SetSubObjectLevel (EP_SL_EDGE);
	...
	case epmode_inset_face:
		if (editablePolyMeshSelLevel[selLevel] != MNM_SL_FACE) ip->SetSubObjectLevel (EP_SL_FACE);
...
int ChamferMouseProc::proc (HWND hwnd, int msg, int point, int flags, IPoint2 m ) {	
	...
	static float baseTensionOrDepth = 1.0f;
	static float tensionOrDepth = 1.0f;
    ...
    // Determine whether we are adjusting amount only, or segments and tension/depth as well, and the appropriate tension/depth
    // minimum
    bool isAmountOnly = false;
    float minTensionOrDepth = -0.5f;
    int selLevel = mpEditPoly->GetMNSelLevel();
    IParamBlock2* params = mpEditPoly->getParamBlock();
    DbgAssert((selLevel == MNM_SL_EDGE) || (selLevel == MNM_SL_VERTEX));
    if (selLevel == MNM_SL_EDGE)
    {
        minTensionOrDepth = 0.0f;
        int miteringType = EPOLY_EDGE_CHAMFER_MITER_TRI;
        params->GetValue (ep_edge_chamfer_mitering_type, ip->GetTime(), miteringType);
	...
	case MOUSE_POINT:
		...
		case 0:
			...
			if (!isAmountOnly)
            {
                if (selLevel == MNM_SL_EDGE)
                {
                    params->GetValue(ep_edge_chamfer_tension, ip->GetTime(), baseTensionOrDepth);
                    params->GetValue(ep_edge_chamfer_segments, ip->GetTime(), baseSegments);
                }
                else
                {
                    params->GetValue(ep_vertex_chamfer_depth, ip->GetTime(), baseTensionOrDepth);
                    params->GetValue(ep_vertex_chamfer_num_segments, ip->GetTime(), baseSegments);
                }
			}
			om = m;
		...
		case 1:
			switch (selLevel) {
			case MNM_SL_VERTEX:
				params->SetValue(ep_vertex_chamfer, ip->GetTime(), chamfer);
				break;
			case MNM_SL_EDGE:
				params->SetValue(ep_edge_chamfer, ip->GetTime(), chamfer);
				break;
			}
			om = m;
			cumDelta = IPoint2(0,0);
	...
	case MOUSE_MOVE:
		...
		if (altPressed && (!isAmountOnly)) {
            ...
            if (selLevel == MNM_SL_EDGE)
            {
                params->SetValue(ep_edge_chamfer_segments, ip->GetTime(), segments);
            }
            else
            {
                params->SetValue(ep_vertex_chamfer_num_segments, ip->GetTime(), segments);
            }
		}
		else {
			switch(point) {
			...
			case 2:
				...
				if (m.x < om.x)
					tensDelta = -tensDelta;
				tensionOrDepth = baseTensionOrDepth + cumTensDelta + tensDelta;
                if (tensionOrDepth < minTensionOrDepth)
                {
                    tensionOrDepth = minTensionOrDepth;
                }
                else if (tensionOrDepth > 1.0f)
                {
                    tensionOrDepth = 1.0f;
                }
                ...
                if (selLevel == MNM_SL_EDGE)
                {
                    params->SetValue(ep_edge_chamfer_tension, ip->GetTime(), tensionOrDepth);
                    params->SetValue(ep_edge_chamfer_depth, ip->GetTime(), 1.0f - tensionOrDepth);    // Also store as depth
                }
                else
                {
                    params->SetValue(ep_vertex_chamfer_depth, ip->GetTime(), tensionOrDepth);
                }

                break;
			}
		}
		mpEditPoly->EpfnDragChamfer (chamfer, tensionOrDepth, ip->GetTime());
		ip->RedrawViews(ip->GetTime(),REDRAW_INTERACTIVE);
		break;
...
MouseCallBack *ChamferCMode::MouseProc(int* numPoints)
{
    ...
    // Three points in all cases except the legacy edge chamfer
    *numPoints = 3;
    if (selLevel == MNM_SL_EDGE)
    {
        int miteringType = EPOLY_EDGE_CHAMFER_MITER_TRI;
        (mpEditPoly->getParamBlock())->GetValue (ep_edge_chamfer_mitering_type, ip->GetTime(), miteringType);
        if (miteringType == EPOLY_EDGE_CHAMFER_MITER_TRI)
        {
            *numPoints = 2;
        }
...
void CutProc::DrawCutter (HWND hWnd, bool bErase, bool bPresent) {
	...
	// Check for auto erasure (NULL in supplied HWND uses the last HWND that was used to draw)
	if (hWnd == NULL)
	{
		// If nothing to erase, just return
		if (mLastHWND == NULL) return;

		// Set for erasure in the last-used window
		bErase = true;
		hWnd = mLastHWND;
	}
	...
	// Remember the last HWND used as a draw for proper erasure
	mLastHWND = bErase ? NULL : hWnd;
}
...
int CutProc::proc (HWND hwnd, int msg, int point, int flags, IPoint2 m ) {
	...
	case MOUSE_POINT:
		...
		// Erase last cutter line (must be done before setting active viewport so old lines in past viewport are erased):
		if (startIndex > -1)
		{
			DrawCutter(NULL, true, true);
		}
		...
		mpEPoly->getParamBlock()->GetValue (ep_cut_start_index, TimeValue(0), mLastHitIndex);
		mpEPoly->getParamBlock()->GetValue (ep_cut_start_level, TimeValue(0), mLastHitLevel);
	...
	case MOUSE_MOVE:
		...
		if (startIndex>-1) {
			...
			mpEPoly->getParamBlock()->GetValue (ep_cut_start_coords, TimeValue(0), startHitPoint);
		...
		mpEPoly->getParamBlock()->GetValue (ep_interactive_full, TimeValue(0), fullyInteractive);
		if (fullyInteractive)
			ip->RedrawViews (ip->GetTime(), REDRAW_INTERACTIVE);
		break;
		}
...
void CutCMode::ExitMode() {
	// Make sure the dotted line gets properly erased, if it was drawn
	proc.DrawCutter(NULL, true, true);
...
void QuickSliceProc::ComputeSliceParams (bool center) {
	...
	if (center) {
		...
		Point3 planeCtr = (mLocal1 + mLocal2) * .5f;
		mpEPoly->EpSetSlicePlane (planeNormal, planeCtr, size);
	}
}
...
HitRecord *WeldMouseProc::HitTest (IPoint2 &m, ViewExp *vpt) {
	...
	mpEditPoly->ClearHitLevelOverride ();
	if (!vpt->NumSubObjHits()) return nullptr;
	HitLog& hitLog = vpt->GetSubObjHitList();
	HitRecord *hr = hitLog.ClosestHit();
	if (targ1>-1) {
		if (targ1 == hr->hitInfo) return nullptr;
		if (mpEditPoly->GetMNSelLevel() == MNM_SL_EDGE) {
			if (!CanWeldEdges (targ1, hr->hitInfo)) return nullptr;
		} else {
			if (!CanWeldVertices (targ1, hr->hitInfo)) return nullptr;
		}
	}
	return hr;
}
...
bool PointToPointProc::AdjacentLoop(int mode, int index)
{
	...
	IMNMeshUtilities13* l_mesh13 = mm.GetTypedInterface<IMNMeshUtilities13>();
		...
		if ((mFirstHit != -1) && (index != -1) && !loopSet)
		{
			...
			if ((index != -1) && (GetCOREInterface()->GetSubObjectLevel() == EP_SL_VERTEX))
			{
				...
				else
				{
					...
					int num_edges = 0;
					std::unique_ptr<MaxSDK::EdgeDescr[]> edges;
					mPointToPointPath->GetPath(edges, num_edges);
			...
			else if ((index != -1) && (GetCOREInterface()->GetSubObjectLevel() == EP_SL_EDGE))
			{
				...
				else
				{
					...
					int num_edges1 = 0;
					std::unique_ptr<MaxSDK::EdgeDescr[]> edges1;
					float d1 = mPointToPointPath->GetPath(edges1, num_edges1);

					int v2 = mpEditPoly->mm.e[index].v2;
					mPointToPointPath->SetEndPoint(v2);

					int num_edges2 = 0;
					std::unique_ptr<MaxSDK::EdgeDescr[]> edges2;
					float d2 = mPointToPointPath->GetPath(edges2, num_edges2);

					int num_edges = 0;
					std::unique_ptr<MaxSDK::EdgeDescr[]> edges = std::move(edges1);
					if (d2 < d1)
					{
						edges = std::move(edges2);
						num_edges = num_edges2;
					}
			...
			else if ((index != -1) && (GetCOREInterface()->GetSubObjectLevel() == EP_SL_FACE))
			{
				...
				else
				{
					int num_edges = 0;
					std::unique_ptr<MaxSDK::EdgeDescr[]> edges;
					float d = mPointToPointPath->GetPath(edges, num_edges);
...
void PointToPointMode::Display(ViewExp *vpt, GraphicsWindow *gw) {
	...
	IPoint3 p(4, 32, 0);
	vpt->getGW()->wText(&p, title.data()); //draw it undrneath the viewport text!
}