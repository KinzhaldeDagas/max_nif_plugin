...
#include "PolyEdUi.h"
#include <PolySelectionProcessor.h>
#include <PolyModeSelectionProcessor.h>
...
// set the default order of rollups
#define EP_ROLLUP_ORDER_SUBOBJ			1
#define EP_ROLLUP_ORDER_GEOM			2
#define EP_ROLLUP_ORDER_SURF			3
#define EP_ROLLUP_ORDER_POLYSMOOTH		4
#define EP_ROLLUP_ORDER_VERTCOLORS		5
#define EP_ROLLUP_ORDER_SUBDIV_SURFACE	6
#define EP_ROLLUP_ORDER_SUBDIV_DISP		7
#define EP_ROLLUP_ORDER_PAINTDEF		8
...
const EPolyManipulatorGrip::ManipulateGrips EditPolyObject::m_gripTypeMap[EPoly13::EPolyManipulatorGripCount] = {EPolyManipulatorGrip::eSoftSelFalloff, EPolyManipulatorGrip::eSoftSelPinch, EPolyManipulatorGrip::eSoftSelBubble,
                                                                                                                 ...
                                                                                                                 EPolyManipulatorGrip::eVertexWeight, EPolyManipulatorGrip::eVertexCrease};
...
EPOLY_INTERFACE, _T("EditablePoly"), IDS_EDITABLE_POLY, &editPolyObjectDesc, FP_MIXIN,
	...
	epfn_divide_face2, _T("insertVertexInFace"), 0, TYPE_INDEX, 0, 3,
		_T("faceID"), 0, TYPE_INDEX,
		_T("vertexCoefficients"), 0, TYPE_FLOAT_TAB_BR,
		_T("select"), 0, TYPE_bool, f_keyArgDefault, false,
	...
	// Add second version to match new name.
	epfn_divide_edge2, _T("insertVertexInEdge"), 0, TYPE_INDEX, 0, 3,
		_T("edgeID"), 0, TYPE_INDEX,
		_T("proportion"), 0, TYPE_FLOAT,
		_T("select"), 0, TYPE_bool, f_keyArgDefault, false,
    ...
    epfn_get_last_operation, _T("GetLastOperation"), 0, TYPE_INT, 0, 0,
	...
	epfn_get_preserve_map, _T("GetPreserveMap"), 0, TYPE_bool, 0, 1,
			_T("mapChannel"), 0, TYPE_INT,
	epfn_set_preserve_map, _T("SetPreserveMap"), 0, TYPE_VOID, 0, 2,
			_T("mapChannel"), 0, TYPE_INT,
	...
	epfn_make_hard_edges, _T("makeHardEdges"), 0, TYPE_bool, 0, 1,
		_T("flag"), 0, TYPE_DWORD,
	epfn_make_smooth_edges, _T("makeSmoothEdges"), 0, TYPE_bool, 0, 1,
		_T("flag"), 0, TYPE_DWORD,
	epfn_select_hard_edges, _T("selectHardEdges"), 0, TYPE_VOID, 0, 0, 
	epfn_select_smooth_edges, _T("selectSmoothEdges"), 0, TYPE_VOID, 0, 0, 
	epfn_triangulate, _T("triangulate"), 0, TYPE_VOID, 0, 0, 
	enums,
		epolyEnumButtonOps, 59,
			...
			_T("MakeHardEdges"), epop_make_hard_edges,
			_T("MakeSmoothEdges"), epop_make_smooth_edges,
			_T("SelectHardEdges"), epop_select_hard_edges,
			_T("SelectSmoothEdges"), epop_select_smooth_edges,
		...
		edgeChamferType, 2,
			_T("Standard"), EPOLY_STANDARD_EDGE_CHAMFER,
			_T("Quad"), EPOLY_QUAD_EDGE_CHAMFER,

	properties,

		epfn_get_edge_chamfer_type, epfn_set_edge_chamfer_type, _T("edgeChamferType"), 0, TYPE_ENUM, edgeChamferType,
		epfn_get_edge_chamfer_quad_intersections, epfn_set_edge_chamfer_quad_intersections, _T("edgeChamferQuadIntersections"), 0, TYPE_BOOL,
...
const TCHAR *LookupEditablePolySelLevel (int selLevel) {
	int count = epfi.enumerations.Count();
   int i;
	for (i=0; i<count; i++)
...
const TCHAR *LookupMNMeshSelLevel (int selLevel) {
	int count = epfi.enumerations.Count();
   int i;
	for (i=0; i<count; i++)
...
class EditPolyConstraintValidatorClass : public PBValidator
{
	...
	{ 
		if(v.i<0||v.i>3)
			return FALSE;
		int last;
		((EditPolyObject*)owner)->getParamBlock()->GetValue(ep_constrain_type, TimeValue(0), last);
...
static GenSubObjType SOT_Poly(NULL, EditPolyIcon::stackPolygon, 0);
...
static ParamBlockDesc2 ep_param_block ( ep_block, _T("ePolyParameters"),
	...
	ep_by_vertex, _T("selByVertex"), TYPE_BOOL, P_TRANSIENT, IDS_SEL_BY_VERTEX,
		p_default, FALSE,
		p_ui, ep_select, TYPE_SINGLECHECKBOX, IDC_SEL_BYVERT,
		p_end,

	ep_ignore_backfacing, _T("ignoreBackfacing"), TYPE_BOOL, P_RESET_DEFAULT, IDS_IGNORE_BACKFACING,
		p_default, FALSE,
		p_accessor, &theEditPolyObjectPBAccessor,
		p_end,

	ep_ignore_occluded, _T("ignoreOccluded"), TYPE_BOOL, P_RESET_DEFAULT, IDS_IGNORE_OCCLUDED,
		p_default, FALSE,
		p_accessor, &theEditPolyObjectPBAccessor,
		p_end,
	...
	ep_select_by_angle, _T("selectByAngle"), TYPE_BOOL, P_RESET_DEFAULT, IDS_SELECT_BY_ANGLE,
		...
		p_ui, ep_select, TYPE_SINGLECHECKBOX, IDC_SELECT_BY_ANGLE,
		p_end,
	...
	ep_ss_use, _T("useSoftSel"), TYPE_BOOL, 0, IDS_USE_SOFTSEL,
		...
		p_ui, ep_softsel, TYPE_SINGLECHECKBOX, IDC_USE_SOFTSEL,
		p_end,

	ep_ss_edist_use, _T("ssUseEdgeDist"), TYPE_BOOL, 0, IDS_USE_EDIST,
		p_default, FALSE,
		p_enable_ctrls, 1, ep_ss_edist,
		p_ui, ep_softsel, TYPE_SINGLECHECKBOX, IDC_USE_EDIST,
		p_end,
	...
	ep_ss_affect_back, _T("affectBackfacing"), TYPE_BOOL, 0, IDS_AFFECT_BACKFACING,
		p_default, TRUE,
		p_ui, ep_softsel, TYPE_SINGLECHECKBOX, IDC_AFFECT_BACKFACING,
		p_end,
	...
	ep_ss_lock, _T("lockSoftSel"), TYPE_BOOL, P_TRANSIENT, IDS_LOCK_SOFTSEL,
		p_accessor,		&theEditPolyObjectPBAccessor,
		p_ui, ep_softsel, TYPE_SINGLECHECKBOX, IDC_LOCK_SOFTSEL,
		p_end,
	...
	ep_surf_thresh, _T("surfaceSmoothness"), TYPE_FLOAT, P_ANIMATABLE, IDS_SHARP,
		...
		p_nonLocalizedName, _T("Surface Smoothness"),
		p_end,

	ep_surf_use_riter, _T("useRenderIterations"), TYPE_BOOL, 0, IDS_USE_RITER,
		p_default, FALSE,
		p_ui, ep_subdivision, TYPE_SINGLECHECKBOX, IDC_USE_RITER,
	...
	ep_surf_riter, _T("renderIterations"), TYPE_INT, P_ANIMATABLE, IDS_RITER,
		...
		p_nonLocalizedName, _T("Render Iterations"),
		p_end,

	ep_surf_use_rthresh, _T("useRenderSmoothness"), TYPE_BOOL, 0, IDS_USE_RSHARP,
		p_default, FALSE,
		p_ui, ep_subdivision, TYPE_SINGLECHECKBOX, IDC_USE_RSHARP,
	...
	ep_surf_rthresh, _T("renderSmoothness"), TYPE_FLOAT, P_ANIMATABLE, IDS_RSHARP,
		...
		p_nonLocalizedName, _T("Render Smoothness"),
		p_end,

	ep_surf_sep_mat, _T("sepByMats"), TYPE_BOOL, 0, IDS_MS_SEP_BY_MATID,
		p_default, FALSE,
		p_ui, ep_subdivision, TYPE_SINGLECHECKBOX, IDC_MS_SEP_BY_MATID,
		p_end,

	ep_surf_sep_smooth, _T("sepBySmGroups"), TYPE_BOOL, 0, IDS_MS_SEP_BY_SMOOTH,
		p_default, FALSE,
		p_ui, ep_subdivision, TYPE_SINGLECHECKBOX, IDC_MS_SEP_BY_SMOOTH,
		p_end,
	...
	ep_sd_use, _T("useSubdivisionDisplacement"), TYPE_BOOL, 0, IDS_USE_DISPLACEMENT,
		p_default, false,
		p_ui, ep_displacement, TYPE_SINGLECHECKBOX, IDC_SD_ENGAGE,
		p_end,

	ep_sd_split_mesh, _T("displaceSplitMesh"), TYPE_BOOL, 0, IDS_SD_SPLITMESH,
		p_default, true,
		p_ui, ep_displacement, TYPE_SINGLECHECKBOX, IDC_SD_SPLITMESH,
		p_end,
	...
	ep_sd_tess_steps, _T("displaceSteps"), TYPE_INT, P_ANIMATABLE, IDS_SD_TESS_STEPS,
		...
		p_nonLocalizedName, _T("Displacement Steps"),
		p_end,
	...
	ep_sd_tess_edge, _T("displaceEdge"), TYPE_FLOAT, P_ANIMATABLE, IDS_SD_TESS_EDGE,
		...
		p_nonLocalizedName, _T("Displacement Edge"),
		p_end,
	...
	ep_sd_tess_distance, _T("displaceDistance"), TYPE_FLOAT, P_ANIMATABLE, IDS_SD_TESS_DISTANCE,
		...
		p_nonLocalizedName, _T("Displacement Distance"),
		p_end,
	...
	ep_sd_tess_angle, _T("displaceAngle"), TYPE_ANGLE, P_ANIMATABLE, IDS_SD_TESS_ANGLE,
		...
		p_nonLocalizedName, _T("Displacement Angle"),
		p_end,

	ep_sd_view_dependent, _T("viewDependent"), TYPE_BOOL, 0, IDS_SD_VIEW_DEPENDENT,
		p_default, false,
		p_ui, ep_displacement, TYPE_SINGLECHECKBOX, IDC_SD_VIEW_DEP,
		p_end,
	...
	ep_edge_chamfer, _T("edgeChamfer"), TYPE_WORLD, P_TRANSIENT, IDS_EDGE_CHAMFER_AMOUNT,
		p_default, 0.0f,
		p_range, 0.0f, 99999999.0f,
		p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_POS_UNIVERSE, IDC_EDGE_CHAMFER, IDC_EDGE_CHAMFER_SPIN, SPIN_AUTOSCALE,
		p_end,

	ep_edge_chamfer_segments, _T("edgeChamferSegments"), TYPE_INT, P_TRANSIENT, IDS_EDGE_CHAMFER_SEGMENTS,
		p_default, 1,
		p_range, 0, 999,
		p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_POS_INT, IDC_EDGE_CHAMFER_SEGMENTS, IDC_EDGE_CHAMFER_SEGMENTS_SPIN, .5f,
		p_end,
	...
	ep_ms_sep_smooth, _T("separateBySmoothing"), TYPE_BOOL, P_TRANSIENT, IDS_MS_SEP_BY_SMOOTH,
		p_default, FALSE,
		p_ui, ep_settings, TYPE_SINGLECHECKBOX, IDC_MS_SEP_BY_SMOOTH,
		p_end,

	ep_ms_sep_mat, _T("separateByMaterial"), TYPE_BOOL, P_TRANSIENT, IDS_MS_SEP_BY_MATID,
		p_default, FALSE,
		p_ui, ep_settings, TYPE_SINGLECHECKBOX, IDC_MS_SEP_BY_MATID,
		p_end,
	...
	ep_extrude_spline_node, _T("extrudeAlongSplineNode"), TYPE_INODE, P_TRANSIENT, IDS_EXTRUDE_ALONG_SPLINE_NODE,
		p_nonLocalizedName, _T("Extrude Along Spline Node"),
		p_end,
	...
	ep_extrude_spline_align, _T("extrudeAlongSplineAlign"), TYPE_BOOL, P_TRANSIENT, IDS_EXTRUDE_ALONG_SPLINE_ALIGN,
		p_default, 0,
		p_ui, ep_settings, TYPE_SINGLECHECKBOX, IDC_EXTRUDE_ALIGN_NORMAL,
	...
	ep_bridge_reverse_triangle, _T("bridgeReverseTriangle"), TYPE_INT, P_TRANSIENT, IDS_BRIDGE_REVERSETRI,
		p_default, false,
		p_ui, ep_settings, TYPE_SINGLECHECKBOX, IDC_REVERSETRI,
		p_end,
	...
	ep_edge_chamfer_mitering_type, _T("edgeChamferMiteringType"), TYPE_INT, P_TRANSIENT, IDS_EDGE_CHAMFER_MITERING_TYPE,
		p_default, EPOLY_EDGE_CHAMFER_MITER_UNIFORM,
		p_range, EPOLY_EDGE_CHAMFER_MITER_QUAD, EPOLY_EDGE_CHAMFER_MITER_PATCH,
		p_ui, ep_settings, TYPE_INTLISTBOX, IDC_EDGE_CHAMFER_MITERING_TYPE, 5, IDS_EDGE_CHAMFER_MITER_QUAD, IDS_EDGE_CHAMFER_MITER_TRI, IDS_EDGE_CHAMFER_MITER_UNIFORM, IDS_EDGE_CHAMFER_MITER_RADIAL, IDS_EDGE_CHAMFER_MITER_PATCH,
		p_vals, EPOLY_EDGE_CHAMFER_MITER_QUAD, EPOLY_EDGE_CHAMFER_MITER_TRI, EPOLY_EDGE_CHAMFER_MITER_UNIFORM, EPOLY_EDGE_CHAMFER_MITER_RADIAL, EPOLY_EDGE_CHAMFER_MITER_PATCH,
		p_end,

	ep_edge_chamfer_tension, _T("edgeChamferTension"), TYPE_FLOAT, P_TRANSIENT, IDS_EDGE_CHAMFER_TENSION,
		p_default, 1.0f,
		p_range, 0.0f, 1.0f,
		p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_POS_FLOAT, IDC_EDGE_CHAMFER_TENSION, IDC_EDGE_CHAMFER_TENSION_SPIN, SPIN_AUTOSCALE,
		p_end,

	ep_edge_chamfer_depth, _T("edgeChamferDepth"), TYPE_FLOAT, P_TRANSIENT, IDS_EDGE_CHAMFER_DEPTH, 
		p_default, 0.5f, 
		p_range, -0.5f, 1.0f, 
		p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_EDGE_CHAMFER_DEPTH, IDC_EDGE_CHAMFER_DEPTH_SPIN, SPIN_AUTOSCALE, 
		p_end,

	ep_edge_chamfer_end_bias, _T("edgeChamferEndBias"), TYPE_FLOAT, P_TRANSIENT, IDS_EDGE_CHAMFER_END_BIAS,
		p_default, 0.5f, 
		p_range, 0.0f, 1.0f,
		p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_POS_FLOAT, IDC_EDGE_CHAMFER_END_BIAS, IDC_EDGE_CHAMFER_END_BIAS_SPIN, SPIN_AUTOSCALE, 
		p_end,

	ep_edge_chamfer_radial_bias, _T("edgeChamferRadialBias"), TYPE_FLOAT, P_TRANSIENT, IDS_EDGE_CHAMFER_RADIAL_BIAS, 
		p_default, 0.0f, 
		p_range, 0.0f, 1.0f, 
		p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_POS_FLOAT, IDC_EDGE_CHAMFER_RADIAL_BIAS, IDC_EDGE_CHAMFER_RADIAL_BIAS_SPIN, SPIN_AUTOSCALE, 
		p_end,

	ep_edge_chamfer_invert, _T("edgeChamferInvert"), TYPE_BOOL, P_TRANSIENT, IDS_EDGE_CHAMFER_INVERT,
		p_default, FALSE,
		p_ui, ep_settings, TYPE_CHECKBUTTON, IDC_EDGE_CHAMFER_INVERT,
		p_end,

	ep_edge_chamfer_smooth, _T("edgeChamferSmooth"), TYPE_BOOL, P_TRANSIENT, IDS_EDGE_CHAMFER_SMOOTH,
		p_default, TRUE,
		p_ui, ep_settings, TYPE_SINGLECHECKBOX, IDC_EDGE_CHAMFER_SMOOTH, p_accessor, &theEditPolyObjectPBAccessor,
		p_end,

	ep_edge_chamfer_smooth_type, _T("edgeChamferSmoothType"),TYPE_INT, P_TRANSIENT, IDS_EDGE_CHAMFER_SMOOTH_TYPE,
		p_default, EPOLY_EDGE_CHAMFER_SMOOTH_ALL,	// Defaults to smooth all
		p_range, EPOLY_EDGE_CHAMFER_SMOOTH_ALL, EPOLY_EDGE_CHAMFER_SMOOTH_CHAMFER,
		p_ui, ep_settings, TYPE_RADIO, 2, IDC_EDGE_CHAMFER_SMOOTH_ALL, IDC_EDGE_CHAMFER_SMOOTH_CHAMFERS,
		p_end,

	ep_edge_chamfer_smooth_threshold, _T("edgeChamferSmoothThreshold"), TYPE_FLOAT, P_TRANSIENT, IDS_EDGE_CHAMFER_SMOOTH_THRESHOLD,
		p_default, 30.0f,		// 30-degree default threshold
		p_range, 0.0f, 180.0f,
		p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_POS_FLOAT, IDC_EDGE_CHAMFER_THRESHOLD, IDC_EDGE_CHAMFER_THRESHOLD_SPIN, SPIN_AUTOSCALE,
		p_end,

	ep_hard_edge_display_color, _T("hardedgedisplaycolor"), TYPE_RGBA, P_RESET_DEFAULT, IDS_HARD_EDGE_DISPLAY_COLOR,
		p_default, Point3(0.0f, 0.93f, 0.156f),		// Defaults to medium green
		p_ui, ep_geom_edge, TYPE_COLORSWATCH, IDC_HARD_COLOR,
		p_end,

	ep_hard_edge_display, 	_T("hardedgedisplay"), 		TYPE_INT, 	P_RESET_DEFAULT, 	IDS_HARD_EDGE_DISPLAY_OPTION, 
		p_default, 0,	
		p_range, 0, 1,
		p_ui, ep_geom_edge, TYPE_SINGLECHECKBOX, IDC_HARD_EDGE_DISPLAY,
		p_end,

	ep_extrude_bias, _T("extrude_bias"), TYPE_FLOAT, P_TRANSIENT, IDS_FACE_EXTRUDE_BIAS,
		p_default, 1.0f,	// Defaults to quad version
		p_range, 0.0f, 1.0f,
		p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_UNIVERSE,
			IDC_FACE_EXTRUDE_BIAS, IDC_FACE_EXTRUDE_BIAS_SPIN, SPIN_AUTOSCALE,
		p_end,

	ep_bevel_bias, _T("bevel_bias"), TYPE_FLOAT, P_TRANSIENT, IDS_BEVEL_BIAS,
		p_default, 1.0f,
		p_range, 0.0f, 1.0f,
		p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_UNIVERSE,
			IDC_BEVEL_BIAS, IDC_BEVEL_BIAS_SPIN, SPIN_AUTOSCALE,
		p_end,

	ep_vertex_chamfer_num_segments, _T("chamferVertexSegments"), TYPE_INT, P_TRANSIENT, IDS_VERTEX_CHAMFER_SEGMENTS,
        p_default, 1,
        p_range, 0, 99,
        p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_POS_INT, IDC_VERTEX_CHAMFER_SEGMENTS, IDC_VERTEX_CHAMFER_SEGMENTS_SPIN, 0.5f,
	    p_end,

	ep_vertex_chamfer_depth, _T("chamferVertexDepth"), TYPE_FLOAT, P_TRANSIENT, IDS_VERTEX_CHAMFER_DEPTH,
        p_default, 0.5f,
        p_range, -0.5f, 1.0f,
        p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_VERTEX_CHAMFER_DEPTH, IDC_VERTEX_CHAMFER_DEPTH_SPIN, SPIN_AUTOSCALE, 
	    p_end,

    ep_vertex_chamfer_smooth, _T("chamferVertexSmooth"), TYPE_BOOL, P_TRANSIENT, IDS_VERTEX_CHAMFER_SMOOTH,
        p_default, TRUE,
        p_ui, ep_settings, TYPE_SINGLECHECKBOX, IDC_VERTEX_CHAMFER_SMOOTH,
        p_end,

	ep_vertex_chamfer_smooth_type, _T("chamferVertexSmoothType"), TYPE_INT, P_TRANSIENT, IDS_VERTEX_CHAMFER_SMOOTH_TYPE,
    	p_default, EPOLY_EDGE_CHAMFER_SMOOTH_ALL,
	    p_range, EPOLY_EDGE_CHAMFER_SMOOTH_ALL, EPOLY_EDGE_CHAMFER_SMOOTH_CHAMFER,
	    p_ui, ep_settings, TYPE_RADIO, 2, IDC_VERTEX_CHAMFER_SMOOTH_ALL, IDC_VERTEX_CHAMFER_SMOOTH_CHAMFERS,
	    p_end,

    ep_vertex_chamfer_smooth_threshold, _T("chamferVertexSmoothThreshold"), TYPE_FLOAT, P_TRANSIENT, IDS_VERTEX_CHAMFER_SMOOTH_THRESHOLD,
        p_default, 30.0f,
        p_range, 0.0f, 180.0f,
        p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_POS_FLOAT, IDC_VERTEX_CHAMFER_THRESHOLD, IDC_VERTEX_CHAMFER_THRESHOLD_SPIN, SPIN_AUTOSCALE,
        p_end,

    ep_enable_enhanced_hotkey_extrude, _T("enableEnhancedHotkeyExtrude"), TYPE_BOOL, P_RESET_DEFAULT, IDS_ENABLE_ENHANCED_HOTKEY_EXTRUDE,
        p_default, TRUE,
        p_end,
    ep_enable_enhanced_retriangulation, _T("enableEnhancedRetriangulation"), TYPE_BOOL, P_RESET_DEFAULT, IDS_ENABLE_ENHANCED_RETRIANGULATION,
        p_default, TRUE,
        p_end,
...
static INT_PTR CALLBACK CurveNameDlgProc (HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND:
		...
		case IDOK:
			...
			GetWindowText(GetDlgItem(hWnd,IDC_CURVE_NAME), name->dataForWrite(), name->length()+1);
	...
	case WM_COMMAND:
		...
		case IDOK:
			...
			GetWindowText (GetDlgItem(hWnd,IDC_DETACH_NAME),
				name->dataForWrite(), name->length()+1);
			thePopupPosition.Scan (hWnd);
			EndDialog(hWnd,1);
			break;
...
void SelectDlgProc::SetEnables (HWND hSel) {
	bool fac = (editablePolyMeshSelLevel[mpEPoly->GetEPolySelLevel()] == MNM_SL_FACE);
	bool edg = (editablePolyMeshSelLevel[mpEPoly->GetEPolySelLevel()] == MNM_SL_EDGE);
    ...
    IParamBlock2* params = mpEPoly->getParamBlock();
    DbgAssert(params != nullptr);
	BOOL multiSelect = params->GetInt (ep_select_mode, TimeValue(0)) == 2;
	BOOL highlightSelect = params->GetInt (ep_select_mode, TimeValue(0)) == 1;
    ...
    but = GetICustButton (GetDlgItem(hSel, IDC_CULL_BACKFACE));
    if (but)
    {

        // Enable the backface culling selection option button, and set the state of the button, consistent with the
        // corresponding parameter block entry, unless we are in object selection mode
        but->Enable(!obj);
        if (!obj)
        {
            BOOL ignoreBackfacing = FALSE;
            Interval interval = FOREVER;
            params->GetValue(ep_ignore_backfacing, timeCurr, ignoreBackfacing, interval);
            but->SetCheck(ignoreBackfacing);
        }
    ...
    but = GetICustButton (GetDlgItem(hSel, IDC_CULL_OCCLUDED));
    if (but)
    {

        // Enable the occluded culling selection option button, and set the state of the button, consistent with the
        // corresponding parameter block entry, unless we are in object selection mode
        but->Enable(!obj);
        if (!obj)
        {
            ...
            Interval interval = FOREVER;
            params->GetValue(ep_ignore_occluded, timeCurr, ignoreOccluded, interval);
            but->SetCheck(ignoreOccluded);
        }
	...
	if (params->GetInt (ep_by_vertex)) facNotElem = false;
	...
	if (spinner) {
		bool useAngle = params->GetInt (ep_select_by_angle) != 0;
...
INT_PTR SelectDlgProc::DlgProc (TimeValue t, IParamMap2 *map, HWND hWnd,
	...
	case WM_INITDIALOG:
		...
		iToolbar->AddTool(EDITPOLY_REGULAR(CTB_CHECKBUTTON, EditPolyIcon::vertexSelection, IDC_SELVERTEX));
		iToolbar->AddTool(EDITPOLY_REGULAR(CTB_CHECKBUTTON, EditPolyIcon::edgeSelection, IDC_SELEDGE));
		iToolbar->AddTool(EDITPOLY_REGULAR(CTB_CHECKBUTTON, EditPolyIcon::borderSelection, IDC_SELBORDER));
		iToolbar->AddTool(EDITPOLY_REGULAR(CTB_CHECKBUTTON, EditPolyIcon::polygonSelection, IDC_SELFACE));
		iToolbar->AddTool(EDITPOLY_REGULAR(CTB_CHECKBUTTON, EditPolyIcon::elementSelection, IDC_SELELEMENT));
		ReleaseICustToolbar(iToolbar);
		...
		ICustButton* but = GetICustButton(GetDlgItem(hWnd, IDC_CULL_BACKFACE));
		if (but)
		{
		...
		but = GetICustButton(GetDlgItem(hWnd, IDC_CULL_OCCLUDED));
		if (but)
		{
			but->SetType(CBT_CHECK);
			ReleaseICustButton(but);
	...
	case WM_COMMAND:
		...
		case IDC_CULL_BACKFACE:
		{
			theHold.Begin();
			BOOL checkboxValue = FALSE;
			mpEPoly->getParamBlock()->GetValue(ep_ignore_backfacing, TimeValue(0), checkboxValue);
			mpEPoly->getParamBlock()->SetValue(ep_ignore_backfacing, TimeValue(0), !checkboxValue);
			theHold.Accept(GetString(IDS_PARAMETERS));
			break;
		}

		case IDC_CULL_OCCLUDED:
		{
			theHold.Begin();
			BOOL checkboxValue = FALSE;
			mpEPoly->getParamBlock()->GetValue(ep_ignore_occluded, TimeValue(0), checkboxValue);
			mpEPoly->getParamBlock()->SetValue(ep_ignore_occluded, TimeValue(0), !checkboxValue);
			theHold.Accept(GetString(IDS_PARAMETERS));
			break;
		}
		}
...
void SoftselDlgProc::DrawCurve (TimeValue t, HWND hWnd,HDC hdc) {
	...
	pblock->GetValue (ep_ss_falloff, t, falloff);
	pblock->GetValue (ep_ss_pinch, t, pinch);
	pblock->GetValue (ep_ss_bubble, t, bubble);
...
void SoftselDlgProc::SetEnables (HWND hSS) {
	...
	pblock->GetValue (ep_ss_use, TimeValue(0), softSel);
	pblock->GetValue (ep_ss_edist_use, TimeValue(0), useEdgeDist);
...
INT_PTR SoftselDlgProc::DlgProc (TimeValue t, IParamMap2 *map, HWND hWnd,
	...
	case CC_SPINNER_BUTTONUP:
		...
		int interactive;
		pEditPoly->getParamBlock()->GetValue (ep_interactive_full, TimeValue(0), interactive);
...
public:
	...
	INT_PTR DlgProc (TimeValue t, IParamMap2 *map, HWND hWnd,
		UINT msg, WPARAM wParam, LPARAM lParam);
	void MaybeEnableControls(HWND hWnd, TimeValue t);
	void InvalidateUI (HWND hParams, ParamID element);
	void DeleteThis() { }
};
...
// Luna task 748A - popup dialog
void PopupDlgProc::InvalidateUI (HWND hParams, ParamID element) {
	HWND hDlgItem;
	TSTR buf;
	...
	if (element<0) {
		...
		InvalidateUI (hParams, ep_extrusion_type);
		InvalidateUI (hParams, ep_bevel_type);
	}
	...
	case ep_lift_edge:
		...
		if (hDlgItem) {
			...
			if (ibut) {
				int liftEdge;
				mpEPoly->getParamBlock()->GetValue (ep_lift_edge, TimeValue(0), liftEdge);
				static TSTR buf;
	...
	case ep_tess_type:
		...
		if (hDlgItem) {
			int faceType;
			mpEPoly->getParamBlock()->GetValue (ep_tess_type, TimeValue(0), faceType);
	...
	case ep_extrude_spline_node:
		...
		if (hDlgItem) {
			...
			mpEPoly->getParamBlock()->GetValue (ep_extrude_spline_node, TimeValue(0), splineNode);
	...
	case ep_extrusion_type:
		hDlgItem = GetDlgItem(hParams, IDC_FACE_EXTRUDE_BIAS);
		if (hDlgItem) {
			auto extrusionType = mpEPoly->getParamBlock()->GetInt(ep_extrusion_type);
			auto localNormSelected = extrusionType == 1;
			auto *bias = GetICustEdit(hDlgItem);
			if (bias) {
				bias->Enable(localNormSelected);
				ReleaseICustEdit(bias);
			}
			auto *biasSpin = GetISpinner(GetDlgItem(hParams, IDC_FACE_EXTRUDE_BIAS_SPIN));
			if (biasSpin) {
	...
	case ep_bevel_type:
		hDlgItem = GetDlgItem(hParams, IDC_BEVEL_BIAS);
		if (hDlgItem) {
			auto bevelType = mpEPoly->getParamBlock()->GetInt(ep_bevel_type);
			...
			auto *biasSpin = GetISpinner(GetDlgItem(hParams, IDC_BEVEL_BIAS_SPIN));
			if (biasSpin) {
				biasSpin->Enable(localNormSelected);
				ReleaseISpinner(biasSpin);
			}
...
void PopupDlgProc::MaybeEnableControls(HWND hWnd, TimeValue t)

{
	...
	if (GetDlgItem(hWnd, IDC_EDGE_CHAMFER_INVERT))
	{
		...
		mpEPoly->getParamBlock()->GetValue(ep_edge_chamfer_mitering_type, t, miteringType);
		mpEPoly->getParamBlock()->GetValue(ep_edge_chamfer_smooth, t, smooth);

		bool tensionEnabled = miteringType == EPOLY_EDGE_CHAMFER_MITER_QUAD;
		bool depthEnabled = miteringType == EPOLY_EDGE_CHAMFER_MITER_UNIFORM || miteringType == EPOLY_EDGE_CHAMFER_MITER_RADIAL || miteringType == EPOLY_EDGE_CHAMFER_MITER_PATCH;
		bool enableBias = miteringType != EPOLY_EDGE_CHAMFER_MITER_TRI;

		// Legacy chamfer options
		mpEPoly->getParamBlock()->SetValue(ep_edge_chamfer_type, t, depthEnabled ? EPOLY_QUAD_EDGE_CHAMFER : EPOLY_STANDARD_EDGE_CHAMFER);
		mpEPoly->getParamBlock()->SetValue(ep_edge_chamfer_quad_intersections, t, depthEnabled);

		// Enable tension spinner when under quad mitering type.
		...
		// Enable depth spinner when under uniform mitering type.
    ...
    if (smoothingControl != NULL)
    {
        ...
        Interval interval = FOREVER;
		IParamBlock2* params = mpEPoly->getParamBlock();
        params->GetValue(ep_vertex_chamfer_smooth, t, doSmooth, interval);
...
INT_PTR PopupDlgProc::DlgProc (TimeValue t, IParamMap2 *map, HWND hWnd,
	...
	case WM_INITDIALOG:
		...
		// We handle that here:
		BOOL align;
		mpEPoly->getParamBlock()->GetValue (ep_extrude_spline_align, t, align);
	...
	case WM_COMMAND:
		...
		{
			mpEPoly->getParamBlock()->EnableNotifications(false);
			mpEPoly->getParamBlock()->SetValue(ep_edge_chamfer_invert, t, false);
			mpEPoly->getParamBlock()->EnableNotifications(true);
			mpEPoly->RefreshScreen();
		}
			break;
		...
		{
			mpEPoly->getParamBlock()->EnableNotifications(false);
			mpEPoly->getParamBlock()->SetValue(ep_edge_chamfer_open, t, false);
			mpEPoly->getParamBlock()->EnableNotifications(true);
			mpEPoly->RefreshScreen();
		}
			break;
...
void PreserveMapsUIHandler::Initialize (HWND hWnd) {
	...
	if (pMesh != NULL) {
		for (int chan=-NUM_HIDDENMAPS; chan<pMesh->MNum(); chan++) {
...
void PreserveMapsUIHandler::DoDialog () {
	DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_SETTINGS_PRESERVE_MAPS),
		GetCOREInterface()->GetMAXHWnd(), PreserveMapsSettingsDlgProc, NULL);
}
...
void GeomDlgProc::SetEnables (HWND hGeom) {
	...
	bool edg = (editablePolyMeshSelLevel[sl]==MNM_SL_EDGE) ? true : false;
	ICustButton *but = NULL;
...
BOOL SelectionModeOverride ::StartOverride()
{
	...
	mpObj->getParamBlock()->GetValue (ep_select_mode, TimeValue(0), mLastSelection);
...
BOOL ConstraintOverride::StartOverride()
{
	...
	mpObj->getParamBlock()->GetValue (ep_constrain_type, TimeValue(0), mPrevConstraint);
...
class SubobjectPickModeOverride: public CommandModeOverride
{

public:
	SubobjectPickModeOverride(EditPolyObject *m):CommandModeOverride(m,-1){}
	BOOL StartOverride();
};

BOOL SubobjectPickModeOverride::StartOverride()
{
	if(mActiveOverride==TRUE) //already TRUE, exit
		return FALSE;
	mActiveOverride = TRUE;
	mOldCommandMode = mpObj->EpActionGetCommandMode();
	mPrevPaintMode = mpObj->GetPaintMode();
	mpObj->EpActionToggleCommandMode (epmode_subobjectpick);
	mCommandMode= epmode_subobjectpick;	
	SubobjectPickMode* mode = (SubobjectPickMode*) mpObj->getCommandMode(epmode_subobjectpick);
	mode->mInOverride = true;
	return TRUE;
}

class PointToPointModeOverride : public CommandModeOverride
{

public:
	PointToPointModeOverride(EditPolyObject *m) :CommandModeOverride(m, -1) {}
	BOOL StartOverride();
};

BOOL PointToPointModeOverride::StartOverride()
{
	if (mActiveOverride == TRUE) //already TRUE, exit
		return FALSE;
	mActiveOverride = TRUE;
	mOldCommandMode = mpObj->EpActionGetCommandMode();
	mPrevPaintMode = mpObj->GetPaintMode();
	mpObj->EpActionToggleCommandMode(epmode_point_to_point);
	mCommandMode = epmode_point_to_point;
	return TRUE;
}
...
INT_PTR GeomDlgProc::DlgProc (TimeValue t, IParamMap2 *pmap, HWND hWnd,
	...
	int constrainType;
	mpEPoly->getParamBlock()->GetValue (ep_constrain_type, TimeValue(0), constrainType);
...
void SubobjControlDlgProc::SetEnables (HWND hGeom) {
	int sl = mpEPoly->GetEPolySelLevel ();
	int msl = editablePolyMeshSelLevel[sl];
	...
	but = GetICustButton (GetDlgItem (hGeom, IDC_HARD_EDGE));
	if (but != NULL) {
	...
	but = GetICustButton (GetDlgItem (hGeom, IDC_SMOOTH_EDGE));
	if (but != NULL) {
		but->SetRightClickNotify(TRUE);
		but->Enable (!brdr);
		ReleaseICustButton (but);
	...
	case EP_SL_EDGE:
		...
		case EDATA_CREASE:
			...
			defaultValue = 0.0f;
				break;
        case EDATA_DEPTH:
            spin = GetISpinner(GetDlgItem(hWnd, IDC_ES_DEPTHSPIN));
            defaultValue = 0.5f;
            break;
			}
...
INT_PTR SubobjControlDlgProc::DlgProc (TimeValue t, IParamMap2 *pmap, HWND hWnd,
	...
	case WM_INITDIALOG: 
        ...
        spin = SetupFloatSpinner(hWnd, IDC_ES_DEPTHSPIN, IDC_ES_DEPTH, -0.5f, 1.0f, 0.5f, 0.1f);
        if (spin != NULL)
        {
            spin->SetAutoScale(TRUE);
            ReleaseISpinner(spin);
	...
	case WM_PAINT:
		...
		UpdatePerDataDisplay (t, EP_SL_VERTEX, VDATA_CREASE, hWnd);
		UpdatePerDataDisplay (t, EP_SL_EDGE, EDATA_KNOT, hWnd);
		UpdatePerDataDisplay (t, EP_SL_EDGE, EDATA_CREASE, hWnd);
		UpdatePerDataDisplay(t, EP_SL_EDGE, EDATA_DEPTH, hWnd);
		mUIValid = true;
		return FALSE;
	...
	case WM_COMMAND:
		...
		case IDC_HARD_EDGE:
			if(HIWORD(wParam) == BN_RIGHTCLICK)
				mpEPoly->EpActionButtonOp (epop_select_hard_edges);
			else
				mpEPoly->EpActionButtonOp (epop_make_hard_edges);
			break;

		case IDC_SMOOTH_EDGE:
			if(HIWORD(wParam) == BN_RIGHTCLICK)
				mpEPoly->EpActionButtonOp (epop_select_smooth_edges);
			else
				mpEPoly->EpActionButtonOp (epop_make_smooth_edges);
			break;
	...
	case CC_SPINNER_BUTTONDOWN:
		...
		case IDC_VS_CREASESPIN:
			if (!mpEPoly->GetMeshPtr()->numv) break;
			mpEPoly->BeginPerDataModify (MNM_SL_VERTEX, VDATA_CREASE);
			break;
		...
		case IDC_ES_CREASESPIN:
			...
			break;
        case IDC_ES_DEPTHSPIN:
            if ((mpEPoly->GetMeshPtr())->nume != 0)
            {
                mpEPoly->BeginPerDataModify(MNM_SL_EDGE, EDATA_DEPTH);
            }
	...
	case CC_SPINNER_CHANGE:
		...
		case IDC_VS_CREASESPIN:
			if (!mpEPoly->GetMeshPtr()->numv) break;
			mpEPoly->SetVertexDataValue (VDATA_CREASE, spin->GetFVal(), MN_SEL, t);
			break;
		case IDC_ES_WEIGHTSPIN:
			if (!mpEPoly->GetMeshPtr()->nume) break;
        ...
        case IDC_ES_DEPTHSPIN:
            if ((mpEPoly->GetMeshPtr())->nume != 0)
            {
                mpEPoly->SetEdgeDataValue(EDATA_DEPTH, spin->GetFVal(), MN_SEL, t);
            }
...
void SubdivisionDlgProc::UpdateEnables (HWND hWnd, TimeValue t) {
	...
	pblock->GetValue (ep_surf_use_rthresh, t, useRSmooth);
	...
	int updateType;
	pblock->GetValue (ep_surf_update, t, updateType);
...
bool GetSelectBySmoothParams (Interface *ip, DWORD usedBits, DWORD & smG, bool & clear) {
	...
	if (DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_EP_SELECTBYSMOOTH),
		GetCOREInterface()->GetMAXHWnd(), SelectBySmoothDlgProc, (LPARAM)&buttonsToUse)) {
...
INT_PTR VertexDlgProc::DlgProc (TimeValue t, IParamMap2 *map, HWND hWnd,
	...
	case WM_PAINT:
		...
		mpEPoly->getParamBlock()->GetValue (ep_vert_color_selby, t, byIllum);
	...
	case CC_COLOR_CHANGE:
		...
		switch (LOWORD(wParam)) {
		case IDC_VERT_COLOR:
			mpEPoly->SetVertexColor (Color(iCol->GetAColor()), 0, MN_SEL, t);
			break;
		case IDC_VERT_ILLUM:
			mpEPoly->SetVertexColor (Color(iCol->GetAColor()), MAP_SHADING, MN_SEL, t);
			break;
		}
		break;
...
INT_PTR FaceDlgProc::DlgProc (TimeValue t, IParamMap2 *map, HWND hWnd,
	...
	case CC_SPINNER_BUTTONUP:
		...
		case IDC_MAT_IDSPIN:
			...
			mpEPoly->LocalDataChanged(TOPO_CHANNEL);
			mpEPoly->RefreshScreen ();
...
INT_PTR PolyColorDlgProc::DlgProc (TimeValue t, IParamMap2 *map, HWND hWnd,
	...
	case CC_COLOR_CHANGE:
		...
		switch (LOWORD(wParam)) {
		case IDC_VERT_COLOR:
			mpEPoly->SetFaceColor (Color(iCol->GetAColor()), 0, MN_SEL, t);
			break;
		case IDC_VERT_ILLUM:
			mpEPoly->SetFaceColor (Color(iCol->GetAColor()), MAP_SHADING, MN_SEL, t);
			break;
		}
		break;
...
void AdvancedDisplacementDlgProc::SetEnables (HWND hDisp) {
	...
	if (mpEPoly && mpEPoly->getParamBlock())
		mpEPoly->getParamBlock()->GetValue (ep_asd_style, TimeValue(0), style);
...
INT_PTR AdvancedDisplacementDlgProc::DlgProc (TimeValue t, IParamMap2 *map,
	...
	case WM_INITDIALOG:
		...
		if (mpEPoly && mpEPoly->getParamBlock()) {
			int min, max;
			mpEPoly->getParamBlock()->GetValue (ep_asd_min_iters, t, min);
			mpEPoly->getParamBlock()->GetValue (ep_asd_max_iters, t, max);
...
void DisplacementDlgProc::SetEnables (HWND hDisp) {
	...
	int method=0;
	if (mpEPoly && mpEPoly->getParamBlock ()) {
		mpEPoly->getParamBlock()->GetValue (ep_sd_use, TimeValue(0), useSD);
		mpEPoly->getParamBlock()->GetValue (ep_sd_method, TimeValue(0), method);
	}
...
class PolySubModSelectionProcessor: public SubModSelectionProcessor
{
protected:
	// override
	...
	virtual void OverrideDoubleClickProc(ViewExp* vpt, int flags) override {
		// double click to select a loop
		EditPolyObject* pPolyObj = dynamic_cast<EditPolyObject*>(obj);
		if (NULL != pPolyObj)
		{
			theHold.Begin();
			pPolyObj->DoubleClickSelect(vpt->ToPointer()->GetSubObjHitList().ClosestHit());
			if (pPolyObj->GetSelLevel() == MNM_SL_EDGE) 
			{
				theHold.Accept(GetString(IDS_SELECT_EDGE_LOOP));
			} 
			else
...
public:
	PolySubModSelectionProcessor(TransformModBox *mc, BaseObject* o, IObjParam* i) 
		: SubModSelectionProcessor(mc,o,i){}
};

template <class BasePolyMode, class ModBoxType>
using PolyDoubleClickModeDecorator = DoubleClickModeDecorator<BasePolyMode, PolySubModSelectionProcessor, ModBoxType>;
...
//  "Vertex Properties" or "Polygon: Material IDs"
...
void EditPolyObject::UpdateUIToSelectionLevel (int oldSelLevel) {
	...
	int oldmsl = editablePolyMeshSelLevel[oldSelLevel];

	// Otherwise, we need to get rid of all remaining rollups and replace them.
	HWND hFocus = GetFocus ();

	// Destroy old versions but only if necessary
	//  "Vertex Properties" or "Polygon: Material IDs"
	if (msl != oldmsl || (msl != MNM_SL_VERTEX && msl != MNM_SL_FACE)) {
		if (pSurface) {
			rsSurface = IsRollupPanelOpen (pSurface->GetHWnd());
	...
	if (msl != MNM_SL_FACE) {
		if (pPolySmooth) {
			rsPolySmooth = IsRollupPanelOpen (pPolySmooth->GetHWnd());
			DestroyCPParamMap2 (pPolySmooth);
			pPolySmooth = NULL;
		}
		if (pPolyColor) {
			rsPolyColor = IsRollupPanelOpen (pPolyColor->GetHWnd());
			DestroyCPParamMap2 (pPolyColor);
			pPolyColor = NULL;
		}
	}
	if (pSubobjControls) {
		rsSubobjControls = IsRollupPanelOpen (pSubobjControls->GetHWnd());
	...
	if (msl != MNM_SL_OBJECT) {
		pSubobjControls = CreateCPParamMap2 (geomIDs[selLevel], pblock, ip, hInstance,
			...
			rsSubobjControls ? 0 : APPENDROLL_CLOSED, &theSubobjControlDlgProc, NULL, ROLLUP_CAT_STANDARD + EP_ROLLUP_ORDER_SUBOBJ);
	}
	...
	if (msl != oldmsl && (msl == MNM_SL_VERTEX || msl == MNM_SL_FACE))
	{
		...
		pSurface = CreateCPParamMap2(surfIDs[msl], pblock, ip, hInstance,
			...
			rsSurface ? 0 : APPENDROLL_CLOSED, surfProcs[msl], NULL, ROLLUP_CAT_STANDARD + EP_ROLLUP_ORDER_SURF);
		
		// Required so that the controls can be queried on material changes etc. 
		// Otherwise the window handle is null.
		mtlref->hwnd = pSurface->GetHWnd();
		noderef->hwnd = pSurface->GetHWnd();
	}
	
	if (msl == MNM_SL_FACE && oldmsl != MNM_SL_FACE) {
		...
		pPolySmooth = CreateCPParamMap2(ep_face_smoothing, pblock, ip, hInstance,
			...
			rsPolySmooth ? 0 : APPENDROLL_CLOSED, &thePolySmoothDlgProc, NULL, ROLLUP_CAT_STANDARD + EP_ROLLUP_ORDER_POLYSMOOTH);
		// Polygon: Vertex Colors
		pPolyColor = CreateCPParamMap2 (ep_face_color, pblock, ip, hInstance,
			...
			rsPolyColor ? 0 : APPENDROLL_CLOSED, &thePolyColorDlgProc, NULL, ROLLUP_CAT_STANDARD + EP_ROLLUP_ORDER_VERTCOLORS);
	}
...
void EditPolyObject::InvalidateNumberSelected()
{
	...
	RECT r = { 0,0,0,0 };
	HWND label = GetDlgItem( hWnd, IDC_NUMSEL_LABEL );
	if ( label && GetClientRect( label, &r ) )
	{
	...
	theSelectDlgProc.SetNumSelLabel(hWnd);
}
...
void EditPolyObject::UpdateDisplacementParams () {
	...
	TimeValue t = GetCOREInterface()->GetTime();	// (none of these parameters should be animatable, but it's good form to use current time.)
	BOOL currentUseDisp, currentDispSplit;
	pblock->GetValue (ep_sd_use, t, currentUseDisp);
	if ((currentUseDisp && !useDisp) || (!currentUseDisp && useDisp))
		pblock->SetValue (ep_sd_use, t, useDisp);
	pblock->GetValue (ep_sd_split_mesh, t, currentDispSplit);
	...
	pblock->GetValue (ep_sd_method, t, currentMethod);
	...
	pblock->GetValue (ep_sd_tess_steps, t, currentU);
	pblock->GetValue (ep_sd_tess_edge, t, currentEdge);
	pblock->GetValue (ep_sd_tess_distance, t, currentDist);
	pblock->GetValue (ep_sd_tess_angle, t, currentAngle);
	pblock->GetValue (ep_sd_view_dependent, t, currentView);
	...
	pblock->GetValue (ep_asd_style, t, currentStyle);
	if (subdivStyle != currentStyle) pblock->SetValue (ep_asd_style, t, subdivStyle);

	int currentMinIters, currentMaxIters, currentMaxTris;
	pblock->GetValue (ep_asd_min_iters, t, currentMinIters);
	pblock->GetValue (ep_asd_max_iters, t, currentMaxIters);
	pblock->GetValue (ep_asd_max_tris, t, currentMaxTris);
...
void EditPolyObject::BeginEditParams (IObjParam  *ip, ULONG flags,Animatable *prev) {
    typedef PolySelectionProcessor<PolySubModSelectionProcessor, EditPolyObject> PolyObjectSelectionProcessor;
    typedef PolyModeSelectionProcessor<PolyObjectSelectionProcessor, EditPolyObject> PolyObjectModeSelectionProcessor;
	...
	if (selLevel != EP_SL_OBJECT) {
		...
		pSubobjControls = CreateCPParamMap2 (geomIDs[selLevel], pblock, ip, hInstance,
			MAKEINTRESOURCE (geomDlgs[selLevel]), GetString (geomDlgNames[selLevel]),
			rsSubobjControls ? 0 : APPENDROLL_CLOSED, &theSubobjControlDlgProc, NULL, ROLLUP_CAT_STANDARD + EP_ROLLUP_ORDER_SUBOBJ);
	} else pSubobjControls = NULL;

	// "Edit Geometry"
	pGeom = CreateCPParamMap2 (ep_geom, pblock, ip, hInstance,
		MAKEINTRESOURCE (IDD_EP_GEOM), GetString (IDS_EDIT_GEOM),
		rsGeom ? 0 : APPENDROLL_CLOSED, &theGeomDlgProc, NULL, ROLLUP_CAT_STANDARD + EP_ROLLUP_ORDER_GEOM);

	if (msl == MNM_SL_FACE || msl == MNM_SL_VERTEX) {
		//  "Vertex Properties" or "Polygon: Material IDs"
		pSurface = CreateCPParamMap2(surfIDs[msl], pblock, ip, hInstance,
			MAKEINTRESOURCE(surfDlgs[msl]), GetString(surfDlgNames[msl]),
			rsSurface ? 0 : APPENDROLL_CLOSED, surfProcs[msl], NULL, ROLLUP_CAT_STANDARD + EP_ROLLUP_ORDER_SURF);
	...
	if(surfIDs[msl]==ep_face)
	{
		...
		pPolyColor = CreateCPParamMap2 (ep_face_color, pblock, ip, hInstance,
			MAKEINTRESOURCE (IDD_EP_POLY_COLOR), GetString (IDS_EP_POLY_COLOR),
			rsPolyColor ? 0 : APPENDROLL_CLOSED, &thePolyColorDlgProc, NULL, ROLLUP_CAT_STANDARD + EP_ROLLUP_ORDER_VERTCOLORS);
	} else { pPolySmooth = NULL; pPolyColor = NULL; }
	...
	pSubdivision = CreateCPParamMap2 (ep_subdivision, pblock, ip, hInstance,
		...
		rsSubdivision ? 0 : APPENDROLL_CLOSED, &theSubdivisionDlgProc, NULL, ROLLUP_CAT_STANDARD + EP_ROLLUP_ORDER_SUBDIV_SURFACE);
	pDisplacement = CreateCPParamMap2 (ep_displacement, pblock, ip, hInstance,
		...
		rsDisplacement ? 0 : APPENDROLL_CLOSED, &theDisplacementProc, NULL, ROLLUP_CAT_STANDARD + EP_ROLLUP_ORDER_SUBDIV_DISP);
	pPaintDeform = CreateCPParamMap2 (ep_paintdeform, pblock, ip, hInstance,
		...
		rsPaintDeform ? 0 : APPENDROLL_CLOSED, &thePaintDeformDlgProc, NULL, ROLLUP_CAT_STANDARD + EP_ROLLUP_ORDER_PAINTDEF);
    ...
    moveMode = new DoubleClickModeDecorator<MoveModBoxCMode, PolyObjectModeSelectionProcessor, MoveModBox>(this, ip);
    baseMode = moveMode;
    hotkeyActionModes.Append(1, &baseMode);
    rotMode = new DoubleClickModeDecorator<RotateModBoxCMode, PolyObjectModeSelectionProcessor, RotateModBox>(this, ip);
    uscaleMode = new DoubleClickModeDecorator<UScaleModBoxCMode, PolyObjectModeSelectionProcessor, ScaleModBox>(this, ip);
    baseMode = uscaleMode;
    hotkeyActionModes.Append(1, &baseMode);
    nuscaleMode = new DoubleClickModeDecorator<NUScaleModBoxCMode, PolyObjectModeSelectionProcessor, ScaleModBox>(this, ip);
    baseMode = nuscaleMode;
    hotkeyActionModes.Append(1, &baseMode);
    squashMode = new DoubleClickModeDecorator<SquashModBoxCMode, PolyObjectModeSelectionProcessor, ScaleModBox>(this, ip);
    baseMode = squashMode;
    hotkeyActionModes.Append(1, &baseMode);
    selectMode = new DoubleClickModeDecorator<SelectModBoxCMode, PolyObjectSelectionProcessor, SelectModBox>(this, ip);

    // Set up the selection processors for the appropriate modes to perform an extrude when the shift hotkey combination is
    // used while dragging, moving cloning to the control-plus-shift hotkey; note that for vertex selections, we retain the
    // clone operation for both hotkey combinations, while for face selections, we employ the postprocessed form of the extrude
    // operation
    int dummyVal = 0;
    int nHotkeyActionModes = hotkeyActionModes.Count();
    for (int mode = 0; mode != nHotkeyActionModes; ++mode)
    {
        PolyObjectModeSelectionProcessor* selectionProcessor = static_cast<PolyObjectModeSelectionProcessor*>((hotkeyActionModes[mode])->MouseProc(&dummyVal));
        DbgAssert(selectionProcessor != nullptr);
        selectionProcessor->SetHotkeyAction(PolyObjectModeSelectionProcessor::POLY_HOTKEY_SHIFT, PolyObjectModeSelectionProcessor::POLY_ACTION_EXTRUDE, GetString(IDS_EXTRUDE));
        selectionProcessor->SetHotkeyAction(PolyObjectModeSelectionProcessor::POLY_HOTKEY_CTRL_SHIFT, PolyObjectModeSelectionProcessor::POLY_ACTION_CLONE, GetString(IDS_CLONE));
        selectionProcessor->SetHotkeyAction(PolyObjectModeSelectionProcessor::POLY_HOTKEY_SHIFT, PolyObjectModeSelectionProcessor::POLY_ACTION_CLONE, GetString(IDS_CLONE), MNM_SL_VERTEX);
        selectionProcessor->SetHotkeyAction(PolyObjectModeSelectionProcessor::POLY_HOTKEY_SHIFT, PolyObjectModeSelectionProcessor::POLY_ACTION_EXTRUDE_POSTPROCESSED, GetString(IDS_EXTRUDE), MNM_SL_FACE);
    }

    // We have no extrude action in Rotate mode, but we want to support cloning with the control-plus-shift hotkey for
    // consistency with move and scale modes; we will assign the clone action to the shift hotkey as well
    PolyObjectModeSelectionProcessor* rotProcessor = static_cast<PolyObjectModeSelectionProcessor*>(rotMode->MouseProc(&dummyVal));
    DbgAssert(rotProcessor != nullptr);
    rotProcessor->SetHotkeyAction(PolyObjectModeSelectionProcessor::POLY_HOTKEY_SHIFT, PolyObjectModeSelectionProcessor::POLY_ACTION_CLONE, GetString(IDS_CLONE));
    rotProcessor->SetHotkeyAction(PolyObjectModeSelectionProcessor::POLY_HOTKEY_CTRL_SHIFT, PolyObjectModeSelectionProcessor::POLY_ACTION_CLONE, GetString(IDS_CLONE));
	...
	subobjectPickMode = new SubobjectPickMode(this,ip);
	pointToPointMode = new PointToPointMode(this,ip);
	...
	if(actionOverrideMan)
	{
		...
		mOverrides.ZeroCount(); //is this a memory leak?
...
*/
		...
		//subobject pick mode
		item = pTable->GetAction(ID_EP_SUBOBJECT_PICK);
		mOverrides.Append(1,&item);
		SubobjectPickModeOverride *subobjectModeOverride = new SubobjectPickModeOverride(this);
		actionOverrideMan->ActivateActionItemOverride(item,subobjectModeOverride);

		item = pTable->GetAction(ID_EP_POINT_TO_POINT);
		mOverrides.Append(1, &item);
		PointToPointModeOverride *pointToPointModeOverride = new PointToPointModeOverride(this);
		actionOverrideMan->ActivateActionItemOverride(item, pointToPointModeOverride);
...
void EditPolyObject::EndEditParams (IObjParam *ip, ULONG flags,Animatable *next) {
	...
	editSSMode = NULL;
	if(subobjectPickMode) delete subobjectPickMode;
	subobjectPickMode = NULL;
	if(pointToPointMode) delete pointToPointMode;
	pointToPointMode = NULL;
	...
	if (pSelect) {
		rsSelect = IsRollupPanelOpen (pSelect->GetHWnd());
		DestroyCPParamMap2 (pSelect);
		pSelect = NULL;
	}
	if (pSoftsel) {
		rsSoftsel = IsRollupPanelOpen (pSoftsel->GetHWnd());
		DestroyCPParamMap2 (pSoftsel);
		pSoftsel = NULL;
	}
	if (pGeom) {
		rsGeom = IsRollupPanelOpen (pGeom->GetHWnd());
	...
	if (pSubdivision) {
		rsSubdivision = IsRollupPanelOpen (pSubdivision->GetHWnd());
		DestroyCPParamMap2 (pSubdivision);
		pSubdivision = NULL;
	}
	if (pDisplacement) {
		rsDisplacement = IsRollupPanelOpen (pDisplacement->GetHWnd());
		DestroyCPParamMap2 (pDisplacement);
		pDisplacement = NULL;
	}
	if (pPaintDeform) {
		rsPaintDeform = IsRollupPanelOpen (pPaintDeform->GetHWnd());
...
void EditPolyObject::ExitPickLiftEdgeMode () {
	if (pickLiftEdgeMode) GetCOREInterface()->DeleteMode (pickLiftEdgeMode);
}

void EditPolyObject::ExitPickBridgeModes () {
	if (pickBridgeTarget1) GetCOREInterface()->DeleteMode (pickBridgeTarget1);
	if (pickBridgeTarget2) GetCOREInterface()->DeleteMode (pickBridgeTarget2);
}

void EditPolyObject::ExitAllCommandModes (bool exSlice, bool exStandardModes) {
	if( InPaintMode() ) EndPaintMode();

	if (createVertMode) GetCOREInterface()->DeleteMode (createVertMode);
	if (createEdgeMode) GetCOREInterface()->DeleteMode (createEdgeMode);
	if (createFaceMode) GetCOREInterface()->DeleteMode (createFaceMode);
	if (divideEdgeMode) GetCOREInterface()->DeleteMode (divideEdgeMode);
	if (divideFaceMode) GetCOREInterface()->DeleteMode (divideFaceMode);
	if (extrudeMode) GetCOREInterface()->DeleteMode (extrudeMode);
	if (extrudeVEMode) GetCOREInterface()->DeleteMode (extrudeVEMode);
	if (chamferMode) GetCOREInterface()->DeleteMode (chamferMode);
	if (bevelMode) GetCOREInterface()->DeleteMode (bevelMode);
	if (insetMode) GetCOREInterface()->DeleteMode (insetMode);	// Luna task 748R
	if (outlineMode) GetCOREInterface()->DeleteMode (outlineMode);
	if (extrudeVEMode) GetCOREInterface()->DeleteMode (extrudeVEMode);
	if (cutMode) GetCOREInterface()->DeleteMode (cutMode);	// Luna task 748D
	if (quickSliceMode) GetCOREInterface()->DeleteMode (quickSliceMode);	// Luna task 748J
	if (weldMode) GetCOREInterface()->DeleteMode (weldMode);
	if (liftFromEdgeMode) GetCOREInterface()->DeleteMode (liftFromEdgeMode);	// Luna task 748P 
	if (pickLiftEdgeMode) GetCOREInterface()->DeleteMode (pickLiftEdgeMode);	// Luna task 748P 
	if (editTriMode) GetCOREInterface()->DeleteMode (editTriMode);
	if (turnEdgeMode) GetCOREInterface()->DeleteMode (turnEdgeMode);
	if (bridgeBorderMode) GetCOREInterface()->DeleteMode (bridgeBorderMode);
	if (bridgeEdgeMode) GetCOREInterface()->DeleteMode (bridgeEdgeMode);
	if (bridgePolyMode) GetCOREInterface()->DeleteMode (bridgePolyMode);
	if (pickBridgeTarget1) GetCOREInterface()->DeleteMode (pickBridgeTarget1);
	if (pickBridgeTarget2) GetCOREInterface()->DeleteMode (pickBridgeTarget2);
	if (editSSMode) GetCOREInterface()->DeleteMode(editSSMode);
	if (subobjectPickMode) GetCOREInterface()->DeleteMode (subobjectPickMode);
	if (pointToPointMode) GetCOREInterface()->DeleteMode (pointToPointMode);
	if (exStandardModes) {
		// remove our SO versions of standardmodes:
		if (moveMode) GetCOREInterface()->DeleteMode (moveMode);
		if (rotMode) GetCOREInterface()->DeleteMode (rotMode);
		if (uscaleMode) GetCOREInterface()->DeleteMode (uscaleMode);
		if (nuscaleMode) GetCOREInterface()->DeleteMode (nuscaleMode);
		if (squashMode) GetCOREInterface()->DeleteMode (squashMode);
		if (selectMode) GetCOREInterface()->DeleteMode (selectMode);
	}
...
// Luna task 748Y - UI redesign
void EditPolyObject::InvalidateDialogElement (ParamID elem) {
...
void EditPolyObject::UpdatePerDataDisplay (TimeValue t, int msl, int channel) {
	...
	if (msl != editablePolyMeshSelLevel[selLevel]) return;
	ResetGripUI();
...
void EditPolyObject::ApplyPaintSel( MeshPaintHost* host ) {
	...
	GetMesh().SetFlag(MN_MESH_SOFTSELECTION_WEIGHT_INVALID);
	GetMesh().InvalidateHardwareMesh(1);
}
...
void EditPolyObject::ApplyPaintDeform( MeshPaintHost* host,  BitArray *partialRevert ) {
	...
	if (partialRevert)
	{
		IMNMeshUtilities8 *mnInterface = mesh.GetTypedInterface<IMNMeshUtilities8>();
...
void EditPolyObject::OnParamSet(PB2Value& v, ParamID id, int tabIndex, TimeValue t) {
	...
	case ep_ss_lock:	{
			...
			if (GetCOREInterface()->GetCommandPanelTaskMode () == TASK_MODE_MODIFY)  //the paint activation requires the modify panel since it does an enum mod context
			{
	...
	case ep_paintdeform_mode:
		...
		break;
	case ep_ignore_backfacing:
	{
		// When we enable ignore back facing, we disable ignore occluded
		if (v.i)
			pblock->SetValue(ep_ignore_occluded, t, !v.i);

		// Make sure our button's state reflects its status
		HWND hwnd = pSelect != nullptr ? pSelect->GetHWnd() : nullptr;
		ICustButton* pBut = GetICustButton(GetDlgItem(hwnd, IDC_CULL_BACKFACE));
		if (pBut != nullptr)
		{
			pBut->SetCheck(v.i);
	...
	case ep_ignore_occluded:
	{
		// When we enable ignore occluded, we disable ignore back facing
		if (v.i)
			pblock->SetValue(ep_ignore_backfacing, t, !v.i);

		// Make sure our button's state reflects its status
		HWND hwnd = pSelect != nullptr ? pSelect->GetHWnd() : nullptr;
		ICustButton* pBut = GetICustButton(GetDlgItem(hwnd, IDC_CULL_OCCLUDED));
		if (pBut != nullptr)
		{
			pBut->SetCheck(v.i);
			ReleaseICustButton(pBut);
...
const TCHAR *EditPolyObject::GetPaintUndoString (PaintModeID mode) {
...
void EditPolyObject::SmGrpFloater()
{
	...
	else if (this->ip != nullptr)
	{
		// only show this floater when BeginEditParam is called, or the floater is useless even we show it.
...
bool EditPolyObject::GetManipulateGrip(EPoly13::ManipulateGrips val)
{
    ...
    DbgAssert(val < EPoly13::EPolyManipulatorGripCount);
    if (val < EPoly13::EPolyManipulatorGripCount)
    {
...
void EditPolyObject::SetManipulateGrip(bool on, EPolyManipulatorGrip::ManipulateGrips val)
{
	SetUpManipGripIfNeeded();
}

bool EditPolyObject::GetManipulateGrip(EPolyManipulatorGrip::ManipulateGrips val)
{
...
void EditPolyObject::SetUpManipGripIfNeeded()
{
	...
	if(GetCOREInterface7()->InManipMode()==TRUE )
	{
		if(GetIGripManager()->GetActiveGrip() == &mEditablePolyManiGrip)
		{
			...
			{
				// For the manipulate grip, the grip setup is through GetIGripManager()->SetGripActive(IBaseGrip *grip) function ,
				// so SetGripActive(IBaseGrip *grip) is corresponding with SetGripsInactive() which closes the grips.
				// But for the editable poly grip and edit poly modifier Grip ,the grips setup is through ShowGrip function, 
				// so ShowGrip function is corresponding with HideGrip() function for these two types grips when close the grips.
				GetIGripManager()->SetGripsInactive();
			}
			else
			{
...
void EditPolyObject::SetUpManipGripVisibility()
{
		...
		GetIGripManager()->GetActiveGrip() == &mEditablePolyManiGrip)
	{
        ...
        {
            mEditablePolyManiGrip.SetUpVisibility((BOOL)(copy[g]), g);
        }
	}
}
