...
void MeshPaintMgr::PaintStroke_Deform( PaintStrokeContext* stroke ) {
	...
	int extraAlloc = (int)(stroke->pointGatherCount * 0.2f);
...
IOResult MeshPaintHost::Load(ILoad *iload) {
	...
	while (IO_OK==(res=iload->OpenChunk())) {
		...
		case PAINT_SEL_BUF_CHUNK:
			paintSelCount = (int)iload->CurChunkLength() / sizeof(float);
		...
		case PAINT_SEL_DEFORM_CHUNK:
			paintDeformCount = (int)iload->CurChunkLength() / sizeof(Point3);
...
const TCHAR* MeshPaintHostRestore::GetName() {
...
const TCHAR* MeshPaintPresetContext::GetParamName( int paramID ) {
...
const MCHAR* MeshPaintPresetContext::GetDisplayParamName( int paramID ) {
	int index = GetParamIndex(paramID);
	if( index<0 ) return NULL;
...
const TCHAR* PaintDeformPresetContext::GetContextName() {
...
const TCHAR* PaintSoftSelPresetContext::GetContextName() {
...
FPValue PaintDeformPresetParams::GetDisplayParamValue( int paramID ) {
	...
	switch( paramID ) {
	case 1:
		if( mode==PAINTMODE_PAINTPUSHPULL )		text = MaxSDK::GetResourceStringAsMSTR(IDS_PAINTDEFORM_PUSHPULL);
		else if( mode==PAINTMODE_PAINTRELAX )	text = MaxSDK::GetResourceStringAsMSTR(IDS_PAINTDEFORM_RELAX);
		else if( mode==PAINTMODE_ERASEDEFORM )	text = MaxSDK::GetResourceStringAsMSTR(IDS_PAINTDEFORM_ERASE);
		break;
	case 2:
		if( mode==PAINTMODE_PAINTPUSHPULL ) {
			if( axis==PAINTAXIS_DEFORMEDNORMS )	text = MaxSDK::GetResourceStringAsMSTR(IDS_PAINTDEFORM_AXIS_DEFORMEDNORMS);
			else if( axis==PAINTAXIS_ORIGNORMS ) text = MaxSDK::GetResourceStringAsMSTR(IDS_PAINTDEFORM_AXIS_ORIGNORMS);
			else if( axis==PAINTAXIS_XFORM_X )	text = MaxSDK::GetResourceStringAsMSTR(IDS_PAINTDEFORM_AXIS_XFORM_X);
			else if( axis==PAINTAXIS_XFORM_Y )	text = MaxSDK::GetResourceStringAsMSTR(IDS_PAINTDEFORM_AXIS_XFORM_Y);
			else if( axis==PAINTAXIS_XFORM_Z )	text = MaxSDK::GetResourceStringAsMSTR(IDS_PAINTDEFORM_AXIS_XFORM_Z);
		}
		break;
	case 3:
		if( mode==PAINTMODE_PAINTPUSHPULL ) {
			text = FormatUniverseValue(value);
		}
		break;
	}
...
FPValue PaintSoftSelPresetParams::GetDisplayParamValue( int paramID ) {
	...
	switch( paramID ) {
	case 1:
		if( mode==PAINTMODE_PAINTSEL )			text = MaxSDK::GetResourceStringAsMSTR(IDS_PAINTSEL_PAINT);
		else if( mode==PAINTMODE_ERASESEL )		text = MaxSDK::GetResourceStringAsMSTR(IDS_PAINTSEL_ERASE);
		else if( mode==PAINTMODE_BLURSEL )		text = MaxSDK::GetResourceStringAsMSTR(IDS_PAINTSEL_BLUR);
		break;
	case 2:
		if( mode==PAINTMODE_PAINTSEL ) {
			text.printf(_T("%f"), value);
		}
		break;
	}
