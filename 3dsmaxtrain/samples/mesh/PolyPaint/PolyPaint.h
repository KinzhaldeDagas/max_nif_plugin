...
class ResourceString
{
	public:
		...
		{
			if( strBuf[0]==0 ) LoadString(MaxSDK::GetHInstance(), strID, strBuf, _countof(strBuf));
			return strBuf;
		}
};
...
class DllExport MeshPaintHandler {
	public:
		...
		virtual const TCHAR *GetPaintUndoString (PaintModeID mode) = 0;
...
template class DllExport Tab<float>; // This declaration prevents warning C4251
template class DllExport Tab<Point3>; // This declaration prevents warning C4251

class DllExport MeshPaintHost {
	public:
		//-- Implemented by the system
		MeshPaintHost();
		~MeshPaintHost();
	...
	public ReferenceTarget // Must derive from ReferenceTarget to be a PainterCanvas
{
	...
	public:
		...
		void				DeleteThis();
		virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("MeshPaintMgr"); }
		void				Init();
		void				Free();
...
DllExport MeshPaintMgr* GetMeshPaintMgr();
...
class MeshPaintPresetContext : public IBrushPresetContext, public FPStaticInterface {
	public:
		...
		const MCHAR*				GetParamName( int paramID );
