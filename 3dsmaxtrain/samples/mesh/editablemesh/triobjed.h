...
class EditTriObject : public TriObject, public EventUser, public ISubMtlAPI, public IMeshSelect,
								...
								public AttachMatDlgUser, public EditSSCB {
...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_SCA_BASE_MESH) : _T("Editable Mesh");}
	void BeginEditParams(IObjParam  *ip, ULONG flags,Animatable *prev);
	void EndEditParams(IObjParam *ip, ULONG flags,Animatable *next);
	int NumSubs() { return 1; }		// only the Point Controller Container.
...
public:
    ...
    int GetSubObjectLevel() override { return GetSubobjectLevel(); }
	...
	// Object methods		
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_SCA_BASE_MESH) : _T("Editable Mesh");}
	...
	//canUndo - means the attach is support the undo, for some reason with material it looks like it 
	//gets into cases were the material cannot be undo
	//holdGeom - means that the base geometry will be held.  This is needed when doing multiple
	//attaches.  In this case we only need to hold the inital geometry of the first attach since
	//when we undo we want to go back to that state.  This flag determines if the base geom gets held
	void Attach (INode *node, bool & canUndo, bool holdGeom);
	...
	BitArray GetVertSel() override { return GetMesh().VertSel(); }
	BitArray GetFaceSel() override { return GetMesh().FaceSel(); }
	BitArray GetEdgeSel() override { return GetMesh().EdgeSel(); }
...
protected:
	...
	TSTR GetFailedFaceCreationTitle() {return GetString(IDS_SCA_BASE_MESH);}
	...
	void DisplayDataChanged() {m_ETri->LocalDataChanged(DISP_ATTRIB_CHANNEL);}

	void ShowExistingVertexCursor() { SetCursor(GetInterface()->GetSysCursor(SYSCUR_SELECT));}
	void ShowCreateVertexCursor();

public:
	CreateFaceMouseProc(EditTriObject* in_e, IObjParam* in_i);
...
public:
	...
	const TCHAR *buttonText() { return GetString(IDS_RB_ATTACH); }
...
public:
	...
	int proc(HWND hwnd, int msg, int point, int flags, IPoint2 m );
	void DrawCutter (HWND hWnd,IPoint2 &m, bool bErase, bool bPresent);
};
...
public:
	...
	MeshTopoRestore(EditTriObject *et, ChannelMask chan);
	bool After ();
	void Restore(int isUndo);
	void Redo();
	void EndHold() {et->ClearAFlag(A_HELD);}
...
public:
	HWND hwnd;
	EditTriObject *eo;
	SingleRefMakerMeshMtl() {hwnd = NULL; eo = NULL;}
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("SingleRefMakerMeshMtl"); }
};
...
public:
	HWND hwnd;
	EditTriObject *eo;
	SingleRefMakerMeshNode() {hwnd = NULL; eo = NULL;}
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("SingleRefMakerMeshNode"); }
};
