...
static GenSubObjType SOT_Vertex(NULL,EditableMeshIcon::stackVertex,0);
static GenSubObjType SOT_Edge(NULL, EditableMeshIcon::stackEdge, 0);
static GenSubObjType SOT_Face(NULL, EditableMeshIcon::stackFace, 0);
static GenSubObjType SOT_Poly(NULL, EditableMeshIcon::stackPolygon, 0);
static GenSubObjType SOT_Element(NULL, EditableMeshIcon::stackElement, 0);
...
void EditTriObject::InvalidateNumberSelected () 
{
	...
	RECT r = { 0,0,0,0 };
	HWND label = GetDlgItem( hSel, IDC_NUMSEL_LABEL );
	if ( label && GetClientRect( label, &r ) )
	{
		InvalidateRect( hSel, &r, FALSE );
	}
	else
	{
...
void EditTriObject::SetNumSelLabel() {	
	...
	case SL_VERTEX:
		...
		if (num==1) {
			...
			for (j=0; j<vertSel.GetSize(); j++) if (vertSel[j]) break;
	...
	case SL_ELEMENT:
		...
		if (num==1) {
			...
			for (j=0; j<faceSel.GetSize(); j++) if (faceSel[j]) break;
	...
	case SL_EDGE:
		...
		if (num==1) {
			...
			for (j=0; j<edgeSel.GetSize(); j++) if (edgeSel[j]) break;
...
void EditTriObject::SetSelDlgEnables() {
	...
	EnableWindow (GetDlgItem (hSel, IDC_DELETE_ISOLATED_VERTS), poly||fac||edg);
...
static INT_PTR CALLBACK SelectDlgProc (HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
   EditTriObject *eo = DLGetWindowLongPtr<EditTriObject *>(hWnd);
	...
	case WM_INITDIALOG:
	{
      ...
      DLSetWindowLongPtr(hWnd, lParam);         
		...
		ReleaseISpinner( SetupFloatSpinner(hWnd, IDC_NORMAL_SCALESPIN, IDC_NORMAL_SCALE, 0.01f, 999999999.0f, eo->normScale, 0.1f) );
		...
		iToolbar->AddTool(EDITABLEMESH_REGULAR(CTB_CHECKBUTTON,EditableMeshIcon::vertexSelection,IDC_SELVERTEX));
		iToolbar->AddTool(EDITABLEMESH_REGULAR(CTB_CHECKBUTTON,EditableMeshIcon::edgeSelection,IDC_SELEDGE));
		iToolbar->AddTool(EDITABLEMESH_REGULAR(CTB_CHECKBUTTON,EditableMeshIcon::faceSelection,IDC_SELFACE));
		iToolbar->AddTool(EDITABLEMESH_REGULAR(CTB_CHECKBUTTON,EditableMeshIcon::polygonSelection,IDC_SELPOLY));
		iToolbar->AddTool(EDITABLEMESH_REGULAR(CTB_CHECKBUTTON,EditableMeshIcon::elementSelection,IDC_SELELEMENT));
		ReleaseICustToolbar(iToolbar);
		eo->RefreshSelType();
		...
		CheckDlgButton (hWnd, IDC_DELETE_ISOLATED_VERTS, eo->deleteIsolatedVerts);
		break;
	}
	...
	case WM_COMMAND:
		...
		case IDC_DELETE_ISOLATED_VERTS:
			eo->deleteIsolatedVerts = IsDlgButtonChecked (hWnd, IDC_DELETE_ISOLATED_VERTS)?true:false;
			break;
...
INT_PTR CALLBACK AffectRegionDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
   EditTriObject *em = DLGetWindowLongPtr<EditTriObject*>(hWnd);
	ISpinnerControl *spin;
	Rect rect;
...
static INT_PTR CALLBACK GeomDlgProc (HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
   EditTriObject *eo = DLGetWindowLongPtr<EditTriObject*>(hWnd);
	...
	switch (msg) {
	case WM_INITDIALOG:
		if (!EditTriObject::weldBoxSizeInitialized)
		{
			EditTriObject::weldBoxSizeInitialized = TRUE;

			ViewExp22& vpt = static_cast<ViewExp22&>(GetCOREInterface()->GetActiveViewExp());
			EditTriObject::pickBoxSize = EditTriObject::weldBoxSize = vpt.GetPickingSelectionRadius();
		}
...
static INT_PTR CALLBACK SurfaceDlgProc (HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
   EditTriObject *eo = DLGetWindowLongPtr<EditTriObject*>(hWnd);
	...
	case WM_INITDIALOG:
      ...
      DLSetWindowLongPtr(hWnd, lParam);
		...
		if (GetDlgItem (hWnd, IDC_SMOOTH_GRP1)) {	// Face stuff:
			...
			ReleaseISpinner( SetupIntSpinner (hWnd, IDC_MAT_IDSPIN, IDC_MAT_ID, 1, MAX_MATID, 1) );
			ReleaseISpinner( SetupIntSpinner (hWnd, IDC_MAT_IDSPIN_SEL, IDC_MAT_ID_SEL, 1, MAX_MATID, 1) );
			
			CheckDlgButton(hWnd, IDC_CLEARSELECTION, 1);                                 
			SetupMtlSubNameCombo (hWnd, eo); 

			ReleaseISpinner( SetupFloatSpinner (hWnd, IDC_SMOOTH_THRESHSPIN, IDC_SMOOTH_THRESH, 0.0f, 180.0f, autoSmoothThresh, 0.1f) );
		...
		if (GetDlgItem (hWnd, IDC_VERT_SELRSPIN)) {	// Vertex stuff:
			...
			ReleaseISpinner( SetupIntSpinner (hWnd, IDC_VERT_SELRSPIN, IDC_VERT_SELR, 0, 255, selDeltaR) );
			ReleaseISpinner( SetupIntSpinner (hWnd, IDC_VERT_SELGSPIN, IDC_VERT_SELG, 0, 255, selDeltaG) );
			ReleaseISpinner( SetupIntSpinner (hWnd, IDC_VERT_SELBSPIN, IDC_VERT_SELB, 0, 255, selDeltaB) );
		...
		if (GetDlgItem (hWnd, IDC_ANGLETHRESH)) {	// Edge stuff:
			ReleaseISpinner( SetupFloatSpinner (hWnd, IDC_ANGLETHRESHSPIN, IDC_ANGLETHRESH, 0.f, 180.f, autoEdgeThresh, 0.1f) );
	...
	case WM_PAINT:
		// TODO ColorManagement: setting up a new ColorSwatch for every WM_PAINT
		// call is both dangerous and inefficient. Swatch handling here needs to
		// be re-worked. /coz 2024-02-01
	...
	case CC_COLOR_CHANGE:
		...
		switch (LOWORD(wParam)) {
		case IDC_VERT_COLOR:
			if (eo->selLevel == SL_VERTEX) eo->SetVertColor (Color(iCol->GetAColor()));
			else eo->SetFaceColor (Color(iCol->GetAColor()));
			break;
		case IDC_VERT_ILLUM:
			if (eo->selLevel == SL_VERTEX) eo->SetVertColor (Color(iCol->GetAColor()), MAP_SHADING);
			else eo->SetFaceColor (Color(iCol->GetAColor()), MAP_SHADING);
			break;
	...
	case CC_SPINNER_BUTTONUP:
		...
		case IDC_MAT_IDSPIN:
			...
			if (HIWORD(wParam) || msg==WM_CUSTEDIT_ENTER) {
				...
				}  //mz fix for 963711.  We would call accept without doing a begin. ssr will be NULL if begin isn't called.
			} else {
...
ActionTable* GetEMeshActions()
{
	...
	if (i<NUM_ACTIONS) {
		DebugPrint (_T("EMesh: Please match SDK action ID's with the resource ID's in Editable Mesh.\n"));
		DbgAssert(0);
	}
#endif
...
void EditTriObject::SetUIParam (meshUIParam uiCode, float val) {
	...
	case MuiDeleteIsolatedVerts:
		...
		CheckDlgButton (hSel, IDC_DELETE_ISOLATED_VERTS, deleteIsolatedVerts);
		break;
	}
}
...
void EditTriObject::ToggleCommandMode(meshCommandMode mode) {
	...
	case McmAttach:
		...
		if (ip->GetCommandMode() == editSSMode) ip->DeleteMode(editSSMode);
	...
	case McmEditSoftSelection:
		if (ip->GetCommandMode()==editSSMode)
			ip->DeleteMode(editSSMode);
		else ip->PushCommandMode(editSSMode);
		break;
	}
}
