...
void EditTriObject::Transform (TimeValue t, Matrix3& partm, Matrix3 tmAxis, 
	...
	if ((numAxis==NUMAXIS_INDIVIDUAL && selLevel != SL_VERTEX) && useClusterTransform) {
...
void EditTriObject::HideSelectedFaces() {
	...
	for (int i=0; i<GetMesh().getNumFaces(); i++) {
		if (faceSel[i]) GetMesh().faces[i].Hide();
	}
...
void EditTriObject::DeleteSelected() {
	...
	case SL_EDGE:
	{
		...
		for (i=0; i<GetMesh().getNumFaces(); i++) {
			for (j=0; j<3; j++) if (edgeSel[i*3+j]) break;
			if (j==3) continue;
			fset.Set(i);
		...
		if (iso.NumberSet()) {
			if( deleteIsolatedVerts ) tmd.VDelete (iso);
		}
...
void EditTriObject::Attach (INode *node, bool & canUndo, bool holdGeom) {
	...
	if (clearMapChannel)
		GetMesh().SetFlag(MESH_CLEARMAPCHANNEL);
	else
		GetMesh().ClearFlag(MESH_CLEARMAPCHANNEL);
...
int __cdecl cmpDepth( const void *arg1, const void *arg2 )
	{
	INode *n1 = *(INode **)arg1;
	INode *n2 = *(INode **)arg2;
	int d1 = n1->GetNodeLong();
	int d2 = n2->GetNodeLong();
	if (d1 > d2)
		return -1;
	if (d2 > d1)
		return 1;
...
static BOOL AppendSystemNodes(INode *attachNode, INodeTab &nodeTab);
static BOOL nodeInTab(INodeTab &nodeTab, INode *node);
static BOOL reftargInTab(Tab<ReferenceTarget *> &nodeTab, ReferenceTarget *node);

static BOOL reftargInTab(Tab<ReferenceTarget *> &itemTab, ReferenceTarget *item)
{
	for (int i=0; i<itemTab.Count(); i++)
		if (item == itemTab[i]) return true;
	return false;
}
...
static BOOL nodeInTab(INodeTab &nodeTab, INode *node)
{
	for (int i=0; i<nodeTab.Count(); i++)
		if (node == nodeTab[i]) return true;
	return false;
}

static BOOL AppendSystemNodes(INode *attachNode, INodeTab &nodeTab)
{
    Tab<ReferenceTarget *> driverTab;
	INodeTab sysTab,finalNodeTab;

	for (int i=0; i<nodeTab.Count(); i++)
	{
		Control *tmCont = nodeTab[i]->GetTMController();
		ReferenceTarget *driver = GetDriverController(tmCont);
		if (driver)
		{
			if (reftargInTab(driverTab,driver)) continue; // already got these system nodes
			driverTab.Append(1,&driver); // add driver to tab of processed drivers
			// add entire system to final node tab
		    driver->GetSystemNodes(sysTab, kSNCDelete);
	        finalNodeTab.Resize(finalNodeTab.Count()+sysTab.Count());
			for (int j=0; j<sysTab.Count(); j++) {
				if (sysTab[j] != NULL) {
					finalNodeTab.Append(1,&(sysTab[j]));
				}
			}
		}
		else finalNodeTab.Append(1,&(nodeTab[i])); // node not from a system, so just add it to final node tab
	}
	if (nodeInTab(finalNodeTab,attachNode)) return false; // do not attach to self - most likely user will attach a system to self
	nodeTab = finalNodeTab;
...
void EditTriObject::MultiAttach (INodeTab &nodeTab) {
	...
	// Below may be necessary for systems (definitely biped)
	// Presort so that delete all children before their parent.
	// This avoids some problems when deleting multiple objects, some of
	// which may share transform controllers with other objects.
	// depth info added 030320  --prs.
	for (int i=0; i<nodeTab.Count(); i++) {
		...
		nodeTab[i]->SetNodeLong(depth);
		}
	nodeTab.Sort(cmpDepth);
	...
	{
		if (i == 0)
			Attach(nodeTab[i], canUndo,true);
		else
			Attach(nodeTab[i], canUndo,false);
	}
...
void MakeSelVertsPlanar (MeshDelta & md, Mesh & m, BitArray & sel, float *vsw) {
	...
	for (i=0; i<numf; i++) {
      ...
      int j;
		for (j=0; j<3; j++) if (sel[vv[j]]) break;
		if (j==3) continue;
...
void MakeSelFacesPlanar (MeshDelta & md, Mesh & m, BitArray & fsel, float *vsw) {
	...
	const BitArray& faceSel = m.FaceSel();
...
void EditTriObject::MakePlanar () {
	...
	case SL_EDGE:
	{
		...
		for (j=0; j<GetMesh().numFaces; j++) {
			for (k=0; k<3; k++) if (edgeSel[j*3+k]) break;
			if (k==3) continue;
			tempSel.Set (GetMesh().faces[j].v[k]);
			tempSel.Set (GetMesh().faces[j].v[(k+1)%3]);
			if ((k<2) && (edgeSel[j*3+k+1] || edgeSel[j*3+(k+2)%3]))
...
float EditTriObject::GetAlpha (int mp, int *num, bool *differs) {
	...
	const BitArray& faceSel = GetMesh().FaceSel();
	for (int i=0; i<GetMesh().getNumFaces(); i++) {
		if (flev && !faceSel[i]) continue;
		DWORD *tt = cf[i].t;
		DWORD *vv = GetMesh().faces[i].v;
		const BitArray& vertSel = GetMesh().VertSel();
		for (int j=0; j<3; j++) {
			if (vlev && !vertSel[vv[j]]) continue;
			if (num) (*num)++;
			if (!init) {
				alpha = cv[tt[j]].x;
				init = true;
...
void EditTriObject::SetEdgeVis(BOOL vis) {
	...
	for (int j=0; j<maxedge; j++) {
		if (edgeSel[j]) tmd.SetSingleEdgeVis (GetMesh(), j, vis, af);
	}
	ApplyMeshDelta (tmd, this, ip->GetTime());
...
void EditTriObject::FlipNormals() {
	...
	MeshNormalSpec *normals = GetMesh().GetSpecifiedNormals();

	MeshDelta tmd(GetMesh());
	const BitArray& faceSel = GetMesh().FaceSel();
	for (int i=0; i<GetMesh().getNumFaces(); i++) 
	{
		BOOL explicitNormal = false;
		if (normals && normals->GetNumNormals() != 0)
			explicitNormal = true;
		if (faceSel[i] && (!explicitNormal) )
			tmd.FlipNormal (GetMesh(), i);
	}
	ApplyMeshDelta (tmd, this, ip->GetTime());
	...
	if (normals)
	{
		normals->SetParent(&GetMesh());
		MeshNormalFace *fn = normals->GetFaceArray();


		if (fn != NULL)
		{
			//first reorder all the indices

			BitArray selectedNormals;
			selectedNormals.SetSize(normals->GetNumNormals());
			selectedNormals.ClearAll();

			//compute a list of all the vertics that share both a selected and unselected face
			//these are our border vertices
			const BitArray& faceSel = GetMesh().FaceSel();
			for (int i=0; i<GetMesh().getNumFaces(); i++) 
			{
				if (faceSel[i] )
				{				
					for (int j=0; j<3; j++) 
					{
						int id = fn[i].GetNormalID(j);
						selectedNormals.Set(id,TRUE);
					}
				}
			}

			BitArray normalsNeedsToBeBroken;
			normalsNeedsToBeBroken.SetSize(normals->GetNumNormals());
			normalsNeedsToBeBroken.ClearAll();
			for (int i=0; i<GetMesh().getNumFaces(); i++) 
			{
				if (!faceSel[i] )
				{				
					...
					{
						int id = fn[i].GetNormalID(j);
						if (selectedNormals[id])
						{
							normalsNeedsToBeBroken.Set(id,TRUE);
						}
					}
				}
			}
			...
			//break our edge shared by both a selected and non selected face
			//so we dont interpolate normals across the flip
			for (int i=0; i<GetMesh().getNumFaces(); i++) 
			{
				if (faceSel[i] )
				{
					if ( i < normals->GetNumFaces())
					{								
						for (int j=0; j<3; j++) 
						{
							int vertexId = GetMesh().faces[i].v[j];
							int normalId = fn[i].GetNormalID(j);
							if (normalsNeedsToBeBroken[normalId])
							{
								Point3 n = normals->GetNormal(i,j);
								normals->SetNormal(i,j,n);	
								int newNormalId = fn[i].GetNormalID(j);
								fn[i].SetSpecified(j,true);
								normals->SetNormalExplicit(newNormalId,true);
								normals->SetNormalExplicit(normalId,true);
							}
						}
					}
				}
			...
			//just a bitarray to keep from double flipping a vertex
			BitArray processedNormals;
			processedNormals.SetSize(normals->GetNumNormals());
			processedNormals.ClearAll();

			int originalNormalCount = normals->GetNumNormals();
			for (int i=0; i<GetMesh().getNumFaces(); i++) 
			{
				if (faceSel[i] && i < normals->GetNumFaces())
				{				
					for (int j=0; j<3; j++) 
					{
						int normalId = fn[i].GetNormalID(j);
						if ((normalId >=0) && (normalId < originalNormalCount) && !processedNormals[normalId])
						{
							//if it is not explict make it and flip
							if (!normals->GetNormalExplicit(normalId))
							{
								Point3 n = normals->GetNormal(i,j);
								n *= -1.0f;
								normals->SetNormal(i,j,n);
								int newNormalId = fn[i].GetNormalID(j);
								normals->SetNormalExplicit(newNormalId,true);
								if (newNormalId < processedNormals.GetSize())
									processedNormals.Set(newNormalId,TRUE);
							}
							//it is already explicit so just flip the existing normal
							else 
							{
								Point3 n = normals->GetNormal(i,j);
								n *= -1.0f;
								normals->GetNormalArray ()[normalId] = n;
								processedNormals.Set(normalId,TRUE);
							}						
							normals->SetNormalExplicit(normalId,true);						
						}
						fn[i].SetSpecified(j,true);
...
void EditTriObject::SetMatIndex (DWORD index) {
	...
	const BitArray& faceSel = GetMesh().FaceSel();
	for (int j=0; j<GetMesh().getNumFaces(); j++) {
		if (faceSel[j]) tmd.SetMatID (j,(MtlID)index);
	}
	ApplyMeshDelta (tmd, this, ip->GetTime());
	ip->RedrawViews(ip->GetTime());
}
...
void ExplodeToObjects (Mesh *mesh, float thresh, INode *node, TSTR &name,
	...
	if (!selOnly || (mesh->FaceSel().NumberSet() == mesh->numFaces)) {
		...
		for (int i=0; i<mesh->numFaces; i++) if (fc[i] == fc.count-1) faceSel.Clear (i);
		fc.count--;
	}
...
int PickFaceMouseProc::proc (HWND hwnd, int msg, int point, int flags, IPoint2 m) {
	...
	case MOUSE_POINT:
		...
		snapPoint = vpt.SnapPoint (m, m, NULL);
		snapPoint = vpt.MapCPToWorld (snapPoint);
		hr = HitTestFaces (m, vpt.ToPointer(), bary, &snapPoint);
	...
	case MOUSE_FREEMOVE:			
		...
		vpt.SnapPreview (m, m, NULL, SNAP_FORCE_3D_RESULT);//|SNAP_SEL_OBJS_ONLY);
		if (HitTestFaces(m,vpt.ToPointer(),NULL)) SetCursor(ip->GetSysCursor(SYSCUR_SELECT));
		else SetCursor(LoadCursor(NULL,IDC_ARROW));		
...
CreateFaceMouseProc::CreateFaceMouseProc(EditTriObject* in_e, IObjParam* in_i)
: CreateFaceMouseProcTemplate(in_i), m_ETri(in_e) {
	if (!hCurCreateVert) hCurCreateVert = UI::MouseCursors::LoadMouseCursor(UI::MouseCursors::CreateVertex);
}

bool CreateFaceMouseProc::CreateVertex(const Point3& in_worldLocation, int& out_vertex) {
	out_vertex = m_ETri->CreateVertex(in_worldLocation);
	return out_vertex >= 0;
			}

bool CreateFaceMouseProc::CreateFace(MaxSDK::Array<int>& in_vertices) {
	return m_ETri->CreateFace(in_vertices.asArrayPtr(), (int)in_vertices.length());
		}
...
{
	Mesh mesh = m_ETri->GetMesh();
	if(in_vertex>=0&&in_vertex<mesh.numVerts)
	{
		ModContextList mcList;
		INodeTab nodes;
		GetCOREInterface()->GetModContexts(mcList,nodes);
		if(nodes[0])
			return nodes[0]->GetObjectTM(GetInterface()->GetTime()) * mesh.verts[in_vertex];
	}
...
BOOL AttachPickMode::Pick(IObjParam *ip,ViewExp *vpt) {
	...
	// if attaching a node from a system, attach entire system
	// returns false only if you try to attach a system to itself, SA 9/2/04
	INodeTab nodeTab;
	nodeTab.Append(1,&node);
	if (!AppendSystemNodes(nodes[0],nodeTab)) {
		TSTR buf1 = GetString(IDS_SA_NO_SYS_ATTACH);
		TSTR buf2 = GetString(IDS_SCA_BASE_MESH);
		MaxSDK::MaxMessageBox(ip->GetMAXHWnd(), buf1, buf2, MB_OK | MB_ICONINFORMATION);
		nodes.DisposeTemporary(); 
		return FALSE;
	}
	if (nodeTab.Count() > 1)
	...
	// no systems - attaching a single node
	BOOL ret = TRUE;
...
void AttachHitByName::proc(INodeTab &nodeTab) {
	...
	if (!AppendSystemNodes(nodes[0],nodeTab)) {
		TSTR buf1 = GetString(IDS_SA_NO_SYS_ATTACH);
		TSTR buf2 = GetString(IDS_SCA_BASE_MESH);
		MaxSDK::MaxMessageBox(eo->ip->GetMAXHWnd(), buf1, buf2, MB_OK | MB_ICONINFORMATION);
		ret = FALSE;
	}

	if (nodes[0]->GetMtl() && ret) {
      int i;
...
void CutEdgeProc::DrawCutter (HWND hWnd,IPoint2 &m, bool bErase, bool bPresent) {
	IPoint2 pts[2];
    pts[0].x = m1.x;
    pts[0].y = m1.y;
    pts[1].x = m.x;
...
int CutEdgeProc::proc (HWND hwnd, int msg, int point, int flags, IPoint2 m ) {
	...
	case MOUSE_ABORT:
		...
		if (e1set) DrawCutter (hwnd, oldm2, true, true);
		e1set = FALSE;
		return FALSE;
	...
	case MOUSE_DBLCLICK:
		if (!e1set) break;
		// Erase last cutter line:
		DrawCutter (hwnd, oldm2, true, false);
	...
	case MOUSE_POINT:
		...
		hr = HitTestEdges(m,vpt.ToPointer());
		if (!hr) {
		...
		if (!e1set) {
			...
			e1set = TRUE;
			m1 = betterM;
			DrawCutter (hwnd, m, false, true);
			oldm2=m;
			break;
		}

		// Erase last cutter line:
		DrawCutter (hwnd, oldm2, true, true);
	...
	case MOUSE_MOVE:
		...
		if (HitTestEdges(m,vpt.ToPointer())) SetCursor(ip->GetSysCursor(SYSCUR_SELECT));
		else SetCursor(LoadCursor(NULL,IDC_ARROW));

		ip->RedrawViews (ip->GetTime());
		if (e1set) DrawCutter (hwnd,oldm2, false, true);
		break;
		}
	case MOUSE_FREEMOVE:
		{
...
int WeldVertMouseProc::proc (HWND hwnd, int msg, int point, int flags, IPoint2 m) {
	...
	case MOUSE_POINT:
		if (point==1) {
			if (!HitTestVerts(m,vpt.ToPointer(),targetVert)) {
				targetVert = -1;
			}
		}
		break;
	...
	case MOUSE_MOVE:
		int vert;
		if (HitTestVerts(m,vpt.ToPointer(),vert)) SetCursor(ip->GetSysCursor(SYSCUR_SELECT));
