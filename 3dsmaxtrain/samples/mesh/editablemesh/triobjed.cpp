...
class EditTriObjectClassDesc : public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Editable Mesh"); }
...
EditSSMode*			EditTriObject::editSSMode = NULL;
...
BOOL				EditTriObject::deleteIsolatedVerts = TRUE;
...
int                 EditTriObject::pickBoxSize     = 0;
int                 EditTriObject::weldBoxSize     = 0;
BOOL				EditTriObject::weldBoxSizeInitialized = FALSE;
...
EditTriObject::EditTriObject() {
	...
	arValid.SetEmpty();
	etFlags = 0;
	SuspendAll suspendAll (TRUE, TRUE, TRUE);
	ReplaceReference ( ET_POINT_CTRL_CONTAINER_REF, NewDefaultPointControllerContainer());
}
...
RefTargetHandle EditTriObject::Clone(RemapDir& remap) {
	...
	for (int i=0; i<cont.Count(); i++) {
		...
		if (!ntri->cont.Count()) ntri->CreateContArray();
		ntri->ReplaceReference (ET_VERT_BASE_REF + i, remap.CloneRef(cont[i]));
	}
	for (int i=0; i<3; i++) ntri->selSet[i] = selSet[i];
...
void EditTriObject::SetSubobjectLevel(int level) {
	...
	selLevel = level;
	GetMesh().SetFlag(MESH_SUBSELECTION_INVALID);
...
{	
	...
	{
		// why are we here?
		DbgAssert(!_T("Doing Display() on invalid view port!"));
		return FALSE;
	}
...
//		gw->polyline (2, rp, NULL, NULL, FALSE, es);
	...
	if ((selLevel == SL_VERTEX) || (ip->GetCommandMode()==createFaceMode)) {
		...
		// TODO: This is a temporary solution for fixing Nitrous viewport display 
		// problem in RampageSP. In simcity we should improve this code by porting 
		// the entire plugin into retained mode.

		// This code simply put the red edges of the creating faces on top of original wireframe.
		bool nitrousViewport = MaxSDK::Graphics::IsRetainedModeEnabled();
		if (nitrousViewport)
		{
			gw->multiplePass(-1, TRUE, 1.020f);
		}

		gw->startMarkers();
		BitArray vertSel = mesh.VertSel();
		const BitArray& vertHide = mesh.VertHide();
		for (int i=0; i<mesh.numVerts; i++) {
			if (vertHide[i]) continue;
#ifdef MESH_CAGE_BACKFACE_CULLING
...
void EditTriObject::SelectSubComponent (HitRecord *hitRec, BOOL selected, BOOL all, BOOL invert) {
	...
	bool localIgnoreVisEdge = ignoreVisEdge ? true : false;

	switch (selLevel) {
	case SL_VERTEX:
	{
		BitArray& vertSel = GetMesh().VertSel();
		nsel = vertSel;
		nsel.SetSize (GetMesh().numVerts, TRUE);
		for (hr=hitRec; hr!=NULL; hr = hr->Next()) {
			nsel.Set (hr->hitInfo, invert ? !vertSel[hr->hitInfo] : selected);
			if (!all) break;
		}
		SetVertSel (nsel, this, t);
		// JBW: macro-recorder
		macroRecorder->FunctionCall(_T("select"), 1, 0, mr_index, mr_prop, _T("mesh.verts"), mr_reftarg, this, mr_bitarray, &vertSel);
		break;
	}
	...
	case SL_EDGE:
		if (msci && localSelByVert) {
			...
			const BitArray& edgeSel = GetMesh().EdgeSel();
			for (hr=hitRec; hr != NULL; hr=hr->Next()) {
				if (localSelByVert) {
		...
		// JBW: macro-recorder
		macroRecorder->FunctionCall(_T("select"), 1, 0, mr_index, mr_prop, _T("mesh.edges"), mr_reftarg, this, mr_bitarray, &GetMesh().EdgeSel());
		break;
	...
	case SL_FACE:
		if (msci && localSelByVert) {
			...
			const BitArray& currFaceSel = GetMesh().FaceSel();
			for (hr=hitRec; hr != NULL; hr = hr->Next()) {
				if (localSelByVert) {
					DWORDTab & list = ae->list[hr->hitInfo];
					for (int i=0; i<list.Count(); i++) {
						...
						for (int j=0; j<2; j++) {
							if (me.f[j] == UNDEFINED) continue;
							nsel.Set (me.f[j], invert ? !currFaceSel[me.f[j]] : selected);
						}
					}
				} else {
					nsel.Set (hr->hitInfo, invert ? !currFaceSel[hr->hitInfo] : selected);
				}
				if (!all) break;
			}
		}
		...
		// JBW: macro-recorder
		macroRecorder->FunctionCall(_T("select"), 1, 0, mr_index, mr_prop, _T("mesh.faces"), mr_reftarg, this, mr_bitarray, &GetMesh().FaceSel());
		break;

	case SL_POLY:
	case SL_ELEMENT:
		/*
		all modes except the sub selection if you have selection by angle
		and hit a selected subobject you dont use the angle by selection
		*/
		bool maybeSkip = true;
		...
		if (msci) {
			...
			if (localSelByVert) {
				BitArray vhit;
				vhit.SetSize(GetMesh().numVerts);
				const BitArray& vertSel = GetMesh().VertSel();
				for (hr=hitRec; hr!=NULL; hr=hr->Next()) 
				{
					if (vertSel[hr->hitInfo] && maybeSkip)
						localIgnoreVisEdge = false;
				...
				{
					pConv->VertexToElement(GetMesh(), af, vhit, nsel);
				}
				else 
				{
					pConv->VertexToPolygon(GetMesh(), af, vhit, nsel, GetPolyFaceThresh(), localIgnoreVisEdge);
				}
			} 
			else 
			{
				...
				for (hr=hitRec; hr!=NULL; hr=hr->Next()) 
				{
					if (GetMesh().FaceSel()[hr->hitInfo] && maybeSkip)
						localIgnoreVisEdge = false;
					fhit.Set (hr->hitInfo);
					if (!all) break;
				}
				if (selLevel == SL_ELEMENT) 
				{ 
					pConv->FaceToElement (GetMesh(), af, fhit, nsel);
				}
				else 
				{
					pConv->FaceToPolygon (GetMesh(), af, fhit, nsel, GetPolyFaceThresh (), localIgnoreVisEdge);
				}
			}
		} else {
			...
			for (hr=hitRec; hr != NULL; hr=hr->Next()) 
			{
				if (!localSelByVert) 
				{
					if (selLevel == SL_ELEMENT) 
					{
						GetMesh().ElementFromFace (hr->hitInfo, nsel, af);
					}
					else
					{
						if (GetMesh().FaceSel()[hr->hitInfo] && maybeSkip)
						...
						GetMesh().PolyFromFace (hr->hitInfo, nsel, GetPolyFaceThresh (), localIgnoreVisEdge, af);
					}
				} 
				else 
				{
					...
					for (int i=0; i<list.Count(); i++) 
					{
						...
						for (int j=0; j<2; j++) 
						{
							...
							if (selLevel==SL_ELEMENT) 
							{
								GetMesh().ElementFromFace (me.f[j], nsel, af);
							}
							else 
							{
								if (GetMesh().FaceSel()[hr->hitInfo] && maybeSkip)
								...
								GetMesh().PolyFromFace (me.f[j], nsel, GetPolyFaceThresh(), localIgnoreVisEdge, af);
							}
						}
					}
				}
...
BOOL EditTriObject::SelectSubAnim(int subNum) {
	if (!subNum) return FALSE;	// cannot select Point Controller Container.
	subNum--;
...
void EditTriObject::ActivateSubobjSel (int level, XFormModes& modes) {
	...
	InvalidateTempData (SELECT_CHANNEL|SUBSEL_TYPE_CHANNEL); // OLP 0605 no need to invalidate geom and topo caches.
...
void EditTriObject::GetSubObjectTMs (SubObjAxisCallback *cb,TimeValue t,
	...
	case SL_EDGE:
		for (int i=0, ct=TempData()->ClusterNormals(MESH_EDGE)->Count(); i<ct; i++) {
	...
	default:
		for (int i=0,ct=TempData()->ClusterNormals(MESH_FACE)->Count(); i<ct; i++) {
			tm = TempData()->ClusterTM (i) * otm;
			cb->TM(tm,i);
		}
		break;
...
TempMoveRestore::TempMoveRestore (EditTriObject *em, bool useMaps) {
	...
	if (useMaps) {
		maps.SetCount (em->GetMesh().getNumMaps() + NUM_HIDDENMAPS);
		const int mapCount = em->GetMesh().getNumMaps();
...
ChannelMask TempMoveRestore::ChannelsChanged () {
	ChannelMask ret = GEOM_CHANNEL;
	for (int nmp=0; nmp<maps.Count(); nmp++) {
		if (!maps[nmp]) continue;
...
MeshVertRestore::MeshVertRestore(EditTriObject *et) {
	...
	rvData = NULL;
	int vdNum = et->GetMesh().getNumVData();
	if (!vdNum) {
		uvData = NULL;
		return;
	}
	
	uvdSupport.SetSize(et->GetMesh().getNumVData());
	for(int i = 0; i < uvdSupport.GetSize(); ++i) {
		uvdSupport.Set(i, et->GetMesh().vDataSupport(i));
	}
	
	uvData = new PerData[vdNum];
	for (int i=0; i<vdNum; i++) {
		uvData[i] = *et->GetMesh().GetVD(i);
	}
}
...
void MeshVertRestore::Restore(int isUndo) {
	...
	if (isUndo) {
		...
		vdNum = et->GetMesh().getNumVData();
		if (vdNum && !rvData)
		{
			rvdSupport.SetSize(et->GetMesh().getNumVData());
			for (int i = 0; i < rvdSupport.GetSize(); ++i) {
				rvdSupport.Set(i, et->GetMesh().vDataSupport(i));
			}

			rvData = new PerData[vdNum];
			for (int i=0; i<vdNum; i++) {
				rvData[i] = *et->GetMesh().GetVD(i);
			}
		}
	}
	...
	{
		for (int i = 0; i < uvdSupport.GetSize(); ++i) {
			et->GetMesh().setVDataSupport(i, uvdSupport[i]);
		}
		for (int i=0; i<vdNum; i++) {
			*et->GetMesh().GetVD(i) = uvData[i];
		}
	}
	et->GetMesh().ClearFlag(MESH_PARTIALCACHEINVALID);
	et->LocalDataChanged(GEOM_CHANNEL);
...
void MeshVertRestore::Redo() {
	...
	{
		for (int i = 0; i < rvdSupport.GetSize(); ++i) {
			et->GetMesh().setVDataSupport(i, rvdSupport[i]);
		}
		for (int i=0; i<vdNum; i++) {
			*et->GetMesh().GetVD(i) = rvData[i];
		}
	}
	et->GetMesh().InvalidateGeomCache();
	et->GetMesh().ClearFlag(MESH_PARTIALCACHEINVALID);
...
MeshTopoRestore::MeshTopoRestore(EditTriObject *et, ChannelMask chan) {
	this->et = et;
	channels = chan|SELECT_CHANNEL;
	if (channels & TOPO_CHANNEL) channels |= GEOM_CHANNEL;
	umesh.DeepCopy (&(et->GetMesh()), channels);

	MaxSDK::MeshChannelContainer& container = umesh.GetChannelDataContainer();
	MaxSDK::MeshTopoChannelData* tc = container.GetOrCreateMeshTopoChannelData();
	tc->numFaces = et->GetMesh().numFaces; // protects against PART_VERT_COLOR without PART_TOPO.
	tc->SyncParentInterface(&umesh);
...
bool MeshTopoRestore::After () {
	...
	MaxSDK::MeshChannelContainer& container = rmesh.GetChannelDataContainer();
	MaxSDK::MeshTopoChannelData* tc = container.GetOrCreateMeshTopoChannelData();
	tc->numFaces = et->GetMesh().numFaces; // protects against PART_VERT_COLOR without PART_TOPO.
	tc->SyncParentInterface(&rmesh);
...
void MeshTopoRestore::Restore(int isUndo) {
	...
	// SetPtCont() on every controller to restore Point Controller Container properly	
...
IOResult EditTriObject::Load(ILoad *iload) {
	...
	while (iload->PeekNextChunkID() == GENSELSET_ID_CHUNK) {
		int which = -1;
		iload->OpenChunk();
		bool badChunkRead = false;
		if (iload->CurChunkLengthRemaining() != 0)
	...
	// If we have specified normals, don't forget to add the XTC object to manage them:
	MeshNormalSpec *pSpec = mesh.GetSpecifiedNormals();
	if ((pSpec != NULL) && pSpec->GetNumFaces())
	{
		AddXTCObject (new TriObjectNormalXTC());
	}
...
void EditTriObject::UpdateNamedSelDropDown () {
	...
	for (int i=0; i<ns.Count(); i++) {
		...
		{
			ip->SetCurNamedSelSet (*(ns.names[i]));
			break;
		}
	}
}
	...
	case WM_INITDIALOG:	{
		...
		for (int i=0; i<setList->Count(); i++) {
			int pos  = SendDlgItemMessage(hWnd,IDC_NS_LIST,LB_ADDSTRING,0,
				(LPARAM)(const TCHAR*)*(setList->names[i]));
...
void NamedSetDelete::Restore (int isUndo) {
	int max = oldSets.Count();
	if (et->selSet[nsl].Count() < max) max = et->selSet[nsl].Count();
	for (int i=0; i<max; i++) *(et->selSet[nsl].sets[i]) = *(oldSets[i]);
}
...
void EditTriObject::SynchContArray(int nv) {
	...
	if (!cct) return;
	if (cct == nv) return;
	if (pointCtrlContainer) pointCtrlContainer->SetNumSubControllers(nv, TRUE);
	if (cct>nv) {
	...
	cont.Resize (nv);
	Control *dummy=NULL;
	for (int i=cct; i<nv; i++) cont.Append (1, &dummy);
}

void EditTriObject::AllocContArray(int count) {
	cont.SetCount (count);
	if (pointCtrlContainer) pointCtrlContainer->SetNumSubControllers (count);
}
...
// Old version (0) had no Point Controller Container, just the array of point controllers themselves.
...
RefTargetHandle EditTriObject::GetReference(int i) {	
	if (i <= ET_POINT_CTRL_CONTAINER_REF) return pointCtrlContainer;
...
void EditTriObject::SetReference(int i, RefTargetHandle rtarg) {
	if(i == ET_POINT_CTRL_CONTAINER_REF) {
		pointCtrlContainer = (PointControllerContainer*)rtarg;
		if (pointCtrlContainer) pointCtrlContainer->SetNumSubControllers(cont.Count());
	} else {
...
TSTR EditTriObject::SubAnimName(int i, bool localized)
{
	if (i == ET_POINT_CTRL_CONTAINER_REF)
		return localized ? GetString(IDS_POINTCTRLCONTAINER) : _T("Point Controller Container");
	TSTR buf;
	if (i < (cont.Count() + ET_VERT_BASE_REF))
		buf.printf((localized ? GetString(IDS_RB_POINTNUM) : _T("Vertex %d")), i + 1 - ET_VERT_BASE_REF);
	return buf;
}
...
BOOL EditTriObject::PlugControl(TimeValue t,int i) {
	...
	ReplaceReference ( i + ET_VERT_BASE_REF, NewDefaultPoint3Controller());
	SuspendAnimate(); 
	AnimateOff();
	SuspendSetKeyMode();
	theHold.Suspend ();
...
void EditTriObject::SetPtCont (int i, Control *c) {
	cont[i]=c;
	if (pointCtrlContainer/* && c*/) pointCtrlContainer->SetSubController(i, c);
}

void EditTriObject::SetPointAnim (TimeValue t, int i, Point3 pt) {
	if (cont.Count() && cont[i]) {
		cont[i]->SetValue(t,&pt);
	}
...
void EditTriObject::InvalidateAffectRegion () {
	arValid = NEVER;
	GetMesh().SetFlag(MESH_SOFTSELECTION_WEIGHT_INVALID);
	if (!tempData) return;
...
ObjectState EditTriObject::Eval(TimeValue time) {	
	...
	if (!geomValid.InInterval(time)) {
		...
		for (int i=0; i<cont.Count(); i++) {
			if (cont[i])  {
				cont[i]->GetValue(time,&GetMesh().verts[i],geomValid);
				mesh.InvalidateVertexCache(i); // Invalidate only modified vertices
			}
		}
		mesh.SetFlag(MESH_PARTIALCACHEINVALID);
		// Since we're changing vertex positions, we need to let
		// any specified normals know that they need to be recomputed.
		MeshNormalSpec *pNormals = GetMesh().GetSpecifiedNormals();
		if (pNormals && pNormals->GetNumFaces())
			pNormals->ClearFlag (MESH_NORMAL_NORMALS_COMPUTED);
		// This is the part of InvalidateTempData that is not covered by InvalidateVertexCache so
		// it has been kept when InvalidateTempData was removed from here.
		if (!tempMove) {
			if (tempData) tempData->Invalidate (GEOM_CHANNEL);
			InvalidateAffectRegion ();
		}
	}
	...
	// If we're using specified normals, check that they're all
...
RefResult EditTriObject::NotifyRefChanged (const Interval& changeInt, RefTargetHandle hTarget,
...
BOOL EditTriObject::AssignController (Animatable *control, int subAnim) {
	...
	if (subAnim==ET_POINT_CTRL_CONTAINER_REF) {
		int n = cont.Count();
		pointCtrlContainer->SetNumSubControllers(n);
		for (int i=0; i<n; i++) if (cont[i]) pointCtrlContainer->SetSubController(i,cont[i]);
	}
...
void EditTriObject::SetSelFaceMtlID(ModContext* mc, MtlID id, BOOL bResetUnsel) {
	...
	const BitArray& faceSel = GetMesh().FaceSel();
	for (int i=0; i<GetMesh().getNumFaces(); i++) {
		if (faceSel[i]) GetMesh().setFaceMtlIndex(i, id);
...
int EditTriObject::GetMaxMtlID(ModContext* mc) {
	MtlID mtlID = 0;
	for (int i=0; i<GetMesh().numFaces; i++) mtlID = std::max(mtlID, GetMesh().faces[i].getMatID());
	return mtlID;
}
...
void EditTriObject::LocalDataChanged () {
	InvalidateNumberSelected ();
	InvalidateSurfaceUI ();
	InvalidateTempData (SELECT_CHANNEL);
	GetMesh().SetFlag(MESH_SUBSELECTION_INVALID);
...
void EditTriObject::SetSelLevel (DWORD lev) {
	...
	else InvalidateTempData (SUBSEL_TYPE_CHANNEL);
}
...
void EditTriObject::SetVertSel(const BitArray &set, IMeshSelect *imod, TimeValue t) {
...
void EditTriObject::SetFaceSel(const BitArray &set, IMeshSelect *imod, TimeValue t) {
	if (ip) ip->ClearCurNamedSelSet();
...
void EditTriObject::SetEdgeSel(const BitArray &set, IMeshSelect *imod, TimeValue t) {
	if (ip) ip->ClearCurNamedSelSet();
...
void EditTriObject::SetSel (int nsl, const BitArray & set, IMeshSelect *imod, TimeValue t) {
	switch (nsl) {
...
void EditTriObject::LocalDataChanged (ChannelMask channels) {
	...
	{
		// If we have specified normals, we'll need to recompute the nonexplicit ones.
		MeshNormalSpec *pNormals = GetMesh().GetSpecifiedNormals();
		if (pNormals && pNormals->GetNumFaces())
			pNormals->ClearFlag (MESH_NORMAL_NORMALS_COMPUTED);
	}
...
void EditTriObject::ApplyMeshDelta (MeshDelta & md, MeshDeltaUser *mdu, TimeValue t) {
	...
	ChannelMask partsChanged = md.PartsChanged ();
	if (theHold.Holding() && !TestAFlag (A_HELD)) {
		if (partsChanged & (TOPO_CHANNEL|TEXMAP_CHANNEL|VERTCOLOR_CHANNEL)) {
...
void EditTriObject::InvalidateTempData (ChannelMask channels) {
	if (!tempMove) {
		if (tempData) tempData->Invalidate (channels);
		if (channels & (TOPO_CHANNEL|GEOM_CHANNEL|SELECT_CHANNEL|SUBSEL_TYPE_CHANNEL))
			InvalidateAffectRegion ();
	}
	if (channels & TOPO_CHANNEL) GetMesh().InvalidateTopologyCache();
	if (channels & GEOM_CHANNEL) GetMesh().InvalidateGeomCache ();
}
...
void EditTriObject::DragMoveRestore () {
	...
	dragRestored = TRUE;
	GetMesh().SetFlag(MESH_PARTIALCACHEINVALID);
...
void EditTriObject::DragMove (MeshDelta & md, MeshDeltaUser *mdu) {
	...
	for (int i=0; i<md.vMove.Count(); i++) {
		...
		GetMesh().InvalidateVertexCache(j); // Invalidate only modified vertices
	}
	// And the map equivalents:
	for (int mp=-NUM_HIDDENMAPS; mp<md.GetMapNum(); mp++) {
		if (!md.getMapSupport (mp)) continue;
		for (int i=0; i<md.Map(mp).vSet.Count(); i++) {
...
void EditTriObject::DragMoveClear () {
	...
	{
		GetMesh().InvalidateVertexCache(i);
}
	GetMesh().InvalidateGeomCache();
}

void EditTriObject::DoAccept(TimeValue t)
{

}

void EditTriObject::SetPinch(TimeValue t, float ipinch)
{
	ISpinnerControl* spin = GetISpinner (GetDlgItem (hAR, IDC_PINCHSPIN));
	spin->SetValue(ipinch, TRUE);
	ReleaseISpinner(spin);
	InvalidateAffectRegion();
}
void EditTriObject::SetBubble(TimeValue t, float ibubble)
{
	ISpinnerControl* spin = GetISpinner (GetDlgItem (hAR, IDC_BUBBLESPIN));
	spin->SetValue(ibubble, TRUE);
	ReleaseISpinner(spin);
	InvalidateAffectRegion();

}
void EditTriObject::SetFalloff(TimeValue t,float ifalloff)
{
	ISpinnerControl* spin = GetISpinner (GetDlgItem (hAR, IDC_FALLOFFSPIN));
	spin->SetValue(ifalloff, TRUE);
	ReleaseISpinner(spin);
	InvalidateAffectRegion();
}


float EditTriObject::GetPinch(TimeValue t)
{
	ISpinnerControl* spin = GetISpinner (GetDlgItem (hAR, IDC_PINCHSPIN));
...
float EditTriObject::GetBubble(TimeValue t)
{
	ISpinnerControl* spin = GetISpinner (GetDlgItem (hAR, IDC_BUBBLESPIN));
	float val = spin->GetFVal();
	ReleaseISpinner(spin);
...
float EditTriObject::GetFalloff(TimeValue t)
{
	ISpinnerControl* spin = GetISpinner (GetDlgItem (hAR, IDC_FALLOFFSPIN));
	float val = spin->GetFVal();
	ReleaseISpinner(spin);
