...
INT_PTR EditPolySubobjControlDlgProc::DlgProc (TimeValue t, IParamMap2 *pmap, HWND hWnd,
	...
	case WM_PAINT:
		...
		case MNM_SL_EDGE: 
			...
			UpdatePerDataDisplay(t, EPM_SL_EDGE, EDATA_DEPTH, hWnd);
			break;
		}
		mUIValid = true;
		return FALSE;
	...
	case CC_COLOR_CLOSE:
		if(LOWORD(wParam) == IDC_HARD_COLOR) {
			...
			if(swatch != NULL) {
				...
				Color theColor(swatch->GetAColor());
				theHold.Begin ();
	...
	case CC_SPINNER_CHANGE:
        {
            ...
            case IDC_VS_CREASESPIN:
                ...
                mpMod->getParamBlock()->SetValue (epm_data_channel, t, (spinnerID == IDC_VS_WEIGHTSPIN) ? VDATA_WEIGHT : VDATA_CREASE);	
                break;
            case IDC_ES_WEIGHTSPIN:
            case IDC_ES_CREASESPIN:
            case IDC_ES_DEPTHSPIN:
                {
                    ...
                    int valToSet = EDATA_KNOT;
                    switch (spinnerID)
                    {
                    case IDC_ES_CREASESPIN:
                        valToSet = EDATA_CREASE;
                        break;
                    case IDC_ES_DEPTHSPIN:
                        valToSet = EDATA_DEPTH;
                        break;
                    default:
                        break;
                    }

                    mpMod->getParamBlock()->SetValue (epm_data_channel, t, valToSet);
                }
