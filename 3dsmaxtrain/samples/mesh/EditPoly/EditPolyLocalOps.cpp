...
public:
	...
	void Restore (int isUndo) {
		...
		mpMod->EpModLocalDataChanged(DISP_ATTRIB_CHANNEL);
	}
	void Redo () {
		if (!mpData->GetPolyOpData()) return;
		if (mpData->GetPolyOpData()->OpID() != ep_op_create) return;
	...
	int Size () { return 20; }
	TSTR Description() { return _T("CreateVertex"); }
};
...
public:
	...
	TSTR Description() { return _T("CreateFace"); }
};
...
public:
	...
	int Size () { return 16; }
	TSTR Description() { return _T("CreateEdge"); }
};
...
public:
	...
	int Size () { return 16; }
	TSTR Description() { return _T("SetDiagonal"); }
};
...
public:
	...
	int Size () { return 52; }
	TSTR Description() { return _T("Cut"); }
};
...
public:
	...
	int Size () { return 16; }
	TSTR Description() { return _T("DivideEdge"); }
};
...
public:
	...
	int Size () { return 24; }
	TSTR Description() { return _T("DivideFace"); }
};
	...
	if (macroRecorder->Enabled())
	{
		...
		if (pNode != NULL)
		{
			macroRecorder->FunctionCall(_T("$.modifiers[#Edit_Poly].DivideFace"), 2, 1,
				mr_int, face+1, mr_name, baryCoordString.data(), _T("node"), mr_reftarg, pNode);
		}
		else
		{
			macroRecorder->FunctionCall(_T("$.modifiers[#Edit_Poly].DivideFace"), 2, 0,
				mr_int, face+1, mr_name, baryCoordString.data());
		}
		macroRecorder->EmitScript();
	}
}
...
public:
	...
	int Size () { return 16; }
	TSTR Description() { return _T("TargetWeld"); }
};
...
public:
	...
	int Size () { return 16; }
	TSTR Description() { return _T("SetHingeEdge"); }
};
...
public:
	...
	int Size () { return 16; }
	TSTR Description() { return _T("TurnEdge"); }
};
...
public:
	...
	int Size () { return 12; }
	TSTR Description() { return _T("Attach"); }
};
...
void EditPolyMod::EpModAttach (INode *node, INode *pNodeArg, TimeValue t) {
	...
	EpModLocalDataChanged (ALL_CHANNELS - SUBSEL_TYPE_CHANNEL);
...
void EditPolyMod::EpModDetachToObject (TSTR & name, TimeValue t) {
	...
	for (int nd=0; nd<list.Count(); nd++) {
		...
		if (holdStarted)
			theHold.Accept(GetString(IDS_CLONE));
	}
...
public:
	...
	int Size () { return 9; }
	TSTR Description() { return _T("TogglePreserveMap"); }
};
...
void EditPolyMod::EpModSetPreserveMap (int mapChannel, bool preserve) {
	...
	if (mpCurrentOperation && mpCurrentOperation->PreserveMapSettings()) {
		EpModLocalDataChanged ((mapChannel<1) ? VERTCOLOR_CHANNEL : TEXMAP_CHANNEL);
	}
...
public:
	...
	void Restore (int isUndo) {
		...
		mpMod->EpModLocalDataChanged (GEOM_CHANNEL|DISP_ATTRIB_CHANNEL);
	}
	void Redo () {
		if (!mpData->GetPolyOpData() || (mpData->GetPolyOpData()->OpID() != ep_op_transform)) return;
		LocalTransformData *pTransform = (LocalTransformData *) mpData->GetPolyOpData();
	...
	TSTR Description() { return _T("GeomDelta"); }
};
...
void EditPolyMod::EpModDragInit (INode *pNode, TimeValue t) {
	...
	if (!m_isPostprocessedExtrudeInProgress)
	{
		pTransform->SetRetriangulationVertexLimit();
	}
}
...
void EditPolyMod::EpModDragMove (Tab<Point3> & delta, INode *pNode, TimeValue t) {
	...
	EpModLocalDataChanged (GEOM_CHANNEL|DISP_ATTRIB_CHANNEL);
}
...
void EditPolyMod::EpModDragAccept (INode *pNode, TimeValue t) {
   ...
   int j;
	...
	int numVerts = pData->GetMesh() ? (int)pData->GetMesh()->numv : pData->mVertSel.NumberSet();
