...
// Some defines for this module

enum {
	CHAMFER_EDGE_SMOOTH_OFF=0,
	CHAMFER_EDGE_SMOOTH_ON
};


// ==============================================================================
// class EditPolyModPolyOperationGrip
// ==============================================================================

EditPolyModPolyOperationGrip::EditPolyModPolyOperationGrip()
	: mpEditPoly(NULL),mReentryLock(false)
{}
...
void EditPolyModPolyOperationGrip::Init(EditPolyMod* editPoly)
{	
	if (editPoly != NULL)
	{
		mpEditPoly = editPoly;
...
EditPolyMod*  EditPolyModPolyOperationGrip::GetEditPolyMod()
{
	return mpEditPoly;
}
void EditPolyModPolyOperationGrip::SetUpUI()
{
	IGripManager* igripman = GetIGripManager();
	if (igripman)
	{
		igripman->ResetAllUI();
	}
}
bool EditPolyModPolyOperationGrip::SupportsOkayApplyCancel()
{
	return true;
}
void EditPolyModPolyOperationGrip::CommonOK( TimeValue t )
{
	if(!mReentryLock)
	{
		MaxSDK::VariableGuard<bool> guard(mReentryLock, true);
		DbgAssert(mpEditPoly != NULL);
		if (mpEditPoly == NULL)
			return;
		if( mpEditPoly->GetPolyOperation())
		{
			mpEditPoly->GetPolyOperation()->HideGrip();
			StartSetValue(-1,t);
		...
		else
		{
			...
			if (igripman)
			{
				igripman->SetGripsInactive();
			}
			mpEditPoly->EpModCancel();
			mpEditPoly->EpModRefreshScreen();
		}
		mpEditPoly = NULL;
	}
}
void EditPolyModPolyOperationGrip::CommonApply( TimeValue t )
{
	if(!mReentryLock)
	{
		MaxSDK::VariableGuard<bool> guard(mReentryLock, true);
		...
		StartSetValue(-1,t);
		mpEditPoly->EpModCommitAndRepeat (t);
		mpEditPoly->EpModRefreshScreen ();
		EndSetValue(-1,t,true);
	}
}

void EditPolyModPolyOperationGrip::CommonCancel()
{
	if(!mReentryLock)
	{
		MaxSDK::VariableGuard<bool> guard(mReentryLock, true);
...
// ==============================================================================
// class EPManipulatorGrip
// ==============================================================================

void EPManipulatorGrip::SetUpVisibility(BOOL isVisible, int indexGrip)
{
    IGripManager* gripMan = GetIGripManager();

    // If a valid grip index has been passed, set visibility
    int nGrips = GetNumGripItems();
    DbgAssert((indexGrip >= 0) && (indexGrip < nGrips));
	if ((indexGrip >= 0) && (indexGrip < nGrips))
	{
		bool isHidden = true;
        if (isVisible)
        {
        ...
        gripMan->HideGripItem(indexGrip, isHidden);

        // If we just hid the item, check to see whether all items are now hidden; if they are, set grips inactive
        if (isHidden)
        {
            int gripIndex = 0;
            bool areAllHidden = true;
            while (areAllHidden && (gripIndex != nGrips))
            {
                areAllHidden = !(gripMan->IsGripItemVisible(gripIndex));
                ++gripIndex;
            }

            if (areAllHidden)
            {
                gripMan->SetGripsInactive();
            }
        } 
	}
...
void EPManipulatorGrip::UpdateGripVisibility()
{
	if(NULL != mpEditPoly)
	{
		mpEditPoly->SetUpManipGripVisibility();
	}
}

BaseInterface* EPManipulatorGrip::GetInterface(Interface_ID id)
{
	if(id == IBASEGRIP2_INTERFACE)
	{
...
{
	return EPManipulatorGrip::EPManipulatorGripCount;
}
...
{
	string = GetString(IDS_MANIPULATE);
}
bool EPManipulatorGrip::GetText(int which,TSTR &string)
{
	...
	{
	case eFalloff:
		string = GetString(IDS_FALLOFF);
		break;
	case eBubble:
		string = GetString(IDS_BUBBLE);
		break;
	case ePinch:
		string = GetString(IDS_PINCH);
		break;
	case eSetFlow:
		string = GetString(IDS_SETFLOW);
		break;
	case eLoopShift:
		string = GetString(IDS_LOOPSHIFT);
		break;
	case eRingShift:
		string = GetString(IDS_RINGSHIFT);
		break;
	case eEdgeCrease:
		string = GetString(IDS_EDGECREASE);
		break;
	case eEdgeWeight:
		string = GetString(IDS_EDGEWEIGHT);
		break;
	case eEdgeDepth:
		string = GetString(IDS_EDGEDEPTH);
		break;
	case eVertexCrease:
		string = GetString(IDS_VERTEXCREASE);
		break;
	case eVertexWeight:
		string = GetString(IDS_VERTEXWEIGHT);
		break;
	default:
		return false;
	};
...
bool EPManipulatorGrip::GetResolvedIconName(int which,MSTR &string)
{
	...
	case eEdgeWeight:
		...
		break;
    case eEdgeDepth:
        string = EDGEDEPTH_PNG;
        break;
	case eVertexCrease:
		string = VERTEXCREASE_PNG;
		break;
	case eVertexWeight:
...
bool EPManipulatorGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eFalloff:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_ss_falloff, t, value.mFloat);
		break;
	case eBubble:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_ss_bubble, t, value.mFloat);
		break;
	case ePinch:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_ss_pinch, t, value.mFloat);
		break;
	case eSetFlow:
	case eLoopShift:
	case eRingShift:
	...
	case eEdgeCrease:
		{
			int num = 0;
			bool uniform(true);
			value.mFloat = GetEditPolyMod()->GetEdgeDataFloatValue (EDATA_CREASE, &num, &uniform, MN_SEL, NULL);
		}
		break;
	case eEdgeWeight:
		{
			int num = 0;
			bool uniform(true);
			value.mFloat = GetEditPolyMod()->GetEdgeDataFloatValue (EDATA_KNOT, &num, &uniform, MN_SEL, NULL);
    ...
    case eEdgeDepth:
        {
            int num = 0;
            bool uniform = true;
            value.mFloat = (GetEditPolyMod())->GetEdgeDataFloatValue(EDATA_DEPTH, &num, &uniform, MN_SEL, NULL);
        }
        break;
	case eVertexCrease:
		{
			int num = 0;
			bool uniform(true);
			value.mFloat = GetEditPolyMod()->GetVertexDataFloatValue (VDATA_CREASE, &num, &uniform, MN_SEL, NULL);
		}
		break;
	case eVertexWeight:
		{
			int num = 0;
			bool uniform(true);
			value.mFloat = GetEditPolyMod()->GetVertexDataFloatValue (VDATA_WEIGHT, &num, &uniform, MN_SEL, NULL);
		}
		break;
	default:
		return false;
...
bool EPManipulatorGrip::StartSetValue(int which,TimeValue t)
{
	...
	case ePinch:
		...
		break;
	case eSetFlow:
		ExecuteMAXScriptScript(_T("PolyBoost.FlowSel = PolyBGetSel 2"), MAXScript::ScriptSource::NonEmbedded, TRUE);//this is from PolyTools.ms, SetFlowSpinnerCallback
		ExecuteMAXScriptScript(_T("PolyBSetFlowSpinner PolyToolsUI.SetFlowAutoLoop 0 0 1"), MAXScript::ScriptSource::NonEmbedded, TRUE);//this is from PolyTools.ms, SetFlowSpinnerCallback
		break;
	case eLoopShift:
	case eRingShift:
		{
	...
	case eEdgeCrease:
		{
			GetEditPolyMod()->BeginPerDataModify (MNM_SL_EDGE, EDATA_CREASE);
		}
		break;
	case eEdgeWeight:
		{
			GetEditPolyMod()->BeginPerDataModify (MNM_SL_EDGE, EDATA_KNOT);
		}
		break;
	case eEdgeDepth:
		{
			(GetEditPolyMod())->BeginPerDataModify(MNM_SL_EDGE, EDATA_DEPTH);
		}
		break;
	case eVertexCrease:
		{
			GetEditPolyMod()->BeginPerDataModify (MNM_SL_VERTEX, VDATA_CREASE);
		}
		break;
	case eVertexWeight:
		{
			GetEditPolyMod()->BeginPerDataModify (MNM_SL_VERTEX, VDATA_WEIGHT);
		}
		break;
	default:
		return false;
...
bool EPManipulatorGrip::SetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	case eSetFlow:
		{
			TSTR strScript;
			strScript.printf(_T("PolyBSetFlowSpinner false %d PolyToolsUI.SetFlowSpeed 0"), value.mInt);
			ExecuteMAXScriptScript(strScript.data(), MAXScript::ScriptSource::NonEmbedded, TRUE); //this is from PolyTools.ms, SetFlowSpinnerCallback
			value.mInt = 0; //keep the value as zero for the spinner.
		}
		break;
	case eLoopShift:
	...
	case eEdgeCrease: 
		{
			GetEditPolyMod()->DoPerDataModify (MNM_SL_EDGE, EDATA_CREASE, value.mFloat);
		}
		break;
	case eEdgeWeight:
		{
			GetEditPolyMod()->DoPerDataModify (MNM_SL_EDGE, EDATA_KNOT, value.mFloat);
		}
		break;
	case eEdgeDepth:
		{
			(GetEditPolyMod())->DoPerDataModify(MNM_SL_EDGE, EDATA_DEPTH, value.mFloat);
		}
		break;
	case eVertexCrease:
		{
			GetEditPolyMod()->DoPerDataModify (MNM_SL_VERTEX, VDATA_CREASE, value.mFloat);
		}
		break;
	case eVertexWeight:
		{
			GetEditPolyMod()->DoPerDataModify (MNM_SL_VERTEX, VDATA_WEIGHT, value.mFloat);
		}
		break;
	default:
		return false;
...
bool EPManipulatorGrip::EndSetValue(int which,TimeValue t,bool accepted)
{
	...
	case ePinch:
		...
		break;
	case eSetFlow:
		ExecuteMAXScriptScript(_T("PolyBAdjustUndo PolyBoost.FlowSel"), MAXScript::ScriptSource::NonEmbedded, TRUE);//this is from PolyTools.ms, SetFlowSpinnerCallback
		break;
	case eLoopShift:
	case eRingShift:	
		// reset keys
		...
		GetEditPolyMod()->setCtrlKey(false );
		GetEditPolyMod()->EpModRefreshScreen ();	
		GetEditPolyMod()->EpModLocalDataChanged(SELECT_CHANNEL);
		break;
	case eEdgeCrease:
	case eEdgeWeight:
    case eEdgeDepth:
	case eVertexCrease:
	case eVertexWeight:
		GetEditPolyMod()->EndPerDataModify (accepted);
		break;
	default:
		return false;
	}
...
bool EPManipulatorGrip::ResetValue(TimeValue t,int which)
{
	...
	case eEdgeCrease:
		{
			GetEditPolyMod()->DoPerDataModify (MNM_SL_EDGE, EDATA_CREASE, fResetValue);
		}
		break;
	case eEdgeWeight:
		{
			GetEditPolyMod()->DoPerDataModify (MNM_SL_EDGE, EDATA_KNOT, fResetValue);
		}
		break;
	case eEdgeDepth:
		{
			(GetEditPolyMod())->DoPerDataModify(MNM_SL_EDGE, EDATA_DEPTH, fResetValue);
		}
		break;
	case eVertexCrease:
		{
			GetEditPolyMod()->DoPerDataModify (MNM_SL_VERTEX, VDATA_CREASE, fResetValue);
		}
		break;	
	case eVertexWeight:
		{
			GetEditPolyMod()->DoPerDataModify (MNM_SL_VERTEX, VDATA_WEIGHT, fResetValue);
		}
		break;	
	default:
		return false;
...
bool EPManipulatorGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eFalloff:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_ss_falloff, t) == TRUE ? true : false;
	case eBubble:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_ss_bubble,  t) == TRUE ? true : false;
	case ePinch:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_ss_pinch,  t) == TRUE ? true : false;
	}
	return false;
}
...
{
	return 4; //would be 5 for the old one since the old inluded okay and apply (remember cancel was implicit)
}
IBaseGrip::Type BevelGrip::GetType(int which)
{
	DbgAssert(which>=0&&which<GetNumGripItems());
	switch(which)
	{
	case eBevelType:
...
bool BevelGrip::GetText(int which,TSTR &string)
{
	...
	case eBevelType:
		{
			...
			int type;
			GetEditPolyMod()->getParamBlock()->GetValue (epm_bevel_type, 0, type);
			switch(type)
			{
				case 0:
					string = /*TSTR(GetString(IDS_BEVEL_TYPE)) + TSTR(": ") +*/ GetString(IDS_GROUP_NORMALS);
					break;
				case 1:
					string = /*TSTR(GetString(IDS_BEVEL_TYPE));// + TSTR(": ") +*/ GetString(IDS_LOCAL_NORMALS);
					break;
				case 2:
					string = /*TSTR(GetString(IDS_BEVEL_TYPE));// + TSTR(": ") +*/ GetString(IDS_BY_POLYGON);
					break;
			}
			break;
		}
	case eBevelHeight:
		{
			string =GetString(IDS_HEIGHT);// + TSTR(": ") + TSTR(str);
		}
		break;
	case eBevelOutline:
		{
			string =GetString(IDS_OUTLINE);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eBevelBias:
	{
		string = GetString(IDS_BIAS);//+ TSTR(": ") + TSTR(str);
	}
	break;
...
bool BevelGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eBevelType:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_bevel_type, t) == TRUE ? true : false;
	case eBevelHeight:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_bevel_height,  t) == TRUE ? true : false;
	case eBevelOutline:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_bevel_outline,  t) == TRUE ? true : false;
	case eBevelBias:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_bevel_face_bias, t) == TRUE ? true : false;
	}
	return false;
}
...
bool BevelGrip::GetComboOptions(int which, Tab<IBaseGrip::ComboLabel*> &comboOptions)
{
	if(which== eBevelType)
	{
		...
		label->mIcon = string;
		label->mLabel = GetString(IDS_GROUP_NORMALS);
		comboOptions.Append(1,&label);
		label = new IBaseGrip::ComboLabel();
		string = BEVEL_NORMAL_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_LOCAL_NORMALS);
		comboOptions.Append(1,&label);
		label = new IBaseGrip::ComboLabel();
		string = BEVEL_POLYGON_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_BY_POLYGON);
		comboOptions.Append(1,&label);
		return true;
	}
	return false;
...
bool BevelGrip::GetRange(int which,IBaseGrip::GripValue &minRange, IBaseGrip::GripValue &maxRange)
{
	...
	case eBevelOutline:
		...
		break;
	case eBevelBias:
		theDef = GetEditPolyMod()->getParamBlock()->GetParamDef(epm_bevel_face_bias);
		if (theDef.flags&P_HAS_RANGE)
		{
			minRange.mFloat = theDef.range_low.f;
			maxRange.mFloat = theDef.range_high.f;
...
void BevelGrip::SetUpUI()
{
	if (GetEditPolyMod()->getParamBlock()->GetInt(epm_bevel_type, 0) == 1)
		GetIGripManager()->HideGripItem(3, false);
	else
		GetIGripManager()->HideGripItem(3, true);

	EditPolyModPolyOperationGrip::SetUpUI();
}
...
bool BevelGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eBevelType:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_bevel_type, t, value.mInt);
		break;
	case eBevelHeight:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_bevel_height, t, value.mFloat);
		break;
	case eBevelOutline:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_bevel_outline, t, value.mFloat);
		break;
	case eBevelBias:
		GetEditPolyMod()->getParamBlock()->GetValue(epm_bevel_face_bias, t, value.mFloat);
		break;
	default:
		return false;
	}
...
bool BevelGrip::SetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eBevelType:
		if(value.mInt>3) //go back to zero
			value.mInt =0;
		if(value.mInt<0)
			value.mInt = 3;
	...
	case eBevelOutline:
		...
		break;
	case eBevelBias:
		GetEditPolyMod()->getParamBlock()->SetValue(epm_bevel_face_bias, t, value.mFloat);
		GetEditPolyMod()->getParamBlock()->GetValue(epm_bevel_face_bias, t, value.mFloat);
		break;
	default:
		return false;
...
bool BevelGrip::ResetValue(TimeValue t, int which)
{
	...
	case eBevelType:
		{
			...
			GetEditPolyMod()->getParamBlock()->GetValue(epm_bevel_type,t, iResetVal);
			theHold.Accept (GetString(IDS_BEVEL));
		}
			break;
	case eBevelHeight:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_bevel_height,t,fResetVal);
			theHold.Accept (GetString(IDS_BEVEL));
		}
		break;
	case eBevelOutline:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_bevel_outline,t, fResetVal);
			theHold.Accept (GetString(IDS_BEVEL));
	...
	case eBevelBias:
	{
		fResetVal = 1.0f;
		theHold.Begin();
		GetEditPolyMod()->getParamBlock()->SetValue(epm_bevel_face_bias, t, fResetVal);
		GetCOREInterface()->RedrawViews(GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
		GetEditPolyMod()->getParamBlock()->GetValue(epm_bevel_face_bias, t, fResetVal);
		theHold.Accept(GetString(IDS_BEVEL));
...
bool BevelGrip::GetResetValue(int which,IBaseGrip::GripValue &resetValue)
{
	...
	case eBevelBias:
	{
		const ParamDef& pd = pbd->paramdefs[epm_bevel_face_bias];
		resetValue.mFloat = pd.def.f;
	}
	break;
...
{
	string =GetString(IDS_FACE_EXTRUDE);
}
...
bool ExtrudeFaceGrip::GetText(int which,TSTR &string)
{
	...
	case eExtrudeFaceType:
		{
			...
			int type;
			GetEditPolyMod()->getParamBlock()->GetValue (epm_extrude_face_type, 0, type);
			switch(type)
			{
			case 0:
				string = GetString(IDS_GROUP_NORMALS);
				break;
			case 1:
				string = GetString(IDS_LOCAL_NORMALS);
				break;
			case 2:
				string = GetString(IDS_BY_POLYGON);
				break;
	...
	case eExtrudeFaceHeight:
		{
			string = GetString(IDS_HEIGHT);
		}
		break;
	case eExtrudeFaceBias:
	{
		string = GetString(IDS_BIAS);
	}
	break;
...
bool ExtrudeFaceGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eExtrudeFaceType:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_extrude_face_type, t) == TRUE ? true : false;
	case eExtrudeFaceHeight:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_extrude_face_height,  t) == TRUE ? true : false;
	case eExtrudeFaceBias:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_extrude_face_bias, t) == TRUE ? true : false;
...
bool ExtrudeFaceGrip::GetRange(int which,IBaseGrip::GripValue &minRange, IBaseGrip::GripValue &maxRange)
{
	...
	case eExtrudeFaceHeight:
		...
		break;
	case eExtrudeFaceBias:
		theDef = GetEditPolyMod()->getParamBlock()->GetParamDef(epm_extrude_face_bias);
		if (theDef.flags&P_HAS_RANGE)
		{
			minRange.mFloat = theDef.range_low.f;
			maxRange.mFloat = theDef.range_high.f;
...
void ExtrudeFaceGrip::SetUpUI()
{
	if (GetEditPolyMod()->getParamBlock()->GetInt(epm_extrude_face_type, 0) == 1)
		GetIGripManager()->HideGripItem(2, false);
	else
		GetIGripManager()->HideGripItem(2, true);
...
bool ExtrudeFaceGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eExtrudeFaceType:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_extrude_face_type, t, value.mInt);
		break;
	case eExtrudeFaceHeight:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_extrude_face_height, t, value.mFloat);
		break;
	case eExtrudeFaceBias:
		GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_face_bias, t, value.mFloat);
		break;
	default:
		return false;
	}
...
bool ExtrudeFaceGrip::SetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	case eExtrudeFaceHeight:
		...
		break;
	case eExtrudeFaceBias:
		GetEditPolyMod()->getParamBlock()->SetValue(epm_extrude_face_bias, t, value.mFloat);
		GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_face_bias, t, value.mFloat);
		break;
	default:
		return false;
...
bool ExtrudeFaceGrip::ResetValue(TimeValue t, int which)
{
	...
	case eExtrudeFaceType:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_face_type, t , iResetVal);
			theHold.Accept (GetString(IDS_EXTRUDE));
		}
		break;
	case eExtrudeFaceHeight:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_face_height, t, fResetVal);
			theHold.Accept (GetString(IDS_EXTRUDE));
		}
		break;
	case eExtrudeFaceBias:
	{
		fResetVal = 1.0f;
		theHold.Begin();
		GetEditPolyMod()->getParamBlock()->SetValue(epm_extrude_face_bias, t, fResetVal);
		GetCOREInterface()->RedrawViews(GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
		GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_face_bias, t, fResetVal);
		theHold.Accept(GetString(IDS_EXTRUDE));
	}
	break;
	default:
...
bool ExtrudeFaceGrip::GetResetValue(int which,IBaseGrip::GripValue &resetValue)
{
	...
	case eExtrudeFaceBias:
	{
		const ParamDef& pd = pbd->paramdefs[epm_extrude_face_bias];
		resetValue.mFloat = pd.def.f;
	}
	break;
...
{
	string =GetString(IDS_EDGE_EXTRUDE);
}
...
bool ExtrudeEdgeGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eExtrudeEdgeWidth:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_extrude_edge_width, t) == TRUE ? true : false;
	case eExtrudeEdgeHeight:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_extrude_edge_height,  t) == TRUE ? true : false;
	}
	return false;
}
...
bool ExtrudeEdgeGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eExtrudeEdgeHeight:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_extrude_edge_height, t, value.mFloat);
		break;
	case eExtrudeEdgeWidth:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_extrude_edge_width, t, value.mFloat);
		break;
	default:
		return false;
	}
	...
	case eExtrudeEdgeHeight:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_edge_height, t, fResetVal);
			theHold.Accept (GetString(IDS_EXTRUDE));
		}
		break;
	case eExtrudeEdgeWidth:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_edge_width, t,fResetVal);
			theHold.Accept (GetString(IDS_EXTRUDE));
...
{
	string =GetString(IDS_VERTEX_EXTRUDE);
}
...
bool ExtrudeVertexGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eExtrudeVertexWidth:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_extrude_vertex_width, t) == TRUE ? true : false;
	case eExtrudeVertexHeight:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_extrude_vertex_height,  t) == TRUE ? true : false;
	}
	return false;
}
...
bool ExtrudeVertexGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eExtrudeVertexHeight:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_extrude_vertex_height, t, value.mFloat);
		break;
	case eExtrudeVertexWidth:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_extrude_vertex_width, t, value.mFloat);
		break;
	default:
		return false;
	}
	...
	case eExtrudeVertexHeight:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_vertex_height, t, fResetVal);
			theHold.Accept (GetString(IDS_EXTRUDE));
		}
		break;
	case eExtrudeVertexWidth:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_vertex_width, t,fResetVal);
			theHold.Accept (GetString(IDS_EXTRUDE));
...
bool OutlineGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eOutlineAmount:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_outline, t) == TRUE ? true : false;
	}
	return false;
}
...
bool OutlineGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eOutlineAmount:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_outline, t, value.mFloat);
		break;
	default:
		return false;
	}
	...
	case eOutlineAmount:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_outline, t, fResetVal);
			theHold.Accept (GetString(IDS_OUTLINE));
		}
		break;
	default:
	...
	case eInsetType:
		{
			...
			int type;
			GetEditPolyMod()->getParamBlock()->GetValue (epm_inset_type, 0, type);
			switch(type)
			{
			case 0:
				string = GetString(IDS_GROUP);
				break;
			case 1:
				string = GetString(IDS_BY_POLYGON);
				break;
...
bool InsetGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eInsetAmount:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_inset, t) == TRUE ? true : false;
	}
	return false;
}
...
bool InsetGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eInsetType:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_inset_type, t, value.mInt);
		break;
	case eInsetAmount:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_inset, t, value.mFloat);
		break;
	default:
		return false;
	}
	...
	case eInsetType:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_inset_type, t, iResetVal);
			theHold.Accept (GetString(IDS_INSET));
		}
		break;
	case eInsetAmount:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_inset, t, fResetVal);
			theHold.Accept (GetString(IDS_INSET));
		}
		break;
	default:
...
bool InsetGrip::GetComboOptions(int which, Tab<IBaseGrip::ComboLabel*> &comboOptions)
{
	if(which== eInsetType)
	{
		...
		TSTR string = BEVEL_GROUP_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_GROUP);
		comboOptions.Append(1,&label);
		label = new IBaseGrip::ComboLabel();
		string = BEVEL_POLYGON_PNG;
		label->mIcon = string;
...
{
	string =GetString(IDS_CONNECT_EDGES);
}
...
bool ConnectEdgeGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eConnectEdgeSegments:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_connect_edge_segments, t) == TRUE ? true : false;
	case eConnectEdgePinch:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_connect_edge_pinch, t) == TRUE ? true : false;
	case eConnectEdgeSlide:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_connect_edge_slide, t) == TRUE ? true : false;
	}
	return false;
}
...
bool ConnectEdgeGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eConnectEdgeSegments:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_connect_edge_segments, t, value.mInt);
		break;
	case eConnectEdgePinch:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_connect_edge_pinch, t, value.mInt);
		break;
	case eConnectEdgeSlide:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_connect_edge_slide, t, value.mInt);
		break;
	default:
		return false;
	}
	...
	case eConnectEdgeSegments:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_connect_edge_segments, t,iResetSegVal);
			theHold.Accept (GetString(IDS_CONNECT_EDGES));
		}
		break;
	case eConnectEdgePinch:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_connect_edge_pinch, t,iResetPinchVal);
			theHold.Accept (GetString(IDS_CONNECT_EDGES));
		}
		break;
	case eConnectEdgeSlide:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_connect_edge_slide, t,iResetSlideVal);
			theHold.Accept (GetString(IDS_CONNECT_EDGES));
...
bool RelaxGrip::GetText(int which,TSTR &string)
{
	...
	case eRelaxIterations:
		{
			string =GetString(IDS_ITERATIONS);
		}
		break;
	case eRelaxBoundaryPoints:
		{
			string = GetString(IDS_RELAX_HOLD_BOUNDARY_POINTS);
		}
		break;
	case eRelaxOuterPoints:
		{
			string = GetString(IDS_RELAX_HOLD_OUTER_POINTS);
		}
        break;
	default:
		return false;
...
bool RelaxGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eRelaxAmount:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_relax_amount, t) == TRUE ? true : false;
	case eRelaxIterations:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_relax_iters, t) == TRUE ? true : false;
	}
	return false;
}
...
bool RelaxGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eRelaxAmount:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_relax_amount, t, value.mFloat);
		break;
	case eRelaxIterations:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_relax_iters, t, value.mInt);
		break;
	case eRelaxBoundaryPoints:
		{
			int val = 0;
		    GetEditPolyMod()->getParamBlock()->GetValue (epm_relax_hold_boundary, t, val);
	...
	case eRelaxOuterPoints:
		{
			int val = 0;
			GetEditPolyMod()->getParamBlock()->GetValue (epm_relax_hold_outer, t, val);
			value.mbool = ( val == 0 ? false:true);
		}
		break;
	default:
	...
	case eRelaxAmount:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_relax_amount, t, fResetVal);
			theHold.Accept (GetString(IDS_RELAX));
		}
		break;
	case eRelaxIterations:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_relax_iters, t, iResetIterVal);
			theHold.Accept (GetString(IDS_RELAX));
	...
	case eRelaxBoundaryPoints:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_relax_hold_boundary, t, iResetVal);
			theHold.Accept (GetString(IDS_RELAX));
	...
	case eRelaxOuterPoints:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_relax_hold_outer, t, iResetVal);
			theHold.Accept (GetString(IDS_RELAX));
...
IBaseGrip::Type ChamferVerticesGrip::GetType(int which)
{
    ...
    case eChamferVerticesAmount:
        return IBaseGrip::eUniverse;
    case eChamferVerticesSegments:
        return IBaseGrip::eInt;
    case eChamferVerticesDepth:
        return IBaseGrip::eFloat;
    case eChamferVerticesOpen:
        return IBaseGrip::eToggle;
    case eChamferVerticesSmooth:
        return IBaseGrip::eToggle;
    case eChamferVerticesSmoothType:
        return IBaseGrip::eCombo;
    case eChamferVerticesSmoothThreshold:
        return IBaseGrip::eFloat;
    default:
        break;
    }
...
bool ChamferVerticesGrip::GetText(int which,TSTR &string)
{
    ...
    {
    case eChamferVerticesAmount:
        string = GetString(IDS_VERTEX_CHAMFER_AMOUNT);
        break;
    case eChamferVerticesSegments:
        string = GetString(IDS_VERTEX_CHAMFER_SEGMENTS);
        break;
    case eChamferVerticesDepth:
        string = GetString(IDS_VERTEX_CHAMFER_DEPTH);
        break;
    case eChamferVerticesOpen:
        string = GetString(IDS_OPEN_CHAMFER);
        break;
    case eChamferVerticesSmooth:
        string = GetString(IDS_VERTEX_CHAMFER_SMOOTH);
        break;
    case eChamferVerticesSmoothType:
        string = GetString(IDS_VERTEX_CHAMFER_SMOOTH_TYPE);
        break;
    case eChamferVerticesSmoothThreshold:
        string = GetString(IDS_VERTEX_CHAMFER_SMOOTH_THRESHOLD);
        break;
    default:
        return false;
...
bool ChamferVerticesGrip::GetResolvedIconName(int which,MSTR &string)
{
    ...
    case eChamferVerticesAmount:
        ...
        break;
    case eChamferVerticesSegments:
        string = CHAMFER_VERTICES_SEGMENTS_PNG;
        break;
    case eChamferVerticesDepth:
        string = CHAMFER_VERTICES_DEPTH_PNG;
        break;
    case eChamferVerticesOpen:
        string = CHAMFER_VERTICES_OPEN_PNG;
        break;
    case eChamferVerticesSmooth:
        string = CHAMFER_VERTICES_SMOOTH_PNG;
        break;
    case eChamferVerticesSmoothThreshold:
        string = CHAMFER_VERTICES_SMOOTH_THRESHOLD_PNG;
        break;
    default:
        return false;
    }
...
{
    BOOL hasKey = FALSE;
    IParamBlock2* params = (GetEditPolyMod())->getParamBlock();
    switch (which)
    {
    case eChamferVerticesAmount:
        hasKey = params->KeyFrameAtTimeByID(epm_chamfer_vertex, t);
        break;
    case eChamferVerticesSegments:
        hasKey = params->KeyFrameAtTimeByID(epm_vertex_chamfer_num_segments, t);
        break;
    case eChamferVerticesDepth:
        hasKey = params->KeyFrameAtTimeByID(epm_vertex_chamfer_depth, t);
        break;
    case eChamferVerticesSmoothThreshold:
        hasKey = params->KeyFrameAtTimeByID(epm_vertex_chamfer_smooth_threshold, t);
    default:
        break;
    }
...
{
    bool isCombo = false;
    if (which == eChamferVerticesSmoothType)
    {
        IBaseGrip::ComboLabel* smoothAllLabel = new IBaseGrip::ComboLabel();
        smoothAllLabel->mIcon = CHAMFER_VERTICES_SMOOTH_ALL_PNG;
        smoothAllLabel->mLabel = GetString(IDS_VERTEX_CHAMFER_SMOOTH_ALL);
        comboOptions.Append(1, &smoothAllLabel);
        IBaseGrip::ComboLabel* smoothChamfersLabel = new IBaseGrip::ComboLabel();
        smoothChamfersLabel->mIcon = CHAMFER_VERTICES_SMOOTH_CHAMFERS_PNG;
        smoothChamfersLabel->mLabel = GetString(IDS_VERTEX_CHAMFER_SMOOTH_CHAMFERS);
        comboOptions.Append(1, &smoothChamfersLabel);
        isCombo = true;
    }
...
bool ChamferVerticesGrip::GetRange(int which,IBaseGrip::GripValue &minRange, IBaseGrip::GripValue &maxRange)
{
    ...
    {
    case eChamferVerticesAmount:
        theDef = params->GetParamDef(epm_chamfer_vertex);
        break;
    case eChamferVerticesSegments:
        theDef = params->GetParamDef(epm_vertex_chamfer_num_segments);
        break;
    case eChamferVerticesDepth:
        theDef = params->GetParamDef(epm_vertex_chamfer_depth);
        break;
    case eChamferVerticesSmoothThreshold:
        theDef = params->GetParamDef(epm_vertex_chamfer_smooth_threshold);
        break;
    default:
        isFound = false;
        break;
...
bool ChamferVerticesGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
    ...
    {
    case eChamferVerticesAmount:
        params->GetValue(epm_chamfer_vertex, t, value.mFloat, interval);
        break;
    case eChamferVerticesSegments:
        params->GetValue(epm_vertex_chamfer_num_segments, t, value.mInt, interval);
        break;
    case eChamferVerticesDepth:
        params->GetValue(epm_vertex_chamfer_depth, t, value.mFloat, interval);
        break;
    case eChamferVerticesOpen:
        {
            value.mbool = false;
            BOOL isOpen = FALSE;
            params->GetValue(epm_chamfer_vertex_open, t, isOpen, interval);
            if (isOpen)
            {
                value.mbool = true;
            }
    ...
    case eChamferVerticesSmooth:
        ...
        break;
    case eChamferVerticesSmoothType:
        params->GetValue(epm_vertex_chamfer_smooth_type, t, value.mInt, interval);
        break;
    case eChamferVerticesSmoothThreshold:
        params->GetValue(epm_vertex_chamfer_smooth_threshold, t, value.mFloat, interval);
        break;
    default:
        isFound = false;
        break;
...
bool ChamferVerticesGrip::SetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
    ...
    case eChamferVerticesAmount:
        ...
        params->SetValue(epm_chamfer_vertex, t, value.mFloat);
        params->GetValue(epm_chamfer_vertex, t, value.mFloat, interval);
        break;
    case eChamferVerticesSegments:
        if (value.mInt < 0)
        ...
        params->SetValue(epm_vertex_chamfer_num_segments, t, value.mInt);
        params->GetValue(epm_vertex_chamfer_num_segments, t, value.mInt, interval);
        break;
    case eChamferVerticesDepth:
        if (value.mFloat < -0.5f)
        {
            value.mFloat = -0.5f;
        }
        ...
        params->SetValue(epm_vertex_chamfer_depth, t, value.mFloat);
        params->GetValue(epm_vertex_chamfer_depth, t, value.mFloat, interval);
        break;
    case eChamferVerticesOpen:
        {
            ...
            params->SetValue(epm_chamfer_vertex_open, t, isOpen);
            params->GetValue(epm_chamfer_vertex_open, t, isOpen, interval);
        }
        break;
    case eChamferVerticesSmooth:
        {
            ...
            params->SetValue(epm_vertex_chamfer_smooth, t, doSmooth);
            params->GetValue(epm_vertex_chamfer_smooth, t, doSmooth, interval);
            MaybeEnableControls();
        }
        break;
    case eChamferVerticesSmoothType:
        if (value.mInt < EP_EDGE_CHAMFER_SMOOTH_ALL)
        {
            value.mInt = EP_EDGE_CHAMFER_SMOOTH_ALL;
        }
        else if (value.mInt > EP_EDGE_CHAMFER_SMOOTH_CHAMFER)
        {
            value.mInt = EP_EDGE_CHAMFER_SMOOTH_CHAMFER;
        }

        params->SetValue(epm_vertex_chamfer_smooth_type, t, value.mInt);
        params->GetValue(epm_vertex_chamfer_smooth_type, t, value.mInt, interval);
        break;
    case eChamferVerticesSmoothThreshold:
        if (value.mFloat < 0.0f)
        ...
        else if (value.mFloat > 180.0f)
        {
            value.mFloat = 180.0f;
        }

        params->SetValue(epm_vertex_chamfer_smooth_threshold, t, value.mFloat);
        params->GetValue(epm_vertex_chamfer_smooth_threshold, t, value.mFloat, interval);
        break;
    default:
        isFound = false;
...
bool ChamferVerticesGrip::ResetValue(TimeValue t, int which)
{
    ...
    {
    case eChamferVerticesAmount:
        paramID = epm_chamfer_vertex;
        break;
    case eChamferVerticesSegments:
        paramID = epm_vertex_chamfer_num_segments;
        break;
    case eChamferVerticesDepth:
        paramID = epm_vertex_chamfer_depth;
        break;
    case eChamferVerticesOpen:
        paramID = epm_chamfer_vertex_open;
        break;
    case eChamferVerticesSmooth:
        paramID = epm_vertex_chamfer_smooth;
        break;
    case eChamferVerticesSmoothType:
        paramID = epm_vertex_chamfer_smooth_type;
        break;
    case eChamferVerticesSmoothThreshold:
        paramID = epm_vertex_chamfer_smooth_threshold;
        break;
    default:
        isFound = false;
        break;
    ...
    {
        IParamBlock2* params = (GetEditPolyMod())->getParamBlock();
        ParamDef theDef = params->GetParamDef(paramID);
        if (theDef.flags & P_HAS_DEFAULT)
        {
            theHold.Begin();
            Interval interval = FOREVER;
            ...
            case TYPE_WORLD:
                {
                    ...
                    params->SetValue(paramID, t, (theDef.def).f);
                    coreInterface->RedrawViews(coreInterface->GetTime(), REDRAW_INTERACTIVE);
                    params->GetValue(paramID, t, dummyVal, interval);
                }
                break;
            case TYPE_INT:
            case TYPE_BOOL:
                {
                    ...
                    params->SetValue(paramID, t, (theDef.def).i);
                    coreInterface->RedrawViews(coreInterface->GetTime(), REDRAW_INTERACTIVE);
                    params->GetValue(paramID, t, dummyVal, interval);
                }
                break;
...
bool ChamferVerticesGrip::GetResetValue(int which,IBaseGrip::GripValue &resetValue)
{
    ...
    if (isFound)
    {
        ...
        if (theDef.flags & P_HAS_DEFAULT)
        {
            ...
            case TYPE_FLOAT:
            case TYPE_WORLD:
                resetValue.mFloat = (theDef.def).f;
                break;
            case TYPE_INT:
                resetValue.mInt = (theDef.def).i;
                break;
            case TYPE_BOOL:
                resetValue.mbool = false;
                if ((theDef.def).i)
                {
                    resetValue.mbool = true;
                }
                break;
            default:
                isFound = false;
...
void ChamferVerticesGrip::SetUpUI()
{
    MaybeEnableControls();
    EditPolyModPolyOperationGrip::SetUpUI();
}

void ChamferVerticesGrip::MaybeEnableControls()
{
    IParamBlock2* params = (GetEditPolyMod())->getParamBlock();
    IGripManager* gripManager = GetIGripManager();

    // Enable smoothing controls only if smoothing is active
    BOOL doSmooth = FALSE;
    Interval interval = FOREVER;
    bool isSmoothingActive = false;
    params->GetValue(epm_vertex_chamfer_smooth, 0, doSmooth, interval);
    if (doSmooth)
    {
        isSmoothingActive = true;
    }

    gripManager->ActivateGripItem(eChamferVerticesSmoothType, isSmoothingActive);
    gripManager->ActivateGripItem(eChamferVerticesSmoothThreshold, isSmoothingActive);
}
...
IBaseGrip::Type ChamferEdgeGrip::GetType(int which)
{
	...
	case eChamferEdgeSegments:
		return IBaseGrip::eInt;
	case eChamferEdgeTension:
		return IBaseGrip::eFloat;
	case eChamferEdgeOpen:
		return IBaseGrip::eToggle;
	case eChamferEdgeInvert:
		return IBaseGrip::eToggle;
	case eChamferEdgeSmooth:
		return IBaseGrip::eToggle;
	case eChamferEdgeSmoothType:
		return IBaseGrip::eCombo;
	case eChamferEdgeSmoothThreshold:
		return IBaseGrip::eFloat;
	case eChamferEdgeMiteringType:
		return IBaseGrip::eCombo;
	case eChamferEdgeEndBias:
		return IBaseGrip::eFloat;
	case eChamferEdgeRadialBias:
		return IBaseGrip::eFloat;
	case eChamferEdgeDepth:
		return IBaseGrip::eFloat;
	}
	return IBaseGrip::eInt;
}
...
bool ChamferEdgeGrip::GetText(int which,TSTR &string)
{
	...
	case eChamferEdgeAmount:
		{
			string =GetString(IDS_AMOUNT);// + TSTR(": ") + TSTR(str);
		}
		break;
	case eChamferEdgeSegments:
		{
			string =GetString(IDS_SEGMENTS);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eChamferEdgeTension:
		{
			string = GetString(IDS_EDGE_CHAMFER_TENSION);
		}
		break;
	case eChamferEdgeOpen:
		{
			string = GetString(IDS_OPEN);
		}
		break;
	case eChamferEdgeInvert:
		{
			string =GetString(IDS_EDGE_CHAMFER_INVERT);
		}
		break;
	case eChamferEdgeSmooth:
		{
			string =GetString(IDS_EDGE_CHAMFER_SMOOTH);
		}
		break;
	case eChamferEdgeSmoothType:
		{
			string =GetString(IDS_EDGE_CHAMFER_SMOOTH_TYPE);
		}
		break;
	case eChamferEdgeSmoothThreshold:
		{
			string =GetString(IDS_EDGE_CHAMFER_SMOOTH_THRESHOLD);
		}
		break;
	case eChamferEdgeMiteringType:
		{
			int miteringType;
			GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_mitering_type, 0, miteringType);
			switch (miteringType)
			{
			case EP_EDGE_CHAMFER_QUAD_MITER:
				string = GetString(IDS_EDGE_CHAMFER_MITER_QUAD);
				break;
			case EP_EDGE_CHAMFER_TRI_MITER:
				string = GetString(IDS_EDGE_CHAMFER_MITER_TRI);
				break;
			case EP_EDGE_CHAMFER_UNIFORM_MITER:
				string = GetString(IDS_EDGE_CHAMFER_MITER_UNIFORM);
				break;
            case EP_EDGE_CHAMFER_RADIAL_MITER:
                string = GetString(IDS_EDGE_CHAMFER_MITER_RADIAL);
                break;
            case EP_EDGE_CHAMFER_PATCH_MITER:
                string = GetString(IDS_EDGE_CHAMFER_MITER_PATCH);
                break;
			default:
				string = GetString(IDS_EDGE_CHAMFER_MITERING_TYPE);
				break;
			}
	...
	case eChamferEdgeEndBias:
		{
			string = GetString(IDS_EDGE_CHAMFER_END_BIAS);
		}
		break;
	case eChamferEdgeRadialBias:
		{
			string = GetString(IDS_EDGE_CHAMFER_RADIAL_BIAS);
		}
		break;
	case eChamferEdgeDepth:
		{
			string = GetString(IDS_EDGE_CHAMFER_DEPTH);
		}
		break;
	default:
		return false;
...
DWORD ChamferEdgeGrip::GetCustomization(int which)
{
	...
	case eChamferEdgeSmoothType:
	case eChamferEdgeSmoothThreshold:
	//case eChamferEdgeQuadIntersections:
		word = (int)(IBaseGrip::eSameRow);
		break;
	}
	return word;
...
bool ChamferEdgeGrip::GetResolvedIconName(int which,MSTR &string)
{
	...
	case eChamferEdgeSegments:
		...
		break;
	case eChamferEdgeTension:
		string = CHAMFER_EDGE_TENSION_PNG;
		break;
	case eChamferEdgeOpen:
		string = CHAMFER_EDGE_OPEN_PNG;
		break;
	case eChamferEdgeInvert:
		string = CHAMFER_EDGE_INVERT_PNG;
		break;
	case eChamferEdgeSmooth:
		string =  CHAMFER_EDGE_SMOOTH_PNG;
		break;
	case eChamferEdgeSmoothThreshold:
		string =  CHAMFER_EDGE_SMOOTH_THRESHOLD_PNG;
		break;
	case eChamferEdgeEndBias:
		string = CHAMFER_EDGE_END_BIAS_PNG;
		break;
	case eChamferEdgeRadialBias:
		string = CHAMFER_EDGE_RADIAL_BIAS_PNG;
		break;
	case eChamferEdgeDepth:
		string = CHAMFER_EDGE_DEPTH_PNG;
		break;
	default:
		return false;
	}
...
bool ChamferEdgeGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eChamferEdgeAmount:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_chamfer_edge, t) == TRUE ? true : false;
	case eChamferEdgeTension:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_edge_chamfer_tension, t) == TRUE ? true : false;
	case eChamferEdgeSegments:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_edge_chamfer_segments, t) == TRUE ? true : false;
	case eChamferEdgeSmoothThreshold:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_edge_chamfer_smooth_threshold, t) == TRUE ? true : false;
	case eChamferEdgeEndBias:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_edge_chamfer_end_bias, t) == TRUE ? true : false;
	case eChamferEdgeRadialBias:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_edge_chamfer_radial_bias, t) == TRUE ? true : false;
	case eChamferEdgeDepth:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_edge_chamfer_depth, t) == TRUE ? true : false;
	}
	return false;
}
...
{
	IBaseGrip::ComboLabel* label = nullptr;
	TSTR string;
	...
	case eChamferEdgeSmoothType:
		label = new IBaseGrip::ComboLabel();
		string = CHAMFER_EDGE_SMOOTH_ALL_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_EDGE_CHAMFER_SMOOTH_ALL);
		comboOptions.Append(1, &label);
		label = new IBaseGrip::ComboLabel();
		string = CHAMFER_EDGE_SMOOTH_CHAMFERS_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_EDGE_CHAMFER_SMOOTH_CHAMFERS);
		comboOptions.Append(1, &label);
		return true;
	case eChamferEdgeMiteringType:
		label = new IBaseGrip::ComboLabel();
		string = CHAMFER_EDGE_MITERING_QUAD_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_EDGE_CHAMFER_MITER_QUAD);
		comboOptions.Append(1, &label);
		label = new IBaseGrip::ComboLabel();
		string = CHAMFER_EDGE_MITERING_TRI_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_EDGE_CHAMFER_MITER_TRI);
		comboOptions.Append(1, &label);
		label = new IBaseGrip::ComboLabel();
		string = CHAMFER_EDGE_MITERING_UNIFORM_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_EDGE_CHAMFER_MITER_UNIFORM);
		comboOptions.Append(1, &label);
		label = new IBaseGrip::ComboLabel();
		string = CHAMFER_EDGE_MITERING_RADIAL_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_EDGE_CHAMFER_MITER_RADIAL);
		comboOptions.Append(1, &label);
		label = new IBaseGrip::ComboLabel();
		string = CHAMFER_EDGE_MITERING_PATCH_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_EDGE_CHAMFER_MITER_PATCH);
		comboOptions.Append(1, &label);
		return true;
	default:
		break;
...
bool ChamferEdgeGrip::GetRange(int which,IBaseGrip::GripValue &minRange, IBaseGrip::GripValue &maxRange)
{
	...
	case eChamferEdgeSegments:
		...
		break;
	case eChamferEdgeTension:
		theDef = GetEditPolyMod()->getParamBlock()->GetParamDef(epm_edge_chamfer_tension);
		if(theDef.flags&P_HAS_RANGE)
		{
			minRange.mFloat = 0.0f;
			maxRange.mFloat = 1.0f;
		...
		break;
	case eChamferEdgeSmoothThreshold:
		theDef = GetEditPolyMod()->getParamBlock()->GetParamDef(epm_edge_chamfer_smooth_threshold);
		if(theDef.flags&P_HAS_RANGE)
		{
			minRange.mFloat = 0.0f;
			maxRange.mFloat = 180.0f;
		}
		break;
	case eChamferEdgeEndBias:
		theDef = GetEditPolyMod()->getParamBlock()->GetParamDef(epm_edge_chamfer_end_bias);
		if (theDef.flags & P_HAS_RANGE)
		{
			minRange.mFloat = 0.0f;
			maxRange.mFloat = 1.0f;
		...
		break;
	case eChamferEdgeRadialBias:
		theDef = GetEditPolyMod()->getParamBlock()->GetParamDef(epm_edge_chamfer_radial_bias);
		if (theDef.flags & P_HAS_RANGE)
		{
			minRange.mFloat = 0.0f;
			maxRange.mFloat = 1.0f;
		...
		break;
	case eChamferEdgeDepth:
		theDef = GetEditPolyMod()->getParamBlock()->GetParamDef(epm_edge_chamfer_depth);
		if (theDef.flags & P_HAS_RANGE)
		{
			minRange.mFloat = -0.5f;
			maxRange.mFloat = 1.0f;
...
bool ChamferEdgeGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eChamferEdgeAmount:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_chamfer_edge, t, value.mFloat);
		break;
	case eChamferEdgeSegments:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_segments, t, value.mInt);
		break;
	case eChamferEdgeTension:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_tension, t, value.mFloat);
		break;
	case eChamferEdgeOpen:
		{
			// need to go BOOL(int) to bool
            int val = 0;
		    GetEditPolyMod()->getParamBlock()->GetValue (epm_chamfer_edge_open, t, val);
			value.mbool = (val == 0 ? false: true);
		}
		break;
	case eChamferEdgeInvert:
		{
			int val = 0 ;
			GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_invert, t, val);
			value.mbool = ( val == 0 ? false:true );
		}
		break;
	case eChamferEdgeSmooth:
		{
			// need to go BOOL(int) to bool
            int val = 0;
		    GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_smooth, t, val);
			value.mbool = (val == 0 ? false: true);
		}
		break;
	case eChamferEdgeSmoothType:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_smooth_type, t, value.mInt);
		break;
	case eChamferEdgeSmoothThreshold:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_smooth_threshold, t, value.mFloat);
		break;
	case eChamferEdgeMiteringType:
		GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_mitering_type, t, value.mInt);
		break;
	case eChamferEdgeEndBias:
		GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_end_bias, t, value.mFloat);
		break;
	case eChamferEdgeRadialBias:
		GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_radial_bias, t, value.mFloat);
		break;
	case eChamferEdgeDepth:
		GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_depth, t, value.mFloat);
		break;
	default:
		return false;
	}
...
void ChamferEdgeGrip::MaybeEnableControls() {
	IGripManager *gripManager = GetIGripManager();
	...
	switch (miteringType)
	{
	case EP_EDGE_CHAMFER_UNIFORM_MITER: // Uniform, patch, and radial
	case EP_EDGE_CHAMFER_RADIAL_MITER:
	case EP_EDGE_CHAMFER_PATCH_MITER:
		gripManager->HideGripItem(eChamferEdgeTension, true);
		gripManager->ActivateGripItem(eChamferEdgeTension, false);
		gripManager->ActivateGripItem(eChamferEdgeEndBias, true);
		gripManager->ActivateGripItem(eChamferEdgeRadialBias, true);
		gripManager->HideGripItem(eChamferEdgeDepth, false);
		gripManager->ActivateGripItem(eChamferEdgeDepth, true);
		break;
	case EP_EDGE_CHAMFER_QUAD_MITER: // Quad
		gripManager->HideGripItem(eChamferEdgeTension, false);
		gripManager->ActivateGripItem(eChamferEdgeTension, true);
		gripManager->ActivateGripItem(eChamferEdgeEndBias, true);
		gripManager->ActivateGripItem(eChamferEdgeRadialBias, true);
		gripManager->HideGripItem(eChamferEdgeDepth, true);
		gripManager->ActivateGripItem(eChamferEdgeDepth, false);
		break;
	case EP_EDGE_CHAMFER_TRI_MITER: // Tri
		// Standard chamfer doesn't have these options
		gripManager->HideGripItem(eChamferEdgeTension, false);
		gripManager->ActivateGripItem(eChamferEdgeTension, false);
		gripManager->ActivateGripItem(eChamferEdgeEndBias, false);
		gripManager->ActivateGripItem(eChamferEdgeRadialBias, false);
		gripManager->HideGripItem(eChamferEdgeDepth, true);
		gripManager->ActivateGripItem(eChamferEdgeDepth, true);
		break;
	default:
	...
	// Only enable additional smoothing options if smoothing is turned on
	int smoothingOn;
	GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_smooth, 0, smoothingOn);
	bool enableSmoothingOptions = smoothingOn ? true : false;
	gripManager->ActivateGripItem(eChamferEdgeSmoothType,enableSmoothingOptions);
	gripManager->ActivateGripItem(eChamferEdgeSmoothThreshold,enableSmoothingOptions);
}

//need to activate grips -- Certain controls aren't available for standard chamfering!
void ChamferEdgeGrip::SetUpUI()
{
	MaybeEnableControls();
...
bool ChamferEdgeGrip::SetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	case eChamferEdgeTension:
		...
		if (value.mFloat > 1.0f)
			value.mFloat = 1.0f;
		GetEditPolyMod()->getParamBlock()->SetValue(epm_edge_chamfer_tension, t, value.mFloat);
		GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_tension, t, value.mFloat);
		break;
	case eChamferEdgeOpen:
		{
		    ...
		    GetEditPolyMod()->getParamBlock()->GetValue (epm_chamfer_edge_open, t, val);
			GetEditPolyMod()->getParamBlock()->EnableNotifications(false);
			GetEditPolyMod()->getParamBlock()->SetValue(epm_edge_chamfer_invert, t, false);
			IGripManager* gripManager = GetIGripManager();
			gripManager->ResetGripUI(eChamferEdgeInvert);
			GetEditPolyMod()->getParamBlock()->EnableNotifications(true);
		}
		break;
	case eChamferEdgeInvert:
		{
			int val = (int)value.mbool ;
			GetEditPolyMod()->getParamBlock()->SetValue (epm_edge_chamfer_invert, t, val);
		    GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_invert, t, val);
			GetEditPolyMod()->getParamBlock()->EnableNotifications(false);
			GetEditPolyMod()->getParamBlock()->SetValue(epm_chamfer_edge_open, t, false);
			IGripManager* gripManager = GetIGripManager();
			gripManager->ResetGripUI(eChamferEdgeOpen);
			GetEditPolyMod()->getParamBlock()->EnableNotifications(true);
		}
		break;
	case eChamferEdgeSmooth:
		{
			int val =(int)value.mbool ;
			GetEditPolyMod()->getParamBlock()->SetValue (epm_edge_chamfer_smooth, t, val);
		    GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_smooth, t, val);
			MaybeEnableControls();
		}
		break;
	case eChamferEdgeSmoothType:
		if(value.mInt>EP_EDGE_CHAMFER_SMOOTH_CHAMFER) //go back to zero
			value.mInt = EP_EDGE_CHAMFER_SMOOTH_ALL;
		if(value.mInt<EP_EDGE_CHAMFER_SMOOTH_ALL)
			value.mInt = EP_EDGE_CHAMFER_SMOOTH_CHAMFER;
		GetEditPolyMod()->getParamBlock()->SetValue (epm_edge_chamfer_smooth_type, t, value.mInt);
		GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_smooth_type, t, value.mInt);
		break;
	case eChamferEdgeSmoothThreshold:
		if( value.mFloat < 0.0f )
			value.mFloat = 0.0f;
		if( value.mFloat > 180.0f )
			value.mFloat = 180.0f;
		GetEditPolyMod()->getParamBlock()->SetValue (epm_edge_chamfer_smooth_threshold, t, value.mFloat);
		GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_smooth_threshold, t, value.mFloat);
		break;
	case eChamferEdgeMiteringType:
		if (value.mInt > EP_EDGE_CHAMFER_PATCH_MITER) // go back to zero
			value.mInt = EP_EDGE_CHAMFER_QUAD_MITER;
		if (value.mInt < EP_EDGE_CHAMFER_QUAD_MITER)
			value.mInt = EP_EDGE_CHAMFER_PATCH_MITER;
		GetEditPolyMod()->getParamBlock()->SetValue(epm_edge_chamfer_mitering_type, t, value.mInt);
		GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_mitering_type, t, value.mInt);
	...
	case eChamferEdgeEndBias:
		...
		if (value.mFloat > 1.0f)
			value.mFloat = 1.0f;
		GetEditPolyMod()->getParamBlock()->SetValue(epm_edge_chamfer_end_bias, t, value.mFloat);
		GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_end_bias, t, value.mFloat);
		break;
	case eChamferEdgeRadialBias:
		if (value.mFloat < 0.0f)
		...
		if (value.mFloat > 1.0f)
			value.mFloat = 1.0f;
		GetEditPolyMod()->getParamBlock()->SetValue(epm_edge_chamfer_radial_bias, t, value.mFloat);
		GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_radial_bias, t, value.mFloat);
		break;
	case eChamferEdgeDepth:
		if (value.mFloat < -0.5f)
		...
		if (value.mFloat > 1.0f)
			value.mFloat = 1.0f;
		GetEditPolyMod()->getParamBlock()->SetValue(epm_edge_chamfer_depth, t, value.mFloat);
		GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_depth, t, value.mFloat);
		break;
	default:
		return false;
...
bool ChamferEdgeGrip::ResetValue(TimeValue t, int which)
{
	...
	{
	case eChamferEdgeAmount:
		paramID = epm_chamfer_edge;
		break;
	case eChamferEdgeSegments:
		paramID = epm_edge_chamfer_segments;
		break;
	case eChamferEdgeTension:
		paramID = epm_edge_chamfer_tension;
		break;
	case eChamferEdgeOpen:
		paramID = epm_chamfer_edge_open;
		break;
	case eChamferEdgeInvert:
		paramID = epm_edge_chamfer_invert;
		break;
	case eChamferEdgeSmooth:
		paramID = epm_edge_chamfer_smooth;
		break;
	case eChamferEdgeSmoothType:
		paramID = epm_edge_chamfer_smooth_type;
		break;
	case eChamferEdgeSmoothThreshold:
		paramID = epm_edge_chamfer_smooth_threshold;
		break;
	case eChamferEdgeMiteringType:
		paramID = epm_edge_chamfer_mitering_type;
		break;
	case eChamferEdgeEndBias:
		paramID = epm_edge_chamfer_end_bias;
		break;
	case eChamferEdgeRadialBias:
		paramID = epm_edge_chamfer_radial_bias;
		break;
	case eChamferEdgeDepth:
		paramID = epm_edge_chamfer_depth;
		break;
	default:
		isFound = false;
		break;
...
{
	if(mEdge1Picked == (LPCTSTR)GetString(IDS_BRIDGE_PICKING_EDGE_1))
		string =  GetString(IDS_BRIDGE_PICKING_EDGE_1);
	else if (mEdge2Picked ==  (LPCTSTR)GetString(IDS_BRIDGE_PICKING_EDGE_2))
		string = GetString(IDS_BRIDGE_PICKING_EDGE_2);
	else
		string =GetString(IDS_BRIDGE_EDGES);
}
...
void BridgeEdgeGrip::SetEdge1Picked(int num)
{
	TSTR buf;
	buf.printf(_T("%d"), num);
	mEdge1Picked = TSTR(GetString(IDS_EDGE)) + _T(" ") + buf;
...
void BridgeEdgeGrip::SetEdge2Picked(int num)
{
	TSTR buf;
	buf.printf(_T("%d"), num);
	mEdge2Picked = TSTR(GetString(IDS_EDGE)) + _T(" ") + buf;
...
{
	mEdge1Picked =  GetString(IDS_BRIDGE_PICKING_EDGE_1);
...
{
	mEdge1Picked =  GetString(IDS_BRIDGE_PICK_EDGE_1);
	mPick1Icon = PICK_EMPTY_PNG;
	GetIGripManager()->ResetGripUI((int)eBridgeEdgePick1);
}
...
{
	mEdge2Picked =  GetString(IDS_BRIDGE_PICKING_EDGE_2);
...
{
	mEdge2Picked =  GetString(IDS_BRIDGE_PICK_EDGE_2);
	mPick2Icon = PICK_EMPTY_PNG;
	GetIGripManager()->ResetGripUI((int)eBridgeEdgePick2);
}
...
bool BridgeEdgeGrip::GetText(int which,TSTR &string)
{
	...
	case eBridgeEdgeSmooth:
		{
			string =GetString(IDS_SMOOTH);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eBridgeEdgeAdjacent:
		{
			string =GetString(IDS_BRIDGE_ADJACENT);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eBridgeEdgeReverseTri:
		{
			string =GetString(IDS_BRIDGE_REVERSE_TRI);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eBridgeEdgeType:
		{
			//Group Normals, 1 for Local Normals, 2 for By Polygon 
			int type;
			GetEditPolyMod()->getParamBlock()->GetValue (epm_bridge_selected, 0, type);
			switch(type)
			{
			case 0:
				string =  GetString(IDS_BRIDGE_SPECIFIC_EDGES);
				break;
			case 1:
				string =  GetString(IDS_BRIDGE_EDGE_SELECTION);
				break;
...
bool BridgeEdgeGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eBridgeEdgeSegments:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_bridge_segments, t) == TRUE ? true : false;
	case eBridgeEdgeSmooth:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_bridge_smooth_thresh, t) == TRUE ? true : false;
	case eBridgeEdgeAdjacent:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_bridge_adjacent, t) == TRUE ? true : false;
	}
	return false;
...
bool BridgeEdgeGrip::GetComboOptions(int which, Tab<IBaseGrip::ComboLabel*> &comboOptions)
{
	if(which== eBridgeEdgeType)
	{
		...
		label->mIcon = string;
		label->mLabel = GetString(IDS_BRIDGE_SPECIFIC_EDGES);
		comboOptions.Append(1,&label);
		label = new IBaseGrip::ComboLabel();
		string = BRIDGE_SELECTED_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_BRIDGE_EDGE_SELECTION);
		comboOptions.Append(1,&label);
		return true;
	}
	return false;
...
bool BridgeEdgeGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eBridgeEdgeType:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_bridge_selected, t, value.mInt);
		break;
	case eBridgeEdgeSegments:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_bridge_segments, t, value.mInt);
		break;
	case eBridgeEdgeSmooth:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_bridge_smooth_thresh, t, value.mFloat);
		break;
	case eBridgeEdgeAdjacent:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_bridge_adjacent, t, value.mFloat);
		break;
	case eBridgeEdgeReverseTri:
		{
			int val =0;
			GetEditPolyMod()->getParamBlock()->GetValue (epm_bridge_reverse_triangle, t, val);
			value.mbool = (val == 0 ? false:true);
		}
		break;
	case eBridgeEdgePick1:
...
{
	int val;
	GetEditPolyMod()->getParamBlock()->GetValue(epm_bridge_selected, GetCOREInterface()->GetTime(),val);
	if( 1 == val )
	{
	...
	case eBridgeEdgeType:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_bridge_selected, t, iResetVal);
			theHold.Accept (GetString(IDS_BRIDGE_EDGES));
		}
		break;
	case eBridgeEdgeSegments:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_bridge_segments, t, iResetSegVal);
			theHold.Accept (GetString(IDS_BRIDGE_EDGES));
		}
		break;
	case eBridgeEdgeSmooth:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_bridge_smooth_thresh, t, fResetVal);
			theHold.Accept (GetString(IDS_BRIDGE_EDGES));
	...
	case eBridgeEdgeAdjacent:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_bridge_adjacent, t, fResetVal);
			theHold.Accept (GetString(IDS_BRIDGE_EDGES));
	...
	case eBridgeEdgeReverseTri:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_bridge_reverse_triangle, t, iResetVal);
			theHold.Accept (GetString(IDS_BRIDGE_EDGES));
...
{
	if(mEdge1Picked == (LPCTSTR)GetString(IDS_BRIDGE_PICKING_BORDER_1))
		string = GetString(IDS_BRIDGE_PICKING_BORDER_1);
	else if(mEdge2Picked ==  (LPCTSTR)GetString(IDS_BRIDGE_PICKING_BORDER_2))
		string =  GetString(IDS_BRIDGE_PICKING_BORDER_2);
	else
		string =GetString(IDS_BRIDGE_BORDERS);
}
...
{
	mEdge1Picked =  GetString(IDS_BRIDGE_PICKING_BORDER_1);
	mPick1Icon = PICK_EMPTY_PNG;
	GetIGripManager()->ResetGripUI((int)eBridgeBorderPick1);
}
...
{
	mEdge1Picked =  GetString(IDS_BRIDGE_BORDER_PICK_1);
	mPick1Icon = PICK_EMPTY_PNG;
	GetIGripManager()->ResetGripUI((int)eBridgeBorderPick1);
}
...
{
	mEdge2Picked =  GetString(IDS_BRIDGE_PICKING_BORDER_2);
	mPick2Icon = PICK_EMPTY_PNG;
	GetIGripManager()->ResetGripUI((int)eBridgeBorderPick2);
}
...
{
	mEdge2Picked =  GetString(IDS_BRIDGE_BORDER_PICK_2);
	mPick2Icon = PICK_EMPTY_PNG;
	GetIGripManager()->ResetGripUI((int)eBridgeBorderPick2);
}
...
bool BridgeBorderGrip::GetText(int which,TSTR &string)
{
	...
	case eBridgeBorderType:
		{
			...
			{
			case 0:
				string =  GetString(IDS_BRIDGE_SPECIFIC_BORDERS);
				break;
			case 1:
				string =  GetString(IDS_BRIDGE_BORDER_SELECTION);
				break;
	...
	case eBridgeBorderTaper:
		{
			string =GetString(IDS_TAPER);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eBridgeBorderBias:
		{
	...
	case eBridgeBorderTwist1:
		{
			string =GetString(IDS_TWIST1);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eBridgeBorderTwist2:
		{
			string =GetString(IDS_TWIST2);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eBridgeBorderPick1:
		{
...
bool BridgeBorderGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	case eBridgeBorderSegments:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_bridge_segments, t) == TRUE ? true : false;
	case eBridgeBorderTaper:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_bridge_taper, t) == TRUE ? true : false;
	case eBridgeBorderBias:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_bridge_bias, t) == TRUE ? true : false;
	case eBridgeBorderSmooth:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_bridge_smooth_thresh, t) == TRUE ? true : false;
	case eBridgeBorderTwist1:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_bridge_twist_1, t) == TRUE ? true : false;
	case eBridgeBorderTwist2:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_bridge_twist_2, t) == TRUE ? true : false;
...
bool BridgeBorderGrip::GetComboOptions(int which, Tab<IBaseGrip::ComboLabel*> &comboOptions)
{
	if(which== eBridgeBorderType)
	{
		...
		TSTR string = BRIDGE_SPECIFIC_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_BRIDGE_SPECIFIC_BORDERS);
		comboOptions.Append(1,&label);
		label = new IBaseGrip::ComboLabel();
		string = BRIDGE_SELECTED_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_BRIDGE_BORDER_SELECTION);
		comboOptions.Append(1,&label);
		return true;
	}
	return false;
...
bool BridgeBorderGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	case eBridgeBorderSegments:
		...
		break;
	case eBridgeBorderTaper:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_bridge_taper, t, value.mFloat);
		break;
	case eBridgeBorderBias:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_bridge_bias, t, value.mFloat);
		break;
	case eBridgeBorderSmooth:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_bridge_smooth_thresh, t, value.mFloat);
		break;
	case eBridgeBorderTwist1:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_bridge_twist_1, t, value.mInt);
		break;
	case eBridgeBorderTwist2:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_bridge_twist_2, t, value.mInt);
		break;
	case eBridgeBorderPick1:
		{
			//value.mbool, 0 means not in pick mode, 1 does.
	...
	case eBridgeBorderTaper:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_bridge_taper, t, fResetVal);
			theHold.Accept (GetString(IDS_BRIDGE_BORDERS));
		}
		break;
	case eBridgeBorderBias:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_bridge_bias, t, fResetVal);
			theHold.Accept (GetString(IDS_BRIDGE_BORDERS));
		}
		break;
	case eBridgeBorderSmooth:
	...
	case eBridgeBorderTwist1:
		{
		    ...
		    GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_bridge_twist_1, t, iResetTwistVal);
			theHold.Accept (GetString(IDS_BRIDGE_BORDERS));
		}
		break;
	case eBridgeBorderTwist2:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_bridge_twist_2, t, iResetTwistVal);
			theHold.Accept (GetString(IDS_BRIDGE_BORDERS));
		}
		break;
	default:
...
void BridgePolygonGrip::GetGripName(TSTR &string)
{
	...
	if(mPoly1Picked == (LPCTSTR)GetString(IDS_BRIDGE_PICKING_POLYGON_1))
		string = GetString(IDS_BRIDGE_PICKING_POLYGON_1);
	else if(mPoly2Picked == (LPCTSTR)GetString(IDS_BRIDGE_PICKING_POLYGON_2))
		string = GetString(IDS_BRIDGE_PICKING_POLYGON_2);
	else
		string =GetString(IDS_BRIDGE_POLYGONS);
}
...
void BridgePolygonGrip::SetPoly1Picked(int num)
{
	TSTR buf;
	buf.printf(_T("%d"), num);
	mPoly1Picked = TSTR(GetString(IDS_FACE)) + _T(" ") + buf;
	mPick1Icon = PICK_FULL_PNG;
...
void BridgePolygonGrip::SetPoly2Picked(int num)
{
	TSTR buf;
	buf.printf(_T("%d"), num);
	mPoly2Picked = TSTR(GetString(IDS_FACE)) + _T(" ") + buf;
	mPick2Icon = PICK_FULL_PNG;
...
{
	mPoly1Picked =  GetString(IDS_BRIDGE_PICKING_POLYGON_1);
	mPick1Icon = PICK_EMPTY_PNG;
	GetIGripManager()->ResetGripUI((int)eBridgePolygonPick1);
}
...
{
	mPoly1Picked =  GetString(IDS_BRIDGE_PICK_POLYGON_1);
	mPick1Icon = PICK_EMPTY_PNG;
	GetIGripManager()->ResetGripUI((int)eBridgePolygonPick1);
}
...
{
	mPoly2Picked =  GetString(IDS_BRIDGE_PICKING_POLYGON_2);
	mPick2Icon = PICK_EMPTY_PNG;
	GetIGripManager()->ResetGripUI((int)eBridgePolygonPick2);
}
...
{
	mPoly2Picked =  GetString(IDS_BRIDGE_PICK_POLYGON_2);
	mPick2Icon = PICK_EMPTY_PNG;
	GetIGripManager()->ResetGripUI((int)eBridgePolygonPick2);
}
	...
	case eBridgePolygonType:
		{
			...
			{
			case 0:
				string =  GetString(IDS_BRIDGE_SPECIFIC_POLYGONS);
				break;
			case 1:
				string =  GetString(IDS_BRIDGE_POLYGON_SELECTION);
				break;
...
bool BridgePolygonGrip::GetComboOptions(int which, Tab<IBaseGrip::ComboLabel*> &comboOptions)
{
	if(which== eBridgePolygonType)
	{
		...
		TSTR string = BRIDGE_SPECIFIC_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_BRIDGE_SPECIFIC_POLYGONS);
		comboOptions.Append(1,&label);
		label = new IBaseGrip::ComboLabel();
		string = BRIDGE_SELECTED_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_BRIDGE_POLYGON_SELECTION);
		comboOptions.Append(1,&label);
		return true;
	}
	return false;
...
void HingeGrip::GetGripName(TSTR &string)
{
	...
	if(mEdgePicked == (LPCTSTR)GetString(IDS_PICKING_HINGE_EDGE))
		string = mEdgePicked;
	else
		string =GetString(IDS_HINGE);
}
...
{
	mEdgePicked =  GetString(IDS_PICKING_HINGE_EDGE);
...
void HingeGrip::SetEdgePicked(int num)
{
	MSTR buf;
	if (mList.Count() == 1) 
		buf.printf (_T("%s %d"), GetString (IDS_EDGE), num);
	else 
...
{
	mEdgePicked =  GetString(IDS_PICK_HINGE_EDGE);
	mPickIcon = PICK_EMPTY_PNG;
	GetIGripManager()->ResetGripUI((int)eHingePick);
}
...
bool HingeGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eHingeAngle:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_hinge_angle, t) == TRUE ? true : false;
	case eHingeSegments:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_hinge_segments, t) == TRUE ? true : false;
	}
	return false;
}
...
bool HingeGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	case eHingeAngle:
		{
			float valueRad = 0.0f;
			GetEditPolyMod()->getParamBlock()->GetValue (epm_hinge_angle, t, valueRad);
			value.mFloat = RadToDeg(valueRad);
		}
		break;
	case eHingeSegments:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_hinge_segments, t, value.mInt);
		break;
	case eHingePick:
		{
			//value.mbool, 0 means not in pick mode, 1 does.
	...
	case eHingeAngle:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_hinge_angle, t, fResetVal);
			theHold.Accept (GetString(IDS_HINGE));
		}
		break;
	case eHingeSegments:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_hinge_segments, t, iResetSegVal);
			theHold.Accept (GetString(IDS_HINGE));
...
{
	if(mSplinePicked == (LPCTSTR)GetString(IDS_PICKING_SPLINE))
		string = GetString(IDS_PICKING_SPLINE);
	else
		string =GetString(IDS_EXTRUDE_ALONG_SPLINE);
}

void ExtrudeAlongSplineGrip::SetSplinePickModeStarted()
{
	mSplinePicked = GetString(IDS_PICKING_SPLINE);
	mPickIcon = PICK_EMPTY_PNG;
...
{
	mSplinePicked = GetString(IDS_PICK_SPLINE);
	mPickIcon = PICK_EMPTY_PNG;
	GetIGripManager()->ResetGripUI((int)eExtrudeAlongSplinePickSpline);
}
...
bool ExtrudeAlongSplineGrip::GetText(int which,TSTR &string)
{
	...
	case eExtrudeAlongSplineTaperAmount:
		{
			string =GetString(IDS_TAPER_AMOUNTS);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eExtrudeAlongSplineTaperCurve:
		{
			string =GetString(IDS_TAPER_CURVE);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eExtrudeAlongSplineTwist:
		{
			string =GetString(IDS_TWIST);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eExtrudeAlongSplineAlignToFaceNormal:
		{
			string =GetString(IDS_ALIGN_TO_FACE_NORMAL);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eExtrudeAlongSplineRotation:
		{
			string =GetString(IDS_ROTATION);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eExtrudeAlongSplinePickSpline:
		{
...
bool ExtrudeAlongSplineGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eExtrudeAlongSplineSegments:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_extrude_spline_segments, t) == TRUE ? true : false;
	case eExtrudeAlongSplineTaperAmount:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_extrude_spline_taper, t) == TRUE ? true : false;
	case eExtrudeAlongSplineTaperCurve:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_extrude_spline_taper_curve, t) == TRUE ? true : false;
	case eExtrudeAlongSplineTwist:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_extrude_spline_twist, t) == TRUE ? true : false;
	case eExtrudeAlongSplineRotation:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_extrude_spline_rotation, t) == TRUE ? true : false;
	}
	return false;
}
...
bool ExtrudeAlongSplineGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eExtrudeAlongSplineSegments:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_extrude_spline_segments, t, value.mInt);
		break;
	case eExtrudeAlongSplineTaperAmount:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_extrude_spline_taper, t, value.mFloat);
		break;
	case eExtrudeAlongSplineTaperCurve:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_extrude_spline_taper_curve, t, value.mFloat);
		break;
	case eExtrudeAlongSplineTwist:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_extrude_spline_twist, t, value.mFloat);
		break;
	case eExtrudeAlongSplineAlignToFaceNormal:
		{
			//need to go BOOL (int) to bool
			int val =0;
			GetEditPolyMod()->getParamBlock()->GetValue (epm_extrude_spline_align, t, val);
			value.mbool =  ( val==0 ? false :true);
		}
		break;
	case eExtrudeAlongSplineRotation:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_extrude_spline_rotation, t, value.mFloat);
		break;
	case eExtrudeAlongSplinePickSpline:
		{
...
{
	int val;
	GetEditPolyMod()->getParamBlock()->GetValue (epm_extrude_spline_align,  GetCOREInterface()->GetTime(), val);
	if(!val)
	{
	...
	else
	{
		...
		GetEditPolyMod()->getParamBlock()->GetValue (epm_extrude_spline_node, TimeValue(0), splineNode);
		if (splineNode) 
	...
	case eExtrudeAlongSplineSegments:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_spline_segments, t, iResetSegVal);
			theHold.Accept (GetString(IDS_EXTRUDE_ALONG_SPLINE));
		}
		break;
	case eExtrudeAlongSplineTaperAmount:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_spline_taper, t, fResetVal);
			theHold.Accept (GetString(IDS_EXTRUDE_ALONG_SPLINE));
	...
	case eExtrudeAlongSplineTaperCurve:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_spline_taper_curve, t, fResetVal);
			theHold.Accept (GetString(IDS_EXTRUDE_ALONG_SPLINE));
	...
	case eExtrudeAlongSplineTwist:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_spline_twist, t, fResetVal);
			theHold.Accept (GetString(IDS_EXTRUDE_ALONG_SPLINE));
	...
	case eExtrudeAlongSplineAlignToFaceNormal:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_spline_align, t, iResetVal);
			theHold.Accept (GetString(IDS_EXTRUDE_ALONG_SPLINE));
	...
	case eExtrudeAlongSplineRotation:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_spline_rotation, t,fResetVal);
			theHold.Accept (GetString(IDS_EXTRUDE_ALONG_SPLINE));
...
{
	string =GetString(IDS_WELD_VERTS);
}
...
bool WeldVerticesGrip::GetText(int which,TSTR &string)
{
	...
	case eWeldVerticesWeldThreshold:
		{
			string =GetString(IDS_WELDTHRESHOLD);// + TSTR(": ") + TSTR(str);
		}
		break;
	case eWeldVerticesNum:
		{
			string = TSTR(GetString(IDS_WELD_BEFORE)) + _T(": ") + 
				mStrBefore + _T(", ") + GetString(IDS_WELD_AFTER) +  _T(": ") + 
				mStrAfter;
		}
		break;
	default:
...
bool WeldVerticesGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eWeldVerticesWeldThreshold:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_weld_vertex_threshold, t) == TRUE ? true : false;
	}
	return false;
}
...
bool WeldVerticesGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eWeldVerticesWeldThreshold:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_weld_vertex_threshold, t, value.mFloat);
		break;
	default:
		return false;
	}
	...
	case eWeldVerticesWeldThreshold:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_weld_vertex_threshold, t, fResetVal);
			theHold.Accept (GetString(IDS_WELD_VERTS));
		}
		break;
	default:
...
{
	string =GetString(IDS_WELD_EDGES);
}
...
bool WeldEdgesGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eWeldEdgesWeldThreshold:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_weld_edge_threshold, t) == TRUE ? true : false;
	}
	return false;
}
...
bool WeldEdgesGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eWeldEdgesWeldThreshold:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_weld_edge_threshold, t, value.mFloat);
		break;
	default:
		return false;
	}
	...
	case eWeldEdgesWeldThreshold:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_weld_edge_threshold, t, fResetVal);
			theHold.Accept (GetString(IDS_WELD_VERTS));
		}
		break;
	default:
...
{
	string =GetString(IDS_MESHSMOOTH);
}
...
bool MSmoothGrip::GetText(int which,TSTR &string)
{
	...
	case eMSmoothSmoothness:
		{
			string =GetString(IDS_SMOOTHNESS);// + TSTR(": ") + TSTR(str);
		}
		break;
	case eMSmoothSmoothingGroups:
		{
			string =GetString(IDS_SMOOTHING_GROUPS);//+ TSTR(": ") + TSTR(str);
		}
		break;
	case eMSmoothMaterials:
		{
			string =GetString(IDS_MATERIALS);//+ TSTR(": ") + TSTR(str);
		}
		break;
	default:
		return false;
...
bool MSmoothGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eMSmoothSmoothness:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_ms_smoothness, t) == TRUE ? true : false;
	}
	return false;
}
...
bool MSmoothGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eMSmoothSmoothness:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_ms_smoothness, t, value.mFloat);
		break;
	case eMSmoothSmoothingGroups:
		{
			int val = 0;
			GetEditPolyMod()->getParamBlock()->GetValue (epm_ms_sep_smooth, t, val);
			value.mbool = (val == 0 ? false:true );
		}
		break;
	case eMSmoothMaterials:
		{
			int val = 0;
			GetEditPolyMod()->getParamBlock()->GetValue (epm_ms_sep_mat, t, val);
			value.mbool = (val == 0 ? false:true );
		}
		break;
	default:
	...
	case eMSmoothSmoothness:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_ms_smoothness, t, fResetVal);
			theHold.Accept (GetString(IDS_MESHSMOOTH));
		}
		break;
	case eMSmoothSmoothingGroups:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_ms_sep_smooth,t, iResetVal);
			theHold.Accept (GetString(IDS_MESHSMOOTH));
	...
	case eMSmoothMaterials:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_ms_sep_mat, t, iResetVal);
			theHold.Accept (GetString(IDS_MESHSMOOTH));
...
{
	string =GetString(IDS_TESSELLATE);
}
...
bool TessellateGrip::GetText(int which,TSTR &string)
{
	...
	case eTessellateType:
		{
			int type;
			GetEditPolyMod()->getParamBlock()->GetValue (epm_tess_type, 0, type);
			switch(type)
			{
			case 0:
				string =  GetString(IDS_EDGE);
				break;
			case 1:
				string =  GetString(IDS_FACE);
				break;
	...
	case eTessellateTension:
		{
			string =GetString(IDS_TESS_TENSION);
		}
		break;
	default:
		return false;
...
bool TessellateGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	{
	case eTessellateTension:
		return GetEditPolyMod()->getParamBlock()->KeyFrameAtTimeByID(epm_tess_tension, t) == TRUE ? true : false;
	}
	return false;
}
...
bool TessellateGrip::GetComboOptions(int which, Tab<IBaseGrip::ComboLabel*> &comboOptions)
{
	if(which== eTessellateType)
	{
		...
		label->mIcon = string;
		label->mLabel = GetString(IDS_EDGE);
		comboOptions.Append(1,&label);
		label = new IBaseGrip::ComboLabel();
		string = TESSELLATE_FACE_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_FACE);
		comboOptions.Append(1,&label);
		return true;
	}
	return false;
...
bool TessellateGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eTessellateType:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_tess_type, t, value.mInt);
		break;
	case eTessellateTension:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_tess_tension, t, value.mFloat);
		break;
	default:
		return false;
	}
...
{
	int val;
	GetEditPolyMod()->getParamBlock()->GetValue (epm_tess_type,  GetCOREInterface()->GetTime(), val);
	if( 1 == val )
	...
	case eTessellateType:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_tess_type, t, iResetVal);
			theHold.Accept (GetString(IDS_TESSELLATE));
		}
		break;
	case eTessellateTension:
		{
			...
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_tess_tension, t, fResetVal);
			theHold.Accept (GetString(IDS_TESSELLATE));
		}
		break;
	default:
