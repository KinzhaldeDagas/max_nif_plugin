...
#include "../editablepoly/PolyLoopSelection.h"
#include "../editablepoly/SubObjectSelectionUtils.h"
#include <vector>
#include <ICUIMouseConfigManager.h>
#include <WaitingStatus.h>

static GenSubObjType SOT_Vertex(NULL, EditPolyIcon::stackVertex, 0);
static GenSubObjType SOT_Edge(NULL, EditPolyIcon::stackEdge, 0);
static GenSubObjType SOT_Border(NULL, EditPolyIcon::stackBorder, 0);
static GenSubObjType SOT_Face(NULL, EditPolyIcon::stackPolygon, 0);
static GenSubObjType SOT_Element(NULL, EditPolyIcon::stackElement, 0);
...
SubobjectPickMode*			EditPolyMod::subobjectPickMode	= NULL;
PointToPointMode*			EditPolyMod::pointToPointMode	= NULL;

// Initialize static mapping array between SDK-exposed grip type and internal type from EPolyManipulatorGrip
const EPolyManipulatorGrip::ManipulateGrips EditPolyMod::m_gripTypeMap[EPolyMod13::EditPolyManipulatorGripCount] = {EPolyManipulatorGrip::eSoftSelFalloff, EPolyManipulatorGrip::eSoftSelPinch, EPolyManipulatorGrip::eSoftSelBubble,
                                                                                                                    EPolyManipulatorGrip::eSetFlow, EPolyManipulatorGrip::eLoopShift, EPolyManipulatorGrip::eRingShift,
                                                                                                                    EPolyManipulatorGrip::eEdgeCrease, EPolyManipulatorGrip::eEdgeWeight,
                                                                                                                    EPolyManipulatorGrip::eVertexCrease, EPolyManipulatorGrip::eVertexWeight};

extern bool HasAdjacentSelection(MNMesh* mm, int subObjectLevel, int subObjectIndex, bool findRing, bool& isRing);

// Send a warning message
static void WarnCollapsedSave()
{
    bool doWarning = EditPolyMod::GetCollapsedSaveWarning();
    if (doWarning)
    {
        TSTR warningTitle = GetString(IDS_EDIT_POLY_MOD);
        TSTR warningMsg = GetString(IDS_SAVE_COLLAPSED_WARNING);
        Interface* coreInterface = GetCOREInterface();
        LogSys* sysLog = coreInterface->Log();
        sysLog->LogEntry(SYSLOG_WARN, DISPLAY_DIALOG, warningTitle, warningMsg);
        EditPolyMod::SetCollapsedSaveWarning(false);
    }
}
...
EditPolyMod::EditPolyMod()
	: m_editPolyVersion(EDIT_POLY_2021)
	...
	, mPreserveMapSettings(true,false)
	, mpPointCtrlContainer(NULL)
	, mLastConstrainType(0)
	, mRegisteredModeCallback(false)
{
//	DebugPrint(_T("EditPolyMod constructor: %p\n"), this);
	...
	theHold.Suspend();
	ReplaceReference (EDIT_POINT_CTRL_CONTAINER_REF, NewDefaultPointControllerContainer());
	theHold.Resume();
...
EditPolyMod::~EditPolyMod()
{
//	DebugPrint(_T("EditPolyMod destructor: %p\n"), this);
	m_iColorSelectedCageSwatch = NULL;
	m_iColorCageSwatch         = NULL;
	UnRegisterCMChangeForGrip();
}
...
void EditPolyMod::SavePreferences ()
{
   ...
   int createShapeSmooth = (mCreateShapeType == IDC_SHAPE_SMOOTH) ? 1 : 0;
	_stprintf_s(buf, _countof(buf), _T("%d"), createShapeSmooth);
	BOOL res = MaxSDK::Util::WritePrivateProfileString(kINI_SECTION_NAME, kINI_CREATE_SHAPE_SMOOTH_OPT, buf, iniFileName);
	DbgAssert(res);
	_stprintf_s(buf, _countof(buf), _T("%d"), mDetachToObject);
	res = MaxSDK::Util::WritePrivateProfileString(kINI_SECTION_NAME, kINI_DETACH_TO_OBJECT_OPT, buf, iniFileName);
	DbgAssert(res);
	_stprintf_s(buf, _countof(buf), _T("%d"), mDetachAsClone);
	res = MaxSDK::Util::WritePrivateProfileString(kINI_SECTION_NAME, kINI_DETACH_AS_CLONE_OPT, buf, iniFileName);
	DbgAssert(res);
}
...
void EditPolyMod::LoadPreferences ()
{
	...
	if (pathMgr->DoesFileExist(iniFileName)) 
	{
		// The ini file should always be saved and read with the same locale settings
		MaxLocaleHandler localeGuard;
		_stprintf_s(defaultVal, _countof(defaultVal), _T("%d"), ((mCreateShapeType == IDC_SHAPE_SMOOTH) ? 1 : 0));
		if (MaxSDK::Util::GetPrivateProfileString(kINI_SECTION_NAME, kINI_CREATE_SHAPE_SMOOTH_OPT, defaultVal, buf, _countof(buf), iniFileName))
		{
			int createShapeSmooth = _tstoi(buf);
			mCreateShapeType = (createShapeSmooth ? IDC_SHAPE_SMOOTH : IDC_SHAPE_LINEAR);
   }
		_stprintf_s(defaultVal, _countof(defaultVal), _T("%d"), mDetachToObject);
		if (MaxSDK::Util::GetPrivateProfileString(kINI_SECTION_NAME, kINI_DETACH_TO_OBJECT_OPT, defaultVal, buf, _countof(buf), iniFileName))
		{
			int detachToObject = _tstoi(buf);
      mDetachToObject = (detachToObject != 0);
   }
		_stprintf_s(defaultVal, _countof(defaultVal), _T("%d"), mDetachAsClone);
		if (MaxSDK::Util::GetPrivateProfileString(kINI_SECTION_NAME, kINI_DETACH_AS_CLONE_OPT, defaultVal, buf, _countof(buf), iniFileName))
		{
			int detachAsClone = _tstoi(buf);
...
void EditPolyMod::ModifyObject(TimeValue t, ModContext& mc, ObjectState* os, INode* node)
{
	...
	EditPolyData* pData = (EditPolyData*)mc.localData;
	if (!pData)
		mc.localData = pData = new EditPolyData(pObj, t, false);

	if (pData->CacheValidity().InInterval(t) && pData->GetMesh())
	{
		mesh = *pData->GetMesh();
		if (pData->mUseRampageWeldMath)
	...
	else
	{
		...
		if (pData->mUseRampageWeldMath)
			mesh.SetFlag(MN_MESH_USE_MAX2012_WELD_MATH, TRUE);
		int limit = mpParams->GetInt(epm_debug_last_delta, 0);
		pData->ApplyAllOperations(mesh, limit);
	...
	// Need to refresh grips (if any) after modification to update any mesh-related elements
	ResetGripUI();
	...
	if (chosenOp != NULL)
	{
		...
		const int numValidVerticesBefore = mesh.numv - mesh.NumDeadVerts();
		...
		{
			if (mc.tm)
				*(chosenOp->ModContextTM()) = Inverse(*mc.tm);
			else
				chosenOp->ModContextTM()->IdentityMatrix();
		}
		chosenOp->GetNode(this, t, ourValidity);

		// If we're doing a transform and have point controllers,
		// update the local transform data.
		if (chosenOp->OpID() == ep_op_transform)
			UpdateTransformData(t, geomValidity, pData);

		// If this operation does not support propagation of user-specified face vertex normals, clear the normals
		// container now
		bool canPropagateNormals = chosenOp->CanPropagateSpecifiedNormals();
		if (!canPropagateNormals)
		{
			mesh.ClearSpecifiedNormals();
		}

		bool ret = chosenOp->Apply(mesh, pData->GetPolyOpData());
		if (ret && chosenOp->CanDelete())
			mesh.CollapseDeadStructs();

		const int numValidVerticesAfter = mesh.numv - mesh.NumDeadVerts();
		...
		else if ((id == ep_op_weld_vertex) || (id == ep_op_weld_edge))
		{
			...
			{
				mWeldEdgesGrip.SetNumVerts(numValidVerticesBefore, numValidVerticesAfter);
			}
		}

		//  If we have specify normals, recalculate in case we modified any topology or geometry
		MNNormalSpec* normals = mesh.GetSpecifiedNormals();
		if (normals != NULL)
		{
			normals->ClearFlag(MNNORMAL_NORMALS_BUILT);
			normals->ClearFlag(MNNORMAL_NORMALS_COMPUTED);
			normals->CheckNormals();
		}
	}

	// Handle display of hard edges
	int hardDisplay = mpParams->GetInt(epm_hard_edge_display, TimeValue(0));
	if (hardDisplay)
	{
		Color hardColor = mpParams->GetColor(epm_hard_edge_display_color);
		...
		Color24* colors = (Color24*)mesh.edgeData(EDATA_COLOR);
		if (colors)
		{
			...
			mesh.dispFlags |= MNDISP_USE_EDGE_COLORS;
		}
	}
	...
	// this section deals with EditPoly above and OSD and losing the OSD Display interface in some cases due to how
	// caching works get our OSD display interfaces on the in and out meshes
	iMNMESH_GPU_Display* gpu_MNMeshIn_Display = (iMNMESH_GPU_Display*)mesh.GetInterface(
			IMNMESH_GPU_DISPLAY_INTERFACE); // store off the incoming OSD GPU display interface
	iMNMESH_GPU_Display* gpu_MNMeshOut_Display =
			(iMNMESH_GPU_Display*)pObj->GetMesh().GetInterface(IMNMESH_GPU_DISPLAY_INTERFACE);

	if (gpu_MNMeshIn_Display && gpu_MNMeshOut_Display) // meshes flowing up the stack get deleted willy nilly so need a
													   // more permanent place to store ie the local data
	{

		// if we have not allocated a container for the interface do so
		if (pData->mOSD_GPU_Display == NULL)
			pData->mOSD_GPU_Display = new iMNMESH_GPU_Display();

		// see if anyone has assigned an override display and if so store it off
		// we need to store it due to the way caching works, if we get cached above the OSD modifier
		// we lose the OSD interface connection since OSD is bypassed.  This is why the interface should
		// be attached to the INode and not the mesh flowing up the stack which is TBD
		if (gpu_MNMeshIn_Display->mpCustomRenderItem != NULL)
		{
			pData->mOSD_GPU_Display->mpCustomRenderItem =
					gpu_MNMeshIn_Display->mpCustomRenderItem; // this is an autoptr so dont need to deal with ownership
		}

		if (pData->mOSD_GPU_Display->mpCustomRenderItem)
			gpu_MNMeshOut_Display->mpCustomRenderItem = pData->mOSD_GPU_Display->mpCustomRenderItem;
	}
...
Matrix3 EditPolyMod::CompSliceMatrix (TimeValue t,INode *inode,ModContext *mc) {
	...
	Matrix3 planeTM(1);
	mpSliceControl->GetValue (t, &planeTM, CTRL_RELATIVE);
...
DWORD EditPolyMod::CurrentHitLevel (int *selByVert) {
   ...
   mpParams->GetValue (epm_by_vertex, TimeValue(0), *selByVert);
   if (*selByVert) {
...
MNMesh *EditPolyMod::GetMeshToHitTest (EditPolyData *pData) {
   ...
   if (mpCurrentOperation) {
      if (mpCurrentOperation->OpID() == ep_op_transform) 
...
void EditPolyMod::SetupBackFaceCulling(TimeValue t, const HitRegion& hr, GraphicsWindow* gw, DWORD& hitFlags)
{
    ...
    {
        mpParams->GetValue(epm_ignore_backfacing, t, ignoreBackfacing);
        mpParams->GetValue(epm_ignore_occluded, t, ignoreOccluded);
    }
          ...
          ViewExp *vpt, ModContext* mc,HitRegion &hr,GraphicsWindow *gw, EditPolyData *pData)
{
   ...
   if ((keepOld && !pData->mHighlightedItems.empty()) || GetSelectMode() == PS_SUBOBJ_SEL)
   {
		switch (mSelLevelOverride)
		{
		case MNM_SL_VERTEX:
   ...
   if (hr.type == POINT_RGN && (testVerts || testEdges))
   {
...
int EditPolyMod::HitTest (TimeValue t, INode* inode, int type, int crossing, int flags,
   ...
   {
		int res = HitTestHighlight(t,inode,flags,vpt,mc,hr,gw,pData);
   ...
   if (!inMayaSelectionMode && (GetSelectMode() == PS_NONE_SEL) && !pointToPointMode->mInMode)
   {
	   mSelLevelOverride = -1;
	   //clear our preview
	   pData->mHighlightedItems.clear();
   ...
   SetupBackFaceCulling(t, hr, gw, hitLev);
   ...
   if (hr.type == POINT_RGN && (hitLev == SUBHIT_MNVERTS || hitLev == SUBHIT_MNEDGES))
   {
    ...
    // helper object goes out of scope
    WaitingStatus waitingStatus;
    DWORD flagsCurr = flags | hitLev;
    if ((hr.type != POINT_RGN) && (flagsCurr & SUBHIT_IGNOREOCCLUDED))
    {
        TCHAR* statusMsg = GetString(IDS_COMPUTING_OCCLUSION_MESSAGE);
        waitingStatus.SetWaitingStatus(statusMsg);
    }

   int res = pMesh->SubObjectHitTest(gw, gw->getMaterial(), &hr, flagsCurr, hitList);

   if (!hitList.IsEmpty())
   {
	   const bool filterResults = mHitTestResult && mIgnoreNewInResult && pData->GetMesh() != nullptr;
	   MNMesh* filterMesh = filterResults ? pData->GetMesh() : pMesh;

		DWORD numThings = ~0;
		if (filterResults)
		{
			if (hitLev & SUBHIT_MNVERTS)
				numThings = filterMesh->numv;
			else if (hitLev & SUBHIT_MNEDGES)
				numThings = filterMesh->nume;
			else if (hitLev & SUBHIT_MNFACES)
				numThings = filterMesh->numf;
		}

		// Log every hit to allow us to cycle between them
		for (const MeshSubHitRec& rec : hitList)
		{
			const DWORD index = rec.index;
			if (index >= numThings)
			{
				res--;
				continue;
   ...
   if ((GetKeyState(VK_SHIFT) & 0x8000) && (type == HITTYPE_POINT) )
   {
	   ...
	   if (pMesh->SubObjectHitTest(gw, gw->getMaterial(), &hr, hitLev|SUBHIT_ABORTONHIT| SUBHIT_UNSELONLY, tempHitList))
	   {
		   mMeshHit = true;
	   }
  ...
  if(selLevel>0)
  {
	 ...
	 if(pData->mHighlightedItems.empty()==false&&GetMeshToHitTest(pData))
	   { 
		   ...
		   IColorManager *pClrMan = GetColorManager();;
		   Point3 oldSubColor = pClrMan->GetOldUIColor(COLOR_SUBSELECTION); //hack to fix bud with render3dface in mnmesh.. automatically takes it from this!
		   auto oldGizmoColor = pClrMan->GetOldUIColor(COLOR_SEL_GIZMOS);
   ...
   if (ShowGizmo())
   {
      ...
      if (pMesh) {
         ...
         if (dispFlags & MNDISP_VERTTICKS) {
            float *ourvw = (dispFlags & MNDISP_SELVERTS) ? SoftSelection(t, pData) : NULL;
   ...
   if ((ip != nullptr) && (ip->GetCommandMode() == createFaceMode)) createFaceMode->Display(gw);
   
   if (subobjectPickMode && subobjectPickMode->inMode)
   {
	   EditPolyData *pData = (EditPolyData *) mc->localData;
	   MNMesh *pMesh = GetMeshToDisplay(pData);
	   subobjectPickMode->Display (gw,pMesh,inode);
   }
...
void EditPolyMod::GetWorldBoundBox(TimeValue t, INode* inode, ViewExp *vpt, Box3& box, ModContext *mc) {
   ...
   if (ShowGizmo ()|| GetSelectMode()==PS_SUBOBJ_SEL || GetSelectMode()==PS_MULTI_SEL || (selLevel != EPM_SL_OBJECT)) {
      Matrix3 tm = inode->GetObjectTM(t);
      EditPolyData *pData = (EditPolyData *) mc->localData;
      MNMesh *pMesh = GetMeshToDisplay(pData);
      if (!pMesh) return;
	  ...
	  int selectedCount = 0;
	  if (selLevel == EPM_SL_VERTEX)
		  selectedCount = pMesh->TargetVertsBySelection(MNM_SL_VERTEX);
	  else if (selLevel == EPM_SL_EDGE || selLevel == EPM_SL_BORDER)
		  selectedCount = pMesh->TargetVertsBySelection(MNM_SL_EDGE);
	  else if ((selLevel == EPM_SL_FACE) || (selLevel == EPM_SL_ELEMENT))
		  selectedCount = pMesh->TargetVertsBySelection(MNM_SL_FACE);

	  if (selectedCount > 0)
		  box = pMesh->getBoundingBox(&tm, true);
	  else
...
void EditPolyMod::GetSubObjectTMs (SubObjAxisCallback *cb,TimeValue t,INode *node,ModContext *mc) {
	...
	case EPM_SL_BORDER:
		...
		for (i=0; i<ct; i++) {
			if(refCoordSystem == COORDS_LOCAL_ALIGNED) {
				Point3 normal = Normalize((*(pTemp->ClusterNormals(MNM_SL_EDGE)))[i]);
				Point3 clusterCenter = (*(pTemp->ClusterCenters(MNM_SL_EDGE)))[i];
				Point3 tangent;
	...
	default:
		...
		for (i=0; i<ct; i++) {
			if(refCoordSystem == COORDS_LOCAL_ALIGNED) {
				Point3 normal = Normalize((*(pTemp->ClusterNormals(MNM_SL_FACE)))[i]);
				Point3 clusterCenter = (*(pTemp->ClusterCenters(MNM_SL_FACE)))[i];
				Point3 tangent;
				if(normal.x == 0.0f && normal.y == 0.0f)
					tangent = Point3(1,0,0);
				else
...
void EditPolyMod::AllocPointControllers (int count) {
   ...
   if (mpPointCtrlContainer) mpPointCtrlContainer->SetNumSubControllers (count);
   for (int i=oldCount; i<count; i++) {
      mPointControl[i] = NULL;
      if (mpPointCtrlContainer) mpPointCtrlContainer->SetSubController(i, NULL);
   }
}
...
void EditPolyMod::PlugControllers (TimeValue t, BitArray &set, INode *pNode) {
   ...
   for (j=0; j<mPointNode.length(); j++) {
      if (mPointNode[j] == const_cast<const TCHAR*>(pNode->GetName())) break;
   }
...
BOOL EditPolyMod::PlugController (TimeValue t, int vertex, int nodeStart, INode *pNode) {
   ...
   ResumeAnimate ();
   mpPointCtrlContainer->SetSubController (i, mPointControl[i]);
   return TRUE;
}
...
RefTargetHandle EditPolyMod::GetReference(int i) {
   ...
   case EDIT_POINT_CTRL_CONTAINER_REF: return mpPointCtrlContainer;
   }
...
void EditPolyMod::SetReference(int i, RefTargetHandle rtarg) {
   ...
   case EDIT_SLICEPLANE_REF:
      ...
      return;
   case EDIT_POINT_CTRL_CONTAINER_REF:
      mpPointCtrlContainer = (PointControllerContainer*)rtarg;
      if (mpPointCtrlContainer) mpPointCtrlContainer->SetNumSubControllers (mPointControl.Count());
      return;
   }
...
RefResult EditPolyMod::NotifyRefChanged( const Interval& changeInt,RefTargetHandle hTarget, 
   ...
   if (message == REFMSG_CHANGE) {
      if (hTarget == mpParams) {
         ...
         case epm_selected_cage_color:
            ...
            break;
		 case epm_hard_edge_display_color:
			 UpdateHardEdgeColorDisplay();
			 break;
		 case epm_hard_edge_display:
			 UpdateHardEdgeDisplayType();
			 break;
         }
      }
   }
...
TSTR EditPolyMod::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case EDIT_PBLOCK_REF: return localized ? GetString(IDS_PARAMETERS) : _T("Parameters");
	case EDIT_SLICEPLANE_REF: return localized ? GetString(IDS_EDITPOLY_SLICE_PLANE) : _T("Slice Plane");
	case EDIT_POINT_CTRL_CONTAINER_REF: return localized ? GetString(IDS_EDITPOLY_POINTCTRLCONTAINER) : _T("Point Controller Container");
	}

	i -= EDIT_POINT_BASE_REF;
	if ((i < 0) || (i >= mPointControl.Count()))
		return _T("");
	...
	if (mPointRange.Count() > 2)
	{
		...
		if (j != mPointRange.Count())
			buf.printf(_T("%s: %s %d"), mPointNode[j - 1], (localized ? GetString(IDS_VERTEX) : _T("Vertex")), i + 1 - mPointRange[j - 1]);
	}
	else
	{
		// Edit Poly only applied to one node, so leave out the node name.
		buf.printf(_T("%s %d"), (localized ? GetString(IDS_VERTEX) : _T("Vertex")), i + 1);
	}
	return buf;
}
...
BOOL EditPolyMod::AssignController (Animatable *control, int subAnim) {
   ...
   if (subAnim==EDIT_POINT_CTRL_CONTAINER_REF) {
      int n = mPointControl.Count();
      mpPointCtrlContainer->SetNumSubControllers(n);
      for (int i=0; i<n; i++) if (mPointControl[i]) mpPointCtrlContainer->SetSubController(i,mPointControl[i]);
   }
...
BOOL EditPolyMod::SelectSubAnim (int subNum) {
   ...
   for (i=0; i<nodes.Count(); i++) {
      if (name == const_cast<const TCHAR*>(nodes[i]->GetName())) break;
   }
...
void EditPolyBackspaceUser::Notify() {
   ...
   if ((mpEditPoly->selLevel > EPM_SL_OBJECT) && (mpEditPoly->selLevel < EPM_SL_FACE)) {
      ...
      case MNM_SL_VERTEX: 
		  mpEditPoly->EpModButtonOp (ep_op_remove_vertex); 
		  break;
      case MNM_SL_EDGE: 
		  mpEditPoly->EpModButtonOp (GetKeyState(VK_CONTROL)<0 ? ep_op_remove_edge_remove_verts: ep_op_remove_edge); 
		  break;
      }
...
void EditPolyMod::SetCommandModeFromOld(int oldCommandMode)
{
	...
	case ep_mode_subobjectpick:
		EpModToggleCommandMode(ep_mode_subobjectpick);
		break;
	case ep_mode_point_to_point:
		EpModToggleCommandMode(ep_mode_point_to_point);
		break;
...
void EditPolyMod::SetSubobjectLevel(int level) {
   ...
   EpModLocalDataChanged (SUBSEL_TYPE_CHANNEL|DISP_ATTRIB_CHANNEL);
    ...
    if (ip)
    {
        ...
        for (int i=0; i<list.Count(); i++)
        {
			...
			pData->mDoubleClickSelector.Reset();
        }
...
void EditPolyMod::DoubleClickSelect(HitRecord *hitRec)
{
	if (hitRec == NULL) return;
	...
	EditPolyData *pData = (EditPolyData*)hitRec->modContext->localData;
	...
	if (!pData->GetNewSelection ()) {
		...
		{
			BitArray newSel = pData->mDoubleClickSelector.OnDoubleClickEdge(*pData->GetMesh(), add, sub, hitRec->hitInfo);
			*pData->GetNewSelection() = newSel;
		}
		else if (selLevel == EPM_SL_VERTEX)
		{
			BitArray newSel = pData->mDoubleClickSelector.OnDoubleClickVertex(*pData->GetMesh(), add, sub, hitRec->hitInfo);
			*pData->GetNewSelection() = newSel;
		}
		else if (selLevel == EPM_SL_FACE)
		{
			BitArray newSel = pData->mDoubleClickSelector.OnDoubleClickFace(*pData->GetMesh(), add, sub, hitRec->hitInfo);
			*pData->GetNewSelection() = newSel;
		}
		else if (selLevel == EPM_SL_ELEMENT)
		{
			if (sub && !add)
			{
				ClearSelection(EPM_SL_ELEMENT);
				pData->SetupNewSelection(meshSelLevel[selLevel]);
				pData->SynchBitArrays();
	...
	const TCHAR *levelName = LookupMNMeshSelLevel (meshSelLevel[selLevel]);
	...
	if (newSel)
	{
		if (numContexts > 1)
		{
			macroRecorder->FunctionCall(_T("$.modifiers[#Edit_Poly].Select"),
				...
				mr_bitarray, newSel,
				_T("node"), mr_reftarg, hitRec->nodeRef);
		}
		else
		{
	...
	if (pData->ApplyNewSelection(this))
	{
...
void EditPolyMod::SelectSubComponent (HitRecord *pFirstHit, BOOL selected, BOOL all, BOOL invert)
{
   ...
   // In Maya Selection mode, SHIFT is used as add/substract selection, so disable loop or ring functionality.
   if (!inMayaSelectionMode && (GetKeyState(VK_SHIFT) & 0x8000) && mMeshHit)//Select loop or ring if Shift key is pressed
   {
	   if (selLevel == EPM_SL_EDGE)
	   ...
	   pointToPointMode->proc.mActiveNode = pFirstHit->nodeRef;
	   pointToPointMode->proc.SetFirstHit(pFirstHit->hitInfo, pFirstHit->nodeRef);
	   return;
   }

   for (HitRecord* hr = pFirstHit; hr != NULL; hr = hr->Next())
   {
	   EditPolyData* pData = (EditPolyData*)hr->modContext->localData;
	   switch (selLevel)
	   {
	   case EPM_SL_VERTEX:
		   pData->mDoubleClickSelector.OnSingleClickVertex(hr->hitInfo);
		   break;
	   case EPM_SL_EDGE:
		   pData->mDoubleClickSelector.OnSingleClickEdge(hr->hitInfo);
		   break;
	   case EPM_SL_FACE:
		   pData->mDoubleClickSelector.OnSingleClickFace(hr->hitInfo);
		   break;
	   }

	   // If we aren't logging every hit, stop after the first one
	   if (!all)
		   break;
   }
   ...
   if (mSelLevelOverride>0) //special multi or highlight selection
   {
	   ...
	   for (hr=pFirstHit; hr!=NULL; hr = hr->Next())
	   {
			...
			for (int highlightIndex : pData->mHighlightedItems)
				pData->GetNewSelection()->Set (highlightIndex, true);
	   ...
	   const TCHAR *levelName = LookupMNMeshSelLevel (mSelLevelOverride);
   ...
   if (selByVert) {
      ...
      if (mpParams->GetInt (epm_select_by_angle) && (selLevel == EPM_SL_FACE)) {
         ...
         for (hr=pFirstHit; hr!=NULL; hr = hr->Next()) {
			...
			{
				BitArray currentSel;
				pData->GetMesh()->getFaceSel(currentSel);
	  ...
	  if (!inMayaSelectionMode && (GetKeyState(VK_SHIFT) & 0x8000) && mMeshHit) //Select loop or ring if Shift key is pressed
	  {
			...
			if (l_mesh13)
			{
				...
				if (selLevel == EPM_SL_EDGE)
				{
					...
					BitArray temp = (*newSel) & (~currentSel);
					if (temp.NumberSet() == 1) {
						mm->SelectEdgeRing(temp);
						if ((temp & currentSel).NumberSet() > 0) {
							*newSel |= temp;
							foundLoopRing = true;
						}
					}
...
void EditPolyMod::ClearSelection(int selLevel) {
   ...
   int msl = meshSelLevel[selLevel];
   const TCHAR *levelName = LookupMNMeshSelLevel (msl);
   ...
   if (changeOccurred) EpModLocalDataChanged(SELECT_CHANNEL);
}
...
void EditPolyMod::SelectAll(int selLevel) {
   ...
   for (int i=0; i<list.Count(); i++) {
      ...
      BitArray *pSel = pData->GetNewSelection();
      // Special test -- If in Borders mode, select all BORDER EDGES -- otherwise select all items
	  if (selLevel == EPM_SL_BORDER)
	  {
		  pData->SelectAllBorderEdges(*pSel);
	  }
	  else
		  pSel->SetAll ();
...
void EditPolyMod::InvertSelection(int selLevel) {
   ...
   for (int i=0; i<list.Count(); i++) {
	  ...
	  {
		  *pSel = *pCurrent;
		  pData->SelectInvertBorderEdges(*pSel);
	  }
	  else
	  {
...
BaseInterface *EditPolyMod::GetInterface (Interface_ID id) {
	...
	else if (id == EPOLY_MOD18_INTERFACE) return (EPolyMod18 *)this;
	else if (id == EPOLY_MOD21_INTERFACE) return (EPolyMod21 *)this;
...
void EditPolyMod::UpdateCache(TimeValue t) {
   ...
   NotifyDependents(Interval(t,t), PART_GEOM|PART_SELECT|PART_SUBSEL_TYPE|PART_DISPLAY|PART_TOPO, REFMSG_MOD_EVAL);
   mUpdateCachePosted = FALSE;
}

bool EditPolyMod::IsVertexSelected (EditPolyData *pData, int vertexID) {
   int stackSel=0;
   mpParams->GetValue (epm_stack_selection, 0, stackSel);
   if (!stackSel) {
...
int EditPolyMod::EpModConvertSelection (int epSelLevelFrom, int epSelLevelTo, bool requireAll) {
   ...
   for (int listi=0; listi<list.Count(); listi++) {
      EditPolyData *pData = (EditPolyData*)list[listi]->localData;
      if (!pData) continue;
...
void EditPolyMod::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc) {
   if (!mc->localData) return;
...
const int kChunkEditPolyVersion = 0x150;
...
IOResult EditPolyMod::Save(ISave *isave) {
   ...
   isave->BeginChunk(kChunkEditPolyVersion);
   isave->Write(&m_editPolyVersion, sizeof(int), &nb);
   isave->EndChunk();
...
IOResult EditPolyMod::Load(ILoad *iload) {
   ...
   // Note that we initialize the version to the EDIT_POLY_ORIGINAL, as for files for which this version is appropriate, the
   // version chunk will not be present
   m_editPolyVersion = EDIT_POLY_ORIGINAL;
   while (IO_OK==(res=iload->OpenChunk())) {
      ...
      case kChunkMeshPaintHandler:
         ...
         break;
      case kChunkEditPolyVersion:
         res = iload->Read(&m_editPolyVersion, sizeof(int), &nb);
         break;
	  case kManipBitArray:
		  {
			 BitArray temp;
...
const USHORT kChunkUseRampageWeldMath = 0x270;
const USHORT kChunkDoBadVertRemovalOnSelection = 0x271;
const USHORT kChunkSelSetId = 0x272;
const USHORT kChunkSelSet = 0x273;
...
IOResult EditPolyMod::SaveLocalData(ISave *isave, LocalModData *ld) {
   ...
   for (PolyOperationRecord *pop = pData->mpOpList; pop != NULL; pop=pop->Next())
   {
      ...
      // If the PolyOperation associated with operation record pop does not support the requested save version, abort the save
      // now
      const PolyOperation* opCurr = pop->Operation();
      bool isSaveSupported = opCurr->IsSaveVersionSupported();
      if (!isSaveSupported)
      {
         return IO_ERROR;
      }

	  // the difference between ep_op_remove_edge_remove_verts and ep_op_remove_edge is captured 
	  // by pop->Operation()->Save (isave). Since ep_op_remove_edge_remove_verts is not present in 
	  // previous versions of max, need to store ep_op_remove_edge
      int id = pop->Operation()->OpID();
	  if (id == ep_op_remove_edge_remove_verts)
		  id = ep_op_remove_edge;
   ...
   if (currentOpData) {
	  ...
	  // ep_op_remove_edge_remove_verts never current op, don't need to handle here
      isave->Write (&id, sizeof(int), &nb);
      isave->EndChunk ();
   ...
   if (pData->mDoBadVertRemovalOnSelection)
   {
	   isave->BeginChunk(kChunkDoBadVertRemovalOnSelection);
	   isave->EndChunk();
   }

	{
		// Save named selection sets
		if (pData->vselSet.Count())
		{
			isave->BeginChunk(kChunkSelSetId);
			int index = 0;
			isave->Write(&index, sizeof(index), &nb);
			isave->EndChunk();
			isave->BeginChunk(kChunkSelSet);
			pData->vselSet.Save(isave);
			isave->EndChunk();
		}

		if (pData->fselSet.Count())
		{
			isave->BeginChunk(kChunkSelSetId);
			int index = 1;
			isave->Write(&index, sizeof(index), &nb);
			isave->EndChunk();
			isave->BeginChunk(kChunkSelSet);
			pData->fselSet.Save(isave);
			isave->EndChunk();
		}

		if (pData->eselSet.Count())
		{
			isave->BeginChunk(kChunkSelSetId);
			int index = 2;
			isave->Write(&index, sizeof(index), &nb);
...
IOResult EditPolyMod::LoadLocalData(ILoad *iload, LocalModData **pld) {
   ...
   *pld = pData;
   ...
   int id(0);
   ULONG nb(0);
   PolyOperation* currentOp = nullptr;
   PolyOperationRecord* currentRecord = nullptr;
   PolyOperationRecord* lastRecord = nullptr;
   LocalPolyOpData* currentData = nullptr;
   ...
   while (IO_OK==(res=iload->OpenChunk())) {
      ...
      case kChunkOperationID:
         {
            ...
            PolyOperation* opCurr = GetPolyOperationByID(id);

            // Abort loading if we receive a PolyOperation ID which is unsupported by this version, otherwise, create an
            // instance of the indicated operation
            if (opCurr == nullptr)
            {
               TSTR errorTitle = GetString(IDS_EDIT_POLY_MOD);
               TSTR errorMsg = GetString(IDS_LOAD_INVALID_POLY_OPERATION);
               Interface* coreInterface = GetCOREInterface();
               LogSys* sysLog = coreInterface->Log();
               sysLog->LogEntry(SYSLOG_ERROR, NO_DIALOG, errorTitle, errorMsg);
               res = IO_ERROR;
            }
            else
            {
               currentOp = opCurr->Clone();
	  ...
	  case kChunkUseRampageWeldMath:
		  pData->mUseRampageWeldMath = true;
		  break;
	  case kChunkDoBadVertRemovalOnSelection:
		  pData->mDoBadVertRemovalOnSelection = true;
		  break;
	  case kChunkSelSetId:
	  {
		  int which = 0;
		  ...
		  iload->CloseChunk();
		  if (iload->PeekNextChunkID() != kChunkSelSet)
			  break;

		  iload->OpenChunk();
		  if (which == 0)
			  res = pData->vselSet.Load(iload);
		  else if (which == 1)
			  res = pData->fselSet.Load(iload);
		  else if (which == 2)
			  res = pData->eselSet.Load(iload);
   ...
   DWORD loFileVer = LOWORD(fileVer);
   DWORD hiFileVer = HIWORD(fileVer);
   if ((hiFileVer <= MAX_RELEASE_R14))
   {
	   pData->mUseRampageWeldMath = true;
   }
   // The "bad vertex" removal fix went in after build 1167 of 2019.1, so anything saved earlier needs to be tagged.
   if ((hiFileVer < MAX_RELEASE_R21) || (hiFileVer == MAX_RELEASE_R21 && loFileVer < 1167))
   {
	   pData->mDoBadVertRemovalOnSelection = true;
   }
...
bool EditPolyMod::ShouldCollapseOnSave()
{
	// If saving to the same version of 3ds Max, we don't need to collapse -- save time!
	if (GetSavingVersion() == 0)
		return false;

    // Fetch the ModContext entities for each object to which this modifier is presently applied
    EditPolyModContextEnumProc contexts(this);
    EnumModContexts(&contexts);

    // Check each PolyOperation applied by this modifier to any object, to determine whether any operation is unsupported in
    // the requested save version
    int indexContext = 0;
    bool doCollapseOnSave = false;
    int nContexts = contexts.GetNumberOfObjects();    
    while ((!doCollapseOnSave) && (indexContext != nContexts))
    {
        ModContext* context;
        INode* node;

        contexts.GetInformationForObject(context, node, indexContext);
        DbgAssert(context != nullptr);
        EditPolyData* modData = static_cast<EditPolyData*>(context->localData);
        if (modData != nullptr)
        {
            PolyOperationRecord* opRecordCurr = modData->mpOpList;
            while ((!doCollapseOnSave) && (opRecordCurr != nullptr))
            {
                const PolyOperation* opCurr = opRecordCurr->Operation();
                doCollapseOnSave = !(opCurr->IsSaveVersionSupported());
                opRecordCurr = opRecordCurr->Next();
            }
        }
    ...
    // If any PolyOperation used in this modifier is unsupported in the specified save version, notify the user that the
    // modifier stack will be collapsed up to, and including, this object in the saved file
    if (doCollapseOnSave)
    {
        WarnCollapsedSave();
    }

    // Return a flag indicating whether the stack should be collapsed
    return doCollapseOnSave;
}
...
void EditPolyMod::RescaleWorldUnits (float f) {
   ...
   EpModLocalDataChanged(ALL_CHANNELS);   // Could affect everything - whether welds occur, etc.
}
...
void EditPolyMod::ResetOperationParams () {
   ...
   {
      if (mpParams->GetControllerByIndex(i) == NULL) continue;
      mpParams->RemoveControllerByIndex(i,0);
   }
...
public:
   ...
   int Size () { return 8; }
   TSTR Description() { return _T("RemoveLocalPolyOpData"); }
};
...
void EditPolyMod::EpModCancel ()
{
   ...
   EpModLocalDataChanged (TOPO_CHANNEL|GEOM_CHANNEL|SELECT_CHANNEL|VERTCOLOR_CHANNEL|TEXMAP_CHANNEL);
...
void EditPolyMod::EpModCommit (TimeValue t, bool macroRecord, bool clearAfter, bool undoable)
{
   ...
   for (int i=0; i<list.Count(); i++)
   {
      ...
      localOp->GetValues (this, t); // Don't affect validity, since we're "freezing" these parameters.
      ...
      localOp->GetNode (this, t);

      // If we're doing a transform and have point controllers,
      // update the local transform data.
      if (localOp->OpID() == ep_op_transform) UpdateTransformData (t, pData);

      // If this operations does not support propagation of specified normals, clear them now
      bool canPropagateNormals = localOp->CanPropagateSpecifiedNormals();
      if (!canPropagateNormals)
      {
         mesh.ClearSpecifiedNormals();
      }
      ...
      if (undoable)
      {
         if (theHold.Holding ()) {
            ...
            DebugPrint (_T("Edit Poly WARNING: Operation being committed without restore object.\n"));
            DbgAssert(0);
         }
      }
...
PolyOperation *EditPolyMod::GetPolyOperation ()
{
   ...
   // We are creating a new PolyOperation; set the triangulation algorithm to be employed by this operation, as specified by
   // the epm_enable_enhanced_retriangulation parameter block entry
   Interval interval = FOREVER;
   BOOL isEnhancedRetriangulationActive = FALSE;
   Interface* coreInterface = GetCOREInterface();
   TimeValue timeCurr = coreInterface->GetTime();
   mpParams->GetValue(epm_enable_enhanced_retriangulation, timeCurr, isEnhancedRetriangulationActive, interval);
   ...
   mpCurrentOperation->SetTriangulationType(triangulationType);
...
void EditPolyMod::EpSetLastOperation (int op) {
   ...
   case ep_op_remove_edge_remove_verts:
      mLastOperation = op;
   ...
   case ep_op_extrude_face_postprocessed:
   case ep_op_finalize_extrude_face_postprocessed:
      mLastOperation = op;
   ...
   case ep_op_select_ring:
      ...
      break;
   case ep_op_make_hard_edges:
      mLastOperation = op;
      if (pButton) pButton->SetTooltip (true, GetString (IDS_EDITPOLY_MAKE_HARD_EDGES));
      break;
   case ep_op_make_smooth_edges:
      mLastOperation = op;
      if (pButton) pButton->SetTooltip (true, GetString (IDS_EDITPOLY_MAKE_SMOOTH_EDGES));
      break;
   case ep_op_select_hard_edges:
      mLastOperation = op;
      if (pButton) pButton->SetTooltip (true, GetString (IDS_EDITPOLY_SELECT_HARD_EDGES));
      break;
   case ep_op_select_smooth_edges:
      mLastOperation = op;
      if (pButton) pButton->SetTooltip (true, GetString (IDS_EDITPOLY_SELECT_SMOOTH_EDGES));
      break;
   case ep_op_bridge_border:
	   mLastOperation = op;
	   if (pButton) pButton->SetTooltip(true, GetString(IDS_BRIDGE_BORDERS));
	   break;
   case ep_op_bridge_polygon:
	   mLastOperation = op;
	   if (pButton) pButton->SetTooltip(true, GetString(IDS_BRIDGE_POLYGONS));
	   break;
   case ep_op_bridge_edge:
	   mLastOperation = op;
	   if (pButton) pButton->SetTooltip(true, GetString(IDS_BRIDGE_EDGES));
	   break;
   }
...
bool EditPolyMod::EpMeshGetMapChannelActive (int mapChannel, INode *node) {
   ...
   if ((mapChannel <= -NUM_HIDDENMAPS) || (mapChannel >= mesh->MNum())) return false;
...
void EditPolyMod::EPMeshSetVertexFlags( BitArray & in_vset, const DWORD in_flags, DWORD in_fmask, const bool in_undoable, INode * in_node )
{
   ...
   if ( in_flags & (MN_HIDDEN|MN_SEL) )
   {
      ...
      EpModLocalDataChanged (channels);
   }
}
...
void EditPolyMod::EPMeshSetEdgeFlags( BitArray & in_eset, const DWORD in_flags, DWORD in_fmask, const bool in_undoable, INode * in_node )
{
   ...
   if (in_flags & MN_SEL)
   {
      EpModLocalDataChanged(SELECT_CHANNEL|DISP_ATTRIB_CHANNEL);
   }
}
...
void EditPolyMod::getTransformationMatrices( Matrix3 & refMatrix, Matrix3 & worldTransformationMatrix, INode * node )
{
   ...
   ViewExp& view = ip->GetActiveViewExp();
	 if ( ! view.IsAlive() )
	 {
	 	// why are we here
		 refMatrix.Zero();
		 worldTransformationMatrix.Zero();
	   return;
	 }
...
public:
   ...
   EditPolyAddSetRestore (GenericNamedSelSetList *sl, TSTR &n) : setList(sl), name(n) { }
   void Restore(int isUndo) {
      set  = *setList->GetSet(name);
      setList->RemoveSet(name);
   }
   void Redo() { setList->InsertSet (set, 0, name); }
         
   TSTR Description() {return _T("Add NS Set");}
};
...
public:
   ...
   EditPolyDeleteSetRestore(GenericNamedSelSetList *sl, TSTR& n) {
      setList = sl;
      BitArray *ptr = setList->GetSet(n);
      if (ptr) set = *ptr;
      name = n;
   }
   void Restore(int isUndo) { setList->InsertSet(set, 0, name); }
   void Redo() { setList->RemoveSet(name); }
   TSTR Description() {return _T("Delete Set");}
};
...
public:
   ...
   GenericNamedSelSetList* setList;
   int index;
   EditPolySetNameRestore(GenericNamedSelSetList* sl, int i, TSTR& n)
	   : newName(n)
	   , oldName(*sl->names[i])
	   , setList(sl)
	   , index(i)
   ...
   void Restore(int isUndo) { *setList->names[index] = oldName; }
   void Redo() { *setList->names[index] = newName; }
   TSTR Description() {return _T("Set Name");}
};
...
void EditPolyMod::ActivateSubSelSet(TSTR &setName)
{
	...
	for (int i = 0; i < mcList.Count(); i++) {
		...
		BitArray* set = meshData->GetNamedSel(nsl).GetSet(setName);
		if (!set)
			continue;
...
void EditPolyMod::NewSetFromCurSel(TSTR &setName) {
   ...
   ip->GetModContexts(mcList,nodes);
	...
	for (int i = 0; i < mcList.Count(); i++)
	{
		...
		GenericNamedSelSetList& set = meshData->GetNamedSel(nsl);
		BitArray *ssel = set.GetSet(setName);

		switch (nsl) {
		case NS_VERTEX:
		if (ssel)
			*ssel = meshData->mVertSel;
		else
		{
			set.AppendSet(meshData->mVertSel, 0, setName);
			if (theHold.Holding())
				theHold.Put(new EditPolyAddSetRestore(&set, setName));
		}
		break;
		case NS_FACE:
			if (ssel)
				*ssel = meshData->mFaceSel;
			else
			{
				set.AppendSet(meshData->mFaceSel, 0, setName);
				if (theHold.Holding())
					theHold.Put(new EditPolyAddSetRestore(&set, setName));
			}
			break;
		case NS_EDGE:
			if (ssel)
				*ssel = meshData->mEdgeSel;
			else
			{
				set.AppendSet(meshData->mEdgeSel, 0, setName);
				if (theHold.Holding())
					theHold.Put(new EditPolyAddSetRestore(&set, setName));
			}
			break;
...
void EditPolyMod::RemoveSubSelSet(TSTR &setName) {
   ...
   for (int i = 0; i < mcList.Count(); i++) {
	  ...
	  GenericNamedSelSetList& set = meshData->GetNamedSel(nsl);
	  if (theHold.Holding())
		  theHold.Put(new EditPolyDeleteSetRestore(&set, setName));
...
void EditPolyMod::SetupNamedSelDropDown() {
   ...
   // Grab the first, they should all be identical
   EditPolyData* meshData = (EditPolyData*)mcList[0]->localData;
   if (!meshData)
	   return;
...
void EditPolyMod::SetNamedSelSetName(int i,TSTR &newName) {
   ...
   for (int i = 0; i < mcList.Count(); i++)
   {
	   ...
	   GenericNamedSelSetList& setList = meshData->GetNamedSel(nsl);
	   if (theHold.Holding())
		   theHold.Put(new EditPolySetNameRestore(&setList, i, newName));

	   *setList.names[i] = newName;
   }
}
...
void EditPolyMod::NewSetByOperator(TSTR &newName,Tab<int> &sets,int op) {
   ...
   for (int i = 0; i < mcList.Count(); i++) {
      ...
      BitArray bits = *setList.sets[sets[0]];

      for (int i=1; i<sets.Count(); i++) {
         switch (op) {
         case NEWSET_MERGE:
            bits |= *setList.sets[sets[i]];
            break;
	  ...
	  setList.AppendSet(bits, 0, newName);
      if (theHold.Holding()) theHold.Put(new EditPolyAddSetRestore(&setList, newName));
   }
}
...
void EditPolyMod::NSCopy() {
   ...
   for (int i = 0; i < mcList.Count(); i++) {
	  ...
	  GenericNamedSelSetList& setList = meshData->GetNamedSel(nsl);
	  if (!clip)
		  clip = new MeshNamedSelClip(*setList.names[index]);
...
void EditPolyMod::NSPaste() {
   ...
   for (int i = 0; i < mcList.Count(); i++) {
      ...
      if (theHold.Holding())
		  theHold.Put(new EditPolyAddSetRestore(&setList, name));
   }  
...
int EditPolyMod::SelectNamedSet() {
	...
	return DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_NAMEDSET_SEL), ip->GetMAXHWnd(), EditPolyPickSetDlgProc, (LPARAM)&set.names);
}
...
void EditPolyMod::UpdateNamedSelDropDown () {
   ...
   for (int i = 0; i < set.sets.Count(); ++i)
   {
	   switch (selLevel)
	   {
	   case EPM_SL_VERTEX:
		   if (*set.sets[i] == meshData->mVertSel)
			   selectedEntryIndex = i;
		   break;
	   case EPM_SL_EDGE:
	   case EPM_SL_BORDER:
		   if (*set.sets[i] == meshData->mEdgeSel)
			   selectedEntryIndex = i;
		   break;
	   default:
		   if (*set.sets[i] == meshData->mFaceSel)
			   selectedEntryIndex = i;
		   break;
	   }
   ...
   if (selectedEntryIndex >= 0)
	   ip->SetCurNamedSelSet(*set.names[selectedEntryIndex]);
   else
	   ip->ClearCurNamedSelSet();
}
...
BOOL EditPolyMod::GetUniqueSetName(TSTR &name) 
{
	...
	{
		BOOL unique = TRUE;
		for (int i=0; i<setList.names.Count(); i++)
		{
...
void EditPolyMod::UpdateCageColor() 
{
   ...
   {
      m_CageColor = Point3(m_iColorCageSwatch->GetAColor());
   }
}
...
void EditPolyMod::UpdateSelectedCageColor() 
{
   ...
   {
      m_SelectedCageColor = Point3(m_iColorSelectedCageSwatch->GetAColor());
   }
}

// Load the hard edge color into the UI
void EditPolyMod::UpdateHardEdgeColorDisplay(HWND hWnd) 
{
	if(!hWnd) {
		if(!mpDialogSubobj) return;
		hWnd = mpDialogSubobj->GetHWnd();
		if(!hWnd) return;
	}
	IColorSwatch *swatch = GetIColorSwatch(GetDlgItem(hWnd, IDC_HARD_COLOR));
	if(swatch != NULL) {
		// Grab color from param block
		Color theColor = getParamBlock()->GetColor(epm_hard_edge_display_color);
		swatch->SetColor(theColor);
		ReleaseIColorSwatch(swatch);
	}
}

// Load the hard edge display type into the UI
void EditPolyMod::UpdateHardEdgeDisplayType(HWND hWnd) 
{
	if(!hWnd) {
		if(!mpDialogSubobj) return;
		hWnd = mpDialogSubobj->GetHWnd();
	...
	BOOL option = getParamBlock()->GetInt(epm_hard_edge_display) ? TRUE : FALSE;
	SetCheckBox(hWnd, IDC_HARD_EDGE_DISPLAY, option);
}
...
void EditPolyMod::UpdateLoopEdgeSelection( const int in_spinVal )
{
   ...
   for (int i = 0; i < l_mcList.Count(); i++) 
   {
      ...
      IMNMeshUtilities8* l_meshToBridge = pMesh->GetTypedInterface<IMNMeshUtilities8>();
...
void EditPolyMod::RegisterCMChangedForGrip()
{
	...
	ResetGripUI();
	if (mRegisteredModeCallback == false)
	{
		GetCOREInterface()->RegisterCommandModeChangedCallback(this);
		mRegisteredModeCallback = true;
	}
}

void EditPolyMod::UnRegisterCMChangeForGrip()
{
	if (mRegisteredModeCallback == true)
	{
...
bool EditPolyMod::GetManipulateGrip(EPolyMod13::ManipulateGrips val)
{
    ...
    DbgAssert(val < EPolyMod13::EditPolyManipulatorGripCount);
    if (val < EPolyMod13::EditPolyManipulatorGripCount)
    {
        isManipulated = (GetEPolyManipulatorGrip())->GetManipulateGrip(m_gripTypeMap[val]);
    }

	return isManipulated;
}

void EditPolyMod::SetManipulateGrip(bool on, EPolyManipulatorGrip::ManipulateGrips val)
{
	SetUpManipGripIfNeeded();
}

bool EditPolyMod::GetManipulateGrip(EPolyManipulatorGrip::ManipulateGrips val)
{
    return (GetEPolyManipulatorGrip())->GetManipulateGrip(val);
}
...
bool EditPolyMod::ManipGripIsValid()
{
	...
	int useSoftsel=0;
	mpParams->GetValue (epm_ss_use, TimeValue(0), useSoftsel);
	if(useSoftsel)
	{
		if( GetManipulateGrip(EPolyManipulatorGrip::eSoftSelFalloff) ||  
			GetManipulateGrip(EPolyManipulatorGrip::eSoftSelBubble)||
			GetManipulateGrip(EPolyManipulatorGrip::eSoftSelPinch))
			return true;
	}
	if( GetEPolySelLevel() == EPM_SL_EDGE) // only valid for edge modes.
	{
		if( GetManipulateGrip(EPolyManipulatorGrip::eSetFlow) ||  
		GetManipulateGrip(EPolyManipulatorGrip::eLoopShift) ||
		GetManipulateGrip(EPolyManipulatorGrip::eRingShift) || 
		GetManipulateGrip(EPolyManipulatorGrip::eEdgeCrease) || 
		GetManipulateGrip(EPolyManipulatorGrip::eEdgeWeight) ||
		GetManipulateGrip(EPolyManipulatorGrip::eEdgeDepth))
		return true;
	}
	if( GetEPolySelLevel() == EPM_SL_VERTEX) // only valid for vertex modes.
	{
		if( GetManipulateGrip(EPolyManipulatorGrip::eVertexCrease) || 
		GetManipulateGrip(EPolyManipulatorGrip::eVertexWeight))
		return true;
	}
	return false;
}
...
void EditPolyMod::SetUpManipGripIfNeeded()
{
	...
	{
		if(gripManager->GetActiveGrip() == &mManipGrip)
		{
			if(ManipGripIsValid()==false)
				gripManager->SetGripsInactive();
			else
			{
				SetUpManipGripVisibility();			
			}
		}
		else if(gripManager->GetActiveGrip() == NULL)
		{
			//in manip mode but no grip may be active, so we may need to turn it on!
			if(ManipGripIsValid())
			{
				gripManager->SetGripActive(&mManipGrip);
				SetUpManipGripVisibility();
			}
		}
	}
...
void EditPolyMod::SetUpManipGripVisibility()
{
	...
	//if on
	if(GetCOREInterface7()->InManipMode()==TRUE && 
		gripManager->GetActiveGrip() == &mManipGrip)
	{
		BitArray copy;
		copy.SetSize((int)EPolyManipulatorGrip::EPolyManipulatorGripCount);
		copy.Set((int)EPolyManipulatorGrip::eSoftSelFalloff, GetManipulateGrip(EPolyManipulatorGrip::eSoftSelFalloff)) ;
		copy.Set((int)EPolyManipulatorGrip::eSoftSelPinch,GetManipulateGrip(EPolyManipulatorGrip::eSoftSelPinch)) ;
		copy.Set((int)EPolyManipulatorGrip::eSoftSelBubble,GetManipulateGrip(EPolyManipulatorGrip::eSoftSelBubble)) ;
		copy.Set((int)EPolyManipulatorGrip::eSetFlow,GetManipulateGrip(EPolyManipulatorGrip::eSetFlow)) ;
		copy.Set((int)EPolyManipulatorGrip::eLoopShift,GetManipulateGrip(EPolyManipulatorGrip::eLoopShift)) ;
		copy.Set((int)EPolyManipulatorGrip::eRingShift, GetManipulateGrip(EPolyManipulatorGrip::eRingShift)) ;
		copy.Set((int)EPolyManipulatorGrip::eEdgeCrease, GetManipulateGrip(EPolyManipulatorGrip::eEdgeCrease)) ;
		copy.Set((int)EPolyManipulatorGrip::eEdgeWeight, GetManipulateGrip(EPolyManipulatorGrip::eEdgeWeight)) ;
		copy.Set(EPolyManipulatorGrip::eEdgeDepth, GetManipulateGrip(EPolyManipulatorGrip::eEdgeDepth)) ;
		copy.Set((int)EPolyManipulatorGrip::eVertexCrease, GetManipulateGrip(EPolyManipulatorGrip::eVertexCrease)) ;
		copy.Set((int)EPolyManipulatorGrip::eVertexWeight, GetManipulateGrip(EPolyManipulatorGrip::eVertexWeight)) ;
		if( GetEPolySelLevel() != EPM_SL_EDGE) // if not in edge, turn off the edge ones!
		{
			copy.Clear((int) EPolyManipulatorGrip::eSetFlow);
			copy.Clear((int) EPolyManipulatorGrip::eLoopShift);
			copy.Clear((int) EPolyManipulatorGrip::eRingShift);
			copy.Clear((int) EPolyManipulatorGrip::eEdgeCrease);
			copy.Clear((int) EPolyManipulatorGrip::eEdgeWeight);
			copy.Clear(EPolyManipulatorGrip::eEdgeDepth);
		}
		if( GetEPolySelLevel() != EPM_SL_VERTEX) // if not in vertex, turn off the vertex ones!
		{
			copy.Clear((int) EPolyManipulatorGrip::eVertexCrease);
			copy.Clear((int) EPolyManipulatorGrip::eVertexWeight);
		}
		int useSoftsel=0;
		mpParams->GetValue (epm_ss_use, TimeValue(0), useSoftsel);
		if(useSoftsel ==0)
		{
			copy.Clear((int) EPolyManipulatorGrip::eSoftSelBubble);
			copy.Clear((int) EPolyManipulatorGrip::eSoftSelFalloff);
			copy.Clear((int) EPolyManipulatorGrip::eSoftSelPinch);
		}

		// Set grip visibility
        for (int g = 0; g != EPolyManipulatorGrip::EPolyManipulatorGripCount; ++g)
        {
            mManipGrip.SetUpVisibility((BOOL)(copy[g]), g);
        }
	}
}
...
float EditPolyMod::GetVertexDataFloatValue (int channel, int *numSel, bool *uniform, DWORD flags, INode *in_pNode) {
	bool foundAny = false;
	if (numSel) *numSel = 0;
	...
	// If in the middle of an interactive operation, return the param block's working value
	if(GetPolyOperationID() == ep_op_set_vertex_data && (getParamBlock()->GetInt (epm_data_channel, t) == channel)) {
		float value = getParamBlock()->GetFloat (epm_data_value, t);
		if(numSel)
			*numSel = 1;
		if(uniform)
			*uniform = true;
	...
	// Copy the default to the return value, just in case
	memcpy (&ret, VertexDataDefault(channel), sizeof(float));

	// We need to look through all ModContexts
	ModContextList list;
	INodeTabAutoDispose nodes;
	ip->GetModContexts(list,nodes);
	for (int mcx=0; mcx<list.Count(); mcx++)
	{
		if (in_pNode != NULL && nodes[mcx]->GetActualINode() != in_pNode) continue;
		MNMesh *mesh = ((EditPolyData *) list[mcx]->localData)->GetMesh();
		if(mesh) {
			// Got a mesh -- Now start getting the data values
			foundAny = true;
			float *vd = mesh->vertexFloat (channel);
			for (i=0; i<mesh->numv; i++) {
				if (mesh->v[i].GetFlag (MN_DEAD)) continue;
				if (!mesh->v[i].GetFlag (flags)) continue;
				if (!found && vd) ret = vd[i];
				found++;
				if (vd && (ret != vd[i])) {
					// Got differing values -- stop here
					mcx = list.Count();
					break;
				}
			}
			if (uniform && (i<mesh->numv)) *uniform = FALSE;
		}
	}
...
float EditPolyMod::GetEdgeDataFloatValue (int channel, int *numSel, bool *uniform, DWORD flags, INode *in_pNode) {
	bool foundAny = false;
	if (numSel) *numSel = 0;
	if (uniform) *uniform = TRUE;
	float ret=1.0f;
	...
	// If in the middle of an interactive operation, return the param block's working value
	if(GetPolyOperationID() == ep_op_set_edge_data && (getParamBlock()->GetInt (epm_data_channel, t) == channel)) {
		float value = getParamBlock()->GetFloat (epm_data_value, t);
		if(numSel)
			*numSel = 1;
		if(uniform)
	...
	for (int mcx=0; mcx<list.Count(); mcx++)
	{
		...
		if(mesh) {
			// Got a mesh -- Now start getting the data values
			foundAny = true;
			float *ed = mesh->edgeFloat (channel);
			for (i=0; i<mesh->nume; i++) {
				if (mesh->e[i].GetFlag (MN_DEAD)) continue;
				if (!mesh->e[i].GetFlag (flags)) continue;
				if (!found && ed) ret = ed[i];
				found++;
				if (ed && (ret != ed[i])) {
					// Got differing values -- stop here
					mcx = list.Count();
			...
			if (uniform && (i<mesh->nume)) *uniform = FALSE;
		}
	}
...
static bool inPerDataOperation = false;
static int perDataType;
static int perDataChannel;

void EditPolyMod::BeginPerDataModify (int mnSelLevel, int channel) {
	DbgAssert(inPerDataOperation == false);
	theHold.Begin ();
	switch(mnSelLevel) {
	case MNM_SL_VERTEX:
		EpModSetOperation (ep_op_set_vertex_data);
		break;
	case MNM_SL_EDGE:
		EpModSetOperation (ep_op_set_edge_data);
		break;
	}
	perDataType = mnSelLevel;
	perDataChannel = channel;
	inPerDataOperation = true;
}

bool EditPolyMod::InPerDataModify () {
	return inPerDataOperation;
}

void EditPolyMod::DoPerDataModify (int mnSelLevel, int channel, float value) {
	if (!inPerDataOperation)
		BeginPerDataModify(mnSelLevel, channel);
	TimeValue t = GetCOREInterface()->GetTime();
	getParamBlock()->SetValue (epm_data_channel, t, channel);	
	getParamBlock()->SetValue (epm_data_value, t, value);
}

void EditPolyMod::EndPerDataModify (bool success) {
	if (!inPerDataOperation) {
		DbgAssert(0);
		return;
	}
	if (success) {
		EpModCommitUnlessAnimating (GetCOREInterface()->GetTime());
		switch(perDataType) {
		case MNM_SL_VERTEX:
			switch(perDataChannel) {
				case VDATA_WEIGHT:
		...
		case MNM_SL_EDGE:
			switch(perDataChannel) {
				...
				case EDATA_CREASE:
					...
					break;
                case EDATA_DEPTH:
                    theHold.Accept(GetString(IDS_CHANGE_DEPTH));
                    break;
			}
			break;
		}
		...
		theHold.Cancel();
	EpModRefreshScreen ();
	inPerDataOperation = false;
}

void EditPolyMod::UpdatePerDataDisplay (TimeValue t, int msl, int channel) {
	if (msl == MNM_SL_CURRENT) msl = meshSelLevel[selLevel];
	if (msl != meshSelLevel[selLevel]) return;
	ResetGripUI();
	HWND hGeom = GetDlgHandle (ep_subobj);
	if (!hGeom) return;
	if(channel < 0) {
		// Update all channels for the selection level
		switch(msl) {
		case MNM_SL_VERTEX:
			TheSubobjDlgProc()->UpdatePerDataDisplay (t, selLevel, VDATA_WEIGHT, hGeom);
			TheSubobjDlgProc()->UpdatePerDataDisplay (t, selLevel, VDATA_CREASE, hGeom);
			break;
		case MNM_SL_EDGE:
			TheSubobjDlgProc()->UpdatePerDataDisplay (t, selLevel, EDATA_KNOT, hGeom);
			TheSubobjDlgProc()->UpdatePerDataDisplay (t, selLevel, EDATA_CREASE, hGeom);
            (TheSubobjDlgProc())->UpdatePerDataDisplay(t, selLevel, EDATA_DEPTH, hGeom);
			break;
		}
	}
	else {
		TheSubobjDlgProc()->UpdatePerDataDisplay (t, selLevel, channel, hGeom);
	}
}

void EditPolyMod::ResetVertexData (int channel, INode *in_pNode) {
	//Get mesh and perform validation checks
	MNMesh *mesh = EpModGetMesh (in_pNode);
	if ( mesh == NULL )
		return;
	mesh->freeVData (channel);
	EpModLocalDataChanged(GEOM_CHANNEL);
//	macroRecorder->FunctionCall(_T("$.modifiers[#Edit_Poly].resetVertexData"), 1, 0,
//		mr_int, channel);
//	macroRecorder->EmitScript ();
}

void EditPolyMod::ResetEdgeData (int channel, INode *in_pNode) {
	//Get mesh and perform validation checks
	MNMesh *mesh = EpModGetMesh (in_pNode);
	if ( mesh == NULL )
		return;
	mesh->freeEData (channel);
	EpModLocalDataChanged(TOPO_CHANNEL);
//	macroRecorder->FunctionCall(_T("$.modifiers[#Edit_Poly].resetEdgeData"), 1, 0,
//		mr_int, channel);
//	macroRecorder->EmitScript ();
}
