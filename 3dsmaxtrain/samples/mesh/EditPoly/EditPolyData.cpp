...
void CheckPerEdgeData(MNMesh* mesh)
{
    DbgAssert(mesh);
    if (mesh)
    {
        // Watch for bad per-edge data
        for (int i = 0; i < mesh->EDNum(); i++) {
            if (!mesh->eDataSupport(i)) continue;
            bool OK = (mesh->GetED(i)->alloc >= mesh->nume);
            if (!OK)
            {
                DbgAssert(0);
            }
...
EditPolyData::EditPolyData (PolyObject *pObj, TimeValue t, bool doClearNormals) : mpMesh(NULL), mpMeshOutput(NULL),
	mFlags(0x0), mpNewSelection(NULL), mpOpList(NULL), mpTemp(NULL), mpTempOutput(NULL), mpPolyOpData(NULL),
	mSlicePlaneSize(-1.0f), mpPaintMesh(NULL), mUseRampageWeldMath(false), mDoBadVertRemovalOnSelection(false), mOSD_GPU_Display(NULL)
{
	Initialize (pObj, t, doClearNormals);
}
...
EditPolyData::~EditPolyData() {
	...
	if (mOSD_GPU_Display) 
	{
		mOSD_GPU_Display->mpCustomRenderItem=NULL;
		delete mOSD_GPU_Display;
	}
	mOSD_GPU_Display=NULL;
}

void EditPolyData::Initialize (PolyObject *pObj, TimeValue t, bool doClearNormals) {
...
LocalModData *EditPolyData::Clone() {
	...
	d->mUseRampageWeldMath = mUseRampageWeldMath;
	d->mDoBadVertRemovalOnSelection = mDoBadVertRemovalOnSelection;
...
void EditPolyData::SetCache(PolyObject *pObj, TimeValue t) {
	...
	*mpMesh = pObj->GetMesh();
	...
	DebugPrint (_T("EditPolyData::Setting main mesh cache.\n"));
#endif
...
#ifdef __DEBUG_PRINT_EDIT_POLY
	DebugPrint (_T("EditPolyData::Setting output cache.\n"));
#endif
}
...
void EditPolyData::SetVertHide (const BitArray &set, EditPolyMod *mod) {
	...
	InvalidateTempData (DISP_ATTRIB_CHANNEL);
}

void EditPolyData::SetFaceHide (const BitArray &set, EditPolyMod *mod) {
...
void EditPolyData::SetVertSel(const BitArray &set, IMeshSelect *imod, TimeValue t) {
...
void EditPolyData::SetEdgeSel(const BitArray &set, IMeshSelect *imod, TimeValue t) {
	EditPolyMod *mod = (EditPolyMod *) imod;
...
void EditPolyData::SetFaceSel(const BitArray &set, IMeshSelect *imod, TimeValue t) {
	EditPolyMod *mod = (EditPolyMod *) imod;
...
void EditPolyData::ApplyAllOperations(MNMesh & mesh, int limit)
{
#ifdef __DEBUG_PRINT_EDIT_POLY
	...
	if (mpOpList) {
		...
		newMapVertices.SetCount (mesh.MNum() + NUM_HIDDENMAPS);
		for (int mp=-NUM_HIDDENMAPS; mp<mesh.MNum(); mp++) newMapVertices[mp+NUM_HIDDENMAPS] = 0;
		int index = 0;
        bool doClearNormals = false;
		for (pOpRec=mpOpList; pOpRec != NULL; pOpRec=pOpRec->Next()) {
            const PolyOperation* opCurr = pOpRec->Operation();
			newFaces += opCurr->NewFaceCount();
			newVertices += opCurr->NewVertexCount();
			newEdges += opCurr->NewEdgeCount ();
            bool canPropagateNormals = opCurr->CanPropagateSpecifiedNormals();
			...
			for (int mp=-NUM_HIDDENMAPS; mp<mesh.MNum(); mp++) {
				...
				newMapVertices[mp+NUM_HIDDENMAPS] += opCurr->NewMapVertexCount(mp);
			}
        ...
        // If any operation in the list that we are to execute does not support propagation of specified normals, clear the
        // normals now
        if (doClearNormals)
        {
            mesh.ClearSpecifiedNormals();
...
void EditPolyData::InvalidateTempData (ChannelMask channels)
{
	...
	if (channels & (GEOM_CHANNEL|TOPO_CHANNEL))
	{
		InvalidateDistances ();
	}

	if (mpTemp) mpTemp->Invalidate (channels);
	if (mpTempOutput) mpTempOutput->Invalidate (channels);
}
...
void EditPolyData::SelectAllBorderEdges(BitArray &selection)
{
	if (!GetMesh()) return;	// shouldn't happen.
	selection.ClearAll();
	for (int i = 0; i<mpMesh->nume; i++) {
		MNEdge& theEdge = mpMesh->e[i];
		if (theEdge.GetFlag(MN_DEAD)) continue;
		if (theEdge.f1 < 0 || theEdge.f2 < 0)
			selection.Set(i);
	}
}

void EditPolyData::SelectInvertBorderEdges(BitArray &selection)
{
	if (!GetMesh()) return;	// shouldn't happen.
	for (int i = 0; i<mpMesh->nume; i++) {
		MNEdge& theEdge = mpMesh->e[i];
		if (theEdge.GetFlag(MN_DEAD)) continue;
		if (theEdge.f1 < 0 || theEdge.f2 < 0)
			selection.Set(i, selection[i] ? FALSE : TRUE);
	}
}
...
void EditPolyData::SelectEdgeRing(IMeshSelect *imod) {
	...
	if (!mDoBadVertRemovalOnSelection)
	{
		TemporaryNoBadVerts nbv(*mpMesh);
...
void EditPolyData::SelectByHardEdge (EditPolyMod *pMod) {
	if (pMod->GetMNSelLevel() != MNM_SL_EDGE) return;
	if (!mpMesh) return;

	SetupNewSelection (MNM_SL_EDGE);
	BitArray *pSel = GetNewSelection ();
	for (int i=0; i<pSel->GetSize(); i++)
	{
		MNEdge &theEdge = mpMesh->e[i];
		if(theEdge.GetFlag(MN_DEAD) || (theEdge.f2 < 0)) continue;
		if ((mpMesh->f[theEdge.f1].smGroup & mpMesh->f[theEdge.f2].smGroup) == 0)
			pSel->Set (i);
	}

	ApplyNewSelection (pMod, false);
}

void EditPolyData::SelectBySmoothEdge (EditPolyMod *pMod) {
	if (pMod->GetMNSelLevel() != MNM_SL_EDGE) return;
	if (!mpMesh) return;
...
public:
	...
	int Size () { return 8; }
	TSTR Description() { return _T("SetPolyOpData"); }
};
...
ReferenceTarget* FindFirstNonXRefTarg(IXRefItem* pXRefItem)
{
...
class CollectModContextEnumProc : public ModContextEnumProc
{
public:
	virtual ~CollectModContextEnumProc() {}
	CollectModContextEnumProc(ModContextList* pList) : mpList(pList) {}
...
LocalModData *EditPolyMod::GetPrimaryLocalModData ()
{
	...
	{
		CollectModContextEnumProc collectProc(&list);
		this->EnumModContexts(&collectProc);
	}
