...
void EditPolyMod::OnParamSet(PB2Value& v, ParamID id, int tabIndex, TimeValue t) {
	...
	case epm_paintdeform_mode:
		...
		break;
	case epm_ignore_backfacing:
	{
		// When we enable ignore back facing, we disable ignore occluded
		if (v.i)
			mpParams->SetValue(epm_ignore_occluded, t, !v.i);

		// Make sure our button's state reflects its status
		HWND hwnd = mpDialogSelect != nullptr ? mpDialogSelect->GetHWnd() : nullptr;
	...
	case epm_ignore_occluded:
	{
		// When we enable ignore occluded, we disable ignore back facing
		if (v.i)
			mpParams->SetValue(epm_ignore_backfacing, t, !v.i);
...
const TCHAR *EditPolyMod::GetPaintUndoString (PaintModeID mode) {
