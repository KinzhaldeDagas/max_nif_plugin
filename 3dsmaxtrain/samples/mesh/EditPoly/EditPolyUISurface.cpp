...
extern EditPolySubobjControlDlgProc *TheSubobjDlgProc ();
...
void EditPolyMod::InvalidateSurfaceUI() {
	...
	if (mpDialogSubobj)
	{
		TheSubobjDlgProc()->Invalidate ();
		HWND hWnd = mpDialogSubobj->GetHWnd();
		if (hWnd) InvalidateRect (hWnd, NULL, FALSE);
	}

	//Make sure any active caddies get updated properly
	ResetGripUI();
