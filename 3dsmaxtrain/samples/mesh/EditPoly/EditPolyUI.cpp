...
enum epolyModEnumList { epolyModEnumButtonOps, epolyModEnumCommandModes, epolyModEnumPickModes,
	epolyModSelLevel, epolyModMeshSelLevel, epolyModEdgeChamferType
};
...
EPOLY_MOD_INTERFACE, _T("EditPolyMod"), IDS_EDIT_POLY_MOD, &emodDesc, FP_MIXIN,
    ...
    epmod_get_last, _T("GetLastOperation"), 0, TYPE_INT, 0, 0,
	...
	epmod_list_delta_ops, _T("ListDeltaOps"), 0, TYPE_VOID, 0, 0,

	enums,
		epolyModEnumButtonOps, 86,
			...
			_T("RemoveEdgeRemoveVertices"), ep_op_remove_edge_remove_verts,
			_T("MakeHardEdges"), ep_op_make_hard_edges,
			_T("MakeSmoothEdges"), ep_op_make_smooth_edges,
			_T("SelectHardEdges"), ep_op_select_hard_edges,
			_T("SelectSmoothEdges"), ep_op_select_smooth_edges,
		...
		epolyModEdgeChamferType, 2,
			_T("Standard"), EP_STANDARD_EDGE_CHAMFER,
			_T("Quad"), EP_QUAD_EDGE_CHAMFER,

	properties,

		epmod_get_edge_chamfer_type, epmod_set_edge_chamfer_type, _T("edgeChamferType"), 0, TYPE_ENUM, epolyModEdgeChamferType,
		epmod_get_edge_chamfer_quad_intersections, epmod_set_edge_chamfer_quad_intersections, _T("edgeChamferQuadIntersections"), 0, TYPE_BOOL,
...
const TCHAR *LookupEditPolySelLevel (int selLevel) {
	int count = editpolymod_fi.enumerations.Count();
   int i;
	for (i=0; i<count; i++)
...
class EditPolyModPBAccessor : public PBAccessor {
	...
	void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)
		{ 
			...
			EditPolyMod *mpEditPolyMod = (EditPolyMod *)owner;
			switch (id)
			{
			case epm_debug_last_delta:
			{
				ModContextList list;
				INodeTab nodes;
				...
				for (int i = 0; i < list.Count(); i++)
				{
					...
					pData->InvalidateCache();
				}
			}
			default:
				mpEditPolyMod->OnParamSet(v, id, tabIndex, t);
				break;
			}
		}
...
class EditPolyConstraintValidatorClass : public PBValidator
{
	...
	{ 
		if(v.i<0||v.i>3)
			return FALSE;
		int last;
		((EditPolyMod*)owner)->getParamBlock()->GetValue(epm_constrain_type, TimeValue(0), last);
...
BOOL isSmartExtrudeEnabledByDefault() {

	BOOL isEnabled = TRUE;

	TCHAR* defaultSetting = _tgetenv(_T("ADSK_3DSMAX_SMART_EXTRUDE"));

	if (defaultSetting)
	{
		isEnabled = _tstoi(defaultSetting);
	}
	return isEnabled;
}
...
static ParamBlockDesc2 edit_mod_param_blk (kEditPolyParams, _T("Edit Poly Parameters"), 0, &emodDesc,
	...
	p_default, false,
	p_ui, ep_animate, TYPE_SINGLECHECKBOX, IDC_SHOW_CAGE,
	p_end,

	epm_stack_selection, _T("useStackSelection"), TYPE_BOOL, P_RESET_DEFAULT, IDS_EDITPOLY_USE_STACK_SELECTION,
	p_default, 0,
	p_ui, ep_select, TYPE_SINGLECHECKBOX, IDC_EDITPOLY_STACK_SELECTION,
	p_end,
	...
	epm_ignore_occluded, _T("ignoreOccluded"), TYPE_BOOL, P_RESET_DEFAULT, IDS_EDITPOLY_IGNORE_OCCLUDED,
	p_default, FALSE,
	p_accessor, &theEditPolyModPBAccessor,
	p_end,
	...
	p_default, true,
	p_ui, ep_geom, TYPE_SINGLECHECKBOX, IDC_EDITPOLY_DELETE_ISO_VERTS,
	p_end,
	...
	p_nonLocalizedName, _T("Polygon Extrude Height"),
	p_end,
	...
	p_nonLocalizedName, _T("Vertex Extrude Height"),
	p_end,
	...
	p_nonLocalizedName, _T("Edge Extrude Height"),
	p_end,
	...
	p_nonLocalizedName, _T("Vertex Extrude Width"),
	p_end,
	...
	p_nonLocalizedName, _T("Edge Extrude Width"),
	p_end,
	...
	p_nonLocalizedName, _T("Bevel Height"),
	p_end,
	...
	p_nonLocalizedName, _T("Bevel Outline"),
	p_end,
	...
	p_nonLocalizedName, _T("Outline Amount"),
	p_end,
	...
	p_nonLocalizedName, _T("Inset amount"),
	p_end,
	...
	p_nonLocalizedName, _T("Vertex Chamfer Amount"),
	p_end,

	epm_chamfer_vertex_open, _T("chamferVertexOpen"), TYPE_BOOL, P_TRANSIENT, IDS_OPEN_CHAMFER,
	p_default, FALSE,
	p_ui, ep_settings, TYPE_CHECKBUTTON, IDC_CHAMFER_VERTEX_OPEN,
	p_end,
	...
	p_nonLocalizedName, _T("Edge Chamfer Amount"),
	p_end,
	...
	p_nonLocalizedName, _T("Vertex Weld Threshold"),
	p_end,
	...
	p_nonLocalizedName, _T("Edge Weld Threshold"),
	p_end,
	...
	p_nonLocalizedName, _T("Tessellate By"),
	p_end,
	...
	p_nonLocalizedName, _T("Tessellation Tension"),
	p_end,
	...
	p_nonLocalizedName, _T("Connect Edge Segments"),
	p_end,
	...
	p_nonLocalizedName, _T("Extrude Along Spline Segments"),
	p_end,
	...
	p_nonLocalizedName, _T("Extrude Along Spline Taper Amount"),
	p_end,
	...
	p_nonLocalizedName, _T("Extrude Along Spline Taper Curve"),
	p_end,
	...
	p_nonLocalizedName, _T("Extrude Along Spline Twist"),
	p_end,
	...
	p_nonLocalizedName, _T("Extrude Along Spline Rotation"),
	p_end,
	...
	p_nonLocalizedName, _T("Extrude Along Spline Align"),
	p_end,
	...
	p_nonLocalizedName, _T("Hinge From Edge Angle"),
	p_end,
	...
	p_nonLocalizedName, _T("Hinge From Edge Segments"),
	p_end,
	...
	p_nonLocalizedName, _T("Autosmooth Threshold"),
	p_end,
	...
	p_default, 0,
	p_nonLocalizedName, _T("Material ID to Set"),
	p_end,
	...
	p_default, FALSE,
	p_ui, ep_surface, TYPE_SINGLECHECKBOX, IDC_CLEARSELECTION,
	p_end,
	...
	p_nonLocalizedName, _T("Bridge Segments"),
	p_end,
	...
	p_nonLocalizedName, _T("Bridge Taper"),
	p_end,
	...
	p_nonLocalizedName, _T("Bridge Bias"),
	p_end,
	...
	p_nonLocalizedName, _T("Bridge Smoothing Threshold"),
	p_end,
	...
	p_nonLocalizedName, _T("Bridge Adjacent"),
	p_end,
	...
	p_nonLocalizedName, _T("Connect Edge Pinch"),
	p_end,
	...
	p_nonLocalizedName, _T("Connect Edge Slide"),
	p_end,
	...
	epm_edge_chamfer_mitering_type, _T("edgeChamferMiteringType"), TYPE_INT, P_TRANSIENT, IDS_EDGE_CHAMFER_MITERING_TYPE,
	p_default, EP_EDGE_CHAMFER_UNIFORM_MITER,
	p_range, EP_EDGE_CHAMFER_QUAD_MITER, EP_EDGE_CHAMFER_PATCH_MITER,
	...
	p_vals, EP_EDGE_CHAMFER_QUAD_MITER, EP_EDGE_CHAMFER_TRI_MITER, EP_EDGE_CHAMFER_UNIFORM_MITER, EP_EDGE_CHAMFER_RADIAL_MITER, EP_EDGE_CHAMFER_PATCH_MITER, 
	p_end,

	epm_edge_chamfer_tension, _T("edgeChamferTension"), TYPE_FLOAT, P_TRANSIENT, IDS_EDGE_CHAMFER_TENSION,
	p_default, 1.0f,
	p_range, 0.0f, 1.0f,
	p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_POS_FLOAT, IDC_EDGE_CHAMFER_TENSION, IDC_EDGE_CHAMFER_TENSION_SPIN, SPIN_AUTOSCALE,
	p_end,

	epm_edge_chamfer_depth, _T("edgeChamferDepth"), TYPE_FLOAT, P_TRANSIENT, IDS_EDGE_CHAMFER_DEPTH, 
	p_default, 0.5f, 
	p_range, -0.5f, 1.0f, 
	p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_EDGE_CHAMFER_DEPTH, IDC_EDGE_CHAMFER_DEPTH_SPIN, SPIN_AUTOSCALE, 
	p_end,

	epm_edge_chamfer_end_bias, _T("edgeChamferEndBias"), TYPE_FLOAT, P_TRANSIENT, IDS_EDGE_CHAMFER_END_BIAS,
	p_default, 0.5f, 
	p_range, 0.0f, 1.0f,
	p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_POS_FLOAT, IDC_EDGE_CHAMFER_END_BIAS, IDC_EDGE_CHAMFER_END_BIAS_SPIN, SPIN_AUTOSCALE, 
	p_end,

	epm_edge_chamfer_radial_bias, _T("edgeChamferRadialBias"), TYPE_FLOAT, P_TRANSIENT, IDS_EDGE_CHAMFER_RADIAL_BIAS, 
	p_default, 0.0f, 
	p_range, 0.0f, 1.0f, 
	...
	epm_edge_chamfer_invert, _T("edgeChamferInvert"), TYPE_BOOL, P_TRANSIENT, IDS_EDGE_CHAMFER_INVERT,
	p_default, FALSE,
	p_ui, ep_settings, TYPE_CHECKBUTTON, IDC_EDGE_CHAMFER_INVERT,
	p_end,

	epm_edge_chamfer_smooth, _T("edgeChamferSmooth"), TYPE_BOOL, P_TRANSIENT, IDS_EDGE_CHAMFER_SMOOTH,
	p_default, TRUE,		// Smoothing defaults to ON
	p_ui, ep_settings, TYPE_SINGLECHECKBOX, IDC_EDGE_CHAMFER_SMOOTH,
	p_end,

	epm_edge_chamfer_smooth_type, _T("edgeChamferSmoothType"), TYPE_INT, P_TRANSIENT, IDS_EDGE_CHAMFER_SMOOTH_TYPE,
	p_default, EP_EDGE_CHAMFER_SMOOTH_ALL,	// Defaults to smoothing whole object
	p_range, EP_EDGE_CHAMFER_SMOOTH_ALL, EP_EDGE_CHAMFER_SMOOTH_CHAMFER,
	p_ui, ep_settings, TYPE_RADIO, 2, IDC_EDGE_CHAMFER_SMOOTH_ALL, IDC_EDGE_CHAMFER_SMOOTH_CHAMFERS,
	p_end,

	epm_edge_chamfer_smooth_threshold, _T("edgeChamferSmoothThreshold"), TYPE_FLOAT, P_TRANSIENT, IDS_EDGE_CHAMFER_SMOOTH_THRESHOLD,
	p_default, 30.0f,				// Defaults to 30-degree smoothing threshold
	p_range, 0.0f, 180.0f,
	...
	epm_data_channel, _T("dataChannel"), TYPE_INT, P_TRANSIENT, IDS_DATA_CHANNEL,
	p_end,

	epm_data_value, _T("dataValue"), TYPE_FLOAT, P_TRANSIENT, IDS_DATA_VALUE,
	p_end,

	epm_hard_edge_display_color, _T("hardedgedisplaycolor"), TYPE_RGBA, P_RESET_DEFAULT, IDS_HARD_EDGE_DISPLAY_COLOR,
	p_default, Point3(0.0f, 0.93f, 0.156f),		// Defaults to medium green
	p_end,

	epm_hard_edge_display, _T("hardedgedisplay"), TYPE_INT, P_RESET_DEFAULT, IDS_HARD_EDGE_DISPLAY_OPTION,
	p_default, EP_HARD_EDGE_DISPLAY_OFF,
	p_range, 0, 2,	// IDS_DONT_COLOR_HARD_EDGES...IDS_COLOR_HARD_VISIBLE_EDGES
	p_end,

	epm_debug_last_delta, _T("lastDeltaIndex"), TYPE_INT, 0, IDS_LAST_DELTA_INDEX,
	p_default, -1,
	p_accessor, &theEditPolyModPBAccessor,
	p_end,

	epm_extrude_face_bias, _T("extrudeFaceBias"), TYPE_FLOAT, P_ANIMATABLE, IDS_EXTRUDEFACEBIAS,
	p_default, 1.0f,
	p_range, 0.0f, 1.0f,
	p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_UNIVERSE,
	IDC_FACE_EXTRUDE_BIAS, IDC_FACE_EXTRUDE_BIAS_SPIN, SPIN_AUTOSCALE,
	p_nonLocalizedName, _T("Extrude Face Bias"),
	p_end,

	epm_bevel_face_bias, _T("bevelFaceBias"), TYPE_FLOAT, P_ANIMATABLE, IDS_BEVELFACEBIAS,
	p_default, 1.0f,
	p_range, 0.0f, 1.0f,
	p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_UNIVERSE,
	IDC_BEVEL_BIAS, IDC_BEVEL_BIAS_SPIN, SPIN_AUTOSCALE,
	p_nonLocalizedName, _T("Bevel Face Bias"),
	p_end,

    // The following item is kept around for legacy purposes
    epm_edge_chamfer_type, _T("edgeChamferTypeOBSOLETE"), TYPE_INT, P_TRANSIENT, IDS_EDGE_CHAMFER_TYPE,
    p_default, EP_QUAD_EDGE_CHAMFER,	// Defaults to quad chamfer
    p_range, EP_STANDARD_EDGE_CHAMFER, EP_QUAD_EDGE_CHAMFER,
    p_end,

    // The following item is kept around for legacy purposes
    epm_edge_chamfer_quad_intersections, _T("edgeChamferQuadIntersectionsOBSOLETE"), TYPE_BOOL, P_TRANSIENT, IDS_EDGE_CHAMFER_QUAD_INTERSECTIONS,
    p_default, FALSE,
    p_end,

    epm_vertex_chamfer_num_segments, _T("chamferVertexSegments"), TYPE_INT, P_TRANSIENT, IDS_VERTEX_CHAMFER_SEGMENTS,
    p_default, 1,
    p_range, 0, 99,
    p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_POS_INT, IDC_VERTEX_CHAMFER_SEGMENTS, IDC_VERTEX_CHAMFER_SEGMENTS_SPIN, 0.5f,
    p_end,

    epm_vertex_chamfer_depth, _T("chamferVertexDepth"), TYPE_FLOAT, P_TRANSIENT, IDS_VERTEX_CHAMFER_DEPTH,
    p_default, 0.5f,
    p_range, -0.5f, 1.0f,
    p_ui, ep_settings, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_VERTEX_CHAMFER_DEPTH, IDC_VERTEX_CHAMFER_DEPTH_SPIN, SPIN_AUTOSCALE, 
    p_end,

    epm_vertex_chamfer_smooth, _T("chamferVertexSmooth"), TYPE_BOOL, P_TRANSIENT, IDS_VERTEX_CHAMFER_SMOOTH,
    p_default, TRUE,
    p_ui, ep_settings, TYPE_SINGLECHECKBOX, IDC_VERTEX_CHAMFER_SMOOTH,
    p_end,

    epm_vertex_chamfer_smooth_type, _T("chamferVertexSmoothType"), TYPE_INT, P_TRANSIENT, IDS_VERTEX_CHAMFER_SMOOTH_TYPE,
    p_default, EP_EDGE_CHAMFER_SMOOTH_ALL,
    p_range, EP_EDGE_CHAMFER_SMOOTH_ALL, EP_EDGE_CHAMFER_SMOOTH_CHAMFER,
    p_ui, ep_settings, TYPE_RADIO, 2, IDC_VERTEX_CHAMFER_SMOOTH_ALL, IDC_VERTEX_CHAMFER_SMOOTH_CHAMFERS,
    p_end,

    epm_vertex_chamfer_smooth_threshold, _T("chamferVertexSmoothThreshold"), TYPE_FLOAT, P_TRANSIENT, IDS_VERTEX_CHAMFER_SMOOTH_THRESHOLD,
    p_default, 30.0f,
    p_range, 0.0f, 180.0f,
    ...
    epm_enable_enhanced_hotkey_extrude, _T("enableEnhancedHotkeyExtrude"), TYPE_BOOL, P_RESET_DEFAULT, IDS_ENABLE_ENHANCED_HOTKEY_EXTRUDE,
    p_default, isSmartExtrudeEnabledByDefault(),
    p_end,

    epm_enable_enhanced_retriangulation, _T("enableEnhancedRetriangulation"), TYPE_BOOL, P_RESET_DEFAULT, IDS_ENABLE_ENHANCED_RETRIANGULATION,
    p_default, TRUE,
    p_end,
...
void EditPolySelectProc::SetEnables (HWND hParams) {
	...
	int stackSelect = params->GetInt (epm_stack_selection, TimeValue(0)) ? true : false;
	BOOL multiSelect = params->GetInt (epm_select_mode, TimeValue(0)) == 2;
	BOOL highlightSelect = params->GetInt (epm_select_mode, TimeValue(0)) == 1;
    ...
    bool areCullingControlsEnabled = (!stackSelect) && soLevel;
    pBut = GetICustButton(GetDlgItem(hParams, IDC_CULL_BACKFACE));
    if (pBut != nullptr)
    {

        // Enable the backface culling selection option button, and set the state of the button, consistent with the
        // corresponding parameter block entry, if we are in an appropriate selection mode
        pBut->Enable(areCullingControlsEnabled);
        if (areCullingControlsEnabled)
        {
            BOOL ignoreBackfacing = FALSE;
            Interval interval = FOREVER;
            params->GetValue(epm_ignore_backfacing, timeCurr, ignoreBackfacing, interval);
            pBut->SetCheck(ignoreBackfacing);
        }
    ...
    pBut = GetICustButton(GetDlgItem(hParams, IDC_CULL_OCCLUDED));
    if (pBut != nullptr)
    {
        ...
        if (areCullingControlsEnabled)
        {
            ...
            Interval interval = FOREVER;
            params->GetValue(epm_ignore_occluded, timeCurr, ignoreOccluded, interval);
            pBut->SetCheck(ignoreOccluded);
        }
	...
	if (params->GetInt (epm_by_vertex)) facNotElem = false;
	...
	if (spinner) {
		bool useAngle = params->GetInt (epm_select_by_angle) != 0;
...
INT_PTR EditPolySelectProc::DlgProc (TimeValue t, IParamMap2 *map,
	...
	case WM_SUBOBJECTPICK:
			if (wParam == 1)
				GetCOREInterface()->SetSubObjectLevel(EPM_SL_VERTEX);
			else if (wParam == 2)
				GetCOREInterface()->SetSubObjectLevel(EPM_SL_EDGE);
			else if (wParam == 3)
				GetCOREInterface()->SetSubObjectLevel(EPM_SL_FACE);
		break;
	case WM_COMMAND:
		switch (LOWORD(wParam)) {
		case IDC_SELVERTEX:
		...
		case IDC_CULL_BACKFACE:
		{
			...
			mpMod->getParamBlock()->GetValue(epm_ignore_backfacing, TimeValue(0), checkboxValue);
		...
		case IDC_CULL_OCCLUDED:
		{
			theHold.Begin();
			BOOL checkboxValue = FALSE;
			mpMod->getParamBlock()->GetValue(epm_ignore_occluded, TimeValue(0), checkboxValue);
			mpMod->getParamBlock()->SetValue(epm_ignore_occluded, TimeValue(0), !checkboxValue);
			theHold.Accept(GetString(IDS_PARAMETERS));
			break;
		}
		}
...
void EditPolySoftselDlgProc::DrawCurve (TimeValue t, HWND hWnd,HDC hdc) {
	...
	pblock->GetValue (epm_ss_falloff, t, falloff);
	pblock->GetValue (epm_ss_pinch, t, pinch);
	pblock->GetValue (epm_ss_bubble, t, bubble);
...
BOOL SelectionModeOverride ::StartOverride()
{
	if(mActiveOverride==TRUE) //already TRUE, exit
		return FALSE;
	if (mpMod == nullptr || mpMod->getParamBlock() == nullptr)
		return FALSE;
	mActiveOverride = TRUE;

	mpMod->getParamBlock()->GetValue (epm_select_mode, TimeValue(0), mLastSelection);
...
BOOL ConstraintOverride::StartOverride()
{
	...
	if (mpMod == nullptr || mpMod->getParamBlock() == nullptr)
		return FALSE;
	mActiveOverride = TRUE;
	mpMod->getParamBlock()->GetValue (epm_constrain_type, TimeValue(0), mPrevConstraint);
...
public:
	SubobjectPickModeOverride(EditPolyMod *m):CommandModeOverride(m,-1){}
	BOOL StartOverride();
...
BOOL SubobjectPickModeOverride::StartOverride()
{
	...
	mOldCommandMode = mpMod->EpModGetCommandMode();
	mPrevPaintMode = mpMod->GetPaintMode();
	mpMod->EpModToggleCommandMode (ep_mode_subobjectpick);
	mCommandMode= ep_mode_subobjectpick;	
	SubobjectPickMode* mode = (SubobjectPickMode*) mpMod->getCommandMode(ep_mode_subobjectpick);
...
public:
	PointToPointModeOverride(EditPolyMod *m) :CommandModeOverride(m, -1) {}
	BOOL StartOverride();
...
BOOL PointToPointModeOverride::StartOverride()
{
	...
	mOldCommandMode = mpMod->EpModGetCommandMode();
	mPrevPaintMode = mpMod->GetPaintMode();
	mpMod->EpModToggleCommandMode(ep_mode_point_to_point);
	mCommandMode = ep_mode_point_to_point;

	return TRUE;
}


class PolyModSubModSelectionProcessor: public SubModSelectionProcessor
{
protected:
	// override
	...
	virtual void OverrideDoubleClickProc(ViewExp* vpt, int flags) override {
		...
		EditPolyMod* pPolyMod = dynamic_cast<EditPolyMod*>(obj);
		if (NULL != pPolyMod)
		{
			if (pPolyMod->GetMNSelLevel() == MNM_SL_EDGE) 
			{
				theHold.Begin();
				pPolyMod->DoubleClickSelect(vpt->ToPointer()->GetSubObjHitList().ClosestHit());
				theHold.Accept(GetString(IDS_LOOP_EDGE_SEL));
			}
			else if (pPolyMod->GetMNSelLevel() == MNM_SL_FACE || pPolyMod->GetMNSelLevel() == MNM_SL_VERTEX)
			{
				theHold.Begin();
				pPolyMod->DoubleClickSelect(vpt->ToPointer()->GetSubObjHitList().ClosestHit());
...
public:
	PolyModSubModSelectionProcessor(TransformModBox *mc, BaseObject* o, IObjParam* i) 
		: SubModSelectionProcessor(mc,o,i){}
};

template <class BasePolyMode, class SelectionProcessorType, class ModBoxType>
class PolyDoubleClickModeDecorator: public BasePolyMode
{
private:
	SelectionProcessorType polyModMouseProc;
	ModBoxType theProc;			
public:
	PolyDoubleClickModeDecorator(BaseObject* o, IObjParam* i) :
	  BasePolyMode(o, i),
		  theProc(o, i),
		  polyModMouseProc(&theProc, o, i)
	  {
		  polyModMouseProc.SetSupportTransformGizmo(TRUE);
	  }

	  MouseCallBack *MouseProc(int *numPoints) 
	  { 
		  *numPoints=2;
		  return &polyModMouseProc; 
	  }
};

template <class BasePolyMode, class SelectionProcessorType>
class PolyDoubleClickModeDecorator<BasePolyMode, SelectionProcessorType, SelectModBox>: public BasePolyMode
{
private:
	SelectionProcessorType polyModMouseProc;
public:
...
#define ROLLUP_ORDER_SUBOBJ		1
#define ROLLUP_ORDER_GEOM		2
#define ROLLUP_ORDER_MATIDS		3
#define ROLLUP_ORDER_POLYSMOOTH	4
#define ROLLUP_ORDER_PAINTDEF	5

void EditPolyMod::BeginEditParams (IObjParam  *ip, ULONG flags,Animatable *prev) {
    typedef PolySelectionProcessor<PolyModSubModSelectionProcessor, EditPolyMod> PolyModSelectionProcessor;
    typedef PolyModeSelectionProcessor<PolyModSelectionProcessor, EditPolyMod> PolyModModeSelectionProcessor;
	...
	if (selLevel != EPM_SL_OBJECT) {
		mpDialogSubobj = CreateCPParamMap2 (ep_subobj, mpParams, ip, hInstance,
			...
			mRollupSubobj ? APPENDROLL_CLOSED : 0, TheSubobjDlgProc(), NULL, ROLLUP_CAT_STANDARD + ROLLUP_ORDER_SUBOBJ);
	...
	mpDialogGeom = CreateCPParamMap2 (ep_geom, mpParams, ip, hInstance,
		...
		mRollupGeom ? APPENDROLL_CLOSED : 0, TheGeomDlgProc(), NULL, ROLLUP_CAT_STANDARD + ROLLUP_ORDER_GEOM);
	...
	if (meshSelLevel[selLevel] == MNM_SL_FACE) {
		mpDialogSurface = CreateCPParamMap2 (ep_surface, mpParams, ip, hInstance,
			...
			mRollupSurface ? APPENDROLL_CLOSED : 0, TheSurfaceDlgProc(), NULL, ROLLUP_CAT_STANDARD + ROLLUP_ORDER_MATIDS);
		mpDialogFaceSmooth = CreateCPParamMap2 (ep_face_smooth, mpParams, ip, hInstance,
			...
			mRollupFaceSmooth ? APPENDROLL_CLOSED : 0, TheFaceSmoothDlgProc(), NULL, ROLLUP_CAT_STANDARD + ROLLUP_ORDER_POLYSMOOTH);
	...
	mpDialogPaintDeform = CreateCPParamMap2 (ep_paintdeform, mpParams, ip, hInstance,
		...
		mRollupPaintDeform ? APPENDROLL_CLOSED : 0, ThePaintDeformDlgProc(), NULL, ROLLUP_CAT_STANDARD + ROLLUP_ORDER_PAINTDEF);

    // Create sub object editing modes
    Tab<CommandMode*> hotkeyActionModes;

    CommandMode* baseMode = nullptr;
    selectMode = new PolyDoubleClickModeDecorator<SelectModBoxCMode, PolyModSelectionProcessor, SelectModBox>(this, ip);
    moveMode = new PolyDoubleClickModeDecorator<MoveModBoxCMode, PolyModModeSelectionProcessor, MoveModBox>(this, ip);
    baseMode = moveMode;
    hotkeyActionModes.Append(1, &baseMode);
    rotMode = new PolyDoubleClickModeDecorator<RotateModBoxCMode, PolyModModeSelectionProcessor, RotateModBox>(this, ip);
    uscaleMode = new PolyDoubleClickModeDecorator<UScaleModBoxCMode, PolyModModeSelectionProcessor, ScaleModBox>(this, ip);
    baseMode = uscaleMode;
    hotkeyActionModes.Append(1, &baseMode);
    nuscaleMode = new PolyDoubleClickModeDecorator<NUScaleModBoxCMode, PolyModModeSelectionProcessor, ScaleModBox>(this, ip);
    baseMode = nuscaleMode;
    hotkeyActionModes.Append(1, &baseMode);
    squashMode = new PolyDoubleClickModeDecorator<SquashModBoxCMode, PolyModModeSelectionProcessor, ScaleModBox>(this, ip);
    baseMode = squashMode;
    hotkeyActionModes.Append(1, &baseMode);
    ...
    {
        PolyModModeSelectionProcessor* selectionProcessor = static_cast<PolyModModeSelectionProcessor*>((hotkeyActionModes[mode])->MouseProc(&dummyVal));
        DbgAssert(selectionProcessor != nullptr);
        selectionProcessor->SetHotkeyAction(PolyModModeSelectionProcessor::POLY_HOTKEY_SHIFT, PolyModModeSelectionProcessor::POLY_ACTION_EXTRUDE, GetString(IDS_EXTRUDE));
        selectionProcessor->SetHotkeyAction(PolyModModeSelectionProcessor::POLY_HOTKEY_CTRL_SHIFT, PolyModModeSelectionProcessor::POLY_ACTION_CLONE, GetString(IDS_CLONE));
        selectionProcessor->SetHotkeyAction(PolyModModeSelectionProcessor::POLY_HOTKEY_SHIFT, PolyModModeSelectionProcessor::POLY_ACTION_CLONE, GetString(IDS_CLONE), MNM_SL_VERTEX);
        selectionProcessor->SetHotkeyAction(PolyModModeSelectionProcessor::POLY_HOTKEY_SHIFT, PolyModModeSelectionProcessor::POLY_ACTION_EXTRUDE_POSTPROCESSED, GetString(IDS_EXTRUDE), MNM_SL_FACE);
    }
    ...
    PolyModModeSelectionProcessor* rotProcessor = static_cast<PolyModModeSelectionProcessor*>(rotMode->MouseProc(&dummyVal));
    DbgAssert(rotProcessor != nullptr);
    rotProcessor->SetHotkeyAction(PolyModModeSelectionProcessor::POLY_HOTKEY_SHIFT, PolyModModeSelectionProcessor::POLY_ACTION_CLONE, GetString(IDS_CLONE));
    rotProcessor->SetHotkeyAction(PolyModModeSelectionProcessor::POLY_HOTKEY_CTRL_SHIFT, PolyModModeSelectionProcessor::POLY_ACTION_CLONE, GetString(IDS_CLONE));
...
void EditPolyMod::NotifyGripInvisible(void* param, NotifyInfo*)
{
	...
	if(em)
	{
		PolyOperation* op = em->GetPolyOperation();
		if( op && op->IsGripShown())
		{
			op->HideGrip();
		}
	}
...
void EditPolyMod::SetNumSelLabel () {
	...
	// Take this opportunity to update the caddy
	ResetGripUI();
}
...
void EditPolyMod::UpdateUIToSelectionLevel (int oldSelLevel) {
	...
	int oldmsl = meshSelLevel[oldSelLevel];
...
void EditPolyMod::ExitAllCommandModes (bool exSlice, bool exStandardModes) {
	...
	if (subobjectPickMode) ip->DeleteMode (subobjectPickMode);	
	if (pointToPointMode) ip->DeleteMode (pointToPointMode);	
...
void EditPolyMod::InvalidateDialogElement (int elem) {
	...
	if (mpDialogSubobj)
	{
		mpDialogSubobj->Invalidate (elem);
		switch (elem) {
			case epm_data_value:
				{
					invalidateSurfUI = TRUE;				
...
void EditPolyMod::ListDeltaOps()
{
	...
	if (list.Count() == 0) return;
	EditPolyData *pData = (EditPolyData*)list[0]->localData;
	...
	int index = 0;
	ScriptPrint(_T("Ops Limit %d\n"), limit);
	bool limitHit = false;
	for (pOpRec = pData->mpOpList; pOpRec != NULL; pOpRec = pOpRec->Next())
	{
		if (index == limit)
			limitHit = true;
		if (limitHit)
			ScriptPrint(_T("%d : Operation DISABLED %s\n"), index++, pOpRec->Operation()->Name());
		else
			ScriptPrint(_T("%d : Operation %s\n"), index++, pOpRec->Operation()->Name());
	}
}
