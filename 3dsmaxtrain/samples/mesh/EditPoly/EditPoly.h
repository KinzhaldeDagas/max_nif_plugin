...
#include "../../Include/PolyCommandIDs.h"
#include "iGPU_Display.h"
#include "EPoly.h"
#include <Geom/PointToPointPath.h>
#include "resource.h"
#include "MNChannelData.h"

 /* Class for temporarily setting MN_MESH_NO_BAD_VERTS flag and restoring when destroyed
 *
 * This is used for cases where MNMesh wants to clean up bowtie-type vertices in a mesh behind the scenes during
 * selection, which causes problems. The cleanup happens "behind the scenes", outside of the Undo system, and can change
 * topology, so we want to keep it from happening.  Setting this flag tells the MNMesh that there are no bad vertices to
 * clean up.  We restore it when finished so that later operations can function properly.
 *
 * Example - SelectEdgeRing calls MNMesh::FillInMesh, which in turn calls MNMesh::EliminateBadVerts, which can change
 * topology. The following suppresses this by telling the mesh it has no bad verts:
 *
 * \code
 * MNMesh mesh;
 * {
...
enum {
	...
	EP_HARD_EDGE_DISPLAY_WHEN_VISIBLE	// Display hard edges only when edges are visible
};
...
// Maxscript accessors to legacy chamfer properties
#define EPOLY_MOD21_INTERFACE Interface_ID(0xa464ee4, 0x3b99667a)
#define GetEPolyMod21Interface(cd) (EPolyMod21 *)(cd)->GetInterface(EPOLY_MOD21_INTERFACE)

// Reference ID's for EPolyMod
enum EditPolyRefID {
	EDIT_PBLOCK_REF,
	EDIT_SLICEPLANE_REF,
	EDIT_POINT_CTRL_CONTAINER_REF,
	EDIT_POINT_BASE_REF
};
...
class SubobjectPickMode;
class PointToPointMode;
...
public:
	...
	IOResult SaveBasics(ISave *isave, const Tab<ParamID>& paramList);
    ...
    // Returns a flag indicating whether this operation supports propagation of any user-specified normals on face vertices to
    // the post-operation geometry
    virtual bool CanPropagateSpecifiedNormals() const { return true; }

	// Specifies the triangulation algorithm to be used by this operation, if the operation supports non-legacy triangulation
	// algorithms. By default, the triangulation type is set to TRIANGULATION_LEGACY.
	//
	// triangulationType: triangulation algorithm to be employed; input
	inline void SetTriangulationType(MNMeshTriangulationType triangulationType)
	{
		mTriangulationType = triangulationType;
	}
	...
	virtual void GetParameters (Tab<ParamID> & paramList) { }
	...
	void GetValues(EPolyMod* epoly, TimeValue t) { Interval valid = FOREVER; GetValues(epoly, t, valid); }
	...
	void GetNode(EPolyMod* epoly, TimeValue t) { Interval valid = FOREVER; GetNode(epoly, t, valid); }
    ...
    // Should return a flag indicating whether this PolyOperation supports saving to the version returned by GetSavingVersion.
    virtual bool IsSaveVersionSupported() const { return true; }

	virtual void DeleteThis () { delete this; }

	virtual bool HasGrip() {return false;}
	virtual bool IsGripShown(){return mGripShown;}
	virtual void ShowGrip(EditPolyMod *){};
	virtual void HideGrip();
protected:

    // Enumeration representing PolyOperation version parameter
    // CAUTION: This value must remain constant for any given major release of 3ds Max.  That is, 3ds Max 2020 FCS, 2020 Update 1 and 2020 Update 2 must all use the
    // POLYOP_2020 value.  This is because various operations key off of this value for proper operation, and if, for example, a 2020 Update version saves a
    // file with a different version type to the 2020 file format, if that file is loaded by 3ds Max 2020 FCS it will find an unknown version number and likely fail.
    // Instead, if changing code for updates within a major release, store additional information in the class you are deriving from PolyOperation.
    enum PolyOperationVersionType
    {
        POLYOP_ORIGINAL = 0,
        POLYOP_2018_4   = 1,
        POLYOP_2018_5   = 2,
        POLYOP_2019_1   = 3,
        POLYOP_2020     = 4,
        POLYOP_2020_2   = 5,  // CAUTION: DO NOT write files with this version; it breaks 2020, which isn't expecting it. This is here to catch & fix files containing it.
		POLYOP_2026     = 6,
    };

	virtual bool Do (MNMesh & mesh, LocalPolyOpData *pOpData) { return false; }

	// Retriangulates the specified faces, employing the algorithm specified by mTriangulationType.
	//
	// mesh:       mesh containing the faces for which triangulations are to be computed; modified
	// faceList:   entry faceList[f] is the index, in the mesh.f array, of a face to be retriangulated; input
	// nFacesList: number of faces in list faceList; input
	inline void TriangulateFaces(MNMesh& mesh, const int* faceList, int nFacesList)
	{
	...
	inline void TriangulateFaces(MNMesh& mesh, const IntTab& faceList)
	{
		int nFacesList = faceList.Count();
		if (nFacesList != 0)
		{
			TriangulateFaces(mesh, &(faceList[0]), nFacesList);
		}
	}

	bool mGripShown;
	int mPolyOperationVersion;
    bool fixed_2020_Update2 = false;               // Flag that indicates we've repaired 2020 Update 2 version on load
	MNMeshTriangulationType mTriangulationType;    // Triangulation scheme to be used by this PolyOperation, indexed according to the MNMeshTriangulationType enumeration, if supported
};
...
class EPolyMod21
	: public EPolyMod18
{
	BEGIN_FUNCTION_MAP_PARENT(EPolyMod18)
		// Property accessors
		PROP_FNS(epmod_get_edge_chamfer_type, EpModGetEdgeChamferType, epmod_set_edge_chamfer_type, EpModSetEdgeChamferType, TYPE_ENUM);
		PROP_FNS(epmod_get_edge_chamfer_quad_intersections, EpModGetEdgeChamferQuadIntersections, epmod_set_edge_chamfer_quad_intersections, EpModSetEdgeChamferQuadIntersections, TYPE_BOOL);
	END_FUNCTION_MAP

	///  \brief 
	/// Get the current Edit Poly Edge Chamfer type (obsolete).
	/// 
	///  \return  The current Edit Poly Edge Chamfer type, or -1 if the current
	/// type is not any of the Edit Poly Edge Chamfer types. 
	virtual int EpModGetEdgeChamferType() { return -1; }

	///  \brief 
	/// Set the current Edit Poly Edge Chamfer type (obsolete).
	/// 
	///  \param[in] chamferType - Standard = 0, Quad = 1.
	virtual void EpModSetEdgeChamferType(int chamferType) { }

	///  \brief 
	/// Get the current Edit Poly Edge Chamfer quad intersections (obsolete).
	/// 
	///  \return  The current Edit Poly Edge Chamfer quadIntersections
	virtual bool EpModGetEdgeChamferQuadIntersections() { return FALSE; }

	///  \brief 
	/// Set the current Edit Poly Edge Chamfer quad intersections (obsolete).
	/// 
	///  \param[in] quadIntersections
	virtual void EpModSetEdgeChamferQuadIntersections(bool quadIntersections) { }
};

// ================================================================================
// Definition of the Editable Poly Modifier 
// ================================================================================
class EditPolyMod :   public Modifier
					, public IMeshSelect
					, public EPolyMod21
					, public EventUser
					, public AttachMatDlgUser
					, public MeshPaintHandler
					, public EditSSCB
...
private:
    ...
    // Enumeration to track versioning of this modifier
    enum EditPolyModVersionType
    {
        EDIT_POLY_ORIGINAL = 0,
        EDIT_POLY_2021     = 1
    };

    // Version value for this instance
    int m_editPolyVersion;

    // When true, calls to HitTest and HitTestHighlighted for a POINT_RGN test type are interpreted as being selection hit
    // tests, so that the Backfacing and Occluded culling options in the selection panel are honored
    bool m_doPointSelectionHitTest;

	// When true, a postprocessed extrude operation is in progress
	bool m_isPostprocessedExtrudeInProgress;
	...
	PointControllerContainer	*mpPointCtrlContainer;		// Point Controller Container - includes all nodes' points wrapped into one.
	Tab<Control*>		mPointControl;

	// These tables describe the names of the nodes associated with each block of points in the Point Controller Container.
	...
	static void					InitializeOperationList();	// Allocates Memory for all PolyOperation concrete classes.
public:
	static void					ClearOperationList();		// Clear all PolyOperations out of memory by cleaning up mOpList.
private:
	...
	static SubobjectPickMode			*subobjectPickMode;
	static PointToPointMode				*pointToPointMode;
    ...
    // Map from EPolyMod13 grip enum, exposed in the SDK, to the actual enum from EPolyManipulatorGrip, which may include
    // further options
    static const EPolyManipulatorGrip::ManipulateGrips m_gripTypeMap[EPolyMod13::EditPolyManipulatorGripCount];
	...
	// From Animatable
	void				DeleteThis() { delete this; }
	void				GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_EDIT_POLY_MOD) : _T("Edit Poly");}  
	...
	const TCHAR				*GetObjectName(bool localized) const override { return localized ? GetString(IDS_EDIT_POLY_MOD) : _T("Edit Poly"); }
	...
	ChannelMask			ChannelsUsed()  {return GEOM_CHANNEL|TOPO_CHANNEL|SELECT_CHANNEL|TEXMAP_CHANNEL|VERTCOLOR_CHANNEL; }
	ChannelMask			ChannelsChanged() {return GEOM_CHANNEL|TOPO_CHANNEL|SELECT_CHANNEL|TEXMAP_CHANNEL|SUBSEL_TYPE_CHANNEL|VERTCOLOR_CHANNEL; }
	...
	float* SoftSelection(TimeValue t, EditPolyData* pData) { Interval interval = FOREVER; return SoftSelection(t, pData, interval); }
...
public:
	...
	void UpdateTransformData(TimeValue t, EditPolyData* pData) { Interval valid = FOREVER; UpdateTransformData(t, valid, pData); }
    ...
    bool ShouldCollapseOnSave() override;
    ...
    // Selection; as the Backfacing and Occluded culling options in the selection control panel should be active only for
    // selection operations, the corresponding parameter block entries are enforced for POINT_RGN hit tests via the HitTest and
    // HitTestHighlighted functions only between a pair of calls to BeginPointSelectionHitTesting and
    // EndPointSelectionHitTesting.
    void BeginPointSelectionHitTesting();
    void EndPointSelectionHitTesting();
    ...
    // Shift-extrusion:
    void ExtrudeSelSubComponents(TimeValue timeCurr, bool doPostprocessFaceExtrude = false);
    void UpdateExtrudeSelSubComponentsPostprocessed(TimeValue timeCurr);
    void FinalizeExtrudeSelSubComponentsPostprocessed(TimeValue timeCurr);
	...
	void 	EpModLocalDataChanged (ChannelMask channels);
    ...
    int     EpModGetLast() override { return EpGetLastOperation(); }

	// Edge Chamfer legacy properties accessors
	int EpModGetEdgeChamferType() override;
	void EpModSetEdgeChamferType(int chamferType) override;
	bool EpModGetEdgeChamferQuadIntersections() override;
	void EpModSetEdgeChamferQuadIntersections(bool quadIntersections) override;
	...
	void		UpdateHardEdgeColorDisplay(HWND hWnd = NULL);
	void		UpdateHardEdgeDisplayType(HWND hWnd = NULL);
    ...
    // Get and set functions for grip state, based upon EPolyMod13 grip enumeration exposed in the SDK
	void SetManipulateGrip(bool on, EPolyMod13::ManipulateGrips val);
	bool GetManipulateGrip(EPolyMod13::ManipulateGrips val);

    // Get and set functions for grip state, based upon EPolyManipulatorGrip grip enumeration, which may contain additional
    // options
	void SetManipulateGrip(bool on, EPolyManipulatorGrip::ManipulateGrips val);
	bool GetManipulateGrip(EPolyManipulatorGrip::ManipulateGrips val);
	...
	// New methods for Max 2016 -- Support for editing Vertex and Edge data (initially, Weight and Crease values for OpenSubDiv)
	float	GetVertexDataFloatValue (int channel, int *numSel, bool *uniform, DWORD vertexFlags, INode *in_pNode = NULL );
	float	GetEdgeDataFloatValue (int channel, int *numSel, bool *uniform, DWORD edgeFlags, INode *in_pNode = NULL );
	void	ResetVertexData (int channel, INode *in_pNode = NULL );
	void	ResetEdgeData (int channel, INode *in_pNode = NULL );
	void	BeginPerDataModify (int mnSelLevel, int channel);
	bool	InPerDataModify ();
	void	DoPerDataModify (int mnSelLevel, int channel, float value);
	void	EndPerDataModify (bool success);
	/**
	* Update the Weight and Crease per-data display
	* \param mnSelLev - Selection level (MNM_SL_XXX)
	* \param channel - Desired channel, VDATA_XXX for vertices, EDATA_XXX for edges, or -1 for all
	*/
	...
	void ModeChanged(CommandMode*, CommandMode* );
	void RegisterCMChangedForGrip();
	void UnRegisterCMChangeForGrip();
	//grip callback
...
private:
	// whether a command mode callback was registered or not
	...
	void		SetupBackFaceCulling(TimeValue t, const HitRegion& hr, GraphicsWindow* gw, DWORD& hitFlags);
...
public:
    static void ResetChamferLoadVersionWarning() { s_doChamferLoadVersionWarning = true; }
    static bool GetChamferLoadVersionWarning() { return s_doChamferLoadVersionWarning; }
    static void ClearChamferLoadVersionWarning() { s_doChamferLoadVersionWarning = false; }

    static void SetExtrudeLoadVersionWarning(bool doExtrudeLoadVersionWarning) { s_doExtrudeLoadVersionWarning = doExtrudeLoadVersionWarning; }
    static bool GetExtrudeLoadVersionWarning() { return s_doExtrudeLoadVersionWarning; }

    static void ResetGenericSaveVersionWarning() { s_doGenericSaveVersionWarning = true; }
    static bool GetGenericSaveVersionWarning() { return s_doGenericSaveVersionWarning; }
    static void ClearGenericSaveVersionWarning() { s_doGenericSaveVersionWarning = false; }

    static void SetCollapsedSaveWarning(bool doCollapsedSaveWarning) { s_doCollapsedSaveWarning = doCollapsedSaveWarning; }
    static bool GetCollapsedSaveWarning() { return s_doCollapsedSaveWarning; }

private:
    static bool s_doChamferLoadVersionWarning;
    static bool s_doExtrudeLoadVersionWarning;
    static bool s_doGenericSaveVersionWarning;
    static bool s_doCollapsedSaveWarning;
};
...
friend class EditPolyMod;
	...
	// State related to double click selection handling
	DoubleClickSelector mDoubleClickSelector;

	bool mUseRampageWeldMath = false;
	bool mDoBadVertRemovalOnSelection = false;	// Pre-2019-Update1 did this
...
public:
	...
	EditPolyData (PolyObject *pObj, TimeValue t, bool doClearNormals = true);
	EditPolyData ();
	~EditPolyData();

	void			Initialize (PolyObject *pObj, TimeValue t, bool doClearNormals = true);
	LocalModData	*Clone();
	...
	BitArray GetVertSel() override { return mVertSel; }
	BitArray GetFaceSel() override { return mFaceSel; }
	BitArray GetEdgeSel() override { return mEdgeSel; }

	void 	SetVertSel (const BitArray &set, IMeshSelect *imod, TimeValue t) override;
	void 	SetFaceSel (const BitArray &set, IMeshSelect *imod, TimeValue t) override;
	void 	SetEdgeSel (const BitArray &set, IMeshSelect *imod, TimeValue t) override;

	void	SetVertHide (const BitArray & set, EditPolyMod *pmod);
	void	SetFaceHide (const BitArray & set, EditPolyMod *pmod);
	...
	void 			ApplyAllOperations (MNMesh & mesh, int limit);
	...
	void		InvalidateTempData (ChannelMask channels);
	...
	void SelectByHardEdge (EditPolyMod *pMod);
	void SelectBySmoothEdge (EditPolyMod *pMod);
	void AcquireStackSelection (EditPolyMod *pMod, int level);
	void SelectAllBorderEdges(BitArray &selection);		// Find border edges and select them in the supplied edge array; non-border edges will be unselected
	void SelectInvertBorderEdges(BitArray &selection);	// Find border edges and invert the selection
	...
	iMNMESH_GPU_Display *mOSD_GPU_Display;
};
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Edit Poly"); }
...
public:
	EditPolyPickFaceMouseProc(EPolyMod* e, IObjParam *i) : mpEPoly(e), ip(i), face(0), mpNode(NULL) { }
...
public:
	EditPolyConnectVertsMouseProc (EPolyMod *e, IObjParam *i) : mpEPoly(e), ip(i), v1(0), v2(0) { }
	
	HitRecord		*HitTestVertices (IPoint2 & m, ViewExp *vpt);
	void			DrawDiag (HWND hWnd, const IPoint2 & m, bool clear = false);
	void			SetV1 (int vv);
...
public:
	...
	int							ID() { return CID_POLY_CREATEVERT; }
...
public:
	...
	int 						Class() { return MODIFY_COMMAND; }
	int 						ID() { return CID_POLY_CREATEEDGE; }
...
protected:
	...
	void DisplayDataChanged() {m_EPoly->EpModLocalDataChanged(DISP_ATTRIB_CHANNEL);}
...
public:
	...
	int 						Class() { return MODIFY_COMMAND; }
	int 						ID() { return CID_POLY_CREATEFACE; }
...
public:
	EditPolyAttachPickMode() : mpEditPoly(NULL), ip(NULL),mSuspendUI(false) { }
...
public:
	...
	int 						Class() { return MODIFY_COMMAND; }
	int 						ID() { return CID_POLY_DIVIDEEDGE; }
	MouseCallBack				*MouseProc(int *numPoints) {*numPoints=1; return &proc;}
	ChangeForegroundCallback	*ChangeFGProc() {return &fgProc;}
	BOOL						ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void 						EnterMode();
...
public:
	...
	int 						Class() { return MODIFY_COMMAND; }
	int 						ID() { return CID_POLY_DIVIDEFACE; }
	MouseCallBack				*MouseProc(int *numPoints) {*numPoints=1; return &proc;}
	ChangeForegroundCallback	*ChangeFGProc() {return &fgProc;}
	BOOL 						ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void 						EnterMode();
...
public:
	EditPolyBridgeMouseProc (EPolyMod *e, IObjParam *i, int hitLevel)
		: mpEPoly(e), ip(i), hit1(0), hit2(0), mHitLevel(hitLevel) { }
...
public:
	...
	int 						Class() { return MODIFY_COMMAND; }
	int 						ID() { return CID_POLY_BRIDGE_BORDER; }
...
public:
	...
	int 						Class() { return MODIFY_COMMAND; }
	int 						ID() { return CID_POLY_BRIDGE_EDGE; }
	MouseCallBack				*MouseProc(int *numPoints) {*numPoints=3; return &proc;}
	ChangeForegroundCallback	*ChangeFGProc() {return &fgProc;}
	BOOL 						ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void 						EnterMode();
...
public:
	...
	int 						Class() { return MODIFY_COMMAND; }
	int 						ID() { return CID_POLY_BRIDGE_POLYGON; }
	MouseCallBack				*MouseProc(int *numPoints) {*numPoints=2; return &proc;}
	ChangeForegroundCallback	*ChangeFGProc() {return &fgProc;}
	BOOL 						ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void 						EnterMode();
...
public:
	...
	int 						Class() { return MODIFY_COMMAND; }
	int 						ID() { return CID_POLY_EXTRUDE; }
...
private:
	...
	// Parameter tensionOrDepth represents a tension value, for legacy reasons, for edge chamfers, and a depth value for vertex
	// chamfers
	void Drag (TimeValue t, float amount, int segments, float tensionOrDepth);

public:
	EditPolyChamferMouseProc (EPolyMod* e, IObjParam *i) : mpEditPoly(e), ip(i), mInChamfer(false), mChamferOperation(0), mStartAmount(0.0f) { }
	int proc (HWND hwnd, int msg, int point, int flags, IPoint2 m);
};
...
public:
	...
	int 						Class() { return MODIFY_COMMAND; }
	int 						ID() { return CID_POLY_POLYCHAMFER; }
...
public:
	EditPolyBevelMouseProc(EPolyMod* e, IObjParam *i) : mpEditPoly(e), ip(i),
		m0set(false), m1set(false), height(0.0F), mStartOutline(0.0f), mStartHeight(0.0f), mInBevel(false) { }
	int proc(HWND hwnd, int msg, int point, int flags, IPoint2 m);
};
...
public:
	EditPolyOutlineMouseProc(EPolyMod* e, IObjParam *i) : mpEditPoly(e), ip(i), mInOutline(false), mStartAmount(0.0F) { }
	int proc(HWND hwnd, int msg, int point, int flags, IPoint2 m);
};
...
public:
	...
	int							Class() { return MODIFY_COMMAND; }
	int							ID() { return CID_POLY_OUTLINE_FACE; }
	MouseCallBack				*MouseProc(int *numPoints) { *numPoints=2; return &mouseProc; }
	ChangeForegroundCallback	*ChangeFGProc() { return &fgProc; }
	BOOL 						ChangeFG( CommandMode *oldMode ) { return oldMode->ChangeFGProc() != &fgProc; }
	void 						EnterMode();
...
public:
	EditPolyExtrudeVEMouseProc(EPolyMod* e, IObjParam *i) : mpEditPoly(e), ip(i), mInExtrude(false), mStartHeight(0.0f), mStartWidth(0.0f) { }
	int proc(HWND hwnd, int msg, int point, int flags, IPoint2 m );
};
...
public:
	...
	int 						Class() { return MODIFY_COMMAND; }
	int 						ID() { return CID_POLY_EXTRUDE_VERTEX_OR_EDGE; }
	MouseCallBack				*MouseProc(int *numPoints) { *numPoints=2; return &mouseProc; }
	ChangeForegroundCallback	*ChangeFGProc() {return &fgProc;}
	BOOL						ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void 						EnterMode();
...
private:
	...
	// Storage for last dotted-line draw info
	HWND		mLastHWND = NULL;
	...
	void		DrawCutter (HWND hwnd, bool clear = false);
...
public:
	EditPolyCutProc (EPolyMod* e, IObjParam *i) : mpEPoly(e), ip(i), mStartLevel(0), mStartIndex(-1), mLastHitLevel(0), mLastHitIndex(0), mSuspendPreview(false) { }
	int proc(HWND hwnd, int msg, int point, int flags, IPoint2 m );
};
...
public:
	EditPolyQuickSliceProc(EPolyMod *e, IObjParam *i) : mpEPoly(e), mpInterface(i), mSlicing(false), mSuspendPreview(false) { }
	void	DrawSlicer (HWND hWnd, bool clear = false);
...
public:
	...
	int							Class() { return MODIFY_COMMAND; }
	int							ID() { return CID_POLY_QUICKSLICE; }
	MouseCallBack				*MouseProc(int *numPoints) {*numPoints=3; return &proc;}
	ChangeForegroundCallback	*ChangeFGProc() {return &fgProc;}
	BOOL						ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void						EnterMode();
...
public:
	...
	void		DrawWeldLine (HWND hWnd, IPoint2 &m, bool clear = false);
...
public:
	EditPolyHingeFromEdgeProc (EPolyMod *e, IObjParam *i) : mpEPoly(e), ip(i), mpHitRec(NULL), mEdgeFound(false), mStartAngle(0.0f) { }
...
public:
	...
	int							Class() { return MODIFY_COMMAND; }
	int							ID() { return CID_POLY_HINGE_FROM_EDGE; }
	MouseCallBack				*MouseProc(int *numPoints) {*numPoints=2; return &proc;}
	ChangeForegroundCallback	*ChangeFGProc() {return &fgProc;}
	BOOL						ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void						EnterMode();
...
public:
	...
	int							Class() { return MODIFY_COMMAND; }
	int							ID() { return CID_POLY_PICK_HINGE; }
	MouseCallBack				*MouseProc(int *numPoints) {*numPoints=1; return &proc;}
	ChangeForegroundCallback	*ChangeFGProc() {return &fgProc;}
	BOOL						ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void 						EnterMode();
...
public:
	EditPolyPickBridgeTargetProc (EPolyMod* e, IObjParam *i, int end)
		: mpEPoly(e), ip(i), mpBridgeNode(NULL), mWhichEnd(end) { }
...
public:
	...
	int 						Class() { return MODIFY_COMMAND; }
	int 						ID() { return CID_POLY_PICK_BRIDGE_1; }
	MouseCallBack				*MouseProc(int *numPoints) {*numPoints=1; return &proc;}
	ChangeForegroundCallback	*ChangeFGProc() {return &fgProc;}
	BOOL 						ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void 						EnterMode();
...
public:
	...
	int 						Class() { return MODIFY_COMMAND; }
	int 						ID() { return CID_POLY_PICK_BRIDGE_2; }
	MouseCallBack				*MouseProc(int *numPoints) {*numPoints=1; return &proc;}
	ChangeForegroundCallback	*ChangeFGProc() {return &fgProc;}
	BOOL 						ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void 						EnterMode();
...
public:
	...
	int 						Class() { return MODIFY_COMMAND; }
	int 						ID() { return CID_POLY_EDITTRI; }
	MouseCallBack				*MouseProc(int *numPoints) {*numPoints=1; return &proc;}
	ChangeForegroundCallback	*ChangeFGProc() {return &fgProc;}
	BOOL 						ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void 						EnterMode();
...
public:
	...
	int							Class() { return MODIFY_COMMAND; }
	int							ID() { return CID_POLY_TURNEDGE; }
	MouseCallBack				*MouseProc(int *numPoints) {*numPoints=1; return &proc;}
	ChangeForegroundCallback	*ChangeFGProc() {return &fgProc;}
	BOOL						ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void 						EnterMode();
...
#define			SUBOBJECT_PICK_COLORID			0x411408aa

class SubobjectPickProc : public MouseCallBack {
	friend class SubobjectPickMode;
private:
	EditPolyMod *mpEditPoly;
	IObjParam *ip;

	
	//sublevel 0 none, 1 vertex, 2 edge, 3 face/poly
	//subindex the sub object hit index
	//return node that was hit
	INode* HitTest(IPoint2 &m, ViewExp& vpt,int& subLevel, int& subIndex );

public:
	SubobjectPickProc (EditPolyMod* e, IObjParam *i) : mpEditPoly(e), ip(i), hitSubLevel(-1), hitSubIndex(-1) , hitNode(nullptr)
	{ 
	}
	int proc(HWND hwnd, int msg, int point, int flags, IPoint2 m );
...
class SubobjectPickMode : public CommandMode {
private:
	ChangeFGObject fgProc;	
	SubobjectPickProc proc;
	EditPolyMod* mpEditPoly;

public:
	SubobjectPickMode(EditPolyMod* e, IObjParam *i) : fgProc(e), proc(e,i), mpEditPoly(e), inMode(false) , mInOverride(false)
	{ 
	}
	int Class() { return MODIFY_COMMAND; }
	int ID() { return CID_POLY_SUBOBJECT_PICK; }
	MouseCallBack *MouseProc(int *numPoints) {*numPoints=2; return &proc;}
	ChangeForegroundCallback *ChangeFGProc() {return &fgProc;}
	BOOL ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void EnterMode();
	void ExitMode();
	void Display (GraphicsWindow *gw, MNMesh * mm, INode* inode);
	bool inMode;
	bool mInOverride;

};

class PointToPointProc : public MouseCallBack {
friend class SubobjectPickMode;
private:
	...
	//return 0 none, 1 vertex, 2 edge, 3 face/poly
	int HitTest(IPoint2 &m, ViewExp& vpt, bool setHitNode);
	
	void RebuildList(int startEdgeIndex);

	

public:
	PointToPointProc (EditPolyMod* e, IObjParam *i) : mpEditPoly(e), ip(i), mFirstHit(-1), mSecondHit(-1) , mPointToPointPath(nullptr), mInLoop(false)
	{ 
		mInLoop = false;
		mFirstHit = -1;
		mSecondHit = -1;
	...
	int proc(HWND hwnd, int msg, int point, int flags, IPoint2 m );
	MaxSDK::PointToPointPath*  mPointToPointPath;
	int mFirstHit;
	int mSecondHit;
	BitArray originalSelection;
	BitArray selection;
	//since we can only select across one node we need to find which one if the modifier is instanced
	//this tracks that node
	INode* mActiveNode;

	bool AdjacentLoop(int mode, int index);
	bool mInLoop;
	void StoreOffOriginalSelection();
	void ShortLoop(int mode, int start, int end, MNMesh *mm, BitArray& loop);

	void RestoreSelection();

	//sets the first hit sub object index 
	void SetFirstHit(int index, INode* lastHitNode);

};



class PointToPointMode : public CommandMode {
private:
	ChangeFGObject fgProc;	
...
public:
	PointToPointProc proc;
	PointToPointMode(EditPolyMod* e, IObjParam *i) : fgProc(e), proc(e,i), mpEditPoly(e), mInMode(false), mShiftLaunched(false)
	{ 
	}
	int Class() { return ALWAYS_PROCESS_RIGHTCLICK_COMMAND; }
	int ID() { return CID_POLY_POINT_TO_POINT; }
	MouseCallBack *MouseProc(int *numPoints) {*numPoints=1; return &proc;}
	ChangeForegroundCallback *ChangeFGProc() {return &fgProc;}
	BOOL ChangeFG(CommandMode *oldMode) {return oldMode->ChangeFGProc()!= &fgProc;}
	void EnterMode();
	void ExitMode();
	void Display (ViewExp *vpt, GraphicsWindow *gw);
	bool mInMode;
	//this mode can be launched by holding the shift key while in select
	//this mode has some special processing like
	// releasing the shift key ends the mode
	// clicking on and an adjacent face does a loop
	// short loop takes precedent over closest path
	bool mShiftLaunched;
...
public:
	...
	TSTR	Description() { return _T("EditPolySelectRestore"); }
};
...
public:
	...
	void	EndHold() { mpData->ClearFlag (kEPDataHeld); }
	TSTR	Description() { return _T("EditPolyHideRestore"); }
};
...
public:
	...
	TSTR Description() {return _T("RingLoopRestore");}
};
...
public:
	...
	void Restore(int isUndo)
	{
		PolyOperation *op = mpMod->GetPolyOperation();
		if(op)
		{
			if(op->HasGrip())
				op->HideGrip();
	...
	void Redo(){}
	int Size(){ return 8;}
	TSTR Description() { return _T("GripVisible");}
...
public:
	...
	TSTR Description() { return _T("AddOperation"); }
};
...
public:
	...
	void Restore (int isUndo);
	void Redo ();
	TSTR Description () { return _T("Edit Poly flag change restore"); }
...
public:
	LocalCutData (): mEndVertex(0), mVersion(2) { }
...
public:
	...
	void	ClearVertices ()
	{
		// This is a hack to set zero verts without deallocating them for performance.
		MaxSDK::MNGeomChannelData* gc = mVertexMesh.GetChannelDataContainer().GetOrCreateMNGeomChannelData();
		gc->numv = 0;
		gc->SyncParentInterface(&mVertexMesh);
	}
...
private:
	...
	// Maximum number of moved vertices for a which retriangulation is to be performed upon completion of the present
	// operation, as discussed in the description of the SetRetriangulationVertexLimit function.
	int mNVerticesRetriangulatedMax;

public:
	LocalTransformData(): mNVerticesRetriangulatedMax(0) { }
	LocalTransformData(const LocalTransformData *pFrom): mOffset(pFrom->mOffset), mNodeName(pFrom->mNodeName), mNVerticesRetriangulatedMax(pFrom->mNVerticesRetriangulatedMax) { }
	...
	void	SetNodeName (const TSTR & name) { mNodeName = name; }
	...
	// Sets the maximum number of moved vertices for a which retriangulation is performed upon completion of the
	// present operation. In particular, if the total number of vertices for which the final displacement imposed by
	// the present transformation is not greater than nVerticesRetriangulatedMax, then triangulations are recomputed
	// for any face, about a moved vertex, whose triangulation is judged to have potentially been made invalid by the
	// transformation. A face triangulation is judged to have potentially been made invalid if any triangle at any
	// moved vertex is found to be collapsed, or if the outward normal of any such triangle has a negative component
	// along the face average normal direction; note that under this latter criterion, certain grossly non-planar faces
	// may be unnecessarily retriangulated, however, a general condition accounting for such faces, which may be
	// checked in a reasonably computationally inexpensive manner, is not readily available. Note that if this function
	// is not called, retriangulation is not performed, consistent with the behavior for
	// nVerticesRetriangulatedMax = 0.
	//
	// nVerticesRetriangulatedMax: maximum number of moved vertices for a which retriangulation is to be performed upon
	//                             completion of the present operation, as discussed in the function description, on
	//                             the range nVerticesRetriangulatedMax >= 0;
	inline void SetRetriangulationVertexLimit(int nVerticesRetriangulatedMax = 10)
	{
		DbgAssert(nVerticesRetriangulatedMax >= 0);
		mNVerticesRetriangulatedMax = nVerticesRetriangulatedMax;
	}	

	// Returns the maximum number of moved vertices for a which retriangulation is to be performed upon completion of
	// the present operation, as discussed in the description of the SetRetriangulationVertexLimit function.
	inline int GetRetriangulationVertexLimit()
	{
		return mNVerticesRetriangulatedMax;
	}
