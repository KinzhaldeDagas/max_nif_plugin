...
#define EDITPOLY_QUAD_CHAMFER_VERSION (QCHAM_VERSION_OLYMPUS)   // This is the version of the Quad Chamfer algorithm we support -- Update as necessary here only
static constexpr USHORT kQuadChamferVersion = 0x0602;           // File chunk ID for quad chamfer version

 // Externals
extern Matrix3 MakeZXMatrix(const Point3& zVector, const Point3& xVector);

void MaybeNotifyUserOfSaveIncompatibility()
{
    if (EditPolyMod::GetGenericSaveVersionWarning())
    {
        TSTR title = GetString(IDS_EDIT_POLY_MOD);
        TSTR message = GetString(IDS_SAVE_INCOMPATIBILITY_WARNING);
        GetCOREInterface()->Log()->LogEntry(SYSLOG_WARN, DISPLAY_DIALOG, title, message);
        // No more notifications on this save
        EditPolyMod::ClearGenericSaveVersionWarning();
    }
}
...
PolyOperation::PolyOperation()
	...
	, mNewEdgeCt(0)
	, mGripShown(false)
	, mPolyOperationVersion(POLYOP_2026)
	, mTriangulationType(TRIANGULATION_LEGACY)
{
	// constructor
}
...
void PolyOperation::CopyBasics(PolyOperation *pCopyTo)
{
	...
	pCopyTo->mTriangulationType = mTriangulationType;
...
bool PolyOperation::Apply(MNMesh & mesh, LocalPolyOpData *pOpData) {
	...
	int onumf = mesh.numf;
	int onume = mesh.nume;
	mNewMapVertCt.SetCount(mesh.MNum() + NUM_HIDDENMAPS);
	for (int mp = -NUM_HIDDENMAPS; mp < mesh.MNum(); mp++) {
	...
	if ((mesh.MNum() + NUM_HIDDENMAPS) > mNewMapVertCt.Count())
	{
...
static constexpr USHORT kSelection = 0x400;
static constexpr USHORT kParameter = 0x410;
static constexpr USHORT kTransform = 0x420;
static constexpr USHORT kModContextTM = 0x430;
static constexpr USHORT kPreserveMapSettings = 0x440;
static constexpr USHORT kVersion = 0x450;
static constexpr USHORT kSoftSelection = 0x460;
static constexpr USHORT kTriangulationType = 0x470;

IOResult PolyOperation::SaveBasics(ISave *isave)
{
    Tab<ParamID> paramList;

    // Fetch parameter list and pass to generic function
    GetParameters(paramList);
    return SaveBasics(isave, paramList);
}

IOResult PolyOperation::SaveBasics(ISave *isave, const Tab<ParamID>& paramList)
{
    ...
    // Important: If saving to a previous version of Max, we need to write an operation version
    // that the target version of Max is guaranteed to know about, lest we crash...
    int writeVersion = mPolyOperationVersion;   // Default to whatever we are now

    // GetSavingVersion() tells us what version of Max we're saving to, or zero if saving to the current version.
    switch (GetSavingVersion())
    {
    case MAX_RELEASE_R22:
        if (writeVersion > POLYOP_2020)
        {
            writeVersion = POLYOP_2020;
        }
        break;
    }
    
	isave->BeginChunk(kVersion);
    isave->Write(&writeVersion, sizeof(int), &nb);
	isave->EndChunk();
	...
	// Save the triangulation type parameter
	isave->BeginChunk(kTriangulationType);
	int triangulationType = mTriangulationType;
	isave->Write(&triangulationType, sizeof(int), &nb);
	isave->EndChunk();
...
IOResult PolyOperation::LoadBasics(ILoad *iload) {
	...
	while (IO_OK == (res = iload->OpenChunk())) {
		...
		case kVersion:
            ...
            // Fix for improperly upgraded 2020 Update 2 files
            if (mPolyOperationVersion == POLYOP_2020_2)
            {
                // Set the version to 2020 FCS which is safe for all 2020 versions, but set the flag that indicates
                // that we fixed it so other PolyOperations that might care will know
                mPolyOperationVersion = POLYOP_2020;
                fixed_2020_Update2 = true;
                // Let's also inform the user that this file should be saved -- this will save it in its fixed form
                iload->SetObsolete();
            }
			if (res != IO_OK) break;
			break;
		...
		case kSoftSelection:
			...
			break;
		case kTriangulationType:
			{
				int triangulationType = TRIANGULATION_LEGACY;
				res = iload->Read(&triangulationType, sizeof(int), &nb);
				DbgAssert((triangulationType >= TRIANGULATION_LEGACY) && (triangulationType <= TRIANGULATION_ENHANCED_2024));
				mTriangulationType = MNMeshTriangulationType(triangulationType);
			}
			break;
		}
		iload->CloseChunk();
...
bool PolyOpCreate::Do(MNMesh & mesh, LocalPolyOpData *pOpData)
{
	...
	if (pCreateData->NumFaces() > 0)
	{
		...
		for (int i = 0; i < pCreateData->NumFaces(); i++)
		{
			...
			if (j < deg) continue;
			int nf = mesh.CreateFace(deg, vtx, mTriangulationType);
			if (nf < 0) {	// Try flipping the face over.
				int *w = new int[deg];
				for (int k = 0; k < deg; k++) w[k] = vtx[deg - 1 - k];
				nf = mesh.CreateFace(deg, w, mTriangulationType);
				delete[] w;
			}
			if (nf > -1) {
...
bool PolyOpSlice::Do(MNMesh & mesh, LocalPolyOpData *pOpData)
{
	...
	return mesh.Slice(normal, offset, MNEPS, mSplit != 0, false, false, MN_SEL, mTriangulationType);
}
...
bool PolyOpSliceFaces::Do(MNMesh & mesh, LocalPolyOpData *pOpData)
{
	Point3 normal = Normalize(mMCTM.VectorTransform(mPlane.VectorTransform(Point3(0, 0, 1))));
	float offset = DotProd(normal, mMCTM * mPlane.GetTrans());
	return mesh.Slice(normal, offset, MNEPS, mSplit != 0, false, true, MN_EDITPOLY_OP_SELECT, mTriangulationType);
}
...
bool PolyOpCut::Do(MNMesh & mesh, LocalPolyOpData *pOpData)
{
	...
	for (int i = 0; i < pData->NumCuts(); i++)
	{
		...
		case MNM_SL_VERTEX:
			...
			if (ver == 1)
				endVertex = mesh.Cut_Versioned(startIndex, end, normal, mSplit ? true : false, 1, mTriangulationType);
			else
				endVertex = mesh.Cut(startIndex, end, normal, mSplit ? true : false, mTriangulationType);
			break;
		...
		case MNM_SL_EDGE:
			...
			if (ver == 1)
				endVertex = mesh.Cut_Versioned(nv, end, normal, mSplit ? true : false, 1, mTriangulationType);
			else
				endVertex = mesh.Cut(nv, end, normal, mSplit ? true : false, mTriangulationType);
			break;

		case MNM_SL_FACE:
			if (startIndex >= mesh.numf) continue;

			endVertex = mesh.CutFace(startIndex, start, end, normal, mSplit ? true : false, mTriangulationType);
			break;
		}
	}
...
bool PolyOpMeshSmooth::Do(MNMesh & mesh, LocalPolyOpData *pOpData)
{
	...
	mesh.SplitFacesUsingBothSidesOfEdge(0, mTriangulationType);
...
bool PolyOpMakePlanarIn::Do(MNMesh & mesh, LocalPolyOpData *pOpData) {
	...
	float ss, numSel = 0;
	int numss = mSoftSelection.Count();
...
bool PolyOpRemoveVertex::Do(MNMesh& mesh, LocalPolyOpData* pOpData)
{
	...
	return mesh.RemoveVertices(to_delete.data(), int(to_delete.size()));
}
...
public:
	...
	bool Do(MNMesh & mesh, LocalPolyOpData *pOpData) { return mesh.ConnectVertices(MN_EDITPOLY_OP_SELECT, mTriangulationType); }
	PolyOperation *Clone() {
...
public:
	...
	bool Do(MNMesh & mesh, LocalPolyOpData *pOpData);
    virtual void RescaleWorldUnits(float f) override
    {
        mThreshold *= f;
    }
    PolyOperation *Clone() {
...
bool PolyOpExtrudeVertex::Do(MNMesh & mesh, LocalPolyOpData *pOpData)
{
	...
	// Topology change:
	if (!mesh.ExtrudeVertices(MN_EDITPOLY_OP_SELECT, &chamData, tUpDir, mTriangulationType)) return false;

	// Apply map changes based on base width:
	int i;
	Tab<UVVert> tMapDelta;
	for (int mapChannel = -NUM_HIDDENMAPS; mapChannel < mesh.MNum(); mapChannel++) {
...
{
private:
    int mVersion{EDITPOLY_QUAD_CHAMFER_VERSION};
    int mNumSegments{0};
    int mSmoothingType{EP_EDGE_CHAMFER_SMOOTH_ALL};
    float mAmount{0.0f};
    float mDepth{0.0f};
    float mSmoothingThreshold{0.0f};
    BOOL mOpen{FALSE};
    BOOL mDoSmoothing{FALSE};
    ChamferVerticesGrip* mChamferVerticesGrip{nullptr};
...
public:
    ...
    void GetParameters(Tab<ParamID> & paramList)
    {
        ...
        paramList[2] = epm_vertex_chamfer_num_segments;
        paramList[3] = epm_vertex_chamfer_depth;
        paramList[4] = epm_vertex_chamfer_smooth;
        paramList[5] = epm_vertex_chamfer_smooth_type;
        paramList[6] = epm_vertex_chamfer_smooth_threshold;
    }
    ...
    void* Parameter(int paramID)
    {
        ...
        switch (paramID)
        {
        case epm_chamfer_vertex:
            param = &mAmount;
            break;
        case epm_chamfer_vertex_open:
            param = &mOpen;
            break;
        case epm_vertex_chamfer_num_segments:
            param = &mNumSegments;
            break;
        case epm_vertex_chamfer_depth:
            param = &mDepth;
            break;
        case epm_vertex_chamfer_smooth:
            param = &mDoSmoothing;
            break;
        case epm_vertex_chamfer_smooth_type:
            param = &mSmoothingType;
            break;
        case epm_vertex_chamfer_smooth_threshold:
            param = &mSmoothingThreshold;
            break;
        default:
            break;
        }
...
bool PolyOpChamferVertex::Do(MNMesh& mesh, LocalPolyOpData* pOpData)
{
	...
	// In current versions of the chamfer operation, merge across any hidden edges and elminate collinear vertices on the input
	// mesh prior to processing, consistent with the approach taken by the Chamfer modifier
	if (mVersion >= QCHAM_VERSION_VESTA_UPDATE2)
	{
		mesh.MakePolyMesh();
	}

    // Proceed according to number of segments requested
    bool isAltered = false;
    DWORD vertexFlag = MN_EDITPOLY_OP_SELECT;
    if (mNumSegments == 0)
    {
        MNChamferData chamData;

        // Zero segments; use legacy vertex chamfer
        IMNMeshUtilities8* l_meshToChamfer = mesh.GetTypedInterface<IMNMeshUtilities8>();
        if (l_meshToChamfer != NULL)
        {
            bool open = mOpen ? true : false;
            isAltered = l_meshToChamfer->ChamferVertices(vertexFlag, &chamData, open);
        }
    ...
    else
    {
        ...
        // Non-zero number of segments; use new segmented quad vertex chamfer implementation
        ...
        MNQuadVertexChamferParameters params(vertexFlag, optionFlags, geometryOption, mNumSegments, mAmount, mDepth, 1.0f, angleFactorVertex);
        ...
        {
            isAltered = chamferInterface->QuadVertexChamfer(params);
        }

        // Adjust smoothing groups as requested
        if (mDoSmoothing && isAltered)
        {
            float angleTol = DegToRad(mSmoothingThreshold);
            switch (mSmoothingType)
            {
		    case EP_EDGE_CHAMFER_SMOOTH_ALL:

                // Perform smoothing over the entire post-chamfer geometry
                mesh.AutoSmooth(angleTol, false, false);
                break;
            case EP_EDGE_CHAMFER_SMOOTH_CHAMFER:
		        {

                    // Smooth only the newly-created chamfered surface faces
                    MNMeshUtilities meshUtils(&mesh);
                    meshUtils.AutoSmoothIsland(angleTol, MN_FACE_CREATED);
                }
                break;
            default:
                DbgAssert(false);
...
IOResult PolyOpChamferVertex::Save(ISave* isave)
{
    constexpr int nParamsOld = 2;
    ULONG nWritten;
    Tab<ParamID> paramList;

    // Fetch parameters list, truncating to as necessary to contain only parameters supported by the selected save version
    GetParameters(paramList);
    bool isPreTheseus = false;
    int saveVersion = GetSavingVersion();
    if ((saveVersion != 0) && (saveVersion < MAX_RELEASE_R23))
    {
        isPreTheseus = true;
        paramList.SetCount(nParamsOld, FALSE);

        // Pre-2021 versions do not support the new quad vertex chamfer through Edit Poly; issue a warning
        if (mNumSegments > 0)
        {
            MaybeNotifyUserOfSaveIncompatibility();
        }
    }

    // Save basic PolyOoperation data, including all relevant chamfer parameters other than version
	isave->BeginChunk(kPolyOperationBasics);
	IOResult ioResult = SaveBasics(isave, paramList);
	isave->EndChunk();

    // Save version
    if ((!isPreTheseus) && (ioResult == IO_OK))
    {
        isave->BeginChunk(kQuadChamferVersion);
        ioResult = isave->Write(&mVersion, sizeof(int), &nWritten);
        isave->EndChunk();
    }
...
IOResult PolyOpChamferVertex::Load(ILoad* iload)
{
    ULONG nWritten;

    // Ensure that members absent from pre-2021 files are properly set before loading
    mVersion = QCHAM_VERSION_ELWOOD;
    mNumSegments = 0;

    // Read data from file
    IOResult ioResult = iload->OpenChunk();
    while (ioResult == IO_OK)
    {
        USHORT chunkID = iload->CurChunkID();
        switch (chunkID)
        {
        case kPolyOperationBasics:
            ioResult = LoadBasics(iload);
            break;
        case kQuadChamferVersion:
            ioResult = iload->Read(&mVersion, sizeof(int), &nWritten);
            break;
        default:
            break;
        }
        ...
        if (ioResult == IO_OK)
        {
            ioResult = iload->OpenChunk();
        }
    }

    // If the chamfer version in the file is newer than our current version, some setting may not be supported, or the chamfer
    // surface calculation may be different than in the version that saved the file; issue a warning, if one has not already
    // been issued
    if (mVersion > EDITPOLY_QUAD_CHAMFER_VERSION)
    {
        bool doWarning = EditPolyMod::GetChamferLoadVersionWarning();
        if (doWarning)
        {
            TSTR warningTitle = GetString(IDS_EDIT_POLY_MOD);
            TSTR warningMsg = GetString(IDS_CHAMFER_LOADING_FUTURE_VERSION);
            Interface* coreInterface = GetCOREInterface();
            DbgAssert(coreInterface != NULL);
            BOOL isQuite = coreInterface->GetQuietMode();
            BOOL doDisplayDialog = DISPLAY_DIALOG;
            if (isQuite)
            {
                doDisplayDialog = NO_DIALOG;
            }

            (coreInterface->Log())->LogEntry(SYSLOG_WARN, doDisplayDialog, warningTitle, warningMsg, mVersion, EDITPOLY_QUAD_CHAMFER_VERSION);
        }

        EditPolyMod::ClearChamferLoadVersionWarning();
    }

    // Unles we had an error condition, set return flag to indicate success
    if (ioResult != IO_ERROR)
    {
        ioResult = IO_OK;
    }
...
bool PolyOpCreateEdge::Do(MNMesh & mesh, LocalPolyOpData *pOpData)
{
	...
	for (int edge = 0; edge < pCreateEdge->NumEdges(); edge++)
	{
		...
		int nf, ne;
		mesh.SeparateFace(ff, v1pos, v2pos, nf, ne, true, true, false, mTriangulationType);
		ret = true;
	}
...
private:
	bool mCtrlKey;
	unsigned mLoadedVersion;

	bool DoLegacy(MNMesh& mesh, LocalPolyOpData* pOpData);

public:

	PolyOpRemoveEdge()
			: mCtrlKey(false)
			, mLoadedVersion(MAX_RELEASE)
	{
	}
	// Implement PolyOperation methods:
	...
	PolyOperation* Clone()
	{
		...
		ret->mLoadedVersion = mLoadedVersion;
		return ret;
	}
	void DeleteThis() { delete this; }
	...
	bool IsSaveVersionSupported() const override;
};

constexpr USHORT kControlKey = 0x1601;
constexpr USHORT kSaveVersion = 0x1602;
...
IOResult PolyOpRemoveEdge::Save(ISave *isave) {
	...
	isave->BeginChunk(kSaveVersion);
	isave->Write(&mLoadedVersion, sizeof(mLoadedVersion), &nb);
	isave->EndChunk();
	return IO_OK;
}
...
IOResult PolyOpRemoveEdge::Load(ILoad* iload)
{
	...
	while (IO_OK == (res = iload->OpenChunk()))
	{
		...
		case kSaveVersion:
		{
			res = iload->Read(&mLoadedVersion, sizeof(mLoadedVersion), &nb);
		}
		break;
		}
	...
	if (mLoadedVersion == 0)
	{
		// Enables legacy behavior if required.
		mLoadedVersion = iload->FileSaveAsVersion();
	}

	return IO_OK;
}

bool PolyOpRemoveEdge::DoLegacy(MNMesh& mesh, LocalPolyOpData* pOpData)
{
	bool ret = false;
	for (int i = 0; i < mesh.nume; i++)
	{
		...
		if (mCtrlKey)
		{
			...
			mesh.RemoveVertices(verts_to_delete.data(), int(verts_to_delete.size()));
		}
	}
...
{
	if (mLoadedVersion < MAX_RELEASE_R24_PREVIEW)
	{
		return DoLegacy(mesh, pOpData);
	}
	...
	if (mCtrlKey)
	{
		// set the edges's vertices as user
		mesh.ClearVFlags(MN_USER);
		mesh.PropegateComponentFlags(
				MNM_SL_VERTEX, MN_USER, MNM_SL_EDGE, MN_EDITPOLY_OP_SELECT);
	}
...
bool PolyOpRemoveEdge::IsSaveVersionSupported() const
{
	// Don't collapse when using legacy.
	if (mLoadedVersion < MAX_RELEASE_R24_PREVIEW)
	{
		return true;
	}

	// This PolyOperation was changed in Max 2022.
	// It does not support any save operation to a versions prior to its
	// redesign

	int saveVersion = GetSavingVersion();
	if (saveVersion != 0 && saveVersion < MAX_RELEASE_R24)
	{
		return false;
	}

	// Return a flag indicating whether the present save version is supported
	return true;
}
...
bool PolyOpConnectEdge::Do(MNMesh & mesh, LocalPolyOpData *pOpData) {
	...
	{
		l_ret = l_meshToBridge->ConnectEdges(MN_EDITPOLY_OP_SELECT, mSegments, mPinch, mSlide, mTriangulationType);
	}
...
bool PolyOpExtrudeEdge::Do(MNMesh & mesh, LocalPolyOpData *pOpData) {
	...
	// Topology change:
	if (!mesh.ExtrudeEdges(MN_EDITPOLY_OP_SELECT, &chamData, tUpDir, mTriangulationType)) return false;
...
public:
	PolyOpChamferEdge()
		...
		, mTension(1.0f)
		, mInvert(false)
		, mType(EP_STANDARD_EDGE_CHAMFER)
		, mSmooth(false)
		, mSmoothType(EP_EDGE_CHAMFER_SMOOTH_ALL)
		, mSmoothThreshold(30.0f)
		, mQuadIntersections(true)
		, mMiteringType(EP_EDGE_CHAMFER_UNIFORM_MITER)
		, mEndBias(0.5f)
		, mRadialBias(0.0f)
		, mDepth(0.5f)
		, mChamferEdgeGrip(NULL)
		, mQCVersion(EDITPOLY_QUAD_CHAMFER_VERSION)
	{ }
	~PolyOpChamferEdge() { }
	...
	TCHAR* Name()
	{
		return GetString(IDS_EDITPOLY_CHAMFER_EDGE);
	}
	int DialogID()
	{
		return IDD_SETTINGS_CHAMFER_EDGE;
	}

	void GetParameters(Tab<ParamID>& paramList)
	{
		paramList.SetCount(14);
		...
		paramList[5] = epm_edge_chamfer_type;		// Legacy Chamfer type (standard or quad)
		...
		paramList[10] = epm_edge_chamfer_mitering_type; // Mitering type
		paramList[11] = epm_edge_chamfer_end_bias; // End bias
		paramList[12] = epm_edge_chamfer_radial_bias; // Radial Bias
		paramList[13] = epm_edge_chamfer_depth; // Depth
	}
	void* Parameter(int paramID)
	{
		switch (paramID)
		{
		case epm_chamfer_edge:
			return &mAmount;
		case epm_chamfer_edge_open:
			return &mOpen;
		case epm_edge_chamfer_segments:
			return &mSegments;
		case epm_edge_chamfer_tension:
			return &mTension;
		case epm_edge_chamfer_invert:
			return &mInvert;
		case epm_edge_chamfer_type:
			return &mType;
		case epm_edge_chamfer_smooth:
			return &mSmooth;
		case epm_edge_chamfer_smooth_type:
			return &mSmoothType;
		case epm_edge_chamfer_smooth_threshold:
			return &mSmoothThreshold;
		case epm_edge_chamfer_quad_intersections:
			return &mQuadIntersections;
		case epm_edge_chamfer_mitering_type:
			return &mMiteringType;
		case epm_edge_chamfer_end_bias:
			return &mEndBias;
		case epm_edge_chamfer_radial_bias:
			return &mRadialBias;
		case epm_edge_chamfer_depth:
			return &mDepth;
		default:
			return nullptr;;
		}
	...
	PolyOperation* Clone()
	{
		...
		ret->mMiteringType = mMiteringType;
		ret->mEndBias = mEndBias;
		ret->mRadialBias = mRadialBias;
		ret->mDepth = mDepth;
...
#define PRIOR_2021_CHAMFER_EDGE_PARAM_COUNT 10
IOResult PolyOpChamferEdge::Load(ILoad* iload)
{
	...
	while (IO_OK == (res = iload->OpenChunk()))
	{
		...
		case kQuadChamferVersion:
            ...
            if (EditPolyMod::GetChamferLoadVersionWarning() && mQCVersion > EDITPOLY_QUAD_CHAMFER_VERSION)
            {
                // This chamfer comes from a newer version of Max.
                // We want to let the user know the file they just loaded might not behave properly, so log and display a warning
                TSTR title = GetString(IDS_EDIT_POLY_MOD);
                TSTR message = GetString(IDS_CHAMFER_LOADING_FUTURE_VERSION);
                GetCOREInterface()->Log()->LogEntry(SYSLOG_WARN, DISPLAY_DIALOG, title, message, mQCVersion, EDITPOLY_QUAD_CHAMFER_VERSION);
                // No more notifications on this load
                EditPolyMod::ClearChamferLoadVersionWarning();
            }
			break;
		}
	...
	// Convert old chamfer type and quad intersections to the right mitering type.
    DWORD savedVer = iload->FileSaveAsVersion();
    if (savedVer < MAX_RELEASE_R23)
    {
        if (mType == EP_QUAD_EDGE_CHAMFER)
        {
            mMiteringType = mQuadIntersections ? EP_EDGE_CHAMFER_UNIFORM_MITER : EP_EDGE_CHAMFER_QUAD_MITER;
        }
        else
        {
            mMiteringType = EP_EDGE_CHAMFER_TRI_MITER;
        }
    }
...
IOResult PolyOpChamferEdge::Save(ISave* isave)
{
    ...
    if (saveVersion != 0)
    {
        ...
        // Check for features that the target release doesn't support; in general, apart from chamfers with tri mitering,
        // saving to a file format implying a major release prior to the introduction of the version of this chamfer operation,
        // and opening in that prior release, may at minimum yield a different result than what is obeserved in this release
        if (mMiteringType != EP_EDGE_CHAMFER_TRI_MITER)
        {
            int minSafeRelease = MAX_RELEASE_R26;
            switch (mQCVersion)
            {
            case QCHAM_VERSION_ELWOOD:
                minSafeRelease = MAX_RELEASE_R17;
                break;
            case QCHAM_VERSION_PHOENIX:
                minSafeRelease = MAX_RELEASE_R18;
                break;
            case QCHAM_VERSION_IMOOGI:
                minSafeRelease = MAX_RELEASE_R20;
                break;
            case QCHAM_VERSION_ATHENA_ALPHA1:
                minSafeRelease = MAX_RELEASE_R22;
                break;
            case QCHAM_VERSION_ATHENA_UPDATE1:
            case QCHAM_VERSION_ATHENA_UPDATE2:
            case QCHAM_VERSION_ATHENA_UPDATE3:
            case QCHAM_VERSION_THESEUS:
                minSafeRelease = MAX_RELEASE_R23;
                break;
            case QCHAM_VERSION_THESEUS_UPDATE1:
            case QCHAM_VERSION_THESEUS_UPDATE3:
                minSafeRelease = MAX_RELEASE_R24;
            case QCHAM_VERSION_HEIMDALL_UPDATE1:
            case QCHAM_VERSION_HEIMDALL_UPDATE3:
                minSafeRelease = MAX_RELEASE_R25;
                break;
			case QCHAM_VERSION_VESTA_UPDATE1:
			case QCHAM_VERSION_VESTA_UPDATE2:
            case QCHAM_VERSION_OLYMPUS:
                minSafeRelease = MAX_RELEASE_R26;
                break;
            default:
                break;
            }

            if (saveVersion < minSafeRelease)
            {
                MaybeNotifyUserOfSaveIncompatibility();
            }
        }

        // Make sure we save the proper values for the chamfer type and the quad intersections
        if (saveVersion <= MAX_RELEASE_R22)
        {
            paramCount = PRIOR_2021_CHAMFER_EDGE_PARAM_COUNT;
            mType = EP_STANDARD_EDGE_CHAMFER;
            mQuadIntersections = false;
            switch (mMiteringType)
            {
            case EP_QUAD_EDGE_CHAMFER:
                mType = EP_QUAD_EDGE_CHAMFER;
                break;
...
bool PolyOpChamferEdge::Do(MNMesh& mesh, LocalPolyOpData* pOpData)
{
	...
	bool open = mInvert || mOpen;

	bool altered = false;

	// In current versions of the chamfer operation, merge across any hidden edges and elminate collinear vertices on the input
	// mesh prior to processing, consistent with the approach taken by the Chamfer modifier
	if (mQCVersion >= QCHAM_VERSION_VESTA_UPDATE2)
	{
		mesh.MakePolyMesh();
	}
	...
	if (mMiteringType != EP_EDGE_CHAMFER_TRI_MITER)
	{
        ...
        // Set option flags
        bool isUniformBaseType = false;
        switch (mMiteringType)
        {
        case EP_EDGE_CHAMFER_UNIFORM_MITER:
        case EP_EDGE_CHAMFER_RADIAL_MITER:
        case EP_EDGE_CHAMFER_PATCH_MITER:
            isUniformBaseType = true;
            break;
        default:
            break;
        }

		MN_QCHAM_TYPE geometryOption = open ? (mInvert ? QCHAM_CHAMFERSONLY : QCHAM_NOCHAMFERS) : QCHAM_CHAMFEREDOBJECT;
		DWORD optionFlags = ((mQCVersion >= QCHAM_VERSION_ATHENA_UPDATE2) ? QCHAM_OPTION_LIMIT_EFFECT : QCHAM_OPTION_ELWOOD_DEFAULTS) | 
                            ...
                            ((mMiteringType == EP_EDGE_CHAMFER_PATCH_MITER) ? QCHAM_OPTION_PATCH_CORNERS : 0) |
                            ((mMiteringType == EP_EDGE_CHAMFER_RADIAL_MITER) ? QCHAM_OPTION_RADIAL_CORNERS : 0);
		...
		if (chamferInterface)
		{
			...
			MNQuadChamferExtendedParameters params(mAmount, 0.0f, 1.0f, mSegments, mTension, mDepth, 0.5f, 0, 0.0f, mEndBias, optionFlags, geometryOption, l_flag, mQCVersion, mRadialBias, 1.0f);
			altered = chamferInterface->QuadChamfer(params);
		}
	}
	else // Other cases handled by legacy chamfer
	{
		IMNMeshUtilities18* chamferInterface = mesh.GetTypedInterface<IMNMeshUtilities18>();
		DbgAssert(chamferInterface);
		if (chamferInterface)
		{
			MN_QCHAM_TYPE output = (open ? (mInvert ? QCHAM_CHAMFERSONLY : QCHAM_NOCHAMFERS) : QCHAM_CHAMFEREDOBJECT);
			altered = chamferInterface->ChamferEdges(l_flag, pMeshChamData10, output, mSegments);
		}
...
bool PolyOpCapHoles::Do(MNMesh & mesh, LocalPolyOpData *pOpData)
{
	...
	const int oldFNum = mesh.FNum();

	mesh.FillInBorders(&brdr, mTriangulationType);
	
	// Assign the new faces to a new smoothing group
	DWORD newsm = mesh.GetNewSmGroup();
	for (i = oldFNum; i < mesh.FNum(); i++) mesh.F(i)->smGroup = newsm;
...
public:
	PolyOpExtrudeFace() : mHeight(0.0f), mType(0), mBias(1.0f), mExtrudeFaceGrip(NULL) { }
	~PolyOpExtrudeFace() { }
	...
	if ((mType < 2) || (mType ==3)){
		...
		if (mType == 0) {
			...
			else
			{
                ...
                case POLYOP_2019_1:
                    {
                        IMNMeshUtilities15* l_mesh15 = mesh.GetTypedInterface<IMNMeshUtilities15>();
				...
				case POLYOP_2026:
					{
						...
						const bool useLegacyDPoint3Add = mPolyOperationVersion < POLYOP_2026;
                        l_mesh16->GetExtrudeDirection(&chamData, MN_EDITPOLY_OP_SELECT, mBias, useLegacyDPoint3Add);
                    }
                    break;
                default:
                    DbgAssert(false);
...
// PolyOperation PolyOpExtrudeFacePostprocessed performs the initial topology changes required for an extrude operation via the
// mesh's IMNFaceExtrude interface, setting up the data structures of the interface for further postprocessing during an
// interactive move-mode extrude or scale-mode inset operation.
class PolyOpExtrudeFacePostprocessed: public PolyOperation
{
...
public:
    ...
    // Constructor.
    PolyOpExtrudeFacePostprocessed();

    // Destructor.
    virtual ~PolyOpExtrudeFacePostprocessed();

    // Virtual functions inherited from PolyOperation; see EditPoly.h.
    virtual int OpID() override;
    virtual bool SelLevelSupport(int selLevel) override;
    virtual int MeshSelLevel() override;
    virtual TCHAR* Name() override;
    virtual void GetParameters(Tab<ParamID>& paramList) override;
    virtual void* Parameter(int paramID) override;
    virtual bool ChangesSelection() override;
    virtual bool Do(MNMesh& mesh, LocalPolyOpData* pOpData) override;
    virtual PolyOperation* Clone() override;
    virtual bool IsSaveVersionSupported() const override;
...
private:
    ...
    // Chunk ID for save/load of version data.
    static constexpr USHORT m_versionChunkID = 0x601;

    // Current version of the face extrude postprocessing. Employed for any newly-created PolyOperation, that is, any PolyOperation
    // not loaded from file. Also used a maximum to which versions loaded from file are clipped.
    static constexpr IMNFaceExtrude::IMNFaceExtrudeVersionType m_versionCurr = IMNFaceExtrude::IMN_FACE_EXTRUDE_VER_2023;

    // Version of the face extrude postprocessing to employ in this PolyOperation.
    IMNFaceExtrude::IMNFaceExtrudeVersionType m_version;
};

PolyOpExtrudeFacePostprocessed::PolyOpExtrudeFacePostprocessed():
    PolyOperation(),
    m_version(m_versionCurr)
{
}
...
int PolyOpExtrudeFacePostprocessed::OpID()
{
    return ep_op_extrude_face_postprocessed;
}

bool PolyOpExtrudeFacePostprocessed::SelLevelSupport(int selLevel)
{
...
int PolyOpExtrudeFacePostprocessed::MeshSelLevel()
{
    return MNM_SL_FACE;
}

TCHAR* PolyOpExtrudeFacePostprocessed::Name()
{
    return GetString(IDS_EDITPOLY_EXTRUDE_FACE);
}

void PolyOpExtrudeFacePostprocessed::GetParameters(Tab<ParamID>& paramList)
{
    paramList.ZeroCount();
}

void* PolyOpExtrudeFacePostprocessed::Parameter(int paramID)
{
    return nullptr;
}

bool PolyOpExtrudeFacePostprocessed::ChangesSelection()
{
    return true;
}

bool PolyOpExtrudeFacePostprocessed::Do(MNMesh& mesh, LocalPolyOpData* pOpData)
{
    UNUSED_PARAM(pOpData);

    // Fetch the IMNFaceExtrude interface for the specified mesh, and begin the postprocessed extrusion operation
    IMNFaceExtrude* extrudeInterface = mesh.GetTypedInterface<IMNFaceExtrude>();
    DbgAssert(extrudeInterface != nullptr);
    extrudeInterface->SetVersion(m_version);
    extrudeInterface->BeginFaceExtrude(MN_EDITPOLY_OP_SELECT);

    return true;
}

PolyOperation* PolyOpExtrudeFacePostprocessed::Clone()
{
    PolyOpExtrudeFacePostprocessed* clonedOp = new PolyOpExtrudeFacePostprocessed();
    CopyBasics(clonedOp);
    clonedOp->m_version = m_version;
    return clonedOp;
}

bool PolyOpExtrudeFacePostprocessed::IsSaveVersionSupported() const
{

    // Check the requested save version against the minimum supported Max release for version m_version of the Smart Extrude
    // operation; note that the minimum release corresponds to the introduction of support for version m_version in this
    // PolyOperation, not in IMNFaceExtrude
    bool isSupported = true;
    int saveVersion = GetSavingVersion();
    if (saveVersion != 0)
    {
        int minVersion = MAX_RELEASE_R24;
        switch (m_version)
        {
        case IMNFaceExtrude::IMN_FACE_EXTRUDE_VER_2023:
            minVersion = MAX_RELEASE_R26;
        case IMNFaceExtrude::IMN_FACE_EXTRUDE_VER_2022:
            minVersion = MAX_RELEASE_R25;
            break;
        case IMNFaceExtrude::IMN_FACE_EXTRUDE_VER_2021:
            break;
        default:
            DbgAssert(false);
            break;
        ...
        // If the requested save version is earlier than the minimum support release for this version of the Smart Extrude
        // operation, we must collapse on save
...
void PolyOpExtrudeFacePostprocessed::DeleteThis()
{
    delete this;
}

IOResult PolyOpExtrudeFacePostprocessed::Save(ISave* isave)
{

    // Save basic PolyOperation data
	isave->BeginChunk(kPolyOperationBasics);
	IOResult ioResult = SaveBasics(isave);
	isave->EndChunk();
    ...
    if (ioResult == IO_OK)
    {
        ...
        isave->BeginChunk(m_versionChunkID);
        ioResult = isave->Write(&version, sizeof(int), &nWritten);
        isave->EndChunk();
    }
...
IOResult PolyOpExtrudeFacePostprocessed::Load(ILoad* iload)
{
    ULONG nWritten;

    // Prior to Max 2022.2, extrude version information was not saved; initialize our extrude version value to that employed in
    // those Max releases
    int version = IMNFaceExtrude::IMN_FACE_EXTRUDE_VER_2021;
    ...
    while (ioResult == IO_OK)
    {
        ...
        case kPolyOperationBasics:
            ioResult = LoadBasics(iload);
            break;
        case m_versionChunkID:
            ioResult = iload->Read(&version, sizeof(int), &nWritten);
            break;
        default:
            break;
        }
    ...
    // If the extrude version in the file is newer than our current version, clip to the current verion; as some postprocessing
    // operations supported in the source version may be unavailable, or implemented differently, in the current version, issue
    // a warning, if one has not already been issued
    DbgAssert(version >= IMNFaceExtrude::IMN_FACE_EXTRUDE_VER_2021);
    if (version > m_versionCurr)
    {
        version = m_versionCurr;
        bool doWarning = EditPolyMod::GetExtrudeLoadVersionWarning();
        if (doWarning)
        {
            TSTR warningTitle = GetString(IDS_EDIT_POLY_MOD);
            TSTR warningMsg = GetString(IDS_EXTRUDE_LOADING_FUTURE_VERSION);
            Interface* coreInterface = GetCOREInterface();
            DbgAssert(coreInterface != NULL);
            BOOL isQuite = coreInterface->GetQuietMode();
            BOOL doDisplayDialog = DISPLAY_DIALOG;
            ...
            (coreInterface->Log())->LogEntry(SYSLOG_WARN, doDisplayDialog, warningTitle, warningMsg);
        }

        EditPolyMod::SetExtrudeLoadVersionWarning(false);
    }

    // Set member version value
    m_version = (IMNFaceExtrude::IMNFaceExtrudeVersionType)version;
...
// PolyOperation PolyOpFinalizeExtrudeFacePostprocessed finalizes postprocessing, via a call to the mesh IMNFaceExtrude
// interface's EndFaceExtrude function, at the end of an interactive move-mode extrude or scale-mode inset operation.
class PolyOpFinalizeExtrudeFacePostprocessed: public PolyOperation
{
...
public:
    ...
    // Constructor.
    PolyOpFinalizeExtrudeFacePostprocessed();

    // Destructor.
    virtual ~PolyOpFinalizeExtrudeFacePostprocessed();
...
PolyOpFinalizeExtrudeFacePostprocessed::PolyOpFinalizeExtrudeFacePostprocessed():
    PolyOperation()
{
}
...
int PolyOpFinalizeExtrudeFacePostprocessed::OpID()
{
    return ep_op_finalize_extrude_face_postprocessed;
}

bool PolyOpFinalizeExtrudeFacePostprocessed::SelLevelSupport(int selLevel)
{
    bool isSupported = false;
    if (meshSelLevel[selLevel] == MNM_SL_FACE)
    {
...
int PolyOpFinalizeExtrudeFacePostprocessed::MeshSelLevel()
{
    return MNM_SL_FACE;
}

TCHAR* PolyOpFinalizeExtrudeFacePostprocessed::Name()
{
    return GetString(IDS_EDITPOLY_EXTRUDE_FACE);
}

void PolyOpFinalizeExtrudeFacePostprocessed::GetParameters(Tab<ParamID>& paramList)
{
    paramList.ZeroCount();
}

void* PolyOpFinalizeExtrudeFacePostprocessed::Parameter(int paramID)
{
    return nullptr;
}

bool PolyOpFinalizeExtrudeFacePostprocessed::ChangesSelection()
{
    return true;
}

bool PolyOpFinalizeExtrudeFacePostprocessed::Do(MNMesh& mesh, LocalPolyOpData* pOpData)
{
    UNUSED_PARAM(pOpData);

    // Fetch the IMNFaceExtrude interface for the specified mesh, and perform final postprocessing for the preent extrusion
    // operation; note that if a postprocessed operation is not actually underway, that is, if a corresponding call to
    // BeginFaceExtrude has not be made, then the call to EndFaceExtrude will have no effect, and will be of negligible cost
    IMNFaceExtrude* extrudeInterface = mesh.GetTypedInterface<IMNFaceExtrude>();
    DbgAssert(extrudeInterface != nullptr);
    extrudeInterface->EndFaceExtrude();

    // Eliminate any MN_DEAD geometric entities from the mesh
    mesh.CollapseDeadStructs();

    return true;
}

PolyOperation* PolyOpFinalizeExtrudeFacePostprocessed::Clone()
{
    PolyOpFinalizeExtrudeFacePostprocessed* clonedOp = new PolyOpFinalizeExtrudeFacePostprocessed();
    CopyBasics(clonedOp);
    return clonedOp;
}

bool PolyOpFinalizeExtrudeFacePostprocessed::IsSaveVersionSupported() const
{

    // This PolyOperation was introduced in Max 2022, and does not support any save operation to a versions prior to its
    // introduction
    bool isSupported = true;
    int saveVersion = GetSavingVersion();
    if ((saveVersion != 0) && (saveVersion < MAX_RELEASE_R24))
...
void PolyOpFinalizeExtrudeFacePostprocessed::DeleteThis()
{
    delete this;
}
...
public:
	PolyOpBevelFace() : mHeight(0.0f), mOutline(0.0f), mType(0), mBias(1.0f), mBevelGrip(NULL) { }
	~PolyOpBevelFace() { }
	// Implement PolyOperation methods:
...
bool PolyOpBridgeEdge::Do(MNMesh & mesh, LocalPolyOpData *pOpdata) {
	...
	if (l_meshToBridge != NULL)
	{
		...
		if (mUseSelected)
			ret = l_meshToBridge->BridgeSelectedEdges(MN_EDITPOLY_OP_SELECT, mThresh, mSegments, mAdjacent, reverseTriangles, mTriangulationType);
		else
		{
			...
			ret = l_meshToBridge->BridgeTwoEdges(mTarget1 - 1, mTarget2 - 1, mSegments, 0, 0, 0, false, mTriangulationType);
		}
...
bool PolyOpRetriangulate::Do(MNMesh& mesh, LocalPolyOpData *pOpData)
{
	...
	// Build a list of faces selected for retriangulation
	for (int f = 0; f != mesh.numf; ++f)
	{
		const MNFace& faceCurr = mesh.f[f];
		bool isDead = faceCurr.GetFlag(MN_DEAD);
		bool isSelected = faceCurr.GetFlag(MN_EDITPOLY_OP_SELECT);
		if (isSelected && (!isDead))
		{
			faceList.Append(1, &f, 10);
		}
	}

	// Recompute triangulations for the selected faces based upon the algorithm indicated by mTriangulationType
	TriangulateFaces(mesh, faceList);

	// Return a flag indicating whether any face was actually retriangulated
	int nFacesList = faceList.Count();
	return (nFacesList != 0);
}
...
const USHORT kChunkLocalTransformRetriangulatedVertexLimit = 0x12c6;
...
IOResult LocalTransformData::Save(ISave *isave) {
	...
	if (num > 0) {
		...
		isave->BeginChunk(kChunkLocalTransformRetriangulatedVertexLimit);
		isave->Write(&mNVerticesRetriangulatedMax, sizeof(int), &nb);
		isave->EndChunk();
	}
...
IOResult LocalTransformData::Load(ILoad *iload) {
	...
	// The ability to retriangulate any required faces after applying a transformation was first implemented in Max
	// 2024; for compatibility with legacy models, for which the value of mNVerticesRetriangulatedMax will not be
	// present in the file, initialize to a zero value, disabling retriangulation
	mNVerticesRetriangulatedMax = 0;
	while (IO_OK == (res = iload->OpenChunk())) {
		switch (iload->CurChunkID())
		{
		...
		case kChunkLocalTransformRetriangulatedVertexLimit:
			res = iload->Read(&mNVerticesRetriangulatedMax, sizeof(int), &nb);
			DbgAssert(mNVerticesRetriangulatedMax >= 0);
			break;
		}
		iload->CloseChunk();
		if (res != IO_OK) return res;
...
private:
	...
	// Checks the triangles about each vertex verticesMoved[v], in the triangulation of mesh mesh, for geometric
	// validity, retriangulating the faces to whose triangulations any potentially invalid triangles belong. In
	// particular, a triangle is judged to be potentially invalid if it is collapsed, or if the sense of its outward
	// normal is opposite that of the face average outward normal; note that under this latter criterion, certain
	// grossly non-planar faces may be unnecessarily retriangulated, however, a general condition accounting for such
	// faces, which may be checked in a reasonably computationally inexpensive manner, is not readily available.
	//
	// mesh:          mesh to which the vertices specified in the verticesMoved array belong, potentially modified by
	//                recalculation of required face triangulations, and presumed on entry to be in a filled-in state,
	//                so that mesh.GetFlag(MN_MESH_FILLED_IN) = true; modified
	// verticesMoved: list of indices, in the mesh.v array, of vertices moved during the present transformation
	//                operation; input
	void RetriangulateFacesAtMovedVertices(MNMesh& mesh, const IntTab& verticesMoved);
};
...
bool PolyOpTransform::Do(MNMesh & mesh, LocalPolyOpData *pOpData) {
	...
	IntTab verticesMoved;

	verticesMoved.Resize(max);
	for (int i = 0; i < max; i++)
	{
		mesh.v[i].p += offset[i];
		float offsetDistSq = LengthSquared(offset[i]);
		if (offsetDistSq > 0.0f)
		{
			verticesMoved.Append(1, &i);
		}
	}

	// This PolyOperation is used during all of the interactive operations that are postprocessed by the IMNFaceExtrude
	// class, e.g. the move-mode extrude and scale-mode inset operations, so fetch the interface here and update our
	// postprocessing, noting that if a postprocessed operation is not actually underway, that is, if a corresponding
	// call to BeginFaceExtrude has not be made, then the update call will have no effect, and will be of negligible
	// cost
	IMNFaceExtrude* extrudeInterface = mesh.GetTypedInterface<IMNFaceExtrude>();
	DbgAssert(extrudeInterface != nullptr);
	extrudeInterface->UpdateFaceExtrude();

	// If the present transformation operation has been completed, and the number of vertices moved during the
	// operation is less than the specified limit for retriangulation, check validity of the triangulations of the
	// faces about each merged vertex; if any such triangulation is found to have potentially been made geometrically
	// invalid by this operation, compute a new triangulation for the corresponding face
	int nTemporaryOffsets = data->NumTempOffsets();
	if (nTemporaryOffsets == 0)
	{
		int nVerticesMoved = verticesMoved.Count();
		int nVerticesRetriangulatedMax = data->GetRetriangulationVertexLimit();
		if (nVerticesMoved <= nVerticesRetriangulatedMax)
		{
			RetriangulateFacesAtMovedVertices(mesh, verticesMoved);
		}
	}
...
void PolyOpTransform::RetriangulateFacesAtMovedVertices(MNMesh& mesh, const IntTab& verticesMoved)
{
	static constexpr int nVerticesTriangle = 3;
	static constexpr int nFacesVertexAllocated = 4;
	static constexpr float areaCoeff = 0.5f;
	static constexpr float areaCoeffSq = areaCoeff * areaCoeff;
	static constexpr float areaTolFactor = 1.0E+03f * (std::numeric_limits<float>::epsilon());
	static constexpr float areaTolFactorSq = areaTolFactor * areaTolFactor;

	// Check in debug mode to ensure that the mesh is in the expected state
	DbgAssert(mesh.GetFlag(MN_MESH_FILLED_IN));

	// Proceed only if a non-zero number of vertices were moved during the present transformation operation
	int nVerticesMoved = verticesMoved.Count();
	if (nVerticesMoved != 0)
	{
		IntTab facesAtMovedVertices,facesRetriangulated,faceTriangles;
		Tab<Point3> triangleNormals;
		BitArray isVertexMoved,isFaceAtMovedVertex;

		// Build a unique list facesAtMovedVertices of faces having one or more moved vertex on their bounding curves
		isVertexMoved.SetSize(mesh.numv);
		isFaceAtMovedVertex.SetSize(mesh.numf);
		isVertexMoved.ClearAll();
		isFaceAtMovedVertex.ClearAll();
		facesAtMovedVertices.Resize(nVerticesMoved * nFacesVertexAllocated);
		for (int v = 0; v != nVerticesMoved; ++v)
		{
			DbgAssert((verticesMoved[v] >= 0) && (verticesMoved[v] < mesh.numv));
			bool isDead = (mesh.v[verticesMoved[v]]).GetFlag(MN_DEAD);
			if (!isDead)
			{
				isVertexMoved.Set(verticesMoved[v]);
				IntTab& vertexFaces = mesh.vfac[verticesMoved[v]];
				int nFacesVertex = vertexFaces.Count();
				for (int f = 0; f != nFacesVertex; ++f)
				{
					if (!(isFaceAtMovedVertex[vertexFaces[f]]))
					{
						facesAtMovedVertices.Append(1, &(vertexFaces[f]), 10);
						isFaceAtMovedVertex.Set(vertexFaces[f]);
					}
				}
			}
		}

		// For each face specified in the facesAtMovedVertices array, check the each triangle in the face triangulation
		// about any moved vertex, and if any such triangle is potentially invalid, append the face to the list of
		// faces to be retriangulated
		int nFacesAtMovedVertices = facesAtMovedVertices.Count();
		facesRetriangulated.Resize(nFacesAtMovedVertices);
		for (int f = 0; f != nFacesAtMovedVertices; ++f)
		{
			MNFace& faceCurr = mesh.f[facesAtMovedVertices[f]];
			faceCurr.GetTriangles(faceTriangles);
			int nTriangles = faceCurr.deg - nVerticesTriangle + 1;
			DbgAssert(nTriangles > 0);
			DbgAssert((faceTriangles.Count()) == nTriangles * nVerticesTriangle);

			// Compute the outward normal vector faceNormal for face f, and the outward normal vector
			// triangleNormals[t] for each triangle t of the face triangulation; note that the magnitude of
			// triangleNormals[t] as computed will be given by areaTriangle / areaCoeff, where areaTriangle is the area
			// of triangle t, while similarly, the magnitude of faceNormal will be areaFaceProjected / areaCoeff, where
			// areaFaceProjected is the area of the projection of face f into its average plane
			Point3 faceNormal(0.0f, 0.0f, 0.0f);
			triangleNormals.SetCount(nTriangles, FALSE);
			for (int t = 0; t != nTriangles; ++t)
			{
				Point3 xVerticesTriangle[nVerticesTriangle];

				const int* triangleVertices = &(faceTriangles[t * nVerticesTriangle]);
				for (int v = 0; v != nVerticesTriangle; ++v)
				{
					DbgAssert((triangleVertices[v] >= 0) && (triangleVertices[v] < faceCurr.deg));
					xVerticesTriangle[v] = (mesh.v[faceCurr.vtx[triangleVertices[v]]]).p;
				}

				triangleNormals[t] = CrossProd(xVerticesTriangle[1] - xVerticesTriangle[0], xVerticesTriangle[2] - xVerticesTriangle[0]);
				faceNormal += triangleNormals[t];
			}

			// Check each triangle in the triangulation of face f either until we determine that the triangulation has
			// potentially been invalidated by the present transformation operation, or until each triangle has been
			// checked
			int indexTriangleInFace = 0;
			bool isFaceRetriangulated = false;
			float faceNormalLengthSq = LengthSquared(faceNormal);
			float areaFaceProjectedSq = areaCoeffSq * faceNormalLengthSq;
			float areaTolSq = areaTolFactorSq * areaFaceProjectedSq;
			while ((!isFaceRetriangulated) && (indexTriangleInFace != nTriangles))
			{
				const int* triangleVertices = &(faceTriangles[indexTriangleInFace * nVerticesTriangle]);

				// Check geometric validity of triangle indexTriangleInFace only if it could possibly have been
				// affected by the present operation, that is, only if one or more moved vertices lie along the
				// triangle's bounding curve
				int indexVertexInTriangle = 0;
				BOOL isTriangleAtMovedVertex = FALSE;
				while ((!isTriangleAtMovedVertex) && (indexVertexInTriangle != nVerticesTriangle))
				{
					int indexVertex = faceCurr.vtx[triangleVertices[indexVertexInTriangle]];
					isTriangleAtMovedVertex = isVertexMoved[indexVertex];
					if (isTriangleAtMovedVertex)
					{
						float triangleNormalLengthSq = LengthSquared(triangleNormals[indexTriangleInFace]);
						float areaTriangleSq = areaCoeffSq * triangleNormalLengthSq;
						float normalProd = DotProd(triangleNormals[indexTriangleInFace], faceNormal);

						// Vertex indexVertexInTriangle of triangle indexTriangleInFace was moved during the present
						// operation; if the area of triangle is, to tolerance, zero, or if the sense of the triangle's
						// outward normal is opposite that of the face outward normal, identify the face triangulation
						// as having potentially been invalidated by the present operation
						isFaceRetriangulated = ((areaTriangleSq < areaTolSq) || (normalProd < 0.0f));
					}

					++indexVertexInTriangle;
				}

				++indexTriangleInFace;
			}

			// If the triangulation for face f was idneitified above as being potentially invalidated by the present
			// transformation operation, append to our list of faces to be retriangulated
			if (isFaceRetriangulated)
			{
				facesRetriangulated.Append(1, &(facesAtMovedVertices[f]));
			}
		}

		// Retriangulate all faces identified above
		int nFacesRetriangulated = facesRetriangulated.Count();
		if (nFacesRetriangulated != 0)
		{
			geo::span<const int> retriangulatedFaces(&(facesRetriangulated[0]), size_t(nFacesRetriangulated));
			mesh.RetriangulateFaces(retriangulatedFaces, mTriangulationType);
		}
	}
}
...
void EditPolyMod::InitializeOperationList()
{
    ...
    mOpList[numOps++] = new PolyOpExtrudeFacePostprocessed();
    mOpList[numOps++] = new PolyOpFinalizeExtrudeFacePostprocessed();
...
void EditPolyMod::BeginPointSelectionHitTesting()
{
    m_doPointSelectionHitTest = true;
}

void EditPolyMod::EndPointSelectionHitTesting()
{
...
void EditPolyMod::ExtrudeSelSubComponents(TimeValue timeCurr, bool doPostprocessFaceExtrude)
{
    constexpr int clusterFaceExtrudeType = 0;

    // Proceed only if the current selection type makes sense, and we are presently editing paramters
    int selection = meshSelLevel[selLevel];
    if ((selection != MNM_SL_OBJECT) && (ip != nullptr))
    {
        ...
        else
        {
            ...
            // Perform the extrude operation according to selection type
            switch (selection)
            {
            case MNM_SL_VERTEX:
                {
                    float heightInit,widthInit;

                    // Vertex selection; temporarily adjust height and width parameters, so that we start with a zero offset
                    // for the shift-extrude operation
                    Interval interval = FOREVER;
                    mpParams->EnableNotifications(FALSE);
                    mpParams->GetValue(epm_extrude_vertex_height, timeCurr, heightInit, interval);
                    mpParams->GetValue(epm_extrude_vertex_width, timeCurr, widthInit, interval);
                    mpParams->SetValue(epm_extrude_vertex_height, timeCurr, 0.0f);
                    mpParams->SetValue(epm_extrude_vertex_width, timeCurr, 0.0f);
                    mpParams->EnableNotifications(TRUE);
                    EpModButtonOp(ep_op_extrude_vertex);
                    mpParams->EnableNotifications(FALSE);
                    mpParams->SetValue(epm_extrude_vertex_height, timeCurr, heightInit);
                    mpParams->SetValue(epm_extrude_vertex_width, timeCurr, widthInit);
                    mpParams->EnableNotifications(TRUE);
                }
                break;
            case MNM_SL_EDGE:

                // Edge selection; employ PolyOpCloneEdge, so that only boundary edges are extruded
                if (!mSuspendCloneEdges)
                {
                    EpModButtonOp(ep_op_clone_edge);
                }
            ...
            case MNM_SL_FACE:
                {
                    ...
                    // Face selection; if we want to perform postprocessing on our extrude operation, manage it through the
                    // ep_op_extrude_face_postprocessed operation, which employs the IMNFaceExtrude interface, otherwise,
                    // simply perform the topology change now via the ep_op_extrude_face operation
                    Interval interval = FOREVER;
                    BOOL isEnhancedExtrudeActive = FALSE;
                    mpParams->GetValue(epm_enable_enhanced_hotkey_extrude, timeCurr, isEnhancedExtrudeActive, interval);
                    if (doPostprocessFaceExtrude && isEnhancedExtrudeActive)
                    {

                        // Postprocessed extrusion; simply employ the appropriate PolyOperation
                        EpModButtonOp(ep_op_extrude_face_postprocessed);
                        m_isPostprocessedExtrudeInProgress = true;
                    }
                    else
                    {
                        int typeInit;
                        ...
                        // Non-postprocessed extrusion; temporarily adjust height and extrusion type parameters, so that we
                        // start with a zero offset for the shift-extrude operation, and perform a cluster extrusion
                        mpParams->EnableNotifications(FALSE);
                        mpParams->GetValue(epm_extrude_face_height, timeCurr, heightInit, interval);
                        mpParams->GetValue(epm_extrude_face_type, timeCurr, typeInit, interval);
                        mpParams->SetValue(epm_extrude_face_height, timeCurr, 0.0f);
                        mpParams->SetValue(epm_extrude_face_type, timeCurr, clusterFaceExtrudeType);
                        mpParams->EnableNotifications(TRUE);
                        EpModButtonOp(ep_op_extrude_face);
                        mpParams->EnableNotifications(FALSE);
                        mpParams->SetValue(epm_extrude_face_height, timeCurr, heightInit);
                        mpParams->SetValue(epm_extrude_face_type, timeCurr, typeInit);
                        mpParams->EnableNotifications(TRUE);
                    }
                }
                break;
...
void EditPolyMod::UpdateExtrudeSelSubComponentsPostprocessed(TimeValue timeCurr)
{

    // Nothing to do here; this function simply exists in order to ease the sharing of the PolyModeSelectionProcessor code
    // between Edit Poly and Editable Poly
    UNUSED_PARAM(timeCurr);
}

void EditPolyMod::FinalizeExtrudeSelSubComponentsPostprocessed(TimeValue timeCurr)
{
    ...
    if (isEnhancedExtrudeActive && (selection == MNM_SL_FACE) && (ip != nullptr))
    {
        ...
        // Employ the ep_op_finalize_extrude_face_postprocessed operation to finalize the postprocessing for the present
        // geometry through the IMNFaceExtrudeInterface interface
        EpModButtonOp(ep_op_finalize_extrude_face_postprocessed);
    }

    // Reset flag to indicate that the postprocess extrusion operation has been completed
    m_isPostprocessedExtrudeInProgress = false;
}
...
int EditPolyMod::EpModGetEdgeChamferType()
{
	ScriptPrint(GetString(IDS_WARNING_CHAMFER_TYPE_OBSOLETE));

	int miteringType = -1;
	mpParams->GetValue(epm_edge_chamfer_mitering_type, TimeValue(0), miteringType);
	switch (miteringType)
	{
	case EP_EDGE_CHAMFER_TRI_MITER:
		return EP_STANDARD_EDGE_CHAMFER;
	case EP_EDGE_CHAMFER_QUAD_MITER:
	case EP_EDGE_CHAMFER_UNIFORM_MITER:
	case EP_EDGE_CHAMFER_RADIAL_MITER:
	case EP_EDGE_CHAMFER_PATCH_MITER:
		return EP_QUAD_EDGE_CHAMFER;
	default:
		return -1;
	}
}

void EditPolyMod::EpModSetEdgeChamferType(int chamferType)
{
	ScriptPrint(GetString(IDS_WARNING_CHAMFER_TYPE_OBSOLETE));
	switch (chamferType)
	{
	case EP_STANDARD_EDGE_CHAMFER:
		mpParams->SetValue(epm_edge_chamfer_type, TimeValue(0), EP_STANDARD_EDGE_CHAMFER);
		mpParams->SetValue(epm_edge_chamfer_mitering_type, TimeValue(0), EP_EDGE_CHAMFER_TRI_MITER);
		break;
	case EP_QUAD_EDGE_CHAMFER:
	{
		int quadInt = -1;
		mpParams->GetValue(epm_edge_chamfer_quad_intersections, TimeValue(0), quadInt);
		bool hasQuadIntersection = quadInt != 0;
		mpParams->SetValue(epm_edge_chamfer_type, TimeValue(0), EP_QUAD_EDGE_CHAMFER);
		mpParams->SetValue(epm_edge_chamfer_mitering_type, TimeValue(0), hasQuadIntersection ? EP_EDGE_CHAMFER_UNIFORM_MITER : EP_EDGE_CHAMFER_QUAD_MITER);
	}
	break;
	default:
...
bool EditPolyMod::EpModGetEdgeChamferQuadIntersections()
{
	ScriptPrint(GetString(IDS_WARNING_CHAMFER_QUAD_INTERSECTIONS_OBSOLETE));

	int miteringType = -1;
	mpParams->GetValue(epm_edge_chamfer_mitering_type, TimeValue(0), miteringType);
	if (miteringType == EP_EDGE_CHAMFER_TRI_MITER)
	{
	...
	mpParams->GetValue(epm_edge_chamfer_quad_intersections, TimeValue(0), quadIntersections);

	return quadIntersections == 1;
}

void EditPolyMod::EpModSetEdgeChamferQuadIntersections(bool quadIntersections)
{
	ScriptPrint(GetString(IDS_WARNING_CHAMFER_QUAD_INTERSECTIONS_OBSOLETE));
	...
	mpParams->SetValue(epm_edge_chamfer_quad_intersections, TimeValue(0), quadIntersections);
	mpParams->SetValue(epm_edge_chamfer_mitering_type, TimeValue(0), quadIntersections ? EP_EDGE_CHAMFER_UNIFORM_MITER : EP_EDGE_CHAMFER_QUAD_MITER);
}
...
static void TransformMacroRecord(const TCHAR *fname, int valType, void *val, Matrix3 *partm = NULL,
	Matrix3 *tmAxis = NULL, BOOL localOrigin = false) {

	const TCHAR *optionNames[4];
	int optionTypes[4];
...
void EditPolyMod::Transform(TimeValue t, Matrix3& partm, Matrix3 tmAxis, BOOL localOrigin, Matrix3 xfrm, int type) {
	...
	for (int nd = 0; nd < mcList.Count(); nd++) {
		...
		float *vsw = SoftSelection(t, pData);
		if (!mesh || !td) continue;
...
void EditPolyMod::EpResetSlicePlane() {
	...
	Matrix3 tm(1);
	mpSliceControl->GetValue(0, &tm, CTRL_RELATIVE);
...
Matrix3 EditPolyMod::EpGetSlicePlaneTM(TimeValue t) {
	Matrix3 tm(1);
	mpSliceControl->GetValue(t, &tm, CTRL_RELATIVE);
	return tm;
}
