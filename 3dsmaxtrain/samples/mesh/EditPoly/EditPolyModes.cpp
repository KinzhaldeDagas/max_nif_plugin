...
void EditPolyExtrudeProc::EndExtrude (TimeValue t, bool accept)
{
    ...
    if (accept)
    {
		...
		if (params->GetInt(epm_animation_mode))
        {
    ...
    {
        theHold.Cancel();
        params->SetValue(epm_current_operation, t, ep_op_null);
    }
}
...
void EditPolyOutlineMouseProc::Cancel()
{
    ...
    IParamBlock2* params = mpEditPoly->getParamBlock();
    params->SetValue(epm_current_operation, timeCurr, ep_op_null);
    mpEditPoly->EpModRefreshScreen();
}
...
void EditPolyChamferMouseProc::Drag (TimeValue t, float amount, int segments, float tensionOrDepth)
{
    if (!mInChamfer) return;
    ...
    if (mChamferOperation==ep_op_chamfer_edge)
    {
        ...
        // Edge chamfer; for legacy reasons, since some mitering types use tension rather than depth, tensionOrDepth represents
        // a tension value
        int chamferType;
        params->GetValue (epm_edge_chamfer_type, ip->GetTime(), chamferType);
        params->SetValue (epm_chamfer_edge, t, amount);
        if (chamferType == EP_QUAD_EDGE_CHAMFER)
        {
            params->SetValue (epm_edge_chamfer_segments, t, segments);
            params->SetValue(epm_edge_chamfer_tension, t, tensionOrDepth);
            params->SetValue(epm_edge_chamfer_depth, t, 1.0f - tensionOrDepth);
        }
    }
    else
    {
        DbgAssert(mChamferOperation == ep_op_chamfer_vertex);

        // Vertex chamfer; tensionOrDepth represents depth, as quad vertex chamfer uses depth always, and legacy vertex chamfer
        // has zero segments, so value is irrelevant
        params->SetValue(epm_chamfer_vertex, t, amount);
        params->SetValue(epm_vertex_chamfer_num_segments, t, segments);
        params->SetValue(epm_vertex_chamfer_depth, t, tensionOrDepth);
    }
}
...
{	
    ...
    if (selLevel == MNM_SL_EDGE)
    {
        ...
        int miteringType = EP_EDGE_CHAMFER_TRI_MITER;
        params->GetValue (epm_edge_chamfer_mitering_type, ip->GetTime(), miteringType);
    ...
    case MOUSE_POINT:
        switch(point) {
        case 0:
            Begin (ip->GetTime(), selLevel);
            chamfer = 0;
            if (selLevel == MNM_SL_EDGE)
            {
                if (!isAmountOnly)
                {
                    params->GetValue(epm_edge_chamfer_tension, ip->GetTime(), baseTensionOrDepth);
                    params->GetValue(epm_edge_chamfer_segments, ip->GetTime(), baseSegments);
                }
                else
                {
                    params->GetValue(epm_edge_chamfer_segments, ip->GetTime(), segments);
                }
            }
            else
            {
                params->GetValue(epm_vertex_chamfer_depth, ip->GetTime(), baseTensionOrDepth);
                params->GetValue(epm_vertex_chamfer_num_segments, ip->GetTime(), baseSegments);                
            }
        ...
        case 1:		// Sets the chamfer amount
            ...
            break;
        case 2:		// Sets the tension or depth
            wasAltPressed = false;
            End (ip->GetTime(), true);
            ip->RedrawViews (ip->GetTime(),REDRAW_END);
            return FALSE;
    ...
    case MOUSE_MOVE:
        ...
        Drag (ip->GetTime(), chamfer, segments, tensionOrDepth);
        ip->RedrawViews(ip->GetTime(),REDRAW_INTERACTIVE);
        break;
...
void EditPolyQuickSliceProc::ComputeSliceParams () {
	...
	Point3 planeCenter = mWorld2ModContext * planeCtr;
	mpEPoly->EpSetSlicePlane (planeNormal, planeCenter, mpInterface->GetTime());
}
