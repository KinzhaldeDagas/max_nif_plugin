...
class ADDClassDesc:public ClassDesc {
    ...
    public:
		...
		const TCHAR*    NonLocalizedClassName ( )          { return _T("Add");     }
...
BOOL ImageFilter_Add::Render(HWND hWnd) {
    ...
    line64 = static_cast<BMM_Color_64*>(calloc(srcmap->Width(),sizeof(BMM_Color_64)));
    if (line64 == NULL)
        return (FALSE);

    //-- Allocate Foreground Line
    fgnd64 = static_cast<BMM_Color_64*>(calloc(srcmap->Width(),sizeof(BMM_Color_64)));
    if (fgnd64 == NULL) {
        free(line64);
        return (FALSE);
    }
    ...
    if (mskmap) {
        mask16 = static_cast<WORD*>(calloc(srcmap->Width(),sizeof(WORD)));
        if (!mask16) {
            free(line64);
            free(fgnd64);
        }

        mwidth = std::min(srcmap->Width(),mskmap->Width());
    }
    ...
    width  = std::min(srcmap->Width(),frgmap->Width());
    ...
    for (int iy = 0; iy < srcmap->Height(); iy++) {
        ...
        for (int ix = 0; ix < srcmap->Width(); ix++,m16++,l64++,f64++) {
            ...
            r = std::max(l64->r,f64->r);
            g = std::max(l64->g,f64->g);
            b = std::max(l64->b,f64->b);
            a = std::max(l64->a,f64->a);
        ...
        for (int ix = 0; ix < srcmap->Width(); ix++,l64++,f64++) {
            ...
            l64->r = std::max(l64->r,f64->r);
            l64->g = std::max(l64->g,f64->g);
            l64->b = std::max(l64->b,f64->b);
            l64->a = std::max(l64->a,f64->a);
