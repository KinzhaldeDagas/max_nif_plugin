...
class WIPEClassDesc:public ClassDesc {
     ...
     public:
		...
		const TCHAR*    NonLocalizedClassName()            { return _T("Wipe"); }
...
INT_PTR	ImageFilter_Wipe::Control(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam)	{
...
BOOL ImageFilter_Wipe::Render(HWND hWnd) {
	...
	float count = (float)(std::max(ifi->FilterRange.Count()-1,1));
	...
	for (iy = 0; iy < srcmap->Height(); iy++) {
		...
		if (data.type == IDC_RIGHT && data.reverse && frgmap) {
			...
			x0 = std::max(srcmap->Width() - x1 - 1,0);
...
BOOL ImageFilter_Wipe::LoadConfigure ( void *ptr, DWORD piDataSize ) {
