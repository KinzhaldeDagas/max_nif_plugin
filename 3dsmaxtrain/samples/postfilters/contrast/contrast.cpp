...
class CONClassDesc:public ClassDesc {
	  ...
	  public:
		 ...
		 const TCHAR*    NonLocalizedClassName()			{ return _T("Contrast"); }
...
INT_PTR ImageFilter_Contrast::Control(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam) {
...
BOOL ImageFilter_Contrast::Render(HWND hWnd) {
	...
	for (int iy = 0; iy < srcmap->Height(); iy++) {
		...
		for (int ix = 0; ix < srcmap->Width(); ix++,l64++) {
			...
			l64->r = std::max(std::min( r + k + bright, 65535),0);
			l64->g = std::max(std::min( g + k + bright, 65535),0);
			l64->b = std::max(std::min( b + k + bright, 65535),0);
...
BOOL ImageFilter_Contrast::LoadConfigure ( void *ptr, DWORD piDataSize ) {
