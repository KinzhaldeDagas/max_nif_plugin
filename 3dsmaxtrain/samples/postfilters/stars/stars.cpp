...
class STARSClassDesc:public ClassDesc {
	...
	public:
		...
		const TCHAR*    NonLocalizedClassName()         { return	_T("Stars"); }
...
INT_PTR ImageFilter_Stars::Control(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam)	{
	...
	switch (message) {
		...
		case WM_INITDIALOG:	{
			...
			SetDlgItemText(hWnd, IDC_CUST_FILENAME, data.getFileName());
		...
		case WM_COMMAND:
			...
			switch (LOWORD(wParam)) {
				...
				case IDC_CAMERAS: {
					switch(HIWORD(wParam)) {
						case CBN_SELCHANGE: {
							...
							if(selection != CB_ERR) {
								int characterSize = SendDlgItemMessage(hWnd, IDC_CAMERAS, CB_GETLBTEXTLEN, selection, 0L);
								TSTR cameraName;
								cameraName.Resize(characterSize);
								SendDlgItemMessage(hWnd, IDC_CAMERAS, CB_GETLBTEXT, selection, (LPARAM)cameraName.dataForWrite());
				...
				case IDOK: {
					...
					TCHAR fileName[256];
					GetDlgItemText(hWnd, IDC_CUST_FILENAME, fileName, 256);
					fileName[255] = 0;
...
int ImageFilter_Stars::FCirc(float x, float y, float radius, float aspect, int brite, BMM_Color_48 *c) {
	...
	float rsqr,alpha=0.0f,alpha1,alpha2;
...
BOOL ImageFilter_Stars::Render(HWND hWnd) {
	...
	if(data.dataType == DATA_CUSTOM) {
		...
		if((input_stream=_tfopen(data.getFileName(),_T("rb")))!=NULL)
			goto got_file;

		// Search thru the plugin paths
		for(p = 0; p < Max()->GetPlugInEntryCount(); ++p) {
			const MSTR dir = Max()->GetPlugInDir(p);
			TCHAR fname[256];
			_tcsncpy(fname,dir,255);
			_tcsncat(fname,data.getFileName(),255);
			if((input_stream=_tfopen(fname,_T("rb")))!=NULL)
				goto got_file;
			}
		...
		got_file:
		if(fread(&starcount,sizeof(int),1,input_stream)!=1)
			{
			bad_infile:
		...
		int filesize = ftell(input_stream);
		fseek(input_stream, 4L, SEEK_SET);
		...
		if(fread(stardata,sizeof(Star),starcount,input_stream) != (size_t)starcount)
			goto bad_infile;
	...
	INode *camNode;
	TSTR ourCamera(data.getCameraName());
...
BOOL ImageFilter_Stars::LoadCameras(HWND hWnd)	{
	...
	if(camFiller.OK()) {
		...
		int item = SendDlgItemMessage(hWnd, IDC_CAMERAS, CB_FINDSTRINGEXACT, (WPARAM)-1, (LPARAM)data.getCameraName());
		if(item != CB_ERR)
...
//-----------------------------------------------------------------------------
// #> ImageFilter_Stars::ValidatePiData()
...
BOOL ImageFilter_Stars::ValidatePiData( ImageFilterInfo *fi, ILoad *pILoad )
{
	...
	if (piDataSize == sizeof(STARSDATA_A))
	{
...
BOOL ImageFilter_Stars::LoadConfigure ( void *ptr, DWORD piDataSize ) {
	if (ptr == NULL)
		return FALSE;
	STARSDATA_W *buf = (STARSDATA_W *)ptr;
	if (buf->version == STARSVERSION) 
	{
		if (piDataSize == sizeof(STARSDATA_A))
		{
			...
			memcpy((void *)&data,ptr,sizeof(STARSDATA_A));
			WStr cameraName = WStr::FromCP(codePage, data.camera_a, _countof(data.camera_a));
			wcsncpy(data.camera_w, cameraName.data(), _countof(data.camera_w)-1);
			WStr fileName = WStr::FromCP(codePage, data.filename_a, _countof(data.filename_a));
			wcsncpy(data.filename_w, fileName.data(), _countof(data.filename_w)-1);
			data.camera_a[0] = '\0';
			data.filename_w[0] = '\0';
		}
		else
		{
			memcpy((void *)&data,ptr,sizeof(STARSDATA_W));
		}
...
BOOL ImageFilter_Stars::SaveConfigure ( void *ptr ) {
     if (ptr) {
        memcpy(ptr,(void *)&data,sizeof(STARSDATA_W));
		return (TRUE);
	} else
        return (FALSE);
}
