...
struct STARSDATA_W : public STARSDATA_A {
	wchar_t camera_w[256];
	wchar_t filename_w[256];
	void	setCameraName(const TCHAR* pName)
	{
		WStr cameraName = WStr::FromMCHAR(pName);
		wcsncpy(camera_w,cameraName.data(),_countof(camera_w)-1);
	}
	const TCHAR* getCameraName() const 
	{
		return camera_w;
	}
	void	setFileName(const TCHAR* pName)
	{
		WStr fileName = WStr::FromMCHAR(pName);
		wcsncpy(filename_w,fileName.data(),_countof(filename_w)-1);
	}
