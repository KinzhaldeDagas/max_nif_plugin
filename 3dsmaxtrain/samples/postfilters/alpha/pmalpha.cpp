...
class PMALPHAClassDesc:public ClassDesc {
     ...
     public:
		...
		const TCHAR*    NonLocalizedClassName()            { return _T("PMAlpha"); }
...
BOOL ImageFilter_PMAlpha::Render(HWND hWnd) {
	...
	if (mskmap) {
		...
		mwidth = std::min(width,mskmap->Width());
	}  
		...
		for (int iy = 0; iy < height; iy++) {
			...
			if (mask16) {
				...
				for (int ix = 0; ix < width; ix++,s64++,f64++,m16++) {
					...
					res_r = std::min(std::max(res_r,0.0f),1.0f);
					res_g = std::min(std::max(res_g,0.0f),1.0f);
					res_b = std::min(std::max(res_b,0.0f),1.0f);
					res_a = std::min(std::max(res_a,0.0f),1.0f);
