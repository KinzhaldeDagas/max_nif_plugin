...
class VNormal 
{
	public:
		...
		VNormal(const VNormal& copyVNormal) 
		{
			m_Normal = copyVNormal.m_Normal ;
			m_Smooth = copyVNormal.m_Smooth ;
			m_Init   = copyVNormal.m_Init ;
			m_S	  	 = copyVNormal.m_S;
			m_T		 = copyVNormal.m_T;
			m_SxT	 = copyVNormal.m_SxT ;
			if (copyVNormal.m_Next != NULL)
			{
				m_Next = new VNormal(*(copyVNormal.m_Next));
			}
			else
			{
				m_Next = NULL ;
...
inline bool TMNegParity(const Matrix3 &Mat)
{
...
extern TSTR FindFile(const TCHAR *File);
...
bool					GetFileResource(const TCHAR *Name, const TCHAR *Type, 
