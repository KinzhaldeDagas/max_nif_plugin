...
class MSEmulator : public Texmap {
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_CLASS_NAME) : _T("Emulator");}
...
class MSEmulatorClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Emulator"); }
	SClass_ID		SuperClassID() { return TEXMAP_CLASS_ID; }
	Class_ID		ClassID() { return MSEMULATOR_CLASS_ID; }
...
{
	IAutoMParamDlg* dlg = MSEmulatorDesc.CreateParamDlgs(hwMtlEdit, imp, this);
...
TSTR MSEmulator::GetSubTexmapSlotName(int i, bool localized)
{
...
TSTR MSEmulator::SubAnimName(int i, bool localized)
{
	return _T("");
}

RefResult MSEmulator::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
   PartID& partID, RefMessage message, BOOL propagate ) 
{
...
DWORD_PTR MSEmulator::GetActiveTexHandle(TimeValue t, TexHandleMaker& thmaker)
{
	return(NULL);
}
