...
class ShaderMat
{
	public:
		...
		//
		virtual			~ShaderMat();
		bool			Evaluate(float Time, LPDIRECT3DDEVICE9 Device, RenderMesh *RMesh);

		void			ResetMaterial();
		void			SetName(TSTR &Name);
		void			Set(MatShaderInfo &Info);
		void			SetStatesFromChannels(int ChannelUse);
		bool			LoadTexture(RMatChannel Channel,LPDIRECT3DDEVICE9 Device);
		void			UnLoadTexture(RMatChannel Channel);
		void			SetChannelName(const TSTR &Name, bool On, RMatChannel Channel);
		void			ReLoadAll(LPDIRECT3DDEVICE9 Device);
		bool			AllLoaded(int ChannelUse);
		bool			AllLoadedBut(int ChannelUse, int NotChannelUse);
		int				NumLoaded(int &Index);
		void			SetChannelsMat(LPDIRECT3DDEVICE9 Device);
		void			SetChannelsLight(LPDIRECT3DDEVICE9 Device);
		void			SetChannelsSpecular(LPDIRECT3DDEVICE9 Device);
		void			SetLightGroup();
...
{
	if(m_Name.Length() && m_Name != _T("None"))
	{
		return(true);
	}
...
inline void	ShaderMat::SetChannelName(const TSTR &Name, bool On, RMatChannel Channel)
{
...
inline bool ShaderMat::LoadTexture(RMatChannel Channel, LPDIRECT3DDEVICE9 Device)
{
