...
bool Movie::Load(TSTR &FileName, int &Width, int &Height, int &Bpp, int DestBpp)
{
	...
	else
	{
		...
		m_TargetFmt = (LPBITMAPV4HEADER)malloc(MaxSDK::max(FmtLength,sizeof(BITMAPV4HEADER)));
		...
		if(Bpp != 24)
		{
			TSTR	Str = GetString(IDS_LOAD_FILES);
			Str += m_Name + TSTR(GetString(IDS_UNSUPPORTED));
		...
		m_TargetFmt->bV4Size			= static_cast<DWORD>(MaxSDK::max(FmtLength,sizeof(BITMAPV4HEADER)));	// SR DCAST64: Downcast to 4G limit.
