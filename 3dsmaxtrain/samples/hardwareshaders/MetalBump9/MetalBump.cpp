...
static ParamBlockDesc2 MaterialParamBlk(0,_T("Params"),0,&gDefaultShaderDesc, 
   ...
   p_range,       1000,4000, 
   p_accessor,    &gPSCMAccessor,
   p_nonLocalizedName, _T("Bump Scale"),
   p_end,
   ...
   p_accessor,    &gPSCMAccessor,
   p_nonLocalizedName, _T("Specular"),
   p_end,
   ...
   p_accessor,    &gPSCMAccessor,
   p_nonLocalizedName, _T("reflect amount"),
   p_end,
...
// lighting manager to reload the light list.   
   ...
   if (INode* newNode = GetNotifyParam<NOTIFY_SCENE_ADDED_NODE, NOTIFY_SCENE_PRE_DELETED_NODE>(info))
   {
   ...
   if(m_Device)
   {
      ...
      if(!ref)
      {
         if(Mode.Format != D3DFMT_A8R8G8B8 && Mode.Format != D3DFMT_X8R8G8B8)
         {
            ...
            MaxSDK::MaxMessageBox(NULL, Str, error, MB_OK | MB_ICONWARNING | MB_APPLMODAL);
            return(E_FAIL);
         }   
...
ParamDlg* MaxShader::CreateEffectDlg(HWND hwMtlEdit, IMtlParams *imp) 
{
   ...
   dlg  = gDefaultShaderDesc.CreateParamDlgs(hwMtlEdit,imp,this);
...
TSTR MaxShader::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? (GetString(IDS_COORDS)) : _T("Coordinates"); break;

	case 1: return localized ? (GetString(IDS_VSPARAMS)) : _T("Metal Bump9"); break;
...
int  MaxShader::GetOutFileName(TSTR &fullname, TSTR &fnameOrig, int i) {
   ...
   _tcscpy(baseName, GetCOREInterface()->GetDir(APP_AUTOBACK_DIR).data());
...
// ip->SetUseEnvironmentMap(useEnvMap);
   ...
   if (success) {
      ...
      if( FAILED( D3DXSaveTextureToFile( outputPath.GetCStr(), D3DXIFF_DDS, pcubetex, NULL ) ) )
      {
         assert(0);
         return;
      }
...
bool MaxShader::BrowseOutFile(TSTR &file) {
	...
	ofn.lpstrFileTitle = const_cast<TCHAR*>(_T(""));
	ofn.nMaxFileTitle = 0;
	...
	while (GetSaveFileName(&ofn))
	{
		...
		TCHAR type[MAX_PATH];
		BMMSplitFilename(ofn.lpstrFile, NULL, 0, NULL, 0, type, _countof(type));
		...
		if (file_exists(fileName))
		{
			...
			if (MaxSDK::MaxMessageBox(hWnd, buf, GetString(IDS_SAVE_AS), MB_ICONQUESTION | MB_YESNO) == IDNO)
			{
