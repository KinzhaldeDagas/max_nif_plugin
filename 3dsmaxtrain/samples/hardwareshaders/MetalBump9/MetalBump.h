...
class MaxShader : public ReferenceTarget, public IDX9DataBridge, TimeChangeCallback {
	...
	public:
		...
		ShaderUpdate         *m_ShaderUpdateArray[CHANNEL_MAX];
		...
		void GetClassName(MSTR& s, bool localized) const override					{ s = localized ? GetString(IDS_BPSCLASS_NAME) : _T("Metal Bump9");}
...
public:
		...
		const TCHAR * GetName(){return _T("Metal Bump");}
		...
		int WriteBM(Bitmap *bm, const MaxSDK::AssetManagement::AssetUser& name);
...
class DefaultClassDesc:public ClassDesc2 
{
	...
	const TCHAR*	NonLocalizedClassName()		 { return _T("Metal Bump9"); }
	SClass_ID		SuperClassID()				 { return(REF_TARGET_CLASS_ID); }
	Class_ID		ClassID()					 { return(MAX_SHADER_CLASS_ID); }
