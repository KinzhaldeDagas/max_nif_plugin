...
class Mb_selectClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Mb_select"); }
...
DWORD Mb_select::Start( ) 
{
   ...
   // support.  Now mb_select just uses DX9, but we've kept it for future upgrades.
	...
	_tcscpy(filename,ip->GetDir(APP_MAX_SYS_ROOT_DIR).data());
	...
	_tcscpy(filename,ip->GetDir(APP_MAX_SYS_ROOT_DIR).data());
	_tcscpy(dxDDSName,ip->GetDir(APP_MAX_SYS_ROOT_DIR).data());
