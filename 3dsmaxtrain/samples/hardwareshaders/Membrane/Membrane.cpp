...
static ParamBlockDesc2 pixelshader_param_blk ( pixelshader_params, _T("params"),  0, &MembraneShaderDesc, 
	...
	pb_tv,				_T("tv"),			TYPE_BOOL,	0,	IDS_TV,
		p_default,		FALSE,
		p_ui,			TYPE_SINGLECHECKBOX,	IDC_ANIMATE,
		p_end,
...
static const TCHAR* FindMapFile(const TCHAR* file)
{
	...
	if (found)
	{
		...
		_tcsncpy_s(filepath, _countof(filepath), theFile.GetCStr(), _TRUNCATE);
		return filepath;
	}
	else
	{
...
TSTR MembraneShader::SubAnimName(int i, bool localized)
{
	// TODO: Return the sub-anim names
	switch (i)
	{
	case 0: return localized ? GetString(IDS_COORDS) : _T("Coordinates");
	case 1: return localized ? GetString(IDS_VSPARAMS) : _T("Parameters");
	default: return GetSubTexmapTVName(i - 1, localized);
	}
}
*/

RefResult MembraneShader::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
...
void MembranePixelShader::LoadTexture( LPDIRECT3DTEXTURE9 * ppTex, const TCHAR * filename )
{
	HRESULT hr;
	...
	//load texture
	const TCHAR *cubeMapPath = FindMapFile(filename);
...
HRESULT MembranePixelShader::Initialize(Material *mtl, INode *node)
{
	...
	const TCHAR *cubeMapPath = FindMapFile(_T("PurpleGreenBright.tga"));
...
HRESULT MembraneVertexShader::InitValid(Mesh *mesh, INode *node)
{
	...
	const TCHAR *vertexShaderPath = FindMapFile(_T("membrane.nvv"));	
