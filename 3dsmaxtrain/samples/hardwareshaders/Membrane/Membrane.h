...
class MembraneShader : public ReferenceTarget, public IDX9DataBridge {
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_BPSCLASS_NAME) : _T("Membrane Shader");}
...
public :
	...
	void LoadTexture( LPDIRECT3DTEXTURE9 * ppTex, const TCHAR * filename );
};
...
class MembraneShaderClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Membrane Shader"); }
	SClass_ID		SuperClassID() {return REF_TARGET_CLASS_ID;}
	Class_ID		ClassID() {return MEMBRANE_CLASS_ID;}
