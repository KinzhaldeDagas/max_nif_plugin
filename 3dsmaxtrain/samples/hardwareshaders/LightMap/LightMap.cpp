...
class LightMap : public ReferenceTarget, public IDX9DataBridge {
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_CLASS_NAME) : _T("LightMap");}
...
public:
		...
		void SetLightmap(const MaxSDK::AssetManagement::AssetUser& file);
		void SetDiffusemap(const MaxSDK::AssetManagement::AssetUser& file);

		// From IDXDataBridge
		ParamDlg * CreateEffectDlg(HWND hWnd, IMtlParams * imp);
		void DisableUI();
		void SetDXData(IHardwareMaterial * pHWMtl, Mtl * pMtl);
		const TCHAR * GetName(){return GetString(IDS_EFFECT_NAME);}
...
class LightMapClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("LightMap"); }
	SClass_ID		SuperClassID() { return REF_TARGET_CLASS_ID; }
	Class_ID		ClassID() { return LIGHTMAP_CLASS_ID; }
			...
			case pb_lightmap_filename:
				{
					...
					AssetUser asset = IAssetManager::GetInstance()->GetAsset(v.s,kBitmapAsset);
					map->SetLightmap(asset);
				}
				break;
...
INT_PTR LightMapDlgProc::DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	TSTR p,f,e,name;
...
static ParamBlockDesc2 lightmap_param_blk ( lightmap_params, _T("params"),  0, &LightMapDesc, 
	...
	pb_lightmap_on,			_T("lightmap_on"), TYPE_BOOL,	0,	IDS_LIGHTMAP_ON,
		p_ui,			TYPE_SINGLECHECKBOX,		IDC_LIGHTMAP_ON,
		p_default,		TRUE,
		p_accessor,		&pscm_accessor,
		p_end,
...
void LightMap::SetLightmap(const MaxSDK::AssetManagement::AssetUser& file)
{
	BitmapInfo bi;
	bi.SetAsset(file); 
	PBBitmap bt(bi);
...
void LightMap::SetDiffusemap(const MaxSDK::AssetManagement::AssetUser& file)
{
	BitmapInfo bi;
	bi.SetAsset(file); 
	PBBitmap bt(bi);
...
ParamDlg * LightMap::CreateEffectDlg(HWND hWnd, IMtlParams * imp)
{
	...
	LightMapDesc.RestoreRolloutState();
	return dlg;
}
...
void LightMap::SetDXData(IHardwareMaterial * pHWMtl, Mtl * pMtl)
{
	...
	if(reInit)
	{
		...
		GraphicsWindow *gw = GetCOREInterface()->GetActiveViewExp().getGW();
	...
	if(diffuseOn)
	{
		...
		pHWMtl->SetTextureAddressMode(diffuseStage, D3DSAMP_ADDRESSU, D3DTADDRESS_WRAP);
		pHWMtl->SetTextureAddressMode(diffuseStage, D3DSAMP_ADDRESSU, D3DTADDRESS_WRAP);
...
class LightMapLightAccessor : public IAssetAccessor	{
public:

	LightMapLightAccessor(LightMap* aLightMap) : mLightMap(aLightMap) {}
...
MaxSDK::AssetManagement::AssetUser LightMapLightAccessor::GetAsset() const	{
	return mLightMap->light->bi.GetAsset();
}

bool LightMapLightAccessor::SetAsset(const MaxSDK::AssetManagement::AssetUser& aNewAsset)	{
	mLightMap->pblock->SetValue(pb_lightmap_filename,0,aNewAsset);
	return true;
}


class LightMapDiffuseAccessor : public IAssetAccessor	{
public:

	LightMapDiffuseAccessor(LightMap* aLightMap) : mLightMap(aLightMap) {}
...
MaxSDK::AssetManagement::AssetUser LightMapDiffuseAccessor::GetAsset() const	{
	return mLightMap->diffuse->bi.GetAsset();
}

bool LightMapDiffuseAccessor::SetAsset(const MaxSDK::AssetManagement::AssetUser& aNewAsset)	{
	mLightMap->pblock->SetValue(pb_diffuse_filename,0,aNewAsset);
	return true;

}

// LAM - 4/21/03 - added
void LightMap::EnumAuxFiles(AssetEnumCallback& nameEnum, DWORD flags) 
{
	...
	if(flags & FILE_ENUM_ACCESSOR_INTERFACE)	{
		...
		if(diffuse)	{
			callback->DeclareAsset(LightMapDiffuseAccessor(this));
		}
		if(light)	{
			callback->DeclareAsset(LightMapLightAccessor(this));
		}
	}
	else	{
		if(diffuse)	{	
...
void LightMapDAD::Drop(ReferenceTarget *dropThis, HWND hwnd, POINT p, SClass_ID type, DADMgr*, BOOL) 
{
	TCHAR				Name[256];
	TSTR path,file,ext;
	PBBitmap			Bitmap;
	...
	if(!_tcslen(Name))
	{
		_tcscpy(Name,GetString(IDS_NONE));
	}
