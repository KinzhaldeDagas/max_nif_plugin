...
class ViewportLoaderClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("ViewportManager"); }
...
static ParamBlockDesc2 param_blk ( viewport_manager_params, _T("DirectX Manager"),  0, &theViewportLoaderClassDesc, 
	...
	pb_enabled, 	_T("enabled"),		TYPE_BOOL, 	0, 	IDS_ENABLED, 
		p_default, 		FALSE, 
		p_ui,			TYPE_SINGLECHECKBOX, IDC_ENABLED,
...
ParamDlg *ViewportLoader::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp)
{
	...
	mEdit = hwMtlEdit;
	mparam = imp;;
	mainDlg =  theViewportLoaderClassDesc.CreateParamDlgs(hwMtlEdit, imp, this);
...
RefResult ViewportLoader::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,PartID &partID, RefMessage message, BOOL propagate)
{
	return REF_SUCCEED;
}
...
TSTR ViewportLoader::SubAnimName(int i, bool localized)
{
	// we use the data from the Actual shader
	if (effect)
	{
		IDXDataBridge* vp = (IDXDataBridge*)effect->GetInterface(VIEWPORT_SHADER_CLIENT_INTERFACE);
		const TCHAR* name = vp->GetName();
		return name;
	}
	else
		return _T("");
...
{
	bool active = false;
	if(GetDevice()==NULL && !MaxSDK::Graphics::IsRetainedModeEnabled())
		return active;
...
bool ViewportLoader::SaveFXFile(const TCHAR * fileName)
{
	if(!pblock)
		return false;
...
bool ViewportLoader::IsCurrentEffectEnabled()
{
	...
	if (NULL != pblock)
		pblock->GetValue(pb_enabled, 0, l_pluginEnabled, FOREVER);
...
bool ViewportLoader::SetCurrentEffectEnabled(bool enabled)
{
	if (NULL != pblock)
	{
		pblock->SetValue(pb_enabled, 0, (BOOL)enabled);
...
{
	MtlBase* l_pMaterial = FindOwnerMaterial();
	...
	return 
		...
		( NULL != ::GetDevice() || MaxSDK::Graphics::IsRetainedModeEnabled()) && 
...
bool ViewportLoader::IsCurrentEffectSupported()
{
	return 
		...
		( NULL != GetEffectCD(effectIndex - 1) ) && 
		( NULL != effect ) && 
		// DirectX device should have been initialized successfully.
		( NULL != ::GetDevice() );
}
...
void ViewportLoader::HandleEffectIndexChanged(TimeValue t, int effectIndexNew)
{
	if (effectIndexNew == effectIndex)
	{
		// nothing is needed to do.
		return;
	}

	effectIndex = effectIndexNew;
	LoadEffect(GetEffectCD(effectIndex - 1));

	// update the UI
	if (IsCurrentEffectSupported())
	{
		EnableDlgItem(IDC_ENABLED, TRUE);
	}
	else
	{
		SetCurrentEffectEnabled(false);
		EnableDlgItem(IDC_ENABLED, FALSE);
	}

	EnableDlgItem(IDC_SAVE_FX, IsSaveFxSupported());
...
void ViewportLoader::HandleDxStdMtlChanged(TimeValue t, bool enabled)
{
	...
	if(!effect && !enabled){
		...
		EnableDlgItem(IDC_ENABLED, !enabled);
	}
	EnableDlgItem(IDC_EFFECTLIST, !enabled);
...
{
	IParamMap2* l_pParamMap = pblock->GetMap();
	if (NULL != l_pParamMap)
	{
...
const TCHAR* ViewportLoader::GetName(bool localized)
{
	return localized ? GetString(IDS_VIEWPORT_MANAGER) : _T("DirectX Manager");
}
