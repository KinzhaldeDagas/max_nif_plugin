...
void ViewportManagerControl::EditMtlBaseNotify( void *param, NotifyInfo *info ) 
{
	...
	MtlBase* mtlbase = GetNotifyParam<NOTIFY_MTLBASE_PARAMDLG_PRE_OPEN>(info);
...
DWORD ViewportManagerControl::Start( ) 
{
	...
	if ( CheckForDX() )
	{
		ShowManager = MaxSDK::Util::GetPrivateProfileInt(_T("DXSettings"),_T("ManagerEnabled"),1,filename);
	...
	else
	{
		...
		ShowManager = MaxSDK::Util::GetPrivateProfileInt(_T("OGLSettings"),_T("ManagerEnabled"),0,filename);
		sm->SetVisible(ShowManager);
	}
...
void ViewportManagerControl::LoadManager( MtlBase* mtl )
{
	...
	// for scripted materials, operate on the delegate.
	while (mtl->GetInterface(I_MAXSCRIPTPLUGIN))
	{
		if (mtl->GetReference(0)) // ref 0 is the delegate
			mtl = (MtlBase*)mtl->GetReference(0);
		else
			break;
	}
