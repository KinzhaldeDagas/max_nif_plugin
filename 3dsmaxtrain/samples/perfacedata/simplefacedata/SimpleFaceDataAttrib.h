...
public:
	...
	void		GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_SIMPLEFACEDATAATTRIB_NAME) : _T("SimpleFaceData");}
...
public:  // "internal" helper functions
	...
	scope in which it is obtained. Unique_ptrs transfer and free the allocated data they wrap.
	*/
	static std::unique_ptr<ISimpleFPChannel> WrapFaceChannel(IFaceDataChannel* pChan, int type);
...
public: 	// ISimpleFaceDataChannel external interface
	...
	const TCHAR* GetChannelName() override;
...
class SimpleFaceDataAttribClassDesc : public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("SimpleFaceData"); }
	SClass_ID		SuperClassID() { return CUST_ATTRIB_CLASS_ID; }
	Class_ID		ClassID() { return SimpleFaceData_ClassID; }
