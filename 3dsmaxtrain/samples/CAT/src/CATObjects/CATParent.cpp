...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATParent"); }
...
public:
	...
	void Refresh() {
		...
		if (pCATParentTrans)
		{
			...
			mEdtCATName->SetText(pCATParentTrans->GetCATName().data());
	...
	INT_PTR CALLBACK DlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		case CC_SPINNER_CHANGE:
			...
			case IDC_SPIN_CATUNITS:
				bool arePtrsValid = mpCATParent && mSpnCATUnits;
				if (arePtrsValid)
				{
					ICATParentTrans* pCATParentTrans = mpCATParent->GetCATParentTrans();
					if (pCATParentTrans != NULL)
...
public:
	...
	CATRigRollout() {
		...
		bShowFolders = true;
		_tcscpy_s(strCurrentFolder, _countof(strCurrentFolder), _T(""));
	}
	...
	BOOL InitControls(HWND hDlg, CATParent *catParent)
	{
		...
		if (catparent->GetRigFilename() != _T(""))
		{
			...
			_tcsnccpy_s(lastSelectedFilename, _countof(lastSelectedFilename), catparent->GetRigFilename().data(), catparent->GetRigFilename().Length());
			TSTR path, filename, extension;
			SplitFilename(catparent->GetRigFilename(), &path, &filename, &extension);
		...
		if (!dirfound) ChangeDirectory(GetCOREInterface()->GetDir(APP_USER_SETTINGS_DIR).data());
	...
	void DoSelectionChanged() {
		...
		if (item != LB_ERR) {
			...
			if (nIndex == 0) {
				...
				_tcscpy_s(lastSelectedFilename, _countof(lastSelectedFilename), GetString(IDS_NONE));
				return;
			}
			else if (nIndex > 0) {
				SetSelectedRigFilename(vecFileNames[nIndex]);
				_tcsnccpy_s(lastSelectedFilename, _countof(lastSelectedFilename), vecFileNames[nIndex], MAX_PATH);
			}
		}
	}
	...
	void RefreshPresetList(TCHAR *currentFile = NULL)
	{
		...
		_stprintf_s(emptyRig, MAX_PATH, GetString(IDS_NONE));
			...
			if (!_tcscmp(strCurrentFolder, _T(""))) {
				...
				for (int nDrive = 1; nDrive <= 26; nDrive++) {
					if (nDrive <= 2 || !_chdrive(nDrive)) {
						...
						DbgAssert(drive);
						_stprintf_s(drive, 4, _T("%c:\\"), nDrive + 'A' - 1);
						_stprintf_s(driveName, _countof(driveName), _T("<%c:>"), nDrive + 'A' - 1);
				...
				if (0 == _tcscmp(strCurrentFolder, szParentDir)) {
					...
					_tcscpy_s(szParentDir, _countof(szParentDir), _T(""));
				}

				// Insert the '..' directory.
				size_t buflen = _tcsnlen(szParentDir, _countof(szParentDir)) + 4;
				TCHAR *folder = new TCHAR[buflen];
				DbgAssert(folder);

				_tcscpy_s(folder, buflen, szParentDir);
				...
				_stprintf_s(searchName, _countof(searchName), _T("%s\\*.*"), strCurrentFolder);
				...
				do {
					if (find.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
						if (_tcscmp(find.cFileName, _T(".")) != 0 && _tcscmp(find.cFileName, _T("..")) != 0) {
							...
							_stprintf_s(fileName, _countof(fileName), _T("<%s>"), find.cFileName);

							// We need to save the full path, so allocate some
							// memory.  If this fails, don't even bother adding
							// it to the list box.
							size_t buflen = _tcsnlen(strCurrentFolder, _countof(strCurrentFolder)) + _tcsnlen(find.cFileName, _countof(find.cFileName)) + 8;
							TCHAR *folder = new TCHAR[buflen];

							if (folder) {
								_stprintf_s(folder, buflen, _T("%s\\%s"), strCurrentFolder, find.cFileName);
		...
		if (!catparent->GetCATParentTrans() || !(catparent->GetCATParentTrans() && catparent->GetCATParentTrans()->GetRootHub())) {
			...
			// in a vector.
			_stprintf_s(searchName, _countof(searchName), _T("%s\\*.%s"), strCurrentFolder, CAT_RIG_PRESET_EXT);
			hFind = FindFirstFile(searchName, &find);
			if (hFind != INVALID_HANDLE_VALUE) {
				do {
					if (find.dwFileAttributes != FILE_ATTRIBUTE_DIRECTORY) {
						// Copy the filename and remove the extension.
						_tcscpy_s(fileName, _countof(fileName), find.cFileName);
						fileName[_tcsnlen(fileName, _countof(fileName)) - _tcsnlen(CAT_RIG_PRESET_EXT, _countof(CAT_RIG_PRESET_EXT)) - 1] = _T('\0');

						// We need to save the full path, so allocate some
						// memory.  If this fails, don't even bother adding
						// it to the list box.
						size_t buflen = _tcsnlen(strCurrentFolder, _countof(strCurrentFolder)) + _tcsnlen(find.cFileName, _countof(find.cFileName)) + 8;
						TCHAR *file = new TCHAR[buflen];

						if (file) {
							_stprintf_s(file, buflen, _T("%s\\%s"), strCurrentFolder, find.cFileName);
							id = (int)SendMessage(hListPresets, LB_INSERTSTRING, (WPARAM)-1, (LPARAM)fileName);
							SendMessage(hListPresets, LB_SETITEMDATA, id, vecFileNames.size());
							vecFileNames.push_back(file);

							// Select the most recently used file.
							if (!_tcsncicmp(catparent->GetRigFilename().data(), file, MAX_PATH) || !_tcsncicmp(lastSelectedFilename, file, MAX_PATH))
								SendMessage(hListPresets, LB_SETCURSEL, id, 0);
						}
					}
				} while (FindNextFile(hFind, &find));
		...
		_stprintf_s(searchName, _countof(searchName), _T("%s\\*.%s"), strCurrentFolder, CAT3_RIG_PRESET_EXT);
		hFind = FindFirstFile(searchName, &find);
		if (hFind != INVALID_HANDLE_VALUE) {
			do {
				if (find.dwFileAttributes != FILE_ATTRIBUTE_DIRECTORY) {
					// Copy the filename and remove the extension.
					_tcscpy_s(fileName, _countof(fileName), find.cFileName);
					fileName[_tcsnlen(fileName, _countof(fileName)) - _tcsnlen(CAT3_RIG_PRESET_EXT, _countof(CAT3_RIG_PRESET_EXT)) - 1] = _T('\0');
				...
				case IDC_PRESETS_LIST:
				{
					...
					if (nItem != LB_ERR) {
						...
						if (nIndex < 0) {
							...
							if (!bSuccess) {
								TSTR msgBoxText = GetString(IDS_ERR_NOACCESS);
								MaxSDK::MaxMessageBox(hWnd, msgBoxText, GetString(IDS_ERROR), MB_OK | MB_ICONSTOP);
							}
							RefreshPresetList();
						}
						else if (catparenttrans/* && !catparenttrans->GetRootHub()*/)
						{
							...
							Matrix3 origTM(1);
							INode* origNode = catparenttrans->GetNode();
							if (origNode)
							{
								origTM = origNode->GetNodeTM(GetCOREInterface()->GetTime());
							}
							theHold.Begin();
							...
							if (NULL != pNode)
							{
								...
								pNode->SetNodeTM(GetCOREInterface()->GetTime(), origTM);
							}
							else
							{
								// TODO: Messagebox?
			...
			case BN_BUTTONUP:
				...
				case IDC_BTN_LOADRIG:
					...
					{						
						...
						if (MatchPattern(extension, TSTR(CAT3_RIG_PRESET_EXT), TRUE))
						{
							...
							_stprintf_s(filename, _countof(filename), file.data());
						...
						{
							Matrix3 origTM(1);
							if (INode* origNode = catparenttrans->GetNode())
							{
								origTM = origNode->GetNodeTM(GetCOREInterface()->GetTime());
							}
							if (catparenttrans->LoadRig(filename))
							{
								...
								// the catparenttrans becomes invalid after LoadRig() so we need to reference the new instance from catparent!
								if (catparent && catparent->GetCATParentTrans())
								{
									if (INode* pNode = catparent->GetCATParentTrans()->GetNode())
									{
										pNode->SetNodeTM(GetCOREInterface()->GetTime(), origTM);
									}
								}								
				...
				case IDC_BTN_SAVERIG:
				{
					...
					if (result)
					{
						...
						{
							TSTR msgBoxText = GetString(IDS_ERR_FILESAVE);
							MaxSDK::MaxMessageBox(hWnd, msgBoxText, GetString(IDS_ERR_RIGEXPORT), MB_OK);
						}
						// the rig has a .rg3 filename => remember it and change dir to it
						_tcsnccpy_s(lastSelectedFilename, _countof(lastSelectedFilename), catparent->GetRigFilename().data(), catparent->GetRigFilename().Length());
						TSTR path, filename, extension;
...
void CATParent::GetClassName(MSTR& s, bool localized) const { s = localized ? GetString(IDS_CL_CATPARENT) : _T("CATParent"); }

void CATParent::CATParentNodeCreateNotify(void *param, NotifyInfo *info)
{
	INode* node = GetNotifyParam<NOTIFY_NODE_CREATED>(info);
	CATParent *pCATParent = (CATParent*)param;
	if (node && node->GetObjectRef()->FindBaseObject() == pCATParent) {
...
static void OnChangeParentsNotifyProc(void* arg, NotifyInfo* pInfo)
{
	if (pInfo != NULL)
	{
		if (INode* pChild = GetNotifyParam<NOTIFY_NODE_LINKED>(pInfo))
		{
	...
	while (IO_OK == (res = iload->OpenChunk())) {
		...
		case CATPARENTCHUNK_SAVE_VERSION:
			...
			if (dwFileSaveVersion > CAT_VERSION_CURRENT && !g_bOutOfDateWarningDisplayed) {
				_tcscpy_s(buf, _countof(buf), GetString(IDS_ERR_FUTUREFILE1));
				_tcscat_s(buf, _countof(buf), GetString(IDS_ERR_FUTUREFILE2));
				_tcscat_s(buf, _countof(buf), GetString(IDS_ERR_FUTUREFILE3));
				DWORD dwResult = MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), buf, GetString(IDS_ERR_OODATEVER), MB_YESNO | MB_ICONASTERISK);
			...
			if (dwFileSaveVersion < CAT_VERSION_1300) {
				_tcscpy_s(buf, _countof(buf), GetString(IDS_ERR_OLDFILE1));
				_tcscat_s(buf, _countof(buf), GetString(IDS_ERR_OLDFILE2));
				_tcscat_s(buf, _countof(buf), GetString(IDS_ERR_OLDFILE3));
				_tcscat_s(buf, _countof(buf), GetString(IDS_ERR_OLDFILE4));
				_tcscat_s(buf, _countof(buf), GetString(IDS_ERR_OLDFILE5));
				DWORD dwResult = MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), buf, GetString(IDS_ERR_UPGRADEPROB), MB_YESNO | MB_ICONASTERISK);
...
ICATParentTrans* CATParent::GetCATParentTrans(bool bSearch)
{
	if (catparenttrans == NULL && bSearch)
	{
...
const ICATParentTrans* CATParent::GetCATParentTrans(bool bSearch) const {
	return const_cast<const ICATParentTrans*>(const_cast<CATParent*>(this)->GetCATParentTrans(bSearch));
}

void CATParent::SetCATParentTrans(ICATParentTrans* parentTrans)
{
	catparenttrans = static_cast<CATParentTrans*>(parentTrans);
}
...
TSTR CATParent::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_CLIPHIERARCHY) : _T("Layers"); // "Layers"
	default: return _T("");
	}
}
...
const TCHAR* CATParent::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_CATOBJECT) : _T("CAT Object");
}
