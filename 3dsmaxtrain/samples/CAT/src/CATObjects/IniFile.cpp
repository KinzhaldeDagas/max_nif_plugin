...
bool IniFile::Load()
{
	...
	int nCurrentGroup = -1;
	int nGroup, nKey;
	TCHAR *szGroup = nullptr, *szKey = nullptr, *szValue = nullptr;
...
bool IniFile::Save()
{
	if (mFileName.isNull()) return false;
	...
	if (!testWriter.Open(mFileName, false, encoding))
		return false;
...
int IniFile::AddGroup(const TCHAR *szGroup)
{
	...
	if (szGroup == NULL && mVecGroups.size() > 0) return -1;

	if (FindGroup(szGroup) != -1) return -1;
	int nGroup = (int)mVecGroups.size();

	IniGroupRecord *pNewGroup = new IniGroupRecord(szGroup);
	DbgAssert(pNewGroup);
	mVecGroups.push_back(pNewGroup);
...
int IniFile::AddKey(int nGroup, const TCHAR* szKey, const TCHAR* szValue/*=""*/)
{
	...
	mDirty = true;
	return GetGroup(nGroup).AddKey(IniKeyRecord(szKey, szValue));
}
...
{
	GetGroup(nGroup).GetKey(nKey).second = szValue;
	mDirty = true;
}
