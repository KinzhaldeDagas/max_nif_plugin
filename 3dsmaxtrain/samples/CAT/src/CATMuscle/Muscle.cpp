...
public:
	...
	void Refresh(TimeValue t)
	{
		...
		edtName->SetText(this->muscle->GetName().data());
	...
	void InitControls(HWND hDlg, Muscle *muscle)
	{
		...
		Color clr = *muscle->GetColour();
		swColour = GetIColorSwatch(GetDlgItem(hWnd, IDC_COLOURSWATCH_START), clr, (muscle->GetName() + GetString(IDS_START1)).data());
		swColour->SetModal();
	...
	BOOL CALLBACK DlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_CUSTEDIT_ENTER:
			...
			{
			case IDC_EDIT_NAME:
				if (edtName != nullptr && muscle != nullptr)
				{
					TCHAR strbuf[LBL_LENGTH];
					edtName->GetText(strbuf, LBL_LENGTH);
		...
		case CC_SPINNER_CHANGE: {
			if (muscle != nullptr)
			{
				...
				switch (LOWORD(wParam))
				{ // Switch on ID
				case IDC_SPIN_NUMVSEGS:
					muscle->SetNumVSegs(ccSpinner->GetIVal());
					break;
				case IDC_SPIN_NUMUSEGS:
					muscle->SetNumUSegs(ccSpinner->GetIVal());
					break;
		...
		case CC_COLOR_CHANGE:
			...
			{
				Color clr = Color(swColour->GetAColor());
...
static void MuscleNodeCreateNotify(void *param, NotifyInfo *info)
{
	...
	if (node && node->GetObjectRef()->FindBaseObject() == muscle) {
...
BaseInterface* Muscle::GetInterface(Interface_ID id)
{
	...
	if (id == CATMUSCLE_SYSTEMDRIVER_INTERFACE_ID) return (ICATMuscleSystemDriver*)this;
...
int MuscleCreateCallBack::proc(ViewExp *vpt, int msg, int point, int flags, IPoint2 m, Matrix3& mat)
{
	...
	if (msg == MOUSE_FREEMOVE) {
		...
		case 1:
		{
			...
			Point3 bl = Point3(std::min(tm.GetTrans()[0], 0.0f), std::min(tm.GetTrans()[1], 0.0f), 0.0f);
			Point3 br = Point3(std::max(tm.GetTrans()[0], 0.0f), std::min(tm.GetTrans()[1], 0.0f), 0.0f);
			Point3 tl = Point3(std::min(tm.GetTrans()[0], 0.0f), std::max(tm.GetTrans()[1], 0.0f), 0.0f);
			Point3 tr = Point3(std::max(tm.GetTrans()[0], 0.0f), std::max(tm.GetTrans()[1], 0.0f), 0.0f);
...
TSTR Muscle::SubAnimName(int i, bool localized)
{
	if (i < NUMNODEREFS)
	{
		if (tabHandles[i])
			return tabHandles[i]->GetName();
		else
			return _T("");
	}
	i -= NUMNODEREFS;
	...
	{
		if (i % 2 == 0)
			return (spherename + (localized ? GetString(IDS_HLD_DISTORTION) : _T("Distortion")));
		if (i % 2 == 1)
			return (spherename + (localized ? GetString(IDS_HLD_HARDNESS) : _T("Hardness")));
	}
	return _T("");
}
...
void MuscleCloneNotify(void *param, NotifyInfo *info)
{
	...
	NotifyPostNodesCloned* pClonedNodes = GetNotifyParam<NOTIFY_POST_NODES_CLONED>(info);
	...
	if (pClonedNodes && muscles->pOrig != NULL && muscles->pClone != NULL)
	{
...
void Muscle::UpdateName()
{
	...
	if (tabHandles[HDL_A])	tabHandles[HDL_A]->SetName((newname + _T("_A")).data());
	if (tabHandles[HDL_AC])	tabHandles[HDL_AC]->SetName((newname + _T("_AC")).data());
	if (tabHandles[HDL_AB])	tabHandles[HDL_AB]->SetName((newname + _T("_AB")).data());
	if (tabHandles[HDL_ACB])	tabHandles[HDL_ACB]->SetName((newname + _T("_ACB")).data());

	if (tabHandles[HDL_B])	tabHandles[HDL_B]->SetName((newname + _T("_B")).data());
	if (tabHandles[HDL_BD])	tabHandles[HDL_BD]->SetName((newname + _T("_BD")).data());
	if (tabHandles[HDL_BA])	tabHandles[HDL_BA]->SetName((newname + _T("_BA")).data());
	if (tabHandles[HDL_BAD])	tabHandles[HDL_BAD]->SetName((newname + _T("_BAD")).data());

	if (tabHandles[HDL_C])	tabHandles[HDL_C]->SetName((newname + _T("_C")).data());
	if (tabHandles[HDL_CA])	tabHandles[HDL_CA]->SetName((newname + _T("_CA")).data());
	if (tabHandles[HDL_CD])	tabHandles[HDL_CD]->SetName((newname + _T("_CD")).data());
	if (tabHandles[HDL_CAD])	tabHandles[HDL_CAD]->SetName((newname + _T("_CAD")).data());

	if (tabHandles[HDL_D])	tabHandles[HDL_D]->SetName((newname + _T("_D")).data());
	if (tabHandles[HDL_DB])	tabHandles[HDL_DB]->SetName((newname + _T("_DB")).data());
	if (tabHandles[HDL_DC])	tabHandles[HDL_DC]->SetName((newname + _T("_DC")).data());
	if (tabHandles[HDL_DBC])	tabHandles[HDL_DBC]->SetName((newname + _T("_DBC")).data());
	...
	case DEFORMER_BONES: {
		...
		for (int v = 0; v < tabStrips.Count(); v++)
		{
			...
			for (int j = 0; j < nNumUSegs; j++) {
				...
				segnode->SetName((newname + strStrandNum + strSegNum).data());
				//////////////////////////////////////////////////////////////////////
				// user Properties.
				// these will be used by the runtime libraries
...
void Muscle::PasteMuscle(ReferenceTarget* pasteRef)
{
	...
	if (pasteRef != nullptr)
	{
		...
		else
		{
			...
			INode* node = pasteRef->GetTypedInterface<INode>();
			if (node != nullptr)
			{
	...
	if (TestFlag(MUSCLEFLAG_HANDLES_VISIBLE))
	{
		for (int i = 0; i < NUMNODEREFS; i++)
		{
			if (tabHandles[i]) {
				...
				if (!tabHandles[i]->GetParentNode()->IsRootNode() && tabHandles[i]->GetParentNode()->GetTMController()->GetInterface(I_DRIVER) == this)
					continue;
...
IOResult Muscle::LoadMuscle(ILoad *iload)
{
	...
	while (IO_OK == (res = iload->OpenChunk())) {
		...
		case MUSCLE_NUMSAVES_CHUNK: {
			...
			g_nNumSaves = std::max(loadednumsaves, g_nNumSaves);
			break;
		}
#endif
