...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Muscle Handle"); }
	...
	void proc(ILoad *) {
		...
		if (pobj->dVersion < CATMUSCLE_VERSION_0850) {
			...
			if (node &&
				...
				node->GetParentNode()->GetTMController()->GetInterface(I_DRIVER) != node->GetTMController()->GetInterface(I_DRIVER)) {
