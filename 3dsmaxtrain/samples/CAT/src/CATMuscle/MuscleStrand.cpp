...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Muscle Strand"); }
...
public:
	...
	BOOL CALLBACK DlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case CC_SPINNER_CHANGE: {
			bool arePtrsValid = muscle && spnUStart && spnUEnd && spnRadius;
			if (arePtrsValid)
			{
...
void MuscleStrand::UpdateName(const MCHAR* nodeName)
{
	...
	if (tabHandles[NODE_ST])		tabHandles[NODE_ST]->SetName((newname + GetString(IDS_MNAME_START)).data());
	if (tabHandles[NODE_ST_HDL])	tabHandles[NODE_ST_HDL]->SetName((newname + GetString(IDS_MNAME_STARTHDL)).data());
	if (tabHandles[NODE_EN])		tabHandles[NODE_EN]->SetName((newname + GetString(IDS_MNAME_END)).data());
	if (tabHandles[NODE_EN_HDL])	tabHandles[NODE_EN_HDL]->SetName((newname + GetString(IDS_MNAME_ENDHDL)).data());
}
