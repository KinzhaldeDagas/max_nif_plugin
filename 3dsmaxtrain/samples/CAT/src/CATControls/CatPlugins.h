...
class ZeroToOneParamsAccessor : public PBAccessor
{
	void Set(PB2Value& v, ReferenceMaker*, ParamID, int tabIndex, TimeValue)
	{
		...
		v.f = std::min(std::max(v.f, 0.0f), 1.0f);
	}
};
...
// general helper function to dsiplay an error mesage for a number of seconds on the status bar prompt
inline void LogErrMsgOnPrompt(const MSTR& name)
{
	MSTR msg = GetString(IDS_NULL_CATPARENT_CAP);
	msg.Append(name);
	TSTR buf = GetString(IDS_NULL_CATPARENT_MSG);
	msg.Append(buf);
