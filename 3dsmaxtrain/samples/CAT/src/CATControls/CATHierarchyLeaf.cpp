...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATHierarchyLeaf"); }
...
void CATHierarchyLeaf::GetClassName(MSTR& s, bool localized) const { s = localized ? GetString(IDS_CL_CATHIERARCHYLEAF) : _T("CATHierarchyLeaf"); }
...
// just in case it gets used after we've deleted.
...
TSTR CATHierarchyLeaf::SubAnimName(int i, bool localized)
{
	return _T("");
}
...
RefTargetHandle CATHierarchyLeaf::Clone(RemapDir& remap) {
	...
	newCATHierarchyLeaf->ReplaceReference(PBLOCK_REF, clonedpblock);

	// P_NO_REF's:
	// PB_LEAFPARENT, handled in post patch.
	// PB_WEIGHTS, handled in post patch.
