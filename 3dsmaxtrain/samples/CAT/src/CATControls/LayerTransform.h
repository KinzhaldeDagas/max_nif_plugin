...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override { s = _T("LayerTransform"); } //GetString(IDS_CL_LAYER_TRANSFORM);}

	RefTargetHandle Clone(RemapDir &remap);
	RefResult NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate);

	int NumSubs() { return 1; }
	TSTR SubAnimName(int i, bool localized) override { return localized ? GetString(IDS_LYR_TRANSFORM) : _T("Layer Transform"); }
...
public:
	...
	void* GetInterface(ULONG id) {
		if (id == I_DRIVER) 	return (void *)this;
