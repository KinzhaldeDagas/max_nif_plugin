...
public:
	...
	virtual void UpdateCATRig()
	{
		if (root != nullptr) {
			ICATParentTrans* trans = root->GetCATParentTrans();
			if (trans != nullptr) {
				trans->UpdateCharacter();
			}
			else {
				DbgAssert(0); // Should always have a good parent trans.
			}
		}
	};
...
static void SliderButtonUp(BOOL accept, const TCHAR* undo_msg) {
	if (accept)	theHold.Accept(undo_msg);
	else		theHold.Cancel();
}
...
public:
	...
	BOOL	RightClick(IObjParam *ip, ViewExp *vpt) override { UNREFERENCED_PARAMETER(ip); UNREFERENCED_PARAMETER(vpt); return TRUE; }
	...
	void ExitMode(IObjParam* ip) override
	{
		...
		rigNodes.SetCount(0, TRUE);
	}

	// the user is moving the mouse over the window
	BOOL HitTest(IObjParam* ip, HWND hWnd, ViewExp*, IPoint2 m, int) override
	{
	...
	{
		if (!pick_filter.Filter(node)) {
			return FALSE; // Make sure the thing we are picking can be a geometry.
		}

		// Filter out our rig nodes
		auto iter = std::find(rigNodes.begin(), rigNodes.end(), node);
		if(iter != rigNodes.end()) {
			return FALSE;
		}

		// Make sure that from our rig nodes we cannot reach the target to create a loop.
		node->BeginDependencyTest();
		for (INode* rigNode : rigNodes) {
			rigNode->NotifyDependents(FOREVER, 1, REFMSG_TEST_DEPENDENCY);
	...
	BOOL Pick(IObjParam*, ViewExp* vpt) override
	{
	...
	void Setup(CATHierarchyRoot *root, TCHAR* msg, ICustButton* pnb, PICKMODES pickid)
	{
		...
		// Grab the nodes to dependency check against.
		rigNodes.SetCount(0, TRUE);
		ICATParentTrans* catparenttrans = root->GetCATParentTrans();
		if (catparenttrans != nullptr) {
			catparenttrans->AddSystemNodes(rigNodes, kSNCGatherSystemNodes); // KZ, use kSNCGatherSystemNodes so that we don't close the UI and things.
		}
	}
};
...
{
	GetCOREInterface()->UnRegisterTimeChangeCallback(this);
	GetCOREInterface()->ClearPickMode(); // KZ, clear the pick mode because it is holding a pointer to a UI control.
...
void CATWindowGraph::GetGraphKey()
{
	...
	Control* _ctrlTime = NULL;
	Control* _ctrlValue = NULL;
	Control* _ctrlTangent = NULL;
	Control* _ctrlInTanLen = NULL;
	Control* _ctrlOutTanLen = NULL;

	Interval iv;

	if (ctrlBranch->GetNumLayers() > 0)
		ctrlBranch->GetGraphKey(iKeyNum,
			&_ctrlTime, fTimeVal, fPrevKeyTime, fNextKeyTime,
			&_ctrlValue, fValueVal, minVal, maxVal,
			&_ctrlTangent, fTangentVal,
			&_ctrlInTanLen, fInTanLenVal,
			&_ctrlOutTanLen, fOutTanLenVal,
			&ctrlSlider);
	...
	//	CATGraphKey *key = ctrlBranch->GetCATGraphKey(iKeyNum);
		...
		// Time
	//	if(key->ctrlTime){
	ReplaceReference(TIME, _ctrlTime);
	if (ctrlTime) {
		iv = FOREVER;
	...
	// Value
//	if(key->ctrlValue){
	ReplaceReference(VALUE, _ctrlValue);
	if (ctrlValue) {
		iv = FOREVER;
	...
	// Tangent
//	if(key->ctrlTangent){
	ReplaceReference(TANGENT, _ctrlTangent);
	if (ctrlTangent) {
		iv = FOREVER;
	...
	// InTanLen
//	if(key->ctrlInTanLen){
	ReplaceReference(IN_TAN_LEN, _ctrlInTanLen);
	if (ctrlInTanLen) {
		iv = FOREVER;
...
//	if(key->ctrlOutTanLen){
	ReplaceReference(OUT_TAN_LEN, _ctrlOutTanLen);
	if (ctrlOutTanLen) {
		iv = FOREVER;
...
void CATWindowGraph::SetModeP3(int ShowP3)
{
	...
	*/
	TimeValue t = GetCOREInterface()->GetTime();
	Control* _ctrlX = nullptr;
	Control* _ctrlY = nullptr;
	Control* _ctrlZ = nullptr;
	ctrlBranch->GetP3Ctrls(_ctrlX, _ctrlY, _ctrlZ);

	Interval iv;
	float spnVal;
	// X
	ReplaceReference(CTRL_X, _ctrlX);
	if (ctrlX) {
		iv = FOREVER;
	...
	// Y
	ReplaceReference(CTRL_Y, _ctrlY);
	if (ctrlY) {
		iv = FOREVER;
	...
	// Z
	ReplaceReference(CTRL_Z, _ctrlZ);
	if (ctrlZ) {
		iv = FOREVER;
...
void CATWindowGraph::CopyGraph()
{
	...
	int iTargetValue = std::max(ctrlBranch->GetActiveLeaf(), 0);
...
INT_PTR CALLBACK CATWindowGraph::DialogProc(HWND, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
		...
		case BN_CLICKED:
			...
			case IDC_BTN_TRACKVIEW:
			{
				...
				if (root->pCATTreeView) {
					...
					pTrackViewNode->AddController(ctrlBranch, ctrlBranch->GetBranchName().data(), CATPARENT_CLASS_ID);
					root->pCATTreeView->SetRootTrack(pTrackViewNode);
					root->pCATTreeView->ExpandTracks();
	...
	case WM_MOUSEMOVE: {
		...
		if ((fwKeys == MK_LBUTTON) && bDragingKey)
		{
			...
			float alpha = heightMin - beta * minY;
			yPos = (((dGraphHeight - yPos) - alpha) / beta);
			yPos = std::min(std::max(minVal, yPos), maxVal);
			...
			if (bkey) {
				...
				if (ctrlTime)
				{
					xPos = std::min(fNextKeyTime - 3.0f, std::max(fPrevKeyTime + 3.0f, xPos));
			...
			else if (bInTan)
			{
				...
				if (ctrlInTanLen)
				{
					tanLen = (float)std::max(std::min(tanLen, 1.0f), 0.0f);
...
HWND CreateCATWindow(HINSTANCE hInstance, HWND hWnd, CATHierarchyRoot *catHierarchy/*=NULL*/) {
	...
	if (!hDlg) {// the window wasnt created, display an error
		...
		// ...
		// Display the string.
		MaxSDK::MaxMessageBox(NULL, (LPCTSTR)lpMsgBuf, GetString(IDS_ERROR), MB_OK | MB_ICONINFORMATION);
		// Free the buffer.
		LocalFree(lpMsgBuf);
	}
...
int CATWindow::BuildNavigatorList(NavigatorData* data, int nItem/*=0*/)
{
	...
	static constexpr const TCHAR *strPrefix[3] = {
		_T("    "),		// not expandable
		_T("[  ]"),		// expanded
		_T("[+]")		// collapsed
	};
	...
	if (branch->bExpanded) {
		for (int i = 0; i < branch->GetNumBranches(); i++) {
			NavigatorData *childData = new NavigatorData((CATHierarchyBranch*)branch->GetBranch(i), branch->GetBranchName(i).data(), data->level + 1);
