...
public:
	...
	const TCHAR*	ClassName() { return _T("LayerTransform"); }
	const TCHAR*	NonLocalizedClassName() { return _T("LayerTransform"); }
...
public:
	...
	INT_PTR CALLBACK DlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		...
		{
			bool arePtrsValid = ctrlLayerTransform && spnStartTime && spnEndTime;
			if (arePtrsValid)
			{
				Interval range = ctrlLayerTransform->layerinfo->GetLoopRange();
				switch (LOWORD(wParam))
				{
				case IDC_SPIN_START_TIME:
					range.SetStart((TimeValue)(spnStartTime->GetFVal() * (float)GetTicksPerFrame()));
					break;
				case IDC_SPIN_END_TIME:
					range.SetEnd((TimeValue)(spnEndTime->GetFVal() * (float)GetTicksPerFrame()));
					break;
				}
...
INode* LayerTransform::Initialise(NLAInfo* layerinfo, Control *prs)
{
	...
	node->SetName((layerinfo->GetName() + GetString(IDS_TRANSFORM)).data());
	node->SetTMController(this);
...
int LayerTransform::Display(TimeValue t, INode*, ViewExp *vpt, int flags)
{
	if (layerinfo) {
		if (layerinfo->TestFlag(LAYER_DISPLAY_GHOST)) {
			...
			int solo = root->GetSoloLayer();
			CATMode catmode = catparenttrans->GetCATMode();
			// KZ 11/2022, skip UI update, nothing changed and it causes flickering
			catparenttrans->SetCATMode(NORMAL, true/*skip UI update*/);
			root->SoloLayer(layerinfo->GetIndex());
			...
			root->SoloLayer(solo);
			// KZ 11/2022, skip UI update, nothing changed and it causes flickering
			catparenttrans->SetCATMode(catmode, true/*skip UI update*/);
			catparenttrans->CATMessage(t, CAT_INVALIDATE_TM);
		}
	}
	else
