...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATLimbData2"); }
...
public:
	...
	void InitControls(HWND hWnd)
	{
		...
		if (limb && limb->GetCATParentTrans()  && limb->GetCATParentTrans()->GetCATMode() != SETUPMODE)
		{
			btnCopy->Disable();
			btnPaste->Disable();
			btnPasteMirrored->Disable();
		}
		else if (limb && !limb->CanPasteControl())
		{
			btnPaste->Disable();
			btnPasteMirrored->Disable();
		}
		...
		if (limb && !limb->GetisLeg())
			SET_TEXT(hWnd, IDC_CHK_PALM_ANKLE, catCfg.Get(_T("ButtonTexts"), _T("chkPalm"), GetString(IDS_PALM)));
		else					SET_TEXT(hWnd, IDC_CHK_PALM_ANKLE, catCfg.Get(_T("ButtonTexts"), _T("chkAnkle"), GetString(IDS_ANKLE)));

		if (limb && limb->GetPalmTrans())
			SET_CHECKED(hWnd, IDC_CHK_PALM_ANKLE, TRUE);
		if (limb && limb->GetCollarbone())
			SET_CHECKED(hWnd, IDC_CHK_COLLARBONE, TRUE);
	...
	virtual INT_PTR DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM)
	{
		...
		case CC_SPINNER_CHANGE:
			if (limb != nullptr)
			{
				...
				{ // Switch on ID
				case IDC_SPIN_NUMBONES:
					limb->SetNumBones(spnNumBones->GetIVal(), FALSE);
					break;
#ifdef SOFTIK
				case IDC_SPIN_IKSOFTLIMIT_MIN:
					limb->SetIKSoftLimitMin(spnIKSoftLimitMin->GetFVal());
					break;
				case IDC_SPIN_IKSOFTLIMIT_MAX:
					limb->SetIKSoftLimitMax(spnIKSoftLimitMax->GetFVal());
					break;
#endif
				}
...
static void LimbBoneCloneNotify(void *param, NotifyInfo *info)
{
	...
	INode* node = GetNotifyParam<NOTIFY_NODE_CLONED>(info);
...
RefTargetHandle LimbData2::Clone(RemapDir& remap)
{
	...
	// If pNewHub was nullptr, then we have cloning order issue. We should be cloning the Hub with the limb controllers.
...
void SetLimbPointer(INode* pANode, LimbData2* pSetLimb)
{
	...
	if (pMtxCtrl != NULL)
	{
		...
		{
			DbgAssert(FALSE && "FALSE: What is this?");
		}
	}
}
...
TSTR LimbData2::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case SUB_WEIGHTS: return localized ? GetString(IDS_WEIGHTS) : _T("Weights");
	case SUB_LAYERIKFKRATIO: return localized ? GetString(IDS_LYR_IKFKRATIO) : _T("layerIKFKRatio");
	case SUB_LAYERLIMBIKPOS: return localized ? GetString(IDS_LYR_LIMBIKPOS) : _T("layerLimbIKPos");
	case SUB_LAYERFORCEFEEDBACK: return localized ? GetString(IDS_LYR_RETARG) : _T("layerRetargeting");
	case SUB_LAYERIKTRANS: return localized ? GetString(IDS_LYR_IKTARGTRANS) : _T("layerIKTargetTrans");
	default: return _T("");
	}
}
...
float LimbData2::GetIKFKRatio(TimeValue t, Interval& valid, int boneid/*=-1*/)
{
	...
	ikfkratio = std::min(1.0f, std::max(ikfkratio, 0.0f));
...
float LimbData2::GetForceFeedback(TimeValue t, Interval& valid) {
	...
	ffeedback = std::min(1.0f, std::max(ffeedback, 0.0f));
	return ffeedback;
}
...
float LimbData2::GetLimbIKPos(TimeValue t, Interval &valid) {
	...
	// limit low
	ikposratio = std::max(ikposratio, 0.0f);
	// Limit high
	if (GetPalmTrans() != NULL)
		ikposratio = MaxSDK::min(ikposratio, GetNumBones() + 1);
	else
		ikposratio = MaxSDK::min(ikposratio, GetNumBones());
	return ikposratio;
}
...
Matrix3 LimbData2::GettmFKTarget(TimeValue t, Interval &valid, const Matrix3* tmIKChainParent, Matrix3& tmFKPalmLocal)
{
	...
	for (int i = 0; i < lastIKbone - 2; i++)
	{
		...
		double fk_long_child = std::min(child_lengths, this_fk_bone_to_fk_target_length + bone_length);
...
void LimbData2::GetIKTMs(TimeValue t, Interval &valid, const Point3& p3LimbRoot, const Matrix3& tmFKParent, Matrix3 &IKTarget, Matrix3 &FKTarget, int boneid/*=0*/)
{
	...
	if (!bIKValid) {
		...
		if (dIKFKRatio > 0)
		{
			...
			float angle = acos(std::min(DotProd(p3ToIKPos, p3ToFKPos), 1.0f));
			angle *= dIKFKRatio;
...
//		bool iAngleDet = DotProd(p3Axis, tmIkBase.GetRow(X)) > 0;
		...
		////bool bIKAxisDet =
		////float dAngleToFK = -acos(std::min(1.0f, DotProd(p3BoneFKPos, p3ToChild)));
		...
		case rigAssignment:
			...
			case idBoneName:
				...
				if (load->GetVersion() < CAT_VERSION_1720) {
					...
					pblock->SetValue(PB_NAME, 0, (side + name).data());
				}
				break;
			case idBoneColour:
			{
...
Ray LimbData2::GetReachVec(TimeValue t, Interval& valid, const Matrix3& tmHub, const Point3& p3HubScale, float &weight)
{
	...
	if (TestFlag(LIMBFLAG_RAMP_REACH_BY_SOFT_LIMITS)) {
		...
		float reachramp = std::min(1.0f, std::max((curr_limblength - iksoftlimit_min) / (iksoftlimit_max - iksoftlimit_min), 0.0f));
...
BOOL LimbData2::RemovePalmAnkle()
{
	...
	Interface::SuspendSceneRedrawGuard suspendSceneRedrawGuard;
	SuspendAll suspendRefMessages(false, false, false, false, false, false, true);
