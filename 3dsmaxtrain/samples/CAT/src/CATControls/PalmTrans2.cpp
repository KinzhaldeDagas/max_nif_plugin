...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("PalmTrans2"); }
...
public:
	...
	void LoadHandPose(const TCHAR* HandPoseFilePath)
	{
		...
		MaxLocaleHandler lh(LC_NUMERIC, _T("C"));
	...
	void ApplyHandPose(float weight)
	{
		...
		int numDigits = std::min(numPalmDigits, numFileDigits);
		...
		for (int i = 0; i < numDigits; i++)
		{
			...
			if (digit) {
				...
				numSegs = std::min(numSegs, numFileSegs);
	...
	void RefreshAvailableHandPoses()
	{
		...
		// in a vector.
		_stprintf_s(searchName, _countof(searchName), _T("%s\\*.%s"), strCurrentFolder, CAT_HANDPOSE_EXT);
		hFind = FindFirstFile(searchName, &find);
		if (hFind == INVALID_HANDLE_VALUE) return;
		...
		do {
			if (find.dwFileAttributes != FILE_ATTRIBUTE_DIRECTORY) {
				...
				fileName[_tcsnlen(fileName, _countof(fileName)) - _tcsnlen(CAT_HANDPOSE_EXT, _countof(CAT_HANDPOSE_EXT)) - 1] = _T('\0');

				// We need to save the full path, so allocate some
				// memory.  If this fails, don't even bother adding
				// it to the list box.
				size_t fileBuflen = _tcsnlen(strCurrentFolder, _countof(strCurrentFolder)) + _tcsnlen(find.cFileName, _countof(find.cFileName)) + 8;
				TCHAR *file = new TCHAR[fileBuflen];
				size_t presetBufLen = _tcsnlen(fileName, _countof(fileName)) + 4;
				TCHAR *preset = new TCHAR[presetBufLen];

				if (file && preset) {
					_stprintf_s(file, fileBuflen, _T("%s\\%s"), strCurrentFolder, find.cFileName);
					_tcscpy_s(preset, presetBufLen, fileName);
		...
		case CC_SPINNER_CHANGE: {
			if (palm != nullptr)
			{
				...
				if (palm->GetCATParentTrans()->GetLengthAxis() == Z)
				{
					...
					case IDC_SPIN_WEIGHT:
						if (handposeloaded)
							ApplyHandPose(fSpinnerVal / 100.0f);
						break;
					case IDC_SPIN_SPREAD:
						ModifyDigits(AngAxis(Point3(0.0f, 1.0f, 0.0f), -DegToRad(fSpinnerVal)), TRUE, TRUE);
						break;
					case IDC_SPIN_CURL:
						ModifyDigits(AngAxis(Point3(1.0f, 0.0f, 0.0f), -DegToRad(fSpinnerVal)), FALSE);
						break;
					case IDC_SPIN_BEND:
						ModifyDigits(AngAxis(Point3(1.0f, 0.0f, 0.0f), -DegToRad(fSpinnerVal)), TRUE);
						break;
					case IDC_SPIN_ROLL:
						ModifyDigits(AngAxis(Point3(0.0f, 0.0f, 1.0f), -DegToRad(fSpinnerVal)), TRUE, TRUE);
						break;
					}
				}
				else
				{
					...
					case IDC_SPIN_SPREAD:
						...
						break;
					case IDC_SPIN_CURL:
						ModifyDigits(AngAxis(Point3(0.0f, 0.0f, -1.0f), -DegToRad(fSpinnerVal)), FALSE);
						break;
					case IDC_SPIN_BEND:
						ModifyDigits(AngAxis(Point3(0.0f, 0.0f, -1.0f), -DegToRad(fSpinnerVal)), TRUE);
						break;
					case IDC_SPIN_ROLL:
						ModifyDigits(AngAxis(Point3(-1.0f, 0.0f, 0.0f), -DegToRad(fSpinnerVal)), TRUE, TRUE);
						break;
					}
				}
...
public:
	...
	INT_PTR CALLBACK DlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_CUSTEDIT_ENTER:
			...
			{
			case IDC_EDIT_NAME: {
				if (edtName != nullptr && palm != nullptr)
				{
					TCHAR strbuf[128];
...
static ParamBlockDesc2 PalmTrans2_t_param_blk(PalmTrans2::pb_idParams, _T("Palm Params"), 0, &PalmTrans2Desc,
	...
	PalmTrans2::PB_LAYERTARGETALIGN, _T(""), TYPE_FLOAT, P_ANIMATABLE + P_RESET_DEFAULT, IDS_LAYERTARGETALIGN,
		...
		p_nonLocalizedName, _T("LayerTargetAlign"),
		p_end,
	...
	PalmTrans2::PB_CATDIGITBEND, _T(""), TYPE_POINT3, P_ANIMATABLE, IDS_CATDIGITBEND,
		p_nonLocalizedName, _T("CATDigitBend"),
		p_end,
...
void PalmTrans2::GetIKValue(TimeValue t, const Point3& p3PalmTarget, const Matrix3& tmFKWorld, Matrix3& tmWorld, Point3& /*p3LocalScale*/, Interval& ivLocalValid)
{
	...
	float boneAngle = (float)acos(std::min(DotProd(p3ZVect, Normalize(tmIKWorld.GetRow(lengthaxis))), 1.0f));
...
float PalmTrans2::GetExtensionLength(TimeValue t, const Matrix3& tmPalmFKLocal)
{
	...
	if (dTargetAlign < 1.0f)
	{
		float palmlength = GetBoneLength();
		int lengthaxis = GetLengthAxis();
		float localAngle = (float)acos(std::min(1.f, std::max(-1.f, tmPalmFKLocal.GetRow(lengthaxis)[lengthaxis])));
...
TSTR PalmTrans2::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case SUBLIMB: return localized ? GetString(IDS_LIMBDATA) : _T("LimbData");
	case SUBROTATION: return localized ? GetString(IDS_LAYERTRANS) : _T("LayerTrans");
	case SUBTARGETALIGN: return localized ? GetString(IDS_LAYERTARGETALIGN) : _T("LayerTargetAlign");
	default: return _T("");
	}
}
...
RefResult PalmTrans2::NotifyRefChanged(const Interval&, RefTargetHandle hTarg, PartID& partID, RefMessage msg, BOOL)
{
	...
	case REFMSG_CHANGE:
	{
		...
		// Added the check for PART_OBJ to fix issue
...
TSTR PalmTrans2::GetRigName()
{
	...
	{
		MSTR name = catName + limbName + GetName();
...
float PalmTrans2::GetTargetAlign(TimeValue t, Interval& ivValid)
{
	...
	return std::min(1.0f, std::max(val, 0.0f));;
}
...
{
	LimbData2* pLimb = GetLimb();
	return (pLimb != NULL) ? pLimb->GetGroupColour() : Color{};
}
