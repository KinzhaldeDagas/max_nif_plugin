...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATHierarchyBranch"); }
	...
	else if (ctrl->NumSubs() > 0)
	{
		for (int i = 0; i < ctrl->NumSubs(); i++)
		{
			if (ctrl->SubAnim(i) && ctrl->SubAnim(i)->SuperClassID() == CTRL_FLOAT_CLASS_ID)
			{
				...
				TSTR name = ctrl->SubAnimName(i, true);
				if (catmotionlimb)
				{
					ctrlBranch = (CATHierarchyBranch2*)attributeBranch->AddBranch(ctrl->SubAnimName(i, true));
					ctrlBranch->SetSubAnimIndex(i);
				...
				{
					ctrlLeaf = (CATHierarchyLeaf*)attributeBranch->AddLeaf(ctrl->SubAnimName(i, true));
					ctrl->AssignController((Control*)ctrlLeaf, i);
				}
			}
		}
...
void CATHierarchyBranch2::GetP3Ctrls(Control*& ctrlX, Control*& ctrlY, Control*& ctrlZ)
{
	...
	if (GetNumBranches() == 3)
	{
		ctrlX = dynamic_cast<Control*>(GetBranch(0));
		ctrlY = dynamic_cast<Control*>(GetBranch(1));
		ctrlZ = dynamic_cast<Control*>(GetBranch(2));
	}
	// Hub offset
	else if (GetNumLeaves() == 3)
	{
		ctrlX = dynamic_cast<Control*>(GetLeaf(0));
		ctrlY = dynamic_cast<Control*>(GetLeaf(1));
		ctrlZ = dynamic_cast<Control*>(GetLeaf(2));
	}
}
...
void CATHierarchyBranch2::GetYRange(TimeValue t, float &minY, float &maxY)
{
	...
	for (int i = 0; i < GetNumControllerRefs(); i++)
	{
		...
		for (int j(1); j <= numKeys; ++j)
		{
			...
			{
				computeControlKeys(key1, key2, outTan, inTan);
				minY = std::min(minY, (std::min(outTan.y, inTan.y)));
				maxY = std::max(maxY, (std::max(outTan.y, inTan.y)));
			}
			else
			{
				minY = std::min(minY, (std::min(key1.y, key2.y)));
				maxY = std::max(maxY, (std::max(key1.y, key2.y)));
			}
			key1 = key2;
			isOutTan1 = isOutTan2;
			isInTan1 = isInTan2;
...
BOOL CATHierarchyBranch2::SavePreset(CATPresetWriter *save, TimeValue t)
{
	...
	for (i = 0; i < GetNumLeaves(); i++)
		save->FromController(GetLeaf(i), t, SubAnimName(i, true));
...
TSTR CATHierarchyBranch2::SubAnimName(int i, bool localized)
{
	if (g_bSavingCAT3Rig)
		GetString(IDS_SAVINGRIG);
	int nNumLeaves = GetNumLeaves();
	if (i < nNumLeaves)
		return GetLeafName(i);
	else if ((i - nNumLeaves) < GetNumBranches())
		return GetBranchName(i - nNumLeaves);
...
RefTargetHandle CATHierarchyBranch2::Clone(RemapDir& remap)
{
	...
	newCATHierarchyBranch->ReplaceReference(PBLOCK_REF, clonedpblock);

	// PB_BRANCHTAB were cloned, since we own them setup their parent branches
	for (int i = 0; i < GetNumBranches(); i++)
	{
		CATHierarchyBranch* newBranch = newCATHierarchyBranch->GetBranch(i);
		if (newBranch != nullptr) {
			newBranch->SetBranchParent(newCATHierarchyBranch);
		}
	}

	// Clone manually the P_NO_REF's that we own, the rest reconnect in post patch if cloned by their owners.
	//PB_ROOT, was originally ommited, handled on post patch
	//PB_BRANCHPARENT, was originally ommited, handled on post patch
	//PB_BRANCHOWNER, was originally ommited, handled on post patch
	//PB_LEAFTAB
	newCATHierarchyBranch->pblock->SetCount(PB_LEAFTAB, GetNumLeaves());
	for (int i = 0; i < GetNumLeaves(); i++)
	{
		CATHierarchyLeaf* leaf = GetLeaf(i);
		CATHierarchyLeaf* newleaf = (CATHierarchyLeaf*)remap.CloneRef(leaf);
		newCATHierarchyBranch->SetLeaf(i, newleaf);
	}
	//PB_CONTROLLERTAB
	newCATHierarchyBranch->pblock->SetCount(PB_CONTROLLERTAB, GetNumControllerRefs());
	for (int i = 0; i < GetNumControllerRefs(); i++)
	{
		Control* con = GetControllerRef(i);
		Control* newCon = (Control*)remap.CloneRef(con);
		newCATHierarchyBranch->SetControllerRef(i, newCon);
	}
	//PB_LIMBS_TAB, was originally ommited, handled in post patch.
