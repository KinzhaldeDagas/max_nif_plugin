...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("DigitData"); }
...
void DigitData::Initialise(PalmTrans2* palm, int id, BOOL loading)
{
	...
	if (!loading) {
		...
		pblock->SetValue(PB_DIGITNAME, 0, (DigitName + DigitID).data());
	}
}
...
void DigitData::PoseLikeMe(TimeValue t, int nCATMode, DigitData* digit)
{
	...
	int numBones = std::min(GetNumBones(), digit->GetNumBones());
	Matrix3 tmBoneRot(1);
