...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATSpineTrans2"); }
...
class SpineTrans2ParamAccessor : public PBAccessor
{
public:
	void Get(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t, Interval& valid)
	{
		...
		UNUSED_PARAM(valid);
		UNUSED_PARAM(t);
		if (id == SpineTrans2::PB_SPINEDATA_REFTARG)
		{
			SpineTrans2* pSpineTrans2 = dynamic_cast<SpineTrans2*>(owner);
			if (pSpineTrans2 && pSpineTrans2->pblock)
				v.control = pSpineTrans2->pblock->GetControllerByID(SpineTrans2::PB_SPINEDATA, 0);
		}
		else
			DbgAssert(false);
	}
	void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)
	{
		...
		if (id == SpineTrans2::PB_SPINEDATA_REFTARG)
		{
			// TODO: validate v.r is null or SpineData2
			if (!theHold.RestoreOrRedoing() && (v.control == nullptr || dynamic_cast<SpineData2*>(v.control) != nullptr))
			{
				SpineTrans2* pSpineTrans2 = dynamic_cast<SpineTrans2*>(owner);
				if (pSpineTrans2 && pSpineTrans2->pblock)
					pSpineTrans2->pblock->SetControllerByID(SpineTrans2::PB_SPINEDATA, 0, v.control);
			}
		}
		else
			DbgAssert(false);
...
static SpineTrans2ParamAccessor pbSpineTrans2Acessor;
...
static ParamBlockDesc2 SpineTrans2_param_blk(SpineTrans2PBlock_params, _T("SpineLink params"), 0, &SpineTrans2Desc,
	...
	// PB_SPINEDATA holds a SpineTrans2 controller reference but is a TYPE_FLOAT (bad!). Fixing by keeping this the
	// holder SpineTrans2 but redirecting access to go through PB_SPINEDATA_REFTARG which is correctly a TYPE_REFTARG.
	SpineTrans2::PB_SPINEDATA, _T(""), TYPE_FLOAT, P_ANIMATABLE, NULL,
		p_end,
	...
	SpineTrans2::PB_POSWT, _T(""), TYPE_FLOAT, 0, 0,
		p_end,
	SpineTrans2::PB_SPINEDATA_REFTARG, _T("SpineData"), TYPE_REFTARG, P_SUBANIM | P_NO_REF, IDS_CL_SPINEDATA2,
		p_nonLocalizedName, _T("CATSpineData2"),
		p_accessor, &pbSpineTrans2Acessor,
		p_end,
	p_end
);
...
TSTR SpineTrans2::SubAnimName(int i, bool localized)
{
	switch (i)
	{
...
TSTR SpineTrans2::GetRigName()
{
	...
	TSTR spineName = _T("<<unknown>>");
	TSTR catName = _T("<<unknown>>");
	TSTR boneName = GetName();
	TSTR retName = spineName + boneName;
	if (nullptr != pSpine)
	{
		spineName = pSpine->GetName();				
		...
		else
		{
			...
			if (nNumLinks > 1 && boneName.isNull())
			{
				boneName.printf(_T("%d"), (GetBoneID() + 1));
			}
			retName = spineName + boneName;
		}
	}
	...
	{
		MSTR name = catName + retName;
		LogErrMsgOnPrompt(name);
	}	

	return catName + retName;
}
...
void* SpineTrans2::GetInterface(ULONG id) {
	if (id == I_DRIVER)			return (ReferenceTarget*)GetSpine()->GetTipHub();
	return CATNodeControl::GetInterface(id);
}

SpineData2* SpineTrans2::GetSpine()
{
	return (SpineData2*)pblock->GetReferenceTarget(PB_SPINEDATA_REFTARG);
}

const SpineData2* SpineTrans2::GetSpine() const
{
	return (const SpineData2*)pblock->GetReferenceTarget(PB_SPINEDATA_REFTARG);
}

void SpineTrans2::SetSpine(SpineData2* sp)
{
	pblock->SetValue(PB_SPINEDATA_REFTARG, 0, sp);
}
