...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATCollarBone"); }
...
TSTR CollarBoneTrans::SubAnimName(int i, bool localized)
{
	//	return GetString(IDS_layerTrans);
	switch (i)
	{
	case SUBROTATION: return localized ? GetString(IDS_LAYERTRANS) : _T("LayerTrans");
	case SUBSCALE: return localized ? GetString(IDS_SCALE) : _T("Scale");
	default: return _T("");
	}
}
