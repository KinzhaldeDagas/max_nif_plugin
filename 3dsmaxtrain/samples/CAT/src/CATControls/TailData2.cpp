...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("TailData"); }
...
public:
	...
	void InitControls(HWND hDlg, IParamMap2 *map)
	{
		...
		ICATParentTrans* pCATTransParent = tail->GetCATParentTrans();
		if (nullptr != pCATTransParent && (pCATTransParent->GetCATMode() != SETUPMODE))
		{
			btnCopy->Disable();
			btnPaste->Disable();
	...
	virtual INT_PTR DlgProc(TimeValue, IParamMap2 *map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM)
	{
		...
		case CC_SPINNER_CHANGE:
			...
			case IDC_SPIN_TAIL_LENGTH:
				bool arePtrsValid = tail && spnLength;
				if (arePtrsValid)
				{
					if (theHold.Holding())
					{
...
TSTR TailData2::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case PBLOCK_REF: return localized ? GetString(IDS_PARAMS) : _T("Parameters");
	case WEIGHTS: return localized ? GetString(IDS_WEIGHTS) : _T("Weights");
	default: return _T("");
	}
}
...
static void TailBoneCloneNotify(void *param, NotifyInfo *info)
{
	...
	{
		Hub *hub = tail->GetHub();
		if ((hub != nullptr) && hub->GetTail(tail->GetTailID()) != tail)
		{
