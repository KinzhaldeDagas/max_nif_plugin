...
void CATNodeControl::SetNodeTM(TimeValue t, const Matrix3& tmCurrent)
{
	INode* pNode = GetNode();
	DbgAssert(pNode != NULL);
	if (pNode != NULL)
	...
	for (int i = 0; i < tabArbBones.Count(); i++) {
		...
		if (pArbBone->GetBoneID() != i)
		{
			if (!GetCOREInterface()->GetQuietMode())
			{
				...
				MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), message + errordetails /*+ boneaddress*/ + bonename + summary, GetString(IDS_ERR_FILELOAD), MB_OK);
			}
...
TSTR CATNodeControl::GetRigName() {	
	...
	DbgAssert(pCATParentTrans);
	if (nullptr != pCATParentTrans)
		catName = pCATParentTrans->GetCATName();
	else
	{
		MSTR name = catName + GetName();
		LogErrMsgOnPrompt(name);
	}
	return catName + GetName();
};
...
BOOL CATNodeControl::PasteLayer(CATControl* pPasteCtrl, int fromindex, int toindex, DWORD flags, RemapDir &remap)
{
	...
	if (res)
	{
		...
		int nArbBones = std::min(GetNumArbBones(), pPasteNodeCtrl->GetNumArbBones());
...
BOOL CATControl::PasteERNNodes(CATControl* pastectrl, CATCharacterRemap &remap)
{
	...
	int numchildcontrols = std::min(NumChildCATControls(), pastectrl->NumChildCATControls());
...
void AddControllerHierarchy(ReferenceTarget* from, ReferenceTarget* to, RemapDir &remap) {
	...
	for (int i = 0; i < std::min(from->NumRefs(), to->NumRefs()); i++) {
...
BOOL CATControl::BuildMapping(CATControl* pastectrl, CATCharacterRemap &remap, BOOL includeERNnodes)
{
	...
	int i;
	// Build a mapping o
	int numlayercontrollers = std::min(NumLayerControllers(), pastectrl->NumLayerControllers());
...
void CATNodeControl::SetValue(TimeValue t, void *val, int commit, GetSetMethod method)
{
	...
	case XFORM_MOVE:
	{
		if (TestCCFlag(CNCFLAG_IMPOSE_POS_LIMITS)) {
			...
			float new_x = std::min(std::max(pos_total.x, pos_limits_neg.x), pos_limits_pos.x);
			float new_y = std::min(std::max(pos_total.y, pos_limits_neg.y), pos_limits_pos.y);
			float new_z = std::min(std::max(pos_total.z, pos_limits_neg.z), pos_limits_pos.z);
	...
	// here we figure out how much our children will be translated, and if any of them are pinned,
	// we calculate the translation that we need to make to keep our locked children in their
	...
	case XFORM_ROTATE:
	{
		...
		if (ccflags & (CNCFLAG_IMPOSE_ROT_LIMITS)) {
			...
			float ang_total[3];			
			Quat quat;
			double sinv = std::sin(ptr->aa.angle / 2.0), cosv = std::cos(ptr->aa.angle / 2.0);
			quat.x = ptr->aa.axis[X] * sinv;
			quat.y = ptr->aa.axis[Y] * sinv;
			quat.z = ptr->aa.axis[Z] * sinv;
			quat.w = cosv;
			
			float ang_temp[3];			
			QuatToEuler(quat, ang_temp, EULERTYPE_XYZ);
			ang_total[X] = ang_initial[X] + (ang_temp[X]);
			ang_total[Y] = ang_initial[Y] + (ang_temp[Y]);
			ang_total[Z] = ang_initial[Z] + (ang_temp[Z]);

			// these are the limits of rotation values that can be applied to the bone based on user settings 
			// on "Joint Limits" and the current bone rotation:
			float restricted_x_angle = DegToRad(std::min(std::max(RadToDeg(ang_total[X]), rot_limits_neg.x), rot_limits_pos.x));
			float restricted_y_angle = DegToRad(std::min(std::max(RadToDeg(ang_total[Y]), rot_limits_neg.y), rot_limits_pos.y));
			float restricted_z_angle = DegToRad(std::min(std::max(RadToDeg(ang_total[Z]), rot_limits_neg.z), rot_limits_pos.z));

			// apply the values only if the new values are really different than the current rotation values 
			// (ang_initial[]):
...
void CATNodeControl::CalcPositionStretch(TimeValue t, Matrix3& tmCurrent, Point3 p3Pivot, const Point3& p3Initial, const Point3& p3Final)
{
	...
	p3Axis.Unify();
	float dAngle = acos(std::min(1.0f, DotProd(p3ToInitial, p3ToFinal)));
	Matrix3 tmRot;
...
void CATNodeControl::ImposeLimits(TimeValue /*t*/, const Matrix3 &tmBoneParent, Matrix3 &tmBoneWorld, Point3& p3ScaleLocal, Interval& /*ivValid*/)
{
	...
	// this will remove scaling data:
	AffineParts parts;
	decomp_affine(tmBoneLocal, &parts);

	if (ccflags&CNCFLAG_IMPOSE_POS_LIMITS)
	{
		parts.t.x = std::min(std::max(pos_limits_neg.x, parts.t.x), pos_limits_pos.x);
		parts.t.y = std::min(std::max(pos_limits_neg.y, parts.t.y), pos_limits_pos.y);
		parts.t.z = std::min(std::max(pos_limits_neg.z, parts.t.z), pos_limits_pos.z);
	}
	...
	if (ccflags & CNCFLAG_IMPOSE_ROT_LIMITS)
	{
		...
		// once you turn the "Joint Limits"'s Rotation checkbox ON, we enter this function to respect the current 
		// values in the UI:
		ang[0] = std::min(DegToRad(rot_limits_pos.x), std::max(ang[0], DegToRad(rot_limits_neg.x)));
		ang[1] = std::min(DegToRad(rot_limits_pos.y), std::max(ang[1], DegToRad(rot_limits_neg.y)));
		ang[2] = std::min(DegToRad(rot_limits_pos.z), std::max(ang[2], DegToRad(rot_limits_neg.z)));
	...
	{
		p3ScaleLocal.x = std::min(std::max(scl_limits_min.x, p3ScaleLocal.x), scl_limits_max.x);
		p3ScaleLocal.y = std::min(std::max(scl_limits_min.y, p3ScaleLocal.y), scl_limits_max.y);
		p3ScaleLocal.z = std::min(std::max(scl_limits_min.z, p3ScaleLocal.z), scl_limits_max.z);
	}
...
void CATNodeControl::RefDeleted(RefMakerHandle rm)
{
...
INT_PTR CALLBACK CATNodeControlSetupDlgCallBack::DlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CUSTEDIT_ENTER:
	{
		...
		{
		case IDC_EDIT_NAME: {
			if (edtName != nullptr && bone != nullptr)
			{
				TCHAR strbuf[128];
	...
	case CC_SPINNER_CHANGE: {
		if (bone != nullptr)
		{
			...
			if (theHold.Holding() && !bone->TestAFlag(A_HELD))
				theHold.Put(new EditRestore(bone));
			switch (LOWORD(wParam))
			{
			case IDC_SPIN_X:
				if (bone->TestCCFlag(CNCFLAG_LOCK_SETUPMODE_LOCAL_SCL))
				{
					break;
				}
				bone->SetObjX(spnX->GetFVal());
				break;
			case IDC_SPIN_Y:
				if (bone->TestCCFlag(CNCFLAG_LOCK_SETUPMODE_LOCAL_SCL))
				{
	...
	case CC_COLOR_CLOSE:
	case CC_COLOR_CHANGE:			bone->SetBonesRigColour(Color(swchColour->GetAColor()));		break;
...
void CATNodeControl::BeginEditParams(IObjParam *ip, ULONG flags, Animatable *prev)
{
	...
	if (flags&BEGIN_EDIT_MOTION || flags&BEGIN_EDIT_LINKINFO)
	{
		...
		if (GetLayerTrans())
		{
			GetLayerTrans()->BeginEditParams(ip, flags, prev);
		}
	}
