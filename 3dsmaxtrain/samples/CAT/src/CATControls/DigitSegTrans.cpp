...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATDigitSegTrans"); }
...
// An accessor that passes values directly through to the object.
...
class DigitSegTransParamAccessor : public PBAccessor
{
public:

	void Get(PB2Value& v, ReferenceMaker* owner, ParamID id, int /*tabIndex*/, TimeValue /*t*/, Interval& /*valid*/)
	{
		if (id == DigitSegTrans::PB_BONELENGTH)
		{
		...
		else if (id == DigitSegTrans::PB_DIGITDATA_REFTARG)
		{
			DigitSegTrans* pDigitSeg = dynamic_cast<DigitSegTrans*>(owner);
			if (pDigitSeg && pDigitSeg->pblock)
				v.control = pDigitSeg->pblock->GetControllerByID(DigitSegTrans::PB_DIGITDATA, 0);
		}
		else
			DbgAssert(false);
	...
	void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int /*tabIndex*/, TimeValue /*t*/)
	{
		...
		{
			// TODO: validate v.r is null or DigitData
			if (!theHold.RestoreOrRedoing() && (v.control == nullptr || dynamic_cast<DigitData*>(v.control) != nullptr))
			{
				DigitSegTrans* pDigitSeg = dynamic_cast<DigitSegTrans*>(owner);
				if (pDigitSeg && pDigitSeg->pblock)
					pDigitSeg->pblock->SetControllerByID(DigitSegTrans::PB_DIGITDATA, 0, v.control);
			}
		}
		else
			DbgAssert(false);
...
static DigitSegTransParamAccessor pbDigitAcessor;
...
static ParamBlockDesc2 DigitSegTrans_param_blk(DigitSegTrans::PBLOCK_REF, _T("DigitSegTransParams"), 0, &DigitSegTransDesc,
	...
	// PB_DIGITDATA holds a DigitData controller reference but is a TYPE_FLOAT (bad!). Fixing by keeping this the
	// holder DigitData but redirecting access to go through PB_DIGITDATA_REFTARG which is correctly a TYPE_REFTARG.
	DigitSegTrans::PB_DIGITDATA, _T(""), TYPE_FLOAT, P_ANIMATABLE, NULL,
		p_end,
	...
	DigitSegTrans::PB_BENDWEIGHT, _T("CATWeight"), TYPE_FLOAT, 0, NULL,
		...
		p_end,
	DigitSegTrans::PB_DIGITDATA_REFTARG, _T("Digit"), TYPE_REFTARG, P_SUBANIM | P_NO_REF, IDS_CL_DIGITDATA,
		p_nonLocalizedName, _T("DigitData"),
		p_accessor, &pbDigitAcessor,
		p_end,
	p_end
);
...
void DigitSegTrans::ApplySetupOffset(TimeValue t, Matrix3 tmOrigParent, Matrix3& tmWorldTransform, Point3 &p3BoneLocalScale, Interval& ivValid)
{
	...
	if (GetBoneID() == 0)
	{
		// Pre CAT3 we had a bad inheritance system where the toes inherited rotation off the iktarget.
...
TSTR DigitSegTrans::SubAnimName(int i, bool localized)
{

	switch (i)
	{
	case PBLOCK_REF: return localized ? GetString(IDS_PARAMS) : _T("Parameters");
	case LAYERTRANS: return localized ? GetString(IDS_LAYERTRANS) : _T("LayerTrans");
	default: return _T("");
	}
}
...
TSTR DigitSegTrans::GetRigName()
{
	...
	ICATParentTrans* pCATParentTrans = GetCATParentTrans();
	DigitData* pDigit = GetDigit();
	DbgAssert(pCATParentTrans && pLimb && pDigit);
	TSTR catName = _T("<<unknown>>");
	TSTR limbName = _T("<<unknown>>");
	TSTR digitName = _T("<<unknown>>");
	...
	{
		MSTR name = catName + limbName + digitName + boneName;
		LogErrMsgOnPrompt(name);
	}
	return catName + limbName + digitName + boneName;
}
...
{
	return static_cast<DigitData*>(pblock->GetReferenceTarget(PB_DIGITDATA_REFTARG));
}

void DigitSegTrans::SetDigit(DigitData* d)
{
	pblock->SetValue(PB_DIGITDATA_REFTARG, 0, d);
}
