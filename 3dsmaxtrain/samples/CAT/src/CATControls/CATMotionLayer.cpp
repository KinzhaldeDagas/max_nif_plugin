...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATMotionLayer"); }
...
TSTR CATMotionLayer::SubAnimName(int i, bool localized)
{
	if (i < NLAInfo::NumSubs())
		return NLAInfo::SubAnimName(i, localized);
	//	i -= NLAInfo::NumSubs();

	switch (i)
	{
	case REF_CATHIERARHYROOT: return localized ? GetString(IDS_CATHIERARCHYROOT) : _T("CATHierarchyroot");
	default: return _T("");
	}
}
