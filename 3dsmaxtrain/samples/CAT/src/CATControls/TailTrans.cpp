...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("TailTrans"); }
...
TSTR TailTrans::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case PBLOCK_REF: return localized ? GetString(IDS_CL_TAILDATA) : _T("TailData"); // GetString(IDS_PARAMS);
...
TSTR TailTrans::GetRigName()
{
	...
	TailData2* pTail = GetTail();
	TSTR tailName = _T("<<unknown>>");
	if (nullptr != pTail)
	{
		tailName = pTail->GetName();
	...
	{
		MSTR name = catName + tailName + boneName;
		LogErrMsgOnPrompt(name);
	}

	return catName + tailName + boneName;
}
