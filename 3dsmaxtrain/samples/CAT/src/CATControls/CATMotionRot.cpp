...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATMotionRot"); }
...
void CATMotionRot::GetValue(TimeValue t, void * val, Interval&valid, GetSetMethod method)
{
	...
	for (int i = 0; i < 3; i++)
		p3CATMotion[i] = std::max(std::min(p3CATMotion[i], 3600.f), -3600.f);
...
*/
TSTR CATMotionRot::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? GetString(IDS_PARAMS) : _T("Parameters");
	case 1: return localized ? GetString(IDS_CL_CATMOTIONLIMB) : _T("CATMotionLimb");
	default: return _T("");
	}
}
...
void CATMotionRot::RefDeleted(RefMakerHandle rm)
{
	return ReferenceTarget::RefDeleted(rm);
}
