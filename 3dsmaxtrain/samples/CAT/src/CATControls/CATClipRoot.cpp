...
public:
	...
	const TCHAR* NonLocalizedClassName() { return _T("LayerRoot"); }
...
public:
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("PasteLayerInfo"); }
...
BOOL OpenLayerManagerWindow(INode* catparent_node)
{
	...
	{
		ProcessUnknownMAXScriptException(_T("CreateCATLayerManager Script"), false, false, true);
	}
...
BOOL RefreshLayerManagerWindow(INode* catparent_node)
{
	...
	{
		ProcessUnknownMAXScriptException(_T("RefreshCATLayerManager Script"), false, false, true);
	}
...
BOOL DoCollapse(INode* catparent_node)
{
	...
	{
		ProcessUnknownMAXScriptException(_T("CATCollapseLayers Script"), false, false, true);
	}
...
INT_PTR CALLBACK ClipRolloutData::DlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CUSTEDIT_ENTER:
		{
			...
			bool arePtrsValid = edtClipName && pClipRoot;
			if (arePtrsValid && nLayer >= 0 && edtClipName->GotReturn())
			{
				TCHAR strbuf[128];
	...
	case CC_COLOR_CHANGE:
		...
		{
			Color clr(swatchLayerColour->GetAColor());
	...
	case CC_SPINNER_CHANGE:
		...
		case IDC_SPIN_CLIPWEIGHT:
			{
				...
				bool arePtrsValid = spnGlobalWeight && pClipRoot;
				if (arePtrsValid && nLayer >= 0)
				{
		...
		case IDC_SPIN_CLIPWEIGHTLOCAL:
			{
				...
				// but only update the clip layers list box once the
				// drag operation is complete.
				bool arePtrsValid = spnLocalWeight && pClipRoot;
				if (arePtrsValid && nLayer >= 0)
				{
					// If we are here from a cust-edit, we are not holding yet.
		...
		case IDC_SPIN_TIMEWARP:
			{
				bool arePtrsValid = spnTimeWarp && pClipRoot && catparenttrans;
				if (arePtrsValid && nLayer >= 0)
				{
					// If we are here from a cust-edit, we are not holding yet.
...
0xff, 0xc0,		// X X X X X X X X X X - - - - - -
	...
	swatchLayerColour = GetIColorSwatch(GetDlgItem(hDlg, IDC_LAYER_COLOUR), Color(0.0f, 0.0f, 0.0f), GetString(IDS_LYR_COLOR));
...
void ClipRolloutData::RefreshLayerColourSwatchState(int nLayer) 
{
	...
	swatchLayerColour->SetColor((nLayer >= 0 && pClipRoot->GetLayer(nLayer)->ApplyAbsolute()) ? pClipRoot->GetLayer(nLayer)->GetColour() : Color(ColorMan()->GetColor(kBackground)));
...
void ClipRolloutData::RefreshSpinners(int nLayer) {
	...
	if ((nLayer == -1) || pClipRoot->NumLayers() == 0) {
		...
		if (pClipRoot->localBranchWeights) {
			...
			Control* layerCon = pClipRoot->localBranchWeights->GetLayer(nLayer);
			if (layerCon != nullptr) {
				spnLocalWeight->SetKeyBrackets(layerCon->IsKeyAtTime(tvTime, 0));
			}
		}
		else {
			spnLocalWeight->Enable(FALSE);
...
BOOL CATClipRoot::InsertLayer(const TSTR& name, int at, ClipLayerMethod method)
{
	...
	NLAInfo* pNewInfo = GetLayer(at); // GetLayer can return null
	if (!pNewInfo) {
		if (theHold.Holding() && newundo) theHold.Cancel();
		return FALSE;
	}
...
void CATClipRoot::RemoveLayer(int at)
{
	...
	Interface::SuspendSceneRedrawGuard suspendSceneRedrawGuard;
	...
	suspendSceneRedrawGuard.Resume();
...
BOOL DoScriptImport(const TSTR& function, INode* catparent_node, const TSTR& file = _T(""), const TSTR& camfile = _T(""), BOOL quiet = FALSE)
{
	...
	{
		ProcessUnknownMAXScriptException(_T("DoScriptImport Script"), false, false, true);
	}
...
float CATClipRoot::GetLayerWeightLocal(int n, TimeValue t) const
{
	...
	{
		Control* layerCon = localBranchWeights->GetLayer(n);
		if(layerCon == nullptr) {
			return 1.0f;
		}
		layerCon->GetValue(t, (void*)&weight, iv, CTRL_ABSOLUTE);
	}
	return weight;
}
...
TSTR CATClipRoot::SubAnimName(int i, bool localized)
{
	if (i < (nNumValues + nNumBranches))
		return localized ? GetString(IDS_CAT1_SUBANIM) : _T("CAT1_SubAnim"); // CATClipThing::SubAnimName(i, localized);
	else
	{
		i -= (nNumValues + nNumBranches);
		if (i < nNumLayers)
			return tabLayers[i]->GetName();
		else
			return _T("");
	}
}
...
{
	CATClipRoot *newctrl = (CATClipRoot*)CreateInstance(CTRL_FLOAT_CLASS_ID, CATCLIPROOT_CLASS_ID);
	remap.AddEntry(this, newctrl);

	// This updates catparenttrans as a post patch if it was cloned.
	remap.PatchPointer((RefTargetHandle*)&newctrl->catparenttrans, catparenttrans);

	int numlayers = NumLayers();
	newctrl->ResizeList(numlayers, TRUE);
	for (int i = 0; i < numlayers; i++) {
		newctrl->ReplaceReference(i, remap.CloneRef(tabLayers[i]));
	}
	newctrl->nSelected = nSelected;
...
void CATClipRoot::PasteLayer(BOOL instance, BOOL duplicatelayerinfo, int targetLayerIdx, CATCharacterRemap* pRemap)
{
	...
	// hold on the original reference since they can be nullified in the call to pCATParent->BuildMapping() below:
	NLAInfo* pLayerInfo = g_PasteLayerData.layerinfo;
	CATClipRoot* pClipRoot = g_PasteLayerData.cliproot;
	...
	Interface::SuspendSceneRedrawGuard suspendSceneRedrawGuard;
	SuspendAll suspendAnimate(false, false, true, false);
	...
	NLAInfo* newlayer = nullptr;
	CATCharacterRemap* remap = nullptr;
	if (pRemap == nullptr)
	{
	...
	ICATParentTrans* pSrcParent = pClipRoot->GetCATParentTrans();

	// Populate one RemapDir for all the cloning so that things attach together properly.
	RemapDir* rmdir = NewRemapDir();

	// If we are instancing, keep the original layer info
	if (instance)
	{
		newlayer = pLayerInfo;	 //g_PasteLayerData.layerinfo;
	}
	else
	{
		// Map the hubs for the Motion Editor
		Hub* pasteHub = (Hub*)pClipRoot->GetRootHub();
		Hub* ourHub = (Hub*)GetRootHub();
		if (pasteHub != nullptr && ourHub != nullptr)
		{
			rmdir->AddEntry((ReferenceTarget*)pasteHub, (ReferenceTarget*)ourHub);
		}
		// Map the layer roots.
		rmdir->AddEntry(pClipRoot, this);
		// Map parent transforms to support pasting a layer from one character to another.
		if (pDstParent != nullptr && pSrcParent != nullptr)
		{
			rmdir->AddEntry(pSrcParent, pDstParent);
		}

		// Clone the layer we are pasting
		newlayer = (NLAInfo*)remap->FindMapping(pLayerInfo);
		if ((newlayer == nullptr) && pLayerInfo)
		{
			newlayer = static_cast<NLAInfo*>(pLayerInfo->Clone(*rmdir));
		}
	}
	...
	if (!newlayer)
	{
		...
		theHold.Cancel();
		MACRO_ENABLE;
		rmdir->DeleteThis(); // performs all the bach patching
		rmdir = nullptr;
		return;
	}

	// Pass ownership of the layer to the target rig(if we are pasting to a new rig).
	// If we are instancing the layer, this part causes issues but still mostly works.
		...
		pDstParent->GetRootHub())
	{
		...
		for (CATCharacterRemap::iterator itr = remap->begin(); itr != remap->end(); itr++)
		{
			...
			if (pSrc != NULL && pDst != NULL)
			{
				...
				{					
					pDst->PasteLayer(pSrc, pLayerInfo->GetIndex(), tolayer, flags, *rmdir);
				}
			}
		}
...
{
	if (ctxt != kSNCGatherSystemNodes) // KZ, don't close the UI if we just want to grab the nodes.
	{
		PreResetCleanup();
		CloseRangesView();
...
void  CATClipRoot::ShowRangesView()
{
	...
	if (!pRangesViewOps) {
		...
		pRangesViewOps = GetTrackView((catparenttrans->GetCATName() + _T(": Layer Ranges")).data()); // globalize? no - used to get tv, could be stored in file
	}
	if (pRangesViewOps) {
		// KZ, tv nodes must be hung off of scene nodes to be hidden otherwise the TV will display nothing. See TreeView::FilterAnim.
		ITrackViewNode *pTrackViewNode = CreateITrackViewNode(FALSE);
...
void CATClipRoot::ShowGlobalWeightsView()
{
	...
	if (!pGlobalWeightsViewOps) {
		...
		pGlobalWeightsViewOps = GetTrackView((catparenttrans->GetCATName() + _T(": Global Layer Weights")).data()); // don't globalize tv names (?)
	}
	if (pGlobalWeightsViewOps) {
		...
		Control* selectedLayer = nullptr; //find the selected layer controller
		for (int i = 0; i < NumLayers(); i++)
		{
			Control* c = GetLayer(i)->GetWeightsController();
			pTrackViewNode->AddController(c, GetLayer(i)->GetName().data(), CAT_TRACKVIEW_TVCLSID);
			if (GetSelectedLayer() == i) {
				selectedLayer = c;
			}
		}
		...
		//pre-select the correct layer controller in the trackview.
		if (selectedLayer != nullptr) {
			pGlobalWeightsViewOps->SelectTrack(selectedLayer);
		}
	}
}
...
void CATClipRoot::ShowLocalWeightsView(CATClipValue *localweights)
{
	...
	if (!pLocalWeightsViewOps) {
		...
		pLocalWeightsViewOps = GetTrackView((catparenttrans->GetCATName() + _T(": Local Layer Weights")).data()); // don't globalize tv names (?)
	}
	if (pLocalWeightsViewOps) {
		ITrackViewNode *pTrackViewNode = CreateITrackViewNode(CAT_HIDE_TV_ROOT_TRACKS);
		...
		for (int i = 0; i < NumLayers(); i++)
		{
			Control* c = localweights->GetLayer(i);
			pTrackViewNode->AddController(c, GetLayer(i)->GetName().data(), CAT_TRACKVIEW_TVCLSID);
			if (GetSelectedLayer() == i) {
				selectedLayer = c;
			}
		...
		// pre-select the correct layer controller in the trackview.
		if (selectedLayer != nullptr) {
			pLocalWeightsViewOps->SelectTrack(selectedLayer);
		}
	}
}
...
void CATClipRoot::ShowTimeWarpView()
{
	...
	if (!pTimeWarpViewOps) {
		...
		pTimeWarpViewOps = GetTrackView((catparenttrans->GetCATName() + _T(": Layer Time Warp Curves")).data());  // don't globalize tv names (?)
	}
	if (pTimeWarpViewOps) {
		ITrackViewNode *pTrackViewNode = CreateITrackViewNode(CAT_HIDE_TV_ROOT_TRACKS);
		pTrackViewNode->HideChildren(FALSE);
		...
		Control* selectedTimeWarpControl = nullptr;
		...
		for (int i = 0; i < NumLayers(); i++) {
			...
			if (i == GetSelectedLayer()) {
				//create and grab the time warp controller
				pInfo->CreateTimeController();
				selectedTimeWarpControl = pInfo->GetTimeController();
			}

			// Add the info the the curve editor
			HoldSuspend hs;
			pTrackViewNode->AddController(GetLayer(i), GetLayer(i)->GetName().data(), CAT_TRACKVIEW_TVCLSID);
		}

		//setup the view to select the time warp track
		pTimeWarpViewOps->ExpandTracks();
		if(selectedTimeWarpControl != nullptr) {
			pTimeWarpViewOps->SelectTrack(selectedTimeWarpControl);
		}
	}
}
