...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATMotionTailRot"); }
...
void CATMotionTailRot::GetValue(TimeValue t, void * val, Interval &valid, GetSetMethod)
{
	...
	if (index >= 0) {
		if (id == 0) {
			...
			CATMotionTailRot* parentctrl = dynamic_cast<CATMotionTailRot*>(parentbone->GetLayerTrans()->GetLayer(index)->GetRotationController());
