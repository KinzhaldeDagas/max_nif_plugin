...
public:
	...
	const TCHAR* NonLocalizedClassName() { return _T("LayerFloat"); }
...
public:
	...
	const TCHAR* NonLocalizedClassName() { return _T("LayerMatrix3"); }
...
public:
	...
	SClass_ID SuperClassID() { return CTRL_FLOAT_CLASS_ID; }
	void GetClassName(MSTR& s, bool localized) const override { s = _T("CATDummyFloat"); }
};
...
public:
	...
	SClass_ID SuperClassID() { return CTRL_MATRIX3_CLASS_ID; }
	void GetClassName(MSTR& s, bool localized) const override { s = _T("CATDummyMatrix3"); }
};
...
void CATClipFloat::SetValue(TimeValue t, const void* valParent, void* val, int commit, GetSetMethod method,
		LayerRange range, BOOL bWeighted)
{
	int iCATMode = GetCATMode();
	CATClipRoot* clipRoot = GetRoot();
	if (iCATMode == SETUPMODE && !TestFlag(CLIP_FLAG_KEYFREEFORM) && clipRoot &&
			!clipRoot->TestFlag(CLIP_FLAG_COLLAPSINGLAYERS))
	{
		// Save state for undo
		HoldTrack();
	...
	if (!TestFlag(CLIP_FLAG_KEYFREEFORM) && clipRoot &&
			!clipRoot->LayerHasContribution(nSelected, t, range, GetWeightsCtrl()))
		return;
...
void CATClipMatrix3::ApplyInheritance(TimeValue t, Matrix3 &ptm, int layerid, DWORD flags)
{
	...
	if (flags&INHERIT_ANY_MASK) {
		...
		if (flags&INHERIT_ROT_MASK || flags&INHERIT_SCL_MASK) {
			...
			// I think this is a rather bad solution myself and I would have simply
		...
		while (load->ok() && !done && ok) {
			...
			case rigAssignment:
				...
				case idFlags: {
					...
					// for some unexpected reason, the flags are inverted when SETTING the inheritance flags
...
void CATClipMatrix3::GetTransformation(TimeValue t, const Matrix3& tmOrigParent, Matrix3 &tmValue, Interval& valid, const Point3& p3ParentScale, Point3 &p3LocalScale, LayerRange range/*=LAYER_RANGE_ALL*/, BOOL bWeighted/*=TRUE*/)
{
	...
	// This is the guts...
	for (int i = std::max(range.nFirst, nFirstActiveLayer); i <= range.nLast; i++) {
