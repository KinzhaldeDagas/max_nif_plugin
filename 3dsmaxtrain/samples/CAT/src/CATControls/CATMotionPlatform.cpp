...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATMotionPlatform"); }
...
static ParamBlockDesc2 weightshift_param_blk(CATMotionPlatform::PBLOCK_REF, _T("CATMotionPlatform params"), 0, &CATMotionPlatformDesc,
	...
	CATMotionPlatform::PB_FOLLOW_PATH, _T("PathFollow"), TYPE_FLOAT, P_ANIMATABLE, IDS_PATHFOLLOW_W,
		...
		p_nonLocalizedName, _T("PathFollowWeight"),
		p_end,
	...
	CATMotionPlatform::PB_STEPSHAPE, _T("StepShape"), TYPE_FLOAT, P_ANIMATABLE, IDS_CL_STEPSHAPE,
		p_nonLocalizedName, _T("CATStepShape"),
		p_end,
	CATMotionPlatform::PB_STEPMASK, _T(""), TYPE_FLOAT, P_ANIMATABLE, IDS_MASK,
		p_nonLocalizedName, _T("Movement Mask"),
		p_end,
	...
	CATMotionPlatform::PB_PRINT_NODES, _T("FootPrintNodes"), TYPE_INODE_TAB, 0, 0, IDS_FOOTPRINTS,
		p_nonLocalizedName, _T("FootPrints"),
		p_end,
	p_end
);

class DisableFootprintCalculationRestore : public RestoreObj
{
	CATMotionPlatform* mPlatform = nullptr;
	bool mParity = false;
public:
	DisableFootprintCalculationRestore(CATMotionPlatform* p, bool parity)
			: RestoreObj(), mPlatform(p), mParity(parity)
	{ }
	void Restore(int /*isUndo*/) override
	{
		...
		if (mParity) {
			mPlatform->DisableFootPrintsCalculation();
		}
		else {
			mPlatform->EnableFootPrintsCalculation();
		}
	}
	void Redo() override
	{
...
static void DisableFootSteps_PreNodeDelete(void* ptr, NotifyInfo* info)
{
	CATMotionPlatform* platform = reinterpret_cast<CATMotionPlatform*>(ptr);
	if (platform == nullptr)
	{
		DbgAssert(false);
		return;
	...
	if (theHold.Holding()) {
		theHold.Put(new DisableFootprintCalculationRestore(platform, false));
	}
	platform->DisableFootPrintsCalculation();
}

static void EnableFootSteps_PostNodeDelete(void* ptr, NotifyInfo* info)
{
	CATMotionPlatform* platform = reinterpret_cast<CATMotionPlatform*>(ptr);
	if (platform == nullptr)
	{
	...
	if (theHold.Holding()) {
		theHold.Put(new DisableFootprintCalculationRestore(platform, true));
	}
	platform->EnableFootPrintsCalculation();
}
...
CATMotionPlatform::CATMotionPlatform() {
	...
	// KZ 06/2024: Hack to patch around a strange crash when deleting and undoing.
	//	The footsteps are constantly being recalculated during this process and its breaking lots of things.
	RegisterNotification(&DisableFootSteps_PreNodeDelete, (void*)this, NOTIFY_NODES_PRE_DELETE);
	RegisterNotification(&EnableFootSteps_PostNodeDelete, (void*)this, NOTIFY_NODES_POST_DELETE);
}

CATMotionPlatform::~CATMotionPlatform() {
	DeleteAllRefs();

	UnRegisterNotification(&DisableFootSteps_PreNodeDelete, (void*)this, NOTIFY_NODES_PRE_DELETE);
	UnRegisterNotification(&EnableFootSteps_PostNodeDelete, (void*)this, NOTIFY_NODES_POST_DELETE);
}
...
class MotionPlatformPostPatch : public PostPatchProc
{
public:
	CATMotionPlatform* plat;
	CATMotionPlatform* clonedPlat;

	MotionPlatformPostPatch(CATMotionPlatform* plat, CATMotionPlatform* clonedPlat)
	{
		this->plat = plat;
		this->clonedPlat = clonedPlat;
	}
	virtual int Proc(RemapDir& remap) override
	{
		// Handle the footprints. Since we own them we will regenerate them as needed but leave old ones alone if not cloned.
		const int printNodesCount = plat->pblock->Count(CATMotionPlatform::PB_PRINT_NODES);
		clonedPlat->pblock->SetCount(CATMotionPlatform::PB_PRINT_NODES, printNodesCount);
		//clonedPlat->pblock->SetCount(CATMotionPlatform::PB_CAT_PRINT_INV_TM, printNodesCount); // Populate these in CalcAllPrintPos below.
		//clonedPlat->pblock->SetCount(CATMotionPlatform::PB_CAT_PRINT_TM, printNodesCount);
		for (int i = 0; i < printNodesCount; ++i)
		{
			INode* oldNode = plat->pblock->GetINode(CATMotionPlatform::PB_PRINT_NODES, 0, i);
			INode* newNode = nullptr;
			if (oldNode != nullptr) {
				newNode = (INode*)remap.FindMapping(oldNode);
			}
			// If we did not clone the nodes, clear our reference to it
			// If we did clone the node, then we should use that one
			clonedPlat->pblock->SetValue(CATMotionPlatform::PB_PRINT_NODES, 0, newNode, i);
		}
		clonedPlat->EnableFootPrintsCalculation(); // Disabled in ::Clone.
		clonedPlat->CalcAllPrintPos();
...
RefTargetHandle CATMotionPlatform::Clone(RemapDir& remap)
{
	...
	newCATMotionPlatform->DisableFootPrintsCalculation(); // Re-enable in post patch.
	...
	// Make sure that we are not sharing footprint nodes, we own our own. Handled in post patch.
	clonedpblock->SetCount(PB_CAT_PRINT_INV_TM, 0);
	clonedpblock->SetCount(PB_CAT_PRINT_TM, 0);
	clonedpblock->SetCount(PB_PRINT_NODES, 0);

	newCATMotionPlatform->ReplaceReference(PBLOCK_REF, clonedpblock);

	// P_NO_REF's:
	// PB_FOOTTRANS, handled in post patch.
	// PB_CATMOTIONLIMB, handled in post patch.

	BaseClone(this, newCATMotionPlatform, remap);
	remap.AddPostPatchProc(new MotionPlatformPostPatch(this, newCATMotionPlatform), true);
...
void CATMotionPlatform::GetValue(TimeValue t, void* val, Interval&valid, GetSetMethod)
{
	...
	if ((catparenttrans == nullptr) || (iktarget == nullptr))
	{
		return;
	}
	...
	AngAxis ax;
	if (lengthaxis == Z) {
		ax.angle = acos(std::min(1.0f, DotProd(Point3(0.0f, 0.0f, 1.0f), tmSetup.GetRow(Z))));
		if (ax.angle > 0.0001f) {
		...
		ax.angle = acos(std::min(1.0f, DotProd(Point3(1.0f, 0.0f, 0.0f), tmSetup.GetRow(X))));
		if (ax.angle > 0.0001f) {
...
{
	// KZ 06/2024: Hack to disable calculating foot prints during delete and undo.
	if (!mCalculateFootprints) {
		return FALSE;
	}
	...
	// Lock the following to prevent being spammed with messages.
	{
		...
		if (bCreateNodes)
		{
			...
			for (int i = 0; i < numSteps; i++) {
				...
				if (!pblock->GetINode(PB_PRINT_NODES, 0, i)) {	// If there is no node (just created or deleted)
					...
					INode* footprintnode = ip->CreateObjectNode(helper);
		...
		// TODO : make sure these initialisations actually work
...
BOOL CATMotionPlatform::SnapPrintToGround(TimeValue t, INode *footprint, Mesh *grndMesh, const Matrix3 &invGrndTM, const Matrix3 &grndTM)
{
	...
	else
	{
		...
		connected = grndMesh->IntersectRay(printUpRay, distToTarg, surfaceNormal);
		if (connected)
		{
			tmFootPrint.PreTranslate(FloatToVec(distToTarg, lengthaxis));
			Point3 printPos = tmFootPrint.GetTrans();
...
//	nStepNumber = std::min(std::max(nStepNumber, 0), nNumSteps-2);
