...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override { s = _T("CATGizmoTransform"); } //GetString(IDS_CL_GIZMOTRANSFORM);}

	RefTargetHandle Clone(RemapDir &remap);
	RefResult NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate);

	int NumSubs() { return 1; }
	TSTR SubAnimName(int i, bool localized) override { return localized ? GetString(IDS_TARG_CONT) : _T("TargetController"); }
...
public:
	...
	void* GetInterface(ULONG id) {
		//	if (id==I_DRIVER && target_node) 	return (void *)target_node->GetTMController()->GetInterface(id);
		//	else 								return NULL;
