...
void ExtraRigNodes::RemoveExtraRigNodes(Tab<INode*> removednodes)
{
	...
	for (int i = 0; i < removednodes.Count(); i++)
	{
		...
		// if a driver controller is defined, remove all children as well.
		ReferenceTarget* pDriver = (ReferenceTarget*)extranode->GetTMController()->GetInterface(I_DRIVER);
		if (pDriver != NULL)
		{
			INodeTab extranode_systemnodes;
			pDriver->GetSystemNodes(extranode_systemnodes, kSNCFileSave);
...
void ExtraRigNodes::AddSystemNodes(INodeTab &nodes, SysNodeContext ctxt)
{
	...
	for (int i = 0; i < nExtraNodes; i++)
	{
		...
		if (pNode != NULL)
		{
			...
			// if a driver controller is defined, add all children as well.
			ReferenceTarget* pDriver = (ReferenceTarget*)pNode->GetTMController()->GetInterface(I_DRIVER);
			if (pDriver != NULL)
			{
				// Expand the system
				INodeTab extranode_systemnodes;
				pDriver->GetSystemNodes(extranode_systemnodes, ctxt);
				for (int j = 0; j < extranode_systemnodes.Count(); ++j)
				{
