...
public:
	...
	enum {
		PB_SPINEDATA, // Holder of the controller SpineTrans2 controller but is hidden. Access through PB_SPINEDATA_REFTARG.
		PB_LINKNUM_DEPRECATED,
		PB_TMSETUP,
		PB_ROTWT,
		PB_POSWT,
		PB_SPINEDATA_REFTARG // External access to the SpineTrans2 controller hold by PB_SPINEDATA.
	};
...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_CL_SPINETRANS2) : _T("CATSpineTrans2"); }

	int NumSubs() { return NUMPARAMS; }
	TSTR SubAnimName(int i, bool localized) override;// { return _T(""); } //GetString(IDS_PARAMS); }
