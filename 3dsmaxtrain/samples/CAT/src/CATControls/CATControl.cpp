...
{
	DbgAssert(mBlockedClass != NULL);
	HoldSuspend hs;
	mBlockedClass->IncrementBlockEvaluation();
}

CATControl::BlockEvaluation::~BlockEvaluation()
{
	DbgAssert(mBlockedClass != NULL && mBlockedClass->IsEvaluationBlocked());
	HoldSuspend hs;
	mBlockedClass->DecrementBlockEvaluation();
}
...
void CATControl::SetCCFlag(ULONG f, BOOL on) {
	if ((ccflags & f) != (on ? f : 0)) {
		...
		if (f & ~(CNCFLAG_RETURN_EXTRA_KEYS | CNCFLAG_EVALUATING)) {
			UpdateUI();
		}
	}
}

void CATControl::IncrementBlockEvaluation()
{
	DbgAssert(mBlockEvaluation >= 0);

	HoldActions hold(IDS_HLD_BONESETTING);
	HoldData(mBlockEvaluation);
	mBlockEvaluation++;
}

void CATControl::DecrementBlockEvaluation()
{
	DbgAssert(mBlockEvaluation > 0);
...
BOOL CATControl::SavePose(TSTR filename)
{
	...
	return (GetCATParentTrans()) ? GetCATParentTrans()->SaveClip(filename, flags, clipLength, layerrange, this) : false;
}
...
public:
	...
	void proc(ILoad *) {
		...
		while (ctrl->IsEvaluationBlocked()) {
			ctrl->DecrementBlockEvaluation();
		}
