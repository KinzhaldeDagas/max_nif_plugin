...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATHIPivotTrans"); }
...
TSTR HIPivotTrans::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case PRS: return localized ? GetString(IDS_PRS) : _T("PRS");
	case PIVOTPOS: return localized ? GetString(IDS_PIVOTPOS) : _T("PivotPos");
	case PIVOTROT: return localized ? GetString(IDS_PIVOTROT) : _T("PivotRot");
	default: return _T("");
	}
}
