...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("LayerInfo"); }
...
RefTargetHandle NLAInfo::Clone(RemapDir& remap)
{
	...
	remap.AddEntry(this, newNLAInfo);
...
TSTR NLAInfo::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case REF_WEIGHT: return localized ? GetString(IDS_WEIGHTS) : _T("Weights");
	case REF_TIMING: return localized ? GetString(IDS_TIMEWARP) : _T("TimeWarp");
	case REF_TRANSFORM: return localized ? GetString(IDS_TRANSFORM) : _T("Transform");
	default: return _T("");
	}
}
