...
#include "CATToken.h"
  ...
  */
static constexpr DictionaryEntry dictBaseTokenEntries[] = {
...
static constexpr DictionaryEntry dictStandardTokenEntries[] = {
	//  <NAME>					<IDENTIFIER ID>			<IDENTIFIER TYPE>		<FOLLOWING TYPE>
...
BOOL Dictionary::AddEntry(const DictionaryEntry* entry)
{
	const TCHAR *c;
	TCHAR lc;
	int nodeid = 0, nextid;
...
void Dictionary::BuildDictionary(const DictionaryEntry* table)
{
	const DictionaryEntry* entry = table;
	while (entry->name) {
...
const DictionaryEntry* Dictionary::GetEntryAtNode(int nodeID)
{
...
const DictionaryEntry* Dictionary::GetEntry(int entryID)
{
...
// in the dictionary.
const DictionaryEntry* Dictionary::Lookup(const TCHAR* s)
{
	int node = 0;
	for (; *s; s++)
...
const DictionaryEntry* Dictionary::PartialLookup(const TCHAR* s, int &nFirst, int &nLength, BOOL bAllowPartialMatches/*=TRUE*/)
{
	const DictionaryEntry *entry = NULL;
	int node, nextnode;
	int first, last;
		...
		while (s[last]) {
			...
			if (nextnode == -1) {
				...
				if (node == -1 && bAllowPartialMatches) {
					...
					const DictionaryEntry *pNextEntry = PartialLookup(&s[first + 1], nNextFirst, nNextLength, FALSE);
					nNextFirst += first + 1;
