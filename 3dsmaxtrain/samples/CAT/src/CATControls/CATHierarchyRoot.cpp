...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATHierarchyRoot"); }
...
void CATHierarchyRoot::GetClassName(MSTR& s, bool localized) const { s = localized ? GetString(IDS_CL_CATHIERARCHYROOT) : _T("CATHierarchyRoot"); }
...
static ParamBlockDesc2 cathierarchy_root_param_blk(CATHierarchyRoot::PBLOCK_REF, _T("params"), 0, &CATHierarchyRootDesc,
	...
	CATHierarchyRoot::PB_PATH_NODE, _T("PathNode"), TYPE_INODE, 0, IDS_PATH_NODE,
		...
		p_nonLocalizedName, _T("PAth Node"),
		p_end,
	CATHierarchyRoot::PB_GROUND_NODE, _T("GroundNode"), TYPE_INODE, 0, IDS_GROUND_NODE,
		...
		p_nonLocalizedName, _T("Ground Node"),
		p_end,
...
{
	if (!GetWeights()) {
		TSTR msgBoxText = GetString(IDS_NOHIERARCHY);
		MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), msgBoxText, GetString(IDS_ERROR), MB_OK);
		return FALSE;
	}
	...
	if (!load.ok()) {
		TSTR msgBoxText = GetString(FILENOFND);
		MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), msgBoxText, GetString(IDS_ERROR), MB_OK);
		return FALSE;
	}
	...
	if (bNewLayer) {
		...
		if (slot < 0) {
			TSTR msgBoxText = GetString(IDS_PRESETNOCREATE);
			MaxSDK::MaxMessageBox(ip->GetMAXHWnd(), msgBoxText, GetString(IDS_ERROR), MB_OK);
			return FALSE;
		}
	}
	else if (slot < 0 || slot >= GetWeights()->GetNumLayers()) {
		TSTR msgBoxText = GetString(IDS_PRESETNOLOAD);
		MaxSDK::MaxMessageBox(ip->GetMAXHWnd(), msgBoxText, GetString(IDS_ERROR), MB_OK);
		return FALSE;
	}
...
//	pClipRootInterface->SetKALiftingCtrl(0, leafKALifting);
	...
	if (pCATParent != NULL)
	{
		...
		if (pHub != NULL)
		{
			...
			catmotionhub->Initialise(index, pHub, this);
...
// to NULL, just in case they get used after we've deleted.
...
TSTR CATHierarchyRoot::SubAnimName(int i, bool localized)
{
	if (i == 0)
		return localized ? GetString(IDS_WEIGHTS) : _T("Weights");
	if ((i - 1) < GetNumBranches())
		return GetBranchName(i - 1);
	if ((i - 1) == GetNumBranches())
		return localized ? GetString(IDS_TIMEWARP) : _T("TimeWarp");
	return _T("");
}
...
public:
	...
	void EndHold() {
		if (root)
		{
			INode* pathNode = root->GetPathNode();
			if (pathNode)
				pathNode->ClearAFlag(A_PLUGIN3);
			root->IUpdateDistCovered();
		}		
	}
	...
	else
	{
		...
		while (t <= iCATMotion.End())	// plot the stepgraph
		{
			...
			if (nextStepTime > nextDistTime) {
				t = std::max(nextDistTime, t + tpf);	// Our smallest step must be at least one frame, this
				distance = nextStepDist;		// prevents an infinate loop when we are walking o
			}
			else {
				t = nextStepTime;
				distance = ctrlDistCovered->GetValue(std::max(std::min(nextStepTime, ivPathMoving.End()), ivPathMoving.Start()));
...
void CATHierarchyRoot::GettmPath(TimeValue t, Matrix3 &val, Interval &valid, float pathOffset, BOOL isFoot)
{
	...
	//			newT = t;
				...
				//			fwDist = std::max(pathOffset, fwDist);
			}
		}
		else
			fwDist = pathOffset;
...
RefTargetHandle CATHierarchyRoot::Clone(RemapDir& remap)
{
	...
	newCATHierarchyRoot->ReplaceReference(PBLOCK_REF, clonedpblock);

	// PB_BRANCHTAB were cloned, since we own them setup their parent branches
	for (int i = 0; i < GetNumBranches(); i++) {
		CATHierarchyBranch* newBranch = newCATHierarchyRoot->GetBranch(i);
		if(newBranch != nullptr) {
			newBranch->SetBranchParent(newCATHierarchyRoot);
		}
	}

	// Clone manually the P_NO_REF's that we own, the rest reconnect in post patch if cloned by their owners.
	// PB_BRANCHPARENT, was originally ommited, handled in post patch.
	// PB_BRANCHOWNER, was originally ommited, handled in post patch.
	// PB_CATPARENT, was originally ommited and not set in the code anywhere, for older file compatibility
	// PB_CATMOTIONHUB, we own this, clones all the branches
	CATMotionHub2* catmotionhub = (CATMotionHub2*)pblock->GetReferenceTarget(CATHierarchyRoot::PB_CATMOTIONHUB);
	newCATHierarchyRoot->pblock->SetValue(CATHierarchyRoot::PB_CATMOTIONHUB, 0, remap.CloneRef(catmotionhub));
	// PB_NLAINFO, was originally ommited, handle in post patch.
