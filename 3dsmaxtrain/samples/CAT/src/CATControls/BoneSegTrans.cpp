...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATBoneSegTrans"); }
...
static ParamBlockDesc2 bonesegtrans_param_blk(BoneSegTrans::pb_params, _T("BoneSegTransParams"), 0, &BoneSegTransDesc,
	...
	BoneSegTrans::PB_BONEDATA, _T("Bone"), TYPE_REFTARG, P_SUBANIM, IDS_BONEDATA,
		p_nonLocalizedName, _T("BoneData"),
		p_end,
...
TSTR BoneSegTrans::GetRigName()
{
	...
	if (boneData)
	{
		bonename = boneData->GetName();
	}
	TSTR limbname = _T("<<unknown>>");	
	LimbData2* limbData = ((nullptr != boneData) ? boneData->GetLimbData() : nullptr);
	if (nullptr != limbData)
		limbname = limbData->GetName();
	...
	if (nullptr != boneData)
	{
		...
		{
			bonename.printf(_T("%d"), GetBoneData()->GetBoneID() + 1);
		}

		iNumSegs = boneData->GetNumBones();
		if (iNumSegs > 1)
		{
			if (name.Length() == 0)
			{
			...
			fullName = limbname + bonename + name;
		}
		else
			fullName = limbname + bonename;
	}
	
	if (nullptr != pCATParentTrans)
		fullName = pCATParentTrans->GetCATName() + fullName;
	else
	{		
		fullName = catName + fullName;
		LogErrMsgOnPrompt(fullName);
	}
