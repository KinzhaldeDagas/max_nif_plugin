...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATPivotRot"); }
...
float PivotRot::GetYval(TimeValue t, int LoopT)
{
	...
	starttime = std::max(std::min(starttime, KEY1time), 0.f);
	KEY1time = std::min(std::max(starttime, KEY1time), KEY2time);
	endtime = MaxSDK::min(endtime, STEPTIME100);
	KEY2time = std::max(std::min(KEY2time, endtime), KEY1time);
