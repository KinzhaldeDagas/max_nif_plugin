...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATFootBend"); }
...
float FootBend::GetYval(TimeValue t, int LoopT)
{
	...
	/*	KEY1time = std::min(std::max(-STEPTIME25, KEY1time), KEY2time);
		KEY2time = std::max(std::min(KEY2time, KEY3time), KEY1time);
		KEY3time = std::min(std::max(KEY3time, KEY4time), KEY2time);
		KEY4time = std::max(std::min(KEY4time, STEPTIME100 + STEPTIME25), KEY3time);
	*/
