...
void KneeAngle::GetYRange(TimeValue t, float	&minY, float &maxY)
{
	...
	float KAMax = std::max(std::max(liftval, midliftval), std::max(plantval, midplantval));
	float KAMin = std::min(std::min(liftval, midliftval), std::min(plantval, midplantval));
