...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("HubTrans"); }
...
void Hub::GetClassName(MSTR& s, bool localized) const { s = localized ? GetString(IDS_CL_HUBTRANS) : _T("HubTrans"); }
...
public:
	...
	void InitControls(HWND hDlg, IParamMap2 *map)
	{
		...
		if (iobj && iobj->CustomMeshAvailable()) {
		...
		if (hub && hub->GetCATParentTrans()->GetCATMode() != SETUPMODE)
		{
			btnAddLeg->Disable();
			btnAddArm->Disable();
			btnAddSpine->Disable();
		...
		else if (hub && !hub->CanPasteControl())
		{
			btnPaste->Disable();
			btnPasteMirrored->Disable();
		}
	...
	virtual INT_PTR DlgProc(TimeValue, IParamMap2 *map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_CUSTEDIT_ENTER:
		{
			...
			{
			case IDC_EDIT_NAME: {
				if (edtName != nullptr && hub != nullptr)
				{
					TCHAR strbuf[128];
		...
		case CC_SPINNER_CHANGE: {
			if (hub != nullptr)
			{
				...
				switch (LOWORD(wParam))
				{ // TODO put the enum into the Hub class
...
void Hub::SetValue(TimeValue t, void *val, int commit, GetSetMethod method)
{
	...
	if (GetCATParentTrans() &&
			GetCATParentTrans()->GetCATLayerRoot() && !GetCATParentTrans()->GetCATLayerRoot()->TestFlag(
					CLIP_FLAG_COLLAPSINGLAYERS))
	{
...
void Hub::GettmOrient(TimeValue t, Matrix3& tm)
{
	...
	if (GetCATParentTrans()  && (GetCATMode() == SETUPMODE) &&
			!GetCATParentTrans()->GetCATLayerRoot()->TestFlag(CLIP_FLAG_COLLAPSINGLAYERS))
	{
	...
	if (dDangleRatio > 0.0f)
	{
		...
		AngAxis ax;
		ax.angle = acos(std::min(1.0f, DotProd(P3_UP_VEC, tm.GetRow(lengthaxis)))) * dDangleRatio;
...
bool Hub::ApplyReach(TimeValue t, Interval& ivValid, Matrix3 &tmSpineTip, Matrix3& tmSpineResult, const Point3& p3HubScale)
{
	...
	if (TestCCFlag(HUBFLAG_ALLOW_IKCONTRAINT_ROT)) totWeight = std::max(2.0f, totWeight);
...
void Hub::CacheReachRotations(Tab<float> weights, Tab<Ray> &shiftDists, const Point3& p3OldHubPos, const Point3& p3NewHubPos)
{
	...
	if (TestCCFlag(HUBFLAG_ALLOW_IKCONTRAINT_ROT))
	{
		...
		ax.angle = acos(std::max(-1.0f, std::min(1.0f, DotProd(Normalize(vec1), Normalize(vec2)))));
...
Matrix3 Hub::ApproxCharacterTransform(TimeValue t)
{
	...
	dCosRatio = std::min(dCosRatio, 1.0f);
...
TSTR Hub::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case SUBTRANS: return localized ? GetString(IDS_LAYERTRANS) : _T("LayerTrans");
	case SUBWEIGHTS: return localized ? GetString(IDS_WEIGHTS) : _T("Weights");
	default: return _T("");
	}
}
