...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATBoneData"); }
...
public:
	...
	void InitControls(HWND hWnd, BoneData* bone)
	{
		...
		if (bone->GetCATParentTrans()  && bone->GetCATParentTrans()->GetCATMode() != SETUPMODE)
		{
			btnCopy->Disable();
			btnPaste->Disable();
...
#define COS_LIMIT(v) (std::max(std::min((v), 1.0f), -1.0f))
...
void BoneData::GetIKValue(TimeValue t, const Point3& p3CurrPos, const Matrix3& tmParent, Matrix3& tmValue, Point3& p3LocalScale, Interval& ivValid)
{
	...
	numbones = std::min((int)floor(pLimb->GetLimbIKPos(t, validIK)) + 1, numbones);
		...
		if (!(iBoneID == (numbones - 1) && pLimb->GetPalmTrans()))
		{
			if ((iBoneID <= (numbones - 2)) ||
				...
				else
				{
					...
					float ik_long_child = std::min(child_lengths, this_bone_to_ik_target_length + bone_length);
...
TSTR BoneData::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case SUBLIMB: return localized ? GetString(IDS_LIMBDATA) : _T("LimbData");
	case SUBTRANS: return localized ? GetString(IDS_LAYERTRANS) : _T("LayerTrans");
	default: return _T("");
	}
}
	...
	case REFMSG_CHANGE:
		...
		else if (mLayerTrans == hTarg)
		{
			...
			if (pLimb != NULL)
			{
				...
				if (!pLimb->IsEvaluationBlocked() && pLimb->GetIKFKRatio(t, valid) < 1)
					pLimb->UpdateLimb();
				else
				{
					if (GetBoneID() > 0)
...
float BoneData::GetTwistAngle(int nSegID, Interval& ivValid)
{
	...
	float dTwistWeight = 0.0f;
	if (numBones != 0)
	{
		if (nSegID > numBones) {
			nSegID = numBones;
		}
		const float dSegRatio = (float)nSegID / (float)numBones;
		dTwistWeight = GetTwistWeight(dSegRatio);
	}
	...
	const float dInitWeight = GetTwistWeight(0);
	const float dFinalWeight = GetTwistWeight(1.0f);

	// KZ 11/2023, Max 2025; There is a discontinuity here when values are positive or both negative. 1 of them will be against zero
	// rather than a weighted parent|child angle. Means handling the divide by zero case below is ambiguous about what to do.
	// Could change scene behavior. Work around is make their limb 1 bone without twisting.
	const float dAngleFromParent = (dInitWeight < 0) ? mdParentTwistAngle * dInitWeight : 0.0f;
	const float dAngleToChild = (dFinalWeight > 0) ? mdChildTwistAngle * dFinalWeight : 0.0f;

	if (abs(dFinalWeight - dInitWeight) < 0.0001f) {
		// Init and final are too close together, means no twisting, Use all child.
		dTwistWeight = 1.0f; 
	}
	else {
...
public:
	...
	DWORD GetFileSaveVersion() {
		DbgAssert(bone);
		if (bone && bone->GetFileSaveVersion() > 0) return bone->GetFileSaveVersion();
