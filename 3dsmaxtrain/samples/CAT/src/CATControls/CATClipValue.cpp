...
void CATClipValue::InsertController(int n, BOOL makenew)
{
	...
	int nNewLayers = std::max(n + 1, nCurrLayers + 1);
...
//				EndEditParams(ipClip, flagsBegin);
	...
	case CLIP_LAYER_CHECK_NUM_LAYERS:
		if (m_pClipRoot && GetNumLayers() != tabLayers.Count()) {
			...
			MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), message + errordetails + summary, GetString(IDS_ERR_FILELOAD), MB_OK);
...
void CATClipValue::SavePostLayerCallback()
{
	...
	TSTR rigdatafile = GetUserSettingsPath(_T("newlayercallback.txt"));
...
TSTR CATClipValue::SubAnimName(int i, bool localized)
{
	if (TestFlag(CLIP_FLAG_SETUPPOSE_CONTROLLER))
	{
		if (i == 0)
			return localized ? GetString(IDS_SETUP) : _T("Setup");
		i--;
	}
	CATClipRoot* pRoot = GetRoot();
	if (pRoot != NULL)
...
{	
	...
	if ((flags & BEGIN_EDIT_MOTION) && m_pClipRoot)
...
{
	DbgAssert(m_pClipRoot);
	if ((flagsBegin & BEGIN_EDIT_MOTION) && m_pClipRoot)
	{
		...
		while (load->ok() && !done && ok) {
			...
			case rigAssignment:
				...
				case idFlags: {
					...
					// for some unexpectred reason, the flags are inverted when SETTING the inheritance flags
...
BOOL RemapNodePointers(Control* fromctrl, Control* toctrl, ICATParent* fromcatparent, ICATParent* tocatparent)
{
	if (fromctrl->GetInterface(LINK_CONSTRAINT_INTERFACE) && toctrl->GetInterface(LINK_CONSTRAINT_INTERFACE)) {
		...
		int minimum_from_to = std::min(fromnumSubs, tonumSubs);
