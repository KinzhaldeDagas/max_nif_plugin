...
static ParamBlockDesc2 datamusher_param_blk(CATp3::pb_params, _T("p3 params"), 0, &CATp3Desc,
	...
	CATp3::PB_LIFTOFFSET_TAB, _T("liftOffset"), TYPE_FLOAT_TAB, 0, P_ANIMATABLE + P_TV_SHOW_ALL + P_VARIABLE_SIZE, IDS_HUBOFFSETLIFT,
		p_nonLocalizedName, _T("HubOffsetLift"),
		p_end,
	p_end
);
...
TSTR CATp3::SubAnimName(int i, bool localized)
{
	int p3flags = pblock->GetInt(PB_P3FLAGS);
	...
	if (p3flags & P3FLAG_POS)
	{
		...
		{
		case 0:
			strSubName = localized ? GetString(IDS_WEIGHTSHIFT) : _T("WeightShift");
			return strSubName + _T(" ") + currLimbName;
		case 1:
			strSubName = localized ? GetString(IDS_PUSH) : _T("Push");
			return strSubName + _T(" ") + currLimbName;
		case 2:
			strSubName = localized ? GetString(IDS_LIFT) : _T("Lift");
			return strSubName + _T(" ") + currLimbName;
		case 3:
			strSubName = localized ? GetString(IDS_LIFTOFFSET) : _T("LiftOffset");
			return strSubName + _T(" ") + currLimbName;
		default: return _T("");
		}
	}
	else
	{
		...
		{
		case 0:
			strSubName = localized ? GetString(IDS_PITCH) : _T("Pitch");
			return strSubName + _T(" ") + currLimbName;
		case 1:
			strSubName = localized ? GetString(IDS_ROLL) : _T("Roll");
			return strSubName + _T(" ") + currLimbName;
		case 2:
			strSubName = localized ? GetString(IDS_TWIST) : _T("Twist");
			return strSubName + _T(" ") + currLimbName;
		default: return _T("");
		}
	}
