...
public:
	...
	const TCHAR*	ClassName() { return _T("CATRigRootNodeCtrl"); }
	const TCHAR*	NonLocalizedClassName() { return _T("CATRigRootNodeCtrl"); }
...
public:
	...
	INT_PTR CALLBACK DlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		...
		{
			bool arePtrsValid = ctrlRootNodeCtrl && spnStartTime && spnEndTime;
			if (arePtrsValid)
			{
				Interval range = ctrlRootNodeCtrl->clip_range;
				switch (LOWORD(wParam))
...
INode* RootNodeCtrl::Initialise(ICATParentTrans* catparenttrans)
{
	...
	node->SetName((catparenttrans->GetCATName() + GetString(IDS_TRANSFORM)).data());
