...
struct tagDictionaryEntry {
	LPCTSTR name;				// the identifier's name
...
private:
	...
	std::vector<const DictionaryEntry*> entries;	// indexed by the id type
	...
	void BuildDictionary(const DictionaryEntry* table);
...
public:
	...
	// in the dictionary.
	const DictionaryEntry* Lookup(const TCHAR* s);
	...
	const DictionaryEntry* PartialLookup(const TCHAR *s, int &nFirst, int &nLength, BOOL bAllowPartialMatches = TRUE);

	// Return a pointer to the entry pointed to by the specified node.
	const DictionaryEntry* GetEntryAtNode(int nodeID);

	// Return a pointer to the specified entry.
	const DictionaryEntry* GetEntry(int entryID);
};
...
public:
	...
	const DictionaryEntry* GetEntry() const { return dictEntry; }
};
