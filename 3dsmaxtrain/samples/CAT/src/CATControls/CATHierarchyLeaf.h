...
public:
	...
	void SetActiveLayer(int activeLayer) { pblock->SetValue(PB_ACTIVELAYER, 0, std::max(0, activeLayer)); }
	...
	CATHierarchyBranch2* GetLeafParent() { 
		if (pblock)
		{
			return (CATHierarchyBranch2*)pblock->GetReferenceTarget(PB_LEAFPARENT);
		}
		return nullptr;
	}
	...
	CATHierarchyLeaf* GetWeights() { 
		if (pblock)
		{
			return (CATHierarchyLeaf*)pblock->GetReferenceTarget(PB_WEIGHTS); 
		}
		return nullptr;
	}
