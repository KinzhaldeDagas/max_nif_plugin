...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATMotionLimb"); }
...
void CATMotionLimb::RefDeleted(RefMakerHandle rm)
{
	...
	for (int i = 0; i < limb->GetNumBones(); i++) {
		...
		if (i == 0)// && !limb->GetisLeg())
		{
			...
			/*		// Put the CATMotion controller into the layer
					...
					TSTR name;
					//	GetClassName(name, true);
					ctrl->GetClassName(name, true);
...
RefTargetHandle CATMotionLimb::Clone(RemapDir& remap)
{
	...
	newCATMotionLimb->ReplaceReference(PBLOCK_REF, clonedpblock);

	// Clone manually the P_NO_REF's (previously these were not necessarily cloned here. Possible bugs introduced here).
	// PB_CATPARENT, was originally omitted and not used in the code anywhere.
	// PB_LIMB, was originally omitted, handled in post patch. We do not own these.
	// PB_CATHIERARCHY, was originally omitted, handled in post patch since we don't own this, a CATHierarchyBranch2 does.
	// PB_CATHIERARCHYROOT, was originally omitted, handled in post patch, we down own this.
	// PB_CATMOTION_STEPPING_NODES, was originally omitted, handled in post patch. We do not own this, a CATClipMatrix3 does as a layer.
...
Interval CATMotionLimb::GetStepTimeRange(TimeValue t)
{
	...
	phaseStart = std::max(std::min(0.5f, phaseStart), -0.5f);
	phaseStart = phaseStart * STEPTIME100;

	phaseEnd = pblock->GetFloat(PB_PHASEOFFSET, BaseSegEndT);
	phaseEnd = std::max(std::min(0.5f, phaseEnd), -0.5f);
...
float CATMotionLimb::GetPathOffset()
{
	...
	Control* catmotionhub = nullptr;
	LimbData2* limb = GetLimb();
	if (limb != nullptr)
	{
		Hub* hub = limb->GetHub();
		if (hub != nullptr)
		{
			CATClipMatrix3* control = hub->GetLayerTrans();
			if (control != nullptr)
			{
				catmotionhub = control->GetLayer(index);
			}
		}
	}
	DbgAssert(catmotionhub);
	// Get the path offset from CATMotionHub
	if (catmotionhub != nullptr && catmotionhub->ClassID() == GetCATMotionHub2Desc()->ClassID())
