...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATParentTrans"); }
	...
	INT_PTR CALLBACK DlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case CC_SPINNER_CHANGE: {
			...
			case IDC_SPN_CLIP_END:
				bool arePtrsValid = spnTimeRangeStart && spnTimeRangeEnd;
				if (arePtrsValid && spnTimeRangeStart->GetIVal() > spnTimeRangeEnd->GetIVal())
...
public:
			...
			do {
				if (find.dwFileAttributes != FILE_ATTRIBUTE_DIRECTORY) {
					...
					if (file && clip) {
						...
						// One of these must have failed.
						// May as well clean up even though there's no point, cos
						// the system will have crashed horribly before now.
						delete[] file;
						delete[] clip;
					}
				}
					...
					do {
						if (find.dwFileAttributes != FILE_ATTRIBUTE_DIRECTORY) {
							...
							if (file && clip) {
								...
								// One of these must have failed, for some unknown reason.
								// May as well clean up even though this may never be hit.
								delete[] file;
								delete[] clip;
							}
						}
	...
	void InitControls(HWND hDlg, IParamMap2 *map)
	{
		...
		if (!ChangeDirectory(catCfg.Get(INI_POSES_PATH)))
			ChangeDirectory(GetCOREInterface()->GetDir(APP_USER_SETTINGS_DIR).data());
...
BOOL CATParentTrans::SaveRig(const MSTR& file)
{
	...
	if (!save.ok()) {
		TSTR msgBoxText = GetString(IDS_ERR_FILENOCREATE);
		MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), msgBoxText, GetString(IDS_ERR_FILECREATE), MB_OK);
		return FALSE;
	}
...
void CATParentTrans::RefDeleted(RefMakerHandle rm)
{
...
TSTR CATParentTrans::SubAnimName(int i, bool localized)
{

	switch (i)
	{
	case PBLOCK_REF: return localized ? GetString(IDS_PARAMS) : _T("Parameters");
	case PRS: return localized ? GetString(IDS_PRS) : _T("PRS");
	case LAYERROOT: return localized ? GetString(IDS_CLIPHIERARCHY) : _T("Layers"); // "Layers"
	default: return _T("");
	}
}
...
public:
	...
	{
		// Ignore CATPARENTTRANS_CLASS_ID and CATCLIPROOT_CLASS_ID
		// There is no reason why these types should halt the test
		if(rm->ClassID() == CATPARENTTRANS_CLASS_ID || rm->ClassID() == CATCLIPROOT_CLASS_ID)
...
public:
	...
	void proc(ILoad *) {
		...
		if (GetFileSaveVersion() < CAT_VERSION_1700) {
			TSTR msgBoxText = GetString(IDS_ERR_NOLOAD_C1C3);
			MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), msgBoxText, GetString(IDS_ERR_CAT1LOAD), MB_OK);
		}
	...
	while (IO_OK == (res = iload->OpenChunk())) {
		...
		//		iload->SetObsolete();
			...
			if (dwFileSaveVersion > CAT_VERSION_CURRENT && !g_bOutOfDateWarningDisplayed) {
				...
				DWORD dwResult = MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), buf, GetString(IDS_ERR_FUTUREVER4), MB_YESNO | MB_ICONASTERISK);
...
float CATParentTrans::GetCATUnits() const {
	...
	return std::max(catunits, 0.0000001f);
}
...
void CATParentTrans::SetCATMode(CATMode mode, bool skipUIInvalidate) {
	...
	{
		// KZ 11/2022, When skipping UI update, use a less aggressive message to notify things to update. Hope things aren't broken in new and fun ways.
		CATMessage(GetCOREInterface()->GetTime(), CAT_UPDATE, -1);
	}
}
...
INode* CATParentTrans::LoadClip(TSTR file, int layerindex, int flags, TimeValue startT, CATControl* bodypart)
{
	...
	// This is so slow, disable messages to speed it up.
	// KZ 11/2023, it might be slow but it breaks needed invalidation load things properly. Removing.
	{
		// MaxReferenceMsgLock lock; // Breaks correct invalidation from loaded data.
			...
			case rigBeginGroup:
			{
				...
				case idArbBone:
				{
					...
					if (filelengthaxis != lengthaxis) {
						...
						MaxSDK::MaxMessageBox(ip->GetMAXHWnd(), buf, GetString(IDS_ERR_FILELOAD), MB_OK);
						//	theHold.Restore();
						done = TRUE;
						ok = FALSE;
						break;
					...
					if (bodypart && (load.CurIdentifier() == idTail) && bodypart->ClassID() != TAILDATA2_CLASS_ID) {
						TSTR msgBoxText = GetString(IDS_ERR_TAIL);
						MaxSDK::MaxMessageBox(ip->GetMAXHWnd(), msgBoxText, GetString(IDS_ERR_FILELOAD), MB_OK);
			...
			case rigAssignment:
				...
				case idBodyPartID: {
					...
					if (!bodypart || (0 != _tcsicmp(currbodypartid.data(), filebodypartid.data()))) {
						...
						MaxSDK::MaxMessageBox(ip->GetMAXHWnd(), msg, GetString(IDS_ERR_FILELOAD), MB_OK);
						done = TRUE;
						ok = FALSE;
						break;
					}
...
BOOL CATParentTrans::SaveClip(TSTR filename, int flags, Interval timerange, Interval layerrange, CATControl* bodypart)
{
	...
	if (!save.ok()) {
		TSTR msgBoxText = GetString(IDS_ERR_FILENOCREATE);
		MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), msgBoxText, GetString(IDS_ERR_FILECREATEERR), MB_OK);
		return FALSE;
	}
	...
	if (timerange.Start() != FOREVER.Start() && timerange.End() != FOREVER.End()) {
		...
		timerange = GetCOREInterface()->GetAnimRange();
		animLength = timerange.End() - timerange.Start();
	}
	...
	// Removing this didn't effect the saving of keys that landed on the boundary times.
...
INode* CATParentTrans::LoadCATBinaryRig(const MSTR& file, MSTR& name, Matrix3 * tm)
{
		...
		__CATCH(...)
	{
		if (!pCATCreateCB->IsCreating())
		{
			TSTR msgBoxText = GetString(IDS_ERR_RIGUPDATE);
			MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), msgBoxText, _T("If you see this, something is not working")/*GetString(IDS_ERR_RIGUPDATE2)*/, MB_OK);
		}
		else
		{
			TSTR msgBoxText = GetString(IDS_ERR_RIGLOAD);
			MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), msgBoxText, GetString(IDS_ERR_RIGLOAD2), MB_OK);
		}
	...
	TimeValue start_t, TimeValue end_t, TimeValue &t)
{
	...
	t = std::min(nextpkeyt, std::min(nextrkeyt, std::min(nextskeyt, nextkeyt)));
}
...
public:
	...
	virtual	bool proc(Animatable *theAnim)
	{
		...
		else
		{
			...
			{
				CATParent* pParent = dynamic_cast<CATParent*>(theAnim);
				if (pParent != NULL)
				{
					__TRY
					{
						// Clean the pointer on any CATParent
						if (pParent->GetCATParentTrans(false) == mpDeadPtr)
							pParent->SetCATParentTrans(NULL);
					}
					__CATCH(...)
					{
					}
...
void FillDefDigitSegs(DigitData* pDigit, INode* defStore[3])
{
	...
	int nDigitSegs = std::min(pDigit->GetNumBones(), 3);
...
void FillDefDigit(PalmTrans2* pPalm, INode* defStore[5][3])
{
	...
	int nDigits = std::min(pPalm->GetNumDigits(), 5);
