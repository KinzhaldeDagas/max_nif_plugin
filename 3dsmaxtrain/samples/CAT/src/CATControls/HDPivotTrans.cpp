...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATHDPivotTrans"); }
...
TSTR HDPivotTrans::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case SUB_PRS: return localized ? GetString(IDS_PRS) : _T("PRS");
	case SUB_PIVOT: return localized ? GetString(IDS_PIVOT1) : _T("Pivot");
#ifdef _DEBUG
	case SUB_WSPIVOT: return localized ? GetString(IDS_WSPIVOT) : _T("WSPivot");
#endif
	case SUB_PBLOCK: return localized ? GetString(IDS_PBLOCK) : _T("pblock");
	default: return _T("");
	}
}
...
int HDPivotTrans::Display(TimeValue, INode*, ViewExp *vpt, int flags)
{
	...
	if (node && (catnodecontroller = (CATNodeControl*)node->GetTMController()->GetInterface(I_CATNODECONTROL)) != NULL) {
		if (catnodecontroller->GetCATParentTrans())
