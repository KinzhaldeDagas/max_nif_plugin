...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("CATTransformOffset"); }
...
void CATTransformOffset::GetValue(TimeValue t, void *val, Interval &valid, GetSetMethod method)
{
	...
	if (offset->GetPositionController() && offset->GetRotationController() && offset->GetScaleController())
	{
		if (flags&CATTMOFFSET_FLAG_APPLY_OFFSET_IN_XY_PLANE) {
			...
			AngAxis ax;
			ax.angle = -acos((float)std::min(1.0f, DotProd(pos_par.GetRow(Z), P3_UP_VEC)));
...
TSTR CATTransformOffset::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case TRANSFORM: return localized ? GetString(IDS_TRANSFORM) : _T("Transform");
	case OFFSET: return localized ? GetString(IDS_OFFSET) : _T("Offset");
	default: return _T("");
	}
}
