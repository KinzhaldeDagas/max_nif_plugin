...
TCHAR *GetUserSettingsPath(const TCHAR *filename/*=NULL*/)
{
	if (!filename) _sntprintf_s(pPathBuffer, MAX_PATH, _T("%s\\CAT"), GetCOREInterface()->GetDir(APP_USER_SETTINGS_DIR).data());
	else _sntprintf_s(pPathBuffer, MAX_PATH, _T("%s\\CAT\\%s"), GetCOREInterface()->GetDir(APP_USER_SETTINGS_DIR).data(), filename);
	return pPathBuffer;
}
...
BOOL DialogOpenRigPreset(HINSTANCE hInstance, HWND hWnd, TCHAR* fileBuf, int bufSize)
{
	...
	if (fileBuf[0] != _T('\0'))
	{
		...
		_tcscpy_s(fileBuf, bufSize, fname.data());
		_tcscat_s(fileBuf, bufSize, extension.data());
	}
...
BOOL DialogSaveRigPreset(HINSTANCE hInstance, HWND hWnd, TCHAR* fileBuf, int bufSize, const TCHAR* initialName)
{
	TSTR path, fname, extension;
...
BOOL DialogSaveMotionPreset(HINSTANCE hInstance, HWND hWnd, TCHAR *fileBuf, int bufSize, const TCHAR *initialName/*=NULL*/, TCHAR *initialDir/*=NULL*/)
{
	fileBuf[0] = _T('\0');
	OPENFILENAME ofn;
...
BOOL DialogSaveHandPosePreset(HINSTANCE hInstance, HWND hWnd, TCHAR *fileBuf, int bufSize, const TCHAR *initialName/*=NULL*/, TCHAR *initialDir/*=NULL*/)
{
	fileBuf[0] = _T('\0');
	OPENFILENAME ofn;
