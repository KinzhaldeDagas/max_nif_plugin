...
#define DECL_FUNC_NO_INTERVAL_2(funcName) \
	auto funcName(TimeValue t, int which, Interval&& valid = FOREVER) { return funcName(t, which, valid); }
...
{			   
	...
	static MaxSDK::SharedMeshPtr 	smStaticMesh[2];
	static bool smStaticMeshesInitialized;
...
//	static float 	dlgRayBias;
	...
	float 	atmosOpacity;
	float 	atmosColAmt;
	MaxSDK::SharedMeshPtr	mSpotSharedMesh = nullptr;
	MaxSDK::SharedMeshPtr	mpSharedMesh = nullptr;
	ExclList exclList;
...
public:
	...
	static void UpdateUICheckbox( HWND hwnd, int dlgItem, const TCHAR *name, int val );
	...
	BOOL 	IsDaylightOrSunlightSystem();
#ifdef SIMPLEDIR
	...
	float GetHotspot(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetHotspot)
	void 	SetFallsize(	TimeValue time, float f );
	float GetFallsize(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetFallsize)
	void 	SetAtten(		TimeValue time, int which, float f );
	float GetAtten(TimeValue t, int which, Interval& valid);
	DECL_FUNC_NO_INTERVAL_2(GetAtten)
	void 	SetTDist(		TimeValue time, float f );
	float GetTDist(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetTDist)
	void 	SetDecayRadius(	TimeValue time, float f );
	float GetDecayRadius(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetDecayRadius)
	...
	Point3 GetRGBColor(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetRGBColor)
	void 	SetHSVColor(	TimeValue t, Point3& hsv );
	Point3 GetHSVColor(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetHSVColor)
	void 	SetShadColor(	TimeValue t, const Point3& rgb ) override;
	Point3 GetShadColor(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetShadColor)
	void	SetShadMult(	TimeValue t, float m );
    float GetShadMult(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetShadMult)
	...
	float GetIntensity(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetIntensity)
	void 	SetContrast( 	TimeValue time, float f );
	float GetContrast(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetContrast)
	void 	SetDiffuseSoft(	TimeValue time, float f );
	float GetDiffuseSoft(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetDiffuseSoft)
	void 	SetAspect(		TimeValue t, float f );
	float GetAspect(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetAspect)
	...
	float GetMapBias(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetMapBias)
	void 	SetMapRange(	TimeValue t, float f );
	float GetMapRange(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetMapRange)
	void 	SetMapSize(		TimeValue t, int f );
	int GetMapSize(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetMapSize)
	void 	SetRayBias(		TimeValue t, float f );
	float GetRayBias(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetRayBias)
	...
	float GetAtmosOpacity(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetAtmosOpacity)
	void 	SetAtmosColAmt(			TimeValue t, float f );
	float GetAtmosColAmt(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetAtmosColAmt)
...
#endif	// !PRIVATE_INCOMPATIBLE_OMNI_LIGHT
	void 	GetClassName(MSTR& s, bool localized) const override 	{ s = localized ? GetString(IDS_DB_LIGHT_CLASS) : _T("Light"); }
	...
	TSTR SubAnimName(int i, bool localized) override
	{
		...
		case PROJMAP_REF:     if (projMap)     return (localized ? TSTR(GetString(IDS_DS_PROJMAP)) : TSTR(_T("Projection Map"))) + _T(": ") + projMap->GetFullName(localized);
									 else             return localized ? GetString(IDS_DS_PROJMAP) : TSTR(_T("Projection Map"));
		case SHADPROJMAP_REF: if (shadProjMap) return (localized ? TSTR(GetString(IDS_DS_SHADPROJMAP)) : TSTR(_T("Shadow Projection Map"))) + _T(": ") + shadProjMap->GetFullName(localized);
									 else             return localized ? GetString(IDS_DS_SHADPROJMAP) : TSTR(_T("Shadow Projection Map"));
#else
		case PROJMAP_REF:     return localized ? TSTR(GetString(IDS_DS_PROJMAP)) : _T("Projection Map");
		case SHADPROJMAP_REF: return localized ? TSTR(GetString(IDS_DS_SHADPROJMAP)) : _T("Shadow Projection Map");
#endif

		case SHADTYPE_REF:    return localized ? GetString(IDS_DS_SHAD_GEN) : _T("Shadow Parameters");
		default:              return _T("");
		}
	}
