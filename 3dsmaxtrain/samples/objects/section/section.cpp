...
static const int SHAPE_OBJ_PARAM_BLOCK_ID   = SHAPEOBJPBLOCK; // A macro!! defined in splshape.h, yet the class it defines it for is in object.h. How unexpected.
...
public:
	...
	void GetClassName(MSTR& s, bool localized) const override { s = _T("LinesShape"); }
...
//------------------------------------------------------------------
	...
	public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_DB_SECTIONER) : _T("Sectioner"); }
...
public:
		...
		const TCHAR* GetObjectName(bool localized) const override	{ return localized ? GetString(IDS_OBJECTNAME) : _T("Section"); }
...
{	
	...
	{
		m_pmapCreate->SetUserDlgProc(nullptr);
		m_pmapCreate->SetParamBlock(nullptr);
		m_pmapParam->SetParamBlock(nullptr);
	}
}
...
static INT_PTR CALLBACK nameDlgProc(HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	switch (msg) {
		case WM_INITDIALOG:
			...
			SendMessage(GetDlgItem(hWnd, IDC_NAME), WM_SETTEXT, 0, (LPARAM)(LPCTSTR)name->data());
			break;
		case WM_COMMAND:
			switch (LOWORD(wParam)) {
			case IDOK: {
				...
				SendMessage(GetDlgItem(hWnd, IDC_NAME), WM_GETTEXT, strLen+1, (LPARAM)(LPCTSTR)name->data());
				EndDialog(hWnd, 1);
				break;
			}
			case IDCANCEL:
...
void SectionObject::TraverseWorld(INode *node, const int level, Box3& boundBox)
{
	...
	if (m_SectionScope == SECTION_SCOPE_LIMIT) 
	{
		...
		if (obj) {
			Box3 nodeBBox;
			auto objectTM = node->GetObjectTM(m_curTime);
			obj->GetDeformBBox(m_curTime, nodeBBox, &objectTM);
...
void SectionObject::SliceASceneNode(void)
{
		...
		os.obj->SuperClassID() == GEOMOBJECT_CLASS_ID) 
	{
		...
		{
			geom_obj->GetLocalBoundBox( m_curTime, m_curNode, view.ToPointer(), bbox );
		}
		else
		{
			geom_obj->GetWorldBoundBox( m_curTime, m_curNode, view.ToPointer(), bbox );
		}
...
//  this weld operation is fairly naive
//  here's what it does
...
//  And the greedy algorithm results in bad sections for some cases, esp. patches
...
TSTR SectionObject::GetParameterName(int pbIndex, bool localized)
{
	switch (pbIndex)
	{
	case PB_LENGTH: return localized ? GetString(IDS_PARAM_LENGTH) : _T("Length");
	case PB_WIDTH:  return localized ? GetString(IDS_PARAM_WIDTH)  : _T("Width");
	default:        return _T("");
	}
}
...
const int ID_EXECUTE_VIEWEXP_DISABLE_INVALIDATION = 0x8001;
const int ID_EXECUTE_VIEWEXP_RESTORE_INVALIDATION = 0x8002;

class DisableViewportInvalidationGuard
{
	ViewExp* mpView;
public:
	DisableViewportInvalidationGuard(ViewExp* pView) : mpView(pView)
	{
		if (NULL != mpView)
		{
			mpView->Execute(ID_EXECUTE_VIEWEXP_DISABLE_INVALIDATION);
		}
	}

	~DisableViewportInvalidationGuard()
	{
		if (NULL != mpView)
		{
			mpView->Execute(ID_EXECUTE_VIEWEXP_RESTORE_INVALIDATION);
		}
	}
};
...
int SectionObject::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags) 
{
	...
	*/
	...
	// If UpdateSlice performs an update, it calls NotifyDependents; suppress viewport invalidation
	DisableViewportInvalidationGuard guard(vpt);
	UpdateSlice(inode,t);
	...
	MeshRenderHelper(shape, gw, ma, flags, vpt, COMP_ALL | (inode->Selected()?COMP_OBJSELECTED:0), 1);
...
RefResult SectionObject::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
     PartID& partID, RefMessage message, BOOL propagate ) 
{
#ifdef DEBUG_REFS
...
TSTR SectionObject::SubAnimName(int i, bool localized)
{
	if (i < SHAPE_OBJ_NUM_SUBS)
	{
		return localized ? GetString(IDS_DB_CREATE_VALS) : _T("Section Parameters");
	}
	else
	{
		return _T("");
...
class SectionObjClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Section"); }
	SClass_ID		SuperClassID()	{ return SHAPE_CLASS_ID; }
	Class_ID		ClassID()		{ return sectionClassID; } 
...
class LinesClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Lines"); }
	SClass_ID		SuperClassID()	{ return SHAPE_CLASS_ID; }
	Class_ID		ClassID()		{ return linesClassID; };
	const TCHAR* 	Category()		{ return GetString(IDS_DB_GENERAL);  }
	};
