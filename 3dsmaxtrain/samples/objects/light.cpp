...
// The unique 32-bit Class IDs of the driven controller
#define DRIVEN_CONTROL_CID1 0x77e3272e
#define DRIVEN_CONTROL_CID2 0x13747060

#define DAYLIGHT_DRIVEN_CONTROL_CID1 0x35924ed0
#define DAYLIGHT_DRIVEN_CONTROL_CID2 0x59cf079c
...
class OmniLightClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Omni"); }
...
class TSpotLightClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Target Spot"); }
	SClass_ID		SuperClassID() { return LIGHT_CLASS_ID; }
...
class DirLightClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Free Direct"); }
	SClass_ID		SuperClassID() { return LIGHT_CLASS_ID; }
...
class TDirLightClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Target Direct"); }
	SClass_ID		SuperClassID() { return LIGHT_CLASS_ID; }
...
class FSpotLightClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Free Spot"); }
	SClass_ID		SuperClassID() { return LIGHT_CLASS_ID; }
...
MaxSDK::SharedMeshPtr GeneralLight::smStaticMesh[2];
bool GeneralLight::smStaticMeshesInitialized = false;
...
*/
		...
		case IDC_LFALLOFFSPINNER:
			...
			gl->BuildSpotMesh(std::max(gl->hotsizeSpin->GetFVal(), gl->fallsizeSpin->GetFVal()));
			break;
		...
		{	
			...
			switch (id) 
			{
				case IDC_LIGHT_COLOR: 
					{
						AColor rgba = gl->colorSwatch->GetAColor();
						gl->SetRGBColor(t, Point3(rgba));
					}
					break;
				case IDC_SHADOW_COLOR: 
					{
						AColor rgba = gl->shadColorSwatch->GetAColor();
						gl->SetShadColor(t, Point3(rgba));
					}
					break;
			}
...
*/
		...
		Color color = Color(GetShadColor(t));
		...
		shadProjMapName->SetText(shadProjMap?shadProjMap->GetFullName(true).data():GetString(IDS_DB_NONE));
...
void GeneralLight::AssignProjectorMap(Texmap *m, BOOL newmat) {	
	...
	if (projMap) {
		projMapName->SetText(projMap->GetFullName(true).data());
		if (newmat) 
...
void GeneralLight::AssignShadProjMap(Texmap *m, BOOL newmat) {	
	...
	if (shadProjMap) {
		shadProjMapName->SetText(shadProjMap->GetFullName(true).data());
		if (newmat) 
...
const TCHAR *GeneralLight::GetObjectName(bool localized) const
{ 
	switch(type) {
	case OMNI_LIGHT:  return localized ? GetString(IDS_DB_OMNI_LIGHT)  : _T("Omni Light");
	case DIR_LIGHT:   return localized ? GetString(IDS_DB_DIR_LIGHT)   : _T("Free Directional Light");
	case TDIR_LIGHT:  return localized ? GetString(IDS_DS_TDIR_LIGHT)  : _T("Target Directional Light");
	case FSPOT_LIGHT: return localized ? GetString(IDS_DB_FREE_SPOT)   : _T("Free Spot");
	case TSPOT_LIGHT: return localized ? GetString(IDS_DB_TARGET_SPOT) : _T("Target Spot");
	default:          return NULL;
	}
}
...
#define SET_QUAD(face, v0, v1, v2, v3) \
	smStaticMesh[DIR_MESH]->GetMesh().faces[face].setVerts(v0, v1, v2); \
	smStaticMesh[DIR_MESH]->GetMesh().faces[face].setEdgeVisFlags(1,1,0); \
	smStaticMesh[DIR_MESH]->GetMesh().faces[face+1].setVerts(v0, v2, v3); \
	smStaticMesh[DIR_MESH]->GetMesh().faces[face+1].setEdgeVisFlags(0,1,1);
...
void GeneralLight::BuildStaticMeshes()
{
	if(!meshBuilt) {
		...
		smStaticMesh[OMNI_MESH]->GetMesh().setNumVerts(nverts);
		smStaticMesh[OMNI_MESH]->GetMesh().setNumFaces(nfaces);
		float s = 8.0f;
		smStaticMesh[OMNI_MESH]->GetMesh().setVert(0, Point3( FZ,FZ, -s));
		smStaticMesh[OMNI_MESH]->GetMesh().setVert(1, Point3( s, FZ, FZ));
		smStaticMesh[OMNI_MESH]->GetMesh().setVert(2, Point3( FZ, s, FZ));
		smStaticMesh[OMNI_MESH]->GetMesh().setVert(3, Point3(-s, FZ, FZ));
		smStaticMesh[OMNI_MESH]->GetMesh().setVert(4, Point3( FZ,-s, FZ));
		smStaticMesh[OMNI_MESH]->GetMesh().setVert(5, Point3( FZ,FZ,  s));
		smStaticMesh[OMNI_MESH]->GetMesh().faces[0].setVerts(0,1,4);
		smStaticMesh[OMNI_MESH]->GetMesh().faces[1].setVerts(0,4,3);
		smStaticMesh[OMNI_MESH]->GetMesh().faces[2].setVerts(0,3,2);
		smStaticMesh[OMNI_MESH]->GetMesh().faces[3].setVerts(0,2,1);
		smStaticMesh[OMNI_MESH]->GetMesh().faces[4].setVerts(5,1,2);
		smStaticMesh[OMNI_MESH]->GetMesh().faces[5].setVerts(5,2,3);
		smStaticMesh[OMNI_MESH]->GetMesh().faces[6].setVerts(5,3,4);
		smStaticMesh[OMNI_MESH]->GetMesh().faces[7].setVerts(5,4,1);
		for (int i=0; i<nfaces; i++) {
			smStaticMesh[OMNI_MESH]->GetMesh().faces[i].setSmGroup(i);
			smStaticMesh[OMNI_MESH]->GetMesh().faces[i].setEdgeVisFlags(1,1,1);
			}
		smStaticMesh[OMNI_MESH]->GetMesh().buildNormals();
		smStaticMesh[OMNI_MESH]->GetMesh().EnableEdgeList(1);

		// Build an "arrow"
		nverts = 13;
		nfaces = 16;
		smStaticMesh[DIR_MESH]->GetMesh().setNumVerts(nverts);
		smStaticMesh[DIR_MESH]->GetMesh().setNumFaces(nfaces);
		s = 4.0f;
		float s4 = 16.0f;
		smStaticMesh[DIR_MESH]->GetMesh().setVert( 0, Point3( -s,-s, FZ));
		smStaticMesh[DIR_MESH]->GetMesh().setVert( 1, Point3(  s,-s, FZ));
		smStaticMesh[DIR_MESH]->GetMesh().setVert( 2, Point3(  s, s, FZ));
		smStaticMesh[DIR_MESH]->GetMesh().setVert( 3, Point3( -s, s, FZ));
		smStaticMesh[DIR_MESH]->GetMesh().setVert( 4, Point3( -s,-s, -s4));
		smStaticMesh[DIR_MESH]->GetMesh().setVert( 5, Point3(  s,-s, -s4));
		smStaticMesh[DIR_MESH]->GetMesh().setVert( 6, Point3(  s, s, -s4));
		smStaticMesh[DIR_MESH]->GetMesh().setVert( 7, Point3( -s, s, -s4));
		s *= (float)2.0;
		smStaticMesh[DIR_MESH]->GetMesh().setVert( 8, Point3( -s,-s, -s4));
		smStaticMesh[DIR_MESH]->GetMesh().setVert( 9, Point3(  s,-s, -s4));
		smStaticMesh[DIR_MESH]->GetMesh().setVert(10, Point3(  s, s, -s4));
		smStaticMesh[DIR_MESH]->GetMesh().setVert(11, Point3( -s, s, -s4));
		smStaticMesh[DIR_MESH]->GetMesh().setVert(12, Point3( FZ,FZ, -s4-s));
		SET_QUAD( 0, 1, 0, 4, 5);
		SET_QUAD( 2, 0, 3, 7, 4);
		SET_QUAD( 4, 3, 2, 6, 7);
		SET_QUAD( 6, 2, 1, 5, 6);
		...
		smStaticMesh[DIR_MESH]->GetMesh().faces[12].setVerts(8,12,9);
		smStaticMesh[DIR_MESH]->GetMesh().faces[12].setEdgeVisFlags(1,1,1);
		smStaticMesh[DIR_MESH]->GetMesh().faces[13].setVerts(9,12,10);
		smStaticMesh[DIR_MESH]->GetMesh().faces[13].setEdgeVisFlags(1,1,1);
		smStaticMesh[DIR_MESH]->GetMesh().faces[14].setVerts(10,12,11);
		smStaticMesh[DIR_MESH]->GetMesh().faces[14].setEdgeVisFlags(1,1,1);
		smStaticMesh[DIR_MESH]->GetMesh().faces[15].setVerts(11,12,8);
		smStaticMesh[DIR_MESH]->GetMesh().faces[15].setEdgeVisFlags(1,1,1);
      for (int i=0; i<nfaces; i++)
			smStaticMesh[DIR_MESH]->GetMesh().faces[i].setSmGroup(i);
		smStaticMesh[DIR_MESH]->GetMesh().buildNormals();
		smStaticMesh[DIR_MESH]->GetMesh().EnableEdgeList(1);
...
void GeneralLight::BuildSpotMesh(float coneSize)
{
	...
	int nverts = 9;
	int nfaces = 8;
	mSpotSharedMesh->GetMesh().setNumVerts(nverts);
	mSpotSharedMesh->GetMesh().setNumFaces(nfaces);
	...
	mSpotSharedMesh->GetMesh().setVert(0, Point3( FZ, FZ, FZ));
	mSpotSharedMesh->GetMesh().setVert(1, Point3( -r, FZ, d));
	mSpotSharedMesh->GetMesh().setVert(2, Point3( -s, -s, d));
	mSpotSharedMesh->GetMesh().setVert(3, Point3( FZ, -r, d));
	mSpotSharedMesh->GetMesh().setVert(4, Point3(  s, -s, d));
	mSpotSharedMesh->GetMesh().setVert(5, Point3(  r, FZ, d));
	mSpotSharedMesh->GetMesh().setVert(6, Point3(  s,  s, d));
	mSpotSharedMesh->GetMesh().setVert(7, Point3( FZ,  r, d));
	mSpotSharedMesh->GetMesh().setVert(8, Point3( -s,  s, d));
	mSpotSharedMesh->GetMesh().faces[0].setVerts(0,1,2);
	mSpotSharedMesh->GetMesh().faces[1].setVerts(0,2,3);
	mSpotSharedMesh->GetMesh().faces[2].setVerts(0,3,4);
	mSpotSharedMesh->GetMesh().faces[3].setVerts(0,4,5);
	mSpotSharedMesh->GetMesh().faces[4].setVerts(0,5,6);
	mSpotSharedMesh->GetMesh().faces[5].setVerts(0,6,7);
	mSpotSharedMesh->GetMesh().faces[6].setVerts(0,7,8);
	mSpotSharedMesh->GetMesh().faces[7].setVerts(0,8,1);
	for (int i=0; i<nfaces; i++) {
		mSpotSharedMesh->GetMesh().faces[i].setSmGroup(i);
		mSpotSharedMesh->GetMesh().faces[i].setEdgeVisFlags(1,1,1);
	}
	mSpotSharedMesh->GetMesh().buildNormals();
	mSpotSharedMesh->GetMesh().EnableEdgeList(1);
}
...
void GeneralLight::UpdateUI(TimeValue t)
	{
	if ( hGeneralLight && !waitPostLoad && GetWindowLongPtr(hGeneralLight,GWLP_USERDATA)==(LONG_PTR)this && pblock) 
	{
		Color colrgb = Color(GetRGBColor(t));
//		redSpin->SetValue( FLto255f(color.x), FALSE );
//		greenSpin->SetValue( FLto255f(color.y), FALSE );
//		blueSpin->SetValue( FLto255f(color.z), FALSE );
		colorSwatch->SetColor(colrgb);
...
//		CheckDlgButton( hShadow, IDC_OBJECT_SHADOWS, GetShadow() );
		...
		if( IsSpot() ) {
			...
			BuildSpotMesh(std::max(hot, fall));
		...
		Color colShadow = Color(GetShadColor(t));
		shadColorSwatch->SetColor(colShadow);
		...
		if (projMapName)
			projMapName->SetText(projMap?projMap->GetFullName(true).data():GetString(IDS_DB_NONE));
		if (shadProjMapName)
			shadProjMapName->SetText(shadProjMap?shadProjMap->GetFullName(true).data():GetString(IDS_DB_NONE));
		// DS 8/28/00
		// added: DC 08/03/2001
...
void GeneralLight::BuildMeshes(BOOL isnew) {
	if( IsSpot() ) {
		...
		if(type == DIR_LIGHT||type == TDIR_LIGHT) {
			BuildStaticMeshes();
			mpSharedMesh = smStaticMesh[DIR_MESH];
			}
		else
			mpSharedMesh = mSpotSharedMesh;
		}
	else {
		BuildStaticMeshes();
		mpSharedMesh = smStaticMesh[type == OMNI_LIGHT ? OMNI_MESH : DIR_MESH];
		}
	}
...
{
	if (false == smStaticMeshesInitialized){
		for (int i=0;i<_countof(smStaticMesh); ++i){
			smStaticMesh[i] = new MaxSDK::SharedMesh();
		}
		smStaticMeshesInitialized = true;
	}

	mSpotSharedMesh = new MaxSDK::SharedMesh();
...
BOOL GeneralLight::IsDaylightOrSunlightSystem() {
	if (!HasTarg(type)) {
		...
		while ((rm=di.Next()) != NULL) {	
			...
			if (nd) {
				if (nd->GetTMController()->ClassID()==Class_ID(DAYLIGHT_DRIVEN_CONTROL_CID1,DAYLIGHT_DRIVEN_CONTROL_CID2) // Daylight system
					|| nd->GetTMController()->ClassID()==Class_ID(DRIVEN_CONTROL_CID1,DRIVEN_CONTROL_CID2)) //Sunlight system
				return TRUE;
				}
			}
		}
...
// ReplaceReference( AS_SHADTYPE_REF, NULL );
	...
	if (IsSpot()&&!IsDir()) {
		...
		SuspendAnimate();
		SetHotspot(t, GetHotspot(t));
		SetFallsize(t, GetFallsize(t));
		ResumeAnimate();
		}
...
class ShadTypeRest: public RestoreObj, public ReferenceMaker {
	public:
		...
		virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("ShadTypeRest"); }
...
void GeneralLight::SetRGBColor(TimeValue t, const Point3& rgb) 
{
	Point3 rgbCopy = rgb;
	pblock->SetValue(PB_COLOR, t, rgbCopy);
	NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
}
...
void GeneralLight::SetShadColor(TimeValue t, const Point3 &rgb) 
{
	Point3 copy = rgb;
	pblock->SetValue(type == OMNI_LIGHT ? PB_OMNISHADCOLOR : PB_SHADCOLOR, t, copy);
	NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
}
...
void GeneralLight::SetShadow( int a ) 				
{
	...
	shadow = a;
	if (currentEditLight==this) {
		UpdateUICheckbox( hGeneralLight, IDC_CAST_SHADOWS, _T("baseobject.castShadows"), a ); // castShadows is also a node level property
	...
	if (type == TSPOT_LIGHT) {
		...
		if (GetTargetPoint(t, inode, pt)) {
			...
			if(!inode->IsFrozen() && !inode->Dependent())
			{
				...
				// otherwise, use the standard target-line color.
				Color color(inode->GetWireColor());
				if (color != Color(GetUIColor(COLOR_LIGHT_OBJ)))
...
void GeneralLight::UpdateUICheckbox( HWND hwnd, int dlgItem, const TCHAR *name, int val )
{
	if ( hwnd )
	{
...
{
	box = mpSharedMesh->GetMesh().getBoundingBox(tm);
}
...
void GeneralLight::GetLocalBoundBox(TimeValue t, INode* inode, ViewExp* vpt, Box3& box )
{
	...
	box = mpSharedMesh->GetMesh().getBoundingBox();//same as GetDeformBBox
...
void GeneralLight::GetWorldBoundBox(TimeValue t, INode* inode, ViewExp* vpt, Box3& box )
	...
	nv = mpSharedMesh->GetMesh().getNumVerts();
	box.Init();
	if(!(extDispFlags & EXT_DISP_ZOOM_EXT)) 
		box.IncludePoints(mpSharedMesh->GetMesh().verts,nv,&tm);
	else
		box += loc;
	tm = inode->GetObjectTM(t);
...
int GeneralLight::HitTest(TimeValue t, INode* inode, int type, int crossing, int flags, IPoint2 *p, ViewExp *vpt) 
{
	...
	res = mpSharedMesh->GetMesh().select( gw, mtl, &hitRegion, flags & HIT_ABORTONHIT);
	...
	MaxSDK::Graphics::IRenderItemContainer& targetRenderItemContainer)
{
	...
	MaxSDK::Graphics::Utilities::MeshEdgeRenderItemPtr pMeshItem = new MaxSDK::Graphics::Utilities::MeshEdgeRenderItem(mpSharedMesh, true);
	if (pNode->Dependent())
	{
...
int GeneralLight::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags) 
{
	...
	MeshRenderHelper(mpSharedMesh->GetMesh(), gw, gw->getMaterial(), flags, vpt, COMP_ALL);
...
RefResult GeneralLight::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate ) 
{
	...
	case REFMSG_GET_PARAM_NAME_NONLOCALIZED: {
		...
		if ( hTarget == ( RefTargetHandle )pblock )
		{
			if (type == OMNI_LIGHT) {
				switch (gpn->index) {
					case PB_COLOR:             gpn->name = _T("Color");                   break;
					case PB_INTENSITY:         gpn->name = _T("Multiplier");              break;
					case PB_CONTRAST:          gpn->name = _T("Contrast");                break;
					case PB_DIFFSOFT:          gpn->name = _T("Diffuse Soften");          break;
					case PB_OMNIATSTART1:      gpn->name = _T("Attenuation Near Start");  break;
					case PB_OMNIATEND1:        gpn->name = _T("Attenuation Near End");    break;
					case PB_OMNIATSTART:       gpn->name = _T("Attenuation Far Start");   break;
					case PB_OMNIATEND:         gpn->name = _T("Attenuation Far End");     break;
					case PB_OMNIDECAY:         gpn->name = _T("Decay Falloff");           break;
					case PB_OMNISHADCOLOR:     gpn->name = _T("Shadow Color");            break;
					case PB_OMNIATMOS_OPACITY: gpn->name = _T("Atmosphere Opacity");      break;
					case PB_OMNIATMOS_COLAMT:  gpn->name = _T("Atmosphere Color Amount"); break;
					case PB_OMNISHADMULT:      gpn->name = _T("Shadow Density");          break;
					}
				}
			else {
				switch (gpn->index) {
					...
					case PB_CONTRAST:      gpn->name = _T("Contrast");                break;
					case PB_DIFFSOFT:      gpn->name = _T("Diffuse Soften");          break;
					case PB_HOTSIZE:       gpn->name = _T("Hotspot");                 break;
					case PB_FALLSIZE:      gpn->name = _T("Falloff");                 break;
					case PB_ASPECT:        gpn->name = _T("Aspect Ratio");            break;
					case PB_ATTENSTART1:   gpn->name = _T("Attenuation Near Start");  break;
					case PB_ATTENEND1:     gpn->name = _T("Attenuation Near End");    break;
					case PB_ATTENSTART:    gpn->name = _T("Attenuation Far Start");   break;
					case PB_ATTENEND:      gpn->name = _T("Attenuation Far End");     break;
					case PB_DECAY:         gpn->name = _T("Decay Falloff");           break;
					case PB_TDIST:         gpn->name = _T("Target Distance");         break;
					case PB_SHADCOLOR:     gpn->name = _T("Shadow Color");            break;
					case PB_ATMOS_OPACITY: gpn->name = _T("Atmosphere Opacity");      break;
					case PB_ATMOS_COLAMT:  gpn->name = _T("Atmosphere Color Amount"); break;
					case PB_SHADMULT:      gpn->name = _T("Shadow Density");          break;
					}
				}
		}
		return REF_HALT;
...
void GeneralLight::FixOldVersions(ILoad *iload) {
	...
	BuildSpotMesh(std::max(GetHotspot(0), GetFallsize(0)));
	}
...
private:
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("TSpotCreationManager"); }
...
class SCOmni: public ShadeContext {
	public:
		...
		Matrix3 MatrixTo(RefFrame ito) { return Matrix3::Identity; }
		Matrix3 MatrixFrom(RefFrame ifrom) { return Matrix3::Identity; }
...
//		projMap = gl->GetProjMap();
		if( gl->projMap ) gl->projMap->Update(t);
	}
	if (shadow)
	{
		if (dontNeedShadBuf)
...
void GeneralLight::InitICAParamDialog( TimeValue t,  IObjParam *ip, ULONG flags,Animatable *prev )
{
	...
	Color color = Color(GetRGBColor(t));
