...
class GridObject : public SimpleObject2, public RealWorldMapSizeInterface {
	...
	public:
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_PW_GRID_OBJECT) : _T("Plane"); }
...
class GridObjClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Plane"); }
...
static_assert(grid_params == GRIDHELP_PARAMBLOCK_ID, "");
...
static ParamBlockDesc2 grid_param_blk ( grid_params, _T("GridParameters"),  0, &gridObjDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF_NO, 
	...
	grid_rscale, 	_T("renderScale"), 			TYPE_FLOAT, 		P_ANIMATABLE, 	IDS_PW_SCALE, 
		...
		p_nonLocalizedName, _T("Render Scale"), 
		p_end, 
...
//195843 watje to handle out of memory problem on dense meshes
	if (!notAllocated)
		...
		int res = MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), buf1, buf2, MB_OK | MB_ICONEXCLAMATION);
		return;
		}
