...
public:
	...
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_TH_TRIPATCH) : _T("TriPatch"); }
	...
	void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_TH_TRIPATCHOBJECT_CLASS) : _T("TriPatchObject"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Tri Patch"); }
...
INT_PTR TriPatchTypeInDlgProc::DlgProc(
...
// from IDisplay
unsigned long TriPatchObject::GetObjectDisplayRequirement() const 
{
...
TSTR TriPatchObject::GetParameterName(int pbIndex, bool localized)
{
	switch (pbIndex) {
	...
	BOOL propagate )
{
	...
	case REFMSG_CHANGE:
		...
		// Edit the partID for our dependents, they need to know we are invalidating our points
		partID |= PART_GEOM;
		break;
