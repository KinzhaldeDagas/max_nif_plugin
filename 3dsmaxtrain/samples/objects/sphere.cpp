...
public:			
	...
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_SPHERE) : _T("Sphere"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Sphere"); }
...
static_assert(sphere_params == SPHERE_PARAMBLOCK_ID, "");
...
static ParamBlockDesc2 sphere_param_blk(sphere_params, _T("SphereParameters"), 0, &sphereDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF_NO,
	...
	p_nonLocalizedName, _T("Slice From"),
	p_end,
	...
	p_nonLocalizedName, _T("Slice To"),
	p_end,
	p_end
	);
...
void MakeMeshCapTexture(Mesh &mesh, const Matrix3 &itm, int fstart, int fend, BOOL usePhysUVs)
{
	if(fstart == fend)
		return;
...
void MakeMeshCapTexture(Mesh &mesh, const Matrix3 &itm, BitArray& capFaces, BOOL usePhysUVs) {
...
{
	float radius;
	pblock2->GetValue(sphere_radius,t,radius);
...
{
	int smooth, recenter;
	pblock2->GetValue(sphere_smooth,t,smooth);
	pblock2->GetValue(sphere_recenter,t,recenter);	
	float hemi;
	pblock2->GetValue(sphere_hemi,t,hemi);
	...
	pblock2->GetValue(sphere_radius,t,r);
