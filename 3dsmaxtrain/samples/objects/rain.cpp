...
class RainParticle : public GenParticle {
	public:
		...
		// From BaseObject
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_SPRAY) : _T("Spray");}
...
class SnowParticle : public GenParticle {
	public:
		...
		// From BaseObject
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_SNOW) : _T("Snow");}
...
class RainClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Spray"); }
...
class SnowClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Snow"); }
	SClass_ID		SuperClassID() {return GEOMOBJECT_CLASS_ID;}
	Class_ID		ClassID() {return Class_ID(SNOW_CLASS_ID,0);}
	const TCHAR* 	Category() {return GetString(IDS_RB_PARTICLESYSTEMS);}
	};
...
static ParamUIDesc descParamRain[] = {
	...
	// Constant birth rate
	ParamUIDesc(PB_CONSTANT,TYPE_SINGLECHECKBOX,IDC_PARTICLE_CONSTANTRATE),
	...
	// Hide Emitter
	ParamUIDesc(PB_HIDEEMITTER,TYPE_SINGLECHECKBOX,IDC_HIDE_EMITTER),
...
void ParticleDlgProc::SetupMaxRate(HWND hWnd,IParamBlock *pblock,TimeValue t)
	{
	int count, life;
	pblock->GetValue(PB_RNDPARTICLES,t,count);
	pblock->GetValue(PB_LIFETIME,t,life);
	if (life <= 0) life = 1;
	TSTR buf;
...
void ParticleDlgProc::EnableTumble(HWND hWnd, IParamBlock *pblock, TimeValue t) // for Snow
{
	...
	int renderType;
	pblock->GetValue(PB_RENDER,t,renderType);
...
GenParticle::GenParticle(BOOL rain)
	...
	ReplaceReference(0, CreateParameterBlock(descVer3, length, CURRENT_VERSION));
	assert(pblock);
...
public:
	...
	virtual void proc(ILoad *iload) {
		...
		DWORD oldVer = ((GenParticle*)(cb->GetTarget()))->pblock->GetVersion();
		...
		if (oldVer<3) {
			...
			int vc;
			((GenParticle*)targ)->pblock->GetValue(PB_VPTPARTICLES,0,vc);
...
#if 0
	int vc, rc;
	pblock->GetValue(PB_VPTPARTICLES,0,vc);
	pblock->GetValue(PB_RNDPARTICLES,0,rc);
	SetAFlag(A_RENDER);
	if (vc!=rc) {
		ParticleInvalid();		
		NotifyDependents(FOREVER,PART_ALL,REFMSG_CHANGE);
...
int	GenParticle::GetParticleCount()
	{
	int c;
	if (TestAFlag(A_RENDER)) pblock->GetValue(PB_RNDPARTICLES,0,c);
	else pblock->GetValue(PB_VPTPARTICLES,0,c);
...
void GenParticle::MapKeys(TimeMap *map,DWORD flags)
	...
	pblock->GetValue(PB_STARTTIME,0,start);
	pblock->GetValue(PB_LIFETIME,0,life);
...
void GenParticle::BirthParticle(INode *node,TimeValue bt,int index)
	...
	pblock->GetValue(PB_EMITTERWIDTH,bt,width);
	pblock->GetValue(PB_EMITTERHEIGHT,bt,height);
	pblock->GetValue(PB_SPEED,bt,initVel);
	pblock->GetValue(PB_VARIATION,bt,var);
...
void GenParticle::UpdateParticles(TimeValue t,INode *node)
	...
	pblock->GetValue(PB_STARTTIME,t,t0);
	pblock->GetValue(PB_LIFETIME,t,life);
	pblock->GetValue(PB_DROPSIZE,t,parts.size);	
	pblock->GetValue(PB_CONSTANT,t,constant);	
	...
	while (tvalid < t) {
		...
		if (fullframe)
		{if (!constant) {pblock->GetValue(PB_BIRTHRATE,tvalid,brate);}
...
BOOL GenParticle::EmitterVisible()
	{
	int hide;
	pblock->GetValue(PB_HIDEEMITTER,0,hide);
	return !hide;
	}
...
TSTR GenParticle::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_VPTPARTICLES:	return localized ? GetString(IDS_RB_VIEWPARTICLES) : _T("Viewport Particles");
		case PB_SPEED:				return localized ? GetString(IDS_RB_SPEED)         : _T("Speed");
		case PB_VARIATION:		return localized ? GetString(IDS_RB_VARIATION)     : _T("Variation");
		case PB_STARTTIME:		return localized ? GetString(IDS_RB_STARTTIME)     : _T("Start Time");
		case PB_LIFETIME:			return localized ? GetString(IDS_RB_LIFETIME)      : _T("Life Time");
		case PB_EMITTERWIDTH:	return localized ? GetString(IDS_RB_WIDTH)         : _T("Width");
		case PB_EMITTERHEIGHT:	return localized ? GetString(IDS_RB_LENGTH)        : _T("Length");
		case PB_BIRTHRATE:		return localized ? GetString(IDS_RB_BIRTHRATE)     : _T("Birth Rate");
		case PB_TUMBLE:			return localized ? GetString(IDS_RB_TUMBLE)        : _T("Tumble");
		case PB_SCALE:				return localized ? GetString(IDS_RB_TUMBLERATE)    : _T("Tumble Rate");
		case PB_DROPSIZE:
			if (ClassID() == Class_ID(RAIN_CLASS_ID, 0)) {
				return localized ? GetString(IDS_RB_DROPSIZE)  : _T("Drop Size");
			} else {
				return localized ? GetString(IDS_RB_FLAKESIZE) : _T("Flake Size");
				}
			break;
...
Mesh *GenParticle::GetFacingRenderMesh(
	...
	pblock->GetValue(PB_DROPSIZE,t,size);
	pblock->GetValue(PB_LIFETIME,t,life);
	if (life<=0) life = 1;
...
void GenParticle::GetWorldBoundBox(
	...
	{
	int type, life;
	pblock->GetValue(PB_DISPTYPE,0,type);
	pblock->GetValue(PB_LIFETIME,0,life);
...
MarkerType RainParticle::GetMarkerType() 
	...
	switch (type) {
		case 0:
			...
			pblock->GetValue(PB_LIFETIME,0,theRainDraw.life);
...
float RainParticle::ParticleSize(TimeValue t,int i)
	...
	int type;
	TimeValue life;
	pblock->GetValue(PB_RENDER,t,type);	
	pblock->GetValue(PB_LIFETIME,t,life);
	float sz = CompParticleSize(parts.ages[i],life,parts.size);
...
void SnowParticle::UpdateParticles(TimeValue t,INode *node)
	...
	pblock->GetValue(PB_TUMBLE,t,theSnowDraw.tumble);
	pblock->GetValue(PB_SCALE,t,theSnowDraw.scale);	
...
MarkerType SnowParticle::GetMarkerType() 
	...
	switch (type) {
		case 0: {			
			...
			pblock->GetValue(PB_LIFETIME,0,theSnowDraw.life);
...
Mesh* SnowParticle::GetRenderMesh(TimeValue t, INode *inode, View& view, BOOL& needDelete)
	...
	pblock->GetValue(PB_DROPSIZE,t,size);
	pblock->GetValue(PB_TUMBLE,t,tumble);
	pblock->GetValue(PB_SCALE,t,scale);
	pblock->GetValue(PB_RENDER,t,type);
	pblock->GetValue(PB_LIFETIME,t,life);
	if (life<=0) life = 1;
	scale /= 50.0f;
