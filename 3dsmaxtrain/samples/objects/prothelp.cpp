...
public:
   ...
   const TCHAR*	 NonLocalizedClassName() { return _T("Protractor"); }
...
void ProtHelpObject::UpdateUI(TimeValue t)
{
   if ( hProtHelpParams && GetWindowLongPtr(hProtHelpParams,GWLP_USERDATA)==(LONG_PTR)this ) {
      ...
      if(refNode[0]) {
         GetDlgItemText(hProtHelpParams, IDC_PROT_TARGET1_NAME, buf, _countof(buf));
      ...
      if(refNode[1]) {
         GetDlgItemText(hProtHelpParams, IDC_PROT_TARGET2_NAME, buf, _countof(buf));
...
RefResult ProtHelpObject::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
