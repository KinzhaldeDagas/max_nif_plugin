...
class MorphTarget {
   public:
      ...
      Matrix3 tm;
      int refCount;
      MorphTarget(Object *o,const TSTR &n,const Matrix3 &m) {obj=0;name=new TSTR(n);refCount=1;tm = m;}
...
class AddMorphTargRestore : public RestoreObj {
   public:         
      ...
      TSTR Description() { return _T("Morp Targ"); }
   };
...
int MorphTargList::AddTarg(
   ...
   maker->ReplaceReference(i+offset,obj);       
   return i;
   }
...
// compile-time validates size of MorphKey class to ensure its size doesn't change since instances are 
// saved as a binary blob to scene file. Changes in size will break file i/o
static_assert(sizeof(MorphKey) == sizeof(DWORD) + sizeof(TimeValue), "");
...
// WARNING - this class description also exists in maxscrpt\maxkeys.cpp
template <class KT>
class GenMorphCont : public MorphControl, public ILockedTrackImp  {
   public:
      MorphTargList targs;
      KT keys;
      ObjectState ob;
      ...
      BOOL lockCache;
	  TypedSingleRefMaker<TriObject> triObj;
...
// WARNING - this class description also exists in IGame\IGameModifierImp.h
...
// compile-time validates size of CubicMorphKey class to ensure its size doesn't change since instances are 
// saved as a binary blob to scene file. Changes in size will break file i/o
static_assert(sizeof(CubicMorphKey) == sizeof(MorphKey) + 7 * sizeof(float) + sizeof(int), "");
...
class CubicMorphCont : public GenMorphCont<CubicMorphKeyTab> {
   public:                         
      ...
      void GetClassName(MSTR& s, bool localized) const override {s = localized ?GetString(IDS_RB_CUBICMORPHCONTROL_CLASS) : _T("Cubic Morph Controller");}
      void EditTrackParams(TimeValue t,ParamDimensionBase *dim,const TCHAR *pname,HWND hParent,IObjParam *ip,DWORD flags);
...
class CubicMorphContClassDesc:public ClassDesc {
   ...
   const TCHAR*            NonLocalizedClassName() { return _T("Cubic Morph Controller"); }
   SClass_ID               SuperClassID() { return CTRL_MORPH_CLASS_ID; }
   Class_ID                ClassID() { return Class_ID(CUBICMORPHCONT_CLASS_ID,0); }
...
class CubicMorphRest : public RestoreObj {
   public:
      ...
      TSTR Description() { return _T("Morph Restore"); }
   };
...
// WARNING - this class description also exists in maxscrpt\maxkeys.cpp and IGame\IGameModifierImp.h
class TargetWeight {
	// Warning - instances of this class are saved as a binary blob to scene file
	// Adding/removing members will break file i/o
   public:
...
// compile-time validates size of TargetWeight class to ensure its size doesn't change since instances are 
// saved as a binary blob to scene file. Changes in size will break file i/o
static_assert(sizeof(TargetWeight) == sizeof(int) + sizeof(float), "");
...
class BaryMorphKey : public MorphKey {
   ...
   public:              
      ...
      BaryMorphKey& operator=(const BaryMorphKey &src) {
...
// compile-time validates size of BaryMorphKey class to ensure its size doesn't change since instances are 
// saved as a binary blob to scene file. Changes in size will break file i/o
//NOTE: class contains pointer - size different for 32 and 64 bit builds
static_assert(sizeof(BaryMorphKey) == sizeof(MorphKey) + 7 * sizeof(float) + sizeof(void*) + 4, ""); // 4 bytes padding - 64 bit build only
...
class BaryMorphCont : public GenMorphCont<BaryMorphKeyTab> {
   public:                         
      ...
      void GetClassName(MSTR& s, bool localized) const override {s = localized ?GetString(IDS_RB_BARYMORPHCONTROL_CLASS) : _T("Barycentric Morph Controller");}
      void EditTrackParams(TimeValue t,ParamDimensionBase *dim,const TCHAR *pname,HWND hParent,IObjParam *ip,DWORD flags);
      void AddNewKey(TimeValue t,DWORD flags);
...
class BaryMorphContClassDesc:public ClassDesc {
   ...
   const TCHAR*	  NonLocalizedClassName() { return _T("Barycentric Morph Controller"); }
   SClass_ID       SuperClassID() {return CTRL_MORPH_CLASS_ID;}
   Class_ID        ClassID() {return Class_ID(BARYMORPHCONT_CLASS_ID,0);}
   const TCHAR*    Category() {return _T("Morph");}
...
class FindMorphObjDEP : public DependentEnumProc {
   public:
      Object *morphObj;
	  MorphControl *morphCtrl;

      FindMorphObjDEP(MorphControl *_morphCtrl) {morphCtrl = _morphCtrl; morphObj=NULL;}
      int proc(ReferenceMaker *rmaker) {
         if (rmaker==morphCtrl) return DEP_ENUM_CONTINUE;
		 ...
		 return DEP_ENUM_SKIP; // just look at direct dependents. checked
         }
   };
...
template <class KT>
TSTR GenMorphCont<KT>::SubAnimName(int i, bool localized)
{
	return Control::SubAnimName(i, localized);
}
...
template <class KT>
void* GenMorphCont<KT>::GetInterface(ULONG id)
{
	switch (id) {
		case I_LOCKED:
...
IOResult GenMorphCont<KT>::Save(ISave *isave)
   ...
   for (int i=0; i<targs.Count(); i++) {
      ...
      isave->WriteWString((*targs[i].name));
      isave->EndChunk();
...
//! [Animatable.ReverseTime Example]
template <class KT>
...
//! [Animatable.GetFlagKeyIndex Example]
template <class KT>
...
void CubicMorphCont::Update(TimeValue t,TimeValue realT)
   ...
   if (!targs.Count()) {
      ...
      if (triObj == nullptr)
		  triObj = CreateNewTriObject();
      ob = ObjectState(triObj);
...
IOResult CubicMorphKeyTab::Save(ISave *isave)
   ...
   if (Count()>0) {
      ...
      isave->WriteVoid(Addr(0), sizeof(CubicMorphKey)*Count(), &nb);
      isave->EndChunk();
      }
   ...
   while (IO_OK==(res=iload->OpenChunk())) {
      switch (iload->CurChunkID()) {
         case MORPHKEY_TABLE_CHUNK: {
            ...
            SetCount(nkeys);
            if (nkeys>0) 
               res = iload->ReadVoid(Addr(0),nkeys*sizeof(CubicMorphKey),&nb);
            break;
            }
...
class CubicMorphKeyInfo : public ReferenceMaker {
   public:
      ...
      virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("CubicMorphKeyInfo"); }
...
private:
      virtual void SetReference(int i, RefTargetHandle rtarg) {cont=(CubicMorphCont*)rtarg;}
public:
...
static INT_PTR CALLBACK CubicMorphKeyInfoWndProc(
      HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
   {
   CubicMorphKeyInfo *k = DLGetWindowLongPtr<CubicMorphKeyInfo*>(hWnd);
...
void CubicMorphCont::EditTrackParams(
      TimeValue t,ParamDimensionBase *dim,const TCHAR *pname,HWND hParent,
...
IOResult BaryMorphKeyTab::Save(ISave *isave)
   ...
   for (int i=0; i<ct; i++) {
      ...
      isave->WriteVoid(&(*this)[i],sizeof(BaryMorphKey)-sizeof(void*),&nb); // ignore Tab<TargetWeight>*
      isave->EndChunk();
      
      for (int j=0; j<(*this)[i].wts->Count(); j++) {
         isave->BeginChunk(BARYKEYTABLE_TARG);
         isave->WriteVoid(&(*this)[i].WTS()[j],sizeof(TargetWeight),&nb);
         isave->EndChunk();
         }     
      }
   ...
   while (IO_OK==(res=iload->OpenChunk())) {
      switch (iload->CurChunkID()) {
         ...
         case BARYKEYTABLE_KEYDATA:
            ix++;
            res=iload->ReadVoid(&(*this)[ix],sizeof(BaryMorphKey)-sizeof(void*),&nb); // ignore Tab<TargetWeight>*
            (*this)[ix].wts = new Tab<TargetWeight>;
            break;

         case BARYKEYTABLE_TARG: {
            TargetWeight wt;
            res=iload->ReadVoid(&wt,sizeof(wt),&nb);
...
void BaryMorphCont::Update(TimeValue t,TimeValue realT)
   ...
   for (int i=0; i<targs.Count(); i++) {
      ...
      if (wts[i]!=0.0f || !i) {
         ...
         // Make sure all objects are deformable
         if (objs[i] != NULL && !objs[i]->IsDeformable()) {
            ...
            if (o != objs[i]) {
				...
				// laperrr | 07.05.04 | Defect 904747 (CER)
				// I fixed the crasher (check for Null ptr) above, but I will leave the
				// assignment in the "objs" array even when "o" is NULL, because it seems
				// loops below expect only deformable objects, and we know the original
				// "objs[i]" object was non-deformable (see test above). I made sure these
				// loops were properly skipping any NULL ptr in the array.
				objs[i] = o;
				// Mark it as needing deletion (if it's not the first object)
				if (i && o != NULL) del[i] = TRUE;
				}
			}
		}
	}
   ...
   // Delete any objects that were created from ConvertToType()    
   for (int i=0; i<targs.Count(); i++) {
      if (del[i] && objs[i]) objs[i]->DeleteThis();
      }
...
{  
	if(GetLocked()==false)
	{
	   ...
	   // Deselect all keys
	   for (int i=0; i<keys.Count(); i++) keys[i].ClearFlag(KEY_SELECTED);
...
class BaryMorphKeyInfo : public ReferenceMaker {
   public:
      ...
      virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("BaryMorphKeyInfo"); }
      ...
      RefTargetHandle GetReference(int i) {return cont;}
private:
      virtual void SetReference(int i, RefTargetHandle rtarg) {cont=(BaryMorphCont*)rtarg;}
public:
...
void BaryMorphKeyInfo::Update()
   ...
   if (numSel==1) {
      ...
      for (int i=0; i<cont->targs.Count(); i++) {
         ...
         SendMessage(hList,LB_ADDSTRING,0,(LONG_PTR)(const TCHAR*)name);
         }
...
static INT_PTR CALLBACK BaryMorphKeyInfoWndProc(
      HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
   {
   BaryMorphKeyInfo *k = DLGetWindowLongPtr<BaryMorphKeyInfo*>(hWnd);
