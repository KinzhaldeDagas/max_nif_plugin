...
{
	friend class PickControlNode;
	friend class ParticleMesherObjCreateCallBack;
   public:  
...
//      wired in.
      ...
      const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_PW_PARTICLEMESHER_OBJECT) : _T("Mesher"); }
      BOOL HasUVW();
...
class ParticleMesherClassDesc: public ClassDesc2 
{
   ...
   const TCHAR*	NonLocalizedClassName() { return _T("Mesher"); }
...
static ParamBlockDesc2 particlemesher_param_blk ( particlemesher_params, _T("ParticleMesherParameters"),  0, &particleMesherDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF_NO, 
   ...
   particlemesher_rendertimeonly,   _T("renderTimeOnly"),   TYPE_BOOL,     0,          IDS_PW_RENDER,
      p_ui,          TYPE_SINGLECHECKBOX,  IDC_RENDER_TIME_ONLY, 
      p_end, 
   particlemesher_extranodes,  _T(""),    TYPE_INODE_TAB,0,    0,    IDS_PW_EXTRANODES,
      p_nonLocalizedName, _T("ExtraNodes"),
      p_end,
   ...
   particlemesher_usecustombounds,  _T("useCustomBounds"),  TYPE_BOOL,     0,          IDS_PW_CBOUNDS,
       p_default,    FALSE,
      p_ui,          TYPE_SINGLECHECKBOX,  IDC_BBOX, 
      p_end, 
   ...
   particlemesher_useallpf,   _T("useAllPFEvents"),   TYPE_BOOL,     0,          IDS_USEALLPFEVENTS,
       p_default,    TRUE,
      p_ui,           TYPE_SINGLECHECKBOX,    IDC_USEALLPF, 
      p_end, 
   particlemesher_pfeventlist,    _T("pfEventList"),  TYPE_INODE_TAB,      0, P_AUTO_UI|P_VARIABLE_SIZE, IDS_PFEVENTLIST,
      ...
      p_nonLocalizedName, _T("PF Events"),
      p_end,
...
RefResult ParticleMesherObject::NotifyRefChanged(
   ...
   RefResult res = SimpleObject2::NotifyRefChanged(changeInt, hTarget, partID,  message, propagate) ;
   switch (message) {
      case REFMSG_CHANGE:
         if (hTarget == pblock2)
            {
      ...
      case REFMSG_TARGET_DELETED:
         if (hTarget == pblock2)
         {
            lastTime = GetCOREInterface()->GetTime();
         }
         break;
      }
   return res;
...
static INT_PTR CALLBACK AddDlgProc(
   ...
   {
   ParticleMesherObject *mod = DLGetWindowLongPtr<ParticleMesherObject*>(hWnd);
