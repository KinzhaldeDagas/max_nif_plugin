...
class MorphObject: public GeomObject {          
   ...
   public:
      ...
      const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_MORPH) : _T("Morph"); }
      ...
      void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_RB_MORPHOBJECT_CLASS) : _T("MorphObj");}
...
private:
      virtual void SetReference(int i, RefTargetHandle rtarg) {morphCont = (MorphControl*)rtarg;}
public:
   };          
...
class MorphObjClassDesc:public ClassDesc {
   ...
   const TCHAR*	 NonLocalizedClassName() { return _T("Morph"); }
...
class CreateMorphMode : public CommandMode, ReferenceMaker {      
   public:     
	  ...
	  CreateMorphMode() { node = svNode = NULL; obj = NULL; }
      ...
      virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("CreateMorphMode"); }
      int NumRefs() {return 1;}
...
class CreateMorphRestore : public RestoreObj {
   public:        
      ...
      void Redo() {}
      TSTR Description() {return _T("Create Morph");}
   };
...
class CreateMorphNotify : public RestoreObj {
   public:        
      ...
      TSTR Description() {return _T("Create Morph Notify");}
   };
...
MorphObject::MorphObject(BOOL loading)
   ...
   morphCont = NULL;
   if (!loading) 
      ReplaceReference(0,newDefaultMorphControl());   
   }
...
void MorphObject::SetupTargetList()
   ...
   for (int i=0; i<morphCont->NumMorphTargs(); i++) {
      SendMessage(GetDlgItem(hParams2,IDC_MORPHTARG_LIST),
         LB_ADDSTRING,0,(LPARAM)SubAnimName(i, true).data());
      }
   if (sel>=0) {
      SendMessage(GetDlgItem(hParams2,IDC_MORPHTARG_LIST),
         LB_SETCURSEL,sel,0);
      ...
      edit->Enable();
      edit->SetText(SubAnimName(sel, true).data());
            ...
            case IDC_MORPHTARG_LIST:
               if (HIWORD(wParam)==LBN_SELCHANGE) {
                  ...
                  if (mo->sel < 0) {
                     ...
                     ICustEdit *edit = GetICustEdit(GetDlgItem(hWnd,IDC_MORPHTARG_NAME));
                     edit->Enable();
                     edit->SetText(mo->SubAnimName(mo->sel, true).data());
                     ReleaseICustEdit(edit);
                     }
...
TSTR MorphObject::SubAnimName(int i, bool localized)
{
	if (i >= morphCont->NumMorphTargs())
	{
		return localized ? GetString(IDS_RB_MORPH) : _T("Morph");
	}
	else
	{
		TSTR name;
...
RefTargetHandle MorphObject::Clone(RemapDir& remap)
   ...
   obj->ReplaceReference(0,remap.CloneRef(morphCont));
   obj->sel = sel;
