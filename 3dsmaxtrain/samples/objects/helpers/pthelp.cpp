...
private:
	...
	Interval cached_value_ivalid = NEVER;
	float cached_size = 0.f;
	int cached_centerMarker = 0;
	int cached_axisTripod = 0;
	int cached_cross = 0;
	int cached_box = 0;
	int cached_screenSize = 0;
	int cached_drawOnTop = 0;
...
public:
	...
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_POINT_HELPER_NAME) : _T("Point Helper"); }
	...
	void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_DB_POINTHELPER_CLASS) : _T("PointHelper"); }
...
private:
	...
	void MaybeUpdateCachedValues(TimeValue t);
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Point"); }
	...
	TimeValue t, INode* inode, ViewExp* vpt, Box3& box)
{
	...
	MaybeUpdateCachedValues(t);
	float size = cached_size;

	float zoom = 1.0f;
	if (cached_screenSize) {
...
int PointHelpObject::DrawAndHit(TimeValue t, INode *inode, ViewExp *vpt)
{
	...
	if (cached_drawOnTop) vpt->getGW()->setRndLimits(limits & ~GW_Z_BUFFER);
	...
	if (cached_axisTripod) {
		DrawAxis(vpt, tm, size, cached_screenSize);
	}
	...
	if (cached_centerMarker) {
		vpt->getGW()->marker(&pt, X_MRKR);
	}

	if (cached_cross) {
		// X
		pts[0].Set(-size, 0.0f, 0.0f); pts[1].Set(size, 0.0f, 0.0f);
		vpt->getGW()->polyline(2, pts, NULL, NULL, FALSE, NULL);

		// Y
		pts[0].Set(0.0f, -size, 0.0f); pts[1].Set(0.0f, size, 0.0f);
		vpt->getGW()->polyline(2, pts, NULL, NULL, FALSE, NULL);

		// Z
		pts[0].Set(0.0f, 0.0f, -size); pts[1].Set(0.0f, 0.0f, size);
		vpt->getGW()->polyline(2, pts, NULL, NULL, FALSE, NULL);
	}
	...
	if (cached_box) {
		...
		// Bottom
		pts[0].Set(-size, -size, -size);
		pts[1].Set(-size, size, -size);
		pts[2].Set(size, size, -size);
		pts[3].Set(size, -size, -size);
...
void PointHelpObject::MaybeUpdateCachedValues(TimeValue t)
{
	if (!cached_value_ivalid.InInterval(t))
	{
		cached_value_ivalid = FOREVER;
		pblock2->GetValue(pointobj_size, t, cached_size, cached_value_ivalid);
		pblock2->GetValue(pointobj_centermarker, t, cached_centerMarker, cached_value_ivalid);
		pblock2->GetValue(pointobj_axistripod, t, cached_axisTripod, cached_value_ivalid);
		pblock2->GetValue(pointobj_cross, t, cached_cross, cached_value_ivalid);
		pblock2->GetValue(pointobj_box, t, cached_box, cached_value_ivalid);
		pblock2->GetValue(pointobj_screensize, t, cached_screenSize, cached_value_ivalid);
		pblock2->GetValue(pointobj_drawontop, t, cached_drawOnTop, cached_value_ivalid);
	}
}
...
static void DrawAnAxis(ViewExp *vpt, Point3 axis)
{
	...
	if (axis.x != 0.0 || axis.y != 0.0) {
		v2.Set(axis.y, -axis.x, axis.z) *= (float)0.1;
	}
	else {
		v2.Set(axis.x, axis.z, -axis.y) *= (float)0.1;
	}
