...
class GridHelpObjClassDesc :public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Grid"); }
...
void GridHelpObject::GetBBox(TimeValue t,  Matrix3& tm, Box3& box) {	
	...
	pblock->GetValue(grid_length, t, length);
	pblock->GetValue(grid_width, t, width);
...
{
	float x=0.f, y=0.f;
	pblock->GetValue(grid_length, t, x);
	pblock->GetValue(grid_width, t, y);
...
int GridHelpObject::Select(TimeValue t, INode *inode, GraphicsWindow *gw, Material *mtl, HitRegion *hr, int abortOnHit ) {
	...
	pblock->GetValue(grid_length, t, height);
	pblock->GetValue(grid_width, t, width);
	if ( width==0 || height==0 )
		return 0;
...
TSTR GridHelpObject::SubAnimName(int i, bool localized)
{
	return localized ? GetString(IDS_DB_PARAMETERS) : _T("Parameters");
}
