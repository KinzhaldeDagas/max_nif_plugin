...
//custom edits
ICustEdit *ExposeTransform::localEulerXEdit = NULL;	
ICustEdit *ExposeTransform::localEulerYEdit = NULL;	
ICustEdit *ExposeTransform::localEulerZEdit = NULL;
ICustEdit *ExposeTransform::worldEulerXEdit = NULL;	
ICustEdit *ExposeTransform::worldEulerYEdit = NULL;	
ICustEdit *ExposeTransform::worldEulerZEdit = NULL;
ICustEdit *ExposeTransform::localPositionXEdit = NULL;	
ICustEdit *ExposeTransform::localPositionYEdit = NULL;	
ICustEdit *ExposeTransform::localPositionZEdit = NULL;
ICustEdit *ExposeTransform::worldPositionXEdit = NULL;	
ICustEdit *ExposeTransform::worldPositionYEdit = NULL;	
ICustEdit *ExposeTransform::worldPositionZEdit = NULL;
ICustEdit *ExposeTransform::boundingBoxLengthEdit = NULL; 
ICustEdit *ExposeTransform::boundingBoxWidthEdit = NULL; 
ICustEdit *ExposeTransform::boundingBoxHeightEdit = NULL;
ICustEdit *ExposeTransform::distanceEdit =NULL;
ICustEdit *ExposeTransform::angleEdit = NULL;
//custom buttons
ICustButton *ExposeTransform::localEulerXButton = NULL;	
ICustButton *ExposeTransform::localEulerYButton = NULL;	
ICustButton *ExposeTransform::localEulerZButton = NULL;
ICustButton *ExposeTransform::worldEulerXButton = NULL;	
ICustButton *ExposeTransform::worldEulerYButton = NULL;	
ICustButton *ExposeTransform::worldEulerZButton = NULL;
ICustButton *ExposeTransform::localPositionXButton = NULL;	
ICustButton *ExposeTransform::localPositionYButton = NULL;	
ICustButton *ExposeTransform::localPositionZButton = NULL;
ICustButton *ExposeTransform::worldPositionXButton = NULL;	
ICustButton *ExposeTransform::worldPositionYButton = NULL;	
ICustButton *ExposeTransform::worldPositionZButton = NULL;
ICustButton *ExposeTransform::boundingBoxLengthButton = NULL; 
ICustButton *ExposeTransform::boundingBoxWidthButton = NULL; 
ICustButton *ExposeTransform::boundingBoxHeightButton = NULL;
ICustButton *ExposeTransform::distanceButton =NULL;
ICustButton *ExposeTransform::angleButton = NULL;
...
public:
	...
	void proc(ILoad *iload)
	{
		IParamBlock2PostLoadInfo* postLoadInfo = (IParamBlock2PostLoadInfo*)
			eT->pblock_info->GetInterface( IPARAMBLOCK2POSTLOADINFO_ID );
		...
		if(postLoadInfo)
		{
			...
			switch(version)
			{
			case EXPOSETRANSFORM_INFO_BLK_VERSION:
				{
					//nothing to do, we got the latest parameter block version
				} 
				break;

			default: // Paramblock with no versioning
				{
					// need to rescale offset time to improperly scaled value in previous releases
					Control* controller = eT->pblock_info->GetControllerByID(pb_time_offset_badScaling);
					int scalingFactor = GetTicksPerFrame();
					if (controller == nullptr)
					{
						// no controller, just rescale value
						TimeValue offSetTime = eT->pblock_info->GetTimeValue(pb_time_offset_badScaling);
						offSetTime /= scalingFactor;
						eT->pblock_info->SetValue(pb_time_offset, 0, offSetTime);
					}
					else
					{
						DefaultRemapDir remap{};
						Control *newController = (Control*)controller->Clone(remap);
						if (newController == nullptr || newController == controller)
						{
							// can't mess with controller, just use rescaled value
							TimeValue offSetTime = eT->pblock_info->GetTimeValue(pb_time_offset_badScaling);
							offSetTime /= scalingFactor;
							eT->pblock_info->SetValue(pb_time_offset, 0, offSetTime);
							return;
						...
						bool handled = false;
						if (newController->CanApplyEaseMultCurves() && newController->IsLeaf())
						{
							// look for a multiplier curve that is keyable, has no keys, and has value
							// of scaling factor. 
							// This would be a controller that we added in the old OnFilePreSaveOld method when
							// saving a file to a version of max that has bad pb2 parameter scaling.
							// If found, remove that multiplier curve.
							int numMultCurves = newController->NumMultCurves();
							for (int i = 0; i < numMultCurves; i++)
							{
								Control* mult = newController->GetMultCurve(i);
								if (mult && mult->IsKeyable())
								{
									IKeyControl* ki = GetKeyControlInterface(mult);
									if (ki && ki->GetNumKeys() == 0)
									{
										float val = 0.f;
										mult->GetValue(0, &val, FOREVER);
										if (int(val+.5) == scalingFactor)
										{
											newController->DeleteMultCurve(i);
											handled = true;
											break;
										}
									}
							...
							if (!handled)
							{
								// a multiplier curve wasn't found, add one with scaling factor.
								Control* mult = NewDefaultFloatController();
								float scalingValue = 1.0f/scalingFactor;
								mult->SetValue(0, &scalingValue);
								newController->AppendMultCurve(mult);
								handled = true;
							}
						}
						if (!handled)
						{
							// couldn't handle via multiplier curve, rescale controller
							newController->RescaleWorldUnits(1.0f/scalingFactor);
						}
						eT->pblock_info->SetControllerByID(pb_time_offset, 0, newController, FALSE);
					}
				}
				break;
			}
		...
		int numParams = eT->pblock_expose->NumParams();
		for(int i = 0; i < numParams; ++i)
		{
			BaseExposeControl * control = (BaseExposeControl*) eT->pblock_expose->GetControllerByIndex(i,0);
			if(control)
			{
	...
	void Get(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t, Interval &valid)
	{
		if(owner)
		{
			...
			switch (id)
			{
				...
				case pb_localeuler:
					//localeuler
					control = (BaseExposeControl*) eT->pblock_expose->GetControllerByID(pb_localeulerx,0);
					if(control)
					{
						control->GetValue(t,&(v.p->x),FOREVER);
					}
					control = (BaseExposeControl*) eT->pblock_expose->GetControllerByID(pb_localeulery,0);
					if(control)
					{
						control->GetValue(t,&(v.p->y),FOREVER);
					}
					control = (BaseExposeControl*) eT->pblock_expose->GetControllerByID(pb_localeulerz,0);
					if(control)
					{
				...
				case pb_worldeuler:
					//worldeuler
					control = (BaseExposeControl*) eT->pblock_expose->GetControllerByID(pb_worldeulerx,0);
					if(control)
					{
						control->GetValue(t,&(v.p->x),FOREVER);
					}
					control = (BaseExposeControl*) eT->pblock_expose->GetControllerByID(pb_worldeulery,0);
					if(control)
					{
						control->GetValue(t,&(v.p->y),FOREVER);
					}
					control = (BaseExposeControl*) eT->pblock_expose->GetControllerByID(pb_worldeulerz,0);
					if(control)
					{
						control->GetValue(t,&(v.p->z),FOREVER);
					}
		...
		if(owner)
		{
			...
			switch (id)
			{
				...
				case pb_worldeuler:
					...
					control = (BaseExposeControl*) eT->pblock_expose->GetControllerByID(id,0);
					if(control)
					{
...
static ParamBlockDesc2 exposetransform_expose_blk (
	...
	pb_worldboundingbox_size, 	_T("worldBoundingBoxSize"), TYPE_POINT3, 	P_READ_ONLY|P_ANIMATABLE|P_TRANSIENT, 	IDS_WORLDBOUNDINGBOX_SIZE, 
		p_nonLocalizedName, _T("World Bounding Box Size"),
		p_end,
	pb_worldboundingbox_width, 	_T("worldBoundingBoxWidth"), TYPE_FLOAT, 	P_READ_ONLY|P_ANIMATABLE|P_TRANSIENT, 	IDS_WORLDBOUNDINGBOX_WIDTH, 
		p_nonLocalizedName, _T("World Bounding Box Width"),
		p_end,
	pb_worldboundingbox_length,	_T("worldBoundingBoxLength"), TYPE_FLOAT, 	P_READ_ONLY|P_ANIMATABLE|P_TRANSIENT, 	IDS_WORLDBOUNDINGBOX_LENGTH, 
		p_nonLocalizedName, _T("World Bounding Box Length"),
		p_end,
	pb_worldboundingbox_height,	_T("worldBoundingBoxHeight"), TYPE_FLOAT, 	P_READ_ONLY|P_ANIMATABLE|P_TRANSIENT, 	IDS_WORLDBOUNDINGBOX_HEIGHT, 
		p_nonLocalizedName, _T("World Bounding Box Height"),
		p_end,
...
static ParamBlockDesc2 exposetransform_info_blk( 
	...
	exposetransform_info, _T("ExposeTransformInfo"),  0, &ExposeTransformDesc, P_AUTO_CONSTRUCT+P_AUTO_UI+P_VERSION, 
	
	// Optional version number required because P_VERSION is set
	EXPOSETRANSFORM_INFO_BLK_VERSION,

	// Optional Reference id required because P_AUTO_CONSTRUCT is set
	PBLOCK_INFO,
...
//		p_validator,    &ExposeValidator,
		...
		p_nonLocalizedName, _T("Expose Node"),
		p_end,
	...
	pb_use_time_offset,	_T("useTimeOffset"), TYPE_BOOL, P_RESET_DEFAULT, IDS_USE_TIME_OFFSET,
		p_default, 		FALSE,
		p_ui, 			TYPE_SINGLECHECKBOX, IDC_USE_TIME_OFFSET, 
		p_accessor,		&exposetransforminfo_accessor,
		p_end,
	pb_time_offset_badScaling, _T(""), TYPE_TIMEVALUE, P_ANIMATABLE|P_OBSOLETE|P_INVISIBLE, 0,
		p_end,
	pb_time_offset, _T("timeOffset"), TYPE_TIMEVALUE, P_ANIMATABLE,IDS_TIME_OFFSET,
		...
		p_accessor,		&exposetransforminfo_accessor,
		p_nonLocalizedName, _T("Time Offset"),
		p_end,
	p_end
	);
...
static ParamBlockDesc2 exposetransform_display_blk( 
	...
	pb_pointobj_screensize,	_T("constantscreensize"), TYPE_BOOL, P_ANIMATABLE, IDS_POINT_SCREENSIZE,
		...
		p_nonLocalizedName, _T("Constant Screen Size"),
		p_end, 
	...
	pb_pointobj_drawontop,	    _T("drawontop"),       TYPE_BOOL, P_ANIMATABLE, IDS_POINT_DRAWONTOP,
		...
		p_nonLocalizedName, _T("Draw On Top"),
		p_end, 
...
static ParamBlockDesc2 exposetransform_output_blk (
	...
	pb_display_exposed,	_T("displayExposedVals"),	TYPE_BOOL, P_RESET_DEFAULT, IDS_DISPLAY_EXPOSED, 
		p_default, 		TRUE,
		p_ui, 			TYPE_SINGLECHECKBOX, IDC_DISPLAY_EXPOSED_VALUES, 
...
{	
	...
	pblock_expose->SetControllerByID(pb_localeuler, 0, c, FALSE);
	c = new FloatExposeControl(this,pb_localeulerx);
	pblock_expose->SetControllerByID(pb_localeulerx, 0, c, FALSE);
	c = new FloatExposeControl(this,pb_localeulery);
	pblock_expose->SetControllerByID(pb_localeulery, 0, c, FALSE);
	c = new FloatExposeControl(this,pb_localeulerz);
	pblock_expose->SetControllerByID(pb_localeulerz, 0, c, FALSE);
	//worldeuler
	c = new EulerExposeControl(this,pb_worldeuler);
	pblock_expose->SetControllerByID(pb_worldeuler, 0, c, FALSE);
	c = new FloatExposeControl(this,pb_worldeulerx);
	pblock_expose->SetControllerByID(pb_worldeulerx, 0, c, FALSE);
	c = new FloatExposeControl(this,pb_worldeulery);
	pblock_expose->SetControllerByID(pb_worldeulery, 0, c, FALSE);
	c = new FloatExposeControl(this,pb_worldeulerz);
	pblock_expose->SetControllerByID(pb_worldeulerz, 0, c, FALSE);
	//localposition
	c = new Point3ExposeControl(this,pb_localposition);
	pblock_expose->SetControllerByID(pb_localposition, 0, c, FALSE);
	c = new FloatExposeControl(this,pb_localpositionx);
	pblock_expose->SetControllerByID(pb_localpositionx, 0, c, FALSE);
	c = new FloatExposeControl(this,pb_localpositiony);
	pblock_expose->SetControllerByID(pb_localpositiony, 0, c, FALSE);
	c = new FloatExposeControl(this,pb_localpositionz);
	pblock_expose->SetControllerByID(pb_localpositionz, 0, c, FALSE);
	//worldposition
	c = new Point3ExposeControl(this,pb_worldposition);
	pblock_expose->SetControllerByID(pb_worldposition, 0, c, FALSE);
	c = new FloatExposeControl(this,pb_worldpositionx);
	pblock_expose->SetControllerByID(pb_worldpositionx, 0, c, FALSE);
	c = new FloatExposeControl(this,pb_worldpositiony);
	pblock_expose->SetControllerByID(pb_worldpositiony, 0, c, FALSE);
	c = new FloatExposeControl(this,pb_worldpositionz);
	pblock_expose->SetControllerByID(pb_worldpositionz, 0, c, FALSE);
	//distance
	c = new FloatExposeControl(this,pb_distance);
	pblock_expose->SetControllerByID(pb_distance, 0, c, FALSE);
	//worldboundingbox
	c = new Point3ExposeControl(this,pb_worldboundingbox_size);
	pblock_expose->SetControllerByID(pb_worldboundingbox_size, 0, c, FALSE);
	c = new FloatExposeControl(this,pb_worldboundingbox_width);
	pblock_expose->SetControllerByID(pb_worldboundingbox_width, 0, c, FALSE);
	c = new FloatExposeControl(this,pb_worldboundingbox_height);
	pblock_expose->SetControllerByID(pb_worldboundingbox_height, 0, c, FALSE);
	c = new FloatExposeControl(this,pb_worldboundingbox_length);
	pblock_expose->SetControllerByID(pb_worldboundingbox_length, 0, c, FALSE);
	//angle
	c = new FloatExposeControl(this,pb_angle);
	pblock_expose->SetControllerByID(pb_angle,0,c,FALSE);
	suspendRecalc= FALSE;
...
Point3 ExposeTransform::GetExposedPoint3Value(TimeValue t,int paramID,Control *) //control not used yet
{
	...
	BOOL wasSuspendRecalc = suspendRecalc;
	suspendRecalc = TRUE;
	BOOL useTimeOffset = FALSE;
	pblock_info->GetValue(pb_use_time_offset,0,useTimeOffset,FOREVER);
...
void ExposeTransform::WMOutputCommand(int id, int notify, HWND hWnd,HWND hCtrl)
{
	...
	case IDC_LE_X:
		SetUpClipBoard(TSTR(_T("localEulerX")));
		break;
	case IDC_LE_Y:
		SetUpClipBoard(TSTR(_T("localEulerY")));
		break;
	case IDC_LE_Z:
		SetUpClipBoard(TSTR(_T("localEulerZ")));
		break;
	case IDC_WE_X:
		SetUpClipBoard(TSTR(_T("worldEulerX")));
		break;
	case IDC_WE_Y:
		SetUpClipBoard(TSTR(_T("worldEulerY")));
		break;
	case IDC_WE_Z:
		SetUpClipBoard(TSTR(_T("worldEulerZ")));
		break;
	case IDC_LP_X:
		SetUpClipBoard(TSTR(_T("localPositionX")));
		break;
	case IDC_LP_Y:
		SetUpClipBoard(TSTR(_T("localPositionY")));
		break;
	case IDC_LP_Z:
		SetUpClipBoard(TSTR(_T("localPositionZ")));
		break;
	case IDC_WP_X:
		SetUpClipBoard(TSTR(_T("worldPositionX")));
		break;
	case IDC_WP_Y:
		SetUpClipBoard(TSTR(_T("worldPositionY")));
		break;
	case IDC_WP_Z:
		SetUpClipBoard(TSTR(_T("worldPositionZ")));
		break;
	case IDC_B_L:
		SetUpClipBoard(TSTR(_T("worldBoundingBoxLength")));
		break;
	case IDC_B_W:
		SetUpClipBoard(TSTR(_T("worldBoundingBoxWidth")));
		break;
	case IDC_B_H:
		SetUpClipBoard(TSTR(_T("worldBoundingBoxHeight")));
		break;
	case IDC_D:
		SetUpClipBoard(TSTR(_T("distance")));
		break;

	case IDC_A:
		SetUpClipBoard(TSTR(_T("angle")));
		break;
...
void ExposeTransform::SetUpClipBoard(const TSTR &localName)
{
	if(OpenClipboard(NULL))
	{
		...
		if(myNode)
		{
			TSTR final = TSTR(_T("$")) + myNode->GetName() + _T(".")+ localName;
			int byteLen = (final.Length()+1)*sizeof(TCHAR);
			//allocate handle mem
			HANDLE handle = GlobalAlloc(GMEM_MOVEABLE,byteLen); 
			if(handle)
			{
				//set up the handle
				TCHAR *str = (LPTSTR) GlobalLock(handle);
				memcpy(str,final.data(),byteLen);
				GlobalUnlock(handle);			
...
void ExposeTransform::InitOutput(TimeValue t)
{
	...
	if(pblock_output->GetMap())
	{
		...
		if(localEulerXEdit==NULL) //assume one for all
		{
			...
			localEulerXButton->SetTooltip(TRUE,(LPTSTR)GetString(IDS_SCRIPTBUTTON));
			...
			localEulerYButton->SetTooltip(TRUE,(LPTSTR)GetString(IDS_SCRIPTBUTTON));
			...
			localEulerZButton->SetTooltip(TRUE,(LPTSTR)GetString(IDS_SCRIPTBUTTON));
			...
			worldEulerXButton->SetTooltip(TRUE,(LPTSTR)GetString(IDS_SCRIPTBUTTON));
			...
			worldEulerYButton->SetTooltip(TRUE,(LPTSTR)GetString(IDS_SCRIPTBUTTON));
			...
			worldEulerZButton->SetTooltip(TRUE,(LPTSTR)GetString(IDS_SCRIPTBUTTON));
			...
			localPositionXButton->SetTooltip(TRUE,(LPTSTR)GetString(IDS_SCRIPTBUTTON));
			...
			localPositionYButton->SetTooltip(TRUE,(LPTSTR)GetString(IDS_SCRIPTBUTTON));
			...
			localPositionZButton->SetTooltip(TRUE,(LPTSTR)GetString(IDS_SCRIPTBUTTON));
			...
			worldPositionXButton->SetTooltip(TRUE,(LPTSTR)GetString(IDS_SCRIPTBUTTON));
			...
			worldPositionYButton->SetTooltip(TRUE,(LPTSTR)GetString(IDS_SCRIPTBUTTON));
			...
			worldPositionZButton->SetTooltip(TRUE,(LPTSTR)GetString(IDS_SCRIPTBUTTON));
			...
			boundingBoxLengthButton->SetTooltip(TRUE,(LPTSTR)GetString(IDS_SCRIPTBUTTON));
			...
			boundingBoxWidthButton->SetTooltip(TRUE,(LPTSTR)GetString(IDS_SCRIPTBUTTON));
...
void ExposeTransform::RecalcData()
{
	...
	valid = TRUE;
	if (suspendRecalc == FALSE)
	{
		suspendRecalc = TRUE; //don't get into any loops here

		int numParams = pblock_expose->NumParams();
		for (int i = 0; i < numParams; ++i)
		{
			Control* cTest = pblock_expose->GetControllerByIndex(i, 0);
			if (cTest != NULL)
			{
...
RefTargetHandle ExposeTransform::Clone(RemapDir& remap) 
	...
	int numParams =  newob->pblock_expose->NumParams();
	for(int i = 0; i < numParams; ++i)
	{
		BaseExposeControl* control = static_cast<BaseExposeControl*>( newob->pblock_expose->GetControllerByIndex(i,0) );
		if(control)
		{
...
static void Text( ViewExp *vpt, const TCHAR *str, const Point3 &pt )
	{	
