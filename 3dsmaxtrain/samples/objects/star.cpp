...
class StarObject: public SimpleSpline, public IParamArray {
	...
	public:
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_TH_STAR) : _T("Star"); }
		void InitNodeName(TSTR& s) { s = GetString(IDS_TH_STAR); }
		Class_ID ClassID() { return Class_ID(STAR_CLASS_ID,0); }
		void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_TH_STAR_CLASS) : _T("Star"); }
...
class StarObjClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Star"); }
...
public:
	...
	virtual void proc(ILoad *iload) {
		...
		DWORD oldVer = ((StarObject*)(cb->GetTarget()))->pblock->GetVersion();
...
INT_PTR StarTypeInDlgProc::DlgProc(
...
void StarObject::EndEditParams( IObjParam *ip, ULONG flags,Animatable *next )
{
	...
	pblock->GetValue(PB_POINTS,ip->GetTime(),dlgPoints);
	pblock->GetValue(PB_DISTORT,ip->GetTime(),dlgDistortion);
}
	...
	if (msg==MOUSE_POINT||msg==MOUSE_MOVE) {
		switch(point) {
			...
			case 1: 
				...
				if ((msg==MOUSE_POINT) && (Length(m-cPt)<3 )) {
					return CREATE_ABORT;
					}
...
TSTR StarObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_RADIUS1: return localized ? GetString(IDS_TH_RADIUS1)    : _T("Radius 1");
		case PB_RADIUS2: return localized ? GetString(IDS_TH_RADIUS2)    : _T("Radius 2");
		case PB_FILLET1: return localized ? GetString(IDS_AP_FILLET1)    : _T("Fillet Radius 1");
		case PB_FILLET2: return localized ? GetString(IDS_AP_FILLET2)    : _T("Fillet Radius 2");
		case PB_POINTS:  return localized ? GetString(IDS_TH_STARPOINTS) : _T("Points");
		case PB_DISTORT: return localized ? GetString(IDS_TH_DISTORTION) : _T("Distortion");
		default:         return _T("");
		}
	}
