...
//		BOOL updatingUI;
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? ::GetString(IDS_TH_TEXT) : _T("Text"); }
		void InitNodeName(TSTR& s) { s = ::GetString(IDS_TH_TEXT); }		
		Class_ID ClassID() { return Class_ID(TEXT_CLASS_ID,0); }  
		void GetClassName(MSTR& s, bool localized) const override { s = localized ? ::GetString(IDS_TH_TEXT_CLASS) : _T("Text"); }
		...
		float GetSize(TimeValue t, Interval& valid);
		void SetKerning(TimeValue t, float s);
		float GetKerning(TimeValue t, Interval& valid);
		void SetLeading(TimeValue t, float s);
		float GetLeading(TimeValue t, Interval& valid);
		...
		bool ConvertStringToGlyphIndices();
...
class TextObjClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Text"); }
...
INT_PTR TextParamDialogProc::DlgProc(TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam) {
	...
	switch ( msg ) {
		case WM_INITDIALOG: {
			...
			if (text->string2.isNull())
				SendMessage(GetDlgItem(hWnd,IDC_TEXTENTRY), WM_SETTEXT, 0, (LPARAM)(LPCTSTR)text->string.data());
			else
				SendMessage(GetDlgItem(hWnd,IDC_TEXTENTRY), WM_SETTEXT, 0, (LPARAM)(LPCTSTR)text->string2.data());
...
void TextObject::EndEditParams( IObjParam *ip, ULONG flags,Animatable *next )
{
	...
	pblock->GetValue(PB_SIZE,ip->GetTime(),dlgSize);
	pblock->GetValue(PB_KERNING,ip->GetTime(),dlgKerning);
	pblock->GetValue(PB_LEADING,ip->GetTime(),dlgLeading);
...
TextObject::TextObject() : SimpleSpline() 
	...
	ReplaceReference(USERPBLOCK, CreateParameterBlock(descVer2, PBLOCK_LENGTH, CURRENT_VERSION));
	assert(pblock);
...
TextObject::~TextObject()
	...
	int chars = chShapesMBCS.Count();
	for(int i = 0; i < chars; ++i)
		delete chShapesMBCS[i];
	chShapesMBCS.Delete(0, chars);
...
RefTargetHandle TextObject::Clone(RemapDir& remap) {
	...
	for(int i = 0; i < chShapesMBCS.Count(); ++i) {
		CharShape *cs = new CharShape;
		*cs = *chShapesMBCS[i];
		newob->chShapesMBCS.Append(1, &cs);
		}
...
TSTR TextObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_SIZE:    return localized ? ::GetString(IDS_TH_SIZE)    : _T("Size");
		case PB_KERNING: return localized ? ::GetString(IDS_TH_KERNING) : _T("Kerning");
		case PB_LEADING: return localized ? ::GetString(IDS_TH_LEADING) : _T("Leading");
		default:         return _T("");
		}
	}
...
namespace
{
	//I know codepage - locale is not one to one corresponding relationship, but no choice
	//better to write the locale info in the scene file in the future.
	const TCHAR* GetLocaleFromCodePage(UINT codePage)
	{
		switch(codePage)
		{
		case 932: return _T("Japanese");
		case 949: return _T("Korean");
		case 936: return _T("chinese-simplified");
		case 1252: return _T("English");  
		default:
			return _T("English");
		}
	}

	int char_to_codeA(const char buffer[]){
	int code;
	int wkcode,cnt;
	char wkbuf;
...
bool TextObject::ConvertStringToGlyphIndices()
{
	const TCHAR* curtLocale = GetLocaleFromCodePage(stringCodePage);
	MaxLocaleHandler lh(LC_ALL, curtLocale); //Set the new locale

	MaxSDK::Util::MaxStringCastCP stringCP = string.ToCP(stringCodePage);
	const char* p = stringCP.data();
	CStr stringA = p;
	int chars = stringA.Length();
	glyphIndices.reserve(chars);
	uniscribe_glyph usg;
	
	for(int i = 0; i < chars; i++) {
		UINT index;
		UCHAR charp = stringA[i];
		
		// \U+XXXX or \u+XXXXX check
		if ((stringA[i] == '\\') && (stringA[i+1] == 'U' || stringA[i+1] == 'u') && (stringA[i+2] == '+')){
			int unicode_code = char_to_codeA(&stringA.data()[i]+3);
			if(unicode_code < 0){
				index = (UCHAR)stringA[i];
			}
			else {
				index = 0x10000000 | unicode_code;		// unicode flag on (1000XXXX)
				i = i + 6;
			}
		}
		else if (_istleadbyte(charp))
		{
			union {
				unsigned char b[2];
				unsigned short c;
...
//DebugPrint(_T("Constructing textShape, font:[%s]\n"),fontName.data());
	...
	{
		//Create the glygh index in old way to match the indices in chShapes.
		//And we cannot copy chShapes to chShapesMBCS, otherwise it will be STP scene file with multibyte old indeics, which calculated based on
		//previous locale, with current locale written to the scene file, which ends up with unreadable shape info.
...
RefResult TextObject::NotifyRefChanged(
	...
	if(hTarget != pblock && hTarget != ipblock)
		return ShapeObject::NotifyRefChanged(changeInt, hTarget, partID, message, propagate);
	if(message == REFMSG_CHANGE && manualUpdate)
		return REF_STOP;
	return SimpleSpline::NotifyRefChanged(changeInt, hTarget, partID, message, propagate);
	}
...
int TextObject::UpdateCharCache() {
	...
	int charsMBCS = chShapesMBCS.Count();
	for(i = 0; i < charsMBCS; ++i) {
		CharShape *cs = chShapesMBCS[i];
		cs->keep = FALSE;
		unsigned int index = cs->index;
	...
	// Get rid of the ones we don't need anymore
	for(i = charsMBCS-1; i >= 0; --i) {
		if(!chShapesMBCS[i]->keep) {
			delete chShapesMBCS[i];
			chShapesMBCS.Delete(i, 1);
			charsMBCS--;
			}
		}
	...
	for(i = 0; i < glyphIndices.size(); ++i) {
		...
		if(add) {
			...
			if(!theBezFontManager.BuildCharacterEx(fontHandle, index, 1000.0f, s->shape, s->width, textVersion, false))
				s->width = 300.0f;	// Fake it if the character isn't there
				chShapes.Append(1, &s, 5);
				chars++;
		}
	}
	MaxSDK::Util::CharIterator<const TCHAR> stringIt(string2.data());
	for (stringIt = string2.data(); !stringIt.AtEnd(); ++stringIt) {
		MaxSDK::Util::Char charT = stringIt.Get();
		UINT index = charT.CharCode();
		...
		if(index > 0x7f) 
		{
			...
			wchar_t tempW[3];
			char    temp[8];
			charT.ToUTF16(tempW, _countof(tempW));

			WideCharToMultiByte(CP_ACP, WC_NO_BEST_FIT_CHARS, tempW, -1, temp, 8, NULL, &bUsedDefaultChar);
			//for high ascii whose char value is different in UTF32 and ASCII map, treat it as Unicode
			if (bUsedDefaultChar || index != (UCHAR)temp[0])
				index |= 0x10000000;		// need unicode flag on (1000XXXX), for STP
		}
	
		BOOL add = TRUE;
		for(int j = 0; j < charsMBCS; ++j) {
			if(index == (UINT)chShapesMBCS[j]->index) {
				add = FALSE;
				break;
			}
		}
		if(add) {
			CharShape *s = new CharShape(index);
			if(!theBezFontManager.BuildCharacterEx(fontHandle, index, 1000.0f, s->shape, s->width, textVersion, true))
				s->width = 300.0f;	// Fake it if the character isn't there
				chShapesMBCS.Append(1, &s, 5);
				charsMBCS++;
		}
	}
...
void static ConvertUnicodeToHexStringAndShapes(TSTR &string, CStr &stringA, unsigned int codepage, CharShapeTab &chShapesMBCS, BEZFONTHANDLE fontHandle, BOOL textVersion)
{
	MaxSDK::Util::CharIterator<const TCHAR> stringIt(string.data());
	bool IsRegeneratedIndex = (chShapesMBCS.Count() == 0 && fontHandle);

	size_t sLength = string.Length();
	size_t strSize = sLength*7 + 1;
	char*  buf     = stringA.dataForWrite(strSize);
	...
	for (; !stringIt.AtEnd(); ++stringIt)
	{
		MaxSDK::Util::Char charT = stringIt.Get();
		UINT index = 0;

		int convLen;
		index = charT.CharCode();
		if (charT.CharCode() <= 127)
		{
			buf[len++] = (char)index;
		}
		else
		{
			...
			convLen = WideCharToMultiByte(codepage, WC_NO_BEST_FIT_CHARS, tempW, -1, temp, 8, NULL, &bUsedDefaultChar); // I think 8 bytes is big enough 

			//Some high ascii character value in UTF32 is different from char value in ascii map, the value in UTF32 is larger than 231
			//in that case, we should treat it as Unicode
			if (!bUsedDefaultChar && index == (UCHAR)temp[0]) 
			{
				strcat(buf, temp);
				//in above WideCharToMultiByte(), -1 is set and means null terminator is added in temp
				len += convLen - 1;
			}
			else
			{
				sprintf(temp, "\\U+%04X",(unsigned int)charT.CharCode());
				strcat(buf, temp);
				len += 7;
				if (IsRegeneratedIndex)
					index |= 0x10000000;		// need unicode flag on (1000XXXX), for STP
			}
		}
