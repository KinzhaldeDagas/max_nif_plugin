...
RefResult EggShape::NotifyRefChanged( const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate) 
{
...
TSTR EggShape::SubAnimName(int i, bool localized)
{
	...
	return SimpleSpline::SubAnimName(i, localized);
}

void EggShape::InitNodeName(TSTR& s) 
{ 
	s = GetString(IDS_NODE_NAME); 
}
...
void EggShape::GetClassName(MSTR& s, bool localized) const 
{ 
	s = localized ? GetString(IDS_CLASS_NAME) : _T("Egg"); 
}

const TCHAR *EggShape::GetObjectName(bool localized) const 
{ 
	return localized ? GetString(IDS_OBJECT_NAME) : _T("Egg"); 
}
...
void EggShape::MakeEggShape(BOOL should_make_donut, BezierShape& ashape, float size, float size2, float rotation, float offset)
{
	...
	float angle_inc = static_cast<float>(M_PI/2);
	...
	angle = 0;
	angle_inc = static_cast<float>((EGG_SIDE_ANGLE_SPAN/3)*TO_RAD);
	...
	angle = static_cast<float>(EGG_SIDE_ANGLE_SPAN*TO_RAD);
	angle_inc = static_cast<float>((EGG_TIP_ANGLE_SPAN/3)*TO_RAD);
	for(int i=0; i<4; i++, angle+=angle_inc)
	{ 
		if(i==0)
		{
	...
	angle = static_cast<float>((180-EGG_SIDE_ANGLE_SPAN)*TO_RAD);
	angle_inc = static_cast<float>((EGG_SIDE_ANGLE_SPAN/3)*TO_RAD);
	for(int i=0; i<4; i++, angle+=angle_inc)
	{ 
...
{	//Calculate the offset position
	...
	for(int i=0; i<4; i++, angle+=angle_inc)
	{ 
		float divided_by = (small_size+size2) < 1.0f ? 1.0f : (small_size+size2);
		...
		divided_by = (size+size2) < 1.0f ? 1.0f : (size+size2);
		// We also have the last one :)
		if(i==3)
		{
