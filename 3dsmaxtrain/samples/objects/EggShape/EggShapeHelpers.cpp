...
( 
	...
	egg_shape_make_donut, 	_T("egg_outline"), 	TYPE_BOOL,	P_ANIMATABLE|P_RESET_DEFAULT, 	IDS_OUTLINE, 
		p_default, 			TRUE, 
		p_ms_default, 		TRUE,
		p_ui, 				TYPE_SINGLECHECKBOX,		IDC_DONUT_CHECK,
		p_tooltip,			IDS_PARAM_OUTLINE_TOOLTIP,
		p_nonLocalizedName, _T("Outline"),
		p_end,
	egg_shape_thickness, 		_T("egg_thickness"), 	TYPE_WORLD, P_ANIMATABLE|P_RESET_DEFAULT, 	IDS_THICKNESS, 
		p_default, 			0.f, 
		...
		p_tooltip,			IDS_PARAM_THICK_TOOLTIP,
		p_dim,				stdWorldDim,
		p_nonLocalizedName, _T("Thickness"),
		p_end,
	egg_shape_rotation,		_T("egg_angle"), TYPE_FLOAT, P_ANIMATABLE|P_RESET_DEFAULT, 	IDS_ANGLE, 
		...
		p_ui, 				TYPE_SPINNER,	EDITTYPE_FLOAT,	IDC_ROT_EDIT,	IDC_ROT_SPIN,	0.1f, 
		p_tooltip,			IDS_PARAM_ANGLE_TOOLTIP,
		p_nonLocalizedName, _T("Angle"),
		p_end,
	egg_shape_length, 		_T("egg_length"), 	TYPE_WORLD, P_ANIMATABLE|P_RESET_DEFAULT, 	IDS_LENGTH, 
		p_default, 			0.f, 
		...
		p_tooltip,			IDS_PARAM_LENGTH_TOOLTIP,
		p_dim,				stdWorldDim,
		p_nonLocalizedName, _T("Length"),
		p_end,
	egg_shape_width, 		_T("egg_width"), 	TYPE_WORLD, P_ANIMATABLE|P_RESET_DEFAULT, 	IDS_WIDTH,  
		p_default, 			0.f, 
		...
		case 1:
			{
				...
				float com_factor = static_cast<float>(6/(4-CENTER_OF_MASS_OFFSET));
...
void EggAccessor::Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)
{
	...
	case egg_shape_width :
		...
		break;
	default:
		DbgAssert(0);		// Throws assert if a parameter isn't accounted for
		break;
	}
}

// The Egg object normally intercepts calls to set its Length and Width parameters and sets the other parameter to a given ratio
// in order to enforce shape rules.  When the system is performing rescaling operations, this needs to be turned off, otherwise
// the parameters get double-scaled, with awful results.  We provide an interface to the PBRescaleParamAccessor so the system will
// call it instead of our normal accessor.

BaseInterface* EggAccessor::GetInterface(Interface_ID id) {
	if(id == PB_RESCALE_PARAM_ACCESSOR_INTERFACE)
		return (PBRescaleParamAccessor*)this;
	return PBAccessor::GetInterface(id);
}

// The following is the PBRescaleParamAccessor's Set method, used when rescaling parameters.
// Does nothing at present, prevents automatic parameter ratio enforcement during rescale
void EggAccessor::Set(PB2Value& v, float scaleFactor, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t) {
	UNREFERENCED_PARAMETER(v);
	UNREFERENCED_PARAMETER(scaleFactor);
	UNREFERENCED_PARAMETER(owner);
	UNREFERENCED_PARAMETER(tabIndex);
	UNREFERENCED_PARAMETER(t);
	// Safety check: Make sure that if anyone adds the accessor to any additional parameters in the future, they process them appropriately here
	switch(id)
	{
	case egg_shape_length :
	case egg_shape_width :
