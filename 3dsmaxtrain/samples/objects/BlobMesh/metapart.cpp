...
template<> struct MaxSDK::Util::IsPlainOldDataType< BunchData > { };
#endif
...
#ifndef ADSK_MAXSDK_NO_POD_TYPE_CHECK
template<> struct MaxSDK::Util::IsPlainOldDataType< Surfpush > { };
#endif
corners::corners()
{ 
	field=-0.6f;
...
{ 
	...
	{ 
		...
		for (int i=bloblst[going].pstart-1;i>bloblst[bcount].pstop;i--)
			plst[pos--]=plst[i];
		pos=bloblst[bcount].pstop+1;
		for (int i=0;i<plen;i++)
			plst[pos++]=plst2[i];
		bloblst[bcount].pstop+=plen;
		for (int i=bcount+1;i<going;i++)
		{ 
...
{ 
	...
	{ 
		inset=FALSE;
		curblob=0;
		while ((curblob<=bnum)&&((curblob==bcount)||((inset=CheckBounds(bcount,bloblst[curblob].vmin,bloblst[curblob].vmax)) == FALSE)))
			curblob++;
	    if (inset)
		{ 
			if (curblob<bcount) 
...
{ 
  ...
  // if particle fits into an existing set, add it
	while ((bcount<bnum)&&(OneInSet(bcount)||(inset=CheckBounds(bcount,minpoint[plst[*incount]],maxpoint[plst[*incount]])) == FALSE))
		bcount++;
	(*incount)++;
...
{  
	BOOL ok = TRUE;
	int i = 0;
	// Make certain we don't exceed maximum heap size in 'new'
	double dMemSize = (double)sizeof(corners)*(double)levelmax;
	if( (dMemSize < 0.0) || (dMemSize > (double)_HEAP_MAXREQ) )
		return FALSE;
	try {
		for (i=0;(i<levels)&&(ok);i++)
...
{ 
	...
	s++;
	level[cube[s].gridnum].field=CalcField(cube[s].pos);
	// do not combine the following 2 loops. A 64 bit compiler optimization error will
	// occur. The loop is unwound, and for the 2 to 4th comparisons the old cube field
	// value is used instead of the new. The new field value is placed into memory after
	// the compare occurs.  Oops...
	for (int i=start;i<start+4;i++)
		cube[i].field=level[cube[i].gridnum].field;
	for (int i=start;i<start+4;i++)
		cube[i].sign=cube[i].field>=0.0f;
}
...
{ 
   ...
   for( int i = 0; i < lcount; /* increment is in loop; was a while() before */ )
	{ 
		vlst[0]=line[i].p1;
		vlst[1]=line[i++].p2;
		vcount=2;
...
{ 
	...
	else 
	{
		...
		// the compare occurs.  Oops...
		for (int i=0;i<4;i++)
			curcube[i].field=alllevels[0][curcube[i].gridnum].field;
		for (int i=0;i<4;i++)
			curcube[i].sign=curcube[i].field>=0.0f;
	}
...
{ 
	...
	{ 
		...
		cube[s].field=level[cube[s].gridnum].field;
	}
	for (int s=start;s<start+4;s++)
		cube[s].sign=cube[s].field>=0.0f;
}
...
{ 
	...
	{ 
		...
		for (int i=1;i<alive;i++) RecalcBounds(0,minpoint[i],maxpoint[i]);
	}
	vertex=0;face=0;
