...
class BoolObject : public IBoolObject, public MeshOpProgress {
   ...
   public:
      ...
      const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_BOOLEAN) : _T("Boolean"); }
      ...
      void GetClassName(MSTR& s, bool localized) const override {s = localized ? GetString(IDS_RB_BOOLEAN_CLASS) : _T("Boolean");}
...
class BoolObjClassDesc:public ClassDesc {
   ...
   const TCHAR*  NonLocalizedClassName() { return _T("Boolean"); }
...
class CreateBoolMode : public CommandMode, ReferenceMaker {    
   public:     
	  ...
	  CreateBoolMode() { node = svNode = NULL; obj = NULL; }
	  ...
	  virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("CreateBoolMode"); }
      int NumRefs() {return 1;}
      RefTargetHandle GetReference(int i) {return node;}
private:
      virtual void SetReference(int i, RefTargetHandle rtarg) {node = (INode*)rtarg;}
public:
       RefResult NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
         PartID& partID, RefMessage message, BOOL propagate);     
   };
...
class CreateBoolRestore : public RestoreObj {
   public:        
      ...
      void Redo() {}
      TSTR Description() {return _T("Create Boolean");}
   };
...
class CreateBoolNotify : public RestoreObj {
   public:        
      ...
      TSTR Description() {return _T("Create Bool Notify");}
   };
...
void BoolObject::ExtractOperand(int which)
   ...
   if (extractCopy) obj = (Object*)CloneRefHierarchy(obj);
...
void BoolObject::SetOperandA (TimeValue t, INode *node) {
   ...
   opaName = TSTR(_T("B_")) + node->GetName();
...
void BoolObject::SetOperandB (TimeValue t, INode *node, INode *boolNode,
   ...
   opbName = TSTR(_T("B_")) + node->GetName();
   ...
   opCont          = (Control*)remap->CloneRef(opCont);
   remap->Backpatch();
   remap->DeleteThis();
...
void BoolObject::SetupUI2(BOOL useName)
   ...
   TSTR name = TSTR(_T("A: ")) + ((ob1||useName) ? opaName.data() : _T(""));
   SendMessage(hList,LB_ADDSTRING,0,(LPARAM)(const TCHAR*)name);
   name = TSTR(_T("B: ")) + ((ob2||useName) ? opbName.data() : _T(""));
   SendMessage(hList,LB_ADDSTRING,0,(LPARAM)(const TCHAR*)name);
...
static INT_PTR CALLBACK BoolParamDlgProc1( 
   ...
   {
   BoolObject *bo = DLGetWindowLongPtr<BoolObject*>(hWnd);
...
TSTR BoolObject::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return opaName; // GetString(IDS_RB_OPERANDA);
	case 1: return localized ? GetString(IDS_RB_OPERANDATRANSFORM) : _T("Operand A Transform");
	case 2: return opbName; // GetString(IDS_RB_OPERANDB);
	case 3: return localized ? GetString(IDS_RB_OPERANDBTRANSFORM) : _T("Operand B Transform");
	}
	return _T("Error");
}
...
int BoolObject::
   ...
   if (TestFlag(BOOL_DISPRESULT) && ob1 && ob2) {
      UpdateMesh(t,FALSE,inode->Selected());    
      gw->setTransform(inode->GetObjectTM(t));
      MeshRenderHelper(mesh, gw, inode->Mtls(), flags, vpt, COMP_ALL, inode->NumMtls());
   ...
   if (disp) { 
      ...
      if ((disp==1 || disp&DRAW_A) && (ob=GetPipeObj(t,0)) != NULL) {
          INodeTransformed n(inode,GetOpTM(t,0));
          if (inode->Selected()) {
      ...
      if ((disp==1 || disp&DRAW_B) && (ob=GetPipeObj(t,1)) != NULL) {
         INodeTransformed n(inode,GetOpTM(t,1));
         if (inode->Selected()) {
