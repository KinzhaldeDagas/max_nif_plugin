...
BOOL HDIKCreateChain::RemoveIKChain(INode* chainNode) const
{
	...
	if (tmCont->ClassID() != IKDRIVEN_CLASSID) return FALSE;

	IKDrivenControl* driven = (IKDrivenControl*)tmCont;
	IKDriverControl* driver = driven->GetIKDriver();
	if (!driver) return FALSE;

	driver->RemoveIKChainControllers(t);
...
BOOL HDIKCreateChain::CreateIKChain(INode* startJoint, INode* endJoint, BOOL makeEE)
{
	TimeValue t = GetCOREInterface()->GetTime();
	INode *node = endJoint;
	IKDriverControl* driver = nullptr;
	IKDrivenControl* driven = nullptr;
	...
	while (node) {
		...
		if (!tmCont->IsReplaceable()) {
			...
			// See if it's an HD IK Driven
			if (tmCont->ClassID() == IKDRIVEN_CLASSID) {
				driven = (IKDrivenControl*)tmCont;

				// If we haven't already decided on a driver, use this one.
				if (!driver) {
					driver = driven->GetIKDriver();
				}
				else {
					// We can't assign to a chain which overlaps two different other chains
					if (driver != driven->GetIKDriver()) {
						return FALSE;
					}
				}
	...
	if (!driver) driver = CreateIKDriverControl();
	Matrix3 tm;
	AffineParts parts;
	Point3 angles;
	...
	while (node) {
		...
		driven = CreateIKDrivenControl(driver, node);
		...
		// Init the IK driven using the previous controller's values at the time
		// we're assigning the new chain
		driven->SetInitPos(parts.t);
		driven->SetInitRot(angles);
		...
		if (!newAPI)
			tmCont->GetIKJoints(&posData3, &rotData3);
		if (node == endJoint)
		{
			rotData3.active[0] = rotData3.active[1] = rotData3.active[2] = FALSE;
		}
		if (newAPI)
			driven->InitIKJoints2(&posData3, &rotData3);
		else
			driven->InitIKJoints(&posData3, &rotData3);

		// Plug-in the driven controller
		node->SetTMController(driven);
	...
	if (makeEE) {
		...
		if (cont && cont->ClassID() == IKDRIVEN_CLASSID) {
			driven = (IKDrivenControl*)cont;
			driven->MakeEE(TRUE, (1 << 0), endJoint->GetNodeTM(t).GetTrans(), Quat());
		}
	}
...
public:
	...
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_BONE_OBJ_NAME) : _T("Bone"); }
	BOOL HasUVW();
...
public:
	...
	const TCHAR*  NonLocalizedClassName() { return _T("Bone"); }
...
public:
	...
	const TCHAR*  NonLocalizedClassName() { return _T("Bones"); }
...
private:
	...
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("BonesCreationManager"); }
...
BonesCreateMode::BonesCreateMode()
{
	...
	boneParamBlkDesc = new ParamBlockDesc2
		(
			...
			p_nonLocalizedName, _T("Side Fins"),
			p_end,
			...
			p_nonLocalizedName, _T("Side Fins Size"),
			p_end,
			...
			p_nonLocalizedName, _T("Side Fins Start Taper"),
			p_end,
			...
			p_nonLocalizedName, _T("Side Fins End Taper"),
			p_end,
			...
			p_nonLocalizedName, _T("Front Fin"),
			p_end,
			...
			p_nonLocalizedName, _T("Front Fin Size"),
			p_end,
			...
			p_nonLocalizedName, _T("Front Fin Start Taper"),
			p_end,
			...
			p_nonLocalizedName, _T("Front Fin End Taper"),
			p_end,
			...
			p_nonLocalizedName, _T("Back Fin"),
			p_end,
			...
			p_nonLocalizedName, _T("Back Fin Size"),
			p_end,
			...
			p_nonLocalizedName, _T("Back Fin Start Taper"),
			p_end,
			...
			p_nonLocalizedName, _T("Back Fin End Taper"),
			p_end,
			...
			p_nonLocalizedName, _T("Generate Mapping Coords."),
			p_end,
...
void BonesCreateMode::GetINIFilename(TCHAR* filename)
{
	...
	_tcscpy(filename, ip ? ip->GetDir(APP_PLUGCFG_DIR).data() : _T(""));
...
static INT_PTR CALLBACK AssignSplineIKSolverDlgProc(
	...
	case WM_INITDIALOG:
		...
		sikcPB2->GetValue(IIKChainControl::kSplineTypeChoice, t, curveType);
		...
		sikcPB2->GetValue(IIKChainControl::kAutoSplineCreate, t, splCreate);
		...
		sikcPB2->GetValue(IIKChainControl::kCreateHelper, t, hlprCreate);
		...
		sikcPB2->GetValue(IIKChainControl::kLinktoRootNode, t, linkRoot);
		SetCheckBox(hWnd, IDC_LINK_ROOT_CHCK, linkRoot);

		sikcPB2->GetValue(IIKChainControl::kHelperCentermarker, t, checked);
		SetCheckBox(hWnd, IDC_CENTRAL_MARKER_CHCK, checked);

		sikcPB2->GetValue(IIKChainControl::kHelperAxisTripod, t, checked);
		SetCheckBox(hWnd, IDC_AXIS_TRIPOD_CHCK, checked);

		sikcPB2->GetValue(IIKChainControl::kHelperCross, t, checked);
		SetCheckBox(hWnd, IDC_CROSS_CHCK, checked);

		sikcPB2->GetValue(IIKChainControl::kHelperBox, t, checked);
		SetCheckBox(hWnd, IDC_BOX_CHCK, checked);

		sikcPB2->GetValue(IIKChainControl::kHelperScreensize, t, checked);
		SetCheckBox(hWnd, IDC_CONST_SC_SIZE_CHCK, checked);

		sikcPB2->GetValue(IIKChainControl::kHelperDrawontop, t, checked);
		SetCheckBox(hWnd, IDC_DRAW_TOP_CHCK, checked);


		sikcPB2->GetValue(IIKChainControl::kSplineKnotCount, t, nKnots);
		spin1 = SetupIntSpinner(hWnd, IDC_SPLKNOT_NUM_SPIN, IDC_SPLKNOT_NUM_EDIT, 2, INT_MAX, nKnots);

		sikcPB2->GetValue(IIKChainControl::kHelpersize, t, hlprSize);
	...
	if (sel == 0) {
		...
		if (ikco) {
			...
			if (iname == _T("SplineIKSolver")) { // that means it's a SplineIK Solver
				...
				ikNC->GetValue(IIKChainControl::kAutoSplineCreate, GetCOREInterface()->GetTime(), sCreate);
				//watje CANCELFIX
				if ((sCreate) && (iret)) {
					//create a spline using the parameters specified in the above dialog box.
					int nKnots;
					ikNC->GetValue(IIKChainControl::kSplineKnotCount, GetCOREInterface()->GetTime(), nKnots);
					int curveType;
					ikNC->GetValue(IIKChainControl::kSplineTypeChoice, GetCOREInterface()->GetTime(), curveType);
...
// sikcPB2 = ikNode->GetTMController()->GetParamBlockByID(IIKChainControl::kParamBlock);
	...
	BOOL hlprChecked;
	int linkRoot;
	ikCPb->GetValue(IIKChainControl::kCreateHelper, t, hlprChecked);
	ikCPb->GetValue(IIKChainControl::kLinktoRootNode, t, linkRoot);
...
// same BOOL variable "checked"  is used for the state of all the subsequent checkboxes
		...
		isplikBlock->GetValue(ISplineIKControl::sm_point_node_list, t, firstHelperNode, 0);
		if (firstHelperNode) {
...
void BonesCreationManager::MakeEndEffector()
{
	...
	driven->MakeEE(TRUE, 1, tm.GetTrans(), Quat());
}
...
static Matrix3 ComputeExtraBranchLinkPosition(TimeValue t, INode *parNode, Matrix3 constTM, BOOL ignoreKids = FALSE)
{
	...
	else
	{
		...
		if (obj->ClassID() == BONE_OBJ_CLASSID)
		{
			BoneObj *bobj = (BoneObj*)obj;
			float length;
			bobj->ParamBlock()->GetValue(boneobj_length, t, length);
...
static void MakeNubBone(TimeValue t, INode *nubBone)
{
	...
	if (obj->ClassID() == BONE_OBJ_CLASSID) {
		...
		bobj->ParamBlock()->GetValue(boneobj_width, t, width);
		bobj->ParamBlock()->GetValue(boneobj_height, t, height);
	...
	case MOUSE_POINT:
	{
		...
		if (curNode == NULL) {
			...
			if (overBone) {
				...
				{
					if (GetCOREInterface()->DisplayFilterIsNodeHidden(i, ob->SuperClassID(), ob->ClassID(), parNode))
					{
...
public:
	...
	static void CopyRefinedJointData(Control* parCont, Control* newCont, BOOL ikDriven);
};
...
void BoneObjUserDlgProc::RefineSegment(INode *parNode, float refinePercentage)
{
	...
	float length;
	bobj->pblock2->GetValue(boneobj_length, t, length);
	...
	if (int numKids = parNode->NumberOfChildren()) // if there are any children at all
	{
		...
		for (int i = 0; i < numKids; i++)
		{
			...
			// Handle IK driven controllers specially
			if (childTMCont->ClassID() == IKDRIVEN_CLASSID)
			{
				IKDrivenControl* driven = (IKDrivenControl*)childTMCont;

				// Setup the reference frame of the driven children so that they
	...
	if (parCont && !(parCont->ClassID() == IKDRIVEN_CLASSID))
	{
...
// newNode->RealignBoneToChild(t);
	...
	if (((parCont = parNode->GetTMController()) != NULL) && (parCont->ClassID() == IKDRIVEN_CLASSID))
	{
		IKDrivenControl* driven = (IKDrivenControl*)parCont;
		IKDriverControl* driver = driven->GetIKDriver();

		// Create a new driven control
		driven = CreateIKDrivenControl(driver, newNode);
...
void BoneObjUserDlgProc::CopyRefinedJointData(Control* parCont, Control* newCont, BOOL ikDriven)
{
...
BOOL PickBoneRefineMode::HitTest(IObjParam *ip, HWND hWnd, ViewExp *vpt, IPoint2 m, int flags)
{
	...
	if (pickedNode != NULL)
	{
		...
		if (nobj && nobj->ClassID() == BONE_OBJ_CLASSID) {
			...
			float length;
			bobj->pblock2->GetValue(boneobj_length, ip->GetTime(), length);
...
{
	BOOL res;
	pblock2->GetValue(boneobj_genmap, TimeValue(0), res);
	return res;
}
