...
class ArcObject: public SimpleSpline, public IParamArray {
	...
	public:
		...
		void InitNodeName(TSTR& s) { s = GetObjectName(true); }
		Class_ID ClassID();
...
class ArcObjClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Arc"); }
...
class HalfRoundObjClassDesc:public ClassDesc {
	public:
	int 			IsPublic() { return 0; }
	void*			Create(BOOL loading = FALSE) { return new ArcObject(HALFROUND); }
	const TCHAR*	ClassName() { return GetString(IDS_TH_HALFROUND); }
	const TCHAR*	NonLocalizedClassName() { return _T("Half Round"); }
	SClass_ID		SuperClassID() { return SHAPE_CLASS_ID; }
   	Class_ID		ClassID() { return Class_ID(HALF_ROUND_CLASS_ID,0); }
	const TCHAR* 	Category() { return GetString(IDS_TH_SPLINES);  }
	void			ResetClassParams(BOOL fileReset);
	};

class QuarterRoundObjClassDesc:public ClassDesc {
	public:
	int 			IsPublic() { return 0; }
	void*			Create(BOOL loading = FALSE) { return new ArcObject(QUARTERROUND); }
	const TCHAR*	ClassName() { return GetString(IDS_TH_QUARTERROUND); }
	const TCHAR*	NonLocalizedClassName() { return _T("Quarter Round"); }
	SClass_ID		SuperClassID() { return SHAPE_CLASS_ID; }
   	Class_ID		ClassID() { return Class_ID(QUARTER_ROUND_CLASS_ID,0); }
	const TCHAR* 	Category() { return GetString(IDS_TH_SPLINES);  }
	void			ResetClassParams(BOOL fileReset);
	};
...
static HalfRoundObjClassDesc halfRoundObjDesc;
static QuarterRoundObjClassDesc quarterRoundObjDesc;

ClassDesc* GetArcDesc() { return &arcObjDesc; }
ClassDesc* GetHalfRoundDesc() { return &halfRoundObjDesc; }
ClassDesc* GetQuarterRoundDesc() { return &quarterRoundObjDesc; }
...
void HalfRoundObjClassDesc::ResetClassParams(BOOL fileReset)
	{
	ArcObject::crtPos         = Point3(0,0,0);
	ArcObject::crtRadius      = 0.0f;
	ArcObject::crtFrom        = 0.0f;
...
void QuarterRoundObjClassDesc::ResetClassParams(BOOL fileReset)
	{
	ArcObject::crtPos         = Point3(0,0,0);
	ArcObject::crtRadius      = 0.0f;
	ArcObject::crtFrom        = 0.0f;
...
static ParamUIDesc descParam[] = {
	...
	// Pie Slice
	ParamUIDesc(PB_PIE,TYPE_SINGLECHECKBOX,IDC_ARCPIE),	
				
	// Reverse
	ParamUIDesc(PB_REVERSE,TYPE_SINGLECHECKBOX,IDC_REVERSE),	
	};

#define PARAMDESC_LENGTH 5

static ParamUIDesc hrDescParam[] = {
	// Radius
	ParamUIDesc(
		PB_RADIUS,
		EDITTYPE_UNIVERSE,
...
INT_PTR ArcTypeInDlgProc::DlgProc(
	...
	if (pmapCreate && pmapParam) {
		...
		if(mArcType)
		{
			pmapParam = CreateCPParamMap(
				hrDescParam,HR_PARAMDESC_LENGTH,
				pblock,
				ip,
				hInstance,
				MAKEINTRESOURCE(IDD_HALFROUNDPARAM),
...
void ArcObject::EndEditParams( IObjParam *ip,ULONG flags,Animatable *next )
{
	...
	if(mArcType == ARC)
		pblock->GetValue(PB_PIE,ip->GetTime(),dlgPie);

	pblock->GetValue(PB_REVERSE,ip->GetTime(),dlgReverse);
}
...
ArcObject::ArcObject(int arcType) : SimpleSpline() 
{
	mArcType = arcType;
	Init();
}
...
void ArcObject::Init()
{
	...
	if(mArcType != ARC)
	{
		if(mArcType == HALFROUND)
			pblock->SetValue(PB_TO,0,180.0f);
		else
			pblock->SetValue(PB_TO,0,90.0f);

		pblock->SetValue(PB_FROM,0,0.0f);
		pblock->SetValue(PB_PIE,0,TRUE);
		pblock->SetValue(PB_REVERSE,0,FALSE);
	}
	else
	{
...
TSTR ArcObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_RADIUS: return localized ? GetString(IDS_TH_RADIUS) : _T("Radius");
		case PB_FROM:   return localized ? GetString(IDS_TH_FROM)   : _T("From");
		case PB_TO:     return localized ? GetString(IDS_TH_TO)     : _T("To");
		default:        return _T("");
		}
	}
...
Class_ID ArcObject::ClassID()
{
	switch(mArcType) 
	{
		case HALFROUND: return Class_ID(HALF_ROUND_CLASS_ID,0);
		case QUARTERROUND: return Class_ID(QUARTER_ROUND_CLASS_ID,0);
		default: return Class_ID(ARC_CLASS_ID,0);
	}
}

const TCHAR *ArcObject::GetObjectName(bool localized) const 
{ 
	switch(mArcType)
	{
		case QUARTERROUND: return localized ? GetString(IDS_TH_QUARTERROUND) : _T("Quarter Round");
		case HALFROUND:    return localized ? GetString(IDS_TH_HALFROUND )   : _T("Half Round");
		default:           return localized ? GetString(IDS_TH_ARC)          : _T("Arc");
	}
}

void ArcObject::GetClassName(MSTR& s, bool localized) const
{
	switch(mArcType)
	{
		case QUARTERROUND: s = localized ? GetString(IDS_TH_QUARTERROUND_CLASS) : _T("QuarterRound"); break;
		case HALFROUND:    s = localized ? GetString(IDS_TH_HALFROUND_CLASS )   : _T("HalfRound");    break;
		default:           s = localized ? GetString(IDS_TH_ARC_CLASS)          : _T("Arc");          break;
	}
}
