...
class BasicFlectorModClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Basic Deflector Modifier"); }
...
class CreateBasicFlectorProc : public MouseCallBack,ReferenceMaker 
{
	private:
		...
		virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("CreateBasicFlectorProc"); } // from Animatable
...
//watje ref to hold the collision engine
	monoflect_colliderp,  _T("colliderplane"),		TYPE_REFTARG, 	0, 	IDS_FLECTORTYPETBLE,
		p_nonLocalizedName, _T("Flector Dialogs"),
		p_end,
...
{	
	...
	float K=(float)GetSystemUnitScale(UNITS_CENTIMETERS);
	float stepsize = dt;
...
TSTR BasicFlectorObj::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	...
	case monoflecdlg: return localized ? GetString(IDS_DLG_MONOF) : _T("Parameters");
	default: return _T("");
	}
}
