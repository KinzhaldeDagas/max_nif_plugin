...
//		static IParamMap *pmapParam;
		...
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_EP_SSPAWNDEFLECTOR_OBJECT) : _T("SOmniFlect");}
...
class SSpawnDeflClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("SOmniFlect"); }
...
class SSpawnDeflMod : public SimpleWSMMod {
	public:				
		...
		// From Animatable
		void GetClassName(MSTR& s, bool localized) const override {s= localized ? GetString(IDS_EP_SSPAWNDEFLECTORMOD) : _T("SOmniFlectMod");}
		SClass_ID SuperClassID() {return WSM_CLASS_ID;}
		void DeleteThis() {delete this;}
		Class_ID ClassID() { return SSPAWNDEFLMOD_CLASS_ID;}
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_EP_SSPAWNDEFLECTORBINDING) : _T("SOmniFlector Binding");}
...
class SSpawnDeflModClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("SOmniFlectMod"); }
	SClass_ID		SuperClassID() {return WSM_CLASS_ID; }
	Class_ID		ClassID() {return SSPAWNDEFLMOD_CLASS_ID;}
...
//watje load up the engine
		...
		{	macroRec->Disable();
			...
			Control* cont = obj->pblock2->GetControllerByID(sspflectrobj_radius);
			if (cont) cols->pblock->SetControllerByID(collisionsphere_radius, 0, cont, FALSE);
...
// Main reflection / refraction loop starts here
	float rsquare = 0.0f, rplus = 0.0f, rminus = 0.0f, TempDP = 0.0f;
