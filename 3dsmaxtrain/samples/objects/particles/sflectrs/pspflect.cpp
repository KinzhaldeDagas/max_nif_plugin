...
//		static IParamMap *pmapParam;
		...
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_EP_PLANARSPAWNDEF_OBJECT) : _T("POmniFlect");}
...
class PSpawnDeflClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("POmniFlect"); }
...
class PSpawnDeflMod : public SimpleWSMMod {
	public:				
		...
		// From Animatable
		void GetClassName(MSTR& s, bool localized) const override {s= localized ? GetString(IDS_EP_PLANARSPAWNDEFMOD) : _T("POmniFlectMod");}
		SClass_ID SuperClassID() {return WSM_CLASS_ID;}
		void DeleteThis() {delete this;}
		Class_ID ClassID() { return PLANARSPAWNDEFMOD_CLASS_ID;}
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_EP_PLANARSPAWNDEFMODBINDING) : _T("POmniFlector Binding");}
...
class PSpawnDeflModClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("POmniFlectMod"); }
	SClass_ID		SuperClassID() {return WSM_CLASS_ID; }
	Class_ID		ClassID() {return PLANARSPAWNDEFMOD_CLASS_ID;}
...
//and stuff it into our param block
	...
	if (colp)
	{
		...
		colp->SetWidth(pblock2->GetControllerByID(pspflectrobj_radius));
		colp->SetHeight(pblock2->GetControllerByID(pspflectrobj_radius1));
		colp->SetQuality(pblock2->GetControllerByID(pspflectrobj_quality));
	}
	macroRec->Enable();
...
//note we must check to see if it exists since the engine is a plugin and can be removed
		...
		{	macroRec->Disable();
			...
			colp->SetWidth(obj->pblock2->GetControllerByID(pspflectrobj_radius));
			colp->SetHeight(obj->pblock2->GetControllerByID(pspflectrobj_radius1));
			colp->SetQuality(obj->pblock2->GetControllerByID(pspflectrobj_quality));
