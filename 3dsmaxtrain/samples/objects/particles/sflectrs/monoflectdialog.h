...
class BasicFlectorObj : public DynamModObject // ,ITestInterface // Bayboro 9/18/01
{
	public:	
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_AP_MONONAME) : _T("MonoFlector"); }
		static IParamMap2Ptr pmap[numpblocks];
...
class BasicFlectorMod : public SimpleWSMMod 
{
	public:				
		...
		// From Animatable
		void GetClassName(MSTR& s, bool localized) const override {s= localized ? GetString(IDS_EP_BASICDEFLECTORMOD) : _T("Basic Deflector Modifier");}
		SClass_ID SuperClassID() {return WSM_CLASS_ID;}
		void DeleteThis() {delete this;}
		Class_ID ClassID() { return BASICFLECTORMOD_CLASSID;}
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_EP_BASICDEFLECTORBINDING) : _T("Basic Deflector Binding");}
...
class MonoFlector: public BasicFlectorType 
{
	public:
		...
		virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("MonoFlector"); } // from Animatable
