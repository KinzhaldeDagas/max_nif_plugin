...
class PBombObject : public SimpleWSMObject // ,IOperatorInterface  // Bayboro 9/18/01
{
	public:
		...
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_PBOMB) : _T("PBomb");}
...
class PBombClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("PBomb"); }
...
class PBombMod : public SimpleWSMMod {
	public:				
		...
		// From Animatable
		void GetClassName(MSTR& s, bool localized) const override {s= localized ? GetString(IDS_RB_PBOMBMOD) : _T("PBombMod");}
		SClass_ID SuperClassID() {return WSM_CLASS_ID;}
		void DeleteThis() {delete this;}
		Class_ID ClassID() { return PBOMBMOD_CLASS_ID;}
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_PBOMBBINDING) : _T("PBombBinding");}
...
class PBombModClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("PBombMod"); }
	SClass_ID		SuperClassID() {return WSM_CLASS_ID;}
	Class_ID		ClassID() {return PBOMBMOD_CLASS_ID;}
...
static ParamUIDesc descParamBomb[] = {
	...
	ParamUIDesc(PB_RANGEON,TYPE_SINGLECHECKBOX,IDC_PBOMB_RANGEON),
	};
...
public:
	...
	virtual void proc(ILoad *iload) 
	{
		...
		DWORD oldVer = ((PBombObject*)(cb->GetTarget()))->pblock->GetVersion();
...
void PBombObject::BeginEditParams(IObjParam *ip,ULONG flags,Animatable *prev)
{
...
/*	if (dorange && hoops)
	{   int newv;
	    ...
	    mesh.setVert(newv, Point3(0.0f, 0.0f, 0.0f));
		int vi=norvs;
	    for (int i=norfs; i<norfs+rangefaces; i++) 
	    { int i1 = vi+1;
...
TSTR PBombObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_SYMMETRY: 	return localized ? GetString(IDS_RB_SYMMETRY)  : _T("Symmetry");
		...
		case PB_LASTSFOR:		return localized ? GetString(IDS_RB_LASTSFOR)  : _T("Lasts For");
		case PB_DELTA_V:		return localized ? GetString(IDS_AP_STRENGTH)  : _T("Strength");
		case PB_DECAY:			return localized ? GetString(IDS_RB_DECAY)     : _T("Range");
		case PB_DECAYTYPE:	return localized ? GetString(IDS_RB_DECAYTYPE) : _T("Decay Type");
		case PB_ICONSIZE:		return localized ? GetString(IDS_RB_ICONSIZE)  : _T("Icon Size");
		default: 				return _T("");
		}
	}
