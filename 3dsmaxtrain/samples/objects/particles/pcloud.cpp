...
class CreatePCPartDelNode : public RestoreObj {
   public:        
      ...
      void Redo() 
         { if (dist==shownormname) obj->normname=_T("");
           else if (dist) obj->distname=_T("");
...
class CreatePCPartPickNode : public RestoreObj {
   public:        
      ...
      void Restore(int isUndo) {
         if (dist==shownormname)
         { if (!obj->normname.isNull()) obj->normname = name; }
         else if (dist)
...
static ParamUIDesc descParamPType[] = {
   ...
   // Display type
   ParamUIDesc(PB_PCNOTDRAFT,TYPE_SINGLECHECKBOX,IDC_SP_DRAFTMODE),
...
static ParamUIDesc descParamPSpin[] = {
   ...
   // IPC Enable
   ParamUIDesc(PB_PCIPCOLLIDE_ON,TYPE_SINGLECHECKBOX,IDC_INTERP_BOUNCEON),
...
class CreatePCloudProc : public MouseCallBack,ReferenceMaker {
   private:
      ...
      virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("CreatePCloudProc"); } // from Animatable
...
{ if ( CloudObject ) 
   { 
      ...
      {  // RB 4-9-96: Normally the hold isn't holding when this 
		 ...
		 CloudObject->DeleteAllRefsFromMe();
		 CloudObject->DeleteAllRefsToMe();
		 CloudObject->DeleteThis();
         CloudObject = NULL;
         theHold.Resume();
         ...
         switch ( point ) {
            case 0:
              ...
              createInterface->GetMacroRecorder()->EmitScript();
               ...
               // link it up
               l_CloudNode = createInterface->CreateObjectNode( CloudObject);
               attachedToNode = TRUE;
               assert( l_CloudNode );             
...
INT_PTR PCParticleGenDlgProc::DlgProc(
...
INT_PTR PCParticleSpinDlgProc::DlgProc(
      TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{  switch (msg) {
      case WM_INITDIALOG: 
      {break;
...
INT_PTR PCParticleDisableDlgProc::DlgProc(
      TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{  switch (msg) {
      case WM_INITDIALOG: {
...
INT_PTR PCParticleParmDlgProc::DlgProc(
      TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{  switch (msg) {
      case WM_INITDIALOG: {
...
INT_PTR PCParticleSpawnDlgProc::DlgProc(
      TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
...
TSTR PCloudParticle::GetFilename()
{
    // PCloud.CST is moved to plugcfg_ln
    ...
    filename.append(GetString(IDS_AP_PCLOUDCST));
...
{ TCHAR newname[NLEN];
  int i = 0, save = 0;
  PCloudParticle *po = DLGetWindowLongPtr<PCloudParticle*>(hWnd);
...
void PCloudParticle::BeginEditParams(IObjParam *ip, ULONG flags,Animatable *prev)
{ 
...
void PCloudParticle::BuildEmitter(TimeValue t, Mesh& amesh)
   {
   float width(0.0f), height(0.0f), depth(0.0f), wholewid(0.0f);
   int sym(0), csymv=20, csymf=18, startv(0), startf(0);
   mvalid = FOREVER;
   ...
   { float u,cu,su;
     ...
     int il=NUM_SEGS;
     for (int f=0,i=0; f<2*NUM_SEGS; f+=2) {
         int i1 = i+1,i2=il+1;
...
{ Object *pobj;     
  ...
  int nc,subv=0,subf=0,subtvnum=0,tface,tvert;
...
BOOL PCloudParticle::ComputeParticleStart(TimeValue t0,int c)
   ...
   {   parts.FreeAll();if (sdata) delete sdata;sdata=NULL;  maincount=0;
      ...
      if (playing) GetCOREInterface()->EndAnimPlayback();
       TSTR name;name=(cnode ? cnode->GetName() : GetString(IDS_AP_PCLOUD));
...
{  Face *face; 
   Point3 n,p, bry;
   float d(0.0f), rn(0.0f), a(0.0f), at(0.0f);
...
void PCloudParticle::UpdateParticles(TimeValue t,INode *node)
   ...
   int c(0), birth(0), counter(0), tpf=GetTicksPerFrame(), count=0, anioff(0);
   VelDir ptvel;
    Matrix3 tm,lasttm; 
...
//         parts.radius[j]=ptvel.Size;       else
      ...
      Point3 *fnorms=NULL;TriObject *triOb=NULL;Matrix3 disttm;Object *pobj=NULL;
...
//       tm=node->GetObjTMBeforeWSM(acheck<stepSize?t0:(tvalid-stepSize));
      ...
      for (int j=counter; j<maincount; j++) {
         if (born>=birth) break;
...
TSTR PCloudParticle::GetParameterName(int pbIndex, bool localized)
   {
   switch (pbIndex) {
      ...
      case PB_DIRX:                  return localized ? GetString(IDS_AP_SPDIRX)              : _T("Direction Vector X");
      case PB_DIRY:                  return localized ? GetString(IDS_AP_SPDIRY)              : _T("Direction Vector Y");
      case PB_DIRZ:                  return localized ? GetString(IDS_AP_SPDIRZ)              : _T("Direction Vector Z");
      case PB_DIRVAR:                return localized ? GetString(IDS_AP_SPDIRVAR)            : _T("Speed Direction Variation");
      ...
      case PB_EMITRWID:              return localized ? GetString(IDS_AP_RADLEN)              : _T("Emitter Rad/Len");
      case PB_EMITRHEIGHT:           return localized ? GetString(IDS_RB_EMITRWID)            : _T("Emitter Width");
      case PB_EMITRDEPTH:            return localized ? GetString(IDS_AP_EMITRHEIGHT)         : _T("Emitter Height");
      ...
      case PB_SPAWNDIEAFTER:         return localized ? GetString(IDS_AP_SPAWNDIEAFTER)       : _T("Die  X frames after collision");
      case PB_SPAWNDIEAFTERVAR:      return localized ? GetString(IDS_AP_SPAWNDIEAFTERVAR)    : _T("Die  X frames after collision variation");
      case PB_PCIPCOLLIDE_ON:        return localized ? GetString(IDS_AP_IPCOLLIDE_ON)        : _T("Interparticle Collisions On");
      case PB_PCIPCOLLIDE_STEPS:     return localized ? GetString(IDS_AP_IPCOLLIDE_STEPS)     : _T("Interparticle Collision Steps");
      case PB_PCIPCOLLIDE_BOUNCE:    return localized ? GetString(IDS_AP_IPCOLLIDE_BOUNCE)    : _T("Interparticle Collision Bounce");
      case PB_PCIPCOLLIDE_BOUNCEVAR: return localized ? GetString(IDS_AP_IPCOLLIDE_BOUNCEVAR) : _T("Interparticle Collision Bounce Variation");
      default:                       return _T("");
      }
   }  
   ...
   { aniend=GetAnimEnd();
     int oneframe=GetTicksPerFrame();
      ...
      po->pblock->GetValue(PB_SPINAXISTYPE,0,axisentered,FOREVER);
       if ((po->thePCloudDraw.indir.inaxis=(axisentered==DIRTRAVEL)) == TRUE)
...
{  float Mval(0.0f), Uval(0.0f), Vval(0.0f), Wval(0.0f), Angle(0.0f), elapsedtime(0.0f);
   int type(0), count(0), maptype(0), anioff(0), anifr(0);
   float width(0.0f);
   Point3 Center;
   TVFace defface;
...
const TCHAR* PCloudClassDesc::NonLocalizedClassName()
{
	return _T("PCloud");
}

const TCHAR *PCloudClassDesc::Category () {return GetString(IDS_RB_PARTICLESYSTEMS);}
const TCHAR *PCloudParticle::GetObjectName(bool localized) const {return localized ? GetString(IDS_AP_PCLOUDGC) : _T("PCloud");}


void PCloudParticle::GetTimes(TimeLst &times,TimeValue t,int anifr,int ltype)
{ int n,found,tmax,tnums=0,tgen;
  TimeValue tframe;
...
{ 
	...
	{ 
		if (hgen) 
		{
			...
			SetWindowText(GetDlgItem(hgen, IDC_AP_PCLOUDOBJDIRNAME), name);
		}
	}
	else if (dist) 
	{ 
		if (hparam) 
		{
			...
			FormatName(name.dataForWrite());
			SetWindowText(GetDlgItem(hparam, IDC_AP_FILLPICKOBJECT), name);
		}
	}
	else
	{ 
...
// if (dodist) {nodes.SetCount(1);nodes[0]=node;}
    if (dodist==1)
     ...
     nodes.SetCount(0);
     if ((po->flags=(node->IsGroupHead()?1:0)) != 0) MakeGroupNodeList(node,&nodes,subtree,ip->GetTime());
...
BOOL PCPickNorm::Pick(IObjParam *ip,ViewExp *vpt)
   ...
   po->ReplaceReference(MBASE,node);
   theHold.Accept(GetString(IDS_AP_COMPICK));
   ...
   switch (message) {      
      case REFMSG_TARGET_DELETED:   
         { if (hTarget==custnode) 
            ...
            custnode=NULL;custname=_T(" ");cancelled=FALSE;
            }
           if (hTarget==distnode) {if (theHold.Holding()) theHold.Put(new CreatePCPartDelNode(this,distnode->GetName(),1));
              distnode=NULL;distname=_T(" ");cancelled=FALSE;
            createmeth=0;
            pblock->SetValue(PB_CREATEIN,0,createmeth);
           }
           if (hTarget==mbase) {if (theHold.Holding()) theHold.Put(new CreatePCPartDelNode(this,mbase->GetName(),shownormname));mbase=NULL;normname=_T(" ");cancelled=FALSE;}
           BOOL notfound=TRUE; 
...
public:
	...
	virtual void proc(ILoad *iload) {
		...
		DWORD oldVer = ((PCloudParticle*)(cb->GetTarget()))->pblock->GetVersion();
...
PCloudParticle::PCloudParticle()
   ...
   ReplaceReference(PBLK, CreateParameterBlock(spdescVer5, PBLOCK_LENGTH_PCLOUD, CURRENT_VERSION));
