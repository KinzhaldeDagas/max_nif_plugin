...
{	
	public:		
		...
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_AP_PFOLLOWOBJ) : _T("PathFollowObject");}
...
class PFollowClassDesc:public ClassDesc 
{
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Path Follow"); }
...
class PFollowMod : public SimpleWSMMod 
{
	public:				
		...
		// From Animatable
		void GetClassName(MSTR& s, bool localized) const override {s= localized ? GetString(IDS_AP_PFOLLOWMOD) : _T("Path FollowMod");}
		SClass_ID SuperClassID() {return WSM_CLASS_ID;}
		void DeleteThis() {delete this;}
		Class_ID ClassID() { return PFOLLOWMOD_CLASS_ID;}
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_AP_PATHFOLLOWBINDING) : _T("PathFollowBinding");}
...
class PFollowModClassDesc:public ClassDesc 
{
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Path FollowMod"); }
	SClass_ID		SuperClassID() {return WSM_CLASS_ID; }
	Class_ID		ClassID() {return PFOLLOWMOD_CLASS_ID;}
...
static ParamUIDesc descParam[] = {
	...
	// Range Limit On
	ParamUIDesc(PB_LIMITRANGE,TYPE_SINGLECHECKBOX,IDC_AP_RANGELIMITON),
	...
	// Range Limit On
	ParamUIDesc(PB_KSPEED,TYPE_SINGLECHECKBOX,IDC_AP_CONSTSPEED),
...
class CreatePFollowObjectProc : public MouseCallBack,ReferenceMaker {
	private:
		...
		virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("CreatePFollowObjectProc"); } // from Animatable
...
{ if ( PFollowObj ) 
	...
	PFollowObj->EndEditParams( (IObjParam*)createInterface, 
		...
		{	// RB 4-9-96: Normally the hold isn't holding when this 
			...
			theHold.Suspend(); 
			PFollowObj->DeleteAllRefsFromMe();
			PFollowObj->DeleteAllRefsToMe();
			PFollowObj->DeleteThis();
			PFollowObj = NULL;
			theHold.Resume();
		...
		case MOUSE_POINT:
			switch ( point ) {
              ...
              createInterface->GetMacroRecorder()->EmitScript();
					...
					// link it up
					INode *l_PFollowNode = createInterface->CreateObjectNode( PFollowObj);
					attachedToNode = TRUE;
					assert( l_PFollowNode );					
					createCB = NULL;
					createInterface->SelectNode( l_PFollowNode );
					
					// Reference the new node so we'll get notifications.
					theHold.Suspend();
					ReplaceReference( 0, l_PFollowNode);
					theHold.Resume();
...
//					macroRec->Enable();
	...
	int result; 
	if (dostuff)
	{ result=createmethod(vpx.ToPointer(),msg,point,flag,m,mat);
	  createInterface->RedrawViews(createInterface->GetTime()); 
...
INT_PTR PFollowObjectDlgProc::DlgProc(
...
{	TimeValue tpf=GetTicksPerFrame();
	custnode = NULL;
	mf = NULL;
	ReplaceReference(0, CreateParameterBlock(PFollowdescVer0, PBLOCK_PFLENGTH, CURRENT_VERSION));
	assert(pblock);	
	reset=FALSE;
...
TSTR PFollowObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_KSPEED:			return localized ? GetString(IDS_AP_KSPEED)       : _T("Constant Speed");
		case PB_LIMITRANGE:		return localized ? GetString(IDS_AP_RANGEON)      : _T("Range Limited");
		case PB_RANGEDIST:		return localized ? GetString(IDS_AP_RANGE)        : _T("Range Value");
		case PB_SPLINETYPE:		return localized ? GetString(IDS_AP_SPLINETYPE)   : _T("Spline Follow Type");
		case PB_TANGCHAOS:		return localized ? GetString(IDS_AP_TANGCHAOS)    : _T("Tangent Chaos");
		case PB_TANGCHAOSVAR:	return localized ? GetString(IDS_AP_TANGCHAOSVAR) : _T("Tang Chaos Var");
		case PB_TANGDIR:			return localized ? GetString(IDS_AP_TANGENTDIR)   : _T("Tangent Dir");
		case PB_SPIRCHAOS:		return localized ? GetString(IDS_AP_SPIRCHAOS)    : _T("Spiral Chaos");
		case PB_SPIRCHAOSVAR:	return localized ? GetString(IDS_AP_SPIRCHAOSVAR) : _T("Spiral Chaos Var");
		case PB_SPIRALDIR:		return localized ? GetString(IDS_AP_SPIRALDIR)    : _T("Spiral Dir");
		case PB_TIMESTART:		return localized ? GetString(IDS_AP_TIMESTART)    : _T("Start Time");
		case PB_TIMETRAVEL:		return localized ? GetString(IDS_AP_TIMETRAVEL)   : _T("Travel Time");
		case PB_TIMETRAVVAR:		return localized ? GetString(IDS_AP_TRAVELVAR)    : _T("Travel Var");
		case PB_TIMESTOP:			return localized ? GetString(IDS_AP_TIMESTOP)     : _T("Stop Time");
		case PB_ICONSIZE:			return localized ? GetString(IDS_AP_ICONSIZE)     : _T("Icon Size");
		default:						return _T("");
		}
	}
...
{
public:
	Point3 Map(int, Point3 p) const override
	{
		return p;
	}
...
//		srand(pd->lastseed);
	...
	if (pathOb->NumberOfCurves(t) <= 0) return FALSE;
...
// do this if we haven't seen the particle yet, if the particle is out of range, or if the particle is new
	...
	TimeValue intime	= t;
	TimeValue endtime	= pd->psaved[index].Tc + pd->psaved[index].Tt;
	TimeValue difftime	= 0;
	...
	switch (message) {		
		...
		case REFMSG_CHANGE:
			{ if (pblock && (pblock==hTarget))
			  ...
			  SimpleWSMObject::NotifyRefChanged (changeInt, hTarget, partID, message, propagate);;
			}
			break;
			}
		default: SimpleWSMObject::NotifyRefChanged (changeInt, hTarget, partID, message, propagate);;
		}
	return REF_SUCCEED;
	}
