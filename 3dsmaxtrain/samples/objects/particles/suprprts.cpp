...
{  InTmBegin.NoTrans();
   ...
   float Kx(0.0f), Ky(0.0f), Kz(0.0f);
	...
	float tsqrt = (Ox-Ay)*(Oz-Ay)+(Ax-Nz)*(Ax-Nz)+(Ny-Ox)*(Ny-Ox);
	if (tsqrt < 0.0f)
		return;
   bigtheta=(float)atan(sqrt(tsqrt)/(Nx+Oy+Az-1));
...
{ size_t len= _tcslen(name);
  if (len>MAXNAME) name[MAXNAME]=_T('\0');
}
...
BOOL WriteInt(DWORD *buf,FILE *f)
{
	return(fwrite(buf,isize,1,f)==1);
}

BOOL GenNewSaveFile(int osize,int size,int custsettings,FILE *f,const TCHAR *filename,int vers, bool convertNameFromAscii)
{
	...
	ok=(WriteInt(&custsettings,f)&&WriteInt(&vers,f)&&WriteInt(&size,f)&&WriteInt(&optionFlags,f));
	if (custsettings>0)
	{
		int inNameLen = convertNameFromAscii ? NLEN : NLENBytes;
		int outNameLen = NLENBytes;
		int cset=(custsettings-1),odata=osize+inNameLen;
		AName Name;CName cName;
		void* inName = convertNameFromAscii ? (void*)cName : (void*)Name;
		int ofs=(size-osize)+(outNameLen-inNameLen),tofs=cset*ofs;
		BYTE *data=new BYTE[size];for (int x=0;x<size;x++) data[x]=0;
		cset*=odata;cset+=HLEN;
		while (cset>=HLEN)
		{
			fseek(f,cset,SEEK_SET);
			if (fread(inName,inNameLen,1,f)==1)
			{
				if (fread(data,osize,1,f)!=1) goto badend;
				fseek(f,cset+tofs,SEEK_SET);
				if (convertNameFromAscii)
				{
					TSTR convName = TSTR::FromCP(GetCOREInterface14()->DefaultTextLoadCodePage(), cName);
					wcscpy(Name,convName);
				}
				if (fwrite(Name,1,outNameLen,f)!=outNameLen) goto badend;
...
// convert from 16 char ascii to 255 char ascii
BOOL GenNewNameLen(int size,int custsettings,FILE *f,const TCHAR *filename,int vers)
{
...
{ 
  int i;
  for (i=0;(i<len)&&(active[i]!=partnum);i++);
  int mnum=len-i-1;
...
{ int found=FALSE;int i=0;
  while ((i<max)&&((found=((hit==sl.ppts[i].p1)&&(colobj==sl.ppts[i].p2)) ) == 0)  ) i++;
  return (found);
}
                  ...
                  const Point3& deftex, const TVFace& Zerod, const int subf, const BOOL mirror)
{
   ...
   int mapCount = triOb->GetMesh().getNumMaps();
   if (cmesh->getNumMaps()<mapCount)
      cmesh->setNumMaps( mapCount, TRUE );
...
int computeHorizontalExtent(HWND hListBox, BOOL useTabs, int cTabs, LPINT lpnTabs)
{
   ...
   { /* compute the buffer size */
      ...
      if (useTabs) {
         DWORD sz = GetTabbedTextExtent(dc, Buffer, static_cast<int>(_tcslen(Buffer)), cTabs, lpnTabs ); // SR DCAST64: Downcast to 2G limit.
         cx = LOWORD(sz);
      }
      else {
         SIZE sz;
