...
class SphereDefObject : public SimpleWSMObject2 {
	public:
		...
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_SPHDEFLECTOR) : _T("SDeflector");}
...
class SphereDeflectorClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("SDeflector"); }
...
class SphereDeflectMod : public SimpleWSMMod {
	public:				
		...
		// From Animatable
		void GetClassName(MSTR& s, bool localized) const override {s= localized ? GetString(IDS_RB_SPHDEFLECTMOD) : _T("SDeflectMod");}
		SClass_ID SuperClassID() {return WSM_CLASS_ID;}
		void DeleteThis() {delete this;}
		Class_ID ClassID() { return SPHEREDEFMOD_CLASS_ID;}
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_RB_SPHDEFBINDING) : _T("SDeflectBinding");}
...
class SphereDeflectorModClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("SDeflectMod"); }
	SClass_ID		SuperClassID() {return WSM_CLASS_ID; }
	Class_ID		ClassID() {return SPHEREDEFMOD_CLASS_ID;}
...
(	sdeflectrobj_params, 
	...
	sdeflectrobj_radius,	_T("radius"),		TYPE_FLOAT,		P_ANIMATABLE + P_RESET_DEFAULT,		IDS_AP_ICONSIZED,
		...
		p_nonLocalizedName, _T("Diameter"),
		p_end,
...
/*
TSTR SphereDefObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
...
//watje load up the engine
		...
		{	//macroRecorder->Disable();
			...
			Control* cont = obj->pblock2->GetControllerByID(sdeflectrobj_radius);
			if (cont) {
				 cols->pblock->SetControllerByID(collisionsphere_radius, 0, cont, FALSE);
	...
	public:
		...
		void proc(ILoad *iload) {  
			    ...
			    (LOWORD(iload->GetFileSaveVersion()) < 48)) // MAX v.3.9, build 48
			{
				...
				ParamID defl_chan[3] = {sdeflectrobj_bouncevar, sdeflectrobj_chaos, sdeflectrobj_friction};
				...
				cont = n->pblock2->GetControllerByID(sdeflectrobj_velocity);
