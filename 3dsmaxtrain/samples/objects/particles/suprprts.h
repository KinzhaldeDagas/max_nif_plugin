...
typedef wchar_t AName[NLEN];
typedef char CName[NLEN];
const int NLENBytes = NLEN*sizeof(wchar_t);
...
extern void DL_CacheData(ParticleSys *p0,ParticleSys *p1);
...
extern BOOL ReadInt(DWORD *buf,FILE *f);
extern BOOL WriteInt(int *buf,FILE *f);
extern BOOL WriteInt(DWORD *buf,FILE *f);
extern BOOL GenNewSaveFile(int osize,int size,int custsettings,FILE *f,const TCHAR *filename,int vers, bool convertNameFromAscii);
extern BOOL GenNewNameLen(int size,int custsettings,FILE *f,const TCHAR *filename,int vers);
