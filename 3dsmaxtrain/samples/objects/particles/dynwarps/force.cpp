...
class ForceObject : public SimpleWSMObject // , IOperatorInterface  // Bayboro 9/18/01
{
	public:
		...
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_AP_FORCE) : _T("Push");}
...
{	public:
		...
		const TCHAR*	NonLocalizedClassName() { return _T("Push"); }
...
{	public:				
		...
		// From Animatable
		void GetClassName(MSTR& s, bool localized) const override {s= localized ? GetString(IDS_AP_FORCEMOD) : _T("PushMod");}
		SClass_ID SuperClassID() {return WSM_CLASS_ID;}
		void DeleteThis() {delete this;}
		Class_ID ClassID() { return FORCEMOD_CLASS_ID;}
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_AP_FORCEBINDING) : _T("Push Binding");}
...
{	public:
		...
		const TCHAR*	NonLocalizedClassName() { return _T("PushMod"); }
		SClass_ID		SuperClassID() {return WSM_CLASS_ID;}
		Class_ID		ClassID() {return FORCEMOD_CLASS_ID;}
...
static ParamUIDesc descParamForce[] = {
	...
	// Reversible
	ParamUIDesc(PB_REVERSIBLE,TYPE_SINGLECHECKBOX,IDC_AP_REVERSIBLE),
	...
	// Range on
	ParamUIDesc(PB_RANGEON,TYPE_SINGLECHECKBOX,IDC_AP_RANGE),
...
TSTR ForceObject::GetParameterName(int pbIndex, bool localized)
{	switch (pbIndex)
	{	case PB_ONTIME:		return localized ? GetString(IDS_AP_ONTIME)      : _T("On Time");
		...
		case PB_STRENGTH:		return localized ? GetString(IDS_AP_STRENGTH)    : _T("Basic Force");
		...
		case PB_TARGETVEL:	return localized ? GetString(IDS_AP_TARGETVEL)   : _T("Target Speed");
...
{	ParticleObject *obj = GetParticleInterface(os->obj);
	if (obj && nodeRef)
	{
		force.obj = static_cast<ForceObject*>(GetWSMObject(t));
		// If the forcefield target of the modifier gets deleted, there's no object -- so no point
		// in setting up the ForceField -- so only do the work if it's present. MAXX-35650
		if (force.obj)
		{
			force.node=nodeRef;
