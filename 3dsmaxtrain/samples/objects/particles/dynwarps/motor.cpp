...
{	public:
		...
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_AP_MOTOR) : _T("Motor");}
...
{	public:
		...
		const TCHAR*	NonLocalizedClassName() { return _T("Motor"); }
...
{	public:				
		...
		// From Animatable
		void GetClassName(MSTR& s, bool localized) const override {s= localized ? GetString(IDS_AP_MOTORMOD) : _T("MotorMod");}
		SClass_ID SuperClassID() {return WSM_CLASS_ID;}
		void DeleteThis() {delete this;}
		Class_ID ClassID() { return MOTORMOD_CLASS_ID;}
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_AP_MOTORBINDING) : _T("Motor Binding");}
...
{	public:
		...
		const TCHAR*	NonLocalizedClassName() { return _T("MotorMod"); }
		SClass_ID		SuperClassID() {return WSM_CLASS_ID;}
		Class_ID		ClassID() {return MOTORMOD_CLASS_ID;}
...
static ParamUIDesc descParamMotor[] = {
	...
	// Feedback
	ParamUIDesc(PB_FEEDBACKON,TYPE_SINGLECHECKBOX,IDC_AP_FEEDBACKON),

	// Reversible
	ParamUIDesc(PB_REVERSIBLE,TYPE_SINGLECHECKBOX,IDC_AP_REVERSIBLET),
	...
	// Reversible
	ParamUIDesc(PB_ENABLESINES,TYPE_SINGLECHECKBOX,IDC_AP_VARIATIONON),
	...
	// Range on
	ParamUIDesc(PB_RANGEON,TYPE_SINGLECHECKBOX,IDC_AP_RANGEM),
...
{	int FToTick=(int)((float)TIME_TICKSPERSEC/(float)GetFrameRate());
	ReplaceReference( 0,CreateParameterBlock(descMotorVer0, PBLOCK_LENGTH, CURRENT_VERSION));
	assert(pblock);	
	ff = NULL;
	mf = NULL;
...
INT_PTR MotorDlgProc::DlgProc(TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{	switch (msg) 
	{	case WM_INITDIALOG: 
		{	hwnd=hWnd;
			Update(t);
...
TSTR MotorObject::GetParameterName(int pbIndex, bool localized)
{	switch (pbIndex)
	{	case PB_ONTIME:		return localized ? GetString(IDS_AP_ONTIME)      : _T("On Time");
		case PB_OFFTIME:		return localized ? GetString(IDS_AP_OFFTIME)     : _T("Off Time");
		case PB_STRENGTH:		return localized ? GetString(IDS_AP_STRENGTHT)   : _T("Basic Torque");
		case PB_UNITS:			return localized ? GetString(IDS_AP_UNITS)       : _T("Units");
		case PB_FEEDBACKON:	return localized ? GetString(IDS_AP_FEEDBACKON)  : _T("Feedback On");
		case PB_REVERSIBLE:	return localized ? GetString(IDS_AP_REVERSIBLE)  : _T("Reversible");
		case PB_TARGETVEL:	return localized ? GetString(IDS_AP_TARGETREVS)  : _T("Target Revs");
		case PB_REVSUNITS:	return localized ? GetString(IDS_AP_REVSUNITS)   : _T("Revs Units");
		case PB_CONTROLGAIN:	return localized ? GetString(IDS_AP_CONTROLGAIN) : _T("Control Gain");
		case PB_ENABLESINES:	return localized ? GetString(IDS_AP_ENABLESINES) : _T("Enable Variation");
		case PB_TIMEPER1:		return localized ? GetString(IDS_AP_TIMEPER1)    : _T("Variation Period 1");
		case PB_AMP1:			return localized ? GetString(IDS_AP_AMP1)        : _T("Amplitude 1");
		case PB_PHASPER1:		return localized ? GetString(IDS_AP_PHASPER1)    : _T("Variation Phase 1");
		case PB_TIMEPER2:		return localized ? GetString(IDS_AP_TIMEPER2)    : _T("Variation Period 2");
		case PB_AMP2:			return localized ? GetString(IDS_AP_AMP2)        : _T("Amplitude 2");
		case PB_PHASPER2:		return localized ? GetString(IDS_AP_PHASPER2)    : _T("Variation Phase 2");
		case PB_RANGEON:		return localized ? GetString(IDS_AP_RANGEON)     : _T("Range Enable");
		case PB_RANGEVAL:		return localized ? GetString(IDS_AP_RANGEVAL)    : _T("Range Value");
