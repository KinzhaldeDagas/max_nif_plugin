...
class UniDefObject : public SimpleWSMObject2 {
	public:
		...
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_AP_UNIDEFLECTOR) : _T("UDeflector");}
...
class UniDeflectorClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("UDeflector"); }
...
class UniDeflectMod : public SimpleWSMMod {
	public:				
		...
		// From Animatable
		void GetClassName(MSTR& s, bool localized) const override {s= localized ? GetString(IDS_AP_UNIDEFMOD) : _T("UDeflectorMod");}
		SClass_ID SuperClassID() {return WSM_CLASS_ID;}
		void DeleteThis() {delete this;}
		Class_ID ClassID() { return UNIDEFMOD_CLASS_ID;}
		RefTargetHandle Clone(RemapDir& remap);
		const TCHAR *GetObjectName(bool localized) const override {return localized ? GetString(IDS_AP_UNIDEFBINDING) : _T("UDeflectorBinding");}
...
class UniDeflectorModClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("UDeflectorMod"); }
	SClass_ID		SuperClassID() {return WSM_CLASS_ID; }
	Class_ID		ClassID() {return UNIDEFMOD_CLASS_ID;}
...
(	udeflectrobj_params, 
	...
	udeflectrobj_bouncevar, _T("bouncevar"),	TYPE_PCNT_FRAC,		P_ANIMATABLE + P_RESET_DEFAULT,		IDS_RB_BVAR,
		...
		p_nonLocalizedName, _T("Bounce Variation"),
		p_end,
	...
	udeflectrobj_radius,	_T("radius"),		TYPE_FLOAT,		P_ANIMATABLE + P_RESET_DEFAULT,		IDS_AP_ICONSIZE,
		...
		p_nonLocalizedName, _T("Icon Size"),
		p_end,
	...
	udeflectrobj_velocity,	_T("inheritVelocity"),	TYPE_FLOAT,	P_ANIMATABLE + P_RESET_DEFAULT,		IDS_AP_VELOCITY,
		...
		p_nonLocalizedName, _T("Velocity Inherit"),
		p_end,
...
class CreateUniObjectProc : public MouseCallBack,ReferenceMaker {
	private:
		...
		virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("CreateUniObjectProc"); } // from Animatable
...
{ if ( UniObj ) 
	{
		...
		{	// RB 4-9-96: Normally the hold isn't holding when this 
			...
			theHold.Suspend(); 
			UniObj->DeleteAllRefsFromMe();
			UniObj->DeleteAllRefsToMe();
			UniObj->DeleteThis();
			UniObj = NULL;
			theHold.Resume();
		...
		case MOUSE_POINT:
			switch ( point ) {
              ...
              createInterface->GetMacroRecorder()->EmitScript();
					...
					// link it up
					INode *l_CloudNode = createInterface->CreateObjectNode( UniObj);
					attachedToNode = TRUE;
...
{	
	TSTR name = TSTR(GetString(IDS_AP_ITEMSTR)) + (custnode ? custname : TSTR(GetString(IDS_AP_NONE))); 
...
INT_PTR UniObjectDlgProc::DlgProc(
...
void UniDefObject::BeginEditParams(IObjParam *ip,ULONG flags,Animatable *prev)
{
	this->ip = ip;
...
void UniDefObject::EndEditParams(IObjParam *ip, ULONG flags,Animatable *next)
{
...
/*
TSTR UniDefObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_BOUNCE:	return localized ? GetString(IDS_RB_BOUNCE)   : _T("Bounce");
		case PB_BVAR:		return localized ? GetString(IDS_RB_BVAR)     : _T("Bounce Variation");
		case PB_CHAOS:		return localized ? GetString(IDS_RB_CHAOS)    : _T("Chaos");
		case PB_RADIUS:	return localized ? GetString(IDS_AP_ICONSIZE) : _T("Icon Size");
		case PB_FRICTION:	return localized ? GetString(IDS_AP_FRICTION) : _T("Friction");
		case PB_VELOCITY:	return localized ? GetString(IDS_AP_VELOCITY) : _T("Velocity Inherit");
		default:				return _T("");
		}
	}
*/
...
int RayIntersect(Ray& ray, float& at, Point3& norm,Mesh *amesh,MaxSDK::VertexNormal* vnorms,Point3 *fnorms)
	{
	DWORD fi;
	Point3 bary;
...
void GetVFLst(Mesh* dmesh,MaxSDK::VertexNormal* vnorms,Point3* fnorms)	 
...
//DebugPrint(_T("   pos %f %f %f\n"),inp.x,inp.y,inp.z);
//DebugPrint(_T("   vel %f %f %f\n"),vel.x,vel.y,vel.z);
...
class UniDeflectObjectLoad : public PostLoadCallback 
{
	public:
		...
		{  
			...
			{
				Control *cont = NULL, *mult = NULL;
				float pcnt = 0.01f;

				ParamID defl_chan[3] = {udeflectrobj_bouncevar, udeflectrobj_chaos, udeflectrobj_friction};
				...
				if (HIWORD(iload->GetFileSaveVersion()) >= 3900)
					for( int i=0; i<3; i++)
					{	cont = n->pblock2->GetControllerByID(defl_chan[i]);
						if (cont == NULL)
							n->pblock2->SetValue(defl_chan[i], 0, 0.01f*n->pblock2->GetFloat(defl_chan[i], 0));
						else
						{	if (cont->CanApplyEaseMultCurves() && cont->IsLeaf())
				...
				cont = n->pblock2->GetControllerByID(udeflectrobj_velocity);
