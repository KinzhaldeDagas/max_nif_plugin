...
static ParamUIDesc descParamSuprSpray[] = {
   ...
   // Hide Emitter
   ParamUIDesc(PB_EMITRHID,TYPE_SINGLECHECKBOX,IDC_SP_EMITHID),
...
static ParamUIDesc descParamPGen[] = {
   ...
   ParamUIDesc(PB_SUBFRAMEMOVE,TYPE_SINGLECHECKBOX,IDC_SP_GENSMPLMOVE),
   ParamUIDesc(PB_SUBFRAMETIME,TYPE_SINGLECHECKBOX,IDC_SP_GENSMPLTIME),
   ParamUIDesc(PB_SUBFRAMEROT,TYPE_SINGLECHECKBOX,IDC_SP_GENSMPLROT),
...
static ParamUIDesc descParamPType[] = {
   ...
   ParamUIDesc(PB_METAAUTOCOARSE,TYPE_SINGLECHECKBOX,IDC_SP_AUTOCOARSE),

   // Display type
   ParamUIDesc(PB_SSNOTDRAFT,TYPE_SINGLECHECKBOX,IDC_SP_DRAFTMODE),

   // Use Subtree Checkbox
   ParamUIDesc(PB_USESUBTREE,TYPE_SINGLECHECKBOX,IDC_AP_USESUBTREE),
...
static ParamUIDesc descParamPSpin[] = {
   ...
   // IPC Enable
   ParamUIDesc(PB_IPCOLLIDE_ON,TYPE_SINGLECHECKBOX,IDC_INTERP_BOUNCEON),
...
static ParamUIDesc descPSpawning[] = {
   ...
   ParamUIDesc(PB_SPAWNINHERITV,TYPE_SINGLECHECKBOX,IDC_AP_SPAWNSUMV),

   // Spawning Speed Fixed
   ParamUIDesc(PB_SPAWNSPEEDFIXED,TYPE_SINGLECHECKBOX,IDC_AP_SPAWNSPEEDFIXED),
   ...
   // Spawning Speed Fixed
   ParamUIDesc(PB_SPAWNSCALEFIXED,TYPE_SINGLECHECKBOX,IDC_AP_SPAWNSCALEFIXED),
...
static ParamUIDesc descBParamPSpin[] = {
   ...
   // IPC Enable
   ParamUIDesc(PB_BLIPCOLLIDE_ON,TYPE_SINGLECHECKBOX,IDC_INTERP_BOUNCEON),
...
static ParamUIDesc BdescParamPType[] = {
   ...
   // Display type
   ParamUIDesc(PB_BLNOTDRAFT,TYPE_SINGLECHECKBOX,IDC_SP_DRAFTMODE),
...
static ParamUIDesc BdescParamPGen[] = {
   ...
   ParamUIDesc(PB_SUBFRAMEMOVE,TYPE_SINGLECHECKBOX,IDC_SP_GENSMPLMOVE),
   ParamUIDesc(PB_SUBFRAMETIME,TYPE_SINGLECHECKBOX,IDC_SP_GENSMPLTIME),
   ParamUIDesc(PB_SUBFRAMEROT2,TYPE_SINGLECHECKBOX,IDC_SP_GENSMPLROT),
...
class CreateSSBlizProc : public MouseCallBack,ReferenceMaker {
   private:
      ...
      virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("CreateSSBlizProc"); } // from Animatable
...
{ if ( SSBlizObject ) 
   ...
   SSBlizObject->EndEditParams( (IObjParam*)createInterface, 
      ...
      {  // RB 4-9-96: Normally the hold isn't holding when this 
         ...
         theHold.Suspend(); 
		 SSBlizObject->DeleteAllRefsFromMe();
		 SSBlizObject->DeleteAllRefsToMe();
		 SSBlizObject->DeleteThis();
         SSBlizObject = NULL;
         theHold.Resume();
         if (theHold.GetGlobalPutCount()!=lastPutCount) 
            GetSystemSetting(SYSSET_CLEAR_UNDO);
         ...
         switch ( point ) {
            case 0:
              ...
              createInterface->GetMacroRecorder()->EmitScript();
               ...
               // link it up
               l_CloudNode = createInterface->CreateObjectNode( SSBlizObject);
               attachedToNode = TRUE;
               assert( l_CloudNode );             
            ...
            default:          
               if (createCB) {                  
                  res = createCB->proc(vpx.ToPointer(),msg,point,flag,m,mat);
...
INT_PTR SSParticleGenDlgProc::DlgProc(
...
INT_PTR BLParticleDlgProc::DlgProc(
      TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{  switch (msg) {
      case WM_INITDIALOG: {
         break;
...
INT_PTR SSParticleDlgProc::DlgProc(
      TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{  switch (msg) {
      case WM_INITDIALOG: {
         break;
...
INT_PTR SSParticleDisableDlgProc::DlgProc(
      TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{  switch (msg) {
      case WM_INITDIALOG: {
...
INT_PTR BLParticleDisableDlgProc::DlgProc(
      TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{  switch (msg) {
      case WM_INITDIALOG: {
         iBut = GetICustButton(GetDlgItem(hWnd,IDC_AP_OBJECTPICK));
...
TSTR CommonParticle::GetFilename()
{
    // Blizzard.cst is moved to plugcfg_ln
    TSTR filename(ip->GetDir(APP_PLUGCFG_LN_DIR));
    ...
    {
        filename.append(GetString(IDS_AP_BLIZZARDCST));
    }
    else
    {
        filename.append(GetString(IDS_AP_SSPRAYCST));
    }
...
{
	TSTR filename(GetFilename());
	FILE *f;
	int vers,i,osize;
	DWORD optionFlags; // bits 0 to 7 - # of chars in name, bit 8 - if set, unicode names
	...
	if((ReadInt(&custsettings,f))&&(ReadInt(&vers,f))&&(ReadInt(&osize,f))&&(ReadInt(&optionFlags,f)))
	{
		int Namelen = optionFlags & 0x0ff;
		bool unicodeNames = (optionFlags & (1<<8)) == (1<<8);
		if ( ((Namelen!=NLEN)&& (fileok=GenNewNameLen(size,custsettings,f,filename,CURRENT_VERSION)) == TRUE)||
			(((vers!=CURRENT_VERSION)||!unicodeNames)&&(fileok=GenNewSaveFile(osize,size,custsettings,f,filename,CURRENT_VERSION, !unicodeNames)) == TRUE))
		{
			fileok=((ReadInt(&custsettings,f))&&(ReadInt(&vers,f))&&(ReadInt(&osize,f))&&(ReadInt(&optionFlags,f)));
		}
		if (fileok&&(vers==CURRENT_VERSION)&&(optionFlags & 0x0ff)&&(optionFlags & (1<<8)))
		{
			NameLst=new AName[custsettings]; 
			for (i=0;i<custsettings;i++)
				if (fread(NameLst[i],NLENBytes,1,f)==1)
...
onerr:
	...
	{
		custsettings=0;
		MaxSDK::MaxMessageBox(NULL, GetString(IDS_RB_BADFILE), _T(""), MB_ICONINFORMATION);
	}
	if (fileok) fclose(f);
...
{ TSTR filename(GetFilename());
  FILE *f;
  long startpt;
  int vers,newsets,baselen=size+NLENBytes;
  vers=CURRENT_VERSION;
  newsets=custsettings-1;
  if ((f = _tfopen(filename,_T("r+b"))) == NULL) 
  { MaxSDK::MaxMessageBox(NULL, GetString(IDS_AP_WRITEPRO), _T(""), MB_ICONINFORMATION);
   return 0;
  }
  if (custsettings==1) 
  {fclose(f);_tremove(filename);custsettings=0;
...
int CommonParticle::SaveSettings(int overwrite,TCHAR *newname)
{
  FILE *f;
  int vers,newsets;
  DWORD optionFlags = 0x0ff | (1<<8); // bits 0 to 7 - # of chars in name (255), bit 8 - if set, unicode names

  TSTR msgBoxText = GetString(IDS_AP_SETEXISTS);
  if ((overwrite > -1) && (MaxSDK::MaxMessageBox(NULL, msgBoxText, GetString(IDS_AP_WARNING), MB_ICONQUESTION | MB_YESNO) == IDNO))
    return 0;
  vers=CURRENT_VERSION;
  newsets=custsettings+1;
  TSTR filename(GetFilename());
 ...
 // longest one
  ...
  if (custsettings==0)
  { if (!(WriteInt(&newsets,f)&&WriteInt(&vers,f)&&WriteInt(&size,f)&&WriteInt(&optionFlags,f))) goto errend;
  } 
  else 
  { if (overwrite>=0) 
    { overwrite=overwrite*(size+NLENBytes)+HLEN;
  ...
  int ival,i;
  float fval;
  if (fwrite(newname,1,NLENBytes,f)!=NLENBytes) goto errend;
  for (i=0;i<plength;i++)
...
{ TSTR filename(GetFilename());
  ...
  { setnum=setnum*(size+NLENBytes)+HLEN;
  ...
  ParamBlockDescID *descVer=((ClassID()==BLIZZARD_CLASS_ID)?BdescVer6:spdescVer6);
 int plength=((ClassID()==BLIZZARD_CLASS_ID)?PBLOCK_LENGTH_BLIZZARD:PBLOCK_LENGTH_SUPRSPRAY);
  if (fread(newname,1,NLENBytes,f)!=NLENBytes) goto errend;
  for (i=0;i<plength;i++)
  {   if (descVer[i].type==TYPE_INT) 
...
{ TCHAR newname[NLEN];
  int i = 0, save = 0;
  CommonParticle *po = DLGetWindowLongPtr<CommonParticle*>(hWnd);
         ...
         { switch(LOWORD(wParam)) {
            ...
            case IDC_SP_LOAD:
               ...
               { i = SendMessage(GetDlgItem(po->hParams2,IDC_SP_SETLIST),
                ...
                po->custCtrlEdit->GetText(newname,NLEN);
               if (save) 
               {int tstblk=0,tstlen=static_cast<int>(_tcslen(newname)); // SR DCAST64: Downcast to 2G limit.
                 while ((tstblk<tstlen)&&(newname[tstblk]==_T(' '))) tstblk++;
                 if (tstblk>=tstlen) 
                  MaxSDK::MaxMessageBox(NULL, GetString(IDS_RB_NONAME), _T(""), MB_ICONINFORMATION);
                 else 
                 { i=0;
             ...
             { CheckSpawnButtons(po->pblock,po->spawn,po->pickCB.repi);
               ...
               else MaxSDK::MaxMessageBox(NULL, GetString(IDS_RB_BADNAME), _T(""), MB_ICONINFORMATION);
               break;
            }
         break;
         }
...
INT_PTR ComParticleSpawnDlgProc::DlgProc(
      TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
...
INT_PTR BParticleSpinDlgProc::DlgProc(
      TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{  switch (msg) {
      case WM_INITDIALOG: 
      { Update(t);
...
INT_PTR SSParticleSpinDlgProc::DlgProc(
      TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{  switch (msg) {
      case WM_INITDIALOG: 
      { Update(t);
...
{int FToTick=(int)((float)TIME_TICKSPERSEC/(float)GetFrameRate());
   ...
   ReplaceReference(PBLK, CreateParameterBlock(spdescVer6, PBLOCK_LENGTH_SUPRSPRAY, CURRENT_VERSION));
...
void SuprSprayParticle::BeginEditParams(IObjParam *ip, ULONG flags,Animatable *prev)
{ 
...
void SuprSprayParticle::EndEditParams( IObjParam *ip, ULONG flags,Animatable *next)
{
	TimeValue t0,t2;
	...
	else
	{
		...
		pmapPType->SetUserDlgProc(nullptr);
		 pmapPGen->SetUserDlgProc(nullptr);
		pmapParam->SetUserDlgProc(nullptr);
		pmapSpawn->SetUserDlgProc(nullptr);
		pmapPSpin->SetUserDlgProc(nullptr);

		pmapParam->SetParamBlock(nullptr);
		 pmapPGen->SetParamBlock(nullptr);
		pmapPType->SetParamBlock(nullptr);
		pmapPSpin->SetParamBlock(nullptr);
		pmapEmitV->SetParamBlock(nullptr);
		 pmapBubl->SetParamBlock(nullptr);
		pmapSpawn->SetParamBlock(nullptr);
	}
	ip->ClearPickMode();
	ip= NULL;
	creating = FALSE;
...
{  Mtl *submtl;
   ...
   Mtl *nmtl=NULL;
   TSTR newname;
   TypedSingleRefMaker<MtlBaseLib> glib(new MtlBaseLib);
   int tabmax = 256;
   newname=TSTR(_T("CMat"))+node->GetName();
   if (_tcslen(newname)>16) newname.Resize(16);
...
{ int tabmax=256;
  ...
  { theSuprSprayDraw.bboxpt=new boxlst[count];
    ...
    { TimeValue tofs=(ocount>0?times.tl[tcount].tl:t);
     ...
     { if ((tmpnode=nlist[cgen]) == NULL) tmpnode=custnode;
...
/* if (node!=custnode)
   ...
   int submtls=0;
   Mtl* m=node->GetMtl();
   if (!((m!=NULL)&&(submtls=m->NumSubMtls()) != 0)) 
     submtls=-1;
   int tottv;
...
{  int tnums = 0, numV = 0, numF = 0, tvnum = 0, ismapped = 0;
   INode *tmpnode = NULL;
...
BOOL SuprSprayParticle::ComputeParticleStart(TimeValue t0,int c)
   ...
   {   parts.FreeAll();if (sdata) delete sdata;sdata=NULL;maincount=0;
      ...
      if (playing) GetCOREInterface()->EndAnimPlayback();
       TSTR name;name=(cnode ? cnode->GetName() : GetString(IDS_AP_SUPRSPRAY));
...
TSTR SuprSprayParticle::GetParameterName(int pbIndex, bool localized)
   {
   switch (pbIndex) {
      case PB_OFFAXIS:             return localized ? GetString(IDS_AP_OFFAXIS)             : _T("Off Axis");
      case PB_AXISSPREAD:          return localized ? GetString(IDS_AP_AXISSPREAD)          : _T("Axis Spread");
      case PB_OFFPLANE:            return localized ? GetString(IDS_AP_OFFPLANE)            : _T("Off Plane");
      case PB_PLANESPREAD:         return localized ? GetString(IDS_AP_PLANESPREAD)         : _T("Plane Spread");
      case PB_SPEED:               return localized ? GetString(IDS_RB_SPEED)               : _T("Speed");
      case PB_SPEEDVAR:            return localized ? GetString(IDS_RB_SPEEDVAR)            : _T("Speed Variation");
      case PB_PBIRTHRATE:          return localized ? GetString(IDS_RB_PBIRTHRATE)          : _T("Birth Rate");
      case PB_PTOTALNUMBER:        return localized ? GetString(IDS_RB_PTOTALNUMBER)        : _T("Total Number");
      case PB_DISPLAYPORTION:      return localized ? GetString(IDS_RB_DISPLAYPORTION)      : _T("Percentage Displayed");
      case PB_EMITSTART:           return localized ? GetString(IDS_RB_EMITSTART)           : _T("Emitter Start");
      case PB_EMITSTOP:            return localized ? GetString(IDS_RB_EMITSTOP)            : _T("Emitter Stop");
      case PB_DISPUNTIL:           return localized ? GetString(IDS_RB_DISPUNTIL)           : _T("Display Until");
      case PB_LIFE:                return localized ? GetString(IDS_RB_LIFE)                : _T("Life");
      case PB_LIFEVAR:             return localized ? GetString(IDS_RB_LIFEVAR)             : _T("Life Variation");
      case PB_SIZE:                return localized ? GetString(IDS_RB_SIZE)                : _T("Size");
      case PB_SIZEVAR:             return localized ? GetString(IDS_RB_SIZEVAR)             : _T("Size Variation");
      case PB_GROWTIME:            return localized ? GetString(IDS_RB_GROWTIME)            : _T("Growth Time");
      case PB_FADETIME:            return localized ? GetString(IDS_RB_FADETIME)            : _T("Fade Time");
      case PB_RNDSEED:             return localized ? GetString(IDS_RB_RNDSEED)             : _T("Random Seed");
      case PB_METATENSION:         return localized ? GetString(IDS_RB_METATENSION)         : _T("Metaparticle Tension");
      case PB_METATENSIONVAR:      return localized ? GetString(IDS_RB_METATENSIONVAR)      : _T("Metaparticle Tension Variation");
      case PB_METACOURSE:          return localized ? GetString(IDS_RB_METACOURSE)          : _T("Metaparticle Coarseness");
      case PB_MAPPINGTIME:         return localized ? GetString(IDS_RB_MAPPINGTIME)         : _T("Mapping Time Base");
      case PB_MAPPINGDIST:         return localized ? GetString(IDS_RB_MAPPINGDIST)         : _T("Mapping Distance Base");
      case PB_SPINTIME:            return localized ? GetString(IDS_RB_SPINTIME)            : _T("Spin Time");
      case PB_SPINTIMEVAR:         return localized ? GetString(IDS_RB_SPINTIMEVAR)         : _T("Spin Time Variation");
      case PB_SPINPHASE:           return localized ? GetString(IDS_RB_SPINPHASE)           : _T("Spin Phase");
      case PB_SPINPHASEVAR:        return localized ? GetString(IDS_RB_SPINPHASEVAR)        : _T("Spin Phase Variation");
      case PB_SPINAXISX:           return localized ? GetString(IDS_RB_SPINAXISX)           : _T("X Spin Vector");
      case PB_SPINAXISY:           return localized ? GetString(IDS_RB_SPINAXISY)           : _T("Y Spin Vector");
      case PB_SPINAXISZ:           return localized ? GetString(IDS_RB_SPINAXISZ)           : _T("Z Spin Vector");
      case PB_SPINAXISVAR:         return localized ? GetString(IDS_RB_SPINAXISVAR)         : _T("Spin Axis Variation");
      case PB_EMITVINFL:           return localized ? GetString(IDS_RB_EMITVINFL)           : _T("Object Motion Inheritance");
      case PB_EMITVMULT:           return localized ? GetString(IDS_RB_EMITVMULT)           : _T("Object Motion Multiplier");
      case PB_EMITVMULTVAR:        return localized ? GetString(IDS_RB_EMITVMULTVAR)        : _T("Object Motion Multiplier Variation");
      case PB_BUBLAMP:             return localized ? GetString(IDS_RB_BUBLAMP)             : _T("Bubble Amplitude");
      case PB_BUBLAMPVAR:          return localized ? GetString(IDS_RB_BUBLAMPVAR)          : _T("Bubble Amplitude Variation");
      case PB_BUBLPER:             return localized ? GetString(IDS_RB_BUBLPER)             : _T("Bubble Period");
      case PB_BUBLPERVAR:          return localized ? GetString(IDS_RB_BUBLPERVAR)          : _T("Bubble Period Variation");
      case PB_BUBLPHAS:            return localized ? GetString(IDS_RB_BUBLPHAS)            : _T("Bubble Phase");
      case PB_BUBLPHASVAR:         return localized ? GetString(IDS_RB_BUBLPHASVAR)         : _T("Bubble Phase Variation");
      case PB_EMITRWID:            return localized ? GetString(IDS_RB_EMITRWID)            : _T("Emitter Width");
      case PB_STRETCH:             return localized ? GetString(IDS_AP_STRETCH)             : _T("Blur Stretch");
      case PB_OFFSETAMOUNT:        return localized ? GetString(IDS_AP_OFFSETAMT)           : _T("Animation Offset Amount");
      case PB_SPAWNDIRCHAOS:       return localized ? GetString(IDS_AP_SPAWNDIRCHAOS)       : _T("Spawn Direction Chaos");
      case PB_SPAWNSPEEDCHAOS:     return localized ? GetString(IDS_AP_SPAWNSPEEDCHAOS)     : _T("Spawn Speed Chaos");
      case PB_SPAWNSCALECHAOS:     return localized ? GetString(IDS_AP_SPAWNSCALECHAOS)     : _T("Spawn Scale Chaos");
      case PB_SPAWNMULTVAR:        return localized ? GetString(IDS_EP_SPAWNMULTVAR)        : _T("Spawn Multiplier Variation");
      case PB_SPAWNPERCENT:        return localized ? GetString(IDS_EP_SPAWNAFFECTS)        : _T("Spawn Affects");
      case PB_SSSPAWNDIEAFTER:     return localized ? GetString(IDS_AP_SPAWNDIEAFTER)       : _T("Die  X frames after collision");
      case PB_SSSPAWNDIEAFTERVAR:  return localized ? GetString(IDS_AP_SPAWNDIEAFTERVAR)    : _T("Die  X frames after collision variation");
      case PB_IPCOLLIDE_ON:        return localized ? GetString(IDS_AP_IPCOLLIDE_ON)        : _T("Interparticle Collisions On");
      case PB_IPCOLLIDE_STEPS:     return localized ? GetString(IDS_AP_IPCOLLIDE_STEPS)     : _T("Interparticle Collision Steps");
      case PB_IPCOLLIDE_BOUNCE:    return localized ? GetString(IDS_AP_IPCOLLIDE_BOUNCE)    : _T("Interparticle Collision Bounce");
      case PB_IPCOLLIDE_BOUNCEVAR: return localized ? GetString(IDS_AP_IPCOLLIDE_BOUNCEVAR) : _T("Interparticle Collision Bounce Variation");
      default:                     return _T("");
      }
   }
   ...
   { aniend=GetAnimEnd();
     ...
     { int axisentered = 0;
      po->pblock->GetValue(PB_SPINAXISTYPE,0,axisentered,FOREVER);
       if ((po->theSuprSprayDraw.indir.inaxis=(axisentered==DIRTRAVEL)) == TRUE)
...
RefTargetHandle SuprSprayParticle::Clone(RemapDir& remap) 
   ...
   for (int ix=0;ix<llist.Count();ix++) newob->llist[ix]=llist[ix];
   newob->dflags=dflags;
   newob->dispmesh=NULL;
...
{  float Mval(0.0f), Uval(0.0f), Vval(0.0f), Wval(0.0f), Angle(0.0f), elapsedtime(0.0f);
   int type(0), count(0), maptype(0), anifr(0);
   float width(0.0f);
   Point3 Center;
   BOOL mirror=FALSE;
   TVFace defface;
   ...
   needDelete = TRUE;
   return pm;}
   TimeValue mt(0), aniend=GetAnimEnd();
   dispt=t;
    anifr=aniend+GetTicksPerFrame();
...
//    type=RENDTYPE1;
   ...
   else
   {
      ...
      GetCOREInterface()->GetActiveViewExp().GetAffineTM(wcam);
...
//   pm->setNumTVerts(gtv=(custmtl?gtvnum:count*numV));
   ...
   int axisentered;indir.vel=Zero;indir.K=0;
    pblock->GetValue(PB_SPINAXISTYPE,0,axisentered,FOREVER);
   if ((indir.inaxis=(axisentered==DIRTRAVEL)) == TRUE)
   ...
   { if ((!isrend)&&(GetAsyncKeyState (VK_ESCAPE)) )
     ...
     if (indir.inaxis)
      ...
      {  v  = (view.projType?otherV:Normalize(camV-parts[i]));
       ...
       {    Point3 a,b,Rv;
         float Theta(0.0f), R=Length(Rv=(parts[i]-camV)),angle=parts.radius[i]*R*Thetah/view.screenW;
...
const TCHAR* SuprSprayClassDesc::NonLocalizedClassName()
{
	return _T("Super Spray");
}

const TCHAR *SuprSprayClassDesc::Category () {return GetString(IDS_RB_PARTICLESYSTEMS);}
const TCHAR *SuprSprayParticle::GetObjectName(bool localized) const {return localized ? GetString(IDS_AP_SUPRSPRAYGC) : _T("SuperSpray");}
...
BlizzardParticle::BlizzardParticle()
   ...
   ReplaceReference(PBLK, CreateParameterBlock(BdescVer6, PBLOCK_LENGTH_BLIZZARD, CURRENT_VERSION));
   pblock->SetValue(PB_SPEED,0,10.0f);
   pblock->SetValue(PB_SPEEDVAR,0,0.0f);
   pblock->SetValue(PB_PBIRTHRATE,0,10);
   pblock->SetValue(PB_PTOTALNUMBER,0,100);
...
void BlizzardParticle::BeginEditParams( IObjParam *ip, ULONG flags,Animatable *prev)
{  
	SimpleParticle::BeginEditParams(ip,flags,prev);
	editOb = this;
	this->ip = ip;
...
void BlizzardParticle::EndEditParams(IObjParam *ip, ULONG flags,Animatable *next)
{   
	TimeValue t0,t2;
	SetFlag(dflags,APRTS_ROLLUP1_OPEN,IsRollupPanelOpen(hparam));
	SetFlag(dflags,APRTS_ROLLUP2_OPEN,IsRollupPanelOpen(hgen));
...
BOOL BlizzardParticle::ComputeParticleStart(TimeValue t0,int c)
   ...
   {   parts.FreeAll();if (sdata) delete sdata;sdata=NULL;  maincount=0;
      BOOL playing=GetCOREInterface()->IsAnimPlaying();
      if (playing) GetCOREInterface()->EndAnimPlayback();
       TSTR name;name=(cnode ? cnode->GetName() : GetString(IDS_AP_BLIZZARD));
      TSTR buf; buf=TSTR(GetString(IDS_OFM_PART));
      GetCOREInterface()->Log()->LogEntry(SYSLOG_ERROR,DISPLAY_DIALOG,
         GetString(IDS_OFM_ERROR),_T("%s: \n\n%s\n"),buf,name);
     return(0);
...
{
   TimeValue t0,dt,t2,grow,fade;
   int i(0), j(0), c(0), birth(0), counter(0), tpf=GetTicksPerFrame(), count=0, anioff(0);
   VelDir ptvel;
   int isrend = TestAFlag(A_RENDER), bmethod(0), onscreen(0), oneframe(0);
   // variable for new collision scheme (Bayboro 2/5/01)
   CollisionCollection cc;
...
{  float Mval=1.0f, Uval(0.0f), Vval(0.0f), Wval(0.0f), Angle(0.0f), elapsedtime(0.0f), tumble(0.0f), scale(0.0f);
   int type(0), count(0), maptype(0), anifr(0);
   float height(0.0f);
   Point3 Center;
   TVFace defface;
...
TSTR BlizzardParticle::GetParameterName(int pbIndex, bool localized)
   {
   switch (pbIndex) {
      case PB_TUMBLE:                return localized ? GetString(IDS_AP_TUMBLE)              : _T("Tumble");
      case PB_TUMBLERATE:            return localized ? GetString(IDS_AP_TUMBLERATE)          : _T("Tumble Rate");
      case PB_EMITRLENGTH:           return localized ? GetString(IDS_AP_EMITRLENGTH)         : _T("Emitter Length");
      case PB_SPEED:                 return localized ? GetString(IDS_RB_SPEED)               : _T("Speed");
      case PB_SPEEDVAR:              return localized ? GetString(IDS_RB_SPEEDVAR)            : _T("Speed Variation");
      case PB_PBIRTHRATE:            return localized ? GetString(IDS_RB_PBIRTHRATE)          : _T("Birth Rate");
      case PB_PTOTALNUMBER:          return localized ? GetString(IDS_RB_PTOTALNUMBER)        : _T("Total Number");
      ...
      case PB_BUBLAMPVAR:            return localized ? GetString(IDS_RB_BUBLAMPVAR)          : _T("Bubble Amplitude Variation");
      case PB_BUBLPER:               return localized ? GetString(IDS_RB_BUBLPER)             : _T("Bubble Period");
      case PB_BUBLPERVAR:            return localized ? GetString(IDS_EP_SPAWNAFFECTS)        : _T("Spawn Affects");
      case PB_BUBLPHAS:              return localized ? GetString(IDS_EP_SPAWNMULTVAR)        : _T("Spawn Multiplier Variation");
      case PB_BUBLPHASVAR:           return localized ? GetString(IDS_RB_BUBLPHASVAR)         : _T("Bubble Phase Variation");
      case PB_EMITRWID:              return localized ? GetString(IDS_RB_EMITRWID)            : _T("Emitter Width");
      case PB_OFFSETAMOUNT:          return localized ? GetString(IDS_AP_OFFSETAMT)           : _T("Animation Offset Amount");
      case PB_SPAWNDIRCHAOS:         return localized ? GetString(IDS_AP_SPAWNDIRCHAOS)       : _T("Spawn Direction Chaos");
      case PB_SPAWNSPEEDCHAOS:       return localized ? GetString(IDS_AP_SPAWNSPEEDCHAOS)     : _T("Spawn Speed Chaos");
      case PB_SPAWNSCALECHAOS:       return localized ? GetString(IDS_AP_SPAWNSCALECHAOS)     : _T("Spawn Scale Chaos");
      case PB_BLSPAWNDIEAFTER:       return localized ? GetString(IDS_AP_SPAWNDIEAFTER)       : _T("Die  X frames after collision");
      case PB_BLSPAWNDIEAFTERVAR:    return localized ? GetString(IDS_AP_SPAWNDIEAFTERVAR)    : _T("Die  X frames after collision variation");
      case PB_BLIPCOLLIDE_ON:        return localized ? GetString(IDS_AP_IPCOLLIDE_ON)        : _T("Interparticle Collisions On");
      case PB_BLIPCOLLIDE_STEPS:     return localized ? GetString(IDS_AP_IPCOLLIDE_STEPS)     : _T("Interparticle Collision Steps");
      case PB_BLIPCOLLIDE_BOUNCE:    return localized ? GetString(IDS_AP_IPCOLLIDE_BOUNCE)    : _T("Interparticle Collision Bounce");
      case PB_BLIPCOLLIDE_BOUNCEVAR: return localized ? GetString(IDS_AP_IPCOLLIDE_BOUNCEVAR) : _T("Interparticle Collision Bounce Variation");
      default:                       return _T("");
      }
   }  
   ...
   switch (message) {      
      ...
      case REFMSG_CHANGE:
         {int pblst=0;
           ...
           cancelled=FALSE;
		   // Edit the partID for our dependents, they need to know we are invalidating all our parts
		   partID |= PART_ALL;
         }
         break;
/*    case REFMSG_SUBANIM_STRUCTURE_CHANGED:
         ...
         if (editOb==this) InvalidateUI();
         break;*/
      default: SimpleParticle::NotifyRefChanged (changeInt, hTarget, partID, message, propagate);;
      }
   return REF_SUCCEED;
   }
...
public:
	...
	void proc(ILoad *iload) {
		...
		DWORD oldVer = ((CommonParticle*)(cb->GetTarget()))->pblock->GetVersion();
...
const TCHAR* BlizzardClassDesc::NonLocalizedClassName()
{
	return _T("Blizzard");
}

const TCHAR *BlizzardClassDesc::Category ()  {return GetString(IDS_RB_PARTICLESYSTEMS);}
const TCHAR *BlizzardParticle::GetObjectName(bool localized) const {return localized ? GetString(IDS_AP_BLIZZARDGC) : _T("Blizzard");}
...
void CommonParticle::AddToList(INode *newnode,int i,BOOL add)
{
	...
	{ 
		INode *null_ptr = NULL;
		nlist.Insert(i,1,&null_ptr);
	}    
	ReplaceReference(i+BASER,newnode);
...
{  
   ...
   else if (!MaxSDK::Graphics::IsHardwareHitTesting(vpt))
   ...
   // This part was not ported to retained mode, so it should still be called.
   if (EmitterVisible()) {
...
{  
   ...
   if (doupdate && !IsRetainedModeEnabled()) Update(t,inode);
   GraphicsWindow *gw = vpt->getGW();
   DWORD rlim  = gw->getRndLimits();
   ...
   if (EmitterVisible()) {
      gw->setTransform(inode->GetObjTMBeforeWSM(t));  
      MeshRenderHelper(mesh, gw, &particleMtl, flags, vpt, COMP_ALL);
      }
   ...
   { 
		...
		// skip the legacy mesh rendering if graphics system is enabled.
		if(dispmesh) // dispmesh was NULL in bug 257748  DB 11/00
			MeshRenderHelper(*dispmesh, gw, inode->Mtls(), flags, vpt, COMP_ALL, inode->NumMtls());
   }
   return(0);
}

// from IDisplay
unsigned long CommonParticle::GetObjectDisplayRequirement() const
{
	return MaxSDK::Graphics::ObjectDisplayRequireLegacyDisplayMode;
}

bool CommonParticle::UpdatePerNodeItems(
						const MaxSDK::Graphics::UpdateDisplayContext& updateDisplayContext,
						MaxSDK::Graphics::UpdateNodeContext& nodeContext,
						MaxSDK::Graphics::IRenderItemContainer& targetRenderItemContainer)
{
	...
	BOOL doupdate=((!cancelled)&&((t!=tvalid)||!valid));
	INode* renderNode = nodeContext.GetRenderNode().GetMaxNode();
	NullView nullView;
	BOOL needdel;

	if (!doupdate) 
	{
		doupdate=CheckMtlChange(renderNode->GetMtl(),wasmulti);
	}
	if (doupdate)
	{
		Update(t,renderNode);
	...
	int ptype,disptype,type=GetDrawType(this,ptype,disptype);
	SimpleParticle::UpdatePerNodeItems(updateDisplayContext, 
		nodeContext, 
	...
	if ((t!=dispt)||doupdate||!dispmesh)
	{
		...
		SetAFlag(A_NOTREND);
		dispmesh=GetRenderMesh(t,renderNode,nullView,needdel);  
		ClearAFlag(A_NOTREND);
	}
	...
	if(dispmesh)
	{
		...
		IMeshDisplay2* pMeshDisplay2 = static_cast<IMeshDisplay2*>(
			dispmesh->GetInterface(IMesh_DISPLAY2_INTERFACE_ID));
		...
		pMeshDisplay2->GetRenderItems(generateRenderItemsContext,nodeContext,renderItems);
