...
{
public:
	void Init(const Matrix3& start, const Matrix3& end);
...
void TMInterpolater::Init(const Matrix3& start, const Matrix3 &end)
{
	startTM = start;
	endTM = end;
...
class CollisionPlaneClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("PlanarCollision"); }
...
void CollisionPlane::DeleteThis() 
{ 
	delete this; 
}

RefResult CollisionPlane::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message) 
	{
			case REFMSG_CHANGE:
...
class CollisionSphereClassDesc:public ClassDesc2 {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("SphericalCollision"); }
	SClass_ID		SuperClassID() {return REF_MAKER_CLASS_ID; }
...
// de-inlined from icollision.h:
void CollisionSphere::DeleteThis() 
{ 
	delete this;
}

RefResult CollisionSphere::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message) 
	{
		case REFMSG_CHANGE:
...
static float VarK2( float radiusSq, const Point3 &vel)
...
//--------------------------------------------------------------//
	...
	float timeMd = 0.0f, distSqMd = 0.0f, timeLocal;
	...
	if (!wasAbove)
	{
		...
		particleIsFast = (localSpeedLen > radius);
		if (particleIsFast == TRUE)
