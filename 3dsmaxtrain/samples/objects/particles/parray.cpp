...
class CreatePAPartDelNode : public RestoreObj {
	public:   		
		...
		void Redo() 
			{  if (dist) obj->distname==_T("");else obj->custname=_T("");
...
class CreatePAPartPickNode : public RestoreObj {
	public:   		
		...
		void Restore(int isUndo) {
			...
			else  obj->distname=_T("");}
			else
			{ if (obj->custnode) obj->custname = name;
			else  obj->custname=_T("");}
			obj->ShowName(dist);
			}
		void Redo() 
		{ 
...
static ParamUIDesc descParamPArray[] = {
	...
	// Use Subobjects
	ParamUIDesc(PB_USESELECTED,TYPE_SINGLECHECKBOX,IDC_SP_USESUBOBJS),
...
static ParamUIDesc descParamPType[] = {
	...
	// Display type
	ParamUIDesc(PB_PANOTDRAFT,TYPE_SINGLECHECKBOX,IDC_SP_DRAFTMODE),
...
static ParamUIDesc descParamPSpin[] = {
	...
	// IPC Enable
	ParamUIDesc(PB_PAIPCOLLIDE_ON,TYPE_SINGLECHECKBOX,IDC_INTERP_BOUNCEON),
...
class CreatePArrayProc : public MouseCallBack,ReferenceMaker {
	private:
		...
		virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("CreatePArrayProc"); } // from Animatable
...
{ if ( PArrayObject ) 
	...
	PArrayObject->EndEditParams( (IObjParam*)createInterface, 
		...
		{	// RB 4-9-96: Normally the hold isn't holding when this 
			...
			theHold.Suspend(); 
			PArrayObject->DeleteAllRefsFromMe();
			PArrayObject->DeleteAllRefsToMe();
			PArrayObject->DeleteThis();
			PArrayObject = NULL;
		...
		case MOUSE_POINT:
			switch ( point ) {
              ...
              createInterface->GetMacroRecorder()->EmitScript();
					...
					// link it up
					l_CloudNode = createInterface->CreateObjectNode( PArrayObject);
					attachedToNode = TRUE;
					assert( l_CloudNode );					
					createCB = NULL;
...
{ int tabmax=256;
  ...
  { thePArrayDraw.bboxpt=new boxlst[count];
    ...
    { TimeValue tofs=(ocount>0?times.tl[tcount].tl:t);
	  ...
	  { if ((tmpnode=nlist[cgen]) == NULL) tmpnode=onode;
	  } else tmpnode=onode;
...
{	int tnums = 0, numV = 0, numF = 0, tvnum = 0, ismapped = 0;
	...
	{ TimeValue tofs=(tnums>1?times.tl[i].tl:t);
	  ...
	  BOOL badnode=TRUE;
	  if ((cgen>-1)&&(cgen<nlist.Count())) {
            badnode=((tmpnode=nlist[cgen]) == NULL);
          }
	  if (badnode)
...
{ 
	if (dist)
	{
		if (hparam) 
		{
			TSTR name = TSTR(GetString(IDS_AP_OBJECTSTR)) + (distnode ? distname : TSTR(GetString(IDS_AP_NONE)));
			FormatName(name.dataForWrite());
			SetWindowText(GetDlgItem(hparam, IDC_SP_CHUNKPICKOBJECT), name);
		}
	}
	else
	{
	...
	int subtree,flags;
	 if (dodist==1)
	  { nodes.SetCount(1);nodes[0]=node;
		...
		po->ReplaceReference(DISTNODE,node);
...
INT_PTR ParticleDlgProc::DlgProc(
...
INT_PTR ParticleGenDlgProc::DlgProc(
		TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
	{  int acourse;
       float size;
      switch (msg) 
...
INT_PTR PAParticleSpinDlgProc::DlgProc(
		TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{	switch (msg) {
		case WM_INITDIALOG: 
		{break;
...
INT_PTR ParticleDisableDlgProc::DlgProc(
		TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
	{int FToTick=(int)((float)TIME_TICKSPERSEC/(float)GetFrameRate());
      BOOL dofrag=FALSE;
	switch (msg) {
...
TSTR PArrayParticle::GetFilename()
{
    // Parray.CST is moved to plugcfg_ln
    ...
    filename.append(GetString(IDS_AP_PARRAYCST));
...
{ TSTR filename(GetFilename());
  ...
  if (custsettings==1) 
  {fclose(f);_tremove(filename);custsettings=0;delete[] NameLst;NameLst=NULL;
...
{ TCHAR newname[NLEN];
  int i = 0, save = 0;
  PArrayParticle *po = DLGetWindowLongPtr<PArrayParticle*>(hWnd);
...
INT_PTR PAParticleSpawnDlgProc::DlgProc(
		TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
...
PArrayParticle::PArrayParticle()
	...
	ReplaceReference(0, CreateParameterBlock(spdescVer6, PBLOCK_LENGTH_PARRAY, CURRENT_VERSION));
...
public:
	...
	void proc(ILoad *iload) {
		...
		DWORD oldVer = ((PArrayParticle*)(cb->GetTarget()))->pblock->GetVersion();
...
void PArrayParticle::BeginEditParams(
	...
	if (pmapParam) pmapParam->SetUserDlgProc(new ParticleDlgProc(this));
...
void PArrayParticle::EndEditParams( IObjParam *ip, ULONG flags,Animatable *next)
{	
...
void FillInEdges(TEdge &gl,OTEdge &el,DWORD tl,int *nxtlst)
{
	...
	while (e < el.Count())
	{
		...
		{ 
			f = (f ? 0 : 1);
			DWORD index = el[e].f[f];
			if (index != 0xffffffff)
			{
				int num = EdgeFound(gl,index);
				if (num > -1)
				{
					gl[num].sides++;
				}
				else if (nxtlst[index] == -2)
				{
...
int FillInGroups(int faces,int basegroup,int *toplist,int* nextlist,Face *f,Point3 *v,OTEdge& el)
{
	...
	while ((fullcount < Groups)&&(count < faces))
	{
		...
		while ((i < Groups) && (count < faces))
		{
			if (!full[i])
			{
				topptr[0]=f[toplist[i]].v[0];
				topptr[1]=f[toplist[i]].v[1];
				...
				{
					lastnum=toplist[i];
					while (nextlist[lastnum]>-1)
					{
						lastnum = nextlist[lastnum];
					}
					int minj=ThreeFace;
					nextlist[minj] = -1;
					nextlist[lastnum] = minj;
					count++;
...
{ int i,edges,numedges=3*infaces,*lst;
  ...
  edges=0;
  arealst=new float[numedges];
  assert(arealst!=NULL);
  edgelst=new fEdge[numedges];
  assert(edgelst!=NULL);
...
{ int i,j,face,ncounter,firstface,*lst,localv;
  ...
  int count=0;
  const BitArray& vertSel = mesh->VertSel();
  for (i=0;i<inverts;i++)
   if (doall ||vertSel[i])
   { if (lst[count]>0)
...
void GetGroupMeshPtr(INode *node,Mesh **mesh,TimeValue t,Matrix3 curtm,Interval &tmpValid,int subtree=0)
{
	...
	if ((nc=node->NumberOfChildren())>0)
	{
		...
		{
			INode *nxtnode=node->GetChildNode(j);
			if (nxtnode->IsGroupHead())
				GetGroupMeshPtr(nxtnode,mesh,t,curtm,tmpValid,subtree);
			else if ((subtree)||(nxtnode->IsGroupMember()))
				AddToMesh(nxtnode,mesh,t,curtm,tmpValid,subtree);
		}
	}
}
...
void PArrayParticle::GetInfoFromObject(float thick,int *c,INode *distnode,TimeValue t,TimeValue lastt)
{
	...
	if (distnode->IsGroupHead())
	{
		...
		if (oldmesh) 
		{
			...
			for (int j=0;j<oldmesh->getNumVerts();j++) 
				oldmesh->verts[j] = oldmesh->verts[j]*itm;
		}
	}
	else
	{
		if ((oldtriOb=TriIsUseable(oldpobj=distnode->EvalWorldState(t).obj,t))==NULL)
			return;
	...
	{ 
		...
		{ 
			...
			for (int j=0;j<curmesh->getNumVerts();j++) 
				curmesh->verts[j]=curmesh->verts[j]*itm;
		}
	}
	else
	{ 
	...
	{
		int* toplist  = new int[oldf];
		int* nextlist = new int[oldf];
		DbgAssert(toplist != NULL);
		DbgAssert(nextlist != NULL);
		pblock->GetValue(PB_SMOOTHANG,0,NAngle,FOREVER); 

		Point3* norm = new Point3[oldf];
		DbgAssert(norm != NULL);
...
//	srand(emitrnd);
	...
	{	if (! (((type==VERTS)&&(triOb->GetMesh().VertSel().NumberSet()>0))||((type==EDGES)&&(triOb->GetMesh().EdgeSel().NumberSet()>0))  )   )
		...
		BitArray& vertSel = triOb->GetMesh().VertSel();
		const BitArray& faceSel = triOb->GetMesh().FaceSel();
		if (type==VERTS)
			for (int i=0;i<oldf;i++)
			{	if (faceSel[i])
				for (int j=0;j<3;j++) vertSel.Set(triOb->GetMesh().faces[i].v[j]);
			}
		}
	}
...
BOOL PArrayParticle::ComputeParticleStart(TimeValue t0,TimeValue lastt,INode *node,int c)
	...
	{   parts.FreeAll();if (sdata) delete sdata;sdata=NULL;	maincount=0;
	    ...
	    TSTR name;name=(cnode ? cnode->GetName() : GetString(IDS_RB_PARRAY));
...
void PArrayParticle::UpdateParticles(TimeValue t,INode *node)
{
	...
	int i, j, c(0), birth,counter,tpf=GetTicksPerFrame(),anioff;
	float FperT;
	VelDir ptvel;
	VelDir2 ptvel2;
    Matrix3 tm,lasttm;
...
//	TimeValue fstep=oneframe;t2+=fstep;
	...
	int count(0), sper(0), spmult(0);
	float smper(0.0f); BOOL first=(tvalid<t0);
...
TSTR PArrayParticle::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_DISTRIBUTION:				return localized ? GetString(IDS_RB_DISTRIBUTION)        : _T("Emitter Distribution");
		case PB_EMITTERCOUNT:				return localized ? GetString(IDS_RB_EMITTERCOUNT)        : _T("Number of Emitters");
		...
		case PB_ANGLEDIV:						return localized ? GetString(IDS_RB_ANGLEDIV)            : _T("Divergence Angle");
		...
		case PB_FRAGTHICKNESS:				return localized ? GetString(IDS_RB_FRAGTHICKNESS)       : _T("Fragment Thickness");
		case PB_FRAGCOUNT:					return localized ? GetString(IDS_RB_FRAGCOUNT)           : _T("Fragment Count");
		...
		case PB_SPAWNGENS:					return localized ? GetString(IDS_AP_SPAWNGENS)           : _T("Spawn Generations");
		case PB_SPAWNCOUNT:					return localized ? GetString(IDS_AP_SPAWNCOUNT)          : _T("Spawn Multiplier");
		...
		case PB_SPAWNLIFEVLUE:				return localized ? GetString(IDS_AP_SPAWNLIFEVLUE)       : _T("Spawn Lifespan");
		...
		case PB_USESELECTED:					return localized ? GetString(IDS_AP_USESELECTED)         : _T("Use Selected Subobjects");
		case PB_PASPAWNDIEAFTER:			return localized ? GetString(IDS_AP_SPAWNDIEAFTER)       : _T("Die  X frames after collision");
		case PB_PASPAWNDIEAFTERVAR:		return localized ? GetString(IDS_AP_SPAWNDIEAFTERVAR)    : _T("Die  X frames after collision variation");
		case PB_PAIPCOLLIDE_ON:				return localized ? GetString(IDS_AP_IPCOLLIDE_ON)        : _T("Interparticle Collisions On");
		case PB_PAIPCOLLIDE_STEPS:			return localized ? GetString(IDS_AP_IPCOLLIDE_STEPS)     : _T("Interparticle Collision Steps");
		case PB_PAIPCOLLIDE_BOUNCE:		return localized ? GetString(IDS_AP_IPCOLLIDE_BOUNCE)    : _T("Interparticle Collision Bounce");
		case PB_PAIPCOLLIDE_BOUNCEVAR:	return localized ? GetString(IDS_AP_IPCOLLIDE_BOUNCEVAR) : _T("Interparticle Collision Bounce Variation");
		default: 								return _T("");
		}
	}
...
//		type=1;
  ...
  { aniend=GetAnimEnd();
	...
	po->pblock->GetValue(PB_SPINAXISTYPE,0,axisentered,FOREVER);
        po->thePArrayDraw.indir.inaxis = (axisentered == 1);
	if (po->thePArrayDraw.indir.inaxis == TRUE)
...
//		type=RENDTYPE1;
	...
	if ((indir.inaxis=(axisentered==1)) == TRUE)
	{ 
...
const TCHAR* PArrayClassDesc::NonLocalizedClassName()
{
	return _T("PArray");
}

const TCHAR *PArrayClassDesc::Category ()	{return GetString(IDS_RB_PARTICLESYSTEMS);}
const TCHAR *PArrayParticle::GetObjectName(bool localized) const {return localized ? GetString(IDS_RB_PARRAYGC) : _T("PArray");}
...
{   
	...
	else if(!MaxSDK::Graphics::IsRetainedModeEnabled())
	{
	 //For Nitrous,there is no need to draw mesh by using legacy method after port logic to IObjectDisplay2
	  ...
	  gw->setRndLimits(rlim);
	  gw->setTransform(mat);
	  MeshRenderHelper(*dispmesh, gw, inode->Mtls(), flags, vpt, COMP_ALL | ((flags&DISP_SHOWSUBOBJECT)?COMP_OBJSELECTED:0), inode->NumMtls());
	  gw->setRndLimits(rlim);
	}
	return(0);
}

bool PArrayParticle::UpdatePerNodeItems(const MaxSDK::Graphics::UpdateDisplayContext& updateDisplayContext,
										MaxSDK::Graphics::UpdateNodeContext& nodeContext,
										MaxSDK::Graphics::IRenderItemContainer& targetRenderItemContainer)
{
	...
	if (!OKtoDisplay(t)) return false;

	INode* inode = nodeContext.GetRenderNode().GetMaxNode();
	BOOL doupdate=((!cancelled)&&((t!=tvalid)||!valid));
	if (!doupdate) doupdate=CheckMtlChange(inode->GetMtl(),wasmulti);
	if (doupdate)
		Update(t,inode);
	...
	if ((type<2)||(type==3)) // Draw particles
	{
		//there is no need for display mesh's rendering
		return false;
	}
	...
	if(dispmesh && dispmesh->getNumVerts()>0)
	{
		using namespace MaxSDK::Graphics;

		mRenderItemHandles.ClearAllRenderItems();
		IMeshDisplay2* pMeshDisplay = static_cast<IMeshDisplay2*>(dispmesh->GetInterface(IMesh_DISPLAY2_INTERFACE_ID));
