...
class GSphereObject : public SimpleObject2, public RealWorldMapSizeInterface 
{
	friend class GSphereTypeInDlgProc;
	friend class GSphereObjCreateCallBack;
	public:	
		// Class vars
		static IObjParam *ip;
...
//      wired in.
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_GEOSPHERE) : _T("GeoSphere"); }
		BOOL HasUVW();
...
//      is available in the descriptors. REFMSG_GET_PARAM_NAME_LOCALIZED, etc. become unnecessary as well
...
class GSphereClassDesc: public ClassDesc2 
{
	...
	void*			Create(BOOL loading = FALSE)
    {
        ...
        return new GSphereObject(loading);
    }
	const TCHAR*	ClassName() { return GetString(IDS_GEOSPHERE_CLASS); }
	const TCHAR*	NonLocalizedClassName() { return _T("GeoSphere"); }
...
// geo_param param IDs
enum { 
	geo_radius = GEOSPHERE_RADIUS, 
	geo_segs = GEOSPHERE_SEGS,
	geo_type = GEOSPHERE_TYPE,
	geo_hemi = GEOSPHERE_HEMISHPERE,
	geo_smooth = GEOSPHERE_SMOOTH,
	geo_basepivot = GEOSPHERE_BASEPIVOT,
	geo_mapping = GEOSPHERE_GENUVS,
};

static_assert(geo_params == GEOSPHERE_PARAMBLOCK_ID, "");
...
namespace
{
	class MapCoords_Accessor : public PBAccessor
	{
		...
		{
			if (owner) ((GSphereObject*)owner)->UpdateUI();
		}
	};
...
//
//		line 1: block ID, internal name, local (subanim) name, flags AUTO_UI here means 
//				the rollout will be automatically created (see BeginEditParams for details)
...
static ParamBlockDesc2 geo_param_blk ( geo_params, _T("GeosphereParameters"),  0, &gsphereDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF_NO, 
	...
	geo_hemi, 	_T("hemisphere"),		TYPE_BOOL, 		P_ANIMATABLE,				IDS_HEMI,
		p_default, 		FALSE, 
		p_ui, 			TYPE_SINGLECHECKBOX, 	IDC_HEMI, 
		p_end, 
	...
	geo_basepivot, 	_T("baseToPivot"), 	TYPE_BOOL, 		0,				IDS_BASEPIVOT, 
		p_default, 		FALSE, 
		p_ui, 			TYPE_SINGLECHECKBOX, 	IDC_BASEPIVOT, 
		p_end, 
	...
	geo_mapping, 	_T("mapCoords"), 	TYPE_BOOL, 		0,				IDS_MAPPING,
		p_default, 		TRUE, 
		p_ui, 			TYPE_SINGLECHECKBOX, IDC_MAPPING, 
...
INT_PTR GSphereTypeInDlgProc::DlgProc(
...
//      on the descriptor
	...
	if (so->TestAFlag(A_OBJ_CREATING)) {
		...
		GSphereObject::typeinCreate = TRUE;
	}
...
class GSphereParamDlgProc : public ParamMap2UserDlgProc {
	public:
		GSphereObject *mpGSphereObj;
        HWND mhWnd;
		GSphereParamDlgProc(GSphereObject *o) {mpGSphereObj=o;}
	   INT_PTR DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
		void DeleteThis() {delete this;}
        void Update();
        BOOL GetRWSState();
...
void GSphereParamDlgProc::Update()
{
    BOOL usePhysUVs = mpGSphereObj->GetUsePhysicalScaleUVs();
    CheckDlgButton(mhWnd, IDC_REAL_WORLD_MAP_SIZE, usePhysUVs);
    EnableWindow(GetDlgItem(mhWnd, IDC_REAL_WORLD_MAP_SIZE), mpGSphereObj->HasUVW());
}

BOOL GSphereParamDlgProc::GetRWSState()
{
    BOOL check = IsDlgButtonChecked(mhWnd, IDC_REAL_WORLD_MAP_SIZE);
    return check;
}

INT_PTR GSphereParamDlgProc::DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg) {
    case WM_INITDIALOG: {
        mhWnd = hWnd;
    ...
    case WM_COMMAND:
        ...
        case IDC_REAL_WORLD_MAP_SIZE: {
            ...
            theHold.Begin();
            mpGSphereObj->SetUsePhysicalScaleUVs(check);
            theHold.Accept(GetString(IDS_DS_PARAMCHG));
            mpGSphereObj->ip->RedrawViews(mpGSphereObj->ip->GetTime());
            break;
        }
...
void GSphereObject::BeginEditParams(IObjParam *ip, ULONG flags, Animatable *prev)
{
	...
	geo_param_blk.SetUserDlgProc(new GSphereParamDlgProc(this));
}
...
void GSphereObject::BuildMesh(TimeValue t)
	...
	Matrix3 id;
        if (usePhysUVs) {
            float uvScale = ((float) 2.0f * PI * radius);
            mesh.ApplyUVWMap (MAP_SPHERICAL, uvScale, uvScale/2, 1.0f, 0, 0, 0, 0, id);
        } else
            mesh.ApplyUVWMap (MAP_SPHERICAL, 1.0f, 1.0f, 1.0f, 0, 0, 0, 0, id);
    }
...
int GSphereObject::IntersectRay(
	...
	//if (!smooth) {
		return SimpleObject2::IntersectRay(t, ray, at, norm);
		}	
...
{
	GSphereObject* newob = new GSphereObject(FALSE);	
...
BuildNURBSSphere(float radius, BOOL hemi, BOOL recenter, BOOL genUVs)
{
	...
	TCHAR bname[80];
	TCHAR sname[80];
	_tcscpy(bname, GetString(IDS_GEOSPHERE));
...
void GSphereObject::UpdateUI()
{
	if (ip == NULL)
		return;
	GSphereParamDlgProc* dlg = static_cast<GSphereParamDlgProc*>(geo_param_blk.GetUserDlgProc());
	dlg->Update();
}

BOOL GSphereObject::GetUsePhysicalScaleUVs()
{
    return ::GetUsePhysicalScaleUVs(this);
}

void GSphereObject::SetUsePhysicalScaleUVs(BOOL flag)
{
    BOOL curState = GetUsePhysicalScaleUVs();
    if (curState == flag)
        return;
    if (theHold.Holding())
        theHold.Put(new RealWorldScaleRecord<GSphereObject>(this, curState));
