...
class CircleObject: public SimpleSpline, public IParamArray {
	...
	public:
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_TH_CIRCLE) : _T("Circle"); }
		void InitNodeName(TSTR& s) { s = GetString(IDS_TH_CIRCLE); }
		Class_ID ClassID() { return Class_ID(CIRCLE_CLASS_ID,0); }
		void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_TH_CIRCLE_CLASS) : _T("Circle"); }
...
class CircleObjClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Circle"); }
...
TSTR CircleObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
