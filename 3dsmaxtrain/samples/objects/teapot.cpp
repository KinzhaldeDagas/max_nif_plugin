...
public:			
	...
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_TEAPOT) : _T("Teapot"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Teapot"); }
...
static_assert(teapot_params == TEAPOT_PARAMBLOCK_ID, "");
...
public:
	...
	void proc(ILoad *iload)
	{
		...
		mTeapot.pblock2->GetValue(teapot_teapart, 0, oldpart);
		switch (oldpart) {
		case OLD_POT_BOTH:
...
{
	float radius;
	pblock2->GetValue(teapot_radius,t,radius);
...
{
	int smooth;
	pblock2->GetValue(teapot_smooth,t,smooth);
	if (!smooth) {
