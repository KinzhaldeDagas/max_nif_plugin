...
class QuadPatchObject : public GeomObject, public IParamArray {			   
	...
	public:
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_TH_QUADPATCH) : _T("QuadPatch"); }
		...
		void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_TH_QUADPATCHOBJECT_CLASS) : _T("QuadPatchObject"); }

		int NumSubs() { return 1; }  
		Animatable* SubAnim(int i) { return pblock; }
		TSTR SubAnimName(int i, bool localized) override { return localized ? GetString(IDS_TH_PARAMETERS) : _T("Parameters");}
...
class QuadPatchClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Quad Patch"); }
...
void QuadPatchObject::EndEditParams( IObjParam *ip, ULONG flags, Animatable *next )
{
	...
	pblock->GetValue(PB_LSEGS,ip->GetTime(),dlgLSegs);
	pblock->GetValue(PB_WSEGS,ip->GetTime(),dlgWSegs);	
	pblock->GetValue(PB_TEXTURE,ip->GetTime(),dlgTexture);	
}
...
int QuadPatchObject::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags) {
	...
	if(!MaxSDK::Graphics::IsRetainedModeEnabled())
	{
		if(!(gw->getRndMode() & GW_BOX_MODE))
		{
			...
			if(mesh.getNumVerts())
			{
				MeshRenderHelper(mesh, gw, inode->Mtls(), flags, vpt, COMP_ALL | (inode->Selected() ? COMP_OBJSELECTED : 0), inode->NumMtls());
			}
		}
	}
	MeshRenderHelper(patch, gw, inode->Mtls(), flags, vpt, COMP_ALL | (inode->Selected() ? COMP_OBJSELECTED : 0), inode->NumMtls());
	return(0);
}
...
TSTR QuadPatchObject::GetParameterName(int pbIndex, bool localized) 
	{
	switch (pbIndex) {
		...
		case PB_WSEGS:  return localized ? GetString(IDS_RB_WSEGS)  : _T("Width Segments");
		case PB_LSEGS:  return localized ? GetString(IDS_RB_LSEGS)  : _T("Length Segments");
		default:        return _T("");
		}
	}
