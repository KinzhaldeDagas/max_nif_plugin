...
class BoxObject : public GenBoxObject, public RealWorldMapSizeInterface {
	friend class BoxObjCreateCallBack;
	friend class BoxTypeInDlgProc;
private:
...
public:
	...
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_BOX) : _T("Box"); }
	...
	void BuildUVs(int genUVs, float l, float w, float h, int lsegs, int wsegs, int hsegs, int nfaces, Point3 va, Mesh &mesh);
};

// class variables for box class.
IObjParam *BoxObject::ip = nullptr;
...
public:
	...
	const TCHAR*	ClassName() override { return GetString(IDS_RB_BOX_CLASS); }
	const TCHAR*	NonLocalizedClassName() { return _T("Box"); }
	...
	const TCHAR*	InternalName() override { return _T("Box"); }	// returns fixed parsable name (scripter-visible name)
...
static_assert(box_params == BOXOBJ_PARAMBLOCK_ID, "");
...
static ParamBlockDesc2 box_param_blk(box_params, _T("BoxParameters"), 0, &boxObjDesc, P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF_NO,
	...
	box_lsegs, _T("lengthsegs"), TYPE_INT, P_ANIMATABLE, IDS_RB_LSEGS,
		...
		p_nonLocalizedName, _T("Length Segments"),
		p_end,
...
BoxObject::BoxObject(BOOL loading) : mPolyBoxSmoothingGroupFix(true), mBoxVersion(currentVersion), mIsSavingOld(false)
{
...
const int kChunkBoxVersion = 0x0110;
...
IOResult BoxObject::Save(ISave *isave)
{
	...
	isave->BeginChunk(kChunkBoxVersion);
	if (mIsSavingOld)
	{
		mIsSavingOld = false;
		int oldVersion = 0;
		...
		isave->Write(&mBoxVersion, sizeof(int), &nb);
	isave->EndChunk();
...
IOResult BoxObject::Load(ILoad *iload)
{
	...
	while (IO_OK == (res = iload->OpenChunk())) {
		...
		case kChunkPolyFix:
			...
			break;
		case kChunkBoxVersion:
			iload->Read(&mBoxVersion, sizeof(int), &nb);
			break;
		}
		iload->CloseChunk();
...
void BoxObject::BuildMesh(TimeValue t)
{
	...
	BuildUVs(genUVs, l, w, h, lsegs, wsegs, hsegs, nfaces, va, mesh);
	mesh.InvalidateTopologyCache();
}

void BoxObject::BuildUVs(int genUVs, float l, float w, float h, int lsegs, int wsegs, int hsegs, int nfaces, Point3 va, Mesh &mesh)
{
	if (genUVs) {
		int ls = lsegs + 1;
		int ws = wsegs + 1;
		...
		// num UV faces by proj. (ie: separates front/back faces)
		int imax = mBoxVersion ? 2 : 1;

		// Set the correct UV coords for TVerts
		// X axis face
		for (int i = 0; i < imax; i++)
		{
			v = 0.0f;
			for (int iz = 0; iz < hs; iz++) {
				u = 0.0f;
				for (int iy = 0; iy < ls; iy++) {
		...
		// Every face (tri) of the mesh gets linked to the 3 TVerts it's formed by
		for (int nf = 0; nf < nfaces; nf++) {
			Face& f = mesh.faces[nf];
			DWORD* nv = f.getAllVerts();
			Point3 v[3];
			for (int i = 0; i < 3; i++)
				v[i] = mesh.getVert(nv[i]);
			int dir = direction(v);
			int ntv[3]; // New texture vertices that make up the face
			for (int i = 0; i < 3; i++) {
				int iu, iv;
				int base; // Positive directions take up the first half of the base
				switch (dir) {
				case POSX: case NEGX:
					iu = int(((float)lsegs*(v[i].y - va.y) / l) + .5f);
					iv = int(((float)hsegs*(v[i].z - va.z) / h) + .5f);
					if (dir == NEGX) iu = lsegs - iu;
					base = (mBoxVersion == 0 || dir == POSX) ? xbase : ybase / 2; // if mBoxVersion == 0 take the standard base
					ntv[i] = (base + iv*ls + iu);								  // else you need to consider 'dir' since you have twice as many vertices
					break;														  // since they are not shared anymore (front/back faces are split)
				case POSY: case NEGY:
					iu = int(((float)wsegs*(v[i].x - va.x) / w) + .5f);
					iv = int(((float)hsegs*(v[i].z - va.z) / h) + .5f);
					if (dir == POSY) iu = wsegs - iu;
					base = (mBoxVersion == 0 || dir == POSY) ? ybase : ybase + (zbase - ybase) / 2;
					ntv[i] = (base + iv*ws + iu);
					break;
				case POSZ: case NEGZ:
					iu = int(((float)wsegs*(v[i].x - va.x) / w) + .5f);
					iv = int(((float)lsegs*(v[i].y - va.y) / l) + .5f);
					if (dir == NEGZ) iu = wsegs - iu;
					base = (mBoxVersion == 0 || dir == POSZ) ? zbase : zbase + (ntverts - zbase) / 2;
					ntv[i] = (base + iv*ws + iu);
					break;
				}
			}
...
#define NF(s1, s2, s1r, s1c, s2r, s2c) \
	...
	Matrix3 mat;
	mat.IdentityMatrix();
	Object *obj = CreateNURBSObject(nullptr, &nset, mat);
	return obj;
}
