...
class NGonObject: public SimpleSpline, public IParamArray {
	...
	public:
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_TH_NGON) : _T("NGon"); }
		void InitNodeName(TSTR& s) { s = GetString(IDS_TH_NGON); }
		Class_ID ClassID() { return Class_ID(NGON_CLASS_ID,0); }
		void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_TH_NGON_CLASS) : _T("NGon"); }
...
class NGonObjClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("NGon"); }
...
static ParamUIDesc descParam[] = {
	...
	// Circular
	ParamUIDesc(PB_CIRCULAR,TYPE_SINGLECHECKBOX,IDC_OBCIRCULAR),
...
public:
	...
	virtual void proc(ILoad *iload) {
		...
		DWORD oldVer = ((NGonObject*)(cb->GetTarget()))->pblock->GetVersion();
...
INT_PTR NGonTypeInDlgProc::DlgProc(
...
void NGonObject::EndEditParams( IObjParam *ip, ULONG flags,Animatable *next )
{
	...
	pblock->GetValue(PB_CIRCULAR,ip->GetTime(),dlgCircular);	
	pblock->GetValue(PB_SCRIBE,ip->GetTime(),dlgScribe);	
}
...
TSTR NGonObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		...
		case PB_SIDES:    return localized ? GetString(IDS_TH_SIDES)    : _T("Sides");
		case PB_CIRCULAR: return localized ? GetString(IDS_TH_CIRCULAR) : _T("Circular");
		case PB_SCRIBE:   return localized ? GetString(IDS_TH_SCRIBE)   : _T("Scribing");
		default:          return _T("");
		}
	}
