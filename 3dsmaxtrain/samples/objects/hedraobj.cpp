...
class HedraObject : public SimpleObject {
	friend class HedraObjCreateCallBack;
	public:
		// Class vars
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_HEDRA) : _T("Hedra");}
		BOOL HasUVW();
...
class HedraClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Hedra"); }
		...
		case CC_SPINNER_CHANGE:
			switch (LOWORD(wParam)) {				
				case IDC_PARAM_PSPIN: {
					IParamArray *pa = map->GetParamBlock();
					float p, q;
					pa->GetValue(PB_P,t,p);
					pa->GetValue(PB_Q,t,q);
					if (p+q>1.0f) {
						q = 1.0f-p;
...
void HedraObject::EndEditParams( IObjParam *ip, ULONG flags,Animatable *next )
{
	...
	pblock->GetValue(PB_FAMILY,ip->GetTime(),dlgFamily);	
}
...
TSTR HedraObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_RADIUS:	return localized ? GetString(IDS_RB_RADIUS) : _T("Radius");
		case PB_FAMILY:	return localized ? GetString(IDS_RB_FAMILY) : _T("Family");
		case PB_P:			return localized ? GetString(IDS_RB_PVALUE) : _T("P Value");
		case PB_Q:			return localized ? GetString(IDS_RB_QVALUE) : _T("Q Value");
		case PB_SCALEP:	return localized ? GetString(IDS_RB_PSCALE) : _T("P Scale");
		case PB_SCALEQ:	return localized ? GetString(IDS_RB_QSCALE) : _T("Q Scale");
		case PB_SCALER:	return localized ? GetString(IDS_RB_RSCALE) : _T("R Scale");
		default:				return _T("Parameter");
		}
	}
