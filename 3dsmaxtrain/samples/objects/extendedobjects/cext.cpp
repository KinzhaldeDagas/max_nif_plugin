...
public:
	...
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_CEXT) : _T("C-Ext"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("C-Ext"); }
...
static_assert(cext_params == CEXT_PARAMBLOCK_ID, "");
...
void FixCExtTopBotWidth(IParamBlock2 *pblock2, TimeValue t, ParamID sidelenId, ParamID width1Id, ParamID width2Id, ParamID id, PB2Value &v)
{
	...
	pblock2->GetValue(sidelenId, t, sidelen);
	pblock2->GetValue(width1Id, t, width1);
	pblock2->GetValue(width2Id, t, width2);
...
void FixCExtSideWidth(IParamBlock2 *pblock2, TimeValue t, ParamID sidewidthId, ParamID toplenId, ParamID botlenId, ParamID id, PB2Value &v)
{
	...
	pblock2->GetValue(sidewidthId, t, sidewidth);
	pblock2->GetValue(toplenId, t, toplen);
	pblock2->GetValue(botlenId, t, botlen);
...
void InitCExtTopBotWidth(IParamBlock2 *pblock2, TimeValue t, ParamID sidelenId, ParamID topwidthId, ParamID botwidthId)
{
	...
	pblock2->GetValue(sidelenId, t, sidelen);
	pblock2->GetValue(topwidthId, t, topwidth);
	pblock2->GetValue(botwidthId, t, botwidth);
	// Sum of top and bot width should be lower than side length
...
void CExtObject::BuildMesh(TimeValue t)
{
	...
	int createmeth = pblock2->GetInt(cext_centercreate, 0);
...
{
	float radius;
	pblock2->GetValue(cext_botlength, t, radius);
