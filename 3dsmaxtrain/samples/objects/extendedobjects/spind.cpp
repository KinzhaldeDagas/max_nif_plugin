...
class SpindleObject : public SimpleObject, public IParamArray, public RealWorldMapSizeInterface {
	friend class SpindleTypeInDlgProc;
	friend class SpindleCapValsDlgProc;
	friend class SpindleObjCreateCallBack;
public:
	// Class vars
	...
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_SPINDLE) : _T("Spindle"); }
	BOOL HasUVW();
...
public:
	...
	void*			Create(BOOL loading = FALSE)
	{
		...
		return new SpindleObject(loading);
	}
	const TCHAR*	ClassName() { return GetString(IDS_AP_SPINDLE_CLASS); }
	const TCHAR*	NonLocalizedClassName() { return _T("Spindle"); }
...
static ParamUIDesc descParam[] = {
	...
	// Gen Smoothing
	ParamUIDesc(PB_SMOOTHON,TYPE_SINGLECHECKBOX,IDC_SP_SMOOTH),			

	// Gen Slice
	ParamUIDesc(PB_SLICEON,TYPE_SINGLECHECKBOX,IDC_SP_SLICEON),			
...
INT_PTR SpindleTypeInDlgProc::DlgProc(
...
BOOL SpindleCapValsDlgProc::GetRWSState()
{
	BOOL check = IsDlgButtonChecked(mhWnd, IDC_REAL_WORLD_MAP_SIZE);
	return check;
}

void SpindleCapValsDlgProc::Update()
{
	if (ob == NULL) return;
...
INT_PTR SpindleCapValsDlgProc::DlgProc(
TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{ 	switch (msg) {
	case WM_INITDIALOG: {
			mhWnd = hWnd;
...
SpindleObject::SpindleObject(BOOL loading)
{
	ReplaceReference(0, CreateParameterBlock(SpindledescVer0, PBLOCK_LENGTH, CURRENT_VERSION));
...
void SpindleObject::BeginEditParams(IObjParam *ip,ULONG flags,Animatable *prev)
{
	...
	if (pmapCreate && pmapParam) {
		...
		SpindleCapValsDlgProc* dlg = static_cast<SpindleCapValsDlgProc*>(pmapParam->GetUserDlgProc());
		if (dlg != NULL) {
...
void SpindleObject::EndEditParams( IObjParam *ip, ULONG flags,Animatable *next )
{
	...
	pblock->GetValue(PB_CSEGS,ip->GetTime(),dlgCSegs);
...
float height, float capheight, float pie1, float pie2, int genUVs, BOOL usePhysUVs)
{
	...
	{ int tvcount=0,nexttv;
		...
		float hlevel;
		float heightScale = usePhysUVs ? height-2.0f*capheight - 2.0f*blend: 1.0f;
...
{	
	...
	BuildSpindleMesh(mesh, segs, csegs,smooth, hsegs, doPie,radius, blend, height, capheight,pie1, pie2, genUVs, GetUsePhysicalScaleUVs());
}
...
TSTR SpindleObject::GetParameterName(int pbIndex, bool localized)
{
	switch (pbIndex) {
	...
	case PB_CSEGS:     return localized ? GetString(IDS_RB_CAPSEGS)   : _T("Cap Segments");
...
{
	SpindleObject* newob = new SpindleObject(FALSE);	
...
BOOL SpindleObject::GetUsePhysicalScaleUVs()
{
	return ::GetUsePhysicalScaleUVs(this);
}


void SpindleObject::SetUsePhysicalScaleUVs(BOOL flag)
{
	BOOL curState = GetUsePhysicalScaleUVs();
	if (curState == flag)
	return;
	if (theHold.Holding())
	theHold.Put(new RealWorldScaleRecord<SpindleObject>(this, curState));
