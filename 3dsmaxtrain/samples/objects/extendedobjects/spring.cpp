...
class SpringObject : public DynamHelperObject {
	friend class SpringParamDlg;
	friend class SpringEmitterCreateCallback;
	public:
...
// PostLoadCallback
//------------------------------------------------------------------------------
class SpringObject::SpringPLC : public PostLoadCallback
{
	public:
		SpringPLC(SpringObject& aSpring) : mSpring(aSpring) { }
		void proc(ILoad *iload) 
		{
			mSpring.SetThisNode(mSpring.thisnode);
			mSpring.thisnode = NULL;
			delete this;
		}

	private:
		SpringObject& mSpring;
};
...
static ParamUIDesc descSpringParam1[] = {
	...
	// IsRenderable?
	ParamUIDesc(PB_RENDERABLE,TYPE_SINGLECHECKBOX,IDC_SPRING_RENDERTHIS),			
	...
	ParamUIDesc(PB_MAPMEMAPME,TYPE_SINGLECHECKBOX,IDC_SPRING_MAPME),			
			
	// Nonlinear Response
	ParamUIDesc(PB_NONLINEAR,TYPE_SINGLECHECKBOX,IDC_SPRING_NL),			
...
class CreateMSpringProc : public MouseCallBack,ReferenceMaker {
	private:
		...
		virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("CreateMSpringProc"); }
...
class SpringCreateCPartRestoreNode : public RestoreObj {
	public:   		
		SpringObject *obj;
		TSTR name,namer;
		INode *save,*saver;
		SpringCreateCPartRestoreNode(SpringObject *o) {
...
class SpringCreateCPart2RestoreNode : public RestoreObj {
	public:   		
		SpringObject *obj;
		TSTR name,namer;
		INode *save,*saver;
		SpringCreateCPart2RestoreNode(SpringObject *o) {
...
{	
	...
	theHold.Begin();
	if (dodist)
	{ theHold.Put(new SpringCreateCPart2RestoreNode(po));
	po->ReplaceReference(SpringObject::kREF_CUSTNODE2, node);
	  po->custname2 = TSTR(node->GetName());
	  // Automatically check show result and do one update
	  po->ShowName2();	
	}
	else
	{ theHold.Put(new SpringCreateCPartRestoreNode(po));
		po->ReplaceReference(SpringObject::kREF_CUSTNODE, node);
...
{ float depth,fillet;
  if (type==RECT)
  { float width;
    po->pblock->GetValue(PB_RECTWIDTH,t,width);
    po->pblock->GetValue(PB_RECTDEPTH,t,depth);
	if (width<depth) depth=width;
  }
  else
  { po->pblock->GetValue(PB_DSECDEPTH,t,depth);
  }
  int spinnum=(type==RECT?PB_RECTFILLET:PB_DSECFILLET);
  po->pblock->GetValue(spinnum,t,fillet);
  depth*=0.5f;
  BOOL change = (fillet>depth);
  if (change==TRUE)
...
{ float mdiam,diam;
  po->pblock->GetValue(PB_OD,t,mdiam);
  int spinnum=(type==ROUND?PB_RNDDIA:(type==RECT?PB_RECTWIDTH:PB_DSECWIDTH));
  po->pblock->GetValue(spinnum,t,diam);
  BOOL change = (diam>=mdiam);
  if (change==TRUE)
  { 
	  SuspendSetKeyMode();
...
{ if (!po->editOb) return;
  po->ShowNames();
  int emethod;po->pblock->GetValue(PB_ENDSMETHOD,0,emethod);
  BoundParams(!emethod);
  int manualsegs;po->pblock->GetValue(PB_MANUALSEGSET,0,manualsegs);
  AutoSegs(manualsegs);
  int wire;po->pblock->GetValue(PB_WIRETYPE,0,wire);
  WireType(wire,t);
}

INT_PTR SpringParamDlg::DlgProc(
			  ...
			  { switch ( LOWORD(wParam) ) 
				{
				 case IDC_SPRING_ODSPIN: 
				  { int wtype;
					po->pblock->GetValue(PB_WIRETYPE,t,wtype);
...
{	
	...
	ReplaceReference(kREF_PBLK, CreateParameterBlock(descSpringVer0, length, CURRENT_VERSION));
	assert(pblock);
...
void SpringObject::BeginEditParams(IObjParam *ip, ULONG flags,Animatable *prev)
{
...
void SpringObject::EndEditParams(IObjParam *ip, ULONG flags,Animatable *next)
{	
	...
	{	Matrix3 mat1, mat2;
		...
		{	forcedir = Normalize(forcedir);
			...
			{	difflen = freelen - len;
				...
				int isnonlinear;
				pblock->GetValue(PB_NONLINEAR,0,isnonlinear);
				if (((isnonlinear)&&(springtype == COMPRESSION)&&(len < freelen))||
					((isnonlinear)&&(springtype == BIDIRECT)&&(len < freelen)))
				{	int wiretype;
					float wiresize, coils;
					pblock->GetValue(PB_WIRETYPE,0,wiretype);
					if (wiretype==ROUND)
						pblock->GetValue(PB_RNDDIA,0,wiresize);
...
{	if (TestAFlag(A_RENDER))
	...
	{	ivalid=FOREVER;
		...
		if (C < 0.5f) 
			C = 0.5f;
		pblock->GetValue(PB_SMOOTH,0,SMOOTH);
...
TSTR SpringObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_ENDSMETHOD:			return localized ? GetString(IDS_AP_ENDSMETHOD)    : _T("End Placement Method");
		case PB_NOREFLENGTH:			return localized ? GetString(IDS_AP_NOREFLENGTH)   : _T("Free Spring Height");
		case PB_OD:						return localized ? GetString(IDS_AP_OD)            : _T("Diameter");
		case PB_COILS:					return localized ? GetString(IDS_AP_COILS)         : _T("Number of Turns");
		case PB_COILDIR:				return localized ? GetString(IDS_AP_COILDIR)       : _T("Turn Direction");
		case PB_MANUALSEGSET:		return localized ? GetString(IDS_AP_SEGMENTMETHOD) : _T("Segmentation Method");
		case PB_SEGSPERTURN:			return localized ? GetString(IDS_AP_SEGSPERTURN)   : _T("Segments Per Turn");
		case PB_SEGMENTS:				return localized ? GetString(IDS_AP_SEGMENTS)      : _T("Segments Along Turn");
		case PB_SMOOTH:				return localized ? GetString(IDS_AP_SMOOTH)        : _T("Smooth Spring");
		case PB_RENDERABLE:			return localized ? GetString(IDS_AP_RENDER)        : _T("Renderable Spring");

		case PB_WIRETYPE:				return localized ? GetString(IDS_AP_WIRETYPE)      : _T("Wire ");
		case PB_RNDDIA:				return localized ? GetString(IDS_AP_RNDDIA)        : _T("Round Wire Diameter");
		case PB_RNDSIDES:				return localized ? GetString(IDS_AP_RNDSIDES)      : _T("Round Wire Side Count");

		case PB_RECTWIDTH:			return localized ? GetString(IDS_AP_RECTWIDTH)     : _T("Rectangular Wire Width");
		case PB_RECTDEPTH:			return localized ? GetString(IDS_AP_RECTDEPTH)     : _T("Rectangular Wire Depth");
		case PB_RECTFILLET:			return localized ? GetString(IDS_AP_RFILLET)       : _T("Rectangular Wire Fillet Size");
		case PB_RECTFILLETSIDES:	return localized ? GetString(IDS_AP_RFILLETSIDES)  : _T("Rectangular Fillet Sides");
		case PB_RECTROTANGLE:		return localized ? GetString(IDS_AP_RECTROT)       : _T("Rectangular Wire Rotation Angle");

		case PB_DSECWIDTH:			return localized ? GetString(IDS_AP_DSECWIDTH)     : _T("D-Section Wire Width");
		case PB_DSECDEPTH:			return localized ? GetString(IDS_AP_DSECDEPTH)     : _T("D-Section Wire Depth");
		case PB_DSECFILLET:			return localized ? GetString(IDS_AP_DFILLET)       : _T("D-Section Wire Fillet Size");
		case PB_DSECFILLETSIDES:	return localized ? GetString(IDS_AP_DFILLETSIDES)  : _T("D-Section Wire Fillet Sides");
		case PB_DSECRNDSIDES:		return localized ? GetString(IDS_AP_DSECRNDSIDES)  : _T("D-Section Round Sides");
		case PB_DSECROTANGLE:		return localized ? GetString(IDS_AP_DSECROT)       : _T("D-Section Wire Rotation Angle");

		case PB_FREELENGTH:			return localized ? GetString(IDS_AP_FREELENGTH)    : _T("Dynamics Spring Relaxed Height");
		case PB_KVALUE:				return localized ? GetString(IDS_AP_KVALUE)        : _T("Dynamics K Constant Value");
		case PB_KVALUNITS:			return localized ? GetString(IDS_AP_KVALUNITS)     : _T("Dynamics K Constant Units");
		case PB_SPRINGDIR:			return localized ? GetString(IDS_AP_SPRINGDIR)     : _T("Dynamics Spring Direction");
		case PB_MAPMEMAPME:			return localized ? GetString(IDS_AP_MAPME)         : _T("Generate Mapping Coordinates");
		case PB_NONLINEAR:			return localized ? GetString(IDS_AP_NL)            : _T("Enable Nonlinearity");
		default:							return _T("");
		}
	}
		...
		case REFMSG_TARGET_DELETED:	
			{ if (hTarget==custnode)
			  { if (theHold.Holding() && !TestAFlag(A_HELD)) 
					theHold.Put(new SpringCreateCPartRestoreNode(this));
			  ...
			  { if (theHold.Holding() && !TestAFlag(A_HELD)) 
					theHold.Put(new SpringCreateCPart2RestoreNode(this));
...
IOResult SpringObject::DoLoadLegacy(ILoad *iload)
{
	IOResult res = IO_OK;
	...
	iload->SetObsolete();
	iload->RegisterPostLoadCallback(new SpringPLC(*this));

	return IO_OK;
}

IOResult SpringObject::DoLoad(ILoad *iload)
{
	IOResult res = IO_OK;
...
INode* SpringObject::GetThisNode() const
{
	if (mThisNodeMonitor)
	{
...
void SpringObject::SetThisNode(INode* node)
{
	DbgAssert(mThisNodeMonitor != NULL);
...
const TCHAR* SpringClassDesc::NonLocalizedClassName() { return _T("Spring"); }
const TCHAR *SpringClassDesc::Category ()	{return GetString(IDS_EP_DYNPRIMS);}
const TCHAR *SpringObject::GetObjectName(bool localized) const {return localized ? GetString(IDS_AP_SPRING_OBJECT) : _T("Spring");}
