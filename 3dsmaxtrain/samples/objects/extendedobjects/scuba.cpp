...
class ScubaObject : public SimpleObject, public IParamArray, public RealWorldMapSizeInterface {
	friend class ScubaTypeInDlgProc;
	friend class ScubaObjCreateCallBack;
	friend class ScubaHeightDlgProc;
	public:
		// Class vars
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_SCUBA) : _T("Capsule"); }
		BOOL HasUVW();
		void SetGenUVW(BOOL sw);

		// Animatable methods
		void DeleteThis() { delete this; }
		Class_ID ClassID() { return CAPSULE_CLASS_ID; }
...
class ScubaClassDesc:public ClassDesc {
	...
	void*			Create(BOOL loading = FALSE)
    {
        ...
        return new ScubaObject(loading);
    }
	const TCHAR*	ClassName() { return GetString(IDS_AP_SCUBA_CLASS); }
	const TCHAR*	NonLocalizedClassName() { return _T("Capsule"); }
...
static ParamUIDesc descParam[] = {
	...
	// Gen Smoothing
	ParamUIDesc(PB_SMOOTHON,TYPE_SINGLECHECKBOX,IDC_SC_SMOOTH),			

	// Gen Slice
	ParamUIDesc(PB_SLICEON,TYPE_SINGLECHECKBOX,IDC_SC_SLICEON),			
...
{ float radius,height,r2,minh;
  BOOL con,minush;
	...
	pblock->GetValue(PB_HEIGHT,(increate?0:t),height);
	r2=2.0f*radius;
	minush=height<0.0f;
	minh=((minush)?-r2:r2);
...
INT_PTR ScubaTypeInDlgProc::DlgProc(
...
BOOL ScubaHeightDlgProc::GetRWSState()
{
    BOOL check = IsDlgButtonChecked(mhWnd, IDC_REAL_WORLD_MAP_SCALE);
    return check;
}

void ScubaHeightDlgProc::Update()
{
    if (ob == NULL) return;
    BOOL usePhysUVs = ob->GetUsePhysicalScaleUVs();
    CheckDlgButton(mhWnd, IDC_REAL_WORLD_MAP_SCALE, usePhysUVs);
    EnableWindow(GetDlgItem(mhWnd, IDC_REAL_WORLD_MAP_SCALE), ob->HasUVW());
}

INT_PTR ScubaHeightDlgProc::DlgProc(
		TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
	{ 	switch (msg) {
    case WM_INITDIALOG: {
        mhWnd = hWnd;
    ...
    case WM_COMMAND:
        ...
        case IDC_REAL_WORLD_MAP_SCALE: {
            BOOL check = IsDlgButtonChecked(hWnd, IDC_REAL_WORLD_MAP_SCALE);
            theHold.Begin();
...
TSTR ScubaObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
...
RefTargetHandle ScubaObject::Clone(RemapDir& remap) 
	{
	ScubaObject* newob = new ScubaObject(FALSE);	
...
BOOL ScubaObject::GetUsePhysicalScaleUVs()
{
    return ::GetUsePhysicalScaleUVs(this);
}


void ScubaObject::SetUsePhysicalScaleUVs(BOOL flag)
{
    BOOL curState = GetUsePhysicalScaleUVs();
    if (curState == flag)
        return;
    if (theHold.Holding())
        theHold.Put(new RealWorldScaleRecord<ScubaObject>(this, curState));
