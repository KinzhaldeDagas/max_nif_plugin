...
class GengonObject : public SimpleObject, public IParamArray, public RealWorldMapSizeInterface {
	friend class GengonTypeInDlgProc;
	friend class GengonObjCreateCallBack;
	public:
		// Class vars
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_GENGON) : _T("Gengon"); }
		BOOL HasUVW();
...
class GengonClassDesc:public ClassDesc {
	...
	void*			Create(BOOL loading = FALSE)
    {
        ...
        return new GengonObject(loading);
    }
	const TCHAR*	ClassName() { return GetString(IDS_AP_GENGON_CLASS); }
	const TCHAR*	NonLocalizedClassName() { return _T("Gengon"); }
...
static ParamUIDesc descParam[] = {
	...
	// Gen Smoothing
	ParamUIDesc(PB_SMOOTH,TYPE_SINGLECHECKBOX,IDC_OT_SMOOTH),
	
	// Gen UVs
	ParamUIDesc(PB_GENUVS,TYPE_SINGLECHECKBOX,IDC_GENTEXTURE),			
	};
...
{ float s,fillet,r;
  int sides;
	...
	pblock->GetValue(PB_SIDES,t,sides);
	pblock->GetValue(PB_RADIUS,t,r);
	pblock->GetValue(PB_FILLET,(increate?0:t),fillet);
...
BOOL GengonCapValsDlgProc::GetRWSState()
{
    BOOL check = IsDlgButtonChecked(mhWnd, IDC_REAL_WORLD_MAP_SIZE);
    return check;
}

void GengonCapValsDlgProc::Update()
{
    if (ob == NULL) return;
...
INT_PTR GengonCapValsDlgProc::DlgProc(
...
INT_PTR GengonTypeInDlgProc::DlgProc(
		TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
	{
	switch (msg) {
		case WM_COMMAND:
...
GengonObject::GengonObject(BOOL loading) 
	{
	ReplaceReference(0, CreateParameterBlock(gengondescVer1,PBLOCK_LENGTH, CURRENT_VERSION));
...
void GengonObject::BeginEditParams(IObjParam *ip,ULONG flags,Animatable *prev)
	...
	if (pmapCreate && pmapParam) {
        ...
        GengonCapValsDlgProc* dlg = static_cast<GengonCapValsDlgProc*>(pmapParam->GetUserDlgProc());
        if (dlg != NULL) {
	...
	if(pmapParam) {
		// A callback for the type in.
		pmapParam->SetUserDlgProc(new GengonCapValsDlgProc(this));
		}	
	}
...
{		
	...
	pblock->GetValue(PB_HSEGS,ip->GetTime(),dlgHSegs);
	pblock->GetValue(PB_FSEGS,ip->GetTime(),dlgFSegs);
	pblock->GetValue(PB_SIDES,ip->GetTime(),dlgSides);	
	pblock->GetValue(PB_SSEGS,ip->GetTime(),dlgSSegs);	
}
...
void BuildGengonMesh(Mesh &mesh,
		...
		float radius1, float fillet, float height, int genUVs,int smooth, BOOL usePhysUVs)
	{
	Point3 p;
	...
	{
        // This is a bug in the normal (non-usePhysUVs) UVs.
        // I only fixed in the usePhysUVs case so as not to break old files.
        // Scott Morrison 10/20/04
        if (usePhysUVs) {
            deltax=mesh.verts[endpt].x-mesh.verts[startpt].x;
            deltay=mesh.verts[endpt].y-mesh.verts[startpt].y;
        } else {
	...
	{ float sidedist=(float)sqrt((deltax*deltax)+(deltay*deltay));
	  float sdenom = (fillet+sidedist)*segs;
      float udenom = usePhysUVs ? 1.0f : 2.0f*radius1;
	  float fdist = (fsegs>0?(fillet/fsegs)/sdenom:0.0f);
      // This is a bug in the normal (non-usePhysUVs) UVs.
      // I only fixed in the usePhysUVs case so as not to break old files.
      // The division by sidesegs is correct since deltaX and deltaY are
      // relative to a single segment, not the whole side.
      // Scott Morrison 10/20/04
      float sdist = (sidedist/(usePhysUVs ? 1.0f : sidesegs))/sdenom;
      ...
      float uScale = usePhysUVs ? 2*sdenom : 1.0f;
      if (usePhysUVs) sdist /= 2.0f;
	  for (i=0;i<=llsegs;i++)
	  { hlevel=1.0f-hcount++/(float)llsegs;
		mesh.setTVert(tvcount++,u=0.0f,heightScale*hlevel,0.0f);
		for (iseg=0;iseg<segs;iseg++) {
            for (fcount=0;fcount<fsegs;fcount++) {
                u += fdist;
                mesh.setTVert(tvcount++,uScale*u,heightScale*hlevel,0.0f);
		}
...
void GengonObject::BuildMesh(TimeValue t)
	...
	BuildGengonMesh(mesh,
		...
		radius, fillet, height, genUVs,smooth, GetUsePhysicalScaleUVs());
	}
		...
		switch(point) {
			...
			case 2:
				...
				{ ob->increate=FALSE;return CREATE_ABORT;}
				  ...
				  { int sides;
					ob->pblock->GetValue(PB_SIDES,0,sides);
					s=(float)sqrt(2.0f*r*r*(1-(float)cos(TWOPI/sides)));
					s*=0.5f;
				  }
				}
...
BOOL GengonObject::OKtoDisplay(TimeValue t) 
	{
	float radius;
	pblock->GetValue(PB_RADIUS,t,radius);
...
TSTR GengonObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_SIDES:  return localized ? GetString(IDS_RB_SIDES)   : _T("Sides");
		case PB_RADIUS: return localized ? GetString(IDS_RB_RADIUS)  : _T("Radius");
		case PB_FILLET: return localized ? GetString(IDS_RB_FILLET)  : _T("Fillet");
		case PB_HEIGHT: return localized ? GetString(IDS_RB_HEIGHT)  : _T("Height");
		case PB_SSEGS:  return localized ? GetString(IDS_RB_SSEGS)   : _T("Side Segments");
		case PB_FSEGS:  return localized ? GetString(IDS_RB_FSEGS)   : _T("Fillet Segments");
		case PB_HSEGS:  return localized ? GetString(IDS_RB_HSEGS)   : _T("Height Segments");
		case PB_GENUVS: return localized ? GetString(IDS_MXS_GENUVS) : _T("mapCoords");
		case PB_SMOOTH: return localized ? GetString(IDS_MXS_SMOOTH) : _T("Smooth");
		default:        return  _T("");
		}
	}
...
RefTargetHandle GengonObject::Clone(RemapDir& remap) 
	{
	GengonObject* newob = new GengonObject(FALSE);	
...
BOOL GengonObject::GetUsePhysicalScaleUVs()
{
    return ::GetUsePhysicalScaleUVs(this);
}


void GengonObject::SetUsePhysicalScaleUVs(BOOL flag)
{
    BOOL curState = GetUsePhysicalScaleUVs();
    if (curState == flag)
        return;
    if (theHold.Holding())
        theHold.Put(new RealWorldScaleRecord<GengonObject>(this, curState));
