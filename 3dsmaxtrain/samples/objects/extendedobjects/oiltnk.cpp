...
class OilTnkObject : public SimpleObject, public IParamArray, public RealWorldMapSizeInterface {
	friend class OilTnkTypeInDlgProc;
	friend class OilTnkCapValsDlgProc;
	friend class OilTnkObjCreateCallBack;
	public:
		// Class vars
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_OILTNK) : _T("OilTank"); }
		BOOL HasUVW();
...
class OilTnkClassDesc:public ClassDesc {
	...
	void*			Create(BOOL loading = FALSE)
    {
        ...
        return new OilTnkObject(loading);
    }
	const TCHAR*	ClassName() { return GetString(IDS_AP_OILTNK_CLASS); }
	const TCHAR*	NonLocalizedClassName() { return _T("OilTank"); }
...
static ParamUIDesc descParam[] = {
	...
	// Gen Smoothing
	ParamUIDesc(PB_SMOOTHON,TYPE_SINGLECHECKBOX,IDC_OT_SMOOTH),			

	// Gen Slice
	ParamUIDesc(PB_SLICEON,TYPE_SINGLECHECKBOX,IDC_OT_SLICEON),			
...
INT_PTR OilTnkTypeInDlgProc::DlgProc(
...
{ float radius,height,minc,capheight,hh;
  BOOL con;
	...
	pblock->GetValue(PB_CAPHEIGHT,(increate?0:t),capheight);
	pblock->GetValue(PB_RADIUS,t,radius);
	pblock->GetValue(PB_CENTERS,t,con);
	pblock->GetValue(PB_HEIGHT,t,height);
...
{ float maxb,height,capheight,blend;
  BOOL con;
	...
	pblock->GetValue(PB_CAPHEIGHT,t,capheight);
	pblock->GetValue(PB_CENTERS,t,con);
	pblock->GetValue(PB_HEIGHT,t,height);
	pblock->GetValue(PB_BLEND,(increate?0:t),blend);
	height=(float)fabs(height);
	if (con) height+=2.0f*capheight;
...
BOOL OilTnkCapValsDlgProc::GetRWSState()
{
    BOOL check = IsDlgButtonChecked(mhWnd, IDC_REAL_WORLD);
    return check;
}

void OilTnkCapValsDlgProc::Update()
{
    if (ob == NULL) return;
    BOOL usePhysUVs = ob->GetUsePhysicalScaleUVs();
    CheckDlgButton(mhWnd, IDC_REAL_WORLD, usePhysUVs);
    EnableWindow(GetDlgItem(mhWnd, IDC_REAL_WORLD), ob->HasUVW());
}

INT_PTR OilTnkCapValsDlgProc::DlgProc(
		TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
	{ 	switch (msg) {
    case WM_INITDIALOG: {
        mhWnd = hWnd;
    ...
    case WM_COMMAND:
        ...
        case IDC_REAL_WORLD: {
            BOOL check = IsDlgButtonChecked(hWnd, IDC_REAL_WORLD);
            theHold.Begin();
...
OilTnkObject::OilTnkObject(BOOL loading) 
	{
	ReplaceReference(0, CreateParameterBlock(OilTnkdescVer0, PBLOCK_LENGTH, CURRENT_VERSION));
...
void OilTnkObject::BeginEditParams(IObjParam *ip,ULONG flags,Animatable *prev)
	...
	if (pmapCreate && pmapParam) {
        ...
        OilTnkCapValsDlgProc* dlg = static_cast<OilTnkCapValsDlgProc*>(pmapParam->GetUserDlgProc());
        if (dlg != NULL) {
...
void BuildOilTnkMesh(Mesh &mesh,
		...
		float height, float capheight, float pie1, float pie2, int genUVs, BOOL usePhysUVs)
	{
	Point3 p;		   
  ...
  else
  {
	...
	float tangle=theta/capsegs, chrad=tnkrad*(float)sin(theta-alpha), tmpangle = 0.0f;
	...
	{ int tvcount=0,nexttv;
	  ...
	  float hlevel;
      float heightScale = usePhysUVs ? height-2.0f*capheight-2.0*blend : 1.0f;
...
void OilTnkObject::BuildMesh(TimeValue t)
	...
	BuildOilTnkMesh(mesh, segs, smooth, hsegs, doPie,radius, blend, height, capheight,pie1, pie2, genUVs, GetUsePhysicalScaleUVs());
	}
	...
	if (msg==MOUSE_POINT||msg==MOUSE_MOVE) {
		switch(point) {
			case 0:
				...
				ob->pblock->GetValue(PB_CENTERS,0,con);
			...
			case 3:
				...
				if (ch>=rmax) ch=rmax;
				float blend;
				ob->pblock->GetValue(PB_BLEND,0,blend);
...
TSTR OilTnkObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_RADIUS:    return localized ? GetString(IDS_RB_RADIUS)    : _T("Radius");
		case PB_CAPHEIGHT: return localized ? GetString(IDS_RB_CAPHEIGHT) : _T("Cap Height");
		case PB_HEIGHT:    return localized ? GetString(IDS_RB_HEIGHT)    : _T("Height");
		case PB_BLEND:     return localized ? GetString(IDS_RB_BLEND)     : _T("Blend");
		...
		case PB_SMOOTHON:  return localized ? GetString(IDS_RB_SMOOTHON)  : _T("Smooth On");
		case PB_SLICEON:   return localized ? GetString(IDS_RB_SLICEON)   : _T("Slice On");
		case PB_SLICEFROM: return localized ? GetString(IDS_RB_SLICEFROM) : _T("Slice From");
		case PB_SLICETO:   return localized ? GetString(IDS_RB_SLICETO)   : _T("Slice To");
...
RefTargetHandle OilTnkObject::Clone(RemapDir& remap) 
	{
	OilTnkObject* newob = new OilTnkObject(FALSE);	
...
BOOL OilTnkObject::GetUsePhysicalScaleUVs()
{
    return ::GetUsePhysicalScaleUVs(this);
}


void OilTnkObject::SetUsePhysicalScaleUVs(BOOL flag)
{
    BOOL curState = GetUsePhysicalScaleUVs();
    if (curState == flag)
        return;
    if (theHold.Holding())
        theHold.Put(new RealWorldScaleRecord<OilTnkObject>(this, curState));
