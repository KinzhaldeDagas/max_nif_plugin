...
//**************************************************************************/
// DESCRIPTION: Defines a hose object
// AUTHOR: Unknown - 
//***************************************************************************/
...
class DamperObject : public DynamHelperObject {
	friend class DamperParamDlg;
	friend class DamperEmitterCreateCallback;
	public:
		...
		enum ERefIds 
		{
			...
			kREF_CUSTNODE,
			kREF_CUSTNODE2,
			kREF_THIS_NODE_MONITOR,
			kREF_NUM, // must always be last 
		};

		static DamperObject *editOb;
		INode *custnode2,*custnode;
...
public:
		...
		INode* GetThisNode() const;
		void SetThisNode(INode* node);
	...
	private:
		INode* thisnode;
		ReferenceTarget* mThisNodeMonitor;

		static const USHORT kVERSION_CHUNK = 0x0110;
		static const USHORT kCOM_CUSTNAME_CHUNK = 0x0100;
		static const USHORT kCOM_CUSTNAME2_CHUNK = 0x0101;
		// legacy chunk, not saved in versions kVERSION_01 and higher
		static const USHORT kCOM_CNODE_CHUNK = 0x0102; 
...
// PostLoadCallback
//------------------------------------------------------------------------------
class DamperObject::DamperPLC : public PostLoadCallback
{
	public:
		DamperPLC(DamperObject& aDamper) : mDamper(aDamper) { }
		void proc(ILoad *iload) 
		{
			mDamper.SetThisNode(mDamper.thisnode);
			mDamper.thisnode = NULL;
			delete this;
		}

	private:
		DamperObject& mDamper;
};
...
static ParamUIDesc descDamperParam1[] = {
	...
	// IsRenderable?
	ParamUIDesc(PB_RENDERABLE,TYPE_SINGLECHECKBOX,IDC_DAMPER_RENDERTHIS),			
	...
	// Smooth Cylinder
	ParamUIDesc(PB_SMOOTHCYL,TYPE_SINGLECHECKBOX,IDC_SMOOTHCYL),			
...
// Rod Parameters
	...
	// Smooth Rod
	ParamUIDesc(PB_SMOOTHPISTON,TYPE_SINGLECHECKBOX,IDC_SMOOTHROD),			

// Boot parameters

	// Enable Boot
	ParamUIDesc(PB_ENABLEBOOT,TYPE_SINGLECHECKBOX,IDC_BOOTON),			
	...
	// Smooth Boot
	ParamUIDesc(PB_SMOOTHBOOT,TYPE_SINGLECHECKBOX,IDC_SMOOTHBOOT),			
...
//Dynamics Parameters
	...
	// Texture Coords
	ParamUIDesc(PB_MAPMEMAPME,TYPE_SINGLECHECKBOX,IDC_DAMPER_MAPME),			
};
...
class CreateMDamperProc : public MouseCallBack,ReferenceMaker {
	private:
		...
		virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("CreateMDamperProc"); }
...
class DamperCreateCPartRestoreNode : public RestoreObj {
	public:   		
		DamperObject *obj ;
		TSTR name,namer;
		INode *save;
		...
		DamperCreateCPartRestoreNode(DamperObject *o) 
		{
...
class DamperCreateCPart2RestoreNode : public RestoreObj {
	public:   		
		DamperObject *obj;
		TSTR name,namer;
		INode *save,*saver;
		DamperCreateCPart2RestoreNode(DamperObject *o) 
		{
		...
		case MOUSE_POINT:
			switch ( point ) {
              ...
              createInterface->GetMacroRecorder()->EmitScript();
					...
					SSBlizObject->SetThisNode(l_CloudNode);
					attachedToNode = TRUE;
					assert( l_CloudNode );					
...
{	
	...
	theHold.Begin();
	if (dodist)
	{ theHold.Put(new DamperCreateCPart2RestoreNode(po));
	po->ReplaceReference(DamperObject::kREF_CUSTNODE2,node);
	  po->custname2 = TSTR(node->GetName());
	  // Automatically check show result and do one update
	  po->ShowName2();	
	}
	else
	{ theHold.Put(new DamperCreateCPartRestoreNode(po));
	po->ReplaceReference(DamperObject::kREF_CUSTNODE,node);
...
{ float outdiam,indiam,maxval;
  po->pblock->GetValue(PB_BOOTSTOPDIA,t,outdiam);
  po->pblock->GetValue(PB_BOOTDIA1,t,indiam);
  maxval=0.99f*outdiam;
  BOOL change = (maxval<indiam);
  if (change==TRUE)
...
{ float outdiam,indiam,outmin;
  po->pblock->GetValue(PB_BOOTDIA2,t,outdiam);
  po->pblock->GetValue(PB_BOOTDIA1,t,indiam);
  outmin=1.01f*indiam;
  BOOL change = (outdiam<outmin);
  if (change==TRUE) po->pblock->SetValue(PB_BOOTDIA2,t,outmin);
...
{ float stopthick,stopfillet;
  po->pblock->GetValue(PB_BOOTSTOPLEN,t,stopthick);
  po->pblock->GetValue(PB_BOOTSTOPFILLET,t,stopfillet);
  BOOL change=(stopthick<stopfillet);
  if (change==TRUE) po->pblock->SetValue(PB_BOOTSTOPFILLET,t,stopthick);
...
{ float outdiam,indiam,stopdiam,minstop;
  po->pblock->GetValue(PB_BOOTDIA1,t,outdiam);
  po->pblock->GetValue(PB_PISTONDIA,t,indiam);
  po->pblock->GetValue(PB_BOOTSTOPDIA,t,stopdiam);
  bootmin=1.01f*indiam;
  BOOL changes = FALSE,changed = (outdiam<bootmin);
  if (changed==TRUE) {po->pblock->SetValue(PB_BOOTDIA1,t,bootmin);outdiam=bootmin;}
  minstop=1.05f*indiam;
  changes=(stopdiam<minstop);
  if (changes==TRUE) po->pblock->SetValue(PB_BOOTSTOPDIA,t,minstop);  
...
{ float outdiam,indiam,maxin;
  po->pblock->GetValue(PB_CYLDIA,t,outdiam);
  po->pblock->GetValue(PB_INSIDEDIA,t,indiam);
  maxin=0.99f*outdiam;
  BOOL change = (maxin<indiam);
  if (change==TRUE) po->pblock->SetValue(PB_INSIDEDIA,t,maxin);
  FixFSpinnerLimits(po->hParams,IDC_CYLINSIDEDIASPIN,INSIDEDIAM,maxin,change);
  float maxpiston=0.95f*outdiam,pistondiam;
  po->pblock->GetValue(PB_PISTONDIA,t,pistondiam);
  change=(maxpiston<pistondiam);
  if (change==TRUE){ pistondiam=maxpiston;po->pblock->SetValue(PB_PISTONDIA,t,maxpiston);}
  ...
  po->pblock->GetValue(PB_BOOTDIA1,t,bootdiam);
  change=(maxboot<bootdiam);
  if (change==TRUE) po->pblock->SetValue(PB_BOOTDIA1,t,maxboot);
...
{ if (!po->editOb) return;
  po->ShowNames();
  int emethod;
  po->pblock->GetValue(PB_ENDSMETHOD,0,emethod);
  BoundParams(!emethod);
  int booton;
  po->pblock->GetValue(PB_ENABLEBOOT,t,booton);
  BootParams(booton);
  if (booton)
  { float stopdiam,maindiam,pistondiam;
    po->pblock->GetValue(PB_BOOTSTOPDIA,t,stopdiam);
    po->pblock->GetValue(PB_CYLDIA,t,maindiam);
...
{ 
	...
	{ 
		...
		ok = (po->GetThisNode() != NULL);
		
		int act;
		po->pblock->GetValue(PB_DYNTYPE,0,act);
		ActuatorParams(act);
		DamperParams(!act);
	}
	else
...
INT_PTR DamperParamDlg::DlgProc(
		...
		case WM_COMMAND:
			...
			{ case IDC_SPRING_PICKOBJECT1: 
			  ...
			  case IDC_CYLDIA: 
				  ...
				  break;				   
			  case IDC_BOOTON: 
				  { int ison;po->pblock->GetValue(PB_ENABLEBOOT,0,ison);
					BootParams(ison);
				  }
				  break;				   
			  case IDC_DAMPERON: 
...
{	int FToTick=(int)((float)TIME_TICKSPERSEC/(float)GetFrameRate());
	...
	custnode2 = custnode = thisnode = NULL;
	mThisNodeMonitor = NULL;
	ReplaceReference(kREF_PBLK, CreateParameterBlock(descDamperVer0, length, CURRENT_VERSION));
	assert(pblock);
	...
	ReplaceReference(
		kREF_THIS_NODE_MONITOR, 
		static_cast<ReferenceTarget*>(ip->CreateInstance(REF_TARGET_CLASS_ID, NODEMONITOR_CLASS_ID)));
	DbgAssert(mThisNodeMonitor != NULL);
}
...
{	SetAFlag(A_RENDER);
	int renderme;
	pblock->GetValue(PB_RENDERABLE,t,renderme);
	if (!renderme) 
	{	MeshInvalid();
...
{	if (TestAFlag(A_RENDER))
	...
	{	ivalid=FOREVER;
		...
		{	
			...
			if (GetThisNode() != NULL) {
				S = GetThisNode()->GetObjTMBeforeWSM(t);
			}
		...
		int smoothcyl, smoothrod, smoothboot = 0;
		...
		// These variables are set to magic numbers as yet unknown in origin (ie no explanation for them)
		int bootsides = 3;
		int bootfolds = 1;
		int bootres = 4;
		int bootfilletsegs = 0;

		float bootdia1		= 0.0f;
		float bootdia2		= 0.0f;
		float bootstopdia	= 0.0f;
		float bootstoplen	= 0.0f;
		float bootstoploc	= 0.0f;
		float bootstopfillet	= 0.0f;
		float diffboot		= 0.0f;
		...
		int NvertsPerRingBoot=0, NRingsBoot=0, NvertsBoot=0, NfacesBoot=0, NtvertsBoot=0;
		if (IsBooted)
		...
		int mapmenow;
		pblock->GetValue(PB_MAPMEMAPME,0,mapmenow);
...
TSTR DamperObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_ENDSMETHOD:				return localized ? GetString(IDS_AP_ENDSMETHOD)         : _T("End Placement Method");
		case PB_NOREFLENGTH:				return localized ? GetString(IDS_AP_NOREFDL)            : _T("Free Damper Height");
		case PB_RENDERABLE:				return localized ? GetString(IDS_AP_RENDER)             : _T("Renderable Spring");

		case PB_MOUNTDIA:					return localized ? GetString(IDS_AP_MOUNTDIA)           : _T("Base Stud Diameter");
		case PB_MOUNTLEN:					return localized ? GetString(IDS_AP_MOUNTLEN)           : _T("Base Stud Height");
		case PB_CYLDIA:					return localized ? GetString(IDS_AP_CYLDIA)             : _T("Cylinder Diameter");
		case PB_CYLLEN:					return localized ? GetString(IDS_AP_CYLLEN)             : _T("Cylinder Height");
		case PB_CYLSIDES:					return localized ? GetString(IDS_AP_CYLSIDES)           : _T("Cylinder Sides");
		case PB_CYLFILLET1:				return localized ? GetString(IDS_AP_CYLFILLET1)         : _T("Cylinder Fillet 1");
		case PB_CYLFILLET1SEGS:			return localized ? GetString(IDS_AP_CYLFILLET1SEGS)     : _T("Cylinder Fillet 1 Segments");
		case PB_CYLFILLET2:				return localized ? GetString(IDS_AP_CYLFILLET2)         : _T("Cylinder Fillet 2");
		case PB_CYLFILLET2SEGS:			return localized ? GetString(IDS_AP_CYLFILLET2SEGS)     : _T("Cylinder Fillet 2 Segments");
		case PB_INSIDEDIA:				return localized ? GetString(IDS_AP_INSIDEDIA)          : _T("Inside Diameter");
		case PB_SMOOTHCYL:				return localized ? GetString(IDS_AP_SMOOTHCYL)          : _T("Smooth Cylinder");

		case PB_PISTONDIA:				return localized ? GetString(IDS_AP_PISTONDIA)          : _T("Piston Diameter");
		case PB_PISTONLEN:				return localized ? GetString(IDS_AP_PISTONLEN)          : _T("Piston Height");
		case PB_PISTONSIDES:				return localized ? GetString(IDS_AP_PISTONSIDES)        : _T("Piston Sides");
		case PB_SMOOTHPISTON:			return localized ? GetString(IDS_AP_SMOOTHPISTON)       : _T("Smooth Piston");

		case PB_ENABLEBOOT:				return localized ? GetString(IDS_AP_ENABLEBOOT)         : _T("Enable Boot");
		case PB_BOOTDIA1:					return localized ? GetString(IDS_AP_BOOTDIA1)           : _T("Boot Small Diameter");
		case PB_BOOTDIA2:					return localized ? GetString(IDS_AP_BOOTDIA2)           : _T("Boot Large Diameter");
		case PB_BOOTSIDES:				return localized ? GetString(IDS_AP_BOOTSIDES)          : _T("Boot Sides");
		case PB_BOOTFOLDS:				return localized ? GetString(IDS_AP_BOOTFOLDS)          : _T("Boot Folds");
		case PB_BOOTRESOLUTION:			return localized ? GetString(IDS_AP_BOOTRESOLUTION)     : _T("Boot Fold Resolution");
		case PB_BOOTSTOPDIA:				return localized ? GetString(IDS_AP_BOOTSTOPDIA)        : _T("Boot Stop Diameter");
		case PB_BOOTSTOPLEN:				return localized ? GetString(IDS_AP_BOOTSTOPLEN)        : _T("Boot Stop Height");
		case PB_BOOTSETBACK:				return localized ? GetString(IDS_AP_BOOTSETBACK)        : _T("Boot Stop Setback");
		case PB_BOOTSTOPFILLET:			return localized ? GetString(IDS_AP_BOOTSTOPFILLET)     : _T("Boot Stop Fillet");
		case PB_BOOTSTOPFILLETSEGS:	return localized ? GetString(IDS_AP_BOOTSTOPFILLETSEGS) : _T("Boot Stop Fillet Segements");
		case PB_SMOOTHBOOT:				return localized ? GetString(IDS_AP_SMOOTHBOOT)         : _T("Smooth Boot");

		case PB_DYNTYPE:					return localized ? GetString(IDS_AP_DYNTYPE)            : _T("Dynamics Object Type");
		case PB_DYNDRAG:					return localized ? GetString(IDS_AP_DYNDRAG)            : _T("Drag");
		case PB_DYNDRAGUNITS:			return localized ? GetString(IDS_AP_DYNDRAGUNITS)       : _T("Drag Units");
		case PB_SPRINGDIR:				return localized ? GetString(IDS_AP_DAMPERDIR)          : _T("Damper Direction");
		case PB_DYNFORCE:					return localized ? GetString(IDS_AP_DYNFORCE)           : _T("Force");
		case PB_DYNFORCEUNITS:			return localized ? GetString(IDS_AP_DYNFORCEUNITS)      : _T("Force Units");
		case PB_MAPMEMAPME:				return localized ? GetString(IDS_AP_MAPME)              : _T("Generate Mapping Coordinates");
...
RefTargetHandle DamperObject::Clone(RemapDir& remap) 
{
	...
	newob->ReplaceReference(kREF_PBLK, remap.CloneRef(pblock));	
	if (custnode) {
		newob->ReplaceReference(kREF_CUSTNODE,custnode);
	}
	if (custnode2) {
		newob->ReplaceReference(kREF_CUSTNODE2,custnode2);
	}

	DbgAssert(mThisNodeMonitor != NULL);
	newob->ReplaceReference(kREF_THIS_NODE_MONITOR, remap.CloneRef(mThisNodeMonitor));	
...
{	switch(i) {
		case kREF_PBLK: return(RefTargetHandle)pblock;
		case kREF_CUSTNODE: return (RefTargetHandle)custnode;
		case kREF_CUSTNODE2: return (RefTargetHandle)custnode2;
		case kREF_THIS_NODE_MONITOR: return mThisNodeMonitor;
		default: return NULL;
		}
	}
...
{ 
	switch (i) 
	{
		...
		case kREF_CUSTNODE: 
			custnode = (INode *)rtarg; 
			custname = (custnode ? custnode->GetName() : NULL);
			break;

		case kREF_CUSTNODE2: 
			custnode2 = (INode *)rtarg; 
			custname2 = (custnode2 ? custnode2->GetName() : NULL);
			break;

		case kREF_THIS_NODE_MONITOR:
			mThisNodeMonitor = rtarg;
			break;
	}
}
...
RefResult DamperObject::NotifyRefChanged( 
		...
		case REFMSG_TARGET_DELETED:	
			{ if (hTarget==custnode)
			  { if (theHold.Holding() && !TestAFlag(A_HELD)) 
					theHold.Put(new DamperCreateCPartRestoreNode(this));
			    DeleteReference(kREF_CUSTNODE);
				custnode=NULL;cancelled=FALSE;
				custname=_T(" ");				
				}
			  if (hTarget==custnode2) 
			  { if (theHold.Holding() && !TestAFlag(A_HELD)) 
					theHold.Put(new DamperCreateCPart2RestoreNode(this));
			    DeleteReference(kREF_CUSTNODE2);
	...
	return SimpleObject::NotifyRefChanged(changeInt, hTarget, partID, message, propagate);
	}
...
{ 	
	...
	isave->BeginChunk(kCOM_CUSTNAME_CHUNK);		
	isave->WriteWString(custname);
	isave->EndChunk();

	isave->BeginChunk(kCOM_CUSTNAME2_CHUNK);		
...
{
	// Legacy max files do not contain a version number
	IOResult res = IO_OK;
...
IOResult DamperObject::DoLoadLegacy(ILoad *iload)
{
	IOResult res = IO_OK;
	...
	iload->SetObsolete();
	iload->RegisterPostLoadCallback(new DamperPLC(*this));

	return IO_OK;
}

IOResult DamperObject::DoLoad(ILoad *iload)
{
	IOResult res = IO_OK;
...
INode* DamperObject::GetThisNode() const
{
	if (mThisNodeMonitor)
	{
		INodeMonitor* nodeMonitor = 
			static_cast<INodeMonitor*>(mThisNodeMonitor->GetInterface(IID_NODEMONITOR));
		DbgAssert(nodeMonitor != NULL);
		return nodeMonitor->GetNode();
	}
	return NULL;
}

void DamperObject::SetThisNode(INode* node)
{
	DbgAssert(mThisNodeMonitor != NULL);
	...
	return nodeMonitor->SetNode(node);
}

const TCHAR *DamperClassDesc::ClassName ()	{return GetString(IDS_AP_DAMPER);}
const TCHAR* DamperClassDesc::NonLocalizedClassName() { return _T("Damper"); }
const TCHAR *DamperClassDesc::Category ()	{return GetString(IDS_EP_DYNPRIMS);}
const TCHAR *DamperObject::GetObjectName(bool localized) const {return localized ? GetString(IDS_AP_DAMPER_OBJECT) : _T("Damper");}
