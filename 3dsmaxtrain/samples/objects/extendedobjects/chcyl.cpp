...
class ChCylinderObject : public SimpleObject2, public RealWorldMapSizeInterface {
	...
	public:
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_CHCYLINDER) : _T("ChamferCyl"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("ChamferCyl"); }
...
static_assert(chcyl_params == CHCYL_PARAMBLOCK_ID, "");
...
void FixChCylFillet(IParamBlock2* pblock2, TimeValue t, ParamID radiusId, ParamID heightId, ParamID filletId, ParamID id, PB2Value &v)
{
	...
	pblock2->GetValue(radiusId, t, radius);
	pblock2->GetValue(heightId, t, height);
	pblock2->GetValue(filletId, t, fillet);
...
{
	float radius;
	pblock2->GetValue(chcyl_radius, t, radius);
