...
public:
	...
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_LEXT) : _T("L-Ext"); }
...
public:
	...
	const TCHAR*	NonLocalizedClassName() { return _T("L-Ext"); }
...
static_assert(lext_params == LEXT_PARAMBLOCK_ID, "");

void FixLExtWidth(IParamBlock2 *pblock2, TimeValue t, ParamID lengthId, ParamID widthId, ParamID id, PB2Value &v)
{
	float length, width;

	pblock2->GetValue(widthId, t, width);
	pblock2->GetValue(lengthId, t, length);
...
void LextObject::BuildMesh(TimeValue t)
{
	...
	int createmeth = pblock2->GetInt(lext_centercreate, 0);
...
{
	float length;
	pblock2->GetValue(lext_blength, t, length);
