...
{
	friend class HoseParamDlg;
	friend class HoseEmitterCreateCallback;
	public:
...
// PostLoadCallback
//------------------------------------------------------------------------------
class HoseObject::HosePLC : public PostLoadCallback
{
	public:
		HosePLC(HoseObject& aHose) : mHose(aHose) { }
		void proc(ILoad *iload) 
		{
			mHose.SetThisNode(mHose.mThisNode);
			mHose.mThisNode = NULL;
			delete this;
		}
...
static ParamUIDesc descHoseParam1[] = {
	...
	// Flex On
	ParamUIDesc(PB_FLEXON,TYPE_SINGLECHECKBOX,IDC_HOSE_FLEXON),			
...
class CreateMHoseProc : public MouseCallBack,ReferenceMaker {
	private:
		...
		virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("CreateMHoseProc"); }
		int NumRefs() { return kREF_NUM; }
		RefTargetHandle GetReference(int i) 
		{ 
			DbgAssert(kREF_CLOUD_NODE == i);
			return (RefTargetHandle)CloudNode; 
		} 
		
		void SetReference(int i, RefTargetHandle rtarg) 
		{ 
			DbgAssert(kREF_CLOUD_NODE == i);
			CloudNode = (INode *)rtarg; 
		}
...
class HoseCreateCPartRestoreNode : public RestoreObj {
	public:   		
		HoseObject *obj;
		TSTR name,namer;
		INode *save,*saver;
		HoseCreateCPartRestoreNode(HoseObject *o) {
...
class HoseCreateCPart2RestoreNode : public RestoreObj {
	public:   		
		HoseObject *obj;
		TSTR name,namer;
		INode *save,*saver;
		HoseCreateCPart2RestoreNode(HoseObject *o) {
	...
	switch (message) {
		...
		case REFMSG_TARGET_SELECTIONCHANGE:
		 	...
		 	if ( SSBlizObject && CloudNode==hTarget ) {
				...
				theHold.Suspend();
				DeleteReference(kREF_CLOUD_NODE);
				theHold.Resume();
				goto endEdit;
				}
			// fall through
...
{ if ( SSBlizObject ) 
	{ 
		...
		{	theHold.Suspend();
			DeleteReference(kREF_CLOUD_NODE);  // sets cloudNode = NULL
			theHold.Resume();}
	}
}
...
{ float width,height,fillet;
	...
	pblock->GetValue(PB_RECTWIDTH,t,width);
	pblock->GetValue(PB_RECTDEPTH,t,height);
	pblock->GetValue(PB_RECTFILLET,(increate?0:t),fillet);
		...
		case MOUSE_POINT:
			switch ( point ) {
              ...
              createInterface->GetMacroRecorder()->EmitScript();
					...
					theHold.Suspend();
					ReplaceReference(kREF_CLOUD_NODE, l_cloudNode);
					theHold.Resume();
					mat.IdentityMatrix();
				default:				
					if (createCB) {						
...
//						macroRec->Enable();
						...
						if (res==CREATE_ABORT)
							goto abort;
						if (res==CREATE_STOP)
						    theHold.Accept(OperationDesc(IDS_AP_CREATE, GetString(IDS_AP_CREATE), hInstance, SSBlizObject));
...
//				macroRec->Enable();
				if (res==CREATE_ABORT) 
					goto abort;
				if (res==CREATE_STOP)
					theHold.Accept(OperationDesc(IDS_AP_CREATE, GetString(IDS_AP_CREATE), hInstance, SSBlizObject));	// TH
...
{	
	...
	theHold.Begin();
	if (dodist)
	{ theHold.Put(new HoseCreateCPart2RestoreNode(po));
		po->ReplaceReference(HoseObject::kREF_CUSTNODE2,node);
	  po->custname2 = TSTR(node->GetName());
	  // Automatically check show result and do one update
	  po->ShowName2();	
	}
	else
	{ theHold.Put(new HoseCreateCPartRestoreNode(po));
	  po->ReplaceReference(HoseObject::kREF_CUSTNODE,node);
...
{ if (!po->editOb) return;
  ...
  BoundParams(!emethod);
  po->pblock->GetValue(PB_FLEXON,0,flexon);
  FlexParams(flexon);
  po->pblock->GetValue(PB_WIRETYPE,0,hshape);
  HoseShape(hshape);
}

INT_PTR HoseParamDlg::DlgProc(
...
{	
	...
	case WM_COMMAND:
		...
		case IDC_HOSE_FLEXON: 
			{ int flexon; po->pblock->GetValue(PB_FLEXON,0,flexon);
			FlexParams(flexon);				  
			}
			break;				   
...
HoseObject::HoseObject()
{
	...
	pblock = NULL;
	custnode = custnode2 = NULL;

	int FToTick=(int)((float)TIME_TICKSPERSEC/(float)GetFrameRate());
	int length = PBLOCK_HOSE_LENGTH;
	ReplaceReference(kREF_PBLK, CreateParameterBlock(descHoseVer0, length, CURRENT_VERSION));
	DbgAssert(pblock);
...
INode* HoseObject::GetThisNode() const
{
	if (mThisNodeMonitor)
	{
...
void HoseObject::SetThisNode(INode* node)
{
	DbgAssert(mThisNodeMonitor != NULL);
...
void HoseObject::GetWorldBoundBox(TimeValue t, INode *inode, ViewExp* vpt, Box3& box)
{	
	SetThisNode(inode);
...
TSTR HoseObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_ENDSMETHOD:			return localized ? GetString(IDS_AP_ENDSMETHOD)       : _T("End Placement Method");
		case PB_NOREFLENGTH:			return localized ? GetString(IDS_EP_HOSEHEIGHT)       : _T("Hose Height");
		case PB_SEGMENTS:				return localized ? GetString(IDS_EP_SEGMENTS)         : _T("Segments Along Hose");
		case PB_SMOOTH:				return localized ? GetString(IDS_AP_SMOOTH)           : _T("Smooth Spring");
		case PB_RENDERABLE:			return localized ? GetString(IDS_EP_RENDERHOSE)       : _T("Renderable Hose");

		case PB_WIRETYPE:				return localized ? GetString(IDS_AP_HOSETYPE)         : _T("Hose Cross-Section Type");
		case PB_RNDDIA:				return localized ? GetString(IDS_AP_HOSEDIA)          : _T("Round Hose Diameter");
		case PB_RNDSIDES:				return localized ? GetString(IDS_AP_HOSESIDES)        : _T("Round Hose Sides");

		case PB_RECTWIDTH:			return localized ? GetString(IDS_AP_HOSERECTWIDTH)    : _T("Rectangular Hose Width");
		case PB_RECTDEPTH:			return localized ? GetString(IDS_AP_HOSERECTDEPTH)    : _T("Rectangular Hose Depth");
		case PB_RECTFILLET:			return localized ? GetString(IDS_AP_HOSERFILLET)      : _T("Rectangular Hose Fillet Size");
		case PB_RECTFILLETSIDES:	return localized ? GetString(IDS_AP_HOSERFILLETSIDES) : _T("Rectangular Hose Fillet Segs");
		case PB_RECTROTANGLE:		return localized ? GetString(IDS_AP_HOSERECTROT)      : _T("Rectangular Hose Section Rotation");

		case PB_DSECWIDTH:			return localized ? GetString(IDS_AP_HOSEDSECWIDTH)    : _T("D-Section Hose Width");
		case PB_DSECDEPTH:			return localized ? GetString(IDS_AP_HOSEDSECDEPTH)    : _T("D-Section Hose Depth");
		case PB_DSECFILLET:			return localized ? GetString(IDS_AP_HOSEDFILLET)      : _T("D-Section Hose Fillet Size");
		case PB_DSECFILLETSIDES:	return localized ? GetString(IDS_AP_HOSEDFILLETSIDES) : _T("D-Section Hose Fillet Segs");
		case PB_DSECRNDSIDES:		return localized ? GetString(IDS_AP_HOSEDSECRNDSIDES) : _T("D-Section Hose Round Segs");
		case PB_DSECROTANGLE:		return localized ? GetString(IDS_AP_HOSEDSECROT)      : _T("D-Section Hose Section Rotation");

      case PB_MAPMEMAPME:			return localized ? GetString(IDS_AP_HOSEMAPME)        : _T("Generate Mapping Coordinates");

		case PB_FLEXON:				return localized ? GetString(IDS_AP_FLEXON)           : _T("Flex Section Enabled");
		case PB_FLEXSTART:			return localized ? GetString(IDS_AP_FLEXSTART)        : _T("Flex Section Start");
		case PB_FLEXSTOP:				return localized ? GetString(IDS_AP_FLEXSTOP)         : _T("Flex Section Stop");
		case PB_FLEXCYCLES:			return localized ? GetString(IDS_AP_FLEXCYCLES)       : _T("Flex Cycle Count");
		case PB_FLEXDIAMETER:		return localized ? GetString(IDS_AP_FLEXDIAMETER)     : _T("Flex Section Diameter");

		case PB_TENSION1:				return localized ? GetString(IDS_AP_TENSION1)         : _T("Top Tension");
		case PB_TENSION2:				return localized ? GetString(IDS_AP_TENSION2)         : _T("Bottom Tension");
...
RefTargetHandle HoseObject::Clone(RemapDir& remap) 
	...
	if (custnode) newob->ReplaceReference(kREF_CUSTNODE, custnode);
	if (custnode2) newob->ReplaceReference(kREF_CUSTNODE2, custnode2);
		...
		case REFMSG_TARGET_DELETED:	
			{ if (hTarget==custnode)
			  { if (theHold.Holding() && !TestAFlag(A_HELD)) 
					theHold.Put(new HoseCreateCPartRestoreNode(this));
			  ...
			  { if (theHold.Holding() && !TestAFlag(A_HELD)) 
					theHold.Put(new HoseCreateCPart2RestoreNode(this));
...
IOResult HoseObject::DoLoadLegacy(ILoad *iload)
{
	ULONG nb;
	IOResult res = IO_OK;
	...
	while (IO_OK == (res = iload->OpenChunk())) 
	{
		switch (iload->CurChunkID()) 
		{
			...
			case kCOM_CNODE_CHUNK: 
			{
				...
				if (refid >= 0) {
					iload->RecordBackpatch(refid, (void**)&mThisNode);
				}
			}
				break;
				}
	...
	iload->SetObsolete();
	iload->RegisterPostLoadCallback(new HosePLC(*this));

	return IO_OK;
}

IOResult HoseObject::DoLoad(ILoad *iload)
{
	IOResult res = IO_OK;
...
const TCHAR* HoseClassDesc::NonLocalizedClassName() { return _T("Hose"); }
const TCHAR *HoseClassDesc::Category ()	{return GetString(IDS_RB_EXTENDED);}
const TCHAR *HoseObject::GetObjectName(bool localized) const {return localized ? GetString(IDS_AP_HOSE_OBJECT) : _T("Hose");}
