...
public:
	...
	const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_RB_CYLINDER) : _T("Cylinder"); }
...
static_assert(cyl_params == CYLINDER_PARAMBLOCK_ID, "");
...
void BuildCylinderPoly(MNMesh & mesh, int segs, int smooth, int lsegs,
	...
	if (capsegs == 1) {
		...
		if (doPie) mesh.RetriangulateFace(nf, TRIANGULATION_LEGACY);
...
{
	float radius = 0.f;
	pblock2->GetValue(cyl_radius, t, radius);
