...
#define MAX_TURNS		float(1000.0f)
...
class HelixObject: public SimpleShape, public IParamArray {			   
	...
	public:	
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_TH_HELIX) : _T("Helix"); }
		void InitNodeName(TSTR& s) { s = GetString(IDS_TH_HELIX); }		
		Class_ID ClassID() { return Class_ID(HELIX_CLASS_ID,0); }  
		void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_TH_HELIX_CLASS) : _T("Helix"); }
...
class HelixObjClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Helix"); }
...
INT_PTR HelixTypeInDlgProc::DlgProc(
...
void HelixObject::EndEditParams( IObjParam *ip,ULONG flags,Animatable *next )
{
	...
	pblock->GetValue(PB_TURNS,ip->GetTime(),dlgTurns);
	pblock->GetValue(PB_BIAS,ip->GetTime(),dlgBias);
...
TSTR HelixObject::GetParameterName(int pbIndex, bool localized)
	{
	switch (pbIndex) {
		case PB_RADIUS1:   return localized ? GetString(IDS_RB_RADIUS1)   : _T("Radius 1");
		case PB_RADIUS2:   return localized ? GetString(IDS_RB_RADIUS2)   : _T("Radius 2");
		case PB_HEIGHT:    return localized ? GetString(IDS_TH_HEIGHT)    : _T("Height");
		case PB_TURNS:     return localized ? GetString(IDS_TH_TURNS)     : _T("Turns");
		case PB_BIAS:      return localized ? GetString(IDS_TH_BIAS)      : _T("Bias");
		case PB_DIRECTION: return localized ? GetString(IDS_TH_DIRECTION) : _T("Direction");
		default:           return _T("");
		}
	}
