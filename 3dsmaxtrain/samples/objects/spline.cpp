...
class SplineObjCreateCallBack: public CreateMouseCallBack {
	...
	public:
		...
		BOOL TolerateOrthoMode() {return TRUE;}
	};

class SplineObject: public SplineShape, public IParamArray {			   
	friend class SplineObjCreateCallBack;
	friend INT_PTR CALLBACK SplineParamDialogProc( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam );
	...
	public:
		...
		const TCHAR *GetObjectName(bool localized) const override { return localized ? GetString(IDS_TH_LINE) : _T("Line"); }
		void InitNodeName(TSTR& s) { s = GetString(IDS_TH_LINE); }		
		Class_ID ClassID() { return Class_ID(SPLINE3D_CLASS_ID,0); }  
		void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_TH_LINE_CLASS) : _T("Line"); }
...
public:
      ...
      // SR NOTE64: VS2005 gets confused with scoping; so we explicitely state that we want 
      //            the global class definition of ISplineSelect, and NOT the privately 
      //            derived instance from SplineShape.
...
class SplineObjClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Line"); }
...
static int GetDefaultDragType()
{
	int dragType = INDEX_BEZIER;
	MarketDefaults* dflts = GetMarketDefaults();
	if(dflts){
		dflts->SetSectionName(_T("Primitive Spline"));
		int val = 0;
		if(dflts->GetInt(_T("DefaultDragType"), val) && 1 == val){
			dragType = INDEX_CORNER;
		}
	}
	return dragType;
}
...
void SplineObjClassDesc::ResetClassParams(BOOL fileReset)
	...
	SplineObject::crtDragType    = GetDefaultDragType();
...
INT_PTR SplineTypeInDlgProc::DlgProc(
...
{
	// Once we're done editing, turn off the closure prompt override
	SetAlwaysShowSplineClosurePrompt(false);
...
int SplineObjCreateCallBack::proc(ViewExp *vpt,int msg, int point, int flags, IPoint2 m, Matrix3& mat ) {
	...
	{
		vpt->SnapPreview(m, m, NULL, SNAP_IN_3D | SNAP_OVERRIDE_UNSEL_OBJS_ONLY | SNAP_DURING_CREATION);
		return TRUE;
	}
	...
	if(ob->createDone) {
		ob->suspendSnap = FALSE;
		// Once created, turn off the "always prompt" option
		SetAlwaysShowSplineClosurePrompt(false);
		return CREATE_STOP;
		}

	// For the Line object, when initially creating it we want to always have the closure prompt to preserve
	// legacy behavior.
	SetAlwaysShowSplineClosurePrompt(true);
	...
	// refer to the 'Create' method of spline3d, if mat equals NULL, spline will snap a relative
	// point in the viewport's construction plane. But when cross-viewport-creation is enabled, 
	// we should snap a world-space 3d points, passing a identified matrix have the desired behavior.
	static Matrix3 constructPlan(1);
	if (point == 0) {
		// use the first point plan as the construct plan
		vpt->GetConstructionTM(constructPlan);
	}
	int res = theSpline->Create(vpt,msg,point,flags,m,&constructPlan,ob->ip);
	
	switch(res) {
		case CREATE_ABORT:
			ob->suspendSnap = FALSE;
			ob->createDone = TRUE;
			// Once creation is done, turn off the "always prompt" option
			SetAlwaysShowSplineClosurePrompt(false);
...
TSTR SplineObject::GetParameterName(int pbIndex, bool localized)
	{
//	switch (pbIndex) {
//		}
	return _T("");
...
//DebugPrint(_T("GetReference(%d):"),i);
	if(loadingOldFile) {
		if(i == 0) {
//DebugPrint(_T("%p [nullpblock]\n"),nullpblock);
			return nullpblock;
			}
//DebugPrint(_T("%p [ipblock]\n"),ipblock);
		return ipblock;
		}
...
//DebugPrint(_T("SetReference(%d, %p) SC:%X CID:%X:%X --"),i, rtarg, scid, cid.PartA(), cid.PartB());
	if(loadingOldFile) {
		if(i == 0) {
//DebugPrint(_T(" [nullpblock]\n"));
			nullpblock = (IParamBlock*)rtarg;
			return;
			}
//DebugPrint(_T(" [ipblock]\n"));
		ipblock=(IParamBlock*)rtarg;
//int nparams = ipblock->NumParams();
//DebugPrint(_T("pblock params:%d\n"),nparams);
//for(int p = 0; p < nparams; ++p)
//  DebugPrint(_T("Param %d type %d\n"), p, ipblock->GetParameterType(p));
		return;					   
		}
//DebugPrint(_T(" [SS]\n"));
...
//DebugPrint(_T("SO RemapRefOnLoad called (%d)"),iref);
	if(loadingOldFile) {
//DebugPrint(_T("Loading old, left as is\n"));
		return iref;
		}
...
class SplinePostLoadCallback : public PostLoadCallback {
	public:
		...
		void proc(ILoad *iload) {
			...
			if(so->ipblock) {
				so->ipblock->GetValue(IPB_STEPS, 0, steps);
				so->ipblock->GetValue(IPB_OPTIMIZE, 0, optimize);
				so->ipblock->GetValue(IPB_ADAPTIVE, 0, adaptive);
...
unsigned long SplineObject::GetObjectDisplayRequirement() const
{
	unsigned long objectDiplayRequirements = suspendSnap || drawTicks ? MaxSDK::Graphics::ObjectDisplayRequireLegacyDisplayMode : 0;
	return objectDiplayRequirements | __super::GetObjectDisplayRequirement();
}
