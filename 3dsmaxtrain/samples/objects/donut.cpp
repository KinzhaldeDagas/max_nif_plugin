...
class DonutObject: public SimpleSpline, public IParamArray {
	...
	public:
		...
		const TCHAR *GetObjectName(bool localized) const override
		{
			if (mPipe)
			{
				return localized ? GetString(IDS_PIPE_CLASSNAME) : _T("PipeObject");
			}
			else
			{
				return localized ? GetString(IDS_TH_DONUT) : _T("Donut");
			}
		}
		void InitNodeName(TSTR& s) { if(mPipe) s = GetString(IDS_PIPE_CLASSNAME); else s = GetString(IDS_TH_DONUT); }
		Class_ID ClassID();
		void GetClassName(MSTR& s, bool localized) const override
		{
			if (mPipe)
			{
				s = localized ? GetString(IDS_PIPE_CLASSNAME) : _T("PipeObject");
			}
			else
			{
				s = localized ? GetString(IDS_TH_DONUT_CLASS) : _T("Donut");
			}
		}
...
class DonutObjClassDesc:public ClassDesc {
	...
	const TCHAR*	NonLocalizedClassName() { return _T("Donut"); }
...
class PipeObjClassDesc:public ClassDesc {
	public:
	int 			IsPublic() { return 0; }
	void*			Create(BOOL loading = FALSE) { return new DonutObject(TRUE); }
	const TCHAR*	ClassName() { return GetString(IDS_PIPE_CLASSNAME); }
	const TCHAR*	NonLocalizedClassName() { return _T("PipeObject"); }
	SClass_ID		SuperClassID() { return SHAPE_CLASS_ID; }
   	Class_ID		ClassID() { return Class_ID(PIPE_CLASS_ID,0); }
	const TCHAR* 	Category() { return GetString(IDS_TH_SPLINES);  }
	void			ResetClassParams(BOOL fileReset);
	};
...
static PipeObjClassDesc pipeObjDesc;

ClassDesc* GetDonutDesc() { return &donutObjDesc; }
ClassDesc* GetPipeDesc() { return &pipeObjDesc; }
...
void PipeObjClassDesc::ResetClassParams(BOOL fileReset)
	{
	DonutObject::crtPos         = Point3(0,0,0);
	DonutObject::crtRadius1     = 0.0f;
	DonutObject::crtRadius2     = 0.0f;
...
static ParamUIDesc descParamPipe[] = {
	// Radius 1
	ParamUIDesc(
		PB_RADIUS1,
		EDITTYPE_UNIVERSE,
...
INT_PTR DonutTypeInDlgProc::DlgProc(
	...
	if (pmapCreate && pmapParam) {
		...
		if(mPipe)
		{
			pmapParam = CreateCPParamMap(
				descParamPipe,PARAMDESCPIPE_LENGTH,
				pblock,
				ip,
				hInstance,
				MAKEINTRESOURCE(IDD_PIPEPARAM),
...
DonutObject::DonutObject(BOOL pipe) : SimpleSpline() 
{
	mPipe = pipe;
	Init();
}
...
Class_ID DonutObject::ClassID()
{
	if(mPipe)
		return Class_ID(PIPE_CLASS_ID,0); 
	else
		return Class_ID(DONUT_CLASS_ID,0); 
}  
...
TSTR DonutObject::GetParameterName(int pbIndex, bool localized) 
	{
	switch (pbIndex) {
		case PB_RADIUS1:
			return localized ? GetString(IDS_TH_RADIUS1) : _T("Radius 1");
		case PB_RADIUS2:
			if(mPipe)
				return localized ? GetString(IDS_THICKNESS) : _T("Thickness");
			else
			return localized ? GetString(IDS_TH_RADIUS2) : _T("Radius 2");
		default:
			return _T("");
		}
	}
