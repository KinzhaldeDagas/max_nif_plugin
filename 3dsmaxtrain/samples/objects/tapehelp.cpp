...
public:
	 ...
	 const TCHAR*  NonLocalizedClassName() { return _T("Tape"); }
...
void TapeHelpObject::UpdateUI(TimeValue t)
{
	 ...
	 if(dirLen) {
			...
			_stprintf(buf, _T("%g"), acos((double)dirPt.x) * RadToDegDbl);
			SetDlgItemText(hTapeHelpParams, IDC_TAPE_X_AXIS, buf);
			_stprintf(buf, _T("%g"), acos((double)dirPt.y) * RadToDegDbl);
			SetDlgItemText(hTapeHelpParams, IDC_TAPE_Y_AXIS, buf);
			_stprintf(buf, _T("%g"), acos((double)dirPt.z) * RadToDegDbl);
...
TapeHelpObject::TapeHelpObject() : HelperObject()
{
	...
	enable   = 1; // Enable hit detection so that the tape helper can be selected by clicking it when it is created through MAXScript/the SDK
	editting = 0;
	lastDist = 0.0f;
...
RefResult TapeHelpObject::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
		 PartID& partID, RefMessage message, BOOL propagate ) 
{
	 switch (message) {
			...
			case REFMSG_GET_PARAM_NAME_LOCALIZED: {
				 ...
				 switch (gpn->index) {
						case 0:
							 gpn->name = GetString(IDS_DB_TAPE_LENGTH);
							 break;
				 }
				 return REF_HALT;
			}
...
//    TargetObject *targObject;
			...
			virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("TapeHelpCreationManager"); }
...
public:
	 ...
	 void EnterMode() { UI::MouseCursors::LoadMouseCursor(UI::MouseCursors::Crosshair); }
...
void TapeHelpCreationManager::CreateNewObject()
{
	...
	if (tapeObject)
	{
		tapeObject->Enable(0); // Disable hit detection until the tape helper has been fully initialized to avoid problems when it is created through the UI
