...
#include "IStereoCompatibleCamera.h" 
...
public:
	...
	float GetFOV(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetFOV)
	void SetLens(TimeValue t, float f);
	float GetLens(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetLens)
	...
	float GetClipDist(TimeValue t, int which, Interval &valid);
	DECL_FUNC_NO_INTERVAL_2(GetClipDist)
	...
	BOOL GetDOFEnable(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetDOFEnable)
	void  SetDOFFStop(TimeValue t, float fS);
	float GetDOFFStop(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetDOFFStop)
// mjm - begin - 07.17.00
	void SetMultiPassEffectEnabled(TimeValue t, BOOL enabled);
	BOOL GetMultiPassEffectEnabled(TimeValue t, Interval& valid);
	DECL_FUNC_NO_INTERVAL_1(GetMultiPassEffectEnabled)
...
// mjm - end
	...
	float GetEnvRange(TimeValue t, int which, Interval& valid);
	DECL_FUNC_NO_INTERVAL_2(GetEnvRange)
	...
	const TCHAR *GetObjectName(bool localized) const override
	{
		if (localized)
		{
			return hasTarget ? GetString(IDS_DB_TARGET_CAM) : GetString(IDS_DB_FREE_CAM);
		}
		else
		{
			return hasTarget ? _T("Target Camera") : _T("Free Camera");
		}
	}
	...
	void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_DB_SIMPLECAM_CLASS) : _T("SimpleCamera"); }
...
//			case 2:
// nac - end
				return localized ? GetString(IDS_MP_EFFECT) : _T("MultiPass Effect");
// mjm - end
			default:
				return _T("");
		}
...
// mjm - end
	...
	virtual void RegisterIStereoCompatibleCameraCallback(IStereoCameraCallback* callback);
	virtual void UnRegisterIStereoCompatibleCameraCallback(IStereoCameraCallback* callback);
	virtual bool HasTargetNode(TimeValue t) const;
	virtual float GetTargetDistance(TimeValue t) const;
...
private:
	bool RaisePreCameraTargetChanged(bool bTarget, float& targetDistance, TimeValue t);
	bool RaisePreSetCameraTargetDistance(float distance, TimeValue t);
	bool RaisePreCameraTargetDistanceUpdated(float& distance, TimeValue t);
	IStereoCameraCallback*	mStereoCameraCallBack;
