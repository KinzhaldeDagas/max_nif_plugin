...
TSTR ShapeSnap::iconName(int index) const
{
...
ShapeSnap::ShapeSnap(){ //constructor
	...
	name[PERP_SUB] = GetString(IDS_PERP);
	name[TAN_SUB] = GetString(IDS_TAN);


	markerdata[0]=OsnapMarker(8,mark0verts,mark0es);
	markerdata[1]=OsnapMarker(11,mark1verts,mark1es);

	iconNames[PERP_SUB] = _T("SnapTools/MaxSDK/Shape/Perpendicular");
	iconNames[TAN_SUB] = _T("SnapTools/MaxSDK/Shape/Tangent");
...
//watje 3-7-99 fix to handle hidden spline segs
	...
	for (int i=0; i<segs; ++i, p1++, p2++)
	{
		...
		if (vis) 
		{
			ppoint = PerpToSeg(p1->p,p2->p,relpoint,extend,look);
			if(ppoint != NULL) 
			{
	...
	if (!open) 
	{
		if (vis) 
		{
			ppoint = PerpToSeg(pline.pts->p,p1->p,relpoint);
			if(ppoint != NULL) 
			{
				theman->RecordHit(new OsnapHit(*ppoint, this, PERP_SUB, NULL));
			}
...
//watje 3-7-99 fix to handle hidden spline segs
	...
	for (int i = 0; i < segs; ++i, p1++, p2++)
	{
		...
		if (vis) 
		{
			ppoint = TanToSeg(p1->p,p2->p,relpoint);
			if(ppoint != NULL) 
			{
	...
	if (pline.IsClosed()) 
	{
		if (vis) 
		{
		ppoint = TanToSeg(pline.pts->p,p1->p,relpoint);
			if(ppoint != NULL) 
			{
				theman->RecordHit(new OsnapHit(*ppoint, this, TAN_SUB, NULL));
			}
...
//	void*			Create(OsnapManager *pman) { return new ShapeSnap(pman); }
	...
	const TCHAR*	NonLocalizedClassName() { return _T("ShapeSnap"); }
