...
TSTR gridSnap::iconName(int index) const
{
...
gridSnap::gridSnap()
{
	name[INT_SUB]= GetString(IDS_GRID_INT);
	name[EDGE_SUB]= GetString(IDS_GRID_EDGES);
	markerdata[0]=OsnapMarker(5,mark0verts,mark0es);
	markerdata[1]=OsnapMarker(5,mark0verts,mark0es);

	iconNames[INT_SUB]= _T("SnapTools/MaxSDK/Grid/GridPoints");
	iconNames[EDGE_SUB]= _T("SnapTools/MaxSDK/Grid/GridLines");
}
	...
	if(	GetActive(INT_SUB))
	{
		...
		if (gridSpacing > 0.0f || gridSpacing < 0.0f)
		{
			...
			if (theman->GetVpt() && theman->GetVpt()->getGW()) {
				theman->GetVpt()->getGW()->setTransform(Matrix3{});
			}
...
//	void*			Create(OsnapManager *pman) { return new gridSnap(pman); }
	...
	// This is the non-localized class name for scripting
	const TCHAR*    NonLocalizedClassName() { return _T("gridSnap"); }
