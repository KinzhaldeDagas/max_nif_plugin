...
TSTR PivotSnap::iconName(int index) const
{
...
PivotSnap::PivotSnap()
{
	name[PIV_SUB]= GetString(IDS_PIVOT);
	name[BBOX_SUB]= GetString(IDS_BBOX);
	markerdata[0]=OsnapMarker(5,mark0verts,mark0es);
	markerdata[1]=OsnapMarker(13,mark1verts,mark1es);

	iconNames[PIV_SUB] = _T("SnapTools/MaxSDK/Pivot/Pivot");
	iconNames[BBOX_SUB] = _T("SnapTools/MaxSDK/Pivot/BoundingBox");
};
...
{	
	...
	BOOL wsm = (inode->GetProperty(PROPID_HAS_WSM) != 0);
	...
	if(	GetActive(PIV_SUB))
	{
		...
		// aszabo|May.31.04|Pivot snap will record snap-hits only if the mouse is 
		// over object. This is meant to make our snap system feel smoother.
		BOOL bGotHit = pobj->HitTest(t, inode, HITTYPE_POINT, GetCOREInterface()->GetCrossing(), 
			HIT_ABORTONHIT | HIT_ANYSOLID, p, theman->GetVpt());
		...
		if (bGotHit)
		{
			...
			Point3 pivpt(atm.GetTrans() + InvTm.GetTrans());
...
//	void*			Create(OsnapManager *pman) { return new PivotSnap(pman); }
	...
	const TCHAR*	NonLocalizedClassName() { return _T("PivotSnap"); }
