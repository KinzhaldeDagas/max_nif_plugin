...
void XmeshSnap::Snap(Object* pobj, IPoint2 *p, TimeValue t)
{
	...
	std::unique_ptr<MeshAccess> maccess;
	if (m)
	{
		maccess.reset(new MeshAccess(m));
		bInSubObjSel = (m->selLevel != MESH_OBJECT);
	}
	else if (mn)
	{
		maccess.reset(new MeshAccess(mn));
	...
	bool bIgnoreSelSubObj)
{
	...
	const BitArray* meshVertHide = m.VertHide();
...
#pragma omp parallel for
	...
	{
		if (sv[i] != NULL &&		
				(meshVertHide == nullptr || !(*meshVertHide)[i]) &&	
...
//	void*			Create(OsnapManager *pman) { return new XmeshSnap(pman); }
	...
	const TCHAR*	NonLocalizedClassName() { return _T("XmeshSnap"); }
