...
{ 
	...
	_renderTime() = TIME_NegInfinity;
	_inRender() = false;
	_inFileLoad() = false;
	GetClassDesc()->MakeAutoParamBlocks(this);
	fillPBlockWithReportHandles();
}
...
void PFOperatorInstanceShape::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_INSTANCESHAPE_CLASS_NAME) : _T("Shape Instance");
}
...
int PFOperatorInstanceShape::SetProperty(ULONG id, void *data)
{
	if (id == PROPID_FINALUPDATETIME)
	{
		TimeValue* t = (TimeValue*)data;
		_renderTime() = *t;
		return TRUE;
	}
...
void PFOperatorInstanceShape::SetReference(int i, RefTargetHandle rtarg)
{
	...
	case kInstanceShape_reference_object:	
		_object() = (INode *)rtarg;			
		if (!theHold.RestoreOrRedoing() && !_inFileLoad()) {
			resetObjectList();
	...
	case kInstanceShape_reference_material:	
		_material() = (Mtl *)rtarg;			
		if (theHold.Redoing() && !_inFileLoad()) {
			BuildMeshes(GetCOREInterface()->GetTime(), true);
		}
		break;
	}
...
RefResult PFOperatorInstanceShape::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
													 PartID& partID, RefMessage message, BOOL propagate)
{
	if ( hTarget == pblock() ) {
		...
		case REFMSG_CHANGE:
			{
				...
				case kInstanceShape_mtlIDOffset:
		...
		case kInstanceShape_RefMsg_UpdateShape:
			...
			RefreshUI(kInstanceShape_message_type);
			NotifyDependents(FOREVER, 0, REFMSG_CHANGE, NOTIFY_ALL, TRUE);
			theHold.Accept(GetString(IDS_UPDATEPARTICLESHAPE));
			return REF_STOP;
		// New Random Seed
...
IOResult PFOperatorInstanceShape::Load(ILoad* iload)
{
	_inFileLoad() = true;
...
const TCHAR* PFOperatorInstanceShape::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_INSTANCESHAPE_OBJECT_NAME) : _T("Shape Instance ");
}
...
//+--------------------------------------------------------------------------+
class ShapeIndexCollector
{
public:
	ShapeIndexCollector(void)
		:	m_numItems(0)
		,	m_numIndices(NULL)
		,	m_corrTable(NULL)
	{}
	...
	~ShapeIndexCollector(void)
	{
		if (m_numItems > 0)
		{
			...
			for(int i=0; i<m_numItems; ++i)
				delete [] m_corrTable[i];
			delete [] m_corrTable;
		}
	}

	void Init(int numItems, int numNew)
	{
		if (numItems > 0) 
		{
			m_numItems = numItems;
			m_numIndices = new int[m_numItems];
			m_corrTable = new int*[m_numItems];
			for(int i=0; i<m_numItems; ++i) 
			{
				m_numIndices[i] = 0;
				m_corrTable[i] = new int[numNew];
			}
		}
	}

	void Set(int particleIndex, int itemIndex)
	{
		(m_corrTable[itemIndex])[m_numIndices[itemIndex]] = particleIndex;
		m_numIndices[itemIndex] += 1;
	}

	void Get(int itemIndex, Tab<int>& data)
	{
		data.ZeroCount();
		if (m_numIndices[itemIndex])
			data.Append(m_numIndices[itemIndex], m_corrTable[itemIndex]);
	}
...
inline bool EqualFaces(const Face* f0, const Face* f1)
{
	return ((f0->v[0] == f1->v[0]) &&
		(f0->v[1] == f1->v[1]) && 
		(f0->v[2] == f1->v[2]) &&
		(f0->smGroup == f1->smGroup) &&
		(f0->flags == f1->flags));
}

inline bool VertsAreClose(const Point3* v0, const Point3* v1, const Point3& eps)
{
	Point3 dif = *v1 - *v0;
	return (fabs(dif.x) <= eps.x) && (fabs(dif.y) <= eps.y) && (fabs(dif.z) <= eps.z);
}

bool EqualMeshes(const Mesh* pMesh0, const Mesh* pMesh1)
{
	if (pMesh0 == pMesh1)
		return true;

	if ((NULL == pMesh0) || (NULL == pMesh1))
		return false;

	int numFaces = pMesh0->numFaces;
	int numVerts = pMesh0->numVerts;
	if ((numVerts != pMesh1->numVerts) || (numFaces != pMesh1->numFaces))
		return false;

	// compare faces
	Face* pF0 = pMesh0->faces;
	Face* pF1 = pMesh1->faces;
	for(int i=0; i<numFaces; ++i, ++pF0, ++pF1)
	{
		if (!EqualFaces(pF0, pF1))
			return false;
	}

	Box3 bbox = (const_cast<Mesh*>(pMesh0))->getBoundingBox();
	Point3 boxSize = bbox.pmax - bbox.pmin;
	Point3 sizeEps = 0.001f*boxSize;
	Point3* pV0 = pMesh0->verts;
	Point3* pV1 = pMesh1->verts;
	for(int i=0; i<numVerts; ++i, ++pV0, ++pV1)
	{
		if (!VertsAreClose(pV0, pV1, sizeEps))
			return false;
	}

	// more checks for mesh mapping and other things


	return true;
}

#define kShrinkableSize 128
									   ...
									   IPFIntegrator* integrator)
{
	...
	variation = std::min(variation, kMaxVariation);
	...
	if (pblock()->GetInt(kInstanceShape_fastShapeEvaluation, 0) != 0)
	{
		TimeValue updateTime = getUpdateTime();
		isFinalStep = ((updateTime == timeEnd.tick) && (fabs(timeEnd.fraction) < 0.01f));
	}
	...
	bool assignNodeHandles = false;
	for(int i=0; i<meshNodeHandle().Count(); ++i)
	{
		if (meshNodeHandle(i))
		{
			assignNodeHandles = true;
			break;
		}
	}
	
	IParticleChannelINodeHandleW* chShapeNodeHandle = NULL;
	IParticleChannelINodeHandleR* chShapeNodeHandleR = NULL;
	if (assignNodeHandles)
	{
			chShapeNodeHandle = (IParticleChannelINodeHandleW*)chCont->EnsureInterface(PARTICLECHANNELSHAPENODEHANDLEW_INTERFACE,
																	ParticleChannelINodeHandle_Class_ID,
																	true, PARTICLECHANNELSHAPENODEHANDLER_INTERFACE,
																	PARTICLECHANNELSHAPENODEHANDLEW_INTERFACE, true);
			chShapeNodeHandleR = GetParticleChannelShapeNodeHandleRInterface(pCont);
			if ((NULL == chShapeNodeHandle) || (NULL == chShapeNodeHandleR))
				assignNodeHandles = false;
	}
	...
	bool forceInitScale = false;
	IParticleChannelPoint3W* chScale = NULL;
	if (setScale) {
		forceInitScale = (NULL == GetParticleChannelScaleRInterface(pCont));
	...
	bool forceInitShape = (NULL == GetParticleChannelShapeRInterface(pCont));
	IParticleChannelMeshW* chMesh = (IParticleChannelMeshW*)chCont->EnsureInterface(PARTICLECHANNELSHAPEW_INTERFACE,
																ParticleChannelMesh_Class_ID,
																true, PARTICLECHANNELSHAPER_INTERFACE,
																PARTICLECHANNELSHAPEW_INTERFACE, true );
	
	IParticleChannelMeshR* chMeshR = GetParticleChannelShapeRInterface(pCont);
	if ((chMesh == NULL) || (chMeshR == NULL))
		return false;	// can't modify Shape channel in the container
	...
	if (chNew->IsAllOld() && !animatedShape && !forceInitShape && !forceInitScale) 
		return true;
	...
	ShapeIndexCollector sic;
	if (!(animatedShape || acquireShape)) 
		sic.Init(numMesh, numNew);
	ShapeIndexCollector hic;
	if (assignNodeHandles)
		hic.Init(numMesh, numNew);

	// assign mesh to each particle
	for (i = 0; i < amount; i++)
	{
		bool isNew = chNew->IsNew(i);
		if (!animatedShape && !isNew && !forceInitShape && !forceInitScale) 
			continue;
		...
		if (setScale && (isNew || forceInitScale)) {
	...
	if (assignNodeHandles)
	{
		...
		for(i=0; i<numMesh; ++i) 
		{
			hic.Get(i, meshIndex);
			if (meshIndex.Count())
				chShapeNodeHandle->SetValue(meshIndex, meshNodeHandle(i));
		}
	}
	...
	if (!(animatedShape || acquireShape)) 
	{
		...
		int oldSharedValuesNum = chMeshR->GetValueCount();
		bool canOptimizeSharing = false;
		int newOldMap[kShrinkableSize];
		int numNewValues = 0;
		if ((numMesh <= kShrinkableSize) && (oldSharedValuesNum <= kShrinkableSize))
		{	// merge two shared tables with a check if they have overlap
			for(int i=0; i<numMesh; ++i)
			{
				newOldMap[i] = -1;
				for(int j=0; j<oldSharedValuesNum; ++j)
				{
					if (EqualMeshes(meshData(i), chMeshR->GetValueByIndex(j)))
					{
						newOldMap[i] = j;
						break;
					}
			...
			canOptimizeSharing = (numNewValues < numMesh);
		}
		if (canOptimizeSharing)
		{
			// remap index in shared table to the first particle index that uses the mesh data
			Tab<int> sharedToParticleIndex;
			sharedToParticleIndex.SetCount(oldSharedValuesNum);
			for(int i=0; i<oldSharedValuesNum; ++i)
			{
				sharedToParticleIndex[i] = -1;
			}
			for(int i=amount-1; i>=0; --i)
			{
				if (!chNew->IsNew(i))
				{
					int sharedIndex = chMeshR->GetValueIndex(i);
					if (sharedIndex >= 0)
						sharedToParticleIndex[sharedIndex] = i;
				}
			}
			for(int i=0; i<numMesh; ++i)
			{
				if (newOldMap[i] >= 0)
					newOldMap[i] = sharedToParticleIndex[newOldMap[i]];
			}
		}
		...
		for(i=0; i<numMesh; ++i) 
		{
			...
			if (meshIndex.Count())
			{
				if (canOptimizeSharing && (newOldMap[i] >= 0))
					chMesh->CopyValue(newOldMap[i], meshIndex);
				else
...
//|							From PFOperatorInstanceShape					 |
//+--------------------------------------------------------------------------+
int PFOperatorInstanceShape::NumHandleToReportClassIDs(void) const
{
	return pblock()->Count(kInstanceShape_handlesToReport)/2;
}

//+--------------------------------------------------------------------------+
//|							From PFOperatorInstanceShape					 |
//+--------------------------------------------------------------------------+
Class_ID PFOperatorInstanceShape::HandleToReportClassID(int index) const
{
	Class_ID cid(	ULONG(pblock()->GetInt(kInstanceShape_handlesToReport, 0, 2*index)),
					ULONG(pblock()->GetInt(kInstanceShape_handlesToReport, 0, 2*index+1)) );
	return cid;
}
...
void PFOperatorInstanceShape::BuildMeshes(const TimeValue t, bool setMaterial /* = false */)
{
	...
	_meshData().ZeroCount();
	_meshTM().ZeroCount();
	_meshNodeHandle().ZeroCount();
	if (node)	
		stackNodes.Append(1, &node, 10);
...
//					delete multi;
	...
	if (acquireMaterial && setMaterial) {
		// NOTE: following code seems wrong. If not holding, nothing is done with the multi that was created.
		// Added multi->MaybeAutoDelete() below to fix leakage.
		if (theHold.Holding()) {
...
int PFOperatorInstanceShape::BuildMeshesFromNode(const TimeValue t, INode *iNode)
{
	...
	ULONG nodeHandle = isNodeToBeReported(iNode) ? iNode->GetHandle() : 0;
	if (triObj == NULL) 
	{
		if (nodeHandle)
		{
			Mesh* nullMesh(NULL);
			_meshData().Append(1, &nullMesh, 10);
			_meshTM().Append(1, &nodeTM, 10);
			_meshNodeHandle().Append(1, &nodeHandle, 10);
		}
	...
	if (separateElements) {		// separate elements into meshes
		...
		for (DWORD i = 0; i < elemList.count; ++i) {
...
{
	int mtlIDOffset = pblock()->GetInt(kInstanceShape_mtlIDOffset, 0);
	...
	else 
	{
		...
		{
			if ((multi == NULL) && (mat != NULL))
			{
				MultiMtl* multiMtl = NewDefaultMultiMtl();
				multi = multiMtl;
				GetCOREInterface()->AssignNewName(multi);
				((Animatable*)(multi))->SetAFlag(A_PLUGIN1);
				int matNum = mat->NumSubMtls();
				int numMtls = (matNum ? matNum : 1) + mtlIDOffset;
				multiMtl->SetNumSubMtls(numMtls);
				TSTR emptyMtlName = _T("");
				for(int i=0; i<mtlIDOffset; ++i)
				{
					multiMtl->SetSubMtlAndName(i, NULL, emptyMtlName);
				}
				if (matNum > 0)
				{
					for(int i=0; i<matNum; ++i)
					{
						...
						if (subMtl != NULL)
						{
							multiMtl->SetSubMtlAndName(mtlIDOffset+i, subMtl, mat->GetSubMtlSlotName(i, true));
						}
					}
				}
				else
				{
					multiMtl->SetSubMtlAndName(mtlIDOffset, mat, mat->GetName());	
				}
			}
		}
		_matOffset(iNode) = mtlIDOffset;
	}
	return multi;
}
...
void PFOperatorInstanceShape::OrderMeshIndices()
{
	...
	for (int i = 0; i < meshCount; i++) {
		...
		Point3 bbMax = _meshData(i) ? _meshData(i)->getBoundingBox().Max() : Point3::Origin;
		xCoordIndex[i].first = nodeTM.PointTransform(bbMax).x;
...
{
	variation = std::max(variation, 0.0f);
	variation = std::min(variation, kMaxVariation);
	variation *= 0.7f;
...
void PFOperatorInstanceShape::notifyMtlChanged(NotifyInfo* info)
{
	...
	INode* node = GetNotifyParam<NOTIFY_NODE_POST_MTL>(info); DbgAssert(node);
	// check if the node is referenced
	if (node && HasNodeInHierarchy(_object(), node)) {
		if (theHold.Holding()) {
			BuildMeshes(GetCOREInterface()->GetTime(), true);
...
TimeValue PFOperatorInstanceShape::getUpdateTime() const
{
	if (inRender())
		return renderTime();
...
//+--------------------------------------------------------------------------+
void PFOperatorInstanceShape::fillPBlockWithReportHandles(void)
{
	int classIDsToReport[] = {	int(VRayProxyClassID.PartA()), int(VRayProxyClassID.PartB()), 
								int(VRaySphereClassID.PartA()), int(VRaySphereClassID.PartB()) };
	pblock()->Append(kInstanceShape_handlesToReport, sizeof(classIDsToReport)/sizeof(int), classIDsToReport);
}

//+--------------------------------------------------------------------------+
void PFOperatorInstanceShape::prepareHandlesReportCriteria(void)
{
	_reportHandleType() = pblock()->GetInt(kInstanceShape_reportNodeHandles, 0);
	_handlesToReport().ZeroCount();
	if (reportHandleType() == kInstanceShape_reportNodeHandles_selected)
	{
		const int numHandlesToReport = NumHandleToReportClassIDs();
		for(int i=0; i<numHandlesToReport; ++i)
		{
			Class_ID cid(	ULONG(pblock()->GetInt(kInstanceShape_handlesToReport, 0, 2*i)),
				ULONG(pblock()->GetInt(kInstanceShape_handlesToReport, 0, 2*i+1)) );
			_handlesToReport().Append(1, &cid);
		}
	}
}

//+--------------------------------------------------------------------------+
bool PFOperatorInstanceShape::isNodeToBeReported(INode* pNode) const
{
	if (NULL == pNode)
		return false;

	if (reportHandleType() == kInstanceShape_reportNodeHandles_none)
		return false;
	if (reportHandleType() == kInstanceShape_reportNodeHandles_all)
		return true;
	...
	for(int i=0; i<handlesToReport().Count(); ++i)
	{
		if (handlesToReport(i) == cid)
			return true;
	}
	return false;
}
...
BOOL PFOperatorInstanceShapePickNodeCallback::Filter(INode *iNode)
{
	...
	while (stack.Count())
	{
		...
		for(int i=0; i<op()->NumHandleToReportClassIDs(); ++i)
			if (cid == op()->HandleToReportClassID(i))
				return TRUE;
