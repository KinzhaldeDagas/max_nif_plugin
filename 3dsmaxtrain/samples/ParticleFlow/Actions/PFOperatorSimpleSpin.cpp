...
void PFOperatorSimpleSpin::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_SIMPLESPIN_CLASS_NAME) : _T("Spin");
}
...
RefResult PFOperatorSimpleSpin::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
const TCHAR* PFOperatorSimpleSpin::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_SIMPLESPIN_OBJECT_NAME) : _T("Spin ");
}
...
bool PFOperatorSimpleSpin::HasDynamicName(TSTR& nameSuffix)
{
	...
	Control* ctrl = pblock()->GetControllerByID(kSimpleSpin_spinrate);
	bool isRateAnimated = (ctrl) ? (ctrl->IsAnimated()!=0) : false;
	ctrl = pblock()->GetControllerByID(kSimpleSpin_variation);
									 ...
									 IPFIntegrator* integrator)
{
	...
	Control* ctrl = pblock()->GetControllerByID(kSimpleSpin_X);
	if (ctrl != NULL) axisXAnimated = (ctrl->IsAnimated() != 0);
	ctrl = pblock()->GetControllerByID(kSimpleSpin_Y);
	if (ctrl != NULL) axisYAnimated = (ctrl->IsAnimated() != 0);
	ctrl = pblock()->GetControllerByID(kSimpleSpin_Z);
	if (ctrl != NULL) axisZAnimated = (ctrl->IsAnimated() != 0);
	ctrl = pblock()->GetControllerByID(kSimpleSpin_variation);
	if (ctrl != NULL) varAnimated = (ctrl->IsAnimated() != 0);
	ctrl = pblock()->GetControllerByID(kSimpleSpin_divergence);
	if (ctrl != NULL) divAnimated = (ctrl->IsAnimated() != 0);
	ctrl = pblock()->GetControllerByID(kSimpleSpin_spinrate);
