...
//|							From Animatable									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
void PFTestScale::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_TEST_SCALE_CLASS_NAME) : _T("Scale Test");
}
...
//|							From ReferenceMaker								 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
RefResult PFTestScale::NotifyRefChanged(const Interval& changeInt,RefTargetHandle hTarget,PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message) {
		case REFMSG_CHANGE:
			if ( hTarget == pblock() ) {
...
//|							From BaseObject									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
const TCHAR* PFTestScale::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_TEST_SCALE_OBJECT_NAME) : _T("Scale Test ");
}
	...
	case kScaleTest_testType_scale:
		...
		if ((!isValueAnimated) && (!isVarAnimated)) {
			...
			if (testVar > 0) {
				...
				nameSuffix += buf;
				_stprintf_s(buf, 32, _T("%d"), testVar);
				nameSuffix += buf;
			}
							...
							Tab<float>& testTime)
{
	...
	for(int i=0; i<count; i++)
	{
		...
		case kScaleTest_axisType_minimum:
			currentValue = std::min(cur3DValue.x, std::min(cur3DValue.y, cur3DValue.z));
			break;
		case kScaleTest_axisType_median:
			currentValue = std::max(std::min(cur3DValue.x, cur3DValue.y),
				std::max(std::min(cur3DValue.x, cur3DValue.z), std::min(cur3DValue.y, cur3DValue.z)));
			break;
		case kScaleTest_axisType_maximum:
			currentValue = std::max(cur3DValue.x, std::max(cur3DValue.y, cur3DValue.z));
			break;
