...
void PFOperatorMarkShape::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_MARKSHAPE_CLASS_NAME) : _T("Shape Mark");
}
...
RefResult PFOperatorMarkShape::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
const TCHAR* PFOperatorMarkShape::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_MARKSHAPE_OBJECT_NAME) : _T("Shape Mark ");
}
		...
		for (int i = 0; i < amount; i++)
		{
			...
			if (alignSpeed && angleDistort && (LengthSquared(orgSpeed) > 0.0f)) {
				...
				if (lengthFactor == 0.0f) {
					...
					lengthFactor = std::min(maxDistort, (1.0f / lengthFactor));
				}
