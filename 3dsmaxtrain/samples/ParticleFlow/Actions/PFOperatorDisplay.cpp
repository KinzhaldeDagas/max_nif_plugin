	...
	for (auto it : srcMeshes)
	{
		...
		else
		{
			...
			if (pMesh)
			{
				...
				maxNumMaps = std::max(maxNumMaps, pMesh->getNumMaps());
		...
		for(int runType = 0; runType < 2; runType++)
		{
			...
			else
			{
				...
				if (meshCombine.numVerts > 0)
				{
					...
					{
                        //Workaround to get a non const pointer, as generateRenderItemsContext is a const reference, but we should set the MaterialRquiredStream in it, so the mesh built by PrepareDisplay/GetRenderItems is correct
                        MaxSDK::Graphics::GenerateMeshRenderItemsContext* pNonconstGenerateRenderItemsContext = const_cast<MaxSDK::Graphics::GenerateMeshRenderItemsContext*>(&generateRenderItemsContext);
                        //which is done by calling GenerateDefaultContext
                        pNonconstGenerateRenderItemsContext->GenerateDefaultContext(updateDisplayContext);
...
//|							From Animatable									 |
//+--------------------------------------------------------------------------+
void PFOperatorDisplay::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_DISPLAY_CLASS_NAME) : _T("DisplayParticles");
}
...
//|							From Animatable									 |
//+--------------------------------------------------------------------------+
TSTR PFOperatorDisplay::SubAnimName(int i, bool localized)
{
	return localized ? PFActions::GetString(IDS_PARAMETERS) : _T("Parameters");
}
...
//|							From BaseObject									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
const TCHAR* PFOperatorDisplay::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_DISPLAY_OBJECT_NAME) : _T("Display ");
}
			...
			for(i=0; i<block->Count(); i++) {
				...
				if (pMesh) {
						...
						|| ((displayType == kDisplay_type_geometry) && (newRndLimits & GW_BOX_MODE)))
					{
						...
						MeshRenderHelper(*pMesh, gw, curMtl, flags, vpt, COMP_ALL, numMtls);
					}
				} else {
		     ...
		     (selectedType == kDisplay_type_geometry))
	{
		...
		if (chShape != NULL)	{
			...
			if (!meshBox.IsEmpty())	{
				...
				Point3 h(std::max(fabs(pmin.x),fabs(pmax.x)), std::max(fabs(pmin.y),fabs(pmax.y)), std::max(fabs(pmin.z),fabs(pmax.z)) );
