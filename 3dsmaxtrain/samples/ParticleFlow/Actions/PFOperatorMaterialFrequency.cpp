...
void PFOperatorMaterialFrequency::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_MATERIALFREQUENCY_CLASS_NAME) : _T("Material Frequency");
}
...
RefResult PFOperatorMaterialFrequency::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
		...
		case REFMSG_CHANGE:
			{
				...
				if ((lastUpdateID == kMaterialFrequency_material) || (lastUpdateID == kMaterialFrequency_mtlIDOffset))
				{
					if (updateFromMXSMtl())
					{
					...
					if (lastUpdateID == kMaterialFrequency_mtlIDOffset)
					{
						if (theHold.Holding() && pblock()->GetInt(kMaterialFrequency_mtlIDOffset, 0))
						{
							if (pblock()->GetInt(kMaterialFrequency_assignID, 0) == 0)
								pblock()->SetValue(kMaterialFrequency_assignID, 0, TRUE);
						}
					}
					return REF_STOP;
				}
...
const TCHAR* PFOperatorMaterialFrequency::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_MATERIALFREQUENCY_OBJECT_NAME) : _T("Material Frequency ");
}
...
ParamID PFOperatorMaterialFrequency::assignMtlParamID(void)		const { return kMaterialFrequency_assignMaterial;}
ParamID PFOperatorMaterialFrequency::mtlParamID(void)			const { return kMaterialFrequency_material;}
ParamID PFOperatorMaterialFrequency::numSubMtlsParamID(void)	const { return -1;}
ParamID PFOperatorMaterialFrequency::randParamID(void)			const { return kMaterialFrequency_randomSeed;}
int PFOperatorMaterialFrequency::mtlIDOffset(void)	const 
{ 
	return pblock()->GetInt(kMaterialFrequency_mtlIDOffset, 0);
}
