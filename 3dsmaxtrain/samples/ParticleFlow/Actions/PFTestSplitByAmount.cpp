...
//|							From Animatable									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
void PFTestSplitByAmount::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_TEST_SPLITBYAMOUNT_CLASS_NAME) : _T("Split Amount");
}
...
//|							From ReferenceMaker								 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
RefResult PFTestSplitByAmount::NotifyRefChanged(const Interval& changeInt,RefTargetHandle hTarget,PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message) {
		case REFMSG_CHANGE:
			if ( hTarget == pblock() ) {
...
//|							From BaseObject									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
const TCHAR* PFTestSplitByAmount::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_TEST_SPLITBYAMOUNT_OBJECT_NAME) : _T("Split Amount ");
}
...
bool PFTestSplitByAmount::HasDynamicName(TSTR& nameSuffix)
{
	...
	case kSplitByAmount_testType_fraction:
		ctrl = pblock()->GetControllerByID(kSplitByAmount_fraction);
		isAnimated = (ctrl != NULL) ? (ctrl->IsAnimated() != 0) : false;
		if (isAnimated) {
			nameSuffix = GetString(IDS_FRACTION);
		} else {
			_stprintf_s(buf, 32, _T("%d%%"),int(pblock()->GetFloat(kSplitByAmount_fraction, 0)*100.0f + 0.5f));
			nameSuffix = buf;
		}
		break;
	case kSplitByAmount_testType_everyN:
		ctrl = pblock()->GetControllerByID(kSplitByAmount_everyN);
		isAnimated = (ctrl != NULL) ? (ctrl->IsAnimated() != 0) : false;
		if (isAnimated) {
			...
			_stprintf_s(buf, 32, _T(" %d"),n);
			nameSuffix += buf;
			if (n == 1) {
				nameSuffix = GetString(IDS_ALL);
			} else {
				nameSuffix = GetString(IDS_EVERY);
				_stprintf_s(buf, 32, _T(" %d"),pblock()->GetInt(kSplitByAmount_everyN, 0));
				nameSuffix += buf;
	...
	case kSplitByAmount_testType_firstN:
		...
		_stprintf_s(buf, 32, _T(" %d"),pblock()->GetInt(kSplitByAmount_firstN, 0));
		nameSuffix += buf;
		break;
							...
							Tab<float>& testTime)
{
	...
	if (!chNew->IsAllOld())
	{
		...
		float firstN = float(pblock()->GetInt(kSplitByAmount_firstN, timeStart));
		...
		// of the main particle system. This is done to make "first N"
