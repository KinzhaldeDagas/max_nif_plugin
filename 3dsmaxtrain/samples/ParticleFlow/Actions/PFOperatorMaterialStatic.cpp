...
void PFOperatorMaterialStatic::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_MATERIALSTATIC_CLASS_NAME) : _T("Material Static");
}
...
RefResult PFOperatorMaterialStatic::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
		...
		case REFMSG_CHANGE:
			{
				...
				if ((lastUpdateID == kMaterialStatic_material) || (lastUpdateID == kMaterialStatic_mtlIDOffset))
				{
					if (updateFromMXSMtl())
					{
					...
					if (lastUpdateID == kMaterialStatic_mtlIDOffset)
					{
						if (theHold.Holding() && pblock()->GetInt(kMaterialStatic_mtlIDOffset, 0))
						{
							if (pblock()->GetInt(kMaterialStatic_assignID, 0) == 0)
								pblock()->SetValue(kMaterialStatic_assignID, 0, TRUE);
						}
					}
					return REF_STOP;
				}
...
const TCHAR* PFOperatorMaterialStatic::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_MATERIALSTATIC_OBJECT_NAME) : _T("Material Static ");
}
...
ParamID PFOperatorMaterialStatic::assignMtlParamID(void)	const { return kMaterialStatic_assignMaterial;}
ParamID PFOperatorMaterialStatic::mtlParamID(void)			const { return kMaterialStatic_material;}
ParamID PFOperatorMaterialStatic::numSubMtlsParamID(void)	const { return kMaterialStatic_numSubMtls;}
ParamID PFOperatorMaterialStatic::randParamID(void)			const { return kMaterialStatic_randomSeed;}
int PFOperatorMaterialStatic::mtlIDOffset(void)			const 
{ 
	return pblock()->GetInt(kMaterialStatic_mtlIDOffset, 0);
}
