...
//|							From Animatable									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
void PFTestDuration::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_TEST_DURATION_CLASS_NAME) : _T("Age Test");
}
...
//|							From ReferenceMaker								 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
RefResult PFTestDuration::NotifyRefChanged(const Interval& changeInt,RefTargetHandle hTarget,PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message) {
		case REFMSG_CHANGE:
			if ( hTarget == pblock() ) {
...
//|							From BaseObject									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
const TCHAR* PFTestDuration::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_TEST_DURATION_OBJECT_NAME) : _T("Age Test ");
}
...
//		nameSuffix = GetString(IDS_ABSOLUTE_ABBR);
	...
	testValue /= tpf;
	TCHAR buf[32];
	_stprintf_s(buf, 32, _T("%d"), testValue);
	nameSuffix += buf;
	...
	if (variation > 0) {
		...
		nameSuffix += buf;
		_stprintf_s(buf, 32, _T("%d"), variation);
		nameSuffix += buf;
	}
	return true;
}
							...
							Tab<float>& testTime)
{
	...
	bool adjustableTiming = (pblock()->GetInt(kDuration_adjustableTiming, proceedTime) != 0);

	// acquire TestDuration private particle channel; if not present then create it		
	IParticleChannelPTVW* chTestW = NULL;
	IParticleChannelPTVR* chTestR = NULL;
	if (!adjustableTiming)
	{
		chTestW = (IParticleChannelPTVW*)chCont->EnsureInterface(PARTICLECHANNELTESTDURATIONW_INTERFACE,
																ParticleChannelPTV_Class_ID,
																true, PARTICLECHANNELTESTDURATIONR_INTERFACE,
																PARTICLECHANNELTESTDURATIONW_INTERFACE, false,
																actionNode, (Object*)this);
		chTestR = (IParticleChannelPTVR*)chCont->GetPrivateInterface(PARTICLECHANNELTESTDURATIONR_INTERFACE, (Object*)this);
		if ((chTestR == NULL) || (chTestW == NULL)) 
			return false; // can't set test value for newly entered particles
	}
	int i, count;
	...
	if (variation != 0) 
		randGen = randLinker().GetRandGenerator(pCont);
	...
	for(i=0; i<count; i++)
	{
		...
		if (adjustableTiming || isNew)
		{
			...
			if (testType == kDuration_testType_age)
				...
				if (initEventStart)
					chEventStartW->SetValue(i, curParticleValue);
		...
		if (curParticleValue > timeEnd) 
			continue; // particle has been proceeded beyond 
