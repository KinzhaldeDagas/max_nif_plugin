...
void PFOperatorSimpleOrientation::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_SIMPLEORIENTATION_CLASS_NAME) : _T("Rotation");
}
...
RefResult PFOperatorSimpleOrientation::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
const TCHAR* PFOperatorSimpleOrientation::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_SIMPLEORIENTATION_OBJECT_NAME) : _T("Rotation ");
}
