...
//|							From Animatable									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
void PFTestSplitSelected::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_TEST_SPLITSELECTED_CLASS_NAME) : _T("Split Selected");
}
...
//|							From ReferenceMaker								 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
RefResult PFTestSplitSelected::NotifyRefChanged(const Interval& changeInt,RefTargetHandle hTarget,PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message) {
		case REFMSG_CHANGE:
			if ( hTarget == pblock() ) {
...
//|							From BaseObject									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
const TCHAR* PFTestSplitSelected::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_TEST_SPLITSELECTED_OBJECT_NAME) : _T("Split Selected ");
}
