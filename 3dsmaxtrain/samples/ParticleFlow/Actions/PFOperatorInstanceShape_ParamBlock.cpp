...
(
	...
	// required param specs
		...
		// separate group members
			...
			// optional tagged param specs
					p_default,		FALSE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_GROUPMEMBERS,
			p_end,
		// separate object and children
			...
			// optional tagged param specs
					p_default,		FALSE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_CHILDREN,
			p_end,
		// separate object elements
			...
			// optional tagged param specs
					p_default,		FALSE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_ELEMENTS,
			p_end,
		// shape scale
		...
		// acquire material data
			...
			// optional tagged param specs
					p_default,		TRUE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_MATERIAL,
			p_end,
		// sub-material ID Offset
			kInstanceShape_mtlIDOffset, _T("SubMtl_ID_Offset"),
									TYPE_INT,
									P_RESET_DEFAULT,
									IDS_SUBMTLIDOFFSET,
			// optional tagged param specs
		...
		// multi-shape random order
			...
			// optional tagged param specs
					p_default,		FALSE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_ORDER,
			p_end,
		// animated shape
			...
			// optional tagged param specs
					p_default,		FALSE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_ANIMATED,
			p_end,
		// acquire current shape
			...
			// optional tagged param specs
					p_default,		FALSE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_ACQUIRE,
			p_end,
		// fast shape evaluation
			kInstanceShape_fastShapeEvaluation, _T("Fast_Shape_Evaluation"),
									TYPE_BOOL,
									P_RESET_DEFAULT,
									IDS_FASTSHAPEEVALUATION,
			// optional tagged param specs
					p_default,		TRUE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_FASTEVALUATION,
			p_end,
		// sync type
		...
		// add random offset to the synchronization
			...
			// optional tagged param specs
					p_default,		FALSE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_SYNCRANDOM,
			p_end,
		// random offset
		...
		// set scale channel
			...
			// optional tagged param specs
					p_default,		TRUE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_SETSCALE,
			p_end,
		...
		// indicator to define what nodes to report (as handles) that were used for shapes
			kInstanceShape_reportNodeHandles, _T("Report_Node_Handles"),
									TYPE_INT,
									P_RESET_DEFAULT,
									IDS_REPORTNODEHANDLES,
			// optional tagged param specs
					p_default,		kInstanceShape_reportNodeHandles_selected,
			p_end,

		// list of class IDs (as pair of DWORDs) to be reported as node handles for particle shapes
			kInstanceShape_handlesToReport,		_T("Handles_To_Report"),
					TYPE_INT_TAB,		0,	P_VARIABLE_SIZE,	IDS_HANDLESTOREPORT,//string name,			
			p_end,
											   ...
											   WPARAM wParam, LPARAM lParam )
{
	...
	INode* obj;
	TSTR name;
	BOOL separateAny, animatedShape, addRandomOffset, setScale, acquireMtl;
	...
	case WM_COMMAND:
		...
		case kInstanceShape_message_type:
			...
			separateAny = pblock->GetInt(kInstanceShape_sepGroup, t) ||
							pblock->GetInt(kInstanceShape_sepHierarchy, t) ||
							pblock->GetInt(kInstanceShape_sepElements, t);
			acquireMtl = pblock->GetInt(kInstanceShape_material, t);
			UpdateParticleShapeDlg(hWnd, op->meshData());
			UpdateRandomShapeDlg(hWnd, separateAny);
			UpdateMtlIDOffsetDlg(hWnd, acquireMtl);
			break;
...
void PFOperatorInstanceShapeDlgProc::UpdateAnimatedSahpeDlg( HWND hWnd, BOOL animatedShape, BOOL addRandomOffset )
{
	...
	EnableWindow( GetDlgItem( hWnd, IDC_FASTEVALUATION ), animatedShape );
...
void PFOperatorInstanceShapeDlgProc::UpdateMtlIDOffsetDlg( HWND hWnd, BOOL acquireMtl)
{
	EnableWindow( GetDlgItem( hWnd, IDC_TEXT_MTLIDOFFSET ), acquireMtl );
	ISpinnerControl* spin = GetISpinner( GetDlgItem( hWnd, IDC_MTLIDOFFSETSPIN ) );
	spin->Enable( acquireMtl );
	ReleaseISpinner( spin );
}
