...
void PFOperatorSimpleMapping::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_SIMPLEMAPPING_CLASS_NAME) : _T("Mapping");
}
...
RefResult PFOperatorSimpleMapping::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
const TCHAR* PFOperatorSimpleMapping::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_SIMPLEMAPPING_OBJECT_NAME) : _T("Mapping ");
}
									 ...
									 IPFIntegrator* integrator)
{
	...
	TimeValue pbSyncTime = 0, prevSyncTime = 0;

	map->SetTVFace(NULL); // no tvFace data for all particles
	UVVert mapValue = Point3::Origin;
	mapValue.x = pblock()->GetFloat(kSimpleMapping_U, 0);
	mapValue.y = pblock()->GetFloat(kSimpleMapping_V, 0);
	mapValue.z = pblock()->GetFloat(kSimpleMapping_W, 0);
	Control* ctrl = pblock()->GetControllerByID(kSimpleMapping_U);
	bool isUAnimated = (ctrl != NULL) ? (ctrl->IsAnimated() != 0) : false;
	ctrl = pblock()->GetControllerByID(kSimpleMapping_V);
	bool isVAnimated = (ctrl != NULL) ? (ctrl->IsAnimated() != 0) : false;
	ctrl = pblock()->GetControllerByID(kSimpleMapping_W);
	bool isWAnimated = (ctrl != NULL) ? (ctrl->IsAnimated() != 0) : false;
	bool isAnimated = isUAnimated || isVAnimated || isWAnimated;

	if (!isAnimated) {
		map->SetUVVert(mapValue);
	} else {
		IObject* chMapObj = chCont ? chCont->GetChannel(PARTICLECHANNELSHAPETEXTUREW_INTERFACE, nullptr) : nullptr;
		IParticleChannelExt2* chExt2 = chMapObj ? (IParticleChannelExt2*)chMapObj->GetInterface(PARTICLECHANNELEXT2_INTERFACE) : nullptr;
		if (chExt2)
			chExt2->SuspendSharedDataShrinkage();

		for(int i=0; i<count; i++) 
		{
			if (isAnimated)
			{
				syncTime = chTime->GetValue(i);
				...
				pbSyncTime = syncTime.TimeValue();
			}
		
			if ((pbSyncTime != prevSyncTime) && isAnimated)
			{
				if (isUAnimated)
					mapValue.x = pblock()->GetFloat(kSimpleMapping_U, pbSyncTime);
				if (isVAnimated)
					mapValue.y = pblock()->GetFloat(kSimpleMapping_V, pbSyncTime);
				if (isWAnimated)
					mapValue.z = pblock()->GetFloat(kSimpleMapping_W, pbSyncTime);
				prevSyncTime = pbSyncTime;
			}
			map->SetUVVert(i, mapValue);
		}

		if (chExt2)
			chExt2->ResumeSharedDataShrinkage(true);
	}
