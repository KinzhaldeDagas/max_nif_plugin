...
//|							From Animatable									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
void PFOperatorForceSpaceWarp::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_FORCESPACEWARP_CLASS_NAME) : _T("Force");
}
...
//|							From Animatable									 |
//+--------------------------------------------------------------------------+
TSTR PFOperatorForceSpaceWarp::SubAnimName(int i, bool localized)
{
	if (i == 0)
...
RefResult PFOperatorForceSpaceWarp::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
//|							From BaseObject								 |
//+--------------------------------------------------------------------------+
const TCHAR* PFOperatorForceSpaceWarp::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_FORCESPACEWARP_OBJECT_NAME) : _T("Force ");
}
...
bool PFOperatorForceSpaceWarp::updateObjectNames(ParamID pblockID)
{
