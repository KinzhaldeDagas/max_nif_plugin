...
{ 
	...
	SlateModelWrapper::SetDirection( SlateModelWrapper::Direction_Out ); 
}
...
//|							From Animatable									 |
//+--------------------------------------------------------------------------+
void PFTestCollisionSpaceWarp::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_TEST_COLLISIONSPACEWARP_CLASS_NAME) : _T("Collision");
}
...
//|							From Animatable									 |
//+--------------------------------------------------------------------------+
TSTR PFTestCollisionSpaceWarp::SubAnimName(int i, bool localized)
{
...
//|							From ReferenceMaker								 |
//+--------------------------------------------------------------------------+
RefResult PFTestCollisionSpaceWarp::NotifyRefChanged(const Interval& changeInt,RefTargetHandle hTarget,PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message) {
		case REFMSG_CHANGE:
			if (pblock() == hTarget) {
...
//|							From BaseObject									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
const TCHAR* PFTestCollisionSpaceWarp::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_TEST_COLLISIONSPACEWARP_OBJECT_NAME) : _T("Collision ");
}
...
bool PFTestCollisionSpaceWarp::Proceed(IObject* pCont, 
							...
							Tab<float>& testTime)
{
//	DebugPrint(_T("Collision Calc Start start = %d   end = %d \n"), TimeValue(timeStart), TimeValue(timeEnd));
	// grab integrator
	...
	if (testType == 4) { // will collide
		cc.CheckCollision((int)timeEnd+willCollideFrame, fakePosition, fakeSpeed, 
							(timeEnd-timeStart+PreciseTimeValue(1+willCollideFrame)).PreciseTime(), 0, &fakeHitTime, FALSE);
	} else {
		cc.CheckCollision((int)timeEnd, fakePosition, fakeSpeed, (timeEnd-timeStart).PreciseTime(), 0, &fakeHitTime, FALSE);
	}
...
//	DebugPrint(_T("Collision Calc Start EndTime = %d   Count = %d \n"), (int)timeEnd, iQuant);
...
//	DebugPrint(_T("Collision Calc End\n"));
...
//|							From PFTestCollisionSpaceWarp					 |
//+--------------------------------------------------------------------------+
void PFTestCollisionSpaceWarp::UpdatePViewUI(ParamID updateID) const
{
	for(int i=0; i<NumPViewParamMaps(); i++) {
...
bool PFTestCollisionSpaceWarp::updateObjectNames(ParamID pblockID)
{
