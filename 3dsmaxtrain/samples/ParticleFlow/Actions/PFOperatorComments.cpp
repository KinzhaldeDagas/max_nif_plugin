...
void PFOperatorComments::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_COMMENTS_CLASS_NAME) : _T("Notes");
}
...
RefResult PFOperatorComments::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
const TCHAR* PFOperatorComments::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_COMMENTS_OBJECT_NAME) : _T("Notes ");
}
