...
TCHAR* GetString(int id)
{
	...
	TCHAR* result = NULL;
	if (hInstance)
		result = LoadString(hInstance, id, bp[active], _countof(buf1)) ? bp[active] : NULL;
...
{
	DbgAssert(mesh);
	DbgAssert(std::min(vertex1,vertex2) >= 0);
	DbgAssert(std::max(vertex1,vertex2) < mesh->getNumVerts());
...
{
	double fn2 = double(faceNormal.x)*double(faceNormal.x) + double(faceNormal.y)*double(faceNormal.y) + double(faceNormal.z)*double(faceNormal.z);
	if (fn2 <= 0.0) return -1.0f;
	double pd2 = double(pointDif.x)*double(pointDif.x) + double(pointDif.y)*double(pointDif.y) + double(pointDif.z)*double(pointDif.z);
	if (pd2 <= 0.0) return 1.0f;
	double pf2 = double(pointDif.x)*double(faceNormal.x) + double(pointDif.y)*double(faceNormal.y) + double(pointDif.z)*double(faceNormal.z);
	if (pf2 < 0) return float(-(pf2*pf2)/(fn2*pd2));
...
bool ClosestPointOnMesh(const Point3& toPoint, Mesh* mesh, Point3& worldLocation, Point2& localCoords, int& faceIndex, float& dist2)
{
	...
	for(int i=0; i<numF; i++, f++) {
		...
		goodEdge[0] = ClosestPointOnEdge(toPoint, v0, v1, curRatio, curWorldPoint);
		if (goodEdge[0] == TRUE) {
			dif = toPoint - curWorldPoint;
			curDistance2 = DotProd(dif, dif);
			if (curDistance2 <= dist2) {
		...
		goodEdge[1] = ClosestPointOnEdge(toPoint, v0, v2, curRatio, curWorldPoint);
		if (goodEdge[1] == TRUE) {
			dif = toPoint - curWorldPoint;
			curDistance2 = DotProd(dif, dif);
			if (curDistance2 <= dist2) {
		...
		goodEdge[2] = ClosestPointOnEdge(toPoint, v1, v2, curRatio, curWorldPoint);
		if (goodEdge[2] == TRUE) {
			dif = toPoint - curWorldPoint;
			curDistance2 = DotProd(dif, dif);
			if (curDistance2 <= dist2) {
...
void PFProbabilityData::Init(Mesh* inMesh, Matrix3* tm, int probType, TimeValue t)
{
	...
	if (probType == kLocationType_selVertex) {
		if (inMesh->VertSel().IsEmpty()) {
			...
			const BitArray& vertSel = inMesh->VertSel();
			_selVertices().SetCount(vertSel.NumberSet());
			for(i=0, j=0; i<inMesh->getNumVerts(); i++)
				if (vertSel[i]) _selVertex(j++) = i;
	...
	if ((probabilityType() == kLocationType_edge) || (probabilityType() == kLocationType_selEdge)) {
		...
		const BitArray& inMeshEdgeSel = inMesh->EdgeSel();
		for(i=0, face=mesh->faces; i<count; i++, face++) {
			...
			for(j=0; j<3; j++) {
				...
				if (IsBadFace(mesh, i)) continue; 
				if (useSelOnly) 
					if (inMeshEdgeSel[3*i+j] == 0) continue; // the edge is not selected
		...
		const BitArray& faceSel = inMesh->FaceSel();
		for(i=0, face=mesh->faces; i<count; i++, face++) {
			_probNode(i).faceIndex = i;
			_probNode(i).accumValue = totalShare();
			_probNode(i).probValue = (faceSel[i]) ? FaceArea(mesh, i) : 0.0f;
			_totalShare() += _probNode(i).probValue;
		}
	} else { // surface
		for(i=0, face=mesh->faces; i<count; i++, face++) {
...
void PFNodeGeometry::Init(INode* inode, int animated, int subframe, int location, RandGenerator* randG)
{
	...
	for(int i=0; i<meshCount; i++) _mesh(i) = NULL;
	_tm().SetCount(frameSize() + 1);
	for(int i=0; i<tm().Count(); i++) _tm(i) = NULL;
	_probData().SetCount(meshCount);
	for(int i=0; i<meshCount; i++) _probData(i) = NULL;
}
	...
	if (abs(indexDif) > frameSize()) { // total refresh
		...
		if (indexDif > 0) {
			for(int i=0; i<indexDif; i++)
				if (tm(i) != NULL) delete _tm(i);
			for(int i=0; i<=(frameSize()-indexDif); i++)
				_tm(i) = tm(i+indexDif);
			for(int i=(frameSize()-indexDif+1); i<=frameSize(); i++)
				_tm(i) = NULL;
		} else {
			for(int i=frameSize(); i>(frameSize()+indexDif); i--)
				if (tm(i) != NULL) delete _tm(i);
			for(int i=frameSize(); i>=(-indexDif); i--)
				_tm(i) = tm(i+indexDif);
			for(int i=0; i<(-indexDif); i++)
				_tm(i) = NULL;
		}
...
Matrix3 PFObjectMaterialShadeContext::MatrixTo(RefFrame ito)
{
	...
	if (ito == REF_CAMERA)
		res = res * Matrix3(
			...
			Point3(0.f, 0.f, 1.f), 
			Point3(0.f, -1.f, 0.f),
			Point3(0.f, 0.f, 0.f)); // Compare PointFrom()
	return res;
}

Matrix3 PFObjectMaterialShadeContext::MatrixFrom (RefFrame ifrom)
{
	Matrix3 res = Matrix3::Identity;
	if (ifrom == REF_CAMERA)
		res = Matrix3(
...
bool VerifyEmittersMXSSync(IParamBlock2* pblock, ParamID kRealRefObjectsID, ParamID kMXSRefObjectsID)
{
...
bool UpdateFromRealRefObjects(IParamBlock2* pblock, ParamID kRealRefObjectsID, ParamID kMXSRefObjectsID, 
...
bool UpdateFromMXSRefObjects(IParamBlock2* pblock, ParamID kRealRefObjectsID, ParamID kMXSRefObjectsID, 
...
void MacrorecObjects(ReferenceTarget* rtarg, IParamBlock2* pblock, ParamID paramID, const TCHAR* paramName)
{
