...
void PFOperatorSimpleBirth::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_SIMPLEBIRTH_CLASS_NAME) : _T("Birth");
}
...
RefResult PFOperatorSimpleBirth::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
const TCHAR* PFOperatorSimpleBirth::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_SIMPLEBIRTH_OBJECT_NAME) : _T("Birth ");
}
...
bool PFOperatorSimpleBirth::HasDynamicName(TSTR& nameSuffix)
{
	...
	if (start == finish) {
		...
		_stprintf_s(buf, 64, _T(" %d "), start);
		nameSuffix += buf;
	} else {
		_stprintf_s(buf, 64, _T("%d-%d "), start, finish);
		nameSuffix = TSTR(buf);
	}
	nameSuffix += GetString(IDS_TOTAL_ABBR);
	_stprintf_s(buf, 64, _T(":%d"), EmitTotalAmount());
	nameSuffix += buf;
	return true;
}
									 ...
									 IPFIntegrator* integrator)
{
	...
	if (birthType == kSimpleBirth_type_rate) {
		Control* rateControl = pblock()->GetControllerByID(kSimpleBirth_rate);
		if (rateControl != NULL)
...
int PFOperatorSimpleBirth::EmitTotalAmount() const
{
	...
	case kSimpleBirth_type_rate:
	{
		...
		bool isRateAnimated = false;
		Control* ctrl = pblock()->GetControllerByID(kSimpleBirth_rate);
