...
void PFOperatorFacingShape::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_FACINGSHAPE_CLASS_NAME) : _T("Shape Facing");
}
...
RefResult PFOperatorFacingShape::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
const TCHAR* PFOperatorFacingShape::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_FACINGSHAPE_OBJECT_NAME) : _T("Shape Facing ");
}
									 ...
									 IPFIntegrator* integrator)
{
	...
	bool forceInitOrient = false;
	if (object()) 
	{
		forceInitOrient = (NULL == GetParticleChannelOrientationRInterface(pCont));
	...
	bool forceInitScale = (NULL == GetParticleChannelScaleRInterface(pCont));
...
//	}
	...
	for (int i = 0; i < amount; i++)
	{
		...
		if (inScreen) {
			...
			if (variation != 0.0f) {
				if (isNew || forceInitScale) 
					chLocalSclW->SetValue(i, GetScaleFactor(randVarGen, variation));
