...
(
	...
	// required param specs
		...
		// first/last disparity: test value differs for first particles from the last ones
			...
			// optional tagged param specs
					p_default,		FALSE,
//					p_ui,			TYPE_SINGLECHECKBOX,	IDC_FIRSTTOLAST,
			p_end,
...
//					p_ui,			TYPE_SPINNER,	EDITTYPE_POS_INT,	IDC_LASTINDEX,	IDC_LASTINDEXSPIN,	1.0f,
		...
		// adjustable timing
			kDuration_adjustableTiming,	 _T("Adjustable_Age"),
									TYPE_BOOL,
									P_RESET_DEFAULT,
									IDS_ADJUSTABLEAGE,
			// optional tagged param specs
					p_default,		FALSE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_ADJUSTABLETIMING,
			p_end,
...
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
INT_PTR TestDurationDlgProc::DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg) 
	{
	case WM_INITDIALOG:
