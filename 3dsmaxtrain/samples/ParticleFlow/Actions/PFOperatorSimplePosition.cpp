...
void PFOperatorSimplePosition::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_SIMPLEPOSITION_CLASS_NAME) : _T("Position Icon");
}
...
RefResult PFOperatorSimplePosition::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
const TCHAR* PFOperatorSimplePosition::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_SIMPLEPOSITION_OBJECT_NAME) : _T("Position Icon ");
}
...
// acquire all necessary channels, create additional if needed
	...
	Control* paramCtrl = pblock()->GetControllerByID(kSimplePosition_inheritAmount);
...
int PFOperatorSimplePosition::AllowedDistinctPoints(int locationType, int emitterType, int numDistinctPoints)
{
	...
	case kSimplePosition_locationType_vertex:
		...
		case IPFSystem::kEmitterType_rectangle:
			return std::min(4, numDistinctPoints);
		case IPFSystem::kEmitterType_circle:
			return 1;
		case IPFSystem::kEmitterType_box:
			return std::min(8, numDistinctPoints);
		case IPFSystem::kEmitterType_sphere:
			return std::min(6, numDistinctPoints);
		default:
			return 1;
		}
													...
													Tab<Point3>& distinctPoints, int emitterType)
{
	...
	for (int i = count; i < numDistinctPoints; i++)
	{
		Point3 newPoint;
		for (int j = -1; j < i;)
		{
