...
//|							From Animatable									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
void PFTestSpeedGoToTarget::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_TEST_SPEEDGOTOTARGET_CLASS_NAME) : _T("Find Target");
}
...
//|							From ReferenceMaker								 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
RefResult PFTestSpeedGoToTarget::NotifyRefChanged(const Interval& changeInt,RefTargetHandle hTarget,PartID& partID, RefMessage message, BOOL propagate)
{
...
int PFTestSpeedGoToTarget::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags)
{
	...
	MeshRenderHelper(*GetIconShape(t), gw, curMtl, flags, vpt, COMP_ALL, numMtls);
	MeshRenderHelper(*GetIconMesh(t), gw, curMtl, flags, vpt, COMP_ALL, numMtls);
...
//|							From BaseObject									 |
//+--------------------------------------------------------------------------+
const TCHAR* PFTestSpeedGoToTarget::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_TEST_SPEEDGOTOTARGET_OBJECT_NAME) : _T("Find Target ");
}
							...
							Tab<float>& testTime)
{
	...
	Control* iconSizeCtrl = pblock()->GetControllerByID(kSpeedGoToTarget_iconSize);
	if (iconSizeCtrl != NULL)
		iconSizeAnimated = (iconSizeCtrl->IsAnimated() != 0);
	bool easeInAnimated = false;
	Control* easeInCtrl = pblock()->GetControllerByID(kSpeedGoToTarget_easeIn);
	if (easeInCtrl != NULL)
	...
	Control* dockingDistanceCtrl = pblock()->GetControllerByID(kSpeedGoToTarget_dockingDistance);
...
bool PFTestSpeedGoToTarget::updateObjectNames(ParamID pblockID)
{
	if (pblock() == NULL) return false;
