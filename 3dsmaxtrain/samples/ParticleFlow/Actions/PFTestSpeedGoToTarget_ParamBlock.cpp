...
(
	...
	// required param specs
		...
		// use cruise speed
			...
			// optional tagged param specs
					p_default,		TRUE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_USECRUISESPEED,
			p_end,
		// speed value
			...
			// optional tagged param specs
					...
					p_nonLocalizedName, _T("Cruise Speed"),
			p_end,
		// speed variation
			...
			// optional tagged param specs
					...
					p_nonLocalizedName, _T("Cruise Speed Variation"),
			p_end,
		// acceleration maximum
			...
			// optional tagged param specs
					...
					p_nonLocalizedName, _T("Acceleration Limit"),
			p_end,
		// sync type
		...
		// test distance
			...
			// optional tagged param specs
					...
					p_nonLocalizedName, _T("Test Distance"),
			p_end,
		// timing type
		...
		// subframe sampling
			...
			// optional tagged param specs
					p_default,		TRUE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_SUBFRAME,
			p_end,
		// use docking speed
			...
			// optional tagged param specs
					p_default,		FALSE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_USEDOCKINGSPEED,
			p_end,
		// docking speed
			...
			// optional tagged param specs
					...
					p_nonLocalizedName, _T("Docking Speed"),
			p_end,
		// docking easy in
			...
			// optional tagged param specs
					...
					p_nonLocalizedName, _T("Docking Speed Variation"),
			p_end,
		...
		// follow target animation
			...
			// optional tagged param specs
					p_default,		FALSE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_FOLLOWTARGET,
			p_end,
		// follow target animation
			...
			// optional tagged param specs
					p_default,		TRUE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_LOCKTARGET,
			p_end,
		// point type
		...
		// docking distance
			...
			// optional tagged param specs
					...
					p_nonLocalizedName, _T("Docking Distance"),
			p_end,
		// color scheme
			...
			// optional tagged param specs
					p_default,		TRUE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_COLORCOORDINATED,
			p_end,
...
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
INT_PTR TestSpeedGoToTargetDlgProc::DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg) 
	{
	case WM_INITDIALOG:
...
void TestSpeedGoToTargetDlgProc::UpdateDlg( HWND hWnd, IParamBlock2* pblock, TimeValue time )
{
	...
	if (useBySpeed  && useCruiseSpeed) {
		...
		if (speedVar == 0.0f) {
			Control* ctrl = pblock->GetControllerByID(kSpeedGoToTarget_cruiseSpeedVariation);
	...
	if (!useRandom && useByTime) {
		...
		if (timeVar == 0) {
			Control* ctrl = pblock->GetControllerByID(kSpeedGoToTarget_timeVariation);
			if (ctrl != NULL) useRandom = (ctrl->IsAnimated() != 0);
		} else {
			useRandom = true;
		}
		if (!useRandom && useDockingSpeed) {
			...
			if (speedVar == 0.0f) {
				Control* ctrl = pblock->GetControllerByID(kSpeedGoToTarget_dockingSpeedVariation);
				if (ctrl != NULL) useRandom = (ctrl->IsAnimated() != 0);
			} else {
				useRandom = true;
			}
