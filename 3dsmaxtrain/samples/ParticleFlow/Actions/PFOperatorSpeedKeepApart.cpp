...
void PFOperatorSpeedKeepApart::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_SPEEDKEEPAPART_CLASS_NAME) : _T("Keep Apart");
}
...
//|							From Animatable									 |
//+--------------------------------------------------------------------------+
TSTR PFOperatorSpeedKeepApart::SubAnimName(int i, bool localized)
{
	if (i == 0)
		return localized ? GetString(IDS_PARAMETERS) : _T("Parameters");
	//	if (i == 1) return localized ? GetString(IDS_SCRIPTWIRING) : _T("Script Wiring");
	return localized ? GetString(IDS_UNDEFINED) : _T("<Undefined>");
}
...
//|							From ReferenceMaker								 |
//+--------------------------------------------------------------------------+
void PFOperatorSpeedKeepApart::UpdatePViewUI(IParamBlock2* pblock, ParamID updateID)
{
...
RefResult PFOperatorSpeedKeepApart::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
const TCHAR* PFOperatorSpeedKeepApart::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_SPEEDKEEPAPART_OBJECT_NAME) : _T("Keep Apart ");
}
...
void PFOperatorSpeedKeepApart::macrorecGroups()
{
	...
	selItemNames = selItemNames + _T(")");
	macroRec->EmitScript();
	macroRec->SetProperty(this, _T("Selected_Events"), mr_name, selItemNames.data());
}
...
void PFOperatorSpeedKeepApart::macrorecSystems()
{
	...
	selItemNames = selItemNames + _T(")");
	macroRec->EmitScript();
	macroRec->SetProperty(this, _T("Selected_Systems"), mr_name, selItemNames.data());
}
