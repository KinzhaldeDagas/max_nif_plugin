	...
	// required param specs
		// test true for particles when transition period ends
			...
			// optional tagged param specs
					p_default,		TRUE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_HASTEST,
			p_end,
		// sync by
		...
		// match initial spin
			...
			// optional tagged param specs
					p_default,		TRUE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_MATCHSPIN,
			p_end,
		// spin rate
		...
		// stop spinning particles when they reach the target rotation
			...
			// optional tagged param specs
					p_default,		TRUE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_STOPSPIN,
			p_end,
		// random seed
...
void TestGoToRotationDlgProc::UpdateSyncDlg( HWND hWnd, int syncBy )
{
	...
	TCHAR label[32];
	_stprintf_s(label, 32, _T("%s:"),GetString(ids));
