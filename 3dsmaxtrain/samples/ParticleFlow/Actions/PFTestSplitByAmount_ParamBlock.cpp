	...
	// required param specs
		...
		// every n-th particle
			...
			// optional tagged param specs
					...
					p_nonLocalizedName, _T("Every Nth"),
			p_end,
		// first n particle
		...
		// per emission source
			...
			// optional tagged param specs
					p_default,		FALSE,
					p_ui,			TYPE_SINGLECHECKBOX,	IDC_PERSOURCE,
			p_end,
		// random seed
...
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
INT_PTR TestSplitByAmountDlgProc::DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	IParamBlock2* pblock;
	switch (msg) 
	{
