...
{
	bool res(true);
	if (!verifyMtlsSync(mtl, _material()))
	{
		if (mtl != NULL)
		{
		...
		Mtl* refMtl = mtl;
		if ((idOffset > 0) && (mtl != NULL))
		{
			MultiMtl* multi = NewDefaultMultiMtl();
			if (multi == NULL)
				return false;
			int numSubs = mtl->NumSubMtls();
			multi->SetNumSubMtls((numSubs ? numSubs : 1) + idOffset);
			TSTR emptyMtlName = _T("");
			for(int i=0; i<idOffset; ++i)
			{
				multi->SetSubMtlAndName(i, NULL, emptyMtlName);
			}
			if (numSubs == 0)
			{
				multi->SetSubMtlAndName(idOffset, mtl, mtl->GetName());
			}
			else
			{
				for(int i=0; i<numSubs; ++i)
					multi->SetSubMtlAndName(i+idOffset, mtl->GetSubMtl(i), mtl->GetSubMtlSlotName(i, true));
			}
			refMtl = multi;
		}
		...
		if (refMtl == NULL) {
			...
			res = ReplaceReference(kMaterial_reference_material, refMtl) == REF_SUCCEED;
		}
	}
	return res;
}
...
bool PFOperatorMaterial::verifyMtlsMXSSync()
{
	...
	return verifyMtlsSync(pblockMtl, refMtl);
}

//+--------------------------------------------------------------------------+
//|							From PFOperatorMaterial							 |
//+--------------------------------------------------------------------------+
bool PFOperatorMaterial::verifyMtlsSync(Mtl* pblockMtl, Mtl* refMtl)
{
	...
	if (!refMtl->IsMultiMtl())
		return false;

	int numSubs = pblockMtl->NumSubMtls();
	if (numSubs == 0)
	{
		return (pblockMtl == refMtl->GetSubMtl(idOffset));
	}

	for(int i=0; i<numSubs; ++i)
	{
		if (pblockMtl->GetSubMtl(i) != refMtl->GetSubMtl(i+idOffset))
			return false;
	}
	return true;
}
