...
void PFOperatorSimpleSpeed::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_SIMPLESPEED_CLASS_NAME) : _T("Speed");
}
...
RefResult PFOperatorSimpleSpeed::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
const TCHAR* PFOperatorSimpleSpeed::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_SIMPLESPEED_OBJECT_NAME) : _T("Speed ");
}
