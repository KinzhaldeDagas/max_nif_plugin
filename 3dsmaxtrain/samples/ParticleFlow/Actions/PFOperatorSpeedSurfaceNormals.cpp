...
void PFOperatorSpeedSurfaceNormals::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_SPEEDSURFACENORMALS_CLASS_NAME) : _T("Speed By Surface");
}
...
RefResult PFOperatorSpeedSurfaceNormals::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
const TCHAR* PFOperatorSpeedSurfaceNormals::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_SPEEDSURFACENORMALS_OBJECT_NAME) : _T("Speed By Surface ");
}
...
bool PFOperatorSpeedSurfaceNormals::Init(IObject* pCont, Object* pSystem, INode* node, Tab<Object*>& actions, Tab<INode*>& actionNodes)
{
	...
	Tab<INode*> nodeList;
	for(int i=0; i<pblock()->Count(kSpeedBySurface_objects); i++)
...
bool PFOperatorSpeedSurfaceNormals::updateObjectNames(ParamID pblockID)
{
