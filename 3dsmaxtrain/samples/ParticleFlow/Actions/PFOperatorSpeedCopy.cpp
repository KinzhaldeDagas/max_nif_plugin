...
void PFOperatorSpeedCopy::GetClassName(MSTR& s, bool localized) const
{
   s = localized ? GetString(IDS_OPERATOR_SPEEDCOPY_CLASS_NAME) : _T("SpeedByIcon");
}
...
RefResult PFOperatorSpeedCopy::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
//|                     From BaseObject                            |
//+--------------------------------------------------------------------------+
const TCHAR* PFOperatorSpeedCopy::GetObjectName(bool localized) const
{
   return localized ? GetString(IDS_OPERATOR_SPEEDCOPY_OBJECT_NAME) : _T("Speed By Icon ");
}
...
void BuildLogoSpeedCopy( PolyShape* shape)
{
   ...
   static int lineNum[] = { 2, 5, 7, 10, 12, 15, 17, 20, num_points, 0 };
   ...
   shape->NewShape();
   int pointIndex = 0;
   for (int lineIndex = 0; lineIndex < logo_lines; ++lineIndex )
   {
      PolyLine* line = shape->NewLine();
      if ( line == NULL ) return;

      for( ; pointIndex < lineNum[lineIndex]; ++pointIndex )
