	...
	// required param specs
		...
		// spin axis x
			...
			// optional tagged param specs
					...
					p_nonLocalizedName, _T("Spin X Axis"),
			p_end,
		// spin axis y
			...
			// optional tagged param specs
					...
					p_nonLocalizedName, _T("Spin Y Axis"),
			p_end,
		// spin axis z
			...
			// optional tagged param specs
					...
					p_nonLocalizedName, _T("Spin Z Axis"),
			p_end,
		// divergence
...
INT_PTR PFOperatorSimpleSpinDlgProc::DlgProc(TimeValue t, IParamMap2* map, HWND hWnd,
	...
	if(map && hWnd) {
		...
		if(ppb) {
			...
			if (!bRandom) { // maybe variation is animated
				varAnimated = false;
				Control* ctrl = ppb->GetControllerByID(kSimpleSpin_variation);
				if (ctrl != NULL)
			...
			if (!bRandom) { // maybe divergence is non-zero
				if ((iDir != kSS_Rand_3D) && (iDir != kSS_Speed_Space_Follow)) {
					...
					if (!bRandom) { // maybe divergence is animated
						divAnimated = false;
						Control* ctrl = ppb->GetControllerByID(kSimpleSpin_divergence);
						if (ctrl != NULL)
