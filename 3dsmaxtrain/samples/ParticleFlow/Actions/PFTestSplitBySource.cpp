...
//|							From Animatable									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
void PFTestSplitBySource::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_TEST_SPLITBYSOURCE_CLASS_NAME) : _T("Split Source");
}
...
//|							From ReferenceMaker								 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
RefResult PFTestSplitBySource::NotifyRefChanged(const Interval& changeInt,RefTargetHandle hTarget,PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message) {
		case REFMSG_CHANGE:
			if ( hTarget == pblock() ) {
...
//|							From BaseObject									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
const TCHAR* PFTestSplitBySource::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_TEST_SPLITBYSOURCE_OBJECT_NAME) : _T("Split Source ");
}
...
void PFTestSplitBySource::macrorecSources()
{
	...
	selItemNames = selItemNames + _T(")");
	macroRec->EmitScript();
	macroRec->SetProperty(this, _T("Selected_Sources"), mr_name, selItemNames.data());
}
