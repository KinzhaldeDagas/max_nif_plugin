...
//|							From Animatable									 |
//+--------------------------------------------------------------------------+
void PFTestSpawn::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_TEST_SPAWN_CLASS_NAME) : _T("Spawn");
}
...
//|							From Animatable									 |
//+--------------------------------------------------------------------------+
TSTR PFTestSpawn::SubAnimName(int i, bool localized)
{
...
//|							From ReferenceMaker								 |
//+--------------------------------------------------------------------------+
RefResult PFTestSpawn::NotifyRefChanged(const Interval& changeInt,RefTargetHandle hTarget,PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message) {
		case REFMSG_CHANGE:
			if ( hTarget == pblock() ) {
...
//|							From BaseObject									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
const TCHAR* PFTestSpawn::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_TEST_SPAWN_OBJECT_NAME) : _T("Spawn ");
}
							...
							Tab<float>& testTime)
{
	...
	int paramAnim = IsControlAnimated(pblock()->GetControllerByID(kSpawn_numOffsprings));
	paramAnim = paramAnim || IsControlAnimated(pblock()->GetControllerByID(kSpawn_numVariation));
	int paramFrameAnim = IsControlAnimated(pblock()->GetControllerByID(kSpawn_spawnRate));
	int paramDistAnim = IsControlAnimated(pblock()->GetControllerByID(kSpawn_stepSize));
	...
	int isSpawnAbleAnimated = IsControlAnimated(pblock()->GetControllerByID(kSpawn_spawnAble));
	if ((!chNew->IsAllOld()) 
...
calcAccumDef:
				while (lastTime < timeEnd) {
					if (accumDef + lastRate >= 1.0f) {
						...
						lastTime = spawnTime;
						accumDef = 1.0f;
						break; // chengqing : break the dead loop...to fix DID 691395 : Hang with Particle Flow.
					} else {
						accumDef += lastRate;
						syncLastTime += 1;
						lastTime += 1;
...
//|							From PFTestSpawn								 |
//+--------------------------------------------------------------------------+
void PFTestSpawn::UpdatePViewUI(ParamID updateID) const
{
	for(int i=0; i<NumPViewParamMaps(); i++) {
