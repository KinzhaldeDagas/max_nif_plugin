...
PFNodeGeometry* PFOperatorPositionOnObjectLocalData::GetNodeGeometry(int index)
{
	int counter = 0;
	for(int i=0; i<geomData().Count(); i++)
		if (geomData(i) != NULL) {
...
#pragma message( "Unicode watch - LocationPoint being written via void* - extremely high risk - LocationPoint contains INode*  - 32/64 bit build incompatibility - File: " __FILE__ )
		if ((res = isave->WriteVoid(points().Addr(0), num*sizeof(LocationPoint), &nb)) != IO_OK) return res;
		isave->EndChunk();
	}

	isave->BeginChunk(IPFActionState::kChunkData3);
	int vCount = vertexCounter();
	if ((res = isave->Write(&vCount, sizeof(int), &nb)) != IO_OK) return res;
	isave->EndChunk();

	isave->BeginChunk(IPFActionState::kChunkData4);
	int mCount = meshCounter();
	if ((res = isave->Write(&mCount, sizeof(int), &nb)) != IO_OK) return res;
	isave->EndChunk();
...
IOResult PFOperatorPositionOnObjectState::Load(ILoad* iload)
{
	...
	while (IO_OK==(res=iload->OpenChunk()))
	{
		...
		case IPFActionState::kChunkData2:
			res=iload->ReadVoid(_points().Addr(0), sizeof(LocationPoint)*points().Count(), &nb);
			break;

		case IPFActionState::kChunkData3:
			vCount = 0;;
			res=iload->Read(&vCount, sizeof(int), &nb);
			_vertexCounter() = vCount;
			break;

		case IPFActionState::kChunkData4:
			mCount = 0;;
			res=iload->Read(&mCount, sizeof(int), &nb);
			_meshCounter() = mCount;
			break;
		}
		iload->CloseChunk();
...
void PFOperatorPositionOnObject::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_POSITIONONOBJECT_CLASS_NAME) : _T("Position Object");
}
...
RefResult PFOperatorPositionOnObject::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
const TCHAR* PFOperatorPositionOnObject::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_POSITIONONOBJECT_OBJECT_NAME) : _T("Position Object ");
}
...
IObject* PFOperatorPositionOnObject::GetCurrentState(IObject* pContainer)
{
	...
	actionState->_vertexCounter() = vertexCounter();
	actionState->_meshCounter() = meshCounter();
	return actionState;
}
...
void PFOperatorPositionOnObject::SetCurrentState(IObject* aSt, IObject* pContainer)
{
	...
	_vertexCounter() = actionState->vertexCounter();
	_meshCounter() = actionState->meshCounter();
}
...
bool PFOperatorPositionOnObject::HasDynamicName(TSTR& nameSuffix)
{
	...
	if (count > 0) {
		...
		if (count > 1) {
			TCHAR buf[32];
			_stprintf_s(buf, 32, _T(" +%d"), count-1);
			nameSuffix += buf;
		}
	} else {
...
// acquire all necessary channels, create additional if needed
	...
	if (locationType == kPositionOnObject_locationType_vertexAll) 
	{
		distinctOnly = false;
		useNonUniform = false;
	}
	...
	Control* paramCtrl = pblock()->GetControllerByID(kPositionOnObject_inheritAmount);
	...
	paramCtrl = pblock()->GetControllerByID(kPositionOnObject_inheritAmountVar);
	...
	for(int i=0; i<amount; i++)
	{
		...
		if (lockOnEmitter || isNew)
		{
			...
			if (isNew && (!lockOnEmitter || initLocalPos))
			{
				...
				if (locationType == kPositionOnObject_locationType_vertexAll) {
					nodeGeom = localData->GetNodeGeometry(meshCounter());
					if (nodeGeom == NULL) continue;
					Mesh* placeMesh = nodeGeom->GetMesh(time);
					if (placeMesh == NULL) continue;
					if (placeMesh->numVerts <= vertexCounter()) continue;
					if (useOffset) {
					...
					locationPoint.location.x = 0.0f;
					locationPoint.location.y = 0.0f;
					locationPoint.location.z = surfaceOffset;
					locationPoint.init = true;
					locationPoint.node = nodeGeom->GetNode();
					locationPoint.pointIndex = vertexCounter() + placeMesh->numFaces;
					nodeGeom->GetLocationPoint(time, worldLocation, locationPoint.location, locationPoint.pointIndex);
					_vertexCounter() += 1;
					if (vertexCounter() >= placeMesh->numVerts) {
						_vertexCounter() = 0;
						_meshCounter() += 1;
						PFNodeGeometry* nextNodeGeom = localData->GetNodeGeometry(meshCounter());
						if (nextNodeGeom == NULL) _meshCounter() = 0;
					}
					validPointGenerated = true;
				}
				if (!validPointGenerated) {
					...
					for(int j=0; j<attemptsMax; j++)
					{
						...
						case kPositionOnObject_locationType_vertexAll:
							...
							if (inode != NULL) {
								...
								if ((mtl != NULL) && (mesh != NULL)) {
									...
									case kPositionOnObject_densityType_grayscaleOpacity:
										densityValue = std::min(sc.out.c.r+sc.out.c.g+sc.out.c.b,
...
bool PFOperatorPositionOnObject::updateObjectNames(ParamID pblockID)
{
