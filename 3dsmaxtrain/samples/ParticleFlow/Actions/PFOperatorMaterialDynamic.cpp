...
void PFOperatorMaterialDynamic::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_MATERIALDYNAMIC_CLASS_NAME) : _T("Material Dynamic");
}
...
RefResult PFOperatorMaterialDynamic::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
		...
		case REFMSG_CHANGE:
			{
				...
				if ((lastUpdateID == kMaterialDynamic_material) || (lastUpdateID == kMaterialDynamic_mtlIDOffset)) 
				{
					if (updateFromMXSMtl()) 
					{
					...
					if (lastUpdateID == kMaterialDynamic_mtlIDOffset)
					{
						if (theHold.Holding() && pblock()->GetInt(kMaterialDynamic_mtlIDOffset, 0))
						{
							if (pblock()->GetInt(kMaterialDynamic_assignID, 0) == 0)
							pblock()->SetValue(kMaterialDynamic_assignID, 0, TRUE);
						}
					}
					return REF_STOP;
				}
...
const TCHAR* PFOperatorMaterialDynamic::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_MATERIALDYNAMIC_OBJECT_NAME) : _T("Material Dynamic ");
}
...
bool PFOperatorMaterialDynamic::HasDynamicName(TSTR& nameSuffix)
{
	...
	Mtl* mtl = pblock()->GetMtl(mtlParamID(), 0);
	if (mtl == NULL) {
		nameSuffix = GetString(IDS_NONE);
	} else {
		nameSuffix = mtl->GetName();
		int idOffset = mtlIDOffset();
		if (idOffset > 0)
		{
			TCHAR buf[64];
			_stprintf_s(buf, 64, _T(" >> %d "), idOffset);
			nameSuffix += buf;
		}
	}
	return true;
									 ...
									 IPFIntegrator* integrator)
{
	...
	Control* rateCtrl = pblock()->GetControllerByID(kMaterialDynamic_rate);
	...
	int subMtlIDOffset = mtlIDOffset();
	...
	case kMaterialDynamic_type_materialID:
		for(i=0; i<count; i++) {
			...
			if (mtlID < 0) mtlID = 0;
			chMtlIDW->SetValue(i, mtlID + subMtlIDOffset);
		}
		break;
...
ParamID PFOperatorMaterialDynamic::assignMtlParamID(void)	const { return kMaterialDynamic_assignMaterial;}
ParamID PFOperatorMaterialDynamic::mtlParamID(void)			const { return kMaterialDynamic_material;}
ParamID PFOperatorMaterialDynamic::numSubMtlsParamID(void)	const { return kMaterialDynamic_numSubMtls;}
ParamID PFOperatorMaterialDynamic::randParamID(void)		const { return kMaterialDynamic_randomSeed;}
int PFOperatorMaterialDynamic::mtlIDOffset(void) const 
{ 
	return (pblock()->GetInt(kMaterialDynamic_type, 0) == kMaterialDynamic_type_particleID)
				? 0 : pblock()->GetInt(kMaterialDynamic_mtlIDOffset, 0);
}
