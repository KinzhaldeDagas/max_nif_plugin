...
void PFOperatorExit::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_EXIT_CLASS_NAME) : _T("DeleteParticles");
}
...
RefResult PFOperatorExit::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
const TCHAR* PFOperatorExit::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_EXIT_OBJECT_NAME) : _T("Delete ");
}
...
bool PFOperatorExit::HasDynamicName(TSTR& nameSuffix)
{
	...
	case kExit_type_byAge:
		...
		nameSuffix += _T(" ");
		{
			...
			TCHAR buf[32];
			_stprintf_s(buf, 32, _T("%d"), lifespan/tpf);
			nameSuffix += buf;
									 ...
									 IPFIntegrator* integrator)
{
	...
	IParticleChannelBoolR* chSelect = NULL;
