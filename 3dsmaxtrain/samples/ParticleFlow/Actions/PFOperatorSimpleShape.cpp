...
void PFOperatorSimpleShape::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_SIMPLESHAPE_CLASS_NAME) : _T("ShapeStandard");
}
...
RefResult PFOperatorSimpleShape::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
const TCHAR* PFOperatorSimpleShape::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_SIMPLESHAPE_OBJECT_NAME) : _T("Shape ");
}
									 ...
									 IPFIntegrator* integrator)
{
	...
	if (chNew->IsAllOld() && !forceInitShape && !forceInitScale) 
		return true;

	bool isScaleAnimated = false;
	float scaleAtZero = 1.0f;
	if (useScale) {
		Control* scaleCtrl = pblock()->GetControllerByID(kSimpleShape_scale);
	...
	if (chNew->IsAllNew() || forceInitShape) {
	...
	if (useScale) {
		...
		if (chNew->IsAllNew() || forceInitScale) {
			if (isScaleAnimated) {
				for(int i=0; i<amount; i++) {
...
void PFOperatorSimpleShape::BuildMeshPyramid(float size)
{
	...
	float right_x  = 0.942809f * size;
	float left_x   = -0.471405f * size;
	float front_y  = 0.816497f*size;
	float back_y   = -0.816497f*size;
	float bottom_z = -0.333333f * size;
