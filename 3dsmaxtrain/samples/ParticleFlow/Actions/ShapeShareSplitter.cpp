							 ...
							 const int numMaterials)
{
	...
	bool isNotShared = !chShape->IsShared() || (chShape->GetValueCount() >= m_count);
	// is not shared by material properties
	isNotShared = isNotShared || materialNotShared(chMtlIndex, numMaterials);
	// is not shared by mapping;
...
bool ShapeShareSplitter::materialNotShared(const IParticleChannelIntR* chMtlIndex, const int numMaterials) const
{
	...
	{
		int mtlIndex = chMtlIndex->GetValue(i) % numMaterials;
