...
//|							From Animatable									 |
//+--------------------------------------------------------------------------+
void PFTestSpawnOnCollision::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_TEST_SPAWNONCOLLISION_CLASS_NAME) : _T("Collision Spawn");
}
...
//|							From Animatable									 |
//+--------------------------------------------------------------------------+
TSTR PFTestSpawnOnCollision::SubAnimName(int i, bool localized)
{
...
//|							From ReferenceMaker								 |
//+--------------------------------------------------------------------------+
RefResult PFTestSpawnOnCollision::NotifyRefChanged(const Interval& changeInt,RefTargetHandle hTarget,PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message) {
		case REFMSG_CHANGE:
			if ( hTarget == pblock() ) 
...
//|							From BaseObject									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
const TCHAR* PFTestSpawnOnCollision::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_TEST_SPAWNONCOLLISION_OBJECT_NAME) : _T("Collision Spawn ");
}
...
bool PFTestSpawnOnCollision::updateObjectNames(ParamID pblockID)
{
...
void PFTestSpawnOnCollision::UpdatePViewUI(ParamID updateID) const
{
	for(int i=0; i<NumPViewParamMaps(); i++) {
