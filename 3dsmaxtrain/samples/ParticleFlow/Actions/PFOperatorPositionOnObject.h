...
//	int NodeGeometryCount() const;
	...
	PFNodeGeometry* GetNodeGeometry(int index);
...
protected:
	friend class PFOperatorPositionOnObject;
		...
		const Tab<LocationPoint>& points()	const	{ return m_points; }
		int	  vertexCounter()		const { return m_vertexCounter; }
		int	  meshCounter()			const { return m_meshCounter; }

		// access to class members
		ULONG&				_actionHandle()		{ return m_actionHandle; }
		RandGenerator*		_randGen()			{ return &m_randGen; }
		Tab<LocationPoint>&	_points()			{ return m_points; }
		int&				_vertexCounter()	{ return m_vertexCounter; }
		int&				_meshCounter()		{ return m_meshCounter; }
...
public:
	...
	const TCHAR *dialogTitle()	{ return GetString(IDS_SELECTEMITTEROBJECTS); }
...
private:
	...
	int m_vertexCounter, m_meshCounter;
};
