...
void PFOperatorSimpleScale::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_SIMPLESCALE_CLASS_NAME) : _T("ScaleParticles");
}
...
RefResult PFOperatorSimpleScale::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget,
...
const TCHAR* PFOperatorSimpleScale::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_SIMPLESCALE_OBJECT_NAME) : _T("Scale ");
}
									 ...
									 Object* pSystem, INode* pNode, INode* actionNode, IPFIntegrator* integrator)
{
	...
	Control* paramCtrl = pblock()->GetControllerByID(kSimpleScale_XScaleFactor);
	...
	paramCtrl = pblock()->GetControllerByID(kSimpleScale_YScaleFactor);
	...
	paramCtrl = pblock()->GetControllerByID(kSimpleScale_ZScaleFactor);
	...
	paramCtrl = pblock()->GetControllerByID(kSimpleScale_XScaleVariation);
	...
	paramCtrl = pblock()->GetControllerByID(kSimpleScale_YScaleVariation);
	...
	paramCtrl = pblock()->GetControllerByID(kSimpleScale_ZScaleVariation);
