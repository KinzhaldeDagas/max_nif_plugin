...
//|							From Animatable									 |
//+--------------------------------------------------------------------------+
void PFOperatorRender::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_OPERATOR_RENDER_CLASS_NAME) : _T("RenderParticles");
}
...
//|							From Animatable									 |
//+--------------------------------------------------------------------------+
TSTR PFOperatorRender::SubAnimName(int i, bool localized)
{
...
//|							From ReferenceMaker								 |
//+--------------------------------------------------------------------------+
RefResult PFOperatorRender::NotifyRefChanged(const Interval& changeInt,RefTargetHandle hTarget,PartID& partID, RefMessage message, BOOL propagate)
{
	switch (message) {
		case REFMSG_CHANGE:
			if (pblock() == hTarget) {
...
//|							From BaseObject									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
const TCHAR* PFOperatorRender::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_OPERATOR_RENDER_OBJECT_NAME) : _T("Render ");
}
...
Mesh* PFOperatorRender::GetRenderMesh(IObject* pCont, TimeValue time, Object* pSystem, INode *inode, View& view, BOOL& needDelete)
{
	...
	IParticleChannelBoolR* chVisRnR = GetParticleChannelVisibilityRNRInterface(pCont);
	...
	for(i=0; i<count; i++)
	{
		...
		if (chVisOpR && chVisOpR->GetValue(i))
			continue;
		if (chVisRnR && chVisRnR->GetValue(i))
			continue;
	...
	for(i=0; i<count; i++)
	{
		...
		int mapCount = pMesh->getNumMaps();
		for(mp=0; mp<mapCount; mp++) {
			...
			if (tvertOffset[mp] == 0) { // the map channel needs expansion
				...
				for (j = 0; j < numMapVerts; j++) { // zero out verts
					renderMesh->setMapVert(mp, j, Point3::Origin);	
				}
				renderMesh->setNumMapFaces(mp, faceNum);
				for(j=0; j<faceNum; j++){  // zero out vertex indices
...
//|							From PFOperatorRender							 |
//+--------------------------------------------------------------------------+
void PFOperatorRender::UpdatePViewUI(ParamID updateID) const
{
	for(int i=0; i<NumPViewParamMaps(); i++) {
