...
//|							From Animatable									 |
//+--------------------------------------------------------------------------+
void PFTestGoToRotation::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? GetString(IDS_TEST_GOTOROTATION_CLASS_NAME) : _T("Go To Rotation");
}
...
//|							From Animatable									 |
//+--------------------------------------------------------------------------+
TSTR PFTestGoToRotation::SubAnimName(int i, bool localized)
{
...
//|							From ReferenceMaker								 |
//+--------------------------------------------------------------------------+
RefResult PFTestGoToRotation::NotifyRefChanged(const Interval& changeInt,RefTargetHandle hTarget,PartID& partID, RefMessage message, BOOL propagate)
{
	IParamMap2* map;
...
//|							From BaseObject									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
const TCHAR* PFTestGoToRotation::GetObjectName(bool localized) const
{
	return localized ? GetString(IDS_TEST_GOTOROTATION_OBJECT_NAME) : _T("Go To Rotation ");
}
