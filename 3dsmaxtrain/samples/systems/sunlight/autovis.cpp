...
{
	return mdays[leap][month];
}

int GetMonthLengthInDays(const int month, const bool is_leap_year)
{
    if(DbgVerify((month >= 1) && (month <= 12)))
    {
        const int leap_index = is_leap_year ? 1 : 0;
        return (mdays[leap_index][month] - mdays[leap_index][month - 1]);
    }
    else
    {
        return 0;
...
int GetMonthLengthInDays(const int month, const int year)
{
    return GetMonthLengthInDays(month, isleap(year));
}

/***************************************************************************/
double
gregorian2julian(int month, int day, int year)
{
	long ydays, ydays1;
...
int gregorian2julian_int(int month, int day, int year)
{
    // Remove the 0.5 fraction when converting to an integer
    return int(gregorian2julian(month, day, year) - 0.5);
}

void julian2gregorian_int(int jdate, int* month, int* day, int* year)
{
    // Add back the 0.5 removed by gregorian2julian_int()
    return julian2gregorian(jdate + 0.5, month, day, year);
}
...
sunLocator(double latitude, double longitude, long month, long day,
			  long year, long hour, long mins, long sec,
...
JulianCalendar fusetime(SYSTEMTIME& tstruct){
	JulianCalendar result;
...
void fracturetime(JulianCalendar time, SYSTEMTIME& t){
	...
	DbgAssert(fabs(wholepart + decpart - days) < (.001 / (24.0 * 60.0 * 60.0)));
...
uTimevect decday_to_hms( double fod){
	...
	DbgAssert(fod>=0.0 && fod<1.0); // 0.0<= fod <1.0 fraction of day
...
uTimevect secs_to_hms( int secs){
	...
	//	DbgAssert(secs >= 0 && secs <= SECS_PER_DAY); 
	result.i =  secs / 3600;
