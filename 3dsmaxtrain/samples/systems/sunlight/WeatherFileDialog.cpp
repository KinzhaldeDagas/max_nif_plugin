...
{
	m_title = MaxSDK::GetResourceStringAsMSTR(IDS_WEATHER_FILE);
	bmmHistoryList::m_initDir = _T("");
	bmmHistoryList::LoadDefaults();
	
	if (bmmHistoryList::DefaultPath()[0])
		initDir = bmmHistoryList::DefaultPath();
	else 
...
INT_PTR WINAPI WeatherHookProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	else if (message == WM_NOTIFY)
	{
		...
		if (nmh->code == CDN_INITDONE && custom)
		{
			...
			// Align the history label to the file list
			...
			// Align the history combo box to the file list
			...
			historyComboWP.rcNormalPosition.right = fileListWP.rcNormalPosition.right;
		...
		if (nmh->code == CDN_FILEOK)
		{
			...
			TSTR messageBoxTitle = MaxSDK::GetResourceStringAsMSTR(IDS_OPENFILE);
...
void WeatherFileDialog::GetWeatherFile(HWND hWnd)
{
	...
	const TCHAR* str = mData.mFileName.data();
	...
	// All Cache Files
	fl.Append(MaxSDK::GetResourceStringAsMSTR(IDS_ALL_WEATHER_FILES).data());
	fl.Append(MaxSDK::GetResourceStringAsMSTR(IDS_ALL_WEATHER_FILES_WILDCARD).data());

	TSTR title = MaxSDK::GetResourceStringAsMSTR(IDS_OPEN_WEATHER_FILE);
	...
	TSTR default_extension = MaxSDK::GetResourceStringAsMSTR(IDS_WEATHER_EXT);
...
static bool GetTruncatedString(HWND hedit, const TSTR& name, TSTR& truncName)
{
	// get the font
	HFONT Font = NULL;
	LOGFONT lf;
	...
	for (int i=0;i<len;i++) 
	{
		DLGetTextExtent(hdc,truncName.data(),&sz);
		if (sz.cx <= w) 
			break;
		truncated = true;
		new_len--;
		truncName.Resize(new_len);
	}
...
void WeatherFileDialog::InitDialog(HWND hDlg)
{
	...
	if(mData.mFileName.Length()>0)
	{
		...
		truncated = GetTruncatedString(GetDlgItem(hDlg,IDC_WEATHER_FILENAME_HIDDEN),mData.mFileName,truncname);
	...
	else
	{
		...
		SetText(GetDlgItem(hDlg,IDC_WEATHER_FILENAME), MaxSDK::GetResourceStringAsMSTR(IDS_NO_WEATHER_FILE_LOADED).data());
		valid = FALSE;
	}
	...
	if(valid)
	{
		...
		if(mWeatherFileCache.mWeatherHeaderInfo.mLat>0)
			_stprintf(lat,_T("(%4.3f %s, "),mWeatherFileCache.mWeatherHeaderInfo.mLat,MaxSDK::GetResourceStringAsMSTR(IDS_NORTH_INITIAL).data());
		else
			_stprintf(lat,_T("(%4.3f %s, "),-mWeatherFileCache.mWeatherHeaderInfo.mLat,MaxSDK::GetResourceStringAsMSTR(IDS_SOUTH_INITIAL).data());

		if(mWeatherFileCache.mWeatherHeaderInfo.mLon>0)
			_stprintf(lon,_T("%4.3f %s)"),mWeatherFileCache.mWeatherHeaderInfo.mLon,MaxSDK::GetResourceStringAsMSTR(IDS_EAST_INITIAL).data());
		else
			_stprintf(lon,_T("%4.3f %s)"),-mWeatherFileCache.mWeatherHeaderInfo.mLon,MaxSDK::GetResourceStringAsMSTR(IDS_WEST_INITIAL).data());

		TSTR latlon = TSTR(lat) + lon;
...
//		SetText(GetDlgItem(hDlg,IDC_TIME_ZONE),getTimeZone(mWeatherFileCache.mWeatherHeaderInfo.mLon));
		...
		{
			_stprintf(str,_T("%3.1f %s"),mWeatherFileCache.mWeatherHeaderInfo.mNumRecordsPerHour,MaxSDK::GetResourceStringAsMSTR(IDS_PER_HOUR).data());
			SetText(GetDlgItem(hDlg,IDC_DATA_PERIOD),str);
		}
		else
		{
			SetText(GetDlgItem(hDlg,IDC_DATA_PERIOD),MaxSDK::GetResourceStringAsMSTR(IDS_MULTIPLE).data());
		}

		_stprintf(str,_T("%d"),mWeatherFileCache.mNumEntries);
		SetText(GetDlgItem(hDlg,IDC_TOTAL_PERIODS),str);
	}
	else
	{
		TSTR error = _T("");
		SetText(GetDlgItem(hDlg,IDC_TIME_ZONE),MaxSDK::GetResourceStringAsMSTR(IDS_ERROR_READING_WEATHER_FILE).data());
	...
	setPeriod1->SetTooltip(true,MaxSDK::GetResourceStringAsMSTR(IDS_CHANGE_TIME_PERIOD).data());
	ICustButton *setPeriod2 = GetICustButton(GetDlgItem(hDlg,IDC_SET_PERIOD2));
	setPeriod2->SetTooltip(true,MaxSDK::GetResourceStringAsMSTR(IDS_CHANGE_TIME_PERIOD).data());
	ICustButton *setPeriod3 = GetICustButton(GetDlgItem(hDlg,IDC_SET_PERIOD3));
	setPeriod3->SetTooltip(true,MaxSDK::GetResourceStringAsMSTR(IDS_CHANGE_TIME_PERIOD).data());
	...
	{
		TSTR name = MaxSDK::GetResourceStringAsMSTR(perFrameTable[i]);
		if(!name.isNull())
		{
			SendMessage(hCombo, CB_ADDSTRING, 0, (LPARAM)name.data());
		}
	}
	...
	if (!hIconImages)
	{
		...
		LoadMAXFileIcon(_T("daylight"), hIconImages, kBackground, FALSE);
	}
...
TSTR WeatherFileDialog::GetTimeString(Int3 &time)
{
	...
	int second = time.z;
	TCHAR what[128];
	_stprintf(what, _T("%02d:%02d:%02d"),hour,minute,second);
	return what;
...
TSTR WeatherFileDialog::GetDateString(Int3 &date)
{
	...
	static int month [] = {
		...
		TSTR dayStr = MaxSDK::GetResourceStringAsMSTR(day[dayofWeek]);
		TSTR monStr = MaxSDK::GetResourceStringAsMSTR(month[x-1]);
		_stprintf_s(what,256,_T("%s, %s %d, %d "), dayStr.data(),monStr.data(),
			y,z);
		return TSTR(what);
...
void WeatherFileDialog::SetDateWindows(HWND hDlg,BOOL valid)
{
	...
	if(valid==TRUE)
	{
		if(mData.mAnimated==TRUE)
		{
			...
			animStart = dateString +_T("; ") + timeString;
			...
			animEnd = dateString +_T("; ") + timeString;
		...
		else
		{
			...
			dateTime = dateString +_T("; ") + timeString;
			SetWindowText(GetDlgItem(hDlg,IDC_DATE_TIME),dateTime.data());
		}
	}
	else
...
{	
	...
	TCHAR str[32];
	_stprintf(str,_T("%d"),totalFrames);
...
void WeatherFileDialog::SetPeriodInit(HWND hDlg, Int3 &MDY,Int3 &HMS)
{
	...
	TSTR str = dateString +_T("; ") + timeString;
...
WeatherFileDialog::WeatherFileDialog() 
	: mHWND(0)
{
