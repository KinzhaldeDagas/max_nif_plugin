...
const TCHAR* NatLightAssemblyClassDesc::NonLocalizedClassName()
{
...
HINSTANCE NatLightAssemblyClassDesc::HInstance()
{ 
	return MaxSDK::GetHInstance(); // returns owning module handle
}
...
void NatLightAssemblyClassDesc::ResetClassParams(BOOL fileReset) 
{
    ...
    if (ip)
    {
        // Safety check: If these classes are not there, we fallback to the built-in defaults
        if (ip->GetDllDir().ClassDir().FindClassEntry(LIGHT_CLASS_ID, mSunID) == nullptr)
            mSunID = Class_ID(DIR_LIGHT_CLASS_ID, 0); // Default to Directional
        if (ip->GetDllDir().ClassDir().FindClassEntry(LIGHT_CLASS_ID, mSkyID) == nullptr)
            mSkyID = Class_ID(0x7bf61478, 0x522e4705); // Default to Skylight
    }
