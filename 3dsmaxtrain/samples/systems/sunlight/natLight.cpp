...
void NatLightAssembly::InitializeStaticObjects()
{
	...
	sMainParams = new ParamBlockDesc2 (
		...
		WEATHER_FILE_PARAM, _T("weatherFileName"),TYPE_FILENAME, (P_READ_ONLY|P_READ_SECOND_FLAG_VALUE), P_ENUMERATE_AS_ASSET, IDS_WEATHER_FILE_NAME,
		p_accessor, sAccessor,
...
MSTR NatLightAssembly::sStandardSunlightName = MaxSDK::GetResourceStringAsMSTR(IDS_STANDARD_SUNLIGHT); // static member initialization
const MCHAR* NatLightAssembly::StandardSunClassName()
{
	return NatLightAssembly::sStandardSunlightName.data();
}
...
static void SetController(Object* obj, const TCHAR* name, Control* driven)
{
	if (obj != NULL && obj->SuperClassID() == LIGHT_CLASS_ID) {
		...
		try {
			...
			vl.val = driven == NULL ? NULL : MAXControl::intern(driven, defaultDim);
		...
		{
			ProcessMAXScriptException(e, _T("SetController"), false, false, true);
		}
		catch (...)
		{
			ProcessUnknownMAXScriptException(_T("SetController"), false, false, true);
		}
	}
}
...
SunDriver* NatLightAssembly::FindDriver() const
{
	...
	int i;
	ReferenceTarget* driver;
	
	for (i = 0; i < sizeof(controllers) / sizeof(controllers[0]); ++i) {
		Control* c = this->*controllers[i];
		if (c != NULL
				&& c->IsSubClassOf(Class_ID(DAYLIGHT_DRIVEN_CONTROL_CID1,
					DAYLIGHT_DRIVEN_CONTROL_CID2))
				&& (driver = c->GetReference(0)) != NULL
				&& driver->IsSubClassOf(DAYLIGHT_CLASS_ID))
			return static_cast<SunDriver*>(driver);
	}
	return NULL;
}
...
{
	s = MaxSDK::GetResourceStringAsMSTR(IDS_DAY_CLASS_NODE_NAME); //only one place use IDS_DAY_CLASS_NODE_NAME, need to investigae further
}
...
//	tm.NoScale();
	RemoveScaling(tm);
	const float scaleFactor = vpt.NonScalingObjectSize() * vpt.GetVPWorldWidth( tm.GetTrans() ) / 360.0f;
...
)
{
	...
	sSunMesh->BuildMesh();
	MeshRenderHelper(*sSunMesh, gw, gw->getMaterial(), flags, vpt, COMP_ALL);
...
MSTR NatLightAssembly::sObjectName = MaxSDK::GetResourceStringAsMSTR(IDS_NAT_LIGHT_NAME); // static member initialization
const TCHAR* NatLightAssembly::GetObjectName(bool localized) const
{
	return localized ? sObjectName.data() : _T("DaylightAssemblyHead");
}
...
RefTargetHandle NatLightAssembly::Clone(RemapDir &remap)
{
   ...
   newl->SunDriverManualChanged(newl->GetDaylightControlType());
	return(newl);
}


//ClassID for the sky environment - prevents dependency on the full mr SDK
...
)
{
	switch (message) {
		case REFMSG_CHANGE: {
			...
			// If the sunlight has changed, then the its possible the
			// background has changed as well.
			Texmap* pBGTex = GetCOREInterface()->GetEnvironmentMap();
			if (pBGTex != NULL && (MRSKYENVSHADER_CLASS_ID == pBGTex->ClassID() || PHYSICAL_SUN_SKY_ENV_CLASS_ID == pBGTex->ClassID()))
			{
				pBGTex->NotifyDependents(changeInt, PART_DISPLAY, REFMSG_CHANGE, NOTIFY_ALL, FALSE);
			}
		} break;
	}
	return REF_SUCCEED;
...
void ErrorPLCB::proc(ILoad *iload)
{
	...
	mNLA->SunDriverManualChanged(mNLA->GetDaylightControlType());//still need to set everything eup
...
void NatLightAssembly::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? MaxSDK::GetResourceStringAsMSTR(IDS_NAT_LIGHT_NAME) : MSTR(_T("DaylightAssemblyHead"));
}
...
TSTR NatLightAssembly::SubAnimName(int i, bool localized)
{
	return TSTR();
}
...
)
{
	...
	mpEditing = editing;
#if defined(NO_MOTION_PANEL)
	mpDriver = SunDriver::IsCreateModeActive() ? NULL : mpEditing->FindDriver();
#endif	// defined(NO_MOTION_PANEL)
	mpIp = ip;
	mFlags = flags;
	...
	if (mDlgWnd == NULL) {
		ip->AddRollupPage(
			...
			DialogProc,
			MaxSDK::GetResourceStringAsMSTR(IDS_NATLIGHT_PARAM_NAME).data(),
...
#if defined(NO_MOTION_PANEL)
		if (mpDriver != NULL)
			mpDriver->BeginEditParams(ip, flags, prev);
#endif	// defined(NO_MOTION_PANEL)
...
#if defined(NO_MOTION_PANEL)
		if (mpDriver != NULL)
			mpDriver->EndEditParams(ip, flags, next);
#endif	// defined(NO_MOTION_PANEL)
...
void NatLightAssembly::ParamDlgProc::Update(NatLightAssembly* natLight)
{
	if (mpEditing == natLight && mDlgWnd != NULL) {
		...
		int sunIdx = findEntry(sunCombo, mpEditing->mpSunObj);
		if (sunIdx < 0 && mpEditing->mpSunObj && mpEditing->mpSunObj->ClassID() == MR_PHYSSUN_CLASS_ID && pINVIDIARenderersHelper && false == pINVIDIARenderersHelper->IsMentalrayDllPresent())
		{
			//Case where we have loaded a scene with mr sun but mental ray is not present, we need to have a name appearing in the combo box
			//As we don't add the mr sun name when mray is not loaded
			const MSTR legacyMrSun = MaxSDK::GetResourceStringAsMSTR(IDS_MR_SUN_LEGACY);
			addLightToCombo(sunCombo, legacyMrSun.data(), s_mrPhysSunClassDesc);
			sunIdx = findEntry(sunCombo, mpEditing->mpSunObj);
			DbgAssert(sunIdx >= 0);
		}

		SendMessage(sunCombo, CB_SETCURSEL,sunIdx, 0);

		HWND skyCombo = GetDlgItem(mDlgWnd, IDC_SKY_COMBO);
		int skyIdx = findEntry(skyCombo, mpEditing->mpSkyObj);
		if (skyIdx < 0 && mpEditing->mpSunObj && mpEditing->mpSkyObj->ClassID() == MRPHYSSKY_LIGHT_OBJECT_CLASSID && pINVIDIARenderersHelper && false == pINVIDIARenderersHelper->IsMentalrayDllPresent())
		{
			//Case where we have loaded a scene with mr sky but mental ray is not present, we need to have a name appearing in the combo box
			//As we don't add the mr sun name when mray is not loaded
			const MSTR legacyMrSky = MaxSDK::GetResourceStringAsMSTR(IDS_MR_SKY_LEGACY);
			addLightToCombo(skyCombo, legacyMrSky.data(), s_mrPhysSkyClassDesc);
			skyIdx = findEntry(skyCombo, mpEditing->mpSkyObj);
			DbgAssert(skyIdx >= 0);
		}
		SendMessage(skyCombo, CB_SETCURSEL, skyIdx, 0);
...
const MCHAR* GetSunName( ClassDesc& cd) 
{
	if (cd.ClassID() == NatLightAssembly::GetStandardSunClass()) 
	{
		...
		return NatLightAssembly::StandardSunClassName();
	}
	return cd.ClassName();
}
...
)
{
	...
	if (&sc != NULL) 
	{
		for (int isub = sc.Count(ACC_ALL); --isub >= 0; ) 
		{
			...
			if (pCd != NULL) 
			{
				//Is it mental ray sun ? If mental ray dll is not loaded, then ignore.
				if ( pINVIDIARenderersHelper && (false == pINVIDIARenderersHelper->IsMentalrayDllPresent() ) ){
					if (pCd->ClassID() == MR_PHYSSUN_CLASS_ID){
						s_mrPhysSunClassDesc = pCd; //remember the mr phys sun classdesc
						continue;
					}
					if (pCd->ClassID() == MRPHYSSKY_LIGHT_OBJECT_CLASSID){
						s_mrPhysSkyClassDesc = pCd;//remember the mr phys sky classdesc
						continue;
					}
				}
	...
	SendMessage(sunCombo, CB_INSERTSTRING, 0, reinterpret_cast<LPARAM>(LPCTSTR(MaxSDK::GetResourceStringAsMSTR(IDS_NO_SUNLIGHT).data())));
	SendMessage(skyCombo, CB_INSERTSTRING, 0, reinterpret_cast<LPARAM>(LPCTSTR(MaxSDK::GetResourceStringAsMSTR(IDS_NO_SKYLIGHT).data())));
}
} // namespace
...
void NatLightAssembly::SunDriverManualChanged(IDaylightSystem2::DaylightControlType type)
{
	SunDriver* thedriver = FindDriver();	
	...
	if(type == IDaylightSystem2::eWeatherFile)
	{
		...
		if(thedriver)
		{
			thedriver->controlledByWeatherFile = true;
			thedriver->natLight = this;
		...
		thedriver->controlledByWeatherFile = false;
...
)
{
	...
	switch(id) {
		case SUN_PARAM: {
			TSTR sunName = MaxSDK::GetResourceStringAsMSTR(IDS_LIGHT_NAME);
			p->SetSunAndSky(t, SUN_REF, SUN_PARAM, sunName, v.r);
		} break;

		case SKY_PARAM: {
			TSTR skyName = MaxSDK::GetResourceStringAsMSTR(IDS_SKY_NAME);
			p->SetSunAndSky(t, SKY_REF, SKY_PARAM, skyName, v.r);
		} break;

		case MANUAL_PARAM: {
			SunDriver* thedriver = p->FindDriver();	
			if(v.i==WEATHER_MODE){
				if(thedriver)
				{
		...
		case WEATHER_FILE_PARAM:
			...
			/*	To Do.
				...
				if(BMMGetUniversalName(resolveFile, _countof(resolveFile), v.s)) {
					 if(v.s != NULL)   {
							v.Free(TYPE_STRING);
					 }
					 v.s = _tcscpy((TCHAR*)malloc((_tcslen(resolveFile) + 1)*sizeof(TCHAR)), resolveFile);
				}
				*/
			}break;
	}
...
int NatLightAssembly::FindManual::proc(INode* head)
{
	...
	ReferenceTarget* driver;
	if (tm != NULL
			&& tm->IsSubClassOf(Class_ID(DAYLIGHT_DRIVEN_CONTROL_CID1,
				DAYLIGHT_DRIVEN_CONTROL_CID2))
			&& (driver = tm->GetReference(0)) != NULL
			&& driver->IsSubClassOf(DAYLIGHT_CLASS_ID)
			) 
	{
		if(static_cast<SunDriver*>(driver)->GetManual(mT)==DATETIME_MODE)
			++mDateTimeCount;
		else if (static_cast<SunDriver*>(driver)->GetManual(mT)==WEATHER_MODE)
			++mWeatherCount;
		else
			++mManualCount;
...
int NatLightAssembly::SetManual::proc(INode* head)
{
	...
	if (tm != NULL
			...
			&& (driver = tm->GetReference(0)) != NULL
			&& driver->IsSubClassOf(DAYLIGHT_CLASS_ID)) {
		static_cast<SunDriver*>(driver)->SetManual(mT, mManual);
	}
	return DEP_ENUM_SKIP;
}
...
void NatLightAssembly::SetPosition(const Point3& position)
{
	SunDriver* thedriver = FindDriver();
	if(thedriver)
		thedriver->SetPosition(position);
}
...
Point3 NatLightAssembly::GetPosition() const
{
	...
	SunDriver* thedriver = FindDriver();
	if(thedriver)
		position = thedriver->GetPosition();
	return position;
}
...
void  NatLightAssembly::SetOrbitalScale(float orbScale)
{
	...
	SunDriver* thedriver = FindDriver();
	if(thedriver)
		thedriver->SetRad(ip->GetTime(),orbScale);
}
...
float NatLightAssembly::GetOrbitalScale() const
{
	...
	float orbScale = 0.0f;
	if(thedriver)
		orbScale = thedriver->GetRad(ip->GetTime());
	return orbScale;
}
...
void NatLightAssembly::SetNorthDirection(float angle)
{
	...
	SunDriver* thedriver = FindDriver();
	if(thedriver)
		thedriver->SetNorth(ip->GetTime(),angle);
}
...
float NatLightAssembly::GetNorthDirection() const
{
	...
	float angle = 0.0f;
	if(thedriver)
		angle = thedriver->GetNorth(ip->GetTime());
	return angle;
}
...
void NatLightAssembly::SetCompassDiameter(float compassDiameter)
{
	SunDriver* thedriver = FindDriver();
	if(thedriver)
		thedriver->SetCompassDiameter(compassDiameter);
}
...
float NatLightAssembly::GetCompassDiameter() const
{
	...
	if(thedriver)
		compassDiameter = thedriver->GetCompassDiameter();
...
void NatLightAssembly::SetTimeOfDay(const Point3& time)
{
	...
	if(thedriver)
	{
		thedriver->SetHour(ip->GetTime(),time.x);
		thedriver->SetMin(ip->GetTime(),time.y);
		thedriver->SetSec(ip->GetTime(),time.z);
	}
}
...
Point3 NatLightAssembly::GetTimeOfDay() const
{
	...
	if(thedriver)
	{
		time.x = thedriver->GetHour(ip->GetTime());
		time.y = thedriver->GetMin(ip->GetTime());
		time.z = thedriver->GetSec(ip->GetTime());
	}
	return time;
}
...
void NatLightAssembly::SetDate(const Point3& date)
{
	...
	if(thedriver)
	{
		thedriver->SetMon(ip->GetTime(),date.x);
		thedriver->SetDay(ip->GetTime(),date.y);
		thedriver->SetYr(ip->GetTime(),date.z);
	}
}
...
Point3 NatLightAssembly::GetDate() const
{
	...
	if(thedriver)
	{
		date.x = thedriver->GetMon(ip->GetTime());
		date.y = thedriver->GetDay(ip->GetTime());
		date.z = thedriver->GetYr(ip->GetTime());
	}
	return date;
}
...
void NatLightAssembly::SetLatLong(float latitude, float longitude)
{
	...
	if(thedriver)
	{
		if(GetDaylightControlType()!= IDaylightSystem2::eWeatherFile)
		{
			thedriver->SetLat(ip->GetTime(),latitude);
			thedriver->SetLong(ip->GetTime(),longitude);
			thedriver->SetZone(ip->GetTime(),getTimeZone(longitude));
			thedriver->calculateCity(ip->GetTime());
			...
			thedriver->calculate(ip->GetTime(),FOREVER);
		}
	}
}
...
float NatLightAssembly::GetLatitude() const
{
	...
	float latitude = 0.0f;
	if(thedriver)
		latitude = thedriver->GetLat(ip->GetTime());
	return latitude;
}
...
float NatLightAssembly::GetLongitude() const
{
	...
	float longitude = 0.0f;
		if(thedriver)
			longitude = thedriver->GetLong(ip->GetTime());
	return longitude;
}
...
void NatLightAssembly::SetDaylightSavingTime(BOOL isDaylightSavingTime)
{
	...
	SunDriver* thedriver = FindDriver();
	if(thedriver)
		thedriver->SetDst(ip->GetTime(),isDaylightSavingTime);
}
...
BOOL NatLightAssembly::GetDaylightSavingTime() const
{
	...
	if(thedriver)
		isDaylightSavingTime = thedriver->GetDst(ip->GetTime());
...
FPInterfaceDesc NatLightAssembly::mFPInterfaceDesc(DAYLIGHT_SYSTEM_INTERFACE, 
	...
	NatLightAssembly::eGetPosition, _T("GetPosition"), 0, TYPE_POINT3_BV, 0, 0,
	NatLightAssembly::eGetOrbitalScale, _T("GetOrbitalScale"), 0, TYPE_FLOAT, 0, 0,
	NatLightAssembly::eGetNorthDirection, _T("GetNorthDirection"), 0, TYPE_FLOAT, 0, 0,
	NatLightAssembly::eGetCompassDiameter, _T("GetCompassDiameter"), 0, TYPE_FLOAT, 0, 0,
	NatLightAssembly::eGetTimeOfDay, _T("GetTimeOfDay"), 0, TYPE_POINT3_BV, 0, 0,
	NatLightAssembly::eGetDate, _T("GetDate"), 0, TYPE_POINT3_BV, 0, 0,
	NatLightAssembly::eGetLatitude, _T("GetLatitude"), 0, TYPE_FLOAT, 0, 0,
	NatLightAssembly::eGetLongitude, _T("GetLongitude"), 0, TYPE_FLOAT, 0, 0,
	NatLightAssembly::eIsDaylightSavingsTime, _T("GetDaylightSavingTime"), 0, TYPE_BOOL, 0, 0,
	NatLightAssembly::eGetTimeZone, _T("GetTimeZone"), 0, TYPE_FLOAT, 0, 1,
		_T("longitude"), 0, TYPE_FLOAT,
...
void NatLightAssembly::LoadWeatherFile(const TCHAR *filename)
{
...
TSTR NatLightAssembly::GetLocation()
{
	...
	if(thedriver)
	{
		...
		if(	thedriver->GetCity())
			str = TSTR(thedriver->GetCity());
	}
	return str;
}
...
void NatLightAssembly::GetAltitudeAzimuth(TimeValue t,float &altitude, float &azimuth) 
{
	...
	if(thedriver)
	{
		Point2 val = thedriver->GetAzAlt(t,inter);
		azimuth = (val.x);
		altitude = (val.y);  
	}
}
...
bool NatLightAssembly::GetTemps(float &dryBulbTemperature, float &dewPointTemperature)
{
	if(GetDaylightControlType()==IDaylightSystem2::eWeatherFile&&mWeatherFileMissing==false &&mErrorLoadingWeatherFile==false&&mWeatherFileCache.mDataVector.size()>0)
	{
		...
		TimeValue t= GetCOREInterface()->GetTime();
		const DaylightWeatherData data = mWeatherFileCache.GetNthEntry(t);
		//check source and uncertainty, 
		if(data.mDataUncertainty.Length()>=( DEW_POINT_TEMPERATURE_UNCERTAINTY +1))
		{
			if(data.mDataUncertainty[ DRY_BULB_TEMPERATURE_SOURCE]==_T('?')||data.mDataUncertainty[DEW_POINT_TEMPERATURE_SOURCE]==_T('?')||
				data.mDataUncertainty[DRY_BULB_TEMPERATURE_UNCERTAINTY] == _T('0') || data.mDataUncertainty[DEW_POINT_TEMPERATURE_UNCERTAINTY] ==_T('0'))
				return false;
		}
...
bool NatLightAssembly::GetTempsLocal(const DaylightWeatherData &data ,float &dryBulbTemperature, float &dewPointTemperature)
{
...
bool NatLightAssembly::GetExtraTerrestialRadiationLocal(const DaylightWeatherData &data ,float &extraterrestrialHorizontalRadiation, float &extraterrestrialDirectNormalRadiation)
{
...
bool NatLightAssembly::GetRadiationLocal(const DaylightWeatherData &data ,float &globalHorizontalRadiation, float &directNormalRadiation,
											 float &diffuseHorizontalRadiation)
{
...
bool NatLightAssembly::GetIlluminanceLocal(const DaylightWeatherData &data,float &globalHorizontalIlluminance, float &directNormalIlluminance,
											 float &diffuseHorizontalIlluminance, float &zenithLuminance)
{
	//check source and uncertainty, we only check for the directnormal and diffuse since they are key, the others
	//can be uncertain and will be set to -1 if so.
	...
	if(data.mDataUncertainty.Length()>ZENITH_ILLUMINANCE_UNCERTAINTY&&data.mDataUncertainty[ZENITH_ILLUMINANCE_SOURCE]==_T('?')||data.mDataUncertainty[ZENITH_ILLUMINANCE_UNCERTAINTY]==_T('0'))
...
void NatLightAssembly::RenderFrame(void *param, NotifyInfo *nInfo)
{
	...
	{
		if (TimeValue* t = GetNotifyParam<NOTIFY_RENDER_PREEVAL>(nInfo))
		{
			Interval valid(*t,*t);
...
void NatLightAssembly::SetSunLocationBasedOnWeatherFile(SunDriver *thedriver, TimeValue t, Interval &valid)
{
	if(mWeatherFileMissing==false &&mErrorLoadingWeatherFile==false)
	{
		...
		if(inSet==false)
		{
			...
			//be called from within SunDriver::GetValue()
			DisableRefMsgs();
			thedriver->SetWeatherMDYHMS(t,data.mMonth,data.mDay,data.mYear,data.mHour,data.mMinute,0,dst);
			EnableRefMsgs();
			ResumeAnimate();
		}
		inSet = false;
...
void NatLightAssembly::SetUpBasedOnWeatherFile()
{
	...
	if(GetDaylightControlType()== IDaylightSystem2::eWeatherFile)
	{
		//double make sure the driver is set up, may not be if from post load
		SunDriver* thedriver = FindDriver();
		if(thedriver)
		{
			thedriver->controlledByWeatherFile = true;
			...
			if(mWeatherFileMissing==false)
			{
				...
				if(filename.Length()>0)
				{
					...
					if(numEntries)
					{
						...
						TSTR city =  mWeatherFileCache.mWeatherHeaderInfo.mCity + _T(", ") +
						mWeatherFileCache.mWeatherHeaderInfo.mRegion + _T(", ") + mWeatherFileCache.mWeatherHeaderInfo.mCountry;
						SuspendAnimate();
						AnimateOff();
						thedriver->SetLat(ip->GetTime(),mWeatherFileCache.mWeatherHeaderInfo.mLat);
						thedriver->SetLong(ip->GetTime(),mWeatherFileCache.mWeatherHeaderInfo.mLon);
						thedriver->SetZone(ip->GetTime(),mWeatherFileCache.mWeatherHeaderInfo.mTimeZone);
						thedriver->SetCity(city);
						//nope we set the thedriver->calculateCity(ip->GetTime());
						Interval valid;
						SetSunLocationBasedOnWeatherFile(thedriver,ip->GetTime(),valid);
						SetEnergyValuesBasedOnWeatherFile(ip->GetTime(),valid);
						thedriver->calculate(ip->GetTime(),FOREVER);
						thedriver->ValuesUpdated();
						ResumeAnimate();
					}
					else
