...
inline double angle(const double x, const double y) { return (atan((x)/(y))+((y)<=0.0?PI:(x)<=0.0?2*PI:0.0)); }
#define sqr(x) ((x)*(x))
...
//////////////////////////////////////////////////////////////////
inline double dtr(double dang){return TWOPI * dang/360.0;}
inline double rtd(double rang){return 360.0 * rang/TWOPI;}
...
double gregorian2julian(int month, int day, int year);
int gregorian2julian_int(int month, int day, int year);     // to be used if storing julian date as int
...
int sunloc(void);
void julian2gregorian(double jdate, int* month, int* day, int* year);
void julian2gregorian_int(int jdate, int* month, int* day, int* year);// to be used if storing julian date as int
...
// Returns the number of days, so far running through year, until the first day of the given month
// (i.e. the sum of all previous months for that year)
int GetMDays(int leap, int month);

// Returns the duration of the given month, in days
