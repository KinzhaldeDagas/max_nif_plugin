...
WeatherHeaderInfo WeatherFileCache::ReadHeader(const TCHAR* filename)
{
	...
	WeatherData::EnergyPlusWeatherFileReader reader(gcnew System::String(filename));

	WeatherData::WeatherHeader^ header = reader.ReadWeatherHeader();
...
int WeatherFileCache::ReadWeatherData(const TCHAR* filename)
{
	...
	try
	{
		WeatherData::WeatherDataSet^ weatherData = reader.ReadWeatherDataSet();
		...
		for each(WeatherData::WeatherEntry^ entry in weatherData->Entries)
		{
			data = ConvertEntryToStruct(this, entry,firstIsTrue);
			firstIsTrue = false;
	...
	catch(...) //got an error
	{
		...
		mNumEntries = 0;
		TSTR title(MaxSDK::GetResourceStringAsMSTR(IDS_WEATHER_FILE));
		TSTR message(MaxSDK::GetResourceStringAsMSTR(IDS_WEATHER_FILE_LOAD_ERROR));
		if (GetCOREInterface()->GetQuietMode()) //don't show dialog
		{
		...
		{
			MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), message.data(), title.data(), MB_OK |  MB_ICONWARNING | MB_APPLMODAL);
		}
	}
...
WeatherFileCache::WeatherFileCache()
		: mNumEntries(0)
		, mSubtractHour(true)
{
	Clear();
...
{	mWeatherHeaderInfo.mCity = _T("");
	mWeatherHeaderInfo.mCountry = _T("");
...
DaylightWeatherData WeatherFileCacheFiltered::GetNthEntry(const TimeValue t) const
{
    if(DbgVerify(!mDataVector.empty()))
    {
	    //first we change t into a frame, which is then an index into our array.
	    const TimeValue start  = GetCOREInterface10()->GetAutoKeyDefaultKeyTime();

	    if(t > start)
	    {
            const size_t index = std::min<size_t>((t - start) / GetTicksPerFrame(), mDataVector.size() - 1);
            return mDataVector[index];
	    }
        else 
        {
            return mDataVector[0];
        }
    }
    else
    {
        return DaylightWeatherData();
    }
}
