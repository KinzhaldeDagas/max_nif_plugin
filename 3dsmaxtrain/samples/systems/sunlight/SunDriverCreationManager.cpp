...
//**************************************************************************/
#pragma unmanaged
#include "SunDriverCreationManager.h"
#include "sunclass.h"
#include "compass.h"
#include "natlight.h"
#include <macrorec.h>
...
SunDriverCreationManager::SunDriverCreationManager() :
	node0(nullptr),
	theDriver(nullptr),
...
// This initializes a few variables, creates a new sun driver object and 
// starts the editing of the objects parameters.
void SunDriverCreationManager::Begin( IObjCreate *ioc, ClassDesc *desc, bool daylight )
{
	// This just initializes the variables.
	createInterface = ioc;
	cDesc           = desc;	 //class descriptor of the driver controller
	DeleteAllRefsFromMe();
	attachedToNode = FALSE;
...
int SunDriverCreationManager::NumRefs()
{
	return REFERENCE_COUNT;
}

// This method sets the ith reference.  
void SunDriverCreationManager::SetReference(int i, RefTargetHandle rtarg) 
{ 
	switch(i) 
	{
	...
	case SUN_DRIVER_REFERENCE_INDEX:
		theDriver = static_cast<SunDriver *>(rtarg);
		break;
...
RefTargetHandle SunDriverCreationManager::GetReference(int i) 
{ 
	switch(i) 
	{
	case NODE_0_REFERENCE_INDEX: 
...
//SunDriverCreationManager::~SunDriverCreationManager
void SunDriverCreationManager::End()
{
	if (NULL != theDriver) {
		theDriver->EndEditParams( (IObjParam*)createInterface, 
			TRUE/*destroy*/, NULL );
		DeleteAllRefsFromMe();
	}
}
...
RefResult SunDriverCreationManager::NotifyRefChanged(
	...
	BOOL propagate) 
{
	...
	{ 
		...
		// This is used so that if the user deselects this driven node
		...
		{
			// refresh the panel
			HWND temp = theDriver->hDriverParams;
			theDriver->EndEditParams( (IObjParam*)createInterface, FALSE/*destroy*/,NULL );
			// This creates a new SunDriver object and starts
			// the parameter editing process.
			CreateNewDriver(temp);	
...
// This method is called to create a new sun driver object and 
// begin the parameter editing process.
void SunDriverCreationManager::CreateNewDriver(HWND rollup)
{
	theHold.Suspend();
	ReplaceReference(SUN_DRIVER_REFERENCE_INDEX, 
		new SunDriver(daylightSystem), 
		true);
	theHold.Resume();
	//Possibly point to an existing dialog rollup
	theDriver->hDriverParams = rollup;
...
int SunDriverCreationManager::proc( 
	HWND hwnd,
	int msg,
	int point,
	int flag,
		...
		case MOUSE_MOVE: 
		{
			switch (point) 
			{
				case 0: {
					...
					pt0 = m;
					// Make sure the driver object exists
					assert(theDriver);
					...
					if ( attachedToNode) {
						...
						// This creates a new SunDriver object (theDriver)
				...
				case 1: {
					...
					if (msg == MOUSE_POINT) 
					{
						...
						if (theDriver->daylightSystem) 
						{
							...
							natLightObj->SetMultController(new DrivenControl(theDriver,LIGHT_MULT));
							natLightObj->SetIntenseController(new DrivenControl(theDriver,LIGHT_INTENSE));
							natLightObj->SetSkyCondController(new DrivenControl(theDriver,LIGHT_SKY));
						} 
						else 
						{
							// Create the Light
							...
							TSTR nodename = MaxSDK::GetResourceStringAsMSTR(IDS_LIGHT_NAME);
							createInterface->MakeNameUnique(nodename);
							newNode->SetName(nodename);
						}

						// Create a new driven controller of the driver control.
						DrivenControl* driven = new DrivenControl(theDriver,LIGHT_TM);
						...
						INodeMonitor* nodeMon = static_cast<INodeMonitor*>(theDriver->theObjectMon->GetInterface(IID_NODEMONITOR));
						nodeMon->SetNode(newNode);
						//theDriver->thePoint=dummyNode;
						//the driver references the point so it can get
						//notified when the	user rotates it
						theDriver->ReplaceReference(REF_POINT, dummyNode );
						DbgAssert(theDriver->thePoint == dummyNode);
						// Indicate that the system is attached
						attachedToNode = TRUE;

						if (daylightSystem) {
							theDriver->ReplaceReference(REF_LOOKAT, static_cast<Control*>(
							...
							assert(ilookat != NULL);
							ilookat->AppendTarget(theDriver->thePoint);
							ilookat->SetTargetAxis(2);
							ilookat->SetTargetAxisFlip(true);
							ilookat->SetVLisAbs(false);
							ilookat->SetVectorLineLength(100.0f);
							theDriver->theLookat->SetRotationController(lookat);

							theDriver->ReplaceReference(REF_MULTIPLIER, static_cast<Control*>(
						...
						theDriver->SetRad(TimeValue(0),1.0f);
				...
				case 2: {
					...
					if (node0) {
						...
						r = (float)fabs(vpx.SnapLength(vpx.GetCPDisp(center,Point3(0,0,1),pt0,m)));
						...
						theDriver->NotifyDependents(FOREVER, PART_OBJ, REFMSG_CHANGE);
					...
					if (msg == MOUSE_POINT) {
						...
						INodeMonitor* nodeMon = 
							static_cast<INodeMonitor*>(theDriver->theObjectMon->GetInterface(IID_NODEMONITOR));
						createInterface->SelectNode(nodeMon->GetNode());
						if (daylightSystem) {
							SunDriver::SetInCreate(false);
							theDriver->UpdateUI(t);
						}
						ignoreSelectionChange = FALSE;
						// This registers the undo object with the system so
						// the user may undo the creation.
						theHold.Accept(MaxSDK::GetResourceStringAsMSTR(IDS_UNDO).data());
		...
		case MOUSE_ABORT:
			{
				...
				// This creates a new SunDriver object and starts the parameter editing process.
				CreateNewDriver(temp);  
                ...
                for (int iPanel = 0; iPanel < GetViewPanelManager()->GetViewPanelCount(); ++iPanel)
                {
                    IViewPanel* panel = GetViewPanelManager()->GetViewPanel(iPanel);
                    if (panel && panel->IsViewPanelVisible())
                    {
                        int numViews = (int)panel->GetNumberOfViewports();
                        for (int i = 0; i < numViews; i++)
                        {
                            ViewExp10* vp10 = reinterpret_cast<ViewExp10*>(panel->GetViewExpByIndex(i).Execute(ViewExp::kEXECUTE_GET_VIEWEXP_10));
                            if (vp10)
                            {
                                vp10->Invalidate();
                            }
