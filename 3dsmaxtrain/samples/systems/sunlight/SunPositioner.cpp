...
ParamBlockDesc2 SunPositionerObject::m_pb_desc(
    ...
    kMainParamID_JulianDay, _T("julian_day"), TYPE_INT, P_ANIMATABLE, IDS_SUNPOS_PARAM_JULIANDAY,
        ...
        p_accessor, &m_pb_accessor,
        p_nonLocalizedName, _T("Julian Day"),
        p_end,
    kMainParamID_TimeInSeconds, _T("time_in_seconds"), TYPE_INT, P_ANIMATABLE, IDS_SUNPOS_PARAM_TIMEINSECONDS,
        ...
        p_accessor, &m_pb_accessor,
        p_nonLocalizedName, _T("Time in Seconds"),
        p_end,
    ...
    kMainParamID_TimeZone, _T("time_zone"), TYPE_FLOAT, P_ANIMATABLE, IDS_SUNPOS_PARAM_TIMEZONE,
        ...
        p_range, -24.0f, 24.0f,
        p_accessor, &m_pb_accessor,
        p_nonLocalizedName, _T("Time Zone"),
        p_end,
...
{  
    ...
    if(m_param_block != nullptr)
    {
        ...
        {
            set_city_location(GetCOREInterface()->GetTime(), default_city->latitude, default_city->longitude, default_city->get_ui_name().data());
        }
    }
}
...
const TCHAR* SunPositionerObject::GetObjectName(bool localized) const
{
    return localized ? get_class_descriptor().ClassName() : get_class_descriptor().NonLocalizedClassName();
}
...
void SunPositionerObject::DrawSun(const TimeValue t, ViewExp& vpt, GraphicsWindow& gw, INode& node, const int flags) 
{
    ...
    // Draw the sun mesh
    {
        ...
        MeshRenderHelper(m_sun_mesh, &gw, gw.getMaterial(), flags, &vpt, COMP_ALL);
    }
...
void SunPositionerObject::GetClassName(TSTR& s, bool localized) const
{
    s = localized ? get_class_descriptor().ClassName() : get_class_descriptor().NonLocalizedClassName();
}
...
TSTR SunPositionerObject::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case 0: return localized ? MaxSDK::GetResourceStringAsMSTR(IDS_PARAMETERS) : MSTR(_T("Parameters"));
	default: return TSTR();
	}
}
...
void SunPositionerObject::install_sunsky_environment(const bool overwrite_existing_environment, const bool redirect_existing_sunsky_environment) const
{
    ...
    if(sunsky_env == nullptr)
    {
        ...
        if((env_map == nullptr) || overwrite_existing_environment)
        {
            ...
            sunsky_env->SetName(sunsky_env->ClassName(true)); // set name, otherwise the name will be empty
...
const TCHAR* SunPositionerObject::ClassDescriptor::NonLocalizedClassName()
{
	return _T("Sun Positioner");
}
    ...
    Matrix3& mat)
{
    ...
    else if(msg == MOUSE_MOVE)
    {
        ...
        case 3:
        {
            ...
            const Point3 sun_direction = ob->GetSunDirection(GetCOREInterface()->GetTime(), valid);
