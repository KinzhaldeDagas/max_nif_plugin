...
class DrivenControl; // defined below
...
class SunDriver: public ReferenceTarget, public TimeChangeCallback { 
	friend class SunDriverPostLoad;
	...
	public:
		...
		DECL_FUNC_NO_INTERVAL_1(calculate)
		...
		float GetLat(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetLat)
		float GetLong(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetLong)
		float GetNorth(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetNorth)
		void GetTime(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetTime)
		int GetZone(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetZone)
		int GetDst(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetDst)
		int GetManual(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetManual)
		float GetSkyCondition(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetSkyCondition)
		Point2 GetAzAlt(TimeValue t, Interval& valid );
		DECL_FUNC_NO_INTERVAL_1(GetAzAlt)
		...
		int GetHour(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetHour)
		int GetMin(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetMin)
		int GetSec(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetSec)
		int GetMon(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetMon)
		int GetDay(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetDay)
		int GetYr(TimeValue t, Interval& valid);
		DECL_FUNC_NO_INTERVAL_1(GetYr)

#if defined(NO_MOTION_PANEL)
		static bool IsCreateModeActive();
#endif	// defined(NO_MOTION_PANEL)
		
		// The driven controllers call this method to retrieve their
		// value.  This allows the driver object to control the entire
		// system.
...
class DrivenControl : public Control {
	public:		
		// This controller maintains a reference to the driver.
		SunDriver *driver;
		// This is the id of the driven controller. Passed to the driver.
		ULONG id;
		// Does this belong to a daylight system?
		bool	daylightSystem;

		// Constructors, destructor, assignment operator.
		DrivenControl(bool daylight, BOOL loading=FALSE) : daylightSystem(daylight) { driver = NULL; id = 0; }
		DrivenControl(const DrivenControl& ctrl);
		DrivenControl(const SunDriver* m, int i);
		...
		// russom - August 16, 2006 - 775527
		BOOL CanAssignController(int subAnim){ return driver->CanAssignController(subAnim); }
		BOOL AssignController(Animatable *control,int subAnim) { return driver->AssignController(control, subAnim); }
		...
		Class_ID ClassID()
			{ return daylightSystem
				? Class_ID(DAYLIGHT_DRIVEN_CONTROL_CID1,DAYLIGHT_DRIVEN_CONTROL_CID2)
				: Class_ID(DRIVEN_CONTROL_CID1,DRIVEN_CONTROL_CID2); }  
		SClass_ID SuperClassID();
		void GetClassName(MSTR& s, bool localized) const override;
		void DeleteThis() { delete this; }		
		int IsKeyable(){ return 1;}
		BOOL GetNextKeyTime(TimeValue t,DWORD flags,TimeValue &nt){return driver->GetNextKeyTime(t,flags,nt);};

		//  This implementation simply calls the same method of the SunDriver
		// object.  This is done because the UI controls managed by
		// the ring driver control the entire system.
		void BeginEditParams( IObjParam *ip, ULONG flags,Animatable *prev) { assert(driver); driver->BeginEditParams(ip,flags,prev); } 
		// This is called to end the parameter editing.  Again, the 
		// sun driver method is called.
		void EndEditParams( IObjParam *ip, ULONG flags,Animatable *next) { if (driver != NULL) driver->EndEditParams(ip,flags,next); } 

		// --- Inherited virtual methods from ReferenceMaker and Target ---
		RefTargetHandle Clone(RemapDir& remap);
		// The driven controller has a single reference to the sun driver.
		int NumRefs() { return 1; };	
		RefTargetHandle GetReference(int i)  
			{ assert(i==0); return driver; }
private:
		virtual void SetReference(int i, RefTargetHandle rtarg) 
			{ assert(i==0); driver = (SunDriver *)rtarg; }		
public:
