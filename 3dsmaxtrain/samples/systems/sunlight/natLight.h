...
public:
	...
	virtual const TCHAR *GetObjectName(bool localized) const override;
	virtual BOOL HasUVW();
...
public:
	...
	void LoadWeatherFile(const TCHAR *filename);
	...
	BEGIN_FUNCTION_MAP 
		...
		FN_0(eGetPosition, TYPE_POINT3_BV, GetPosition);
		FN_0(eGetOrbitalScale, TYPE_FLOAT, GetOrbitalScale);
		FN_0(eGetNorthDirection, TYPE_FLOAT, GetNorthDirection);
		FN_0(eGetCompassDiameter, TYPE_FLOAT, GetCompassDiameter);
		FN_0(eGetTimeOfDay, TYPE_POINT3_BV, GetTimeOfDay);
		FN_0(eGetDate, TYPE_POINT3_BV, GetDate);
		FN_0(eGetLatitude, TYPE_FLOAT, GetLatitude);
		FN_0(eGetLongitude, TYPE_FLOAT, GetLongitude);
		FN_0(eIsDaylightSavingsTime, TYPE_BOOL, GetDaylightSavingTime);
		FN_1(eGetTimeZone, TYPE_INT, GetTimeZone, TYPE_FLOAT);
	...
	//for sundriver interaction key for daylight weather control.
	void SetEnergyValuesBasedOnWeatherFile(TimeValue t, Interval &valid);
	void SetSunLocationBasedOnWeatherFile(SunDriver *thedriver, TimeValue t, Interval &valid);
	void SetUpBasedOnWeatherFile();
	void SunDriverManualChanged( IDaylightSystem2::DaylightControlType type);
	...
	static const MCHAR* StandardSunClassName();

	static void InitializeStaticObjects();
	static void DestroyStaticObjects();

    static bool GetTempsLocal(const DaylightWeatherData &data, float &dryBulbTemperature, float &dewPointTemperature);
    static bool GetExtraTerrestialRadiationLocal(const DaylightWeatherData &data, float &extraterrestrialHorizontalRadiation, float &extraterrestrialDirectNormalRadiation);
    static bool GetRadiationLocal(const DaylightWeatherData &data, float &globalHorizontalRadiation, float &directNormalRadiation,
        float &diffuseHorizontalRadiation);
    static bool GetIlluminanceLocal(const DaylightWeatherData &data, float &globalHorizontalIlluminance, float &directNormalIlluminance,
        float &diffuseHorizontalIlluminance, float &zenithLuminance);
...
private:
	...
	static MSTR				sStandardSunlightName;
...
void SetUpSun(SClass_ID sid, 
	const Class_ID& systemClassID, 
	Interface* ip, 
	GenLight* obj, 
	const Point3& sunColor);
