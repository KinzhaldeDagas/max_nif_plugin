...
const CityList::Entry& CityList::operator[]( size_t i ) const
{
	return mCityList[i];
}

const CityList::Entry* CityList::GetDefaultCity() const
{
	return mDefaultExists ? &mpDefaultCity : nullptr;
}

UINT CityList::Count() const
{
	return (UINT)mCityList.size();
}

MSTR CityList::Entry::get_ui_name() const
{
    MSTR ui_name = name;

    // Remove leading + or * (used in the city file to identify "major" cities)
    if((ui_name.Length() > 0) && ((ui_name[0] == _T('+')) || (ui_name[0] == _T('*'))))
    {
        ui_name.remove(0, 1);
    }

    // Remove trailing carriage return and line feed
    while((ui_name.Length() > 0) && (isspace(ui_name[ui_name.Length() - 1])))
    {
        ui_name.remove(ui_name.Length() - 1, 1);
    }
