...
extern BOOL doLocationDialog(HWND hParent, float* latitude, float* longitude, MSTR& cityName);
...
namespace 
{
	...
	// did more harm than good, since the SunDriver affected may not be active
	...
	TCHAR holdLastName[HOLD_LAST_NAME_LENGTH] = {0};
}

// externals from GEOLOC.CPP
extern int lastCity;
extern TCHAR* lastCityName;
...
public:
	CityRestore(SunDriver* driver, const TCHAR* name);

	virtual void Restore(int isUndo);
	virtual void Redo();

	virtual int Size() { return sizeof(*this); }
	virtual TSTR Description() { return _T("Undo Sunlight City Name"); }

	virtual void DeleteThis();
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("CityRestore"); } // from Animatable

private:
	std::basic_string<TCHAR> _undoCity;
	std::basic_string<TCHAR> _redoCity;
};

CityRestore::CityRestore(SunDriver* driver, const TCHAR* name)
{
	SetRef(driver);
	if(NULL != driver && NULL != driver->GetCity())
	{
		_redoCity = driver->GetCity();
	}
...
{
	SunDriver *driver = static_cast<SunDriver*>(GetRef());
	if(NULL != driver)
	{
		driver->SetCity(_undoCity.c_str());
		driver->UpdateUI(GetCOREInterface()->GetTime());
	}
}
...
void CityRestore::Redo() 
{
	...
	if(NULL != driver)
	{
		driver->SetCity(_redoCity.c_str());
		driver->UpdateUI(GetCOREInterface()->GetTime());
	}
}
...
public:
	ManualRestore(SunDriver* driver, int oldManual);

	virtual void Restore(int isUndo);
	virtual void Redo();

	virtual int Size() { return sizeof(*this); }
	virtual TSTR Description() { return _T("Undo Set Daylight Control Type"); }

	virtual void DeleteThis(){delete this;}
	virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("ManualRestore"); } // from Animatable
...
ManualRestore::ManualRestore(SunDriver* driver, int oldManual)
{
	SetRef(driver);
	if(driver!=NULL)
	{
		TimeValue t = GetCOREInterface()->GetTime();
		mRedoManual = driver->GetManual(t);
	}
...
void ManualRestore::Restore(int isUndo)
{
	...
	if(driver !=NULL)
	{
		driver->NotifyNatLightManualChanged((IDaylightSystem2::DaylightControlType)(mUndoManual));
	}
}
...
void ManualRestore::Redo()
{
	...
	if(NULL != driver)
	{
		driver->NotifyNatLightManualChanged((IDaylightSystem2::DaylightControlType)(mRedoManual));
	}
}
...
// This method returns a new instance of the driven controller.
Control* GetNewDrivenControl(SunDriver *driver, int i);

// Initialize the class variables...
//HWND SunDriver::hDriverParams = NULL;
IObjParam *SunDriver::iObjParams = nullptr;


ISpinnerControl *SunDriver::radSpin = nullptr;
ISpinnerControl *SunDriver::latSpin = nullptr;
ISpinnerControl *SunDriver::longSpin = nullptr;
ISpinnerControl *SunDriver::hourSpin = nullptr;
ISpinnerControl *SunDriver::minSpin = nullptr;
ISpinnerControl *SunDriver::secSpin = nullptr;
ISpinnerControl *SunDriver::yearSpin = nullptr;
ISpinnerControl *SunDriver::monthSpin = nullptr;
ISpinnerControl *SunDriver::daySpin = nullptr;
ISpinnerControl *SunDriver::northSpin = nullptr;
ISpinnerControl *SunDriver::zoneSpin = nullptr;
ICustStatus	*SunDriver::altEdit = nullptr;
ICustStatus	*SunDriver::azEdit = nullptr;
ICustStatus	*SunDriver::cityDisplay = nullptr;
// no longer static 
// SYSTEMTIME SunDriver::theTime = nullptr;
HIMAGELIST SunDriver::hIconImages = NULL;

bool SunDriver::sInCreate = false;

void SunDriver::SetInCreate(bool value)
{
	sInCreate = value;
}

bool SunDriver::IsInCreate()
{
	return sInCreate;
}
...
void SunDriver::SetPosition(const Point3& position)
{
...
Point3 SunDriver::GetPosition()
{
	...
	if(thePoint)
	{
		...
		ret = mat.PointTransform(pos);
	}
	return ret;
}

void SunDriver::SetCompassDiameter(float compassDiameter)
{
...
float SunDriver::GetCompassDiameter()
{
...
void SunDriver::GetClassName(MSTR& s, bool localized) const
{
	if (localized)
	{
		s = MaxSDK::GetResourceStringAsMSTR(daylightSystem ? IDS_DAY_CLASS : IDS_SUN_CLASS);
	}
	else
	{
		s = daylightSystem ? _T("Daylight") : _T("Sunlight");
	}
}

Animatable* SunDriver::SubAnim(int i)
{
	switch (i) {
	case SUBANIM_PBLOCK:
		return pblock;
...
TSTR SunDriver::SubAnimName(int i, bool localized)
{
	switch (i)
	{
	case SUBANIM_PBLOCK: return localized ? MaxSDK::GetResourceStringAsMSTR(IDS_DB_PARAMETERS) : MSTR(_T("Parameters"));
	case SUBANIM_LOOKAT: return localized ? MaxSDK::GetResourceStringAsMSTR(IDS_LOOKAT) : MSTR(_T("Manual Position"));
	case SUBANIM_MULTIPLIER: return localized ? MaxSDK::GetResourceStringAsMSTR(IDS_MULTIPLIER) : MSTR(_T("Manual Intensity"));
	}
	return TSTR();
}

// russom - August 16, 2006 - 775527
BOOL SunDriver::CanAssignController(int subAnim)
{
	return( daylightSystem && ((subAnim == SUBANIM_LOOKAT) || (subAnim == SUBANIM_MULTIPLIER)) );
}

// russom - August 16, 2006 - 775527
BOOL SunDriver::AssignController(Animatable *control,int subAnim)
{
	if( !daylightSystem )
		return FALSE;
...
#define SUNDRIVER_VERSION_SUNLIGHT 2 //2 is same as 1 except we revert the long
#define SUNDRIVER_VERSION_DAYLIGHT 5 //4 is same as 5 except we revert the long
...
void NotifyMaxReset(void *ptr, NotifyInfo *info)
{
	...
	if (info->intcode == NOTIFY_FILE_PRE_OPEN)
		if (NULL != info->callParam && IOTYPE_MAX != *GetNotifyParam<NOTIFY_FILE_PRE_OPEN>(info))
			return;

	//set the natLight to NULL!
	SunDriver *sl = (SunDriver*)ptr;
	sl->natLight = NULL;
...
static constexpr const TCHAR *JapaneseCityName =_T("Tokyo Japan");
static constexpr const TCHAR *ChineseCityName =_T("Beijing China");
static constexpr const TCHAR *KoreanCityName =_T("Seoul Korea");
static constexpr const TCHAR *EnglishCityName =_T("San Francisco, CA");

void SunDriver::SetCityInfo(const TCHAR* name, int& zone, float& lati, float& longi)
{
...
// Constructor.
SunDriver::SunDriver(bool daylight) {
	...
	if (daylightSystem)
		ReplaceReference( REF_PARAM, CreateParameterBlock( desc4Daylight, 9, SUNDRIVER_VERSION_DAYLIGHT ) );
	else
		ReplaceReference( REF_PARAM, CreateParameterBlock( desc1, 7, SUNDRIVER_VERSION_SUNLIGHT ) );
	...
	const CityList::Entry* pEntry = CityList::GetInstance().GetDefaultCity();
	...
	JulianCalendar jd = fusetime(theTime);
...
{
	static_cast<SunDriver*>(param)->RefThePointClone();
...
void SunDriver::RefThePointClone()
{
	if (thePoint != NULL) {
		INode* p = thePoint;
		thePoint = NULL;
...
// This method is called to return a copy of the ring driver.
RefTargetHandle SunDriver::Clone(RemapDir& remap) {
	SunDriver* newm = new SunDriver(daylightSystem);	
...
void SunDriver::UpdateUI(TimeValue t)
{
	if ( hDriverParams ) {
		...
		EnableWindow(GetDlgItem(hDriverParams,IDC_MANUAL_POSITION), !IsInCreate());
		EnableWindow(GetDlgItem(hDriverParams,IDC_CONTROLLER_POS), !IsInCreate());
		EnableWindow(GetDlgItem(hDriverParams,IDC_WEATHER_DATA_FILE), !IsInCreate());

		TimeValue t = GetCOREInterface()->GetTime();
		CheckDlgButton(hDriverParams, IDC_MANUAL_POSITION,
			manual== NatLightAssembly::MANUAL_MODE);
		CheckDlgButton(hDriverParams, IDC_CONTROLLER_POS,
			manual == NatLightAssembly::DATETIME_MODE);
		CheckDlgButton(hDriverParams, IDC_WEATHER_DATA_FILE,
		...
		if(manual==NatLightAssembly::WEATHER_MODE) //only enable setup with weather file.
			EnableWindow(GetDlgItem(hDriverParams,IDC_SETUP),TRUE);
		else
			EnableWindow(GetDlgItem(hDriverParams,IDC_SETUP),FALSE);

		if (daylightSystem)
		{
		}

		EnableWindow(GetDlgItem(hDriverParams,IDC_DST), dateTime);
		EnableWindow(GetDlgItem(hDriverParams,IDC_GETLOC), dateTime);
		...
		CheckDlgButton(hDriverParams,IDC_DST, GetDst(t));
		TCHAR buf[10];
		_itot_s(static_cast<int>(rtd(az)), buf, _countof(buf), 10);
		if(azEdit) azEdit->SetText(buf);
		_itot_s(static_cast<int>(rtd(alt)), buf, _countof(buf), 10);
...
// The driver controller of a system plug-in should implement this 
// method to give MAX a list of nodes that are part of the system.   
// The driver controller should fill in the given table with the 
...
// tmController of each selected node to retrieve the driver 
// controller and then call GetSystemNodes() on the driver 
// controller to get the list of nodes.
void SunDriver::GetSystemNodes(INodeTab &nodes, SysNodeContext c)
{
	if (thePoint) {
...
RefTargetHandle SunDriver::GetReference(int i)  
{ 
	switch (i) 
	{
	case REF_PARAM:
...
void SunDriver::SetReference(int i, RefTargetHandle rtarg) 
{
	switch (i) 
	{
	case REF_PARAM:
...
BOOL SunDriver::GetNextKeyTime(TimeValue t,DWORD flags,TimeValue &nt)
{
...
// This method gets called by each driven controller and based on the drivens
// ID, it is free to do whatever it wants. In the current system there is only 
// the driven controller of the light.
void SunDriver::GetValue(TimeValue t, void *val, Interval &valid, GetSetMethod method,
	int id) {
		Matrix3 tmat;
		Matrix3 *mat;
		tmat.IdentityMatrix();
...
float SunDriver::calcIntensity(TimeValue t, Interval& valid)
{
...
void SunDriver::SetValue(TimeValue t, void *val, int commit, GetSetMethod method, int id)
{
	switch (id) {
	case LIGHT_TM:
		...
		if (GetManual(t)==NatLightAssembly::MANUAL_MODE && theMultiplier != NULL) {
			...
			if (pI != NULL) {
				...
				if (p != NULL) {
					...
					factor = p->GetPhysicalUnit(t, dummy_interval);
				}
			}
...
RefResult SunDriver::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
	PartID& partID, RefMessage message, BOOL propagate )
{
	...
	case REFMSG_GET_PARAM_NAME_LOCALIZED: {
		...
		switch (gpn->index) {
		case PB_DATE: gpn->name = MaxSDK::GetResourceStringAsMSTR(IDS_SOL_DATE);  break;
		case PB_TIME: gpn->name = MaxSDK::GetResourceStringAsMSTR(IDS_SOL_TIME);  break;
		case PB_RAD:  gpn->name = MaxSDK::GetResourceStringAsMSTR(IDS_RAD);       break;
		case PB_LAT:  gpn->name = MaxSDK::GetResourceStringAsMSTR(IDS_LAT);       break;
		case PB_LONG: gpn->name = MaxSDK::GetResourceStringAsMSTR(IDS_LONG);      break;
		case PB_SKY:  gpn->name = MaxSDK::GetResourceStringAsMSTR(IDS_SKY_COVER); break;
		}
		return REF_HALT;
	}
	...
	case REFMSG_GET_PARAM_NAME_NONLOCALIZED: {
		GetParamName *gpn = (GetParamName*)partID;
		switch (gpn->index) {
		case PB_DATE: gpn->name = _T("Solar Date");    break;
		case PB_TIME: gpn->name = _T("Solar Time");    break;
		case PB_RAD:  gpn->name = _T("Orbital Scale"); break;
		case PB_LAT:  gpn->name = _T("Latitude");      break;
		case PB_LONG: gpn->name = _T("Longitude");     break;
		case PB_SKY:  gpn->name = _T("Sky Cover");     break;
		}
		return REF_HALT;
	}
...
// Sets the city name.
void SunDriver::SetCity(const TCHAR* name)
{
	if (name != NULL)
	{
		_tcsncpy_s(city, _countof(city), name, _TRUNCATE);
	}
	else
	{
		*city = _T('\0');
...
void SunDriver::calculateCity(TimeValue t)
{
	if(controlledByWeatherFile==false)
	{
		...
		for (int i = 0; i < citylist.Count(); ++i)
		{
				...
				fabs(GetLong(t) - citylist[i].longitude) < eps)
			{
				...
				if (cityName[0] == _T('+') || cityName[0] == _T('*'))
				{
					cityName = &cityName[1];
				}
				break;
...
void SunDriver::SetRad(TimeValue t, float r) { 
	pblock->SetValue( PB_RAD, t, r );
}

float SunDriver::GetRad(TimeValue t, Interval& valid ) { 	
	float f;
...
void SunDriver::SetLat(TimeValue t, float r) { 
	pblock->SetValue( PB_LAT, t, r );
}

float SunDriver::GetLat(TimeValue t, Interval& valid ) { 	
	float f;
	pblock->GetValue( PB_LAT, t, f, valid );
	return f;
}

void SunDriver::SetLong(TimeValue t, float r) { 
	pblock->SetValue( PB_LONG, t, r );
}

float SunDriver::GetLong(TimeValue t, Interval& valid ) { 	
	float f;
...
void SunDriver::SetZone(TimeValue t, int h) { 
	pblock->SetValue( PB_ZONE, t, h );
}

int SunDriver::GetZone(TimeValue t, Interval& valid ) { 	
	int i;
	pblock->GetValue( PB_ZONE, t, i, valid );
	return i;
}

void SunDriver::SetDst(TimeValue t, BOOL h) { 
	pblock->SetValue( PB_DST, t, h );
}

BOOL  SunDriver::GetDst(TimeValue t, Interval& valid ) { 	
	BOOL b;
	pblock->GetValue( PB_DST, t, b, valid );
	return b;
}

void SunDriver::SetManual(TimeValue t, int h) { 
	if (daylightSystem) {
		int oldManual; 
...
int  SunDriver::GetManual(TimeValue t, Interval& valid ) {
...
void SunDriver::SetSkyCondition(TimeValue t, float sky)
{
	if (daylightSystem)
		pblock->SetValue( PB_SKY, t, sky );
}

float SunDriver::GetSkyCondition(TimeValue t, Interval& valid)
{
	if (daylightSystem) {
		float f;
...
void SunDriver::SetNorth(TimeValue t, float r) { 
	if (thePoint) align_north(t,r);
}

float SunDriver::GetNorth(TimeValue t, Interval& valid ) { 	
...
//
void SunDriver::SetDate(TimeValue t, const JulianCalendar& jd) { 
	double t1;
...
void SunDriver::SetTime(TimeValue t, const JulianCalendar& jd) { 
...
void SunDriver::GetTime(TimeValue t, Interval& valid ) {
	if(0){
...
void SunDriver::SetHour(TimeValue t, int h) { 
	theTime.wHour = h;
	SetTime(t,fusetime(theTime));
}

int SunDriver::GetHour(TimeValue t, Interval& valid ) { 	
	GetTime(t,valid);//throw away return, we want side-effect
	return theTime.wHour;
}

void SunDriver::SetMin(TimeValue t, int h) { 
	theTime.wMinute = h;
	SetTime(t,fusetime(theTime));
}

int SunDriver::GetMin(TimeValue t, Interval& valid ) { 	
	GetTime(t,valid);//throw away return, we want side-effect
	return theTime.wMinute;
}

void SunDriver::SetSec(TimeValue t, int h) { 
	theTime.wSecond = h;
	SetTime(t,fusetime(theTime));
}

int SunDriver::GetSec(TimeValue t, Interval& valid ) { 	
	GetTime(t,valid);//throw away return, we want side-effect
	return theTime.wSecond;
}

void SunDriver::SetMon(TimeValue t, int h) { 
	theTime.wMonth = h;
	SetDate(t,fusetime(theTime));
}

int SunDriver::GetMon(TimeValue t, Interval& valid ) { 	
	GetTime(t,valid);//throw away return, we want side-effect
	return theTime.wMonth;
}

void SunDriver::SetDay(TimeValue t, int h) { 
	theTime.wDay = h;
	SetDate(t,fusetime(theTime));
}

int SunDriver::GetDay(TimeValue t, Interval& valid ) { 	
	GetTime(t,valid);//throw away return, we want side-effect
	return theTime.wDay;
}

void SunDriver::SetYr(TimeValue t, int h) { 
	theTime.wYear = h;
	SetDate(t,fusetime(theTime));
}

int SunDriver::GetYr(TimeValue t, Interval& valid ) { 	
	GetTime(t,valid);//throw away return, we want side-effect
	return theTime.wYear;
}

void SunDriver::SetWeatherMDYHMS(TimeValue t,int month,int day, int year, int hour, int minute, int second,BOOL dst)
{
...
Point2 SunDriver::GetAzAlt(TimeValue t, Interval& valid ) {
	Point2 result;
	GetTime(t,valid);
	calculate(t,FOREVER);
	result.x = (float) az;
...
void SunDriver::calculate(TimeValue t, Interval& valid){
...
//method to rotate the helper object from the UI of the driver controller
void SunDriver::align_north(TimeValue now, float north){
	assert(thePoint);
...
void SunDriver::NotifyNatLightManualChanged(IDaylightSystem2::DaylightControlType type)
{
	TimeValue t = GetCOREInterface()->GetTime();
	Interface* ip = GetCOREInterface();
	INodeTab nodes;
	...
	for(int i=0;i<nodes.Count();++i)
	{
		if(nodes[i])
		{
			...
			if(bi)
			{
				...
				nLA->SunDriverManualChanged(type);
			}
		}
	}

}
void SunDriver::ValuesUpdated()
{
...
INT_PTR CALLBACK MainParamDialogProc( HWND hDlg, UINT message, 
	WPARAM wParam, LPARAM lParam )
{
	TCHAR buf[10];
	SunDriver *mc = DLGetWindowLongPtr<SunDriver *>( hDlg);
	...
	case WM_INITDIALOG: {
		...
		// reset these copies
		holdLastCity = -1;
		_tcsncpy(holdLastName, mc->GetCity(), _countof(holdLastName));
		...
		_itot_s(static_cast<int>(rtd(mc->Getaz())), buf, _countof(buf), 10);
		mc->azEdit->SetText(buf);
		_itot_s(static_cast<int>(rtd(mc->Getalt())), buf, _countof(buf), 10);
		...
		if (pBut)
		{
			if (!mc->hIconImages)
			{
				...
				LoadMAXFileIcon(_T("daylight"), mc->hIconImages, kBackground, FALSE);
			}	
			if(mc->hIconImages)
			{
	...
	case CC_SPINNER_BUTTONUP:
		if (!HIWORD(wParam) && message != WM_CUSTEDIT_ENTER)
		{
			...
			if ((holdLastCity != -1 || *holdLastName != _T('\0')) && theHold.Holding())
				theHold.Put(new CityRestore(mc, holdLastName));
			_tcsncpy(holdLastName, mc->GetCity(), _countof(holdLastName));
			holdLastCity = -1;
			theHold.Accept(MaxSDK::GetResourceStringAsMSTR(IDS_UNDO_PARAM).data());
		}
		return TRUE;
		break;
	...
	case WM_COMMAND:
		{
			...
			case IDC_DST:
				theHold.Begin();
				mc->SetDst(now,IsDlgButtonChecked(mc->hDriverParams,IDC_DST));
				theHold.Accept(MaxSDK::GetResourceStringAsMSTR(IDS_UNDO_PARAM).data());
				break;
			case IDC_GETLOC: 
				...
				MSTR city;
				if (doLocationDialog(hDlg, &lat, &longi, city)) {
					// the "holdLastCity" string may be out of sync with the driver
					// since it only shows the currently selected driver regardless
					...
					_tcsncpy_s(holdLastName, _countof(holdLastName), city, _TRUNCATE);
				}
				break;
			}
			INodeMonitor* nodeMon = NULL;
...
// This method is called when the sun drivers parameters may be edited
// in the motion branch of the command panel.  
void SunDriver::BeginEditParams( IObjParam *ip, ULONG flags,Animatable *prev )
{
	...
	if ( !hDriverParams  ) {
		...
		hDriverParams = ip->AddRollupPage( 
			...
			MainParamDialogProc,
			MaxSDK::GetResourceStringAsMSTR(IDS_SUN_DLG_NAM).data(), 
			(LPARAM)this
		...
		DLSetWindowLongPtr( hDriverParams, this);    
		/*
		...
		CheckDlgButton(hDriverParams,IDC_DST,temp );
		*/
	}
	//JH 11/20/00
...
// This method is called when the user is finished editing the sun drivers
// parameters in the command panel.		
void SunDriver::EndEditParams( IObjParam *ip, ULONG flags,Animatable *next )
{
...
IOResult SunDriver::Save(ISave *isave) 
{
	ULONG nb;
	IOResult res;
...
class SunDriverPostLoad : public PostLoadCallback {
public:
	SunDriver *bo;
	SunDriverPostLoad(SunDriver *b) {bo=b;}
	void replaceSunlightWithDaylight();
	void proc(ILoad *iload) {
		int sunDriverVersion = bo->daylightSystem
			? SUNDRIVER_VERSION_DAYLIGHT : SUNDRIVER_VERSION_SUNLIGHT;
		int nParams = bo->daylightSystem ? 9 : 7;
		ParamBlockDescID* pDesc = bo->daylightSystem ? desc4Daylight : desc1;
		if (bo->pblock->GetVersion() != sunDriverVersion) {
			switch (bo->pblock->GetVersion()) {
			case 0:
				bo->ReplaceReference(REF_PARAM,
					UpdateParameterBlock(
					desc, 7, bo->pblock,
					pDesc, nParams, sunDriverVersion));
			...
			case 2:
				bo->ReplaceReference(REF_PARAM,
					...
					desc3Daylight, 9, SUNDRIVER_VERSION_DAYLIGHT));
			...
			case 3:
				bo->ReplaceReference(REF_PARAM,
					...
					desc4Daylight, 9, SUNDRIVER_VERSION_DAYLIGHT));
				bo->daylightSystem = true;
				bo->SetLong(TimeValue(0),-1 * bo->GetLong(TimeValue(0)));
				iload->SetObsolete();
				break;
...
IOResult SunDriver::Load(ILoad *iload) 
{
...
IOResult SunDriver::DoLoad(ILoad *iload)
{
	IOResult res;
...
IOResult SunDriver::DoLoadLegacy(ILoad *iload) 
{
	ULONG nb;
	TCHAR	*cp;
	IOResult res;
	...
	mbOldAnimation = TRUE;
	iload->RegisterPostLoadCallback(new SunDriverPostLoad(this));
	while (IO_OK == (res = iload->OpenChunk())) 
	{
		switch (iload->CurChunkID())  
		{
...
void SunDriver::TimeChanged(TimeValue t)
{
	if(controlledByWeatherFile&&natLight!=NULL)
	{
		Interval valid;
...
void DrivenControl::GetClassName(MSTR& s, bool localized) const
{
	if (localized)
	{
		s = MaxSDK::GetResourceStringAsMSTR(id == LIGHT_TM ? IDS_DRIVEN_POS_CLASS : IDS_DRIVEN_FLOAT_CLASS);
	}
	else
	{
		s = (id == LIGHT_TM) ? _T("Sunlight Driven Controller") : _T("Sunlight Driven Intensity Controller");
	}
}

// This constructor creates a reference from the driven controller to 
// the sun driver object.
DrivenControl::DrivenControl(const SunDriver* m, int i) {
...
SClass_ID DrivenControl::SuperClassID()
{
	switch (id) {
	case LIGHT_TM:
		{
...
// This method is called to create a copy of the driven controller.
RefTargetHandle DrivenControl::Clone(RemapDir& remap) {
	DrivenControl *sl = new DrivenControl(daylightSystem);
	sl->id = id;
...
// calls the driver object to get the value.

void DrivenControl::GetValue(TimeValue t, void *val, Interval &valid, 
	GetSetMethod method) {
		// Ensure the ring driver exists.
		assert(driver);
		driver->GetValue(t,val,valid,method,id);	
}

void DrivenControl::SetValue(TimeValue t, void *val, int commit, 
	GetSetMethod method)
{
	driver->SetValue(t, val, commit, method, id);
}
...
const int kDrivenIDChunk = 0x4444;
IOResult DrivenControl::Save(ISave *isave)
{
	isave->BeginChunk(kDrivenIDChunk);
	ULONG nb;
...
class DrivenControlPostLoad : public PostLoadCallback {
public:
	DrivenControlPostLoad(DrivenControl* driven) : mpDriven(driven) {}

	void proc(ILoad* iload) {
		if (mpDriven->driver != NULL) {
			if (mpDriven->daylightSystem != mpDriven->driver->daylightSystem) {
				mpDriven->daylightSystem = mpDriven->driver->daylightSystem;
				iload->SetObsolete();
			}
		}
		delete this;
...
IOResult DrivenControl::Load(ILoad *iload)
{
	...
	iload->RegisterPostLoadCallback(new DrivenControlPostLoad(this));
...
void DrivenControl::GetSunTime( TimeValue t, SYSTEMTIME&	sunt ){
	driver->GetTime(t,FOREVER);
	int hours = driver->GetZone(t,FOREVER);
	if(driver->GetDst(t,FOREVER)) hours--;


	sunt.wYear = driver->theTime.wYear;
	sunt.wMonth = driver->theTime.wMonth;
	sunt.wDay = driver->theTime.wDay;

	sunt.wHour = (driver->theTime.wHour + hours)%24;
	sunt.wMinute = driver->theTime.wMinute;
	sunt.wSecond = driver->theTime.wSecond;
}


void DrivenControl::GetSunLoc(TimeValue t, Point2& origin, Point2& orient){
	origin.x = driver->GetLat(t, FOREVER);
	origin.y = driver->GetLong(t, FOREVER);
	Point2 temp =driver->GetAzAlt(t,FOREVER); 
	orient.x = temp.x;
	orient.y = temp.y;
}
...
#if defined(NO_MOTION_PANEL)
bool SunDriver::IsCreateModeActive()
{
	return SunDriverCreateMode::GetInstance()->IsActive();
}
#endif	// defined(NO_MOTION_PANEL)
