...
static constexpr MapTable mapTable[] = {
	IDS_ASIAN_SUBCONTINENT,	_T("india.map"),
	IDS_AFRICA,				_T("africa.map"),
	IDS_ASIA,				_T("asia.map"),
	IDS_AUSTRALIA,			_T("aust.map"),
	IDS_EUROPE,				_T("europe.map"),
	IDS_NORTH_AMERICA,		_T("namer.map"),
	IDS_SOUTH_AMERICA,		_T("samer.map"),
	IDS_CANADA,				_T("canada.map"),
	IDS_WORLD,				_T("world.map")
};
...
class LocationDialog
{
	...
	private:
		...
		HANDLE openMapFile(const TCHAR* mapName);
...
void LocationDialog::initMapList(HWND h)
{
	...
	{
		TSTR mapName = MaxSDK::GetResourceStringAsMSTR(mapTable[i].index);
		if(!mapName.isNull())
		{
			SendMessage(h, CB_ADDSTRING, 0, (LPARAM)mapName.data());
		}
	}
	...
	// make sure that we have initialized the cityList
	CityList::GetInstance().init();
	...
	const CityList::Entry* defaultCity = CityList::GetInstance().GetDefaultCity();
	if (defaultCity != NULL)
	{
...
// Handle a change of city.
{
	...
	lastCityName = CityList::GetInstance()[itemIndex].get_ui_name();
...
HANDLE LocationDialog::openMapFile(const TCHAR* mapName)
{
	HANDLE hMap = NULL;
	...
	TCHAR *slash = _tcsrchr(fileName, _T('\\'));
	if (slash)
	{
	...
	else
	{
		...
		_tcscpy(fileName,GetCOREInterface()->GetDir(APP_PLUGCFG_DIR).data());
	...
	if (hMap == INVALID_HANDLE_VALUE)
	{
		...
		_tcscpy(fileName,GetCOREInterface()->GetDir(APP_MAXSTART_DIR).data());
		len = static_cast<int>(_tcslen(fileName));   // SR DCAST64: Downcast to 2G limit.
		if (len)
		{
			if (_tcscmp(&fileName[len-1],_T("\\")))
		...
		if (hMap == INVALID_HANDLE_VALUE)
		{
			...
			_tcscpy(fileName,GetCOREInterface()->GetDir(APP_MAX_SYS_ROOT_DIR).data());
			len = static_cast<int>(_tcslen(fileName));   // SR DCAST64: Downcast to 2G limit.
			if (len)
			{
				if (_tcscmp(&fileName[len-1],_T("\\")))
...
// Draw the current map.
{
	...
	if (hDC != NULL)
	{
		...
		// Set a clipping region when painting on the window dc,
		// otherwise parts of the map will be drawn outside 
		// of the actual image button boundaries.
		HRGN clipRegion = CreateRectRgnIndirect( &mapRect );
		if ( clipRegion )
		{
...
// Update the list of cities.
{
	...
	{
		const CityList::Entry& cl = CityList::GetInstance()[i];
		...
		{ 
            const MSTR city_ui_name = cl.get_ui_name();

			if ((cl.name[0] == _T('+')) || (cl.name[0] == _T('*'))) 
			{
				isBig[i] = TRUE;
...
int LocationDialog::mapClick(HWND hDlg, HWND hButton)
{
	...
	{ 
		...
		if (foundClosest)
		{
			...
			lastCityName = CityList::GetInstance()[closestCity].get_ui_name();
