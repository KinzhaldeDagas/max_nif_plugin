...
const TCHAR* CompassRoseObject::GetObjectName(bool localized) const 
{ 
	return localized ? sObjectName.data() : _T("Compass");
}
	...
	TimeValue t, INode* inode, ViewExp* vpt, Box3& box ) 
{
	...
	box = GetCompassRoseAxisBoundingBox(vpt,tm,showAxis?axisLength:0.0f, TRUE);
}
	...
	TimeValue t, INode* inode, ViewExp* vpt, Box3& box )
{
	...
	box = GetCompassRoseAxisBoundingBox(vpt,tm,showAxis?axisLength:0.0f, FALSE);
	assert(!box.IsEmpty());
}
...
int CompassRoseObject::HitTest(TimeValue t, INode *inode, int type, int crossing, int flags, IPoint2 *p, ViewExp *vpt) {
	...
	if (showAxis) 
    {
		DrawCompassRoseAxis(vpt, axisLength, false, false);
	}

	{
		DWORD limits = vpt->getGW()->getRndLimits();
...
int CompassRoseObject::Display(TimeValue t, INode* inode, ViewExp *vpt, int flags) {
	...
	if (showAxis) 
    {
		DrawCompassRoseAxis(vpt,axisLength,inode->Selected(),inode->IsFrozen());
	}
	if(!inode->IsFrozen())
...
RefResult CompassRoseObject::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, 
...
void CompassRoseObject::GetClassName(MSTR& s, bool localized) const
{
	s = localized ? MaxSDK::GetResourceStringAsMSTR(IDS_DB_COMPASS_CLASS) : MSTR(_T("Compass"));
}
