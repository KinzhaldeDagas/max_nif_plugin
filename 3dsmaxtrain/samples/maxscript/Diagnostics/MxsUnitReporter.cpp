...
MSTR MxsUnitReporter::CreateMessage(const MCHAR* frame_name, const MCHAR* expected, const MCHAR* actual, const MCHAR* message, const MCHAR* fileName, unsigned int lineNumber, bool testing_for_equivalence)
{
	MSTR result;

	if (testing_for_equivalence)
	{
		if (NULL == message)
		{
	...
	else
	{
		if (NULL == message)
		{
			result.printf(_M("Assert - Expected value unequal to %s, got %s - (%s) in frame %s, at line %i"), expected, actual, fileName, frame_name, lineNumber);
		}
		else
		{
			result.printf(_M("Assert - %s - Expected value unequal to %s, got %s - (%s) in frame %s, at line %i"), message, expected, actual, fileName, frame_name, lineNumber);
		}
	}
