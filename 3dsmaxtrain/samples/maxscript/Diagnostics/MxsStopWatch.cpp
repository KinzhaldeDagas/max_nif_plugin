...
MxsStopWatch::get_property(Value** arg_list, int count)
{
	...
	if (prop == n_IsRunning)
		...
		return_value (Integer::intern( mWatch.GetTimerCurrentState() ));
	else if (prop == n_ElapsedTime)
		return_value (Double::intern( mWatch.GetElapsedTime() ));
	else
