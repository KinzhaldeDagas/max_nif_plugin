...
const int NUM_VARTYPES = _countof( g_aVarTypes );
		...
		if (SUCCEEDED(hr))
		{
			if(pta->typekind == TKIND_ENUM)
			{
				...
				for (int i=0; i<pta->cVars; i++)
				{
					...
					{
						*pval = Name::intern(TSTR::FromOLESTR(bstrName));
			...
			else if (pta->typekind == TKIND_DISPATCH)
			{
				if (V_VT(var) == VT_DISPATCH && V_DISPATCH(var))
				{
					...
					*pval = new MSDispatch (V_DISPATCH(var), TSTR::FromOLESTR(bstrName).data(), pTypeInfo);
					handled = true;
				}
				else
					*pval = &undefined;
			...
			else if (pta->typekind == TKIND_COCLASS && pta->cImplTypes >0)
			{
				...
				if (pDisp.p)
				{
					CComBSTR bstrName;
					pTypeInfo->GetDocumentation(MEMBERID_NIL, &bstrName, NULL, NULL, NULL);
					*pval = new MSDispatch (pDisp, TSTR::FromOLESTR(bstrName).data(), pTI);
					handled = true;
				}
				else
					*pval = &undefined;				
		...
		case VT_R4:
			{
				...
				DebugPrint(_T("%s\n"), TSTR::FromOLESTR(vDisp.bstrVal).data());
#endif
...
variant_from_value(Value* val, VARIANTARG* var, ITypeInfo* pTypeInfo)
{
	...
	if (pTypeInfo != NULL)
	{
		...
		{	
			if(pta->typekind == TKIND_ENUM)
			{
				CComBSTR name_val(val->to_string());
				VARDESC* pvd=NULL;
				for (int i=0;i<pta->cVars;i++)
				{
					pTypeInfo->GetVarDesc(i, &pvd);
	...
	{
		V_VT(var) = VT_BSTR;
		CComBSTR bstrString(val->to_string());
		BSTR bstrStringPtr;
		// Return a copy of the string.
		bstrString.CopyTo(&bstrStringPtr);      //FIXME, ATL 7.0.
		V_BSTR(var) = bstrStringPtr;			// The destructor for CComBSTR will free the returned BSTR, so...
	}
