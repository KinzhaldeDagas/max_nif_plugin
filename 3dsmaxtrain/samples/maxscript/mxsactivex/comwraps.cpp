...
MSDispatch::MSDispatch(IDispatch* ipdisp, const TCHAR* iname, ITypeInfo* ti)
{
	HRESULT hr;
	...
	if (FAILED(hr)) throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_CREATE_FAILED), iname);
...
MSDispatch::get_vf(Value** arg_list, int count)
{
	...
	arg = arg_list[0];
	if (Number::is_number(arg) && (index = arg->to_int()) < 0)
	...
	HRESULT hr;
	VariantInit(&vIndex);
	TSTR strArg = arg->to_string();
	if (Number::is_number(arg))
	{
...
MSDispatch::map(node_map& m)
{
	...
	hr = m_comObject->m_pdisp.GetPropertyByName(TSTR(_T("Count")).ToOLESTR(), &cVar);
	if (FAILED(hr)) return &undefined;
	cElements = V_I4(&cVar);
	...
	three_value_locals_tls(index, result, arg_save);
...
local_visible_class_instance(MSComMethod, "MSComMethod")

MSComMethod::MSComMethod(const TCHAR* iname, DISPID id, MSComObject* icom_obj, CMethodInfo* pMethodInfo)
{
