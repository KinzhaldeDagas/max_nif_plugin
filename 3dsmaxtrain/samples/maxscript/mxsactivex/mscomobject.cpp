...
void activex_init()
{
	install_rollout_control(Name::intern(_T("ActiveXControl")), ActiveXControl::create);
	define_system_global ( _T("enableAccelerators"), get_enable_accel,	set_enable_accel);	
}


#define def_create_fail()	throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_CREATE_FAILED), caption);
...
#define def_prop_error(gs,p)															\
	throw RuntimeError(_T("Cannot ") _T(#gs) _T(" the property, ."), p->to_string());
...
ActiveXControl::check_hr_error(HRESULT hr)
{
	if ( FAILED(hr))
	{
		LPVOID lpMsgBuf;
		TSTR msg;
		MaxSDK::GetResourceStringAsMSTR(IDS_CREATE_FAILED, msg);
		msg.append(caption->to_string());
		msg.append(MaxSDK::GetResourceStringAsMSTR(IDS_REPORT_ERR_MSG));
...
// ActiveXControl
/////////////////////////////////////////////////////////////////////////////
local_visible_class_instance (ActiveXControl, "ActiveXControl")
void
ActiveXControl::add_control(Rollout *ro, HWND parent, HINSTANCE hInst, int& current_y)
{
	...
	const TCHAR*	control_text = caption->eval()->to_string();

	MAXScript::ValidateCanRunMAXScriptSystemCommand(_T("ActiveXControl"), (TSTR(_T("\"")) + control_text + _T("\"")).data());
	...
	rect.left = MAXScript::GetUIScaledValue(pos.left);
	rect.top = MAXScript::GetUIScaledValue(pos.top);
	rect.bottom = MAXScript::GetUIScaledValue(pos.top + pos.height);
	rect.right = MAXScript::GetUIScaledValue(pos.left + pos.width);
...
// apply proper iCharOffset
	...
	TSTR strCtr = control_text;
#else
	TSTR strCtr = control_text+(8-iCharOffset);
#endif
	LPCOLESTR pszName = strCtr.ToOLESTR();
	...
	if (SUCCEEDED(hr))
	{
		...
		if (!li.fRuntimeKeyAvail) 
		{
			...
			if (m_ax.IsWindow())
				m_ax.DestroyWindow();
			throw RuntimeError((MaxSDK::GetResourceStringAsMSTR(IDS_UNLICENSED_CONTROL)), caption);
			return;
		}
	}
	...
	Value* events = _get_control_param(keyparms, keyparm_count, Name::intern(_T("setupEvents")));
	if (events == &unsupplied || events->to_bool())
		m_comObject->SetupEvents(true);

	Value* roc = _get_control_param(keyparms, keyparm_count, Name::intern(_T("releaseOnClose")));
	m_releaseOnClose = (roc == &unsupplied || roc->to_bool()) ? true : false;
}

void ActiveXControl::adjust_control(int& current_y)
{
	if (parent_rollout == nullptr || parent_rollout->page == nullptr)
		return;

	// set up control bounds into layout data pos
	// current_y contains current running y pos in panel, pos.top is initialized to
	// that value + standard vertical inter-control spacing
	// the control is asked to fill in pos.left, pos.right with width, pos.bottom with height
	// and then any supplied layout parameters are applied
	layout_data pos;
...
ActiveXControl::get_property(Value** arg_list, int count)
{
	...
	if ( prop == n_size )
	{
		int w = MAXScript::GetValueUIUnscaled(rect.right-rect.left);
		int h = MAXScript::GetValueUIUnscaled(rect.bottom-rect.top);
		return_value_tls (new Point2Value(w, h));
	}
	else if ( prop == n_pos )
	{
		int x = MAXScript::GetValueUIUnscaled(rect.left);
		int y = MAXScript::GetValueUIUnscaled(rect.top);
		return_value_tls (new Point2Value(x, y));
	}
	else if ( prop == n_description )
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			desc.printf(_T("%s (%s)?%s-%lu"), TSTR::FromOLESTR(bstrName).data(), TSTR::FromOLESTR(bstrDoc).data(),
				(!bstrHelp) ? _T("") : TSTR::FromOLESTR(bstrHelp).data(), lHelpContext);
		}
		return_value_tls (new String(desc));
	}
	else if (prop == n_hwnd)
	{
		one_typed_value_local_tls(Array* hwnds);
		if (parent_rollout != NULL && parent_rollout->page != NULL && m_ax.m_hWnd && IsWindow(m_ax.m_hWnd))
		{
			vl.hwnds = new Array(1);
			vl.hwnds->append(IntegerPtr::intern((INT_PTR)m_ax.m_hWnd));
		}
		else
			vl.hwnds = new Array(0);
		return_value_tls(vl.hwnds);
	}
	...
	try
	{
		MAXScriptException::ScopedMXSCallstackCaptureDisable scopedMXSCallstackCaptureDisable(_tls);
...
ActiveXControl::set_property(Value** arg_list, int count)
{
	...
	if ( prop == n_size )
	{
		if (m_ax.m_hWnd && IsWindow(m_ax.m_hWnd)) 
		{
			IPoint2 size = to_ipoint2(val);
			rect.right = rect.left + MAXScript::GetUIScaledValue(size.x);
			rect.bottom = rect.top + MAXScript::GetUIScaledValue(size.y);
			m_ax.MoveWindow(&rect);			
		}
		return val;
	}
	else if ( prop == n_pos )
	{
		if (m_ax.m_hWnd  && IsWindow(m_ax.m_hWnd)) 
		{
			IPoint2 pos = to_ipoint2(val);
			rect.left = MAXScript::GetUIScaledValue(pos.x);
			rect.top = MAXScript::GetUIScaledValue(pos.y);
			m_ax.MoveWindow(&rect);
		}
		return val;
	}
	...
	{
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_READ_ONLY_PROPERTY), _T("description"));
		return &undefined;
	}
...
void
MSComObject::AxError(const TCHAR* icap, const TCHAR* got)
{
	if (thread_local(try_mode)) // Don't display and throw an error instead
	{
		throw RuntimeError(icap);
	}
	...
	if (m_parent && !(m_parent->parent_rollout->flags & RO_SILENT_ERRORS))
	{
		...
		listener_message(MXS_ERROR_MESSAGE_BOX, (WPARAM)save_string(_T("MAXScript")), (LPARAM)save_string(cap), FALSE);
	}
}
	...
	{		
		...
		if (pMethodInfo)
		{
			...
			else
			{
				...
				DISPPARAMS dispparams = {(count>1) ? &vIndex : NULL, NULL, (count>1) ? 1U : 0U, 0U};
		...
		else
		{
			...
			TSTR lname = prop->to_string();
			LPCOLESTR pname = lname.ToOLESTR();
			DISPID dispid;
...
MSComObject::is_readonly_vf(Value** arg_list, int count)
{
	...
	{
		error_message_box(e, MaxSDK::GetResourceStringAsMSTR(IDS_ROLLOUT_HANDLER));
	}
	catch (...)
	{
		def_prop_error(get, prop);
...
MSComObject::set_property(Value** arg_list, int count)
{
	...
	{	
		...
		if (!pMethodInfo)
		{
			...
			hr = m_pdisp.PutPropertyByName(TSTR(prop->to_string()).ToOLESTR(), &var);
...
MSComObject::show_props_vf(Value** arg_list, int count)
{
	...
	if ((out = (CharStream*)key_arg(to)) != (CharStream*)&unsupplied)
	{
		if (out == (CharStream*)&undefined)   // to:undefined -> no output, return true or false if prop found
			...
			throw TypeError (MaxSDK::GetResourceStringAsMSTR(IDS_SHOWPROPERTIES_NEED_STREAM), out);
	}
	else
...
MSComObject::show_methods_vf(Value** arg_list, int count)
{
	...
	if ((out = (CharStream*)key_arg(to)) != (CharStream*)&unsupplied)
	{
		...
		if (!is_charstream(out))
			throw TypeError (MaxSDK::GetResourceStringAsMSTR(IDS_SHOWMETHODS_NEED_STREAM), out);
	}
	else
		out = thread_local(current_stdout);
...
MSComObject::map_point_vf(Value** arg_list, int count)
{
	...
	check_gen_arg_count_with_keys(mapPoint, 2, count + 1);
	IPoint2 p = to_ipoint2(arg_list[0]);
	Value* v = nullptr;
	...
	if (applyUIScaling)
	{
		point.x = MAXScript::GetValueUIUnscaled(point.x);
		point.y = MAXScript::GetValueUIUnscaled(point.y);
	}
...
MSComObject::get_CursorPos_vf(Value** arg_list, int count)
{
	...
	check_gen_arg_count_with_keys(getCursorPos, 1, count + 1);
	POINT p;
	GetCursorPos(&p);
...
MSComObject::show_events_vf(Value** arg_list, int count)
{
	...
	if ((out = (CharStream*)key_arg(to)) != (CharStream*)&unsupplied)
	{
		if (out == (CharStream*)&undefined)   // to:undefined -> no output, return true or false if prop found
			out = NULL;
		if (!is_charstream(out))
			throw TypeError (MaxSDK::GetResourceStringAsMSTR(IDS_SHOWEVENTS_NEED_STREAM), out);
	}
	else
		out = thread_local(current_stdout);
	...
	if	(pdispparams)
	{
		...
		if (mi) 
		{
			...
			if ( handler != &undefined)
			{
				...
				value_local_array_tls(arg_list, cArgs);
				value_local_array_tls(ref_list, cArgs);
...
CEventListener::SetupEvents(bool advise)
{
	...
	if( SUCCEEDED( hr ) )
	{
		...
		else
		{
			...
			{
				m_sink_succeed = false; // following may delete this instance, set member variable first
...
show_all_axcontrols_cf(Value** arg_list, int count)
{
	...
	{
		if (!is_charstream(out))
			throw TypeError (MaxSDK::GetResourceStringAsMSTR(IDS_SHOWALLAXCONTROLS_NEED_STREAM), out);
	}
	else
		out = thread_local(current_stdout);
	...
	if( FAILED( hResult ) ) throw RuntimeError( MaxSDK::GetResourceStringAsMSTR(IDS_SHOWALLAXCONTROLS_FAILED) );
	...
	while( !tDone )
	{
		...
		if( hResult == S_OK )
		{
			...
			if( SUCCEEDED( hResult ) )
			{
				...
				out->printf(_T("\"%s\" \"%s\" \"%s\" \n"), TSTR::FromOLESTR(pszName).data(), TSTR::FromOLESTR(pszProgid).data(), TSTR::FromOLESTR(pszClsid).data());
...
#define def_fwd_generic(fn, name)													\
	...
	{																				\
		...
		else throw NoMethodError (_M(name), evald_args[0]);								\
		return result;																\
	}
...
load_picture_cf(Value** arg_list, int count)
{
	...
	const MSTR scripts_dir = path_mgr->GetDir(APP_SCRIPTS_DIR);
	const MSTR userScripts_dir = path_mgr->GetDir(APP_USER_SCRIPTS_DIR);
	const MSTR image_dir = path_mgr->GetDir(APP_IMAGE_DIR);
	const MSTR startup_dir = path_mgr->GetDir(APP_STARTUPSCRIPTS_DIR);
	const MSTR userStartup_dir = path_mgr->GetDir(APP_USER_STARTUPSCRIPTS_DIR);
	const MSTR icon_dir = GetColorManager()->GetIconFolder();
	const MSTR userIcon_dir = path_mgr->GetDir(APP_USER_ICONS_DIR);

	MaxSDK::Util::Path maxSysIconFolder = path_mgr->GetDir(APP_MAX_SYS_ROOT_DIR); // maxroot
		
	// This gets you \UI_LN\Icons
	maxSysIconFolder.Append(_T("\\UI_LN\\Icons"));

	TCHAR source_dir[MAX_PATH] = _T("");
	if ( thread_local(source_file) )
		BMMSplitFilename(thread_local(source_file)->to_string(), source_dir, _countof(source_dir), NULL, 0, NULL, 0);
	...
	BOOL found = MSZipPackage::search_current_package(fileName, path) ||
		...
		SearchPath(maxSysIconFolder.GetCStr(), fileName, NULL, MAX_PATH, path, &fpart) ||
	...
	if (!found)
		for (int i = 0; i < TheManager->GetMapDirCount(); i++){
			found = SearchPath(TheManager->GetMapDir(i).data(), fileName, NULL, MAX_PATH, path, &fpart);
			if (found == TRUE)
				break;
		}
	if (!found)
	...
	if (found)
	{
		...
		VARIANT var;
		V_VT(&var) = VT_BSTR;	
		TSTR strPath =TSTR(path); 
		V_BSTR(&var) =strPath.ToBSTR();
...
{
	// Listview.hittest <hWnd> <point2_pos> applyUIScaling:<true>
	check_arg_count_with_keys(hitTest, 2, count);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(Array* result);
	...
	IPoint2 p = to_ipoint2(arg_list[1]);
	Value* v = nullptr;
	BOOL applyUIScaling = bool_key_arg(applyUIScaling, v, TRUE);
	if (applyUIScaling)
	{
...
get_indent_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(item.iIndent));
}
...
{
	check_arg_count(addChild, 2, count);
