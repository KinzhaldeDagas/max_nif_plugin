...
GetPolygonCount_cf(Value** arg_list, int count)
{
	...
	one_typed_value_local_tls (Array* total) 
...
GetTriMeshFaceCount_cf(Value** arg_list, int count)
{
	...
	GetTriMeshFaceCount (MAXScript_time_tls(), refobj, faces, vertnum);
...
NumMapsUsed_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern (refobj->NumMapsUsed()));
}
...
PointSelection_cf(Value** arg_list, int count)
{
	...
	return_value (Float::intern(refobj->PointSelection(pointindex)));
}
...
NumSurfaces_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(refobj->NumSurfaces (MAXScript_time_tls())));
...
IsSurfaceUVClosed_cf (Value** arg_list, int count)
{
	...
	if (surfnum < 1 || surfnum > refobj->NumSurfaces(MAXScript_time_tls())) 
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_RK_ISSURFACEUVCLOSED_INDEX_OUT_OF_RANGE));

	refobj->SurfaceClosed (MAXScript_time_tls(), surfnum-1, inu, inv);
...
AverageSelVertNormal_cf(Value** arg_list, int count)
{
	...
	/* get the associated mesh and return the average center of the selected vertices */
	Mesh &themesh = ((TriObject *) obj)->GetMesh();
	return_value( new Point3Value(AverageSelVertNormal ( themesh ) ));
...
Value* GetPatchSteps_cf(Value** arg_list, int count)
{
	...
	if (obj->ClassID() != Class_ID(PATCHOBJ_CLASS_ID,0)) 
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_RK_PATCH_OPERATION_ON_NONMESH), obj->GetObjectName(true));

		//throw RuntimeError (_T("Patch operation on non-Patch: "), obj->GetObjectName(true));

	return_value (Integer::intern( ((PatchObject*)obj)->GetMeshSteps()));
}
...
Value* GetEulerQuatAngleRatio_cf (Value** arg_list, int count)
{
	...
	if (theangle == &unsupplied)
		...
		angle = ConvertValueToID(eulerAngles, elements(eulerAngles), theangle); 
	...
	else if (is_thunk(arg2))
	{
		...
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		one_typed_value_local_tls(EulerAnglesValue* eulerValue);
