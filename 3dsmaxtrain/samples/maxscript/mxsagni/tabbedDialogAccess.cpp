...
static Class_ID Value_to_Dialog_ClassID(Value* arg)
{
	if (is_array(arg))
	{
		...
		if (theArray->size != 2) 
			throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_EXPECTED_TWO_ELEMENT_INT_ARRAY_OR_TABBED_DIALOG_NAMEVAL),arg);
...
static Class_ID Value_to_Page_ClassID(Value* arg)
{
	if (is_array(arg))
	{
		Array* theArray = (Array*)arg;
		if (theArray->size != 2) 
			throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_EXPECTED_TWO_ELEMENT_INT_ARRAY_OR_INT),arg);
		return Class_ID(theArray->data[0]->to_int(), theArray->data[1]->to_int());
	}
	else
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_EXPECTED_TWO_ELEMENT_INT_ARRAY_OR_INT),arg);
...
{
	ITabPage *page = NULL;
	if (Number::is_integer_number(arg))
	{
		int index = arg->to_int();
		MXS_range_check(index, 1, dialog->GetNbPages(), MaxSDK::GetResourceStringAsMSTR(IDS_DIALOG_PAGE_INDEX_OUT_OF_RANGE));
...
tabbedDialogs_getDialogID_cf(Value** arg_list, int count)
{
	...
	if (dialog)
	{
		...
		vl.result->append(Integer64::intern(cid.PartA()));
		vl.result->append(Integer64::intern(cid.PartB()));
...
tabbedDialogs_getNumPages_cf(Value** arg_list, int count)
{
	...
	ITabbedDialog *dialog = manager->GetTabbedDialog(Value_to_Dialog_ClassID(arg_list[0]));
	if (dialog)
		return_value (Integer::intern(dialog->GetNbPages()));
	return &undefined;
}
