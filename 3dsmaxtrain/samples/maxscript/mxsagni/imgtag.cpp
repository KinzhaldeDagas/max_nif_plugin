...
protected:
	...
	bool     m_doDisplayTransform;  // applies only when specifying bitmap, not iconName
...
void ImgTag::run_event_handler (Value* event, int xPos, int yPos, int fwKeys, bool capture)
{
	...
	if (event_handlers && (handler = event_handlers->get(name)) != NULL && (parent_rollout->flags & RO_CONTROLS_INSTALLED))
	{
		...
		ScopedPushPlugin scopedPushPlugin(parent_rollout->current_plugin, _tls);
		ScopedPushStruct scopedPushStruct(parent_rollout->current_struct, _tls);
...
int ImgTag::SetBitmap(Value* val)
{
	...
	else
	{
		...
		HDC hDC = GetDC(hWnd);
		PBITMAPINFO bmi = mbm->bm->ToDib(32, NULL, FALSE, m_doDisplayTransform);
		if(m_hBitmap) 
			DeleteObject(m_hBitmap);
...
void ImgTag::add_control(Rollout *ro, HWND parent, HINSTANCE hInstance, int& current_y)
{
	...
	m_doDisplayTransform = bool_control_param(doDisplayTransform, v, true);
	...
	if(val != &unsupplied)
	{
		type_check(val, String, _T("set .bitmap"));
		SetBitmap(val);
	}
