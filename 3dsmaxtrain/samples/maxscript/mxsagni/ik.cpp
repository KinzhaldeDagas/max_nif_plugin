...
ik_getAxisActive_cf(Value** arg_list, int count)
{
	...
	def_IK_joint_types();
	int whichType = ConvertValueToID(IKJointTypes, elements(IKJointTypes), arg_list[1]);
	...
	return_value_tls( new BitArrayValue (res) );
}
...
ik_setAxisActive_cf(Value** arg_list, int count)
{
	...
	type_check(newVal, BitArrayValue, _T("setAxisActive"))
		for (int i = 0; i <3; i++) 
...
ik_setAxisLimit_cf(Value** arg_list, int count)
{
	...
	BitArrayValue* newVal = (BitArrayValue*)arg_list[2];
	type_check(newVal, BitArrayValue, _T("setAxisLimit"))
		for (int i = 0; i <3; i++) 
...
ik_setAxisEase_cf(Value** arg_list, int count)
{
	...
	BitArrayValue* newVal = (BitArrayValue*)arg_list[2];
	type_check(newVal, BitArrayValue, _T("setAxisEase"))
		for (int i = 0; i <3; i++) 
...
ik_getAxisMin_cf(Value** arg_list, int count)
{
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	return_value_tls( new Point3Value (res) );
}
...
ik_getAxisDamping_cf(Value** arg_list, int count)
{
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	return_value_tls( new Point3Value (workingData->damping) );
}
...
ik_setAxisDamping_cf(Value** arg_list, int count)
{
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	return_value_tls( new Point3Value (oldVal) );
}
...
ik_setAxisSpringOn_cf(Value** arg_list, int count)
{
	...
	if (data3)
	{
		...
		BitArrayValue* newVal = (BitArrayValue*)arg_list[2];
		type_check(newVal, BitArrayValue, _T("setAxisSpringOn"))
			for (int i = 0; i <3; i++) 
...
ik_getAxisSpring_cf(Value** arg_list, int count)
{
	...
	{
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		return_value_tls(new Point3Value(data3->spring));
	}
	else
		return &undefined;
}
...
ik_getAxisSpringTension_cf(Value** arg_list, int count)
{
	...
	{
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		return_value_tls(new Point3Value(data3->springTension));
	}
	else
		return &undefined;
}
...
ik_getStartTime_cf (Value** arg_list, int count)
{
	...
	// LAM - defect ??? 
	ReferenceTarget* ref = (controller) ? (ReferenceTarget*)controller->GetInterface(I_DRIVER) : NULL;
	if (ref && ref->ClassID() == IKDRIVER_CLASSID)
	{
		IKDriverControl* theDriver = (IKDriverControl*)ref;
		return_value (MSTime::intern(theDriver->GetStartTime()));
	}
	else
		return &undefined;
}
...
ik_setStartTime_cf (Value** arg_list, int count)
{
	...
	{
		IKDriverControl* theDriver = (IKDriverControl*)ref;
		theDriver->SetStartTime(arg_list[1]->to_timevalue());
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
...
ik_getEndTime_cf (Value** arg_list, int count)
{
	...
	{
		IKDriverControl* theDriver = (IKDriverControl*)ref;
		return_value (MSTime::intern(theDriver->GetEndTime()));
	}
	else
		return &undefined;
...
ik_setEndTime_cf (Value** arg_list, int count)
{
	...
	{
		IKDriverControl* theDriver = (IKDriverControl*)ref;
		theDriver->SetEndTime(arg_list[1]->to_timevalue());
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		needs_redraw_set(_tls);
		return (arg_list[1]);
	}
...
ik_getIterations_cf (Value** arg_list, int count)
{
	...
	{
		IKDriverControl* theDriver = (IKDriverControl*)ref;
		return_value (Integer::intern(theDriver->GetIterations()));
	}
	else
		return &undefined;
}
...
ik_setIterations_cf (Value** arg_list, int count)
{
	...
	{
		IKDriverControl* theDriver = (IKDriverControl*)ref;
		theDriver->SetIterations (arg_list[1]->to_int());
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		needs_redraw_set(_tls);
		return (arg_list[1]);
	}
...
ik_getPosThreshold_cf (Value** arg_list, int count)
{
	...
	{
		IKDriverControl* theDriver = (IKDriverControl*)ref;
		return_value (Float::intern(theDriver->GetPosThresh()));
	}
	else
		return &undefined;
}
...
ik_setPosThreshold_cf (Value** arg_list, int count)
{
	...
	{
		IKDriverControl* theDriver = (IKDriverControl*)ref;
		theDriver->SetPosThresh(arg_list[1]->to_float());
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		needs_redraw_set(_tls);
		return (arg_list[1]);
	}
...
ik_getRotThreshold_cf (Value** arg_list, int count)
{
	...
	{
		IKDriverControl* theDriver = (IKDriverControl*)ref;
		return_value (Float::intern(RadToDeg(theDriver->GetRotThresh())));
	}
	else
		return &undefined;
...
ik_setRotThreshold_cf (Value** arg_list, int count)
{
	...
	{
		IKDriverControl* theDriver = (IKDriverControl*)ref;
		theDriver->SetRotThresh (DegToRad(arg_list[1]->to_float()));
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		needs_redraw_set(_tls);
		return (arg_list[1]);
	}
