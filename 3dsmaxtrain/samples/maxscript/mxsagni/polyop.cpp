...
polyop_setVertSelection_cf(Value** arg_list, int count)
{
	...
	ValueToBitArrayP(arg_list[1], verts, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_VERTEX_INDEX_OUT_OF_RANGE), MESH_VERTSEL_ALLOWED, poly);
	IMeshSelect* ms = (owner) ? GetMeshSelectInterface(owner) : NULL;
	IMeshSelectData* msd = (owner) ? GetMeshSelectDataInterface(owner) : NULL;
	if (ms && msd) {
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		msd->SetVertSel(verts, ms, MAXScript_time_tls());
		ms->LocalDataChanged();
	}
	return &ok;
}
...
polyop_setEdgeSelection_cf(Value** arg_list, int count)
{
	...
	ValueToBitArrayP(arg_list[1], edges, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_EDGE_INDEX_OUT_OF_RANGE), MESH_EDGESEL_ALLOWED, poly);
	IMeshSelect* ms = (owner) ? GetMeshSelectInterface(owner) : NULL;
	IMeshSelectData* msd = (owner) ? GetMeshSelectDataInterface(owner) : NULL;
	if (ms && msd) {
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		msd->SetEdgeSel(edges, ms, MAXScript_time_tls());
		ms->LocalDataChanged();
	}
	return &ok;
}
...
polyop_setFaceSelection_cf(Value** arg_list, int count)
{
	...
	ValueToBitArrayP(arg_list[1], faces, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_FACE_INDEX_OUT_OF_RANGE), MESH_FACESEL_ALLOWED, poly);
	IMeshSelect* ms = (owner) ? GetMeshSelectInterface(owner) : NULL;
	IMeshSelectData* msd = (owner) ? GetMeshSelectDataInterface(owner) : NULL;
	if (ms && msd) {
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		msd->SetFaceSel(faces, ms, MAXScript_time_tls());
		ms->LocalDataChanged();
	}
	return &ok;
}
...
polyop_getVertFlags_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(vertIndex+1, 1, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_VERTEX_INDEX_OUT_OF_RANGE));
	return_value (Integer::intern(poly->V(vertIndex)->ExportFlags()));
}
...
polyop_getEdgeFlags_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(edgeIndex+1, 1, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_EDGE_INDEX_OUT_OF_RANGE));
	return_value (Integer::intern(poly->E(edgeIndex)->ExportFlags()));
}
...
polyop_getFaceFlags_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(faceIndex+1, 1, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_FACE_INDEX_OUT_OF_RANGE));
	return_value (Integer::intern(poly->F(faceIndex)->ExportFlags()));
}
...
polyop_getNumVerts_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(poly->VNum()));
}
...
polyop_getNumEdges_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(poly->ENum()));
}
...
polyop_getNumFaces_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(poly->FNum()));
}
...
polyop_getBorderFromEdge_cf(Value** arg_list, int count)
{
	...
	if (e->f2 != -1) return_value_tls(vl.edges);  // LAM: 2/7/01 - defect 275470 fix
...
polyop_getFaceVerts_cf(Value** arg_list, int count)
{
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(Array* verts);
...
polyop_getFaceEdges_cf(Value** arg_list, int count)
{
	...
	if (f->GetFlag(MN_DEAD)) return &undefined;
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(Array* edges);
...
///////////////////////
// ============================================================================
// <array_array> getFacesVerts <Poly poly> <facelist>
Value*
polyop_getFacesVerts_cf(Value** arg_list, int count)
{
	check_arg_count(getFacesVerts, 2, count);
	MNMesh* poly = get_polyForValue(arg_list[0], MESH_READ_ACCESS, NULL, getFacesVerts);
	int nFaces = poly->FNum();
	BitArray faces(nFaces);
	ValueToBitArrayP(arg_list[1], faces, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_FACE_INDEX_OUT_OF_RANGE), MESH_FACESEL_ALLOWED, poly);

	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	two_typed_value_locals_tls(Array* result, Array* verts);
...
// ============================================================================
// <array_array> getFacesEdges <Poly poly> <facelist>
Value*
polyop_getFacesEdges_cf(Value** arg_list, int count)
{
	check_arg_count(getFacesEdges, 2, count);
	MNMesh* poly = get_polyForValue(arg_list[0], MESH_READ_ACCESS, NULL, getFacesEdges);
	int nFaces = poly->FNum();
	BitArray faces(nFaces);
	ValueToBitArrayP(arg_list[1], faces, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_FACE_INDEX_OUT_OF_RANGE), MESH_FACESEL_ALLOWED, poly);

	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	two_typed_value_locals_tls(Array* result, Array* edges);
	vl.result = new Array(faces.NumberSet());
	for (int faceIndex = 0; faceIndex < nFaces; faceIndex++)
	{
		if (faces[faceIndex])
...
// ============================================================================
// <array_array> getEdgesVerts <Poly poly> <edgelist>
Value*
polyop_getEdgesVerts_cf(Value** arg_list, int count)
{
	check_arg_count(getEdgesVerts, 2, count);
	MNMesh* poly = get_polyForValue(arg_list[0], MESH_READ_ACCESS, NULL, getEdgesVerts);
	if (!poly->GetFlag(MN_MESH_FILLED_IN)) return &undefined;
	int nEdges = poly->ENum();
	BitArray edges(nEdges);
	ValueToBitArrayP(arg_list[1], edges, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_EDGE_INDEX_OUT_OF_RANGE), MESH_EDGESEL_ALLOWED, poly);
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	two_typed_value_locals_tls(Array* result, Array* verts);
	vl.result = new Array(edges.NumberSet());
...
polyop_getEdgeFaces_cf(Value** arg_list, int count)
{
	...
	if (e->GetFlag(MN_DEAD)) return &undefined;
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(Array* faces);
...
// ============================================================================
// <array_array> getEdgesFaces <Poly poly> <edgelist>
Value*
polyop_getEdgesFaces_cf(Value** arg_list, int count)
{
	check_arg_count(getEdgesFaces, 2, count);
	MNMesh* poly = get_polyForValue(arg_list[0], MESH_READ_ACCESS, NULL, getEdgesFaces);
	if (!poly->GetFlag(MN_MESH_FILLED_IN)) return &undefined;
	int nEdges = poly->ENum();
	BitArray edges(nEdges);
	ValueToBitArrayP(arg_list[1], edges, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_EDGE_INDEX_OUT_OF_RANGE), MESH_EDGESEL_ALLOWED, poly);
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	two_typed_value_locals_tls(Array* result, Array* faces);
	vl.result = new Array(edges.NumberSet());
	for (int edgeIndex = 0; edgeIndex < nEdges; edgeIndex++)
	{
		if (edges[edgeIndex])
...
polyop_getFaceDeg_cf(Value** arg_list, int count)
{
	...
	MNFace* f = poly->F(faceIndex);
	if (f->GetFlag(MN_DEAD)) return &undefined;
	return_value (Integer::intern(f->deg));
}
...
polyop_getFaceMatID_cf(Value** arg_list, int count)
{
	...
	MNFace* f = poly->F(faceIndex);
	if (f->GetFlag(MN_DEAD)) return &undefined;
	return_value (Integer::intern(f->material+1));
}

// ============================================================================
// <array> getFacesMatID <Poly poly> <bitArray faceList>
Value*
polyop_getFacesMatID_cf(Value** arg_list, int count)
{
	check_arg_count(getFacesMatID, 2, count);
	MNMesh* poly = get_polyForValue(arg_list[0], MESH_READ_ACCESS, NULL, getFacesMatID);
	int nFaces = poly->FNum();
	BitArray faces(nFaces);
	ValueToBitArrayP(arg_list[1], faces, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_FACE_INDEX_OUT_OF_RANGE), MESH_FACESEL_ALLOWED, poly);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(Array* matIds);
	vl.matIds = new Array(faces.NumberSet());
	...
	for (int i = 0; i < nFaces; i++)
	{
		...
		{
			if (!poly->f[i].GetFlag(MN_DEAD))
				vl.matIds->append(Integer::intern(poly->F(i)->material + 1));
			else
				vl.matIds->append(&undefined);
		}
	}
...
polyop_getVert_cf(Value** arg_list, int count)
{
	...
	int vertIndex =  arg_list[1]->to_int()-1;
	MXS_range_check(vertIndex+1, 1, poly->VNum(), MaxSDK::GetResourceStringAsMSTR(IDS_POLY_VERTEX_INDEX_OUT_OF_RANGE));
...
// ============================================================================
// <point3_array> getVerts <Poly poly> <vertlist> node:<node>
Value*
polyop_getVerts_cf(Value** arg_list, int count)
{
	check_arg_count_with_keys(getVerts, 2, count);
	MNMesh* poly = get_polyForValue(arg_list[0], MESH_READ_ACCESS, NULL, getVerts);
	int nVerts = poly->VNum();
	BitArray verts(nVerts);
	ValueToBitArrayP(arg_list[1], verts, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_VERTEX_INDEX_OUT_OF_RANGE), MESH_VERTSEL_ALLOWED, poly);
	...
	for (int i = 0; i < nVerts; i++)
	{
		if (verts[i])
		{
			Point3 p = poly->V(i)->p; // LOCAL COORDINATES!!!
...
polyop_setVert_cf(Value** arg_list, int count)
{
	...
	if (epi) 
	{
		...
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->ApplyDelta(delta, epi, MAXScript_time_tls());
	}
	return &ok;
}
...
// ============================================================================
// <BitArray> getVertsByMatId <Poly poly> <int mtlId>
Value*
polyop_getVertsByMatId_cf(Value** arg_list, int count)
{
	check_arg_count(getVertsByMatId, 2, count);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BitArrayValue* result);
	MNMesh* poly = get_polyForValue(arg_list[0], MESH_READ_ACCESS, NULL, getVertsByMatId);
	int mtlID = arg_list[1]->to_int() - 1; //convert to 0 base
	BitArray matVerts(poly->numv);

	for (int i = 0; i < poly->FNum(); i++)
	{
		MNFace* f = poly->F(i);

		if (f->material != mtlID || f->GetFlag(MN_DEAD))
			continue;
...
polyop_getElementsUsingFace_cf(Value** arg_list, int count)
{
	...
	if (fence != &unsupplied) 
		ValueToBitArrayP(fence, fencefaces, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_FACE_INDEX_OUT_OF_RANGE), MESH_FACESEL_ALLOWED, poly);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
...
polyop_getFaceArea_cf(Value** arg_list, int count)
{
	...
	int faceIndex =  arg_list[1]->to_int()-1;
	MXS_range_check(faceIndex+1, 1, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_FACE_INDEX_OUT_OF_RANGE));
		return_value (Float::intern(Length(poly->GetFaceNormal(faceIndex))/2.0f));
}

// ============================================================================
// <BitArray> getFacesByMatIds <Poly poly> <int mtlId>
Value*
polyop_getFacesByMatId_cf(Value** arg_list, int count)
{
	check_arg_count(getFacesByMatId, 2, count);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BitArrayValue* result);
	MNMesh* poly = get_polyForValue(arg_list[0], MESH_READ_ACCESS, NULL, getFacesByMatId);
	int mtlID = arg_list[1]->to_int() - 1; // convert to 0 base.
	vl.result = new BitArrayValue(poly->FNum());

	for (int i = 0; i < poly->FNum(); i++)
	{
		if (mtlID != poly->f[i].material || poly->f[i].GetFlag(MN_DEAD))
			continue;

		vl.result->bits.Set(i);
	}
...
// ============================================================================
// <BitArray> getAllFaces <Poly poly>
Value*
polyop_getAllFaces_cf(Value** arg_list, int count)
{
	check_arg_count(getAllFaces, 1, count);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BitArrayValue* result);
	MNMesh* poly = get_polyForValue(arg_list[0], MESH_READ_ACCESS, NULL, getAllFaces);
	vl.result = new BitArrayValue(poly->FNum());
...
polyop_attach_cf(Value** arg_list, int count)
{
	...
	if (targetNode == NULL)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_POLY_ATTACH_REQUIRES_TARGET_NODE), arg_list[0]);
	...
	if (sourceNode == NULL)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_POLY_ATTACH_REQUIRES_SOURCE_NODE), arg_list[1]);

	if (epi) {
		bool canUndo = true;
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->EpfnAttach (sourceNode, canUndo, targetNode, MAXScript_time_tls());
		if (!canUndo) {
			theHold.Release();
...
polyop_weldVerts_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(v1+1, 1, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_VERTEX_INDEX_OUT_OF_RANGE));
	int v2=arg_list[2]->to_int()-1;
	MXS_range_check(v2+1, 1, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_VERTEX_INDEX_OUT_OF_RANGE));
...
polyop_weldEdges_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(e1+1, 1, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_EDGE_INDEX_OUT_OF_RANGE));
	int e2=arg_list[2]->to_int()-1;
	MXS_range_check(e2+1, 1, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_EDGE_INDEX_OUT_OF_RANGE));
	EPoly* epi = NULL;
	if (owner) {
		epi = (EPoly*)((Animatable*)owner)->GetInterface(EPOLY_INTERFACE);
	}
...
polyop_createPolygon_cf(Value** arg_list, int count)
{
	...
	if (epi) 
		faceIndex = epi->EpfnCreateFace(v.Addr(0), degree);
...
polyop_autoSmooth_cf(Value** arg_list, int count)
{
	...
	{
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->EpfnAutoSmooth(MAXScript_time_tls());
	}
	return &ok;
}
...
polyop_setDiagonal_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(v1+1, 1, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_FACE_VERTEX_INDEX_OUT_OF_RANGE));
	int v2=arg_list[3]->to_int()-1;
	MXS_range_check(v2+1, 1, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_FACE_VERTEX_INDEX_OUT_OF_RANGE));
	EPoly* epi = NULL;
	if (owner) {
		epi = (EPoly*)((Animatable*)owner)->GetInterface(EPOLY_INTERFACE);
	}
...
polyop_getFaceSmoothGroup_cf(Value** arg_list, int count)
{
	...
	int faceIndex =  arg_list[1]->to_int()-1;
	MXS_range_check(faceIndex+1, 1, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_FACE_INDEX_OUT_OF_RANGE));
	return_value (Integer::intern(poly->F(faceIndex)->smGroup));
}
...
polyop_propagateFlags_cf(Value** arg_list, int count)
{
	...
	def_poly_so_types();
	int slTo = ConvertValueToID(polysoTypes, elements(polysoTypes), arg_list[1]);
	int flTo = arg_list[2]->to_int();
	int slFrom = ConvertValueToID(polysoTypes, elements(polysoTypes), arg_list[3]);
...
polyop_detachFaces_cf(Value** arg_list, int count)
{
	...
	if (asNode && (myNode == NULL))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_POLY_DETACH_TO_NODE_REQUIRES_NODE));
	EPoly* epi = NULL;
	if (owner) {
		epi = (EPoly*)((Animatable*)owner)->GetInterface(EPOLY_INTERFACE);
	}
	...
	if (epi) {
		...
		{
			MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
			res = epi->EpfnDetachToObject(name, MNM_SL_FACE, MN_TARG, !del, myNode, MAXScript_time_tls());
		}
		else
...
polyop_detachEdges_cf(Value** arg_list, int count)
{
	...
	if (epi) {
		...
		{
			MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
			res = epi->EpfnDetachToObject(name, MNM_SL_EDGE, MN_TARG, !del, myNode, MAXScript_time_tls());
		}
		else
...
polyop_detachVerts_cf(Value** arg_list, int count)
{
	...
	if (epi) {
		...
		{
			MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
			res = epi->EpfnDetachToObject(name, MNM_SL_VERTEX, MN_TARG, !del, myNode, MAXScript_time_tls());
		}
		else
...
polyop_getSlicePlane_cf(Value** arg_list, int count)
{
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(RayValue* res);
...
{
	check_arg_count_with_keys(setSlicePlane, 3, count);
	ReferenceTarget* owner = NULL;
...
{
	check_arg_count_with_keys(slice, 3, count);
	ReferenceTarget* owner = NULL;
...
//	}
	...
	return_value (Integer::intern(res+1));
}
...
polyop_cutEdge_cf(Value** arg_list, int count)
{
	...
	int nEdges = poly->ENum();
	MXS_range_check(edge1Index+1, 1, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_EDGE_INDEX_OUT_OF_RANGE));
	MXS_range_check(edge2Index+1, 1, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_EDGE_INDEX_OUT_OF_RANGE));
...
polyop_makeVertsPlanar_cf(Value** arg_list, int count)
{
	...
	if (epi) {
		...
		epi->EpSetVertexFlags(verts, MN_TARG, false);
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->EpfnMakePlanar (MNM_SL_VERTEX, MN_TARG, MAXScript_time_tls());
	}
	return &ok;
}
...
//	else world_from_current_coordsys(normal,NO_TRANSLATE + NO_SCALE);
	...
	if (epi) {
		...
		epi->EpSetVertexFlags(verts, MN_TARG, false);
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->EpfnMoveToPlane (normal, offset, MNM_SL_VERTEX, MN_TARG, MAXScript_time_tls());
	}
	return &ok;
}
...
polyop_makeEdgesPlanar_cf(Value** arg_list, int count)
{
	...
	if (epi) {
		...
		epi->EpSetEdgeFlags(edges, MN_TARG, false);
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->EpfnMakePlanar (MNM_SL_EDGE, MN_TARG, MAXScript_time_tls());
	}
	return &ok;
}
...
//	else world_from_current_coordsys(normal,NO_TRANSLATE + NO_SCALE);
	...
	if (epi) {
		...
		epi->EpSetEdgeFlags(edges, MN_TARG, false);
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->EpfnMoveToPlane (normal, offset, MNM_SL_EDGE, MN_TARG, MAXScript_time_tls());
	}
	return &ok;
}
...
polyop_makeFacesPlanar_cf(Value** arg_list, int count)
{
	...
	if (epi) {
		...
		epi->EpSetFaceFlags(faces, MN_TARG, false);
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->EpfnMakePlanar (MNM_SL_FACE, MN_TARG, MAXScript_time_tls());
	}
	return &ok;
}
...
//	else world_from_current_coordsys(normal,NO_TRANSLATE + NO_SCALE);
	...
	if (epi) {
		...
		epi->EpSetFaceFlags(faces, MN_TARG, false);
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->EpfnMoveToPlane (normal, offset, MNM_SL_FACE, MN_TARG, MAXScript_time_tls());
	}
	return &ok;
}
...
polyop_createShape_cf(Value** arg_list, int count)
{
	...
	if (myNode == NULL)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_POLY_CREATESHAPE_REQUIRES_NODE), arg_list[0]);
	EPoly* epi = NULL;
	if (owner) {
		epi = (EPoly*)((Animatable*)owner)->GetInterface(EPOLY_INTERFACE);
	}
...
polyop_setEdgeVis_cf(Value** arg_list, int count)
{
	...
	if (epi)
		epi->LocalDataChanged(DISP_ATTRIB_CHANNEL);
	return &ok;
}
...
polyop_extrudeFaces_cf(Value** arg_list, int count)
{
	...
	if (epi) {
		...
		epi->EpSetFaceFlags(faces, MN_TARG, false);
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->EpfnExtrudeFaces (arg_list[2]->to_float(), MN_TARG, MAXScript_time_tls());
	}
	return &ok;
}
...
polyop_bevelFaces_cf(Value** arg_list, int count)
{
	...
	if (epi) {
		...
		epi->LocalDataChanged(TOPO_CHANNEL|SELECT_CHANNEL); // needed to force invalidation of temp data
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->EpfnBevelFaces (arg_list[2]->to_float(), arg_list[3]->to_float(), MN_SEL, MAXScript_time_tls());
	}
	return &ok;
}
...
polyop_chamferVerts_cf(Value** arg_list, int count)
{
	...
	if (epi) {
		...
		epi->LocalDataChanged(TOPO_CHANNEL|SELECT_CHANNEL); // needed to force invalidation of temp data
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->EpfnChamferVerticesOpen(arg_list[2]->to_float(),l_open,  MAXScript_time_tls());
	}
	return &ok;
}
...
polyop_chamferEdges_cf(Value** arg_list, int count)
{
	...
	if (epi) {
		...
		epi->LocalDataChanged(TOPO_CHANNEL|SELECT_CHANNEL); // needed to force invalidation of temp data
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->EpfnChamferEdgesOpen(arg_list[2]->to_float(),l_open, MAXScript_time_tls());
	}
	return &ok;
}
...
static Value* SetNumMapVertsP(Value* source, int channel, int count, bool keep, const TCHAR* fn_name)
{	
// Note: can have map verts without having map faces
	...
	if (epi) 
		epi->LocalDataChanged ((channel<1) ? VERTCOLOR_CHANNEL : TEXMAP_CHANNEL);
	return &ok;
}

static Value* GetNumMapVertsP(Value* source, int channel, const TCHAR* fn_name)
{	
// can have map verts without having map faces
	MNMesh* poly = get_polyForValue(source, MESH_READ_ACCESS, NULL, fn_name);
	MXS_range_check(channel, -NUM_HIDDENMAPS, MAX_MESHMAPS-1, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_CHANNEL_OUT_OF_RANGE));
	checkMapChannel (poly, channel);
	return_value (Integer::intern(poly->M(channel)->numv));
}
...
polyop_setNumMaps_cf(Value** arg_list, int count)
{
	...
	if (epi) 
		epi->LocalDataChanged ((numMaps<1 || !keep) ? VERTCOLOR_CHANNEL|TEXMAP_CHANNEL : TEXMAP_CHANNEL);
	return &ok;
}
...
polyop_getNumMaps_cf(Value** arg_list, int arg_count)
{
	...
	return_value (Integer::intern(poly->MNum()));
}
...
polyop_setMapSupport_cf(Value** arg_list, int arg_count)
{
	...
	if (support) {
		...
		if (channel < poly->MNum() && !poly->M(channel)->GetFlag (MN_DEAD)) {
...
polyop_getMapSupport_cf(Value** arg_list, int arg_count)
{
	...
	int channel = arg_list[1]->to_int();
	MXS_range_check(channel, -NUM_HIDDENMAPS, MAX_MESHMAPS-1, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_CHANNEL_OUT_OF_RANGE));
	return bool_result(channel < poly->MNum() && !poly->M(channel)->GetFlag (MN_DEAD));
}

// ============================================================================
// SetNumMapVerts <Poly poly> <Integer mapChannel> <Integer count> keep:<boolean=false>
Value* polyop_setNumMapVerts_cf(Value** arg_list, int count)
{
...
Value* polyop_getNumMapVerts_cf(Value** arg_list, int arg_count)
{
...
polyop_getNumMapFaces_cf(Value** arg_list, int arg_count)
{
	...
	int channel = arg_list[1]->to_int();
	checkMapChannel (poly, channel);
	return_value (Integer::intern(poly->M(channel)->numf));
}
...
// can have map verts without having map faces
	...
	checkMapChannel (poly, channel);
	MXS_range_check(index+1, 1, poly->M(channel)->numv, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_VERTEX_INDEX_OUT_OF_RANGE));
//	poly->MV(channel,index) = uvw; // poly->MV returns a UVVert = not a pointer or reference
	poly->M(channel)->v[index] = uvw;

	if (owner)
	{
		EPoly* epi = (EPoly*)((Animatable*)owner)->GetInterface(EPOLY_INTERFACE);
		if (epi)
		{
			epi->LocalDataChanged((channel < 1) ? VERTCOLOR_CHANNEL : TEXMAP_CHANNEL);
		}
...
polyop_setMapFace_cf(Value** arg_list, int arg_count)
{
	...
	MXS_range_check(faceindex+1, 1, poly->M(channel)->numf, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_FACE_INDEX_OUT_OF_RANGE));
	if (!is_array(arg_list[3]))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_SETMAPFACE_VERTEX_LIST_MUST_BE_ARRAY_GOT), arg_list[3]);
	...
	for (int i = 0; i < degree; i++) {
		int vertIndex=vertArray->data[i]->to_int()-1;
		MXS_range_check(vertIndex+1, 1, nMapVerts, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_VERTEX_INDEX_OUT_OF_RANGE));
		v[i]=vertIndex;
	}
	poly->MF(channel,faceindex)->MakePoly(degree, v.Addr(0));
...
polyop_setVertColor_cf(Value** arg_list, int arg_count)
{
	...
	int nVerts = poly->VNum();
	BitArray verts (nVerts);
	ValueToBitArrayP(arg_list[2], verts, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_VERTEX_INDEX_OUT_OF_RANGE), MESH_VERTSEL_ALLOWED, poly);
	...
	if (epi) {
		...
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->SetVertexColor (color, channel, MN_TARG, MAXScript_time_tls());
...
// can have map verts without having map faces
	...
	int nFaces = poly->FNum();
	BitArray faces (nFaces);
	ValueToBitArrayP(arg_list[2], faces, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_FACE_INDEX_OUT_OF_RANGE), MESH_FACESEL_ALLOWED, poly);
	Point3 color = arg_list[3]->to_point3()/255.0f;
	EPoly* epi = NULL;
	if (owner) {
		epi = (EPoly*)((Animatable*)owner)->GetInterface(EPOLY_INTERFACE);
	...
	if (epi) {
		...
		epi->BeginVertexColorModify(channel);
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->SetFaceColor (color, channel, MN_TARG, MAXScript_time_tls());
		epi->EndVertexColorModify(true);
		epi->LocalDataChanged ((channel<1) ? VERTCOLOR_CHANNEL : TEXMAP_CHANNEL);
	}
	return &ok;
...
polyop_setNumVDataChannels_cf(Value** arg_list, int count)
{
	...
	if (epi) 
		epi->LocalDataChanged(GEOM_CHANNEL);	
	return &ok;
}
...
polyop_getNumVDataChannels_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(poly->VDNum()));
}
...
polyop_setVDataChannelSupport_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(channel+1, 1, MAX_VERTDATA, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTDATA_CHANNEL_OUT_OF_RANGE));
	EPoly* epi = NULL;
	if (owner) {
		epi = (EPoly*)((Animatable*)owner)->GetInterface(EPOLY_INTERFACE);
	}
...
polyop_getVDataValue_cf(Value** arg_list, int count)
{
	...
	int index=arg_list[2]->to_int()-1;
	MXS_range_check(index+1, 1, poly->GetVD(channel)->Count(), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTDATA_INDEX_OUT_OF_RANGE));
	return_value (Float::intern(poly->vertexFloat(channel)[index]));
}
...
polyop_setVDataValue_cf(Value** arg_list, int arg_count)
{
	...
	if (epi) {
		...
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->SetVertexDataValue (channel, val, MN_TARG, MAXScript_time_tls());
...
polyop_setNumEDataChannels_cf(Value** arg_list, int count)
{
	...
	int numChannels=arg_list[1]->to_int();
	MXS_range_check(numChannels, 0, MAX_EDGEDATA, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_NUMEDGEDATA_OUT_OF_RANGE));
	Value* tmp = NULL;
	bool keep = bool_key_arg(keep, tmp, TRUE) == TRUE; 
	EPoly* epi = NULL;
	if (owner) {
...
polyop_getNumEDataChannels_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(poly->EDNum()));
}
...
polyop_setEDataChannelSupport_cf(Value** arg_list, int count)
{
	...
	int channel = arg_list[1]->to_int()-1;
	MXS_range_check(channel+1, 1, MAX_EDGEDATA, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_EDGEDATA_CHANNEL_OUT_OF_RANGE));
	EPoly* epi = NULL;
	if (owner) {
		epi = (EPoly*)((Animatable*)owner)->GetInterface(EPOLY_INTERFACE);
	}
...
polyop_getEDataValue_cf(Value** arg_list, int count)
{
	...
	if (!poly->eDataSupport(channel))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_VERTEXDATA_SUPPORT_NOT_ENABLED_FOR_CHANNEL), arg_list[1]);
	int index=arg_list[2]->to_int()-1;
	MXS_range_check(index+1, 1, poly->GetED(channel)->Count(), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_EDGEDATA_INDEX_OUT_OF_RANGE));
	return_value (Float::intern(poly->edgeFloat(channel)[index]));
}
...
polyop_setEDataValue_cf(Value** arg_list, int arg_count)
{
	...
	int nEdges = poly->ENum();
	BitArray edges (nEdges);
	ValueToBitArrayP(arg_list[2], edges, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_EDGE_INDEX_OUT_OF_RANGE), MESH_EDGESEL_ALLOWED, poly);
	float val = arg_list[3]->to_float();
	EPoly* epi = NULL;
	if (owner) {
		epi = (EPoly*)((Animatable*)owner)->GetInterface(EPOLY_INTERFACE);
	...
	if (epi) {
		...
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		epi->SetEdgeDataValue (channel, val, MN_TARG, MAXScript_time_tls());
		epi->EndPerDataModify(true);
		epi->LocalDataChanged ((channel<1) ? VERTCOLOR_CHANNEL : TEXMAP_CHANNEL);
	}
	return &ok;
...
polyop_freeEData_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(channel+1, 1, MAX_EDGEDATA, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_EDGEDATA_CHANNEL_OUT_OF_RANGE));
	if (!poly->eDataSupport(channel))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_EDGEDATA_SUPPORT_NOT_ENABLED_FOR_CHANNEL), arg_list[1]);
...
// ============================================================================
// <bool> checkTriangulation <Poly poly>
Value*
polyop_checkTriangulation_cf(Value** arg_list, int count)
{
	check_arg_count(getNumEdges, 1, count);
	MNMesh* poly = get_polyForValue(arg_list[0], MESH_READ_ACCESS, NULL, checkTriangulation);

	// Make sure there is nothing wrong with the triangulation of the faces.  Returns true if triangulation is OK, false otherwise.
	// This uses the same algorithm as MNFace::GetTriangles

	MNMesh& mm = *poly;
	for (int i = 0; i < mm.numf; ++i)
	{
		MNFace& theFace = mm.f[i];
		if (theFace.GetFlag(MN_DEAD)) continue;
		int deg = theFace.deg;
		if (deg <= 3) continue;
		if (deg == 4) {
			int other1 = (theFace.diag[0] + 1) % deg;
			int other2 = (theFace.diag[1] + 1) % deg;
			if (other1 == theFace.diag[0] || other1 == theFace.diag[1] || other2 == theFace.diag[0] || other2 == theFace.diag[1])
				return bool_result(false);
		}
		else // Higher-degree face; Analyze diagonals & resulting triangles
		{
			int diagSize = (deg - 3) * 2;
			int triSize = (deg - 2) * 3;
			// We keep track of "eliminated" vertices by keeping a number, for each
			// non-eliminated vertex, that tells how many eliminated verts follow:
			Tab<int> deadCount;
			deadCount.SetCount(deg);
			BitArray chopped;
			chopped.SetSize(deg);
			for (int i = 0; i < deg; i++) deadCount[i] = 0;
			int tnum = 0;
			// By presorting diagonals in an order that increases by second (higher)
			// index and decreases by first (lower) index, such as:
			// (1,3),(4,6),(3,6),(1,6),(6,8),(1,8) (for a 9-gon)
			// we ensure that this algorithm can chop off a triangle at each step.
			// Once we have a triangle chopped off for each diagonal, we can
			// take the last 3 "nonchopped" verts and make them a triangle.
			for (int i = 0; i < diagSize; i += 2) {
				int next = (theFace.diag[i] + deadCount[theFace.diag[i]] + 1) % deg;
				int third = (next + deadCount[next] + 1) % deg;
				if (third != theFace.diag[i + 1]) {
					return bool_result(false);	// Error in diagonals
				}
				// Chop this triangle.
				tnum += 3;
				deadCount[theFace.diag[i]] += deadCount[next] + 1;
				chopped.Set(next);
			}
			tnum += 3;
			if (tnum != triSize) {
