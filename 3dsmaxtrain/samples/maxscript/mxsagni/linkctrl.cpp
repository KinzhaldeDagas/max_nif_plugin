...
// ============================================================================
visible_class_s (LinkControl, RolloutControl)
...
protected:
	...
	bool     m_bMouseOver;
	bool     m_bVisited;
	MAXScript::ScriptSource m_addressScriptSource;
...
public:
	...
	void SetAddress(const TCHAR *text);
	...
	: RolloutControl(name, caption, keyparms, keyparm_count)
{
	...
	m_hToolTip = NULL;
	m_hFont = NULL;
	MaxSDK::GetResourceStringAsMSTR(IDS_GET_DEFAULT_MXS_HYPERLINK_URL, m_address);
	...
	m_bMouseOver = false;
	m_bVisited = false;
	m_addressScriptSource = MAXScript::ScriptSource::NotSpecified;
}
...
// ============================================================================
void LinkControl::SetText(const TCHAR *text)
{
	SIZE size;
	POINT pnt = {0,0};
	...
	MoveWindow(m_hWnd, pnt.x, pnt.y, size.cx, GetUIScaledValue(parent_rollout->text_height+2), TRUE);
...
// ============================================================================
void LinkControl::SetAddress(const TCHAR *text)
{
	m_address = text;
	// Store current scriptSource so we know later if set the address from a scene embedded script
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	m_addressScriptSource = _tls ? _tls->scriptSource : MAXScript::ScriptSource::NonEmbedded;
...
void LinkControl::FillToolInfo(TOOLINFO *ti)
{
	...
	ti->hwnd = m_hWnd;
	ti->uId = TOOLTIP_ID;
	ti->lpszText = const_cast<TCHAR*>(m_address.data());
...
{
	bool openHyperlink = true;

	// If the address was set by an embedded script (either via the rollout definition or by setting the address property)
	if (m_addressScriptSource != MAXScript::ScriptSource::NonEmbedded)
	{
		if (GetCOREInterface()->GetQuietMode())
		{
			// If in quiet mode, don't open it
			openHyperlink = false;
		}
		else
		{
			// If not in quiet mode, warn the user and ask them if they still want to open it
			auto hyperlinkSecurityWarningDialog = new QHyperlinkSecurityWarningDialog((QWidget*)(GetCOREInterface()->GetQmaxMainWindow()), m_address);
			openHyperlink = (hyperlinkSecurityWarningDialog->exec() == QDialog::Accepted);
		}
	}

	if (openHyperlink)
	{
		HINSTANCE hShell = ShellExecute(MAXScript_interface->GetMAXHWnd(), _T("open"), m_address, NULL, NULL, SW_SHOW);
	...
	LinkControl::compute_layout(Rollout *ro, layout_data* pos)
{
	...
	DLGetTextExtent(ro->rollout_dc, text, &size, true);   
...
void LinkControl::add_control(Rollout *ro, HWND parent, HINSTANCE hInstance, int& current_y)
{
	...
	if(val != &unsupplied)
	{
		...
		// Address specified in rollout definition, see if rollout definition source was scene file embedded
		m_addressScriptSource = (ro->flags & RO_SOURCE_SCENE_EMBEDDED) ? MAXScript::ScriptSource::Embedded : MAXScript::ScriptSource::NonEmbedded;
	}
	else
	{
		// Default address does not need to be handled as scene embedded since we know the address string is safe
		MaxSDK::GetResourceStringAsMSTR(IDS_GET_DEFAULT_MXS_HYPERLINK_URL, m_address);
		m_addressScriptSource = MAXScript::ScriptSource::NonEmbedded;
	}
	...
	m_hWnd = CreateWindow(
		...
		GetUIScaledValue(pos.left), GetUIScaledValue(pos.top), 
		GetUIScaledValue(pos.width), GetUIScaledValue(pos.height),
...
Value* LinkControl::get_property(Value** arg_list, int count)
{
	...
	if(prop == n_color)
	{
		...
		{
			COLORREF t_color = (m_colorSpecified) ? m_color : GetCustSysColor(COLOR_BTNTEXT);
			return_value (ColorValue::intern(AColor(t_color)));
		}
		else
			return &undefined;
	}
	else if(prop == n_hovercolor)
	{
		...
		{
			COLORREF t_hoverColor = (m_hoverColorSpecified) ? m_hoverColor : GetCustSysColor(COLOR_HIGHLIGHTTEXT);
			return_value (ColorValue::intern(AColor(t_hoverColor)));
		}
		else
			return &undefined;
	}
	...
	{
		if(parent_rollout && parent_rollout->page)
			return_value (ColorValue::intern(AColor(m_visitedColor)));
		else
			return &undefined;
	}
...
void LinkCtrlInit()
{
	...
	if(!registered)
	{
		...
		wcex.hIcon         = NULL;
		wcex.hIconSm       = NULL;
		wcex.hCursor       = UI::MouseCursors::LoadMouseCursor(UI::MouseCursors::HandPointer);
	...
	install_rollout_control(Name::intern(_T("HyperLink")), LinkControl::create);
}
