...
RenderFrame_cf(Value** arg_list, int count){ 
	...
	res = MAXScript_interface->OpenCurRenderer(NULL,view.ToPointer());
	res = MAXScript_interface->CurRendererRenderFrame(
		MAXScript_time_tls(),bm);
...
TSetScale_cf(Value** arg_list, int count)
{
	...
	TimeValue tv = MAXScript_time_tls();
...
TestMatrix3_cf(Value** arg_list, int count)
{
	...
	nm3 = node->GetNodeTM(MAXScript_time_tls());
	om3 = node->GetObjectTM(MAXScript_time_tls());		
	...
	node->SetNodeTM(MAXScript_time_tls(),nm3);
...
ViewLocalRot_cf(Value** arg_list, int count) 
{
	...
	tmat.IdentityMatrix();
	c->GetValue(MAXScript_time_tls(), &tmat, valid, CTRL_RELATIVE);
...
ViewWorldRot_cf(Value** arg_list, int count) 
{
	...
	Control *c = node->GetTMController();
	tmat = node->GetParentTM(MAXScript_time_tls());
	c->GetValue(MAXScript_time_tls(), &tmat, valid, CTRL_RELATIVE);
...
NumCycles_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(NumCycles(arg_list[0]->to_interval(), 
...
NewBitmapInfo_cf(Value** arg_list, int count)
{
	...
	bi->SetType(ConvertValueToID(bmpTypes, elements(bmpTypes), arg_list[0]));	
...
BitmapWrite_cf(Value** arg_list, int count)
{
	...
	{
		throw RuntimeError(_T("Error reading bitmap"));
...
//NC	bMap = TheManager->Load(arg_list[0]->to_bmi(), &status);
	...
	if (status != BMMRES_SUCCESS) 
		return new ConvertIDToValue(bmmerrorTypes, status);
...
GetRenderMeshBug_cf(Value** arg_list, int count)
{
	...
	Mesh* cMeshPtr = ((GeomObject*)os.obj)->GetRenderMesh(
		MAXScript_time_tls(), node, nullView,needDel);
...
SnapAngle_cf(Value** arg_list, int count)
{
	...
	return_value (Float::intern(MAXScript_interface->SnapAngle (arg_list[0]->to_float(), fastsnap->to_bool(), forcesnap->to_bool()) ));
}
...
GetMatID_cf(Value** arg_list, int count)
{
	...
	if (refobj->SuperClassID() == SHAPE_CLASS_ID)
	{
		...
		{
			if (theshape->NumberOfPieces(MAXScript_time_tls(), the_curve-1) >= the_piece && the_piece > 0)
			{
				return_value (Integer::intern ( (theshape->GetMatID (MAXScript_time_tls(), the_curve-1, the_piece-1) + 1 )));
			}
		}
...
GetCommandStackSize_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(MAXScript_interface->GetCommandStackSize()));
}
...
GetSavingVersion_cf (Value** arg_list, int count)
{
	...
	return_value (Integer::intern (GetSavingVersion()));
...
{
	return_value (Float::intern(100.0f * ((GenLight*)obj)->GetAtmosOpacity(t, valid)));
}
void    
...
{
	return_value (Float::intern(100.0f * ((GenLight*)obj)->GetAtmosColAmt(t, valid)));
}
void    
...
{
	return_value (Float::intern(((GenLight*)obj)->GetDecayRadius(t, valid)));
}
void    
...
{
	return_value (Float::intern(((GenLight*)obj)->GetShadMult(t, valid)));
}
void    
