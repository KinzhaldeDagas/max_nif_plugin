...
{
	Bitmap* map = nullptr;
	Bitmap* mapTmp = nullptr;
	BitmapInfo bi,biTmp;
	float w = 0.f, h = 0.f;
	BITMAPINFO* bInfo = nullptr;
	bool hasAlpha = false;
	...
	hasAlpha = captureAlpha && bInfo->bmiHeader.biBitCount >= 32;
...
*/
// TODO ColorManagement: This function shouldn't be needed. We should remove the
// requirement of linear pixels for saving the bitmaps; maybe by changing
// BitmapIO::GetOuputPixels() to use Bitmap::GetLinearPixels() instead of
// Bitmap::GetPixels() ? /coz 2024-05-03
void DeGammaCorrectBitmap(Bitmap* map)
{
	...
	if(cpm->GetColorPipelineMode() == ColorPipelineMode::kGAMMA)
	{
		float gamma = cpm->GetDisplayGamma();
	...
	// TODO ColorManagement: handle all modes properly /coz 2024-05-13(!)
}

Value*
setclipboardBitmap_cf(Value** arg_list, int count)
{
	// setclipboardBitmap <bitmap> doDisplayTransform:<bool>
	...
	Value* tmp;
	bool doDisplayTransform = bool_key_arg(doDisplayTransform, tmp, true);

	int depth = (mbm->bm->HasAlpha()) ? 32 : 24;
	PBITMAPINFO bmi = mbm->bm->ToDib(depth, nullptr, 0, doDisplayTransform);
