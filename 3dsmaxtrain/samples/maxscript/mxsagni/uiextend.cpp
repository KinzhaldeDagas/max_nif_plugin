...
EnumAuxFiles_cf(Value** arg_list, int count)
{
	...
	MAXScript_interface->EnumAuxFiles(nameEnum, 
		ConvertValueToID(enumfileTypes, elements(enumfileTypes), arg_list[0]));
	return &ok;
}
...
public:
	BOOL state;
	BOOL Exit(HWND hWnd) { 
		MaxSDK::MaxMessageBox(hWnd, _T("Max is exiting:"), _T("TestApp Info"), MB_OK);
		return state;
	};
};
	...
	void Display(TimeValue t, ViewExp *vpt, int flags) 
		{ 
			...
			if ( ! vpt || ! vpt->IsAlive() )
			{
				...
				out->printf(_T("Doing Display() on inactive viewport!"));
				return;
			}
	...
	void GetViewportRect(TimeValue t, ViewExp *vpt, Rect *rect)
		{ 
			...
			if ( ! vpt || ! vpt->IsAlive() )
			{
				...
				out->printf(_T("Doing GetViewportRect() on inactive viewport!"));
				return;
			}
