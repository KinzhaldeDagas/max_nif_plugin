...
get_MAX_version_cf(Value** arg_list, int count)
{
	...
	DbgAssert(!buildNumberString.isNull());
...
{
	AssetUser asset = IAssetManager::GetInstance()->GetAsset(val->to_filename(), kPostRenderScript);
...
Value* get_preserveStackPosition()
{
	return MAXScript_interface->GetPreserveStackPosition() ? &true_value : &false_value;
}


Value* set_preserveStackPosition(Value* val)
{
	MAXScript_interface->SetPreserveStackPosition(val->to_bool());
	return val;
}
...
{
	// SetSnapSpinner() is in core.dll, and doesn't update the toolbar button. 
	BOOL useSnap = val->to_bool();
	BOOL usingSnap = GetSnapSpinner();
	if (useSnap != usingSnap)
		MAXScript_interface->ExecuteMAXCommand(MAXCOM_SPINSNAP_TOGGLE);
	return val;
}
...
{
	return MAXScript_interface14->GetPlayPreviewWhenDone() ? &true_value : &false_value;
}

Value*
set_playPreviewWhenDone(Value *val)
{
	MAXScript_interface14->SetPlayPreviewWhenDone(val->to_bool());
	return val;
}
...
get_INI_setting_cf(Value** arg_list, int count)
{
	...
	Value* v;
	MSTR defaultVal = string_key_arg(defaultValue, v, nullptr);

	int numArgs = _count_with_keys(arg_list, count);
	if (numArgs == 1 || numArgs == 2) // Get sections or section keys
	{
		...
		const TCHAR* section = (numArgs == 2) ? arg_list[1]->to_string() : nullptr;
	...
	else // Get value of specific key
	{
		check_arg_count(getIniSetting, 3, numArgs);
		TSTR  fileName = arg_list[0]->to_filename();
		AssetId assetId;
		if (IAssetManager::GetInstance()->StringToAssetId(fileName, assetId))
			fileName = IAssetManager::GetInstance()->GetAsset(assetId).GetFileName();
...
del_INI_setting_cf(Value** arg_list, int count)
{
	...
	if (count == 2 || count == 3) // delete section or section key
	{
		...
		if (sceneScriptSecurityManager && sceneScriptSecurityManager->IsProtectedINIFile(fileName))
			MAXScript::ValidateCanRunMAXScriptSystemCommand(_T("delINISetting"), arg_list, count);
...
Value* get_INI_force_utf16_default_cf(Value** arg_list, int count)
{
	check_arg_count(getIniForceUTF16Default, 0, count);
	return bool_result(forceUTF16_default);
}
...
set_INI_setting_cf(Value** arg_list, int count)
{
	...
	MaxSDK::ISceneScriptSecurityManager* sceneScriptSecurityManager = MaxSDK::ISceneScriptSecurityManager::GetInstance();
	if (sceneScriptSecurityManager && sceneScriptSecurityManager->IsProtectedINIFile(fileName))
		MAXScript::ValidateCanRunMAXScriptSystemCommand(_T("setINISetting"), arg_list, count);
	if (sceneScriptSecurityManager && sceneScriptSecurityManager->IsProtectedFromOpeningForWriting(fileName))
		MAXScript::ValidateCanRunMAXScriptSystemCommand(_T("setINISetting"), arg_list, count);
