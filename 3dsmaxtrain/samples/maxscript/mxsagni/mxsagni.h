...
extern void		ValueToBitArray(Value* inval, BitArray &theBitArray, int maxSize, const MSTR& errmsg=_T(""), int selTypesAllowed=0);
extern void		ValueToBitArrayM(Value* inval, BitArray &theBitArray, int maxSize, const MSTR& errmsg, int selTypesAllowed, Mesh* mesh);
extern void		ValueToBitArrayP(Value* inval, BitArray &theBitArray, int maxSize, const MSTR& errmsg, int selTypesAllowed, MNMesh* poly);

extern INode*	_get_valid_node(MAXNode* _node, const TCHAR* errmsg); // see #define get_valid_node below
extern BezierShape*		set_up_spline_access(INode* node, int index);
extern Mesh*	_get_meshForValue(Value* value, int accessType, ReferenceTarget** owner, const TCHAR* fn);
extern Mesh*	getWorldStateMesh(INode* node, BOOL renderMesh);
extern Mesh*	_get_worldMeshForValue(Value* value, const TCHAR* fn, BOOL renderMesh);
extern MNMesh*	_get_polyForValue(Value* value, int accessType, ReferenceTarget** owner, const TCHAR* fn);
...
#define get_worldMeshForValue(_value, _fn, _renderMesh) _get_worldMeshForValue(_value, _T(#_fn)##, _renderMesh)
#define get_polyForValue(_value, _accessType, _owner, _fn) _get_polyForValue(_value, _accessType, _owner, _T(#_fn)##)
...
#define get_valid_node(_node, _fn) _get_valid_node((MAXNode*)_node, _T(#_fn) _T(" requires a node"))

#define elements(array)			(_countof(array))
#define def_not_implemented()	throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_RK_FN_NOT_YET_IMPLEMENTED))
