...
{
public:	
	HKeyClass(const TCHAR* name) : ValueMetaClass (name) { } 
...
public:
	...
	HKey();
	HKey(HKEY key, HKey* parentKey, const TCHAR* name, REGSAM accessRights);
	...
	Value*		deep_copy(HashTable* remap) override { return this; }
...
HKeyClass::apply(Value** arg_list, int count,  CallContext* cc)
{
	...
	type_check(parentKeyVal, HKey, _T("HKey"));
	...
	const TCHAR* keyName = arg_list[1]->eval()->to_string();
...
HKey::HKey(HKEY key, HKey* parentKey, const TCHAR* name, REGSAM accessRights) : key(key), parentKey(parentKey), accessRights(accessRights)
{
	tag = class_tag(HKey);
	if (name) 
		keyName= name;
...
HKey::gc_trace()
{
	...
	Value::gc_trace();
	if (parentKey && parentKey->is_not_marked())
		parentKey->gc_trace();
}
...
HKey::get_property(Value** arg_list, int count)
{
	...
	if (prop == n_name)
		...
		return_value (Integer::intern(accessRights));
...
HKey::set_property(Value** arg_list, int count)
{
	...
	if (prop == n_name)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_READ_ONLY_PROPERTY), prop);
	else if (prop == n_accessRights)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_READ_ONLY_PROPERTY), prop);
	else if (prop == n_parentKey)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_READ_ONLY_PROPERTY), prop);
...
ConstHKey::gc_trace()
{
	// mark me & trace sub-objects
	HKey::gc_trace();
	if (keyName && keyName->is_not_marked())
		keyName->gc_trace();
}
...
registry_closeKey_cf(Value** arg_list, int count)
{
	...
	type_check(arg_list[0], HKey, _T("HKey"));
...
registry_flushKey_cf(Value** arg_list, int count)
{
	...
	MAXScript::ValidateCanRunMAXScriptSystemCommand(_T("registry.flushKey"), arg_list, count);
...
registry_deleteSubKey_cf(Value** arg_list, int count)
{
	...
	MAXScript::ValidateCanRunMAXScriptSystemCommand( _T("registry.deleteSubKey"), arg_list, count);
...
registry_deleteKey_cf(Value** arg_list, int count)
{
	...
	MAXScript::ValidateCanRunMAXScriptSystemCommand(_T("registry.deleteKey"), arg_list, count);
...
registry_createKey_cf(Value** arg_list, int count)
{
	...
	MAXScript::ValidateCanRunMAXScriptSystemCommand(_T("registry.createKey"), arg_list, count);
	...
	const TCHAR* keyName = arg_list[1]->to_string();
	...
	long res = RegCreateKeyEx(hParentKey,keyName,0,NULL,options,accessRights,NULL,&hKey,&disposition);
	if (res == ERROR_SUCCESS)
	{
		if (HKeyThunk)
...
registry_deleteValue_cf(Value** arg_list, int count)
{
	...
	MAXScript::ValidateCanRunMAXScriptSystemCommand(_T("registry.deleteValue"), arg_list, count);
...
registry_setValue_cf(Value** arg_list, int count)
{
	...
	MAXScript::ValidateCanRunMAXScriptSystemCommand(_T("registry.setValue"), arg_list, count);
	...
	const TCHAR* valName = arg_list[1]->to_string();

	Value* typeValue = arg_list[2];
	def_registry_value_types();
	DWORD type = ConvertValueToID(registryValueTypes, elements(registryValueTypes), typeValue);
	Value* dataValue = arg_list[3];
	long res;

	if (type == REG_DWORD)
	{
		integer_type_check(dataValue, _T("Integer"));
		DWORD intData = dataValue->to_int();
		res = RegSetValueEx(hKey,valName,0,type,(BYTE *)&intData,sizeof(DWORD));
	}
	else if (type == REG_SZ || type == REG_EXPAND_SZ)
	{
		const TCHAR* string = dataValue->to_string();
		int iTotLength = static_cast<int>(_tcslen(string)+1);
	...
	else if (type == REG_MULTI_SZ)
	{
		...
		iTotLength = 0;
		for (i = 0; i < n; i++)
			iTotLength += static_cast<int>(_tcslen(dataArray->data[i]->to_string()) + 1);
		...
		for (i = 0; i < n; i++)
		{
			iLength = static_cast<int>(_tcslen(dataArray->data[i]->to_string()) + 1);
...
registry_queryValue_cf(Value** arg_list, int count)
{
	...
	two_value_locals_tls(type, value);
	type_check(arg_list[0], HKey, _T("HKey"));
	HKey *keyVal = (HKey*)arg_list[0];
	HKEY hKey = keyVal->key;
	if (hKey == NULL) 
	...
	if (res == ERROR_SUCCESS)
	{
		...
		def_registry_value_types();
		vl.type = ConvertIDToValue(registryValueTypes, elements(registryValueTypes), type, &undefined);
		...
		else if (type == REG_SZ || (type == REG_EXPAND_SZ && !expand))
		{
			...
			DWORD nSize = iBufLen * sizeof(TCHAR);
			res = RegQueryValueEx(hKey,valName,0,&type,(BYTE *)szBuf, &nSize);
			if (res == ERROR_SUCCESS)
...
registry_getSubKeyName_cf(Value** arg_list, int count)
{
	...
	if (res == ERROR_SUCCESS)
	{
		MXS_range_check(index + 1, 1, int(numSubKeys), _T("subKey index out of range"));
		maxSubKeyLen += 1;
		...
		if (res == ERROR_SUCCESS)
		{
			...
			{
				MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
				one_value_local_tls(val);
				vl.val = new String (szBuf);
				nameThunk->assign(vl.val);
			}
		}
		delete[] szBuf;
	}
...
registry_getValueName_cf(Value** arg_list, int count)
{
	...
	if (res == ERROR_SUCCESS)
	{
		MXS_range_check(index + 1, 1, int(numValues), _T("value index out of range"));
		maxValueLen += 1;
...
#define def_global(n, v)		\
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	three_value_locals_tls(name, val, thunk);
	if (!GetCOREInterface17()->InSecureMode())
	{
		def_global(_T("HKEY_CLASSES_ROOT"), new (GC_STATIC) ConstHKey(HKEY_CLASSES_ROOT, new String(_T("HKEY_CLASSES_ROOT"))));
		def_global(_T("HKEY_CURRENT_CONFIG"), new (GC_STATIC) ConstHKey(HKEY_CURRENT_CONFIG, new String(_T("HKEY_CURRENT_CONFIG"))));
		def_global(_T("HKEY_CURRENT_USER"), new (GC_STATIC) ConstHKey(HKEY_CURRENT_USER, new String(_T("HKEY_CURRENT_USER"))));
		def_global(_T("HKEY_LOCAL_MACHINE"), new (GC_STATIC) ConstHKey(HKEY_LOCAL_MACHINE, new String(_T("HKEY_LOCAL_MACHINE"))));
		def_global(_T("HKEY_USERS"), new (GC_STATIC) ConstHKey(HKEY_USERS, new String(_T("HKEY_USERS"))));
		def_global(_T("HKEY_PERFORMANCE_DATA"), new (GC_STATIC) ConstHKey(HKEY_PERFORMANCE_DATA, new String(_T("HKEY_PERFORMANCE_DATA"))));
	}
	else
	{
		def_global(_T("HKEY_CLASSES_ROOT"), &undefined);
		def_global(_T("HKEY_CURRENT_CONFIG"), &undefined);
		def_global(_T("HKEY_CURRENT_USER"), &undefined);
		def_global(_T("HKEY_LOCAL_MACHINE"), &undefined);
		def_global(_T("HKEY_USERS"), &undefined);
		def_global(_T("HKEY_PERFORMANCE_DATA"), &undefined);
	}
}
