...
ScripterBase::Viewport::GetViewportDIB_Result ScripterBase::Viewport::GetViewportDIB(ViewExp &vpt, BitmapInfo &bi, Bitmap* &bmp, bool captureAlpha, bool gammaCorrect)
{
	...
	bool isOCIOMode = pColorPipelineMgr->Settings()->IsOCIOBased();
	if (isOCIOMode)
	{
		// Set Raw space as the color from viewport DIB is already in display
		// space. This ensures that pixels won't go through any color
		// conversions.
		MSTR dataSpaceName = pColorPipelineMgr->Settings()->GetDataColorSpaceName();
		bi.SetRequestedColorSpace(dataSpaceName, ColSpaceSource::SystemHeuristics);
		// No need to do gamma correct
		gammaCorrect = false;
	}
