	...
	systemTools_setEnvVar_cf(Value** arg_list, int count)
{
	...
	MAXScript::ValidateCanRunMAXScriptSystemCommand(_T("systemTools.setEnvVariable"), arg_list, count);
...
Value*
	systemTools_getEnvVars_cf(Value** arg_list, int count)
{
	// systemTools.getEnvVariables
	check_arg_count(systemTools.getEnvVariables, 0, count);

	// Get a pointer to the environment block
	LPTCH lpvEnv = GetEnvironmentStrings();

	// If the returned pointer is NULL, return undefined
	if (lpvEnv == NULL)
	{
		return &undefined;
	}

	// Create array to store the environment variables
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	...
	while (*lpszVariable)
	{
		vl.res->append(new String(lpszVariable));
	...
	systemTools_setmaxstdio_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(new_maxstdio, 512, 2048, _T("systemTools.setmaxstdio arg"));
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
