...
meshop_setHiddenVerts_cf(Value** arg_list, int arg_count)
{
	...
	ValueToBitArrayM(arg_list[1], verts, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTEX_INDEX_OUT_OF_RANGE), MESH_VERTSEL_ALLOWED, mesh);
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
	mesh->VertHide()=verts;
	if (mdu) mdu->LocalDataChanged (DISP_ATTRIB_CHANNEL | SELECT_CHANNEL);
	return &ok;
}
...
meshop_setHiddenFaces_cf(Value** arg_list, int arg_count)
{
	...
	ValueToBitArrayM(arg_list[1], faces, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_FACE_INDEX_OUT_OF_RANGE), MESH_FACESEL_ALLOWED, mesh);
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
 	for (int fi = 0; fi < nFaces; fi++)
...
meshop_getNumVerts_cf(Value** arg_list, int arg_count)
{
	...
	return_value (Integer::intern(get_meshForValue(arg_list[0], MESH_READ_ACCESS, NULL, getNumVerts)->getNumVerts()));
}
...
meshop_setNumVerts_cf(Value** arg_list, int arg_count)
{
	...
	if (nNewVerts < 0)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_COUNT_MUST_NOT_BE_NEG_NUMBER), arg_list[1]);
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
	MeshDelta tmd (*mesh);
	int nVerts=mesh->getNumVerts();
	...
	{
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		GetMeshDeltaUserDataInterface(owner)->ApplyMeshDelta(tmd, mdu, MAXScript_time_tls());
	}
	else 
		tmd.Apply (*mesh);
	return &ok;
...
meshop_getNumFaces_cf(Value** arg_list, int arg_count)
{
	...
	return_value (Integer::intern(get_meshForValue(arg_list[0], MESH_READ_ACCESS, NULL, getNumFaces)->getNumFaces()));
}
...
meshop_setNumFaces_cf(Value** arg_list, int arg_count)
{
	...
	if (mesh->getNumVerts() == 0)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_MUST_HAVE_AT_LEAST_ONE_VERTEX_TO_CREATE_FACE));
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
	MeshDelta tmd (*mesh);
	int nFaces=mesh->getNumFaces();
...
static Value* GetNumMapVerts(Value* source, int channel, const TCHAR* fn_name)
{
	// can have map verts without having map faces
	Mesh* mesh = _get_meshForValue(source, MESH_READ_ACCESS, NULL, fn_name);
	MXS_range_check(channel, -NUM_HIDDENMAPS, MAX_MESHMAPS - 1, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_CHANNEL_OUT_OF_RANGE));
	if (!mesh->mapSupport(channel))
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_MAP_SUPPORT_NOT_ENABLED_FOR_CHANNEL), Integer::intern(channel));
	return_value(Integer::intern(mesh->getNumMapVerts(channel)));
}

static Value* SetNumMapVerts(Value* source, int channel, int count, bool keep, const TCHAR* fn_name)
{
	...
	if (!mesh->mapSupport(channel))
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_MAP_SUPPORT_NOT_ENABLED_FOR_CHANNEL), Integer::intern(channel));
	if (count < 0)
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_COUNT_MUST_NOT_BE_NEG_NUMBER), Integer::intern(count));
	MeshDeltaUser* mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
meshop_getDisplacementMapping_cf(Value** arg_list, int arg_count)
{
	...
	if (!owner)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_DISPLACEMENT_MAPPING_NOT_APPLICABLE_TO_TRIMESH), arg_list[0]);
...
meshop_getSubdivisionMethod_cf(Value** arg_list, int arg_count)
{
	...
	return ConvertIDToValue(emeshsubdivmethodtypes, elements(emeshsubdivmethodtypes), owner->DisplacmentApprox().type);
}
...
meshop_setSubdivisionMethod_cf(Value** arg_list, int arg_count)
{
	...
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
	def_emesh_subdiv_method_types();
	int subdivMethodType = ConvertValueToID(emeshsubdivmethodtypes, elements(emeshsubdivmethodtypes), arg_list[1]);
...
meshop_getSubdivisionStyle_cf(Value** arg_list, int arg_count)
{
	...
	return ConvertIDToValue(emeshsubdivstyletypes, elements(emeshsubdivstyletypes), owner->DisplacmentApprox().subdiv);
}
...
meshop_setSubdivisionStyle_cf(Value** arg_list, int arg_count)
{
	...
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
	def_emesh_subdiv_style_types();
	int subdivStyleType = ConvertValueToID(emeshsubdivstyletypes, elements(emeshsubdivstyletypes), arg_list[1]);
...
meshop_getSubdivisionSteps_cf(Value** arg_list, int arg_count)
{
	...
	if (!owner)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_DISPLACEMENT_MAPPING_NOT_APPLICABLE_TO_TRIMESH), arg_list[0]);
	return_value (Integer::intern(owner->DisplacmentApprox().u));
}
...
meshop_setSubdivisionSteps_cf(Value** arg_list, int arg_count)
{
	...
	if (steps < 0)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_SUBDIVISION_STEPS_MUST_NOT_BE_NEG_NUMBER), arg_list[1]);
	MeshDeltaUser *mdu = GetMeshDeltaUserInterface(owner);
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
meshop_getSubdivisionEdge_cf(Value** arg_list, int arg_count)
{
	...
	if (!owner)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_DISPLACEMENT_MAPPING_NOT_APPLICABLE_TO_TRIMESH), arg_list[0]);
	return_value (Float::intern(owner->DisplacmentApprox().edge));
}
...
meshop_setSubdivisionEdge_cf(Value** arg_list, int arg_count)
{
	...
	if (edge < 0)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_SUBDIVISION_EDGE_MUST_NOT_BE_NEG_NUMBER), arg_list[1]);
	MeshDeltaUser *mdu = GetMeshDeltaUserInterface(owner);
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
meshop_getSubdivisionDistance_cf(Value** arg_list, int arg_count)
{
	...
	if (!owner)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_DISPLACEMENT_MAPPING_NOT_APPLICABLE_TO_TRIMESH), arg_list[0]);
	return_value (Float::intern(owner->DisplacmentApprox().dist));
}
...
meshop_setSubdivisionDistance_cf(Value** arg_list, int arg_count)
{
	...
	if (distance < 0)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_SUBDIVISION_DISTANCE_MUST_NOT_BE_NEG_NUMBER), arg_list[1]);
	MeshDeltaUser *mdu = GetMeshDeltaUserInterface(owner);
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
meshop_getSubdivisionAngle_cf(Value** arg_list, int arg_count)
{
	...
	if (!owner)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_DISPLACEMENT_MAPPING_NOT_APPLICABLE_TO_TRIMESH), arg_list[0]);
	return_value (Float::intern(owner->DisplacmentApprox().ang));
}
...
meshop_setSubdivisionAngle_cf(Value** arg_list, int arg_count)
{
	...
	if (angle < 0)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_SUBDIVISION_ANGLE_MUST_NOT_BE_NEG_NUMBER), arg_list[1]);
	MeshDeltaUser *mdu = GetMeshDeltaUserInterface(owner);
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
meshop_getSubdivisionMinLevels_cf(Value** arg_list, int arg_count)
{
	...
	if (!owner)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_DISPLACEMENT_MAPPING_NOT_APPLICABLE_TO_TRIMESH), arg_list[0]);
	return_value (Integer::intern(owner->DisplacmentApprox().minSub));
}
...
meshop_setSubdivisionMinLevels_cf(Value** arg_list, int arg_count)
{
	...
	if (levels < 0)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_SUBDIVISION_LEVELS_MUST_NOT_BE_NEG_NUMBER), arg_list[1]);
	MeshDeltaUser *mdu = GetMeshDeltaUserInterface(owner);
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
meshop_getSubdivisionMaxLevels_cf(Value** arg_list, int arg_count)
{
	...
	if (!owner)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_DISPLACEMENT_MAPPING_NOT_APPLICABLE_TO_TRIMESH), arg_list[0]);
	return_value (Integer::intern(owner->DisplacmentApprox().maxSub));
}
...
meshop_setSubdivisionMaxTriangles_cf(Value** arg_list, int arg_count)
{
	...
	if (maxTris < 0)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_SUBDIVISION_TRIANGLES_MUST_NOT_BE_NEG_NUMBER), arg_list[1]);
	MeshDeltaUser *mdu = GetMeshDeltaUserInterface(owner);
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
meshop_getSelByVertex_cf(Value** arg_list, int arg_count)
{
	...
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (!mdu)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_PROPERTY_NOT_APPLICABLE_TO_TRIMESH), arg_list[0]);
	int paramVal;
...
meshop_getSSEdgeDist_cf(Value** arg_list, int arg_count)
{
	...
	return_value (Integer::intern(paramVal));
}
...
meshop_getPlanarThreshold_cf(Value** arg_list, int arg_count)
{
	...
	return_value (Float::intern(paramVal));
}
...
meshop_getVertsUsingEdge_cf(Value** arg_list, int arg_count)
{
	...
	ValueToBitArrayM(arg_list[1], edges, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_EDGE_INDEX_OUT_OF_RANGE), MESH_EDGESEL_ALLOWED, mesh);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BitArrayValue* verts);
...
meshop_getPolysUsingFace_cf(Value** arg_list, int count)
{
	...
	ValueToBitArrayM(arg_list[1], infaces, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_FACE_INDEX_OUT_OF_RANGE), MESH_FACESEL_ALLOWED, mesh);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BitArrayValue* faces);
	vl.faces = new BitArrayValue (nFaces);
	for (int i=0; i<nFaces; i++) {
...
meshop_getEdgesReverseEdge_cf(Value** arg_list, int arg_count)
{
	...
	ValueToBitArrayM(arg_list[1], inedges, 3*nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_EDGE_INDEX_OUT_OF_RANGE), MESH_EDGESEL_ALLOWED, mesh);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BitArrayValue* edges);
	vl.edges = new BitArrayValue (3*nFaces);
	AdjEdgeList el(*mesh);
...
meshop_getElementsUsingFace_cf(Value** arg_list, int count)
{
	...
	if (fence != &unsupplied) 
		ValueToBitArrayM(fence, fencefaces, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_FACE_INDEX_OUT_OF_RANGE), MESH_FACESEL_ALLOWED, mesh);
	AdjEdgeList ae(*mesh);
	AdjFaceList af(*mesh, ae);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BitArrayValue* elementfaces);
	...
	return_value_tls(vl.elementfaces);
}
...
meshop_getFaceArea_cf(Value** arg_list, int arg_count)
{
	...
	return_value (Float::intern(area/2.0f));
}
...
meshop_getFaceCenter_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(faceIndex+1, 1, mesh->getNumFaces(), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_FACE_INDEX_OUT_OF_RANGE));
...
meshop_minVertexDistanceFrom_cf(Value** arg_list, int arg_count)
{
	...
	int nVerts=mesh->getNumVerts();
	MXS_range_check(vertIndex+1, 1, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTEX_INDEX_OUT_OF_RANGE));
	Point3 p = mesh->verts[vertIndex];
	BitArray verts (nVerts);
	ValueToBitArrayM(arg_list[2], verts, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTEX_INDEX_OUT_OF_RANGE), MESH_VERTSEL_ALLOWED, mesh);
	if (verts.IsEmpty()) return_value (Float::intern(-1.0f));
	bool first = true;
	float minDist = -1.0f;
	for (int i = 0; i < nVerts; i++) {
		if (!verts[i]) continue;
	...
	return_value (Float::intern(sqrt(minDist)));
}
...
// ============================================================================
// <bitarray> getFacesByMatId <Mesh mesh> <int mtlId>
Value*
meshop_getFacesByMatId_cf(Value** arg_list, int arg_count)
{
	check_arg_count(getFacesByMatId, 2, arg_count);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BitArrayValue* result);
	int mtlID = arg_list[1]->to_int() - 1; // convert to 0 based 
	Mesh* mesh = get_meshForValue(arg_list[0], MESH_READ_ACCESS, NULL, getFacesByMatId);
	vl.result = new BitArrayValue(mesh->getNumFaces());
	for (int i = 0; i < mesh->getNumFaces(); i++)
	{
		if (mtlID != mesh->faces[i].getMatID())
			continue;

		vl.result->bits.Set(i); //return value in 1 based
	}
...
// ============================================================================
// <bitarray> getVertsByMatId <Mesh mesh> <int mtlId>
Value*
meshop_getVertsByMatId_cf(Value** arg_list, int count)
{
	check_arg_count(getVertsByMatId, 2, count);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BitArrayValue* result);
	Mesh* mesh = get_meshForValue(arg_list[0], MESH_READ_ACCESS, NULL, getVertsByMatId);
	int matID = arg_list[1]->to_int() - 1; //convert to 0 based array;
	BitArray matVerts(mesh->numVerts);
	...
	for (int i = 0; i < mesh->getNumFaces(); i++)
	{
		...
		DWORD *verts = mesh->faces[i].getAllVerts();
		for (int j = 0; j < 3; j++)
		{
			if (matVerts[verts[j]])
				continue;

			matVerts.Set(verts[j]);
		}
	}

	vl.result = new BitArrayValue(matVerts);
	return_value_tls(vl.result);
}

// ============================================================================
// <bitarray> getAllFaces <Mesh mesh>
Value*
meshop_getAllFaces_cf(Value** arg_list, int arg_count)
{
	check_arg_count(getAllFaces, 1, arg_count);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BitArrayValue* result);
	Mesh* mesh = get_meshForValue(arg_list[0], MESH_READ_ACCESS, NULL, getAllFaces);
	vl.result = new BitArrayValue(mesh->getNumFaces());
	vl.result->bits.SetAll();
	return_value_tls(vl.result);
}

// ============================================================================
// int getFaceMatId <Mesh mesh> int face
Value*
meshop_getFaceMatId_cf(Value** arg_list, int count)
{
	check_arg_count(getFaceMatId, 2, count);
	Mesh* mesh = get_meshForValue(arg_list[0], MESH_READ_ACCESS, NULL, getFaceMatId);
	int face = arg_list[1]->to_int() - 1;
	MXS_range_check(face + 1, 1, mesh->getNumFaces(), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_FACE_INDEX_OUT_OF_RANGE));
	return_value(Integer::intern(mesh->faces[face].getMatID() + 1));
}

// ============================================================================
// <array> getFacesMatId <Mesh mesh> <bit_array facelist>
Value*
meshop_getFacesMatId_cf(Value** arg_list, int count)
{
	check_arg_count(getFacesMatId, 2, count);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(Array* matIds);
	Mesh* mesh = get_meshForValue(arg_list[0], MESH_READ_ACCESS, NULL, getFacesMatId);
	int nFaces = mesh->getNumFaces();
	BitArray whichFaces(nFaces);
	ValueToBitArrayM(arg_list[1], whichFaces, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_FACE_INDEX_OUT_OF_RANGE), MESH_FACESEL_ALLOWED, mesh);
	vl.matIds = new Array(whichFaces.NumberSet());
	for (int i = 0; i < nFaces; i++)
	{
		if (!whichFaces[i]) continue;
		
		vl.matIds->append(Integer::intern(mesh->faces[i].getMatID() + 1));
	}

	return_value_tls(vl.matIds);
}

// ============================================================================
//setFacesMatId <Mesh mesh> <bit_array facelist> <int matId>
Value*
meshop_setFaceMatId_cf(Value** arg_list, int count)
{
	check_arg_count(setFaceMatId, 3, count);
	ReferenceTarget *owner;
	Mesh* mesh = get_meshForValue(arg_list[0], MESH_BASE_OBJ, &owner, setFaceMatId);
	int matId = arg_list[2]->to_int() - 1;
	if (matId < 0)
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_MATID_MUST_BE_VE_NUMBER_GOT), arg_list[2]);
...
// 
	...
	// if source mesh comes from a node, it is not owned. Make sure to delete it.
	MaxSDK::AutoPtr<Mesh> pUnownedSourceMesh;
	if (arg_list[1]->is_kind_of(class_tag(MAXNode))) {
		pUnownedSourceMesh.Reset(sourceMesh); // take ownership
	...
	int attachMat = ConvertValueToID(mtlAttachTypes, elements(mtlAttachTypes), key_arg_or_default(attachMat, n_neither));
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	Matrix3 node1tm = node1 ? node1->GetObjectTM(MAXScript_time_tls()) : Matrix3(TRUE);
	Matrix3 node2tm = (sourceInLocalCoords && node2) ? node2->GetObjectTM(MAXScript_time_tls()) : Matrix3(TRUE);
	Matrix3 tm = node2tm * Inverse(node1tm);

	BOOL clearMapChannel = AttachDefaultClearMapChannel();
	if (clearMapChannel)
		targetMesh->SetFlag(MESH_CLEARMAPCHANNEL);
	else
		targetMesh->ClearFlag(MESH_CLEARMAPCHANNEL);
	...
	if (!canUndo) {
		theHold.Release();
		HoldSuspend hs; // LAM - 6/13/02 - defect 306957
...
//	else world_from_current_coordsys(normal,NO_TRANSLATE + NO_SCALE);
	int nEdges = 3*mesh->getNumFaces();
	MXS_range_check(edge1Index+1, 1, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_EDGE_INDEX_OUT_OF_RANGE));
	MXS_range_check(edge2Index+1, 1, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_EDGE_INDEX_OUT_OF_RANGE));
	Value* tmp;
...
{	
	...
	ValueToBitArrayM(arg_list[1], whichVerts, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTEX_INDEX_OUT_OF_RANGE), MESH_VERTSEL_ALLOWED, mesh);
...
{	
	...
	Point3 weldpointVal; 
	Point3* weldpoint = nullptr;
	if (is_point3(tmp))
	{
		weldpointVal = tmp->to_point3();
		weldpoint = &weldpointVal;
	}
	if (weldpoint) {
		Value* inNode = key_arg(node);
...
meshop_createPolygon_cf(Value** arg_list, int count)
{
	...
	if (!is_array(arg_list[1]))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_CREATEPOLYGON_VERTEX_LIST_MUST_BE_ARRAY_GOT), arg_list[1]);
	Array* vertArray = (Array*)arg_list[1];
	Value* tmp;
	int smGroup = int_key_arg(smGroup, tmp, 0); 
	int matID   = int_key_arg(matID, tmp, 1); 
	if (matID < 1)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_MATID_MUST_BE_VE_NUMBER_GOT), Integer::intern(matID));
...
meshop_detachVerts_cf(Value** arg_list, int count)
{
	...
	vl.res = &ok;
	if (asMesh) vl.res = new MeshValue(newMesh,TRUE);
...
// possibly add controllers for new verts
	if (mdu) {
		...
		mdu->PlugControllersSel(MAXScript_time_tls(),sel);
	}
...
meshop_getVert_cf(Value** arg_list, int count)
{
	...
	int vertIndex =  arg_list[1]->to_int()-1;
	MXS_range_check(vertIndex+1, 1, mesh->getNumVerts(), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTEX_INDEX_OUT_OF_RANGE));
...
// <point3_array> getVerts <Mesh mesh> <vertlist> node:<node=unsupplied>
Value*
meshop_getVerts_cf(Value** arg_list, int count)
{
	check_arg_count_with_keys(getVerts, 2, count);
	Mesh* mesh = get_meshForValue(arg_list[0], MESH_READ_ACCESS, NULL, getVerts);
	int nVerts = mesh->getNumVerts();
	BitArray verts(nVerts);
	ValueToBitArrayM(arg_list[1], verts, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTEX_INDEX_OUT_OF_RANGE), MESH_VERTSEL_ALLOWED, mesh);
	MAXNode* theNode = nullptr;
	Value* inNode = key_arg(node);
	if (is_node(inNode)) theNode = (MAXNode*)inNode;
	else if (is_node(arg_list[0])) theNode = (MAXNode*)arg_list[0];
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(Array* result);
	vl.result = new Array(verts.NumberSet());
	for (int i = 0; i < nVerts; i++)
	{
		if (verts[i])
		{
			Point3 p = mesh->verts[i]; // LOCAL COORDINATES!!!
			if (theNode) theNode->object_to_current_coordsys(p);
			vl.result->append(new Point3Value(p));
		}
	}
	return_value_tls(vl.result);
...
meshop_setVert_cf(Value** arg_list, int count)
{
	...
	if (arg_list[2]->is_kind_of(class_tag(Array)))
	{
		...
		if (posArray->size != nVertsSpecified)
			throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_VERT_AND_POS_ARRAYS_NOT_SAME_SIZE));
		...
		{
			Array* vertArray = (Array*)arg_list[1];
			MaxSDK::Array<IndexedPoint3SortVal> sortVals(nVertsSpecified);
...
meshop_moveVert_cf(Value** arg_list, int count)
{
	...
	else
	{
		...
		if (is_node(inNode))
			((MAXNode*)inNode)->object_from_current_coordsys_scaleRotate(offset);
		else if (is_node(arg_list[0]))
			((MAXNode*)arg_list[0])->object_from_current_coordsys_scaleRotate(offset);
	}
...
// <point3> getFace <Mesh mesh> <int faceIndex>
Value*
meshop_getFace_cf(Value** arg_list, int count)
{
	check_arg_count(getFace, 2, count);
	Mesh* mesh = get_meshForValue(arg_list[0], MESH_READ_ACCESS, NULL, getFace);
	int faceIndex = arg_list[1]->to_int() - 1;
	MXS_range_check(faceIndex + 1, 1, mesh->getNumFaces(), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_FACE_INDEX_OUT_OF_RANGE));
	DWORD *verts = mesh->faces[faceIndex].getAllVerts();
	return new Point3Value((float)verts[0] + 1, (float)verts[1] + 1, (float)verts[2] + 1);
}

// <point3_array> getFaces <Mesh mesh> <facelist>
Value*
meshop_getFaces_cf(Value** arg_list, int count)
{
	check_arg_count(getFaces, 2, count);
	Mesh* mesh = get_meshForValue(arg_list[0], MESH_READ_ACCESS, NULL, getFace);
	int nFaces = mesh->getNumFaces();
	BitArray faces(nFaces);
	ValueToBitArrayM(arg_list[1], faces, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_FACE_INDEX_OUT_OF_RANGE), MESH_FACESEL_ALLOWED, mesh);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(Array* result);
	vl.result = new Array(faces.NumberSet());
	for (int faceIndex = 0; faceIndex < nFaces; faceIndex++)
	{
		if (faces[faceIndex])
		{
			DWORD *verts = mesh->faces[faceIndex].getAllVerts();
			vl.result->append(new Point3Value((float)verts[0] + 1, (float)verts[1] + 1, (float)verts[2] + 1));
		}
	}
	return_value_tls(vl.result);
}
...
meshop_divideFaceByEdges_cf(Value** arg_list, int count)
{
	...
	int nFaces = mesh->getNumFaces();
	MXS_range_check(faceIndex+1, 1, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_FACE_INDEX_OUT_OF_RANGE));
	MXS_range_check(edge1Index+1, 1, 3*nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_EDGE_INDEX_OUT_OF_RANGE));
	MXS_range_check(edge2Index+1, 1, 3*nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_EDGE_INDEX_OUT_OF_RANGE));
	Value* tmp;
	bool fixNeighbors = bool_key_arg(fixNeighbors, tmp, TRUE) == TRUE; 
	bool split = bool_key_arg(split, tmp, FALSE) == TRUE; 
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
...
//			(*hdir)[i] = pdir;
		...
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_BEVELFACE__INVALID_DIR_ARGUMENT), dir);
...
meshop_divideFace_cf(Value** arg_list, int count)
{
	...
	if (is_point3(tmp)) {
		...
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_BARYCOORD_NOT_POINT3), tmp);
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
	MeshDelta tmd;
...
meshop_extrudeFaces_cf(Value** arg_list, int count)
{
	...
	if (dir == n_independent) 
		...
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_EXTRUDEFACE__INVALID_DIR_ARGUMENT), dir);
	mesh->FaceSel() = faces;
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
{	
	...
	ValueToBitArrayM(arg_list[1], whichEdges, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_EDGE_INDEX_OUT_OF_RANGE), MESH_EDGESEL_ALLOWED, mesh);
	float thresh = DegToRad (arg_list[2]->to_float());
	def_autoedge_types();
	int type = ConvertValueToID(autoedgeTypes, elements(autoedgeTypes), key_arg_or_default(type, n_setClear));
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
{	
	...
	MXS_range_check(edgeIndex+1, 1, 3*mesh->getNumFaces(), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_EDGE_INDEX_OUT_OF_RANGE));
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
meshop_extrudeEdges_cf(Value** arg_list, int count)
{
	...
	if (dir == n_independent) 
		...
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_EXTRUDEEDGE__INVALID_DIR_ARGUMENT), dir);
	mesh->EdgeSel() = edges;
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
...
// possibly add controllers for new verts. 
	if (mdu) mdu->PlugControllersSel(MAXScript_time_tls(),sel);
	if (height == 0.f && outline == 0.0f) return &ok;
	tmd.ClearAllOps();
	tmd.InitToMesh(*mesh);
	if (is_point3(dir)) 
...
meshop_setNumMaps_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(numMaps, 0, MAX_MESHMAPS, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_NUMMAPS_OUT_OF_RANGE));
	Value* tmp;
...
meshop_getNumMaps_cf(Value** arg_list, int arg_count)
{
	...
	return_value (Integer::intern(mesh->getNumMaps()));
}
...
meshop_setMapSupport_cf(Value** arg_list, int arg_count)
{
	...
	MXS_range_check(channel, -NUM_HIDDENMAPS, mesh->getNumMaps()-1, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_CHANNEL_OUT_OF_RANGE));
...
meshop_setNumMapFaces_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(channel, -NUM_HIDDENMAPS, MAX_MESHMAPS-1, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_CHANNEL_OUT_OF_RANGE));
	if (!mesh->mapSupport(channel))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_MAP_SUPPORT_NOT_ENABLED_FOR_CHANNEL), arg_list[1]);
	if (mesh->getNumMapVerts(channel) == 0)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_MUST_HAVE_AT_LEAST_ONE_MAP_VERTEX_TO_CREATE_MAP_FACE));
	if (fcount < 1)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_COUNT_MUST_NOT_BE_NEG_NUMBER), arg_list[2]);
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
meshop_getNumMapFaces_cf(Value** arg_list, int arg_count)
{
	...
	if (!mesh->mapSupport(channel))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_MAP_SUPPORT_NOT_ENABLED_FOR_CHANNEL), arg_list[1]);
	return_value (Integer::intern(GetNumMapFaces(mesh, channel)));
}
...
// can have map verts without having map faces
	...
	if (!mesh->mapSupport(channel))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_MAP_SUPPORT_NOT_ENABLED_FOR_CHANNEL), arg_list[1]);
	MXS_range_check(index+1, 1, mesh->getNumMapVerts(channel), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_VERTEX_INDEX_OUT_OF_RANGE));
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
meshop_setMapFace_cf(Value** arg_list, int arg_count)
{
	...
	if (!mesh->mapSupport(channel))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_MAP_SUPPORT_NOT_ENABLED_FOR_CHANNEL), arg_list[1]);
	MXS_range_check(index+1, 1, GetNumMapFaces(mesh, channel), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_FACE_INDEX_OUT_OF_RANGE));
	IPoint3 faceVertsI ((int)faceVerts.x-1, (int)faceVerts.y-1, (int)faceVerts.z-1);
	int nMapVerts = mesh->getNumMapVerts(channel);
	MXS_range_check(faceVertsI.x+1, 1, nMapVerts, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_VERTEX_INDEX_OUT_OF_RANGE));
	MXS_range_check(faceVertsI.y+1, 1, nMapVerts, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_VERTEX_INDEX_OUT_OF_RANGE));
	MXS_range_check(faceVertsI.z+1, 1, nMapVerts, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_VERTEX_INDEX_OUT_OF_RANGE));
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
meshop_applyUVWMap_cf(Value** arg_list, int count)
{
	...
	def_uvwmap_types();
	int uvwmapType = ConvertValueToID(uvwmapTypes, elements(uvwmapTypes), arg_list[1]);
	Value* tmp;
...
meshop_buildMapFaces_cf(Value** arg_list, int count)
{
	...
	if (mesh->getNumMapVerts(channel) == 0) {
		...
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_BUILDMAPFACES_MESH_HAS_NO_MAP_VERTICES), arg_list[1]);
	}
...
{	
	...
	int channel = arg_list[1]->to_int();
	MXS_range_check(channel, 0, MAX_MESHMAPS-1, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_CHANNEL_OUT_OF_RANGE));
	if (!mesh->mapSupport(channel))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_MAP_SUPPORT_NOT_ENABLED_FOR_CHANNEL), arg_list[1]);
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
meshop_deleteMapVertSet_cf(Value** arg_list, int count)
{
	...
	BitArray verts (nVerts);
	ValueToBitArray(arg_list[2], verts, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_VERTEX_INDEX_OUT_OF_RANGE));
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BitArrayValue* affectedFaces);
	vl.affectedFaces = new BitArrayValue (GetNumMapFaces(mesh, channel));
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
	mesh->DeleteMapVertSet(channel, verts, &vl.affectedFaces->bits);
	if (mdu) mdu->LocalDataChanged (MapChannelID(channel));
	return_value_tls(vl.affectedFaces);
}
...
meshop_getMapFacesUsingMapVert_cf(Value** arg_list, int arg_count)
{
	...
	ValueToBitArray(arg_list[2], mapVerts, nMapVerts, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_VERTEX_INDEX_OUT_OF_RANGE));
	int nMapFaces=GetNumMapFaces(mesh, channel);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BitArrayValue* mapFaces);
...
meshop_getMapVertsUsingMapFace_cf(Value** arg_list, int arg_count)
{
	...
	ValueToBitArray(arg_list[2], mapFaces, nMapFaces, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_FACE_INDEX_OUT_OF_RANGE));
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BitArrayValue* mapVerts);
...
{	
	...
	if (!mesh->mapSupport(channel)) {
		if (channel == 0)
			...
			throw MAXException (MaxSDK::GetResourceStringAsMSTR(IDS_MAP_SUPPORT_NOT_ENABLED_FOR_CHANNEL));
	}
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BitArrayValue* matches);
...
// can have map verts without having map faces
	...
	int nFaces=mesh->getNumFaces();
	BitArray faces(nFaces);
	ValueToBitArrayM(arg_list[2], faces, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_FACE_INDEX_OUT_OF_RANGE), MESH_FACESEL_ALLOWED, mesh);
	MXS_range_check(channel, -NUM_HIDDENMAPS, MAX_MESHMAPS-1, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_CHANNEL_OUT_OF_RANGE));
...
{	
	...
	int vertIndex =  arg_list[1]->to_int()-1;
	MXS_range_check(vertIndex+1, 1, mesh->GetVD(VDATA_WEIGHT)->Count(), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTDATA_INDEX_OUT_OF_RANGE));
...
{	
	...
	if (!mesh->vDataSupport(VDATA_WEIGHT))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_VERTEXDATA_SUPPORT_NOT_ENABLED_FOR_CHANNEL), Integer::intern(VDATA_WEIGHT+1));
...
{	
	...
	int vertIndex =  arg_list[1]->to_int()-1;
	MXS_range_check(vertIndex+1, 1, mesh->GetVD(VDATA_SELECT)->Count(), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTDATA_INDEX_OUT_OF_RANGE));
...
{	
	...
	if (!mesh->vDataSupport(VDATA_SELECT))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_VERTEXDATA_SUPPORT_NOT_ENABLED_FOR_CHANNEL), Integer::intern(VDATA_SELECT+1));
	int nVerts=mesh->GetVD(VDATA_SELECT)->Count();
	BitArray verts (nVerts);
	ValueToBitArray(arg_list[1], verts, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTDATA_INDEX_OUT_OF_RANGE));
	float weight =  arg_list[2]->to_float();
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
	float *vsw = mesh->getVSelectionWeights ();
...
{	
	...
	if (!mesh->vDataSupport(VDATA_CORNER))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_VERTEXDATA_SUPPORT_NOT_ENABLED_FOR_CHANNEL), Integer::intern(VDATA_CORNER+1));
	int vertIndex =  arg_list[1]->to_int()-1;
	MXS_range_check(vertIndex+1, 1, mesh->GetVD(VDATA_CORNER)->Count(), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTDATA_INDEX_OUT_OF_RANGE));
	return_value (Float::intern(mesh->vertexFloat(VDATA_CORNER)[vertIndex]));
}
...
{	
	...
	if (!mesh->vDataSupport(VDATA_CORNER))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_VERTEXDATA_SUPPORT_NOT_ENABLED_FOR_CHANNEL), Integer::intern(VDATA_CORNER+1));
	int nVerts=mesh->GetVD(VDATA_CORNER)->Count();
	BitArray verts (nVerts);
	ValueToBitArray(arg_list[1], verts, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTDATA_INDEX_OUT_OF_RANGE));
...
{	
	...
	if (!mesh->vDataSupport(VDATA_ALPHA))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_VERTEXDATA_SUPPORT_NOT_ENABLED_FOR_CHANNEL), Integer::intern(VDATA_ALPHA+1));
	int vertIndex =  arg_list[1]->to_int()-1;
	MXS_range_check(vertIndex+1, 1, mesh->GetVD(VDATA_ALPHA)->Count(), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTDATA_INDEX_OUT_OF_RANGE));
	return_value (Float::intern(mesh->vertexFloat(VDATA_ALPHA)[vertIndex]));
}
...
{	
	...
	if (!mesh->vDataSupport(VDATA_ALPHA))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_VERTEXDATA_SUPPORT_NOT_ENABLED_FOR_CHANNEL), Integer::intern(VDATA_ALPHA+1));
	int nVerts=mesh->GetVD(VDATA_ALPHA)->Count();
	BitArray verts (nVerts);
	ValueToBitArray(arg_list[1], verts, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTDATA_INDEX_OUT_OF_RANGE));
	float weight =  arg_list[2]->to_float();
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
...
meshop_setNumVDataChannels_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(numChannels, 0, MAX_VERTDATA, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_NUMVERTDATA_OUT_OF_RANGE));
	Value* tmp;
	bool keep = bool_key_arg(keep, tmp, TRUE) == TRUE; 
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
meshop_getNumVDataChannels_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(mesh->getNumVData()));
}
...
meshop_setVDataChannelSupport_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(vdChannel+1, 1, MAX_VERTDATA, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTDATA_CHANNEL_OUT_OF_RANGE));
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
...
meshop_getVDataValue_cf(Value** arg_list, int count)
{
	...
	if (!mesh->vDataSupport(vdChannel))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_VERTEXDATA_SUPPORT_NOT_ENABLED_FOR_CHANNEL), arg_list[1]);
	int index=arg_list[2]->to_int()-1;
	MXS_range_check(index+1, 1, mesh->GetVD(vdChannel)->Count(), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTDATA_INDEX_OUT_OF_RANGE));
	return_value (Float::intern(mesh->vertexFloat(vdChannel)[index]));
}
...
{	
	...
	if (count_with_keys() == 3) {
		...
		int mod_index = 0;
		if (Number::is_number(arg_list[1]))
		...
		if (mdu == NULL)
			throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_MOD_NO_MESH_DELTA_I) , arg_list[1]);
		if (!mdu->Editing()) 
				throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_MOD_NOT_ACTIVE) , arg_list[1]);
		...
		if (mdu == NULL)
			throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_BASEOBJECT_NO_MESHDELTA_I) , arg_list[0]);
		if (!mdu->Editing()) 
				throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_EDITABLEMESH_NOT_ACTIVE) , arg_list[0]);
		whichParam=arg_list[1];
	}
	def_MeshDeltaUserParam_types();
	meshUIParam param = (meshUIParam)ConvertValueToID(MeshDeltaUserParamTypes, elements(MeshDeltaUserParamTypes), whichParam);
...
// ============================================================================
// <int> getNumVelocity <Mesh mesh> 
Value*
meshop_getNumVelocity_cf(Value** arg_list, int arg_count)
{
	check_arg_count(getNumVelocity, 1, arg_count);

	Mesh* mesh = get_meshForValue(arg_list[0], MESH_READ_ACCESS, NULL, getNumVelocity);
	int ct = 0;

	if (mesh->GetInterface(IVERTEX_VELOCITY_INTERFACE_ID))
	{
		((IVertexVelocity*)mesh->GetInterface(IVERTEX_VELOCITY_INTERFACE_ID))->Velocity(ct);
	}
	return_value(Integer::intern(ct));	
}

// ============================================================================
// // <point3_array> getVelocity <Mesh mesh> <vertlist> 
Value*
meshop_getVelocity_cf(Value** arg_list, int arg_count)
{
	check_arg_count(getVelocity, 2, arg_count);
	Mesh* mesh = get_meshForValue(arg_list[0], MESH_READ_ACCESS, NULL, getVelocity);
	int ct = 0;
	const Point3* velArray = nullptr;
	if (mesh->GetInterface(IVERTEX_VELOCITY_INTERFACE_ID))
		velArray = ((IVertexVelocity*)mesh->GetInterface(IVERTEX_VELOCITY_INTERFACE_ID))->Velocity(ct);
	BitArray verts(ct);
	ValueToBitArrayM(arg_list[1], verts, ct, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTEX_INDEX_OUT_OF_RANGE), MESH_VERTSEL_ALLOWED, mesh);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(Array* result);
	vl.result = new Array(ct);
	for (int i = 0; i < ct; i++) 
	{
		if (verts[i])
		{
			vl.result->append(new Point3Value(velArray[i]));
		}
	}
	return_value_tls(vl.result);
}
