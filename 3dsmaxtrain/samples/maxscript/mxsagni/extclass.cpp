...
visible_class_instance(BigMatrixRowArray, "BigMatrixRowArray")
		...
		BigMatrixRowArray::BigMatrixRowArray(BigMatrixValue* bmv, int row)
		: bmv(bmv)
		, row(row)
{
...
void BigMatrixRowArray::collect()
{
	// If collecting BigMatrixRowArray and owning BigMatrixValue in same pass, BigMatrixValue
	// may have already been deleted. BigMatrixValue dtor will clear 'bmv' backpointer.
	// Or, BigMatrixValue may have been resized, resulting in 'bmv' having been cleared.
	// Code is set up such that BigMatrixValue is a weak reference cache, allowing BigMatrixRowArray values
	// it owns to be deleted while the BigMatrixValue still exists. Clear ourself from BigMatrixValue,
	if (bmv)
		bmv->rowArrays[row] = nullptr;
	delete this;
}

void BigMatrixRowArray::sprin1(CharStream* s)
{
	// check to see if BigMatrixValue was resized and we are no longer applicable as we were pointing
	// at row that no longer exists.
	if (bmv == nullptr)
	{
		s->puts(MaxSDK::GetResourceStringAsMSTR(IDS_BIGMATRIX_RESIZED_BAD_ROW));
		return;
	}
...
Value* BigMatrixRowArray::get_vf(Value** arg_list, int count)
{
	...
	if (bmv == nullptr)
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_BIGMATRIX_RESIZED_BAD_ROW));
...
Value* BigMatrixRowArray::put_vf(Value** arg_list, int count)
{
	...
	if (index > bmv->bm.n)
		throw RuntimeError(
				MaxSDK::GetResourceStringAsMSTR(IDS_RK_ARRAY_INDEX_MUST_BE_LESS_THAN_NUMBER_OF_COLUMNS_GOT), arg);

	bmv->bm[row][index - 1] = arg_list[1]->to_float();
	return arg_list[1];
}

Value* BigMatrixRowArray::coerce_vf(Value** arg_list, int count)
{
	Value* cl = arg_list[0];
	if (!is_valueclass(cl))
		throw TypeError(MaxSDK::GetResourceStringAsMSTR(IDS_TYPE_CONVERSION_NEEDS_A_CLASS_ARGUMENT), cl);
	if (cl == class_tag(Array))
	{
		// check to see if BigMatrixValue was resized and we are no longer applicable as we were pointing
		// at a row that no longer exists.
		if (bmv == nullptr)
		...
		vl.result = new Array(bmv->bm.n);
		for (int n = 0; n < bmv->bm.n; n++)
			vl.result->append(Float::intern(bmv->bm[row][n]));
		return_value(vl.result);
	}
	else
		return Value::coerce_vf(arg_list, count);
}

void BigMatrixRowArray::gc_trace()
{
	/* trace sub-objects & mark me */
	Value::gc_trace();
	// keep owning BigMatrixValue alive
...
visible_class_instance(BigMatrixValue, "BigMatrix")
		...
		Value* BigMatrixValueClass::apply(Value** arg_list, int count, CallContext* cc)
{
...
void BigMatrixValue::gc_trace()
{
	/* trace sub-objects & mark me */
	Value::gc_trace();

	// Uncomment following to have all BigMatrixRowArray sub-values have the same lifetime as this
	// owning BigMatrixValue;
	// Code is currently set up so that BigMatrixRowArray sub-values can be deleted while this
	// owning BigMatrixValue still exists. The BigMatrixValue acts as a weak reference cache - when a
	// BigMatrixRowArray is deleted the corresponding Tab entry here is nulled.
	/*
	int count = rowArrays.Count();
	for (int m = 0; m < count; m++)
	{
		BigMatrixRowArray *rowArray = rowArrays[m];
		if (rowArray != nullptr && rowArray->is_not_marked())
			rowArray->gc_trace();
	}
	*/
}

void BigMatrixValue::collect()
{
	int count = rowArrays.Count();
	for (int m = 0; m < count; m++)
	{
		BigMatrixRowArray* rowArray = rowArrays[m];
		if (rowArray != nullptr)
			rowArray->bmv_deleted();
	}
	delete this;
}

void BigMatrixValue::sprin1(CharStream* s)
{
	s->puts(_T("#BigMatrix(\n"));
	int maxItemsR = bm.m;
	int maxItemsC = bm.n;
...
Value* BigMatrixValue::get_property(Value** arg_list, int count)
{
...
Value* BigMatrixValue::set_property(Value** arg_list, int count)
{
	Value* prop = arg_list[1];
	if (prop == n_rows)
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_RK_CANNOT_SET_BIGMATRIX_PROPERTY), prop);
	else if (prop == n_columns)
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_RK_CANNOT_SET_BIGMATRIX_PROPERTY), prop);
	return Value::set_property(arg_list, count);
}

Value* BigMatrixValue::get_vf(Value** arg_list, int count)
{
	check_arg_count(get, 2, count + 1);
	Value* arg = arg_list[0];
	int index;
	...
	{
		int row = index - 1;
		if (rowArrays[row] == nullptr)
			rowArrays[row] = new BigMatrixRowArray(this, row);
		result = rowArrays[row];
	}

	return result;
}

Value* BigMatrixValue::put_vf(Value** arg_list, int count)
{
	check_arg_count(put, 3, count + 1);
	Value* arg = arg_list[0];
	int rowIndex = arg->to_int();
	MXS_range_check(rowIndex, 1, bm.m, MaxSDK::GetResourceStringAsMSTR(IDS_BIGMATRIX_ROW_INDEX_OUT_OF_RANGE));
	Array* rowVals = (Array*)arg_list[1];
	type_check(rowVals, Array, _T("BigMatrix row"));

	// create float tab first and then assign elements, in case array contains value that cannot be
	// converted to float. Don't want to bail 1/2 way through setting a row.
	Tab<float> vals;
	vals.SetCount(rowVals->size);
	for (int i = 0; i < rowVals->size; ++i)
		vals[i] = rowVals->data[i]->to_float();

	float* rowData = bm[rowIndex - 1];
...
Value* BigMatrixValue::plus_vf(Value** arg_list, int count)
{
	if (!is_bigmatrix(arg_list[0]))
		throw RuntimeError(
				MaxSDK::GetResourceStringAsMSTR(IDS_RK_RIGHT_HAND_OPERATOR_SHOULD_BE_A_BIGMATRIX_GOT), arg_list[0]);
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BigMatrixValue * result);
	vl.result = new BigMatrixValue(bm.m, bm.n);
	for (int m = 0; m < bm.m; m++)
		for (int n = 0; n < bm.n; n++)
			vl.result->bm[m][n] = bm[m][n] + bmat[m][n];
	return_value_tls(vl.result);
}

Value* BigMatrixValue::transpose_vf(Value** arg_list, int count)
{
...
Value* BigMatrixValue::setSize_vf(Value** arg_list, int count)
{
	...
	int old_count = rowArrays.Count();
	// disconnect BigMatrixRowArray instances pointing at rows no longer present for when numbers of rows is decreased
	for (int m = mm; m < old_count; m++)
	{
		BigMatrixRowArray* rowArray = rowArrays[m];
		if (rowArray != nullptr)
			rowArray->bmv_deleted();
	...
	rowArrays.SetCount(mm);
	// null new elements for when numbers of rows is increased
	for (int m = old_count; m < mm; m++)
		rowArrays[m] = nullptr;

	return this;
}

Value* BigMatrixValue::clear_vf(Value** arg_list, int count)
{
	check_arg_count(clear, 1, count + 1);
	bm.Clear();
	// disconnect BigMatrixRowArray instances pointing at rows no longer present
	int old_count = rowArrays.Count();
...
void MAXNoteKeyArray::sprin1(CharStream* s)
{
	...
	BOOL print_all_elements = GetPrintAllElements();
	int num_keys = note_track->note_track->NumKeys();
	int i;
	s->puts(_T("#keys("));
	for (i = 0; i < num_keys; i++)
	{
		if (!print_all_elements && i >= 20) // limit display to 20 elements
		{
			s->puts(_T("..."));
			break;
		}
...
Value* MAXNoteKeyArray::get_vf(Value** arg_list, int count)
{
...
Value* MAXNoteKeyArray::put_vf(Value** arg_list, int count)
{
	deletion_check(note_track);
...
Value* MAXNoteKeyArray::append_vf(Value** arg_list, int count)
{
	deletion_check(note_track);
...
Value* MAXNoteKeyArray::deleteItem_vf(Value** arg_list, int count)
{
...
Value* MAXNoteKeyArray::findItem_vf(Value** arg_list, int count)
{
...
Value* MAXNoteKeyArray::getNoteKeyTime_vf(Value** arg_list, int count)
{
...
Value* MAXNoteKeyArray::getNoteKeyIndex_vf(Value** arg_list, int count)
{
	...
	for (int i = 0; i < note_track->note_track->NumKeys(); i++)
		if (t == note_track->note_track->GetKeyTime(i))
			return_value(Integer::intern(i + 1));
	return &undefined;
}

Value* MAXNoteKeyArray::sortNoteKeys_vf(Value** arg_list, int count)
{
...
Value* MAXNoteKeyArray::addNewNoteKey_vf(Value** arg_list, int count)
{
	...
	else if (Number::is_number(arg0) || is_time(arg0))
	{
		t = arg0->to_timevalue();
		i = 1;
	}
...
Value* MAXNoteKeyArray::deleteNoteKeys_vf(Value** arg_list, int count)
{
...
Value* MAXNoteKeyArray::deleteNoteKey_vf(Value** arg_list, int count)
{
...
Value* MAXNoteKeyArray::map(node_map& m)
{
	deletion_check(note_track);
	int num_keys = note_track->note_track->NumKeys();
	int i;
...
Value* MAXNoteKeyArray::join_vf(Value** arg_list, int count)
{
	throw NoMethodError(_T("join"), this);
	return &undefined;
}
Value* MAXNoteKeyArray::sort_vf(Value** arg_list, int count)
{
	throw NoMethodError(_T("sort"), this);
	return &undefined;
}

/* ---------------- MAXNoteKeyArray property access ------------------------- */

Value* MAXNoteKeyArray::get_count(Value** arg_list, int count)
{
	deletion_check(note_track);
	return_value(Integer::intern(note_track->note_track->NumKeys()));
}

Value* MAXNoteKeyArray::set_count(Value** arg_list, int count)
{
	deletion_check(note_track);
...
void MAXNoteKey::sprin1(CharStream* s)
{
	if (deletion_check_test(note_track))
	{
	...
	if (key_index >= note_track->note_track->NumKeys())
		...
		s->printf(_T("#Note key(%d @ %gf)"), key_index + 1,
				note_track->note_track->GetKeyTime(key_index) / (float)GetTicksPerFrame());
}


Value* MAXNoteKey::delete_vf(Value** arg_list, int count)
{
	//	delete <key>
	deletion_check(note_track);
...
Value* MAXNoteKey::get_time(Value** arg_list, int count)
{
	deletion_check(note_track);
	if (key_index >= note_track->note_track->NumKeys())
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_KEY_NO_LONGER_EXISTS_IN_CONTROLLER), note_track);
...
Value* MAXNoteKey::set_time(Value** arg_list, int count)
{
	deletion_check(note_track);
	TimeValue t = arg_list[0]->to_timevalue();
	if (key_index >= note_track->note_track->NumKeys())
...
Value* MAXNoteKey::get_selected(Value** arg_list, int count)
{
	deletion_check(note_track);
	if (key_index >= note_track->note_track->NumKeys())
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_KEY_NO_LONGER_EXISTS_IN_CONTROLLER), note_track);
...
Value* MAXNoteKey::set_selected(Value** arg_list, int count)
{
	deletion_check(note_track);
	if (!Number::is_integer_number(arg_list[0]))
...
Value* MAXNoteKey::get_value(Value** arg_list, int count)
{
	deletion_check(note_track);
	if (key_index >= note_track->note_track->NumKeys())
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_KEY_NO_LONGER_EXISTS_IN_CONTROLLER), note_track);
...
Value* MAXNoteKey::set_value(Value** arg_list, int count)
{
	deletion_check(note_track);
...
Value* add_note_track_cf(Value** arg_list, int count)
{
...
Value* delete_note_track_cf(Value** arg_list, int count)
{
...
Value* delete_all_note_tracks_cf(Value** arg_list, int count)
{
	check_arg_count(deleteAllNoteTracks, 1, count);
	Animatable* anim = setup_anim_access(arg_list[0]);
	if (anim)
		anim->DeleteAllNoteTracks();
	return &ok;
}

Value* has_note_tracks_cf(Value** arg_list, int count)
{
...
Value* num_note_tracks_cf(Value** arg_list, int count)
{
...
Value* get_note_track_cf(Value** arg_list, int count)
{
...
visible_class_instance(MAXNoteTrack, "NoteTrack")
		...
		Value* MAXNoteTrackClass::apply(Value** arg_list, int count, CallContext* cc)
{
...
Value* MAXNoteTrack::intern(DefNoteTrack* dnt)
{
	if (dnt == NULL)
		return &undefined;
	...
	MAXNoteTrack* mxc;
	MAXNoteTrack** mxcp;
	if (maxwrapper_cache_get(MAXNoteTrack, dnt, mxc, mxcp) && mxc->note_track == dnt)
		return mxc;
	else
	{
...
void MAXNoteTrack::sprin1(CharStream* s)
{
	check_for_deletion();
	s->printf(_T("Notetrack:%s"), note_track->trackName);
}

void MAXNoteTrack::gc_trace()
{
	Value::gc_trace();
}

Value* MAXNoteTrack::copy_vf(Value** arg_list, int count)
{
	check_for_deletion();
	check_gen_arg_count_with_keys(copy, 1, count + 1);
...
Value* MAXNoteTrack::get_property(Value** arg_list, int count)
{
	check_for_deletion();
	Value* prop = arg_list[0];
	if (prop == n_name)
...
Value* MAXNoteTrack::set_property(Value** arg_list, int count)
{
	check_for_deletion();
	Value* prop = arg_list[1];
	if (prop == n_name)
...
Value* MAXNoteTrack::get_keys(Value** arg_list, int count)
{
	check_for_deletion();
	return new MAXNoteKeyArray(note_track);
}

Value* MAXNoteTrack::set_keys(Value** arg_list, int count)
{
	throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_CANNOT_DIRECTLY_SET_KEY_ARRAY));
	return &undefined;
}

Value* MAXNoteTrack::getNoteKeyTime_vf(Value** arg_list, int count)
{
...
Value* MAXNoteTrack::getNoteKeyIndex_vf(Value** arg_list, int count)
{
	check_for_deletion();
	check_arg_count(getNoteKeyIndex, 2, count + 1);
	TimeValue t = arg_list[0]->to_timevalue();
	for (int i = 0; i < note_track->NumKeys(); i++)
		if (t == note_track->GetKeyTime(i))
			return_value(Integer::intern(i + 1));
	return &undefined;
}

Value* MAXNoteTrack::sortNoteKeys_vf(Value** arg_list, int count)
{
...
Value* MAXNoteTrack::addNewNoteKey_vf(Value** arg_list, int count)
{
...
Value* MAXNoteTrack::deleteNoteKeys_vf(Value** arg_list, int count)
{
...
Value* MAXNoteTrack::deleteNoteKey_vf(Value** arg_list, int count)
{
...
BOOL MSTrackViewFilter::proc(Animatable* anim, Animatable* client, int subNum)
{
	...
	{
		filter_fn = &undefined;
		ProcessUnknownMAXScriptException(_T("TrackView Filter Function"), true, true, true);
	}
	return res;
}
// #endif
...
Value* tvw_pickTrackDlg_cf(Value** arg_list, int count)
{
...
Value* MAXFilter::intern(FilterKernel* ifilter)
{
	if (ifilter == NULL)
		return &undefined;
	...
	MAXFilter* mxa;
	MAXFilter** mxap;
	if (maxwrapper_cache_get(MAXFilter, ifilter, mxa, mxap) && mxa->filter == ifilter)
		return mxa;
	else
	{
...
void MAXFilter::sprin1(CharStream* s)
{
	TSTR cnm;
	check_for_deletion();
	filter->GetClassName(cnm, true);
	s->printf(_T("%s:%s"), cnm, filter->name);
}

Value* MAXFilter::get_name(Value** arg_list, int count)
{
	check_for_deletion();
	return new String(filter->name);
}

Value* MAXFilter::set_name(Value** arg_list, int count)
{
	check_for_deletion();
...
Value* set_anti_alias_filter(Value* val)
{
...
Value* make_max_filter(MAXClass* cls, ReferenceTarget* existing, Value** arg_list, int count)
{
	FilterKernel* e;
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
...
MAXSuperClass filter_class(_T("Filter"), FILTER_KERNEL_CLASS_ID, &maxwrapper_class, make_max_filter, p_end);
...
visible_class_instance(NodeEventCallbackValue, "NodeEventCallback")
		...
		Value* NodeEventCallbackValueClass::apply(Value** arg_list, int count, CallContext* cc)
{
	...
	static BOOL sShutdownCallbackRegistered =
			RegisterNotification(NodeEventCallbackValue_Shutdown, nullptr, NOTIFY_SYSTEM_SHUTDOWN);
...
void NodeEventCallbackValue::gc_trace()
{
	/* trace sub-objects & mark me */
	Value::gc_trace();
...
void NodeEventCallbackValue::sprin1(CharStream* s)
{
	s->printf(_T("NodeEventCallback<0x%p>"), this);
}

Value* NodeEventCallbackValue::get_property(Value** arg_list, int count)
{
...
Value* NodeEventCallbackValue::set_property(Value** arg_list, int count)
{
	...
	{
		GetISceneEventManager()->UnRegisterCallback(m_CallbackKey);
		m_CallbackKey = GetISceneEventManager()->RegisterCallback(
				this, polling, polling ? 0 : delayMilliseconds, polling ? FALSE : mouseUp);
	}
	return val;
}

Value* NodeEventCallbackValue::get_props_vf(Value** arg_list, int count)
{
...
int NodeEventCallbackValue::EventNameToIndex(Value* eventName)
{
	def_nodeeventcallback_event_types();
	return ::ConvertValueToID(nodeeventcallbackEventTypes, elements(nodeeventcallbackEventTypes), eventName, -2);
}

void NodeEventCallbackValue::CallHandler(Value* eventName, NodeKeyTab& nodes)
{
	if (!enabled)
		return;
	...
	if (event_fn != &undefined || all_fn != &undefined)
	{
		...
		try
		{
			...
			// call the handler function. Re-acquire since may have been removed via all handler. Also make sure still
			// enabled
			event_fn = m_CallbackFunctions->data[event_index];
		...
		{
			ProcessUnknownMAXScriptException(MaxSDK::GetResourceStringAsMSTR(IDS_NODEEVENTCALLBACK), true, true, true);
		}
	}
}
...
void NodeEventCallbackValue::TopologyChanged(NodeKeyTab& nodes)
{
...
void NodeEventCallbackValue::MaterialStructured(NodeKeyTab& nodes)
{
	CallHandler(n_materialStructured, nodes);
}

void NodeEventCallbackValue::MaterialOtherEvent(NodeKeyTab& nodes)
{
...
void NodeEventCallbackValue::ControllerStructured(NodeKeyTab& nodes)
{
	CallHandler(n_controllerStructured, nodes);
}

void NodeEventCallbackValue::ControllerOtherEvent(NodeKeyTab& nodes)
{
...
void NodeEventCallbackValue::WireColorChanged(NodeKeyTab& nodes)
{
	CallHandler(n_wireColorChanged, nodes);
}

void NodeEventCallbackValue::RenderPropertiesChanged(NodeKeyTab& nodes)
{
	CallHandler(n_renderPropertiesChanged, nodes);
}

void NodeEventCallbackValue::DisplayPropertiesChanged(NodeKeyTab& nodes)
{
	CallHandler(n_displayPropertiesChanged, nodes);
}

void NodeEventCallbackValue::UserPropertiesChanged(NodeKeyTab& nodes)
{
	CallHandler(n_userPropertiesChanged, nodes);
}

void NodeEventCallbackValue::PropertiesOtherEvent(NodeKeyTab& nodes)
{
...
void NodeEventCallbackValue::SubobjectSelectionChanged(NodeKeyTab& nodes)
{
	CallHandler(n_subobjectSelectionChanged, nodes);
}

void NodeEventCallbackValue::SelectionChanged(NodeKeyTab& nodes)
{
	CallHandler(n_selectionChanged, nodes);
}

void NodeEventCallbackValue::HideChanged(NodeKeyTab& nodes)
{
	CallHandler(n_hideChanged, nodes);
}

void NodeEventCallbackValue::FreezeChanged(NodeKeyTab& nodes)
{
	CallHandler(n_freezeChanged, nodes);
}

void NodeEventCallbackValue::DisplayOtherEvent(NodeKeyTab& nodes)
{
...
void NodeEventCallbackValue::CallbackBegin()
{
	processing++;
...
void NodeEventCallbackValue::CallbackEnd()
{
	NodeKeyTab nodes_empty;
...
BaseInterface* NodeEventCallbackValue::GetInterface(Interface_ID id)
{
...
Value* triggerNodeEventCallback_cf(Value** arg_list, int count)
{
...
{
public:
	CollectNodeEventCallbackValueEnumerator(Array* instances)
...
Value* getNodeEventCallbacks_cf(Value** arg_list, int count)
{
...
void ProcessCallbackFunctionDisplay(
		NodeEventCallbackValue* pNodeEventCallbackValue, Value* eventID, Value* cbFn, Array* resArray, CharStream* out)
{
	TSTR fnName;
	...
	if (is_maxscriptfunction(cbFn))
	{
		...
		if (is_sourcefilewrapper(body) && body->mSourceFileName)
		{
			sourceFile = body->mSourceFileName->to_string();
		}
	}
...
Value* showNodeEventCallbacks_cf(Value** arg_list, int count)
{
	...
	for (int i = 0; i < vl.instances->size; i++)
	{
		...
		for (int j = 0; j < elements(nodeeventcallbackEventTypes); j++)
		{
			...
			if (cbFn != &undefined)
				ProcessCallbackFunctionDisplay(
						pNodeEventCallbackValue, nodeeventcallbackEventTypes[j].val, cbFn, vl.res, out);
		}
	}
	if (asArray)
		return_value_tls(vl.res);
