...
/* -------------------- ish3 : ProgressBar  ------------------- */
visible_class_s (ish3_ProgressBar, RolloutControl)
...
LRESULT CALLBACK ish3_ProgressBar::WndProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_CLOSE:
		MXSToolTipExtender::instance().RemoveToolTipExt(UGC); // Remove extender-based tooltip, if any
		break;
	}
	return TRUE;
}
	...
	ish3_ProgressBar::get_property(Value** arg_list, int count)
{
	...
	if (prop == n_value)
		return_value (Integer::intern((int)value) );
	...
	ish3_ProgressBar::set_enable()
{
	...
	if (parent_rollout != NULL && parent_rollout->page != NULL && progbar != NULL)
	{
...
void ProgressBarInit()
{
	...
	install_rollout_control(Name::intern(_T("progressBar")), ish3_ProgressBar::create);
}
