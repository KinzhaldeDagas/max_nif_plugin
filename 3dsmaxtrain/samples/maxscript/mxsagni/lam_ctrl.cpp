...
getLinkCount_cf(Value** arg_list, int count)
{
	...
	if (c->ClassID() != LINKCTRL_CLASSID)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_NOT_LINK_CONTROL), arg_list[0]);
	return_value (Integer::intern(((ILinkCtrl*)c)->GetNumTargets()));
}
...
getLinkTime_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(i, 1, ((ILinkCtrl*)c)->GetNumTargets(), MaxSDK::GetResourceStringAsMSTR(IDS_LINK_CONTROL_INDEX_OUT_OF_RANGE));
	return_value (MSTime::intern(((ILinkCtrl*)c)->GetLinkTime(i-1)));
}
...
setLinkNode_cf(Value** arg_list, int count)
{
	...
	if (pLinkerInst)
	{
		MXS_range_check(i, 1, pLinkerInst->GetNumTargets(), MaxSDK::GetResourceStringAsMSTR(IDS_LINK_CONTROL_INDEX_OUT_OF_RANGE));
...
struct MAXAKeyClass : public ValueMetaClass
{
	MAXAKeyClass(const TCHAR* name) : ValueMetaClass(name) {}
...
public:
	...
	void		Invalidate();
	Value*		deep_copy(HashTable* remap) override;
...
MAXAKey::get_property(Value** arg_list, int count)
{
	...
	if (prop == n_face) 
		return_value (Integer::intern(k->face));
...
{
	AKey* k = setup_key_access();
	return_value (Float::intern(ToTCBUI(k->tens)));
}
...
{
	AKey* k = setup_key_access();
	return_value (Float::intern(ToTCBUI(k->bias)));
}
...
{
	AKey* k = setup_key_access();
	return_value (Float::intern(ToTCBUI(k->cont)));
}
...
{
	AKey* k = setup_key_access();
	return_value (Float::intern(ToEaseUI(k->easeIn)));
}
...
{
	AKey* k = setup_key_access();
	return_value (Float::intern(ToEaseUI(k->easeOut)));
}
...
{
	deletion_check(controller);
	return_value (MSTime::intern(controller->controller->GetKeyTime(key_index)));
}
...
MAXAKey::set_selected(Value** arg_list, int count)
{
	...
	controller->controller->SelectKeyByIndex(key_index, arg_list[0]->to_bool());
	return arg_list[0];
}
...
Value*
MAXAKey::deep_copy(HashTable* remap)
{
	return this;
}
...
MAXAKey::sprin1(CharStream* s)
{
	...
	TSTR cnm;
	controller->GetClassName(cnm, true);
	if (key_index >= controller->controller->NumKeys())
...
attach_get_node(ReferenceTarget* obj, Value* prop, TimeValue t, Interval& valid)
{
	...
	if (theNode) return_value (MAXNode::intern(theNode));
	return &undefined;
}
...
attach_set_node(ReferenceTarget* obj, Value* prop, TimeValue t, Value* val)
{
	...
	if (ac)
	{
		...
		if (!res)
			throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_ATTACH_SET_NODE_FAILED), val);
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		needs_redraw_set(_tls);
	}
}
...
attach_set_manupdate(ReferenceTarget* obj, Value* prop, TimeValue t, Value* val)
{
	...
	if (ac)
	{
		...
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		if (!val->to_bool()) needs_redraw_set(_tls);
	}
}
...
Attachment_GetKey_cf(Value** arg_list, int count)
{
	...
	if (c->GetInterface(I_ATTACHCTRL) == NULL)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_NOT_ATTACH_CONTROL), arg_list[0]);
	int	index = arg_list[1]->to_int();
	MXS_range_check(index, 1, c->NumKeys(), MaxSDK::GetResourceStringAsMSTR(IDS_ATTACH_KEY_INDEX_OUT_OF_RANGE));
...
Attachment_AddNewKey_cf(Value** arg_list, int count)
{
	...
	{	if (arg_list[2] == n_select)
			...
			throw TypeError (MaxSDK::GetResourceStringAsMSTR(IDS_UNRECOGNIZED_FLAG_FOR_ADDNEWKEY), arg_list[2]);
	}
	c->AddNewKey(t, flags);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	needs_redraw_set(_tls);
...
euler_get_order(ReferenceTarget* obj, Value* prop, TimeValue t, Interval& valid)
{
	...
	if (ec)
		return_value (Integer::intern(ec->GetOrder()+1));
	else
		return &undefined;
}
...
euler_set_order(ReferenceTarget* obj, Value* prop, TimeValue t, Value* val)
{
	...
	MXS_range_check(order, 1, 9, MaxSDK::GetResourceStringAsMSTR(IDS_AXIS_ORDER_INDEX_OUT_OF_RANGE));
	IEulerControl* ec = (IEulerControl*)obj->GetInterface(I_EULERCTRL);
...
ListController_GetName_cf(Value** arg_list, int count)
{
	...
	if (c->ClassID() != FLOATLIST_CONTROL_CLASSID && c->ClassID() != POINT3LIST_CONTROL_CLASSID &&
		...
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_NOT_LIST_CONTROL), arg_list[0]);

	int index = arg_list[1]->to_int();
	MXS_range_check(index, 1, ((IListControl*)c)->GetListCount(), MaxSDK::GetResourceStringAsMSTR(IDS_LIST_CONTROL_INDEX_OUT_OF_RANGE));
...
ListController_SetName_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(index, 1, lc->GetListCount(), MaxSDK::GetResourceStringAsMSTR(IDS_LIST_CONTROL_INDEX_OUT_OF_RANGE));
...
ListController_GetActive_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(((IListControl*)c)->GetActive()+1));  // array indexes are 1-based !!!
}
