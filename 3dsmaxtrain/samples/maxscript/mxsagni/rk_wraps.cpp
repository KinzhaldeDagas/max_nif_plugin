...
{
	// createPreview outputAVI:<bool> percentSize:<int> start:<int> end:<int> skip:<int> fps:<int>
	//		dspGeometry:<bool> dspShapes:<bool> dspLights:<bool> dspCameras:<bool> dspHelpers:<bool> dspSpaceWarps:<bool>
	//		dspParticles:<bool> dspBones:<bool> dspGrid:<bool> dspSafeFrame:<bool> dspFrameNums:<bool>
	//		rndLevel:<#smoothhighlights | #smooth | #facethighlights | #facet | #flat | #hiddenline | #litwireframe | #wireframe | #box> 
	//		dspBkg:<bool> useMPCamEffect:<bool> filename:<filename> snippet:<string>
	//		dspCamViewName:<bool> autoPlay:<bool>
	...
	// 		vpEdgedFaces:<bool> vpTextures:<bool> 
	// Note: rndLevel not applicable for Nitrous driver. To set Nitrous viewport settings, set vpPreset/vpStyle/vpEdgedFaces/vpTextures)
	//
	check_arg_count_with_keys(CreatePreview, 0, count);
	PreviewParams previewParams = GetCOREInterface14()->GetPreviewParams();
	Interval animRange = MAXScript_interface->GetAnimRange();
	int smallestFrame = animRange.Start() / GetTicksPerFrame();
	int largestFrame = animRange.End() / GetTicksPerFrame();

	if(previewParams.end < previewParams.start) {
		previewParams.start = smallestFrame;
		previewParams.end = largestFrame;
	}
	if(previewParams.start < smallestFrame)
		previewParams.start = smallestFrame;
	if(previewParams.end > largestFrame)
		previewParams.end = largestFrame;

	Value* v;
	previewParams.outputType	= !bool_key_arg(outputAVI, v, !previewParams.outputType);
	previewParams.pctSize		= int_key_arg(percentSize, v, previewParams.pctSize);
	previewParams.start			= int_key_arg(start, v, previewParams.start);
	previewParams.end			= int_key_arg(end, v, previewParams.end);
	previewParams.skip			= int_key_arg(skip, v, previewParams.skip);
	previewParams.fps			= int_key_arg(fps, v, previewParams.fps);
	previewParams.dspGeometry	= bool_key_arg(dspGeometry, v, previewParams.dspGeometry);
	previewParams.dspShapes		= bool_key_arg(dspShapes, v, previewParams.dspShapes);
	previewParams.dspLights		= bool_key_arg(dspLights, v, previewParams.dspLights);
	previewParams.dspCameras	= bool_key_arg(dspCameras, v, previewParams.dspCameras);
	previewParams.dspHelpers	= bool_key_arg(dspHelpers, v, previewParams.dspHelpers);
	previewParams.dspSpaceWarps	= bool_key_arg(dspSpaceWarps, v, previewParams.dspSpaceWarps);
	previewParams.dspParticles	= bool_key_arg(dspParticles, v, previewParams.dspParticles);
	previewParams.dspBones		= bool_key_arg(dspBones, v, previewParams.dspBones);

	previewParams.dspGrid		= bool_key_arg(dspGrid, v, previewParams.dspGrid);
	previewParams.dspSafeFrame	= bool_key_arg(dspSafeFrame, v, previewParams.dspSafeFrame);
	previewParams.dspFrameNums	= bool_key_arg(dspFrameNums, v, previewParams.dspFrameNums);

	previewParams.useMPCamEffect = bool_key_arg(useMPCamEffect, v, previewParams.useMPCamEffect);

	// rndLvl is index in dropdown list in preview dialog, which excludes Z Wireframe. 
	v = key_arg(rndLevel);
	if ((v != &unsupplied) )
	{
		def_preview_renderlevel();
		previewParams.rndLevel = ConvertValueToID(previewRenderLevel, elements(previewRenderLevel), v, -1);
	}

	previewParams.dspBkg		= bool_key_arg(dspBkg, v, previewParams.dspBkg);

	previewParams.dspCamViewName = bool_key_arg(dspCamViewName, v, previewParams.dspCamViewName);
	previewParams.autoPlay = bool_key_arg(autoPlay, v, previewParams.autoPlay);

	// nitrous viewport settings
	//
	// preset
	v = key_arg(vpPreset);
	if (v != &unsupplied)
	{
		def_preview_viewportpreset();
		previewParams.vpPreset = ConvertValueToID(previewViewportPreset, elements(previewViewportPreset), v, -1);
	}
	// visual style
	v = key_arg(vpStyle);
	if (v != &unsupplied)
	{
		def_preview_visualstyle();
		previewParams.vpStyle = ConvertValueToID(previewVisualstyle, elements(previewVisualstyle), v, -1);
	}
	// edged faces & show textures
	previewParams.vpEdgedFaces = bool_key_arg(vpEdgedFaces, v, previewParams.vpEdgedFaces);
	previewParams.vpTextures = bool_key_arg(vpTextures, v, previewParams.vpTextures);

	// MAXX-45594 - Add support for Filename and snippet
	MSTR	mFilename = filename_key_arg(filename, v, nullptr);
	MSTR	mSnippet = string_key_arg(snippet, v, nullptr);

	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	MAXScript_interface->CreatePreview(&previewParams, &mFilename, &mSnippet, _tls->scriptSource);
	return &ok;
}
...
SetSysCur_cf(Value** arg_list, int count)
{
	...
	SetCursor(MAXScript_interface->GetSysCursor(
		ConvertValueToID(cursorTypes, elements(cursorTypes), arg_list[0])));
	return &ok;
}
...
GetCoordCenter_cf(Value** arg_list, int count)
{
	...
	return ConvertIDToValue(originTypes, elements(originTypes), coord);	
}
...
SetCoordCenter_cf(Value** arg_list, int count)
{
	...
	MAXScript_interface->SetCoordCenter(
		ConvertValueToID(originTypes, elements(originTypes), arg_list[0]));	
	return &ok;
}
...
GetRefCoordSys_cf(Value** arg_list, int count)
{
	...
	return ConvertIDToValue(coordTypes, elements(coordTypes), coord);	
}
...
SetRefCoordSys_cf(Value** arg_list, int count)
{
	...
	MAXScript_interface->SetRefCoordSys(
		ConvertValueToID(coordTypes, elements(coordTypes), arg_list[0]));	
	return 	arg_list[0];	
}
...
GetNumAxis_cf(Value** arg_list, int count)
{
	...
	def_axis_types();
	return ConvertIDToValue(axisTypes, elements(axisTypes), MAXScript_interface->GetNumAxis());
}
...
MaterialBrowseDlg_cf(Value** arg_list, int count)
{
	...
	for (int i=0; i < count; i++)
	  flags |= ConvertValueToID(browseTypes, elements(browseTypes), arg_list[i]);
...
FileSaveMatLib_cf(Value** arg_list, int count)
{
	...
	quiet = MAXScript_interface->GetQuietMode();
...
SetStatusXYZ_cf(Value** arg_list, int count)
{
	...
	MAXScript_interface->SetStatusXYZ(
		...
		ConvertValueToID(statusTypes, elements(statusTypes), arg_list[0]));
	return &ok;	
}
...
GetGridSpacing_cf(Value** arg_list, int count)
{
	...
	return_value (Float::intern(MAXScript_interface->GetGridSpacing())); 	
}
...
GetGridMajorLines_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(MAXScript_interface->GetGridMajorLines())); 	
}
...
NodeInvalRect_cf(Value** arg_list, int count)
{
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	InvalidateNodeRect(node,MAXScript_time_tls());
	return &ok;
}
...
SetBkgImageAspect_cf(Value** arg_list, int count)
{
	...
	MAXScript_interface->setBkgImageAspect(
		ConvertValueToID(bkgaspectTypes, elements(bkgaspectTypes), arg_list[0]));
	return &ok;	
}
...
GetBkgImageAspect_cf(Value** arg_list, int count)
{
	...
	return ConvertIDToValue(bkgaspectTypes, elements(bkgaspectTypes), bi);	
}
...
SetBkgORType_cf(Value** arg_list, int count)
{
	...
	MAXScript_interface->setBkgORType(arg_list[0]->to_int(), 
		ConvertValueToID(bkgorTypes, elements(bkgorTypes), arg_list[1]));
	return &ok;	
}
...
GetBkgORType_cf(Value** arg_list, int count)
{
	...
	return ConvertIDToValue(bkgorTypes, elements(bkgorTypes), val);
}
...
GetBkgStartTime_cf(Value** arg_list, int count)
{
	...
	return_value (MSTime::intern(MAXScript_interface->getBkgStartTime()));
}
...
GetBkgSyncFrame_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(MAXScript_interface->getBkgSyncFrame()));	
}
...
GetBkgFrameNum_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(MAXScript_interface->getBkgFrameNum(arg_list[0]->to_timevalue())));
}
...
BoxPickNode_cf(Value** arg_list, int count)
{
	...
	if (applyUIScaling)
	{
		b.left = MAXScript::GetUIScaledValue(b.left);
		b.bottom = MAXScript::GetUIScaledValue(b.bottom);
		b.right = MAXScript::GetUIScaledValue(b.right);
		b.top = MAXScript::GetUIScaledValue(b.top);
	}
...
FencePickNode_cf(Value** arg_list, int count)
{
	...
	type_check(arg_list[0], Array, _T("FencePickNode"));
	Array* pts =  (Array*)arg_list[0];	
	MaxSDK::Array<IPoint2> points(pts->size+1);
	BOOL doCrossing = key_arg_or_default(crossing, &true_value)->to_bool();
	...
	for(i=0; i < pts->size; i++)
	{
		...
		if (applyUIScaling)
		{
			IPoint2 &point = points[i];
			point.x = MAXScript::GetUIScaledValue(point.x);
			point.y = MAXScript::GetUIScaledValue(point.y);
		}
	}
...
FlashNodes_cf(Value** arg_list, int count)
{
	...
	INodeTab flash;
	type_check(arg_list[0], Array, _T("FlashNodes"));	
...
AssignNewName_cf(Value** arg_list, int count)
{
	...
	if (arg_list[0]->is_kind_of(class_tag(MAXMaterial)))
		...
		throw TypeError (MaxSDK::GetResourceStringAsMSTR(IDS_ASSIGNNEWNAME_NEEDS_MAT_TEX), arg_list[0]);
	return &ok;
}
...
GetBackGroundController_cf(Value** arg_list, int count)
{
	...
	return_value (MAXControl::intern(MAXScript_interface->GetBackGroundController()));
}
...
GetCommandPanelTaskMode_cf(Value** arg_list, int count)
{
	...
	def_cpanel_types();
	return ConvertIDToValue(cpanelTypes, elements(cpanelTypes), 
...
GetNamedSelSetItem_cf(Value** arg_list, int count)
{
	...
	if (set_ind < 1 || set_ind > namedSelSetManager->GetNumNamedSelSets())
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_RK_GETNAMEDSELSETITEM_SET_INDEX_OUT_OF_RANGE));		
	if (item_ind < 1 || item_ind > namedSelSetManager->GetNamedSelSetItemCount(set_ind-1)) 
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_RK_GETNAMEDSELSETITEM_ITEM_INDEX_OUT_OF_RANGE));
	return_value (MAXNode::intern(namedSelSetManager->GetNamedSelSetItem(set_ind-1, item_ind-1)));
}
...
GetNamedSelSetItemCount_cf(Value** arg_list, int count)
{
	...
	int set_ind = arg_list[0]->to_int();
	if (set_ind < 1 || set_ind > namedSelSetManager->GetNumNamedSelSets())
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_RK_GETNAMEDSELSETITEMCOUNT_SET_INDEX_OUT_OF_RANGE));		
	return_value (Integer::intern(namedSelSetManager->GetNamedSelSetItemCount(set_ind-1)));
}
...
GetNamedSelSetName_cf(Value** arg_list, int count)
{
	...
	int set_ind = arg_list[0]->to_int();
	if (set_ind < 1 || set_ind > namedSelSetManager->GetNumNamedSelSets())
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_RK_GETNAMEDSELSETNAME_SET_INDEX_OUT_OF_RANGE));			
...
GetNumNamedSelSets_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(namedSelSetManager->GetNumNamedSelSets()));	
}
...
GetRendApertureWidth_cf(Value** arg_list, int count)
{
	...
	return_value (Float::intern(MAXScript_interface->GetRendApertureWidth()));
}
...
SetRendApertureWidth_cf(Value** arg_list, int count)
{
	...
	MAXScript_interface17->SetRendFormatToCustom();
...
GetRendImageAspect_cf(Value** arg_list, int count)
{
	...
	return_value (Float::intern(MAXScript_interface->GetRendImageAspect()));
}
...
RescaleWorldUnits_cf(Value** arg_list, int count)
{
	...
	INodeTab nodeTab;
	INodeTab* p_nodeTab = nullptr;
	if (count == 2)
	{
		if (arg_list[1] == n_selOnly)
			selectedOnly = true;
		else
		{
			Array* nodes = (Array*)arg_list[1];
			type_check(nodes, Array, _T("rescaleWorldUnits argument"));
			for (int i = 0; i < nodes->size; i++)
				nodeTab.AppendNode(nodes->operator[](i)->to_node());
			p_nodeTab = &nodeTab;
		}
	}
	MAXScript_interface->RescaleWorldUnits(arg_list[0]->to_float(), selectedOnly, p_nodeTab);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
...
SetCommandPanelTaskMode_cf(Value** arg_list, int count)
{
	...
	def_cpanel_types();	
	int imode = ConvertValueToID(cpanelTypes, elements(cpanelTypes), mode);
...
ConvertDirIDToInt_cf(Value** arg_list, int count)
{
	...
	def_dir_types();
	int dir = ConvertValueToID(dirTypes, elements(dirTypes), arg_list[0]);		
	return_value (Integer::intern(dir));
}

Value* 
ConvertIntToDirID_cf(Value** arg_list, int count)
{
	check_arg_count(ConvertIntToDirID, 1, count);
	def_dir_types();
	Value *dir = ConvertIDToValue(dirTypes, elements(dirTypes), arg_list[0]->to_int());		
	return dir;
}
...
GetDir_cf(Value** arg_list, int count)
{
	...
	def_dir_types();
	int dir = ConvertValueToID(dirTypes, elements(dirTypes), arg_list[0]);
	if (dir == MXS_MACROSCRIPTS_DIR)
		return new String(TSTR (TheManager->GetDir(APP_MAX_SYS_ROOT_DIR))+_T("macroScripts"));
	else if (dir == MXS_WEB_DIR)
		return new String(TSTR (TheManager->GetDir(APP_MAX_SYS_ROOT_DIR))+_T("web"));
	else if (dir == MXS_MAX_SYSTEM_ICONS_DIR)
		return new String(TSTR (TheManager->GetDir(APP_MAX_SYS_ROOT_DIR))+_T("ui_ln\\icons"));
...
SetDir_cf(Value** arg_list, int count)
{
	...
	MAXScript::ValidateCanRunMAXScriptSystemCommand(_T("SetDir"), arg_list, count);
	def_dir_types();
	int dir = ConvertValueToID(dirTypes, elements(dirTypes), arg_list[0]);
...
/* This drags out the metadata from the file, as well as the gamma, into named
   by-reference parameters */
void GetFileMetaInformation(BitmapInfo &bi, Value **arg_list, int count, bool metadata)
{
	// Create my gamma output parameter
	Value* gammaValue = key_arg(gamma);
	Thunk* gammaThunk = NULL;
	if (gammaValue != &unsupplied) 
		gammaThunk = gammaValue->to_thunk();

	if (gammaThunk)
	{
		if (bi.GetCustomFlags() & BMM_CUSTOM_GAMMA)
			gammaThunk->assign(Float::intern(bi.GetCustomGamma()));
		else if (bi.GetCustomFlags() & BMM_CUSTOM_FILEGAMMA)
			gammaThunk->assign(n_auto);
		else
			gammaThunk->assign(n_default);
	}

	// If this is false, we only wanted the gamma info. The metadata makes
	// no sense when loading files, and is only relevant when saving them.
	if (!metadata) return;

	// Create my PIData output parameter
	Value* pidataValue = key_arg(metaData);
	Thunk* pidataThunk = NULL;
	if (pidataValue != &unsupplied) 
		pidataThunk = pidataValue->to_thunk();

	if (pidataThunk)
	{
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		one_typed_value_local_tls(Array* result);
		int len = bi.GetPiDataSize();
		unsigned char *data = (unsigned char *)bi.GetPiData();

		vl.result = new Array(len);
		for (int i=0; i < len; i++)
		{
			vl.result->append(Integer::intern(data[i]));
		}

		pidataThunk->assign(vl.result);
	}
}
...
SelectSaveBitMap_cf(Value** arg_list, int count)
{
	...
	if (filename != &unsupplied)
		bi.SetName(filename->to_filename().data());
...
{
	// usage LoadDllsFromDir <dir> <plugin_dll_name> [forceLoad:<true|false>]
	// When forceLoad is true, the plug-in is not defer loaded, otherwise it is.
	// By default the plug-in is defer loaded, to conserve legacy behaviour of this function
	check_arg_count_with_keys(loadDllsFromDir, 2, count);
	MAXScript::ValidateCanRunMAXScriptSystemCommand( _T("LoadDllsFromDir"), arg_list, count);

	Value* tmp = NULL;
	bool forceLoadDll = (TRUE == bool_key_arg(forceLoad, tmp, false));
	...
	if (dir.last(_T('\\')) != dir.Length()-1 && dir.last(_T('/')) != dir.Length()-1)
		dir += TSTR(_T("\\"));
	int res = MAXScript_interface->GetDllDir().LoadDllsFromDir(dir, arg_list[1]->to_string(), forceLoadDll);
	if (res != 0) {
...
GetPosTaskWeight_cf(Value** arg_list, int count)
{
	...
	INode *node = arg_list[0]->to_node();
	return_value (Float::intern(node->GetPosTaskWeight()));
}
...
GetRenderID_cf(Value** arg_list, int count)
{
	...
	INode *node = arg_list[0]->to_node();
	return_value (Integer::intern(node->GetRenderID()));	
}
...
GetRotTaskWeight_cf(Value** arg_list, int count)
{
	...
	INode *node = arg_list[0]->to_node();
	return_value (Float::intern(node->GetRotTaskWeight()));	
}
...
GetTMController_cf(Value** arg_list, int count)
{
	...
	INode *node = arg_list[0]->to_node();
	return_value (MAXControl::intern(node->GetTMController()));
}

Value*
GetDriverController_cf(Value** arg_list, int count)
{
	check_arg_count(GetDriverController, 1, count);
	INode *node = arg_list[0]->to_node();
	Control* tmControl = node->GetTMController();
	if (tmControl) {
		return_value (MAXRefTarg::intern(static_cast<ReferenceTarget*>(tmControl->GetInterface(I_DRIVER))));
	}
	return &undefined;
}
...
GetVisController_cf(Value** arg_list, int count)
{
	...
	if (ctrl)
		return_value (MAXControl::intern(ctrl));
	return &undefined;
}
...
set_group_open_cf(Value** arg_list, int count)
{
	...
	if (!node->IsGroupHead())
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_NOT_VALID_GROUP_HEAD), arg_list[0]);
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	INodeTab nodes;
...
GetXController_cf(Value** arg_list, int count)
{
	...
	Control *c = arg_list[0]->to_controller();
	if(!c) return &undefined; 	
	return_value (MAXControl::intern(c->GetXController()));
}
...
GetYController_cf(Value** arg_list, int count)
{
	...
	Control *c = arg_list[0]->to_controller();
	if(!c) return &undefined; 
	return_value (MAXControl::intern(c->GetYController()));
}
...
GetZController_cf(Value** arg_list, int count)
{
	...
	Control *c = arg_list[0]->to_controller();
	if(!c) return &undefined; 
	return_value (MAXControl::intern(c->GetZController()));
}*/
...
static BOOL MatchString(const TCHAR *s, const TCHAR *p, BOOL ignoreCase)
{
	int i = 0;
...
IntersectRayEx_cf(Value** arg_list, int count)
{
	...
	if (((TriObject*)obj)->GetMesh().IntersectRay(os_ray, at, normal, fi, bary))
	{
		...
		return_value_tls (vl.result); // LAM - 5/18/01 - was return vl.result
	}
	else
		return &undefined;
}
...
degToRad_cf(Value** arg_list, int count)
{
	...
	return_value (Float::intern(DegToRad(arg_list[0]->to_float())));
...
radToDeg_cf(Value** arg_list, int count)
{
	...
	return_value (Float::intern(RadToDeg(arg_list[0]->to_float())));
}
...
{
	return IsNetworkRenderServer_cf(arg_list, count);
}

Value*
IsNetworkRenderServer_cf(Value** arg_list, int count)
{
	return MAXScript_interface->IsNetworkRenderServer() ? &true_value : &false_value;
}
...
fx_set_active_cf(Value** arg_list, int count)
{
	...
	if (val->tag != class_tag(MAXAtmospheric) && val->tag != class_tag(MAXEffect))
 		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_WANTED_ATMOS_EFFECT), val);
...
{
	if (!Number::is_integer_number(val))
		type_check(val, MAXNode, _T("Boolean Object access"));
	INode* node = val->to_node();
	deletion_check(((MAXNode*)val));	
	
	Object* obj = node->GetObjectRef();
	if (obj->ClassID() != NEWBOOL_CLASS_ID)
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_RK_INVALID_BOOLEAN_OBJECT), val);
...
{		
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	ObjectState os = nodeA->GetObjectRef()->Eval(MAXScript_time_tls());
	if (os.obj->IsParticleSystem() || !(os.obj->IsDeformable() || os.obj->CanConvertToType(defObjectClassID)))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_RK_CREATEBOOLEANOBJECT_SOURCE_OBJECT), arg_list[0]);
	
	// make the new boolean obj and add source obj as operand A
	IBoolObject* obj = (IBoolObject*)MAXScript_interface->CreateInstance(
		GEOMOBJECT_CLASS_ID, NEWBOOL_CLASS_ID);
	if (!obj) throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_RK_BOOLEAN_OBJECT_NOT_CREATABLE));
	obj->SetOperandA (MAXScript_time_tls(), nodeA);
	...
	{			
		...
		obj->SetOperandB (MAXScript_time_tls(), nodeB, nodeA,
...
setOperandB_cf(Value** arg_list, int count)
{
	...
	deletion_check(((MAXNode*)arg_list[1]));
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	obj->SetOperandB (MAXScript_time_tls(), nodeB, arg_list[0]->to_node(), 
		arg_list[2]->to_int()-1, arg_list[3]->to_int()-1);
	needs_redraw_set(_tls);
	return &ok;
}
...
GetBoolOp_cf(Value** arg_list, int count)
{
	...
	IBoolObject* obj = setup_bool_access(arg_list[0]);
	return_value (Integer::intern(obj->GetBoolOp()+1));
}
...
GetBoolCutType_cf(Value** arg_list, int count)
{
	...
	IBoolObject* obj = setup_bool_access(arg_list[0]);
	return_value (Integer::intern(obj->GetBoolCutType()+1));	
}
...
GetUpdateMode_cf(Value** arg_list, int count)
{
	...
	IBoolObject* obj = setup_bool_access(arg_list[0]);
	return_value (Integer::intern(obj->GetUpdateMode()+1));
}
...
setup_indexed_particle(TCHAR* op, int& index, Value** arg_list, int count)
{
	...
	Value* val = arg_list[0];
	type_check(val, MAXNode, _T("Particle Object access"));
	INode* node = val->to_node();
	deletion_check(((MAXNode*)val));	
	...
	if (os.obj->SuperClassID() != GEOMOBJECT_CLASS_ID ||
		(sp = (SimpleParticle*)os.obj->GetInterface(I_SIMPLEPARTICLEOBJ)) == NULL)
			throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_SIMPLEPARTICLE_OPERATION_ON_NONSIMPLEPARTICLESYSTEM), os.obj->GetObjectName(true));

	index = arg_list[1]->to_int() - 1;	// get & check particle index, cvt from 1-based to 0-based
	if (index < 0 || index >= sp->parts.Count())
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_PARTICLE_INDEX_OUT_OF_RANGE), arg_list[1]);

	TimeValue t;
	if ((t = MAXScript_time_tls()) != sp->tvalid)
...
SetParticlePos_cf(Value** arg_list, int count)
{
	...
	obj->SetParticlePosition(MAXScript_time_tls(), index, arg_list[2]->to_point3());
...
SetParticleVelocity_cf(Value** arg_list, int count)
{
	...
	obj->SetParticleVelocity(MAXScript_time_tls(), index, arg_list[2]->to_point3());
	obj->NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
	return arg_list[2];
}
...
SetParticleAge_cf(Value** arg_list, int count)
{
	...
	obj->SetParticleAge(MAXScript_time_tls(), index, arg_list[2]->to_timevalue());
	obj->NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
	return arg_list[2];
}*/
...
set_trackbar_filter(Value* val)
{
	...
	MAXScript_interface->GetTrackBar()->SetFilter(
		ConvertValueToID(tbarFiltTypes, elements(tbarFiltTypes), val));
	return val;
}
...
{
	def_tbar_filter_types();
	return ConvertIDToValue(tbarFiltTypes, elements(tbarFiltTypes), 
		MAXScript_interface->GetTrackBar()->GetFilter());
}

Value*
set_trackbar_showaudio(Value* val)
{
	MAXScript_interface->GetTrackBar()->SetShowAudio(val->to_bool()? true: false);
	return val;
}

Value*
get_trackbar_showaudio()
{
	return MAXScript_interface->GetTrackBar()->GetShowAudio() ? &true_value : &false_value;	
}
...
get_trackbar_next_key_cf(Value** arg_list, int count)
{
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	TimeValue t = MAXScript_interface->GetTrackBar()->GetNextKey(MAXScript_time_tls(),TRUE);	
	return (t==MAXScript_time_tls()) ? &undefined : MSTime::intern(t);
}
...
get_trackbar_prev_key_cf(Value** arg_list, int count)
{
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	TimeValue t = MAXScript_interface->GetTrackBar()->GetNextKey(MAXScript_time_tls(), FALSE);
	return (t==MAXScript_time_tls()) ? &undefined : MSTime::intern(t);
}
...
static constexpr prop_type props[] = {
	...
	{ PID_CATEGORY,			PROPSET_DOCSUMMARYINFO, _T("Category") },		// 0x00000002L / DocumentSummaryInformation
	...
	{ PID_HEADINGPAIR,		PROPSET_DOCSUMMARYINFO, _T("Headers") },		// 0x0000000cL / DocumentSummaryInformation
	// If add new items, leave above ordering alone, or will need to adjust code in getMAXFileFileProperties_cf
};

// from App/PropertyEditor.cpp
static BOOL ParseDateFromString(const TCHAR* pValue, FILETIME* ft)
	{
	TCHAR	szDateSeparator[2];
	...
	const TCHAR*	p = pValue;
	const TCHAR*	pCs = pValue;
	TCHAR	szComponent[80];
	TCHAR	szNextChar[2];
	int		nDate[3];
	int		component = 0;

	_tcscpy(szComponent, _T(""));
	szNextChar[1] = _T('\0');
	do {
		if (*p == szDateSeparator[0]) {
			nDate[component] = _ttoi(szComponent);
			_tcscpy(szComponent, _T(""));
			component++;
			p++;
			pCs = p;
			...
			szNextChar[0] = *p;
			_tcscat(szComponent, szNextChar);
			p++;
			}
		} while (*p && (component != 3));
	...
	if (_tcslen(szComponent) > 0) {
		nDate[component] = _ttoi(szComponent);
		}

	int dateFormat = _ttoi(szDateFormat);
	// 0 - Month-Day-Year 
	// 1 - Day-Month-Year 
	// 2 - Year-Month-Day 
...
// find index of PID_DOCPARTS in PROPSET_DOCSUMMARYINFO, return -1 if not found
int FindDocPartsIndex()
{
	PROPSPEC spec{PRSPEC_PROPID,PID_HEADINGPAIR };
	if (int idx = MAXScript_interface->FindProperty(PROPSET_DOCSUMMARYINFO, &spec) != -1)
	{
		if (MAXScript_interface->GetPropertyVariant(PROPSET_DOCSUMMARYINFO, idx))
		{
			spec.propid = PID_DOCPARTS;
			return MAXScript_interface->FindProperty(PROPSET_DOCSUMMARYINFO, &spec);
		}
	}
	return -1;
}
...
fp_num_properties_cf(Value** arg_list, int count)
{
	...
	int set = ConvertValueToID(propertysetTypes, elements(propertysetTypes), arg_list[0]);
	int num = MAXScript_interface->GetNumProperties(set);
	if (num != 0 && set == PROPSET_DOCSUMMARYINFO && (FindDocPartsIndex() != -1))
	{
		num--; // Don't count document parts
	}

	return_value (Integer::intern(num));
}
...
fp_get_property_name_cf(Value** arg_list, int count)
{
	...
	int num = MAXScript_interface->GetNumProperties(set);
	int docPartsIndex = -1;
	if (num != 0 && set == PROPSET_DOCSUMMARYINFO && ((docPartsIndex = FindDocPartsIndex()) != -1))
	{
		num--; // Don't count document parts
	}

	if (index < 0 || index > num-1)
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_RK_FILEPROPS_GETPROPERTY_INDEX_OUT_OF_RANGE));
	
	// skip over document parts
	if (docPartsIndex != -1 && index >= docPartsIndex)
		index += 1;

	const PROPSPEC* ps = MAXScript_interface->GetPropertySpec(set, index);
	
	if (ps->ulKind==PRSPEC_LPWSTR)
		return new String(TSTR::FromOLESTR(ps->lpwstr));	
	else
	{
...
Value* PropVariantToValue(const PROPVARIANT* pProp, UINT codePage = CP_ACP)
{
	switch (pProp->vt)
	{
	case VT_LPWSTR:
	case VT_LPSTR:
	case VT_BSTR:
		// a BSTR is a WORD followed by a UNICODE string. The pointer points just after the WORD
		return_value(new String(VariantToString(pProp, codePage).ToMCHAR().data())); 
	case VT_I1:
		return_value(Integer::intern(pProp->cVal));
	case VT_UI1:
		return_value(Integer::intern(pProp->bVal));
	case VT_I2:
		return_value(Integer::intern(pProp->iVal));
	case VT_UI2:
		return_value(Integer::intern(pProp->uiVal));
	case VT_I4:
		return_value(Integer::intern(pProp->lVal));
	case VT_UI4:
		return_value(Integer::intern(pProp->ulVal));
	case VT_I8:
		return_value(Integer64::intern(pProp->hVal.QuadPart));
	case VT_UI8:
		return_value(Integer64::intern(pProp->uhVal.QuadPart));
	case VT_INT:
		return_value(Integer::intern(pProp->intVal));
	case VT_UINT:
		return_value(Integer::intern(pProp->uintVal));
	case VT_INT_PTR:
#if !defined(_WIN64)
		return_value(IntegerPtr::intern(pProp->intVal));
#else
		return_value(IntegerPtr::intern(pProp->hVal.QuadPart));
#endif
	case VT_R4:
		return_value(Float::intern(pProp->fltVal));
	case VT_R8:
		return_value(Double::intern(pProp->dblVal));
	case VT_BOOL:
		return pProp->boolVal ? &true_value : &false_value;
	case VT_FILETIME:
		SYSTEMTIME sysTime;
		TCHAR time_str[64];
		FileTimeToSystemTime(&pProp->filetime, &sysTime);
		GetDateFormat(LOCALE_USER_DEFAULT, DATE_SHORTDATE, &sysTime, NULL, time_str, _countof(time_str));
		return_value(new String(time_str));
	case VT_CLSID:
		CLSID guid = *pProp->puuid;
		LPOLESTR psz = NULL;
		StringFromCLSID(guid, &psz);
		TSTR cid = TSTR::FromOLESTR(psz);
		CoTaskMemFree(psz);
		return_value(new String(cid));
	}
	return &undefined;
}

Value* fp_get_property_val_cf(Value** arg_list, int count)
{
	...
	if (index < 0 || index > num-1)
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_RK_FILEPROPS_GETPROPERTYVALUE_INDEX_OUT_OF_RANGE));
	...
	if (set == PROPSET_DOCSUMMARYINFO && pSpec->propid == PID_HEADINGPAIR) // Contents page
	{
		...
		for (UINT i=0; i<pHeading->cElems; i+=2) 
			vl.result->append(new String(VariantToString(pHeading->pElems[i]).ToMCHAR().data())); 
		return_value_tls(vl.result);		
	}
	
	return PropVariantToValue(pProp);
}
...
fp_get_items_cf(Value** arg_list, int count)
{
	...
	const TCHAR* header_name = arg_list[0]->to_string();
			...
			if ((idx = ip->FindProperty(PROPSET_DOCSUMMARYINFO, &spec)) != -1) {
				if ((pPropDocPart = ip->GetPropertyVariant(PROPSET_DOCSUMMARYINFO, idx)) != NULL) {
					...
					for (UINT i=0; i<pHeading->cElems; i+=2) {
						...
						if (_tcsicmp(VariantToString(pHeading->pElems[i]).ToMCHAR(), header_name))
						{
							nDocPart += ndocs;
							continue;
						}
						...
						std::vector<TSTR> pDocPartStrings;
						VariantToStringVector(pPropDocPart, pDocPartStrings);
							
						MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
						one_typed_value_local_tls(Array* result);
						vl.result = new Array(ndocs);
						for (int j=0; j<ndocs; j++) {
							vl.result->append(new String(pDocPartStrings[nDocPart].data())); 
							nDocPart++;
						}
						return_value_tls(vl.result);
					}
...
fp_find_property_cf(Value** arg_list, int count)
{
	...
	int set = ConvertValueToID(propertysetTypes, elements(propertysetTypes), arg_list[0]);
	const TCHAR *pname = arg_list[1]->to_string();
	PROPSPEC spec;	
	WStr wsz = WStr::FromMCHAR(pname);
	...
	if(set==PROPSET_SUMMARYINFO || set==PROPSET_DOCSUMMARYINFO) 
	{
		...
		if (!found) return_value (Integer::intern(0)); // return 0 if not found
	}
	else
	{
		spec.ulKind = PRSPEC_LPWSTR;
		spec.lpwstr = const_cast<wchar_t*>(wsz.data());
	}	
	int ind = MAXScript_interface->FindProperty(set, &spec);	
	return_value (Integer::intern(ind+1));
}
...
fp_add_property_cf(Value** arg_list, int count)
{
	...
	WStr wszName = WStr::FromMCHAR(pname);
	TSTR szVal;
	CLSID guid;
	...
	if(set==PROPSET_SUMMARYINFO || set==PROPSET_DOCSUMMARYINFO) // if adding a summary page property
	{
		...
		if (!found || spec.propid==PID_HEADINGPAIR)
			throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_RK_INVALID_FILEPROP_SUMMARY_PROPERTY), arg_list[1]); 
	}
	else
	{
		spec.ulKind = PRSPEC_LPWSTR;
		spec.lpwstr = const_cast<wchar_t*>(wszName.data());
	}
	...
	{		
		if(type_date)
		{
			...
			else throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_RK_INVALID_FILEPROP_DATE_FORMAT), val);
		}
		else if(type_guid)
		{
			WStr guid_str = WStr::FromMCHAR(val->to_string());
			if (CLSIDFromString(guid_str, &guid) == S_OK)
			{
				var.vt = VT_CLSID;
				var.puuid = &guid;
			}
			else throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_RK_INVALID_FILEPROP_CLSID_FORMAT), val);
		}
		else 
		{
			szVal=val->to_string();
			MaxSDK::Util::SetStringToVariant(&var, szVal.data());
		}
	...
	else if(is_bool(val))
	{
		...
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_RK_INVALID_FILEPROP_VALUE_TYPE), val);
...
//	int c = arg_list[0]->to_int()-1;
	...
	return_value (Integer::intern(c+1));	
}

Value* 
GetNumberSelectFilters_cf(Value** arg_list, int count)
{
	check_arg_count(GetNumberSelectFilters, 0, count);
//	def_coord_types();	
//	int c = arg_list[0]->to_int()-1;
	int c = MAXScript_interface->GetNumberSelectFilters();	
	return_value (Integer::intern(c));	
}
...
{
	check_arg_count(SetDisplayFilter, 2, count);
	//	def_coord_types();	
	int c = arg_list[0]->to_int()-1;
...
{
	check_arg_count(GetDisplayFilter, 1, count);
	//	def_coord_types();	
	int c = arg_list[0]->to_int()-1;
...
{
	check_arg_count(GetNumberDisplayFilters, 0, count);
	//	def_coord_types();	
//	int c = arg_list[0]->to_int()-1;
...
//	def_coord_types();	
	...
	if ( (c <0 ) || (c >= MAXScript_interface->GetNumberDisplayFilters()) )
		...
		{
		const TCHAR *name = MAXScript_interface->GetDisplayFilterName(c);	
...
{
	check_arg_count(GetSelectFilterName, 1, count);
//	def_coord_types();	
	int c = arg_list[0]->to_int()-1;
	if ( (c <0 ) || (c >= MAXScript_interface->GetNumberSelectFilters()))
		{
		...
		{
		const TCHAR *name = MAXScript_interface->GetSelectFilterName(c);	
		return new String(name);	
		}
}
...
SetRenderType_cf(Value **arg_list, int count)
{
	...
	RenderUIType rtype = (RenderUIType)ConvertValueToID(renderTypes, elements(renderTypes), arg_list[0]);
...
GetRenderType_cf(Value **arg_list, int count)
{
	...
	RenderUIType rtype = MAXScript_interface7->GetRenderType();
	def_max_render_types();
	return ConvertIDToValue(renderTypes, elements(renderTypes), rtype, &undefined);
}


Value* getMAXFileFileProperties_cf(Value** arg_list, int count)
{
	// getMAXFileFileProperties <filename>
	check_arg_count(getMAXFileFileProperties, 1, count);
	WStr fname = arg_list[0]->to_filename();

	LPSTORAGE pStorage = nullptr;
	HRESULT res = StgOpenStorage(fname, nullptr, STGM_DIRECT | STGM_READ | STGM_SHARE_DENY_WRITE, NULL, 0, &pStorage);
	if (res != S_OK)
	{
		return &undefined;
	}
	...
	IPropertySetStorage* pPropertySetStorage = nullptr;
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	six_typed_value_locals_tls(Array * res, MXSDictionaryValue * dictionary, String * propName, Value * propValue,
			MXSDictionaryValue * headers_dictionary, Array * header_section_array);
	vl.res = new Array(3);
	vl.res->size = 3;

	// Get the SummaryInfo property set interface
	vl.res->data[0] = vl.dictionary = new MXSDictionaryValue(MXSDictionaryValue::key_string);
	IPropertyStorage* pSummaryInfoStorage = nullptr;
	if (S_OK == pPropertySetStorage->Open(FMTID_SummaryInformation, STGM_READ | STGM_SHARE_EXCLUSIVE, &pSummaryInfoStorage))
	{
		// propCount is 17, reflecting the first 16 entries in props[] (all in set PROPSET_SUMMARYINFO) plus 1.
		// The last entry is a special PropSpec with propid of 1. This PropSpec exists in each set, and holds the codepage
		// that strings in the set were stored using.
		const int propCount = 17;
		PROPSPEC PropSpec[propCount];
		PROPVARIANT PropVar[propCount];
		for (int i = 0; i < propCount - 1; ++i)
		{
			PropSpec[i].ulKind = PRSPEC_PROPID;
			PropSpec[i].propid = props[i].id;
		}

		const int codePagePropSpecIndex = propCount - 1;
		PropSpec[codePagePropSpecIndex].ulKind = PRSPEC_PROPID;
		PropSpec[codePagePropSpecIndex].propid = 1; // Read the property storage's codepage.

		HRESULT hr = pSummaryInfoStorage->ReadMultiple(propCount, PropSpec, PropVar);
		if (S_OK == hr)
		{
			UINT codePage = CP_ACP;
			if (PropVar[codePagePropSpecIndex].vt == VT_I2)
			{
				if (PropVar[codePagePropSpecIndex].uiVal == 1200)
				{
					// 1200 is a special codepage. Means that, on-disk, data is UTF-16,
					// but was translated to ACP when loaded.
				}
				else if (PropVar[codePagePropSpecIndex].uiVal != GetACP())
				{
					codePage = PropVar[codePagePropSpecIndex].uiVal;
				}
			}
			for (int i = 0; i < propCount - 1; ++i)
			{
				if (PropVar[i].vt != 0)
				{
					vl.propValue = PropVariantToValue(&PropVar[i], codePage);
					vl.propName = new String(props[i].name);
					vl.dictionary->put(vl.propName, vl.propValue);
				}
			}
		}
		FreePropVariantArray(propCount, PropVar);
		pSummaryInfoStorage->Release();
		pSummaryInfoStorage = nullptr;
	}

	// Get the DocumentSummaryInfo property set interface
	vl.res->data[1] = vl.dictionary = new MXSDictionaryValue(MXSDictionaryValue::key_string);
	IPropertyStorage* pDocumentSummaryInfoStorage = nullptr;
	if (S_OK == pPropertySetStorage->Open(FMTID_DocSummaryInformation, STGM_READ | STGM_SHARE_EXCLUSIVE, &pDocumentSummaryInfoStorage))
	{
		// propCount is 6, reflecting the last 4 entries in props[] (all in set PROPSET_DOCSUMMARYINFO) plus 2.
		// The second to last entry has propid of PID_DOCPARTS and contains the data associated with the items in PID_HEADINGPAIR.
		// The data associated with PID_HEADINGPAIR is a vector of variants consisting of pairs of a string and a number. The string 
		// is a heading name, and the number tells how many document parts are under that heading. The document part data is sequentially 
		// stored in PID_DOCPARTS.
		// The last entry is a special PropSpec with propid of 1. This PropSpec exists in each set, and holds the codepage
		// that strings in the set were stored using.
		const int propCount = 6;
		PROPSPEC PropSpec[propCount];
		PROPVARIANT PropVar[propCount];
		const int propTableOffset = 16; // the offset in props to where the PROPSET_DOCSUMMARYINFO entries begin
		for (int i = 0; i < propCount - 2; ++i)
		{
			PropSpec[i].ulKind = PRSPEC_PROPID;
			PropSpec[i].propid = props[i + propTableOffset].id;
		}

		const int headingPairPropSpecIndex = propCount - 3; // PID_HEADINGPAIR

		const int docPartsPropSpecIndex = propCount - 2;
		PropSpec[docPartsPropSpecIndex].ulKind = PRSPEC_PROPID;
		PropSpec[docPartsPropSpecIndex].propid = PID_DOCPARTS;

		const int codePagePropSpecIndex = propCount - 1;
		PropSpec[codePagePropSpecIndex].ulKind = PRSPEC_PROPID;
		PropSpec[codePagePropSpecIndex].propid = 1; // Read the property storage's codepage.

		HRESULT hr = pDocumentSummaryInfoStorage->ReadMultiple(propCount, PropSpec, PropVar);
		if (S_OK == hr)
		{
			UINT codePage = CP_ACP;
			if (PropVar[codePagePropSpecIndex].vt == VT_I2)
			...
			for (int i = 0; i < propCount - 2; ++i)
			{
				...
				{
					vl.propValue = PropVariantToValue(&PropVar[i], codePage);
					vl.propName = new String(props[i + propTableOffset].name);
					vl.dictionary->put(vl.propName, vl.propValue);
				}
			}

			// Scan through document contents
			// PropVar[headingPairPropSpecIndex] corresponds to PID_HEADINGPAIR
			// PropVar[docPartsPropSpecIndex] corresponds to PID_DOCPARTS
			if ((PropVar[headingPairPropSpecIndex].vt == (VT_VARIANT | VT_VECTOR)) &&
					VariantIsStringVector(PropVar[docPartsPropSpecIndex]))
			{
				vl.headers_dictionary = new MXSDictionaryValue(MXSDictionaryValue::key_string);
				vl.propName = new String(props[propTableOffset + 3].name); // "Headers"
				vl.dictionary->put(vl.propName, vl.headers_dictionary);

				std::vector<TSTR> pDocPart;
				VariantToStringVector(&PropVar[docPartsPropSpecIndex], pDocPart, codePage);

				int nDocPart = 0;
				CAPROPVARIANT* pHeading = &PropVar[headingPairPropSpecIndex].capropvar;
				for (UINT i = 0; i < pHeading->cElems; i += 2)
				{
					int count = pHeading->pElems[i + 1].lVal;
					vl.header_section_array = new Array(count);
					vl.propName = (String*)PropVariantToValue(&pHeading->pElems[i], codePage);
					vl.headers_dictionary->put(vl.propName, vl.header_section_array);
					for (int j = 0; j < count; j++)
					{
						vl.header_section_array->append(new String(pDocPart[nDocPart]));
						nDocPart++;
					}
				}
			}
		...
		pDocumentSummaryInfoStorage = nullptr;
	}

	// Get the User Defined property set interface
	vl.res->data[2] = vl.dictionary = new MXSDictionaryValue(MXSDictionaryValue::key_string);
	IPropertyStorage* pUserDefinedPropertyStorage = nullptr;
	if (S_OK == pPropertySetStorage->Open(FMTID_UserDefinedProperties, STGM_READ | STGM_SHARE_EXCLUSIVE, &pUserDefinedPropertyStorage))
	{
		...
		if (S_OK == pUserDefinedPropertyStorage->Enum(&pIPropertyEnum))
		{
			...
			if (numUserProps != 0)
			{
				...
				if (S_OK == hr)
				{
					...
					{
						vl.propValue = PropVariantToValue(&pPropVar[i], codePage);
						vl.propName = new String(pPropSpec[i].lpwstr);
						vl.dictionary->put(vl.propName, vl.propValue);
					}
				}
		...
		pUserDefinedPropertyStorage = nullptr;
	}
...
Value* clearMAXFilePersonalInfo_cf(Value** arg_list, int count)
{
	// clearMAXFilePersonalInfo <filename>
	check_arg_count(clearMAXFilePersonalInfo, 1, count);
	WStr fname = arg_list[0]->to_filename();
	...
	MaxSDK::Util::Path theFile(fname);
	theFile.SaveBaseFile();
	HRESULT hres = StgOpenStorageEx(fname, mode, STGFMT_DOCFILE, 0, &StgOptions, NULL, IID_IStorage, (void**)&pIStorage);
	...
	// Get the SummaryInfo property set interface
	IPropertyStorage* pSummaryInfoStorage = nullptr;
	if (S_OK == pPropertySetStorage->Open(FMTID_SummaryInformation, mode, &pSummaryInfoStorage))
	{
		// clear properties holding user name
		const int propCount = 2;
		PROPSPEC PropSpec[propCount];
		PropSpec[0].ulKind = PRSPEC_PROPID;
		PropSpec[0].propid = PIDSI_CREATOR;
		PropSpec[1].ulKind = PRSPEC_PROPID;
		PropSpec[1].propid = PIDSI_LASTAUTHOR;

		HRESULT hr = pSummaryInfoStorage->DeleteMultiple(propCount, PropSpec);
		DbgAssert(hr == S_OK);
		pSummaryInfoStorage->Release();
		pSummaryInfoStorage = nullptr;
	}
	...
	// Get the DocumentSummaryInfo property set interface
	IPropertyStorage* pDocumentSummaryInfoStorage = nullptr;
	if (S_OK == pPropertySetStorage->Open(FMTID_DocSummaryInformation, mode, &pDocumentSummaryInfoStorage))
	{
		const int propCount = 3;
		PROPSPEC PropSpec[propCount];
		PROPVARIANT PropVar[propCount];

		const int headingPairPropSpecIndex = 0;
		PropSpec[headingPairPropSpecIndex].ulKind = PRSPEC_PROPID;
		PropSpec[headingPairPropSpecIndex].propid = PID_HEADINGPAIR;

		const int docPartsPropSpecIndex = 1;
		PropSpec[docPartsPropSpecIndex].ulKind = PRSPEC_PROPID;
		PropSpec[docPartsPropSpecIndex].propid = PID_DOCPARTS;

		const int codePagePropSpecIndex = 2;
		PropSpec[codePagePropSpecIndex].ulKind = PRSPEC_PROPID;
		PropSpec[codePagePropSpecIndex].propid = 1; // Read the property storage's codepage.
		...
		if (S_OK == hr)
		{
			...
			// Scan through document contents looking for "Render Data" heading, and 
			// then look for the User Name and Computer Name items in that. If found,
			// rewrite the heading and doc parts without them.
			// 
			// PropVar[headingPairPropSpecIndex] corresponds to PID_HEADINGPAIR
			// PropVar[docPartsPropSpecIndex] corresponds to PID_DOCPARTS
			if ((PropVar[headingPairPropSpecIndex].vt == (VT_VARIANT | VT_VECTOR)) &&
					VariantIsStringVector(PropVar[docPartsPropSpecIndex]))
			{
				TSTR renderDataText(_T("Render Data"));
				TSTR userNameText(_T("User Name="));
				TSTR computerNameText(_T("Computer Name="));
				...
				for (UINT i = 0; i < pHeading->cElems; i += 2)
				{
					TSTR headingName = VariantToString(&pHeading->pElems[i], codePage);
					int count = pHeading->pElems[i + 1].lVal;
					bool isRenderData = headingName == renderDataText;
					if (isRenderData)
					{
						for (int j = 0; j < count; j++)
						{
							TSTR val = pDocPart[nDocPart];
							bool isUserName = val.StartsWith(userNameText);
							bool isComputerNamae = val.StartsWith(computerNameText);
							if (isUserName || isComputerNamae)
							{
								elementsToDelete.push_back(nDocPart);
								--pHeading->pElems[i + 1].lVal;
							}
							nDocPart++;
						}
					}
				...
				// if found elements to delete, do so and rewrite 
				if (!elementsToDelete.empty())
				{
					int offset = 0;
					for (auto ele : elementsToDelete)
					{
						pDocPart.erase(pDocPart.begin() + (ele - offset));
						++offset;
					}
					std::vector<const wchar_t*> newDocPartData;
					newDocPartData.reserve(pDocPart.size());
					for (auto& ele : pDocPart)
					{
						newDocPartData.emplace_back(ele.data());
					}
					PROPVARIANT newPropVar[2];
					HRESULT hr1 = PropVariantCopy(&newPropVar[0], &PropVar[headingPairPropSpecIndex]);
					HRESULT hr2 = InitPropVariantFromStringVector(newDocPartData.data(), (ULONG)newDocPartData.size(), &newPropVar[1]);
					HRESULT hr3 = pDocumentSummaryInfoStorage->WriteMultiple(2, PropSpec, newPropVar, 2);
					HRESULT hr4 = FreePropVariantArray(2, newPropVar);
					DbgAssert(hr1 == S_OK && hr2 == S_OK && hr3 == S_OK && hr4 == S_OK);
				}
			}
		}
		FreePropVariantArray(propCount, PropVar);
