...
#define def_dir_types()											\
	mxs_value_id_pair dirTypes[] = {									\
		...
		{ APP_PLUGINDATA_DIR,			n_plugindata },		\
		{ APP_PRESETS_DIR,			n_presets },		\
};
...
// 
// see class PreviewParams - member vpPreset
#define def_preview_viewportpreset()				\
	mxs_value_id_pair previewViewportPreset[] = {	\
		{ 0,	n_highquality },					\
		{ 1,	n_standard },						\
		{ 2,	n_performance },					\
		{ 3,	n_dxmode },							\
...
// see class PreviewParams - member vpStyle
#define def_preview_visualstyle()					\
	mxs_value_id_pair previewVisualstyle[] = {		\
		{ 0,	n_defaultshading },					\
		{ 2,	n_facets },							\
		{ 3,	n_flatcolor },						\
		{ 4,	n_hiddenLine },						\
...
//
// End of Nitrous Viewport Settings
...
#define def_Locale_categories_types()		\
	mxs_value_id_pair LocaleCategoriesTypes[] = {	\
		{ LC_ALL,		n_LC_ALL },			\
		{ LC_COLLATE,	n_LC_COLLATE },		\
		{ LC_CTYPE,		n_LC_CTYPE },		\
		{ LC_MONETARY,	n_LC_MONETARY },	\
		{ LC_NUMERIC,	n_LC_NUMERIC },		\
		{ LC_TIME,		n_LC_TIME },		\
	};

#define def_showwindow_commands() \
	mxs_value_id_pair showwindow_commands[] = { \
		{ SW_HIDE,				n_hide, }, \
		{ SW_NORMAL,			n_normal, }, \
		{ SW_SHOWNORMAL,		n_showNormal, }, \
		{ SW_SHOWMINIMIZED,		n_showMinimized, }, \
		{ SW_MAXIMIZE,			n_maximize, }, \
		{ SW_SHOWMAXIMIZED,		n_showMaximized, }, \
		{ SW_SHOWNOACTIVATE,	n_showNoActivate, }, \
		{ SW_SHOW,				n_show, }, \
		{ SW_MINIMIZE,			n_minimize, }, \
		{ SW_SHOWMINNOACTIVE,	n_showMinNoActive, }, \
		{ SW_SHOWNA,			n_showNa, }, \
		{ SW_RESTORE,			n_restore, }, \
		{ SW_SHOWDEFAULT,		n_showDefault, }, \
		{ SW_FORCEMINIMIZE,		n_forceMinimize, }, \
	};

#define def_windowlong_index()					\
	mxs_value_id_pair windowlong_index[] = {	\
		{ GWL_EXSTYLE,		n_exStyle },		\
		{ GWLP_HINSTANCE,	n_instance },		\
		{ GWLP_ID,			n_id },				\
		{ GWL_STYLE,		n_style },			\
		{ GWLP_USERDATA,	n_userData },		\
		{ GWLP_WNDPROC,		n_wndproc },		\
		{ DWLP_DLGPROC,		n_dlgproc },		\
		{ DWLP_MSGRESULT,	n_msgResult },		\
		{ DWLP_USER,		n_user },			\
	};
