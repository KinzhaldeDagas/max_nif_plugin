	...
	subdivideSegment_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(seg_index, 1, spline->Segments(), MaxSDK::GetResourceStringAsMSTR(IDS_SPLINE_SEGMENT_INDEX_OUT_OF_RANGE));
	...
	setPropertyController_cf(Value** arg_list, int count)
{
	...
	if (c != (MAXControl*)&undefined && !c->is_kind_of(class_tag(MAXControl)))
	...
	{
		if (c != (MAXControl*)&undefined && c->flags & MAX_CTRL_NO_DIM) 
		{
			ParamDimension *dim;
	...
	setPlaybackSpeed(Value* val)
{
	...
	MXS_range_check(speed, 1, 5, MaxSDK::GetResourceStringAsMSTR(IDS_PLAYBACKSPEED_OUT_OF_RANGE));
	...
	setMTLMeditObjType_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(objType, 1, 4, MaxSDK::GetResourceStringAsMSTR(IDS_MEDIT_OBJECT_TYPE_INDEX_OUT_OF_RANGE));
	...
	setMTLMeditTiling_cf(Value** arg_list, int count)
{
	...
	int objType = arg_list[1]->to_int();
	MXS_range_check(objType, 1, 4, MaxSDK::GetResourceStringAsMSTR(IDS_MEDIT_TILE_TYPE_INDEX_OUT_OF_RANGE));
	...
	qsort_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(startIndex, 1, theArray->size, MaxSDK::GetResourceStringAsMSTR(IDS_QSORT_STARTINDEX_BAD));
		int endIndex=int_key_arg(end, tmp, theArray->size);
	MXS_range_check(endIndex, 1, theArray->size, MaxSDK::GetResourceStringAsMSTR(IDS_QSORT_ENDINDEX_BAD));
	...
	bsearch_cf(Value** arg_list, int count)
{
	...
	Value* tmp;
	int startIndex=int_key_arg(start, tmp, 1);
	MXS_range_check(startIndex, 1, theArray->size, MaxSDK::GetResourceStringAsMSTR(IDS_BSEARCH_STARTINDEX_BAD));
		int endIndex=int_key_arg(end, tmp, theArray->size);
	MXS_range_check(endIndex, 1, theArray->size, MaxSDK::GetResourceStringAsMSTR(IDS_BSEARCH_ENDINDEX_BAD));
		if (endIndex < startIndex)
	...
	insertItem_cf(Value** arg_list, int count)
{
	...
	if (!Number::is_number(arg) || (index = arg->to_int()) < 1)
...
Value* amin_cf(Value** arg_list, int count)
{
	Value* smallest;
	int i;
	...
	if (arg_list[0]->is_kind_of(class_tag(Array))) 
	{
		...
		for (i = 1; i < theArray->size; i++)
		{
			...
			else if (theArray->data[i]->gt_vf(&smallest, 1) == &false_value)
				smallest = theArray->data[i];
		}
	}
	else 
	{
		...
		for (i = 1; i < count; i++)
		{
			if (smallest->lt_vf(&arg_list[i], 1) == &false_value)
				smallest = arg_list[i];
			else if (arg_list[i]->gt_vf(&smallest, 1) == &false_value)
				smallest = arg_list[i];
		}
	}
	return smallest;
...
Value* amax_cf(Value** arg_list, int count)
{
	Value* largest;
	int i;
	if (count == 0) return &undefined;
	if (arg_list[0]->is_kind_of(class_tag(Array)))
	{
		...
		for (i = 1; i < theArray->size; i++)
		{
			...
			else if (theArray->data[i]->lt_vf(&largest, 1) == &false_value)
				largest = theArray->data[i];
		}
	}
	else 
	{
		...
		for (i = 1; i < count; i++)
		{
			if (largest->gt_vf(&arg_list[i], 1) == &false_value)
				largest = arg_list[i];
			else if (arg_list[i]->lt_vf(&largest, 1) == &false_value)
				largest = arg_list[i];
		}
	}
	return largest;
...
{
	int i = val->to_int();
	MXS_range_check(i, 1, 4, MaxSDK::GetResourceStringAsMSTR(IDS_RENDTIMETYPE_OUT_OF_RANGE));
...
{
	int i = val->to_int();
	MXS_range_check(i, 1, 3, MaxSDK::GetResourceStringAsMSTR(IDS_RENDVIDCORRECTMETHOD_OUT_OF_RANGE));
...
{
	int i = val->to_int();
	MXS_range_check(i, 1, 2, MaxSDK::GetResourceStringAsMSTR(IDS_RENDFIELDORDER_OUT_OF_RANGE));
...
{
	int i = val->to_int();
	MXS_range_check(i, 1, 2, MaxSDK::GetResourceStringAsMSTR(IDS_RENDNTSC_PAL_OUT_OF_RANGE));
...
{
	int i = val->to_int();
	MXS_range_check(i, 1, 255, MaxSDK::GetResourceStringAsMSTR(IDS_RENDSUPERBLACKTHRESH_OUT_OF_RANGE));
...
#define ET_POINT_CTRL_CONTAINER_REF 0
	...
	getPointController_cf(Value** arg_list, int count)
{
	...
	if (obj->ClassID() == Class_ID(EDITTRIOBJ_CLASS_ID, 0)) {
		...
		MXS_range_check(pointIndex, 1, em->GetMesh().getNumVerts(), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTEX_INDEX_OUT_OF_RANGE));
		...
		MXS_range_check(vertIndex, 1, pointCount, MaxSDK::GetResourceStringAsMSTR(IDS_SPLINE_VERTEX_INDEX_OUT_OF_RANGE));
	...
	setAxisConstraints(Value* val)
{
	...
	MAXScript_interface->SetAxisConstraints(state); // calls UpdateOsnapDlg
	return val;
}
...
Value* getAutoBackupEnabled()
{
	return GetAutoBackupInterface()->GetEnable() ? &true_value : &false_value;
}

Value* setAutoBackupEnabled(Value* val)
{
	GetAutoBackupInterface()->SetEnable(val->to_bool());
	return val;
}

Value* getAutoBackupTime()
{
	return_value (Integer::intern(GetAutoBackupInterface()->GetInterval()));
}

Value* setAutoBackupTime(Value* val)
{
	IAutoBackup* autobackup = GetAutoBackupInterface();
	autobackup->SetInterval(val->to_int());
	val = Integer::intern(autobackup->GetInterval());
	return val;
}
	...
	eulerToQuat_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(order, 1, 12, MaxSDK::GetResourceStringAsMSTR(IDS_AXIS_ORDER_INDEX_OUT_OF_RANGE));
...
public:
	...
	void add_control(Rollout *ro, HWND parent, HINSTANCE hInstance, int& current_y)
	{
		...
		width = MaxSDK::min(size.cx, pos.width);
	...
	doesFileExist_cf(Value** arg_list, int count)
{
	...
	return (DoesFileExist(arg_list[0]->to_filename().data(), allowDirectory, ignoreCache) ? &true_value : &false_value);
}
	...
	doesDirectoryExist_cf(Value** arg_list, int count)
{
	...
	Value* tmp;
	bool ignoreCache = bool_key_arg(ignoreCache, tmp, FALSE) != FALSE;
	return (DoesDirectoryExist(arg_list[0]->to_filename().data(), ignoreCache) ? &true_value : &false_value);
}
	...
	get_productAppID()
{
	if (productAppID == NULL)
	{
		...
		vl.code = vl.parser->compile_all(vl.source, MAXScript::ScriptSource::NonEmbedded);
		vl.source->close();
	...
	internetCheckConnection_cf(Value** arg_list, int count)
{
	...
	if (url)
		MAXScript::ValidateCanRunMAXScriptSystemCommand(_T("Internet.CheckConnection"), arg_list, count);
	...
	getSubMtl_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(which, 1, m->NumSubMtls(), MaxSDK::GetResourceStringAsMSTR(IDS_SUBMTL_INDEX_OUT_OF_RANGE));
...
{
	check_arg_count_with_keys(getSubMtlSlotName, 2, count);
	...
	Value* v = nullptr;
	BOOL localizedName = bool_key_arg(localizedName, v, TRUE);

	TSTR s_name = m->GetSubMtlSlotName(which - 1, localizedName);
	...
	getSubTexmap_cf(Value** arg_list, int count)
{
	...
	MtlBase *m = arg_list[0]->to_mtlbase();
	int which = arg_list[1]->to_int();
	MXS_range_check(which, 1, m->NumSubTexmaps(), MaxSDK::GetResourceStringAsMSTR(IDS_SUBTEXMAP_INDEX_OUT_OF_RANGE));
...
{
	check_arg_count_with_keys(getSubTexmapSlotName, 2, count);
	...
	TSTR s_name = m->GetSubTexmapSlotName(which - 1, localizedName);
	return new String(s_name);
}
...
Value*
getHideByCategory_nonrenderables()
{
	DWORD f = GetCOREInterface()->GetHideByCategoryFlags();
	return (f & HIDE_NONRENDERABLES) ? &true_value : &false_value;
}
Value*
setHideByCategory_nonrenderables(Value* val)
{
	DWORD f = HIDE_NONRENDERABLES;
	DWORD f1 = GetCOREInterface()->GetHideByCategoryFlags();
	bool isHidden = (f1 & f) ? true : false; // currently hidden if != 0
	bool hide = val->to_bool() ? true : false;
	if (isHidden != hide)
		GetCOREInterface()->ExecuteMAXCommand(MAXCOM_HIDE_NONRENDERABLE_TOGGLE);
	return val;
}
...
class GetInstancesProc1_AnimEnum : public AnimEnum
{
public:
	std::unordered_set<Value*>& maxClasses;
	MXSDictionaryValue* result;
	bool allClasses;
	GetInstancesProc1_AnimEnum(
			std::unordered_set<Value*>& maxClasses, bool allClasses, MXSDictionaryValue* result, int scope)
			: AnimEnum(scope)
	...
	int proc(Animatable *anim, Animatable *client, int subNum)
	{
		...
		if (sid == MAXSCRIPT_WRAPPER_CLASS_ID)
			return ANIM_ENUM_STOP;
		Class_ID cid = anim->ClassID();
		MAXClass* cls = lookup_MAXClass(&cid, sid);
		if (cls && (allClasses || maxClasses.find(cls) != maxClasses.end()))
		{
			Value* item = MAXClass::make_wrapper_for((ReferenceTarget*)anim);
			if (item != &undefined)
			{
				Array* existingList = (Array*)result->get(cls);
				if ((Value*)existingList == &undefined)
				{
					existingList = new Array(1);
					result->put(cls, existingList);
				}
				TrackViewPick tvp;
				tvp.anim = (ReferenceTarget*)anim;
				tvp.client = (ReferenceTarget*)client;
				tvp.subNum = subNum;
				existingList->append(new TrackViewPickValue(tvp));
			}
		}
...
class GetInstancesProc2_AnimEnum : public AnimEnum
{
public:
	MAXClass* mc;
	Array* result;
	GetInstancesProc2_AnimEnum(MAXClass* mc, Array* result, int scope)
			: AnimEnum(scope)
			, mc(mc)
			, result(result)
	{
...
class GetInstancesProc1_RefEnum : public RefEnumProc
{
public:
	std::unordered_set<Value*>& maxClasses;
	MXSDictionaryValue* result;
	...
	GetInstancesProc1_RefEnum(
			std::unordered_set<Value*>& maxClasses, bool allClasses, MXSDictionaryValue* result, bool processChildren)
			: maxClasses(maxClasses)
			, allClasses(allClasses)
			, result(result)
...
class GetInstancesProc2_RefEnum : public RefEnumProc
{
public:
	MAXClass* mc;
	Array* result;
	BOOL processChildren;
	GetInstancesProc2_RefEnum(MAXClass* mc, Array* result, BOOL processChildren)
			: mc(mc)
			, result(result)
			, processChildren(processChildren)
	{
...
class GetInstancesProc1_AnimListEnum : public Animatable::EnumAnimList
{
public:
	std::unordered_set<Value*>& maxClasses;
	MXSDictionaryValue* result;
	AnimHandle nextAnimHandle;
	bool allClasses;
	GetInstancesProc1_AnimListEnum(std::unordered_set<Value*>& maxClasses, bool allClasses, MXSDictionaryValue* result)
			: maxClasses(maxClasses)
			, allClasses(allClasses)
			, result(result)
			, nextAnimHandle(Animatable::GetNextHandle())
	{
	}
	bool proc(Animatable* theAnim)
	{
		if (theAnim == NULL)
			return true;
		if (!theAnim->IsRefMaker() || !((ReferenceMaker*)theAnim)->IsRefTarget())
			return true;
		// Creating a MAXMaterialLibrary instance creates a MtlBaseLib instance. Could have
		// other cases where creating a MAXWrapper causes a new Animatable to be created.
		// So just look at animatables existing when enumeration began.
		if (Animatable::GetHandleByAnim(theAnim) >= nextAnimHandle)
			return true;
		SClass_ID sid = theAnim->SuperClassID();
		if (sid == MAXSCRIPT_WRAPPER_CLASS_ID)
			return true;
		// don't wrap if there are no other references to the ReferenceTarget. It could be 
		// a static instance, or it could be an Object* held by an ObjectState. ObjectState
		// holds the object as a raw pointer.
 		ReferenceTarget* refTarg = (ReferenceTarget*)theAnim;
 		if (!refTarg->HasDependents())
 			return true;
...
class GetInstancesProc2_AnimListEnum : public Animatable::EnumAnimList
{
public:
	MAXClass* mc;
	Array* result;
	AnimHandle nextAnimHandle;
	GetInstancesProc2_AnimListEnum(MAXClass* mc, Array* result)
			: mc(mc)
			, result(result)
			, nextAnimHandle(Animatable::GetNextHandle())
	{
	...
	getClassInstances_cf(Value** arg_list, int count)
{
	...
	{
		arrayArg = true;
		Array* maxClassesArg = (Array*)arg_list[0];
		for (int i = 0; i < maxClassesArg->size; ++i)
		{
			Value* ele = maxClassesArg->data[i];
			if (!ele->is_kind_of(class_tag(MAXClass)))
				type_check(ele, MAXClass, _T("getClassInstances"));
			maxClasses.insert(ele);
		}
	}
	else if (arg_list[0] == n_all)
	{
		allClasses = true;
	}
	else
	{
		mc = (MAXClass*)arg_list[0];
		if (!mc->is_kind_of(class_tag(MAXClass)))
	...
	{
		if (arrayArg || allClasses)
		{
			MXSDictionaryValue* result = new MXSDictionaryValue(MXSDictionaryValue::key_maxclass);
			vl.result = result;
			GetInstancesProc1_AnimListEnum gip(maxClasses, allClasses, result);
		...
		else
		{
			...
			vl.result = result;
			GetInstancesProc2_AnimListEnum gip(mc, result);
			Animatable::EnumerateAllAnimatables(gip);
			return_value_tls(vl.result);
		}
	}
	...
	if (asTrackViewPick)
	{
		if (target)
		{
			...
			if (arrayArg || allClasses)
			{
				MXSDictionaryValue* result = new MXSDictionaryValue(MXSDictionaryValue::key_maxclass);
				vl.result = result;
				GetInstancesProc1_AnimEnum gip(maxClasses, allClasses, result, scope);
			...
			else
			{
				Array* result = new Array(0);
				vl.result = result;
				GetInstancesProc2_AnimEnum gip(mc, result, scope);
				target->EnumAnimTree(&gip, NULL, 0);
			}
		}
	}
	else
	{
		if (arrayArg || allClasses)
		{
			MXSDictionaryValue* result = new MXSDictionaryValue(MXSDictionaryValue::key_maxclass);
			vl.result = result;
			GetInstancesProc1_RefEnum gip(maxClasses, allClasses, result, processChildren);
		...
		else
		{
			Array* result = new Array(0);
			vl.result = result;
			GetInstancesProc2_RefEnum gip(mc, result, processChildren);
			target->EnumRefHierarchy(gip, true, true, true, quickClear);
		}
	}
...
{
public:
	HashTable* classCount; // collect values into a hashtable
...
Value* getImporterExporterExtensions_cf(Value** arg_list, int count)
{
	check_arg_count(getImporterExporterExtensions, 1, count);
	one_typed_value_local(Array * res);
	vl.res = new Array(0);
	if (arg_list[0] == &undefined)
		return_value(vl.res); // case when some plugins are missing
	MAXClass* mc = (MAXClass*)arg_list[0];
	if (!mc->is_kind_of(class_tag(MAXClass)))
		throw RuntimeError(_T("getImporterExporterExtensions() requires a MAXClass parameter, got: "), mc);
	if (mc->sclass_id == SCENE_IMPORT_CLASS_ID)
	{
		ClassDirectory& cdir = MAXScript_interface->GetDllDir().ClassDir();
		ClassEntry* ce = cdir.FindClassEntry(mc->sclass_id, mc->class_id);
		if (ce)
		{
			ClassDesc* cd = ce->FullCD();
			SceneImport* si = (SceneImport*)cd->Create();
			if (si)
			{
				int extCount = si->ExtCount();
				for (int jx = 0; jx < extCount; ++jx)
					vl.res->append(new String(si->Ext(jx)));
				delete si;
			}
		}
	}
	else if (mc->sclass_id == SCENE_EXPORT_CLASS_ID)
	{
		ClassDirectory& cdir = MAXScript_interface->GetDllDir().ClassDir();
		ClassEntry* ce = cdir.FindClassEntry(mc->sclass_id, mc->class_id);
		if (ce)
		{
			ClassDesc* cd = ce->FullCD();
			SceneExport* se = (SceneExport*)cd->Create();
			if (se)
			{
				int extCount = se->ExtCount();
				for (int jx = 0; jx < extCount; ++jx)
					vl.res->append(new String(se->Ext(jx)));
				delete se;
			}
		}
	}
	else
		throw RuntimeError(_T("getImporterExporterExtensions() requires an importer or exporter MAXClass, got: "), mc);
	return_value(vl.res);
}
...
{
	check_arg_count_with_keys(getClassName, 1, count);
	TSTR name;
	...
	if (ref)
	{
		...
		ref->GetClassName(name, localizedName);
	}
	...
	setSnapMarkSize(Value* val)
{
	...
	MAXScript_interface17->UpdateOsnapDlg();
	return val;
}
...
{
	IOsnapManager7* theman = IOsnapManager7::GetInstance();
	theman->SetSnapToFrozen(val->to_bool());
	MAXScript_interface17->UpdateOsnapDlg();
	return val;
}
...
{
	IOsnapManager7* theman = IOsnapManager7::GetInstance();
	theman->SetDisplaySnapRubberBand(val->to_bool()); // calls UpdateOsnapDlg
	return val;
}
	...
	snap_getOSnapName_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(which, 1, nsnaps, _T("Object snap index out of range: "));
	return &undefined;
}
	...
	snap_getOSnapItemName_cf(Value** arg_list, int count)
{
	...
	while((thesnap = theman->Enumerate(NEXT)) != NULL) {
		...
		if (nsnaps == which) {
			...
			MXS_range_check(item, 1, nitems, _T("Object snap item index out of range: "));
...
Value* setSnapAngle(Value* val)
{
	MAXScript_interface7->SetSnapAngle(val->to_float());
	MAXScript_interface17->UpdateOsnapDlg();
	return val;
}
...
Value* getSnapAngle()
{
	return_value(Float::intern(MAXScript_interface7->GetSnapAngle()));
}

Value* setSnapPercent(Value* val)
{
	MAXScript_interface7->SetSnapPercent(val->to_float());
	MAXScript_interface17->UpdateOsnapDlg();
	return val;
}

Value* getSnapPercent()
{
	return_value(Float::intern(MAXScript_interface7->GetSnapPercent()));
}

Value* setSnapShapeAngleConstraintEnabled(Value* val)
{
	SetEnableShapeAngleConstraints(val->to_bool());
	MAXScript_interface17->UpdateOsnapDlg();
	return val;
}

Value* getSnapShapeAngleConstraintEnabled()
{
	return bool_result(GetEnableShapeAngleConstraints());
}

Value* setSnapShapeAngleConstraintValue(Value* val)
{
	SetShapeAngleConstraint(val->to_float());
	MAXScript_interface17->UpdateOsnapDlg();
	return val;
}

Value* getSnapShapeAngleConstraintValue()
{
	return_value(Float::intern(GetShapeAngleConstraint()));
}

Value* getSnapAactive()
{
	return bool_result(MAXScript_interface7->GetSnapActive());
}

Value* setSnapAactive(Value* val)
{
	MAXScript_interface7->SetSnapActive(val->to_bool());
	return val;
}

Value* getSnapType()
{
	int code = MAXScript_interface7->GetSnapType();
	Value* type = (code == SNAP_2D) ? n_2D : (code == SNAP_25D) ? n_2_5D : (code == SNAP_3D) ? n_3D : &undefined;
	return type;
}

Value* setSnapType(Value* val)
{
	int code;
	if (val == n_2D)
		code = SNAP_2D;
		...
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_BAD_SNAP_MODE_TYPE), val);
	MAXScript_interface7->SetSnapType(code);
	return val;
}

Value* getAngleSnapActive()
{
	return bool_result(MAXScript_interface7->ASnapStatus());
}

Value* setAngleSnapActive(Value* val)
{
	MAXScript_interface17->SetASnapStatus(val->to_bool());
	return val;
}

Value* getPercentSnapActive()
{
	return bool_result(MAXScript_interface7->PSnapStatus());
}

Value* setPercentSnapActive(Value* val)
{
	MAXScript_interface17->SetPSnapStatus(val->to_bool());
	return val;
}
...
static BOOL MyComparePixels(float p1, float p2, float var, float &dif)
{
	dif = fabsf(p1 - p2);
	return (dif > var);
}
...
{
	// CompareBitmaps <filename>File1 <filename>File2 <int>tolerance(# different pixels) <int>variation(0-255) useAlpha:<boolean> errorMsg:<&string> errorCount:<&int> maxDelta:<&float>
	// CompareBitmaps <bitmap>bitmap1 <bitmap>bitmap2 <int>tolerance(# different pixels) <int>variation(0-255) useAlpha:<boolean> errorMsg:<&string> errorCount:<&int> maxDelta:<&float>
	...
	Value* errorCountValue = key_arg(errorCount);
	Thunk* errorCountThunk = NULL;
	if (errorCountValue != &unsupplied && errorCountValue->_is_thunk())
		errorCountThunk = errorCountValue->to_thunk();

	Value* maxDeltaValue = key_arg(maxDelta);
	Thunk* maxDeltaThunk = NULL;
	if (maxDeltaValue != &unsupplied && maxDeltaValue->_is_thunk())
		maxDeltaThunk = maxDeltaValue->to_thunk();
	...
	if (isbm1Loaded == BMMRES_SUCCESS && isbm2Loaded == BMMRES_SUCCESS && bm1 && bm2 && bi1 && bi2)
	{
		...
		if ((biWidth == bi2Width) && (biHeight == bi2Height))
		{
			...
			for (int i = 0; i < biWidth; i++)
			{
				for (int j = 0; j < biHeight; j++)
				{
					...
					{
						float diff = 0.;
						BOOL isDiff = MyComparePixels(pi1[c], pi2[c], variance, diff);
						if (diff > maxDelta)
							maxDelta = diff;
						if (isDiff)
						{
							pixelDifferent = true;
							if (maxDeltaThunk == nullptr)
								break;
						}
					}
					if (pixelDifferent)
	 ...
	 if (errorCountThunk)
		errorCountThunk->assign(Integer::intern(mycount));

	 if (maxDeltaThunk)
		maxDeltaThunk->assign(Float::intern(maxDelta));
...
// We need to override the _invalid_parameter_handler registered by minidump code so that we 
// don't crash if invalid parameter is passed as format string to formattedPrint.
// See 3dswin\src\app\minidump.cpp for where minidump code is registering _invalid_parameter_handler

// An _invalid_parameter_handler implementation that throws a runtime error
static void __cdecl RTLInvalidParameterHandler(const wchar_t * expression,
	const wchar_t * function,
	const wchar_t * file,
	unsigned int line, uintptr_t pReserved)
{
	throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_INVALID_PARAMETER_WAS_PASSED_TO_FUNCTION));
}

// An RAII class that does a push/set/pop of _invalid_parameter_handler
class RTLInvalidParameterHandlerHelper : public MaxSDK::Util::Noncopyable
{
private:
	_invalid_parameter_handler m_pfnOldHandler = nullptr;
public:
	RTLInvalidParameterHandlerHelper()
	{
		m_pfnOldHandler = _set_invalid_parameter_handler(&RTLInvalidParameterHandler);
	}
	~RTLInvalidParameterHandlerHelper()
	{
		_set_invalid_parameter_handler(m_pfnOldHandler);
	}
};
	...
	formattedPrint_cf(Value** arg_list, int count)
{
	...
	Value* tmp;
	bool userLocale = bool_key_arg(userLocale, tmp, false);
	const _locale_t locale = userLocale ? GetUserLocale() : GetCNumericLocale();

	RTLInvalidParameterHandlerHelper _RTLInvalidParameterHandlerHelper;
	...
	if (is_float(arg))
	{
		...
		buf.printf_l(format, locale, EPS(arg->to_float()));
	}
	else if (is_double(arg))
	{
		if (!formatGiven) 
			format = defaultFloatFormat;
		buf.printf_l(format, locale, EPS(arg->to_double()));
	}
	else if (is_int(arg))
	{
		if (!formatGiven) 
			format = _T("%I32d");
		buf.printf_l(format, locale, arg->to_int());
	}
	else if (is_int64(arg))
	{
		if (!formatGiven) 
			format = _T("%I64d");
		buf.printf_l(format, locale, arg->to_int64());
	}
	else if (is_intptr(arg))
	{
		if (!formatGiven) 
			format = _T("%Id");
		buf.printf_l(format, locale, arg->to_intptr());
	}
	else if (is_string(arg))
	{
		if (!formatGiven) 
			format = _T("%s");
		buf.printf_l(format, locale, arg->to_string());
	}
	else if (is_bool(arg))
	{
		if (!formatGiven) 
			format = _T("%d");
		buf.printf_l(format, locale, arg->to_bool());
	}
	else if (is_ray(arg))
	{
		Ray r = arg->to_ray();
		...
		buf.printf_l(formatRay, locale, EPS(r.p.x), EPS(r.p.y), EPS(r.p.z), EPS(r.dir.x), EPS(r.dir.y), EPS(r.dir.z));
	}
	else if (is_point2(arg))
	{
		...
		buf.printf_l(formatP3, locale, EPS(p.x), EPS(p.y));
	}
	else if (is_point3(arg))
	{
		...
		buf.printf_l(formatP3, locale, EPS(p.x), EPS(p.y), EPS(p.z));
	}
	else if (is_point4(arg))
	{
		...
		buf.printf_l(formatP4, locale, EPS(p.w), EPS(p.x), EPS(p.y), EPS(p.z));
	}
	else if (is_quat(arg))
	{
		Quat q = arg->to_quat();
		...
		buf.printf_l(formatQ, locale, EPS(q.x), EPS(q.y), EPS(q.z), EPS(q.w));
	}
	else if (is_angaxis(arg))
	{
		...
		buf.printf_l(formatAA, locale, EPS(RadToDeg(aa.angle)), EPS(aa.axis.x), EPS(aa.axis.y), EPS(aa.axis.z));
	}
	else if (is_eulerangles(arg))
	{
		...
		buf.printf_l(formatEA, locale, EPS(RadToDeg(eav->angles[0])), EPS(RadToDeg(eav->angles[1])), EPS(RadToDeg(eav->angles[2])));
	}
	...
	else if (is_box2(arg))
	{
		...
		buf.printf_l(formatB2, locale, b.x(), b.y(), b.w(), b.h());
	}
	else if (is_color(arg))
	{
		...
		formatC += Rparen;
		buf.printf_l(formatC, locale, EPS(color.r * 255.0f), EPS(color.g * 255.0f), EPS(color.b * 255.0f), EPS(color.a * 255.0f));
	}
	else if (is_time(arg))
	{
		if (!formatGiven) 
		...
		buf.printf_l(formatT, locale, EPS(arg->to_timevalue() / (float)GetTicksPerFrame()));
	}
	...
	pasteBitmap_cf(Value** arg_list, int count)
{
	...
	/*	MXS_range_check(fromX,0,src_bm->bi.Width()-1,_T("source X startpoint"));
	MXS_range_check(fromY,0,src_bm->bi.Height()-1,_T("source Y startpoint"));
	MXS_range_check(destX,0,targ_bm->bi.Width()-1,_T("target X startpoint"));
	MXS_range_check(destY,0,targ_bm->bi.Height()-1,_T("target Y startpoint"));
	*/
	...
	weldSpline_cf(Value** arg_list, int count)
{
	...
	if (obj->ClassID() != splineShapeClassID && obj->ClassID() != Class_ID(SPLINE3D_CLASS_ID,0))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_SHAPE_OPERATION_ON_NONSPLINESHAPE), obj->GetObjectName(true));
	...
	getPointPos_cf(Value** arg_list, int count)
{
	...
	{
		int index = arg_list[1]->to_int();
		MXS_range_check(index,1,ob->NumPoints(),_T("point number out of range"));
	...
	nodeGetBoundingBox_cf(Value** arg_list, int count)
{
	...
	check_arg_count_with_keys(nodeGetBoundingBox, 2, count);
	...
	intersectRayScene(INode* node, Value* vRay)
{
	...
	{
		// We hit "behind" the ray. Bad bad raycast, go to your room.
		if (at < 0.f) {
			return &undefined;
		}
	...
	LayerManager_getLayerObject_cf(Value** arg_list, int count)
{
	...
	if (is_integer(arg_list[0]))
	{
		...
		MXS_range_check(index, 0, theman->GetLayerCount()-1,_T("Layer index out of range: "));
	...
	colorPickerDlg_cf(Value** arg_list, int count)
{
	...
	if (alpha)
	{
		...
		if (res) 
			vl.result = new ColorValue(color);
	}
	else
	{
		// Although the script asked for the non-alpha version, launch the alpha
		// version to retain the color fidelity. /coz 2024-02-02
		
		AColor color = arg_list[0]->to_acolor();
		color.a = 1.0f;
		
		auto colpick = GetCurColPick();
		colpick->Execute(0x505, 1); // Force disable alpha
		res = HSVDlg_Do(MAXScript_interface->GetMAXHWnd(), &color, ppos, NULL, name);
		colpick->Execute(0x505, 0); // turn of forced disable alpha.
		color.a = 1.0f;
	...
	getBitmapInfo_cf(Value** arg_list, int count)
{
	...
	else
	{
		...
		BOOL found = 
			...
			SearchPath(MAXScript_interface->GetDir(APP_IMAGE_DIR).data(), fileName, NULL, MAX_PATH, filepath, &fpart);
		if (!found)
		{
...
Value* HiddenDOSCommand_cf(Value** arg_list, int count)
{
	...
	MAXScript::ValidateCanRunMAXScriptSystemCommand( _T("HiddenDOSCommand"), arg_list, count);
...
Value* getNumAnimatables_cf(Value** arg_list, int count)
{
	check_arg_count(getNumAnimatables, 0, count);
	return_value(IntegerPtr::intern(Animatable::GetNumAnimatables()));
}

Value* getNextAnimHandle_cf(Value** arg_list, int count)
{
	check_arg_count(getNextAnimHandle, 0, count);
	return_value(IntegerPtr::intern(Animatable::GetNextHandle()));
}
	...
	get_max_file_asset_metadata_cf(Value** arg_list, int count)
{
	...
	const MSTR scene_dir = MAXScript_interface->GetDir(APP_SCENE_DIR);
		...
		SearchPath(NULL, fileName, NULL, MAX_PATH, buf, NULL))
	{
		...
		HRESULT hres = GetCOREInterface11()->OpenMAXStorageFile(WStr::FromMCHAR(buf).data(), &pIStorage);
	...
	set_max_file_asset_metadata_cf(Value** arg_list, int count)
{
		...
		SearchPath(NULL, fileName, NULL, MAX_PATH, buf, NULL))
	{
		...
		HRESULT hres = StgOpenStorageEx(WStr::FromMCHAR(buf).data(), mode, STGFMT_DOCFILE, 0, &StgOptions, NULL,
	...
	while (NULL != (rm = di.Next())) {
		...
		if (sid == BASENODE_CLASS_ID) {
			...
			if (node->TestAFlag(A_IS_DELETED))
				return false;
			return (node->IsSceneNode() || includeXrefScenes || !node->IsSceneXRefNode());
		}
...
ValidateSingleRefMaker_cf(Value** arg_list, int count)
{
	...
	TSTR className1 = srm7->ClassName(true);
	TSTR className2 = thePB7a->ClassName(true);
...
FPValue res;
ExecuteMAXScriptScript("maxops.pivotMode", MAXScript::ScriptSource::NonEmbedded, TRUE, &res);
...
Value* test_function_cf(Value** arg_list, int count)
{
	if (count != 0)
	{
		ClearAFlagInAllAnimatables(A_WORK4);
	}
	else
	{
		int index = Animatable::RequestFlagBit();
		Animatable::ClearFlagBitInAllAnimatables(index);
		Animatable::ReleaseFlagBit(index);
	}
	return &ok;
}
def_visible_primitive(test_function, "test_function");
...
#define DIRECTVALUETHUNK_TAG ((ValueMetaClass*)200212L) 

class DirectValueThunk : public Thunk
{
	Value*& mVal;
public:
	DirectValueThunk(Value*& value) : mVal(value)
	{
		tag = DIRECTVALUETHUNK_TAG;
	}
	Value* eval() { return mVal; }
	void gc_trace() {}
	void collect() {}
	void sprin1(CharStream* s) { s->printf(_T("DirectValueThunk")); }
...
def_visible_primitive(test_byRef_function_arg, "test_byRef_function_arg");

Value* test_byRef_function_arg_cf(Value** arg_list, int count)
{
	check_arg_count(test_byRef_function_arg, 1, count);
	Value* fn = arg_list[0];
	if (!is_function(fn))
		throw TypeError(_T("Expected function"), fn);

	ScopedMaxScriptEvaluationContext scopedMaxScriptEvaluationContext;
	MAXScript_TLS* _tls = scopedMaxScriptEvaluationContext.Get_TLS();
	two_typed_value_locals_tls(Value* arg, DirectValueThunk* thunk);
	vl.arg = &false_value;
	vl.thunk = new DirectValueThunk(vl.arg);

	Value* args[3]{ 
		&keyarg_marker,
		Name::intern(_T("byrefParam")),
		new ThunkReference(vl.thunk)
	};
	fn->apply(args, 3);
...
public:
	...
	bool proc(Animatable *theAnim)
	{
		...
		if (outputAnimInfo)
		{
			...
			TSTR animClassName = theAnim->ClassName(true);
				...
				if (theRefTarget)
				{
					...
					else
					{
						...
						if (theLogSys)
						{
							...
							TSTR className;
							failingRefMaker->GetClassName(className, true);
...
Value*
getDecimalChars_cf(Value** arg_list, int count)
{
	// localeInfo.getDecimalChars()
	// returns decimal separator char from GetLocaleInfo, c runtime, c++ runtime, and .net
	check_arg_count(localeInfo.getDecimalChars, 0, count);
	TSTR localeInfo, cRuntime, cppRuntime, dotNet;
	TCHAR decimal_point[2] = { 0 };

	GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SDECIMAL, localeInfo.dataForWrite(2), 2);

	lconv* lc = localeconv();
	decimal_point[0] = *(lc->_W_decimal_point);
	cRuntime = decimal_point;

	std::locale loc2;
	decimal_point[0] = std::use_facet< std::numpunct<TCHAR> >(loc2).decimal_point();
	cppRuntime = decimal_point;

	FPValue fpv;
	BOOL ok = ExecuteMAXScriptScript(_T("(dotnetclass \"System.Globalization.CultureInfo\").CurrentCulture.NumberFormat.NumberDecimalSeparator"), MAXScript::ScriptSource::NonEmbedded, TRUE, &fpv, FALSE);
	if (ok)
	{
		dotNet = fpv.s;
	}
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(Array* res);
	vl.res = new Array(4);
	vl.res->append(new String(localeInfo));
	vl.res->append(new String(cRuntime));
	vl.res->append(new String(cppRuntime));
	vl.res->append(new String(dotNet));
	return_value_tls(vl.res);
}

Value*
getLocale_cf(Value** arg_list, int count)
{
	// <string> localeInfo.getLocale <which>
	check_arg_count(localeInfo.getLocale, 1, count);
	def_Locale_categories_types();
	int which = ConvertValueToID(LocaleCategoriesTypes, _countof(LocaleCategoriesTypes), arg_list[0]);
	TSTR locale = _tsetlocale(which, nullptr);
	return_value(new String(locale));
}

Value*
setLocale_cf(Value** arg_list, int count)
{
	// localeInfo.setLocale <which> <locale string>
	check_arg_count(localeInfo.setLocale, 2, count);
	def_Locale_categories_types();
	int which = ConvertValueToID(LocaleCategoriesTypes, _countof(LocaleCategoriesTypes), arg_list[0]);
	const TCHAR* locale = arg_list[1]->to_string();
	// get existing locale name
	TSTR oldlocale = _tsetlocale(which, nullptr);
	// set new locale
	const TCHAR* newLocale = _tsetlocale(which, locale);
	return_value(newLocale ? (Value*)new String(oldlocale) : &false_value);
}

Value*
getCPlusPlusLocale_cf(Value** arg_list, int count)
{
	// <string> localeInfo.getCPlusPlusLocale()
	check_arg_count(localeInfo.getCPlusPlusLocale, 0, count);
	std::locale loc;
	std::string name = loc.name();
	TSTR locale = TSTR::FromACP(name.c_str());
	return_value(new String(locale));
}

Value*
setCPlusPlusLocale_cf(Value** arg_list, int count)
{
	// localeInfo.setCPlusPlusLocale <locale string>
	check_arg_count(localeInfo.setCPlusPlusLocale, 1, count);
	TSTR locale = arg_list[0]->to_string();
	// get existing locale name
	std::locale loc;
	std::string name = loc.name();
	TSTR oldlocale = TSTR::FromACP(name.c_str());
	// set new locale
	bool success = false;
	try
	{
		CStr new_locale = locale.ToACP();
		std::locale gloc(new_locale);
		std::locale::global(gloc);
		success = true;
	}
	catch (const std::runtime_error &)
	{
	}
	return_value(success ? (Value*)new String(oldlocale) : &false_value);
}


Value*
getPerThreadLocale_cf(Value** arg_list, int count)
{
	// <bool> localeInfo.getPerThreadLocale()
	check_arg_count(localeInfo.getPerThreadLocale, 0, count);
	return bool_result(_configthreadlocale(0) == _ENABLE_PER_THREAD_LOCALE);
}

Value*
setPerThreadLocale_cf(Value** arg_list, int count)
{
	// localeInfo.setPerThreadLocale <bool>
	check_arg_count(localeInfo.setPerThreadLocale, 1, count);
	bool res = _configthreadlocale(0) == _ENABLE_PER_THREAD_LOCALE;
	int argVal = arg_list[0]->to_bool() ? _ENABLE_PER_THREAD_LOCALE : _DISABLE_PER_THREAD_LOCALE;
	_configthreadlocale(argVal);
	return bool_result(res);
}

// Convert from ParamType2 / Integer to maxscript type name :
Value*
ParamTypeToTypeName_cf(Value** arg_list, int count)
{
	// <type_name> paramTypeToTypeName <int>
	check_arg_count(paramTypeToTypeName, 1, count);
	ParamType2 type = (ParamType2)arg_list[0]->to_int();
	const Value* res = MSPluginClass::lookup_name_from_paramType(type);
	if (res == nullptr)
		res = &undefined;
	return_value(const_cast<Value*>(res));
}

// Convert from maxscript type name to ParamType2 / Integer:
// 
Value*
TypeNameToParamType_cf(Value** arg_list, int count)
{
	// <int> typeNameToParamType <type_name>
	check_arg_count(typeNameToParamType, 1, count);
	type_check(arg_list[0], Name, _T("typeNameToParamType"));
	ParamType2 res = MSPluginClass::lookup_paramType_from_name(arg_list[0]);
	if (res == -1)
		return &undefined;
	else
		return_value(Integer::intern(res));
}
