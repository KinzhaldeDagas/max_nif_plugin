...
refs_dependsOn_cf(Value** arg_list, int count)
{
	...
	if (arg_list[0]->is_kind_of(class_tag(MAXWrapper)))
	{
		...
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		two_typed_value_locals_tls(Array* result, Value* item;);
...
refs_getnumrefs_cf(Value** arg_list, int count)
{
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	return_value_tls(Integer::intern(numRefs));
}
...
refs_getreference_cf(Value** arg_list, int count)
{
	...
	if (refTarg != NULL)
	{
		...
		MXS_range_check(whichRef, 1, refTarg->NumRefs(),_T("reference index"));
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		return_value_tls(WrapObjectReference(refTarg, whichRef-1));
	}
	return &undefined;
}
...
refs_getnumIndirectrefs_cf(Value** arg_list, int count)
{
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	return_value_tls(Integer::intern(numIndirectRefs));
}
...
refs_getIndirectreference_cf(Value** arg_list, int count)
{
	...
	if (refTarg != NULL)
	{
		...
		if (iirm)
			numIndirectRefs = iirm->NumIndirectRefs();
		MXS_range_check(whichRef, 1, numIndirectRefs,_T("indirect reference index"));
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		return_value_tls(MAXClass::make_wrapper_for(iirm->GetIndirectReference(whichRef-1)));
	}
	return &undefined;
}
...
notifyDependents_cf(Value** arg_list, int count)
{
	...
	if (is_int(partIDVal))
		...
		partID = ConvertValueToID(refmsgParts, elements(refmsgParts), partIDVal, PART_ALL);
...
typedef std::pair<AnimHandle, TSTR> AnimClassNameByHandlePair;
typedef std::unordered_map<AnimHandle, TSTR> AnimClassNameByHandleMap;
class CaptureAnimatableInfoEnum : public Animatable::EnumAnimList
{
public:
	AnimClassNameByHandleMap animClassNameByHandleMap;
	CaptureAnimatableInfoEnum()
	{
	}
	bool proc(Animatable* theAnim)
	{
		AnimHandle handle = Animatable::GetHandleByAnim(theAnim);
		TSTR className = theAnim->ClassName(true);
		if (className.isNull())
			className = _T("<<unknown>>");
		animClassNameByHandleMap.insert(AnimClassNameByHandlePair(handle, className));
		return true;
	}
};

Value* refs_testAddRemoveRefToAll_cf(Value** arg_list, int count)
{
	// refs.testAddRemoveRefToAll()
	check_arg_count(testAddRemoveRefToAll, 0, count);
	CaptureAnimatableInfoEnum captureAnimatableInfoEnum;
	Animatable::EnumerateAllAnimatables(captureAnimatableInfoEnum);
	AnimClassNameByHandleMap& animClassNameByHandleMap = captureAnimatableInfoEnum.animClassNameByHandleMap;
	SingleRefMaker srm;
	bool allOk = true;

	AnimHandle nHandles = Animatable::GetNextHandle();
	AnimHandle nHandlesMax = nHandles;
	for (AnimHandle handle = 0; handle < nHandles; ++handle)
	{
		auto iter = animClassNameByHandleMap.find(handle);
		if (iter != animClassNameByHandleMap.end() && !(*iter).second.isNull())
		{
			if (Animatable* anim = Animatable::GetAnimByHandle(handle))
			{
				if (ReferenceTarget* rtarg = dynamic_cast<ReferenceTarget*>(anim))
				{
					TSTR className = rtarg->ClassName(true);
					DWORD numAnimatables = Animatable::GetNumAnimatables();
					srm.SetRef(rtarg);
					srm.SetRef(nullptr);
					if (Animatable::GetAnimByHandle(handle) == nullptr)
					{
						mprintf(_T("Error - Adding/Removing ref to %s caused ref to be deleted. AnimHandle: %d\n"),
								className.data(), handle);
						animClassNameByHandleMap[handle].Resize(0);
						allOk = false;
					}
					AnimHandle nHandlesNow = Animatable::GetNextHandle();
					if (numAnimatables != Animatable::GetNumAnimatables() || nHandlesNow != nHandlesMax)
					{
						for (auto& ele : animClassNameByHandleMap)
						{
							if (!ele.second.isNull())
							{
								if (Animatable::GetAnimByHandle(ele.first) == nullptr)
								{
									mprintf(_T("Error - Adding/Removing ref to %s caused another ref %s to be ")
											_T("deleted. ")
											_T("AnimHandle: %d / %d\n"),
											className.data(), ele.second.data(), handle, ele.first);
									ele.second.Resize(0);
									allOk = false;
								}
							}
					...
					if (nHandlesNow != nHandlesMax)
					{
						mprintf(_T("Warning - Adding/Removing ref to %s caused %d new refs to be created. AnimHandle: %d\n"),
								className.data(), nHandlesNow - nHandlesMax, handle);
						nHandlesMax = nHandlesNow;
					}
				}
			}
		}
	}
	return allOk ? &true_value : &false_value;
}
