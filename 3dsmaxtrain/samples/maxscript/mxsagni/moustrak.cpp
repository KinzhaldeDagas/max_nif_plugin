...
class TrackMouseCallBack : public MouseCallBack, ReferenceMaker
{
	public:
		...
		virtual void GetClassName(MSTR& s, bool localized) const override { s = _M("TrackMouseCallBack"); } // from Animatable
		// ReferenceMaker
		int			NumRefs() { return nodeTab.Count(); }
		RefTargetHandle GetReference(int i) { return nodeTab[i]; }
private:
		virtual void		SetReference(int i, RefTargetHandle rtarg) { nodeTab[i] = (INode*)rtarg; }
public:
...
class TrackMouseCommandMode : public CommandMode
{
	public:
		...
		void EnterMode() { 
			...
			MAXScript_interface->PushPrompt(MaxSDK::GetResourceStringAsMSTR(IDS_TRACK_MOUSE).data());
...
do_track_callback(TrackMouseCallBack* ppcb, Value* message, int flags)
{
	...
	catch (MAXScriptException& e)
	{
		...
		ppcb->track_callback = NULL;
		ppcb->vl->result = &undefined;
		ProcessMAXScriptException(e, _T("TrackMouseCallBack function"), false, true, true);
	}
	catch (...)
	{
		// any error disables the callback
		ppcb->track_callback = NULL;
		ppcb->vl->result = &undefined;
		ProcessUnknownMAXScriptException(_T("TrackMouseCallBack function"), false, true, true);
	}

	if (_tls->redraw_mode && _tls->needs_redraw)
	{
		MAXScript_interface->RedrawViews(MAXScript_interface->GetTime());
		needs_redraw_clear(_tls);
	}
	BOOL res = ppcb->vl->result == n_continue;
	scopedMaxScriptEvaluationContext.ExitContext(ppcb->vl->result); // make sure stack-based values are handled correctly
...
{	
	...
	// If a node wasn't supplied to the mouseTrack MAXScript function using the "on:" keyword argument and
	// the construction plane is orthogonal to view, we don't process any mouse events (except right clicks),
	// since it doesn't really make sense to do so in that situation because rays casted from the mouse position
	// will never intersect the construction plane
	// We make an exception for right clicks because those are commonly used to stop the tracking of the mouse
	if (vl->node_val == &unsupplied && MAXScript_interface->IsCPEdgeOnInView() && cbmsg != n_mouseAbort)
	{
		cbmsg = NULL;
	}
...
RefResult
TrackMouseCallBack::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate)
{
...
mouse_track_cf(Value** arg_list, int count)
{
	...
	Value** vavl;
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	value_local_array_tls(vavl, sizeof(tmvl) / sizeof(Value*));
	tmvl* vl = (tmvl*)vavl;
	// show any prompt
