...
extern void UIScaleValue(Box2 &rect);
extern void UIUnscaleValue(Box2 &rect);
...
Value* VP_SetType_cf(Value **arg_list, int count)
{
	...
	int cmd = vpt_lookup[ConvertValueToID(viewTypes, elements(viewTypes), arg_list[0], -1)];
	if(cmd >= 0) {
...
Value* VP_GetType_cf(Value **arg_list, int count)
{
	...
	if (panelValue != &unsupplied)
	{
		...
		MXS_range_check(viewPanelIndex, 1, GetViewPanelManager()->GetViewPanelCount(), MaxSDK::GetResourceStringAsMSTR(IDS_VIEW_PANEL_INDEX_OUT_OF_RANGE));
	}
	...
	Value *vptType = ConvertIDToValue(viewTypes, elements(viewTypes), vpt->GetViewType(), &undefined);
...
Value* VP_GetClipScale_cf(Value **arg_list, int count)
{
	...
	return_value (Float::intern(clipScale));
}
...
Value* VP_SetCamera_cf(Value **arg_list, int count)
{
	...
	if(vpt.IsAlive() && camnode)
	{
		...
		else if(os.obj && os.obj->SuperClassID() == LIGHT_CLASS_ID) // LAM - 9/5/01
		{
			...
			((LightObject *)os.obj)->EvalLightState(MAXScript_time_tls(),iv,&ls);
...
Value* VP_GetCamera_cf(Value **arg_list, int count)
{
	...
	if(camnode) return_value (MAXNode::intern(camnode));
	return &undefined;
}
...
Value* VP_SetLayout_cf(Value **arg_list, int count)
{
	...
	int layoutType = ConvertValueToID(vpLayouts, elements(vpLayouts), arg_list[0], -1);
	if(layoutType != -1)
	{
...
Value* VP_GetLayout_cf(Value **arg_list, int count)
{
	...
	Value *layoutKey = ConvertIDToValue(vpLayouts,
		elements(vpLayouts),
...
Value* VP_SetRenderLevel_cf(Value **arg_list, int count)
{
	...
	int renderLevel = ConvertValueToID(vpRenderLevel, elements(vpRenderLevel), arg_list[0], -1);
	if(renderLevel != -1)
	{
...
Value* VP_GetRenderLevel_cf(Value **arg_list, int count)
{
	...
	Value *renderLevelKey = ConvertIDToValue(vpRenderLevel,
...
Value* VP_GetTransparencyLevel_cf(Value **arg_list, int count)
{
	...
	return_value (Integer::intern(level));
}
...
{
	return_value (Integer::intern(MAXScript_interface7->getActiveViewportIndex()+1));
}
...
{
	IViewPanel* pPanel = GetViewPanelManager()->GetActiveViewPanel(); // TVP Compliant
	if (pPanel)
	{
		...
		if (index >= 0)
		{
			...
			if (vpt.IsAlive())
			{
				return_value (Integer::intern(vpt.GetViewID() + 1)); 	// 0 based to 1 based
			}
		}
	}
	return &undefined;
...
Value* VP_GetPickingSelectionRadiusScale()
{
	IViewPanel* pPanel = GetViewPanelManager()->GetActiveViewPanel(); // TVP Compliant
	if (pPanel == nullptr)
		return &undefined;
	...
	ViewExp22& vpt = static_cast<ViewExp22&>(pPanel->GetViewExpByIndex(index));
	if (!vpt.IsAlive())
		return &undefined;

	return_value(Float::intern(vpt.GetPickingSelectionRadiusScale()));
}

Value* VP_SetPickingSelectionRadiusScale(Value *val)
{
	IViewPanel* pPanel = GetViewPanelManager()->GetActiveViewPanel(); // TVP Compliant
	if (pPanel == nullptr)
		return &undefined;

	// Property is global, just grab the currently active viewport
	int index = pPanel->GetActiveViewportIndex();
	if (index < 0)
		return &undefined;
	...
	float selectionRadiusScale = val->to_float();
	vpt.SetPickingSelectionRadiusScale(selectionRadiusScale);

	return val;
}

Value* VP_GetPickingSelectionRadius()
{
	IViewPanel* pPanel = GetViewPanelManager()->GetActiveViewPanel(); // TVP Compliant
	if (pPanel == nullptr)
		return &undefined;
	...
	return_value(Integer::intern(vpt.GetPickingSelectionRadius()));
}

Value* VP_SetPickingSelectionRadius(Value *val)
{
	IViewPanel* pPanel = GetViewPanelManager()->GetActiveViewPanel(); // TVP Compliant
	if (pPanel == nullptr)
		return &undefined;
	...
	int selectionRadius = val->to_int();
	vpt.SetPickingSelectionRadius(selectionRadius);
...
VP_GetNumViews()
{
	return_value (Integer::intern(MAXScript_interface7->getNumViewports()));
}
...
static Rect GetDeviceRect(HWND vptHWND, int bShowSafeFrame)
{
	...
	Rect rect;
	float as = 0.0;
	float rendAspect = MAXScript_interface->GetRendPixelAspect();
...
{
	// viewport.setRegionRect <viewport index> <box2> viewPanelIndex:<view panel index> applyUIScaling:<true>
	...
	if (applyUIScaling && !region.IsEmpty())
		UIScaleValue(region);
...
VP_GetRegionRect_cf(Value **arg_list, int count)
{
	...
	else
	{
		...
		else
		{
			...
			// Use this delta to resolve the precision issue caused by floating point 
			// when converting a float value to int.
			float precisionDelta = 0.05f;
			region.left	= dr.left + (int)(devRect.left * (dr.right - dr.left) + precisionDelta);
			region.right	= dr.left + (int)(devRect.right * (dr.right - dr.left) + precisionDelta);
			region.top	= dr.top  + (int)(devRect.top * (dr.bottom - dr.top) + precisionDelta);
			region.bottom	= dr.top  + (int)(devRect.bottom * (dr.bottom - dr.top) + precisionDelta);
		}
	}
	
	BOOL removeUIScaling = bool_key_arg(removeUIScaling, tmp, TRUE); 
	if (removeUIScaling && !region.IsEmpty())
		UIUnscaleValue(region);
...
{
	// viewport.setBlowupRect <viewport index> <box2> viewPanelIndex:<view panel index> applyUIScaling:<true>
...
{
	// viewport.pan <x> <y>  applyUIScaling:<true>
	check_arg_count_with_keys(VP_Pan, 2, count);
	...
	if (isValid)
	{
		Point2 pt( arg_list[0]->to_float(), arg_list[1]->to_float());
		Value* v = nullptr;
		BOOL applyUIScaling = bool_key_arg(applyUIScaling, v, TRUE);
		if (applyUIScaling)
			pt *= MaxSDK::GetUIScaleFactor();
		vp10.Pan(pt);
		return &true_value;
	}
	return &false_value;
...
Value* VP_GetFocalDistance_cf(Value **arg_list, int count)
{
	...
	return_value (Float::intern( vp10.GetFocalDist() ));
}
...
Value* VP_GetScreenScaleFactor_cf(Value **arg_list, int count)
{
	...
	return_value (Float::intern(screenScaleFactor));
}
...
{
	// viewport.getViewportDib index:<int> viewPanelIndex:<int> captureAlpha:<bool> gammaCorrect:<bool>
	...
	Bitmap *map = CreateBitmapFromBInfo((void**)&buf, bmih->biWidth, bmih->biHeight, captureAlpha);
	delete [] buf;
	free(bmi);
	DeleteDC(hdcMem);
	ReleaseDC(gw_hwnd, hdc);
...
Value* VP_GetAdaptiveDegCameraDistancePriority_cf(Value **arg_list, int count)
{
	...
	return_value (Float::intern(dist));
}
...
Value* VP_GetAdaptiveDegScreenSizePriority_cf(Value **arg_list, int count)
{
	...
	return_value (Float::intern(size));
}
...
Value* VP_GetAdaptiveDegMinSize_cf(Value **arg_list, int count)
{
	...
	return_value (Integer::intern(size));
}
...
Value* VP_GetID_cf( Value **arg_list, int count )
{
	...
	if(viewID>=0) return_value (Integer::intern(viewID));
	return &undefined;
}
...
Value* VP_GetViewNumEx_cf( Value **arg_list, int count )
{
	...
	{
		return_value (Integer::intern( (int)pPanel->GetNumberOfViewports() ));
	}
}
...
// ============================================================================
#define VIEWEXP_EXECUTE_IS_CANVAS_NAVIGATION_MODE 1000
#define VIEWEXP_EXECUTE_SET_CANVAS_NAVIGATION_MODE 1001

Value* VP_IsCanvasNavigationMode_cf(Value **arg_list, int count)
{
	check_arg_count(IsCanvasNavigationMode, 0, count);

	bool result = MAXScript_interface->GetActiveViewExp().Execute(VIEWEXP_EXECUTE_IS_CANVAS_NAVIGATION_MODE) ? true : false;

	return bool_result(result);
}

Value* VP_SetInPresentationMode_cf(Value **arg_list, int count)
{
	check_arg_count(IsInPresentationMode, 1, count);
    
    const bool onoff        = (TRUE == arg_list[0]->to_bool());

	ViewExp& viewExp		= MAXScript_interface->GetActiveViewExp();
	ViewExp22* pViewExp22	= dynamic_cast<ViewExp22*>(&viewExp);
	if(nullptr != pViewExp22){
		pViewExp22->SetInPresentationMode(onoff);
	}

	return &ok;
}

Value* VP_IsInPresentationMode_cf(Value **arg_list, int count)
{
	check_arg_count(IsInPresentationMode, 0, count);

	ViewExp& viewExp		= MAXScript_interface->GetActiveViewExp();
	ViewExp22* pViewExp22	= dynamic_cast<ViewExp22*>(&viewExp);
	const bool result		= (nullptr != pViewExp22) && pViewExp22->IsInPresentationMode();

	return bool_result(result);
}

Value* VP_SetCanvasNavigationMode_cf(Value **arg_list, int count)
{
	check_arg_count(SetCanvasNavigationMode, 1, count);

	bool result = MAXScript_interface->GetActiveViewExp().Execute(VIEWEXP_EXECUTE_SET_CANVAS_NAVIGATION_MODE, (ULONG_PTR)(arg_list[0]->to_bool() ? true : false)) ? true : false;

	return bool_result(result);
}

Value* VP_AppendTooltip_cf(Value **arg_list, int count)
{
	check_arg_count(AppendTooltip, 1, count);
	ViewExp18* vp18 = dynamic_cast<ViewExp18*>(&MAXScript_interface->GetActiveViewExp());
	if (vp18)
	{
		const MSTR content = arg_list[0]->to_mstr();
		if (!content.isNull())
		{
			vp18->AppendTooltip(content);
...
Value* VP_GetHWnd_cf(Value** arg_list, int count)
{
	// <int_HWND>viewport.getHWnd index:<int> viewPanelIndex:<int> 
	check_arg_count_with_keys(getHWnd, 0, count);
	...
	if (gw == nullptr)
		return &undefined;
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	return_value_tls(new IntegerPtr((INT_PTR)gw->getHWnd()));
}

Value* VP_getViewportFromScreenCoord_cf(Value** arg_list, int count)
{
	// <array>viewport.getViewportFromScreenCoord <point2> applyUIScaling:<bool> removeUIScaling:<bool> getNode:<bool>
	check_arg_count_with_keys(getViewportFromScreenCoord, 1, count);
	...
	HWND hWnd;
	INode* node = nullptr;
	BOOL getNode = bool_key_arg(getNode, tmp, FALSE);
	INode** pnode = getNode ? &node : nullptr;

	if (MAXScript_interface17->GetViewportFromScreenCoord(pt, panelIndex, vptIndex, vptID, ptLocal, hWnd, pnode))
	{
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		one_typed_value_local_tls(Array * result);
		vl.result = new Array(6);
		vl.result->append(Integer::intern(panelIndex + 1));
		vl.result->append(Integer::intern(vptIndex + 1));
		vl.result->append(Integer::intern(vptID + 1));
		vl.result->append(IntegerPtr::intern((INT_PTR)hWnd));
		if (removeUIScaling)
		{
			ptLocal.x = MAXScript::GetValueUIUnscaled(ptLocal.x);
			ptLocal.y = MAXScript::GetValueUIUnscaled(ptLocal.y);
		}
		vl.result->append(new Point2Value(ptLocal.x, ptLocal.y));
		vl.result->append(MAXNode::intern(node));
