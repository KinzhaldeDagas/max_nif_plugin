	...
	if (MAXScript_interface->GetCommandPanelTaskMode() == TASK_MODE_MODIFY)
	{
		if (is_node(obj))
		{
			...
			if ((BaseObject*)obj != MAXScript_interface->GetCurEditObject())
				throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_EDITABLEPOLY_NOT_ACTIVE), obj->GetObjectName(true));
			if (obj->GetInterface(IID_XREF_ITEM))
				throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_POLYOP_ON_NON_POLY), obj->GetObjectName(true));
...
#define def_poly_mode(_cfn, _op, _socode)						\
	...
	def_struct_primitive( _cfn,		polyOps, #_op);
