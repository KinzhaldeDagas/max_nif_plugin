...
public:
						BigMatrixRowArray(BigMatrixValue* bmv, int row);
	...
	void bmv_deleted() { bmv = nullptr; } // called by BigMatrixValue when it is deleted
...
public:
	BigMatrix			bm;
	Tab<BigMatrixRowArray*>	rowArrays;
...
public:
	...
	MAXNoteTrack(const TCHAR* iname);
...
#	define				is_notetrack(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MAXNoteTrack))
	...
	const TCHAR*		class_name() { return _T("NoteTrack"); }
...
public:
	...
	void		collect() { delete this; }
	ScripterExport void		sprin1(CharStream* s);
	const TCHAR*			class_name() { return _T("Filter"); }
...
public:
	...
	ScripterExport void		sprin1(CharStream* s);
	void		gc_trace();
	const TCHAR* class_name() const { return _T("NodeEventCallback"); }
	Value*		get_property(Value** arg_list, int count);
	Value*		set_property(Value** arg_list, int count);
