...
public:
	CMemStream( );
	~CMemStream( );
	bool OpenFile( const TCHAR *fname, unsigned int favor_type = FAVOR_ACP, int code_page = CP_ACP );
...
// ============================================================================
bool CMemStream::OpenFile( const TCHAR *fname, unsigned int favor_type, int code_page)
{
	using namespace MaxSDK::Util;

	Close();

	TextFile::Reader fileReader;
    // Original open mode was "rb"
	if (fileReader.Open(fname, favor_type | code_page, MaxSDK::Util::TextFile::Unchanged))
	{

		size_t    size;
		MaxString content       = fileReader.ReadFull();
		LPCTSTR   contentString = content.ToMCHAR(&size);

		m_start = new TCHAR[size+1];
		_tcscpy_s(m_start, size+1, contentString);
...
// ============================================================================
const TCHAR* CMemStream::GetFileName( )
{
	return m_fname;
}
...
TCHAR* CMemStream::ReadLine( )
{
	...
	// reset m_next to NULL. This will prevent a call to ReadToken from using 
	// an obsolete m_next value.
	m_next = NULL;
...
TCHAR* CMemStream::ReadBlock( const TCHAR *open, const TCHAR *close )
{
	...
	TCHAR *start = m_cur - (_tcslen(open)+1);
	m_cur = start;
...
{
public:
	IMemStream* OpenFile( const TCHAR *fname, unsigned int favor_type = FAVOR_ACP, int code_page = CP_ACP );
	IMemStream* OpenString( const TCHAR *str );
	void Close( FPInterface *memStream );

	DECLARE_DESCRIPTOR(CMemStreamMgr)
	BEGIN_FUNCTION_MAP
		FN_3(fnOpenFile, TYPE_INTERFACE, OpenFile, TYPE_FILENAME, TYPE_ENUM, TYPE_INT);
...
CMemStreamMgr memStreamMgr(
	...
	/*functions*/
		IMemStreamMgr::fnOpenFile, _T("openFile"), 0, TYPE_INTERFACE, 0, 3,
			_T("fname"), 0, TYPE_FILENAME,
			_T("favor_type"), 0, TYPE_ENUM, FavorEncodingType, f_keyArgDefault, FAVOR_ACP,
			_T("code_page"), 0, TYPE_INT, f_keyArgDefault, CP_ACP,
	...
	enums,
		FavorEncodingType, 2,
			_T("FAVOR_ACP"), FAVOR_ACP,
			_T("FAVOR_UTF8"), FAVOR_UTF8,
	p_end 
);

// ============================================================================
IMemStream* CMemStreamMgr::OpenFile( const TCHAR *fname, unsigned int favor_type, int code_page)
{
	CMemStream *memStream = new CMemStream;
	if(!memStream->OpenFile(fname, favor_type, code_page))
	{
		delete memStream;
		return NULL;
	}
