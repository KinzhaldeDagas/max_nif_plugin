...
void 
ValueToBitArray(Value* inval, BitArray &theBitArray, int maxSize, const MSTR& errmsg, int selTypesAllowed) {
	if (inval->is_kind_of(class_tag(Array))) {
		...
		for (int k = 0; k < theArray->size; k++) {
			...
			if (maxSize != -1) {
				MXS_range_check(index, 1, maxSize, errmsg);
			}
			else
		...
		for (int k = 0; k < ((BitArrayValue*)inval)->bits.GetSize(); k++) {
			if ((((BitArrayValue*)inval)->bits)[k]) {
				if (maxSize != -1) {
					MXS_range_check(k+1, 1, maxSize, errmsg);
				}
				else
	...
	else if (Number::is_number(inval)) {
		...
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_CANNOT_CONVERT_TO_BITARRAY), inval);

}

void 
ValueToBitArrayM(Value* inval, BitArray &theBitArray, int maxSize, const MSTR& errmsg, int selTypesAllowed, Mesh* mesh) {
...
INode*
_get_valid_node(MAXNode* _node, const TCHAR* errmsg) {
	if (is_node(_node))
	{	
...
set_up_spline_access(INode* node, int index)
{
	...
	MSPlugin* p = (MSPlugin*)obj->GetInterface(I_MAXSCRIPTPLUGIN);
	if (p && p->get_delegate())
	...
	MXS_range_check(index, 1, s->SplineCount(), MaxSDK::GetResourceStringAsMSTR(IDS_SHAPE_SPLINE_INDEX_OUT_OF_RANGE));
	return s;
}
...
Mesh* 
_get_meshForValue(Value* value, int accessType, ReferenceTarget** owner, const TCHAR* fn) {
	Mesh* mesh = NULL;
	if (owner) *owner = NULL;
...
getWorldStateMesh(INode* node, BOOL renderMesh)
{
	...
	const ObjectState& os = node->EvalWorldState(MAXScript_time_tls());// Get the object that is referenced at the time
	Object* ob = os.obj;
	if (!ob || (ob->SuperClassID() != GEOMOBJECT_CLASS_ID && ob->SuperClassID() != SHAPE_CLASS_ID))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_CANNOT_GET_MESH_FROM_OBJECT), node->GetName());
	Mesh* resMesh = NULL;

	if (renderMesh) {
		//Initialize this BOOL value to avoid GetRenderMesh() doesn't set it.
		//Assume obj->GetRenderMesh() returns a valid mesh without set needDel.
		//Initialize needDel to TRUE may cause crash. If the return mesh is obj's member.
		//Initialize needDel to FALSE may result memory leak. If the mesh is new created instance.
		//To avoid crash here, setting it to FALSE is relative safer choice
		BOOL needDel = FALSE; 
		NullView nullView;	// Set up a null viewport so that we get the right mesh..
		Mesh *mesh = ((GeomObject*)ob)->GetRenderMesh(MAXScript_time_tls(),node,nullView,needDel); // Type cast to a geometric object and then retriev
		if (!mesh) return NULL;
		...
		TriObject *tri = (TriObject *)ob->ConvertToType(MAXScript_time_tls(), triObjectClassID);
		if (!tri) return NULL;
		resMesh = new Mesh (tri->GetMesh());
		if (ob != tri) tri->AutoDelete();
	}
	Matrix3 tm2 = node->GetObjTMAfterWSM(MAXScript_time_tls());
	for (int i = 0; i < resMesh->getNumVerts(); i++)
		resMesh->verts[i] = resMesh->verts[i] * tm2;
/*
	if (node->GetObjectRef() == ob) {
		Matrix3 tm = node->GetObjectTM(MAXScript_time_tls());
		for (int i = 0; i < resMesh->getNumVerts(); i++)
...
*/
    ...
    // Ensure that we transform normals, as well as mesh vertex coordinates
    MeshNormalSpec* normals = resMesh->GetSpecifiedNormals();
    if (normals != nullptr)
    {
        normals->Transform(tm2);
    }
...
Mesh* _get_worldMeshForValue(Value* value, const TCHAR* fn, BOOL renderMesh) {
	Mesh* mesh = NULL;
	if (value->is_kind_of(class_tag(MAXNode))) {
...
world_to_current_coordsys(Point3& p, int mode)
{
	...
	Matrix3 m (1);
	ViewExp& view = MAXScript_interface->GetActiveViewExp();
	if ( ! view.IsAlive() )
	{
		// why are we here
	...
	{
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		m = coordsys->to_node()->GetObjectTM(MAXScript_time_tls());
	}
...
MNMesh* 
_get_polyForValue(Value* value, int accessType, ReferenceTarget** owner, const TCHAR* fn) {
	MNMesh* poly = NULL;
	if (owner) {
		*owner = NULL;
	}
	if (value->is_kind_of(class_tag(MAXNode))) {
		...
		if (accessType == MESH_WRITE_ACCESS && obj != base_obj)
			throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_CANNOT_CHANGE_POLY_WITH_MODIFIERS_PRESENT), obj->GetObjectName(true));
		else if (accessType == MESH_BASE_OBJ || accessType == MESH_WRITE_ACCESS)
		{
			if (base_obj->ClassID() != EPOLYOBJ_CLASS_ID)
				throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_POLY_OPERATION_ON_NONPOLY), obj->GetObjectName(true));
			if (owner != NULL)
				*owner = base_obj;
		...
		{
			MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
			ObjectState os = obj->Eval(MAXScript_time_tls());
			obj = (Object*)os.obj;
		...
		((((MAXObject*)value)->obj->ClassID() == EPOLYOBJ_CLASS_ID) ||
		 (((MAXObject*)value)->obj->ClassID() == Class_ID(POLYOBJ_CLASS_ID, 0))))		// Also allow operation on PolyMesh values
	{
		poly = &((PolyObject*)((MAXObject*)value)->obj)->GetMesh();
		if (owner) *owner = ((MAXObject*)value)->obj;
	}
	else {
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_POLY_OPERATION_ON_NONPOLY), value);
	}
	return poly;
}

void 
ValueToBitArrayP(Value* inval, BitArray &theBitArray, int maxSize, const MSTR& errmsg, int selTypesAllowed, MNMesh* poly) {
	if (inval == n_selection) {
		switch (selTypesAllowed)
		{
			case MESH_VERTSEL_ALLOWED:
