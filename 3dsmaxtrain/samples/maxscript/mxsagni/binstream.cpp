...
{
	s->printf(_T("<BinStream:%s>"), (const TCHAR*)name);
}
...
// ============================================================================
// BinStream fopen <String> <String> <optional: codepage:"UTF-8" | "UTF-16" | ... refer to CodePageList.h>
Value* fopen_cf(Value** arg_list, int count)
{
	check_arg_count_with_keys(fopen, 2, count);

	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(BinStream *stream);

	vl.stream = new BinStream();
	if(!vl.stream) 
		return &undefined;

	TSTR filename = arg_list[0]->to_filename();
	vl.stream->name = filename;
	AssetId assetId;
	if (IAssetManager::GetInstance()->StringToAssetId(vl.stream->name, assetId))
		vl.stream->name = IAssetManager::GetInstance()->GetAsset(assetId).GetFileName();

	vl.stream->mode = arg_list[1]->to_string();
	Value* codepage = key_arg_or_default(codepage, Integer::intern(CP_ACP));
	
	if (codepage->is_kind_of(class_tag(String)))
	{
		const TCHAR* codePageStr = codepage->to_string();

		unsigned int cp = 0;
		if (TextFile::GetCodePageNumberFromString(codePageStr, cp))
			vl.stream->codepage = cp;
		
	}
	else if (codepage->is_kind_of(class_tag(Integer)))
	{
		vl.stream->codepage = codepage->to_int();
	}

	// checkFileOpenModeValidity will throw exception if first char of mode is not 'r', 'w', or 'a'.
	checkFileOpenModeValidity(vl.stream->mode);

	if (vl.stream->mode[0] == _T('a') || vl.stream->mode[0] == _T('w') || (vl.stream->mode[0] == _T('r') && vl.stream->mode[1] == _T('+')))
	{
		MaxSDK::ISceneScriptSecurityManager* sceneScriptSecurityManager = MaxSDK::ISceneScriptSecurityManager::GetInstance();
		if (sceneScriptSecurityManager && sceneScriptSecurityManager->IsProtectedINIFile(filename))
			MAXScript::ValidateCanRunMAXScriptSystemCommand(_T("fopen"), arg_list, count);
		if (sceneScriptSecurityManager && sceneScriptSecurityManager->IsProtectedFromOpeningForWriting(filename))
			MAXScript::ValidateCanRunMAXScriptSystemCommand(_T("fopen"), arg_list, count);
	}

	// if not explicitly specifying 't' or 'b' in the mode, append 'b' to open as binary
	if (_tcschr(vl.stream->mode,_T('t')) == NULL && _tcschr(vl.stream->mode,_T('b')) == NULL)
		vl.stream->mode.append(_T("b"));

	// for a360 support - allows binary diff syncing
	if (vl.stream->mode[0] == _T('w') || vl.stream->mode[0] == _T('a') || (vl.stream->mode[0] == _T('r') && vl.stream->mode[1] == _T('+') ))
	{
		MaxSDK::Util::Path storageNamePath(vl.stream->name);
		storageNamePath.SaveBaseFile();
	}

	vl.stream->desc = _tfopen(vl.stream->name, vl.stream->mode);

	if(vl.stream->desc) 
		return_value(vl.stream);

	delete vl.stream; // LAM - defect 290069
	return &undefined;
}
...
// ============================================================================
// Boolean fseek <BinStream> <Integer64> <#seek_set | #seek_cur | #seek_end>
Value* fseek_cf(Value** arg_list, int count)
{
	...
	if(!_fseeki64(stream->desc, pos, seek_type))
		return &true_value;
...
Value* ftell_cf(Value** arg_list, int count)
{
	...
	INT64 res = _ftelli64(stream->desc);
	if(res != -1L) return_value (Integer64::intern(res));
	return &undefined;
}
...
#pragma message("UNICODE WATCH - write it as code page. - File: " __FILE__ )
Value* WriteString_cf(Value** arg_list, int count)
{
	...
	TSTR str = arg_list[1]->to_string();
	if (stream->codepage == MSDE_CP_UTF16) //UTF-16
	{
		const wchar_t* s = str.ToUTF16();
		do {
			if(fputwc(*s, stream->desc) == WEOF)
				return &false_value;
		} while(*s++);
	}
	else
	{
		const char* s = str.ToCP(stream->codepage);
		do {
...
Value* ReadByte_cf(Value** arg_list, int count)
{
	...
	{
		if(type == n_unsigned)
			return_value (Integer::intern((unsigned char)c));
		else if(type == n_signed)
			return_value (Integer::intern((signed char)c));
		else
			return_value (Integer::intern((char)c));
	}
...
Value* ReadShort_cf(Value** arg_list, int count)
{
	...
	{
		if(type == n_unsigned)
			return_value (Integer::intern((unsigned short)c));
		else if(type == n_signed)
			return_value (Integer::intern((signed short)c));
		else
			return_value (Integer::intern((short) c));
	}
...
Value* ReadLong_cf(Value** arg_list, int count)
{
	...
	{
		if(type == n_unsigned)
			return_value (Integer::intern((unsigned long)c));
		else if(type == n_signed)
			return_value (Integer::intern((signed long)c));
		else
			return_value (Integer::intern((long)c));
	}
	return &undefined;
}
...
Value* ReadLongLong_cf(Value** arg_list, int count)
{
	...
	{
		if(type == n_unsigned)
			return_value (Integer64::intern((unsigned long long)c));
		else if(type == n_signed)
			return_value (Integer64::intern((signed long long)c));
		else
			return_value (Integer64::intern((long long)c));
	}
	return &undefined;
}
...
Value* ReadString_cf(Value** arg_list, int count)
{
	...
	// this is probably pretty slow, but i need to read until a zero not a newline.
#pragma message("UNICODE WATCH - read it by code page - File: " __FILE__ )
	if (stream->codepage == MSDE_CP_UTF16)
	{
		const int kSizeOfBuf = 4096;
		wchar_t buf[kSizeOfBuf];
		for(i = 0; i < kSizeOfBuf; i++)
		{
			buf[i] = fgetwc(stream->desc);
			if(feof(stream->desc)) return &undefined;
			if(buf[i] == 0) break;
		}
		buf[i] = 0;
		return new String(TSTR::FromUTF16(buf));
	}
	else
	{
		const int kSizeOfBuf = 4096;
		char buf[kSizeOfBuf];
		for(i = 0; i < kSizeOfBuf-1; i++)
		{
		...
		buf[i] = 0;
		return new String(TSTR::FromCP(stream->codepage, buf).data());
	}
