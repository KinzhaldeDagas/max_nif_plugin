...
Value* launch_exec_cf(Value** arg_list, int count)
{
	...
	MAXScript::ValidateCanRunMAXScriptSystemCommand( _T("ShellLaunch"), arg_list, count);

	type_check(arg_list[1], String, _T("ShellLaunch [Path and Filename] [Parameters]"));
	...
	BMMSplitFilename(fileName, tpath, _countof(tpath), NULL, 0, NULL, 0);
	BMMAppendSlash(tpath);

	HINSTANCE o = ShellExecute(maxWnd,_T("open"),fileName,arg_list[1]->to_string(),tpath,SW_SHOWNORMAL);
...
Value* filterstring_cf(Value** arg_list, int count)
{
	...
	type_check(arg_list[0], String, _T("FilterString [String to filter] [Tokens]"));
	type_check(arg_list[1], String, _T("FilterString [String to filter] [Tokens]"));
	...
	const MCHAR* cmds       = arg_list[0]->to_string();
	const MCHAR* separators = arg_list[1]->to_string();
	...
	if (splitEmptyTokens)
	{
		...
		const MCHAR* nextCharIsSep = _tcschr(separators, *rPtr);
	...
	if (rPtr != eosPtr)
	{
		...
		while( token != NULL )  
		{
			...
			if (splitEmptyTokens)
			{
				...
				if (nextChar <= eosPtr)
				{
					const MCHAR *nextCharIsSep = _tcschr(separators, *nextChar);
					while (nextCharIsSep)
					{
						vl.result->append(new String(_T("")));
						++nextChar;
