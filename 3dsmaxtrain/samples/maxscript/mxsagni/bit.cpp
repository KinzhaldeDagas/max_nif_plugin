...
bit_and_cf(Value** arg_list, int count)
{
	...
	if (is_int64(arg_list[0]))
		return_value (Integer64::intern(arg_list[0]->to_int64() & arg_list[1]->to_int64()));
	else if (is_intptr(arg_list[0]))
		return_value (IntegerPtr::intern(arg_list[0]->to_intptr() & arg_list[1]->to_intptr()));
	else
		return_value (Integer::intern(arg_list[0]->to_int() & arg_list[1]->to_int()));
}
...
bit_not_cf(Value** arg_list, int count)
{
	...
	if (is_int64(arg_list[0]))
		return_value (Integer64::intern(~arg_list[0]->to_int64()));
	else if (is_intptr(arg_list[0]))
		return_value (IntegerPtr::intern(~arg_list[0]->to_intptr()));
	else
...
bit_shift_cf(Value** arg_list, int count)
{
	...
	if (is_int64(arg_list[0]))
	{
		...
		MXS_range_check(shiftCount, -int64_num_bits, int64_num_bits, MaxSDK::GetResourceStringAsMSTR(IDS_BIT_INDEX_OR_SHIFT_COUNT_OUT_OF_RANGE));
		if (shiftCount > 0)
			val <<= shiftCount;
		else if (shiftCount < 0)
			val >>= (UINT) abs(shiftCount);
		return_value (Integer64::intern(val));
	}
	else if (is_intptr(arg_list[0]))
	{
		UINT_PTR val = arg_list[0]->to_intptr();
		int intptr_num_bits = sizeof(INT_PTR) * 8;
		MXS_range_check(shiftCount, -intptr_num_bits, intptr_num_bits, MaxSDK::GetResourceStringAsMSTR(IDS_BIT_INDEX_OR_SHIFT_COUNT_OUT_OF_RANGE));
		if (shiftCount > 0)
			val <<= shiftCount;
		else if (shiftCount < 0)
			val >>= (UINT) abs(shiftCount);
	...
	else
	{
		...
		MXS_range_check(shiftCount, -int_num_bits, int_num_bits, MaxSDK::GetResourceStringAsMSTR(IDS_BIT_INDEX_OR_SHIFT_COUNT_OUT_OF_RANGE));
		if (shiftCount > 0)
			val <<= shiftCount;
		else if (shiftCount < 0)
			val >>= (UINT) abs(shiftCount);
		return_value (Integer::intern(val));
	}
}
...
bit_set_cf(Value** arg_list, int count)
{
	...
	if (is_int64(arg_list[0]))
	{
		...
		mask <<= (whichBit-1);
		int int64_num_bits = sizeof(INT64) * 8;
		MXS_range_check(whichBit, 1, int64_num_bits, MaxSDK::GetResourceStringAsMSTR(IDS_BIT_INDEX_OR_SHIFT_COUNT_OUT_OF_RANGE));
		return_value (Integer64::intern((arg_list[2]->to_bool()) ? val | mask : val & ~mask));
	}
	else if (is_intptr(arg_list[0]))
	{
		...
		mask <<= (whichBit-1);
		int intptr_num_bits = sizeof(INT_PTR) * 8;
		MXS_range_check(whichBit, 1, intptr_num_bits, MaxSDK::GetResourceStringAsMSTR(IDS_BIT_INDEX_OR_SHIFT_COUNT_OUT_OF_RANGE));
		return_value (IntegerPtr::intern((arg_list[2]->to_bool()) ? val | mask : val & ~mask));
	}
	else
	{
		...
		int int_num_bits = sizeof(int) * 8;
		MXS_range_check(whichBit, 1, int_num_bits, MaxSDK::GetResourceStringAsMSTR(IDS_BIT_INDEX_OR_SHIFT_COUNT_OUT_OF_RANGE));
		return_value (Integer::intern((arg_list[2]->to_bool()) ? val | mask : val & ~mask));
	}
}
...
bit_flip_cf(Value** arg_list, int count)
{
	...
	if (is_int64(arg_list[0]))
	{
		...
		int int64_num_bits = sizeof(INT64) * 8;
		MXS_range_check(whichBit, 1, int64_num_bits, MaxSDK::GetResourceStringAsMSTR(IDS_BIT_INDEX_OR_SHIFT_COUNT_OUT_OF_RANGE));
		return_value (Integer64::intern(val ^ mask));
	}
	else if (is_intptr(arg_list[0]))
	{
		INT_PTR val = arg_list[0]->to_intptr();
		...
		int intptr_num_bits = sizeof(INT_PTR) * 8;
		MXS_range_check(whichBit, 1, intptr_num_bits, MaxSDK::GetResourceStringAsMSTR(IDS_BIT_INDEX_OR_SHIFT_COUNT_OUT_OF_RANGE));
		return_value (IntegerPtr::intern(val ^ mask));
	}
	else
	{
		int val = arg_list[0]->to_int();
		...
		int int_num_bits = sizeof(int) * 8;
		MXS_range_check(whichBit, 1, int_num_bits, MaxSDK::GetResourceStringAsMSTR(IDS_BIT_INDEX_OR_SHIFT_COUNT_OUT_OF_RANGE));
		return_value (Integer::intern(val ^ mask));
	}
}
...
bit_charAsInt_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(val & ((sizeof(TCHAR) == 1) ? 0x0FF : 0x0FFFF)));
}
...
bit_intAsHex_cf(Value** arg_list, int count)
{
	...
	if (is_int64(arg_list[0]))
	{
		...
		if (ifUnion.i64Valpart[1] == 0)
			wsprintf (buf, _T("%xL"), ifUnion.i64Valpart[0]);
		else
			wsprintf (buf, _T("%x%08xL"), ifUnion.i64Valpart[1], ifUnion.i64Valpart[0]);
	}
	else if (is_intptr(arg_list[0]))
		wsprintf (buf, _T("%pP"), arg_list[0]->to_intptr());
	else
		wsprintf (buf, _T("%x"), arg_list[0]->to_int());
	return new String(buf);
}
...
bit_intAsFloat_cf(Value** arg_list, int count)
{
	...
	return_value (Float::intern(ifUnion.fVal));
}
...
bit_floatAsInt_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(ifUnion.iVal));
}
...
bit_int64AsDouble_cf(Value** arg_list, int count)
{
	...
	shareIntFloat ifUnion;
	ifUnion.i64Val = arg_list[0]->to_int64();
	return_value (Double::intern(ifUnion.dVal));
}
...
bit_doubleAsInt64_cf(Value** arg_list, int count)
{
	...
	return_value (Integer64::intern(ifUnion.i64Val));
}
...
bit_swapBytes_cf(Value** arg_list, int count)
{
	...
	MXS_range_check(from, 1, max_num_bytes, _T("byte index out of range: "));
	MXS_range_check(to, 1, max_num_bytes, _T("byte index out of range: "));
	from--;
	to--;
	...
	if (is_int64(arg_list[0]))
		...
		return_value (IntegerPtr::intern(ifUnion.iptrVal));
	return_value (Integer::intern(ifUnion.iVal));
}
...
bit_hexAsInt_cf(Value** arg_list, int count)
{
	...
	const TCHAR* buf = arg_list[0]->to_string();
	INT64 val = 0;
	int i = 0;
	...
	for (; i < len; i++)
	{
		...
		if (_istdigit(c))
			...
			throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_INVALID_HEX_STRING), arg_list[0]);
	}
	return_value (Integer::intern(val));
}
