...
local_visible_class_instance (CCRootClassValue, "CCRootClass");

/* ------------------- CurvePointValue class instance ----------------------- */
visible_class_instance (CurvePointValue, "ccPoint");

Value* CurvePointValueClass::apply(Value** arg_list, int count, CallContext* cc)
{
	...
	four_value_locals(a0, a1, a2, res);
	...
	vl.res = cp_val = new CurvePointValue (cp);
...
void CurvePointValue::sprin1(CharStream* s)
{
	s->printf(_T("#CCPoint(@[%g,%g])"), cp.p.x, cp.p.y);
}

Value* CurvePointValue::get_property(Value** arg_list, int count)
{  
	Value* prop = arg_list[0];
	if (curve)
	{
		if (pt_index < 0 || pt_index >= curve->GetNumPts())
			throw MaxSDK::GetResourceStringAsMSTR(IDS_CURVE_NO_LONGER_EXISTS);
		cp = curve->GetPoint(MAXScript_time(), pt_index);
	}
...
Value* CurvePointValue::set_property(Value** arg_list, int count)
{
	...
	if (prop == n_value)       cp.p = val->to_point2();
		...
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_READONLY_PROPERTY), _T("end"));
...
local_visible_class_instance (CurvePointsArray, "CurvePointsArray");
...
Value* CurvePointsArray::intern(ICurveCtl *icctl, ICurve* crv)
{
...
void CurvePointsArray::sprin1(CharStream* s)
{
	if (check_for_deletion_test())
	{
		s->printf(MaxSDK::GetResourceStringAsMSTR(IDS_CPARRAY_FOR_DELETED_CURVE).data());
		return;
	}
...
Value* CurvePointsArray::get_vf(Value** arg_list, int count)
{
	check_for_deletion();
	Value *arg, *result;
	int     index;
	...
	if (!Number::is_number(arg) || (index = arg->to_int()) < 1)
		throw TypeError (MaxSDK::GetResourceStringAsMSTR(IDS_ARRAY_INDEX_MUST_BE_VE_NUMBER_GOT), arg);
...
Value* CurvePointsArray::put_vf(Value** arg_list, int count)
{  
	check_for_deletion();
	Value *arg, *val;
	int     index;
	...
	if (!Number::is_number(arg) || (index = arg->to_int()) < 1)
		throw TypeError (MaxSDK::GetResourceStringAsMSTR(IDS_ARRAY_INDEX_MUST_BE_VE_NUMBER_GOT), arg);
	if (index > curve->GetNumPts())
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_POINT_INDEX_OUT_OF_RANGE), arg);
	if (!is_curvepoint(val))
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_INVALID_POINT_ARG), val);
...
Value* CurvePointsArray::map(node_map& m)
{
	check_for_deletion();
...
Value* CurvePointsArray::get_count(Value** arg_list, int count)
{
	check_for_deletion();
	return_value (Integer::intern(curve->GetNumPts()));
}

Value* CurvePointsArray::set_count(Value** arg_list, int count)
{
	check_for_deletion();
	throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_CANNOT_SET_ARRAY_SIZE));
	return &undefined;
}
...
Value* MAXCurve::intern(ICurve* icurve, ICurveCtl* icctl)
{
	if (icurve == NULL) 
		return &undefined;
...
void MAXCurve::sprin1(CharStream* s)
{
	AppDataChunk *ad = curve->GetAppDataChunk(CURVE_CONTROL_CLASS_ID, REF_MAKER_CLASS_ID, APPID_CURVENAME);
	const TCHAR* name = ad ? static_cast<const TCHAR*>(ad->data) : _T("");
	s->printf(_T("ccCurve%s%s"), ad?_T(" : "):_T(""), name);
}

void MAXCurve::gc_trace()
{
	MAXWrapper::gc_trace();
}
...
Value* MAXCurve::get_property(Value** arg_list, int count)
{
	Value* prop = arg_list[0]; 
	if (prop == n_name)
	{
	...
	else if (prop == n_width)        return_value (Integer::intern(width));
	else if (prop == n_disabledWidth)   return_value (Integer::intern(dwidth));
	else if (prop == n_disabledStyle)   
	{
		def_penStyle_types();
		return ConvertIDToValue(pen_styles, elements(pen_styles), dstyle);
	}
	else if (prop == n_lookupTableSize) return_value (Integer::intern(curve->GetLookupTableSize()));
	else if (prop == n_style)        
	{
		def_penStyle_types();
		return ConvertIDToValue(pen_styles, elements(pen_styles), style);
	}
	else if (prop == n_numPoints)    return_value (Integer::intern(curve->GetNumPts()));
	else if (prop == n_points)
	{
		return_value (CurvePointsArray::intern(cctl, curve));
	}
	else return Value::get_property(arg_list, count);
}

Value* MAXCurve::set_property(Value** arg_list, int count)
{  
	Value* val = arg_list[0];
	Value* prop = arg_list[1];
	...
	if (prop == n_name)
	{
		const TCHAR* data = save_string(val->to_string());
		...
		curve->AddAppDataChunk(CURVE_CONTROL_CLASS_ID, REF_MAKER_CLASS_ID, 
			APPID_CURVENAME, static_cast<int>(_tcslen(data) + 1), (void*)data);         // SR DCAST64: Downcast to 2G limit.
	}
	...
	{
		def_penStyle_types();
		style = ConvertValueToID(pen_styles, elements(pen_styles), val);
		curve->SetPenProperty(color,width,style);
	}
	else if (prop == n_disabledStyle)
	{
		def_penStyle_types();
		dstyle = ConvertValueToID(pen_styles, elements(pen_styles), val);
		curve->SetDisabledPenProperty(dcolor,dwidth,dstyle);
	}
	...
	else if (prop == n_numPoints)
	{
		...
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_READONLY_PROPERTY), _T("points"));
	else 
...
Value* MAXCurve::getValue_vf(Value** arg_list, int count)
{
	...
	return_value (Float::intern(curve->GetValue(arg_list[0]->to_timevalue(),
		arg_list[1]->to_float(), FOREVER, lookup)));
}

Value* MAXCurve::isAnimated_vf(Value** arg_list, int count)
{
	// isAnimated <ccCurve> <index>
	check_gen_arg_count(isAnimated, 2, count);
	int ind = arg_list[0]->to_int();
	if (ind < 1 || ind > curve->GetNumPts())
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_POINT_INDEX_OUT_OF_RANGE));
	return curve->IsAnimated(ind-1) ? &true_value : &false_value;
}

Value* MAXCurve::getSelectedPts_vf(Value** arg_list, int count)
{
...
Value* MAXCurve::setSelectedPts_vf(Value** arg_list, int count)
{
	...
	for(int i=1; i < count; i++)
	{
		...
		else throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_INVALID_ARG), val);
	}
...
Value* MAXCurve::setPoint_vf(Value** arg_list, int count)
{
	...
	if (!is_curvepoint(cc_val))
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_INVALID_POINT_ARG), cc_val);
	if (ind < 1 || ind > curve->GetNumPts())
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_POINT_INDEX_OUT_OF_RANGE));
...
Value* MAXCurve::getPoint_vf(Value** arg_list, int count)
{  
	...
	if (ind < 1 || ind > curve->GetNumPts())
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_POINT_INDEX_OUT_OF_RANGE));
	return new CurvePointValue(curve->GetPoint(MAXScript_time(), ind-1));

}

Value* MAXCurve::insertPoint_vf(Value** arg_list, int count)
{
...
Value* MAXCurve::deletePoint_vf(Value** arg_list, int count)
{
...
Value* MAXCurve::delete_vf(Value** arg_list, int count)
{
	// delete <ccCurve>
...
LRESULT CALLBACK CCTL_WndProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
	...
	value_local_array_tls(arg_list, count);
...
MSResourceMakerCallback::MSResourceMakerCallback(MAXCurveCtl *cc)
{
	mcc = cc;
}

MSResourceMakerCallback::~MSResourceMakerCallback()
{
}
...
void MAXCurveCtl::add_control(Rollout *ro, HWND parent, HINSTANCE hInst, int& current_y)
{
	...
	if (cctlOwner == nullptr)
		cctlOwner = new TypedSingleRefMaker<ICurveCtl>;
	cctlOwner->Set(cctl);
	ccflags = cctl->GetCCFlags();
	uiFlags = ccflags^CC_ALL_RCMENU;
	rcmFlags = ccflags&CC_ALL_RCMENU;
	MSResourceMakerCallback* resource_cb = new MSResourceMakerCallback(this);
	...
	else 
	{
		...
		width = MaxSDK::min(size.cx, pos.width); height = ro->text_height;
		top = pos.top; 
	...
	if (ro->init_values)
	{
		def_cmdMode_types()
			...
			ConvertValueToID(cmd_modes, elements(cmd_modes), commandMode);
		...
		if (uiFlagsVal != &unsupplied)
		{
			...
			if (!is_array(uiFlagsVal))
				throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_NEEDS_UIFLAGS_GOT), uiFlagsVal);
			def_uiFlag_types();
			for (int i=0; i < ((Array*)uiFlagsVal)->size; i++)
				uiFlags |= ConvertValueToID(ui_flags, elements(ui_flags), ((Array*)uiFlagsVal)->data[i]);
		}
		if (rcmFlagsVal != &unsupplied)
		{
			rcmFlags = 0;
			if (!is_array(rcmFlagsVal))
				throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_NEEDS_RCMFLAGS_GOT), rcmFlagsVal);
			def_rcmFlag_types();
			for (int i=0; i < ((Array*)rcmFlagsVal)->size; i++)
				rcmFlags |= ConvertValueToID(rcm_flags, elements(rcm_flags), ((Array*)rcmFlagsVal)->data[i]);
		}
		else
		{
...
void MAXCurveCtl::adjust_control(int& current_y)
{
	if (parent_rollout == nullptr || parent_rollout->page == nullptr)
		return;

	WORD cc_id = control_ID;
	WORD label_id = cc_id + 1;
	...
	if (label)
	{
		...
		width = MaxSDK::min(size.cx, pos.width); height = parent_rollout->text_height;
	...
	HWND frame = GetDlgItem(parent_rollout->page, cc_id);
	if (frame)
	{
		top = pos.top + label_height;
		width = pos.width; height = cc_height;
		int ui_width = GetUIScaledValue(width);
		int ui_height = GetUIScaledValue(height);

		InvalidateRect(frame, NULL, TRUE);
		SetWindowPos(frame, NULL, GetUIScaledValue(left), GetUIScaledValue(top), ui_width, ui_height, SWP_NOZORDER);
		HWND cc_hwnd = cctl->GetHWND();
		if (cc_hwnd)
			SetWindowPos(cc_hwnd, NULL, 0, 0, ui_width, ui_height, SWP_NOZORDER);
	}
}
...
MAXCurveCtl::~MAXCurveCtl()
{
	if (cctl)
	{
		cctl->RegisterResourceMaker(nullptr);
		cctl = nullptr;
	}
	if (cctlOwner)
	{
...
void MAXCurveCtl::gc_trace()
{
	RolloutControl::gc_trace();
}

BOOL MAXCurveCtl::handle_message(Rollout *ro, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	else if (message == WM_CLOSE)
	{
		...
		cctlOwner->Set(nullptr);

	}
	return FALSE;
}

Value* MAXCurveCtl::get_property(Value** arg_list, int count)
{
	Value* prop = arg_list[0];

	if (!cctl) return &undefined; // control not yet created 
	if (prop == n_visible)     return cctl->IsActive() ? &true_value : &false_value;
	else if (prop == n_numCurves) return_value (Integer::intern (cctl->GetNumCurves()));
	...
	{
		def_cmdMode_types();
		return ConvertIDToValue(cmd_modes, elements(cmd_modes), cctl->GetCommandMode());
	}
	...
	else if (prop == n_curves)
	{
		...
		return_value_tls(vl.result); //Hey!
	}
	...
	else if (prop == n_xvalue) return_value (Float::intern(xvalue));
	return RolloutControl::get_property(arg_list, count);
}

Value* MAXCurveCtl::set_property(Value** arg_list, int count)
{
	Value* val = arg_list[0];
	Value* prop = arg_list[1]; 
	...
	{
		def_cmdMode_types();
		cctl->SetCommandMode(ConvertValueToID(cmd_modes, elements(cmd_modes), val));
	}
	else if (prop == n_visible) 
	{
		if (!Number::is_integer_number(val))
			type_check(val, Boolean, _T("visible:"));
		bool visible = val->to_bool() ? true : false;
	...
	else if (prop == n_uiFlags)
	{
		uiFlags = 0;
		if (!is_array(val))
			throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_NEEDS_UIFLAGS_GOT), val);
		def_uiFlag_types();
		for (int i=0; i < ((Array*)val)->size; i++)
			uiFlags |= ConvertValueToID(ui_flags, elements(ui_flags), ((Array*)val)->data[i]);
	...
	else if (prop == n_rcmFlags)
	{
		rcmFlags = 0;
		if (!is_array(val))
			throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_NEEDS_RCMFLAGS_GOT), val);
		def_rcmFlag_types();
		for (int i=0; i < ((Array*)val)->size; i++)
			rcmFlags |= ConvertValueToID(rcm_flags, elements(rcm_flags), ((Array*)val)->data[i]);
		cctl->SetCCFlags(rcmFlags|uiFlags);
	}
	else if (prop == n_xvalue)
	{
		...
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_READONLY_PROPERTY), _T("curves"));
	else return RolloutControl::set_property(arg_list, count);
	if (cctl) cctl->Redraw();
	return val;
}

void MAXCurveCtl::set_enable()
{
...
Value* MAXCurveCtl::zoom_vf(Value** arg_list, int count)
{
	// zoom <ccCtl> #all
...
void cc_init()
{
	install_rollout_control(Name::intern(_T("CurveControl")), MAXCurveCtl::create);
