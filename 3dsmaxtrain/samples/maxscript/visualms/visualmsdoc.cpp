...
BEGIN_MESSAGE_MAP(CVisualMSDoc, CDocument)
	...
	ON_COMMAND(ID_RENAME_CAPTION_SETTINGS, OnRenameCaptionSettings)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CVisualMSDoc::SetTitle(LPCTSTR lpszTitle)
{
	...
	m_strTitle = m_sTitle + _T(" - ") + MaxSDK::GetResourceStringAsMSTR(IDS_APP_TITLE).data();
...
// ============================================================================
void CVisualMSDoc::SetUseCA(bool useCA)
{
	m_useCA = useCA;
}
bool CVisualMSDoc::GetUseCA() { return m_useCA; }
...
void CVisualMSDoc::OnEditCopy() 
{
	...
	GetTempPath(_countof(fname), fname);
	_tcscat(fname, _T("~TMPBUF.VMS"));
	SaveVMS(fname, true);
}
...
void CVisualMSDoc::OnEditClear() 
{
	...
	else
	{
		...
		while(i >= 0)
		{
			// when use visual editor for Custom Attributes, only group box & label object can be removed
			CGuiObj* obj = (CGuiObj*)m_objects[i];
			if (dynamic_cast<CGroupBoxObj*>(obj) == NULL 
				&& dynamic_cast<CLabelObj*>(obj) == NULL)
			{
				i = GetNextSel(i);
				continue;
			}
...
bool CVisualMSDoc::CheckScriptLinkage() 
{
	...
	{
		if(YesNoBoxf(MaxSDK::GetResourceStringAsMSTR(IDS_ASK_TO_UNLINK).data()) == IDYES)
		{
...
BOOL CVisualMSDoc::SaveModified() 
{
	...
	int res = MaxSDK::MaxMessageBox(GetCOREInterface()->GetMAXHWnd(), (LPCTSTR)str, MaxSDK::GetResourceStringAsMSTR(IDS_APP_TITLE), MB_YESNOCANCEL | MB_ICONEXCLAMATION);
...
BOOL CVisualMSDoc::OnNewDocument()
{
	...
	if (m_useCA)
	{
		formObj = new CFormCAObj(GetFormView(), CRect(0,0,300,300));
	}
	else
	{
		formObj = new CFormObj(GetFormView(), CRect(0,0,162,300));
	}
	ASSERT(formObj != NULL);
...
void CVisualMSDoc::OnFileOpen() 
{
	...
	CFileDialog dlg(TRUE, NULL, m_sFileName,
		...
		_T("Visual MAXScript Files (*.vms)|*.vms|All Files (*.*)|*.*||"));
...
void CVisualMSDoc::OnFileSaveAs() 
{
	...
	CFileDialog dlg(FALSE, _T("*.ms"), NULL,
		OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_ENABLESIZING,
		_T("Visual MAXScript Files (*.vms)|*.vms|MAXScript Files (*.ms;*.mcr)|*.ms; *.mcr|All Files (*.*)|*.*||"));
...
{
	bool res = false;
	TSTR fullFilePath(fname);
	StandardizeCapitalizationOfFilename(fullFilePath.dataForWrite(), static_cast<int>(fullFilePath.AllocatedChars()));

	CString ext = fname.Mid(fname.ReverseFind(_T('.')));
	if(!ext.CompareNoCase(_T(".ms")) || !ext.CompareNoCase(_T(".mcr")))
		res = ExportScript(fullFilePath.data());
	else if(!ext.CompareNoCase(_T(".vms")))
		res = SaveVMS(fullFilePath.data());

	if(res)
	{
		SetFileName(fullFilePath.data());
		SetModifiedFlag(FALSE);
	}
...
{
	TextFile::Writer fileWriter;
	Interface14 *iface = GetCOREInterface14();
	UINT codepage = iface->DefaultTextSaveCodePage(true);

    // Original open mode was "wt"
	if(!fileWriter.Open(fname, false, codepage | TextFile::Writer::WRITE_BOM))
	{
		MessageBoxf(MaxSDK::GetResourceStringAsMSTR(IDS_CANT_OPEN__FOR_WRITING).data(), fname);
		return FALSE;
	}
...
static bool at_spaced_newline(CString& str)
{
	...
	for (int i = str.GetLength()-1; i > 0; --i)
		...
		else if (!_istspace(c)) break;
	return line > 1;
}

bool CVisualMSDoc::GenScript(CString& script, const TCHAR* indentp)
{
	CString tmp;
	CString indent;
	...
	for (int i = 1; i < m_objects.GetSize(); i++)
	{
		...
		tmp = _T("");
		obj->Emit(tmp);
		if(tmp == _T("") || tmp.IsEmpty()) continue;
...
void CVisualMSDoc::GetDataForParamEditor(MaxSDK::Array<UIData4ParamEditor> &uiDatas, CString &addSource, const FPValue* fpv)
{
	CGuiObj *obj;
	for (int i = 1; i < m_objects.GetSize(); i++)
	{
		...
		// check if obj is a Custom Attribute
		CString uiName = obj->Name();
		int count = fpv->s_tab->Count();
		bool isCA = false;
		for (int i=0; i<count; i++)
		{
			CString quotedUiName = _T("'");
			quotedUiName += uiName;
			quotedUiName += _T("'");
			if ((uiName.CompareNoCase( (*(fpv->s_tab))[i] ) == 0) 
				|| quotedUiName.CompareNoCase( (*(fpv->s_tab))[i] ) == 0)
			{
				isCA = true;
				break;
			}
		...
		if (isCA)
		{
			// objData: Xpos, Ypos, width, height, caption
			UIData4ParamEditor uiData;
			uiData.uiName = (LPTSTR)(LPCTSTR)uiName;
			obj->GetShape(uiData.posX, uiData.posY, uiData.width, uiData.height);
// 			uiData.posX = obj->GetPropInt(IDX_XPOS);
// 			uiData.posY = obj->GetPropInt(IDX_YPOS);
// 			uiData.width = obj->GetPropInt(IDX_WIDTH);
// 			uiData.height = obj->GetPropInt(IDX_HEIGHT);
			CString tCaption = obj->GetPropStr(IDX_CAPTION);

			uiData.caption = (LPTSTR)(LPCTSTR)tCaption;
			uiDatas.append(uiData);
		}
		else
		{
			// if it is not CA, then add code to addSource
			CString tmp = _T("");
			obj->Emit(tmp);
			if(tmp == _T("") || tmp.IsEmpty()) continue;
			addSource += tmp;
...
bool CVisualMSDoc::SaveVMS(const CString &fname, bool bSelOnly)
{
	...
	{
		MessageBoxf(MaxSDK::GetResourceStringAsMSTR(IDS_ERROR_SAVING_FILE_).data(), (LPCTSTR)fname);
		return false;
	}
...
bool CVisualMSDoc::MergeVMS(const CString &fname, bool bPaste)
{
	...
	{
		MessageBoxf(MaxSDK::GetResourceStringAsMSTR(IDS_ERROR_LOADING_FILE_).data(), (LPCTSTR)fname);
		return false;
	}
	DeSelectAll();
	...
	{
		MessageBoxf(MaxSDK::GetResourceStringAsMSTR(IDS__IS_NOT_A_VALID_VISUAL_MAXSCRIPT_BINARY).data(), (LPCTSTR)fname);
		ar.Close();
		f.Close();
		return false;
	}
	...
	{
		MessageBoxf(MaxSDK::GetResourceStringAsMSTR(IDS__HAS_AN_UNSUPPORTED_FILE_VERSION).data(), (LPCTSTR)fname);
		ar.Close();
		f.Close();
		return false;
	}
...
// ============================================================================
void CVisualMSDoc::OnRenameCaptionSettings()
{
	// Find current selected object, just choose the first one
	bool hasSelected = false;
	CString captionText;
	CGuiObj* currentObj = NULL;
	for (int i=1; i<m_objects.GetSize(); i++)
	{
		currentObj = (CGuiObj*)m_objects[i];
		if (currentObj->IsSelected())
		{
			captionText = currentObj->Caption();
			hasSelected = true;
			break;
		}		
	}

	if (hasSelected && currentObj != NULL)
	{
		CCaptionDlg dlg(captionText);
		if (dlg.DoModal())
		{
			currentObj->SetPropStr(IDX_CAPTION, dlg.m_caption);
			currentObj->OnPropChange(IDX_CAPTION);
			Invalidate();
			SetModifiedFlag(TRUE);
		}
	}
...
static constexpr struct { const TCHAR* name; int id; } itemCodes[] =
{
...
int CVisualMSDoc::ItemIDFromName(const TCHAR* itemName)
{
