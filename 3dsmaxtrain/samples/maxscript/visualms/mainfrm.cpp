...
int CMainFrame::CreatePropBar()
{
   ...
   m_propBar.AddView(MaxSDK::GetResourceStringAsMSTR(IDS_PROPERTIES).data(), RUNTIME_CLASS(CPropView), pDoc);
   m_propBar.AddView(MaxSDK::GetResourceStringAsMSTR(IDS_EVENTS).data(), RUNTIME_CLASS(CEventView), pDoc);
...
// ============================================================================
IMPLEMENT_DYNCREATE(CMainCAFrame, CMainFrame)

	BEGIN_MESSAGE_MAP(CMainCAFrame, CMainFrame)
		//{{AFX_MSG_MAP(CMainFrame)
		ON_WM_CREATE()
		ON_COMMAND(ID_TOGGLE_GRID, OnViewGrid)
		ON_WM_GETMINMAXINFO()
...
// ============================================================================
CMainCAFrame::CMainCAFrame() 
{
	SetThreadName (-1, "Visual CA MAXScript");
}
CMainCAFrame::~CMainCAFrame() 
{
}

void CMainCAFrame::PressCtrlButton(UINT nID)
{
	m_mainToolBar.GetToolBarCtrl().CheckButton(nID, TRUE);
}

// ============================================================================
int CMainCAFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	if(CFrameWnd::OnCreate(lpCreateStruct) == -1)
		return -1;
	if(CreateMainToolBar() == -1)
	...
	LoadWindowPosition(_T("MainCAFrame"), GetSafeHwnd());

	return 0;
}

// ============================================================================
void CMainCAFrame::OnDestroy() 
{
	SaveWindowPosition(_T("MainCAFrame"), GetSafeHwnd());
...
void CMainCAFrame::OnClose()
{
	OnFileExit();
}

// ============================================================================
void CMainCAFrame::OnFileExit() 
{
	CVisualMSDoc *pDoc = CVisualMSDoc::GetDoc();
	if(pDoc)
	{
...
// ============================================================================
int CMainCAFrame::CreateMainToolBar()
{
	if(!m_mainToolBar.CreateEx(this, TBSTYLE_FLAT, WS_CHILD | WS_VISIBLE |
		CBRS_TOP | CBRS_GRIPPER | CBRS_TOOLTIPS | CBRS_FLYBY | CBRS_SIZE_DYNAMIC) ||
		!m_mainToolBar.LoadToolBar(MAKEINTRESOURCE(IDR_CATOOLS)))
	{
		TRACE0("Failed to create main toolbar for Custom Attributes\n");
		return -1;
	}
	...
	m_mainToolBar.SetButtonStyle(m_mainToolBar.CommandToIndex(ID_CTRL_SELECT), TBBS_CHECKGROUP);
	m_mainToolBar.SetButtonStyle(m_mainToolBar.CommandToIndex(ID_CTRL_LABEL), TBBS_CHECKGROUP);
	m_mainToolBar.SetButtonStyle(m_mainToolBar.CommandToIndex(ID_CTRL_GROUPBOX), TBBS_CHECKGROUP);
	m_mainToolBar.EnableDocking(CBRS_ALIGN_ANY);
...
// ============================================================================
void CMainCAFrame::OnChangeCreateMode(UINT nID)
{
	CVisualMSDoc *pDoc = (CVisualMSDoc*)GetActiveDocument();
	ASSERT_VALID(pDoc);
	pDoc->SetCreateMode(nID);
...
// ============================================================================
LRESULT CMainCAFrame::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	switch(message)
	{
	case WM_CREATEOBJ:
