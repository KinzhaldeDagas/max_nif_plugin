...
class CRubberRect : public CRectTracker
{
	void AdjustRect(int nHandle, LPRECT lpRect)
	{
		...
		SetStatus(PANE_SIZE, _T("%d x %d"), rect.right-rect.left, rect.bottom-rect.top);
...
void CFormEd::SetUseCA(bool useCA)
{
	m_useCA = useCA;
}

bool CFormEd::GetUseCA()
{
	return m_useCA;
}
...
void CFormEd::DrawRectTrackers(CDC *pDC)
{
	...
	if (m_useCA)
	{
		CRect rect;
		GetClientRect(rect);
		VertDotLine(pDC, 158, 0, rect.Height());
	}
}
...
void CFormEd::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	SetStatus(PANE_POS, _T("%d, %d"), point.x, point.y);
	...
	: CGuiObj(pParent)
{
	...
	Class() = _T("rollout");
	Name()  = MaxSDK::GetResourceStringAsMSTR(IDS_UNNAMED_ROLLOUT).data();
	Caption() = MaxSDK::GetResourceStringAsMSTR(IDS_UNTITLED).data();
...
// ============================================================================
CFormCAObj::CFormCAObj(CWnd *pParent, const CRect &rect)
	: CFormObj(pParent, rect)
{
	m_initWidth = rect.Width();
	m_formEd.SetUseCA(true);
}

// ============================================================================
void CFormCAObj::SetRect(const CRect &rect)
{
	CRect r;
	r = rect;
	r.left = c_formOffset;
...
// ============================================================================
int CFormCAObj::OnPropChange(int idx)
{
	CRect rect;
	switch(idx)
	{
