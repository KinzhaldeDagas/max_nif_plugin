...
void CCtrlSizer::AdjustRect(int nHandle, LPRECT lpRect)
{
	...
	if(pDoc)
	{
		...
		if(m_pObj)
		{
			...
			if(change.left)
			{
				m_pObj->m_properties[IDX_XPOS] = int(rect.left);
				pDoc->UpdateAllViews(NULL, HINT_CHANGEPROP, (CObject*)IDX_XPOS);
			}
			if(change.top)
			{
				m_pObj->m_properties[IDX_YPOS] = int(rect.top);
				pDoc->UpdateAllViews(NULL, HINT_CHANGEPROP, (CObject*)IDX_YPOS);
			}
			if(change.right)
			{
				m_pObj->m_properties[IDX_WIDTH] = int(rect.right);
				pDoc->UpdateAllViews(NULL, HINT_CHANGEPROP, (CObject*)IDX_WIDTH);
			}
			if(change.bottom)
			{
				m_pObj->m_properties[IDX_HEIGHT] = int(rect.bottom);
				pDoc->UpdateAllViews(NULL, HINT_CHANGEPROP, (CObject*)IDX_HEIGHT);
			}
		}
	}

	SetStatus(PANE_POS, _T("%d, %d"), rect.left, rect.top);
	SetStatus(PANE_SIZE, _T("%d x %d"), rect.right, rect.bottom);
...
void CGuiObj::Emit(CString &out)
{
	...
	{
		CProperty& property = m_properties[i];
		if ((property.m_flags & FLAG_DONTEMIT) || property.HasDefaultValue())
			continue;

		CString sVal = GetPropStr(i);
		if (property.m_type == PROP_STRING)
		{
			// replace '\\' with "\\"
			sVal.Replace(_T("\\"), _T("\\\\"));
			// replace '\r' with "\r"
			sVal.Replace(_T("\r"), _T("\\r"));
			// replace '\n' with "\n"
			sVal.Replace(_T("\n"), _T("\\n"));
			// replace '\t' with "\t"
			sVal.Replace(_T("\t"), _T("\\t"));
		...
		if (!sVal.IsEmpty() && sVal.Compare(_T("#()")))
			out += _T(" ") + property.m_name + _T(":") + sVal;
	}

	// when use absolute layout, make sure object is aligned to left
	out += _T(" align:#left");
	out += m_comment;
}
...
int CGuiObj::FindProperty(const TCHAR* propName)
{
...
int CGuiObj::FindHandler(const TCHAR* eventName)
{
...
void CGuiObj::SetPropery(const TCHAR* propName, float f)
{
...
void CGuiObj::SetPropery(const TCHAR* propName, int i)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
void CGuiObj::SetPropery(const TCHAR* propName, bool b)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
void CGuiObj::SetPropery(const TCHAR* propName, const TCHAR* s)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
void CGuiObj::SetProperyLit(const TCHAR* propName, const TCHAR* s)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
void CGuiObj::SetPropery(const TCHAR* propName, Point2 p2)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());

	if (_tcsicmp(propName, _T("pos")) == 0 || _tcsicmp(propName, _T("position")) == 0)
	{
...
void CGuiObj::SetPropery(const TCHAR* propName, Point3 p3)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
void CGuiObj::SetPropery(const TCHAR* propName, Tab<const TCHAR*>* sa)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
void CGuiObj::SetHandler(const TCHAR* eventName, const TCHAR* source, int arg_start, int arg_end, int body_start, int body_end)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	// if (h >= 0)
	{
		...
		const_cast<TCHAR*>(source)[body_end] = _T('\0');
		CString body = &source[body_start], &trimBody = m_handlers[h].m_text;
		const_cast<TCHAR*>(source)[body_end] = c;
		...
		// args do
		c = source[arg_end];
		const_cast<TCHAR*>(source)[arg_end] = _T('\0');
		m_handlers[h].m_args = &source[arg_start];
		m_handlers[h].m_args.TrimRight();
		const_cast<TCHAR*>(source)[arg_end] = c;
	}
}

void    
CGuiObj::AddComment(const TCHAR* code, int src_from, int src_to)
{
	if (src_from < src_to)
	{
...
// ------- CCodeObj ----------------------------
CCodeObj::CCodeObj(const TCHAR* code) : CGuiObj(NULL) // : code(code)
{
	m_createID = ID_CODE;
	m_selected = FALSE;
