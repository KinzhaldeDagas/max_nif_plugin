...
void CProperty::operator =(const CString &src)
{
	...
	case PROP_LITERAL:
		...
		break;
	case PROP_INTEGER:
		if (_stscanf(src, _T(" %d "), &m_integer) != 1)
			m_flags |= FLAG_BADVALUE;
		break;
	case PROP_REAL:
		if (_stscanf(src, _T(" %g "), &m_real) != 1)
			m_flags |= FLAG_BADVALUE;
		break;
	case PROP_BOOLEAN:
		if (CString(_T("true")) == src)
			m_boolean = true;
		else if (CString(_T("false")) == src)
			m_boolean = false;
		else
			m_flags |= FLAG_BADVALUE;
		break;
	case PROP_POINT2:
		if (_stscanf(src, _T(" [ %g , %g ] "), &m_point2.x, &m_point2.y) != 2)
			m_flags |= FLAG_BADVALUE;
		break;
	case PROP_POINT3:
		if (_stscanf(src, _T(" [ %g , %g , %g ] "), &m_point3.x, &m_point3.y, &m_point3.z) != 3)
			m_flags |= FLAG_BADVALUE;
		break;
	}
...
// ============================================================================
void CProperty::operator =(LPCTSTR lpcstr)
{
	CString src = lpcstr;
	*this = src;
...
void CProperty::operator =(Point3 p3)
{
	...
	case PROP_LITERAL:
	case PROP_STRING:
		m_string.Format(_T("[%g,%g,%g]"), p3.x, p3.y, p3.z);
		break;
	case PROP_POINT3:
		m_point3 = p3;
		break;
...
void CProperty::operator =(Point2 p2)
{
	...
	case PROP_LITERAL:
	case PROP_STRING:
		m_string.Format(_T("[%g,%g]"), p2.x, p2.y);
		break;
	case PROP_POINT2:
		m_point2 = p2;
		break;
...
CProperty::operator LPCTSTR()
{
	...
	{
	case PROP_INTEGER:
		m_string.Format(_T("%d"), m_integer);
		break;
	case PROP_REAL:
		m_string.Format(_T("%g"), m_real);
		break;
	case PROP_BOOLEAN:
		if(m_boolean)
			m_string = _T("true");
	...
	case PROP_OPTION:
		...
		break;
	case PROP_POINT2:
		m_string.Format(_T("[%g,%g]"), m_point2.x, m_point2.y);
		break;
	case PROP_POINT3:
		m_string.Format(_T("[%g,%g,%g]"), m_point3.x, m_point3.y, m_point3.z);
		break;
	case PROP_STRING:
//		m_string.Format("\"%s\"", m_string);  // TODO: WHY WAS THIS OUTPUTTING QUOTES ???
		break;
	case PROP_NAME:
		m_string.Format(_T("#%s"), m_string);
		break;
	}
	return m_string;
}
