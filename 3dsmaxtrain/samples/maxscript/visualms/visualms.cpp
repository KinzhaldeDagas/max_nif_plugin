...
// ============================================================================
void VertDotLine(CDC *pDC, int x, int y, int h)
{
	pDC->SelectObject(new CPen(PS_DOT, 0, GetSysColor(COLOR_3DSHADOW)));
	pDC->MoveTo(x, y);
	pDC->LineTo(x, y+h);
...
// ============================================================================
void SetStatus(int pane, const TCHAR *fmt, ...)
{
	static TCHAR msg[1024];
	va_list arglist;
	...
	if(fmt == NULL)
	{
		pWnd->SendMessage(WM_SETSTATUS, (WPARAM)pane, (LPARAM)(_T("")));
		return;
	}
...
// ============================================================================
void  MessageBoxf(const TCHAR *fmt, ...)
{
	static TCHAR msg[1024];
	va_list arglist;
	...
	MaxSDK::MaxMessageBox(NULL, msg, NULL, MB_OK);
}

// ============================================================================
int YesNoBoxf(const TCHAR *fmt, ...)
{
	static TCHAR msg[1024];
	va_list arglist;
	...
	return MaxSDK::MaxMessageBox(NULL, msg, NULL, MB_YESNO);
}

// ============================================================================
int YesNoCancelBoxf(const TCHAR *fmt, ...)
{
	static TCHAR msg[1024];
	va_list arglist;
	...
	return MaxSDK::MaxMessageBox(NULL, msg, NULL, MB_YESNOCANCEL | MB_ICONEXCLAMATION);
}
...
void SaveWindowPosition( LPCTSTR keyName, HWND hWnd )
{
	...
	CString fname = GetCOREInterface()->GetDir(APP_PLUGCFG_DIR).data();
	fname += _T("\\visualms.ini");

	MaxSDK::Util::WritePrivateProfileString(_T("WindowPositions"), keyName, (LPCTSTR)posStr, (LPCTSTR)fname);
}
...
void LoadWindowPosition( LPCTSTR keyName, HWND hWnd )
{
	...
	MaxSDK::Util::GetPrivateProfileString(_T("WindowPositions"), keyName, _T(""), buf, 100, (LPCTSTR)fname);

	RECT rect;
	if( _stscanf(buf, _T("%i %i %i %i"), &rect.left, &rect.top, &rect.right, &rect.bottom) == 4 )
	{
		WINDOWPLACEMENT wp;
		wp.length = sizeof(WINDOWPLACEMENT);
