	...
	: CGuiObj(pParent)
{
	...
	m_properties[IDX_XPOS] = int(rect.left);
	m_properties[IDX_YPOS] = int(rect.top);
	m_properties[IDX_WIDTH] = int(rect.right);
	m_properties[IDX_HEIGHT] = int(rect.bottom);
...
void CStdObj::SetRect(const CRect &rect)
{
	...
	m_properties[IDX_XPOS] = int(r.left);
	m_properties[IDX_YPOS] = int(r.top);
	m_properties[IDX_WIDTH] = int(r.right);
	m_properties[IDX_HEIGHT] = int(r.bottom);
...
//	idx_height = m_properties.Add(CProperty(_T("height"), CString("")));
	idx_title = m_properties.Add(CProperty(_T("title"), CString(MaxSDK::GetResourceStringAsMSTR(IDS_CHOOSE_A_COLOR).data())));
...
CString CComboBoxObj::GetPropStr(int idx) const
{
	...
	{
		CString str;
		str.Format(_T("%d"), GetPropInt(idx));
		return str;
	}
...
// ============================================================================
void CSpinnerObj::SetPropery(const TCHAR* propName, int i)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());

	int j = FindProperty(propName);
	
	// Width of CSpinnerObj should contains the length of label
	// However, in Maxscript Engine, the length does not contain it
	// So, I add this length when initialize this property
	if (j == IDX_WIDTH)
	{
		i += m_textWidth;
	}
...
void CSpinnerObj::SetShape(int left, int top, int width, int height)
{
	SetPropInt(IDX_XPOS, left);
	SetPropInt(IDX_YPOS, top);
	SetPropInt(IDX_WIDTH, width + m_textWidth);
...
void CSpinnerObj::GetShape(int& left, int& top, int& width, int& height)
{
	left = GetPropInt(IDX_XPOS);
	top  = GetPropInt(IDX_YPOS);
	width  = GetPropInt(IDX_WIDTH) - m_textWidth;
...
void CSpinnerObj::SetRect(const CRect &rect)
{
	...
	m_labelWnd.MoveWindow(r.left, r.top, textWidth-2, r.bottom, TRUE);
	m_wnd.MoveWindow(r.left+textWidth, r.top, r.right-textWidth-13, r.bottom, TRUE);
	m_spn.MoveWindow(r.left+r.right-13, r.top, 10, 16, TRUE);
...
int CSpinnerObj::OnPropChange(int idx)
{
	...
	{
		m_labelWnd.SetWindowText(Caption());
		m_textWidth =  GetTextWidth(&m_labelWnd, Caption());
		CRect rect;
		GetRect(rect);
		SetRect(rect);
		return 1;
...
void CSpinnerObj::Emit(CString &out)
{
	...
	// The difference from CGuiObj::Emit() is width of spinner contains the length of label
	// However, in Maxscript Engine, the length does not contain it
	// So we need to subtract label length from m_properties[IDX_WIDTH] and make the output string
	// consistent with MaxScript Engine
	int width = GetPropInt(IDX_WIDTH);
	CString wVal;
	wVal.Format(_T("%d"),width - m_textWidth);
	out += _T(" ") + m_properties[IDX_WIDTH].m_name + _T(":") + wVal;

	for (int i = IDX_WIDTH+1; i < m_properties.GetSize(); i++)
	{
