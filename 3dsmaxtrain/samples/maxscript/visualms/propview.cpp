...
int CPropView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	GetListCtrl().InsertColumn(0, MaxSDK::GetResourceStringAsMSTR(IDS_PROPERTY).data(), LVCFMT_LEFT, 80, -1);
	GetListCtrl().InsertColumn(1, MaxSDK::GetResourceStringAsMSTR(IDS_VALUE).data(), LVCFMT_LEFT, 80, -1);
	...
	CFont*  parentFont = GetParent()->GetFont();
	LOGFONT fontInfo;

	parentFont->GetLogFont(&fontInfo);
	
	_tcscpy(fontInfo.lfFaceName, _T("Arial"));
