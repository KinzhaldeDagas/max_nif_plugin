...
public:
	...
	CVMSDocTemplate(
		...
		bool useCA);	// whether this doc template is used for Custom Attributes
	~CVMSDocTemplate();
...
// ============================================================================
class CVisualMSCAThread : public CVisualMSThread
{
	DECLARE_DYNCREATE(CVisualMSCAThread)

protected:
	CVisualMSCAThread();           // protected constructor used by dynamic creation

public:
	static CVisualMSCAThread* Create(bool bVisible = true);

	virtual BOOL InitInstance();
	virtual int ExitInstance();

protected:
	virtual ~CVisualMSCAThread();
