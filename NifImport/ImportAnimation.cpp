...
FPValue GetScriptedProperty(FPValue& thing, TCHAR* propName) {
	...
	if ((thing.i != 0) && (!isUndefined)) try { //Safe handling for nullptr
...
Value* GetFunction(Value* thing, TCHAR* funcName) {
	...
	if ((thing != 0)) try { //Safe handling for nullptr
...
static FPValue myAddNewNoteKey(Value* noteTrack, int frame)
{
	...
	try {
		...
		// try it again.
		while (vl.fn != nullptr && is_globalthunk(vl.fn))
			vl.fn = static_cast<GlobalThunk*>(vl.fn)->cell;
		while (vl.fn != nullptr && is_constglobalthunk(vl.fn))
		...
		if (vl.fn != nullptr && vl.fn->_is_function()) {
			Value* args[4];
			...
			// If the result isn't nullptr, try to convert it to a material.
			...
			if (INode *n = GetNode(target)) {
				...
				for (vector<StringKey>::iterator itr = textKeys.begin(); itr != textKeys.end(); ++itr) {
					...
					if (wildmatch("start*", (*itr).data)) {
						...
						NoteKey *key = new NoteKey(t, A2THelper(buffer, line.c_str(), _countof(buffer)), 0);
						nt->keys.Append(1, &key);
					}
					else {
						NoteKey *key = new NoteKey(t, A2THelper(buffer, (*itr).data.c_str(), _countof(buffer)), 0);
						nt->keys.Append(1, &key);
					}
				}
				...
				TSTR script;
				script +=
					TEXT("fn getActorManager obj = (\n")
					TEXT("   local nt = undefined\n")
					TEXT("   n = numNoteTracks obj\n")
					TEXT("   for i = 1 to n do (\n")
					TEXT("      local nt = getNoteTrack obj i\n")
					TEXT("      if (nt.name == \"ActorManager\") then ( return nt )\n")
					TEXT("   )\n")
					TEXT("   nt = notetrack \"ActorManager\"\n")
					TEXT("   addNoteTrack obj nt\n")
					TEXT("   return nt\n")
					TEXT(")\n")
					TEXT("fn addNoteKey nt frame tag = (\n")
					TEXT("   local Key = addNewNoteKey nt.keys frame\n")
					TEXT("   Key.value = tag\n")
					TEXT(")\n");

				script += FormatText(TEXT("nt = getActorManager $'%s'\n"), target.c_str());

				for (vector<StringKey>::iterator itr = textKeys.begin(); itr != textKeys.end(); ++itr) {
					TimeValue t = TimeToFrame(time + (*itr).time);

					if (wildmatch(TEXT("start*"), (*itr).data)) {
						stringlist args = TokenizeCommandLine((*itr).data.c_str(), true);
						if (args.empty()) continue;
						bool hasName = false;
						bool hasLoop = loop;
						...
						if (!hasName) {
							string name = FormatString(TEXT("EMPTY_SEQUENCE_AT_%df"), int(t * FramesPerSecond / TicksPerFrame));
							args.push_back(TEXT("-name"));
							args.push_back(name);
						}
						if (!hasLoop) {
							args.push_back(TEXT("-loop"));
						}
						if (!hasAt) {
							args.push_back(TEXT("-at"));
							args.push_back(TEXT("y"));
						}
						tstring line = JoinCommandLine(args);
						script += FormatText(TEXT("addNoteKey nt (%d/ticksPerFrame) \"%s\"\n"), t, line.c_str());
					}
					else {
						script += FormatText(TEXT("addNoteKey nt (%d/ticksPerFrame) \"%s\"\n"), t, (*itr).data.c_str());
					}
				...
				//ExecuteMAXScriptScript(script, TRUE, nullptr);
#endif
			}
		}
	...
	)
{
	...
	if (ImpNode *node = ni.i->CreateNode())
	{
		...
		tnode->SetUserPropBool(TEXT("np_ignore"), TRUE);
